...
static TR::RealRegister::RegNum choose_rX(TR::Instruction *,
                                          TR::RealRegister *);

OMR::Power::MemoryReference::MemoryReference(TR::CodeGenerator *cg)
    : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _modBase(NULL), _length(0), _staticRelocation(NULL),
      _unresolvedSnippet(NULL), _conditions(NULL), _flag(0) {
  _symbolReference =
      new (cg->trHeapMemory()) TR::SymbolReference(cg->comp()->getSymRefTab());
  _offset = _symbolReference->getOffset();
}

OMR::Power::MemoryReference::MemoryReference(TR::Register *br, TR::Register *ir,
                                             uint8_t len, TR::CodeGenerator *cg)
    : _baseRegister(br), _baseNode(NULL), _indexRegister(ir), _indexNode(NULL),
      _modBase(NULL), _unresolvedSnippet(NULL), _staticRelocation(NULL),
      _conditions(NULL), _length(len), _flag(0) {
  _symbolReference =
      new (cg->trHeapMemory()) TR::SymbolReference(cg->comp()->getSymRefTab());
  _offset = _symbolReference->getOffset();
}

OMR::Power::MemoryReference::MemoryReference(TR::Register *br, int32_t disp,
                                             uint8_t len, TR::CodeGenerator *cg)
    : _baseRegister(br), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _modBase(NULL), _unresolvedSnippet(NULL),
      _staticRelocation(NULL), _conditions(NULL), _length(len), _offset(disp),
      _flag(0) {
  _symbolReference =
      new (cg->trHeapMemory()) TR::SymbolReference(cg->comp()->getSymRefTab());
}
...
OMR::Power::MemoryReference::MemoryReference(TR::Node *rootLoadOrStore,
                                             uint32_t len,
                                             TR::CodeGenerator *cg)
  ...
  TR::MethodSymbol *methodSymbol =
      rootLoadOrStore->getSymbol()->getMethodSymbol();
  if (rootLoadOrStore->getOpCode().isIndirect()) {
    ...
    if (!ref->isUnresolved() && isLocalObject) {
      ...
      if (ref->isUnresolved()) {
        // Stack late mapping forces us to evaluate local object's base into a
        // register rather than getting the stack pointer as the base which is
        // not correct.
        if (isLocalObject)
          cg->evaluate(base);

        self()->setUnresolvedSnippet(
            new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(
                cg, rootLoadOrStore, ref, isStore, false));
    ...
    if (symbol->isStatic()) {
      ...
      if (!symbol
               ->isMethodMetaData()) { // must be either auto or parm or error.
        _baseRegister = cg->getStackPointerRegister();
      } else {
        _baseRegister = cg->getMethodMetaDataRegister();
      }
...
OMR::Power::MemoryReference::MemoryReference(TR::Node *node,
                                             TR::SymbolReference *symRef,
                                             uint32_t len,
                                             TR::CodeGenerator *cg)
    : _baseRegister(NULL), _indexRegister(NULL), _unresolvedSnippet(NULL),
      _modBase(NULL), _flag(0), _baseNode(NULL), _indexNode(NULL),
      _symbolReference(symRef), _offset(0), _conditions(NULL), _length(len),
...
OMR::Power::MemoryReference::MemoryReference(TR::Node *node,
                                             ...
                                             int32_t displacement, uint32_t len,
                                             TR::CodeGenerator *cg) {
  ...
  TR_ASSERT(mr.getStaticRelocation() == NULL,
            "Relocated memory reference should not be re-used.");

  if (mr.getUnresolvedSnippet() != NULL) {
    _unresolvedSnippet = new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(
        cg, node, _symbolReference,
...
bool OMR::Power::MemoryReference::useIndexedForm() {
  return ((_indexRegister == NULL && !self()->isUsingDelayedIndexedForm())
              ? false
              : true);
}
...
int32_t OMR::Power::MemoryReference::getTOCOffset() {
  ...
  if (self()->isUsingStaticTOC()) {
    return _symbolReference->getSymbol()->getStaticSymbol()->getTOCIndex() *
           sizeof(intptrj_t);
  }
  return 0;
}

void OMR::Power::MemoryReference::addToOffset(TR::Node *node, intptrj_t amount,
                                              TR::CodeGenerator *cg) {
  ...
  if (displacement < LOWER_IMMED || displacement > UPPER_IMMED) {
    ...
    if (_baseRegister != NULL && self()->isBaseModifiable())
      ...
      if (_baseRegister && _baseRegister->containsInternalPointer()) {
        ...
        newBase->setPinningArrayPointer(
            _baseRegister->getPinningArrayPointer());
      }
    }
    ...
    if (_baseRegister != NULL) {
      if (node->getOpCode().isLoadConst() && node->getRegister()) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, newBase,
                                    _baseRegister, node->getRegister());
      } else {
        if (TR::Compiler->target.is64Bit() &&
            (upper < LOWER_IMMED || upper > UPPER_IMMED)) {
...
void OMR::Power::MemoryReference::forceIndexedForm(TR::Node *node,
                                                   TR::CodeGenerator *cg,
...
void OMR::Power::MemoryReference::bookKeepingRegisterUses(
...
void OMR::Power::MemoryReference::populateMemoryReference(
    TR::Node *subTree, TR::CodeGenerator *cg) {
  ...
  // Skip sign extension if the offset is known to be >= 0; this is only safe to
  // do if the child is the result of a load, otherwise the upper 32 bits may be
  // undefined
  if (subTree->getOpCodeValue() == TR::i2l && subTree->isNonNegative() &&
      subTree->getFirstChild()->getOpCode().isLoadVar() &&
      subTree->getReferenceCount() == 1 && subTree->getRegister() == NULL) {
    if (performTransformation(cg->comp(),
                              "O^O PPC Evaluator: Skip unecessary sign "
                              "extension on index [%p].\n",
                              subTree)) {
      static bool dontSkipIndexSignExt =
          feGetEnv("TR_dontSkipIndexSignExt") != NULL;
  ...
  if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL) {
    ...
    if (subTree->getOpCode().isArrayRef() ||
      ...
      } else if (integerChild->getEvaluationPriority(cg) >
                     addressChild->getEvaluationPriority(cg) &&
                 !(subTree->getOpCode().isArrayRef() &&
      ...
      if (TR::Compiler->target.is64Bit()) { // 64-bit
        intptrj_t amount =
        ...
        if (subTree->getFirstChild()->getOpCode().isLoadConst())
          self()->addToOffset(
              subTree, subTree->getFirstChild()->getLongInt() << amount, cg);
        else
        // lshl
        //    i2l
        //       iconst
        //    iconst(lconst)
        {
          self()->addToOffset(
              subTree,
              subTree->getFirstChild()->getFirstChild()->getInt() << amount,
              cg);
          cg->decReferenceCount(subTree->getFirstChild()->getFirstChild());
        }
      } else
        self()->addToOffset(subTree,
                            subTree->getFirstChild()->getInt()
                                << subTree->getSecondChild()->getInt(),
                            cg);
      ...
      if (symbol->isRegisterMappedSymbol()) {
        if (_baseRegister != NULL) {
          TR::Register *tempReg;
          if (!symbol->isMethodMetaData()) { // must be either auto or parm or
                                             // error.
      ...
      if (TR::Compiler->target.is32Bit() || !symbol->isConstObjectRef())
        self()->addToOffset(subTree, subTree->getSymbolReference()->getOffset(),
                            cg);
      // TODO: aliasing sets?
      cg->decReferenceCount(subTree); // need to decrement ref count because
               ...
               subTree->getOpCodeValue() ==
                   TR::iconst || // subTree->getOpCode().isLoadConst ?
      ...
      if (_baseRegister != NULL) {
        self()->consolidateRegisters(cg->evaluate(subTree), subTree,
                                     cg->canClobberNodesRegister(subTree), cg);
      } else {
        _baseRegister = cg->evaluate(subTree);
        _baseNode = subTree;
...
void OMR::Power::MemoryReference::consolidateRegisters(TR::Register *srcReg,
                                                       TR::Node *srcTree,
                                                       bool srcModifiable,
                                                       TR::CodeGenerator *cg) {
  ...
  TR::Node *tempNode =
      (srcTree == NULL) ? cg->getAppendInstruction()->getNode() : srcTree;
  ...
  if (self()->getUnresolvedSnippet() != NULL) {
    ...
    if (_indexRegister != NULL) {
      ...
      else if ((srcReg && (srcReg->containsCollectedReference() ||
                           srcReg->containsInternalPointer())) ||
               (_indexRegister &&
      ...
      generateTrg1Src2Instruction(cg, TR::InstOpCode::add, tempNode,
                                  tempTargetRegister, _indexRegister, srcReg);
    ...
    if (_indexRegister != NULL) {
      ...
      else if ((_baseRegister && (_baseRegister->containsCollectedReference() ||
      ...
      self()->setBaseModifiable();
               ...
               (self()->getOffset(*comp) != 0 || self()->hasDelayedOffset())) {
      if (self()->isBaseModifiable())
        tempTargetRegister = _baseRegister;
        ...
        tempTargetRegister = srcReg;
                           ...
                           srcReg->containsInternalPointer())) ||
               (_baseRegister && (_baseRegister->containsCollectedReference() ||
                                  _baseRegister->containsInternalPointer()))) {
        if (srcTree != NULL && srcTree->isInternalPointer() &&
            srcTree->getPinningArrayPointer()) {
          tempTargetRegister = cg->allocateRegister();
      ...
      generateTrg1Src2Instruction(cg, TR::InstOpCode::add, tempNode,
                                  tempTargetRegister, _baseRegister, srcReg);
      if (_baseRegister != tempTargetRegister) {
        self()->decNodeReferenceCounts(cg);
        _baseNode = NULL;
      }
...
void OMR::Power::MemoryReference::assignRegisters(
  ...
  if (_modBase != NULL) {
    ...
    _conditions->assignPostConditionRegisters(currentInstruction,
                                              _modBase->getKind(), cg);
    _conditions->assignPreConditionRegisters(currentInstruction->getPrev(),
                                             _modBase->getKind(), cg);
  ...
  if (_baseRegister != NULL) {
    ...
    if ((assignedBaseRegister != NULL) &&
      ...
      if ((alternativeRegister =
               machine->findBestFreeRegister(currentInstruction, TR_GPR, true,
                                             false, _baseRegister)) == NULL) {
        alternativeRegister = machine->freeBestRegister(
            currentInstruction, _baseRegister, NULL, true);
      }
      machine->coerceRegisterAssignment(
          currentInstruction, _baseRegister,
          toRealRegister(alternativeRegister)->getRegisterNumber());
      assignedBaseRegister = alternativeRegister;
    } else {
      if (assignedBaseRegister == NULL) {
        assignedBaseRegister =
            machine->assignOneRegister(currentInstruction, _baseRegister, true);
      }
    }
  ...
  if (_indexRegister != NULL) {
    ...
    if (assignedIndexRegister == NULL) {
      assignedIndexRegister =
          machine->assignOneRegister(currentInstruction, _indexRegister, false);
    }
...
void OMR::Power::MemoryReference::mapOpCode(
...
uint8_t *OMR::Power::MemoryReference::generateBinaryEncoding(
    TR::Instruction *currentInstruction, uint8_t *cursor,
    TR::CodeGenerator *cg) {
  ...
  base = (self()->getBaseRegister() == NULL)
             ? cg->machine()->getRealRegister(TR::RealRegister::gr0)
             : toRealRegister(self()->getBaseRegister());
  index = (self()->getIndexRegister() == NULL)
              ? NULL
              : toRealRegister(self()->getIndexRegister());

  if (TR::Compiler->target.is64Bit() && self()->isTOCAccess()) {
    uint32_t preserve = *wcursor;
    TR::RealRegister *target =
        toRealRegister(currentInstruction->getMemoryDataRegister());
    ...
    if (self()->getUnresolvedSnippet() != NULL) {
      ...
      cg->addRelocation(
          new (cg->trHeapMemory()) TR::LabelRelative24BitRelocation(
              cursor, getUnresolvedSnippet()->getSnippetLabel()));
      *wcursor = 0x48000000; // b SnippetLabel;
      ...
      if (displacement != PTOC_FULL_INDEX) {
        if (displacement < LOWER_IMMED || displacement > UPPER_IMMED) {
          *wcursor = preserve | lower;
          toRealRegister(getModBase())
              ->setRegisterFieldRA(wcursor); // mb is the modified base
          cursor += PPC_INSTRUCTION_LENGTH;
        }
      } else {
      ...
      uint64_t addr =
          symbol->isStatic()
              ? (uint64_t)symbol->getStaticSymbol()->getStaticAddress()
              : (uint64_t)symbol->getMethodSymbol()->getMethodAddress();
      if (displacement != PTOC_FULL_INDEX) {
        if (!self()->getSymbolReference()->isUnresolved() &&
            TR_PPCTableOfConstants::getTOCSlot(displacement) == 0) {
        ...
        *wcursor = preserve | lower;
        ...
        if (comp->compileRelocatableCode() && symbol->isStatic() &&
          ...
          if (comp->getOption(TR_UseSymbolValidationManager)) {
            ...
            cg->addExternalRelocation(
                new (cg->trHeapMemory()) TR::ExternalRelocation(
                    cursor, (uint8_t *)recordInfo, TR_ClassAddress, cg),
                __FILE__, __LINE__, node);
          }
  ...
  if (self()->getUnresolvedSnippet() != NULL) {
    ...
    getUnresolvedSnippet()->setAddressOfDataReference(cursor);
    getUnresolvedSnippet()->setMemoryReference(self());
    cg->addRelocation(new (cg->trHeapMemory()) TR::LabelRelative24BitRelocation(
        cursor, getUnresolvedSnippet()->getSnippetLabel()));
    *wcursor = 0x48000000; // b SnippetLabel;
    wcursor++;
    cursor += PPC_INSTRUCTION_LENGTH;
    ...
    if (self()->isUsingDelayedIndexedForm()) {
      ...
      *wcursor = preserve;
      ...
      base->setRegisterFieldRB(wcursor);
               ...
               (self()->offsetRequireWordAlignment() &&
                (displacement & 3 != 0))) {
      if (self()->isBaseModifiable()) {
        *wcursor = 0x3c000000 | upper;
        base->setRegisterFieldRT(wcursor);
        ...
        *wcursor = preserve | lower;
        ...
        TR::RealRegister *rX =
            cg->machine()->getRealRegister(choose_rX(currentInstruction, base));
...
uint32_t
OMR::Power::MemoryReference::estimateBinaryLength(TR::CodeGenerator &codeGen) {
  ...
  if (_indexRegister != NULL) {
    if (self()->isUsingDelayedIndexedForm()) {
      if (self()->getOffset(*comp) < LOWER_IMMED ||
          self()->getOffset(*comp) > UPPER_IMMED) {
        return (3 * PPC_INSTRUCTION_LENGTH);
...
extern "C" void voom() {}
void OMR::Power::MemoryReference::accessStaticItem(TR::Node *node,
  ...
  TR_ASSERT(symbol->isStatic() || symbol->isMethod(),
            "accessStaticItem:problem with new symbol hierarchy");

  TR_ASSERT(_baseRegister == NULL, "32Bit JIT has relocation problem for AOT");

  bool isStatic = symbol->isStatic() && !ref->isUnresolved();
  bool isStaticField =
      isStatic && (ref->getCPIndex() > 0) && !symbol->isClassObject();
  ...
  if (isPicSite && !cg->comp()->compileRelocatableCode() &&
    ...
    intptrj_t address =
        (intptrj_t)symbol->getStaticSymbol()->getStaticAddress();
    loadAddressConstantInSnippet(
        cg, node ? node : cg->getCurrentEvaluationTreeTop()->getNode(), address,
        reg, NULL, isStore ? TR::InstOpCode::Op_st : TR::InstOpCode::Op_load,
        false, NULL);
    return;
  }
  ...
  if (useUnresSnippetToAvoidRelo) {
    ...
    bool reloIsRequired = symbol->isStatic() && symbol->isClassObject() &&
                          ref->getCPIndex() == -1;
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (cg->comp()->compileRelocatableCode()) {
      /* don't want to trash node prematurely by the code for handling other
       * symbols */
      ...
      if (symbol->isDebugCounter()) {
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(cg, true, nodeForSymbol, 1, reg, NULL, false,
                            TR_DebugCounter);
        return;
      }
      if (symbol->isCountForRecompile()) {
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(cg, true, nodeForSymbol, TR_CountForRecompile, reg,
                            NULL, false, TR_GlobalValue);
        return;
        ...
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        loadAddressConstantInSnippet(cg, nodeForSymbol, 0, reg, NULL,
                                     TR::InstOpCode::addi, false, NULL);
        return;
        ...
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(cg, true, nodeForSymbol, (intptrj_t)ref, reg, NULL,
    ...
    // startPC symbols should not be placed in the TOC as their value is not
    // known at this time and they are different from method to method
    ...
    //  (otherwise optimizer will remove the ResolveCHK and we can be sure pTOC
    ...
    if ((ref->isUnresolved() || useUnresSnippetToAvoidRelo) &&
        (topNode->getOpCodeValue() == TR::ResolveCHK ||
         tocIndex == PTOC_FULL_INDEX)) {
      snippet = new (cg->trHeapMemory())
          TR::UnresolvedDataSnippet(cg, node, ref, isStore, false);
      cg->addSnippet(snippet);
    }

    // TODO: Improve the code sequence for cases when we know pTOC is full.
    TR::MemoryReference *tocRef = new (cg->trHeapMemory()) TR::MemoryReference(
        cg->getTOCBaseRegister(), 0, sizeof(uintptrj_t), cg);
    ...
    generateTrg1MemInstruction(cg, loadOp, node == NULL ? topNode : node,
                               addrReg, tocRef);
    if (snippet != NULL)
    ...
    if (ref->isUnresolved() || comp->compileRelocatableCode()) {
      ...
      if (symbol->isDebugCounter()) {
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(cg, comp->compileRelocatableCode(), nodeForSymbol,
                            1, reg, NULL, false, TR_DebugCounter);
        return;
      } else if (symbol->isCountForRecompile()) {
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(cg, comp->compileRelocatableCode(), nodeForSymbol,
                            TR_CountForRecompile, reg, NULL, false,
                            TR_GlobalValue);
        return;
      } else if (symbol->isRecompilationCounter()) {
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(cg, comp->compileRelocatableCode(), nodeForSymbol,
                            0, reg, NULL, false, TR_BodyInfoAddressLoad);
        return;
      } else if (symbol->isCompiledMethod()) {
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(cg, comp->compileRelocatableCode(), nodeForSymbol,
                            0, reg, NULL, false, TR_RamMethodSequence);
        return;
      } else if (symbol->isStartPC()) {
        // use inSnippet, as the relocation mechanism is already set up there
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        ...
        TR::Register *reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(cg, comp->compileRelocatableCode(), nodeForSymbol,
                            1, reg, NULL, false, TR_DataAddress);
        return;
    ...
    if (symbol->isStartPC()) {
      loadAddressConstantInSnippet(cg, node, 0, reg, NULL, TR::InstOpCode::addi,
                                   false, NULL);
      return;
    }

    self()->setBaseModifiable();
    rel1 = generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, reg,
                                      cg->hiValue(addr) & 0x0000ffff);
    self()->addToOffset(node, LO_VALUE(addr), cg);

    if (comp->compileRelocatableCode()) {
      TR_ASSERT_FATAL(symbol->isClassObject(),
      ...
      staticRelocation = new (cg->trHeapMemory()) TR::PPCPairedRelocation(
          rel1, NULL, (uint8_t *)recordInfo, TR_ClassAddress, node);
...
void OMR::Power::MemoryReference::setSymbol(TR::Symbol *symbol,
                                            TR::CodeGenerator *cg) {
  TR_ASSERT(_symbolReference->getSymbol() == NULL ||
                _symbolReference->getSymbol() == symbol,
