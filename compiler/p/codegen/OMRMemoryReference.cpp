...
static TR::RealRegister::RegNum
choose_rX(TR::Instruction*, TR::RealRegister*);
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  if (mr.getUnresolvedSnippet() != NULL) {
    _unresolvedSnippet = new (cg->trHeapMemory())
      TR::UnresolvedDataSnippet(cg,
                                node,
                                _symbolReference,
                                mr.getUnresolvedSnippet()->isUnresolvedStore(),
                                false);
...
void
OMR::Power::MemoryReference::addToOffset(TR::Node* node,
                                         intptrj_t amount,
                                         TR::CodeGenerator* cg)
{
      ...
      if (node->getOpCode().isLoadConst() && node->getRegister()) {
        ...
        if (TR::Compiler->target.is64Bit() &&
          ...
          generateTrg1Src2Instruction(
            cg, TR::InstOpCode::add, node, newBase, _baseRegister, tempReg);
          cg->stopUsingRegister(tempReg);
        } else
          generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::addis, node, newBase, _baseRegister, upper);
        if (lower != 0)
          generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::addi2, node, newBase, newBase, lower);
      }
    } else
...
void
OMR::Power::MemoryReference::forceIndexedForm(TR::Node* node,
                                              TR::CodeGenerator* cg,
...
void
OMR::Power::MemoryReference::adjustForResolution(TR::CodeGenerator* cg)
{
...
void
OMR::Power::MemoryReference::decNodeReferenceCounts(TR::CodeGenerator* cg)
{
  if (_baseRegister != NULL) {
    if (_baseNode != NULL) {
...
void
OMR::Power::MemoryReference::bookKeepingRegisterUses(TR::Instruction* instr,
                                                     TR::CodeGenerator* cg)
{
  if (_baseRegister != NULL) {
...
static bool
isLoadConstAndShift(TR::Node* subTree, TR::CodeGenerator* cg)
{
  if (TR::Compiler->target.is64Bit()) {
...
void
OMR::Power::MemoryReference::populateMemoryReference(TR::Node* subTree,
                                                     TR::CodeGenerator* cg)
{
  ...
  if (subTree->getOpCodeValue() == TR::i2l && subTree->isNonNegative() &&
    ...
    if (performTransformation(
          cg->comp(),
          "O^O PPC Evaluator: Skip unecessary sign extension on index [%p].\n",
          subTree)) {
  ...
  if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL) {
    ...
    if (subTree->getOpCode().isArrayRef() ||
      ...
      if (TR::Compiler->target.is32Bit() || !symbol->isConstObjectRef())
        self()->addToOffset(
          subTree, subTree->getSymbolReference()->getOffset(), cg);
      // TODO: aliasing sets?
      ...
      if (_baseRegister != NULL) {
        self()->consolidateRegisters(cg->evaluate(subTree),
                                     subTree,
                                     cg->canClobberNodesRegister(subTree),
                                     cg);
      } else {
        _baseRegister = cg->evaluate(subTree);
        _baseNode = subTree;
...
void
OMR::Power::MemoryReference::consolidateRegisters(TR::Register* srcReg,
                                                  TR::Node* srcTree,
                                                  bool srcModifiable,
                                                  TR::CodeGenerator* cg)
{
...
void
OMR::Power::MemoryReference::assignRegisters(
  ...
  TR::CodeGenerator* cg)
{
  ...
  if (_modBase != NULL) {
    ...
    _conditions->assignPostConditionRegisters(
      currentInstruction, _modBase->getKind(), cg);
    _conditions->assignPreConditionRegisters(
      currentInstruction->getPrev(), _modBase->getKind(), cg);
  ...
  if (_baseRegister != NULL) {
    ...
    if ((assignedBaseRegister != NULL) &&
      ...
      if ((alternativeRegister = machine->findBestFreeRegister(
             currentInstruction, TR_GPR, true, false, _baseRegister)) == NULL) {
...
void
OMR::Power::MemoryReference::mapOpCode(TR::Instruction* currentInstruction)
{
...
uint8_t*
OMR::Power::MemoryReference::generateBinaryEncoding(
  TR::Instruction* currentInstruction,
  uint8_t* cursor,
  TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is64Bit() && self()->isTOCAccess()) {
    ...
    if (self()->getUnresolvedSnippet() != NULL) {
      ...
      cg->addRelocation(new (cg->trHeapMemory())
                          TR::LabelRelative24BitRelocation(
...
static TR::RealRegister::RegNum
choose_rX(TR::Instruction* currentInstruction, TR::RealRegister* base)
{
...
{}
void
OMR::Power::MemoryReference::accessStaticItem(TR::Node* node,
                                              ...
                                              TR::CodeGenerator* cg)
{
  ...
  if (isPicSite && !cg->comp()->compileRelocatableCode() &&
    ...
    loadAddressConstantInSnippet(
      cg,
      node ? node : cg->getCurrentEvaluationTreeTop()->getNode(),
      address,
      reg,
      NULL,
      isStore ? TR::InstOpCode::Op_st : TR::InstOpCode::Op_load,
      false,
      NULL);
    return;
  }
  ...
  if (useUnresSnippetToAvoidRelo) {
    ...
    bool reloIsRequired =
      symbol->isStatic() && symbol->isClassObject() && ref->getCPIndex() == -1;
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (cg->comp()->compileRelocatableCode()) {
      ...
      if (symbol->isDebugCounter()) {
        TR::Register* reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(
          cg, true, nodeForSymbol, 1, reg, NULL, false, TR_DebugCounter);
        return;
      }
      if (symbol->isCountForRecompile()) {
        ...
        TR::Register* reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(
          cg, true, nodeForSymbol, 1, reg, NULL, false, TR_BodyInfoAddressLoad);
        return;
      } else if (symbol->isCompiledMethod()) {
        TR::Register* reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(
          cg, true, nodeForSymbol, 1, reg, NULL, false, TR_RamMethodSequence);
        return;
      } else if (symbol->isStartPC()) {
        // use inSnippet, as the relocation mechanism is already set up there
        TR::Register* reg = _baseRegister = cg->allocateRegister();
        loadAddressConstantInSnippet(
          cg, nodeForSymbol, 0, reg, NULL, TR::InstOpCode::addi, false, NULL);
        return;
      } else if (isStaticField && !ref->isUnresolved()) {
        TR::Register* reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(
          cg, true, nodeForSymbol, 1, reg, NULL, false, TR_DataAddress);
        return;
    ...
    TR::MemoryReference* tocRef = new (cg->trHeapMemory())
      TR::MemoryReference(cg->getTOCBaseRegister(), 0, sizeof(uintptrj_t), cg);
    ...
    generateTrg1MemInstruction(
      cg, loadOp, node == NULL ? topNode : node, addrReg, tocRef);
    if (snippet != NULL)
    ...
    if (symbol->isStartPC()) {
      loadAddressConstantInSnippet(
        cg, node, 0, reg, NULL, TR::InstOpCode::addi, false, NULL);
      return;
    }

    self()->setBaseModifiable();
    rel1 = generateTrg1ImmInstruction(
      cg, TR::InstOpCode::lis, node, reg, cg->hiValue(addr) & 0x0000ffff);
...
void
OMR::Power::MemoryReference::setSymbol(TR::Symbol* symbol,
                                       TR::CodeGenerator* cg)
{
