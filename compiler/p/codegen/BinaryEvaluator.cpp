    ...
    TR::CodeGenerator* cg)
{
    if (cg->comp()->useCompressedPointers() && node->containsCompressionSequence()) {
        ...
        if (n->getOpCodeValue() == TR::ladd || n->getOpCodeValue() == TR::lsub) {
            ...
            if (n->getFirstChild()->getOpCodeValue() == TR::iu2l || n->getFirstChild()->getOpCodeValue() == TR::a2l || n->getFirstChild()->getOpCode().isShift()) {
...
TR::Register* OMR::Power::TreeEvaluator::iaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCp9 && firstChild->getOpCodeValue() == TR::imul && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL) {
        trgReg = cg->allocateRegister();
        TR::Register* src2Reg = cg->evaluate(secondChild);
    ...
    bool& highZero)
{
    ...
    if (child->isHighWordZero()) {
        ...
        if (child->getReferenceCount() == 1 && reg == NULL) {
            if (firstOp == TR::iu2l || firstOp == TR::su2l || (firstOp == TR::lushr && (child->getSecondChild()->getOpCodeValue() == TR::iconst || child->getSecondChild()->getOpCodeValue() == TR::iuconst) && (child->getSecondChild()->getInt() & LONG_SHIFT_MASK) == 32)) {
...
TR::Register* OMR::Power::TreeEvaluator::laddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is32Bit()) {
        if (!setsOrReadsCC && (secondOp == TR::lconst || secondOp == TR::luconst) && secondChild->getRegister() == NULL) {
            src1Reg = cg->evaluate(firstChild);
    ...
    } else // 64 bit target
    {
        ...
        if (TR::Compiler->target.cpu.id() >= TR_PPCp9 && !setsOrReadsCC && (node->getOpCodeValue() == TR::ladd || node->getOpCodeValue() == TR::aladd) && firstChild->getOpCodeValue() == TR::lmul && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL) {
            trgReg = cg->allocateRegister();
            ...
            // to be an lconst
            else if (!setsOrReadsCC && (secondOp == TR::iconst || secondOp == TR::iuconst) && // may be true if aladd?
        ...
        if ((node->getOpCodeValue() == TR::aladd || node->getOpCodeValue() == TR::aluadd) && node->isInternalPointer()) {
            if (node->getPinningArrayPointer()) {
                trgReg->setContainsInternalPointer();
                trgReg->setPinningArrayPointer(node->getPinningArrayPointer());
            } else {
...
TR::Register* OMR::Power::TreeEvaluator::lsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!setsOrReadsCC && (secondChild->getOpCodeValue() == TR::lconst || secondChild->getOpCodeValue() == TR::luconst) && secondChild->getRegister() == NULL) {
        TR::Register* src1Reg = cg->evaluate(firstChild);
        ...
        TR::Register* lowReg = cg->allocateRegister();
        TR::Register* highReg = cg->allocateRegister();
        if (!setsOrReadsCC && (firstChild->getOpCodeValue() == TR::lconst || firstChild->getOpCodeValue() == TR::luconst) && firstChild->getRegister() == NULL) {
            TR::Register* src2Reg = cg->evaluate(secondChild);
...
TR::Register* OMR::Power::TreeEvaluator::lmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((secondChild->getOpCodeValue() == TR::lconst || secondChild->getOpCodeValue() == TR::luconst) && secondChild->getRegister() == NULL) {
...
void simplifyANDRegImm(TR::Node* node, TR::Register* trgReg, TR::Register* srcReg, TR::Node* constNode, TR::CodeGenerator* cg)
{
    ...
    if ((localLow.getValue() == -1) || (contiguousBits(localLow.getValue())) || (localLow.getHighBits() == 0) || (localLow.getLowBits() == 0)) {
        lowSpecial = true;
    }

    if ((localHigh.getValue() == -1) || (contiguousBits(localHigh.getValue())) || (localHigh.getHighBits() == 0) || (localHigh.getLowBits() == 0)) {
        highSpecial = true;
    }
...
TR::Register* OMR::Power::TreeEvaluator::landEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        if ((secondOp == TR::lconst || secondOp == TR::luconst) && secondChild->getRegister() == NULL) {
...
TR::Register* OMR::Power::TreeEvaluator::lorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((node->getFirstChild()->isHighWordZero() || node->getSecondChild()->isHighWordZero()) && !((secondOp == TR::lconst || secondOp == TR::luconst) && node->getSecondChild()->getRegister() == NULL) && !(TR::Compiler->target.is64Bit())) {
        return carrylessLongEvaluatorWithAnalyser(node, cg,
            TR::InstOpCode::OR,
            TR::InstOpCode::OR,
            TR::InstOpCode::mr);
