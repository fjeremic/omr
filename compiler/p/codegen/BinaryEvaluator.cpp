...
static TR::Register*
ldiv64Evaluator(TR::Node* node, TR::CodeGenerator* cg);
static TR::Register*
lrem64Evaluator(TR::Node* node, TR::CodeGenerator* cg);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (localVal.getValue() == -1 && (node->getOpCodeValue() == TR::ixor)) {
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::subfic, node, trgReg, src1Reg, -1);
    } else if (localVal.getHighBits() == 0) {
      generateTrg1Src1ImmInstruction(
        cg, immedOp, node, trgReg, src1Reg, localVal.getLowBits());
    } else if (localVal.getLowBits() == 0) {
      generateTrg1Src1ImmInstruction(
        cg, immedShiftedOp, node, trgReg, src1Reg, localVal.getHighBits());
    } else {
      TR::Register* tempReg = cg->allocateRegister();
      generateTrg1Src1ImmInstruction(
        cg, immedOp, node, tempReg, src1Reg, localVal.getLowBits());
      generateTrg1Src1ImmInstruction(
        cg, immedShiftedOp, node, trgReg, tempReg, localVal.getHighBits());
...
static bool
genNullTestForCompressedPointers(TR::Node* node,
                                 TR::Register*& trgReg,
                                 TR::Register*& condReg,
                                 TR::Register* src1Reg,
                                 TR::Register*& src2Reg,
  ...
  if (cg->comp()->useCompressedPointers() &&
    ...
    if (!isNonZero && !useBranchless) {
      if (n->getFirstChild()->getOpCode().isShift() &&
        ...
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::cmpli8, node, condReg, src1Reg, NULLVALUE);
      }
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::beq, node, doneSkipAdd, condReg);
    }
    return true;
  }
  return false;
...
TR::Register*
OMR::Power::TreeEvaluator::iaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.cpu.id() >= TR_PPCp9 &&
    ...
    generateTrg1Src3Instruction(cg,
                                TR::InstOpCode::maddld,
                                node,
                                trgReg,
                                mulSrc1Reg,
                                mulSrc2Reg,
    ...
    if (secondChild->getOpCode().isLoadConst() &&
        secondChild->getRegister() == NULL) {
      trgReg = addConstantToInteger(
        node, src1Reg, secondChild->get32bitIntegralValue(), cg);
    } else {
      TR::Register* src2Reg = cg->evaluate(secondChild);
      trgReg = cg->allocateRegister();
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::add, node, trgReg, src1Reg, src2Reg);
    }
  }
...
static void
genericLongAnalyzer(TR::CodeGenerator* cg,
                    TR::Node*& child,
                    TR::Register*& lowReg,
                    TR::Register*& highReg,
                    bool& highZero)
...
static TR::Register*
carrylessLongEvaluatorWithAnalyser(TR::Node* node,
                                   TR::CodeGenerator* cg,
                                   ...
                                   TR::InstOpCode::Mnemonic copyRegRegOpCode)
{
  ...
  generateTrg1Src2Instruction(
    cg, lowRegRegOpCode, node, lowReg, src1Low, src2Low);
  ...
  if (firstHighZero) {
    if (secondHighZero || node->getOpCodeValue() == TR::land) {
      ...
      generateTrg1Src1Instruction(
        cg, copyRegRegOpCode, node, highReg, src2High);
    }
    ...
    if (node->getOpCodeValue() == TR::land) {
      ...
      generateTrg1Src1Instruction(
        cg, copyRegRegOpCode, node, highReg, src1High);
    }
  } else {
    generateTrg1Src2Instruction(
      cg, highRegRegOpCode, node, highReg, src1High, src2High);
  }
...
static TR::Register*
laddEvaluatorWithAnalyser(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* src1Low = NULL;
  TR::Register* src1High = NULL;
  TR::Register* src2Low = NULL;
  ...
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::addc, node, lowReg, src1Low, src2Low);

  if (firstHighZero) {
    if (secondHighZero) {
      loadConstant(cg, node, 0, highReg);
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::addze, node, highReg, highReg);
    } else {
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::addze, node, highReg, src2High);
    }
  } else if (secondHighZero) {
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::addze, node, highReg, src1High);
  } else {
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::adde, node, highReg, src1High, src2High);
  }
...
TR::Register*
OMR::Power::TreeEvaluator::laddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  TR::Register* src1Reg = NULL;
      ...
      if (!setsOrReadsCC &&
        ...
        if ((node->getOpCodeValue() == TR::luaddc) &&
            TR_PPCComputeCC::setCarryBorrow(
              node->getChild(2), false, &carryReg, cg)) {
  ...
  } else // 64 bit target
  {
    ...
    if (useBranchless && hasCompressedPointers) {
      ...
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::neg, node, tReg, decomprReg);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::sradi, node, tReg, tReg, 63);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::AND, node, tReg, tReg, src2Reg);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::add, node, trgReg, tReg, src1Reg);
      node->setRegister(trgReg);
      cg->decReferenceCount(firstChild);
      cg->decReferenceCount(secondChild);
      if (src1Reg == trgReg)
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCp9 && !setsOrReadsCC &&
      ...
      if (!setsOrReadsCC &&
        ...
        trgReg = addConstantToLong(
          node, src1Reg, secondChild->getLongInt(), trgReg, cg);
      }
      ...
      // to be an lconst
               ...
               secondChild->getRegister() == NULL) {
        trgReg = addConstantToLong(
          node, src1Reg, (int64_t)secondChild->getInt(), trgReg, cg);
      } else {
        ...
        if (setsOrReadsCC) {
          ...
          generateTrg1Src2Instruction(
            cg, regToRegOpCode, node, trgReg, src1Reg, src2Reg);
        } else {
          generateTrg1Src2Instruction(
            cg, TR::InstOpCode::add, node, trgReg, src1Reg, src2Reg);
        }
    ...
    if (hasCompressedPointers && doneSkipAdd) {
      ...
      generateDepLabelInstruction(
        cg, TR::InstOpCode::label, node, doneSkipAdd, deps);
    }
...
TR::Register*
OMR::Power::TreeEvaluator::isubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* secondChild = node->getSecondChild();
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* trgReg = NULL;
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (firstChild->getOpCode().isLoadConst() &&
      ...
      if (value >= LOWER_IMMED && value <= UPPER_IMMED) {
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::subfic, node, trgReg, src2Reg, value);
      } else // constant won't fit
      {
        src1Reg = cg->evaluate(firstChild);
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::subf, node, trgReg, src2Reg, src1Reg);
      }
    } else // no constants
    {
      src1Reg = cg->evaluate(firstChild);
...
TR::Register*
OMR::Power::TreeEvaluator::asubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
lsub64Evaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* secondChild = node->getSecondChild();
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* trgReg = NULL;
  ...
  if (cg->comp()->useCompressedPointers() &&
    ...
    genNullTestForCompressedPointers(
      node, trgReg, condReg, src1Reg, src2Reg, doneSkipSub, cg);
    ...
    if (useBranchless) {
      ...
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::AND, node, tReg, tReg, src2Reg);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::subf, node, trgReg, tReg, src1Reg);
    ...
    if (doneSkipSub) {
      ...
      cg->stopUsingRegister(condReg);
      generateDepLabelInstruction(
        cg, TR::InstOpCode::label, node, doneSkipSub, deps);
    }
  } else {
      ...
      } else // no constants or setsOrReadsCC
      {
        ...
        if (setsOrReadsCC) {
          ...
          if ((node->getOpCodeValue() == TR::lusubb) &&
              TR_PPCComputeCC::setCarryBorrow(
                node->getChild(2), true, &borrowReg, cg)) {
          ...
          generateTrg1Src2Instruction(
            cg, regToRegOpCode, node, trgReg, src2Reg, src1Reg);
        } else {
          generateTrg1Src2Instruction(
            cg, TR::InstOpCode::subf, node, trgReg, src2Reg, src1Reg);
        }
...
// also handles lusub
TR::Register*
OMR::Power::TreeEvaluator::lsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  if (TR::Compiler->target.is64Bit())
  ...
  if (!setsOrReadsCC &&
    ...
    if (!setsOrReadsCC &&
      ...
      if (highValue == 0) {
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::subfze, node, highReg, src2Reg->getHighOrder());
      } else if (highValue == -1) {
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::subfme, node, highReg, src2Reg->getHighOrder());
      } else {
        TR::Register* tempReg = cg->allocateRegister();
...
TR::Register*
OMR::Power::TreeEvaluator::imulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* trgReg;
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
...
static TR::Register*
lmulEvaluatorWithAnalyser(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mullw, node, lowReg, src1Low, src2Low);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mulhwu, node, highReg, src1Low, src2Low);
  ...
  if (firstHighZero) {
    if (!secondHighZero) {
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::mullw, node, temp1Reg, src1Low, src2High);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::add, node, highReg, highReg, temp1Reg);
    }
  } else {
    TR_ASSERT(secondHighZero,
              "One of the long operands must have vacant high register");
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::mullw, node, temp1Reg, src2Low, src1High);
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::add, node, highReg, highReg, temp1Reg);
  }
...
//
TR::Register*
OMR::Power::TreeEvaluator::dualMulHelper64(TR::Node* node,
                                           TR::Node* lmulNode,
                                           TR::Node* lumulhNode,
                                           TR::CodeGenerator* cg)
{
...
//
TR::Register*
OMR::Power::TreeEvaluator::dualMulHelper32(TR::Node* node,
                                           TR::Node* lmulNode,
                                           TR::Node* lumulhNode,
                                           TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mullw, node, r1Reg, alReg, blReg);

  // mulhwu  r2, al, al     ;;     r2 =      (al * bl)h
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mulhwu, node, r2Reg, alReg, blReg);

  // mullw    t, ah, bl     ;;      t =      (ah * bl)l
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mullw, node, tReg, ahReg, blReg);

  // addc    r2, r2,  t     ;; (r2,C) =       r2 + t
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::addc, node, r2Reg, r2Reg, tReg);

  // mulhwu  r3, ah, bl     ;;     r3 = bl =             (ah * bl)h
  // bl is overwritten here
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mulhwu, node, r3Reg, ahReg, blReg);

  // mullw    t, ah, bh     ;;      t =                  (ah * bh)l
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mullw, node, tReg, ahReg, bhReg);

  // adde    r3, r3,  t     ;; (r3,C) =                  r3 + t + C
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::adde, node, r3Reg, r3Reg, tReg);

  // mulhwu  r4, ah, bh     ;;     r4 = ah = (ah * bh)h
  // ah is overwritten here
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mulhwu, node, r4Reg, ahReg, bhReg);
  ...
  // mullw    t, al, bh     ;;      t =                              (al * bh)l
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mullw, node, tReg, alReg, bhReg);
  ...
  // mulhwu   t, al, bh     ;;      t =                  (al * bh)h
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mulhwu, node, tReg, alReg, bhReg);
...
TR::Register*
OMR::Power::TreeEvaluator::lmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if ((secondChild->getOpCodeValue() == TR::lconst ||
    ...
    if (lowValue == 0) {
      ...
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::mr, node, lowReg, src1Low);
      if (highValue == 0) {
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::mr, node, highReg, src1High);
      } else if (highValue == 1) {
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::add, node, highReg, src1High, src1Low);
      } else {
        TR::Register* temp1Reg = cg->allocateRegister();
        mulConstant(node, temp1Reg, src1Low, highValue, cg);
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::add, node, highReg, src1High, temp1Reg);
        cg->stopUsingRegister(temp1Reg);
      }
    } else if (lowValue == -1 && highValue == -1) {
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::subfic, node, lowReg, src1Low, 0);
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::subfze, node, highReg, src1High);
    } else {
      ...
      if (leadingZeroes(abs(lowValue)) >= 24) {
        ...
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::mullw, node, lowReg, src1Low, temp1Reg);
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::mulhwu, node, temp2Reg, src1Low, temp1Reg);
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::mullw, node, temp3Reg, src1High, temp1Reg);
      } else {
        // the constant is fairly big, so put it in RA
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::mullw, node, lowReg, temp1Reg, src1Low);
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::mulhwu, node, temp2Reg, temp1Reg, src1Low);
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::mullw, node, temp3Reg, temp1Reg, src1High);
      }
      cg->stopUsingRegister(temp1Reg);

      if (highValue == 0) {
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::add, node, highReg, temp2Reg, temp3Reg);
        cg->stopUsingRegister(temp2Reg);
        cg->stopUsingRegister(temp3Reg);
      } else {
        TR::Register* temp4Reg = cg->allocateRegister();
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::add, node, temp4Reg, temp2Reg, temp3Reg);
        cg->stopUsingRegister(temp2Reg);
        cg->stopUsingRegister(temp3Reg);
        ...
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::add, node, highReg, temp4Reg, temp5Reg);
    ...
    if (firstChild->isHighWordZero() || secondChild->isHighWordZero()) {
      ...
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::mullw, node, lowReg, src1Low, src2Low);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::mulhwu, node, temp1Reg, src1Low, src2Low);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::mullw, node, temp2Reg, src1High, src2Low);

      TR::Register* temp3Reg = cg->allocateRegister();
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::add, node, temp3Reg, temp1Reg, temp2Reg);
      cg->stopUsingRegister(temp1Reg);
      cg->stopUsingRegister(temp2Reg);

      TR::Register* temp4Reg = cg->allocateRegister();
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::mullw, node, temp4Reg, src1Low, src2High);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::add, node, highReg, temp3Reg, temp4Reg);
      cg->stopUsingRegister(temp3Reg);
      cg->stopUsingRegister(temp4Reg);
    }
  }
...
// also handles iumulh
TR::Register*
OMR::Power::TreeEvaluator::imulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* trgReg = cg->allocateRegister();
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCodeValue() == TR::iconst ||
    ...
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::mulhw, node, trgReg, tempReg, src1Reg);
    cg->stopUsingRegister(tempReg);
  } else // no constants
  {
    // want the smaller of the sources in the RB position of a multiply
...
TR::Register*
OMR::Power::TreeEvaluator::lmulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (secondChild->getOpCodeValue() == TR::lconst ||
      ...
      // put the large magic number into the RA position
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::mulhd, node, trgReg, tempReg, src1Reg);
      cg->stopUsingRegister(tempReg);
    } else {
      // want the smaller of the sources in the RB position of a multiply
      // the second child is assumed to be the large magic number
  ...
  // put the large magic number into the RA position
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mulhw, node, temp1Reg, second_lowReg, first_highReg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mulhw, node, temp2Reg, second_highReg, first_lowReg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mullw, node, temp3Reg, second_highReg, first_highReg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::mulhw, node, highReg, second_highReg, first_highReg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::addc, node, lowReg, temp1Reg, temp2Reg);
  generateTrg1Src1Instruction(
    cg, TR::InstOpCode::addze, node, highReg, highReg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::addc, node, lowReg, lowReg, temp3Reg);
  generateTrg1Src1Instruction(
    cg, TR::InstOpCode::addze, node, highReg, highReg);
  if (secondChild->getOpCodeValue() == TR::lconst ||
      secondChild->getOpCodeValue() == TR::luconst) {
...
static TR::Register*
signedIntegerDivisionOrRemainderAnalyser(TR::Node* node,
                                         TR::CodeGenerator* cg,
                                         ...
                                         TR::Register* tmp2Reg = NULL)
{
  ...
  if (divisorValue == 1) {
    ...
    if (isRemainder)
      ...
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::neg, node, trgReg, dividendReg);
    ...
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::addze, node, trgReg, trgReg);
    if (isRemainder) {
      generateShiftLeftImmediate(
        cg, node, trgReg, trgReg, trailingZeroes(divisorValue));
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::subf, node, trgReg, trgReg, dividendReg);
    } else {
      if (isNonPositivePowerOf2(divisorValue))
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::neg, node, trgReg, trgReg);
    }
  } else if (TR::Compiler->target.cpu.id() >= TR_PPCp9 && isRemainder) {
    if (divisorReg == NULL)
      divisorReg = cg->evaluate(node->getSecondChild());
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::modsw, node, trgReg, dividendReg, divisorReg);
  } else {
    if (tmp1Reg == NULL) {
    ...
    // put the large magic number into the RA position
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::mulhw, node, tmp1Reg, tmp1Reg, dividendReg);
    if ((divisorValue > 0) && (magicNumber < 0)) {
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::add, node, tmp1Reg, dividendReg, tmp1Reg);
    } else if ((divisorValue < 0) && (magicNumber > 0)) {
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::subf, node, tmp1Reg, dividendReg, tmp1Reg);
    }
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::srawi, node, tmp1Reg, tmp1Reg, shiftAmount);
    if (divisorValue > 0) {
    ...
    if (isRemainder) {
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::add, node, tmp1Reg, tmp1Reg, tmp2Reg);
      if (divisorReg != NULL)
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::mullw, node, tmp2Reg, tmp1Reg, divisorReg);
      else
        mulConstant(node, tmp2Reg, tmp1Reg, divisorValue, cg);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::subf, node, trgReg, tmp2Reg, dividendReg);
    } else
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::add, node, trgReg, tmp1Reg, tmp2Reg);
  }
...
signedLongDivisionOrRemainderAnalyser(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (isPowerOf2(divisor)) {
    ...
    if (rootOpCode == TR::lrem) {
      ...
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::addze, node, temp2Reg, temp1Reg);
      cg->stopUsingRegister(temp1Reg);

      TR::Register* temp3Reg = cg->allocateRegister();
      generateShiftLeftImmediateLong(
        cg, node, temp3Reg, temp2Reg, trailingZeroes(divisor));
      cg->stopUsingRegister(temp2Reg);

      TR::Register* temp4Reg = cg->allocateRegister();
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::subf, node, temp4Reg, temp3Reg, dividendReg);
      cg->stopUsingRegister(temp3Reg);
      return temp4Reg;
    } else // rootOpCode == TR::ldiv
    {
      ...
      if (isNonPositivePowerOf2(divisor)) {
        TR::Register* temp3Reg = cg->allocateRegister();
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::neg, node, temp3Reg, temp2Reg);
        cg->stopUsingRegister(temp2Reg);
        return temp3Reg;
      } else {
        return temp2Reg;
    ...
    // put the large magic number into the RA position
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::mulhd, node, temp1Reg, magicReg, dividendReg);
    cg->stopUsingRegister(magicReg);

    if ((divisor > 0) && (magicNumber < 0)) {
      temp2Reg = cg->allocateRegister();
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::add, node, temp2Reg, dividendReg, temp1Reg);
      cg->stopUsingRegister(temp1Reg);
    } else if ((divisor < 0) && (magicNumber > 0)) {
      temp2Reg = cg->allocateRegister();
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::subf, node, temp2Reg, dividendReg, temp1Reg);
      cg->stopUsingRegister(temp1Reg);
    } else
      temp2Reg = temp1Reg;
    ...
    TR::Register* temp4Reg = cg->allocateRegister();
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::sradi, node, temp3Reg, temp2Reg, shiftAmount);
    cg->stopUsingRegister(temp2Reg);

    if (divisor > 0) {
      if (TR::Compiler->target.is64Bit())
        generateShiftRightLogicalImmediateLong(
          cg, node, temp4Reg, dividendReg, 63);
      else
        generateShiftRightLogicalImmediate(cg, node, temp4Reg, dividendReg, 31);
    } else {
      if (TR::Compiler->target.is64Bit())
        generateShiftRightLogicalImmediateLong(
          cg, node, temp4Reg, temp3Reg, 63);
      else
    ...
    TR::Register* temp5Reg = cg->allocateRegister();
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::add, node, temp5Reg, temp3Reg, temp4Reg);
    cg->stopUsingRegister(temp3Reg);
    cg->stopUsingRegister(temp4Reg);
    ...
    if (rootOpCode == TR::lrem) {
      ...
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::subf, node, temp7Reg, temp6Reg, dividendReg);
...
// also handles iudiv
TR::Register*
OMR::Power::TreeEvaluator::idivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  TR::Register* trgReg;
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (divisor != 0) {
    ...
    if (testNeeded) {
      ...
      if (false /*testNeeded*/) {
        ...
        condReg = cg->allocateRegister(TR_CCR);
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::cmpi4, node, condReg, divisorReg, -1);
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::neg, node, trgReg, dividendReg);
        generateConditionalBranchInstruction(
          cg, TR::InstOpCode::beq, node, doneLabel, condReg);
        cg->stopUsingRegister(condReg);
      }
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::divw, node, trgReg, dividendReg, divisorReg);
      if (false /*testNeeded*/)
  ...
  if (divisor != CONSTANT64(0)) {
    ...
    if (testNeeded) {
      ...
      if (false /*testNeeded*/) {
        ...
        condReg = cg->allocateRegister(TR_CCR);
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::cmpi8, node, condReg, divisorReg, -1);
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::neg, node, trgReg, dividendReg);
        generateConditionalBranchInstruction(
          cg, TR::InstOpCode::beq, node, doneLabel, condReg);
      ...
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::divd, node, trgReg, dividendReg, divisorReg);
      if (false /*testNeeded*/)
        generateLabelInstruction(cg, TR::InstOpCode::label, node, doneLabel);
    }
  }
  ...
  bool isRemainder)
{
  ...
  if (!cg->canClobberNodesRegister(firstChild)) {
    ...
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::mr, node, dd_l, dividend->getLowOrder());
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::mr, node, dd_h, dividend->getHighOrder());
  } else {
  ...
  *dd_lowReg = dd_l;
  ...
  if (!cg->canClobberNodesRegister(secondChild)) {
    ...
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::mr, node, dr_l, divisor->getLowOrder());
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::mr, node, dr_h, divisor->getHighOrder());
  } else {
  ...
  *dr_lowReg = dr_l;
  ...
  if (!isDividendImpossible32Bit && !isDivisorImpossible32Bit) {
    ...
    if (secondChild->getOpCode().isLoadConst() &&
      ...
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::srawi, node, tmp1Reg, dd_l, 31);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::cmpl4, node, cr0Reg, tmp1Reg, dd_h);
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::bne, node, callLabel, cr0Reg);

      signedIntegerDivisionOrRemainderAnalyser(node,
                                               cg,
                                               dd_l,
                                               drConst,
                                               isRemainder,
                                               isRemainder ? dr_l : dd_l,
                                               dr_l,
                                               tmp1Reg,
                                               tmp2Reg);
      generateTrg1Src1ImmInstruction(cg,
      ...
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::OR, node, tmp1Reg, dd_h, dr_h);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::cmpli4, node, cr0Reg, tmp1Reg, 0);
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::bne, node, callLabel, cr0Reg);
      ...
      if (isRemainder) {
        if (TR::Compiler->target.cpu.id() >= TR_PPCp9) {
          generateTrg1Src2Instruction(
            cg, TR::InstOpCode::moduw, node, dr_l, dd_l, dr_l);
        } else {
          generateTrg1Src2Instruction(
            cg, TR::InstOpCode::divwu, node, tmp2Reg, dd_l, dr_l);
          generateTrg1Src2Instruction(
            cg, TR::InstOpCode::mullw, node, tmp1Reg, tmp2Reg, dr_l);
          generateTrg1Src2Instruction(
            cg, TR::InstOpCode::subf, node, dr_l, tmp1Reg, dd_l);
        }
      } else
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::divwu, node, dd_l, dd_l, dr_l);
    }
  ...
  generateDepLabelInstruction(
    cg, TR::InstOpCode::label, node, doneLabel, dependencies);
}

// also handles ludiv
TR::Register*
OMR::Power::TreeEvaluator::ldivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  strengthReducingLongDivideOrRemainder32BitMode(node,
                                                 cg,
                                                 dependencies,
                                                 &dd_highReg,
                                                 &dd_lowReg,
...
// also handles iurem
TR::Register*
OMR::Power::TreeEvaluator::iremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* trgReg;
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
    ...
    if (dividend != 0x80000000) {
      ...
      if (TR::Compiler->target.cpu.id() >= TR_PPCp9) {
        ...
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::divw, node, trgReg, dividendReg, divisorReg);
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::mullw, node, trgReg, divisorReg, trgReg);
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::subf, node, trgReg, trgReg, dividendReg);
      }
      cg->decReferenceCount(firstChild);
...
lrem64Evaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst())
    ...
    if (dividend != CONSTANT64(0x8000000000000000)) {
      ...
      if (TR::Compiler->target.cpu.id() >= TR_PPCp9) {
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::modsd, node, trgReg, dividendReg, divisorReg);
      } else {
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::divd, node, trgReg, dividendReg, divisorReg);
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::mulld, node, trgReg, divisorReg, trgReg);
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::subf, node, trgReg, trgReg, dividendReg);
      }
      cg->decReferenceCount(firstChild);
...
TR::Register*
OMR::Power::TreeEvaluator::lremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  if (TR::Compiler->target.is64Bit())
...
static bool
isPower9Extswsli(TR::CodeGenerator* cg, TR::Node* node)
{
...
static bool
isZeroExtendThenShiftLeftCandidate(TR::CodeGenerator* cg, TR::Node* node)
{
...
static uint64_t
getExtensionMask(TR::ILOpCode op)
{
...
static TR::Register*
integerShiftLeft(TR::Node* node, uint32_t operandSize, TR::CodeGenerator* cg)
{
    ...
    if (isZeroExtendThenShiftLeftCandidate(cg, node)) {
      ...
      if (mask == 0)
        ...
        generateTrg1Src1Imm2Instruction(
          cg, TR::InstOpCode::rldic, node, trg, lhs, rhs, mask);
      else
        generateTrg1Src1Imm2Instruction(
          cg, TR::InstOpCode::rlwinm, node, trg, lhs, rhs, mask);
      ...
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::extswsli, node, trg, lhs, rhs);
...
static TR::Register*
lshl32Evaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (node->getSecondChild()->getOpCode().isLoadConst()) {
    ...
    if (rhs == 0) {
      ...
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::mr, node, trgHigh, lhsHigh);
    } else if (rhs < 32) {
      ...
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::OR, node, trgHigh, trgHigh, tmp);

      cg->stopUsingRegister(tmp);
    } else if (rhs == 32) {
      generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgLow, 0);
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::mr, node, trgHigh, lhsLow);
    } else // (rhs > 32)
    {
      generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgLow, 0);
    ...
    if (lhsNode->getOpCode().isLoadConst() && lhsNode->getLongIntLow() == 0) {
      ...
      generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgLow, 0);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::slw, node, trgHigh, lhsHigh, rhs);
      ...
      // y_0 = x_0 << n
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::slw, node, trgLow, lhsLow, rhs);

      // y_1 = (x_0 >>> (32 - n)) | (x_0 << (n - 32))
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::subfic, node, trgHigh, rhs, 32);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::srw, node, trgHigh, lhsLow, trgHigh);

      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, node, tmp, rhs, -32);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::slw, node, tmp, lhsLow, tmp);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::OR, node, trgHigh, trgHigh, tmp);
      ...
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::subfic, node, tmp, rhs, 32);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::srw, node, tmp, lhsLow, tmp);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::OR, node, trgHigh, trgHigh, tmp);
...
TR::Register*
OMR::Power::TreeEvaluator::bshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return integerShiftLeft(node, 1, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::sshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return integerShiftLeft(node, 2, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::ishlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return integerShiftLeft(node, 4, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::lshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  if (TR::Compiler->target.is64Bit())
...
static bool
isMaskThenShiftRightCandidate(TR::CodeGenerator* cg,
                              TR::Node* node,
...
static TR::Register*
integerShiftRight(TR::Node* node,
  ...
  if (node->getSecondChild()->getOpCode().isLoadConst()) {
    ...
    if (isMaskThenShiftRightCandidate(cg, node, operandBits, rhs, isUnsigned)) {
      ...
      if (mask == 0)
        ...
        generateTrg1Src1Imm2Instruction(
          cg, TR::InstOpCode::rldicl, node, trg, lhs, (64 - rhs), mask);
      else
        generateTrg1Src1Imm2Instruction(
          cg, TR::InstOpCode::rlwinm, node, trg, lhs, (32 - rhs), mask);
      ...
      if (operandSize > 4)
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::sradi, node, trg, lhs, rhs);
      else
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::srawi, node, trg, lhs, rhs);
    }
  } else if (isUnsigned) {
    TR::Register* lhs = cg->evaluate(node->getFirstChild());
    TR::Register* rhs = cg->evaluate(node->getSecondChild());
    ...
    if (operandSize > 4)
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::srad, node, trg, lhs, rhs);
    else
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::sraw, node, trg, lhs, rhs);
  }
...
static TR::Register*
lshr32Evaluator(TR::Node* node, bool isUnsigned, TR::CodeGenerator* cg)
{
  TR::Register* trgLow = cg->allocateRegister();
  TR::Register* trgHigh = cg->allocateRegister();
  TR::Register* trg = cg->allocateRegisterPair(trgLow, trgHigh);
  ...
  if (node->getSecondChild()->getOpCode().isLoadConst()) {
    ...
    if (rhs == 0) {
      ...
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::OR, node, trgLow, trgLow, tmp);

      if (isUnsigned)
        generateShiftRightLogicalImmediate(cg, node, trgHigh, lhsHigh, rhs);
      else
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::srawi, node, trgHigh, lhsHigh, rhs);

      cg->stopUsingRegister(tmp);
    } else if (rhs == 32) {
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::mr, node, trgLow, lhsHigh);

      if (isUnsigned)
        generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgHigh, 0);
      else
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::srawi, node, trgHigh, lhsHigh, 31);
    } else // (rhs > 32)
    {
      if (isUnsigned) {
        ...
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::srawi, node, trgLow, lhsHigh, rhs - 32);
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::srawi, node, trgHigh, lhsHigh, 31);
      }
    }
    ...
    if (lhsNode->getOpCode().isLoadConst() && lhsNode->getLongIntHigh() == 0) {
      ...
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::srw, node, trgLow, lhsLow, rhs);
      generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgHigh, 0);
      ...
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::subfic, node, trgLow, rhs, 32);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::slw, node, trgLow, lhsHigh, trgLow);

      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, node, tmp, rhs, -32);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::srw, node, tmp, lhsHigh, tmp);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::OR, node, trgLow, trgLow, tmp);

      // y_1 = x_1 >>> n
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::srw, node, trgHigh, lhsHigh, rhs);
      ...
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::subfic, node, tmp, rhs, 32);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::slw, node, tmp, lhsHigh, tmp);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::OR, node, trgLow, trgLow, tmp);
    ...
    if (lhsNode->getOpCode().isLoadConst() && lhsNode->getLongIntHigh() == 0) {
      ...
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::sraw, node, trgLow, lhsLow, rhs);
      ...
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::subfic, node, tmp1, rhs, 32);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::srawi, node, trgLow, tmp1, 31);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, node, tmp2, rhs, -32);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::sraw, node, tmp2, lhsHigh, tmp2);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::AND, node, trgLow, trgLow, tmp2);

      // y_1 = x_1 >> n
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::sraw, node, trgHigh, lhsHigh, rhs);
      ...
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::subfic, node, tmp1, rhs, 32);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::slw, node, tmp2, lhsHigh, tmp1);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::OR, node, trgLow, trgLow, tmp2);
      ...
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::sraw, node, tmp2, lhsHigh, tmp2);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::srawi, node, tmp1, tmp1, 31);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::AND, node, tmp1, tmp1, tmp2);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::OR, node, trgLow, trgLow, tmp1);
...
TR::Register*
OMR::Power::TreeEvaluator::bshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return integerShiftRight(node, 1, false, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::bushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return integerShiftRight(node, 1, true, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::sshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return integerShiftRight(node, 2, false, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::sushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return integerShiftRight(node, 2, true, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::ishrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return integerShiftRight(node, 4, false, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::iushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return integerShiftRight(node, 4, true, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::lshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  if (TR::Compiler->target.is64Bit())
...
TR::Register*
OMR::Power::TreeEvaluator::lushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  if (TR::Compiler->target.is64Bit())
...
TR::Register*
OMR::Power::TreeEvaluator::irolEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
...
TR::Register*
OMR::Power::TreeEvaluator::lrolEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
void
simplifyANDRegImm(TR::Node* node,
                  TR::Register* trgReg,
                  TR::Register* srcReg,
                  int64_t value,
                  TR::CodeGenerator* cg,
                  TR::Node* constNode = NULL)
{
             ...
             0) // no bits in upper word
  {
    ...
    if (contiguousBits(lowWord)) {
      generateTrg1Src1Imm2Instruction(
        cg, TR::InstOpCode::rlwinm, node, trgReg, srcReg, 0, lowWord);
      if (((lowWord & 0x80000001) == 0x80000001) && (lowWord != 0xffffffff))
        generateTrg1Src1Imm2Instruction(
          cg, TR::InstOpCode::rlwinm, node, trgReg, trgReg, 0, 0xffffffff);
    } else
  ...
  } else // bits in upper word
  {
    if (contiguousBits(value)) {
      ...
      if ((lz == false) && (tz == true)) {
        generateTrg1Src1Imm2Instruction(
          cg, TR::InstOpCode::rldicr, node, trgReg, srcReg, 0, value);
        return;
      } else if ((lz == true) && (tz == false)) {
        generateTrg1Src1Imm2Instruction(
          cg, TR::InstOpCode::rldicl, node, trgReg, srcReg, 0, value);
        return;
      }
    }
    TR::Register* tmpReg;
    ...
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::AND, node, trgReg, srcReg, tmpReg);
                  ...
                  TR::Node* constNode)
{
  ...
  if (localVal.getValue() == 0) {
    ...
    generateTrg1Src1Imm2Instruction(
      cg, TR::InstOpCode::rlwinm, node, trgReg, srcReg, 0, localVal.getValue());
  } else if (localVal.getHighBits() == 0) {
                  ...
                  TR::CodeGenerator* cg)
{
  ...
  if (lowSpecial == true || highSpecial == true) {
    simplifyANDRegImm(node,
                      trgReg->getLowOrder(),
                      srcReg->getLowOrder(),
                      constNode->getLongIntLow(),
                      cg);
    simplifyANDRegImm(node,
                      trgReg->getHighOrder(),
                      srcReg->getHighOrder(),
                      constNode->getLongIntHigh(),
                      cg);
  } else {
...
TR::Register*
OMR::Power::TreeEvaluator::landEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  TR::Register* trgReg = NULL;
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if ((secondOp == TR::lconst || secondOp == TR::luconst) &&
        secondChild->getRegister() == NULL) {
      simplifyANDRegImm(
        node, trgReg, src1Reg, secondChild->getLongInt(), cg, secondChild);
    } else {
      TR::Register* src2Reg = cg->evaluate(secondChild);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::AND, node, trgReg, src2Reg, src1Reg);
    }
  } else // 32 bit target
  {
    if ((secondOp == TR::lconst || secondOp == TR::luconst) &&
      ...
      if (firstChild->isHighWordZero() || secondChild->isHighWordZero()) {
        return carrylessLongEvaluatorWithAnalyser(node,
                                                  cg,
                                                  TR::InstOpCode::AND,
                                                  TR::InstOpCode::AND,
                                                  TR::InstOpCode::mr);
  ...
  if (TR::Compiler->target.is64Bit()) {
    if ((secondOp == TR::lconst || secondOp == TR::luconst) &&
      ...
      if ((node->getOpCodeValue() == TR::lxor) && longConst == (int64_t)-1) {
        ...
        src1Reg = cg->evaluate(firstChild);
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::subfic, node, trgReg, src1Reg, longConst);
      } else if ((longConst & 0xffffffff) ==
                 longConst) // upper 32 bits are all zero
      {
        return iorTypeEvaluator(
          node, immedOp, immedShiftedOp, regOp, regOp_r, cg);
      }
    }
    if (trgReg == NULL) {
      TR::Register* src2Reg = cg->evaluate(secondChild);
    ...
    if ((secondOp == TR::lconst || secondOp == TR::luconst) &&
      ...
      if (localVal.getValue() == 0) {
        ...
        generateTrg1ImmInstruction(
          cg, TR::InstOpCode::li, node, trgReg->getLowOrder(), -1);
      } else {
      ...
      if (localVal.getValue() == 0) {
        generateTrg1Src1Instruction(cg,
                 ...
                 (node->getOpCodeValue() == TR::lor)) {
        generateTrg1ImmInstruction(
          cg, TR::InstOpCode::li, node, trgReg->getHighOrder(), -1);
      } else {
        if (localVal.getHighBits() == 0) {
          generateTrg1Src1ImmInstruction(cg,
                                         immedOp,
...
TR::Register*
OMR::Power::TreeEvaluator::lorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::Power::TreeEvaluator::lxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::ILOpCodes secondOp = node->getSecondChild()->getOpCodeValue();
...
// also handles iuand
TR::Register*
OMR::Power::TreeEvaluator::iandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* trgReg = cg->allocateRegister();
  TR::Node* secondChild = node->getSecondChild();
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (cg->isRotateAndMask(node)) {
      ...
      simplifyANDRegImm(node,
                        ...
                        (int32_t)secondChild->get64bitIntegralValue(),
                        cg,
                        secondChild);
  } else {
    generateTrg1Src2Instruction(cg,
...
TR::Register*
OMR::Power::TreeEvaluator::iorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return iorTypeEvaluator(node,
                          TR::InstOpCode::ori,
                          TR::InstOpCode::oris,
                          TR::InstOpCode::OR,
                          TR::InstOpCode::or_r,
...
TR::Register*
OMR::Power::TreeEvaluator::ixorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return iorTypeEvaluator(node,
                          TR::InstOpCode::xori,
                          TR::InstOpCode::xoris,
...
TR::Register*
OMR::Power::TreeEvaluator::ixfrsEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  TR::Register* src1Reg = cg->evaluate(firstChild);
  ...
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::srawi, node, tmp1Reg, src1Reg, 31);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::srawi, node, tmp2Reg, src2Reg, 31);
  // trg = absolute value of the first child
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::XOR, node, trgReg, src1Reg, tmp1Reg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::subf, node, trgReg, tmp1Reg, trgReg);
  // apply sign of the second child to trg
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::XOR, node, trgReg, trgReg, tmp2Reg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::subf, node, trgReg, tmp2Reg, trgReg);
...
TR::Register*
OMR::Power::TreeEvaluator::lxfrsEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  TR::Register* src1Reg = cg->evaluate(firstChild);
  ...
  if (TR::Compiler->target.is32Bit()) {
    ...
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::srawi, node, tmp1Reg, src1Reg->getHighOrder(), 31);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::srawi, node, tmp2Reg, src2Reg->getHighOrder(), 31);
    // trg = absolute value of the first child
    generateTrg1Src2Instruction(cg,
                                TR::InstOpCode::XOR,
                                node,
...
TR::Register*
OMR::Power::TreeEvaluator::idozEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::xoris, node, tmp1Reg, src1Reg, 0x8000);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::xoris, node, tmp2Reg, src2Reg, 0x8000);

  // tmp1Reg = a - b
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::subfc, node, tmp1Reg, tmp2Reg, tmp1Reg);

  // tmp2Reg = -1 if a <= b
  //           0  if a >  b
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::subfe, node, tmp2Reg, tmp1Reg, tmp1Reg);

  TR::Register* trgReg = cg->allocateRegister();
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::andc, node, trgReg, tmp1Reg, tmp2Reg);
