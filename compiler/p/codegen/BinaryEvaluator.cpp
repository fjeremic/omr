    ...
    if (!isNonZero && !useBranchless) {
      if (n->getFirstChild()->getOpCode().isShift() && n->getFirstChild()->getFirstChild()->getRegister()) {
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::cmpli8, node, condReg, n->getFirstChild()->getFirstChild()->getRegister(), NULLVALUE);
      } else {
...
TR::Register* OMR::Power::TreeEvaluator::iaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.cpu.id() >= TR_PPCp9 && firstChild->getOpCodeValue() == TR::imul
  ...
  } else // 64 bit target
  {
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCp9 && !setsOrReadsCC
      ...
      // to be an lconst
        ...
        if (setsOrReadsCC) {
          TR_ASSERT(node->getOpCodeValue() == TR::ladd || node->getOpCodeValue() == TR::luadd
  ...
  if (cg->comp()->useCompressedPointers() && node->containsCompressionSequence()) {
    ...
    if (!setsOrReadsCC && (secondChild->getOpCode().isLoadConst()) && secondChild->getRegister() == NULL) {
      ...
      } else // no constants or setsOrReadsCC
      {
        ...
        if (setsOrReadsCC) {
          TR_ASSERT(node->getOpCodeValue() == TR::lsub || node->getOpCodeValue() == TR::lusub
...
TR::Register* OMR::Power::TreeEvaluator::lrolEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    TR::Register* shiftAmountReg = cg->evaluate(secondChild);
    generateTrg1Src2ImmInstruction(
      cg, TR::InstOpCode::rldcl, node, targetRegister, srcRegister, shiftAmountReg, CONSTANT64(0xffffffffffffffff));
  }
...
void simplifyANDRegImm(
  TR::Node* node, TR::Register* trgReg, TR::Register* srcReg, int32_t value, TR::CodeGenerator* cg, TR::Node* constNode)
{
  ...
  } else // 32 bit target
  {
    if ((secondOp == TR::lconst || secondOp == TR::luconst) && secondChild->getRegister() == NULL) {
      ...
      if (firstChild->isHighWordZero() || secondChild->isHighWordZero()) {
        ...
        trgReg = cg->allocateRegisterPair(cg->allocateRegister(), cg->allocateRegister());
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::AND, node, trgReg->getLowOrder(), src2Reg->getLowOrder(), src1Reg->getLowOrder());
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::AND, node, trgReg->getHighOrder(), src2Reg->getHighOrder(), src1Reg->getHighOrder());
      }
    }
  }
  node->setRegister(trgReg);
    ...
    if ((secondOp == TR::lconst || secondOp == TR::luconst) && secondChild->getRegister() == NULL) {
      ...
      if (localVal.getValue() == 0) {
        ...
        if (localVal.getHighBits() == 0) {
          ...
          generateTrg1Src1ImmInstruction(
            cg, immedShiftedOp, node, trgReg->getLowOrder(), src1Reg->getLowOrder(), localVal.getHighBits());
        } else {
          TR::Register* tempReg = cg->allocateRegister();
      ...
      if (localVal.getValue() == 0) {
        ...
        if (localVal.getHighBits() == 0) {
          ...
          generateTrg1Src1ImmInstruction(
            cg, immedShiftedOp, node, trgReg->getHighOrder(), src1Reg->getHighOrder(), localVal.getHighBits());
        } else {
          TR::Register* tempReg = cg->allocateRegister();
...
TR::Register* OMR::Power::TreeEvaluator::lxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if ((node->getFirstChild()->isHighWordZero() || node->getSecondChild()->isHighWordZero())
    && !((secondOp == TR::lconst || secondOp == TR::luconst) && node->getSecondChild()->getRegister() == NULL)
    && !(TR::Compiler->target.is64Bit())) {
    return carrylessLongEvaluatorWithAnalyser(node, cg, TR::InstOpCode::XOR, TR::InstOpCode::XOR, TR::InstOpCode::mr);
  } else {
    return lorTypeEvaluator(
...
TR::Register* OMR::Power::TreeEvaluator::lxfrsEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is32Bit()) {
    ...
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::XOR, node, trgReg->getHighOrder(), src1Reg->getHighOrder(), tmp1Reg);
    generateTrg1Src2Instruction(cg, TR::InstOpCode::subfc, node, trgReg->getLowOrder(), tmp1Reg, trgReg->getLowOrder());
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::subfe, node, trgReg->getHighOrder(), tmp1Reg, trgReg->getHighOrder());
    // apply sign of the second child to trg
    ...
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::subfe, node, trgReg->getHighOrder(), tmp2Reg, trgReg->getHighOrder());
  } else {
    trgReg = cg->allocateRegister();
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi, node, tmp1Reg, src1Reg, 31);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi, node, tmp2Reg, src2Reg, 31);
