...
struct TR_tocHashEntry {
  ...
  intptrj_t
      _keyTag; // class-loader or anonClass used as key2 for hashed "names"
  int32_t _flag;
...
class TR_PPCTableOfConstants : public TableOfConstants {
  ...
  int32_t _upCursor, _downCursor, _upCursorAfterPermanentEntries,
      _downCursorAfterPermanentEntries;
...
public:
  ...
  static void *initTOC(TR_FrontEnd *vm, TR::PersistentInfo *,
                       uintptrj_t systemTOC);
  static void reinitializeMemory();
  static void shutdown(TR_FrontEnd *vm);
  static int32_t lookUp(int32_t val, struct TR_tocHashEntry *lk, int32_t *s,
                        TR::CodeGenerator *cg); // return index
  static int32_t lookUp(int8_t *n, int32_t len, bool isAddr, intptrj_t loader,
                        TR::CodeGenerator *cg); // return index
  static int32_t lookUp(TR::SymbolReference *symRef,
                        TR::CodeGenerator *);             // return index
  ...
  static int32_t allocateChunk(uint32_t numEntries, TR::CodeGenerator *cg,
                               bool grabMonitor = true);
