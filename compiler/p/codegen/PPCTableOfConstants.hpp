...
public:
  ...
  static void* initTOC(TR_FrontEnd* vm,
                       TR::PersistentInfo*,
                       uintptrj_t systemTOC);
  static void reinitializeMemory();
  static void shutdown(TR_FrontEnd* vm);
  static int32_t lookUp(int32_t val,
                        struct TR_tocHashEntry* lk,
                        int32_t* s,
                        TR::CodeGenerator* cg); // return index
  static int32_t lookUp(int8_t* n,
                        int32_t len,
                        bool isAddr,
                        intptrj_t loader,
                        TR::CodeGenerator* cg); // return index
  ...
  static int32_t allocateChunk(uint32_t numEntries,
                               TR::CodeGenerator* cg,
...
inline TR_PPCTableOfConstants*
toPPCTableOfConstants(void* p)
{
