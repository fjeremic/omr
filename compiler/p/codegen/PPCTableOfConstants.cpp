...
void*
TR_PPCTableOfConstants::initTOC(TR_FrontEnd* fe,
...
void
TR_PPCTableOfConstants::reinitializeMemory()
{
  TR::PersistentInfo* pinfo =
  ...
  memset(tocManagement->getTOCBase() +
         ...
         (tocManagement->getTOCSize() >> 1) -
           tocManagement->getDownCursorAfterPermanentEntries() *
             sizeof(uintptrj_t));
  memset(tocManagement->getTOCPtr(),
         0,
         (tocManagement->getTOCSize() >> 1) -
...
void
TR_PPCTableOfConstants::shutdown(TR_FrontEnd* fe)
{
  TR_ASSERT(0, "shutdown only enabled for zEmulator.\n");
}

static int32_t
hashValue(int8_t* key, int32_t len)
{
...
int32_t
TR_PPCTableOfConstants::lookUp(int32_t val,
...
int32_t
TR_PPCTableOfConstants::lookUp(int8_t* name,
                               int32_t len,
                               bool isAddr,
                               intptrj_t keyTag,
                               TR::CodeGenerator* cg)
...
int32_t
TR_PPCTableOfConstants::lookUp(double dvalue, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
...
int32_t
TR_PPCTableOfConstants::lookUp(float fvalue, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
...
int32_t
TR_PPCTableOfConstants::lookUp(TR::SymbolReference* symRef,
                               TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
...
int32_t
TR_PPCTableOfConstants::allocateChunk(uint32_t numEntries,
                                      TR::CodeGenerator* cg,
                                      bool grabMonitor)
{
  TR_PPCTableOfConstants* tocManagement = toPPCTableOfConstants(
    TR_PersistentMemory::getNonThreadSafePersistentInfo()->getPersistentTOC());
...
uintptrj_t
TR_PPCTableOfConstants::getTOCSlot(int32_t offset)
{
  TR_PPCTableOfConstants* tocManagement = toPPCTableOfConstants(
    TR_PersistentMemory::getNonThreadSafePersistentInfo()->getPersistentTOC());
  uintptrj_t* base =
    tocManagement->getTOCBase(); // no lock needed because base does not change

  return base[offset / sizeof(uintptrj_t)];
}

void
TR_PPCTableOfConstants::setTOCSlot(int32_t offset, uintptrj_t v)
{
  TR_PPCTableOfConstants* tocManagement = toPPCTableOfConstants(
    TR_PersistentMemory::getNonThreadSafePersistentInfo()->getPersistentTOC());
...
void
TR_PPCTableOfConstants::permanentEntriesAddtionComplete() // only zEmulator uses
                                                          // this
{
  TR_PPCTableOfConstants* tocManagement = toPPCTableOfConstants(
    TR_PersistentMemory::getNonThreadSafePersistentInfo()->getPersistentTOC());
...
bool
TR_PPCTableOfConstants::isPermanentEntriesAddtionComplete() // only zEmulator
                                                            // uses this
{
  bool retValue;
  TR_PPCTableOfConstants* tocManagement = toPPCTableOfConstants(
