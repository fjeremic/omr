...
void* TR_PPCTableOfConstants::initTOC(TR_FrontEnd* fe, TR::PersistentInfo* pinfo, uintptrj_t systemTOC)
{
  ...
  // Set Down Cursor to be TR_PPCnumRuntimeHelpers+1, so nothing between [0..TR_PPCnumRuntimeHelpers] should be used for
  // hashtable.
...
void TR_PPCTableOfConstants::reinitializeMemory()
{
  ...
  memset(tocManagement->getTOCPtr(), 0,
    (tocManagement->getTOCSize() >> 1) - (tocManagement->getUpCursorAfterPermanentEntries() + 1) * sizeof(uintptrj_t));
  ...
  *offsetInSlot = 0;
  ...
  while (chain != CHAIN_END) {
    ...
    case TR_FLAG_tocNameKey:
      if ((strcmp((char*)hash[idx]._nKey, (char*)tmplate->_nKey) == 0) && (hash[idx]._keyTag == tmplate->_keyTag)) {
  ...
  case TR_FLAG_tocNameKey: {
    ...
    if (tocManagement->getNameACursor() - tocManagement->getNameAStart() + nlen + 1 < tocManagement->getNameASize()) {
  ...
  case TR_FLAG_tocFloatKey:
    ...
    if (TR::Compiler->target.cpu.isBigEndian())
      slotValue = (*offsetInSlot == 0) ? (((uintptrj_t)hash[idx]._fKey) << 32)
                                       : (getTOCSlot(rval * sizeof(intptrj_t)) | hash[idx]._fKey);
    else
      slotValue = (*offsetInSlot == 0) ? ((uintptrj_t)hash[idx]._fKey)
                                       : (getTOCSlot(rval * sizeof(intptrj_t)) | (((uintptrj_t)hash[idx]._fKey) << 32));
