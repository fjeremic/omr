...
void *TR_PPCTableOfConstants::initTOC(TR_FrontEnd *fe,
                                      TR::PersistentInfo *pinfo,
                                      uintptrj_t systemTOC) {
  int32_t tsize = TR::Options::getCmdLineOptions()->getTOCSize();

  TR_ASSERT(pinfo->getPersistentTOC() == 0,
            "TOC needs to be initialized once only.\n");

  if (tsize == 0) return (void *)0x1;  // error code
  ...
  TR_PPCTableOfConstants *tocManagement =
      new (PERSISTENT_NEW) TR_PPCTableOfConstants(tsize);
  if (!tocManagement) return (void *)0x1;  // error code
  pinfo->setPersistentTOC(tocManagement);

  // Allocate the TOC memory: use relocationData for now -- we will get a
  // specific allocator
  uint8_t *tocPtr;
  tocPtr = fe->allocateRelocationData(0, tsize);
  if (!tocPtr)
    return (
        void *)0x1;  // error code // FIXME: should set persistent toc to null
...
#if defined(JITTEST)
  // Store at the index of TR_PPCnumRuntimeHelpers, the systemTOC. Note: Slot
  // tocBase[TR_PPCnumRuntimeHelpers-1] is free. See
  // TR::TreeEvaluator::restoreTOCRegister.
  tocBase[TR_PPCnumRuntimeHelpers] = systemTOC;
  // Set Down Cursor to be TR_PPCnumRuntimeHelpers+1, so nothing between
  // [0..TR_PPCnumRuntimeHelpers] should be used for hashtable.
...
void TR_PPCTableOfConstants::reinitializeMemory() {
  TR::PersistentInfo *pinfo =
      TR_PersistentMemory::getNonThreadSafePersistentInfo();
  TR_PPCTableOfConstants *tocManagement =
      toPPCTableOfConstants(pinfo->getPersistentTOC());

  if (!tocManagement) return;

  // It is not mandatory to clean up this memory, but it may make debugging
  // easier should it be required
  memset(tocManagement->getTOCBase() +
             tocManagement->getDownCursorAfterPermanentEntries(),
         0,
  ...
  memset(tocManagement->getTOCPtr(), 0,
         (tocManagement->getTOCSize() >> 1) -
             (tocManagement->getUpCursorAfterPermanentEntries() + 1) *
                 sizeof(uintptrj_t));
...
static int32_t hashValue(int8_t *key, int32_t len) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (feed != 0) value ^= feed >> 24;
    value &= ~feed;
  }
  return value;
}

int32_t TR_PPCTableOfConstants::lookUp(int32_t val,
                                       struct TR_tocHashEntry *tmplate,
                                       int32_t *offsetInSlot,
                                       TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();

  if (comp->compileRelocatableCode() ||
      (comp->getOption(TR_EnableHCR) &&
       comp->getOption(TR_HCRPatchClassPointers)) ||
      comp->getOption(TR_MimicInterpreterFrameShape))
    return PTOC_FULL_INDEX;

  if (comp->isOptServer()) {
    if (comp->getMethodHotness() < warm || comp->isDLT() ||
        comp->isProfilingCompilation() || cg->getCurrentBlock()->isCold())
      return PTOC_FULL_INDEX;
  }
  ...
  if (tocManagement == NULL) return PTOC_FULL_INDEX;
  ...
  *offsetInSlot = 0;
    ...
    switch (hash[idx]._flag & tmplate->_flag) {
      ...
      case TR_FLAG_tocFloatKey:
        if (hash[idx]._fKey == tmplate->_fKey) {
          if (hash[idx]._flag & TR_FLAG_tocFloatHigh) *offsetInSlot = 4;
          tocManagement->getTOCMonitor()->exit();
          return rval;
        }
        break;
  ...
  if (rval == PTOC_FULL_INDEX) {
    if (restoreIdx != CHAIN_END) hash[restoreIdx]._collisionChain = CHAIN_END;
    if (collisionArea != 0) tocManagement->setCollisionCursor(idx);
    hash[idx]._flag = 0;
    tocManagement->getTOCMonitor()->exit();
    return rval;
  }
  ...
  switch (tmplate->_flag) {
    case TR_FLAG_tocNameKey: {
      ...
      if (tocManagement->getNameACursor() - tocManagement->getNameAStart() +
              nlen + 1 <
      ...
      if (hash[idx]._nKey != NULL)
        ...
        // Effectively making this entry not shared, since _staicCPIndex is a
        // null string.
    ...
    case TR_FLAG_tocFloatKey:
      ...
      if (TR::Compiler->target.cpu.isBigEndian())
        slotValue =
            (*offsetInSlot == 0)
                ? (((uintptrj_t)hash[idx]._fKey) << 32)
                : (getTOCSlot(rval * sizeof(intptrj_t)) | hash[idx]._fKey);
      else
        slotValue = (*offsetInSlot == 0)
                        ? ((uintptrj_t)hash[idx]._fKey)
                        : (getTOCSlot(rval * sizeof(intptrj_t)) |
                           (((uintptrj_t)hash[idx]._fKey) << 32));
...
int32_t TR_PPCTableOfConstants::lookUp(int8_t *name, int32_t len, bool isAddr,
                                       intptrj_t keyTag,
                                       TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  ...
  if (isAddr) {
    ...
    *(int64_t *)&seed[4] ^= *(intptrj_t *)name;
    ...
    // For name entries, we will also use the loader/anonClass as key2, in
...
int32_t TR_PPCTableOfConstants::lookUp(double dvalue, TR::CodeGenerator *cg) {
  ...
  *(int64_t *)seed ^= *(int64_t *)&dvalue;
  ...
  if (tindex == PTOC_FULL_INDEX) return tindex;
...
int32_t TR_PPCTableOfConstants::lookUp(TR::SymbolReference *symRef,
                                       TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  ...
  if (!symRef->isUnresolved() || symRef->getCPIndex() < 0 ||
      sym->isAddressOfClassObject() || sym->isConstObjectRef() ||
      sym->isConst()) {
    ...
    *(intptrj_t *)local_buffer = addr;
    ...
    TR_OpaqueClassBlock *myClass =
        comp->getOwningMethodSymbol(symRef->getOwningMethodIndex())
            ->getResolvedMethod()
            ->containingClass();
    ...
    if (sym->isClassObject()) {
      ...
      if (sym->addressIsCPIndexOfStatic()) {
        ...
        st2cEntry._cpKey =
            (intptrj_t)symRef->getOwningMethod(comp)->constantPool();
        ...
        className =
            (int8_t *)TR::Compiler->cls.classNameChars(comp, symRef, nlen);
      }
      ...
      name =
          (int8_t *)comp->getOwningMethodSymbol(symRef->getOwningMethodIndex())
              ->getResolvedMethod()
...
int32_t TR_PPCTableOfConstants::allocateChunk(uint32_t numEntries,
                                              TR::CodeGenerator *cg,
                                              bool grabMonitor) {
  TR_PPCTableOfConstants *tocManagement = toPPCTableOfConstants(
      TR_PersistentMemory::getNonThreadSafePersistentInfo()
          ->getPersistentTOC());

  if (tocManagement == NULL || cg->comp()->compileRelocatableCode() ||
      (cg->comp()->getOption(TR_EnableHCR) &&
       cg->comp()->getOption(TR_HCRPatchClassPointers)))
    return PTOC_FULL_INDEX;

  if (grabMonitor) tocManagement->getTOCMonitor()->enter();
  ...
  // Should post an event, fail this compilation, and relocate the TOC ---
  // future work
  result = PTOC_FULL_INDEX;
  /* fall through */

Lexit:
  if (grabMonitor) tocManagement->getTOCMonitor()->exit();
  return result;
}
...
void TR_PPCTableOfConstants::onClassUnloading(void *tagPtr) {
  ...
  for (idx = 0; idx < endIx; idx++) {
    if ((hash[idx]._flag & (TR_FLAG_tocNameKey | TR_FLAG_tocStatic2ClassKey)) &&
        hash[idx]._keyTag == (intptrj_t)tagPtr) {
      hash[idx]._keyTag = -1;
...
uintptrj_t TR_PPCTableOfConstants::getTOCSlot(int32_t offset) {
  ...
  uintptrj_t *base =
      tocManagement
          ->getTOCBase();  // no lock needed because base does not change
...
void TR_PPCTableOfConstants::
    permanentEntriesAddtionComplete()  // only zEmulator uses this
{
  TR_PPCTableOfConstants *tocManagement = toPPCTableOfConstants(
      TR_PersistentMemory::getNonThreadSafePersistentInfo()
          ->getPersistentTOC());
  ...
  tocManagement->setDownCursorAfterPermanentEntries(
      tocManagement->getDownCursor());
  tocManagement->setPermanentEntriesAddtionComplete(true);
  tocManagement->getTOCMonitor()->exit();
}
bool TR_PPCTableOfConstants::
    isPermanentEntriesAddtionComplete()  // only zEmulator uses this
{
  bool retValue;
  TR_PPCTableOfConstants *tocManagement = toPPCTableOfConstants(
      TR_PersistentMemory::getNonThreadSafePersistentInfo()
