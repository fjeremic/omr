...
static bool reversedConditionalBranchOpCode(TR::InstOpCode::Mnemonic op,
...
uint8_t *TR::PPCAlignmentNopInstruction::generateBinaryEncoding() {
  ...
  if (currentMisalign) {
    ...
    if (trace)
      traceMsg(cg()->comp(),
               "Expanding alignment nop %p into %u instructions: [ %p ", self(),
               nopsToAdd, lastNop);
  ...
  if (label) {
    if (label->getCodeLocation() != NULL) {
      ...
      } else // too far - need fix up
      {
        ...
        *(int32_t *)cursor |=
            ((label->getCodeLocation() - cursor) & 0x03fffffc);
      }
    } else {
      if (!getFarRelocation()) {
        insertConditionRegister((uint32_t *)cursor);
...
int32_t TR::PPCConditionalBranchInstruction::estimateBinaryLength(
  ...
  if (needsAOTRelocation()) {
    ...
    case TR_AbsoluteHelperAddress:
      cg()->addExternalRelocation(
          new (cg()->trHeapMemory())
              TR::ExternalRelocation(cursor, (uint8_t *)getSymbolReference(),
...
static TR::Instruction *loadReturnAddress(TR::Node *node, uintptrj_t value,
                                          TR::Register *trgReg,
...
static void insertMaskField(uint32_t *instruction, TR::InstOpCode::Mnemonic op,
                            int64_t lmask) {
  int32_t encoding;
...
int32_t TR::PPCVirtualGuardNOPInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
