...
uint8_t* OMR::Power::Instruction::generateBinaryEncoding()
{
  ...
  if (self()->getOpCodeValue() == TR::InstOpCode::depend || self()->getOpCodeValue() == TR::InstOpCode::assocreg) {
    ...
    if (self()->getOpCodeValue() == TR::InstOpCode::genop) {
      TR::RealRegister::RegNum nopreg
        = TR::Compiler->target.cpu.id() > TR_PPCp6 ? TR::RealRegister::gr2 : TR::RealRegister::gr1;
  ...
  if (haveHint()) {
    ...
    if (getOpCode().setsCTR())
      *(int32_t*)instructionStart
        |= (getLikeliness() ? PPCOpProp_BranchLikelyMaskCtr : PPCOpProp_BranchUnlikelyMaskCtr);
    else
...
void TR::PPCImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  if (needsAOTRelocation()) {
    ...
    case TR_AbsoluteHelperAddress:
      cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                    cursor, (uint8_t*)getSymbolReference(), TR_AbsoluteHelperAddress, cg()),
  ...
  if (std::find(comp->getStaticPICSites()->begin(), comp->getStaticPICSites()->end(), this)
    ...
    void* addressToPatch
      = TR::Compiler->target.is64Bit() ? (TR::Compiler->target.cpu.isBigEndian() ? cursor : (cursor - 4)) : cursor;
...
uint8_t* TR::PPCSrc1Instruction::generateBinaryEncoding()
{
  ...
  if (getOpCodeValue() == TR::InstOpCode::mtfsf | getOpCodeValue() == TR::InstOpCode::mtfsfl
...
static TR::Instruction* loadReturnAddress(
  TR::Node* node, uintptrj_t value, TR::Register* trgReg, TR::Instruction* cursor)
{
...
void TR::PPCTrg1ImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  if (std::find(comp->getStaticPICSites()->begin(), comp->getStaticPICSites()->end(), this)
    ...
    cg()->jitAddPicToPatchOnClassUnload(
      (void*)(TR::Compiler->target.is64Bit() ? node->getLongInt() : node->getInt()), (void*)cursor);
  }
    ...
    cg()->jitAddPicToPatchOnClassUnload(
      (void*)(cg()
                ...
                ->createResolvedMethod(cg()->trMemory(),
                  (TR_OpaqueMethodBlock*)(TR::Compiler->target.is64Bit() ? node->getLongInt() : node->getInt()),
...
uint8_t* TR::PPCTrg1ImmInstruction::generateBinaryEncoding()
{
  ...
  if (getOpCodeValue() == TR::InstOpCode::mtcrf || getOpCodeValue() == TR::InstOpCode::mfocrf) {
    *((int32_t*)cursor) |= (getSourceImmediate() << 12);
    if ((TR::Compiler->target.cpu.id() >= TR_PPCgp) && ((getSourceImmediate() & (getSourceImmediate() - 1)) == 0))
...
static void insertMaskField(uint32_t* instruction, TR::InstOpCode::Mnemonic op, int64_t lmask)
{
  ...
  if (opCode.isDoubleWord()) {
    ...
    if (opCode.useMaskEnd()) {
      TR_ASSERT(contiguousBits(lmask) && ((lmask & CONSTANT64(0x8000000000000000)) != 0)
          && ((lmask == -1) || ((lmask & 0x1) == 0)),
      ...
      TR_ASSERT((bitnum != 0) || (lmask == -1) || ((lmask & 0x1) == 0)
          || (op != TR::InstOpCode::rldic && op != TR::InstOpCode::rldimi && op != TR::InstOpCode::rldic_r
