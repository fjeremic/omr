...
void TR::PPCImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (std::find(comp->getStaticPICSites()->begin(), comp->getStaticPICSites()->end(), this) != comp->getStaticPICSites()->end()) {
        ...
        void* addressToPatch = TR::Compiler->target.is64Bit() ? (TR::Compiler->target.cpu.isBigEndian() ? cursor : (cursor - 4)) : cursor;
...
uint8_t* TR::PPCSrc1Instruction::generateBinaryEncoding()
{
    ...
    if (getOpCodeValue() == TR::InstOpCode::mtfsf | getOpCodeValue() == TR::InstOpCode::mtfsfl || getOpCodeValue() == TR::InstOpCode::mtfsfw) {
...
uint8_t* TR::PPCTrg1Src1ImmInstruction::generateBinaryEncoding()
{
    ...
    insertTargetRegister(toPPCCursor(cursor));
    insertSource1Register(toPPCCursor(cursor));
    if (getOpCodeValue() == TR::InstOpCode::srawi || getOpCodeValue() == TR::InstOpCode::srawi_r || getOpCodeValue() == TR::InstOpCode::sradi || getOpCodeValue() == TR::InstOpCode::sradi_r || getOpCodeValue() == TR::InstOpCode::extswsli) {
...
static void insertMaskField(uint32_t* instruction, TR::InstOpCode::Mnemonic op, int64_t lmask)
{
    ...
    if (opCode.isDoubleWord()) {
        ...
        if (opCode.useMaskEnd()) {
            TR_ASSERT(contiguousBits(lmask) && ((lmask & CONSTANT64(0x8000000000000000)) != 0) && ((lmask == -1) || ((lmask & 0x1) == 0)),
            ...
            TR_ASSERT((bitnum != 0) || (lmask == -1) || ((lmask & 0x1) == 0) || (op != TR::InstOpCode::rldic && op != TR::InstOpCode::rldimi && op != TR::InstOpCode::rldic_r && op != TR::InstOpCode::rldimi_r),
