...
uint8_t *TR::PPCAlignmentNopInstruction::generateBinaryEncoding() {
  ...
  if (currentMisalign) {
    ...
    for (uint32_t i = 1; i < nopsToAdd; i++) {
      ...
      if (trace) traceMsg(cg()->comp(), "%p ", nop);
    }

    if (trace) traceMsg(cg()->comp(), "]\n");
  } else {
    if (trace)
      traceMsg(cg()->comp(),
...
int32_t TR::PPCAlignmentNopInstruction::estimateBinaryLength(
...
uint8_t *TR::PPCConditionalBranchInstruction::generateBinaryEncoding() {
  ...
  if (haveHint()) {
    if (getFarRelocation()) reverseLikeliness();
...
int32_t TR::PPCTrg1ImmInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  setEstimatedBinaryLength(PPC_INSTRUCTION_LENGTH);
  return currentEstimate + getEstimatedBinaryLength();
}
...
void TR::PPCTrg1ImmInstruction::addMetaDataForCodeAddress(uint8_t *cursor) {
  ...
  if (std::find(comp->getStaticPICSites()->begin(),
    ...
    cg()->jitAddPicToPatchOnClassUnload(
        (void *)(TR::Compiler->target.is64Bit() ? node->getLongInt()
                                                : node->getInt()),
        (void *)cursor);
  }
...
int32_t TR::PPCMemSrc1Instruction::estimateBinaryLength(
    int32_t currentEstimate) {
...
int32_t TR::PPCTrg1MemInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  setEstimatedBinaryLength(getMemoryReference()->estimateBinaryLength(*cg()));
  return currentEstimate + getEstimatedBinaryLength();
}
