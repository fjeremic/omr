...
static bool
reversedConditionalBranchOpCode(TR::InstOpCode::Mnemonic op,
                                TR::InstOpCode::Mnemonic* rop);
...
OMR::Power::Instruction::generateBinaryEncoding()
{
  ...
  if (self()->getOpCodeValue() == TR::InstOpCode::depend ||
      self()->getOpCodeValue() == TR::InstOpCode::assocreg) {
  } else {
    cursor = self()->getOpCode().copyBinaryToBuffer(instructionStart);
    if (self()->getOpCodeValue() == TR::InstOpCode::genop) {
      TR::RealRegister::RegNum nopreg = TR::Compiler->target.cpu.id() > TR_PPCp6
                                          ? TR::RealRegister::gr2
                                          : TR::RealRegister::gr1;
...
uint8_t*
TR::PPCAlignmentNopInstruction::generateBinaryEncoding()
{
  bool trace = cg()->comp()->getOption(TR_TraceCG);
  uint32_t currentMisalign =
    reinterpret_cast<uintptr_t>(cg()->getBinaryBufferCursor()) % _alignment;

  if (currentMisalign) {
    uint32_t nopsToAdd =
      (_alignment - currentMisalign) / PPC_INSTRUCTION_LENGTH;

    // For performance reasons, the last nop added might be different than the
    // others, e.g. on P6 and above a group-ending nop is typically used. Since
    // we add nops in reverse order, we add this special nop first. All other
    // padding instructions will be regular nops.
    TR::Instruction* lastNop =
      generateInstruction(cg(), getOpCodeValue(), getNode(), self());
    lastNop->setEstimatedBinaryLength(PPC_INSTRUCTION_LENGTH);

    if (trace)
      traceMsg(cg()->comp(),
               "Expanding alignment nop %p into %u instructions: [ %p ",
               self(),
               nopsToAdd,
               lastNop);
    ...
    for (uint32_t i = 1; i < nopsToAdd; i++) {
      TR::Instruction* nop =
        generateInstruction(cg(), TR::InstOpCode::nop, getNode(), self());
    ...
    if (trace)
      traceMsg(cg()->comp(),
               "Eliminating alignment nop %p, since the next instruction is "
               "already aligned\n",
               self());
  }
  ...
  // When the trace log prints the list of instructions after binary encoding,
  // we don't want this instruction to show up any more. Removing it from the
  // linked list of instructions does this without affecting this instruction's
  // next pointer, so the binary encoding loop can continue and encode the
  // actual nops we emitted as if nothing happened.
  self()->remove();
...
int32_t
TR::PPCAlignmentNopInstruction::estimateBinaryLength(int32_t currentEstimate)
{
...
uint8_t
TR::PPCAlignmentNopInstruction::getBinaryLengthLowerBound()
{
  return 0;
}

uint8_t*
TR::PPCLabelInstruction::generateBinaryEncoding()
{
    ...
    if (label->getCodeLocation() != NULL) {
      *(int32_t*)cursor |= ((label->getCodeLocation() - cursor) & 0x03fffffc);
      ...
      cg()->addRelocation(new (cg()->trHeapMemory())
                            TR::LabelRelative24BitRelocation(cursor, label));
    }
    cursor += 4;
  } else // regular LABEL
  {
...
int32_t
TR::PPCLabelInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  if (getOpCode().isBranchOp()) {
                                ...
                                TR::InstOpCode::Mnemonic* rop)
{
  switch (op) {
    ...
    default:
      TR_ASSERT(0,
                "New PPC conditional branch opcodes have to have corresponding "
                "reversed opcode: %d\n",
                (int32_t)op);
...
uint8_t*
TR::PPCConditionalBranchInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  TR::LabelSymbol* label = getLabelSymbol();
  uint8_t* cursor = instructionStart;
  ...
  if (label) {
    if (label->getCodeLocation() != NULL) {
      ...
      } else // too far - need fix up
      {
        ...
        bool linkForm =
          reversedConditionalBranchOpCode(getOpCodeValue(), &reversedOp);
        TR::InstOpCode reversedOpCode(reversedOp);
        TR::InstOpCode extraOpCode(linkForm ? TR::InstOpCode::bl
                                            : TR::InstOpCode::b);
  ...
  if (haveHint()) {
    ...
    if (getOpCode().setsCTR())
      *(int32_t*)instructionStart |=
        (getLikeliness() ? PPCOpProp_BranchLikelyMaskCtr
                         : PPCOpProp_BranchUnlikelyMaskCtr);
    else
      *(int32_t*)instructionStart |=
        (getLikeliness() ? PPCOpProp_BranchLikelyMask
                         : PPCOpProp_BranchUnlikelyMask);
  }
...
int32_t
TR::PPCConditionalBranchInstruction::estimateBinaryLength(
...
uint8_t*
TR::PPCAdminInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  int i;
...
int32_t
TR::PPCAdminInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  setEstimatedBinaryLength(0);
  return currentEstimate;
}
  ...
  if (std::find(comp->getStaticPICSites()->begin(),
    ...
    void* addressToPatch =
      TR::Compiler->target.is64Bit()
        ? (TR::Compiler->target.cpu.isBigEndian() ? cursor : (cursor - 4))
        : cursor;
...
uint8_t*
TR::PPCImmInstruction::generateBinaryEncoding()
{
  TR::Compilation* comp = cg()->comp();
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
...
uint8_t*
TR::PPCImm2Instruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
uint8_t*
TR::PPCSrc1Instruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
uint8_t*
TR::PPCDepImmInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
uint8_t*
TR::PPCTrg1Instruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
uint8_t*
TR::PPCTrg1Src1Instruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;

  if (isRegCopy() && (toRealRegister(getTargetRegister()) ==
                      toRealRegister(getSource1Register()))) {
  } else {
    cursor = getOpCode().copyBinaryToBuffer(instructionStart);
    insertTargetRegister(toPPCCursor(cursor));
    insertSource1Register(toPPCCursor(cursor));
...
int32_t
TR::PPCTrg1ImmInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  setEstimatedBinaryLength(PPC_INSTRUCTION_LENGTH);
  return currentEstimate + getEstimatedBinaryLength();
}
...
loadReturnAddress(TR::Node* node,
                  uintptrj_t value,
                  TR::Register* trgReg,
...
TR::PPCTrg1ImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  if (std::find(comp->getStaticPICSites()->begin(),
    ...
    cg()->jitAddPicToPatchOnClassUnload((void*)(TR::Compiler->target.is64Bit()
                                                  ? node->getLongInt()
                                                  : node->getInt()),
                                        (void*)cursor);
  }
    ...
    cg()->jitAddPicToPatchOnClassUnload(
      (void*)(cg()
                ...
                ->createResolvedMethod(
                  cg()->trMemory(),
                  (TR_OpaqueMethodBlock*)(TR::Compiler->target.is64Bit()
                                            ? node->getLongInt()
                                            : node->getInt()),
...
uint8_t*
TR::PPCTrg1ImmInstruction::generateBinaryEncoding()
{
...
TR::PPCTrg1Src1ImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  if (std::find(comp->getStaticPICSites()->begin(),
                comp->getStaticPICSites()->end(),
                this) != comp->getStaticPICSites()->end()) {
    cg()->jitAddPicToPatchOnClassUnload((void*)(getSourceImmPtr()),
                                        (void*)cursor);
  }
  if (std::find(comp->getStaticMethodPICSites()->begin(),
                comp->getStaticMethodPICSites()->end(),
    ...
    cg()->jitAddPicToPatchOnClassUnload(
      (void*)(cg()
                ...
                ->createResolvedMethod(
                  cg()->trMemory(),
                  (TR_OpaqueMethodBlock*)(getSourceImmPtr()),
                  comp->getCurrentMethod())
                ->classOfMethod()),
      (void*)cursor);
  }
...
uint8_t*
TR::PPCTrg1Src1ImmInstruction::generateBinaryEncoding()
{
  ...
  insertTargetRegister(toPPCCursor(cursor));
  insertSource1Register(toPPCCursor(cursor));
  if (getOpCodeValue() == TR::InstOpCode::srawi ||
      getOpCodeValue() == TR::InstOpCode::srawi_r ||
      getOpCodeValue() == TR::InstOpCode::sradi ||
      getOpCodeValue() == TR::InstOpCode::sradi_r ||
...
static void
insertMaskField(uint32_t* instruction,
                ...
                int64_t lmask)
{
  ...
  // For these instructions the immediate is not a mask but a 1-bit immediate
  // operand
  ...
  // For these instructions the immediate is not a mask but a 2-bit immediate
  // operand
  if (op == TR::InstOpCode::xxpermdi || op == TR::InstOpCode::xxsldwi) {
    encoding = (((uint32_t)lmask) & 0x3) << 8;
    *instruction |= encoding;
    return;
  }

  if (op == TR::InstOpCode::addex || op == TR::InstOpCode::addex_r) {
    encoding = (((uint32_t)lmask) & 0x3) << 9;
    *instruction |= encoding;
    return;
  }

  // For these instructions the immediate is not a mask but a 4-bit immediate
  // operand
  ...
  if (opCode.isDoubleWord()) {
    ...
    if (opCode.useMaskEnd()) {
      ...
      TR_ASSERT(
        (bitnum != 0) || (lmask == -1) || ((lmask & 0x1) == 0) ||
          (op != TR::InstOpCode::rldic && op != TR::InstOpCode::rldimi &&
           op != TR::InstOpCode::rldic_r && op != TR::InstOpCode::rldimi_r),
  ...
  } else // single word
  {
    ...
    if (op == TR::InstOpCode::drrnd || op == TR::InstOpCode::dqua) {
      ...
      encoding = 32 * maskBegin + maskEnd
                 << 1; // shift encrypted mask into position
    }
  }
  *instruction |= encoding;
}
...
uint8_t*
TR::PPCTrg1Src1Imm2Instruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
uint8_t*
TR::PPCTrg1Src2Instruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
...
uint8_t*
TR::PPCTrg1Src2ImmInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
uint8_t*
TR::PPCTrg1Src3Instruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
uint8_t*
TR::PPCSrc2Instruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
uint8_t*
TR::PPCMemSrc1Instruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
...
uint8_t*
TR::PPCMemInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  getMemoryReference()->mapOpCode(this);
...
int32_t
TR::PPCMemSrc1Instruction::estimateBinaryLength(int32_t currentEstimate)
{
...
uint8_t*
TR::PPCTrg1MemInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  ...
  // The control for the different values is done through asserts in the
  // constructor
  if (haveHint()) {
...
int32_t
TR::PPCTrg1MemInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  setEstimatedBinaryLength(getMemoryReference()->estimateBinaryLength(*cg()));
  return currentEstimate + getEstimatedBinaryLength();
}
...
uint8_t*
TR::PPCControlFlowInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
int32_t
TR::PPCControlFlowInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  switch (getOpCodeValue()) {
    ...
    default:
      TR_ASSERT(false,
                "unknown control flow instruction (estimateBinaryLength)");
  }
  return currentEstimate + getEstimatedBinaryLength();
}
...
#ifdef J9_PROJECT_SPECIFIC
uint8_t*
TR::PPCVirtualGuardNOPInstruction::generateBinaryEncoding()
{
  ...
  if (cg()->sizeOfInstructionToBePatched(this) == 0 ||
      // AOT needs an explicit nop, even if there are patchable instructions at
      // this site because 1) Those instructions might have AOT data relocations
      // (and therefore will be incorrectly patched again) 2) We might want to
      // re-enable the code path and unpatch, in which case we would have to
      // know what the old instruction was
...
int32_t
TR::PPCVirtualGuardNOPInstruction::estimateBinaryLength(int32_t currentEstimate)
{
