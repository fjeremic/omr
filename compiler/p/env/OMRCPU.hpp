...
#define VALID_PROCESSOR                                                        \
  TR_ASSERT(id() >= TR_FirstPPCProcessor && id() <= TR_LastPPCProcessor,       \
            "Not a valid PPC Processor Type")
...
public:
  ...
  /** @brief Determines whether the Transactional Memory (TM) facility is
   * available on the current processor. Alias of getPPCSupportsTM() as a
   * platform agnostic query.
   *
   ...
   */
  bool isTargetWithinIFormBranchRange(intptrj_t targetAddress,
