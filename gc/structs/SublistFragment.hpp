...
public:
   ...
   * Disconnects the fragment from the reserved area in the sublist.  New
   * allocates will require a refresh of the fragment.  Allows the sublist to be
   * manipulated by other routines (no cached pointers).
   */
  MMINLINE static void flush(J9VMGC_SublistFragment* sublistFragment)
  {
    /* Pool count needs to be incremented */
    ((MM_SublistPool*)sublistFragment->parentList)
      ->incrementCount(sublistFragment->count);
