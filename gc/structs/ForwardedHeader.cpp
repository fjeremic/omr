...
#if defined(FORWARDEDHEADER_DEBUG)
void MM_ForwardedHeader::Assert(bool condition, const char* assertion, const char* file, uint32_t line)
{
    if (!condition) {
...
void MM_ForwardedHeader::ForwardedHeaderDump(omrobjectptr_t destinationObjectPtr)
{
#if defined(OMR_GC_COMPRESSED_POINTERS)
    fprintf(stderr, "MM_ForwardedHeader@%p[%p(%p):%x:%x] -> %p(%p)\n", this, _objectPtr, (uintptr_t*)(*_objectPtr),
        _preserved.slot, _preserved.overlap, destinationObjectPtr, (uintptr_t*)(*destinationObjectPtr));
#else /* defined (OMR_GC_COMPRESSED_POINTERS) */
    fprintf(stderr, "MM_ForwardedHeader@%p[%p(%p):%x] -> %p(%p)\n", this, _objectPtr, (uintptr_t*)(*_objectPtr),
        _preserved.slot, destinationObjectPtr, (uintptr_t*)(*destinationObjectPtr));
    ...
    omrobjectptr_t destinationObjectPtr, uintptr_t forwardedTag)
{
    ...
    uintptr_t newValue = (((uintptr_t)destinationObjectPtr | forwardedTag) << 32)
    ...
    if (MM_AtomicOperations::lockCompareExchange((volatile uintptr_t*)&objectHeader->slot, oldValue, newValue)
        != oldValue) {
        /* If we lost forwarding it, return where we are really forwarded. Another thread could raced us to forward on
         * another location or (Concurrent world) self-forward it. In the later case, we will return NULL */
...
uintptr_t MM_ForwardedHeader::copySetup(omrobjectptr_t destinationObjectPtr, uintptr_t* remainingSizeToCopy)
{
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    volatile MutableHeaderFields* objectHeader
        = (volatile MutableHeaderFields*)((fomrobject_t*)destinationObjectPtr + _forwardingSlotOffset);
...
void MM_ForwardedHeader::copySection(
    omrobjectptr_t destinationObjectPtr, uintptr_t remainingSizeToCopy, uintptr_t sizeToCopy)
{
...
omrobjectptr_t MM_ForwardedHeader::setSelfForwardedObject()
{
    ...
    if (oldValue != lockCompareExchangeObjectHeader(&objectHeader->slot, oldValue, newValue)) {
        /* If we lost on self-forwarding, return where we are really forwarded. We could still be self-forwarded
         * (another thread raced us) or
         * strictly forwarded (another thread successfully copied the object). Either way, getNonStrictForwardedObject()
         * should return us where we really are. */
...
uintptr_t MM_ForwardedHeader::winObjectSectionToCopy(volatile fomrobject_t* copyProgressSlot, fomrobject_t oldValue,
    uintptr_t* remainingSizeToCopy, uintptr_t outstandingCopies)
{
    ...
    *remainingSizeToCopy -= sizeToCopy;
    ...
    fomrobject_t newValue
        = (fomrobject_t)(*remainingSizeToCopy | (outstandingCopies << OUTSTANDING_COPIES_SHIFT)) | _beingCopiedTag;
...
void MM_ForwardedHeader::copyOrWaitOutline(omrobjectptr_t destinationObjectPtr)
{
    volatile MutableHeaderFields* objectHeader
        = (volatile MutableHeaderFields*)((fomrobject_t*)destinationObjectPtr + _forwardingSlotOffset);
        ...
        do {
            ...
            if (0 == remainingSizeToCopy) {
                if (participatingInCopy) {
                    MM_AtomicOperations::storeSync();
                    fomrobject_t newValue
                        = (fomrobject_t)(((outstandingCopies - 1) << OUTSTANDING_COPIES_SHIFT) | _beingCopiedTag);
                    if (oldValue != lockCompareExchangeObjectHeader(&objectHeader->slot, oldValue, newValue)) {
                        continue;
                    }
