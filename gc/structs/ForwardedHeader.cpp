...
#if defined(FORWARDEDHEADER_DEBUG)
void
MM_ForwardedHeader::Assert(bool condition,
...
MM_ForwardedHeader::ForwardedHeaderDump(omrobjectptr_t destinationObjectPtr)
{
  ...
  fprintf(stderr,
          "MM_ForwardedHeader@%p[%p(%p):%x:%x] -> %p(%p)\n",
          this,
          _objectPtr,
  ...
  fprintf(stderr,
          "MM_ForwardedHeader@%p[%p(%p):%x] -> %p(%p)\n",
          this,
          _objectPtr,
          (uintptr_t*)(*_objectPtr),
          _preserved.slot,
 ...
 */
omrobjectptr_t
MM_ForwardedHeader::setForwardedObjectInternal(
...
uintptr_t
MM_ForwardedHeader::copySetup(omrobjectptr_t destinationObjectPtr,
...
void
MM_ForwardedHeader::copySection(omrobjectptr_t destinationObjectPtr,
                                uintptr_t remainingSizeToCopy,
...
MM_ForwardedHeader::setSelfForwardedObject()
{
  ...
  if (oldValue != lockCompareExchangeObjectHeader(
                    &objectHeader->slot, oldValue, newValue)) {
...
uintptr_t
MM_ForwardedHeader::winObjectSectionToCopy(
  ...
  while (true) {
    ...
    do {
      ...
      sizeToCopy = winObjectSectionToCopy(
        &objectHeader->slot, oldValue, &remainingSizeToCopy, outstandingCopies);
    } while (0 == sizeToCopy);
...
MM_ForwardedHeader::copyOrWaitWinner(omrobjectptr_t destinationObjectPtr)
{
  ...
  while (true) {
    ...
    do {
      ...
      uintptr_t outstandingCopies =
        ((uintptr_t)oldValue & _copySizeAlignement) >> OUTSTANDING_COPIES_SHIFT;
