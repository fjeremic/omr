...
#if defined(FORWARDEDHEADER_DEBUG)
void MM_ForwardedHeader::Assert(bool condition, const char *assertion,
...
void MM_ForwardedHeader::ForwardedHeaderDump(
    omrobjectptr_t destinationObjectPtr) {
#if defined(OMR_GC_COMPRESSED_POINTERS)
  fprintf(stderr, "MM_ForwardedHeader@%p[%p(%p):%x:%x] -> %p(%p)\n", this,
          _objectPtr, (uintptr_t *)(*_objectPtr), _preserved.slot,
          _preserved.overlap, destinationObjectPtr,
...
/**
 * Mark the object with a forwarding tag. This may overwrite the forwarding slot
 * with a pointer to the specified forwarded location with the forwarded bit
 * set, if the object has not been forwarded by another thread since this
 * MM_ForwardedHeader was instantiated. In any case the actual forwarded
 * location is returned.
 *
 * @param[in] destinationObjectPtr the forwarded location
 * @return the actual location of the forwarded object
 */
omrobjectptr_t MM_ForwardedHeader::setForwardedObjectInternal(
    omrobjectptr_t destinationObjectPtr, uintptr_t forwardedTag) {
  ...
  /* Forwarded tag should be in low bits of the pointer and at the same time be
   * in forwarding slot */
  ...
  uintptr_t newValue =
      (((uintptr_t)destinationObjectPtr | forwardedTag) << 32) |
      (((uintptr_t)destinationObjectPtr >> 32) & 0xffffffff);
#else  /* defined (OMR_GC_COMPRESSED_POINTERS) && \
          !defined(OMR_ENV_LITTLE_ENDIAN) */
  ...
  if (MM_AtomicOperations::lockCompareExchange(
          (volatile uintptr_t *)&objectHeader->slot, oldValue, newValue) !=
      oldValue) {
    /* If we lost forwarding it, return where we are really forwarded. Another
     * thread could raced us to forward on another location or (Concurrent
     * world) self-forward it. In the later case, we will return NULL */
 ...
 *
 * @return a (strict) pointer to the forwarded location, or NULL if this not a
 * (strict) forwarded pointer.
 */
omrobjectptr_t MM_ForwardedHeader::getForwardedObject() {
...
omrobjectptr_t MM_ForwardedHeader::getNonStrictForwardedObject() {
  omrobjectptr_t forwardedObject = NULL;
...
uintptr_t MM_ForwardedHeader::copySetup(omrobjectptr_t destinationObjectPtr,
                                        uintptr_t *remainingSizeToCopy) {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  volatile MutableHeaderFields *objectHeader =
      (volatile MutableHeaderFields *)((fomrobject_t *)destinationObjectPtr +
                                       _forwardingSlotOffset);
  ...
  *remainingSizeToCopy = *remainingSizeToCopy - copyOffset;
  ...
  /* but obey minimum increment size (to hide overhead associated with each
   * section) */
  ...
  *remainingSizeToCopy -= alignmentResidue;
  ...
  objectHeader->slot =
      (fomrobject_t)(*remainingSizeToCopy | (0 << OUTSTANDING_COPIES_SHIFT)) |
      _beingCopiedTag;
  /* Make sure that destination object header is visible to other potential
   * participating threads. About to be executed atomic as part of forwarding
   * operation is executed on source object header, hence it will not
   * synchronize memory in the destination object header.
   */
...
void MM_ForwardedHeader::copySection(omrobjectptr_t destinationObjectPtr,
                                     uintptr_t remainingSizeToCopy,
                                     uintptr_t sizeToCopy) {
  uintptr_t copyOffset =
      sizeof(fomrobject_t) * (_forwardingSlotOffset + 1) + remainingSizeToCopy;

  void *dstStartAddress =
      (void *)((uintptr_t)destinationObjectPtr + copyOffset);
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
omrobjectptr_t MM_ForwardedHeader::setSelfForwardedObject() {
  volatile MutableHeaderFields *objectHeader =
      (volatile MutableHeaderFields *)((fomrobject_t *)_objectPtr +
                                       _forwardingSlotOffset);
  ...
  if (oldValue != lockCompareExchangeObjectHeader(&objectHeader->slot, oldValue,
                                                  newValue)) {
    /* If we lost on self-forwarding, return where we are really forwarded. We
     * could still be self-forwarded (another thread raced us) or strictly
     * forwarded (another thread successfully copied the object). Either way,
     * getNonStrictForwardedObject() should return us where we really are. */
...
void MM_ForwardedHeader::restoreSelfForwardedPointer() {
...
uintptr_t MM_ForwardedHeader::winObjectSectionToCopy(
  ...
  *remainingSizeToCopy -= sizeToCopy;
  ...
  fomrobject_t newValue =
      (fomrobject_t)(*remainingSizeToCopy |
                     (outstandingCopies << OUTSTANDING_COPIES_SHIFT)) |
      _beingCopiedTag;
  if (oldValue ==
      lockCompareExchangeObjectHeader(copyProgressSlot, oldValue, newValue)) {
    return sizeToCopy;
  }
...
void MM_ForwardedHeader::wait(uintptr_t *spinCount) {
...
void MM_ForwardedHeader::copyOrWaitOutline(
    omrobjectptr_t destinationObjectPtr) {
  volatile MutableHeaderFields *objectHeader =
      (volatile MutableHeaderFields *)((fomrobject_t *)destinationObjectPtr +
                                       _forwardingSlotOffset);
  ...
  while (true) {
    ...
    do {
      ...
      uintptr_t outstandingCopies =
          (oldValue & _copySizeAlignement) >> OUTSTANDING_COPIES_SHIFT;

      if (0 == remainingSizeToCopy) {
        if (participatingInCopy) {
          MM_AtomicOperations::storeSync();
          fomrobject_t newValue = (fomrobject_t)(
              ((outstandingCopies - 1) << OUTSTANDING_COPIES_SHIFT) |
              _beingCopiedTag);
          if (oldValue != lockCompareExchangeObjectHeader(&objectHeader->slot,
                                                          oldValue, newValue)) {
            continue;
          }
          participatingInCopy = false;
        }
        /* No work left to help with. Just keep waiting and checking till
         * copying/fixing is done. */
        wait(&spinCount);
        continue;
      }
      ...
      sizeToCopy =
          winObjectSectionToCopy(&objectHeader->slot, oldValue,
                                 &remainingSizeToCopy, outstandingCopies);
    } while (0 == sizeToCopy);
...
void MM_ForwardedHeader::copyOrWaitWinner(omrobjectptr_t destinationObjectPtr) {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  volatile MutableHeaderFields *objectHeader =
      (volatile MutableHeaderFields *)((fomrobject_t *)destinationObjectPtr +
                                       _forwardingSlotOffset);
  ...
  while (true) {
    ...
    do {
      ...
      uintptr_t outstandingCopies =
          ((uintptr_t)oldValue & _copySizeAlignement) >>
          OUTSTANDING_COPIES_SHIFT;
