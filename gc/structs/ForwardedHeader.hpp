...
/* If 'being copied hint' is set, it hints that destination might still be being copied (although it might have just
   completed). It tells the caller it should go and fetch master info from the destination header to tell if coping is
   really complete. If hint is reset, the copying is definitely complete, no need to fetch the master info. This hint is
   not necessary for correctness of copying protocol, it's just an optimization to avoid visiting destination object
   header in cases when it's likely not in data cash (GC thread encountering already forwarded object) */
...
/* actually limit on outstandingCopy count is conservatively set to a lower number than what is the bit allotment
 * (OUTSTANDING_COPIES_MASK_BASE), since having too many copies in parallel may be counterproductive */
...
private:
    ...
    struct MutableHeaderFields {
        ...
        /* this field must be here to reserve space if slots are 4 bytes long (extend to 8 bytes starting from
         * &MutableHeaderFields.clazz) */
        uint32_t overlap;
    ...
    const uintptr_t _forwardingSlotOffset; /**< fomrobject_t offset from _objectPtr to fomrobject_t slot that will hold
    ...
    static const fomrobject_t _beingCopiedTag
        = (fomrobject_t)OMR_BEING_COPIED_TAG; /**< used in destination object, but using the same bit as _forwardedTag
                                                 in source object */
    ...
    static const uintptr_t _minIncrement
        = (131072 & _remainingSizeMask); /**< min size of copy section; does not have to be a power of 2, but it has to
...
private:
    MMINLINE_DEBUG static fomrobject_t lockCompareExchangeObjectHeader(
        volatile fomrobject_t* address, fomrobject_t oldValue, fomrobject_t newValue)
    {
#if defined(OMR_GC_COMPRESSED_POINTERS)
     ...
     */
    static uintptr_t winObjectSectionToCopy(volatile fomrobject_t* copyProgressSlot, fomrobject_t oldValue,
...
#define ForwardedHeaderDump(destinationObjectPtr)
    ...
    /**
     * Initial step for destination object fixup - restore object flags and overlap, while still maintaining progess
     * info and being copied bit.
     */
    ...
    {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
        /* Check the hint bit in the forwarding pointer itself, before fetching the master info in the destination
         * object header */
        if (isBeingCopied()) {
     ...
     *
     * This method will assert if the object is not reverse forwarded. Use isReverseForwardedPointer() to test before
     * calling.
     *
