 ...
 * by a pool, and as long as no slots are being deleted (as in the
 * out-of-process case), we can just use the pool iterator.
 *
 * @ingroup GC_Structs
 */
class GC_HashTableIterator {
 ...
 public:
  ...
  /**
   * Prevent the hash table from growing. This allows the iteration to be
   * interrupted and more elements may be added to the table before resuming.
   * Elements still should not be deleted while iteration is interrupted.
   */
