...
/**
 * Generic interface for iterating over all objects and non-objects in a memory
 * segment. Subclasses should provide at least two constructors: one which takes
 * one or more parameters indicating the chunk of heap to be walked, and one
 * that does not. If the latter constructor is used, the iterator is
 * <i>uninitialized</i> until reset() is called. An uninitialized iterator
 * <b>must</b> return NULL when nextObject() or nextObjectNoAdvance() is called.
 * @note After an iterator is initialized, a caller should either call
 * nextObject() or nextObjectNoAdvance(), i.e. not mix and match calls to both.
 *
 * @ingroup GC_Structs
 */
