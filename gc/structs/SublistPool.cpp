 ...
 * Allocate and initialize all internal values and support structures for the
 * subpool to be ready for use.
 *
 * @return true if the initialization is successful, false otherwise.
 */
bool MM_SublistPool::initialize(MM_EnvironmentBase *env,
 ...
 */
void MM_SublistPool::tearDown(MM_EnvironmentBase *env) {
  if (_mutex) {
...
void MM_SublistPool::freePuddles(MM_EnvironmentBase *env,
                                 MM_SublistPuddle *list) {
 ...
 */
MM_SublistPuddle *MM_SublistPool::createNewPuddle(MM_EnvironmentBase *env) {
  uintptr_t puddleSize;

  /* If the sublist has a maximum size, be sure we aren't attempting to grow
   * beyond it */
  if (_maxSize) {
 ...
 * Reserve memory from the sublist and update the fragment.  If there is no room
 * available in the current sublist memory, allocate a new sublist puddle (until
 * the maximum sublist size is reached).
 *
 * @return true if the fragment allocate is successful, false otherwise.
 */
bool MM_SublistPool::allocate(MM_EnvironmentBase *env,
                              MM_SublistFragment *fragment) {
  uintptr_t puddleSize = 0;
  MM_SublistPuddle *emptyPuddle = NULL;

  /* Attempt to allocate a fragment from the current allocation puddle. If
  ...
  if (_allocPuddle && _allocPuddle->getNext()) {
    ...
    if (_maxSize) {
      ...
      /* If the available size to grow is greater than the suggested size,
       * reduce */
      if (puddleSize > _growSize) {
        puddleSize = _growSize;
      }
    ...
    /* Get a new puddle to add to the sublist pool */
    if (NULL == (emptyPuddle = MM_SublistPuddle::newInstance(
                     env, puddleSize, this, _allocCategory))) {
      omrthread_monitor_exit(_mutex);
      return false;
    }
    Assert_MM_true(emptyPuddle->isEmpty());
  ...
  /* Now that we have allocated our fragment, it is safe to expose the puddle to
   * the rest of the VM */
  if (NULL == _list) {
 ...
 */
uintptr_t *MM_SublistPool::allocateElementNoContention(
    MM_EnvironmentBase *env) {
  uintptr_t *element;
  MM_SublistPuddle *emptyPuddle;

  /* Allocate a new fragment from the current alloc puddle (if successful, we
   * are done) */
  if (_allocPuddle &&
      (NULL != (element = _allocPuddle->allocateElementNoContention()))) {
    return element;
  }
...
void MM_SublistPool::compact(MM_EnvironmentBase *env) {
  MM_SublistPuddle *currentPuddle, *nextPuddle;
  MM_SublistPuddle *sourcePuddle, *destinationPuddle;
  MM_SublistPuddle *lastPuddle = NULL;

  /* Use the list of puddles to iterate through and reset the list pointer to
   * NULL as we will add puddles back
   */
  currentPuddle = _list;
  _list = NULL;

  /* Run through the list of puddles deciding whether they should be compacted
   * into one another, killed or added directly to the list
   */
  ...
  while (currentPuddle) {
    ...
    if (!destinationPuddle) {
      ...
      /* We already have a puddle that is partially full.  Find which puddle has
       * the lesser number of allocated elements, and copy it into the other
       * puddle.  It may be this is only a partial copy
       */
      ...
      if (destinationPuddle->isFull()) {
        ...
        if (sourcePuddle->isEmpty()) {
          /* The source puddle is empty - kill it and set the destination puddle
           * to NULL (we have no destination candidate as it is full)
           */
          _currentSize -= sourcePuddle->totalSize();
          MM_SublistPuddle::kill(env, sourcePuddle);
          destinationPuddle = NULL;
        } else {
          /* The destination is full and the source is not - the source becomes
           * the new destination candidate
           */
          destinationPuddle = sourcePuddle;
        }
      } else {
        /* If we couldn't fill the destination, the source puddle must be empty
         * - kill it */
        _currentSize -= sourcePuddle->totalSize();
        MM_SublistPuddle::kill(env, sourcePuddle);
      }
  ...
  /* If there is a destination puddle after compacting the list, it is the alloc
   * puddle. */
  if (destinationPuddle) {
    if (lastPuddle) {
 ...
 * @note does not clear any fragments which may have cached ranges within the
 * puddles.
 */
void MM_SublistPool::clear(MM_EnvironmentBase *env) {
  /* Reset the size */
  _currentSize = 0;
 ...
 * correct.
 */
uintptr_t MM_SublistPool::countElements() { return _count; }

void MM_SublistPool::startProcessingSublist() {
...
MM_SublistPuddle *MM_SublistPool::popPreviousPuddle(
    MM_SublistPuddle *returnedPuddle) {
  omrthread_monitor_enter(_mutex);
