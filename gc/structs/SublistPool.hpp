...
/**
 * A thread-safe growable list that supports batch-reservation of
 * uintptr_t-sized elements. An MM_SublistPool is a pool of memory which
 * consists of a linked list of zero or more <i>puddles</i> (instances of
 * MM_SublistPuddle). A thread can reserve a block of memory from the list (an
 * instance of MM_SublistFragment), and then operate without contention on that
 * fragment.
 */
...
private:
  ...
  uintptr_t _maxSize;
  volatile uintptr_t
      _count; /**< A count for number of elements across all sublistPuddles */
  OMR::GC::AllocationCategory::Enum _allocCategory;

  MM_SublistPuddle *_previousList; /**< A list of the non-empty puddles when
                                      #startProcessingSublist() was called */
...
public:
   ...
   * the list of previous puddles. The puddles may be retrieved by calling
   * #popPreviousPuddle().
   */
  void startProcessingSublist();

  /**
   * Pop a puddle from the list of puddles which were active when
   * #startProcessingSublist() was called. Return returnedPuddle to the list of
   * puddles. It should be a puddle returned by a previous call to this
   * function. This is protected by a lock, so may safely be called by multiple
   * threads.
   *
  ...
  MM_SublistPool()
      : _list(NULL), _allocPuddle(NULL), _mutex(NULL), _growSize(0),
        _currentSize(0), _maxSize(0), _count(0),
