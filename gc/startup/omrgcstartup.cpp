...
static omr_error_t collectorCreationHelper(OMR_VM *omrVM,
                                           MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_OMRVM(omrVM);
  MM_GCExtensionsBase *extensions = MM_GCExtensionsBase::getExtensions(omrVM);
  MM_GlobalCollector *globalCollector =
      extensions->configuration->createGlobalCollector(env);
...
static omr_error_t heapCreationHelper(OMR_VM *omrVM,
                                      MM_StartupManager *startupManager,
  ...
  MM_GCExtensionsBase *extensions = MM_GCExtensionsBase::getExtensions(omrVM);
  extensions->_lazyCollectorInit =
      !createCollector; /* Delay initialize of sweep pool manager if needed. */

  /* The 'fake' environment is necessary because obtaining an environment for a
   * thread is done by calling out to MM_Configuration, which itself uses the
   * environment to obtain pointers to MM_GCExtensionsBase and OMR::GC::Forge,
   * etc. While this circular dependency could be eliminated, it would require
   * changes to many interfaces. */
  ...
  extensions->collectorLanguageInterface =
      startupManager->createCollectorLanguageInterface(&envBase);
  ...
  extensions->heap =
      extensions->configuration->createHeap(&envBase, extensions->memoryMax);
  ...
  /* It is necessary to have a dispatcher up before the collector can be
   * initialized, since the number of sweep chunks is determined by the number
   * of threads. */
  extensions->dispatcher = extensions->configuration->createDispatcher(
      &envBase, NULL, NULL, OMR_OS_STACK_SIZE);
  ...
  extensions->environments =
      extensions->configuration->createEnvironmentPool(&envBase);
  ...
  if (0 != omrthread_monitor_init_with_name(&extensions->gcStatsMutex, 0,
                                            "MM_GCExtensions::gcStats")) {
  ...
  extensions->configuration->prepareParameters(
      omrVM, extensions->initialMemorySize, extensions->minNewSpaceSize,
  ...
  if (0 != omrthread_monitor_init_with_name(
               &extensions->gcExclusiveAccessMutex, 0,
  ...
  if (0 != omrthread_monitor_init_with_name(
               &extensions->_lightweightNonReentrantLockPoolMutex, 0,
  ...
  if (createCollector &&
      (OMR_ERROR_NONE != collectorCreationHelper(omrVM, &envBase))) {
    rc = OMR_ERROR_INTERNAL;
    goto done;
  }

  memorySpace = extensions->configuration->createDefaultMemorySpace(
      &envBase, extensions->heap, &parameters);
  ...
  extensions->configuration->defaultMemorySpaceAllocated(extensions,
                                                         memorySpace);
  extensions->heap->setDefaultMemorySpace(memorySpace);

  if (startupManager->isVerboseEnabled()) {
    extensions->verboseGCManager =
        startupManager->createVerboseManager(&envBase);
    if (NULL == extensions->verboseGCManager) {
      omrtty_printf("Failed to create verbose GC manager.\n");
      rc = OMR_ERROR_INTERNAL;
      goto done;
    }
    extensions->verboseGCManager->configureVerboseGC(
        omrVM, startupManager->getVerboseFileName(), 1, 0);
...
omr_error_t OMR_GC_ShutdownDispatcherThreads(OMR_VMThread *omrVMThread) {
  ...
  if (NULL != extensions->dispatcher) {
    extensions->dispatcher->shutDownThreads();
    extensions->dispatcher->kill(
        MM_EnvironmentBase::getEnvironment(omrVMThread));
      ...
      while (NULL != (region = regionIterator.nextRegion())) {
        if (region->isCommitted()) {
          globalCollector->heapAddRange(
              ...
              region->getSize(), region->getLowAddress(),
...
omr_error_t OMR_GC_ShutdownCollector(OMR_VMThread *vmThread) {
...
omr_error_t OMR_GC_ShutdownHeapAndCollector(OMR_VM *omrVM) {
