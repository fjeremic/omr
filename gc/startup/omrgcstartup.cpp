...
static omr_error_t collectorCreationHelper(OMR_VM* omrVM, MM_EnvironmentBase* env)
{
...
static omr_error_t heapCreationHelper(OMR_VM* omrVM, MM_StartupManager* startupManager, bool createCollector)
{
    OMRPORT_ACCESS_FROM_OMRVM(omrVM);
    ...
    extensions->configuration->prepareParameters(omrVM, extensions->initialMemorySize, extensions->minNewSpaceSize,
        extensions->newSpaceSize, extensions->maxNewSpaceSize, extensions->minOldSpaceSize, extensions->oldSpaceSize,
        extensions->maxOldSpaceSize, extensions->maxSizeDefaultMemorySpace, MEMORY_TYPE_DISCARDABLE, &parameters);

    if (0
        != omrthread_monitor_init_with_name(
            &extensions->gcExclusiveAccessMutex, 0, "GCExtensions::gcExclusiveAccessMutex")) {
