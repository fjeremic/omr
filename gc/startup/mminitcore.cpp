...
extern "C"
{
  ...
  /**
   * Initialize the threads mutator information (RS pointers, reference list
   * pointers etc) for GC/MM purposes.
   *
   * @note vmThread MAY NOT be initialized completely from an execution model
   * perspective.
  ...
  intptr_t initializeMutatorModel(OMR_VMThread* omrVMThread)
  {
    ...
    omrVMThread->_gcOmrVMThreadExtensions =
      extensions->configuration->createEnvironment(extensions, omrVMThread);
    if (NULL != omrVMThread->_gcOmrVMThreadExtensions) {
      if (extensions->isStandardGC()) {
        void* lowAddress = extensions->heapBaseForBarrierRange0;
        void* highAddress =
          (void*)((uintptr_t)extensions->heapBaseForBarrierRange0 +
        ...
        omrVMThread->heapBaseForBarrierRange0 =
          extensions->heapBaseForBarrierRange0;
        omrVMThread->heapSizeForBarrierRange0 =
          extensions->heapSizeForBarrierRange0;
        ...
        omrVMThread->heapSizeForBarrierRange0 =
          (uintptr_t)heapTop - (uintptr_t)heapBase;

        /* lowTenureAddress and highTenureAddress are actually supposed to be
         * the low and high addresses of the heap for which card dirtying is
         * required (the JIT uses this as a range check to determine if it needs
         * to dirty a card when writing into an object).  Setting these for
         * Tarok is just a work-around until a more generic solution is
         * implemented
         */
  ...
  /**
   * Cleanup Mutator specific resources (TLH, thread extension, etc) on
   * shutdown.
   */
