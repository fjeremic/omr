...
class MM_SweepPoolManager : public MM_BaseVirtual {
 ...
 public:
   ...
   */
  virtual void flushFinalChunk(MM_EnvironmentBase* env,
                               MM_MemoryPool* memoryPool) = 0;

  /**
   *  Finally if there is at least 1 entry in the subspace, the last entry
   * should be connected to NULL in the free list
   */
  virtual void connectFinalChunk(MM_EnvironmentBase* env,
                                 MM_MemoryPool* memoryPool) = 0;

  virtual void poolPostProcess(MM_EnvironmentBase* env,
                               MM_MemoryPool* memoryPool) = 0;
   ...
   * properly.
   * @note the free list is not guaranteed to be in a consistent state at the
   * end of the routine
   ...
   */
  virtual bool addFreeMemory(MM_EnvironmentBase* env,
   ...
   */
  virtual void updateTrailingFreeMemory(MM_EnvironmentBase* env,
                                        MM_ParallelSweepChunk* sweepChunk,
                                        uintptr_t* heapSlotFreeHead,
                                        uintptr_t heapSlotFreeCount) = 0;
