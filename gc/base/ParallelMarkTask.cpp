...
uintptr_t MM_ParallelMarkTask::getVMStateID() { return OMRVMSTATE_GC_MARK; }

void MM_ParallelMarkTask::run(MM_EnvironmentBase *env) {
  env->_workStack.prepareForWork(
      env, (MM_WorkPackets *)(_markingScheme->getWorkPackets()));
...
void MM_ParallelMarkTask::setup(MM_EnvironmentBase *env) {
...
void MM_ParallelMarkTask::cleanup(MM_EnvironmentBase *env) {
   ...
   * partially duplicates info in -Xtgc:parallel */
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  Trc_MM_ParallelMarkTask_parallelStats(
      env->getLanguageVMThread(), (uint32_t)env->getSlaveID(),
      (uint32_t)omrtime_hires_delta(0, env->_workPacketStats._workStallTime,
                                    OMRPORT_TIME_DELTA_IN_MILLISECONDS),
      (uint32_t)omrtime_hires_delta(0, env->_workPacketStats._completeStallTime,
                                    OMRPORT_TIME_DELTA_IN_MILLISECONDS),
...
void MM_ParallelMarkTask::synchronizeGCThreads(MM_EnvironmentBase *env,
                                               const char *id) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
...
bool MM_ParallelMarkTask::synchronizeGCThreadsAndReleaseMaster(
...
bool MM_ParallelMarkTask::synchronizeGCThreadsAndReleaseSingleThread(
    MM_EnvironmentBase *env, const char *id) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  uint64_t startTime = omrtime_hires_clock();
  bool result =
