...
MM_ParallelMarkTask::cleanup(MM_EnvironmentBase* env)
{
  ...
  Trc_MM_ParallelMarkTask_parallelStats(
    ...
    (uint32_t)omrtime_hires_delta(0,
                                  env->_workPacketStats._workStallTime,
                                  OMRPORT_TIME_DELTA_IN_MILLISECONDS),
    (uint32_t)omrtime_hires_delta(0,
                                  env->_workPacketStats._completeStallTime,
                                  OMRPORT_TIME_DELTA_IN_MILLISECONDS),
    (uint32_t)omrtime_hires_delta(
      0, env->_markStats._syncStallTime, OMRPORT_TIME_DELTA_IN_MILLISECONDS),
...
void
MM_ParallelMarkTask::synchronizeGCThreads(MM_EnvironmentBase* env,
                                          const char* id)
{
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
...
bool
MM_ParallelMarkTask::synchronizeGCThreadsAndReleaseMaster(
...
bool
MM_ParallelMarkTask::synchronizeGCThreadsAndReleaseSingleThread(
  MM_EnvironmentBase* env,
  const char* id)
{
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
