...
void MM_WorkStack::prepareForWork(MM_EnvironmentBase *env,
                                  MM_WorkPackets *workPackets) {
  if (NULL == _workPackets) {
    _workPackets = workPackets;
    /* this is our first time using this work stack instance so the packets
     * should be NULL */
    Assert_MM_true(NULL == _inputPacket);
    Assert_MM_true(NULL == _outputPacket);
    Assert_MM_true(NULL == _deferredPacket);
 ...
 * packet (empty or does not exist)
 *
...
void *MM_WorkStack::popNoWaitFailed(MM_EnvironmentBase *env) {
  if (NULL != _inputPacket) {
    /* The current input packet has been used up - return it to the output list
     * for resuse */
    _workPackets->putPacket(env, _inputPacket);
    _inputPacket = NULL;
  }
...
void MM_WorkStack::pushFailed(MM_EnvironmentBase *env, void *element1,
                              void *element2) {
  if (_outputPacket) {
    /* The output packet is full - move it to the input list */
    _workPackets->putOutputPacket(env, _outputPacket);
...
/**
 * Immediately flush the output packet back to the shared pool so that it can be
...
void *MM_WorkStack::popNoWaitFromCurrentInputPacket(MM_EnvironmentBase *env) {
  ...
  if (NULL != _inputPacket) {
    ...
    if (NULL == result) {
      /* The current input packet has been used up - return it to the output
       * list for reuse */
      _workPackets->putPacket(env, _inputPacket);
      _inputPacket = NULL;
    }
...
bool MM_WorkStack::inputPacketAvailableFromWorkPackets(
...
bool MM_WorkStack::retrieveInputPacket(MM_EnvironmentBase *env) {
  ...
  if (NULL == _inputPacket) {
    /* If the output packet contains at least a free entry - invert the
     * input/output */
