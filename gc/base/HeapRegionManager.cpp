...
MM_HeapRegionManager::MM_HeapRegionManager(MM_EnvironmentBase* env, uintptr_t regionSize, uintptr_t tableDescriptorSize,
...
MM_HeapRegionManager* MM_HeapRegionManager::newInstance(MM_EnvironmentBase* env, uintptr_t regionSize,
...
void MM_HeapRegionManager::insertHeapRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptor* heapRegion)
{
    ...
    if (nextHeapRegion) {
        J9_LINEAR_LINKED_LIST_ADD_BEFORE(_nextRegion, _previousRegion, _auxRegionDescriptorList,
            (MM_HeapRegionDescriptor*)nextHeapRegion, (MM_HeapRegionDescriptor*)heapRegion);
    } else {
        if (J9_LINEAR_LINKED_LIST_IS_EMPTY(_auxRegionDescriptorList)) {
            J9_LINEAR_LINKED_LIST_ADD(
                _nextRegion, _previousRegion, _auxRegionDescriptorList, (MM_HeapRegionDescriptor*)heapRegion);
        } else {
            J9_LINEAR_LINKED_LIST_ADD_AFTER(_nextRegion, _previousRegion, _auxRegionDescriptorList,
                (MM_HeapRegionDescriptor*)lastHeapRegion, (MM_HeapRegionDescriptor*)heapRegion);
        }
    }
...
void MM_HeapRegionManager::removeHeapRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptor* heapRegion)
{
...
MM_HeapRegionDescriptor* MM_HeapRegionManager::createAuxiliaryRegionDescriptor(
    MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, void* lowAddress, void* highAddress)
{
    writeLock();
...
MM_HeapRegionDescriptor* MM_HeapRegionManager::internalCreateAuxiliaryRegionDescriptor(
    MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, void* lowAddress, void* highAddress)
{
...
void MM_HeapRegionManager::reassociateRegionWithSubSpace(
    MM_EnvironmentBase* env, MM_HeapRegionDescriptor* region, MM_MemorySubSpace* subSpace)
{
    writeLock();
...
MM_HeapRegionDescriptor* MM_HeapRegionManager::getNextTableRegion(MM_HeapRegionDescriptor* heapRegion)
{
    return findFirstUsedRegion(
        (MM_HeapRegionDescriptor*)((uintptr_t)heapRegion + (_tableDescriptorSize * heapRegion->_regionsInSpan)));
}

MM_HeapRegionDescriptor* MM_HeapRegionManager::findFirstUsedRegion(MM_HeapRegionDescriptor* start)
{
...
MM_HeapRegionDescriptor* MM_HeapRegionManager::internalAllocateAuxiliaryRegionDescriptor(
...
MM_HeapRegionDescriptor* MM_HeapRegionManager::internalAllocateAndInitializeRegionTable(
...
void MM_HeapRegionManager::internalFreeRegionTable(
    MM_EnvironmentBase* env, MM_HeapRegionDescriptor* tableBase, uintptr_t tableElementCount)
{
    if (NULL != _regionDescriptorDestructor) {
...
MM_HeapRegionDescriptor* MM_HeapRegionManager::auxillaryDescriptorForAddress(const void* address)
{
...
MM_HeapRegionDescriptor* MM_HeapRegionManager::mapRegionTableIndexToDescriptor(uintptr_t index)
{
    return (MM_HeapRegionDescriptor*)((uintptr_t)_regionTable + (_tableDescriptorSize * index));
}

uintptr_t MM_HeapRegionManager::mapDescriptorToRegionTableIndex(MM_HeapRegionDescriptor* region)
{
