...
MM_HeapRegionManager::newInstance(
  MM_EnvironmentBase* env,
  uintptr_t regionSize,
  uintptr_t tableDescriptorSize,
  MM_RegionDescriptorInitializer regionDescriptorInitializer,
  MM_RegionDescriptorDestructor regionDescriptorDestructor)
{
  MM_HeapRegionManager* regionManager =
    (MM_HeapRegionManager*)env->getForge()->allocate(
      sizeof(MM_HeapRegionManager),
      OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (regionManager) {
    new (regionManager) MM_HeapRegionManager(env,
                                             regionSize,
                                             tableDescriptorSize,
...
void
MM_HeapRegionManager::insertHeapRegion(MM_EnvironmentBase* env,
                                       MM_HeapRegionDescriptor* heapRegion)
{
  ...
  if (nextHeapRegion) {
    J9_LINEAR_LINKED_LIST_ADD_BEFORE(_nextRegion,
                                     _previousRegion,
    ...
    if (J9_LINEAR_LINKED_LIST_IS_EMPTY(_auxRegionDescriptorList)) {
      J9_LINEAR_LINKED_LIST_ADD(_nextRegion,
                                _previousRegion,
                                _auxRegionDescriptorList,
                                (MM_HeapRegionDescriptor*)heapRegion);
    } else {
      J9_LINEAR_LINKED_LIST_ADD_AFTER(_nextRegion,
                                      _previousRegion,
                                      _auxRegionDescriptorList,
...
void
MM_HeapRegionManager::removeHeapRegion(MM_EnvironmentBase* env,
                                       MM_HeapRegionDescriptor* heapRegion)
{
  ...
  J9_LINEAR_LINKED_LIST_REMOVE(
    _nextRegion, _previousRegion, _auxRegionDescriptorList, heapRegion);
...
MM_HeapRegionManager::createAuxiliaryRegionDescriptor(
  MM_EnvironmentBase* env,
  ...
  void* highAddress)
{
  ...
  toReturn = internalCreateAuxiliaryRegionDescriptor(
    env, subSpace, lowAddress, highAddress);
  writeUnlock();
  return toReturn;
}
...
void
MM_HeapRegionManager::destroyAuxiliaryRegionDescriptor(
  MM_EnvironmentBase* env,
...
void
MM_HeapRegionManager::internalDestroyAuxiliaryRegionDescriptor(
  MM_EnvironmentBase* env,
  MM_HeapRegionDescriptor* descriptor)
{
...
void
MM_HeapRegionManager::reassociateRegionWithSubSpace(
  MM_EnvironmentBase* env,
  ...
  void* highAddress)
{
  MM_HeapRegionDescriptor* desc =
    (MM_HeapRegionDescriptor*)env->getForge()->allocate(
      ...
      OMR_GET_CALLSITE());
  if (NULL != desc) {
    if (!_regionDescriptorInitializer(
          env, this, desc, lowAddress, highAddress)) {
      desc = NULL;
    }
  }
  return desc;
...
void
MM_HeapRegionManager::internalFreeAuxiliaryRegionDescriptor(
  MM_EnvironmentBase* env,
  MM_HeapRegionDescriptor* descriptor)
{
  ...
  void* highHeapEdge)
{
  ...
  if (NULL != table) {
    ...
    for (uintptr_t i = 0; i < regionCount; i++) {
      ...
      base = next;
      descriptor =
        (MM_HeapRegionDescriptor*)((uint8_t*)descriptor + _tableDescriptorSize);
    }
  }
  return table;
}
...
void
MM_HeapRegionManager::internalFreeRegionTable(
  MM_EnvironmentBase* env,
...
uintptr_t
MM_HeapRegionManager::mapDescriptorToRegionTableIndex(
  MM_HeapRegionDescriptor* region)
{
