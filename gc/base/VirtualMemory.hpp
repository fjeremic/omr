...
class MM_VirtualMemory : public MM_BaseVirtual {
   ...
   */
private:
  uintptr_t _pageSize;    /**< Page size for this virtual memory object (before
                             reservation requested, after reservation real) */
  uintptr_t _pageFlags;   /**< Flags describing the pages used for the virtual
                             memory object */
  uintptr_t _tailPadding; /**< The number of bytes of padding at the end of the
                             virtual memory. This padding will be committed
...
protected:
  ...
  void *_baseAddress; /**< The address returned from port - needed for freeing
                         memory at shutdown */
...
protected:
  /*
   * use "OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE" for
   * mode
   */
  static MM_VirtualMemory *
  newInstance(MM_EnvironmentBase *env, uintptr_t heapAlignment, uintptr_t size,
              uintptr_t pageSize, uintptr_t pageFlags, uintptr_t tailPadding,
              void *preferredAddress, void *ceiling, uintptr_t mode,
              uintptr_t options, uint32_t memoryCategory);

  bool initialize(MM_EnvironmentBase *env, uintptr_t size,
                  void *preferredAddress, void *ceiling, uintptr_t options,
  ...
  MM_VirtualMemory(MM_EnvironmentBase *env, uintptr_t heapAlignment,
                   uintptr_t pageSize, uintptr_t pageFlags,
                   uintptr_t tailPadding, uintptr_t mode)
      : MM_BaseVirtual(), _pageSize(pageSize), _pageFlags(pageFlags),
        _tailPadding(tailPadding), _heapBase(0), _heapTop(0), _reserveSize(0),
        _mode(mode), _consumerCount(0), _identifier(),
        _extensions(env->getExtensions()), _baseAddress(NULL),
   ...
   * @param[in] address - the start of the range to modify, must be aligned to
   * the physical page size
   * @param byteAmount - the size of the range to modify. Might NOT to be
   * aligned to page size. Will be aligned inside.
   *
   ...
   */
  MMINLINE void *getHeapTop() { return _heapTop; };

  /**
   * Return the size of the pages used in the virtual memory object
   */
  MMINLINE uintptr_t getPageSize() { return _pageSize; }

  /**
   * Return the flags describing the pages used in the virtual memory object
   */
  MMINLINE uintptr_t getPageFlags() { return _pageFlags; }

  /**
   * Return number of memory consumers attached to this virtual memory object
   * @return consumers number
   */
  MMINLINE uintptr_t getConsumerCount() { return _consumerCount; }

  /**
   * Increment number of memory consumers attached to this virtual memory object
   * by one
   */
  MMINLINE void incrementConsumerCount() { _consumerCount += 1; }

  /**
   * Decrement number of memory consumers attached to this virtual memory object
   * by one
   */
  MMINLINE void decrementConsumerCount() { _consumerCount -= 1; }
