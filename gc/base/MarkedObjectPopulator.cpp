...
void
MM_MarkedObjectPopulator::initializeObjectHeapBufferedIteratorState(
...
void
MM_MarkedObjectPopulator::reset(MM_HeapRegionDescriptor* region,
...
uintptr_t
MM_MarkedObjectPopulator::populateObjectHeapBufferedIteratorCache(
  ...
  GC_ObjectHeapBufferedIteratorState* state) const
{
  ...
  if (NULL != startPtr) {
     ...
     * them? */
    MM_HeapMapIterator iterator(
      state->extensions, markMap, startPtr, endPtr, false);
...
void
MM_MarkedObjectPopulator::advance(
  uintptr_t size,
  GC_ObjectHeapBufferedIteratorState* state) const
{
