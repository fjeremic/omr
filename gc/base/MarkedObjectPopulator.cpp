...
void MM_MarkedObjectPopulator::initializeObjectHeapBufferedIteratorState(
...
void MM_MarkedObjectPopulator::reset(MM_HeapRegionDescriptor* region,
...
uintptr_t MM_MarkedObjectPopulator::populateObjectHeapBufferedIteratorCache(
  ...
  if (NULL != startPtr) {
    ...
    /* TODO: what to do if we've been asked to include dead objects? There's no
     * obvious way to include them. Perhaps it doesn't matter? Or we can infer
     * them? */
    MM_HeapMapIterator iterator(state->extensions, markMap, startPtr, endPtr,
                                false);
...
void MM_MarkedObjectPopulator::advance(
    uintptr_t size,
    GC_ObjectHeapBufferedIteratorState* state) const {
