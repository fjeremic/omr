...
public:
  ...
  MMINLINE MM_MemorySpace *getDefaultMemorySpace() {
  ...
  MMINLINE MM_HeapRegionManager *getHeapRegionManager() {
  ...
  /**
   * Return the page size used for the heap memory.  This should only be used
   * for reporting purposes only!  (e.g. In MM_HeapSplit, the low extent and
   * high extent may have different size pages, in this case the minimum value
   * is returned)
   */
  ...
  virtual bool attachArena(MM_EnvironmentBase *env, MM_PhysicalArena *arena,
                           uintptr_t size) = 0;
  virtual void detachArena(MM_EnvironmentBase *env,
                           MM_PhysicalArena *arena) = 0;
  ...
  /**
   * Called after the heap has been initialized so that it can properly
   * initialize HeapRegionManager.
   *
   * @param env[in] the thread which performed the initialization
   * @param manager[in] - the manager to initialize for this heap
   */
  virtual bool initializeHeapRegionManager(MM_EnvironmentBase *env,
                                           MM_HeapRegionManager *manager) = 0;

  struct MM_CommonGCData *initializeCommonGCData(MM_EnvironmentBase *env,
                                                 struct MM_CommonGCData *data);
  void initializeCommonGCStartData(MM_EnvironmentBase *env,
                                   struct MM_CommonGCStartData *data);
  void initializeCommonGCEndData(MM_EnvironmentBase *env,
                                 struct MM_CommonGCEndData *data);
   ...
   */
  MM_Heap(MM_EnvironmentBase *env, uintptr_t maximumMemorySize,
          MM_HeapRegionManager *regionManager)
      : MM_BaseVirtual(), _omrVM(env->getOmrVM()),
        _portLibrary(env->getPortLibrary()), _defaultMemorySpace(NULL),
        _memorySpaceList(NULL), _maximumMemorySize(maximumMemorySize),
        _heapResizeStats(), _percolateStats(),
