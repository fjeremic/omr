...
protected:
  ...
  MM_HeapLinkedFreeHeader
      *_lastFreeEntry; /**< address of the last free entry in the pool; valid
                          after compact; NOT maintained during allocation */
...
protected:
   ...
   */
  void updateMemoryPoolStatistics(MM_EnvironmentBase *env, uintptr_t freeBytes,
                                  uintptr_t freeEntryCount,
  ...
  MMINLINE bool internalRecycleHeapChunk(void *addrBase, void *addrTop,
  ...
  MMINLINE bool appendToList(MM_EnvironmentBase *env, void *addrBase,
                             void *addrTop, uintptr_t minimumSize,
  ...
  MMINLINE bool insertToList(MM_EnvironmentBase *env, void *addrBase,
                             void *addrTop, uintptr_t minimumSize,
                             MM_HeapLinkedFreeHeader *&freeListHead,
                             MM_HeapLinkedFreeHeader *&freeListTail) {
    bool const compressed = compressObjectReferences();
  ...
  void connectFinalMemoryToPool(MM_EnvironmentBase *env, void *address,
                                uintptr_t size);
  void abandonMemoryInPool(MM_EnvironmentBase *env, void *address,
                           uintptr_t size);
...
public:
  ...
  //	MMINLINE MM_SweepPoolManagerAddressOrderedListBase
  //*getSweepPoolManager()
  {
    /*
  ...
  virtual void printCurrentFreeList(MM_EnvironmentBase *env,
                                    const char *area) = 0;

  virtual void recalculateMemoryPoolStatistics(MM_EnvironmentBase *env) = 0;
#if defined(OMR_GC_IDLE_HEAP_MANAGER)
  uintptr_t releaseFreeEntryMemoryPages(MM_EnvironmentBase *env,
                                        MM_HeapLinkedFreeHeader *freeEntry);
#endif
  /**
   * Create a MemoryPoolAddressOrderedList object.
   */
  MM_MemoryPoolAddressOrderedListBase(MM_EnvironmentBase *env,
                                      uintptr_t minimumFreeEntrySize)
      : MM_MemoryPool(env, minimumFreeEntrySize), _referenceHeapFreeList(NULL),
        _sweepPoolState(NULL), _sweepPoolManager(NULL),
        _lastFreeEntry(NULL){
  ...
  MM_MemoryPoolAddressOrderedListBase(MM_EnvironmentBase *env,
      ...
      : MM_MemoryPool(env, minimumFreeEntrySize, name),
        _referenceHeapFreeList(NULL), _sweepPoolState(NULL),
        _sweepPoolManager(NULL), _lastFreeEntry(NULL){
                                     //		_typeId = __FUNCTION__;
                                 };
