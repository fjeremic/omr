...
private:
  ...
  MM_GCExtensionsBase
      *_extensions; /**< cache extensions for use in teardown() */
...
protected:
public:
  bool initialize(MM_EnvironmentBase *env, ModronLnrlOptions *options,
                  const char *name);
  void tearDown();
  ...
  MM_LightweightNonReentrantLock()
      : MM_BaseNonVirtual(), _initialized(false), _tracing(NULL),
        _extensions(NULL) {
