...
private:
  MM_EnvironmentBase
      *_owningEnv; /**< The environment with which the receiver is associated */
  MM_TLHAllocationSupport
      _tlhAllocationSupport; /**< TLH Allocation sub interface class */

#if defined(OMR_GC_NON_ZERO_TLH)
  MM_TLHAllocationSupport
      _tlhAllocationSupportNonZero; /**< TLH Allocation sub interface class */
#endif                              /* defined(OMR_GC_NON_ZERO_TLH) */

  bool _cachedAllocationsEnabled; /**< Are cached allocations enabled? */
  uintptr_t
      _bytesAllocatedBase; /**< Bytes allocated at the start of an allocation
                              request.  Relative to _stats.bytesAllocated(). */
...
public:
  ...
  /* BEN TODO: Collapse the env->enable/disableInlineTLHAllocate with these
   * enable/disableCachedAllocations */
...
private:
  ...
  MM_TLHAllocationInterface(MM_EnvironmentBase *env)
      : MM_ObjectAllocationInterface(env), _owningEnv(env),
        _tlhAllocationSupport(env, true),
#if defined(OMR_GC_NON_ZERO_TLH)
        _tlhAllocationSupportNonZero(env, false),
#endif /* defined(OMR_GC_NON_ZERO_TLH) */
        _cachedAllocationsEnabled(true), _bytesAllocatedBase(0) {
