...
public:
  ...
  /**< returning pointer to our vmem handle (have to specify a pointer to const,
   * to ensure caller will not modify it) */
   ...
   */
  MM_HeapVirtualMemory(MM_EnvironmentBase *env, uintptr_t heapAlignment,
                       uintptr_t maximumMemorySize,
                       MM_HeapRegionManager *regionManager)
      : MM_Heap(env, maximumMemorySize, regionManager), _vmemHandle(),
        _heapAlignment(heapAlignment), _physicalArena(NULL) {
    _typeId = __FUNCTION__;
  }

  /* Split heap needs access to the _vmem instance for region manager
   * initialization */
