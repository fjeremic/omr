...
private:
  ...
  MM_GCExtensionsBase
      *_extensions; /**< The GC extensions associated with the JVM */
...
private:
  ...
  /**
   * Helper method that filters out special 'objects' like dead or forwarded
   * ones
   */
...
public:
  /**
   * @param memorySegment the memory segment to be walked, from
   * <code>heapBase</code> to <code>heapAlloc</code>.
   ...
   */
  GC_ObjectHeapIteratorAddressOrderedList(MM_GCExtensionsBase *extensions,
...
#endif
        _isDeadObject(false), _isSingleSlotHole(false), _deadObjectSize(0),
        _pastFirstObject(false), _extensions(extensions) {
  ...
  GC_ObjectHeapIteratorAddressOrderedList(MM_GCExtensionsBase *extensions,
      ...
      : /* Iterator base values */
        ...
        /* Iterator scan values */
        _isDeadObject(false), _isSingleSlotHole(false), _deadObjectSize(0),
        _pastFirstObject(skipFirstObject), _extensions(extensions) {
  }
  ...
  MMINLINE virtual omrobjectptr_t nextObject() {
    ...
    while (_scanPtr < _scanPtrTop) {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
      /* There are no known users of this type of iteration, while there are
       * still forwarded objects in the heap */
      ...
      if (!_isDeadObject) {
        ...
        _isSingleSlotHole =
            _extensions->objectModel.isSingleSlotDeadObject(_scanPtr);
        if (_isSingleSlotHole) {
          _deadObjectSize =
              _extensions->objectModel.getSizeInBytesSingleSlotDeadObject(
                  _scanPtr);
        } else {
          _deadObjectSize =
   ...
   */
  MMINLINE bool isDeadObject() { return _isDeadObject; }

  /**
   * Returns true if the slot hole is a singleton
   */
  MMINLINE bool isSingleSlotHole() { return _isSingleSlotHole; }

  /**
   * Returns the size of the dead object
   */
  MMINLINE uintptr_t getDeadObjectSize() { return _deadObjectSize; }
};
