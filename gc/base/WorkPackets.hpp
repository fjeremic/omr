...
protected:
  ...
  MM_PacketList
      _relativelyFullPacketList;     /**< List for relatively full packets */
  ...
  void emptyToOverflow(MM_EnvironmentBase *env, MM_Packet *packet,
  ...
  virtual MM_WorkPacketOverflow *
  createOverflowHandler(MM_EnvironmentBase *env, MM_WorkPackets *workPackets);
...
public:
  ...
  MMINLINE bool tracingExhausted() {
    return (_emptyPacketList.getCount() + _deferredPacketList.getCount() +
                _deferredFullPacketList.getCount() ==
            _activePackets);
  };

  MMINLINE uintptr_t getThreadWaitCount() { return _inputListWaitCount; }
   ...
   */
  MMINLINE uintptr_t getActivePacketCount() { return _activePackets; };
  ...
  MMINLINE uintptr_t getDeferredPacketCount() {
    return (_deferredPacketList.getCount() +
            _deferredFullPacketList.getCount());
  };
  ...
  MM_WorkPackets(MM_EnvironmentBase *env)
      : MM_BaseVirtual(), _packetsPerBlock(0), _maxPackets(0),
        _activePackets(0), _packetsBlocksTop(0), _allocatingPackets(NULL),
        _emptyPacketList(env), _fullPacketList(env),
        _relativelyFullPacketList(env), _nonEmptyPacketList(env),
        _deferredPacketList(env), _deferredFullPacketList(env),
        _inputListMonitor(NULL), _inputListWaitCount(0), _inputListDoneIndex(0),
