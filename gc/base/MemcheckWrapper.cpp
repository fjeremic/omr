...
static uintptr_t hashFn(void *key, void *userData) { return *(uintptr_t *)key; }

void valgrindCreateMempool(MM_GCExtensionsBase *extensions,
                           MM_EnvironmentBase *env, uintptr_t poolAddr) {
  ...
  extensions->memcheckHashTable =
      hashTableNew(env->getPortLibrary(), tableName, 0, entrySize, 0, 0,
                   OMRMEM_CATEGORY_VM, hashFn, hashEqualFn, 0, 0);
...
void valgrindMempoolAlloc(MM_GCExtensionsBase *extensions,
                          uintptr_t baseAddress, uintptr_t size) {
...
MMINLINE void valgrindFreeObjectDirect(MM_GCExtensionsBase *extensions,
...
void valgrindClearRange(MM_GCExtensionsBase *extensions, uintptr_t baseAddress,
                        uintptr_t size) {
  if (size == 0) {
    return;
  }
...
void valgrindFreeObject(MM_GCExtensionsBase *extensions,
                        uintptr_t baseAddress) {
  int objSize;
...
bool valgrindCheckObjectInPool(MM_GCExtensionsBase *extensions,
  ...
  MUTEX_ENTER(extensions->memcheckHashTableMutex);
  bool exists =
      hashTableFind(extensions->memcheckHashTable, &baseAddress) != NULL
          ? true
          : false;
  MUTEX_EXIT(extensions->memcheckHashTableMutex);
  return exists;
...
void valgrindResizeObject(MM_GCExtensionsBase *extensions,
                          uintptr_t baseAddress, uintptr_t oldSize,
                          uintptr_t newSize) {
