    ...
    MM_EnvironmentBase* env, uintptr_t minimumFreeEntrySize, const char* name)
{
    ...
    memoryPool = (MM_MemoryPoolAddressOrderedList*)env->getForge()->allocate(
        sizeof(MM_MemoryPoolAddressOrderedList), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (memoryPool) {
...
bool MM_MemoryPoolAddressOrderedList::initialize(MM_EnvironmentBase* env)
{
    ...
    _largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(env,
        (uint16_t)ext->largeObjectAllocationProfilingTopK, ext->largeObjectAllocationProfilingThreshold,
    ...
    _largeObjectAllocateStats
        = MM_LargeObjectAllocateStats::newInstance(env, (uint16_t)ext->largeObjectAllocationProfilingTopK,
    ...
    /* At this moment we do not know who is creator of this pool, so we do not set _largeObjectCollectorAllocateStats
     * yet. Tenure SubSpace for Gencon will set _largeObjectCollectorAllocateStats to _largeObjectAllocateStats (we
     * append collector stats to mutator stats) SemiSpace will leave _largeObjectCollectorAllocateStats at NULL (no
     * interest in Collector stats) Tenure SubSpace for Flat will leave _largeObjectCollectorAllocateStats at NULL (no
     * interest in Colletor stats)
     */
...
bool MM_MemoryPoolAddressOrderedList::initializeSweepPool(MM_EnvironmentBase* env)
{
    if (NULL == _sweepPoolState) {
         ...
         */
        _sweepPoolManager
            = (MM_SweepPoolManagerAddressOrderedListBase*)env->getExtensions()->sweepPoolManagerAddressOrderedList;
    }
    return true;
}
...
MMINLINE void MM_MemoryPoolAddressOrderedList::addHint(MM_HeapLinkedFreeHeader* freeEntry, uintptr_t lookupSize)
{
...
MMINLINE J9ModronAllocateHint* MM_MemoryPoolAddressOrderedList::findHint(uintptr_t lookupSize)
{
...
MMINLINE void MM_MemoryPoolAddressOrderedList::removeHint(MM_HeapLinkedFreeHeader* freeEntry)
{
...
MMINLINE void MM_MemoryPoolAddressOrderedList::updateHint(
    MM_HeapLinkedFreeHeader* oldFreeEntry, MM_HeapLinkedFreeHeader* newFreeEntry)
{
 ...
 */
void MM_MemoryPoolAddressOrderedList::updateHintsBeyondEntry(MM_HeapLinkedFreeHeader* freeEntry)
{
    J9ModronAllocateHint* hint = _hintActive;
 ...
 ****************************************
 */
MMINLINE void* MM_MemoryPoolAddressOrderedList::internalAllocate(MM_EnvironmentBase* env, uintptr_t sizeInBytesRequired,
    bool lockingRequired, MM_LargeObjectAllocateStats* largeObjectAllocateStats)
{
...
retry:
    ...
    while (currentFreeEntry) {
        ...
        /* while we are walking, keep track of the largest free entry.  We will need this in the case of allocation
         * failure to update the pool's largest free */
    ...
    if (recycleHeapChunk(recycleEntry, ((uint8_t*)recycleEntry) + recycleEntrySize, previousFreeEntry,
    ...
    /* Collector object allocate stats for Survivor are not interesting (_largeObjectCollectorAllocateStats is null for
     * Survivor) */
...
fail_allocate:
    /* Since we failed to allocate, update the largest free entry so that outside callers will be able to skip this
     * pool, next time, in Tarok configurations */
...
void* MM_MemoryPoolAddressOrderedList::allocateObject(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
    ...
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, bool lockingRequired)
{
    void* addr = internalAllocate(
        env, allocDescription->getContiguousBytes(), lockingRequired, _largeObjectCollectorAllocateStats);
...
MMINLINE bool MM_MemoryPoolAddressOrderedList::internalAllocateTLH(MM_EnvironmentBase* env,
...
retry:
    ...
    /* Collector TLH allocate stats for Survivor are not interesting (_largeObjectCollectorAllocateStats is null for
     * Survivor) */
    if (NULL != largeObjectAllocateStats) {
...
void* MM_MemoryPoolAddressOrderedList::allocateTLH(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
...
void* MM_MemoryPoolAddressOrderedList::collectorAllocateTLH(MM_EnvironmentBase* env,
    ...
    bool lockingRequired)
{
    ...
    if (internalAllocateTLH(
            env, maximumSizeInBytesRequired, addrBase, addrTop, lockingRequired, _largeObjectCollectorAllocateStats)) {
        base = addrBase;
        allocDescription->setTLHAllocation(true);
        allocDescription->setNurseryAllocation((_memorySubSpace->getTypeFlags() == MEMORY_TYPE_NEW) ? true : false);
        allocDescription->setMemoryPool(this);
 ...
 */
MM_HeapLinkedFreeHeader* MM_MemoryPoolAddressOrderedList::rebuildFreeListInRegion(
    MM_EnvironmentBase* env, MM_HeapRegionDescriptor* region, MM_HeapLinkedFreeHeader* previousFreeEntry)
{
    ...
    /* TODO 108399 Determine whether this is still necessary (OMR_SCAVENGER_DEBUG is defined in Scqavenger.cpp and is
     * not accessible here) */
 ...
 *
 */
void MM_MemoryPoolAddressOrderedList::expandWithRange(
    MM_EnvironmentBase* env, uintptr_t expandSize, void* lowAddress, void* highAddress, bool canCoalesce)
{
    ...
    if (canCoalesce) {
        ...
        if (previousFreeEntry
            && (lowAddress == (void*)(((uintptr_t)previousFreeEntry) + previousFreeEntry->getSize()))) {
        ...
        if (nextFreeEntry && (highAddress == (void*)nextFreeEntry)) {
            ...
            assume0(
                (NULL == nextFreeEntry->getNext(compressed)) || (newFreeEntry < nextFreeEntry->getNext(compressed)));
    ...
    MM_EnvironmentBase* env, uintptr_t contractSize, void* lowAddress, void* highAddress)
{
    ...
    while (currentFreeEntry) {
        if ((lowAddress >= currentFreeEntry)
            && (highAddress <= (void*)(((uintptr_t)currentFreeEntry) + currentFreeEntry->getSize()))) {
            break;
        }
        previousFreeEntry = currentFreeEntry;
        currentFreeEntry = currentFreeEntry->getNext(compressed);
 ...
 *
 */
void MM_MemoryPoolAddressOrderedList::addFreeEntries(MM_EnvironmentBase* env, MM_HeapLinkedFreeHeader*& freeListHead,
    MM_HeapLinkedFreeHeader*& freeListTail, uintptr_t freeListMemoryCount, uintptr_t freeListMemorySize)
{
    ...
    if (previousFreeEntry == NULL) {
        ...
        if ((uint8_t*)freeListTail->afterEnd() == (uint8_t*)_heapFreeList) {
            ...
            assume0(
                (NULL == _heapFreeList->getNext(compressed)) || (freeListTail < _heapFreeList->getNext(compressed)));
        ...
        if ((uint8_t*)previousFreeEntry->afterEnd() == (uint8_t*)freeListHead) {
            ...
            assume0(
                (NULL == freeListHead->getNext(compressed)) || (previousFreeEntry < freeListHead->getNext(compressed)));
...
bool MM_MemoryPoolAddressOrderedList::removeFreeEntriesWithinRange(MM_EnvironmentBase* env, void* lowAddress,
    void* highAddress, uintptr_t minimumSize, MM_HeapLinkedFreeHeader*& retListHead,
    MM_HeapLinkedFreeHeader*& retListTail, uintptr_t& retListMemoryCount, uintptr_t& retListMemorySize)
{
    bool const compressed = compressObjectReferences();
    ...
    while (currentFreeEntry && (((uint8_t*)currentFreeEntry->afterEnd()) <= highAddress)) {
        ...
        if (appendToList(env, (void*)currentFreeEntry, (void*)currentFreeEntry->afterEnd(), minimumSize, retListHead,
                retListTail)) {
            retListMemoryCount++;
...
bool MM_MemoryPoolAddressOrderedList::recycleHeapChunk(
    void* addrBase, void* addrTop, MM_HeapLinkedFreeHeader* previousFreeEntry, MM_HeapLinkedFreeHeader* nextFreeEntry)
{
    bool const compressed = compressObjectReferences();
 ...
 */
void* MM_MemoryPoolAddressOrderedList::findFreeEntryEndingAtAddr(MM_EnvironmentBase* env, void* addr)
{
    bool const compressed = compressObjectReferences();
 ...
 * void *, void *)
 */
uintptr_t MM_MemoryPoolAddressOrderedList::getAvailableContractionSizeForRangeEndingAt(
 ...
 */
void* MM_MemoryPoolAddressOrderedList::findFreeEntryTopStartingAtAddr(MM_EnvironmentBase* env, void* addr)
{
    bool const compressed = compressObjectReferences();
    MM_HeapLinkedFreeHeader* currentFreeEntry;
 ...
 */
void* MM_MemoryPoolAddressOrderedList::getFirstFreeStartingAddr(MM_EnvironmentBase* env)
{
 ...
 */
void* MM_MemoryPoolAddressOrderedList::getNextFreeStartingAddr(MM_EnvironmentBase* env, void* currentFree)
{
    bool const compressed = compressObjectReferences();
 ...
 *
 */
void MM_MemoryPoolAddressOrderedList::moveHeap(MM_EnvironmentBase* env, void* srcBase, void* srcTop, void* dstBase)
{
    bool const compressed = compressObjectReferences();
    ...
    while (currentFreeEntry) {
        if (((void*)currentFreeEntry >= srcBase) && ((void*)currentFreeEntry < srcTop)) {
            ...
            newFreeEntry = (MM_HeapLinkedFreeHeader*)((((uintptr_t)currentFreeEntry) - ((uintptr_t)srcBase))
                + ((uintptr_t)dstBase));
 ...
 */
bool MM_MemoryPoolAddressOrderedList::recycleHeapChunk(void* chunkBase, void* chunkTop)
{
    bool const compressed = compressObjectReferences();
    bool recycled = false;
 ...
 */
void MM_MemoryPoolAddressOrderedList::printCurrentFreeList(MM_EnvironmentBase* env, const char* area)
{
    bool const compressed = compressObjectReferences();
    ...
    while (currentFreeEntry) {
        omrtty_printf(
            "Free chunk %p -> %p (%i) \n", currentFreeEntry, currentFreeEntry->afterEnd(), currentFreeEntry->getSize());
        currentFreeEntry = currentFreeEntry->getNext(compressed);
    }
}
 ...
 */
bool MM_MemoryPoolAddressOrderedList::isMemoryPoolValid(MM_EnvironmentBase* env, bool postCollect)
{
    bool const compressed = compressObjectReferences();
 ...
 */
uintptr_t MM_MemoryPoolAddressOrderedList::getCurrentLargestFree(MM_EnvironmentBase* env)
{
    bool const compressed = compressObjectReferences();
    MM_HeapLinkedFreeHeader* currentFreeEntry = _heapFreeList;
    uintptr_t largestFree = 0;
 ...
 */
uintptr_t MM_MemoryPoolAddressOrderedList::getCurrentFreeMemorySize(MM_EnvironmentBase* env)
{
    bool const compressed = compressObjectReferences();
    MM_HeapLinkedFreeHeader* currentFreeEntry = _heapFreeList;
...
void MM_MemoryPoolAddressOrderedList::recalculateMemoryPoolStatistics(MM_EnvironmentBase* env)
{
    bool const compressed = compressObjectReferences();
    uintptr_t largestFreeEntry = 0;
    uintptr_t freeBytes = 0;
...
uintptr_t MM_MemoryPoolAddressOrderedList::releaseFreeMemoryPages(MM_EnvironmentBase* env)
{
