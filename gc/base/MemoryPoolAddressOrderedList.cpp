 ...
 */
MM_MemoryPoolAddressOrderedList *MM_MemoryPoolAddressOrderedList::newInstance(
    MM_EnvironmentBase *env, uintptr_t minimumFreeEntrySize) {
 ...
 */
MM_MemoryPoolAddressOrderedList *MM_MemoryPoolAddressOrderedList::newInstance(
    MM_EnvironmentBase *env, uintptr_t minimumFreeEntrySize, const char *name) {
  MM_MemoryPoolAddressOrderedList *memoryPool;

  memoryPool = (MM_MemoryPoolAddressOrderedList *)env->getForge()->allocate(
      sizeof(MM_MemoryPoolAddressOrderedList),
      OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
  if (memoryPool) {
    memoryPool = new (memoryPool)
        MM_MemoryPoolAddressOrderedList(env, minimumFreeEntrySize, name);
...
bool MM_MemoryPoolAddressOrderedList::initialize(MM_EnvironmentBase *env) {
  ...
  _largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(
      env, (uint16_t)ext->largeObjectAllocationProfilingTopK,
      ext->largeObjectAllocationProfilingThreshold,
      ext->largeObjectAllocationProfilingVeryLargeObjectThreshold,
      (float)ext->largeObjectAllocationProfilingSizeClassRatio / (float)100.0,
      _extensions->heap->getMaximumMemorySize(),
      tlhMaximumSize + _minimumFreeEntrySize, _extensions->tlhMinimumSize);
#else
  _largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(
      env, (uint16_t)ext->largeObjectAllocationProfilingTopK,
      ext->largeObjectAllocationProfilingThreshold,
   ...
   * _largeObjectCollectorAllocateStats yet. Tenure SubSpace for Gencon will set
   * _largeObjectCollectorAllocateStats to _largeObjectAllocateStats (we append
   * collector stats to mutator stats) SemiSpace will leave
   * _largeObjectCollectorAllocateStats at NULL (no interest in Collector stats)
   * Tenure SubSpace for Flat will leave _largeObjectCollectorAllocateStats at
   * NULL (no interest in Colletor stats)
   */

  if (!_heapLock.initialize(env, &ext->lnrlOptions,
                            "MM_MemoryPoolAddressOrderedList:_heapLock")) {
    return false;
  }

  if (!_resetLock.initialize(env, &ext->lnrlOptions,
                             "MM_MemoryPoolAddressOrderedList:_resetLock")) {
    return false;
  }
...
bool MM_MemoryPoolAddressOrderedList::initializeSweepPool(
    MM_EnvironmentBase *env) {
  if (NULL == _sweepPoolState) {
    /*
     * Create Sweep Pool State for MPAOL
     */
    MM_Collector *globalCollector =
        (MM_Collector *)_extensions->getGlobalCollector();
    Assert_MM_true(NULL != globalCollector);

    _sweepPoolState = static_cast<MM_SweepPoolState *>(
        globalCollector->createSweepPoolState(env, this));
    if (NULL == _sweepPoolState) {
      return false;
    }
     ...
     */
    _sweepPoolManager =
        (MM_SweepPoolManagerAddressOrderedListBase *)env->getExtensions()
            ->sweepPoolManagerAddressOrderedList;
  }
  return true;
}

void MM_MemoryPoolAddressOrderedList::tearDown(MM_EnvironmentBase *env) {
 ...
 ****************************************
 */
void MM_MemoryPoolAddressOrderedList::clearHints() {
...
MMINLINE void MM_MemoryPoolAddressOrderedList::addHint(
    MM_HeapLinkedFreeHeader *freeEntry, uintptr_t lookupSize) {
  ...
  while (currentActiveHint) {
    /* Check what address range the free header to be added lies in (below,
     * equal, above) */
...
MMINLINE J9ModronAllocateHint *MM_MemoryPoolAddressOrderedList::findHint(
    uintptr_t lookupSize) {
  J9ModronAllocateHint *hint = NULL, *candidateHint, *previousHint;

  /* Be sure to remove stale hints as we search the list (stale hints are
   * addresses < the heap free head) */
...
MMINLINE void MM_MemoryPoolAddressOrderedList::removeHint(
...
MMINLINE void MM_MemoryPoolAddressOrderedList::updateHint(
    MM_HeapLinkedFreeHeader *oldFreeEntry,
 ...
 * For all active hints, find any which have a pointer greater than the given
 * free entry.  If found, reset the pointer to be the free entry.  This is used
 * when free entries are added to the middle of a free list; hints may point
 * beyond valid free entries in the list.
 */
void MM_MemoryPoolAddressOrderedList::updateHintsBeyondEntry(
    MM_HeapLinkedFreeHeader *freeEntry) {
  J9ModronAllocateHint *hint = _hintActive;
 ...
 ****************************************
 */
MMINLINE void *MM_MemoryPoolAddressOrderedList::internalAllocate(
    MM_EnvironmentBase *env, uintptr_t sizeInBytesRequired,
...
retry:
  ...
  while (currentFreeEntry) {
    ...
    /* while we are walking, keep track of the largest free entry.  We will need
     * this in the case of allocation failure to update the pool's largest free
     */
    ...
    Assert_MM_true((NULL == currentFreeEntry) ||
                   (currentFreeEntry > previousFreeEntry));
  }

  /* Check if an entry was found */
  if (!currentFreeEntry) {
#if defined(OMR_GC_CONCURRENT_SWEEP)
    if (_memorySubSpace->replenishPoolForAllocate(env, this,
                                                  sizeInBytesRequired)) {
      goto retry;
    }
#endif /* OMR_GC_CONCURRENT_SWEEP */
  ...
  _largeObjectAllocateStats->decrementFreeEntrySizeClassStats(
      currentFreeEntry->getSize());
  if ((walkCount >= J9MODRON_ALLOCATION_MANAGER_HINT_MAX_WALK) ||
      ((walkCount > 1) && allocateHintUsed)) {
  ...
  recycleEntry = (MM_HeapLinkedFreeHeader *)(((uint8_t *)currentFreeEntry) +
                                             sizeInBytesRequired);

  if (recycleHeapChunk(
          recycleEntry, ((uint8_t *)recycleEntry) + recycleEntrySize,
          previousFreeEntry, currentFreeEntry->getNext(compressed))) {
    updateHint(currentFreeEntry, recycleEntry);
    _largeObjectAllocateStats->incrementFreeEntrySizeClassStats(
        recycleEntrySize);
  } else {
  ...
  /* Collector object allocate stats for Survivor are not interesting
   * (_largeObjectCollectorAllocateStats is null for Survivor) */
...
fail_allocate:
  /* Since we failed to allocate, update the largest free entry so that outside
   * callers will be able to skip this pool, next time, in Tarok configurations
   */
...
void *MM_MemoryPoolAddressOrderedList::allocateObject(
...
void *MM_MemoryPoolAddressOrderedList::collectorAllocate(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
    bool lockingRequired) {
  void *addr =
      internalAllocate(env, allocDescription->getContiguousBytes(),
                       lockingRequired, _largeObjectCollectorAllocateStats);
...
MMINLINE bool MM_MemoryPoolAddressOrderedList::internalAllocateTLH(
    MM_EnvironmentBase *env, uintptr_t maximumSizeInBytesRequired,
...
retry:
  ...
  consumedSize = (maximumSizeInBytesRequired > freeEntrySize)
                     ? freeEntrySize
                     : maximumSizeInBytesRequired;
  ...
  /* Collector TLH allocate stats for Survivor are not interesting
   * (_largeObjectCollectorAllocateStats is null for Survivor) */
  if (NULL != largeObjectAllocateStats) {
...
fail_allocate:
  /* if we failed to allocate a TLH, this pool is either full or so heavily
   * fragmented that it is effectively full */
  _largestFreeEntry = 0;
  if (lockingRequired) {
    _heapLock.release();
  }
...
void *MM_MemoryPoolAddressOrderedList::allocateTLH(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
...
#if defined(OMR_GC_ALLOCATION_TAX)
    if (env->getExtensions()->payAllocationTax) {
      allocDescription->setAllocationTaxSize((uint8_t *)addrTop -
                                             (uint8_t *)addrBase);
    }
#endif /* OMR_GC_ALLOCATION_TAX */
...
void *MM_MemoryPoolAddressOrderedList::collectorAllocateTLH(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
    uintptr_t maximumSizeInBytesRequired, void *&addrBase, void *&addrTop,
    bool lockingRequired) {
  void *base = NULL;
  if (internalAllocateTLH(env, maximumSizeInBytesRequired, addrBase, addrTop,
                          lockingRequired,
                          _largeObjectCollectorAllocateStats)) {
    base = addrBase;
    allocDescription->setTLHAllocation(true);
    allocDescription->setNurseryAllocation(
        (_memorySubSpace->getTypeFlags() == MEMORY_TYPE_NEW) ? true : false);
...
void MM_MemoryPoolAddressOrderedList::reset(Cause cause) {
 ...
 * everything is free. Returns the freelist entry created at the end of the
 * given region
 */
MM_HeapLinkedFreeHeader *
MM_MemoryPoolAddressOrderedList::rebuildFreeListInRegion(
  ...
  /* TODO 108399 Determine whether this is still necessary (OMR_SCAVENGER_DEBUG
   * is defined in Scqavenger.cpp and is not accessible here) */
  ...
  if (createFreeEntry(env, rangeBase, rangeTop, previousFreeEntry, NULL)) {
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_REBUILD_FREE_LIST(
        env->getExtensions()->privateHookInterface, env->getOmrVMThread(),
        rangeBase, rangeTop);
  }
  unlock(env);
...
#if defined(DEBUG)
bool MM_MemoryPoolAddressOrderedList::isValidListOrdering() {
  bool const compressed = compressObjectReferences();
  MM_HeapLinkedFreeHeader *walk = _heapFreeList;
  while (NULL != walk) {
    if ((NULL != walk->getNext(compressed)) &&
 ...
 *
 */
void MM_MemoryPoolAddressOrderedList::expandWithRange(MM_EnvironmentBase *env,
  ...
  /* Find the free entries in the list the appear before/after the range being
   * added */
  previousFreeEntry = NULL;
  ...
  /* Check if the range can be coalesced with either the surrounding free
   * entries */
  if (canCoalesce) {
    /* Check if the range can be fused to the tail previous free entry */
    if (previousFreeEntry &&
        (lowAddress == (void *)(((uintptr_t)previousFreeEntry) +
                                previousFreeEntry->getSize()))) {
      _largeObjectAllocateStats->decrementFreeEntrySizeClassStats(
          previousFreeEntry->getSize());
    ...
    if (nextFreeEntry && (highAddress == (void *)nextFreeEntry)) {
      MM_HeapLinkedFreeHeader *newFreeEntry =
          (MM_HeapLinkedFreeHeader *)lowAddress;
      assume0((NULL == nextFreeEntry->getNext(compressed)) ||
              (newFreeEntry < nextFreeEntry->getNext(compressed)));

      _largeObjectAllocateStats->decrementFreeEntrySizeClassStats(
          nextFreeEntry->getSize());
  ...
  /* No coalescing available - build a free entry from the range that will be
   * inserted into the list */
  MM_HeapLinkedFreeHeader *freeEntry = (MM_HeapLinkedFreeHeader *)lowAddress;

#if defined(OMR_VALGRIND_MEMCHECK)
  valgrindMakeMemUndefined((uintptr_t)freeEntry,
                           sizeof(MM_HeapLinkedFreeHeader));
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
 ...
 * @param highAddress High address of memory to remove (non inclusive)
 *
 * @note The expectation is that the range consists ONLY of free elements (no
 * live data appears).
 *
 */
void *MM_MemoryPoolAddressOrderedList::contractWithRange(
    MM_EnvironmentBase *env, uintptr_t contractSize, void *lowAddress,
    void *highAddress) {
  bool const compressed = compressObjectReferences();
  MM_HeapLinkedFreeHeader *currentFreeEntry, *currentFreeEntryTop,
      *previousFreeEntry, *nextFreeEntry;
  ...
  /* TODO: Could we use hints to find a better starting address?  Are hints
   * still valid? */
  previousFreeEntry = NULL;
  currentFreeEntry = _heapFreeList;
  while (currentFreeEntry) {
    if ((lowAddress >= currentFreeEntry) &&
        (highAddress <= (void *)(((uintptr_t)currentFreeEntry) +
                                 currentFreeEntry->getSize()))) {
      break;
    }
    previousFreeEntry = currentFreeEntry;
  ...
  /* Record the next free entry after the current one which we are going to
   * contract */
  nextFreeEntry = currentFreeEntry->getNext(compressed);

  /* Glue any new free entries that have been split from the original free entry
   * into the free list */
  /* Start back to front, attaching the current entry to the next entry, and the
   * current entry taking the place of the next entry */

  /* Determine what to do with any trailing bytes to the free entry that are not
   * being contracted. */
  currentFreeEntryTop =
      (MM_HeapLinkedFreeHeader *)(((uintptr_t)currentFreeEntry) +
                                  currentFreeEntry->getSize());
  if (currentFreeEntryTop != (MM_HeapLinkedFreeHeader *)highAddress) {
    /* Space at the tail that is not being contracted - is it a valid free
     * entry? */
    if (createFreeEntry(env, highAddress, currentFreeEntryTop, NULL,
                        nextFreeEntry)) {
      ...
      uintptr_t trailingSize =
          ((uintptr_t)currentFreeEntryTop) - ((uintptr_t)highAddress);
      totalContractSize += trailingSize;
    }
  }

  /* Determine what to do with any leading bytes to the free entry that are not
   * being contracted. */
  if (currentFreeEntry != (MM_HeapLinkedFreeHeader *)lowAddress) {
    if (createFreeEntry(env, currentFreeEntry, lowAddress, NULL,
                        nextFreeEntry)) {
      ...
      uintptr_t leadingSize =
          ((uintptr_t)lowAddress) - ((uintptr_t)currentFreeEntry);
      totalContractSize += leadingSize;
    }
  }

  /* Attach our findings to the previous entry, and assume the head of the list
   * if there was none */
  if (previousFreeEntry) {
 ...
 *
 */
void MM_MemoryPoolAddressOrderedList::addFreeEntries(
    MM_EnvironmentBase *env, MM_HeapLinkedFreeHeader *&freeListHead,
  ...
  if (previousFreeEntry == NULL) {
    ...
    if ((uint8_t *)freeListTail->afterEnd() == (uint8_t *)_heapFreeList) {
      _largeObjectAllocateStats->decrementFreeEntrySizeClassStats(
          _heapFreeList->getSize());
      _largeObjectAllocateStats->decrementFreeEntrySizeClassStats(
          freeListTail->getSize());
      freeListTail->expandSize(_heapFreeList->getSize());
      assume0((NULL == _heapFreeList->getNext(compressed)) ||
              (freeListTail < _heapFreeList->getNext(compressed)));
    ...
    assume0((NULL == previousFreeEntry->getNext(compressed)) ||
            (freeListTail < previousFreeEntry->getNext(compressed)));
    ...
    if ((uint8_t *)previousFreeEntry->afterEnd() == (uint8_t *)freeListHead) {
      _largeObjectAllocateStats->decrementFreeEntrySizeClassStats(
          freeListHead->getSize());
      _largeObjectAllocateStats->decrementFreeEntrySizeClassStats(
          previousFreeEntry->getSize());
      previousFreeEntry->expandSize(freeListHead->getSize());
      assume0((NULL == freeListHead->getNext(compressed)) ||
              (previousFreeEntry < freeListHead->getNext(compressed)));
...
bool MM_MemoryPoolAddressOrderedList::removeFreeEntriesWithinRange(
  ...
  MM_HeapLinkedFreeHeader *currentFreeEntry, *previousFreeEntry, *nextFreeEntry,
      *tailFreeEntry;
  ...
  /* Remember the next free entry after the current one which we are going to
   * consume at least part of */
  nextFreeEntry = currentFreeEntry->getNext(compressed);
  ...
  if (currentFreeEntry < (MM_HeapLinkedFreeHeader *)lowAddress) {
     ...
     */
    if (createFreeEntry(env, currentFreeEntry, lowAddress, previousFreeEntry,
                        NULL)) {
   ...
   * returned to caller */
  if (currentFreeEntryTop > (MM_HeapLinkedFreeHeader *)highAddress) {
    /* Space at the tail that is not being returned - is it a valid free entry
     * for this pool ? */
    if (createFreeEntry(env, highAddress, currentFreeEntryTop,
                        previousFreeEntry, NULL)) {
      trailingSize =
          ((uintptr_t)currentFreeEntryTop) - ((uintptr_t)highAddress);
      if (NULL == previousFreeEntry) {
  ...
  /* Append whats left, if big enough to list to be returned. if not big enough
   * it is abandoned */
  if (appendToList(env, baseAddr, topAddr, minimumSize, retListHead,
                   retListTail)) {
    retListMemoryCount++;
  ...
  while (currentFreeEntry &&
         (((uint8_t *)currentFreeEntry->afterEnd()) <= highAddress)) {
    tailFreeEntry = currentFreeEntry->getNext(compressed);

    if (appendToList(env, (void *)currentFreeEntry,
                     (void *)currentFreeEntry->afterEnd(), minimumSize,
                     retListHead, retListTail)) {
      retListMemoryCount++;
  ...
  if (currentFreeEntry && currentFreeEntry < highAddress) {
     ...
     * for this pool ? */
    if (createFreeEntry(env, highAddress, currentFreeEntryTop,
                        previousFreeEntry, tailFreeEntry)) {
      trailingSize =
          ((uintptr_t)currentFreeEntryTop) - ((uintptr_t)highAddress);
    ...
    if (appendToList(env, currentFreeEntry, highAddress, minimumSize,
                     retListHead, retListTail)) {
      retListMemoryCount++;
  ...
  /* Attach the remaining tail to the previous entry, and assume the head of the
   * list if there was none */
  if (previousFreeEntry) {
...
/**
 * Determine the address in this memoryPool where there is at least sizeRequired
 * free bytes in free entries. The free entries must be of size minimumSize to
 * be counted.
 *
 * @param sizeRequired Amount of free memory to be returned
 * @param minimumSize Minimum size of free entry we are intrested in
 *
 * @return address of memory location which has sizeRequired free bytes
 * available in this memoryPool.
 * @return NULL if this memory pool can not satisfy sizeRequired bytes.
 */
void *MM_MemoryPoolAddressOrderedList::findAddressAfterFreeSize(
  ...
  while (NULL != currentFreeEntry) {
    /* Removing entries has left us needing less than the minimum size of the
     * entries. To ensure we get the size needed, bump up the
     * remainingBytesNeeded.
     */
    ...
    if (remainingBytesNeeded >= currentFreeEntry->getSize()) {
      ...
      if (currentFreeEntry->getSize() - remainingBytesNeeded <
          _minimumFreeEntrySize) {
        /* The rest of the entry is too small to meet minimum size (for this
         * pool), include it too */
        return (void *)currentFreeEntry->afterEnd();
      }
...
bool MM_MemoryPoolAddressOrderedList::recycleHeapChunk(
    void *addrBase, void *addrTop, MM_HeapLinkedFreeHeader *previousFreeEntry,
  ...
  if (previousFreeEntry) {
    Assert_MM_true((NULL == nextFreeEntry) ||
                   (previousFreeEntry < nextFreeEntry));
    previousFreeEntry->setNext(nextFreeEntry, compressed);
  } else {
    _heapFreeList = nextFreeEntry;
 ...
 */
void *MM_MemoryPoolAddressOrderedList::findFreeEntryEndingAtAddr(
 ...
 */
uintptr_t
MM_MemoryPoolAddressOrderedList::getAvailableContractionSizeForRangeEndingAt(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
    void *lowAddr, void *highAddr) {
  MM_HeapLinkedFreeHeader *lastFree;

  lastFree =
      (MM_HeapLinkedFreeHeader *)findFreeEntryEndingAtAddr(env, highAddr);
 ...
 */
void *MM_MemoryPoolAddressOrderedList::findFreeEntryTopStartingAtAddr(
    MM_EnvironmentBase *env, void *addr) {
  bool const compressed = compressObjectReferences();
  MM_HeapLinkedFreeHeader *currentFreeEntry;
  ...
  while (currentFreeEntry) {
    ...
    /* Address ordered list - if the current entry is greater than the one we
     * are looking for then there is no valid entry
     */
    if ((void *)currentFreeEntry > addr) {
      break;
    }
 ...
 */
void *MM_MemoryPoolAddressOrderedList::getFirstFreeStartingAddr(
    MM_EnvironmentBase *env) {
 ...
 */
void *MM_MemoryPoolAddressOrderedList::getNextFreeStartingAddr(
 ...
 *
 */
void MM_MemoryPoolAddressOrderedList::moveHeap(MM_EnvironmentBase *env,
                                               void *srcBase, void *srcTop,
                                               void *dstBase) {
  bool const compressed = compressObjectReferences();
  ...
  currentFreeEntry = _heapFreeList;
  while (currentFreeEntry) {
    if (((void *)currentFreeEntry >= srcBase) &&
        ((void *)currentFreeEntry < srcTop)) {
      MM_HeapLinkedFreeHeader *newFreeEntry;
      newFreeEntry =
          (MM_HeapLinkedFreeHeader *)((((uintptr_t)currentFreeEntry) -
                                       ((uintptr_t)srcBase)) +
 ...
 */
void MM_MemoryPoolAddressOrderedList::lock(MM_EnvironmentBase *env) {
  _heapLock.acquire();
}
 ...
 */
void MM_MemoryPoolAddressOrderedList::unlock(MM_EnvironmentBase *env) {
  _heapLock.release();
}
 ...
 */
bool MM_MemoryPoolAddressOrderedList::recycleHeapChunk(void *chunkBase,
                                                       void *chunkTop) {
  bool const compressed = compressObjectReferences();
  bool recycled = false;
  ...
  if ((NULL == _heapFreeList) || (chunkBase < (void *)_heapFreeList)) {
    ...
    while (NULL != currentFreeEntry) {
      ...
      if (NULL == next) {
        ...
        recycled =
            recycleHeapChunk(chunkBase, chunkTop, currentFreeEntry, NULL);
        break;
      } else if ((void *)next > chunkBase) {
        /* Insert chunk between current entry and next one */
        recycled =
            recycleHeapChunk(chunkBase, chunkTop, currentFreeEntry, next);
        break;
      }
 ...
 */
void MM_MemoryPoolAddressOrderedList::printCurrentFreeList(
    MM_EnvironmentBase *env, const char *area) {
  bool const compressed = compressObjectReferences();
  ...
  while (currentFreeEntry) {
    omrtty_printf("Free chunk %p -> %p (%i) \n", currentFreeEntry,
                  currentFreeEntry->afterEnd(), currentFreeEntry->getSize());
    currentFreeEntry = currentFreeEntry->getNext(compressed);
  }
}
 ...
 */
bool MM_MemoryPoolAddressOrderedList::isMemoryPoolValid(MM_EnvironmentBase *env,
                                                        bool postCollect) {
  bool const compressed = compressObjectReferences();
  ...
  if (freeBytes == _freeMemorySize && freeCount == _freeEntryCount &&
 ...
 */
uintptr_t MM_MemoryPoolAddressOrderedList::getCurrentLargestFree(
    MM_EnvironmentBase *env) {
  bool const compressed = compressObjectReferences();
  MM_HeapLinkedFreeHeader *currentFreeEntry = _heapFreeList;
  uintptr_t largestFree = 0;
 ...
 */
uintptr_t MM_MemoryPoolAddressOrderedList::getCurrentFreeMemorySize(
    MM_EnvironmentBase *env) {
  bool const compressed = compressObjectReferences();
  MM_HeapLinkedFreeHeader *currentFreeEntry = _heapFreeList;
...
void MM_MemoryPoolAddressOrderedList::recalculateMemoryPoolStatistics(
    MM_EnvironmentBase *env) {
  bool const compressed = compressObjectReferences();
  uintptr_t largestFreeEntry = 0;
  uintptr_t freeBytes = 0;
  ...
  MM_HeapLinkedFreeHeader *freeHeader =
      (MM_HeapLinkedFreeHeader *)getFirstFreeStartingAddr(env);
...
void MM_MemoryPoolAddressOrderedList::appendCollectorLargeAllocateStats() {
  _largeObjectCollectorAllocateStats = _largeObjectAllocateStats;
}

#if defined(OMR_GC_IDLE_HEAP_MANAGER)
uintptr_t MM_MemoryPoolAddressOrderedList::releaseFreeMemoryPages(
    MM_EnvironmentBase *env) {
