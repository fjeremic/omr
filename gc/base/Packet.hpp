...
public:
  ...
  /**
   * Low tags the address of the owning threads env so we know its no longer
   * owned by a thread but we do know which thread last used it for input/output
   */
  ...
  bool initialize(MM_EnvironmentBase *env, MM_Packet *next, MM_Packet *previous,
   ...
   */
  MM_Packet()
      : MM_BaseNonVirtual(), _baseAddress(NULL), _basePtr(NULL), _topPtr(NULL),
        _currentPtr(NULL), _sublistIndex(0), _owner(NULL), _next(NULL),
        _previous(NULL) {
