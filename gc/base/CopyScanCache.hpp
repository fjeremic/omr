...
public:
   ...
   */
  MM_CopyScanCache()
      : MM_Base(), next(NULL), flags(0), _hasPartiallyScannedObject(false),
        cacheBase(NULL), cacheTop(NULL), cacheAlloc(NULL), scanCurrent(NULL) {}

  MM_CopyScanCache(uintptr_t givenFlags)
      : MM_Base(), next(NULL), flags(givenFlags),
        _hasPartiallyScannedObject(false), cacheBase(NULL), cacheTop(NULL),
