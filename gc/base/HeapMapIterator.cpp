...
bool MM_HeapMapIterator::reset(MM_HeapMap* heapMap, uintptr_t* heapChunkBase, uintptr_t* heapChunkTop)
{
...
omrobjectptr_t MM_HeapMapIterator::nextObject()
{
     ...
     * The second goal we accomplish in part by caching the heap map slot value.  This may seem like unnecessary work
     * but in fact avoids polluting the memory hierarchy and results in a net performance win.  Be aware when changing
     * this code that you should avoid fetching values from memory (non-local to the stack) as much as possible.
     */
    while (_heapSlotCurrent < _heapChunkTop) {
        ...
        if (_heapMapSlotValue != J9MODRON_HMI_SLOT_EMPTY) {
            ...
            if (_useLargeObjectOptimization) {
                sizeInHeapMapBits = MM_Bits::convertBytesToSlots(
