                              ...
                              uint32_t memoryCategory)
{
  MM_VirtualMemory* vmem = (MM_VirtualMemory*)env->getForge()->allocate(
    sizeof(MM_VirtualMemory),
    OMR::GC::AllocationCategory::FIXED,
    OMR_GET_CALLSITE());

  if (vmem) {
    new (vmem) MM_VirtualMemory(
      env, heapAlignment, pageSize, pageFlags, tailPadding, mode);
    if (!vmem->initialize(
          env, size, preferredAddress, ceiling, options, memoryCategory)) {
      vmem->kill(env);
      vmem = NULL;
    }
  }
 ...
 */
bool
MM_VirtualMemory::decommitMemory(void* address,
                                 uintptr_t size,
...
bool
MM_VirtualMemory::setNumaAffinity(uintptr_t numaNode,
                                  void* address,
                                  uintptr_t byteAmount)
{
  ...
  if (_extensions->_numaManager.isPhysicalNUMASupported()) {
    ...
    didSetAffinity =
      (0 == omrvmem_numa_set_affinity(
              numaNode, address, byteAmountPageAligned, &_identifier));
  }
