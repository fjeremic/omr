...
MM_VirtualMemory *MM_VirtualMemory::newInstance(
  ...
  MM_VirtualMemory *vmem = (MM_VirtualMemory *)env->getForge()->allocate(
      sizeof(MM_VirtualMemory), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());

  if (vmem) {
    new (vmem) MM_VirtualMemory(env, heapAlignment, pageSize, pageFlags,
                                tailPadding, mode);
    if (!vmem->initialize(env, size, preferredAddress, ceiling, options,
                          memoryCategory)) {
      vmem->kill(env);
      vmem = NULL;
    }
...
void MM_VirtualMemory::kill(MM_EnvironmentBase *env) {
  tearDown(env);
  env->getForge()->free(this);
}

bool MM_VirtualMemory::initialize(MM_EnvironmentBase *env, uintptr_t size,
                                  void *preferredAddress, void *ceiling,
  ...
  if ((NULL != ceiling) && (params.byteAmount <= (uintptr_t)ceiling)) {
     ...
     * if direction is top down an allocation would be attempted from endAddress
     * first regardless it page aligned or not For unaligned case an allocation
     * will succeed but not in requested pages As far as this is only place in
     * GC used endAddress add rounding here (another case is handling of
     * preferredAddress - we do not care)
     */
    maxEndAddress =
        (void *)MM_Math::roundToFloor(_pageSize, (uintptr_t)maxEndAddress);
  ...
  if (NULL != _heapBase) {
    ...
    if (lastByte == HIGH_ADDRESS) {
      _heapTop = (void *)MM_Math::roundToFloor(
          _heapAlignment, ((uintptr_t)_baseAddress) +
                              (allocateSize - _tailPadding - _heapAlignment));
    } else {
      _heapTop = (void *)MM_Math::roundToFloor(
          _heapAlignment,
          ((uintptr_t)_baseAddress) + (allocateSize - _tailPadding));
    }

    if ((_heapBase >= _heapTop) /* CMVC 45178: Need to catch the case where we
                                   aligned heapTop and heapBase to the same
                                   address and consider it an error. */
        || ((NULL != ceiling) &&
            (_heapTop >
             ceiling)) /* Check that memory we got is located below ceiling */
    ) {
      freeMemory();
      _heapBase = NULL;
    }
...
void MM_VirtualMemory::roundDownTop(uintptr_t rounding) {
  _heapTop = (void *)MM_Math::roundToFloor(
      _heapAlignment, ((uintptr_t)_heapBase) + (_reserveSize - rounding));
}

void *MM_VirtualMemory::reserveMemory(J9PortVmemParams *params) {
  OMRPORT_ACCESS_FROM_OMRVM(_extensions->getOmrVM());

  /* be sure that nothing allocated otherwise it would be lost for memory free
   * operation */
  ...
  if (NULL != _baseAddress) {
    ...
    Assert_MM_true(0 != _pageSize);
    addressToReturn = (void *)MM_Math::roundToCeiling(_heapAlignment,
...
void *MM_VirtualMemory::doubleMapArraylet(
  ...
  uintptr_t mode = OMRPORT_VMEM_MEMORY_MODE_READ |
                   OMRPORT_VMEM_MEMORY_MODE_WRITE |
                   OMRPORT_VMEM_MEMORY_MODE_COMMIT;

  return omrvmem_get_contiguous_region_memory(
      arrayletLeaves, arrayletLeafCount, arrayletLeafSize, byteAmount,
...
bool MM_VirtualMemory::freeMemory() {
  OMRPORT_ACCESS_FROM_OMRVM(_extensions->getOmrVM());
  bool success =
      (0 == omrvmem_free_memory(_baseAddress, _reserveSize, &_identifier));
  if (success) {
    _baseAddress = NULL;
    _reserveSize = 0;
  }
 ...
 */
bool MM_VirtualMemory::commitMemory(void *address, uintptr_t size) {
  OMRPORT_ACCESS_FROM_OMRVM(_extensions->getOmrVM());
  Assert_MM_true(0 != _pageSize);
  ...
  void *commitBase =
      (void *)MM_Math::roundToFloor(_pageSize, (uintptr_t)address);
  void *commitTop = (void *)MM_Math::roundToCeiling(
      _pageSize, (uintptr_t)address + size + _tailPadding);
 ...
 * @param lowValidAddress the end of the previous committed block below address,
 * or NULL if address is the first committed block
 * @param highValidAddress the start of the next committed block above address,
 * or NULL if address is the last committed block
 * @return true if successful, false otherwise.
 */
bool MM_VirtualMemory::decommitMemory(void *address, uintptr_t size,
  ...
  /* port library takes page aligned addresses and sizes only */
  decommitBase =
      (void *)MM_Math::roundToCeiling(_pageSize, (uintptr_t)decommitBase);
  decommitTop =
      (void *)MM_Math::roundToFloor(_pageSize, (uintptr_t)decommitTop);

  if (decommitBase < decommitTop) {
    /* There is still memory to decommit, calculate size */
    uintptr_t decommitSize =
        ((uintptr_t)decommitTop) - ((uintptr_t)decommitBase);
    result =
        omrvmem_decommit_memory(decommitBase, decommitSize, &_identifier) == 0;
  }

  return result;
}

void MM_VirtualMemory::tearDown(MM_EnvironmentBase *env) {
  if (NULL != _heapBase) {
    freeMemory();
    _heapBase = NULL;
  }
...
bool MM_VirtualMemory::setNumaAffinity(uintptr_t numaNode, void *address,
  ...
  if (_extensions->_numaManager.isPhysicalNUMASupported()) {
    ...
    uintptr_t byteAmountPageAligned =
        MM_Math::roundToCeiling(_pageSize, byteAmount);
    /* aligned high address might be higher then heapTop but
     * must be in the heap reserved memory range
     */
    Assert_MM_true(((uintptr_t)address + byteAmountPageAligned) <=
                   ((uintptr_t)_heapBase + _reserveSize));

    didSetAffinity =
        (0 == omrvmem_numa_set_affinity(numaNode, address,
                                        byteAmountPageAligned, &_identifier));
  }
