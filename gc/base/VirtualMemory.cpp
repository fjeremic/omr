...
MM_VirtualMemory* MM_VirtualMemory::newInstance(MM_EnvironmentBase* env, uintptr_t heapAlignment, uintptr_t size,
    uintptr_t pageSize, uintptr_t pageFlags, uintptr_t tailPadding, void* preferredAddress, void* ceiling,
    uintptr_t mode, uintptr_t options, uint32_t memoryCategory)
{
...
bool MM_VirtualMemory::initialize(MM_EnvironmentBase* env, uintptr_t size, void* preferredAddress, void* ceiling,
    uintptr_t options, uint32_t memoryCategory)
{
    ...
    if ((NULL != ceiling) && (params.byteAmount <= (uintptr_t)ceiling)) {
         ...
         * if direction is top down an allocation would be attempted from endAddress first regardless it page aligned or
         * not For unaligned case an allocation will succeed but not in requested pages As far as this is only place in
         * GC used endAddress add rounding here (another case is handling of preferredAddress - we do not care)
         */
    ...
    if (NULL != _heapBase) {
        ...
        if (lastByte == HIGH_ADDRESS) {
            _heapTop = (void*)MM_Math::roundToFloor(
                _heapAlignment, ((uintptr_t)_baseAddress) + (allocateSize - _tailPadding - _heapAlignment));
        } else {
            _heapTop = (void*)MM_Math::roundToFloor(
                _heapAlignment, ((uintptr_t)_baseAddress) + (allocateSize - _tailPadding));
        }

        if ((_heapBase >= _heapTop) /* CMVC 45178: Need to catch the case where we aligned heapTop and heapBase to the
                                       same address and consider it an error. */
...
void* MM_VirtualMemory::doubleMapArraylet(MM_EnvironmentBase* env, void* arrayletLeaves[], UDATA arrayletLeafCount,
 ...
 * @return true if successful, false otherwise.
 */
bool MM_VirtualMemory::decommitMemory(void* address, uintptr_t size, void* lowValidAddress, void* highValidAddress)
{
    bool result = true;
...
bool MM_VirtualMemory::setNumaAffinity(uintptr_t numaNode, void* address, uintptr_t byteAmount)
{
    Assert_MM_true(0 != _pageSize);
