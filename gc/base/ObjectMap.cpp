...
MM_ObjectMap *MM_ObjectMap::newInstance(MM_EnvironmentBase *env) {
  MM_ObjectMap *objectMap;

  objectMap = (MM_ObjectMap *)env->getForge()->allocate(
      sizeof(MM_ObjectMap), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (objectMap) {
...
bool MM_ObjectMap::initialize(MM_EnvironmentBase *env) {
  _objectMap = MM_MarkMap::newInstance(
...
bool MM_ObjectMap::heapAddRange(MM_EnvironmentBase *env,
...
bool MM_ObjectMap::heapRemoveRange(MM_EnvironmentBase *env,
                                   MM_MemorySubSpace *subspace, uintptr_t size,
                                   void *lowAddress, void *highAddress,
...
bool MM_ObjectMap::markValidObjectNoAtomic(MM_EnvironmentBase *env,
                                           omrobjectptr_t objectPtr) {
  return inlineMarkValidObjectNoAtomic(env, objectPtr);
}

bool MM_ObjectMap::markValidObject(MM_EnvironmentBase *env,
                                   omrobjectptr_t objectPtr) {
  return inlineMarkValidObject(env, objectPtr);
}

uintptr_t MM_ObjectMap::setMarkBitsInRange(MM_EnvironmentBase *env,
                                           void *heapBase, void *heapTop,
                                           bool clear) {
  return _objectMap->setBitsInRange(env, heapBase, heapTop, clear);
}

void MM_ObjectMap::markValidObjectForRange(MM_EnvironmentBase *env,
                                           void *heapBase, void *heapTop) {
  ...
  /* We must atomically mark bits in the first and last bytes of the MarkMap
   * belonging to this range. TLH.  This is because other threads may be marking
   * the same byte.
  ...
  GC_ObjectHeapIteratorAddressOrderedList objectHeapIterator(
      env->getExtensions(), (omrobjectptr_t)heapBase, heapTop, false, false);
  ...
  /* Remove one off of the range of heapTop, since heapTop is a non-inclusive
   * bound */
  _objectMap->getSlotIndexAndMask((omrobjectptr_t)((uintptr_t)heapTop - 1),
                                  &slotIndexHigh, &bitMask);
