 ...
 */
void
MM_Collector::collectorExpanded(MM_EnvironmentBase* env,
 ...
 */
bool
MM_Collector::canCollectorExpand(MM_EnvironmentBase* env,
                                 MM_MemorySubSpace* subSpace,
                                 uintptr_t expandSize)
{
  return false;
 ...
 */
bool
MM_Collector::forceKickoff(MM_EnvironmentBase* env,
                           MM_MemorySubSpace* subSpace,
 ...
 */
void
MM_Collector::payAllocationTax(MM_EnvironmentBase* env,
                               MM_MemorySubSpace* subspace,
 ...
 */
bool
MM_Collector::replenishPoolForAllocate(MM_EnvironmentBase* env,
...
MM_Collector::recordExcessiveStatsForGCEnd(MM_EnvironmentBase* env)
{
  ...
  if (stats->endGCTimeStamp > stats->startGCTimeStamp) {
    ...
    stats->totalGCTime +=
      (uint64_t)omrtime_hires_delta(stats->startGCTimeStamp,
  ...
  if (stats->endGCTimeStamp > stats->lastEndGlobalGCTimeStamp) {
    ...
    stats->newGCToUserTimeRatio =
      (float)((int64_t)stats->totalGCTime * 100.0 /
              (int64_t)omrtime_hires_delta(stats->lastEndGlobalGCTimeStamp,
                                           stats->endGCTimeStamp,
                                           OMRPORT_TIME_DELTA_IN_MICROSECONDS));
    stats->avgGCToUserTimeRatio =
      MM_Math::weightedAverage(stats->avgGCToUserTimeRatio,
                               stats->newGCToUserTimeRatio,
 ...
 */
void
MM_Collector::preCollect(MM_EnvironmentBase* env,
                         MM_MemorySubSpace* subSpace,
                         MM_AllocateDescription* allocDescription,
                         uint32_t gcCode)
{
 ...
 */
bool
MM_Collector::checkForExcessiveGC(MM_EnvironmentBase* env,
                                  MM_Collector* collector)
{
  ...
  if (stats->avgGCToUserTimeRatio > extensions->excessiveGCratio) {
    ...
    reclaimedPercent =
      ((float)heapFreeDelta / (float)extensions->heap->getActiveMemorySize()) *
      100;
  ...
  MM_AllocationContext* context)
{
  ...
  if (NULL != allocateDescription) {
    ...
    if (NULL != context) {
      ...
      postCollectAllocationResult =
        baseSubSpace->lockedReplenishAndAllocate(env,
                                                 context,
      ...
      postCollectAllocationResult =
        callingSubSpace->allocateGeneric(env,
                                         allocateDescription,
                                         allocationType,
                                         objectAllocationInterface,
                                         baseSubSpace);
 ...
 */
void
MM_Collector::abortCollection(MM_EnvironmentBase* env,
