 ...
 * This call is typically made from collectorAllocate() class of calls where a
 * collector asks a subspace to expand while trying to allocate memory for
 * objects.  An example is the scavenger, which will attempt to expand the
 * tenure area if it fails to allocate tenure space.  This call is expected to
 * be made under the allocation lock of a particular sub space - if there is no
 * lock, or multiple sub spaces could expand, some form of atomicity to this
 * operation needs to be introduced.
 ...
 */
void MM_Collector::collectorExpanded(MM_EnvironmentBase *env,
 ...
 */
bool MM_Collector::canCollectorExpand(MM_EnvironmentBase *env,
                                      MM_MemorySubSpace *subSpace,
                                      uintptr_t expandSize) {
  return false;
}
 ...
 */
uintptr_t MM_Collector::getCollectorExpandSize(MM_EnvironmentBase *env) {
  return 0;
}
 ...
 * @param allocDescription if non-NULL, contains information about the
 * allocation which triggered the GC. If NULL, the GC is a system GC.
 * @param gcCode requested GC code
 * @return true if kickoff can be forced
 */
bool MM_Collector::forceKickoff(MM_EnvironmentBase *env,
                                MM_MemorySubSpace *subSpace,
 ...
 */
void MM_Collector::payAllocationTax(MM_EnvironmentBase *env,
                                    MM_MemorySubSpace *subspace,
 ...
 */
bool MM_Collector::replenishPoolForAllocate(MM_EnvironmentBase *env,
 ...
 */
void MM_Collector::recordExcessiveStatsForGCStart(MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());

  extensions->excessiveGCStats.gcCount += 1;

  /* Record the heap free so we can calculate how much we've actually freed in
   * the collect */
  extensions->excessiveGCStats.freeMemorySizeBefore =
      extensions->heap->getActualActiveFreeMemorySize();
 ...
 * collect the stats they require
 */
void MM_Collector::recordExcessiveStatsForGCEnd(MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  ...
  /* We use the approximate value here because we want to be sure the math
   * believes we are actually doing sufficient freeing work to not decide there
   * is excessive GC'ing going on.
   */
  stats->freeMemorySizeAfter =
  ...
  if (stats->endGCTimeStamp > stats->startGCTimeStamp) {
    ...
    stats->totalGCTime += (uint64_t)omrtime_hires_delta(
        stats->startGCTimeStamp, stats->endGCTimeStamp,
        OMRPORT_TIME_DELTA_IN_MICROSECONDS);
  }

  if (stats->endGCTimeStamp > stats->lastEndGlobalGCTimeStamp) {
    /* The issue with doing the weighted average on the global collect is that
     * the new space collections can only change the ratio by is the maximum of
     * the weight.  Given that in a generational system, you are likely to
     * experience many more new space collects, this would make the excessive gc
     * detection be very, very slow.
     *
     ...
     * won't really make a decision until after a global" - which give the
     * global a chance to free up a lot of memory, increasing the free % above
     * the threshold, and therefore not trigger an excessive GC.
     */
    stats->newGCToUserTimeRatio =
        (float)((int64_t)stats->totalGCTime * 100.0 /
                (int64_t)omrtime_hires_delta(
                    stats->lastEndGlobalGCTimeStamp, stats->endGCTimeStamp,
                    OMRPORT_TIME_DELTA_IN_MICROSECONDS));
    stats->avgGCToUserTimeRatio = MM_Math::weightedAverage(
        stats->avgGCToUserTimeRatio, stats->newGCToUserTimeRatio,
        extensions->excessiveGCnewRatioWeight);
  }
}
 ...
 */
void MM_Collector::preCollect(MM_EnvironmentBase *env,
                              MM_MemorySubSpace *subSpace,
                              MM_AllocateDescription *allocDescription,
                              uint32_t gcCode) {
  MM_GCExtensionsBase *extensions = env->getExtensions();

  /* There might be a colliding concurrent cycle in progress, that must be
   * completed before we start this one. Specific Collector subclass will have
   * exact knowledge if that is the case.
   */
  completeExternalConcurrentCycle(env);

  /* Record the master GC thread CPU time at the start to diff later */
  _masterThreadCpuTimeStart =
  ...
  if (extensions->doFrequentObjectAllocationSampling) {
    if (NULL == extensions->frequentObjectsStats) {
      extensions->frequentObjectsStats =
          MM_FrequentObjectsStats::newInstance(env);
    }
    if (NULL != extensions->frequentObjectsStats) {
      OMR_VMThread *omrVMThread;
      MM_EnvironmentBase *omrVMThreadEnv;
      MM_FrequentObjectsStats *aggregateFrequentObjectsStats =
          extensions->frequentObjectsStats;
      ...
      while ((omrVMThread = threadListIterator.nextOMRVMThread()) != NULL) {
        ...
        MM_FrequentObjectsStats *frequentObjectsStats =
            omrVMThreadEnv->_objectAllocationInterface
  ...
  extensions->aggressive =
      (env->getCycleStateGCCode().isAggressiveGC() ? 1 : 0);

  _isRecursiveGC = extensions->isRecursiveGC;

  /* ExcessiveGC is tested when a global gc occurs.  The global gc may occur as
   * a result of a local gc.  We use extensions->didGlobalGC to track if a
   * global gc occured. If this is the outter most garbage collect invocation
   * then set the global gc flag to	false.
   */
  if (!_isRecursiveGC) {
    extensions->didGlobalGC = false;

    /* In case of recursive GC calls, we want to time only the outermost GC
     * invocation Only time implicit GCs -- system GCs are accounted for "user"
     * time in GC/total time ratio calculation
     */
  ...
  /* If this is a global collection then set the globalgc flag.  This will allow
   * us to trigger excessiveGC checks from a local collection that has recursed
   * into a global gc .
   */
  if (_globalCollector) {
...
/**
 * Check if we are in a state of excessive GC, and if so, take appropriate
 * action.
 ...
 */
bool MM_Collector::checkForExcessiveGC(MM_EnvironmentBase *env,
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_EXCESSIVEGC_CHECK_GC_ACTIVITY(
      extensions->privateHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_PRIVATE_EXCESSIVEGC_CHECK_GC_ACTIVITY,
      gcCount, stats->totalGCTime,
      omrtime_hires_delta(stats->lastEndGlobalGCTimeStamp,
                          stats->endGCTimeStamp,
                          OMRPORT_TIME_DELTA_IN_MICROSECONDS) -
          stats->totalGCTime,
      stats->newGCToUserTimeRatio, stats->avgGCToUserTimeRatio,
      (float)extensions->excessiveGCratio);

  /* we slide in this FVTest check here so that we can aggressively force
   * excessive GCs to occur */
  if (extensions->fvtest_forceExcessiveAllocFailureAfter > 0) {
    ...
    if (1 == failAfter) {
      ...
      TRIGGER_J9HOOK_MM_OMR_EXCESSIVEGC_RAISED(
          extensions->omrHookInterface, env->getOmrVMThread(),
  ...
  if (extensions->heap->getMemorySize() !=
      extensions->heap->getMaximumMemorySize()) {
    /* No..so don't raise excessive gc just yet */
    return false;
  }

  /* we may need to ignore the fact that we are over the threshold, in
   * transition cases, after OutOfMemoryException, to give some time for Java
   * program to stabilize
   */
  if (stats->avgGCToUserTimeRatio > extensions->excessiveGCratio) {
    /* Ignore the time ratio check unless the amount of heap freed is below a
     * particular ratio. This ends up including expanded memory as free, but
     * that's probably ok
     */
    float reclaimedPercent;
    UDATA heapFreeDelta;

    /* Determine the change in free memory from the GC - a negative value is
     * counted as zero */
    heapFreeDelta =
        (stats->freeMemorySizeBefore >= stats->freeMemorySizeAfter)
            ? 0
            : stats->freeMemorySizeAfter - stats->freeMemorySizeBefore;

    /* Calculate ratio of free space reclaimed this GC */
    reclaimedPercent = ((float)heapFreeDelta /
                        (float)extensions->heap->getActiveMemorySize()) *
                       100;

    TRIGGER_J9HOOK_MM_PRIVATE_EXCESSIVEGC_CHECK_FREE_SPACE(
        extensions->privateHookInterface, env->getOmrVMThread(),
        omrtime_hires_clock(), J9HOOK_MM_PRIVATE_EXCESSIVEGC_CHECK_FREE_SPACE,
    ...
    if (reclaimedPercent <= extensions->excessiveGCFreeSizeRatio * 100) {
      ...
      /* Raise excessive GC level, if at fatal lower level to allow another gc
       * to occur */
      ...
      TRIGGER_J9HOOK_MM_OMR_EXCESSIVEGC_RAISED(
          extensions->omrHookInterface, env->getOmrVMThread(),
          omrtime_hires_clock(), J9HOOK_MM_OMR_EXCESSIVEGC_RAISED, gcCount,
          reclaimedPercent, extensions->excessiveGCFreeSizeRatio * 100,
          extensions->excessiveGCLevel);
...
/**
 * Post collection broadcast event, indicating that the collection has been
 * completed.
 * @param subSpace the memory subspace where the collection occurred
 */
void MM_Collector::postCollect(MM_EnvironmentBase *env,
                               MM_MemorySubSpace *subSpace) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  ...
  if (!_isRecursiveGC) {
    ...
    /* We check gc time vs user time on intervals between the ends of global
     * GCs: ulg*ulululg* (g=global; l=local; u=user; we sum the gc times between
     * the *s and compare against the total time between the *s.) We need to
     * clear the accumulated gc times and track the end of this gc if a global
     * collection occured, in order to adhere to the interval above. This needs
     * to be done here to reset the stats when a system gc has occured.
     */
 ...
 *
 */
void *MM_Collector::garbageCollect(
    MM_EnvironmentBase *env, MM_MemorySubSpace *callingSubSpace,
    MM_AllocateDescription *allocateDescription, uint32_t gcCode,
    MM_ObjectAllocationInterface *objectAllocationInterface,
    MM_MemorySubSpace *baseSubSpace, MM_AllocationContext *context) {
  ...
  _gcCompleted =
      internalGarbageCollect(env, callingSubSpace, allocateDescription);
  ...
  if (NULL != allocateDescription) {
    MM_MemorySubSpace::AllocationType allocationType =
        allocateDescription->getAllocationType();
    ...
    if (NULL != context) {
      ...
      postCollectAllocationResult = baseSubSpace->lockedReplenishAndAllocate(
          env, context, objectAllocationInterface, allocateDescription,
          allocationType);
    } else if (NULL != baseSubSpace) {
      /* try allocating in this subspace. indicate that this thread just did a
       * GC and is ok to try a parent, if current subspace fails to satisfy */
 ...
 */
void MM_Collector::setThreadFailAllocFlag(MM_EnvironmentBase *env, bool flag) {
  OMR_VMThread *vmThread = NULL;

  GC_OMRVMThreadListIterator threadListIterator(env->getOmrVM());
  while ((vmThread = threadListIterator.nextOMRVMThread()) != NULL) {
    MM_EnvironmentBase::getEnvironment(vmThread)->_failAllocOnExcessiveGC =
        flag;
  }
}

bool MM_Collector::isTimeForGlobalGCKickoff() {
 ...
 * The abort consists of halting any activity related to garbage collection, and
 * resetting/releasing said work such that a resumption would be the equivalent
 * of a fresh start.  All hidden references to heap objects should be
 * flushed/released at this stage.
 */
void MM_Collector::abortCollection(MM_EnvironmentBase *env,
 ...
 */
bool MM_Collector::collectorStartup(MM_GCExtensionsBase *extensions) {
  Assert_MM_unreachable();
  return true;
}
 ...
 * threads.
 */
void MM_Collector::collectorShutdown(MM_GCExtensionsBase *extensions) {
  Assert_MM_unreachable();
}

bool MM_Collector::isMarked(void *objectPtr) {
  Assert_MM_unreachable();
  return false;
}
