 ...
 */
void MM_Collector::collectorExpanded(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, uintptr_t expandSize)
{
 ...
 */
bool MM_Collector::canCollectorExpand(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, uintptr_t expandSize)
{
    return false;
}
 ...
 */
bool MM_Collector::forceKickoff(
 ...
 */
void MM_Collector::payAllocationTax(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace,
 ...
 */
bool MM_Collector::replenishPoolForAllocate(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool, uintptr_t size)
{
    return false;
}
...
void MM_Collector::recordExcessiveStatsForGCEnd(MM_EnvironmentBase* env)
{
    ...
    if (stats->endGCTimeStamp > stats->lastEndGlobalGCTimeStamp) {
        ...
        stats->avgGCToUserTimeRatio = MM_Math::weightedAverage(
            stats->avgGCToUserTimeRatio, stats->newGCToUserTimeRatio, extensions->excessiveGCnewRatioWeight);
    }
}
 ...
 */
bool MM_Collector::checkForExcessiveGC(MM_EnvironmentBase* env, MM_Collector* collector)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    MM_ExcessiveGCStats* stats = &extensions->excessiveGCStats;
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_EXCESSIVEGC_CHECK_GC_ACTIVITY(extensions->privateHookInterface, env->getOmrVMThread(),
        ...
        omrtime_hires_delta(stats->lastEndGlobalGCTimeStamp, stats->endGCTimeStamp, OMRPORT_TIME_DELTA_IN_MICROSECONDS)
            - stats->totalGCTime,
    ...
    if (stats->avgGCToUserTimeRatio > extensions->excessiveGCratio) {
        ...
        TRIGGER_J9HOOK_MM_PRIVATE_EXCESSIVEGC_CHECK_FREE_SPACE(extensions->privateHookInterface, env->getOmrVMThread(),
            omrtime_hires_clock(), J9HOOK_MM_PRIVATE_EXCESSIVEGC_CHECK_FREE_SPACE, gcCount, stats->newGCToUserTimeRatio,
            stats->avgGCToUserTimeRatio, (float)extensions->excessiveGCratio, heapFreeDelta, reclaimedPercent,
 ...
 */
void MM_Collector::postCollect(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
 ...
 *
 */
void* MM_Collector::garbageCollect(MM_EnvironmentBase* env, MM_MemorySubSpace* callingSubSpace,
 ...
 */
void MM_Collector::abortCollection(MM_EnvironmentBase* env, CollectionAbortReason reason)
{
    Assert_MM_unreachable();
}
