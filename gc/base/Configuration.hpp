 ...
 * New configurations should derive from this and add themselves to the
 * configMap array.
 */
...
protected:
  ...
  const MM_AlignmentType _alignmentType; /**< the alignment type must be applied
                                            to memory settings */
  const uintptr_t _defaultRegionSize;    /**< default region size, in bytes */
  const uintptr_t
      _defaultArrayletLeafSize; /**< default arraylet leaf size, in bytes */
  const MM_GCWriteBarrierType
      _writeBarrierType; /**< write barrier to install for GC */
...
public:
  ...
  virtual MM_GlobalCollector *
  createGlobalCollector(MM_EnvironmentBase *env) = 0;
  MM_Heap *createHeap(MM_EnvironmentBase *env, uintptr_t heapBytesRequested);
  virtual MM_Heap *
  createHeapWithManager(MM_EnvironmentBase *env, uintptr_t heapBytesRequested,
                        MM_HeapRegionManager *regionManager) = 0;
  virtual MM_HeapRegionManager *
  createHeapRegionManager(MM_EnvironmentBase *env) = 0;
  virtual MM_MemorySpace *
  createDefaultMemorySpace(MM_EnvironmentBase *env, MM_Heap *heap,
                           MM_InitializationParameters *parameters) = 0;
  MM_EnvironmentBase *createEnvironment(MM_GCExtensionsBase *extensions,
  ...
  bool initializeHeapRegionDescriptor(MM_EnvironmentBase *env,
                                      MM_HeapRegionDescriptor *region) {
    return _delegate.initializeHeapRegionDescriptorExtension(env, region);
  }
  void teardownHeapRegionDescriptor(MM_EnvironmentBase *env,
                                    MM_HeapRegionDescriptor *region) {
  ...
  /**
   * Delegated method to determine when to start tracking heap fragmentation,
   * which should be inhibited until the heap has grown to a stable operational
   * size.
   */
  ...
  /**
   * Called once during startup to indicate that the default memory space has
   * been allocated. The configuration may use this opportunity to configure
   * values related to the heap address.
   *
   ...
   */
  virtual void defaultMemorySpaceAllocated(MM_GCExtensionsBase *extensions,
                                           void *defaultMemorySpace);

  virtual void kill(MM_EnvironmentBase *env);

  MM_Configuration(MM_EnvironmentBase *env, MM_GCPolicy gcPolicy,
                   MM_AlignmentType alignmentType, uintptr_t defaultRegionSize,
                   uintptr_t defaultArrayletLeafSize,
                   MM_GCWriteBarrierType writeBarrierType,
                   MM_GCAllocationType allocationType)
      : MM_BaseVirtual(), _delegate(gcPolicy), _alignmentType(alignmentType),
        _defaultRegionSize(defaultRegionSize),
        _defaultArrayletLeafSize(defaultArrayletLeafSize),
        _writeBarrierType(writeBarrierType), _allocationType(allocationType) {
...
protected:
  ...
  allocateNewEnvironment(MM_GCExtensionsBase *extensions,
                         OMR_VMThread *omrVMThread) = 0;
  ...
  /**
   * Initializes the NUMAManager.  All overriding implementations should call
   * this.
   *
   * @param env[in] - the current environment
   * @return whether NUMAMAnager was initialized or not.  False implies startup
   * failure.
   */
...
private:
  /**
   * Sets GC parameters that are dependent on the number of gc threads (if not
  ...
  uintptr_t getAlignment(MM_GCExtensionsBase *extensions,
  ...
  bool initializeRunTimeObjectAlignmentAndCRShift(MM_EnvironmentBase *env,
                                                  MM_Heap *heap);
