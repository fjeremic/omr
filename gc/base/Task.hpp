...
class MM_Task : public MM_BaseVirtual {
 ...
 public:
  ...
  /**
   * Single call setup routine for tasks invoked by the master thread before the
   * task is dispatched.
   */
  virtual void masterSetup(MM_EnvironmentBase* env);

  /**
   * Single call cleanup routine for tasks invoked by the master thread after
   * the task has been dispatched and all slave threads (if any) have been
   * quiesced.
   */
  virtual void masterCleanup(MM_EnvironmentBase* env);

  /**
   * @note This should not be called by anyone but Dispatcher or
   *ParallelDispatcher
   **/
  MMINLINE virtual void setThreadCount(uintptr_t threadCount) {
    assume0(1 == threadCount);
  }
  MMINLINE virtual uintptr_t getThreadCount() { return 1; }

  MMINLINE virtual void setSynchronizeMutex(
      omrthread_monitor_t synchronizeMutex) {
   ...
   * @note no-op
   */
  virtual void synchronizeGCThreads(MM_EnvironmentBase* env,
                                    const char* id,
                                    uint64_t* stallTime) {}
   ...
   * @note All tasks must implement this method - the IDs are defined in @ref
   * j9modron.h
   */
  ...
  /* Called within marking scheme to check if we should suspend the state of the
   * mark and return to the caller.
  ...
  MM_Task(MM_EnvironmentBase* env, MM_Dispatcher* dispatcher)
      : MM_BaseVirtual(), _dispatcher(dispatcher), _oldVMstate(0) {
