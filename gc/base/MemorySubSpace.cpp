...
void MM_MemorySubSpace::reportSystemGCStart(MM_EnvironmentBase* env, uint32_t gcCode)
{
    ...
    if (J9_EVENT_IS_HOOKED(_extensions->privateHookInterface, J9HOOK_MM_PRIVATE_SYSTEM_GC_START)) {
        ...
        ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_SYSTEM_GC_START(_extensions->privateHookInterface, env->getOmrVMThread(),
            omrtime_hires_clock(), J9HOOK_MM_PRIVATE_SYSTEM_GC_START, gcCode, &commonData);
    }
}
...
void MM_MemorySubSpace::reportSystemGCEnd(MM_EnvironmentBase* env)
{
    ...
    Trc_OMRMM_SystemGCEnd(env->getOmrVMThread(), _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_NEW),
        _extensions->heap->getActiveMemorySize(MEMORY_TYPE_NEW),
        _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_OLD),
        _extensions->heap->getActiveMemorySize(MEMORY_TYPE_OLD),
        (_extensions->largeObjectArea ? _extensions->heap->getApproximateActiveFreeLOAMemorySize(MEMORY_TYPE_OLD) : 0),
    ...
    if (J9_EVENT_IS_HOOKED(_extensions->privateHookInterface, J9HOOK_MM_PRIVATE_SYSTEM_GC_END)) {
        ...
        ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_SYSTEM_GC_END(_extensions->privateHookInterface, env->getOmrVMThread(),
            omrtime_hires_clock(), J9HOOK_MM_PRIVATE_SYSTEM_GC_END, env->getExclusiveAccessTime(), &commonData);
    }
}
 ...
 */
void MM_MemorySubSpace::reportHeapResizeAttempt(MM_EnvironmentBase* env, uintptr_t amount, uintptr_t type)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

    MM_HeapResizeStats* resizeStats = _extensions->heap->getResizeStats();

    uint64_t resizeTime = (type == HEAP_EXPAND) ? resizeStats->getLastExpandTime() : resizeStats->getLastContractTime();
 ...
 * Event reporting
 */
void MM_MemorySubSpace::reportAllocationFailureStart(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_MemorySubSpace::registerMemorySubSpace(MM_MemorySubSpace* memorySubSpace)
{
...
void MM_MemorySubSpace::unregisterMemorySubSpace(MM_MemorySubSpace* memorySubSpace)
{
...
MM_MemorySubSpace* MM_MemorySubSpace::getTopLevelMemorySubSpace(uintptr_t typeFlags)
{
    Assert_MM_true(typeFlags == (getTypeFlags() & typeFlags));
    MM_MemorySubSpace* topLevelSubSpace = this;

    while (
        topLevelSubSpace->getParent() && (typeFlags == (topLevelSubSpace->getParent()->getTypeFlags() & typeFlags))) {
 ...
 */
MM_MemoryPool* MM_MemorySubSpace::getMemoryPool(MM_EnvironmentBase* env, void* addrBase, void* addrTop, void*& highAddr)
{
    highAddr = NULL;
    return getMemoryPool();
}
...
uintptr_t MM_MemorySubSpace::getActiveLOAMemorySize(uintptr_t includeMemoryType)
{
    uintptr_t memory;
...
uintptr_t MM_MemorySubSpace::getActiveSurvivorMemorySize(uintptr_t includeMemoryType)
{
    uintptr_t memory;
 ...
 */
uintptr_t MM_MemorySubSpace::getActualActiveFreeMemorySize(uintptr_t includeMemoryType)
{
    uintptr_t freeMemory;
 ...
 * @return the total free memory currently available for allocation from subspaces of the specified type.
 */
uintptr_t MM_MemorySubSpace::getApproximateActiveFreeLOAMemorySize(uintptr_t includeMemoryType)
{
    uintptr_t freeMemory;
...
uintptr_t MM_MemorySubSpace::getApproximateActiveFreeSurvivorMemorySize(uintptr_t includeMemoryType)
{
    uintptr_t freeMemory;
 ...
 * @return the total free memory currently available for allocation from subspaces of the specified type.
 */
uintptr_t MM_MemorySubSpace::getApproximateActiveFreeMemorySize(uintptr_t includeMemoryType)
{
    uintptr_t freeMemory;
...
bool MM_MemorySubSpace::completeFreelistRebuildRequired(MM_EnvironmentBase* env)
{
...
void MM_MemorySubSpace::mergeHeapStats(MM_HeapStats* heapStats, uintptr_t includeMemoryType)
{
    /* Call on children */
    MM_MemorySubSpace* child;
    child = _children;
...
void MM_MemorySubSpace::systemGarbageCollect(MM_EnvironmentBase* env, uint32_t gcCode)
{
    if (_parent) {
    ...
    if (_collector && _usesGlobalCollector && !_collector->isDisabled(env)) {
        ...
        if ((J9MMCONSTANT_EXPLICIT_GC_IDLE_GC == gcCode) && (_extensions->gcOnIdle)) {
            ...
            TRIGGER_J9HOOK_MM_PRIVATE_HEAP_RESIZE(_extensions->privateHookInterface, env->getOmrVMThread(),
                omrtime_hires_clock(), J9HOOK_MM_PRIVATE_HEAP_RESIZE, HEAP_RELEASE_FREE_PAGES, getTypeFlags(),
...
bool MM_MemorySubSpace::garbageCollect(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, uint32_t gcCode)
{
...
void* MM_MemorySubSpace::allocateGeneric(MM_EnvironmentBase* env, MM_AllocateDescription* allocateDescription,
 ...
 */
void MM_MemorySubSpace::payAllocationTax(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
    ...
    MM_EnvironmentBase* env, MM_MemorySubSpace* baseSubSpace, MM_AllocateDescription* allocDescription)
{
    if (_extensions->payAllocationTax) {
        if (_parent) {
            ...
            Assert_MM_true(
                _usesGlobalCollector); /* If you don't have a parent, you really should be using the global */
            if (_usesGlobalCollector) {
 ...
 */
bool MM_MemorySubSpace::canExpand(MM_EnvironmentBase* env, uintptr_t expandSize)
{
 ...
 */
uintptr_t MM_MemorySubSpace::adjustExpansionWithinUserIncrement(MM_EnvironmentBase* env, uintptr_t expandSize)
{
    return expandSize;
}
 ...
 */
uintptr_t MM_MemorySubSpace::expand(MM_EnvironmentBase* env, uintptr_t expandSize)
{
 ...
 */
uintptr_t MM_MemorySubSpace::contract(MM_EnvironmentBase* env, uintptr_t contractSize)
{
    uint64_t timeStart, timeEnd;
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
 ...
 */
bool MM_MemorySubSpace::expanded(
 ...
 * Memory described by the range has added to the heap and been made available to the subspace as free memory.
 */
bool MM_MemorySubSpace::expanded(MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, uintptr_t size,
 ...
 */
void MM_MemorySubSpace::addExistingMemory(MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, uintptr_t size,
    void* lowAddress, void* highAddress, bool canCoalesce)
{
    /* Should never get here */
    Assert_MM_unreachable();
 ...
 * @warn Temporary routine that should not be used.
 */
void* MM_MemorySubSpace::removeExistingMemory(
 ...
 */
bool MM_MemorySubSpace::heapAddRange(
 ...
 *
 */
bool MM_MemorySubSpace::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size,
 ...
 */
bool MM_MemorySubSpace::canContract(MM_EnvironmentBase* env, uintptr_t contractSize)
{
 ...
 */
void* MM_MemorySubSpace::findFreeEntryEndingAtAddr(MM_EnvironmentBase* env, void* addr)
{
    Assert_MM_unreachable();
    return NULL;
}
 ...
 */
void* MM_MemorySubSpace::findFreeEntryTopStartingAtAddr(MM_EnvironmentBase* env, void* addr)
{
    Assert_MM_unreachable();
    return NULL;
}
 ...
 */
void* MM_MemorySubSpace::getNextFreeStartingAddr(MM_EnvironmentBase* env, void* currentFree)
{
    Assert_MM_unreachable();
    return NULL;
}
 ...
 *
 */
void MM_MemorySubSpace::moveHeap(MM_EnvironmentBase* env, void* srcBase, void* srcTop, void* dstBase)
{
    Assert_MM_unreachable();
}
#endif /* OMR_GC_MODRON_STANDARD */
 ...
 *
 */
void MM_MemorySubSpace::checkResize(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, bool _systemGC)
{}
 ...
 */
intptr_t MM_MemorySubSpace::performResize(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
    return 0;
}
 ...
 * @todo Provide class documentation
 */
void* MM_MemorySubSpace::collectorAllocateTLH(MM_EnvironmentBase* env, MM_Collector* requestCollector,
 ...
 */
void MM_MemorySubSpace::setAllocateAtSafePointOnly(MM_EnvironmentBase* env, bool safePoint)
{
    /* Call on children */
    MM_MemorySubSpace* child;
    child = _children;
...
void MM_MemorySubSpace::recycleRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptor* region)
{
 ...
 */
uintptr_t MM_MemorySubSpace::findLargestFreeEntry(MM_EnvironmentBase* env, MM_AllocateDescription* allocateDescription)
{
...
uintptr_t MM_MemorySubSpace::counterBalanceContract(
    MM_EnvironmentBase* env, uintptr_t contractSize, uintptr_t contractAlignment)
{
    if (NULL != _parent) {
 ...
 * @return the adjusted contract size that is allowed to the receiver.
 */
uintptr_t MM_MemorySubSpace::counterBalanceContract(MM_EnvironmentBase* env, MM_MemorySubSpace* previousSubSpace,
 ...
 */
uintptr_t MM_MemorySubSpace::counterBalanceContractWithExpand(MM_EnvironmentBase* env,
    MM_MemorySubSpace* previousSubSpace, MM_MemorySubSpace* contractSubSpace, uintptr_t contractSize,
    uintptr_t contractAlignment, uintptr_t expandSize)
{
    if (NULL != _physicalSubArena) {
        ...
        if (maximumExpandSize < adjustedExpandSize) {
            ...
            /* If the adjusted expand request has been reduced to 0, then we cannot expand.  Adjust the contraction size
             * to account for the lack of an expand, and return the value.  The expandSizeDelta can be increased through
             * rounding to be greater than the adjustedExpandSize (hence the >= check)
             */
            if (expandSizeDelta >= adjustedExpandSize) {
                adjustedContractSize
                    = (adjustedContractSize > adjustedExpandSize) ? adjustedContractSize - adjustedExpandSize : 0;
 ...
 */
void MM_MemorySubSpace::enqueueCounterBalanceExpand(
 ...
 * receiver.
 */
void MM_MemorySubSpace::triggerEnqueuedCounterBalancing(MM_EnvironmentBase* env)
{
 ...
 */
bool MM_MemorySubSpace::replenishPoolForAllocate(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool, uintptr_t size)
{
...
MM_HeapRegionDescriptor* MM_MemorySubSpace::getNextRegion(MM_HeapRegionDescriptor* region)
{
    return region->_nextRegionInSubSpace;
}

MM_HeapRegionDescriptor* MM_MemorySubSpace::selectRegionForContraction(MM_EnvironmentBase* env, uintptr_t numaNode)
{
    Assert_MM_unreachable();
    return NULL;
}
