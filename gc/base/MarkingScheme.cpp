...
MM_MarkingScheme *MM_MarkingScheme::newInstance(MM_EnvironmentBase *env) {
  ...
  markingScheme = (MM_MarkingScheme *)env->getForge()->allocate(
      sizeof(MM_MarkingScheme), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (markingScheme) {
...
bool MM_MarkingScheme::initialize(MM_EnvironmentBase *env) {
  _markMap = MM_MarkMap::newInstance(
      env, _extensions->heap->getMaximumPhysicalRange());
 ...
 */
bool MM_MarkingScheme::heapAddRange(MM_EnvironmentBase *env,
 ...
 * @return true if operation completes with success
 */
bool MM_MarkingScheme::heapRemoveRange(MM_EnvironmentBase *env,
  ...
  if (result) {
    _markMap->heapRemoveRange(env, size, lowAddress, highAddress,
...
/**
 * Set up the marking scheme for marking the heap in preparation for walking the
 * heap. This should be called in place of masterSetupForGC if no actual garbage
...
void MM_MarkingScheme::workerSetupForGC(MM_EnvironmentBase *env) {
...
void MM_MarkingScheme::workerCleanupAfterGC(MM_EnvironmentBase *env) {
 ...
 */
bool MM_MarkingScheme::isMarkedOutline(omrobjectptr_t objectPtr) {
  return isMarked(objectPtr);
}

bool MM_MarkingScheme::markObjectNoCheck(MM_EnvironmentBase *env,
                                         omrobjectptr_t objectPtr,
                                         bool leafType) {
  return inlineMarkObjectNoCheck(env, objectPtr, leafType);
}

bool MM_MarkingScheme::markObject(MM_EnvironmentBase *env,
                                  omrobjectptr_t objectPtr, bool leafType) {
 ...
 * 				 range
 * *************************************************************************/
uintptr_t MM_MarkingScheme::numMarkBitsInRange(MM_EnvironmentBase *env,
 ...
 *
 * parameters  - heapBase - base of region of heap whoose mark bits are to be
 * set heapTop  - top of region of heap whoose mark bits to be set clear
 * - if TRUE set BITS OFF; otherwise set bits ON
 *
 * return      - the amount of memory actually cleard
 * *************************************************************************/
uintptr_t MM_MarkingScheme::setMarkBitsInRange(MM_EnvironmentBase *env,
                                               void *heapBase, void *heapTop,
                                               bool clear) {
 ...
 */
uintptr_t MM_MarkingScheme::scanObject(MM_EnvironmentBase *env,
                                       omrobjectptr_t objectPtr) {
  ...
  GC_ObjectScanner *objectScanner = _delegate.getObjectScanner(
      env, objectPtr, &objectScannerState, SCAN_REASON_PACKET, &sizeToDo);
  if (NULL != objectScanner) {
    ...
    while (NULL != (slotObject = objectScanner->getNextSlot())) {
      ...
      inlineMarkObjectNoCheck(env, slotObject->readReferenceFromSlot(),
                              isLeafSlot);
    }
  }
  return sizeToDo;
 ...
 */
void MM_MarkingScheme::markLiveObjectsInit(MM_EnvironmentBase *env,
                                           bool initMarkMap) {
...
/**
 *  Mark Roots
 *  Create Root Scanner and Mark all roots including classes and classloaders if
 * dynamic class unloading is enabled
 *  @param[in] env - passed Environment
 *  @param[in] shouldScan instruct should scanning also be active while roots
 * are marked
 */
...
void MM_MarkingScheme::markLiveObjectsScan(MM_EnvironmentBase *env) {
  completeMarking(env);
}
...
#if defined(OMR_GC_REALTIME)
      MM_WorkPacketsSATB *workPacketsSATB =
          MM_WorkPacketsSATB::newInstance(env);
      _extensions->sATBBarrierRememberedSet =
          MM_RememberedSetSATB::newInstance(env, workPacketsSATB);
...
void MM_MarkingScheme::fixupForwardedSlotOutline(GC_SlotObject *slotObject) {
