...
{
  _markMap =
    MM_MarkMap::newInstance(env, _extensions->heap->getMaximumPhysicalRange());
...
bool
MM_MarkingScheme::markObjectNoCheck(MM_EnvironmentBase* env,
...
bool
MM_MarkingScheme::markObject(MM_EnvironmentBase* env,
                             omrobjectptr_t objectPtr,
                             bool leafType)
{
 ...
 * *************************************************************************/
uintptr_t
MM_MarkingScheme::numMarkBitsInRange(MM_EnvironmentBase* env,
                                     void* heapBase,
                                     void* heapTop)
{
 ...
 * *************************************************************************/
uintptr_t
MM_MarkingScheme::setMarkBitsInRange(MM_EnvironmentBase* env,
                                     void* heapBase,
                                     void* heapTop,
                                     bool clear)
{
...
MM_MarkingScheme::scanObject(MM_EnvironmentBase* env, omrobjectptr_t objectPtr)
{
  ...
  if (NULL != objectScanner) {
    ...
    while (NULL != (slotObject = objectScanner->getNextSlot())) {
      ...
      inlineMarkObjectNoCheck(
        env, slotObject->readReferenceFromSlot(), isLeafSlot);
    }
  }
  return sizeToDo;
}
