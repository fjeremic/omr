...
intptr_t MM_LightweightNonReentrantReaderWriterLock::initialize(
    uintptr_t spinCount) {
  ...
  if (J9THREAD_RWMUTEX_OK !=
      omrthread_rwmutex_init(
          &_rwmutex, 0,
          "MM_LightweightNonReentrantReaderWriterLock::_rwmutex")) {
...
intptr_t MM_LightweightNonReentrantReaderWriterLock::tearDown() {
  intptr_t ret = LWRW_OK;
#if !defined(J9MODRON_USE_CUSTOM_READERWRITERLOCK)
...
intptr_t MM_LightweightNonReentrantReaderWriterLock::enterRead() {
  intptr_t ret = LWRW_OK;
#if defined(J9MODRON_USE_CUSTOM_READERWRITERLOCK)
  uint32_t oldValue;
  uint32_t newValue;
  ...
  for (;;) {
    ...
    uint32_t retValue = MM_AtomicOperations::lockCompareExchangeU32(
        (volatile uint32_t*)&_status, oldValue, newValue);
    if (oldValue == retValue) {
      break;
    } else if ((LWRW_READER_MODE == (retValue & LWRW_READER_MODE)) &&
               0 == (retValue & LWRW_MASK_READERS)) {
...
intptr_t MM_LightweightNonReentrantReaderWriterLock::exitRead() {
  intptr_t ret = LWRW_OK;
#if defined(J9MODRON_USE_CUSTOM_READERWRITERLOCK)
  uint32_t oldValue;
  uint32_t newValue;
...
intptr_t MM_LightweightNonReentrantReaderWriterLock::enterWrite() {
  intptr_t ret = LWRW_OK;
#if defined(J9MODRON_USE_CUSTOM_READERWRITERLOCK)
  uint32_t oldValue;
  uint32_t newValue;
...
intptr_t MM_LightweightNonReentrantReaderWriterLock::exitWrite() {
  intptr_t ret = LWRW_OK;
#if defined(J9MODRON_USE_CUSTOM_READERWRITERLOCK)
  MM_AtomicOperations::writeBarrier();
