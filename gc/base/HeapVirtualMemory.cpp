...
#define ONE_GB ((uintptr_t)1 * 1024 * 1024 * 1024)
  ...
  extensions->overflowSafeAllocSize =
    ((HIGH_ADDRESS - (uintptr_t)(memoryManager->getHeapTop(&_vmemHandle))) + 1);
...
void*
MM_HeapVirtualMemory::doubleMapArraylet(
  ...
  UDATA pageSize)
{
  MM_MemoryManager* memoryManager =
    MM_GCExtensionsBase::getExtensions(_omrVM)->memoryManager;
  return memoryManager->doubleMapArraylet(&_vmemHandle,
                                          env,
                                          arrayletLeaves,
                                          arrayletLeafCount,
                                          arrayletLeafSize,
 ...
 */
void
MM_HeapVirtualMemory::detachArena(MM_EnvironmentBase* env,
 ...
 */
bool
MM_HeapVirtualMemory::attachArena(MM_EnvironmentBase* env,
                                  MM_PhysicalArena* arena,
                                  uintptr_t size)
{
 ...
 */
bool
MM_HeapVirtualMemory::decommitMemory(void* address,
                                     uintptr_t size,
                                     ...
                                     void* highValidAddress)
{
  MM_GCExtensionsBase* extensions = MM_GCExtensionsBase::getExtensions(_omrVM);
  MM_MemoryManager* memoryManager = extensions->memoryManager;
  return memoryManager->decommitMemory(
                                   ...
                                   void* highAddress)
{
  ...
  env->getExtensions()->identityHashDataAddRange(
    env, subspace, size, lowAddress, highAddress);
                                      ...
                                      void* highValidAddress)
{
  ...
  env->getExtensions()->identityHashDataRemoveRange(
    env, subspace, size, lowAddress, highAddress);
...
bool
MM_HeapVirtualMemory::initializeHeapRegionManager(MM_EnvironmentBase* env,
