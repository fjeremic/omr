    ...
    MM_EnvironmentBase* env, uintptr_t heapAlignment, uintptr_t size, MM_HeapRegionManager* regionManager)
{
    ...
    heap = (MM_HeapVirtualMemory*)env->getForge()->allocate(
        sizeof(MM_HeapVirtualMemory), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (heap) {
...
bool MM_HeapVirtualMemory::initialize(MM_EnvironmentBase* env, uintptr_t size)
{
    ...
    if (extensions->padToPageSize) {
        ...
        if (created) {
            /* overflow protection must be there to play role of padding even top is not so close to the end of the
             * memory */
...
#define ONE_GB ((uintptr_t)1 * 1024 * 1024 * 1024)
    ...
    if (!created
        && !memoryManager->createVirtualMemoryForHeap(env, &_vmemHandle, effectiveHeapAlignment, size, padding,
            (void*)(extensions->preferredHeapBase), (void*)(extensions->heapCeiling))) {
        return false;
    }
...
void* MM_HeapVirtualMemory::doubleMapArraylet(MM_EnvironmentBase* env, void* arrayletLeaves[], UDATA arrayletLeafCount,
    UDATA arrayletLeafSize, UDATA byteAmount, struct J9PortVmemIdentifier* newIdentifier, UDATA pageSize)
{
    MM_MemoryManager* memoryManager = MM_GCExtensionsBase::getExtensions(_omrVM)->memoryManager;
    return memoryManager->doubleMapArraylet(
        &_vmemHandle, env, arrayletLeaves, arrayletLeafCount, arrayletLeafSize, byteAmount, newIdentifier, pageSize);
}
 ...
 */
void MM_HeapVirtualMemory::detachArena(MM_EnvironmentBase* env, MM_PhysicalArena* arena)
{
 ...
 */
bool MM_HeapVirtualMemory::attachArena(MM_EnvironmentBase* env, MM_PhysicalArena* arena, uintptr_t size)
{
 ...
 * @note This is a bit of a strange function to have as public API.  Should it be removed?
 */
bool MM_HeapVirtualMemory::decommitMemory(void* address, uintptr_t size, void* lowValidAddress, void* highValidAddress)
{
    MM_GCExtensionsBase* extensions = MM_GCExtensionsBase::getExtensions(_omrVM);
    MM_MemoryManager* memoryManager = extensions->memoryManager;
...
bool MM_HeapVirtualMemory::heapAddRange(
 ...
 *
 */
bool MM_HeapVirtualMemory::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size,
...
bool MM_HeapVirtualMemory::initializeHeapRegionManager(MM_EnvironmentBase* env, MM_HeapRegionManager* manager)
{
    bool result = false;
