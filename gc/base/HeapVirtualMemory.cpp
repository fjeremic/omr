 ...
 *
 * @note The actual heap size might be smaller than the requested size, in order
 * to satisfy alignment requirements. Use getMaximumSize() to get the actual
 * allocation size.
 */
MM_HeapVirtualMemory* MM_HeapVirtualMemory::newInstance(
    MM_EnvironmentBase* env, uintptr_t heapAlignment, uintptr_t size,
    MM_HeapRegionManager* regionManager) {
  MM_HeapVirtualMemory* heap;

  heap = (MM_HeapVirtualMemory*)env->getForge()->allocate(
      sizeof(MM_HeapVirtualMemory), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (heap) {
...
bool MM_HeapVirtualMemory::initialize(MM_EnvironmentBase* env, uintptr_t size) {
  ...
  /* we need to ensure that we allocate the heap with region alignment since the
   * region table requires that */
  MM_HeapRegionManager* manager = getHeapRegionManager();
  effectiveHeapAlignment =
      MM_Math::roundToCeiling(manager->getRegionSize(), effectiveHeapAlignment);
  ...
  if (extensions->padToPageSize) {
     ...
     * In this case extra padding is not required because of overflow protection
     * padding can be used instead
     */
    uintptr_t effectiveSize =
        MM_Math::roundToCeiling(manager->getRegionSize(), size);
    void* preferredHeapBase = (void*)((uintptr_t)0 - effectiveSize);

    created = memoryManager->createVirtualMemoryForHeap(
        env, &_vmemHandle, effectiveHeapAlignment, size, padding,
        preferredHeapBase, (void*)(extensions->heapCeiling));
    if (created) {
      /* overflow protection must be there to play role of padding even top is
       * not so close to the end of the memory */
      forcedOverflowProtection = true;
    } else
#endif /* (defined(AIXPPC) && !defined(PPC64)) */
    {
      /* Ignore extra full page padding if page size is too large (hard coded
       * here for 1G or larger) */
#define ONE_GB ((uintptr_t)1 * 1024 * 1024 * 1024)
  ...
  if (!created && !memoryManager->createVirtualMemoryForHeap(
                      env, &_vmemHandle, effectiveHeapAlignment, size, padding,
  ...
  if (forcedOverflowProtection ||
      (HIGH_ADDRESS - ((uintptr_t)memoryManager->getHeapTop(&_vmemHandle)) <
       (OVERFLOW_ROUNDING)) ||
  ...
  extensions->overflowSafeAllocSize =
      ((HIGH_ADDRESS - (uintptr_t)(memoryManager->getHeapTop(&_vmemHandle))) +
       1);
...
void MM_HeapVirtualMemory::tearDown(MM_EnvironmentBase* env) {
...
void MM_HeapVirtualMemory::kill(MM_EnvironmentBase* env) {
  tearDown(env);
 ...
 */
void* MM_HeapVirtualMemory::getHeapBase() {
  MM_MemoryManager* memoryManager =
      MM_GCExtensionsBase::getExtensions(_omrVM)->memoryManager;
 ...
 */
void* MM_HeapVirtualMemory::getHeapTop() {
  MM_MemoryManager* memoryManager =
      MM_GCExtensionsBase::getExtensions(_omrVM)->memoryManager;
...
void* MM_HeapVirtualMemory::doubleMapArraylet(
    MM_EnvironmentBase* env, void* arrayletLeaves[], UDATA arrayletLeafCount,
    UDATA arrayletLeafSize, UDATA byteAmount,
    struct J9PortVmemIdentifier* newIdentifier, UDATA pageSize) {
  MM_MemoryManager* memoryManager =
      MM_GCExtensionsBase::getExtensions(_omrVM)->memoryManager;
  return memoryManager->doubleMapArraylet(&_vmemHandle, env, arrayletLeaves,
                                          arrayletLeafCount, arrayletLeafSize,
                                          byteAmount, newIdentifier, pageSize);
}
#endif /* defined(OMR_GC_DOUBLE_MAP_ARRAYLETS) */

uintptr_t MM_HeapVirtualMemory::getPageSize() {
  MM_MemoryManager* memoryManager =
      MM_GCExtensionsBase::getExtensions(_omrVM)->memoryManager;
  return memoryManager->getPageSize(&_vmemHandle);
}

uintptr_t MM_HeapVirtualMemory::getPageFlags() {
  MM_MemoryManager* memoryManager =
      MM_GCExtensionsBase::getExtensions(_omrVM)->memoryManager;
 ...
 * The value returned represents the difference between the lowest and highest
 * possible address range the heap can ever occupy.  This value includes any
 * memory that may never be used by the heap (e.g., in a segmented heap
 * scenario).
 * @return Maximum size that the heap will ever span.
 */
uintptr_t MM_HeapVirtualMemory::getMaximumPhysicalRange() {
  MM_MemoryManager* memoryManager =
      MM_GCExtensionsBase::getExtensions(_omrVM)->memoryManager;
 ...
 */
void MM_HeapVirtualMemory::detachArena(MM_EnvironmentBase* env,
                                       MM_PhysicalArena* arena) {
 ...
 * This reserves the address space within the receiver for the arena, and
 * connects the arena to the list of those associated to the receiver (in
 * address order).
 *
 * @return true if the arena was attached successfully, false otherwise.
 * @note The memory reseved is not commited.
 */
bool MM_HeapVirtualMemory::attachArena(MM_EnvironmentBase* env,
                                       MM_PhysicalArena* arena,
                                       uintptr_t size) {
  ...
  while (insertionTail) {
    if ((((uintptr_t)insertionTail->getLowAddress()) -
         ((uintptr_t)candidateBase)) >= size) {
      break;
    }
  ...
  /* If we have reached the end of the currentArena list, check if there is room
   * between the candidateBase and the end of virtual memory */
  if (!insertionTail) {
    if ((memoryManager->calculateOffsetToHeapTop(&_vmemHandle, candidateBase)) <
        size) {
      return false;
    }
  }
 ...
 * @note This is a bit of a strange function to have as public API.  Should it
 * be removed?
 */
bool MM_HeapVirtualMemory::commitMemory(void* address, uintptr_t size) {
 ...
 * be removed?
 */
bool MM_HeapVirtualMemory::decommitMemory(void* address, uintptr_t size,
                                          void* lowValidAddress,
                                          void* highValidAddress) {
  MM_GCExtensionsBase* extensions = MM_GCExtensionsBase::getExtensions(_omrVM);
  MM_MemoryManager* memoryManager = extensions->memoryManager;
  return memoryManager->decommitMemory(&_vmemHandle, address, size,
 ...
 */
uintptr_t MM_HeapVirtualMemory::calculateOffsetFromHeapBase(void* address) {
  MM_GCExtensionsBase* extensions = MM_GCExtensionsBase::getExtensions(_omrVM);
  MM_MemoryManager* memoryManager = extensions->memoryManager;
...
bool MM_HeapVirtualMemory::heapAddRange(MM_EnvironmentBase* env,
  ...
  env->getExtensions()->identityHashDataAddRange(env, subspace, size,
 ...
 *
 */
bool MM_HeapVirtualMemory::heapRemoveRange(MM_EnvironmentBase* env,
                                           MM_MemorySubSpace* subspace,
                                           uintptr_t size, void* lowAddress,
                                           void* highAddress,
                                           void* lowValidAddress,
  ...
  env->getExtensions()->identityHashDataRemoveRange(env, subspace, size,
                                                    lowAddress, highAddress);
...
bool MM_HeapVirtualMemory::initializeHeapRegionManager(
    MM_EnvironmentBase* env, MM_HeapRegionManager* manager) {
  bool result = false;

  /* since this kind of heap is backed by contiguous memory, tell the heap
   * region manager (which was just initialized by super) that we want to enable
   * this range of regions for later use.
   */
  MM_MemoryManager* memoryManager =
      MM_GCExtensionsBase::getExtensions(_omrVM)->memoryManager;
