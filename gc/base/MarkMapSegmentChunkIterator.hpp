...
/**
 * Iterate over chunks of an area of memory by splitting the extent into even
 * size chunks, then using the mark map to find the first object in each chunk.
...
public:
  ...
  GC_MarkMapSegmentChunkIterator(MM_GCExtensionsBase *extensions,
                                 ...
                                 UDATA chunkSize)
      : _extensions(extensions), _chunkSize(chunkSize),
        _segmentBytesRemaining((UDATA)highAddress - (UDATA)lowAddress),
        _markedObjectIterator(extensions),
        _nextChunkBase((UDATA *)lowAddress){};

  /**
   * @note Any chunk returned from this method must have either an object or a
   * free header beginning at the first slot in the chunk.
   *
   * @param markMap[in] The mark map to use when finding the next chunk
   * @param base (OUT parameter) a pointer to the base of the next chunk will be
   * stored into this address
   * @param top (OUT parameter) a pointer to the top of the next chunk will be
   * stored into this address
