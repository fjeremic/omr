...
MM_EnvironmentBase *
MM_EnvironmentBase::newInstance(MM_GCExtensionsBase *extensions,
...
bool MM_EnvironmentBase::initialize(MM_GCExtensionsBase *extensions) {
  setEnvironmentId(
      MM_AtomicOperations::add(&extensions->currentEnvironmentCount, 1) - 1);
  setAllocationColor(extensions->newThreadAllocationColor);

  if (extensions->isStandardGC()) {
    /* pass veryLargeObjectThreshold = 0 to initialize limited size of
     * veryLargeEntryPool for thread (to reduce footprint), but if the threshold
     * is bigger than maxHeap size, we would pass orignal threshold to indicate
     * no veryLargeEntryPool needed
     */
    ...
    if (!_freeEntrySizeClassStats.initialize(
            this, extensions->largeObjectAllocationProfilingTopK,
  ...
  if (extensions->isSegregatedHeap()) {
    _regionWorkList = MM_RegionPoolSegregated::allocateHeapRegionQueue(
...
void MM_EnvironmentBase::tearDown(MM_GCExtensionsBase *extensions) {
#if defined(OMR_GC_SEGREGATED_HEAP)
  ...
  if (getOmrVMThread() == extensions->vmThreadAllocatedMost) {
    /* we lose information about thread that allocated most, but at least the
     * user of this value will not crash */
 ...
 */
uintptr_t MM_EnvironmentBase::pushVMstate(uintptr_t newState) {
 ...
 */
void MM_EnvironmentBase::popVMstate(uintptr_t newState) {
  _omrVMThread->vmState = newState;
}

void MM_EnvironmentBase::reportExclusiveAccessAcquire() {
  OMRPORT_ACCESS_FROM_OMRPORT(_portLibrary);

  /* record statistics */
  U_64 meanResponseTime = _omrVM->exclusiveVMAccessStats.totalResponseTime /
                          (_omrVM->exclusiveVMAccessStats.haltedThreads +
                           1); /* +1 for the requester */
  _exclusiveAccessTime = _omrVM->exclusiveVMAccessStats.endTime -
                         _omrVM->exclusiveVMAccessStats.startTime;
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_EXCLUSIVE_ACCESS(
      this->getExtensions()->privateHookInterface, _omrVMThread);
  /* now the new trigger */
  TRIGGER_J9HOOK_MM_PRIVATE_EXCLUSIVE_ACCESS_ACQUIRE(
      this->getExtensions()->privateHookInterface, _omrVMThread,
      omrtime_hires_clock(), J9HOOK_MM_PRIVATE_EXCLUSIVE_ACCESS_ACQUIRE,
      _exclusiveAccessTime, _meanExclusiveAccessIdleTime,
      _lastExclusiveAccessResponder, _exclusiveAccessHaltedThreads);
}

void MM_EnvironmentBase::reportExclusiveAccessRelease() {
  OMRPORT_ACCESS_FROM_OMRPORT(_portLibrary);
  TRIGGER_J9HOOK_MM_PRIVATE_EXCLUSIVE_ACCESS_RELEASE(
      this->getExtensions()->privateHookInterface, _omrVMThread,
      omrtime_hires_clock(), J9HOOK_MM_PRIVATE_EXCLUSIVE_ACCESS_RELEASE);
}

void MM_EnvironmentBase::allocationFailureStartReportIfRequired(
  ...
  if (!_allocationFailureReported) {
    ...
    Trc_MM_AllocationFailureStart(
        ...
        (extensions->largeObjectArea
             ? extensions->heap->getApproximateActiveFreeLOAMemorySize(
                   MEMORY_TYPE_OLD)
             : 0),
        (extensions->largeObjectArea
             ? extensions->heap->getActiveLOAMemorySize(MEMORY_TYPE_OLD)
             : 0),
    ...
    if (J9_EVENT_IS_HOOKED(extensions->privateHookInterface,
      ...
      ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_ALLOCATION_FAILURE_START(
          extensions->privateHookInterface, getOmrVMThread(),
          omrtime_hires_clock(), J9HOOK_MM_PRIVATE_ALLOCATION_FAILURE_START,
          allocDescription->getBytesRequested(), &commonData, flags,
...
void MM_EnvironmentBase::allocationFailureEndReportIfRequired(
  ...
  if (_allocationFailureReported) {
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_FAILED_ALLOCATION_COMPLETED(
        extensions->privateHookInterface, getOmrVMThread(),
        omrtime_hires_clock(), J9HOOK_MM_PRIVATE_FAILED_ALLOCATION_COMPLETED,
    ...
    if (J9_EVENT_IS_HOOKED(extensions->privateHookInterface,
      ...
      ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_ALLOCATION_FAILURE_END(
          extensions->privateHookInterface, getOmrVMThread(),
          omrtime_hires_clock(), J9HOOK_MM_PRIVATE_ALLOCATION_FAILURE_END,
          getExclusiveAccessTime(), &commonData, allocDescription);
    }
...
void MM_EnvironmentBase::acquireVMAccess() { _delegate.acquireVMAccess(); }

void MM_EnvironmentBase::releaseVMAccess() { _delegate.releaseVMAccess(); }

void MM_EnvironmentBase::acquireExclusiveVMAccess() {
...
void MM_EnvironmentBase::releaseExclusiveVMAccess() {
  _exclusiveCount -= 1;
  if (0 == _exclusiveCount) {
...
bool MM_EnvironmentBase::isExclusiveAccessRequestWaiting() {
  return _delegate.isExclusiveAccessRequestWaiting();
}

bool MM_EnvironmentBase::acquireExclusiveVMAccessForGC(MM_Collector *collector,
                                                       bool failIfNotFirst,
                                                       bool flushCaches) {
  MM_GCExtensionsBase *extensions = getExtensions();
  ...
  if (_omrVMThread->exclusiveCount > 0) {
    /* Did the current thread get exclusive vm access via (or already come
     * through this code) the GC exclusive vm access APIs.
     */
    if (_omrVMThread != extensions->gcExclusiveAccessThreadId) {
      /* The current thread did not get exclusive vm access via the GC exclusive
       * vm access API */
      /* If another thread has started to get exclusive vm access for a GC cache
       * that value so it can be restored later. It is possible for a thread to
       * win setting itself as the thread requesting exclusive vm access for a
       * GC but not actually be the next thread to perform a GC.  It can happen
       * if a system gc is requested while holding exclusive.
       */
      _cachedGCExclusiveAccessThreadId =
          (OMR_VMThread *)extensions->gcExclusiveAccessThreadId;
      /* Current thread has exclusive VM access so there is no reason to grab
       * the mutex! */
  ...
  _exclusiveAccessBeatenByOtherThread =
      !(collector->getExclusiveAccessCount() == collectorAccessCount);
...
void MM_EnvironmentBase::releaseExclusiveVMAccessForGC() {
  MM_GCExtensionsBase *extensions = MM_GCExtensionsBase::getExtensions(_omrVM);
...
uintptr_t MM_EnvironmentBase::relinquishExclusiveVMAccess() {
  return _delegate.relinquishExclusiveVMAccess();
}

void MM_EnvironmentBase::assumeExclusiveVMAccess(uintptr_t exclusiveCount) {
  _delegate.assumeExclusiveVMAccess(exclusiveCount);
}

MM_MemorySubSpace *MM_EnvironmentBase::getDefaultMemorySubSpace() {
  return getMemorySpace()->getDefaultMemorySubSpace();
}

MM_MemorySubSpace *MM_EnvironmentBase::getTenureMemorySubSpace() {
  return getMemorySpace()->getTenureMemorySubSpace();
}

bool MM_EnvironmentBase::saveObjects(omrobjectptr_t objectPtr) {
  ...
  if (NULL == _omrVMThread->_savedObject1) {
    ...
    Assert_MM_true((heapBase <= _omrVMThread->_savedObject1) &&
                   (heapTop > _omrVMThread->_savedObject1));
  }
  ...
  if (NULL == _omrVMThread->_savedObject2) {
    ...
    Assert_MM_true((heapBase <= _omrVMThread->_savedObject2) &&
                   (heapTop > _omrVMThread->_savedObject2));
  }
...
void MM_EnvironmentBase::restoreObjects(omrobjectptr_t *objectPtrIndirect) {
  void *heapBase = getExtensions()->heap->getHeapBase();
  void *heapTop = getExtensions()->heap->getHeapTop();
