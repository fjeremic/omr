...
protected:
  ...
  enum {
    slave_status_inactive =
        0, /* Must be 0 - set at initialization time by memset */
  ...
  omrthread_monitor_t
      _dispatcherMonitor; /**< Provides signalling between threads for startup
                             and shutting down as well as the thread that
                             initiated the shutdown */

  /* The synchronize mutex should eventually be a table of mutexes that are
   * distributed to each */
  /* Task as they are dispatched.  For now, since there is only one task active
   * at any time, a */
  ...
  uintptr_t _threadCountMaximum; /**< maximum threadcount - this is the size of
                                    the thread tables etc */
  uintptr_t _threadCount;        /**< number of threads currently forked */
  uintptr_t
      _activeThreadCount; /**< number of threads actively running a task */
...
protected:
  ...
  /**
   * Decides whether the dispatcher also start a separate thread to be the
   * master GC thread. Usually no, because the master thread will be the thread
   * that requested the GC.
   */
...
public:
  ...
  MMINLINE virtual uintptr_t threadCountMaximum() {
    return _threadCountMaximum;
  }
  ...
  static MM_ParallelDispatcher *newInstance(MM_EnvironmentBase *env,
  ...
  MM_ParallelDispatcher(MM_EnvironmentBase *env, omrsig_handler_fn handler,
                        void *handler_arg, uintptr_t defaultOSStackSize)
      : MM_Dispatcher(env),
        _extensions(MM_GCExtensionsBase::getExtensions(env->getOmrVM())),
        _threadShutdownCount(0), _threadTable(NULL), _statusTable(NULL),
        _taskTable(NULL), _slaveThreadMutex(NULL), _dispatcherMonitor(NULL),
        _synchronizeMutex(NULL), _slaveThreadsReservedForGC(false),
        _inShutdown(false), _threadCountMaximum(1), _threadCount(1),
        _activeThreadCount(1), _handler(handler), _handler_arg(handler_arg),
