...
#include "omrcomp.h"
/* #include "ModronAssertions.h" -- removed for now because it causes a compile
 * error in TraceOutput.cpp on xlC */
...
class MM_HeapLinkedFreeHeader {
 ...
 private:
  /*
   * CMVC 130331
   * The following private member functions must be declared before the public
   * functions due to a bug in an old version of GCC used on the HardHat Linux
   * platforms. These functions can't be in-lined if they're not declared before
 ...
 private:
  ...
  MMINLINE uintptr_t getNextImpl(bool compressed) {
    ...
    if (compressed) {
       ...
       * endian-flipped, so flip it back. See
       * MM_HeapLinkedFreeHeader::setNextImpl.
       */
 ...
 public:
   ...
   */
  static MMINLINE MM_HeapLinkedFreeHeader* getHeapLinkedFreeHeader(
      void* pointer) {
  ...
  MMINLINE MM_HeapLinkedFreeHeader* getNext(bool compressed) {
     ...
     */
    return (MM_HeapLinkedFreeHeader*)((getNextImpl(compressed)) &
                                      ~((uintptr_t)J9_GC_OBJ_HEAP_HOLE_MASK));
  }

  /**
   * Set the next free header in the linked list and mark the receiver as a
   * multi-slot hole Set to NULL to terminate the list.
   */
  MMINLINE void setNext(MM_HeapLinkedFreeHeader* freeEntryPtr,
                        bool compressed) {
  ...
  /**
   * Set the next free header in the linked list, preserving the type of the
   * receiver Set to NULL to terminate the list.
   */
  MMINLINE void updateNext(MM_HeapLinkedFreeHeader* freeEntryPtr,
                           bool compressed) {
    setNextImpl(
        ((uintptr_t)freeEntryPtr) |
            (getNextImpl(compressed) & (uintptr_t)J9_GC_OBJ_HEAP_HOLE_MASK),
        compressed);
  }
   ...
   * @param[in] freeEntrySize the number of bytes to be consumed (must be a
   * multiple of sizeof(uintptr_t))
   * @return The header written or null if the space was too small and was
   * filled with single-slot holes
   */
  MMINLINE static MM_HeapLinkedFreeHeader* fillWithHoles(
      void* addrBase, uintptr_t freeEntrySize, bool compressed) {
    ...
    if (freeEntrySize < sizeof(MM_HeapLinkedFreeHeader)) {
      ...
      while (0 != freeEntrySize) {
        ((MM_HeapLinkedFreeHeader*)addrBase)
            ->setNextImpl(J9_GC_SINGLE_SLOT_HOLE, compressed);
       ...
       * (note that this is not correct for other allocation schemes) */
   ...
   * @param[in] nextHead Pointer to the new head of list, with nextHead->_next
   * pointing to previous value of currentHead on return
   * @note Caller must set nextHead->_size
   */
  MMINLINE static void linkInAsHead(volatile uintptr_t* currentHead,
                                    MM_HeapLinkedFreeHeader* nextHead,
                                    bool compressed) {
    uintptr_t oldValue, newValue;
    do {
      oldValue = *currentHead;
      newValue = MM_AtomicOperations::lockCompareExchange(currentHead, oldValue,
                                                          (uintptr_t)nextHead);
