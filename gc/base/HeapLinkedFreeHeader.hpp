...
public:
    ...
    {
        setNextImpl(
            ((uintptr_t)freeEntryPtr) | (getNextImpl(compressed) & (uintptr_t)J9_GC_OBJ_HEAP_HOLE_MASK), compressed);
    }
    ...
    MMINLINE static MM_HeapLinkedFreeHeader* fillWithHoles(void* addrBase, uintptr_t freeEntrySize, bool compressed)
    {
        ...
        if (freeEntrySize < sizeof(MM_HeapLinkedFreeHeader)) {
            ...
            /* this is too big to use single slot holes so generate an AOL-style hole (note that this is not correct for
