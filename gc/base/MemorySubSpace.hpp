...
typedef enum { MODRON_COUNTER_BALANCE_TYPE_NONE = 1, MODRON_COUNTER_BALANCE_TYPE_EXPAND } ModronCounterBalanceType;
...
public:
    ...
    /* Subclass may or may not implement this. MemorySubSpace that have children may implement this where merged stats
     * will be reported. */
    virtual MM_LargeObjectAllocateStats* getLargeObjectAllocateStats();

    virtual void resetLargestFreeEntry();
    /**
     * Called by the by the collector when a region is determined to be unused to allow the receiver to recycle the
     * given region which it is responsible for managing.
    ...
    /**
     * Generic allocation failure handler.  This will attempt to acquire exclusive VM access and perform a garbage
     * collection to reclaim resources.  The given "allocationType" variable is consulted to determine which allocation
     * routine is to be called, after the collection, but before giving up exclusive to ensure that the failing
     * allocation succeeds before other allocations can be attempted.
     *
     * @param[in] env The current thread
     * @param[in] allocateDescription The allocation request which triggered the failure
     * @param[in] objectAllocationInterface The allocation interface which attempted to satisfy the allocate (only
     * relevant for TLH allocation failures, NULL if not a TLH allocation failure)
    ...
    virtual void* allocationRequestFailed(MM_EnvironmentBase* env, MM_AllocateDescription* allocateDescription,
        ...
        = 0;
    /**
     * A generic helper which is used by most implementations of allocationRequestFailed to issue the actual allocation
     * request between GC attempt stages.  It decodes the allocationType to determine which actual allocation call to
     * issue (Object, Leaf, or TLH).
     *
     * @param[in] env The current thread
     ...
     * @param[in] objectAllocationInterface The allocation interface through which the allocate request was issued (only
     * relevant if the allocationType is TLH, otherwise this can be NULL)
     * @param[in] attemptSubspace The subspace in which to issue the allocation call
     *
     * @return The address of the allocation or NULL on failure (in the case of Object or Leaf allocation, this is the
     * address of the allocate.  In the case of TLH, this is the base address of the TLH)
     */
    void* allocateGeneric(MM_EnvironmentBase* env, MM_AllocateDescription* allocateDescription,
        AllocationType allocationType, MM_ObjectAllocationInterface* objectAllocationInterface,
    ...
    /**
     * Called under exclusive to request that the subspace attempt to replenish the given context and satisfy the given
     * allocateDescription of the given allocationType
     *
     * @param[in] env The current thread
     * @param[in] context The allocation context which the sender failed to replenish
     * @param[in] objectAllocationInterface The alocation interface through which the original allocation call was
     ...
     */
    virtual void* lockedReplenishAndAllocate(MM_EnvironmentBase* env, MM_AllocationContext* context,
        MM_ObjectAllocationInterface* objectAllocationInterface, MM_AllocateDescription* allocateDescription,
    ...
    virtual uintptr_t collectorExpand(
        MM_EnvironmentBase* env, MM_Collector* requestCollector, MM_AllocateDescription* allocDescription);
    ...
    /**
     * Find top level parent MemorySubSpace that matches provided typeFlags. Assumption is that this memory space
     * already matches the flags.
     */
    ...
    virtual bool percolateGarbageCollect(
    ...
    virtual uintptr_t counterBalanceContract(MM_EnvironmentBase* env, MM_MemorySubSpace* previousSubSpace,
        MM_MemorySubSpace* contractSubSpace, uintptr_t contractSize, uintptr_t contractAlignment);
    virtual uintptr_t counterBalanceContractWithExpand(MM_EnvironmentBase* env, MM_MemorySubSpace* previousSubSpace,
    ...
    virtual void* removeExistingMemory(
     ...
     */
    MM_MemorySubSpace(MM_EnvironmentBase* env, MM_Collector* collector, MM_PhysicalSubArena* physicalSubArena,
