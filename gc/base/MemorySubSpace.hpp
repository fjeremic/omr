...
private:
  ...
  MM_LightweightNonReentrantLock
      _lock; /**< Lock used to ensure region list is maintained safely */
  ...
  /* TODO: RTJ Merge: This flag appears to only be used in MemorySubSpaceFlat --
   * should it move there? */
...
private:
protected:
  void generateAllocationFailureStats(MM_EnvironmentBase *env,
...
public:
  ...
  void reportHeapResizeAttempt(MM_EnvironmentBase *env, uintptr_t amount,
                               uintptr_t type);
  ...
  /**
   * The type of allocation we were trying to perform when calling
   * allocationRequestFailed.  This is used to determine what call-out function
   * to use to restart the allocation after we have dealt with the failure
   */
  enum AllocationType {
  ...
  MMINLINE MM_PhysicalSubArena *getPhysicalSubArena() const {
    return _physicalSubArena;
  }

  virtual const char *getName() { return MEMORY_SUBSPACE_NAME_UNDEFINED; }
  virtual const char *getDescription() {
    return MEMORY_SUBSPACE_DESCRIPTION_UNDEFINED;
  }
  ...
  virtual MM_MemoryPool *getMemoryPool(MM_EnvironmentBase *env, void *addrBase,
                                       void *addrTop, void *&highAddr);
  ...
  /* Subclass may or may not implement this. MemorySubSpace that have children
   * may implement this where merged stats will be reported. */
  virtual MM_LargeObjectAllocateStats *getLargeObjectAllocateStats();

  virtual void resetLargestFreeEntry();
  /**
   * Called by the by the collector when a region is determined to be unused to
   * allow the receiver to recycle the given region which it is responsible for
   * managing.
   * @param env[in] The calling thread (usually the master thread of the GC)
   * @param region[in] The region to be recycled
   */
  virtual void recycleRegion(MM_EnvironmentBase *env,
                             MM_HeapRegionDescriptor *region);
  virtual uintptr_t
  findLargestFreeEntry(MM_EnvironmentBase *env,
                       MM_AllocateDescription *allocateDescription);

  MMINLINE void isAllocatable(bool isAllocatable) {
    _isAllocatable = isAllocatable;
  }
  ...
  MMINLINE void setNext(MM_MemorySubSpace *memorySubSpace) {
    _next = memorySubSpace;
  }
  MMINLINE void setPrevious(MM_MemorySubSpace *memorySubSpace) {
    _previous = memorySubSpace;
  }
  ...
  MMINLINE void setCurrentSize(uintptr_t currentSize) {
  ...
  virtual uintptr_t
  getApproximateActiveFreeMemorySize(uintptr_t includeMemoryType);
  virtual uintptr_t
  getApproximateActiveFreeLOAMemorySize(uintptr_t includeMemoryType);
  virtual uintptr_t
  getApproximateActiveFreeSurvivorMemorySize(uintptr_t includeMemoryType);
  ...
  MMINLINE void setCollector(MM_Collector *collector) {
  ...
  /**
   * Generic allocation failure handler.  This will attempt to acquire exclusive
   * VM access and perform a garbage collection to reclaim resources.  The given
   * "allocationType" variable is consulted to determine which allocation
   * routine is to be called, after the collection, but before giving up
   * exclusive to ensure that the failing allocation succeeds before other
   * allocations can be attempted.
   *
   * @param[in] env The current thread
   * @param[in] allocateDescription The allocation request which triggered the
   * failure
   * @param[in] objectAllocationInterface The allocation interface which
   * attempted to satisfy the allocate (only relevant for TLH allocation
   * failures, NULL if not a TLH allocation failure)
   * @param[in] baseSubSpace The subspace which was first requested to satisfy
   * the allocate
   * @param[in] previousSubSpace The subspace which tried to handle the failure
   * before calling the receiver
   *
   * @return The address of the allocated object or base address of the
   * allocated TLH.  NULL returned on failure.
   */
  virtual void *allocationRequestFailed(
      ...
      MM_MemorySubSpace *baseSubSpace, MM_MemorySubSpace *previousSubSpace) = 0;
  /**
   * A generic helper which is used by most implementations of
   * allocationRequestFailed to issue the actual allocation request between GC
   * attempt stages.  It decodes the allocationType to determine which actual
   * allocation call to issue (Object, Leaf, or TLH).
   *
   * @param[in] env The current thread
   ...
   * @param[in] objectAllocationInterface The allocation interface through which
   * the allocate request was issued (only relevant if the allocationType is
   * TLH, otherwise this can be NULL)
   * @param[in] attemptSubspace The subspace in which to issue the allocation
   * call
   *
   * @return The address of the allocation or NULL on failure (in the case of
   * Object or Leaf allocation, this is the address of the allocate.  In the
   * case of TLH, this is the base address of the TLH)
   */
  ...
  virtual void *
  allocateTLH(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
              MM_ObjectAllocationInterface *objectAllocationInterface,
              MM_MemorySubSpace *baseSubSpace,
              MM_MemorySubSpace *previousSubSpace,
              bool shouldCollectOnFailure) {
  ...
  /**
   * Called under exclusive to request that the subspace attempt to replenish
   * the given context and satisfy the given allocateDescription of the given
   * allocationType
   *
   * @param[in] env The current thread
   * @param[in] context The allocation context which the sender failed to
   * replenish
   * @param[in] objectAllocationInterface The alocation interface through which
   * the original allocation call was initiated (only used by TLH allocations,
   * can be NULL in other cases)
   * @param[in] allocateDescription The allocation request which initiated the
   * allocation failure
   * @param[in] allocationType The type of allocation request we eventually must
   * satisfy
   *
   * @return The result of the allocation attempted under exclusive
   */
  virtual void *lockedReplenishAndAllocate(
      MM_EnvironmentBase *env, MM_AllocationContext *context,
      MM_ObjectAllocationInterface *objectAllocationInterface,
      MM_AllocateDescription *allocateDescription,
      AllocationType allocationType) {
    Assert_MM_unreachable();
  ...
  virtual void setAllocateAtSafePointOnly(MM_EnvironmentBase *env,
                                          bool safePoint);
  ...
  /**
   * Find top level parent MemorySubSpace that matches provided typeFlags.
   * Assumption is that this memory space already matches the flags.
   */
  ...
  virtual bool percolateGarbageCollect(MM_EnvironmentBase *env,
                                       MM_AllocateDescription *allocDescription,
                                       uint32_t gcCode);
  virtual bool garbageCollect(MM_EnvironmentBase *env,
                              MM_AllocateDescription *allocDescription,
                              uint32_t gcCode);
  ...
  virtual void payAllocationTax(MM_EnvironmentBase *env,
                                MM_MemorySubSpace *baseSubSpace,
                                MM_AllocateDescription *allocDescription);
  void payAllocationTax(MM_EnvironmentBase *env,
                        MM_AllocateDescription *allocDescription);

  void reportAllocationFailureStart(MM_EnvironmentBase *env,
                                    MM_AllocateDescription *allocDescription);
  void reportAllocationFailureEnd(MM_EnvironmentBase *env);

  /**
   * Report that exclusive access was acquired to satisfy an allocation.
   * Exclusive access was force acquired in order to perform a GC and guarantee
   * a succesful allocation.  However, upon acquiring exclusive access, the
   * allocation was satisfied WITHOUT having to perform a GC.  This routine is
   ...
   */
  void reportAcquiredExclusiveToSatisfyAllocate(
      MM_EnvironmentBase *env, MM_AllocateDescription *allocateDescription);

  bool setResizable(bool resizable);

  bool canExpand(MM_EnvironmentBase *env, uintptr_t expandSize);
  virtual uintptr_t adjustExpansionWithinUserIncrement(MM_EnvironmentBase *env,
  ...
  virtual void checkResize(MM_EnvironmentBase *env,
                           MM_AllocateDescription *allocDescription = NULL,
                           bool _systemGC = false);
  virtual intptr_t
  performResize(MM_EnvironmentBase *env,
                MM_AllocateDescription *allocDescription = NULL);
  ...
  uintptr_t counterBalanceContract(MM_EnvironmentBase *env,
  ...
  virtual uintptr_t counterBalanceContract(MM_EnvironmentBase *env,
                                           ...
                                           uintptr_t contractSize,
                                           uintptr_t contractAlignment);
  virtual uintptr_t counterBalanceContractWithExpand(
  ...
  uintptr_t getAvailableContractionSizeForRangeEndingAt(
      MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
      void *lowAddr, void *highAddr);
  virtual bool expanded(MM_EnvironmentBase *env, MM_PhysicalSubArena *subArena,
                        MM_HeapRegionDescriptor *region, bool canCoalesce);
  virtual bool expanded(MM_EnvironmentBase *env, MM_PhysicalSubArena *subArena,
                        uintptr_t size, void *lowAddress, void *highAddress,
                        bool canCoalesce);
  ...
  selectRegionForContraction(MM_EnvironmentBase *env, uintptr_t numaNode);
  ...
  /**
   * Called after the heap geometry changes to allow any data structures
   * dependent on this to be updated. This call could be triggered by memory
   * ranges being added to or removed from the heap or memory being moved from
  ...
  virtual void *findFreeEntryTopStartingAtAddr(MM_EnvironmentBase *env,
                                               void *addr);
  virtual void *getFirstFreeStartingAddr(MM_EnvironmentBase *env);
  virtual void *getNextFreeStartingAddr(MM_EnvironmentBase *env,
                                        void *currentFree);
  virtual void moveHeap(MM_EnvironmentBase *env, void *srcBase, void *srcTop,
                        void *dstBase);

  virtual uintptr_t
  getAvailableContractionSize(MM_EnvironmentBase *env,
                              MM_AllocateDescription *allocDescription);
  ...
  void enqueueCounterBalanceExpand(MM_EnvironmentBase *env,
  ...
  virtual bool replenishPoolForAllocate(MM_EnvironmentBase *env,
  ...
  /**
   * Some optimizations require knowing if this subspace is involved in a
   * copying semi-space
   */
   ...
   * Not thread safe - caller has to make sure no other threads are modifying
   * the stats for any of children.
   */
   ...
   */
  MM_MemorySubSpace(MM_EnvironmentBase *env, MM_Collector *collector,
                    MM_PhysicalSubArena *physicalSubArena,
                    bool usesGlobalCollector, uintptr_t minimumSize,
                    uintptr_t initialSize, uintptr_t maximumSize,
                    uintptr_t memoryType, uint32_t objectFlags)
      : MM_BaseVirtual(), _next(NULL), _previous(NULL), _children(NULL),
        _regionList(NULL), _concurrentCollectable(false),
        _memoryType(memoryType), _objectFlags(objectFlags),
        _extensions(env->getExtensions()), _collector(collector),
        _memorySpace(NULL), _parent(NULL), _physicalSubArena(physicalSubArena),
        _initialSize(initialSize), _minimumSize(minimumSize), _currentSize(0),
        _maximumSize(maximumSize), _uniqueID(0),
        _usesGlobalCollector(usesGlobalCollector), _isAllocatable(true),
        _counterBalanceType(MODRON_COUNTER_BALANCE_TYPE_NONE),
        _counterBalanceSize(0), _counterBalanceChainHead(NULL),
        _counterBalanceChain(NULL), _contractionSize(0), _expansionSize(0) {
