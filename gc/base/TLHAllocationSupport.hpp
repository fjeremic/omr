...
private:
  ...
  uint8_t** const
    _pointerToHeapTop; /**< pointer to Heap Top field for this TLH in J9VMThread
                          structure (can be heapTop or nonZeroHeapTop) */
  intptr_t* const
    _pointerToTlhPrefetchFTA; /**< pointer to tlhPrefetchFTA field for this TLH
                                 in J9VMThread structure (can be tlhPrefetchFTA
                                 or nonZeroTlhPrefetchFTA) */

  MM_ObjectAllocationInterface*
    _objectAllocationInterface; /**< Pointer to TLHAllocationInterface instance
                                 */
...
private:
  ...
  void setupTLH(MM_EnvironmentBase* env,
                void* addrBase,
                void* addrTop,
                MM_MemorySubSpace* memorySubSpace,
                MM_MemoryPool* memoryPool);
...
#if defined(OMR_GC_OBJECT_ALLOCATION_NOTIFY)
  void objectAllocationNotify(MM_EnvironmentBase* env,
                              void* heapBase,
                              void* heapTop);
#endif /* OMR_GC_OBJECT_ALLOCATION_NOTIFY */
   ...
   */
  MMINLINE void markValidObjectForRange(MM_EnvironmentBase* env,
                                        void* heapBase,
                                        void* heapTop)
  {
    env->getExtensions()->getObjectMap()->markValidObjectForRange(
      env, heapBase, heapTop);
  }
#endif
