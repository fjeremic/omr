...
public:
  MM_MemorySubSpace
      *_memorySubSpace;       /**< The memory subspace of a cached TLH. */
  MM_MemoryPool *_memoryPool; /**< The memory pool of a cached TLH. */
};

/**
 * Object allocation interface definition for TLH style allocators.
 * Implementation of the Thread Local Heap allocation style, each thread
 * receiving a per-instance version of the type to manage its caches.
...
private:
  ...
  uint8_t **const _pointerToHeapAlloc; /**< pointer to Heap Allocation field for
                                          this TLH in J9VMThread structure (can
                                          be heapAlloc or nonZeroHeapAlloc) */
  uint8_t **const _pointerToHeapTop; /**< pointer to Heap Top field for this TLH
                                        in J9VMThread structure (can be heapTop
                                        or nonZeroHeapTop) */
  intptr_t *const
      _pointerToTlhPrefetchFTA; /**< pointer to tlhPrefetchFTA field for this
                                   TLH in J9VMThread structure (can be
                                   tlhPrefetchFTA or nonZeroTlhPrefetchFTA) */

  MM_ObjectAllocationInterface
      *_objectAllocationInterface; /**< Pointer to TLHAllocationInterface
                                      instance */

  MM_HeapLinkedFreeHeaderTLH
      *_abandonedList; /**< List of abandoned TLHs. Shaped like a free list. */
  uintptr_t _abandonedListSize; /**< Number of entries in the abandoned list. */

  const bool _zeroTLH; /**< if true this TLH is primary (might be cleared by
                          batchClearTLH), if false this is secondary TLH (and it
                          would not be cleared ever) */

public:
protected:
private:
  void *allocateTLH(MM_EnvironmentBase *env,
   ...
   * TLH properly under frequent hook and unhook of ObjectAllocInstrumentable,
   * which toggles enable/disable of TLH. If TLH is disabled, realHeapAlloc
   * holds the true TLH alloc ptr.
   */
  MMINLINE void setRealAlloc(void *realAllocPtr) {
  ...
  MMINLINE void setObjectFlags(uint32_t flags) {
  ...
  bool refresh(MM_EnvironmentBase *env,
               MM_AllocateDescription *allocDescription,
               bool shouldCollectOnFailure);

  void *allocateFromTLH(MM_EnvironmentBase *env,
                        MM_AllocateDescription *allocDescription,
                        bool shouldCollectOnFailure);

  void setupTLH(MM_EnvironmentBase *env, void *addrBase, void *addrTop,
...
#if defined(OMR_GC_OBJECT_ALLOCATION_NOTIFY)
  void objectAllocationNotify(MM_EnvironmentBase *env, void *heapBase,
                              void *heapTop);
#endif /* OMR_GC_OBJECT_ALLOCATION_NOTIFY */
   ...
   */
  MMINLINE void markValidObjectForRange(MM_EnvironmentBase *env, void *heapBase,
                                        void *heapTop) {
    env->getExtensions()->getObjectMap()->markValidObjectForRange(env, heapBase,
                                                                  heapTop);
  }
#endif
  ...
  MM_TLHAllocationSupport(MM_EnvironmentBase *env, bool zeroTLH)
      : _omrVMThread(env->getOmrVMThread()), _languageTLH(),
        ...
        _pointerToTlhPrefetchFTA(
            _languageTLH.getPointerToTlhPrefetchFTA(env, zeroTLH)),
        _objectAllocationInterface(NULL), _abandonedList(NULL),
        _abandonedListSize(0), _zeroTLH(zeroTLH){};
