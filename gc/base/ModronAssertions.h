...
/* Currently, there are startup errors pointed out by these assertions so only
 * enable them on combination spec until they are fixed for everyone */
#if defined(OMR_GC_DEBUG_ASSERTS)

  extern void omrGcDebugAssertionOutput(OMRPortLibrary* portLibrary,
...
#define Assert_GC_true_with_message(                                           \
  __env__, __condition, __message, __parameter)                                \
  do {                                                                         \
    if (!(__condition)) {                                                      \
      omrGcDebugAssertionOutput(__env__->getPortLibrary(),                     \
                                __env__->getOmrVMThread(),                     \
                                __message,                                     \
                                __parameter);                                  \
...
#define Assert_GC_true_with_message2(                                          \
  __env__, __condition, __message, __parameter1, __parameter2)                 \
  do {                                                                         \
    if (!(__condition)) {                                                      \
      omrGcDebugAssertionOutput(__env__->getPortLibrary(),                     \
                                __env__->getOmrVMThread(),                     \
...
#define Assert_GC_true_with_message3(                                          \
  __env__, __condition, __message, __parameter1, __parameter2, __parameter3)   \
  do {                                                                         \
    if (!(__condition)) {                                                      \
      omrGcDebugAssertionOutput(__env__->getPortLibrary(),                     \
                                __env__->getOmrVMThread(),                     \
...
#define Assert_GC_true_with_message4(__env__,                                  \
                                     __condition,                              \
                                     __message,                                \
                                     __parameter1,                             \
                                     __parameter2,                             \
  ...
  do {                                                                         \
    if ((uintptr_t)(__pointer) & (__env__->getObjectAlignmentInBytes() - 1)) { \
      Assert_GC_true_with_message2(                                            \
        __env__,                                                               \
        false,                                                                 \
        "Pointer: %p has is not object aligned (to %zu bytes) \n",             \
        __pointer,                                                             \
        __env__->getObjectAlignmentInBytes());                                 \
    }                                                                          \
  } while (false)
...
#define Assert_GC_true_with_message4(__env__,                                  \
  ...
  Assert_MM_true(__condition)
#define Assert_MM_objectAligned(__env__, __pointer)                            \
  Assert_MM_true_internal((uintptr_t)(__pointer) &                             \
                          (__env__->getObjectAlignmentInBytes() - 1))
