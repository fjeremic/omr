...
extern "C"
{
  ...
  int J9THREAD_PROC dispatcher_thread_proc(void* info)
  {
    ...
    uintptr_t rc;
    omrsig_protect(dispatcher_thread_proc2,
                   info,
                   dispatcher->getSignalHandler(),
...
MM_ParallelDispatcher::initialize(MM_EnvironmentBase* env)
{
  ...
  if (omrthread_monitor_init_with_name(
        &_slaveThreadMutex, 0, "MM_ParallelDispatcher::slaveThread") ||
      omrthread_monitor_init_with_name(
        &_dispatcherMonitor, 0, "MM_ParallelDispatcher::dispatcherControl") ||
      omrthread_monitor_init_with_name(
        &_synchronizeMutex, 0, "MM_ParallelDispatcher::synchronize")) {
    goto error_no_memory;
  }

  /* Initialize the thread tables */
  _threadTable =
    (omrthread_t*)forge->allocate(_threadCountMaximum * sizeof(omrthread_t),
                                  OMR::GC::AllocationCategory::FIXED,
                                  OMR_GET_CALLSITE());
  if (!_threadTable) {
    goto error_no_memory;
  ...
  _statusTable =
    (uintptr_t*)forge->allocate(_threadCountMaximum * sizeof(uintptr_t*),
                                OMR::GC::AllocationCategory::FIXED,
                                OMR_GET_CALLSITE());
  if (!_statusTable) {
    goto error_no_memory;
  ...
  _taskTable =
    (MM_Task**)forge->allocate(_threadCountMaximum * sizeof(MM_Task*),
                               OMR::GC::AllocationCategory::FIXED,
                               OMR_GET_CALLSITE());
  if (!_taskTable) {
    goto error_no_memory;
...
MM_ParallelDispatcher::startUpThreads()
{
  ...
  while (slaveThreadCount < _threadCountMaximum) {
    ...
    threadForkResult =
      createThreadWithCategory(&(_threadTable[slaveThreadCount]),
                               _defaultOSStackSize,
                               getThreadPriority(),
                               0,
 ...
 */
uintptr_t
MM_ParallelDispatcher::recomputeActiveThreadCountForTask(
...
void
MM_ParallelDispatcher::prepareThreadsForTask(MM_EnvironmentBase* env,
                                             MM_Task* task,
                                             uintptr_t threadCount)
{
  omrthread_monitor_enter(_slaveThreadMutex);
...
void
MM_ParallelDispatcher::reinitAfterFork(MM_EnvironmentBase* env,
