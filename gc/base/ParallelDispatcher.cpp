...
uintptr_t dispatcher_thread_proc2(OMRPortLibrary *portLib, void *info) {
  ...
  omrVMThread = MM_EnvironmentBase::attachVMThread(
      omrVM, "GC Slave", MM_EnvironmentBase::ATTACH_GC_DISPATCHER_THREAD);
  if (NULL == omrVMThread) {
    goto startup_failed;
  }
  ...
  MM_EnvironmentBase::detachVMThread(
      omrVM, omrVMThread, MM_EnvironmentBase::ATTACH_GC_DISPATCHER_THREAD);
...
int J9THREAD_PROC dispatcher_thread_proc(void *info) {
  OMR_VM *omrVM = ((slaveThreadInfo *)info)->omrVM;
  MM_ParallelDispatcher *dispatcher = ((slaveThreadInfo *)info)->dispatcher;
  OMRPORT_ACCESS_FROM_OMRVM(omrVM);
  uintptr_t rc;
  omrsig_protect(
      dispatcher_thread_proc2, info, dispatcher->getSignalHandler(),
      dispatcher->getSignalHandlerArg(),
 ...
 * </code>
 */
void MM_ParallelDispatcher::slaveEntryPoint(MM_EnvironmentBase *env) {
...
void MM_ParallelDispatcher::masterEntryPoint(MM_EnvironmentBase *env) {
...
MM_ParallelDispatcher *MM_ParallelDispatcher::newInstance(
    MM_EnvironmentBase *env, omrsig_handler_fn handler, void *handler_arg,
    uintptr_t defaultOSStackSize) {
  MM_ParallelDispatcher *dispatcher;

  dispatcher = (MM_ParallelDispatcher *)env->getForge()->allocate(
      sizeof(MM_ParallelDispatcher), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (dispatcher) {
    new (dispatcher)
...
void MM_ParallelDispatcher::kill(MM_EnvironmentBase *env) {
...
bool MM_ParallelDispatcher::initialize(MM_EnvironmentBase *env) {
  OMR::GC::Forge *forge = env->getForge();

  _threadCountMaximum = env->getExtensions()->gcThreadCount;
  Assert_MM_true(0 < _threadCountMaximum);

  if (omrthread_monitor_init_with_name(&_slaveThreadMutex, 0,
                                       "MM_ParallelDispatcher::slaveThread") ||
      omrthread_monitor_init_with_name(
          &_dispatcherMonitor, 0, "MM_ParallelDispatcher::dispatcherControl") ||
      omrthread_monitor_init_with_name(&_synchronizeMutex, 0,
                                       "MM_ParallelDispatcher::synchronize")) {
    goto error_no_memory;
  }

  /* Initialize the thread tables */
  _threadTable = (omrthread_t *)forge->allocate(
      _threadCountMaximum * sizeof(omrthread_t),
  ...
  _statusTable = (uintptr_t *)forge->allocate(
      _threadCountMaximum * sizeof(uintptr_t *),
      OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
  if (!_statusTable) {
    goto error_no_memory;
  }
  ...
  _taskTable = (MM_Task **)forge->allocate(
      _threadCountMaximum * sizeof(MM_Task *),
      OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
  if (!_taskTable) {
    goto error_no_memory;
  }
...
bool MM_ParallelDispatcher::startUpThreads() {
  ...
  while (slaveThreadCount < _threadCountMaximum) {
    ...
    threadForkResult = createThreadWithCategory(
        &(_threadTable[slaveThreadCount]), _defaultOSStackSize,
        getThreadPriority(), 0, dispatcher_thread_proc, (void *)&slaveInfo,
...
void MM_ParallelDispatcher::shutDownThreads() {
  _inShutdown = true;
  ...
  /* TODO: Shutdown should account for threads that failed to start up in the
   * first place */
  omrthread_monitor_enter(_slaveThreadMutex);
 ...
 */
void MM_ParallelDispatcher::wakeUpThreads(uintptr_t count) {
 ...
 */
void MM_ParallelDispatcher::setThreadCount(uintptr_t threadCount) {
 ...
 */
uintptr_t MM_ParallelDispatcher::recomputeActiveThreadCountForTask(
    MM_EnvironmentBase *env, MM_Task *task, uintptr_t threadCount) {
  ...
  if (!_extensions->isMetronomeGC()) {
     ...
     *  3) 1 if we have not yet started the GC helpers or are in process of
     * shutting down the GC helper threads.
     */
    _activeThreadCount = adjustThreadCount(_threadCount);
  }

  /* Caller might have tried to override thread count for this task with an
   * explicit value. Obey it, only if <= than what we calculated it should be
   * (there might not be more active threads available and ready to run).
   */
...
uintptr_t MM_ParallelDispatcher::adjustThreadCount(uintptr_t maxThreadCount) {
  uintptr_t toReturn = maxThreadCount;

  /* Did user specify number of gc threads? */
  if (!_extensions->gcThreadCountForced) {
    /* No ...Use a sensible number of threads for current heap size. Using too
     * many can lead to unacceptable pause times due to insufficient
     * parallelism. Additionally, it can lead to excessive fragmentation,
     * causing aborts and percolates.
     */
    MM_Heap *heap = (MM_Heap *)_extensions->heap;
    uintptr_t heapSize = heap->getActiveMemorySize();
    uintptr_t maximumThreadsForHeapSize =
        (heapSize > MINIMUM_HEAP_PER_THREAD)
            ? heapSize / MINIMUM_HEAP_PER_THREAD
            : 1;
    if (maximumThreadsForHeapSize < maxThreadCount) {
      Trc_MM_ParallelDispatcher_adjustThreadCount_smallHeap(
          maximumThreadsForHeapSize);
      toReturn = maximumThreadsForHeapSize;
    }

    OMRPORT_ACCESS_FROM_OMRVM(_extensions->getOmrVM());
    /* No, use the current active CPU count (unless it would overflow our
     * threadtables) */
    uintptr_t activeCPUs =
...
void MM_ParallelDispatcher::prepareThreadsForTask(MM_EnvironmentBase *env,
                                                  MM_Task *task,
...
void MM_ParallelDispatcher::acceptTask(MM_EnvironmentBase *env) {
  uintptr_t slaveID = env->getSlaveID();
...
void MM_ParallelDispatcher::completeTask(MM_EnvironmentBase *env) {
  uintptr_t slaveID = env->getSlaveID();
...
void MM_ParallelDispatcher::cleanupAfterTask(MM_EnvironmentBase *env) {
  omrthread_monitor_enter(_slaveThreadMutex);
 ...
 */
uintptr_t MM_ParallelDispatcher::getThreadPriority() {
 ...
 */
void MM_ParallelDispatcher::setThreadInitializationComplete(
    MM_EnvironmentBase *env) {
  uintptr_t slaveID = env->getSlaveID();

  /* Set the status of the thread to waiting and notify that the thread has
   * started up */
  omrthread_monitor_enter(_dispatcherMonitor);
...
void MM_ParallelDispatcher::reinitAfterFork(MM_EnvironmentBase *env,
