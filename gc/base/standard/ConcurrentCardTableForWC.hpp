...
typedef enum { MARK_DIRTY_CARD_SAFE = 1, MARK_SAFE_CARD_DIRTY } CardAction;
...
class MM_ConcurrentCardTableForWC : public MM_ConcurrentCardTable {
  ...
  uintptr_t countCardsInRange(MM_EnvironmentBase *env, Card *rangeStart,
                              Card *rangeEnd);
...
public:
  ...
  void prepareCardTableChunk(MM_EnvironmentBase *env, Card *chunkStart,
                             Card *chunkEnd, CardAction action);
  virtual void initializeFinalCardCleaning(MM_EnvironmentBase *env);

  static void prepareCardTableAsyncEventHandler(OMR_VMThread *omrVMThread,
                                                void *userData);

  /* TODO: For now always return false but we could just check if card is
   * CARD_DIRTY as we know such cards are sure to be cleaned in future either
   * after a STW card table prepare phase or during FCC.
   */
  MMINLINE virtual bool isObjectInUncleanedDirtyCard(MM_EnvironmentBase *env,
   ...
   */
  MM_ConcurrentCardTableForWC(MM_EnvironmentBase *env,
                              MM_MarkingScheme *markingScheme,
                              MM_ConcurrentGC *collector)
      : MM_ConcurrentCardTable(env, markingScheme, collector),
        _cardTablePreparedForCleaning(false), _callback(NULL) {
