 ...
 */
MM_ConcurrentCardTable *MM_ConcurrentCardTableForWC::newInstance(
  ...
  cardTable = (MM_ConcurrentCardTableForWC *)env->getForge()->allocate(
      sizeof(MM_ConcurrentCardTableForWC), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (NULL != cardTable) {
 ...
 */
bool MM_ConcurrentCardTableForWC::initialize(MM_EnvironmentBase *env,
                                             MM_Heap *heap) {
  ...
  _callback->registerCallback(env, prepareCardTableAsyncEventHandler, this,
                              true);
...
void MM_ConcurrentCardTableForWC::tearDown(MM_EnvironmentBase *env) {
 ...
 *
 */
void MM_ConcurrentCardTableForWC::prepareCardTableAsyncEventHandler(
    OMR_VMThread *omrVMThread, void *userData) {
  MM_ConcurrentCardTableForWC *cardTable =
      (MM_ConcurrentCardTableForWC *)userData;
 ...
 *
 */
void MM_ConcurrentCardTableForWC::prepareCardTable(MM_EnvironmentBase *env) {
 ...
 *
 * On a weakly ordered platform the update of a reference and dirtying of a card
 * could be reversed so a card is dirtied before the associated update is
 * visible. As we need to be sure that we see all the updated references when we
 * rescan the cards marked objects we need to ensure the card table and objects
 * are synchronized. As we don't want to have to issue a store barrier after
 * every reference update (far too expensive) we instead stop all threads to
 * ensure all its updates are flushed. We then walk the card table and flag any
 * dirty cards (i.e set them to CARD_CLEAN_SAFE) to show that all updates to its
 * associated objects are visible. We then restart all threads and start
 * concurrent card cleaning. If a card is subsequently redirtied it is set again
 * to CARD_DIRTY by the write barrier.
 *
 * Thus during the concurrent phase we can clean all unclean cards (CARD_DIRTY
 * or CARD_CLEAN_SAFE) but any marked as CARD_DIRTY at final card cleaning time
 * must be re-cleaned to ensure we don't miss any object references.
 */
void MM_ConcurrentCardTableForWC::prepareCardsForCleaning(
  ...
  // ToDo make this more intelligent; no point using helpers if only a "few"
  // cards to process. Also, may want to chop chunk up into less parts in 2nd
  // phase of concurrent card cleaning
  MM_ConcurrentPrepareCardTableTask prepareCardTableTask(
      env, _dispatcher, this, _firstCardInPhase, _lastCardInPhase,
 ...
 */
MMINLINE bool MM_ConcurrentCardTableForWC::getExclusiveCardTableAccess(
    MM_EnvironmentBase *env, CardCleanPhase currentPhase,
    bool threadAtSafePoint) {
  /* Because the WC CardTable requires exclusive access to prepare the cards for
   * cleaning, we cannot gain exclusive access to the card table if the thread
   * is not at a safe point. Request async call back on this thread if this is
   * the case so we can prepare card table
   */
 ...
 */
MMINLINE void MM_ConcurrentCardTableForWC::releaseExclusiveCardTableAccess(
    MM_EnvironmentBase *env) {
 ...
 */
uintptr_t MM_ConcurrentCardTableForWC::countCardsInRange(
    MM_EnvironmentBase *env, Card *chunkStart, Card *chunkEnd) {
  uintptr_t cardsInRange = 0;
  CleaningRange *cleaningRange = _cleaningRanges;

  /* Find cleaning range that corresponds to start of card table range to be
   * counted */
  while (cleaningRange < _lastCleaningRange &&
         cleaningRange->topCard < chunkStart) {
    cleaningRange++;
  }
   ...
   */
  while (cleaningRange < _lastCleaningRange &&
         cleaningRange->baseCard < chunkEnd) {
 ...
 *
 * This function is passed the start and end of a chunk of the card table which
 * needs preparing for card cleaning; be it concurrent or final card cleaning.
 * As the chunk may contain holes (non-contiguous heap) we use the
 * _cleaningRanges array to determine which cards within the chunk actually need
 * processing, ie a chunk can span part of one or more cleaning ranges. The
 * "action" passed to this function defines what we do to each unclean card
 * within the chunk. If action is MARK_DIRTY_CARD_SAFE we modify all cards with
 * value CARD_DIRTY to CARD_CLEAN_SAFE; for action MARK_SAFE_CARD_DIRTY we reset
 * any cards marked as CARD_CLEAN_SAFE to CARD_DIRTY.
 *
 * @param chunkStart - first card to be processed
 * @param chunkEnd   - last card to be processed
 * @param action - defines what to do to each un-clean card, value is either
 * MARK_DIRTY_CARD_SAFE or MARK_SAFE_CARD_DIRTY.
 */
void MM_ConcurrentCardTableForWC::prepareCardTableChunk(MM_EnvironmentBase *env,
                                                        Card *chunkStart,
                                                        Card *chunkEnd,
                                                        CardAction action) {
  ...
  prepareUnitFactor = ((prepareUnitFactor == 1)
                           ? 1
                           : prepareUnitFactor * PREPARE_PARALLEL_MULTIPLIER);
  prepareUnitSize = countCardsInRange(env, chunkStart, chunkEnd);
  prepareUnitSize = prepareUnitSize / prepareUnitFactor;
  prepareUnitSize = prepareUnitSize > 0
                        ? MM_Math::roundToCeiling(PREPARE_UNIT_SIZE_ALIGNMENT,
                                                  prepareUnitSize)
  ...
  for (CleaningRange *range = _cleaningRanges; range < _lastCleaningRange;
    ...
    prepareAddress =
        chunkStart > range->baseCard ? chunkStart : range->baseCard;
    prepareSizeRemaining = chunkEnd > range->topCard
                               ? range->topCard - prepareAddress
                               : chunkEnd - prepareAddress;

    while (prepareSizeRemaining > 0) {
      /* Calculate the size of card table chunk to be processed next */
      currentPrepareSize = (prepareUnitSize > prepareSizeRemaining)
                               ? prepareSizeRemaining
                               : prepareUnitSize;

      /* Check if the thread should clear the corresponding mark map range for
      ...
      if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
        ...
        for (Card *currentCard = firstCard; currentCard < endCard;
             currentCard++) {
          /* Are we are on an uintptr_t boundary ?. If so scan the card table a
           * uintptr_t at a time until we find a slot which is non-zero or the
           * end of card table found. This is based on the premise that the card
           * table will be mostly empty and scanning an uintptr_t at a time will
           * reduce the time taken to scan the card table.
           */
          if (((Card)CARD_CLEAN == *currentCard) &&
              ((uintptr_t)currentCard % sizeof(uintptr_t) == 0)) {
            uintptr_t *nextSlot = (uintptr_t *)currentCard;
            while (((Card *)nextSlot < endCard) &&
                   (*nextSlot == SLOT_ALL_CLEAN)) {
              nextSlot++;
            }

            /*
             * Either end of scan or a slot which contains a dirty card found.
             * Reset scan ptr
             */
 ...
 *
 * If we prepared some cards during concurrent phase then we need to check to
 * ensure we managed to process them all, i.e. did we complete concurrent card
 * cleaning pass. If not we need to reset any prepared cards (i.e those set to
 * CARD_CLEAN_SAFE) which were not cleaned back to card_DIRTY so that they are
 * rescanned during final card cleaning.
 */
void MM_ConcurrentCardTableForWC::initializeFinalCardCleaning(
    MM_EnvironmentBase *env) {
  ...
  if (_cardTablePreparedForCleaning) {
    ...
    if (_currentCleaningRange < _lastCleaningRange) {
      ...
      MM_ConcurrentPrepareCardTableTask prepareCardTableTask(
          env, _dispatcher, this, (Card *)_currentCleaningRange->nextCard,
          _lastCard, MARK_SAFE_CARD_DIRTY);
      _dispatcher->run(env, &prepareCardTableTask);
    }
