...
private:
     ...
     * @param allocDescription[in] The allocation description which triggered the GC (NULL if this collection was not
     ...
     *	@param initMarkMap instruct should mark map be initialized (might be already partially done like in
     *conrurrentGC)
     */
...
protected:
    ...
    MM_ParallelSweepScheme* createSweepScheme(MM_EnvironmentBase* env, MM_GlobalCollector* globalCollector)
    {
...
#if defined(OMR_GC_MODRON_COMPACTION)
    MM_CompactScheme* getCompactScheme(MM_EnvironmentBase* env) { return _compactScheme; }
#endif /* OMR_GC_MODRON_COMPACTION */
