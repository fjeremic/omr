...
private:
   ...
   */
  void sweep(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
             bool rebuildMarkBits);

  /**
   * Answer whether a complete rebuild of freelist should be performed during
   * the sweep phase.
   ...
   */
  bool completeFreelistRebuildRequired(MM_EnvironmentBase *env,
   ...
   * @param allocDescription[in] The allocation description which triggered the
   * GC (NULL if this collection was not triggered by an allocation)
   * @param activeSubspaceMaxExpansionInSpace[in] The maximum expansion size of
   * the active subspace
   * @param gcCode[in] a code describing the type of GC cycle
   *
   * @return true if a compaction is required, false otherwise
   */
  bool shouldCompactThisCycle(MM_EnvironmentBase *env,
   ...
   */
  bool
  compactRequiredBeforeHeapContraction(MM_EnvironmentBase *env,
                                       MM_AllocateDescription *allocDescription,
   ...
   *	@param initMarkMap instruct should mark map be initialized (might be
   *already partially done like in conrurrentGC)
   */
   ...
   */
  void masterThreadSweepStart(MM_EnvironmentBase *env,
                              MM_AllocateDescription *allocDescription);

  /**
   * 	Complete Sweep operation for concurrentGC, empty for all other cases
   */
  void masterThreadSweepComplete(MM_EnvironmentBase *env,
                                 SweepCompletionReason reason);
   ...
   */
  void masterThreadCompact(MM_EnvironmentBase *env,
                           MM_AllocateDescription *allocDescription,
                           bool rebuildMarkBits);
#endif /* OMR_GC_MODRON_COMPACTION */
   ...
   */
  void cleanupAfterGC(MM_EnvironmentBase *env,
                      MM_AllocateDescription *allocDescription);

  /**
   * redistribute free memory in tenure after global collection (move free
   * memory from LOA to SOA)
   */
...
protected:
  ...
  virtual void
  masterThreadGarbageCollect(MM_EnvironmentBase *env,
                             MM_AllocateDescription *allocDescription,
  ...
  /**
   * compaction would change freeEntries stats, merge FreeEntryAllocateStats
   * after compaction. currently processLargeAllocateStatsAfterCompact() is same
   * as processLargeAllocateStatsAfterSweep()
   */
  ...
  createSweepScheme(MM_EnvironmentBase *env,
...
public:
  ...
  /**
   *  Fixes up all unloaded objects so that the heap can be walked and only live
   * objects returned
   *  @param reason fix heap reason
   */
  uintptr_t fixHeapForWalk(MM_EnvironmentBase *env, UDATA walkFlags,
   ...
   */
  MM_MarkingScheme *getMarkingScheme() { return _markingScheme; }

#if defined(OMR_GC_MODRON_COMPACTION)
  MM_CompactScheme *getCompactScheme(MM_EnvironmentBase *env) {
...
#endif /* OMR_GC_MODRON_COMPACTION */
        ,
        _markingScheme(NULL), _sweepScheme(NULL), _heapWalker(NULL),
        _dispatcher(_extensions->dispatcher), _cycleState(),
        _collectionStatistics(), _fixHeapForWalkCompleted(false) {
