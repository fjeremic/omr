...
public:
  ...
  virtual bool synchronizeGCThreadsAndReleaseMaster(MM_EnvironmentBase *env,
                                                    const char *id);
  virtual bool
  synchronizeGCThreadsAndReleaseSingleThread(MM_EnvironmentBase *env,
                                             const char *id);
   ...
   */
  MM_ParallelSweepTask(MM_EnvironmentBase *env, MM_Dispatcher *dispatcher,
                       MM_ParallelSweepScheme *sweepScheme)
      : MM_ParallelTask(env, dispatcher), _sweepScheme(sweepScheme) {
...
protected:
  ...
  MM_MarkMap *_currentMarkMap; /**< The MarkMap which the ParallelGlobalGC gave
                                  to us to use for this cycle */
  uint8_t
      *_currentSweepBits; /*< The base address of the raw bits used by the
                             _currentMarkMap (sweep knows about this in order to
                             perform some optimized types of map walking) */

  void *_heapBase;

  MM_SweepHeapSectioning
      *_sweepHeapSectioning; /**< pointer to Sweep Heap Sectioning */

  J9Pool *_poolSweepPoolState; /**< Memory pools for SweepPoolState*/
  omrthread_monitor_t _mutexSweepPoolState; /**< Monitor to protect memory pool
                                               operations for sweepPoolState*/
...
protected:
  ...
  void sweepMarkMapBody(uintptr_t *&markMapCurrent, uintptr_t *&markMapChunkTop,
                        uintptr_t *&markMapFreeHead,
                        uintptr_t &heapSlotFreeCount,
                        uintptr_t *&heapSlotFreeCurrent,
                        uintptr_t *&heapSlotFreeHead);
  void sweepMarkMapHead(uintptr_t *markMapFreeHead, uintptr_t *markMapChunkBase,
                        uintptr_t *&heapSlotFreeHead,
                        uintptr_t &heapSlotFreeCount);
  void sweepMarkMapTail(uintptr_t *markMapCurrent, uintptr_t *markMapChunkTop,
                        uintptr_t &heapSlotFreeCount);
  ...
  static void hookMemoryPoolNew(J9HookInterface **hook, uintptr_t eventNum,
                                void *eventData, void *userData);
  static void hookMemoryPoolKill(J9HookInterface **hook, uintptr_t eventNum,
                                 void *eventData, void *userData);
...
public:
  ...
  /**
   * Sets the mark map to be used by the sweep scheme for this sweep operation.
   * Called by the ParallelGlobalGC to configure sweep and could be called
   * again, if sweep is to change to operate on a new mark map.
  ...
  /**
   * Accurately measure the dark matter within the mark map uintptr_t beginning
   * at heapSlotFreeCurrent. Leading dark matter (before the first marked
   * object) is not included. Trailing dark matter (after the last marked
   * object) is included, even if it falls outside of the mark map range.
   *
   ...
   * @param heapSlotFreeCurrent[in] the first slot in the section of heap to be
   * measured
   *
   * @return the dark matter, measured in bytes, in the range
   */
  uintptr_t performSamplingCalculations(MM_ParallelSweepChunk *sweepChunk,
  ...
  MM_ParallelSweepScheme(MM_EnvironmentBase *env)
      : MM_BaseVirtual(), _chunksPrepared(0), _extensions(env->getExtensions()),
        _dispatcher(_extensions->dispatcher), _currentMarkMap(NULL),
        _currentSweepBits(NULL), _heapBase(NULL), _sweepHeapSectioning(NULL),
        _poolSweepPoolState(NULL), _mutexSweepPoolState(0) {
    _typeId = __FUNCTION__;
  }
