...
bool MM_CopyScanCacheList::initialize(MM_EnvironmentBase *env,
                                      volatile uintptr_t *cachedEntryCount) {
  ...
  _sublists = (struct CopyScanCacheSublist *)extensions->getForge()->allocate(
      sizeof(struct CopyScanCacheSublist) * _sublistCount,
...
void MM_CopyScanCacheList::tearDown(MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
...
bool MM_CopyScanCacheList::resizeCacheEntries(
    MM_EnvironmentBase *env, uintptr_t allocateCacheEntryCount,
  ...
  /* If -Xgc:fvtest=scanCacheCountn has been specified, then restrict the number
   * of scan caches to n. Stop all future resizes from having any effect. */
   ...
   * the free caches are scattered across the chunks and cross reference
   * themselves */

  return true;
}

void MM_CopyScanCacheList::removeAllHeapAllocatedChunks(
      ...
      while (cache != NULL) {
        if (0 != (cache->flags & OMR_SCAVENGER_CACHE_TYPE_HEAP)) {
          ...
          if (NULL == previousCache) {
            ...
            _sublists[index]._cacheHead =
                (MM_CopyScanCacheStandard *)cache->next;
          } else {
...
bool MM_CopyScanCacheList::appendCacheEntries(MM_EnvironmentBase *env,
                                              uintptr_t cacheEntryCount) {
  bool result = false;
  MM_CopyScanCacheStandard *sublistTail = NULL;
  MM_CopyScanCacheChunk *chunk = MM_CopyScanCacheChunk::newInstance(
      env, cacheEntryCount, _chunkHead, &sublistTail);
  if (NULL != chunk) {
...
MM_CopyScanCacheStandard *MM_CopyScanCacheList::appendCacheEntriesInHeap(
  ...
  MM_CopyScanCacheChunkInHeap *chunk = MM_CopyScanCacheChunkInHeap::newInstance(
      env, _chunkHead, memorySubSpace, requestCollector, &sublistTail,
      &entries);
  if (NULL != chunk) {
    uintptr_t index = getSublistIndex(env);
...
uintptr_t MM_CopyScanCacheList::getApproximateEntryCount() {
  uintptr_t entries = 0;
  for (uintptr_t i = 0; i < _sublistCount; i++) {
...
bool MM_CopyScanCacheList::areAllCachesReturned() {
  return (getApproximateEntryCount() == _totalAllocatedEntryCount);
}

MMINLINE void MM_CopyScanCacheList::incrementCount(
    CopyScanCacheSublist *sublist, uintptr_t value) {
...
MMINLINE void MM_CopyScanCacheList::decrementCount(
    CopyScanCacheSublist *sublist, uintptr_t value) {
...
void MM_CopyScanCacheList::pushCache(MM_EnvironmentBase *env,
                                     MM_CopyScanCacheStandard *cacheEntry) {
  MM_CopyScanCacheList::CopyScanCacheSublist *list =
      &_sublists[getSublistIndex(env)];
...
MM_CopyScanCacheStandard *MM_CopyScanCacheList::popCache(
