...
bool MM_CopyScanCacheList::initialize(MM_EnvironmentBase* env, volatile uintptr_t* cachedEntryCount)
{
    ...
    _sublists = (struct CopyScanCacheSublist*)extensions->getForge()->allocate(
        sizeof(struct CopyScanCacheSublist) * _sublistCount, OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (NULL == _sublists) {
        result = false;
    } else {
...
void MM_CopyScanCacheList::removeAllHeapAllocatedChunks(MM_EnvironmentStandard* env)
{
...
bool MM_CopyScanCacheList::appendCacheEntries(MM_EnvironmentBase* env, uintptr_t cacheEntryCount)
{
    bool result = false;
...
MMINLINE void MM_CopyScanCacheList::incrementCount(CopyScanCacheSublist* sublist, uintptr_t value)
{
...
MMINLINE void MM_CopyScanCacheList::decrementCount(CopyScanCacheSublist* sublist, uintptr_t value)
{
...
void MM_CopyScanCacheList::pushCache(MM_EnvironmentBase* env, MM_CopyScanCacheStandard* cacheEntry)
{
...
MM_CopyScanCacheStandard* MM_CopyScanCacheList::popCache(MM_EnvironmentBase* env)
{
    uintptr_t index = getSublistIndex(env);
