...
protected:
   ...
   * @param appendCollectorLargeAllocateStats - if true, configure the pool to
   * append Collector allocates to Mutator allocates (default is to gather only
   * Mutator)
   */
  virtual MM_MemoryPool *
  createMemoryPool(MM_EnvironmentBase *env,
...
public:
  ...
  MM_ConfigurationStandard(MM_EnvironmentBase *env, MM_GCPolicy gcPolicy,
      ...
      : MM_Configuration(env, gcPolicy, mm_regionAlignment, regionSize,
