...
class MM_ConcurrentGC : public MM_ParallelGlobalGC {
   ...
   */
private:
  typedef enum { MARK_BITS = 1, CARD_TABLE } InitType;
  ...
  typedef enum { SOA = 1, LOA } MeteringType;

  typedef enum { VOTE_UNDEFINED = 0, VOTE_SOA, VOTE_LOA } MeteringVote;
  ...
  void *_heapBase;
  bool _rebuildInitWorkForRemove; /**< set if heap contraction triggered
                                     _initRanges table update */
  ...
  bool _stwCollectionInProgress; /**< if set, the final STW phase is in progress
                                    (mutators not running) */
...
protected:
  ...
  void *_heapAlloc;
  bool _rebuildInitWorkForAdd; /**< set if heap expansion triggered _initRanges
                                  table update */
...
public:
   ...
   */
private:
  MMINLINE float getAllocToTraceRateMin() {
    return (float)(_allocToTraceRate * _allocToTraceRateMinFactor);
  };
  MMINLINE float getAllocToTraceRateMax() {
    return (float)(_allocToTraceRate * _allocToTraceRateMaxFactor);
  };
  MMINLINE float getAllocToTraceRateNormal() {
    return (float)_allocToTraceRateNormal;
  };
  ...
  bool getInitRange(MM_EnvironmentBase *env, void **from, void **to,
                    InitType *type, bool *concurrentCollectable);
  bool allInitRangesProcessed() {
    return (_nextInitRange == _numInitRanges ? true : false);
  };
  ...
  void kickoffCardCleaning(MM_EnvironmentBase *env,
  ...
  bool timeToKickoffConcurrent(MM_EnvironmentBase *env,
                               MM_AllocateDescription *allocDescription);

  bool tracingRateDropped(MM_EnvironmentBase *env);
#if defined(OMR_GC_MODRON_SCAVENGER)
  uintptr_t potentialFreeSpace(MM_EnvironmentBase *env,
                               MM_AllocateDescription *allocDescription);
  ...
  void reportConcurrentAborted(MM_EnvironmentBase *env,
  ...
  void reportConcurrentFinalCardCleaningEnd(MM_EnvironmentBase *env,
                                            uint64_t duration);
  void reportConcurrentCollectionStart(MM_EnvironmentBase *env);
  void reportConcurrentCollectionEnd(MM_EnvironmentBase *env,
                                     uint64_t duration);

  void reportConcurrentBackgroundThreadActivated(MM_EnvironmentBase *env);
  void reportConcurrentBackgroundThreadFinished(MM_EnvironmentBase *env,
                                                uintptr_t traceTotal);

  void reportConcurrentCompleteTracingStart(MM_EnvironmentBase *env);
  void reportConcurrentCompleteTracingEnd(MM_EnvironmentBase *env,
                                          uint64_t duration);

  void reportConcurrentRememberedSetScanStart(MM_EnvironmentBase *env);
  void reportConcurrentRememberedSetScanEnd(MM_EnvironmentBase *env,
                                            uint64_t duration);
...
#if defined(OMR_GC_CONCURRENT_SWEEP)
  void concurrentSweep(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace,
                       MM_AllocateDescription *allocDescription);
...
protected:
   ...
   * @return the value of _conHelperRequest.
   */
  ConHelperRequest switchConHelperRequest(ConHelperRequest from,
                                          ConHelperRequest to);

  uintptr_t doConcurrentInitialization(MM_EnvironmentBase *env,
                                       uintptr_t initToDo);
  uintptr_t doConcurrentTrace(MM_EnvironmentBase *env,
                              MM_AllocateDescription *allocDescription,
                              uintptr_t sizeToTrace,
                              MM_MemorySubSpace *subspace, bool tlhAllocation);
  void signalThreadsToActivateWriteBarrier(MM_EnvironmentBase *env);
  void resumeConHelperThreads(MM_EnvironmentBase *env);
  uintptr_t calculateInitSize(MM_EnvironmentBase *env,
                              uintptr_t allocationSize);
  uintptr_t calculateTraceSize(MM_EnvironmentBase *env,
                               MM_AllocateDescription *allocDescription);
  void concurrentMark(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace,
                      MM_AllocateDescription *allocDescription);
...
public:
  ...
  static void signalThreadsToActivateWriteBarrierAsyncEventHandler(
  ...
  virtual void payAllocationTax(MM_EnvironmentBase *env,
                                ...
                                MM_AllocateDescription *allocDescription);
  bool concurrentFinalCollection(MM_EnvironmentBase *env,
                                 MM_MemorySubSpace *subspace);
   ...
   * but only do so once for every CONCURRENT_STOP_SAMPLE_GRAIN call to this
   * routine.
   * @return  TRUE if concurrent collection is in progress; FALSE otherwise
   */
  MMINLINE bool
  isExclusiveAccessRequestWaitingSparseSample(MM_EnvironmentBase *env,
                                              uintptr_t value) {
    /* Have we reached a sample point ? */
    if ((0 == (value & CONCURRENT_STOP_SAMPLE_GRAIN)) &&
        env->isExclusiveAccessRequestWaiting()) {
      return true;
    } else {
      return false;
    }
   ...
   */
  virtual bool isStwCollectionInProgress() { return _stwCollectionInProgress; }
  ...
  MM_ConcurrentGC(MM_EnvironmentBase *env)
      : MM_ParallelGlobalGC(env), _heapBase(NULL),
        _rebuildInitWorkForRemove(false)
#if defined(OMR_GC_LARGE_OBJECT_AREA)
        ,
        _meteringHistory(NULL), _currentMeteringHistory(0), _meteringType(SOA)
#endif /* OMR_GC_LARGE_OBJECT_AREA */
        ,
        _conHelpersTable(NULL),
        _conHelperThreads((uint32_t)_extensions->concurrentBackground),
        _conHelpersStarted(0), _conHelpersShutdownCount(0),
        _conHelpersActivationMonitor(NULL), _stwCollectionInProgress(false),
        _initializeMarkMap(false), _initWorkMonitor(NULL),
        _initWorkCompleteMonitor(NULL), _concurrentTuningMonitor(NULL),
        _initRanges(NULL), _numInitRanges(0), _numPhysicalInitRanges(0),
        _nextInitRange(0), _initializers(0), _initSetupDone(false),
        _bytesToCleanPass1(0), _bytesToCleanPass2(0), _bytesToTracePass1(0),
        ...
        _lastAverageAlloc2TraceRate(0), _maxAverageAlloc2TraceRate(0),
        _lastTotalTraced(0), _lastConHelperTraceSizeCount(0),
        _alloc2ConHelperTraceRate(0), _forcedKickoff(false),
        _languageKickoffReason(NO_LANGUAGE_KICKOFF_REASON),
        _conHelpersRequest(CONCURRENT_HELPER_WAIT), _concurrentCycleState(),
        _cardTable(NULL), _heapAlloc(NULL), _rebuildInitWorkForAdd(false),
        _retuneAfterHeapResize(false), _callback(NULL), _stats() {
