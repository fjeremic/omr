 ...
 *
 * PHASE1(-Xgc:cardCleaningPasses == 1|2) during the first phase we process all
 * card for heapBase thru to card for head of free list at start of card
 * cleaning
 *
 * PHASE2(-Xgc:cardCleaningPasses == 1|2) during phase 2 we process all
 * remaining cards, if any, in the card table not cleaned during phase1.
 *
 * PHASE3(-Xgc:cardCleaingPasses == 2) during the 3rd phase we re-clean all
 * cards in the card table
 *
 * @ingroup GC_Modron_Standard
 */
 ...
 * @warn All card table functions assume EXCLUSIVE ranges, ie they take a base
 * and top card where base card is first card in range and top card is card
 * immediately AFTER the end of the range.
 * @ingroup GC_Modron_Standard
 */
...
private:
  ...
  bool allocateCardTableEntriesForHeapRange(MM_EnvironmentBase *env,
                                            MM_MemorySubSpace *subspace,
                                            uintptr_t size, void *lowAddress,
                                            void *highAddress,
                                            bool clearNewCards);
  bool freeCardTableEntriesForHeapRange(MM_EnvironmentBase *env, uintptr_t size,
                                        void *lowAddress, void *highAddress,
                                        void *lowValidAddress,
                                        void *highValidAddress);
  bool allocateTLHMarkMapEntriesForHeapRange(MM_EnvironmentBase *env,
                                             MM_MemorySubSpace *subspace,
                                             uintptr_t size, void *lowAddress,
                                             void *highAddress);
  bool freeTLHMarkMapEntriesForHeapRange(MM_EnvironmentBase *env,
                                         uintptr_t size, void *lowAddress,
                                         void *highAddress,
                                         void *lowValidAddress,
                                         void *highValidAddress);

  void setTLHMarkBits(MM_EnvironmentBase *env, uintptr_t slotIndex,
                      uintptr_t slotBits);
  void clearTLHMarkBits(MM_EnvironmentBase *env, uintptr_t slotIndex,
                        uintptr_t slotBits);
  static uintptr_t calculateTLHMarkMapSize(MM_EnvironmentBase *env,
  ...
  void processTLHMarkBits(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace,
                          void *tlhBase, void *tlhTop, BitMapAction action);

protected:
  bool initialize(MM_EnvironmentBase *env, MM_Heap *heap);

  bool cleanSingleCard(MM_EnvironmentBase *env, Card *card,
                       uintptr_t bytesToClean, uintptr_t *totalBytesCleaned);
  Card *getNextDirtyCard(MM_EnvironmentBase *env, Card cardMask,
  ...
  virtual bool getExclusiveCardTableAccess(MM_EnvironmentBase *env,
  ...
  MMINLINE bool cardTableNeedsPreparing(CardCleanPhase currentPhase) {
    return ((currentPhase < _lastCardCleanPhase) && (currentPhase % 3 < 2))
               ? true
               : false;
  }

  bool cardTableBeingPrepared(CardCleanPhase currentPhase) {
    return (currentPhase % 3 == 1 ? true : false);
  };
  bool cardCleaningInProgress(CardCleanPhase currentPhase) {
  ...
  MMINLINE void incConcurrentCleanedCards(uintptr_t numCards,
                                          CardCleanPhase currentPhase) {
    switch (currentPhase) {
...
public:
   ...
   * @param[in] markingScheme The marking scheme which the card table will use
   * to find object starts for precise card area marking
   * @param[in] collector The collector with which we interact
   * @return The new isntance (NULL on failure)
   */
  static MM_ConcurrentCardTable *newInstance(MM_EnvironmentBase *env,
                                             MM_Heap *heap,
   ...
   * @param[in] env The thread which initiated the expansion request (typically
   ...
   * @param[in] cleanNewCards True if the new cards included in this range
   * should be cleared (true if the subspace is collectable)
   * @return true if expansion is successful
   */
  bool heapAddRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace,
                    uintptr_t size, void *lowAddress, void *highAddress,
                    bool cleaNewCards);
  /**
   * Called when a range of memory has been removed from the heap.
   * @param[in] env The thread which initiated the contraction request
   * (typically the master GC thread)
   * @param[in] subspace The expanding subspace
   ...
   * @param[in] highAddress The top address (non-inclusive) of the memory added
   * to the heap
   * @param[in] lowValidAddress The first valid address previous to the lowest
   * in the heap range being removed
   * @param[in] highValidAddress The first valid address following the highest
   * in the heap range being removed
   * @param[in] cleanNewCards True if the new cards included in this range
   * should be cleared (true if the subspace is collectable)
   * @return true if contraction is successful
   */
  bool heapRemoveRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace,
                       uintptr_t size, void *lowAddress, void *highAddress,
                       void *lowValidAddress, void *highValidAddress);
  /**
   * Called when the heap geometry has been reconfigured but no memory was added
   * or removed from the heap (happens during tilt).
   * @param[in] env The thread which caused the heap geometry change (typically
   * the master GC thread)
  ...
  /**
   * Determine how many bytes of card table correspond to the supplied heap
   * range. The current implementation will not count the heapTop card in this
   * size.
   *
   * @param[in] heapBase Base address of heap range
   * @param[in] heapTop Top (non-inclusive) address of heap range
   * @return The size, in bytes, of all the cards which map to the supplied heap
   * range
   * @note Only called by MM_ConcurrentCardTable and MM_ConcurrentGC
   */
  uintptr_t cardBytesForHeapRange(MM_EnvironmentBase *env, void *heapBase,
                                  void *heapTop);
  /**
   * Dirties the card backing the given range of the heap.  Will dirty the
   * heapBase and heapTop cards, inclusively.
   * @param[in] env The thread which caused the remembered set to resize
   * @param[in] heapBase The base address of the fragment of the heap excluded
   * from the remembered set
   * @param[in] heapTop The byte after the highest address of the heap excluded
   * from the remembered set
   * @note Called when the remembered set resizes to exclude a range of the heap
   */
  void dirtyCardsInRange(MM_EnvironmentBase *env, void *heapBase,
                         void *heapTop);
  /**
   * Called to clear the cards backing the heap regions which are in active but
   * not concurrently collectable subspaces. The purpose of this call is to
   * clear the cards backing active nursery heap ranges, under the gencon
   * policy.
   * @param[in] env The thread which reported the concurrent work stack overflow
   * @note Called only from MM_ConcurrentGC during concurrent work stack
   * overflow
   */
  void clearNonConcurrentCards(MM_EnvironmentBase *env);

  /**
   * Prepare for next concurrent marking cycle (atomically sets internal state
   * of the card table).
   ...
   * to trace represented by the input value "sizeToDo". Also, updates
   * "sizeDone" the actual size traced which may be less than sizeToDo if a GC
   * is requested whilst we are tracing or all cards cleaned.
   *
   * @param isMutator TRUE if called on a mutator thread, FALSE if called on a
   * background thread
   * @param sizeToDo the amount of objects to be traced
   * @param threadAtSafePoint Whether the calling thread is at a safe point or
   * not
   * @return FALSE if a GC occurs during during card table preperation.
   */
  bool cleanCards(MM_EnvironmentBase *env, bool isMutator, uintptr_t sizeToDo,
                  uintptr_t *sizeDone, bool threadAtSafePoint);
  /**
   ...
   *
   * @param bytesTraced  - reference to counter to pass back count of bytes
   * traced to caller
   *
   ...
   * otherwise
   */
  bool isObjectInDirtyCardNoCheck(MM_EnvironmentBase *env,
   ...
   */
  virtual bool isObjectInUncleanedDirtyCard(MM_EnvironmentBase *env,
                                            omrobjectptr_t object);
   ...
   *
   * Determine whether the referenced object is within an active (incomplete)
   * TLH.
   *
   ...
   * Check that all bits in the TLH mark bits map are OFF. All bits should be
   * OFF
   *
   * @return TRUE if TLH mark bits all off; FALSE otherwise
   */
  static void tlhCleared(J9HookInterface **hook, uintptr_t eventNum,
   ...
   */
  static void tlhRefreshed(J9HookInterface **hook, uintptr_t eventNum,
                           void *eventData, void *userData);
   ...
   */
  MM_ConcurrentCardTable(MM_EnvironmentBase *env,
                         MM_MarkingScheme *markingScheme,
                         MM_ConcurrentGC *collector)
      : MM_CardTable(), _tlhMarkMapMemoryHandle(), _tlhMarkBits(NULL),
        _cardTableReconfigured(false), _cleanAllCards(false),
        _omrVM(env->getOmrVM()), _collector(collector),
        _extensions(MM_GCExtensionsBase::getExtensions(_omrVM)),
        _dispatcher(_extensions->dispatcher), _markingScheme(markingScheme),
        _cardCleanPhase(UNINITIALIZED), _lastCardCleanPhase(UNINITIALIZED),
        _cleaningRanges(NULL), _currentCleaningRange(NULL),
        _lastCleaningRange(NULL), _maxCleaningRanges(0), _lastCard(NULL),
        _firstCardInPhase(NULL), _lastCardInPhase(NULL),
