...
bool
MM_ConcurrentGCIncrementalUpdate::heapAddRange(MM_EnvironmentBase* env,
                                               ...
                                               void* highAddress)
{
  ...
  bool result =
    MM_ConcurrentGC::heapAddRange(env, subspace, size, lowAddress, highAddress);

  if (result) {
    /* expand the card table */
    result = ((MM_ConcurrentCardTable*)_cardTable)
               ->heapAddRange(
                 env, subspace, size, lowAddress, highAddress, clearCards);
    if (!result) {
      /* Expansion of Concurrent Card Table has failed
       * ParallelGlobalGC expansion must be reversed
       */
      MM_ParallelGlobalGC::heapRemoveRange(
 ...
 */
void
MM_ConcurrentGCIncrementalUpdate::hookCardCleanPass2Start(
