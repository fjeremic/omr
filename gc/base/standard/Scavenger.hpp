...
private:
  ...
  volatile uintptr_t
    _waitingCount; /**< count of threads waiting  on scan cache queues (blocked
                      via _scanCacheMonitor); threads never wait on
                      _freeCacheMonitor */
  uintptr_t
  ...
  volatile bool
    _rescanThreadsForRememberedObjects; /**< Indicates that thread-referenced
                                           objects were tenured and threads must
                                           be rescanned */

  volatile uintptr_t
    _backOutDoneIndex; /**< snapshot of _doneIndex, when backOut was detected */
...
public:
   ...
   */
  MMINLINE void deepScan(MM_EnvironmentStandard* env,
                         omrobjectptr_t objectPtr,
                         ...
                         uintptr_t priorityFieldOffset2)
  {
    ...
    if (shouldStartDeepScan(env, objectPtr)) {
      deepScanOutline(
        env, objectPtr, priorityFieldOffset1, priorityFieldOffset2);
    }
  }
  ...
  void deepScanOutline(MM_EnvironmentStandard* env,
                       omrobjectptr_t objectPtr,
                       uintptr_t priorityFieldOffset1,
                       uintptr_t priorityFieldOffset2);
  ...
  MMINLINE bool isWorkAvailableInCacheWithCheck(MM_CopyScanCacheStandard* cache)
  {
   ...
   */
  MMINLINE void reinitCache(MM_CopyScanCacheStandard* cache,
                            void* base,
                            void* top);
  ...
  virtual void masterThreadGarbageCollect(
    ...
    MM_AllocateDescription* allocDescription,
    bool initMarkMap = false,
    bool rebuildMarkBits = false);
