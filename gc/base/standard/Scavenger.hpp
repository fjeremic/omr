...
private:
  ...
  const uintptr_t
      _objectAlignmentInBytes; /**< Run-time objects alignment in bytes */
  bool _isRememberedSetInOverflowAtTheBeginning; /**< Cached RS Overflow flag at
  ...
  volatile uintptr_t _doneIndex; /**< sequence ID of completeScan loop, which we
                                    may have a few during one GC cycle */
  ...
  void *_evacuateSpaceBase,
      *_evacuateSpaceTop; /**< cached base and top heap pointers within evacuate
                             subspace */
  void *_survivorSpaceBase,
      *_survivorSpaceTop; /**< cached base and top heap pointers within survivor
                             subspace */
  ...
  MM_CycleState _cycleState; /**< Embedded cycle state to be used as the master
                                cycle state for GC activity */
  MM_CollectionStatisticsStandard
      _collectionStatistics; /** Common collect stats (memory, time etc.) */

  MM_CopyScanCacheList
      _scavengeCacheFreeList; /**< pool of unused copy-scan caches */
  MM_CopyScanCacheList _scavengeCacheScanList; /**< scan lists */
  volatile uintptr_t
      _cachedEntryCount; /**< non-empty scanCacheList count (not the total count
                            of caches in the lists) */
  uintptr_t _cachesPerThread; /**< maximum number of copy and scan caches
                                 required per thread at any one time */
  omrthread_monitor_t
      _scanCacheMonitor; /**< monitor to synchronize threads on scan lists */
  omrthread_monitor_t
      _freeCacheMonitor; /**< monitor to synchronize threads on free list */
  uintptr_t _waitingCountAliasThreshold; /**< Only alias a copy cache IF the
                                            number of threads waiting hasn't
                                            reached the threshold*/
  volatile uintptr_t
      _waitingCount; /**< count of threads waiting  on scan cache queues
                        (blocked via _scanCacheMonitor); threads never wait on
                        _freeCacheMonitor */
  uintptr_t
      _cacheLineAlignment; /**< The number of bytes per cache line which is used
                              to determine which boundaries in memory represent
                              the beginning of a cache line */
  volatile bool
      _rescanThreadsForRememberedObjects; /**< Indicates that thread-referenced
                                             objects were tenured and threads
                                             must be rescanned */

  volatile uintptr_t _backOutDoneIndex; /**< snapshot of _doneIndex, when
                                           backOut was detected */
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  MM_MasterGCThread _masterGCThread; /**< An object which manages the state of
                                        the master GC thread */
  ...
  uint64_t _concurrentScavengerSwitchCount; /**< global counter of cycle start
                                               and cycle end transitions */
  volatile bool _shouldYield; /**< Set by the first GC thread that observes that
                                 a criteria for yielding is met. Reset only when
                                 the concurrent phase is finished. */
...
private:
  /**
   * Flush the threads reference and remembered set caches before waiting in
   * getNextScanCache. This removes the requirement of a synchronization point
   * after calls to completeScan when it is followed by reference or remembered
   * set processing.
  ...
  MMINLINE bool isBackOutFlagRaised() {
    return _extensions->isScavengerBackOutFlagRaised();
  }

  /**
   * Check if concurrent phase of the cycle should yield to an external
   * activity. If so, set the flag so that other GC threads react appropriately
   */
  ...
  MMINLINE bool shouldAbortScanLoop(MM_EnvironmentStandard *env) {
    ...
    if (IS_CONCURRENT_ENABLED) {
      /* Concurrent Scavenger needs to drain the scan queue in last scan loop
       * before aborted handling starts. It is however fine to leave it
       * populated, if we want to yield in a middle of concurrent phase which
       * aborted, since there will be at least one scan loop afterwards in
       * complete phase that will drain it. Bottom line, we don't care about
       * isBackOutFlagRaised when deciding whether to yield.
       */

      shouldAbort = _shouldYield;
      if (shouldAbort) {
        Assert_MM_true(concurrent_state_scan == _concurrentState);
        /* Since we are aborting the scan loop without synchornizing with other
         * GC threads (before which we flush buffers), we have to do it now.
         * There should be no danger in not synchonizing with other threads,
         * since we can only abort/yield in main scan loop and not during
         * clearable STW phase, where is a potential danger of entering a scan
         * loop without ensuring all threads flushed buffers from previous scan
         * loop.
         */
...
public:
   ...
   */
  static void hookGlobalCollectionStart(J9HookInterface **hook,
   ...
   */
  static void hookGlobalCollectionComplete(J9HookInterface **hook,
                                           uintptr_t eventNum, void *eventData,
                                           void *userData);
   ...
   */
  MMINLINE GC_ObjectScanner *getObjectScanner(MM_EnvironmentStandard *env,
                                              omrobjectptr_t objectptr,
                                              void *objectScannerState,
                                              uintptr_t flags);

  uintptr_t calculateCopyScanCacheSizeForWaitingThreads(
      uintptr_t maxCacheSize, uintptr_t threadCount, uintptr_t waitingThreads);
  uintptr_t calculateCopyScanCacheSizeForQueueLength(uintptr_t maxCacheSize,
                                                     uintptr_t threadCount,
                                                     uintptr_t scanCacheCount);
  MMINLINE uintptr_t
  calculateOptimumCopyScanCacheSize(MM_EnvironmentStandard *env);
  MMINLINE MM_CopyScanCacheStandard *
  reserveMemoryForAllocateInSemiSpace(MM_EnvironmentStandard *env,
                                      omrobjectptr_t objectToEvacuate,
                                      uintptr_t objectReserveSizeInBytes);
  MM_CopyScanCacheStandard *
  reserveMemoryForAllocateInTenureSpace(MM_EnvironmentStandard *env,
                                        omrobjectptr_t objectToEvacuate,
                                        uintptr_t objectReserveSizeInBytes);
   ...
   */
  MMINLINE bool copyAndForward(MM_EnvironmentStandard *env,
                               GC_SlotObject *slotObject);

  MMINLINE bool copyAndForward(MM_EnvironmentStandard *env,
                               volatile omrobjectptr_t *objectPtrIndirect);

  MMINLINE omrobjectptr_t copy(MM_EnvironmentStandard *env,
                               MM_ForwardedHeader *forwardedHeader);

  MMINLINE void updateCopyScanCounts(MM_EnvironmentBase *env,
                                     uint64_t slotsScanned,
                                     uint64_t slotsCopied);
  bool splitIndexableObjectScanner(MM_EnvironmentStandard *env,
   ...
   */
  MMINLINE bool scavengeObjectSlots(MM_EnvironmentStandard *env,
                                    MM_CopyScanCacheStandard *scanCache,
                                    omrobjectptr_t objectPtr, uintptr_t flags,
                                    omrobjectptr_t *rememberedSetSlot);
  MMINLINE MM_CopyScanCacheStandard *
  incrementalScavengeObjectSlots(MM_EnvironmentStandard *env,
                                 omrobjectptr_t objectPtr,
                                 MM_CopyScanCacheStandard *scanCache);

  /**
   * For fast traversal of deep structure nodes - scan objects with self
   * referencing fields with priority Split into two functions deepScan and
   * deepScanOutline. Frequently called checks (see lazy start check) must be
   * inlined
   * @param env The environment.
   * @param objectPtr The pointer to the object.
   * @param priorityFieldOffset1 Offset to the first priority field of the
   * object
   * @param priorityFieldOffset2 Offset to the second priority field, if it
   * can't follow through in one direction, it will attempt to use the second
   * self referencing field
   */
  MMINLINE void deepScan(MM_EnvironmentStandard *env, omrobjectptr_t objectPtr,
                         uintptr_t priorityFieldOffset1,
                         uintptr_t priorityFieldOffset2) {
    /**
     * Inhibit the special treatment routine with relatively high probability to
     * skip over most false positives (shorter lists), while only marginally
     * delay detection of very deep structures.
     */
    if (shouldStartDeepScan(env, objectPtr)) {
      deepScanOutline(env, objectPtr, priorityFieldOffset1,
                      priorityFieldOffset2);
    }
  }
   ...
   */
  MMINLINE bool shouldStartDeepScan(MM_EnvironmentStandard *env,
                                    omrobjectptr_t objectPtr) {
    /* Check last few LSB of the object address for probability 1/16 */
    return (0 == ((uintptr_t)objectPtr & 0x78));
  }

  void deepScanOutline(MM_EnvironmentStandard *env, omrobjectptr_t objectPtr,
                       uintptr_t priorityFieldOffset1,
                       uintptr_t priorityFieldOffset2);

  MMINLINE bool scavengeRememberedObject(MM_EnvironmentStandard *env,
                                         omrobjectptr_t objectPtr);
   ...
   */
  bool shouldRememberObject(MM_EnvironmentStandard *env,
                            omrobjectptr_t objectPtr);
  ...
  void
  backoutFixupAndReverseForwardPointersInSurvivor(MM_EnvironmentStandard *env);
   ...
   *
   * @return true if Global GC was executed, false if concurrent kickoff forced
   * or Global GC is not possible
   */
  bool percolateGarbageCollect(MM_EnvironmentBase *env,
  ...
  MMINLINE void clearFailedTenureThresholdFlag() {
    _failedTenureThresholdReached = false;
  };
  MMINLINE void setFailedTenureThresholdFlag() {
    _failedTenureThresholdReached = true;
  };
  MMINLINE void setFailedTenureLargestObject(uintptr_t size) {
    _failedTenureLargestObject = size;
  };
  MMINLINE uintptr_t getFailedTenureLargestObject() {
    return _failedTenureLargestObject;
  };
  MMINLINE bool failedTenureThresholdReached() {
  ...
  MMINLINE MM_CopyScanCacheStandard *
  aliasToCopyCache(MM_EnvironmentStandard *env, GC_SlotObject *scannedSlot,
                   MM_CopyScanCacheStandard *scanCache,
                   MM_CopyScanCacheStandard *copyCache);
  MMINLINE uintptr_t scanCacheDistanceMetric(MM_CopyScanCacheStandard *cache,
                                             GC_SlotObject *scanSlot);
  MMINLINE uintptr_t copyCacheDistanceMetric(MM_CopyScanCacheStandard *cache);

  MMINLINE MM_CopyScanCacheStandard *
  getNextScanCacheFromList(MM_EnvironmentStandard *env);
  void addCopyCachesToFreeList(MM_EnvironmentStandard *env);
  MMINLINE void
  addCacheEntryToScanListAndNotify(MM_EnvironmentStandard *env,
                                   MM_CopyScanCacheStandard *newCacheEntry);

  MMINLINE bool isWorkAvailableInCache(MM_CopyScanCacheStandard *cache) {
   ...
   */
  MMINLINE void reinitCache(MM_CopyScanCacheStandard *cache, void *base,
                            void *top);

  /**
   * An attempt to get a preallocated scan cache header, free list will be
   * locked
   * @param env - current thread environment
   ...
   * @param env - current thread environment
   * @return pointer to allocated chunk of scan cache headers or NULL if attempt
   * fail
   */
   ...
   * To be reused cache should be has not been scanned and has no scan work to
   * do
   * @param env - current thread environment
   * @param cache cache to be flushed
   * @return true is cache can be reused
   */
  bool canLocalCacheBeReused(MM_EnvironmentStandard *env,
                             MM_CopyScanCacheStandard *cache);

  /**
   * Release local Copy cache
   * Put cache to scanned list if it has not been scanned and has scan work to
   * do
   * @param env - current thread environment
   * @param cache cache to be flushed
   ...
   */
  MM_CopyScanCacheStandard *
  releaseLocalCopyCache(MM_EnvironmentStandard *env,
                        MM_CopyScanCacheStandard *cache);
   ...
   * @param env - current thread environment
   * @param cache cache to be flushed
   * @return true if sufficent amount of memory is left in the cache to create a
   * 'remainder' for later usage
   */
  bool clearCache(MM_EnvironmentStandard *env, MM_CopyScanCacheStandard *cache);

  /**
   * Called (typically at the end of GC) to explicitly abandon the TLH
   * remainders (for the calling thread)
   */
  void abandonSurvivorTLHRemainder(MM_EnvironmentStandard *env);
  void abandonTenureTLHRemainder(MM_EnvironmentStandard *env,
                                 bool preserveRemainders = false);
   ...
   */
  void addAllRememberedObjectsToOverflow(MM_EnvironmentStandard *env,
  ...
  /**
   * Determine if the specified remembered object was referenced by a thread or
   * stack.
   *
   * @param env[in] the current thread
   ...
   */
  bool isRememberedThreadReference(MM_EnvironmentStandard *env,
                                   omrobjectptr_t objectPtr);
   ...
   * @return true if the object is a remembered thread reference
   */
  bool processRememberedThreadReference(MM_EnvironmentStandard *env,
                                        omrobjectptr_t objectPtr);
  ...
  /**
   * Merge thread local stats for current phase/increment in to global current
   * increment stats
   */
   ...
   */
  void mergeGCStatsBase(MM_EnvironmentBase *env,
   ...
   * @param minimumSurvivalRate The minimum survival rate required to consider
   * tenuring.
  ...
  void setAliasThreshold(uintptr_t waitingCountAliasThreshold) {
    _waitingCountAliasThreshold = waitingCountAliasThreshold;
  }
...
protected:
  ...
  virtual bool internalGarbageCollect(MM_EnvironmentBase *env,
                                      MM_MemorySubSpace *subSpace,
                                      MM_AllocateDescription *allocDescription);
  virtual void internalPreCollect(MM_EnvironmentBase *env,
                                  MM_MemorySubSpace *subSpace,
                                  MM_AllocateDescription *allocDescription,
                                  uint32_t gcCode);
  virtual void internalPostCollect(MM_EnvironmentBase *env,
                                   MM_MemorySubSpace *subSpace);
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  /**
   * Perform partial initialization if Garbage Collection is called earlier then
   * GC Master Thread is activated
   * @param env Master GC thread.
   */
  virtual MM_ConcurrentPhaseStatsBase *getConcurrentPhaseStats() {
    return &_concurrentPhaseStats;
  }
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
...
public:
  ...
  virtual void
  preConcurrentInitializeStatsAndReport(MM_EnvironmentBase *env,
                                        MM_ConcurrentPhaseStatsBase *stats);
  virtual uintptr_t masterThreadConcurrentCollect(MM_EnvironmentBase *env);
  virtual void
  postConcurrentUpdateStatsAndReport(MM_EnvironmentBase *env,
                                     MM_ConcurrentPhaseStatsBase *stats,
  ...
  /**
   * All open copy caches (even if not full) are pushed onto scan queue. Unused
   * memory is abondoned.
   * @param env Invoking thread, for which copy caches are to be released. Could
   * be either GC or mutator thread.
   * @param final If true (typically at the end of a cycle), abandon TLH
   * remainders, too. Otherwise keep them for possible future copy cache
   * refresh.
   */
   ...
   */
  void triggerConcurrentScavengerTransition(
  ...
  bool isMutatorThreadInSyncWithCycle(MM_EnvironmentBase *env) {
    return (env->_concurrentScavengerSwitchCount ==
            _concurrentScavengerSwitchCount);
  }

  /**
   * Enabled/disable approriate thread local resources when starting or
   * finishing Concurrent Scavenger Cycle
   */
   ...
   * @return Boolean indicating if the object pointer is within the heap
   * boundaries.
   */
  MMINLINE bool isHeapObject(omrobjectptr_t objectPtr) {
    return ((_heapBase <= (uint8_t *)objectPtr) &&
            (_heapTop > (uint8_t *)objectPtr));
  }

  MMINLINE bool isObjectInNewSpace(omrobjectptr_t objectPtr) {
  ...
  MMINLINE bool isObjectInEvacuateMemory(omrobjectptr_t objectPtr) {
     ...
     * ranges */
    return ((void *)objectPtr >= _evacuateSpaceBase) &&
           ((void *)objectPtr < _evacuateSpaceTop);
  }

  MMINLINE void *getEvacuateBase() { return _evacuateSpaceBase; }

  MMINLINE void *getEvacuateTop() { return _evacuateSpaceTop; }

  MMINLINE void *getSurvivorBase() { return _survivorSpaceBase; }

  MMINLINE void *getSurvivorTop() { return _survivorSpaceTop; }
  ...
  /**
   * Attempt to add the specified object to the current thread's remembered set
   * fragment. Grow the remembered set if necessary and, if that fails,
   * overflow. The object must already have its remembered bits set.
   *
   * @param env[in] the current thread
   * @param objectPtr[in] the object to remember
   */
  void addToRememberedSetFragment(MM_EnvironmentStandard *env,
                                  omrobjectptr_t objectPtr);

  /**
   * Provide public (out-of-line) access to private (inline) copyAndForward(),
   * copy() for client language runtime. Slot holding reference will be updated
   * with new address for referent on return.
   * @param[in] env Environment pointer for calling thread
   * @param[in/out] slotPtr Pointer to slot holding reference to object to be
   * copied and forwarded
   */
  bool copyObjectSlot(MM_EnvironmentStandard *env,
                      volatile omrobjectptr_t *slotPtr);
  bool copyObjectSlot(MM_EnvironmentStandard *env, GC_SlotObject *slotObject);
  omrobjectptr_t copyObject(MM_EnvironmentStandard *env,
                            MM_ForwardedHeader *forwardedHeader);
   ...
   * tenure) the object and install a forwarding pointer at the new location.
   * The object may have already been copied. In either case, update the slot to
   * point at the new location of the object.
   *
   * @param env[in] the current thread
   * @param objectPtrIndirect[in/out] the thread or stack slot to be scavenged
   */
  void copyAndForwardThreadSlot(MM_EnvironmentStandard *env,
                                omrobjectptr_t *objectPtrIndirect);

  /**
   * This function is called at the end of scavenging if any stack- (or thread-)
   * referenced objects were tenured during the scavenge. It is called by the
   * RootScanner on each thread or stack slot.
   *
   * @param env[in] the current thread
   * @param objectPtrIndirect[in] the slot to process
   */
  void rescanThreadSlot(MM_EnvironmentStandard *env,
                        omrobjectptr_t *objectPtrIndirect);

  uintptr_t getArraySplitAmount(MM_EnvironmentStandard *env,
  ...
  virtual void *createSweepPoolState(MM_EnvironmentBase *env,
                                     MM_MemoryPool *memoryPool);
  virtual void deleteSweepPoolState(MM_EnvironmentBase *env,
                                    void *sweepPoolState);
  ...
  virtual void collectorExpanded(MM_EnvironmentBase *env,
                                 MM_MemorySubSpace *subSpace,
                                 uintptr_t expandSize);
  virtual bool canCollectorExpand(MM_EnvironmentBase *env,
                                  MM_MemorySubSpace *subSpace,
                                  uintptr_t expandSize);
  ...
  MM_Scavenger(MM_EnvironmentBase *env, MM_HeapRegionManager *regionManager)
      : MM_Collector(), _delegate(env),
        _objectAlignmentInBytes(env->getObjectAlignmentInBytes()),
        _isRememberedSetInOverflowAtTheBeginning(false),
        _extensions(env->getExtensions()), _dispatcher(_extensions->dispatcher),
        _doneIndex(0), _activeSubSpace(NULL), _evacuateMemorySubSpace(NULL),
        _survivorMemorySubSpace(NULL), _tenureMemorySubSpace(NULL),
        _evacuateSpaceBase(NULL), _evacuateSpaceTop(NULL),
        _survivorSpaceBase(NULL), _survivorSpaceTop(NULL), _tenureMask(0),
        _expandFailed(false), _failedTenureThresholdReached(false),
        _countSinceForcingGlobalGC(0), _expandTenureOnFailedAllocate(true),
        _minTenureFailureSize(UDATA_MAX), _minSemiSpaceFailureSize(UDATA_MAX),
        _cycleState(), _collectionStatistics(), _cachedEntryCount(0),
        _cachesPerThread(0), _scanCacheMonitor(NULL), _freeCacheMonitor(NULL),
        _waitingCountAliasThreshold(0), _waitingCount(0), _cacheLineAlignment(0)
#if !defined(OMR_GC_CONCURRENT_SCAVENGER)
        ,
        _rescanThreadsForRememberedObjects(false)
#endif
        ,
        _backOutDoneIndex(0), _heapBase(NULL), _heapTop(NULL),
        _regionManager(regionManager)
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
        ,
        _masterGCThread(env), _concurrentState(concurrent_state_idle),
        _concurrentScavengerSwitchCount(0), _shouldYield(false)
#endif /* #if defined(OMR_GC_CONCURRENT_SCAVENGER) */
