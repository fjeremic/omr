...
private:
    ...
    uintptr_t _waitingCountAliasThreshold; /**< Only alias a copy cache IF the number of threads waiting hasn't reached
                                              the threshold*/
    volatile uintptr_t _waitingCount; /**< count of threads waiting  on scan cache queues (blocked via
                                         _scanCacheMonitor); threads never wait on _freeCacheMonitor */
    uintptr_t _cacheLineAlignment; /**< The number of bytes per cache line which is used to determine which boundaries
                                      in memory represent the beginning of a cache line */
    volatile bool _rescanThreadsForRememberedObjects; /**< Indicates that thread-referenced objects were tenured and
                                                         threads must be rescanned */
    ...
    volatile bool _shouldYield; /**< Set by the first GC thread that observes that a criteria for yielding is met. Reset
                                   only when the concurrent phase is finished. */
...
private:
    ...
    /**
     * Check if concurrent phase of the cycle should yield to an external activity. If so, set the flag so that other GC
     * threads react appropriately
     */
    ...
    MMINLINE bool shouldAbortScanLoop(MM_EnvironmentStandard* env)
    {
        ...
        if (IS_CONCURRENT_ENABLED) {
             ...
             * It is however fine to leave it populated, if we want to yield in a middle of concurrent phase which
             * aborted, since there will be at least one scan loop afterwards in complete phase that will drain it.
             * Bottom line, we don't care about isBackOutFlagRaised when deciding whether to yield.
             */

            shouldAbort = _shouldYield;
            if (shouldAbort) {
                Assert_MM_true(concurrent_state_scan == _concurrentState);
                /* Since we are aborting the scan loop without synchornizing with other GC threads (before which we
                 * flush buffers), we have to do it now. There should be no danger in not synchonizing with other
                 * threads, since we can only abort/yield in main scan loop and not during clearable STW phase, where is
                 * a potential danger of entering a scan loop without ensuring all threads flushed buffers from previous
                 * scan loop.
                 */
...
public:
     ...
     */
    static void hookGlobalCollectionComplete(
    ...
    uintptr_t calculateCopyScanCacheSizeForQueueLength(
     ...
     * Split into two functions deepScan and deepScanOutline. Frequently called checks (see lazy start check) must be
     * inlined
     * @param env The environment.
     * @param objectPtr The pointer to the object.
     ...
     */
    MMINLINE void deepScan(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr, uintptr_t priorityFieldOffset1,
     ...
     */
    MMINLINE bool shouldStartDeepScan(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
    {
        /* Check last few LSB of the object address for probability 1/16 */
        return (0 == ((uintptr_t)objectPtr & 0x78));
    }

    void deepScanOutline(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr, uintptr_t priorityFieldOffset1,
        uintptr_t priorityFieldOffset2);
    ...
    MMINLINE uintptr_t isTiltedScavenge() { return _extensions->tiltedScavenge ? 1 : 0; }
...
public:
     ...
     * @param final If true (typically at the end of a cycle), abandon TLH remainders, too. Otherwise keep them for
     * possible future copy cache refresh.
     */
    ...
    bool isConcurrentInProgress() { return concurrent_state_idle != _concurrentState; }
