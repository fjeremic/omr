...
public:
    MM_GCRememberedSet
        _rememberedSetStruct; /**< The VM-readable struct containing the remembered set "global" indexes. */
protected:
private:
...
public:
    ...
    void initializeFragment(
        MM_EnvironmentBase* env, MM_GCRememberedSetFragment* fragment); /* "Nulls" out a fragment. */
    void storeInFragment(MM_EnvironmentBase* env, MM_GCRememberedSetFragment* fragment,
        UDATA* value); /* This guarantees the store will occur, but a new fragment may be fetched. */
    bool isFragmentValid(MM_EnvironmentBase* env, const MM_GCRememberedSetFragment* fragment);
    void preserveLocalFragmentIndex(MM_EnvironmentBase* env,
        MM_GCRememberedSetFragment* fragment); /* Called by the code that enables the double-barrier. */
    void restoreLocalFragmentIndex(MM_EnvironmentBase* env,
        MM_GCRememberedSetFragment* fragment); /* Called by the root scanner to disable the double-barrier. */
    ...
    void flushFragments(
        MM_EnvironmentBase* env); /* Ensures all fragments will be seen as invalid next time they are accessed. */
...
private:
    void setGlobalIndex(
        MM_EnvironmentBase* env, UDATA indexValue); /* Increments the appropriate global index (global or preserved). */
};
