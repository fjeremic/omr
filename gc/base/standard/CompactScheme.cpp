 ...
 */
MM_CompactScheme* MM_CompactScheme::newInstance(MM_EnvironmentBase* env, MM_MarkingScheme* markingScheme)
{
    MM_CompactScheme* compactScheme;

    compactScheme = (MM_CompactScheme*)env->getForge()->allocate(
        sizeof(MM_CompactScheme), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (compactScheme) {
...
public:
    ...
    omrobjectptr_t getAddr() { return ((_addr & 3) != 3) ? 0 : (omrobjectptr_t)(_addr & ~3); }
    ...
    intptr_t getObjectOrdinal(intptr_t offset) const { return countBits(_bits & makeMask(offset)); }
...
void MM_CompactScheme::setFreeChunkSize(omrobjectptr_t deadObject, uintptr_t deadObjectSize)
{
    if (deadObjectSize == 0) {
        return;
    }
...
void MM_CompactScheme::workerSetupForGC(MM_EnvironmentStandard* env, bool singleThreaded)
{
 ...
 */
void MM_CompactScheme::createSubAreaTable(MM_EnvironmentStandard* env, bool singleThreaded)
{
...
void MM_CompactScheme::compact(MM_EnvironmentBase* envBase, bool rebuildMarkBits, bool aggressive)
{
...
void MM_CompactScheme::flushPool(MM_EnvironmentStandard* env, MM_CompactMemoryPoolState* poolState)
{
    MM_MemoryPool* memoryPool = poolState->_memoryPool;

    if (poolState->_freeListHead) {
        memoryPool->addFreeEntries(
            env, poolState->_freeListHead, poolState->_previousFreeEntry, poolState->_freeHoles, poolState->_freeBytes);
    }
 ...
 *
 */
MMINLINE void MM_CompactScheme::addFreeEntry(MM_EnvironmentStandard* env, MM_MemorySubSpace* memorySubSpace,
    MM_CompactMemoryPoolState* poolState, void* currentFreeBase, uintptr_t currentFreeSize)
{
    ...
    if (lowChunkSize > lowPool->getMinimumFreeEntrySize()) {
        ...
        lowPool->createFreeEntry(
            env, currentFreeBase, (uint8_t*)currentFreeBase + lowChunkSize, poolState->_previousFreeEntry, NULL);
...
void MM_CompactScheme::moveObjects(
    MM_EnvironmentStandard* env, uintptr_t& objectCount, uintptr_t& byteCount, uintptr_t& skippedObjectCount)
{
    MM_HeapRegionManager* regionManager = _heap->getHeapRegionManager();
    GC_HeapRegionIteratorStandard regionIterator(regionManager);
    MM_HeapRegionDescriptorStandard* region = NULL;
 ...
 */
size_t MM_CompactScheme::setFreeChunkPageAligned(omrobjectptr_t from, omrobjectptr_t to)
{
    assume0(from && to);
...
void MM_CompactScheme::evacuateSubArea(MM_EnvironmentStandard* env, MM_HeapRegionDescriptorStandard* subAreaRegion,
    SubAreaEntry* subAreaTableEvacuate, intptr_t i, uintptr_t& objectCount, uintptr_t& byteCount,
    uintptr_t& skippedObjectCount)
{
    ...
    do {
            ...
            if ((subAreaTableEvacuate[j].state == SubAreaEntry::ready)
                && (SubAreaEntry::ready
                    == MM_AtomicOperations::lockCompareExchange(
                        &subAreaTableEvacuate[j].state, SubAreaEntry::ready, SubAreaEntry::busy))) {
        ...
        Trc_MM_CompactScheme_evacuateSubArea_evacuated(
            env->getLanguageVMThread(), firstObject, endObject, nbytes, subAreaTableEvacuate[j].firstObject);
        ...
        if (size < minFreeChunk) {
            ...
            uintptr_t state = MM_AtomicOperations::lockCompareExchange(
                &subAreaTableEvacuate[j].state, SubAreaEntry::busy, SubAreaEntry::ready);
            Assert_MM_true(state == SubAreaEntry::busy);
        }
    } while (objectPtr);
 ...
 */
MMINLINE void MM_CompactScheme::saveForwardingPtr(
    CompactTableEntry& entry, omrobjectptr_t objectPtr, omrobjectptr_t forwardingPtr, intptr_t& page, intptr_t& counter)
{
 ...
 */
omrobjectptr_t MM_CompactScheme::doCompact(MM_EnvironmentStandard* env, MM_MemorySubSpace* memorySubSpace,
    omrobjectptr_t start, omrobjectptr_t finish, omrobjectptr_t& deadObject, uintptr_t& nobjects, uintptr_t& nbytes,
    bool evacuate)
{
    /* we use the MM_HeapMapWordIterator in this function so ensure that it is safe (can't be tested during init since
     * assertions don't work at that point) */
    ...
    MM_HeapMapIterator markedObjectIterator(
        _extensions, _markMap, (uintptr_t*)start, (uintptr_t*)pageStart(pageIndex(finish)));
...
omrobjectptr_t MM_CompactScheme::getForwardingPtr(omrobjectptr_t objectPtr) const
{
...
void MM_CompactScheme::fixupObjects(MM_EnvironmentStandard* env, uintptr_t& objectCount)
{
    MM_HeapRegionManager* regionManager = _heap->getHeapRegionManager();
    GC_HeapRegionIteratorStandard regionIterator(regionManager);
    MM_HeapRegionDescriptorStandard* region = NULL;
...
void MM_CompactScheme::fixupSubArea(MM_EnvironmentStandard* env, omrobjectptr_t firstObject, omrobjectptr_t finish,
...
void MM_CompactScheme::rebuildMarkbitsInSubArea(
    MM_EnvironmentStandard* env, MM_HeapRegionDescriptorStandard* region, SubAreaEntry* subAreaTable, intptr_t i)
{
...
bool MM_CompactScheme::changeSubAreaAction(MM_EnvironmentBase* env, SubAreaEntry* entry, uintptr_t newAction)
{
    ...
    if (previousAction != newAction) {
        ...
        if (action == previousAction) {
            ...
            /* during this phase it's only legitimate to change to newAction so if currentAction changed underneath us
             * that had better be its new value */
