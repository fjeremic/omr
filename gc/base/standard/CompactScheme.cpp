 ...
 */
MM_CompactScheme *MM_CompactScheme::newInstance(
    MM_EnvironmentBase *env, MM_MarkingScheme *markingScheme) {
  MM_CompactScheme *compactScheme;

  compactScheme = (MM_CompactScheme *)env->getForge()->allocate(
      sizeof(MM_CompactScheme), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (compactScheme) {
...
MMINLINE uintptr_t makeMask(intptr_t maskSize) {
  return ((uintptr_t)1 << maskSize) - 1;
}

MMINLINE intptr_t countBits(uintptr_t x) {
  intptr_t count = 0;
  while (x) {
    count++;
    x &= x - 1;
...
enum {
  ...
  /* Invalid value is used when, because of object growth due to hash, the hint
   * exceeds or is equal to maxValue. The hint would then be invalid.
   */
...
class CompactTableEntry {
 ...
 private:
  MMINLINE uintptr_t getHintShiftCount(intptr_t index) const {
 ...
 public:
  ...
  void setHint(intptr_t index, intptr_t value) {
    ...
    if (value >= maxValue) {
      /* Adding a hash to an object may grow the object. It is possible that
       * because of this growth, value may be greater than or equal to maxValue.
       * If this happens, we want to set this hint as invalid.
       */
      value = invalidValue;
    }
   ...
   * chunk, in UDATAs
   */
  intptr_t getHint(intptr_t index) const {
    Assert_MM_true((index >= 0) && (index < maxHints));
...
bool MM_CompactScheme::initialize(MM_EnvironmentBase *env) {
  return _delegate.initialize(env, _omrVM, _markMap, this);
}

void MM_CompactScheme::tearDown(MM_EnvironmentBase *env) {
 ...
 */
void MM_CompactScheme::kill(MM_EnvironmentBase *env) {
  tearDown(env);
  env->getForge()->free(this);
}

size_t MM_CompactScheme::getFreeChunkSize(omrobjectptr_t freeChunk) {
  if (freeChunk == 0) {
    return 0;
  }
  /* This duplicates code in
   * GC_ObjectHeapIteratorAddressOrderedList::nextObject() */
  ...
  if (_extensions->objectModel.isSingleSlotDeadObject(freeChunk)) {
    return _extensions->objectModel.getSizeInBytesSingleSlotDeadObject(
        freeChunk);
  }
  return _extensions->objectModel.getSizeInBytesMultiSlotDeadObject(freeChunk);
}

void MM_CompactScheme::setFreeChunkSize(omrobjectptr_t deadObject,
                                        uintptr_t deadObjectSize) {
  if (deadObjectSize == 0) {
    return;
  }
...
size_t MM_CompactScheme::setFreeChunk(omrobjectptr_t from, omrobjectptr_t to) {
  size_t size = (size_t)to - (size_t)from;
  setFreeChunkSize(from, size);
  return size;
}

void MM_CompactScheme::workerSetupForGC(MM_EnvironmentStandard *env,
                                        bool singleThreaded) {
...
void MM_CompactScheme::masterSetupForGC(MM_EnvironmentStandard *env) {
  _heap = _extensions->heap;
  _rootManager = _heap->getHeapRegionManager();
  _heapBase = (uintptr_t)_heap->getHeapBase();
  _compactTable =
      (CompactTableEntry *)_markingScheme->getMarkMap()->getMarkBits();
  _subAreaTable =
      (SubAreaEntry *)
          _extensions->sweepHeapSectioning->getBackingStoreAddress();
  _subAreaTableSize = _extensions->sweepHeapSectioning->getBackingStoreSize();
  _delegate.masterSetupForGC(env);
}

omrobjectptr_t MM_CompactScheme::freeChunkEnd(omrobjectptr_t chunk) {
  if (!chunk) {
    return NULL;
  }
 ...
 */
void MM_CompactScheme::createSubAreaTable(MM_EnvironmentStandard *env,
                                          bool singleThreaded) {
  ...
  if (max_subarea_num > necessary_subareas) {
    min_subarea_size = _heap->getMaximumPhysicalRange() /
                       (max_subarea_num - necessary_subareas);
  } else {
    min_subarea_size = _heap->getMaximumPhysicalRange();
  }
  uintptr_t size = (DESIRED_SUBAREA_SIZE >= min_subarea_size)
                       ? DESIRED_SUBAREA_SIZE
                       : min_subarea_size;
  ...
  if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
      ...
      /* Calculate number of sub areas..take care to avoid overflow if size is
       * large */
 ...
 */
void MM_CompactScheme::setRealLimitsSubAreas(MM_EnvironmentStandard *env) {
  ...
  for (uintptr_t i = 1; _subAreaTable[i].state != SubAreaEntry::end_heap; i++) {
    /* i=1 because first region starts with heapAlloc thus we don't need to find
     * its first object */
    ...
    if (changeSubAreaAction(env, &_subAreaTable[i],
                            SubAreaEntry::setting_real_limits)) {
      uintptr_t *start =
          (uintptr_t *)pageStart(pageIndex(_subAreaTable[i].freeChunk));
      uintptr_t *end =
          (uintptr_t *)pageStart(pageIndex(_subAreaTable[i + 1].freeChunk));
      MM_HeapMapIterator markedObjectIterator(_extensions, _markMap, start,
                                              end);
 ...
 */
void MM_CompactScheme::removeNullSubAreas(MM_EnvironmentStandard *env) {
  ...
  if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
    ...
    uintptr_t j = 0;
    for (uintptr_t i = 0; _subAreaTable[i].state != SubAreaEntry::end_heap;
         i++) {
      if (NULL != _subAreaTable[i].firstObject) {
        ...
        if ((j > 0) && (_subAreaTable[j - 1].state == SubAreaEntry::init)) {
          _compactFrom = (_compactFrom < _subAreaTable[j - 1].firstObject)
                             ? _compactFrom
                             : _subAreaTable[j - 1].firstObject;
          _compactTo = (_compactTo > _subAreaTable[j].firstObject)
                           ? _compactTo
                           : _subAreaTable[j].firstObject;
        }
 ...
 */
void MM_CompactScheme::completeSubAreaTable(MM_EnvironmentStandard *env) {
  if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
    MM_HeapRegionDescriptorStandard *region = NULL;
...
void MM_CompactScheme::compact(MM_EnvironmentBase *envBase,
  ...
  if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
    ...
    /* TODO: Perhaps the task dispatch should occur internally within so that
     * the initialization doesn't need to be done at a synchronize point?
     */
   ...
   *  o the compaction is aggressive. We use a single sub area per segment to
   * avoid potentially having multiple holes created per segment, thereby
   * fragmenting the space. This will result in singlethreaded compaction per
   * segment, and so should only be done in extreme OOM situations. o no slave
   * GC threads
   */
  ...
  /* If a single threaded compaction force compact to run on master thread.
   * Required to ensure all events issued on master thread.
   */
  if (!singleThreaded ||
      env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
...
void MM_CompactScheme::flushPool(MM_EnvironmentStandard *env,
                                 MM_CompactMemoryPoolState *poolState) {
  MM_MemoryPool *memoryPool = poolState->_memoryPool;

  if (poolState->_freeListHead) {
    memoryPool->addFreeEntries(env, poolState->_freeListHead,
                               poolState->_previousFreeEntry,
                               poolState->_freeHoles, poolState->_freeBytes);
  }
...
void MM_CompactScheme::rebuildFreelist(MM_EnvironmentStandard *env) {
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
      ...
      if (NULL != subAreaTable[i].freeChunk) {
        if (subAreaTable[i].freeChunk == subAreaTable[i].firstObject) {
          ...
          if (NULL == currentFreeBase) {
            currentFreeBase = (void *)subAreaTable[i]
                                  .firstObject;  // orizzz - round down to page
          }
        } else {
          /* There is some free area in the sub area but not all of it */
          if (NULL != currentFreeBase) {
            currentFreeSize = (uintptr_t)subAreaTable[i].firstObject -
                              (uintptr_t)currentFreeBase;

#if defined(DEBUG_PAINT_FREE)
            memset(currentFreeBase, 0xBB, currentFreeSize);
#endif /* DEBUG_PAINT_FREE */
            addFreeEntry(env, memorySubSpace, poolState, currentFreeBase,
                         currentFreeSize);
          }
          currentFreeSize = 0;
    ...
    if (NULL != poolState->_freeListHead) {
      ...
      poolState->_memoryPool->createFreeEntry(
          env, poolState->_previousFreeEntry,
...
/*
 * Call appropriate Memory Pool to add a new free entry to the pool. If the free
 * entry spans more than one subpool then it will be split into 2 free entries.
 *
 ...
 *
 */
MMINLINE void MM_CompactScheme::addFreeEntry(
    MM_EnvironmentStandard *env, MM_MemorySubSpace *memorySubSpace,
    MM_CompactMemoryPoolState *poolState, void *currentFreeBase,
   ...
   */
  lowPool = memorySubSpace->getMemoryPool(
      env, currentFreeBase, (uint8_t *)currentFreeBase + currentFreeSize,
      highAddr);
  ...
  lowChunkSize = (highAddr ? (uint8_t *)highAddr - (uint8_t *)currentFreeBase
                           : currentFreeSize);
  ...
  if (NULL != highAddr) {
    ...
    highChunkSize =
        ((uint8_t *)currentFreeBase + currentFreeSize) - (uint8_t *)highAddr;
    ...
    if (highChunkSize > highPool->getMinimumFreeEntrySize()) {
      ...
      highPool->createFreeEntry(
          env, highAddr, (uint8_t *)highAddr + highChunkSize, NULL, NULL);

      /* Update chunk stats */
      poolState->_freeBytes += highChunkSize;
      poolState->_freeHoles += 1;
      poolState->_largestFreeEntry =
          OMR_MAX(poolState->_largestFreeEntry, highChunkSize);
...
void MM_CompactScheme::moveObjects(MM_EnvironmentStandard *env,
                                   uintptr_t &objectCount, uintptr_t &byteCount,
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    ...
    for (i = 0; subAreaTable[i].state != SubAreaEntry::end_segment; i++) {
      if (changeSubAreaAction(env, &subAreaTable[i],
                              SubAreaEntry::evacuating)) {
        evacuateSubArea(env, region, subAreaTable, i, objectCount, byteCount,
                        skippedObjectCount);
      }
    }
 ...
 */
size_t MM_CompactScheme::setFreeChunkPageAligned(omrobjectptr_t from,
                                                 omrobjectptr_t to) {
  assume0(from && to);
...
void MM_CompactScheme::evacuateSubArea(
    MM_EnvironmentStandard *env, MM_HeapRegionDescriptorStandard *subAreaRegion,
  ...
  do {
    freeChunk = 0;
    for (j++; j < i;
         j++) {  // keeps searching from the prev. value to prevent inf loop
      if ((subAreaTableEvacuate[j].state == SubAreaEntry::ready) &&
          (SubAreaEntry::ready == MM_AtomicOperations::lockCompareExchange(
                                      &subAreaTableEvacuate[j].state,
                                      SubAreaEntry::ready,
                                      SubAreaEntry::busy))) {
    ...
    nobjects = nbytes = 0;
    objectPtr = doCompact(env, subspace, objectPtr, endObject, freeChunk,
                          nobjects, nbytes, true);
    ...
    Trc_MM_CompactScheme_evacuateSubArea_evacuated(
        env->getLanguageVMThread(), firstObject, endObject, nbytes,
        subAreaTableEvacuate[j].firstObject);
    if (size < minFreeChunk) {
      Trc_MM_CompactScheme_evacuateSubArea_bytesRemainingIgnored(
          env->getLanguageVMThread(), size,
          subAreaTableEvacuate[j].firstObject);
    } else {
      Trc_MM_CompactScheme_evacuateSubArea_bytesRemaining(
          env->getLanguageVMThread(), size,
          subAreaTableEvacuate[j].firstObject);
    }
    ...
    if (size < minFreeChunk) {
      uintptr_t state = MM_AtomicOperations::lockCompareExchange(
          &subAreaTableEvacuate[j].state, SubAreaEntry::busy,
          SubAreaEntry::full);
  ...
  if (objectPtr == 0) {
    ...
    if (size < minFreeChunk) {
      Trc_MM_CompactScheme_evacuateSubArea_subAreaFullIgnored(
          env->getLanguageVMThread(), firstObject, endObject, size);
    } else {
      Trc_MM_CompactScheme_evacuateSubArea_subAreaFull(
          env->getLanguageVMThread(), firstObject, endObject, size);
    }
    ...
    if (size < minFreeChunk) {
      uintptr_t state = MM_AtomicOperations::lockCompareExchange(
          &subAreaTableEvacuate[i].state, SubAreaEntry::init,
          SubAreaEntry::full);
  ...
  if (objectPtr == firstObject) {  // nothing evacuated
     ...
     */
    GC_ObjectHeapIteratorAddressOrderedList objectHeapIterator(
        _extensions, firstObject, pageStart(pageIndex(endObject)), true);
    ...
    if (objectPtr == 0) {
      ...
      omrobjectptr_t lastLiveEnd = (omrobjectptr_t)(
          (uintptr_t)lastLive +
          _extensions->objectModel.getConsumedSizeInBytesWithHeader(lastLive));
      Assert_MM_true(lastLiveEnd >= pageStart(pageIndex(endObject)) &&
                     lastLiveEnd <= endObject);
      setFreeChunk(lastLiveEnd, endObject);

      Trc_MM_CompactScheme_evacuateSubArea_subAreaAlreadyCompacted(
          env->getLanguageVMThread(), firstObject, endObject);
    ...
    if (objectPtr == 0) {
      ...
      if (size < minFreeChunk) {
        Trc_MM_CompactScheme_evacuateSubArea_subAreaAlreadyCompactedFreeSpaceIgnored(
            env->getLanguageVMThread(), firstObject, endObject);
      } else {
    ...
    nobjects = nbytes = 0;
    doCompact(env, subspace, objectPtr, endObject, freeChunk, nobjects, nbytes,
              false);
  ...
  /* Some, but not all objects were evacuated from the sub area i.  The
   * remaining objects are compacted using sliding within the sub area. */
  ...
  if (size < minFreeChunk) {
    Trc_MM_CompactScheme_evacuateSubArea_subAreaCompactedBFreeSpaceIgnored(
        env->getLanguageVMThread(), firstObject, endObject, nbytes, size);
  } else {
    Trc_MM_CompactScheme_evacuateSubArea_subAreaCompactedBFreeSpaceRemaining(
        env->getLanguageVMThread(), firstObject, endObject, nbytes, size);
    Trc_OMRMM_CompactScheme_evacuateSubArea_subAreaCompactedBFreeSpaceRemaining(
        env->getOmrVMThread(), firstObject, endObject, nbytes, size);
  }
  ...
  if (size < minFreeChunk) {
    uintptr_t state = MM_AtomicOperations::lockCompareExchange(
        &subAreaTableEvacuate[i].state, SubAreaEntry::init, SubAreaEntry::full);
    Assert_MM_true(state == SubAreaEntry::init);
  } else {
    uintptr_t state = MM_AtomicOperations::lockCompareExchange(
        &subAreaTableEvacuate[i].state, SubAreaEntry::init,
 ...
 */
MMINLINE void MM_CompactScheme::saveForwardingPtr(CompactTableEntry &entry,
 ...
 */
omrobjectptr_t MM_CompactScheme::doCompact(
    MM_EnvironmentStandard *env, MM_MemorySubSpace *memorySubSpace,
    omrobjectptr_t start, omrobjectptr_t finish, omrobjectptr_t &deadObject,
    uintptr_t &nobjects, uintptr_t &nbytes, bool evacuate) {
  /* we use the MM_HeapMapWordIterator in this function so ensure that it is
   * safe (can't be tested during init since assertions don't work at that
   * point) */
  Assert_MM_true(0 ==
                 (sizeof_page % J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP));
  /* Sanity checks */
  ...
  MM_HeapMapIterator markedObjectIterator(
      _extensions, _markMap, (uintptr_t *)start,
  ...
  for (objectPtr = markedObjectIterator.nextObject(); objectPtr != 0;
    ...
    if (evacuate && (pageIndex(objectPtr) != page)) {
      ...
      for (uintptr_t bias = 0; bias < sizeof_page;
           bias += J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP) {
        ...
        while ((NULL != (p = pagePieceIterator.nextObject())) &&
               (pageByteCount <= deadObjectSize)) {
          pageByteCount +=
              _extensions->objectModel.getConsumedSizeInBytesWithHeaderForMove(
                  p);
        }
      }
    ...
    if (deadObject == objectPtr) {
      /* if deadObject == objectPtr just continue with the walk. Do not move any
       * more objects as this is not needed and it will cause corruption if
       * objects grow after this point.
       */
  ...
  if (!evacuate) {
    ...
    if (deadObjectSize == 0) {
      ...
      if (deadObjectSize > 2 * sizeof(uintptr_t)) {
        uintptr_t junk =
            _extensions->objectModel.getSizeInBytesMultiSlotDeadObject(
                deadObject);
...
omrobjectptr_t MM_CompactScheme::getForwardingPtr(
    omrobjectptr_t objectPtr) const {
  ...
  if (n <= maxHints) {
    ...
    if (hint != invalidValue) {
      ...
      MM_CompactSchemeFixupObject::verifyForwardingPtr(objectPtr,
                                                       forwardingPtr);
      return forwardingPtr;
    }
  } else if (maxHints > 0) {
    ...
    for (intptr_t i = maxHints - 1; i >= 0; --i) {
      ...
      /* Check if the hint is invalid due to awkward object growth on
       * compaction. */
      if (hint != invalidValue) {
        n -= i + 1;
        Assert_MM_true(n > 0);
  ...
  for (intptr_t i = 0; i < n; i++) {
    size_t size = _extensions->objectModel.getConsumedSizeInBytesWithHeader(
        forwardingPtr);
...
void MM_CompactScheme::fixupObjects(MM_EnvironmentStandard *env,
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    ...
    for (i = 0; subAreaTable[i].state != SubAreaEntry::end_segment; i++) {
      if (changeSubAreaAction(env, &subAreaTable[i], SubAreaEntry::fixing_up)) {
        fixupSubArea(
            env, subAreaTable[i].firstObject, subAreaTable[i + 1].firstObject,
...
void MM_CompactScheme::fixupObjectSlot(GC_SlotObject *slotObject) {
  omrobjectptr_t forwardedPtr =
      getForwardingPtr(slotObject->readReferenceFromSlot());
  slotObject->writeReferenceToSlot(forwardedPtr);
}

void MM_CompactScheme::fixupSubArea(MM_EnvironmentStandard *env,
                                    omrobjectptr_t firstObject,
                                    omrobjectptr_t finish, bool markedOnly,
                                    uintptr_t &objectCount) {
  ...
  if (markedOnly) {
    ...
    MM_HeapMapIterator markedObjectIterator(
        _extensions, _markMap, (uintptr_t *)firstObject, (uintptr_t *)finish);
    ...
    GC_ObjectHeapIteratorAddressOrderedList objectIterator(
        _extensions, firstObject, finish, false);
...
void MM_CompactScheme::rebuildMarkbits(MM_EnvironmentStandard *env) {
  MM_HeapRegionManager *regionManager = _heap->getHeapRegionManager();
  GC_HeapRegionIteratorStandard regionIterator(regionManager);
  MM_HeapRegionDescriptorStandard *region = NULL;
  SubAreaEntry *subAreaTable = _subAreaTable;
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    ...
    intptr_t i;
    for (i = 0; subAreaTable[i].state != SubAreaEntry::end_segment; i++) {
      /* We only have to rebuild the markbits for sub areas which contain moved
       * objects */
      if (subAreaTable->state != SubAreaEntry::fixup_only) {
        if (changeSubAreaAction(env, &subAreaTable[i],
                                SubAreaEntry::rebuilding_mark_bits)) {
...
void MM_CompactScheme::rebuildMarkbitsInSubArea(
    MM_EnvironmentStandard *env, MM_HeapRegionDescriptorStandard *region,
  ...
  _markMap->setBitsInRange(env, pageStart(pageIndex(start)),
                           pageStart(pageIndex(end)), true);
  ...
  GC_ObjectHeapIteratorAddressOrderedList objectIterator(_extensions, start,
                                                         end, false);
  omrobjectptr_t objectPtr;
  while (NULL != (objectPtr = objectIterator.nextObject())) {
...
/*
 * fixHeapForWalk isn't required in Phase 4, since it simply attempts to fix up
 * any areas which weren't compacted. In Tarok, regions are entirely compacted
 * or entirely fixed up. There is no case in which a region may be only
 * partially compacted.
 */
void MM_CompactScheme::fixHeapForWalk(MM_EnvironmentBase *env) {
  MM_CompactFixHeapForWalkTask fixHeapForWalkTask(env, _dispatcher, this);
  _dispatcher->run(env, &fixHeapForWalkTask);
}

void MM_CompactScheme::parallelFixHeapForWalk(MM_EnvironmentBase *env) {
  MM_HeapRegionManager *regionManager = _heap->getHeapRegionManager();
  GC_HeapRegionIteratorStandard regionIterator(regionManager);
  MM_HeapRegionDescriptorStandard *region = NULL;
  SubAreaEntry *subAreaTable = _subAreaTable;
...
bool MM_CompactScheme::changeSubAreaAction(MM_EnvironmentBase *env,
                                           SubAreaEntry *entry,
                                           uintptr_t newAction) {
  bool successful = false;
  uintptr_t previousAction = entry->currentAction;
  if (previousAction != newAction) {
    uintptr_t action = MM_AtomicOperations::lockCompareExchange(
        &entry->currentAction, previousAction, newAction);
    if (action == previousAction) {
      successful = true;
    } else {
      /* during this phase it's only legitimate to change to newAction so if
       * currentAction changed underneath us that had better be its new value */
