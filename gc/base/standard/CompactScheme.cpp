...
void MM_CompactScheme::masterSetupForGC(MM_EnvironmentStandard* env) {
  ...
  _subAreaTable =
      (SubAreaEntry*)_extensions->sweepHeapSectioning->getBackingStoreAddress();
...
MMINLINE void MM_CompactScheme::addFreeEntry(
  ...
  if (NULL != highAddr) {
    ...
    if (highChunkSize > highPool->getMinimumFreeEntrySize()) {
      ...
      highPool->createFreeEntry(env, highAddr,
                                (uint8_t*)highAddr + highChunkSize, NULL, NULL);
 ...
 */
omrobjectptr_t MM_CompactScheme::doCompact(MM_EnvironmentStandard* env,
                                           MM_MemorySubSpace* memorySubSpace,
...
void MM_CompactScheme::rebuildMarkbitsInSubArea(
    MM_EnvironmentStandard* env,
    MM_HeapRegionDescriptorStandard* region,
    SubAreaEntry* subAreaTable,
    intptr_t i) {
