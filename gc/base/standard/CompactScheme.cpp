...
void
MM_CompactScheme::setFreeChunkSize(omrobjectptr_t deadObject,
                                   uintptr_t deadObjectSize)
{
  ...
  MM_HeapLinkedFreeHeader::fillWithHoles(
    deadObject, deadObjectSize, compressed);
}
...
void
MM_CompactScheme::workerSetupForGC(MM_EnvironmentStandard* env,
                                   bool singleThreaded)
{
 ...
 */
void
MM_CompactScheme::createSubAreaTable(MM_EnvironmentStandard* env,
                                     bool singleThreaded)
{
  ...
  if (max_subarea_num > necessary_subareas) {
    min_subarea_size =
      _heap->getMaximumPhysicalRange() / (max_subarea_num - necessary_subareas);
  } else {
...
MM_CompactScheme::setRealLimitsSubAreas(MM_EnvironmentStandard* env)
{
  ...
  for (uintptr_t i = 1; _subAreaTable[i].state != SubAreaEntry::end_heap; i++) {
    ...
    if (changeSubAreaAction(
          env, &_subAreaTable[i], SubAreaEntry::setting_real_limits)) {
      uintptr_t* start =
      ...
      MM_HeapMapIterator markedObjectIterator(
        _extensions, _markMap, start, end);
...
void
MM_CompactScheme::compact(MM_EnvironmentBase* envBase,
      ...
      if (NULL != subAreaTable[i].freeChunk) {
        if (subAreaTable[i].freeChunk == subAreaTable[i].firstObject) {
          ...
          if (NULL == currentFreeBase) {
            currentFreeBase =
              (void*)subAreaTable[i].firstObject; // orizzz - round down to page
          }
        } else {
...
#endif /* DEBUG_PAINT_FREE */
            addFreeEntry(
              env, memorySubSpace, poolState, currentFreeBase, currentFreeSize);
          }
          currentFreeSize = 0;
        ...
        if (NULL != currentFreeBase) {
          currentFreeSize =
            (uintptr_t)subAreaTable[i].firstObject - (uintptr_t)currentFreeBase;
 ...
 */
MMINLINE void
MM_CompactScheme::addFreeEntry(MM_EnvironmentStandard* env,
                               ...
                               uintptr_t currentFreeSize)
{
   ...
   */
  lowPool =
    memorySubSpace->getMemoryPool(env,
                                  currentFreeBase,
                                  (uint8_t*)currentFreeBase + currentFreeSize,
                                  highAddr);
...
void
MM_CompactScheme::moveObjects(MM_EnvironmentStandard* env,
                              ...
                              uintptr_t& skippedObjectCount)
{
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    ...
    for (i = 0; subAreaTable[i].state != SubAreaEntry::end_segment; i++) {
      if (changeSubAreaAction(
            env, &subAreaTable[i], SubAreaEntry::evacuating)) {
        evacuateSubArea(env,
                        region,
                        subAreaTable,
                        i,
 ...
 */
size_t
MM_CompactScheme::setFreeChunkPageAligned(omrobjectptr_t from,
                                          omrobjectptr_t to)
{
  assume0(from && to);
...
void
MM_CompactScheme::evacuateSubArea(
  ...
  uintptr_t& skippedObjectCount)
{
  ...
  do {
    ...
    nobjects = nbytes = 0;
    objectPtr = doCompact(
      env, subspace, objectPtr, endObject, freeChunk, nobjects, nbytes, true);
    ...
    if (size < minFreeChunk) {
      Trc_MM_CompactScheme_evacuateSubArea_bytesRemainingIgnored(
        env->getLanguageVMThread(), size, subAreaTableEvacuate[j].firstObject);
    } else {
    ...
    if (size < minFreeChunk) {
      uintptr_t state = MM_AtomicOperations::lockCompareExchange(
        &subAreaTableEvacuate[j].state, SubAreaEntry::busy, SubAreaEntry::full);
      Assert_MM_true(state == SubAreaEntry::busy);
    } else {
      uintptr_t state =
        MM_AtomicOperations::lockCompareExchange(&subAreaTableEvacuate[j].state,
                                                 SubAreaEntry::busy,
                                                 SubAreaEntry::ready);
      Assert_MM_true(state == SubAreaEntry::busy);
    }
  ...
  if (objectPtr == 0) {
    ...
    if (size < minFreeChunk) {
      ...
      uintptr_t state =
        MM_AtomicOperations::lockCompareExchange(&subAreaTableEvacuate[i].state,
                                                 SubAreaEntry::init,
                                                 SubAreaEntry::ready);
      Assert_MM_true(state == SubAreaEntry::init);
    }
  ...
  if (objectPtr == firstObject) { // nothing evacuated
    ...
    nobjects = nbytes = 0;
    doCompact(
      env, subspace, objectPtr, endObject, freeChunk, nobjects, nbytes, false);
  ...
  if (size < minFreeChunk) {
    ...
    uintptr_t state = MM_AtomicOperations::lockCompareExchange(
      &subAreaTableEvacuate[i].state, SubAreaEntry::init, SubAreaEntry::ready);
    Assert_MM_true(state == SubAreaEntry::init);
  }
  return;
}
 ...
 */
MMINLINE void
MM_CompactScheme::saveForwardingPtr(CompactTableEntry& entry,
 ...
 */
omrobjectptr_t
MM_CompactScheme::doCompact(MM_EnvironmentStandard* env,
                            MM_MemorySubSpace* memorySubSpace,
    ...
    if (evacuate && (pageIndex(objectPtr) != page)) {
      ...
      for (uintptr_t bias = 0; bias < sizeof_page;
        ...
        while ((NULL != (p = pagePieceIterator.nextObject())) &&
          ...
          pageByteCount +=
            _extensions->objectModel.getConsumedSizeInBytesWithHeaderForMove(p);
        }
      }
...
MM_CompactScheme::getForwardingPtr(omrobjectptr_t objectPtr) const
{
  ...
  for (intptr_t i = 0; i < n; i++) {
    size_t size =
      _extensions->objectModel.getConsumedSizeInBytesWithHeader(forwardingPtr);
...
void
MM_CompactScheme::fixupObjects(MM_EnvironmentStandard* env,
                               uintptr_t& objectCount)
{
  MM_HeapRegionManager* regionManager = _heap->getHeapRegionManager();
  GC_HeapRegionIteratorStandard regionIterator(regionManager);
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    ...
    for (i = 0; subAreaTable[i].state != SubAreaEntry::end_segment; i++) {
      if (changeSubAreaAction(env, &subAreaTable[i], SubAreaEntry::fixing_up)) {
        fixupSubArea(env,
                     subAreaTable[i].firstObject,
                     subAreaTable[i + 1].firstObject,
                     subAreaTable[i].state == SubAreaEntry::fixup_only,
                     objectCount);
      }
    }
    /* Number of regions in regionTable, including
...
void
MM_CompactScheme::fixupSubArea(MM_EnvironmentStandard* env,
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    ...
    for (i = 0; subAreaTable[i].state != SubAreaEntry::end_segment; i++) {
      ...
      if (subAreaTable->state != SubAreaEntry::fixup_only) {
        if (changeSubAreaAction(
              env, &subAreaTable[i], SubAreaEntry::rebuilding_mark_bits)) {
...
void
MM_CompactScheme::rebuildMarkbitsInSubArea(
  MM_EnvironmentStandard* env,
  ...
  intptr_t i)
{
  ...
  _markMap->setBitsInRange(
    env, pageStart(pageIndex(start)), pageStart(pageIndex(end)), true);
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    ...
    for (i = 0; subAreaTable[i].state != SubAreaEntry::end_segment; i++) {
      if (subAreaTable[i].state == SubAreaEntry::fixup_only) {
        if (changeSubAreaAction(
              env, &subAreaTable[i], SubAreaEntry::fixing_heap_for_walk)) {
          omrobjectptr_t start = subAreaTable[i].firstObject;
...
bool
MM_CompactScheme::changeSubAreaAction(MM_EnvironmentBase* env,
                                      SubAreaEntry* entry,
                                      uintptr_t newAction)
{
