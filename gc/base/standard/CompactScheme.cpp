...
void MM_CompactScheme::moveObjects(MM_EnvironmentStandard* env, uintptr_t& objectCount, uintptr_t& byteCount, uintptr_t& skippedObjectCount)
{
    MM_HeapRegionManager* regionManager = _heap->getHeapRegionManager();
    GC_HeapRegionIteratorStandard regionIterator(regionManager);
    MM_HeapRegionDescriptorStandard* region = NULL;
...
void MM_CompactScheme::evacuateSubArea(MM_EnvironmentStandard* env, MM_HeapRegionDescriptorStandard* subAreaRegion, SubAreaEntry* subAreaTableEvacuate, intptr_t i, uintptr_t& objectCount, uintptr_t& byteCount, uintptr_t& skippedObjectCount)
{
    ...
    do {
        ...
        for (j++; j < i; j++) { // keeps searching from the prev. value to prevent inf loop
            if ((subAreaTableEvacuate[j].state == SubAreaEntry::ready) && (SubAreaEntry::ready == MM_AtomicOperations::lockCompareExchange(&subAreaTableEvacuate[j].state, SubAreaEntry::ready, SubAreaEntry::busy))) {
...
void MM_CompactScheme::fixupSubArea(MM_EnvironmentStandard* env, omrobjectptr_t firstObject, omrobjectptr_t finish, bool markedOnly, uintptr_t& objectCount)
{
...
void MM_CompactScheme::rebuildMarkbitsInSubArea(MM_EnvironmentStandard* env, MM_HeapRegionDescriptorStandard* region, SubAreaEntry* subAreaTable, intptr_t i)
{
