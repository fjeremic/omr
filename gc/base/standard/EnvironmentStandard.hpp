...
public:
  MM_CopyScanCacheStandard
      *_survivorCopyScanCache; /**< the current copy cache for flipping */
  MM_CopyScanCacheStandard
      *_deferredScanCache; /**< a copy cache about to be pushed to scan queue,
                              but before that may be merged with some other
                              caches that collectively form contiguous memory */
  MM_CopyScanCacheStandard
      *_deferredCopyCache; /**< a copy cache about to be pushed to scan queue,
                              but before that may be merged with some other
                              caches that collectively form contiguous memory */
  MM_CopyScanCacheStandard
...
public:
  static MM_EnvironmentStandard *newInstance(MM_GCExtensionsBase *extensions,
  ...
  virtual void initializeGCThread() {
    /* before a thread turning into a GC one, it shortly acted as a mutator
     * (during thread attach sequence), which means it may have allocated or
     * exacuted an object access barrier. */
    flushGCCaches();
  }

  MMINLINE static MM_EnvironmentStandard *
  getEnvironment(OMR_VMThread *omrVMThread) {
    return static_cast<MM_EnvironmentStandard *>(
        omrVMThread->_gcOmrVMThreadExtensions);
  }
  MMINLINE static MM_EnvironmentStandard *
  getEnvironment(MM_EnvironmentBase *env) {
    return static_cast<MM_EnvironmentStandard *>(env);
  }

  MM_EnvironmentStandard(OMR_VMThread *omrVMThread)
      : MM_EnvironmentBase(omrVMThread), _survivorCopyScanCache(NULL),
        _deferredScanCache(NULL), _deferredCopyCache(NULL),
        _tenureCopyScanCache(NULL), _effectiveCopyScanCache(NULL),
        _tenureTLHRemainderBase(NULL), _tenureTLHRemainderTop(NULL),
        _loaAllocation(false), _survivorTLHRemainderBase(NULL),
...
#if defined(OMR_GC_MODRON_SCAVENGER)
  /**
   * Flush (to global stores) the local remembered object caches in thread-local
   * GC_Environment
   */
