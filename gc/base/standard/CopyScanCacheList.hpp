...
private:
  ...
  struct CopyScanCacheSublist {
    ...
    MM_LightweightNonReentrantLock
        _cacheLock;        /**< Lock for getting/putting caches */
  ...
  struct CopyScanCacheSublist
      *_sublists; /**< An array of CopyScanCacheSublist structures which is
                     _sublistCount elements long */
  uintptr_t _sublistCount; /**< the number of lists (split for parallelism).
                              Must be at least 1 */
  ...
  volatile uintptr_t
      *_cachedEntryCount; /* pointer to cachedEntryCount, that is shared among
                             all lists (of all nodes) */
...
private:
   ...
   * Also decrement the shared counter if sublist counter value after operation
   * is zero Must be called inside of a locked region
   *
   * @param sublistIndex sublist number counter should be incremented
   * @param value the positive value to increment
   */
...
public:
   ...
   */
  MMINLINE uintptr_t getAllocatedCacheCount() {
   ...
   * @param allocatedCacheEntryCount[in] The number of cache entries which this
   * list should be resized to contain
  ...
  appendCacheEntriesInHeap(MM_EnvironmentStandard *env,
                           MM_MemorySubSpace *memorySubSpace,
                           MM_Collector *requestCollector);

  /**
   * Walk all sublists and count total number of entries
   * Return true if all permanent scan caches are currently returned back to
   * list
   * @return true if total number of entries attached to sublists is equal total
   * number of allocated scan caches
   */
  ...
  MM_CopyScanCacheList()
      : MM_BaseVirtual(), _allocationInHeap(false), _sublists(NULL),
        _sublistCount(0), _chunkHead(NULL), _incrementEntryCount(0),
        _totalAllocatedEntryCount(0), _cachedEntryCount(NULL) {
