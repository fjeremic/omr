 ...
 * Initialization
 */
MM_PhysicalSubArenaVirtualMemorySemiSpace* MM_PhysicalSubArenaVirtualMemorySemiSpace::newInstance(
    MM_EnvironmentBase* env, MM_Heap* heap)
{
    MM_PhysicalSubArenaVirtualMemorySemiSpace* subArena;

    subArena = (MM_PhysicalSubArenaVirtualMemorySemiSpace*)env->getForge()->allocate(
        sizeof(MM_PhysicalSubArenaVirtualMemorySemiSpace), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (subArena) {
 ...
 */
bool MM_PhysicalSubArenaVirtualMemorySemiSpace::initialize(MM_EnvironmentBase* env)
{
 ...
 */
void MM_PhysicalSubArenaVirtualMemorySemiSpace::tearDown(MM_EnvironmentBase* env)
{
    ...
    if (NULL != _subSpace) {
        _subSpace->heapRemoveRange(env, _subSpace, ((uintptr_t)_highAddress) - ((uintptr_t)_lowAddress), _lowAddress,
            _highAddress, lowValidAddress, highValidAddress);
 ...
 */
bool MM_PhysicalSubArenaVirtualMemorySemiSpace::inflate(MM_EnvironmentBase* env)
{
    ...
    if (_parent->attachSubArena(env, this, _subSpace->getInitialSize(), attachPolicy)) {
        ...
        if (NULL
            == (_highSemiSpaceRegion = getHeapRegionManager()->createAuxiliaryRegionDescriptor(
        ...
        if ((_highSemiSpaceRegion->getLowAddress() != semiSpaceMiddle)
            ...
            omrtty_printf("!!! Fatal Error in MM_PhysicalSubArenaVirtualMemorySemiSpace::inflate - bad address range "
                          "(%p,%p) for highSemiSpaceRegion, expected to be (%p, %p)\n",
                _highSemiSpaceRegion->getLowAddress(), _highSemiSpaceRegion->getHighAddress(), semiSpaceMiddle,
                _highAddress);
            Assert_MM_inflateInvalidRange();
        }
        if (NULL
            == (_lowSemiSpaceRegion = getHeapRegionManager()->createAuxiliaryRegionDescriptor(
        ...
        if ((_lowSemiSpaceRegion->getLowAddress() != _lowAddress)
            ...
            omrtty_printf("!!! Fatal Error in MM_PhysicalSubArenaVirtualMemorySemiSpace::inflate - bad address range "
                          "(%p,%p) for lowSemiSpaceRegion, expected to be (%p, %p)\n",
                _lowSemiSpaceRegion->getLowAddress(), _lowSemiSpaceRegion->getHighAddress(), _lowAddress,
                semiSpaceMiddle);
            Assert_MM_inflateInvalidRange();
        }

        /* Inform the semi spaces that they have been expanded */
        bool result = subSpaceAllocate->expanded(env, this, _highSemiSpaceRegion->getSize(),
            _highSemiSpaceRegion->getLowAddress(), _highSemiSpaceRegion->getHighAddress(), false);
        subSpaceAllocate->heapReconfigured(env);
        result = result
            && subSpaceSurvivor->expanded(env, this, _lowSemiSpaceRegion->getSize(),
                _lowSemiSpaceRegion->getLowAddress(), _lowSemiSpaceRegion->getHighAddress(), false);
 ...
 */
uintptr_t MM_PhysicalSubArenaVirtualMemorySemiSpace::expand(MM_EnvironmentBase* env, uintptr_t expandSize)
{
    ...
    assume0(expandSize
        == MM_Math::roundToCeiling(
            MM_GCExtensions::getExtensions(env)->heapAlignment, expandSize)); /* expand requests must be aligned */
    ...
    maximumExpandSize
        = MM_Math::roundToFloor(heapAlignment * 2, _subSpace->getMaximumSize() - _subSpace->getCurrentSize());
    ...
    if (!((MM_PhysicalArenaVirtualMemory*)_parent)
             ->canExpand(env, this, (void*)(((uintptr_t)_lowAddress) - totalExpandSize), totalExpandSize)) {
    ...
    if (NULL != _lowArena) {
        ...
        availableSize
            = ((uintptr_t)_lowAddress) - ((uintptr_t)((MM_PhysicalArenaVirtualMemory*)_parent)->getLowAddress());
    }
    ...
    Assert_MM_true(
        totalExpandSize == MM_Math::roundToCeiling(2 * getHeapRegionManager()->getRegionSize(), totalExpandSize));
 ...
 */
static void psavmssMoveFunction(
    OMR_VM* omrVM, omrobjectptr_t* objectIndirect, void* localData, uint32_t isDerivedPointer)
{
    ...
    if (NULL != objectPtr) {
        if ((objectPtr >= (omrobjectptr_t)userData->srcBase) && (objectPtr < (omrobjectptr_t)userData->srcTop)) {
            objectPtr = (omrobjectptr_t)(
                (((uintptr_t)objectPtr) - ((uintptr_t)userData->srcBase)) + ((uintptr_t)userData->dstBase));
 ...
 */
uintptr_t MM_PhysicalSubArenaVirtualMemorySemiSpace::contract(MM_EnvironmentBase* env, uintptr_t contractSize)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    bool debug = extensions->debugDynamicNewSpaceSizing;
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
    ...
    Assert_MM_true(contractSize
        == MM_Math::roundToCeiling(effectiveAlignment, contractSize)); /* expand requests must be aligned */
     ...
     */
    uintptr_t maximumContractSize
        = MM_Math::roundToFloor(effectiveAlignment * 2, _subSpace->getCurrentSize() - _subSpace->getMinimumSize());
    ...
    if (debug) {
        omrtty_printf("\tlowseg:(%p %p) highseg:(%p %p)\n", _lowSemiSpaceRegion->getLowAddress(),
            _lowSemiSpaceRegion->getHighAddress(), _highSemiSpaceRegion->getLowAddress(),
    ...
    void* allocateLeadingFreeTop
        = subSpaceAllocate->findFreeEntryTopStartingAtAddr(env, allocateRegion->getLowAddress());
    ...
    uintptr_t allocateTrailingFreeSize
        = ((uintptr_t)allocateRegion->getHighAddress()) - ((uintptr_t)allocateTrailingFreeBase);
    ...
    if (extensions->tiltedScavenge) {
        uintptr_t remainingFreeSizeForAllocate
            = MM_Math::roundToCeiling(effectiveAlignment, (remainingFreeSize / 100) * (100 - survivorSpaceRatio));
    ...
    if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
        ...
        if (allocateSegmentBase > allocateLeadingFreeTop) {
            if (extensions->isConcurrentScavengerEnabled()) {
                 ...
                 * that have been allocated and hashed during the last concurrent phase (hence, in hybrid
                 * Survivor-Allocate that is subject to movement).
                 */
                if (debug) {
                    omrtty_printf("\tHeap movement (%p %p) to (%p %p) required, but not allowed with CS enabled.\n",
                        allocateLeadingFreeTop, allocateTrailingFreeBase, allocateSegmentBase,
            ...
            /* TODO: can this walk be parallelized? Probably not trivially, since the mark map isn't up to date at this
             * point */
            ...
            heapWalker->allObjectSlotsDo(env, psavmssMoveFunction, (void*)&psavmssMoveData,
        ...
        if (allocateLeadingFreeSize > 0) {
            subSpaceAllocate->removeExistingMemory(
                env, this, allocateLeadingFreeSize, allocateLeadingFreeBase, allocateLeadingFreeTop);
        }
        if (allocateTrailingFreeSize > 0) {
            subSpaceAllocate->removeExistingMemory(
                env, this, allocateTrailingFreeSize, (void*)allocateTrailingFreeBase, allocateRegion->getHighAddress());
        }

        /* Adjust the region boundaries */
        /* we are subtly assuming that allocate size is at least as large as survivor size (the converse makes no sense)
         * so ensure this assumption is correct */
        ...
        if (allocateSegmentBase > allocateLeadingFreeTop) {
            ...
            MM_ContractSlotScanner contractSlotScanner(env, (void*)allocateLeadingFreeTop,
                (void*)(((uintptr_t)allocateLeadingFreeTop) + heapSizeToMove), (void*)allocateSegmentBase);
        ...
        if (allocateSegmentBase < allocateLeadingFreeTop) {
            ...
            subSpaceAllocate->addExistingMemory(env, this,
                ((uintptr_t)allocateLeadingFreeTop) - ((uintptr_t)allocateSegmentBase), (void*)allocateSegmentBase,
        ...
        if (debug) {
            omrtty_printf("\tAdd free range back (%p %p) size %p\n", (void*)allocateSpaceUsedHeapTop,
                (void*)allocateSegmentTop, (uintptr_t)(allocateSegmentTop - (uintptr_t)allocateSpaceUsedHeapTop));
        }

        subSpaceAllocate->addExistingMemory(env, this,
            (uintptr_t)(allocateSegmentTop - (uintptr_t)allocateSpaceUsedHeapTop), (void*)allocateSpaceUsedHeapTop,
            (void*)allocateSegmentTop, true);

        /* Rebuild the survivor free list */
        subSpaceSurvivor->addExistingMemory(env, this,
            ((uintptr_t)survivorSegmentTop) - ((uintptr_t)survivorSegmentBase), (void*)survivorSegmentBase,
            (void*)survivorSegmentTop, true);
        ...
        _subSpace->heapRemoveRange(env, _subSpace, removeMemorySize, removeMemoryBase, removeMemoryTop,
            previousValidAddressNotRemoved, (void*)allocateSegmentBase);
        _subSpace->heapReconfigured(env);

        /* Decommit the heap (the return value really doesn't matter here - its already too late) */
        _heap->decommitMemory(
            removeMemoryBase, removeMemorySize, previousValidAddressNotRemoved, (void*)allocateSegmentBase);
        ...
        /* Decommit the heap (the return value really doesn't matter here - its already too late) */
        _heap->decommitMemory(
            removeMemoryBase, removeMemorySize, previousValidAddressNotRemoved, (void*)survivorSegmentBase);
 ...
 */
bool MM_PhysicalSubArenaVirtualMemorySemiSpace::canExpand(MM_EnvironmentBase* env)
{
    return _resizable;
}
 ...
 */
bool MM_PhysicalSubArenaVirtualMemorySemiSpace::canContract(MM_EnvironmentBase* env)
{
    if (_resizable) {
 ...
 * the alignment of the sizes, the sizes are equal to the total semispace size, the sizes fall within ration limits,
 * etc.
 * @note Assumes that there is a single free entry in the allocate space and that it is at the END of the range
 * (segment).
...
void MM_PhysicalSubArenaVirtualMemorySemiSpace::tilt(
    MM_EnvironmentBase* env, uintptr_t allocateSpaceSize, uintptr_t survivorSpaceSize, bool updateMemoryPools)
{
    ...
    if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
        ...
        getHeapRegionManager()->resizeAuxillaryRegion(
            env, _highSemiSpaceRegion, highBase, _highSemiSpaceRegion->getHighAddress());
        ...
        getHeapRegionManager()->resizeAuxillaryRegion(
            env, _lowSemiSpaceRegion, _lowSemiSpaceRegion->getLowAddress(), expandTop);
        ...
        /* Adjust the upper boundary of the allocate (low) segment */
        getHeapRegionManager()->resizeAuxillaryRegion(
            env, _highSemiSpaceRegion, lowTop, _highSemiSpaceRegion->getHighAddress());
     ...
     * Caller may choose not to do it. For example, if the more up-to-date list is to be rebuilt by following sweep,
    ...
    if (updateMemoryPools) {
        ...
        if (expandBase > expandTop) {
            ...
            subSpaceAllocate->removeExistingMemory(
                env, this, ((uintptr_t)expandBase) - ((uintptr_t)expandTop), expandTop, expandBase);
            subSpaceSurvivor->addExistingMemory(
                env, this, ((uintptr_t)expandBase) - ((uintptr_t)expandTop), expandTop, expandBase, true);
        } else
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
        {
            subSpaceSurvivor->removeExistingMemory(
                env, this, ((uintptr_t)expandTop) - ((uintptr_t)expandBase), expandBase, expandTop);
            subSpaceAllocate->addExistingMemory(
                env, this, ((uintptr_t)expandTop) - ((uintptr_t)expandBase), expandBase, expandTop, true);
        }
    }
    ...
    ((MM_MemorySubSpaceSemiSpace*)_subSpace)
        ->setSurvivorSpaceSizeRatio(
            survivorSpaceSize / ((_highSemiSpaceRegion->getSize() + _lowSemiSpaceRegion->getSize()) / 100));
 ...
 * @todo This is just an exploratory implementation until the details are actually hammered out.
 */
void MM_PhysicalSubArenaVirtualMemorySemiSpace::tilt(MM_EnvironmentBase* env, uintptr_t survivorSpaceSizeRequested)
{
    /*
    ...
    survivorSpaceSize = (survivorSpaceSize > extensions->absoluteMinimumNewSubSpaceSize)
        ? survivorSpaceSize
        : MM_Math::roundToCeiling(heapAlignment, extensions->absoluteMinimumNewSubSpaceSize);
    ...
    minimumBytesFree = (uintptr_t)(
        (_lowSemiSpaceRegion->getSize() + _highSemiSpaceRegion->getSize()) * survivorSpaceMinimumSizeRatio);
    ...
    /* region size rounding is required on these minimum and maximum calculations since they can be used to change the
     * requested survivor space */
    ...
    if (debug) {
        omrtty_printf("\tAvailable: %d(%p)  Required: %d(%p)\n", availableBytesForSurvivorSpace,
            availableBytesForSurvivorSpace, survivorSpaceSize, survivorSpaceSize);
    }

    /* Associate the high/low region to the allocate/survivor subspace */
    tilt(
        env, (_lowSemiSpaceRegion->getSize() + _highSemiSpaceRegion->getSize()) - survivorSpaceSize, survivorSpaceSize);
}
/**
 * Expand the receiver by the given size.
 * Expand by the given size with no safety checks.  Restrictions on expansion size based on the amount of room available
 * and the room between the receiver and any neighbouring PSA are ignored.  The size for expansion may be reduced based
 * on splitting requirements (the split may not be even, or possible).
 ...
 */
uintptr_t MM_PhysicalSubArenaVirtualMemorySemiSpace::expandNoCheck(MM_EnvironmentBase* env, uintptr_t expandSize)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    ...
    uintptr_t splitExpandSize
        = calculateExpansionSplit(env, expandSize, &allocateSpaceExpandSize, &survivorSpaceExpandSize);
    Assert_MM_true(
        splitExpandSize == MM_Math::roundToCeiling(getHeapRegionManager()->getRegionSize(), splitExpandSize));
    ...
    Assert_MM_true(
        allocateSpaceExpandSize == MM_Math::roundToCeiling(extensions->heapAlignment, allocateSpaceExpandSize));
    Assert_MM_true(
        survivorSpaceExpandSize == MM_Math::roundToCeiling(extensions->heapAlignment, survivorSpaceExpandSize));
    /* Find where the allocate subspace is */
    MM_MemorySubSpace* subSpaceAllocate = ((MM_MemorySubSpaceSemiSpace*)_subSpace)->getMemorySubSpaceAllocate();
    MM_MemorySubSpace* subSpaceSurvivor = ((MM_MemorySubSpaceSemiSpace*)_subSpace)->getMemorySubSpaceSurvivor();
    ...
    if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
        ...
        if (0 != allocateSpaceExpandSize) {
            freeRangeToTransferBase
                = subSpaceAllocate->findFreeEntryEndingAtAddr(env, (void*)_lowSemiSpaceRegion->getHighAddress());
            freeRangeToTransferBase
                = (void*)MM_Math::roundToCeiling(extensions->heapAlignment, ((uintptr_t)freeRangeToTransferBase));
            ...
            uintptr_t freeRangeAvailableToTransferSize
                = ((uintptr_t)_lowSemiSpaceRegion->getHighAddress()) - ((uintptr_t)freeRangeToTransferBase);
        ...
        subSpaceAllocate->removeExistingMemory(
            env, this, survivorSpaceExpandSize, freeRangeToTransferBase, _lowSemiSpaceRegion->getHighAddress());
        ...
        getHeapRegionManager()->resizeAuxillaryRegion(
            env, _highSemiSpaceRegion, freeRangeToTransferBase, _highSemiSpaceRegion->getHighAddress());
        ...
        _subSpace->heapAddRange(
            env, _subSpace, splitExpandSize, newLowAddress, (void*)(((uintptr_t)newLowAddress) + splitExpandSize));
        _subSpace->heapReconfigured(env);
        ...
        subSpaceSurvivor->reset();
        subSpaceSurvivor->addExistingMemory(env, this, _highSemiSpaceRegion->getSize(),
            _highSemiSpaceRegion->getLowAddress(), _highSemiSpaceRegion->getHighAddress(), true);
        ...
        /* ensure that the lower region is expanded first, so it is out of the way of the high region */
        getHeapRegionManager()->resizeAuxillaryRegion(
            env, _lowSemiSpaceRegion, newLowAddress, (void*)((uintptr_t)newLowAddress + newLowSpacesize));
        getHeapRegionManager()->resizeAuxillaryRegion(
            env, _highSemiSpaceRegion, _lowSemiSpaceRegion->getHighAddress(), _highSemiSpaceRegion->getHighAddress());
        ...
        subSpaceAllocate->addExistingMemory(env, this, allocateSpaceExpandSize, _highSemiSpaceRegion->getLowAddress(),
            previousHighSemiSpaceSegmentBase, true);

        subSpaceSurvivor->reset();
        subSpaceSurvivor->addExistingMemory(env, this, _lowSemiSpaceRegion->getSize(),
            _lowSemiSpaceRegion->getLowAddress(), _lowSemiSpaceRegion->getHighAddress(), true);
...
uintptr_t MM_PhysicalSubArenaVirtualMemorySemiSpace::calculateExpansionSplit(
    MM_EnvironmentBase* env, uintptr_t requestExpandSize, uintptr_t* allocateSpaceSize, uintptr_t* survivorSpaceSize)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    ...
    *survivorSpaceSize = 0;
    ...
    if (extensions->tiltedScavenge) {
        ...
        survivorSpaceExpandSize
            = MM_Math::roundToCeiling(extensions->heapAlignment, (expandSize / 100) * survivorSpaceSizeRatio);
        survivorSpaceExpandSize
            = MM_Math::roundToCeiling(getHeapRegionManager()->getRegionSize(), survivorSpaceExpandSize);
        ...
        allocateSpaceExpandSize
            = MM_Math::roundToFloor(getHeapRegionManager()->getRegionSize(), allocateSpaceExpandSize);
    ...
    if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
        ...
        /* Find the trailing free entry in the memory space (single segment) that will get transfered to the other semi
         * space */
        void* freeRangeToTransferBase
            = subSpaceAllocate->findFreeEntryEndingAtAddr(env, _lowSemiSpaceRegion->getHighAddress());
        ...
        uintptr_t initialFreeRangeToTransferSize
            = ((uintptr_t)_lowSemiSpaceRegion->getHighAddress()) - ((uintptr_t)freeRangeToTransferBase);
        uintptr_t freeRangeToTransferSize
            = MM_Math::roundToFloor(extensions->heapAlignment, initialFreeRangeToTransferSize);
        freeRangeToTransferSize
            = MM_Math::roundToFloor(getHeapRegionManager()->getRegionSize(), freeRangeToTransferSize);
        ...
        if (freeRangeToTransferSize < survivorSpaceExpandSize) {
            ...
            Assert_MM_true(
                survivorSpaceExpandSize % extensions->heapAlignment == 0); // transfer size was align guaranteed before
    ...
    MM_EnvironmentBase* env, uintptr_t expandSizeDeltaAlignment, uintptr_t expandSize)
{
    ...
    assume0(
        adjustedExpandSize == calculateExpansionSplit(env, adjustedExpandSize, &allocateSpaceSize, &survivorSpaceSize));
