 ...
 */
void MM_ConcurrentCardTable::tlhRefreshed(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
    MM_CacheRefreshedEvent* event = (MM_CacheRefreshedEvent*)eventData;
    MM_EnvironmentBase* env = MM_EnvironmentBase::getEnvironment(event->currentThread);

    ((MM_ConcurrentCardTable*)userData)
        ->processTLHMarkBits(env, (MM_MemorySubSpace*)event->subSpace, event->cacheBase, event->cacheTop, SET);
}
 ...
 * @see MM_ParallelGlobalGC::TLHRefreshed()
 */
void MM_ConcurrentCardTable::tlhCleared(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
    MM_CacheClearedEvent* event = (MM_CacheClearedEvent*)eventData;
    MM_EnvironmentBase* env = MM_EnvironmentBase::getEnvironment(event->currentThread);

    ((MM_ConcurrentCardTable*)userData)
        ->processTLHMarkBits(env, (MM_MemorySubSpace*)event->subSpace, event->cacheBase, event->cacheTop, CLEAR);
}
 ...
 */
uintptr_t MM_ConcurrentCardTable::calculateTLHMarkMapSize(MM_EnvironmentBase* env, uintptr_t cardTableSize)
{
    uintptr_t size;

    size = MM_Math::roundToCeiling(
        sizeof(uint32_t), MM_Math::roundToCeiling(BITS_IN_BYTE, cardTableSize) / BITS_IN_BYTE);
 ...
 */
bool MM_ConcurrentCardTable::allocateCardTableEntriesForHeapRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace,
    uintptr_t size, void* lowAddress, void* highAddress, bool clearNewCards)
{
 ...
 */
bool MM_ConcurrentCardTable::freeCardTableEntriesForHeapRange(MM_EnvironmentBase* env, uintptr_t size, void* lowAddress,
    ...
    MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size, void* lowAddress, void* highAddress)
{
    ...
    if (_extensions->isFvtestForceConcurrentTLHMarkMapCommitFailure()) {
        ...
        if (NULL != _tlhMarkBits) {
            ...
            /* Adjust the lowTLHMArkMap to account for rounding (since this is really the highTLHMarkMap address of
             * another range). as we dont want to commit a slot we have previously commited andn more importantly we
             * dont want to clear and existing slot. */
            ...
            commited
                = memoryManager->commitMemory(&_tlhMarkMapMemoryHandle, (void*)&(_tlhMarkBits[lowTLHMarkMap]), mapSize);
            ...
            if (commited) {
                ...
                Trc_MM_ConcurrentCardTable_activeTLHMarkMapCommitFailed(
                    env->getLanguageVMThread(), (void*)&(_tlhMarkBits[lowTLHMarkMap]), mapSize);
            }
        }
    }
    return commited;
 ...
 * @param highAddress The top address (non-inclusive) of the memory added to the heap
 */
bool MM_ConcurrentCardTable::freeTLHMarkMapEntriesForHeapRange(MM_EnvironmentBase* env, uintptr_t size,
    ...
    if (_extensions->isFvtestForceConcurrentTLHMarkMapDecommitFailure()) {
        ...
        if (NULL != _tlhMarkBits) {
            ...
            if (NULL != lowValidAddress) {
                ...
                /* Adjust the low valid range to account for rounding (since this is really the high address of another
                 * range) */
            ...
            if (0 != mapSize) {
                MM_MemoryManager* memoryManager = _extensions->memoryManager;
                decommited
                    = memoryManager->decommitMemory(&_tlhMarkMapMemoryHandle, (void*)&(_tlhMarkBits[lowTLHMarkMap]),
                        mapSize, (NULL != lowValidAddress) ? (void*)&(_tlhMarkBits[lowValidTLHMarkMap]) : NULL,
                        (NULL != highValidAddress) ? (void*)&(_tlhMarkBits[highValidTLHMarkMap]) : NULL);
                if (!decommited) {
                    Trc_MM_ConcurrentCardTable_activeTLHMarkMapDecommitFailed(env->getLanguageVMThread(),
 ...
 */
bool MM_ConcurrentCardTable::heapAddRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size,
 ...
 *
 */
bool MM_ConcurrentCardTable::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size,
    void* lowAddress, void* highAddress, void* lowValidAddress, void* highValidAddress)
{
    bool result = true;
    /*
    ...
    if (NULL != _heapAlloc) {
        ...
        /* Adjust card table  to reflect heap change */
        result
            = freeCardTableEntriesForHeapRange(env, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
        ...
        if (result) {
            if (subspace->isConcurrentCollectable()) {
                ...
                result = freeTLHMarkMapEntriesForHeapRange(
...
MM_ConcurrentCardTable* MM_ConcurrentCardTable::newInstance(
    MM_EnvironmentBase* env, MM_Heap* heap, MM_MarkingScheme* markingScheme, MM_ConcurrentGC* collector)
{
    MM_ConcurrentCardTable* cardTable = (MM_ConcurrentCardTable*)env->getForge()->allocate(
        sizeof(MM_ConcurrentCardTable), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (NULL != cardTable) {
 ...
 */
bool MM_ConcurrentCardTable::initialize(MM_EnvironmentBase* env, MM_Heap* heap)
{
    ...
    if (initialized) {
...
#endif /* OMR_GC_MODRON_SCAVENGER */
        {
            ...
            if (!memoryManager->createVirtualMemoryForMetadata(
                    env, &_tlhMarkMapMemoryHandle, sizeof(uintptr_t), tlhMarkMapSizeRequired)) {
                return false;
            }
            ...
            (*mmPrivateHooks)
                ->J9HookRegisterWithCallSite(
                    mmPrivateHooks, J9HOOK_MM_PRIVATE_CACHE_CLEARED, tlhCleared, OMR_GET_CALLSITE(), (void*)this);
            (*mmPrivateHooks)
                ->J9HookRegisterWithCallSite(
                    mmPrivateHooks, J9HOOK_MM_PRIVATE_CACHE_REFRESHED, tlhRefreshed, OMR_GET_CALLSITE(), (void*)this);
        }
 ...
 */
uintptr_t MM_ConcurrentCardTable::cardBytesForHeapRange(MM_EnvironmentBase* env, void* heapBase, void* heapTop)
{
 ...
 */
void MM_ConcurrentCardTable::dirtyCardsInRange(MM_EnvironmentBase* env, void* heapBase, void* heapTop)
{
...
void MM_ConcurrentCardTable::initializeCardCleaning(MM_EnvironmentBase* env)
{
    ...
    if (_extensions->cardCleaningPasses > 0) {
        MM_AtomicOperations::lockCompareExchangeU32(
            (volatile uint32_t*)&_cardCleanPhase, (uint32_t)_cardCleanPhase, (uint32_t)UNINITIALIZED);
...
void MM_ConcurrentCardTable::prepareCardsForCleaning(MM_EnvironmentBase* env)
{
    ...
    case PHASE1_PREPARING:
        ...
        MM_AtomicOperations::lockCompareExchange(
            (volatile uintptr_t*)&_currentCleaningRange, (uintptr_t)_currentCleaningRange, (uintptr_t)_cleaningRanges);
        break;
 ...
 */
void MM_ConcurrentCardTable::reportCardCleanPass2Start(MM_EnvironmentBase* env)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

    TRIGGER_J9HOOK_MM_PRIVATE_CARD_CLEANING_PASS_2_START(_extensions->privateHookInterface, env->getOmrVMThread(),
...
MMINLINE bool MM_ConcurrentCardTable::getExclusiveCardTableAccess(
    MM_EnvironmentBase* env, CardCleanPhase currentPhase, bool threadAtSafePoint)
{
    ...
    if (!cardTableBeingPrepared(currentPhase)) {
        if (currentPhase
            == (CardCleanPhase)MM_AtomicOperations::lockCompareExchangeU32(
                (volatile uint32_t*)&_cardCleanPhase, (uint32_t)currentPhase, (uint32_t)currentPhase + 1)) {
 ...
 */
MMINLINE void MM_ConcurrentCardTable::releaseExclusiveCardTableAccess(MM_EnvironmentBase* env)
{
 ...
 */
bool MM_ConcurrentCardTable::cleanCards(
    ...
    if (NULL == nextDirtyCard) {
        ...
        if (cardCleaningInProgress(currentCleaningPhase)) {
            MM_AtomicOperations::lockCompareExchangeU32((volatile uint32_t*)&_cardCleanPhase,
                (uint32_t)currentCleaningPhase, (uint32_t)currentCleaningPhase + 1);
        }
    }
 ...
 */
bool MM_ConcurrentCardTable::cleanSingleCard(
    MM_EnvironmentBase* env, Card* card, uintptr_t bytesToClean, uintptr_t* totalBytesCleaned)
{
 ...
 */
void MM_ConcurrentCardTable::initializeFinalCardCleaning(MM_EnvironmentBase* env)
{
    if (_cardTableReconfigured) {
        determineCleaningRanges(env);
    } else {
 ...
 *
 */
bool MM_ConcurrentCardTable::finalCleanCards(MM_EnvironmentBase* env, uintptr_t* bytesTraced)
{
    ...
    for (; (nextDirtyCard = getNextDirtyCard(env, _finalCardCleanMask, false)) != NULL;) {
 ...
 *
 */
void MM_ConcurrentCardTable::processTLHMarkBits(
    MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, void* tlhBase, void* tlhTop, BitMapAction action)
{
    ...
    assume((uintptr_t*)tlhBase >= (uintptr_t*)heapBase && (uintptr_t*)tlhTop <= (uintptr_t*)_heapAlloc,
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        ...
        if (region->getSubSpace()->isConcurrentCollectable()) {
            uintptr_t currentSlot
                = (((uintptr_t)region->getLowAddress()) - numericalHeapBase) >> TLH_MARKING_INDEX_SHIFT;
 ...
 */
bool MM_ConcurrentCardTable::isObjectInActiveTLH(MM_EnvironmentBase* env, omrobjectptr_t object)
{
 ...
 */
bool MM_ConcurrentCardTable::isCardInActiveTLH(MM_EnvironmentBase* env, Card* card)
{
    uintptr_t markIndex, markBit;
    bool active;
 ...
 */
bool MM_ConcurrentCardTable::isObjectInDirtyCardNoCheck(MM_EnvironmentBase* env, omrobjectptr_t object)
{
    bool dirty;
 ...
 */
bool MM_ConcurrentCardTable::isObjectInUncleanedDirtyCard(MM_EnvironmentBase* env, omrobjectptr_t object)
{
    ...
    if (!isCardCleaningComplete() && (currentRange != _lastCleaningRange) && (currentRange->nextCard < card)) {
        return true;
    }
 ...
 * @return TRUE if reference is to an object within a dirty card; FALSE otherwise
 */
bool MM_ConcurrentCardTable::isObjectInDirtyCard(MM_EnvironmentBase* env, omrobjectptr_t object)
{
    bool dirty = false;
 ...
 */
Card* MM_ConcurrentCardTable::getNextDirtyCard(MM_EnvironmentBase* env, Card cardMask, bool concurrentCardClean)
{
    ...
    while (NULL != firstCard) {
        ...
        for (currentCard = firstCard; currentCard < lastCardToClean; currentCard++) {
            ...
            if (firstCard != (Card*)currentRange->nextCard) {
                 ...
                 */
                if (firstCard
                    != (Card*)MM_AtomicOperations::lockCompareExchange(
                        (volatile uintptr_t*)&currentRange->nextCard, (uintptr_t)firstCard, (uintptr_t)currentCard)) {
                    break;
                }
        ...
        if (currentCard < lastCardToClean) {
            ...
            MM_AtomicOperations::lockCompareExchange((volatile uintptr_t*)&currentRange->nextCard,
            ...
            MM_AtomicOperations::lockCompareExchange(
                (volatile uintptr_t*)&_currentCleaningRange, (uintptr_t)currentRange, (uintptr_t)nextRange);
 ...
 *
 */
MMINLINE void MM_ConcurrentCardTable::setTLHMarkBits(MM_EnvironmentBase* env, uintptr_t slotIndex, uintptr_t slotBits)
{
 ...
 *
 */
MMINLINE void MM_ConcurrentCardTable::clearTLHMarkBits(MM_EnvironmentBase* env, uintptr_t slotIndex, uintptr_t slotBits)
{
    volatile uintptr_t* tlhMarkMapSlot = (uintptr_t*)&(_tlhMarkBits[slotIndex]);
    uintptr_t oldValue, newValue;
 ...
 *
 */
bool MM_ConcurrentCardTable::cardHasMarkedObjects(MM_EnvironmentBase* env, Card* card)
{
    bool hasMarked;
