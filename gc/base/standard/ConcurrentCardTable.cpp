 ...
 */
void
MM_ConcurrentCardTable::tlhRefreshed(J9HookInterface** hook,
                                     uintptr_t eventNum,
                                     void* eventData,
                                     void* userData)
{
 ...
 */
void
MM_ConcurrentCardTable::tlhCleared(J9HookInterface** hook,
                                   uintptr_t eventNum,
                                   void* eventData,
                                   void* userData)
{
 ...
 */
bool
MM_ConcurrentCardTable::allocateCardTableEntriesForHeapRange(
  MM_EnvironmentBase* env,
  ...
  void* highValidAddress)
{
  ...
  /* Commit the card table memory.  We must point to the address just being the
   * highCard to get the correct range */
  return decommitCardTableMemory(
    env, lowCard, highCard, lowValidCard, highValidCard);
}
 ...
 */
bool
MM_ConcurrentCardTable::allocateTLHMarkMapEntriesForHeapRange(
  MM_EnvironmentBase* env,
  MM_MemorySubSpace* subspace,
  uintptr_t size,
  void* lowAddress,
  void* highAddress)
{
  ...
  if (_extensions->isFvtestForceConcurrentTLHMarkMapCommitFailure()) {
    ...
    if (NULL != _tlhMarkBits) {
      ...
      commited =
        memoryManager->commitMemory(&_tlhMarkMapMemoryHandle,
    ...
    if (NULL != _tlhMarkBits) {
      ...
      if (0 != mapSize) {
        ...
        decommited = memoryManager->decommitMemory(
          ...
          (void*)&(_tlhMarkBits[lowTLHMarkMap]),
          mapSize,
          (NULL != lowValidAddress) ? (void*)&(_tlhMarkBits[lowValidTLHMarkMap])
                                    : NULL,
 ...
 */
bool
MM_ConcurrentCardTable::heapAddRange(MM_EnvironmentBase* env,
                                     MM_MemorySubSpace* subspace,
                                     uintptr_t size,
                                     void* lowAddress,
                                     void* highAddress,
                                     bool clearNewCards)
{
  ...
  if (result) {
    if (subspace->isConcurrentCollectable()) {
       ...
       */
      result = allocateTLHMarkMapEntriesForHeapRange(
 ...
 */
bool
MM_ConcurrentCardTable::heapRemoveRange(MM_EnvironmentBase* env,
                                        MM_MemorySubSpace* subspace,
                                        uintptr_t size,
                                        void* lowAddress,
                                        void* highAddress,
  ...
  if (NULL != _heapAlloc) {
    ...
    if (result) {
      if (subspace->isConcurrentCollectable()) {
         ...
         * actions required */
        result = freeTLHMarkMapEntriesForHeapRange(env,
                                                   size,
                                                   lowAddress,
                                                   highAddress,
                                                   lowValidAddress,
...
MM_ConcurrentCardTable::newInstance(MM_EnvironmentBase* env,
                                    MM_Heap* heap,
 ...
 */
uintptr_t
MM_ConcurrentCardTable::cardBytesForHeapRange(MM_EnvironmentBase* env,
                                              void* heapBase,
                                              void* heapTop)
{
 ...
 */
void
MM_ConcurrentCardTable::dirtyCardsInRange(MM_EnvironmentBase* env,
                                          void* heapBase,
                                          void* heapTop)
{
...
MM_ConcurrentCardTable::initializeCardCleaning(MM_EnvironmentBase* env)
{
  ...
  if (_extensions->cardCleaningPasses > 0) {
    MM_AtomicOperations::lockCompareExchangeU32(
      (volatile uint32_t*)&_cardCleanPhase,
 ...
 */
bool
MM_ConcurrentCardTable::cleanCards(MM_EnvironmentBase* env,
                                   bool isMutator,
                                   uintptr_t sizeToDo,
                                   uintptr_t* sizeDone,
                                   bool threadAtSafePoint)
{
  ...
  *sizeDone = 0;
  ...
  if (cardTableNeedsPreparing(currentCleaningPhase)) {
    ...
    if (getExclusiveCardTableAccess(
          env, currentCleaningPhase, threadAtSafePoint)) {
  ...
  while (cleanedSoFar < sizeToDo && currentCleaningPhase == _cardCleanPhase) {
    ...
    if (!cleanSingleCard(
          env, nextDirtyCard, (sizeToDo - cleanedSoFar), &cleanedSoFar)) {
      break;
    }
 ...
 */
bool
MM_ConcurrentCardTable::cleanSingleCard(MM_EnvironmentBase* env,
                                        Card* card,
 ...
 */
bool
MM_ConcurrentCardTable::finalCleanCards(MM_EnvironmentBase* env,
                                        uintptr_t* bytesTraced)
{
  ...
  for (; (nextDirtyCard = getNextDirtyCard(env, _finalCardCleanMask, false)) !=
     ...
     * addresses */
    MM_HeapMapIterator markedObjectIterator(
      _extensions, markMap, heapBase, heapTop);
    objects = 0;
    while (NULL != (objectPtr = markedObjectIterator.nextObject())) {
      objects += 1;
      traceCount +=
 ...
 */
void
MM_ConcurrentCardTable::processTLHMarkBits(MM_EnvironmentBase* env,
                                           MM_MemorySubSpace* subspace,
                                           void* tlhBase,
                                           void* tlhTop,
                                           BitMapAction action)
{
  ...
  if (base < top) {
    ...
    if (tailIndex > headIndex) {
      ...
      uintptr_t mask =
        (SET == action) ? TLH_MARKING_SET_ALL_BITS : TLH_MARKING_CLEAR_ALL_BITS;
 ...
 */
bool
MM_ConcurrentCardTable::isObjectInActiveTLH(MM_EnvironmentBase* env,
 ...
 */
bool
MM_ConcurrentCardTable::isObjectInDirtyCardNoCheck(MM_EnvironmentBase* env,
                                                   omrobjectptr_t object)
{
  bool dirty;
 ...
 */
bool
MM_ConcurrentCardTable::isObjectInUncleanedDirtyCard(MM_EnvironmentBase* env,
                                                     omrobjectptr_t object)
{
 ...
 */
bool
MM_ConcurrentCardTable::isObjectInDirtyCard(MM_EnvironmentBase* env,
                                            omrobjectptr_t object)
{
  bool dirty = false;
 ...
 */
Card*
MM_ConcurrentCardTable::getNextDirtyCard(MM_EnvironmentBase* env,
                                         Card cardMask,
                                         bool concurrentCardClean)
{
  ...
  while (NULL != firstCard) {
    ...
    if (currentCard < lastCardToClean) {
      ...
      MM_AtomicOperations::lockCompareExchange(
        (volatile uintptr_t*)&currentRange->nextCard,
        (uintptr_t)currentRange->nextCard,
        (uintptr_t)currentRange->topCard);

      /* Switch to next cleaning range */
      CleaningRange* nextRange = currentRange + 1;
      MM_AtomicOperations::lockCompareExchange(
        (volatile uintptr_t*)&_currentCleaningRange,
        (uintptr_t)currentRange,
        (uintptr_t)nextRange);
 ...
 */
MMINLINE void
MM_ConcurrentCardTable::setTLHMarkBits(MM_EnvironmentBase* env,
                                       uintptr_t slotIndex,
                                       uintptr_t slotBits)
{
 ...
 */
MMINLINE void
MM_ConcurrentCardTable::clearTLHMarkBits(MM_EnvironmentBase* env,
                                         uintptr_t slotIndex,
                                         uintptr_t slotBits)
{
  volatile uintptr_t* tlhMarkMapSlot = (uintptr_t*)&(_tlhMarkBits[slotIndex]);
 ...
 */
bool
MM_ConcurrentCardTable::cardHasMarkedObjects(MM_EnvironmentBase* env,
                                             Card* card)
{
  bool hasMarked;
