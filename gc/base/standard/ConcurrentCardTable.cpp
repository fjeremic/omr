 ...
 */
void MM_ConcurrentCardTable::tlhRefreshed(J9HookInterface **hook,
  ...
  ((MM_ConcurrentCardTable *)userData)
      ->processTLHMarkBits(env, (MM_MemorySubSpace *)event->subSpace,
                           event->cacheBase, event->cacheTop, SET);
}

/**
 * Called when a full TLH is cleared.
 * We use this broadcast event to trigger the update of the TLH mark bits and
 * set the ObjectMap bits
 * @see MM_ParallelGlobalGC::TLHRefreshed()
 */
void MM_ConcurrentCardTable::tlhCleared(J9HookInterface **hook,
                                        uintptr_t eventNum, void *eventData,
                                        void *userData) {
  ...
  ((MM_ConcurrentCardTable *)userData)
      ->processTLHMarkBits(env, (MM_MemorySubSpace *)event->subSpace,
                           event->cacheBase, event->cacheTop, CLEAR);
}

/**
 *						Card Table and TLH Mark Map
 *Sizing
 * 						==================================
 *
 */
 ...
 */
uintptr_t MM_ConcurrentCardTable::calculateTLHMarkMapSize(
    MM_EnvironmentBase *env, uintptr_t cardTableSize) {
  uintptr_t size;

  size = MM_Math::roundToCeiling(
      sizeof(uint32_t),
...
/**
 * Commit card table entries into memory that correspond to the newly added heap
 * range.
 *
 ...
 */
bool MM_ConcurrentCardTable::allocateCardTableEntriesForHeapRange(
    MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size,
  ...
  /* Commit the card table memory.  We must point to the address just being the
   * highCard to get the correct range */
...
/**
 * Decommit card table entries from memory that correspond to the removed heap
 * range.
 *
 ...
 * the heap
 */
bool MM_ConcurrentCardTable::freeCardTableEntriesForHeapRange(
    MM_EnvironmentBase *env, uintptr_t size, void *lowAddress,
  ...
  /* Commit the card table memory.  We must point to the address just being the
   * highCard to get the correct range */
  return decommitCardTableMemory(env, lowCard, highCard, lowValidCard,
                                 highValidCard);
}

/**
 * Commit the TLH mark map region into memory that correspond to the newly added
 * heap range.
 *
 * @param size The amount of memory added to the heap
 * @param lowAddress The base address of the memory added to the heap
 ...
 * heap
 */
bool MM_ConcurrentCardTable::allocateTLHMarkMapEntriesForHeapRange(
    MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size,
  ...
  if (_extensions->isFvtestForceConcurrentTLHMarkMapCommitFailure()) {
    commited = false;
    Trc_MM_ConcurrentCardTable_activeTLHMarkMapCommitFailureForced(
        env->getLanguageVMThread());
  } else {
    /* Do we have any TLH mark bits ? */
    if (NULL != _tlhMarkBits) {
      /* Determine the low and high entries which map from the heap address
       * ranges */
      /* We assume that expansion rules keep addresses on mark map boundaries */
      uintptr_t numericalHeapBase = (uintptr_t)getHeapBase();
      uintptr_t lowTLHMarkMap = (((uintptr_t)lowAddress) - numericalHeapBase) >>
                                TLH_MARKING_INDEX_SHIFT;
      uintptr_t highTLHMarkMap =
          (((uintptr_t)highAddress) - numericalHeapBase) >>
          TLH_MARKING_INDEX_SHIFT;

      /* Adjust the lowTLHMArkMap to account for rounding (since this is really
       * the highTLHMarkMap address of another range). as we dont want to commit
       * a slot we have previously commited andn more importantly we dont want
       * to clear and existing slot. */
      if ((lowTLHMarkMap << TLH_MARKING_INDEX_SHIFT) <
          (((uintptr_t)lowAddress) - numericalHeapBase)) {
        lowTLHMarkMap += 1;
      }

      /* Commit the tlh mark bit memory to span the low and high indexes just
       * calculated. However as we always commit multiples of uintptr_t we need
       * to check if the the above shift has rounded down the number of
       * uintptr_t slots we need to commit. If it has we need to commit an extra
       * uintptr_t worth otherwise will will end up trying to set TLH bits in
       * uncomitted memory.
       */
      if ((highTLHMarkMap << TLH_MARKING_INDEX_SHIFT) <
          (((uintptr_t)highAddress) - numericalHeapBase)) {
        highTLHMarkMap += 1;
      }
      ...
      commited = memoryManager->commitMemory(
          &_tlhMarkMapMemoryHandle, (void *)&(_tlhMarkBits[lowTLHMarkMap]),
          mapSize);
      ...
      if (commited) {
        ...
        Trc_MM_ConcurrentCardTable_activeTLHMarkMapCommitFailed(
            env->getLanguageVMThread(), (void *)&(_tlhMarkBits[lowTLHMarkMap]),
            mapSize);
      }
    }
  }
 ...
 * heap
 */
bool MM_ConcurrentCardTable::freeTLHMarkMapEntriesForHeapRange(
    MM_EnvironmentBase *env, uintptr_t size, void *lowAddress,
    void *highAddress, void *lowValidAddress, void *highValidAddress) {
  ...
  if (_extensions->isFvtestForceConcurrentTLHMarkMapDecommitFailure()) {
    decommited = false;
    Trc_MM_ConcurrentCardTable_activeTLHMarkMapDecommitFailureForced(
        env->getLanguageVMThread());
  } else {
    /* Do we have any TLH mark bits ? */
    if (NULL != _tlhMarkBits) {
      ...
      /* Decommit the tlh mark bit memory to span the low and high indexes just
       * calculated. However as we always decommit multiples of uintptr_t we
       * need to check if the the above shift has rounded down the number of
       * uintptr_t slots we need to commit. If it has we need to commit an extra
       * uintptr_t worth otherwise will will end up trying to set TLH bits in
       * uncomitted memory.
       */
      ...
      if (NULL != lowValidAddress) {
        lowValidTLHMarkMap =
            (((uintptr_t)lowValidAddress) - numericalHeapBase) >>
            TLH_MARKING_INDEX_SHIFT;
        /* Adjust the low valid range to account for rounding (since this is
         * really the high address of another range) */
        if ((lowValidTLHMarkMap << TLH_MARKING_INDEX_SHIFT) <
            (((uintptr_t)lowValidAddress) - numericalHeapBase)) {
      ...
      if (NULL != highValidAddress) {
        highValidTLHMarkMap =
            (((uintptr_t)highValidAddress) - numericalHeapBase) >>
            TLH_MARKING_INDEX_SHIFT;
      }
      ...
      if (0 != mapSize) {
        ...
        decommited = memoryManager->decommitMemory(
            &_tlhMarkMapMemoryHandle, (void *)&(_tlhMarkBits[lowTLHMarkMap]),
            mapSize,
            (NULL != lowValidAddress)
                ? (void *)&(_tlhMarkBits[lowValidTLHMarkMap])
                : NULL,
            (NULL != highValidAddress)
                ? (void *)&(_tlhMarkBits[highValidTLHMarkMap])
                : NULL);
        if (!decommited) {
          Trc_MM_ConcurrentCardTable_activeTLHMarkMapDecommitFailed(
              env->getLanguageVMThread(),
              (void *)&(_tlhMarkBits[lowTLHMarkMap]), mapSize,
              (NULL != lowValidAddress)
                  ? (void *)&(_tlhMarkBits[lowValidTLHMarkMap])
                  : NULL,
              (NULL != highValidAddress)
 ...
 */
bool MM_ConcurrentCardTable::heapAddRange(MM_EnvironmentBase *env,
  ...
  bool result = allocateCardTableEntriesForHeapRange(
      env, subspace, size, lowAddress, highAddress, clearNewCards);
  ...
  if (result) {
    if (subspace->isConcurrentCollectable()) {
       ...
       */
      result = allocateTLHMarkMapEntriesForHeapRange(env, subspace, size,
 ...
 *
 */
bool MM_ConcurrentCardTable::heapRemoveRange(MM_EnvironmentBase *env,
                                             MM_MemorySubSpace *subspace,
                                             uintptr_t size, void *lowAddress,
                                             void *highAddress,
  ...
  if (NULL != _heapAlloc) {
    ...
    /* Adjust card table  to reflect heap change */
    result = freeCardTableEntriesForHeapRange(
        env, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
    ...
    if (result) {
      if (subspace->isConcurrentCollectable()) {
         ...
         */
        /* If attempt to shrink TLH Mark Map fails return false, no other
         * actions required */
        result = freeTLHMarkMapEntriesForHeapRange(env, size, lowAddress,
 ...
 *
 */
void MM_ConcurrentCardTable::heapReconfigured(MM_EnvironmentBase *env) {}
 ...
 */
MM_ConcurrentCardTable *MM_ConcurrentCardTable::newInstance(
    MM_EnvironmentBase *env, MM_Heap *heap, MM_MarkingScheme *markingScheme,
    MM_ConcurrentGC *collector) {
  MM_ConcurrentCardTable *cardTable =
      (MM_ConcurrentCardTable *)env->getForge()->allocate(
          sizeof(MM_ConcurrentCardTable), OMR::GC::AllocationCategory::FIXED,
          OMR_GET_CALLSITE());
  if (NULL != cardTable) {
 ...
 */
bool MM_ConcurrentCardTable::initialize(MM_EnvironmentBase *env,
                                        MM_Heap *heap) {
  ...
  if (initialized) {
    ...
    {
      uintptr_t cardTableSizeRequired =
          calculateCardTableSize(env, heap->getMaximumPhysicalRange());
      /* Now instantiate the Virtual Memory object for the TLH Mark map */
      uintptr_t tlhMarkMapSizeRequired =
          calculateTLHMarkMapSize(env, cardTableSizeRequired);

      MM_MemoryManager *memoryManager = _extensions->memoryManager;
      if (!memoryManager->createVirtualMemoryForMetadata(
              env, &_tlhMarkMapMemoryHandle, sizeof(uintptr_t),
              tlhMarkMapSizeRequired)) {
        return false;
      }

      _tlhMarkBits =
          (uintptr_t *)(memoryManager->getHeapBase(&_tlhMarkMapMemoryHandle));

      /* Register hooks for TLH clear and refresh. Needed to maintian TLH mark
       * bits */
      (*mmPrivateHooks)
          ->J9HookRegisterWithCallSite(
              mmPrivateHooks, J9HOOK_MM_PRIVATE_CACHE_CLEARED, tlhCleared,
              OMR_GET_CALLSITE(), (void *)this);
      (*mmPrivateHooks)
          ->J9HookRegisterWithCallSite(
              mmPrivateHooks, J9HOOK_MM_PRIVATE_CACHE_REFRESHED, tlhRefreshed,
              OMR_GET_CALLSITE(), (void *)this);
    }
 ...
 *
 */
void MM_ConcurrentCardTable::tearDown(MM_EnvironmentBase *env) {
 ...
 */
uintptr_t MM_ConcurrentCardTable::cardBytesForHeapRange(MM_EnvironmentBase *env,
                                                        void *heapBase,
                                                        void *heapTop) {
 ...
 *
 * Iterate over all segments, and for any which are owned by active
 * non-concurrently collectable subspaces, e.g. allocate subspaces, clear the
 * corresponding cards in card table.
 *
 */
void MM_ConcurrentCardTable::clearNonConcurrentCards(MM_EnvironmentBase *env) {
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    ...
    // todo this is a single memset. We should really use helper threads to
    // speed it up
 ...
 */
void MM_ConcurrentCardTable::dirtyCardsInRange(MM_EnvironmentBase *env,
                                               void *heapBase, void *heapTop) {
 ...
 *
 */
void MM_ConcurrentCardTable::initializeCardCleaning(MM_EnvironmentBase *env) {
  initializeCardCleaningStatistics();

  if (_extensions->cardCleaningPasses > 0) {
    MM_AtomicOperations::lockCompareExchangeU32(
        (volatile uint32_t *)&_cardCleanPhase, (uint32_t)_cardCleanPhase,
 ...
 *
 */
void MM_ConcurrentCardTable::prepareCardsForCleaning(MM_EnvironmentBase *env) {
  void *firstFree;
  MM_MemorySubSpace *subspace;
  MM_Heap *heap = (MM_Heap *)_extensions->heap;
  uintptr_t currentFree =
  ...
  switch (_cardCleanPhase) {
    case PHASE1_PREPARING:
      ...
      /* Provided we are not already out of free storage we clean cards up to
       * start of free list */
      _lastCardInPhase =
          (NULL != firstFree) ? heapAddrToCardAddr(env, firstFree) : _lastCard;
      ...
      MM_AtomicOperations::lockCompareExchange(
          (volatile uintptr_t *)&_currentCleaningRange,
          (uintptr_t)_currentCleaningRange, (uintptr_t)_cleaningRanges);
      break;
 ...
 */
void MM_ConcurrentCardTable::reportCardCleanPass2Start(
    MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

  TRIGGER_J9HOOK_MM_PRIVATE_CARD_CLEANING_PASS_2_START(
      _extensions->privateHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CARD_CLEANING_PASS_2_START);
}
 ...
 */
MMINLINE bool MM_ConcurrentCardTable::getExclusiveCardTableAccess(
    MM_EnvironmentBase *env, CardCleanPhase currentPhase,
  ...
  if (!cardTableBeingPrepared(currentPhase)) {
    if (currentPhase ==
        (CardCleanPhase)MM_AtomicOperations::lockCompareExchangeU32(
            (volatile uint32_t *)&_cardCleanPhase, (uint32_t)currentPhase,
 ...
 */
MMINLINE void MM_ConcurrentCardTable::releaseExclusiveCardTableAccess(
    MM_EnvironmentBase *env) {
 ...
 *
 * @param isMutator TRUE if called on a mutator thread, FALSE if called on a
 * background thread
 ...
 */
bool MM_ConcurrentCardTable::cleanCards(MM_EnvironmentBase *env, bool isMutator,
                                        uintptr_t sizeToDo, uintptr_t *sizeDone,
                                        bool threadAtSafePoint) {
  Card *nextDirtyCard;
  ...
  *sizeDone = 0;
  ...
  if (cardTableNeedsPreparing(currentCleaningPhase)) {
    ...
    if (getExclusiveCardTableAccess(env, currentCleaningPhase,
                                    threadAtSafePoint)) {
      if (0 != _extensions->fvtest_concurrentCardTablePreparationDelay) {
        omrthread_sleep_interruptable(
            (I_64)_extensions->fvtest_concurrentCardTablePreparationDelay, 0);
      }
  ...
  if (gcCount != _extensions->globalGCStats.gcCount) {
    /* A gc has occured while attempting to acquire exclusive access to the card
     * table */
    return false;
  }
  ...
  while (cleanedSoFar < sizeToDo && currentCleaningPhase == _cardCleanPhase) {
     ...
     * we are done
     **/
    if ((Card *)NULL == nextDirtyCard ||
        (Card *)EXCLUSIVE_VMACCESS_REQUESTED == nextDirtyCard) {
      break;
    }

    /*
     * If the object is in an active TLH and provided no concurrent work stack
     *overflow has occurred then we are done as all live objects in the card
     *will be processed later. This is true as we know the object will have been
     ...
     *popped it, or (2) We have popped it and deferred it (re-pushed it to a
     *deferred packet).
     *
     * Either way we don't need to process any objects on this card now.
     *
     * If concurrent work stack overflow has occurred the above conditions do
     *not hold as to relieve work stack overflow we empty packets by dirtying
     *cards for their referenced objects. Therefore we cannot be sure tracing
     *into all active TLH's will be deferred.
     */
    if (isCardInActiveTLH(env, nextDirtyCard) &&
        !stats->getConcurrentWorkStackOverflowOcurred()) {
      continue;
    }
    ...
    if (!cleanSingleCard(env, nextDirtyCard, (sizeToDo - cleanedSoFar),
                         &cleanedSoFar)) {
      break;
    }
   ...
   *
   * Note: This will not be 100% accurate as _cardCleanPhase can change during a
   * call to getNextDirtyCard so we may count the occasional card in wrong phase
   * but the counts will be accurate enough for use currently made of them.
   */
  ...
  if (NULL == nextDirtyCard) {
    ...
    if (cardCleaningInProgress(currentCleaningPhase)) {
      MM_AtomicOperations::lockCompareExchangeU32(
          (volatile uint32_t *)&_cardCleanPhase, (uint32_t)currentCleaningPhase,
 ...
 */
bool MM_ConcurrentCardTable::cleanSingleCard(MM_EnvironmentBase *env,
                                             Card *card, uintptr_t bytesToClean,
                                             uintptr_t *totalBytesCleaned) {
  ...
  /* Ensure we at least start to clean last object in card by making sure tax to
   * pay at least as big as one cards worth. If last object is large pointer
   * array  we may only partially retrace it but it will be pushed so another
   * mutator can pick up where this thread left off.
   */
  ...
  MM_HeapMapIterator markedObjectIterator(
      _extensions, _markingScheme->getMarkMap(), heapBase, heapTop);

  /* Re-trace all objects which START in this card */
  MM_ConcurrentGCStats *stats = _collector->getConcurrentGCStats();
  while (NULL != (objectPtr = markedObjectIterator.nextObject())) {
    /* Check to see if another thread  is waiting for exclusive VM access. If so
     * get out quick.
     */
    ...
    /*
     * If the object is in an active TLH and provided no concurrent work stack
     *overflow has occurred then we are done as we know this object, and any
     *others in the same card,  will be processed later. This is true as we know
     *the object will have been pushed to a work packet when it was marked and
     *as its in a active TLH either:
     *
     *		 1) We have marked and pushed a reference to the object but not yet
     *popped it, or 2) We have popped it and deferred it (re-pushed it to a
     *deferred packet). Either way we don't need to process any objects on this
     *card now and we don't need to re-dirty the card as we know we will revisit
     *it again later, and all cards for an active TLH are cleared when TLH is
     *refreshed anyway.
     *
     * If concurrent work stack overflow has occurred the above conditions do
     *not hold to relieve work stack overflow we empty packets by dirtying cards
     *for their referenced objects. Therefore we cannot be sure all tracing into
     *this particular TLH will be deferred.
     */
    if (isObjectInActiveTLH(env, objectPtr) &&
        !(stats->getConcurrentWorkStackOverflowOcurred())) {
      return true;
    }
    ...
    sizeDone += _markingScheme->scanObject(
        env, objectPtr, SCAN_REASON_DIRTY_CARD, (sizeToDo - sizeDone));
  }
  ...
  *totalBytesCleaned += sizeDone;
   ...
   * so we re-visit any such objects later to trace nursery references. This
   * will mean we re-trace objects not in the RS but RS overflow is assumed to
   * be an exceptional circumstance.
   */
  if (rememberedObjectsFound &&
      (env->getExtensions()->isRememberedSetInOverflowState())) {
    *card = (Card)CARD_DIRTY;
  }
 ...
 *
 * Called by STW to do any necessary initialization prior to final card
 * cleaning.
 */
void MM_ConcurrentCardTable::initializeFinalCardCleaning(
    MM_EnvironmentBase *env) {
  if (_cardTableReconfigured) {
    determineCleaningRanges(env);
  } else {
 ...
 *
 */
bool MM_ConcurrentCardTable::finalCleanCards(MM_EnvironmentBase *env,
  ...
  for (; (nextDirtyCard = getNextDirtyCard(env, _finalCardCleanMask, false)) !=
         NULL;) {
    ...
    /* Then iterate over all marked objects in the heap between the two
     * addresses */
    MM_HeapMapIterator markedObjectIterator(_extensions, markMap, heapBase,
                                            heapTop);
    objects = 0;
    while (NULL != (objectPtr = markedObjectIterator.nextObject())) {
      objects += 1;
      traceCount +=
          _markingScheme->scanObject(env, objectPtr, SCAN_REASON_DIRTY_CARD);
    }
 ...
 * Set or clear bits within the TLH mark bit map. The bit map contains one bit
 * for every card in the card table and we only set those bits for cards which
 * are wholly contained within a TLH.
 *
 ...
 *
 */
void MM_ConcurrentCardTable::processTLHMarkBits(MM_EnvironmentBase *env,
                                                MM_MemorySubSpace *subspace,
                                                void *tlhBase, void *tlhTop,
                                                BitMapAction action) {
  ...
  assume((uintptr_t *)tlhTop > (uintptr_t *)tlhBase,
         "processTLHMarkBits(): Invalid tlh limits passed");

  void *heapBase = getHeapBase();
  assume((uintptr_t *)tlhBase >= (uintptr_t *)heapBase &&
             (uintptr_t *)tlhTop <= (uintptr_t *)_heapAlloc,
         "processTLHMarkBits(): tlh not within heap");
  ...
  if (base < top) {
     ...
     * occurred we cannot clear the cards as we cannot guarantee that all
     * tracing into them will have been deferred.
     *
     ...
     */
    if ((CLEAR == action) && !(_collector->getConcurrentGCStats()
                                   ->getConcurrentWorkStackOverflowOcurred())) {
    ...
    /* ..and finally calculate the index of the slot within the TLH mark bit map
     * for tlhBase */
    ...
    if (tailIndex > headIndex) {
      ...
      uintptr_t mask = (SET == action) ? TLH_MARKING_SET_ALL_BITS
                                       : TLH_MARKING_CLEAR_ALL_BITS;
    ...
    if (SET == action) {
      /* Because we must ensure we defer tracing into any object in an active
       * TLH we need a store barrier here so that the TLH mark bits are flushed
       * to main memory before we expose any objects in the new TLH. That way no
       * thread can possibly attempt to trace an object in a active TLH and find
       * the TLH bits still off.
       */
      MM_AtomicOperations::storeSync();
    }
 ...
 */
bool MM_ConcurrentCardTable::isCardTableEmpty(MM_EnvironmentBase *env) {
  bool empty = true;
 ...
 */
bool MM_ConcurrentCardTable::isTLHMarkBitsEmpty(MM_EnvironmentBase *env) {
  bool empty = true;
  ...
  uintptr_t numericalHeapBase = (uintptr_t)getHeapBase();
  while (NULL != (region = regionIterator.nextRegion())) {
    /* We only need to clear bits for segments owned by concurrently collectable
     * subspaces */
    if (region->getSubSpace()->isConcurrentCollectable()) {
      uintptr_t currentSlot =
          (((uintptr_t)region->getLowAddress()) - numericalHeapBase) >>
          TLH_MARKING_INDEX_SHIFT;
      uintptr_t endSlot =
          (((uintptr_t)region->getHighAddress()) - numericalHeapBase) >>
          TLH_MARKING_INDEX_SHIFT;

      /* Make sure we walk all bits; endSlot may have been rounded down and we
       * don't want to miss last few bits. We always round up the high address
       * when we commit and clear so this should be OK.
       */
      if ((endSlot << TLH_MARKING_INDEX_SHIFT) <
          (((uintptr_t)region->getHighAddress()) - numericalHeapBase)) {
        endSlot += 1;
      }
 ...
 * @return TRUE if reference is to an object within an active TLH; FALSE
 * otherwise
 */
bool MM_ConcurrentCardTable::isObjectInActiveTLH(MM_EnvironmentBase *env,
  ...
  if ((void *)object >= heapBase && (void *)object < _heapAlloc) {
    ...
    active =
        (_tlhMarkBits[markIndex] & getTLHMarkBitMask(markBit)) ? true : false;
  } else {
 ...
 */
bool MM_ConcurrentCardTable::isCardInActiveTLH(MM_EnvironmentBase *env,
                                               Card *card) {
  uintptr_t markIndex, markBit;
  bool active;
 ...
 * @return TRUE if reference is to an object within a dirty card; FALSE
 * otherwise
 */
bool MM_ConcurrentCardTable::isObjectInDirtyCardNoCheck(MM_EnvironmentBase *env,
                                                        omrobjectptr_t object) {
  bool dirty;
 ...
 */
bool MM_ConcurrentCardTable::isObjectInUncleanedDirtyCard(
    MM_EnvironmentBase *env, omrobjectptr_t object) {
  ...
  if (!isCardCleaningComplete() && (currentRange != _lastCleaningRange) &&
 ...
 * otherwise
 */
bool MM_ConcurrentCardTable::isObjectInDirtyCard(MM_EnvironmentBase *env,
                                                 omrobjectptr_t object) {
  bool dirty = false;
 ...
 *
 */
void MM_ConcurrentCardTable::determineCleaningRanges(MM_EnvironmentBase *env) {
  bool initDone = false;
  ...
  while (!initDone) {
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
      ...
      if (subspace->isActive() &&
          (_cleanAllCards || subspace->isConcurrentCollectable())) {
        numRanges += 1;

        /* If there is space in cleaningRanges array so add it... */
        if (numRanges <= _maxCleaningRanges) {
          nextRange->baseCard =
              heapAddrToCardAddr(env, region->getLowAddress());
          nextRange->topCard =
              heapAddrToCardAddr(env, region->getHighAddress());
          nextRange->nextCard = nextRange->baseCard;
          nextRange->numCards =
              (uintptr_t)(nextRange->topCard - nextRange->baseCard);
    ...
    if (numRanges > _maxCleaningRanges) {
      ...
      _cleaningRanges = (CleaningRange *)env->getForge()->allocate(
          sizeRequired, OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
 ...
 *
 */
void MM_ConcurrentCardTable::resetCleaningRanges(MM_EnvironmentBase *env) {
  for (CleaningRange *range = _cleaningRanges; range < _lastCleaningRange;
       range++) {
 ...
 */
Card *MM_ConcurrentCardTable::getNextDirtyCard(MM_EnvironmentBase *env,
                                               Card cardMask,
                                               bool concurrentCardClean) {
  ...
  while (NULL != firstCard) {
     ...
     */
    /* CMVC 132231 - cache _lastCardInPhase since it's volatile and min reads
     * its arguments twice */
    ...
    for (currentCard = firstCard; currentCard < lastCardToClean;
         currentCard++) {
      /* Are we are on an uintptr_t boundary? If so scan the card table a
       * uintptr_t at a time until we find a slot which is non-zero or the end
       * of card table found. This is based on the premise that the card table
       * will be mostly empty and scanning an uintptr_t at a time will reduce
       * the time taken to scan the card table.
       */
      if (((Card)CARD_CLEAN == *currentCard) &&
          (0 == (uintptr_t)currentCard % sizeof(uintptr_t))) {
        uintptr_t *nextSlot = (uintptr_t *)currentCard;
        /* Last card may be in middle of a slot so only scan up to an including
         *last complete slots worth of cards; then go card at a time
         **/
        uintptr_t *lastSlot = (uintptr_t *)MM_Math::roundToFloor(
            sizeof(uintptr_t), (uintptr_t)lastCardToClean);
        while ((nextSlot < lastSlot) && (SLOT_ALL_CLEAN == *nextSlot)) {
          nextSlot += 1;
        }
        /*
         * Either end of scan or a slot which contains a dirty card found. Reset
         * scan ptr
         */
      ...
      if (firstCard != (Card *)currentRange->nextCard) {
        ...
        /* Update next card to clean for next caller of getNextDirtyCard. If we
         * fail then someone beat us to it so re-sync with race winner and start
         * again
         */
        if (firstCard != (Card *)MM_AtomicOperations::lockCompareExchange(
                             (volatile uintptr_t *)&currentRange->nextCard,
                             (uintptr_t)firstCard, (uintptr_t)currentCard)) {
          break;
        }
    ...
    /* We get here if we break out of FOR loop when another thread beat us to
     * next dirty card or we reach then end of the card table.
     *
     * Did we reach end of card table segment ?
     */
    if (currentCard < lastCardToClean) {
      /* No..so someone must have beat us to next dirty card. In which case we
       * need to restart scan. First though make sure no thread is waiting for
       * exclusive access.
       */
      if (concurrentCardClean && env->isExclusiveAccessRequestWaiting()) {
        return (Card *)EXCLUSIVE_VMACCESS_REQUESTED;
      ...
      /* Range complete so set nextCard of cleaning range to top card to show
       * cleaning range finsished */
      MM_AtomicOperations::lockCompareExchange(
          (volatile uintptr_t *)&currentRange->nextCard,
          (uintptr_t)currentRange->nextCard, (uintptr_t)currentRange->topCard);

      /* Switch to next cleaning range */
      CleaningRange *nextRange = currentRange + 1;
      MM_AtomicOperations::lockCompareExchange(
          (volatile uintptr_t *)&_currentCleaningRange, (uintptr_t)currentRange,
          (uintptr_t)nextRange);

      currentRange = (CleaningRange *)_currentCleaningRange;
      firstCard = currentRange < _lastCleaningRange
                      ? (Card *)currentRange->nextCard
                      : (Card *)NULL;
    } else {
      /* We have reached the last card to be processed in this phase
       * of card cleaning. We must update _next card to clean so when we start
       * to clean cards in next phase we start at last card in this phase. If we
       * fail then another thread beat us to it
       */
      if (firstCard != (Card *)MM_AtomicOperations::lockCompareExchange(
                           (volatile uintptr_t *)&currentRange->nextCard,
                           (uintptr_t)firstCard, (uintptr_t)currentCard)) {
 ...
 *
 */
MMINLINE void MM_ConcurrentCardTable::setTLHMarkBits(MM_EnvironmentBase *env,
                                                     uintptr_t slotIndex,
                                                     uintptr_t slotBits) {
  ...
  } while (oldValue != MM_AtomicOperations::lockCompareExchange(
                           tlhMarkMapSlot, oldValue, newValue));
}
 ...
 *
 */
MMINLINE void MM_ConcurrentCardTable::clearTLHMarkBits(MM_EnvironmentBase *env,
                                                       uintptr_t slotIndex,
                                                       uintptr_t slotBits) {
  volatile uintptr_t *tlhMarkMapSlot = (uintptr_t *)&(_tlhMarkBits[slotIndex]);
  uintptr_t oldValue, newValue;
  /* All bits to be cleared should already be set */
  assume((*tlhMarkMapSlot & slotBits) == slotBits,
         "TLH mark bits to be cleared not set!!");
 ...
 *
 */
bool MM_ConcurrentCardTable::cardHasMarkedObjects(MM_EnvironmentBase *env,
                                                  Card *card) {
  bool hasMarked;
