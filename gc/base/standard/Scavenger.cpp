...
extern "C" {
uintptr_t
allocateMemoryForSublistFragment(void *vmThreadRawPtr,
                                 J9VMGC_SublistFragment *fragmentPrimitive);
 ...
 */
MMINLINE bool
MM_Scavenger::copyAndForward(MM_EnvironmentStandard *env,
                             volatile omrobjectptr_t *objectPtrIndirect) {
  bool toReturn = false;
...
MMINLINE bool
MM_Scavenger::scavengeObjectSlots(MM_EnvironmentStandard *env,
...
#if defined(OMR_SCAVENGER_TRACE)
        omrtty_printf("{SCAV: slaveID %zu _cachedEntryCount %zu _waitingCount "
                      "%zu Scan cache from list (%p)}\n",
                      env->getSlaveID(), _cachedEntryCount, _waitingCount,
                      cache);
#endif /* OMR_SCAVENGER_TRACE */
 ...
 */
MMINLINE bool
MM_Scavenger::scavengeRememberedObject(MM_EnvironmentStandard *env,
                                       omrobjectptr_t objectPtr) {
  bool shouldBeRemembered = scavengeObjectSlots(
...
MM_Scavenger::createCacheInHeap(MM_EnvironmentStandard *env) {
...
#if defined(OMR_SCAVENGER_TRACE)
      omrtty_printf("{SCAV: Temporary chunk of scan cache headers allocated in "
                    "Survivor (%p)}\n",
                    cache);
#endif /* OMR_SCAVENGER_TRACE */
    }
  ...
  if (NULL == cache) {
#if defined(OMR_SCAVENGER_TRACE)
    omrtty_printf("{SCAV: Temporary chunk of scan cache headers can not be "
                  "allocated in Heap!}\n");
#endif /* OMR_SCAVENGER_TRACE */
  }
...
MM_CopyScanCacheStandard *
MM_Scavenger::releaseLocalCopyCache(MM_EnvironmentStandard *env,
                                    MM_CopyScanCacheStandard *cache) {
...
void MM_Scavenger::setBackOutFlag(MM_EnvironmentBase *env,
  ...
  if (_extensions->getScavengerBackOutState() != backOutState) {
    ...
    if (backOutStarted > backOutState) {
      Trc_MM_ScavengerBackout(env->getLanguageVMThread(),
                              backOutFlagCleared < backOutState ? "true"
                                                                : "false");
