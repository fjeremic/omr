...
/* OMRTODO temporary workaround to allow both ut_j9mm.h and ut_omrmm.h to be
 * included. Dependency on ut_j9mm.h should be removed in the future.
 */
...
/* VM Design 1774: Ideally we would pull these cache line values from the port
 * library but this will suffice for a quick implementation
 */
...
#define HOTFIELD_ALIGNMENT_BIAS(descriptor, heapObjectAlignment) \
  (((descriptor) >> 1) * (heapObjectAlignment))

extern "C" {
uintptr_t allocateMemoryForSublistFragment(
    void *vmThreadRawPtr, J9VMGC_SublistFragment *fragmentPrimitive);
#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
void oldToOldReferenceCreated(MM_EnvironmentBase *env,
                              omrobjectptr_t objectPtr);
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */
}

uintptr_t MM_Scavenger::getVMStateID() {
  return OMRVMSTATE_GC_COLLECTOR_SCAVENGER;
}

void MM_Scavenger::hookGlobalCollectionStart(J9HookInterface **hook,
                                             uintptr_t eventNum,
                                             void *eventData, void *userData) {
  MM_GlobalGCStartEvent *event = (MM_GlobalGCStartEvent *)eventData;
  MM_EnvironmentBase *env =
      MM_EnvironmentBase::getEnvironment(event->currentThread);

  ((MM_Scavenger *)userData)->globalCollectionStart(env);
}

void MM_Scavenger::hookGlobalCollectionComplete(J9HookInterface **hook,
                                                uintptr_t eventNum,
                                                void *eventData,
                                                void *userData) {
 ...
 */
void *MM_Scavenger::createSweepPoolState(MM_EnvironmentBase *env,
 ...
 */
void MM_Scavenger::deleteSweepPoolState(MM_EnvironmentBase *env,
                                        void *sweepPoolState) {
  Assert_MM_unreachable();
}
 ...
 */
MM_Scavenger *MM_Scavenger::newInstance(MM_EnvironmentStandard *env,
                                        MM_HeapRegionManager *regionManager) {
  MM_Scavenger *scavenger;

  scavenger = (MM_Scavenger *)env->getForge()->allocate(
      sizeof(MM_Scavenger), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (scavenger) {
 ...
 */
void MM_Scavenger::kill(MM_EnvironmentBase *env) {
  MM_EnvironmentStandard *envStandard =
 ...
 * Initialization
 */
bool MM_Scavenger::initialize(MM_EnvironmentBase *env) {
  J9HookInterface **mmOmrHooks =
      J9_HOOK_INTERFACE(_extensions->omrHookInterface);

  /* Register hook for global GC end. */
  (*mmOmrHooks)
      ->J9HookRegisterWithCallSite(mmOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_START,
                                   hookGlobalCollectionStart,
                                   OMR_GET_CALLSITE(), (void *)this);
  (*mmOmrHooks)
      ->J9HookRegisterWithCallSite(mmOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_END,
                                   hookGlobalCollectionComplete,
                                   OMR_GET_CALLSITE(), (void *)this);
  ...
  if (omrthread_monitor_init_with_name(&_scanCacheMonitor, 0,
                                       "MM_Scavenger::scanCacheMonitor")) {
    return false;
  }

  /* do not spin when acquiring monitor to notify blocking thread about new work
   */
  ((J9ThreadAbstractMonitor *)_scanCacheMonitor)->flags &=
      ~J9THREAD_MONITOR_TRY_ENTER_SPIN;

  if (omrthread_monitor_init_with_name(&_freeCacheMonitor, 0,
                                       "MM_Scavenger::freeCacheMonitor")) {
    return false;
  }

  /* No thread can use more than _cachesPerThread cache entries at 1 time (flip,
   * tenure, scan, large, possibly deferred) So long as (N * _cachesPerThread)
   * cache entries exist,the head of the scan list will contain a valid entry.
   * We set the appropriate number of caches per thread here */
   ...
   *  Xmnx >= 4GB			---> 256MB
   */
  uintptr_t incrementNewSpaceSize =
      OMR_MAX(_extensions->maxNewSpaceSize / 16, 32 * 1024 * 1024);
  incrementNewSpaceSize =
      OMR_MIN(incrementNewSpaceSize, _extensions->maxNewSpaceSize);
  incrementNewSpaceSize = OMR_MIN(incrementNewSpaceSize, 256 * 1024 * 1024);

  uintptr_t incrementCacheCount =
      incrementNewSpaceSize / _extensions->scavengerScanCacheMinimumSize;
  uintptr_t totalActiveCacheCount =
  ...
  if (!_scavengeCacheFreeList.resizeCacheEntries(env, totalActiveCacheCount,
                                                 incrementCacheCount)) {
    return false;
  }
...
void MM_Scavenger::tearDown(MM_EnvironmentBase *env) {
  ...
  (*mmOmrHooks)
      ->J9HookUnregister(mmOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_START,
                         hookGlobalCollectionStart, (void *)this);
  (*mmOmrHooks)
      ->J9HookUnregister(mmOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_END,
                         hookGlobalCollectionComplete, (void *)this);
}

/**
 * Perform any collector initialization particular to the concurrent collector.
 */
bool MM_Scavenger::collectorStartup(MM_GCExtensionsBase *extensions) {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  if (_extensions->concurrentScavenger) {
 ...
 * Currently this just involves stopping the concurrent background helper
 * threads.
 */
void MM_Scavenger::collectorShutdown(MM_GCExtensionsBase *extensions) {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  if (_extensions->concurrentScavenger) {
...
void MM_Scavenger::setupForGC(MM_EnvironmentBase *env) {}

void MM_Scavenger::masterSetupForGC(MM_EnvironmentStandard *env) {
  ...
  /* Cache heap ranges for fast "valid object" checks (this can change in an
   * expanding heap situation, so we refetch every cycle) */
  ...
  uintptr_t regionSize =
      _extensions->heap->getHeapRegionManager()->getRegionSize();
  Assert_MM_true((0 != regionSize) &&
                 (0 == ((uintptr_t)_heapBase % regionSize)));

  /* Clear the cycle gc statistics. Increment level stats will be cleared just
   * prior to increment start. */
  clearCycleGCStats(env);

  /* invoke language-specific interface callback */
  _delegate.masterSetupForGC(env);

  /* Allow expansion in the tenure area on failed promotions (but no resizing on
   * the semispace) */
  _expandTenureOnFailedAllocate = true;
  _activeSubSpace =
      (MM_MemorySubSpaceSemiSpace *)(env->_cycleState->_activeSubSpace);
  ...
  /* Find tenure memory sub spaces for collection ( allocate and survivor are
   * context specific) */
  ...
  scavengerStats->_tenureSpaceAllocBytesAcumulation +=
      heapStatsTenureSpace._allocBytes;
  scavengerStats->_semiSpaceAllocBytesAcumulation +=
      heapStatsSemiSpace._allocBytes;

  /* Check if scvTenureAdaptiveTenureAge has not been initialized or forced (by
   * cmdline option) */
  if (0 == _extensions->scvTenureAdaptiveTenureAge) {
    /* With larger initial Nursery sizes, we'll reduce initial tenure age, to
     * help promote peristant object sooner. */
    _extensions->scvTenureAdaptiveTenureAge = OMR_OBJECT_HEADER_AGE_DEFAULT;

    uintptr_t tenureAgeAdjustement = MM_Math::floorLog2(
        _extensions->heap->getActiveMemorySize(MEMORY_TYPE_NEW) /
        MINIMUM_NEW_SPACE_SIZE);
  ...
  _activeSubSpace->cacheRanges(_evacuateMemorySubSpace, &_evacuateSpaceBase,
                               &_evacuateSpaceTop);
  _activeSubSpace->cacheRanges(_survivorMemorySubSpace, &_survivorSpaceBase,
                               &_survivorSpaceTop);

  /* assume that value of RS Overflow flag will not be changed until
   * scavengeRememberedSet() call, so handle it first */
  _isRememberedSetInOverflowAtTheBeginning = isRememberedSetInOverflowState();
  _extensions->rememberedSet.startProcessingSublist();
}

void MM_Scavenger::workerSetupForGC(MM_EnvironmentStandard *env) {
  ...
  env->_scavengerRememberedSet.fragmentSize =
      (uintptr_t)OMR_SCV_REMSET_FRAGMENT_SIZE;
 ...
 * Run a scavenge.
 */
void MM_Scavenger::scavenge(MM_EnvironmentBase *envBase) {
  MM_EnvironmentStandard *env = MM_EnvironmentStandard::getEnvironment(envBase);
  MM_ParallelScavengeTask scavengeTask(env, _dispatcher, this,
                                       env->_cycleState);
...
void MM_Scavenger::reportScavengeStart(MM_EnvironmentStandard *env) {
...
void MM_Scavenger::reportScavengeEnd(MM_EnvironmentStandard *env,
                                     bool lastIncrement) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  ...
  if (lastIncrement) {
    ...
    Trc_MM_Tiltratio(env->getLanguageVMThread(),
                     _extensions->scavengerStats._tiltRatio);
  }

  TRIGGER_J9HOOK_MM_PRIVATE_SCAVENGE_END(
      _extensions->privateHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_PRIVATE_SCAVENGE_END,
      env->_cycleState->_activeSubSpace, lastIncrement);
}

void MM_Scavenger::reportGCStart(MM_EnvironmentStandard *env) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  ...
  TRIGGER_J9HOOK_MM_OMR_LOCAL_GC_START(
      _extensions->omrHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_OMR_LOCAL_GC_START,
      _extensions->globalGCStats.gcCount, _extensions->scavengerStats._gcCount);
}

void MM_Scavenger::reportGCEnd(MM_EnvironmentStandard *env) {
  ...
  Trc_MM_LocalGCEnd(
      ...
      (_extensions->largeObjectArea
           ? _extensions->heap->getApproximateActiveFreeLOAMemorySize(
                 MEMORY_TYPE_OLD)
           : (uintptr_t)0),
      (_extensions->largeObjectArea
           ? _extensions->heap->getActiveLOAMemorySize(MEMORY_TYPE_OLD)
           : (uintptr_t)0),
      _extensions->incrementScavengerStats._tenureAge);

  Trc_OMRMM_LocalGCEnd(
      env->getOmrVMThread(),
      _extensions->incrementScavengerStats._rememberedSetOverflow,
      _extensions->incrementScavengerStats._causedRememberedSetOverflow,
      _extensions->incrementScavengerStats._scanCacheOverflow,
  ...
  TRIGGER_J9HOOK_MM_OMR_LOCAL_GC_END(
      _extensions->omrHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_OMR_LOCAL_GC_END,
      env->_cycleState->_activeSubSpace, _extensions->globalGCStats.gcCount,
...
void MM_Scavenger::clearThreadGCStats(MM_EnvironmentBase *env,
                                      bool firstIncrement) {
  env->_scavengerStats.clear(firstIncrement);
}

void MM_Scavenger::clearIncrementGCStats(MM_EnvironmentBase *env,
                                         bool firstIncrement) {
  _extensions->incrementScavengerStats.clear(firstIncrement);
}

void MM_Scavenger::clearCycleGCStats(MM_EnvironmentBase *env) {
  _extensions->scavengerStats.clear(true);
}

void MM_Scavenger::mergeGCStatsBase(MM_EnvironmentBase *env,
                                    MM_ScavengerStats *finalGCStats,
                                    MM_ScavengerStats *scavStats) {
  finalGCStats->_rememberedSetOverflow |= scavStats->_rememberedSetOverflow;
  finalGCStats->_causedRememberedSetOverflow |=
      scavStats->_causedRememberedSetOverflow;
  finalGCStats->_scanCacheOverflow |= scavStats->_scanCacheOverflow;
  finalGCStats->_scanCacheAllocationFromHeap |=
      scavStats->_scanCacheAllocationFromHeap;
  ...
  finalGCStats->_failedTenureLargest = OMR_MAX(
      scavStats->_failedTenureLargest, finalGCStats->_failedTenureLargest);
  ...
  finalGCStats->_survivorTLHRemainderCount +=
      scavStats->_survivorTLHRemainderCount;
  finalGCStats->_tenureTLHRemainderCount += scavStats->_tenureTLHRemainderCount;

  finalGCStats->_semiSpaceAllocationCountLarge +=
      scavStats->_semiSpaceAllocationCountLarge;
  finalGCStats->_semiSpaceAllocationCountSmall +=
      scavStats->_semiSpaceAllocationCountSmall;

  finalGCStats->_tenureSpaceAllocationCountLarge +=
      scavStats->_tenureSpaceAllocationCountLarge;
  finalGCStats->_tenureSpaceAllocationCountSmall +=
      scavStats->_tenureSpaceAllocationCountSmall;

  /* TODO: Fix this. Not true when merging Master GC threads stats for standard
     (non CS) Scavenger. Assert_MM_true(finalGCStats->_flipHistoryNewIndex ==
     scavStats->_flipHistoryNewIndex); */

  for (int i = 1; i <= OBJECT_HEADER_AGE_MAX + 1; ++i) {
    finalGCStats->getFlipHistory(0)->_flipBytes[i] +=
        scavStats->getFlipHistory(0)->_flipBytes[i];
    finalGCStats->getFlipHistory(0)->_tenureBytes[i] +=
        scavStats->getFlipHistory(0)->_tenureBytes[i];
  }
  ...
  for (uintptr_t i = 0; i < OMR_SCAVENGER_DISTANCE_BINS; i++) {
    finalGCStats->_copy_distance_counts[i] +=
        scavStats->_copy_distance_counts[i];
  }
#endif /* OMR_SCAVENGER_TRACK_COPY_DISTANCE */
  for (uintptr_t i = 0; i < OMR_SCAVENGER_CACHESIZE_BINS; i++) {
    finalGCStats->_copy_cachesize_counts[i] +=
        scavStats->_copy_cachesize_counts[i];
  }
...
void MM_Scavenger::mergeThreadGCStats(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_OMRVM(_omrVM);

  /* Protect the merge with the mutex (this is done by multiple threads in the
   * parallel collector) */
  ...
  /* Merge language specific statistics. No known interesting data per increment
   * - they are merged directly to aggregate cycle stats */
  _delegate.mergeGCStats_mergeLangStats(env);

  omrthread_monitor_exit(_extensions->gcStatsMutex);

  /* record the thread-specific parallelism stats in the trace buffer. This
   * aprtially duplicates info in -Xtgc:parallel */
  Trc_MM_ParallelScavenger_parallelStats(
      env->getLanguageVMThread(), (uint32_t)env->getSlaveID(),
      (uint32_t)omrtime_hires_delta(0, scavStats->_workStallTime,
                                    OMRPORT_TIME_DELTA_IN_MILLISECONDS),
      (uint32_t)omrtime_hires_delta(0, scavStats->_completeStallTime,
                                    OMRPORT_TIME_DELTA_IN_MILLISECONDS),
      (uint32_t)omrtime_hires_delta(0, scavStats->_syncStallTime,
                                    OMRPORT_TIME_DELTA_IN_MILLISECONDS),
      ...
      scavStats->_releaseFreeListCount, scavStats->_acquireScanListCount,
      scavStats->_releaseScanListCount);
}

void MM_Scavenger::mergeIncrementGCStats(MM_EnvironmentBase *env,
                                         bool lastIncrement) {
  ...
  /* Language specific stats were supposed to be merged directly from thread
   * local to cycle global. No need to merge them here. */
  ...
  if (lastIncrement) {
    ...
    MM_ScavengerStats::FlipHistory *flipHistoryPrevious =
        finalGCStats->getFlipHistory(1);
    flipHistoryPrevious->_flipBytes[0] =
        finalGCStats->_semiSpaceAllocBytesAcumulation;
    flipHistoryPrevious->_tenureBytes[0] =
        finalGCStats->_tenureSpaceAllocBytesAcumulation;
 ...
 * @return true if GC stats should be calculated for this round, false
 * otherwise.
 */
bool MM_Scavenger::canCalcGCStats(MM_EnvironmentStandard *env) {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  ...
  /* If no backout and we actually did a scavenge this time around then it's
   * safe to gather stats */
  canCalculate &=
      (!isBackOutFlagRaised() &&
       (0 <
 ...
 */
void MM_Scavenger::calcGCStats(MM_EnvironmentStandard *env) {
  ...
  if (canCalcGCStats(env)) {
    ...
    uintptr_t initialFree =
        env->_cycleState->_activeSubSpace->getActualActiveFreeMemorySize();
    uintptr_t tenureAggregateBytes = 0;
    float tenureBytesDeviation = 0;

    /* First collection  ? */
    if (scavengerGCStats->_gcCount > 1) {
      scavengerGCStats->_avgInitialFree = (uintptr_t)MM_Math::weightedAverage(
          (float)scavengerGCStats->_avgInitialFree, (float)initialFree,
          INITIAL_FREE_HISTORY_WEIGHT);
      ...
      scavengerGCStats->_avgTenureLOABytes =
          (uintptr_t)MM_Math::weightedAverage(
              (float)scavengerGCStats->_avgTenureLOABytes,
              (float)scavengerGCStats->_tenureLOABytes,
              TENURE_BYTES_HISTORY_WEIGHT);
#else  /* OMR_GC_LARGE_OBJECT_AREA */
      tenureAggregateBytes = scavengerGCStats->_tenureAggregateBytes;
#endif /* OMR_GC_LARGE_OBJECT_AREA */
      scavengerGCStats->_avgTenureBytes = (uintptr_t)MM_Math::weightedAverage(
          (float)scavengerGCStats->_avgTenureBytes, (float)tenureAggregateBytes,
          TENURE_BYTES_HISTORY_WEIGHT);
      tenureBytesDeviation =
          (float)tenureAggregateBytes - scavengerGCStats->_avgTenureBytes;
      scavengerGCStats->_avgTenureBytesDeviation =
          (uintptr_t)MM_Math::weightedAverage(
              (float)scavengerGCStats->_avgTenureBytesDeviation,
              MM_Math::abs(tenureBytesDeviation), TENURE_BYTES_HISTORY_WEIGHT);
    } else {
      scavengerGCStats->_avgInitialFree = initialFree;

      /* We can assume that in the first GC, about half of the objects are long
       * lived objects, so we use this heuristic to give a rough estimate for
       * the starting point */
      scavengerGCStats->_avgTenureBytes =
          (uintptr_t)(scavengerGCStats->_flipBytes / 2);
    }
#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
    if (_extensions->debugConcurrentMark) {
      ...
      omrtty_printf(
          ...
          tenureAggregateBytes, scavengerGCStats->_avgTenureBytes,
          tenureBytesDeviation, scavengerGCStats->_avgTenureBytesDeviation);
    }
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */
  }
}
...
uintptr_t MM_Scavenger::calculateCopyScanCacheSizeForWaitingThreads(
...
uintptr_t MM_Scavenger::calculateCopyScanCacheSizeForQueueLength(
    uintptr_t maxCacheSize, uintptr_t threadCount, uintptr_t scanCacheCount) {
  uintptr_t minCacheSize = _extensions->scavengerScanCacheMinimumSize;
  uintptr_t range = maxCacheSize - minCacheSize;
  uintptr_t cacheSize =
      minCacheSize + ((range / threadCount) * (scanCacheCount + 1));

  return MM_Math::roundToCeiling(_extensions->getObjectAlignmentInBytes(),
                                 cacheSize);
}
 ...
 *
 * Perform the hot path checks inline but if calculations are required call
 * helper functions.
...
MM_Scavenger::calculateOptimumCopyScanCacheSize(MM_EnvironmentStandard *env) {
  ...
  if (waitingThreads > 0) {
    uintptr_t cacheSizeBasedOnWaitingCount =
        calculateCopyScanCacheSizeForWaitingThreads(maxCacheSize, threadCount,
                                                    waitingThreads);
  ...
  if (env->approxScanCacheCount < threadCount) {
    uintptr_t cacheSizeBasedOnScanCacheCount =
        calculateCopyScanCacheSizeForQueueLength(maxCacheSize, threadCount,
                                                 env->approxScanCacheCount);
...
MM_Scavenger::reserveMemoryForAllocateInSemiSpace(
    MM_EnvironmentStandard *env, omrobjectptr_t objectToEvacuate,
  ...
  /*
   * Please note that condition like (top >= start + size) might cause wrong
   * functioning due overflow so to be safe (top - start >= size) must be used
   */
  if ((NULL != env->_survivorCopyScanCache) &&
      (((uintptr_t)env->_survivorCopyScanCache->cacheTop -
        (uintptr_t)env->_survivorCopyScanCache->cacheAlloc) >= cacheSize)) {
    /* A survivor copy scan cache exists and there is a room, use the current
     * copy cache */
    ...
    if (objectReserveSizeInBytes < _minSemiSpaceFailureSize) {
      ...
      if (((uintptr_t)env->_survivorTLHRemainderTop -
           (uintptr_t)env->_survivorTLHRemainderBase) >= cacheSize) {
        Assert_MM_true(NULL != env->_survivorTLHRemainderBase);
        allocateResult = true;
        ...
        addrBase = _survivorMemorySubSpace->collectorAllocate(
            env, this, &allocDescription);
        if (NULL != addrBase) {
        ...
        allocateResult = (NULL != _survivorMemorySubSpace->collectorAllocateTLH(
                                      env, this, &allocDescription,
                                      scanCacheSize, addrBase, addrTop));
    ...
    if (allocateResult) {
      ...
      /* release local cache first. along the path we may realize that a cache
      ...
      if (NULL != copyCache) {
        ...
        copyCache->flags |=
            OMR_SCAVENGER_CACHE_TYPE_SEMISPACE | OMR_SCAVENGER_CACHE_TYPE_COPY;
       ...
       * NOTE: Since this is used across multiple threads there is a race
       * condition between checking and setting the minimum.  This means that
       * this value may not actually be the lowest value, or may increase.
       */
...
MM_CopyScanCacheStandard *MM_Scavenger::reserveMemoryForAllocateInTenureSpace(
    MM_EnvironmentStandard *env, omrobjectptr_t objectToEvacuate,
    uintptr_t objectReserveSizeInBytes) {
  void *addrBase = NULL;
  void *addrTop = NULL;
  ...
  if ((NULL != env->_tenureCopyScanCache) &&
      (((uintptr_t)env->_tenureCopyScanCache->cacheTop -
        (uintptr_t)env->_tenureCopyScanCache->cacheAlloc) >= cacheSize)) {
    /* A tenure copy scan cache exists and there is a room, use the current copy
     * cache */
    ...
    if (cacheSize < _minTenureFailureSize) {
      /* try to use TLH remainder from previous discard. */
      if (((uintptr_t)env->_tenureTLHRemainderTop -
           (uintptr_t)env->_tenureTLHRemainderBase) >= cacheSize) {
        ...
        addrBase = _tenureMemorySubSpace->collectorAllocate(env, this,
                                                            &allocDescription);
        if (NULL != addrBase) {
          addrTop = (void *)(((uint8_t *)addrBase) + cacheSize);
          /* Check that there is no overflow */
        ...
        allocDescription.setCollectorAllocateExpandOnFailure(true);
        uintptr_t scanCacheSize = calculateOptimumCopyScanCacheSize(env);
        allocateResult = (NULL != _tenureMemorySubSpace->collectorAllocateTLH(
                                      env, this, &allocDescription,
                                      scanCacheSize, addrBase, addrTop));
    ...
    if (allocateResult) {
       ...
       * structure can be re-used */
      MM_CopyScanCacheStandard *cacheToReuse =
          releaseLocalCopyCache(env, env->_tenureCopyScanCache);
      ...
      env->_scavengerStats._failedTenureLargest = OMR_MAX(
          objectReserveSizeInBytes, env->_scavengerStats._failedTenureLargest);
    }
  }
...
/**
 * Update the given slot to point at the new location of the object, after
 * copying the object if it was not already. Attempt to copy (either flip or
 * tenure) the object and install a forwarding pointer at the new location. The
 * object may have already been copied. In either case, update the slot to point
 * at the new location of the object.
 *
 * @param objectPtrIndirect the slot to be updated
 * @return true if the new location of the object is in new space
 * @return false otherwise
 */
MMINLINE bool MM_Scavenger::copyAndForward(
    MM_EnvironmentStandard *env, volatile omrobjectptr_t *objectPtrIndirect) {
  bool toReturn = false;

  /* clear effectiveCopyCache to support aliasing check -- will be updated if
   * copy actually takes place */
  env->_effectiveCopyScanCache = NULL;

  omrobjectptr_t objectPtr = *objectPtrIndirect;
  if (NULL != objectPtr) {
    if (isObjectInEvacuateMemory(objectPtr)) {
      /* Object needs to be copy and forwarded.  Check if the work has already
       * been done */
      ...
      if (NULL != forwardPtr) {
        ...
        /* CS: ensure it's fully copied before exposing this new version of the
         * object */
        ...
        *objectPtrIndirect = forwardPtr;
        ...
        if (NULL == destinationObjectPtr) {
          ...
          /* raise the alert and return (true - must look like a new object was
           * handled) */
          toReturn = true;
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
          if (_extensions->concurrentScavenger) {
            /* We have no place to copy. We will return the original location of
             * the object. But we must prevent any other thread of making a copy
             * of this object.
       ...
       * for example when the partial scan state of a cache has been lost in
       * scan cache overflow
       */
      toReturn = true;
#if defined(OMR_GC_MODRON_SCAVENGER_STRICT)
 ...
 * @return false otherwise
 */
MMINLINE bool MM_Scavenger::copyAndForward(MM_EnvironmentStandard *env,
  ...
  if (NULL != env->_effectiveCopyScanCache) {
    env->_scavengerStats.countCopyDistance(
        (uintptr_t)slotObject->readAddressFromSlot(),
        (uintptr_t)slotObject->readReferenceFromSlot());
  }
#endif /* OMR_SCAVENGER_TRACK_COPY_DISTANCE */
  return result;
}

bool MM_Scavenger::copyObjectSlot(MM_EnvironmentStandard *env,
                                  volatile omrobjectptr_t *slotPtr) {
...
omrobjectptr_t MM_Scavenger::copyObject(MM_EnvironmentStandard *env,
...
omrobjectptr_t MM_Scavenger::copy(MM_EnvironmentStandard *env,
                                  MM_ForwardedHeader *forwardedHeader) {
  ...
  uintptr_t objectAge =
      _extensions->objectModel.getPreservedAge(forwardedHeader);
  ...
  _extensions->objectModel.calculateObjectDetailsForCopy(
      env, forwardedHeader, &objectCopySizeInBytes, &objectReserveSizeInBytes,
      &hotFieldsDescriptor);
  ...
  if (0 == (((uintptr_t)1 << objectAge) & _tenureMask)) {
    ...
    if (NULL != copyCache) {
      ...
      Trc_MM_Scavenger_semispaceAllocateFailed(env->getLanguageVMThread(),
                                               objectReserveSizeInBytes, "yes");

      uintptr_t spaceAvailableForObject =
          _activeSubSpace->getMaxSpaceForObjectInEvacuateMemory(
              forwardedHeader->getObject());
      Assert_GC_true_with_message4(
          env, objectCopySizeInBytes <= spaceAvailableForObject,
          "Corruption in Evacuate at %p: calculated object size %zu larger "
          "then available %zu, Forwarded Header at %p\n",
          forwardedHeader->getObject(), objectCopySizeInBytes,
          spaceAvailableForObject, forwardedHeader);
      ...
      if (NULL != copyCache) {
        ...
        Trc_MM_Scavenger_tenureAllocateFailed(
            env->getLanguageVMThread(), objectReserveSizeInBytes,
            env->_scavengerStats._failedTenureLargest, "no");
      }
    }
  } else {
    /* Move straight to tenuring on the object */
#if defined(J9VM_INTERP_NATIVE_SUPPORT)
    /* adjust the reserved object's size if we are aligning hot fields and this
     * class has a known hot field */
    if (_extensions->scavengerAlignHotFields &&
        HOTFIELD_SHOULD_ALIGN(hotFieldsDescriptor)) {
      /* this optimization is a source of fragmentation (alloc request size
       * always assumes maximum padding, but free entry created by sweep in
       * tenure could be less than that (since some of unused padding can
       * overlap with next copied object)). we limit this optimization for
       * arrays up to the size of 2 cache lines, beyond which the benefits of
       * the optimization are believed to be non-existant */
      if (!_extensions->objectModel.isIndexable(forwardedHeader) ||
          (objectReserveSizeInBytes <= 2 * _cacheLineAlignment)) {
        /* set the descriptor field if we should be aligning (since assuming
         * that 0 means no is not safe) */
        hotFieldsAlignment = hotFieldsDescriptor;
        /* for simplicity, add the maximum padding we could need (and back off
         * after allocation) */
        objectReserveSizeInBytes +=
            (_cacheLineAlignment - _objectAlignmentInBytes);
        Assert_MM_objectAligned(env, objectReserveSizeInBytes);
      }
    }
  ...
  if (0 != hotFieldsAlignment) {
    uintptr_t remainingInCacheLine =
        _cacheLineAlignment -
        ((uintptr_t)destinationObjectPtr % _cacheLineAlignment);
    uintptr_t alignmentBias =
        HOTFIELD_ALIGNMENT_BIAS(hotFieldsAlignment, _objectAlignmentInBytes);
    /* do alignment only if the object cannot fit in the remaining space in the
     * cache line */
    if ((remainingInCacheLine < objectCopySizeInBytes) &&
        (alignmentBias < remainingInCacheLine)) {
      hotFieldPadSize = ((remainingInCacheLine + _cacheLineAlignment) -
                         (alignmentBias % _cacheLineAlignment)) %
                        _cacheLineAlignment;
      ...
      destinationObjectPtr =
          (omrobjectptr_t)((uintptr_t)destinationObjectPtr + hotFieldPadSize);
    }
    /* and update the reserved size so that we "un-reserve" the extra memory we
     * said we might need.  This is done by removing the excess reserve since we
     * already accounted for the hotFieldPadSize by bumping the destination
     * pointer and now we need to revert to the amount needed for the object
     * allocation and its array alignment so the rest of the method continues to
     * function without needing to know about this extra alignment calculation
     */
    objectReserveSizeInBytes = objectReserveSizeInBytes -
                               (_cacheLineAlignment - _objectAlignmentInBytes);
  }
#endif /* J9VM_INTERP_NATIVE_SUPPORT */
  ...
  if (IS_CONCURRENT_ENABLED) {
    /* For smaller objects, we allow duplicate (copy first and try to win
     * forwarding). For larger objects, there is only one copy (threads setup
     * destination header, one wins, and other participate in copying or wait
     * till copy is complete). 1024 is somewhat arbitrary threshold, so that
     * most of time we do not have to go through relatively expensive setup
     * procedure.
     */
    if (objectCopySizeInBytes <= 1024) {
      ...
      initialSizeToCopy = forwardedHeader->copySetup(destinationObjectPtr,
                                                     &remainingSizeToCopy);
      /* set the hint in the f/w pointer, that the object might still be in the
       * processes of copying */
      destinationObjectPtr =
          forwardedHeader->setForwardedObjectWithBeingCopiedHint(
              destinationObjectPtr);
  ...
  {
    destinationObjectPtr =
        forwardedHeader->setForwardedObject(destinationObjectPtr);
  }

  if (originalDestinationObjectPtr == destinationObjectPtr) {
    /* Succeeded in forwarding the object, or we allow duplicate (did not even
     * tried to forward yet). */

#if defined(J9VM_INTERP_NATIVE_SUPPORT)
    if (NULL != hotFieldPadBase) {
      bool const compressed = env->compressObjectReferences();
      /* lay down a hole (XXX:  This assumes that we are using AOL
...
#if defined(OMR_VALGRIND_MEMCHECK)
    valgrindMempoolAlloc(_extensions, (uintptr_t)destinationObjectPtr,
                         objectReserveSizeInBytes);
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
    ...
    if (!allowDuplicateOrConcurrentDisabled) {
      ...
      forwardedHeader->copySection(destinationObjectPtr, remainingSizeToCopy,
                                   initialSizeToCopy);

      /* Try to copy more aligned sections. Once no more sections to copy, wait
       * till other threads are done with their sections */
      forwardedHeader->copyOrWaitWinner(destinationObjectPtr);

      /* Fixup most of the destination object (part that overlaps with forwarded
       * header) */
      forwardedHeader->commenceFixup(destinationObjectPtr);

      /* Object model specific fixup, like age */
      _extensions->objectModel.fixupForwardedObject(
          forwardedHeader, destinationObjectPtr, objectAge);
...
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
    {
      memcpy((void *)destinationObjectPtr, forwardedHeader->getObject(),
             objectCopySizeInBytes);

      /* Copy the preserved fields from the forwarded header into the
       * destination object */
    ...
    // Object is definitely dead but at many places (glue :
    // ScavangerRootScanner) We use it's forwardedHeader to check it.
    valgrindMakeMemDefined((uintptr_t)forwardedHeader->getObject(),
                           sizeof(MM_ForwardedHeader));
    ...
    omrtty_printf("{SCAV: Copied %p[%p] -> %p[%p]}\n",
                  forwardedHeader->getObject(),
                  *((uintptr_t *)(forwardedHeader->getObject())),
     ...
     * has been copied (since mutator may access the object as soon as
  ...
  if (originalDestinationObjectPtr == destinationObjectPtr) {
    ...
    /* object has been copied so if scanning hierarchically set
     * effectiveCopyCache to support aliasing check */
    ...
    if (copyCache->flags & OMR_SCAVENGER_CACHE_TYPE_TENURESPACE) {
      ...
      scavStats->getFlipHistory(0)->_tenureBytes[oldObjectAge + 1] +=
          objectReserveSizeInBytes;
#if defined(OMR_GC_LARGE_OBJECT_AREA)
      ...
      scavStats->getFlipHistory(0)->_flipBytes[oldObjectAge + 1] +=
          objectReserveSizeInBytes;
    }
  } else {
    /* We have not used the reserved space now, but we will for subsequent
     * allocations. If this space was reserved for an individual object, we
     * might have created a TLH remainder from previous cache just before
     * reserving this space. This space eventaully can create another remainder.
     * At that point, ideally (to recycle as much memory as possibly) we could
     * enqueue this remainder, but as a simple solution we will now abandon the
     * current remainder (we assert across the code, there is at most one at a
     * give point of time). If we see large amount of discards even with low
     * discard threshold, we may reconsider enqueueing discarded TLHs.
     */
    if (copyCache->flags & OMR_SCAVENGER_CACHE_TYPE_TENURESPACE) {
    ...
    /* Failed to forward (someone else did it). Re-fetch the forwarding info and
     * (for Concurrent Scavenger only) ensure it's fully copied before letting
     * the caller expose this new version of the object */
    MM_ForwardedHeader(forwardedHeader->getObject())
        .copyOrWait(destinationObjectPtr);
  }
...
GC_ObjectScanner *MM_Scavenger::getObjectScanner(MM_EnvironmentStandard *env,
                                                 omrobjectptr_t objectptr,
                                                 void *objectScannerState,
                                                 uintptr_t flags) {
  return _delegate.getObjectScanner(env, objectptr, (void *)objectScannerState,
                                    flags);
}

uintptr_t MM_Scavenger::getArraySplitAmount(MM_EnvironmentStandard *env,
                                            uintptr_t sizeInElements) {
  uintptr_t scvArraySplitAmount = 0;

  if (backOutStarted != _extensions->getScavengerBackOutState()) {
    /* pointer arrays are split into segments to improve parallelism. split
     * amount is proportional to array size. the less busy we are, the smaller
     * the split amount, while obeying specified minimum and maximum. but for
     * single-threaded backout, do not split arrays.
     */
    scvArraySplitAmount =
        sizeInElements / (_dispatcher->activeThreadCount() + 2 * _waitingCount);
    scvArraySplitAmount =
        OMR_MAX(scvArraySplitAmount, _extensions->scvArraySplitMinimumAmount);
    scvArraySplitAmount =
        OMR_MIN(scvArraySplitAmount, _extensions->scvArraySplitMaximumAmount);
  }
  return scvArraySplitAmount;
}

bool MM_Scavenger::splitIndexableObjectScanner(
    MM_EnvironmentStandard *env, GC_ObjectScanner *objectScanner,
    uintptr_t startIndex, omrobjectptr_t *rememberedSetSlot) {
  bool result = false;

  if (backOutStarted != _extensions->getScavengerBackOutState()) {
    Assert_MM_true(objectScanner->isIndexableObject());
    GC_IndexableObjectScanner *indexableScanner =
        (GC_IndexableObjectScanner *)objectScanner;
    uintptr_t maxIndex = indexableScanner->getIndexableRange();

    uintptr_t scvArraySplitAmount =
        getArraySplitAmount(env, maxIndex - startIndex);
    ...
    if (endIndex < maxIndex) {
      ...
      if (NULL != splitCache) {
        /* set up the split copy cache and clone the object scanner into the
         * cache */
        omrarrayptr_t arrayPtr =
            (omrarrayptr_t)indexableScanner->getArrayObject();
        void *arrayTop =
            (void *)((uintptr_t)arrayPtr +
                     _extensions->indexableObjectModel.getSizeInBytesWithHeader(
                         arrayPtr));
        ...
        indexableScanner->splitTo(env, splitCache->getObjectScanner(),
                                  scvArraySplitAmount);
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
...
/**
 * GC thread calls this to update its slots copied/scanned counts after scanning
 * a range of objects. This may trigger an update to the scaling factor that
 * resets this threads counts.
 *
 * @param[in] slotsScanned number of slots scanned on thread from
 * MM_CopyScanCache since last call to this method
 * @param[in] slotsCopied number of slots copied on thread from MM_CopyScanCache
 * since last call to this method
 */
MMINLINE void MM_Scavenger::updateCopyScanCounts(MM_EnvironmentBase *env,
                                                 uint64_t slotsScanned,
                                                 uint64_t slotsCopied) {
  ...
  uint64_t updateResult = _extensions->copyScanRatio.update(
      env, &(env->_scavengerStats._slotsScanned),
      &(env->_scavengerStats._slotsCopied), _waitingCount);
  if (0 != updateResult) {
    _extensions->copyScanRatio.majorUpdate(
        env, updateResult, _cachedEntryCount,
        _scavengeCacheScanList.getApproximateEntryCount());
  }
}

MMINLINE bool MM_Scavenger::scavengeObjectSlots(
    MM_EnvironmentStandard *env, MM_CopyScanCacheStandard *scanCache,
    omrobjectptr_t objectPtr, uintptr_t flags,
  ...
  if ((NULL == scanCache) || !scanCache->isSplitArray()) {
    ...
    objectScanner =
        getObjectScanner(env, objectPtr, &objectScannerState, flags);
    if ((NULL == objectScanner) || objectScanner->isLeafObject()) {
      /* Object scanner will be NULL if object not scannable by cli (eg, empty
       * pointer array, primitive array) */
  ...
  if ((NULL != scanCache) && objectScanner->isIndexableObject()) {
    ...
    Assert_MM_true(scanCache->isSplitArray() &&
                   (0 < scanCache->_arraySplitIndex));
    Assert_MM_true(rememberedSetSlot == scanCache->_arraySplitRememberedSlot);
  }
#endif /* defined(OMR_GC_MODRON_SCAVENGER_STRICT) */

  if (objectScanner->isIndexableObject()) {
    /* set scanning bounds for this scanner; if non-empty tail, clone scanner
     * into split array cache and add cache to worklist */
    uintptr_t splitIndex =
        (NULL != scanCache) ? scanCache->_arraySplitIndex : 0;
    if (!splitIndexableObjectScanner(env, objectScanner, splitIndex,
                                     rememberedSetSlot)) {
  ...
  if (shouldRemember && (NULL != rememberedSetSlot)) {
    ...
    Assert_MM_true(objectPtr ==
                   (omrobjectptr_t)((uintptr_t)*rememberedSetSlot &
                                    ~(uintptr_t)DEFERRED_RS_REMOVE_FLAG));
    /* Set the remembered set slot to the object pointer in case it was still
     * marked for removal. */
  ...
  if (_extensions->shouldScavengeNotifyGlobalGCOfOldToOldReference() &&
      IS_CONCURRENT_ENABLED && !isParentInNewSpace && !shouldRemember) {
    /* Old object that has only references to old objects. If parent object has
     * already been scanned (in Marking sense) since it has been tenured, let
     * Concurrent Marker know it has a newly created old reference, otherwise it
     * may miss to find it. */
...
void MM_Scavenger::deepScanOutline(MM_EnvironmentStandard *env,
                                   omrobjectptr_t objectPtr,
                                   uintptr_t priorityFieldOffset1,
                                   uintptr_t priorityFieldOffset2) {
  void *currentDeepObj = objectPtr;
  uintptr_t priorityField = priorityFieldOffset1;
  /* Throttle - Deep scan should be terminated when the free list is utilized
   * more than 50% */
  uintptr_t freeListUtilizationLimit =
      _scavengeCacheFreeList.getAllocatedCacheCount() / 2;
  ...
  do {
    GC_SlotObject prioritySlot(
        env->getOmrVM(),
        (fomrobject_t *)(((uintptr_t)currentDeepObj) + priorityField));
    copyAndForward(env, &prioritySlot);
    /* Did we encounter an already visited/NULL object? */
    if (NULL == env->_effectiveCopyScanCache) {
      /* Can't continue any further - attempt to deep scan with other self
       * referencing field (e.g, prev field) */
      if ((priorityField == priorityFieldOffset2) ||
          (priorityFieldOffset2 == 0)) {
        break;
      }
    ...
    /* The successfully copied object slot can possibly be overwritten with NULL
     * by a mutator (CS). To avoid race condition, we need a NULL check before
     * we proceed. */
...
/**
 * Scans the slots of a non-indexable object, remembering objects as required.
 * Scanning is interrupted as soon as there is a copy cache that is preferred to
 * the current scan cache. This is returned in nextScanCache.
 *
 ...
 */
MMINLINE MM_CopyScanCacheStandard *MM_Scavenger::incrementalScavengeObjectSlots(
    MM_EnvironmentStandard *env, omrobjectptr_t objectPtr,
    MM_CopyScanCacheStandard *scanCache) {
  /* Get an object scanner from the CLI if not resuming from a scan cache that
  ...
  if (!scanCache->_hasPartiallyScannedObject) {
    if (!scanCache->isSplitArray()) {
      /* try to get a new scanner instance from the cli */
      objectScanner =
          getObjectScanner(env, objectPtr, scanCache->getObjectScanner(),
                           GC_ObjectScanner::scanHeap);
      if ((NULL == objectScanner) || objectScanner->isLeafObject()) {
        /* Object scanner will be NULL if object not scannable by cli (eg, empty
         * pointer array, primitive array) */
        if (NULL != objectScanner) {
    ...
    if (objectScanner->isIndexableObject()) {
      /* set scanning bounds for this scanner; if non-empty tail, add split
       * array cache to worklist and clone this indexableScanner into split
       * cache */
      if (!splitIndexableObjectScanner(env, objectScanner,
                                       scanCache->_arraySplitIndex,
  ...
  while (NULL != (slotObject = objectScanner->getNextSlot())) {
    ...
    MM_CopyScanCacheStandard *copyCache = env->_effectiveCopyScanCache;
    if (NULL != copyCache) {
      /* Copy cache will be set only if a referent object is copied (ie, if not
  ...
  if (scanCache->_shouldBeRemembered) {
    if (NULL != scanCache->_arraySplitRememberedSlot) {
      ...
      Assert_MM_true(objectPtr ==
                     (omrobjectptr_t)(
                         (uintptr_t) * (scanCache->_arraySplitRememberedSlot) &
                         ~(uintptr_t)DEFERRED_RS_REMOVE_FLAG));
      /* Set the remembered set slot to the object pointer in case it was still
       * marked for removal. */
  ...
  bool isParentInNewSpace = isObjectInNewSpace(objectPtr);
  if (_extensions->shouldScavengeNotifyGlobalGCOfOldToOldReference() &&
      IS_CONCURRENT_ENABLED && !isParentInNewSpace &&
      !scanCache->_shouldBeRemembered) {
    /* Old object that has only references to old objects. If parent object has
     * already been scanned (in Marking sense) since it has been tenured, let
     * Concurrent Marker know it has a newly created old reference, otherwise it
     * may miss to find it. */
...
MMINLINE void MM_Scavenger::flushBuffersForGetNextScanCache(
    MM_EnvironmentStandard *env) {
  _delegate.flushReferenceObjects(env);
  flushRememberedSet(env);
}

MM_CopyScanCacheStandard *MM_Scavenger::getNextScanCache(
    MM_EnvironmentStandard *env) {
  ...
  if (NULL != cache) {
    /* deferred copy caches are used to merge memory-contiguous caches that got
     * chopped up due to large objects not fitting and resuing remainder. we
     * want to delay scanning them as much as possible (up to the size of the
     * original cache size being chopped up),
     * but we still want to do it before we synchronizing on scan queue and
     * realizing no more work is awailable */
  ...
  while (!doneFlag && !shouldAbortScanLoop(env)) {
    while (_cachedEntryCount > 0) {
      ...
      if (NULL != cache) {
        /* Check if there are threads waiting that should be notified because of
         * pending entries */
...
#if defined(OMR_SCAVENGER_TRACE)
        omrtty_printf(
            "{SCAV: slaveID %zu _cachedEntryCount %zu _waitingCount %zu Scan "
            "cache from list (%p)}\n",
    ...
    if (doneIndex == _doneIndex) {
      if ((env->_currentTask->getThreadCount() == _waitingCount) &&
          (0 == _cachedEntryCount)) {
        _waitingCount = 0;
        _doneIndex += 1;
        flushBuffersForGetNextScanCache(env);
        ...
        while ((0 == _cachedEntryCount) && (doneIndex == _doneIndex) &&
          ...
          if (doneIndex != _doneIndex) {
            env->_scavengerStats.addToCompleteStallTime(waitStartTime,
                                                        waitEndTime);
          } else {
    ...
    /* Set the local done flag and if we are done and the waiting count is 0
     * (last thread) exit */
...
/**
 * Scans all the objects to scan in the scanCache, remembering objects as
 * required, and flushing the cache at the end.
 */
void MM_Scavenger::completeScanCache(MM_EnvironmentStandard *env,
                                     MM_CopyScanCacheStandard *scanCache) {
  ...
  if (scanCache->isSplitArray()) {
    /* Advance the scan pointer to the top of the cache to signify that this has
     * been scanned */
    ...
    bool shouldBeRemembered = scavengeObjectSlots(
        env, scanCache, objectPtr, GC_ObjectScanner::scanHeap,
        scanCache->_arraySplitRememberedSlot);
    ...
    while (isWorkAvailableInCache(scanCache)) {
      GC_ObjectHeapIteratorAddressOrderedList heapChunkIterator(
          _extensions, (omrobjectptr_t)scanCache->scanCurrent,
          (omrobjectptr_t)scanCache->cacheAlloc, false);
      /* Advance the scan pointer to the top of the cache to signify that this
       * has been scanned */
      scanCache->scanCurrent = scanCache->cacheAlloc;
      /* Scan the chunk for all live objects */
      while ((objectPtr = heapChunkIterator.nextObjectNoAdvance()) != NULL) {
        /* If the object should be remembered and it is in old space, remember
         * it */
        bool shouldBeRemembered = scavengeObjectSlots(
  ...
  scanCache->flags &= ~OMR_SCAVENGER_CACHE_TYPE_SCAN;
  /* Done with the cache - build a free list entry in the hole, release the
   * cache to the free list (if not used), and continue */
  flushCache(env, scanCache);
}

/**
 * Scans the objects to scan in the scanCache, remembering objects as required.
 * Slots are scanned until there is an opportunity to alias the scan cache to a
 * copy cache. When this happens, scanning is interrupted and the present scan
 * cache is either pushed onto the scan list or "deferred" in thread-local
 * storage. Deferring is done to reduce contention due to the increased need to
 * change scan cache. If the cache is scanned completely without interruption
 * the cache is flushed at the end.
 */
void MM_Scavenger::incrementalScanCacheBySlot(
    MM_EnvironmentStandard *env, MM_CopyScanCacheStandard *scanCache) {
nextCache:
  /* mark that cache is in use as a scan cache */
  Assert_MM_true(0 == (scanCache->flags & OMR_SCAVENGER_CACHE_TYPE_SCAN));
  ...
  while (isWorkAvailableInCache(scanCache)) {
    ...
    GC_ObjectHeapIteratorAddressOrderedList heapChunkIterator(
        _extensions, (omrobjectptr_t)scanCache->scanCurrent,
        (omrobjectptr_t)cacheAlloc, false);
    ...
    while ((objectPtr = heapChunkIterator.nextObjectNoAdvance()) != NULL) {
      ...
      if (scanCache->_hasPartiallyScannedObject) {
        ...
        Assert_MM_true(0 != (scanCache->flags & OMR_SCAVENGER_CACHE_TYPE_SCAN));
#endif /* defined(OMR_GC_MODRON_SCAVENGER_STRICT) */
        /* If the scanCache has partially scanned objects then we must be
         * aliasing to one of the copy caches, which means the nextScanCache has
         * to have a value!
         */
        Assert_MM_true(NULL != nextScanCache);
        /* interrupt scan, save scan state of cache before deferring */
        scanCache->scanCurrent = objectPtr;
        /* Only save scan cache if it is not a copy cache, and then don't add to
         * scanlist - this can cause contention, just defer to later time on
         * same thread if deferred cache is occupied, then queue current scan
         * cache on scan list
         */
        scanCache->flags &= ~OMR_SCAVENGER_CACHE_TYPE_SCAN;
...
bool MM_Scavenger::completeScan(MM_EnvironmentStandard *env) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());

  /* take a snapshot of ID of this scan cycle (which will change in
   * getNextScanCache() once all threads agree to leave the scan loop) */
  ...
  if (_extensions->_forceRandomBackoutsAfterScan) {
    if (0 == (rand() % _extensions->_forceRandomBackoutsAfterScanPeriod)) {
      omrtty_printf(
          "Forcing backout at workUnitIndex: %zu lastSyncPointReached: %s\n",
  ...
  while (NULL != (scanCache = getNextScanCache(env))) {
    ...
    omrtty_printf("{SCAV: Completing scan (%p) %p-%p-%p-%p}\n", scanCache,
                  scanCache->cacheBase, scanCache->cacheAlloc,
                  scanCache->scanCurrent, scanCache->cacheTop);
#endif /* OMR_SCAVENGER_TRACE */
  ...
  /* A slow  thread can see backOutFlag raised by a fast thread aborting in the
   * next scan cycle. By checking that thread local doneIndex of the current
   * scan cycle matches the doneIndex from scan cycle that raised the flag, we
   * ensure consistent behavior (return of backOutRaised flag) of all threads
   * within this scan cycle, so that all threads proceed consistently to the
   * next step (being just another scan cycle, or backout procedure).
   */
  bool backOutRaisedThisScanCycle =
      isBackOutFlagRaised() && (doneIndex == _backOutDoneIndex);

  Assert_MM_true(backOutRaisedThisScanCycle ||
                 (0 == env->_scavengerRememberedSet.count));

  return !backOutRaisedThisScanCycle;
}

void MM_Scavenger::workThreadGarbageCollect(MM_EnvironmentStandard *env) {
  ...
  /*
   * There is a hidden assumption that RS Overflow flag would not be changed
   * between beginning of scavenge and this point, otherwise it is hang
   * situation when one group of threads might wait for more work in complete
   * scan and another group of threads be got stuck on sync point in
   * scavengeRememberedSetOverflow()
   *
  ...
  /* If -Xgc:fvtest=forceScavengerBackout has been specified, set backout flag
   * every 3rd scavenge */
  if (_extensions->fvtest_forceScavengerBackout) {
    if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env,
                                                                UNIQUE_ID)) {
 ...
 ****************************************
 */
void MM_Scavenger::clearRememberedSetLists(MM_EnvironmentStandard *env) {
  _extensions->rememberedSet.clear(env);
}

void MM_Scavenger::addAllRememberedObjectsToOverflow(
    MM_EnvironmentStandard *env, MM_RSOverflow *overflow) {
  ...
  while ((region = regionIterator.nextRegion()) != NULL) {
    GC_ObjectHeapIteratorAddressOrderedList objectIterator(_extensions, region,
                                                           false);
    omrobjectptr_t objectPtr;
...
void MM_Scavenger::addToRememberedSetFragment(MM_EnvironmentStandard *env,
                                              omrobjectptr_t objectPtr) {
  Assert_MM_true(NULL != objectPtr);
  Assert_MM_true(!isObjectInNewSpace(objectPtr));
  Assert_MM_true(_extensions->objectModel.isRemembered(objectPtr));

  if (env->_scavengerRememberedSet.fragmentCurrent >=
      env->_scavengerRememberedSet.fragmentTop) {
    /* There wasn't enough room in the current fragment - allocate a new one */
    if (allocateMemoryForSublistFragment(
            env->getOmrVMThread(),
            (J9VMGC_SublistFragment *)&env->_scavengerRememberedSet)) {
  ...
  uintptr_t *rememberedSetEntry =
      env->_scavengerRememberedSet.fragmentCurrent++;
  *rememberedSetEntry = (uintptr_t)objectPtr;

#if defined(OMR_SCAVENGER_TRACE_REMEMBERED_SET)
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  omrtty_printf(
      "{SCAV: Add to remembered set %p; env count = %lld; ext count = %lld}\n",
      objectPtr, env->_scavengerRememberedSet.count,
      _extensions->rememberedSet.countElements());
#endif /* OMR_SCAVENGER_TRACE_REMEMBERED_SET */
}

void MM_Scavenger::rememberObject(MM_EnvironmentStandard *env,
                                  omrobjectptr_t objectPtr) {
  /* Try to set the REMEMBERED bit in the flags field (if it hasn't already been
   * set) */
  if (!isObjectInNewSpace(objectPtr)) {
    if (_extensions->objectModel.atomicSetRememberedState(objectPtr,
                                                          STATE_REMEMBERED)) {
      /* The object has been successfully marked as REMEMBERED - allocate an
       * entry in the remembered set */
      addToRememberedSetFragment(env, objectPtr);
    }
  }
}

bool MM_Scavenger::isRememberedThreadReference(MM_EnvironmentStandard *env,
                                               omrobjectptr_t objectPtr) {
  Assert_MM_false(IS_CONCURRENT_ENABLED);
  Assert_MM_true(NULL != objectPtr);
  Assert_MM_true(!isObjectInNewSpace(objectPtr));
...
bool MM_Scavenger::processRememberedThreadReference(MM_EnvironmentStandard *env,
                                                    omrobjectptr_t objectPtr) {
  Assert_MM_false(IS_CONCURRENT_ENABLED);
  Assert_MM_true(NULL != objectPtr);
  Assert_MM_true(!isObjectInNewSpace(objectPtr));
  ...
  switch (age) {
    case OMR_TENURED_STACK_OBJECT_CURRENTLY_REFERENCED:
      result = true;
      /* downgrade state */
      _extensions->objectModel.setRememberedBits(
          objectPtr, OMR_TENURED_STACK_OBJECT_RECENTLY_REFERENCED);
      break;
    case OMR_TENURED_STACK_OBJECT_RECENTLY_REFERENCED:
      result = true;
      /* downgrade state */
 ...
 ********************************************************************/
bool MM_Scavenger::shouldRememberObject(MM_EnvironmentStandard *env,
                                        omrobjectptr_t objectPtr) {
  Assert_MM_true((NULL != objectPtr) && (!isObjectInNewSpace(objectPtr)));

  GC_ObjectScannerState objectScannerState;

  /* This method should be only called for RS pruning scan (whether in backout
   * or not), which is either single threaded (overflow or backout), or if
   * multi-threaded it does no work sharing. So we must not split, if it's
   * indexable
   */
  uintptr_t scannerFlags =
      GC_ObjectScanner::scanRoots | GC_ObjectScanner::indexableObjectNoSplit;

  GC_ObjectScanner *objectScanner =
      getObjectScanner(env, objectPtr, &objectScannerState, scannerFlags);
  if (NULL != objectScanner) {
...
/**
 * Scans all the slots of the given object, treating REFERENCE_MIXED as MIXED.
 * Also scan indirectly referenced objects in object class.
 * @return true if object should be remembered at the end of scanning.
 */
MMINLINE bool MM_Scavenger::scavengeRememberedObject(
    MM_EnvironmentStandard *env, omrobjectptr_t objectPtr) {
  bool shouldBeRemembered = scavengeObjectSlots(
      env, NULL, objectPtr, GC_ObjectScanner::scanRoots, NULL);
  if (_extensions->objectModel.hasIndirectObjectReferents(
          (CLI_THREAD_TYPE *)env->getLanguageVMThread(), objectPtr)) {
...
void MM_Scavenger::scavengeRememberedSetOverflow(MM_EnvironmentStandard *env) {
  /* Reset the local remembered set fragment */
  env->_scavengerRememberedSet.fragmentCurrent = NULL;
  env->_scavengerRememberedSet.fragmentTop = NULL;
  env->_scavengerRememberedSet.fragmentSize =
  ...
  if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
     ...
     * Objects that no longer need remembering will be pruned at the end of the
     * scavenge.
     */
    omrobjectptr_t objectPtr = NULL;
...
MMINLINE void MM_Scavenger::flushRememberedSet(MM_EnvironmentStandard *env) {
  if (0 != env->_scavengerRememberedSet.count) {
    MM_SublistFragment::flush(
        (J9VMGC_SublistFragment *)&env->_scavengerRememberedSet);
  }
}

void MM_Scavenger::pruneRememberedSet(MM_EnvironmentStandard *env) {
  if (isRememberedSetInOverflowState()) {
...
void MM_Scavenger::pruneRememberedSetOverflow(MM_EnvironmentStandard *env) {
  /* Reset the local remembered set fragment */
  env->_scavengerRememberedSet.fragmentCurrent = NULL;
  env->_scavengerRememberedSet.fragmentTop = NULL;
  env->_scavengerRememberedSet.fragmentSize =
  ...
  if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
    ...
    /* Clear the overflow state. Probability is high that we'll wind up
     * re-overflowing. */
    clearRememberedSetOverflowState();
    clearRememberedSetLists(env);

    /* Walk the tenure memory subspace finding all tenured objects flagged as
     * remembered */
    MM_HeapRegionDescriptorStandard *region = NULL;
    GC_MemorySubSpaceRegionIteratorStandard regionIterator(
        _tenureMemorySubSpace);
    while ((region = regionIterator.nextRegion()) != NULL) {
      /* Verify or clear remembered bits for each tenured object currently
       * flagged as remembered */
      GC_ObjectHeapIteratorAddressOrderedList objectIterator(_extensions,
                                                             region, false);
      omrobjectptr_t objectPtr;
      while ((objectPtr = objectIterator.nextObject()) != NULL) {
        if (_extensions->objectModel.isRemembered(objectPtr)) {
          ...
          if (shouldBeRemembered) {
            ...
            /* Add tenured object to the thread's remembered set list if
             * possible. Otherwise, this will force
             * setRememberedSetOverflowState(). */
            addToRememberedSetFragment(env, objectPtr);
          } else {
            ...
            if (_extensions
               ...
               * spot when object is scavenged and is unnecessary to do it here.
               */
              oldToOldReferenceCreated(env, objectPtr);
            }
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */
...
void MM_Scavenger::pruneRememberedSetList(MM_EnvironmentStandard *env) {
  /* Remembered set walk */
  ...
  while ((puddle = remSetIterator.nextList()) != NULL) {
    if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
      ...
      while ((slotPtr = (omrobjectptr_t *)remSetSlotIterator.nextSlot()) !=
             NULL) {
        objectPtr = *slotPtr;
        ...
        if (NULL == objectPtr) {
          ...
          objectPtr = (omrobjectptr_t)((uintptr_t)objectPtr &
                                       ~(uintptr_t)DEFERRED_RS_REMOVE_FLAG);
          ...
          if (!IS_CONCURRENT_ENABLED || !shouldRememberObject(env, objectPtr)) {
            ...
            _extensions->objectModel.clearRemembered(objectPtr);
            remSetSlotIterator.removeSlot();
            /* Inform interested parties (Concurrent Marker) that an object has
             * been removed from the remembered set. In non-concurrent Scavenger
             * this is the only way to create an old-to-old reference, that has
             * parent object being marked. In Concurrent Scavenger, it can be
             * created even with parent object that was not in RS to start with.
             * So this is handled in a more generic spot when object is
             * scavenged and is unnecessary to do it here.
             */
#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
            if (_extensions
                    ->shouldScavengeNotifyGlobalGCOfOldToOldReference() &&
             ...
             * references => reset the deferred flag. todo: consider doing
             * double remembering, if remembered during CS cycle, to avoid the
             * rescan of the object
             */
            *slotPtr = objectPtr;
          }
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
          ...
          if (!IS_CONCURRENT_ENABLED &&
              processRememberedThreadReference(env, objectPtr)) {
            /* the object was tenured from the stack on a previous scavenge --
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
void MM_Scavenger::scavengeRememberedSetListDirect(
    MM_EnvironmentStandard *env) {
  Trc_MM_ParallelScavenger_scavengeRememberedSetList_Entry(
      env->getLanguageVMThread());

  MM_SublistPuddle *puddle = NULL;
  while (NULL !=
         (puddle = _extensions->rememberedSet.popPreviousPuddle(puddle))) {
    Trc_MM_ParallelScavenger_scavengeRememberedSetList_startPuddle(
        env->getLanguageVMThread(), puddle);
    ...
    while ((slotPtr = (omrobjectptr_t *)remSetSlotIterator.nextSlot()) !=
      ...
      if (0 == ((uintptr_t)objectPtr & DEFERRED_RS_REMOVE_FLAG)) {
        if (!_extensions->objectModel.hasIndirectObjectReferents(
          ...
          numElements += 1;
          *slotPtr =
              (omrobjectptr_t)((uintptr_t)objectPtr | DEFERRED_RS_REMOVE_FLAG);
          bool shouldBeRemembered = scavengeObjectSlots(
  ...
  Trc_MM_ParallelScavenger_scavengeRememberedSetList_Exit(
      env->getLanguageVMThread());
}

void MM_Scavenger::scavengeRememberedSetListIndirect(
    MM_EnvironmentStandard *env) {
  Trc_MM_ParallelScavenger_scavengeRememberedSetList_Entry(
      env->getLanguageVMThread());
        ...
        if (_extensions->objectModel.hasIndirectObjectReferents(
          ...
          *slotPtr =
          ...
          bool shouldBeRemembered =
              _delegate.scavengeIndirectObjectSlots(env, objectPtr);
...
void MM_Scavenger::scavengeRememberedSetList(MM_EnvironmentStandard *env) {
  Assert_MM_false(IS_CONCURRENT_ENABLED);
  ...
  while (NULL !=
    ...
    while ((slotPtr = (omrobjectptr_t *)remSetSlotIterator.nextSlot()) !=
      ...
      if (NULL != objectPtr) {
         ...
         */
        *slotPtr =
            (omrobjectptr_t)((uintptr_t)*slotPtr | DEFERRED_RS_REMOVE_FLAG);
        bool shouldBeRemembered = scavengeObjectSlots(
            env, NULL, objectPtr, GC_ObjectScanner::scanRoots, slotPtr);
        if (_extensions->objectModel.hasIndirectObjectReferents(
                (CLI_THREAD_TYPE *)env->getLanguageVMThread(), objectPtr)) {
        ...
        if (shouldBeRemembered) {
          /* We want to remember this object after all; clear the flag for
           * removal. */
          *slotPtr = (omrobjectptr_t)((uintptr_t)*slotPtr &
                                      ~(uintptr_t)DEFERRED_RS_REMOVE_FLAG);
        }
      } else {
        remSetSlotIterator.removeSlot();
 ...
 */
void MM_Scavenger::scavengeRememberedSet(MM_EnvironmentStandard *env) {
  if (_isRememberedSetInOverflowAtTheBeginning) {
    env->_scavengerStats._rememberedSetOverflow = 1;
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    /* For CS, in case of OF, we deal with both direct and indirect refs with
     * only one pass. */
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    /* Indirect refs are dealt within the root scanning phase (first STW phase),
       while the direct references are dealt within the main scan phase
...
void MM_Scavenger::copyAndForwardThreadSlot(MM_EnvironmentStandard *env,
                                            omrobjectptr_t *objectPtrIndirect) {
  ...
  if (NULL != objectPtr) {
    if (isObjectInEvacuateMemory(objectPtr)) {
      ...
      if (!IS_CONCURRENT_ENABLED && !isInNewSpace) {
        ...
        /* the object was tenured while it was referenced from the stack. Undo
         * the forward, and process it in the rescan pass. */
...
void MM_Scavenger::rescanThreadSlot(MM_EnvironmentStandard *env,
                                    omrobjectptr_t *objectPtrIndirect) {
  Assert_MM_false(IS_CONCURRENT_ENABLED);
  ...
  if (NULL != objectPtr) {
    if (isObjectInEvacuateMemory(objectPtr)) {
      /* the slot is still pointing at evacuate memory. This means that it must
       * have been left unforwarded in the first pass so that we would process
       * it here.
       */
      ...
      *objectPtrIndirect = tenuredObjectPtr;
      ...
      /*
       * This call sets OMR_TENURED_STACK_OBJECT_CURRENTLY_REFERENCED Remembered
       * state in the object header:
       * - if object has any Remembered state set already (it means object has
       * been added to RS) just upgrade it to
       * OMR_TENURED_STACK_OBJECT_CURRENTLY_REFERENCED
       * - if object has no Remembered state set add object to the Remembered
       * Set here.
       */
      if (_extensions->objectModel.atomicSwitchReferencedState(
              tenuredObjectPtr,
 ...
 *
 * MM_Scavenger::reinitCache(MM_CopyScanCacheStandard *cache, void *base, void
 * *top)
 */

MMINLINE void MM_Scavenger::reinitCache(MM_CopyScanCacheStandard *cache,
                                        void *base, void *top) {
...
MMINLINE MM_CopyScanCacheStandard *MM_Scavenger::getFreeCache(
    MM_EnvironmentStandard *env) {
  /* Check the free list */
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
...
MM_CopyScanCacheStandard *MM_Scavenger::createCacheInHeap(
    MM_EnvironmentStandard *env) {
#if defined(OMR_SCAVENGER_TRACE)
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
#endif /* OMR_SCAVENGER_TRACE */
  ...
  /* Recheck the free list - avoids a potential timing hole here after releasing
   * the lock previously. This keeps the lock ordering (scan is an outer lock of
   * free) wrt/the api (can enter this call with the scan lock held)
   */
  MM_CopyScanCacheStandard *cache = _scavengeCacheFreeList.popCache(env);
  ...
  if (NULL == cache) {
    ...
    cache = _scavengeCacheFreeList.appendCacheEntriesInHeap(
        env, _survivorMemorySubSpace, this);
    if (NULL != cache) {
      /* temporary chunk of scanCaches is created in Survivor */
#if defined(OMR_SCAVENGER_TRACE)
      omrtty_printf(
          "{SCAV: Temporary chunk of scan cache headers allocated in Survivor "
          "(%p)}\n",
          cache);
#endif /* OMR_SCAVENGER_TRACE */
    }
  ...
  if (NULL == cache) {
    ...
    cache = _scavengeCacheFreeList.appendCacheEntriesInHeap(
        env, _tenureMemorySubSpace, this);
    if (NULL != cache) {
      /* temporary chunk of scanCaches is created in Tenure */
#if defined(OMR_SCAVENGER_TRACE)
      omrtty_printf(
          "{SCAV: Temporary chunk of scan cache headers allocated in Tenure "
          "(%p)}\n",
          cache);
#endif /* OMR_SCAVENGER_TRACE */
    }
...
#if defined(OMR_SCAVENGER_TRACE)
    omrtty_printf(
        "{SCAV: Temporary chunk of scan cache headers can not be allocated in "
        "Heap!}\n");
#endif /* OMR_SCAVENGER_TRACE */
  }
...
void MM_Scavenger::flushCache(MM_EnvironmentStandard *env,
                              MM_CopyScanCacheStandard *cache) {
  if (0 == (cache->flags & OMR_SCAVENGER_CACHE_TYPE_COPY)) {
#if defined(OMR_SCAVENGER_TRACE)
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
    omrtty_printf("{SCAV: Flushing cache (%p) %p-%p-%p-%p}\n", cache,
                  cache->cacheBase, cache->cacheAlloc, cache->scanCurrent,
                  cache->cacheTop);
#endif /* OMR_SCAVENGER_TRACE */
...
bool MM_Scavenger::canLocalCacheBeReused(MM_EnvironmentStandard *env,
                                         MM_CopyScanCacheStandard *cache) {
  /* has not been scanned and no scan work to do - so it can be reused */
  return (NULL != cache) && !cache->isCurrentlyBeingScanned() &&
         !cache->isScanWorkAvailable();
}

MM_CopyScanCacheStandard *MM_Scavenger::releaseLocalCopyCache(
    MM_EnvironmentStandard *env, MM_CopyScanCacheStandard *cache) {
  ...
  if (NULL != cache) {
    ...
    /* Check if the cache contains elements that need to be scanned - if not,
    ...
    if (cache->isCurrentlyBeingScanned()) {
      ...
      if (NULL != env->_deferredCopyCache) {
        /* Deferred copy cache already exists. Check if should be merged with
         * current cache */
        /* Deferred copy cache should also never be a scan cache */
        Assert_MM_false(env->_deferredCopyCache->flags &
                        OMR_SCAVENGER_CACHE_TYPE_SCAN);
        /* We have two copy caches (deferred and current). Do they create
         * contiguous memory with no objects (fully or partially) scanned in
         * between? */
        if ((env->_deferredCopyCache->cacheAlloc == cache->scanCurrent) &&
            !cache->_hasPartiallyScannedObject) {
          /* append current copy cache to the deferred one. yet, decide whether
           * to keep deferring it or push it for scanning */
          Assert_MM_true((cache->flags & ~OMR_SCAVENGER_CACHE_TYPE_HEAP) ==
                         (env->_deferredCopyCache->flags &
                          ~OMR_SCAVENGER_CACHE_TYPE_HEAP));
          Assert_MM_false(cache->flags & OMR_SCAVENGER_CACHE_TYPE_SPLIT_ARRAY);
          if (remainderCreated) {
            /* keep deferring the joint copy cache, there might be more appends
             * to come */
            env->_deferredCopyCache->cacheAlloc = cache->cacheAlloc;
            cacheToReuse = cache;
            cache = NULL;
          } else {
            /* this was last possible append. we want to finally add the
             * deferred cache to the scan list */
            /* we use deferredCopyCache for merged one. this way we preserve
             * partial scanned object info, if any exists */
            env->_deferredCopyCache->cacheAlloc = cache->cacheAlloc;
          ...
          /* deferred and current copy caches are discontiguous. pushing the
           * current one, if scan work available */
        ...
        /* No deferred cache exists. Decide what to do with current one (defer,
         * push for scanning, or just ignore) */
        if (cache->isScanWorkAvailable()) {
      ...
      if (cache) {
        /* Preceding code made sure that if there is a cache to push, it has
         * some scan work */
        ...
        /* It is not being scanned, and it does have entries to scan - add to
         * scan list */
        /* Mark the cache entry as unused as a copy destination */
        Assert_MM_true(cache->flags & OMR_SCAVENGER_CACHE_TYPE_COPY);
        cache->flags &= ~OMR_SCAVENGER_CACHE_TYPE_COPY;
        /* must not have local references still in use before adding to global
         * list */
...
bool MM_Scavenger::clearCache(MM_EnvironmentStandard *env,
                              MM_CopyScanCacheStandard *cache) {
  MM_MemorySubSpace *allocSubSpace = NULL;
  uintptr_t discardSize =
      (uintptr_t)cache->cacheTop - (uintptr_t)cache->cacheAlloc;
  ...
  if (0 < discardSize) {
    if (cache->flags & OMR_SCAVENGER_CACHE_TYPE_TENURESPACE) {
      ...
      if (discardSize < env->getExtensions()->tlhTenureDiscardThreshold) {
        ...
        env->_loaAllocation = (OMR_SCAVENGER_CACHE_TYPE_LOA ==
                               (cache->flags & OMR_SCAVENGER_CACHE_TYPE_LOA));
      }
       ...
       * OMR_SCAVENGER_CACHE_TYPE_TENURESPACE or
       * OMR_SCAVENGER_CACHE_TYPE_SEMISPACE might be set. However discardSize
       * must be zero in this case and we should not go here
       */
      Assert_MM_unreachable();
    }
  }

  /* Broadcast details of that portion of memory within which objects have been
   * allocated */
  TRIGGER_J9HOOK_MM_PRIVATE_CACHE_CLEARED(
      _extensions->privateHookInterface, env->getOmrVMThread(), allocSubSpace,
...
void MM_Scavenger::abandonSurvivorTLHRemainder(MM_EnvironmentStandard *env) {
  if (NULL != env->_survivorTLHRemainderBase) {
    Assert_MM_true(NULL != env->_survivorTLHRemainderTop);
    env->_scavengerStats._flipDiscardBytes +=
        (uintptr_t)env->_survivorTLHRemainderTop -
        (uintptr_t)env->_survivorTLHRemainderBase;
    _survivorMemorySubSpace->abandonHeapChunk(env->_survivorTLHRemainderBase,
                                              env->_survivorTLHRemainderTop);
    env->_survivorTLHRemainderBase = NULL;
    env->_survivorTLHRemainderTop = NULL;
  }
}

void MM_Scavenger::abandonTenureTLHRemainder(MM_EnvironmentStandard *env,
                                             bool preserveRemainders) {
  if (NULL != env->_tenureTLHRemainderBase) {
    Assert_MM_true(NULL != env->_tenureTLHRemainderTop);
    _tenureMemorySubSpace->abandonHeapChunk(env->_tenureTLHRemainderBase,
                                            env->_tenureTLHRemainderTop);

    if (!preserveRemainders) {
      env->_scavengerStats._tenureDiscardBytes +=
          (uintptr_t)env->_tenureTLHRemainderTop -
          (uintptr_t)env->_tenureTLHRemainderBase;
      env->_tenureTLHRemainderBase = NULL;
      env->_tenureTLHRemainderTop = NULL;
    }
    /* In case of Mutator threads (for concurrent scavenger) isMasterThread() is
       not sufficient, we must also make a thread check with getThreadType()*/
    else if ((env->isMasterThread()) &&
             (GC_MASTER_THREAD == env->getThreadType())) {
...
void MM_Scavenger::addCopyCachesToFreeList(MM_EnvironmentStandard *env) {
...
MMINLINE void MM_Scavenger::addCacheEntryToScanListAndNotify(
    MM_EnvironmentStandard *env, MM_CopyScanCacheStandard *newCacheEntry) {
  _scavengeCacheScanList.pushCache(env, newCacheEntry);
  if (0 != _waitingCount) {
    /* Added an entry to the list - notify any other threads that a new entry
...
MMINLINE uintptr_t MM_Scavenger::scanCacheDistanceMetric(
    MM_CopyScanCacheStandard *scanCache, GC_SlotObject *scanSlot) {
  /* distance from referring slot to prospective referent copy location */
  return ((uintptr_t)scanCache->cacheAlloc -
          (uintptr_t)scanSlot->readAddressFromSlot());
}

MMINLINE uintptr_t
MM_Scavenger::copyCacheDistanceMetric(MM_CopyScanCacheStandard *copyCache) {
  /* (best estimate) distance from first reference slot to prospective referent
   * copy location */
  return ((uintptr_t)copyCache->cacheAlloc -
          ((uintptr_t)copyCache->scanCurrent + OMR_MINIMUM_OBJECT_SIZE));
}

MMINLINE MM_CopyScanCacheStandard *MM_Scavenger::aliasToCopyCache(
    MM_EnvironmentStandard *env, GC_SlotObject *scannedSlot,
    MM_CopyScanCacheStandard *scanCache, MM_CopyScanCacheStandard *copyCache) {
  /* Only alias a copy cache IF there are <= _waitingCountAliasThreshold threads
   * waiting (defaulted to 20% of active GC threads, option
   * XXgc:aliasInhibitingThresholdPercentage). If the current thread is the only
   * producer and it aliases a copy cache then it will be the only thread able
   * to consume. This will alleviate the stalling issues.
   *
   * @NOTE See Github Issue 3089 (Investigate Scavenger's Aliasing Inhibiting
   * Condition)
   */
  if (_waitingCount <= _waitingCountAliasThreshold) {
    /* Only alias if the scanCache != copyCache. IF the caches are the same
     * there is no benefit to aliasing. The checks afterwards will ensure that a
     * very similar copy order will happen if the copyCache changes from the
     * currently aliased scan cache
     *
     * Perform this check first since these values are likely in the threads L1
     * cache */
    if (scanCache == copyCache) {
      return NULL;
    }

    /* If the scanCache is not an aliased copy cache try to alias to the copy
     * cache since the copy cache will always be better if it has objects to
     * scan */
    ...
    /* alias and switch to copy cache if it has scan work available and a
     * shorter copy distance */
    if (copyCacheDistanceMetric(copyCache) <
        scanCacheDistanceMetric(scanCache, scannedSlot)) {
      if (copyCache->cacheAlloc != copyCache->scanCurrent) {
        env->_scavengerStats._aliasToCopyCacheCount += 1;
        scanCache->_hasPartiallyScannedObject = true;
        return copyCache;
...
MMINLINE MM_CopyScanCacheStandard *MM_Scavenger::getNextScanCacheFromList(
    MM_EnvironmentStandard *env) {
 ...
 */
bool MM_Scavenger::scavengeCompletedSuccessfully(MM_EnvironmentStandard *env) {
  return !isBackOutFlagRaised();
}

/**
 * Save the thread environment remainder references to extensions, this is
 * required for the master thread as it is an implicit thread and in general
 * changes from cycle to cycle. This is done at the end of the GC cycle (or end
 * of each phase for concurrent scavenger).
 */
void MM_Scavenger::saveMasterThreadTenureTLHRemainders(
    MM_EnvironmentStandard *env) {
  _extensions->_masterThreadTenureTLHRemainderTop = env->_tenureTLHRemainderTop;
  _extensions->_masterThreadTenureTLHRemainderBase =
      env->_tenureTLHRemainderBase;
  env->_tenureTLHRemainderBase = NULL;
  env->_tenureTLHRemainderTop = NULL;
}

/**
 * Restore master thread TLH remainder boundaries that have been preserved from
 * the end of the previous cycle. This is done to minimze the number of bytes
 * discarded by reusing memory from the previously allocated TLH.
 */
void MM_Scavenger::restoreMasterThreadTenureTLHRemainders(
    MM_EnvironmentStandard *env) {
  if ((NULL != _extensions->_masterThreadTenureTLHRemainderTop) &&
    ...
    env->_tenureTLHRemainderBase =
        _extensions->_masterThreadTenureTLHRemainderBase;
    env->_tenureTLHRemainderTop =
        _extensions->_masterThreadTenureTLHRemainderTop;
    _extensions->_masterThreadTenureTLHRemainderTop = NULL;
    _extensions->_masterThreadTenureTLHRemainderBase = NULL;
  }
}

MMINLINE bool MM_Scavenger::checkAndSetShouldYieldFlag(
    MM_EnvironmentStandard *env) {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  /* Don't rely on various conditions being same during one concurrent phase.
   * If one GC thread decided that we need to yield, we must yield, there is no
   * way back. Hence, we store the result in _shouldYield, and rely on it for
   * the rest of concurrent phase. Master info if we should yield comes from
   * exclusive VM access request being broadcasted to this thread
   * (isExclusiveAccessRequestWaiting()) But since that request in the thread is
   * not cleared even when implicit master GC thread enters STW phase, and since
   * this yield check is invoked in common code that can run both during STW and
   * concurrent phase, we have to additionally check we are indeed in concurrent
   * phase before deciding to yield. Most of the time we could rely on being in
   * 'concurrent_state_scan' but it's more reliable to actually check if
   * exclusive access is indeed being requested (hence scavenger_shouldYield()
   * call too).
   */
  if (!_shouldYield && env->isExclusiveAccessRequestWaiting() &&
      _delegate.shouldYield()) {
    _shouldYield = true;
  }
  return _shouldYield;
#else
 ...
 */
void MM_Scavenger::setBackOutFlag(MM_EnvironmentBase *env,
                                  BackOutState backOutState) {
  /* Skip triggering of trace point and hook if we trying to set flag to true
   * multiple times */
  if (_extensions->getScavengerBackOutState() != backOutState) {
    _backOutDoneIndex = _doneIndex;
    /* Ensure that other CPUs see correct _backOutDoneIndex, by the time they
     * see _backOutFlag is set */
    ...
    if (backOutStarted > backOutState) {
      Trc_MM_ScavengerBackout(
          env->getLanguageVMThread(),
          backOutFlagCleared < backOutState ? "true" : "false");
      TRIGGER_J9HOOK_MM_PRIVATE_SCAVENGER_BACK_OUT(
          _extensions->privateHookInterface, env->getOmrVM(),
          backOutFlagCleared < backOutState);
    }
  }
}

bool MM_Scavenger::backOutFixSlotWithoutCompression(
    volatile omrobjectptr_t *slotPtr) {
  omrobjectptr_t objectPtr = *slotPtr;
...
bool MM_Scavenger::backOutFixSlot(GC_SlotObject *slotObject) {
  ...
  if (NULL != objectPtr) {
    ...
    Assert_MM_false(forwardHeader.isForwardedPointer());
    if (forwardHeader.isReverseForwardedPointer()) {
      slotObject->writeReferenceToSlot(
          forwardHeader.getReverseForwardedPointer());
#if defined(OMR_SCAVENGER_TRACE_BACKOUT)
      OMRPORT_ACCESS_FROM_OMRVM(_omrVM);
      omrtty_printf("{SCAV: Back out object slot %p[%p->%p]}\n", objectPtr,
                    slotObject->readAddressFromSlot(),
                    slotObject->readReferenceFromSlot());
      Assert_MM_true(
          isObjectInEvacuateMemory(slotObject->readReferenceFromSlot()));
#endif /* OMR_SCAVENGER_TRACE_BACKOUT */
      return true;
    }
  }
...
void MM_Scavenger::backOutObjectScan(MM_EnvironmentStandard *env,
                                     omrobjectptr_t objectPtr) {
  GC_SlotObject *slotObject = NULL;
  GC_ObjectScannerState objectScannerState;
  GC_ObjectScanner *objectScanner = getObjectScanner(
      env, objectPtr, &objectScannerState, GC_ObjectScanner::scanRoots);
  if (NULL != objectScanner) {
#if defined(OMR_SCAVENGER_TRACE_BACKOUT)
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
...
void MM_Scavenger::backoutFixupAndReverseForwardPointersInSurvivor(
    MM_EnvironmentStandard *env) {
  GC_MemorySubSpaceRegionIteratorStandard evacuateRegionIterator(
      _activeSubSpace);
    ...
    if (isObjectInEvacuateMemory((omrobjectptr_t)rootRegion->getLowAddress())) {
      ...
      while ((objectPtr = evacuateHeapIterator.nextObjectNoAdvance()) != NULL) {
        ...
        if (header.isForwardedPointer()) {
          ...
          /* A reverse forwarded object is a hole whose 'next' pointer actually
           * points at the original object. This keeps tenure space walkable
           * once the reverse forwarded objects are abandoned.
           */
          ...
          valgrindMakeMemUndefined((uintptr_t)freeHeader,
                                   (uintptr_t)sizeof(MM_HeapLinkedFreeHeader));
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
...
#if defined(OMR_SCAVENGER_TRACE_BACKOUT)
          omrtty_printf(
              "{SCAV: Back out forward pointer %p[%p]@%p -> %p[%p]}\n",
  ...
  if (compressed) {
    GC_MemorySubSpaceRegionIteratorStandard evacuateRegionIterator1(
        _activeSubSpace);
    while (NULL != (rootRegion = evacuateRegionIterator1.nextRegion())) {
      if (isObjectInEvacuateMemory(
              (omrobjectptr_t)rootRegion->getLowAddress())) {
        /*
         * CMVC 179190:
         * The call to "reverseForwardedObject", above, destroys our ability to
         * detect if this object needs its destroyed slot fixed up (but the
         * above loop must complete before we have the information with which to
         * fixup the destroyed slot).  Fixing up a slot in dark matter could
         * crash, though, since the slot could point to contracted memory or
         * could point to corrupted data updated in a previous backout.  The
         * simple work-around for this problem is to check if the slot points at
         * a readable part of the heap (specifically, tenure or survivor - the
         * only locations which would require us to fix up the slot) and only
         * read and fixup the slot in those cases. This means that we could
         * still corrupt the slot but we will never crash during fixup and
         * nobody else should be trusting slots found in dead objects.
         */
        GC_ObjectHeapIteratorAddressOrderedList evacuateHeapIterator(
            _extensions, rootRegion, false);
        omrobjectptr_t objectPtr = NULL;
        ...
        while ((objectPtr = evacuateHeapIterator.nextObjectNoAdvance()) !=
          ...
          omrtty_printf("{SCAV: Fixup destroyed slot %p@%p -> %u->%u}\n",
                        objectPtr, fwdObjectPtr, originalOverlap,
                        header.getPreservedOverlap());
#endif /* OMR_SCAVENGER_TRACE_BACKOUT */
        }
      }
    }
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
bool MM_Scavenger::fixupSlotWithoutCompression(
    volatile omrobjectptr_t *slotPtr) {
  omrobjectptr_t objectPtr = *slotPtr;
...
bool MM_Scavenger::fixupSlot(GC_SlotObject *slotObject) {
  omrobjectptr_t objectPtr = slotObject->readReferenceFromSlot();
...
void MM_Scavenger::fixupObjectScan(MM_EnvironmentStandard *env,
                                   omrobjectptr_t objectPtr) {
  GC_SlotObject *slotObject = NULL;
  GC_ObjectScannerState objectScannerState;
  GC_ObjectScanner *objectScanner = getObjectScanner(
      env, objectPtr, (void *)&objectScannerState, GC_ObjectScanner::scanRoots);
  if (NULL != objectScanner) {
    while (NULL != (slotObject = objectScanner->getNextSlot())) {
...
void MM_Scavenger::processRememberedSetInBackout(MM_EnvironmentStandard *env) {
  omrobjectptr_t *slotPtr;
  omrobjectptr_t objectPtr;
  MM_SublistPuddle *puddle;
      ...
      while ((slotPtr = (omrobjectptr_t *)remSetSlotIterator.nextSlot()) !=
        ...
        if (NULL == objectPtr) {
          ...
          if ((uintptr_t)objectPtr & DEFERRED_RS_REMOVE_FLAG) {
            ...
            /* The object did not have Nursery references at initial RS scan,
             * but one could have been added during CS cycle by a mutator. */
            if (!shouldRememberObject(env, objectPtr)) {
              ...
              /* We are not removing it after all, since the object has Nursery
               * references => reset the deferred flag. */
              *slotPtr = objectPtr;
            }
          } else {
...
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
  {
    /* Walk the remembered set removing any tagged entries (back out of a
     * tenured copy that is remembered) and scanning remembered objects for
     * reverse fwd info
     */
...
void MM_Scavenger::completeBackOut(MM_EnvironmentStandard *env) {
  /* Work to be done (for non Concurrent Scavenger):
   * 1) Flush copy scan caches
   * 2) Walk the evacuate space, fixing up objects and installing reverse
   * forward pointers in survivor space 3) Restore the remembered set 4) Client
   * language completion of back out
   */
  ...
  /* Ensure we've pushed all references from buffers out to the lists and
   * flushed RS fragments*/
  flushBuffersForGetNextScanCache(env);
  ...
  if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
    ...
    /* 2
     * a) Mark the overflow scan as invalid (backing out of objects moved into
     * old space) b) If the remembered set is in an overflow state, i)
     * Unremember any objects that moved from new space to old ii) Walk old
     * space and build up the overflow list
     */
    ...
    if (isRememberedSetInOverflowState()) {
      ...
      if (IS_CONCURRENT_ENABLED) {
        ...
        /* the list is built because after reverse fwd ptrs are installed, the
         * heap becomes unwalkable */
        clearRememberedSetLists(env);
        ...
        /*
         * 2.c)Walk the evacuate space, fixing up objects and installing reverse
         * forward pointers in survivor space
         */
        backoutFixupAndReverseForwardPointersInSurvivor(env);

        /* 3) Walk the remembered set, updating list pointers as well as
         * remembered object ptrs */
#if defined(OMR_SCAVENGER_TRACE_BACKOUT)
      ...
      /* RS not in overflow */
      if (!IS_CONCURRENT_ENABLED) {
        /* Walk the evacuate space, fixing up objects and installing reverse
         * forward pointers in survivor space */
        backoutFixupAndReverseForwardPointersInSurvivor(env);
      }
 ...
 */
void MM_Scavenger::masterThreadGarbageCollect(
    MM_EnvironmentBase *envBase, MM_AllocateDescription *allocDescription,
  ...
  /* We might be running in a context of either main or master thread, but
   * either way we must have exclusive access */
  Assert_MM_mustHaveExclusiveVMAccess(env->getOmrVMThread());

  if (_extensions->trackMutatorThreadCategory) {
    /* This thread is doing GC work, account for the time spent into the GC
     * bucket */
    omrthread_set_category(env->getOmrVMThread()->_os_thread,
  ...
  if (lastIncrement) {
    ...
    if (scavengeCompletedSuccessfully(env)) {
      ...
      /* If -Xgc:fvtest=forcePoisonEvacuate has been specified, poison(fill
       * poison pattern) evacuate space */
      ...
      if (_extensions->scvTenureStrategyAdaptive) {
        /* Adjust the tenure age based on the percentage of new space used.
         * Also, avoid / by 0 */
        ...
        if ((newSpaceConsumedSize <
             (_extensions->scvTenureRatioLow * newSpaceSizeScale)) &&
          ...
          if ((newSpaceConsumedSize >
              ...
              (_extensions->scvTenureAdaptiveTenureAge >
               OBJECT_HEADER_AGE_MIN)) {
            _extensions->scvTenureAdaptiveTenureAge--;
          }
        }
      }
    } else {
      /* Build free list in survivor profile - the scavenge was unsuccessful, so
       * rebuild the free list */
      _activeSubSpace->masterTeardownForAbortedGC(env);
    }
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    /* Although evacuate is functionally irrelevant at this point since we are
     * finishing the cycle, it is still useful for debugging (CS must not see
     * live objects in Evacuate). Thus re-caching evacuate ranges to point to
     * reserved/empty space of Survivor */
    ...
    /* If there was a failed tenure of a size greater than the threshold, set
     * the flag. */
    ...
    if (_extensions->scavengerStats._failedTenureCount > 0) {
      if (_extensions->scavengerStats._failedTenureBytes >=
        ...
        Trc_MM_Scavenger_masterThreadGarbageCollect_setFailedTenureFlag(
            env->getLanguageVMThread(),
            _extensions->scavengerStats._failedTenureLargest);
        setFailedTenureThresholdFlag();
        setFailedTenureLargestObject(
            _extensions->scavengerStats._failedTenureLargest);
      }
    }
    if (_extensions->processLargeAllocateStats) {
  ...
  if (_extensions->trackMutatorThreadCategory) {
    ...
    omrthread_set_category(env->getOmrVMThread()->_os_thread, 0,
                           J9THREAD_TYPE_SET_GC);
  }
...
void MM_Scavenger::processLargeAllocateStatsBeforeGC(MM_EnvironmentBase *env) {
  MM_MemorySpace *defaultMemorySpace =
      _extensions->heap->getDefaultMemorySpace();
  MM_MemorySubSpace *defaultMemorySubspace =
      defaultMemorySpace->getDefaultMemorySubSpace();
  MM_MemorySubSpace *tenureMemorySubspace =
      defaultMemorySpace->getTenureMemorySubSpace();

  /* merge largeObjectAllocateStats in nursery space */
  if (defaultMemorySubspace->isPartOfSemiSpace()) {
    /* SemiSpace stats include only Mutator stats (no Collector stats during
     * flipping) */
    defaultMemorySubspace->getTopLevelMemorySubSpace(MEMORY_TYPE_NEW)
        ->mergeLargeObjectAllocateStats(env);
  }

  /* TODO: remove the below 2 lines(resetLargeObjectAllocateStats), so that we
   * do not loose direct mutator allocation info */
  MM_MemoryPool *tenureMemoryPool = tenureMemorySubspace->getMemoryPool();
  tenureMemoryPool->resetLargeObjectAllocateStats();
}

void MM_Scavenger::processLargeAllocateStatsAfterGC(MM_EnvironmentBase *env) {
  MM_MemorySpace *defaultMemorySpace =
      _extensions->heap->getDefaultMemorySpace();
  MM_MemorySubSpace *tenureMemorySubspace =
      defaultMemorySpace->getTenureMemorySubSpace();
  ...
  /* TODO: need to consider allocation form mutators for averaging later,
   * currently only average allocation from collectors */
  memoryPool->averageLargeObjectAllocateStats(
      env, _extensions->scavengerStats._tenureAggregateBytes);
  /* merge FreeEntry AllocateStats in tenure space */
  memoryPool->mergeFreeEntryAllocateStats();
  MM_LargeObjectAllocateStats *stats =
      memoryPool->getLargeObjectAllocateStats();
  stats->setTimeMergeAverage(omrtime_hires_clock() - startTime);

  stats->verifyFreeEntryCount(memoryPool->getActualFreeEntryCount());
  /* estimate Fragmentation */
  if ((GLOBALGC_ESTIMATE_FRAGMENTATION == (_extensions->estimateFragmentation &
                                           GLOBALGC_ESTIMATE_FRAGMENTATION)) &&
    ...
    ((MM_CollectionStatisticsStandard *)env->_cycleState->_collectionStatistics)
        ->_tenureFragmentation = MACRO_FRAGMENTATION;
  } else {
    stats->resetRemainingFreeMemoryAfterEstimate();
  }
}

void MM_Scavenger::reportGCCycleFinalIncrementEnding(
    MM_EnvironmentStandard *env) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  ...
  TRIGGER_J9HOOK_MM_OMR_GC_CYCLE_END(
      _extensions->omrHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_OMR_GC_CYCLE_END,
 ...
 * @param highAddress The top address (non-inclusive) of the memory added to the
 * heap
 * @return true if operation completes with success
 */
bool MM_Scavenger::heapAddRange(MM_EnvironmentBase *env,
                                MM_MemorySubSpace *subspace, uintptr_t size,
 ...
 * @param highAddress The top address (non-inclusive) of the memory added to the
 * heap
 * @param lowValidAddress The first valid address previous to the lowest in the
 * heap range being removed
 * @param highValidAddress The first valid address following the highest in the
 * heap range being removed
 * @return true if operation completes with success
 */
bool MM_Scavenger::heapRemoveRange(MM_EnvironmentBase *env,
                                   MM_MemorySubSpace *subspace, uintptr_t size,
                                   void *lowAddress, void *highAddress,
                                   void *validLowAddress,
                                   void *validHighAddress) {
  return true;
}

/**
 * Report API for when an expansion has occurred during a collection.
 * @seealso MM_Collector::collectorExpanded(MM_EnvironmentBase *,
 * MM_MemorySubSpace *, uintptr_t)
 * @note This call is NOT made pre/post collection, when actual user type
 * allocation requests are being statisfied.
 ...
 */
void MM_Scavenger::collectorExpanded(MM_EnvironmentBase *env,
  ...
  if (0 == expandSize) {
    ...
    env->_scavengerStats._tenureExpandedTime +=
...
/**
 * Answer whether the subspace can expand on a collector-invoked allocate
 * request. The query is made only on collectorAllocate() type requests when the
 * allocation fails, and a decision on whether to expand the subspace to satisfy
 * the allocate is being made.
 * @seealso MM_Collector::canCollectorExpand(MM_EnvironmentBase *,
 * MM_MemorySubSpace *, uintptr_t)
 * @note Call is only made during collection and during a collectorAllocate()
 * type request to the subspace.
 * @return true if the subspace is allowed to expand, false otherwise.
 */
bool MM_Scavenger::canCollectorExpand(MM_EnvironmentBase *env,
                                      MM_MemorySubSpace *subSpace,
                                      uintptr_t expandSize) {
 ...
 * The query is made only on collectorAllocate() type requests when the
 * allocation fails, and a decision on how much to expand the subspace to
 * satisfy the allocate is being made.
 ...
 */
uintptr_t MM_Scavenger::getCollectorExpandSize(MM_EnvironmentBase *env) {
  MM_ScavengerStats *scavengerGCStats = &_extensions->scavengerStats;
  uintptr_t expandSize =
      ((uintptr_t)(scavengerGCStats->_avgTenureBytes *
                   _extensions->scavengerCollectorExpandRatio));
  expandSize =
      OMR_MIN(_extensions->scavengerMaximumCollectorExpandSize, expandSize);
  return expandSize;
}

/**
 * Perform any pre-collection work as requested by the garbage collection
 * invoker.
 */
void MM_Scavenger::internalPreCollect(MM_EnvironmentBase *env,
                                      MM_MemorySubSpace *subSpace,
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  /* Cycle state is initialized only once at the beginning of a cycle. We do not
   * want, in mid-end cycle phases, to reset some members that are initialized
   * at the beginning (such as verboseContextID).
   */
...
/**
 * Perform any post-collection work as requested by the garbage collection
 * invoker.
 */
void MM_Scavenger::internalPostCollect(MM_EnvironmentBase *env,
                                       MM_MemorySubSpace *subSpace) {
 ...
 */
bool MM_Scavenger::internalGarbageCollect(
    MM_EnvironmentBase *envBase, MM_MemorySubSpace *subSpace,
    MM_AllocateDescription *allocDescription) {
  MM_EnvironmentStandard *env = (MM_EnvironmentStandard *)envBase;
  MM_ScavengerStats *scavengerGCStats = &_extensions->scavengerStats;
  MM_MemorySubSpaceSemiSpace *subSpaceSemiSpace =
      (MM_MemorySubSpaceSemiSpace *)subSpace;
  MM_MemorySubSpace *tenureMemorySubSpace =
      subSpaceSemiSpace->getTenureMemorySubSpace();

  if (subSpaceSemiSpace->getMemorySubSpaceAllocate()
          ->shouldAllocateAtSafePointOnly()) {
    /* There is no point in doing Scavenge, since we are about to complete
     * Global, which will likely free up space in Nursery to satisfy AF that
     * triggered this Scavenge. AllocateAtSafePointOnly flag is set exactly and
     * only when Concurrent Mark execution mode is set to CONCURRENT_EXHAUSTED.
     * Ideally, we should assert that the mode is CONCURRENT_EXHAUSTED, but
     * Global GCs are not visible from here.
     */
    Trc_MM_Scavenger_percolate_concurrentMarkExhausted(
        env->getLanguageVMThread());

    bool result =
        percolateGarbageCollect(env, subSpace, NULL, CONCURRENT_MARK_EXHAUSTED,
  ...
  if (_extensions->concurrentScavenger && isBackOutFlagRaised()) {
    bool result = percolateGarbageCollect(
        env, subSpace, NULL, ABORTED_SCAVENGE,
        J9MMCONSTANT_IMPLICIT_GC_PERCOLATE_ABORTED_SCAVENGE);
  ...
  /* First, if the previous scavenge had a failed tenure of a size greater than
   * the threshold, ask parent MSS to try a collect.
   */
  if (failedTenureThresholdReached()) {
    Trc_MM_Scavenger_percolate_failedTenureThresholdReached(
        env->getLanguageVMThread(), getFailedTenureLargestObject(),
        _extensions->heap->getPercolateStats()->getScavengesSincePercolate());
    ...
    MM_AllocateDescription percolateAllocDescription(
        getFailedTenureLargestObject(), OMR_GC_ALLOCATE_OBJECT_TENURED, false,
        true);

    /* We do an aggressive percolate if the last scavenge also percolated */
    uint32_t aggressivePercolate =
        _extensions->heap->getPercolateStats()->getScavengesSincePercolate() <=
                1
    ...
    bool result =
        percolateGarbageCollect(env, subSpace, &percolateAllocDescription,
                                FAILED_TENURE, aggressivePercolate);
  ...
  /*
   * Second, if the previous scavenge failed to expand tenure, ask parent MSS to
   * try a collect.
   */
  if (expandFailed()) {
    ...
    bool result = percolateGarbageCollect(env, subSpace, NULL, EXPAND_FAILED,
                                          aggressivePercolate);
  ...
  /* If the tenure MSS is not expandable and/or  there is insufficent space left
   * to tenure the average number of bytes tenured by a scavenge then percolate
   * the collect to avoid an aborted scavenge and its associated time consuming
   * backout
   */
  if ((tenureMemorySubSpace->maxExpansionInSpace(env) +
       tenureMemorySubSpace->getApproximateActiveFreeMemorySize()) <
      scavengerGCStats->_avgTenureBytes) {
    Trc_MM_Scavenger_percolate_insufficientTenureSpace(
        env->getLanguageVMThread(),
    ...
    /* Percolate the collect to parent MSS */
    bool result =
        percolateGarbageCollect(env, subSpace, NULL, INSUFFICIENT_TENURE_SPACE,
                                J9MMCONSTANT_IMPLICIT_GC_PERCOLATE);
  ...
  /* If it has been too long since a global GC, execute one instead of a
   * scavenge. */
  // TODO Probably should rename this -Xgc option as it may not always result in
  // a ggc in futre, e.g if we implement multiple generations.
  if (_extensions->maxScavengeBeforeGlobal) {
    if (_countSinceForcingGlobalGC++ >= _extensions->maxScavengeBeforeGlobal) {
      ...
      /* Percolate the collect to parent MSS */
      bool result = percolateGarbageCollect(env, subSpace, NULL, MAX_SCAVENGES,
                                            J9MMCONSTANT_IMPLICIT_GC_PERCOLATE);
  ...
  if (!_extensions->concurrentMark) {
    ...
    float maxTenureFreeRatio =
        _extensions->heapFreeMaximumRatioMultiplier / 100.0f;
    float midTenureFreeRatio = (_extensions->heapFreeMinimumRatioMultiplier +
                                _extensions->heapFreeMaximumRatioMultiplier) /
                               200.0f;
    uintptr_t soaFreeMemorySize =
        _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_OLD) -
        _extensions->heap->getApproximateActiveFreeLOAMemorySize(
            MEMORY_TYPE_OLD);

    /* We suspect that next scavenge will cause Tenure expansion, while Tenure
     * free ratio is (was) already high enough */
    if ((scavengerGCStats->_avgTenureBytes > soaFreeMemorySize) &&
        (_extensions->freeOldHeapSizeOnLastGlobalGC >
         (_extensions->oldHeapSizeOnLastGlobalGC * midTenureFreeRatio))) {
      Trc_MM_Scavenger_percolate_preventTenureExpand(
          env->getLanguageVMThread());

      bool result =
          percolateGarbageCollect(env, subSpace, NULL, PREVENT_TENURE_EXPAND,
                                  J9MMCONSTANT_IMPLICIT_GC_PERCOLATE);
      Assert_MM_true(result);
      return true;
    }
    /* There was Tenure heap growth since last Global GC, and we suspect
     * (assuming live set size did not grow) we might be going beyond max free
     * bound. */
    if ((_extensions->heap->getActiveMemorySize(MEMORY_TYPE_OLD) >
         _extensions->oldHeapSizeOnLastGlobalGC) &&
        (_extensions->heap->getActiveMemorySize(MEMORY_TYPE_OLD) *
             maxTenureFreeRatio <
         (_extensions->heap->getActiveMemorySize(MEMORY_TYPE_OLD) -
          previousUsedOldHeap))) {
      ...
      bool result = percolateGarbageCollect(env, subSpace, NULL,
  ...
  bool shouldPercolate =
      _delegate.internalGarbageCollect_shouldPercolateGarbageCollect(
          env, &percolateReason, &gcCode);
  if (shouldPercolate) {
    bool didPercolate =
        percolateGarbageCollect(env, subSpace, NULL, percolateReason, gcCode);
  ...
  if (isRememberedSetInOverflowState() && _extensions->scavengerRsoScanUnsafe) {
    /* NOTE: No need to set that the collect was unsuccessful - we will actually
     * execute the scavenger after percolation.
     */

    Trc_MM_Scavenger_percolate_rememberedSetOverflow(
        env->getLanguageVMThread());

    /* Percolate the collect to parent MSS */
    percolateGarbageCollect(env, subSpace, NULL, RS_OVERFLOW,
                            J9MMCONSTANT_IMPLICIT_GC_PERCOLATE);
  }
   ...
   * the fact so other parties can react, e.g concurrrent can adjust KO
   * threshold
   */
 ...
 *
 * @param allocate descriptor describing the allocation request the collector
 * should aim to satify
 * @param percolateReason code for the percolate
 * @param gcCode GC code requested
 * @return true if Global GC was executed, false if concurrent kickoff forced or
 * Global GC is not possible
 */
bool MM_Scavenger::percolateGarbageCollect(
    MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
    MM_AllocateDescription *allocDescription, PercolateReason percolateReason,
    uint32_t gcCode) {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  /* before doing percolate global, we have to complete potentially ongoing
   * concurrent Scavenge cycle */
  if (_extensions->concurrentScavenger && isConcurrentInProgress()) {
    triggerConcurrentScavengerTransition(env, allocDescription);
  }
#endif

  /* save the cycle state since we are about to call back into the collector to
   * start a new global cycle */
  MM_CycleState *scavengeCycleState = env->_cycleState;
  Assert_MM_true(NULL != scavengeCycleState);
  env->_cycleState = NULL;

  /* Set last percolate reason */
  _extensions->heap->getPercolateStats()->setLastPercolateReason(
      percolateReason);

  /* Percolate the collect to parent MSS */
  bool result =
      subSpace->percolateGarbageCollect(env, allocDescription, gcCode);
...
/**
 * Re-size all structures which are dependent on the current size of the heap.
 * No new memory has been added to a heap reconfiguration.  This call typically
 * is the result of having segment range changes (memory redistributed between
 * segments) or the meaning of memory changed.
 *
 */
void MM_Scavenger::heapReconfigured(MM_EnvironmentBase *env) {}

void MM_Scavenger::globalCollectionStart(MM_EnvironmentBase *env) {
...
void MM_Scavenger::globalCollectionComplete(MM_EnvironmentBase *env) {
...
void MM_Scavenger::reportGCCycleStart(MM_EnvironmentStandard *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  MM_CommonGCData commonData;
...
uintptr_t MM_Scavenger::calculateTiltRatio() {
   ...
   * 								Nursery size
   *
   *  To avoid of using of uint64_t and prevent an overflow of uintptr_t on
   * 32-bit platforms change formula to
   *
   * 								New_memory_size
   * 	tilt_ratio =  --------------------------------------------------
   * 							Nursery size / 100
   *
   * Quality of calculation if good enough because Nursery size is a large
   * number (at least grater then 1K)
   */
...
void MM_Scavenger::reportGCIncrementEnd(MM_EnvironmentStandard *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  MM_CollectionStatisticsStandard *stats =
      (MM_CollectionStatisticsStandard *)
          env->_cycleState->_collectionStatistics;
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_GC_INCREMENT_END(
      _extensions->privateHookInterface, env->getOmrVMThread(), stats->_endTime,
      J9HOOK_MM_PRIVATE_GC_INCREMENT_END, stats);

  /* reset fragmentation indicator after reporting fragmentation */
  stats->_tenureFragmentation = NO_FRAGMENTATION;
}

uintptr_t MM_Scavenger::calculateTenureMask() {
  ...
  if (_extensions->scvTenureStrategyFixed) {
    newMask |=
        calculateTenureMaskUsingFixed(_extensions->scvTenureFixedTenureAge);
  }
  if (_extensions->scvTenureStrategyAdaptive) {
    newMask |=
        calculateTenureMaskUsingFixed(_extensions->scvTenureAdaptiveTenureAge);
  }
  if (_extensions->scvTenureStrategyLookback) {
    newMask |= calculateTenureMaskUsingLookback(
        _extensions->scvTenureStrategySurvivalThreshold);
  }
  if (_extensions->scvTenureStrategyHistory) {
    newMask |= calculateTenureMaskUsingHistory(
        _extensions->scvTenureStrategySurvivalThreshold);
  }
...
uintptr_t MM_Scavenger::calculateTenureMaskUsingLookback(
    double minimumSurvivalRate) {
  ...
  /* We need a normalized representation of the initial generation size over
   * history. */
  ...
  for (uintptr_t index = 1; index < SCAVENGER_FLIP_HISTORY_SIZE; index++) {
    uintptr_t initialGenerationSize =
        stats->getFlipHistory(index)->_flipBytes[1] +
  ...
  /* Second, we calculate the standard deviation of the initial generation size
   * over history. */
  ...
  for (uintptr_t index = 1; index < SCAVENGER_FLIP_HISTORY_SIZE; index++) {
    ...
    if (initialGenerationSize > 0) {
      double delta =
          (double)initialGenerationSize - averageInitialGenerationSize;
  ...
  if (0 == count) {
    ...
    standardDeviationOfInitialGenerationSize =
        sqrt(accumulatedSquareDeltas / (double)count);
  }

  /* This normalized initial generation size (calculated using the standard
   * deviation) is used for determining how far back in history we should be
   * looking. The larger this is, the shallower we look back.
   */
  uintptr_t normalizedInitialGenerationSize =
      (uintptr_t)OMR_MAX(0.0, averageInitialGenerationSize -
                                  standardDeviationOfInitialGenerationSize);

  for (uintptr_t age = 0; age <= OBJECT_HEADER_AGE_MAX + 1; ++age) {
    /* skip the first row (it's the current scavenge, and is all zero right
     * now). Also skip the last row in the history (there's no previous to
     * compare it to)
     */
    bool shouldTenureThisAge = true;
    uintptr_t currentGenerationBytes =
        stats->getFlipHistory(1)->_flipBytes[age];

    /* The lookback distance is determined by the size of the generation.
     * We use a simple logarithmic heuristic.
     * If the generation would occupy at least half of survivor space, only look
     * back one collection. If the generation would occupy at least a quarter of
     * survivor space, look back two collections. If the generation would occupy
     * at least an eighth of survivor space, look back three collections. . . .
     */
    ...
    if (requiredLookback >= age) {
      /* this generation is too young to have enough history to satisfy the
       * lookback */
      shouldTenureThisAge = false;
    } else {
      Assert_MM_true(1 <= requiredLookback);
      Assert_MM_true(requiredLookback < SCAVENGER_FLIP_HISTORY_SIZE);
      for (uintptr_t lookback = 1;
           (lookback <= requiredLookback) && shouldTenureThisAge; lookback++) {
    ...
    if (shouldTenureThisAge) {
      /* Objects in this age are historically still dying at a rate of less than
       * 1%. Tenure them. */
...
uintptr_t MM_Scavenger::calculateTenureMaskUsingHistory(
    double minimumSurvivalRate) {
  Assert_MM_true(0.0 <= minimumSurvivalRate);
  Assert_MM_true(1.0 >= minimumSurvivalRate);
  ...
  for (uintptr_t age = 0; age < OBJECT_HEADER_AGE_MAX; ++age) {
    ...
    /* Skip the first row in the history (it's the current scavenge, and is all
     * zero right now). Also skip the last row in the history (there's no
     * previous to compare it to).
     */
    for (uintptr_t lookback = 1; lookback < SCAVENGER_FLIP_HISTORY_SIZE - 1;
         lookback++) {
      uintptr_t currentBytes =
          stats->getFlipHistory(lookback + 1)->_flipBytes[age];
      uintptr_t nextBytes =
          stats->getFlipHistory(lookback)->_flipBytes[age + 1] +
          stats->getFlipHistory(lookback)->_tenureBytes[age + 1];
      if (0 == currentBytes) {
        shouldTenureThisAge = false;
        break;
      } else if (((double)nextBytes / (double)currentBytes) <
                 minimumSurvivalRate) {
        shouldTenureThisAge = false;
        break;
      }
    ...
    if (shouldTenureThisAge) {
      /* Objects in this age historically survive at a rate above the minimum
       * survival rate. Tenure them. */
      mask |= ((uintptr_t)1 << age);
    }
  }
...
uintptr_t MM_Scavenger::calculateTenureMaskUsingFixed(uintptr_t tenureAge) {
...
void MM_Scavenger::resetTenureLargeAllocateStats(MM_EnvironmentBase *env) {
  MM_MemorySpace *defaultMemorySpace =
      _extensions->heap->getDefaultMemorySpace();
  MM_MemorySubSpace *tenureMemorySubspace =
      defaultMemorySpace->getTenureMemorySubSpace();
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
bool MM_Scavenger::isConcurrentWorkAvailable(MM_EnvironmentBase *env) {
...
bool MM_Scavenger::scavengeInit(MM_EnvironmentBase *env) {
  ...
  while ((walkThread = threadIterator.nextOMRVMThread()) != NULL) {
    MM_EnvironmentStandard *threadEnvironment =
        MM_EnvironmentStandard::getEnvironment(walkThread);
...
bool MM_Scavenger::scavengeRoots(MM_EnvironmentBase *env) {
  Assert_MM_true(concurrent_state_roots == _concurrentState);

  MM_ConcurrentScavengeTask scavengeTask(
      env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_ROOTS,
      env->_cycleState);
  _dispatcher->run(env, &scavengeTask);
...
bool MM_Scavenger::scavengeScan(MM_EnvironmentBase *envBase) {
  ...
  MM_ConcurrentScavengeTask scavengeTask(
      env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_SCAN,
      env->_cycleState);
  _dispatcher->run(env, &scavengeTask);
...
bool MM_Scavenger::scavengeComplete(MM_EnvironmentBase *envBase) {
  MM_EnvironmentStandard *env = MM_EnvironmentStandard::getEnvironment(envBase);
  ...
  MM_ConcurrentScavengeTask scavengeTask(
      env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_COMPLETE,
      env->_cycleState);
  _dispatcher->run(env, &scavengeTask);
...
void MM_Scavenger::mutatorSetupForGC(MM_EnvironmentBase *envBase) {
  MM_EnvironmentStandard *env = MM_EnvironmentStandard::getEnvironment(envBase);
...
void MM_Scavenger::threadReleaseCaches(MM_EnvironmentBase *envBase,
                                       bool final) {
  MM_EnvironmentStandard *env = MM_EnvironmentStandard::getEnvironment(envBase);
  ...
  if (isConcurrentInProgress()) {
    ...
    if (NULL != env->_survivorCopyScanCache) {
      Assert_MM_true(env->_survivorCopyScanCache->flags &
                     OMR_SCAVENGER_CACHE_TYPE_COPY);
      env->_survivorCopyScanCache->flags &= ~OMR_SCAVENGER_CACHE_TYPE_COPY;
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
      env->_scavengerStats._releaseScanListCount += 1;
    ...
    if (NULL != env->_deferredCopyCache) {
      Assert_MM_true(env->_deferredCopyCache->flags &
    ...
    if (NULL != env->_tenureCopyScanCache) {
      Assert_MM_true(env->_tenureCopyScanCache->flags &
                     OMR_SCAVENGER_CACHE_TYPE_COPY);
      env->_tenureCopyScanCache->flags &= ~OMR_SCAVENGER_CACHE_TYPE_COPY;
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
      env->_scavengerStats._releaseScanListCount += 1;
    ...
    if (final) {
      /* If it's an intermediate release (for example mutator threads releasing
       * VM access in a middle of Concurrent Scavenger cycle, keep copy cache
       * remainders around (do not abandon yet), to be reused if the threads
       * re-acquires VM access during the same CS cycle.
       */
      abandonSurvivorTLHRemainder(env);
      abandonTenureTLHRemainder(env, true);
    }
...
bool MM_Scavenger::scavengeIncremental(MM_EnvironmentBase *env) {
  Assert_MM_mustHaveExclusiveVMAccess(env->getOmrVMThread());
  bool result = false;
  bool timeout = false;
  ...
  while (!timeout) {
    switch (_concurrentState) {
      ...
      case concurrent_state_roots: {
        ...
        if (isBackOutFlagRaised()) {
          /* if we aborted during root processing, continue with the cycle while
           * still in STW mode */
...
void MM_Scavenger::workThreadProcessRoots(MM_EnvironmentStandard *env) {
  workerSetupForGC(env);
  ...
  /* Push any thread local copy caches to scan queue and abandon unused memory
   * to make it walkable. This is important to do only for GC threads that will
   * not be used in concurrent phase, but at this point we don't know which
   * threads Scheduler will not use, so we do it for every thread.
   */
...
void MM_Scavenger::workThreadScan(MM_EnvironmentStandard *env) {
  /* This is where the most of scan work should occur in CS. Typically as a
   * concurrent task (background threads), but in some corner cases it could be
   * scheduled as a STW task */
  ...
  /* We might have yielded without exausting scan work. Push any open caches to
   * the scan queue, so that GC threads from final STW phase pick them up. Most
   * of the time, STW phase will have a superset of GC threads, so they could
   * just resume the work on their own caches, but this is not 100% guarantied
   * (the control of what threads are inolved is in Dispatcher's domain).
   */
  threadReleaseCaches(env, true);
...
void MM_Scavenger::workThreadComplete(MM_EnvironmentStandard *env) {
  ...
  /* Complete scan loop regardless if we already aborted. If so, the scan
   * operation will just fix up pointers that still point to forwarded objects.
   * This is important particularly for Tenure space where recovery procedure
   * will not walk the Tenure space for exhaustive fixup.
   */
  completeScan(env);

  if (!isBackOutFlagRaised()) {
    /* If aborted, the clearable work will be done by mandatory percolate global
     * GC */
    rootScanner.scanClearable(env);
  }
  rootScanner.flush(env);
...
uintptr_t MM_Scavenger::masterThreadConcurrentCollect(MM_EnvironmentBase *env) {
  if (concurrent_state_scan == _concurrentState) {
    clearIncrementGCStats(env, false);
    ...
    MM_ConcurrentScavengeTask scavengeTask(
        env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_SCAN,
        env->_cycleState);
    /* Concurrent background task will run with different (typically lower)
     * number of threads. */
    _dispatcher->run(env, &scavengeTask,
                     _extensions->concurrentScavengerBackgroundThreads);

    /* Now that we are done with concurrent scanning in this cycle (where we
     * could possibly be interested in its value), record shouldYield Flag for
     * reporting purposes and reset it. */
    if (_shouldYield) {
      if (NULL == _extensions->gcExclusiveAccessThreadId) {
        /* We terminated concurrent cycle due to a external request. We will not
         * move to 'complete' phase, but stay in concurrent scan phase and try
         * to resume work after the external party is done (when we are able to
         * regain VM access)
         */
      ...
      _activeSubSpace->flip(env,
                            MM_MemorySubSpaceSemiSpace::disable_allocation);
    }
    ...
    /* someone else might have done this phase (and the rest of the cycle),
     * forced in STW, before we even got a chance to run. */
    Assert_MM_true(concurrent_state_idle == _concurrentState);
    return 0;
  }
}

void MM_Scavenger::preConcurrentInitializeStatsAndReport(
    MM_EnvironmentBase *env, MM_ConcurrentPhaseStatsBase *stats) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  Assert_MM_true(NULL == env->_cycleState);
  env->_cycleState = &_cycleState;
...
void MM_Scavenger::postConcurrentUpdateStatsAndReport(
    MM_EnvironmentBase *env, MM_ConcurrentPhaseStatsBase *stats,
    UDATA bytesConcurrentlyScanned) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
...
void MM_Scavenger::switchConcurrentForThread(MM_EnvironmentBase *env) {
  /* If a thread local counter is behind the global one (or ahead in case of a
   * rollover), we need to trigger a switch. It means we recently transitioned
   * from a cycle start to cycle end or vice versa. If state is idle, we just
   * completed a cycle. If state is scan (or rarely complete), we just started a
   * cycle (and possibly even complete concurrent work)
   */

  Assert_MM_false((concurrent_state_init == _concurrentState) ||
                  (concurrent_state_roots == _concurrentState));
  if (env->_concurrentScavengerSwitchCount != _concurrentScavengerSwitchCount) {
    Trc_MM_Scavenger_switchConcurrent(
        env->getLanguageVMThread(), _concurrentState,
...
void MM_Scavenger::triggerConcurrentScavengerTransition(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription) {
  /* About to block. A dedicated master GC thread will take over for the
   * duration of STW phase (start or end) */
  ...
  /* Ensure switchConcurrentForThread is invoked for each mutator thread. It
   * will be done indirectly, first time a thread acquires VM access after
   * exclusive VM access is released, through a VM access hook. */
  GC_OMRVMThreadListIterator threadIterator(_extensions->getOmrVM());
  OMR_VMThread *walkThread = NULL;
...
void MM_Scavenger::completeConcurrentCycle(MM_EnvironmentBase *env) {
  /* this is supposed to be called by an external cycle (for example
   * ConcurrentGC, STW phase) that is just to be started, but cannot before
   * Scavenger is complete */
  Assert_MM_true(NULL == env->_cycleState);
  if (isConcurrentInProgress()) {
    env->_cycleState = &_cycleState;
...
#if defined(OMR_ENV_DATA64) && defined(OMR_GC_FULL_POINTERS)
void MM_Scavenger::scavenger_poisonSlots(MM_EnvironmentBase *env) {
  /* This will poison only the root slots */
  _delegate.poisonSlots(env);
}
void MM_Scavenger::scavenger_healSlots(MM_EnvironmentBase *env) {
