...
void MM_Scavenger::hookGlobalCollectionStart(
    J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
...
void MM_Scavenger::hookGlobalCollectionComplete(
    J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
 ...
 */
void* MM_Scavenger::createSweepPoolState(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool)
{
 ...
 */
void MM_Scavenger::deleteSweepPoolState(MM_EnvironmentBase* env, void* sweepPoolState)
{
    Assert_MM_unreachable();
}
 ...
 */
MM_Scavenger* MM_Scavenger::newInstance(MM_EnvironmentStandard* env, MM_HeapRegionManager* regionManager)
{
...
bool MM_Scavenger::initialize(MM_EnvironmentBase* env)
{
    ...
    (*mmOmrHooks)
        ->J9HookRegisterWithCallSite(
            mmOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_START, hookGlobalCollectionStart, OMR_GET_CALLSITE(), (void*)this);
    (*mmOmrHooks)
        ->J9HookRegisterWithCallSite(
            mmOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_END, hookGlobalCollectionComplete, OMR_GET_CALLSITE(), (void*)this);
     ...
     * deferred) So long as (N * _cachesPerThread) cache entries exist,the head of the scan list will contain a valid
     * entry. We set the appropriate number of caches per thread here */
    ...
    uintptr_t totalActiveCacheCount
        = _extensions->heap->getActiveMemorySize(MEMORY_TYPE_NEW) / _extensions->scavengerScanCacheMinimumSize;
...
void MM_Scavenger::masterSetupForGC(MM_EnvironmentStandard* env)
{
    ...
    /* Cache heap ranges for fast "valid object" checks (this can change in an expanding heap situation, so we refetch
     * every cycle) */
    ...
    if (0 == _extensions->scvTenureAdaptiveTenureAge) {
        ...
        uintptr_t tenureAgeAdjustement
            = MM_Math::floorLog2(_extensions->heap->getActiveMemorySize(MEMORY_TYPE_NEW) / MINIMUM_NEW_SPACE_SIZE);
...
void MM_Scavenger::reportScavengeEnd(MM_EnvironmentStandard* env, bool lastIncrement)
{
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_SCAVENGE_END(_extensions->privateHookInterface, env->getOmrVMThread(),
        omrtime_hires_clock(), J9HOOK_MM_PRIVATE_SCAVENGE_END, env->_cycleState->_activeSubSpace, lastIncrement);
}
...
void MM_Scavenger::reportGCStart(MM_EnvironmentStandard* env)
{
    ...
    TRIGGER_J9HOOK_MM_OMR_LOCAL_GC_START(_extensions->omrHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
        J9HOOK_MM_OMR_LOCAL_GC_START, _extensions->globalGCStats.gcCount, _extensions->scavengerStats._gcCount);
}
...
void MM_Scavenger::reportGCEnd(MM_EnvironmentStandard* env)
{
    ...
    Trc_MM_LocalGCEnd(env->getLanguageVMThread(), _extensions->incrementScavengerStats._rememberedSetOverflow,
        ...
        _extensions->incrementScavengerStats._scanCacheOverflow, _extensions->incrementScavengerStats._failedFlipCount,
        _extensions->incrementScavengerStats._failedFlipBytes, _extensions->incrementScavengerStats._failedTenureCount,
        _extensions->incrementScavengerStats._failedTenureBytes, _extensions->incrementScavengerStats._flipCount,
        ...
        (_extensions->largeObjectArea ? _extensions->heap->getApproximateActiveFreeLOAMemorySize(MEMORY_TYPE_OLD)
                                      : (uintptr_t)0),
    ...
    TRIGGER_J9HOOK_MM_OMR_LOCAL_GC_END(_extensions->omrHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
        ...
        _extensions->incrementScavengerStats._gcCount, _extensions->incrementScavengerStats._rememberedSetOverflow,
        _extensions->incrementScavengerStats._causedRememberedSetOverflow,
        _extensions->incrementScavengerStats._scanCacheOverflow, _extensions->incrementScavengerStats._failedFlipCount,
        _extensions->incrementScavengerStats._failedFlipBytes, _extensions->incrementScavengerStats._failedTenureCount,
...
void MM_Scavenger::clearThreadGCStats(MM_EnvironmentBase* env, bool firstIncrement)
{
    env->_scavengerStats.clear(firstIncrement);
}

void MM_Scavenger::clearIncrementGCStats(MM_EnvironmentBase* env, bool firstIncrement)
{
...
void MM_Scavenger::mergeGCStatsBase(
    MM_EnvironmentBase* env, MM_ScavengerStats* finalGCStats, MM_ScavengerStats* scavStats)
{
    ...
    finalGCStats->_failedTenureLargest = OMR_MAX(scavStats->_failedTenureLargest, finalGCStats->_failedTenureLargest);
...
void MM_Scavenger::mergeThreadGCStats(MM_EnvironmentBase* env)
{
    ...
    Trc_MM_ParallelScavenger_parallelStats(env->getLanguageVMThread(), (uint32_t)env->getSlaveID(),
        ...
        (uint32_t)scavStats->_syncStallCount, scavStats->_acquireFreeListCount, scavStats->_releaseFreeListCount,
        scavStats->_acquireScanListCount, scavStats->_releaseScanListCount);
}

void MM_Scavenger::mergeIncrementGCStats(MM_EnvironmentBase* env, bool lastIncrement)
{
    ...
    /* Language specific stats were supposed to be merged directly from thread local to cycle global. No need to merge
     * them here. */
    ...
    if (canCalcGCStats(env)) {
        ...
        if (scavengerGCStats->_gcCount > 1) {
            scavengerGCStats->_avgInitialFree = (uintptr_t)MM_Math::weightedAverage(
                (float)scavengerGCStats->_avgInitialFree, (float)initialFree, INITIAL_FREE_HISTORY_WEIGHT);
            ...
            scavengerGCStats->_avgTenureBytes = (uintptr_t)MM_Math::weightedAverage(
                (float)scavengerGCStats->_avgTenureBytes, (float)tenureAggregateBytes, TENURE_BYTES_HISTORY_WEIGHT);
            ...
            /* We can assume that in the first GC, about half of the objects are long lived objects, so we use this
             * heuristic to give a rough estimate for the starting point */
 ...
 */
MMINLINE uintptr_t MM_Scavenger::calculateOptimumCopyScanCacheSize(MM_EnvironmentStandard* env)
{
...
MMINLINE MM_CopyScanCacheStandard* MM_Scavenger::reserveMemoryForAllocateInSemiSpace(
    MM_EnvironmentStandard* env, omrobjectptr_t objectToEvacuate, uintptr_t objectReserveSizeInBytes)
{
    ...
    if ((NULL != env->_survivorCopyScanCache)
        && (((uintptr_t)env->_survivorCopyScanCache->cacheTop - (uintptr_t)env->_survivorCopyScanCache->cacheAlloc)
            >= cacheSize)) {
 ...
 */
MMINLINE bool MM_Scavenger::copyAndForward(MM_EnvironmentStandard* env, volatile omrobjectptr_t* objectPtrIndirect)
{
    bool toReturn = false;
 ...
 * @return false otherwise
 */
MMINLINE bool MM_Scavenger::copyAndForward(MM_EnvironmentStandard* env, GC_SlotObject* slotObject)
{
...
bool MM_Scavenger::copyObjectSlot(MM_EnvironmentStandard* env, volatile omrobjectptr_t* slotPtr)
{
    return copyAndForward(env, slotPtr);
}

bool MM_Scavenger::copyObjectSlot(MM_EnvironmentStandard* env, GC_SlotObject* slotObject)
{
    return copyAndForward(env, slotObject);
}

omrobjectptr_t MM_Scavenger::copyObject(MM_EnvironmentStandard* env, MM_ForwardedHeader* forwardedHeader)
{
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
...
omrobjectptr_t MM_Scavenger::copy(MM_EnvironmentStandard* env, MM_ForwardedHeader* forwardedHeader)
{
    ...
    if (0 == (((uintptr_t)1 << objectAge) & _tenureMask)) {
        ...
        if (NULL != copyCache) {
            ...
            Assert_GC_true_with_message4(env, objectCopySizeInBytes <= spaceAvailableForObject,
                "Corruption in Evacuate at %p: calculated object size %zu larger then available %zu, Forwarded Header "
                "at %p\n",
                forwardedHeader->getObject(), objectCopySizeInBytes, spaceAvailableForObject, forwardedHeader);

            copyCache
                = reserveMemoryForAllocateInTenureSpace(env, forwardedHeader->getObject(), objectReserveSizeInBytes);
            if (NULL != copyCache) {
        ...
        if (_extensions->scavengerAlignHotFields && HOTFIELD_SHOULD_ALIGN(hotFieldsDescriptor)) {
             ...
             * but free entry created by sweep in tenure could be less than that (since some of unused padding can
             * overlap with next copied object)).
             * we limit this optimization for arrays up to the size of 2 cache lines, beyond which the benefits of the
             * optimization are believed to be non-existant */
        ...
        if (NULL != copyCache) {
            ...
            Trc_MM_Scavenger_tenureAllocateFailed(
                env->getLanguageVMThread(), objectReserveSizeInBytes, env->_scavengerStats._failedTenureLargest, "yes");
            ...
            copyCache
                = reserveMemoryForAllocateInSemiSpace(env, forwardedHeader->getObject(), objectReserveSizeInBytes);
            if (NULL != copyCache) {
                /* Adjust the age value*/
                if (objectAge < OBJECT_HEADER_AGE_MAX) {
                    objectAge += 1;
    ...
    if (0 != hotFieldsAlignment) {
        ...
        if ((remainingInCacheLine < objectCopySizeInBytes) && (alignmentBias < remainingInCacheLine)) {
            hotFieldPadSize = ((remainingInCacheLine + _cacheLineAlignment) - (alignmentBias % _cacheLineAlignment))
                % _cacheLineAlignment;
         ...
         * removing the excess reserve since we already accounted for the hotFieldPadSize by bumping the destination
         * pointer and now we need to revert to the amount needed for the object allocation and its array alignment so
         * the rest of the method continues to function without needing to know about this extra alignment calculation
         */
    ...
    if (IS_CONCURRENT_ENABLED) {
         ...
         * For larger objects, there is only one copy (threads setup destination header, one wins, and other participate
         * in copying or wait till copy is complete). 1024 is somewhat arbitrary threshold, so that most of time we do
         * not have to go through relatively expensive setup procedure.
         */
    ...
    if (originalDestinationObjectPtr == destinationObjectPtr) {
        ...
        if (!allowDuplicateOrConcurrentDisabled) {
            ...
            /* Try to copy more aligned sections. Once no more sections to copy, wait till other threads are done with
             * their sections */
        ...
        omrtty_printf("{SCAV: Copied %p[%p] -> %p[%p]}\n", forwardedHeader->getObject(),
            *((uintptr_t*)(forwardedHeader->getObject())), destinationObjectPtr, *((uintptr_t*)destinationObjectPtr));
    ...
    if (originalDestinationObjectPtr == destinationObjectPtr) {
        ...
        /* We have not used the reserved space now, but we will for subsequent allocations. If this space was reserved
         * for an individual object, we might have created a TLH remainder from previous cache just before reserving
         * this space. This space eventaully can create another remainder. At that point, ideally (to recycle as much
         * memory as possibly) we could enqueue this remainder, but as a simple solution we will now abandon the current
         * remainder (we assert across the code, there is at most one at a give point of time). If we see large amount
         * of discards even with low discard threshold, we may reconsider enqueueing discarded TLHs.
         */
        if (copyCache->flags & OMR_SCAVENGER_CACHE_TYPE_TENURESPACE) {
        ...
        /* Failed to forward (someone else did it). Re-fetch the forwarding info and (for Concurrent Scavenger only)
         * ensure it's fully copied before letting the caller expose this new version of the object */
...
GC_ObjectScanner* MM_Scavenger::getObjectScanner(
...
uintptr_t MM_Scavenger::getArraySplitAmount(MM_EnvironmentStandard* env, uintptr_t sizeInElements)
{
...
bool MM_Scavenger::splitIndexableObjectScanner(MM_EnvironmentStandard* env, GC_ObjectScanner* objectScanner,
 ...
 */
MMINLINE void MM_Scavenger::updateCopyScanCounts(MM_EnvironmentBase* env, uint64_t slotsScanned, uint64_t slotsCopied)
{
...
MMINLINE bool MM_Scavenger::scavengeObjectSlots(MM_EnvironmentStandard* env, MM_CopyScanCacheStandard* scanCache,
    omrobjectptr_t objectPtr, uintptr_t flags, omrobjectptr_t* rememberedSetSlot)
{
    ...
    if (objectScanner->isIndexableObject()) {
        /* set scanning bounds for this scanner; if non-empty tail, clone scanner into split array cache and add cache
         * to worklist */
    ...
    if (shouldRemember && (NULL != rememberedSetSlot)) {
        ...
        Assert_MM_true(
            objectPtr == (omrobjectptr_t)((uintptr_t)*rememberedSetSlot & ~(uintptr_t)DEFERRED_RS_REMOVE_FLAG));
    ...
    if (_extensions->shouldScavengeNotifyGlobalGCOfOldToOldReference() && IS_CONCURRENT_ENABLED && !isParentInNewSpace
        && !shouldRemember) {
        /* Old object that has only references to old objects. If parent object has already been scanned (in Marking
         * sense)
         * since it has been tenured, let Concurrent Marker know it has a newly created old reference, otherwise it may
         * miss to find it. */
...
void MM_Scavenger::deepScanOutline(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr,
    uintptr_t priorityFieldOffset1, uintptr_t priorityFieldOffset2)
{
    ...
    MM_EnvironmentStandard* env, omrobjectptr_t objectPtr, MM_CopyScanCacheStandard* scanCache)
{
    ...
    if (!scanCache->_hasPartiallyScannedObject) {
        ...
        if (objectScanner->isIndexableObject()) {
            /* set scanning bounds for this scanner; if non-empty tail, add split array cache to worklist and clone this
             * indexableScanner into split cache */
            if (!splitIndexableObjectScanner(
                    env, objectScanner, scanCache->_arraySplitIndex, scanCache->_arraySplitRememberedSlot)) {
                /* scan to end of array if can't split */
                ((GC_IndexableObjectScanner*)objectScanner)->scanToLimit();
            }
        }
    ...
    if (scanCache->_shouldBeRemembered) {
        if (NULL != scanCache->_arraySplitRememberedSlot) {
            ...
            Assert_MM_true(objectPtr
                == (omrobjectptr_t)(
                    (uintptr_t) * (scanCache->_arraySplitRememberedSlot) & ~(uintptr_t)DEFERRED_RS_REMOVE_FLAG));
            /* Set the remembered set slot to the object pointer in case it was still marked for removal. */
...
MMINLINE void MM_Scavenger::flushBuffersForGetNextScanCache(MM_EnvironmentStandard* env)
{
    _delegate.flushReferenceObjects(env);
    flushRememberedSet(env);
}

MM_CopyScanCacheStandard* MM_Scavenger::getNextScanCache(MM_EnvironmentStandard* env)
{
    ...
    if (NULL != cache) {
        /* deferred copy caches are used to merge memory-contiguous caches that got chopped up due to large objects not
         * fitting and resuing remainder. we want to delay scanning them as much as possible (up to the size of the
         * original cache size being chopped up), but we still want to do it before we synchronizing on scan queue and
 ...
 */
void MM_Scavenger::completeScanCache(MM_EnvironmentStandard* env, MM_CopyScanCacheStandard* scanCache)
{
    ...
    if (scanCache->isSplitArray()) {
        ...
        while (isWorkAvailableInCache(scanCache)) {
            GC_ObjectHeapIteratorAddressOrderedList heapChunkIterator(
                _extensions, (omrobjectptr_t)scanCache->scanCurrent, (omrobjectptr_t)scanCache->cacheAlloc, false);
            /* Advance the scan pointer to the top of the cache to signify that this has been scanned */
            scanCache->scanCurrent = scanCache->cacheAlloc;
            ...
            while ((objectPtr = heapChunkIterator.nextObjectNoAdvance()) != NULL) {
                ...
                bool shouldBeRemembered
                    = scavengeObjectSlots(env, scanCache, objectPtr, GC_ObjectScanner::scanHeap, NULL);
                if (shouldBeRemembered) {
                    rememberObject(env, objectPtr);
                }
            }
    ...
    scanCache->flags &= ~OMR_SCAVENGER_CACHE_TYPE_SCAN;
    /* Done with the cache - build a free list entry in the hole, release the cache to the free list (if not used), and
     * continue */
 ...
 */
void MM_Scavenger::incrementalScanCacheBySlot(MM_EnvironmentStandard* env, MM_CopyScanCacheStandard* scanCache)
{
nextCache:
    /* mark that cache is in use as a scan cache */
    Assert_MM_true(0 == (scanCache->flags & OMR_SCAVENGER_CACHE_TYPE_SCAN));
...
bool MM_Scavenger::completeScan(MM_EnvironmentStandard* env)
{
    ...
    /* take a snapshot of ID of this scan cycle (which will change in getNextScanCache() once all threads agree to leave
     * the scan loop) */
...
void MM_Scavenger::workThreadGarbageCollect(MM_EnvironmentStandard* env)
{
    ...
    /*
     * There is a hidden assumption that RS Overflow flag would not be changed between beginning of scavenge and this
     * point, otherwise it is hang situation when one group of threads might wait for more work in complete scan and
     * another group of threads be got stuck on sync point in scavengeRememberedSetOverflow()
     *
...
void MM_Scavenger::addAllRememberedObjectsToOverflow(MM_EnvironmentStandard* env, MM_RSOverflow* overflow)
{
...
void MM_Scavenger::addToRememberedSetFragment(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
...
void MM_Scavenger::rememberObject(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
...
bool MM_Scavenger::isRememberedThreadReference(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
    Assert_MM_false(IS_CONCURRENT_ENABLED);
    Assert_MM_true(NULL != objectPtr);
    Assert_MM_true(!isObjectInNewSpace(objectPtr));
...
bool MM_Scavenger::processRememberedThreadReference(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
    Assert_MM_false(IS_CONCURRENT_ENABLED);
    Assert_MM_true(NULL != objectPtr);
    Assert_MM_true(!isObjectInNewSpace(objectPtr));
 ...
 ********************************************************************/
bool MM_Scavenger::shouldRememberObject(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
 ...
 */
MMINLINE bool MM_Scavenger::scavengeRememberedObject(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
            ...
            while ((objectPtr = objectIterator.nextObject()) != NULL) {
                if (_extensions->objectModel.isRemembered(objectPtr)) {
                    ...
                    if (shouldBeRemembered) {
                        ...
                        /* Add tenured object to the thread's remembered set list if possible. Otherwise, this will
                         * force setRememberedSetOverflowState(). */
                        addToRememberedSetFragment(env, objectPtr);
                    } else {
                        ...
                        if (_extensions->shouldScavengeNotifyGlobalGCOfOldToOldReference() && !IS_CONCURRENT_ENABLED) {
                            /* Inform interested parties (Concurrent Marker) that an object has been removed from the
                             * remembered set. In non-concurrent Scavenger this is the only way to create an old-to-old
                             * reference, that has parent object being marked. In Concurrent Scavenger, it can be
                             * created even with parent object that was not in RS to start with. So this is handled in a
                             * more generic spot when object is scavenged and is unnecessary to do it here.
                             */
                            oldToOldReferenceCreated(env, objectPtr);
                        }
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */
        ...
        if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
            ...
            while ((slotPtr = (omrobjectptr_t*)remSetSlotIterator.nextSlot()) != NULL) {
                ...
                if (NULL == objectPtr) {
                    ...
                    /* The object did not have Nursery references at initial RS scan, but one could have been added
                     * during CS cycle by a mutator. */
                    if (!IS_CONCURRENT_ENABLED || !shouldRememberObject(env, objectPtr)) {
                        ...
                        /* Inform interested parties (Concurrent Marker) that an object has been removed from the
                         * remembered set. In non-concurrent Scavenger this is the only way to create an old-to-old
                         * reference, that has parent object being marked. In Concurrent Scavenger, it can be created
                         * even with parent object that was not in RS to start with. So this is handled in a more
                         * generic spot when object is scavenged and is unnecessary to do it here.
                         */
#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
                        if (_extensions->shouldScavengeNotifyGlobalGCOfOldToOldReference() && !IS_CONCURRENT_ENABLED) {
                            oldToOldReferenceCreated(env, objectPtr);
                        ...
                        /* We are not removing it after all, since the object has Nursery references => reset the
                         * deferred flag. todo: consider doing double remembering, if remembered during CS cycle, to
                         * avoid the rescan of the object
                         */
                        *slotPtr = objectPtr;
                    }
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
                    ...
                    if (!IS_CONCURRENT_ENABLED && processRememberedThreadReference(env, objectPtr)) {
                        /* the object was tenured from the stack on a previous scavenge -- keep it around for a bit
                         * longer */
                        Trc_MM_ParallelScavenger_scavengeRememberedSet_keepingRememberedObject(
                            env->getLanguageVMThread(), objectPtr,
                            _extensions->objectModel.getRememberedBits(objectPtr));
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
void MM_Scavenger::scavengeRememberedSetListDirect(MM_EnvironmentStandard* env)
{
...
void MM_Scavenger::scavengeRememberedSetListIndirect(MM_EnvironmentStandard* env)
{
    Trc_MM_ParallelScavenger_scavengeRememberedSetList_Entry(env->getLanguageVMThread());
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
        /* Indirect refs are dealt within the root scanning phase (first STW phase), while the direct references are
           dealt within the main scan phase (typically concurrent). */
...
void MM_Scavenger::copyAndForwardThreadSlot(MM_EnvironmentStandard* env, omrobjectptr_t* objectPtrIndirect)
{
    ...
    if (NULL != objectPtr) {
        if (isObjectInEvacuateMemory(objectPtr)) {
            ...
            if (!IS_CONCURRENT_ENABLED && !isInNewSpace) {
                ...
                /* the object was tenured while it was referenced from the stack. Undo the forward, and process it in
                 * the rescan pass. */
            ...
            if (_extensions->isOld(objectPtr)) {
                if (_extensions->objectModel.atomicSwitchReferencedState(objectPtr,
                        OMR_TENURED_STACK_OBJECT_RECENTLY_REFERENCED, OMR_TENURED_STACK_OBJECT_CURRENTLY_REFERENCED)) {
                    Trc_MM_ParallelScavenger_copyAndForwardThreadSlot_renewingRememberedObject(
                        env->getLanguageVMThread(), objectPtr, OMR_TENURED_STACK_OBJECT_RECENTLY_REFERENCED);
                }
            }
        }
    }
...
void MM_Scavenger::rescanThreadSlot(MM_EnvironmentStandard* env, omrobjectptr_t* objectPtrIndirect)
{
    Assert_MM_false(IS_CONCURRENT_ENABLED);
    ...
    if (NULL != objectPtr) {
        if (isObjectInEvacuateMemory(objectPtr)) {
            ...
            *objectPtrIndirect = tenuredObjectPtr;
             ...
             * - if object has any Remembered state set already (it means object has been added to RS) just upgrade it
             * to OMR_TENURED_STACK_OBJECT_CURRENTLY_REFERENCED
...
MMINLINE void MM_Scavenger::reinitCache(MM_CopyScanCacheStandard* cache, void* base, void* top)
{
...
MMINLINE MM_CopyScanCacheStandard* MM_Scavenger::getFreeCache(MM_EnvironmentStandard* env)
{
    /* Check the free list */
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
    ...
    if (NULL == cache) {
        ...
        bool result
            = _scavengeCacheFreeList.resizeCacheEntries(env, 1 + _scavengeCacheFreeList.getAllocatedCacheCount(), 0);
...
MM_CopyScanCacheStandard* MM_Scavenger::createCacheInHeap(MM_EnvironmentStandard* env)
{
#if defined(OMR_SCAVENGER_TRACE)
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
#endif /* OMR_SCAVENGER_TRACE */
...
void MM_Scavenger::flushCache(MM_EnvironmentStandard* env, MM_CopyScanCacheStandard* cache)
{
...
bool MM_Scavenger::canLocalCacheBeReused(MM_EnvironmentStandard* env, MM_CopyScanCacheStandard* cache)
{
    ...
    if (NULL != cache) {
        ...
        if (cache->isCurrentlyBeingScanned()) {
            ...
            if (NULL != env->_deferredCopyCache) {
                ...
                /* We have two copy caches (deferred and current). Do they create contiguous memory with no objects
                 * (fully or partially) scanned in between? */
                if ((env->_deferredCopyCache->cacheAlloc == cache->scanCurrent) && !cache->_hasPartiallyScannedObject) {
                    /* append current copy cache to the deferred one. yet, decide whether to keep deferring it or push
                     * it for scanning */
                    ...
                    if (remainderCreated) {
                        ...
                        /* we use deferredCopyCache for merged one. this way we preserve partial scanned object info, if
                         * any exists */
                        env->_deferredCopyCache->cacheAlloc = cache->cacheAlloc;
                    ...
                    /* deferred and current copy caches are discontiguous. pushing the current one, if scan work
                     * available */
                ...
                /* No deferred cache exists. Decide what to do with current one (defer, push for scanning, or just
                 * ignore) */
                if (cache->isScanWorkAvailable()) {
...
bool MM_Scavenger::clearCache(MM_EnvironmentStandard* env, MM_CopyScanCacheStandard* cache)
{
...
void MM_Scavenger::abandonSurvivorTLHRemainder(MM_EnvironmentStandard* env)
{
    if (NULL != env->_survivorTLHRemainderBase) {
        ...
        env->_scavengerStats._flipDiscardBytes
            += (uintptr_t)env->_survivorTLHRemainderTop - (uintptr_t)env->_survivorTLHRemainderBase;
...
void MM_Scavenger::abandonTenureTLHRemainder(MM_EnvironmentStandard* env, bool preserveRemainders)
{
    if (NULL != env->_tenureTLHRemainderBase) {
        ...
        if (!preserveRemainders) {
            env->_scavengerStats._tenureDiscardBytes
                += (uintptr_t)env->_tenureTLHRemainderTop - (uintptr_t)env->_tenureTLHRemainderBase;
            env->_tenureTLHRemainderBase = NULL;
            env->_tenureTLHRemainderTop = NULL;
        }
        /* In case of Mutator threads (for concurrent scavenger) isMasterThread() is not sufficient, we must also make a
           thread check with getThreadType()*/
...
MMINLINE uintptr_t MM_Scavenger::scanCacheDistanceMetric(MM_CopyScanCacheStandard* scanCache, GC_SlotObject* scanSlot)
{
...
MMINLINE uintptr_t MM_Scavenger::copyCacheDistanceMetric(MM_CopyScanCacheStandard* copyCache)
{
...
MMINLINE MM_CopyScanCacheStandard* MM_Scavenger::aliasToCopyCache(MM_EnvironmentStandard* env,
    GC_SlotObject* scannedSlot, MM_CopyScanCacheStandard* scanCache, MM_CopyScanCacheStandard* copyCache)
{
    /* Only alias a copy cache IF there are <= _waitingCountAliasThreshold threads waiting (defaulted to 20% of active
     * GC threads, option XXgc:aliasInhibitingThresholdPercentage). If the current thread is the only producer and it
     * aliases a copy cache then it will be the only thread able to consume. This will alleviate the stalling issues.
     *
...
MMINLINE MM_CopyScanCacheStandard* MM_Scavenger::getNextScanCacheFromList(MM_EnvironmentStandard* env)
{
 ...
 */
void MM_Scavenger::saveMasterThreadTenureTLHRemainders(MM_EnvironmentStandard* env)
{
 ...
 */
void MM_Scavenger::restoreMasterThreadTenureTLHRemainders(MM_EnvironmentStandard* env)
{
...
MMINLINE bool MM_Scavenger::checkAndSetShouldYieldFlag(MM_EnvironmentStandard* env)
{
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    /* Don't rely on various conditions being same during one concurrent phase.
     * If one GC thread decided that we need to yield, we must yield, there is no way back. Hence, we store the result
     * in _shouldYield, and rely on it for the rest of concurrent phase. Master info if we should yield comes from
     * exclusive VM access request being broadcasted to this thread (isExclusiveAccessRequestWaiting()) But since that
     * request in the thread is not cleared even when implicit master GC thread enters STW phase, and since this yield
     * check is invoked in common code that can run both during STW and concurrent phase, we have to additionally check
     * we are indeed in concurrent phase before deciding to yield. Most of the time we could rely on being in
     * 'concurrent_state_scan' but it's more reliable to actually check if exclusive access is indeed being requested
     * (hence scavenger_shouldYield() call too).
     */
 ...
 */
void MM_Scavenger::setBackOutFlag(MM_EnvironmentBase* env, BackOutState backOutState)
{
...
bool MM_Scavenger::backOutFixSlotWithoutCompression(volatile omrobjectptr_t* slotPtr)
{
    omrobjectptr_t objectPtr = *slotPtr;
...
void MM_Scavenger::backOutObjectScan(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
    GC_SlotObject* slotObject = NULL;
    GC_ObjectScannerState objectScannerState;
...
void MM_Scavenger::backoutFixupAndReverseForwardPointersInSurvivor(MM_EnvironmentStandard* env)
{
            ...
            while ((objectPtr = evacuateHeapIterator.nextObjectNoAdvance()) != NULL) {
                ...
                if (header.isForwardedPointer()) {
                    ...
                    UDATA evacuateObjectSizeInBytes
                        = _extensions->objectModel.getConsumedSizeInBytesWithHeader(forwardedObject);
                    ...
                    omrtty_printf("{SCAV: Back out forward pointer %p[%p]@%p -> %p[%p]}\n", objectPtr, *objectPtr,
                        forwardedObject, freeHeader->getNext(env), freeHeader->getSize());
    ...
    if (compressed) {
        ...
        while (NULL != (rootRegion = evacuateRegionIterator1.nextRegion())) {
            if (isObjectInEvacuateMemory((omrobjectptr_t)rootRegion->getLowAddress())) {
                /*
                 * CMVC 179190:
                 * The call to "reverseForwardedObject", above, destroys our ability to detect if this object needs its
                 * destroyed slot fixed up (but the above loop must complete before we have the information with which
                 * to fixup the destroyed slot).  Fixing up a slot in dark matter could crash, though, since the slot
                 * could point to contracted memory or could point to corrupted data updated in a previous backout.  The
                 * simple work-around for this problem is to check if the slot points at a readable part of the heap
                 * (specifically, tenure or survivor - the only locations which would require us to fix up the slot) and
                 * only read and fixup the slot in those cases. This means that we could still corrupt the slot but we
                 * will never crash during fixup and nobody else should be trusting slots found in dead objects.
                 */
                GC_ObjectHeapIteratorAddressOrderedList evacuateHeapIterator(_extensions, rootRegion, false);
                omrobjectptr_t objectPtr = NULL;
                ...
                while ((objectPtr = evacuateHeapIterator.nextObjectNoAdvance()) != NULL) {
                    ...
                    omrtty_printf("{SCAV: Fixup destroyed slot %p@%p -> %u->%u}\n", objectPtr, fwdObjectPtr,
                        originalOverlap, header.getPreservedOverlap());
#endif /* OMR_SCAVENGER_TRACE_BACKOUT */
                }
            }
        }
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
bool MM_Scavenger::fixupSlotWithoutCompression(volatile omrobjectptr_t* slotPtr)
{
    omrobjectptr_t objectPtr = *slotPtr;
...
void MM_Scavenger::fixupObjectScan(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
    GC_SlotObject* slotObject = NULL;
    GC_ObjectScannerState objectScannerState;
    GC_ObjectScanner* objectScanner
                ...
                if (NULL == objectPtr) {
                    ...
                    if ((uintptr_t)objectPtr & DEFERRED_RS_REMOVE_FLAG) {
                        ...
                        if (!shouldRememberObject(env, objectPtr)) {
                            /* A simple mask out can be used - we are guaranteed to be the only manipulator of the
                             * object */
                            _extensions->objectModel.clearRemembered(objectPtr);
                            remSetSlotIterator.removeSlot();

                            /* No need to inform anybody about creation of old-to-old reference (see regular pruning
                             * pass). For CS, this is already handled during scanning of old objects
                             */
                        } else {
                            /* We are not removing it after all, since the object has Nursery references => reset the
                             * deferred flag. */
    ...
    if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
        ...
        if (isRememberedSetInOverflowState()) {
            ...
            if (IS_CONCURRENT_ENABLED) {
                        ...
                        while ((objectPtr = evacuateHeapIterator.nextObjectNoAdvance()) != NULL) {
                            ...
                            if (NULL != fwdObjectPtr) {
                                ...
                                evacuateHeapIterator.advance(
                                    _extensions->objectModel.getConsumedSizeInBytesWithHeaderBeforeMove(fwdObjectPtr));
#else
                                evacuateHeapIterator.advance(
                                    _extensions->objectModel.getConsumedSizeInBytesWithHeader(fwdObjectPtr));
                ...
                /*
                 * 2.c)Walk the evacuate space, fixing up objects and installing reverse forward pointers in survivor
                 * space
                 */
        ...
        if (scavengeCompletedSuccessfully(env)) {
            ...
            if (_extensions->scvTenureStrategyAdaptive) {
                ...
                if ((newSpaceConsumedSize < (_extensions->scvTenureRatioLow * newSpaceSizeScale))
                    ...
                    if ((newSpaceConsumedSize > (_extensions->scvTenureRatioHigh * newSpaceSizeScale))
...
void MM_Scavenger::processLargeAllocateStatsBeforeGC(MM_EnvironmentBase* env)
{
    ...
    /* TODO: remove the below 2 lines(resetLargeObjectAllocateStats), so that we do not loose direct mutator allocation
     * info */
...
void MM_Scavenger::reportGCCycleFinalIncrementEnding(MM_EnvironmentStandard* env)
{
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
 ...
 */
bool MM_Scavenger::heapAddRange(
 ...
 * @return true if operation completes with success
 */
bool MM_Scavenger::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size,
    void* lowAddress, void* highAddress, void* validLowAddress, void* validHighAddress)
{
    return true;
}
 ...
 */
void MM_Scavenger::collectorExpanded(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, uintptr_t expandSize)
{
 ...
 */
bool MM_Scavenger::canCollectorExpand(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, uintptr_t expandSize)
{
 ...
 */
void MM_Scavenger::internalPreCollect(
 ...
 */
void MM_Scavenger::internalPostCollect(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace)
{
    ...
    MM_EnvironmentBase* envBase, MM_MemorySubSpace* subSpace, MM_AllocateDescription* allocDescription)
{
    ...
    if (subSpaceSemiSpace->getMemorySubSpaceAllocate()->shouldAllocateAtSafePointOnly()) {
         ...
         * CONCURRENT_EXHAUSTED. Ideally, we should assert that the mode is CONCURRENT_EXHAUSTED, but Global GCs are not
         * visible from here.
         */
    ...
    if (failedTenureThresholdReached()) {
        Trc_MM_Scavenger_percolate_failedTenureThresholdReached(env->getLanguageVMThread(),
            getFailedTenureLargestObject(), _extensions->heap->getPercolateStats()->getScavengesSincePercolate());
        ...
        bool result
            = percolateGarbageCollect(env, subSpace, &percolateAllocDescription, FAILED_TENURE, aggressivePercolate);
     ...
     */
    if ((tenureMemorySubSpace->maxExpansionInSpace(env) + tenureMemorySubSpace->getApproximateActiveFreeMemorySize())
        < scavengerGCStats->_avgTenureBytes) {
        Trc_MM_Scavenger_percolate_insufficientTenureSpace(env->getLanguageVMThread(),
            tenureMemorySubSpace->maxExpansionInSpace(env), tenureMemorySubSpace->getApproximateActiveFreeMemorySize(),
            scavengerGCStats->_avgTenureBytes);
    ...
    if (_extensions->maxScavengeBeforeGlobal) {
        if (_countSinceForcingGlobalGC++ >= _extensions->maxScavengeBeforeGlobal) {
            ...
            /* Percolate the collect to parent MSS */
            bool result
                = percolateGarbageCollect(env, subSpace, NULL, MAX_SCAVENGES, J9MMCONSTANT_IMPLICIT_GC_PERCOLATE);
    ...
    if (!_extensions->concurrentMark) {
        uintptr_t previousUsedOldHeap
            = _extensions->oldHeapSizeOnLastGlobalGC - _extensions->freeOldHeapSizeOnLastGlobalGC;
        float maxTenureFreeRatio = _extensions->heapFreeMaximumRatioMultiplier / 100.0f;
        float midTenureFreeRatio
            = (_extensions->heapFreeMinimumRatioMultiplier + _extensions->heapFreeMaximumRatioMultiplier) / 200.0f;
        ...
        /* We suspect that next scavenge will cause Tenure expansion, while Tenure free ratio is (was) already high
         * enough */
        ...
        /* There was Tenure heap growth since last Global GC, and we suspect (assuming live set size did not grow) we
         * might be going beyond max free bound. */
 ...
 */
bool MM_Scavenger::percolateGarbageCollect(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace,
...
void MM_Scavenger::reportGCCycleStart(MM_EnvironmentStandard* env)
{
    ...
    TRIGGER_J9HOOK_MM_OMR_GC_CYCLE_START(_extensions->omrHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
        J9HOOK_MM_OMR_GC_CYCLE_START, _extensions->getHeap()->initializeCommonGCData(env, &commonData),
...
uintptr_t MM_Scavenger::calculateTenureMaskUsingLookback(double minimumSurvivalRate)
{
    ...
    for (uintptr_t index = 1; index < SCAVENGER_FLIP_HISTORY_SIZE; index++) {
        uintptr_t initialGenerationSize
            = stats->getFlipHistory(index)->_flipBytes[1] + stats->getFlipHistory(index)->_tenureBytes[1];
        ...
        if (requiredLookback >= age) {
            ...
            for (uintptr_t lookback = 1; (lookback <= requiredLookback) && shouldTenureThisAge; lookback++) {
                ...
                uintptr_t currentTotalBytes
                    = currentFlipBytes + stats->getFlipHistory(lookback)->_tenureBytes[currentAgeIndex];
...
uintptr_t MM_Scavenger::calculateTenureMaskUsingHistory(double minimumSurvivalRate)
{
    Assert_MM_true(0.0 <= minimumSurvivalRate);
    Assert_MM_true(1.0 >= minimumSurvivalRate);
...
void MM_Scavenger::threadReleaseCaches(MM_EnvironmentBase* envBase, bool final)
{
    MM_EnvironmentStandard* env = MM_EnvironmentStandard::getEnvironment(envBase);
    ...
    if (isConcurrentInProgress()) {
        ...
        if (final) {
            /* If it's an intermediate release (for example mutator threads releasing VM access in a middle of
             * Concurrent Scavenger cycle, keep copy cache remainders around (do not abandon yet), to be reused if the
             * threads re-acquires VM access during the same CS cycle.
             */
            abandonSurvivorTLHRemainder(env);
            abandonTenureTLHRemainder(env, true);
        }
...
{
    /* This is where the most of scan work should occur in CS. Typically as a concurrent task (background threads), but
     * in some corner cases it could be scheduled as a STW task */
     ...
     * from final STW phase pick them up. Most of the time, STW phase will have a superset of GC threads, so they could
     * just resume the work on their own caches, but this is not 100% guarantied (the control of what threads are
     * inolved is in Dispatcher's domain).
     */
    threadReleaseCaches(env, true);
...
void MM_Scavenger::workThreadComplete(MM_EnvironmentStandard* env)
{
     ...
     * still point to forwarded objects. This is important particularly for Tenure space where recovery procedure will
     * not walk the Tenure space for exhaustive fixup.
     */
    completeScan(env);
...
uintptr_t MM_Scavenger::masterThreadConcurrentCollect(MM_EnvironmentBase* env)
{
    if (concurrent_state_scan == _concurrentState) {
        ...
        /* return the number of bytes scanned since the caller needs to pass it into postConcurrentUpdateStatsAndReport
         * for stats reporting */
        return scavengeTask.getBytesScanned();
    } else {
        /* someone else might have done this phase (and the rest of the cycle), forced in STW, before we even got a
         * chance to run. */
...
void MM_Scavenger::switchConcurrentForThread(MM_EnvironmentBase* env)
{
     ...
     * If state is idle, we just completed a cycle. If state is scan (or rarely complete), we just started a cycle (and
     * possibly even complete concurrent work)
     */
    ...
    if (env->_concurrentScavengerSwitchCount != _concurrentScavengerSwitchCount) {
        Trc_MM_Scavenger_switchConcurrent(env->getLanguageVMThread(), _concurrentState, _concurrentScavengerSwitchCount,
