...
void
MM_Scavenger::hookGlobalCollectionStart(J9HookInterface** hook,
                                        uintptr_t eventNum,
                                        void* eventData,
                                        void* userData)
{
...
void
MM_Scavenger::hookGlobalCollectionComplete(J9HookInterface** hook,
                                           uintptr_t eventNum,
                                           void* eventData,
                                           void* userData)
{
 ...
 */
void*
MM_Scavenger::createSweepPoolState(MM_EnvironmentBase* env,
 ...
 */
void
MM_Scavenger::deleteSweepPoolState(MM_EnvironmentBase* env,
                                   void* sweepPoolState)
{
  Assert_MM_unreachable();
}
                          ...
                          MM_HeapRegionManager* regionManager)
{
  ...
  scavenger =
    (MM_Scavenger*)env->getForge()->allocate(sizeof(MM_Scavenger),
...
MM_Scavenger::initialize(MM_EnvironmentBase* env)
{
  ...
  (*mmOmrHooks)
    ->J9HookRegisterWithCallSite(mmOmrHooks,
  ...
  if (omrthread_monitor_init_with_name(
        &_scanCacheMonitor, 0, "MM_Scavenger::scanCacheMonitor")) {
    return false;
  }
  ...
  if (omrthread_monitor_init_with_name(
        &_freeCacheMonitor, 0, "MM_Scavenger::freeCacheMonitor")) {
    return false;
  }
  ...
  if (!_scavengeCacheFreeList.resizeCacheEntries(
        env, totalActiveCacheCount, incrementCacheCount)) {
    return false;
  }
...
MM_Scavenger::masterSetupForGC(MM_EnvironmentStandard* env)
{
  ...
  _activeSubSpace->cacheRanges(
    _evacuateMemorySubSpace, &_evacuateSpaceBase, &_evacuateSpaceTop);
  _activeSubSpace->cacheRanges(
    _survivorMemorySubSpace, &_survivorSpaceBase, &_survivorSpaceTop);
...
MM_Scavenger::scavenge(MM_EnvironmentBase* envBase)
{
  ...
  MM_ParallelScavengeTask scavengeTask(
    env, _dispatcher, this, env->_cycleState);
...
MM_Scavenger::reportScavengeStart(MM_EnvironmentStandard* env)
{
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_SCAVENGE_START(_extensions->privateHookInterface,
                                           env->getOmrVMThread(),
                                           omrtime_hires_clock(),
...
MM_Scavenger::reportScavengeEnd(MM_EnvironmentStandard* env, bool lastIncrement)
{
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_SCAVENGE_END(_extensions->privateHookInterface,
                                         env->getOmrVMThread(),
                                         omrtime_hires_clock(),
...
MM_Scavenger::reportGCStart(MM_EnvironmentStandard* env)
{
  ...
  TRIGGER_J9HOOK_MM_OMR_LOCAL_GC_START(_extensions->omrHookInterface,
                                       env->getOmrVMThread(),
                                       omrtime_hires_clock(),
...
void
MM_Scavenger::clearIncrementGCStats(MM_EnvironmentBase* env,
                                    bool firstIncrement)
{
...
void
MM_Scavenger::mergeGCStatsBase(MM_EnvironmentBase* env,
...
MM_Scavenger::mergeThreadGCStats(MM_EnvironmentBase* env)
{
  ...
  Trc_MM_ParallelScavenger_parallelStats(
    ...
    (uint32_t)omrtime_hires_delta(
      0, scavStats->_workStallTime, OMRPORT_TIME_DELTA_IN_MILLISECONDS),
    (uint32_t)omrtime_hires_delta(
      0, scavStats->_completeStallTime, OMRPORT_TIME_DELTA_IN_MILLISECONDS),
    (uint32_t)omrtime_hires_delta(
      0, scavStats->_syncStallTime, OMRPORT_TIME_DELTA_IN_MILLISECONDS),
  ...
  if (canCalcGCStats(env)) {
    ...
    if (scavengerGCStats->_gcCount > 1) {
      scavengerGCStats->_avgInitialFree = (uintptr_t)MM_Math::weightedAverage(
        (float)scavengerGCStats->_avgInitialFree,
        (float)initialFree,
      ...
      scavengerGCStats->_avgTenureBytes = (uintptr_t)MM_Math::weightedAverage(
        (float)scavengerGCStats->_avgTenureBytes,
...
uintptr_t
MM_Scavenger::calculateCopyScanCacheSizeForQueueLength(uintptr_t maxCacheSize,
                                                       uintptr_t threadCount,
...
MM_Scavenger::calculateOptimumCopyScanCacheSize(MM_EnvironmentStandard* env)
{
  ...
  if (waitingThreads > 0) {
    uintptr_t cacheSizeBasedOnWaitingCount =
      calculateCopyScanCacheSizeForWaitingThreads(
        maxCacheSize, threadCount, waitingThreads);
  ...
  if (env->approxScanCacheCount < threadCount) {
    uintptr_t cacheSizeBasedOnScanCacheCount =
      calculateCopyScanCacheSizeForQueueLength(
        maxCacheSize, threadCount, env->approxScanCacheCount);
  ...
  if ((NULL != env->_tenureCopyScanCache) &&
    ...
    if (cacheSize < _minTenureFailureSize) {
      ...
      if (((uintptr_t)env->_tenureTLHRemainderTop -
        ...
        addrBase = _tenureMemorySubSpace->collectorAllocate(
          env, this, &allocDescription);
        if (NULL != addrBase) {
          addrTop = (void*)(((uint8_t*)addrBase) + cacheSize);
          /* Check that there is no overflow */
    ...
    if (allocateResult) {
      ...
      if (NULL != copyCache) {
        ...
        copyCache->flags &= OMR_SCAVENGER_CACHE_TYPE_HEAP;
        copyCache->flags |=
          OMR_SCAVENGER_CACHE_TYPE_TENURESPACE | OMR_SCAVENGER_CACHE_TYPE_COPY;
...
bool
MM_Scavenger::copyObjectSlot(MM_EnvironmentStandard* env,
...
omrobjectptr_t
MM_Scavenger::copyObject(MM_EnvironmentStandard* env,
...
omrobjectptr_t
MM_Scavenger::copy(MM_EnvironmentStandard* env,
                   MM_ForwardedHeader* forwardedHeader)
{
  ...
  if (0 == (((uintptr_t)1 << objectAge) & _tenureMask)) {
    ...
    if (NULL != copyCache) {
      ...
      Assert_GC_true_with_message4(
        env,
        objectCopySizeInBytes <= spaceAvailableForObject,
        "Corruption in Evacuate at %p: calculated object size %zu larger then "
        "available %zu, Forwarded Header at %p\n",
        forwardedHeader->getObject(),
        objectCopySizeInBytes,
        spaceAvailableForObject,
        forwardedHeader);
  ...
  if (IS_CONCURRENT_ENABLED) {
    ...
    if (objectCopySizeInBytes <= 1024) {
      ...
      initialSizeToCopy =
        forwardedHeader->copySetup(destinationObjectPtr, &remainingSizeToCopy);
  ...
  if (originalDestinationObjectPtr == destinationObjectPtr) {
    ...
    if (NULL != hotFieldPadBase) {
      ...
      MM_HeapLinkedFreeHeader::fillWithHoles(
        hotFieldPadBase, hotFieldPadSize, compressed);
    }
#endif /* J9VM_INTERP_NATIVE_SUPPORT */

#if defined(OMR_VALGRIND_MEMCHECK)
    valgrindMempoolAlloc(
      _extensions, (uintptr_t)destinationObjectPtr, objectReserveSizeInBytes);
#endif /* defined(OMR_VALGRIND_MEMCHECK) */

#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    if (!allowDuplicateOrConcurrentDisabled) {
      /* Copy a non-aligned section */
      forwardedHeader->copySection(
        destinationObjectPtr, remainingSizeToCopy, initialSizeToCopy);
...
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
    {
      memcpy((void*)destinationObjectPtr,
             forwardedHeader->getObject(),
             objectCopySizeInBytes);
...
GC_ObjectScanner*
MM_Scavenger::getObjectScanner(MM_EnvironmentStandard* env,
                               omrobjectptr_t objectptr,
                               void* objectScannerState,
                               uintptr_t flags)
{
  return _delegate.getObjectScanner(
    env, objectptr, (void*)objectScannerState, flags);
}

uintptr_t
MM_Scavenger::getArraySplitAmount(MM_EnvironmentStandard* env,
...
bool
MM_Scavenger::splitIndexableObjectScanner(MM_EnvironmentStandard* env,
  ...
  if (backOutStarted != _extensions->getScavengerBackOutState()) {
    ...
    if (endIndex < maxIndex) {
      ...
      if (NULL != splitCache) {
        ...
        indexableScanner->splitTo(
          env, splitCache->getObjectScanner(), scvArraySplitAmount);
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
 ...
 */
MMINLINE void
MM_Scavenger::updateCopyScanCounts(MM_EnvironmentBase* env,
                                   ...
                                   uint64_t slotsCopied)
{
  ...
  uint64_t updateResult =
    _extensions->copyScanRatio.update(env,
                                      &(env->_scavengerStats._slotsScanned),
                                      &(env->_scavengerStats._slotsCopied),
                                      _waitingCount);
  if (0 != updateResult) {
                                  ...
                                  omrobjectptr_t* rememberedSetSlot)
{
  ...
  if (objectScanner->isIndexableObject()) {
    ...
    if (!splitIndexableObjectScanner(
          env, objectScanner, splitIndex, rememberedSetSlot)) {
...
void
MM_Scavenger::deepScanOutline(MM_EnvironmentStandard* env,
                              omrobjectptr_t objectPtr,
  ...
  MM_CopyScanCacheStandard* scanCache)
{
  ...
  if (!scanCache->_hasPartiallyScannedObject) {
    if (!scanCache->isSplitArray()) {
      /* try to get a new scanner instance from the cli */
      objectScanner = getObjectScanner(env,
                                       objectPtr,
                                       scanCache->getObjectScanner(),
    ...
    if (objectScanner->isIndexableObject()) {
       ...
       * cache */
      if (!splitIndexableObjectScanner(env,
                                       objectScanner,
  ...
  if (_extensions->_forceRandomBackoutsAfterScan) {
    if (0 == (rand() % _extensions->_forceRandomBackoutsAfterScanPeriod)) {
      omrtty_printf(
        ...
        env->getWorkUnitIndex(),
        env->_lastSyncPointReached);
      setBackOutFlag(env, backOutFlagRaised);
      omrthread_monitor_enter(_scanCacheMonitor);
      if (0 != _waitingCount) {
        omrthread_monitor_notify_all(_scanCacheMonitor);
...
void
MM_Scavenger::addAllRememberedObjectsToOverflow(MM_EnvironmentStandard* env,
                                                MM_RSOverflow* overflow)
{
  ...
  while ((region = regionIterator.nextRegion()) != NULL) {
    GC_ObjectHeapIteratorAddressOrderedList objectIterator(
      _extensions, region, false);
    omrobjectptr_t objectPtr;
...
void
MM_Scavenger::addToRememberedSetFragment(MM_EnvironmentStandard* env,
                                         omrobjectptr_t objectPtr)
{
...
void
MM_Scavenger::rememberObject(MM_EnvironmentStandard* env,
                             omrobjectptr_t objectPtr)
{
...
bool
MM_Scavenger::isRememberedThreadReference(MM_EnvironmentStandard* env,
                                          omrobjectptr_t objectPtr)
{
  Assert_MM_false(IS_CONCURRENT_ENABLED);
  Assert_MM_true(NULL != objectPtr);
...
bool
MM_Scavenger::processRememberedThreadReference(MM_EnvironmentStandard* env,
                                               omrobjectptr_t objectPtr)
{
  Assert_MM_false(IS_CONCURRENT_ENABLED);
  Assert_MM_true(NULL != objectPtr);
 ...
 ********************************************************************/
bool
MM_Scavenger::shouldRememberObject(MM_EnvironmentStandard* env,
                                   omrobjectptr_t objectPtr)
{
...
void
MM_Scavenger::copyAndForwardThreadSlot(MM_EnvironmentStandard* env,
...
void
MM_Scavenger::rescanThreadSlot(MM_EnvironmentStandard* env,
                               omrobjectptr_t* objectPtrIndirect)
{
  Assert_MM_false(IS_CONCURRENT_ENABLED);
...
MMINLINE void
MM_Scavenger::reinitCache(MM_CopyScanCacheStandard* cache,
                          void* base,
                          void* top)
{
...
void
MM_Scavenger::flushCache(MM_EnvironmentStandard* env,
...
bool
MM_Scavenger::canLocalCacheBeReused(MM_EnvironmentStandard* env,
                                    MM_CopyScanCacheStandard* cache)
{
    ...
    if (cache->isCurrentlyBeingScanned()) {
      ...
      if (NULL != env->_deferredCopyCache) {
        ...
        if ((env->_deferredCopyCache->cacheAlloc == cache->scanCurrent) &&
          ...
          Assert_MM_true(
            (cache->flags & ~OMR_SCAVENGER_CACHE_TYPE_HEAP) ==
...
bool
MM_Scavenger::clearCache(MM_EnvironmentStandard* env,
                         MM_CopyScanCacheStandard* cache)
{
   ...
   * allocated */
  TRIGGER_J9HOOK_MM_PRIVATE_CACHE_CLEARED(_extensions->privateHookInterface,
                                          env->getOmrVMThread(),
                                          allocSubSpace,
                                          cache->cacheBase,
                                          cache->cacheAlloc,
...
MMINLINE void
MM_Scavenger::addCacheEntryToScanListAndNotify(
  MM_EnvironmentStandard* env,
...
MMINLINE MM_CopyScanCacheStandard*
MM_Scavenger::aliasToCopyCache(MM_EnvironmentStandard* env,
 ...
 */
void
MM_Scavenger::restoreMasterThreadTenureTLHRemainders(
  MM_EnvironmentStandard* env)
{
...
void
MM_Scavenger::backOutObjectScan(MM_EnvironmentStandard* env,
                                omrobjectptr_t objectPtr)
{
  GC_SlotObject* slotObject = NULL;
  GC_ObjectScannerState objectScannerState;
  ...
  if (NULL != objectScanner) {
    ...
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
    omrtty_printf(
      "{SCAV: Back out slots in object %p[%p]\n", objectPtr, *objectPtr);
#endif /* OMR_SCAVENGER_TRACE_BACKOUT */
    while (NULL != (slotObject = objectScanner->getNextSlot())) {
...
void
MM_Scavenger::backoutFixupAndReverseForwardPointersInSurvivor(
  MM_EnvironmentStandard* env)
{
  ...
  while (NULL != (rootRegion = evacuateRegionIterator.nextRegion())) {
    ...
    if (isObjectInEvacuateMemory((omrobjectptr_t)rootRegion->getLowAddress())) {
      ...
      omrtty_printf("{SCAV: Back out forward pointers in region [%p:%p]}\n",
                    rootRegion->getLowAddress(),
                    rootRegion->getHighAddress());
#endif /* OMR_SCAVENGER_TRACE_BACKOUT */
...
#if defined(OMR_VALGRIND_MEMCHECK)
          valgrindMempoolAlloc(_extensions,
                               (uintptr_t)originalObject,
...
void
MM_Scavenger::fixupObjectScan(MM_EnvironmentStandard* env,
                              omrobjectptr_t objectPtr)
{
  GC_SlotObject* slotObject = NULL;
  GC_ObjectScannerState objectScannerState;
              ...
              if (NULL != fwdObjectPtr) {
                ...
                evacuateHeapIterator.advance(
                  _extensions->objectModel
                    .getConsumedSizeInBytesWithHeaderBeforeMove(fwdObjectPtr));
#else
                evacuateHeapIterator.advance(
                  _extensions->objectModel.getConsumedSizeInBytesWithHeader(
                    fwdObjectPtr));
  ...
  bool rebuildMarkBits)
{
  ...
  if (_extensions->trackMutatorThreadCategory) {
    ...
    omrthread_set_category(
      env->getOmrVMThread()->_os_thread, 0, J9THREAD_TYPE_SET_GC);
  }
 ...
 */
bool
MM_Scavenger::heapAddRange(MM_EnvironmentBase* env,
 ...
 */
bool
MM_Scavenger::heapRemoveRange(MM_EnvironmentBase* env,
                              MM_MemorySubSpace* subspace,
                              uintptr_t size,
                              void* lowAddress,
                              void* highAddress,
 ...
 */
void
MM_Scavenger::collectorExpanded(MM_EnvironmentBase* env,
                                MM_MemorySubSpace* subSpace,
 ...
 */
bool
MM_Scavenger::canCollectorExpand(MM_EnvironmentBase* env,
                                 MM_MemorySubSpace* subSpace,
                                 uintptr_t expandSize)
{
 ...
 */
void
MM_Scavenger::internalPreCollect(MM_EnvironmentBase* env,
                                 MM_MemorySubSpace* subSpace,
                                 MM_AllocateDescription* allocDescription,
                                 uint32_t gcCode)
{
 ...
 */
void
MM_Scavenger::internalPostCollect(MM_EnvironmentBase* env,
                                  MM_MemorySubSpace* subSpace)
{
 ...
 */
bool
MM_Scavenger::internalGarbageCollect(MM_EnvironmentBase* envBase,
                                     MM_MemorySubSpace* subSpace,
                                     MM_AllocateDescription* allocDescription)
{
  ...
  if (subSpaceSemiSpace->getMemorySubSpaceAllocate()
    ...
    bool result = percolateGarbageCollect(env,
                                          subSpace,
                                          NULL,
  ...
  if (expandFailed()) {
    ...
    bool result = percolateGarbageCollect(
      env, subSpace, NULL, EXPAND_FAILED, aggressivePercolate);
  ...
  if (isRememberedSetInOverflowState() && _extensions->scavengerRsoScanUnsafe) {
    ...
    /* Percolate the collect to parent MSS */
    percolateGarbageCollect(
      env, subSpace, NULL, RS_OVERFLOW, J9MMCONSTANT_IMPLICIT_GC_PERCOLATE);
  }
 ...
 */
bool
MM_Scavenger::percolateGarbageCollect(MM_EnvironmentBase* env,
                                      MM_MemorySubSpace* subSpace,
                                      MM_AllocateDescription* allocDescription,
...
MM_Scavenger::reportGCIncrementEnd(MM_EnvironmentStandard* env)
{
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_GC_INCREMENT_END(_extensions->privateHookInterface,
                                             env->getOmrVMThread(),
                                             stats->_endTime,
...
MM_Scavenger::calculateTenureMaskUsingLookback(double minimumSurvivalRate)
{
   ...
   */
  uintptr_t normalizedInitialGenerationSize = (uintptr_t)OMR_MAX(
    0.0,
    averageInitialGenerationSize - standardDeviationOfInitialGenerationSize);
  ...
  for (uintptr_t age = 0; age <= OBJECT_HEADER_AGE_MAX + 1; ++age) {
    ...
    if (requiredLookback >= age) {
      ...
      for (uintptr_t lookback = 1;
           (lookback <= requiredLookback) && shouldTenureThisAge;
           lookback++) {
...
MM_Scavenger::scavengeRoots(MM_EnvironmentBase* env)
{
  ...
  MM_ConcurrentScavengeTask scavengeTask(
    ...
    _dispatcher,
    this,
    MM_ConcurrentScavengeTask::SCAVENGE_ROOTS,
    env->_cycleState);
  _dispatcher->run(env, &scavengeTask);
...
MM_Scavenger::scavengeScan(MM_EnvironmentBase* envBase)
{
  ...
  MM_ConcurrentScavengeTask scavengeTask(
    ...
    _dispatcher,
    this,
    MM_ConcurrentScavengeTask::SCAVENGE_SCAN,
    env->_cycleState);
  _dispatcher->run(env, &scavengeTask);
...
MM_Scavenger::scavengeComplete(MM_EnvironmentBase* envBase)
{
  ...
  MM_ConcurrentScavengeTask scavengeTask(
    ...
    _dispatcher,
    this,
    MM_ConcurrentScavengeTask::SCAVENGE_COMPLETE,
    env->_cycleState);
  _dispatcher->run(env, &scavengeTask);
...
MM_Scavenger::masterThreadConcurrentCollect(MM_EnvironmentBase* env)
{
  if (concurrent_state_scan == _concurrentState) {
     ...
     * number of threads. */
    _dispatcher->run(
      env, &scavengeTask, _extensions->concurrentScavengerBackgroundThreads);
...
void
MM_Scavenger::preConcurrentInitializeStatsAndReport(
  MM_EnvironmentBase* env,
...
MM_Scavenger::switchConcurrentForThread(MM_EnvironmentBase* env)
{
  ...
  if (env->_concurrentScavengerSwitchCount != _concurrentScavengerSwitchCount) {
    Trc_MM_Scavenger_switchConcurrent(env->getLanguageVMThread(),
                                      _concurrentState,
                                      _concurrentScavengerSwitchCount,
                                      env->_concurrentScavengerSwitchCount);
...
void
MM_Scavenger::triggerConcurrentScavengerTransition(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocDescription)
{
