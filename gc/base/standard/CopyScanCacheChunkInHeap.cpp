...
MM_CopyScanCacheChunkInHeap *MM_CopyScanCacheChunkInHeap::newInstance(
    MM_EnvironmentStandard *env, MM_CopyScanCacheChunk *nextChunk,
  ...
  uintptr_t sizeToAllocate =
      sizeof(MM_HeapLinkedFreeHeader) + sizeof(MM_CopyScanCacheChunkInHeap);
  ...
  if (sizeToAllocate < extensions->tlhMinimumSize) {
    ...
    numberOfCaches = ((extensions->tlhMinimumSize - sizeToAllocate) /
                      sizeof(MM_CopyScanCacheStandard)) +
                     1;
  }

  /* total size required to allocate */
  sizeToAllocate += numberOfCaches * sizeof(MM_CopyScanCacheStandard);
  /* this is going to be allocated on the heap so ensure that the chunk we are
   * allocating is adjusted for heap alignment (since object consumed sizes
   * already have this requirement) */
  sizeToAllocate =
      MM_Math::roundToCeiling(env->getObjectAlignmentInBytes(), sizeToAllocate);
  ...
  if (NULL != addrBase) {
    ...
    chunk = (MM_CopyScanCacheChunkInHeap *)((uintptr_t)addrBase +
    ...
    if (chunk->initialize(env, numberOfCaches, nextChunk,
                          OMR_SCAVENGER_CACHE_TYPE_HEAP, sublistTail)) {
...
void MM_CopyScanCacheChunkInHeap::kill(MM_EnvironmentBase *env) {
  tearDown(env);
