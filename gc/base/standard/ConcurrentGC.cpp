 ...
 */
void J9ConcurrentWriteBarrierStore(
    OMR_VMThread* vmThread, omrobjectptr_t destinationObject, omrobjectptr_t storedObject)
{
...
int J9THREAD_PROC con_helper_thread_proc(void* info)
{
    ...
    omrsig_protect(con_helper_thread_proc2, info, signalHandler, signalHandlerArg,
        OMRPORT_SIG_FLAG_SIGALLSYNC | OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION, &rc);
...
uintptr_t con_helper_thread_proc2(OMRPortLibrary* portLib, void* info)
{
    ...
    OMR_VMThread* omrThread = MM_EnvironmentBase::attachVMThread(
        omrVM, "Concurrent Mark Helper", MM_EnvironmentBase::ATTACH_GC_HELPER_THREAD);
...
void MM_ConcurrentGC::reportConcurrentKickoff(MM_EnvironmentBase* env)
{
    ...
    Trc_MM_ConcurrentKickoff(env->getLanguageVMThread(), _stats.getTraceSizeTarget(), _stats.getKickoffThreshold(),
        _stats.getRemainingFree());

    MM_CommonGCData commonData;

    TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_KICKOFF(_extensions->privateHookInterface, env->getOmrVMThread(),
        omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_KICKOFF,
        _extensions->getHeap()->initializeCommonGCData(env, &commonData), _stats.getTraceSizeTarget(),
        _stats.getKickoffThreshold(), _stats.getRemainingFree(), _stats.getKickoffReason(), _languageKickoffReason);
}

void MM_ConcurrentGC::reportConcurrentAborted(MM_EnvironmentBase* env, CollectionAbortReason reason)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

    Trc_MM_ConcurrentAborted(env->getLanguageVMThread());

    TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_ABORTED(_extensions->privateHookInterface, env->getOmrVMThread(),
...
void MM_ConcurrentGC::reportConcurrentHalted(MM_EnvironmentBase* env)
{
    ...
    Trc_MM_ConcurrentHalted(env->getLanguageVMThread(), (uintptr_t)_stats.getExecutionModeAtGC(),
        _stats.getTraceSizeTarget(), _stats.getTotalTraced(), _stats.getMutatorsTraced(), _stats.getConHelperTraced(),
        cardTable->getCardTableStats()->getConcurrentCleanedCards(), _stats.getCardCleaningThreshold(),
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_HALTED(_extensions->privateHookInterface, env->getOmrVMThread(),
        omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_HALTED, (uintptr_t)_stats.getExecutionModeAtGC(),
        _stats.getTraceSizeTarget(), _stats.getTotalTraced(), _stats.getMutatorsTraced(), _stats.getConHelperTraced(),
        cardTable->getCardTableStats()->getConcurrentCleanedCards(), _stats.getCardCleaningThreshold(),
        _stats.getConcurrentWorkStackOverflowOcurred(), _stats.getConcurrentWorkStackOverflowCount(),
        (uintptr_t)cardTable->isCardCleaningComplete(), _concurrentDelegate.reportConcurrentScanningMode(env),
        (uintptr_t)_markingScheme->getWorkPackets()->tracingExhausted());
}

void MM_ConcurrentGC::reportConcurrentFinalCardCleaningStart(MM_EnvironmentBase* env)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

    Trc_MM_ConcurrentCollectionCardCleaningStart(
        env->getLanguageVMThread(), _stats.getConcurrentWorkStackOverflowCount());
    TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_CARD_CLEANING_START(_extensions->privateHookInterface,
        env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_CARD_CLEANING_START,
        _stats.getConcurrentWorkStackOverflowCount());
}

void MM_ConcurrentGC::reportConcurrentFinalCardCleaningEnd(MM_EnvironmentBase* env, uint64_t duration)
{
    MM_ConcurrentCardTable* cardTable = (MM_ConcurrentCardTable*)_cardTable;
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

    Trc_MM_ConcurrentCollectionCardCleaningEnd(env->getLanguageVMThread());
    TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_CARD_CLEANING_END(_extensions->privateHookInterface,
        env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_CARD_CLEANING_END,
        duration, cardTable->getCardTableStats()->getFinalCleanedCardsPhase1(),
        ...
        cardTable->getCardTableStats()->getCardCleaningPhase3Kickoff(), _stats.getConcurrentWorkStackOverflowCount());
}
...
void MM_ConcurrentGC::reportConcurrentCollectionStart(MM_EnvironmentBase* env)
{
    ...
    if (J9_EVENT_IS_HOOKED(_extensions->privateHookInterface, J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START)) {
        ...
        ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START(_extensions->privateHookInterface,
            env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START, &commonData,
            _stats.getTraceSizeTarget(), _stats.getTotalTraced(), _stats.getMutatorsTraced(),
            _stats.getConHelperTraced(), cardTable->getCardTableStats()->getConcurrentCleanedCards(),
            _stats.getCardCleaningThreshold(), _stats.getConcurrentWorkStackOverflowOcurred(),
            _stats.getConcurrentWorkStackOverflowCount(), _stats.getThreadsToScanCount(),
            _stats.getThreadsScannedCount(), _stats.getCardCleaningReason());
    }
}

void MM_ConcurrentGC::reportConcurrentCollectionEnd(MM_EnvironmentBase* env, uint64_t duration)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    if (J9_EVENT_IS_HOOKED(_extensions->privateHookInterface, J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END)) {
        ...
        ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END(_extensions->privateHookInterface,
            env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END, duration,
            env->getExclusiveAccessTime(), &commonData);
    }
}

void MM_ConcurrentGC::reportConcurrentBackgroundThreadActivated(MM_EnvironmentBase* env)
{
...
void MM_ConcurrentGC::reportConcurrentBackgroundThreadFinished(MM_EnvironmentBase* env, uintptr_t traceTotal)
{
...
void MM_ConcurrentGC::reportConcurrentCompleteTracingStart(MM_EnvironmentBase* env)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

    Trc_MM_ConcurrentCompleteTracingStart(env->getLanguageVMThread(), _stats.getConcurrentWorkStackOverflowCount());

    TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COMPLETE_TRACING_START(_extensions->privateHookInterface,
        env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_COMPLETE_TRACING_START,
        _stats.getConcurrentWorkStackOverflowCount());
}

void MM_ConcurrentGC::reportConcurrentCompleteTracingEnd(MM_EnvironmentBase* env, uint64_t duration)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

    Trc_MM_ConcurrentCompleteTracingEnd(
        env->getLanguageVMThread(), _stats.getCompleteTracingCount(), _stats.getConcurrentWorkStackOverflowCount());

    TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COMPLETE_TRACING_END(_extensions->privateHookInterface, env->getOmrVMThread(),
        omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_COMPLETE_TRACING_END, duration,
        _stats.getCompleteTracingCount(), _stats.getConcurrentWorkStackOverflowCount());
}

void MM_ConcurrentGC::reportConcurrentRememberedSetScanStart(MM_EnvironmentBase* env)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

    Trc_MM_ConcurrentRememberedSetScanStart(env->getLanguageVMThread(), _stats.getConcurrentWorkStackOverflowCount());
    TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_START(_extensions->privateHookInterface,
        env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_START,
        _stats.getConcurrentWorkStackOverflowCount());
}

void MM_ConcurrentGC::reportConcurrentRememberedSetScanEnd(MM_EnvironmentBase* env, uint64_t duration)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

    Trc_MM_ConcurrentRememberedSetScanEnd(env->getLanguageVMThread(), _stats.getRSObjectsFound(),
        _stats.getRSScanTraceCount(), _stats.getConcurrentWorkStackOverflowCount());
    TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_END(_extensions->privateHookInterface,
        env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_END, duration,
        _stats.getRSObjectsFound(), _stats.getRSScanTraceCount(), _stats.getConcurrentWorkStackOverflowCount());
}
 ...
 *
 */
void MM_ConcurrentGC::signalThreadsToActivateWriteBarrierAsyncEventHandler(OMR_VMThread* omrVMThread, void* userData)
{
...
MM_ConcurrentGC* MM_ConcurrentGC::newInstance(MM_EnvironmentBase* env)
{
    MM_ConcurrentGC* concurrentGC = (MM_ConcurrentGC*)env->getForge()->allocate(
        sizeof(MM_ConcurrentGC), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
bool MM_ConcurrentGC::initialize(MM_EnvironmentBase* env)
{
    ...
    if (_conHelperThreads > 0) {
        ...
        _conHelpersTable = (omrthread_t*)env->getForge()->allocate(
            _conHelperThreads * sizeof(omrthread_t), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
        if (!_conHelpersTable) {
            goto error_no_memory;
        }
    ...
    _cardCleaningFactorPass1 = interpolateInRange(INITIAL_CARD_CLEANING_FACTOR_PASS1_1,
        INITIAL_CARD_CLEANING_FACTOR_PASS1_8, INITIAL_CARD_CLEANING_FACTOR_PASS1_10, _allocToTraceRateNormal);
    _maxCardCleaningFactorPass1 = interpolateInRange(MAX_CARD_CLEANING_FACTOR_PASS1_1, MAX_CARD_CLEANING_FACTOR_PASS1_8,
    ...
    if (_secondCardCleanPass) {
        _cardCleaningFactorPass2 = interpolateInRange(INITIAL_CARD_CLEANING_FACTOR_PASS2_1,
            INITIAL_CARD_CLEANING_FACTOR_PASS2_8, INITIAL_CARD_CLEANING_FACTOR_PASS2_10, _allocToTraceRateNormal);

        _maxCardCleaningFactorPass2 = interpolateInRange(MAX_CARD_CLEANING_FACTOR_PASS2_1,
            MAX_CARD_CLEANING_FACTOR_PASS2_8, MAX_CARD_CLEANING_FACTOR_PASS2_10, _allocToTraceRateNormal);
    } else {
    ...
    _cardCleaningThresholdFactor = interpolateInRange(CARD_CLEANING_THRESHOLD_FACTOR_1,
        CARD_CLEANING_THRESHOLD_FACTOR_8, CARD_CLEANING_THRESHOLD_FACTOR_10, _allocToTraceRateNormal);
    _allocToTraceRateMinFactor = (float)1
        / interpolateInRange(
            MIN_ALLOC_2_TRACE_RATE_1, MIN_ALLOC_2_TRACE_RATE_8, MIN_ALLOC_2_TRACE_RATE_10, _allocToTraceRateNormal);
    _allocToTraceRateMaxFactor = interpolateInRange(
        MAX_ALLOC_2_TRACE_RATE_1, MAX_ALLOC_2_TRACE_RATE_8, MAX_ALLOC_2_TRACE_RATE_10, _allocToTraceRateNormal);

    if (_extensions->debugConcurrentMark) {
        OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
        omrtty_printf("Initial tuning statistics: Card Cleaning Factors Pass1=\"%.3f\" Pass2=\"%.3f\" (Maximum: "
                      "Pass1=\"%.3f\" Pass2=\"%.3f\")\n",
            _cardCleaningFactorPass1, _cardCleaningFactorPass2, _maxCardCleaningFactorPass1,
 ...
 */
MMINLINE float MM_ConcurrentGC::interpolateInRange(float val1, float val8, float val10, uintptr_t traceRate)
{
    float ret;
    ...
    while (!initDone) {
        ...
        while (NULL != (region = regionIterator.nextRegion())) {
            ...
            if (_numInitRanges < _numPhysicalInitRanges) {
                ...
                _initRanges[i].initBytes
                    = _markingScheme->numMarkBitsInRange(env, _initRanges[i].base, _initRanges[i].top);
        ...
        if (_numInitRanges > _numPhysicalInitRanges) {
            ...
            _initRanges = (InitWorkItem*)env->getForge()->allocate(
                sizeof(InitWorkItem) * _numInitRanges, OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
            if (NULL == _initRanges) {
                initDone = true;
                    ...
                    if ((_initRanges[x].type == MARK_BITS) && ((_initRanges[x].subspace)->isConcurrentCollectable())) {
                        ...
                        _initRanges[i].initBytes
                            = ((MM_ConcurrentCardTable*)_cardTable)
                                  ->cardBytesForHeapRange(env, _initRanges[i].base, _initRanges[i].top);
 ...
 */
bool MM_ConcurrentGC::getInitRange(
    MM_EnvironmentBase* env, void** from, void** to, InitType* type, bool* concurrentCollectable)
{
    uint32_t i;
    ...
    while (i < _numInitRanges) {
        ...
        if (localFrom < _initRanges[i].top) {
            ...
            if (localFrom
                == (uint8_t*)MM_AtomicOperations::lockCompareExchange(
                    (volatile uintptr_t*)&(_initRanges[i].current), (uintptr_t)localFrom, (uintptr_t)localTo)) {
                ...
                *concurrentCollectable = (_initRanges[i].subspace)->isConcurrentCollectable() ? true : false;
                ...
                Trc_MM_ConcurrentGC_getInitRange_Succeed(
                    env->getLanguageVMThread(), *from, *to, *type, *concurrentCollectable ? "true" : "false");
...
MM_ConcurrentGC::ConHelperRequest MM_ConcurrentGC::switchConHelperRequest(ConHelperRequest from, ConHelperRequest to)
{
...
MMINLINE MM_ConcurrentGC::ConHelperRequest MM_ConcurrentGC::getConHelperRequest(MM_EnvironmentBase* env)
{
...
void MM_ConcurrentGC::conHelperEntryPoint(OMR_VMThread* omrThread, uintptr_t slaveID)
{
    ...
    while (CONCURRENT_HELPER_SHUTDOWN != request) {
        ...
        /* perform trace work */
        while ((CONCURRENT_HELPER_MARK == request) && _markingScheme->getWorkPackets()->inputPacketAvailable(env)
            && spinLimiter.spin()) {
        ...
        /* clean cards */
        while ((CONCURRENT_HELPER_MARK == request) && (CONCURRENT_CLEAN_TRACE == _stats.getExecutionMode())
            && _cardTable->isCardCleaningStarted() && !_cardTable->isCardCleaningComplete() && spinLimiter.spin()) {
 ...
 * FALSE otherwise
 */
bool MM_ConcurrentGC::initializeConcurrentHelpers(MM_GCExtensionsBase* extensions)
{
    ...
    for (conHelperThreadCount = 0; conHelperThreadCount < _conHelperThreads; conHelperThreadCount++) {
        ...
        threadForkResult = createThreadWithCategory(&(_conHelpersTable[conHelperThreadCount]), OMR_OS_STACK_SIZE,
            J9THREAD_PRIORITY_MIN, 0, con_helper_thread_proc, (void*)&conHelperThreadInfo,
 ...
 */
void MM_ConcurrentGC::shutdownConHelperThreads(MM_GCExtensionsBase* extensions)
{
...
void MM_ConcurrentGC::tuneToHeap(MM_EnvironmentBase* env)
{
        ...
        if ((NULL == env->_cycleState) || env->_cycleState->_gcCode.isExplicitGC() || !_stwCollectionInProgress) {
            ...
            if (_secondCardCleanPass) {
                ...
                _bytesToTracePass1 = (uintptr_t)MM_Math::weightedAverage(
                    (float)_bytesToTracePass1, (float)bytesTracedPass1, LIVE_PART_HISTORY_WEIGHT);
                _bytesToTracePass2 = (uintptr_t)MM_Math::weightedAverage(
                    (float)_bytesToTracePass2, (float)bytesTracedPass2, LIVE_PART_HISTORY_WEIGHT);
                _bytesToCleanPass1 = (uintptr_t)MM_Math::weightedAverage(
                    (float)_bytesToCleanPass1, (float)bytesCleanedPass1, CARD_CLEANING_HISTORY_WEIGHT);
                _bytesToCleanPass2 = (uintptr_t)MM_Math::weightedAverage(
                    (float)_bytesToCleanPass2, (float)bytesCleanedPass2, CARD_CLEANING_HISTORY_WEIGHT);
            } else {
                _bytesToTracePass1 = (uintptr_t)MM_Math::weightedAverage(
                    (float)_bytesToTracePass1, (float)totalTraced, LIVE_PART_HISTORY_WEIGHT);
                _bytesToCleanPass1 = (uintptr_t)MM_Math::weightedAverage(
                    (float)_bytesToCleanPass1, (float)totalCleaned, CARD_CLEANING_HISTORY_WEIGHT);
             ...
             */
            totalBytesToTrace = _stats.getTraceSizeCount() + _stats.getConHelperTraceSizeCount()
                + _stats.getCompleteTracingCount() + _stats.getFinalTraceCount();
            totalBytesToClean
                = _stats.getCardCleanCount() + _stats.getConHelperCardCleanCount() + _stats.getFinalCardCleanCount();

            if (_secondCardCleanPass) {
                float pass1Ratio = _cardCleaningFactorPass2 > 0
                    ? (_cardCleaningFactorPass1 / (_cardCleaningFactorPass1 + _cardCleaningFactorPass2))
                    : 1;
                _bytesToTracePass1 = (uintptr_t)((float)totalBytesToTrace * _bytesTracedInPass1Factor);
                _bytesToTracePass2 = MM_Math::saturatingSubtract(totalBytesToTrace, _bytesToTracePass1);
     ...
     * the KO point.
     */
    kickoffThreshold = (_stats.getInitWorkRequired() / _allocToInitRate) + (_traceTargetPass1 / _allocToTraceRateNormal)
    ...
    kickoffThresholdPlusBuffer
        = (uintptr_t)((float)kickoffThreshold + boost + ((float)_extensions->concurrentSlack * kickoffProportion));
    _stats.setKickoffThreshold(kickoffThresholdPlusBuffer);
    _stats.setCardCleaningThreshold((uintptr_t)(
        (float)cardCleaningThreshold + boost + ((float)_extensions->concurrentSlack * cardCleaningProportion)));
    ...
    if (_extensions->debugConcurrentMark) {
        ...
        omrtty_printf("Tune to heap : Trace target Pass 1=\"%zu\" (Trace=\"%zu\" Clean=\"%zu\")\n", _traceTargetPass1,
            _bytesToTracePass1, _bytesToCleanPass1);
        omrtty_printf("               Trace target Pass 2=\"%zu\" (Trace=\"%zu\" Clean=\"%zu\")\n", _traceTargetPass2,
            _bytesToTracePass2, _bytesToCleanPass2);
    ...
    Trc_MM_ConcurrentGC_tuneToHeap_Exit2(env->getLanguageVMThread(), _stats.getTraceSizeTarget(),
...
void MM_ConcurrentGC::updateTuningStatistics(MM_EnvironmentBase* env)
{
    ...
    if (env->_cycleState->_gcCode.isExplicitGC() || (CONCURRENT_TRACE_ONLY > _stats.getExecutionModeAtGC())) {
        ...
        _tenureLiveObjectFactor
            = MM_Math::weightedAverage(_tenureLiveObjectFactor, newLiveObjectFactor, LIVE_PART_HISTORY_WEIGHT);
        ...
        _tenureNonLeafObjectFactor
            = MM_Math::weightedAverage(_tenureNonLeafObjectFactor, newNonLeafObjectFactor, NON_LEAF_HISTORY_WEIGHT);
        ...
        case CONCURRENT_TRACE_ONLY:
            _cardCleaningFactorPass1 = MM_Math::weightedAverage(
                _cardCleaningFactorPass1, _maxCardCleaningFactorPass1, CARD_CLEANING_HISTORY_WEIGHT);
            _cardCleaningFactorPass2 = MM_Math::weightedAverage(
                _cardCleaningFactorPass2, _maxCardCleaningFactorPass2, CARD_CLEANING_HISTORY_WEIGHT);
            _bytesTracedInPass1Factor = MM_Math::weightedAverage(
                _bytesTracedInPass1Factor, ALL_BYTES_TRACED_IN_PASS_1, BYTES_TRACED_IN_PASS_1_HISTORY_WEIGHT);
            break;
        ...
        case CONCURRENT_FINAL_COLLECTION:
            ...
            _cardCleaningFactorPass1 = MM_Math::weightedAverage(
                _cardCleaningFactorPass1, newCardCleaningFactorPass1, CARD_CLEANING_HISTORY_WEIGHT);
            _bytesTracedInPass1Factor = MM_Math::weightedAverage(
                _bytesTracedInPass1Factor, newBytesTracedInPass1Factor, BYTES_TRACED_IN_PASS_1_HISTORY_WEIGHT);
            ...
            if (_secondCardCleanPass) {
                ...
                _cardCleaningFactorPass2 = MM_Math::weightedAverage(
                    _cardCleaningFactorPass2, newCardCleaningFactorPass2, CARD_CLEANING_HISTORY_WEIGHT);
            }

            break;
        default:
            Assert_GC_true_with_message(env,
                (CONCURRENT_ROOT_TRACING <= executionModeAtGC) && (CONCURRENT_TRACE_ONLY > executionModeAtGC),
        ...
        if (_extensions->debugConcurrentMark) {
            ...
            omrtty_printf("Update tuning statistics: Total Traced=\"%zu\" (Pass 2 KO=\"%zu\")  Total Cleaned=\"%zu\" "
                          "(Pass 2 KO=\"%zu\")\n",
                totalTraced, _totalTracedAtPass2KO, totalCleaned, _totalCleanedAtPass2KO);
            omrtty_printf("                          Tenure Live object Factor=\"%.3f\" Tenure non-leaf object "
                          "factor=\"%.3f\" \n",
 ...
 */
uintptr_t MM_ConcurrentGC::calculateInitSize(MM_EnvironmentBase* env, uintptr_t allocationSize)
{
 ...
 * @return the allocation tax
 */
uintptr_t MM_ConcurrentGC::calculateTraceSize(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
    float thisTraceRate;
...
#endif /* OMR_GC_MODRON_SCAVENGER */
    {
        ...
        /* in the case of Tarok phase4, the pool from the allocation description doesn't have enough context to help
         * guide concurrent decisions so we need the parent */
     ...
     */
    workCompleteSoFar = (_stats.getTraceSizeCount() + _stats.getCardCleanCount() + _stats.getConHelperTraceSizeCount()
 ...
 */
bool MM_ConcurrentGC::periodicalTuningNeeded(MM_EnvironmentBase* env, uintptr_t freeSize)
{
    ...
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
    ...
    nurseryPromotion = scavengerStats->_avgTenureBytes == 0
        ? 1
        : (uintptr_t)(scavengerStats->_avgTenureBytes
            + (env->getExtensions()->tenureBytesDeviationBoost * scavengerStats->_avgTenureBytesDeviation));
    ...
    if (LOA == _meteringType) {
        ...
        currentOldFree
            = oldSubspace->getApproximateActiveFreeMemorySize() - oldSubspace->getApproximateActiveFreeLOAMemorySize();
        headRoom = (uintptr_t)(_extensions->concurrentKickoffTenuringHeadroom
            * (_extensions->getLastGlobalGCFreeBytes() - _extensions->lastGlobalGCFreeBytesLOA));
    }
#else
    ...
    if (NULL != stats) {
        uintptr_t fragmentation = (uintptr_t)((env->getExtensions())->concurrentSlackFragmentationAdjustmentWeight
    ...
    uint64_t potentialFree
        = (uint64_t)currentNurseryFree + ((uint64_t)nurseryInitialFree * (uint64_t)scavengesRemaining);
 ...
 * @param  freeSize the current amount of free space in the old area
 */
void MM_ConcurrentGC::periodicalTuning(MM_EnvironmentBase* env, uintptr_t freeSize)
{
    ...
    if (_lastFreeSize == LAST_FREE_SIZE_NEEDS_INITIALIZING) {
        ...
        if (_conHelpersStarted > 0) {
            ...
            _alloc2ConHelperTraceRate = MM_Math::weightedAverage(
                _alloc2ConHelperTraceRate, newConHelperRate, CONCURRENT_HELPER_HISTORY_WEIGHT);
 ...
 *
 */
void MM_ConcurrentGC::kickoffCardCleaning(MM_EnvironmentBase* env, ConcurrentCardCleaningReason reason)
{
 ...
 */
bool MM_ConcurrentGC::replenishPoolForAllocate(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool, uintptr_t size)
{
 ...
 */
void MM_ConcurrentGC::payAllocationTax(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace,
 ...
 * @note This is a potential GC point.
 */
void MM_ConcurrentGC::concurrentMark(
    MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, MM_AllocateDescription* allocDescription)
{
    ...
    while (!taxPaid) {
        ...
        case CONCURRENT_ROOT_TRACING:
            ...
            Assert_GC_true_with_message(env,
                (CONCURRENT_ROOT_TRACING < nextExecutionMode) || (CONCURRENT_TRACE_ONLY == nextExecutionMode),
                "MM_ConcurrentMarkingDelegate::getNextTracingMode(CONCURRENT_ROOT_TRACING) = %zu\n", nextExecutionMode);
        ...
        default:
            ...
            Assert_GC_true_with_message(env,
                (CONCURRENT_ROOT_TRACING < executionMode) && (CONCURRENT_TRACE_ONLY > executionMode),
                "MM_ConcurrentStats::_executionMode = %zu\n", executionMode);
            ...
            if (_stats.switchExecutionMode(executionMode, nextExecutionMode)) {
                Assert_GC_true_with_message2(env,
                    (CONCURRENT_ROOT_TRACING < nextExecutionMode) && (CONCURRENT_TRACE_ONLY >= nextExecutionMode),
                    "MM_ConcurrentStats::_executionMode = %zu; "
                    "MM_ConcurrentMarkingDelegate::getNextTracingMode(MM_ConcurrentStats::_executionMode) = %zu\n",
...
void MM_ConcurrentGC::signalThreadsToActivateWriteBarrier(MM_EnvironmentBase* env)
{
 ...
 */
bool MM_ConcurrentGC::forceKickoff(
    MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocateDescription* allocDescription, uint32_t gcCode)
{
    if (_extensions->concurrentKickoffEnabled
        && _concurrentDelegate.canForceConcurrentKickoff(env, gcCode, &_languageKickoffReason)) {
 ...
 */
bool MM_ConcurrentGC::timeToKickoffConcurrent(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
 ...
 * Finish all concurrent sweep activities.
 */
void MM_ConcurrentGC::completeConcurrentSweepForKickoff(MM_EnvironmentBase* env)
{
    /* If concurrent sweep is not enabled, do nothing */
    if (!_extensions->concurrentSweep) {
        return;
 ...
 */
uintptr_t MM_ConcurrentGC::doConcurrentInitialization(MM_EnvironmentBase* env, uintptr_t initToDo)
{
 ...
 */
uintptr_t MM_ConcurrentGC::doConcurrentTrace(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    uintptr_t sizeToTrace, MM_MemorySubSpace* subspace, bool threadAtSafePoint)
{
    uintptr_t sizeTraced = 0;
     ...
     */
    while (!env->isExclusiveAccessRequestWaiting() && (sizeTraced < sizeToTrace) && (sizeTraced != sizeTracedPreviously)
        ...
        if (!env->isExclusiveAccessRequestWaiting() && sizeTraced < sizeToTrace) {
            ...
            if (CONCURRENT_TRACE_ONLY == _stats.getExecutionMode()) {
                 ...
                 */
                if ((_markingScheme->getWorkPackets()->tracingExhausted() || tracingRateDropped(env))
                ...
                if (!((MM_ConcurrentCardTable*)_cardTable)->isCardCleaningComplete()) {
                    ...
                    if (!env->isThreadScanned()) {
                        ...
                        if (_markingScheme->getWorkPackets()->tracingExhausted()) {
                             ...
                             * must be near completing the tracing. However, the lower priority concurrent helpers could
                             * be suspended and hanging onto packets and so preventing us from getting to
                             * CONCURENT_EXHAUSTED. This will delay the final collection; maybe until an AF which we
                             * don't want. We need to tell the concurrent helpers to release any packets they have so we
                             * can get them processed. This is easily achieved by suspending them. Anyway, now is also a
                             * good time to tell the concurrent helpers to park themselves for the imminent global
                             * collect.
                             */
    ...
    if (!isGcOccurred) {
        ...
        if ((_markingScheme->getWorkPackets()->inputPacketAvailable(env))
            || ((NULL != _cardTable) && _cardTable->isCardCleaningStarted() && !_cardTable->isCardCleaningComplete())) {
            resumeConHelperThreads(env);
        }
    }
 ...
 */
bool MM_ConcurrentGC::concurrentFinalCollection(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace)
{
 ...
 */
uintptr_t MM_ConcurrentGC::localMark(MM_EnvironmentBase* env, uintptr_t sizeToTrace)
{
 ...
 */
MMINLINE bool MM_ConcurrentGC::cleanCards(
    MM_EnvironmentBase* env, bool isMutator, uintptr_t sizeToDo, uintptr_t* sizeDone, bool threadAtSafePoint)
{
    ...
    env->_cycleState = &_concurrentCycleState;
    bool gcOccurred
        = ((MM_ConcurrentCardTable*)_cardTable)->cleanCards(env, isMutator, sizeToDo, sizeDone, threadAtSafePoint);
    flushLocalBuffers(env);
    env->_cycleState = NULL;
    ...
    MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocateDescription* allocDescription, uint32_t gcCode)
{
    ...
    if ((J9MMCONSTANT_EXPLICIT_GC_IDLE_GC == gcCode) && (CONCURRENT_OFF < executionModeAtGC)) {
        ...
        if (_extensions->isRememberedSetInOverflowState()
            || ((CONCURRENT_OFF < executionModeAtGC) && (CONCURRENT_TRACE_ONLY > executionModeAtGC))) {
        CollectionAbortReason reason
            = (_extensions->isRememberedSetInOverflowState() ? ABORT_COLLECTION_REMEMBERSET_OVERFLOW
    ...
    MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocateDescription* allocDescription)
{
     ...
     * It's not good to do it in concurrentFinalCollection(), as an AF may occur before we get chance to do final
     * collection, so we may miss to restore it. It's too late to do it in postCollect(), as the AF will make us re-try
     * allocate just after collection is done, but postCollect() is invoked after the re-try.
     */
 ...
 */
void MM_ConcurrentGC::internalPostCollect(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace)
{
 ...
 */
void MM_ConcurrentGC::abortCollection(MM_EnvironmentBase* env, CollectionAbortReason reason)
{
 ...
 */
bool MM_ConcurrentGC::heapAddRange(
 ...
 */
bool MM_ConcurrentGC::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size,
    void* lowAddress, void* highAddress, void* lowValidAddress, void* highValidAddress)
{
    Trc_MM_ConcurrentGC_heapRemoveRange_Entry(
        env->getLanguageVMThread(), subspace, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
    ...
    result = result
        && ((MM_ConcurrentCardTable*)_cardTable)
               ->heapRemoveRange(env, subspace, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
...
void MM_ConcurrentGC::scanRememberedSet(MM_EnvironmentBase* env)
{
    ...
    while ((puddle = rememberedSetIterator.nextList()) != NULL) {
        if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
            ...
            while ((slotPtr = (omrobjectptr_t*)rememberedSetSlotIterator.nextSlot()) != NULL) {
                 ...
                 * by finalCleanCards()
                 */
                if ((objectPtr >= _heapBase) && (objectPtr < _heapAlloc) && _markingScheme->isMarkedOutline(objectPtr)
     ...
     * Since popNoWait is used, we never stalled before this point. All stall time will be from this point till RS scan
     * end event */
 ...
 */
void MM_ConcurrentGC::oldToOldReferenceCreated(MM_EnvironmentBase* env, omrobjectptr_t objectPtr)
{
...
void MM_ConcurrentGC::updateMeteringHistoryAfterGC(MM_EnvironmentBase* env)
{
    ...
    if (MM_GCExtensionsBase::METER_DYNAMIC == _extensions->concurrentMetering) {
        ...
        _currentMeteringHistory
            = ((_currentMeteringHistory + 1) == _meteringHistorySize) ? 0 : _currentMeteringHistory + 1;
    }
}
#endif /* OMR_GC_LARGE_OBJECT_AREA */
