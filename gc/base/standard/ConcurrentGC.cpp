 ...
 */
bool MM_ConcurrentGC::getInitRange(MM_EnvironmentBase* env, void** from, void** to, InitType* type, bool* concurrentCollectable)
{
    uint32_t i;
...
void MM_ConcurrentGC::tuneToHeap(MM_EnvironmentBase* env)
{
    ...
    if (0 == _stats.getKickoffThreshold() || _retuneAfterHeapResize) {
        ...
        if ((NULL == env->_cycleState) || env->_cycleState->_gcCode.isExplicitGC() || !_stwCollectionInProgress) {
			 ...
			 */
            totalBytesToTrace = _stats.getTraceSizeCount() + _stats.getConHelperTraceSizeCount() + _stats.getCompleteTracingCount() + _stats.getFinalTraceCount();
            totalBytesToClean = _stats.getCardCleanCount() + _stats.getConHelperCardCleanCount() + _stats.getFinalCardCleanCount();
	 ...
	 * the KO point.
	 */
    kickoffThreshold = (_stats.getInitWorkRequired() / _allocToInitRate) + (_traceTargetPass1 / _allocToTraceRateNormal) + (_traceTargetPass2 / (_allocToTraceRateNormal * _allocToTraceRateCardCleanPass2Boost));
...
MM_ConcurrentGC::calculateTraceSize(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
	 ...
	 */
    workCompleteSoFar = (_stats.getTraceSizeCount() + _stats.getCardCleanCount() + _stats.getConHelperTraceSizeCount() + _stats.getConHelperCardCleanCount());
...
bool MM_ConcurrentGC::periodicalTuningNeeded(MM_EnvironmentBase* env, uintptr_t freeSize)
{
    ...
    if (LAST_FREE_SIZE_NEEDS_INITIALIZING == _lastFreeSize || ((_lastFreeSize > freeSize) && (_lastFreeSize - freeSize > _tuningUpdateInterval))) {
        return true;
    }
 ...
 */
void MM_ConcurrentGC::payAllocationTax(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, MM_MemorySubSpace* baseSubSpace, MM_AllocateDescription* allocDescription)
{
 ...
 * @note This is a potential GC point.
 */
void MM_ConcurrentGC::concurrentMark(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, MM_AllocateDescription* allocDescription)
{
 ...
 */
bool MM_ConcurrentGC::forceKickoff(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocateDescription* allocDescription, uint32_t gcCode)
{
 ...
 */
void MM_ConcurrentGC::concurrentSweep(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, MM_AllocateDescription* allocDescription)
{
    ...
    bool threadAtSafePoint)
{
	 ...
	 */
    while (!env->isExclusiveAccessRequestWaiting() && (sizeTraced < sizeToTrace) && (sizeTraced != sizeTracedPreviously) && (CONCURRENT_CLEAN_TRACE >= _stats.getExecutionMode())) {
    ...
    if (!isGcOccurred) {
        ...
        if ((NULL != _cardTable) && ((MM_ConcurrentCardTable*)_cardTable)->isCardCleaningComplete() && _markingScheme->getWorkPackets()->tracingExhausted() && _concurrentDelegate.isConcurrentScanningComplete(env)) {
 ...
 */
void MM_ConcurrentGC::internalPreCollect(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocateDescription* allocDescription, uint32_t gcCode)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
 ...
 */
bool MM_ConcurrentGC::internalGarbageCollect(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocateDescription* allocDescription)
{
 ...
 */
bool MM_ConcurrentGC::heapAddRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size, void* lowAddress, void* highAddress)
{
 ...
 */
bool MM_ConcurrentGC::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size, void* lowAddress, void* highAddress, void* lowValidAddress, void* highValidAddress)
{
