...
bool MM_ConcurrentGC::initialize(MM_EnvironmentBase *env) {
  ...
  if (_extensions->debugConcurrentMark) {
    ...
    omrtty_printf("                           Allocate to trace Rate Factors "
                  "Minimum=\"%f\" Maximum=\"%f\"\n",
...
MM_ConcurrentGC::switchConHelperRequest(ConHelperRequest from,
                                        ConHelperRequest to) {
...
void MM_ConcurrentGC::tuneToHeap(MM_EnvironmentBase *env) {
  ...
  if (_extensions->debugConcurrentMark) {
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
    omrtty_printf("Tune to heap : Trace target Pass 1=\"%zu\" (Trace=\"%zu\" "
                  "Clean=\"%zu\")\n",
                  _traceTargetPass1, _bytesToTracePass1, _bytesToCleanPass1);
    omrtty_printf("               Trace target Pass 2=\"%zu\" (Trace=\"%zu\" "
                  "Clean=\"%zu\")\n",
                  _traceTargetPass2, _bytesToTracePass2, _bytesToCleanPass2);
    omrtty_printf(
...
void MM_ConcurrentGC::updateTuningStatistics(MM_EnvironmentBase *env) {
  ...
  if (env->_cycleState->_gcCode.isExplicitGC() ||
    ...
    case CONCURRENT_FINAL_COLLECTION:
      ...
      /* What factor of the tracing work was done beofre we started 2nd pass of
       * card cleaning ?*/
    ...
    if (_extensions->debugConcurrentMark) {
      ...
      omrtty_printf("Update tuning statistics: Total Traced=\"%zu\" (Pass 2 "
                    "KO=\"%zu\")  Total Cleaned=\"%zu\" (Pass 2 KO=\"%zu\")\n",
                    totalTraced, _totalTracedAtPass2KO, totalCleaned,
                    _totalCleanedAtPass2KO);
      omrtty_printf("                          Tenure Live object "
                    "Factor=\"%.3f\" Tenure non-leaf object factor=\"%.3f\" \n",
                    _tenureLiveObjectFactor, _tenureNonLeafObjectFactor);
      omrtty_printf("                          Card Cleaning Factors: "
                    "Pass1=\"%s\" Pass2=\"%s\"\n",
 ...
 */
uintptr_t
MM_ConcurrentGC::calculateTraceSize(MM_EnvironmentBase *env,
  ...
  while (!taxPaid) {
    ...
    case CONCURRENT_INIT_COMPLETE:
      if (_extensions->optimizeConcurrentWB) {
        ...
        /* TODO: Once optimizeConcurrentWB enabled by default this code will be
         * deleted */
    ...
    case CONCURRENT_EXHAUSTED:
       ...
       * can trigger the final collection here provided we are at a safe point.
       * If not then GC will be triggered by next allocation.
       */
      if (threadAtSafePoint) {
