 ...
 * External entry point for write barrier. Calls card table function to dirty
 * card for given object which has had an object reference modified.
 *
 ...
 */
void J9ConcurrentWriteBarrierStore(OMR_VMThread *vmThread,
                                   omrobjectptr_t destinationObject,
                                   omrobjectptr_t storedObject) {
 ...
 * External entry point for write barrier. Calls card table function to dirty
 * card for given object which has had multiple object references mofified.
 *
 * @param destinationObject Address of object containing modified reference
 */
void J9ConcurrentWriteBarrierBatchStore(OMR_VMThread *vmThread,
                                        omrobjectptr_t destinationObject) {
  MM_EnvironmentBase *env = MM_EnvironmentBase::getEnvironment(vmThread);
  MM_GCExtensionsBase *extensions = env->getExtensions();
 ...
 */
int J9THREAD_PROC con_helper_thread_proc(void *info) {
  ...
  omrsig_handler_fn signalHandler =
      collector->_concurrentDelegate.getProtectedSignalHandler(
          &signalHandlerArg);
  omrsig_protect(
      con_helper_thread_proc2, info, signalHandler, signalHandlerArg,
...
uintptr_t con_helper_thread_proc2(OMRPortLibrary *portLib, void *info) {
  ...
  OMR_VMThread *omrThread = MM_EnvironmentBase::attachVMThread(
      omrVM, "Concurrent Mark Helper",
      MM_EnvironmentBase::ATTACH_GC_HELPER_THREAD);

  /* Signal that the concurrent helper thread has started (or not) */
  conHelperThreadInfo->threadFlags =
      NULL != omrThread ? CON_HELPER_INFO_FLAG_OK : CON_HELPER_INFO_FLAG_FAIL;
...
void oldToOldReferenceCreated(MM_EnvironmentBase *env,
  ...
  ((MM_ConcurrentGC *)env->getExtensions()->getGlobalCollector())
...
void MM_ConcurrentGC::reportConcurrentKickoff(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

  Trc_MM_ConcurrentKickoff(
      env->getLanguageVMThread(), _stats.getTraceSizeTarget(),
      _stats.getKickoffThreshold(), _stats.getRemainingFree());
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_KICKOFF(
      ...
      omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_KICKOFF,
      _extensions->getHeap()->initializeCommonGCData(env, &commonData),
      _stats.getTraceSizeTarget(), _stats.getKickoffThreshold(),
      _stats.getRemainingFree(), _stats.getKickoffReason(),
      _languageKickoffReason);
}

void MM_ConcurrentGC::reportConcurrentAborted(MM_EnvironmentBase *env,
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_ABORTED(
      _extensions->privateHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_ABORTED, reason);
}

void MM_ConcurrentGC::reportConcurrentHalted(MM_EnvironmentBase *env) {
  MM_ConcurrentCardTable *cardTable = (MM_ConcurrentCardTable *)_cardTable;
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

  Trc_MM_ConcurrentHalted(
      env->getLanguageVMThread(), (uintptr_t)_stats.getExecutionModeAtGC(),
      _stats.getTraceSizeTarget(), _stats.getTotalTraced(),
      _stats.getMutatorsTraced(), _stats.getConHelperTraced(),
  ...
  Trc_MM_ConcurrentHaltedState(
      env->getLanguageVMThread(),
      cardTable->isCardCleaningComplete() ? "complete" : "incomplete",
      _concurrentDelegate.isConcurrentScanningComplete(env) ? "complete"
                                                            : "incomplete",
      _markingScheme->getWorkPackets()->tracingExhausted() ? "complete"
                                                           : "incomplete");

  TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_HALTED(
      _extensions->privateHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_HALTED,
      (uintptr_t)_stats.getExecutionModeAtGC(), _stats.getTraceSizeTarget(),
      _stats.getTotalTraced(), _stats.getMutatorsTraced(),
...
void MM_ConcurrentGC::reportConcurrentFinalCardCleaningStart(
    MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

  Trc_MM_ConcurrentCollectionCardCleaningStart(
      env->getLanguageVMThread(), _stats.getConcurrentWorkStackOverflowCount());
...
void MM_ConcurrentGC::reportConcurrentFinalCardCleaningEnd(
    MM_EnvironmentBase *env, uint64_t duration) {
  MM_ConcurrentCardTable *cardTable = (MM_ConcurrentCardTable *)_cardTable;
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_CARD_CLEANING_END(
      _extensions->privateHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(),
      J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_CARD_CLEANING_END, duration,
...
void MM_ConcurrentGC::reportConcurrentCollectionStart(MM_EnvironmentBase *env) {
  MM_ConcurrentCardTable *cardTable = (MM_ConcurrentCardTable *)_cardTable;
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

  Trc_MM_ConcurrentCollectionStart(
      env->getLanguageVMThread(),
  ...
  uint64_t exclusiveAccessTimeMicros = omrtime_hires_delta(
      0, env->getExclusiveAccessTime(), OMRPORT_TIME_DELTA_IN_MICROSECONDS);
  uint64_t meanExclusiveAccessIdleTimeMicros =
      omrtime_hires_delta(0, env->getMeanExclusiveAccessIdleTime(),
  ...
  if (J9_EVENT_IS_HOOKED(_extensions->privateHookInterface,
    ...
    ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START(
        _extensions->privateHookInterface, env->getOmrVMThread(),
        omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START,
        &commonData, _stats.getTraceSizeTarget(), _stats.getTotalTraced(),
        _stats.getMutatorsTraced(), _stats.getConHelperTraced(),
        cardTable->getCardTableStats()->getConcurrentCleanedCards(),
        _stats.getCardCleaningThreshold(),
        _stats.getConcurrentWorkStackOverflowOcurred(),
        _stats.getConcurrentWorkStackOverflowCount(),
        _stats.getThreadsToScanCount(), _stats.getThreadsScannedCount(),
        _stats.getCardCleaningReason());
  }
}

void MM_ConcurrentGC::reportConcurrentCollectionEnd(MM_EnvironmentBase *env,
                                                    uint64_t duration) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

  Trc_MM_ConcurrentCollectionEnd(
      env->getLanguageVMThread(),
      _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_NEW),
      _extensions->heap->getActiveMemorySize(MEMORY_TYPE_NEW),
      _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_OLD),
  ...
  if (J9_EVENT_IS_HOOKED(_extensions->privateHookInterface,
    ...
    ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END(
        _extensions->privateHookInterface, env->getOmrVMThread(),
        omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END,
        duration, env->getExclusiveAccessTime(), &commonData);
  }
}

void MM_ConcurrentGC::reportConcurrentBackgroundThreadActivated(
    MM_EnvironmentBase *env) {
  Trc_MM_ConcurrentBackgroundThreadActivated(env->getLanguageVMThread());
  TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_BACKGROUND_THREAD_ACTIVATED(
      _extensions->privateHookInterface, env->getOmrVMThread());
}

void MM_ConcurrentGC::reportConcurrentBackgroundThreadFinished(
    MM_EnvironmentBase *env, uintptr_t traceTotal) {
  Trc_MM_ConcurrentBackgroundThreadFinished(env->getLanguageVMThread());
  TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_BACKGROUND_THREAD_FINISHED(
      _extensions->privateHookInterface, env->getOmrVMThread(), traceTotal);
}

void MM_ConcurrentGC::reportConcurrentCompleteTracingStart(
    MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

  Trc_MM_ConcurrentCompleteTracingStart(
      env->getLanguageVMThread(), _stats.getConcurrentWorkStackOverflowCount());
...
void MM_ConcurrentGC::reportConcurrentCompleteTracingEnd(
    MM_EnvironmentBase *env, uint64_t duration) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

  Trc_MM_ConcurrentCompleteTracingEnd(
      env->getLanguageVMThread(), _stats.getCompleteTracingCount(),
      _stats.getConcurrentWorkStackOverflowCount());

  TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COMPLETE_TRACING_END(
      _extensions->privateHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_COMPLETE_TRACING_END,
      duration, _stats.getCompleteTracingCount(),
      _stats.getConcurrentWorkStackOverflowCount());
}

void MM_ConcurrentGC::reportConcurrentRememberedSetScanStart(
    MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

  Trc_MM_ConcurrentRememberedSetScanStart(
      env->getLanguageVMThread(), _stats.getConcurrentWorkStackOverflowCount());
...
void MM_ConcurrentGC::reportConcurrentRememberedSetScanEnd(
    MM_EnvironmentBase *env, uint64_t duration) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

  Trc_MM_ConcurrentRememberedSetScanEnd(
      env->getLanguageVMThread(), _stats.getRSObjectsFound(),
      _stats.getRSScanTraceCount(),
      _stats.getConcurrentWorkStackOverflowCount());
  TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_END(
      _extensions->privateHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(),
      J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_END, duration,
      _stats.getRSObjectsFound(), _stats.getRSScanTraceCount(),
      _stats.getConcurrentWorkStackOverflowCount());
}

/**
 * Aysnc callback routine to signal all threads that they needs to start
 * dirtying cards.
 *
 * @note Caller assumed to be at a safe point
 *
 */
void MM_ConcurrentGC::signalThreadsToActivateWriteBarrierAsyncEventHandler(
    OMR_VMThread *omrVMThread, void *userData) {
 ...
 */
MM_ConcurrentGC *MM_ConcurrentGC::newInstance(MM_EnvironmentBase *env) {
  MM_ConcurrentGC *concurrentGC = (MM_ConcurrentGC *)env->getForge()->allocate(
      sizeof(MM_ConcurrentGC), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
 ...
 *
 */
void MM_ConcurrentGC::kill(MM_EnvironmentBase *env) {
  tearDown(env);
 ...
 * Instantiate card table and concurrent RAS objects(if required) and initialize
 * all monitors required by concurrent. Allocate and initialize the concurrent
 * helper thread table.
 *
 * @return TRUE if initialization completed OK;FALSE otheriwse
 */
bool MM_ConcurrentGC::initialize(MM_EnvironmentBase *env) {
  ...
  if (_extensions->optimizeConcurrentWB) {
    ...
    _callback->registerCallback(
        env, signalThreadsToActivateWriteBarrierAsyncEventHandler, this);
  }

  if (_conHelperThreads > 0) {
    /* Get storage for concurrent helper thread table */
    _conHelpersTable = (omrthread_t *)env->getForge()->allocate(
        _conHelperThreads * sizeof(omrthread_t),
  ...
  /* Init pointer to heap base; we will reset _heapAlloc in expanded as heap
   * grows */
  ...
  if (omrthread_monitor_init_with_name(
          &_conHelpersActivationMonitor, 0,
          "MM_ConcurrentGC::conHelpersActivation")) {
    goto error_no_memory;
  }

  /* Initialize the initialisation work monitor */
  if (omrthread_monitor_init_with_name(&_initWorkMonitor, 0,
                                       "MM_ConcurrentGC::initWork")) {
    goto error_no_memory;
  }

  /* Initialize the tuning monitor monitor */
  if (omrthread_monitor_init_with_name(&_concurrentTuningMonitor, 0,
                                       "MM_ConcurrentGC::concurrentTuning")) {
    goto error_no_memory;
  }

  /* ..and the initialization work complete monitor */
  if (omrthread_monitor_init_with_name(&_initWorkCompleteMonitor, 0,
                                       "MM_ConcurrentGC::initWorkComplete")) {
    goto error_no_memory;
  }

  _allocToInitRate =
      _extensions->concurrentLevel * CONCURRENT_INIT_BOOST_FACTOR;
  ...
  _cardCleaningFactorPass1 = interpolateInRange(
      INITIAL_CARD_CLEANING_FACTOR_PASS1_1,
      INITIAL_CARD_CLEANING_FACTOR_PASS1_8,
      INITIAL_CARD_CLEANING_FACTOR_PASS1_10, _allocToTraceRateNormal);
  _maxCardCleaningFactorPass1 = interpolateInRange(
      MAX_CARD_CLEANING_FACTOR_PASS1_1, MAX_CARD_CLEANING_FACTOR_PASS1_8,
      MAX_CARD_CLEANING_FACTOR_PASS1_10, _allocToTraceRateNormal);
  _bytesTracedInPass1Factor = ALL_BYTES_TRACED_IN_PASS_1;
  if (_secondCardCleanPass) {
    _cardCleaningFactorPass2 = interpolateInRange(
        INITIAL_CARD_CLEANING_FACTOR_PASS2_1,
        INITIAL_CARD_CLEANING_FACTOR_PASS2_8,
        INITIAL_CARD_CLEANING_FACTOR_PASS2_10, _allocToTraceRateNormal);

    _maxCardCleaningFactorPass2 = interpolateInRange(
        MAX_CARD_CLEANING_FACTOR_PASS2_1, MAX_CARD_CLEANING_FACTOR_PASS2_8,
        MAX_CARD_CLEANING_FACTOR_PASS2_10, _allocToTraceRateNormal);
  } else {
    _cardCleaningFactorPass2 = 0;
    _maxCardCleaningFactorPass2 = 0;
  }

  _cardCleaningThresholdFactor = interpolateInRange(
      CARD_CLEANING_THRESHOLD_FACTOR_1, CARD_CLEANING_THRESHOLD_FACTOR_8,
      CARD_CLEANING_THRESHOLD_FACTOR_10, _allocToTraceRateNormal);
  _allocToTraceRateMinFactor =
      (float)1 /
      interpolateInRange(MIN_ALLOC_2_TRACE_RATE_1, MIN_ALLOC_2_TRACE_RATE_8,
                         MIN_ALLOC_2_TRACE_RATE_10, _allocToTraceRateNormal);
  _allocToTraceRateMaxFactor =
      interpolateInRange(MAX_ALLOC_2_TRACE_RATE_1, MAX_ALLOC_2_TRACE_RATE_8,
                         MAX_ALLOC_2_TRACE_RATE_10, _allocToTraceRateNormal);

  if (_extensions->debugConcurrentMark) {
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
    omrtty_printf(
        "Initial tuning statistics: Card Cleaning Factors Pass1=\"%.3f\" "
        "Pass2=\"%.3f\" (Maximum: Pass1=\"%.3f\" Pass2=\"%.3f\")\n",
        _cardCleaningFactorPass1, _cardCleaningFactorPass2,
        _maxCardCleaningFactorPass1, _maxCardCleaningFactorPass2);
    omrtty_printf(
        "                           Card Cleaning Threshold Factor=\"%.3f\"\n",
        _cardCleaningThresholdFactor);
    omrtty_printf(
        "                           Allocate to trace Rate Factors "
        "Minimum=\"%f\" Maximum=\"%f\"\n",
  ...
  if (_extensions->largeObjectArea) {
    if (_extensions->concurrentMetering == MM_GCExtensionsBase::METER_DYNAMIC) {
      ...
      _meteringHistory = (MeteringHistory *)env->getForge()->allocate(
          historySize, OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    ...
    } else if (_extensions->concurrentMetering ==
               MM_GCExtensionsBase::METER_BY_LOA) {
      _meteringType = LOA;
    }
  }
 ...
 */
void MM_ConcurrentGC::tearDown(MM_EnvironmentBase *env) {
 ...
 */
MMINLINE float MM_ConcurrentGC::interpolateInRange(float val1, float val8,
                                                   float val10,
                                                   uintptr_t traceRate) {
  float ret;
 ...
 * For the card table we create one entry for each segment of heap which is
 * concurrently collectable. All associated cards in card table will be set to
 * clean (0x00).
 */
void MM_ConcurrentGC::determineInitWork(MM_EnvironmentBase *env) {
  bool initDone = false;
  ...
  while (!initDone) {
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
      ...
      if (_numInitRanges < _numPhysicalInitRanges) {
        ...
        _initRanges[i].initBytes = _markingScheme->numMarkBitsInRange(
            env, _initRanges[i].base, _initRanges[i].top);
        _initRanges[i].type = MARK_BITS;
        _initRanges[i].chunkSize =
            INIT_CHUNK_SIZE * _markingScheme->numHeapBytesPerMarkMapByte();
        i++;
      }
    ...
    if (_numInitRanges > _numPhysicalInitRanges) {
      ...
      /* TODO: dynamically allocating this structure.  Should the VM tear itself
       * down in this scenario?
       */
      _initRanges = (InitWorkItem *)env->getForge()->allocate(
          sizeof(InitWorkItem) * _numInitRanges,
          OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
      if (NULL == _initRanges) {
        initDone = true;
      ...
      if (_extensions->configuration->isIncrementalUpdateBarrierEnabled()) {
        /* Add init ranges for all card table ranges for concurrently
         * collectable segments */
        for (I_32 x = i - 1; x >= 0; x--) {
          if ((_initRanges[x].type == MARK_BITS) &&
              ((_initRanges[x].subspace)->isConcurrentCollectable())) {
            _initRanges[i].base = _initRanges[x].base;
            _initRanges[i].top = _initRanges[x].top;
            _initRanges[i].current = _initRanges[i].base;
            _initRanges[i].subspace = _initRanges[x].subspace;
            _initRanges[i].initBytes =
                ((MM_ConcurrentCardTable *)_cardTable)
                    ->cardBytesForHeapRange(env, _initRanges[i].base,
                                            _initRanges[i].top);
 ...
 */
void MM_ConcurrentGC::resetInitRangesForConcurrentKO() {
  for (uint32_t i = 0; i < _numInitRanges; i++) {
    _initRanges[i].current = _initRanges[i].base;
  }
 ...
 */
void MM_ConcurrentGC::resetInitRangesForSTW() {
  for (uint32_t i = 0; i < _numInitRanges; i++) {
    if (MARK_BITS == _initRanges[i].type &&
        (!((_initRanges[i].subspace)->isConcurrentCollectable()))) {
      _initRanges[i].current = _initRanges[i].base;
    } else {
 ...
 */
bool MM_ConcurrentGC::getInitRange(MM_EnvironmentBase *env, void **from,
                                   void **to, InitType *type,
  ...
  while (i < _numInitRanges) {
    ...
    if (localFrom < _initRanges[i].top) {
      ...
      if ((uintptr_t)((uint8_t *)_initRanges[i].top - (uint8_t *)localFrom) <=
          chunkSize) {
         ...
         */
        if (localTo >=
            (void *)((uint8_t *)_initRanges[i].top - (chunkSize / 2))) {
          localTo = _initRanges[i].top;
        }
      }

      if (localFrom == (uint8_t *)MM_AtomicOperations::lockCompareExchange(
                           (volatile uintptr_t *)&(_initRanges[i].current),
                           (uintptr_t)localFrom, (uintptr_t)localTo)) {
        /* Got the range so return details */
        *from = localFrom;
        *to = localTo;
        *type = _initRanges[i].type;
        *concurrentCollectable =
            (_initRanges[i].subspace)->isConcurrentCollectable() ? true : false;

        Trc_MM_ConcurrentGC_getInitRange_Succeed(
            env->getLanguageVMThread(), *from, *to, *type,
            *concurrentCollectable ? "true" : "false");
      ...
      oldIndex = i;
      newIndex = oldIndex + 1;
      /* Dont care if this fails, it just means another thread someone beat us
       * to it */
      MM_AtomicOperations::lockCompareExchangeU32(&_nextInitRange, oldIndex,
                                                  newIndex);
      /* ..and refresh local cached value, may have been chnaged by other
       * threads */
      i = (uint32_t)_nextInitRange;
    }
  }
 ...
 */
bool MM_ConcurrentGC::tracingRateDropped(MM_EnvironmentBase *env) {
  // Always return false for now until we understand how to fix this code to
  // ensure we do not prematurely report that the rate has dropped and so start
  // card cleaning too early.
  return false;
...
MM_ConcurrentGC::ConHelperRequest MM_ConcurrentGC::switchConHelperRequest(
    ConHelperRequest from, ConHelperRequest to) {
...
MMINLINE MM_ConcurrentGC::ConHelperRequest MM_ConcurrentGC::getConHelperRequest(
    MM_EnvironmentBase *env) {
...
void MM_ConcurrentGC::conHelperEntryPoint(OMR_VMThread *omrThread,
                                          uintptr_t slaveID) {
  ...
  while (CONCURRENT_HELPER_SHUTDOWN != request) {
    ...
    Assert_GC_true_with_message(env,
                                CONCURRENT_OFF != _stats.getExecutionMode(),
    ...
    while ((CONCURRENT_HELPER_MARK == request) &&
           ...
           !_cardTable->isCardCleaningComplete() && spinLimiter.spin()) {
    ...
    if (CONCURRENT_HELPER_MARK == request) {
      request = switchConHelperRequest(CONCURRENT_HELPER_MARK,
                                       CONCURRENT_HELPER_WAIT);
    }
 ...
 *
 */
void MM_ConcurrentGC::shutdownAndExitConHelperThread(OMR_VMThread *omrThread) {
  MM_EnvironmentBase::detachVMThread(
      _extensions->getOmrVM(), omrThread,
      MM_EnvironmentBase::ATTACH_GC_HELPER_THREAD);
  omrthread_monitor_enter(_conHelpersActivationMonitor);
 ...
 * Attach the required number of concurrent helper threads which are attached
 * with minimum priority so they do not compete with applciation threads for use
 * of procesors; they are only intended to use up any spare cycles not used by
 * application threads.
 *
 * @return TRUE if concurrent helper thread attached and initailzed OK;
 * FALSE otherwise
 */
bool MM_ConcurrentGC::initializeConcurrentHelpers(
  ...
  for (conHelperThreadCount = 0; conHelperThreadCount < _conHelperThreads;
       conHelperThreadCount++) {
    conHelperThreadInfo.threadFlags = 0;
    conHelperThreadInfo.threadID = conHelperThreadCount;
    conHelperThreadInfo.collector = this;

    threadForkResult = createThreadWithCategory(
        &(_conHelpersTable[conHelperThreadCount]), OMR_OS_STACK_SIZE,
        J9THREAD_PRIORITY_MIN, 0, con_helper_thread_proc,
        (void *)&conHelperThreadInfo, J9THREAD_CATEGORY_SYSTEM_GC_THREAD);
 ...
 */
void MM_ConcurrentGC::shutdownConHelperThreads(
    MM_GCExtensionsBase *extensions) {
 ...
 *
 */
void MM_ConcurrentGC::resumeConHelperThreads(MM_EnvironmentBase *env) {
  if (_conHelpersStarted > 0) {
    omrthread_monitor_enter(_conHelpersActivationMonitor);
    if (!env->isExclusiveAccessRequestWaiting()) {
 ...
 * card cleaning) will need to be performed during the next concurrent mark
 * cycle.
 */
void MM_ConcurrentGC::tuneToHeap(MM_EnvironmentBase *env) {
  ...
  if (0 == _stats.getKickoffThreshold() || _retuneAfterHeapResize) {
    totalBytesToTrace = (uintptr_t)(heapSize * _tenureLiveObjectFactor *
                                    _tenureNonLeafObjectFactor);
    _bytesToTracePass1 =
        (uintptr_t)((float)totalBytesToTrace * _bytesTracedInPass1Factor);
    _bytesToTracePass2 =
        MM_Math::saturatingSubtract(totalBytesToTrace, _bytesToTracePass1);
    _bytesToCleanPass1 =
        (uintptr_t)((float)totalBytesToTrace * _cardCleaningFactorPass1);
    _bytesToCleanPass2 =
        (uintptr_t)((float)totalBytesToTrace * _cardCleaningFactorPass2);
    _retuneAfterHeapResize =
        false; /* just in case we have a resize before first concurrent cycle */
  } else {
    /* Re-tune based on actual amount traced if we completed tracing on last
     * cycle */
    if ((NULL == env->_cycleState) ||
        env->_cycleState->_gcCode.isExplicitGC() || !_stwCollectionInProgress) {
      /* Nothing to do - we can't update statistics on a system GC or when no
       * cycle is running */
    } else if (CONCURRENT_EXHAUSTED <= _stats.getExecutionModeAtGC()) {
      uintptr_t totalTraced =
          _stats.getTraceSizeCount() + _stats.getConHelperTraceSizeCount();
      uintptr_t totalCleaned =
          _stats.getCardCleanCount() + _stats.getConHelperCardCleanCount();

      if (_secondCardCleanPass) {
        assume0(_totalCleanedAtPass2KO != HIGH_VALUES);
        uintptr_t bytesTracedPass1 = _totalTracedAtPass2KO;
        uintptr_t bytesTracedPass2 =
            MM_Math::saturatingSubtract(totalTraced, _totalTracedAtPass2KO);
        uintptr_t bytesCleanedPass1 = _totalCleanedAtPass2KO;
        uintptr_t bytesCleanedPass2 =
            MM_Math::saturatingSubtract(totalCleaned, _totalCleanedAtPass2KO);

        _bytesToTracePass1 = (uintptr_t)MM_Math::weightedAverage(
            (float)_bytesToTracePass1, (float)bytesTracedPass1,
            LIVE_PART_HISTORY_WEIGHT);
        _bytesToTracePass2 = (uintptr_t)MM_Math::weightedAverage(
            (float)_bytesToTracePass2, (float)bytesTracedPass2,
            LIVE_PART_HISTORY_WEIGHT);
        _bytesToCleanPass1 = (uintptr_t)MM_Math::weightedAverage(
            (float)_bytesToCleanPass1, (float)bytesCleanedPass1,
            CARD_CLEANING_HISTORY_WEIGHT);
        _bytesToCleanPass2 = (uintptr_t)MM_Math::weightedAverage(
            (float)_bytesToCleanPass2, (float)bytesCleanedPass2,
            CARD_CLEANING_HISTORY_WEIGHT);
      } else {
        _bytesToTracePass1 = (uintptr_t)MM_Math::weightedAverage(
            (float)_bytesToTracePass1, (float)totalTraced,
            LIVE_PART_HISTORY_WEIGHT);
        _bytesToCleanPass1 = (uintptr_t)MM_Math::weightedAverage(
            (float)_bytesToCleanPass1, (float)totalCleaned,
            CARD_CLEANING_HISTORY_WEIGHT);
        _bytesToTracePass2 = 0;
        _bytesToCleanPass2 = 0;
      }
    } else if (_stats.getExecutionModeAtGC() == CONCURRENT_CLEAN_TRACE) {
      /* Assume amount to be traced on next cycle will what we traced this time
       * PLUS the tracing we did to complete processing of any work packets that
       * remained at the start of the collection PLUS tracing done during final
       * card cleaning. This is an over estimate but will get us back on track.
       */
      totalBytesToTrace =
          _stats.getTraceSizeCount() + _stats.getConHelperTraceSizeCount() +
          _stats.getCompleteTracingCount() + _stats.getFinalTraceCount();
      ...
      if (_secondCardCleanPass) {
        ...
        _bytesToCleanPass2 =
            MM_Math::saturatingSubtract(totalBytesToClean, _bytesToCleanPass1);
      } else {
      ...
      /* We did not trace enough to use amount traced to predict trace target so
       * best we can do is estimate based on current heap size, live object
       * factor, leaf object factor etc.
       */
      totalBytesToTrace = (uintptr_t)(heapSize * _tenureLiveObjectFactor *
                                      _tenureNonLeafObjectFactor);
      _bytesToTracePass1 =
  ...
  if (_rebuildInitWorkForAdd || _rebuildInitWorkForRemove) {
    if (_extensions->isConcurrentScavengerInProgress()) {
      ...
      /* Hold _initWorkMonitor to prevent any thread from
       * starting/joining/leaving init work, since we are about to change the
       * _initRange table (that initializer threads consume) */
      omrthread_monitor_enter(_initWorkMonitor);

      /* If there are no active initializers, we are safe to proceed and modify
       * _initRagne table, otherwise skip it (rebuild flag will remain set, and
       * the table will be reconfigurated at a later, safe point (like beginning
       * of STW phase). Even if happens to be 0 initializers, but we are still
       * doing the init, skip it. It is safe for heap expansion (caller of this
       * method already did initialization for that  part of the memory), and
       * more importantly it will not trigger unncessary
       * redo of the init work, if we were to reset the able in a middle of init
       * phase. */
      if ((0 == _initializers) &&
          (_stats.getExecutionMode() != CONCURRENT_INIT_RUNNING)) {
  ...
  /*
   * Trace target is simply a sum of all work we predict we need to complete
   * marking. Initialization work is accounted for separately
   */
  ...
  /* Calculate the KO point for concurrent. As we trace at different rates
   * during the initialization and marking phases we need to allow for that when
   * calculating the KO point.
   */
  kickoffThreshold =
      (_stats.getInitWorkRequired() / _allocToInitRate) +
      (_traceTargetPass1 / _allocToTraceRateNormal) +
      (_traceTargetPass2 /
       (_allocToTraceRateNormal * _allocToTraceRateCardCleanPass2Boost));

  /* Determine card cleaning thresholds */
  cardCleaningThreshold =
      ((uintptr_t)((float)kickoffThreshold / _cardCleaningThresholdFactor));

  /* We need to ensure that we complete tracing just before we run out of
   * storage otherwise we will more than likely get an AF whilst last few
   * allocates are paying finishing off the last bit of tracing. So we create a
   * buffer zone by bringing forward the KO threshold. We remember by how much
   * so we can make the necessary adjustments to calculations in
   * calculateTraceSize().
   *
   * Two factors are applied to increase the thresholds:
   *  1) a boost factor (10% of the kickoff threshold) is applied to both
   * thresholds 2) a user-specified slack value (in bytes) is added
   * proportionally to each threshold
   *
   * e.g. if kickoffThreshold = 10M, cardCleaningThreshold = 2M and
   * concurrentSlack = 100M 1) the boost will be 1M (10% of 10M) 2) the kickoff
   * slack will be 100M 3) the cardcleaning slack will be 20M (100M * (10M /
   * 2M)) resulting in a final kickoffThreshold = 111M and a
   * cardCleaningThreshold = 23M
   */
  float boost = ((float)kickoffThreshold * CONCURRENT_KICKOFF_THRESHOLD_BOOST) -
                (float)kickoffThreshold;
  float kickoffProportion = 1.0;
  float cardCleaningProportion =
      (float)cardCleaningThreshold / (float)kickoffThreshold;

  kickoffThresholdPlusBuffer =
      (uintptr_t)((float)kickoffThreshold + boost +
                  ((float)_extensions->concurrentSlack * kickoffProportion));
  _stats.setKickoffThreshold(kickoffThresholdPlusBuffer);
  _stats.setCardCleaningThreshold((uintptr_t)(
      (float)cardCleaningThreshold + boost +
      ((float)_extensions->concurrentSlack * cardCleaningProportion)));
  _kickoffThresholdBuffer =
      MM_Math::saturatingSubtract(kickoffThresholdPlusBuffer, kickoffThreshold);

  if (_extensions->debugConcurrentMark) {
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
    omrtty_printf(
        "Tune to heap : Trace target Pass 1=\"%zu\" (Trace=\"%zu\" "
        "Clean=\"%zu\")\n",
        _traceTargetPass1, _bytesToTracePass1, _bytesToCleanPass1);
    omrtty_printf(
        "               Trace target Pass 2=\"%zu\" (Trace=\"%zu\" "
        "Clean=\"%zu\")\n",
        _traceTargetPass2, _bytesToTracePass2, _bytesToCleanPass2);
    omrtty_printf(
        "               KO threshold=\"%zu\" KO threshold buffer=\"%zu\"\n",
  ...
  Trc_MM_ConcurrentGC_tuneToHeap_Exit2(
      env->getLanguageVMThread(), _stats.getTraceSizeTarget(),
      _stats.getInitWorkRequired(), _stats.getKickoffThreshold());
}
 ...
 */
void MM_ConcurrentGC::adjustTraceTarget() {
  uintptr_t newTraceTarget, totalBytesToTrace;
  MM_Heap *heap = (MM_Heap *)_extensions->heap;
  uintptr_t heapSize = heap->getActiveMemorySize(MEMORY_TYPE_OLD);

  /* Reset bytes to trace and clean based on new heap size and the average live
   * rate */
  totalBytesToTrace = (uintptr_t)(heapSize * _tenureLiveObjectFactor *
                                  _tenureNonLeafObjectFactor);
  _bytesToTracePass1 =
  ...
  _bytesToCleanPass1 =
      (uintptr_t)((float)_bytesToTracePass1 * _cardCleaningFactorPass1);
  _bytesToCleanPass2 =
      (uintptr_t)((float)_bytesToTracePass2 * _cardCleaningFactorPass2);

  /* Calculate new trace target */
  newTraceTarget = _bytesToTracePass1 + _bytesToTracePass2 +
                   _bytesToCleanPass1 + _bytesToCleanPass2;
 ...
 *
 * Calculate the proportion of live objects in heap and the proportion of
 * non-leaf objects in the heap at the end of this collection. These factors are
 * used if the heap changes in size to predict how much of new heap will need to
 * be traced before next collection.
 *
 * Also, reset the card cleaning factors based on cards we cleaned this cycle.
 *
 */
void MM_ConcurrentGC::updateTuningStatistics(MM_EnvironmentBase *env) {
  /* Dont update statistics if its a system GC or we aborted a concurrent
   * collection cycle
   */
  if (env->_cycleState->_gcCode.isExplicitGC() ||
      (CONCURRENT_TRACE_ONLY > _stats.getExecutionModeAtGC())) {
    return;
  } else {
    ...
    uintptr_t heapSize = heap->getActiveMemorySize(MEMORY_TYPE_OLD);
    uintptr_t freeSize =
        heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_OLD);
    ...
    _tenureLiveObjectFactor = MM_Math::weightedAverage(
        _tenureLiveObjectFactor, newLiveObjectFactor, LIVE_PART_HISTORY_WEIGHT);
    ...
    /* If we did not complete mark add in those we missed. This will be an over
     * estimate but will get us back on track
     */
    ...
    _tenureNonLeafObjectFactor = MM_Math::weightedAverage(
        _tenureNonLeafObjectFactor, newNonLeafObjectFactor,
        NON_LEAF_HISTORY_WEIGHT);
    ...
    switch (executionModeAtGC) {
      ...
      case CONCURRENT_TRACE_ONLY:
        _cardCleaningFactorPass1 = MM_Math::weightedAverage(
            _cardCleaningFactorPass1, _maxCardCleaningFactorPass1,
            CARD_CLEANING_HISTORY_WEIGHT);
        _cardCleaningFactorPass2 = MM_Math::weightedAverage(
            _cardCleaningFactorPass2, _maxCardCleaningFactorPass2,
            CARD_CLEANING_HISTORY_WEIGHT);
        _bytesTracedInPass1Factor = MM_Math::weightedAverage(
            _bytesTracedInPass1Factor, ALL_BYTES_TRACED_IN_PASS_1,
            BYTES_TRACED_IN_PASS_1_HISTORY_WEIGHT);
        break;
      ...
      case CONCURRENT_FINAL_COLLECTION:
        ...
        if (_secondCardCleanPass) {
          assume0(_totalTracedAtPass2KO != HIGH_VALUES &&
                  _totalCleanedAtPass2KO != HIGH_VALUES);
        ...
        /* What factor of the tracing work was done beofre we started 2nd pass
         * of card cleaning ?*/
        newBytesTracedInPass1Factor =
            (float)totalTracedPass1 / (float)totalTraced;
        newCardCleaningFactorPass1 =
            (float)totalCleanedPass1 / (float)totalTraced;
        newCardCleaningFactorPass1 =
            OMR_MIN(newCardCleaningFactorPass1, _maxCardCleaningFactorPass1);
        _cardCleaningFactorPass1 = MM_Math::weightedAverage(
            _cardCleaningFactorPass1, newCardCleaningFactorPass1,
            CARD_CLEANING_HISTORY_WEIGHT);
        _bytesTracedInPass1Factor = MM_Math::weightedAverage(
            _bytesTracedInPass1Factor, newBytesTracedInPass1Factor,
            BYTES_TRACED_IN_PASS_1_HISTORY_WEIGHT);

        /* Only update pass 2 average if we did 2 passes */
        if (_secondCardCleanPass) {
          newCardCleaningFactorPass2 =
              (float)totalCleanedPass2 / (float)totalTraced;
          newCardCleaningFactorPass2 =
              OMR_MIN(newCardCleaningFactorPass2, _maxCardCleaningFactorPass2);
          _cardCleaningFactorPass2 = MM_Math::weightedAverage(
              _cardCleaningFactorPass2, newCardCleaningFactorPass2,
              CARD_CLEANING_HISTORY_WEIGHT);
        }
      ...
      default:
        Assert_GC_true_with_message(
            env,
            (CONCURRENT_ROOT_TRACING <= executionModeAtGC) &&
                (CONCURRENT_TRACE_ONLY > executionModeAtGC),
            "MM_ConcurrentStats::_executionModeAtGC = %zu\n",
            executionModeAtGC);
        _cardCleaningFactorPass1 = MM_Math::weightedAverage(
            _cardCleaningFactorPass1, _maxCardCleaningFactorPass1,
            CARD_CLEANING_HISTORY_WEIGHT);
    ...
    if (_extensions->debugConcurrentMark) {
      ...
      omrtty_printf(
          "Update tuning statistics: Total Traced=\"%zu\" (Pass 2 KO=\"%zu\")  "
          "Total Cleaned=\"%zu\" (Pass 2 KO=\"%zu\")\n",
          totalTraced, _totalTracedAtPass2KO, totalCleaned,
          _totalCleanedAtPass2KO);
      omrtty_printf(
          "                          Tenure Live object Factor=\"%.3f\" Tenure "
          "non-leaf object factor=\"%.3f\" \n",
          _tenureLiveObjectFactor, _tenureNonLeafObjectFactor);
      omrtty_printf(
          "                          Card Cleaning Factors: Pass1=\"%s\" "
          "Pass2=\"%s\"\n",
          pass1Factor, pass2Factor);
      omrtty_printf(
          "                          Bytes traced in Pass 1 Factor=\"%.3f\"\n",
 ...
 * Calculate how much initialization work should be done by a mutator for the
 * current allocation request. We dont need to worry about work being available
 * at the right time as we do when tracing. Initialization work will either be
 * available or it will be all allocated; one exhausted no more will arrive at a
 * later time as is the case with tracing. We boost the target trace rate during
 * initialization in order to minimize the time to complete the initialization
 * phase.
 *
 * @param allocationSize  the size of the allocation
 * @return the allocation tax
 */
uintptr_t MM_ConcurrentGC::calculateInitSize(MM_EnvironmentBase *env,
                                             uintptr_t allocationSize) {
  /* During initialization we boost the target trace rate in order to complete
   * init ASAP */
 ...
 * @return the allocation tax
 */
uintptr_t MM_ConcurrentGC::calculateTraceSize(
...
#endif /* OMR_GC_MODRON_SCAVENGER */
  {
    ...
    /* in the case of Tarok phase4, the pool from the allocation description
     * doesn't have enough context to help guide concurrent decisions so we need
     * the parent */
   ...
   * so that the trace rate calculation works out. Basically we are attempting
   * to ensure that all work is assigned to mutators by the time free space
   * drops to _kickoffThresholdBuffer.
   */
  remainingFree = (remainingFree > _kickoffThresholdBuffer)
                      ? remainingFree - _kickoffThresholdBuffer
                      : 0;
  ...
  traceTarget =
      _pass2Started ? _traceTargetPass1 + _traceTargetPass2 : _traceTargetPass1;

  /* Provided we are not into buffer zone already and we have not already done
   * more work than we predicted calculate required trace rate for this allocate
   * request to keep us on track.
   */
  if ((remainingFree > 0) && (workCompleteSoFar < traceTarget)) {
    thisTraceRate =
        (float)((traceTarget - workCompleteSoFar) / (float)(remainingFree));

    if (thisTraceRate > _allocToTraceRate) {
      /* The "over tracing" should not only adjust to the current ratio between
       * free space and estimated remaining tracing, but also try to do even
       * more tracing, in order to correct the ratio back to the required alloc
       * to trace rate.
       */
      thisTraceRate +=
          ((thisTraceRate - _allocToTraceRate) * OVER_TRACING_BOOST_FACTOR);
    ...
    if (thisTraceRate > _alloc2ConHelperTraceRate) {
      /* ..calculate tax for mutator taking into account any tracing being done
       * by concurrent helpers */
      sizeToTrace = (uintptr_t)(allocationSize *
                                (thisTraceRate - _alloc2ConHelperTraceRate));
    } else {
      /* Background thread is doing enough tracing on its own so mutator gets
       * away without paying any tax */
      sizeToTrace = 0;
    }
  } else {
    /* We are already in buffer zone of free space or we have already done all
     * work we predicted and we are still not done. We run a high risk of
     * hitting AF before we do complete so trace at the max from now on in an
     * attempt to complete the work before we do hit AF.
     */
 ...
 */
bool MM_ConcurrentGC::periodicalTuningNeeded(MM_EnvironmentBase *env,
                                             uintptr_t freeSize) {
  /* Is it time to update statistics. If _lastFreeSize <= freeSize another
   * thread has already updated statistics for this interval
   */
 ...
 *
 * @return  Number of bytes available for allocation before old area is
 * exhausted
 */
MMINLINE uintptr_t MM_ConcurrentGC::potentialFreeSpace(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription) {
  uintptr_t nurseryPromotion, nurseryInitialFree, currentOldFree,
      currentNurseryFree;
  MM_MemorySpace *memorySpace =
      env->getExtensions()->heap->getDefaultMemorySpace();
  ...
  /* Have we done at least 1 scavenge ? If not no statistics available so return
   * high values */
  ...
  nurseryPromotion =
      scavengerStats->_avgTenureBytes == 0
          ? 1
          : (uintptr_t)(scavengerStats->_avgTenureBytes +
                        (env->getExtensions()->tenureBytesDeviationBoost *
                         scavengerStats->_avgTenureBytesDeviation));

#if defined(OMR_GC_LARGE_OBJECT_AREA)
  /* Do we need to tax this allocation ? */
  if (LOA == _meteringType) {
    nurseryPromotion = scavengerStats->_avgTenureLOABytes == 0
                           ? 1
                           : scavengerStats->_avgTenureLOABytes;
    currentOldFree = oldSubspace->getApproximateActiveFreeLOAMemorySize();
    headRoom = (uintptr_t)(_extensions->concurrentKickoffTenuringHeadroom *
                           _extensions->lastGlobalGCFreeBytesLOA);
  } else {
    assume0(SOA == _meteringType);
    currentOldFree = oldSubspace->getApproximateActiveFreeMemorySize() -
                     oldSubspace->getApproximateActiveFreeLOAMemorySize();
    headRoom = (uintptr_t)(_extensions->concurrentKickoffTenuringHeadroom *
                           (_extensions->getLastGlobalGCFreeBytes() -
                            _extensions->lastGlobalGCFreeBytesLOA));
  }
#else
  currentOldFree = oldSubspace->getApproximateActiveFreeMemorySize();
  ...
  MM_LargeObjectAllocateStats *stats =
      oldSubspace->getMemoryPool()->getLargeObjectAllocateStats();
  if (NULL != stats) {
    uintptr_t fragmentation = (uintptr_t)(
        (env->getExtensions())->concurrentSlackFragmentationAdjustmentWeight *
        stats->getRemainingFreeMemoryAfterEstimate());
   ...
   * fill the old space. If we know next scavenge will percolate then we have no
   * scavenges remaining before concurren KO is required
   */
  ...
  /*
   * The headroom remaining will be the maximum value between 1 , and the
   * current headroom calculation. We want a worst case headroom, so that we
   * avoid percolate. By having a higher estimate of headroom, we will have less
   * scavenges remaining, and will start concurrent mark earlier
   */
  ...
  scavengesRemaining = MM_Math::saturatingSubtract(
      scavengesRemaining, (uintptr_t)scavengesRemainingHeadroom);

  /* Now calculate how many bytes we can therefore allocate in the nursery
   * before we will fill the tenure area. On 32 bit platforms this can be
   * greater than 4G hence the 64 bit maths. We assume that the heap will not be
   * big enough on 64 bit platforms to overflow 64 bits!
   */

  uint64_t potentialFree =
      (uint64_t)currentNurseryFree +
      ((uint64_t)nurseryInitialFree * (uint64_t)scavengesRemaining);

#if !defined(OMR_ENV_DATA64)
  /* On a 32 bit platforms the amount of free space could be more than 4G.
   * Therefore if the amount of potential free space is greater than can be
   * expressed in 32 bits we just return 4G.
   */
 ...
 * Determine how much work the concurrent helper has got through in last
 * interval and adjust the counters.
 *
 * @param  freeSize the current amount of free space in the old area
 */
void MM_ConcurrentGC::periodicalTuning(MM_EnvironmentBase *env,
                                       uintptr_t freeSize) {
  ...
  if (_lastFreeSize == LAST_FREE_SIZE_NEEDS_INITIALIZING) {
    _lastFreeSize = freeSize;
    _tuningUpdateInterval =
        (uintptr_t)((float)freeSize * TUNING_HEAP_SIZE_FACTOR);
  ...
  } else if ((_lastFreeSize > freeSize) &&
             (_lastFreeSize - freeSize) >= _tuningUpdateInterval) {
    /* This thread first to update for this interval so calculate
     * total traced so far
     */
    uintptr_t totalTraced =
        _stats.getTraceSizeCount() + _stats.getCardCleanCount();
    uintptr_t freeSpaceUsed = _lastFreeSize - freeSize;

    /* Update concurrent helper trace rate if we have any */
    if (_conHelpersStarted > 0) {
      uintptr_t conTraced = _stats.getConHelperTraceSizeCount() +
                            _stats.getConHelperCardCleanCount();
      newConHelperRate = (float)(conTraced - _lastConHelperTraceSizeCount) /
                         (float)(freeSpaceUsed);
      _lastConHelperTraceSizeCount = conTraced;
      _alloc2ConHelperTraceRate =
          MM_Math::weightedAverage(_alloc2ConHelperTraceRate, newConHelperRate,
                                   CONCURRENT_HELPER_HISTORY_WEIGHT);

      totalTraced += conTraced;
    }

    _lastAverageAlloc2TraceRate =
        ((float)(totalTraced - _lastTotalTraced)) / (float)freeSpaceUsed;
...
/**
 * Start card cleaning.
 * Either free space has reached the card cleaning kickoff level or we have run
 * out of other work so may as well start card cleaning early.
 *
 */
void MM_ConcurrentGC::kickoffCardCleaning(MM_EnvironmentBase *env,
                                          ConcurrentCardCleaningReason reason) {
  /* Switch to CONCURRENT_CLEAN_TRACE...if we fail someone beat us to it */
  if (_stats.switchExecutionMode(CONCURRENT_TRACE_ONLY,
 ...
 * The given pool was unable to satisfy an allocation request of (at least) the
 * given size.  See if there is work that can be done to increase the free
 * stores of the pool so that the request can be met.
 * @note This call is made under the pools allocation lock (or equivalent)
 * @return True if the pool was replenished with a free entry that can satisfy
 * the size, false otherwise.
 */
bool MM_ConcurrentGC::replenishPoolForAllocate(MM_EnvironmentBase *env,
 ...
 */
void MM_ConcurrentGC::payAllocationTax(
 ...
 * @note This is a potential GC point.
 */
void MM_ConcurrentGC::concurrentMark(MM_EnvironmentBase *env,
  ...
  /* Boost priority of thread whilst we are paying our tax; dont want low
   * priority application threads holding on to work packets
   */
  ...
  while (!taxPaid) {
    ...
    switch (executionMode) {
      ...
      case CONCURRENT_CLEAN_TRACE:
        ...
        if (sizeToTrace > 0) {
          sizeTraced = doConcurrentTrace(env, allocDescription, sizeToTrace,
                                         subspace, threadAtSafePoint);
        }
      ...
      case CONCURRENT_INIT_COMPLETE:
        if (_extensions->optimizeConcurrentWB) {
          ...
          /* TODO: Once optimizeConcurrentWB enabled by default this code will
           * be deleted */
          _stats.switchExecutionMode(CONCURRENT_INIT_COMPLETE,
                                     CONCURRENT_ROOT_TRACING);
        }
        break;

      case CONCURRENT_EXHAUSTED:
        /* If another thread has set exhausted whilst we were allocating then we
         * can trigger the final collection here provided we are at a safe
         * point. If not then GC will be triggered by next allocation.
         */
        if (threadAtSafePoint) {
      ...
      case CONCURRENT_ROOT_TRACING:
        nextExecutionMode =
            _concurrentDelegate.getNextTracingMode(CONCURRENT_ROOT_TRACING);
        Assert_GC_true_with_message(
            env,
            (CONCURRENT_ROOT_TRACING < nextExecutionMode) ||
                (CONCURRENT_TRACE_ONLY == nextExecutionMode),
            "MM_ConcurrentMarkingDelegate::getNextTracingMode(CONCURRENT_ROOT_"
            "TRACING) = %zu\n",
            nextExecutionMode);
        if (_stats.switchExecutionMode(CONCURRENT_ROOT_TRACING,
                                       nextExecutionMode)) {
      ...
      default:
        /* Client language defines 1 or more execution modes with values >
         * CONCURRENT_ROOT_TRACING */
        Assert_GC_true_with_message(
            env,
            (CONCURRENT_ROOT_TRACING < executionMode) &&
                (CONCURRENT_TRACE_ONLY > executionMode),
            "MM_ConcurrentStats::_executionMode = %zu\n", executionMode);
        nextExecutionMode =
            _concurrentDelegate.getNextTracingMode(executionMode);
        if (_stats.switchExecutionMode(executionMode, nextExecutionMode)) {
          Assert_GC_true_with_message2(
              ...
              (CONCURRENT_ROOT_TRACING < nextExecutionMode) &&
                  (CONCURRENT_TRACE_ONLY >= nextExecutionMode),
              "MM_ConcurrentStats::_executionMode = %zu; "
              "MM_ConcurrentMarkingDelegate::getNextTracingMode(MM_"
              "ConcurrentStats::_executionMode) = %zu\n",
              executionMode, nextExecutionMode);
          /* Collect some roots */
          bool collectedRoots = false;
          _concurrentDelegate.collectRoots(env, executionMode, &collectedRoots,
                                           &taxPaid);
          if (collectedRoots) {
            /* Resume concurrent helper threads to help mark any roots we have
             * found. Another CONCURRENT_ROOT_TRACING thread may have beat us to
             * it but if thread already active this notify will be ignored
             */
...
void MM_ConcurrentGC::signalThreadsToActivateWriteBarrier(
    MM_EnvironmentBase *env) {
  ...
  while (CONCURRENT_INIT_COMPLETE == _stats.getExecutionMode()) {
    /* We may or may not have exclusive access but another thread may have beat
     * us to it and prepared the threads or even collected.
     */
    if (env->acquireExclusiveVMAccessForGC(this, true, false)) {
      ...
      _stats.switchExecutionMode(CONCURRENT_INIT_COMPLETE,
                                 CONCURRENT_ROOT_TRACING);
      /* Cancel any outstanding call backs on other threads as this thread has
 ...
 */
bool MM_ConcurrentGC::forceKickoff(MM_EnvironmentBase *env,
                                   MM_MemorySubSpace *subSpace,
                                   MM_AllocateDescription *allocDescription,
                                   uint32_t gcCode) {
  if (_extensions->concurrentKickoffEnabled &&
      _concurrentDelegate.canForceConcurrentKickoff(env, gcCode,
                                                    &_languageKickoffReason)) {
 ...
 */
bool MM_ConcurrentGC::timeToKickoffConcurrent(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription) {
 ...
 */
void MM_ConcurrentGC::concurrentSweep(
    MM_EnvironmentBase *env, MM_MemorySubSpace *subspace,
    MM_AllocateDescription *allocDescription) {
  uintptr_t oldVMstate = env->pushVMstate(OMRVMSTATE_GC_CONCURRENT_SWEEP);
  ((MM_ConcurrentSweepScheme *)_sweepScheme)
      ->payAllocationTax(env, subspace, allocDescription);
  env->popVMstate(oldVMstate);
}
 ...
 */
void MM_ConcurrentGC::completeConcurrentSweep(MM_EnvironmentBase *env) {
  /* If concurrent sweep is not enabled, do nothing */
  if (!_extensions->concurrentSweep) {
    return;
  }

  MM_ConcurrentSweepScheme *concurrentSweep =
      (MM_ConcurrentSweepScheme *)_sweepScheme;

  /* If concurrent sweep was not on this cycle, or has already been completed,
   * do nothing */
 ...
 * Finish all concurrent sweep activities.
 */
void MM_ConcurrentGC::completeConcurrentSweepForKickoff(
    MM_EnvironmentBase *env) {
  /* If concurrent sweep is not enabled, do nothing */
  if (!_extensions->concurrentSweep) {
    return;
 ...
 *
 * @param initToDo  The amount of initialization work, in bytes, this mutator
 * has been taxed to do.
 *
 * @return the amount of initialization work actually done
 */
uintptr_t MM_ConcurrentGC::doConcurrentInitialization(MM_EnvironmentBase *env,
                                                      uintptr_t initToDo) {
      ...
      switch (type) {
        case MARK_BITS:
          if (concurrentCollectable) {
            ...
            initDone +=
                _markingScheme->setMarkBitsInRange(env, from, to, false);
          }
          break;
        case CARD_TABLE:
          if (NULL != _cardTable) {
            initDone += ((MM_ConcurrentCardTable *)_cardTable)
                            ->clearCardsInRange(env, from, to);
          }
          break;
        default:
          assume0(0);
  ...
  if (_initializers > 0) {
    ...
    if (initDone < initToDo && !env->isExclusiveAccessRequestWaiting()) {
      /* Yes..so block here and wait for master thread to finish and update
       * execution mode
       */
      omrthread_monitor_enter(_initWorkCompleteMonitor);
      omrthread_monitor_exit(_initWorkMonitor);
    ...
    if (allInitRangesProcessed()) {
      ...
      _stats.switchExecutionMode(CONCURRENT_INIT_RUNNING,
 ...
 * Mutator pays its allocation tax by tracing references until tax paid of no
 * more tracing work to do.
 *
 * @param sizeToTrace  The amount of tracing work, in bytes, this mutator has
 * left to do.
 ...
 */
uintptr_t MM_ConcurrentGC::doConcurrentTrace(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
    uintptr_t sizeToTrace, MM_MemorySubSpace *subspace,
  ...
  if ((CONCURRENT_TRACE_ONLY == _stats.getExecutionMode()) &&
      (remainingFree < _stats.getCardCleaningThreshold())) {
    kickoffCardCleaning(env, CARD_CLEANING_THRESHOLD_REACHED);
  }

  uintptr_t bytesTraced = 0;
  bool completedConcurrentScanning = false;
  if (_concurrentDelegate.startConcurrentScanning(
          env, &bytesTraced, &completedConcurrentScanning)) {
  ...
  /* Scan this threads stack, if it hasn't been done already. We cant do so if
   * this is a TLH allocation as we are not at a safe point.
   */
  if (!env->isThreadScanned() && threadAtSafePoint) {
    /* If call back is late, i.e after the collect,  then ignore it */
    /* CMVC 119942 : add a top range of CONCURRENT_EXHAUSTED so that we don't
     * scan threads at the last second and report them as being scanned when we
     * haven't actually had a chance to follow any of the references
     */
    ...
    if ((CONCURRENT_ROOT_TRACING <= mode) && (CONCURRENT_EXHAUSTED > mode)) {
      if (_concurrentDelegate.scanThreadRoots(env)) {
        ...
        /* Resume concurrent helper threads to help mark any roots we have
         * found. If thread already active this notify will be ignored
         */
        resumeConHelperThreads(env);
      }
    }
  ...
  /* Loop marking and cleaning cards until we have paid all our tax, another
   * thread requests exclusive VM access to do a gc, or another thread has
   * swithed to exhausted
   */
  while (!env->isExclusiveAccessRequestWaiting() &&
         (sizeTraced < sizeToTrace) && (sizeTraced != sizeTracedPreviously) &&
         (CONCURRENT_CLEAN_TRACE >= _stats.getExecutionMode())) {
    /* CMVC 131721 - record the amount traced up until now. If any iteration of
     * this loop fails to increase the sizeTraced, break out of the loop and
     * return to mutating
     */
    ...
    if (!env->isExclusiveAccessRequestWaiting() && sizeTraced < sizeToTrace) {
      ...
      if (CONCURRENT_TRACE_ONLY == _stats.getExecutionMode()) {
        ...
        if ((_markingScheme->getWorkPackets()->tracingExhausted() ||
             tracingRateDropped(env)) &&
            _stats.isRootTracingComplete()) {
      ...
      if (CONCURRENT_CLEAN_TRACE == _stats.getExecutionMode()) {
        if (!((MM_ConcurrentCardTable *)_cardTable)->isCardCleaningComplete()) {
          ...
          if (cleanCards(env, true, (sizeToTrace - sizeTraced), &bytesCleaned,
                         threadAtSafePoint)) {
            if (bytesCleaned > 0) {
              /* Update global count */
          ...
          /* We have completed card cleaning and currently have nothing to
           * trace. If we have not scanned this thread yet (and this should be a
           * very rare event) then break out of tracing loop and return.
           * Hopefully this will allow the thread to get to and async message
           * check and so call us back to get the thread scanned.
           */
          if (!env->isThreadScanned()) {
            ...
            if (_markingScheme->getWorkPackets()->tracingExhausted()) {
              ...
              /*
               * Tracing is not yet exhausted; some packets are held by other
               * threads. If we get here we have finished all card cleaning and
               * cannot get an input packet so we must be near completing the
               * tracing. However, the lower priority concurrent helpers could
               * be suspended and hanging onto packets and so preventing us from
               * getting to CONCURENT_EXHAUSTED. This will delay the final
               * collection; maybe until an AF which we don't want. We need to
               * tell the concurrent helpers to release any packets they have so
               * we can get them processed. This is easily achieved by
               * suspending them. Anyway, now is also a good time to tell the
               * concurrent helpers to park themselves for the imminent global
               * collect.
               */
              switchConHelperRequest(CONCURRENT_HELPER_MARK,
                                     CONCURRENT_HELPER_WAIT);
  ...
  if (!isGcOccurred) {
    /* If no more work left (and concurrent scanning is complete or disabled)
     * then switch to exhausted now */
    if ((NULL != _cardTable) &&
        ((MM_ConcurrentCardTable *)_cardTable)->isCardCleaningComplete() &&
        _markingScheme->getWorkPackets()->tracingExhausted() &&
        _concurrentDelegate.isConcurrentScanningComplete(env)) {
      if (_stats.switchExecutionMode(CONCURRENT_CLEAN_TRACE,
    ...
    /* If there is work available on input lists then notify any waiting
     * concurrent helpers */
    if ((_markingScheme->getWorkPackets()->inputPacketAvailable(env)) ||
        ((NULL != _cardTable) && _cardTable->isCardCleaningStarted() &&
 ...
 */
bool MM_ConcurrentGC::concurrentFinalCollection(MM_EnvironmentBase *env,
                                                MM_MemorySubSpace *subSpace) {
  /* Switch to FINAL_COLLECTION; if we fail another thread beat us to it so just
   * return */
  if (_stats.switchExecutionMode(CONCURRENT_EXHAUSTED,
    ...
    if (env->acquireExclusiveVMAccessForGC(this, true, true)) {
      ...
      garbageCollect(env, subSpace, NULL, J9MMCONSTANT_IMPLICIT_GC_DEFAULT,
                     NULL, NULL, NULL);
...
/**
 * Do local mark.
 * Process reference until allocation tax paid in full or we asked to stop
 * because a GC is outstanding. Roots are popped from the work stack and either:
 * 1) deferred - if reference in and active TLH, or
 * 2) ignored - if reference in a dirty card, or
 * 3) traced - call marking scheme to trace referenced object for further
 * references
 *
 * @param sizeToTrace The mutators allocation tax
 * @return The actual amount of work done
 */
uintptr_t MM_ConcurrentGC::localMark(MM_EnvironmentBase *env,
  ...
  Assert_MM_true(_concurrentCycleState._referenceObjectOptions ==
                 MM_CycleState::references_default);
  env->_cycleState = &_concurrentCycleState;
  ...
  while (NULL != (objectPtr = (omrobjectptr_t)env->_workStack.popNoWait(env))) {
    ...
    if ((uintptr_t)objectPtr & PACKET_ARRAY_SPLIT_TAG) {
      continue;
    } else if (((MM_ConcurrentCardTable *)_cardTable)
                   ->isObjectInActiveTLH(env, objectPtr)) {
      ...
      sizeTraced += sizeof(fomrobject_t);
                   ...
                   ->isObjectInUncleanedDirtyCard(env, objectPtr)) {
       ...
       */
      sizeTraced +=
          _extensions->objectModel.getSizeInBytesWithHeader(objectPtr);

      _concurrentDelegate.processItem(env, objectPtr);

      /* If its a partially processed array this will leave the low_tagged
       * scanPtr on the stack. Rather than check for it now we will just ignore
       * the reference when we pop it or before we exit localMark()
       */
    } else {
    ...
    if (env->isExclusiveAccessRequestWaiting()) {
      ...
      uintptr_t conHelperRequest = switchConHelperRequest(
          CONCURRENT_HELPER_MARK, CONCURRENT_HELPER_WAIT);
  ...
  if (((uintptr_t)((omrobjectptr_t)env->_workStack.peek(env))) &
      PACKET_ARRAY_SPLIT_TAG) {
 ...
 * Alos, updates "sizeDone" with the actual size traced which may be less than
 * sizeToDo if a GC is requested whilst we are tracing.
 */
MMINLINE bool MM_ConcurrentGC::cleanCards(MM_EnvironmentBase *env,
                                          bool isMutator, uintptr_t sizeToDo,
                                          uintptr_t *sizeDone,
                                          bool threadAtSafePoint) {
  ...
  bool gcOccurred =
      ((MM_ConcurrentCardTable *)_cardTable)
          ->cleanCards(env, isMutator, sizeToDo, sizeDone, threadAtSafePoint);
  flushLocalBuffers(env);
  env->_cycleState = NULL;
 ...
 *
 */
void MM_ConcurrentGC::concurrentWorkStackOverflow() {
 ...
 *
 */
void MM_ConcurrentGC::clearConcurrentWorkStackOverflow() {
  _stats.setConcurrentWorkStackOverflowOcurred(false);

#if defined(OMR_GC_MODRON_SCAVENGER)
  MM_WorkPacketsConcurrent *packets =
      (MM_WorkPacketsConcurrent *)_markingScheme->getWorkPackets();
  packets->resetWorkPacketsOverflow();
#endif /* OMR_GC_MODRON_SCAVENGER */
}

/**
 * By the time we are called all tracing activity should be stopped and any
 * concurrent helper threads should have parked themselves. Before the final
 * collection can take place we need to ensure that all objects in dirty cards
 * are retraced and the remembered set is processed.
 *
 * @see MM_ParallelGlobalGC::internalPreCollect()
 */
void MM_ConcurrentGC::internalPreCollect(
    MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
    MM_AllocateDescription *allocDescription, uint32_t gcCode) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);

  Trc_MM_ConcurrentGC_internalPreCollect_Entry(env->getLanguageVMThread(),
                                               subSpace);
...
#if defined(OMR_GC_CONCURRENT_SWEEP)
  /**
   * Finish off any sweep work still pending before the GC.  We do this to make
   * sure the heap is walkable (the sweep could have connected part of an entry
   * that spans many chunks, thus creating an unwalkable portion of the heap).
   */
  ...
  /* Set flag to show STW collector is active; some operations need to know if
   * they are called during a global collect or not, eg heapAddRange
   */
  _stwCollectionInProgress = true;

  /* Assume for now we will need to initialize the mark map. If we subsequenly
   * find we got far enough through the concurrent mark cycle then we will reset
   * this flag
   */
  ...
  if (executionModeAtGC == CONCURRENT_OFF) {
    ...
    _concurrentCycleState = MM_CycleState();
    _concurrentCycleState._type = _cycleType;
    MM_ParallelGlobalGC::internalPreCollect(env, subSpace, allocDescription,
                                            gcCode);
  } else {
   ...
   * If we have had a RS overflow or an explicit gc (resulting for idleness)
   * then abort the concurrent collection regardless of how far we got to force
   * a full STW mark.
   */
#if defined(OMR_GC_IDLE_HEAP_MANAGER)
  if ((J9MMCONSTANT_EXPLICIT_GC_IDLE_GC == gcCode) &&
      (CONCURRENT_OFF < executionModeAtGC)) {
    ...
    MM_ParallelGlobalGC::internalPreCollect(env, subSpace, allocDescription,
      ...
      if (_extensions->isRememberedSetInOverflowState() ||
          ((CONCURRENT_OFF < executionModeAtGC) &&
           (CONCURRENT_TRACE_ONLY > executionModeAtGC))) {
    CollectionAbortReason reason =
        (_extensions->isRememberedSetInOverflowState()
             ? ABORT_COLLECTION_REMEMBERSET_OVERFLOW
             : ABORT_COLLECTION_INSUFFICENT_PROGRESS);
    abortCollection(env, reason);
    /* concurrent cycle was aborted so we need to kick off a new cycle and set
     * up the cycle state */
    MM_ParallelGlobalGC::internalPreCollect(env, subSpace, allocDescription,
                                            gcCode);
  } else if (CONCURRENT_TRACE_ONLY <= executionModeAtGC) {
    /* We are going to complete the concurrent cycle to generate the
     * GCStart/GCIncrement events */
    reportGCStart(env);
    ...
    if (_stats.switchExecutionMode(executionModeAtGC, CONCURRENT_OFF)) {
      ...
      if (_extensions->configuration
              ->isSnapshotAtTheBeginningBarrierEnabled()) {
        if (((MM_WorkPacketsSATB *)_markingScheme->getWorkPackets())
                ->inUsePacketsAvailable(env)) {
          ((MM_WorkPacketsSATB *)_markingScheme->getWorkPackets())
              ->moveInUseToNonEmpty(env);
    ...
    if (CONCURRENT_FINAL_COLLECTION > executionModeAtGC) {
      ...
      if (!_markingScheme->getWorkPackets()->tracingExhausted()) {
        ...
        uint64_t startTime = omrtime_hires_clock();
        /* Get assistance from all slave threads to complete processing of any
         * remaining work packets. In the event of work stack overflow we will
         * just dirty cards which will get processed during final card cleaning.
         */
        MM_ConcurrentCompleteTracingTask completeTracingTask(
            env, _dispatcher, this, env->_cycleState);
        _dispatcher->run(env, &completeTracingTask);

        reportConcurrentCompleteTracingEnd(env,
                                           omrtime_hires_clock() - startTime);
      }
    }
    ...
    if (_extensions->scavengerEnabled) {
      ...
      /* Get assistance from all slave threads to reset all mark bits for any
       * NEW areas of heap */
      MM_ConcurrentClearNewMarkBitsTask clearNewMarkBitsTask(env, _dispatcher,
                                                             this);
      _dispatcher->run(env, &clearNewMarkBitsTask);

      /* If remembered set if not empty then re-scan any objects in the
       * remembered set */
      if (!(_extensions->rememberedSet.isEmpty())) {
        MM_ConcurrentScanRememberedSetTask scanRememberedSetTask(
            env, _dispatcher, this, env->_cycleState);
        _dispatcher->run(env, &scanRememberedSetTask);
      }

      reportConcurrentRememberedSetScanEnd(env,
                                           omrtime_hires_clock() - startTime);
    }
#endif /* OMR_GC_MODRON_SCAVENGER */
    ...
    if (_extensions->configuration->isIncrementalUpdateBarrierEnabled()) {
      ...
      do {
        ...
        MM_ConcurrentFinalCleanCardsTask cleanCardsTask(env, _dispatcher, this,
                                                        env->_cycleState);
        ((MM_ConcurrentCardTable *)_cardTable)
            ->initializeFinalCardCleaning(env);
      ...
      reportConcurrentFinalCardCleaningEnd(env,
                                           omrtime_hires_clock() - startTime);
#if defined(DEBUG)
  ...
  Trc_MM_ConcurrentGC_internalPreCollect_Exit(env->getLanguageVMThread(),
                                              subSpace);
}
 ...
 */
bool MM_ConcurrentGC::internalGarbageCollect(
    MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
    MM_AllocateDescription *allocDescription) {
  _extensions->globalGCStats.gcCount += 1;

  masterThreadGarbageCollect(
      env, static_cast<MM_AllocateDescription *>(allocDescription),
      _initializeMarkMap, false);

  /* Restore normal allocation rules.
   * It's not good to do it in concurrentFinalCollection(), as an AF may occur
   * before we get chance to do final collection, so we may miss to restore it.
   * It's too late to do it in postCollect(), as the AF will make us re-try
   * allocate just after collection is done, but postCollect() is invoked after
   * the re-try.
   */
 ...
 * The mark phase of a global collection has completed. The only thing we need
 * do here is to perform any concurrent RAS checks if they have been requested.
 */
void MM_ConcurrentGC::postMark(MM_EnvironmentBase *envModron) {
  /*
...
/**
 * A global collection has been completed. All we need do here is request a
 * retune of concurrent adaptive parameters.
 * @see MM_ParallelGlobalGC::internalPostCollect()
 */
void MM_ConcurrentGC::internalPostCollect(MM_EnvironmentBase *env,
                                          MM_MemorySubSpace *subSpace) {
  Trc_MM_ConcurrentGC_internalPostCollect_Entry(env->getLanguageVMThread(),
                                                subSpace);
  ...
  /* Re tune for next concurrent cycle if we have had a heap resize or we got
   * far enough last time. We only re-tune on a system GC in the event of a heap
   * resize.
   */
  if (_retuneAfterHeapResize ||
      _stats.getExecutionModeAtGC() > CONCURRENT_OFF) {
    tuneToHeap(env);
  }
  ...
  if (_extensions->optimizeConcurrentWB) {
    /* Reset vmThread flag so mutators don't dirty cards until next concurrent
     * KO */
  ...
  Trc_MM_ConcurrentGC_internalPostCollect_Exit(env->getLanguageVMThread(),
                                               subSpace);
}
 ...
 * @warning Any caller of this routine is assumed to have exclusive VM access.
 * This implies that all concurrent tracers will have stopped including any
 * concurrent helper threads which should be parked.
 */
void MM_ConcurrentGC::abortCollection(MM_EnvironmentBase *env,
                                      CollectionAbortReason reason) {
  ...
  /* Since all of the current mark data is being flushed make sure to flush the
   * reference lists so they can be properly rebuilt by the next mark phase.
   */
  ...
  /* Unconditionally change execution mode back to OFF. On a subsequent
   * allocation we will then restart a concurrent mark cycle. We probably won't
   * finish before we get an AF but we may as well do as much as possible.
   *
  ...
  Assert_GC_true_with_message(env, CONCURRENT_OFF == _stats.getExecutionMode(),
                              "MM_ConcurrentStats::_executionMode = %zu\n",
                              _stats.getExecutionMode());
}
 ...
 */
void MM_ConcurrentGC::prepareHeapForWalk(MM_EnvironmentBase *envModron) {
 ...
 */
bool MM_ConcurrentGC::heapAddRange(MM_EnvironmentBase *env,
                                   MM_MemorySubSpace *subspace, uintptr_t size,
                                   void *lowAddress, void *highAddress) {
  Trc_MM_ConcurrentGC_heapAddRange_Entry(env->getLanguageVMThread(), subspace,
                                         size, lowAddress, highAddress);
  ...
  bool result = MM_ParallelGlobalGC::heapAddRange(env, subspace, size,
 ...
 * @param highAddress The top address (non-inclusive) of the memory removed from
 * the heap
 ...
 */
bool MM_ConcurrentGC::heapRemoveRange(MM_EnvironmentBase *env,
                                      MM_MemorySubSpace *subspace,
                                      uintptr_t size, void *lowAddress,
                                      void *highAddress, void *lowValidAddress,
                                      void *highValidAddress) {
  Trc_MM_ConcurrentGC_heapRemoveRange_Entry(
      env->getLanguageVMThread(), subspace, size, lowAddress, highAddress,
  ...
  bool result = MM_ParallelGlobalGC::heapRemoveRange(
      env, subspace, size, lowAddress, highAddress, lowValidAddress,
      highValidAddress);

  /* ...and then contract the card table */
  result = result &&
           ((MM_ConcurrentCardTable *)_cardTable)
               ->heapRemoveRange(env, subspace, size, lowAddress, highAddress,
                                 lowValidAddress, highValidAddress);
 ...
 */
void MM_ConcurrentGC::heapReconfigured(MM_EnvironmentBase *env) {
  /* If called outside a global collection for a heap expand/contract..
   */
  if (!_stwCollectionInProgress &&
      (_rebuildInitWorkForAdd || _rebuildInitWorkForRemove)) {
     ...
     *  we will try to memset ranges that are now contracted (decommitted
     * memory) when we resume the init work.
     */
    if (_stats.getExecutionMode() < CONCURRENT_INIT_COMPLETE) {
      tuneToHeap(env);
    } else {
      /* Heap expand/contract is during a concurrent cycle..we need to adjust
       * the trace target so that the trace rate is adjusted correctly on
       * subsequent allocates.
       */
      adjustTraceTarget();
    }
  }
 ...
 *
 * This routine is called by ConcurrentClearNewMarkBits task on master and any
 * slave threads during internalPreCollect().
 */
void MM_ConcurrentGC::clearNewMarkBits(MM_EnvironmentBase *env) {
  void *from, *to;
  InitType type;
  bool concurrentCollectable;
 ...
 * If a concurrent cycle gets halted then some work packets may not get
 * processed. Process them now so the time to do so is not incorrectly reported
 * against stages of collect that follow.
 *
 */
void MM_ConcurrentGC::completeTracing(MM_EnvironmentBase *env) {
  omrobjectptr_t objectPtr;
  uintptr_t bytesTraced = 0;
  env->_workStack.reset(env, _markingScheme->getWorkPackets());
 ...
 */
void MM_ConcurrentGC::finalCleanCards(MM_EnvironmentBase *env) {
  omrobjectptr_t objectPtr;
  bool moreCards = true;
  bool moreRefs = true;
  uintptr_t totalTraced = 0;
  ...
  while (moreRefs) {
    ...
    while (objectPtr) {
      ...
      if (objectPtr) {
        ...
        if ((uintptr_t)objectPtr & PACKET_ARRAY_SPLIT_TAG) {
          continue;
          /* If object in a dirty card we will re-visit later so ignore it for
           * now */
        } else {
          bool dirty;
          ...
          if (_extensions->scavengerEnabled) {
            dirty = ((MM_ConcurrentCardTable *)_cardTable)
                        ->isObjectInDirtyCard(env, objectPtr);
          } else
#endif /* OMR_GC_MODRON_SCAVENGER */
          {
            dirty = ((MM_ConcurrentCardTable *)_cardTable)
                        ->isObjectInDirtyCardNoCheck(env, objectPtr);
          }
    ...
    if (moreCards) {
      ...
      moreCards = ((MM_ConcurrentCardTable *)_cardTable)
                      ->finalCleanCards(env, &bytesCleaned);
...
#if defined(OMR_GC_MODRON_SCAVENGER)
/**
 * Scan remembered set looking for any MARKED objects which are not in dirty
 * cards. A marked object which is not in a dirty card needs rescanning now for
 * any references to the nursery which will not have been traced by concurrent
 * mark
 *
 * This routine is called ConcurrentScanRememberedSetTask on master and any
 * slave threads during internalPreCollect().
 */
void MM_ConcurrentGC::scanRememberedSet(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  ...
  uintptr_t bytesTraced = 0;
  uintptr_t maxPushes =
      _markingScheme->getWorkPackets()->getSlotsInPacket() / 2;
  ...
  while ((puddle = rememberedSetIterator.nextList()) != NULL) {
    if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
      ...
      while (
          (slotPtr = (omrobjectptr_t *)rememberedSetSlotIterator.nextSlot()) !=
          NULL) {
        objectPtr = *slotPtr;
        /* For all objects in remembered set that have been marked scan the
         * object unless its card is dirty in which case we leave it for later
         * processing by finalCleanCards()
         */
        if ((objectPtr >= _heapBase) && (objectPtr < _heapAlloc) &&
          ...
          if (_extensions->dirtCardDuringRSScan) {
            ...
            /* VMDESIGN 2048 -- due to barrier elision optimizations, the JIT
             * may not have dirtied cards for some objects in the remembered
             * set. Therefore we may discover references to both nursery and
             * tenure objects while scanning remembered objects.
             */

            bytesTraced += _markingScheme->scanObject(
                env, objectPtr, SCAN_REASON_REMEMBERED_SET_SCAN);

            /* Have we pushed enough new references? */
            if (env->_workStack.getPushCount() >= maxPushes) {
              /* To reduce the chances of mark stack overflow, we do some
               * marking of what we have just pushed.
               *
               * WARNING. If we HALTED concurrent then we will process any
               * remaining workpackets at this point. This will make RS
               * processing appear more expensive than it really is.
               */
              while (NULL !=
                     (objectPtr =
                          (omrobjectptr_t)env->_workStack.popNoWait(env))) {
                bytesTraced += _markingScheme->scanObject(env, objectPtr,
                                                          SCAN_REASON_PACKET);
              }
              env->_workStack.clearPushCount();
            }
  ...
  env->_workStack.clearPushCount();
  /* sort of abusing addToWorkStallTime to record the point when thread is
   * finished with RS Scan Work
   * Since popNoWait is used, we never stalled before this point. All stall time
   * will be from this point till RS scan end event */
  env->_workPacketStats.addToWorkStallTime(0, omrtime_hires_clock());

  /* Flush this threads reference object buffer, work stack, returning any
   * packets to appropriate lists */
  flushLocalBuffers(env);
...
/**
 * Process event from an external GC (Scavenger) when old-to-old reference is
 * created from an existing old-to-new or new-to-new reference. We need to
 * rescan the parent object, as slots which during original scan contained new
 * area references could now contain references into old area.
 * @param objectPtr  Parent old object that has a reference to a child old
 * object
 */
void MM_ConcurrentGC::oldToOldReferenceCreated(MM_EnvironmentBase *env,
                                               omrobjectptr_t objectPtr) {
 ...
 */
bool MM_ConcurrentGC::collectorStartup(MM_GCExtensionsBase *extensions) {
 ...
 */
void MM_ConcurrentGC::collectorShutdown(MM_GCExtensionsBase *extensions) {
  shutdownConHelperThreads(extensions);
  MM_ParallelGlobalGC::collectorShutdown(extensions);
}

void MM_ConcurrentGC::flushLocalBuffers(MM_EnvironmentBase *env) {
 ...
 * @see MM_ParallelGlobalGC::collectorShutdown()
 */
void MM_ConcurrentGC::updateMeteringHistoryBeforeGC(MM_EnvironmentBase *env) {
  ...
  if (MM_GCExtensionsBase::METER_DYNAMIC == _extensions->concurrentMetering) {
    ...
    oldFreeBeforeGC =
        _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_OLD);
    loaFreeBeforeGC = _extensions->heap->getApproximateActiveFreeLOAMemorySize(
        MEMORY_TYPE_OLD);
 ...
 * @see MM_ParallelGlobalGC::collectorShutdown()
 */
void MM_ConcurrentGC::updateMeteringHistoryAfterGC(MM_EnvironmentBase *env) {
  /* If LOA disabled or a system GC then nothing to do */
  if (!_extensions->largeObjectArea ||
      env->_cycleState->_gcCode.isExplicitGC()) {
    return;
  ...
  if (MM_GCExtensionsBase::METER_DYNAMIC == _extensions->concurrentMetering) {
    ...
    oldFreeAfterGC =
        _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_OLD);
    loaFreeAfterGC = _extensions->heap->getApproximateActiveFreeLOAMemorySize(
        MEMORY_TYPE_OLD);
     ...
     * Othewise we use history of previous collectiosn to decide which one to
     * meter
     */
    if (0 == _meteringHistory[_currentMeteringHistory].soaFreeBeforeGC) {
      ...
      _meteringType = SOA;
               ...
               0 == _meteringHistory[_currentMeteringHistory].loaFreeBeforeGC) {
       ...
       */
      soaExhaustion =
          (float)soaFreeAfterGC /
          (float)(_meteringHistory[_currentMeteringHistory].soaFreeBeforeGC);
      loaExhaustion =
          (float)loaFreeAfterGC /
          (float)(_meteringHistory[_currentMeteringHistory].soaFreeBeforeGC);

      /* ..and add vote into history */
      _meteringHistory[_currentMeteringHistory].vote =
          soaExhaustion >= loaExhaustion ? VOTE_SOA : VOTE_LOA;
    ...
    _currentMeteringHistory =
        ((_currentMeteringHistory + 1) == _meteringHistorySize)
            ? 0
            : _currentMeteringHistory + 1;
  }
}
#endif /* OMR_GC_LARGE_OBJECT_AREA */
