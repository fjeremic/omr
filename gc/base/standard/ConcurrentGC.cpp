  ...
  int J9THREAD_PROC con_helper_thread_proc(void* info)
  {
    ...
    omrsig_handler_fn signalHandler =
      collector->_concurrentDelegate.getProtectedSignalHandler(
        &signalHandlerArg);
    omrsig_protect(con_helper_thread_proc2,
                   info,
                   signalHandler,
                   signalHandlerArg,
...
void
MM_ConcurrentGC::reportConcurrentAborted(MM_EnvironmentBase* env,
...
void
MM_ConcurrentGC::reportConcurrentFinalCardCleaningEnd(MM_EnvironmentBase* env,
                                                      uint64_t duration)
{
  MM_ConcurrentCardTable* cardTable = (MM_ConcurrentCardTable*)_cardTable;
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
MM_ConcurrentGC::reportConcurrentCollectionStart(MM_EnvironmentBase* env)
{
  ...
  uint64_t meanExclusiveAccessIdleTimeMicros =
    omrtime_hires_delta(0,
                        env->getMeanExclusiveAccessIdleTime(),
...
void
MM_ConcurrentGC::reportConcurrentCollectionEnd(MM_EnvironmentBase* env,
                                               uint64_t duration)
{
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void
MM_ConcurrentGC::reportConcurrentBackgroundThreadActivated(
...
void
MM_ConcurrentGC::reportConcurrentBackgroundThreadFinished(
  MM_EnvironmentBase* env,
...
void
MM_ConcurrentGC::reportConcurrentCompleteTracingEnd(MM_EnvironmentBase* env,
                                                    uint64_t duration)
{
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void
MM_ConcurrentGC::reportConcurrentRememberedSetScanEnd(MM_EnvironmentBase* env,
                                                      uint64_t duration)
{
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
 ...
 */
void
MM_ConcurrentGC::signalThreadsToActivateWriteBarrierAsyncEventHandler(
...
MM_ConcurrentGC::initialize(MM_EnvironmentBase* env)
{
  ...
  if (omrthread_monitor_init_with_name(
        &_initWorkMonitor, 0, "MM_ConcurrentGC::initWork")) {
    goto error_no_memory;
  }

  /* Initialize the tuning monitor monitor */
  if (omrthread_monitor_init_with_name(
        &_concurrentTuningMonitor, 0, "MM_ConcurrentGC::concurrentTuning")) {
    goto error_no_memory;
  }

  /* ..and the initialization work complete monitor */
  if (omrthread_monitor_init_with_name(
        &_initWorkCompleteMonitor, 0, "MM_ConcurrentGC::initWorkComplete")) {
    goto error_no_memory;
  }
  ...
  _cardCleaningFactorPass1 =
    interpolateInRange(INITIAL_CARD_CLEANING_FACTOR_PASS1_1,
                       INITIAL_CARD_CLEANING_FACTOR_PASS1_8,
                       INITIAL_CARD_CLEANING_FACTOR_PASS1_10,
                       _allocToTraceRateNormal);
  _maxCardCleaningFactorPass1 =
    interpolateInRange(MAX_CARD_CLEANING_FACTOR_PASS1_1,
  ...
  if (_secondCardCleanPass) {
    _cardCleaningFactorPass2 =
      interpolateInRange(INITIAL_CARD_CLEANING_FACTOR_PASS2_1,
                         INITIAL_CARD_CLEANING_FACTOR_PASS2_8,
                         INITIAL_CARD_CLEANING_FACTOR_PASS2_10,
                         _allocToTraceRateNormal);

    _maxCardCleaningFactorPass2 =
      interpolateInRange(MAX_CARD_CLEANING_FACTOR_PASS2_1,
  ...
  _cardCleaningThresholdFactor =
    interpolateInRange(CARD_CLEANING_THRESHOLD_FACTOR_1,
                       CARD_CLEANING_THRESHOLD_FACTOR_8,
                       CARD_CLEANING_THRESHOLD_FACTOR_10,
                       _allocToTraceRateNormal);
  _allocToTraceRateMinFactor =
    (float)1 / interpolateInRange(MIN_ALLOC_2_TRACE_RATE_1,
 ...
 */
MMINLINE float
MM_ConcurrentGC::interpolateInRange(float val1,
                                    float val8,
                                    float val10,
                                    uintptr_t traceRate)
{
 ...
 */
bool
MM_ConcurrentGC::getInitRange(MM_EnvironmentBase* env,
                              void** from,
                              void** to,
                              InitType* type,
  ...
  while (i < _numInitRanges) {
    ...
    if (localFrom < _initRanges[i].top) {
      ...
      MM_AtomicOperations::lockCompareExchangeU32(
        &_nextInitRange, oldIndex, newIndex);
...
MM_ConcurrentGC::initializeConcurrentHelpers(MM_GCExtensionsBase* extensions)
{
  ...
  for (conHelperThreadCount = 0; conHelperThreadCount < _conHelperThreads;
    ...
    threadForkResult =
      createThreadWithCategory(&(_conHelpersTable[conHelperThreadCount]),
                               OMR_OS_STACK_SIZE,
...
MM_ConcurrentGC::tuneToHeap(MM_EnvironmentBase* env)
{
  ...
  if (0 == _stats.getKickoffThreshold() || _retuneAfterHeapResize) {
    ...
    if ((NULL == env->_cycleState) ||
      ...
      if (_secondCardCleanPass) {
        ...
        _bytesToTracePass1 =
          (uintptr_t)MM_Math::weightedAverage((float)_bytesToTracePass1,
                                              (float)bytesTracedPass1,
                                              LIVE_PART_HISTORY_WEIGHT);
        _bytesToTracePass2 =
          (uintptr_t)MM_Math::weightedAverage((float)_bytesToTracePass2,
                                              (float)bytesTracedPass2,
                                              LIVE_PART_HISTORY_WEIGHT);
        _bytesToCleanPass1 =
          (uintptr_t)MM_Math::weightedAverage((float)_bytesToCleanPass1,
                                              (float)bytesCleanedPass1,
                                              CARD_CLEANING_HISTORY_WEIGHT);
        _bytesToCleanPass2 =
          (uintptr_t)MM_Math::weightedAverage((float)_bytesToCleanPass2,
                                              (float)bytesCleanedPass2,
                                              CARD_CLEANING_HISTORY_WEIGHT);
      } else {
        _bytesToTracePass1 =
          (uintptr_t)MM_Math::weightedAverage((float)_bytesToTracePass1,
  ...
  Trc_MM_ConcurrentGC_tuneToHeap_Exit2(env->getLanguageVMThread(),
                                       _stats.getTraceSizeTarget(),
                                       _stats.getInitWorkRequired(),
                                       _stats.getKickoffThreshold());
}
...
MM_ConcurrentGC::updateTuningStatistics(MM_EnvironmentBase* env)
{
  ...
  if (env->_cycleState->_gcCode.isExplicitGC() ||
    ...
    _tenureNonLeafObjectFactor =
      MM_Math::weightedAverage(_tenureNonLeafObjectFactor,
                               newNonLeafObjectFactor,
    ...
    switch (executionModeAtGC) {
      ...
      case CONCURRENT_TRACE_ONLY:
        _cardCleaningFactorPass1 =
          MM_Math::weightedAverage(_cardCleaningFactorPass1,
                                   _maxCardCleaningFactorPass1,
                                   CARD_CLEANING_HISTORY_WEIGHT);
        _cardCleaningFactorPass2 =
          MM_Math::weightedAverage(_cardCleaningFactorPass2,
                                   _maxCardCleaningFactorPass2,
                                   CARD_CLEANING_HISTORY_WEIGHT);
        _bytesTracedInPass1Factor =
          MM_Math::weightedAverage(_bytesTracedInPass1Factor,
                                   ALL_BYTES_TRACED_IN_PASS_1,
 ...
 */
uintptr_t
MM_ConcurrentGC::calculateInitSize(MM_EnvironmentBase* env,
 ...
 */
bool
MM_ConcurrentGC::periodicalTuningNeeded(MM_EnvironmentBase* env,
                                        uintptr_t freeSize)
{
  ...
  if (_lastFreeSize == LAST_FREE_SIZE_NEEDS_INITIALIZING) {
    ...
    if (_conHelpersStarted > 0) {
      ...
      _alloc2ConHelperTraceRate =
        MM_Math::weightedAverage(_alloc2ConHelperTraceRate,
                                 newConHelperRate,
 ...
 */
void
MM_ConcurrentGC::kickoffCardCleaning(MM_EnvironmentBase* env,
 ...
 */
bool
MM_ConcurrentGC::replenishPoolForAllocate(MM_EnvironmentBase* env,
 ...
 */
void
MM_ConcurrentGC::payAllocationTax(MM_EnvironmentBase* env,
 ...
 */
void
MM_ConcurrentGC::concurrentMark(MM_EnvironmentBase* env,
                                MM_MemorySubSpace* subspace,
                                MM_AllocateDescription* allocDescription)
{
  ...
  while (!taxPaid) {
    ...
    switch (executionMode) {
      ...
      case CONCURRENT_CLEAN_TRACE:
        ...
        if (sizeToTrace > 0) {
          sizeTraced = doConcurrentTrace(
            env, allocDescription, sizeToTrace, subspace, threadAtSafePoint);
        }
      ...
      default:
        ...
        if (_stats.switchExecutionMode(executionMode, nextExecutionMode)) {
          ...
          _concurrentDelegate.collectRoots(
            env, executionMode, &collectedRoots, &taxPaid);
          if (collectedRoots) {
 ...
 */
bool
MM_ConcurrentGC::forceKickoff(MM_EnvironmentBase* env,
                              MM_MemorySubSpace* subSpace,
                              MM_AllocateDescription* allocDescription,
                              uint32_t gcCode)
{
  if (_extensions->concurrentKickoffEnabled &&
      _concurrentDelegate.canForceConcurrentKickoff(
        env, gcCode, &_languageKickoffReason)) {
 ...
 */
bool
MM_ConcurrentGC::timeToKickoffConcurrent(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocDescription)
{
 ...
 */
void
MM_ConcurrentGC::concurrentSweep(MM_EnvironmentBase* env,
                                 MM_MemorySubSpace* subspace,
                                 MM_AllocateDescription* allocDescription)
{
 ...
 */
uintptr_t
MM_ConcurrentGC::doConcurrentInitialization(MM_EnvironmentBase* env,
                                            uintptr_t initToDo)
{
 ...
 */
bool
MM_ConcurrentGC::concurrentFinalCollection(MM_EnvironmentBase* env,
                                           MM_MemorySubSpace* subSpace)
{
  ...
  if (_stats.switchExecutionMode(CONCURRENT_EXHAUSTED,
    ...
    if (env->acquireExclusiveVMAccessForGC(this, true, true)) {
      ...
      garbageCollect(env,
                     subSpace,
                     NULL,
                     J9MMCONSTANT_IMPLICIT_GC_DEFAULT,
                     NULL,
                     NULL,
                     NULL);
 ...
 */
MMINLINE bool
MM_ConcurrentGC::cleanCards(MM_EnvironmentBase* env,
                            bool isMutator,
                            uintptr_t sizeToDo,
                            uintptr_t* sizeDone,
                            bool threadAtSafePoint)
 ...
 */
void
MM_ConcurrentGC::internalPreCollect(MM_EnvironmentBase* env,
                                    MM_MemorySubSpace* subSpace,
                                    MM_AllocateDescription* allocDescription,
                                    uint32_t gcCode)
{
  ...
  if (executionModeAtGC == CONCURRENT_OFF) {
    ...
    MM_ParallelGlobalGC::internalPreCollect(
      env, subSpace, allocDescription, gcCode);
  } else {
  ...
  if ((J9MMCONSTANT_EXPLICIT_GC_IDLE_GC == gcCode) &&
    ...
    if (_extensions->scavengerEnabled) {
       ...
       * NEW areas of heap */
      MM_ConcurrentClearNewMarkBitsTask clearNewMarkBitsTask(
        env, _dispatcher, this);
    ...
    if (_extensions->configuration->isIncrementalUpdateBarrierEnabled()) {
      ...
      do {
        ...
        MM_ConcurrentFinalCleanCardsTask cleanCardsTask(
          env, _dispatcher, this, env->_cycleState);
  ...
  MM_AllocateDescription* allocDescription)
{
  ...
  masterThreadGarbageCollect(
    env,
    static_cast<MM_AllocateDescription*>(allocDescription),
    _initializeMarkMap,
    false);
 ...
 */
void
MM_ConcurrentGC::internalPostCollect(MM_EnvironmentBase* env,
                                     MM_MemorySubSpace* subSpace)
{
 ...
 */
void
MM_ConcurrentGC::abortCollection(MM_EnvironmentBase* env,
                                 CollectionAbortReason reason)
{
 ...
 */
bool
MM_ConcurrentGC::heapAddRange(MM_EnvironmentBase* env,
                              MM_MemorySubSpace* subspace,
                              uintptr_t size,
                              void* lowAddress,
                              void* highAddress)
{
  Trc_MM_ConcurrentGC_heapAddRange_Entry(
  ...
  bool result = MM_ParallelGlobalGC::heapAddRange(
    env, subspace, size, lowAddress, highAddress);
 ...
 */
bool
MM_ConcurrentGC::heapRemoveRange(MM_EnvironmentBase* env,
                                 MM_MemorySubSpace* subspace,
                                 uintptr_t size,
                                 void* lowAddress,
                                 void* highAddress,
...
{
  Trc_MM_ConcurrentGC_heapRemoveRange_Entry(env->getLanguageVMThread(),
                                            subspace,
                                            size,
                                            lowAddress,
                                            highAddress,
  ...
  bool result = MM_ParallelGlobalGC::heapRemoveRange(env,
                                                     subspace,
                                                     size,
                                                     lowAddress,
                                                     highAddress,
  ...
  result = result && ((MM_ConcurrentCardTable*)_cardTable)
                       ->heapRemoveRange(env,
                                         subspace,
                                         size,
                                         lowAddress,
          ...
          if (_extensions->dirtCardDuringRSScan) {
            ...
            if (env->_workStack.getPushCount() >= maxPushes) {
              ...
              while (
                ...
                (objectPtr = (omrobjectptr_t)env->_workStack.popNoWait(env))) {
                bytesTraced += _markingScheme->scanObject(
                  env, objectPtr, SCAN_REASON_PACKET);
              }
              env->_workStack.clearPushCount();
            }
          }
 ...
 */
void
MM_ConcurrentGC::oldToOldReferenceCreated(MM_EnvironmentBase* env,
                                          omrobjectptr_t objectPtr)
{
