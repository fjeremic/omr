...
void poisonReferenceSlots(
    OMR_VMThread* omrVMThread, MM_HeapRegionDescriptor* region, omrobjectptr_t object, void* userData)
{
 ...
 */
static void fixObject(OMR_VMThread* omrVMThread, MM_HeapRegionDescriptor* region, omrobjectptr_t object, void* userData)
{
 ...
 */
static void hookGlobalGcSweepEndRsoSafetyFixHeap(
...
static void hookGlobalGcSweepEndAbortedCSFixHeap(
    ...
    if (extensions->isScavengerBackOutFlagRaised()) {
        ...
        while (NULL != (region = regionIterator.nextRegion())) {
            if (MEMORY_TYPE_NEW == (region->getTypeFlags() & MEMORY_TYPE_NEW)) {
                ...
                MM_HeapMapIterator markedObjectIterator(extensions,
                    ((MM_ParallelGlobalGC*)extensions->getGlobalCollector())->getMarkingScheme()->getMarkMap(),
...
MM_ParallelGlobalGC* MM_ParallelGlobalGC::newInstance(MM_EnvironmentBase* env)
{
    ...
    globalGC = (MM_ParallelGlobalGC*)env->getForge()->allocate(
        sizeof(MM_ParallelGlobalGC), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (globalGC) {
...
bool MM_ParallelGlobalGC::initialize(MM_EnvironmentBase* env)
{
     ...
     */
    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_ALLOCATION_FAILURE_CYCLE_START,
            globalGCHookAFCycleStart, OMR_GET_CALLSITE(), NULL);
    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_ALLOCATION_FAILURE_CYCLE_END,
            globalGCHookAFCycleEnd, OMR_GET_CALLSITE(), NULL);

#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START, globalGCHookCCStart,
            OMR_GET_CALLSITE(), NULL);
    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(
            mmPrivateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END, globalGCHookCCEnd, OMR_GET_CALLSITE(), NULL);
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */

    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(
            mmPrivateHooks, J9HOOK_MM_PRIVATE_SYSTEM_GC_START, globalGCHookSysStart, OMR_GET_CALLSITE(), NULL);
    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(
            mmPrivateHooks, J9HOOK_MM_PRIVATE_SYSTEM_GC_END, globalGCHookSysEnd, OMR_GET_CALLSITE(), NULL);

#if defined(OMR_GC_MODRON_SCAVENGER)
    if (_extensions->scavengerEnabled) {
        /* Hook the global collector to guarantee heap walk safety in the event of an RSO */
        /* NOTE: This will be a one time hook across all instances as the function and user data will be identical -
         * i.e., we will only get one Hook registered no matter how many scavengers are created/initialized
         */
        (*mmPrivateHooks)
            ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_SWEEP_END,
 ...
 ****************************************
 */
void MM_ParallelGlobalGC::cleanupAfterGC(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
    ...
    /* Null tenure TLH (Copy Cache) references for all GC slave and Mutator (for concurrent scavenger) threads as the
     * memory will be invalidated on sweep cycle*/
...
void MM_ParallelGlobalGC::masterThreadGarbageCollect(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, bool initMarkMap, bool rebuildMarkBits)
{
    ...
    if (_compactThisCycle) {
        ...
        if (COMPACT_PREVENTED_NONE != compactPreventedReason) {
            ...
            Trc_MM_CompactPrevented(
                env->getLanguageVMThread(), getCompactionPreventedReasonAsString(compactPreventedReason));
        ...
        if (!_fixHeapForWalkCompleted) {
            ...
            if (compactedThisCycle) {
                ...
                _extensions->globalGCStats.fixHeapForWalkTime
                    = omrtime_hires_delta(startTime, omrtime_hires_clock(), OMRPORT_TIME_DELTA_IN_MICROSECONDS);
...
#if defined(OMR_GC_MODRON_COMPACTION)
bool MM_ParallelGlobalGC::shouldCompactThisCycle(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
...

    {
        ...
        if ((J9MMCONSTANT_EXPLICIT_GC_IDLE_GC == gcCode.getCode()) && (_extensions->gcOnIdle)) {
            ...
            float totalFragmentationRatio
                = ((float)totalFragmentation) / ((float)freeMemorySize + (float)totalSize / 2);

            Trc_ParallelGlobalGC_shouldCompactThisCycle(
                env->getLanguageVMThread(), totalFragmentationRatio, _extensions->gcOnIdleCompactThreshold);
...
bool MM_ParallelGlobalGC::compactRequiredBeforeHeapContraction(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, uintptr_t contractionSize)
{
...
void MM_ParallelGlobalGC::sweep(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, bool rebuildMarkBits)
{
...
#if defined(OMR_GC_MODRON_COMPACTION)
    /* Decide is a compaction is required - this decision must be made after we sweep since we use the
     * largestFreeEntrySize, as changed by sweep, to determine if a compaction should be done */
    _compactThisCycle = shouldCompactThisCycle(
        env, allocDescription, activeSubSpace->maxExpansionInSpace(env), env->_cycleState->_gcCode);
...
bool MM_ParallelGlobalGC::completeFreelistRebuildRequired(MM_EnvironmentBase* env, SweepCompletionReason* reason)
{
    *reason = NOT_REQUIRED;
...
void MM_ParallelGlobalGC::masterThreadSweepStart(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
...
void MM_ParallelGlobalGC::masterThreadSweepComplete(MM_EnvironmentBase* env, SweepCompletionReason reason)
{
...
void MM_ParallelGlobalGC::masterThreadCompact(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, bool rebuildMarkBits)
{
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
    MM_CompactStats* compactStats = &_extensions->globalGCStats.compactStats;
    ...
    MM_ParallelCompactTask compactTask(
        env, _dispatcher, _compactScheme, rebuildMarkBits, env->_cycleState->_gcCode.shouldAggressivelyCompact());
...
void MM_ParallelGlobalGC::masterThreadRestartAllocationCaches(MM_EnvironmentBase* env)
{
...
void MM_ParallelGlobalGC::internalPostCollect(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace)
{
...
void MM_ParallelGlobalGC::processLargeAllocateStatsBeforeGC(MM_EnvironmentBase* env)
{
    MM_MemorySpace* defaultMemorySpace = _extensions->heap->getDefaultMemorySpace();
...
void MM_ParallelGlobalGC::processLargeAllocateStatsAfterCompact(MM_EnvironmentBase* env)
{
    processLargeAllocateStatsAfterSweep(env);
}

void MM_ParallelGlobalGC::processLargeAllocateStatsAfterSweep(MM_EnvironmentBase* env)
{
    MM_MemorySpace* defaultMemorySpace = _extensions->heap->getDefaultMemorySpace();
    MM_MemorySubSpace* tenureMemorySubspace = defaultMemorySpace->getTenureMemorySubSpace();
    MM_MemoryPool* memoryPool = tenureMemorySubspace->getMemoryPool();
...
void MM_ParallelGlobalGC::reportGCCycleFinalIncrementEnding(MM_EnvironmentBase* env)
{
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
...
uintptr_t MM_ParallelGlobalGC::fixHeapForWalk(
    MM_EnvironmentBase* env, UDATA walkFlags, uintptr_t walkReason, MM_HeapWalkerObjectFunc walkFunction)
{
 ...
 */
bool MM_ParallelGlobalGC::heapAddRange(
 ...
 */
bool MM_ParallelGlobalGC::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size,
    void* lowAddress, void* highAddress, void* lowValidAddress, void* highValidAddress)
{
    bool result = _markingScheme->heapRemoveRange(
    ...
    result = result
        && _delegate.heapRemoveRange(env, subspace, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
 ...
 */
void* MM_ParallelGlobalGC::createSweepPoolState(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool)
{
 ...
 */
void MM_ParallelGlobalGC::deleteSweepPoolState(MM_EnvironmentBase* env, void* sweepPoolState)
{
    Assert_MM_true(NULL != _sweepScheme);
...
void MM_ParallelGlobalGC::completeExternalConcurrentCycle(MM_EnvironmentBase* env)
{
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    if (_extensions->isConcurrentScavengerEnabled()) {
...
void MM_ParallelGlobalGC::reportGCCycleEnd(MM_EnvironmentBase* env)
{
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_GC_POST_CYCLE_END(_extensions->privateHookInterface, env->getOmrVMThread(),
...
void MM_ParallelGlobalGC::reportGCStart(MM_EnvironmentBase* env)
{
    ...
    TRIGGER_J9HOOK_MM_OMR_GLOBAL_GC_START(_extensions->omrHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
        J9HOOK_MM_OMR_GLOBAL_GC_START, _extensions->globalGCStats.gcCount, scavengerCount,
        env->_cycleState->_gcCode.isExplicitGC() ? 1 : 0, env->_cycleState->_gcCode.isAggressiveGC() ? 1 : 0,
        _bytesRequested);
}
...
void MM_ParallelGlobalGC::reportGlobalGCIncrementStart(MM_EnvironmentBase* env)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
    uintptr_t scavengerCount = 0;
#if defined(OMR_GC_MODRON_SCAVENGER)
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_GLOBAL_GC_INCREMENT_START(_extensions->privateHookInterface, env->getOmrVMThread(),
        omrtime_hires_clock(), J9HOOK_MM_PRIVATE_GLOBAL_GC_INCREMENT_START, _extensions->globalGCStats.gcCount,
        scavengerCount, _bytesRequested);
}

void MM_ParallelGlobalGC::reportGlobalGCCollectComplete(MM_EnvironmentBase* env)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ParallelGlobalGC::reportGCEnd(MM_EnvironmentBase* env)
{
    ...
    Trc_MM_GlobalGCEnd(env->getLanguageVMThread(),
        _extensions->globalGCStats.workPacketStats.getSTWWorkStackOverflowOccured(),
        _extensions->globalGCStats.workPacketStats.getSTWWorkStackOverflowCount(), approximateTotalActiveFreeMemorySize,
    ...
    /* these are assigned to temporary variable out-of-line since some preprocessors get confused if you have directives
     * in macros */
    uintptr_t approximateActiveFreeMemorySize = 0;
    uintptr_t activeMemorySize = 0;

    TRIGGER_J9HOOK_MM_OMR_GLOBAL_GC_END(_extensions->omrHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
        J9HOOK_MM_OMR_GLOBAL_GC_END, _extensions->globalGCStats.workPacketStats.getSTWWorkStackOverflowOccured(),
        _extensions->globalGCStats.workPacketStats.getSTWWorkStackOverflowCount(),
        _extensions->globalGCStats.workPacketStats.getSTWWorkpacketCountAtOverflow(),
...
void MM_ParallelGlobalGC::reportMarkStart(MM_EnvironmentBase* env)
{
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_MARK_START(
        _extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_PRIVATE_MARK_START);
}
...
void MM_ParallelGlobalGC::reportMarkEnd(MM_EnvironmentBase* env)
{
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_MARK_END(
        _extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_PRIVATE_MARK_END);
}
...
void MM_ParallelGlobalGC::reportSweepStart(MM_EnvironmentBase* env)
{
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_SWEEP_START(
        _extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_PRIVATE_SWEEP_START);
}
...
void MM_ParallelGlobalGC::reportSweepEnd(MM_EnvironmentBase* env)
{
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_SWEEP_END(
        _extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_PRIVATE_SWEEP_END);
}
...
void MM_ParallelGlobalGC::reportCompactStart(MM_EnvironmentBase* env)
{
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_COMPACT_START(_extensions->privateHookInterface, env->getOmrVMThread(),
        omrtime_hires_clock(), J9HOOK_MM_PRIVATE_COMPACT_START, _extensions->globalGCStats.gcCount);
}
...
void MM_ParallelGlobalGC::reportCompactEnd(MM_EnvironmentBase* env)
{
    ...
    TRIGGER_J9HOOK_MM_OMR_COMPACT_END(
        _extensions->omrHookInterface, env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_OMR_COMPACT_END);
}
#endif /* OMR_GC_MODRON_COMPACTION */
