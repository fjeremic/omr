...
void
poisonReferenceSlots(OMR_VMThread* omrVMThread,
...
void
healReferenceSlots(OMR_VMThread* omrVMThread,
                   MM_HeapRegionDescriptor* region,
                   omrobjectptr_t object,
                   void* userData)
{
...
static void
globalGCHookAFCycleStart(J9HookInterface** hook,
                         uintptr_t eventNum,
                         void* eventData,
                         void* userData);
static void
globalGCHookAFCycleEnd(J9HookInterface** hook,
                       uintptr_t eventNum,
                       void* eventData,
                       void* userData);
#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
static void
globalGCHookCCStart(J9HookInterface** hook,
                    uintptr_t eventNum,
                    void* eventData,
                    void* userData);
static void
globalGCHookCCEnd(J9HookInterface** hook,
                  uintptr_t eventNum,
                  void* eventData,
                  void* userData);
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */
static void
globalGCHookSysStart(J9HookInterface** hook,
                     uintptr_t eventNum,
                     void* eventData,
                     void* userData);
static void
globalGCHookSysEnd(J9HookInterface** hook,
                   uintptr_t eventNum,
                   void* eventData,
                   void* userData);
 ...
 */
static void
fixObject(OMR_VMThread* omrVMThread,
          MM_HeapRegionDescriptor* region,
          omrobjectptr_t object,
          void* userData)
{
 ...
 */
static void
hookGlobalGcSweepEndRsoSafetyFixHeap(J9HookInterface** hook,
                                     uintptr_t eventNum,
                                     void* eventData,
                                     void* userData)
{
  ...
  if (!extensions->scavengerRsoScanUnsafe) {
    ...
    pggc->fixHeapForWalk(
      env, MEMORY_TYPE_OLD_RAM, FIXUP_DEBUG_TOOLING, fixObject);
  }
}

#if defined(OMR_GC_CONCURRENT_SCAVENGER)
static void
hookGlobalGcSweepEndAbortedCSFixHeap(J9HookInterface** hook,
                                     UDATA eventNum,
                                     void* eventData,
                                     void* userData)
{
...
MM_ParallelGlobalGC::initialize(MM_EnvironmentBase* env)
{
  ...
  _heapWalker =
    MM_ParallelHeapWalker::newInstance(this, _markingScheme->getMarkMap(), env);
  if (NULL == _heapWalker) {
    goto error_no_memory;
  }
  ...
  if (_extensions->scavengerEnabled) {
    ...
    (*mmPrivateHooks)
      ->J9HookRegisterWithCallSite(mmPrivateHooks,
                                   ...
                                   hookGlobalGcSweepEndRsoSafetyFixHeap,
                                   OMR_GET_CALLSITE(),
                                   this);
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    if (_extensions->isConcurrentScavengerEnabled()) {
      (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(mmPrivateHooks,
                                     J9HOOK_MM_PRIVATE_SWEEP_END,
                                     hookGlobalGcSweepEndAbortedCSFixHeap,
                                     OMR_GET_CALLSITE(),
                                     this);
    }
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
 ...
 */
void
MM_ParallelGlobalGC::cleanupAfterGC(MM_EnvironmentBase* env,
...
#if defined(OMR_GC_MODRON_COMPACTION)
bool
MM_ParallelGlobalGC::shouldCompactThisCycle(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocDescription,
  ...
  MM_GCCode gcCode)
{
...

  {
    ...
    float darkMatterRatio =
      ((float)darkMatterBytes) / ((float)freeMemorySize + (float)totalSize / 2);
 ...
 */
bool
MM_ParallelGlobalGC::compactRequiredBeforeHeapContraction(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocDescription,
...
void
MM_ParallelGlobalGC::sweep(MM_EnvironmentBase* env,
                           MM_AllocateDescription* allocDescription,
                           bool rebuildMarkBits)
{
  ...
  _compactThisCycle =
    shouldCompactThisCycle(env,
                           allocDescription,
                           activeSubSpace->maxExpansionInSpace(env),
...
bool
MM_ParallelGlobalGC::completeFreelistRebuildRequired(
  MM_EnvironmentBase* env,
...
MM_ParallelGlobalGC::markAll(MM_EnvironmentBase* env, bool initMarkMap)
{
  ...
  /* run the mark */
  MM_ParallelMarkTask markTask(
    env, _dispatcher, _markingScheme, initMarkMap, env->_cycleState);
...
void
MM_ParallelGlobalGC::masterThreadSweepStart(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocDescription)
{
  _sweepScheme->setMarkMap(_markingScheme->getMarkMap());
  _sweepScheme->sweepForMinimumSize(
    env, env->_cycleState->_activeSubSpace, allocDescription);
}

void
MM_ParallelGlobalGC::masterThreadSweepComplete(MM_EnvironmentBase* env,
                                               SweepCompletionReason reason)
{
...
#if defined(OMR_GC_MODRON_COMPACTION)
void
MM_ParallelGlobalGC::masterThreadCompact(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocDescription,
  bool rebuildMarkBits)
{
...
void
MM_ParallelGlobalGC::masterThreadRestartAllocationCaches(
  MM_EnvironmentBase* env)
{
...
void
MM_ParallelGlobalGC::internalPostCollect(MM_EnvironmentBase* env,
                                         MM_MemorySubSpace* subSpace)
{
...
void
MM_ParallelGlobalGC::processLargeAllocateStatsAfterCompact(
  MM_EnvironmentBase* env)
{
  processLargeAllocateStatsAfterSweep(env);
}
...
void
MM_ParallelGlobalGC::processLargeAllocateStatsAfterSweep(
  MM_EnvironmentBase* env)
{
  MM_MemorySpace* defaultMemorySpace =
    _extensions->heap->getDefaultMemorySpace();
...
bool
MM_ParallelGlobalGC::internalGarbageCollect(
  MM_EnvironmentBase* env,
  MM_MemorySubSpace* subSpace,
  MM_AllocateDescription* allocDescription)
{
 ...
 */
uintptr_t
MM_ParallelGlobalGC::fixHeapForWalk(MM_EnvironmentBase* env,
                                    UDATA walkFlags,
                                    ...
                                    MM_HeapWalkerObjectFunc walkFunction)
{
  ...
  _heapWalker->allObjectsDo(
    env, walkFunction, &fixedObjectCount, walkFlags, true, false);
 ...
 */
bool
MM_ParallelGlobalGC::heapAddRange(MM_EnvironmentBase* env,
                                  MM_MemorySubSpace* subspace,
                                  uintptr_t size,
                                  void* lowAddress,
                                  void* highAddress)
{
  bool result =
    _markingScheme->heapAddRange(env, subspace, size, lowAddress, highAddress);
  if (0 == result) {
...
#if defined(OMR_GC_OBJECT_MAP)
  result = _extensions->getObjectMap()->heapAddRange(
...
#if defined(OMR_GC_OBJECT_MAP)
  _extensions->getObjectMap()->heapRemoveRange(
    env, subspace, size, lowAddress, highAddress, NULL, NULL);
objectMap_failed_heapAddRange:
#endif /* defined(OMR_GC_OBJECT_MAP) */
  _sweepScheme->heapRemoveRange(
    env, subspace, size, lowAddress, highAddress, NULL, NULL);
sweepScheme_failed_heapAddRange:
  _markingScheme->heapRemoveRange(
    env, subspace, size, lowAddress, highAddress, NULL, NULL);
 ...
 */
bool
MM_ParallelGlobalGC::heapRemoveRange(MM_EnvironmentBase* env,
                                     MM_MemorySubSpace* subspace,
                                     uintptr_t size,
                                     void* lowAddress,
                                     void* highAddress,
...
{
  bool result = _markingScheme->heapRemoveRange(env,
                                                subspace,
                                                size,
                                                lowAddress,
                                                highAddress,
                                                lowValidAddress,
                                                highValidAddress);
  result = result && _sweepScheme->heapRemoveRange(env,
                                                   subspace,
                                                   size,
                                                   lowAddress,
                                                   highAddress,
  ...
  result = result && _delegate.heapRemoveRange(env,
                                               subspace,
                                               size,
                                               lowAddress,
                                               highAddress,
 ...
 */
void*
MM_ParallelGlobalGC::createSweepPoolState(MM_EnvironmentBase* env,
 ...
 */
void
MM_ParallelGlobalGC::deleteSweepPoolState(MM_EnvironmentBase* env,
                                          void* sweepPoolState)
{
  Assert_MM_true(NULL != _sweepScheme);
...
MM_ParallelGlobalGC::reportMarkStart(MM_EnvironmentBase* env)
{
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_MARK_START(_extensions->privateHookInterface,
                                       env->getOmrVMThread(),
                                       omrtime_hires_clock(),
...
MM_ParallelGlobalGC::reportMarkEnd(MM_EnvironmentBase* env)
{
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_MARK_END(_extensions->privateHookInterface,
                                     env->getOmrVMThread(),
                                     omrtime_hires_clock(),
...
MM_ParallelGlobalGC::reportSweepStart(MM_EnvironmentBase* env)
{
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_SWEEP_START(_extensions->privateHookInterface,
                                        env->getOmrVMThread(),
                                        omrtime_hires_clock(),
...
MM_ParallelGlobalGC::reportSweepEnd(MM_EnvironmentBase* env)
{
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_SWEEP_END(_extensions->privateHookInterface,
                                      env->getOmrVMThread(),
                                      omrtime_hires_clock(),
                                      J9HOOK_MM_PRIVATE_SWEEP_END);
}
...
MM_ParallelGlobalGC::reportCompactStart(MM_EnvironmentBase* env)
{
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_COMPACT_START(_extensions->privateHookInterface,
                                          env->getOmrVMThread(),
                                          omrtime_hires_clock(),
...
MM_ParallelGlobalGC::reportCompactEnd(MM_EnvironmentBase* env)
{
  ...
  TRIGGER_J9HOOK_MM_OMR_COMPACT_END(_extensions->omrHookInterface,
                                    env->getOmrVMThread(),
                                    omrtime_hires_clock(),
