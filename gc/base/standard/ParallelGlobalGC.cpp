...
void poisonReferenceSlots(OMR_VMThread *omrVMThread,
                          MM_HeapRegionDescriptor *region,
                          omrobjectptr_t object, void *userData) {
...
void healReferenceSlot(MM_EnvironmentBase *env, GC_SlotObject *slotObject) {
  ...
  if ((shadowHeapTop > referenceFromSlot) &&
      (shadowHeapBase <= referenceFromSlot)) {
    uintptr_t heapBase = (uintptr_t)extensions->heap->getHeapBase();
    uintptr_t healedHeapAddress =
        heapBase + (referenceFromSlot - shadowHeapBase);
...
void healReferenceSlots(OMR_VMThread *omrVMThread,
                        MM_HeapRegionDescriptor *region, omrobjectptr_t object,
                        void *userData) {
  GC_ObjectIterator objectIterator(omrVMThread->_vm, object);
  GC_SlotObject *slotObject = NULL;
...
static void globalGCHookAFCycleStart(J9HookInterface **hook, uintptr_t eventNum,
                                     void *eventData, void *userData);
static void globalGCHookAFCycleEnd(J9HookInterface **hook, uintptr_t eventNum,
                                   void *eventData, void *userData);
#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
static void globalGCHookCCStart(J9HookInterface **hook, uintptr_t eventNum,
                                void *eventData, void *userData);
static void globalGCHookCCEnd(J9HookInterface **hook, uintptr_t eventNum,
                              void *eventData, void *userData);
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */
static void globalGCHookSysStart(J9HookInterface **hook, uintptr_t eventNum,
                                 void *eventData, void *userData);
static void globalGCHookSysEnd(J9HookInterface **hook, uintptr_t eventNum,
                               void *eventData, void *userData);

/**
 * Function to fix single dead object on the heap
 *
 * NOTE that this function is expected to represent the SUPERSET of dead object
 * fixup needs in that it will fix all objects which are known to be dead.  Any
 * optimizations to this function must be careful not to violate that meaning
 * (see CMVC 122959 for an example of such a mistake).
 */
static void fixObject(OMR_VMThread *omrVMThread,
                      MM_HeapRegionDescriptor *region, omrobjectptr_t object,
                      void *userData) {
  MM_GCExtensionsBase *extensions =
      MM_GCExtensionsBase::getExtensions(omrVMThread->_vm);
  MM_ParallelGlobalGC *collector =
      (MM_ParallelGlobalGC *)extensions->getGlobalCollector();
  /* Check the mark state of each object. If it isn't marked, build a dead
   * object */
  if (!collector->getMarkingScheme()->isMarked(object)) {
     ...
     * This could have been done directly inside internalRecycleHeapChunk
     * (MemoryPoolAddressOrderedListBase.hpp) but due to current limitation of
     * API that requires address of object to be freed which we need to check
     * from a set stored in extensions, which weren't further passed we will
     * check it here. But in case new API is added, it is better to move there.
     */
    if (valgrindCheckObjectInPool(extensions, (uintptr_t)object))
      valgrindFreeObject(extensions, (uintptr_t)object);
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
    memorySubSpace->abandonHeapChunk(object,
                                     ((U_8 *)object) + deadObjectByteSize);
    /* the userdata is a counter of dead objects fixed up so increment it here
     * as a uintptr_t */
 ...
 */
static void hookGlobalGcSweepEndRsoSafetyFixHeap(J9HookInterface **hook,
                                                 uintptr_t eventNum,
                                                 void *eventData,
                                                 void *userData) {
  ...
  extensions->scavengerRsoScanUnsafe =
      !extensions->isRememberedSetInOverflowState();
  if (!extensions->scavengerRsoScanUnsafe) {
    MM_ParallelGlobalGC *pggc = (MM_ParallelGlobalGC *)userData;
    pggc->fixHeapForWalk(env, MEMORY_TYPE_OLD_RAM, FIXUP_DEBUG_TOOLING,
                         fixObject);
  }
}

#if defined(OMR_GC_CONCURRENT_SCAVENGER)
static void hookGlobalGcSweepEndAbortedCSFixHeap(J9HookInterface **hook,
                                                 UDATA eventNum,
                                                 void *eventData,
                                                 void *userData) {
  MM_SweepEndEvent *event = (MM_SweepEndEvent *)eventData;
  MM_EnvironmentStandard *env =
      MM_EnvironmentStandard::getEnvironment(event->currentThread);
  MM_GCExtensionsBase *extensions = env->getExtensions();
  ...
  if (extensions->isScavengerBackOutFlagRaised()) {
    GC_HeapRegionIteratorStandard regionIterator(
        extensions->getHeap()->getHeapRegionManager());
      ...
      if (MEMORY_TYPE_NEW == (region->getTypeFlags() & MEMORY_TYPE_NEW)) {
        ...
        MM_HeapMapIterator markedObjectIterator(
            ...
            ((MM_ParallelGlobalGC *)extensions->getGlobalCollector())
                ->getMarkingScheme()
                ->getMarkMap(),
            (UDATA *)lowAddress, (UDATA *)highAddress);
        void *prevEndObjectPtr = lowAddress;
        omrobjectptr_t objectPtr = NULL;
        while (NULL != (objectPtr = markedObjectIterator.nextObject())) {
          UDATA objectSize =
              extensions->objectModel.getConsumedSizeInBytesWithHeader(
                  objectPtr);
          if (prevEndObjectPtr != objectPtr) {
            region->getSubSpace()->abandonHeapChunk(prevEndObjectPtr,
                                                    objectPtr);
            holeCount += 1;
          }
 ...
 * Initialization
 */
MM_ParallelGlobalGC *MM_ParallelGlobalGC::newInstance(MM_EnvironmentBase *env) {
  MM_ParallelGlobalGC *globalGC;

  globalGC = (MM_ParallelGlobalGC *)env->getForge()->allocate(
      sizeof(MM_ParallelGlobalGC), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (globalGC) {
...
void MM_ParallelGlobalGC::kill(MM_EnvironmentBase *env) {
  tearDown(env);
 ...
 */
bool MM_ParallelGlobalGC::initialize(MM_EnvironmentBase *env) {
  J9HookInterface **mmPrivateHooks =
      J9_HOOK_INTERFACE(_extensions->privateHookInterface);
  ...
  _heapWalker = MM_ParallelHeapWalker::newInstance(
      this, _markingScheme->getMarkMap(), env);
  if (NULL == _heapWalker) {
    goto error_no_memory;
  }
  ...
  (*mmPrivateHooks)
      ->J9HookRegisterWithCallSite(
          mmPrivateHooks, J9HOOK_MM_PRIVATE_ALLOCATION_FAILURE_CYCLE_START,
          globalGCHookAFCycleStart, OMR_GET_CALLSITE(), NULL);
  (*mmPrivateHooks)
      ->J9HookRegisterWithCallSite(
          mmPrivateHooks, J9HOOK_MM_PRIVATE_ALLOCATION_FAILURE_CYCLE_END,
          globalGCHookAFCycleEnd, OMR_GET_CALLSITE(), NULL);

#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
  (*mmPrivateHooks)
      ->J9HookRegisterWithCallSite(
          mmPrivateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START,
          globalGCHookCCStart, OMR_GET_CALLSITE(), NULL);
  (*mmPrivateHooks)
      ->J9HookRegisterWithCallSite(mmPrivateHooks,
                                   J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END,
                                   globalGCHookCCEnd, OMR_GET_CALLSITE(), NULL);
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */

  (*mmPrivateHooks)
      ->J9HookRegisterWithCallSite(
          mmPrivateHooks, J9HOOK_MM_PRIVATE_SYSTEM_GC_START,
          globalGCHookSysStart, OMR_GET_CALLSITE(), NULL);
  (*mmPrivateHooks)
      ->J9HookRegisterWithCallSite(
          mmPrivateHooks, J9HOOK_MM_PRIVATE_SYSTEM_GC_END, globalGCHookSysEnd,
          OMR_GET_CALLSITE(), NULL);

#if defined(OMR_GC_MODRON_SCAVENGER)
  if (_extensions->scavengerEnabled) {
    /* Hook the global collector to guarantee heap walk safety in the event of
     * an RSO */
    /* NOTE: This will be a one time hook across all instances as the function
     * and user data will be identical - i.e., we will only get one Hook
     * registered no matter how many scavengers are created/initialized
     */
    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(
            mmPrivateHooks, J9HOOK_MM_PRIVATE_SWEEP_END,
            hookGlobalGcSweepEndRsoSafetyFixHeap, OMR_GET_CALLSITE(), this);
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    if (_extensions->isConcurrentScavengerEnabled()) {
      (*mmPrivateHooks)
          ->J9HookRegisterWithCallSite(
              mmPrivateHooks, J9HOOK_MM_PRIVATE_SWEEP_END,
              hookGlobalGcSweepEndAbortedCSFixHeap, OMR_GET_CALLSITE(), this);
    }
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
  }
#endif /* OMR_GC_MODRON_SCAVENGER */
 ...
 */
void MM_ParallelGlobalGC::tearDown(MM_EnvironmentBase *env) {
...
uintptr_t MM_ParallelGlobalGC::getVMStateID() {
 ...
 ****************************************
 */
void MM_ParallelGlobalGC::cleanupAfterGC(
  ...
  /* Null tenure TLH (Copy Cache) references for all GC slave and Mutator (for
   * concurrent scavenger) threads as the memory will be invalidated on sweep
   * cycle*/
...
void MM_ParallelGlobalGC::masterThreadGarbageCollect(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
  ...
  if (_compactThisCycle) {
    ...
    CompactPreventedReason compactPreventedReason = (CompactPreventedReason)(
  ...
  /* If the J9VM_DEBUG_ATTRIBUTE_ALLOW_USER_HEAP_WALK flag is set then fix the
   * heap so that it can be walked by debugging tools
   */
  if (_delegate.isAllowUserHeapWalk() ||
      env->_cycleState->_gcCode.isRASDumpGC()) {
    if (!_fixHeapForWalkCompleted) {
      ...
      if (compactedThisCycle) {
        ...
        _extensions->globalGCStats.fixHeapForWalkTime =
            omrtime_hires_delta(startTime, omrtime_hires_clock(),
      ...
      /* since this is the superset of all walk operations, we can safely set
       * the flag that states other walks can be omitted for this cycle as
       * redundant (CMVC 122959)
       */
  ...
  if (compactedThisCycle) {
     ...
     * Concurrent Scavenger requires this be done after fixup heap for walk
     * pass.
     */
    env->_cycleState->_activeSubSpace->checkResize(
        env, allocDescription, env->_cycleState->_gcCode.isExplicitGC());
  }
#endif
...
#if defined(OMR_GC_MODRON_COMPACTION)
bool MM_ParallelGlobalGC::shouldCompactThisCycle(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
  ...
  if ((_extensions->compactOnIdle) &&
      (J9MMCONSTANT_EXPLICIT_GC_IDLE_GC == gcCode.getCode())) {
    compactReason = COMPACT_FORCED_GC;
    goto compactionReqd;
  }
#endif

  /* RAS dump compact requests override all other options. If a dump agent
   * requested a compact we always honour it in order to produce optimal heap
   * dumps
   */
  ...
  if (allocDescription) {
    ...
    uintptr_t largestFreeEntry =
        memorySpace->findLargestFreeEntry(env, allocDescription);
  ...
  /* If -Xgc:compactToSatisfyAllocate has been specified, then we skip the rest
   * of the triggers */
  ...
  if (_extensions->scavengerEnabled) {
    ...
    uintptr_t failedTenureLargest =
        _extensions->scavengerStats._failedTenureLargest;
    if (failedTenureLargest > 0) {
      MM_MemorySpace *memorySpace = env->getMemorySpace();
      MM_AllocateDescription tenureAllocDescription(
          failedTenureLargest, OMR_GC_ALLOCATE_OBJECT_TENURED, false, true);
      uintptr_t largestTenureFreeEntry =
          memorySpace->findLargestFreeEntry(env, &tenureAllocDescription);
  ...
  if (gcCode.isAggressiveGC() &&
      (_extensions->globalGCStats.compactStats._lastHeapCompaction + 1 <
   ...
   */
  totalBytesAllocated =
      allocStats->_allocationBytes + allocStats->_tlhAllocatedFresh;

  /* ..and what percentage of allocations were tlh's */
  tlhPercent =
      allocStats->_tlhRefreshCountFresh > 0
          ? (uintptr_t)(((uint64_t)allocStats->_tlhAllocatedFresh * 100) /
                        (uint64_t)totalBytesAllocated)
          : 0;

  /* Check at least 50% of free space at end of last GC has been consumed by tlh
   * allocations */
  if (tlhPercent > 50) {
    /* Calculate average size of tlh allocated since tenure area last collected
     */
    uintptr_t avgTlh =
        allocStats->_tlhAllocatedFresh / allocStats->_tlhRefreshCountFresh;

    /* Compaction trigger is a multiple of the minimum tlh size */
    uintptr_t compaction_trigger_avgtlh =
        _extensions->tlhMinimumSize * MINIMUM_TLHSIZE_MULTIPLIER;
   ...
   *
   * So if we are not fully expanded but we are low on storage there is little
   * point doing a compaction, we just need to expand. However, if we are fully
   * expanded but low on free memory there is little more we can do to avoid OOM
   * at this point other than compact to get as much free memory as possible.
   */
  if (activeSubspaceMaxExpansionInSpace == 0) {
    uintptr_t oldFree =
        _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_OLD);
    uintptr_t oldSize = _extensions->heap->getActiveMemorySize(MEMORY_TYPE_OLD);
    uintptr_t desperateFree = (oldSize / OLDFREE_DESPERATE_RATIO_DIVISOR) *
                              OLDFREE_DESPERATE_RATIO_MULTIPLIER;
...

  {
    ...
    MM_MemorySubSpace *memorySubSpace =
        _extensions->heap->getDefaultMemorySpace()->getTenureMemorySubSpace();
    ...
    float darkMatterRatio = ((float)darkMatterBytes) /
                            ((float)freeMemorySize + (float)totalSize / 2);
...
#if defined(OMR_GC_IDLE_HEAP_MANAGER)
    if ((J9MMCONSTANT_EXPLICIT_GC_IDLE_GC == gcCode.getCode()) &&
        (_extensions->gcOnIdle)) {
      ...
      float totalFragmentationRatio =
          ((float)totalFragmentation) /
          ((float)freeMemorySize + (float)totalSize / 2);

      Trc_ParallelGlobalGC_shouldCompactThisCycle(
          env->getLanguageVMThread(), totalFragmentationRatio,
          _extensions->gcOnIdleCompactThreshold);
 ...
 */
bool MM_ParallelGlobalGC::compactRequiredBeforeHeapContraction(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
  ...
  if (env->_cycleState->_gcCode.isExplicitGC() &&
      _extensions->nocompactOnSystemGC) {
  ...
  if (0 != actualSoftMx) {
    if (actualSoftMx < _extensions->heap->getActiveMemorySize()) {
       ...
       * highly likely we'll need to compact in order to meet the contract, so
       * do it */
      goto compactionReqd;
    }
  }
   ...
   *
   * - no compaction performed this GC (if we get here then we have decided not
   * to compact for other reasons so non eed to check that)
   * - the last GC did not do a shrink and compact
   * - there is no free chunk at the end of the heap or the chunk at the end of
   * the heap is less than 10% of required shrink size
   */
  if ((_extensions->globalGCStats.compactStats._lastHeapCompaction + 1 ==
       _extensions->globalGCStats.gcCount) &&
      (_extensions->heap->getResizeStats()->getLastHeapContractionGCCount() +
           1 ==
       _extensions->globalGCStats.gcCount)) {
    return false;
  }
   ...
   */
  lengthLastFree =
      env->_cycleState->_activeSubSpace->getAvailableContractionSize(
          env, allocDescription);
  ...
  if (lengthLastFree > 0) {
    uintptr_t minContractSize =
        (contractionSize / MINIMUM_CONTRACTION_RATIO_DIVISOR) *
...
compactionReqd:
  _extensions->globalGCStats.compactStats._compactPreventedReason =
      _delegate.checkIfCompactionShouldBePrevented(env);
  if (COMPACT_PREVENTED_NONE !=
      _extensions->globalGCStats.compactStats._compactPreventedReason) {
    return false;
  }
...
void MM_ParallelGlobalGC::sweep(MM_EnvironmentBase *env,
...
#if defined(OMR_GC_MODRON_COMPACTION)
  /* Decide is a compaction is required - this decision must be made after we
   * sweep since we use the largestFreeEntrySize, as changed by sweep, to
   * determine if a compaction should be done */
  _compactThisCycle = shouldCompactThisCycle(
      env, allocDescription, activeSubSpace->maxExpansionInSpace(env),
      env->_cycleState->_gcCode);

  if (!_compactThisCycle)
#endif /* OMR_GC_MODRON_COMPACTION */
  {
    /* Decide whether we need to expand or shrink the heap. If the decision is
     * to shrink, we may need to force a compaction to assist the contraction.
     */
    activeSubSpace->checkResize(env, allocDescription, isExplicitGC);
  }

  /* If we need to completely rebuild the freeelist (impending compaction or
   * contraction), then do it */
...
#endif /* OMR_GC_MODRON_COMPACTION */
    {
      /* We now have accurate free space statistics so recalculate any
       * expand/contract amount as it will no doubt have changed
       */
      activeSubSpace->checkResize(env, allocDescription, isExplicitGC);
    }
  }
  ...
  if (0 != activeSubSpace->getContractionSize()) {
    _compactThisCycle = compactRequiredBeforeHeapContraction(
        env, allocDescription, activeSubSpace->getContractionSize());
  }
#endif /* OMR_GC_MODRON_COMPACTION */
...
bool MM_ParallelGlobalGC::completeFreelistRebuildRequired(
...
#endif /* OMR_GC_MODRON_COMPACTION */
      if (activeSubSpace->getActiveLOAMemorySize(MEMORY_TYPE_OLD) > 0 &&
          0 != activeSubSpace->getExpansionSize()) {
    // todo remove once we sort out how to reallocate sweep chunks if heap
    // expands
    // after a concurrent sweep cycle has started but for now we need to
    // complete sweep if current LOA size is > 0.
...
void MM_ParallelGlobalGC::markAll(MM_EnvironmentBase *env, bool initMarkMap) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  ...
  /* run the mark */
  MM_ParallelMarkTask markTask(env, _dispatcher, _markingScheme, initMarkMap,
                               env->_cycleState);
...
void MM_ParallelGlobalGC::masterThreadSweepStart(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription) {
  _sweepScheme->setMarkMap(_markingScheme->getMarkMap());
  _sweepScheme->sweepForMinimumSize(env, env->_cycleState->_activeSubSpace,
                                    allocDescription);
}

void MM_ParallelGlobalGC::masterThreadSweepComplete(
    MM_EnvironmentBase *env, SweepCompletionReason reason) {
  _sweepScheme->completeSweep(env, reason);
}

#if defined(OMR_GC_MODRON_COMPACTION)
void MM_ParallelGlobalGC::masterThreadCompact(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
    bool rebuildMarkBits) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  MM_CompactStats *compactStats = &_extensions->globalGCStats.compactStats;
  ...
  MM_ParallelCompactTask compactTask(
      env, _dispatcher, _compactScheme, rebuildMarkBits,
  ...
  _extensions->globalGCStats.compactStats._lastHeapCompaction =
      _extensions->globalGCStats.gcCount;
}
#endif /* OMR_GC_MODRON_COMPACTION */

void MM_ParallelGlobalGC::masterThreadRestartAllocationCaches(
    MM_EnvironmentBase *env) {
  ...
  while ((walkThread = vmThreadListIterator.nextOMRVMThread()) != NULL) {
    ...
    /* CMVC 123281: setThreadScanned(false) is called here because the
     * concurrent collector uses this as the reset point for all threads scanned
     * state. This should really be moved into the STW thread scan phase.
     */
 ...
 ****************************************
 */
void MM_ParallelGlobalGC::internalPreCollect(
...
void MM_ParallelGlobalGC::tenureMemoryPoolPostCollect(MM_EnvironmentBase *env) {
  ...
  if (_extensions->largeObjectArea && _sweepScheme->isSweepCompleted(env)) {
    /* resize LOA only when the sweep is completed (to avoid concurrent sweep's
     * confusion due to the resize) */
    MM_MemoryPoolLargeObjects *memoryPool =
        (MM_MemoryPoolLargeObjects *)tenureMemorySubspace->getMemoryPool();
    memoryPool->resizeLOA(env);
  }
}

void MM_ParallelGlobalGC::internalPostCollect(MM_EnvironmentBase *env,
  ...
  _extensions->globalGCStats.workPacketStats.setSTWWorkStackOverflowOccured(
      false);
  _extensions->allocationStats.clear();
  _extensions->setLastGlobalGCFreeBytes(
      _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_OLD));
...
void MM_ParallelGlobalGC::processLargeAllocateStatsBeforeGC(
    MM_EnvironmentBase *env) {
  MM_MemorySpace *defaultMemorySpace =
      _extensions->heap->getDefaultMemorySpace();
  ...
  /* TODO: average LargeObjectAllocateStats for allocation form mutators in
   * ScavengerEnabled case */
#if defined(OMR_GC_MODRON_SCAVENGER)
  if (!_extensions->scavengerEnabled) {
#endif /* defined(OMR_GC_MODRON_SCAVENGER) */
    memoryPool->averageLargeObjectAllocateStats(
        env, _extensions->allocationStats.bytesAllocated());
#if defined(OMR_GC_MODRON_SCAVENGER)
  }
#endif /* defined(OMR_GC_MODRON_SCAVENGER) */

  memoryPool->getLargeObjectAllocateStats()->setTimeMergeAverage(
...
void MM_ParallelGlobalGC::processLargeAllocateStatsAfterCompact(
    MM_EnvironmentBase *env) {
  processLargeAllocateStatsAfterSweep(env);
}

void MM_ParallelGlobalGC::processLargeAllocateStatsAfterSweep(
    MM_EnvironmentBase *env) {
  MM_MemorySpace *defaultMemorySpace =
      _extensions->heap->getDefaultMemorySpace();
  MM_MemorySubSpace *tenureMemorySubspace =
...
void MM_ParallelGlobalGC::reportGCCycleFinalIncrementEnding(
    MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
...
void MM_ParallelGlobalGC::postMark(MM_EnvironmentBase *env) {
  _markingScheme->getMarkMap()->setMarkMapValid(true);
}

void MM_ParallelGlobalGC::setupForGC(MM_EnvironmentBase *env) {}

bool MM_ParallelGlobalGC::internalGarbageCollect(
    MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
    MM_AllocateDescription *allocDescription) {
  ...
  if (_disableGC) {
    env->_cycleState->_activeSubSpace->checkResize(env, allocDescription,
                                                   false);
    env->_cycleState->_activeSubSpace->performResize(env, allocDescription);
  } else {
 ...
 */
void MM_ParallelGlobalGC::prepareHeapForWalk(MM_EnvironmentBase *env) {
  GC_OMRVMInterface::flushCachesForGC(env);
...
/**
 * Fixing the heap to be walkable involves finding all objects NOT marked and
 * converting them to dark matter. For the most part, a heap is usually walkable
 * (objects that are dead can be walked, but their fields are suspect). However,
 * after a class unload, there is no way to skip over a dead object, as the
 * class pointer, which contains sizing information, is now a dangling pointer.
 *
 * Note that this represents a superset of walk requirements and will call
 * fixObject, the most generic of the fixup routines.  Other fixup operations or
 * optimizations to the fixup process should not assume that they can modify or
 * replace this function since there are callers which assume that this will fix
 * up all dead objects and that contract must be preserved as well as the rule
 * that this be a superset of all fixup operations (see CMVC 122959).
 */
uintptr_t MM_ParallelGlobalGC::fixHeapForWalk(
    MM_EnvironmentBase *env, UDATA walkFlags, uintptr_t walkReason,
    MM_HeapWalkerObjectFunc walkFunction) {
  ...
  _heapWalker->allObjectsDo(env, walkFunction, &fixedObjectCount, walkFlags,
                            true, false);

  _extensions->globalGCStats.fixHeapForWalkTime = omrtime_hires_delta(
      startTime, omrtime_hires_clock(), OMRPORT_TIME_DELTA_IN_MICROSECONDS);
 ...
 */
bool MM_ParallelGlobalGC::heapAddRange(MM_EnvironmentBase *env,
                                       MM_MemorySubSpace *subspace,
                                       uintptr_t size, void *lowAddress,
                                       void *highAddress) {
  bool result = _markingScheme->heapAddRange(env, subspace, size, lowAddress,
                                             highAddress);
  if (0 == result) {
  ...
  result =
      _sweepScheme->heapAddRange(env, subspace, size, lowAddress, highAddress);
  if (0 == result) {
    goto sweepScheme_failed_heapAddRange;
  }

#if defined(OMR_GC_OBJECT_MAP)
  result = _extensions->getObjectMap()->heapAddRange(env, subspace, size,
...
#if defined(OMR_GC_OBJECT_MAP)
  _extensions->getObjectMap()->heapRemoveRange(env, subspace, size, lowAddress,
                                               highAddress, NULL, NULL);
objectMap_failed_heapAddRange:
#endif /* defined(OMR_GC_OBJECT_MAP) */
  _sweepScheme->heapRemoveRange(env, subspace, size, lowAddress, highAddress,
                                NULL, NULL);
sweepScheme_failed_heapAddRange:
  _markingScheme->heapRemoveRange(env, subspace, size, lowAddress, highAddress,
                                  NULL, NULL);
 ...
 */
bool MM_ParallelGlobalGC::heapRemoveRange(MM_EnvironmentBase *env,
                                          MM_MemorySubSpace *subspace,
                                          uintptr_t size, void *lowAddress,
                                          void *highAddress,
                                          void *lowValidAddress,
                                          void *highValidAddress) {
  bool result = _markingScheme->heapRemoveRange(env, subspace, size, lowAddress,
                                                highAddress, lowValidAddress,
                                                highValidAddress);
  result = result && _sweepScheme->heapRemoveRange(
                         env, subspace, size, lowAddress, highAddress,
                         lowValidAddress, highValidAddress);

  result = result && _delegate.heapRemoveRange(env, subspace, size, lowAddress,
                                               highAddress, lowValidAddress,
                                               highValidAddress);
 ...
 */
void MM_ParallelGlobalGC::heapReconfigured(MM_EnvironmentBase *env) {
  _sweepScheme->heapReconfigured(env);
}

bool MM_ParallelGlobalGC::collectorStartup(MM_GCExtensionsBase *extensions) {
#if defined(OMR_GC_MODRON_SCAVENGER)
...
void MM_ParallelGlobalGC::collectorShutdown(MM_GCExtensionsBase *extensions) {
#if defined(OMR_GC_MODRON_SCAVENGER)
  if (extensions->scavengerEnabled && (NULL != extensions->scavenger)) {
 ...
 */
uint32_t MM_ParallelGlobalGC::getGCTimePercentage(MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions =
...
static void globalGCHookSysEnd(J9HookInterface **hook, uintptr_t eventNum,
  ...
  /* Save end time so at next AF we can get a realistic time outside gc, while
   * it will never be used it may be useful for debugging. */
  extensions->heap->getResizeStats()->setLastAFEndTime(omrtime_hires_clock());
}

static void globalGCHookAFCycleStart(J9HookInterface **hook, uintptr_t eventNum,
                                     void *eventData, void *userData) {
  MM_AllocationFailureCycleStartEvent *event =
      (MM_AllocationFailureCycleStartEvent *)eventData;
  OMR_VMThread *omrVMThread = event->currentThread;
  MM_GCExtensionsBase *extensions =
      MM_GCExtensionsBase::getExtensions(omrVMThread->_vm);
  OMRPORT_ACCESS_FROM_OMRVMTHREAD(omrVMThread);
  ...
  extensions->heap->getResizeStats()->setGlobalGCCountAtAF(
      extensions->globalGCStats.gcCount);
}

static void globalGCHookAFCycleEnd(J9HookInterface **hook, uintptr_t eventNum,
                                   void *eventData, void *userData) {
  MM_AllocationFailureCycleEndEvent *event =
      (MM_AllocationFailureCycleEndEvent *)eventData;
  OMR_VMThread *omrVMThread = event->currentThread;
  MM_GCExtensionsBase *extensions =
      MM_GCExtensionsBase::getExtensions(omrVMThread->_vm);
  OMRPORT_ACCESS_FROM_OMRVMTHREAD(omrVMThread);
  ...
  if ((event->subSpaceType == MEMORY_TYPE_NEW) &&
      ((extensions->heap->getResizeStats()->getGlobalGCCountAtAF()) ==
       (extensions->globalGCStats.gcCount))) {
    return;
  }
  ...
  /* If we have contracted and compacted on this GC then reset ratio ticks as
   * compact will obviously result in a large increase in time in GC and could
   * result in an unexpected/undesirable ratio EXPAND on next GC
   */
#if defined(OMR_GC_MODRON_COMPACTION)
  if (extensions->globalGCStats.compactStats._lastHeapCompaction ==
          extensions->globalGCStats.gcCount &&
      extensions->heap->getResizeStats()->getLastHeapContractionGCCount() ==
          extensions->globalGCStats.gcCount) {
    extensions->heap->getResizeStats()->resetRatioTicks();
  } else {
#endif /* OMR_GC_MODRON_COMPACTION */
 ...
 */
void *MM_ParallelGlobalGC::createSweepPoolState(MM_EnvironmentBase *env,
 ...
 */
void MM_ParallelGlobalGC::deleteSweepPoolState(MM_EnvironmentBase *env,
                                               void *sweepPoolState) {
  Assert_MM_true(NULL != _sweepScheme);
  _sweepScheme->deleteSweepPoolState(env, sweepPoolState);
}

bool MM_ParallelGlobalGC::isMarked(void *objectPtr) {
  return _markingScheme->getMarkMap()->isBitSet(
      static_cast<omrobjectptr_t>(objectPtr));
}

void MM_ParallelGlobalGC::completeExternalConcurrentCycle(
    MM_EnvironmentBase *env) {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  if (_extensions->isConcurrentScavengerEnabled()) {
    /* ParallelGlobalGC or ConcurrentGC (STW phase) cannot start before
     * Concurrent Scavenger cycle is in progress */
    _extensions->scavenger->completeConcurrentCycle(env);
  }
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
}

void MM_ParallelGlobalGC::reportGCCycleStart(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  MM_CommonGCData commonData;
...
void MM_ParallelGlobalGC::reportGCCycleEnd(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  MM_CommonGCData commonData;
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_GC_POST_CYCLE_END(
      ...
      _extensions->getHeap()->initializeCommonGCData(env, &commonData),
      env->_cycleState->_type,
      _extensions->globalGCStats.workPacketStats
...
void MM_ParallelGlobalGC::reportGCStart(MM_EnvironmentBase *env) {
  ...
  Trc_MM_GlobalGCStart(env->getLanguageVMThread(),
                       _extensions->globalGCStats.gcCount);
  Trc_OMRMM_GlobalGCStart(env->getOmrVMThread(),
                          _extensions->globalGCStats.gcCount);

  TRIGGER_J9HOOK_MM_OMR_GLOBAL_GC_START(
      _extensions->omrHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_OMR_GLOBAL_GC_START,
      _extensions->globalGCStats.gcCount, scavengerCount,
      env->_cycleState->_gcCode.isExplicitGC() ? 1 : 0,
      env->_cycleState->_gcCode.isAggressiveGC() ? 1 : 0, _bytesRequested);
}

void MM_ParallelGlobalGC::reportGCIncrementStart(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ParallelGlobalGC::reportGCIncrementEnd(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  MM_CollectionStatisticsStandard *stats =
      (MM_CollectionStatisticsStandard *)
          env->_cycleState->_collectionStatistics;
...
void MM_ParallelGlobalGC::reportGlobalGCIncrementStart(
    MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  uintptr_t scavengerCount = 0;
#if defined(OMR_GC_MODRON_SCAVENGER)
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_GLOBAL_GC_INCREMENT_START(
      _extensions->privateHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_PRIVATE_GLOBAL_GC_INCREMENT_START,
      _extensions->globalGCStats.gcCount, scavengerCount, _bytesRequested);
}

void MM_ParallelGlobalGC::reportGlobalGCCollectComplete(
    MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ParallelGlobalGC::reportGCEnd(MM_EnvironmentBase *env) {
  ...
  uintptr_t totalActiveMemorySizeTotal =
      newActiveMemorySize + oldActiveMemorySize;
  ...
  /* these are assigned to temporary variable out-of-line since some
   * preprocessors get confused if you have directives in macros */
  uintptr_t approximateActiveFreeMemorySize = 0;
  uintptr_t activeMemorySize = 0;

  TRIGGER_J9HOOK_MM_OMR_GLOBAL_GC_END(
      _extensions->omrHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_OMR_GLOBAL_GC_END,
      _extensions->globalGCStats.workPacketStats
          .getSTWWorkStackOverflowOccured(),
      _extensions->globalGCStats.workPacketStats.getSTWWorkStackOverflowCount(),
      _extensions->globalGCStats.workPacketStats
          .getSTWWorkpacketCountAtOverflow(),
      approximateNewActiveFreeMemorySize, newActiveMemorySize,
      approximateOldActiveFreeMemorySize, oldActiveMemorySize,
...
void MM_ParallelGlobalGC::reportGlobalGCIncrementEnd(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  MM_CommonGCData commonData;
...
void MM_ParallelGlobalGC::reportMarkStart(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ParallelGlobalGC::reportMarkEnd(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_MARK_END(
      _extensions->privateHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_PRIVATE_MARK_END);
}

void MM_ParallelGlobalGC::reportSweepStart(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ParallelGlobalGC::reportSweepEnd(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_SWEEP_END(
      _extensions->privateHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_PRIVATE_SWEEP_END);
}

#if defined(OMR_GC_MODRON_COMPACTION)
void MM_ParallelGlobalGC::reportCompactStart(MM_EnvironmentBase *env) {
  CompactReason compactReason =
      (CompactReason)(_extensions->globalGCStats.compactStats._compactReason);

  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  Trc_MM_CompactStart(env->getLanguageVMThread(),
                      getCompactionReasonAsString(compactReason));
  Trc_OMRMM_CompactStart(env->getOmrVMThread(),
                         getCompactionReasonAsString(compactReason));
...
void MM_ParallelGlobalGC::reportCompactEnd(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  Trc_MM_CompactEnd(env->getLanguageVMThread(),
                    _extensions->globalGCStats.compactStats._movedBytes);
  Trc_OMRMM_CompactEnd(env->getOmrVMThread(),
                       _extensions->globalGCStats.compactStats._movedBytes);

  TRIGGER_J9HOOK_MM_OMR_COMPACT_END(
      _extensions->omrHookInterface, env->getOmrVMThread(),
      omrtime_hires_clock(), J9HOOK_MM_OMR_COMPACT_END);
}
#endif /* OMR_GC_MODRON_COMPACTION */

#if defined(OMR_ENV_DATA64) && defined(OMR_GC_FULL_POINTERS)
void MM_ParallelGlobalGC::poisonHeap(MM_EnvironmentBase *env) {
  /* This will poison only the heap slots */
  _heapWalker->allObjectsDo(env, poisonReferenceSlots, NULL, 0, true, false);
}

void MM_ParallelGlobalGC::healHeap(MM_EnvironmentBase *env) {
