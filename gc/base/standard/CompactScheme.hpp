...
public:
  ...
  MM_CompactMemoryPoolState()
      : _memoryPool(NULL), _freeListHead(NULL), _freeBytes(0), _freeHoles(0),
        _largestFreeEntry(0), _previousFreeEntry(NULL),
...
private:
  ...
  struct SubAreaEntry {
    ...
    volatile uintptr_t currentAction; /**< record the status of the subarea for
                                         parallelization */
...
protected:
  ...
  MM_HeapRegionManager
      *_rootManager; /**< The root region manager which holds the
                        MM_HeapRegionDescriptor instances which manage the
  ...
  SubAreaEntry *_subAreaTable; /**< Reference to the subAreaTable which is
                                  shared data from the SweepHeapSectioning */
...
public:
  /*
   * Page represents space can be marked by one slot(uintptr_t) of Compressed
   * Mark Map In Compressed Mark Map one bit represents twice more space then
   * one bit of regular Mark Map So, sizeof_page should be double of number of
   * bytes represented by one uintptr_t in Mark Map
   */
...
protected:
  ...
  void saveForwardingPtr(class CompactTableEntry &, omrobjectptr_t objectPtr,
                         omrobjectptr_t forwardingPtr, intptr_t &page,
                         intptr_t &counter);

  omrobjectptr_t doCompact(MM_EnvironmentStandard *env,
                           MM_MemorySubSpace *memorySubSpace,
                           omrobjectptr_t start, omrobjectptr_t finish,
                           omrobjectptr_t &deadObject, uintptr_t &objectCount,
                           uintptr_t &byteCount, bool evacuate);
  ...
  void evacuateSubArea(MM_EnvironmentStandard *env,
                       ...
                       SubAreaEntry *subAreaTableEvacuate, intptr_t i,
                       uintptr_t &objectCount, uintptr_t &byteCount,
                       uintptr_t &skippedObjectCount);

  void moveObjects(MM_EnvironmentStandard *env, uintptr_t &objectCount,
   ...
   */
  void fixupSubArea(MM_EnvironmentStandard *env, omrobjectptr_t firstObject,
  ...
  /**
   * Return the bit number in Compressed Mark Map slot responsible for this
   * object Each bit in Compressed Mark Map represents twice more heap bytes
   * then regular mark map Each page represents number of bytes in heap might be
   * covered by one Compressed Mark Map slot (uintptr_t)
   */
   ...
   */
  void rebuildMarkbitsInSubArea(MM_EnvironmentStandard *env,
                                MM_HeapRegionDescriptorStandard *region,
                                SubAreaEntry *subAreaTable, intptr_t i);

  /**
   * Atomically change the currentAction value of the subArea to the specified
   * action. This allows a worker thread to claim responsibility for performing
   * the specified action on the specified area.
   *
   * @param env[in] the current thread
   * @param entry[in] the subArea to change
   * @param newAction the desired action for the subArea
   *
   * @return true if the action was changed, or false if another thread already
   * changed it to newAction
   */
  bool changeSubAreaAction(MM_EnvironmentBase *env, SubAreaEntry *entry,
                           uintptr_t newAction);

public:
  static MM_CompactScheme *newInstance(MM_EnvironmentBase *env,
  ...
  virtual void compact(MM_EnvironmentBase *env, bool rebuildMarkBits,
                       bool aggressive);
  omrobjectptr_t getForwardingPtr(omrobjectptr_t objectPtr) const;
  void flushPool(MM_EnvironmentStandard *env,
                 MM_CompactMemoryPoolState *freeListState);
  ...
  MM_CompactScheme(MM_EnvironmentBase *env, MM_MarkingScheme *markingScheme)
      : MM_BaseVirtual(), _omrVM(env->getOmrVM()),
        ...
        _markingScheme(markingScheme), _markMap(markingScheme->getMarkMap()),
        _subAreaTableSize(0), _subAreaTable(NULL), _delegate() {
