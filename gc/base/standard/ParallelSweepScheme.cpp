 ...
 */
void MM_ParallelSweepTask::run(MM_EnvironmentBase *env) {
  _sweepScheme->internalSweep(env);
}

/**
 * Initialize sweep statistics per work thread at the beginning of the sweep
 * task.
 */
void MM_ParallelSweepTask::setup(MM_EnvironmentBase *env) {
...
/**
 * Gather sweep statistics into the global statistics counter at the end of the
 * sweep task.
 */
void MM_ParallelSweepTask::cleanup(MM_EnvironmentBase *env) {
  ...
  Trc_MM_ParallelSweepTask_parallelStats(
      ...
      (uint32_t)omrtime_hires_delta(0, env->_sweepStats.idleTime,
                                    OMRPORT_TIME_DELTA_IN_MILLISECONDS),
      env->_sweepStats.sweepChunksProcessed,
      (uint32_t)omrtime_hires_delta(0, env->_sweepStats.mergeTime,
                                    OMRPORT_TIME_DELTA_IN_MILLISECONDS));
}
 ...
 */
void MM_ParallelSweepTask::synchronizeGCThreads(MM_EnvironmentBase *env,
                                                const char *id) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
 ...
 */
bool MM_ParallelSweepTask::synchronizeGCThreadsAndReleaseMaster(
...
bool MM_ParallelSweepTask::synchronizeGCThreadsAndReleaseSingleThread(
    MM_EnvironmentBase *env, const char *id) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  uint64_t startTime = omrtime_hires_clock();
  bool result =
      MM_ParallelTask::synchronizeGCThreadsAndReleaseSingleThread(env, id);
  uint64_t endTime = omrtime_hires_clock();
  env->_sweepStats.addToIdleTime(startTime, endTime);
 ...
 */
MM_ParallelSweepScheme *MM_ParallelSweepScheme::newInstance(
    MM_EnvironmentBase *env) {
  MM_ParallelSweepScheme *sweepScheme;

  sweepScheme = (MM_ParallelSweepScheme *)env->getForge()->allocate(
      sizeof(MM_ParallelSweepScheme), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (sweepScheme) {
 ...
 */
void MM_ParallelSweepScheme::kill(MM_EnvironmentBase *env) {
  tearDown(env);
 ...
 */
bool MM_ParallelSweepScheme::initialize(MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  extensions->sweepHeapSectioning =
      MM_SweepHeapSectioningSegmented::newInstance(env);
  if (NULL == extensions->sweepHeapSectioning) {
    return false;
  }
  _sweepHeapSectioning = extensions->sweepHeapSectioning;

  if (0 != omrthread_monitor_init_with_name(&_mutexSweepPoolState, 0,
                                            "SweepPoolState Monitor")) {
    return false;
  }
 ...
 */
void MM_ParallelSweepScheme::tearDown(MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
 ...
 */
void *MM_ParallelSweepScheme::createSweepPoolState(MM_EnvironmentBase *env,
  ...
  if (NULL == _poolSweepPoolState) {
    _poolSweepPoolState = pool_new(
        sizeof(MM_SweepPoolState), 0, 2 * sizeof(uintptr_t), 0,
  ...
  return MM_SweepPoolState::newInstance(env, _poolSweepPoolState,
                                        _mutexSweepPoolState, memoryPool);
}
 ...
 */
void MM_ParallelSweepScheme::deleteSweepPoolState(MM_EnvironmentBase *env,
                                                  void *sweepPoolState) {
  ((MM_SweepPoolState *)sweepPoolState)
      ->kill(env, _poolSweepPoolState, _mutexSweepPoolState);
}

/**
 * Get the sweep scheme state for the given memory pool.
 */
MM_SweepPoolState *MM_ParallelSweepScheme::getPoolState(
    MM_MemoryPool *memoryPool) {
 ...
 */
bool MM_ParallelSweepScheme::heapAddRange(MM_EnvironmentBase *env,
                                          MM_MemorySubSpace *subspace,
                                          uintptr_t size, void *lowAddress,
                                          void *highAddress) {
  /* this method is called too often in some configurations (ie: Tarok) so we
   * update the sectioning table in heapReconfigured */
  return true;
}
 ...
 * @return true if operation completes with success (always)
 */
bool MM_ParallelSweepScheme::heapRemoveRange(MM_EnvironmentBase *env,
                                             MM_MemorySubSpace *subspace,
                                             uintptr_t size, void *lowAddress,
                                             void *highAddress,
 ...
 *
 */
void MM_ParallelSweepScheme::heapReconfigured(MM_EnvironmentBase *env) {
 ...
 */
void MM_ParallelSweepScheme::setupForSweep(MM_EnvironmentBase *env) {
  /* the markMap uses the heapBase() (not the activeHeapBase()) for its
   * calculations. We must use the same base. */
...
MMINLINE void MM_ParallelSweepScheme::sweepMarkMapBody(
    uintptr_t *&markMapCurrent, uintptr_t *&markMapChunkTop,
    uintptr_t *&markMapFreeHead, uintptr_t &heapSlotFreeCount,
    uintptr_t *&heapSlotFreeCurrent, uintptr_t *&heapSlotFreeHead) {
...
MMINLINE void MM_ParallelSweepScheme::sweepMarkMapHead(
    uintptr_t *markMapFreeHead, uintptr_t *markMapChunkBase,
  ...
  if (markMapFreeHead > markMapChunkBase) {
    ...
    markMapFreeBitIndexHead = J9MODRON_HEAP_SLOTS_PER_MARK_BIT *
                              MM_Bits::trailingZeroes(markMapHeadValue);
...
MMINLINE void MM_ParallelSweepScheme::sweepMarkMapTail(
    uintptr_t *markMapCurrent, uintptr_t *markMapChunkTop,
  ...
  if (markMapCurrent < markMapChunkTop) {
    ...
    markMapFreeBitIndexTail = J9MODRON_HEAP_SLOTS_PER_MARK_BIT *
                              MM_Bits::leadingZeroes(markMapTailValue);
...
uintptr_t MM_ParallelSweepScheme::performSamplingCalculations(
    MM_ParallelSweepChunk *sweepChunk, uintptr_t *markMapCurrent,
    uintptr_t *heapSlotFreeCurrent) {
  const uintptr_t minimumFreeEntrySize =
      sweepChunk->memoryPool->getMinimumFreeEntrySize();
  uintptr_t darkMatter = 0;

  /* this word has objects in it. Sample them for dark matter */
  MM_HeapMapWordIterator markedObjectIterator(*markMapCurrent,
                                              heapSlotFreeCurrent);
  ...
  uintptr_t prevObjectSize =
      _extensions->objectModel.getConsumedSizeInBytesWithHeader(prevObject);

  omrobjectptr_t object = NULL;
  while (NULL != (object = markedObjectIterator.nextObject())) {
    uintptr_t holeSize =
        (uintptr_t)object - ((uintptr_t)prevObject + prevObjectSize);
  ...
  uintptr_t *endOfPrevObject =
      (uintptr_t *)((uintptr_t)prevObject + prevObjectSize);
  uintptr_t *startSearchAt = (uintptr_t *)MM_Math::roundToFloor(
      J9MODRON_HEAP_SLOTS_PER_MARK_SLOT * sizeof(uintptr_t),
      (uintptr_t)endOfPrevObject);
  uintptr_t *endSearchAt = (uintptr_t *)MM_Math::roundToCeiling(
      J9MODRON_HEAP_SLOTS_PER_MARK_SLOT * sizeof(uintptr_t),
      (uintptr_t)endOfPrevObject + minimumFreeEntrySize);
  startSearchAt = OMR_MAX(
      startSearchAt, heapSlotFreeCurrent + J9MODRON_HEAP_SLOTS_PER_MARK_SLOT);
  endSearchAt = OMR_MIN(endSearchAt, (uintptr_t *)sweepChunk->chunkTop);
  if (startSearchAt < endSearchAt) {
    while (startSearchAt < endSearchAt) {
      MM_HeapMapWordIterator nextMarkedObjectIterator(_currentMarkMap,
                                                      startSearchAt);
 ...
 * Given a properly initialized chunk (zeroed + subspace and range information)
 * sweep the area to include inner free list information as well as surrounding
 * details (leading/trailing, projection, etc).  The routine is self contained
 * to run on an individual chunk, and does not rely on surrounding chunks having
 * been processed.
 * @note a chunk can only be processed by a single thread (single threaded
 * routine)
 * @return return TRUE if live objects found in chunk; FALSE otherwise
 */
bool MM_ParallelSweepScheme::sweepChunk(MM_EnvironmentBase *env,
                                        MM_ParallelSweepChunk *sweepChunk) {
  uintptr_t *heapSlotFreeCurrent =
      NULL; /* Current heap slot in chunk being processed */
  ...
  uintptr_t *markMapCurrent =
      NULL; /* Current mark map slot being processed in chunk */
  ...
  bool liveObjectFound =
      false; /* Set if we find at least one live object in chunk */
  MM_SweepPoolManager *sweepPoolManager =
      sweepChunk->memoryPool->getSweepPoolManager();
  ...
  markMapChunkBase =
      (uintptr_t *)(_currentSweepBits +
                    (MM_Math::roundToFloor(
                         J9MODRON_HEAP_SLOTS_PER_MARK_SLOT * sizeof(uintptr_t),
                         (uintptr_t)sweepChunk->chunkBase -
                             (uintptr_t)_heapBase) /
  ...
  heapSlotFreeCount = 0;
  sweepMarkMapBody(markMapCurrent, markMapChunkTop, markMapFreeHead,
                   heapSlotFreeCount, heapSlotFreeCurrent, heapSlotFreeHead);
  ...
  if (heapSlotFreeCount) {
    ...
    sweepPoolManager->addFreeMemory(env, sweepChunk, heapSlotFreeHead,
                                    heapSlotFreeCount);
  }
  ...
  const UDATA darkMatterSampleRate = (0 == _extensions->darkMatterSampleRate)
                                         ? UDATA_MAX
  ...
  while (markMapCurrent < markMapChunkTop) {
    ...
    if (0 == heapSlotFreeCount) {
      ...
      if (0 == (darkMatterCandidates % darkMatterSampleRate)) {
        darkMatterBytes += performSamplingCalculations(
            sweepChunk, markMapCurrent, heapSlotFreeCurrent);
        darkMatterSamples += 1;
      }
    } else {
      /* There is at least a single free slot in the mark map - check the head
       * and tail */
      sweepMarkMapHead(markMapFreeHead, markMapChunkBase, heapSlotFreeHead,
                       heapSlotFreeCount);
      sweepMarkMapTail(markMapCurrent, markMapChunkTop, heapSlotFreeCount);

      if (!sweepPoolManager->addFreeMemory(env, sweepChunk, heapSlotFreeHead,
                                           heapSlotFreeCount)) {
        break;
      }
  ...
  /* Process the trailing free entry - The body processing will handle trailing
   * entries that cover a map slot or more */
  if (*(markMapCurrent - 1) != J9MODRON_OBM_SLOT_EMPTY) {
    heapSlotFreeCount = 0;
    heapSlotFreeHead = heapSlotFreeCurrent;
    sweepMarkMapHead(markMapCurrent, markMapChunkBase, heapSlotFreeHead,
                     heapSlotFreeCount);

    /* Update the sweep chunk table entry with the trailing free information */
    sweepPoolManager->updateTrailingFreeMemory(
        env, sweepChunk, heapSlotFreeHead, heapSlotFreeCount);
  }

  if (darkMatterSamples == 0) {
    /* No samples were taken, so no dark matter was found (avoid division by
     * zero) */
    ...
    double projectionFactor =
        (double)darkMatterCandidates / (double)darkMatterSamples;
    UDATA projectedDarkMatter =
        (UDATA)((double)darkMatterBytes * projectionFactor);
    UDATA chunkSize =
        (UDATA)sweepChunk->chunkTop - (UDATA)sweepChunk->chunkBase;
    UDATA freeSpace = sweepChunk->freeBytes +
                      sweepChunk->leadingFreeCandidateSize +
    ...
    if (projectedDarkMatter >= (chunkSize - freeSpace)) {
       ...
       * This can happen due to trailing dark matter and due to holes in some of
       * the candidates. It usually only happens for very small samples, and
       * will cause us to estimate more dark matter than is possible. In this
       * case, don't apply the projection. Just record what we actually
       * measured, since this is an accurate lower bound.
       */
 ...
 */
uintptr_t MM_ParallelSweepScheme::prepareAllChunks(MM_EnvironmentBase *env) {
 ...
 */
void MM_ParallelSweepScheme::sweepAllChunks(MM_EnvironmentBase *env,
  ...
  for (uintptr_t chunkNum = 0; chunkNum < totalChunkCount; chunkNum++) {
    ...
    if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
      ...
      /* if we are changing memory pool, flush the thread local stats to
       * appropriate (previous) pool */
       ...
       * sizes in free entry stats for the pool we are about to sweep */
      if ((NULL == prevChunk) || (prevChunk->memoryPool != chunk->memoryPool)) {
 ...
 * Given a previously swept chunk, connect its data to the free list of the
 * associated memory subspace. This requires that all previous chunks in the
 * memory subspace have been swept.
 * @note It is expected that the sweep state free information is updated
 * properly.
 * @note the free list is not guaranteed to be in a consistent state at the end
 * of the routine
 * @note a chunk can only be connected by a single thread (single threaded
 * routine)
 * @todo add the general algorithm to the comment header.
 */
void MM_ParallelSweepScheme::connectChunk(MM_EnvironmentBase *env,
                                          MM_ParallelSweepChunk *chunk) {
  MM_SweepPoolManager *sweepPoolManager =
      chunk->memoryPool->getSweepPoolManager();
  sweepPoolManager->connectChunk(env, chunk);
}

void MM_ParallelSweepScheme::connectAllChunks(MM_EnvironmentBase *env,
                                              uintptr_t totalChunkCount) {
 ...
 */
void MM_ParallelSweepScheme::initializeSweepStates(MM_EnvironmentBase *env) {
  MM_MemoryPool *memoryPool;
 ...
 */
void MM_ParallelSweepScheme::flushFinalChunk(MM_EnvironmentBase *env,
                                             MM_MemoryPool *memoryPool) {
  MM_SweepPoolManager *sweepPoolManager = memoryPool->getSweepPoolManager();
...
/**
 * Flush out any remaining free list data from the last chunk processed for
 * every memory subspace.
 */
void MM_ParallelSweepScheme::flushAllFinalChunks(MM_EnvironmentBase *env) {
  MM_MemoryPool *memoryPool;
  MM_HeapMemoryPoolIterator poolIterator(env, _extensions->heap);
...
void MM_ParallelSweepScheme::allPoolsPostProcess(MM_EnvironmentBase *env) {
  MM_MemoryPool *memoryPool;
  MM_HeapMemoryPoolIterator poolIterator(env, _extensions->heap);
 ...
 * Called by all work threads from a task, performs a full sweep on all memory
 * subspaces.
 *
 * @note Do not call directly - used by the dispatcher for work threads.
 */
void MM_ParallelSweepScheme::internalSweep(MM_EnvironmentBase *env) {
 ...
 * sweep as it wants.  In this case, a full sweep of all memory spaces will be
 * performed.
 *
 ...
 */
void MM_ParallelSweepScheme::sweep(MM_EnvironmentBase *env) {
  setupForSweep(env);
 ...
 * Completing the sweep is a noop - the basic sweep operation consists of a full
 * sweep.
 *
 * @note Expect to have the dispatcher and slave threads available for work
 * @note Expect to have exclusive access
 ...
 * required.
 */
void MM_ParallelSweepScheme::completeSweep(MM_EnvironmentBase *env,
 ...
 * Sweeping for a minimum size involves completing a full sweep (there is no
 * incremental operation) then evaluating whether the minimum free size was
 * found.
 *
 ...
 * otherwise.
 */
bool MM_ParallelSweepScheme::sweepForMinimumSize(
  ...
  if (allocateDescription) {
    ...
    return minimumFreeSize <=
           baseMemorySubSpace->findLargestFreeEntry(env, allocateDescription);
  } else {
    return true;
  }
}
 ...
 * @note The base implementation has completed all work, nothing further can be
 * done.
 * @return True if the pool was replenished with a free entry that can satisfy
 * the size, false otherwise.
 */
bool MM_ParallelSweepScheme::replenishPoolForAllocate(MM_EnvironmentBase *env,
                                                      MM_MemoryPool *memoryPool,
                                                      uintptr_t size) {
  return false;
}
...
void MM_ParallelSweepScheme::setMarkMap(MM_MarkMap *markMap) {
