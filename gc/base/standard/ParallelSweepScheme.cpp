...
MM_ParallelSweepTask::cleanup(MM_EnvironmentBase* env)
{
  ...
  Trc_MM_ParallelSweepTask_parallelStats(
    ...
    (uint32_t)omrtime_hires_delta(
      0, env->_sweepStats.idleTime, OMRPORT_TIME_DELTA_IN_MILLISECONDS),
    env->_sweepStats.sweepChunksProcessed,
    (uint32_t)omrtime_hires_delta(
      0, env->_sweepStats.mergeTime, OMRPORT_TIME_DELTA_IN_MILLISECONDS));
}
 ...
 */
void
MM_ParallelSweepTask::synchronizeGCThreads(MM_EnvironmentBase* env,
                                           const char* id)
{
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
 ...
 */
bool
MM_ParallelSweepTask::synchronizeGCThreadsAndReleaseMaster(
...
bool
MM_ParallelSweepTask::synchronizeGCThreadsAndReleaseSingleThread(
  MM_EnvironmentBase* env,
  const char* id)
{
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
...
MM_ParallelSweepScheme::initialize(MM_EnvironmentBase* env)
{
  ...
  if (0 != omrthread_monitor_init_with_name(
             &_mutexSweepPoolState, 0, "SweepPoolState Monitor")) {
    return false;
  }
 ...
 */
void*
MM_ParallelSweepScheme::createSweepPoolState(MM_EnvironmentBase* env,
                                             MM_MemoryPool* memoryPool)
{
  ...
  if (NULL == _poolSweepPoolState) {
    _poolSweepPoolState = pool_new(sizeof(MM_SweepPoolState),
                                   0,
                                   2 * sizeof(uintptr_t),
                                   0,
                                   OMR_GET_CALLSITE(),
  ...
  return MM_SweepPoolState::newInstance(
 ...
 */
void
MM_ParallelSweepScheme::deleteSweepPoolState(MM_EnvironmentBase* env,
                                             void* sweepPoolState)
{
...
uintptr_t
MM_ParallelSweepScheme::performSamplingCalculations(
 ...
 */
bool
MM_ParallelSweepScheme::sweepChunk(MM_EnvironmentBase* env,
                                   MM_ParallelSweepChunk* sweepChunk)
{
  uintptr_t* heapSlotFreeCurrent =
  ...
  heapSlotFreeCount = 0;
  sweepMarkMapBody(markMapCurrent,
                   markMapChunkTop,
                   markMapFreeHead,
                   heapSlotFreeCount,
                   heapSlotFreeCurrent,
  ...
  if (heapSlotFreeCount) {
    ...
    sweepPoolManager->addFreeMemory(
  ...
  while (markMapCurrent < markMapChunkTop) {
    ...
    if (0 == heapSlotFreeCount) {
       ...
       * and tail */
      sweepMarkMapHead(
        markMapFreeHead, markMapChunkBase, heapSlotFreeHead, heapSlotFreeCount);
      sweepMarkMapTail(markMapCurrent, markMapChunkTop, heapSlotFreeCount);

      if (!sweepPoolManager->addFreeMemory(
            env, sweepChunk, heapSlotFreeHead, heapSlotFreeCount)) {
        break;
      }
  ...
  if (*(markMapCurrent - 1) != J9MODRON_OBM_SLOT_EMPTY) {
    ...
    heapSlotFreeHead = heapSlotFreeCurrent;
    sweepMarkMapHead(
      markMapCurrent, markMapChunkBase, heapSlotFreeHead, heapSlotFreeCount);
 ...
 */
void
MM_ParallelSweepScheme::sweepAllChunks(MM_EnvironmentBase* env,
 ...
 */
void
MM_ParallelSweepScheme::connectChunk(MM_EnvironmentBase* env,
...
void
MM_ParallelSweepScheme::connectAllChunks(MM_EnvironmentBase* env,
                                         uintptr_t totalChunkCount)
{
 ...
 */
void
MM_ParallelSweepScheme::flushFinalChunk(MM_EnvironmentBase* env,
                                        MM_MemoryPool* memoryPool)
{
  MM_SweepPoolManager* sweepPoolManager = memoryPool->getSweepPoolManager();
 ...
 */
void
MM_ParallelSweepScheme::completeSweep(MM_EnvironmentBase* env,
 ...
 */
bool
MM_ParallelSweepScheme::replenishPoolForAllocate(MM_EnvironmentBase* env,
                                                 MM_MemoryPool* memoryPool,
                                                 uintptr_t size)
{
  return false;
