...
void MM_ParallelSweepTask::cleanup(MM_EnvironmentBase* env)
{
    ...
    Trc_MM_ParallelSweepTask_parallelStats(env->getLanguageVMThread(), (uint32_t)env->getSlaveID(),
 ...
 */
void MM_ParallelSweepTask::synchronizeGCThreads(MM_EnvironmentBase* env, const char* id)
{
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
 ...
 */
bool MM_ParallelSweepTask::synchronizeGCThreadsAndReleaseMaster(MM_EnvironmentBase* env, const char* id)
{
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
    uint64_t startTime = omrtime_hires_clock();
...
bool MM_ParallelSweepTask::synchronizeGCThreadsAndReleaseSingleThread(MM_EnvironmentBase* env, const char* id)
{
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
    uint64_t startTime = omrtime_hires_clock();
 ...
 */
MM_ParallelSweepScheme* MM_ParallelSweepScheme::newInstance(MM_EnvironmentBase* env)
{
 ...
 */
void* MM_ParallelSweepScheme::createSweepPoolState(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool)
{
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());

    omrthread_monitor_enter(_mutexSweepPoolState);
    if (NULL == _poolSweepPoolState) {
        _poolSweepPoolState = pool_new(sizeof(MM_SweepPoolState), 0, 2 * sizeof(uintptr_t), 0, OMR_GET_CALLSITE(),
 ...
 */
void MM_ParallelSweepScheme::deleteSweepPoolState(MM_EnvironmentBase* env, void* sweepPoolState)
{
 ...
 */
MM_SweepPoolState* MM_ParallelSweepScheme::getPoolState(MM_MemoryPool* memoryPool)
{
 ...
 */
bool MM_ParallelSweepScheme::heapAddRange(
    MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size, void* lowAddress, void* highAddress)
{
    /* this method is called too often in some configurations (ie: Tarok) so we update the sectioning table in
     * heapReconfigured */
    return true;
}
 ...
 * @return true if operation completes with success (always)
 */
bool MM_ParallelSweepScheme::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size,
...
MMINLINE void MM_ParallelSweepScheme::sweepMarkMapBody(uintptr_t*& markMapCurrent, uintptr_t*& markMapChunkTop,
    uintptr_t*& markMapFreeHead, uintptr_t& heapSlotFreeCount, uintptr_t*& heapSlotFreeCurrent,
...
MMINLINE void MM_ParallelSweepScheme::sweepMarkMapHead(
    uintptr_t* markMapFreeHead, uintptr_t* markMapChunkBase, uintptr_t*& heapSlotFreeHead, uintptr_t& heapSlotFreeCount)
{
...
MMINLINE void MM_ParallelSweepScheme::sweepMarkMapTail(
    uintptr_t* markMapCurrent, uintptr_t* markMapChunkTop, uintptr_t& heapSlotFreeCount)
{
...
uintptr_t MM_ParallelSweepScheme::performSamplingCalculations(
    MM_ParallelSweepChunk* sweepChunk, uintptr_t* markMapCurrent, uintptr_t* heapSlotFreeCurrent)
{
    ...
    uintptr_t* endSearchAt = (uintptr_t*)MM_Math::roundToCeiling(
        J9MODRON_HEAP_SLOTS_PER_MARK_SLOT * sizeof(uintptr_t), (uintptr_t)endOfPrevObject + minimumFreeEntrySize);
 ...
 */
bool MM_ParallelSweepScheme::sweepChunk(MM_EnvironmentBase* env, MM_ParallelSweepChunk* sweepChunk)
{
    ...
    heapSlotFreeCount = 0;
    sweepMarkMapBody(
        markMapCurrent, markMapChunkTop, markMapFreeHead, heapSlotFreeCount, heapSlotFreeCurrent, heapSlotFreeHead);
    ...
    const UDATA darkMatterSampleRate
        = (0 == _extensions->darkMatterSampleRate) ? UDATA_MAX : _extensions->darkMatterSampleRate;
    ...
    if (darkMatterSamples == 0) {
        ...
        UDATA freeSpace
            = sweepChunk->freeBytes + sweepChunk->leadingFreeCandidateSize + sweepChunk->trailingFreeCandidateSize;
 ...
 */
void MM_ParallelSweepScheme::sweepAllChunks(MM_EnvironmentBase* env, uintptr_t totalChunkCount)
{
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
    ...
    for (uintptr_t chunkNum = 0; chunkNum < totalChunkCount; chunkNum++) {
        ...
        if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
            ...
            /* if we are starting or changing memory pool, setup frequent allocation sizes in free entry stats for the
             * pool we are about to sweep */
            if ((NULL == prevChunk) || (prevChunk->memoryPool != chunk->memoryPool)) {
 ...
 */
void MM_ParallelSweepScheme::connectChunk(MM_EnvironmentBase* env, MM_ParallelSweepChunk* chunk)
{
    MM_SweepPoolManager* sweepPoolManager = chunk->memoryPool->getSweepPoolManager();
    sweepPoolManager->connectChunk(env, chunk);
}

void MM_ParallelSweepScheme::connectAllChunks(MM_EnvironmentBase* env, uintptr_t totalChunkCount)
{
 ...
 */
void MM_ParallelSweepScheme::flushFinalChunk(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool)
{
    MM_SweepPoolManager* sweepPoolManager = memoryPool->getSweepPoolManager();
 ...
 * little work towards completing the sweep as it wants.  In this case, a full sweep of all memory spaces will be
 * performed.
 *
 ...
 */
void MM_ParallelSweepScheme::completeSweep(MM_EnvironmentBase* env, SweepCompletionReason reason)
{
 ...
 */
bool MM_ParallelSweepScheme::replenishPoolForAllocate(
    MM_EnvironmentBase* env, MM_MemoryPool* memoryPool, uintptr_t size)
{
    return false;
}
