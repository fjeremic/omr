 ...
 */
MM_ConcurrentOverflow* MM_ConcurrentOverflow::newInstance(MM_EnvironmentBase* env, MM_WorkPackets* workPackets)
{
    MM_ConcurrentOverflow* overflow;

    overflow = (MM_ConcurrentOverflow*)env->getForge()->allocate(
        sizeof(MM_ConcurrentOverflow), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (NULL != overflow) {
...
bool MM_ConcurrentOverflow::initialize(MM_EnvironmentBase* env)
{
    ...
    if (result) {
        ...
        if (omrthread_monitor_init_with_name(
                &_cardsClearingMonitor, 0, "MM_ConcurrentOverflow::cardsClearingMonitor")) {
            result = false;
        }
    }
 ...
 *
 */
void MM_ConcurrentOverflow::emptyToOverflow(MM_EnvironmentBase* env, MM_Packet* packet, MM_OverflowType type)
{
 ...
 *
 */
void MM_ConcurrentOverflow::overflowItem(MM_EnvironmentBase* env, void* item, MM_OverflowType type)
{
    MM_ConcurrentGC* collector = (MM_ConcurrentGC*)_extensions->getGlobalCollector();
...
void MM_ConcurrentOverflow::overflowItemInternal(MM_EnvironmentBase* env, void* item, MM_ConcurrentCardTable* cardTable)
{
...
void MM_ConcurrentOverflow::fillFromOverflow(MM_EnvironmentBase* env, MM_Packet* packet)
{
...
#if defined(OMR_GC_MODRON_SCAVENGER)
void MM_ConcurrentOverflow::clearCardsForNewSpace(MM_EnvironmentStandard* env, MM_ConcurrentGC* collector)
{
...
void MM_ConcurrentOverflow::handleOverflow(MM_EnvironmentBase* env)
{
    ...
    while ((region = regionIterator.nextRegion()) != NULL) {
        cardTable->cleanCardTableForRange(
            envStandard, &cardCleanerForMarking, region->getLowAddress(), region->getHighAddress());
    }
