...
MM_ConcurrentOverflow::newInstance(MM_EnvironmentBase *env,
                                   MM_WorkPackets *workPackets) {
  MM_ConcurrentOverflow *overflow;

  overflow = (MM_ConcurrentOverflow *)env->getForge()->allocate(
      sizeof(MM_ConcurrentOverflow), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (NULL != overflow) {
 ...
 */
bool MM_ConcurrentOverflow::initialize(MM_EnvironmentBase *env) {
  bool result = MM_WorkPacketOverflow::initialize(env);

  if (result) {
    /* Initialize monitor for safe initial Cards cleaning in Work Packets
     * Overflow handler */
    if (omrthread_monitor_init_with_name(
            &_cardsClearingMonitor, 0,
 ...
 */
void MM_ConcurrentOverflow::tearDown(MM_EnvironmentBase *env) {
 ...
 *
 */
void MM_ConcurrentOverflow::emptyToOverflow(MM_EnvironmentBase *env,
                                            MM_Packet *packet,
                                            MM_OverflowType type) {
  MM_ConcurrentGC *collector =
      (MM_ConcurrentGC *)_extensions->getGlobalCollector();
  void *objectPtr;
 ...
 *
 */
void MM_ConcurrentOverflow::overflowItem(MM_EnvironmentBase *env, void *item,
                                         MM_OverflowType type) {
  MM_ConcurrentGC *collector =
      (MM_ConcurrentGC *)_extensions->getGlobalCollector();
...
void MM_ConcurrentOverflow::overflowItemInternal(
    MM_EnvironmentBase *env, void *item, MM_ConcurrentCardTable *cardTable) {
  ...
  if ((PACKET_ARRAY_SPLIT_TAG != ((UDATA)item & PACKET_ARRAY_SPLIT_TAG)) &&
    ...
    globalCollector->getMarkingScheme()
        ->getMarkingDelegate()
        ->handleWorkPacketOverflowItem(env, objectPtr);
  }
}

void MM_ConcurrentOverflow::fillFromOverflow(MM_EnvironmentBase *env,
                                             MM_Packet *packet) {
  Assert_MM_unreachable();
}

void MM_ConcurrentOverflow::reset(MM_EnvironmentBase *env) {}

bool MM_ConcurrentOverflow::isEmpty() { return true; }

#if defined(OMR_GC_MODRON_SCAVENGER)
void MM_ConcurrentOverflow::clearCardsForNewSpace(MM_EnvironmentStandard *env,
                                                  MM_ConcurrentGC *collector) {
  /* If scavenger is enabled, we are within a global collect, and we have not
   * already done so, then we need to clear cards for new space so we can
   * resolve overflow by dirtying cards
   */
  if (_extensions->scavengerEnabled && collector->isStwCollectionInProgress()) {
     ...
     *	If any other thread handling an WP Overflow came here while cleaning is
     *in progress it should wait
     */
...
void MM_ConcurrentOverflow::handleOverflow(MM_EnvironmentBase *env) {
  MM_EnvironmentStandard *envStandard =
      MM_EnvironmentStandard::getEnvironment(env);
  if (envStandard->_currentTask->synchronizeGCThreadsAndReleaseMaster(
          env, UNIQUE_ID)) {
    _overflow = false;
  ...
  while ((region = regionIterator.nextRegion()) != NULL) {
    cardTable->cleanCardTableForRange(envStandard, &cardCleanerForMarking,
