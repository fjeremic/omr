...
private:
  ...
  bool _searchMode; /**< mode switch: initialized in Modify mode, after first
                       nextObject call is switched to Search mode */
  MM_HeapMapIterator
      _markedObjectIterator; /**< Iterator to walk marked objects */
...
public:
  ...
  MMINLINE omrobjectptr_t nextObject() {
    if (!_searchMode) {
      ...
      _markedObjectIterator.reset(
          _markMap, (uintptr_t *)_extensions->heapBaseForBarrierRange0,
  ...
  MMINLINE MM_RSOverflow(MM_EnvironmentBase *env)
      : _extensions(env->getExtensions()), _markMap(NULL), _searchMode(false),
