...
public:
   ...
   * 3. Put number of scan caches MM_CopyScanCacheStandard to just barely exceed
   * tlhMinimumSize. Such size should be small enough to easy to allocate and
   * large enough to be reused as tlh
   *
   ...
   * @param[out] sublistTail tail of sublist of scan caches created in this
   * chunk
   * @param[out] entries number of scan cache entries created in this chunk
   * @return pointer to allocated chunk
   */
  static MM_CopyScanCacheChunkInHeap *
  newInstance(MM_EnvironmentStandard *env, MM_CopyScanCacheChunk *nextChunk,
   ...
   */
  MM_CopyScanCacheChunkInHeap(void *addBase, void *addrTop,
                              MM_MemorySubSpace *memorySubSpace)
      : MM_CopyScanCacheChunk(), _addrBase(addBase), _addrTop(addrTop),
