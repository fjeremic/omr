...
MM_CopyScanCacheChunk *MM_CopyScanCacheChunk::newInstance(
    MM_EnvironmentBase *env, uintptr_t cacheEntryCount,
    MM_CopyScanCacheChunk *nextChunk, MM_CopyScanCacheStandard **sublistTail) {
  MM_CopyScanCacheChunk *chunk;

  chunk = (MM_CopyScanCacheChunk *)env->getForge()->allocate(
      sizeof(MM_CopyScanCacheChunk) +
...
void MM_CopyScanCacheChunk::kill(MM_EnvironmentBase *env) {
  tearDown(env);
  env->getForge()->free(this);
}

bool MM_CopyScanCacheChunk::initialize(MM_EnvironmentBase *env,
  ...
  *sublistTail = _baseCache + cacheEntryCount - 1;
  ...
  for (MM_CopyScanCacheStandard *currentCache = *sublistTail;
       currentCache >= _baseCache; currentCache--) {
...
void MM_CopyScanCacheChunk::tearDown(MM_EnvironmentBase *env) {
  _baseCache = NULL;
  _nextChunk = NULL;
}
