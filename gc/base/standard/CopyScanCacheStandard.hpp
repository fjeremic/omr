...
public:
  GC_ObjectScannerState
    _objectScannerState; /**< Space reserved for instantiation of object scanner
                            for current object */
  ...
  omrobjectptr_t*
    _arraySplitRememberedSlot; /**< A pointer to the remembered set slot a split
                                  array came from if applicable. */
...
public:
   ...
   * If so, the array object may be found in scanCurrent and the index in
   * _arraySplitIndex.
   * @return whether the receiver represents a split array
   */
  MMINLINE bool isSplitArray() const
  {
    return (OMR_SCAVENGER_CACHE_TYPE_SPLIT_ARRAY ==
            (flags & OMR_SCAVENGER_CACHE_TYPE_SPLIT_ARRAY));
  }
