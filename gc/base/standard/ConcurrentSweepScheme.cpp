...
private:
    ...
    MM_ParallelSweepChunk*
        _currentSweepChunkReverse; /**< Next chunk to sweep when sweeping high to low in a given memory pool */
    ...
    MM_HeapLinkedFreeHeader*
        _connectNextFreeEntry; /**< Existing free list entry immediately following the connect range */
    UDATA _connectNextFreeEntrySize; /**< Existing free list entry size immediately following the connect range */

    MM_ParallelSweepChunk* _currentInitChunk; /**< Last chunk initialized for concurrent sweep in the given memory pool
                                                 (used for _next pointer) */

    UDATA _freeMemoryConnected; /**< Running count of the total free memory connected into the memory pool  since last
                                   GC */
...
public:
     ...
     */
    static MM_ConcurrentSweepPoolState* newInstance(
...
bool MM_ConcurrentSweepPoolState::initialize(MM_EnvironmentBase* env)
{
    ...
    _sweepChunkIteratorLock.initialize(
        env, &extensions->lnrlOptions, "MM_ConcurrentSweepPoolState:_sweepChunkIteratorLock");
...
public:
    ...
    MM_ConcurrentSweepCompleteSweepTask(
        MM_EnvironmentBase* env, MM_Dispatcher* dispatcher, MM_ConcurrentSweepScheme* concurrentSweepScheme)
...
public:
    ...
    MM_ConcurrentSweepFindMinimumSizeFreeTask(MM_EnvironmentBase* env, MM_Dispatcher* dispatcher,
        MM_ConcurrentSweepScheme* concurrentSweepScheme, MM_MemorySubSpace* memorySubSpace, UDATA minimumFreeSize)
        : MM_ParallelSweepTask(env, dispatcher, concurrentSweepScheme)
...
void MM_ConcurrentSweepFindMinimumSizeFreeTask::run(MM_EnvironmentBase* env)
{
    ((MM_ConcurrentSweepScheme*)_sweepScheme)
        ->workThreadFindMinimumSizeFreeEntry(env, _memorySubSpace, _minimumFreeSize);
}
 ...
 * @return a new instance of the receiver, or NULL on failure.
 */
MM_ConcurrentSweepScheme* MM_ConcurrentSweepScheme::newInstance(MM_EnvironmentBase* env, MM_GlobalCollector* collector)
{
    MM_ConcurrentSweepScheme* sweepScheme;

    sweepScheme = (MM_ConcurrentSweepScheme*)env->getForge()->allocate(
        sizeof(MM_ConcurrentSweepScheme), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (sweepScheme) {
...
bool MM_ConcurrentSweepScheme::initialize(MM_EnvironmentBase* env)
{
    ...
    if (omrthread_monitor_init_with_name(
            &_completeSweepingConcurrentlyLock, 0, "MM_ConcurrentSweepScheme::completeSweepingConcurrentlyLock")) {
        return false;
    }
 ...
 */
void* MM_ConcurrentSweepScheme::createSweepPoolState(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool)
{
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());

    omrthread_monitor_enter(_mutexSweepPoolState);
    if (NULL == _poolSweepPoolState) {
        _poolSweepPoolState = pool_new(sizeof(MM_ConcurrentSweepPoolState), 0, 2 * sizeof(UDATA), 0, OMR_GET_CALLSITE(),
 ...
 */
void MM_ConcurrentSweepScheme::reportConcurrentlyCompletedSweepPhase(MM_EnvironmentBase* env)
{
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());

    Trc_MM_ConcurrentlyCompletedSweepPhase(env->getLanguageVMThread(), _stats._concurrentCompleteSweepBytesSwept);
    TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENTLY_COMPLETED_SWEEP_PHASE(_extensions->privateHookInterface,
        env->getOmrVMThread(), omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENTLY_COMPLETED_SWEEP_PHASE,
        omrtime_hires_delta(_stats._concurrentCompleteSweepTimeStart, _stats._concurrentCompleteSweepTimeEnd,
 ...
 */
void MM_ConcurrentSweepScheme::reportCompletedConcurrentSweep(MM_EnvironmentBase* env, SweepCompletionReason reason)
{
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());

    Trc_MM_CompletedConcurrentSweep(env->getLanguageVMThread(), _stats._completeConnectPhaseBytesConnected);
    TRIGGER_J9HOOK_MM_PRIVATE_COMPLETED_CONCURRENT_SWEEP(_extensions->privateHookInterface, env->getOmrVMThread(),
        omrtime_hires_clock(), J9HOOK_MM_PRIVATE_COMPLETED_CONCURRENT_SWEEP,
        omrtime_hires_delta(
            _stats._completeSweepPhaseTimeStart, _stats._completeSweepPhaseTimeEnd, OMRPORT_TIME_DELTA_IN_MICROSECONDS),
        _stats._completeSweepPhaseBytesSwept,
        omrtime_hires_delta(_stats._completeConnectPhaseTimeStart, _stats._completeConnectPhaseTimeEnd,
            OMRPORT_TIME_DELTA_IN_MICROSECONDS),
 ...
 */
MMINLINE bool MM_ConcurrentSweepScheme::isConcurrentSweepCandidate(MM_MemorySubSpace* memorySubSpace)
{
    return memorySubSpace->isLeafSubSpace() && memorySubSpace->isActive() && (NULL != memorySubSpace->getMemoryPool())
...
void MM_ConcurrentSweepScheme::initializeChunks(MM_EnvironmentBase* env)
{
    ...
    while (0 != totalChunkCount--) {
        ...
        MM_ConcurrentSweepPoolState* sweepState
            = (MM_ConcurrentSweepPoolState*)getPoolState(chunk->memoryPool); /* sweep state for the current pool */
 ...
 * Before a concurrent connection into the free list, reset the sweep states free stats to 0.  The stats accumulated
 * during the connect will be used to update the memory pool as well as other running metrics (total heap swept, largest
 * free entry, etc).
 */
void MM_ConcurrentSweepScheme::preConnectChunk(MM_EnvironmentBase* env, MM_ParallelSweepChunk* chunk)
{
    ...
    Assert_MM_true(
        (sweepState->_connectNextFreeEntry == NULL) || ((void*)sweepState->_connectNextFreeEntry >= chunk->chunkTop));
 ...
 */
void MM_ConcurrentSweepScheme::postConnectChunk(MM_EnvironmentBase* env, MM_ParallelSweepChunk* chunk)
{
    Assert_MM_true(chunk != NULL);
    ...
    if (NULL != sweepState->_connectPreviousFreeEntry) {
         ...
         * The scenario that can result in a corrupted heap is having an object/dead entry span two different chunks,
         * with the last valid connection being at the end of the current chunk.  The result is the 2nd chunk can be
         * unwalkable. The solution is to fix the heap by splitting the overlapping memory in the two chunks, and
         * install a dead object header (or slots) at the beginning of the next chunk.  This ensures that a walk
         * starting at the beginning of the next chunk will be safe.
         */
        void* endFreeEntry
        ...
        if (endFreeEntry == chunk->chunkTop) {
            GC_ObjectHeapIteratorAddressOrderedList objectIterator(_extensions,
                (omrobjectptr_t)sweepState->_connectPreviousFreeEntry, (omrobjectptr_t)chunk->chunkTop, true);
            ...
            while (NULL != (currentObject = objectIterator.nextObjectNoAdvance())) {
                ...
                if ((UDATA)currentObject + entrySize > (UDATA)chunk->chunkTop) {
                    /* Found an overlapping entry - fix the trailing portion that appears in the next chunk to be
                     * walkable */
        ...
        memoryPool->createFreeEntry(env, (void*)sweepState->_connectPreviousFreeEntry,
            (void*)(((UDATA)sweepState->_connectPreviousFreeEntry) + sweepState->_connectPreviousFreeEntrySize));
 ...
 *   ((Previous GC Free memory found) / (Previous GC Total Memory)) = ((Estimated Free Memory) / (Total Memory to be
 * Connected)) which reduces to, (Estimated Free Memory) = ((Previous GC Free memory found) / (Previous GC Total
 * Memory)) * (Total Memory to be Connected)
 *
 */
MMINLINE void MM_ConcurrentSweepScheme::calculateApproximateFree(
    MM_EnvironmentBase* env, MM_MemoryPool* memoryPool, MM_ConcurrentSweepPoolState* sweepState)
{
    UDATA approximateFree;
    UDATA heapSizeRemainingToBeConnected;

    /* Calculate the free memory approximation.  If there is no more connection work to take place, the approximation is
     * 0 */
 ...
 */
void MM_ConcurrentSweepScheme::connectChunk(MM_EnvironmentBase* env, MM_ParallelSweepChunk* chunk)
{
...
void MM_ConcurrentSweepScheme::verifyFreeList(MM_EnvironmentStandard* env, MM_HeapLinkedFreeHeader* freeListHead)
{
    bool const compressed = env->compressObjectReferences();
    for (MM_HeapLinkedFreeHeader* current = freeListHead; NULL != current; current = current->getNext(compressed)) {
        ...
        assume((current->getNext(compressed) == NULL) || (current < current->getNext(compressed)),
            "Free list next pointer is lower in memory");
        assume((current->getNext(compressed) == NULL) || (current->getNext(compressed) > current->afterEnd()),
    ...
    MM_EnvironmentStandard* env, MM_ConcurrentSweepPoolState* sweepState)
{
    ...
    if (NULL != chunk) {
        ...
        /* we can only step back the chunk for this sweep state if the chunk preceding us points to us as its next chunk
         * (otherwise, it is associated with a different pool) */
 ...
 */
bool MM_ConcurrentSweepScheme::incrementalSweepChunk(MM_EnvironmentStandard* env, MM_ParallelSweepChunk* chunk)
{
 ...
 */
bool MM_ConcurrentSweepScheme::incrementalConnectChunk(MM_EnvironmentStandard* env, MM_ParallelSweepChunk* chunk,
    MM_ConcurrentSweepPoolState* sweepState, MM_MemoryPoolAddressOrderedList* memoryPool)
{
 ...
 */
void MM_ConcurrentSweepScheme::initializeStateForConnections(MM_EnvironmentBase* envModron,
    MM_MemoryPoolAddressOrderedList* memoryPool, MM_ConcurrentSweepPoolState* sweepState, MM_ParallelSweepChunk* chunk)
{
 ...
 */
bool MM_ConcurrentSweepScheme::replenishPoolForAllocate(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool, UDATA size)
{
    ...
    if (isConcurrentSweepActive()) {
        ...
        if (chunkTax > 0) {
            ...
            if (chunkTax > 0) {
                ...
                while (chunkTax > 0 && (NULL != (memoryPool = mssPoolIterator.nextPool()))) {
 ...
 */
void MM_ConcurrentSweepScheme::completeSweep(MM_EnvironmentBase* envBase, SweepCompletionReason reason)
{
 ...
 */
void MM_ConcurrentSweepScheme::workThreadCompleteSweep(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentStandard* env = MM_EnvironmentStandard::getEnvironment(envBase);
 ...
 * another chunk to sweep to move total work forward before returning to connecting.  Slave threads are strictly
 * responsible for finding and sweeping chunks. When the master thread has finally connected in the minimum sized free
 * entry, it will flag all slave threads to complete current work and stop any further sweeping.
...
void MM_ConcurrentSweepScheme::workThreadFindMinimumSizeFreeEntry(
    MM_EnvironmentBase* envBase, MM_MemorySubSpace* memorySubSpace, UDATA minimumFreeSize)
{
    MM_MemoryPool* memoryPool;
    MM_EnvironmentStandard* env = MM_EnvironmentStandard::getEnvironment(envBase);
    MM_ConcurrentSweepFindMinimumSizeFreeTask* task = (MM_ConcurrentSweepFindMinimumSizeFreeTask*)env->_currentTask;
        ...
        while (poolNum < poolCount && NULL != (memoryPool = mssPoolIterator.nextPool())) {
            ...
            if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
                ...
                if (task->_foundLiveObject) {
                     ...
                     * call above to point to the next available predecessor.  This will be the chunk prior to the last
                     * swept chunk. If sweepState->_currentSweepChunkReverse is now pointing to NULL, then all the
                     * chunks have been swept; Otherwise the last swept chunk will be the next chunk after
 ...
 * There is no expectation for amount of work done for this routine.  The receiver is entitled to do as much or as
 * little work towards completing the sweep as it wants.  In this case, the minimum setup work for starting a concurrent
 * sweep is performed.
 *
 ...
 */
bool MM_ConcurrentSweepScheme::completeSweepingConcurrently(MM_EnvironmentBase* envModron)
{
 ...
 */
bool MM_ConcurrentSweepScheme::increaseActiveSweepingThreadCount(MM_EnvironmentBase* envModron, bool completeSweeping)
{
    ...
    if (completeSweeping) {
        ...
        /* If we are the first thread to start sweeping during the complete sweep phase concurrently, switch the
         * execution mode */
 ...
 */
bool MM_ConcurrentSweepScheme::decreaseActiveSweepingThreadCount(MM_EnvironmentBase* envModron, bool completeSweeping)
{
    omrthread_monitor_enter(_completeSweepingConcurrentlyLock);
    ...
    if (_stats.isCompletingSweepConcurrently()) {
        ...
        if (0 == _concurrentSweepingThreadCount) {
            ...
            _stats.switchMode(concurrentsweep_mode_completing_sweep_phase_concurrently,
...
/**
 * Walk the chunks forward propagating any projections.  Projections are propagated into the next chunk if the
 * projection is larger than the chunk.  Projections are propagated to the end of the memory pool.
 *
...
void MM_ConcurrentSweepScheme::propagateChunkProjections(
    MM_EnvironmentBase* envModron, MM_ParallelSweepChunk* startingChunkToPropagate)
{
    ...
    while (NULL != currentChunk) {
        ...
        if (projection > (((UDATA)currentChunk->chunkTop) - ((UDATA)currentChunk->chunkBase))) {
            currentChunk->projection
                = projection - (((UDATA)currentChunk->chunkTop) - ((UDATA)currentChunk->chunkBase));
        }
...
void MM_ConcurrentSweepScheme::abandonOverlappedChunks(
    MM_EnvironmentBase* envModron, MM_ParallelSweepChunk* startingChunk, bool isFirstChunkInSubpool)
{
    ...
    do {
        ...
        /* If the chunk has a projection, then the next chunks starting walk point will be well defined (there is no
         * leading free entry that may have started in a previous chunk).  This chunk can be safely ignored.
         */
        ...
        /* If the chunk has a trailing free candidate then there must have been a live object within the chunk, or a
         * projection into the chunk. This is a well defined starting point.  A walk is required to properly abandon any
         * memory overlap as this chunk may be the chunk adjacent to the next subpool (ie: the last chunk in the SOA
         * before the LOA chunks)
         */
        if (NULL != currentChunk->trailingFreeCandidate) {
            walkStart = currentChunk->trailingFreeCandidate;
            CONSS_DEBUG_PRINTF(
                ("trailing(size: %p): set walkstart [%p]\n", currentChunk->trailingFreeCandidateSize, walkStart));
        } else {
            if (NULL != currentChunk->leadingFreeCandidate) {
                /* Check if the chunk is completely empty */
                if (((void*)(((UDATA)currentChunk->leadingFreeCandidate) + currentChunk->leadingFreeCandidateSize)
                        == currentChunk->chunkTop)) {
                    /* If this is the very first chunk in the pool (the pool was entirely empty.)  It will be safe to
                    ...
                    if (isFirstChunkInSubpool && (currentChunk == startingChunk)) {
                        ...
                        /* A chunk has been previously processed -- it is safe to walk the chunk from the beginning, or
                         * at a projection point */

                        /* It's possible to think that this chunk is completely empty, even if there is a projection, so
                         * factor in the previous chunk projection, and use this information to determine the walk
                         * starting point.
                         */
                        ...
                        /* The startingChunk can be a chunk previous to the chunk found with the live object in it (as
                         * per comment in workThreadFindMinimumSizeFreeEntry.)  This must be a chunk before the chunk
                         * with the live object in it.  If there was no live object then this would have to be the first
                         * chunk in the subpool which would have been handle in the case above. It can be skipped
                         * because there will be a live object before the end of this subpools chunks. Do not flag
                         * chunkProcessed because no chunk has been processed yet.
                         */
                        CONSS_DEBUG_PRINTF(("starting empty chunk -- ignore\n"));
                        continue;
                    }
                } else {
                    /* If the chunk does not have a trailing free entry and has a leading free entry, there must be a
                     * live object at the end of the chunk.  The live object can not overlap between chunks, otherwise
                     * there would have been a projection, which is handled above.  The next chunk will be walkable from
                     * the beginning of the chunk.  No walk needs to be done for the current chunk.
                     */
                ...
                /* There is neither a trailing free entry or a leading free entry.  There must be a live object at the
                 * end of the chunk.  If the live object overlaps onto the next chunk there would have been a
                 * projection, which is handled above.  The next chunk will be walkable from the beginning of the chunk,
                 * so no walk needs to be done for this chunk.
                 */
...
void MM_ConcurrentSweepScheme::walkChunkForOverlappingDeadSpace(
    MM_EnvironmentBase* envModron, MM_ParallelSweepChunk* currentChunk, void* walkStart)
{
    CONSS_DEBUG_OMRPORT_ACCESS_FROM_OMRPORT(envModron->getPortLibrary());
    ...
    GC_ObjectHeapIteratorAddressOrderedList objectIterator(
        _extensions, (omrobjectptr_t)walkStart, (omrobjectptr_t)currentChunk->chunkTop, true);
    omrobjectptr_t currentObject;
    UDATA entrySize;
    ...
    while (NULL != (currentObject = objectIterator.nextObjectNoAdvance())) {
        ...
        /* If the currentObject (or dark matter) extends past the top of the chunk (overlaps multiple chunks) then
         * abandon both halves. In order to get here, the currentChunk must not have a projection, so we know that the
        ...
        if ((UDATA)currentObject + entrySize > (UDATA)currentChunk->chunkTop) {
            ...
            /* TODO: when splitting regions spanning more than two chunks all the inner chunks have abandon called
             * twice. */
            ...
            currentChunk->memoryPool->abandonHeapChunk(
                (void*)currentChunk->chunkTop, (void*)((UDATA)currentChunk->chunkTop + entrySize));
            break;
        }
