...
MM_Configuration *MM_ConfigurationGenerational::newInstance(
    MM_EnvironmentBase *env) {
  MM_ConfigurationGenerational *configuration;

  configuration = (MM_ConfigurationGenerational *)env->getForge()->allocate(
      sizeof(MM_ConfigurationGenerational), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
...
void MM_ConfigurationGenerational::tearDown(MM_EnvironmentBase *env) {
...
MM_MemorySubSpaceSemiSpace *MM_ConfigurationGenerational::createSemiSpace(
    MM_EnvironmentBase *envBase, MM_Heap *heap, MM_Scavenger *scavenger,
    MM_InitializationParameters *parameters, UDATA numaNode) {
  MM_EnvironmentStandard *env = MM_EnvironmentStandard::getEnvironment(envBase);
  MM_GCExtensionsBase *ext = env->getExtensions();
  MM_MemorySubSpaceSemiSpace *memorySubSpaceSemiSpace = NULL;
  MM_MemoryPoolAddressOrderedList *memoryPoolAllocate = NULL,
                                  *memoryPoolSurvivor = NULL;
  MM_MemorySubSpaceGeneric *memorySubSpaceGenericAllocate = NULL,
                           *memorySubSpaceGenericSurvivor = NULL;
  ...
  /* Create Sweep Pool Manager for Nursery if we can not use manager created for
   * Tenure */
  ...
  /* allocate space */
  if (NULL ==
      (memoryPoolAllocate = MM_MemoryPoolAddressOrderedList::newInstance(
           env, minimumFreeEntrySize, "Allocate/Survivor1"))) {
    return NULL;
  }
  if (NULL ==
      (memorySubSpaceGenericAllocate = MM_MemorySubSpaceGeneric::newInstance(
           env, memoryPoolAllocate, NULL, false,
           parameters->_minimumNewSpaceSize / 2,
           parameters->_initialNewSpaceSize / 2,
           parameters->_maximumNewSpaceSize, MEMORY_TYPE_NEW, 0))) {
  ...
  /* survivor space */
  if (NULL ==
      (memoryPoolSurvivor = MM_MemoryPoolAddressOrderedList::newInstance(
           env, minimumFreeEntrySize, "Allocate/Survivor2"))) {
    memorySubSpaceGenericAllocate->kill(env);
    return NULL;
  }
  if (NULL ==
      (memorySubSpaceGenericSurvivor = MM_MemorySubSpaceGeneric::newInstance(
           env, memoryPoolSurvivor, NULL, false,
           parameters->_minimumNewSpaceSize / 2,
           parameters->_initialNewSpaceSize / 2,
           parameters->_maximumNewSpaceSize, MEMORY_TYPE_NEW, 0))) {
  ...
  if (NULL == (physicalSubArenaSemiSpace =
                   MM_PhysicalSubArenaVirtualMemorySemiSpace::newInstance(
                       envBase, heap))) {
    memorySubSpaceGenericAllocate->kill(env);
  ...
  if (NULL ==
      (memorySubSpaceSemiSpace = MM_MemorySubSpaceSemiSpace::newInstance(
           env, scavenger, physicalSubArenaSemiSpace,
           memorySubSpaceGenericAllocate, memorySubSpaceGenericSurvivor, false,
...
MM_MemorySpace *MM_ConfigurationGenerational::createDefaultMemorySpace(
    MM_EnvironmentBase *envBase, MM_Heap *heap,
  ...
  if (NULL ==
      (physicalSubArenaFlat =
           MM_PhysicalSubArenaVirtualMemoryFlat::newInstance(env, heap))) {
  ...
  if (NULL ==
      (memorySubSpaceOld = MM_MemorySubSpaceFlat::newInstance(
           env, physicalSubArenaFlat, memorySubSpaceGenericOld, false,
           parameters->_minimumOldSpaceSize, parameters->_initialOldSpaceSize,
           parameters->_maximumOldSpaceSize, MEMORY_TYPE_OLD, 0))) {
    physicalSubArenaFlat->kill(env);
    memorySubSpaceGenericOld->kill(env);
    return NULL;
  }

  /* then we build "new-space" - note that if we fail during this we must
   * remember to kill() the oldspace we created */

  /* join them with a semispace */
  if (NULL ==
      (scavenger = MM_Scavenger::newInstance(env, ext->heapRegionManager))) {
    memorySubSpaceOld->kill(env);
    return NULL;
  }

  if (NULL ==
      (memorySubSpaceNew = createSemiSpace(env, heap, scavenger, parameters))) {
    memorySubSpaceOld->kill(env);
  }
  ...
  if (NULL ==
      (memorySubSpaceGenerational = MM_MemorySubSpaceGenerational::newInstance(
           envBase, memorySubSpaceNew, memorySubSpaceOld, true,
           parameters->_minimumSpaceSize, parameters->_minimumNewSpaceSize,
           parameters->_initialNewSpaceSize, parameters->_maximumNewSpaceSize,
           parameters->_minimumOldSpaceSize, parameters->_initialOldSpaceSize,
           parameters->_maximumOldSpaceSize, parameters->_maximumSpaceSize))) {
  ...
  if (NULL ==
      (physicalArena = MM_PhysicalArenaVirtualMemory::newInstance(env, heap))) {
  ...
  return MM_MemorySpace::newInstance(
      env, heap, physicalArena, memorySubSpaceGenerational, parameters,
      MEMORY_SPACE_NAME_GENERATIONAL, MEMORY_SPACE_DESCRIPTION_GENERATIONAL);
}

/**
 * Gencon can run in a split-heap virtual memory heap so see if the options for
 * that have been set here or defer to our super
 */
MM_Heap *MM_ConfigurationGenerational::createHeapWithManager(
    MM_EnvironmentBase *env, UDATA heapBytesRequested,
    MM_HeapRegionManager *regionManager) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  MM_Heap *heap = NULL;

#if defined(OMR_GC_MODRON_SCAVENGER)
  /* gencon supports split heaps so check that flag here when deciding what kind
   * of MM_Heap to create */
  if (extensions->enableSplitHeap) {
    ...
    heap = MM_HeapSplit::newInstance(env, extensions->heapAlignment, lowSize,
                                     highSize, regionManager);
    /* FUTURE:  if this heap is NULL, we could try again with the arguments in
     * the other order but that would require that we set and "inverted" flag in
     * the extensions and check that in the PSA attach code when determining
     * attachment policy. May allow more versatile use cases of the split heaps,
     * though.
     */
  } else
#endif /* OMR_GC_MODRON_SCAVENGER */
  {
    heap = MM_ConfigurationStandard::createHeapWithManager(
        env, heapBytesRequested, regionManager);
  }
  return heap;
}

void MM_ConfigurationGenerational::defaultMemorySpaceAllocated(
...
uintptr_t MM_ConfigurationGenerational::calculateDefaultRegionSize(
    MM_EnvironmentBase *env) {
