 ...
 */
MM_WorkPacketsSATB *MM_WorkPacketsSATB::newInstance(MM_EnvironmentBase *env) {
  MM_WorkPacketsSATB *workPackets;

  workPackets = (MM_WorkPacketsSATB *)env->getForge()->allocate(
      sizeof(MM_WorkPacketsSATB), MM_AllocationCategory::WORK_PACKETS,
      J9_GET_CALLSITE());
  if (workPackets) {
 ...
 */
bool MM_WorkPacketsSATB::initialize(MM_EnvironmentBase *env) {
 ...
 */
void MM_WorkPacketsSATB::tearDown(MM_EnvironmentBase *env) {
 ...
 */
MM_WorkPacketOverflow *MM_WorkPacketsSATB::createOverflowHandler(
    MM_EnvironmentBase *env, MM_WorkPackets *wp) {
 ...
 */
MM_Packet *MM_WorkPacketsSATB::getBarrierPacket(MM_EnvironmentBase *env) {
 ...
 */
MM_Packet *MM_WorkPacketsSATB::getPacketByOverflowing(MM_EnvironmentBase *env) {
 ...
 */
void MM_WorkPacketsSATB::putInUsePacket(MM_EnvironmentBase *env,
                                        MM_Packet *packet) {
  _inUseBarrierPacketList.push(env, packet);
}

void MM_WorkPacketsSATB::removePacketFromInUseList(MM_EnvironmentBase *env,
                                                   MM_Packet *packet) {
  _inUseBarrierPacketList.remove(packet);
}

void MM_WorkPacketsSATB::putFullPacket(MM_EnvironmentBase *env,
                                       MM_Packet *packet) {
 ...
 */
void MM_WorkPacketsSATB::moveInUseToNonEmpty(MM_EnvironmentBase *env) {
  MM_Packet *head, *tail;
  UDATA count;
  bool didPop;
 ...
 */
float MM_WorkPacketsSATB::getHeapCapacityFactor(MM_EnvironmentBase *env) {
 ...
 */
MM_Packet *MM_WorkPacketsSATB::getInputPacketFromOverflow(
