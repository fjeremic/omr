...
/**
 * Auxilary structure too pass both function and userData as one param to heap
 * walker
 */
 ...
 */
static void heapWalkerObjectFieldSlotDo(OMR_VM *omrVM, omrobjectptr_t object,
                                        GC_SlotObject *slotObject,
                                        MM_HeapWalkerSlotFunc oSlotIterator,
                                        void *localUserData) {
 ...
 */
static void heapWalkerObjectSlotDo(OMR_VM *omrVM, omrobjectptr_t object,
                                   MM_HeapWalkerSlotFunc oSlotIterator,
                                   void *localUserData) {
  ...
  while ((slotObject = objectIterator.nextSlot()) != NULL) {
    heapWalkerObjectFieldSlotDo(omrVM, object, slotObject, oSlotIterator,
                                localUserData);
  }
}
 ...
 */
static void heapWalkerObjectSlotDo(OMR_VMThread *omrVMThread,
                                   MM_HeapRegionDescriptor *region,
                                   omrobjectptr_t object, void *userData) {
  OMR_VM *omrVM = omrVMThread->_vm;
  MM_GCExtensionsBase *extensions = MM_GCExtensionsBase::getExtensions(omrVM);
  MM_HeapWalkerSlotFunc oSlotIterator =
      (MM_HeapWalkerSlotFunc)((SlotObjectDoUserData *)userData)->function;
  void *localUserData = ((SlotObjectDoUserData *)userData)->userData;

  omrobjectptr_t indirectObject =
      extensions->objectModel.getIndirectObject(object);
...
MM_HeapWalker *MM_HeapWalker::newInstance(MM_EnvironmentBase *env) {
  MM_HeapWalker *heapWalker;

  heapWalker = (MM_HeapWalker *)env->getForge()->allocate(
      sizeof(MM_HeapWalker), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (heapWalker) {
...
bool MM_HeapWalker::initialize(MM_EnvironmentBase *env) { return true; }

/**
 * Free the receivers memory and all associated resources.
 */
void MM_HeapWalker::kill(MM_EnvironmentBase *env) {
...
void MM_HeapWalker::rememberedObjectSlotsDo(MM_EnvironmentBase *env,
                                            MM_HeapWalkerSlotFunc function,
                                            void *userData, uintptr_t walkFlags,
                                            bool parallel) {
  ...
  while ((puddle = remSetIterator.nextList()) != NULL) {
    if (!parallel || J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
      ...
      while ((slotPtr = (omrobjectptr_t *)remSetSlotIterator.nextSlot()) !=
             NULL) {
        if (*slotPtr != NULL) {
          heapWalkerObjectSlotDo(omrVMThread, NULL, *slotPtr,
                                 &slotObjectDoUserData);
        }
      }
    }
  }
 ...
 */
void MM_HeapWalker::allObjectSlotsDo(MM_EnvironmentBase *env,
                                     MM_HeapWalkerSlotFunc function,
                                     void *userData, uintptr_t walkFlags,
                                     bool parallel, bool prepareHeapForWalk) {
  SlotObjectDoUserData slotObjectDoUserData = {function, userData, walkFlags};
  uintptr_t modifiedWalkFlags = walkFlags;

#if defined(OMR_GC_MODRON_SCAVENGER)
  /* If J9_MU_WALK_NEW_AND_REMEMBERED_ONLY is specified, and rsOverflow has
   * occurred, any object in old space might be remembered, so we must walk them
   * all
   */
  ...
  allObjectsDo(env, heapWalkerObjectSlotDo, (void *)&slotObjectDoUserData,
               modifiedWalkFlags, parallel, prepareHeapForWalk);

#if defined(OMR_GC_MODRON_SCAVENGER)
  /* If J9_MU_WALK_NEW_AND_REMEMBERED_ONLY is specified, allObjectsDo will only
   * walk new spaces, so we must also go through all slots of objects in the
   * remembered set.
   */
 ...
 */
void MM_HeapWalker::allObjectsDo(MM_EnvironmentBase *env,
                                 MM_HeapWalkerObjectFunc function,
                                 void *userData, uintptr_t walkFlags,
                                 bool parallel, bool prepareHeapForWalk) {
  ...
  MM_HeapRegionManager *regionManager =
      extensions->heap->getHeapRegionManager();
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    if (typeFlags == (region->getTypeFlags() & typeFlags)) {
      ...
      GC_ObjectHeapIteratorAddressOrderedList liveObjectIterator(extensions,
                                                                 region, false);
