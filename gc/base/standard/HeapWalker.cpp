 ...
 */
static void
heapWalkerObjectFieldSlotDo(OMR_VM* omrVM,
 ...
 */
static void
heapWalkerObjectSlotDo(OMR_VM* omrVM,
                       omrobjectptr_t object,
                       MM_HeapWalkerSlotFunc oSlotIterator,
                       void* localUserData)
{
  ...
  while ((slotObject = objectIterator.nextSlot()) != NULL) {
    heapWalkerObjectFieldSlotDo(
      omrVM, object, slotObject, oSlotIterator, localUserData);
  }
}
 ...
 */
static void
heapWalkerObjectSlotDo(OMR_VMThread* omrVMThread,
...
void
MM_HeapWalker::rememberedObjectSlotsDo(MM_EnvironmentBase* env,
                                       MM_HeapWalkerSlotFunc function,
                                       void* userData,
                                       uintptr_t walkFlags,
                                       bool parallel)
  ...
  while ((puddle = remSetIterator.nextList()) != NULL) {
    if (!parallel || J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
      ...
      while ((slotPtr = (omrobjectptr_t*)remSetSlotIterator.nextSlot()) !=
             NULL) {
        if (*slotPtr != NULL) {
          heapWalkerObjectSlotDo(
            omrVMThread, NULL, *slotPtr, &slotObjectDoUserData);
        }
      }
    }
  }
 ...
 */
void
MM_HeapWalker::allObjectSlotsDo(MM_EnvironmentBase* env,
                                MM_HeapWalkerSlotFunc function,
                                void* userData,
                                uintptr_t walkFlags,
                                bool parallel,
 ...
 */
void
MM_HeapWalker::allObjectsDo(MM_EnvironmentBase* env,
                            ...
                            bool prepareHeapForWalk)
{
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    if (typeFlags == (region->getTypeFlags() & typeFlags)) {
      ...
      GC_ObjectHeapIteratorAddressOrderedList liveObjectIterator(
        extensions, region, false);
