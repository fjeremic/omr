...
MM_Heap::getApproximateActiveFreeMemorySize(uintptr_t includeMemoryType)
{
  ...
  while (currentMemorySpace) {
    freeMemory +=
      currentMemorySpace->getApproximateActiveFreeMemorySize(includeMemoryType);
    currentMemorySpace = currentMemorySpace->getNext();
  }
                      ...
                      void* highAddress)
{
  ...
  if (NULL != globalCollector) {
    result = globalCollector->heapAddRange(
                         ...
                         void* highValidAddress)
{
  ...
  bool result = true;
  if (NULL != globalCollector) {
    result = globalCollector->heapRemoveRange(env,
                                              subspace,
                                              size,
                                              lowAddress,
                                              highAddress,
 ...
 */
void
MM_Heap::initializeCommonGCStartData(MM_EnvironmentBase* env,
 ...
 */
void
MM_Heap::initializeCommonGCEndData(MM_EnvironmentBase* env,
                                ...
                                struct MM_CommonGCData* data)
{
  ...
  data->tenureLOATotalBytes =
    (extensions->largeObjectArea ? getActiveLOAMemorySize(MEMORY_TYPE_OLD) : 0);
