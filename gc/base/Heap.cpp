...
uintptr_t MM_Heap::getApproximateActiveFreeSurvivorMemorySize(uintptr_t includeMemoryType)
{
    MM_MemorySpace* currentMemorySpace;
    uintptr_t memory;
 ...
 * @return the total free memory currently available for allocation from subspaces of the specified type.
 */
uintptr_t MM_Heap::getApproximateActiveFreeMemorySize(uintptr_t includeMemoryType)
{
    MM_MemorySpace* currentMemorySpace;
    uintptr_t freeMemory;
 ...
 * @return the total free memory currently available for allocation from subspaces of the specified type.
 */
uintptr_t MM_Heap::getApproximateActiveFreeLOAMemorySize(uintptr_t includeMemoryType)
{
    MM_MemorySpace* currentMemorySpace;
    uintptr_t freeMemory;
...
void MM_Heap::mergeHeapStats(MM_HeapStats* heapStats, uintptr_t includeMemoryType)
{
    MM_MemorySpace* currentMemorySpace = _memorySpaceList;
 ...
 *
 */
bool MM_Heap::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size, void* lowAddress,
...
{
    /* This is only used by the split heap to check if the given object lies inside the gap between committed regions of
     * the heap. Most heap implementations should just inherit this implementation which returns false since they have
     * no gaps.
     */
    return false;
}
 ...
 */
void MM_Heap::initializeCommonGCStartData(MM_EnvironmentBase* env, MM_CommonGCStartData* data)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    MM_HeapStats stats;
 ...
 * finished.
 */
void MM_Heap::initializeCommonGCEndData(MM_EnvironmentBase* env, MM_CommonGCEndData* data)
{
    MM_HeapStats stats;
    mergeHeapStats(&stats, MEMORY_TYPE_OLD);
    initializeCommonGCData(env, &data->commonData);
 ...
 */
struct MM_CommonGCData* MM_Heap::initializeCommonGCData(MM_EnvironmentBase* env, struct MM_CommonGCData* data)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
