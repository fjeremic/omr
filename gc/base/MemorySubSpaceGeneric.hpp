...
class MM_MemorySubSpaceGeneric : public MM_MemorySubSpace {
  MM_MemoryPool
      *_memoryPool; /**< the memory pool associated with this subspace */
  MM_RegionPool
      *_regionPool; /**< the region pool associated with this subspace */
  bool _allocateAtSafePointOnly;

public:
  static MM_MemorySubSpaceGeneric *
  newInstance(MM_EnvironmentBase *env, MM_MemoryPool *memoryPool,
              MM_RegionPool *regionPool, bool usesGlobalCollector,
              uintptr_t minimumSize, uintptr_t initialSize,
              uintptr_t maximumSize, uintptr_t memoryType,
  ...
  virtual bool shouldAllocateAtSafePointOnly() {
    return _allocateAtSafePointOnly;
  }
   ...
   */
  MM_MemorySubSpaceGeneric(MM_EnvironmentBase *env, MM_MemoryPool *memoryPool,
                           MM_RegionPool *regionPool, bool usesGlobalCollector,
                           uintptr_t minimumSize, uintptr_t initialSize,
                           uintptr_t maximumSize, uintptr_t memoryType,
                           uint32_t objectFlags)
      : MM_MemorySubSpace(env, (MM_Collector *)NULL, NULL, usesGlobalCollector,
                          minimumSize, initialSize, maximumSize, memoryType,
                          objectFlags),
        _memoryPool(memoryPool), _regionPool(regionPool),
...
private:
   ...
   * As far as old subspace expected to be contiguous an expansion should be at
   * the top or bottom of the memory range An exception is an initial setup when
   * both of borders are set. The range before such set better to be (NULL,NULL)
   * @param size the size of the expansion
   * @param low low expansion address
   * @param high high expansion address
   */
  void addTenureRange(MM_EnvironmentBase *env, uintptr_t size, void *low,
                      void *high);

  /**
   * Add a contraction of old subspace range
   * As far as old subspace expected to be contiguous a contraction should be at
   * the top or bottom of the memory range
   * @param size the size of the contraction
   * @param low low expansion address
   * @param high high expansion address
   */
  void removeTenureRange(MM_EnvironmentBase *env, uintptr_t size, void *low,
                         void *high);
};
