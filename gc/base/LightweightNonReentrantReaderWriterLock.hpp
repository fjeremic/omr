...
#define LWRW_READER_MODE                                                       \
  ((uint32_t)1) /**< bit0 of lock-word is lock mode, :0=Reader Mode, :1=Writer \
                   Mode */
...
#define LWRW_INCREMENTAL_BASE_READERS                                        \
  ((uint32_t)2) /**< bit15-bit1 is shared reader count, max reader count is  \
                   32767, can be extended to bit16-bit30 in order supporting \
                   more readers, need to match the change of                 \
                   LWRW_INCREMENTAL_BASE_WAITINGWRITERS,                     \
                   LWRW_MASK_WAITINGWRITERS and LWRW_MASK_READERS */
#define LWRW_INCREMENTAL_BASE_WAITINGWRITERS \
  ((uint32_t)(1 << 16)) /**< bit31-bit16 is waiting writer count,  */
 ...
 * it is writer priority lock(write locks have priority over reads, no priority
 * order between waiting writers) and support multiple concurrent writers case.
 *
