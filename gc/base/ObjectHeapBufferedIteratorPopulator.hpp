 ...
 * Instances of subclasses of this class are used to populate the buffers of
 * ObjectHeapBufferedIterators. Instances of subclasses of this class are all
 * singletons and should have no state. Instances of subclasses of this class
 * may be instantiated in debugger extensions to iterate over out of process
 * regions.
 */
...
public:
  ...
  /**
   * Initialize the specified state object in preperation of beginning iteration
   * over the specified region. The receiver may store any necessary information
   * in the state. The state will not be modified by the iterator -- it is
   * reserved for use by the ObjectHeapBufferedIteratorPopulator.
   *
   ...
   * @parm[in] maxCount - the maximum number of objects to populate the cache
   * with
   * @parm[in/out] state - a state struct which the receiver may use to record
   * persistent information.
   * @return the number of objects written to the cache, or 0 if the iteration
   * is finished
   */
  virtual uintptr_t populateObjectHeapBufferedIteratorCache(
    omrobjectptr_t* cache,
    uintptr_t count,
    GC_ObjectHeapBufferedIteratorState* state) const = 0;
