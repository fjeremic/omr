...
MM_GCExtensionsBase* MM_GCExtensionsBase::newInstance(MM_EnvironmentBase* env)
{
    ...
    /* Avoid using OMR::GC::Forge to allocate memory for the extension, since OMR::GC::Forge itself has not been
     * created! */
    extensions
...
{
    /* Avoid using OMR::GC::Forge to free memory for the extension, since OMR::GC::Forge was not used to allocate the
     * memory */
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
    tearDown(env);
...
#define TWO_MB ((uintptr_t)2 * 1024 * 1024)
    ...
    if (omrthread_monitor_init_with_name(
            &_lightweightNonReentrantLockPoolMutex, 0, "GCExtensions::_lightweightNonReentrantLockPoolMutex")) {
        goto failed;
    }
...
void MM_GCExtensionsBase::identityHashDataAddRange(
