...
MM_GCExtensionsBase* MM_GCExtensionsBase::newInstance(MM_EnvironmentBase* env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  MM_GCExtensionsBase* extensions;

  /* Avoid using OMR::GC::Forge to allocate memory for the extension, since
   * OMR::GC::Forge itself has not been created! */
  extensions = static_cast<MM_GCExtensionsBase*>(
      omrmem_allocate_memory(sizeof(MM_GCExtensionsBase), OMRMEM_CATEGORY_MM));
  if (extensions) {
...
void MM_GCExtensionsBase::kill(MM_EnvironmentBase* env) {
  /* Avoid using OMR::GC::Forge to free memory for the extension, since
   * OMR::GC::Forge was not used to allocate the memory */
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  tearDown(env);
  omrmem_free_memory(this);
}

bool MM_GCExtensionsBase::initialize(MM_EnvironmentBase* env) {
  ...
  if (compressObjectReferences()) {
    heapCeiling = LOW_MEMORY_HEAP_CEILING; /* By default, compressed pointers
                                              builds run in the low 64GiB */
  }
...
#error Default GC policy cannot be determined
  ...
  if (!rememberedSet.initialize(env,
                                OMR::GC::AllocationCategory::REMEMBERED_SET)) {
    goto failed;
  }
  ...
  /* there was no GC before VM birth, so this is the first moment we can get
   * "end of previous GC" timestamp */
  ...
  /* Get usable physical memory from portlibrary. This is used by
   * computeDefaultMaxHeap(). It can also be used by downstream projects to
   * compute project specific GC parameters.
   */
...
#define TWO_MB ((uintptr_t)2 * 1024 * 1024)
  ...
  if (!validateDefaultPageParameters(requestedPageSize, requestedPageFlags,
                                     pageSizes, pageFlags)) {
    requestedPageSize = pageSizes[0];
    requestedPageFlags = pageFlags[0];
  }

  if (!validateDefaultPageParameters(gcmetadataPageSize, gcmetadataPageFlags,
                                     pageSizes, pageFlags)) {
    gcmetadataPageSize = pageSizes[0];
  ...
  if (J9HookInitializeInterface(getPrivateHookInterface(), OMRPORTLIB,
                                sizeof(privateHookInterface))) {
    goto failed;
  }

  if (J9HookInitializeInterface(getOmrHookInterface(), OMRPORTLIB,
                                sizeof(omrHookInterface))) {
    goto failed;
  }

  if (omrthread_monitor_init_with_name(
          &gcExclusiveAccessMutex, 0, "GCExtensions::gcExclusiveAccessMutex")) {
    goto failed;
  }

  if (omrthread_monitor_init_with_name(
          &_lightweightNonReentrantLockPoolMutex, 0,
  ...
  _omrVM->_gcCycleOn = 0;
  if (omrthread_monitor_init_with_name(&_omrVM->_gcCycleOnMonitor, 0,
                                       "gcCycleOn")) {
    goto failed;
  }
...
bool MM_GCExtensionsBase::validateDefaultPageParameters(
    uintptr_t pageSize, uintptr_t pageFlags, uintptr_t* pageSizesArray,
...
void MM_GCExtensionsBase::tearDown(MM_EnvironmentBase* env) {
#if defined(OMR_GC_MODRON_SCAVENGER)
...
bool MM_GCExtensionsBase::isConcurrentScavengerInProgress() {
...
void MM_GCExtensionsBase::identityHashDataAddRange(MM_EnvironmentBase* env,
...
void MM_GCExtensionsBase::identityHashDataRemoveRange(
 ...
 * the usable physical memory - half of usable memory with a min of 16 MiB and a
 * max of 512 MiB.
 */
void MM_GCExtensionsBase::computeDefaultMaxHeap(MM_EnvironmentBase* env) {
