...
#define TWO_MB ((uintptr_t)2 * 1024 * 1024)
  ...
  if (!validateDefaultPageParameters(
        requestedPageSize, requestedPageFlags, pageSizes, pageFlags)) {
    requestedPageSize = pageSizes[0];
    requestedPageFlags = pageFlags[0];
  }

  if (!validateDefaultPageParameters(
        gcmetadataPageSize, gcmetadataPageFlags, pageSizes, pageFlags)) {
    gcmetadataPageSize = pageSizes[0];
  ...
  if (J9HookInitializeInterface(
        getPrivateHookInterface(), OMRPORTLIB, sizeof(privateHookInterface))) {
    goto failed;
  }

  if (J9HookInitializeInterface(
        getOmrHookInterface(), OMRPORTLIB, sizeof(omrHookInterface))) {
    goto failed;
  }
  ...
  if (omrthread_monitor_init_with_name(
        &_lightweightNonReentrantLockPoolMutex,
        0,
  ...
  _omrVM->_gcCycleOn = 0;
  if (omrthread_monitor_init_with_name(
        &_omrVM->_gcCycleOnMonitor, 0, "gcCycleOn")) {
    goto failed;
  }
...
bool
MM_GCExtensionsBase::validateDefaultPageParameters(uintptr_t pageSize,
                                                   uintptr_t pageFlags,
...
void
MM_GCExtensionsBase::identityHashDataAddRange(MM_EnvironmentBase* env,
...
void
MM_GCExtensionsBase::identityHashDataRemoveRange(MM_EnvironmentBase* env,
                                                 MM_MemorySubSpace* subspace,
                                                 uintptr_t size,
                                                 void* lowAddress,
                                                 void* highAddress)
