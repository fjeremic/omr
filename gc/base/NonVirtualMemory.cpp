...
MM_NonVirtualMemory::reserveMemory(J9PortVmemParams* params)
{
  ...
  _baseAddress = _extensions->getForge()->allocate(
    bytesToAllocate, OMR::GC::AllocationCategory::GC_HEAP, OMR_GET_CALLSITE());
  void* addressToReturn = _baseAddress;
  if ((NULL != _baseAddress) && (alignment > 0)) {
    addressToReturn =
      (void*)(((uintptr_t)addressToReturn + alignment - 1) & ~(alignment - 1));
  }
 ...
 */
bool
MM_NonVirtualMemory::decommitMemory(void* address,
                                    uintptr_t size,
...
bool
MM_NonVirtualMemory::setNumaAffinity(uintptr_t numaNode,
                                     void* address,
