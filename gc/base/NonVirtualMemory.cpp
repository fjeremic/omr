 ...
 */
MM_NonVirtualMemory* MM_NonVirtualMemory::newInstance(
    MM_EnvironmentBase* env, uintptr_t heapAlignment, uintptr_t size, uint32_t memoryCategory)
{
    MM_NonVirtualMemory* vmem = (MM_NonVirtualMemory*)env->getForge()->allocate(
        sizeof(MM_NonVirtualMemory), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (vmem) {
...
void* MM_NonVirtualMemory::reserveMemory(J9PortVmemParams* params)
{
    ...
    /* should be params->alignmentInBytes but that requires a change in Port to go through, first, so use _heapAlignment
     * for now */
    ...
    _baseAddress
        = _extensions->getForge()->allocate(bytesToAllocate, OMR::GC::AllocationCategory::GC_HEAP, OMR_GET_CALLSITE());
 ...
 * @return true if successful, false otherwise.
 */
bool MM_NonVirtualMemory::decommitMemory(void* address, uintptr_t size, void* lowValidAddress, void* highValidAddress)
{
    return true;
}

bool MM_NonVirtualMemory::setNumaAffinity(uintptr_t numaNode, void* address, uintptr_t byteAmount)
{
    return true;
}
