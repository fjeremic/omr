...
class MM_HeapSplit : public MM_Heap {
 ...
 private:
  MM_HeapVirtualMemory
      *_lowExtent; /**< the sub-heap which represents the committed low range of
                      the heap (old space) */
  MM_HeapVirtualMemory
      *_highExtent; /**< the sub-heap which represents the committed high range
                       of the heap (new space) */

  /* Member Functions */
 public:
  static MM_HeapSplit *newInstance(MM_EnvironmentBase *env,
   ...
   */
  MM_HeapSplit(MM_EnvironmentBase *env, uintptr_t lowExtentSize,
 ...
 protected:
  bool initialize(MM_EnvironmentBase *env, uintptr_t heapAlignment,
