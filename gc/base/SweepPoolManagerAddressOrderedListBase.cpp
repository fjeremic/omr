    ...
    MM_MemoryPool* memoryPool)
{
    ...
    if (sweepState->_connectPreviousFreeEntry) {
        ...
        ((MM_MemoryPoolAddressOrderedListBase*)memoryPool)->connectFinalMemoryToPool(envModron, sweepState->_connectPreviousFreeEntry, sweepState->_connectPreviousFreeEntrySize);
    ...
    ((MM_MemoryPoolAddressOrderedListBase*)memoryPool)->updateMemoryPoolStatistics(envModron, sweepState->_sweepFreeBytes, sweepState->_sweepFreeHoles, sweepState->_largestFreeEntry);
 ...
 * @param trailingCandidateSlotCount trailing candidate size
 */
void MM_SweepPoolManagerAddressOrderedListBase::updateTrailingFreeMemory(MM_EnvironmentBase* env, MM_ParallelSweepChunk* sweepChunk, uintptr_t* heapSlotFreeHead, uintptr_t heapSlotFreeCount)
{
 ...
 */
bool MM_SweepPoolManagerAddressOrderedListBase::addFreeMemory(MM_EnvironmentBase* env, MM_ParallelSweepChunk* sweepChunk, uintptr_t* address, uintptr_t size)
{
    bool result = false;
    ...
    if (address == (uintptr_t*)sweepChunk->chunkBase) {
        ...
        uintptr_t objectSizeDelta = _extensions->objectModel.getConsumedSizeInBytesWithHeader((omrobjectptr_t)(address - J9MODRON_HEAP_SLOTS_PER_MARK_BIT))
