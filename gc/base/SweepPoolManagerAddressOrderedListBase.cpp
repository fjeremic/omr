 ...
 */
bool MM_SweepPoolManagerAddressOrderedListBase::initialize(
 ...
 */
MM_SweepPoolState *MM_SweepPoolManagerAddressOrderedListBase::getPoolState(
    MM_MemoryPool *memoryPool) {
  MM_SweepPoolState *result =
      ((MM_MemoryPoolAddressOrderedListBase *)memoryPool)->getSweepPoolState();
...
MM_SweepPoolManagerAddressOrderedListBase::calculateTrailingDetails(
    MM_ParallelSweepChunk *sweepChunk, uintptr_t *trailingCandidate,
  ...
  uintptr_t trailingCandidateByteCount =
      (uintptr_t)MM_Bits::convertSlotsToBytes(trailingCandidateSlotCount);

  projection = _extensions->objectModel.getConsumedSizeInBytesWithHeader(
                   (omrobjectptr_t)(trailingCandidate -
  ...
  if (projection > trailingCandidateByteCount) {
    ...
    if (projection < trailingCandidateByteCount) {
      sweepChunk->trailingFreeCandidate =
          (void *)(((uintptr_t)trailingCandidate) + projection);
      sweepChunk->trailingFreeCandidateSize =
          trailingCandidateByteCount - projection;
    }
  }
}
 ...
 */
void MM_SweepPoolManagerAddressOrderedListBase::connectChunk(
    MM_EnvironmentBase *env, MM_ParallelSweepChunk *chunk) {
   ...
   */
  MM_MemoryPoolAddressOrderedListBase *memoryPool =
      (MM_MemoryPoolAddressOrderedListBase *)chunk->memoryPool;
  ...
  MM_ParallelSweepChunk *previousConnectChunk =
      sweepState->_connectPreviousChunk;
  /* previousPreviousFreeEntry is only for SPMSAOL */
  MM_HeapLinkedFreeHeader *previousPreviousFreeEntry =
      sweepState->_connectPreviousPreviousFreeEntry;

  MM_HeapLinkedFreeHeader *leadingFreeEntry =
      (MM_HeapLinkedFreeHeader *)chunk->leadingFreeCandidate;
  uintptr_t leadingFreeEntrySize = chunk->leadingFreeCandidateSize;

  Assert_MM_true((NULL == leadingFreeEntry) ||
                 (previousFreeEntry < leadingFreeEntry));
  /* Assume no projection until we prove otherwise */
  uintptr_t projection = 0;

  /* If there is a projection from the previous chunk? It does not necessarily
   * have to be associated to the pool for the projection to actually matter.
   */
  if (NULL != chunk->_previous) {
    Assert_MM_true((0 == chunk->_previous->projection) ||
                   (chunk->_previous->chunkTop == chunk->chunkBase));
  ...
  if (0 != projection) {
    if (projection >
        (((uintptr_t)chunk->chunkTop) - ((uintptr_t)chunk->chunkBase))) {
      chunk->projection = projection - (((uintptr_t)chunk->chunkTop) -
                                        ((uintptr_t)chunk->chunkBase));
      leadingFreeEntry = (MM_HeapLinkedFreeHeader *)NULL;
      leadingFreeEntrySize = 0;
    } else {
      leadingFreeEntry =
          (MM_HeapLinkedFreeHeader *)(((uintptr_t)leadingFreeEntry) +
                                      projection);
      leadingFreeEntrySize -= projection;
    }
  }

  /* See if there is a connection to be made between the previous free and the
   * leading entry */
  if ((previousFreeEntry != NULL) &&
      ((((uint8_t *)previousFreeEntry) + previousFreeEntrySize) ==
       (uint8_t *)leadingFreeEntry) &&
      (previousConnectChunk->memoryPool == memoryPool) &&
      chunk->_coalesceCandidate) {
    /* So should be using same MM_SweepPoolState */
    Assert_MM_true(getPoolState(previousConnectChunk->memoryPool) ==
                   sweepState);
    ...
    omrtty_printf("CC: Previous consumes leading: %p(+%p) -> %p(+%p)\n",
                  previousFreeEntry, previousFreeEntrySize, leadingFreeEntry,
                  leadingFreeEntrySize);
#endif
    memoryPool->getLargeObjectAllocateStats()->decrementFreeEntrySizeClassStats(
        previousFreeEntrySize);
    previousFreeEntrySize += leadingFreeEntrySize;
    sweepState->_sweepFreeBytes += leadingFreeEntrySize;
    sweepState->updateLargestFreeEntry(previousFreeEntrySize,
                                       previousPreviousFreeEntry);
    memoryPool->getLargeObjectAllocateStats()->incrementFreeEntrySizeClassStats(
        previousFreeEntrySize);
  ...
  /* If the leading entry wasn't consumed, check if there is trailing free space
   * in the previous chunk that could connect with the leading entry leading
   * free space */
  if (NULL != previousConnectChunk) {
    if ((NULL != leadingFreeEntry) &&
        ((((uint8_t *)previousConnectChunk->trailingFreeCandidate) +
          previousConnectChunk->trailingFreeCandidateSize) ==
         (uint8_t *)leadingFreeEntry) &&
        (previousConnectChunk->memoryPool == memoryPool) &&
        chunk->_coalesceCandidate) {
      /* The trailing/leading space forms a contiguous chunk - check if it
       * belongs on the free list, abandon otherwise */

      uintptr_t jointFreeSize = leadingFreeEntrySize +
                                previousConnectChunk->trailingFreeCandidateSize;
      if (memoryPool->canMemoryBeConnectedToPool(
              env, previousConnectChunk->trailingFreeCandidate,
              jointFreeSize)) {
        ...
        omrtty_printf("CC: trailing/leading merged %p(+%p) + %p(+%p)\n",
                      ...
                      previousConnectChunk->trailingFreeCandidateSize,
                      leadingFreeEntry, leadingFreeEntrySize);
#endif

        memoryPool->connectOuterMemoryToPool(
            env, previousFreeEntry, previousFreeEntrySize,
            previousConnectChunk->trailingFreeCandidate);

        connectChunkPostProcess(chunk, sweepState,
                                (MM_HeapLinkedFreeHeader *)
                                    previousConnectChunk->trailingFreeCandidate,
                                previousFreeEntry);
        /* only for SPMSAOL */
        previousPreviousFreeEntry = previousFreeEntry;
        previousFreeEntry = (MM_HeapLinkedFreeHeader *)
                                previousConnectChunk->trailingFreeCandidate;
        previousFreeEntrySize = jointFreeSize;

        /* Maintain the free bytes/holes count in the allocate profile for
         * expansion/contraction purposes */
        if (0 != jointFreeSize) {
          sweepState->_sweepFreeBytes += jointFreeSize;
          sweepState->_sweepFreeHoles += 1;
          sweepState->updateLargestFreeEntry(jointFreeSize,
                                             previousPreviousFreeEntry);

          memoryPool->getLargeObjectAllocateStats()
              ->incrementFreeEntrySizeClassStats(jointFreeSize);
        }
      }
      ...
      /* The trailing space of the previous chunk has no connecting partner -
       * check if the trailing space merits an entry in the free list */
      if (memoryPool->canMemoryBeConnectedToPool(
              env, previousConnectChunk->trailingFreeCandidate,
              previousConnectChunk->trailingFreeCandidateSize)) {
        ...
        if (0 != previousConnectChunk->trailingFreeCandidateSize) {
          sweepState->_sweepFreeBytes +=
              previousConnectChunk->trailingFreeCandidateSize;
          sweepState->_sweepFreeHoles += 1;
          sweepState->updateLargestFreeEntry(
              previousConnectChunk->trailingFreeCandidateSize,
              previousPreviousFreeEntry);

          memoryPool->getLargeObjectAllocateStats()
              ->incrementFreeEntrySizeClassStats(
                  previousConnectChunk->trailingFreeCandidateSize);
        }
      }
    }
  ...
  /* If the leading free entry has not been consumed - check if it is now a
   * trailing free candidate.  This scenario can occur if a leading free entry
   * is pushed back from a projection from the previous sweep chunk.  A leading
   * free candidate that spans the entire chunk in this case now becomes a
   * trailing free entry.
   */
  if (leadingFreeEntry && ((void *)(((uintptr_t)leadingFreeEntry) +
                                    leadingFreeEntrySize) == chunk->chunkTop)) {
    ...
    /* If the leading entry still hasn't been consumed, it touches neither the
     * head or tail of a chunk. Check if it merits an entry in the free list
     */
    if (NULL != leadingFreeEntry) {
      if (memoryPool->canMemoryBeConnectedToPool(env, leadingFreeEntry,
                                                 leadingFreeEntrySize)) {

        Assert_MM_true(previousFreeEntry <= leadingFreeEntry);

        memoryPool->connectOuterMemoryToPool(
            env, previousFreeEntry, previousFreeEntrySize, leadingFreeEntry);

        connectChunkPostProcess(chunk, sweepState, leadingFreeEntry,
                                previousFreeEntry);
        /* only for SPMSAOL */
        previousPreviousFreeEntry = previousFreeEntry;
        previousFreeEntry = leadingFreeEntry;
        ...
        if (0 != leadingFreeEntrySize) {
          sweepState->_sweepFreeBytes += leadingFreeEntrySize;
          sweepState->_sweepFreeHoles += 1;
          sweepState->updateLargestFreeEntry(leadingFreeEntrySize,
                                             previousPreviousFreeEntry);

          memoryPool->getLargeObjectAllocateStats()
              ->incrementFreeEntrySizeClassStats(leadingFreeEntrySize);
        }
      } else {
        /* Abandon it. We need to do this in case we encounter a free entry
         * which spans 2 memory pools which is possible when LOA is enabled.
         */
        memoryPool->abandonMemoryInPool(env, leadingFreeEntry,
                                        leadingFreeEntrySize);
      }
    }
  }
  ...
  if (chunk->freeListHead) {
    ...
    memoryPool->connectOuterMemoryToPool(
        env, previousFreeEntry, previousFreeEntrySize, chunk->freeListHead);

    /* Set split candidate information */
    connectChunkPostProcess(chunk, sweepState, chunk->freeListHead,
                            previousFreeEntry);
    ...
    omrtty_printf("CC: head + tail %p(+%p) %p(%p)\n", chunk->freeListHead,
                  chunk->freeListHeadSize, chunk->freeListTail,
                  chunk->freeListTailSize);
#endif
  }
 ...
 */
void MM_SweepPoolManagerAddressOrderedListBase::flushFinalChunk(
    MM_EnvironmentBase *envModron, MM_MemoryPool *memoryPoolBase) {
  ...
  if ((sweepState->_connectPreviousChunk != NULL) &&
    ...
    MM_MemoryPoolAddressOrderedListBase *memoryPool =
        (MM_MemoryPoolAddressOrderedListBase *)memoryPoolBase;

    /* Check if the entry is a candidate */
    if (!memoryPool->canMemoryBeConnectedToPool(
            envModron, sweepState->_connectPreviousChunk->trailingFreeCandidate,
            sweepState->_connectPreviousChunk->trailingFreeCandidateSize)) {
      /* It is not - abandon it */
      memoryPool->abandonMemoryInPool(
          envModron, sweepState->_connectPreviousChunk->trailingFreeCandidate,
          sweepState->_connectPreviousChunk->trailingFreeCandidateSize);
      ...
      memoryPool->connectOuterMemoryToPool(
          envModron, sweepState->_connectPreviousFreeEntry,
      ...
      Assert_MM_true(sweepState->_connectPreviousFreeEntry !=
                     sweepState->_connectPreviousChunk->leadingFreeCandidate);
...
/**
 *  Finally if there is at least 1 entry in the subspace, the last entry should
 * be connected to NULL in the free list
 */
void MM_SweepPoolManagerAddressOrderedListBase::connectFinalChunk(
    MM_EnvironmentBase *envModron, MM_MemoryPool *memoryPool) {
  MM_SweepPoolState *sweepState = getPoolState(memoryPool);

  if (sweepState->_connectPreviousFreeEntry) {

    ((MM_MemoryPoolAddressOrderedListBase *)memoryPool)
        ->connectFinalMemoryToPool(envModron,
                                   sweepState->_connectPreviousFreeEntry,
                                   sweepState->_connectPreviousFreeEntrySize);
  ...
  ((MM_MemoryPoolAddressOrderedListBase *)memoryPool)
      ->updateMemoryPoolStatistics(envModron, sweepState->_sweepFreeBytes,
 ...
 * @param trailingCandidateSlotCount trailing candidate size
 */
void MM_SweepPoolManagerAddressOrderedListBase::updateTrailingFreeMemory(
    MM_EnvironmentBase *env, MM_ParallelSweepChunk *sweepChunk,
 ...
 */
bool MM_SweepPoolManagerAddressOrderedListBase::addFreeMemory(
    MM_EnvironmentBase *env, MM_ParallelSweepChunk *sweepChunk,
  ...
  if (address == (uintptr_t *)sweepChunk->chunkBase) {
    ...
    sweepChunk->leadingFreeCandidateSize =
        (uintptr_t)MM_Bits::convertSlotsToBytes(size);
    Assert_MM_true(sweepChunk->leadingFreeCandidate >
                   sweepChunk->trailingFreeCandidate);

  } else if (address + size == (uintptr_t *)sweepChunk->chunkTop) {
    /* Update the sweep chunk table entry with the trailing free hole
     * information */
    ...
    uintptr_t objectSizeDelta =
        _extensions->objectModel.getConsumedSizeInBytesWithHeader(
            (omrobjectptr_t)(address - J9MODRON_HEAP_SLOTS_PER_MARK_BIT)) -
        (J9MODRON_HEAP_SLOTS_PER_MARK_BIT * sizeof(uintptr_t));
    ...
    MM_MemoryPoolAddressOrderedListBase *memoryPool =
        (MM_MemoryPoolAddressOrderedListBase *)sweepChunk->memoryPool;

    if (memoryPool->connectInnerMemoryToPool(env, address, heapFreeByteCount,
                                             sweepChunk->freeListTail)) {

#if defined(J9MODRON_ALLOCATION_MANAGER_TRACE)
      OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
      omrtty_printf("SW: Free entry %p %p\n", address,
                    ((uint8_t *)address) + heapFreeByteCount);
#endif
      ...
      /* Maintain the free bytes/holes count in the chunk for heap
       * expansion/contraction purposes (will be gathered up in the allocate
       * profile) */
      if (0 != heapFreeByteCount) {
        ...
        memoryPool->getLargeObjectAllocateStats()
            ->incrementFreeEntrySizeClassStats(heapFreeByteCount,
