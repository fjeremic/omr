 ...
 */
bool MM_SweepPoolManagerAddressOrderedListBase::initialize(MM_EnvironmentBase* env)
{
 ...
 */
MM_SweepPoolState* MM_SweepPoolManagerAddressOrderedListBase::getPoolState(MM_MemoryPool* memoryPool)
{
 ...
 */
MMINLINE void MM_SweepPoolManagerAddressOrderedListBase::calculateTrailingDetails(
 ...
 */
void MM_SweepPoolManagerAddressOrderedListBase::connectChunk(MM_EnvironmentBase* env, MM_ParallelSweepChunk* chunk)
{
    ...
    /* If there is a projection from the previous chunk? It does not necessarily have to be associated to the pool for
     * the projection to actually matter.
     */
    ...
    if ((previousFreeEntry != NULL)
        ...
        && (previousConnectChunk->memoryPool == memoryPool) && chunk->_coalesceCandidate) {
    ...
    if (NULL != previousConnectChunk) {
        if ((NULL != leadingFreeEntry)
            ...
            && (previousConnectChunk->memoryPool == memoryPool) && chunk->_coalesceCandidate) {
            /* The trailing/leading space forms a contiguous chunk - check if it belongs on the free list, abandon
             * otherwise */
            ...
            if (memoryPool->canMemoryBeConnectedToPool(
                ...
                memoryPool->connectOuterMemoryToPool(
                    env, previousFreeEntry, previousFreeEntrySize, previousConnectChunk->trailingFreeCandidate);

                connectChunkPostProcess(chunk, sweepState,
                    (MM_HeapLinkedFreeHeader*)previousConnectChunk->trailingFreeCandidate, previousFreeEntry);
                /* only for SPMSAOL */
            ...
            /* The trailing space of the previous chunk has no connecting partner - check if the trailing space merits
             * an entry in the free list */
            if (memoryPool->canMemoryBeConnectedToPool(env, previousConnectChunk->trailingFreeCandidate,
                    previousConnectChunk->trailingFreeCandidateSize)) {
                ...
                if (0 != previousConnectChunk->trailingFreeCandidateSize) {
                    ...
                    sweepState->updateLargestFreeEntry(
                        previousConnectChunk->trailingFreeCandidateSize, previousPreviousFreeEntry);
    ...
    /* If the leading free entry has not been consumed - check if it is now a trailing free candidate.  This scenario
     * can occur if a leading free entry is pushed back from a projection from the previous sweep chunk.  A leading free
     * candidate that spans the entire chunk in this case now becomes a trailing free entry.
     */
    if (leadingFreeEntry && ((void*)(((uintptr_t)leadingFreeEntry) + leadingFreeEntrySize) == chunk->chunkTop)) {
        ...
        if (NULL != leadingFreeEntry) {
            if (memoryPool->canMemoryBeConnectedToPool(env, leadingFreeEntry, leadingFreeEntrySize)) {
                ...
                memoryPool->connectOuterMemoryToPool(env, previousFreeEntry, previousFreeEntrySize, leadingFreeEntry);
                 ...
                 */
                memoryPool->abandonMemoryInPool(env, leadingFreeEntry, leadingFreeEntrySize);
            }
        }
    }
    ...
    if (chunk->freeListHead) {
        ...
        memoryPool->connectOuterMemoryToPool(env, previousFreeEntry, previousFreeEntrySize, chunk->freeListHead);
...
#if defined(J9MODRON_SWEEP_SCHEME_CONNECT_CHUNKS_TRACE)
        omrtty_printf("CC: head + tail %p(+%p) %p(%p)\n", chunk->freeListHead, chunk->freeListHeadSize,
            chunk->freeListTail, chunk->freeListTailSize);
#endif
    }
    ...
    MM_EnvironmentBase* envModron, MM_MemoryPool* memoryPoolBase)
{
    ...
    if ((sweepState->_connectPreviousChunk != NULL)
        ...
        if (!memoryPool->canMemoryBeConnectedToPool(envModron, sweepState->_connectPreviousChunk->trailingFreeCandidate,
                sweepState->_connectPreviousChunk->trailingFreeCandidateSize)) {
            /* It is not - abandon it */
            memoryPool->abandonMemoryInPool(envModron, sweepState->_connectPreviousChunk->trailingFreeCandidate,
                sweepState->_connectPreviousChunk->trailingFreeCandidateSize);

        } else {
            /* It is - fold it into the free list */
            memoryPool->connectOuterMemoryToPool(envModron, sweepState->_connectPreviousFreeEntry,
                sweepState->_connectPreviousFreeEntrySize, sweepState->_connectPreviousChunk->trailingFreeCandidate);

            sweepState->_connectPreviousPreviousFreeEntry = sweepState->_connectPreviousFreeEntry;
            sweepState->_connectPreviousFreeEntry
                = (MM_HeapLinkedFreeHeader*)sweepState->_connectPreviousChunk->trailingFreeCandidate;
            sweepState->_connectPreviousFreeEntrySize = sweepState->_connectPreviousChunk->trailingFreeCandidateSize;
            Assert_MM_true(
                sweepState->_connectPreviousFreeEntry != sweepState->_connectPreviousChunk->leadingFreeCandidate);

            sweepState->_sweepFreeBytes += sweepState->_connectPreviousChunk->trailingFreeCandidateSize;
            sweepState->_sweepFreeHoles += 1;
            sweepState->updateLargestFreeEntry(sweepState->_connectPreviousChunk->trailingFreeCandidateSize,
    ...
    MM_EnvironmentBase* envModron, MM_MemoryPool* memoryPool)
{
    ...
    if (sweepState->_connectPreviousFreeEntry) {
        ...
        ((MM_MemoryPoolAddressOrderedListBase*)memoryPool)
            ->connectFinalMemoryToPool(
                envModron, sweepState->_connectPreviousFreeEntry, sweepState->_connectPreviousFreeEntrySize);

        sweepState->updateLargestFreeEntry(
            sweepState->_connectPreviousFreeEntrySize, sweepState->_connectPreviousPreviousFreeEntry);
    }
    ...
    ((MM_MemoryPoolAddressOrderedListBase*)memoryPool)
        ->updateMemoryPoolStatistics(
            envModron, sweepState->_sweepFreeBytes, sweepState->_sweepFreeHoles, sweepState->_largestFreeEntry);
 ...
 * @param trailingCandidateSlotCount trailing candidate size
 */
void MM_SweepPoolManagerAddressOrderedListBase::updateTrailingFreeMemory(MM_EnvironmentBase* env,
    MM_ParallelSweepChunk* sweepChunk, uintptr_t* heapSlotFreeHead, uintptr_t heapSlotFreeCount)
{
...
bool MM_SweepPoolManagerAddressOrderedListBase::addFreeMemory(
    MM_EnvironmentBase* env, MM_ParallelSweepChunk* sweepChunk, uintptr_t* address, uintptr_t size)
{
    bool result = false;
    ...
    if (address == (uintptr_t*)sweepChunk->chunkBase) {
        ...
        uintptr_t objectSizeDelta = _extensions->objectModel.getConsumedSizeInBytesWithHeader(
        ...
        if (memoryPool->connectInnerMemoryToPool(env, address, heapFreeByteCount, sweepChunk->freeListTail)) {
            ...
            /* Maintain the free bytes/holes count in the chunk for heap expansion/contraction purposes (will be
             * gathered up in the allocate profile) */
            if (0 != heapFreeByteCount) {
                sweepChunk->freeBytes += heapFreeByteCount;
                sweepChunk->freeHoles += 1;
                //				sweepChunk->_largestFreeEntry = max(sweepChunk->_largestFreeEntry ,
                // heapFreeByteCount);
                ...
                memoryPool->getLargeObjectAllocateStats()->incrementFreeEntrySizeClassStats(
                    heapFreeByteCount, &env->_freeEntrySizeClassStats);
            }
