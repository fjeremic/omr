 ...
 */
MM_SweepPoolState*
MM_SweepPoolManagerAddressOrderedListBase::getPoolState(
  ...
  uintptr_t trailingCandidateSlotCount)
{
  ...
  projection =
    _extensions->objectModel.getConsumedSizeInBytesWithHeader(
      (omrobjectptr_t)(trailingCandidate - J9MODRON_HEAP_SLOTS_PER_MARK_BIT)) -
 ...
 */
void
MM_SweepPoolManagerAddressOrderedListBase::connectChunk(
  ...
  MM_ParallelSweepChunk* chunk)
{
  ...
  if (0 != projection) {
    if (projection >
      ...
      leadingFreeEntry =
        (MM_HeapLinkedFreeHeader*)(((uintptr_t)leadingFreeEntry) + projection);
  ...
  if (NULL != previousConnectChunk) {
    if ((NULL != leadingFreeEntry) &&
      ...
      uintptr_t jointFreeSize =
        leadingFreeEntrySize + previousConnectChunk->trailingFreeCandidateSize;
      if (memoryPool->canMemoryBeConnectedToPool(
            env, previousConnectChunk->trailingFreeCandidate, jointFreeSize)) {
        ...
        connectChunkPostProcess(
          chunk,
          sweepState,
          (MM_HeapLinkedFreeHeader*)previousConnectChunk->trailingFreeCandidate,
          previousFreeEntry);
        /* only for SPMSAOL */
  ...
  if (leadingFreeEntry && ((void*)(((uintptr_t)leadingFreeEntry) +
    ...
    if (NULL != leadingFreeEntry) {
      if (memoryPool->canMemoryBeConnectedToPool(
            env, leadingFreeEntry, leadingFreeEntrySize)) {
        ...
        connectChunkPostProcess(
          chunk, sweepState, leadingFreeEntry, previousFreeEntry);
        /* only for SPMSAOL */
        previousPreviousFreeEntry = previousFreeEntry;
        previousFreeEntry = leadingFreeEntry;
        previousFreeEntrySize = leadingFreeEntrySize;
  ...
  if (chunk->freeListHead) {
    ...
    connectChunkPostProcess(
      chunk, sweepState, chunk->freeListHead, previousFreeEntry);
  ...
  MM_MemoryPool* memoryPool)
{
  ...
  ((MM_MemoryPoolAddressOrderedListBase*)memoryPool)
    ->updateMemoryPoolStatistics(envModron,
                                 sweepState->_sweepFreeBytes,
 ...
 */
void
MM_SweepPoolManagerAddressOrderedListBase::updateTrailingFreeMemory(
  MM_EnvironmentBase* env,
  MM_ParallelSweepChunk* sweepChunk,
 ...
 */
bool
MM_SweepPoolManagerAddressOrderedListBase::addFreeMemory(
  MM_EnvironmentBase* env,
  MM_ParallelSweepChunk* sweepChunk,
  uintptr_t* address,
  uintptr_t size)
{
  ...
  if (address == (uintptr_t*)sweepChunk->chunkBase) {
    ...
    if (memoryPool->connectInnerMemoryToPool(
          env, address, heapFreeByteCount, sweepChunk->freeListTail)) {
