...
uintptr_t MM_StartupManager::getUDATAValue(char *option,
                                           uintptr_t *outputValue) {
...
bool MM_StartupManager::getUDATAMemoryValue(char *option,
  ...
  if (NULL != gcOptions) {
    ...
    do {
      ...
      if (NULL == nextSpace) {
        ...
        if (0 == end) {
          /* starting with a space or multiple spaces in a row so skip to next
           * character */
          options += 1;
          continue;
        }
...
bool MM_StartupManager::handleOption(MM_GCExtensionsBase *extensions,
                                     char *option) {
  OMRPORT_ACCESS_FROM_OMRVM(extensions->getOmrVM());
  bool result = true;
  if (0 == strncmp(option, OMR_XMS, OMR_XMS_LENGTH)) {
  ...
  } else if (0 == strncmp(option, OMR_XGCBUFFERED_LOGGING,
                          OMR_XGCBUFFERED_LOGGING_LENGTH)) {
    ...
    if (0 ==
        strncmp(gcpolicy, OMR_GCPOLICY_GENCON, OMR_GCPOLICY_GENCON_LENGTH)) {
    ...
    if (0 >=
        getUDATAValue(option + OMR_XGCTHREADS_LENGTH, &forcedThreadCount)) {
      result = false;
    } else {
...
char *MM_StartupManager::getVerboseFileName(void) { return verboseFileName; }

MM_Configuration *MM_StartupManager::createConfiguration(
    MM_EnvironmentBase *env) {
  /* When multiple configurations are supported, this call can return different
   * concrete implementations based on arguments previously parsed by
   * handleOption().
   */
