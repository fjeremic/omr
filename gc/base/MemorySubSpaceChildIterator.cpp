...
enum {
  ...
  mm_msschild_iterator_children_subspace, /**< Process the children of the
                                             current subspace */
  mm_msschild_iterator_next_subspace /**< Move to the next sibling (or parent if
                                        unavailable) of the current subspace */
};

/**
 * Initialize the iterator to the beginning of the memory subspace list.
 */
void MM_MemorySubSpaceChildIterator::reset(MM_MemorySubSpace *memorySubSpace) {
 ...
 * @return Next memory subspace in the list, or NULL if all spaces have been
 * processed.
 */
MM_MemorySubSpace *MM_MemorySubSpaceChildIterator::nextSubSpace() {
  while (NULL != _memorySubSpace) {
    ...
    case mm_msschild_iterator_next_subspace:
      /* Move to the next sibling (or parent if unavailable) of the current
       * subspace */
      if (NULL == _memorySubSpace) {
        /* How did we get here? */
        assume0(0);
      ...
      /* If the current subspace being processed is the base subspace, we are
       * done */
      ...
      if (NULL == _memorySubSpace->getNext()) {
        /* Moving to the parent means we will just find its next sibling in the
         * list */
