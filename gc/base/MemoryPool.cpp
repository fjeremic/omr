...
bool MM_MemoryPool::initialize(MM_EnvironmentBase *env) { return true; }

/**
 * Free any associated structures or resources in the receiver.
 */
void MM_MemoryPool::tearDown(MM_EnvironmentBase *env) {
 ...
 * free memory.  It will not consider deferred work that may be done to increase
 ...
 */
uintptr_t MM_MemoryPool::getActualFreeMemorySize() { return _freeMemorySize; }
 ...
 */
uintptr_t MM_MemoryPool::getApproximateFreeMemorySize() {
 ...
 */
MM_MemoryPool *MM_MemoryPool::getMemoryPool(void *addr) { return this; }
 ...
 * @return MM_MemoryPool
 */
MM_MemoryPool *MM_MemoryPool::getMemoryPool(uintptr_t size) { return this; }
 ...
 * @param highAddr If range spans the end of memory pool set to the first byte
 * which doesn't belong in the returned pool.
 * @return MM_MemoryPool for storage location addrBase
 */
MM_MemoryPool *MM_MemoryPool::getMemoryPool(MM_EnvironmentBase *env,
...
void MM_MemoryPool::registerMemoryPool(MM_MemoryPool *memoryPool) {
...
void MM_MemoryPool::unregisterMemoryPool(MM_MemoryPool *memoryPool) {
 ...
 */
void *MM_MemoryPool::allocateArrayletLeaf(MM_EnvironmentBase *env,
...
void *MM_MemoryPool::allocateTLH(MM_EnvironmentBase *env,
 ...
 */
void MM_MemoryPool::setSubSpace(MM_MemorySubSpace *memorySubSpace) {
  if (NULL != _children) {
...
void MM_MemoryPool::reset(Cause cause) {
  _freeMemorySize = 0;
  _freeEntryCount = 0;
...
MM_HeapLinkedFreeHeader *MM_MemoryPool::rebuildFreeListInRegion(
...
#if defined(OMR_GC_THREAD_LOCAL_HEAP)
void MM_MemoryPool::abandonTlhHeapChunk(void *addrBase, void *addrTop) {
...
void MM_MemoryPool::resetHeapStatistics(bool memoryPoolCollected) {
 ...
 */
void MM_MemoryPool::mergeHeapStats(MM_HeapStats *heapStats, bool active) {
 ...
 */
void *MM_MemoryPool::findFreeEntryEndingAtAddr(MM_EnvironmentBase *env,
                                               void *addr) {
  Assert_MM_unreachable(); // should never get here
  return NULL;
}
...
/**
 * Find the contraction size available in the given range that MUST end at the
 * high address. This routines shoud find any available free entries that can be
 * removed that are within the given range, ending at a particular range, all
 * while satisfying the described allocation.  The allocDescription is passed so
 * that if the allocation type or size class has an effect on what can/cannot be
 * contracted, it can be factored into the returned value.
 *
 ...
 */
uintptr_t MM_MemoryPool::getAvailableContractionSizeForRangeEndingAt(
 ...
 */
void *MM_MemoryPool::findFreeEntryTopStartingAtAddr(MM_EnvironmentBase *env,
                                                    void *addr) {
  Assert_MM_unreachable(); // should never get here
  return NULL;
}
 ...
 */
void *MM_MemoryPool::getFirstFreeStartingAddr(MM_EnvironmentBase *env) {
  Assert_MM_unreachable(); // should never get here
  return NULL;
}
 ...
 */
void *MM_MemoryPool::getNextFreeStartingAddr(MM_EnvironmentBase *env,
                                             void *currentFree) {
  Assert_MM_unreachable(); // should never get here
  return NULL;
}
 ...
 */
void MM_MemoryPool::moveHeap(MM_EnvironmentBase *env, void *srcBase,
 ...
 */
void MM_MemoryPool::lock(MM_EnvironmentBase *env) {}

/**
 * Unlock any free list information for use.
 */
void MM_MemoryPool::unlock(MM_EnvironmentBase *env) {}

/**
 * @todo Provide function documentation
 */
void *MM_MemoryPool::collectorAllocate(MM_EnvironmentBase *env,
                                       MM_AllocateDescription *allocDescription,
 ...
 * @todo Provide function documentation
 */
void *MM_MemoryPool::collectorAllocateTLH(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
 ...
 */
bool MM_MemoryPool::completeFreelistRebuildRequired(MM_EnvironmentBase *env) {
  return false;
}

/**
 * Convert a chunk of heap into a free list entry, and potentially link it into
 * a list.
 ...
 *
 * @note If both <code>previousFreeEntry</code> and <code>nextFreeEntry</code>
 * are NULL, this call is equivalent to @ref abandonHeapChunk().
 ...
 * When the code that uses these is cleaned up (e.g. sweep), this method should
 * be removed
 *
 * @return true if the heap chunk is large enough to be on the free list, false
 * otherwise
 */
bool MM_MemoryPool::createFreeEntry(MM_EnvironmentBase *env, void *addrBase,
                                    void *addrTop,
                                    MM_HeapLinkedFreeHeader *previousFreeEntry,
 ...
 */
void MM_MemoryPool::addFreeEntries(MM_EnvironmentBase *env,
                                   MM_HeapLinkedFreeHeader *&freeListHead,
                                   MM_HeapLinkedFreeHeader *&freeListTail,
                                   uintptr_t freeMemoryCount,
...
/**
 * Remove  all free entries in a  specified range from pool and return them to
 * caller as an address ordered list.
 *
 ...
 */
bool MM_MemoryPool::removeFreeEntriesWithinRange(
 ...
 */
void *MM_MemoryPool::findAddressAfterFreeSize(MM_EnvironmentBase *env,
...
/**
 * Convert a chunk of heap into a free list entry, and potentially link it into
 * a list. Used for flushing the tail (last free entry).
 * @param addrBase the base address (inclusive) of the heap chunk
 * @param addrTop the top address (exclusive) of the heap chunk
 *
 * @todo This method implies knowledge of a "free list" for managing memory
 ...
 * be removed
 */
bool MM_MemoryPool::isValidListOrdering() {
  Assert_MM_unreachable(); /* implementation required */
  return true;
}
#endif /* DEBUG */
 ...
 */
MM_SweepPoolManager *MM_MemoryPool::getSweepPoolManager() {
 ...
 */
void MM_MemoryPool::kill(MM_EnvironmentBase *env) {
  tearDown(env);
  env->getForge()->free(this);
}

#if defined(OMR_GC_IDLE_HEAP_MANAGER)
uintptr_t MM_MemoryPool::releaseFreeMemoryPages(MM_EnvironmentBase *env) {
  /* Should have been implemented */
  Assert_MM_unreachable();
  return 0;
}
