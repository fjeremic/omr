...
public:
  ...
  MMINLINE bool isCommittedHeapObject(omrobjectptr_t objectPtr) {
    return ((_committedHeapBase <= (uint8_t *)objectPtr) &&
            (_committedHeapTop > (uint8_t *)objectPtr));
  }

  MMINLINE bool inlineMarkValidObjectNoCheckNoAtomic(MM_EnvironmentBase *env,
                                                     omrobjectptr_t objectPtr) {
  ...
  MMINLINE bool inlineMarkValidObjectNoAtomic(MM_EnvironmentBase *env,
                                              omrobjectptr_t objectPtr) {
    bool didMark = false;
  ...
  bool markValidObjectNoAtomic(MM_EnvironmentBase *env,
                               omrobjectptr_t objectPtr);

  MMINLINE bool inlineMarkValidObjectNoCheck(MM_EnvironmentBase *env,
                                             omrobjectptr_t objectPtr) {
    assertSaneObjectPtr(env, objectPtr);
  ...
  MMINLINE bool inlineMarkValidObject(MM_EnvironmentBase *env,
                                      omrobjectptr_t objectPtr) {
    bool didMark = false;
  ...
  MMINLINE bool isValidObject(omrobjectptr_t objectPtr) {
    bool shouldCheck =
        isCommittedHeapObject(objectPtr) &&
        OMR_ARE_NO_BITS_SET((uintptr_t)objectPtr,
                            _extensions->getObjectAlignmentInBytes() - 1);
    if (shouldCheck) {
   ...
   */
  void markValidObjectForRange(MM_EnvironmentBase *env, void *heapBase,
                               void *heapTop);

  MM_ObjectMap(MM_EnvironmentBase *env)
      : MM_BaseVirtual(), _extensions(env->getExtensions()), _objectMap(NULL),
        _heapBase(NULL), _heapTop(NULL), _committedHeapBase(NULL),
