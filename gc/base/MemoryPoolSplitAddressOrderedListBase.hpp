...
public:
  ...
  MMINLINE void addHint(MM_HeapLinkedFreeHeader *freeEntry,
  ...
  MMINLINE void updateHint(MM_HeapLinkedFreeHeader *oldFreeEntry,
  ...
  J9ModronFreeList()
      : _freeList(NULL), _timesLocked(0), _freeSize(0), _freeCount(0),
        _hintActive(NULL), _hintInactive(NULL), _hintLru(0) {}
};
...
class MM_MemoryPoolSplitAddressOrderedListBase
    : public MM_MemoryPoolAddressOrderedListBase {
  /*
   * Data members
   */
private:
...
protected:
  ...
  MMINLINE uintptr_t findGoodStartFreeList() {
    ...
    for (uintptr_t i = 0; i < _heapFreeListCount; ++i) {
      if ((NULL != _heapFreeLists[i]._freeList) &&
          (_heapFreeLists[i]._timesLocked < timesLocked)) {
        index = i;
   ...
   */
  MMINLINE void
  setNextForFreeEntryInFreeList(J9ModronFreeList *freelist,
...
public:
  ...
  /**
   * Merge the current LargeObject allocation distribution stats from free
   * lists.
   */
   ...
   */
  MM_MemoryPoolSplitAddressOrderedListBase(MM_EnvironmentBase *env,
      ...
      : MM_MemoryPoolAddressOrderedListBase(env, minimumFreeEntrySize),
        ...
        _heapFreeListCountExtended(splitAmount), _currentThreadFreeList(0),
        _heapFreeLists(NULL), _largeObjectAllocateStatsForFreeList(NULL),
