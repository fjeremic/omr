...
#define J9MODRON_HMI_HEAPMAP_ALIGNMENT                                         \
  (J9MODRON_HEAP_SLOTS_PER_HEAPMAP_SLOT * sizeof(uintptr_t))
...
private:
  uintptr_t *_heapSlotCurrent;    /**< Current heap slot that corresponds to the
                                     heap map bit index being scanned */
  ...
  bool _useLargeObjectOptimization; /**< Set to true if we want to read objects
                                       from the heap and determine their size in
                                       order to skip mark map bits which are
                                       inside the object.  If this is set to
                                       false, we will blindly return the
                                       addresses representing the set bits in
                                       the mark map */
...
public:
  ...
  bool reset(MM_HeapMap *heapMap, uintptr_t *heapChunkBase,
             uintptr_t *heapChunkTop);

  MM_HeapMapIterator(MM_GCExtensionsBase *extensions, MM_HeapMap *heapMap,
                     uintptr_t *heapChunkBase, uintptr_t *heapChunkTop,
                     bool useLargeObjectOptimization = true)
  ...
  MM_HeapMapIterator(MM_GCExtensionsBase *extensions)
      : _heapSlotCurrent(NULL), _heapChunkTop(NULL), _heapMapSlotCurrent(NULL),
        _bitIndexHead(0), _heapMapSlotValue(0), _extensions(extensions),
