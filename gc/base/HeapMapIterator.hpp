...
private:
    ...
    uintptr_t*
        _heapMapSlotCurrent; /**< Current heap map slot that contains the bits to scan for the corresponding heap */
    ...
    bool _useLargeObjectOptimization; /**< Set to true if we want to read objects from the heap and determine their size
                                         in order to skip mark map bits which are inside the object.  If this is set to
                                         false, we will blindly return the addresses representing the set bits in the
                                         mark map */
...
public:
    ...
    MM_HeapMapIterator(MM_GCExtensionsBase* extensions, MM_HeapMap* heapMap, uintptr_t* heapChunkBase,
        uintptr_t* heapChunkTop, bool useLargeObjectOptimization = true)
