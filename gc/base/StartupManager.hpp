...
protected:
  ...
  static void *
  operator new(size_t); /* Since we override delete, prevent heap allocation. */
...
public:
  ...
  createCollectorLanguageInterface(MM_EnvironmentBase *env) = 0;
  ...
  MM_StartupManager(OMR_VM *omrVM, uintptr_t defaultMinHeapSize,
                    uintptr_t defaultMaxHeapSize)
      : verboseFileName(NULL), omrVM(omrVM),
