...
public:
  struct PacketSublist {
    ...
    MM_LightweightNonReentrantLock
        _lock; /**< Lock for getting/putting packets */

    bool initialize(MM_EnvironmentBase *env) {
      if (!_lock.initialize(env, &env->getExtensions()->lnrlOptions,
                            "MM_PacketList:_sublists[]._lock")) {
        return false;
      }
      return true;
    }

    PacketSublist() : _head(NULL), _tail(NULL) {}
  };

protected:
private:
  struct PacketSublist *_sublists; /**< An array of PacketSublist structures
                                      which is _sublistCount elements long */
...
private:
   ...
   */
  MMINLINE uintptr_t getSublistIndex(MM_EnvironmentBase *env) {
...
public:
   ...
   */
  MMINLINE bool isEmpty() { return 0 == _count; }
  ...
  MM_PacketList(MM_EnvironmentBase *env)
      : MM_BaseNonVirtual(), _sublists(NULL), _sublistCount(0), _count(0) {
