...
private:
  MM_MemoryHandle
      _cardTableMemoryHandle; /**< memory handle for array backing store */
...
public:
  /**
   * Returns the base address of the card table (looked up for inline write
   * barriers, etc)
   * @return The address of the card which tracks changes to the first byte in
   * the heap
   */
  Card *getCardTableStart() { return _cardTableStart; };

  /**
   * Returns a "virtual" base address of the card table, effectively allowing it
   * to be indexed by a shifted heap address, alone. This is calculated by
   * subtracting the shifted address of the first byte of the heap from the base
   * of the card table.
   * @return A representation of the base of the card table which can be indexed
   * by a shifted heap address
   */
  ...
  /**
   * Dirties the card backing the given object.  Called, primarily, by the write
   * barrier. Mark the card in the card table given the address of an object
   * which contains an updated reference if it is not already dirty.
   * @param[in] env The thread which is attempting to write to obj
   * @param[in] objectRef The object being modified
   * @note Called by the write barrier so this must be fast (although the
   * operation is inlined, in the JIT)
   */
  void dirtyCard(MM_EnvironmentBase *env, omrobjectptr_t objectRef);

  /**
   * Dirties the card backing the given object with the passed cardValue.  This
   * is called during specific kinds of card cleaning or during overflow, in
   * Tarok.
   * @param[in] env A GC thread
   * @param[in] objectRef The object whose card is to be dirtied
   * @param[in] cardValue The value to write into the card (must not be
   * CARD_CLEAN or CARD_INVALID)
   */
  void dirtyCardWithValue(MM_EnvironmentBase *env, omrobjectptr_t objectRef,
                          Card cardValue);
   ...
   */
  void dirtyCardRange(MM_EnvironmentBase *env, void *heapAddrFrom,
                      void *heapAddrTo);

  /**
   * Calculate the address of the card which tracks the changes to the given
   * heap address.
   * @param[in] env The thread making the request
   * @param[in] heapAddr The heap address to convert
   * @return The address of the card which tracks changes to the given heap
   * address
  ...
  /**
   * The inverse operation to the heapAddrToCardAddr:  returns the base heap
   * address that the given cardAddr covers.
   * @param[in] env The thread making the request
   ...
   * @param cardCleaner[in] The card cleaner implementation which will be
   * invoked to clean each card
   */
   ...
   * @param[in] heapTop Top (non-inclusive) of heap range whoose associated
   * cards are to be cleared
   * @return The size, in bytes, of the card table fragment cleared
   */
  uintptr_t clearCardsInRange(MM_EnvironmentBase *env, void *heapBase,
                              void *heapTop);

  /**
   * Called to request that that the CardTable for specified heap range be
   * cleaned. This multi-threaded version must be executed under Parallel Task
   * only
   * @param env[in] The thread passed to the driveGlobalCollectMasterThread call
   * @param cardCleaner[in] The card cleaner implementation which will be
   * invoked to clean each card
   * @param lowAddress lowerst address of heap Card Table must be cleaned for
   * (inclusive)
   * @param highAddress highest address of heap Card Table must be cleaned for
   * (exclusive)
   */
  void cleanCardTableForRange(MM_EnvironmentBase *env,
                              MM_CardCleaner *cardCleaner, void *lowAddress,
                              void *highAddress);

  /**
   * Called to request that that the CardTable under a specific region be
   * cleaned.
   * @param env[in] The thread passed to the driveGlobalCollectMasterThread call
   * @param cardCleaner[in] The card cleaner implementation which will be
   * invoked to clean each card
   ...
   */
  void cleanCardsInRegion(MM_EnvironmentBase *env, MM_CardCleaner *cardCleaner,
   ...
   * @return The size, in bytes, of the card table required to track changes to
   * a heap of the given size
   * @note only called from MM_ConcurrentCardTable
   */
  static uintptr_t calculateCardTableSize(MM_EnvironmentBase *env,
                                          uintptr_t heapsize);
  ...
  /**
   * Check is heap memory correspondent with specified interval in card table
   * might be decommited (nobody use it)
  ...
  /**
   * Commits the memory for the portion of the card table corresponding to the
   * heap range in the given region.
   ...
   */
  bool commitCardsForRegion(MM_EnvironmentBase *env,
                            MM_HeapRegionDescriptor *region);
   ...
   * @param baseOfHeapRange[in] The heap address contained in the first card in
   * the range to be bound
   * @param topOfHeapRange[in] The heap address following the last card in the
   * range to be bound
   * @return True if the binding was successful
   */
  bool setNumaAffinityCorrespondingToHeapRange(MM_EnvironmentBase *env,
                                               uintptr_t numaNode,
                                               void *baseOfHeapRange,
                                               void *topOfHeapRange);
...
protected:
  ...
  /**
   * Commits the card table range between lowCard and highCard:  [lowCard,
   * highCard)
   * @return false if the commit failed
   */
  bool commitCardTableMemory(MM_EnvironmentBase *env, Card *lowCard,
                             Card *highCard);

  /**
   * Decommits the card table range between lowCard and highCard:  [lowCard,
   * highCard]
   * @return false if the decommit failed
   */
  bool decommitCardTableMemory(MM_EnvironmentBase *env, Card *lowCard,
                               Card *highCard, Card *lowValidCard,
                               Card *highValidCard);
   ...
   */
  MM_CardTable()
      : MM_BaseVirtual(), _heapAlloc(NULL), _cardTableMemoryHandle(),
        _cardTableStart(NULL), _cardTableVirtualStart(NULL), _heapBase(NULL) {
    _typeId = __FUNCTION__;
  }

private:
  void cleanRange(MM_EnvironmentBase *env, MM_CardCleaner *cardCleaner,
                  Card *low, Card *high);
};
