 ...
 */
void GC_ObjectHeapIteratorAddressOrderedList::reset(uintptr_t* base,
                                                    uintptr_t* top) {
...
uintptr_t GC_ObjectHeapIteratorAddressOrderedList::computeDeadObjectSize() {
  ...
  if (_isSingleSlotHole) {
    deadObjectSize =
        _extensions->objectModel.getSizeInBytesSingleSlotDeadObject(_scanPtr);
  } else {
    deadObjectSize =
        _extensions->objectModel.getSizeInBytesMultiSlotDeadObject(_scanPtr);
  }

  return deadObjectSize;
}

void GC_ObjectHeapIteratorAddressOrderedList::advanceScanPtr(
    uintptr_t increment) {
...
omrobjectptr_t GC_ObjectHeapIteratorAddressOrderedList::nextObjectNoAdvance() {
  ...
  while (_scanPtr < _scanPtrTop) {
    /* These flags were set before we returned the last object, but the object
     * might have changed. */
    _isDeadObject = _extensions->objectModel.isDeadObject(_scanPtr);
    _isSingleSlotHole =
        _isDeadObject
    ...
    if (!_isDeadObject) {
      ...
      if (header.isStrictlyForwardedPointer()) {
        uintptr_t sizeInBytesBeforeMove =
            _extensions->objectModel.getConsumedSizeInBytesWithHeaderBeforeMove(
                header.getForwardedObject());
        _scanPtr =
            (omrobjectptr_t)(((uintptr_t)_scanPtr) + sizeInBytesBeforeMove);
      } else
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
      {
