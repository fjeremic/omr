...
void* MM_MemorySubSpaceSemiSpace::allocateObject(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* baseSubSpace, MM_MemorySubSpace* previousSubSpace, bool shouldCollectOnFailure)
{
    Trc_MM_MSSSS_allocate_entry(env->getLanguageVMThread(), "Object", allocDescription->getBytesRequested(), this,
        getName(), baseSubSpace, previousSubSpace, (uintptr_t)shouldCollectOnFailure);
    ...
    if (shouldCollectOnFailure) {
        ...
        if (previousSubSpace == _parent) {
            ...
            Trc_MM_MSSSS_allocate4(env->getLanguageVMThread(), "Object", allocDescription->getBytesRequested(),
...
void* MM_MemorySubSpaceSemiSpace::allocationRequestFailed(MM_EnvironmentBase* env,
    MM_AllocateDescription* allocateDescription, AllocationType allocationType,
    MM_ObjectAllocationInterface* objectAllocationInterface, MM_MemorySubSpace* baseSubSpace,
    MM_MemorySubSpace* previousSubSpace)
{
    Trc_MM_MSSSS_allocationRequestFailed_entry(env->getLanguageVMThread(), allocateDescription->getBytesRequested(),
        this, getName(), baseSubSpace, previousSubSpace, (uintptr_t)allocationType);

    /* The baseSubSpace in this case isn't really valid - in the event of a scavenge, the baseSubSpace changes.  The
     * current subSpace will become the base for any restarts
     */
    void* addr = NULL;
    ...
    if (!env->acquireExclusiveVMAccessForGC(_collector, true, true)) {
        ...
        addr = allocateGeneric(
            env, allocateDescription, allocationType, objectAllocationInterface, _memorySubSpaceAllocate);
        if (NULL != addr) {
            Trc_MM_MSSSS_allocationRequestFailed_exit(
                env->getLanguageVMThread(), allocateDescription->getBytesRequested(), 1, addr);
            return addr;
        }
        ...
        if (!env->acquireExclusiveVMAccessForGC(_collector)) {
            ...
            Trc_MM_MSSSS_allocationRequestFailed(
                env->getLanguageVMThread(), allocateDescription->getBytesRequested(), 2);
            addr = allocateGeneric(
                env, allocateDescription, allocationType, objectAllocationInterface, _memorySubSpaceAllocate);
            if (NULL != addr) {
                /* Satisfied the allocate after having grabbed exclusive access to perform a GC (without actually
                 * performing the GC).  Raise an event for tracing / verbose to report the occurrence.
                 */
                reportAcquiredExclusiveToSatisfyAllocate(env, allocateDescription);
                Trc_MM_MSSSS_allocationRequestFailed_exit(
                    env->getLanguageVMThread(), allocateDescription->getBytesRequested(), 2, addr);
                return addr;
            }
            allocateDescription->saveObjects(env);
        }
    ...
    /* we will pass this as the target subspace for the restarted allocation because the subspaces will swap, if the
     * collection is successful, but not if we backout so we don't know which subspace will allocate until after the
     ...
     * OMRTODO we might be short-cutting here. more formal approach is to pass allocationRequestFailed to parent and he
     * decides what to do
     */
    MM_MemorySubSpace* topLevelMemorySubSpaceNew = getTopLevelMemorySubSpace(MEMORY_TYPE_NEW);
    addr = _collector->garbageCollect(env, topLevelMemorySubSpaceNew, allocateDescription,
    ...
    if (NULL != addr) {
        ...
        Trc_MM_MSSSS_allocationRequestFailed_exit(
            env->getLanguageVMThread(), allocateDescription->getBytesRequested(), 3, addr);
        return addr;
    }
     ...
     *	Calling of parent for TLH is wrong because objects allocated into it on the in-line path won't have the OLD bit
     *set CMVC 144061
     */
    if (ALLOCATION_TYPE_TLH != allocationType) {
        ...
        addr = _parent->allocationRequestFailed(
            env, allocateDescription, allocationType, objectAllocationInterface, this, this);
    }

    Trc_MM_MSSSS_allocationRequestFailed_exit(
        env->getLanguageVMThread(), allocateDescription->getBytesRequested(), 4, addr);
    return addr;
}

void* MM_MemorySubSpaceSemiSpace::allocateArrayletLeaf(MM_EnvironmentBase* env,
    MM_AllocateDescription* allocDescription, MM_MemorySubSpace* baseSubSpace, MM_MemorySubSpace* previousSubSpace,
...
void* MM_MemorySubSpaceSemiSpace::allocateTLH(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_ObjectAllocationInterface* objectAllocationInterface, MM_MemorySubSpace* baseSubSpace,
    MM_MemorySubSpace* previousSubSpace, bool shouldCollectOnFailure)
{
    /* We may receive this call from the MemorySubSpaceGeneric for which we are the parent but we shouldn't satisfy TLH
     * allocates in this space (since that would allow us to fill old space before collecting new space) so just return
     * NULL. This call has been observed on Vich configurations, at the very least.
     */
    return NULL;
}
 ...
 ****************************************
 */
void MM_MemorySubSpaceSemiSpace::systemGarbageCollect(MM_EnvironmentBase* env, uint32_t gcCode)
{
    if (_collector) {
 ...
 */
bool MM_MemorySubSpaceSemiSpace::isChildActive(MM_MemorySubSpace* memorySubSpace)
{
...
bool MM_MemorySubSpaceSemiSpace::isObjectInEvacuateMemory(omrobjectptr_t objectPtr)
{
...
bool MM_MemorySubSpaceSemiSpace::isObjectInNewSpace(void* objectBase, void* objectTop)
{
    return (objectBase >= _survivorSpaceBase) && (objectTop <= _survivorSpaceTop);
}

uintptr_t MM_MemorySubSpaceSemiSpace::getMaxSpaceForObjectInEvacuateMemory(omrobjectptr_t objectPtr)
{
    uintptr_t result = 0;
 ...
 ****************************************
 */
uintptr_t MM_MemorySubSpaceSemiSpace::getActiveMemorySize(uintptr_t includeMemoryType)
{
 ...
 * The semispace implementation will only report free memory associated to the allocate memory subspace.
 */
uintptr_t MM_MemorySubSpaceSemiSpace::getActualActiveFreeMemorySize(uintptr_t includeMemoryType)
{
    if (includeMemoryType & MEMORY_TYPE_NEW) {
        return _memorySubSpaceAllocate->getActualActiveFreeMemorySize();
    } else {
 ...
 * The semispace implementation will only report free memory associated to the allocate memory subspace.
 */
uintptr_t MM_MemorySubSpaceSemiSpace::getApproximateActiveFreeMemorySize(uintptr_t includeMemoryType)
{
    if (includeMemoryType & MEMORY_TYPE_NEW) {
        return _memorySubSpaceAllocate->getApproximateActiveFreeMemorySize();
    } else {
...
uintptr_t MM_MemorySubSpaceSemiSpace::getActiveSurvivorMemorySize(uintptr_t includeMemoryType)
{
...
uintptr_t MM_MemorySubSpaceSemiSpace::getApproximateActiveFreeSurvivorMemorySize(uintptr_t includeMemoryType)
{
    if (includeMemoryType & MEMORY_TYPE_NEW) {
...
uintptr_t MM_MemorySubSpaceSemiSpace::getActualActiveFreeSurvivorMemorySize(uintptr_t includeMemoryType)
{
    if (getTypeFlags() & MEMORY_TYPE_NEW) {
        return _memorySubSpaceSurvivor->getActiveMemorySize(MEMORY_TYPE_NEW);
    } else {
...
void MM_MemorySubSpaceSemiSpace::abandonHeapChunk(void* addrBase, void* addrTop)
{
    Assert_MM_unreachable();
}
...
void MM_MemorySubSpaceSemiSpace::mergeHeapStats(MM_HeapStats* heapStats, uintptr_t includeMemoryType)
{
    if (includeMemoryType & MEMORY_TYPE_NEW) {
        _memorySubSpaceAllocate->mergeHeapStats(heapStats);
        _memorySubSpaceSurvivor->mergeHeapStats(heapStats);
 ...
 * Initialization
 */
MM_MemorySubSpaceSemiSpace* MM_MemorySubSpaceSemiSpace::newInstance(MM_EnvironmentBase* env, MM_Collector* collector,
    MM_PhysicalSubArena* physicalSubArena, MM_MemorySubSpace* memorySubSpaceAllocate,
    MM_MemorySubSpace* memorySubSpaceSurvivor, bool usesGlobalCollector, uintptr_t minimumSize, uintptr_t initialSize,
    uintptr_t maximumSize)
{
    ...
    memorySubSpace = (MM_MemorySubSpaceSemiSpace*)env->getForge()->allocate(
        sizeof(MM_MemorySubSpaceSemiSpace), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (memorySubSpace) {
        new (memorySubSpace) MM_MemorySubSpaceSemiSpace(env, collector, physicalSubArena, memorySubSpaceAllocate,
            memorySubSpaceSurvivor, usesGlobalCollector, minimumSize, initialSize, maximumSize);
...
bool MM_MemorySubSpaceSemiSpace::initialize(MM_EnvironmentBase* env)
{
    ...
    _largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(env,
        (uint16_t)_extensions->largeObjectAllocationProfilingTopK, _extensions->largeObjectAllocationProfilingThreshold,
        ...
        _extensions->heap->getMaximumMemorySize(), tlhMaximumSize + _extensions->minimumFreeEntrySize,
...
void MM_MemorySubSpaceSemiSpace::flip(MM_EnvironmentBase* env, Flip_step step)
{
    ...
    case restore_allocation_and_set_survivor:
        ...
        _avgBytesAllocatedDuringConcurrent = (uintptr_t)MM_Math::weightedAverage(
            (float)_avgBytesAllocatedDuringConcurrent, (float)(_bytesAllocatedDuringConcurrent), 0.7f);
        _deviationBytesAllocatedDuringConcurrent
            = ((float)_bytesAllocatedDuringConcurrent - (float)_avgBytesAllocatedDuringConcurrent);
        _avgDeviationBytesAllocatedDuringConcurrent = sqrtf(MM_Math::weightedAverage(
            _avgDeviationBytesAllocatedDuringConcurrent * _avgDeviationBytesAllocatedDuringConcurrent,
    ...
    case backout:
        ...
        /* We have objects on both sides of Nursery. We will unify the two sides and do a compacting slide (after
         * percolate global GC) Enforce Allocate be in low address range, since compact slide in percolate global GC
         * moves objects to low addresses. Note: _allocateSpaceBase/Top are stale (not updated in set_allocate), since
         * they are overloaded to point to Evacuate during an active cycle
         */
        if (_allocateSpaceBase < _survivorSpaceBase) {
            ...
            getMemorySpace()->setDefaultMemorySubSpace(getDefaultMemorySubSpace());
            if (debug) {
                omrtty_printf("tilt backout _allocateSpaceBase/Top %llx/%llx _survivorSpaceBase/Top %llx/%llx tilt "
                              "sizes %llx %llx\n",
                    _allocateSpaceBase, _allocateSpaceTop, _survivorSpaceBase, _survivorSpaceTop,
                    (uintptr_t)_allocateSpaceTop - (uintptr_t)_allocateSpaceBase + (uintptr_t)_survivorSpaceTop
                        - (uintptr_t)_survivorSpaceBase,
                    (uintptr_t)0);
            }
        } else {
            _memorySubSpaceSurvivor = _memorySubSpaceEvacuate;
            ...
            if (debug) {
                omrtty_printf("tilt backout forced flip _allocateSpaceBase/Top %llx/%llx _survivorSpaceBase/Top "
                              "%llx/%llx tilt sizes %llx %llx\n",
                    _allocateSpaceBase, _allocateSpaceTop, _survivorSpaceBase, _survivorSpaceTop,
                    (uintptr_t)_allocateSpaceTop - (uintptr_t)_allocateSpaceBase + (uintptr_t)_survivorSpaceTop
                        - (uintptr_t)_survivorSpaceBase,
                    (uintptr_t)0);
        ...
        /* Tilt 100% on PhysicalSubArena level (leave SubSpaces unchanged). Give everything to the low address
         * (Allocate) part */
        _physicalSubArena->tilt(env,
            (uintptr_t)_allocateSpaceTop - (uintptr_t)_allocateSpaceBase + (uintptr_t)_survivorSpaceTop
                - (uintptr_t)_survivorSpaceBase,
            (uintptr_t)0, false);
    ...
    case restore_tilt_after_percolate: {
        ...
        if (debug) {
            omrtty_printf(
                "tilt restore_tilt_after_percolate heapAlignedLastFreeEntry %llx section (%llx) aligned size %llx\n",
        ...
        if (debug) {
            omrtty_printf(
                "tilt restore_tilt_after_percolate allocateSize %llx survivorSize %llx\n", allocateSize, survivorSize);
        }
...
void MM_MemorySubSpaceSemiSpace::cacheRanges(MM_MemorySubSpace* subSpace, void** base, void** top)
{
...
void MM_MemorySubSpaceSemiSpace::poisonEvacuateSpace()
{
    ...
    valgrindClearRange(
        _extensions, (uintptr_t)_allocateSpaceBase, (uintptr_t)_allocateSpaceTop - (uintptr_t)_allocateSpaceBase);
    ...
    valgrindMakeMemNoaccess(
        (uintptr_t)_allocateSpaceBase, (uintptr_t)_allocateSpaceTop - (uintptr_t)_allocateSpaceBase);
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
}

void MM_MemorySubSpaceSemiSpace::tilt(MM_EnvironmentBase* env, uintptr_t allocateSpaceSize, uintptr_t survivorSpaceSize)
{
    _physicalSubArena->tilt(env, allocateSpaceSize, survivorSpaceSize);
}

void MM_MemorySubSpaceSemiSpace::tilt(MM_EnvironmentBase* env, uintptr_t survivorSpaceSizeRatioRequest)
{
    _physicalSubArena->tilt(env, survivorSpaceSizeRatioRequest);
}

void MM_MemorySubSpaceSemiSpace::masterTeardownForSuccessfulGC(MM_EnvironmentBase* env)
{
...
void MM_MemorySubSpaceSemiSpace::masterTeardownForAbortedGC(MM_EnvironmentBase* env)
{
    ...
    if (_extensions->isConcurrentScavengerEnabled()) {
        ...
        /* Restoring allocation after aborted scavenge will probably not help with re-attempts to allocate immediately
         * after an aborted scavenge, but still we have to restore it at some point for attempts after percolate GC.
         * It's simplest to do it right away.
         */
 ...
 */
void MM_MemorySubSpaceSemiSpace::checkSubSpaceMemoryPostCollectTilt(MM_EnvironmentBase* env)
{
    ...
    if (extensions->tiltedScavenge) {
        ...
        if (0 != extensions->scavengerStats._failedFlipCount) {
            ...
            _tiltedAverageBytesFlipped
                = (uintptr_t)MM_Math::weightedAverage((float)_tiltedAverageBytesFlipped, (float)flipBytes, 0.0f);
            ...
            if (flipBytes > _tiltedAverageBytesFlipped) {
                ...
                _tiltedAverageBytesFlipped
                    = (uintptr_t)MM_Math::weightedAverage((float)_tiltedAverageBytesFlipped, (float)flipBytes, 0.2f);
                _tiltedAverageBytesFlippedDelta = (uintptr_t)MM_Math::weightedAverage(
                ...
                _tiltedAverageBytesFlipped
                    = (uintptr_t)MM_Math::weightedAverage((float)_tiltedAverageBytesFlipped, (float)flipBytes, 0.8f);
                _tiltedAverageBytesFlippedDelta = (uintptr_t)MM_Math::weightedAverage(
        ...
        double desiredSurvivorSize
            = (_tiltedAverageBytesFlipped + _tiltedAverageBytesFlippedDelta) * survivorSizeAmplification;

#if defined(OMR_GC_CONCURRENT_SCAVENGER)
        if (_extensions->isConcurrentScavengerEnabled()) {
            /* Account for mutator allocated objects in hybrid survivor/allocated during concurrent phase of Concurrent
             * Scavenger */
 ...
 */
void MM_MemorySubSpaceSemiSpace::checkSubSpaceMemoryPostCollectResize(MM_EnvironmentBase* env)
{
    MM_GCExtensionsBase* extensions = MM_GCExtensionsBase::getExtensions(env->getOmrVM());
    ...
    if (extensions->dynamicNewSpaceSizing) {
        ...
        uint64_t intervalTime = omrtime_hires_delta(
            _lastScavengeEndTime, extensions->scavengerStats._endTime, OMRPORT_TIME_DELTA_IN_MILLISECONDS);
        ...
        uint64_t scavengeTime = omrtime_hires_delta(extensions->scavengerStats._startTime,
            extensions->scavengerStats._endTime, OMRPORT_TIME_DELTA_IN_MILLISECONDS);
        ...
        if (doDynamicNewSpaceSizing) {
            double expectedTimeRatio
                = (extensions->dnssExpectedTimeRatioMaximum + extensions->dnssExpectedTimeRatioMinimum) / 2;
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
            if (_extensions->isConcurrentScavengerEnabled()) {
                /* In CS world, we cannot easily measure GC overhead. This is just a simple adjustment, which is
                 * somewhat ok for light to medium loaded systems. In overloaded systems (where GC background threads
                 * are starved and GC prolonged), we may expand more than necessary. Multi JVM configuration may skew
                 * this even more (perhaps it's better to base it on CPU count, rather than GC thread count?). */
                timeRatio = timeRatio * _extensions->concurrentScavengerBackgroundThreads
            ...
            if ((_averageScavengeTimeRatio > extensions->dnssExpectedTimeRatioMaximum) && (NULL != _physicalSubArena)
                && _physicalSubArena->canExpand(env) && (maxExpansionInSpace(env) != 0)) {
                ...
                _expansionSize = MM_Math::roundToCeiling(
                    extensions->heapAlignment, (uintptr_t)(getCurrentSize() * adjustedExpansionFactor));
            ...
            if (_averageScavengeTimeRatio < extensions->dnssExpectedTimeRatioMinimum && (NULL != _physicalSubArena)
                && _physicalSubArena->canContract(env) && (maxContractionInSpace(env) != 0)) {
                ...
                _contractionSize = MM_Math::roundToCeiling(
                    extensions->heapAlignment, (uintptr_t)(getCurrentSize() * adjustedContractionFactor));
...
void MM_MemorySubSpaceSemiSpace::checkResize(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, bool systemGC)
{
...
intptr_t MM_MemorySubSpaceSemiSpace::performResize(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
...
MM_LargeObjectAllocateStats* MM_MemorySubSpaceSemiSpace::getLargeObjectAllocateStats()
{
    return _largeObjectAllocateStats;
}

void MM_MemorySubSpaceSemiSpace::mergeLargeObjectAllocateStats(MM_EnvironmentBase* env)
{
