...
protected:
  ...
  volatile uintptr_t
      _largestFreeEntry; /**< largest free entry found at the end of a global GC
                            cycle (no meaning during mutator execution) */

  uintptr_t
      _approximateFreeMemorySize; /**< The approximate number of bytes free that
                                     could be made part of the free list */
...
public:
  ...
  MM_LargeObjectAllocateStats *getLargeObjectAllocateStats() {
  ...
  /**
   * initialize frequentAllocation and reset the count of
   * FreeEntrySizeClassStats
   * @param[in] largeObjectAllocateStats
   */
  void resetFreeEntryAllocateStats(
    ...
    largeObjectAllocateStats->getFreeEntrySizeClassStats()
        ->initializeFrequentAllocation(
            topLevelMemoryPool->getLargeObjectAllocateStats());
    largeObjectAllocateStats->getFreeEntrySizeClassStats()->resetCounts();
  }

  /**< Configure the pool to append collector large allocate stats to the
   * mutator large allocate stats */
  virtual void appendCollectorLargeAllocateStats() {}
  /**< Merge stats (from children pools or split free lists) for free entry
   * stats */
   ...
   */
  virtual void
  averageLargeObjectAllocateStats(MM_EnvironmentBase *env,
  ...
  MMINLINE uintptr_t setMinimumFreeEntrySize(uintptr_t minimumFreeEntrySize) {
    return _minimumFreeEntrySize = minimumFreeEntrySize;
  }

  virtual uintptr_t getActualFreeMemorySize();
  virtual uintptr_t getApproximateFreeMemorySize();

  MMINLINE virtual uintptr_t getCurrentLOASize() { return 0; }

  MMINLINE virtual uintptr_t getApproximateFreeLOAMemorySize() { return 0; }

  MMINLINE virtual uintptr_t getActualFreeEntryCount() {
    return _freeEntryCount;
  }

  MMINLINE void setFreeMemorySize(uintptr_t freeMemorySize) {
    _freeMemorySize = freeMemorySize;
  }
  MMINLINE void setFreeEntryCount(uintptr_t entryCount) {
    _freeEntryCount = entryCount;
  }
  MMINLINE void
  setApproximateFreeMemorySize(uintptr_t approximateFreeMemorySize) {
    _approximateFreeMemorySize = approximateFreeMemorySize;
  }

#if defined(DEBUG)
  MMINLINE virtual bool isMemoryPoolValid(MM_EnvironmentBase *env,
                                          bool postCollect) {
    return true;
  }
#endif
  ...
  MMINLINE void setPrevious(MM_MemoryPool *memoryPool) {
    _previous = memoryPool;
  }
  ...
  MMINLINE virtual void resetLargestFreeEntry() { _largestFreeEntry = 0; }
  MMINLINE virtual uintptr_t getLargestFreeEntry() { return _largestFreeEntry; }

  MMINLINE void setLargestFreeEntry(uintptr_t largestFreeEntry) {
    _largestFreeEntry = largestFreeEntry;
  }
  ...
  /**
   * Allocates the contiguous portion of an object.  This means all mixed
   * objects, all indexable objects outside of arraylet VMs, and arraylet spines
   * (including their leaves, if inline) take this path.
   */
  ...
  enum Cause { forSweep, forCompact, any };
   ...
   * When the code that uses these is cleaned up (e.g. sweep), these methods
   * should be removed
   */
  ...
  virtual void addFreeEntries(MM_EnvironmentBase *env,
  ...
  virtual bool removeFreeEntriesWithinRange(
  ...
  virtual void *findAddressAfterFreeSize(MM_EnvironmentBase *env,
   ...
   *
   * @return true if the entry was added to a free list, false if it is now
   * unusable
   */
   ...
   */
  MMINLINE void incrementDarkMatterBytes(uintptr_t bytes) {
  ...
  MMINLINE void incrementDarkMatterSamples(uintptr_t samples) {
    _darkMatterSamples += samples;
  }

  MMINLINE virtual uintptr_t getDarkMatterSamples() {
    return _darkMatterSamples;
  }
...
#endif /* defined(OMR_GC_COMPRESSED_POINTERS) && defined(OMR_GC_FULL_POINTERS) \
        */
        _next(NULL), _previous(NULL), _children(NULL), _poolName("Unknown"),
        _parent(NULL), _memorySubSpace(NULL),
        _minimumFreeEntrySize(minimumFreeEntrySize), _freeMemorySize(0),
        _freeEntryCount(0), _largestFreeEntry(0), _approximateFreeMemorySize(0),
        _allocCount(0), _allocBytes(0), _lastFreeBytes(0),
        _allocDiscardedBytes(0), _allocSearchCount(0),
        _extensions(env->getExtensions()), _largeObjectAllocateStats(NULL),
        _darkMatterBytes(0), _darkMatterSamples(0) {
...
#endif /* defined(OMR_GC_COMPRESSED_POINTERS) && defined(OMR_GC_FULL_POINTERS) \
        */
        _next(NULL), _previous(NULL), _children(NULL), _poolName(name),
        _parent(NULL), _memorySubSpace(NULL),
        _minimumFreeEntrySize(minimumFreeEntrySize), _freeMemorySize(0),
        _freeEntryCount(0), _largestFreeEntry(0), _approximateFreeMemorySize(0),
        _allocCount(0), _allocBytes(0), _lastFreeBytes(0),
