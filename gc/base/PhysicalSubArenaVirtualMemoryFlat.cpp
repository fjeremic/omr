 ...
 */
MM_PhysicalSubArenaVirtualMemoryFlat* MM_PhysicalSubArenaVirtualMemoryFlat::newInstance(
...
bool MM_PhysicalSubArenaVirtualMemoryFlat::inflate(MM_EnvironmentBase* env)
{
    ...
    if (_parent->attachSubArena(env, this, _subSpace->getInitialSize(), modron_pavm_attach_policy_none)) {
        ...
        _region
            = regionManager->createAuxiliaryRegionDescriptor(env, _subSpace->getChildren(), _lowAddress, _highAddress);
        if (NULL != _region) {
            ...
            result = _subSpace->expanded(
                env, this, _region->getSize(), _region->getLowAddress(), _region->getHighAddress(), false);
            _subSpace->heapReconfigured(env);
        }
    }
    return result;
 ...
 */
uintptr_t MM_PhysicalSubArenaVirtualMemoryFlat::expand(MM_EnvironmentBase* env, uintptr_t requestExpandSize)
{
    ...
    expandSize = OMR_MIN(
        expandSize, ((MM_PhysicalArenaVirtualMemory*)_parent)->getPhysicalMaximumExpandSizeHigh(env, _highAddress));
 ...
 */
uintptr_t MM_PhysicalSubArenaVirtualMemoryFlat::expandNoCheck(MM_EnvironmentBase* env, uintptr_t expandSize)
{
    ...
    if (_highAddress != highExpandAddress) {
        ...
        bool result
            = genericSubSpace->heapAddRange(env, genericSubSpace, expandSize, lowExpandAddress, highExpandAddress);
 ...
 */
bool MM_PhysicalSubArenaVirtualMemoryFlat::canContract(MM_EnvironmentBase* env)
{
    return _resizable;
}
 ...
 */
uintptr_t MM_PhysicalSubArenaVirtualMemoryFlat::contract(MM_EnvironmentBase* env, uintptr_t requestContractSize)
{
    ...
    contractSize = OMR_MIN(
        contractSize, ((MM_PhysicalArenaVirtualMemory*)_parent)->getPhysicalMaximumContractSizeLow(env, _highAddress));
