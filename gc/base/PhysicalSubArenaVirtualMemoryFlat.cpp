...
{
  bool result = false;
  if (_parent->attachSubArena(env,
                              this,
                              _subSpace->getInitialSize(),
    ...
    if (NULL != _region) {
      ...
      // child subspace
      result = _subSpace->expanded(env,
                                   this,
                                   _region->getSize(),
                                   _region->getLowAddress(),
                                   _region->getHighAddress(),
                                   false);
      _subSpace->heapReconfigured(env);
    }
  }
                                             ...
                                             uintptr_t requestExpandSize)
{
  ...
  expandSize = OMR_MIN(expandSize,
                       ((MM_PhysicalArenaVirtualMemory*)_parent)
                                                    ...
                                                    uintptr_t expandSize)
{
  ...
  if (_highAddress != highExpandAddress) {
    ...
    getHeapRegionManager()->resizeAuxillaryRegion(
      env, _region, _lowAddress, _highAddress);
                                               ...
                                               uintptr_t requestContractSize)
{
   ...
   */
  contractSize = _subSpace->counterBalanceContract(
    env, contractSize, extensions->heapAlignment);
  ...
  _heap->decommitMemory(
    (void*)contractBase, contractSize, lowValidAddress, highValidAddress);
  ...
  genericSubSpace->heapRemoveRange(env,
                                   _subSpace,
                                   contractSize,
                                   (void*)contractBase,
                                   (void*)contractTop,
