...
MM_PhysicalSubArenaVirtualMemoryFlat::newInstance(MM_EnvironmentBase *env,
                                                  MM_Heap *heap) {
  MM_PhysicalSubArenaVirtualMemoryFlat *subArena;

  subArena = (MM_PhysicalSubArenaVirtualMemoryFlat *)env->getForge()->allocate(
      sizeof(MM_PhysicalSubArenaVirtualMemoryFlat),
 ...
 */
void MM_PhysicalSubArenaVirtualMemoryFlat::kill(MM_EnvironmentBase *env) {
  tearDown(env);
  env->getForge()->free(this);
}

bool MM_PhysicalSubArenaVirtualMemoryFlat::initialize(MM_EnvironmentBase *env) {
 ...
 */
void MM_PhysicalSubArenaVirtualMemoryFlat::tearDown(MM_EnvironmentBase *env) {
 ...
 */
bool MM_PhysicalSubArenaVirtualMemoryFlat::inflate(MM_EnvironmentBase *env) {
  bool result = false;
  if (_parent->attachSubArena(env, this, _subSpace->getInitialSize(),
                              modron_pavm_attach_policy_none)) {
    ...
    if (NULL != _region) {
      ...
      // TODO: Like the semi space arena, this should dispatch directory to the
      // child subspace
      result = _subSpace->expanded(env, this, _region->getSize(),
                                   _region->getLowAddress(),
 ...
 * The expand request size is just a guideline - if the subarena cannot satisfy
 * the request,
 * @return Number of bytes the arena was expanded by
 */
uintptr_t MM_PhysicalSubArenaVirtualMemoryFlat::expand(
    MM_EnvironmentBase *env, uintptr_t requestExpandSize) {
  uintptr_t expandSize;

  /* Adjust the expand size to alignment requirements */
  expandSize = MM_Math::roundToCeiling(env->getExtensions()->heapAlignment,
                                       requestExpandSize);
  expandSize = MM_Math::roundToCeiling(getHeapRegionManager()->getRegionSize(),
                                       expandSize);

  /* Find the physical number of bytes that we could possibly expand by */
  expandSize = OMR_MIN(
      expandSize, ((MM_PhysicalArenaVirtualMemory *)_parent)
                      ->getPhysicalMaximumExpandSizeHigh(env, _highAddress));

  /* If we have a neighbouring arena, check if our expand request can be met -
   * if not, try and contract the neighbour. No matter what the result, adjust
   * the expand size to be as much as our neighbour will allow.
   */
  if (_highArena && ((((uintptr_t)_highArena->getLowAddress()) -
                      ((uintptr_t)_highAddress)) < expandSize)) {
    uintptr_t neighbourContractSize =
        expandSize -
        (((uintptr_t)_highArena->getLowAddress()) - ((uintptr_t)_highAddress));
    assume0(neighbourContractSize % env->getExtensions()->heapAlignment == 0);

    /* Contract the neighbour by the amount needed to achieve the request expand
     * size. Note that the contract may not get back everything we've asked for.
     */
    env->getExtensions()->heap->getResizeStats()->setLastContractReason(
        SATISFY_EXPAND);
    ...
    if ((((uintptr_t)_highArena->getLowAddress()) - ((uintptr_t)_highAddress)) <
        expandSize) {
      expandSize =
          ((uintptr_t)_highArena->getLowAddress()) - ((uintptr_t)_highAddress);
    }
  ...
  /* Check if the requested expand size fits into the current arena (sanity
   * check) */
   ...
   * expand request is ok (sanity check) */
  if (!((MM_PhysicalArenaVirtualMemory *)_parent)
           ->canExpand(env, this, _highAddress, expandSize)) {
 ...
 * Expand by the given size with no safety checks - checking we've assured that
 * the expand size will in fact fit and can be properly distributed within the
 * receivers parameters.
 * @return the size of the expansion (which should match the size supplied)
 */
uintptr_t MM_PhysicalSubArenaVirtualMemoryFlat::expandNoCheck(
    MM_EnvironmentBase *env, uintptr_t expandSize) {
  /* If we couldn't expand, how did we get here? */
  Assert_MM_true(((MM_PhysicalArenaVirtualMemory *)_parent)
                     ->canExpand(env, this, _highAddress, expandSize));
  Assert_MM_true(_lowAddress == _region->getLowAddress());
  ...
  if (_highAddress != highExpandAddress) {
    ...
    MM_MemorySubSpace *genericSubSpace =
        ((MM_MemorySubSpaceFlat *)_subSpace)->getChildSubSpace();

    bool result = genericSubSpace->heapAddRange(
        env, genericSubSpace, expandSize, lowExpandAddress, highExpandAddress);

    getHeapRegionManager()->resizeAuxillaryRegion(env, _region, _lowAddress,
                                                  _highAddress);
 ...
 */
bool MM_PhysicalSubArenaVirtualMemoryFlat::canContract(
 ...
 * @note the contraction size can only be reduced from the value requested
 * (space restrictions have already been imposed).
 * @return The amount of heap actually contracted.
 */
uintptr_t MM_PhysicalSubArenaVirtualMemoryFlat::contract(
    MM_EnvironmentBase *env, uintptr_t requestContractSize) {
  ...
  contractSize = OMR_MIN(
      contractSize, ((MM_PhysicalArenaVirtualMemory *)_parent)
                        ->getPhysicalMaximumContractSizeLow(env, _highAddress));

  /* Find the contract range available */
  uint8_t *contractBase = (uint8_t *)genericSubSpace->findFreeEntryEndingAtAddr(
      env, oldHighAddress);
  uint8_t *contractTop = (uint8_t *)oldHighAddress;
  uintptr_t contractSizeAvailable =
      ((uintptr_t)contractTop) - ((uintptr_t)contractBase);

  /* Calculate the actual physical memory available for contract */
  contractSize = OMR_MIN(contractSizeAvailable, contractSize);
  contractSize =
      MM_Math::roundToFloor(env->getExtensions()->heapAlignment, contractSize);
  contractSize = MM_Math::roundToFloor(getHeapRegionManager()->getRegionSize(),
                                       contractSize);
   ...
   */
  contractSize = _subSpace->counterBalanceContract(env, contractSize,
  ...
  /* Based on the decided contract size, calculate the range of heap to be
   * contracted */
  contractBase = contractTop - contractSize;

  /* Determine the surrounding valid memory addresses of the contract for
   * decommit purposes */
  ...
  genericSubSpace->removeExistingMemory(
      env, this, contractSize, (void *)contractBase, (void *)contractTop);

  /* Everything is ok - decommit the memory */
  _heap->decommitMemory((void *)contractBase, contractSize, lowValidAddress,
                        highValidAddress);
  ...
  genericSubSpace->heapRemoveRange(env, _subSpace, contractSize,
                                   (void *)contractBase, (void *)contractTop,
 ...
 */
uintptr_t MM_PhysicalSubArenaVirtualMemoryFlat::checkCounterBalanceExpand(
  ...
  if (NULL != _highArena) {
    ...
    physicalLimitHighAddress =
        (uintptr_t)((MM_PhysicalArenaVirtualMemory *)_parent)->getHighAddress();
  }
  physicalMaximumExpandSize =
      physicalLimitHighAddress - ((uintptr_t)_highAddress);
...
/**
 * @copydoc
 * MM_PhysicalSubArena::::getAvailableContractionSize(MM_EnvironmentBase *,
 * MM_MemorySubSpace *, MM_AllocateDescription *)
 */
uintptr_t MM_PhysicalSubArenaVirtualMemoryFlat::getAvailableContractionSize(
