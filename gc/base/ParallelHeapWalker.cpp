 ...
 */
void
MM_ParallelHeapWalker::allObjectsDoParallel(MM_EnvironmentBase* env,
                                            MM_HeapWalkerObjectFunc function,
                                            void* userData,
                                            uintptr_t walkFlags)
{
  ...
  while (NULL != (region = regionIterator.nextRegion())) {
    if (walkFlags == (region->getTypeFlags() & walkFlags)) {
      GC_ParallelObjectHeapIterator objectHeapIterator(env,
                                                       region,
 ...
 */
void
MM_ParallelHeapWalker::allObjectsDo(MM_EnvironmentBase* env,
                                    MM_HeapWalkerObjectFunc function,
                                    void* userData,
                                    uintptr_t walkFlags,
                                    bool parallel,
                                    bool prepareHeapForWalk)
{
  if (parallel) {
    ...
    MM_ParallelObjectDoTask objectDoTask(
      env, this, function, userData, walkFlags, parallel);
    env->getExtensions()->dispatcher->run(env, &objectDoTask);
  } else {
    MM_HeapWalker::allObjectsDo(
      env, function, userData, walkFlags, parallel, prepareHeapForWalk);
  }
}
