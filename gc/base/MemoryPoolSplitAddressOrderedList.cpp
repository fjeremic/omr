...
MM_MemoryPoolSplitAddressOrderedList::newInstance(
...
MM_MemoryPoolSplitAddressOrderedList::newInstance(
  ...
  memoryPool = (MM_MemoryPoolSplitAddressOrderedList*)env->getForge()->allocate(
      sizeof(MM_MemoryPoolSplitAddressOrderedList),
      OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
  if (memoryPool) {
    memoryPool = new (memoryPool) MM_MemoryPoolSplitAddressOrderedList(
        env, minimumFreeEntrySize, maxSplit, name);
...
MM_MemoryPoolSplitAddressOrderedList::internalAllocateFromList(
    MM_EnvironmentBase* env, uintptr_t sizeInBytesRequired,
    uintptr_t curFreeList, MM_HeapLinkedFreeHeader** previousFreeEntry,
  ...
  MM_HeapLinkedFreeHeader* currentFreeEntry =
      _heapFreeLists[curFreeList]._freeList;
  ...
  while (NULL != currentFreeEntry) {
    ...
    if (sizeInBytesRequired <= currentFreeEntrySize) {
      ...
      if (!isPreviousReservedFreeEntry(*previousFreeEntry, curFreeList)) {
        if (((walkCountCurrentList >=
              J9MODRON_ALLOCATION_MANAGER_HINT_MAX_WALK) ||
             ((walkCountCurrentList > 1) && allocateHintUsed))) {
          _heapFreeLists[curFreeList].addHint(candidateHintEntry,
                                              candidateHintSize);
        }
...
void* MM_MemoryPoolSplitAddressOrderedList::internalAllocate(
    MM_EnvironmentBase* env, uintptr_t sizeInBytesRequired,
    bool lockingRequired,
    MM_LargeObjectAllocateStats* largeObjectAllocateStatsForFreeList) {
  bool const compressed = compressObjectReferences();
...
retry:
  ...
  if (skipReserved) {
    curFreeList =
        _currentThreadFreeList[env->getEnvironmentId() % _heapFreeListCount];
  } else {
  ...
  do {
    if (NULL != _heapFreeLists[curFreeList]._freeList) {
      ...
      if (skipReserved) {
        ...
        currentFreeEntry =
            internalAllocateFromList(env, sizeInBytesRequired, curFreeList,
                                     &previousFreeEntry, &largestFreeEntry);
        if (NULL != currentFreeEntry) {
          /* found a freeEntry; will release lock only after we handle the
           * remainder */
          break;
        }
      } else {
  ...
  } while ((jumpedToSuggested || (curFreeList != suggestedFreeList)) &&
           skipReserved);
...
skipSearch:
  ...
  if (NULL == currentFreeEntry) {
    ...
    /* Since we failed to allocate, update the largest free entry so that
     * outside callers will be able to skip this pool, next time, in Tarok
     * configurations */
  ...
  Assert_MM_true(env->getExtensions()->objectModel.isDeadObject(
      (omrobjectptr_t)currentFreeEntry));
  ...
  _largeObjectAllocateStatsForFreeList[curFreeList]
      .decrementFreeEntrySizeClassStats(currentFreeEntry->getSize());
  ...
  if (recycleHeapChunk(env, recycleEntry,
                       ((uint8_t*)recycleEntry) + recycleEntrySize,
                       previousFreeEntry, currentFreeEntry->getNext(compressed),
                       curFreeList)) {
    if (!skipReserved &&
        isPreviousReservedFreeEntry(previousFreeEntry, curFreeList)) {
    ...
    _largeObjectAllocateStatsForFreeList[curFreeList]
        .incrementFreeEntrySizeClassStats(recycleEntrySize);
  } else {
    if (!skipReserved &&
        isPreviousReservedFreeEntry(previousFreeEntry, curFreeList)) {
  ...
  /* Was our initial or suggested freelist empty? If not, go back and use it
   * more. */
   ...
   * (_largeObjectCollectorAllocateStatsForFreeList is null for Survivor) */
  if (NULL != largeObjectAllocateStatsForFreeList) {
    largeObjectAllocateStatsForFreeList[curFreeList].allocateObject(
        sizeInBytesRequired);
  }
...
bool MM_MemoryPoolSplitAddressOrderedList::internalAllocateTLH(
...
retry:
    ...
    if (NULL != _heapFreeLists[curFreeList]._freeList) {
      ...
      if (NULL != freeEntry) {
        ...
        if (skipReserved &&
          ...
          freeEntry = freeEntry->getNext(compressed);
          if (NULL != freeEntry) {
            freeEntrySize = freeEntry->getSize();
            break;
          }
  ...
  } while ((jumpedToSuggested || (suggestedFreeList != curFreeList)) &&
           skipReserved);
...
skipSearch:
  ...
  _largeObjectAllocateStatsForFreeList[curFreeList]
      .decrementFreeEntrySizeClassStats(freeEntrySize);
   ...
   * (_largeObjectCollectorAllocateStatsForFreeList is null for Survivor) */
  if (NULL != largeObjectAllocateStatsForFreeList) {
    largeObjectAllocateStatsForFreeList[curFreeList]
        .incrementTlhAllocSizeClassStats(consumedSize);
  }
  ...
  if (!recycleHeapChunk(env, addrTop, topOfRecycledChunk, previousFreeEntry,
                        entryNext, curFreeList)) {
    /* Adjust the free memory size and count */
    Assert_MM_true(_heapFreeLists[curFreeList]._freeSize >= recycleEntrySize);
    Assert_MM_true(_heapFreeLists[curFreeList]._freeCount > 0);
    _heapFreeLists[curFreeList]._freeSize -= recycleEntrySize;
    ...
    _heapFreeLists[curFreeList].updateHint(freeEntry,
                                           (MM_HeapLinkedFreeHeader*)addrTop);
    _largeObjectAllocateStatsForFreeList[curFreeList]
        .incrementFreeEntrySizeClassStats(recycleEntrySize);
  }
...
void MM_MemoryPoolSplitAddressOrderedList::reset(Cause cause) {
  /* Call superclass first .. */
  MM_MemoryPoolSplitAddressOrderedListBase::reset(cause);
  resetReservedFreeEntry();
}

void MM_MemoryPoolSplitAddressOrderedList::postProcess(MM_EnvironmentBase* env,
                                                       Cause cause) {
  bool const compressed = compressObjectReferences();
  ...
  uintptr_t freeListSplitSize =
      _heapFreeLists[0]._freeSize / _heapFreeListCount;
  ...
  if (cause == forSweep) {
    ...
    if (0 < _sweepPoolState->_largestFreeEntry) {
      ...
      if (NULL == _sweepPoolState->_previousLargestFreeEntry) {
        ...
        largestFreeEntry =
            _sweepPoolState->_previousLargestFreeEntry->getNext(compressed);
      }
      Assert_MM_true(_sweepPoolState->_largestFreeEntry ==
                     largestFreeEntry->getSize());
    }
    MM_GCExtensionsBase* extensions = env->getExtensions();
    MM_ParallelSweepChunk* chunk = NULL;
    MM_SweepHeapSectioningIterator sectioningIterator(
        extensions->sweepHeapSectioning);
    ...
    for (chunk = sectioningIterator.nextChunk(); NULL != chunk;
         chunk = sectioningIterator.nextChunk()) {
      if (processedChunkCount >=
          extensions->splitFreeListNumberChunksPrepared) {
      ...
      uintptr_t currentFreeListSize =
          chunk->_accumulatedFreeSize - accumulatedFreeSize;
      if (currentFreeListSize >= freeListSplitSize) {
        ...
        _heapFreeLists[currentFreeListIndex]._freeCount =
            chunk->_accumulatedFreeHoles - accumulatedFreeHoles;
        _heapFreeLists[currentFreeListIndex]._freeSize = currentFreeListSize;

        /* Terminate the tail of the current free list. */
        chunk->_splitCandidatePreviousEntry->setNext(NULL, compressed);
        /**
         * Identify previous reserved entry from the previous LargestFreeEntry
         * which is set during sweep if previousLargestFreeEntry ==
         * splitCandidatePreviousEntry, it means largestFreeEntry ==
         * splitCandidateEntry and the largestFreeEntry is in the next free list
         * if previousLargestFreeEntry < splitCandidatePreviousEntry, it means
         * the largestFreeEntry is in current freeList
         */
        if ((_heapFreeListCount == _reservedFreeListIndex) &&
            (_sweepPoolState->_previousLargestFreeEntry <=
             chunk->_splitCandidatePreviousEntry)) {
          if (_sweepPoolState->_previousLargestFreeEntry ==
              chunk->_splitCandidatePreviousEntry) {
            _reservedFreeListIndex = currentFreeListIndex + 1;
            _previousReservedFreeEntry = NULL;
          } else {
            _reservedFreeListIndex = currentFreeListIndex;
            _previousReservedFreeEntry =
                _sweepPoolState->_previousLargestFreeEntry;
          }
    ...
    _heapFreeLists[currentFreeListIndex]._freeSize =
        freeSize - accumulatedFreeSize;
    _heapFreeLists[currentFreeListIndex]._freeCount =
        freeCount - accumulatedFreeHoles;
  } else {
    ...
    while (NULL != currentFreeList) {
      ...
      currentFreeList = currentFreeList->getNext(compressed);

      if ((_heapFreeLists[currentFreeListIndex]._freeSize >=
           freeListSplitSize) &&
          (currentFreeListIndex < lastFreeListIndex)) {
        previousFreeList->setNext(NULL, compressed);
  ...
  for (currentFreeListIndex = 0; currentFreeListIndex < _heapFreeListCount;
  ...
  if (0 < _reservedFreeEntrySize) {
    if (NULL == _previousReservedFreeEntry) {
      ...
      largestFreeEntry = _previousReservedFreeEntry->getNext(compressed);
    }
 ...
 *
 */
void MM_MemoryPoolSplitAddressOrderedList::expandWithRange(
    MM_EnvironmentBase* env, uintptr_t expandSize, void* lowAddress,
  ...
  for (curFreeListIndex = 0; curFreeListIndex < _heapFreeListCount;
       ++curFreeListIndex) {
    ...
    if (canCoalesce) {
      ...
      if (previousFreeEntry &&
        ...
        /* if 1) we are upsizing reservedFreeEntry (in which case the address of
         * the entry does not change) or 2) if its size is larger than size of
         * reservedFreeEntry, but only if the entry has not been identified yet
         * than update the size of reservedFreeEntry
         */
      ...
      if (nextFreeEntry && (highAddress == (void*)nextFreeEntry)) {
         ...
         *    2) expanded size is larger than size of reservedFreeEntry, but
         * only if the entry has not been identified yet update the size of
         * reservedFreeEntry previousReservedFreeEntry remains unchanged
         */
  ...
  assume0(isMemoryPoolValid(env, true));
  Assert_GC_true_with_message2(env, reservedFreeEntryConsistencyCheck(),
                               "expandWithRange _previousReservedFreeEntry=%p, "
                               "_reservedFreeEntrySize=%zu\n",
                               _previousReservedFreeEntry,
                               _reservedFreeEntrySize);
}
 ...
 *
 */
void* MM_MemoryPoolSplitAddressOrderedList::contractWithRange(
  ...
  Assert_MM_true(NULL !=
                 currentFreeEntry); /* Can't contract what doesn't exist */
  ...
  if (NULL != reservedFreeEntry) {
    if ((reservedFreeEntry >= lowAddress) &&
        (((void*)reservedFreeEntry->afterEnd()) <= highAddress)) {
  ...
  Assert_GC_true_with_message2(
      env, reservedFreeEntryConsistencyCheck(),
      "contractWithRange _previousReservedFreeEntry=%p, "
      "_reservedFreeEntrySize=%zu\n",
 ...
 *
 */
void MM_MemoryPoolSplitAddressOrderedList::addFreeEntries(
  ...
  while (freeEntryToAdd != NULL) {
    _largeObjectAllocateStats->incrementFreeEntrySizeClassStats(
        freeEntryToAdd->getSize());
    freeEntryToAdd = freeEntryToAdd->getNext(compressed);
  }
  ...
  currentFreeEntry = (MM_HeapLinkedFreeHeader*)getFirstFreeStartingAddr(
      env, &currentFreeListIndex);
  ...
  while (NULL != currentFreeEntry) {
    ...
    if ((NULL == previousFreeEntry->getNext(compressed)) &&
  ...
  if (previousFreeEntry == NULL) {
    ...
    Assert_MM_true(currentFreeEntry ==
                   _heapFreeLists[currentFreeListIndex]._freeList);
    ...
    if ((uint8_t*)freeListTail->afterEnd() == (uint8_t*)currentFreeEntry) {
      ...
      uintptr_t expandedSize =
          freeListTail->getSize() + currentFreeEntry->getSize();
      if (isPreviousReservedFreeEntry(previousFreeEntry,
                                      currentFreeListIndex)) {
        Assert_MM_true(currentFreeEntry->getSize() == _reservedFreeEntrySize);
        _reservedFreeEntrySize = expandedSize;

        if (freeListTail != freeListHead) {
          /* need to update _previousReservedFreeEntry to previous of
           * freeListTail */
          MM_HeapLinkedFreeHeader* previous = freeListHead->getNext(compressed);
          while (previous->getNext(compressed) != freeListTail) {
            previous = previous->getNext(compressed);
          }
      ...
      freeListTail->setNext(currentFreeEntry->getNext(compressed), compressed);
      ...
      Assert_MM_true((NULL == currentFreeEntry) ||
                     (freeListTail < currentFreeEntry));
      freeListTail->setNext(currentFreeEntry, compressed);
    }
    ...
    if ((uint8_t*)previousFreeEntry->afterEnd() == (uint8_t*)freeListHead) {
      ...
      uintptr_t expandedSize =
          freeListHead->getSize() + previousFreeEntry->getSize();
      if (isCurrentReservedFreeEntry(previousFreeEntry,
                                     previousFreeListIndex)) {
        _reservedFreeEntrySize = expandedSize;
      }

      previousFreeEntry->expandSize(freeListHead->getSize());
      Assert_MM_true((NULL == freeListHead->getNext(compressed)) ||
      ...
      Assert_MM_true((NULL == freeListHead) ||
                     (previousFreeEntry < freeListHead));
      previousFreeEntry->setNext(freeListHead, compressed);
    }
    _heapFreeLists[previousFreeListIndex]._freeSize += freeListMemorySize;
    _heapFreeLists[previousFreeListIndex]._freeCount +=
        localFreeListMemoryCount;
  }

  Assert_GC_true_with_message2(env, reservedFreeEntryConsistencyCheck(),
                               "addFreeEntries _previousReservedFreeEntry=%p, "
                               "_reservedFreeEntrySize=%zu\n",
                               _previousReservedFreeEntry,
                               _reservedFreeEntrySize);
}
 ...
 */
bool MM_MemoryPoolSplitAddressOrderedList::removeFreeEntriesWithinRange(
  ...
  /* If we got to the end of the free list or to an entry greater than the high
   * address */
  ...
  nextFreeEntry = (MM_HeapLinkedFreeHeader*)getNextFreeStartingAddr(
      env, currentFreeEntry, &nextFreeListIndex);

  /* Assume for now we will remove this entire free entry from the pool */
  Assert_MM_true(_heapFreeLists[currentFreeListIndex]._freeSize >=
                 currentFreeEntry->getSize());
  ...
  /* Determine what to do with any leading bytes of the free entry that are not
   * being returned */
  if (currentFreeEntry < (MM_HeapLinkedFreeHeader*)lowAddress) {
    ...
    if (createFreeEntry(env, currentFreeEntry, lowAddress, previousFreeEntry,
      ...
      if (currentFreeListIndex == nextFreeListIndex) {
        ...
        if (NULL == previousFreeEntry) {
          ...
          previousFreeEntry->setNext(NULL, compressed);
        }
      }
    }
    baseAddr = lowAddress;
  ...
  if (currentFreeEntryTop > (MM_HeapLinkedFreeHeader*)highAddress) {
    ...
    if (createFreeEntry(env, highAddress, currentFreeEntryTop,
      ...
      if (NULL == previousFreeEntry) {
        _heapFreeLists[currentFreeListIndex]._freeList =
            (MM_HeapLinkedFreeHeader*)highAddress;
      } else {
  ...
  /* Now append any whole chunks to the list which fall within the specified
   * range */
  while (currentFreeEntry &&
    ...
    tailFreeEntry = (MM_HeapLinkedFreeHeader*)getNextFreeStartingAddr(
        env, currentFreeEntry, &currentFreeListIndex);
  ...
  /* Attach the remaining tail to the previous entry, and set the head of the
   * list if there was none */
  if (previousFreeEntry) {
    Assert_MM_true((NULL == tailFreeEntry) ||
                   (previousFreeEntry < tailFreeEntry));
  ...
  if ((previousFreeListIndex != currentFreeListIndex) &&
      (currentFreeListIndex < _heapFreeListCount)) {
    _heapFreeLists[previousFreeListIndex]._freeCount +=
        _heapFreeLists[currentFreeListIndex]._freeCount;
    _heapFreeLists[previousFreeListIndex]._freeSize +=
        _heapFreeLists[currentFreeListIndex]._freeSize;
  }

  /* Kill all newly empty free lists */
  for (uintptr_t i = previousFreeListIndex + 1;
       (i <= currentFreeListIndex) && (i < _heapFreeListCount); ++i) {
  ...
  Assert_GC_true_with_message2(
      env, reservedFreeEntryConsistencyCheck(),
      "removeFreeEntriesWithinRange _previousReservedFreeEntry=%p, "
      "_reservedFreeEntrySize=%zu\n",
      _previousReservedFreeEntry, _reservedFreeEntrySize);
  return true;
}
...
uintptr_t MM_MemoryPoolSplitAddressOrderedList::releaseFreeMemoryPages(
  ...
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; i++) {
    ...
    releasedMemory +=
        releaseFreeEntryMemoryPages(env, _heapFreeLists[i]._freeList);
