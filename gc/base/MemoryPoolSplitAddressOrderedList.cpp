 ...
 */
MM_MemoryPoolSplitAddressOrderedList* MM_MemoryPoolSplitAddressOrderedList::newInstance(
    ...
    MM_EnvironmentBase* env, uintptr_t minimumFreeEntrySize, uintptr_t maxSplit, const char* name)
{
    ...
    memoryPool = (MM_MemoryPoolSplitAddressOrderedList*)env->getForge()->allocate(
        sizeof(MM_MemoryPoolSplitAddressOrderedList), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (memoryPool) {
...
MMINLINE MM_HeapLinkedFreeHeader* MM_MemoryPoolSplitAddressOrderedList::internalAllocateFromList(
    MM_EnvironmentBase* env, uintptr_t sizeInBytesRequired, uintptr_t curFreeList,
    MM_HeapLinkedFreeHeader** previousFreeEntry, uintptr_t* largestFreeEntry)
{
...
void* MM_MemoryPoolSplitAddressOrderedList::internalAllocate(MM_EnvironmentBase* env, uintptr_t sizeInBytesRequired,
...
bool MM_MemoryPoolSplitAddressOrderedList::internalAllocateTLH(MM_EnvironmentBase* env,
...
void MM_MemoryPoolSplitAddressOrderedList::postProcess(MM_EnvironmentBase* env, Cause cause)
{
    bool const compressed = compressObjectReferences();
        ...
        for (chunk = sectioningIterator.nextChunk(); NULL != chunk; chunk = sectioningIterator.nextChunk()) {
            ...
            if (currentFreeListSize >= freeListSplitSize) {
                ...
                if ((_heapFreeListCount == _reservedFreeListIndex)
                    && (_sweepPoolState->_previousLargestFreeEntry <= chunk->_splitCandidatePreviousEntry)) {
    ...
    MM_EnvironmentBase* env, uintptr_t expandSize, void* lowAddress, void* highAddress, bool canCoalesce)
{
    ...
    for (curFreeListIndex = 0; curFreeListIndex < _heapFreeListCount; ++curFreeListIndex) {
        ...
        if (canCoalesce) {
            ...
            if (previousFreeEntry
                 ...
                 *    2) if its size is larger than size of reservedFreeEntry, but only if the entry has not been
                 * identified yet than update the size of reservedFreeEntry
                 */
            ...
            if (nextFreeEntry && (highAddress == (void*)nextFreeEntry)) {
                 ...
                 *    2) expanded size is larger than size of reservedFreeEntry, but only if the entry has not been
                 * identified yet update the size of reservedFreeEntry previousReservedFreeEntry remains unchanged
                 */
    ...
    Assert_GC_true_with_message2(env, reservedFreeEntryConsistencyCheck(),
        "expandWithRange _previousReservedFreeEntry=%p, _reservedFreeEntrySize=%zu\n", _previousReservedFreeEntry,
    ...
    MM_EnvironmentBase* env, uintptr_t contractSize, void* lowAddress, void* highAddress)
{
    ...
    Assert_GC_true_with_message2(env, reservedFreeEntryConsistencyCheck(),
        "contractWithRange _previousReservedFreeEntry=%p, _reservedFreeEntrySize=%zu\n", _previousReservedFreeEntry,
        _reservedFreeEntrySize);
    return lowAddress;
}
 ...
 *
 */
void MM_MemoryPoolSplitAddressOrderedList::addFreeEntries(MM_EnvironmentBase* env,
    MM_HeapLinkedFreeHeader*& freeListHead, MM_HeapLinkedFreeHeader*& freeListTail, uintptr_t freeListMemoryCount,
    uintptr_t freeListMemorySize)
{
    ...
    while (NULL != currentFreeEntry) {
        ...
        if ((NULL == previousFreeEntry->getNext(compressed)) && (currentFreeListIndex != _heapFreeListCount)) {
            previousFreeEntry = NULL;
        }
    }
    ...
    Assert_GC_true_with_message2(env, reservedFreeEntryConsistencyCheck(),
        "addFreeEntries _previousReservedFreeEntry=%p, _reservedFreeEntrySize=%zu\n", _previousReservedFreeEntry,
        _reservedFreeEntrySize);
}
 ...
 */
bool MM_MemoryPoolSplitAddressOrderedList::removeFreeEntriesWithinRange(MM_EnvironmentBase* env, void* lowAddress,
    ...
    MM_HeapLinkedFreeHeader*& retListTail, uintptr_t& retListMemoryCount, uintptr_t& retListMemorySize)
{
    ...
    Assert_GC_true_with_message2(env, reservedFreeEntryConsistencyCheck(),
        "removeFreeEntriesWithinRange _previousReservedFreeEntry=%p, _reservedFreeEntrySize=%zu\n",
        _previousReservedFreeEntry, _reservedFreeEntrySize);
    return true;
}

#if defined(OMR_GC_IDLE_HEAP_MANAGER)
uintptr_t MM_MemoryPoolSplitAddressOrderedList::releaseFreeMemoryPages(MM_EnvironmentBase* env)
{
