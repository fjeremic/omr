...
void*
MM_MemoryPoolSplitAddressOrderedList::internalAllocate(
  MM_EnvironmentBase* env,
  uintptr_t sizeInBytesRequired,
  ...
  do {
    if (NULL != _heapFreeLists[curFreeList]._freeList) {
      ...
      if (skipReserved) {
        ...
        currentFreeEntry = internalAllocateFromList(env,
                                                    sizeInBytesRequired,
                                                    curFreeList,
                                                    &previousFreeEntry,
                                                    &largestFreeEntry);
...
skipSearch:
  ...
  if (NULL == currentFreeEntry) {
    ...
    if (_memorySubSpace->replenishPoolForAllocate(
          env, this, sizeInBytesRequired)) {
      skipReserved = true;
      goto retry;
    }
  ...
  if (recycleHeapChunk(env,
                       recycleEntry,
                       ((uint8_t*)recycleEntry) + recycleEntrySize,
                       previousFreeEntry,
                       currentFreeEntry->getNext(compressed),
                       curFreeList)) {
    if (!skipReserved &&
...
bool
MM_MemoryPoolSplitAddressOrderedList::internalAllocateTLH(
  MM_EnvironmentBase* env,
...
skipSearch:
...
#if defined(OMR_GC_CONCURRENT_SWEEP)
    if (_memorySubSpace->replenishPoolForAllocate(
          env, this, _minimumFreeEntrySize)) {
      skipReserved = true;
      goto retry;
    }
#endif /* OMR_GC_CONCURRENT_SWEEP */
  ...
  /* Check if this free entry looks like a dead object. */
  Assert_MM_true(
    env->getExtensions()->objectModel.isDeadObject((omrobjectptr_t)freeEntry));
...
void
MM_MemoryPoolSplitAddressOrderedList::postProcess(MM_EnvironmentBase* env,
                                                  Cause cause)
{
  bool const compressed = compressObjectReferences();
 ...
 */
void
MM_MemoryPoolSplitAddressOrderedList::expandWithRange(MM_EnvironmentBase* env,
 ...
 */
void*
MM_MemoryPoolSplitAddressOrderedList::contractWithRange(MM_EnvironmentBase* env,
                                                        ...
                                                        void* highAddress)
{
  ...
  if (currentFreeEntryTop != (MM_HeapLinkedFreeHeader*)highAddress) {
     ...
     * entry? */
    if (createFreeEntry(
          env, highAddress, currentFreeEntryTop, NULL, nextFreeEntry)) {
  ...
  if (currentFreeEntry != (MM_HeapLinkedFreeHeader*)lowAddress) {
    if (createFreeEntry(
          env, currentFreeEntry, lowAddress, NULL, nextFreeEntry)) {
 ...
 */
void
MM_MemoryPoolSplitAddressOrderedList::addFreeEntries(
  MM_EnvironmentBase* env,
 ...
 */
bool
MM_MemoryPoolSplitAddressOrderedList::removeFreeEntriesWithinRange(
  MM_EnvironmentBase* env,
  void* lowAddress,
  void* highAddress,
  ...
  uintptr_t& retListMemorySize)
{
  ...
  if (currentFreeEntry < (MM_HeapLinkedFreeHeader*)lowAddress) {
     ...
     */
    if (createFreeEntry(
          env, currentFreeEntry, lowAddress, previousFreeEntry, NULL)) {
      uintptr_t leadingSize =
        ((uintptr_t)lowAddress) - ((uintptr_t)currentFreeEntry);
      if (reservedFreeEntry == currentFreeEntry) {
  ...
  if (currentFreeEntryTop > (MM_HeapLinkedFreeHeader*)highAddress) {
     ...
     * for this pool ? */
    if (createFreeEntry(
          env, highAddress, currentFreeEntryTop, previousFreeEntry, NULL)) {
      uintptr_t trailingSize =
        ((uintptr_t)currentFreeEntryTop) - ((uintptr_t)highAddress);
      if (NULL == previousFreeEntry) {
        _heapFreeLists[currentFreeListIndex]._freeList =
  ...
  if (appendToList(
        env, baseAddr, topAddr, minimumSize, retListHead, retListTail)) {
    ++retListMemoryCount;
  ...
  while (currentFreeEntry &&
    ...
    if (appendToList(env,
                     (void*)currentFreeEntry,
                     (void*)currentFreeEntry->afterEnd(),
                     minimumSize,
                     retListHead,
                     retListTail)) {
      ++retListMemoryCount;
  ...
  for (uintptr_t i = previousFreeListIndex + 1;
       (i <= currentFreeListIndex) && (i < _heapFreeListCount);
       ++i) {
...
uintptr_t
MM_MemoryPoolSplitAddressOrderedList::releaseFreeMemoryPages(
  MM_EnvironmentBase* env)
{
