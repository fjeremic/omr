        ...
        if (extensions->_holdRandomThreadBeforeHandlingWorkUnit) {
            if (0 == (rand() % extensions->_holdRandomThreadBeforeHandlingWorkUnitPeriod)) {
                Trc_MM_ParallelTask_handleNextWorkUnit_holdingThread(
                    env->getLanguageVMThread(), env->getWorkUnitIndex(), env->_lastSyncPointReached);
                omrthread_sleep(10);
            }
        }
    }
...
void MM_ParallelTask::synchronizeGCThreads(MM_EnvironmentBase* env, const char* id)
{
...
bool MM_ParallelTask::synchronizeGCThreadsAndReleaseMaster(MM_EnvironmentBase* env, const char* id)
{
    ...
    if (1 < _totalThreadCount) {
        ...
        if (0 == _synchronizeCount) {
            ...
            Assert_GC_true_with_message4(env, _syncPointWorkUnitIndex == env->getWorkUnitIndex(),
                "%s at %p from synchronizeGCThreadsAndReleaseMaster: call with syncPointWorkUnitIndex %zu, expected "
                "%zu\n",
                getBaseVirtualTypeId(), this, env->getWorkUnitIndex(), _syncPointWorkUnitIndex);
        }
...
bool MM_ParallelTask::synchronizeGCThreadsAndReleaseSingleThread(MM_EnvironmentBase* env, const char* id)
{
    ...
    if (1 < _totalThreadCount) {
        ...
        if (0 == _synchronizeCount) {
            ...
            Assert_GC_true_with_message4(env, _syncPointUniqueId == id,
                ...
                getBaseVirtualTypeId(), this, id, _syncPointUniqueId);
            Assert_GC_true_with_message4(env, _syncPointWorkUnitIndex == env->getWorkUnitIndex(),
                "%s at %p from synchronizeGCThreadsAndReleaseSingleThread: call with syncPointWorkUnitIndex %zu, "
                "expected %zu\n",
                getBaseVirtualTypeId(), this, env->getWorkUnitIndex(), _syncPointWorkUnitIndex);
        }
...
void MM_ParallelTask::releaseSynchronizedGCThreads(MM_EnvironmentBase* env)
{
    ...
    Assert_GC_true_with_message2(env, _synchronized,
...
bool MM_ParallelTask::synchronizeGCThreadsAndReleaseMaster(MM_EnvironmentBase* env, const char* id, uint64_t* stallTime)
{
...
void MM_ParallelTask::synchronizeGCThreads(MM_EnvironmentBase* env, const char* id, uint64_t* stallTime)
{
    uint64_t startTime, endTime;
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
