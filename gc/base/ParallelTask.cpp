  ...
  if (1 < _totalThreadCount) {
    ...
    if (0 == _synchronizeCount) {
      ...
      Assert_GC_true_with_message4(
        env,
        _syncPointWorkUnitIndex == env->getWorkUnitIndex(),
        "%s at %p from synchronizeGCThreads: call with syncPointWorkUnitIndex "
        "%zu, expected %zu\n",
        getBaseVirtualTypeId(),
        this,
        env->getWorkUnitIndex(),
...
bool
MM_ParallelTask::synchronizeGCThreadsAndReleaseMaster(MM_EnvironmentBase* env,
                                                      const char* id)
{
...
bool
MM_ParallelTask::synchronizeGCThreadsAndReleaseSingleThread(
...
void
MM_ParallelTask::synchronizeGCThreads(MM_EnvironmentBase* env,
                                      const char* id,
                                      uint64_t* stallTime)
{
  uint64_t startTime, endTime;
