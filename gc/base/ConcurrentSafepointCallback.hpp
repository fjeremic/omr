...
typedef void (*SafepointCallbackHandler)(struct OMR_VMThread *currentThread,
                                         void *userData);
...
public:
  ...
  virtual void registerCallback(MM_EnvironmentBase *env,
                                SafepointCallbackHandler handler,
                                void *userData, bool cancelAfterGC = false);
#else
  virtual void registerCallback(MM_EnvironmentBase *env,
                                SafepointCallbackHandler handler,
                                void *userData);
  ...
  /* Providing this no-op class as a concrete implementation reduces the glue
   * burden on client languages. */
  ...
  MM_ConcurrentSafepointCallback(MM_EnvironmentBase *env)
      : MM_BaseVirtual(), _handler(NULL), _userData(NULL) {
