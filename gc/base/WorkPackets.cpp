...
MM_WorkPackets::newInstance(MM_EnvironmentBase* env)
{
  ...
  workPackets = (MM_WorkPackets*)env->getForge()->allocate(
    sizeof(MM_WorkPackets),
    OMR::GC::AllocationCategory::WORK_PACKETS,
    OMR_GET_CALLSITE());
  if (workPackets) {
...
MM_WorkPackets::initialize(MM_EnvironmentBase* env)
{
  ...
  if (omrthread_monitor_init_with_name(
        &_inputListMonitor, 0, "MM_WorkPackets::inputList")) {
    return false;
  }

  if (omrthread_monitor_init_with_name(
        &_allocatingPackets, 0, "MM_WorkPackets::allocatingPackets")) {
    return false;
  }
...
MM_WorkPackets::initWorkPacketsBlock(MM_EnvironmentBase* env)
{
  ...
  memset((void*)_packetsStart[_packetsBlocksTop],
         0,
  ...
  MM_Packet* tailPtr =
    (MM_Packet*)(((uintptr_t)currentPtr) + totalHeaderSize - sizeof(MM_Packet));
  ...
  for (uintptr_t i = 0; i < _packetsPerBlock; i++) {
    ...
    currentPtr->initialize(
      env, nextPtr, previousPtr, baseAddress, _slotsInPacket);
 ...
 */
void
MM_WorkPackets::emptyToOverflow(MM_EnvironmentBase* env,
                                MM_Packet* packet,
...
MM_WorkPackets::getInputPacket(MM_EnvironmentBase* env)
{
  ...
  bool mustSyncThreadsAndExit =
    (NULL != env->_currentTask) && env->_currentTask->shouldYieldFromTask(env);
 ...
 */
void
MM_WorkPackets::overflowItem(MM_EnvironmentBase* env,
                             void* item,
                             MM_OverflowType type)
{
