 ...
 */
MM_WorkPackets *MM_WorkPackets::newInstance(MM_EnvironmentBase *env) {
  MM_WorkPackets *workPackets;

  workPackets = (MM_WorkPackets *)env->getForge()->allocate(
      sizeof(MM_WorkPackets), OMR::GC::AllocationCategory::WORK_PACKETS,
      OMR_GET_CALLSITE());
  if (workPackets) {
 ...
 */
void MM_WorkPackets::kill(MM_EnvironmentBase *env) {
  tearDown(env);
 ...
 */
bool MM_WorkPackets::initialize(MM_EnvironmentBase *env) {
  uintptr_t initialPacketCount, heapSize;

  /* Initialize these here instead of the constructor initializer list to help
   * us break dependancy cycle, and allow inlining of functions in MM_WorkStack.
   * (Dependancy Cycle between WorkPackets.hpp, WorkStack.hpp, and
   * Environment.hpp)
   */
  ...
  if (omrthread_monitor_init_with_name(&_inputListMonitor, 0,
                                       "MM_WorkPackets::inputList")) {
    return false;
  }

  if (omrthread_monitor_init_with_name(&_allocatingPackets, 0,
                                       "MM_WorkPackets::allocatingPackets")) {
    return false;
  }
  ...
  if (0 != _extensions->workpacketCount) {
    ...
    /* determine the number of initial active packets heuristically, based on
     * heapsize */
    initialPacketCount =
        (uintptr_t)((float)heapSize * getHeapCapacityFactor(env) / _packetSize);
  }

  /* round the initialPacketCount so there is the same number of packets in each
   * packetblock, and we have a sane minimum */
  initialPacketCount =
      MM_Math::roundToFloor(_initialBlocks, initialPacketCount);
  initialPacketCount =
      OMR_MAX(initialPacketCount, _initialBlocks * _minPacketsInBlock);

  /* Want to ensure we have enough packets so that every thread can make forward
   * progress at once. This means that we need to have at least 2 packets per
   * thread (one input and one output.) See CMVC 194431 for more details.
   */
  uintptr_t minimumPacketsForForwardProgress =
      MM_Math::roundToCeiling(_initialBlocks, _extensions->gcThreadCount * 2);
  initialPacketCount =
      OMR_MAX(initialPacketCount, minimumPacketsForForwardProgress);

  _packetsPerBlock = initialPacketCount / _initialBlocks;

  /* If -Xgcworkpackets was specified  we don't allow later allocation of more
   * packets */
  _maxPackets = (0 != _extensions->workpacketCount)
                    ? initialPacketCount
                    : initialPacketCount * _increaseFactor;
 ...
 * @return true on sucess, false on allocation failure or if _maxpackets is
 * already reached
 */
bool MM_WorkPackets::initWorkPacketsBlock(MM_EnvironmentBase *env) {
  ...
  if (NULL == (_packetsStart[_packetsBlocksTop] =
                   (MM_Packet *)env->getForge()->allocate(
                       blockSize, OMR::GC::AllocationCategory::WORK_PACKETS,
                       OMR_GET_CALLSITE()))) {
    return false;
  }
  memset((void *)_packetsStart[_packetsBlocksTop], 0,
         totalHeaderSize); /* initialize only MM_Packet's to avoid paging the
                              rest of the block */

  MM_Packet *headPtr = _packetsStart[_packetsBlocksTop];
  MM_Packet *currentPtr = headPtr;
  MM_Packet *tailPtr = (MM_Packet *)(((uintptr_t)currentPtr) + totalHeaderSize -
                                     sizeof(MM_Packet));
  ...
  for (uintptr_t i = 0; i < _packetsPerBlock; i++) {
    ...
    currentPtr->initialize(env, nextPtr, previousPtr, baseAddress,
                           _slotsInPacket);
 ...
 */
void MM_WorkPackets::tearDown(MM_EnvironmentBase *env) {
  if (NULL != _overflowHandler) {
...
void MM_WorkPackets::reset(MM_EnvironmentBase *env) {
  _overflowHandler->reset(env);
}

/**
 * Iterate over all non empty lists and reset all packets and return them to the
 * empty list.
 */
void MM_WorkPackets::resetAllPackets(MM_EnvironmentBase *env) {
  MM_Packet *packet;
 ...
 */
void MM_WorkPackets::reuseDeferredPackets(MM_EnvironmentBase *env) {
  MM_Packet *packet;
 ...
 */
bool MM_WorkPackets::inputPacketAvailable(MM_EnvironmentBase *env) {
  bool res =
      ((!_fullPacketList.isEmpty()) || (!_relativelyFullPacketList.isEmpty()) ||
 ...
 */
void MM_WorkPackets::emptyToOverflow(MM_EnvironmentBase *env, MM_Packet *packet,
                                     MM_OverflowType type) {
 ...
 */
MM_Packet *MM_WorkPackets::getInputPacketFromOverflow(MM_EnvironmentBase *env) {
  ...
  while (!_overflowHandler->isEmpty()) {
    if (NULL != (overflowPacket = getPacket(env, &_emptyPacketList))) {
      ...
      if (overflowPacket->isEmpty()) {
        /* If we didn't end up filling the packet with anything, don't return it
         * and try again */
 ...
 */
MM_Packet *MM_WorkPackets::getInputPacketNoWait(MM_EnvironmentBase *env) {
  MM_Packet *packet;
  ...
  if ((!_nonEmptyPacketList.isEmpty()) &&
      (_emptyPacketList.getCount() < (_activePackets >> 2))) {
 ...
 */
MM_Packet *MM_WorkPackets::getInputPacket(MM_EnvironmentBase *env) {
  MM_Packet *packet = NULL;
  bool doneFlag = false;
  volatile uintptr_t doneIndex = _inputListDoneIndex;
  bool mustSyncThreadsAndExit = (NULL != env->_currentTask) &&
  ...
  while (!doneFlag) {
    ...
    if (doneIndex == _inputListDoneIndex) {
      ...
      if (((NULL == env->_currentTask) ||
        ...
        while (mustSyncThreadsAndExit || (!inputPacketAvailable(env) &&
                                          (_inputListDoneIndex == doneIndex))) {
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
          ...
          /* This is where all the GC threads end up synchronizing when waiting
           * for work */
          ...
          if (_inputListDoneIndex != doneIndex) {
            ...
            env->_workPacketStats.addToWorkStallTime(waitStartTime,
                                                     waitEndTime);
          }
#endif /* J9MODRON_TGC_PARALLEL_STATISTICS */
 ...
 */
MM_Packet *MM_WorkPackets::getOutputPacket(MM_EnvironmentBase *env) {
  ...
  /* emptyPacketList was empty so attempt to get a packet from one of the list
   * of partial full packets */
  ...
  /* emptyPacketList was empty after attempt to grow.  Try the partially full
   * lists */
  outputPacket = getLeastFullPacket(env, 2);
  if (NULL != outputPacket) {
    return outputPacket;
 ...
 */
MM_Packet *MM_WorkPackets::getPacketByAdddingWorkPacketBlock(
  ...
  /* Check to see if there are any packets on the empty packetlist since another
   * thread may have allocated another WorkPacket block while this thread was
   * attempting to take control of the allocatingPackets field
   */
  packet = getPacket(env, &_emptyPacketList);
  if (NULL == packet) {
    /* Since the current thread now has control of the allocatingPackets field
     * and there are no packets on the emptyPacketList attempt to initialize
     * another workPackets block
     */
    if (initWorkPacketsBlock(env)) {
      /* Successfully initialized another workpacket block so get a packet off
       * of the emptyPacketList */
      packet = getPacket(env, &_emptyPacketList);
    }
  }
 ...
 */
MM_Packet *MM_WorkPackets::getPacketByOverflowing(MM_EnvironmentBase *env) {
  MM_Packet *packet = NULL;
...
/**
 * Get the least-full packet that has capacity of at least the given number of
 * slots
 *
 * @param requiredSlots The required number of free slots
 * @return pointer to a packet, or NULL if none found
 */
MM_Packet *MM_WorkPackets::getLeastFullPacket(MM_EnvironmentBase *env,
                                              int requiredSlots) {
  ...
  for (j = 0, i = 0; i < _maxPacketSearch; i++) {
    ...
    if (curCapacity > maxCapacity) {
      ...
      if ((maxCapacity >=
           _fullPacketThreshold) /* we have a non-empty packet already */
          && (curCapacity <
              _fullPacketThreshold)) { /* we got to the full packet list */
        break;
      }
    }
  }
 ...
 */
MM_Packet *MM_WorkPackets::getPacket(MM_EnvironmentBase *env,
                                     MM_PacketList *list) {
  MM_Packet *packet;
 ...
 */
void MM_WorkPackets::putPacket(MM_EnvironmentBase *env, MM_Packet *packet) {
  MM_PacketList *list;
...
void MM_WorkPackets::notifyWaitingThreads(MM_EnvironmentBase *env) {
  /* Added an entry to a null list - notify any other threads that a new entry
 ...
 */
void MM_WorkPackets::putOutputPacket(MM_EnvironmentBase *env,
                                     MM_Packet *packet) {
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
 ...
 */
MM_Packet *MM_WorkPackets::getDeferredPacket(MM_EnvironmentBase *env) {
  MM_Packet *packet;
 ...
 */
void MM_WorkPackets::putDeferredPacket(MM_EnvironmentBase *env,
                                       MM_Packet *packet) {
  MM_PacketList *list;
  uintptr_t freeSlots = packet->freeSlots();
...
/**
 * Return the heap capactify factor used to determine how many packets to
 * create. This is only called during the initialization of the WorkPackets
 * class.
 *
 * @return the heap capactify factor
 */
float MM_WorkPackets::getHeapCapacityFactor(MM_EnvironmentBase *env) {
  return (float)0.002;
}
 ...
 */
void MM_WorkPackets::overflowItem(MM_EnvironmentBase *env, void *item,
                                  MM_OverflowType type) {
  _overflowHandler->overflowItem(env, item, type);
}

bool MM_WorkPackets::getOverflowFlag() { return _overflowHandler->_overflow; }

void MM_WorkPackets::clearOverflowFlag() {
  _overflowHandler->_overflow = false;
}

bool MM_WorkPackets::handleWorkPacketOverflow(MM_EnvironmentBase *env) {
  bool result = false;
...
MM_WorkPacketOverflow *MM_WorkPackets::createOverflowHandler(
