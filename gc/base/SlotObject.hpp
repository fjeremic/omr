...
{
private:
  volatile fomrobject_t*
    _slot; /**< stored slot address (volatile, because in concurrent GC the
              mutator can change the value in _slot) */
#if defined(OMR_GC_COMPRESSED_POINTERS)
  uintptr_t _compressedPointersShift; /**< the number of bits to shift by when
                                         converting between the compressed
...
public:
  ...
  /**
   * Atomically replace heap reference. It is accepted to fail - some other
   * thread might have raced us and put a more up to date value.
   * @return true if write succeeded
   */
  MMINLINE bool atomicWriteReferenceToSlot(omrobjectptr_t oldReference,
                                           omrobjectptr_t newReference)
  {
    ...
    if (compressObjectReferences()) {
      ...
      swapResult =
        ((uintptr_t)compressedOld ==
         MM_AtomicOperations::lockCompareExchange((uintptr_t*)_slot,
                                                  (uintptr_t)compressedOld,
                                                  (uintptr_t)compressedNew));
    }
