...
protected:
  MM_ParallelSweepChunkArray
      *_head;           /**< head pointer to a list of Chunk arrays */
  ...
  MM_ParallelSweepChunkArray
      *_baseArray; /**< pointer to the base array allocated at initialization */
...
public:
  ...
  MM_SweepHeapSectioning(MM_EnvironmentBase *env)
      : _head(NULL), _totalUsed(0), _totalSize(0), _baseArray(NULL),
...
private:
  MM_ParallelSweepChunkArray
      *_currentArray;     /**< Current chunk array being traversed */
  uint32_t _currentIndex; /**< Current index of chunk within the current array
                             being traversed */

public:
  MM_SweepHeapSectioningIterator(
      const MM_SweepHeapSectioning *sweepHeapSectioning)
