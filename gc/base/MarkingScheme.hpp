...
public:
     ...
     */
    MMINLINE bool inlineMarkObjectNoCheck(MM_EnvironmentBase* env, omrobjectptr_t objectPtr, bool leafType = false)
    {
     ...
     */
    MMINLINE bool inlineMarkObject(MM_EnvironmentBase* env, omrobjectptr_t objectPtr, bool leafType = false)
    {
     ...
     */
    MMINLINE uintptr_t scanObject(MM_EnvironmentBase* env, omrobjectptr_t objectPtr, MM_MarkingSchemeScanReason reason,
        uintptr_t sizeToDo = UDATA_MAX)
    {
        GC_ObjectScannerState objectScannerState;
        GC_ObjectScanner* objectScanner
            = _delegate.getObjectScanner(env, objectPtr, &objectScannerState, reason, &sizeToDo);
     ...
     * the slot is unchanged, but the class slot of the self-forwarded object is restored (self-forwarded bits are
     * reset). This is currently used only in presence of Concurrent Scavenger. When abort is detected CS does not fix
     * up heap references within Scavenger abort handling, but relies on marking in percolate Global to do it. The
     * updates are non-atomic, but even though multiple threads could be doing it, they set it to the same value. During
     * Concurrent Marking, ignore forwarded objects (especially true for self-forwarded objects which are very important
     * during Scavenger aborted cycle to prevent duplicate copies). The fixup will be done after Scavenger Cycle is
     * done, in the final phase of Concurrent GC when we scan Nursery.
     */
