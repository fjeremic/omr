 ...
 */
void reportGlobalGCIncrementStart(J9HookInterface** hook, uintptr_t eventNum,
                                  void* eventData, void* userData) {
  MM_GlobalGCIncrementStartEvent* event =
      (MM_GlobalGCIncrementStartEvent*)eventData;
  MM_EnvironmentBase* env =
      MM_EnvironmentBase::getEnvironment(event->currentThread);

  ((MM_MemoryPoolLargeObjects*)userData)
      ->preCollect(env, env->_cycleState->_gcCode.isExplicitGC(),
 ...
 * Initialization
 */
MM_MemoryPoolLargeObjects* MM_MemoryPoolLargeObjects::newInstance(
    MM_EnvironmentBase* env,
  ...
  memoryPool = (MM_MemoryPoolLargeObjects*)env->getForge()->allocate(
      sizeof(MM_MemoryPoolLargeObjects), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (NULL != memoryPool) {
    memoryPool = new (memoryPool)
        MM_MemoryPoolLargeObjects(env, largeObjectArea, smallObjectArea);
...
bool MM_MemoryPoolLargeObjects::initialize(MM_EnvironmentBase* env) {
  if (!MM_MemoryPool::initialize(env)) {
    return false;
  }

  /* it's important that the LOA is registered first - so that it's last in the
   * memorypool chain */
  ...
  _extensions->heapExpansionMinimumSize =
      OMR_MAX(_extensions->heapExpansionMinimumSize,
              _extensions->largeObjectMinimumSize);
   ...
   *
   * THIS MUST HAPPEN AFTER VERBOSE GC PRINTS LOA SIZE. IT MUST NOT HAPPEN ON
   * SCAVENGES.
   */
  (*mmPrivateHooks)
      ->J9HookRegisterWithCallSite(
          mmPrivateHooks, J9HOOK_MM_PRIVATE_GLOBAL_GC_INCREMENT_START,
          reportGlobalGCIncrementStart, OMR_GET_CALLSITE(), (void*)this);

  uintptr_t minimumFreeEntrySize =
      OMR_MAX(_memoryPoolLargeObjects->getMinimumFreeEntrySize(),
  ...
  _largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(
      ...
      tlhMaximumSize + minimumFreeEntrySize, _extensions->tlhMinimumSize);

  if (NULL == _largeObjectAllocateStats) {
    return false;
  }

  Trc_MM_LOAResize_initialize(env->getLanguageVMThread(),
                              _memoryPoolSmallObjects, _memoryPoolLargeObjects);

  _loaFreeRatioHistory = (double*)env->getForge()->allocate(
      _extensions->loaFreeHistorySize * sizeof(double),
  ...
  /*
   * initialize _loaFreeRatioArray to 0 (even though initialy LOA shold have a
   * free ratio of 1), to prevent early contraction
   */
...
void MM_MemoryPoolLargeObjects::tearDown(MM_EnvironmentBase* env) {
  J9HookInterface** mmPrivateHooks =
      J9_HOOK_INTERFACE(_extensions->privateHookInterface);

  /* Unregister the global GC hooks for this instance */
  (*mmPrivateHooks)
      ->J9HookUnregister(mmPrivateHooks,
...
void MM_MemoryPoolLargeObjects::reset(Cause cause) {
 ...
 */
void MM_MemoryPoolLargeObjects::preCollect(MM_EnvironmentBase* env,
                                           bool systemGC, bool aggressive,
 ...
 */
void MM_MemoryPoolLargeObjects::resizeLOA(MM_EnvironmentBase* env) {
  _soaFreeBytesAfterLastGC =
      _memoryPoolSmallObjects->getApproximateFreeMemorySize();

  float minimumFreeRatio =
      ((float)_extensions->heapFreeMinimumRatioMultiplier) /
      ((float)_extensions->heapFreeMinimumRatioDivisor);

  uintptr_t minimumSOAFreeRequired = uintptr_t(_soaSize * minimumFreeRatio);

  if ((_soaFreeBytesAfterLastGC < minimumSOAFreeRequired) &&
      (LOA_EMPTY != _currentLOABase)) {
    ...
    uintptr_t loaMinimumSize = MM_Math::roundToCeiling(
        _extensions->heapAlignment,
    ...
    loaMinimumSize =
        loaMinimumSize < _memoryPoolLargeObjects->getMinimumFreeEntrySize()
            ? 0
            : loaMinimumSize;

    /* We are short on free memory, but try to be fair to both SOA and LOA.
     * largeObjectAreaInitialRatio is considered optimal and will be used as
     * long as free SOA meets min free ratio requirement. If, for example, SOA
     * free is only 40% of the minimum, than we will contract LOA to 40% of the
     * optimal size.
     */

    uintptr_t newLOAsize =
        _soaFreeBytesAfterLastGC *
        (uintptr_t)(_extensions->largeObjectAreaInitialRatio /
                    minimumFreeRatio);

    Trc_MM_LOAResize_resizeLOA1(env->getLanguageVMThread(), newLOAsize);

    if (newLOAsize < _loaSize) {
      /* The above formula makes sense if LOA is completely free. But if LOA is
       * already partially occupied than we want to contract LOA even less
       * (since LOA clearly shows being useful).
       *
       * The size we contract by is proportionate to free LOA ratio. If, for
       * example, LOA is completely full (no free memory), we will not contract
       * LOA at all.
       *
       * We do not just take the current LOA occupancy, but we look into the
       * history of several global GCs and take the highest LOA occupancy.
       *
       */

      contractRequired =
          (uintptr_t)((_loaSize - newLOAsize) * _minLOAFreeRatio);
      ...
      if (newLOAsize < loaMinimumSize) {
        Assert_GC_true_with_message2(env, (_loaSize >= loaMinimumSize),
                                     "current LOA size(%zu) should not be "
                                     "smaller than minimum LOA size(%zu).\n",
      ...
      /* If minimum required now zero then there is no storage available for
       * transfer */
      if (0 < contractRequired) {
        ...
        newLOABase = (void*)MM_Math::roundToFloor(_extensions->heapAlignment,
                                                  (uintptr_t)newLOABase);

        _memoryPoolLargeObjects->removeFreeEntriesWithinRange(
            env, _currentLOABase, newLOABase,
            _memoryPoolSmallObjects->getMinimumFreeEntrySize(), moveListHead,
            moveListTail, moveListMemoryCount, moveListMemorySize);

        if (NULL != moveListHead) {
          _memoryPoolSmallObjects->addFreeEntries(
              env, moveListHead, moveListTail, moveListMemoryCount,
              moveListMemorySize);
        }

        spaceDelta = (NULL != newLOABase)
                         ? (uintptr_t)newLOABase - (uintptr_t)_currentLOABase
                         : _loaSize;
        oldLOARatio = _currentLOARatio;

        /* Does this leave a reasonable sized LOA ? */
        if (!isSizeEnoughForLOA(
                env, (_loaSize > spaceDelta) ? (_loaSize - spaceDelta) : 0)) {
          /* No.. make LOA empty as not even big enough for one free chunk of
           * LOA */
          spaceDelta = _loaSize;
          _soaSize += spaceDelta;
          _loaSize = 0;
          ...
          /* Rounding during float operations may result in a new LOA ratio less
           * than minimum so fix up if necessary
           */
        ...
        Trc_MM_LOAResize_resizeLOA4(env->getLanguageVMThread(), oldLOARatio,
                                    _currentLOARatio);

        _extensions->heap->getResizeStats()->setLastLoaResizeReason(
            LOA_CONTRACT_MIN_SOA);
        _memorySubSpace->reportHeapResizeAttempt(env, spaceDelta,
                                                 HEAP_LOA_CONTRACT);
        ...
        Assert_GC_true_with_message2(env, (_loaSize >= loaMinimumSize),
                                     "resize LOA size(%zu) should not be "
                                     "smaller than minimum LOA size(%zu).\n",
                                     _loaSize, loaMinimumSize);
      }
    }
 ...
 *
 * We need to ensue that a completely rebuild the freelist  if we need to
 * redistribute free memory between LOA and SOA in order to meet SOA minimum
 * free space requirements.
 *
 * @return TRUE if a complete sweep is required; FALSE otherwise
 */
bool MM_MemoryPoolLargeObjects::completeFreelistRebuildRequired(
    MM_EnvironmentBase* env) {
  uintptr_t soaFree = _memoryPoolSmallObjects->getApproximateFreeMemorySize();
  uintptr_t minimumRequired =
      (_soaSize / _extensions->heapFreeMinimumRatioDivisor) *
      _extensions->heapFreeMinimumRatioMultiplier;

  return ((soaFree < minimumRequired) && (LOA_EMPTY != _currentLOABase));
}

double MM_MemoryPoolLargeObjects::calculateTargetLOARatio(
    MM_EnvironmentBase* env, uintptr_t allocSize) {
  ...
  _minLOAFreeRatio =
      *std::min_element(_loaFreeRatioHistory,
                        _loaFreeRatioHistory + _extensions->loaFreeHistorySize);
  ...
  if (allocSize >= _extensions->largeObjectMinimumSize) {
    /* If the allocation size is 1/5 times greater than current LOA size..expand
     * LOA */
    ...
    /* Belt and braces check. Because _currentLOARatio is a float we need to
     * check that we have not exceeded maximum and if we have round down to
     * maximum
     */
  ...
  if (newLOARatio != _currentLOARatio) {
    Trc_MM_LOAResize_calculateTargetLOARatio(
 ...
 */
double MM_MemoryPoolLargeObjects::resetTargetLOARatio(MM_EnvironmentBase* env) {
  /* Nothing needs to be done if the LOA size is already at minimum size */
  if (_currentLOARatio == _extensions->largeObjectAreaMinimumRatio) {
    return _currentLOARatio;
  }

  Trc_MM_LOAResize_resetTargetLOARatio(
      env->getLanguageVMThread(), _currentLOARatio,
      _extensions->largeObjectAreaMinimumRatio);
...
void MM_MemoryPoolLargeObjects::resetLOASize(MM_EnvironmentBase* env,
                                             double newLOARatio) {
  ...
  if (_currentLOARatio != newLOARatio) {
    ...
    /* Calculate LOA size based on new loa ratio */
    newLOASize = MM_Math::roundToCeiling(
        _extensions->heapAlignment, (uintptr_t)(oldAreaSize * newLOARatio));
    ...
    /* else, could be newLOASize == oldLOASize  (originally expand, but not big
     * enough) */

    /* and new LOA base if LOA ratio > 0 */
    _currentLOABase =
        _currentLOARatio > 0 ? determineLOABase(env, _soaSize) : LOA_EMPTY;
...
void MM_MemoryPoolLargeObjects::resetHeapStatistics(bool memoryPoolCollected) {
...
void MM_MemoryPoolLargeObjects::mergeHeapStats(MM_HeapStats* heapStats,
                                               bool active) {
...
void* MM_MemoryPoolLargeObjects::allocateObject(
    ...
    if (sizeInBytesRequired >= _extensions->largeObjectMinimumSize) {
      ...
      if (_loaSize > 0) {
        ...
        if (addr != NULL) {
          ...
          if (debug) {
            omrtty_printf(
                "LOA allocate: object allocated at %p of size %zu bytes. SOA "
                "LWM is %zu bytes\n",
...
void* MM_MemoryPoolLargeObjects::allocateTLH(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    uintptr_t maximumSizeInBytesRequired, void*& addrBase, void*& addrTop) {
  return _memoryPoolSmallObjects->allocateTLH(
      env, allocDescription, maximumSizeInBytesRequired, addrBase, addrTop);
}
 ...
 */
void* MM_MemoryPoolLargeObjects::findFreeEntryEndingAtAddr(
    MM_EnvironmentBase* env, void* addr) {
...
/**
 * @copydoc
 * MM_MemoryPool::getAvailableContractionSizeForRangeEndingAt(MM_EnvironmentBase
 * *, MM_AllocateDescription *, void *, void *)
 */
uintptr_t
MM_MemoryPoolLargeObjects::getAvailableContractionSizeForRangeEndingAt(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    void* lowAddr, void* highAddr) {
  if (highAddr >= _currentLOABase) {
    return _memoryPoolLargeObjects->getAvailableContractionSizeForRangeEndingAt(
        env, allocDescription, lowAddr, highAddr);
  } else {
    return _memoryPoolSmallObjects->getAvailableContractionSizeForRangeEndingAt(
        env, allocDescription, lowAddr, highAddr);
  }
}
 ...
 */
MM_MemoryPool* MM_MemoryPoolLargeObjects::getMemoryPool(void* addr) {
  if (addr >= _currentLOABase) {
 ...
 * @return MM_MemoryPool
 */
MM_MemoryPool* MM_MemoryPoolLargeObjects::getMemoryPool(uintptr_t size) {
 ...
 */
MM_MemoryPool* MM_MemoryPoolLargeObjects::getMemoryPool(MM_EnvironmentBase* env,
                                                        void* addrBase,
                                                        void* addrTop,
                                                        void*& highAddr) {
 ...
 * receiver. This call will return an accurate count of the current size of all
 * free memory.  It will not consider defered work that may be done to increase
 * current free memory stores.
 * @see getApproximateFreeMemorySize()
 * @return the total free memory currently available for allocation.
 */
uintptr_t MM_MemoryPoolLargeObjects::getActualFreeMemorySize() {
...
uintptr_t MM_MemoryPoolLargeObjects::getActualFreeEntryCount() {
 ...
 * receiver. This call will return an estimated count of the current size of all
 * free memory.  Although this estimate may be accurate, it will consider
 * potential defered work that may be done to increase current free memory
 * stores.
 * @see getActualFreeMemorySize()
 * @return the approximate total free memory available for allocation.
 */
uintptr_t MM_MemoryPoolLargeObjects::getApproximateFreeMemorySize() {
 ...
 *
 */
void MM_MemoryPoolLargeObjects::resetLargestFreeEntry() {
 ...
 */
uintptr_t MM_MemoryPoolLargeObjects::getLargestFreeEntry() {
 ...
 */
void* MM_MemoryPoolLargeObjects::findFreeEntryTopStartingAtAddr(
    MM_EnvironmentBase* env, void* addr) {
  if (addr >= _currentLOABase) {
 ...
 */
void* MM_MemoryPoolLargeObjects::getFirstFreeStartingAddr(
    MM_EnvironmentBase* env) {
 ...
 */
void* MM_MemoryPoolLargeObjects::getNextFreeStartingAddr(
    MM_EnvironmentBase* env, void* currentFree) {
  void* nextFree;
  ...
  if (currentFree < _currentLOABase) {
    nextFree =
        _memoryPoolSmallObjects->getNextFreeStartingAddr(env, currentFree);
    ...
    nextFree =
        _memoryPoolLargeObjects->getNextFreeStartingAddr(env, currentFree);
  }
 ...
 * regions. This involves fixing up any free list information that may change as
 * a result of an address change.
 *
 * This method should not be called; only required if we implement a LOA for
 * semi-spaces
 *
 ...
 *
 */
void MM_MemoryPoolLargeObjects::moveHeap(MM_EnvironmentBase* env, void* srcBase,
 ...
 */
void MM_MemoryPoolLargeObjects::lock(MM_EnvironmentBase* env) {
 ...
 */
void MM_MemoryPoolLargeObjects::unlock(MM_EnvironmentBase* env) {
 ...
 */
void* MM_MemoryPoolLargeObjects::collectorAllocate(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
   ...
   */
  if (sizeInBytesRequired < _soaObjectSizeLWM) {
    addr = _memoryPoolSmallObjects->collectorAllocate(env, allocDescription,
                                                      lockingRequired);
  }
  ...
  if (NULL == addr) {
    ...
    /* We relax normal rule and allow small objects to be allocated in LOA if
     * caller requests */
    if (allocDescription->isCollectorAllocateSatisfyAnywhere() ||
      ...
      if (_loaSize > 0) {
        ...
        if (NULL != addr) {
          ...
          if (debug) {
            omrtty_printf(
                "LOA allocate(collector): normal object allocated at %p of "
                "size %zu bytes. SOA LWM is %zu bytes\n",
                addr, sizeInBytesRequired, _soaObjectSizeLWM);
          }
        }
      }
 ...
 * @todo Provide function documentation
 */
void* MM_MemoryPoolLargeObjects::collectorAllocateTLH(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    uintptr_t maximumSizeInBytesRequired, void*& addrBase, void*& addrTop,
    bool lockingRequired) {
  void* base = _memoryPoolSmallObjects->collectorAllocateTLH(
      env, allocDescription, maximumSizeInBytesRequired, addrBase, addrTop,
      lockingRequired);
   ...
   */
  if ((NULL == base) &&
      allocDescription->isCollectorAllocateSatisfyAnywhere()) {
    base = _memoryPoolLargeObjects->collectorAllocateTLH(
        env, allocDescription, maximumSizeInBytesRequired, addrBase, addrTop,
        lockingRequired);
  }
 ...
 *
 * The heap has expanded so we need to recalculate the LOA boundary and update
 * the SOA and LOA freelists appropriately.
 *
...
void MM_MemoryPoolLargeObjects::expandWithRange(MM_EnvironmentBase* env,
  ...
  Assert_MM_true(MM_Math::roundToCeiling(_extensions->heapAlignment,
  ...
  if (0 == _currentOldAreaSize) {
    ...
    _loaSize = MM_Math::roundToCeiling(_extensions->heapAlignment, newLOASize);

    /* SOA is what is left after LOA allocation */
    _soaSize = oldAreaSize - _loaSize;

    _currentLOABase =
        _loaSize > 0 ? determineLOABase(env, _soaSize) : LOA_EMPTY;

    _memoryPoolSmallObjects->expandWithRange(env, _soaSize, lowAddress,
                                             _currentLOABase, canCoalesce);

    if (_loaSize > 0) {
      _memoryPoolLargeObjects->expandWithRange(env, _loaSize, _currentLOABase,
                                               highAddress, canCoalesce);
    }

    Trc_MM_LOAResize_expandWithRange1(env->getLanguageVMThread(), oldAreaSize,
                                      _currentLOARatio, _currentLOABase,
                                      _loaSize);
  } else {
    ...
    if (0 == _currentLOARatio) {
      _memoryPoolSmallObjects->expandWithRange(env, expandSize, lowAddress,
                                               highAddress, canCoalesce);
      ...
      _memoryPoolLargeObjects->expandWithRange(env, expandSize, lowAddress,
                                               highAddress, canCoalesce);

      /* ..and then redistribute free memory between SOA and LOA */
      redistributeFreeMemory(env, oldAreaSize);

      Trc_MM_LOAResize_expandWithRange2(env->getLanguageVMThread(), oldAreaSize,
                                        _currentLOARatio, _currentLOABase,
                                        _loaSize);
    }
 ...
 *
 * The heap has contracted so we need to recalculate the LOA boundary and update
 * the SOA and LOA freelists appropriately.
 *
 * @param expandSize Number of bytes to remove from the memory pool
 * @param lowAddress Low address of memory to remove (inclusive)
...
void* MM_MemoryPoolLargeObjects::contractWithRange(MM_EnvironmentBase* env,
  ...
  if (0 == _currentLOARatio) {
    ...
    _memoryPoolSmallObjects->contractWithRange(env, contractSize, lowAddress,
                                               highAddress);
  } else {
    assume0(lowAddress >= _currentLOABase);

    /* First remove memory from LOA */
    _memoryPoolLargeObjects->contractWithRange(env, contractSize, lowAddress,
                                               highAddress);

    redistributeFreeMemory(env, oldAreaSize);

    Trc_MM_LOAResize_contractWithRange(env->getLanguageVMThread(), oldAreaSize,
                                       _currentLOARatio, _currentLOABase,
                                       _loaSize);
  }
...
/**
 * Redistribute free memory bewteen the LOA and SOA after a heap
 * expansion/contraction.
 *
 * @param newOldAreaSize Number of bytes in old area AFTER expansion/contraction
 *
 */
void MM_MemoryPoolLargeObjects::redistributeFreeMemory(
    MM_EnvironmentBase* env, uintptr_t newOldAreaSize) {
  ...
  _loaSize = MM_Math::roundToCeiling(
      _extensions->heapAlignment,
  ...
  Assert_MM_true(MM_Math::roundToFloor(_extensions->heapAlignment, _soaSize) ==
                 _soaSize);
  ...
  if (oldLOABase < _currentLOABase) {
    ...
    _memoryPoolLargeObjects->removeFreeEntriesWithinRange(
        env, oldLOABase, _currentLOABase,
        _memoryPoolSmallObjects->getMinimumFreeEntrySize(), freeListHead,
        freeListTail, count, size);

    if (NULL != freeListHead) {
      _memoryPoolSmallObjects->addFreeEntries(env, freeListHead, freeListTail,
                                              count, size);
    }
    ...
    _memoryPoolSmallObjects->removeFreeEntriesWithinRange(
        env, _currentLOABase, oldLOABase,
        _memoryPoolLargeObjects->getMinimumFreeEntrySize(), freeListHead,
        freeListTail, count, size);

    if (NULL != freeListHead) {
      _memoryPoolLargeObjects->addFreeEntries(env, freeListHead, freeListTail,
                                              count, size);
    }
  }
...
bool MM_MemoryPoolLargeObjects::abandonHeapChunk(void* addrBase,
                                                 void* addrTop) {
...
/*
 * Determine base address for LOA given size of SOA. Iterate over segments
 * (which could be non-contiguous) until we have accounted for size of SOA
 *
 *
 */
uintptr_t* MM_MemoryPoolLargeObjects::determineLOABase(MM_EnvironmentBase* env,
                                                       uintptr_t soaSize) {
  ...
  GC_MemorySubSpaceRegionIterator regionIterator(
  ...
  while ((region = regionIterator.nextRegion()) != NULL) {
    ...
    if (sizeLeft >= regionSize) {
      ...
      uintptr_t* loaBase =
          (uintptr_t*)((uint8_t*)region->getLowAddress() + sizeLeft);
      loaBase = (uintptr_t*)MM_Math::roundToCeiling(_extensions->heapAlignment,
                                                    (uintptr_t)loaBase);
...
void MM_MemoryPoolLargeObjects::mergeFreeEntryAllocateStats() {
  ...
  _largeObjectAllocateStats->getFreeEntrySizeClassStats()->merge(
      _memoryPoolSmallObjects->getLargeObjectAllocateStats()
          ->getFreeEntrySizeClassStats());
  _largeObjectAllocateStats->getFreeEntrySizeClassStats()->merge(
      _memoryPoolLargeObjects->getLargeObjectAllocateStats()
          ->getFreeEntrySizeClassStats());
}

void MM_MemoryPoolLargeObjects::mergeTlhAllocateStats() {
  ...
  _largeObjectAllocateStats->getTlhAllocSizeClassStats()->merge(
      _memoryPoolSmallObjects->getLargeObjectAllocateStats()
          ->getTlhAllocSizeClassStats());
  _largeObjectAllocateStats->getTlhAllocSizeClassStats()->merge(
      _memoryPoolLargeObjects->getLargeObjectAllocateStats()
          ->getTlhAllocSizeClassStats());
}

void MM_MemoryPoolLargeObjects::mergeLargeObjectAllocateStats() {
...
void MM_MemoryPoolLargeObjects::averageLargeObjectAllocateStats(
    MM_EnvironmentBase* env, uintptr_t bytesAllocatedThisRound) {
  _largeObjectAllocateStats->resetAverage();
  _memoryPoolSmallObjects->averageLargeObjectAllocateStats(
      env, bytesAllocatedThisRound);
  _memoryPoolLargeObjects->averageLargeObjectAllocateStats(
      env, bytesAllocatedThisRound);
  _largeObjectAllocateStats->mergeAverage(
      _memoryPoolSmallObjects->getLargeObjectAllocateStats());
  _largeObjectAllocateStats->mergeAverage(
      _memoryPoolLargeObjects->getLargeObjectAllocateStats());
}

void MM_MemoryPoolLargeObjects::resetLargeObjectAllocateStats() {
  _largeObjectAllocateStats->resetCurrent();
  _largeObjectAllocateStats->getTlhAllocSizeClassStats()->resetCounts();
...
uintptr_t MM_MemoryPoolLargeObjects::releaseFreeMemoryPages(
    MM_EnvironmentBase* env) {
  uintptr_t releasedMemory =
      _memoryPoolSmallObjects->releaseFreeMemoryPages(env);
