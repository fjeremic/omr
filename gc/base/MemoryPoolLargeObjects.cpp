 ...
 */
void
reportGlobalGCIncrementStart(J9HookInterface** hook,
                             uintptr_t eventNum,
                             void* eventData,
                             void* userData)
{
...
MM_MemoryPoolLargeObjects::newInstance(
  MM_EnvironmentBase* env,
  ...
  MM_MemoryPoolAddressOrderedListBase* smallObjectArea)
{
  ...
  memoryPool = (MM_MemoryPoolLargeObjects*)env->getForge()->allocate(
    sizeof(MM_MemoryPoolLargeObjects),
...
MM_MemoryPoolLargeObjects::initialize(MM_EnvironmentBase* env)
{
   ...
   */
  _extensions->heapExpansionMinimumSize = OMR_MAX(
    _extensions->heapExpansionMinimumSize, _extensions->largeObjectMinimumSize);
  ...
  _largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(
    ...
    tlhMaximumSize + minimumFreeEntrySize,
  ...
  Trc_MM_LOAResize_initialize(env->getLanguageVMThread(),
                              _memoryPoolSmallObjects,
                              _memoryPoolLargeObjects);
 ...
 */
void
MM_MemoryPoolLargeObjects::preCollect(MM_EnvironmentBase* env,
                                      bool systemGC,
                                      bool aggressive,
    ...
    if (newLOAsize < _loaSize) {
      ...
      if (0 < contractRequired) {
        ...
        if (NULL != moveListHead) {
          _memoryPoolSmallObjects->addFreeEntries(env,
                                                  moveListHead,
                                                  moveListTail,
                                                  moveListMemoryCount,
                                                  moveListMemorySize);
        ...
        Trc_MM_LOAResize_resizeLOA4(
          env->getLanguageVMThread(), oldLOARatio, _currentLOARatio);

        _extensions->heap->getResizeStats()->setLastLoaResizeReason(
          LOA_CONTRACT_MIN_SOA);
        _memorySubSpace->reportHeapResizeAttempt(
          env, spaceDelta, HEAP_LOA_CONTRACT);
 ...
 */
bool
MM_MemoryPoolLargeObjects::completeFreelistRebuildRequired(
  MM_EnvironmentBase* env)
{
...
void
MM_MemoryPoolLargeObjects::resetLOASize(MM_EnvironmentBase* env,
                                        double newLOARatio)
{
...
void*
MM_MemoryPoolLargeObjects::allocateObject(
  MM_EnvironmentBase* env,
 ...
 */
MM_MemoryPool*
MM_MemoryPoolLargeObjects::getMemoryPool(MM_EnvironmentBase* env,
                                         void* addrBase,
                                         void* addrTop,
                                         void*& highAddr)
{
 ...
 */
void*
MM_MemoryPoolLargeObjects::findFreeEntryTopStartingAtAddr(
  MM_EnvironmentBase* env,
  void* addr)
{
  if (addr >= _currentLOABase) {
 ...
 */
void
MM_MemoryPoolLargeObjects::moveHeap(MM_EnvironmentBase* env,
                                    void* srcBase,
                                    void* srcTop,
                                    void* dstBase)
{
 ...
 */
void*
MM_MemoryPoolLargeObjects::collectorAllocate(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocDescription,
  bool lockingRequired)
{
   ...
   */
  if (sizeInBytesRequired < _soaObjectSizeLWM) {
    addr = _memoryPoolSmallObjects->collectorAllocate(
      env, allocDescription, lockingRequired);
  }
  ...
  if (NULL == addr) {
    ...
    if (allocDescription->isCollectorAllocateSatisfyAnywhere() ||
      ...
      /* Retry allocation in LOA ..if we have one */
      if (_loaSize > 0) {
        addr = _memoryPoolLargeObjects->collectorAllocate(
          env, allocDescription, lockingRequired);
 ...
 */
void*
MM_MemoryPoolLargeObjects::collectorAllocateTLH(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocDescription,
  uintptr_t maximumSizeInBytesRequired,
  void*& addrBase,
...
{
  void* base =
    _memoryPoolSmallObjects->collectorAllocateTLH(env,
                                                  allocDescription,
  ...
  if ((NULL == base) &&
    ...
    base =
      _memoryPoolLargeObjects->collectorAllocateTLH(env,
                                                    allocDescription,
                                                    maximumSizeInBytesRequired,
                                                    addrBase,
                                                    addrTop,
                                           ...
                                           bool canCoalesce)
{
  ...
  if (0 == _currentOldAreaSize) {
    ...
    _memoryPoolSmallObjects->expandWithRange(
      env, _soaSize, lowAddress, _currentLOABase, canCoalesce);

    if (_loaSize > 0) {
      _memoryPoolLargeObjects->expandWithRange(
        env, _loaSize, _currentLOABase, highAddress, canCoalesce);
    }

    Trc_MM_LOAResize_expandWithRange1(env->getLanguageVMThread(),
                                      oldAreaSize,
                                      _currentLOARatio,
                                      _currentLOABase,
                                      _loaSize);
    ...
    if (0 == _currentLOARatio) {
      _memoryPoolSmallObjects->expandWithRange(
        env, expandSize, lowAddress, highAddress, canCoalesce);
      ...
      Trc_MM_LOAResize_expandWithRange2(env->getLanguageVMThread(),
                                        oldAreaSize,
                                        _currentLOARatio,
                                        _currentLOABase,
                                        _loaSize);
                                             ...
                                             void* highAddress)
{
  ...
  if (0 == _currentLOARatio) {
    ...
    _memoryPoolSmallObjects->contractWithRange(
      env, contractSize, lowAddress, highAddress);
  } else {

    assume0(lowAddress >= _currentLOABase);

    /* First remove memory from LOA */
    _memoryPoolLargeObjects->contractWithRange(
      env, contractSize, lowAddress, highAddress);

    redistributeFreeMemory(env, oldAreaSize);

    Trc_MM_LOAResize_contractWithRange(env->getLanguageVMThread(),
                                       oldAreaSize,
                                       _currentLOARatio,
                                       _currentLOABase,
                                       _loaSize);
 ...
 */
void
MM_MemoryPoolLargeObjects::redistributeFreeMemory(MM_EnvironmentBase* env,
  ...
  if (oldLOABase < _currentLOABase) {
    ...
    if (NULL != freeListHead) {
      _memoryPoolSmallObjects->addFreeEntries(
        env, freeListHead, freeListTail, count, size);
    }
    ...
    if (NULL != freeListHead) {
      _memoryPoolLargeObjects->addFreeEntries(
        env, freeListHead, freeListTail, count, size);
    }
  }
 ...
 */
uintptr_t*
MM_MemoryPoolLargeObjects::determineLOABase(MM_EnvironmentBase* env,
                                            uintptr_t soaSize)
{
...
void
MM_MemoryPoolLargeObjects::averageLargeObjectAllocateStats(
  MM_EnvironmentBase* env,
