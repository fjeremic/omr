...
double
MM_MemoryPoolLargeObjects::calculateTargetLOARatio(MM_EnvironmentBase *env,
                                                   uintptr_t allocSize) {
    ...
    if (sizeInBytesRequired >= _extensions->largeObjectMinimumSize) {
      ...
      if (_loaSize > 0) {
        ...
        if (addr != NULL) {
          ...
          if (debug) {
            omrtty_printf("LOA allocate: object allocated at %p of size %zu "
                          "bytes. SOA LWM is %zu bytes\n",
 ...
 */
void *
MM_MemoryPoolLargeObjects::findFreeEntryEndingAtAddr(MM_EnvironmentBase *env,
                                                     void *addr) {
 ...
 */
void *
MM_MemoryPoolLargeObjects::getNextFreeStartingAddr(MM_EnvironmentBase *env,
                                                   void *currentFree) {
  void *nextFree;
    ...
    if (allocDescription->isCollectorAllocateSatisfyAnywhere() ||
      ...
      if (_loaSize > 0) {
        ...
        if (NULL != addr) {
          ...
          if (debug) {
            omrtty_printf("LOA allocate(collector): normal object allocated at "
                          "%p of size %zu bytes. SOA LWM is %zu bytes\n",
                          addr, sizeInBytesRequired, _soaObjectSizeLWM);
          }
        }
      }
