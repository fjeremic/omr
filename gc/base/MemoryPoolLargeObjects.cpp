 ...
 */
void reportGlobalGCIncrementStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
 ...
 * Initialization
 */
MM_MemoryPoolLargeObjects* MM_MemoryPoolLargeObjects::newInstance(MM_EnvironmentBase* env,
    MM_MemoryPoolAddressOrderedListBase* largeObjectArea, MM_MemoryPoolAddressOrderedListBase* smallObjectArea)
{
...
bool MM_MemoryPoolLargeObjects::initialize(MM_EnvironmentBase* env)
{
    ...
    uintptr_t minimumFreeEntrySize = OMR_MAX(
        _memoryPoolLargeObjects->getMinimumFreeEntrySize(), _memoryPoolSmallObjects->getMinimumFreeEntrySize());
    ...
    _loaFreeRatioHistory = (double*)env->getForge()->allocate(
        _extensions->loaFreeHistorySize * sizeof(double), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
void MM_MemoryPoolLargeObjects::tearDown(MM_EnvironmentBase* env)
{
    ...
    (*mmPrivateHooks)
        ->J9HookUnregister(
            mmPrivateHooks, J9HOOK_MM_PRIVATE_GLOBAL_GC_INCREMENT_START, reportGlobalGCIncrementStart, (void*)this);
 ...
 */
void MM_MemoryPoolLargeObjects::preCollect(
    MM_EnvironmentBase* env, bool systemGC, bool aggressive, uintptr_t bytesRequested)
{
...
void MM_MemoryPoolLargeObjects::resizeLOA(MM_EnvironmentBase* env)
{
    ...
    float minimumFreeRatio
        = ((float)_extensions->heapFreeMinimumRatioMultiplier) / ((float)_extensions->heapFreeMinimumRatioDivisor);
    ...
    if ((_soaFreeBytesAfterLastGC < minimumSOAFreeRequired) && (LOA_EMPTY != _currentLOABase)) {
         ...
         * largeObjectAreaInitialRatio is considered optimal and will be used as long as free SOA meets min free ratio
         * requirement. If, for example, SOA free is only 40% of the minimum, than we will contract LOA to 40% of the
         * optimal size.
         */

        uintptr_t newLOAsize
            = _soaFreeBytesAfterLastGC * (uintptr_t)(_extensions->largeObjectAreaInitialRatio / minimumFreeRatio);

        Trc_MM_LOAResize_resizeLOA1(env->getLanguageVMThread(), newLOAsize);

        if (newLOAsize < _loaSize) {

            /* The above formula makes sense if LOA is completely free. But if LOA is already partially occupied than we
             * want to contract LOA even less (since LOA clearly shows being useful).
             *
             * The size we contract by is proportionate to free LOA ratio. If, for example, LOA is completely full (no
             * free memory), we will not contract LOA at all.
             *
             * We do not just take the current LOA occupancy, but we look into the history of several global GCs and
             * take the highest LOA occupancy.
             *
             */
            ...
            if (0 < contractRequired) {
                ...
                _memoryPoolLargeObjects->removeFreeEntriesWithinRange(env, _currentLOABase, newLOABase,
                    _memoryPoolSmallObjects->getMinimumFreeEntrySize(), moveListHead, moveListTail, moveListMemoryCount,
                    moveListMemorySize);

                if (NULL != moveListHead) {
                    _memoryPoolSmallObjects->addFreeEntries(
                        env, moveListHead, moveListTail, moveListMemoryCount, moveListMemorySize);
                }
 ...
 */
bool MM_MemoryPoolLargeObjects::completeFreelistRebuildRequired(MM_EnvironmentBase* env)
{
    uintptr_t soaFree = _memoryPoolSmallObjects->getApproximateFreeMemorySize();
    uintptr_t minimumRequired
        = (_soaSize / _extensions->heapFreeMinimumRatioDivisor) * _extensions->heapFreeMinimumRatioMultiplier;

    return ((soaFree < minimumRequired) && (LOA_EMPTY != _currentLOABase));
}

double MM_MemoryPoolLargeObjects::calculateTargetLOARatio(MM_EnvironmentBase* env, uintptr_t allocSize)
{
    double newLOARatio = _currentLOARatio;
    float maxLOAFreeRatio
        = ((float)_extensions->heapFreeMaximumRatioMultiplier) / ((float)_extensions->heapFreeMinimumRatioDivisor);
    ...
    if (newLOARatio != _currentLOARatio) {
        Trc_MM_LOAResize_calculateTargetLOARatio(env->getLanguageVMThread(),
            newLOARatio < _currentLOARatio ? "decreased" : "increased", _currentLOARatio, newLOARatio);
    }
...
double MM_MemoryPoolLargeObjects::resetTargetLOARatio(MM_EnvironmentBase* env)
{
    ...
    Trc_MM_LOAResize_resetTargetLOARatio(
        env->getLanguageVMThread(), _currentLOARatio, _extensions->largeObjectAreaMinimumRatio);
...
void MM_MemoryPoolLargeObjects::resetLOASize(MM_EnvironmentBase* env, double newLOARatio)
{
...
void MM_MemoryPoolLargeObjects::mergeHeapStats(MM_HeapStats* heapStats, bool active)
{
...
void* MM_MemoryPoolLargeObjects::allocateObject(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
...
void* MM_MemoryPoolLargeObjects::allocateTLH(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
 ...
 */
void* MM_MemoryPoolLargeObjects::findFreeEntryEndingAtAddr(MM_EnvironmentBase* env, void* addr)
{
 ...
 * void *, void *)
 */
uintptr_t MM_MemoryPoolLargeObjects::getAvailableContractionSizeForRangeEndingAt(
 ...
 */
MM_MemoryPool* MM_MemoryPoolLargeObjects::getMemoryPool(
    MM_EnvironmentBase* env, void* addrBase, void* addrTop, void*& highAddr)
{
 ...
 */
void* MM_MemoryPoolLargeObjects::findFreeEntryTopStartingAtAddr(MM_EnvironmentBase* env, void* addr)
{
    if (addr >= _currentLOABase) {
 ...
 */
void* MM_MemoryPoolLargeObjects::getFirstFreeStartingAddr(MM_EnvironmentBase* env)
{
 ...
 */
void* MM_MemoryPoolLargeObjects::getNextFreeStartingAddr(MM_EnvironmentBase* env, void* currentFree)
{
    void* nextFree;
 ...
 *
 */
void MM_MemoryPoolLargeObjects::moveHeap(MM_EnvironmentBase* env, void* srcBase, void* srcTop, void* dstBase)
{
        ...
        if (allocDescription->isCollectorAllocateSatisfyAnywhere()
            ...
            if (_loaSize > 0) {
                ...
                if (NULL != addr) {
                    ...
                    if (debug) {
                        omrtty_printf("LOA allocate(collector): normal object allocated at %p of size %zu bytes. SOA "
                                      "LWM is %zu bytes\n",
                            addr, sizeInBytesRequired, _soaObjectSizeLWM);
                    }
                }
 ...
 * @todo Provide function documentation
 */
void* MM_MemoryPoolLargeObjects::collectorAllocateTLH(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    uintptr_t maximumSizeInBytesRequired, void*& addrBase, void*& addrTop, bool lockingRequired)
{
    void* base = _memoryPoolSmallObjects->collectorAllocateTLH(
        env, allocDescription, maximumSizeInBytesRequired, addrBase, addrTop, lockingRequired);
...
void MM_MemoryPoolLargeObjects::expandWithRange(
    MM_EnvironmentBase* env, uintptr_t expandSize, void* lowAddress, void* highAddress, bool canCoalesce)
{
    ...
    if (0 == _currentOldAreaSize) {
        ...
        Trc_MM_LOAResize_expandWithRange1(
            env->getLanguageVMThread(), oldAreaSize, _currentLOARatio, _currentLOABase, _loaSize);
    } else {
        ...
        if (0 == _currentLOARatio) {
            ...
            Trc_MM_LOAResize_expandWithRange2(
                env->getLanguageVMThread(), oldAreaSize, _currentLOARatio, _currentLOABase, _loaSize);
        }
...
void* MM_MemoryPoolLargeObjects::contractWithRange(
    MM_EnvironmentBase* env, uintptr_t contractSize, void* lowAddress, void* highAddress)
{
    ...
    if (0 == _currentLOARatio) {
        ...
        Trc_MM_LOAResize_contractWithRange(
            env->getLanguageVMThread(), oldAreaSize, _currentLOARatio, _currentLOABase, _loaSize);
    }
 ...
 *
 */
void MM_MemoryPoolLargeObjects::redistributeFreeMemory(MM_EnvironmentBase* env, uintptr_t newOldAreaSize)
{
    ...
    _loaSize
        = MM_Math::roundToCeiling(_extensions->heapAlignment, (uintptr_t)((float)newOldAreaSize * _currentLOARatio));

    assume0(_loaSize > 0);

    /* ..and SOA is what's left after LOA allocation */
    _soaSize = newOldAreaSize - _loaSize;
    Assert_MM_true(MM_Math::roundToFloor(_extensions->heapAlignment, _soaSize) == _soaSize);
    ...
    if (oldLOABase < _currentLOABase) {
        ...
        _memoryPoolLargeObjects->removeFreeEntriesWithinRange(env, oldLOABase, _currentLOABase,
            _memoryPoolSmallObjects->getMinimumFreeEntrySize(), freeListHead, freeListTail, count, size);
        ...
        _memoryPoolSmallObjects->removeFreeEntriesWithinRange(env, _currentLOABase, oldLOABase,
            _memoryPoolLargeObjects->getMinimumFreeEntrySize(), freeListHead, freeListTail, count, size);
...
bool MM_MemoryPoolLargeObjects::abandonHeapChunk(void* addrBase, void* addrTop)
{
 ...
 *
 */
uintptr_t* MM_MemoryPoolLargeObjects::determineLOABase(MM_EnvironmentBase* env, uintptr_t soaSize)
{
...
uintptr_t MM_MemoryPoolLargeObjects::releaseFreeMemoryPages(MM_EnvironmentBase* env)
{
