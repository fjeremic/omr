...
bool MM_MemoryPoolSplitAddressOrderedListBase::initialize(MM_EnvironmentBase* env)
{
    ...
    _heapFreeLists = (J9ModronFreeList*)extensions->getForge()->allocate(
        sizeof(J9ModronFreeList) * _heapFreeListCountExtended, OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    ...
    /* set multiple factor = 2 for doubling _maxVeryLargeEntrySizes to avoid run out of _veryLargeEntryPool (minus count
     * during decrement) */
    _largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(env,
        (uint16_t)extensions->largeObjectAllocationProfilingTopK, extensions->largeObjectAllocationProfilingThreshold,
    ...
    if (NULL == _largeObjectAllocateStatsForFreeList) {
        ...
        for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
            ...
            /* set multiple factor = 2 for doubling _maxVeryLargeEntrySizes to avoid run out of _veryLargeEntryPool
             * (minus count during decrement) */
...
void MM_MemoryPoolSplitAddressOrderedListBase::tearDown(MM_EnvironmentBase* env)
{
...
{
    void* addr
        = internalAllocate(env, allocDescription->getContiguousBytes(), true, _largeObjectAllocateStatsForFreeList);
    ...
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, bool lockingRequired)
{
    void* addr = internalAllocate(
        env, allocDescription->getContiguousBytes(), lockingRequired, _largeObjectCollectorAllocateStatsForFreeList);
...
void* MM_MemoryPoolSplitAddressOrderedListBase::allocateTLH(MM_EnvironmentBase* env,
...
void* MM_MemoryPoolSplitAddressOrderedListBase::collectorAllocateTLH(MM_EnvironmentBase* env,
    MM_AllocateDescription* allocDescription, uintptr_t maximumSizeInBytesRequired, void*& addrBase, void*& addrTop,
 ...
 */
MM_HeapLinkedFreeHeader* MM_MemoryPoolSplitAddressOrderedListBase::rebuildFreeListInRegion(
    MM_EnvironmentBase* env, MM_HeapRegionDescriptor* region, MM_HeapLinkedFreeHeader* previousFreeEntry)
{
    ...
    /* TODO 108399 Determine whether this is still necessary (OMR_SCAVENGER_DEBUG is defined in Scavenger.cpp and is not
     * accessible here) */
...
bool MM_MemoryPoolSplitAddressOrderedListBase::printFreeListValidity(MM_EnvironmentBase* env)
{
    ...
    for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
        ...
        omrtty_printf("  -- Free List %4zu (head: 0x%p, tail: 0x%p, expected size: %16zu, expected holes: %16zu): ", i,
            _heapFreeLists[i]._freeList, previousFreeEntry, _heapFreeLists[i]._freeSize, _heapFreeLists[i]._freeCount);
...
bool MM_MemoryPoolSplitAddressOrderedListBase::recycleHeapChunk(MM_EnvironmentBase* env, void* addrBase, void* addrTop,
 ...
 * void *, void *)
 */
uintptr_t MM_MemoryPoolSplitAddressOrderedListBase::getAvailableContractionSizeForRangeEndingAt(
 ...
 */
void* MM_MemoryPoolSplitAddressOrderedListBase::findFreeEntryEndingAtAddr(MM_EnvironmentBase* env, void* addr)
{
    bool const compressed = compressObjectReferences();
    MM_HeapLinkedFreeHeader* currentFreeEntry;
 ...
 */
void* MM_MemoryPoolSplitAddressOrderedListBase::findFreeEntryTopStartingAtAddr(MM_EnvironmentBase* env, void* addr)
{
    bool const compressed = compressObjectReferences();
    MM_HeapLinkedFreeHeader* currentFreeEntry;
 ...
 * @return The address of head of free chain
 */
void* MM_MemoryPoolSplitAddressOrderedListBase::getFirstFreeStartingAddr(MM_EnvironmentBase* env)
{
 ...
 * @return The address of next free entry or NULL
 */
void* MM_MemoryPoolSplitAddressOrderedListBase::getNextFreeStartingAddr(MM_EnvironmentBase* env, void* currentFree)
{
 ...
 *
 */
void MM_MemoryPoolSplitAddressOrderedListBase::moveHeap(
    MM_EnvironmentBase* env, void* srcBase, void* srcTop, void* dstBase)
{
    bool const compressed = compressObjectReferences();
    for (uintptr_t i = 0; i < _heapFreeListCount; ++i) {
 ...
 */
void MM_MemoryPoolSplitAddressOrderedListBase::printCurrentFreeList(MM_EnvironmentBase* env, const char* area)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_MemoryPoolSplitAddressOrderedListBase::recalculateMemoryPoolStatistics(MM_EnvironmentBase* env)
{
...
void MM_MemoryPoolSplitAddressOrderedListBase::appendCollectorLargeAllocateStats()
{
