...
bool J9ModronFreeList::initialize(MM_EnvironmentBase *env) {
  if (!_lock.initialize(env, &env->getExtensions()->lnrlOptions,
                        "J9ModronFreeList:_lock")) {
    return false;
  }
...
void J9ModronFreeList::tearDown() { _lock.tearDown(); }

/****************************************
 * Hint Functionality
 ****************************************
 */
void J9ModronFreeList::clearHints() {
...
bool MM_MemoryPoolSplitAddressOrderedListBase::initialize(
  ...
  _currentThreadFreeList = (uintptr_t *)extensions->getForge()->allocate(
      sizeof(uintptr_t) * _heapFreeListCount,
  ...
  _heapFreeLists = (J9ModronFreeList *)extensions->getForge()->allocate(
      sizeof(J9ModronFreeList) * _heapFreeListCountExtended,
      OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
  ...
  /* set multiple factor = 2 for doubling _maxVeryLargeEntrySizes to avoid run
   * out of _veryLargeEntryPool (minus count during decrement) */
  _largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(
      ...
      (float)extensions->largeObjectAllocationProfilingSizeClassRatio /
          (float)100.0,
      _extensions->heap->getMaximumMemorySize(),
      tlhMaximumSize + _minimumFreeEntrySize, _extensions->tlhMinimumSize, 2);

  if (NULL == _largeObjectAllocateStats) {
    return false;
  }

  /* If we ever subclass MM_LargeObjectAllocateStats we will have to allocate
   * this as an array of pointers to MM_LargeObjectAllocateStats and invoke
   * newInstance instead of just initialize Than, we may also need to virtualize
   * initialize() and tearDown().
   */
  _largeObjectAllocateStatsForFreeList =
      (MM_LargeObjectAllocateStats *)extensions->getForge()->allocate(
          sizeof(MM_LargeObjectAllocateStats) * _heapFreeListCountExtended,
          OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
  ...
  if (NULL == _largeObjectAllocateStatsForFreeList) {
    ...
    for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
      new (&_largeObjectAllocateStatsForFreeList[i])
          MM_LargeObjectAllocateStats(env);

      /* set multiple factor = 2 for doubling _maxVeryLargeEntrySizes to avoid
       * run out of _veryLargeEntryPool (minus count during decrement) */
   ...
   * _largeObjectCollectorAllocateStatsForFreeList yet. Tenure SubSpace for
   * Gencon will set _largeObjectCollectorAllocateStatsForFreeList to
   * _largeObjectCollectorAllocateStatsForFreeList (we append collector stats to
   * mutator stats) Tenure SubSpace for Flat will leave
   * _largeObjectCollectorAllocateStatsForFreeList at NULL (no interest in
   * collector stats) We do not even need _largeObjectCollectorAllocateStats
   * here (as we do in plain MPAOL) - we'll just merge whatever
   * _largeObjectCollectorAllocateStatsForFreeList contatins (appended Collector
   * stats or not)
   */

  if (!_resetLock.initialize(
          env, &extensions->lnrlOptions,
          "MM_MemoryPoolSplitAddressOrderedList:_resetLock")) {
    return false;
  }
...
void MM_MemoryPoolSplitAddressOrderedListBase::tearDown(
    MM_EnvironmentBase *env) {
...
void *MM_MemoryPoolSplitAddressOrderedListBase::allocateObject(
  ...
  void *addr = internalAllocate(env, allocDescription->getContiguousBytes(),
                                true, _largeObjectAllocateStatsForFreeList);
...
void *MM_MemoryPoolSplitAddressOrderedListBase::collectorAllocate(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
...
void *MM_MemoryPoolSplitAddressOrderedListBase::allocateTLH(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
...
void *MM_MemoryPoolSplitAddressOrderedListBase::collectorAllocateTLH(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
    uintptr_t maximumSizeInBytesRequired, void *&addrBase, void *&addrTop,
    bool lockingRequired) {
  void *base = NULL;
...
void MM_MemoryPoolSplitAddressOrderedListBase::reset(Cause cause) {
  ...
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
    ...
    /* initialize frequent allocates for each free list based on cummulative
     * large object stats for the pool */
...
MM_MemoryPoolSplitAddressOrderedListBase::rebuildFreeListInRegion(
   ...
   * is defined in Scavenger.cpp and is not accessible here) */
...
bool MM_MemoryPoolSplitAddressOrderedListBase::printFreeListValidity(
    MM_EnvironmentBase *env) {
  ...
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
    ...
    omrtty_printf("  -- Free List %4zu (head: 0x%p, tail: 0x%p, expected size: "
                  "%16zu, expected holes: %16zu): ",
                  i, _heapFreeLists[i]._freeList, previousFreeEntry,
                  _heapFreeLists[i]._freeSize, _heapFreeLists[i]._freeCount);
    listIsSane = listIsSane &&
                 (calculatedSize == _heapFreeLists[i]._freeSize) &&
                 (calculatedHoles == _heapFreeLists[i]._freeCount);
    if (listIsSane) {
      omrtty_printf("VALID\n");
    } else {
      omrtty_printf(
          "INVALID (calculated size: %16zu, calculated holes: %16zu)\n",
...
#if defined(DEBUG)
bool MM_MemoryPoolSplitAddressOrderedListBase::isValidListOrdering() {
  bool const compressed = compressObjectReferences();
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
 ...
 */
void *MM_MemoryPoolSplitAddressOrderedListBase::findAddressAfterFreeSize(
  ...
  MM_HeapLinkedFreeHeader *currentFreeEntry =
      (MM_HeapLinkedFreeHeader *)getFirstFreeStartingAddr(
...
bool MM_MemoryPoolSplitAddressOrderedListBase::recycleHeapChunk(
    MM_EnvironmentBase *env, void *addrBase, void *addrTop,
    MM_HeapLinkedFreeHeader *previousFreeEntry,
    MM_HeapLinkedFreeHeader *nextFreeEntry, uintptr_t curFreeList) {
  bool const compressed = compressObjectReferences();
  ...
  if (internalRecycleHeapChunk(addrBase, addrTop, nextFreeEntry)) {
    if (previousFreeEntry) {
      ...
      _heapFreeLists[curFreeList]._freeList =
          (MM_HeapLinkedFreeHeader *)addrBase;
    }
 ...
 */
uintptr_t MM_MemoryPoolSplitAddressOrderedListBase::
    getAvailableContractionSizeForRangeEndingAt(
        MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
 ...
 */
void *MM_MemoryPoolSplitAddressOrderedListBase::findFreeEntryEndingAtAddr(
 ...
 */
void *MM_MemoryPoolSplitAddressOrderedListBase::findFreeEntryTopStartingAtAddr(
    MM_EnvironmentBase *env, void *addr) {
  bool const compressed = compressObjectReferences();
  MM_HeapLinkedFreeHeader *currentFreeEntry;
 ...
 */
void *MM_MemoryPoolSplitAddressOrderedListBase::getFirstFreeStartingAddr(
 ...
 * @param currentFreeListIndex will return the index of the free list containing
 * the next free starting address but ONLY if the free list index has changed
 * from the current free address to the next free address.
 *
 * @return The address of next free entry or NULL
 */
void *MM_MemoryPoolSplitAddressOrderedListBase::getNextFreeStartingAddr(
 ...
 *
 */
void MM_MemoryPoolSplitAddressOrderedListBase::moveHeap(MM_EnvironmentBase *env,
                                                        void *srcBase,
                                                        void *srcTop,
                                                        void *dstBase) {
  bool const compressed = compressObjectReferences();
 ...
 */
void MM_MemoryPoolSplitAddressOrderedListBase::lock(MM_EnvironmentBase *env) {
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
 ...
 */
void MM_MemoryPoolSplitAddressOrderedListBase::unlock(MM_EnvironmentBase *env) {
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
 ...
 */
void MM_MemoryPoolSplitAddressOrderedListBase::printCurrentFreeList(
  ...
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
    ...
    while (currentFreeEntry) {
      ...
      omrtty_printf(msg, currentFreeEntry, currentFreeEntry->afterEnd(),
...
void MM_MemoryPoolSplitAddressOrderedListBase::recalculateMemoryPoolStatistics(
    MM_EnvironmentBase *env) {
...
uintptr_t MM_MemoryPoolSplitAddressOrderedListBase::getActualFreeMemorySize() {
  uintptr_t result = 0;
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
...
uintptr_t MM_MemoryPoolSplitAddressOrderedListBase::getActualFreeEntryCount() {
  uintptr_t result = 0;
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
...
void MM_MemoryPoolSplitAddressOrderedListBase::resetLargeObjectAllocateStats() {
  ...
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
    ...
    _largeObjectAllocateStatsForFreeList[i]
        .getTlhAllocSizeClassStats()
        ->resetCounts();
  }
}

void MM_MemoryPoolSplitAddressOrderedListBase::mergeFreeEntryAllocateStats() {
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
  ...
  _largeObjectAllocateStats->getFreeEntrySizeClassStats()
      ->mergeCountForVeryLargeEntries();
}

void MM_MemoryPoolSplitAddressOrderedListBase::mergeTlhAllocateStats() {
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
...
void MM_MemoryPoolSplitAddressOrderedListBase::mergeLargeObjectAllocateStats() {
  _largeObjectAllocateStats->resetCurrent();
...
void MM_MemoryPoolSplitAddressOrderedListBase::
    appendCollectorLargeAllocateStats() {
  _largeObjectCollectorAllocateStatsForFreeList =
      _largeObjectAllocateStatsForFreeList;
}
