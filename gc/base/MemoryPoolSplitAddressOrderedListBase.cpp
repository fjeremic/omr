...
{
  if (!_lock.initialize(
        env, &env->getExtensions()->lnrlOptions, "J9ModronFreeList:_lock")) {
    return false;
  }
...
void*
MM_MemoryPoolSplitAddressOrderedListBase::allocateTLH(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocDescription,
...
void*
MM_MemoryPoolSplitAddressOrderedListBase::collectorAllocateTLH(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocDescription,
  uintptr_t maximumSizeInBytesRequired,
  void*& addrBase,
...
bool
MM_MemoryPoolSplitAddressOrderedListBase::printFreeListValidity(
  MM_EnvironmentBase* env)
{
  ...
  for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
    ...
    omrtty_printf("  -- Free List %4zu (head: 0x%p, tail: 0x%p, expected size: "
                  ...
                  i,
                  _heapFreeLists[i]._freeList,
                  previousFreeEntry,
                  _heapFreeLists[i]._freeSize,
...
bool
MM_MemoryPoolSplitAddressOrderedListBase::recycleHeapChunk(
  MM_EnvironmentBase* env,
  void* addrBase,
  void* addrTop,
  MM_HeapLinkedFreeHeader* previousFreeEntry,
 ...
 */
void*
MM_MemoryPoolSplitAddressOrderedListBase::findFreeEntryEndingAtAddr(
  MM_EnvironmentBase* env,
  void* addr)
{
  bool const compressed = compressObjectReferences();
 ...
 */
void*
MM_MemoryPoolSplitAddressOrderedListBase::findFreeEntryTopStartingAtAddr(
  MM_EnvironmentBase* env,
  void* addr)
{
  bool const compressed = compressObjectReferences();
 ...
 */
void
MM_MemoryPoolSplitAddressOrderedListBase::moveHeap(MM_EnvironmentBase* env,
                                                   void* srcBase,
                                                   void* srcTop,
                                                   void* dstBase)
{
 ...
 */
void
MM_MemoryPoolSplitAddressOrderedListBase::printCurrentFreeList(
  MM_EnvironmentBase* env,
  const char* area)
{
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void
MM_MemoryPoolSplitAddressOrderedListBase::recalculateMemoryPoolStatistics(
  MM_EnvironmentBase* env)
{
