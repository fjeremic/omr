...
protected:
  ...
  bool _chunkedArray;
  omrarrayptr_t _spine; /**< field to store the arraylet spine allocated during
                           an arraylet allocation */
  bool _threadAtSafePoint;
  MM_MemoryPool *_memoryPool;

  /* The following fields are applicable to collectorAllocate requests only;
   * ignored on other allocate requests */
  bool _collectorAllocateExpandOnFailure; /**< if the allocation fails then
                                             expand heap if possible to satify
                                             the allocation */
  bool _collectorAllocateSatisfyAnywhere; /**< satify an allocate from any
                                             subpool */
  MM_MemorySubSpace::AllocationType
      _allocationType; /**< Describes what type of work the allocation layer
...
public:
  ...
  MMINLINE bool getTenuredFlag() {
    return (_allocateFlags & OMR_GC_ALLOCATE_OBJECT_TENURED) ==
           OMR_GC_ALLOCATE_OBJECT_TENURED;
  }
  MMINLINE bool getPreHashFlag() {
    return OMR_GC_ALLOCATE_OBJECT_HASHED ==
           (_allocateFlags & OMR_GC_ALLOCATE_OBJECT_HASHED);
  }

  /* NON_ZERO_TLH flag set means JIT requested to skip zero in it (not what its
   * name suggests to allocate from non zero TLH). We internally, decide which
   * TLH this gets allocated from (depending if its dual TLH mode or not), and
   * whether we clear it or not */
  MMINLINE bool getNonZeroTLHFlag() {
    return (_allocateFlags & OMR_GC_ALLOCATE_OBJECT_NON_ZERO_TLH) ==
           OMR_GC_ALLOCATE_OBJECT_NON_ZERO_TLH;
  }

  MMINLINE uintptr_t getAllocateFlags() { return _allocateFlags; }

  MMINLINE void setObjectFlags(uint32_t objectFlags) {
  ...
  /**
   * Contiguous bytes will only differ from bytes requested if this allocation
   * is for an arraylet spine
   */
  MMINLINE uintptr_t getContiguousBytes() {
    return (0 == _spineBytes) ? _bytesRequested : _spineBytes;
  }

  MMINLINE void
  setAllocationType(MM_MemorySubSpace::AllocationType allocationType) {
    _allocationType = allocationType;
  }
  MMINLINE MM_MemorySubSpace::AllocationType getAllocationType() {
  ...
  MMINLINE void setBytesRequested(uintptr_t bytesRequested) {
    _bytesRequested = bytesRequested;
  }
  MMINLINE void setMemorySpace(MM_MemorySpace *memorySpace) {
    _memorySpace = memorySpace;
  }
  MMINLINE void setMemorySubSpace(MM_MemorySubSpace *memorySubSpace) {
    _memorySubSpace = memorySubSpace;
  }

  MMINLINE void setAllocationTaxSize(uintptr_t size) {
    _allocationTaxSize = size;
  }
  MMINLINE uintptr_t getAllocationTaxSize() { return _allocationTaxSize; }

  /**
   * Save the spine to this thread's "saved object" slot so that it will be used
   * as a root and will be updated if the spine moves. NOTE:  This call must be
   * balanced by a following restoreObjects call
   */
  ...
  /**
   * Restore the spine from this thread's "saved object" slot where it was
   * stored for safe marking and update. NOTE:  This call must be balanced by a
   * preceding saveObjects call
   */
  ...
  MMINLINE void payAllocationTax(MM_EnvironmentBase *env) {
    if (0 != _allocationTaxSize) {
      ...
      if (extensions->trackMutatorThreadCategory) {
        /* this thread is doing concurrent GC work, charge time spent against
         * the GC category */
  ...
  MMINLINE void setNurseryAllocation(bool nurseryAlloc) {
    _nurseryAllocation = nurseryAlloc;
  }
  ...
  MMINLINE void setMemoryPool(MM_MemoryPool *memoryPool) {
    _memoryPool = memoryPool;
  }
  MMINLINE MM_MemoryPool *getMemoryPool() { return _memoryPool; }

  MMINLINE void setCollectorAllocateExpandOnFailure(bool expand) {
    _collectorAllocateExpandOnFailure = expand;
  }
  MMINLINE bool isCollectorAllocateExpandOnFailure() {
    return _collectorAllocateExpandOnFailure;
  }

  MMINLINE void setCollectorAllocateSatisfyAnywhere(bool anywhere) {
    _collectorAllocateSatisfyAnywhere = anywhere;
  }
  MMINLINE bool isCollectorAllocateSatisfyAnywhere() {
    return _collectorAllocateSatisfyAnywhere;
  }
   ...
   */
  MMINLINE void setAllocationSucceeded(bool succeeded) {
    _allocationSucceeded = succeeded;
  }
  /**
   ...
   */
  MM_AllocateDescription(uintptr_t bytesRequested, uintptr_t allocateFlags,
                         bool collectAndClimb, bool threadAtSafePoint)
      : MM_Base(), _bytesRequested(bytesRequested),
        _allocateFlags(allocateFlags), _objectFlags(0),
        _allocationSucceeded(false), _memorySpace(NULL), _memorySubSpace(NULL),
        _allocationTaxSize(0), _tlhAllocation(false), _nurseryAllocation(false),
        _loaAllocation(false), _spineBytes(0), _numArraylets(0),
        _chunkedArray(false), _spine(NULL),
        _threadAtSafePoint(threadAtSafePoint), _memoryPool(NULL),
        ...
        _collectAndClimb(collectAndClimb), _climb(false),
