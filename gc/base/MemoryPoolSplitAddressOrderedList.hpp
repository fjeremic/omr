 ...
 * @ingroup GC_Base
 */
class MM_MemoryPoolSplitAddressOrderedList
    : public MM_MemoryPoolSplitAddressOrderedListBase {
  /*
   * Data members
   */
 private:
  ...
  MM_HeapLinkedFreeHeader*
      _previousReservedFreeEntry; /**< combination _previousReservedFreeEntry
                                     and _reservedFreeListIndex are used to
                                     identify or update the reservedFreeEntry */
  uintptr_t
      _reservedFreeListIndex; /**< the reservedFreeEntry is initialized only
                                 once in first pass iterating after sweep,
                                 used/updated only in second pass */
 ...
 private:
   ...
   */
  MMINLINE bool isPreviousReservedFreeEntry(
      MM_HeapLinkedFreeHeader* previousFreeEntry, uintptr_t curFreeList) {
    return ((previousFreeEntry == _previousReservedFreeEntry) &&
   ...
   */
  MMINLINE bool isCurrentReservedFreeEntry(
      MM_HeapLinkedFreeHeader* curFreeEntry, uintptr_t curFreeList) {
    bool const compressed = compressObjectReferences();
    ...
    if (retValue) {
      if (NULL == _previousReservedFreeEntry) {
        ...
        retValue =
            (_previousReservedFreeEntry->getNext(compressed) == curFreeEntry);
      }
    }
  ...
  MMINLINE void resetReservedFreeEntry() {
    ...
    /* if reserved free entry is not avaliable, previous entry and index are set
     * to special values, which is mostly used for debugging purposes
     */
 ...
 public:
  static MM_MemoryPoolSplitAddressOrderedList* newInstance(
   ...
   */
  MM_MemoryPoolSplitAddressOrderedList(MM_EnvironmentBase* env,
