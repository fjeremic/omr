...
public:
   ...
   * The action for each object/slot found may is specific - subclass has to
   * overload this method Note that the caller does not clean the card.  It
   * merely invokes the receiver if it is dirty so the implementation must mark
   * it as clean, if it has that responsibility.
   *
   ...
   */
  virtual void clean(MM_EnvironmentBase *env, void *lowAddress,
                     void *highAddress, Card *cardToClean) = 0;

  /**
   * Return the uintptr_t  corresponding to the VMState for this card cleaner.
   * @note All card cleanders must implement this method - the IDs are defined
   * in @ref j9modron.h
   */
   ...
   */
  MM_CardCleaner() : MM_BaseVirtual() { _typeId = __FUNCTION__; }
