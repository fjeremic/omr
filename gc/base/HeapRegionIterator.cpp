...
GC_HeapRegionIterator::GC_HeapRegionIterator(MM_HeapRegionManager *manager)
    : _space(NULL), _auxRegion(NULL), _tableRegion(NULL),
...
/**
 * Construct a HeapRegionIterator for the regions which belong to the specified
 * memory space
 *
 * @param subspace the memory subspace whose regions should be walked
 */
GC_HeapRegionIterator::GC_HeapRegionIterator(MM_HeapRegionManager *manager,
                                             MM_MemorySpace *space)
    : _space(space), _auxRegion(NULL), _tableRegion(NULL),
      _regionManager(manager),
      _includedRegionsMask(MM_HeapRegionDescriptor::ALL) {
  _auxRegion = _regionManager->getFirstAuxiliaryRegion();
  _tableRegion = _regionManager->getFirstTableRegion();
...
GC_HeapRegionIterator::GC_HeapRegionIterator(MM_HeapRegionManager *manager,
    ...
    : _space(NULL), _auxRegion(NULL), _tableRegion(NULL),
      _regionManager(manager), _includedRegionsMask(includedRegionsMask) {
  _auxRegion = _regionManager->getFirstAuxiliaryRegion();
  _tableRegion = _regionManager->getFirstTableRegion();
}
 ...
 */
bool GC_HeapRegionIterator::shouldIncludeRegion(
