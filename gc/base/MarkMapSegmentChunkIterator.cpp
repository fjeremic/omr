 ...
 */
bool GC_MarkMapSegmentChunkIterator::nextChunk(MM_HeapMap *markMap,
                                               UDATA **base, UDATA **top) {
    ...
    if (firstObject != NULL) {
      ...
      *top = chunkTop;
      if (_extensions->isVLHGC()) {
        /* while this code should work in phase2, the markedObjectIterator can
         * give back a chunkTop which points after an unused region */
        MM_HeapRegionManager *manager =
            _extensions->getHeap()->getHeapRegionManager();
        MM_HeapRegionDescriptor *desc =
            manager->tableDescriptorForAddress(firstObject);
        MM_HeapRegionDescriptor *checkDesc =
            manager->tableDescriptorForAddress((void *)((UDATA)chunkTop - 1));
