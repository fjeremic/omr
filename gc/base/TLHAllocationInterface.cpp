 ...
 */
MM_TLHAllocationInterface* MM_TLHAllocationInterface::newInstance(
    MM_EnvironmentBase* env) {
  MM_TLHAllocationInterface* allocationInterface;

  allocationInterface = (MM_TLHAllocationInterface*)env->getForge()->allocate(
      sizeof(MM_TLHAllocationInterface), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
 ...
 */
void MM_TLHAllocationInterface::reconnect(MM_EnvironmentBase* env,
                                          bool shouldFlush) {
  MM_GCExtensionsBase* extensions = env->getExtensions();
 ...
 */
void* MM_TLHAllocationInterface::allocateFromTLH(
    MM_EnvironmentBase* env,
...
#if defined(OMR_GC_NON_ZERO_TLH)
  /* If requested to skip zero init (NON_ZERO_TLH flag set), and we are in dual
   * TLH (default TLH is batch cleared),
   * we have to redirect this allocation to the non zero TLH */
  if (allocDescription->getNonZeroTLHFlag()) {
    result = _tlhAllocationSupportNonZero.allocateFromTLH(
        env, allocDescription, shouldCollectOnFailure);
  } else
#endif /* defined(OMR_GC_NON_ZERO_TLH) */
  {
    result = _tlhAllocationSupport.allocateFromTLH(env, allocDescription,
...
void* MM_TLHAllocationInterface::allocateObject(
    MM_EnvironmentBase* env,
    MM_AllocateDescription* allocDescription,
  ...
  if (allocDescription->getTenuredFlag()) {
    ...
    if (NULL != cac) {
      result =
          cac->allocateObject(env, allocDescription, shouldCollectOnFailure);
    } else if (NULL != ac) {
      result =
          ac->allocateObject(env, allocDescription, shouldCollectOnFailure);
    } else {
      MM_MemorySubSpace* subspace = memorySpace->getTenureMemorySubSpace();
      result = subspace->allocateObject(env, allocDescription, NULL, NULL,
                                        shouldCollectOnFailure);
    }
  } else {
    ...
    if (NULL == result) {
      if (NULL != ac) {
        ...
        result = memorySpace->getDefaultMemorySubSpace()->allocateObject(
            env, allocDescription, NULL, NULL, shouldCollectOnFailure);
      }
    }
  }
  ...
  env->_oolTraceAllocationBytes +=
      (_stats.bytesAllocated() -
       _bytesAllocatedBase); /* Increment by bytes allocated */

  return result;
}

void* MM_TLHAllocationInterface::allocateArray(
    MM_EnvironmentBase* env,
    MM_AllocateDescription* allocateDescription,
    MM_MemorySpace* memorySpace,
    bool shouldCollectOnFailure) {
  return allocateObject(env, allocateDescription, memorySpace,
                        shouldCollectOnFailure);
}

void* MM_TLHAllocationInterface::allocateArrayletSpine(
    MM_EnvironmentBase* env,
    MM_AllocateDescription* allocateDescription,
    MM_MemorySpace* memorySpace,
    bool shouldCollectOnFailure) {
...
void* MM_TLHAllocationInterface::allocateArrayletLeaf(
    MM_EnvironmentBase* env,
    MM_AllocateDescription* allocateDescription,
    MM_MemorySpace* memorySpace,
    bool shouldCollectOnFailure) {
  ...
  if ((NULL != cac) && (allocateDescription->getTenuredFlag())) {
    result = cac->allocateArrayletLeaf(env, allocateDescription,
                                       shouldCollectOnFailure);
  } else if (NULL != ac) {
    /* ensure that we are allowed to use the AI in this configuration in the
     * Tarok case */
    ...
    Assert_MM_true(memorySpace->getTenureMemorySubSpace() ==
                   memorySpace->getDefaultMemorySubSpace());
    result = ac->allocateArrayletLeaf(env, allocateDescription,
                                      shouldCollectOnFailure);
  } else {
    result = memorySpace->getDefaultMemorySubSpace()->allocateArrayletLeaf(
 ...
 */
void* MM_TLHAllocationInterface::allocateTLH(
    MM_EnvironmentBase* env,
    MM_AllocateDescription* allocDescription,
...
#if defined(OMR_GC_NON_ZERO_TLH)
  if (allocDescription->getNonZeroTLHFlag()) {
    result = _tlhAllocationSupportNonZero.allocateTLH(
        env, allocDescription, memorySubSpace, memoryPool);
  } else
#endif /* defined(OMR_GC_NON_ZERO_TLH) */
  {
    result = _tlhAllocationSupport.allocateTLH(env, allocDescription,
                                               memorySubSpace, memoryPool);
  }
