...
MM_TLHAllocationInterface::newInstance(MM_EnvironmentBase* env)
{
  ...
  allocationInterface = (MM_TLHAllocationInterface*)env->getForge()->allocate(
    sizeof(MM_TLHAllocationInterface),
 ...
 */
void*
MM_TLHAllocationInterface::allocateFromTLH(
...
#endif /* defined(OMR_GC_NON_ZERO_TLH) */
  {
    result = _tlhAllocationSupport.allocateFromTLH(
      env, allocDescription, shouldCollectOnFailure);
  }
...
void*
MM_TLHAllocationInterface::allocateObject(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocDescription,
...
void*
MM_TLHAllocationInterface::allocateArray(
  MM_EnvironmentBase* env,
  ...
  bool shouldCollectOnFailure)
{
  return allocateObject(
    env, allocateDescription, memorySpace, shouldCollectOnFailure);
}

void*
MM_TLHAllocationInterface::allocateArrayletSpine(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocateDescription,
  MM_MemorySpace* memorySpace,
  bool shouldCollectOnFailure)
...
void*
MM_TLHAllocationInterface::allocateArrayletLeaf(
  MM_EnvironmentBase* env,
  MM_AllocateDescription* allocateDescription,
  MM_MemorySpace* memorySpace,
  bool shouldCollectOnFailure)
{
  ...
  if ((NULL != cac) && (allocateDescription->getTenuredFlag())) {
    result = cac->allocateArrayletLeaf(
      env, allocateDescription, shouldCollectOnFailure);
  } else if (NULL != ac) {
    /* ensure that we are allowed to use the AI in this configuration in the
     * Tarok case */
    /* allocation contexts currently aren't supported with generational schemes
    ...
    Assert_MM_true(memorySpace->getTenureMemorySubSpace() ==
                   memorySpace->getDefaultMemorySubSpace());
    result = ac->allocateArrayletLeaf(
      env, allocateDescription, shouldCollectOnFailure);
  } else {
 ...
 */
void*
MM_TLHAllocationInterface::allocateTLH(MM_EnvironmentBase* env,
                                       MM_AllocateDescription* allocDescription,
...
#endif /* defined(OMR_GC_NON_ZERO_TLH) */
  {
    result = _tlhAllocationSupport.allocateTLH(
      env, allocDescription, memorySubSpace, memoryPool);
  }
