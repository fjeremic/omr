...
void* MM_MemorySubSpaceFlat::allocateObject(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* baseSubSpace, MM_MemorySubSpace* previousSubSpace, bool shouldCollectOnFailure)
{
    Trc_MM_MSSFlat_allocate_entry(env->getLanguageVMThread(), "Object", allocDescription->getBytesRequested(), this,
...
void* MM_MemorySubSpaceFlat::allocationRequestFailed(MM_EnvironmentBase* env,
...
{
    Trc_MM_MSSFlat_allocationRequestFailed_entry(env->getLanguageVMThread(), allocateDescription->getBytesRequested(),
    ...
    if (_collector) {
        ...
        /* acquire exclusive access and, after we get it, see if we need to perform a collect or if someone else beat us
         * to it */
        if (!env->acquireExclusiveVMAccessForGC(_collector, true, true)) {
             ...
             * does NOT have exclusive access at this point.  Try and satisfy the allocate based on a GC having
             * occurred.
             */
            ...
            allocateDescription->saveObjects(env);
            /* acquire exclusive access and, after we get it, see if we need to perform a collect or if someone else
             * beat us to it */
            if (!env->acquireExclusiveVMAccessForGC(_collector)) {
                /* we have exclusive access but another thread beat us to the GC so see if they collected enough to
                 * satisfy our request */
                allocateDescription->restoreObjects(env);
                Trc_MM_MSSFlat_allocationRequestFailed(
                    env->getLanguageVMThread(), allocateDescription->getBytesRequested(), 3);
                addr = allocateGeneric(
                    env, allocateDescription, allocationType, objectAllocationInterface, _memorySubSpace);
                if (NULL != addr) {
        ...
        /* there was nothing we could do to satisfy the allocate at this level (we will either OOM or attempt a collect
         * at the higher level in our parent) */
    }
    ...
    Trc_MM_MSSFlat_allocationRequestFailed_exit(
        env->getLanguageVMThread(), allocateDescription->getBytesRequested(), 8, addr);
    return NULL;
}

void* MM_MemorySubSpaceFlat::allocateArrayletLeaf(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* baseSubSpace, MM_MemorySubSpace* previousSubSpace, bool shouldCollectOnFailure)
{
 ...
 */
void* MM_MemorySubSpaceFlat::allocateTLH(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_ObjectAllocationInterface* objectAllocationInterface, MM_MemorySubSpace* baseSubSpace,
    MM_MemorySubSpace* previousSubSpace, bool shouldCollectOnFailure)
{
    if (shouldCollectOnFailure) {
        return _memorySubSpace->allocateTLH(
            env, allocDescription, objectAllocationInterface, baseSubSpace, this, shouldCollectOnFailure);
    } else {
        if (previousSubSpace == _parent) {
 ...
 * Initialization
 */
MM_MemorySubSpaceFlat* MM_MemorySubSpaceFlat::newInstance(MM_EnvironmentBase* env,
    MM_PhysicalSubArena* physicalSubArena, MM_MemorySubSpace* childMemorySubSpace, bool usesGlobalCollector,
    uintptr_t minimumSize, uintptr_t initialSize, uintptr_t maximumSize, uintptr_t memoryType, uint32_t objectFlags)
{
    MM_MemorySubSpaceFlat* memorySubSpace;

    memorySubSpace = (MM_MemorySubSpaceFlat*)env->getForge()->allocate(
        sizeof(MM_MemorySubSpaceFlat), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (memorySubSpace) {
 ...
 */
uintptr_t MM_MemorySubSpaceFlat::adjustExpansionWithinUserIncrement(MM_EnvironmentBase* env, uintptr_t expandSize)
{
 ...
 */
bool MM_MemorySubSpaceFlat::expanded(
 ...
 */
bool MM_MemorySubSpaceFlat::expanded(MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, uintptr_t size,
    ...
    MM_EnvironmentBase* env, MM_Collector* requestCollector, MM_AllocateDescription* allocDescription)
{
    ...
    Trc_MM_MemorySubSpaceFlat_collectorExpand_Entry(
        env->getLanguageVMThread(), requestCollector, allocDescription->getBytesRequested());
...
uintptr_t MM_MemorySubSpaceFlat::releaseFreeMemoryPages(MM_EnvironmentBase* env)
{
