 ...
 */
MM_PhysicalArenaVirtualMemory* MM_PhysicalArenaVirtualMemory::newInstance(
  ...
  arena = (MM_PhysicalArenaVirtualMemory*)env->getForge()->allocate(
      sizeof(MM_PhysicalArenaVirtualMemory), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (arena) {
...
bool MM_PhysicalArenaVirtualMemory::initialize(MM_EnvironmentBase* env) {
 ...
 */
void MM_PhysicalArenaVirtualMemory::detachSubArena(
    MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena) {
  MM_PhysicalSubArenaVirtualMemory *previous, *next;
  MM_PhysicalSubArenaVirtualMemory* currentSubArena =
...
/**
 * Attach a physical subarena of the specified size to the parent arena
 * receiver. This reserves the address space within the receiver for the
 * subarena, and connects the subarena to the list of those associated to the
 * receiver (in address order).  Commit the subarena memory.
 *
 * @param subArena The physical sub arena to attach
 * @param size Maximum size the sub arena will have (also the total reserve
 * size).
 *
 * @return true if the subarena was attached successfully, false otherwise.
 */
bool MM_PhysicalArenaVirtualMemory::attachSubArena(
  ...
  switch (attachPolicy) {
    case modron_pavm_attach_policy_none:
      ...
      /* If we have reached the end of the arena list, check if there is room
    ...
    case modron_pavm_attach_policy_high_memory:
      ...
      /* If there is already a sub arena attached, check if the new sub arena
       * fits at the top */
      if (insertionHead) {
  ...
  /* now that address range is known, just before we commit, bind memory to NUMA
   * node, if applicable */
  if (0 != currentSubArena->getNumaNode()) {
    MM_GCExtensionsBase* ext = env->getExtensions();
    uintptr_t j9NodeNumber =
        ext->_numaManager.getJ9NodeNumber(currentSubArena->getNumaNode());
    if (0 != j9NodeNumber) {
      if (!ext->memoryManager->setNumaAffinity(
              ((MM_HeapVirtualMemory*)_heap)->getVmemHandle(), j9NodeNumber,
              (void*)candidateBase, size)) {
        return false;
      }
    }
  }
 ...
 */
bool MM_PhysicalArenaVirtualMemory::canExpand(
    MM_EnvironmentBase* env, MM_PhysicalSubArenaVirtualMemory* subArena,
    void* expandAddress, uintptr_t expandSize) {
  return (expandAddress >= getLowAddress()) &&
         (expandAddress < getHighAddress()) &&
 ...
 */
uintptr_t MM_PhysicalArenaVirtualMemory::maxExpansion(
    MM_EnvironmentBase* env, MM_PhysicalSubArenaVirtualMemory* subArena,
    void* expandAddress) {
...
/**
 * Determine what the maximum physical expansion allowable starting at the given
 * address is. This routine does not account for any internal sub arenas, but
 * rather what the theoretical maximum expansion is. The routine also assumes
 * that the expansion is towards the high address.
 * @return The number of bytes that are theoretically possible to expand by
 * starting at the given address.
 */
uintptr_t MM_PhysicalArenaVirtualMemory::getPhysicalMaximumExpandSizeHigh(
    MM_EnvironmentBase* env, void* address) {
...
/**
 * Determine what the maximum physical contraction allowable starting at the
 * given address is. This routine does not account for any internal sub arenas,
 * but rather what the theoretical maximum contraction is. The routine also
 * assumes that the contraction is towards the low address.
 * @return The number of bytes that are theoretically possible to contract by
 * starting at the given address.
 */
uintptr_t MM_PhysicalArenaVirtualMemory::getPhysicalMaximumContractSizeLow(
    MM_EnvironmentBase* env, void* address) {
  /* Sanity check if the given address is out of range */
 ...
 */
bool MM_PhysicalArenaVirtualMemory::inflate(MM_EnvironmentBase* env) {
 ...
 * ranges surrounding a particular address range.
 * @return the next highest valid range, or NULL if there is none.
 */
void* MM_PhysicalArenaVirtualMemory::findAdjacentHighValidAddress(
