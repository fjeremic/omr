                                           ...
                                           MM_Heap* heap)
{
  ...
  arena = (MM_PhysicalArenaVirtualMemory*)env->getForge()->allocate(
    sizeof(MM_PhysicalArenaVirtualMemory),
 ...
 */
void
MM_PhysicalArenaVirtualMemory::detachSubArena(MM_EnvironmentBase* env,
  ...
  if (0 != currentSubArena->getNumaNode()) {
    ...
    if (0 != j9NodeNumber) {
      if (!ext->memoryManager->setNumaAffinity(
            ((MM_HeapVirtualMemory*)_heap)->getVmemHandle(),
            j9NodeNumber,
            (void*)candidateBase,
            size)) {
        return false;
 ...
 */
bool
MM_PhysicalArenaVirtualMemory::canExpand(
 ...
 */
uintptr_t
MM_PhysicalArenaVirtualMemory::maxExpansion(
  MM_EnvironmentBase* env,
  MM_PhysicalSubArenaVirtualMemory* subArena,
  void* expandAddress)
{
 ...
 */
uintptr_t
MM_PhysicalArenaVirtualMemory::getPhysicalMaximumExpandSizeHigh(
  MM_EnvironmentBase* env,
  void* address)
{
 ...
 */
uintptr_t
MM_PhysicalArenaVirtualMemory::getPhysicalMaximumContractSizeLow(
  MM_EnvironmentBase* env,
  void* address)
{
  /* Sanity check if the given address is out of range */
 ...
 */
void*
MM_PhysicalArenaVirtualMemory::findAdjacentHighValidAddress(
  MM_EnvironmentBase* env)
{
