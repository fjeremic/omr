 ...
 *
 */
void MM_HeapMap::kill(MM_EnvironmentBase *env) {
  tearDown(env);
  env->getForge()->free(this);
}

bool MM_HeapMap::initialize(MM_EnvironmentBase *env) {
  bool result = false;
  ...
  if (memoryManager->createVirtualMemoryForMetadata(env, &_heapMapMemoryHandle,
                                                    _extensions->heapAlignment,
                                                    heapMapSizeRequired)) {
    _heapMapBits =
        (uintptr_t *)memoryManager->getHeapBase(&_heapMapMemoryHandle);
...
void MM_HeapMap::tearDown(MM_EnvironmentBase *env) {
  MM_MemoryManager *memoryManager = _extensions->memoryManager;
 ...
 *
 */
uintptr_t MM_HeapMap::convertHeapIndexToHeapMapIndex(MM_EnvironmentBase *env,
                                                     uintptr_t offset,
                                                     uintptr_t roundTo) {
  if (!_useCompressedHeapMap) {
    return MM_Math::roundToCeiling(
        roundTo,
        MM_Math::roundToCeiling(J9MODRON_HEAP_SLOTS_PER_HEAPMAP_SLOT, offset) /
            J9MODRON_HEAP_SLOTS_PER_HEAPMAP_SLOT);
  } else {
#if defined(OMR_GC_SEGREGATED_HEAP)
    return MM_Math::roundToCeiling(
        roundTo, (offset >> OMR_SIZECLASSES_LOG_SMALLEST) / 8);
#else
...
uintptr_t MM_HeapMap::getMaximumHeapMapSize(MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  return convertHeapIndexToHeapMapIndex(env, _maxHeapSize,
 ...
 */
bool MM_HeapMap::heapAddRange(MM_EnvironmentBase *env, uintptr_t size,
  ...
  if (_extensions->isFvtestForceMarkMapCommitFailure()) {
     ...
     * size, the conversion routine could get a different rounding result then
     * the actual end address)
     */

    uintptr_t heapOffsetLow =
        _extensions->heap->calculateOffsetFromHeapBase(lowAddress);
    uintptr_t heapOffsetHigh =
        _extensions->heap->calculateOffsetFromHeapBase(highAddress);
    uintptr_t heapMapCommitOffset =
        convertHeapIndexToHeapMapIndex(env, heapOffsetLow, sizeof(uintptr_t));
    uintptr_t heapMapCommitSize =
        convertHeapIndexToHeapMapIndex(env, heapOffsetHigh, sizeof(uintptr_t)) -
        heapMapCommitOffset;
    ...
    commited = memoryManager->commitMemory(
        ...
        heapMapCommitSize);
    if (!commited) {
      Trc_MM_HeapMap_markMapCommitFailed(
 ...
 * @return true if succeed
 */
bool MM_HeapMap::heapRemoveRange(MM_EnvironmentBase *env, uintptr_t size,
                                 void *lowAddress, void *highAddress,
  ...
  if (_extensions->isFvtestForceMarkMapDecommitFailure()) {
    ...
    if (NULL != highValidAddress) {
      validHeapMapCommitHigh =
          (void *)(((uintptr_t)_heapMapBits) + heapMapCommitEnd);
    }
    ...
    if (!decommited) {
      Trc_MM_HeapMap_markMapDecommitFailed(
          env->getLanguageVMThread(),
          (void *)(((uintptr_t)_heapMapBits) + heapMapCommitOffset),
          heapMapCommitSize, validHeapMapCommitLow, validHeapMapCommitHigh);
    }
 ...
 *
 * @param lowAddress - base of region of heap whose heap map bits are to be
 * counted
 ...
 */
uintptr_t MM_HeapMap::numberBitsInRange(MM_EnvironmentBase *env,
                                        void *lowAddress, void *highAddress) {
  uintptr_t baseIndex, topIndex;

  /* Validate passed heap references */
  Assert_MM_true(lowAddress < highAddress);
  Assert_MM_true((uintptr_t)lowAddress ==
 ...
 */
uintptr_t MM_HeapMap::setBitsInRange(MM_EnvironmentBase *env, void *lowAddress,
   ...
   * absolute heapBase not active heapBase. Hence we can't use _heapBaseDelta
   * here
   */
...
uintptr_t MM_HeapMap::setBitsForRegion(MM_EnvironmentBase *env,
                                       MM_HeapRegionDescriptor *region,
                                       bool clear) {
  return setBitsInRange(env, region->getLowAddress(), region->getHighAddress(),
                        clear);
}

bool MM_HeapMap::checkBitsForRegion(MM_EnvironmentBase *env,
                                    MM_HeapRegionDescriptor *region) {
  uintptr_t baseIndex, topIndex;
