...
GC_ObjectHeapBufferedIterator::GC_ObjectHeapBufferedIterator(
    MM_GCExtensionsBase *extensions, MM_HeapRegionDescriptor *region,
    bool includeDeadObjects, uintptr_t maxElementsToCache) {
  init(extensions, region, region->getLowAddress(), region->getHighAddress(),
       includeDeadObjects, maxElementsToCache);
}
...
void GC_ObjectHeapBufferedIterator::init(MM_GCExtensionsBase *extensions,
  ...
  _cacheCount = _populator->populateObjectHeapBufferedIteratorCache(
      _cache, _cacheSizeToUse, &_state);
}

void GC_ObjectHeapBufferedIterator::reset(uintptr_t *base, uintptr_t *top) {
...
omrobjectptr_t GC_ObjectHeapBufferedIterator::nextObject() {
  if (_cacheCount == 0) {
    return NULL;
  }
...
GC_ObjectHeapBufferedIterator::getPopulator() {
  ...
  case MM_HeapRegionDescriptor::BUMP_ALLOCATED_IDLE:
    /* (for all intents and purposes, an IDLE region is the same as a FREE
     * region) */
...
void GC_ObjectHeapBufferedIterator::advance(uintptr_t sizeInBytes) {
  _cacheIndex = 0;
