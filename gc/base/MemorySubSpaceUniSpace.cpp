 ...
 */
void MM_MemorySubSpaceUniSpace::checkResize(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, bool _systemGC)
{
 ...
 */
bool MM_MemorySubSpaceUniSpace::timeForHeapExpand(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
 ...
 */
bool MM_MemorySubSpaceUniSpace::timeForHeapContract(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, bool systemGC)
{
...
MM_MemorySubSpaceUniSpace::calculateTargetContractSize(MM_EnvironmentBase* env, uintptr_t allocSize, bool ratioContract)
{
    ...
    if (allocSize > getApproximateActiveFreeMemorySize()) {
        ...
        uintptr_t maximumFreePercent = ratioContract ? OMR_MIN(_extensions->heapFreeMinimumRatioMultiplier + 5, _extensions->heapFreeMaximumRatioMultiplier + 1) : _extensions->heapFreeMaximumRatioMultiplier + 1;
...
MM_MemorySubSpaceUniSpace::checkForRatioExpand(MM_EnvironmentBase* env, uintptr_t bytesRequired)
{
    ...
    if (gcPercentage < _extensions->heapExpansionGCTimeThreshold) {
        ...
        if (_extensions->heapFreeMaximumRatioMultiplier < 100) {
			 ...
			 */
            maxExpandSize = ((maxFree - currentFree) / (100 - _extensions->heapFreeMaximumRatioMultiplier)) * _extensions->heapFreeMaximumRatioDivisor;
