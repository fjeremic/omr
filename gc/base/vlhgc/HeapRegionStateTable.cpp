...
HeapRegionStateTable::newInstance(Forge* forge,
                                  uintptr_t heapBase,
...
bool
HeapRegionStateTable::initialize(Forge* forge,
                                 uintptr_t heapBase,
                                 uintptr_t regionShift,
                                 uintptr_t regionCount)
{
  ...
  /* 1 byte per region */
  _table = (uint8_t*)forge->allocate(
    regionCount, AllocationCategory::FIXED, OMR_GET_CALLSITE());
  if (NULL == _table) {
    return false;
  }
