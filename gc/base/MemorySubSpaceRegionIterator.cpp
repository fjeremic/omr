...
GC_MemorySubSpaceRegionIterator::GC_MemorySubSpaceRegionIterator(
...
void GC_MemorySubSpaceRegionIterator::initializeStack(uintptr_t fromStackSlot) {
  _leafStackSlot = fromStackSlot;
  while (_subSpaceStack[_leafStackSlot]->getChildren()) {
    _leafStackSlot += 1;
    Assert_MM_true(_leafStackSlot < MAX_STACK_SLOTS);
    _subSpaceStack[_leafStackSlot] =
        _subSpaceStack[_leafStackSlot - 1]->getChildren();
  }
  _region = _subSpaceStack[_leafStackSlot]->getFirstRegion();
}

MM_HeapRegionDescriptor *GC_MemorySubSpaceRegionIterator::nextRegion() {
  ...
  if (NULL != _region) {
    ...
    if (NULL == _region) {
      ...
      if (stackSlot > 0) {
        /* anything lower (in hierarchy) should get reset (and than we get the
         * next region from the leaf subspace) */
