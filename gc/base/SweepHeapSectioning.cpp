...
MM_ParallelSweepChunkArray::newInstance(MM_EnvironmentBase* env,
                                        uintptr_t size,
                                        bool useVmem)
{
  ...
  array = (MM_ParallelSweepChunkArray*)env->getForge()->allocate(
    sizeof(MM_ParallelSweepChunkArray),
    OMR::GC::AllocationCategory::OTHER,
    OMR_GET_CALLSITE());
  if (NULL != array) {
...
MM_SweepHeapSectioning::initialize(MM_EnvironmentBase* env)
{
   ...
   * allocated */
  _head =
    MM_ParallelSweepChunkArray::newInstance(env, totalChunkCountEstimate, true);
  if (NULL == _head) {
    return false;
  }
