...
public:
  static MM_ParallelSweepChunkArray *newInstance(MM_EnvironmentBase *env,
                                                 uintptr_t size, bool useVmem);
  void kill(MM_EnvironmentBase *env);

  MM_ParallelSweepChunkArray(uintptr_t size)
      : MM_BaseVirtual(), _array(NULL), _used(0), _size(size), _next(NULL),
        _memoryHandle(), _useVmem(false) {
 ...
 */
bool MM_ParallelSweepChunkArray::initialize(MM_EnvironmentBase *env,
                                            bool useVmem) {
  bool result = false;
  ...
  if (extensions->isFvtestForceSweepChunkArrayCommitFailure()) {
    ...
    if (useVmem) {
      ...
      if (memoryManager->createVirtualMemoryForMetadata(
              env, &_memoryHandle, extensions->heapAlignment,
              _size * sizeof(MM_ParallelSweepChunk))) {
        void *base = memoryManager->getHeapBase(&_memoryHandle);
        result = memoryManager->commitMemory(
            &_memoryHandle, base, _size * sizeof(MM_ParallelSweepChunk));
        if (!result) {
 ...
 */
void MM_ParallelSweepChunkArray::tearDown(MM_EnvironmentBase *env) {
  if (_useVmem) {
    MM_GCExtensionsBase *extensions = env->getExtensions();
    MM_MemoryManager *memoryManager = extensions->memoryManager;
...
MM_ParallelSweepChunkArray::newInstance(MM_EnvironmentBase *env, uintptr_t size,
                                        bool useVmem) {
  MM_ParallelSweepChunkArray *array;

  array = (MM_ParallelSweepChunkArray *)env->getForge()->allocate(
      sizeof(MM_ParallelSweepChunkArray), OMR::GC::AllocationCategory::OTHER,
      OMR_GET_CALLSITE());
  if (NULL != array) {
 ...
 */
void MM_ParallelSweepChunkArray::kill(MM_EnvironmentBase *env) {
  tearDown(env);
 ...
 */
MM_ParallelSweepChunk *MM_SweepHeapSectioningIterator::nextChunk() {
 ...
 * Free all memory associated to the receiver.
 */
void MM_SweepHeapSectioning::kill(MM_EnvironmentBase *env) {
  tearDown(env);
  env->getForge()->free(this);
}
 ...
 * @return true on success, false on failure.
 */
bool MM_SweepHeapSectioning::initialize(MM_EnvironmentBase *env) {
  uintptr_t totalChunkCountEstimate;

  totalChunkCountEstimate = estimateTotalChunkCount(env);

  /* Allocate the lead array to see if the initial backing store can be
   * allocated */
  _head = MM_ParallelSweepChunkArray::newInstance(env, totalChunkCountEstimate,
                                                  true);
  if (NULL == _head) {
    return false;
  }

  /* Save away the initial array for other uses (routines that need its backing
   * store, such as compact) */
  _baseArray = _head;
 ...
 * Free the receivers internal structures.
 */
void MM_SweepHeapSectioning::tearDown(MM_EnvironmentBase *env) {
 ...
 * Walk all arrays in the receiver reserving the requested number of chunks. Any
 * arrays in excess will have their used count set to 0.
 * @param chunkCount Number of chunks to be reserved in the receiver.
 * @return true if the receiver successfully reserved the chunks, false
 * otherwise.
 */
bool MM_SweepHeapSectioning::initArrays(uintptr_t chunkCount) {
  /* Note that we can't use the iterator here since its used counts are
   * incorrect (may skip arrays if they are currently empty
   */
  ...
  while (0 != remainingChunkCount) {
    ...
    /* set the actual length of the array to its max, unless it is the last
     * array */
  ...
  /* If there are any remaining arrays in current allocated by the sectioning
   * iterator, set their used sizes to NULL
   */
  while (NULL != array) {
    array->_used = 0;
    array = array->_next;
 ...
 * @return true if the receiver successfully reserved enough chunks to represent
 * the heap, false otherwise.
 */
bool MM_SweepHeapSectioning::update(MM_EnvironmentBase *env) {
  ...
  if (totalChunkCount > _totalSize) {
    ...
    /* TODO: Do we want to round the number of chunks allocated to something
     * sane? */
    newArray = MM_ParallelSweepChunkArray::newInstance(
 ...
 * This routine uses the backing store of the base array and uses this memory as
 * the return value.
 * @return base address of the backing store.
 */
void *MM_SweepHeapSectioning::getBackingStoreAddress() {
 ...
 */
uintptr_t MM_SweepHeapSectioning::getBackingStoreSize() {
