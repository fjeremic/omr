 ...
 */
bool MM_ParallelSweepChunkArray::initialize(MM_EnvironmentBase* env, bool useVmem)
{
    bool result = false;
    ...
    if (extensions->isFvtestForceSweepChunkArrayCommitFailure()) {
        ...
        if (useVmem) {
            ...
            if (memoryManager->createVirtualMemoryForMetadata(
                    env, &_memoryHandle, extensions->heapAlignment, _size * sizeof(MM_ParallelSweepChunk))) {
            ...
            if (0 != _size) {
                _array = (MM_ParallelSweepChunk*)env->getForge()->allocate(
                    _size * sizeof(MM_ParallelSweepChunk), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
                result = (NULL != _array);
            } else {
                result = true;
            }
