 ...
 * This class represents the per-thread type associated to each MM_Environment
 * that controls how a thread allocates objects.  This control includes any
 * caching measures (e.g., TLH).
 */
...
protected:
  ...
  MM_AllocationStats
      _stats; /**< Allocation statistics for this allocation interface. */
...
protected:
  ...
  MM_ObjectAllocationInterface(MM_EnvironmentBase *env)
      : MM_BaseVirtual(), _owningEnv(env), _stats(),
...
public:
  ...
  MM_FrequentObjectsStats *getFrequentObjectsStats() {
