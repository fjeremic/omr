...
class MM_PhysicalArena : public MM_BaseVirtual {
   ...
   */
private:
  MM_PhysicalArena *_nextArena;     /**< In multi-PA environments, points to the
                                       next arena in the Heap */
  MM_PhysicalArena *_previousArena; /**< In multi-PA environments, points to the
                                       previous arena in the Heap */
  void *_lowAddress;  /**< The base address of this Physical Arena */
  void *_highAddress; /**< The byte after the highest address byte addressable
                         within this Physical Arena */
...
public:
  ...
  MM_PhysicalArena(MM_EnvironmentBase *env, MM_Heap *heap)
      : _nextArena(NULL), _previousArena(NULL), _lowAddress(NULL),
        _highAddress(NULL), _memorySpace(NULL), _heap(heap), _attached(false) {
    _typeId = __FUNCTION__;
  }

  MM_PhysicalArena(MM_EnvironmentBase *env, MM_MemorySpace *memorySpace)
      : _nextArena(NULL), _previousArena(NULL), _lowAddress(NULL),
        _highAddress(NULL), _memorySpace(memorySpace), _heap(NULL),
        _attached(false) {
    _typeId = __FUNCTION__;
  }
  ...
  MMINLINE void setPreviousArena(MM_PhysicalArena *arena) {
