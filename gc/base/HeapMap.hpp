...
#define J9MODRON_HEAP_SLOTS_PER_HEAPMAP_SLOT                                   \
  (J9MODRON_HEAP_SLOTS_PER_HEAPMAP_BIT * J9BITS_BITS_IN_SLOT)
#define J9MODRON_HEAP_BYTES_PER_HEAPMAP_BIT                                    \
  (J9MODRON_HEAP_SLOTS_PER_HEAPMAP_BIT * sizeof(uintptr_t))
#define J9MODRON_HEAP_BYTES_PER_HEAPMAP_BYTE                                   \
  (J9MODRON_HEAP_BYTES_PER_HEAPMAP_BIT * BITS_IN_BYTE)
#define J9MODRON_HEAP_BYTES_PER_HEAPMAP_SLOT                                   \
  (J9MODRON_HEAP_BYTES_PER_HEAPMAP_BYTE * sizeof(uintptr_t))
...
class MM_HeapMap : public MM_BaseVirtual {
   ...
   */
private:
  const bool _useCompressedHeapMap; /* selects compressed/uncompressed heap map
                                       for realtime/nonrealtime contexts */

protected:
  const uintptr_t
      _heapMapIndexShift; /* number of low-order bits to be shifted out of heap
                             address to obtain heap map slot index */
  const uintptr_t
      _heapMapBitMask; /* bit mask for capturing bit index within heap map slot
                          from (unshifted) heap address */
  const uintptr_t
      _heapMapBitShift; /* number of low-order bits to be shifted out of
                           captured bit index to obtain actual bit index */
...
protected:
  ...
  uintptr_t convertHeapIndexToHeapMapIndex(MM_EnvironmentBase *env,
                                           uintptr_t size, uintptr_t roundTo);

public:
  void kill(MM_EnvironmentBase *env);

  virtual bool heapAddRange(MM_EnvironmentBase *env, uintptr_t size,
                            void *lowAddress, void *highAddress);
  virtual bool heapRemoveRange(MM_EnvironmentBase *env, uintptr_t size,
                               void *lowAddress, void *highAddress,
  ...
  MMINLINE uintptr_t getObjectGrain() {
    return ((uintptr_t)1) << _heapMapBitShift;
  };

  MMINLINE void getSlotIndexAndMask(omrobjectptr_t objectPtr,
                                    uintptr_t *slotIndex, uintptr_t *bitMask) {
  ...
  MMINLINE uintptr_t getBitIndex(omrobjectptr_t objectPtr) {
  ...
  MMINLINE uintptr_t getSlotIndex(omrobjectptr_t objectPtr) {
    uintptr_t slotIndex = ((uintptr_t)objectPtr) - _heapMapBaseDelta;
    slotIndex >>= _heapMapIndexShift;
    return slotIndex;
  }

  MMINLINE bool isBitSet(omrobjectptr_t objectPtr) {
  ...
  MMINLINE bool atomicSetBit(omrobjectptr_t objectPtr) {
    uintptr_t slotIndex, bitMask;
    ...
    do {
      ...
      if (oldValue & bitMask) {
        return false;
                             ...
                             slotAddress, oldValue, oldValue | bitMask));
    return true;
  }

  MMINLINE void atomicSetSlot(uintptr_t slotIndex, uintptr_t slotValue) {
    /* Ensure compiler does not optimize away assign into oldValue */
    ...
    do {
      oldValue = *slotAddress;
                             ...
                             slotAddress, oldValue, oldValue | slotValue));
  }

  MMINLINE uintptr_t getSlot(uintptr_t slotIndex) {
    return _heapMapBits[slotIndex];
  }

  MMINLINE void setSlot(uintptr_t slotIndex, uintptr_t slotValue) {
    _heapMapBits[slotIndex] = slotValue;
  }

  MMINLINE bool setBit(omrobjectptr_t objectPtr) {
    uintptr_t slotIndex, bitMask;
  ...
  MMINLINE bool clearBit(omrobjectptr_t objectPtr) {
    uintptr_t slotIndex, bitMask;
    uintptr_t *slotAddress;
  ...
  uintptr_t numberBitsInRange(MM_EnvironmentBase *env, void *lowAddress,
                              void *highAddress);

  /**
   * Set all heap map bits for a specified heap range either ON or OFF
   *
   * @param lowAddress - base of region of heap whoose heap map bits are to be
   * set
   ...
   */
  uintptr_t setBitsInRange(MM_EnvironmentBase *env, void *lowAddress,
                           void *highAddress, bool clear);
   ...
   * @return the amount of memory actually cleard
   */
  uintptr_t setBitsForRegion(MM_EnvironmentBase *env,
                             MM_HeapRegionDescriptor *region, bool clear);
   ...
   */
  bool checkBitsForRegion(MM_EnvironmentBase *env,
   ...
   */
#if defined(OMR_GC_SEGREGATED_HEAP)
#define J9MODRON_HEAPMAP_SELECT_INDEX_SHIFT(compress)                          \
  (compress ? ((J9MODRON_HEAPMAP_LOG_SIZEOF_UDATA) +                           \
               (OMR_SIZECLASSES_LOG_SMALLEST))                                 \
            : (J9MODRON_HEAPMAP_INDEX_SHIFT))
#define J9MODRON_HEAPMAP_SELECT_BIT_MASK(compress)                             \
  ((1 << (J9MODRON_HEAPMAP_SELECT_INDEX_SHIFT(compress))) - 1)
#define J9MODRON_HEAPMAP_SELECT_BIT_SHIFT(compress)                            \
  (compress ? (OMR_SIZECLASSES_LOG_SMALLEST) : (J9MODRON_HEAPMAP_BIT_SHIFT))
#else
#define J9MODRON_HEAPMAP_SELECT_INDEX_SHIFT(compress)                          \
  (J9MODRON_HEAPMAP_INDEX_SHIFT)
#define J9MODRON_HEAPMAP_SELECT_BIT_MASK(compress)                             \
...
#define J9MODRON_HEAPMAP_SELECT_BIT_SHIFT(compress) (J9MODRON_HEAPMAP_BIT_SHIFT)
  ...
  MM_HeapMap(MM_EnvironmentBase *env, uintptr_t maxHeapSize,
             bool useCompressedHeapMap = false)
      : MM_BaseVirtual(), _useCompressedHeapMap(useCompressedHeapMap),
        _heapMapIndexShift(
            J9MODRON_HEAPMAP_SELECT_INDEX_SHIFT(useCompressedHeapMap)),
        _heapMapBitMask(J9MODRON_HEAPMAP_SELECT_BIT_MASK(useCompressedHeapMap)),
        _heapMapBitShift(
            J9MODRON_HEAPMAP_SELECT_BIT_SHIFT(useCompressedHeapMap)),
        _extensions(env->getExtensions()), _heapBase(NULL), _heapTop(NULL),
        _heapMapMemoryHandle(), _heapMapBaseDelta(0), _heapMapBits(NULL),
