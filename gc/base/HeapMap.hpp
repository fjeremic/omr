...
protected:
  ...
  const uintptr_t
    _heapMapBitShift; /* number of low-order bits to be shifted out of captured
                         bit index to obtain actual bit index */
...
#define J9MODRON_HEAPMAP_SELECT_INDEX_SHIFT(compress)                          \
  (compress                                                                    \
     ? ((J9MODRON_HEAPMAP_LOG_SIZEOF_UDATA) + (OMR_SIZECLASSES_LOG_SMALLEST))  \
