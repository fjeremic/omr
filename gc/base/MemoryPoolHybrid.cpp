...
MM_MemoryPoolHybrid::internalAllocateFromList(
...
void*
MM_MemoryPoolHybrid::internalAllocate(
  MM_EnvironmentBase* env,
  uintptr_t sizeInBytesRequired,
...
bool
MM_MemoryPoolHybrid::internalAllocateTLH(
  MM_EnvironmentBase* env,
                                              ...
                                              uintptr_t size)
{
  ...
  connectInnerMemoryToPool(env,
                           (MM_HeapLinkedFreeHeader*)address,
                           size,
...
void
MM_MemoryPoolHybrid::appendToFreeList(MM_EnvironmentBase* env,
                                      ...
                                      uintptr_t size)
{
  ...
  appendToFreeList(
    env, freeList, freeListTail, (MM_HeapLinkedFreeHeader*)lowAddress);
}

uintptr_t
MM_MemoryPoolHybrid::coalesceExpandRangeWithLastFreeEntry(
...
uintptr_t
MM_MemoryPoolHybrid::coalesceNewFreeEntryWithLastFreeEntry(
  MM_HeapLinkedFreeHeader* lastFreeEntry,
  J9ModronFreeList* freelist,
  MM_HeapLinkedFreeHeader* newFreeEntry)
{
...
void
MM_MemoryPoolHybrid::moveLastFreeEntryToReservedFreeList(
  MM_EnvironmentBase* env,
...
bool
MM_MemoryPoolHybrid::tryContractWithRangeInFreelist(MM_EnvironmentBase* env,
                                                    J9ModronFreeList* freeList,
                                                    ...
                                                    void* highAddress)
{
  ...
  if (NULL != freeList->_freeList) {
    ...
    if ((currentFreeEntry + currentFreeEntry->getSize()) == highAddress) {
      ...
      Assert_MM_true((NULL == currentFreeEntry) ||
                     (previousFreeEntry < currentFreeEntry));
      setNextForFreeEntryInFreeList(
        freeList, previousFreeEntry, currentFreeEntry);
...
MM_MemoryPoolHybrid::postProcess(MM_EnvironmentBase* env, Cause cause)
{
  ...
  if (cause == forSweep) {
    ...
    if (0 < _sweepPoolState->_largestFreeEntry) {
      ...
      appendToReservedFreeList(
        env, (void*)largestFreeEntry, largestFreeEntry->getSize());
    }
 ...
 */
void
MM_MemoryPoolHybrid::expandWithRange(MM_EnvironmentBase* env,
                                     uintptr_t expandSize,
                                     void* lowAddress,
                                     void* highAddress,
                                     bool canCoalesce)
  ...
  if (expandedSize == expandSize) {
    ...
    if (reservedFreeEntryThreshold <= expandedSize) {
      if (expandedSize != expandSize) {
        ...
        moveLastFreeEntryToReservedFreeList(env,
                                            lastFreelist,
                                            perviousLastFreeEntry,
                                            lastFreeEntry,
                                            reservedFreelist,
                                            lastReservedFreeEntry);
      } else {
        /* append to reserved freelist */
        appendToFreeList(
          env, reservedFreelist, lastReservedFreeEntry, lowAddress, expandSize);
      }
    } else {
      if (expandedSize == expandSize) {
        /* append to regular freelist */
        appendToFreeList(
          env, lastFreelist, lastFreeEntry, lowAddress, expandSize);
      }
 ...
 */
void*
MM_MemoryPoolHybrid::contractWithRange(MM_EnvironmentBase* env,
                                       uintptr_t contractSize,
                                       void* lowAddress,
                                       void* highAddress)
{
  ...
  if (!tryContractWithRangeInFreelist(
        env, getReservedFreeList(), contractSize, lowAddress, highAddress)) {
    uintptr_t index = _heapFreeListCount - 1;
    while ((NULL == _heapFreeLists[index]._freeList) && (0 != index)) {
      index--;
    }
    J9ModronFreeList* freelist = &_heapFreeLists[index];
    tryContractWithRangeInFreelist(
      env, freelist, contractSize, lowAddress, highAddress);
  }
                                    ...
                                    uintptr_t freeListMemorySize)
{
  ...
  if (expandedSize == expandSize) {
    ...
    if (reservedFreeEntryThreshold <= expandedSize) {
      if (expandedSize != expandSize) {
        ...
        /* append to reserved freelist */
        appendToFreeList(
          env, reservedFreelist, lastReservedFreeEntry, freeListHead);
  ...
  while (NULL != nextFreeEntry) {
    ...
    if (reservedFreeEntryThreshold <= currentFreeEntry->getSize()) {
      appendToFreeList(
        env, reservedFreelist, lastReservedFreeEntry, currentFreeEntry);
 ...
 */
bool
MM_MemoryPoolHybrid::removeFreeEntriesWithinRange(
  MM_EnvironmentBase* env,
  void* lowAddress,
  void* highAddress,
  ...
  if (NULL != currentFreeEntry) {
    ...
    if (currentFreeEntry < (MM_HeapLinkedFreeHeader*)lowAddress) {
      ...
      if (createFreeEntry(
        ...
        setNextForFreeEntryInFreeList(
          &_heapFreeLists[currentFreeListIndex], previousFreeEntry, NULL);
      }
      baseAddr = lowAddress;
    } else {
      setNextForFreeEntryInFreeList(
