 ...
 */
MM_MemoryPoolHybrid* MM_MemoryPoolHybrid::newInstance(
...
MM_MemoryPoolHybrid* MM_MemoryPoolHybrid::newInstance(
  ...
  memoryPool = (MM_MemoryPoolHybrid*)env->getForge()->allocate(
      sizeof(MM_MemoryPoolHybrid), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (memoryPool) {
    memoryPool = new (memoryPool)
        MM_MemoryPoolHybrid(env, minimumFreeEntrySize, maxSplit, name);
 ...
 ****************************************
 */
MMINLINE MM_HeapLinkedFreeHeader* MM_MemoryPoolHybrid::internalAllocateFromList(
...
void* MM_MemoryPoolHybrid::internalAllocate(
    MM_EnvironmentBase* env, uintptr_t sizeInBytesRequired,
...
bool MM_MemoryPoolHybrid::internalAllocateTLH(
 ...
 ****************************************
 */
MMINLINE void MM_MemoryPoolHybrid::appendToReservedFreeList(
    MM_EnvironmentBase* env, void* address, uintptr_t size) {
  bool const compressed = compressObjectReferences();
  MM_HeapLinkedFreeHeader* freeListTail = getReservedFreeList()->_freeList;
  while ((NULL != freeListTail) &&
         (NULL != freeListTail->getNext(compressed))) {
    freeListTail = freeListTail->getNext(compressed);
  }

  connectInnerMemoryToPool(env, (MM_HeapLinkedFreeHeader*)address, size,
...
void MM_MemoryPoolHybrid::appendToFreeList(
    MM_EnvironmentBase* env, J9ModronFreeList* freeList,
    MM_HeapLinkedFreeHeader* freeListTail,
    MM_HeapLinkedFreeHeader* newFreeEntry) {
  bool const compressed = compressObjectReferences();
  setNextForFreeEntryInFreeList(freeList, freeListTail, newFreeEntry);
  newFreeEntry->setNext(NULL, compressed);
...
MMINLINE void MM_MemoryPoolHybrid::appendToFreeList(
    MM_EnvironmentBase* env, J9ModronFreeList* freeList,
  ...
  appendToFreeList(env, freeList, freeListTail,
                   (MM_HeapLinkedFreeHeader*)lowAddress);
}

uintptr_t MM_MemoryPoolHybrid::coalesceExpandRangeWithLastFreeEntry(
    MM_HeapLinkedFreeHeader* lastFreeEntry, J9ModronFreeList* freelist,
    void* lowAddress, uintptr_t expandSize) {
...
uintptr_t MM_MemoryPoolHybrid::coalesceNewFreeEntryWithLastFreeEntry(
    MM_HeapLinkedFreeHeader* lastFreeEntry, J9ModronFreeList* freelist,
    MM_HeapLinkedFreeHeader* newFreeEntry) {
...
void MM_MemoryPoolHybrid::moveLastFreeEntryToReservedFreeList(
    MM_EnvironmentBase* env, J9ModronFreeList* fromFreeList,
    MM_HeapLinkedFreeHeader* previousFreeEntry,
    MM_HeapLinkedFreeHeader* freeEntry, J9ModronFreeList* toFreeList,
    MM_HeapLinkedFreeHeader* toFreeListTail) {
  bool const compressed = compressObjectReferences();
  Assert_MM_true(NULL == freeEntry->getNext(compressed));
...
bool MM_MemoryPoolHybrid::tryContractWithRangeInFreelist(
    MM_EnvironmentBase* env, J9ModronFreeList* freeList, uintptr_t contractSize,
  ...
  if (NULL != freeList->_freeList) {
    ...
    previousFreeEntry = NULL;
    while (NULL != (nextFreeEntry = currentFreeEntry->getNext(compressed))) {
      previousFreeEntry = currentFreeEntry;
    ...
    if ((currentFreeEntry + currentFreeEntry->getSize()) == highAddress) {
      ...
      Assert_MM_true((NULL == currentFreeEntry) ||
                     (previousFreeEntry < currentFreeEntry));
      setNextForFreeEntryInFreeList(freeList, previousFreeEntry,
                                    currentFreeEntry);
...
void MM_MemoryPoolHybrid::postProcess(MM_EnvironmentBase* env, Cause cause) {
  bool const compressed = compressObjectReferences();
  ...
  if (cause == forSweep) {
    ...
    if (0 < _sweepPoolState->_largestFreeEntry) {
      ...
      if (NULL == _sweepPoolState->_previousLargestFreeEntry) {
        ...
        _heapFreeLists[0]._freeList = largestFreeEntry->getNext(compressed);
      } else {
        largestFreeEntry =
            _sweepPoolState->_previousLargestFreeEntry->getNext(compressed);
        _sweepPoolState->_previousLargestFreeEntry->setNext(
            largestFreeEntry->getNext(compressed), compressed);
      }
      ...
      appendToReservedFreeList(env, (void*)largestFreeEntry,
                               largestFreeEntry->getSize());
    }
    ...
    if (reservedFreeEntryAvaliable) {
      ...
      if (NULL == previousReservedFreeEntry) {
        ...
        _heapFreeLists[reservedFreeListIndex]._freeList =
            largestFreeEntry->getNext(compressed);
      } else {
        largestFreeEntry = previousReservedFreeEntry->getNext(compressed);
        previousReservedFreeEntry->setNext(
            largestFreeEntry->getNext(compressed), compressed);
      }
      _heapFreeLists[currentFreeListIndex]._freeSize -=
          largestFreeEntry->getSize();
 ...
 *
 */
void MM_MemoryPoolHybrid::expandWithRange(MM_EnvironmentBase* env,
   ...
   * it doesn't connect with reserved free entry, it may connect with the last
   * free entry, if the newly created entry is bigger than
   * VeryLargeObjectThreshold, move or append the entry into reserved freelist.
  ...
  if (NULL != reservedFreelist->_freeList) {
    ...
    while (lastReservedFreeEntry->getNext(compressed)) {
      lastReservedFreeEntry = lastReservedFreeEntry->getNext(compressed);
    }
    if (canCoalesce) {
      expandedSize = coalesceExpandRangeWithLastFreeEntry(
          lastReservedFreeEntry, reservedFreelist, lowAddress, expandSize);
    }
  }
  ...
  if (expandedSize == expandSize) {
    ...
    if (NULL != lastFreelist->_freeList) {
      ...
      while (lastFreeEntry->getNext(compressed)) {
        perviousLastFreeEntry = lastFreeEntry;
        lastFreeEntry = lastFreeEntry->getNext(compressed);
      }

      if (canCoalesce) {
        expandedSize = coalesceExpandRangeWithLastFreeEntry(
            lastFreeEntry, lastFreelist, lowAddress, expandSize);
      }
    }

    uintptr_t reservedFreeEntryThreshold =
        env->getExtensions()
    ...
    if (reservedFreeEntryThreshold <= expandedSize) {
      if (expandedSize != expandSize) {
        ...
        moveLastFreeEntryToReservedFreeList(
            env, lastFreelist, perviousLastFreeEntry, lastFreeEntry,
            reservedFreelist, lastReservedFreeEntry);
      } else {
        /* append to reserved freelist */
        appendToFreeList(env, reservedFreelist, lastReservedFreeEntry,
                         lowAddress, expandSize);
      }
    } else {
      if (expandedSize == expandSize) {
        /* append to regular freelist */
        appendToFreeList(env, lastFreelist, lastFreeEntry, lowAddress,
                         expandSize);
      }
 ...
 *
 */
void* MM_MemoryPoolHybrid::contractWithRange(MM_EnvironmentBase* env,
  ...
  /* try contract in reserved freelist first, if can not find the range in
   * reserved freellist, then try contract in the last freelist */
  if (!tryContractWithRangeInFreelist(env, getReservedFreeList(), contractSize,
    ...
    tryContractWithRangeInFreelist(env, freelist, contractSize, lowAddress,
                                   highAddress);
  }
 ...
 *
 */
void MM_MemoryPoolHybrid::addFreeEntries(MM_EnvironmentBase* env,
  ...
  uintptr_t reservedFreeEntryThreshold =
      env->getExtensions()
          ->largeObjectAllocationProfilingVeryLargeObjectThreshold;
  MM_HeapLinkedFreeHeader* currentFreeEntry = NULL;
  MM_HeapLinkedFreeHeader* nextFreeEntry = freeListHead->getNext(compressed);
  ...
  /* try to coalesce freeListHead with reservedFreelist first, then try to
   * coalesce it with regular freelist, if the newly created entry or the rest
   * of free entries is bigger than ReservedFreeEntryThreshold, move or append
   * it into reserved freelist, otherwise into regular freelist.
   */
  if (NULL != reservedFreelist->_freeList) {
    lastReservedFreeEntry = reservedFreelist->_freeList;
    while (lastReservedFreeEntry->getNext(compressed)) {
    ...
    expandedSize = coalesceNewFreeEntryWithLastFreeEntry(
        lastReservedFreeEntry, reservedFreelist, freeListHead);
  }

  if (expandedSize == expandSize) {
    if (NULL != lastFreeEntry) {
      expandedSize = coalesceNewFreeEntryWithLastFreeEntry(
          lastFreeEntry, lastFreelist, freeListHead);
    }
    if (reservedFreeEntryThreshold <= expandedSize) {
      if (expandedSize != expandSize) {
        /* move coalesced free entry to reserved freelist */
  ...
  /* append the rest of freeEntries(except freelistHead) into reserved freelist
   * or regular freelist */
 ...
 */
bool MM_MemoryPoolHybrid::removeFreeEntriesWithinRange(
  ...
  retListMemorySize = 0;
  /* remove any free entries(in regular freelists) with in range, append them to
   * retList, then remove any reserved free entries, insert them into retList */
  ...
  while (currentFreeEntry) {
    ...
    if ((currentFreeEntry >= lowAddress) ||
        (currentFreeEntryTop > lowAddress)) {
      break;
    }
  ...
  if (NULL != currentFreeEntry) {
    ...
    /* Determine what to do with any leading bytes of the free entry that are
     * not being returned */
    if (currentFreeEntry < (MM_HeapLinkedFreeHeader*)lowAddress) {
      /* Space at the head that is not being returned - is it a valid free
       * entry? */
      if (createFreeEntry(env, currentFreeEntry, lowAddress, previousFreeEntry,
        ...
        setNextForFreeEntryInFreeList(&_heapFreeLists[currentFreeListIndex],
                                      previousFreeEntry, NULL);
      }
      baseAddr = lowAddress;
    } else {
  ...
  if (insertToList(env, baseAddr, topAddr, minimumSize, retListHead,
                   retListTail)) {
  ...
  while (NULL != currentFreeEntry) {
    ...
    if (insertToList(env, (void*)currentFreeEntry,
                     (void*)currentFreeEntry->afterEnd(), minimumSize,
                     retListHead, retListTail)) {
      retListMemoryCount += 1;
      retListMemorySize += currentFreeEntry->getSize();
