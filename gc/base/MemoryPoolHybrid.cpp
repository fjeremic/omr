    ...
    MM_EnvironmentBase* env, uintptr_t minimumFreeEntrySize, uintptr_t maxSplit, const char* name)
{
    ...
    memoryPool = (MM_MemoryPoolHybrid*)env->getForge()->allocate(
        sizeof(MM_MemoryPoolHybrid), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (memoryPool) {
 ...
 ****************************************
 */
MMINLINE MM_HeapLinkedFreeHeader* MM_MemoryPoolHybrid::internalAllocateFromList(MM_EnvironmentBase* env,
    uintptr_t sizeInBytesRequired, uintptr_t curFreeList, MM_HeapLinkedFreeHeader** previousFreeEntry,
    ...
    *previousFreeEntry = NULL;
    ...
    while (NULL != currentFreeEntry) {
        ...
        if (sizeInBytesRequired <= currentFreeEntrySize) {
            if (((walkCountCurrentList >= J9MODRON_ALLOCATION_MANAGER_HINT_MAX_WALK)
...
void* MM_MemoryPoolHybrid::internalAllocate(MM_EnvironmentBase* env, uintptr_t sizeInBytesRequired,
    bool lockingRequired, MM_LargeObjectAllocateStats* largeObjectAllocateStatsForFreeList)
{
    bool const compressed = compressObjectReferences();
...
retry:
    ...
    do {
        if (NULL != _heapFreeLists[curFreeList]._freeList) {
            ...
            currentFreeEntry = internalAllocateFromList(
                env, sizeInBytesRequired, curFreeList, &previousFreeEntry, &largestFreeEntry);
    ...
    if (recycleHeapChunk(env, recycleEntry, ((uint8_t*)recycleEntry) + recycleEntrySize, previousFreeEntry,
            currentFreeEntry->getNext(compressed), curFreeList)) {
    ...
    /* Collector object allocate stats for Survivor are not interesting (_largeObjectCollectorAllocateStatsForFreeList
     * is null for Survivor) */
...
bool MM_MemoryPoolHybrid::internalAllocateTLH(MM_EnvironmentBase* env, uintptr_t maximumSizeInBytesRequired,
...
retry:
    ...
    /* Collector TLH allocate stats for Survivor are not interesting (_largeObjectCollectorAllocateStatsForFreeList is
     * null for Survivor) */
    if (NULL != largeObjectAllocateStatsForFreeList) {
 ...
 ****************************************
 */
MMINLINE void MM_MemoryPoolHybrid::appendToReservedFreeList(MM_EnvironmentBase* env, void* address, uintptr_t size)
{
    bool const compressed = compressObjectReferences();
    MM_HeapLinkedFreeHeader* freeListTail = getReservedFreeList()->_freeList;
    while ((NULL != freeListTail) && (NULL != freeListTail->getNext(compressed))) {
...
void MM_MemoryPoolHybrid::appendToFreeList(MM_EnvironmentBase* env, J9ModronFreeList* freeList,
...
MMINLINE void MM_MemoryPoolHybrid::appendToFreeList(MM_EnvironmentBase* env, J9ModronFreeList* freeList,
...
uintptr_t MM_MemoryPoolHybrid::coalesceExpandRangeWithLastFreeEntry(
    MM_HeapLinkedFreeHeader* lastFreeEntry, J9ModronFreeList* freelist, void* lowAddress, uintptr_t expandSize)
{
...
uintptr_t MM_MemoryPoolHybrid::coalesceNewFreeEntryWithLastFreeEntry(
    MM_HeapLinkedFreeHeader* lastFreeEntry, J9ModronFreeList* freelist, MM_HeapLinkedFreeHeader* newFreeEntry)
{
...
void MM_MemoryPoolHybrid::moveLastFreeEntryToReservedFreeList(MM_EnvironmentBase* env, J9ModronFreeList* fromFreeList,
    MM_HeapLinkedFreeHeader* previousFreeEntry, MM_HeapLinkedFreeHeader* freeEntry, J9ModronFreeList* toFreeList,
...
bool MM_MemoryPoolHybrid::tryContractWithRangeInFreelist(
    MM_EnvironmentBase* env, J9ModronFreeList* freeList, uintptr_t contractSize, void* lowAddress, void* highAddress)
{
    bool const compressed = compressObjectReferences();
    bool ret = false;
    MM_HeapLinkedFreeHeader* previousFreeEntry = NULL;
    ...
    if (cause == forSweep) {
        ...
        if (0 < _sweepPoolState->_largestFreeEntry) {
            ...
            if (NULL == _sweepPoolState->_previousLargestFreeEntry) {
                ...
                largestFreeEntry = _sweepPoolState->_previousLargestFreeEntry->getNext(compressed);
                _sweepPoolState->_previousLargestFreeEntry->setNext(largestFreeEntry->getNext(compressed), compressed);
            }
        ...
        for (chunk = sectioningIterator.nextChunk(); NULL != chunk; chunk = sectioningIterator.nextChunk()) {
            ...
            if (currentFreeListSize >= freeListSplitSize) {
                 ...
                 * if previousLargestFreeEntry == splitCandidatePreviousEntry, it means largestFreeEntry ==
                 * splitCandidateEntry and the largestFreeEntry is in the next free list if previousLargestFreeEntry <
                 * splitCandidatePreviousEntry, it means the largestFreeEntry is in current freeList
                 */
        ...
        while (NULL != currentFreeList) {
            ...
            if ((_heapFreeLists[currentFreeListIndex]._freeSize >= freeListSplitSize)
        ...
        if (reservedFreeEntryAvaliable) {
            ...
            if (NULL == previousReservedFreeEntry) {
                ...
                _heapFreeLists[reservedFreeListIndex]._freeList = largestFreeEntry->getNext(compressed);
            } else {
                largestFreeEntry = previousReservedFreeEntry->getNext(compressed);
                previousReservedFreeEntry->setNext(largestFreeEntry->getNext(compressed), compressed);
            }
 ...
 *
 */
void MM_MemoryPoolHybrid::expandWithRange(
    MM_EnvironmentBase* env, uintptr_t expandSize, void* lowAddress, void* highAddress, bool canCoalesce)
{
     ...
     * if the newly created entry is bigger than VeryLargeObjectThreshold, move or append the entry into reserved
     * freelist. otherwise append the entry into regular free list.
     */
    if (NULL != reservedFreelist->_freeList) {
        ...
        if (canCoalesce) {
            expandedSize
                = coalesceExpandRangeWithLastFreeEntry(lastReservedFreeEntry, reservedFreelist, lowAddress, expandSize);
        }
    }
    ...
    if (expandedSize == expandSize) {
        ...
        if (NULL != lastFreelist->_freeList) {
            ...
            if (canCoalesce) {
                expandedSize
                    = coalesceExpandRangeWithLastFreeEntry(lastFreeEntry, lastFreelist, lowAddress, expandSize);
            }
        }
        ...
        if (reservedFreeEntryThreshold <= expandedSize) {
            if (expandedSize != expandSize) {
                ...
                moveLastFreeEntryToReservedFreeList(
                    env, lastFreelist, perviousLastFreeEntry, lastFreeEntry, reservedFreelist, lastReservedFreeEntry);
            } else {
 ...
 *
 */
void* MM_MemoryPoolHybrid::contractWithRange(
    MM_EnvironmentBase* env, uintptr_t contractSize, void* lowAddress, void* highAddress)
{
    ...
    /* try contract in reserved freelist first, if can not find the range in reserved freellist, then try contract in
     * the last freelist */
 ...
 *
 */
void MM_MemoryPoolHybrid::addFreeEntries(MM_EnvironmentBase* env, MM_HeapLinkedFreeHeader*& freeListHead,
 ...
 */
bool MM_MemoryPoolHybrid::removeFreeEntriesWithinRange(MM_EnvironmentBase* env, void* lowAddress, void* highAddress,
    ...
    uintptr_t& retListMemoryCount, uintptr_t& retListMemorySize)
{
    ...
    retListMemorySize = 0;
    /* remove any free entries(in regular freelists) with in range, append them to retList, then remove any reserved
     * free entries, insert them into retList */
    ...
    while (currentFreeEntry) {
        ...
        previousFreeEntry = currentFreeEntry;
        currentFreeEntry
            = (MM_HeapLinkedFreeHeader*)getNextFreeStartingAddr(env, currentFreeEntry, &currentFreeListIndex);
    ...
    if (NULL != currentFreeEntry) {
        ...
        while (NULL != currentFreeEntry) {
            ...
            if (appendToList(env, (void*)currentFreeEntry, (void*)currentFreeEntry->afterEnd(), minimumSize,
    ...
    while (NULL != currentFreeEntry) {
        ...
        if (insertToList(env, (void*)currentFreeEntry, (void*)currentFreeEntry->afterEnd(), minimumSize, retListHead,
                retListTail)) {
            retListMemoryCount += 1;
            retListMemorySize += currentFreeEntry->getSize();
        }
