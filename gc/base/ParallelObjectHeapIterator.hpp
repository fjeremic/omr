...
public:
  ...
  GC_ParallelObjectHeapIterator(MM_EnvironmentBase *env,
                                MM_HeapRegionDescriptor *region, void *base,
                                void *top, MM_MarkMap *markMap,
                                UDATA parallelChunkSize)
      : GC_ObjectHeapIterator(), _env(env),
        _objectHeapIterator(env->getExtensions(), region, base, top, false, 1),
        _segmentChunkIterator(env->getExtensions(), base, top,
                              parallelChunkSize),
        _topAddress(top), _markMap(markMap), _chunkBase(NULL), _chunkTop(NULL) {
