...
typedef struct SegregatedAllocationCacheStats {
  uint64_t bytesPreAllocatedTotal[OMR_SIZECLASSES_NUM_SMALL +
  ...
  uint64_t replenishesTotal[OMR_SIZECLASSES_NUM_SMALL +
                            1]; /**< The amount of times the cache has been
                                   replenished since the cache has existed (per
                                   size class). */
  uint64_t
...
public:
  ...
  void replenishCache(MM_EnvironmentBase *env, uintptr_t sizeInBytes,
                      void *cacheMemory, uintptr_t cacheSize);
...
protected:
  ...
  MM_SegregatedAllocationInterface(MM_EnvironmentBase *env)
      : MM_ObjectAllocationInterface(env), _sizeClasses(NULL),
