...
{
  uint64_t
    bytesPreAllocatedTotal[OMR_SIZECLASSES_NUM_SMALL +
                           1]; /**< The total count of cells pre-allocated since
                                  the cache has existed (per size class). */
  uint64_t
    replenishesTotal[OMR_SIZECLASSES_NUM_SMALL +
                     1]; /**< The amount of times the cache has been replenished
                            since the cache has existed (per size class). */
  uint64_t
    bytesPreAllocatedSinceRestart[OMR_SIZECLASSES_NUM_SMALL +
                                  1]; /**< The count of cells pre-allocated
                                         since the cache was last flushed. */
  uint64_t replenishesSinceRestart[OMR_SIZECLASSES_NUM_SMALL +
                                   1]; /**< The amount of times the cache has
                                          been replenished since the cache was
                                          last flushed. */
...
private:
  ...
  LanguageSegregatedAllocationCacheEntryStruct*
    _allocationCache; /**< The current cache (per size class). */
  uintptr_t
    _replenishSizes[OMR_SIZECLASSES_NUM_SMALL +
                    1]; /**< The next replenish size (per size class). */
  SegregatedAllocationCacheStats
    _allocationCacheStats; /**< Contains stats about the past pre-allocations.
                            */
  MM_SizeClasses* _sizeClasses; /**< The size classes used to map byte sizes to
                                   size class indexes. */

  bool _cachedAllocationsEnabled; /**< Are cached allocations enabled? */

  uintptr_t* _allocationCacheBases[OMR_SIZECLASSES_NUM_SMALL +
                                   1]; /**< The Base of each current cache (per
                                          size class). */
...
public:
  ...
  static MMINLINE MM_SegregatedAllocationInterface*
  getObjectAllocationInterface(
  ...
  static MMINLINE MM_SegregatedAllocationInterface*
  getObjectAllocationInterface(MM_EnvironmentBase* env)
  {
  ...
  virtual void* allocateArray(MM_EnvironmentBase* env,
                              ...
                              MM_MemorySpace* memorySpace,
                              bool shouldCollectOnFailure);
  virtual void* allocateArrayletSpine(
  ...
  uintptr_t getAllocatableSize(uintptr_t sizeClass)
  {
    return (uintptr_t)_allocationCache[sizeClass].top -
           (uintptr_t)_allocationCache[sizeClass].current;
  }
  void* allocateFromCache(MM_EnvironmentBase* env, uintptr_t sizeInBytes);
  void replenishCache(MM_EnvironmentBase* env,
  ...
  virtual bool cachedAllocationsEnabled(MM_EnvironmentBase* env)
  {
    return _cachedAllocationsEnabled;
  }
