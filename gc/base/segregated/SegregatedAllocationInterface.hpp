...
typedef struct SegregatedAllocationCacheStats {
    uint64_t bytesPreAllocatedTotal[OMR_SIZECLASSES_NUM_SMALL
        + 1]; /**< The total count of cells pre-allocated since the cache has existed (per size class). */
    uint64_t replenishesTotal[OMR_SIZECLASSES_NUM_SMALL
        + 1]; /**< The amount of times the cache has been replenished since the cache has existed (per size class). */
    uint64_t bytesPreAllocatedSinceRestart[OMR_SIZECLASSES_NUM_SMALL
        + 1]; /**< The count of cells pre-allocated since the cache was last flushed. */
    uint64_t replenishesSinceRestart[OMR_SIZECLASSES_NUM_SMALL
        + 1]; /**< The amount of times the cache has been replenished since the cache was last flushed. */
...
private:
    ...
    uintptr_t*
        _allocationCacheBases[OMR_SIZECLASSES_NUM_SMALL + 1]; /**< The Base of each current cache (per size class). */
...
public:
    ...
    static MMINLINE MM_SegregatedAllocationInterface* getObjectAllocationInterface(
        MM_ObjectAllocationInterface* allocationInterface)
    {
        return (MM_SegregatedAllocationInterface*)allocationInterface;
    }

    static MMINLINE MM_SegregatedAllocationInterface* getObjectAllocationInterface(MM_EnvironmentBase* env)
    {
    ...
    {
        return (uintptr_t)_allocationCache[sizeClass].top - (uintptr_t)_allocationCache[sizeClass].current;
    }
