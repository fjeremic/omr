...
public:
   ...
   */
private:
  /* TODO: this is temporary as a way to avoid dup code in
   * MemorySubSpaceMetronome::allocate. We will specifically fix this allocate
   * method in a separate design.
   */
  void *allocateMixedObjectOrArraylet(MM_EnvironmentBase *env,
  ...
  MMINLINE void expandRegionPool() {
    if (_regionExpansionBase != _regionExpansionTop) {
      _memoryPoolSegregated->getRegionPool()->addFreeRange(_regionExpansionBase,
                                                           _regionExpansionTop);
...
public:
  static MM_MemorySubSpaceSegregated *
  newInstance(MM_EnvironmentBase *env, MM_PhysicalSubArena *physicalSubArena,
              MM_MemoryPool *memoryPool, bool usesGlobalCollector,
  ...
  MM_MemorySubSpaceSegregated(MM_EnvironmentBase *env,
      ...
      : MM_MemorySubSpaceUniSpace(env, physicalSubArena, usesGlobalCollector,
                                  minimumSize, initialSize, maximumSize,
                                  MEMORY_TYPE_OLD, 0),
        _regionExpansionBase(NULL), _regionExpansionTop(NULL),
