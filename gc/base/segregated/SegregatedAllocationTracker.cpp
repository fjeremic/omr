...
MM_SegregatedAllocationTracker *MM_SegregatedAllocationTracker::newInstance(
    MM_EnvironmentBase *env, volatile uintptr_t *globalBytesInUse,
    uintptr_t flushThreshold) {
  MM_SegregatedAllocationTracker *allocationTracker;
  allocationTracker =
      (MM_SegregatedAllocationTracker *)env->getForge()->allocate(
          sizeof(MM_SegregatedAllocationTracker),
...
bool MM_SegregatedAllocationTracker::initialize(
    MM_EnvironmentBase *env, uintptr_t volatile *globalBytesInUse,
    uintptr_t flushThreshold) {
  _bytesAllocated = 0;
...
void MM_SegregatedAllocationTracker::tearDown(MM_EnvironmentBase *env) {
   ...
   * due to the allocation tracker flush threshold. It also updates the flush
...
void MM_SegregatedAllocationTracker::updateAllocationTrackerThreshold(
    MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  uintptr_t perThreadFlushThreshold =
      extensions->allocationTrackerMaxTotalError;

  if (extensions->currentEnvironmentCount > 0) {
    perThreadFlushThreshold /= extensions->currentEnvironmentCount;
  }

  extensions->allocationTrackerFlushThreshold = OMR_MIN(
      perThreadFlushThreshold, extensions->allocationTrackerMaxThreshold);
}

/**
 * Should be called once Xmx has been set in GCExtensions
 */
void MM_SegregatedAllocationTracker::initializeGlobalAllocationTrackerValues(
    MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();

  /* Only set the allocation tracker max total error if it hasn't been specified
   * on the command line */
  if (UDATA_MAX == extensions->allocationTrackerMaxTotalError) {
    extensions->allocationTrackerMaxTotalError =
        extensions->memoryMax / 100; /* 1% of -Xmx */
  }

  updateAllocationTrackerThreshold(env);
}

void MM_SegregatedAllocationTracker::addBytesAllocated(
...
void MM_SegregatedAllocationTracker::addBytesFreed(MM_EnvironmentBase *env,
                                                   uintptr_t bytesFreed) {
...
/**
 * Atomically adds this thread's bytes in use to the global memory pool's bytes
 * in use variable used to obtain the current free space approximation.
 */
