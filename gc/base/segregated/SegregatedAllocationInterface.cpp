 ...
 */
MM_SegregatedAllocationInterface* MM_SegregatedAllocationInterface::newInstance(MM_EnvironmentBase* env)
{
    MM_SegregatedAllocationInterface* allocationInterface;

    allocationInterface = (MM_SegregatedAllocationInterface*)env->getForge()->allocate(
        sizeof(MM_SegregatedAllocationInterface), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
void* MM_SegregatedAllocationInterface::allocateFromCache(MM_EnvironmentBase* env, uintptr_t sizeInBytes)
{
...
void* MM_SegregatedAllocationInterface::allocateObject(MM_EnvironmentBase* env,
    MM_AllocateDescription* allocateDescription, MM_MemorySpace* memorySpace, bool shouldCollectOnFailure)
{
    ...
    if (shouldCollectOnFailure) {
        ...
        /* Ensure we're allocating from the heap (not immortal or scopes) and that the allocation will be from a small
         * region. */
...
void* MM_SegregatedAllocationInterface::allocateArray(MM_EnvironmentBase* env,
    MM_AllocateDescription* allocateDescription, MM_MemorySpace* memorySpace, bool shouldCollectOnFailure)
{
    void* result = NULL;
 ...
 */
void* MM_SegregatedAllocationInterface::allocateArrayletSpine(MM_EnvironmentBase* env,
    MM_AllocateDescription* allocateDescription, MM_MemorySpace* memorySpace, bool shouldCollectOnFailure)
{
    void* result = NULL;
    MM_MemorySubSpace* subSpace = memorySpace->getDefaultMemorySubSpace();
 ...
 */
void* MM_SegregatedAllocationInterface::allocateArrayletLeaf(MM_EnvironmentBase* env,
    MM_AllocateDescription* allocateDescription, MM_MemorySpace* memorySpace, bool shouldCollectOnFailure)
{
    void* result = NULL;
    MM_MemorySubSpace* subSpace = memorySpace->getDefaultMemorySubSpace();
    ...
    for (uintptr_t sizeClass = 0; sizeClass < OMR_SIZECLASSES_NUM_SMALL + 1; sizeClass++) {
        if (_allocationCache[sizeClass].current < _allocationCache[sizeClass].top) {
            MM_HeapLinkedFreeHeader* chunk
                = MM_HeapLinkedFreeHeader::getHeapLinkedFreeHeader(_allocationCache[sizeClass].current);
...
void MM_SegregatedAllocationInterface::restartCache(MM_EnvironmentBase* env)
{
     ...
     * - if only 1 replenish occurred since the last restart (meaning we didn't use up the entire cache), halve the
     * replenish size
     * - if less cells were pre-allocated than the preceding replenish size (will happen if we need multiple refreshes
     * to reach the current replenish size because the AC has fairly full regions), halve the replenish size
     */
    for (uintptr_t sizeClass = OMR_SIZECLASSES_MIN_SMALL; sizeClass <= OMR_SIZECLASSES_MAX_SMALL; sizeClass++) {
        ...
        if (extensions->allocationCacheInitialSize != _replenishSizes[sizeClass]) {
            ...
            } else if (_allocationCacheStats.bytesPreAllocatedSinceRestart[sizeClass]
 ...
 */
void MM_SegregatedAllocationInterface::replenishCache(
    MM_EnvironmentBase* env, uintptr_t sizeInBytes, void* cacheMemory, uintptr_t cacheSize)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
...
uintptr_t MM_SegregatedAllocationInterface::getReplenishSize(MM_EnvironmentBase* env, uintptr_t sizeInBytes)
{
...
void MM_SegregatedAllocationInterface::enableCachedAllocations(MM_EnvironmentBase* env)
{
    if (!_cachedAllocationsEnabled) {
        _cachedAllocationsEnabled = true;
        restartCache(env);
...
void MM_SegregatedAllocationInterface::disableCachedAllocations(MM_EnvironmentBase* env)
{
    if (_cachedAllocationsEnabled) {
...
void MM_SegregatedAllocationInterface::updateFrequentObjectsStats(MM_EnvironmentBase* env, uintptr_t sizeClass)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    MM_AllocationContextSegregated* ac = (MM_AllocationContextSegregated*)env->getAllocationContext();
    ...
    if ((NULL != _frequentObjectsStats) && (NULL != base) && (NULL != top)) {
        ...
        GC_ObjectHeapIteratorSegregated objectHeapIterator(
            extensions, base, top, ac->_smallRegions[sizeClass]->getRegionType(), cellSize, false, false);
        omrobjectptr_t object = NULL;
        uintptr_t limit = (((uintptr_t)top - (uintptr_t)base) * extensions->frequentObjectAllocationSamplingRate) / 100
            + (uintptr_t)base;
