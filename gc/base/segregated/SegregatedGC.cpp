 ...
 * Initialization
 */
MM_SegregatedGC *MM_SegregatedGC::newInstance(MM_EnvironmentBase *env) {
  MM_SegregatedGC *globalGC;

  globalGC = (MM_SegregatedGC *)env->getForge()->allocate(
      sizeof(MM_SegregatedGC), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (NULL != globalGC) {
...
void MM_SegregatedGC::kill(MM_EnvironmentBase *env) {
  tearDown(env);
 ...
 */
bool MM_SegregatedGC::initialize(MM_EnvironmentBase *env) {
  ...
  _sweepScheme =
      MM_SweepSchemeSegregated::newInstance(env, _markingScheme->getMarkMap());
 ...
 */
void MM_SegregatedGC::tearDown(MM_EnvironmentBase *env) {
  if (NULL != _markingScheme) {
...
bool MM_SegregatedGC::heapAddRange(MM_EnvironmentBase *env,
                                   MM_MemorySubSpace *subspace, uintptr_t size,
                                   void *lowAddress, void *highAddress) {
  return _markingScheme->heapAddRange(env, subspace, size, lowAddress,
                                      highAddress);
}

bool MM_SegregatedGC::heapRemoveRange(MM_EnvironmentBase *env,
                                      MM_MemorySubSpace *subspace,
                                      uintptr_t size, void *lowAddress,
                                      void *highAddress, void *lowValidAddress,
                                      void *highValidAddress) {
  return _markingScheme->heapRemoveRange(env, subspace, size, lowAddress,
...
bool MM_SegregatedGC::collectorStartup(MM_GCExtensionsBase *extensions) {
  return true;
}

void MM_SegregatedGC::collectorShutdown(MM_GCExtensionsBase *extensions) {}

void *MM_SegregatedGC::createSweepPoolState(MM_EnvironmentBase *env,
                                            MM_MemoryPool *memoryPool) {
  /*
   *  This function does nothing and is designed to have implementation of
   * abstract in GlobalCollector, but must return non-NULL value to pass
   * initialization - so, return "this"
   */
  return this;
}
 ...
 */
void MM_SegregatedGC::deleteSweepPoolState(MM_EnvironmentBase *env,
                                           void *sweepPoolState) {}

void MM_SegregatedGC::setupForGC(MM_EnvironmentBase *) {}

void MM_SegregatedGC::abortCollection(MM_EnvironmentBase *env,
                                      CollectionAbortReason reason) {}

/*
 * Garbage Collection
 */
bool MM_SegregatedGC::internalGarbageCollect(
  ...
  MM_SegregatedSweepTask sweepTask(
      env, _dispatcher, _sweepScheme,
      (MM_MemoryPoolSegregated *)env->getDefaultMemorySubSpace()
          ->getMemoryPool());
  ...
  /* Perform the resize now based on expand/contract calculation from
   * checkResize() (above) */
  ...
  while (OMR_VMThread *thread = vmThreadListIterator.nextOMRVMThread()) {
    ...
    ((MM_SegregatedAllocationInterface *)(walkEnv->_objectAllocationInterface))
        ->restartCache(walkEnv);
  }

  return true;
}

void MM_SegregatedGC::internalPreCollect(
    MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
    MM_AllocateDescription *allocDescription, uint32_t gcCode) {
  /* MM_GlobalCollector::internalPreCollect(env, subSpace, allocDescription,
   * gcCode); */
...
void MM_SegregatedGC::internalPostCollect(MM_EnvironmentBase *env,
                                          MM_MemorySubSpace *subSpace) {
  ...
  /* OMRTODO dynamically set the minimum free entry size. See realtime gc for
   * reference */
 ...
 * Reporting
 */
void MM_SegregatedGC::reportGCCycleFinalIncrementEnding(
...
void MM_SegregatedGC::reportGCCycleStart(MM_EnvironmentBase *env) {
...
void MM_SegregatedGC::reportGCCycleEnd(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  MM_CommonGCData commonData;
...
void MM_SegregatedGC::reportMarkStart(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_SegregatedGC::reportMarkEnd(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_SegregatedGC::reportSweepStart(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_SegregatedGC::reportSweepEnd(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_SegregatedGC::reportGCStart(MM_EnvironmentBase *env) {
...
void MM_SegregatedGC::reportGCEnd(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_SegregatedGC::reportGCIncrementStart(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_SegregatedGC::reportGCIncrementEnd(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  MM_CollectionStatisticsStandard *stats =
      (MM_CollectionStatisticsStandard *)
          env->_cycleState->_collectionStatistics;
