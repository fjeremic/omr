...
public:
    ...
    {
        uintptr_t pessimisticBytesInUse = _bytesInUse
            + OMR_MIN(_extensions->allocationTrackerMaxTotalError,
                (_extensions->allocationTrackerMaxThreshold * _extensions->currentEnvironmentCount));
        uintptr_t activeMemorySize = _extensions->getHeap()->getActiveMemorySize();

        /* It's possible that our pessimistic bytes in use approximation goes over the heap size since we assume all
         * threads are on the verge of flushing.
         */
    ...
    static MM_MemoryPoolSegregated* newInstance(MM_EnvironmentBase* env, MM_RegionPoolSegregated* regionPool,
    ...
    virtual void* allocateTLH(
        MM_EnvironmentBase* env, uintptr_t maxSizeInBytesRequired, void*& addrBase, void*& addrTop);
    ...
    virtual void expandWithRange(
        MM_EnvironmentBase* env, uintptr_t expandSize, void* lowAddr, void* highAddr, bool canCoalesce);
...
protected:
    ...
    MM_MemoryPoolSegregated(MM_EnvironmentBase* env, MM_RegionPoolSegregated* regionPool,
        uintptr_t minimumFreeEntrySize, MM_GlobalAllocationManagerSegregated* globalAllocationManager)
