...
public:
  ...
  uintptr_t getBytesInUse() {
    uintptr_t pessimisticBytesInUse =
        _bytesInUse + OMR_MIN(_extensions->allocationTrackerMaxTotalError,
                              (_extensions->allocationTrackerMaxThreshold *
                               _extensions->currentEnvironmentCount));
    uintptr_t activeMemorySize = _extensions->getHeap()->getActiveMemorySize();

    /* It's possible that our pessimistic bytes in use approximation goes over
     * the heap size since we assume all threads are on the verge of flushing.
     */
  ...
  static MM_MemoryPoolSegregated *
  ...
  createAllocationTracker(MM_EnvironmentBase *env);
  ...
  virtual void *allocateTLH(MM_EnvironmentBase *env,
                            uintptr_t maxSizeInBytesRequired, void *&addrBase,
                            void *&addrTop);
#endif /* OMR_GC_THREAD_LOCAL_HEAP */

  virtual void reset(Cause cause = any);

  virtual void addRange(MM_EnvironmentBase *env, void *prevFreeEntry,
                        uintptr_t prevFreeEntrySize, void *currentFreeEntry,
                        uintptr_t currentFreeEntrySize);
  virtual void insertRange(MM_EnvironmentBase *env, void *prevFreeListEntry,
                           uintptr_t prevFreeListEntrySize,
                           void *expandRangeBase, void *expandRangeTop,
                           void *nextFreeListEntry,
                           uintptr_t nextFreeListEntrySize);
  virtual void buildRange(MM_EnvironmentBase *env, void *expandRangeBase,
                          void *expandRangeTop);
  virtual void expandWithRange(MM_EnvironmentBase *env, uintptr_t expandSize,
                               void *lowAddr, void *highAddr, bool canCoalesce);
  virtual void *contractWithRange(MM_EnvironmentBase *env, uintptr_t expandSize,
                                  void *lowAddr, void *highAddr);

  virtual bool abandonHeapChunk(void *addrBase, void *addrTop);

  MMINLINE uintptr_t verbose(MM_EnvironmentBase *env) {
    return _extensions->verbose;
  }
  MMINLINE uintptr_t debug(MM_EnvironmentBase *env) {
    return _extensions->debug;
  }

  void moveInUseToSweep(MM_EnvironmentBase *env);
  void flushCachedFullRegions(MM_EnvironmentBase *env);

  uintptr_t getUsedAndLiveObjectsSize(MM_EnvironmentBase *env,
                                      bool unmark_objects,
                                      bool unmark_live_alloc,
                                      uintptr_t *allocGarbage, uintptr_t *live,
                                      uintptr_t *allocLive);
...
protected:
  ...
  MM_MemoryPoolSegregated(
      ...
      uintptr_t minimumFreeEntrySize,
      MM_GlobalAllocationManagerSegregated *globalAllocationManager)
      : MM_MemoryPool(env, minimumFreeEntrySize), _regionPool(regionPool),
        _globalAllocationManager(globalAllocationManager), _extensions(NULL),
        _bytesInUse(0) {
    _typeId = __FUNCTION__;
  }

private:
  uintptr_t *allocateContiguous(MM_EnvironmentBase *env,
                                MM_AllocateDescription *allocDesc,
                                MM_AllocationContextSegregated *ac);
  void *allocateChunkedArray(MM_EnvironmentBase *env,
                             MM_AllocateDescription *allocDesc,
                             MM_AllocationContextSegregated *ac);
};
