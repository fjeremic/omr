 ...
 */
MM_OverflowSegregated* MM_OverflowSegregated::newInstance(MM_EnvironmentBase* env, MM_WorkPackets* workPackets)
{
    Assert_MM_true(env->getExtensions()->isSegregatedHeap());
    MM_OverflowSegregated* overflow = (MM_OverflowSegregated*)env->getForge()->allocate(
        sizeof(MM_OverflowSegregated), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (NULL != overflow) {
...
void MM_OverflowSegregated::emptyToOverflow(MM_EnvironmentBase* env, MM_Packet* packet, MM_OverflowType type)
{
    void* objectPtr;
...
void MM_OverflowSegregated::overflowItem(MM_EnvironmentBase* env, void* item, MM_OverflowType type)
{
    _overflow = true;
...
void MM_OverflowSegregated::overflowItemInternal(MM_EnvironmentBase* env, void* item)
{
...
void MM_OverflowSegregated::fillFromOverflow(MM_EnvironmentBase* env, MM_Packet* packet)
{
...
void MM_OverflowSegregated::handleOverflow(MM_EnvironmentBase* env)
{
    if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
        ...
        while ((region = (MM_HeapRegionDescriptorSegregated*)regionIterator.nextRegion()) != NULL) {
            GC_ObjectHeapIteratorSegregated objectIterator(_extensions, (omrobjectptr_t)region->getLowAddress(),
                (omrobjectptr_t)region->getHighAddress(), region->getRegionType(), region->getCellSize(), false, false);
            ...
            while ((object = objectIterator.nextObject()) != NULL) {
                ...
                if ((markMap->isBitSet(object))
                    ...
                    /* The subclassing of markingscheme by segregatedmarking scheme needs a lot of work and means
                     * metronome is pretty broken */
