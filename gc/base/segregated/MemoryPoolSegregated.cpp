 ...
 */
MM_MemoryPoolSegregated *MM_MemoryPoolSegregated::newInstance(
  ...
  MM_MemoryPoolSegregated *memoryPool =
      (MM_MemoryPoolSegregated *)env->getForge()->allocate(
          sizeof(MM_MemoryPoolSegregated), OMR::GC::AllocationCategory::FIXED,
          OMR_GET_CALLSITE());
  if (memoryPool) {
    memoryPool = new (memoryPool)
        MM_MemoryPoolSegregated(env, regionPool, minimumFreeEntrySize, gam);
 ...
 * Initialize MPS
 */
bool MM_MemoryPoolSegregated::initialize(MM_EnvironmentBase *env) {
 ...
 */
void MM_MemoryPoolSegregated::tearDown(MM_EnvironmentBase *env) {
  if (NULL != _regionPool) {
...
void MM_MemoryPoolSegregated::flushCachedFullRegions(MM_EnvironmentBase *env) {
  /* delegate to GAM to perform the flushing of per-context full regions to the
   * region pool */
  _globalAllocationManager->flushCachedFullRegions(env);
}

void MM_MemoryPoolSegregated::moveInUseToSweep(MM_EnvironmentBase *env) {
...
void *MM_MemoryPoolSegregated::allocateChunkedArray(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDesc,
  ...
  if (spine) {
    ...
    fomrobject_t *arrayoidPtr =
        _extensions->indexableObjectModel.getArrayoidPointer(spine);
    ...
    for (uintptr_t i = 0; i < numberArraylets; i++) {
      ...
      if (0 < bytesRemaining) {
        ...
        if (arraylet == NULL) {
          ...
          if (region->isSmall()) {
             ...
             * Fortunately, it is legal to simply let the space be reclaimed at
             * the next GC. _regionPool->addFreeRegion(env, region); */
          }
          return NULL;
        }
      } else {
        /* There can be one extra arraylet leaf pointer at the end, which we set
         * to NULL. This allows us to calculate an address for the after-last
         * element without reading beyond allocated memory. This is required for
         * things like arraycopy.
         */
      ...
      bytesRemaining =
          MM_Math::saturatingSubtract(bytesRemaining, arrayletLeafSize);
    }
  }
  return spine;
}
 ...
 */
void *MM_MemoryPoolSegregated::allocateArrayletLeaf(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDesc) {
  MM_AllocationContextSegregated *allocationContext =
      (MM_AllocationContextSegregated *)env->getAllocationContext();
  /* we must have already allocated the parent spine, which is stored in the
   * AllocateDescriptionCore */
 ...
 */
void *
MM_MemoryPoolSegregated::allocateObject(MM_EnvironmentBase *env,
...
uintptr_t *MM_MemoryPoolSegregated::allocateContiguous(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDesc,
    MM_AllocationContextSegregated *ac) {
  const uintptr_t sizeInBytesRequired = allocDesc->getContiguousBytes();
  const uintptr_t sizeClass =
      _extensions->defaultSizeClasses->getSizeClass(sizeInBytesRequired);
  uintptr_t *result = NULL;

  if (sizeClass == OMR_SIZECLASSES_LARGE) {
    /* allocating large also goes through AC so that AC can see and cache the
     * large full page */
    result = ac->allocateLarge(env, sizeInBytesRequired);
  } else {
    result =
        (uintptr_t
             *)(MM_SegregatedAllocationInterface::getObjectAllocationInterface(
                    env)
 ...
 */
void *MM_MemoryPoolSegregated::allocateTLH(MM_EnvironmentBase *env,
...
void MM_MemoryPoolSegregated::reset(Cause cause) { Assert_MM_unreachable(); }

void MM_MemoryPoolSegregated::addRange(MM_EnvironmentBase *env,
                                       void *previousFreeEntry,
                                       uintptr_t previousFreeEntrySize,
...
void MM_MemoryPoolSegregated::insertRange(MM_EnvironmentBase *env,
                                          void *previousFreeListEntry,
                                          uintptr_t previousFreeListEntrySize,
                                          void *expandRangeBase,
                                          void *expandRangeTop,
                                          void *nextFreeListEntry,
                                          uintptr_t nextFreeListEntrySize) {
  Assert_MM_unreachable();
}

void *MM_MemoryPoolSegregated::contractWithRange(MM_EnvironmentBase *env,
...
void MM_MemoryPoolSegregated::expandWithRange(MM_EnvironmentBase *env,
                                              uintptr_t expandSize, void *base,
                                              void *top, bool canCoalesce) {
  Assert_MM_unreachable();
}

void MM_MemoryPoolSegregated::buildRange(MM_EnvironmentBase *env,
                                         void *expandRangeBase,
                                         void *expandRangeTop) {
  abandonHeapChunk(expandRangeBase, expandRangeTop);
}

bool MM_MemoryPoolSegregated::abandonHeapChunk(void *addrBase, void *addrTop) {
  Assert_MM_unreachable();
  return false;
}

uintptr_t MM_MemoryPoolSegregated::getApproximateFreeMemorySize() {
  return (_extensions->getHeap()->getHeapRegionManager()->getHeapSize() -
          getBytesInUse());
}

uintptr_t MM_MemoryPoolSegregated::getApproximateActiveFreeMemorySize() {
  return (_extensions->getHeap()->getActiveMemorySize() - getBytesInUse());
}

uintptr_t MM_MemoryPoolSegregated::getActualFreeMemorySize() {
  uintptr_t single = 0;
  uintptr_t multi = 0;
  ...
  return (single + multi + coalesce) *
         _extensions->getHeap()->getHeapRegionManager()->getRegionSize();
}

uintptr_t MM_MemoryPoolSegregated::getActualFreeEntryCount() {
  Assert_MM_unreachable();
  return 0;
}
 ...
 */
uintptr_t MM_MemoryPoolSegregated::debugGetActualFreeMemorySize() {
  ...
  while (NULL != (walkThread = vmThreadListIterator.nextOMRVMThread())) {
    ...
    totalBytesInUse +=
        walkEnv->_allocationTracker->getUnflushedBytesAllocated(walkEnv);
  }

  return totalBytesInUse;
}

void MM_MemoryPoolSegregated::setFreeMemorySize(uintptr_t freeMemorySize) {}

void MM_MemoryPoolSegregated::setFreeEntryCount(uintptr_t entryCount) {}
