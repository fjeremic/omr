 ...
 */
MM_MemoryPoolSegregated* MM_MemoryPoolSegregated::newInstance(MM_EnvironmentBase* env,
    MM_RegionPoolSegregated* regionPool, uintptr_t minimumFreeEntrySize, MM_GlobalAllocationManagerSegregated* gam)
{
...
MM_SegregatedAllocationTracker* MM_MemoryPoolSegregated::createAllocationTracker(MM_EnvironmentBase* env)
{
                ...
                if (arraylet == NULL) {
                    ...
                    for (uintptr_t j = 0; j < i; j++) {
                        ...
                        MM_HeapRegionDescriptorSegregated* region
                            = (MM_HeapRegionDescriptorSegregated*)regionManager->tableDescriptorForAddress(arraylet);
                        region->clearArraylet(region->whichArraylet(arraylet, arrayletLeafLogSize));
                        /* Arraylet backout means arraylets may be re-used before the next cycle, so we need to correct
                         * for their un-allocation
                         */
                        region->addBytesFreedToArrayletBackout(env);
                    }
                    MM_HeapRegionDescriptorSegregated* region
                        = (MM_HeapRegionDescriptorSegregated*)regionManager->tableDescriptorForAddress(
                            (uintptr_t*)spine);
                    if (region->isSmall()) {
                        region->getMemoryPoolACL()->returnCell(env, (uintptr_t*)spine);
                        /* Small spine backout means the cell may be re-used before the next cycle, so we need to
                         * correct for its un-allocation
                         */
 ...
 */
void* MM_MemoryPoolSegregated::allocateArrayletLeaf(MM_EnvironmentBase* env, MM_AllocateDescription* allocDesc)
{
 ...
 */
void* MM_MemoryPoolSegregated::allocateObject(MM_EnvironmentBase* env, MM_AllocateDescription* allocDesc)
{
    void* result = NULL;
    MM_AllocationContextSegregated* allocationContext = (MM_AllocationContextSegregated*)env->getAllocationContext();
    ...
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDesc, MM_AllocationContextSegregated* ac)
{
    ...
    if (sizeClass == OMR_SIZECLASSES_LARGE) {
        ...
        result = (uintptr_t*)(MM_SegregatedAllocationInterface::getObjectAllocationInterface(env)->allocateFromCache(
 ...
 */
void* MM_MemoryPoolSegregated::allocateTLH(
...
void MM_MemoryPoolSegregated::addRange(MM_EnvironmentBase* env, void* previousFreeEntry,
    uintptr_t previousFreeEntrySize, void* currentFreeEntry, uintptr_t currentFreeEntrySize)
{
    /* Used by the sweep routine to add free chunks to the list - ignored here */
    return;
}

void MM_MemoryPoolSegregated::insertRange(MM_EnvironmentBase* env, void* previousFreeListEntry,
...
void* MM_MemoryPoolSegregated::contractWithRange(
...
void MM_MemoryPoolSegregated::expandWithRange(
    MM_EnvironmentBase* env, uintptr_t expandSize, void* base, void* top, bool canCoalesce)
{
    Assert_MM_unreachable();
}

void MM_MemoryPoolSegregated::buildRange(MM_EnvironmentBase* env, void* expandRangeBase, void* expandRangeTop)
{
