...
class MM_GlobalAllocationManagerSegregated : public MM_GlobalAllocationManager {
 ...
 protected:
  ...
  /**
   * One time initialization of the allocation contexts, occurs after GAM
   * initialization since it's called by the MPS immediately after the region
   * table is created.
   */
  virtual bool initializeAllocationContexts(
      MM_EnvironmentBase *env, MM_RegionPoolSegregated *regionPool);

 public:
  static MM_GlobalAllocationManagerSegregated *newInstance(
      MM_EnvironmentBase *env, MM_RegionPoolSegregated *regionPool);
  ...
  virtual void printAllocationContextStats(
      MM_EnvironmentBase *env, uintptr_t eventNum,
      J9HookInterface **hookInterface) { /* no impl */
  }
