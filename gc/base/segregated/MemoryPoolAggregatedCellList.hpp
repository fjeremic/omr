...
private:
protected:
    MM_HeapLinkedFreeHeader* volatile _freeListHead; /**< Pointer to the first entry in the free list, NULL means there
                                                        are no free entries in this region */
...
public:
    ...
    void expandWithRange(
        MM_EnvironmentBase* env, uintptr_t expandSize, void* lowAddress, void* highAddress, bool canCoalesce)
    {}
    void* contractWithRange(MM_EnvironmentBase* env, uintptr_t contractSize, void* lowAddress, void* highAddress)
    {
        return NULL;
    }
    bool abandonHeapChunk(void* addrBase, void* addrTop) { return true; }

    MMINLINE void addFreeChunk(uintptr_t* freeChunk, uintptr_t freeChunkSize, uintptr_t freeChunkCellCount)
    {
    ...
    uintptr_t* preAllocateCells(
        MM_EnvironmentBase* env, uintptr_t cellSize, uintptr_t desiredBytes, uintptr_t* preAllocatedBytesOutput);
