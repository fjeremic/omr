...
protected:
  MM_HeapLinkedFreeHeader
      *volatile _freeListHead; /**< Pointer to the first entry in the free list,
                                  NULL means there are no free entries in this
                                  region */
  uintptr_t *volatile _heapCurrent;
  uintptr_t *volatile _heapTop;

  MM_LightweightNonReentrantLock
      _lock; /**< Lock used to ensure cell list is maintained safely */
  MM_HeapRegionDescriptorSegregated *_region;

  /* _freeCount + _markCount + "dark matter" = number of cells on region after
   * the sweep */
...
protected:
public:
  MM_MemoryPoolAggregatedCellList(MM_EnvironmentBase *env,
                                  uintptr_t minimumFreeEntrySize)
      : MM_MemoryPool(env, minimumFreeEntrySize), _freeListHead(NULL),
        _heapCurrent(NULL), _heapTop(NULL), _region(NULL), _markCount(0),
        _freeCount(0), _preSweepFreeBytes(0) {
  ...
  void tearDown(MM_EnvironmentBase *env) { _lock.tearDown(); }

  void *allocateObject(MM_EnvironmentBase *env,
                       MM_AllocateDescription *allocDescription) {
    return NULL;
  }
  void expandWithRange(MM_EnvironmentBase *env, uintptr_t expandSize,
                       void *lowAddress, void *highAddress, bool canCoalesce) {}
  void *contractWithRange(MM_EnvironmentBase *env, uintptr_t contractSize,
                          void *lowAddress, void *highAddress) {
    return NULL;
  }
  bool abandonHeapChunk(void *addrBase, void *addrTop) { return true; }

  MMINLINE void addFreeChunk(uintptr_t *freeChunk, uintptr_t freeChunkSize,
                             uintptr_t freeChunkCellCount) {
    MM_HeapLinkedFreeHeader *chunk =
        MM_HeapLinkedFreeHeader::getHeapLinkedFreeHeader(freeChunk);
  ...
  MMINLINE bool hasCell() {
    return (_freeListHead != NULL) || (_heapCurrent < _heapTop);
  }
  uintptr_t *preAllocateCells(MM_EnvironmentBase *env, uintptr_t cellSize,
                              uintptr_t desiredBytes,
                              uintptr_t *preAllocatedBytesOutput);
  ...
  MMINLINE void setPreSweepFreeBytes(uintptr_t preSweepFreeBytes) {
    _preSweepFreeBytes = preSweepFreeBytes;
  }
  MMINLINE void addSweepFreeBytes(MM_EnvironmentBase *env,
                                  uintptr_t sweepFreeBytes) {
  ...
  MMINLINE void refreshCurrentEntry() {
    ...
    if (NULL != _freeListHead) {
      ...
      _heapTop =
          (uintptr_t *)((uintptr_t)_heapCurrent + _freeListHead->getSize());
  ...
  uintptr_t reset(MM_EnvironmentBase *env, uintptr_t sizeClass,
                  void *lowAddress);
  void setRegion(MM_HeapRegionDescriptorSegregated *region) {
    _region = region;
  }
};
