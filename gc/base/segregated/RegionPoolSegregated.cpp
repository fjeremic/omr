...
bool MM_RegionPoolSegregated::initialize(MM_EnvironmentBase* env)
{
    ...
    for (szClass = OMR_SIZECLASSES_MIN_SMALL; szClass <= OMR_SIZECLASSES_MAX_SMALL; szClass++) {
        for (int32_t i = 0; i < NUM_DEFRAG_BUCKETS; i++) {
            ...
            for (uintptr_t j = 0; j < _splitAvailableListSplitCount; j++) {
                ...
                new (&regionQueue[j])
                    MM_LockingHeapRegionQueue(MM_HeapRegionList::HRL_KIND_AVAILABLE, true, true, true);
    ...
    _arrayletFullRegions
        = MM_RegionPoolSegregated::allocateHeapRegionQueue(env, MM_HeapRegionList::HRL_KIND_FULL, true, true, false);
    _arrayletSweepRegions
        = MM_RegionPoolSegregated::allocateHeapRegionQueue(env, MM_HeapRegionList::HRL_KIND_SWEEP, true, true, false);
    ...
    _largeSweepRegions
        = MM_RegionPoolSegregated::allocateHeapRegionQueue(env, MM_HeapRegionList::HRL_KIND_SWEEP, false, true, false);
...
MM_HeapRegionQueue* MM_RegionPoolSegregated::allocateHeapRegionQueue(MM_EnvironmentBase* env,
...
void MM_RegionPoolSegregated::addSingleFree(MM_EnvironmentBase* env, MM_HeapRegionQueue* regionQueue)
{
...
void MM_RegionPoolSegregated::countFreeRegions(uintptr_t* singleFree, uintptr_t* multiFree, uintptr_t* coalesceFree)
{
...
void MM_RegionPoolSegregated::addFreeRange(void* lowAddress, void* highAddress)
{
    ...
    Assert_MM_true(
        0 == range || (lowAddress == firstInRange->getLowAddress() && highAddress == firstInRange->getHighAddress()));
}
...
MM_HeapRegionDescriptorSegregated* MM_RegionPoolSegregated::allocateFromRegionPool(
...
void MM_RegionPoolSegregated::joinBucketListsForSplitIndex(MM_EnvironmentBase* env)
{
 ...
 */
MM_HeapRegionDescriptorSegregated* MM_RegionPoolSegregated::allocateRegionFromSmallSizeClass(
 ...
 */
MM_HeapRegionDescriptorSegregated* MM_RegionPoolSegregated::allocateRegionFromArrayletSizeClass(MM_EnvironmentBase* env)
{
 ...
 */
MM_HeapRegionDescriptorSegregated* MM_RegionPoolSegregated::sweepAndAllocateRegionFromSmallSizeClass(
    MM_EnvironmentBase* env, uintptr_t sizeClass)
{
...
void MM_RegionPoolSegregated::updateOccupancy(uintptr_t sizeClass, uintptr_t occupancy)
{
