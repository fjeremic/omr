 ...
 */
MM_RegionPoolSegregated *MM_RegionPoolSegregated::newInstance(
    MM_EnvironmentBase *env, MM_HeapRegionManager *heapRegionManager) {
  MM_RegionPoolSegregated *regionPool;

  regionPool = (MM_RegionPoolSegregated *)env->getForge()->allocate(
      sizeof(MM_RegionPoolSegregated), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (NULL != regionPool) {
    regionPool =
        new (regionPool) MM_RegionPoolSegregated(env, heapRegionManager);
 ...
 * Initialization.
 */
bool MM_RegionPoolSegregated::initialize(MM_EnvironmentBase *env) {
  uint32_t szClass;
  ...
  _singleFreeList = MM_RegionPoolSegregated::allocateFreeHeapRegionList(
      env, MM_HeapRegionList::HRL_KIND_FREE, true);
  _multiFreeList = MM_RegionPoolSegregated::allocateFreeHeapRegionList(
      env, MM_HeapRegionList::HRL_KIND_MULTI_FREE, false);
  _coalesceFreeList = MM_RegionPoolSegregated::allocateFreeHeapRegionList(
      env, MM_HeapRegionList::HRL_KIND_COALESCE, false);
  if ((_singleFreeList == NULL) || (_multiFreeList == NULL) ||
      (_coalesceFreeList == NULL)) {
    return false;
  }
  _splitAvailableListSplitCount =
      env->getExtensions()->splitAvailableListSplitAmount;
  Assert_MM_true(0 < _splitAvailableListSplitCount);
  for (szClass = OMR_SIZECLASSES_MIN_SMALL;
       szClass <= OMR_SIZECLASSES_MAX_SMALL; szClass++) {
    for (int32_t i = 0; i < NUM_DEFRAG_BUCKETS; i++) {
      uintptr_t splitAvailableListsSize =
          sizeof(MM_LockingHeapRegionQueue) * _splitAvailableListSplitCount;
      _smallAvailableRegions[szClass][i] =
          (MM_LockingHeapRegionQueue *)env->getForge()->allocate(
              splitAvailableListsSize, OMR::GC::AllocationCategory::FIXED,
              OMR_GET_CALLSITE());
      if (NULL == _smallAvailableRegions[szClass][i]) {
        return false;
      }
      MM_LockingHeapRegionQueue *regionQueue =
          _smallAvailableRegions[szClass][i];
      for (uintptr_t j = 0; j < _splitAvailableListSplitCount; j++) {
        /* The available lists should track the free bytes in their regions (4th
         * param = true) */
    ...
    _smallFullRegions[szClass] =
        MM_RegionPoolSegregated::allocateHeapRegionQueue(
            env, MM_HeapRegionList::HRL_KIND_FULL, true, true, false);
    _smallSweepRegions[szClass] =
        MM_RegionPoolSegregated::allocateHeapRegionQueue(
            env, MM_HeapRegionList::HRL_KIND_SWEEP, true, true, false);
    if (NULL == _smallFullRegions[szClass] ||
        NULL == _smallSweepRegions[szClass]) {
      return false;
    }
    _smallOccupancy[szClass] = 0.5;
  }

  /* The available lists should track the free bytes in their regions (4th param
   * = true) */
  ...
  _arrayletSweepRegions = MM_RegionPoolSegregated::allocateHeapRegionQueue(
      env, MM_HeapRegionList::HRL_KIND_SWEEP, true, true, false);
  if ((NULL == _arrayletAvailableRegions) || (NULL == _arrayletFullRegions) ||
      (NULL == _arrayletSweepRegions)) {
    return false;
  }
...
MM_HeapRegionQueue *MM_RegionPoolSegregated::allocateHeapRegionQueue(
...
MM_FreeHeapRegionList *MM_RegionPoolSegregated::allocateFreeHeapRegionList(
    MM_EnvironmentBase *env, MM_HeapRegionList::RegionListKind regionListKind,
    bool singleRegionsOnly) {
  return MM_LockingFreeHeapRegionList::newInstance(env, regionListKind,
                                                   singleRegionsOnly);
}

void MM_RegionPoolSegregated::tearDown(MM_EnvironmentBase *env) {
  if (_singleFreeList) {
  ...
  for (int32_t szClass = OMR_SIZECLASSES_MIN_SMALL;
       szClass <= OMR_SIZECLASSES_MAX_SMALL; szClass++) {
    for (uintptr_t i = 0; i < NUM_DEFRAG_BUCKETS; i++) {
      MM_LockingHeapRegionQueue *regionQueueArray =
          _smallAvailableRegions[szClass][i];
...
void MM_RegionPoolSegregated::addSingleFree(MM_EnvironmentBase *env,
                                            MM_HeapRegionQueue *regionQueue) {
  uintptr_t returnedRegions = regionQueue->length();
  decrementRegionsInUse(returnedRegions);
  _singleFreeList->push(regionQueue);
}

void MM_RegionPoolSegregated::moveInUseToSweep(MM_EnvironmentBase *env) {
  _currentTotalCountOfSweepRegions = 0;
  for (int32_t sizeClass = OMR_SIZECLASSES_MIN_SMALL;
...
void MM_RegionPoolSegregated::countFreeRegions(uintptr_t *singleFree,
                                               uintptr_t *multiFree,
...
/* enqueue the region using the split region list indexed by splitListIndex for
 * size class sizeClass */
void MM_RegionPoolSegregated::enqueueAvailable(
    MM_HeapRegionDescriptorSegregated *region, uintptr_t sizeClass,
    uintptr_t occupancy, uintptr_t splitListIndex) {
  for (int32_t i = 0; i < NUM_DEFRAG_BUCKETS; i++) {
    if (occupancy >= defragBucketThresholds[i]) {
      (&(_smallAvailableRegions[sizeClass][i])[splitListIndex])
          ->enqueue(region);
      break;
    }
  }
...
void MM_RegionPoolSegregated::addFreeRange(void *lowAddress,
                                           void *highAddress) {
  MM_HeapRegionDescriptorSegregated *firstInRange =
      (MM_HeapRegionDescriptorSegregated *)
          _heapRegionManager->tableDescriptorForAddress(lowAddress);
  uintptr_t range = ((uintptr_t)highAddress - (uintptr_t)lowAddress) /
                    firstInRange->getSize();
  ...
  Assert_MM_true(0 == range || (lowAddress == firstInRange->getLowAddress() &&
                                highAddress == firstInRange->getHighAddress()));
}

void MM_RegionPoolSegregated::addFreeRegion(
    MM_EnvironmentBase *env, MM_HeapRegionDescriptorSegregated *region,
    bool alreadyFree) {
...
MM_HeapRegionDescriptorSegregated *
MM_RegionPoolSegregated::allocateFromRegionPool(MM_EnvironmentBase *env,
  ...
  if (region != NULL) {
    incrementRegionsInUse(region->getRange()); /* we must add here because we
                                                  will return remainder later */

    /* We must notify the allocation tracker that a fresh region has been
     * allocated, it will know how to account for bytes lost to internal
     * fragmentation and will account for all the memory allocated in the case
     * of large region allocation.
     */
...
void MM_RegionPoolSegregated::joinBucketListsForSplitIndex(
  ...
  for (int32_t sizeClass = OMR_SIZECLASSES_MIN_SMALL;
    ...
    for (int32_t i = 1; i < NUM_DEFRAG_BUCKETS; i++) {
      primaryQueue->enqueue(
          &(_smallAvailableRegions[sizeClass][i])[splitIndex]);
    }
  }
}
 ...
 */
MM_HeapRegionDescriptorSegregated *
MM_RegionPoolSegregated::allocateRegionFromSmallSizeClass(
    MM_EnvironmentBase *env, uintptr_t sizeClass) {
  MM_HeapRegionDescriptorSegregated *region = NULL;
  /* skip searching the available queue for this size class if we've already
   * exhausted all available regions */
  if (SKIP_AVAILABLE_REGION_FOR_ALLOCATION ==
      _skipAvailableRegionForAllocation[sizeClass]) {
    return region;
  }

  /* try bucket 0, i.e. primary bucket first */
  uintptr_t startList = env->getEnvironmentId() % _splitAvailableListSplitCount;
  MM_LockingHeapRegionQueue *primaryQueueArray =
      _smallAvailableRegions[sizeClass][PRIMARY_BUCKET];
  ...
  /* if primary bucket fails, try the other split queues, starting from the
   * current thread's split index */
  for (uintptr_t j = startList + 1;
       j < startList + _splitAvailableListSplitCount; j++) {
  ...
  if (_isSweepingSmall) {
    for (int32_t i = 1; i < NUM_DEFRAG_BUCKETS; i++) {
      MM_LockingHeapRegionQueue *queueArray =
          _smallAvailableRegions[sizeClass][i];
      for (uintptr_t j = startList;
           j < startList + _splitAvailableListSplitCount; j++) {
    ...
    _skipAvailableRegionForAllocation[sizeClass] =
        SKIP_AVAILABLE_REGION_FOR_ALLOCATION;
  }
  return region;
}
 ...
 */
MM_HeapRegionDescriptorSegregated *
MM_RegionPoolSegregated::allocateRegionFromArrayletSizeClass(
    MM_EnvironmentBase *env) {
  MM_HeapRegionDescriptorSegregated *region =
      _arrayletAvailableRegions->dequeue();
  if (region != NULL) {
    return region;
  }
  return NULL;
 ...
 */
MM_HeapRegionDescriptorSegregated *
MM_RegionPoolSegregated::sweepAndAllocateRegionFromSmallSizeClass(
    MM_EnvironmentBase *env, uintptr_t sizeClass) {
  MM_HeapRegionDescriptorSegregated *region =
      _smallSweepRegions[sizeClass]->dequeue();

  if (region != NULL) {
    _sweepScheme->sweepRegion(env, region);
    /* Keep maintaining the occupancy info even while doing nondeterministic
     * sweeps */
    _smallOccupancy[sizeClass] = (_smallOccupancy[sizeClass] * 0.9f) +
                                 (region->getMemoryPoolACL()->getMarkCount() /
                                  region->getNumCells() * 0.1f);
...
void MM_RegionPoolSegregated::updateOccupancy(uintptr_t sizeClass,
                                              uintptr_t occupancy) {
  assume(occupancy <= 100 && occupancy >= 0, "invalid occupancy estimate");
  _smallOccupancy[sizeClass] =
      _smallOccupancy[sizeClass] * 0.9f + occupancy * 0.001f;
}
