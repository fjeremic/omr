...
private:
  ...
  MM_HeapRegionDescriptorSegregated
      *_prev; /**< used by RegionList implementations to maintain links */
  MM_HeapRegionDescriptorSegregated
      *_next; /**< used by RegionList implementations to maintain links */
   ...
   */
public:
  MM_HeapRegionDescriptorSegregated(MM_EnvironmentBase *env, void *lowAddress,
                                    void *highAddress)
      : MM_HeapRegionDescriptor(env, lowAddress, highAddress), _sizeClass(0),
        _memoryPoolACL(env, 0 /* minimumFreeEntrySize */), _prev(NULL),
        _next(NULL), _regionManager(NULL),
  ...
  MMINLINE void setNext(MM_HeapRegionDescriptorSegregated *next) {
    _next = next;
  }
  MMINLINE MM_HeapRegionDescriptorSegregated *getPrev() { return _prev; }
  MMINLINE void setPrev(MM_HeapRegionDescriptorSegregated *prev) {
    _prev = prev;
  }
  ...
  bool isCanonical() {
    return isReserved() || isSmall() || getRangeCount() >= 1;
  }
  ...
  MM_HeapRegionDescriptorSegregated *getRangeHead() {
    return (MM_HeapRegionDescriptorSegregated *)_headOfSpan;
  }
  void setRangeHead(MM_HeapRegionDescriptorSegregated *head) {
    _headOfSpan = head;
  }
  uintptr_t getRangeCount() { return _regionsInSpan; }
  void setRangeCount(uintptr_t count) { _regionsInSpan = count; }

  uintptr_t formatFresh(MM_EnvironmentBase *env, uintptr_t sizeClass,
                        void *lowAddress);
  void emptyRegionAllocated(MM_EnvironmentBase *env);
  void emptyRegionReturned(MM_EnvironmentBase *env);

  uintptr_t *allocateArraylet(MM_EnvironmentBase *env,
                              omrarrayptr_t parentIndexableObject);
  void setArraylet();
  MMINLINE uintptr_t whichArraylet(uintptr_t *arraylet,
                                   uintptr_t arrayletLeafLogSize) {
    return ((uintptr_t)arraylet - (uintptr_t)getLowAddress()) >>
           arrayletLeafLogSize;
  }
  MMINLINE uintptr_t *getArraylet(uintptr_t index,
                                  uintptr_t arrayletLeafLogSize) {
    return (uintptr_t *)(((uintptr_t)getLowAddress()) +
                         (index << arrayletLeafLogSize));
  }
  MMINLINE uintptr_t *getArrayletParent(uintptr_t index) {
    return _arrayletBackPointers[index];
  }
  MMINLINE void setArrayletParent(uintptr_t index, uintptr_t *backPointer) {
    _arrayletBackPointers[index] = backPointer;
  }
  MMINLINE void clearArraylet(uintptr_t index) {
    _arrayletBackPointers[index] = NULL;
  }
  MMINLINE bool isArrayletUnused(uintptr_t index) {
    return _arrayletBackPointers[index] == NULL;
  }
  MMINLINE bool isArrayletUsed(uintptr_t index) {
    return _arrayletBackPointers[index] != NULL;
  }
  MMINLINE void setNextArrayletIndex(uintptr_t index) {
    _nextArrayletIndex = index;
  }
