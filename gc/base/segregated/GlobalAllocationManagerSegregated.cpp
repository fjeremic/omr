...
MM_GlobalAllocationManagerSegregated::newInstance(
  ...
  MM_GlobalAllocationManagerSegregated *allocationManager =
      (MM_GlobalAllocationManagerSegregated *)env->getForge()->allocate(
          sizeof(MM_GlobalAllocationManagerSegregated),
          OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
  if (allocationManager) {
    allocationManager =
        new (allocationManager) MM_GlobalAllocationManagerSegregated(env);
...
bool MM_GlobalAllocationManagerSegregated::initialize(
    MM_EnvironmentBase *env, MM_RegionPoolSegregated *regionPool) {
  ...
  if (result) {
    ...
    if (0 == _managedAllocationContextCount) {
      ...
      uintptr_t desiredAllocationContextCount =
          2 * omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE);
      uintptr_t regionCount = _extensions->memoryMax / _extensions->regionSize;
      /* heuristic -- ACs are permitted to waste up to 1/8th of the heap in
       * slack regions. This number may need to be adjusted */
      uintptr_t maxAllocationContextCount = regionCount / 8;
      _managedAllocationContextCount = OMR_MAX(
          1, OMR_MIN(desiredAllocationContextCount, maxAllocationContextCount));
    }
...
void MM_GlobalAllocationManagerSegregated::setSweepScheme(
    MM_SweepSchemeSegregated *sweepScheme) {
  _regionPool->setSweepScheme(sweepScheme);
}

void MM_GlobalAllocationManagerSegregated::setMarkingScheme(
    MM_SegregatedMarkingScheme *markingScheme) {
  for (uintptr_t i = 0; i < _managedAllocationContextCount; i++) {
    ((MM_AllocationContextSegregated *)_managedAllocationContexts[i])
...
bool MM_GlobalAllocationManagerSegregated::initializeAllocationContexts(
  ...
  MM_AllocationContextSegregated **contexts =
...
bool MM_GlobalAllocationManagerSegregated::acquireAllocationContext(
  ...
  if (env->getAllocationContext() == NULL) {
    ...
    MM_AllocationContextSegregated *ac = (MM_AllocationContextSegregated *)
        _managedAllocationContexts[allocationContextIndex];
    if (NULL != ac) {
      ac->enter(env);
...
void MM_GlobalAllocationManagerSegregated::releaseAllocationContext(
    MM_EnvironmentBase *env) {
...
void MM_GlobalAllocationManagerSegregated::flushCachedFullRegions(
    MM_EnvironmentBase *env) {
  Assert_MM_true(_managedAllocationContextCount > 0);

  for (uintptr_t i = 0; i < _managedAllocationContextCount; i++) {
    ((MM_AllocationContextSegregated *)(_managedAllocationContexts[i]))
        ->returnFullRegionsToRegionPool(env);
  }
}
