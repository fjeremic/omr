...
/**
 * A region pool which manages lists of small objects (empty, partially used, full, sweep...), arraylet leafs and large
 * objects regions.
 *
 * @ingroup GC_Realtime
 */
...
private:
    MM_HeapRegionManager*
        _heapRegionManager; /**< The Region Manager which contains all the descriptors used for allocation (they contain
                               the Memory Pools which manage their underlying memory) */
    MM_SweepSchemeSegregated* _sweepScheme;

    MM_FreeHeapRegionList* _singleFreeList; /**< Singleton free regions. */
    MM_FreeHeapRegionList*
        _multiFreeList; /**< Contiguous free regions (may include regions that are actually singletons). */
    MM_FreeHeapRegionList* _coalesceFreeList; /**< Free regions that might be coalescable, so avoid allocating from them
                                                 if at all possible. */
     ...
     * most occupied.
     */
    MM_LockingHeapRegionQueue* _smallAvailableRegions[OMR_SIZECLASSES_NUM_SMALL
        + 1][NUM_DEFRAG_BUCKETS]; /**< Regions that are available to be given out to allocation contexts and aren't
                                     entirely free. */
     ...
     * allocated into.
     */
    MM_HeapRegionQueue* _smallFullRegions[OMR_SIZECLASSES_NUM_SMALL
        + 1]; /**< Regions that have been allocated into during this GC cycle. */
    ...
    MM_HeapRegionQueue* _smallSweepRegions[OMR_SIZECLASSES_NUM_SMALL
        + 1]; /**< Regions that are waiting to be swept during this GC cycle. */
     ...
     * most occupied.
     */
    MM_HeapRegionQueue* _arrayletAvailableRegions; /**< Arraylet regions that are available to be given out to
     ...
     */
    MM_HeapRegionQueue*
        _arrayletFullRegions; /**< Arraylet regions that have been allocated into during this GC cycle. */

    /**
     * @note The sweep regions will contain a mix of marked (live) and unmarked (dead) objects.
     */
    MM_HeapRegionQueue*
        _arrayletSweepRegions; /**< Arraylet regions that are waiting to be swept during this GC cycle. */
     ...
     */
    MM_HeapRegionQueue*
        _largeFullRegions; /**< Large object regions that have been allocated into during this GC cycle. */

    /**
     * @note The sweep regions will contain a mix of marked (live) and unmarked (dead) objects.
     */
    MM_HeapRegionQueue*
        _largeSweepRegions; /**< Large object regions that are waiting to be swept during this GC cycle. */
    ...
    uintptr_t _splitAvailableListSplitCount; /* number of split available region queues per size class per defragment
                                                bucket */
    uint8_t _skipAvailableRegionForAllocation[OMR_SIZECLASSES_NUM_SMALL
        + 1]; /* per size class flag to indicate if there is any available regions left for allocation for that size
                 class */
...
public:
     ...
     */
private:
    MMINLINE void incrementRegionsInUse(uintptr_t value) { MM_AtomicOperations::add(&_regionsInUse, value); }
    MMINLINE void decrementRegionsInUse(uintptr_t value) { MM_AtomicOperations::subtract(&_regionsInUse, value); }
...
public:
    ...
    static MM_HeapRegionQueue* allocateHeapRegionQueue(MM_EnvironmentBase* env,
    ...
    {
        return (size + _heapRegionManager->getRegionSize() - 1) & (~(_heapRegionManager->getRegionSize() - 1));
    }
    ...
    { /* TODO: use shift operation once region log2 size is introduced */
        return (size + _heapRegionManager->getRegionSize() - 1) / _heapRegionManager->getRegionSize();
    }
    ...
    {
        assume(sizeClass >= OMR_SIZECLASSES_MIN_SMALL && sizeClass <= OMR_SIZECLASSES_MAX_SMALL,
            "getOccupancy: invalid sizeclass");
        return _initialCountOfSweepRegions[sizeClass];
    }

    MMINLINE uintptr_t getInitialTotalCountOfSweepRegions() const { return _initialTotalCountOfSweepRegions; }
    ...
    MMINLINE uintptr_t getCurrentTotalCountOfSweepRegions() const { return _currentTotalCountOfSweepRegions; }
    ...
    {
        memset(&_skipAvailableRegionForAllocation[0], 0, sizeof(_skipAvailableRegionForAllocation));
    }
