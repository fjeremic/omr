...
private:
public:
  /* OMRTODO Remove _objectsMarked and _scanBytes, they are used to fake marking
   * to create more interesting verbose output */
...
public:
  ...
  virtual bool internalGarbageCollect(MM_EnvironmentBase *, MM_MemorySubSpace *,
                                      MM_AllocateDescription *);
  virtual void internalPreCollect(MM_EnvironmentBase *, MM_MemorySubSpace *,
                                  MM_AllocateDescription *, uint32_t);
  ...
  MM_SegregatedGC(MM_EnvironmentBase *env)
      : MM_GlobalCollector(),
        ...
        _portLibrary(env->getPortLibrary()), _markingScheme(NULL),
        _sweepScheme(NULL), _dispatcher(_extensions->dispatcher), _scanBytes(0),
        _objectsMarked(0) {
