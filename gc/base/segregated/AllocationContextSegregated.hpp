...
public:
  MM_HeapRegionDescriptorSegregated
      *_smallRegions[OMR_SIZECLASSES_NUM_SMALL + 1];
...
private:
  ...
  omrthread_monitor_t
      _mutexSmallAllocations; /**< Allocation lock for small size class */
  omrthread_monitor_t
      _mutexArrayletAllocations; /**< Allocation lock for arraylet size class */
  volatile uint32_t _count;      /**< how many threads are attached to me */

  MM_HeapRegionQueue *
      _perContextSmallFullRegions[OMR_SIZECLASSES_NUM_SMALL +
                                  1]; /**< Per-context Regions that have been
                                         allocated into during this GC cycle. */
  MM_HeapRegionQueue
      *_perContextArrayletFullRegions; /**< Per-context Arraylet regions that
                                          have been allocated into during this
                                          GC cycle. */
  MM_HeapRegionQueue
      *_perContextLargeFullRegions; /**< Per-context Large object regions that
                                       have been allocated into during this GC
                                       cycle. */

  /* Methods */
public:
  static MM_AllocationContextSegregated *
  ...
  virtual void flushForShutdown(MM_EnvironmentBase *env) {
    /* ---- no implementation -----
     * this isn't necessary in Metronome, and calling flush() crashes since the
     * env may not have an initialized feedlet for event reporting
     */
  }
  ...
  /**
   * Acquire exclusive access to the allocation context by setting its count
   * field to 1. Since another thread may be trying to do this at the same time,
   * it must be done with an atomic operation.
   * @return true if the thread succeeded in acquiring exclusive access
   */
  MMINLINE void enter(MM_EnvironmentBase *env) {
   ...
   */
  MMINLINE void exit(MM_EnvironmentBase *env) {
    MM_AtomicOperations::subtractU32((uint32_t *)&_count, 1);
  }

  uintptr_t *preAllocateSmall(MM_EnvironmentBase *env,
                              uintptr_t sizeInBytesRequired);

  virtual uintptr_t *allocateLarge(MM_EnvironmentBase *env,
                                   uintptr_t sizeInBytesRequired);
...
protected:
  MM_AllocationContextSegregated(MM_EnvironmentBase *env,
                                 MM_GlobalAllocationManagerSegregated *gam,
                                 MM_RegionPoolSegregated *regionPool)
      : MM_AllocationContext(), _regionPool(regionPool), _arrayletRegion(NULL),
        _markingScheme(NULL), _mutexSmallAllocations(NULL),
        _mutexArrayletAllocations(NULL), _count(0),
  ...
  virtual bool trySweepAndAllocateRegionFromSmallSizeClass(
      MM_EnvironmentBase *env, uintptr_t sizeClass, uintptr_t *sweepCount,
      uint64_t *sweepStartTime);

  virtual void signalSmallRegionDepleted(MM_EnvironmentBase *env,
                                         uintptr_t sizeClass);
  ...
  MMINLINE
  void flushHelper(MM_EnvironmentBase *env,
  ...
  bool tryAllocateRegionFromSmallSizeClass(MM_EnvironmentBase *env,
                                           uintptr_t sizeClass);
