...
bool MM_HeapRegionDescriptorSegregated::initialize(MM_EnvironmentBase* env, MM_HeapRegionManager* regionManager)
{
...
bool MM_HeapRegionDescriptorSegregated::initializer(MM_EnvironmentBase* env, MM_HeapRegionManager* regionManager,
...
void MM_HeapRegionDescriptorSegregated::setRange(RegionType type, uintptr_t range)
{
 ...
 */
MM_HeapRegionDescriptorSegregated* MM_HeapRegionDescriptorSegregated::splitRange(uintptr_t numRegionsToSplit)
{
    ...
    MM_HeapRegionDescriptorSegregated* second
        = (MM_HeapRegionDescriptorSegregated*)_regionManager->mapRegionTableIndexToDescriptor(
            index + numRegionsToSplit);
    second->resetTailFree(range - numRegionsToSplit);
    return second;
}

bool MM_HeapRegionDescriptorSegregated::joinFreeRangeInit(MM_HeapRegionDescriptorSegregated* possNext)
{
...
uintptr_t MM_HeapRegionDescriptorSegregated::formatFresh(MM_EnvironmentBase* env, uintptr_t sizeClass, void* lowAddress)
{
...
void MM_HeapRegionDescriptorSegregated::addBytesFreedToArrayletBackout(MM_EnvironmentBase* env)
{
    Assert_MM_true(isArraylet());
...
void MM_HeapRegionDescriptorSegregated::addBytesFreedToSmallSpineBackout(MM_EnvironmentBase* env)
{
 ...
 */
void MM_HeapRegionDescriptorSegregated::emptyRegionAllocated(MM_EnvironmentBase* env)
{
    ...
    if (isSmall()) {
        ...
        _memoryPoolACL.addBytesAllocated(
            env, (extensions->regionSize % env->getOmrVM()->_arrayletLeafSize) * getRange());
    } else if (isLarge()) {
...
void MM_HeapRegionDescriptorSegregated::emptyRegionReturned(MM_EnvironmentBase* env)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
...
void MM_HeapRegionDescriptorSegregated::updateCounts(MM_EnvironmentBase* env, bool fromFlush)
{
    if (isArraylet()) {
