...
/* Define realtime arraylet leaf size as the largest small size class we can
 * have */
...
MM_Configuration *MM_ConfigurationSegregated::newInstance(
  ...
  configuration = (MM_ConfigurationSegregated *)env->getForge()->allocate(
      sizeof(MM_ConfigurationSegregated), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
...
bool MM_ConfigurationSegregated::initialize(MM_EnvironmentBase *env) {
  bool success = false;
  ...
  if (MM_Configuration::initialize(env)) {
    ...
    if (NULL != env->getOmrVM()->_sizeClasses) {
      ...
      extensions->arrayletsPerRegion =
          extensions->regionSize / env->getOmrVM()->_arrayletLeafSize;
      success = true;
    }
  }
  return success;
...
void MM_ConfigurationSegregated::tearDown(MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
...
MM_Heap *MM_ConfigurationSegregated::createHeapWithManager(
    MM_EnvironmentBase *env, uintptr_t heapBytesRequested,
    MM_HeapRegionManager *regionManager) {
  return MM_HeapVirtualMemory::newInstance(env,
                                           env->getExtensions()->heapAlignment,
                                           heapBytesRequested, regionManager);
}

MM_MemorySpace *MM_ConfigurationSegregated::createDefaultMemorySpace(
  ...
  if (NULL ==
      (extensions->defaultSizeClasses = MM_SizeClasses::newInstance(env))) {
    return NULL;
  }

  regionPool =
      MM_RegionPoolSegregated::newInstance(env, extensions->heapRegionManager);
  if (NULL == regionPool) {
    return NULL;
  }

  extensions->globalAllocationManager =
      MM_GlobalAllocationManagerSegregated::newInstance(env, regionPool);
  ...
  if (NULL == (physicalSubArena =
                   MM_PhysicalSubArenaRegionBased::newInstance(env, heap))) {
    memoryPool->kill(env);
    return NULL;
  }

  memorySubSpaceSegregated = MM_MemorySubSpaceSegregated::newInstance(
      env, physicalSubArena, memoryPool, true, parameters->_minimumSpaceSize,
  ...
  if (NULL ==
      (physicalArena = MM_PhysicalArenaRegionBased::newInstance(env, heap))) {
    memorySubSpaceSegregated->kill(env);
    return NULL;
  }

  return MM_MemorySpace::newInstance(
      env, heap, physicalArena, memorySubSpaceSegregated, parameters,
      MEMORY_SPACE_NAME_METRONOME, MEMORY_SPACE_DESCRIPTION_METRONOME);
}

void MM_ConfigurationSegregated::defaultMemorySpaceAllocated(
  ...
  ((MM_GlobalAllocationManagerSegregated *)extensions->globalAllocationManager)
      ->setSweepScheme(((MM_SegregatedGC *)extensions->getGlobalCollector())
                           ->getSweepScheme());
  ((MM_GlobalAllocationManagerSegregated *)extensions->globalAllocationManager)
      ->setMarkingScheme(((MM_SegregatedGC *)extensions->getGlobalCollector())
                             ->getMarkingScheme());
}

MM_EnvironmentBase *MM_ConfigurationSegregated::allocateNewEnvironment(
    MM_GCExtensionsBase *extensions, OMR_VMThread *omrVMThread) {
  return MM_EnvironmentBase::newInstance(extensions, omrVMThread);
}

J9Pool *MM_ConfigurationSegregated::createEnvironmentPool(
    MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());

  uintptr_t numberOfElements =
      getConfigurationDelegate()->getInitialNumberOfPooledEnvironments(env);
  /* number of elements, pool flags = 0, 0 selects default pool configuration
   * (at least 1 element, puddle size rounded to OS page size) */
  return pool_new(sizeof(MM_EnvironmentBase), numberOfElements,
                  sizeof(uint64_t), 0, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_MM,
                  POOL_FOR_PORT(OMRPORTLIB));
}

bool MM_ConfigurationSegregated::initializeEnvironment(
    MM_EnvironmentBase *env) {
  if (MM_Configuration::initializeEnvironment(env)) {
    env->getExtensions()->globalAllocationManager->acquireAllocationContext(
        env);
    if (env->getAllocationContext() != NULL) {
      /* BEN TODO: VMDESIGN 1880: When allocation trackers are commonized, the
       * setting of the allocation tracker in the env should be done by the base
       * configuration initializeEnvironment.
       */
      env->_allocationTracker = ((MM_MemoryPoolSegregated *)env->getExtensions()
...
MM_HeapRegionManager *MM_ConfigurationSegregated::createHeapRegionManager(
    MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  uintptr_t descriptorSize =
      sizeof(MM_HeapRegionDescriptorSegregated) +
      sizeof(uintptr_t *) * extensions->arrayletsPerRegion;

  MM_HeapRegionManagerTarok *heapRegionManager =
      MM_HeapRegionManagerTarok::newInstance(
          env, extensions->regionSize, descriptorSize,
          MM_HeapRegionDescriptorSegregated::initializer,
 ...
 */
MM_GlobalCollector *MM_ConfigurationSegregated::createGlobalCollector(
    MM_EnvironmentBase *env) {
  return MM_SegregatedGC::newInstance(env);
}

MM_Dispatcher *MM_ConfigurationSegregated::createDispatcher(
