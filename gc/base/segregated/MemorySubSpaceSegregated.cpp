...
void* MM_MemorySubSpaceSegregated::allocateObject(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* baseSubSpace, MM_MemorySubSpace* previousSubSpace, bool shouldCollectOnFailure)
{
    ...
    if (shouldCollectOnFailure) {
        ...
        /* not setting these object flags causes a failure working with empty arrays but where this is set should
         * probably be hoisted into the caller */
 ...
 */
void* MM_MemorySubSpaceSegregated::allocateArrayletLeaf(MM_EnvironmentBase* env,
    ...
    bool shouldCollectOnFailure)
{
    ...
    /* spine object refers to the barrier-safe "object" reference, as opposed to the internal "heap address" represented
     * by the spine variable */
...
void* MM_MemorySubSpaceSegregated::allocate(
...
void* MM_MemorySubSpaceSegregated::allocateMixedObjectOrArraylet(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, AllocateType allocType)
{
    void* result = NULL;
    ...
    if (NULL != _collector) {
        ...
        if (!env->acquireExclusiveVMAccessForGC(_collector)) {
            /* we have exclusive access but another thread beat us to the GC so see if they collected enough to satisfy
             * our request */
        ...
        result = _collector->garbageCollect(
            env, this, allocDescription, J9MMCONSTANT_IMPLICIT_GC_DEFAULT, NULL, NULL, NULL);
        allocDescription->restoreObjects(env);
        result = allocate(env, allocDescription, allocType);
        ...
        result = _collector->garbageCollect(
            env, this, allocDescription, J9MMCONSTANT_IMPLICIT_GC_AGGRESSIVE, NULL, NULL, NULL);
        allocDescription->restoreObjects(env);
        result = allocate(env, allocDescription, allocType);
...
void* MM_MemorySubSpaceSegregated::allocationRequestFailed(MM_EnvironmentBase* env,
...
uintptr_t MM_MemorySubSpaceSegregated::largestDesirableArraySpine()
{
    return OMR_SIZECLASSES_MAX_SMALL_SIZE_BYTES;
}

void MM_MemorySubSpaceSegregated::abandonHeapChunk(void* addrBase, void* addrTop)
{}

MM_MemorySubSpace* MM_MemorySubSpaceSegregated::getDefaultMemorySubSpace()
{
    return this;
}

MM_MemorySubSpace* MM_MemorySubSpaceSegregated::getTenureMemorySubSpace()
{
    return this;
}

MM_MemorySubSpaceSegregated* MM_MemorySubSpaceSegregated::newInstance(MM_EnvironmentBase* env,
    MM_PhysicalSubArena* physicalSubArena, MM_MemoryPool* memoryPool, bool usesGlobalCollector, uintptr_t minimumSize,
    uintptr_t initialSize, uintptr_t maximumSize)
{
    ...
    memorySubSpace = (MM_MemorySubSpaceSegregated*)env->getForge()->allocate(
        sizeof(MM_MemorySubSpaceSegregated), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (NULL != memorySubSpace) {
        new (memorySubSpace) MM_MemorySubSpaceSegregated(
...
bool MM_MemorySubSpaceSegregated::initialize(MM_EnvironmentBase* env)
{
...
void MM_MemorySubSpaceSegregated::tearDown(MM_EnvironmentBase* env)
{
...
bool MM_MemorySubSpaceSegregated::expanded(
    MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, MM_HeapRegionDescriptor* region, bool canCoalesce)
{
    ...
    if (result) {
        _extensions->indexableObjectModel.expandArrayletSubSpaceRange(
            this, regionLowAddress, regionHighAddress, largestDesirableArraySpine());
    }

    return result;
}

bool MM_MemorySubSpaceSegregated::heapAddRange(
...
bool MM_MemorySubSpaceSegregated::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size,
    void* lowAddress, void* highAddress, void* lowValidAddress, void* highValidAddress)
{
    Assert_MM_unreachable();
    return MM_MemorySubSpaceUniSpace::heapRemoveRange(
        env, subspace, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
}

void MM_MemorySubSpaceSegregated::heapReconfigured(MM_EnvironmentBase* env)
{
...
uintptr_t MM_MemorySubSpaceSegregated::getActualFreeMemorySize()
{
    return _memoryPoolSegregated->getActualFreeMemorySize();
}

uintptr_t MM_MemorySubSpaceSegregated::getApproximateFreeMemorySize()
{
    return _memoryPoolSegregated->getApproximateFreeMemorySize();
}

uintptr_t MM_MemorySubSpaceSegregated::getActiveMemorySize()
{
    return getActiveMemorySize(MEMORY_TYPE_OLD | MEMORY_TYPE_NEW);
}

uintptr_t MM_MemorySubSpaceSegregated::getActiveMemorySize(uintptr_t includeMemoryType)
{
...
uintptr_t MM_MemorySubSpaceSegregated::getActualActiveFreeMemorySize()
{
    return getActualActiveFreeMemorySize(MEMORY_TYPE_OLD | MEMORY_TYPE_NEW);
}

uintptr_t MM_MemorySubSpaceSegregated::getActualActiveFreeMemorySize(uintptr_t includeMemoryType)
{
    if (includeMemoryType & getTypeFlags()) {
        return _memoryPoolSegregated->getActualFreeMemorySize();
    }
...
uintptr_t MM_MemorySubSpaceSegregated::getApproximateActiveFreeMemorySize()
{
    return getApproximateActiveFreeMemorySize(MEMORY_TYPE_OLD | MEMORY_TYPE_NEW);
}

uintptr_t MM_MemorySubSpaceSegregated::getApproximateActiveFreeMemorySize(uintptr_t includeMemoryType)
{
    if (includeMemoryType & getTypeFlags()) {
...
MM_MemoryPool* MM_MemorySubSpaceSegregated::getMemoryPool()
{
