...
private:
   ...
   */
public:
  static MM_SweepSchemeSegregated *newInstance(MM_EnvironmentBase *env,
                                               MM_MarkMap *markMap);
  void kill(MM_EnvironmentBase *env);

  MM_MarkMap *getMarkMap(MM_EnvironmentBase *env);

  void sweep(MM_EnvironmentBase *env, MM_MemoryPoolSegregated *memoryPool,
             bool isFixHeapForWalk);
  virtual void sweepRegion(MM_EnvironmentBase *env,
...
protected:
  ...
  MM_SweepSchemeSegregated(MM_EnvironmentBase *env, MM_MarkMap *markMap)
      : MM_BaseVirtual(), _memoryPool(NULL), _markMap(markMap),
        _extensions(env->getExtensions()), _isFixHeapForWalk(false),
        _clearMarkMapAfterSweep(true) {
    _typeId = __FUNCTION__;
  };

private:
  void unmarkRegion(MM_EnvironmentBase *env,
                    MM_HeapRegionDescriptorSegregated *region);
  void sweepSmallRegion(MM_EnvironmentBase *env,
                        MM_HeapRegionDescriptorSegregated *region);
  void sweepArrayletRegion(MM_EnvironmentBase *env,
                           MM_HeapRegionDescriptorSegregated *region);
  void sweepLargeRegion(MM_EnvironmentBase *env,
                        MM_HeapRegionDescriptorSegregated *region);
  void addBytesFreedAfterSweep(MM_EnvironmentBase *env,
                               MM_HeapRegionDescriptorSegregated *region);
  ...
  MMINLINE bool addFreeChunk(MM_MemoryPoolAggregatedCellList *memoryPoolACL,
                             uintptr_t *freeChunk, uintptr_t freeChunkSize,
  ...
  /**
   * Called to allow subclasses to yield to mutators if required (eg, realtime
   * constraints)
   */
  virtual void yieldFromSweep(MM_EnvironmentBase *env,
                              uintptr_t yieldSlackTime = 0);

  /**
   * Called from incrementalCoalesceFreeRegions() to track range of regions
   * processed since last yield
  ...
  /**
   * Called from incrementalSweepSmall() to track range of regions processed
   * since last yield
   * @return A nonzero value to indicate yield slack time when yield is required
   */
  virtual uintptr_t resetSweepSmallRegionCount(MM_EnvironmentBase *env,
  ...
  /**
   * Calculate the maximum number of regions of a single small region size class
   * to sweep before proceeding to the next size class while sweeping small
   * regions.
   */
  uintptr_t calcSweepSmallRegionsPerIteration(uintptr_t numCellsPerSizeClass) {
    uintptr_t regionCost =
        (SWEEP_CELL_COST * numCellsPerSizeClass) + SWEEP_REGION_COST;
