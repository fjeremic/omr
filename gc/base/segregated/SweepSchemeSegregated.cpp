                                      ...
                                      MM_MarkMap* markMap)
{
  ...
  instance = (MM_SweepSchemeSegregated*)env->getForge()->allocate(
    sizeof(MM_SweepSchemeSegregated),
...
void
MM_SweepSchemeSegregated::yieldFromSweep(MM_EnvironmentBase* env,
                                         uintptr_t yieldSlackTime)
{}

void
MM_SweepSchemeSegregated::sweep(MM_EnvironmentBase* env,
...
void
MM_SweepSchemeSegregated::sweepRegion(MM_EnvironmentBase* env,
...
void
MM_SweepSchemeSegregated::unmarkRegion(
  ...
  MM_HeapRegionDescriptorSegregated* region)
{
  ...
  _markMap->getSlotIndexAndMask(
    (omrobjectptr_t)regionLowAddress, &slotIndexLow, &bitMask);
  ...
  if (lastCellStart + scanBitOffset >= regionHighAddress) {
    _markMap->getSlotIndexAndMask(
      (omrobjectptr_t)lastCellStart, &slotIndexHigh, &bitMask);
  } else {
    _markMap->getSlotIndexAndMask(
      (omrobjectptr_t)(lastCellStart + scanBitOffset),
      &slotIndexHigh,
      &bitMask);
  }
...
void
MM_SweepSchemeSegregated::sweepSmallRegion(
  MM_EnvironmentBase* env,
  MM_HeapRegionDescriptorSegregated* region)
{
  uintptr_t cellSize = region->getCellSize();
  ...
  _markMap->getSlotIndexAndMask(
    (omrobjectptr_t)lastCellAddress, &lastCellSlotIndex, &lastCellBitMask);
  ...
  for (uintptr_t currentCellAddress = (uintptr_t)lowAddress;
    ...
    _markMap->getSlotIndexAndMask(
      (omrobjectptr_t)currentCellAddress, &initialSlotIndex, &bitMask);
...
void
MM_SweepSchemeSegregated::sweepArrayletRegion(
  MM_EnvironmentBase* env,
  MM_HeapRegionDescriptorSegregated* region)
{
...
void
MM_SweepSchemeSegregated::sweepLargeRegion(
  MM_EnvironmentBase* env,
  MM_HeapRegionDescriptorSegregated* region)
{
  MM_MemoryPoolAggregatedCellList* memoryPoolACL = region->getMemoryPoolACL();
...
void
MM_SweepSchemeSegregated::addBytesFreedAfterSweep(
  MM_EnvironmentBase* env,
  MM_HeapRegionDescriptorSegregated* region)
{
...
void
MM_SweepSchemeSegregated::incrementalCoalesceFreeRegions(
  MM_EnvironmentBase* env)
{
...
uintptr_t
MM_SweepSchemeSegregated::resetSweepSmallRegionCount(
  MM_EnvironmentBase* env,
    ...
    for (uintptr_t sizeClass = OMR_SIZECLASSES_MIN_SMALL;
      ...
      while (regionPool->getCurrentCountOfSweepRegions(sizeClass)) {
        ...
        if ((actualSweepRegions = sweepList->dequeue(
               env->getRegionWorkList(), sweepSmallRegionsPerIteration)) > 0) {
          ...
          while ((currentRegion = env->getRegionWorkList()->dequeue()) !=
            ...
            if (currentRegion->getMemoryPoolACL()->getFreeCount() < numCells) {
              ...
              if (currentRegion->getMemoryPoolACL()->getMarkCount() ==
                ...
                regionPool->enqueueAvailable(
                  currentRegion, sizeClass, occupancy, splitIndex);
              }
            } else {
              currentRegion->emptyRegionReturned(env);
