 ...
 */
MM_SweepSchemeSegregated* MM_SweepSchemeSegregated::newInstance(MM_EnvironmentBase* env, MM_MarkMap* markMap)
{
...
void MM_SweepSchemeSegregated::yieldFromSweep(MM_EnvironmentBase* env, uintptr_t yieldSlackTime)
{}

void MM_SweepSchemeSegregated::sweep(
    MM_EnvironmentBase* env, MM_MemoryPoolSegregated* memoryPool, bool isFixHeapForWalk)
{
...
void MM_SweepSchemeSegregated::sweepRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptorSegregated* region)
{
...
void MM_SweepSchemeSegregated::unmarkRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptorSegregated* region)
{
     ...
     * object starts at 16 bytes past its header, which is where we would set the scan bit on the mark map. Note we need
     * to ensure that cell size is at least >16 bytes, otherwise 16 bytes past J9Object* would go off the end of page,
     * hence the if check below. And since there is no scan bit for object that's 16 bytes, this should be safe.
     */
...
void MM_SweepSchemeSegregated::sweepSmallRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptorSegregated* region)
{
    uintptr_t cellSize = region->getCellSize();
    uintptr_t numCells = region->getNumCells();
    ...
    if (freeChunk) {
        if (addFreeChunk(
                memoryPoolACL, (uintptr_t*)freeChunk, freeChunkSize, minimumFreeEntrySize, freeChunkCellCount)) {
            sweepCostCounter += 3;
        }
    }
    if (sweepCostCounter > sweepCostToCheckYield) {
...
void MM_SweepSchemeSegregated::sweepArrayletRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptorSegregated* region)
{
...
void MM_SweepSchemeSegregated::sweepLargeRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptorSegregated* region)
{
    MM_MemoryPoolAggregatedCellList* memoryPoolACL = region->getMemoryPoolACL();
...
uintptr_t MM_SweepSchemeSegregated::resetCoalesceFreeRegionCount(MM_EnvironmentBase* env)
{
    return 0;
}
...
void MM_SweepSchemeSegregated::incrementalCoalesceFreeRegions(MM_EnvironmentBase* env)
{
    ...
    for (uintptr_t i = 0; i < regionCount;) {
        ...
        if (currentRegion->isFree()) {
            ...
            bool joined
                = (range < MAX_REGION_COALESCE) && (coalescing != NULL && coalescing->joinFreeRangeInit(currentRegion));
            if (joined) {
                currentRegion = NULL;
            } else {
                shouldClose = true;
...
void MM_SweepSchemeSegregated::incrementalSweepArraylet(MM_EnvironmentBase* env)
{
    uintptr_t arrayletsPerRegion = env->getExtensions()->arrayletsPerRegion;
...
uintptr_t MM_SweepSchemeSegregated::resetSweepSmallRegionCount(MM_EnvironmentBase* env, uintptr_t yieldSmallRegionCount)
{
    return 0;
}
    ...
    while (regionPool->getCurrentTotalCountOfSweepRegions()) {
        for (uintptr_t sizeClass = OMR_SIZECLASSES_MIN_SMALL; sizeClass <= OMR_SIZECLASSES_MAX_SMALL; sizeClass++) {
            while (regionPool->getCurrentCountOfSweepRegions(sizeClass)) {
                ...
                if ((actualSweepRegions = sweepList->dequeue(env->getRegionWorkList(), sweepSmallRegionsPerIteration))
                    > 0) {
