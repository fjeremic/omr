...
MM_SweepSchemeSegregated::newInstance(MM_EnvironmentBase *env,
                                      MM_MarkMap *markMap) {
  MM_SweepSchemeSegregated *instance;

  instance = (MM_SweepSchemeSegregated *)env->getForge()->allocate(
      sizeof(MM_SweepSchemeSegregated), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (NULL != instance) {
 ...
 */
void MM_SweepSchemeSegregated::kill(MM_EnvironmentBase *env) {
  tearDown(env);
 ...
 *
 */
bool MM_SweepSchemeSegregated::initialize(MM_EnvironmentBase *env) {
  return true;
}
 ...
 *
 */
void MM_SweepSchemeSegregated::tearDown(MM_EnvironmentBase *env) {}

void MM_SweepSchemeSegregated::yieldFromSweep(MM_EnvironmentBase *env,
                                              uintptr_t yieldSlackTime) {}

void MM_SweepSchemeSegregated::sweep(MM_EnvironmentBase *env,
                                     MM_MemoryPoolSegregated *memoryPool,
                                     bool isFixHeapForWalk) {
...
void MM_SweepSchemeSegregated::preSweep(MM_EnvironmentBase *env) {
  _memoryPool->moveInUseToSweep(env);
}

void MM_SweepSchemeSegregated::postSweep(MM_EnvironmentBase *env) {
  incrementalCoalesceFreeRegions(env);
}

void MM_SweepSchemeSegregated::sweepRegion(
...
void MM_SweepSchemeSegregated::unmarkRegion(
    MM_EnvironmentBase *env, MM_HeapRegionDescriptorSegregated *region) {
  ...
  _markMap->getSlotIndexAndMask((omrobjectptr_t)regionLowAddress, &slotIndexLow,
                                &bitMask);

  /*
   * CMVC 149635 : We need to ensure we do not miss to clear any scan bit, which
   * is set for the ref array copy optimization. To not miss the scan bit of an
   * object that's in the last cell of a region, we pretend the last object
   * starts at 16 bytes past its header, which is where we would set the scan
   * bit on the mark map. Note we need to ensure that cell size is at least >16
   * bytes, otherwise 16 bytes past J9Object* would go off the end of page,
   * hence the if check below. And since there is no scan bit for object that's
   * 16 bytes, this should be safe.
   */
  if (lastCellStart + scanBitOffset >= regionHighAddress) {
    _markMap->getSlotIndexAndMask((omrobjectptr_t)lastCellStart, &slotIndexHigh,
                                  &bitMask);
  } else {
    _markMap->getSlotIndexAndMask(
        (omrobjectptr_t)(lastCellStart + scanBitOffset), &slotIndexHigh,
        &bitMask);
  }
  _markMap->setMarkBlock(slotIndexLow, slotIndexHigh, 0);
}

void MM_SweepSchemeSegregated::sweepSmallRegion(
    MM_EnvironmentBase *env, MM_HeapRegionDescriptorSegregated *region) {
  uintptr_t cellSize = region->getCellSize();
  uintptr_t numCells = region->getNumCells();
  ...
  uintptr_t numCellsInBlock = 0; /* block is the set of consecutive cells that
                                    are all unmarked or all marked */
  ...
  _markMap->getSlotIndexAndMask((omrobjectptr_t)lastCellAddress,
                                &lastCellSlotIndex, &lastCellBitMask);

  for (uintptr_t currentCellAddress = (uintptr_t)lowAddress;
       currentCellAddress <= lastCellAddress;
       currentCellAddress += blockSizeInBytes) {
    uintptr_t initialSlotIndex, slotIndex, bitMask;

    _markMap->getSlotIndexAndMask((omrobjectptr_t)currentCellAddress,
                                  &initialSlotIndex, &bitMask);
    ...
    if (0 != (_markMap->getSlot(initialSlotIndex) & bitMask)) {
      ...
      memoryPoolACL->setMarkCount(memoryPoolACL->getMarkCount() +
                                  numCellsInBlock);
      ...
      if ((0 == _markMap->getSlot(initialSlotIndex)) &&
          (initialSlotIndex < lastCellSlotIndex)) {
        slotIndex = initialSlotIndex;
        do {
          slotIndex++;
        } while ((0 == _markMap->getSlot(slotIndex)) &&
                 (slotIndex < lastCellSlotIndex));
  ...
  _memoryPool->getRegionPool()->addDarkMatterCellsAfterSweepForSizeClass(
      region->getSizeClass(),
      numCells - memoryPoolACL->getMarkCount() - memoryPoolACL->getFreeCount());
}

void MM_SweepSchemeSegregated::sweepArrayletRegion(
    MM_EnvironmentBase *env, MM_HeapRegionDescriptorSegregated *region) {
...
void MM_SweepSchemeSegregated::sweepLargeRegion(
    MM_EnvironmentBase *env, MM_HeapRegionDescriptorSegregated *region) {
  MM_MemoryPoolAggregatedCellList *memoryPoolACL = region->getMemoryPoolACL();
...
void MM_SweepSchemeSegregated::addBytesFreedAfterSweep(
    MM_EnvironmentBase *env, MM_HeapRegionDescriptorSegregated *region) {
  /* Bytes freed by large regions are counted when the regions are returned to
   * the free region list, see emptyRefionReturned
   */
  ...
  env->_allocationTracker->addBytesFreed(
      env, (currentFreeBytes - memoryPoolACL->getPreSweepFreeBytes()));
  memoryPoolACL->setPreSweepFreeBytes(currentFreeBytes);
}

uintptr_t MM_SweepSchemeSegregated::resetCoalesceFreeRegionCount(
    MM_EnvironmentBase *env) {
  return 0;
}

bool MM_SweepSchemeSegregated::updateCoalesceFreeRegionCount(uintptr_t range) {
  return false;
}
...
void MM_SweepSchemeSegregated::incrementalCoalesceFreeRegions(
  ...
  for (uintptr_t i = 0; i < regionCount;) {
    ...
    currentRegion = (MM_HeapRegionDescriptorSegregated *)
                        regionManager->mapRegionTableIndexToDescriptor(i);
...
void MM_SweepSchemeSegregated::incrementalSweepLarge(MM_EnvironmentBase *env) {
...
void MM_SweepSchemeSegregated::incrementalSweepArraylet(
    MM_EnvironmentBase *env) {
  uintptr_t arrayletsPerRegion = env->getExtensions()->arrayletsPerRegion;

  MM_RegionPoolSegregated *regionPool = _memoryPool->getRegionPool();
  MM_HeapRegionQueue *arrayletSweepRegions =
      regionPool->getArrayletSweepRegions();
  MM_HeapRegionQueue *arrayletAvailableRegions =
      regionPool->getArrayletAvailableRegions();
  MM_HeapRegionDescriptorSegregated *currentRegion;

  while ((currentRegion = arrayletSweepRegions->dequeue()) != NULL) {
    sweepRegion(env, currentRegion);

    if (currentRegion->getMemoryPoolACL()->getFreeCount() !=
        arrayletsPerRegion) {
...
uintptr_t MM_SweepSchemeSegregated::resetSweepSmallRegionCount(
    MM_EnvironmentBase *env, uintptr_t yieldSmallRegionCount) {
  return 0;
}

bool MM_SweepSchemeSegregated::updateSweepSmallRegionCount() { return false; }

void MM_SweepSchemeSegregated::incrementalSweepSmall(MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *ext = env->getExtensions();
  bool shouldUpdateOccupancy = ext->nonDeterministicSweep;
  MM_RegionPoolSegregated *regionPool = _memoryPool->getRegionPool();
  uintptr_t splitIndex =
      env->getSlaveID() % (regionPool->getSplitAvailableListSplitCount());

  /*
   * Iterate through the regions so that each region is processed exactly once.
   * Interleaved sweeping: free up some number of regions of all sizeClasses
   * right away so that application have somewhere to allocate from and minimize
   * the usage of non deterministic sweeps. Any marked objects are unmarked. If
   * a region holds a marked object, then the region is kept active; if a region
   * contains no marked objects, then it can be returned to a free list.
   */
  ...
  while (regionPool->getCurrentTotalCountOfSweepRegions()) {
    for (uintptr_t sizeClass = OMR_SIZECLASSES_MIN_SMALL;
      ...
      while (regionPool->getCurrentCountOfSweepRegions(sizeClass)) {
        float yetToComplete =
            (float)regionPool->getCurrentCountOfSweepRegions(sizeClass) /
            regionPool->getInitialCountOfSweepRegions(sizeClass);
        float totalYetToComplete =
            (float)regionPool->getCurrentTotalCountOfSweepRegions() /
        ...
        MM_HeapRegionQueue *sweepList =
            regionPool->getSmallSweepRegions(sizeClass);
        MM_HeapRegionDescriptorSegregated *currentRegion;
        uintptr_t numCells = sizeClasses->getNumCells(sizeClass);
        uintptr_t sweepSmallRegionsPerIteration =
            calcSweepSmallRegionsPerIteration(numCells);
        uintptr_t yieldSlackTime =
            resetSweepSmallRegionCount(env, sweepSmallRegionsPerIteration);
        uintptr_t actualSweepRegions;
        if ((actualSweepRegions =
                 sweepList->dequeue(env->getRegionWorkList(),
                                    sweepSmallRegionsPerIteration)) > 0) {
          regionPool->decrementCurrentCountOfSweepRegions(sizeClass,
                                                          actualSweepRegions);
          regionPool->decrementCurrentTotalCountOfSweepRegions(
              actualSweepRegions);
          uintptr_t freedRegions = 0, processedRegions = 0;
          MM_HeapRegionQueue *fullList = env->getRegionLocalFull();
          while ((currentRegion = env->getRegionWorkList()->dequeue()) !=
                 NULL) {
            sweepRegion(env, currentRegion);
            if (currentRegion->getMemoryPoolACL()->getFreeCount() < numCells) {
              uintptr_t occupancy =
                  (currentRegion->getMemoryPoolACL()->getMarkCount() * 100) /
                  numCells;
