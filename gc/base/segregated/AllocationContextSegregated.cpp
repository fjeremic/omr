...
MM_AllocationContextSegregated::newInstance(
...
{
  memset(
    &_perContextSmallFullRegions[0], 0, sizeof(_perContextSmallFullRegions));
...
void
MM_AllocationContextSegregated::flushSmall(MM_EnvironmentBase* env,
                                           uintptr_t sizeClass)
{
 ...
 */
void
MM_AllocationContextSegregated::returnFullRegionsToRegionPool(
  MM_EnvironmentBase* env)
{
  lockContext();
...
void
MM_AllocationContextSegregated::signalSmallRegionDepleted(
  MM_EnvironmentBase* env,
  uintptr_t sizeClass)
{
  /* No implementation */
...
bool
MM_AllocationContextSegregated::tryAllocateRegionFromSmallSizeClass(
  MM_EnvironmentBase* env,
  uintptr_t sizeClass)
{
...
bool
MM_AllocationContextSegregated::trySweepAndAllocateRegionFromSmallSizeClass(
  MM_EnvironmentBase* env,
  uintptr_t sizeClass,
...
bool
MM_AllocationContextSegregated::tryAllocateFromRegionPool(
  MM_EnvironmentBase* env,
  uintptr_t sizeClass)
{
  MM_HeapRegionDescriptorSegregated* region =
 ...
 */
uintptr_t*
MM_AllocationContextSegregated::preAllocateSmall(MM_EnvironmentBase* env,
                                                 uintptr_t sizeInBytesRequired)
{
  ...
  while (!done) {
    ...
    if (NULL != region) {
      ...
      uintptr_t* cellList =
        memoryPoolACL->preAllocateCells(env,
                                        sizeClasses->getCellSize(sizeClass),
                                        replenishSize,
                                        &preAllocatedBytes);
      if (NULL != cellList) {
        Assert_MM_true(preAllocatedBytes > 0);
        if (shouldPreMarkSmallCells(env)) {
          _markingScheme->preMarkSmallCells(
            env, region, cellList, preAllocatedBytes);
        }
...
retry:
  ...
  region = _regionPool->allocateFromRegionPool(
    env, 1, OMR_SIZECLASSES_ARRAYLET, MAX_UINT);
  if (region != NULL) {
    /* cache the small full region in AC */
    _perContextArrayletFullRegions->enqueue(region);
    _arrayletRegion = region;
                                              ...
                                              uintptr_t sizeInBytesRequired)
{
  ...
  while (region == NULL && excess < MAX_UINT) {
    region = _regionPool->allocateFromRegionPool(
      env, neededRegions, OMR_SIZECLASSES_LARGE, excess);
    excess = (2 * excess) + 1;
  }
