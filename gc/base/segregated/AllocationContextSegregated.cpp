...
bool MM_AllocationContextSegregated::initialize(MM_EnvironmentBase* env)
{
    ...
    if (0
        != omrthread_monitor_init_with_name(
            &_mutexSmallAllocations, 0, "MM_AllocationContextSegregated small allocation monitor")) {
        return false;
    }

    if (0
        != omrthread_monitor_init_with_name(
            &_mutexArrayletAllocations, 0, "MM_AllocationContextSegregated arraylet allocation monitor")) {
        return false;
    }

    for (int32_t i = 0; i < OMR_SIZECLASSES_NUM_SMALL + 1; i++) {
        _smallRegions[i] = NULL;
        /* the small allocation lock needs to be acquired before small full region queue can be accessed, no concurrent
    ...
    /* the arraylet allocation lock needs to be acquired before arraylet full region queue can be accessed, no
     * concurrent access should be possible */
    _perContextArrayletFullRegions
        = MM_RegionPoolSegregated::allocateHeapRegionQueue(env, MM_HeapRegionList::HRL_KIND_FULL, true, false, false);
    ...
    _perContextLargeFullRegions
        = MM_RegionPoolSegregated::allocateHeapRegionQueue(env, MM_HeapRegionList::HRL_KIND_FULL, false, true, false);
...
void MM_AllocationContextSegregated::flushSmall(MM_EnvironmentBase* env, uintptr_t sizeClass)
{
 ...
 */
void MM_AllocationContextSegregated::returnFullRegionsToRegionPool(MM_EnvironmentBase* env)
{
    lockContext();
...
void MM_AllocationContextSegregated::signalSmallRegionDepleted(MM_EnvironmentBase* env, uintptr_t sizeClass)
{
    /* No implementation */
}

bool MM_AllocationContextSegregated::tryAllocateRegionFromSmallSizeClass(MM_EnvironmentBase* env, uintptr_t sizeClass)
{
...
bool MM_AllocationContextSegregated::trySweepAndAllocateRegionFromSmallSizeClass(
...
bool MM_AllocationContextSegregated::tryAllocateFromRegionPool(MM_EnvironmentBase* env, uintptr_t sizeClass)
{
    ...
    if (NULL != region) {
         ...
         * we make it reachable via _smallRegions (_smallRegions is accessed from outside the lock which covers this
         * write)
         */
        MM_AtomicOperations::storeSync();
        _smallRegions[sizeClass] = region;
...
bool MM_AllocationContextSegregated::shouldPreMarkSmallCells(MM_EnvironmentBase* env)
{
    return true;
}
 ...
 */
uintptr_t* MM_AllocationContextSegregated::preAllocateSmall(MM_EnvironmentBase* env, uintptr_t sizeInBytesRequired)
{
    ...
    /* BEN TODO 1429: The object allocation interface base class should define all API used by this method such that
     * casting would be unnecessary. */
    ...
    while (!done) {
        ...
        if (NULL != region) {
            ...
            uintptr_t* cellList = memoryPoolACL->preAllocateCells(
                env, sizeClasses->getCellSize(sizeClass), replenishSize, &preAllocatedBytes);
            if (NULL != cellList) {
...
uintptr_t* MM_AllocationContextSegregated::allocateArraylet(MM_EnvironmentBase* env, omrarrayptr_t parent)
{
...
uintptr_t* MM_AllocationContextSegregated::allocateLarge(MM_EnvironmentBase* env, uintptr_t sizeInBytesRequired)
{
