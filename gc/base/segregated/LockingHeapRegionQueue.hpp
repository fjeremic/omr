...
public:
   ...
   * fairly accurate as long as single region push and pop operations are used
   * and updateCounts doesn't get called on a region that lives on the list.
   */
  MM_LockingHeapRegionQueue(RegionListKind regionListKind,
                            bool singleRegionOnly, bool concurrentAccess,
                            bool trackFreeBytes)
      : MM_HeapRegionQueue(regionListKind, singleRegionOnly, trackFreeBytes),
        _head(NULL), _tail(NULL), _needLock(concurrentAccess),
        _lockMonitor(NULL), _totalRegionsCount(0) {
  ...
  /* check that the receiver is not empty before locking it and performing
   * dequeue */
  ...
  virtual uintptr_t dequeue(MM_HeapRegionQueue *targetAsPQ, uintptr_t count) {
    MM_LockingHeapRegionQueue *target =
        MM_LockingHeapRegionQueue::asLockingHeapRegionQueue(targetAsPQ);
    lock();
    target->lock();
   ...
   */
  MMINLINE static MM_LockingHeapRegionQueue *
  asLockingHeapRegionQueue(MM_HeapRegionQueue *pl) {
...
private:
  ...
  uintptr_t dequeueInternal(MM_LockingHeapRegionQueue *target,
                            uintptr_t count) {
    uintptr_t moved = 0;
    while (count-- > 0) {
