 ...
 */
omrobjectptr_t GC_ObjectHeapIteratorSegregated::nextObject() {
      ...
      while (_scanPtr < _smallPtrTop) {
        if (!_extensions->objectModel.isDeadObject(_scanPtr)) {
          ...
          _scanPtr = (omrobjectptr_t)(
              ((uintptr_t)_scanPtr) +
              _extensions->objectModel.getSizeInBytesDeadObject(_scanPtr));
 ...
 *
 */
omrobjectptr_t GC_ObjectHeapIteratorSegregated::nextObjectNoAdvance() {
  omrobjectptr_t object = NULL;
 ...
 */
void GC_ObjectHeapIteratorSegregated::advance(uintptr_t size) {
 ...
 */
void GC_ObjectHeapIteratorSegregated::reset(uintptr_t *base, uintptr_t *top) {
 ...
 */
void GC_ObjectHeapIteratorSegregated::calculateActualScanPtrTop() {
  if (MM_HeapRegionDescriptor::SEGREGATED_SMALL == _type) {
    uintptr_t cellCount =
        ((uintptr_t)_scanPtrTop - (uintptr_t)_scanPtr) / _cellSize;
