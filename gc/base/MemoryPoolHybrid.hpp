...
class MM_MemoryPoolHybrid : public MM_MemoryPoolSplitAddressOrderedListBase {
 ...
 private:
   ...
   */
  MM_HeapLinkedFreeHeader* internalAllocateFromList(
  ...
  void appendToReservedFreeList(MM_EnvironmentBase* env, void* address,
                                uintptr_t size);
   ...
   */
  void appendToFreeList(MM_EnvironmentBase* env, J9ModronFreeList* freeList,
                        MM_HeapLinkedFreeHeader* freeListTail, void* lowAddress,
                        uintptr_t size);
   ...
   * not coalesce
   */
  uintptr_t coalesceExpandRangeWithLastFreeEntry(
   ...
   * can not coalesce
   */
  uintptr_t coalesceNewFreeEntryWithLastFreeEntry(
      MM_HeapLinkedFreeHeader* lastFreeEntry, J9ModronFreeList* freelist,
      MM_HeapLinkedFreeHeader* newFreeEntry);
   ...
   */
  void moveLastFreeEntryToReservedFreeList(
  ...
  /**
   * this one will try to contract range of free memory from last free entry in
   * the freelist. If the range is larger than last free entry size, contract is
   * ignored, If the range is smaller than last free Entry, create last
   * freeEntry from the remainder.
   *
   * @param[in]      env
   * @param[in]	   freelist
   * @param[in]      contractSize			from lowAddress to
   * highAddress
   * @param[in]      lowAddress			lowAddress of contract range
   * @param[in]      lowAddress			lowAddress of contract range
   * @return 			true if can contract, otherwise false
   */
  bool tryContractWithRangeInFreelist(MM_EnvironmentBase* env,
 ...
 public:
  ...
  /* for MemoryPoolHybrid, expand/contract/redistribute only happened at the end
   * of memoryPool(high address), in order to simplify the code, the below four
   * methods (addFreeEntries, removeFreeEntriesWithinRange, expandWithRange,
   * contractWithRange) only handle expand/contract/redistribute at the end of
   * the memory pool.
   */
   ...
   */
  MM_MemoryPoolHybrid(MM_EnvironmentBase* env, uintptr_t minimumFreeEntrySize,
                      uintptr_t splitAmount)
      : MM_MemoryPoolSplitAddressOrderedListBase(env, minimumFreeEntrySize,
                                                 splitAmount) {
  ...
  MM_MemoryPoolHybrid(MM_EnvironmentBase* env, uintptr_t minimumFreeEntrySize,
                      uintptr_t splitAmount, const char* name)
      : MM_MemoryPoolSplitAddressOrderedListBase(env, minimumFreeEntrySize,
                                                 splitAmount, name) {
    _heapFreeListCountExtended = _heapFreeListCount + 1;
    _typeId = __FUNCTION__;
