...
protected:
  MM_GCExtensionsBase
      *const _extensions; /**< A cache of the global extensions */
  uintptr_t
      _managedAllocationContextCount; /**< The number of allocation contexts
                                         which will be instantiated and managed
                                         by the GlobalAllocationManager that are
                                         part of the _managedAllocationContexts
                                         array, other allocation contexts may
                                         exist in certain specs */
...
public:
  ...
  /**
   * Acquires an allocation context for the thread represented by the given env
   * (modifies the env)
   ...
   */
  virtual void printAllocationContextStats(MM_EnvironmentBase *env,
                                           uintptr_t eventNum,
                                           J9HookInterface **hookInterface) = 0;

  /**
   * Instructs the receiver to call flush on all the contexts it manages (meant
   * to be called prior to collect).
   */
  virtual void flushAllocationContexts(MM_EnvironmentBase *env);

  /**
   * Instructs the receiver to call flushForShutdown on all the contexts it
   * manages (meant to be called prior to shut down).
   */
...
protected:
  ...
  MM_GlobalAllocationManager(MM_EnvironmentBase *env)
      : MM_BaseVirtual(), _extensions(env->getExtensions()),
        _managedAllocationContextCount(0), _nextAllocationContext(0),
