 ...
 */
MM_HeapSplit*
MM_HeapSplit::newInstance(MM_EnvironmentBase* env,
...
{
  MM_HeapSplit* heap =
    (MM_HeapSplit*)env->getForge()->allocate(sizeof(MM_HeapSplit),
  ...
  if (NULL != heap) {
    ...
    if (!heap->initialize(
          env, heapAlignment, lowExtentSize, highExtentSize, regionManager)) {
      heap->kill(env);
      heap = NULL;
    }
  }
...
bool
MM_HeapSplit::initialize(MM_EnvironmentBase* env,
                         uintptr_t heapAlignment,
                         uintptr_t lowExtentSize,
                         uintptr_t highExtentSize,
                         MM_HeapRegionManager* regionManager)
...
void*
MM_HeapSplit::doubleMapArraylet(MM_EnvironmentBase* env,
                                void* arrayletLeaves[],
 ...
 */
bool
MM_HeapSplit::attachArena(MM_EnvironmentBase* env,
 ...
 */
bool
MM_HeapSplit::decommitMemory(void* address,
                             uintptr_t size,
                             ...
                             void* highValidAddress)
{
  ...
  if (_lowExtent->getHeapBase() == address) {
    Assert_MM_true(_lowExtent->getMaximumPhysicalRange() == size);
    success = _lowExtent->decommitMemory(
      address, size, lowValidAddress, highValidAddress);
  } else if (_highExtent->getHeapBase() == address) {
    Assert_MM_true(_highExtent->getMaximumPhysicalRange() == size);
    success = _highExtent->decommitMemory(
      address, size, lowValidAddress, highValidAddress);
  } else {
    /* This is neither range so fail */
    Assert_MM_true(false);
...
bool
MM_HeapSplit::initializeHeapRegionManager(MM_EnvironmentBase* env,
                                          MM_HeapRegionManager* manager)
{
  if (!manager->setContiguousHeapRange(
        env, _lowExtent->getHeapBase(), _highExtent->getHeapTop())) {
    return false;
  }
