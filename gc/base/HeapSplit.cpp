 ...
 */
MM_HeapSplit* MM_HeapSplit::newInstance(MM_EnvironmentBase* env, uintptr_t heapAlignment, uintptr_t lowExtentSize,
...
bool MM_HeapSplit::initialize(MM_EnvironmentBase* env, uintptr_t heapAlignment, uintptr_t lowExtentSize,
    uintptr_t highExtentSize, MM_HeapRegionManager* regionManager)
{
    ...
    if (success) {
        ...
        /* only the high extent can require overflow rounding (this will fail in ways which aren't helpful for debugging
         * on the low extent) so ensure that we only set that extension flag when initializing the high extent (of
        ...
        /* successful initialization of split heaps is defined as success in initializing both heap extents and them
         * being in the right order */
        success
            = (NULL != _lowExtent) && (NULL != _highExtent) && (_lowExtent->getHeapBase() < _highExtent->getHeapBase());
        ...
        if (!success) {
            ...
            if (NULL == _lowExtent) {
                extensions->heapInitializationFailureReason
                    = MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_CAN_NOT_INSTANTIATE_SPLIT_HEAP_OLD_SPACE;
            } else if (NULL == _highExtent) {
                extensions->heapInitializationFailureReason
                    = MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_CAN_NOT_INSTANTIATE_SPLIT_HEAP_NEW_SPACE;
            } else {
                extensions->heapInitializationFailureReason
                    = MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_CAN_NOT_INSTANTIATE_SPLIT_HEAP_GEOMETRY;
            }
...
void* MM_HeapSplit::doubleMapArraylet(MM_EnvironmentBase* env, void* arrayletLeaves[], UDATA arrayletLeafCount,
 ...
 */
void MM_HeapSplit::detachArena(MM_EnvironmentBase* env, MM_PhysicalArena* arena)
{
 ...
 */
bool MM_HeapSplit::attachArena(MM_EnvironmentBase* env, MM_PhysicalArena* arena, uintptr_t size)
{
    /* Since we don't presently support any notion of dynamic resizing of the heap, use this assertion to make sure that
     * we don't accidentally try to use that functionality */
    ...
    /* due to overflow rounding, we may ask for less than the entire physical region so ensure that we account for the
     * gap when calculating the high address. */
...
bool MM_HeapSplit::commitMemory(void* address, uintptr_t size)
{
    ...
    /* these need to be a perfect fit so assert that the committed size is equal to the total size of the region at that
 ...
 * @note This is a bit of a strange function to have as public API.  Should it be removed?
 */
bool MM_HeapSplit::decommitMemory(void* address, uintptr_t size, void* lowValidAddress, void* highValidAddress)
{
    bool success = false;
...
bool MM_HeapSplit::initializeHeapRegionManager(MM_EnvironmentBase* env, MM_HeapRegionManager* manager)
{
