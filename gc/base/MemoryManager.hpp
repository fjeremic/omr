...
class MM_MemoryManager : public MM_BaseNonVirtual {
   ...
   */
 private:
  MM_MemoryHandle _preAllocated; /**< stored preallocated memory parameters in
 ...
 private:
   ...
   * An idea is to take entire memory segment (reserved any way - over-allocate
   * strategy) and use this memory later for GC Metadata allocations
   *
 ...
 protected:
  ...
  MM_MemoryManager(MM_EnvironmentBase* env) : _preAllocated() {
 ...
 public:
  ...
  bool createVirtualMemoryForHeap(MM_EnvironmentBase* env,
                                  ...
                                  uintptr_t tailPadding, void* preferredAddress,
                                  void* ceiling);

  /**
   * Creates the correct type of VirtualMemory object for the current platform
   * and configuration
   *
   * @param env environment
   * @param[in/out] handle pointer to memory handle
  ...
  /**
   * Destroy virtual memory instance, plus everything that is heap specific (for
   * example, shadow heap)
   *
   * @param env environment
   * @param[in/out] handle pointer to memory handle
   */
  void destroyVirtualMemoryForHeap(MM_EnvironmentBase* env,
                                   MM_MemoryHandle* handle);

  /**
   * Double maps arraylets, arrays that does not fit into one region are split
   * into leaves, which are then double mapped by this function
   *
   ...
   * @param byteAmount, total byte amount to be allocate contiguous block of
   * meory to double map
   * @param newIdentifier, hold information of newly created contiguous block of
   * memory
   * @param pageSize
   * @param category
   */
#if defined(OMR_GC_DOUBLE_MAP_ARRAYLETS)
  void* doubleMapArraylet(MM_MemoryHandle* handle, MM_EnvironmentBase* env,
                          void* arrayletLeaves[], UDATA arrayletLeafCount,
   ...
   *
   */
  bool decommitMemory(MM_MemoryHandle* handle, void* address, uintptr_t size,
   ...
   */
  bool setNumaAffinity(const MM_MemoryHandle* handle, uintptr_t numaNode,
  ...
  /**
   * Calculate the size of the range from the supplied address to the top of the
   * heap.
   *
   * @param handle pointer to memory handle
   ...
   */
  MMINLINE uintptr_t calculateOffsetToHeapTop(MM_MemoryHandle* handle,
                                              void* address) {
   ...
   */
  MMINLINE uintptr_t calculateOffsetFromHeapBase(MM_MemoryHandle* handle,
                                                 void* address) {
