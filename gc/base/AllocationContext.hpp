...
public:
  ...
  /**
   * Instructs the receiver to invalidate its cache in preparation for shutting
   * down.
   */
   ...
   * @note No collect and climb will occur - the allocation must be satisfied
   * within the specified location and will report a failure otherwise
   ...
   */
  virtual void *
  allocate(MM_EnvironmentBase *env,
  ...
  /**
   * Ideally, this would only be understood by sub-classes which know about TLH
   * allocation but we will use runtime assertions to ensure this is safe, for
   * now
   */
  virtual void *
  allocateTLH(MM_EnvironmentBase *env,
              MM_AllocateDescription *allocateDescription,
              MM_ObjectAllocationInterface *objectAllocationInterface,
  ...
  virtual void *
  allocateArrayletLeaf(MM_EnvironmentBase *env,
                       MM_AllocateDescription *allocateDescription,
                       bool shouldCollectOnFailure) {
    Assert_MM_unreachable();
    return NULL;
