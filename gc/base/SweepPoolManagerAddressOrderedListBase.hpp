...
private:
protected:
  MMINLINE void calculateTrailingDetails(MM_ParallelSweepChunk *sweepChunk,
                                         uintptr_t *trailingCandidate,
                                         uintptr_t trailingCandidateSlotCount);
  MMINLINE virtual void connectChunkPostProcess(
      MM_ParallelSweepChunk *chunk, MM_SweepPoolState *sweepState,
      MM_HeapLinkedFreeHeader *splitCandidate,
      MM_HeapLinkedFreeHeader *splitCandidatePreviousEntry) {}
  MMINLINE void
  updateLargestFreeEntryInChunk(MM_ParallelSweepChunk *chunk,
    ...
    if (chunk->_largestFreeEntry > sweepState->_largestFreeEntry) {
      ...
      if (NULL == chunk->_previousLargestFreeEntry) {
        ...
        sweepState->_previousLargestFreeEntry =
            chunk->_previousLargestFreeEntry;
      }
...
public:
  ...
  virtual void flushFinalChunk(MM_EnvironmentBase *envModron,
                               MM_MemoryPool *memoryPool);
  virtual void connectFinalChunk(MM_EnvironmentBase *envModron,
                                 MM_MemoryPool *memoryPool);
  virtual void poolPostProcess(MM_EnvironmentBase *envModron,
                               MM_MemoryPool *memoryPool) {}
