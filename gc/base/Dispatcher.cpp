...
MM_Dispatcher *MM_Dispatcher::newInstance(MM_EnvironmentBase *env) {
  MM_Dispatcher *dispatcher;

  dispatcher = (MM_Dispatcher *)env->getForge()->allocate(
      sizeof(MM_Dispatcher), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (dispatcher) {
...
bool MM_Dispatcher::initialize(MM_EnvironmentBase *env) { return true; }

void MM_Dispatcher::prepareThreadsForTask(MM_EnvironmentBase *env,
                                          MM_Task *task,
...
void MM_Dispatcher::run(MM_EnvironmentBase *env, MM_Task *task,
  ...
  uintptr_t activeThreads =
      recomputeActiveThreadCountForTask(env, task, newThreadCount);
...
bool MM_Dispatcher::startUpThreads() { return true; }

void MM_Dispatcher::shutDownThreads() {}

uintptr_t MM_Dispatcher::recomputeActiveThreadCountForTask(
