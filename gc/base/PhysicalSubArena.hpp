...
protected:
  ...
  bool _resizable; /**< Flag whether the instance can be resized
                      (expanded/contracted) */
...
protected:
  ...
  /* a helper to get the root region manager from the heap, since it was needed
   * in so many place */
  MM_HeapRegionManager *getHeapRegionManager() {
    return _heap->getHeapRegionManager();
  }
...
public:
  ...
  virtual uintptr_t
  checkCounterBalanceExpand(MM_EnvironmentBase *env,
  ...
  MMINLINE void setSubSpace(MM_MemorySubSpace *subSpace) {
    _subSpace = subSpace;
  }

  MMINLINE void setParent(MM_PhysicalArena *parent) { _parent = parent; }

  virtual void tilt(MM_EnvironmentBase *env, uintptr_t allocateSpaceSize,
                    uintptr_t survivorSpaceSize, bool updateMemoryPools = true);
  virtual void tilt(MM_EnvironmentBase *env,
                    uintptr_t survivorSpaceSizeRequest);
  ...
  /**
   * Set whether resizing is available on the receiver and return the previous
   * value.
  ...
  MM_PhysicalSubArena(MM_Heap *heap)
      : MM_BaseVirtual(), _parent(NULL), _subSpace(NULL), _heap(heap),
        _resizable(true) {
