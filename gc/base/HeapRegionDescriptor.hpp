 ...
 * For Tarok, the heap is divided into a number of regions, each with
 * potentially many different properties (e.g., object size class, node
 * affinity, etc).  The heap region descriptor is found in the heap region table
...
public:
  enum RegionType {
    ...
    SEGREGATED_LARGE =
        3,             /**< Represents spanning regions that contain objects */
    ...
    ADDRESS_ORDERED_IDLE =
        6, /**< Represents regions that have an AOL MemoryPool but are not
              actively being used (that is, they can be made into other kinds of
              regions without penalty) */
    ADDRESS_ORDERED_MARKED =
        7, /**< Represents regions that have an AOL MemoryPool and an accurate
              mark set in extensions->previousMarkMap */
    BUMP_ALLOCATED =
        8, /**< Represents regions that have a BumpPointer MemoryPool */
    BUMP_ALLOCATED_IDLE =
        9, /**< Represents regions that have a BumpPointer MemoryPool but are
              not actively being used (that is, they can be made into other
              kinds of regions without penalty) */
    BUMP_ALLOCATED_MARKED =
        10, /**< Represents regions that have a BumpPointer MemoryPool and an
               accurate mark set in extensions->previousMarkMap */
    LAST_REGION_TYPE /**< This dummy type is sed later as a size of an array for
                        stats of region counts */
  };
  uintptr_t _regionsInSpan; /**< most likely a temporary piece of data which,
                               along with _nextInSet, builds a freelist (public
                               for assertions) */
  MM_HeapRegionDescriptor
      *_headOfSpan; /**< in a spanning region, points to the HEAD of the span.
                       In non-spanning, points at this */
  void *_heapRegionDescriptorExtension; /**< Language-specific extension of per
                                           region descriptor can be attached by
                                           configuration delegate
                                           (MM_ConfigurationDelegate) */

  /**
   * The values of the members in this enum will be ORed into @ref
   * _regionProperties as flags they should therefore be powers of 2 which don't
   * exceed the range of @ref _regionProperties
   */
  enum RegionProperties { ALL = 0xFFFFFFFF, MANAGED = 1 };
...
private:
  ...
  MM_HeapRegionDescriptor
      *_previousRegion; /**< previous region descriptor in the list (used by
                           HeapRegionManager) */
  MM_HeapRegionDescriptor *_nextRegion; /**< next region descriptor in the list
                                           (used by HeapRegionManager) */

  MM_HeapRegionDescriptor
      *_previousRegionInSubSpace; /**< previous region descriptor in the list
                                     (used by MM_MemorySubSpaceGeneric) */
  MM_HeapRegionDescriptor
      *_nextRegionInSubSpace; /**< next region descriptor in the list (used by
                                 MM_MemorySubSpaceGeneric) */

  MM_HeapRegionDescriptor
      *_nextInSet;   /**< most likely a temporary link which is used for passing
                        around ad-hoc "collections" of regions */
  bool _isAllocated; /**< This will become an enumeration of more interesting
                        region states once we have them */

  MM_MemorySubSpace
      *_memorySubSpace; /**< the memory subspace associated with this region */

  RegionType _regionType;     /**< This regions current type */
  MM_MemoryPool *_memoryPool; /**< The memory pool associated with this region.
                                 This may be NULL */
...
public:
   ...
   */
  MMINLINE void setRegionProperty(RegionProperties property) {
    _regionProperties |= property;
  }
   ...
   */
  MMINLINE void clearRegionProperty(RegionProperties property) {
    _regionProperties &= ~property;
  }
  ...
  /**
   * Test whether or not a certain property is set in the @ref _regionProperties
   * bitmap
   * @param[in] property The RegionProperties property to test
   *
   * @return True if the property is set, false otherwise
   */
  MMINLINE bool regionPropertyIsSet(RegionProperties property) {
    return (0 != (getRegionProperties() & property));
  }
  ...
  void *getHighAddress() const {
    ...
    /* Note that we can't assert that this is the same high address as our root
     * backing since, when a spanning region is being released, a root backing
     * becomes temporarily out of sync with a descriptor built on it
     */
    return result;
  }
   ...
   */
  bool isAddressInRegion(const void *address) {
   ...
   */
  MM_MemorySubSpace *getSubSpace() { return _memorySubSpace; }

  /**
   * A helper to request the type flags from the Subspace associated with the
   * receiving region.
   *
   * @return The type flags of the Subspace associated with this region
   */
  uintptr_t getTypeFlags() {
  ...
  RegionType getRegionType() { return _regionType; }

  void setMemoryPool(MM_MemoryPool *memoryPool) { _memoryPool = memoryPool; }

  MM_MemoryPool *getMemoryPool() { return _memoryPool; }

  /**
   * @return True if the memory backing this region is committed
   */
  bool isCommitted() {
    /* currently, we only check to see if we have set a subspace but this will
     * eventually be a meta-data flag */
   ...
   */
  void setNumaNode(uintptr_t numaNode) { _numaNode = numaNode; }

  /**
   * @return the NUMA node the memory in the region is associated with
   */
  uintptr_t getNumaNode() { return _numaNode; }
  ...
  /**
   * Sets a BUMP_ALLOCATED region to BUMP_ALLOCATED_MARKED.  Asserts if called
   * on any other region type. This can be extended to other region types which
   * have a "marked" variant as they are needed.
   */
  ...
  /**
   * Allocate supporting resources (large enough to justify not to preallocate
   * them for all regions at the startup) when region is being committed.
  ...
  static bool initializer(MM_EnvironmentBase *env,
                          ...
                          MM_HeapRegionDescriptor *descriptor, void *lowAddress,
                          void *highAddress);
...
protected:
  void reinitialize(MM_EnvironmentBase *env, void *lowAddress,
                    void *highAddress);
