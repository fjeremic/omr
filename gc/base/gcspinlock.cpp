 ...
 */
intptr_t
omrgc_spinlock_acquire(J9GCSpinlock* spinlock,
                       J9ThreadMonitorTracing* lockTracing)
{
  ...
  for (; spinCount3 > 0; spinCount3--) {
    for (spinCount2 = spinlock->spinCount2; spinCount2 > 0; spinCount2--) {
      if (oldValue == *target) {
        ...
        if (oldValue == (intptr_t)MM_AtomicOperations::lockCompareExchange(
                          (volatile uintptr_t*)target, oldValue, newValue)) {
          result = 0;
          goto done;
        }
      }
      ...
      /* begin tight loop */
      for (uintptr_t spinCount1 = spinlock->spinCount1; spinCount1 > 0;
           spinCount1--) {
  ...
  for (;;) {
    ...
    result = (intptr_t)MM_AtomicOperations::lockCompareExchange(
      (volatile uintptr_t*)target, oldValue, newValue);
    if (oldValue == result) {
      break;
    }
...
#if defined(OMR_THR_JLM)
  if (tracing != NULL) {
    /* if tracing counts are ever shared among multiple spinlocks, the two spin
     * count updates should be atomic */
...
#endif /* OMR_THR_JLM */
  /* On out-of-order memory models (e.g. Power4), ensure that all reads and
   * writes have been completed at this point */
