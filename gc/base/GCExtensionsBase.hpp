...
private:
    ...
    volatile BackOutState _backOutState; /**< set if a thread is unable to copy an object due to lack of free space in
                                            both Survivor and Tenure */
    volatile bool _concurrentGlobalGCInProgress; /**< set to true if concurrent Global GC is in progress */
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    bool debugConcurrentScavengerPageAlignment; /**< if true allows debug output prints for Concurrent Scavenger Page
                                                   Alignment logic */
    uintptr_t concurrentScavengerPageSectionSize; /**< selected section size for Concurrent Scavenger Page */
    void* concurrentScavengerPageStartAddress; /**< start address for Concurrent Scavenger Page, UDATA_MAX if it is not
                                                  initialized */
...
public:
    ...
    void* _masterThreadTenureTLHRemainderBase; /**< base and top pointers of the last unused tenure TLH copy cache, that
                                                  will be loaded to thread env during master setup */
...
#if defined(OMR_GC_MODRON_SCAVENGER)
    MM_ScavengerStats incrementScavengerStats; /**< scavengerStats for the current phase/increment; typically just used
                                                  for reporting purposes */
    MM_ScavengerStats
        scavengerStats; /**< cumulative scavengerStats for all phases/increments (STW and concurrent) within a single
                           cycle; typically used for various heursitics at the end of GC */
    MM_ScavengerCopyScanRatio
        copyScanRatio; /* Most recent estimate of ratio of aggregate slots copied to slots scanned in completeScan() */
#endif /* OMR_GC_MODRON_SCAVENGER */
    ...
    float concurrentKickoffTenuringHeadroom; /**< percentage of free memory remaining in tenure heap. Used in
                                                conjunction with free memory to determine concurrent mark kickoff */
    float tenureBytesDeviationBoost; /**< boost factor for tenuring deviation used for concurrent mark kickoff math */
#endif /* OMR_GC_MODRON_SCAVENGER */
#if defined(OMR_GC_REALTIME)
    MM_RememberedSetSATB* sATBBarrierRememberedSet; /**< The snapshot at the beginning barrier remembered set used for
                                                       the write barrier */
#endif /* defined(OMR_GC_REALTIME) */
    ...
    uint32_t estimateFragmentation; /**< Enable estimate fragmentation, NO_ESTIMATE_FRAGMENTATION,
                                       LOCALGC_ESTIMATE_FRAGMENTATION, GLOBALGC_ESTIMATE_FRAGMENTATION(default) */
    bool processLargeAllocateStats; /**< Enable process LargeObjectAllocateStats */
    uintptr_t largeObjectAllocationProfilingThreshold; /**< object size threshold above which the object is large enough
                                                          for allocation profiling */
    uintptr_t largeObjectAllocationProfilingVeryLargeObjectThreshold; /**< object size threshold above which the object
                                                                         is large enough for recording precise size in
                                                                         allocation profiling */
    uintptr_t
        largeObjectAllocationProfilingVeryLargeObjectSizeClass; /**< index of sizeClass for minimum veryLargeEntry*/
    uint32_t largeObjectAllocationProfilingSizeClassRatio; /**< ratio of lower and upper boundary of a size class in
                                                              large object allocation profiling */
    uint32_t largeObjectAllocationProfilingTopK; /**< number of most allocation size we want to track/report in large
                                                    object allocation profiling */
    MM_FreeEntrySizeClassStats freeEntrySizeClassStatsSimulated; /**< snapshot of free memory status used for simulated
                                                                    allocator for fragmentation estimation */
    uintptr_t freeMemoryProfileMaxSizeClasses; /**< maximum number of sizeClass maintained for heap free memory profile
                                                  (computed from SizeClassRatio) */

    volatile OMR_VMThread* gcExclusiveAccessThreadId; /**< thread token that represents the current "winning" thread for
                                                         performing garbage collection */
    omrthread_monitor_t gcExclusiveAccessMutex; /**< Mutex used for acquiring gc priviledges as well as for signalling
                                                   waiting threads that GC has been completed */
    ...
    uintptr_t tlhSurvivorDiscardThreshold; /**< below this size GC (Scavenger) will discard survivor copy cache TLH, if
                                              alloc not succeeded (otherwise we reuse memory for next TLH) */
    uintptr_t tlhTenureDiscardThreshold; /**< below this size GC (Scavenger) will discard tenure copy cache TLH, if
                                            alloc not succeeded (otherwise we reuse memory for next TLH) */
    ...
    uintptr_t splitFreeListNumberChunksPrepared; /**< Used in MPSAOL postProcess. Shared for all MPSAOLs. Do not
                                                    overwrite during postProcess for any MPSAOL. */
    bool enableHybridMemoryPool;

    bool largeObjectArea;
#if defined(OMR_GC_LARGE_OBJECT_AREA)
    typedef enum { METER_BY_SOA = 0, METER_BY_LOA, METER_DYNAMIC } ConcurrentMetering;
    ...
    float darkMatterCompactThreshold; /**< Value used to trigger compaction when dark matter ratio reaches this
                                         percentage of memory pools memory*/
    ...
    uintptr_t heapExpansionStabilizationCount; /**< GC count required before the heap is allowed to expand due to
                                                  excessvie time after last heap expansion */
    uintptr_t heapContractionStabilizationCount; /**< GC count required before the heap is allowed to contract due to
                                                    excessvie time after last heap expansion */
    ...
    uintptr_t workpacketCount; /**< this value is ONLY set if -Xgcworkpackets is specified - otherwise the workpacket
                                  count is determined heuristically */
    uintptr_t packetListSplit; /**< the number of ways to split packet lists, set by -XXgc:packetListLockSplit=, or
                                  determined heuristically based on the number of GC threads */
    uintptr_t cacheListSplit; /**< the number of ways to split scanCache lists, set by -XXgc:cacheListLockSplit=, or
                                 determined heuristically based on the number of GC threads */

    uintptr_t markingArraySplitMaximumAmount; /**< maximum number of elements to split array scanning work in marking
                                                 scheme */
    uintptr_t markingArraySplitMinimumAmount; /**< minimum number of elements to split array scanning work in marking
                                                 scheme */

    bool rootScannerStatsEnabled; /**< Enable/disable recording of performance statistics for the root scanner.
                                     Defaults to false. */
    bool rootScannerStatsUsed; /**< Flag that indicates if rootScannerStats are used for in the last increment (by any
                                  thread, for any of its roots) */
    ...
    bool fvtest_alwaysApplyOverflowRounding; /**< always round down the allocated heap as if overflow rounding were
                                                required */
    uintptr_t fvtest_forceExcessiveAllocFailureAfter; /**< force excessive GC to occur after this many global GCs */
    void* fvtest_verifyHeapAbove; /**< if non-NULL, will force start-up failure if any part of the heap is below this
                                     value */
    void* fvtest_verifyHeapBelow; /**< if non-NULL, will force start-up failure if any part of the heap is above this
                                     value */
#if defined(OMR_GC_VLHGC)
    bool fvtest_tarokVerifyMarkMapClosure; /**< True if the collector should verify that the new mark map defines a
                                              consistent and closed object graph after a GMP finishes creating it */
#endif /* defined(OMR_GC_VLHGC) */
    bool fvtest_disableInlineAllocation; /**< True if inline allocation should be disabled (i.e. force out-of-line
                                            paths) */

    uintptr_t fvtest_forceSweepChunkArrayCommitFailure; /**< Force failure at Sweep Chunk Array commit operation */
    uintptr_t fvtest_forceSweepChunkArrayCommitFailureCounter; /**< Force failure at Sweep Chunk Array commit operation
                                                                  counter */
#if defined(OMR_ENV_DATA64) && defined(OMR_GC_FULL_POINTERS)
    uintptr_t fvtest_enableReadBarrierVerification; /**< Forces failure at all direct memory read sites */
    uintptr_t fvtest_enableMonitorObjectsReadBarrierVerification; /**< Forces failure at all direct memory read sites
                                                                     for monitor slot objects */
    uintptr_t fvtest_enableClassStaticsReadBarrierVerification; /**< Forces failure at all direct memory read sites for
                                                                   class statics */
    uintptr_t fvtest_enableJNIGlobalWeakReadBarrierVerification; /**< Forces failure at all direct memory read sites for
                                                                    JNI Global weak references */
    uintptr_t fvtest_enableHeapReadBarrierVerification; /**< Forces failure at all direct memory read sites for heap
                                                           references */
#endif /* defined(OMR_ENV_DATA64) && defined(OMR_GC_FULL_POINTERS) */
    ...
    uintptr_t fvtest_forceReferenceChainWalkerMarkMapCommitFailure; /**< Force failure at Reference Chain Walker Mark
                                                                       Map commit operation */
    uintptr_t fvtest_forceReferenceChainWalkerMarkMapCommitFailureCounter; /**< Force failure at Reference Chain Walker
                                                                              Mark Map commit operation counter */

    uintptr_t fvtest_forceCopyForwardHybridRatio; /**< Force to run CopyForward Hybrid mode value = 1-100 the percentage
                                                     of non evacuated eden regions */
    uintptr_t softMx; /**< set through -Xsoftmx, depending on GC policy this number might differ from available heap
                         memory, use MM_Heap::getActualSoftMxSize for calculations */
    ...
    bool gcThreadCountForced; /**< true if number of GC threads is specified in java options. Currently we have a few
                                 ways to do this: -Xgcthreads		-Xthreads= (RT only)	-XthreadCount= */
    ...
    double scvTenureStrategySurvivalThreshold; /**< The survival threshold (from 0.0 to 1.0) used for deciding to tenure
                                                  particular ages. */
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    bool softwareRangeCheckReadBarrier; /**< enable software read barrier instead of hardware guarded loads when running
                                           with CS */
    bool concurrentScavenger; /**< CS enabled/disabled flag */
    bool concurrentScavengerForced; /**< set to true if CS is requested (by cmdline option), but there are more checks
                                       to do before deciding whether the request is to be obeyed */
    bool concurrentScavengerHWSupport; /**< set to true if CS runs with HW support */
    uintptr_t concurrentScavengerBackgroundThreads; /**< number of background GC threads during concurrent phase of
                                                       Scavenge */
    bool concurrentScavengerBackgroundThreadsForced; /**< true if concurrentScavengerBackgroundThreads set via command
                                                        line option */
    uintptr_t concurrentScavengerSlack; /**< amount of bytes added on top of avearge allocated bytes during concurrent
                                           cycle, in calcualtion for survivor size */
    float concurrentScavengerAllocDeviationBoost; /**< boost factor for allocate rate and its deviation, used for tilt
                                                     calcuation in Concurrent Scavenger */
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
    uintptr_t scavengerFailedTenureThreshold;
    uintptr_t maxScavengeBeforeGlobal;
    uintptr_t
        scvArraySplitMaximumAmount; /**< maximum number of elements to split array scanning work in the scavenger */
    uintptr_t
        scvArraySplitMinimumAmount; /**< minimum number of elements to split array scanning work in the scavenger */
    uintptr_t scavengerScanCacheMaximumSize; /**< maximum size of scan and copy caches before rounding, zero (default)
                                                means calculate them */
    uintptr_t scavengerScanCacheMinimumSize; /**< minimum size of scan and copy caches before rounding, zero (default)
                                                means calculate them */
    bool tiltedScavenge;
    ...
    double scavengerCollectorExpandRatio; /**< the ratio of _avgTenureBytes we use to expand when a collectorAllocate()
                                             fails */
    uintptr_t scavengerMaximumCollectorExpandSize; /**< the maximum amount by which we will expand when a
    ...
    bool enableSplitHeap; /**< true if we are using gencon with -Xgc:splitheap (we will fail to boostrap if we can't
                             allocate both ranges) */
    double aliasInhibitingThresholdPercentage; /**< percentage of threads that can be blocked before copy cache aliasing
                                                  is inhibited (set through aliasInhibitingThresholdPercentage=) */
...
#endif /* OMR_GC_MODRON_SCAVENGER */
#endif /* OMR_GC_MODRON_SCAVENGER || OMR_GC_VLHGC */
    double globalMaximumContraction; /**< maximum percentage of committed global heap which can contract in one GC cycle
                                        (set through -Xgc:globalMaximumContraction=) */
    double globalMinimumContraction; /**< minimum percentage of committed global heap which can contract in one GC cycle
                                        (set through -Xgc:globalMinimumContraction=) */

    /* global variables for excessiveGC functionality */
    MM_UserSpecifiedParameterBool excessiveGCEnabled; /**< should we check for excessiveGC? (set through
                                                         -XdisableExcessiveGC and -XenableExcessiveGC) */
    bool isRecursiveGC; /**< is the current executing gc a result of another gc (ie: scavenger triggering a global
                           collect) */
    ...
    MM_SweepHeapSectioning*
        sweepHeapSectioning; /**< Reference to the SweepHeapSectioning to Compact can share the backing store */
    ...
    double concurrentSlackFragmentationAdjustmentWeight; /**< weight(from 0.0 to 5.0) used for calculating free tenure
                                                            space (how much percentage of the fragmentation need to
    ...
    UDATA
    fvtest_concurrentCardTablePreparationDelay; /**< Delay for concurrent card table preparation in milliseconds */

    UDATA
    fvtest_forceConcurrentTLHMarkMapCommitFailure; /**< Force failure at Concurrent TLH Mark Map commit operation */
    UDATA fvtest_forceConcurrentTLHMarkMapCommitFailureCounter; /**< Force failure at Concurrent TLH Mark Map commit
                                                                   operation counter */
    UDATA fvtest_forceConcurrentTLHMarkMapDecommitFailure; /**< Force failure at Concurrent TLH Mark Map decommit
                                                              operation */
    UDATA fvtest_forceConcurrentTLHMarkMapDecommitFailureCounter; /**< Force failure at Concurrent TLH Mark Map decommit
                                                                     operation  counter */
    ...
    MM_HeapMap* previousMarkMap; /**< the previous valid mark map. This can be used to walk marked objects in regions
                                    which have _markMapUpToDate set to true */
    ...
    uintptr_t
        managedAllocationContextCount; /**< The number of allocation contexts which will be instantiated and managed by
                                          the GlobalAllocationManagerRealtime (currently 2*cpu_count) */
    ...
    uint32_t distanceToYieldTimeCheck; /**< Number of condYield that can be skipped before actual checking for yield,
                                          when the quanta time has been relaxed */
    uintptr_t traceCostToCheckYield; /**< tracing cost (in number of objects marked and pointers scanned) after we try
                                        to yield */
    uintptr_t sweepCostToCheckYield; /**< weighted count of free chunks/marked objects before we check yield in sweep
                                        small loop */
    uintptr_t
        splitAvailableListSplitAmount; /**< Number of split available lists per size class, per defragment bucket */
    ...
    uintptr_t debug;
    uintptr_t allocationTrackerMaxTotalError; /**< The total maximum desired error for the free bytes approximation, the
                                                 larger the number, the lower the contention and vice versa */
    uintptr_t allocationTrackerMaxThreshold; /**< The maximum threshold for a single allocation tracker */
    uintptr_t allocationTrackerFlushThreshold; /**< The flush threshold to be used for all allocation trackers, this
                                                  value is adjusted every time a new thread is created/destroyed */
    ...
    MM_ConfigurationOptions
        configurationOptions; /**< holds the options struct, used during startup for selecting a Configuration */
    ...
    bool bufferedLogging; /**< Enabled by -Xgc:bufferedLogging.  Use buffered filestreams when writing logs (e.g.
                             verbose:gc) to a file */
    ...
    uintptr_t heapCeiling; /**< the highest point in memory where objects can be addressed (used for the -Xgc:lowMemHeap
                              option) */
    ...
    HeapInitializationFailureReason heapInitializationFailureReason; /**< Error code provided additional information
                                                                        about heap initialization failure */
    bool scavengerAlignHotFields; /**< True if the scavenger is to check the hot field description for an object in
                                     order to better cache align it when tenuring (enabled with the -Xgc:hotAlignment
                                     option) */
    uintptr_t suballocatorInitialSize; /**< the initial chunk size in bytes for the J9Heap suballocator (enabled with
                                          the -Xgc:suballocatorInitialSize option) */
    uintptr_t suballocatorCommitSize; /**< the commit size in bytes for the J9Heap suballocator (enabled with the
                                         -Xgc:suballocatorCommitSize option) */

#if defined(OMR_GC_COMPRESSED_POINTERS)
    bool shouldAllowShiftingCompression; /**< temporary option to enable compressed reference scaling by shifting
                                            pointers */
    bool shouldForceSpecifiedShiftingCompression; /**< temporary option to enable forcedShiftingCompressionAmount */
    uintptr_t forcedShiftingCompressionAmount; /**< temporary option to force compressed reference scaling to use this
                                                  as the shifted value (typically 0-3 in current usage) */
    ...
    uintptr_t regionSize; /**< The size, in bytes, of a fixed-size table-backed region of the heap (does not apply to
                             AUX regions) */
    MM_NUMAManager
        _numaManager; /**< The object which abstracts the details of our NUMA support so that the GCExtensions and the
                         callers don't need to duplicate the support to interpret our intention */
    bool numaForced; /**< if true, specifies if numa is disabled or enabled (actual value stored in NUMA Manager) by
                        command line option */
...
#if defined(OMR_GC_VLHGC)
    bool tarokDebugEnabled; /**< True if we want to perform additional checks and freed memory poisoning which aid in
                               debugging Tarok problems  */
    uintptr_t tarokGlobalMarkIncrementTimeMillis; /**< The time period in millisecond a Global Mark increment is allowed
                                                     to run (as set by the user, normally dynamic)*/
    uintptr_t fvtest_tarokForceNUMANode; /**< The NUMA node to force the heap onto (UDATA_MAX => no force, 0 =>
                                            interleaved, >0 => specific node) */
    uintptr_t
        fvtest_tarokFirstContext; /**< The allocation context number to use first, when associating the first thread
                                     (assignments will proceed, as a round robin, from this number).  Defaults to 0 */
    bool tarokEnableScoreBasedAtomicCompact; /**< True if atomic compact does use score based compact region selection
                                                heuristic */
    uintptr_t tarokIdealEdenMinimumBytes; /**< The ideal size of the eden space, in bytes, when the heap is at its -Xms
                                             size */
    uintptr_t tarokIdealEdenMaximumBytes; /**< The ideal size of the eden space, in bytes, when the heap is at its -Xmx
                                             size */
    bool tarokEnableIncrementalGMP; /**< True if we want to perform GMP work as a series of increments during the run.
                                       (set to false if we should rely on OOM global collections to perform the GMP) */
    MM_UserSpecifiedParameterUDATA tarokNurseryMaxAge; /**< The maximum age that a region will be before it is excluded
                                                          from a partial garbage collection */
    uintptr_t tarokRememberedSetCardListMaxSize; /* The maximum size in entries of RememberedSetCardList per region */
    uintptr_t tarokRememberedSetCardListSize; /* The average (allocated) size in entries of RememberedSetCardList per
                                                 region */
    ...
    uintptr_t
        tarokGMPIntermission; /** The delay between GMP cycles, specified as the number of GMP increments to skip */
    bool tarokAutomaticGMPIntermission; /** Should the delay between GMP cycles be automatic, or as specified in
                                           tarokGMPIntermission? */
    uintptr_t tarokRegionMaxAge; /**< Maximum age a region can be before it will no longer have its age incremented
                                    after a PGC (saturating age) */
    uintptr_t tarokKickoffHeadroomInBytes; /**< extra bytes reserved for survivor set, in case of sudden changes of
                                              survivor rate. Used in calculation to predict GMP kickoff */
    bool tarokForceKickoffHeadroomInBytes; /** true if user specifies tarokKickoffHeadroomInBytes via
                                              -XXgc:tarokKickoffHeadroomInBytes= */
    uint32_t tarokKickoffHeadroomRegionRate; /**< used by calculating tarokKickoffHeadroomInBytes, the percentage of the
                                                free memory, range: 0(0%)<=the rate<=50(50%) , default=2 (2%)  */
    MM_RememberedSetCardBucket*
        rememberedSetCardBucketPool; /* GC thread local pools of RS Card Buckets for each Region (its Card List) */
    bool tarokEnableDynamicCollectionSetSelection; /**< Enable dynamic selection of regions to include in the collection
                                                      set that reside outside of the nursery */
    uintptr_t
        tarokDynamicCollectionSetSelectionAbsoluteBudget; /**< Number of budgeted regions to dynamically select for PGC
                                                             collection (outside of the required nursery set) */
    double tarokDynamicCollectionSetSelectionPercentageBudget; /**< Percentage increase of nursery region count to use
                                                                  as dynamically selected regions for PGC */
    uintptr_t tarokCoreSamplingAbsoluteBudget; /**< Number of budgeted regions to select for core sampling in a PGC
                                                  collection (outside of the required nursery set) */
    double tarokCoreSamplingPercentageBudget; /**< Percentage increase of nursery region count to use as core sampling
                                                 selected regions for PGC */
    void* tarokTgcSetSelectionDataTable; /**< (TGC USE ONLY!) Table containing all dynamic and core sampling set
                                            selection information */
    bool tarokTgcEnableRememberedSetDuplicateDetection; /** (TGC USE ONLY!) True if we want to enable duplicate card
                                                           stats reported by TGC for RSCL */
    bool tarokPGCShouldCopyForward; /**< True if we want to allow PGC increments to reclaim memory using copy-forward
                                       (default is true) */
    bool tarokPGCShouldMarkCompact; /**< True if we want to allow PGC increments to reclaim memory using compact
                                       (default is true) and require a corresponding mark operation */
    MM_InterRegionRememberedSet*
        interRegionRememberedSet; /**< The remembered set abstraction to be used to track inter-region references found
                                     while processing this cycle */
    bool tarokEnableStableRegionDetection; /**< Enable overflowing RSCSLs for stable regions */
    double tarokDefragmentEmptinessThreshold; /**< Emptiness (freeAndDarkMatter/regionSize) for a region to be
                                                 considered as a target for defragmentation (used for stable region
                                                 detection and region de-fragmentation selection) */
    bool tarokAttachedThreadsAreCommon; /**< True if we want to associate all common threads with the "common context"
                                           which is otherwise only reserved for regions which the collector has
                                           identified as common to all nodes ("common" regions are still moved to this
                                           context, no matter the value of this flag) */
    double tarokCopyForwardFragmentationTarget; /**< The fraction of discarded space targeted in each copy-forward
                                                   collection. The actual amount may be lower or higher than this
                                                   fraction. */
    bool tarokEnableCardScrubbing; /**< Set if card scrubbing in GMPs is enabled (default is true) */
    bool tarokEnableConcurrentGMP; /**< Set if the GMP should attempt to accomplish work concurrently, where possible.
                                      False implies GMP work will only be done in the stop-the-world increments */
    MM_CompactGroupPersistentStats*
        compactGroupPersistentStats; /**< The global persistent stats indexed by compact group number */
    MM_ClassLoaderRememberedSet*
        classLoaderRememberedSet; /**< The remembered set abstraction to be used to track references from instances to
                                     the class loaders containing their defining class */
    ...
    bool tarokEnableLeafFirstCopying; /**< Enable copying of leaf children immediately after parent is copied in
                                         CopyForwardScheme */
    ...
    bool tarokUseProjectedSurvivalCollectionSet; /**< True if we should use a collection set based on the projected
                                                    survival rate of regions*/
    uintptr_t tarokWorkSplittingPeriod; /**< The number of objects which must be scanned between each check that the
                                           depth-first copy-forward implementation makes to see if it should push work
                                           out to other threads */
    MM_UserSpecifiedParameterUDATA
        tarokMinimumGMPWorkTargetBytes; /**< Minimum used for GMP work targets.  This avoids the low-scan-rate -> low
                                           GMP work target -> low scan-rate feedback loop. */
    double tarokConcurrentMarkingCostWeight; /**< How much we weigh concurrentMarking into our GMP scan time cost
                                                calculations */
    bool tarokAutomaticDefragmentEmptinessThreshold; /**< Whether we should use the automatically derived value for
                                                        tarokDefragmentEmptinessThreshold or not */
    bool tarokEnableCopyForwardHybrid; /**< Enable CopyForward Hybrid mode */
    enum ReserveRegions { RESERVE_REGIONS_NO = 0, RESERVE_REGIONS_MOST_ALLOCATABLE, RESERVE_REGIONS_MOST_FREE };
...
#endif
    enum TarokRegionTailCondidateListSortOrder { SORT_ORDER_NOORDER = 0, SORT_ORDER_ASCENDING, SORT_ORDER_DESCENDING };
    TarokRegionTailCondidateListSortOrder tarokTailCandidateListSortOrder;
#endif /* defined (OMR_GC_VLHGC) */

    /* OMR_GC_VLHGC (in for all -- see 82589) */
    bool tarokEnableExpensiveAssertions; /**< True if the collector should perform extra consistency verifications which
                                            are known to be very expensive or poorly parallelized */
    bool tarokEnableAllocationPointerAssertion;
    /* OMR_GC_VLHGC (in for all) */

    MM_SweepPoolManagerAddressOrderedList*
        sweepPoolManagerAddressOrderedList; /**< Pointer to Sweep Pool Manager for MPAOL, used for LOA and nursery */
    MM_SweepPoolManagerAddressOrderedListBase*
        sweepPoolManagerSmallObjectArea; /**< Pointer to Sweep Pool Manager for MPSAOL or Hybrid, used for SOA */
    ...
    bool _holdRandomThreadBeforeHandlingWorkUnit; /**< Whether we should randomly hold up a thread entering
                                                     MM_ParallelTask::handleNextWorkUnit() */
    uintptr_t _holdRandomThreadBeforeHandlingWorkUnitPeriod; /**< How often (in terms of number of times
                                                                MM_ParallelTask::handleNextWorkUnit() is called) to
                                                                randomly hold up a thread entering
                                                                MM_ParallelTask::handleNextWorkUnit() */
    bool _forceRandomBackoutsAfterScan; /**< Whether we should force MM_Scavenger::completeScan() to randomly fail due
                                           to backout */
    uintptr_t _forceRandomBackoutsAfterScanPeriod; /**< How often (in terms of number of times
                                                      MM_Scavenger::completeScan() is called) to randomly have
                                                      MM_Scavenger::completeScan() fail due to backout */

    MM_ReferenceChainWalkerMarkMap*
        referenceChainWalkerMarkMap; /**< Reference to Reference Chain Walker mark map - will be created at first call
    ...
    uintptr_t darkMatterSampleRate; /**< the weight of darkMatterSample for standard gc, default:32, if the weight = 0,
                                       disable darkMatterSampling */

#if defined(OMR_GC_IDLE_HEAP_MANAGER)
    uintptr_t idleMinimumFree; /**< percentage of free heap to be retained as committed, default=0 for gencon, complete
                                  tenture free memory will be decommitted */
    bool gcOnIdle; /**< Enables releasing free heap pages if true while systemGarbageCollect invoked with IDLE GC code,
                      default is false */
    bool compactOnIdle; /**< Forces compaction if global GC executed while VM Runtime State set to IDLE, default is
                           false */
    float gcOnIdleCompactThreshold; /**< Enables compaction when fragmented memory and dark matter exceed this limit.
                                       The larger this number, the more memory can be fragmented before compact is
                                       triggered **/
#endif
...
private:
     ...
     * Note: The only OS supported flags is ZOS however all other platforms have flags array filled by
     * OMRPORT_VMEM_PAGE_FLAG_NOT_USED values So an example of pair for ZOS: [1M, OMRPORT_VMEM_PAGE_FLAG_PAGEABLE] An
     * example For AIX: [64K, OMRPORT_VMEM_PAGE_FLAG_NOT_USED]
     *
     ...
     */
    static bool validateDefaultPageParameters(
        uintptr_t pageSize, uintptr_t pageFlags, uintptr_t* pageSizesArray, uintptr_t* pageFlagsArray);
...
public:
    ...
    MMINLINE bool isFvtestForceSweepChunkArrayCommitFailure()
    {
        return isFvtestForce(
            &fvtest_forceSweepChunkArrayCommitFailure, &fvtest_forceSweepChunkArrayCommitFailureCounter);
    }
    ...
    {
        return isFvtestForce(&fvtest_forceReferenceChainWalkerMarkMapCommitFailure,
    ...
    {
        return isFvtestForce(
            &fvtest_forceConcurrentTLHMarkMapCommitFailure, &fvtest_forceConcurrentTLHMarkMapCommitFailureCounter);
    }

    MMINLINE bool isFvtestForceConcurrentTLHMarkMapDecommitFailure()
    {
        return isFvtestForce(
            &fvtest_forceConcurrentTLHMarkMapDecommitFailure, &fvtest_forceConcurrentTLHMarkMapDecommitFailureCounter);
    }
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */
     ...
     */
    MMINLINE uintptr_t getObjectAlignmentInBytes() { return objectModel.getObjectAlignmentInBytes(); }

    MMINLINE float getDarkMatterCompactThreshold() { return darkMatterCompactThreshold; }
...
#define CONCURRENT_SCAVENGER_PAGE_SECTIONS 64
     ...
     */
    MMINLINE void calculateConcurrentScavengerPageParameters(uintptr_t nurserySize)
    {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
        /*
         ...
         * To do so, find closest power of 2 larger or equal then maximum Nursery size and store 1/64th of it as a
         * Section size Section size can not be smaller then 32M / 64 = 512K
         */
    ...
    MM_GCExtensionsBase()
        ...
        , oolObjectSamplingBytesGranularity(
              16 * 1024 * 1024) /* Default granularity set to 16M (shows <1% perf loss). */
        ...
        , overflowCacheCount(
              0) /**< initial value of 0.  This is set in workpackets initialization or via the commandline */
        , concurrentSweepingEnabled(false)
        , concurrentTracingEnabled(false)
#endif /* defined(OMR_GC_REALTIME) */
        , instrumentableAllocateHookEnabled(
              false) /* by default the hook J9HOOK_VM_OBJECT_ALLOCATE_INSTRUMENTABLE is disabled */
        ...
        , traceCostToCheckYield(
              500) /* weighted sum of marked objects and scanned pointers before we check yield in main tracing loop */
        , sweepCostToCheckYield(
              500) /* weighted count of free chunks/marked objects before we check yield in sweep small loop */
        ...
        , suballocatorInitialSize(
              SUBALLOCATOR_INITIAL_SIZE) /* default for J9Heap suballocator initial size is 200 MB */
        ...
        , shouldAllowShiftingCompression(
              true) /* VM Design 1810: shifting compression enabled, by default, for compressed refs */
...
#if defined(OMR_GC_VLHGC)
        , tarokDebugEnabled(
              false) /* by default, we turn off the Tarok debug options since they are an appreciable performance hit */
