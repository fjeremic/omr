 ...
 * Initialize GAM
 */
bool MM_GlobalAllocationManager::initialize(MM_EnvironmentBase* env) {
  return true;
}
 ...
 */
void MM_GlobalAllocationManager::tearDown(MM_EnvironmentBase* env) {}

/**
 * Flush all allocation contexts such that the cells allocated to them becomes
 * safe for traversal.
 */
void MM_GlobalAllocationManager::flushAllocationContexts(
...
void MM_GlobalAllocationManager::flushAllocationContextsForShutdown(
    MM_EnvironmentBase* env) {
  Assert_MM_true(_managedAllocationContextCount > 0);
