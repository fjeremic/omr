...
void MM_MemoryPoolAddressOrderedListBase::acquireResetLock(
    MM_EnvironmentBase *env) {
  _resetLock.acquire();
}

void MM_MemoryPoolAddressOrderedListBase::releaseResetLock(
    MM_EnvironmentBase *env) {
  _resetLock.release();
}
 ...
 */
bool MM_MemoryPoolAddressOrderedListBase::createFreeEntry(
...
#if defined(OMR_VALGRIND_MEMCHECK)
  valgrindClearRange(env->getExtensions(), (uintptr_t)addrBase,
                     (uintptr_t)addrTop - (uintptr_t)addrBase);
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
 ...
 */
bool MM_MemoryPoolAddressOrderedListBase::connectInnerMemoryToPool(
 ...
 * connected with
 */
void MM_MemoryPoolAddressOrderedListBase::connectOuterMemoryToPool(
    MM_EnvironmentBase *env, void *address, uintptr_t size,
    void *nextFreeEntry) {
 ...
 * @param size free memory size in bytes
 */
void MM_MemoryPoolAddressOrderedListBase::connectFinalMemoryToPool(
    MM_EnvironmentBase *env, void *address, uintptr_t size) {
  Assert_MM_true((NULL == address) || (size >= getMinimumFreeEntrySize()));
 ...
 * @param size free memory size in bytes
 */
void MM_MemoryPoolAddressOrderedListBase::abandonMemoryInPool(
    MM_EnvironmentBase *env, void *address, uintptr_t size) {
#if defined(OMR_VALGRIND_MEMCHECK)
  valgrindClearRange(env->getExtensions(), (uintptr_t)address, size);
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
  abandonHeapChunk((MM_HeapLinkedFreeHeader *)address,
                   (uint8_t *)address + size);
}

#if defined(OMR_GC_IDLE_HEAP_MANAGER)
uintptr_t MM_MemoryPoolAddressOrderedListBase::releaseFreeEntryMemoryPages(
    MM_EnvironmentBase *env, MM_HeapLinkedFreeHeader *freeEntry) {
  bool const compressed = compressObjectReferences();
  ...
  while (NULL != currentFreeEntry) {
    ...
    if (pageSize <= currentFreeEntry->getSize()) {
      uintptr_t addressBase = MM_Math::roundToCeiling(
          pageSize,
          (uintptr_t)currentFreeEntry + sizeof(MM_HeapLinkedFreeHeader));
      /* release/decommit memory after Header */
      uintptr_t totalFreePagesCount =
          (currentFreeEntry->getSize() -
           (addressBase - (uintptr_t)currentFreeEntry)) /
          pageSize;
      if (0 < totalFreePagesCount) {
        ...
        if (0 < _extensions->idleMinimumFree) {
          commitPagesCount =
              totalFreePagesCount * _extensions->idleMinimumFree / 100;
        }
        ...
        if (0 < decommitPagesCount) {
          if (_extensions->heap->decommitMemory(
                  (void *)addressBase, decommitPagesCount * pageSize, NULL,
