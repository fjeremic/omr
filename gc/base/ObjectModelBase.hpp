...
public:
     ...
     */
    MMINLINE UDATA getHeaderSize(omrobjectptr_t objectPtr) { return _delegate.getObjectHeaderSizeInBytes(objectPtr); }
    ...
    MMINLINE void calculateObjectDetailsForCopy(MM_EnvironmentBase* env, MM_ForwardedHeader* forwardedHeader,
        uintptr_t* objectCopySizeInBytes, uintptr_t* objectReserveSizeInBytes, uintptr_t* hotFieldAlignmentDescriptor)
    {
        _delegate.calculateObjectDetailsForCopy(
            env, forwardedHeader, objectCopySizeInBytes, objectReserveSizeInBytes, hotFieldAlignmentDescriptor);
    }
    ...
    MMINLINE bool isDeadObject(void* objectPtr)
    {
        ...
        if (compressObjectReferences()) {
            ...
            result = ((uintptr_t)J9_GC_OBJ_HEAP_HOLE
                == (*(uintptr_t*)headerSlotPtr & (uintptr_t)(J9_GC_OBJ_HEAP_HOLE | OMR_FORWARDED_TAG)));
#else
     ...
     */
    MMINLINE bool isRemembered(omrobjectptr_t objectPtr) { return (getRememberedBits(objectPtr) >= STATE_REMEMBERED); }
     ...
     * @return TRUE if object is indexable, FALSE otherwise
     */
    MMINLINE bool isIndexable(MM_ForwardedHeader* forwardedHeader) { return _delegate.isIndexable(forwardedHeader); }
    ...
    /**
     * Extract the flag bits from an unforwarded object. Flag bits are returned in the low-order byte of the returned
     * value.
     *
