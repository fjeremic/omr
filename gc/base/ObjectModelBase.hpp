...
#define OMR_MINIMUM_OBJECT_ALIGNMENT_SHIFT 3 /* base 2 log of heap alignment   \
                                              */
#define OMR_MINIMUM_OBJECT_SIZE                                                \
  16 /* size of smallest possible object allocation, in bytes */

/**
 * Bit geometry within header flags byte.
 *
 * NOTE: These are normalized to low-order byte. Object header flags must be
 * right-shifted to low-order byte (see
 * GC_ObjectModelBase::getObjectHeaderSlotFlagsShift()) before applying these
 * masks/shifts and subsets of these masks must be left-shifted to align with
 * object header flags.
 */
...
/**
 * Remembered bit states overlay tenured header age flags. These are normalized
 * to low-order byte, as above.
 */
...
#define OMR_TENURED_STACK_OBJECT_RECENTLY_REFERENCED                           \
  (STATE_REMEMBERED + (1 << OMR_OBJECT_METADATA_AGE_SHIFT))
#define OMR_TENURED_STACK_OBJECT_CURRENTLY_REFERENCED                          \
  (STATE_REMEMBERED + (2 << OMR_OBJECT_METADATA_AGE_SHIFT))
...
private:
  ...
  bool _compressObjectReferences;
        ...
        */
  GC_ObjectModelDelegate
    _delegate; /**< instance of object model delegate class */

protected:
  uintptr_t _objectAlignmentInBytes; /**< cached copy of heap object alignment
                                        factor, in bytes */
  uintptr_t
    _objectAlignmentShift; /**< cached copy of heap object alignment shift, must
                              be log2(_objectAlignmentInBytes)  */
...
protected:
  /**
   * Get a pointer to the object model delegate associated with this object
   * model
   */
  MMINLINE GC_ObjectModelDelegate* getObjectModelDelegate()
  {
    return &_delegate;
  }
  ...
  {
    return (void*)(((uintptr_t)(objectPtr)) +
                   _delegate.getObjectHeaderSlotOffset());
  }
...
public:
  ...
  /**
   * Get the value of the flags byte from the header of an object. The flags
   * value is returned in the low-order byte of the returned value.
   *
  ...
  MMINLINE uintptr_t getObjectFlags(omrobjectptr_t objectPtr)
  {
    ...
    return (flags >> getObjectHeaderSlotFlagsShift()) &
           (uintptr_t)OMR_OBJECT_METADATA_FLAGS_MASK;
  }

  /**
   * Get the age of a heap object. The age value is returned in the low order
  ...
  /**
   * Determine the total size of an object, in bytes, including padding bytes
   * added to bring tail of object into heap alignment (see
   * GC_ObjectModelBase::adjustSizeInBytes()). If the object has a discontiguous
   * representation, this method should return the size of the root object that
   * the discontiguous parts depend from. If the object has been expanded, the
   * total must include the expansion size.
   *
   * This method should also account for any expansion bytes added to the object
   * since instantiation.
   *
  ...
  /**
   * Determine the total footprint of an object, in bytes, including padding
   * bytes added to bring tail of object into heap alignment (see
   * GC_ObjectModelBase::adjustSizeInBytes()). If the object has a discontiguous
   * representation, this method should return the size of the root object plus
   * the total of all the discontiguous parts of the object.
   *
   * This method should also account for any expansion bytes added to the object
   * since instantiation.
   *
   * @param[in] objectPtr points to the object to determine size for
   * @return the total size of an object, in bytes, including padding bytes and
   * discontiguous parts
   */
  ...
  /**
   * Object size should be at least OMR_MINIMUM_OBJECT_SIZE and
   * _objectAlignmentInBytes-byte aligned.
   ...
   * MM_AllocateInitialization::allocateAndInitializeObject() to allow the
   * object model to complete the initialization of the memory (allocatedBytes)
   * allocated for the object, after the OMR object flags have been set in the
   * allocated object header.
   *
   * For most languages, allocateInitialization will point to a
   * language-specific subclass of MM_AllocateInitialization and will use
   * MM_AllocateInitialization::getAllocationCategory() to determine and cast to
   * the specific subclass and delegate object initialization to the subclass.
   *
  ...
  initializeAllocation(MM_EnvironmentBase* env,
                       void* allocatedBytes,
                       MM_AllocateInitialization* allocateInitialization)
  {
    return _delegate.initializeAllocation(
      env, allocatedBytes, allocateInitialization);
  }
   ...
   */
  MMINLINE void calculateObjectDetailsForCopy(
  ...
  {
    _delegate.calculateObjectDetailsForCopy(env,
                                            forwardedHeader,
                                            objectCopySizeInBytes,
                                            objectReserveSizeInBytes,
                                            hotFieldAlignmentDescriptor);
  }
#endif /* defined(OMR_GC_MODRON_SCAVENGER) */

  /**
   * Set run-time object alignment and shift values in this object model and in
   * the OMR VM struct. These values are dependent on
   * OMR_VM::_compressedPointersShift, which must be set before calling this
   * method. These values would be const except that they can sometimes be
   * determined only after the object model has been instantiated. In any case,
   * this method should be called only once, before attempting to allocate
   * objects from the heap.
   *
   * @param omrVM pointer to the OMR VM struct, with final value for
   * OMR_VM::_compressedPointersShift
   */
  MMINLINE void setObjectAlignment(OMR_VM* omrVM)
  {
    ...
    _objectAlignmentInBytes =
      OMR_MAX((uintptr_t)1 << omrVM->_compressedPointersShift,
              OMR_MINIMUM_OBJECT_ALIGNMENT);
    _objectAlignmentShift = OMR_MAX(omrVM->_compressedPointersShift,
                                    OMR_MINIMUM_OBJECT_ALIGNMENT_SHIFT);
   ...
   */
  MMINLINE bool isIndexable(omrobjectptr_t objectPtr)
  {
  ...
  MMINLINE bool isDeadObject(void* objectPtr)
  {
    ...
    if (compressObjectReferences()) {
      ...
      /* for ConcurrentScavenger, forwarded bit must be 0, otherwise it's a
       * self-forwarded object */
      result = ((uint32_t)J9_GC_OBJ_HEAP_HOLE ==
                (*(uint32_t*)headerSlotPtr &
                 (uint32_t)(J9_GC_OBJ_HEAP_HOLE | OMR_FORWARDED_TAG)));
#else
      result = ((uint32_t)J9_GC_OBJ_HEAP_HOLE ==
  ...
  MMINLINE bool isSingleSlotDeadObject(omrobjectptr_t objectPtr)
  {
    ...
    if (compressObjectReferences()) {
      result =
        ((uint32_t)J9_GC_SINGLE_SLOT_HOLE ==
         (*(uint32_t*)headerSlotPtr & (uint32_t)J9_GC_OBJ_HEAP_HOLE_MASK));
    } else {
      result =
        ((uintptr_t)J9_GC_SINGLE_SLOT_HOLE ==
  ...
  {
    return MM_HeapLinkedFreeHeader::getHeapLinkedFreeHeader(objectPtr)
      ->getSize();
  }
  ...
  /**
   * Set flags in object header. The bitsToClear and bitsToSet masks are
   * expected to be unshifted (aligned in low-order byte).
   *
   * @param objectPtr Pointer to an object
   * @param bitsToClear unshifted mask to clear bits
   * @param bitsToSet unshifted mask to set bits
   */
  MMINLINE void setObjectFlags(omrobjectptr_t objectPtr,
                               uintptr_t bitsToClear,
                               uintptr_t bitsToSet)
  {
    void* headerSlotPtr = getObjectHeaderSlotAddress(objectPtr);
    if (compressObjectReferences()) {
#if defined(OBJECT_MODEL_MODRON_ASSERTIONS)
      Assert_MM_true(0 == (~(uint32_t)OMR_OBJECT_METADATA_FLAGS_MASK &
                           (uint32_t)bitsToClear));
      Assert_MM_true(
        0 == (~(uint32_t)OMR_OBJECT_METADATA_FLAGS_MASK & (uint32_t)bitsToSet));
#endif /* defined(OBJECT_MODEL_MODRON_ASSERTIONS) */
      ...
      *flagsPtr = (*flagsPtr & ~clear) | set;
...
#if defined(OBJECT_MODEL_MODRON_ASSERTIONS)
      Assert_MM_true(0 == (~(uintptr_t)OMR_OBJECT_METADATA_FLAGS_MASK &
                           (uintptr_t)bitsToClear));
      Assert_MM_true(0 == (~(uintptr_t)OMR_OBJECT_METADATA_FLAGS_MASK &
                           (uintptr_t)bitsToSet));
#endif /* defined(OBJECT_MODEL_MODRON_ASSERTIONS) */
      ...
      uintptr_t clear = (uintptr_t)bitsToClear
  ...
  /**
   * Unconditionally and atomically set object flags. The bitsToClear and
   * bitsToSet masks are expected to be non-zero and unshifted (aligned in
   * low-order byte).
   *
   * Call will fail (return false) if change was idempotent (previous flags
   * unchanged).
   *
   * @param objectPtr Pointer to an object
   ...
   */
  MMINLINE bool atomicSetObjectFlags(omrobjectptr_t objectPtr,
                                     uintptr_t bitsToClear,
                                     uintptr_t bitsToSet)
  {
    bool result = true;
...
#if defined(OBJECT_MODEL_MODRON_ASSERTIONS)
    Assert_MM_true(
      0 == (~(fomrobject_t)OMR_OBJECT_METADATA_FLAGS_MASK & bitsToClear));
    Assert_MM_true(0 ==
                   (~(fomrobject_t)OMR_OBJECT_METADATA_FLAGS_MASK & bitsToSet));
#endif /* defined(OBJECT_MODEL_MODRON_ASSERTIONS) */
    ...
    if (compressObjectReferences()) {
      ...
      uint32_t clear =
        (uint32_t)(bitsToClear << getObjectHeaderSlotFlagsShift());
      uint32_t set = (uint32_t)(bitsToSet << getObjectHeaderSlotFlagsShift());
      uint32_t oldFlags;
      uint32_t newFlags;
      ...
      } while (oldFlags != MM_AtomicOperations::lockCompareExchangeU32(
                             flagsPtr, oldFlags, newFlags));
    } else {
      volatile uintptr_t* flagsPtr = (uintptr_t*)headerSlotPtr;
      uintptr_t clear =
        (uintptr_t)(bitsToClear << getObjectHeaderSlotFlagsShift());
      uintptr_t set = (uintptr_t)(bitsToSet << getObjectHeaderSlotFlagsShift());
      uintptr_t oldFlags;
      uintptr_t newFlags;
      ...
      } while (oldFlags != MM_AtomicOperations::lockCompareExchange(
                             flagsPtr, oldFlags, newFlags));
    }
  ...
  /**
   * Set remembered bits to the object's header. The bits mask is expected to be
   * unshifted (aligned in low-order byte).
   *
   * This method is not thread safe and can produce unexpected results if the
   * object is exposed to other concurrent threads. Use atomicSetObjectFlags()
  ...
  /**
   * Returns the collector bits from object's header. The returned value is
   * shifted down to the low-order byte of the returned value.
   *
   * @param objectPtr Pointer to an object
   ...
   * @param objectPtr Pointer to an object
   */
  MMINLINE void clearRemembered(omrobjectptr_t objectPtr)
  {
    setRememberedBits(objectPtr, STATE_NOT_REMEMBERED);
  }

  /**
   * Conditionally and atomically switch the referenced state of an object. The
   * fromState and toState masks are expected to be unshifted (aligned in
   * low-order byte).
   *
   * Call will return false with no change to object state if referenced state
   * of the object is not fromState.
   *
   * @param objectPtr Pointer to an object
   ...
   */
  MMINLINE bool atomicSwitchReferencedState(omrobjectptr_t objectPtr,
                                            uintptr_t fromState,
                                            uintptr_t toState)
  {
    bool result = true;
...
#if defined(OBJECT_MODEL_MODRON_ASSERTIONS)
    Assert_MM_true(0 ==
                   (~(fomrobject_t)OMR_OBJECT_METADATA_FLAGS_MASK & fromState));
    Assert_MM_true(0 ==
                   (~(fomrobject_t)OMR_OBJECT_METADATA_FLAGS_MASK & toState));
#endif /* defined(OBJECT_MODEL_MODRON_ASSERTIONS) */
    ...
    if (compressObjectReferences()) {
      ...
      uint32_t rememberedMask = (uint32_t)(OMR_OBJECT_METADATA_REMEMBERED_BITS
                                           << getObjectHeaderSlotFlagsShift());
      uint32_t oldFlags;
      uint32_t newFlags;
      ...
      volatile uintptr_t* flagsPtr = (uintptr_t*)headerSlotPtr;
      uintptr_t from =
        (uintptr_t)(fromState << getObjectHeaderSlotFlagsShift());
      uintptr_t to = (uintptr_t)(toState << getObjectHeaderSlotFlagsShift());
      uintptr_t rememberedMask = (uintptr_t)(
        OMR_OBJECT_METADATA_REMEMBERED_BITS << getObjectHeaderSlotFlagsShift());
      uintptr_t oldFlags;
      uintptr_t newFlags;
      ...
      do {
        ...
        if (newFlags == oldFlags) {
          ...
          break;
                             ...
                             flagsPtr, oldFlags, newFlags));
      /* check to verify that this thread and no other thread atomically set
       * flags -> toState */
  ...
  /**
   * Conditionally and atomically set the remembered state of an object. The
   * toState mask is expected to be unshifted (aligned in low-order byte).
   *
   * Call will return false with no change to object state if any remembered
   * bits are set in the object header.
   *
   * @param objectPtr Pointer to an object
   * @param toState unshifted desired new remembered state of object
   * @return true if remembered state has been set (to toState) this call
   */
  MMINLINE bool atomicSetRememberedState(omrobjectptr_t objectPtr,
                                         uintptr_t toState)
  {
    bool result = true;
  ...
  /**
   * Extract the flag bits from an unforwarded object. Flag bits are returned in
   * the low-order byte of the returned value.
   *
  ...
  {
    return ((uintptr_t)(forwardedHeader->getPreservedSlot()) >>
            getObjectHeaderSlotFlagsShift()) &
  ...
  {
    return (getPreservedFlags(forwardedHeader) &
            OMR_OBJECT_METADATA_AGE_MASK) >>
   ...
   * prior to the copy, and sets the object age.
   *
   * @param[in] forwardedHeader pointer to the MM_ForwardedHeader instance
   * encapsulating the object
   ...
   */
  MMINLINE void fixupForwardedObject(MM_ForwardedHeader* forwardedHeader,
...
#if defined(OBJECT_MODEL_MODRON_ASSERTIONS)
    Assert_MM_true((8 * (sizeof(fomrobject_t) - 1)) >=
                   _delegate.getObjectHeaderSlotFlagsShift());
#endif /* defined(OBJECT_MODEL_MODRON_ASSERTIONS) */
  }
};
#if defined(OMR_EXAMPLE)
