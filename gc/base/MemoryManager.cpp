...
{
    MM_MemoryManager* memoryManager = (MM_MemoryManager*)env->getForge()->allocate(
        sizeof(MM_MemoryManager), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
bool MM_MemoryManager::createVirtualMemoryForHeap(MM_EnvironmentBase* env, MM_MemoryHandle* handle,
    uintptr_t heapAlignment, uintptr_t size, uintptr_t tailPadding, void* preferredAddress, void* ceiling)
{
    ...
    if (extensions->isConcurrentScavengerHWSupported()) {
        ...
        concurrentScavengerPageSize
            = extensions->getConcurrentScavengerPageSectionSize() * CONCURRENT_SCAVENGER_PAGE_SECTIONS;
        allocateSize += concurrentScavengerPageSize;
        if (extensions->isDebugConcurrentScavengerPageAlignment()) {
            omrtty_printf(
                "Requested heap size 0x%zx has been extended to 0x%zx for guaranteed alignment\n", size, allocateSize);
        }
    } else {
    ...
    if (NULL == ceiling) {
        instance = MM_VirtualMemory::newInstance(env, heapAlignment, allocateSize, pageSize, pageFlags, tailPadding,
            preferredAddress, ceiling, mode, options, memoryCategory);

#if defined(OMR_ENV_DATA64) && defined(OMR_GC_FULL_POINTERS)
        if (!env->compressObjectReferences()) {
            if (1 == extensions->fvtest_enableReadBarrierVerification) {
                MM_VirtualMemory* instanceShadow
                    = MM_VirtualMemory::newInstance(env, heapAlignment, allocateSize, pageSize, pageFlags, tailPadding,
                        preferredAddress, (void*)OMR_MIN(NON_SCALING_LOW_MEMORY_HEAP_CEILING, (uintptr_t)ceiling), mode,
        ...
        if (env->compressObjectReferences()) {
            ...
            /* Set the commit size for the sub allocator. This needs to be completed before the call to
             * omrmem_ensure_capacity32 */
            ...
            if (!shouldHeapBeAllocatedFirst) {
                if (OMRPORT_ENSURE_CAPACITY_FAILED == omrmem_ensure_capacity32(extensions->suballocatorInitialSize)) {
                    extensions->heapInitializationFailureReason
                        = MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_CAN_NOT_ALLOCATE_LOW_MEMORY_RESERVE;
                    return false;
                }
            }
...
#define TWO_GB_ADDRESS ((void*)((uintptr_t)2 * 1024 * 1024 * 1024))
            ...
            if (extensions->isConcurrentScavengerHWSupported()) {
                ...
                if ((requestedTopAddress > ceiling)
                    && ((void*)((uintptr_t)requestedTopAddress - concurrentScavengerPageSize) <= ceiling)) {
                    /* ZOS 2_TO_64/2_TO_32 options would not allow memory request larger then 64G/32G so total requested
                     * size including tail padding should not exceed it */
                    ...
                    /*
                     * There is no way that Nursery will be pushed above ceiling for valid memory options however we
                     * have no idea about start address. So to guarantee an allocation up to the ceiling we need to
                     * request extended chunk of memory. Set ceiling to NULL to disable ceiling control. This required
                     * bottom-up direction for allocation.
                     */
                ...
                instance = MM_VirtualMemory::newInstance(env, heapAlignment, allocateSize, pageSize, pageFlags,
                    tailPadding, preferredAddress, ceilingToRequest, mode, options, memoryCategory);
            } else {
                if (requestedTopAddress <= ceiling) {
                    ...
                    if (allocationTopDown && extensions->shouldForceSpecifiedShiftingCompression) {
                        ...
                        instance = MM_VirtualMemory::newInstance(env, heapAlignment, allocateSize, pageSize, pageFlags,
                            tailPadding, preferredAddress, maxAddress, mode, options, memoryCategory);
                    } else {
                        if (requestedTopAddress < (void*)NON_SCALING_LOW_MEMORY_HEAP_CEILING) {
                            /*
                             * Attempt to allocate heap below 4G
                             */
                            instance = MM_VirtualMemory::newInstance(env, heapAlignment, allocateSize, pageSize,
                                pageFlags, tailPadding, preferredAddress,
                                (void*)OMR_MIN(NON_SCALING_LOW_MEMORY_HEAP_CEILING, (uintptr_t)ceiling), mode, options,
                                memoryCategory);
                        }
...
#define THIRTY_TWO_GB_ADDRESS ((uintptr_t)32 * 1024 * 1024 * 1024)
                            if (requestedTopAddress <= (void*)THIRTY_TWO_GB_ADDRESS) {
                                /*
                                 * If requested object heap size is in range 28G-32G its allocation with 3-bit shift
                                 * might compromise amount of low memory below 4G To prevent this go straight to 4-bit
                                 * shift if it possible. Set of logical conditions to skip allocation attempt below 32G
                                ...
                                bool skipAllocationBelow32G = (ceiling > (void*)THIRTY_TWO_GB_ADDRESS)
                                    && (requestedTopAddress
                                        > (void*)(THIRTY_TWO_GB_ADDRESS - NON_SCALING_LOW_MEMORY_HEAP_CEILING))
                                    && allocationTopDown;
                                ...
                                if (!skipAllocationBelow32G) {
                                    ...
                                    instance = MM_VirtualMemory::newInstance(env, heapAlignment, allocateSize, pageSize,
                                        pageFlags, tailPadding, preferredAddress,
                                        (void*)OMR_MIN((uintptr_t)THIRTY_TWO_GB_ADDRESS, (uintptr_t)ceiling), mode,
                                        options, memoryCategory);
                                }
                            }
                            ...
                            if ((NULL == instance) && (ceiling > (void*)THIRTY_TWO_GB_ADDRESS)) {
                                instance = MM_VirtualMemory::newInstance(env, heapAlignment, allocateSize, pageSize,
                                    pageFlags, tailPadding, preferredAddress, ceiling, mode, options, memoryCategory);
                            }
                        }
                    }
                }
...
#endif /* defined(OMR_GC_COMPRESSED_POINTERS) */
        {
             ...
             *  - OMRPORT_VMEM_ZOS_USE2TO32G_AREA flag for ZOS is expected to be used for compressedrefs heap allocation
             * only
             */
    ...
    if ((NULL != instance) && extensions->largePageFailOnError
        ...
        extensions->heapInitializationFailureReason
            = MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_CAN_NOT_SATISFY_REQUESTED_PAGE_SIZE;
        instance->kill(env);
        instance = NULL;
        return false;
    }
    ...
    if (NULL != instance) {
        ...
        if (extensions->isConcurrentScavengerHWSupported()) {
            ...
            if (extensions->isDebugConcurrentScavengerPageAlignment()) {
                omrtty_printf("Adjusted heap location: [%p,%p], Concurrent Scavenger Page start address %p, Concurrent "
                              "Scavenger Page size 0x%zx\n",
                    handle->getMemoryBase(), handle->getMemoryTop(),
                    extensions->getConcurrentScavengerPageStartAddress(), concurrentScavengerPageSize);
            }
...
bool MM_MemoryManager::createVirtualMemoryForMetadata(
    MM_EnvironmentBase* env, MM_MemoryHandle* handle, uintptr_t alignment, uintptr_t size)
{
    Assert_MM_true(NULL != handle);
    ...
    if (NULL == handle->getVirtualMemory()) {
        ...
        if (isMetadataAllocatedInVirtualMemory(env)) {
             ...
             * Segmentation is enabled for AIX-64 only, so physical page size is used as a segment size for other
             * platforms To get advantage of preallocation this memory should be allocated in large pages
             */
             ...
             */
            instance = MM_VirtualMemory::newInstance(env, alignment, allocateSize, pageSize, pageFlags, tailPadding,
                preferredAddress, ceiling, mode, options, memoryCategory);
        } else {
            /*
...
void MM_MemoryManager::destroyVirtualMemoryForHeap(MM_EnvironmentBase* env, MM_MemoryHandle* handle)
{
    destroyVirtualMemory(env, handle);
...
void MM_MemoryManager::destroyVirtualMemory(MM_EnvironmentBase* env, MM_MemoryHandle* handle)
{
    Assert_MM_true(NULL != handle);
...
#if defined(OMR_GC_DOUBLE_MAP_ARRAYLETS)
void* MM_MemoryManager::doubleMapArraylet(MM_MemoryHandle* handle, MM_EnvironmentBase* env, void* arrayletLeaves[],
    UDATA arrayletLeafCount, UDATA arrayletLeafSize, UDATA byteAmount, struct J9PortVmemIdentifier* newIdentifier,
    UDATA pageSize)
{
    Assert_MM_true(NULL != handle);
    MM_VirtualMemory* memory = handle->getVirtualMemory();
    Assert_MM_true(NULL != memory);
    return memory->doubleMapArraylet(
        env, arrayletLeaves, arrayletLeafCount, arrayletLeafSize, byteAmount, newIdentifier, pageSize);
}
#endif /* defined(OMR_GC_DOUBLE_MAP_ARRAYLETS) */

bool MM_MemoryManager::commitMemory(MM_MemoryHandle* handle, void* address, uintptr_t size)
{
    Assert_MM_true(NULL != handle);
    MM_VirtualMemory* memory = handle->getVirtualMemory();
    Assert_MM_true(NULL != memory);
...
bool MM_MemoryManager::decommitMemory(
...
bool MM_MemoryManager::isLargePage(MM_EnvironmentBase* env, uintptr_t pageSize)
{
...
bool MM_MemoryManager::setNumaAffinity(
    const MM_MemoryHandle* handle, uintptr_t numaNode, void* address, uintptr_t byteAmount)
{
    Assert_MM_true(NULL != handle);
    MM_VirtualMemory* memory = handle->getVirtualMemory();
    Assert_MM_true(NULL != memory);
