...
bool
MM_MemoryManager::createVirtualMemoryForHeap(MM_EnvironmentBase* env,
                                             ...
                                             void* ceiling)
{
  ...
  if (NULL == ceiling) {
    instance = MM_VirtualMemory::newInstance(env,
                                             heapAlignment,
                                             allocateSize,
                                             pageSize,
                                             pageFlags,
...
#define TWO_GB_ADDRESS ((void*)((uintptr_t)2 * 1024 * 1024 * 1024))
      ...
      void* requestedTopAddress =
        (void*)((uintptr_t)startAllocationAddress + allocateSize + tailPadding);
    ...
    if (extensions->isConcurrentScavengerHWSupported()) {
      ...
      if (baseAligned == topAligned) {
        ...
        Assert_GC_true_with_message3(
          env,
          ((heapBase + size) <= (uintptr_t)handle->getMemoryTop()),
          "End of projected heap (base 0x%zx + size 0x%zx) is larger then Top "
          "allocated %p\n",
          heapBase,
          size,
          handle->getMemoryTop());
...
bool
MM_MemoryManager::createVirtualMemoryForMetadata(MM_EnvironmentBase* env,
                                                 MM_MemoryHandle* handle,
                                                 uintptr_t alignment,
                                                 uintptr_t size)
{
  ...
  if (NULL == handle->getVirtualMemory()) {
    ...
    if (isMetadataAllocatedInVirtualMemory(env)) {
       ...
       */
      instance = MM_NonVirtualMemory::newInstance(
        env, alignment, allocateSize, memoryCategory);
    }
...
void
MM_MemoryManager::destroyVirtualMemoryForHeap(MM_EnvironmentBase* env,
                                              MM_MemoryHandle* handle)
{
  destroyVirtualMemory(env, handle);
...
void
MM_MemoryManager::destroyVirtualMemory(MM_EnvironmentBase* env,
                                       MM_MemoryHandle* handle)
{
  Assert_MM_true(NULL != handle);
...
#if defined(OMR_GC_DOUBLE_MAP_ARRAYLETS)
void*
MM_MemoryManager::doubleMapArraylet(MM_MemoryHandle* handle,
                                    ...
                                    UDATA pageSize)
{
  ...
  return memory->doubleMapArraylet(env,
                                   arrayletLeaves,
                                   arrayletLeafCount,
                                   arrayletLeafSize,
                                   byteAmount,
...
bool
MM_MemoryManager::commitMemory(MM_MemoryHandle* handle,
                               void* address,
                               uintptr_t size)
{
  Assert_MM_true(NULL != handle);
...
bool
MM_MemoryManager::decommitMemory(MM_MemoryHandle* handle,
                                 void* address,
                                 uintptr_t size,
                                 ...
                                 void* highValidAddress)
{
  ...
  MM_VirtualMemory* memory = handle->getVirtualMemory();
  Assert_MM_true(NULL != memory);
  return memory->decommitMemory(
    address, size, lowValidAddress, highValidAddress);
}
...
bool
MM_MemoryManager::setNumaAffinity(const MM_MemoryHandle* handle,
                                  uintptr_t numaNode,
                                  void* address,
