...
uintptr_t MM_MemorySubSpaceGeneric::getActiveMemorySize(uintptr_t includeMemoryType)
{
...
uintptr_t MM_MemorySubSpaceGeneric::getActiveLOAMemorySize(uintptr_t includeMemoryType)
{
    if (getTypeFlags() & includeMemoryType) {
 ...
 */
uintptr_t MM_MemorySubSpaceGeneric::getActualActiveFreeMemorySize(uintptr_t includeMemoryType)
{
    if (getTypeFlags() & includeMemoryType) {
        return _memoryPool->getActualFreeMemorySize();
    } else {
 ...
 */
uintptr_t MM_MemorySubSpaceGeneric::getApproximateActiveFreeMemorySize(uintptr_t includeMemoryType)
{
    if (getTypeFlags() & includeMemoryType) {
        return _memoryPool->getApproximateFreeMemorySize();
    } else {
 ...
 */
uintptr_t MM_MemorySubSpaceGeneric::getApproximateActiveFreeLOAMemorySize(uintptr_t includeMemoryType)
{
    if (getTypeFlags() & includeMemoryType) {
        return _memoryPool->getApproximateFreeLOAMemorySize();
    } else {
...
void MM_MemorySubSpaceGeneric::mergeHeapStats(MM_HeapStats* heapStats, uintptr_t includeMemoryType)
{
    if (getTypeFlags() & includeMemoryType) {
        _memoryPool->mergeHeapStats(heapStats, isActive());
    }
 ...
 */
MM_AllocationFailureStats* MM_MemorySubSpaceGeneric::getAllocationFailureStats()
{
...
void* MM_MemorySubSpaceGeneric::allocateObject(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* baseSubSpace, MM_MemorySubSpace* previousSubSpace, bool shouldCollectOnFailure)
{
    Trc_MM_MSSGeneric_allocate_entry(env->getLanguageVMThread(), "Object", allocDescription->getBytesRequested(), this,
    ...
    /* Typically, JIT stack frame is not built (shouldCollectOnFailure is false) and concurrent mark is not running or
     * not complete (_allocateAtSafePointOnly is false), so we most often proceed with allocate. If we are notified that
     * concurrent mark is complete ( _allocateAtSafePointOnly is true, and typically the stack frame is not built yet,
     * we should not proceed with allocate, but return NULL and force JIT to call us again with the frame built
     * Next time when the frame is built (shouldCollectOnFailure is true, while _allocateAtSafePointOnly is still true)
     * we proceed with allocate (otherwise risk OOM), but on the way out of this allocate, when we pay tax, concurrent
     * mark will trigger the final phase. */
    if (!_allocateAtSafePointOnly || shouldCollectOnFailure) {
        ...
        if (NULL != result) {
            ...
            if (shouldCollectOnFailure) {
                Trc_MM_MSSGeneric_allocate(
                    env->getLanguageVMThread(), "Object", allocDescription->getBytesRequested(), 1, this, _parent);
                result
                    = _parent->allocationRequestFailed(env, allocDescription, ALLOCATION_TYPE_OBJECT, NULL, this, this);
            } else {
                Trc_MM_MSSGeneric_allocate(
                    env->getLanguageVMThread(), "Object", allocDescription->getBytesRequested(), 2, this, _parent);
    ...
    Trc_MM_MSSGeneric_allocate_exit(
        env->getLanguageVMThread(), "Object", allocDescription->getBytesRequested(), this, result);
 ...
 */
void* MM_MemorySubSpaceGeneric::allocateArrayletLeaf(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* baseSubSpace, MM_MemorySubSpace* previousSubSpace, bool shouldCollectOnFailure)
{
    void* result = NULL;
    ...
    if (!_allocateAtSafePointOnly || shouldCollectOnFailure) {
        ...
        if (NULL == result) {
            ...
            if (shouldCollectOnFailure) {
                result
                    = _parent->allocationRequestFailed(env, allocDescription, ALLOCATION_TYPE_LEAF, NULL, this, this);
            } else {
                result
                    = _parent->allocateArrayletLeaf(env, allocDescription, baseSubSpace, this, shouldCollectOnFailure);
            }
        }
    }
...
void* MM_MemorySubSpaceGeneric::allocationRequestFailed(MM_EnvironmentBase* env,
...
void* MM_MemorySubSpaceGeneric::allocateTLH(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_ObjectAllocationInterface* objectAllocationInterface, MM_MemorySubSpace* baseSubSpace,
    MM_MemorySubSpace* previousSubSpace, bool shouldCollectOnFailure)
{
    ...
    if (!_allocateAtSafePointOnly || shouldCollectOnFailure) {
        ...
        if (NULL == result) {
            if (shouldCollectOnFailure) {
                Trc_MM_MSSGeneric_allocate3(env->getLanguageVMThread(), "TLH", allocDescription->getBytesRequested(),
                    this, _parent, (uintptr_t)allocDescription->shouldCollectAndClimb());
                if (allocDescription->shouldCollectAndClimb()) {
                    result = _parent->allocationRequestFailed(
                        env, allocDescription, ALLOCATION_TYPE_TLH, objectAllocationInterface, this, this);
                }
            } else {
                Trc_MM_MSSGeneric_allocate(
                    env->getLanguageVMThread(), "TLH", allocDescription->getBytesRequested(), 2, this, _parent);
                result
                    = _parent->allocateTLH(env, allocDescription, objectAllocationInterface, baseSubSpace, this, false);
            }
        }
    }

    Trc_MM_MSSGeneric_allocate_exit(
        env->getLanguageVMThread(), "TLH", allocDescription->getBytesRequested(), this, result);
...
#if defined(OMR_GC_THREAD_LOCAL_HEAP)
void* MM_MemorySubSpaceGeneric::collectorAllocateTLH(MM_EnvironmentBase* env, MM_Collector* requestCollector,
    MM_AllocateDescription* allocDescription, uintptr_t maximumBytesRequired, void*& addrBase, void*& addrTop)
{
    ...
    if (NULL == result) {
        _memoryPool->lock(env);
        result
            = _memoryPool->collectorAllocateTLH(env, allocDescription, maximumBytesRequired, addrBase, addrTop, false);
        if ((NULL == result) && allocDescription->isCollectorAllocateExpandOnFailure()) {
            if (0 != collectorExpand(env, requestCollector, allocDescription)) {
                allocDescription->setCollectorAllocateSatisfyAnywhere(true);
 ...
 */
bool MM_MemorySubSpaceGeneric::completeFreelistRebuildRequired(MM_EnvironmentBase* env)
{
 ...
 * Initialization
 */
MM_MemorySubSpaceGeneric* MM_MemorySubSpaceGeneric::newInstance(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool,
    MM_RegionPool* regionPool, bool usesGlobalCollector, uintptr_t minimumSize, uintptr_t initialSize,
    uintptr_t maximumSize, uintptr_t memoryType, uint32_t objectFlags)
{
    ...
    if (NULL != memorySubSpace) {
        new (memorySubSpace) MM_MemorySubSpaceGeneric(env, memoryPool, regionPool, usesGlobalCollector, minimumSize,
 ...
 */
bool MM_MemorySubSpaceGeneric::expanded(
 ...
 */
bool MM_MemorySubSpaceGeneric::expanded(MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, uintptr_t size,
 ...
 */
void MM_MemorySubSpaceGeneric::addExistingMemory(MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, uintptr_t size,
    void* lowAddress, void* highAddress, bool canCoalesce)
{
    /* Feed the range to the memory pool */
    _memoryPool->expandWithRange(env, size, lowAddress, highAddress, canCoalesce);
 ...
 */
void* MM_MemorySubSpaceGeneric::findFreeEntryEndingAtAddr(MM_EnvironmentBase* env, void* addr)
{
 ...
 */
void* MM_MemorySubSpaceGeneric::findFreeEntryTopStartingAtAddr(MM_EnvironmentBase* env, void* addr)
{
 ...
 */
void* MM_MemorySubSpaceGeneric::getFirstFreeStartingAddr(MM_EnvironmentBase* env)
{
 ...
 */
void* MM_MemorySubSpaceGeneric::getNextFreeStartingAddr(MM_EnvironmentBase* env, void* currentFree)
{
 ...
 *
 */
void MM_MemorySubSpaceGeneric::moveHeap(MM_EnvironmentBase* env, void* srcBase, void* srcTop, void* dstBase)
{
...
void MM_MemorySubSpaceGeneric::addTenureRange(MM_EnvironmentBase* env, uintptr_t size, void* low, void* high)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
...
void MM_MemorySubSpaceGeneric::removeTenureRange(MM_EnvironmentBase* env, uintptr_t size, void* low, void* high)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    Assert_MM_true((uintptr_t)high - (uintptr_t)low == size);
...
uintptr_t MM_MemorySubSpaceGeneric::releaseFreeMemoryPages(MM_EnvironmentBase* env)
{
