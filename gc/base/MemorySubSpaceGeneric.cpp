 ...
 */
MM_MemoryPool* MM_MemorySubSpaceGeneric::getMemoryPool() { return _memoryPool; }

/**
 * Return the number of memory pools associated to the receiver.
 * @return count of number of memory pools
 */
uintptr_t MM_MemorySubSpaceGeneric::getMemoryPoolCount() {
 ...
 * @return count of number of memory pools
 */
uintptr_t MM_MemorySubSpaceGeneric::getActiveMemoryPoolCount() {
 ...
 * @return MM_MemoryPool
 */
MM_MemoryPool* MM_MemorySubSpaceGeneric::getMemoryPool(void* addr) {
 ...
 * @return MM_MemoryPool
 */
MM_MemoryPool* MM_MemorySubSpaceGeneric::getMemoryPool(uintptr_t size) {
 ...
 */
MM_MemoryPool* MM_MemorySubSpaceGeneric::getMemoryPool(MM_EnvironmentBase* env,
                                                       void* addrBase,
                                                       void* addrTop,
                                                       void*& highAddr) {
 ...
 */
uintptr_t MM_MemorySubSpaceGeneric::getActualFreeMemorySize() {
  if (isActive()) {
 ...
 */
uintptr_t MM_MemorySubSpaceGeneric::getApproximateFreeMemorySize() {
  if (isActive()) {
...
uintptr_t MM_MemorySubSpaceGeneric::getActiveMemorySize() {
...
uintptr_t MM_MemorySubSpaceGeneric::getActiveLOAMemorySize(
    uintptr_t includeMemoryType) {
  if (getTypeFlags() & includeMemoryType) {
 ...
 */
uintptr_t MM_MemorySubSpaceGeneric::getActualActiveFreeMemorySize() {
  return _memoryPool->getActualFreeMemorySize();
}
 ...
 */
uintptr_t MM_MemorySubSpaceGeneric::getApproximateActiveFreeMemorySize() {
  return _memoryPool->getApproximateFreeMemorySize();
}
 ...
 */
uintptr_t MM_MemorySubSpaceGeneric::getApproximateActiveFreeLOAMemorySize() {
...
void MM_MemorySubSpaceGeneric::mergeHeapStats(MM_HeapStats* heapStats) {
...
void MM_MemorySubSpaceGeneric::resetHeapStatistics(bool globalCollect) {
...
void* MM_MemorySubSpaceGeneric::allocateObject(
  ...
  Trc_MM_MSSGeneric_allocate_entry(
      env->getLanguageVMThread(), "Object",
      allocDescription->getBytesRequested(), this, getName(), baseSubSpace,
      previousSubSpace, (uintptr_t)_allocateAtSafePointOnly,
      (uintptr_t)shouldCollectOnFailure, (uintptr_t)_isAllocatable);

  void* result = NULL;

  /* Typically, JIT stack frame is not built (shouldCollectOnFailure is false)
   * and concurrent mark is not running or not complete
   * (_allocateAtSafePointOnly is false), so we most often proceed with
   * allocate. If we are notified that concurrent mark is complete (
   * _allocateAtSafePointOnly is true, and typically the stack frame is not
   * built yet, we should not proceed with allocate, but return NULL and force
   * JIT to call us again with the frame built Next time when the frame is built
   * (shouldCollectOnFailure is true, while _allocateAtSafePointOnly is still
   * true) we proceed with allocate (otherwise risk OOM),
   * but on the way out of this allocate, when we pay tax, concurrent mark will
   * trigger the final phase. */
    ...
    if (NULL != result) {
      ...
      if (shouldCollectOnFailure) {
        ...
        result = _parent->allocationRequestFailed(
            env, allocDescription, ALLOCATION_TYPE_OBJECT, NULL, this, this);
      } else {
        Trc_MM_MSSGeneric_allocate(env->getLanguageVMThread(), "Object",
                                   allocDescription->getBytesRequested(), 2,
                                   this, _parent);
        result = _parent->allocateObject(env, allocDescription, baseSubSpace,
                                         this, shouldCollectOnFailure);
      }
    }
  }

  Trc_MM_MSSGeneric_allocate_exit(env->getLanguageVMThread(), "Object",
 ...
 */
void* MM_MemorySubSpaceGeneric::allocateArrayletLeaf(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* baseSubSpace, MM_MemorySubSpace* previousSubSpace,
    bool shouldCollectOnFailure) {
  void* result = NULL;
  ...
  if (!_allocateAtSafePointOnly || shouldCollectOnFailure) {
    ...
    if (NULL == result) {
      ...
      if (shouldCollectOnFailure) {
        result = _parent->allocationRequestFailed(
            env, allocDescription, ALLOCATION_TYPE_LEAF, NULL, this, this);
      } else {
        result = _parent->allocateArrayletLeaf(
...
void* MM_MemorySubSpaceGeneric::allocationRequestFailed(
    ...
    MM_MemorySubSpace* baseSubSpace, MM_MemorySubSpace* previousSubSpace) {
  void* result = allocateGeneric(env, allocateDescription, allocationType,
...
void* MM_MemorySubSpaceGeneric::allocateTLH(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_ObjectAllocationInterface* objectAllocationInterface,
    MM_MemorySubSpace* baseSubSpace, MM_MemorySubSpace* previousSubSpace,
    bool shouldCollectOnFailure) {
  Trc_MM_MSSGeneric_allocate_entry(
      env->getLanguageVMThread(), "TLH", allocDescription->getBytesRequested(),
   ...
   * _allocateAtSafePointOnly is true), and typically the stack frame is not
   * built yet, we should not proceed with allocate, but return NULL and force
   * JIT to call us again with the frame built. Next time when the frame is
   * built (shouldCollectOnFailure is true, while _allocateAtSafePointOnly is
   * still true) we proceed with allocate (otherwise risk OOM), but on the way
   * out of this allocate, when we pay tax, concurrent mark will trigger the
   * final phase. */
  if (!_allocateAtSafePointOnly || shouldCollectOnFailure) {
    if (_isAllocatable) {
      result = objectAllocationInterface->allocateTLH(env, allocDescription,
                                                      this, _memoryPool);
    }

    if (NULL == result) {
      if (shouldCollectOnFailure) {
        Trc_MM_MSSGeneric_allocate3(
            env->getLanguageVMThread(), "TLH",
        ...
        if (allocDescription->shouldCollectAndClimb()) {
          result = _parent->allocationRequestFailed(
              env, allocDescription, ALLOCATION_TYPE_TLH,
              objectAllocationInterface, this, this);
        }
      } else {
        Trc_MM_MSSGeneric_allocate(env->getLanguageVMThread(), "TLH",
                                   allocDescription->getBytesRequested(), 2,
                                   this, _parent);
        result = _parent->allocateTLH(env, allocDescription,
                                      objectAllocationInterface, baseSubSpace,
                                      this, false);
      }
    }
  }

  Trc_MM_MSSGeneric_allocate_exit(env->getLanguageVMThread(), "TLH",
                                  allocDescription->getBytesRequested(), this,
                                  result);
 ...
 ****************************************
 */
void* MM_MemorySubSpaceGeneric::collectorAllocate(
  ...
  if (NULL == result) {
    ...
    if ((NULL == result) &&
        allocDescription->isCollectorAllocateExpandOnFailure()) {
...
#if defined(OMR_GC_THREAD_LOCAL_HEAP)
void* MM_MemorySubSpaceGeneric::collectorAllocateTLH(
    MM_EnvironmentBase* env, MM_Collector* requestCollector,
    MM_AllocateDescription* allocDescription, uintptr_t maximumBytesRequired,
    void*& addrBase, void*& addrTop) {
  void* result = NULL;

  result = _memoryPool->collectorAllocateTLH(
      env, allocDescription, maximumBytesRequired, addrBase, addrTop, true);
  if (NULL == result) {
    _memoryPool->lock(env);
    result = _memoryPool->collectorAllocateTLH(
        env, allocDescription, maximumBytesRequired, addrBase, addrTop, false);
    if ((NULL == result) &&
        allocDescription->isCollectorAllocateExpandOnFailure()) {
      if (0 != collectorExpand(env, requestCollector, allocDescription)) {
        allocDescription->setCollectorAllocateSatisfyAnywhere(true);
        result = _memoryPool->collectorAllocateTLH(env, allocDescription,
...
void MM_MemorySubSpaceGeneric::abandonHeapChunk(void* addrBase, void* addrTop) {
  /* OMRTODO turn tarokEnableExpensiveAssertions into a generic (non tarok
   * specific option) */
  if (_extensions->tarokEnableExpensiveAssertions) {
    MM_HeapRegionDescriptor* region = NULL;
    GC_MemorySubSpaceRegionIterator regionIterator(this);
    while (NULL != (region = regionIterator.nextRegion())) {
      if ((region->getLowAddress() <= addrBase) &&
          (addrTop <= region->getHighAddress())) {
        break;
      }
    }
 ...
 * Generational style collectors, which move objects from one memory subspace to
 * another during collection, may require that the destination area expand
 * during the collection cycle.  An example of this would be the tenure area in
 * a 2-generational system, if the tenure area memory has been exhausted.
 * @note This call is not protected by any locking mechanism.
 */
uintptr_t MM_MemorySubSpaceGeneric::collectorExpand(
    MM_EnvironmentBase* env, MM_Collector* requestCollector,
    MM_AllocateDescription* allocDescription) {
 ...
 ****************************************
 */
MM_MemorySubSpace* MM_MemorySubSpaceGeneric::getDefaultMemorySubSpace() {
  return _parent->getDefaultMemorySubSpace();
}

MM_MemorySubSpace* MM_MemorySubSpaceGeneric::getTenureMemorySubSpace() {
  return _parent->getTenureMemorySubSpace();
}

bool MM_MemorySubSpaceGeneric::isActive() {
  if (_parent) {
 ...
 */
bool MM_MemorySubSpaceGeneric::completeFreelistRebuildRequired(
...
void MM_MemorySubSpaceGeneric::reset() {
 ...
 * everything is free
 */
void MM_MemorySubSpaceGeneric::rebuildFreeList(MM_EnvironmentBase* env) {
  if (env->getExtensions()->isVLHGC()) {
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
      freeListEntry =
          _memoryPool->rebuildFreeListInRegion(env, region, freeListEntry);
    }
 ...
 * Initialization
 */
MM_MemorySubSpaceGeneric* MM_MemorySubSpaceGeneric::newInstance(
    MM_EnvironmentBase* env, MM_MemoryPool* memoryPool,
    MM_RegionPool* regionPool, bool usesGlobalCollector, uintptr_t minimumSize,
    uintptr_t initialSize, uintptr_t maximumSize, uintptr_t memoryType,
    uint32_t objectFlags) {
  MM_MemorySubSpaceGeneric* memorySubSpace;

  memorySubSpace = (MM_MemorySubSpaceGeneric*)env->getForge()->allocate(
      sizeof(MM_MemorySubSpaceGeneric), OMR::GC::AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (NULL != memorySubSpace) {
    new (memorySubSpace) MM_MemorySubSpaceGeneric(
        env, memoryPool, regionPool, usesGlobalCollector, minimumSize,
...
bool MM_MemorySubSpaceGeneric::initialize(MM_EnvironmentBase* env) {
...
void MM_MemorySubSpaceGeneric::tearDown(MM_EnvironmentBase* env) {
  /* Reset the fields in extensions */
  MM_GCExtensionsBase* extensions = env->getExtensions();
  extensions->heapBaseForBarrierRange0 = 0;
  extensions->heapSizeForBarrierRange0 = 0;
  extensions->setTenureAddressRange(extensions->heapBaseForBarrierRange0,
 ...
 */
bool MM_MemorySubSpaceGeneric::expanded(MM_EnvironmentBase* env,
  ...
  if (result) {
    ...
    _memoryPool->expandWithRange(env, size, lowAddress, highAddress,
                                 canCoalesce);
  }
  return result;
}
 ...
 * Memory described by the range has added to the heap and been made available
 * to the subspace as free memory.
 * @Note this code only exists for Phase 2 Gencon collectors.  No new code
 * should use this API.
 */
bool MM_MemorySubSpaceGeneric::expanded(MM_EnvironmentBase* env,
                                        MM_PhysicalSubArena* subArena,
  ...
  if (result) {
    addExistingMemory(env, subArena, size, lowAddress, highAddress,
                      canCoalesce);
  }
  return result;
}
 ...
 */
void MM_MemorySubSpaceGeneric::addExistingMemory(
    MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, uintptr_t size,
 ...
 * removed from the current memory space's ownership.  Adjust accordingly.
 * @note Size information (current) is not updated.
 * @warn This routine is fairly hacky - is there a better way?
 */
void* MM_MemorySubSpaceGeneric::removeExistingMemory(
    MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, uintptr_t size,
 ...
 */
void* MM_MemorySubSpaceGeneric::findFreeEntryEndingAtAddr(
 ...
 */
void* MM_MemorySubSpaceGeneric::findFreeEntryTopStartingAtAddr(
    MM_EnvironmentBase* env, void* addr) {
 ...
 */
void* MM_MemorySubSpaceGeneric::getFirstFreeStartingAddr(
    MM_EnvironmentBase* env) {
 ...
 */
void* MM_MemorySubSpaceGeneric::getNextFreeStartingAddr(MM_EnvironmentBase* env,
                                                        void* currentFree) {
 ...
 *
 */
void MM_MemorySubSpaceGeneric::moveHeap(MM_EnvironmentBase* env, void* srcBase,
...
void MM_MemorySubSpaceGeneric::addTenureRange(MM_EnvironmentBase* env,
                                              uintptr_t size, void* low,
                                              void* high) {
  MM_GCExtensionsBase* extensions = env->getExtensions();
  ...
  } else if (low == (void*)((uintptr_t)extensions->heapBaseForBarrierRange0 +
                            extensions->heapSizeForBarrierRange0)) {
...
void MM_MemorySubSpaceGeneric::removeTenureRange(MM_EnvironmentBase* env,
                                                 uintptr_t size, void* low,
                                                 void* high) {
  MM_GCExtensionsBase* extensions = env->getExtensions();
  Assert_MM_true((uintptr_t)high - (uintptr_t)low == size);
  ...
  if (low == extensions->heapBaseForBarrierRange0) {
    ...
    extensions->heapSizeForBarrierRange0 -= size;
  } else if (high == (void*)((uintptr_t)extensions->heapBaseForBarrierRange0 +
                             extensions->heapSizeForBarrierRange0)) {
...
uintptr_t MM_MemorySubSpaceGeneric::releaseFreeMemoryPages(
    MM_EnvironmentBase* env) {
