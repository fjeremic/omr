...
private:
  uintptr_t
      _exclusiveAccessCount; /**< The number of times exclusive access requests
                                have been made to use the receiver */
...
protected:
  ...
  uint64_t _masterThreadCpuTimeStart; /**< slot to store the master CPU time at
                                         the beginning of the collection */

public:
  /*
   * Determine if a collector (some parent, typically the global collector)
   * wishes to usurp any minor collection.
   * @return boolean indicating if the parent collector should be invoked in
   * place of a child.
   */
   ...
   * @note This call is only valid for any concluded GC activity and is
   * undefined for in progress activity.
...
protected:
   ...
   * @param aggressive Set TRUE is previous collect failed to free sufficient
   * storage to request that this collect should be more aggressive, ie clear
   * any soft references.
   * @return true if gc completed OK, false otherwise
   */
  virtual bool
  internalGarbageCollect(MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
                         MM_AllocateDescription *allocDescription) = 0;

  /**
   * Perform any collector setup activities.
   * @param env Master GC thread.
   * @param subSpace the memory subspace where the collection is occurring
   * @param allocDescription Allocation description causing the GC (or detailing
   * the GC request)
  ...
  /**
   * Post collection activities,  indicating that the collection has been
   * completed.
   * @param subSpace the memory subspace where the collection occurred
   */
  virtual void internalPostCollect(MM_EnvironmentBase *env,
                                   MM_MemorySubSpace *subSpace) = 0;

  /**
   * Pre-condition initialization for garbage collection cycle.
   * This routine is meant to be an initializer for meta state on a Gc cycle,
   * such as the aggressiveness of collection, excessive GC calculation, and
   * cycle state setup for the master thread.
   * @param env Master GC thread.
   * @param subSpace the memory subspace where the collection is occurring
   * @param allocDescription Allocation description causing the GC (or detailing
   * the GC request)
   * @param gcCode High level reason for invoking the GC
   */
  void preCollect(MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
...
public:
   ...
   * @note All collectors must implement this method - the IDs are defined in
   * @ref j9modron.h
   */
   ...
   * @param highAddress The top address (non-inclusive) of the memory added to
   * the heap
  ...
  virtual bool heapAddRange(MM_EnvironmentBase *env,
                            ...
                            void *lowAddress, void *highAddress) = 0;
   ...
   * @param highAddress The top address (non-inclusive) of the memory removed
   * from the heap
   * @param lowValidAddress The first valid address previous to the lowest in
   * the heap range being removed
   * @param highValidAddress The first valid address following the highest in
   * the heap range being removed
   * @return true if operation completes with success
   */
  ...
  void *garbageCollect(MM_EnvironmentBase *env,
  ...
  virtual bool forceKickoff(MM_EnvironmentBase *env,
                            MM_MemorySubSpace *subSpace,
                            MM_AllocateDescription *allocDescription,
                            uint32_t gcCode);
  ...
  virtual uint32_t getGCTimePercentage(MM_EnvironmentBase *env) { return 0; };
   ...
   */
  void setGlobalCollector(bool isGlobal) { _globalCollector = isGlobal; }

  /**
   * Called to request the number of times the environment has incremented the
   * exclusive access count of the receiver. This is most often used to
   * determine if a GC completed while a thread was blocked requesting
   * exclusive.
   * @return The number of exclusive access requests made to use this collector
   */
  MMINLINE uintptr_t getExclusiveAccessCount() { return _exclusiveAccessCount; }

  /**
   * Called by the environment to increment the number of exclusive access
   * requests made to use this collector.
   */
  MMINLINE void incrementExclusiveAccessCount() { _exclusiveAccessCount += 1; }
  ...
  virtual void deleteSweepPoolState(MM_EnvironmentBase *env,
                                    void *sweepPoolState) = 0;

  virtual void abortCollection(MM_EnvironmentBase *env,
   ...
   * @return true if the collector implementation knows that the given objectPtr
   * is marked (false if not or if the implementation doesn't know)
   */
  ...
  MM_Collector()
      : MM_BaseVirtual(), _exclusiveAccessCount(0), _bytesRequested(0),
        _globalCollector(false), _gcCompleted(false), _isRecursiveGC(false),
        _disableGC(false), _collectorExpandedSize(0),
        _cycleType(OMR_GC_CYCLE_TYPE_DEFAULT), _masterThreadCpuTimeStart(0) {
