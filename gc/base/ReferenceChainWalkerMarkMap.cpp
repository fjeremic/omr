    ...
    MM_EnvironmentBase* env, uintptr_t maxHeapSize)
{
    MM_ReferenceChainWalkerMarkMap* markMap = (MM_ReferenceChainWalkerMarkMap*)env->getForge()->allocate(
        sizeof(MM_ReferenceChainWalkerMarkMap), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (NULL != markMap) {
...
bool MM_ReferenceChainWalkerMarkMap::clearMapForRegions(MM_EnvironmentBase* env, bool commit)
{
            ...
            if (commit) {
                ...
                if (_extensions->isFvtestForceReferenceChainWalkerMarkMapCommitFailure()) {
                    ...
                    if (!memoryManager->commitMemory(&_heapMapMemoryHandle,
                            (void*)(((uintptr_t)_heapMapBits) + heapMapClearIndex), heapMapClearSize)) {
                        result = false;
                        /* print tracepoint in case of real failure */
                        Trc_MM_ReferenceChainWalkerMarkMap_markMapCommitFailed(env->getLanguageVMThread(),
                            (void*)(((uintptr_t)_heapMapBits) + heapMapClearIndex), heapMapClearSize);
                        break;
                    }
                }
