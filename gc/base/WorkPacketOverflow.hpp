...
public:
  volatile bool
      _overflow; /**< Volatile flag to report about overflow latched */

protected:
  MM_WorkPackets *_workPackets;
  omrthread_monitor_t _overflowListMonitor; /**< Monitor to be held when
                                               modifying the overflow list */
private:
  /*
   * Function members
   */
public:
  static MM_WorkPacketOverflow *newInstance(MM_EnvironmentBase *env,
  ...
  virtual void emptyToOverflow(MM_EnvironmentBase *env, MM_Packet *packet,
                               MM_OverflowType type);
  virtual void fillFromOverflow(MM_EnvironmentBase *env, MM_Packet *packet);
  virtual void overflowItem(MM_EnvironmentBase *env, void *item,
                            MM_OverflowType type);
  ...
  MM_WorkPacketOverflow(MM_EnvironmentBase *env, MM_WorkPackets *workPackets)
      : MM_BaseVirtual(), _overflow(false), _workPackets(workPackets),
