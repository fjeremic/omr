...
class MM_MemoryPoolBumpPointer : public MM_MemoryPool {
     ...
     */
private:
    void* _allocatePointer; /**< The base address of the unused portion of the receiver and the next pointer which could
                               be returned by an allocation request */
    ...
    MM_HeapLinkedFreeHeader* _lastFreeEntry; /**< address of the last free entry in the pool; valid after sweepforPGC;
                                                NOT maintained during allocation */
protected:
public:
    /*
     * Function members
...
public:
    ...
    /**
     * Called by the WriteOnceCompactor after it has internally compacted a region to update the receiver's
     * _allocatePointer so that it is consistent with the objects in the receiver's managed memory.
     * @param env[in] A GC thread
     * @param allocatePointer[in] The first byte available for object allocations (if we were permitting fragmented
     * allocates)
     */
    ...
    MMINLINE uintptr_t getFreeMemoryAndDarkMatterBytes()
    {
        ...
        /* note that actualFreeMemory can be measured as less than allocatableMemory if sweep calculated it since it
         * won't see the very end of the pool if it is less than minimumFreeEntrySize.
         */
    ...
    /**
     * Used when a caller wishes to determine the end of the allocated space in the receiver.  Note that the space
     * between this valud and _topPointer has undefined contents.
     * @return The point address where the next object will be allocated or _topPointer if the pool cannot satisfy any
     * more allocates
     */
     ...
     * @note The caller must ensure that access to the receiver is serialized as the implementation does not log or
     ...
     * @param pointer[in] The new value to be used as the receiver's allocation pointer for the next tail-filling
     * opportunity
     */
