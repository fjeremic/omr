...
void MM_NUMAManager::shouldEnablePhysicalNUMA(bool numaEnabled) {
 ...
 */
static int compareNodeNumberFunc(const void* element1, const void* element2) {
...
bool MM_NUMAManager::recacheNUMASupport(MM_EnvironmentBase* env) {
  bool result = true;
  ...
  if (_physicalNumaEnabled) {
    ...
    if (0 != detailResult) {
      /* something went wrong in the underlying call so ignore any NUMA count
       * data we might have received */
      nodeCount = 0;
    }
  } else {
  ...
  if (0 != nodeCount) {
    /* we want to support NUMA either via the machine's physical NUMA or our
     * simulated (aka "purely logical") NUMA */
    uintptr_t nodeArraySize = sizeof(J9MemoryNodeDetail) * nodeCount;
    _activeNodes = (J9MemoryNodeDetail*)env->getForge()->allocate(
        nodeArraySize, OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (NULL == _activeNodes) {
      result = false;
    } else {
      ...
      if (_physicalNumaEnabled) {
        ...
        intptr_t detailResult =
            omrvmem_numa_get_node_details(_activeNodes, &_activeNodeCount);
        ...
        for (uintptr_t i = 0; i < nodeCount; i++) {
          ...
          /* NOTE:  if we ever start counting these resources to determine GC
           * thread counts, this will have to be something other than "1" */
          _activeNodes[i].computationalResourcesAvailable = 1;
        }
      }

      /* Sorting the array in j9NodeNumber ascending order.
       * This sorting is not really required, but gives us ability to make some
       * stronger assertions later in the code
       */
      J9_SORT(_activeNodes, _activeNodeCount, sizeof(J9MemoryNodeDetail),
              compareNodeNumberFunc);

      /* now that we have the total list of active nodes, identify the affinity
       * leaders */
      uintptr_t preferredWithCPU = 0;
      uintptr_t allowedWithCPU = 0;
      /* if "preferred" nodes exist, we will use them as the leaders and fall
       * back to "allowed" if there aren't any so count both and choose them
       * after the loop */
      for (uintptr_t activeNodeIndex = 0; activeNodeIndex < _activeNodeCount;
           activeNodeIndex++) {
        uintptr_t cpu =
            _activeNodes[activeNodeIndex].computationalResourcesAvailable;
        if (0 != cpu) {
          ...
          if (J9NUMA_PREFERRED == policy) {
            ...
            /* nodes with CPUs but DENIED bindings go into the free processor
             * pool */
            _freeProcessorPoolNodeCount += 1;
          }
        }
        /* since we are walking all the nodes, this is also the time to update
         * the maximum node number */
        _maximumNodeNumber = OMR_MAX(
            _maximumNodeNumber, _activeNodes[activeNodeIndex].j9NodeNumber);
      }
      ...
      if (0 != _affinityLeaderCount) {
        uintptr_t affinityLeaderArraySize =
            sizeof(J9MemoryNodeDetail) * _affinityLeaderCount;
        _affinityLeaders = (J9MemoryNodeDetail*)env->getForge()->allocate(
            affinityLeaderArraySize, OMR::GC::AllocationCategory::FIXED,
            OMR_GET_CALLSITE());
        if (NULL == _affinityLeaders) {
          result = false;
        } else {
          ...
          for (uintptr_t activeNodeIndex = 0;
               activeNodeIndex < _activeNodeCount; activeNodeIndex++) {
            if ((0 != _activeNodes[activeNodeIndex]
                          .computationalResourcesAvailable) &&
      ...
      if (0 != _freeProcessorPoolNodeCount) {
        ...
        uintptr_t processorPoolArraySize =
            sizeof(J9MemoryNodeDetail) * _freeProcessorPoolNodeCount;
        _freeProcessorPoolNodes =
            (J9MemoryNodeDetail*)env->getForge()->allocate(
                processorPoolArraySize, OMR::GC::AllocationCategory::FIXED,
                OMR_GET_CALLSITE());
        if (NULL == _freeProcessorPoolNodes) {
          result = false;
        } else {
          ...
          for (uintptr_t activeNodeIndex = 0;
            ...
            if ((0 != _activeNodes[activeNodeIndex]
              ...
              _freeProcessorPoolNodes[nextIndex] =
                  _activeNodes[activeNodeIndex];
              nextIndex += 1;
            }
          }
          Assert_MM_true(nextIndex == _freeProcessorPoolNodeCount);
...
void MM_NUMAManager::setSimulatedNodeCountForFVTest(
    uintptr_t simulatedNodeCount) {
  _simulatedNodeCount = simulatedNodeCount;
  _physicalNumaEnabled = false;
}

uintptr_t MM_NUMAManager::getAffinityLeaderCount() const {
  return _affinityLeaderCount;
}

uintptr_t MM_NUMAManager::getMaximumNodeNumber() const {
  return _maximumNodeNumber;
}

J9MemoryNodeDetail const* MM_NUMAManager::getAffinityLeaders(
    uintptr_t* arrayLength) const {
  *arrayLength = _affinityLeaderCount;
  return _affinityLeaders;
}

J9MemoryNodeDetail const* MM_NUMAManager::getFreeProcessorPool(
    uintptr_t* arrayLength) const {
  *arrayLength = _freeProcessorPoolNodeCount;
  return _freeProcessorPoolNodes;
}

uintptr_t MM_NUMAManager::getComputationalResourcesAvailableForAllNodes()
    const {
  uintptr_t computationalResourceCount = 0;

  for (uintptr_t activeNodeIndex = 0; activeNodeIndex < _activeNodeCount;
       activeNodeIndex++) {
    computationalResourceCount +=
        _activeNodes[activeNodeIndex].computationalResourcesAvailable;
  }
...
bool MM_NUMAManager::isPhysicalNUMASupported() const {
  return (0 != _physicalNumaEnabled);
}

void MM_NUMAManager::shutdownNUMASupport(MM_EnvironmentBase* env) {
  _physicalNumaEnabled = false;
  _simulatedNodeCount = 0;
  /* just call recache since it knows how to clean up when NUMA has been
   * disabled */
