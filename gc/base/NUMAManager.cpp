        ...
        if (NULL == _activeNodes) {
            ...
            if (_physicalNumaEnabled) {
                ...
                for (uintptr_t i = 0; i < nodeCount; i++) {
                    ...
                    /* NOTE:  if we ever start counting these resources to determine GC thread counts, this will have to
             ...
             * This sorting is not really required, but gives us ability to make some stronger assertions later in the
             * code
             */
            ...
            /* if "preferred" nodes exist, we will use them as the leaders and fall back to "allowed" if there aren't
             * any so count both and choose them after the loop */
            ...
            if (0 != _affinityLeaderCount) {
                ...
                _affinityLeaders = (J9MemoryNodeDetail*)env->getForge()->allocate(
                    affinityLeaderArraySize, OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
                if (NULL == _affinityLeaders) {
                    result = false;
                } else {
            ...
            if (0 != _freeProcessorPoolNodeCount) {
                ...
                _freeProcessorPoolNodes = (J9MemoryNodeDetail*)env->getForge()->allocate(
                    processorPoolArraySize, OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
                if (NULL == _freeProcessorPoolNodes) {
                    result = false;
                } else {
...
void MM_NUMAManager::setSimulatedNodeCountForFVTest(uintptr_t simulatedNodeCount)
{
...
J9MemoryNodeDetail const* MM_NUMAManager::getAffinityLeaders(uintptr_t* arrayLength) const
{
    *arrayLength = _affinityLeaderCount;
    return _affinityLeaders;
}

J9MemoryNodeDetail const* MM_NUMAManager::getFreeProcessorPool(uintptr_t* arrayLength) const
{
    *arrayLength = _freeProcessorPoolNodeCount;
    return _freeProcessorPoolNodes;
}

uintptr_t MM_NUMAManager::getComputationalResourcesAvailableForAllNodes() const
{
