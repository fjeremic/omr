...
MM_PhysicalSubArenaRegionBased *MM_PhysicalSubArenaRegionBased::newInstance(
    MM_EnvironmentBase *env, MM_Heap *heap) {
  MM_PhysicalSubArenaRegionBased *arena =
      (MM_PhysicalSubArenaRegionBased *)env->getForge()->allocate(
          sizeof(MM_PhysicalSubArenaRegionBased),
...
bool MM_PhysicalSubArenaRegionBased::initialize(MM_EnvironmentBase *env) {
...
#if defined(OMR_GC_VLHGC)
    _affinityLeaders =
        _extensions->_numaManager.getAffinityLeaders(&_affinityLeaderCount);
#endif /* defined (OMR_GC_VLHGC) */
  }
...
void MM_PhysicalSubArenaRegionBased::tearDown(MM_EnvironmentBase *env) {
  MM_PhysicalSubArena::tearDown(env);
}

bool MM_PhysicalSubArenaRegionBased::inflate(MM_EnvironmentBase *env) {
  return _parent->attachSubArena(env, this, _subSpace->getInitialSize(),
                                 modron_pavm_attach_policy_none);
}

uintptr_t MM_PhysicalSubArenaRegionBased::getNextNumaNode() {
  uintptr_t result = 0;
...
uintptr_t MM_PhysicalSubArenaRegionBased::getPreviousNumaNode() {
  uintptr_t result = 0;
  ...
  if (_extensions->isVLHGC()) {
    ...
    if (_affinityLeaderCount > 0) {
      _nextNUMAIndex =
          (_nextNUMAIndex + _affinityLeaderCount - 1) % _affinityLeaderCount;
 ...
 */
uintptr_t MM_PhysicalSubArenaRegionBased::doContractInSubSpace(
    MM_EnvironmentBase *env, uintptr_t contractSize,
    MM_MemorySubSpace *subspace) {
  uintptr_t didContractBy = 0;
  MM_HeapRegionManagerTarok *manager =
      MM_HeapRegionManagerTarok::getHeapRegionManager(_heap);
  ...
  while (didContractBy < contractSize) {
    ...
    MM_HeapRegionDescriptor *regionToRelease =
        subspace->selectRegionForContraction(env, numaIndex);
    ...
    void *contractBase =
        subspace->removeExistingMemory(env, this, regionSize, base, top);
    ...
    void *lowValidAddress =
        manager->findHighestValidAddressBelow(regionToRelease);
    void *highValidAddress =
        manager->findLowestValidAddressAbove(regionToRelease);

    /* decommits the memory */
    _heap->decommitMemory(contractBase, regionSize, lowValidAddress,
                          highValidAddress);

    void *contractTop = (void *)(((uintptr_t)contractBase) + regionSize);
    /* Broadcast that heap has been removed */
    subspace->heapRemoveRange(env, _subSpace, regionSize, contractBase,
                              contractTop, lowValidAddress, highValidAddress);
 ...
 */
uintptr_t MM_PhysicalSubArenaRegionBased::doExpandInSubSpace(
  ...
  /*
   due to heap address for each numa node has to alignment with region size and
   page size, free region count might be not distributed evenly among numa
   nodes. use numaNodeCountWithFreeRegions and areFreeRegionsForNode() to
   inflate all available regions from numa nodes round robin until reaching
   expanded size.
   */
  ...
  for (uintptr_t idx = 0; idx < _affinityLeaderCount; idx++) {
    if (!manager->areFreeRegionsForNode(env,
                                        _affinityLeaders[idx].j9NodeNumber)) {
  ...
  while (didExpandBy < expandSize) {
    ...
    MM_HeapRegionDescriptor *newRegion =
        manager->acquireSingleTableRegion(env, subspace, numaNode);
    if ((0 != numaNode) && (!manager->areFreeRegionsForNode(env, numaNode))) {
      numaNodeCountWithFreeRegions -= 1;
    }
    ...
    Assert_MM_true((0 == _affinityLeaderCount) ||
                   ((formerNodeIndex + 1) % _affinityLeaderCount) ==
                       _nextNUMAIndex);
  }
  validateNumaSymmetry(env);
  subspace->heapReconfigured(env);
...
uintptr_t MM_PhysicalSubArenaRegionBased::performExpand(MM_EnvironmentBase *env,
                                                        uintptr_t expandSize) {
  uintptr_t result = 0;

  if (((MM_PhysicalArenaRegionBased *)_parent)
          ->canResize(env, this, expandSize)) {
    MM_MemorySubSpace *subSpace = getSubSpace();
    subSpace =
        (NULL == subSpace->getChildren()) ? subSpace : subSpace->getChildren();
...
uintptr_t MM_PhysicalSubArenaRegionBased::expand(MM_EnvironmentBase *env,
                                                 uintptr_t expandSize) {
  return performExpand(env, expandSize);
}

uintptr_t MM_PhysicalSubArenaRegionBased::contract(MM_EnvironmentBase *env,
                                                   uintptr_t expandSize) {
  if (((MM_PhysicalArenaRegionBased *)_parent)
          ->canResize(env, this, expandSize)) {
...
bool MM_PhysicalSubArenaRegionBased::canContract(MM_EnvironmentBase *env) {
  return _resizable;
}

uintptr_t MM_PhysicalSubArenaRegionBased::getAvailableContractionSize(
    MM_EnvironmentBase *env, MM_MemorySubSpace *memorySubSpace,
    MM_AllocateDescription *allocDescription) {
  /* region based heap is discontiguous so we don't have the restriction that we
   * must contract at the end of the subspace */
  return UDATA_MAX;
}
 ...
 */
void MM_PhysicalSubArenaRegionBased::kill(MM_EnvironmentBase *env) {
  tearDown(env);
  env->getForge()->free(this);
}

void MM_PhysicalSubArenaRegionBased::validateNumaSymmetry(
    MM_EnvironmentBase *env) {
  Trc_MM_PhysicalSubArenaRegionBased_validateNumaSymmetry_Entry(
    ...
    if (_extensions->tarokEnableExpensiveAssertions) {
      if (_affinityLeaderCount > 0) {
        ...
        Trc_MM_PhysicalSubArenaRegionBased_validateNumaSymmetry_TotalSummary(
            env->getLanguageVMThread(), highestCount, lowestCount, nodeCount,
            _affinityLeaderCount);
        Assert_MM_true(highestCount <= (lowestCount + 1));
        Assert_MM_true((nodeCount == _affinityLeaderCount) ||
                       (1 >= highestCount));
      }
    }
#endif /* defined (OMR_GC_VLHGC) */
  }
  Trc_MM_PhysicalSubArenaRegionBased_validateNumaSymmetry_Exit(
      env->getLanguageVMThread());
}
