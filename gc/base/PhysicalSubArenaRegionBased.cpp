...
MM_PhysicalSubArenaRegionBased* MM_PhysicalSubArenaRegionBased::newInstance(MM_EnvironmentBase* env, MM_Heap* heap)
{
    MM_PhysicalSubArenaRegionBased* arena = (MM_PhysicalSubArenaRegionBased*)env->getForge()->allocate(
        sizeof(MM_PhysicalSubArenaRegionBased), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (arena) {
...
uintptr_t MM_PhysicalSubArenaRegionBased::doContractInSubSpace(
    MM_EnvironmentBase* env, uintptr_t contractSize, MM_MemorySubSpace* subspace)
{
    ...
    while (didContractBy < contractSize) {
        ...
        subspace->heapRemoveRange(
            env, _subSpace, regionSize, contractBase, contractTop, lowValidAddress, highValidAddress);
...
uintptr_t MM_PhysicalSubArenaRegionBased::doExpandInSubSpace(
    MM_EnvironmentBase* env, uintptr_t expandSize, MM_MemorySubSpace* subspace)
{
    ...
    /*
     due to heap address for each numa node has to alignment with region size and page size, free region count might be
     not distributed evenly among numa nodes. use numaNodeCountWithFreeRegions and areFreeRegionsForNode() to inflate
     all available regions from numa nodes round robin until reaching expanded size.
     */
...
uintptr_t MM_PhysicalSubArenaRegionBased::performExpand(MM_EnvironmentBase* env, uintptr_t expandSize)
{
    uintptr_t result = 0;
...
uintptr_t MM_PhysicalSubArenaRegionBased::expand(MM_EnvironmentBase* env, uintptr_t expandSize)
{
    return performExpand(env, expandSize);
}

uintptr_t MM_PhysicalSubArenaRegionBased::contract(MM_EnvironmentBase* env, uintptr_t expandSize)
{
    if (((MM_PhysicalArenaRegionBased*)_parent)->canResize(env, this, expandSize)) {
...
{
    /* region based heap is discontiguous so we don't have the restriction that we must contract at the end of the
     * subspace */
    return UDATA_MAX;
}
...
void MM_PhysicalSubArenaRegionBased::validateNumaSymmetry(MM_EnvironmentBase* env)
{
