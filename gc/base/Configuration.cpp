    ...
    if (_delegate.initialize(env, _writeBarrierType, _allocationType)) {
      ...
      if (initializeNUMAManager(env)) {
        ...
        extensions->_lightweightNonReentrantLockPool =
          pool_new(sizeof(J9ThreadMonitorTracing),
                   0,
                   0,
                   0,
                   OMR_GET_CALLSITE(),
                   OMRMEM_CATEGORY_MM,
                   POOL_FOR_PORT(env->getPortLibrary()));
...
void
MM_Configuration::defaultMemorySpaceAllocated(MM_GCExtensionsBase* extensions,
...
MM_Heap*
MM_Configuration::createHeap(MM_EnvironmentBase* env,
                             uintptr_t heapBytesRequested)
{
  ...
  MM_Heap* heap = createHeapWithManager(
    env, heapBytesRequested, extensions->heapRegionManager);
  if (NULL != heap) {
...
bool
MM_Configuration::initializeRunTimeObjectAlignmentAndCRShift(
                                    ...
                                    MM_InitializationParameters* parameters)
{
  ...
  parameters->_minimumOldSpaceSize = OMR_MIN(
    maximumHeapSize - parameters->_minimumNewSpaceSize, minimumTenureSpaceSize);
  parameters->_initialOldSpaceSize = OMR_MIN(
    maximumHeapSize - parameters->_initialNewSpaceSize, initialTenureSpaceSize);
...
uintptr_t
MM_Configuration::getAlignment(MM_GCExtensionsBase* extensions,
...
{
  return MM_ParallelDispatcher::newInstance(
    env, handler, handler_arg, defaultOSStackSize);
}
