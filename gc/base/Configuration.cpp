...
void MM_Configuration::kill(MM_EnvironmentBase* env) {
  tearDown(env);
  env->getForge()->free(this);
}

bool MM_Configuration::initialize(MM_EnvironmentBase* env) {
  bool result = false;
  ...
  if (initializeRegionSize(env) && initializeArrayletLeafSize(env)) {
    if (_delegate.initialize(env, _writeBarrierType, _allocationType)) {
      ...
      if (initializeNUMAManager(env)) {
        ...
        extensions->_lightweightNonReentrantLockPool = pool_new(
            sizeof(J9ThreadMonitorTracing), 0, 0, 0, OMR_GET_CALLSITE(),
...
void MM_Configuration::tearDown(MM_EnvironmentBase* env) {
  MM_GCExtensionsBase* extensions = env->getExtensions();

  /* DefaultMemorySpace needs to be killed before
   * ext->heap is freed in MM_Configuration::tearDown. */
  if (NULL != extensions->heap) {
    MM_MemorySpace* modronMemorySpace =
        extensions->heap->getDefaultMemorySpace();
  ...
  /* referenceChainWalkerMarkMap must be destroyed before Memory Manager is
   * killed */
  ...
  /*
   * Clear out the NUMA Manager, too, since we were the first ones to tell it to
   * cache data Do it last, because some collectors allocate/free resources
   * based on number of NUMA nodes.
   */
 ...
 */
MM_EnvironmentBase* MM_Configuration::createEnvironment(
 ...
 */
bool MM_Configuration::initializeEnvironment(MM_EnvironmentBase* env) {
  bool result = false;
  ...
  switch (_allocationType) {
    ...
    case gc_modron_allocation_type_tlh:
      env->_objectAllocationInterface =
          MM_TLHAllocationInterface::newInstance(env);
      break;
#endif /* defined(OMR_GC_THREAD_LOCAL_HEAP) */
#if defined(OMR_GC_SEGREGATED_HEAP)
    case gc_modron_allocation_type_segregated:
      env->_objectAllocationInterface =
          MM_SegregatedAllocationInterface::newInstance(env);
      break;
#endif /* defined(OMR_GC_SEGREGATED_HEAP) */
    default:
...
void MM_Configuration::defaultMemorySpaceAllocated(
    MM_GCExtensionsBase* extensions, void* defaultMemorySpace) {
  /* do nothing */
}

MM_Heap* MM_Configuration::createHeap(MM_EnvironmentBase* env,
  ...
  MM_Heap* heap = createHeapWithManager(env, heapBytesRequested,
                                        extensions->heapRegionManager);
  if (NULL != heap) {
    if (!heap->initializeHeapRegionManager(env,
                                           extensions->heapRegionManager)) {
      heap->kill(env);
      heap = NULL;
    }
    ...
    /* VM Design 1869: kill the heap if it was allocated but not in the area
     * requested by the fvtest options and then let it fall through to the
...
bool MM_Configuration::initializeRunTimeObjectAlignmentAndCRShift(
  ...
  if (env->compressObjectReferences()) {
    ...
    UDATA shift = (extensions->shouldAllowShiftingCompression)
                      ? LOW_MEMORY_HEAP_CEILING_SHIFT
                      : 0;
    ...
    if (heapTop <= (maxAddressValue << shift)) {
      ...
      if (canChangeShift) {
        ...
        while ((heapTop <= (maxAddressValue << underShift)) &&
               (underShift >= 0)) {
          underShift -= 1;
        }
      ...
      /* impossible geometry:  use an assert for now but just return false once
       * we are done testing the shifting */
      Assert_MM_unreachable();
      return false;
    }
#if !defined(S390) && !defined(J9ZOS390)
    /* s390 benefits from smaller shift values but other platforms don't so just
     * force the shift to 3 if it was not 0 to save on testing resources. (still
     * honour the "canChangeShift" flag, though, since we want our testing
     * options to still work)
     */
    if ((canChangeShift) && (0 != shift) &&
...
void MM_Configuration::prepareParameters(
    OMR_VM* omrVM, uintptr_t minimumSpaceSize, uintptr_t minimumNewSpaceSize,
    uintptr_t initialNewSpaceSize, uintptr_t maximumNewSpaceSize,
    uintptr_t minimumTenureSpaceSize, uintptr_t initialTenureSpaceSize,
    uintptr_t maximumTenureSpaceSize, uintptr_t memoryMax,
    uintptr_t tenureFlags, MM_InitializationParameters* parameters) {
  ...
  uintptr_t maximumHeapSize =
      MM_Math::roundToFloor(alignment, heap->getMaximumMemorySize());

  minimumNewSpaceSize =
      MM_Math::roundToCeiling(alignment * 2, minimumNewSpaceSize);
  minimumTenureSpaceSize =
      MM_Math::roundToCeiling(alignment, minimumTenureSpaceSize);

  maximumNewSpaceSize =
      MM_Math::roundToCeiling(alignment * 2, maximumNewSpaceSize);
  maximumTenureSpaceSize =
      MM_Math::roundToCeiling(alignment, maximumTenureSpaceSize);

  minimumSpaceSize =
      OMR_MAX(MM_Math::roundToCeiling(alignment, minimumSpaceSize),
              minimumNewSpaceSize + minimumTenureSpaceSize);
  memoryMax = OMR_MAX(MM_Math::roundToCeiling(alignment, memoryMax),
                      maximumTenureSpaceSize + maximumNewSpaceSize);

  maximumHeapSize = OMR_MIN(maximumHeapSize, memoryMax);

  initialNewSpaceSize =
      MM_Math::roundToCeiling(alignment * 2, initialNewSpaceSize);
  initialTenureSpaceSize =
      MM_Math::roundToCeiling(alignment, initialTenureSpaceSize);
  ...
  parameters->_minimumNewSpaceSize =
      OMR_MIN(maximumHeapSize, minimumNewSpaceSize);
  parameters->_initialNewSpaceSize =
      OMR_MIN(maximumHeapSize, initialNewSpaceSize);
  parameters->_maximumNewSpaceSize =
      OMR_MIN(maximumHeapSize, maximumNewSpaceSize);

  parameters->_minimumOldSpaceSize =
      OMR_MIN(maximumHeapSize - parameters->_minimumNewSpaceSize,
              minimumTenureSpaceSize);
  parameters->_initialOldSpaceSize =
      OMR_MIN(maximumHeapSize - parameters->_initialNewSpaceSize,
              initialTenureSpaceSize);
  parameters->_maximumOldSpaceSize =
      OMR_MIN(maximumHeapSize, maximumTenureSpaceSize);
  parameters->_maximumSpaceSize = maximumHeapSize;
}

uintptr_t MM_Configuration::getAlignment(MM_GCExtensionsBase* extensions,
                                         MM_AlignmentType type) {
  uintptr_t result = 0;
...
bool MM_Configuration::initializeRegionSize(MM_EnvironmentBase* env) {
  bool result = true;
  MM_GCExtensionsBase* extensions = env->getExtensions();
...
bool MM_Configuration::initializeArrayletLeafSize(MM_EnvironmentBase* env) {
  bool result = true;
  OMR_VM* omrVM = env->getOmrVM();
  if (UDATA_MAX != _defaultArrayletLeafSize) {
    uintptr_t arrayletLeafSize = (0 != _defaultArrayletLeafSize)
                                     ? _defaultArrayletLeafSize
...
void MM_Configuration::initializeGCThreadCount(MM_EnvironmentBase* env) {
  MM_GCExtensionsBase* extensions = env->getExtensions();
...
void MM_Configuration::initializeGCParameters(MM_EnvironmentBase* env) {
  MM_GCExtensionsBase* extensions = env->getExtensions();

  /* TODO 108399: May need to adjust -Xmn*, -Xmo* values here if not fully
   * specified on startup options */
  ...
  if (0 == extensions->splitFreeListSplitAmount) {
    ...
    if (extensions->scavengerEnabled) {
      extensions->splitFreeListSplitAmount =
          (extensions->gcThreadCount - 1) / 8 + 1;
    } else
...
bool MM_Configuration::initializeNUMAManager(MM_EnvironmentBase* env) {
  return env->getExtensions()->_numaManager.recacheNUMASupport(env);
}

MM_Dispatcher* MM_Configuration::createDispatcher(
    MM_EnvironmentBase* env, omrsig_handler_fn handler, void* handler_arg,
    uintptr_t defaultOSStackSize) {
  return MM_ParallelDispatcher::newInstance(env, handler, handler_arg,
                                            defaultOSStackSize);
}
