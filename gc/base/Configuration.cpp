    ...
    if (initializeRegionSize(env) && initializeArrayletLeafSize(env)) {
        if (_delegate.initialize(env, _writeBarrierType, _allocationType)) {
            ...
            if (initializeNUMAManager(env)) {
                ...
                extensions->_lightweightNonReentrantLockPool = pool_new(sizeof(J9ThreadMonitorTracing), 0, 0, 0,
                    OMR_GET_CALLSITE(), OMRMEM_CATEGORY_MM, POOL_FOR_PORT(env->getPortLibrary()));
 ...
 */
MM_EnvironmentBase* MM_Configuration::createEnvironment(MM_GCExtensionsBase* extensions, OMR_VMThread* omrVMThread)
{
...
void MM_Configuration::defaultMemorySpaceAllocated(MM_GCExtensionsBase* extensions, void* defaultMemorySpace)
{
    /* do nothing */
}

MM_Heap* MM_Configuration::createHeap(MM_EnvironmentBase* env, uintptr_t heapBytesRequested)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    ...
    if (NULL != heap) {
        ...
        /* VM Design 1869: kill the heap if it was allocated but not in the area requested by the fvtest options and
         * then let it fall through to the normal error handling */
...
bool MM_Configuration::initializeRunTimeObjectAlignmentAndCRShift(MM_EnvironmentBase* env, MM_Heap* heap)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    ...
    if (env->compressObjectReferences()) {
        ...
        /* s390 benefits from smaller shift values but other platforms don't so just force the shift to 3 if it was not
         * 0 to save on testing resources. (still honour the "canChangeShift" flag, though, since we want our testing
...
void MM_Configuration::prepareParameters(OMR_VM* omrVM, uintptr_t minimumSpaceSize, uintptr_t minimumNewSpaceSize,
    uintptr_t initialNewSpaceSize, uintptr_t maximumNewSpaceSize, uintptr_t minimumTenureSpaceSize,
    uintptr_t initialTenureSpaceSize, uintptr_t maximumTenureSpaceSize, uintptr_t memoryMax, uintptr_t tenureFlags,
    MM_InitializationParameters* parameters)
{
    ...
    minimumSpaceSize
        = OMR_MAX(MM_Math::roundToCeiling(alignment, minimumSpaceSize), minimumNewSpaceSize + minimumTenureSpaceSize);
...
uintptr_t MM_Configuration::getAlignment(MM_GCExtensionsBase* extensions, MM_AlignmentType type)
{
    uintptr_t result = 0;
...
bool MM_Configuration::initializeArrayletLeafSize(MM_EnvironmentBase* env)
{
    ...
    if (UDATA_MAX != _defaultArrayletLeafSize) {
        uintptr_t arrayletLeafSize
            = (0 != _defaultArrayletLeafSize) ? _defaultArrayletLeafSize : env->getExtensions()->regionSize;
