...
MM_MasterGCThread::MM_MasterGCThread(MM_EnvironmentBase *env)
    : MM_BaseNonVirtual(), _collectorControlMutex(NULL),
      _masterThreadState(STATE_ERROR), _masterGCThread(NULL),
      _incomingCycleState(NULL), _allocDesc(NULL),
      _extensions(env->getExtensions()), _collector(NULL),
      _runAsImplicit(false), _acquireVMAccessDuringConcurrent(false),
      _concurrentResumable(false) {
  _typeId = __FUNCTION__;
}

uintptr_t MM_MasterGCThread::master_thread_proc2(OMRPortLibrary *portLib,
                                                 void *info) {
  MM_MasterGCThread *masterGCThread = (MM_MasterGCThread *)info;
  /* jump into the master thread procedure and wait for work.  This method will
   * NOT return */
...
int J9THREAD_PROC MM_MasterGCThread::master_thread_proc(void *info) {
  MM_MasterGCThread *masterGCThread = (MM_MasterGCThread *)info;
  ...
  uintptr_t rc = 0;
  omrsig_protect(
      master_thread_proc2, info,
      ((MM_ParallelDispatcher *)extensions->dispatcher)->getSignalHandler(),
      omrVM,
...
bool MM_MasterGCThread::initialize(MM_Collector *collector, bool runAsImplicit,
                                   bool acquireVMAccessDuringConcurrent,
                                   bool concurrentResumable) {
  bool success = true;
  if (omrthread_monitor_init_with_name(
          &_collectorControlMutex, 0,
          "MM_MasterGCThread::_collectorControlMutex")) {
    success = false;
  }
...
void MM_MasterGCThread::tearDown(MM_EnvironmentBase *env) {
...
bool MM_MasterGCThread::startup() {
  /* set the success flag to false and we will set it true if everything
   * succeeds */
  bool success = false;
  ...
  if (_extensions->fvtest_disableExplictMasterThread) {
    ...
    /* hold the monitor over start-up of this thread so that we eliminate any
     * timing hole where it might notify us of its start-up state before we wait
     */
    ...
    intptr_t forkResult = createThreadWithCategory(
        NULL, OMR_OS_STACK_SIZE, J9THREAD_PRIORITY_NORMAL, 0,
        master_thread_proc, this, J9THREAD_CATEGORY_SYSTEM_GC_THREAD);
    if (forkResult == 0) {
      ...
      if (STATE_ERROR != _masterThreadState) {
        /* the master thread managed to start up and is in the waiting state,
         * ready for GC requests */
        success = true;
      }
    } else {
...
void MM_MasterGCThread::shutdown() {
  Assert_MM_true(NULL != _collectorControlMutex);
  if ((STATE_ERROR != _masterThreadState) &&
      (STATE_DISABLED != _masterThreadState)) {
    ...
    /* don't NULL _collector as RAS could still trigger a collection after we've
     * started shutting down */
  }
}

void MM_MasterGCThread::handleSTW(MM_EnvironmentBase *env) {
  Assert_MM_true(NULL != _incomingCycleState);
  env->_cycleState = _incomingCycleState;

  /* this thread effectively inherits exclusive access from the mutator thread
   * -- set its state to indicate this */
...
bool MM_MasterGCThread::handleConcurrent(MM_EnvironmentBase *env) {
  bool workDone = false;
  ...
  do {
    ...
    if (_collector->isConcurrentWorkAvailable(env)) {
      MM_ConcurrentPhaseStatsBase *stats =
          _collector->getConcurrentPhaseStats();
      stats->clear();
      ...
      uintptr_t bytesConcurrentlyScanned =
          _collector->masterThreadConcurrentCollect(env);

      if (!_acquireVMAccessDuringConcurrent) {
        omrthread_monitor_enter(_collectorControlMutex);
      }

      _collector->postConcurrentUpdateStatsAndReport(env, stats,
                                                     bytesConcurrentlyScanned);
      workDone = true;
    }
...
void MM_MasterGCThread::masterThreadEntryPoint() {
  ...
  omrVMThread = MM_EnvironmentBase::attachVMThread(
      _extensions->getOmrVM(), "Dedicated GC Master",
      MM_EnvironmentBase::ATTACH_GC_MASTER_THREAD);
  if (NULL == omrVMThread) {
    /* we failed to attach so notify the creating thread that we should fail to
     * start up */
    omrthread_monitor_enter(_collectorControlMutex);
    _masterThreadState = STATE_ERROR;
    omrthread_monitor_notify(_collectorControlMutex);
    ...
    /* attachVMThread could allocate an execute a barrier (since it that point,
     * this thread acted as a mutator thread. Flush GC chaches (like barrier
     * buffers) before turning into the master thread */
    env->flushGCCaches();
    ...
    /* notify the other side that we are active so that they can continue
     * running */
    _masterThreadState = STATE_TERMINATED;
    _masterGCThread = NULL;
    omrthread_monitor_notify(_collectorControlMutex);
    MM_EnvironmentBase::detachVMThread(
        _extensions->getOmrVM(), omrVMThread,
        MM_EnvironmentBase::ATTACH_GC_MASTER_THREAD);
    omrthread_exit(_collectorControlMutex);
  }
}

bool MM_MasterGCThread::garbageCollect(
  ...
  if (NULL != _collector) {
    ...
    /* once the master thread has stored itself in the _masterGCThread, it
     * should never need to collect - this would hang */
    Assert_MM_true(omrthread_self() != _masterGCThread);
    if (_runAsImplicit || (NULL == _masterGCThread)) {
      /* We might not have _masterGCThread in the startup phase or late in the
       * shutdown phase. For example, there may be a native out-of-memory during
       * startup or RAS may trigger a GC after we've shutdown the master thread.
       */
       ...
       */
      omrthread_monitor_enter(_collectorControlMutex);
      /* The variable assignments below are safe because we hold Xaccess.
       * Otherwise, it is possible (based on the wait/notify mechanism here)
       * that another thread could come in under this mutex and stomp on the
       * "parameters" while another thread is waiting.
       */
      ...
      /* The master thread will claim exclusive VM access. Artificially give it
       * up in this thread so that tools like -Xcheck:vm continue to work. */
