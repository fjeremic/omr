...
MM_MasterGCThread::handleConcurrent(MM_EnvironmentBase* env)
{
  ...
  do {
    ...
    if (_collector->isConcurrentWorkAvailable(env)) {
      ...
      _collector->postConcurrentUpdateStatsAndReport(
        env, stats, bytesConcurrentlyScanned);
      workDone = true;
    }
...
bool
MM_MasterGCThread::garbageCollect(MM_EnvironmentBase* env,
