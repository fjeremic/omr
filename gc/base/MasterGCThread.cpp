...
uintptr_t MM_MasterGCThread::master_thread_proc2(OMRPortLibrary* portLib, void* info)
{
...
int J9THREAD_PROC MM_MasterGCThread::master_thread_proc(void* info)
{
    ...
    uintptr_t rc = 0;
    omrsig_protect(master_thread_proc2, info, ((MM_ParallelDispatcher*)extensions->dispatcher)->getSignalHandler(),
        omrVM, OMRPORT_SIG_FLAG_SIGALLSYNC | OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION, &rc);
    return 0;
}

bool MM_MasterGCThread::initialize(
    MM_Collector* collector, bool runAsImplicit, bool acquireVMAccessDuringConcurrent, bool concurrentResumable)
{
    bool success = true;
...
bool MM_MasterGCThread::startup()
{
    ...
    if (_extensions->fvtest_disableExplictMasterThread) {
        ...
        /* hold the monitor over start-up of this thread so that we eliminate any timing hole where it might notify us
         * of its start-up state before we wait */
        omrthread_monitor_enter(_collectorControlMutex);
        _masterThreadState = STATE_STARTING;
        intptr_t forkResult = createThreadWithCategory(NULL, OMR_OS_STACK_SIZE, J9THREAD_PRIORITY_NORMAL, 0,
...
void MM_MasterGCThread::masterThreadEntryPoint()
{
    ...
    omrVMThread = MM_EnvironmentBase::attachVMThread(
        _extensions->getOmrVM(), "Dedicated GC Master", MM_EnvironmentBase::ATTACH_GC_MASTER_THREAD);
    if (NULL == omrVMThread) {
        ...
        /* attachVMThread could allocate an execute a barrier (since it that point, this thread acted as a mutator
         * thread. Flush GC chaches (like barrier buffers) before turning into the master thread */
        env->flushGCCaches();
        ...
        MM_EnvironmentBase::detachVMThread(
            _extensions->getOmrVM(), omrVMThread, MM_EnvironmentBase::ATTACH_GC_MASTER_THREAD);
        omrthread_exit(_collectorControlMutex);
    }
}

bool MM_MasterGCThread::garbageCollect(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
    ...
    if (NULL != _collector) {
        ...
        /* once the master thread has stored itself in the _masterGCThread, it should never need to collect - this would
         * hang */
        ...
        if (_runAsImplicit || (NULL == _masterGCThread)) {
            ...
            omrthread_monitor_enter(_collectorControlMutex);
            /* The variable assignments below are safe because we hold Xaccess.  Otherwise, it is possible (based on the
             * wait/notify mechanism here) that another thread could come in under this mutex and stomp on the
            ...
            /* The master thread will claim exclusive VM access. Artificially give it up in this thread so that tools
             * like -Xcheck:vm continue to work. */
