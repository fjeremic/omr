...
public:
  MMINLINE MM_PhysicalSubArenaVirtualMemory *getNextSubArena() {
    return _highArena;
  }
  MMINLINE void setNextSubArena(MM_PhysicalSubArenaVirtualMemory *subArena) {
    _highArena = subArena;
  }
  MMINLINE MM_PhysicalSubArenaVirtualMemory *getPreviousSubArena() {
    return _lowArena;
  }
  MMINLINE void
  setPreviousSubArena(MM_PhysicalSubArenaVirtualMemory *subArena) {
    _lowArena = subArena;
  }
  ...
  MMINLINE void *getVirtualLowAddress() {
    return _hasVirtualLowAddress ? _virtualLowAddress : _lowAddress;
  }
  MMINLINE void setVirtualLowAddress(void *virtualLowAddress) {
    _hasVirtualLowAddress = true;
    _virtualLowAddress = virtualLowAddress;
  }
  MMINLINE void *getVirtualHighAddress() {
    return _hasVirtualHighAddress ? _virtualHighAddress : _highAddress;
  }
  ...
  MM_PhysicalSubArenaVirtualMemory(MM_Heap *heap)
      : MM_PhysicalSubArena(heap), _lowArena(NULL), _highArena(NULL),
        _lowAddress(NULL), _highAddress(NULL), _hasVirtualLowAddress(false),
        _hasVirtualHighAddress(false), _virtualLowAddress(NULL),
        _virtualHighAddress(NULL), _expandFromHighRange(false),
        _expandFromLowRange(false), _numaNode(0) {
