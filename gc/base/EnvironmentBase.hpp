...
protected:
  ...
  OMRPortLibrary
      *_portLibrary; /**< the port library associated with the environment */
...
private:
  ...
  MM_AllocationContext *_allocationContext;       /**< The "second-level caching
                                                     mechanism" for this thread */
  MM_AllocationContext *_commonAllocationContext; /**< Common Allocation Context
                                                     shared by all threads */

  uint64_t _exclusiveAccessTime; /**< time (in ticks) of the last exclusive
                                    access request */
  ...
  bool _exclusiveAccessBeatenByOtherThread; /**< true if last exclusive access
                                               ...
                                               GC thread */
  uintptr_t
      _exclusiveCount; /**< count of number of times this thread has acquired
                          but not yet released exclusive access */
  OMR_VMThread *
      _cachedGCExclusiveAccessThreadId; /** only to be used when a thread
                                           requests a GC operation while already
...
protected:
  ...
  MM_HeapRegionQueue
      *_regionLocalFull; /* cached full region queue per slave during sweep */
...
public:
  ...
  MM_ObjectAllocationInterface
      *_objectAllocationInterface; /**< Per-thread interface that guides object
  ...
  MM_CycleState *
      _cycleState; /**< The current GC cycle that this thread is operating on */
  bool _isInNoGCAllocationCall; /**< NOTE:  this is a "best-efforts" flag, only,
                                   for use by an assertion in the collector.  If
                                   this is true, the owning thread is attempting
                                   to perform a NoGC allocation.  If it is
                                   false, nothing can be implied about the
                                   owning thread's state. */
  ...
  MM_WorkPacketStats
      _workPacketStatsRSScan; /**< work packet Stats specifically for RS Scan
                                 Phase of Concurrent STW GC */

  uint64_t _slaveThreadCpuTimeNanos; /**< Total CPU time used by this slave
                                        thread (or 0 for non-slaves) */

  MM_FreeEntrySizeClassStats
      _freeEntrySizeClassStats; /**< GC thread local statistics structure for
                                   heap free entry size (sizeClass) distribution
                                 */

  uintptr_t _oolTraceAllocationBytes; /**< Tracks the bytes allocated since the
                                         last ool object trace */

  uintptr_t approxScanCacheCount; /**< Local copy of approximate entries in
                                     global Cache Scan List. Updated upon
                                     allocation of new cache. */

  MM_Validator *_activeValidator; /**< Used to identify and report crashes
                                     inside Validators */

  MM_MarkStats _markStats;

  MM_RootScannerStats _rootScannerStats; /**< Per thread stats to track the
                                            performance of the root scanner */

  const char
      *_lastSyncPointReached; /**< string indicating latest sync point reached
                                 by this associated env's thread */

#if defined(OMR_GC_SEGREGATED_HEAP)
  MM_SegregatedAllocationTracker
      *_allocationTracker; /**< tracks bytes allocated per thread and
                              periodically flushes allocation data to
                              MM_MemoryPoolSegregated */
#endif                     /* OMR_GC_SEGREGATED_HEAP */

  volatile uint32_t _allocationColor; /**< Flag field to indicate whether
                                         premarking is enabled on the thread */

  MM_CardCleaningStats
      _cardCleaningStats; /**< Per thread stats to track the performance of the
                             card cleaning */
  ...
  uint64_t _concurrentScavengerSwitchCount; /**< local counter of cycle start
...
protected:
  ...
  MMINLINE void setEnvironmentId(uintptr_t environmentId) {
...
public:
  static MM_EnvironmentBase *newInstance(MM_GCExtensionsBase *extensions,
                                         OMR_VMThread *vmThread);

  static OMR_VMThread *
  attachVMThread(OMR_VM *omrVM, const char *threadName,
                 AttachVMThreadReason reason = ATTACH_THREAD) {
    return MM_EnvironmentDelegate::attachVMThread(omrVM, threadName, reason);
  }

  static void detachVMThread(OMR_VM *omrVM, OMR_VMThread *omrThread,
                             AttachVMThreadReason reason = ATTACH_THREAD) {
   ...
   */
  MMINLINE static MM_EnvironmentBase *
  getEnvironment(OMR_VMThread *omrVMThread) {
   ...
   */
  MMINLINE MM_GCExtensionsBase *getExtensions() {
    return (MM_GCExtensionsBase *)_omrVM->_gcOmrVMExtensions;
  }

  MMINLINE MM_MemorySpace *getMemorySpace() {
    return (MM_MemorySpace *)(_omrVMThread->memorySpace);
  }
   ...
   */
  MMINLINE uintptr_t getPriority() {
    return omrthread_get_priority(_omrVMThread->_os_thread);
  }

  /**
   * Set the the thread's priority.
   * @param priority The priority to set the thread to.
   * @returns 0 on success or negative value on failure (priority wasn't
   * changed)
   */
  ...
  /**
   * @deprecated  This function needs to be replaced with one which can describe
   * a set of nodes Determine what the NUMA node affinity is for the thread.
   *
   * @return the index of the node to associate with, where 1 is the first node.
   * (0 indicates no affinity)
   */
  MMINLINE uintptr_t getNumaAffinity() {
    uintptr_t result = 0;
    uintptr_t nodeCount = 1;
    if ((0 != omrthread_numa_get_node_affinity(_omrVMThread->_os_thread,
                                               &result, &nodeCount)) ||
        (0 == nodeCount)) {
      result = 0;
    }
  ...
  /**
   * @deprecated  This function needs to be replaced with one which can describe
   * a set of nodes Set the affinity for the thread so that it runs only (or
   * preferentially) on processors associated with the specified NUMA node.
   *
   * @param numaNodes[in] The array of node numbers to associate with, where 1
   * is the first node. (0 indicates no affinity)
   * @param arrayLength[in] The number of elements in numaNodes (must be at
   * least 1)
   *
   * @return true on success, false on failure
   */
  MMINLINE bool setNumaAffinity(uintptr_t *numaNodes, uintptr_t arrayLength) {
    return 0 == omrthread_numa_set_node_affinity(_omrVMThread->_os_thread,
                                                 numaNodes, arrayLength, 0);
  }
  ...
  /**
   * This will be called for every allocated object.  Note this is not
   * necessarily done when the object is allocated, but will done before start
   * of the next gc for all objects allocated since the last gc.
   */
  bool objectAllocationNotify(omrobjectptr_t omrObject) {
    return _delegate.objectAllocationNotify(omrObject);
  }
   ...
   */
  void allocationFailureStartReportIfRequired(
      MM_AllocateDescription *allocDescription, uintptr_t flags);
   ...
   */
  void allocationFailureEndReportIfRequired(
   ...
   * The calling thread will acquire exclusive access for Gc regardless if other
   * threads beat it to exclusive for the same purposes.
   * @param collector gc intended to be used for collection.
   * @return boolean indicating whether the thread cleanly won exclusive access
   * for its collector or if it had been beaten already.
   *
   * @note this call should be considered a safe-point as the thread may release
   * VM access to allow the other threads to acquire exclusivity.
   * @note this call supports recursion.
   */
  bool acquireExclusiveVMAccessForGC(MM_Collector *collector,
                                     bool failIfNotFirst = false,
                                     bool flushCaches = true);

  /**
   * Release exclusive access.
   * The calling thread will release one level (recursion) of its exclusive
   * access request, and alert other threads that it has completed its intended
   * GC request if this is the last level.
   */
  void releaseExclusiveVMAccessForGC();

  /**
   * Release all recursive levels of exclusive access.
   * The calling thread will release all levels (recursion) of its exclusive
   * access request, and alert other threads that it has completed its intended
   * GC request.
   */
   ...
   * collaborating thread (i.e. main-to-master or master-to-main)
  ...
  uint64_t getMeanExclusiveAccessIdleTime() {
    return _meanExclusiveAccessIdleTime;
  };
  ...
  OMR_VMThread *getLastExclusiveAccessResponder() {
    return _lastExclusiveAccessResponder;
  };

  /**
   * Get the number of threads which were halted for the exclusive access
   * request.
   * @return The number of halted threads
   */
  uintptr_t getExclusiveAccessHaltedThreads() {
    return _exclusiveAccessHaltedThreads;
  };
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  /**
   * Force thread to use out-of-line request for VM access. This may be required
   * if there is there is an event waiting to be hooked the next time the thread
   * acquires VM access.
   */
  ...
  /**
   * Disable inline TLH allocates by hiding the real heap allocation address
   * from JIT/Interpreter in realHeapAlloc and setting heapALloc == HeapTop so
   * TLH looks full.
   *
   */
  ...
  /**
   * Determine if inline TLH allocate is enabled; its enabled if realheapAlloc
   * is NULL.
   * @return TRUE if inline TLH allocates currently enabled for this thread;
   * FALSE otherwise
   */
  bool isInlineTLHAllocateEnabled() {
    return _delegate.isInlineTLHAllocateEnabled();
  }
  ...
  MMINLINE void setWorkUnitToHandle(uintptr_t workUnitToHandle) {
    _workUnitToHandle = workUnitToHandle;
  }
  ...
  MMINLINE void setThreadScanned(bool threadScanned) {
    _threadScanned = threadScanned;
  };
   ...
   */
  MMINLINE MM_AllocationContext *getAllocationContext() const {
   ...
   */
  MMINLINE MM_AllocationContext *getCommonAllocationContext() const {
    return _commonAllocationContext;
  }
   ...
   */
  MMINLINE void
  setCommonAllocationContext(MM_AllocationContext *commonAllocationContext) {
    _commonAllocationContext = commonAllocationContext;
  }

  MMINLINE uint32_t getAllocationColor() const { return _allocationColor; }
  MMINLINE void setAllocationColor(uint32_t allocationColor) {
  ...
  virtual void flushNonAllocationCaches() {
    _delegate.flushNonAllocationCaches();
  }
  virtual void flushGCCaches() {}

  /**
   * Get a pointer to common GC metadata attached to this environment. The GC
   * environment structure is defined by the client language and bound to the
   * environment delegate attached to this class. It is typically used to hold
   * thread-local GC-related stats and caches that are merged at key points into
   * global structures.
   */
  MMINLINE GC_Environment *getGCEnvironment() {
    return _delegate.getGCEnvironment();
  }

#if defined(OMR_GC_SEGREGATED_HEAP)
  MMINLINE MM_HeapRegionQueue *getRegionWorkList() const {
    return _regionWorkList;
  }
  MMINLINE MM_HeapRegionQueue *getRegionLocalFree() const {
    return _regionLocalFree;
  }
  MMINLINE MM_HeapRegionQueue *getRegionLocalFull() const {
    return _regionLocalFull;
  }
#endif /* OMR_GC_SEGREGATED_HEAP */
  ...
  MM_EnvironmentBase(OMR_VMThread *omrVMThread)
      : MM_BaseVirtual()
        ...
        _compressObjectReferences(
            OMRVMTHREAD_COMPRESS_OBJECT_REFERENCES(omrVMThread))
#endif /* defined(OMR_GC_COMPRESSED_POINTERS) && defined(OMR_GC_FULL_POINTERS) \
        */
        ,
        _slaveID(0), _environmentId(0), _omrVM(omrVMThread->_vm),
        _omrVMThread(omrVMThread),
        _portLibrary(omrVMThread->_vm->_runtime->_portLibrary), _delegate(),
        _workUnitIndex(0), _workUnitToHandle(0), _threadScanned(false),
        _allocationContext(NULL), _commonAllocationContext(NULL),
        _exclusiveAccessTime(0), _meanExclusiveAccessIdleTime(0),
        _lastExclusiveAccessResponder(NULL), _exclusiveAccessHaltedThreads(0),
        _exclusiveAccessBeatenByOtherThread(false), _exclusiveCount(0),
        _cachedGCExclusiveAccessThreadId(NULL),
        _allocationFailureReported(false)
#if defined(OMR_GC_SEGREGATED_HEAP)
        ,
        _regionWorkList(NULL), _regionLocalFree(NULL), _regionLocalFull(NULL)
#endif /* OMR_GC_SEGREGATED_HEAP */
        ,
        _objectAllocationInterface(NULL), _workStack(),
        _threadType(MUTATOR_THREAD), _cycleState(NULL),
        _isInNoGCAllocationCall(false), _failAllocOnExcessiveGC(false),
        _currentTask(NULL), _slaveThreadCpuTimeNanos(0),
        _freeEntrySizeClassStats(), _oolTraceAllocationBytes(0),
        approxScanCacheCount(0), _activeValidator(NULL),
  ...
  MM_EnvironmentBase(OMR_VM *omrVM)
      : MM_BaseVirtual()
        ...
        */
        ,
        _slaveID(0), _environmentId(0), _omrVM(omrVM), _omrVMThread(NULL),
        _portLibrary(omrVM->_runtime->_portLibrary), _workUnitIndex(0),
        _workUnitToHandle(0), _threadScanned(false), _allocationContext(NULL),
        _commonAllocationContext(NULL), _exclusiveAccessTime(0),
        _meanExclusiveAccessIdleTime(0), _lastExclusiveAccessResponder(NULL),
