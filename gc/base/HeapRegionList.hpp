 ...
 *
 * Regions on a RegionList can either represent just themselves or can be the
 * cannonical Region for a range of Regions.  A particular RegionList instance
 * will only contain one kind of regions (_singleRegionsOnly true/false
 * respectively).
 *
 * We have two main implementations of RegionList that add additional
 * operations. RegionQueue supports queue operations (FIFO insertion/deletion
 * via enqueue/dequeue). FreeRegionList supports stack operations (LIFO
 * insertion/deletion via push/pop) plus a general removeAny capability to
 * support coalesce.
 *
 * All Regions on a RegionList must be contained in the RegionTable associated
 * with the RegionList.
 */
...
public:
   ...
   *   The valid transitions between region lists are defined by
   * isValidTransition.
   */
  typedef enum {
...
protected:
  uintptr_t _length;
  /**< What kind of region list is this? @see ListKind for description of
   * RegionList kinds */
  RegionListKind _regionListKind;
  /**< Do regions on the list represent only themselves, or do they encode
   * region ranges (Large & MultiFree) */
...
public:
  ...
  /**
   * @param pt The RegionList that contains all regions that will be put on this
   * RegionList.
   * @param regionListKind  The RegionListKind of this RegionList.
   * @param singleRegionsOnly True if this heapRgionlist is for small/arraylet,
   * false if it is for large (multi-region)
   */
  MM_HeapRegionList(RegionListKind regionListKind, bool singleRegionsOnly)
      : _length(0), _regionListKind(regionListKind),
  ...
  bool isFreeList() {
    return _regionListKind == HRL_KIND_FREE ||
           _regionListKind == HRL_KIND_MULTI_FREE;
  }
  ...
  static bool isValidTransition(RegionListKind from, RegionListKind to) {
    if (HRL_KIND_LOCAL_WORK == from || HRL_KIND_LOCAL_WORK == to) {
      return true; // SIGH. Local work blinds us because we need 3 states to
                   // really check it.
    }

    switch (from) {
    case HRL_KIND_MULTI_FREE:
      return HRL_KIND_FREE == to || HRL_KIND_FULL == to ||
             HRL_KIND_COALESCE == to;
    case HRL_KIND_FREE:
      return HRL_KIND_AVAILABLE == to || HRL_KIND_FULL == to ||
             HRL_KIND_COALESCE == to;
    case HRL_KIND_AVAILABLE:
    ...
    case HRL_KIND_SWEEP:
      return HRL_KIND_FREE == to || HRL_KIND_AVAILABLE == to ||
             HRL_KIND_FULL == to;
    case HRL_KIND_COALESCE:
      /* NOTE: COALESCE ==> COALESCE is only allowable because coalesce list is
       * still using locks */
      return HRL_KIND_FREE == to || HRL_KIND_MULTI_FREE == to ||
             HRL_KIND_FULL == to || HRL_KIND_COALESCE == to;
    default:
      return false;
    }
  }
