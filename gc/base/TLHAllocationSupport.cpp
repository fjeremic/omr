 ...
 */
void MM_TLHAllocationSupport::reportClearCache(MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  MM_MemorySubSpace *subspace =
      env->getMemorySpace()->getDefaultMemorySubSpace();

  TRIGGER_J9HOOK_MM_PRIVATE_CACHE_CLEARED(extensions->privateHookInterface,
                                          _omrVMThread, subspace, getBase(),
                                          getAlloc(), getTop());
}
 ...
 */
void MM_TLHAllocationSupport::reportRefreshCache(MM_EnvironmentBase *env) {
  MM_MemorySubSpace *subspace =
      env->getMemorySpace()->getDefaultMemorySubSpace();

  TRIGGER_J9HOOK_MM_PRIVATE_CACHE_REFRESHED(
      env->getExtensions()->privateHookInterface, _omrVMThread, subspace,
      getBase(), getTop());
}

/**
 * Purge the TLH data from the receiver.
 * Remove any ownership of a heap area from the receivers TLH, and make sure the
 * heap is left in a safe, or walkable state.
 *
 * @note The calling environment may not be the receivers owning environment.
 */
void MM_TLHAllocationSupport::clear(MM_EnvironmentBase *env) {
 ...
 * The environment is either newly created or has had its properties changed
 * such that the existing cache is no longer valid.  An example of this is a
 * change of memory space.  Perform the necessary flushing and re-initialization
 * of the cache details such that new allocations will occur in the correct
 * fashion.
 *
 * @param shouldFlush determines whether the existing cache information should
 * be flushed back to the heap.
 */
void MM_TLHAllocationSupport::reconnect(MM_EnvironmentBase *env,
                                        bool shouldFlush) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
 ...
 * Reset the cache details back to a starting state that is appropriate for
 * where it currently is. In this case, the state reset takes into account the
 * ending refresh size and sets an appropriate new starting point.
 *
 * @note The previous cache contents are expected to have been flushed back to
 * the heap.
 */
void MM_TLHAllocationSupport::restart(MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  ...
  _tlh->refreshSize =
      MM_Math::roundToCeiling(extensions->tlhInitialSize, refreshSize / 2);
}

/**
 * Refresh the TLH.
 */
bool MM_TLHAllocationSupport::refresh(MM_EnvironmentBase *env,
  ...
  /* Refresh the TLH only if the allocation request will fit in half the refresh
   * size or in the TLH minimum size.
   */
  ...
  uintptr_t abandonSize =
      (tlhMinimumSize > halfRefreshSize ? tlhMinimumSize : halfRefreshSize);
  if (sizeInBytesRequired > abandonSize) {
    /* increase thread hungriness if we did not refresh */
    if (getRefreshSize() < tlhMaximumSize &&
        sizeInBytesRequired < tlhMaximumSize) {
  ...
  if (NULL != getRealAlloc() && getSize() >= tlhMinimumSize) {
    ...
    MM_HeapLinkedFreeHeaderTLH *newCache =
        (MM_HeapLinkedFreeHeaderTLH *)getRealAlloc();

#if defined(OMR_VALGRIND_MEMCHECK)
    valgrindMakeMemUndefined((uintptr_t)newCache,
  ...
  if ((NULL != _abandonedList) && (sizeInBytesRequired <= tlhMinimumSize)) {
    ...
    _abandonedList =
        (MM_HeapLinkedFreeHeaderTLH *)_abandonedList->getNext(compressed);
    --_abandonedListSize;
    ...
    allocDescription->setNurseryAllocation(
        getMemorySubSpace()->getTypeFlags() == MEMORY_TYPE_NEW);
    ...
    MM_MemorySpace *memorySpace =
        _objectAllocationInterface->getOwningEnv()->getMemorySpace();

    if (NULL != ac) {
      /* ensure that we are allowed to use the AI in this configuration in the
       * Tarok case */
      /* allocation contexts currently aren't supported with generational
       * schemes */
      Assert_MM_true(memorySpace->getTenureMemorySubSpace() ==
                     memorySpace->getDefaultMemorySubSpace());
      didRefresh = (NULL != ac->allocateTLH(env, allocDescription,
                                            _objectAllocationInterface,
                                            shouldCollectOnFailure));
    } else {
      MM_MemorySubSpace *subspace = memorySpace->getDefaultMemorySubSpace();
      didRefresh =
          (NULL != subspace->allocateTLH(env, allocDescription,
                                         _objectAllocationInterface, NULL, NULL,
                                         shouldCollectOnFailure));
    }
  ...
  if (didRefresh) {
    ...
    if (0 < getSize()) {
      ...
      /* TODO VMDESIGN 1322: adjust the amount consumed by the TLH refresh since
       * a TLH refresh may not give you the size requested */
 ...
 */
void *MM_TLHAllocationSupport::allocateFromTLH(
  ...
  if (sizeInBytesRequired <= getSize()) {
    ...
    allocDescription->setMemorySubSpace(
        (MM_MemorySubSpace *)_tlh->memorySubSpace);
 ...
 * This is a placeholder function for all non-TLH implementing configurations
 * until a further revision of the code finally pushes TLH functionality down to
 * the appropriate level, and not so high that all configurations must recognize
 * it. For this implementation we simply redirect back to the supplied memory
 * pool and if successful, populate the localized TLH and supplied
 * AllocationDescription with the appropriate information.
 * @return true on successful TLH replenishment, false otherwise.
 */
void *MM_TLHAllocationSupport::allocateTLH(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
    MM_MemorySubSpace *memorySubSpace, MM_MemoryPool *memoryPool) {
  void *addrBase, *addrTop;

  if (memoryPool->allocateTLH(env, allocDescription, getRefreshSize(), addrBase,
                              addrTop)) {
...
void MM_TLHAllocationSupport::flushCache(MM_EnvironmentBase *env) {
...
void MM_TLHAllocationSupport::setupTLH(MM_EnvironmentBase *env, void *addrBase,
                                       void *addrTop,
...
void MM_TLHAllocationSupport::updateFrequentObjectsStats(
    MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  MM_FrequentObjectsStats *frequentObjectsStats =
      _objectAllocationInterface->getFrequentObjectsStats();
  ...
  if (NULL != frequentObjectsStats) {
    ...
    GC_ObjectHeapIteratorAddressOrderedList objectHeapIterator(
        extensions, (omrobjectptr_t)getBase(), (omrobjectptr_t)getAlloc(),
        false, false);
...
void MM_TLHAllocationSupport::objectAllocationNotify(MM_EnvironmentBase *env,
                                                     void *heapBase,
                                                     void *heapTop) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  MM_CollectorLanguageInterface *cli = extensions->collectorLanguageInterface;
  GC_ObjectHeapIteratorAddressOrderedList objectHeapIterator(
      extensions, (omrobjectptr_t)heapBase, heapTop, false, false);
  omrobjectptr_t object = NULL;
