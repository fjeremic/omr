...
MM_CardTable::initialize(MM_EnvironmentBase* env, MM_Heap* heap)
{
  ...
  if (memoryManager->createVirtualMemoryForMetadata(env,
...
uintptr_t
MM_CardTable::calculateCardTableSize(MM_EnvironmentBase* env,
                                     uintptr_t heapSize)
{
...
bool
MM_CardTable::commitCardTableMemory(MM_EnvironmentBase* env,
                                    Card* lowCard,
                                    Card* highCard)
{
  bool commited = true;
  ...
  if (extensions->isFvtestForceCardTableCommitFailure()) {
    ...
    MM_MemoryManager* memoryManager = extensions->memoryManager;
    commited =
      memoryManager->commitMemory(&_cardTableMemoryHandle,
                                  (void*)lowCard,
                                  (uintptr_t)highCard - (uintptr_t)lowCard);
    if (!commited) {
      Trc_MM_CardTable_cardTableCommitFailed(env->getLanguageVMThread(),
                                             (void*)lowCard,
                                             (uintptr_t)highCard -
                                               (uintptr_t)lowCard);
    }
...
bool
MM_CardTable::decommitCardTableMemory(MM_EnvironmentBase* env,
                                      Card* lowCard,
                                      Card* highCard,
                                      Card* lowValidCard,
                                      Card* highValidCard)
{
  ...
  if (extensions->isFvtestForceCardTableDecommitFailure()) {
    ...
    if (lowAddress < highAddress) {
      MM_MemoryManager* memoryManager = extensions->memoryManager;
      decommited = memoryManager->decommitMemory(&_cardTableMemoryHandle,
                                                 lowAddress,
                                                 ...
                                                 highAddress);
      if (!decommited) {
        Trc_MM_CardTable_cardTableDecommitFailed(env->getLanguageVMThread(),
                                                 lowAddress,
                                                 ((uintptr_t)highAddress) -
                                                   ((uintptr_t)lowAddress),
                                                 lowAddress,
...
bool
MM_CardTable::canMemoryBeReleased(MM_EnvironmentBase* env,
                                  void* low,
                                  void* high)
{
  bool result = true;
...
bool
MM_CardTable::isDirtyOrValue(MM_EnvironmentBase* env,
                             void* heapAddr,
                             Card cardValue)
{
...
void
MM_CardTable::dirtyCardWithValue(MM_EnvironmentBase* env,
...
void
MM_CardTable::dirtyCardRange(MM_EnvironmentBase* env,
                             void* heapAddrFrom,
                             void* heapAddrTo)
{
...
MMINLINE void
MM_CardTable::cleanRange(MM_EnvironmentBase* env,
...
void
MM_CardTable::cleanCardTable(MM_EnvironmentBase* env,
                             MM_CardCleaner* cardCleaner)
{
...
void
MM_CardTable::cleanCardTableForRange(MM_EnvironmentBase* env,
                                     MM_CardCleaner* cardCleaner,
                                     void* lowAddress,
                                     void* highAddress)
{
...
void
MM_CardTable::cleanCardsInRegion(MM_EnvironmentBase* env,
                                 MM_CardCleaner* cardCleaner,
                                 MM_HeapRegionDescriptor* region)
{
  uintptr_t oldVMState = env->pushVMstate(cardCleaner->getVMStateID());
...
uintptr_t
MM_CardTable::clearCardsInRange(MM_EnvironmentBase* env,
                                void* heapBase,
                                void* heapTop)
{
...
bool
MM_CardTable::commitCardsForRegion(MM_EnvironmentBase* env,
                                   MM_HeapRegionDescriptor* region)
{
...
bool
MM_CardTable::setNumaAffinityCorrespondingToHeapRange(MM_EnvironmentBase* env,
                                                      uintptr_t numaNode,
                                                      ...
                                                      void* topOfHeapRange)
{
  ...
  return memoryManager->setNumaAffinity(&_cardTableMemoryHandle,
                                        numaNode,
                                        (void*)lowAddress,
