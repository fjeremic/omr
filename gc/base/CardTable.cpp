...
bool MM_CardTable::initialize(MM_EnvironmentBase *env, MM_Heap *heap) {
  ...
  if (memoryManager->createVirtualMemoryForMetadata(
          env, &_cardTableMemoryHandle, extensions->heapAlignment,
          cardTableSizeRequired)) {
    _cardTableStart =
        (Card *)(memoryManager->getHeapBase(&_cardTableMemoryHandle));
    /* Initialize _heapbase; we will reset _heapAlloc in
     * heapAddRange()/heapRemoveRange() as heap changes */
    _heapBase = (void *)heap->getHeapBase();
    _heapAlloc = (void *)heap->getHeapTop();
    _cardTableVirtualStart =
        (Card *)((uintptr_t)_cardTableStart -
                 (((uintptr_t)getHeapBase()) >> CARD_SIZE_SHIFT));
    initialized = true;
  }
 ...
 *
 */
void MM_CardTable::tearDown(MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  MM_MemoryManager *memoryManager = extensions->memoryManager;
...
uintptr_t MM_CardTable::calculateCardTableSize(MM_EnvironmentBase *env,
                                               uintptr_t heapSize) {
  return MM_Math::roundToCeiling(
      sizeof(uint32_t),
      (MM_Math::roundToCeiling(CARD_SIZE, heapSize) / CARD_SIZE) *
          sizeof(Card));
}

bool MM_CardTable::commitCardTableMemory(MM_EnvironmentBase *env, Card *lowCard,
                                         Card *highCard) {
  bool commited = true;
  MM_GCExtensionsBase *extensions = env->getExtensions();
  ...
  if (extensions->isFvtestForceCardTableCommitFailure()) {
    ...
    MM_MemoryManager *memoryManager = extensions->memoryManager;
    commited =
        memoryManager->commitMemory(&_cardTableMemoryHandle, (void *)lowCard,
                                    (uintptr_t)highCard - (uintptr_t)lowCard);
    if (!commited) {
      Trc_MM_CardTable_cardTableCommitFailed(
          env->getLanguageVMThread(), (void *)lowCard,
          (uintptr_t)highCard - (uintptr_t)lowCard);
    }
  }
  return commited;
...
bool MM_CardTable::decommitCardTableMemory(MM_EnvironmentBase *env,
                                           Card *lowCard, Card *highCard,
                                           Card *lowValidCard,
                                           Card *highValidCard) {
  ...
  if (extensions->isFvtestForceCardTableDecommitFailure()) {
    ...
    /*
     *  Note: it is possible that after aligning to virtual memory page size
     * highAddress <= lowAddress This just mean that we can not decommit now
     */
    ...
    if (lowAddress < highAddress) {
      ...
      decommited = memoryManager->decommitMemory(
          &_cardTableMemoryHandle, lowAddress,
          ((uintptr_t)highAddress) - ((uintptr_t)lowAddress), lowAddress,
          highAddress);
      if (!decommited) {
        Trc_MM_CardTable_cardTableDecommitFailed(
            env->getLanguageVMThread(), lowAddress,
            ((uintptr_t)highAddress) - ((uintptr_t)lowAddress), lowAddress,
            highAddress);
      }
    }
...
void *MM_CardTable::getLowAddressToRelease(MM_EnvironmentBase *env, void *low) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  MM_MemoryManager *memoryManager = extensions->memoryManager;
  ...
  void *lowAddressPageAligned =
      (void *)MM_Math::roundToFloor(pageSize, (uintptr_t)low);
...
void *MM_CardTable::getHighAddressToRelease(MM_EnvironmentBase *env,
                                            void *high) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  MM_MemoryManager *memoryManager = extensions->memoryManager;
  ...
  void *highAddressPageAligned =
      (void *)MM_Math::roundToCeiling(pageSize, (uintptr_t)high);
...
bool MM_CardTable::canMemoryBeReleased(MM_EnvironmentBase *env, void *low,
                                       void *high) {
  bool result = true;
  MM_GCExtensionsBase *extensions = env->getExtensions();
  ...
  void *regionAligned =
      (void *)MM_Math::roundToFloor(regionSizeCardSize, (uintptr_t)low);

  do {
    void *heapAddress = (void *)cardAddrToHeapAddr(env, (Card *)regionAligned);
    MM_HeapRegionDescriptor *region =
        regionManager->regionDescriptorForAddress(heapAddress);
...
bool MM_CardTable::isDirtyOrValue(MM_EnvironmentBase *env, void *heapAddr,
                                  Card cardValue) {
  Card *cardAddr = heapAddrToCardAddr(env, heapAddr);
  return (CARD_DIRTY == *cardAddr) || (cardValue == *cardAddr);
}

void MM_CardTable::dirtyCard(MM_EnvironmentBase *env,
                             omrobjectptr_t objectRef) {
  dirtyCardWithValue(env, objectRef, CARD_DIRTY);
}

void MM_CardTable::dirtyCardWithValue(MM_EnvironmentBase *env,
                                      omrobjectptr_t objectRef, Card newValue) {
  Assert_MM_true(CARD_CLEAN != newValue);
  Assert_MM_true(CARD_INVALID != newValue);
  /* Check passed reference within the heap */
  if (((uintptr_t *)objectRef >= (uintptr_t *)getHeapBase()) &&
      ((uintptr_t *)objectRef < (uintptr_t *)_heapAlloc)) {
...
void MM_CardTable::dirtyCardRange(MM_EnvironmentBase *env, void *heapAddrFrom,
                                  void *heapAddrTo) {
...
Card *MM_CardTable::heapAddrToCardAddr(MM_EnvironmentBase *env,
                                       void *heapAddr) {
  /* Check passed reference within the heap */
...
void *MM_CardTable::cardAddrToHeapAddr(MM_EnvironmentBase *env,
                                       Card *cardAddr) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  MM_MemoryManager *memoryManager = extensions->memoryManager;
  ...
  Assert_MM_true((void *)cardAddr <=
                 memoryManager->getHeapTop(&_cardTableMemoryHandle));

  return (void *)((uintptr_t)getHeapBase() +
                  (((uintptr_t)cardAddr - (uintptr_t)getCardTableStart())
                   << CARD_SIZE_SHIFT));
}

MMINLINE void MM_CardTable::cleanRange(MM_EnvironmentBase *env,
                                       MM_CardCleaner *cardCleaner, Card *low,
                                       Card *high) {
  Card *thisCard = low;
  Card *endCard = high;
...
void MM_CardTable::cleanCardTable(MM_EnvironmentBase *env,
                                  MM_CardCleaner *cardCleaner) {
  /* clean cards for entire heap range */
  cleanCardTableForRange(env, cardCleaner, getHeapBase(), _heapAlloc);
}

void MM_CardTable::cleanCardTableForRange(MM_EnvironmentBase *env,
                                          MM_CardCleaner *cardCleaner,
  ...
  uintptr_t compleCleaningRanges =
      ((uintptr_t)finalCard - (uintptr_t)lowCard) / cardsInCleaningRange;
  Card *highCard = lowCard + (compleCleaningRanges * cardsInCleaningRange);

  Assert_MM_true(((uintptr_t)finalCard - (uintptr_t)highCard) <
                 cardsInCleaningRange);
...
void MM_CardTable::cleanCardsInRegion(MM_EnvironmentBase *env,
                                      MM_CardCleaner *cardCleaner,
                                      MM_HeapRegionDescriptor *region) {
  uintptr_t oldVMState = env->pushVMstate(cardCleaner->getVMStateID());
...
uintptr_t MM_CardTable::clearCardsInRange(MM_EnvironmentBase *env,
...
void MM_CardTable::kill(MM_EnvironmentBase *env) {
  tearDown(env);
  env->getForge()->free(this);
}

#if defined(OMR_GC_VLHGC)
bool MM_CardTable::commitCardsForRegion(MM_EnvironmentBase *env,
                                        MM_HeapRegionDescriptor *region) {
...
bool MM_CardTable::setNumaAffinityCorrespondingToHeapRange(
    MM_EnvironmentBase *env, uintptr_t numaNode, void *baseOfHeapRange,
    void *topOfHeapRange) {
  MM_GCExtensionsBase *extensions = env->getExtensions();
  MM_MemoryManager *memoryManager = extensions->memoryManager;
  Assert_MM_true(0 != numaNode);
  bool hasPhysicalNUMASupport =
      (0 != MM_GCExtensionsBase::getExtensions(env->getOmrVM())
                ->_numaManager.isPhysicalNUMASupported());
  /* it only makes sense to call this if NUMA support is enabled - this is not
   * made to be called from highly generic code */
  ...
  return memoryManager->setNumaAffinity(&_cardTableMemoryHandle, numaNode,
                                        (void *)lowAddress,
