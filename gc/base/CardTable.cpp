...
bool MM_CardTable::commitCardTableMemory(MM_EnvironmentBase* env, Card* lowCard, Card* highCard)
{
    bool commited = true;
    MM_GCExtensionsBase* extensions = env->getExtensions();
...
bool MM_CardTable::decommitCardTableMemory(MM_EnvironmentBase* env, Card* lowCard, Card* highCard, Card* lowValidCard, Card* highValidCard)
{
...
void* MM_CardTable::getHighAddressToRelease(MM_EnvironmentBase* env, void* high)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    MM_MemoryManager* memoryManager = extensions->memoryManager;
...
bool MM_CardTable::canMemoryBeReleased(MM_EnvironmentBase* env, void* low, void* high)
{
    bool result = true;
    MM_GCExtensionsBase* extensions = env->getExtensions();
...
bool MM_CardTable::isDirtyOrValue(MM_EnvironmentBase* env, void* heapAddr, Card cardValue)
{
    Card* cardAddr = heapAddrToCardAddr(env, heapAddr);
    return (CARD_DIRTY == *cardAddr) || (cardValue == *cardAddr);
}

void MM_CardTable::dirtyCard(MM_EnvironmentBase* env, omrobjectptr_t objectRef)
{
    dirtyCardWithValue(env, objectRef, CARD_DIRTY);
}

void MM_CardTable::dirtyCardWithValue(MM_EnvironmentBase* env, omrobjectptr_t objectRef, Card newValue)
{
...
void MM_CardTable::dirtyCardRange(MM_EnvironmentBase* env, void* heapAddrFrom, void* heapAddrTo)
{
...
Card* MM_CardTable::heapAddrToCardAddr(MM_EnvironmentBase* env, void* heapAddr)
{
    /* Check passed reference within the heap */
...
void* MM_CardTable::cardAddrToHeapAddr(MM_EnvironmentBase* env, Card* cardAddr)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    MM_MemoryManager* memoryManager = extensions->memoryManager;
...
void MM_CardTable::cleanCardTable(MM_EnvironmentBase* env, MM_CardCleaner* cardCleaner)
{
    /* clean cards for entire heap range */
    cleanCardTableForRange(env, cardCleaner, getHeapBase(), _heapAlloc);
}

void MM_CardTable::cleanCardTableForRange(MM_EnvironmentBase* env, MM_CardCleaner* cardCleaner, void* lowAddress, void* highAddress)
{
...
void MM_CardTable::cleanCardsInRegion(MM_EnvironmentBase* env, MM_CardCleaner* cardCleaner, MM_HeapRegionDescriptor* region)
{
    uintptr_t oldVMState = env->pushVMstate(cardCleaner->getVMStateID());
...
bool MM_CardTable::commitCardsForRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptor* region)
{
...
bool MM_CardTable::setNumaAffinityCorrespondingToHeapRange(MM_EnvironmentBase* env, uintptr_t numaNode, void* baseOfHeapRange, void* topOfHeapRange)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    MM_MemoryManager* memoryManager = extensions->memoryManager;
