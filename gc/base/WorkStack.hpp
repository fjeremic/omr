...
private:
  /**
   * Push an element to work stack in case if it can be done for current packet
  ...
  /**
   * Push two elements to work stack in case if it can be done for current
   * packet (full or does not exist)
   * @param env[in] The thread which owns the work stack
  ...
  /**
   * Pop a reference from input packet in case if it can be done for current
   * packet (empty or does not exist) Wait if nothing available
   * @param env[in] The thread which owns the work stack
   * @return reference
   */
  ...
  /**
   * Pop a reference from input packet in case if it can be done for current
   * packet (empty or does not exist) No wait if nothing available
   * @param env[in] The thread which owns the work stack
   * @return reference
   */
...
public:
  ...
  /**
   * Ensures that this work stack is backing onto the given workPackets so that
   * it can be used for work
   * @param env[in] The thread which owns the work stack
   * @param workPackets[in] The work packets we want the receiver to use during
   * the mark
   */
  ...
  /**
   * Immediately flush the output packet back to the shared pool so that it can
   ...
   * Pop a reference from input packet (inlined part)
   * Pop a reference from the stacks input packet. If no references remain
   * in current packet then return NULL (and put input packet back to output
   * list for reuse
   */
   ...
   */
  MM_WorkStack()
      : MM_BaseNonVirtual(), _workPackets(NULL), _inputPacket(NULL),
        _outputPacket(NULL), _deferredPacket(NULL) {
