 ...
 */
void hookWalkHeapStart(J9HookInterface** hook, uintptr_t eventNum,
 ...
 */
void hookWalkHeapEnd(J9HookInterface** hook, uintptr_t eventNum,
                     void* eventData, void* userData) {
 ...
 */
void GC_OMRVMInterface::initializeExtensions(MM_GCExtensionsBase* extensions) {
  J9HookInterface** mmPrivateHooks =
      J9_HOOK_INTERFACE(extensions->privateHookInterface);

  (*mmPrivateHooks)
      ->J9HookRegisterWithCallSite(mmPrivateHooks,
                                   J9HOOK_MM_PRIVATE_WALK_HEAP_START,
                                   hookWalkHeapStart, OMR_GET_CALLSITE(), NULL);
  (*mmPrivateHooks)
      ->J9HookRegisterWithCallSite(mmPrivateHooks,
                                   J9HOOK_MM_PRIVATE_WALK_HEAP_END,
                                   hookWalkHeapEnd, OMR_GET_CALLSITE(), NULL);
}

/**
 * Return the OMR hook interface for the Memory Manager
 */
J9HookInterface** GC_OMRVMInterface::getOmrHookInterface(
 ...
 */
void GC_OMRVMInterface::flushCachesForWalk(OMR_VM* omrVM) {
  /*
 ...
 * Flush Cache for GC.
 */
void GC_OMRVMInterface::flushCachesForGC(MM_EnvironmentBase* env) {
  ...
  while ((omrVMThread = threadListIterator.nextOMRVMThread()) != NULL) {
    ...
    MM_EnvironmentBase* threadEnv =
        MM_EnvironmentBase::getEnvironment(omrVMThread);
    MM_AllocationStats* stats =
        threadEnv->_objectAllocationInterface->getAllocationStats();
 ...
 */
void GC_OMRVMInterface::flushNonAllocationCaches(MM_EnvironmentBase* env) {
  OMR_VMThread* omrVMThread;

  GC_OMRVMThreadListIterator threadListIterator(env->getOmrVM());

  while ((omrVMThread = threadListIterator.nextOMRVMThread()) != NULL) {
    GC_OMRVMThreadInterface::flushNonAllocationCaches(
        MM_EnvironmentBase::getEnvironment(omrVMThread));
  }
}
