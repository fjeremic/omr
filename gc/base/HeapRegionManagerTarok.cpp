...
MM_HeapRegionManagerTarok::MM_HeapRegionManagerTarok(
    ...
    : MM_HeapRegionManager(env, regionSize, tableDescriptorSize,
                           regionDescriptorInitializer,
                           regionDescriptorDestructor),
      _freeRegionTableSize(0), _freeRegionTable(NULL) {
  _typeId = __FUNCTION__;
}

MM_HeapRegionManagerTarok *MM_HeapRegionManagerTarok::newInstance(
    MM_EnvironmentBase *env, uintptr_t regionSize,
    uintptr_t tableDescriptorSize,
    MM_RegionDescriptorInitializer regionDescriptorInitializer,
    MM_RegionDescriptorDestructor regionDescriptorDestructor) {
  MM_HeapRegionManagerTarok *regionManager =
      (MM_HeapRegionManagerTarok *)env->getForge()->allocate(
          sizeof(MM_HeapRegionManagerTarok), OMR::GC::AllocationCategory::FIXED,
          OMR_GET_CALLSITE());
  if (NULL != regionManager) {
    new (regionManager) MM_HeapRegionManagerTarok(
...
bool MM_HeapRegionManagerTarok::initialize(MM_EnvironmentBase *env) {
  bool result = MM_HeapRegionManager::initialize(env);

  if (result) {
    uintptr_t maximumNodeNumber =
        env->getExtensions()->_numaManager.getMaximumNodeNumber();

    _freeRegionTableSize = maximumNodeNumber + 1;

    uintptr_t freeRegionTableSizeInBytes =
        _freeRegionTableSize * sizeof(MM_HeapRegionDescriptor *);
    _freeRegionTable = (MM_HeapRegionDescriptor **)env->getForge()->allocate(
        freeRegionTableSizeInBytes, OMR::GC::AllocationCategory::FIXED,
        OMR_GET_CALLSITE());
...
void MM_HeapRegionManagerTarok::tearDown(MM_EnvironmentBase *env) {
  if (NULL != _freeRegionTable) {
...
bool MM_HeapRegionManagerTarok::areFreeRegionsForNode(MM_EnvironmentBase *env,
                                                      uintptr_t numaNode) {
  return (NULL != _freeRegionTable[numaNode]);
}

MM_HeapRegionDescriptor *MM_HeapRegionManagerTarok::acquireSingleTableRegion(
    MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace, uintptr_t numaNode) {
  MM_HeapRegionDescriptor *toReturn = NULL;
  writeLock();
  Trc_MM_HeapRegionManager_acquireSingleTableRegions_Entry(
      env->getLanguageVMThread(), subSpace, numaNode);
  ...
  Trc_MM_HeapRegionManager_acquireSingleTableRegions_Exit(
      env->getLanguageVMThread(), toReturn, numaNode);
  writeUnlock();
  return toReturn;
}

MM_HeapRegionDescriptor *
MM_HeapRegionManagerTarok::internalAcquireSingleTableRegion(
...
void *MM_HeapRegionManagerTarok::findHighestValidAddressBelow(
    MM_HeapRegionDescriptor *targetRegion) {
  ...
  while (cursorIndex < targetIndex) {
    MM_HeapRegionDescriptor *cursorRegion =
        mapRegionTableIndexToDescriptor(cursorIndex);
...
void *MM_HeapRegionManagerTarok::findLowestValidAddressAbove(
    MM_HeapRegionDescriptor *targetRegion) {
...
bool MM_HeapRegionManagerTarok::setContiguousHeapRange(MM_EnvironmentBase *env,
                                                       void *lowHeapEdge,
                                                       void *highHeapEdge) {
  writeLock();
  ...
  /* This assertion would triggered at multiple attempts to initialize heap in
   * gcInitializeDefaults() */
  ...
  _regionTable =
      internalAllocateAndInitializeRegionTable(env, lowHeapEdge, highHeapEdge);
  bool success = false;
...
void MM_HeapRegionManagerTarok::destroyRegionTable(MM_EnvironmentBase *env) {
  if (NULL != _regionTable) {
...
void MM_HeapRegionManagerTarok::internalLinkRegions(
    MM_EnvironmentBase *env, MM_HeapRegionDescriptor *headRegion,
    uintptr_t count) {
  ...
  for (uintptr_t i = 0; i < count; i++) {
    ...
    MM_HeapRegionDescriptor *nextRegion =
        (MM_HeapRegionDescriptor *)((uintptr_t)region + _tableDescriptorSize);
  ...
  MM_HeapRegionDescriptor *lastRegion =
      (MM_HeapRegionDescriptor *)((uintptr_t)headRegion +
                                  ((count - 1) * _tableDescriptorSize));
  lastRegion->_nextInSet = NULL;
}

bool MM_HeapRegionManagerTarok::enableRegionsInTable(MM_EnvironmentBase *env,
...
void MM_HeapRegionManagerTarok::setNodeAndLinkRegions(MM_EnvironmentBase *env,
                                                      void *lowHeapEdge,
                                                      void *highHeapEdge,
                                                      uintptr_t numaNode) {
  ...
  Trc_MM_HeapRegionManager_enableRegionsInTable_Entry(
      env->getLanguageVMThread(), lowHeapEdge, highHeapEdge, numaNode);
  if (highHeapEdge > lowHeapEdge) {
    for (uint8_t *address = (uint8_t *)lowHeapEdge; address < highHeapEdge;
         address += getRegionSize()) {
  ...
  Trc_MM_HeapRegionManager_enableRegionsInTable_Exit(
      env->getLanguageVMThread(), regionCount, firstRegion, numaNode);
}

void MM_HeapRegionManagerTarok::releaseTableRegions(
...
void MM_HeapRegionManagerTarok::internalReleaseTableRegions(
    MM_EnvironmentBase *env, MM_HeapRegionDescriptor *rootRegion) {
...
MM_HeapMemorySnapshot *MM_HeapRegionManagerTarok::getHeapMemorySnapshot(
