...
MM_HeapRegionManagerTarok* MM_HeapRegionManagerTarok::newInstance(MM_EnvironmentBase* env, uintptr_t regionSize,
    uintptr_t tableDescriptorSize, MM_RegionDescriptorInitializer regionDescriptorInitializer,
    MM_RegionDescriptorDestructor regionDescriptorDestructor)
{
    MM_HeapRegionManagerTarok* regionManager = (MM_HeapRegionManagerTarok*)env->getForge()->allocate(
        sizeof(MM_HeapRegionManagerTarok), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
bool MM_HeapRegionManagerTarok::initialize(MM_EnvironmentBase* env)
{
    ...
    if (result) {
        ...
        _freeRegionTable = (MM_HeapRegionDescriptor**)env->getForge()->allocate(
            freeRegionTableSizeInBytes, OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
bool MM_HeapRegionManagerTarok::areFreeRegionsForNode(MM_EnvironmentBase* env, uintptr_t numaNode)
{
...
MM_HeapRegionDescriptor* MM_HeapRegionManagerTarok::internalAcquireSingleTableRegion(
...
void* MM_HeapRegionManagerTarok::findHighestValidAddressBelow(MM_HeapRegionDescriptor* targetRegion)
{
...
void* MM_HeapRegionManagerTarok::findLowestValidAddressAbove(MM_HeapRegionDescriptor* targetRegion)
{
...
bool MM_HeapRegionManagerTarok::setContiguousHeapRange(MM_EnvironmentBase* env, void* lowHeapEdge, void* highHeapEdge)
{
    writeLock();
...
void MM_HeapRegionManagerTarok::internalLinkRegions(
    MM_EnvironmentBase* env, MM_HeapRegionDescriptor* headRegion, uintptr_t count)
{
...
bool MM_HeapRegionManagerTarok::enableRegionsInTable(MM_EnvironmentBase* env, MM_MemoryHandle* handle)
{
    bool result = true;
...
void MM_HeapRegionManagerTarok::setNodeAndLinkRegions(
    MM_EnvironmentBase* env, void* lowHeapEdge, void* highHeapEdge, uintptr_t numaNode)
{
...
void MM_HeapRegionManagerTarok::releaseTableRegions(MM_EnvironmentBase* env, MM_HeapRegionDescriptor* region)
{
    writeLock();
    /* must be a table region */
    Assert_MM_true((region >= _regionTable)
        && (region < (MM_HeapRegionDescriptor*)((uintptr_t)_regionTable + (_tableRegionCount * _tableDescriptorSize))));
    ...
    MM_EnvironmentBase* env, MM_HeapRegionDescriptor* rootRegion)
{
    ...
    Assert_MM_true(
        rootRegion < (MM_HeapRegionDescriptor*)((uintptr_t)_regionTable + (_tableRegionCount * _tableDescriptorSize)));
