...
private:
  ...
  double *_loaFreeRatioHistory; /**<history of LOA free/total size ratio  for a
                                   past few global GCs */
...
private:
  ...
  double calculateTargetLOARatio(MM_EnvironmentBase *,
...
protected:
public:
  static MM_MemoryPoolLargeObjects *
  ...
  void preCollect(MM_EnvironmentBase *env, bool systemGC, bool aggressive,
                  uintptr_t bytesRequested);
  ...
  /**
   * Merge the tlh allocation distribution stats from both children
   * pools(include merging children pools).
   */
  virtual void mergeTlhAllocateStats();
  /**
   * Merge the current LargeObject allocation distribution stats from both
   * children pools(include merging children pools).
   */
  virtual void mergeLargeObjectAllocateStats();
  /**
   * Merge Average allocation distribution stats from both children
   * pools(include averaging children pools).
   */
  virtual void
  ...
  MMINLINE virtual uintptr_t getDarkMatterBytes() {
    return (_memoryPoolSmallObjects->getDarkMatterBytes() +
            _memoryPoolLargeObjects->getDarkMatterBytes());
  }
  ...
  MM_MemoryPoolLargeObjects(
      ...
      MM_MemoryPoolAddressOrderedListBase *largeObjectArea,
      MM_MemoryPoolAddressOrderedListBase *smallObjectArea)
      : MM_MemoryPool(env, 0), _omrVM(env->getOmrVM()), _currentOldAreaSize(0),
        _currentLOABase(NULL), _memoryPoolSmallObjects(smallObjectArea),
        _memoryPoolLargeObjects(largeObjectArea), _loaSize(0), _soaSize(0),
        _currentLOARatio(_extensions->largeObjectAreaInitialRatio),
        _soaObjectSizeLWM(UDATA_MAX), _soaFreeBytesAfterLastGC(0) {
