 ...
 */
MM_MemoryPoolBumpPointer *MM_MemoryPoolBumpPointer::newInstance(
    MM_EnvironmentBase *env, uintptr_t minimumFreeEntrySize) {
  MM_MemoryPoolBumpPointer *memoryPool =
      (MM_MemoryPoolBumpPointer *)env->getForge()->allocate(
          sizeof(MM_MemoryPoolBumpPointer), OMR::GC::AllocationCategory::FIXED,
          OMR_GET_CALLSITE());
  if (NULL != memoryPool) {
    memoryPool =
...
bool MM_MemoryPoolBumpPointer::initialize(MM_EnvironmentBase *env) {
...
void MM_MemoryPoolBumpPointer::tearDown(MM_EnvironmentBase *env) {
 ...
 ****************************************
 */
MMINLINE void *MM_MemoryPoolBumpPointer::internalAllocate(
    MM_EnvironmentBase *env, uintptr_t sizeInBytesRequired) {
  Assert_MM_objectAligned(env, sizeInBytesRequired);
  void *result = NULL;
  uintptr_t spaceRemaining =
      (uintptr_t)_topPointer - (uintptr_t)_allocatePointer;
  if (sizeInBytesRequired <= spaceRemaining) {
    result = _allocatePointer;
    _allocatePointer =
        (void *)((uintptr_t)_allocatePointer + sizeInBytesRequired);
...
void *MM_MemoryPoolBumpPointer::allocateObject(
...
MMINLINE bool MM_MemoryPoolBumpPointer::internalAllocateTLH(
  ...
  if (_minimumFreeEntrySize <= spaceRemaining) {
    ...
    if (newFreeSpace < _minimumFreeEntrySize) {
      /* We are trying to allocate a TLH which is permitted to be larger than
       * the requested size.  If we were going to discard the remainder of the
       * region, anyway, return it as part of the TLH.
       */
      addrTop = _topPointer;
      newFreeSpace = 0;
      _freeEntryCount = 0;
...
void *MM_MemoryPoolBumpPointer::allocateTLH(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
...
void *MM_MemoryPoolBumpPointer::collectorAllocate(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
...
void *MM_MemoryPoolBumpPointer::collectorAllocateTLH(
    MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
    uintptr_t maximumSizeInBytesRequired, void *&addrBase, void *&addrTop,
    bool lockingRequired) {
  void *tlhBase = NULL;
...
void MM_MemoryPoolBumpPointer::reset(Cause cause) {
 ...
 * given region
 */
MM_HeapLinkedFreeHeader *MM_MemoryPoolBumpPointer::rebuildFreeListInRegion(
...
void MM_MemoryPoolBumpPointer::setAllocationPointer(MM_EnvironmentBase *env,
 ...
 *
 */
void MM_MemoryPoolBumpPointer::expandWithRange(MM_EnvironmentBase *env,
 ...
 *
 */
void *MM_MemoryPoolBumpPointer::contractWithRange(MM_EnvironmentBase *env,
...
bool MM_MemoryPoolBumpPointer::abandonHeapChunk(void *addrBase, void *addrTop) {
...
MMINLINE bool MM_MemoryPoolBumpPointer::internalRecycleHeapChunk(
    void *addrBase, void *addrTop, MM_HeapLinkedFreeHeader *next) {
  ...
  MM_HeapLinkedFreeHeader *freeEntry = MM_HeapLinkedFreeHeader::fillWithHoles(
      addrBase, freeEntrySize, compressed);
...
MM_SweepPoolManager *MM_MemoryPoolBumpPointer::getSweepPoolManager() {
  /*
...
void MM_MemoryPoolBumpPointer::recalculateMemoryPoolStatistics(
...
void MM_MemoryPoolBumpPointer::rewindAllocationPointerTo(void *pointer) {
  Assert_MM_true(pointer < _allocatePointer);
  _allocatePointer = pointer;
}

void MM_MemoryPoolBumpPointer::alignAllocationPointer(
    uintptr_t alignmentMultiple) {
 ...
 */
bool MM_MemoryPoolBumpPointer::createFreeEntry(
 ...
 * @see MM_MemoryPool::createFreeEntry()
 */
bool MM_MemoryPoolBumpPointer::createFreeEntry(MM_EnvironmentBase *env,
                                               void *addrBase, void *addrTop) {
 ...
 * @param previousFreeEntry previous element of list (if any) this memory must
 * be connected with
 * @return true if free memory element accepted
 */
bool MM_MemoryPoolBumpPointer::connectInnerMemoryToPool(
  ...
  if (size >= getMinimumFreeEntrySize()) {
    ...
    createFreeEntry(env, (MM_HeapLinkedFreeHeader *)address,
 ...
 * @param address free memory start address
 * @param size free memory size in bytes
 * @param nextFreeEntry next element of list (if any) this memory must be
 * connected with
 */
void MM_MemoryPoolBumpPointer::connectOuterMemoryToPool(MM_EnvironmentBase *env,
                                                        void *address,
                                                        uintptr_t size,
                                                        void *nextFreeEntry) {
 ...
 * @param size free memory size in bytes
 */
void MM_MemoryPoolBumpPointer::connectFinalMemoryToPool(MM_EnvironmentBase *env,
                                                        void *address,
                                                        uintptr_t size) {
  Assert_MM_true((NULL == address) || (size >= getMinimumFreeEntrySize()));
