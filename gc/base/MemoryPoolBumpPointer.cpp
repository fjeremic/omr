 ...
 */
MM_MemoryPoolBumpPointer* MM_MemoryPoolBumpPointer::newInstance(MM_EnvironmentBase* env, uintptr_t minimumFreeEntrySize)
{
    MM_MemoryPoolBumpPointer* memoryPool = (MM_MemoryPoolBumpPointer*)env->getForge()->allocate(
        sizeof(MM_MemoryPoolBumpPointer), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (NULL != memoryPool) {
 ...
 ****************************************
 */
MMINLINE void* MM_MemoryPoolBumpPointer::internalAllocate(MM_EnvironmentBase* env, uintptr_t sizeInBytesRequired)
{
...
void* MM_MemoryPoolBumpPointer::allocateObject(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
    ...
    MM_EnvironmentBase* env, uintptr_t maximumSizeInBytesRequired, void*& addrBase, void*& addrTop)
{
    ...
    if (_minimumFreeEntrySize <= spaceRemaining) {
        ...
        if (newFreeSpace < _minimumFreeEntrySize) {
            /* We are trying to allocate a TLH which is permitted to be larger than the requested size.  If we were
             * going to discard the remainder of the region, anyway, return it as part of the TLH.
             */
            addrTop = _topPointer;
            newFreeSpace = 0;
            _freeEntryCount = 0;
...
void* MM_MemoryPoolBumpPointer::allocateTLH(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
...
void* MM_MemoryPoolBumpPointer::collectorAllocateTLH(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
...
void MM_MemoryPoolBumpPointer::setAllocationPointer(MM_EnvironmentBase* env, void* allocatePointer)
{
 ...
 *
 */
void MM_MemoryPoolBumpPointer::expandWithRange(
 ...
 *
 */
void* MM_MemoryPoolBumpPointer::contractWithRange(
...
void MM_MemoryPoolBumpPointer::recalculateMemoryPoolStatistics(MM_EnvironmentBase* env)
{
...
void MM_MemoryPoolBumpPointer::alignAllocationPointer(uintptr_t alignmentMultiple)
{
 ...
 */
bool MM_MemoryPoolBumpPointer::createFreeEntry(MM_EnvironmentBase* env, void* addrBase, void* addrTop,
...
bool MM_MemoryPoolBumpPointer::connectInnerMemoryToPool(
    MM_EnvironmentBase* env, void* address, uintptr_t size, void* previousFreeEntry)
{
    bool result = false;
 ...
 */
void MM_MemoryPoolBumpPointer::connectOuterMemoryToPool(
    MM_EnvironmentBase* env, void* address, uintptr_t size, void* nextFreeEntry)
{
 ...
 * @param size free memory size in bytes
 */
void MM_MemoryPoolBumpPointer::connectFinalMemoryToPool(MM_EnvironmentBase* env, void* address, uintptr_t size)
{
    Assert_MM_true((NULL == address) || (size >= getMinimumFreeEntrySize()));
