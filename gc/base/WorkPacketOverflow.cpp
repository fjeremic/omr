...
MM_WorkPacketOverflow::initialize(MM_EnvironmentBase* env)
{
  if (omrthread_monitor_init_with_name(
        &_overflowListMonitor, 0, "MM_WorkPacketOverflow::overflowList")) {
    return false;
  }
...
void
MM_WorkPacketOverflow::emptyToOverflow(MM_EnvironmentBase* env,
                                       MM_Packet* packet,
...
void
MM_WorkPacketOverflow::fillFromOverflow(MM_EnvironmentBase* env,
                                        MM_Packet* packet)
{
  Assert_MM_unreachable();
}
...
void
MM_WorkPacketOverflow::overflowItem(MM_EnvironmentBase* env,
                                    void* item,
                                    MM_OverflowType type)
{
  Assert_MM_unreachable();
