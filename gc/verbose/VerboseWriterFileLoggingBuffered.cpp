...
MM_VerboseWriterFileLoggingBuffered::MM_VerboseWriterFileLoggingBuffered(
    ...
    : MM_VerboseWriterFileLogging(env, manager,
                                  VERBOSE_WRITER_FILE_LOGGING_BUFFERED),
      _logFileStream(NULL) {
  /* No implementation */
}
...
MM_VerboseWriterFileLoggingBuffered::newInstance(MM_EnvironmentBase *env,
  ...
  MM_VerboseWriterFileLoggingBuffered *agent =
      (MM_VerboseWriterFileLoggingBuffered *)extensions->getForge()->allocate(
          sizeof(MM_VerboseWriterFileLoggingBuffered),
          OMR::GC::AllocationCategory::DIAGNOSTIC, OMR_GET_CALLSITE());
  if (agent) {
 ...
 */
bool MM_VerboseWriterFileLoggingBuffered::initialize(MM_EnvironmentBase *env,
                                                     const char *filename,
                                                     uintptr_t numFiles,
                                                     uintptr_t numCycles) {
 ...
 */
void MM_VerboseWriterFileLoggingBuffered::tearDown(MM_EnvironmentBase *env) {
 ...
 */
bool MM_VerboseWriterFileLoggingBuffered::openFile(MM_EnvironmentBase *env) {
  ...
  _logFileStream = omrfilestream_open(
      filenameToOpen, EsOpenWrite | EsOpenCreate | EsOpenTruncate, 0666);
 ...
 */
void MM_VerboseWriterFileLoggingBuffered::closeFile(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());

  if (NULL != _logFileStream) {
    omrfilestream_write_text(_logFileStream, getFooter(env),
                             strlen(getFooter(env)), J9STR_CODE_PLATFORM_RAW);
...
void MM_VerboseWriterFileLoggingBuffered::outputString(MM_EnvironmentBase *env,
                                                       const char *string) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  ...
  if (NULL != _logFileStream) {
    omrfilestream_write_text(_logFileStream, string, strlen(string),
                             J9STR_CODE_PLATFORM_RAW);
  } else {
    omrfilestream_write_text(OMRPORT_STREAM_ERR, string, strlen(string),
                             J9STR_CODE_PLATFORM_RAW);
  }
}
