 ...
 */
bool
MM_VerboseWriterFileLoggingBuffered::initialize(MM_EnvironmentBase* env,
                                                const char* filename,
                                                uintptr_t numFiles,
                                                uintptr_t numCycles)
{
...
void
MM_VerboseWriterFileLoggingBuffered::outputString(MM_EnvironmentBase* env,
                                                  const char* string)
{
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  ...
  if (NULL != _logFileStream) {
    omrfilestream_write_text(
      _logFileStream, string, strlen(string), J9STR_CODE_PLATFORM_RAW);
  } else {
    omrfilestream_write_text(
      OMRPORT_STREAM_ERR, string, strlen(string), J9STR_CODE_PLATFORM_RAW);
  }
}
