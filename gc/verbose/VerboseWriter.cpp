...
#define VERBOSEGC_HEADER                                                       \
  "<?xml version=\"1.0\" ?>\n\n<verbosegc "                                    \
  "xmlns=\"http://www.ibm.com/j9/verbosegc\" version=\"%s\">\n\n"
#define VERBOSEGC_FOOTER "</verbosegc>\n"

MM_VerboseWriter::MM_VerboseWriter(WriterType type)
    : MM_Base(), _nextWriter(NULL), _header(NULL), _footer(NULL), _type(type),
      _isActive(false) {}

const char *MM_VerboseWriter::getHeader(MM_EnvironmentBase *env) {
  return _header;
}

const char *MM_VerboseWriter::getFooter(MM_EnvironmentBase *env) {
  return _footer;
}

MM_VerboseWriter *MM_VerboseWriter::getNextWriter() { return _nextWriter; }
...
bool MM_VerboseWriter::initialize(MM_EnvironmentBase *env) {
  ...
  _header = (char *)ext->getForge()->allocate(
      sizeof(char) * headerLength, OMR::GC::AllocationCategory::DIAGNOSTIC,
      OMR_GET_CALLSITE());
  if (NULL == _header) {
    return false;
  }
  ...
  _footer = (char *)ext->getForge()->allocate(
      sizeof(char) * footerLength, OMR::GC::AllocationCategory::DIAGNOSTIC,
      OMR_GET_CALLSITE());
  if (NULL == _footer) {
    ext->getForge()->free(_header);
    return false;
