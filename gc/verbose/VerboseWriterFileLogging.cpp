...
MM_VerboseWriterFileLogging::MM_VerboseWriterFileLogging(
    MM_EnvironmentBase *env, MM_VerboseManager *manager, WriterType type)
    : MM_VerboseWriter(type), _filename(NULL), _mode(single_file),
      _currentFile(0), _currentCycle(0), _tokens(NULL), _manager(manager) {
  /* No implementation */
}
 ...
 */
bool MM_VerboseWriterFileLogging::initialize(MM_EnvironmentBase *env,
                                             const char *filename,
                                             uintptr_t numFiles,
                                             uintptr_t numCycles) {
 ...
 */
void MM_VerboseWriterFileLogging::tearDown(MM_EnvironmentBase *env) {
 ...
 */
bool MM_VerboseWriterFileLogging::initializeTokens(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  char pidBuffer[64];
  ...
  if (sizeof(pidBuffer) <
      omrstr_subst_tokens(pidBuffer, sizeof(pidBuffer), "%pid", _tokens)) {
    return false;
  }
 ...
 *
 * Since token substitution only supports tokens starting with %, all #
 * characters in the filename will be replaced with %seq (unless the # is
 * already preceded by an odd number of % signs, in which case it is replaced
 * with seq).
 *
 * e.g.  foo#   --> foo%seq
 *       foo%#  --> foo%seq
 *       foo%%# --> foo%%%seq
 *
 * If %seq or %# is not specified, and if rotating logs have been requested,
 * ".%seq" is appended to the log name.
 *
 ...
 */
bool MM_VerboseWriterFileLogging::initializeFilename(MM_EnvironmentBase *env,
                                                     const char *filename) {
  MM_GCExtensionsBase *extensions =
      MM_GCExtensionsBase::getExtensions(env->getOmrVM());
  ...
  if (_mode == rotating_files) {
    ...
    _filename = (char *)extensions->getForge()->allocate(
        nameLen, OMR::GC::AllocationCategory::DIAGNOSTIC, OMR_GET_CALLSITE());
    if (NULL == _filename) {
      return false;
    }

    /* copy the original filename into the allocated memory, expanding #s to
     * %seq */
    bool foundSeq = false;
    ...
    *write = '\0';
    ...
    _filename = (char *)extensions->getForge()->allocate(
        strlen(filename) + 1, OMR::GC::AllocationCategory::DIAGNOSTIC,
        OMR_GET_CALLSITE());
    if (NULL == _filename) {
      return false;
    }
 ...
 * @param currentFile the current file number to substitute into the filename
 * template
 *
 * @return NULL on failure, allocated memory on success
 */
char *MM_VerboseWriterFileLogging::expandFilename(MM_EnvironmentBase *env,
                                                  uintptr_t currentFile) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  MM_GCExtensionsBase *extensions =
      MM_GCExtensionsBase::getExtensions(env->getOmrVM());
  ...
  if (_mode == rotating_files) {
    omrstr_set_token(
        _tokens, "seq", "%03zu",
        currentFile +
            1); /* plus one so the filenames start from .001 instead of .000 */
  }

  uintptr_t len = omrstr_subst_tokens(NULL, 0, _filename, _tokens);
  char *filenameToOpen = (char *)extensions->getForge()->allocate(
      len, OMR::GC::AllocationCategory::DIAGNOSTIC, OMR_GET_CALLSITE());
 ...
 */
intptr_t MM_VerboseWriterFileLogging::findInitialFile(MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  MM_GCExtensionsBase *extensions =
      MM_GCExtensionsBase::getExtensions(env->getOmrVM());
  int64_t oldestTime =
      J9CONST64(0x7FFFFFFFFFFFFFFF); /* the highest possible time. */
 ...
 */
void MM_VerboseWriterFileLogging::closeStream(MM_EnvironmentBase *env) {
  closeFile(env);
}
 ...
 */
void MM_VerboseWriterFileLogging::endOfCycle(MM_EnvironmentBase *env) {
 ...
 */
bool MM_VerboseWriterFileLogging::reconfigure(MM_EnvironmentBase *env,
                                              const char *filename,
                                              uintptr_t numFiles,
                                              uintptr_t numCycles) {
  closeFile(env);
