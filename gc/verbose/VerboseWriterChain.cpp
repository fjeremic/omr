...
MM_VerboseWriterChain::MM_VerboseWriterChain()
    : MM_Base(), _buffer(NULL), _writers(NULL) {}

MM_VerboseWriterChain* MM_VerboseWriterChain::newInstance(
    MM_EnvironmentBase* env) {
  MM_GCExtensionsBase* extensions = env->getExtensions();

  MM_VerboseWriterChain* chain =
      (MM_VerboseWriterChain*)extensions->getForge()->allocate(
          sizeof(MM_VerboseWriterChain),
...
void MM_VerboseWriterChain::formatAndOutputV(MM_EnvironmentBase* env,
                                             uintptr_t indent,
                                             const char* format,
                                             va_list args) {
...
void MM_VerboseWriterChain::formatAndOutput(MM_EnvironmentBase* env,
                                            uintptr_t indent,
                                            const char* format,
                                            ...) {
  va_list args;
