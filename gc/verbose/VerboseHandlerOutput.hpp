...
class MM_VerboseHandlerOutput : public MM_Base {
 ...
 protected:
  ...
  OMR_VM* _omrVM;
  J9HookInterface**
      _mmPrivateHooks;           /**< Pointers to the internal Hook interface */
  J9HookInterface** _mmOmrHooks; /**< Pointers to the internal Hook interface */
  MM_VerboseManager*
      _manager; /* VerboseManager used to format and print output */
 public:
 private:
 protected:
  ...
  virtual bool getThreadName(char* buf, uintptr_t bufLen,
                             OMR_VMThread* vmThread);
  virtual void writeVmArgs(MM_EnvironmentBase* env);

  bool getTimeDeltaInMicroSeconds(uint64_t* timeInMicroSeconds,
    ...
    *timeInMicroSeconds = omrtime_hires_delta(
        startTime, endTime, OMRPORT_TIME_DELTA_IN_MICROSECONDS);
    return true;
  }

  /**
   * Return the time elapsed between startTimeInMicroseconds and
   * endTimeInMicroseconds
   * @param [out] timeInMicroSeconds Pointer to the variable where the delta
   * will be stored in
   ...
   * timeInMicroSeconds is set to 0.
   */
  bool getTimeDelta(uint64_t* timeInMicroSeconds,
                    uint64_t startTimeInMicroseconds,
                    uint64_t endTimeInMicroseconds) {
   ...
   * This routine is meant to be overridden by subclasses to implement collector
   * specific output or functionality.
   * @param hook Hook interface used by the JVM.
   * @param eventNum The hook event number.
   * @param eventData hook specific event data.
   */
  virtual void handleInitializedInnerStanzas(J9HookInterface** hook,
                                             uintptr_t eventNum,
                                             void* eventData);
   ...
   * @param eventData hook specific event data.
   */
  void handleInitializedRegion(J9HookInterface** hook, uintptr_t eventNum,
                               void* eventData);
   ...
   * @param indentDepth base indent count for all inner stanza lines.
   */
  virtual void handleCycleEndInnerStanzas(J9HookInterface** hook,
                                          uintptr_t eventNum, void* eventData,
                                          uintptr_t indentDepth);
  ...
  /**
   * Handle any output or data tracking for the allocation failure start event
   * of verbose GC. This routine is meant to be overridden by subclasses to
   * implement collector specific output or functionality.
   * @param hook Hook interface used by the JVM.
   * @param eventNum The hook event number.
   * @param eventData hook specific event data.
   * @param indentDepth base indent count for all inner stanza lines.
  ...
  /**
   * Called before outputting verbose data which is intended to be logically
   * atomic.  Most implementations do nothing with this call but some might need
   * to lock if they permit concurrent event reporting.
   */
  virtual void enterAtomicReportingBlock();

  /**
   * Called after outputting verbose data which is intended to be logically
   * atomic.  Most implementations do nothing with this call but some might need
   * to unlock if they permit concurrent event reporting.
   */
   ...
   */
  void outputMemoryInfo(MM_EnvironmentBase* env, uintptr_t indent,
                        MM_CollectionStatistics* stats);

  virtual bool hasOutputMemoryInfoInnerStanza();

  virtual void outputMemoryInfoInnerStanza(MM_EnvironmentBase* env,
                                           uintptr_t indent,
                                           MM_CollectionStatistics* stats);
   ...
   */
  void outputHeapResizeInfo(MM_EnvironmentBase* env, uintptr_t indent,
                            HeapResizeType resizeType, uintptr_t resizeAmount,
                            uintptr_t resizeCount, uintptr_t subSpaceType,
   ...
   * @param timeInMicroSeconds the total time that all of the resizes took
   */
  void outputCollectorHeapResizeInfo(MM_EnvironmentBase* env, uintptr_t indent,
                                     HeapResizeType resizeType,
                                     uintptr_t resizeAmount,
                                     uintptr_t resizeCount,
                                     uintptr_t subSpaceType, uintptr_t reason,
   ...
   */
  void outputStringConstantInfo(MM_EnvironmentBase* env, uintptr_t indent,
                                uintptr_t candidates, uintptr_t cleared);

 public:
  static MM_VerboseHandlerOutput* newInstance(MM_EnvironmentBase* env,
                                              MM_VerboseManager* manager);

  void kill(MM_EnvironmentBase* env);

  /**
   * Perform necessary hooks and infrastructure setup to enable verbose gc
   * output.
   */
  virtual void enableVerbose();

  /**
   * Perform necessary unhook and infrastructure teardown to disable verbose gc
   * output.
   */
   ...
   * @note should be moved to protected once all standard usage is converted.
   */
  uintptr_t getTagTemplate(char* buf, uintptr_t bufsize, uintptr_t id,
                           uint64_t wallTimeMs);
   ...
   * @param contextId unique identifier of the associated event this is
   * associated with (parent/sibling relationship).
   * @param wallTimeMs wall clock time to be used as the timestamp for the tag.
   * @return number of bytes consumed in the buffer.
   *
   * @note should be moved to protected once all standard usage is converted.
   */
  uintptr_t getTagTemplate(char* buf, uintptr_t bufsize, uintptr_t id,
                           const char* type, uintptr_t contextId,
                           uint64_t wallTimeMs);
   ...
   * @param bufsize maximum size allowed in the character buffer.
   * @param id unique id of the tag being built.
   * @param type Human readable name for the type of the tag - old cycle that
   * has finished.
   * @param type Human readable name for the type of the tag - new cycle that is
   * starting.
   * @param contextId unique identifier of the associated event this is
   * associated with (parent/sibling relationship).
   * @param wallTimeMs wall clock time to be used as the timestamp for the tag.
   ...
   * @note should be moved to protected once all standard usage is converted.
   */
  uintptr_t getTagTemplateWithOldType(char* buf, uintptr_t bufsize,
                                      uintptr_t id, const char* oldType,
                                      const char* newType, uintptr_t contextId,
                                      uint64_t wallTimeMs);
   ...
   * @param contextId unique identifier of the associated event this is
   * associated with (parent/sibling relationship).
   * @param durationus the time difference in microseconds between this stanza
   * and another sibling stanza in past (for example from <gc-start to <gc-end)
   * @param usertimeus the time difference in microseconds taken in user space
   * for this process
   * @param cputimeus the time difference in microseconds taken in kernel space
   * for this process
   * @param wallTimeMs wall clock time to be used as the timestamp for the tag.
   * @return number of bytes consumed in the buffer.
   *
   * @note should be moved to protected once all standard usage is converted.
   */
  uintptr_t getTagTemplateWithDuration(char* buf, uintptr_t bufsize,
                                       uintptr_t id, const char* type,
                                       uintptr_t contextId, uint64_t durationus,
                                       uint64_t usertimeus, uint64_t cputimeus,
                                       uint64_t wallTimeMs);
   ...
   * @param eventData hook specific event data.
   */
  virtual void handleInitialized(J9HookInterface** hook, uintptr_t eventNum,
                                 void* eventData);
   ...
   * @param eventData hook specific event data.
   */
  void handleCycleStart(J9HookInterface** hook, uintptr_t eventNum,
                        void* eventData);

  void handleCycleContinue(J9HookInterface** hook, uintptr_t eventNum,
                           void* eventData);
   ...
   * @param eventData hook specific event data.
   */
  void handleCycleEnd(J9HookInterface** hook, uintptr_t eventNum,
                      void* eventData);
   ...
   * @param eventData hook specific event data.
   */
  void handleExclusiveStart(J9HookInterface** hook, uintptr_t eventNum,
                            void* eventData);
   ...
   * @param eventData hook specific event data.
   */
  void handleExclusiveEnd(J9HookInterface** hook, uintptr_t eventNum,
                          void* eventData);
   ...
   * @param eventData hook specific event data.
   */
  void handleSystemGCStart(J9HookInterface** hook, uintptr_t eventNum,
                           void* eventData);
   ...
   * @param eventData hook specific event data.
   */
  void handleSystemGCEnd(J9HookInterface** hook, uintptr_t eventNum,
                         void* eventData);
   ...
   * @param eventData hook specific event data.
   */
  void handleAllocationFailureStart(J9HookInterface** hook, uintptr_t eventNum,
                                    void* eventData);
   ...
   * @param eventData hook specific event data.
   */
  void handleFailedAllocationCompleted(J9HookInterface** hook,
                                       uintptr_t eventNum, void* eventData);
   ...
   * @param eventData hook specific event data.
   */
  void handleAllocationFailureEnd(J9HookInterface** hook, uintptr_t eventNum,
                                  void* eventData);

  /**
   * Write the verbose stanza for exclusive access acquisition that satisfies an
   * allocation.
   * @param hook Hook interface used by the JVM.
   * @param eventNum The hook event number.
   * @param eventData hook specific event data.
   */
  void handleAcquiredExclusiveToSatisfyAllocation(J9HookInterface** hook,
                                                  uintptr_t eventNum,
                                                  void* eventData);
   ...
   * @param eventData hook specific event data.
   */
  void handleGCStart(J9HookInterface** hook, uintptr_t eventNum,
                     void* eventData);
  ...
  void handleConcurrentStart(J9HookInterface** hook, UDATA eventNum,
                             void* eventData);
  void handleConcurrentEnd(J9HookInterface** hook, UDATA eventNum,
                           void* eventData);

  virtual void handleConcurrentStartInternal(J9HookInterface** hook,
                                             UDATA eventNum, void* eventData) {}
  virtual void handleConcurrentEndInternal(J9HookInterface** hook,
                                           UDATA eventNum, void* eventData);
  virtual const char* getConcurrentTypeString() { return NULL; }

  virtual void handleConcurrentGCOpStart(J9HookInterface** hook,
                                         uintptr_t eventNum, void* eventData) {}
  virtual void handleConcurrentGCOpEnd(J9HookInterface** hook,
                                       uintptr_t eventNum, void* eventData) {}

  void handleHeapResize(J9HookInterface** hook, uintptr_t eventNum,
                        void* eventData);
   ...
   * @param eventData hook specific event data.
   */
  void handleExcessiveGCRaised(J9HookInterface** hook, uintptr_t eventNum,
                               void* eventData);
};
