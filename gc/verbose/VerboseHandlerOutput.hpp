...
protected:
     ...
     */
    bool getTimeDelta(uint64_t* timeInMicroSeconds, uint64_t startTimeInMicroseconds, uint64_t endTimeInMicroseconds)
    {
    ...
    /**
     * Called before outputting verbose data which is intended to be logically atomic.  Most implementations do nothing
     * with this call but some might need to lock if they permit concurrent event reporting.
     */
    virtual void enterAtomicReportingBlock();

    /**
     * Called after outputting verbose data which is intended to be logically atomic.  Most implementations do nothing
     * with this call but some might need to unlock if they permit concurrent event reporting.
     */
...
public:
     ...
     * @param type Human readable name for the type of the tag.
     * @param contextId unique identifier of the associated event this is associated with (parent/sibling relationship).
     * @param durationus the time difference in microseconds between this stanza and another sibling stanza in past (for
     * example from <gc-start to <gc-end)
