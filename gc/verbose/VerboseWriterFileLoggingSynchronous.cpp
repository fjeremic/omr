...
MM_VerboseWriterFileLoggingSynchronous::MM_VerboseWriterFileLoggingSynchronous(
    ...
    : MM_VerboseWriterFileLogging(env, manager,
                                  VERBOSE_WRITER_FILE_LOGGING_SYNCHRONOUS),
...
MM_VerboseWriterFileLoggingSynchronous::newInstance(MM_EnvironmentBase *env,
  ...
  MM_VerboseWriterFileLoggingSynchronous *agent =
      (MM_VerboseWriterFileLoggingSynchronous *)extensions->getForge()
          ->allocate(sizeof(MM_VerboseWriterFileLoggingSynchronous),
                     OMR::GC::AllocationCategory::DIAGNOSTIC,
                     OMR_GET_CALLSITE());
  if (agent) {
 ...
 */
bool MM_VerboseWriterFileLoggingSynchronous::initialize(MM_EnvironmentBase *env,
                                                        const char *filename,
                                                        uintptr_t numFiles,
                                                        uintptr_t numCycles) {
  return MM_VerboseWriterFileLogging::initialize(env, filename, numFiles,
                                                 numCycles);
}

/**
 * Tear down the structures managed by the
 * MM_VerboseWriterFileLoggingSynchronous. Tears down the verbose buffer.
 */
void MM_VerboseWriterFileLoggingSynchronous::tearDown(MM_EnvironmentBase *env) {
 ...
 */
bool MM_VerboseWriterFileLoggingSynchronous::openFile(MM_EnvironmentBase *env) {
  ...
  _logFileDescriptor = omrfile_open(
      filenameToOpen, EsOpenRead | EsOpenWrite | EsOpenCreate | EsOpenTruncate,
      0666);
  if (-1 == _logFileDescriptor) {
    ...
    _logFileDescriptor = omrfile_open(
        filenameToOpen,
        EsOpenRead | EsOpenWrite | EsOpenCreate | EsOpenTruncate, 0666);
    if (-1 == _logFileDescriptor) {
 ...
 */
void MM_VerboseWriterFileLoggingSynchronous::closeFile(
    MM_EnvironmentBase *env) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());

  if (-1 != _logFileDescriptor) {
    omrfile_write_text(_logFileDescriptor, getFooter(env),
                       strlen(getFooter(env)));
...
void MM_VerboseWriterFileLoggingSynchronous::outputString(
    MM_EnvironmentBase *env, const char *string) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());

  if (-1 == _logFileDescriptor) {
    /* we open the file at the end of the cycle so can't have a final empty file
     * at the end of a run */
    openFile(env);
  }
