 ...
 */
MM_VerboseManager *MM_VerboseManager::newInstance(MM_EnvironmentBase *env,
                                                  OMR_VM *vm) {
  MM_GCExtensionsBase *extensions = MM_GCExtensionsBase::getExtensions(vm);

  MM_VerboseManager *verboseManager =
      (MM_VerboseManager *)extensions->getForge()->allocate(
          sizeof(MM_VerboseManager), OMR::GC::AllocationCategory::FIXED,
          OMR_GET_CALLSITE());
  if (verboseManager) {
 ...
 */
void MM_VerboseManager::kill(MM_EnvironmentBase *env) {
  tearDown(env);
 ...
 */
bool MM_VerboseManager::initialize(MM_EnvironmentBase *env) {
 ...
 */
void MM_VerboseManager::tearDown(MM_EnvironmentBase *env) {
  disableVerboseGC();
...
MM_VerboseHandlerOutput *MM_VerboseManager::createVerboseHandlerOutputObject(
...
void MM_VerboseManager::closeStreams(MM_EnvironmentBase *env) {
...
void MM_VerboseManager::enableVerboseGC() {
  if (!_hooksAttached) {
...
void MM_VerboseManager::disableVerboseGC() {
  if (_hooksAttached) {
 ...
 */
MM_VerboseWriter *MM_VerboseManager::findWriterInChain(WriterType type) {
  MM_VerboseWriter *writer = _writerChain->getFirstWriter();
 ...
 */
uintptr_t MM_VerboseManager::countActiveOutputHandlers() {
  MM_VerboseWriter *writer = _writerChain->getFirstWriter();
  uintptr_t count = 0;
 ...
 */
void MM_VerboseManager::disableWriters() {
  MM_VerboseWriter *writer = _writerChain->getFirstWriter();
...
WriterType MM_VerboseManager::parseWriterType(MM_EnvironmentBase *env,
                                              char *filename,
                                              uintptr_t fileCount,
 ...
 */
bool MM_VerboseManager::configureVerboseGC(OMR_VM *omrVM, char *filename,
                                           uintptr_t fileCount,
                                           uintptr_t iterations) {
...
MM_VerboseWriter *MM_VerboseManager::createWriter(MM_EnvironmentBase *env,
                                                  WriterType type,
                                                  char *filename,
                                                  uintptr_t fileCount,
                                                  uintptr_t iterations) {
  ...
  switch (type) {
    ...
    case VERBOSE_WRITER_FILE_LOGGING_SYNCHRONOUS:
      writer = MM_VerboseWriterFileLoggingSynchronous::newInstance(
          env, this, filename, fileCount, iterations);
      if (NULL == writer) {
        ...
        /* if we failed to create a file stream and there is no stderr stream
         * try to create a stderr stream */
        writer = MM_VerboseWriterStreamOutput::newInstance(env, NULL);
      }
      break;
    case VERBOSE_WRITER_FILE_LOGGING_BUFFERED:
      writer = MM_VerboseWriterFileLoggingBuffered::newInstance(
          env, this, filename, fileCount, iterations);
      if (NULL == writer) {
        writer = findWriterInChain(VERBOSE_WRITER_STANDARD_STREAM);
        if (NULL != writer) {
