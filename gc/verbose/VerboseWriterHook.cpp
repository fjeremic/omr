 ...
 */
MM_VerboseWriterHook *MM_VerboseWriterHook::newInstance(
    MM_EnvironmentBase *env) {
  MM_GCExtensionsBase *extensions =
      MM_GCExtensionsBase::getExtensions(env->getOmrVM());

  MM_VerboseWriterHook *agent =
      (MM_VerboseWriterHook *)extensions->getForge()->allocate(
          sizeof(MM_VerboseWriterHook), OMR::GC::AllocationCategory::DIAGNOSTIC,
          OMR_GET_CALLSITE());
  if (agent) {
 ...
 */
bool MM_VerboseWriterHook::initialize(MM_EnvironmentBase *env) {
...
void MM_VerboseWriterHook::outputString(MM_EnvironmentBase *env,
                                        const char *string) {
  ...
  TRIGGER_J9HOOK_MM_OMR_VERBOSE_GC_OUTPUT(
      extensions->omrHookInterface, vmThread, omrtime_hires_clock(), string);
}
