...
bool MM_VerboseHandlerOutput::initialize(MM_EnvironmentBase* env, MM_VerboseManager* manager)
{
    _omrVM = env->getOmrVM();
...
void MM_VerboseHandlerOutput::enableVerbose()
{
    ...
    (*_mmOmrHooks)
        ->J9HookRegisterWithCallSite(
            _mmOmrHooks, J9HOOK_MM_OMR_INITIALIZED, verboseHandlerInitialized, OMR_GET_CALLSITE(), (void*)this);
    (*_mmPrivateHooks)
        ->J9HookRegisterWithCallSite(
            _mmPrivateHooks, J9HOOK_MM_PRIVATE_HEAP_RESIZE, verboseHandlerHeapResize, OMR_GET_CALLSITE(), (void*)this);
...
bool MM_VerboseHandlerOutput::getThreadName(char* buf, uintptr_t bufLen, OMR_VMThread* vmThread)
{
...
uintptr_t MM_VerboseHandlerOutput::getTagTemplate(char* buf, uintptr_t bufsize, uint64_t wallTimeMs)
{
    OMRPORT_ACCESS_FROM_OMRVM(_omrVM);
    uintptr_t bufPos = 0;
...
uintptr_t MM_VerboseHandlerOutput::getTagTemplate(char* buf, uintptr_t bufsize, uintptr_t id, uint64_t wallTimeMs)
{
    OMRPORT_ACCESS_FROM_OMRVM(_omrVM);
    uintptr_t bufPos = 0;
...
uintptr_t MM_VerboseHandlerOutput::getTagTemplateWithOldType(char* buf, uintptr_t bufsize, uintptr_t id,
    const char* oldType, const char* newType, uintptr_t contextId, uint64_t wallTimeMs)
{
    OMRPORT_ACCESS_FROM_OMRVM(_omrVM);
    uintptr_t bufPos = 0;
    bufPos += omrstr_printf(buf, bufsize, "id=\"%zu\" oldtype=\"%s\" newtype=\"%s\" contextid=\"%zu\" timestamp=\"", id,
        oldType, newType, contextId);
    bufPos += omrstr_ftime(buf + bufPos, bufsize - bufPos, VERBOSEGC_DATE_FORMAT_PRE_MS, wallTimeMs);
    bufPos += omrstr_printf(buf + bufPos, bufsize - bufPos, "%03llu", wallTimeMs % 1000);
    bufPos += omrstr_ftime(buf + bufPos, bufsize - bufPos, VERBOSEGC_DATE_FORMAT_POST_MS, wallTimeMs);
    bufPos += omrstr_printf(buf + bufPos, bufsize - bufPos, "\"");
...
uintptr_t MM_VerboseHandlerOutput::getTagTemplate(char* buf, uintptr_t bufsize, uintptr_t id, const char* type,
    uintptr_t contextId, uint64_t timeus, uint64_t wallTimeMs)
{
    OMRPORT_ACCESS_FROM_OMRVM(_omrVM);
    uintptr_t bufPos = 0;
    bufPos
        += omrstr_printf(buf, bufsize, "id=\"%zu\" type=\"%s\" timems=\"%llu.%03.3llu\" contextid=\"%zu\" timestamp=\"",
...
uintptr_t MM_VerboseHandlerOutput::getTagTemplateWithDuration(char* buf, uintptr_t bufsize, uintptr_t id,
    const char* type, uintptr_t contextId, uint64_t durationus, uint64_t usertimeus, uint64_t cputimeus,
    uint64_t wallTimeMs)
{
    OMRPORT_ACCESS_FROM_OMRVM(_omrVM);
    uintptr_t bufPos = 0;
    bufPos += omrstr_printf(buf, bufsize,
        "id=\"%zu\" type=\"%s\" contextid=\"%zu\" durationms=\"%llu.%03.3llu\" usertimems=\"%llu.%03.3llu\" "
        "systemtimems=\"%llu.%03.3llu\" timestamp=\"",
...
void MM_VerboseHandlerOutput::handleInitializedInnerStanzas(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    return;
}

void MM_VerboseHandlerOutput::handleInitializedRegion(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
...
void MM_VerboseHandlerOutput::handleInitialized(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    MM_InitializedEvent* event = (MM_InitializedEvent*)eventData;
    MM_VerboseWriterChain* writer = _manager->getWriterChain();
    MM_EnvironmentBase* env = MM_EnvironmentBase::getEnvironment(event->currentThread);
    ...
    if (env->compressObjectReferences()) {
        ...
        writer->formatAndOutput(
            env, 1, "<attribute name=\"compressedRefsShift\" value=\"0x%zx\" />", event->compressedPointersShift);
    } else
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
        if (_extensions->isConcurrentScavengerEnabled()) {
            writer->formatAndOutput(env, 1, "<attribute name=\"gcthreads Concurrent Scavenger\" value=\"%zu\" />",
                _extensions->concurrentScavengerBackgroundThreads);
        }
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
        if (_extensions->isConcurrentMarkEnabled()) {
            writer->formatAndOutput(env, 1, "<attribute name=\"gcthreads Concurrent Mark\" value=\"%zu\" />",
    ...
    writer->formatAndOutput(
        env, 1, "<attribute name=\"splitFreeListSplitAmount\" value=\"%zu\" />", _extensions->splitFreeListSplitAmount);
...
void MM_VerboseHandlerOutput::handleCycleStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    ...
    getTagTemplate(
        tagTemplate, sizeof(tagTemplate), id, cycleType, 0 /* Needs context id */, omrtime_current_time_millis());
    ...
    if (hasCycleStartInnerStanzas()) {
        writer->formatAndOutput(
            env, 0, "<cycle-start %s intervalms=\"%llu.%03llu\">", tagTemplate, deltaTime / 1000, deltaTime % 1000);
...
void MM_VerboseHandlerOutput::handleCycleContinue(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
...
void MM_VerboseHandlerOutput::handleCycleEnd(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
...
const char* MM_VerboseHandlerOutput::getCurrentCycleType(MM_EnvironmentBase* env)
{
    return getCycleType(env->_cycleState->_type);
}

void MM_VerboseHandlerOutput::handleExclusiveStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    ...
    writer->formatAndOutput(
        env, 0, "<exclusive-start %s intervalms=\"%llu.%03.3llu\">", tagTemplate, deltaTime / 1000, deltaTime % 1000);
    writer->formatAndOutput(env, 1,
        "<response-info timems=\"%llu.%03.3llu\" idlems=\"%llu.%03.3llu\" threads=\"%zu\" lastid=\"%p\" "
        "lastname=\"%s\" />",
...
void MM_VerboseHandlerOutput::handleExclusiveEnd(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    ...
    writer->formatAndOutput(
        env, 0, "<exclusive-end %s durationms=\"%llu.%03llu\" />", tagTemplate, deltaTime / 1000, deltaTime % 1000);
...
void MM_VerboseHandlerOutput::handleSystemGCStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
...
void MM_VerboseHandlerOutput::handleSystemGCEnd(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
...
void MM_VerboseHandlerOutput::handleAllocationFailureStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    ...
    if (gc_policy_gencon == _extensions->configurationOptions._gcPolicy) {
        writer->formatAndOutput(env, 0,
            "<af-start id=\"%zu\" threadId=\"%p\" totalBytesRequested=\"%zu\" %s intervalms=\"%llu.%03llu\" "
            "type=\"%s\" %s",
            manager->getIdAndIncrement(), event->currentThread, event->requestedBytes, tagTemplate, deltaTime / 1000,
            deltaTime % 1000, event->tenure ? "tenure" : "nursery", endOfTag);
    } else {
        writer->formatAndOutput(env, 0,
            "<af-start id=\"%zu\" threadId=\"%p\" totalBytesRequested=\"%zu\" %s intervalms=\"%llu.%03llu\" %s",
            manager->getIdAndIncrement(), event->currentThread, event->requestedBytes, tagTemplate, deltaTime / 1000,
            deltaTime % 1000, endOfTag);
    }
    ...
    J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    ...
    if (TRUE == event->succeeded) {
        writer->formatAndOutput(env, 0, "<allocation-satisfied id=\"%zu\" threadId=\"%p\" bytesRequested=\"%zu\" />",
            id, currentThread->_language_vmthread, bytesRequested, tagTemplate);
    } else {
        writer->formatAndOutput(env, 0, "<allocation-unsatisfied id=\"%zu\" threadId=\"%p\" bytesRequested=\"%zu\" />",
            id, currentThread->_language_vmthread, bytesRequested, tagTemplate);
    }
    writer->flush(env);
    exitAtomicReportingBlock();
...
void MM_VerboseHandlerOutput::handleAllocationFailureEnd(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    ...
    if ((gc_policy_gencon == _extensions->configurationOptions._gcPolicy) && succeeded) {
        ...
        writer->formatAndOutput(
            env, 0, "<af-end %s threadId=\"%p\" success=\"%s\" />", tagTemplate, event->currentThread, successString);
    }
...
void MM_VerboseHandlerOutput::outputMemoryInfo(
...
void MM_VerboseHandlerOutput::printAllocationStats(MM_EnvironmentBase* env)
{
    ...
    if (0 != _extensions->bytesAllocatedMost) {
        ...
        writer->formatAndOutput(env, 1, "<largest-consumer threadName=\"%s%s\" threadId=\"%p\" bytes=\"%zu\" />",
            escapedThreadName, dots, threadID, _extensions->bytesAllocatedMost);
    }
...
void MM_VerboseHandlerOutput::handleGCStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
...
void MM_VerboseHandlerOutput::handleGCEnd(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    ...
    /* convert from nanoseconds to microseconds, compiler should autocast lvalue (int64_t) to rvalue (uint64_t) but just
     * incase */
    ...
    getTagTemplateWithDuration(tagTemplate, sizeof(tagTemplate), _manager->getIdAndIncrement(),
        getCurrentCycleType(env), env->_cycleState->_verboseContextID, durationInMicroseconds, userTimeInMicroseconds,
        systemTimeInMicroseconds, omrtime_current_time_millis());
...
void MM_VerboseHandlerOutput::handleConcurrentStart(J9HookInterface** hook, UDATA eventNum, void* eventData)
{
...
void MM_VerboseHandlerOutput::handleConcurrentEndInternal(J9HookInterface** hook, UDATA eventNum, void* eventData)
{
    ...
    if (stats->isTerminationRequested()) {
        ...
        writer->formatAndOutput(
            env, 0, "<warning details=\"%s\" />", reasonForTermination, _extensions->gcExclusiveAccessThreadId);
    }
}

void MM_VerboseHandlerOutput::handleConcurrentEnd(J9HookInterface** hook, UDATA eventNum, void* eventData)
{
    MM_ConcurrentPhaseEndEvent* event = (MM_ConcurrentPhaseEndEvent*)eventData;
    MM_ConcurrentPhaseStatsBase* stats = (MM_ConcurrentPhaseStatsBase*)event->concurrentStats;
    MM_VerboseWriterChain* writer = _manager->getWriterChain();
...
void MM_VerboseHandlerOutput::handleHeapResize(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    ...
    enterAtomicReportingBlock();
    outputHeapResizeInfo(env, _manager->getIndentLevel(), resizeType, resizeAmount, resizeCount, subSpaceType, reason,
        timeInMicroSeconds);
    exitAtomicReportingBlock();
}

void MM_VerboseHandlerOutput::outputHeapResizeInfo(MM_EnvironmentBase* env, uintptr_t indent, HeapResizeType resizeType,
    ...
    uint64_t timeInMicroSeconds)
{
    ...
    writer->formatAndOutput(env, indent,
        "<heap-resize id=\"%zu\" type=\"%s\" space=\"%s\" amount=\"%zu\" count=\"%zu\" timems=\"%llu.%03llu\" "
        "reason=\"%s\" %s />",
...
void MM_VerboseHandlerOutput::outputCollectorHeapResizeInfo(MM_EnvironmentBase* env, uintptr_t indent,
    HeapResizeType resizeType, uintptr_t resizeAmount, uintptr_t resizeCount, uintptr_t subSpaceType, uintptr_t reason,
    uint64_t timeInMicroSeconds)
{
    OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
    MM_VerboseWriterChain* writer = _manager->getWriterChain();
    ...
    writer->formatAndOutput(env, indent,
        "<heap-resize type=\"%s\" space=\"%s\" amount=\"%zu\" count=\"%zu\" timems=\"%llu.%03llu\" reason=\"%s\" />",
...
void MM_VerboseHandlerOutput::handleExcessiveGCRaised(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    ...
    case excessive_gc_aggressive:
        writer->formatAndOutput(
            env, indentLevel + 1, "<warning details=\"excessive gc activity detected, will attempt aggressive gc\" />");
        break;
    case excessive_gc_fatal:
    case excessive_gc_fatal_consumed:
        writer->formatAndOutput(
            env, indentLevel + 1, "<warning details=\"excessive gc activity detected, will fail on allocate\" />");
        break;
    default:
        writer->formatAndOutput(env, indentLevel + 1,
            "<warning details=\"excessive gc activity detected, unknown level: %d \" />", event->excessiveLevel);
        break;
    }
...
void MM_VerboseHandlerOutput::outputStringConstantInfo(
...
void verboseHandlerInitialized(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
    ((MM_VerboseHandlerOutput*)userData)->handleInitialized(hook, eventNum, eventData);
}

void verboseHandlerHeapResize(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
