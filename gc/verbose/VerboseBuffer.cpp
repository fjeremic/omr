 ...
 */
MM_VerboseBuffer *MM_VerboseBuffer::newInstance(MM_EnvironmentBase *env,
                                                uintptr_t size) {
  MM_GCExtensionsBase *extensions =
      MM_GCExtensionsBase::getExtensions(env->getOmrVM());

  MM_VerboseBuffer *verboseBuffer =
      (MM_VerboseBuffer *)extensions->getForge()->allocate(
          sizeof(MM_VerboseBuffer), OMR::GC::AllocationCategory::DIAGNOSTIC,
          OMR_GET_CALLSITE());
 ...
 * @param size Buffer size
 */
bool MM_VerboseBuffer::initialize(MM_EnvironmentBase *env, uintptr_t size) {
  MM_GCExtensionsBase *extensions =
      MM_GCExtensionsBase::getExtensions(env->getOmrVM());
  ...
  if (NULL == (_buffer = (char *)extensions->getForge()->allocate(
                   size, OMR::GC::AllocationCategory::DIAGNOSTIC,
                   OMR_GET_CALLSITE()))) {
    return false;
  }
 ...
 */
void MM_VerboseBuffer::kill(MM_EnvironmentBase *env) {
  tearDown(env);
 ...
 */
void MM_VerboseBuffer::tearDown(MM_EnvironmentBase *env) {
  if (NULL != _buffer) {
    MM_GCExtensionsBase *extensions =
        MM_GCExtensionsBase::getExtensions(env->getOmrVM());
 ...
 */
bool MM_VerboseBuffer::add(MM_EnvironmentBase *env, const char *string) {
  bool result = true;
  uintptr_t stringLength = strlen(string);
  /* we will need space for the string but also ensure that we aren't going to
   * overrun the buffer with the NUL byte */
...
bool MM_VerboseBuffer::ensureCapacity(MM_EnvironmentBase *env,
                                      uintptr_t spaceNeeded) {
  MM_GCExtensionsBase *extensions =
      MM_GCExtensionsBase::getExtensions(env->getOmrVM());
  bool result = true;

  if (freeSpace() < spaceNeeded) {
    /* Not enough space in the current buffer - try to alloc a larger one and
     * use that */
...
bool MM_VerboseBuffer::vprintf(MM_EnvironmentBase *env, const char *format,
                               va_list args) {
  ...
  uintptr_t spaceUsed =
      omrstr_vprintf(_bufferAlloc, spaceFree, format, argsCopy);
  ...
  if ((spaceUsed + 1) < spaceFree) {
    ...
    /* undo anything that might have been written by the failed call to
     * omrstr_vprintf */
    _bufferAlloc[0] = '\0';
