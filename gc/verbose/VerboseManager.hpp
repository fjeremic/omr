...
protected:
  MM_VerboseWriterChain
      *_writerChain; /**< The chain of writers for new verbose */
  MM_VerboseHandlerOutput
      *_verboseHandlerOutput; /**< New verbose format output handler */
...
protected:
  ...
  virtual MM_VerboseWriter *createWriter(MM_EnvironmentBase *env,
                                         WriterType type, char *filename,
                                         uintptr_t fileCount,
                                         uintptr_t iterations);

  /**
   * Create the output handler specific to the kind of collector currently
   * running.
   * @param env[in] The master GC thread
   * @return An instance of a sub-class of the MM_VerboseHandlerOutput abstract
   * class which can handle output of verbose data for the collector currently
   * running
   */
   ...
   * @param iterations Number of stanza markers to allow per file before moving
   * to the next file
   * @return WriterType representing expected output mechanism.
   */
  virtual WriterType parseWriterType(MM_EnvironmentBase *env, char *filename,
...
public:
  ...
  MM_VerboseManager(OMR_VM *omrVM)
      : MM_VerboseManagerBase(omrVM), _writerChain(NULL),
