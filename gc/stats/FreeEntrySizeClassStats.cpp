...
bool MM_FreeEntrySizeClassStats::initialize(MM_EnvironmentBase *env,
                                            uintptr_t maxAllocateSizes,
  ...
  if (0 != _maxSizeClasses) {
    _count = (uintptr_t *)env->getForge()->allocate(
        sizeof(uintptr_t) * _maxSizeClasses, OMR::GC::AllocationCategory::FIXED,
        OMR_GET_CALLSITE());
    ...
    /* _maxFrequentAllocateSizes is set to 0 when we use this structure to
     * gather TLH allocation profile, which has no need for frequent allocations
     */
    if (0 != _maxFrequentAllocateSizes) {
      _frequentAllocationHead =
          (FrequentAllocation **)env->getForge()->allocate(
              sizeof(FrequentAllocation *) * _maxSizeClasses,
      ...
      _frequentAllocation = (FrequentAllocation *)env->getForge()->allocate(
          sizeof(FrequentAllocation) *
              MAX_FREE_ENTRY_COUNTERS_PER_FREQ_ALLOC_SIZE *
      ...
      if (simulation) {
        _fractionFrequentAllocation = (float *)env->getForge()->allocate(
            sizeof(float) * _maxFrequentAllocateSizes,
            OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
      ...
      if (veryLargeObjectThreshold > env->getExtensions()->memoryMax) {
         ...
         * VERY_LARGE_ENTRY_POOL_SIZE_FOR_THREAD for thread base
        ...
        _veryLargeEntryPool = (FrequentAllocation *)env->getForge()->allocate(
            sizeof(FrequentAllocation) * count,
            OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    ...
    if (!_lock.initialize(env, &env->getExtensions()->lnrlOptions,
                          "MM_FreeEntrySizeClassStats:_lock")) {
      return false;
    }
  }
...
void MM_FreeEntrySizeClassStats::tearDown(MM_EnvironmentBase *env) {
  if (NULL != _count) {
...
void MM_FreeEntrySizeClassStats::mergeCountForVeryLargeEntries() {
  if (NULL != _frequentAllocationHead) {
    for (uintptr_t sizeClassIndex = _veryLargeEntrySizeClass;
         sizeClassIndex < _maxSizeClasses; sizeClassIndex++) {
...
void MM_FreeEntrySizeClassStats::merge(MM_FreeEntrySizeClassStats *stats) {
  Assert_MM_true(stats->_maxSizeClasses <= _maxSizeClasses);
  for (uintptr_t sizeClassIndex = 0; sizeClassIndex < stats->_maxSizeClasses;
       sizeClassIndex++) {
    ...
    if (NULL != _frequentAllocationHead) {
      if (sizeClassIndex >= _veryLargeEntrySizeClass) {
        if (NULL != stats->_frequentAllocationHead[sizeClassIndex]) {
          ...
          FrequentAllocation *src =
              stats->_frequentAllocationHead[sizeClassIndex];
...
uintptr_t MM_FreeEntrySizeClassStats::getFreeMemory(
    const uintptr_t sizeClassSizes[]) {
  uintptr_t totalFreeMemory = 0;

  for (uintptr_t sizeClassIndex = 0; sizeClassIndex < _maxSizeClasses;
       sizeClassIndex++) {
    /* regular sizes */
...
uintptr_t MM_FreeEntrySizeClassStats::getPageAlignedFreeMemory(
    const uintptr_t sizeClassSizes[], uintptr_t pageSize) {
  ...
  for (uintptr_t sizeClassIndex = 0; sizeClassIndex < _maxSizeClasses;
    ...
    if (pageSize < sizeClassSizes[sizeClassIndex]) {
      /* regular sizes */
      resusableFreeMemory +=
          _count[sizeClassIndex] * (sizeClassSizes[sizeClassIndex] - pageSize);

      /*
       * We need to find how many pages of specified size, would fit into free
       * entries. Since we don't have exact info about the start and end of each
       * free entry (we only have low bound size and count of each entry) we use
       * a simple heuristic that freeEntrySize - pageSize of bytes, in average
       * will fit. For example if free entry is 6K and page is 4K, in average 2K
       * will be reusable (50% chances it will be 1 full page, and 50% no page
       * will fit).
       */
...
uintptr_t MM_FreeEntrySizeClassStats::copyTo(MM_FreeEntrySizeClassStats *stats,
                                             const uintptr_t sizeClassSizes[]) {
  uintptr_t totalFreeMemory = 0;
    ...
    if (NULL != _frequentAllocationHead) {
      ...
      while (NULL != curr) {
        ...
        if (sizeClassIndex >= _veryLargeEntrySizeClass) {
          ...
          Assert_MM_true(stats->_frequentAllocateSizeCounters <
                         maxFrequentAllocateSizeCounters);
          statsCurr =
              &stats->_frequentAllocation[stats->_frequentAllocateSizeCounters];
...
void MM_FreeEntrySizeClassStats::resetCounts() {
  for (uintptr_t sizeClassIndex = 0; sizeClassIndex < _maxSizeClasses;
       sizeClassIndex++) {
...
void MM_FreeEntrySizeClassStats::clearFrequentAllocation() {
  if (0 != _maxFrequentAllocateSizes) {
    for (uintptr_t sizeClassIndex = 0; sizeClassIndex < _maxSizeClasses;
         sizeClassIndex++) {
      if (sizeClassIndex < _veryLargeEntrySizeClass) {
...
void MM_FreeEntrySizeClassStats::initializeVeryLargeEntryPool() {
  if (0 != _maxFrequentAllocateSizes) {
    for (uintptr_t sizeClassIndex = _veryLargeEntrySizeClass;
         sizeClassIndex < _maxSizeClasses; sizeClassIndex++) {
      _frequentAllocationHead[sizeClassIndex] = NULL;
...
void MM_FreeEntrySizeClassStats::initializeFrequentAllocation(
    MM_LargeObjectAllocateStats *largeObjectAllocateStats) {
  clearFrequentAllocation();

  const uintptr_t maxFrequentAllocateSizeCounters =
      MAX_FREE_ENTRY_COUNTERS_PER_FREQ_ALLOC_SIZE * _maxFrequentAllocateSizes;
  uintptr_t maxFrequentAllocateSizes = OMR_MIN(
      _maxFrequentAllocateSizes,
      spaceSavingGetCurSize(
          largeObjectAllocateStats->getSpaceSavingSizesAveragePercent()));
  ...
  for (uintptr_t i = 0; i < maxFrequentAllocateSizes; i++) {
    ...
    uintptr_t maxFactor = OMR_MIN(MAX_FREE_ENTRY_COUNTERS_PER_FREQ_ALLOC_SIZE,
                                  (maxHeapSize / frequentAllocSize));

    /* insert this size and a next few multiples (set as maxFactor) of this size
     */
    uintptr_t size = frequentAllocSize;
    for (uintptr_t factor = 1; factor <= maxFactor;
         factor++, size += frequentAllocSize) {
      /* follow the chain of frequent allocation sizes for this sizeClassIndex
       * insert this size at appropriate place (the list is in increasing order)
       */

      uintptr_t sizeClassIndex =
          largeObjectAllocateStats->getSizeClassIndex(size);
      if (sizeClassIndex >= _veryLargeEntrySizeClass) {
        continue;
      }
      ...
      if ((NULL == curr) || (size != curr->_size)) {
        ...
        Assert_MM_true(_frequentAllocateSizeCounters <
                       maxFrequentAllocateSizeCounters);
        ...
        if (NULL == prev) {
          ...
          Assert_MM_true(
              _frequentAllocation[_frequentAllocateSizeCounters]._size >
              prev->_size);
          prev->_nextInSizeClass =
              &_frequentAllocation[_frequentAllocateSizeCounters];
        }

        if (NULL == curr) {
          /* last one in the list */
          _frequentAllocation[_frequentAllocateSizeCounters]._nextInSizeClass =
              NULL;
        } else {
...
uintptr_t MM_FreeEntrySizeClassStats::getFrequentAllocCount(
    uintptr_t sizeClassIndex) {
