...
bool MM_FreeEntrySizeClassStats::initialize(MM_EnvironmentBase* env, uintptr_t maxAllocateSizes,
    uintptr_t maxSizeClasses, uintptr_t veryLargeObjectThreshold, uintptr_t factorVeryLargeEntryPool, bool simulation)
{
    ...
    if (0 != _maxSizeClasses) {
        ...
        /* _maxFrequentAllocateSizes is set to 0 when we use this structure to gather TLH allocation profile, which has
         * no need for frequent allocations */
        if (0 != _maxFrequentAllocateSizes) {
            _frequentAllocationHead = (FrequentAllocation**)env->getForge()->allocate(
                sizeof(FrequentAllocation*) * _maxSizeClasses, OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
            ...
            if (simulation) {
                _fractionFrequentAllocation = (float*)env->getForge()->allocate(
                    sizeof(float) * _maxFrequentAllocateSizes, OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
            ...
            if (veryLargeObjectThreshold > env->getExtensions()->memoryMax) {
                ...
                /* if veryLargeObjectThreshold == 0, set veryLargePool size = VERY_LARGE_ENTRY_POOL_SIZE_FOR_THREAD for
                 * thread base FreeEntrySizeClassStats */
                ...
                _veryLargeEntryPool = (FrequentAllocation*)env->getForge()->allocate(
                    sizeof(FrequentAllocation) * count, OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
uintptr_t MM_FreeEntrySizeClassStats::getFreeMemory(const uintptr_t sizeClassSizes[])
{
...
uintptr_t MM_FreeEntrySizeClassStats::getPageAlignedFreeMemory(const uintptr_t sizeClassSizes[], uintptr_t pageSize)
{
    ...
    for (uintptr_t sizeClassIndex = 0; sizeClassIndex < _maxSizeClasses; sizeClassIndex++) {
        ...
        if (pageSize < sizeClassSizes[sizeClassIndex]) {
             ...
             * Since we don't have exact info about the start and end of each free entry (we only have low bound size
             * and count of each entry) we use a simple heuristic that freeEntrySize - pageSize of bytes, in average
             * will fit. For example if free entry is 6K and page is 4K, in average 2K will be reusable (50% chances it
             * will be 1 full page, and 50% no page will fit).
             */
...
uintptr_t MM_FreeEntrySizeClassStats::copyTo(MM_FreeEntrySizeClassStats* stats, const uintptr_t sizeClassSizes[])
{
    uintptr_t totalFreeMemory = 0;
...
MM_FreeEntrySizeClassStats::FrequentAllocation* MM_FreeEntrySizeClassStats::copyVeryLargeEntry(
    FrequentAllocation* entry)
{
...
void MM_FreeEntrySizeClassStats::initializeFrequentAllocation(MM_LargeObjectAllocateStats* largeObjectAllocateStats)
{
    clearFrequentAllocation();
    ...
    for (uintptr_t i = 0; i < maxFrequentAllocateSizes; i++) {
        ...
        for (uintptr_t factor = 1; factor <= maxFactor; factor++, size += frequentAllocSize) {
            /* follow the chain of frequent allocation sizes for this sizeClassIndex insert this size at appropriate
             * place (the list is in increasing order) */
...
uintptr_t MM_FreeEntrySizeClassStats::getFrequentAllocCount(uintptr_t sizeClassIndex)
{
