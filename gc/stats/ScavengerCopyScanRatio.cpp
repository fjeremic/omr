...
void MM_ScavengerCopyScanRatio::reset(MM_EnvironmentBase *env,
                                      bool resetHistory) {
...
uintptr_t MM_ScavengerCopyScanRatio::record(MM_EnvironmentBase *env,
  ...
  if (SCAVENGER_UPDATE_HISTORY_SIZE <= _historyTableIndex) {
    ...
    /* table full -- sum adjacent pairs of records and shift results to top half
     * of table */
    ...
    uintptr_t zeroBytes =
        (SCAVENGER_UPDATE_HISTORY_SIZE >> 1) * sizeof(UpdateHistory);
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
  /* record current read barries values (we do not want to sum them up and
   * average, we want last value) */
  MM_GCExtensionsBase *ext = env->getExtensions();
  historyRecord->readObjectBarrierUpdate =
      ext->scavengerStats._readObjectBarrierUpdate;
  historyRecord->readObjectBarrierCopy =
      ext->scavengerStats._readObjectBarrierCopy;
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
  historyRecord->time = omrtime_hires_clock();

  /* advance table index if current record is maxed out */
  if (historyRecord->updates >=
      (_historyFoldingFactor * SCAVENGER_THREAD_UPDATES_PER_MAJOR_UPDATE)) {
    _historyTableIndex += 1;
  }
  return threadCount;
}

uint64_t MM_ScavengerCopyScanRatio::getSpannedMicros(
    MM_EnvironmentBase *env, UpdateHistory *historyRecord) {
  OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
  uint64_t start = (historyRecord == _historyTable) ? _resetTimestamp
                                                    : (historyRecord - 1)->time;
  return omrtime_hires_delta(start, historyRecord->time,
                             OMRPORT_TIME_DELTA_IN_MICROSECONDS);
}

void MM_ScavengerCopyScanRatio::failedUpdate(MM_EnvironmentBase *env,
                                             uint64_t copied,
                                             uint64_t scanned) {
  Assert_GC_true_with_message2(
      env, copied <= scanned,
      "MM_ScavengerCopyScanRatio::getScalingFactor(): copied (=%llu) exceeds "
      "scanned (=%llu) -- non-atomic 64-bit read\n",
      copied, scanned);
}
