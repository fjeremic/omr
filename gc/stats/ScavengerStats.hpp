...
public:
    struct FlipHistory {
        ...
        /* Array sizes are OBJECT_HEADER_AGE_MAX + 2 because we want to include age 0 through max as well as age -1
         * (never flipped) */
        uintptr_t
            _flipBytes[OBJECT_HEADER_AGE_MAX + 2]; /**< The historical number of bytes flipped in each age group */
        uintptr_t
            _tenureBytes[OBJECT_HEADER_AGE_MAX + 2]; /**< The historical number of bytes tenured in each age group */
    };
    ...
    uintptr_t _acquireListLockCount; /**< cumulative (for scan&free list) lock count. if this number is much larger than
                                        cumulative acquire list count, it indicates over-splitting */
    ...
    uint64_t _completeStallTime; /**< The time, in hi-res ticks, the thread spent stalled waiting for all other threads
                                    to complete working */
    uint64_t _syncStallTime; /**< The time, in hi-res ticks, the thread spent stalled at a sync point */
    uintptr_t _totalDeepStructures; /**<  The number of deep structures that are scanned with priority (number of
                                       deepScanOutline function calls) */
    uintptr_t _totalObjsDeepScanned; /**< The total number of deep structure objects that are special treated (number of
                                        copyAndForward with priority)*/
    ...
    uintptr_t _semiSpaceAllocBytesAcumulation; /**< Bytes allocated in new space between scavenges, updated on global
                                                  collects. */
    uintptr_t _tenureSpaceAllocBytesAcumulation; /**< Bytes allocated in tenure space between scavenges, updated on
                                                    global collects. */
    ...
    uintptr_t
        _tenureExpandedCount; /**< The number of times the heap was expanded in order to complete the collection */
    uint64_t
        _tenureExpandedTime; /**< Time taken expanding the heap in order to complete the collection, in hi-res ticks */
    ...
    uint64_t
        _slotsCopied; /**< The number of slots copied by the thread since _slotsScanned was last sampled and reset */
    uint64_t
        _slotsScanned; /**< The number of slots scanned by the thread since _slotsCopied was last sampled and reset */

#if defined(OMR_GC_CONCURRENT_SCAVENGER)
    uint64_t _readObjectBarrierCopy; /**< Number of objects copied by read barrier */
    uint64_t _readObjectBarrierUpdate; /**< Number of reference slots updates, which may be (often is) preceded by
                                          object copy */
...
public:
     ...
     */
    MMINLINE uint64_t getStallTime() { return _workStallTime + _completeStallTime + _syncStallTime; }
#endif /* J9MODRON_TGC_PARALLEL_STATISTICS */
    ...
    MMINLINE void countCopyCacheSize(uint64_t copyCacheSize, uint64_t copyCacheSizeMax)
    {
    ...
    /**
     * @return true if at least one full Scavenge cycle is complete (stats are calculated once, at the end of each
     * cycle)
     */
