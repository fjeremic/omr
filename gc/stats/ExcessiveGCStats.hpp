...
/**
 * Storage for statistics tracking all collections that have occurred in the
 * system.
...
public:
   ...
   * @{
   */
  uint64_t startGCTimeStamp; /**< Raw timestamp (no units) of the start of the
                                last GC */
  uint64_t
      endGCTimeStamp; /**< Raw timestamp (no units) of the end of the last GC */
  uintptr_t
      freeMemorySizeBefore; /**< The amount of free memory before the last GC */
  uintptr_t
  ...
  uint64_t lastEndGlobalGCTimeStamp; /**< Raw timestamp (no units) of the end of
                                        the last global GC */
  /** @} */

  MM_ExcessiveGCStats()
      : MM_Base(), gcCount(0), startGCTimeStamp(0), endGCTimeStamp(0),
        freeMemorySizeBefore(0), freeMemorySizeAfter(0),
        newGCToUserTimeRatio(0.0), avgGCToUserTimeRatio(0.0), totalGCTime(0),
