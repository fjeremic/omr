...
public:
  ...
  uintptr_t _totalNurseryHeapSize;    /**< Total active nursery (survivor +
                                         allocate) heap size */
  uintptr_t
      _totalFreeNurseryHeapSize;    /**< Total active free nursery heap size */
  uintptr_t _totalSurvivorHeapSize; /**< Total active survivor heap size */
  uintptr_t
      _totalFreeSurvivorHeapSize; /**< Total active free survivor heap size */
  uintptr_t _rememberedSetCount;  /**< remembered set count */
  uint32_t
      _tenureFragmentation;       /**< fragmentation indicator, can be
                                     NO_FRAGMENTATION, MICRO_FRAGMENTATION,
                                     MACRO_FRAGMENTATION, indicate if fragmentation
                                     info are ready in _microFragmentedSize and
                                     _macroFragmentedSize */
...
public:
  ...
  collectCollectionStatistics(MM_EnvironmentBase *env,
    ...
    if (stats->_scavengerEnabled) {
      ...
      stats->_totalSurvivorHeapSize =
          extensions->heap->getActiveSurvivorMemorySize(MEMORY_TYPE_NEW);
      stats->_totalFreeSurvivorHeapSize =
          extensions->heap->getApproximateActiveFreeSurvivorMemorySize(
              MEMORY_TYPE_NEW);
    ...
    if (NO_FRAGMENTATION != _tenureFragmentation) {
      ...
      if (MACRO_FRAGMENTATION == (MACRO_FRAGMENTATION & _tenureFragmentation)) {
        ...
        stats->_macroFragmentedSize =
            allocateStats->getRemainingFreeMemoryAfterEstimate();
      }
    } else {
  ...
  MM_CollectionStatisticsStandard()
      : MM_CollectionStatistics(), _totalTenureHeapSize(0),
        _totalFreeTenureHeapSize(0), _loaEnabled(false), _totalLOAHeapSize(0),
        _totalFreeLOAHeapSize(0), _scavengerEnabled(false),
        _totalNurseryHeapSize(0), _totalFreeNurseryHeapSize(0),
        _totalSurvivorHeapSize(0), _totalFreeSurvivorHeapSize(0),
        _rememberedSetCount(0), _tenureFragmentation(NO_FRAGMENTATION),
        _microFragmentedSize(0), _macroFragmentedSize(0){};
};
