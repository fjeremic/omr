...
MM_LargeObjectAllocateStats::newInstance(MM_EnvironmentBase* env,
                                         ...
                                         uintptr_t factorVeryLargeEntryPool)
{
  ...
  if (NULL != largeObjectAllocateStats) {
    ...
    if (!largeObjectAllocateStats->initialize(env,
                                              maxAllocateSizes,
...
bool
MM_LargeObjectAllocateStats::initialize(MM_EnvironmentBase* env,
                                        uint16_t maxAllocateSizes,
                                        uintptr_t largeObjectThreshold,
                                        uintptr_t veryLargeObjectThreshold,
                                        float sizeClassRatio,
                                        ...
                                        uintptr_t factorVeryLargeEntryPool)
{
  ...
  if (!_tlhAllocSizeClassStats.initialize(
        env, 0, maxTLHSizeClasses, UDATA_MAX)) {
    return false;
  }
#endif
...
MM_LargeObjectAllocateStats::allocateObject(uintptr_t allocateSize)
{
  ...
  if (allocateSize >= _largeObjectThreshold) {
     ...
     * size class itself. */
    uintptr_t sizeClass = (uintptr_t)(
      pow(_sizeClassRatio,
          (float)ceil(log((float)allocateSize) / _sizeClassRatioLog)));
    spaceSavingUpdate(_spaceSavingSizeClasses, (void*)sizeClass, sizeClass);
  }
}

void
MM_LargeObjectAllocateStats::mergeCurrent(
...
void
MM_LargeObjectAllocateStats::mergeAverage(
  MM_LargeObjectAllocateStats* statsToMerge)
{
  uintptr_t i = 0;
...
uintptr_t
MM_LargeObjectAllocateStats::upSampleAllocStats(MM_EnvironmentBase* env,
                                                uintptr_t allocSize,
                                                uintptr_t bytesAllocated)
{
  ...
  if (allocSize < _tlhMaximumSize) {
    ...
    for (uintptr_t sizeClassIndex = tlhMinimumSizeClassIndex;
         sizeClassIndex <= tlhMaximumSizeClassIndex;
         sizeClassIndex++) {
    ...
    Trc_MM_LargeObjectAllocateStats_upSample(
      ...
      allocSize,
      bytesAllocated,
      (uintptr_t)thisSizeTlhBytesAllocated,
      totalTlhBytesAllocated,
      upSampleRatio,
      bytesAllocatedUpSampled);
  }
#endif
...
MM_LargeObjectAllocateStats::estimateFragmentation(MM_EnvironmentBase* env)
{
  ...
  for (; (0 == unsatisfiedAlloc) &&
    ...
    for (uintptr_t i = 0;
         (i < maxFrequentAllocateSizes) && (0 == unsatisfiedAlloc);
         i++) {
      float objectPercent = convertPercentUDATAToFloat(
        spaceSavingGetKthMostFreqCount(_spaceSavingSizesAveragePercent, i + 1));
      ...
      if (objectPercent >= ignoreObjectPercentThreshold) {
        ...
        if (0 != tlhAllocBytesPerObjectPerObjectStride) {
          unsatisfiedAlloc =
            simulateAllocateTLHs(env,
                                 tlhAllocBytesPerObjectPerObjectStride,
                                 &currentFreeMemory,
                                 allocTLHStridesInteger);
...
uintptr_t
MM_LargeObjectAllocateStats::simulateAllocateObjects(
  ...
  uintptr_t* currentFreeMemory)
{
  ...
  while (
    (allocBytes > 0) &&
    (sizeClassIndex < ext->freeEntrySizeClassStatsSimulated._maxSizeClasses)) {
      ...
      while ((allocBytes > 0) && (NULL != curr)) {
        ...
        if (availableBytes <= allocBytes) {
          ...
          *currentFreeMemory -= curr->_count * currentSize;
          ...
          Trc_MM_LargeObjectAllocateStats_simulateAllocateCommon_full(
            env->getLanguageVMThread(),
            curr->_count - (uintptr_t)freeEntriesUsed,
            curr->_count,
            freeEntriesUsed);
...
uintptr_t
MM_LargeObjectAllocateStats::simulateAllocateTLHs(MM_EnvironmentBase* env,
                                                  uintptr_t allocBytes,
                                                  uintptr_t* currentFreeMemory,
                                                  uintptr_t strides)
{
    ...
    if ((allocBytesForCurrentStride > 0) &&
      ...
      while ((allocBytesForCurrentStride > 0) && (NULL != curr)) {
        ...
        */
        ...
        uintptr_t remainderSize =
          freeEntrySize - (uintptr_t)(freeEntriesUsedFractional * currentSize);
...
void
MM_LargeObjectAllocateStats::averageForSpaceSaving(
  MM_EnvironmentBase* env,
...
void
MM_LargeObjectAllocateStats::average(MM_EnvironmentBase* env,
                                     uintptr_t bytesAllocatedThisRound)
{
  averageForSpaceSaving(env,
                        _spaceSavingSizes,
                        &_spaceSavingSizesAveragePercent,
                        bytesAllocatedThisRound);
  averageForSpaceSaving(env,
...
MMINLINE uintptr_t
MM_LargeObjectAllocateStats::updateFreeEntrySizeClassStats(
  uintptr_t freeEntrySize,
...
uintptr_t
MM_LargeObjectAllocateStats::incrementFreeEntrySizeClassStats(
  uintptr_t freeEntrySize,
  MM_FreeEntrySizeClassStats* freeEntrySizeClassStats,
  uintptr_t count)
{
  ...
  return updateFreeEntrySizeClassStats(freeEntrySize,
                                       freeEntrySizeClassStats,
                                       count,
                                       sizeClassIndex,
                                       prevFrequentAllocation,
                                       frequentAllocation);
}
...
MM_LargeObjectAllocateStats::incrementFreeEntrySizeClassStats(
  uintptr_t freeEntrySize)
{
  return incrementFreeEntrySizeClassStats(
    freeEntrySize, &_freeEntrySizeClassStats, 1);
}

void
MM_LargeObjectAllocateStats::incrementTlhAllocSizeClassStats(
  uintptr_t freeEntrySize)
{
#if defined(OMR_GC_THREAD_LOCAL_HEAP)
  uintptr_t sizeClassIndex = getSizeClassIndex(freeEntrySize);
...
void
MM_LargeObjectAllocateStats::decrementFreeEntrySizeClassStats(
  uintptr_t freeEntrySize)
{
  return decrementFreeEntrySizeClassStats(
    freeEntrySize, &_freeEntrySizeClassStats, 1);
}
  ...
  uintptr_t count)
{
  ...
  updateFreeEntrySizeClassStats(freeEntrySize,
                                freeEntrySizeClassStats,
                                -((intptr_t)count),
                                sizeClassIndex,
                                prevFrequentAllocation,
...
MM_LargeObjectAllocateStats::getSizeClassIndex(uintptr_t size)
{
  ...
  Assert_GC_true_with_message2(
    ...
    "maximum %zu\n",
    result,
    _freeEntrySizeClassStats._maxSizeClasses);

  return result;
}

void
MM_LargeObjectAllocateStats::verifyFreeEntryCount(
  uintptr_t actualFreeEntryCount)
{
  uintptr_t totalCount = 0;
  for (intptr_t sizeClassIndex = 0;
       sizeClassIndex < (intptr_t)getMaxSizeClasses();
       sizeClassIndex++) {
