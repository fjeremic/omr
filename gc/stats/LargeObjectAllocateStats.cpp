...
MM_LargeObjectAllocateStats* MM_LargeObjectAllocateStats::newInstance(MM_EnvironmentBase* env,
    uint16_t maxAllocateSizes, uintptr_t largeObjectThreshold, uintptr_t veryLargeObjectThreshold, float sizeClassRatio,
    uintptr_t maxHeapSize, uintptr_t tlhMaximumSize, uintptr_t tlhMinimumSize, uintptr_t factorVeryLargeEntryPool)
{
    ...
    largeObjectAllocateStats = (MM_LargeObjectAllocateStats*)env->getForge()->allocate(
        sizeof(MM_LargeObjectAllocateStats), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
    ...
    if (NULL != largeObjectAllocateStats) {
        ...
        if (!largeObjectAllocateStats->initialize(env, maxAllocateSizes, largeObjectThreshold, veryLargeObjectThreshold,
                sizeClassRatio, maxHeapSize, tlhMaximumSize, tlhMinimumSize, factorVeryLargeEntryPool)) {
...
bool MM_LargeObjectAllocateStats::initialize(MM_EnvironmentBase* env, uint16_t maxAllocateSizes,
    uintptr_t largeObjectThreshold, uintptr_t veryLargeObjectThreshold, float sizeClassRatio, uintptr_t maxHeapSize,
    uintptr_t tlhMaximumSize, uintptr_t tlhMinimumSize, uintptr_t factorVeryLargeEntryPool)
{
    ...
    if (0 == env->getExtensions()->freeMemoryProfileMaxSizeClasses) {
        ...
        /* to prevent thread race condition -- before largeObjectAllocationProfilingVeryLargeObjectThreshold has been
         * initialized, another thread might use the value, multi-entries is not issue. */
        MM_AtomicOperations::writeBarrier();
        env->getExtensions()->freeMemoryProfileMaxSizeClasses = largestClassSizeIndex + 1;
    }

    if (!_freeEntrySizeClassStats.initialize(env, _maxAllocateSizes,
    ...
    _sizeClassSizes
        = (uintptr_t*)env->getForge()->allocate(sizeof(uintptr_t) * _freeEntrySizeClassStats._maxSizeClasses,
...
void MM_LargeObjectAllocateStats::mergeCurrent(MM_LargeObjectAllocateStats* statsToMerge)
{
...
void MM_LargeObjectAllocateStats::mergeAverage(MM_LargeObjectAllocateStats* statsToMerge)
{
    uintptr_t i = 0;
    ...
    MM_EnvironmentBase* env, uintptr_t allocSize, uintptr_t bytesAllocated)
{
    ...
    if (allocSize < _tlhMaximumSize) {
        ...
        Trc_MM_LargeObjectAllocateStats_upSample(env->getLanguageVMThread(), allocSize, bytesAllocated,
            (uintptr_t)thisSizeTlhBytesAllocated, totalTlhBytesAllocated, upSampleRatio, bytesAllocatedUpSampled);
    }
#endif
...
uintptr_t MM_LargeObjectAllocateStats::convertPercentFloatToUDATA(float percent)
{
    return (uintptr_t)(percent * (float)1000000.0);
}

float MM_LargeObjectAllocateStats::convertPercentUDATAToFloat(uintptr_t percent)
{
    return (float)percent / (float)1000000.0;
}

uintptr_t MM_LargeObjectAllocateStats::estimateFragmentation(MM_EnvironmentBase* env)
{
    ...
    Trc_MM_LargeObjectAllocateStats_estimateFragmentation_entry(
        env->getLanguageVMThread(), initialFreeMemory, initialFreeMemory >> 20, tlhPercent);
     ...
     * could be less, if fragmented. could be more if not fragmented (due to probablistic size decay of very large free
     * entries). stop up until we cannot satisfy allocate or remaining free memory is less than 1% of initial free
     * memory. if we do not make progress (strides are too small), we stop.
     */
    for (; (0 == unsatisfiedAlloc) && (currentFreeMemory > initialFreeMemory / 100)
        ...
        for (uintptr_t i = 0; (i < maxFrequentAllocateSizes) && (0 == unsatisfiedAlloc); i++) {
            ...
            if (objectPercent >= ignoreObjectPercentThreshold) {
                ...
                /* example: for a large object with 30% of all allocates and with 60% of total TLH allocates (10% of
                 * other large object allocations) in this stride we will simulate allocation of 30 / 10 = 3% for the
                 * large object and  30/(30+10) x 60 / 10 = 4.5% (tlhAllocBytesPerObjectPerObjectStride) for TLHs. TLHs
                 * allocations will be further divided into several strides (allocTLHStridesInteger), where each TLH
                 * allocation stride will allocate from different size class.
                 */
                ...
                Trc_MM_LargeObjectAllocateStats_estimateFragmentation_stride(env->getLanguageVMThread(), stride,
                    currentFreeMemory, currentFreeMemory >> 20, objectSize, objectAllocBytesPerObjectStride,
                    objectAllocBytesPerObjectStride >> 20, tlhAllocBytesPerObjectPerObjectStride,
                    tlhAllocBytesPerObjectPerObjectStride >> 20);

                if (0 != tlhAllocBytesPerObjectPerObjectStride) {
                    unsatisfiedAlloc = simulateAllocateTLHs(
                        env, tlhAllocBytesPerObjectPerObjectStride, &currentFreeMemory, allocTLHStridesInteger);
                    if (0 != unsatisfiedAlloc) {
                        break;
                    }
                }

                /* only apply allocInteger(whole number of frequentAllocation) in simulateAllocateObjects, cumulate
                 * fraction part of frequentAllocation for later strides, when the cumulated fraction is bigger than 1,
                 * add the "integer" to allocInteger */
                ...
                if (objectAllocBytesPerObjectStride >= objectSize) {
                    /* TODO: for very large infrequent objects (allocBytes lower (or larger but close to) size), we
                     * should also allocate with probability size/allocBytes */
                    unsatisfiedAlloc
...
uintptr_t MM_LargeObjectAllocateStats::simulateAllocateObjects(
    MM_EnvironmentBase* env, uintptr_t allocBytes, uintptr_t objectSize, uintptr_t* currentFreeMemory)
{
    MM_GCExtensionsBase* ext = env->getExtensions();
    ...
    Trc_MM_LargeObjectAllocateStats_simulateAllocateObjects_entry(env->getLanguageVMThread(), objectSize, allocBytes,
        allocBytes >> 20, *currentFreeMemory, *currentFreeMemory >> 20);
    ...
    while ((allocBytes > 0) && (sizeClassIndex < ext->freeEntrySizeClassStatsSimulated._maxSizeClasses)) {
        ...
        if ((0 != ext->freeEntrySizeClassStatsSimulated._count[sizeClassIndex])
            ...
            Trc_MM_LargeObjectAllocateStats_simulateAllocateCommon_freeEntry_entry(env->getLanguageVMThread(), "Object",
                "regular", sizeClassIndex, _sizeClassSizes[sizeClassIndex],
                ext->freeEntrySizeClassStatsSimulated._count[sizeClassIndex]);

            /* go for smallest to largest sizes within a class size; thus start with the 'regular' size and continue
             * with frequent allocate sizes as ordered in the list (ascending) */
            ...
            Trc_MM_LargeObjectAllocateStats_simulateAllocateObjects_availableBytes(env->getLanguageVMThread(),
                "regular", availableBytes, ext->freeEntrySizeClassStatsSimulated._count[sizeClassIndex],
            ...
            if (availableBytes <= allocBytes) {
                ...
                *currentFreeMemory
                ...
                Trc_MM_LargeObjectAllocateStats_simulateAllocateCommon_full(env->getLanguageVMThread(),
            ...
            */
            ...
            /* we do not know the exact size of free entry(ies). we do know low and hi bounds, and will just approximate
             * it (them) with a random size within the range */
            ...
            Trc_MM_LargeObjectAllocateStats_simulateAllocateCommon_freeEntry_exit(env->getLanguageVMThread(), "Object",
                "regular", sizeClassIndex, ext->freeEntrySizeClassStatsSimulated._count[sizeClassIndex]);
        }
        if (0 != ext->freeEntrySizeClassStatsSimulated.getFrequentAllocCount(sizeClassIndex)) {
            ...
            while ((allocBytes > 0) && (NULL != curr)) {
                ...
                Trc_MM_LargeObjectAllocateStats_simulateAllocateObjects_availableBytes(env->getLanguageVMThread(),
                    "frequent", availableBytes, curr->_count, usedPortionOfFreeEntry, freeEntryOverAllocSizeRatio,
                    objectSize);
                ...
                if (availableBytes <= allocBytes) {
                    ...
                    *currentFreeMemory -= curr->_count * currentSize;
                    ...
                    Trc_MM_LargeObjectAllocateStats_simulateAllocateCommon_full(env->getLanguageVMThread(),
                        curr->_count - (uintptr_t)freeEntriesUsed, curr->_count, freeEntriesUsed);
                ...
                /* there are two kind of remainders:
                 1) freeEntriesUsed of size: freeEntrySize - usedPortionOfFreeEntry
                 2) up to one of size: freeEntrySize - decimal-fraction-of-freeEntriesUsed-float *
                ...
                if (remainderSize < _sizeClassSizes[sizeClassIndex]) {
                    ...
                    if (0 != freeEntriesUsedInteger) {
                        ...
                        if (remainderSize >= _largeObjectThreshold) {
                            uintptr_t updatedFreeEntrySize = incrementFreeEntrySizeClassStats(
                                remainderSize, &ext->freeEntrySizeClassStatsSimulated, freeEntriesUsedInteger);
                            Trc_MM_LargeObjectAllocateStats_simulateAllocateCommon_remainder(env->getLanguageVMThread(),
                                freeEntriesUsedInteger, remainderSize, updatedFreeEntrySize);
                            *currentFreeMemory += updatedFreeEntrySize * freeEntriesUsedInteger;
                        }
                    }
                    ...
                    if (remainderSize < currentSize) {
                        ...
                        updateFreeEntrySizeClassStats(
                            currentSize, &ext->freeEntrySizeClassStatsSimulated, -1, sizeClassIndex, prev, curr);
...
MMINLINE uintptr_t MM_LargeObjectAllocateStats::getNextSizeClass(uintptr_t sizeClassIndex, uintptr_t maxSizeClasses)
{
    return ((sizeClassIndex + 1) % maxSizeClasses);
}

MMINLINE bool MM_LargeObjectAllocateStats::isFirstIterationCompleteForCurrentStride(
...
uintptr_t MM_LargeObjectAllocateStats::simulateAllocateTLHs(
    MM_EnvironmentBase* env, uintptr_t allocBytes, uintptr_t* currentFreeMemory, uintptr_t strides)
{
    MM_GCExtensionsBase* ext = env->getExtensions();
    Assert_MM_true(NULL != ext->freeEntrySizeClassStatsSimulated._frequentAllocationHead);
    ...
    Trc_MM_LargeObjectAllocateStats_simulateAllocateTLHs_entry(
        env->getLanguageVMThread(), allocBytes, allocBytes >> 20, *currentFreeMemory, *currentFreeMemory >> 20);
    /* keep allocating from this size class or any larger */
    while ((allocBytesForCurrentStride > 0)
        ...
        if (0 == _TLHFrequentAllocationSize) {
            ...
            if (0 != ext->freeEntrySizeClassStatsSimulated._count[sizeClassIndex]) {
                Trc_MM_LargeObjectAllocateStats_simulateAllocateCommon_freeEntry_entry(env->getLanguageVMThread(),
                    "TLH", "regular", sizeClassIndex, _sizeClassSizes[sizeClassIndex],
                    ext->freeEntrySizeClassStatsSimulated._count[sizeClassIndex]);
                ...
                Trc_MM_LargeObjectAllocateStats_simulateAllocateTLHs_availableBytes(env->getLanguageVMThread(),
                    "regular", availableBytes, ext->freeEntrySizeClassStatsSimulated._count[sizeClassIndex],
                ...
                if (availableBytes <= allocBytesForCurrentStride) {
                    Trc_MM_LargeObjectAllocateStats_simulateAllocateCommon_satisfy(env->getLanguageVMThread(),
                        "partial", "regular", _sizeClassSizes[sizeClassIndex], sizeClassIndex);
                    Trc_MM_LargeObjectAllocateStats_simulateAllocateCommon_partial(env->getLanguageVMThread(),
                        allocBytesForCurrentStride - availableBytes, allocBytesForCurrentStride, availableBytes);
                ...
                /* there is one kind of remainder:
                 up to one of size: freeEntrySize - decimal-fraction-of-freeEntriesUsed-float *
                 _sizeClassSizes[sizeClassIndex]
                */

                /* we do not know the exact size of free entry(ies). we do know low and hi bounds, and will just
                 * approximate it (them) with a random size within the range */
                uintptr_t minFreeEntrySize = _sizeClassSizes[sizeClassIndex];
                uintptr_t maxFreeEntrySize = UDATA_MAX;
                ...
                Trc_MM_LargeObjectAllocateStats_simulateAllocateCommon_freeEntry_exit(env->getLanguageVMThread(), "TLH",
                    "regular", sizeClassIndex, ext->freeEntrySizeClassStatsSimulated._count[sizeClassIndex]);
...
void MM_LargeObjectAllocateStats::averageForSpaceSaving(MM_EnvironmentBase* env,
    OMRSpaceSaving* spaceSavingToAverageWith, OMRSpaceSaving** spaceSavingAveragePercent,
    uintptr_t bytesAllocatedThisRound)
{
    ...
    /* if bytesAllocatedThisRound == _averageBytesAllocated, the historic average will carry 9/10 weight (new weight is
     * 1/10) if bytesAllocatedThisRound = 9 x _averageBytesAllocated, the historic average will will carry 1/2 weight
     * (new weight is 1/2) as bytesAllocatedThisRound / _averageBytesAllocated ratio approaches infinity, the historic
     * average weight will approach 0 (new weight approaches 1) if _averageBytesAllocated = 9 x bytesAllocatedThisRound,
     * the historic average weight will carry 99/100 weigth (new weight is 1/100) as bytesAllocatedThisRound /
     * _averageBytesAllocated approaches 0, the historic average weight will approach 1 (new weight approaches 0)
     */
    ...
    for (i = 0; i < spaceSavingGetCurSize(spaceSavingToAverageWith); i++) {
        ...
        /* TODO: enable upSampleAllocStats, currently do not count the allocation in TLH, due to inaccurate upSample in
         * some cases */
        uintptr_t bytesAllocatedUpSampled = bytesAllocated;
...
void MM_LargeObjectAllocateStats::average(MM_EnvironmentBase* env, uintptr_t bytesAllocatedThisRound)
{
    averageForSpaceSaving(env, _spaceSavingSizes, &_spaceSavingSizesAveragePercent, bytesAllocatedThisRound);
    averageForSpaceSaving(
        env, _spaceSavingSizeClasses, &_spaceSavingSizeClassesAveragePercent, bytesAllocatedThisRound);
...
MMINLINE uintptr_t MM_LargeObjectAllocateStats::updateFreeEntrySizeClassStats(uintptr_t freeEntrySize,
    MM_FreeEntrySizeClassStats* freeEntrySizeClassStats, intptr_t count, uintptr_t sizeClassIndex,
    MM_FreeEntrySizeClassStats::FrequentAllocation* prev, MM_FreeEntrySizeClassStats::FrequentAllocation* curr)
{
...
uintptr_t MM_LargeObjectAllocateStats::incrementFreeEntrySizeClassStats(
    uintptr_t freeEntrySize, MM_FreeEntrySizeClassStats* freeEntrySizeClassStats, uintptr_t count)
{
    ...
    return updateFreeEntrySizeClassStats(
        freeEntrySize, freeEntrySizeClassStats, count, sizeClassIndex, prevFrequentAllocation, frequentAllocation);
}

uintptr_t MM_LargeObjectAllocateStats::incrementFreeEntrySizeClassStats(uintptr_t freeEntrySize)
{
    return incrementFreeEntrySizeClassStats(freeEntrySize, &_freeEntrySizeClassStats, 1);
}

void MM_LargeObjectAllocateStats::incrementTlhAllocSizeClassStats(uintptr_t freeEntrySize)
{
#if defined(OMR_GC_THREAD_LOCAL_HEAP)
    uintptr_t sizeClassIndex = getSizeClassIndex(freeEntrySize);
...
void MM_LargeObjectAllocateStats::decrementFreeEntrySizeClassStats(uintptr_t freeEntrySize)
{
...
uintptr_t MM_LargeObjectAllocateStats::getSizeClassIndex(uintptr_t size)
{
    ...
    Assert_GC_true_with_message2(
        _env, (logValue >= 0.0), "Error calculation log(), passed %zu, returned %f\n", size, logValue);
    Assert_GC_true_with_message(_env, (_sizeClassRatioLog > 0.0),
    ...
    Assert_GC_true_with_message2(_env,
        ((_freeEntrySizeClassStats._maxSizeClasses == 0) || (result < _freeEntrySizeClassStats._maxSizeClasses)),
...
void MM_LargeObjectAllocateStats::verifyFreeEntryCount(uintptr_t actualFreeEntryCount)
{
