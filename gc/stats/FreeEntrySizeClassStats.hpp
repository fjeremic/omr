...
private:
    ...
    FrequentAllocation** _frequentAllocationHead; /**< for each size class, there is a list of ascending (per size, not
                                                     per count) frequent allocation (exact) sizes) */
    uintptr_t _maxSizeClasses; /**< size of the above array */

    FrequentAllocation*
        _frequentAllocation; /**< an array of FrequentAllocation structs of size _maxFrequentAllocateSizes (current size
    ...
    float* _fractionFrequentAllocation; /**< fraction Array for cumulating the fraction of frequentAllocation during
...
public:
    ...
    /**< @param factorVeryLargeEntryPool : multiple factor for _maxVeryLargeEntrySizes, default = 1, double for
     * splitFreeList case
     *   @param simulation : if true, generate _fractionFrequentAllocation array for cumulating fraction of
     * frequentAllocation during estimating fragmentation, default = false
     */
    bool initialize(MM_EnvironmentBase* env, uintptr_t maxAllocateSizes, uintptr_t maxSizeClasses,
        uintptr_t veryLargeObjectThreshold, uintptr_t factorVeryLargeEntryPool = 1, bool simulation = false);
