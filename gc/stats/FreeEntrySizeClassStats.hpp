...
private:
  ...
  uintptr_t
      _maxFrequentAllocateSizes; /**< max size of _frequentAllocation array */
  uintptr_t
      _maxVeryLargeEntrySizes; /**< max size of _veryLargeEntryPool array */
...
#define MAX_FREE_ENTRY_COUNTERS_PER_FREQ_ALLOC_SIZE 5
  uintptr_t _frequentAllocateSizeCounters; /**< current size of
                                              _frequentAllocation array */

  MM_LightweightNonReentrantLock
      _lock; /**< lock used during merge of thread local stats */
  bool guarantyEnoughPoolSizeForVeryLargeEntry; /**< true for all memory pool,
                                                   false for thread base */
private:
  /**
   * Take a snapshot of this structure stats.
   * @param[out] stats structure where the snapshot is stored
   * @param[in] sizes of all sizeclasses used to calculate total free memory
   * returned
   * @return total free memory represented by this structure
   */
  uintptr_t copyTo(MM_FreeEntrySizeClassStats *stats,
                   const uintptr_t sizeClassSizes[]);

  FrequentAllocation *copyVeryLargeEntry(FrequentAllocation *entry);

public:
  /* return count of 'regular' free entires (not those that are frequent
   * allocates for a given size class */
  uintptr_t getCount(uintptr_t sizeClassIndex) {
    return _count[sizeClassIndex];
  }
  ...
  /* return the head of the lisf of all frequent allocate entries for a give
   * size class */
  FrequentAllocation *getFrequentAllocationHead(uintptr_t sizeClassIndex) {
    return _frequentAllocationHead[sizeClassIndex];
  }
  /* return total free memory represented by this structure */
  uintptr_t getFreeMemory(const uintptr_t sizeClassSizes[]);
  /* return the 'average' number of pages which can be freed */
  uintptr_t getPageAlignedFreeMemory(const uintptr_t sizeClassSizes[],
                                     uintptr_t pageSize);

  /**< @param factorVeryLargeEntryPool : multiple factor for
   * _maxVeryLargeEntrySizes, default = 1, double for splitFreeList case
   *   @param simulation : if true, generate _fractionFrequentAllocation array
   * for cumulating fraction of frequentAllocation during estimating
   * fragmentation, default = false
   */
  bool initialize(MM_EnvironmentBase *env, uintptr_t maxAllocateSizes,
                  uintptr_t maxSizeClasses, uintptr_t veryLargeObjectThreshold,
   ...
   *  correct inconsistent count, which is generated by the lack of size of
   * VeryLargeObjectPool.
   */
  ...
  void initializeFrequentAllocation(
      MM_LargeObjectAllocateStats *largeObjectAllocateStats);
  /**< initialize veryLargeEntryPool -- link free veryLargeEntries in
   * _freeHeadVeryLargeEntry list */
  void initializeVeryLargeEntryPool();

  friend class MM_LargeObjectAllocateStats;

  MM_FreeEntrySizeClassStats()
      : _count(NULL), _frequentAllocationHead(NULL), _maxSizeClasses(0),
        _frequentAllocation(NULL), _veryLargeEntryPool(NULL),
        _fractionFrequentAllocation(NULL), _maxFrequentAllocateSizes(0),
        _maxVeryLargeEntrySizes(0), _veryLargeEntrySizeClass(0),
