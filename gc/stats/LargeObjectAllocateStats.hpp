...
private:
...
#endif
    OMRSpaceSaving*
        _spaceSavingSizes; /**< Internal top-k-frequent data structure to maintain the stats for exact sizes */
    OMRSpaceSaving*
        _spaceSavingSizeClasses; /**< Internal top-k-frequent data structure to maintain the stats for size-classes */
    OMRSpaceSaving* _spaceSavingSizesAveragePercent; /**< Internal top-k-frequent data structure to maintain the stats
                                                        for exact sizes */
    OMRSpaceSaving* _spaceSavingSizeClassesAveragePercent; /**< Internal top-k-frequent data structure to maintain the
                                                              stats for size-classes */
    ...
    uintptr_t _veryLargeEntrySizeClass; /**< index of sizeClass for minimum veryLargeEntry, the cache value of
    ...
    MM_FreeEntrySizeClassStats _freeEntrySizeClassStats; /**< global (still per pool) statistics structure for heap free
    ...
    uintptr_t
        _TLHSizeClassIndex; /**< preserved next value of sizeClassIndex on last invocation of simulateAllocateTLHs */
    uintptr_t _TLHFrequentAllocationSize; /**< preserved next value of FrequentAllocationSize on last invocation of
    ...
    /**
     * Historic weighted averaging of current stats with existing average stats. A private call. Called twice from
     * public call for exact sizes and sizeClasses
     ...
     */
    void averageForSpaceSaving(MM_EnvironmentBase* env, OMRSpaceSaving* spaceSavingToAverageWith,
        OMRSpaceSaving** spaceSavingAveragePercent, uintptr_t bytesAllocatedThisRound);

    /**
     * Large allocation stats are initially done only on out-of-line allocates (does not fit within current TLH), so we
     * will miss to count those that fit within TLH This method extrapolate allocate stats to account even for those
     * that fit within TLH. For given allocation size and total bytes allocated out of line, the method returns
     * extrapolated total amount of bytes (including in-line and out-of-line) for that size. The extrapolation logic
     * relies on distribution of TLH sizes that have been created, and calculates probability that the size fits in each
     * of TLH size classes.
     ...
     */
    uintptr_t simulateAllocateObjects(
        MM_EnvironmentBase* env, uintptr_t totalAllocBytes, uintptr_t objectSize, uintptr_t* currentFreeMemory);
    /**
     ...
     * @param[out] currentFreeMemory free memory after allocate
     */
    uintptr_t simulateAllocateTLHs(
        MM_EnvironmentBase* env, uintptr_t totalAllocBytes, uintptr_t* currentFreeMemory, uintptr_t strides);

    /**
     * share code between incrementFreeEntrySizeClassStats(), decrementFreeEntrySizeClassStats() and
     * simulateAllocateObjects(), simulateAllocateTLHs()
     */
    MMINLINE uintptr_t updateFreeEntrySizeClassStats(uintptr_t freeEntrySize,
...
public:
    /* Based on free memory stats and allocation profile, simulate allocation and project remaining unused (fragmented)
     * memory */
    ...
    static MM_LargeObjectAllocateStats* newInstance(MM_EnvironmentBase* env, uint16_t maxAllocateSizes,
        uintptr_t largeObjectThreshold, uintptr_t veryLargeObjectThreshold, float sizeClassRatio, uintptr_t maxHeapSize,
        uintptr_t tlhMaximumSize, uintptr_t tlhMinimumSize, uintptr_t factorVeryLargeEntryPool = 1);
    void kill(MM_EnvironmentBase* env);

    bool initialize(MM_EnvironmentBase* env, uint16_t maxAllocateSizes, uintptr_t largeObjectThreshold,
        uintptr_t veryLargeObjectThreshold, float sizeClassRatio, uintptr_t maxHeapSize, uintptr_t tlhMaximumSize,
    ...
    uintptr_t incrementFreeEntrySizeClassStats(
        uintptr_t freeEntrySize, MM_FreeEntrySizeClassStats* freeEntrySizeClassStats)
    {
    ...
    void decrementFreeEntrySizeClassStats(
        uintptr_t freeEntrySize, MM_FreeEntrySizeClassStats* inFreeEntrySizeClassStats, uintptr_t count);
    ...
    {
        return _freeEntrySizeClassStats.getPageAlignedFreeMemory(_sizeClassSizes, pageSize);
    }
