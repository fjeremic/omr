...
public:
  ...
  MMINLINE void setFreeBytesAtSystemGCStart(uintptr_t freeBytes) {
    _freeBytesAtSystemGCStart = freeBytes;
  }
  MMINLINE uintptr_t getFreeBytesAtSystemGCStart() {
    return _freeBytesAtSystemGCStart;
  }

  MMINLINE void setLastHeapExpansionGCCount(uintptr_t gccount) {
    _lastHeapExpansionGCCount = gccount;
  }
  MMINLINE uintptr_t getLastHeapExpansionGCCount() {
    return _lastHeapExpansionGCCount;
  }
  MMINLINE void setLastHeapContractionGCCount(uintptr_t gccount) {
    _lastHeapContractionGCCount = gccount;
  }
  MMINLINE uintptr_t getLastHeapContractionGCCount() {
    return _lastHeapContractionGCCount;
  }

  MMINLINE void setLastExpandReason(ExpandReason reason) {
    _lastExpandReason = reason;
  }
  MMINLINE ExpandReason getLastExpandReason() { return _lastExpandReason; }
  MMINLINE void setLastContractReason(ContractReason reason) {
    _lastContractReason = reason;
  }
  MMINLINE ContractReason getLastContractReason() {
    return _lastContractReason;
  }
  MMINLINE void setLastLoaResizeReason(LoaResizeReason reason) {
    _lastLoaResizeReason = reason;
  }
  MMINLINE LoaResizeReason getLastLoaResizeReason() {
    return _lastLoaResizeReason;
  }

  MMINLINE void setLastExpandActualSize(uintptr_t size) {
    _lastActualHeapExpansionSize = size;
  }
  MMINLINE uintptr_t getLastExpandActualSize() {
    return _lastActualHeapExpansionSize;
  }
  MMINLINE void setLastContractActualSize(uintptr_t size) {
    _lastActualHeapContractionSize = size;
  }
  MMINLINE uintptr_t getLastContractActualSize() {
    return _lastActualHeapContractionSize;
  }
  ...
  MMINLINE void setLastContractTime(uint64_t ticks) {
    _lastContractTime = ticks;
  }
  MMINLINE uint64_t getLastContractTime() { return _lastContractTime; }

  MMINLINE void setLastTimeOutsideGC() {
    /* CMVC 125876:  Note that time can go backward (core-swap, for example) so
     * store a 1 as the delta if this is happening since we can't use 0 time
     * deltas and negative deltas can cause arithmetic exceptions
     */
    /* cache the ivars so that they aren't changed concurrently (not sure if
     * this is a threat but better safe than sorry) */
  ...
  MMINLINE void setGlobalGCCountAtAF(uintptr_t count) {
    _globalGCCountAtAF = count;
  }
  ...
  MM_HeapResizeStats()
      : MM_Base(), _lastAFEndTime(0), _thisAFStartTime(0),
        _freeBytesAtSystemGCStart(0), _lastHeapExpansionGCCount(0),
        _lastHeapContractionGCCount(0), _lastActualHeapExpansionSize(0),
        _lastActualHeapContractionSize(0), _lastExpandReason(NO_EXPAND),
        _lastContractReason(NO_CONTRACT), _lastLoaResizeReason(NO_LOA_RESIZE),
        _lastExpandTime(0), _lastContractTime(0), _lastGCPercentage(0),
        _lastTimeOutsideGC(0), _globalGCCountAtAF(0) {
    resetRatioTicks();
  }
};
