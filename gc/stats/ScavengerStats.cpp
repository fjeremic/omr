...
struct MM_ScavengerStats::FlipHistory* MM_ScavengerStats::getFlipHistory(uintptr_t lookback)
{
    ...
    if (SCAVENGER_FLIP_HISTORY_SIZE <= lookback) {
        ...
        uintptr_t historyIndex
            = (SCAVENGER_FLIP_HISTORY_SIZE + _flipHistoryNewIndex - lookback) % SCAVENGER_FLIP_HISTORY_SIZE;
...
{
    if (firstIncrement) {
        /* clear() can be called several times per a cycle (in Concurrent Scavenger), but some stats/params must be
         * reset/updated only once per a cycle */

        /* Increment the histogram offset and loop if necessary */
        /* TODO: this does not properly work for Master GC threads, which is implicit (a random mutator thread), for
         * standard (non CS) Scavenger. Flip history stats (or complete ScavengerStats) should be one place (in
