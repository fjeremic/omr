 ...
 */
static omr_error_t setMinimal(OMR_TraceThread* thr,
                              const char* value,
                              BOOLEAN atRuntime);
static omr_error_t setMaximal(OMR_TraceThread* thr,
                              const char* value,
                              BOOLEAN atRuntime);
static omr_error_t setCount(OMR_TraceThread* thr,
                            const char* value,
                            BOOLEAN atRuntime);
static omr_error_t setPrint(OMR_TraceThread* thr,
                            const char* value,
                            BOOLEAN atRuntime);
static omr_error_t setNone(OMR_TraceThread* thr,
                           const char* value,
                           BOOLEAN atRuntime);
static omr_error_t setIprint(OMR_TraceThread* thr,
                             const char* value,
                             BOOLEAN atRuntime);
static omr_error_t setException(OMR_TraceThread* thr,
                                const char* value,
                                BOOLEAN atRuntime);
#if OMR_ALLOW_OUTPUT_OPTION
static omr_error_t setOutput(OMR_TraceThread* thr,
                             const char* value,
                             BOOLEAN atRuntime);
#endif /* OMR_ALLOW_OUTPUT_OPTION */
static omr_error_t setBuffers(OMR_TraceThread* thr,
                              const char* value,
                              BOOLEAN atRuntime);
static omr_error_t setSuspendResumeCount(OMR_TraceThread* thr,
                                         ...
                                         int32_t resume,
                                         BOOLEAN atRuntime);
static omr_error_t processSuspendOption(OMR_TraceThread* thr,
                                        const char* value,
                                        BOOLEAN atRuntime);
static omr_error_t processResumeOption(OMR_TraceThread* thr,
                                       const char* value,
                                       BOOLEAN atRuntime);
static omr_error_t processSuspendCountOption(OMR_TraceThread* thr,
                                             const char* value,
                                             BOOLEAN atRuntime);
static int selectComponent(const char* cmd,
                           int32_t* first,
                           char traceType,
                           int32_t setActive,
                           BOOLEAN atRuntime);
static omr_error_t setFatalAssert(OMR_TraceThread* thr,
                                  const char* spec,
                                  BOOLEAN atRuntime);
static omr_error_t clearFatalAssert(OMR_TraceThread* thr,
                                    const char* spec,
                                    BOOLEAN atRuntime);

static const char* getPositionalParm(int pos, const char* string, int* size);
static int getParmNumber(const char* string);
static int decimalString2Int(const char* decString,
                             int32_t signedAllowed,
                             omr_error_t* rc,
                             BOOLEAN atRuntime);
 ...
 ******************************************************************************/
static omr_error_t addTraceCmd(OMR_TraceThread* thr,
                               const char* cmd,
                               const char* value,
                               BOOLEAN atRuntime) {
...
static omr_error_t parseBufferSize(const char* const str,
                                   const int argSize,
                                   BOOLEAN atRuntime) {
 ...
 * returns     - UTE return code
 *************************************************************************/
omr_error_t setSuspendResumeCount(OMR_TraceThread* thr,
                                  const char* value,
                                  int32_t resume,
                                  BOOLEAN atRuntime) {
  omr_error_t rc = OMR_ERROR_NONE;
 ...
 *************************************************************************/
void reportCommandLineError(BOOLEAN suppressMessages,
...
static int selectComponent(const char* cmd,
  ...
  if (*cmd == '\0') {
    if (*first) {
       ...
       */
      rc = setTracePointsTo((const char*)UT_ALL, OMR_TRACEGLOBAL(componentList),
                            TRUE, 0, 0, traceType, -1, NULL, atRuntime,
                            setActive);
      if (OMR_ERROR_NONE != rc) {
 ...
 * returns     - void
 *************************************************************************/
int decimalString2Int(const char* decString,
                      int32_t signedAllowed,
                      omr_error_t* rc,
                      BOOLEAN atRuntime) {
