...
typedef omr_error_t (*OMR_TraceOptionFunction)(OMR_TraceThread *thr,
                                               const char *value,
                                               BOOLEAN atRuntime);
 ...
 */
static omr_error_t setMinimal(OMR_TraceThread *thr, const char *value,
                              BOOLEAN atRuntime);
static omr_error_t setMaximal(OMR_TraceThread *thr, const char *value,
                              BOOLEAN atRuntime);
static omr_error_t setCount(OMR_TraceThread *thr, const char *value,
                            BOOLEAN atRuntime);
static omr_error_t setPrint(OMR_TraceThread *thr, const char *value,
                            BOOLEAN atRuntime);
static omr_error_t setNone(OMR_TraceThread *thr, const char *value,
                           BOOLEAN atRuntime);
static omr_error_t setIprint(OMR_TraceThread *thr, const char *value,
                             BOOLEAN atRuntime);
static omr_error_t setException(OMR_TraceThread *thr, const char *value,
                                BOOLEAN atRuntime);
#if OMR_ALLOW_OUTPUT_OPTION
static omr_error_t setOutput(OMR_TraceThread *thr, const char *value,
                             BOOLEAN atRuntime);
#endif /* OMR_ALLOW_OUTPUT_OPTION */
static omr_error_t setBuffers(OMR_TraceThread *thr, const char *value,
                              BOOLEAN atRuntime);
static omr_error_t setSuspendResumeCount(OMR_TraceThread *thr,
                                         const char *value, int32_t resume,
                                         BOOLEAN atRuntime);
static omr_error_t processSuspendOption(OMR_TraceThread *thr, const char *value,
                                        BOOLEAN atRuntime);
static omr_error_t processResumeOption(OMR_TraceThread *thr, const char *value,
                                       BOOLEAN atRuntime);
static omr_error_t processSuspendCountOption(OMR_TraceThread *thr,
                                             const char *value,
                                             BOOLEAN atRuntime);
static int selectComponent(const char *cmd, int32_t *first, char traceType,
                           int32_t setActive, BOOLEAN atRuntime);
static omr_error_t setFatalAssert(OMR_TraceThread *thr, const char *spec,
                                  BOOLEAN atRuntime);
static omr_error_t clearFatalAssert(OMR_TraceThread *thr, const char *spec,
                                    BOOLEAN atRuntime);

static const char *getPositionalParm(int pos, const char *string, int *size);
static int getParmNumber(const char *string);
static int decimalString2Int(const char *decString, int32_t signedAllowed,
                             omr_error_t *rc, BOOLEAN atRuntime);
...
const OMR_TraceOption UTE_OPTIONS[] = {
    ...
    {UT_BUFFERS_KEYWORD, TRUE,
     setBuffers}, /* Not all buffers functions are exposed - but are controlled
                     in the set function*/
 ...
 ******************************************************************************/
static omr_error_t addTraceCmd(OMR_TraceThread *thr, const char *cmd,
                               const char *value, BOOLEAN atRuntime) {
  omr_error_t rc = OMR_ERROR_NONE;
  char *str;

  OMRPORT_ACCESS_FROM_OMRPORT(OMR_TRACEGLOBAL(portLibrary));

  str = (char *)omrmem_allocate_memory(
      strlen(cmd) + (value == NULL ? 1 : strlen(value) + 2),
...
static omr_error_t parseBufferSize(const char *const str, const int argSize,
                                   BOOLEAN atRuntime) {
  ...
  if (firstNonDigit != -1) {
    if (firstNonDigit == (argSize - 1) && firstDigit != -1) {
      ...
      switch (j9_cmdla_toupper(str[argSize - 1])) {
        ...
        default:
          reportCommandLineError(
              atRuntime, "Unrecognised suffix %c specified for buffer size",
              str[argSize - 1]);
      ...
      /* Invalid */
      reportCommandLineError(
          atRuntime, "Invalid option for -Xtrace:buffers - \"%s\"", str);
      return OMR_ERROR_ILLEGAL_ARGUMENT;
    }
  } else {
  ...
  if (newBufferSize < UT_MINIMUM_BUFFERSIZE) {
    reportCommandLineError(
        atRuntime,
        "Specified buffer size %d bytes is too small. Minimum is %d bytes.",
 ...
 * parameters  - thr, string value of the property (nnnk|nnnm[,dynamic]),
 *atRuntime returns     - UTE return code
 ******************************************************************************/
static omr_error_t setBuffers(OMR_TraceThread *thr, const char *value,
                              BOOLEAN atRuntime) {
  ...
  localBuffer =
      (char *)omrmem_allocate_memory(strlen(value) + 1, OMRMEM_CATEGORY_TRACE);
  if (NULL == localBuffer) {
  ...
  for (i = 0; i < numberOfArgs; i++) {
    ...
    if (argSize == 0) {
      reportCommandLineError(atRuntime,
                             "Empty option passed to -Xtrace:buffers");
...
omr_error_t setFormat(const char *datDir) {
  ...
  OMR_TRACEGLOBAL(traceFormatSpec) =
      (char *)omrmem_allocate_memory(strlen(datDir) + 1, OMRMEM_CATEGORY_TRACE);
 ...
 * returns     - UTE return code
 *************************************************************************/
omr_error_t setSuspendResumeCount(OMR_TraceThread *thr, const char *value,
                                  int32_t resume, BOOLEAN atRuntime) {
  omr_error_t rc = OMR_ERROR_NONE;
  const char *p;
  int length;
  int maxLength = 5;
  ...
  if (OMR_ERROR_NONE != rc) {
    if (resume) {
      reportCommandLineError(
          atRuntime,
          "resumecount takes a single integer value from -99999 to +99999");
    } else {
      reportCommandLineError(
          atRuntime,
          "suspendcount takes a single integer value from -99999 to +99999");
    }
  }
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    if (OMR_TRACEGLOBAL(initialSuspendResume) != 0) {
      reportCommandLineError(
          atRuntime, "resumecount and suspendcount may not both be set.");
      rc = OMR_ERROR_INTERNAL;
    } else {
...
omr_error_t processOptions(OMR_TraceThread *thr, const char **opts,
  ...
  if (!atRuntime) {
    for (i = 0; opts[i] != NULL; i += 2) {
      if (j9_cmdla_stricmp((char *)opts[i], UT_DEBUG_KEYWORD) == 0) {
        if (opts[i + 1] != NULL && strlen(opts[i + 1]) == 1 &&
  ...
  /* NULL is allowed for thr only during trace initialization, when only one
   * thread can be accessing the trace engine.
   */
  if ((NULL == thr) &&
  ...
  for (i = 0; opts[i] != NULL; i += 2) {
    ...
    if (NULL != optValue) {
      ...
      if (optValue[0] == '{' && optValue[optValueLen - 1] == '}') {
        optValue = (char *)omrmem_allocate_memory(optValueLen - 1,
                                                  OMRMEM_CATEGORY_TRACE);
        if (!optValue) {
          rc = OMR_ERROR_OUT_OF_NATIVE_MEMORY;
          break;
    ...
    for (size_t j = 0; j < NUMBER_OF_UTE_OPTIONS; j++) {
      if (optNameLen == strlen(UTE_OPTIONS[j].name) &&
        ...
        if (atRuntime && !UTE_OPTIONS[j].runtimeModifiable) {
          reportCommandLineError(atRuntime,
                                 "Option \"%s\" cannot be set at run-time. Set "
                                 "it on the command line at start-up.",
                                 optName);
 ...
 *************************************************************************/
void reportCommandLineError(BOOLEAN suppressMessages, const char *detailStr,
                            ...) {
  OMR_TraceCommandLineErrorFunc errorCallback =
      OMR_TRACEGLOBAL(languageIntf).ReportCommandLineError;
 ...
 *************************************************************************/
static int selectSpecial(const char *string) {
...
static int selectComponent(const char *cmd, int32_t *first, char traceType,
  ...
  if (*cmd == '\0') {
    if (*first) {
       ...
       */
      rc = setTracePointsTo((const char *)UT_ALL,
                            OMR_TRACEGLOBAL(componentList), TRUE, 0, 0,
                            traceType, -1, NULL, atRuntime, setActive);
      if (OMR_ERROR_NONE != rc) {
    ...
    *first = FALSE;
    ...
    rc = setTracePointsTo(cmd, OMR_TRACEGLOBAL(componentList), TRUE, 0, 0,
                          traceType, -1, NULL, atRuntime, setActive);
    if (OMR_ERROR_NONE != rc) {
      UT_DBGOUT(1, ("<UT> Can't turn on all tracepoints\n"));
      return -1;
...
omr_error_t setTraceState(const char *cmd, BOOLEAN atRuntime) {
  ...
  while ((OMR_ERROR_NONE == rc) && (UT_CONTEXT_FINAL != context)) {
    switch (context) {
      ...
      case UT_CONTEXT_INITIAL:
        ...
        if (0 == j9_cmdla_strnicmp(p, UT_MINIMAL_KEYWORD,
                                   strlen(UT_MINIMAL_KEYWORD))) {
        ...
        } else if (0 == j9_cmdla_strnicmp(p, UT_MAXIMAL_KEYWORD,
                                          strlen(UT_MAXIMAL_KEYWORD))) {
          if (!OMR_TRACE_ENGINE_IS_ENABLED(OMR_TRACEGLOBAL(initState))) {
            rc = OMR_ERROR_ILLEGAL_ARGUMENT;
            break;
          }
        ...
        } else if (0 == j9_cmdla_strnicmp(p, UT_COUNT_KEYWORD,
                                          strlen(UT_COUNT_KEYWORD))) {
          if (!OMR_TRACE_ENGINE_IS_ENABLED(OMR_TRACEGLOBAL(initState))) {
            rc = OMR_ERROR_ILLEGAL_ARGUMENT;
            break;
          }
        ...
        } else if (0 == j9_cmdla_strnicmp(p, UT_PRINT_KEYWORD,
                                          strlen(UT_PRINT_KEYWORD))) {
          traceType = UT_PRINT;
          OMR_TRACEGLOBAL(indentPrint) = FALSE;
          p += strlen(UT_PRINT_KEYWORD);
        } else if (0 == j9_cmdla_strnicmp(p, UT_NONE_KEYWORD,
                                          strlen(UT_NONE_KEYWORD))) {
          traceType = UT_NONE;
          p += strlen(UT_NONE_KEYWORD);
        } else if (0 == j9_cmdla_strnicmp(p, UT_IPRINT_KEYWORD,
                                          strlen(UT_IPRINT_KEYWORD))) {
          traceType = UT_PRINT;
          OMR_TRACEGLOBAL(indentPrint) = TRUE;
          p += strlen(UT_IPRINT_KEYWORD);
        } else if (0 == j9_cmdla_strnicmp(p, UT_EXCEPTION_KEYWORD,
                                          strlen(UT_EXCEPTION_KEYWORD))) {
          if (!OMR_TRACE_ENGINE_IS_ENABLED(OMR_TRACEGLOBAL(initState))) {
            rc = OMR_ERROR_ILLEGAL_ARGUMENT;
            break;
          }
        ...
        if (*p == '\0' && traceType != UT_NONE) {
          /* Only the keyword has been specified and the option isn't
           * -Xtrace:none */
          reportCommandLineError(atRuntime, "Option %s requires an argument.",
                                 cmd);
          rc = OMR_ERROR_ILLEGAL_ARGUMENT;
        }
  ...
  if (OMR_ERROR_NONE == rc) {
    if (context != UT_CONTEXT_FINAL) {
      reportCommandLineError(
          atRuntime, "Trace selection specification incomplete: %s", cmd);
      rc = OMR_ERROR_ILLEGAL_ARGUMENT;
    }
  } else {
    reportCommandLineError(atRuntime,
                           "Syntax error encountered at offset %d in: %s",
                           p - cmd, cmd);
  }
...
omr_error_t setOptions(OMR_TraceThread *thr, const char **opts,
  ...
  if (!atRuntime) {
    ...
    if (OMR_ERROR_NONE != rc) {
      /* Can only fail due to OOM copying path to format file, no message
       * required. */
      return OMR_ERROR_ILLEGAL_ARGUMENT;
    }
  }
 ...
 * returns     - void
 *************************************************************************/
int decimalString2Int(const char *decString, int32_t signedAllowed,
                      omr_error_t *rc, BOOLEAN atRuntime) {
  ...
  if (inputIsSigned && !signedAllowed) {
    reportCommandLineError(
        atRuntime, "Signed number not permitted in this context \"%s\".",
        decString);
    *rc = OMR_ERROR_INTERNAL;
  }
  ...
  if (OMR_ERROR_NONE == *rc) {
    ...
    if ((*p != ',') && (*p != '}') && (*p != '\0') && (*p != ' ')) {
      reportCommandLineError(
          atRuntime,
          "Invalid character(s) encountered in decimal number \"%s\".",
          decString);
      *rc = OMR_ERROR_INTERNAL;
    }
  }
  ...
  if (OMR_ERROR_NONE == *rc) {
     ...
     */
    if ((p - decString) < min_string_length ||
        (p - decString) > max_string_length) {
      *rc = OMR_ERROR_INTERNAL;
      reportCommandLineError(atRuntime, "Number too long or too short \"%s\".",
                             decString);
    } else {
