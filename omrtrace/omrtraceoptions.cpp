...
static omr_error_t
setSuspendResumeCount(OMR_TraceThread* thr,
                      const char* value,
                      int32_t resume,
                      BOOLEAN atRuntime);
static omr_error_t
processSuspendOption(OMR_TraceThread* thr,
                     const char* value,
                     BOOLEAN atRuntime);
static omr_error_t
processResumeOption(OMR_TraceThread* thr, const char* value, BOOLEAN atRuntime);
static omr_error_t
processSuspendCountOption(OMR_TraceThread* thr,
                          const char* value,
                          BOOLEAN atRuntime);
static int
selectComponent(const char* cmd,
                int32_t* first,
                char traceType,
                int32_t setActive,
                BOOLEAN atRuntime);
...
static int
decimalString2Int(const char* decString,
 ...
 ******************************************************************************/
static omr_error_t
addTraceCmd(OMR_TraceThread* thr,
            const char* cmd,
            const char* value,
            BOOLEAN atRuntime)
{
  ...
  str = (char*)omrmem_allocate_memory(strlen(cmd) +
                                        (value == NULL ? 1 : strlen(value) + 2),
      ...
      switch (j9_cmdla_toupper(str[argSize - 1])) {
        ...
        default:
          reportCommandLineError(
            atRuntime,
            "Unrecognised suffix %c specified for buffer size",
            str[argSize - 1]);
  ...
  for (i = 0; opts[i] != NULL; i += 2) {
    ...
    if (NULL != optValue) {
      ...
      if (optValue[0] == '{' && optValue[optValueLen - 1] == '}') {
        optValue =
          (char*)omrmem_allocate_memory(optValueLen - 1, OMRMEM_CATEGORY_TRACE);
        if (!optValue) {
          rc = OMR_ERROR_OUT_OF_NATIVE_MEMORY;
          break;
        }
...
setTraceState(const char* cmd, BOOLEAN atRuntime)
{
    ...
    switch (context) {
      ...
      case UT_CONTEXT_INITIAL:
        ...
        if (0 == j9_cmdla_strnicmp(
                   p, UT_MINIMAL_KEYWORD, strlen(UT_MINIMAL_KEYWORD))) {
        ...
        } else if (0 == j9_cmdla_strnicmp(
                          p, UT_MAXIMAL_KEYWORD, strlen(UT_MAXIMAL_KEYWORD))) {
          if (!OMR_TRACE_ENGINE_IS_ENABLED(OMR_TRACEGLOBAL(initState))) {
            rc = OMR_ERROR_ILLEGAL_ARGUMENT;
            break;
          }
          ...
          p += strlen(UT_MAXIMAL_KEYWORD);
                          ...
                          p, UT_COUNT_KEYWORD, strlen(UT_COUNT_KEYWORD))) {
          if (!OMR_TRACE_ENGINE_IS_ENABLED(OMR_TRACEGLOBAL(initState))) {
            rc = OMR_ERROR_ILLEGAL_ARGUMENT;
            break;
          }
          ...
          p += strlen(UT_COUNT_KEYWORD);
                          ...
                          p, UT_PRINT_KEYWORD, strlen(UT_PRINT_KEYWORD))) {
          traceType = UT_PRINT;
          OMR_TRACEGLOBAL(indentPrint) = FALSE;
          p += strlen(UT_PRINT_KEYWORD);
        } else if (0 == j9_cmdla_strnicmp(
                          p, UT_NONE_KEYWORD, strlen(UT_NONE_KEYWORD))) {
          traceType = UT_NONE;
          p += strlen(UT_NONE_KEYWORD);
        } else if (0 == j9_cmdla_strnicmp(
                          p, UT_IPRINT_KEYWORD, strlen(UT_IPRINT_KEYWORD))) {
          traceType = UT_PRINT;
          OMR_TRACEGLOBAL(indentPrint) = TRUE;
          p += strlen(UT_IPRINT_KEYWORD);
        } else if (0 == j9_cmdla_strnicmp(p,
                                          UT_EXCEPTION_KEYWORD,
        ...
        if (*p == '\0' && traceType != UT_NONE) {
           ...
           * -Xtrace:none */
          reportCommandLineError(
            atRuntime, "Option %s requires an argument.", cmd);
          rc = OMR_ERROR_ILLEGAL_ARGUMENT;
        }
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    reportCommandLineError(
      atRuntime, "Syntax error encountered at offset %d in: %s", p - cmd, cmd);
  }
                  ...
                  BOOLEAN atRuntime)
{
  ...
  if (inputIsSigned && !signedAllowed) {
    reportCommandLineError(
      atRuntime,
      "Signed number not permitted in this context \"%s\".",
      decString);
    *rc = OMR_ERROR_INTERNAL;
  }
  ...
  if (OMR_ERROR_NONE == *rc) {
    ...
    if ((p - decString) < min_string_length ||
      ...
      *rc = OMR_ERROR_INTERNAL;
      reportCommandLineError(
        atRuntime, "Number too long or too short \"%s\".", decString);
    } else {
