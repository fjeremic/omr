 ...
 ******************************************************************************/
static omr_error_t addTraceCmd(OMR_TraceThread* thr, const char* cmd, const char* value, BOOLEAN atRuntime)
{
...
static omr_error_t parseBufferSize(const char* const str, const int argSize, BOOLEAN atRuntime)
{
 ...
 * returns     - UTE return code
 *************************************************************************/
omr_error_t setSuspendResumeCount(OMR_TraceThread* thr, const char* value, int32_t resume, BOOLEAN atRuntime)
{
  omr_error_t rc = OMR_ERROR_NONE;
  const char* p;
  int length;
      ...
      if (optNameLen == strlen(UTE_OPTIONS[j].name)
        ...
        if (atRuntime && !UTE_OPTIONS[j].runtimeModifiable) {
          reportCommandLineError(
            atRuntime, "Option \"%s\" cannot be set at run-time. Set it on the command line at start-up.", optName);
...
static int selectComponent(const char* cmd, int32_t* first, char traceType, int32_t setActive, BOOLEAN atRuntime)
{
  ...
  if (*cmd == '\0') {
    if (*first) {
       ...
       */
      rc = setTracePointsTo(
        (const char*)UT_ALL, OMR_TRACEGLOBAL(componentList), TRUE, 0, 0, traceType, -1, NULL, atRuntime, setActive);
      if (OMR_ERROR_NONE != rc) {
