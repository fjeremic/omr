...
omr_error_t
omr_trc_getTraceFileIterator(OMRPortLibrary* portLib,
                             char* fileName,
 ...
 */
omr_error_t
omr_trc_getTracePointIteratorForNextBuffer(
  ...
  UtTracePointIterator** bufferIteratorPtr)
{
  ...
  if (iterator == NULL) {
    UT_DBGOUT_CHECKED(
      1, ("<UT> trcGetTracePointIteratorForBuffer cannot allocate iterator\n"));
  ...
  if (iterator->buffer == NULL) {
    UT_DBGOUT_CHECKED(1,
                      ("<UT> trcGetTracePointIteratorForBuffer cannot allocate "
                       "iterator's buffer\n"));
  ...
  /* set up the iterator */
  bytesRead = omrfile_read(fileIterator->traceFileHandle,
                           &iterator->buffer->record,
  ...
  UT_DBGOUT_CHECKED(4,
                    ("<UT> firstEntry: %d, offset of record: %ld buffer size: "
                     "%d endianness %s\n",
                     iterator->start,
                     offsetof(OMR_TraceBuffer, record),
                     fileIterator->header->bufferSize,
                     (iterator->isBigEndian) ? "bigEndian" : "littleEndian"));
  UT_DBGOUT_CHECKED(2,
                    ("<UT> omr_trc_getTracePointIteratorForNextBuffer: Thread "
                     "%s returning iterator %p\n",
                     iterator->buffer->record.threadName,
                     iterator));
...
uint32_t
omr_trc_getBufferIteratorThreadName(UtTracePointIterator* iter,
                                    char* buffer,
                                    uint32_t buffLen)
{
...
static uint32_t
readConsumeAndSPrintfParameter(OMRPortLibrary* portLib,
                               ...
                               int32_t isBigEndian)
{
  ...
  if (dataType == UT_TRACE_FORMATTER_64BIT_DATA) {
    ...
    u64data = getU_64FromBuffer(
      (UtTraceRecord*)rawParameterData, *offsetInParameterData, isBigEndian);
    if (nWidthAndPrecisions == 2) {
      temp = omrstr_printf(
        NULL, 0, format, precisionOrWidth1, precisionOrWidth2, u64data);
      if (temp > destBufferLength) {
        /* can't write data to dest buffer - it's too full, be cautious and just
         * return */
        UT_DBGOUT_CHECKED(1,
                          ("<UT> readConsumeAndSPrintfParameter destination "
      ...
      temp = omrstr_printf(destBuffer + (*offsetInDestBuffer),
                           destBufferLength - (*offsetInDestBuffer),
                           format,
                           precisionOrWidth1,
                           precisionOrWidth2,
                           u64data);
    ...
    *offsetInParameterData += 8;
    ...
    u32data = getU_32FromBuffer(
      (UtTraceRecord*)rawParameterData, *offsetInParameterData, isBigEndian);
    if (nWidthAndPrecisions == 2) {
      temp = omrstr_printf(
        NULL, 0, format, precisionOrWidth1, precisionOrWidth2, u32data);
      if (temp > destBufferLength) {
        /* can't write data to dest buffer - it's too full, be cautious and just
         * return */
        UT_DBGOUT_CHECKED(1,
    ...
    *offsetInParameterData += (uint32_t)temp;
    ...
    UT_DBGOUT_CHECKED(
      1,
      ("<UT> bad byte width in readConsumeAndSPrintfParameter: %d [%s]\n",
       dataType,
       format));
  }
...
static uint32_t
formatTracePointParameters(UtTracePointIterator* iter,
                           char* destBuffer,
                           uint32_t destBufferLength,
                           const char* format,
                           char* rawParameterData,
                           uint32_t rawParameterDataLength)
{
  ...
  if (destBuffer == NULL || destBufferLength == 0) {
    UT_DBGOUT_CHECKED(
      1,
      ("<UT> formatTracePointParameters called with destination buffer %p and "
       "destination buffer length %u\n",
       destBuffer,
       destBufferLength));
    return 0;
  }
      ...
      if (format[offsetInFormat + 1] == '%') {
        ...
        switch (type) {
          ...
          case 'd':
            ...
            readConsumeAndSPrintfParameter(iter->portLib,
                                           rawParameterData,
          ...
          default:
            UT_DBGOUT_CHECKED(1,
                              ("<UT> formatTracePointParameters unknown trace "
                               "format type: %c [%s]\n",
                               type,
                               formatBuffer));
            break;
...
static const char*
parseTracePoint(OMRPortLibrary* portLib,
                ...
                uint32_t bufferLength)
{
  ...
  if (bufferLength == 0 || buffer == NULL) {
    UT_DBGOUT_CHECKED(1,
                      ("<UT> parseTracePoint called with unpopulated iterator "
                       "formatted tracepoint buffer\n"));
    return NULL;
  }
  ...
  if ((traceId == 0x0000) && (tpLength == UT_TRC_SEQ_WRAP_LENGTH)) {
    ...
    tempUpper = getU_32FromBuffer(
      record,
      offset + TRACEPOINT_RAW_DATA_TIMER_WRAP_UPPER_WORD_OFFSET,
      iter->isBigEndian);
    tempUpper <<= 32;
  ...
  modNameLength =
    getU_32FromBuffer(record,
                      offset + TRACEPOINT_RAW_DATA_MODULE_NAME_LENGTH_OFFSET,
                      iter->isBigEndian);
  ...
  getTimestamp(
    splitTime + iter->startSystem, &hours, &minutes, &seconds, &millis);

  nanos = (uint32_t)((millis * ONEMILLION) +
                     (splitTimeRem * ONEMILLION / iter->timeConversion));

  offsetOfParameters =
    (uint32_t)omrstr_printf(NULL,
                            0,
                            "%02u:%02u:%02u:%09u GMT %.*s.%u - ",
                            hours,
                            minutes,
                            seconds,
                            nanos,
  ...
  if (offsetOfParameters > bufferLength) {
    UT_DBGOUT_CHECKED(1,
                      ("<UT> parseTracePoint called with buffer length %d - "
   ...
   * populate them with */
  if (!formatTracePointParameters(iter,
                                  buffer + offsetOfParameters,
                                  bufferLength - offsetOfParameters,
                                  formatString,
                                  rawParameterData,
                                  rawParameterDataLength)) {
    return NULL;
...
const char*
omr_trc_formatNextTracePoint(UtTracePointIterator* iter,
                             char* buffer,
                             uint32_t bufferLength)
{
  UtTraceRecord* record;
  ...
  if (iter->buffer == NULL) {
    UT_DBGOUT_CHECKED(1,
                      ("<UT> omr_trc_formatNextTracePoint called with "
  ...
  if ((tpLength > offset) || ((offset - tpLength + 1) < iter->start)) {
    if (iter->isCircularBuffer) {
      ...
      UT_DBGOUT_CHECKED(
        4,
        ("<UT> getNextTracePointForIterator: remainder found at end of buffer: "
         "%d tplength = %d tracedata start: %llu end %llu\n",
         remainder,
         tpLength,
         iter->start,
         iter->end));
      ...
      memcpy(iter->tempBuffForWrappedTP,
             tempChrPtr + recordDataLength - remainder,
             remainder);
      /* get the end of the tracepoint from the start of the buffer */
      memcpy(iter->tempBuffForWrappedTP + remainder,
             tempChrPtr + recordDataStart,
             tpLength - remainder);

      /* and parse the temp buffer */
      returnString = parseTracePoint(iter->portLib,
                                     (UtTraceRecord*)iter->tempBuffForWrappedTP,
                                     0,
                                     tpLength,
                                     &iter->currentUpperTimeWord,
                                     iter,
                                     buffer,
                                     bufferLength);
  ...
  return parseTracePoint(iter->portLib,
                         record,
                         offset - tpLength,
                         tpLength,
                         &iter->currentUpperTimeWord,
