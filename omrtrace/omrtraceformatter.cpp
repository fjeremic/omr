...
omr_error_t
omr_trc_getTraceFileIterator(OMRPortLibrary *portLib, char *fileName,
                             UtTraceFileIterator **iteratorPtr,
                             FormatStringCallback getFormatStringFn) {
    ...
    if (format[offsetInFormat] == '%') {
      if (format[offsetInFormat + 1] == '%') {
        ...
        case 'f':
          ...
          break;
        default:
          UT_DBGOUT_CHECKED(1, ("<UT> formatTracePointParameters unknown trace "
                                "format type: %c [%s]\n",
                                type, formatBuffer));
          break;
        }
