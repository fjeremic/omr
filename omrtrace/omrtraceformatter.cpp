...
omr_error_t omr_trc_getTraceFileIterator(
    OMRPortLibrary *portLib, char *fileName, UtTraceFileIterator **iteratorPtr,
    FormatStringCallback getFormatStringFn) {
  ...
  bytesRead =
      omrfile_read(traceFileHandle, &dummyHeader, sizeof(UtTraceFileHdr));
  ...
  if (dummyHeader.endianSignature != UT_ENDIAN_SIGNATURE) {
    /* TODO - If this is the wrong endianess we'll either need to fix up the
     * header and change the formatter to cope with files from other platforms
     * OR document that the native formatter can't cope with that.
     */
   ...
   */
  header = (UtTraceFileHdr *)omrmem_allocate_memory(dummyHeader.header.length,
  ...
  iterator = (UtTraceFileIterator *)omrmem_allocate_memory(
      sizeof(UtTraceFileIterator), OMRMEM_CATEGORY_TRACE);
  ...
  iterator->traceSection =
      ((UtTraceSection *)((char *)header + header->traceStart));
  iterator->serviceSection =
      ((UtServiceSection *)((char *)header + header->serviceStart));
  iterator->startupSection =
      ((UtStartupSection *)((char *)header + header->startupStart));
  iterator->activeSection =
      ((UtActiveSection *)((char *)header + header->activeStart));
  iterator->procSection =
      ((UtProcSection *)((char *)header + header->processorStart));
 ...
 *
 * Important: Unlike trcGetTracePointIteratorForBuffer this does not hold the
 * global trace lock until trcFreeTracePointIterator is called as we assume we
 * are working on data from a file that trace is not currently accessing.
 */
omr_error_t omr_trc_getTracePointIteratorForNextBuffer(
    UtTraceFileIterator *fileIterator,
  ...
  iterator = (UtTracePointIterator *)omrmem_allocate_memory(
      sizeof(UtTracePointIterator), OMRMEM_CATEGORY_TRACE);
  if (iterator == NULL) {
    UT_DBGOUT_CHECKED(
        1,
        ("<UT> trcGetTracePointIteratorForBuffer cannot allocate iterator\n"));
    *bufferIteratorPtr = NULL;
    return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
  }

  iterator->buffer = (OMR_TraceBuffer *)omrmem_allocate_memory(
      fileIterator->header->bufferSize + offsetof(OMR_TraceBuffer, record),
      OMRMEM_CATEGORY_TRACE);
  if (iterator->buffer == NULL) {
    UT_DBGOUT_CHECKED(1, ("<UT> trcGetTracePointIteratorForBuffer cannot "
                          "allocate iterator's buffer\n"));
  ...
  /* set up the iterator */
  bytesRead =
      omrfile_read(fileIterator->traceFileHandle, &iterator->buffer->record,
                   fileIterator->header->bufferSize);
  ...
  iterator->dataLength =
      iterator->buffer->record.nextEntry - iterator->buffer->record.firstEntry;
  iterator->currentUpperTimeWord =
      (uint64_t)(iterator->buffer->record.sequence) &
      J9CONST64(0xFFFFFFFF00000000);
  ...
  iterator->endPlatform =
      omrtime_hires_clock(); /* TODO - Is there a better timestamp we can use
                                here? */
  iterator->endSystem = ((
      uint64_t)omrtime_current_time_millis()); /* TODO - Is there a better
                                                  timestamp we can use here? */
  ...
  UT_DBGOUT_CHECKED(4,
                    ("<UT> firstEntry: %d, offset of record: %ld buffer size: "
                     "%d endianness %s\n",
                     iterator->start, offsetof(OMR_TraceBuffer, record),
                     fileIterator->header->bufferSize,
                     (iterator->isBigEndian) ? "bigEndian" : "littleEndian"));
  UT_DBGOUT_CHECKED(2, ("<UT> omr_trc_getTracePointIteratorForNextBuffer: "
                        "Thread %s returning iterator %p\n",
...
uint32_t omr_trc_getBufferIteratorThreadName(UtTracePointIterator *iter,
                                             char *buffer, uint32_t buffLen) {
...
omr_error_t omr_trc_freeTracePointIterator(UtTracePointIterator *iter) {
  if (iter != NULL) {
    ...
    UT_DBGOUT_CHECKED(
        2, ("<UT> trcFreeTracePointIterator freeing iterator %p\n", iter));
    omrmem_free_memory(iter);
  }
  return OMR_ERROR_NONE;
}

static unsigned char getUnsignedByteFromBuffer(UtTraceRecord *record,
                                               uint32_t offset) {
  char *tempPtr = (char *)record;
  return tempPtr[offset];
}

static uint32_t getTraceIdFromBuffer(UtTraceRecord *record, uint32_t offset) {
  uint32_t ret = 0;
...
static uint16_t getU_16FromBuffer(UtTraceRecord *record, uint32_t offset,
                                  int32_t isBigEndian) {
  uint16_t ret = 0;
...
static uint32_t getU_32FromBuffer(UtTraceRecord *record, uint32_t offset,
                                  int32_t isBigEndian) {
  uint32_t ret = 0;
...
static uint64_t getU_64FromBuffer(UtTraceRecord *record, uint32_t offset,
                                  int32_t isBigEndian) {
  uint64_t ret = 0;
  ...
  /* integer values are written out in platform endianess */
  if (isBigEndian) {
    ret = ((uint64_t)data[0] << 56) | ((uint64_t)data[1] << 48) |
          ((uint64_t)data[2] << 40) | ((uint64_t)data[3] << 32) |
          ((uint64_t)data[4] << 24) | ((uint64_t)data[5] << 16) |
          ((uint64_t)data[6] << 8) | ((uint64_t)data[7]);
  } else {
    ret = ((uint64_t)data[7] << 56) | ((uint64_t)data[6] << 48) |
...
static uint32_t readConsumeAndSPrintfParameter(
    OMRPortLibrary *portLib, char *rawParameterData,
    uint32_t rawParameterDataLength, uint32_t *offsetInParameterData,
    char *destBuffer, uint32_t destBufferLength, uint32_t *offsetInDestBuffer,
    const char *format, int32_t dataType, uint32_t nWidthAndPrecisions,
    int32_t isBigEndian) {
  uintptr_t temp = 0;
  uint64_t u64data = 0;
  uint32_t u32data = 0;
  ...
  if (nWidthAndPrecisions == 1) {
    precisionOrWidth1 = getU_32FromBuffer((UtTraceRecord *)rawParameterData,
                                          *offsetInParameterData, isBigEndian);
    *offsetInParameterData += 4;
  } else if (nWidthAndPrecisions == 2) {
    precisionOrWidth1 = getU_32FromBuffer((UtTraceRecord *)rawParameterData,
                                          *offsetInParameterData, isBigEndian);
    precisionOrWidth2 = getU_32FromBuffer((UtTraceRecord *)rawParameterData,
                                          *offsetInParameterData, isBigEndian);
  ...
  if (dataType == UT_TRACE_FORMATTER_64BIT_DATA) {
    ...
    u64data = getU_64FromBuffer((UtTraceRecord *)rawParameterData,
                                *offsetInParameterData, isBigEndian);
    if (nWidthAndPrecisions == 2) {
      temp = omrstr_printf(NULL, 0, format, precisionOrWidth1,
                           precisionOrWidth2, u64data);
      if (temp > destBufferLength) {
        /* can't write data to dest buffer - it's too full, be cautious and just
         * return */
        UT_DBGOUT_CHECKED(1, ("<UT> readConsumeAndSPrintfParameter destination "
                              "buffer exhausted: %d [%s]\n",
                              dataType, format));
        return 0;
      }
      temp = omrstr_printf(destBuffer + (*offsetInDestBuffer),
                           destBufferLength - (*offsetInDestBuffer), format,
                           precisionOrWidth1, precisionOrWidth2, u64data);
    ...
    *offsetInParameterData += 8;
    ...
    u32data = getU_32FromBuffer((UtTraceRecord *)rawParameterData,
                                *offsetInParameterData, isBigEndian);
    if (nWidthAndPrecisions == 2) {
      temp = omrstr_printf(NULL, 0, format, precisionOrWidth1,
                           precisionOrWidth2, u32data);
      if (temp > destBufferLength) {
        /* can't write data to dest buffer - it's too full, be cautious and just
         * return */
        UT_DBGOUT_CHECKED(1, ("<UT> readConsumeAndSPrintfParameter destination "
      ...
      temp = omrstr_printf(destBuffer + (*offsetInDestBuffer),
                           destBufferLength - (*offsetInDestBuffer), format,
                           precisionOrWidth1, precisionOrWidth2, u32data);
    } else if (nWidthAndPrecisions == 1) {
    ...
    *offsetInParameterData += 4;
    ...
    /* handling a char */
    u8data = getUnsignedByteFromBuffer((UtTraceRecord *)rawParameterData,
                                       *offsetInParameterData);
    ...
    temp =
        omrstr_printf(destBuffer + (*offsetInDestBuffer),
                      destBufferLength - (*offsetInDestBuffer), format, u8data);
    *offsetInParameterData += 1;
  } else if (dataType == UT_TRACE_FORMATTER_STRING_DATA) {
    /* handling a string */
    /* any length already read is a false one - the trace engine will have
     * modified the utf8 length field */
    if (nWidthAndPrecisions == 1) {
      *offsetInParameterData -= 4;
      i16data = (int16_t)getU_16FromBuffer((UtTraceRecord *)rawParameterData,
                                           *offsetInParameterData, isBigEndian);
...
static uint32_t formatTracePointParameters(UtTracePointIterator *iter,
                                           char *destBuffer,
                                           uint32_t destBufferLength,
                                           const char *format,
                                           char *rawParameterData,
                                           uint32_t rawParameterDataLength) {
  uint32_t formatLength;
  uint32_t offsetInFormat = 0, offsetInDestBuffer = 0,
           offsetInParameterData = 0;
  int numberOfStars = 0;
  char type = '\0';
  ...
  if (destBuffer == NULL || destBufferLength == 0) {
    UT_DBGOUT_CHECKED(
        1, ("<UT> formatTracePointParameters called with destination buffer %p "
            "and destination buffer length %u\n",
  ...
  if (format == NULL) {
    UT_DBGOUT_CHECKED(
        1, ("<UT> formatTracePointParameters called with no format string\n"));
    return 0;
  }
   ...
   */
  for (offsetInFormat = 0, offsetInDestBuffer = 0;
       offsetInFormat <= formatLength; offsetInFormat++, offsetInDestBuffer++) {
    if (format[offsetInFormat] == '%') {
      if (format[offsetInFormat + 1] == '%') {
        destBuffer[offsetInDestBuffer] = format[offsetInFormat];
        offsetInFormat++; /* skip the percent, just write a single percent to
        ...
        for (offsetOfType = 0; foundType == FALSE; offsetOfType++) {
          ...
          if (format[offsetInFormat + offsetOfType] == 'x' ||
            ...
            /* 64bit number - its actually 'll' but we'll give it benefit of
             * doubt */
        ...
        switch (type) {
          ...
          case 'd':
            ...
            readConsumeAndSPrintfParameter(
                iter->portLib, rawParameterData, rawParameterDataLength,
                &offsetInParameterData, destBuffer, destBufferLength,
                &offsetInDestBuffer, (const char *)formatBuffer, traceDataType,
                numberOfStars, iter->isBigEndian);
            break;
          case 'p':
          case 'P':
          ...
          case 's':
            ...
            break;
          case 'f':
            /* Floats are promoted to doubles by convention, so all %fs are
             * 64bit */
            traceDataType = UT_TRACE_FORMATTER_64BIT_DATA;
            readConsumeAndSPrintfParameter(
                iter->portLib, rawParameterData, rawParameterDataLength,
            ...
            break;
          default:
            UT_DBGOUT_CHECKED(1, ("<UT> formatTracePointParameters unknown "
                                  "trace format type: %c [%s]\n",
                                  type, formatBuffer));
            break;
        }
        offsetInDestBuffer--;
      ...
      if (offsetInDestBuffer > destBufferLength) {
        ...
        UT_DBGOUT_CHECKED(1,
                          ("<UT> formatTracePointParameters truncated output "
                           "due to buffer exhaustion at format type: %c [%s]\n",
                           type, formatBuffer));
...
static const char *parseTracePoint(OMRPortLibrary *portLib,
                                   UtTraceRecord *record, uint32_t offset,
                                   int tpLength,
                                   uint64_t *timeStampMostSignificantBytes,
                                   UtTracePointIterator *iter, char *buffer,
  ...
  if (bufferLength == 0 || buffer == NULL) {
    UT_DBGOUT_CHECKED(1, ("<UT> parseTracePoint called with unpopulated "
                          "iterator formatted tracepoint buffer\n"));
    return NULL;
  }
  ...
  traceId =
      getTraceIdFromBuffer(record, offset + TRACEPOINT_RAW_DATA_TP_ID_OFFSET);

  /* check for all the control/internal tracepoints */
  /* 0x0010nnnn8 == lost record tracepoint */
  if (traceId == 0x0010) {
    /* 	too much danger of looping to recurse here - return a warning string and
     let omr_trc_formatNextTracePoint(thr, iter) make the call on whether it can
     extract any further data */
    return "*** lost records found in trace buffer - use external formatting "
           "tools for details.\n";
  }

  /* 0x0000nnnn8 == sequence wrap tracepoint */
  if ((traceId == 0x0000) && (tpLength == UT_TRC_SEQ_WRAP_LENGTH)) {
    /* read in the timer upper word, write it into iterator struct, and
     * (recursively) return the next tracepoint parsed */
    tempUpper = getU_32FromBuffer(
        record, offset + TRACEPOINT_RAW_DATA_TIMER_WRAP_UPPER_WORD_OFFSET,
        iter->isBigEndian);
    tempUpper <<= 32;
  ...
  if (tpLength == 4) {
    ...
    longTracePointLength = getUnsignedByteFromBuffer(
        record, offset + TRACEPOINT_RAW_DATA_TP_ID_OFFSET + 2);
    /* the next call to omr_trc_formatNextTracePoint will pick this up and use
     * it appropriately */
  ...
  timeStampLeastSignificantBytes = getU_32FromBuffer(
      record, offset + TRACEPOINT_RAW_DATA_TIMESTAMP_OFFSET, iter->isBigEndian);
  modNameLength = getU_32FromBuffer(
      record, offset + TRACEPOINT_RAW_DATA_MODULE_NAME_LENGTH_OFFSET,
      iter->isBigEndian);

  /* Module name must not be longer than the tracepoint length - start of module
   * name string */
  /* modNameLength == 0 -- no modname - most likely partially overwritten - its
   * unformattable as a result! */
  if (modNameLength == 0 ||
      tpLength < TRACEPOINT_RAW_DATA_MODULE_NAME_DATA_OFFSET ||
      modNameLength >
          (uint32_t)(tpLength - TRACEPOINT_RAW_DATA_MODULE_NAME_DATA_OFFSET)) {
    return omr_trc_formatNextTracePoint(iter, buffer, bufferLength);
  }

  modName =
      &tempPtr[offset +
               TRACEPOINT_RAW_DATA_MODULE_NAME_DATA_OFFSET]; /* the modName was
                                                                written straight
                                                                to buf as (char
                                                                *) */
  if (!strncmp(modName, "INTERNALTRACECOMPONENT", 22)) {
    ...
    traceId -= 257;
    /* temporarily add NULL terminator to the modName in copy of the buffer for
     * getFormatString to be able to do look up on it - save the overwritten
     * data and reinstate once finished. Doing this avoids allocing a temp
     * buffer. We also need to check to see if this is a composite name, e.g.
     * pool(j9mm). If it is then we need to take the string before the opening
     * brace as the component name. We don't do this in getFormatString because
     * that's in the fastpath for -Xtrace:print
     */
  ...
  /* this formula is taken directly from the trace formatter to maintain
   *agreement between representations made by this function and those made by
   *the TraceFormat tool. */
  timeStamp -= iter->startPlatform;

  splitTime = timeStamp / iter->timeConversion;
  splitTimeRem = timeStamp % iter->timeConversion;

  getTimestamp(splitTime + iter->startSystem, &hours, &minutes, &seconds,
               &millis);

  nanos = (uint32_t)((millis * ONEMILLION) +
                     (splitTimeRem * ONEMILLION / iter->timeConversion));

  offsetOfParameters = (uint32_t)omrstr_printf(
      NULL, 0, "%02u:%02u:%02u:%09u GMT %.*s.%u - ", hours, minutes, seconds,
      nanos, modNameLength, modNameString, traceId);

  if (offsetOfParameters > bufferLength) {
    UT_DBGOUT_CHECKED(1, ("<UT> parseTracePoint called with buffer length %d - "
                          "too small for tracepoint details\n",
                          bufferLength));
    return NULL;
  }

  offsetOfParameters = (uint32_t)omrstr_printf(
      buffer, bufferLength, "%02u:%02u:%02u:%09u GMT %.*s.%u - ", hours,
      minutes, seconds, nanos, modNameLength, modNameString, traceId);

  rawParameterData = tempPtr + offset +
                     TRACEPOINT_RAW_DATA_MODULE_NAME_DATA_OFFSET +
                     modNameLength;
  rawParameterDataLength =
      tpLength - (TRACEPOINT_RAW_DATA_MODULE_NAME_DATA_OFFSET + modNameLength);

  /* the parameters will be formatted as question marks if there is no data to
   * populate them with */
  if (!formatTracePointParameters(
          iter, buffer + offsetOfParameters, bufferLength - offsetOfParameters,
...
const char *omr_trc_formatNextTracePoint(UtTracePointIterator *iter,
  ...
  if (iter == NULL) {
    UT_DBGOUT_CHECKED(
        1, ("<UT> omr_trc_formatNextTracePoint called with NULL iterator\n"));
    return NULL;
  }

  if (iter->buffer == NULL) {
    UT_DBGOUT_CHECKED(1, ("<UT> omr_trc_formatNextTracePoint called with "
                          "unpopulated iterator buffer\n"));
    return NULL;
  }

  if (iter->currentPos <= iter->start) {
    /* currentPosition is at or before the start of the data - not possible to
     *continue
     *	or the normal exit condition - either way NULL is the right thing to
     *return */
    return NULL;
  }
  ...
  /* long trace points have length stored in 2bytes, so check we've not
   * overflowed that limit
   */
   ...
   * the current tracepoint length takes us back before where formatting
   * originally started in this buffer, then we have found the partially
   * overwritten tracepoint that means we have got to the end of this buffer. */
  if ((iter->isCircularBuffer) && iter->iteratorHasWrapped &&
      ((offset - tpLength) < iter->end)) {
    return NULL;
  }
   ...
   * 'offset - tpLength' points to the previous tracepoint's 'tpLength', if it
   * exists. 'offset - tpLength + 1' points to the start of the current
   * tracepoint's data, which must be at least 'record->firstEntry' or
   * 'iter->start' if the tracepoint fits in this buffer.
   */
  if ((tpLength > offset) || ((offset - tpLength + 1) < iter->start)) {
    if (iter->isCircularBuffer) {
      ...
      UT_DBGOUT_CHECKED(
          4, ("<UT> getNextTracePointForIterator: remainder found at end of "
              "buffer: %d tplength = %d tracedata start: %llu end %llu\n",
              remainder, tpLength, iter->start, iter->end));

      /* set the iterator's position to be before the incomplete data at the end
       * of the buffer */
      iter->currentPos = iter->recordLength - remainder;

      /* check if the wrapped tracepoint takes us back past the tracepoint data
       * end point (i.e. record.nextEntry) */
      ...
      if (iter->tempBuffForWrappedTP == NULL) {
        UT_DBGOUT_CHECKED(
            1, ("<UT> omr_trc_formatNextTracePoint: cannot allocate %d bytes\n",
                tpLength + 2));
        return NULL;
      }
      ...
      memcpy(iter->tempBuffForWrappedTP,
             tempChrPtr + recordDataLength - remainder, remainder);
      /* get the end of the tracepoint from the start of the buffer */
      memcpy(iter->tempBuffForWrappedTP + remainder,
             tempChrPtr + recordDataStart, tpLength - remainder);

      /* and parse the temp buffer */
      returnString = parseTracePoint(
          iter->portLib, (UtTraceRecord *)iter->tempBuffForWrappedTP, 0,
          tpLength, &iter->currentUpperTimeWord, iter, buffer, bufferLength);

      UT_DBGOUT_CHECKED(1, ("<UT> getNextTracePointForIterator: recombined a "
                            "tracepoint into %s\n",
      ...
      /* the tracepoint is in another buffer and some magic is needed elsewhere
       * to process */
  ...
  return parseTracePoint(iter->portLib, record, offset - tpLength, tpLength,
                         &iter->currentUpperTimeWord, iter, buffer,
                         bufferLength);
}
