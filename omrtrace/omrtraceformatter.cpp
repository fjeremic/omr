...
omr_error_t omr_trc_getTraceFileIterator(
  OMRPortLibrary* portLib, char* fileName, UtTraceFileIterator** iteratorPtr, FormatStringCallback getFormatStringFn)
{
...
omr_error_t omr_trc_getTracePointIteratorForNextBuffer(
  UtTraceFileIterator* fileIterator, UtTracePointIterator** bufferIteratorPtr)
{
  ...
  iterator->endSystem
    = ((uint64_t)omrtime_current_time_millis()); /* TODO - Is there a better timestamp we can use here? */
...
static unsigned char getUnsignedByteFromBuffer(UtTraceRecord* record, uint32_t offset)
{
...
static uint16_t getU_16FromBuffer(UtTraceRecord* record, uint32_t offset, int32_t isBigEndian)
{
  uint16_t ret = 0;
...
static uint32_t getU_32FromBuffer(UtTraceRecord* record, uint32_t offset, int32_t isBigEndian)
{
  uint32_t ret = 0;
...
static uint64_t getU_64FromBuffer(UtTraceRecord* record, uint32_t offset, int32_t isBigEndian)
{
  uint64_t ret = 0;
...
static uint32_t readConsumeAndSPrintfParameter(OMRPortLibrary* portLib, char* rawParameterData,
  uint32_t rawParameterDataLength, uint32_t* offsetInParameterData, char* destBuffer, uint32_t destBufferLength,
  uint32_t* offsetInDestBuffer, const char* format, int32_t dataType, uint32_t nWidthAndPrecisions, int32_t isBigEndian)
{
  uintptr_t temp = 0;
  uint64_t u64data = 0;
  uint32_t u32data = 0;
  ...
  if (dataType == UT_TRACE_FORMATTER_64BIT_DATA) {
    ...
    if (nWidthAndPrecisions == 2) {
      ...
      if (temp > destBufferLength) {
        ...
        UT_DBGOUT_CHECKED(
          1, ("<UT> readConsumeAndSPrintfParameter destination buffer exhausted: %d [%s]\n", dataType, format));
        return 0;
      }
      temp = omrstr_printf(destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format,
      ...
      temp
        = omrstr_printf(destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format, u64data);
    }
    *offsetInParameterData += 8;
    ...
    if (nWidthAndPrecisions == 2) {
      ...
      temp
        = omrstr_printf(destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format, u32data);
    }
    *offsetInParameterData += 4;
    ...
    temp = omrstr_printf(destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format, u8data);
    *offsetInParameterData += 1;
    ...
    if (nWidthAndPrecisions == 1) {
      ...
      *offsetInParameterData += 2;
      ...
      temp = omrstr_printf(
        destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format, i16data, strValue);
    } else {
      ...
      temp
        = omrstr_printf(destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format, strValue);
...
static uint32_t formatTracePointParameters(UtTracePointIterator* iter, char* destBuffer, uint32_t destBufferLength,
  const char* format, char* rawParameterData, uint32_t rawParameterDataLength)
{
      ...
      if (format[offsetInFormat + 1] == '%') {
        ...
        for (offsetOfType = 0; foundType == FALSE; offsetOfType++) {
          ...
          if (format[offsetInFormat + offsetOfType] == 'x' || format[offsetInFormat + offsetOfType] == 'X'
            || format[offsetInFormat + offsetOfType] == 'u' || format[offsetInFormat + offsetOfType] == 'i'
            || format[offsetInFormat + offsetOfType] == 'd' || format[offsetInFormat + offsetOfType] == 'f'
            || format[offsetInFormat + offsetOfType] == 'p' || format[offsetInFormat + offsetOfType] == 'P'
            || format[offsetInFormat + offsetOfType] == 'c' || format[offsetInFormat + offsetOfType] == 's') {
            /* we found the type */
            foundType = TRUE;
        ...
        case 'd':
          ...
          readConsumeAndSPrintfParameter(iter->portLib, rawParameterData, rawParameterDataLength,
            &offsetInParameterData, destBuffer, destBufferLength, &offsetInDestBuffer, (const char*)formatBuffer,
        ...
        default:
          UT_DBGOUT_CHECKED(
            1, ("<UT> formatTracePointParameters unknown trace format type: %c [%s]\n", type, formatBuffer));
          break;
        }
        offsetInDestBuffer--;
      }
...
static const char* parseTracePoint(OMRPortLibrary* portLib, UtTraceRecord* record, uint32_t offset, int tpLength,
  uint64_t* timeStampMostSignificantBytes, UtTracePointIterator* iter, char* buffer, uint32_t bufferLength)
{
  ...
  if ((traceId == 0x0000) && (tpLength == UT_TRC_SEQ_WRAP_LENGTH)) {
     ...
     */
    tempUpper = getU_32FromBuffer(record, offset + TRACEPOINT_RAW_DATA_TIMER_WRAP_UPPER_WORD_OFFSET, iter->isBigEndian);
    tempUpper <<= 32;
  ...
  modNameLength = getU_32FromBuffer(record, offset + TRACEPOINT_RAW_DATA_MODULE_NAME_LENGTH_OFFSET, iter->isBigEndian);
  ...
  modName = &tempPtr[offset
    + TRACEPOINT_RAW_DATA_MODULE_NAME_DATA_OFFSET]; /* the modName was written straight to buf as (char *) */
  ...
  offsetOfParameters = (uint32_t)omrstr_printf(NULL, 0, "%02u:%02u:%02u:%09u GMT %.*s.%u - ", hours, minutes, seconds,
    nanos, modNameLength, modNameString, traceId);

  if (offsetOfParameters > bufferLength) {
    UT_DBGOUT_CHECKED(
      1, ("<UT> parseTracePoint called with buffer length %d - too small for tracepoint details\n", bufferLength));
    return NULL;
  }
...
const char* omr_trc_formatNextTracePoint(UtTracePointIterator* iter, char* buffer, uint32_t bufferLength)
{
  ...
  if ((tpLength > offset) || ((offset - tpLength + 1) < iter->start)) {
    if (iter->isCircularBuffer) {
      ...
      UT_DBGOUT_CHECKED(4,
        ("<UT> getNextTracePointForIterator: remainder found at end of buffer: %d tplength = %d tracedata start: %llu "
         "end %llu\n",
  ...
  return parseTracePoint(
    iter->portLib, record, offset - tpLength, tpLength, &iter->currentUpperTimeWord, iter, buffer, bufferLength);
}
