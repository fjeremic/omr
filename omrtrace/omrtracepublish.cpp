...
omr_error_t publishTraceBuffer(OMR_TraceThread *currentThr,
   ...
   */
  if (NULL != buf->thr) {
    /* CAS is not currently needed because we always publish buffers
     * synchronously from the thread that owns the trace buffer.
     */
    buf->thr->trcBuf = NULL;
  }
  ...
  if ((bufFlags & UT_TRC_BUFFER_ACTIVE) && !(bufFlags & UT_TRC_BUFFER_NEW)) {
    const uint32_t newFlags =
        (bufFlags & (~(UT_TRC_BUFFER_ACTIVE | UT_TRC_BUFFER_NEW))) |
        UT_TRC_BUFFER_FULL;
    /* CAS is not needed because flags is modified only by the thread that owns
     * the buffer */
    buf->flags = newFlags;

    omrthread_monitor_t const subscribersLock =
        OMR_TRACEGLOBAL(subscribersLock);
    omrthread_monitor_enter(subscribersLock);
    for (UtSubscription *subscription =
             (UtSubscription *)OMR_TRACEGLOBAL(subscribers);
...
omr_error_t releaseTraceBuffer(OMR_TraceThread *currentThr,
   ...
   */
  if (NULL != buf->thr) {
    /* CAS is not currently needed because we always release buffers
     * synchronously from the thread that owns the trace buffer.
     */
    buf->thr->trcBuf = NULL;
  }
