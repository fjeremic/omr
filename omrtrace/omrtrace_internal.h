 ...
 * =============================================================================
 */
/* Enable logging to separate exception trace buffers. This is not useful
 * because the exception trace buffers can't be sent to any subscribers.
 */
...
#define UT_TRC_BUFFER_FULL \
  0x00000001 /* indicates a buffer that has been published */
#define UT_TRC_BUFFER_NEW                                                      \
  0x20000000 /* indicates an empty new buffer in use by a thread. cleared when \
                buffer is written to. */
#define UT_TRC_BUFFER_ACTIVE \
  0x80000000 /* indicates a buffer in use by a thread */
...
/* The following structures will form the core of the new trace control
 * mechanisms. */
...
#define OMR_TRACE_ENGINE_IS_ENABLED(initState)  \
  (((initState) >= OMR_TRACE_ENGINE_ENABLED) && \
   ((initState) <= OMR_TRACE_ENGINE_SHUTDOWN_STARTED))
...
struct OMR_TraceGlobal {
  ...
  omrthread_monitor_t subscribersLock;  /* Enforces atomicity of updates to the
                                           list of external trace subscribers */
  int32_t traceInCore; /* If true then we don't queue buffers */
  volatile uint32_t
      allocatedTraceBuffers; /* The number of allocated trace buffers ????*/
  ...
  omrthread_monitor_t
      bufferPoolLock; /* Lock for buffer pool. Do not allow tracepoints while
                         locking, holding, or releasing this monitor. */
  J9Pool *threadPool; /* Pool for allocating all UtThreadData */
  omrthread_monitor_t
      threadPoolLock; /* Lock for thread pool. Do not allow tracepoints while
                         locking, holding, or releasing this monitor. */
};
...
omr_error_t setTracePointsToParsed(const char *componentName,
                                   ...
                                   unsigned char value, int level,
                                   const char *groupName,
                                   BOOLEAN suppressMessages, int32_t setActive);
char *getFormatString(const char *componentName, int32_t tracepoint);
char *getTracePointName(const char *componentName,
                        UtComponentList *componentList, int32_t tracepoint);
...
omr_error_t addTraceConfigKeyValuePair(OMR_TraceThread *thr, const char *cmdKey,
                                       const char *cmdValue);

/*
 * =============================================================================
 *  Functions called by users of the trace library at initialisation/shutdown
 * time. (Runtime functions are called vi OMR_TraceInterface once initialized)
 * =============================================================================
 */
 ...
 * This function fills in the function tables that an application can use to
 * access the trace engine functions.
 *
 * OMR_TraceInterface contains the functions the application can use to access
 * and control the trace engine.
 *
 * UtModuleInterface contains the functions used by a module (library) to
 * initialize it's trace setup and to take trace points. These functions are
 * usually accessed via the macros generated from the TDF files.
 *
 * @param[in,out] utIntf Will be returned a pointer to a newly initialized
 * UtInterface structure.
 * @param[in,out] omrTraceIntf
 * @param[in,out] utModuleIntf
 *
 * @return OMR_ERROR_NONE if successful, an error code describing the failure
 * otherwise.
 */
 ...
 * @param[in]     languageIntf A set of function pointers for accessing function
 * in the OMR based runtime.
 *
 * @return OMR_ERROR_NONE if successful, an error code describing the failure
 * otherwise.
 */
 ...
 * @param[in]  threadId The id of this thread. Must be unique. (The address of a
 * per thread structure is recommended)
 * @param[in]  threadName A name for this thread. Will be displayed when trace
 * data is formatted. May be NULL.
 * @param[in]  thrSynonym1 A synonym for this threads id to aid cross
 * referencing in problem diagnosis.
 * @param[in]  thrSynonym2 A synonym for this threads id to aid cross
 * referencing in problem diagnosis.
 *
 * @return OMR_ERROR_NONE if successful, an error code describing the failure
 * otherwise.
...
omr_error_t threadStart(OMR_TraceThread **thr, const void *threadId,
                        const char *threadName, const void *thrSynonym1,
 ...
 *
 * @param[in] thr The OMR_TraceThread for the currently executing thread. Must
 * not be NULL.
 ...
 */
omr_error_t setOptions(OMR_TraceThread *thr, const char **opts,
                       BOOLEAN atRuntime);

/**
 * @brief Set the directory to search for trace format files.
 * @param[in] datDir Path of the directory containing trace format files
 * (*TraceFormat.dat).
 ...
 *
 * Creates a trace point to record a trace event for the specified thread
 * (generally the currently running thread). The trace point is in the module
 * described by modInfo and has it's trace id masked with it's activation
 * information. (This is generated as part of the trace macros and should simply
 * be passed through.)
 *
 * The spec parameter describes the arguments to the trace point (and is also
 * generated) and those arguments (if any) are contained in the va_list.
 *
 * This function exists to allow users to override the default implementation of
 * UtModuleInterface.Trace(void *env, UtModuleInfo *modInfo, uint32_t traceId,
 * const char *spec, ...); and pass in the var args arguments (since the use of
 * var args prevents simply calling the original UtModuleInterface.Trace
 * function). This allows trace implementors to map from a per thread env
 * parameter that is convenient for their application to a OMR_TraceThread. All
 * the other parameters should be left unchanged.
 *
 * @param[in] thr The OMR_TraceThread for the currently executing thread. Must
 * not be NULL.
 * @param[in] modInfo A pointer to the UtModuleInfo for the module this trace
 * point belongs to.
 ...
 */
void doTracePoint(OMR_TraceThread *thr, UtModuleInfo *modInfo, uint32_t traceId,
                  const char *spec, va_list varArgs);
...
omr_error_t processOptions(OMR_TraceThread *thr, const char **opts,
                           BOOLEAN atRuntime);
omr_error_t expandString(char *returnBuffer, const char *original,
                         BOOLEAN atRuntime);
int hexStringLength(const char *str);
void getTimestamp(int64_t time, uint32_t *pHours, uint32_t *pMinutes,
                  uint32_t *pSeconds, uint32_t *pMillis);
...
omr_error_t getComponentGroup(char *name, char *group, int32_t *count,
                              int32_t **tracePts);
void internalTrace(OMR_TraceThread *thr, UtModuleInfo *modInfo,
                   uint32_t traceId, const char *spec, ...);
/**************************************************************************
...
/** Functions exposed to the outside world via the server interface and used in
 * rastrace code outside of main.c All functions on the server interface (and
 * only functions on the server interface) start with trc **/
omr_error_t trcRegisterRecordSubscriber(OMR_TraceThread *thr,
                                        const char *description,
                                        utsSubscriberCallback subscriber,
                                        utsSubscriberAlarmCallback alarm,
                                        void *userData,
                                        UtSubscription **subscriptionReference);
omr_error_t trcDeregisterRecordSubscriber(OMR_TraceThread *thr,
                                          UtSubscription *subscriptionID);

/** Functions exposed to the outside world via the module interface and used
 * outside main.c All functions on the module interface (and only functions on
 * the module interface) start with j9 **/
void omrTrace(void *env, UtModuleInfo *modInfo, uint32_t traceId,
              const char *spec, ...);

/**
 * @brief Publish a trace buffer.
 * @param[in] currentThr The current thread. Might not be the thread that wrote
 * buf.
 * @param[in] buf The trace buffer to publish.
 * @return an OMR error code
 */
omr_error_t publishTraceBuffer(OMR_TraceThread *currentThr,
                               OMR_TraceBuffer *buf);
 ...
 * @return an OMR error code
 */
omr_error_t releaseTraceBuffer(OMR_TraceThread *currentThr,
                               OMR_TraceBuffer *buf);
