...
#define UT_ASSERT(expr)                                                        \
  do {                                                                         \
    if (!(expr)) {                                                             \
      fprintf(                                                                 \
        stderr, "UT_ASSERT FAILED: %s at %s:%d\n", #expr, __FILE__, __LINE__); \
      *((int*)0) = 42;                                                         \
    }                                                                          \
  } while (0)
...
#define OMR_TRACE_ENGINE_IS_ENABLED(initState)                                 \
  ...
  struct OMR_TraceGlobal
  {
    ...
    volatile UtSubscription*
      subscribers; /* List of external trace subscribers */
    omrthread_monitor_t
      subscribersLock; /* Enforces atomicity of updates to the list of external
                          trace subscribers */
  ...
  omr_error_t addTraceConfigKeyValuePair(OMR_TraceThread* thr,
                                         const char* cmdKey,
                                         const char* cmdValue);
  ...
  /**
   * @brief Fill in the interfaces the runtime will use to access trace
   * functions.
   *
   ...
   * @param[in]     languageIntf A set of function pointers for accessing
   * function in the OMR based runtime.
   *
   * @return OMR_ERROR_NONE if successful, an error code describing the failure
   * otherwise.
   */
   ...
   * @param[in]  threadId The id of this thread. Must be unique. (The address of
   * a per thread structure is recommended)
   ...
   */
  omr_error_t setOptions(OMR_TraceThread* thr,
                         const char** opts,
                         BOOLEAN atRuntime);
   ...
   * information. (This is generated as part of the trace macros and should
   ...
   *
   * This function exists to allow users to override the default implementation
   * of UtModuleInterface.Trace(void *env, UtModuleInfo *modInfo, uint32_t
   * traceId, const char *spec, ...); and pass in the var args arguments (since
   * the use of var args prevents simply calling the original
   * UtModuleInterface.Trace function). This allows trace implementors to map
   * from a per thread env parameter that is convenient for their application to
   * a OMR_TraceThread. All the other parameters should be left unchanged.
   *
   * @param[in] thr The OMR_TraceThread for the currently executing thread. Must
   * not be NULL.
   ...
   * @param[in] spec    The trace point format specification for this trace
   * point
   * @param[in] varArgs The va_list of parameters for this trace point.
   */
  void doTracePoint(OMR_TraceThread* thr,
  ...
  omr_error_t processOptions(OMR_TraceThread* thr,
                             const char** opts,
                             BOOLEAN atRuntime);
  omr_error_t expandString(char* returnBuffer,
                           const char* original,
                           BOOLEAN atRuntime);
  ...
  omr_error_t getComponentGroup(char* name,
                                char* group,
                                int32_t* count,
                                int32_t** tracePts);
  void internalTrace(OMR_TraceThread* thr,
                     UtModuleInfo* modInfo,
                     uint32_t traceId,
                     const char* spec,
                     ...);
  ...
  /** Functions exposed to the outside world via the server interface and used
   * in rastrace code outside of main.c All functions on the server interface
   * (and only functions on the server interface) start with trc **/
   ...
   * @param[in] currentThr The current thread. Might not be the thread that
   * wrote buf.
