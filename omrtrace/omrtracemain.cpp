...
omr_error_t moduleLoaded(OMR_TraceThread* thr, UtModuleInfo* modInfo)
{
    ...
    if (modInfo->traceVersionInfo == NULL) {
        ...
        UT_DBGOUT(1,
            ("<UT> ModuleLoaded refusing registration to %s because it's version is less than the supported UT version "
             "%d\n",
...
omr_error_t moduleUnLoading(OMR_TraceThread* thr, UtModuleInfo* modInfo)
{
    ...
    if (NULL == modInfo->traceVersionInfo) {
        /* this is a pre 142 module - not compatible with this trace engine fail silently to register this module */
        UT_DBGOUT(1,
            ("<UT> ModuleLoaded refusing deregistration to %s because it's version is less than the supported UT "
             "version %d\n",
                modInfo->name, UT_VERSION));
        return OMR_ERROR_NONE;
    } /* else {
...
omr_error_t threadStop(OMR_TraceThread** thrSlot)
{
    ...
    *thrSlot = NULL;
    ...
    if (0 == newCount) {
         ...
         * utTerminateTrace()) atomic decr threadCount, new threadCount is 1 skip freeTrace() because threadCount > 0
        ...
        if (OMR_TRACE_ENGINE_SHUTDOWN_STARTED == OMR_TRACEGLOBAL(initState)) {
            if (OMR_TRACEGLOBAL(traceDebug) >= 2) {
                omrtty_err_printf(
                    "<UT> threadStop entered for final thread " UT_POINTER_SPEC ", freeing buffers\n", thr);
            }
            freeTrace();
        }
    }
 ...
 */
omr_error_t threadStart(OMR_TraceThread** thr, const void* threadId, const char* threadName, const void* threadSynonym1,
    const void* threadSynonym2)
{
    ...
    *thr = newThr;
    ...
    UT_DBGOUT(2,
        ("<UT> thread Id " UT_POINTER_SPEC ", thread name \"%s\", syn1 " UT_POINTER_SPEC ", syn2 " UT_POINTER_SPEC
         " \n",
