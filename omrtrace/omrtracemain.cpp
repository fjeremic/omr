 ...
 ******************************************************************************/
omr_error_t moduleLoaded(OMR_TraceThread *thr, UtModuleInfo *modInfo) {
  UtComponentData *compData = NULL;
  omr_error_t rc = OMR_ERROR_NONE;

  if ((NULL == thr) &&
      (OMR_TRACE_ENGINE_MT_ENABLED == OMR_TRACEGLOBAL(initState))) {
  ...
  if (modInfo->traceVersionInfo == NULL) {
    /* this is a pre 142 module - not compatible with this trace engine fail
     * silently to register this module */
    UT_DBGOUT(1, ("<UT> ModuleLoaded refusing registration to %s because it's "
                  "version is less than the supported UT version %d\n",
                  modInfo->name, UT_VERSION));
    return OMR_ERROR_NONE;
  } /* else {
          this field contains the version number and can be used to modify
  behaviour based on level of module loaded
  } */
  ...
  if (modInfo->intf == NULL) {
    ...
    if (OMR_ERROR_NONE == rc) {
      rc = processComponentDefferedConfig(compData,
    ...
    modInfo->intf = internalUtIntfS.module;
    /* this module has already been registered. Just increment the reference
     * count */
 ...
 * returns     - void
 ******************************************************************************/
omr_error_t moduleUnLoading(OMR_TraceThread *thr, UtModuleInfo *modInfo) {
  omr_error_t rc = OMR_ERROR_NONE;

  /* A module can be unloaded in shutdown phase, as long as the current thread
   * is attached */
  ...
  if (NULL == modInfo->traceVersionInfo) {
    /* this is a pre 142 module - not compatible with this trace engine fail
     * silently to register this module */
    UT_DBGOUT(1, ("<UT> ModuleLoaded refusing deregistration to %s because "
                  "it's version is less than the supported UT version %d\n",
                  modInfo->name, UT_VERSION));
    return OMR_ERROR_NONE;
  } /* else {
          this field contains the version number and can be used to modify
  ...
  if (modInfo->referenceCount > 0) {
    ...
    rc = setTracePointsTo(modInfo->name, OMR_TRACEGLOBAL(componentList), TRUE,
                          0, 0, 0, -1, NULL, FALSE, TRUE);
    if (OMR_ERROR_NONE != rc) {
      UT_DBGOUT(1, ("<UT> problem turning off trace in %s as it unloads\n",
                    modInfo->name));
      /* proceed to remove it from list in case it has gone and we try and
       * manipulate it's control array */
    }
 ...
 * returns     - void
 ******************************************************************************/
omr_error_t threadStop(OMR_TraceThread **thrSlot) {
  ...
  UT_DBGOUT(
      3,
      ("<UT> ThreadStop entered for thread anchor " UT_POINTER_SPEC "\n", thr));
  ...
  if (NULL != trcBuf) {
    if (!OMR_TRACEGLOBAL(traceInCore)) {
      ...
      UT_DBGOUT(3, ("<UT> Purging buffer " UT_POINTER_SPEC
  ...
  /*
   * Mark the thread detached from the trace engine. No more tracepoints after
   * this.
   */
  *thrSlot = NULL;
  ...
  uintptr_t newCount =
      VM_AtomicSupport::subtractU32(&OMR_TRACEGLOBAL(threadCount), 1);

  /*
   * Last thread out frees the trace engine, including OMR_TraceGlobal and the
   * pool of trace buffers.
   *
   * If this is not the last thread out, another thread can start deleting the
   * trace engine now. We must not access the trace engine after this.
   */
  if (0 == newCount) {
    /*
     * Don't allow initState to be read before threadCount. Prevents this thread
     * from missing whether another thread started trace shutdown.
     *
     * e.g.
     * (this thread)						(other thread)
     * read initState, MT_ENABLED
     * 										set initState to SHUTDOWN_STARTED (from
     * utTerminateTrace()) atomic decr threadCount, new threadCount is 1 skip
     * freeTrace() because threadCount > 0 atomic decr threadCount, new
     * threadCount is 0 skip freeTrace() because initState is not
     * SHUTDOWN_STARTED
     */
    ...
    if (OMR_TRACE_ENGINE_SHUTDOWN_STARTED == OMR_TRACEGLOBAL(initState)) {
      if (OMR_TRACEGLOBAL(traceDebug) >= 2) {
        omrtty_err_printf(
            "<UT> threadStop entered for final thread " UT_POINTER_SPEC
            ", freeing buffers\n",
            thr);
      }
      freeTrace();
...
omr_error_t utTerminateTrace(void) {
  ...
  /* This is not threadsafe. We don't expect this function to be called
   * concurrently. */
  ...
  if (OMR_TRACEGLOBAL(lostRecords) != 0) {
    UT_DBGOUT(
        1, ("<UT> Discarded %d trace buffers\n", OMR_TRACEGLOBAL(lostRecords)));
  }
  return result;
}
...
void freeTrace(void) {
  ...
  if (OMR_TRACEGLOBAL(initState) < OMR_TRACE_ENGINE_SHUTDOWN_STARTED) {
    ...
    UT_DBGOUT(
        1, ("<UT> Error: freeTrace called before trace has been finalized\n"));
  }

  /*
   * Set omrTraceglobal to NULL.
   * This prevents new threads from attaching to the trace engine, and new
   * modules from being loaded. OMR_TRACEGLOBAL() macro can't be used after
   * this.
   *
   * OMRTODO
   * We still have a problem where we fail to notice that a concurrent thread is
   * in the process of attaching, and start deleting omrTraceGlobal from under
   * it.
   */
  omrTraceGlobal = NULL;
   ...
   *
   * The UtModuleInfo is normally cleared when a module is unloaded, see
   * moduleUnLoading().
   */
 ...
 */
omr_error_t threadStart(OMR_TraceThread **thr, const void *threadId,
                        const char *threadName, const void *threadSynonym1,
                        const void *threadSynonym2) {
  omr_error_t rc = OMR_ERROR_NONE;
  OMRPORT_ACCESS_FROM_OMRPORT(OMR_TRACEGLOBAL(portLibrary));

  omrthread_monitor_enter(OMR_TRACEGLOBAL(threadPoolLock));
  OMR_TraceThread *newThr =
      (OMR_TraceThread *)pool_newElement(OMR_TRACEGLOBAL(threadPool));
  if (NULL == newThr) {
  ...
  if (NULL == threadName) {
    ...
    /* Obtain storage, copy the thread name, and reference it off
     * OMR_TraceThread */
    char *tempName = (char *)omrmem_allocate_memory(strlen(threadName) + 1,
  ...
  *thr = newThr;
  UT_DBGOUT(
      2, ("<UT> Thread started , thread anchor " UT_POINTER_SPEC "\n", *thr));
  UT_DBGOUT(2, ("<UT> thread Id " UT_POINTER_SPEC
                ", thread name \"%s\", syn1 " UT_POINTER_SPEC
...
omr_error_t initializeTrace(OMR_VM *vm, const char *datDir,
                            const OMR_TraceLanguageInterface *languageIntf)

{
  ...
  omrTraceGlobal = &tempGbl;
  {
    ...
    if (envString != NULL) {
      if (hexStringLength(envString) == 1 && *envString >= '0' &&
          *envString <= '9') {
   ...
   */
  if (0 != omrthread_monitor_init_with_name(&OMR_TRACEGLOBAL(traceLock), 0,
                                            "Global Trace")) {
    UT_DBGOUT(1, ("<UT> Initialization of traceLock failed\n"));
    rc = OMR_ERROR_FAILED_TO_ALLOCATE_MONITOR;
    goto fail;
  }
  if (0 != omrthread_monitor_init_with_name(&OMR_TRACEGLOBAL(subscribersLock),
                                            0, "Global Record Subscribers")) {
    UT_DBGOUT(1, ("<UT> Initialization of subscribersLock failed\n"));
    rc = OMR_ERROR_FAILED_TO_ALLOCATE_MONITOR;
    goto fail;
  }
  if (0 != omrthread_monitor_init_with_name(&OMR_TRACEGLOBAL(freeQueueLock), 0,
                                            "Global Trace Free Queue")) {
    UT_DBGOUT(1, ("<UT> Initialization of freeQueueLock failed\n"));
    rc = OMR_ERROR_FAILED_TO_ALLOCATE_MONITOR;
    goto fail;
  }
  if (0 != omrthread_monitor_init_with_name(&OMR_TRACEGLOBAL(bufferPoolLock), 0,
                                            "Global Trace Buffer Pool")) {
    UT_DBGOUT(1, ("<UT> Initialization of bufferPoolLock failed\n"));
    rc = OMR_ERROR_FAILED_TO_ALLOCATE_MONITOR;
    goto fail;
  }
  if (0 != omrthread_monitor_init_with_name(&OMR_TRACEGLOBAL(threadPoolLock), 0,
                                            "Global Trace Thread Pool")) {
  ...
  newGbl = (OMR_TraceGlobal *)omrmem_allocate_memory(sizeof(OMR_TraceGlobal),
                                                     OMRMEM_CATEGORY_TRACE);
  if (newGbl == NULL) {
  ...
  if (NULL != languageIntf) {
    memcpy(&OMR_TRACEGLOBAL(languageIntf), languageIntf,
           sizeof(OMR_TRACEGLOBAL(languageIntf)));
  } else {
    memset(&OMR_TRACEGLOBAL(languageIntf), 0,
           sizeof(OMR_TRACEGLOBAL(languageIntf)));
  }

  /* Initialize locks and pools for buffers and thread data. */
  OMR_TRACEGLOBAL(bufferPool) = pool_new(
      OMR_TRACEGLOBAL(bufferSize) + offsetof(OMR_TraceBuffer, record), 0, 0, 0,
  ...
  OMR_TRACEGLOBAL(threadPool) =
      pool_new(sizeof(OMR_TraceThread), 0, 0, 0, OMR_GET_CALLSITE(),
               OMRMEM_CATEGORY_TRACE, POOL_FOR_PORT(OMRPORTLIB));
 ...
 *subscription start: -1 means head, NULL means tail, anything else is a buffer
 *               stop: NULL means don't stop, anything else is a buffer
 * returns     - Success or error code
 ******************************************************************************/
omr_error_t trcRegisterRecordSubscriber(
    OMR_TraceThread *thr, const char *description,
    utsSubscriberCallback subscriber, utsSubscriberAlarmCallback alarm,
    void *userData, UtSubscription **subscriptionReference) {
  omr_error_t result = OMR_ERROR_NONE;
  ...
  OMRPORT_ACCESS_FROM_OMRPORT(OMR_TRACEGLOBAL(portLibrary));
  UtSubscription *subscription = (UtSubscription *)omrmem_allocate_memory(
      sizeof(UtSubscription), OMRMEM_CATEGORY_TRACE);
  if (subscription == NULL) {
    UT_DBGOUT(1, ("<UT thr=" UT_POINTER_SPEC
                  "> Out of memory allocating subscription\n",
                  thr));
    return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
  }
  ...
  UT_DBGOUT(
      5,
      ("<UT thr=" UT_POINTER_SPEC "> Acquiring lock for registration\n", thr));
  ...
  subscription->description = (char *)omrmem_allocate_memory(
      sizeof(char) * (descriptionLen + 1), OMRMEM_CATEGORY_TRACE);
  if (subscription->description == NULL) {
    UT_DBGOUT(1, ("<UT thr=" UT_POINTER_SPEC
                  "> Out of memory allocating description\n",
                  thr));
...
out:
  ...
  UT_DBGOUT(
      5,
      ("<UT thr=" UT_POINTER_SPEC "> Releasing lock for registration\n", thr));
  freeTraceLock(thr);
 ...
 * description - Deregisters a subscriber currently consuming trace buffers from
 *the write queue parameters  - thr, subscriptionID returns     - Success or
 *error code
 ******************************************************************************/
omr_error_t trcDeregisterRecordSubscriber(OMR_TraceThread *thr,
  ...
  incrementRecursionCounter(thr);
  UT_DBGOUT(5,
            ("<UT thr=" UT_POINTER_SPEC "> Acquiring lock for deregistration\n",
             thr));
  omrthread_monitor_enter(OMR_TRACEGLOBAL(subscribersLock));
  UT_DBGOUT(
      5,
      ("<UT thr=" UT_POINTER_SPEC "> Lock acquired for deregistration\n", thr));
  ...
  if (findRecordSubscriber(subscriptionID)) {
    ...
    UT_DBGOUT(1, ("<UT thr=" UT_POINTER_SPEC
                  "> Failed to find subscriber to deregister\n",
                  thr));
    result = OMR_ERROR_ILLEGAL_ARGUMENT;
  }
  UT_DBGOUT(5,
            ("<UT thr=" UT_POINTER_SPEC "> Releasing lock for deregistration\n",
             thr));
  omrthread_monitor_exit(OMR_TRACEGLOBAL(subscribersLock));
  UT_DBGOUT(
      5,
      ("<UT thr=" UT_POINTER_SPEC "> Lock released for deregistration\n", thr));
  decrementRecursionCounter(thr);
  return result;
}
 ...
 * description - Places in use trace buffers on the write queue and prompts
 *queue processing parameters  - thr, first, last, pause returns     - Success
 *or error code
 ******************************************************************************/
static omr_error_t trcFlushTraceData(OMR_TraceThread *thr) {
  return OMR_ERROR_NONE;
...
omr_error_t fillInUTInterfaces(UtInterface **utIntf,
                               OMR_TraceInterface *omrTraceIntf,
                               UtModuleInterface *utModuleIntf) {
  omr_error_t rc = OMR_ERROR_NONE;
 ...
 *
 * This function is used by freeTrace(), when omrTraceGlobal may have been
 * NULLed.
 */
void deleteRecordSubscriber(OMR_TraceGlobal *global,
 ...
 *
 * The subscriber must be removed from OMR_TRACEGLOBAL(subscribers) and deleted
 * atomically, with respect to other operations on OMR_TRACEGLOBAL(subscribers),
 * to resolve race conditions between multiple threads attempting to destroy the
 * same subscription.
 *
 ...
 * @post subscription is freed, and must not be accessed anymore
 */
omr_error_t destroyRecordSubscriber(OMR_TraceThread *thr,
                                    UtSubscription *subscription,
                                    BOOLEAN callAlarm) {
  omr_error_t result = OMR_ERROR_NONE;
  ...
  /*
   * Delisting is required to prevent multiple threads from attempting to
   * Deregister the subscription. If multiple callers attempt to deregister the
   * subscriber, only one of them passes the findRecordSubscriber() test.
   *
   * No need to check whether subscription was enlisted, because next and prev
   * ptrs will be NULL if it was not enlisted.
   */
  ...
  if (NULL == OMR_TRACEGLOBAL(subscribers)) {
    OMR_TRACEGLOBAL(traceInCore) = TRUE;
    UT_DBGOUT(5,
              ("<UT thr=" UT_POINTER_SPEC "> Set traceInCore to TRUE\n", thr));
  }
