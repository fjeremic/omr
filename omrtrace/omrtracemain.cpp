...
moduleLoaded(OMR_TraceThread* thr, UtModuleInfo* modInfo)
{
  ...
  if (modInfo->traceVersionInfo == NULL) {
    ...
    UT_DBGOUT(1,
              ("<UT> ModuleLoaded refusing registration to %s because it's "
  ...
  UT_DBGOUT(1,
            ("<UT> ModuleLoaded: %s, interface: " UT_POINTER_SPEC "\n",
             modInfo->name,
             modInfo->intf));
  return OMR_ERROR_NONE;
}
...
moduleUnLoading(OMR_TraceThread* thr, UtModuleInfo* modInfo)
{
  ...
  if (NULL == modInfo->traceVersionInfo) {
     ...
     * silently to register this module */
    UT_DBGOUT(1,
              ("<UT> ModuleLoaded refusing deregistration to %s because it's "
               "version is less than the supported UT version %d\n",
               modInfo->name,
               UT_VERSION));
    return OMR_ERROR_NONE;
  ...
  if (modInfo->referenceCount > 0) {
    ...
    rc = setTracePointsTo(modInfo->name,
                          OMR_TRACEGLOBAL(componentList),
                          TRUE,
                          0,
                          0,
    ...
    if (OMR_ERROR_NONE != rc) {
      UT_DBGOUT(1,
                ("<UT> problem turning off trace in %s as it unloads\n",
                 modInfo->name));
...
threadStop(OMR_TraceThread** thrSlot)
{
  ...
  if (NULL != trcBuf) {
    if (!OMR_TRACEGLOBAL(traceInCore)) {
      ...
      UT_DBGOUT(3,
                ("<UT> Purging buffer " UT_POINTER_SPEC
 ...
 */
omr_error_t
threadStart(OMR_TraceThread** thr,
            const void* threadId,
...

{
  ...
  if (0 != omrthread_monitor_init_with_name(
             &OMR_TRACEGLOBAL(traceLock), 0, "Global Trace")) {
  ...
  if (0 != omrthread_monitor_init_with_name(&OMR_TRACEGLOBAL(subscribersLock),
                                            0,
                                            "Global Record Subscribers")) {
    UT_DBGOUT(1, ("<UT> Initialization of subscribersLock failed\n"));
    rc = OMR_ERROR_FAILED_TO_ALLOCATE_MONITOR;
    goto fail;
  }
  if (0 != omrthread_monitor_init_with_name(
             &OMR_TRACEGLOBAL(freeQueueLock), 0, "Global Trace Free Queue")) {
    UT_DBGOUT(1, ("<UT> Initialization of freeQueueLock failed\n"));
    rc = OMR_ERROR_FAILED_TO_ALLOCATE_MONITOR;
    goto fail;
  }
  if (0 != omrthread_monitor_init_with_name(
             &OMR_TRACEGLOBAL(bufferPoolLock), 0, "Global Trace Buffer Pool")) {
    UT_DBGOUT(1, ("<UT> Initialization of bufferPoolLock failed\n"));
    rc = OMR_ERROR_FAILED_TO_ALLOCATE_MONITOR;
    goto fail;
  }
  if (0 != omrthread_monitor_init_with_name(
             &OMR_TRACEGLOBAL(threadPoolLock), 0, "Global Trace Thread Pool")) {
  ...
  if (NULL != languageIntf) {
    memcpy(&OMR_TRACEGLOBAL(languageIntf),
           languageIntf,
           sizeof(OMR_TRACEGLOBAL(languageIntf)));
  } else {
    memset(
      &OMR_TRACEGLOBAL(languageIntf), 0, sizeof(OMR_TRACEGLOBAL(languageIntf)));
  }

  /* Initialize locks and pools for buffers and thread data. */
  OMR_TRACEGLOBAL(bufferPool) =
    pool_new(OMR_TRACEGLOBAL(bufferSize) + offsetof(OMR_TraceBuffer, record),
             0,
             0,
             0,
             OMR_GET_CALLSITE(),
 ...
 ******************************************************************************/
omr_error_t
trcRegisterRecordSubscriber(OMR_TraceThread* thr,
                            ...
                            UtSubscription** subscriptionReference)
{
  ...
  UT_DBGOUT(
    5, ("<UT thr=" UT_POINTER_SPEC "> Acquiring lock for registration\n", thr));
  omrthread_monitor_enter(OMR_TRACEGLOBAL(subscribersLock));
  getTraceLock(thr);
  UT_DBGOUT(
    5, ("<UT thr=" UT_POINTER_SPEC "> Lock acquired for registration\n", thr));
...
out:
  ...
  if (OMR_ERROR_NONE != result) {
    UT_DBGOUT(1,
              ("<UT> Error starting trace thread for \"%s\": %i\n",
               description,
               result));
  ...
  UT_DBGOUT(
    5, ("<UT thr=" UT_POINTER_SPEC "> Releasing lock for registration\n", thr));
  freeTraceLock(thr);
  omrthread_monitor_exit(OMR_TRACEGLOBAL(subscribersLock));
  UT_DBGOUT(
    5, ("<UT thr=" UT_POINTER_SPEC "> Lock released for registration\n", thr));
 ...
 ******************************************************************************/
omr_error_t
trcDeregisterRecordSubscriber(OMR_TraceThread* thr,
...
omr_error_t
fillInUTInterfaces(UtInterface** utIntf,
 ...
 */
omr_error_t
destroyRecordSubscriber(OMR_TraceThread* thr,
