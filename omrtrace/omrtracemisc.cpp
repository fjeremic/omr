...
void incrementRecursionCounter(OMR_TraceThread *thr) { thr->recursion++; }

void decrementRecursionCounter(OMR_TraceThread *thr) { thr->recursion--; }
...
omr_error_t expandString(char *returnBuffer, const char *original,
  ...
  const char *dateTokenString = "%Y"
                                "%m"
                                "%d";
  const char *timeTokenString = "%H"
                                "%M"
                                "%S";
  ...
  for (; originalCursor < originalLen; originalCursor++) {
    if (original[originalCursor] == '%') {
      ...
      if (original[originalCursor] == 'p') {
        strncpy(&formatString[formatCursor], pidTokenString,
                strlen(pidTokenString));
        formatCursor += strlen(pidTokenString);
      } else if (original[originalCursor] == 'd') {
        strncpy(&formatString[formatCursor], dateTokenString,
                strlen(dateTokenString));
        formatCursor += strlen(dateTokenString);
      } else if (original[originalCursor] == 't') {
        strncpy(&formatString[formatCursor], timeTokenString,
                strlen(timeTokenString));
        formatCursor += strlen(timeTokenString);
      } else {
        /* character following % was not 'p', 'd' or 't' */
        reportCommandLineError(atRuntime,
                               "Invalid special character '%%%c' in a trace "
                               "filename. Only %%p, %%d and %%t are allowed.",
  ...
  if (omrstr_subst_tokens(resultString, MAX_IMAGE_PATH_LENGTH, formatString,
                          stringTokens) > MAX_IMAGE_PATH_LENGTH) {
    returnBuffer[0] = '\0';
...
#define TEMPBUFLEN 150
  ...
  if ((f = omrfile_open(
           "utTrcCounters",
           EsOpenText | EsOpenWrite | EsOpenTruncate | EsOpenCreateNoTag, 0)) <
      0) {
    if ((f = omrfile_open("utTrcCounters",
                          EsOpenText | EsOpenWrite | EsOpenCreate |
                              EsOpenCreateNoTag,
                          0666)) < 0) {
      /* Unable to open tracepoint counter file %s, counters redirected to
       * stderr. */
    }
  }
        ...
        if (compData->tracepointcounters[i] > 0) {
          if (f < 0) {
            omrtty_err_printf("%s.%d %ld \n", compData->qualifiedComponentName,
                              i, compData->tracepointcounters[i]);
          } else {
...
omr_error_t addTraceConfig(OMR_TraceThread *thr, const char *cmd) {
   ...
   */
  UtTraceCfg *cfg = (UtTraceCfg *)omrmem_allocate_memory(
      sizeof(UtTraceCfg) + strlen(cmd) + 1, OMRMEM_CATEGORY_TRACE);
  if (NULL != cfg) {
    initHeader(&cfg->header, UT_TRACE_CONFIG_NAME,
               sizeof(UtTraceCfg) + strlen(cmd) + 1);
    cfg->next = NULL;
    ...
    /* We should update the trace header here (if it has already been
     * initialized) to reflect the change in trace settings. Unfortunately it
     * may be in use by a snap dump or we may have returned a pointer to it via
     * trcGetTraceMetadata so freeing it or nulling it is unsafe. For now we are
     * leaving it unchanged. If nothing has requested the trace header yet and
     * it is NULL then it will be created correctly when it is first used.
     */
 ...
 * parameters  - OMR_TraceThread, trace configuration command key, trace
 *configuration value value can safely be null, but null key is an error.
 * returns     - OMR_ERROR_NONE if it worked,
 * 			OMR_ERROR_OUT_OF_NATIVE_MEMORY or
 *OMR_ERROR_ILLEGAL_ARGUMENT if problem
 ******************************************************************************/
