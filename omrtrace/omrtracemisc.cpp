...
expandString(char* returnBuffer, const char* original, BOOLEAN atRuntime)
{
  ...
  for (; originalCursor < originalLen; originalCursor++) {
    if (original[originalCursor] == '%') {
      ...
      if (original[originalCursor] == 'p') {
        strncpy(
          &formatString[formatCursor], pidTokenString, strlen(pidTokenString));
        formatCursor += strlen(pidTokenString);
      } else if (original[originalCursor] == 'd') {
        strncpy(&formatString[formatCursor],
                dateTokenString,
  ...
  if (omrstr_subst_tokens(
        resultString, MAX_IMAGE_PATH_LENGTH, formatString, stringTokens) >
...
#define TEMPBUFLEN 150
  ...
  if ((f = omrfile_open("utTrcCounters",
                        EsOpenText | EsOpenWrite | EsOpenTruncate |
                          EsOpenCreateNoTag,
                        0)) < 0) {
    if ((f = omrfile_open("utTrcCounters",
...
addTraceConfig(OMR_TraceThread* thr, const char* cmd)
{
  ...
  if (NULL != cfg) {
    initHeader(
      &cfg->header, UT_TRACE_CONFIG_NAME, sizeof(UtTraceCfg) + strlen(cmd) + 1);
    cfg->next = NULL;
 ...
 ******************************************************************************/
omr_error_t
addTraceConfigKeyValuePair(OMR_TraceThread* thr,
                           const char* cmdKey,
                           const char* cmdValue)
{
