  ...
  UtComponentData** componentDataPtr, UtModuleInfo* moduleInfo, const char* componentName)
{
  ...
  if (moduleInfo->traceVersionInfo->traceVersion >= 7 && moduleInfo->containerModule != NULL) {
    ...
    omrstr_printf(
      qualifiedName, MAX_QUALIFIED_NAME_LENGTH, "%s(%s)", moduleInfo->name, moduleInfo->containerModule->name);
  ...
  if (moduleInfo->formatStringsFileName != NULL) {
    componentData->formatStringsFileName
      = (char*)omrmem_allocate_memory(strlen(moduleInfo->formatStringsFileName) + 1, OMRMEM_CATEGORY_TRACE);
...
omr_error_t freeComponentList(OMR_TraceGlobal* global, UtComponentList* componentList)
{
  ...
  while (compData != NULL) {
    ...
    UT_DBGOUT_NOGLOBAL(
      2, global->traceDebug, ("<UT> freeComponentList: freeing CI [%p] from [%p]\n", compData, componentList));
  ...
  while (compDataCursor != NULL) {
    ...
    if (try_scan(&tempName, componentData->componentName) && *tempName == '\0') {
      ...
      if (compDataCursor->moduleInfo != NULL && componentData->moduleInfo->traceVersionInfo->traceVersion >= 6) {
        ...
        memcpy(
          componentData->moduleInfo->active, compDataCursor->moduleInfo->active, compDataCursor->moduleInfo->count);
...
omr_error_t processComponentDefferedConfig(UtComponentData* componentData, UtComponentList* componentList)
{
  ...
  if (componentData->moduleInfo == NULL) {
    UT_DBGOUT(
      1, ("<UT> Can't process defferred config info on a non live component: %s\n", componentData->componentName));
    return OMR_ERROR_ILLEGAL_ARGUMENT;
  }
  ...
  if (componentList->deferredConfigInfoHead != NULL) {
    ...
    while (configInfo != NULL) {
      ...
      if (configAppliesToAll || j9_cmdla_stricmp(configInfo->componentName, componentData->componentName) == 0) {
        ...
        omr_error_t err = setTracePointsTo(componentData->componentName, componentList, configInfo->all,
          configInfo->firstTracePoint, configInfo->lastTracePoint, configInfo->value, configInfo->level,
          configInfo->groupName, suppressMessages, configInfo->setActive);
  ...
  UT_DBGOUT(
    2, ("<UT> addComponentToList: component %s processed deferred config info\n", componentData->componentName));
  return rc;
}
...
omr_error_t removeModuleFromList(UtModuleInfo* module, UtComponentList* componentList)
{
  ...
  UT_DBGOUT(
    2, ("<UT> removeModuleFromList: searching for module %s in componentList %p\n", module->name, componentList));
  ...
  while (compDataCursor != NULL) {
    if (0 == strcmp(compDataCursor->componentName, module->name)) {
      ...
      UT_DBGOUT(
        2, ("<UT> removeModuleFromList: found component %s in componentList %p\n", module->name, componentList));
  ...
  UT_DBGOUT(
    2, ("<UT> removeModuleFromList: didn't find component %s in componentList %p\n", module->name, componentList));
  return OMR_ERROR_INTERNAL;
}

static UtComponentData* getComponentDataNext(
  const char* componentName, UtComponentList* componentList, UtComponentData* position)
{
  UtComponentData* compDataCursor;
...
static UtComponentData* getComponentDataForModule(UtModuleInfo* moduleInfo, UtComponentList* componentList)
{
  UtComponentData* compDataCursor;
  ...
  UT_DBGOUT(
    4, ("<UT> getComponentData: didn't find component for module %p in componentList %p\n", moduleInfo, componentList));
  return NULL;
}

static void updateActiveArray(
  UtComponentData* compData, int32_t first, int32_t last, unsigned char value, int32_t setActive)
{
...
static omr_error_t addDeferredConfigToList(const char* componentName, int32_t all, int32_t first, int32_t last,
  unsigned char value, int level, const char* groupName, UtDeferredConfigInfo** configList, int32_t setActive)
{
 ...
 * component name only. */
static omr_error_t setTracePointsForComponent(const char* componentName, UtComponentList* componentList, int32_t all,
  ...
  int32_t setActive)
{
  ...
  if (0 == j9_cmdla_strnicmp(componentName, UT_ALL, strlen(UT_ALL))) {
    rc = addDeferredConfigToList(
      componentName, all, first, last, value, level, groupName, &componentList->deferredConfigInfoHead, setActive);
    ...
    while (compData != NULL) { /* configuring live and registered module */
      ...
      if (first > maxTraceId) {
        reportCommandLineError(
          suppressMessages, "Unable to set tracepoint %d in %s - tracepoint id out of range", first, componentName);
        return OMR_ERROR_INTERNAL;
      }
      if (last > maxTraceId) {
        reportCommandLineError(
          suppressMessages, "Tracepoint %d not in range 0->%d %s", last, maxTraceId, componentName);
        last = maxTraceId;
        return OMR_ERROR_INTERNAL;
      }
      ...
      if (MODULE_IS_AUXILIARY(modInfo)) {
        reportCommandLineError(
          suppressMessages, "Component %s is marked auxiliary and cannot be configured directly.", componentName);
        return OMR_ERROR_INTERNAL;
      }
 ...
 * and so on
 */
static omr_error_t parseAndSetTracePointsInRange(
  const char* componentName, unsigned char value, int32_t setActive, BOOLEAN suppressMessages)
{
   ...
   */
  if (0 == j9_cmdla_strnicmp(p, UT_TPID, strlen(UT_TPID)) && (p[strlen(UT_TPID)] == '(' || p[strlen(UT_TPID)] == '{')) {
    ...
    if (*temp == '-') {
      ...
      if (!isdigit(*temp)) {
        reportCommandLineError(suppressMessages,
          "Expecting tracepoint range specified as tpnid{componentName.offset1-offset2} e.g. tpnid{j9trc.2-6}");
        return OMR_ERROR_ILLEGAL_ARGUMENT;
      }
      while (isdigit(*temp)) {
        ret++;
      ...
      rc = setTracePointsForComponent(compName, OMR_TRACEGLOBAL(componentList), FALSE, firsttpid, lasttpid, value, -1,
  ...
  int32_t last, unsigned char value, int level, const char* groupName, BOOLEAN suppressMessages, int32_t setActive)
{
  ...
  if (NULL != tempstr) {
    ...
    rc = setTracePointsToParsed(
      compNamesBuffer, componentList, all, first, last, value, level, groupName, suppressMessages, setActive);
 ...
 ******************************************************************************/
static intptr_t openFileFromDirectorySearchList(char* searchPath, char* fileName, int32_t flags, int32_t mode)
{
...
static omr_error_t loadFormatStringsForComponent(UtComponentData* componentData)
{
  omr_error_t rc = OMR_ERROR_NONE;
  ...
  if (fileContents == NULL) {
    ...
    formatFileFD = openFileFromDirectorySearchList(
      OMR_TRACEGLOBAL(traceFormatSpec), componentData->formatStringsFileName, EsOpenText | EsOpenRead, 0);
    if (formatFileFD == -1) {
      UT_DBGOUT(1,
        ("<UT> loadFormatStringsForComponent can't load = %s, from current directory either - marking it unfindeable\n",
          componentData->formatStringsFileName));
...
char* getFormatString(const char* componentName, int32_t tracepoint)
{
  ...
  if (tracepoint >= compData->tracepointCount) {
    UT_DBGOUT(1,
      ("<UT> Unable to get formatString for %s.%d - no such tracepoint - maximum allowable tracepoint for that "
       "component is %d\n",
  ...
  BOOLEAN suppressMessages, int32_t setActive)
{
  ...
  if (groupFound) {
    ...
    reportCommandLineError(
      suppressMessages, "There is no group %s in component %s", groupName, componentData->moduleInfo->name);
    return OMR_ERROR_ILLEGAL_ARGUMENT;
  }
}
...
uint64_t incrementTraceCounter(UtModuleInfo* moduleInfo, UtComponentList* componentList, int32_t tracepoint)
{
  ...
  if (compData->moduleInfo == NULL) {
    UT_DBGOUT(
      1, ("<UT> Unable to increment trace counter %s.%d - no such loaded component\n", moduleInfo->name, tracepoint));
    return 0;
  }
  if (compData->tracepointcounters == NULL) {
    /* first time anything in this component has been counted */
    compData->tracepointcounters
      = (uint64_t*)omrmem_allocate_memory(sizeof(uint64_t) * compData->moduleInfo->count, OMRMEM_CATEGORY_TRACE);
    if (compData->tracepointcounters == NULL) {
