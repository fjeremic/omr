...
static const char *UT_MISSING_TRACE_FORMAT =
    "  Tracepoint format not in dat file";
#define MAX_QUALIFIED_NAME_LENGTH 16

omr_error_t initializeComponentData(UtComponentData **componentDataPtr,
                                    UtModuleInfo *moduleInfo,
                                    const char *componentName) {
  OMRPORT_ACCESS_FROM_OMRPORT(OMR_TRACEGLOBAL(portLibrary));

  UtComponentData *componentData = (UtComponentData *)omrmem_allocate_memory(
      sizeof(UtComponentData), OMRMEM_CATEGORY_TRACE);

  UT_DBGOUT(2, ("<UT> initializeComponentData: %s\n", componentName));
  if (componentData == NULL) {
    UT_DBGOUT(
        1, ("<UT> Unable to allocate componentData for %s\n", componentName));
    return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
  }

  initHeader(&componentData->header, UT_TRACE_COMPONENT_DATA,
             sizeof(UtComponentData));
  componentData->componentName = (char *)omrmem_allocate_memory(
      strlen(componentName) + 1, OMRMEM_CATEGORY_TRACE);
  ...
  if (moduleInfo->traceVersionInfo->traceVersion >= 7 &&
      moduleInfo->containerModule != NULL) {
    char qualifiedName[MAX_QUALIFIED_NAME_LENGTH];
    omrstr_printf(qualifiedName, MAX_QUALIFIED_NAME_LENGTH, "%s(%s)",
                  moduleInfo->name, moduleInfo->containerModule->name);
    componentData->qualifiedComponentName = (char *)omrmem_allocate_memory(
        strlen(qualifiedName) + 1, OMRMEM_CATEGORY_TRACE);
  ...
  if (moduleInfo->formatStringsFileName != NULL) {
    ...
    if (componentData->formatStringsFileName == NULL) {
      UT_DBGOUT(1, ("<UT> Unable to allocate componentData's format strings "
                    "file name field for %s\n",
                    componentName));
      return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
    }
    strcpy(componentData->formatStringsFileName,
           moduleInfo->formatStringsFileName);
  } else {
 ...
 */
void freeComponentData(OMR_TraceGlobal *global,
                       UtComponentData *componentDataPtr) {
  int numFormats, i;
  char *tempString;
  ...
  if (componentDataPtr->qualifiedComponentName !=
          componentDataPtr->componentName &&
...
omr_error_t initializeComponentList(UtComponentList **componentListPtr) {
  ...
  UtComponentList *componentList = (UtComponentList *)omrmem_allocate_memory(
      sizeof(UtComponentList), OMRMEM_CATEGORY_TRACE);
  UT_DBGOUT(2, ("<UT> initializeComponentList: %p\n", componentListPtr));
  if (componentList == NULL) {
    UT_DBGOUT(1, ("<UT> Unable to allocate component list\n"));
    return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
  }
  initHeader(&componentList->header, UT_TRACE_COMPONENT_LIST,
             sizeof(UtComponentList));
 ...
 */
omr_error_t freeComponentList(OMR_TraceGlobal *global,
...
omr_error_t addComponentToList(UtComponentData *componentData,
                               UtComponentList *componentList) {
  ...
  while (compDataCursor != NULL) {
    ...
    if (try_scan(&tempName, componentData->componentName) &&
        *tempName == '\0') {
      ...
      if (compDataCursor->moduleInfo != NULL &&
        ...
        memcpy(componentData->moduleInfo->active,
               compDataCursor->moduleInfo->active,
  ...
  UT_DBGOUT(1, ("<UT> addComponentToList: adding %s [%p] at ",
                componentData->componentName, componentData));
  if (endOfList == NULL) {
    UT_DBGOUT(1, ("<UT> head\n"));
...
omr_error_t processComponentDefferedConfig(UtComponentData *componentData,
                                           UtComponentList *componentList) {
  omr_error_t rc = OMR_ERROR_NONE;
  UtDeferredConfigInfo *configInfo;

  if (componentList == NULL || componentData == NULL) {
    UT_DBGOUT(1, ("<UT> Can't process config info for a NULL component [%p] or "
                  "NULL component list [%p]\n",
                  componentData, componentList));
    return OMR_ERROR_ILLEGAL_ARGUMENT;
  }

  if (componentData->moduleInfo == NULL) {
    UT_DBGOUT(1, ("<UT> Can't process defferred config info on a non live "
                  "component: %s\n",
                  componentData->componentName));
    return OMR_ERROR_ILLEGAL_ARGUMENT;
  }
  ...
  if (componentList->deferredConfigInfoHead != NULL) {
    UT_DBGOUT(2, ("<UT> processComponentDefferedConfig: component %s - "
                  "applying global deferred config info\n",
                  componentData->componentName));
    configInfo = componentList->deferredConfigInfoHead;
    while (configInfo != NULL) {
      BOOLEAN configAppliesToAll =
          j9_cmdla_stricmp(configInfo->componentName, UT_ALL) == 0;

      if (configAppliesToAll ||
          j9_cmdla_stricmp(configInfo->componentName,
                           componentData->componentName) == 0) {
        /* Applying "all" options to components may fail because of missing
         * groups or other inappropriate settings. We suppress messages when
         * applying "all" options to new components.
         */
        BOOLEAN suppressMessages = configAppliesToAll;
        omr_error_t err = setTracePointsTo(
            componentData->componentName, componentList, configInfo->all,
            configInfo->firstTracePoint, configInfo->lastTracePoint,
            configInfo->value, configInfo->level, configInfo->groupName,
    ...
    UT_DBGOUT(2, ("<UT> processComponentDefferedConfig: component %s - apply "
                  "global deferred config info complete\n",
                  componentData->componentName));
  }
...
omr_error_t removeModuleFromList(UtModuleInfo *module,
                                 UtComponentList *componentList) {
  UtComponentData *compDataCursor = componentList->head;
  UtComponentData *prevCompData;
  UT_DBGOUT(2, ("<UT> removeModuleFromList: searching for module %s in "
                "componentList %p\n",
                module->name, componentList));

  /* for flight controller functinality, any unloaded components should be moved
     onto an unloaded list, so their formatting strings and a memcpy of their
     moduleInfo is available */
  while (compDataCursor != NULL) {
    if (0 == strcmp(compDataCursor->componentName, module->name)) {
      UT_DBGOUT(2, ("<UT> removeModuleFromList: found component %s in "
                    "componentList %p\n",
                    module->name, componentList));
      ...
      if (NULL == compDataCursor->moduleInfo) {
        ...
        if (componentList == OMR_TRACEGLOBAL(componentList)) {
          ...
          addComponentToList(compDataCursor,
                             OMR_TRACEGLOBAL(unloadedComponentList));
        } else {
          /* otherwise we are unloading it from some other list so free it
  ...
  UT_DBGOUT(2, ("<UT> removeModuleFromList: didn't find component %s in "
                "componentList %p\n",
                module->name, componentList));
  return OMR_ERROR_INTERNAL;
}

static UtComponentData *getComponentDataNext(const char *componentName,
                                             UtComponentList *componentList,
                                             UtComponentData *position) {
  UtComponentData *compDataCursor;
  ...
  UT_DBGOUT(4, ("<UT> getComponentData: searching for component %s in "
                "componentList %p\n",
  ...
  while (compDataCursor != NULL) {
    ...
    if (try_scan(&tempName, (char *)componentName) && *tempName == '\0') {
      UT_DBGOUT(4, ("<UT> getComponentData: found component %s [%p] in "
                    "componentList %p\n",
...
UtComponentData *getComponentData(const char *componentName,
                                  UtComponentList *componentList) {
  return getComponentDataNext(componentName, componentList, NULL);
}

static UtComponentData *getComponentDataForModule(
    UtModuleInfo *moduleInfo, UtComponentList *componentList) {
  UtComponentData *compDataCursor;

  /* Beginning search from the first component */
  compDataCursor = componentList->head;

  UT_DBGOUT(4, ("<UT> getComponentData: searching for component for module %p "
                "in componentList %p\n",
  ...
  while (compDataCursor != NULL) {
    if (compDataCursor->moduleInfo == moduleInfo) {
      UT_DBGOUT(4, ("<UT> getComponentData: found component %s [%p] in "
                    "componentList %p\n",
                    compDataCursor->qualifiedComponentName, compDataCursor,
                    componentList));
      return compDataCursor;
    }
    compDataCursor = compDataCursor->next;
  }
  UT_DBGOUT(4, ("<UT> getComponentData: didn't find component for module %p in "
                "componentList %p\n",
                moduleInfo, componentList));
  return NULL;
}

static void updateActiveArray(UtComponentData *compData, int32_t first,
                              int32_t last, unsigned char value,
                              int32_t setActive) {
...
static char *newSubString(const char *buffer, size_t ret) {
  OMRPORT_ACCESS_FROM_OMRPORT(OMR_TRACEGLOBAL(portLibrary));
...
/* Utility function for parsing command line options */
static int32_t parseNumFromBuffer(const char *buffer, int ret) {
  int32_t rc = 0;
...
static omr_error_t addDeferredConfigToList(const char *componentName,
                                           int32_t all, int32_t first,
                                           int32_t last, unsigned char value,
                                           int level, const char *groupName,
                                           UtDeferredConfigInfo **configList,
                                           int32_t setActive) {
  ...
  UT_DBGOUT(2, ("<UT> setTracePointsTo: component %s applying to all and "
                "adding to global deferred",
                componentName));
  dconfiginfo = (UtDeferredConfigInfo *)omrmem_allocate_memory(
      sizeof(UtDeferredConfigInfo), OMRMEM_CATEGORY_TRACE);
  if (dconfiginfo == NULL) {
    UT_DBGOUT(
        1,
  ...
  dconfiginfo->componentName = (char *)omrmem_allocate_memory(
      strlen(componentName) + 1, OMRMEM_CATEGORY_TRACE);
  if (dconfiginfo->componentName == NULL) {
    UT_DBGOUT(1, ("<UT> Unable to set tracepoints in %s - can't allocate "
                  "config info componentName\n",
                  componentName));
    return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
  }
  ...
  if (groupName == NULL) {
    ...
    dconfiginfo->groupName = (char *)omrmem_allocate_memory(
        strlen(groupName) + 1, OMRMEM_CATEGORY_TRACE);
    if (dconfiginfo->groupName == NULL) {
      UT_DBGOUT(1, ("<UT> Unable to set tracepoints in %s - can't allocate "
                    "config info groupName\n",
                    componentName));
      return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
    }
...
/* for this function, we assume that componentName has been parsed, and contains
 * a single component name only. */
static omr_error_t setTracePointsForComponent(
    const char *componentName, UtComponentList *componentList, int32_t all,
    int32_t first, int32_t last, unsigned char value, int level,
    const char *groupName, BOOLEAN suppressMessages, int32_t setActive) {
  UtComponentData *compData;
  UtModuleInfo *modInfo = NULL;
  omr_error_t rc = OMR_ERROR_NONE;

  if (0 == j9_cmdla_strnicmp(componentName, UT_ALL, strlen(UT_ALL))) {
    rc = addDeferredConfigToList(
        componentName, all, first, last, value, level, groupName,
  ...
  if (compData == NULL) {
    ...
    addDeferredConfigToList(componentName, all, first, last, value, level,
                            groupName, &componentList->deferredConfigInfoHead,
                            setActive);
  } else {
    while (compData != NULL) { /* configuring live and registered module */
      ...
      UT_DBGOUT(2,
                ("<UT> setTracePointsTo: configuring registered component %s ",
                 componentName));
      if (all) {
        first = 0;
        last = maxTraceId;
      ...
      if (first > maxTraceId) {
        reportCommandLineError(
            suppressMessages,
            "Unable to set tracepoint %d in %s - tracepoint id out of range",
            first, componentName);
        return OMR_ERROR_INTERNAL;
      }
      if (last > maxTraceId) {
        reportCommandLineError(suppressMessages,
                               "Tracepoint %d not in range 0->%d %s", last,
      ...
      if (MODULE_IS_AUXILIARY(modInfo)) {
        reportCommandLineError(suppressMessages,
                               "Component %s is marked auxiliary and cannot be "
                               "configured directly.",
                               componentName);
        return OMR_ERROR_INTERNAL;
      }
      ...
      if (groupName != NULL) {
        ...
        rc = setTracePointGroupTo(groupName, compData, value, suppressMessages,
                                  setActive);
      } else if (level != -1) {
      ...
      /* There may be more than one component with the same name, particularly
       * in the case of libraries declared as submodules in tdf files. e.g.
       * pool, j9util */
 ...
 * and so on
 */
static omr_error_t parseAndSetTracePointsInRange(const char *componentName,
                                                 unsigned char value,
                                                 int32_t setActive,
  ...
  if (0 == j9_cmdla_strnicmp(p, UT_TPID, strlen(UT_TPID)) &&
    ...
    reportCommandLineError(
        suppressMessages,
        "Invalid trace options, use: tpnid{componentName.[integer_offset]}");
    return OMR_ERROR_ILLEGAL_ARGUMENT;
  ...
  while (*temp != '}' && *temp != '\0') {
    ...
    while (*temp != '.') {
      if (*temp == '}' || *temp == '\0') {
        reportCommandLineError(
            suppressMessages,
            "Expecting tpnid{compname.offset} e.g. tpnid{j9trc.4}");
        return OMR_ERROR_ILLEGAL_ARGUMENT;
      }
      temp++;
      ret++;
    ...
    if (*temp == '-') {
      ...
      if (!isdigit(*temp)) {
        reportCommandLineError(
            suppressMessages,
            "Expecting tracepoint range specified as "
            "tpnid{componentName.offset1-offset2} e.g. tpnid{j9trc.2-6}");
        return OMR_ERROR_ILLEGAL_ARGUMENT;
      }
      while (isdigit(*temp)) {
        ret++;
      ...
      rc = setTracePointsForComponent(compName, OMR_TRACEGLOBAL(componentList),
                                      FALSE, firsttpid, lasttpid, value, -1,
                                      NULL, suppressMessages, setActive);
      length += ret;
      p += ret;
    } else {
...
omr_error_t setTracePointsTo(const char *componentName,
                             UtComponentList *componentList, int32_t all,
                             int32_t first, int32_t last, unsigned char value,
                             int level, const char *groupName,
  ...
  if (componentName == NULL) {
    reportCommandLineError(suppressMessages,
                           "Can't set tracepoints for NULL componentName");
    return OMR_ERROR_ILLEGAL_ARGUMENT;
  }
  ...
  UT_DBGOUT(1, ("<UT> setTracePointsTo: component %s all= %s first=%d last=%d "
                "value=%d\n",
  ...
  if (NULL != tempstr) {
    ...
    UT_DBGOUT(
        2, ("<UT> setTracePointsTo found component list: %s\n", componentName));
    ...
    if (componentName[0] == '{') {
      ...
      stripbraces = TRUE;
                ...
                j9_cmdla_strnicmp(componentName, UT_TPNID, strlen(UT_TPNID))) &&
               (tempstr < strchr(componentName, '}'))) {
      /* option is 'tpnid' followed by a list of components enclosed in braces
       */
      componentName += (int)strlen(UT_TPNID) +
                       1; /* skip over the prefix and the open brace */
      stripbraces = TRUE;
    }

    /* separate and process the first component name */
    namelength = tempstr - componentName;
    compNamesBuffer = (char *)omrmem_allocate_memory(strlen(componentName) + 1,
    ...
    compNamesBuffer[namelength] =
        '\0'; /* replace the comma and terminate the string */

    rc = setTracePointsToParsed(compNamesBuffer, componentList, all, first,
                                last, value, level, groupName, suppressMessages,
                                setActive);
    ...
    if (stripbraces == TRUE) {
      compNamesBuffer[namelength - 1] =
          '\0'; /* blat the close brace out of the allocated copy */
    }
    /* recurse here in case the remainder is still a comma separated list */
    rc = setTracePointsTo(compNamesBuffer, componentList, all, first, last,
                          value, level, groupName, suppressMessages, setActive);
    omrmem_free_memory(compNamesBuffer);
    return rc;
  }

  /* if we get here we have a single option, but it might still be enclosed by
   * braces */
  if (componentName[0] == '{') {
    componentName++;
    compNamesBuffer = (char *)omrmem_allocate_memory(strlen(componentName) + 1,
    ...
    rc = setTracePointsToParsed(componentName, componentList, all, first, last,
                                value, level, groupName, suppressMessages,
                                setActive);
  }
  return rc;
}

omr_error_t setTracePointsToParsed(
    const char *componentName, UtComponentList *componentList, int32_t all,
    int32_t first, int32_t last, unsigned char value, int level,
    const char *groupName, BOOLEAN suppressMessages, int32_t setActive) {
  const char *tempstr = NULL;
  ...
  if (strchr(componentName, '.') != NULL) {
    ...
    rc = parseAndSetTracePointsInRange(componentName, value, setActive,
                                       suppressMessages);
    /* can't combine ranges and levels etc, so just duck out now */
    return rc;
  }

  if ((tempstr = strchr(componentName, '{')) != NULL ||
      (tempstr = strchr(componentName, '(')) != NULL) {
    char closingBrace;
    ...
    if (*(tempstr + 1) == closingBrace) {
      reportCommandLineError(suppressMessages,
                             "Error: found empty braces or parentheses");
      return OMR_ERROR_ILLEGAL_ARGUMENT;
    }
    ...
    if (strchr(tempstr, closingBrace) == NULL) {
      reportCommandLineError(suppressMessages,
                             "Error: unclosed braces or parentheses");
      return OMR_ERROR_ILLEGAL_ARGUMENT;
    }

    tempstr++; /* points at first option char now */

    if (0 == j9_cmdla_strnicmp(tempstr, UT_LEVEL_KEYWORD,
                               strlen(UT_LEVEL_KEYWORD)) ||
        *tempstr == 'l' || *tempstr == 'L') {
      ...
      newstr =
          (char *)omrmem_allocate_memory(namelength + 1, OMRMEM_CATEGORY_TRACE);
      if (newstr == NULL) {
        UT_DBGOUT(1, ("<UT> Unable to set tracepoints in %s - can't allocate "
                      "tempname info\n",
                      componentName));
      ...
      newstr2 = (char *)omrmem_allocate_memory(groupnamelength - 1,
                                               OMRMEM_CATEGORY_TRACE);
      if (newstr2 == NULL) {
        UT_DBGOUT(1, ("<UT> Unable to set tracepoints in %s - can't allocate "
                      "tempname info\n",
  ...
  rc = setTracePointsForComponent(componentName, componentList, all, first,
                                  last, value, level, groupName,
 ...
 ******************************************************************************/
static intptr_t openFileFromDirectorySearchList(char *searchPath,
                                                char *fileName, int32_t flags,
                                                int32_t mode) {
 ...
 *
 * Return the float version read, or 0.0F if an error reading the float
 * occurred.
 */
static float getDatFileVersion(char *formatFileContents) {
  float version;
...
static omr_error_t loadFormatStringsForComponent(
  ...
  const unsigned int componentNameLength =
      (const unsigned int)strlen(componentData->componentName);
  ...
  if (componentData->alreadyfailedtoloaddetails != 0) {
    UT_DBGOUT(2, ("<UT> loadFormatStringsForComponent %s returning due to "
  ...
  UT_DBGOUT(
      2, ("<UT> loadFormatStringsForComponent %s parsing filename = %s\n",
  ...
  if (fileContents == NULL) {
    ...
    formatFileFD = openFileFromDirectorySearchList(
        OMR_TRACEGLOBAL(traceFormatSpec), componentData->formatStringsFileName,
        EsOpenText | EsOpenRead, 0);
    if (formatFileFD == -1) {
      UT_DBGOUT(1, ("<UT> loadFormatStringsForComponent can't load = %s, from "
                    "current directory either - marking it unfindeable\n",
                    componentData->formatStringsFileName));
    ...
    fileContents = (char *)omrmem_allocate_memory((uintptr_t)fileSize + 1,
                                                  OMRMEM_CATEGORY_TRACE);
    if (fileContents == NULL) {
    ...
    if (omrfile_read(formatFileFD, fileContents, (int32_t)fileSize) !=
        (int32_t)fileSize) {
  ...
  formatStringsComponentArray = (char **)omrmem_allocate_memory(
      formatStringsComponentArraySize, OMRMEM_CATEGORY_TRACE);
  ...
  while (currenttp < numFormats) {
    if (*tempPtr == '\n') {
      ...
      if (datFileVersion >= 5.1F) {
        ...
        if (period == NULL) {
          UT_DBGOUT(
              1, ("<UT> error parsing 5.1 dat file component name: [%.*s].\n",
                  tempLen, tempPtr));
        } else {
          *period =
              '\0'; /* replace the '.' with a '\0' to make compname coherent */
      ...
      if ((tempLen == componentNameLength) &&
        ...
        /* skip the next four fields: type number, overhead, level and explicit.
         * e.g. " 0 1 1 N " */
        int field;
...
epilogue:
  if (OMR_ERROR_NONE != rc) {
    int i = 0;
    componentData->alreadyfailedtoloaddetails =
        1; /* don't try and load them again next time */
...
char *getFormatString(const char *componentName, int32_t tracepoint) {
  UtComponentData *compData =
      getComponentData(componentName, OMR_TRACEGLOBAL(componentList));
  ...
  if (compData == NULL) {
    ...
    compData =
        getComponentData(componentName, OMR_TRACEGLOBAL(unloadedComponentList));
    if (compData == NULL) {
      UT_DBGOUT(1, ("<UT> Unable to get formatString for %s.%d component not "
                    "registered\n",
                    componentName, tracepoint));
      return NULL;
    }
  ...
  if (compData->tracepointFormattingStrings == NULL) {
    ...
    if (OMR_ERROR_NONE != rc) {
      UT_DBGOUT(1,
                ("<UT> Unable to load formatStrings for %s\n", componentName));
      return NULL;
    }
  }
  ...
  if (tracepoint >= compData->tracepointCount) {
    UT_DBGOUT(1,
              ("<UT> Unable to get formatString for %s.%d - no such tracepoint "
               "- maximum allowable tracepoint for that component is %d\n",
...
omr_error_t setTracePointGroupTo(const char *groupName,
                                 UtComponentData *componentData,
                                 unsigned char value, BOOLEAN suppressMessages,
                                 int32_t setActive) {
  ...
  if (componentData == NULL) {
    UT_DBGOUT(
        1, ("<UT> setTracePointGroupTo called with invalid componentData\n"));
    return OMR_ERROR_ILLEGAL_ARGUMENT;
  }

  if (componentData->moduleInfo == NULL) {
    UT_DBGOUT(
        1, ("<UT> setTracePointGroupTo called on unregistered component: %s\n",
            componentData->componentName));
    return OMR_ERROR_ILLEGAL_ARGUMENT;
  }
  ...
  if (componentData->moduleInfo->groupDetails == NULL) {
    reportCommandLineError(suppressMessages,
                           "Groups not supported in component %s",
                           componentData->componentName);
    return OMR_ERROR_ILLEGAL_ARGUMENT;
  }
  ...
  if ((tempstr = strchr(groupName, ';')) != NULL) {
    ...
    tempgrpname =
        (char *)omrmem_allocate_memory(gnamelength + 1, OMRMEM_CATEGORY_TRACE);
    if (tempgrpname == NULL) {
    ...
    rc = setTracePointGroupTo(tempgrpname, componentData, value,
                              suppressMessages, setActive);
    if (OMR_ERROR_NONE != rc) {
  ...
  while (groupDetails != NULL) {
    if (!j9_cmdla_strnicmp(groupName, groupDetails->groupName,
                           strlen(groupDetails->groupName))) {
  ...
  if (groupFound) {
    ...
    reportCommandLineError(suppressMessages,
                           "There is no group %s in component %s", groupName,
                           componentData->moduleInfo->name);
    return OMR_ERROR_ILLEGAL_ARGUMENT;
  }
}

omr_error_t setTracePointsByLevelTo(UtComponentData *componentData, int level,
                                    unsigned char value, int32_t setActive) {
  omr_error_t rc = OMR_ERROR_NONE;
  ...
  if (componentData == NULL) {
    UT_DBGOUT(
        1,
        ("<UT> setTracepointsByLevelTo called with invalid componentData\n"));
    return OMR_ERROR_ILLEGAL_ARGUMENT;
  }
...
uint64_t incrementTraceCounter(UtModuleInfo *moduleInfo,
                               UtComponentList *componentList,
                               int32_t tracepoint) {
  UtComponentData *compData;
  ...
  if (moduleInfo == NULL) {
    ...
    UT_DBGOUT(2, ("<UT> incrementTraceCounter short circuit returning due to "
                  "NULL compName\n"));
    return 0;
  }
  ...
  if (compData->moduleInfo == NULL) {
    UT_DBGOUT(1, ("<UT> Unable to increment trace counter %s.%d - no such "
                  "loaded component\n",
                  moduleInfo->name, tracepoint));
    return 0;
  }
  if (compData->tracepointcounters == NULL) {
    ...
    memset(compData->tracepointcounters, 0,
           sizeof(uint64_t) * compData->moduleInfo->count);
  }
