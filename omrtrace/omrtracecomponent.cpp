...
omr_error_t
initializeComponentData(UtComponentData** componentDataPtr,
                        ...
                        const char* componentName)
{
  ...
  initHeader(
    &componentData->header, UT_TRACE_COMPONENT_DATA, sizeof(UtComponentData));
  ...
  if (moduleInfo->formatStringsFileName != NULL) {
    ...
    if (componentData->formatStringsFileName == NULL) {
      UT_DBGOUT(1,
                ("<UT> Unable to allocate componentData's format strings file "
                 "name field for %s\n",
                 componentName));
      return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
    }
...
freeComponentData(OMR_TraceGlobal* global, UtComponentData* componentDataPtr)
{
  ...
  UT_DBGOUT_NOGLOBAL(
    2, global->traceDebug, ("<UT> freeComponentData completed\n"));
  return;
}
...
initializeComponentList(UtComponentList** componentListPtr)
{
  ...
  initHeader(
    &componentList->header, UT_TRACE_COMPONENT_LIST, sizeof(UtComponentList));
...
freeComponentList(OMR_TraceGlobal* global, UtComponentList* componentList)
{
  ...
  UT_DBGOUT_NOGLOBAL(
    2, global->traceDebug, ("<UT> freeComponentList: %p\n", componentList));

  while (compData != NULL) {
    tempCD = compData->next;
    UT_DBGOUT_NOGLOBAL(2,
                       global->traceDebug,
...
omr_error_t
addComponentToList(UtComponentData* componentData,
                   UtComponentList* componentList)
{
  ...
  UT_DBGOUT(1,
            ("<UT> addComponentToList: adding %s [%p] at ",
             componentData->componentName,
             componentData));
  if (endOfList == NULL) {
    UT_DBGOUT(1, ("<UT> head\n"));
...
omr_error_t
processComponentDefferedConfig(UtComponentData* componentData,
                               UtComponentList* componentList)
{
  ...
  if (componentList == NULL || componentData == NULL) {
    UT_DBGOUT(1,
              ("<UT> Can't process config info for a NULL component [%p] or "
  ...
  if (componentList->deferredConfigInfoHead != NULL) {
    UT_DBGOUT(2,
              ("<UT> processComponentDefferedConfig: component %s - applying "
               "global deferred config info\n",
               componentData->componentName));
    ...
    while (configInfo != NULL) {
      ...
      if (configAppliesToAll ||
        ...
        omr_error_t err = setTracePointsTo(componentData->componentName,
                                           componentList,
                                           configInfo->all,
                                           configInfo->firstTracePoint,
                                           configInfo->lastTracePoint,
                                           configInfo->value,
                                           configInfo->level,
    ...
    UT_DBGOUT(2,
              ("<UT> processComponentDefferedConfig: component %s - apply "
...
removeModuleFromList(UtModuleInfo* module, UtComponentList* componentList)
{
  ...
  UT_DBGOUT(2,
            ("<UT> removeModuleFromList: didn't find component %s in "
             "componentList %p\n",
             module->name,
             componentList));
...
getComponentDataNext(const char* componentName,
                     UtComponentList* componentList,
                          ...
                          UtComponentList* componentList)
{
  ...
  UT_DBGOUT(4,
            ("<UT> getComponentData: searching for component for module %p in "
             "componentList %p\n",
             moduleInfo,
             componentList));
  ...
  while (compDataCursor != NULL) {
    if (compDataCursor->moduleInfo == moduleInfo) {
      UT_DBGOUT(
        4,
        ("<UT> getComponentData: found component %s [%p] in componentList %p\n",
         compDataCursor->qualifiedComponentName,
         compDataCursor,
         componentList));
      return compDataCursor;
    }
  ...
  UT_DBGOUT(4,
            ("<UT> getComponentData: didn't find component for module %p in "
             "componentList %p\n",
             moduleInfo,
             componentList));
  return NULL;
...
static void
updateActiveArray(UtComponentData* compData,
                  int32_t first,
                  int32_t last,
                  unsigned char value,
                  int32_t setActive)
...
static omr_error_t
addDeferredConfigToList(const char* componentName,
                        int32_t all,
                        int32_t first,
                        int32_t last,
                        unsigned char value,
                        ...
                        int32_t setActive)
{
  ...
  UT_DBGOUT(2,
            ("<UT> setTracePointsTo: component %s applying to all and adding "
             "to global deferred",
             componentName));
  ...
  if (dconfiginfo->componentName == NULL) {
    UT_DBGOUT(1,
              ("<UT> Unable to set tracepoints in %s - can't allocate config "
               "info componentName\n",
               componentName));
    return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
  }
...
static omr_error_t
setTracePointsForComponent(const char* componentName,
                           UtComponentList* componentList,
                           int32_t all,
                           int32_t first,
                           int32_t last,
    ...
    while (compData != NULL) { /* configuring live and registered module */
      ...
      if (last > maxTraceId) {
        reportCommandLineError(suppressMessages,
                               "Tracepoint %d not in range 0->%d %s",
                               last,
                               maxTraceId,
                               componentName);
        last = maxTraceId;
      ...
      if (groupName != NULL) {
        ...
        rc = setTracePointGroupTo(
          groupName, compData, value, suppressMessages, setActive);
      } else if (level != -1) {
                              ...
                              BOOLEAN suppressMessages)
{
  ...
  while (*temp != '}' && *temp != '\0') {
    ...
    if (*temp == '-') {
      ...
      rc = setTracePointsForComponent(compName,
                                      OMR_TRACEGLOBAL(componentList),
                                      FALSE,
                                      firsttpid,
                                      lasttpid,
                                      value,
...
omr_error_t
setTracePointsTo(const char* componentName,
                 UtComponentList* componentList,
                 int32_t all,
                 int32_t first,
                 int32_t last,
                 ...
                 int32_t setActive)
{
  ...
  if (NULL != tempstr) {
    ...
    if (componentName[0] == '{') {
       ...
       */
      componentName +=
        (int)strlen(UT_TPNID) + 1; /* skip over the prefix and the open brace */
      stripbraces = TRUE;
    }
    ...
    if (compNamesBuffer == NULL) {
      UT_DBGOUT(1,
                ("<UT> Unable to set tracepoints in %s - can't allocate "
                 "tempname info\n",
                 componentName));
      return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
    }
    ...
    rc = setTracePointsToParsed(compNamesBuffer,
                                componentList,
                                all,
                                first,
                                last,
    ...
    rc = setTracePointsTo(compNamesBuffer,
                          componentList,
                          all,
                          first,
                          last,
  ...
  if (componentName[0] == '{') {
    ...
    rc = setTracePointsToParsed(componentName,
                                componentList,
                                all,
                                first,
                                last,
...
omr_error_t
setTracePointsToParsed(const char* componentName,
                       UtComponentList* componentList,
                       int32_t all,
                       int32_t first,
                       int32_t last,
                       ...
                       int32_t setActive)
{
  ...
  if (strchr(componentName, '.') != NULL) {
    ...
    rc = parseAndSetTracePointsInRange(
      componentName, value, setActive, suppressMessages);
  ...
  if ((tempstr = strchr(componentName, '{')) != NULL ||
    ...
    UT_DBGOUT(2,
              ("<UT> setTracePointsTo: has detected a suboption %s in %s\n",
               tempstr,
               componentName));
    ...
    if (0 == j9_cmdla_strnicmp(
               tempstr, UT_LEVEL_KEYWORD, strlen(UT_LEVEL_KEYWORD)) ||
        *tempstr == 'l' || *tempstr == 'L') {
      ...
      newstr[namelength] = '\0';
      UT_DBGOUT(
        2, ("<UT> setTracePointsTo: Level detected %d in %s\n", level, newstr));
      ...
      UT_DBGOUT(2,
                ("<UT> setTracePointsTo: Group %s detected in %s\n",
                 groupName,
                 newstr));
      componentName = newstr;
    }
  ...
  rc = setTracePointsForComponent(componentName,
                                  componentList,
                                  all,
                                  first,
                                  last,
 ...
 ******************************************************************************/
static intptr_t
openFileFromDirectorySearchList(char* searchPath,
                                char* fileName,
                                int32_t flags,
                                int32_t mode)
{
  ...
  while (offset < searchPathLen) {
    ...
    UT_DBGOUT(2,
              ("<UT> dat file loader looking for %s at %s\n",
               fileName,
               tempFileNamePath));
    ...
    if (fileHandle != -1) {
      UT_DBGOUT(2,
                ("<UT> dat file loader found for %s at %s\n",
                 fileName,
                 tempFileNamePath));
      return fileHandle;
    }
...
loadFormatStringsForComponent(UtComponentData* componentData)
{
  ...
  UT_DBGOUT(
    2,
    ("<UT> loadFormatStringsForComponent %s\n", componentData->componentName));

  if (componentData->alreadyfailedtoloaddetails != 0) {
    UT_DBGOUT(2,
              ("<UT> loadFormatStringsForComponent %s returning due to "
  ...
  if (fileContents == NULL) {
    ...
    UT_DBGOUT(1,
              ("<UT> loadFormatStringsForComponent trying to load = %s\n",
               componentData->formatStringsFileName));
    formatFileFD =
      openFileFromDirectorySearchList(OMR_TRACEGLOBAL(traceFormatSpec),
                                      componentData->formatStringsFileName,
                                      EsOpenText | EsOpenRead,
                                      0);
    if (formatFileFD == -1) {
      UT_DBGOUT(1,
                ("<UT> loadFormatStringsForComponent can't load = %s, from "
...
getFormatString(const char* componentName, int32_t tracepoint)
{
  ...
  UT_DBGOUT(2,
            ("<UT> getFormatString for: component %s tracepoint %d\n",
             componentName,
             tracepoint));
  if (compData == NULL) {
...
omr_error_t
setTracePointGroupTo(const char* groupName,
                     ...
                     int32_t setActive)
{
  ...
  UT_DBGOUT(2,
            ("<UT> setTraceGroupTo called: groupname %s compdata %p\n",
             groupName,
             componentData));
  ...
  if ((tempstr = strchr(groupName, ';')) != NULL) {
    ...
    rc = setTracePointGroupTo(
      tempgrpname, componentData, value, suppressMessages, setActive);
    if (OMR_ERROR_NONE != rc) {
  ...
  while (groupDetails != NULL) {
    if (!j9_cmdla_strnicmp(groupName,
  ...
  if (groupFound) {
    ...
    reportCommandLineError(suppressMessages,
                           "There is no group %s in component %s",
                           groupName,
...
omr_error_t
setTracePointsByLevelTo(UtComponentData* componentData,
                        int level,
                        unsigned char value,
                        int32_t setActive)
{
  ...
  if (componentData == NULL) {
    UT_DBGOUT(
      1, ("<UT> setTracepointsByLevelTo called with invalid componentData\n"));
    return OMR_ERROR_ILLEGAL_ARGUMENT;
  }
  ...
  if (componentData->moduleInfo->levels == NULL) {
    UT_DBGOUT(2,
              ("<UT> levels not supported in component %s\n",
               componentData->componentName));
    return OMR_ERROR_NONE;
  }
...
uint64_t
incrementTraceCounter(UtModuleInfo* moduleInfo,
                      UtComponentList* componentList,
                      int32_t tracepoint)
{
  UtComponentData* compData;
  ...
  if (moduleInfo == NULL) {
    ...
    UT_DBGOUT(2,
              ("<UT> incrementTraceCounter short circuit returning due to NULL "
               "compName\n"));
    return 0;
  }
  ...
  if (compData->moduleInfo == NULL) {
    UT_DBGOUT(1,
              ("<UT> Unable to increment trace counter %s.%d - no such loaded "
               "component\n",
               moduleInfo->name,
               tracepoint));
    return 0;
  ...
  if (compData->tracepointcounters == NULL) {
    ...
    if (compData->tracepointcounters == NULL) {
      UT_DBGOUT(1,
                ("<UT> Unable to allocate trace counter buffers for %s\n",
                 moduleInfo->name));
      return 0;
    }
    memset(compData->tracepointcounters,
           0,
