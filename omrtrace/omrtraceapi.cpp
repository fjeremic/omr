...
#define OMR_TRACE_THREAD_FROM_VMTHREAD_NONNULL(thr) \
  ((thr)->_trace.omrTraceThread)

/* Calls to control the trace engine life cycle, the sequence is ([] are
 * optional) Note: These functions will be renamed under task 65134 1 -
 * fillInUTInterfaces - Setup function pointers in interfaces via
 * fillInUTInterfaces 2 - initializeTrace 	- Tell trace to intialise via
 * initializeTrace supplying default options. (Usually just the location of the
 * .dat file is standard) 3 - threadStart 		- Notify trace about the
 * current thread since it started before trace was available via threadStart
 * 						- Trace should be notified about about threads
 * beginning and ending via calls to threadStart and threadStop when they start
 * and end. [6] - setOptions 	- set any additional default options or process
 * any options specified on the command line. 8 - Initialize port, thread and
 * any other modules that may have loaded before trace was ready. 9 - The real
 * work happens here, hopefully for a long time. 10 - utTerminateTrace - Tell
 * the trace engine to shutdown. 11 - freeTrace - Tell the trace engine to free
...
omr_error_t omr_trc_startup(OMR_VM *omrVM,
  ...
  newTrcEngine = (OMRTraceEngine *)omrmem_allocate_memory(
      sizeof(*newTrcEngine), OMRMEM_CATEGORY_TRACE);
  ...
  rc = fillInUTInterfaces(&newTrcEngine->utIntf, &newTrcEngine->omrTraceIntfS,
                          &newTrcEngine->utModuleIntfS);
...
omr_error_t omr_trc_startThreadTrace(OMR_VMThread *currentThread,
  ...
  if (NULL != omrTraceGlobal) {
    if (OMR_TRACE_ENGINE_MT_ENABLED != OMR_TRACEGLOBAL(initState)) {
      ...
      rc = threadStart(&OMR_TRACE_THREAD_FROM_VMTHREAD_NONNULL(currentThread),
                       ((NULL == currentThread->_language_vmthread)
                            ? currentThread
...
omr_error_t omr_trc_stopThreadTrace(OMR_VMThread *currentThread) {
  ...
  if (NULL == currentThread) {
    ...
    if (NULL == *thrSlot) {
      ...
      /* It's safe to read omrTraceGlobal because at least one thread is
       * attached to the trace engine. OMRTODO The initState check may not be
       * needed here.
       */
      if ((NULL != omrTraceGlobal) &&
          (OMR_TRACEGLOBAL(initState) >= OMR_TRACE_ENGINE_MT_ENABLED)) {
...
void omr_trc_preForkHandler(void) {
  if ((NULL != omrTraceGlobal) &&
    ...
    UT_DBGOUT(
        1,
        ("<UT> omr_trc_preForkHandler: requesting global subscribers lock.\n"));
    omrthread_monitor_enter(OMR_TRACEGLOBAL(subscribersLock));
    UT_DBGOUT(
        1,
        ("<UT> omr_trc_preForkHandler: obtained global subscribers lock.\n"));

    UT_DBGOUT(1,
              ("<UT> omr_trc_preForkHandler: requesting global trace lock.\n"));
    omrthread_monitor_enter(OMR_TRACEGLOBAL(traceLock));
    UT_DBGOUT(1,
              ("<UT> omr_trc_preForkHandler: obtained global trace lock.\n"));

    UT_DBGOUT(
        1,
        ("<UT> omr_trc_preForkHandler: requesting global thread pool lock.\n"));
    omrthread_monitor_enter(OMR_TRACEGLOBAL(threadPoolLock));
    UT_DBGOUT(
        1,
        ("<UT> omr_trc_preForkHandler: obtained global thread pool lock.\n"));

    UT_DBGOUT(
        1,
        ("<UT> omr_trc_preForkHandler: requesting global buffer pool lock.\n"));
    omrthread_monitor_enter(OMR_TRACEGLOBAL(bufferPoolLock));
    UT_DBGOUT(
        1,
        ("<UT> omr_trc_preForkHandler: obtained global buffer pool lock.\n"));
  }
}
...
void omr_trc_postForkParentHandler(void) {
  if ((NULL != omrTraceGlobal) &&
    ...
    UT_DBGOUT(1, ("<UT> omr_trc_postForkParentHandler: released global buffer "
                  "pool lock.\n"));

    omrthread_monitor_exit(OMR_TRACEGLOBAL(threadPoolLock));
    UT_DBGOUT(1, ("<UT> omr_trc_postForkParentHandler: released global thread "
                  "pool lock.\n"));

    omrthread_monitor_exit(OMR_TRACEGLOBAL(traceLock));
    UT_DBGOUT(
        1,
        ("<UT> omr_trc_postForkParentHandler: released global trace lock.\n"));

    omrthread_monitor_exit(OMR_TRACEGLOBAL(subscribersLock));
    UT_DBGOUT(1, ("<UT> omr_trc_postForkParentHandler: released global "
                  "subscribers lock.\n"));
...
void omr_trc_postForkChildHandler(void) {
  if ((NULL != omrTraceGlobal) &&
    ...
    UT_DBGOUT(
        1,
        ("<UT> omr_trc_postForkChildHandler: released global trace lock.\n"));
...
void postForkCleanupGlobals(OMR_TraceThread *thr) {
...
void postForkCleanupBuffers(OMR_TraceThread *thr) {
...
void postForkCleanupThreads(OMR_TraceThread *thr) {
  J9PoolState threadPoolState;
  OMRPORT_ACCESS_FROM_OMRPORT(OMR_TRACEGLOBAL(portLibrary));

  /* Cleanup and free the name and OMR_TraceThread for all threads which do not
   * exist after fork. */
  OMR_TraceThread *iteratingThread = (OMR_TraceThread *)pool_startDo(
      OMR_TRACEGLOBAL(threadPool), &threadPoolState);
  while (NULL != iteratingThread) {
    if (thr != iteratingThread) {
      if ((NULL != iteratingThread->name) &&
          (UT_NO_THREAD_NAME != iteratingThread->name)) {
