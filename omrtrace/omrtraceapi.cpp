...
omr_error_t omr_trc_startup(
  OMR_VM* omrVM, const OMR_TraceLanguageInterface* languageIntf, const char* datDir, const char** opts)
{
  ...
  if (NULL != omrTraceGlobal) {
    if (OMR_TRACE_ENGINE_MT_ENABLED != OMR_TRACEGLOBAL(initState)) {
      ...
      rc = threadStart(&OMR_TRACE_THREAD_FROM_VMTHREAD_NONNULL(currentThread),
        ((NULL == currentThread->_language_vmthread) ? currentThread : currentThread->_language_vmthread), threadName,
        currentThread->_os_thread, currentThread);
    }
  }
  return rc;
}
