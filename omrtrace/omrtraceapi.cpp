                ...
                const char** opts)
{
  ...
  rc = fillInUTInterfaces(&newTrcEngine->utIntf,
                          &newTrcEngine->omrTraceIntfS,
...
omr_trc_preForkHandler(void)
{
  if ((NULL != omrTraceGlobal) &&
    ...
    UT_DBGOUT(
      1, ("<UT> omr_trc_preForkHandler: obtained global subscribers lock.\n"));
    ...
    UT_DBGOUT(
      1, ("<UT> omr_trc_preForkHandler: obtained global thread pool lock.\n"));
    ...
    UT_DBGOUT(
      1, ("<UT> omr_trc_preForkHandler: obtained global buffer pool lock.\n"));
  }
}
...
omr_trc_postForkParentHandler(void)
{
  if ((NULL != omrTraceGlobal) &&
    ...
    UT_DBGOUT(1,
              ("<UT> omr_trc_postForkParentHandler: released global buffer "
               "pool lock.\n"));

    omrthread_monitor_exit(OMR_TRACEGLOBAL(threadPoolLock));
    UT_DBGOUT(1,
              ("<UT> omr_trc_postForkParentHandler: released global thread "
               "pool lock.\n"));

    omrthread_monitor_exit(OMR_TRACEGLOBAL(traceLock));
    UT_DBGOUT(
      1, ("<UT> omr_trc_postForkParentHandler: released global trace lock.\n"));

    omrthread_monitor_exit(OMR_TRACEGLOBAL(subscribersLock));
    UT_DBGOUT(1,
              ("<UT> omr_trc_postForkParentHandler: released global "
               "subscribers lock.\n"));
...
omr_trc_postForkChildHandler(void)
{
  if ((NULL != omrTraceGlobal) &&
    ...
    omrthread_monitor_exit(OMR_TRACEGLOBAL(traceLock));
    UT_DBGOUT(
      1, ("<UT> omr_trc_postForkChildHandler: released global trace lock.\n"));
