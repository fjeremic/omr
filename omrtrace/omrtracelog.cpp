  ...
  if (trcBuf == NULL) {
    ...
    UT_DBGOUT(1,
              ("<UT> Allocated buffer %i " UT_POINTER_SPEC "\n",
               OMR_TRACEGLOBAL(allocatedTraceBuffers),
               trcBuf));
  }
 ...
 ******************************************************************************/
static void
tracePrint(OMR_TraceThread* thr,
           ...
           va_list var)
{
  ...
  if (modInfo == NULL) {
    ...
    omrstr_printf(qualifiedModuleName,
 ...
 ******************************************************************************/
static void
traceAssertion(OMR_TraceThread* thr,
               UtModuleInfo* modInfo,
               uint32_t traceId,
               va_list var)
{
   ...
   *  Non-indented print
   */
  omrtty_err_printf("%02d:%02d:%02d.%03d " UT_POINTER_SPEC "%8s.%-6d *   ",
                    hh,
                    mm,
                    ss,
                    millis,
       ...
       int bufferType)
{
  ...
  if ((p < (char*)&trcBuf->record) ||
    ...
    UT_DBGOUT(1,
              ("<UT> invalid nextEntry value in record. Freed trace buffer for "
               "thread " UT_POINTER_SPEC " and reinitialized\n",
               thr));
  ...
  if ((OMR_TRACEGLOBAL(bufferSize) - trcBuf->record.nextEntry) >
    ...
    *p = 12 + length;
    ...
    if (lastSequence != (int32_t)(trcBuf->record.sequence >> 32)) {
      ...
      entryLength = 0;
      copyToBuffer(
        thr, bufferType, UT_TRC_SEQ_WRAP_ID, &p, 3, &entryLength, &trcBuf);
      copyToBuffer(
        thr, bufferType, (char*)&lastSequence, &p, 4, &entryLength, &trcBuf);
    ...
    copyToBuffer(
      thr, bufferType, stringVar, &p, (int)stringVarLen, &entryLength, &trcBuf);
      ...
      *p = (unsigned char)entryLength;
        ...
        switch (*str) {
          ...
          case UT_TRACE_DATA_TYPE_STRING: {
            ...
            if (left > length) {
              ...
              copyToBuffer(
                thr, bufferType, stringVar, &p, length, &entryLength, &trcBuf);
            }
            break;
          }
          /*
      ...
      if ((char*)&trcBuf->record + OMR_TRACEGLOBAL(bufferSize) - p >
        ...
        *p = (unsigned char)entryLength;
        ...
        copyToBuffer(
          thr, bufferType, &charVar, &p, sizeof(char), &entryLength, &trcBuf);
        entryLength--;
        p--;
      }
    }
...
static void
logTracePoint(OMR_TraceThread* thr,
              UtModuleInfo* modInfo,
              uint32_t traceId,
              const char* spec,
              va_list varArgs)
{
  ...
  if ((thr->currentOutputMask & UT_EXCEPTION) != 0) {
    ...
    if (*thr != OMR_TRACEGLOBAL(exceptionContext)) {
      ...
      trace(thr,
            NULL,
            (UT_TRC_CONTEXT_ID << 8) | UT_MAXIMAL,
            UT_EXCEPTION_BUFFER,
            pointerSpec,
            thr);
    }
 ...
 ******************************************************************************/
void
doTracePoint(OMR_TraceThread* thr,
             UtModuleInfo* modInfo,
             uint32_t traceId,
             const char* spec,
             va_list varArgs)
{
  ...
  BOOLEAN isRegular =
    FALSE; /* is this a regular tracepoint, and not an auxiliary tracepoint? */
 ...
 ******************************************************************************/
void
internalTrace(OMR_TraceThread* thr,
              UtModuleInfo* modInfo,
              uint32_t traceId,
              const char* spec,
              ...)
