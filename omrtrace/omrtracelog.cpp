 ...
 ******************************************************************************/
static OMR_TraceBuffer* getTrcBuf(OMR_TraceThread* thr, OMR_TraceBuffer* oldBuf, int bufferType)
{
 ...
 * returns     - void
 ******************************************************************************/
static void copyToBuffer(OMR_TraceThread* thr, int bufferType, const char* var, char** p, int length, int* entryLength,
  OMR_TraceBuffer** trcBuf)
{
 ...
 *
 ******************************************************************************/
static void tracePrint(OMR_TraceThread* thr, UtModuleInfo* modInfo, uint32_t traceId, va_list var)
{
  int id;
  char* format;
  ...
  if (modInfo == NULL) {
    ...
    omrstr_printf(
      qualifiedModuleName, MAX_QUALIFIED_NAME_LENGTH, "%s(%s)", modInfo->name, modInfo->containerModule->name);
  ...
  if (OMR_TRACEGLOBAL(indentPrint)) {
     ...
     *  Indented print
     */
    omrtty_err_printf("%02d:%02d:%02d.%03d%c" UT_POINTER_SPEC "%16s.%-6d %c %s %c ", hh, mm, ss, millis, threadSwitch,
      thr->id, qualifiedModuleName, id, excpt, indent, entryexit);
 ...
 *
 ******************************************************************************/
static void traceAssertion(OMR_TraceThread* thr, UtModuleInfo* modInfo, uint32_t traceId, va_list var)
{
  int id;
  const char* format;
  uint32_t hh, mm, ss, millis;
   ...
   */
  omrtty_err_printf(
    "%02d:%02d:%02d.%03d " UT_POINTER_SPEC "%8s.%-6d *   ", hh, mm, ss, millis, thr->id, qualifiedModuleName, id);
  omrtty_err_vprintf(format + 2, var);
 ...
 ******************************************************************************/
static void traceV(
  OMR_TraceThread* thr, UtModuleInfo* modInfo, uint32_t traceId, const char* spec, va_list var, int bufferType)
{
  OMR_TraceBuffer* trcBuf;
  int lastSequence;
  int entryLength;
  ...
  char temp[3];
  static char lengthConversion[] = { 0, sizeof(char), sizeof(short), 0, sizeof(int32_t), sizeof(float), sizeof(char*),
    sizeof(double), sizeof(int64_t), sizeof(long double), 0 };
  OMRPORT_ACCESS_FROM_OMRPORT(OMR_TRACEGLOBAL(portLibrary));
  ...
  if ((p < (char*)&trcBuf->record) || (p > ((char*)&trcBuf->record + OMR_TRACEGLOBAL(bufferSize)))) {
    ...
    UT_DBGOUT(1,
      ("<UT> invalid nextEntry value in record. Freed trace buffer for thread " UT_POINTER_SPEC " and reinitialized\n",
        thr));
 ...
 *
 ******************************************************************************/
static void trace(OMR_TraceThread* thr, UtModuleInfo* modinfo, uint32_t traceId, int bufferType, const char* spec, ...)
{
  va_list var;
...
static void logTracePoint(
  OMR_TraceThread* thr, UtModuleInfo* modInfo, uint32_t traceId, const char* spec, va_list varArgs)
{
  va_list var;
