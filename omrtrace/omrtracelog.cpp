...
omr_error_t initTraceHeader(void) {
  ...
  if (OMR_TRACEGLOBAL(serviceInfo) == NULL) {
    ...
    OMR_TRACEGLOBAL(serviceInfo) = (char *)omrmem_allocate_memory(
        sizeof(UNKNOWN_SERVICE_LEVEL) + 1, OMRMEM_CATEGORY_TRACE);
  ...
  srvSize =
      ((srvSize + UT_STRUCT_ALIGN - 1) / UT_STRUCT_ALIGN) * UT_STRUCT_ALIGN;
  size += srvSize;
  ...
  startSize =
      ((startSize + UT_STRUCT_ALIGN - 1) / UT_STRUCT_ALIGN) * UT_STRUCT_ALIGN;
  ...
  actSize =
      ((actSize + UT_STRUCT_ALIGN - 1) / UT_STRUCT_ALIGN) * UT_STRUCT_ALIGN;
  ...
  trcHdr =
      (UtTraceFileHdr *)omrmem_allocate_memory(size, OMRMEM_CATEGORY_TRACE);
  if (NULL == trcHdr) {
  ...
  ((UtTraceSection *)ptr)->type =
      OMR_TRACEGLOBAL(traceInCore) ? UT_TRACE_INTERNAL : UT_TRACE_EXTERNAL;
 ...
 ******************************************************************************/
static OMR_TraceBuffer *getTrcBuf(OMR_TraceThread *thr, OMR_TraceBuffer *oldBuf,
                                  int bufferType) {
  ...
  if (oldBuf != NULL) {
    ...
    /* Null the thread reference as there's no guarantee it's valid after this
     * point */
    oldBuf->thr = NULL;
  ...
  if (trcBuf == NULL) {
    ...
    if (trcBuf == NULL) {
      if (OMR_TRACEGLOBAL(dynamicBuffers) == TRUE) {
        ...
        /* Native memory allocation failure, falling back to nodynamic trace
         * settings. */
    ...
    VM_AtomicSupport::addU32(
        (volatile uint32_t *)&OMR_TRACEGLOBAL(allocatedTraceBuffers), 1);
   ...
   */
  UT_DBGOUT(5, ("<UT> Buffer " UT_POINTER_SPEC
                " obtained for thread " UT_POINTER_SPEC "\n",
                trcBuf, thr));
  trcBuf->next = NULL;
...
out:
  ...
  /* we reset the contents of the buffer first so that even if we're in the
   * middle of flushing/writing this buffer, the record maintains its
   * consistency.
   */
  ...
  *((char *)&trcBuf->record + trcBuf->record.firstEntry) = '\0';
  ...
  /* in case this buffer is dumped rather than queued we want non zero values
   * for the write times */
 ...
 * returns     - void
 ******************************************************************************/
static void copyToBuffer(OMR_TraceThread *thr, int bufferType, const char *var,
                         char **p, int length, int *entryLength,
                         OMR_TraceBuffer **trcBuf) {
  int bufLeft =
      (int)((char *)&(*trcBuf)->record + OMR_TRACEGLOBAL(bufferSize) - *p);
  int remainder;
  const char *str;
    ...
    *p += length;
    ...
    while (remainder > 0) {
      *trcBuf = getTrcBuf(thr, *trcBuf, bufferType);
      if (*trcBuf != NULL) {
        /* we're about to copy stuff into it so mark the buffer as not new so it
         * can be queued */
        VM_AtomicSupport::bitAndU32((volatile uint32_t *)(&(*trcBuf)->flags),
                                    ~UT_TRC_BUFFER_NEW);
        (*trcBuf)->thr = thr;
        ...
        *p = (char *)&(*trcBuf)->record + (*trcBuf)->record.nextEntry;
        ...
        /* if there's already a tracepoint in the buffer we need to use
         * nextEntry +1 */
        if ((*trcBuf)->record.nextEntry != (*trcBuf)->record.firstEntry) {
          (*p)++;
          bufLeft--;
        } else {
          /* there is a possibility that we will span this entire buffer with
           * the current trace point but not with the data we are currently
           * copying so we need to set this here as a precaution. nextEntry is
           * always reconstructed at the end of the trace point from the cursor
           * so this is overwritten if it's not needed.
           */
 ...
 *
 ******************************************************************************/
static void tracePrint(OMR_TraceThread *thr, UtModuleInfo *modInfo,
                       uint32_t traceId, va_list var) {
  int id;
  char *format;
  ...
  if (modInfo == NULL) {
    ...
    moduleName = "dg";
  } else if (modInfo->traceVersionInfo->traceVersion >= 7 &&
             modInfo->containerModule != NULL) {
    omrstr_printf(qualifiedModuleName, MAX_QUALIFIED_NAME_LENGTH, "%s(%s)",
                  modInfo->name, modInfo->containerModule->name);
  ...
  if (OMR_TRACEGLOBAL(indentPrint)) {
    ...
    omrtty_err_printf("%02d:%02d:%02d.%03d%c" UT_POINTER_SPEC
                      "%16s.%-6d %c %s %c ",
                      hh, mm, ss, millis, threadSwitch, thr->id,
                      qualifiedModuleName, id, excpt, indent, entryexit);
    ...
    omrtty_err_printf("%02d:%02d:%02d.%03d%c" UT_POINTER_SPEC
                      "%16s.%-6d %c %c ",
                      hh, mm, ss, millis, threadSwitch, thr->id,
                      qualifiedModuleName, id, excpt, entryexit);
    omrtty_err_vprintf(format + 2, var);
  }
  omrtty_err_printf("\n");
  freeTraceLock(thr);
 ...
 *
 ******************************************************************************/
static void traceAssertion(OMR_TraceThread *thr, UtModuleInfo *modInfo,
                           uint32_t traceId, va_list var) {
  int id;
  const char *format;
  uint32_t hh, mm, ss, millis;
   ...
   *  Non-indented print
   */
  omrtty_err_printf("%02d:%02d:%02d.%03d " UT_POINTER_SPEC "%8s.%-6d *   ", hh,
                    mm, ss, millis, thr->id, qualifiedModuleName, id);
  omrtty_err_vprintf(format + 2, var);
 ...
 * returns     - void
 ******************************************************************************/
static void traceCount(UtModuleInfo *moduleInfo, uint32_t traceId) {
 ...
 *
 ******************************************************************************/
static void traceV(OMR_TraceThread *thr, UtModuleInfo *modInfo,
                   uint32_t traceId, const char *spec, va_list var,
                   int bufferType) {
  OMR_TraceBuffer *trcBuf;
  int lastSequence;
  int entryLength;
  ...
  if (modInfo != NULL) {
    ...
    if (modInfo->traceVersionInfo->traceVersion >= 7 &&
        modInfo->containerModule != NULL) {
  ...
  if ((p < (char *)&trcBuf->record) ||
      (p > ((char *)&trcBuf->record + OMR_TRACEGLOBAL(bufferSize)))) {
    /* the buffer's been mangled so free it for reuse and acquire another */
    UT_DBGOUT(1, ("<UT> invalid nextEntry value in record. Freed trace buffer "
                  "for thread " UT_POINTER_SPEC " and reinitialized\n",
                  thr));
  ...
  if ((OMR_TRACEGLOBAL(bufferSize) - trcBuf->record.nextEntry) >
      (UT_FASTPATH + length + 4)) {
    /* there is space in the buffer for a minimal entry, so avoid the overhead
     * of copyToBuffer func	*/
    ...
    *(p + 2) = (char)(traceId >> 8);
    ...
    /* if tracepoint is part of other component - write container's name into
     * buffer */
    ...
    *p = 12 + length;
    ...
    if (lastSequence != (int32_t)(trcBuf->record.sequence >> 32)) {
      ...
      entryLength = 0;
      copyToBuffer(thr, bufferType, UT_TRC_SEQ_WRAP_ID, &p, 3, &entryLength,
                   &trcBuf);
      copyToBuffer(thr, bufferType, (char *)&lastSequence, &p, 4, &entryLength,
                   &trcBuf);
    ...
    copyToBuffer(thr, bufferType, (char *)&intVar, &p, 4, &entryLength,
                 &trcBuf);

    /* write name length to buffer */
    copyToBuffer(thr, bufferType, (char *)&length, &p, 4, &entryLength,
                 &trcBuf);

    /* write name to buffer */
    copyToBuffer(thr, bufferType, stringVar, &p, (int)stringVarLen,
                 &entryLength, &trcBuf);

    if (containerModuleVar != NULL) {
      copyToBuffer(thr, bufferType, "(", &p, 1, &entryLength, &trcBuf);
      copyToBuffer(thr, bufferType, containerModuleVar, &p,
                   (int)containerModuleVarLen, &entryLength, &trcBuf);
      copyToBuffer(thr, bufferType, ")", &p, 1, &entryLength, &trcBuf);
    }
  ...
  if (OMR_ARE_ANY_BITS_SET(thr->currentOutputMask, UT_MAXIMAL | UT_EXCEPTION) &&
      (str != NULL)) {
    int i;
     ...
     */
    if ((length > 0) && ((p + length + 1) < (char *)&trcBuf->record +
      ...
      *p = (unsigned char)entryLength;
      ...
      int32_t left;
      while (*str != '\0') {
        left = (int32_t)((char *)&trcBuf->record + OMR_TRACEGLOBAL(bufferSize) -
                         p);
        ...
        case UT_TRACE_DATA_TYPE_INT32: {
          ...
          if (left > (int32_t)sizeof(int32_t)) {
            ...
            copyToBuffer(thr, bufferType, (char *)&intVar, &p, sizeof(int32_t),
                         &entryLength, &trcBuf);
          }
          break;
        }
        ...
        case UT_TRACE_DATA_TYPE_POINTER: {
          ...
          if (left > (int32_t)sizeof(char *)) {
            ...
            copyToBuffer(thr, bufferType, (char *)&ptrVar, &p, sizeof(char *),
                         &entryLength, &trcBuf);
          }
          break;
        }
        ...
        case UT_TRACE_DATA_TYPE_STRING: {
          if ((modInfo != NULL) &&
              (UT_APPLICATION_TRACE_MODULE == modInfo->moduleId)) {
            stringVar = format;
          } else {
          ...
          if (left > length) {
            ...
            copyToBuffer(thr, bufferType, stringVar, &p, length, &entryLength,
                         &trcBuf);
          }
          break;
        }
        ...
        case UT_TRACE_DATA_TYPE_PRECISION: {
          ...
          if (*str == UT_TRACE_DATA_TYPE_STRING) {
            ...
            if (left > (int32_t)sizeof(short)) {
              ...
              copyToBuffer(thr, bufferType, (char *)&shortVar, &p,
                           sizeof(short), &entryLength, &trcBuf);
            ...
            if (length > 0) {
              if (left > length) {
                ...
                copyToBuffer(thr, bufferType, stringVar, &p, length,
                             &entryLength, &trcBuf);
              }
            }
          }
        ...
        case UT_TRACE_DATA_TYPE_INT64: {
          ...
          if (left > (int32_t)sizeof(int64_t)) {
            ...
            copyToBuffer(thr, bufferType, (char *)&i64Var, &p, sizeof(int64_t),
                         &entryLength, &trcBuf);
          }
          break;
        }
        ...
        case UT_TRACE_DATA_TYPE_DOUBLE: {
          ...
          if (left > (int32_t)sizeof(double)) {
            ...
            copyToBuffer(thr, bufferType, (char *)&doubleVar, &p,
                         sizeof(double), &entryLength, &trcBuf);
          }
          break;
        }
        /*
        ...
        case UT_TRACE_DATA_TYPE_SHORT: {
          ...
          if (left > (int32_t)sizeof(short)) {
            ...
            copyToBuffer(thr, bufferType, (char *)&shortVar, &p, sizeof(short),
                         &entryLength, &trcBuf);
          }
          break;
        }
      ...
      if ((char *)&trcBuf->record + OMR_TRACEGLOBAL(bufferSize) - p >
        ...
        *p = (unsigned char)entryLength;
        ...
        copyToBuffer(thr, bufferType, &charVar, &p, sizeof(char), &entryLength,
                     &trcBuf);
        entryLength--;
        p--;
      }
 ...
 *
 ******************************************************************************/
static void trace(OMR_TraceThread *thr, UtModuleInfo *modinfo, uint32_t traceId,
                  int bufferType, const char *spec, ...) {
  va_list var;
...
static void logTracePoint(OMR_TraceThread *thr, UtModuleInfo *modInfo,
                          uint32_t traceId, const char *spec, va_list varArgs) {
  va_list var;
  ...
  if ((thr->currentOutputMask & UT_EXCEPTION) != 0) {
    ...
    if (*thr != OMR_TRACEGLOBAL(exceptionContext)) {
      ...
      trace(thr, NULL, (UT_TRC_CONTEXT_ID << 8) | UT_MAXIMAL,
            UT_EXCEPTION_BUFFER, pointerSpec, thr);
    }
...
void doTracePoint(OMR_TraceThread *thr, UtModuleInfo *modInfo, uint32_t traceId,
  ...
  BOOLEAN isRegular = FALSE; /* is this a regular tracepoint, and not an
                                auxiliary tracepoint? */

  if ((NULL == omrTraceGlobal) ||
      (OMR_TRACE_ENGINE_SHUTDOWN_STARTED == OMR_TRACEGLOBAL(initState))) {
    return;
  }
  ...
  if (isRegular) {
    ...
    /* Auxiliary tracepoints go where the regular tracepoint that created them
     * went. Regular tracepoints need to record where they are about to go in
     * case they generate auxiliary tracepoints.
     */
    thr->currentOutputMask = (unsigned char)(traceId & 0xFF);
  } else {
    /* This block is only executed for auxiliary tracepoints */

    /* The primary user of auxiliary tracepoints is jstacktrace. Sending
     * jstacktrace data to minimal (i.e. throwing away all the stack data) makes
     * no sense - so it is converted to maximal. currentOutputMask is reset
     * below.
     */
...
static UtProcessorInfo *getProcessorInfo(void) {
  ...
  ret = (UtProcessorInfo *)omrmem_allocate_memory(sizeof(UtProcessorInfo),
  ...
  ret->onlineProcessors =
      (uint32_t)omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE);
 ...
 */
OMR_TraceBuffer *allocateTraceBuffer(OMR_TraceThread *currentThread) {
  ...
  newTrcBuffer =
      (OMR_TraceBuffer *)pool_newElement(OMR_TRACEGLOBAL(bufferPool));
