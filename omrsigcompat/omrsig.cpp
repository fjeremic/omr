...
#pragma map(sigactionOS, "\174\174SIGACT")
  ...
  /**
   * Manually process the flags for the registered secondary handler to call it
   * as if it had been evoked by the OS in response to a signal, without
   * re-raising it.
   */
  int omrsig_handler(int sig, void* siginfo, void* uc)
  {
    ...
    if (validSignalNum(sig, false)) {
      ...
      if (handlerIsFunction(&handlerSlot.secondaryAction)) {
        ...
        /* SA_NODEFER - If set, sig will not be added to the process' signal
         * mask on entry to the signal handler unless it is included in sa_mask.
        ...
        if ((0 == ec) &&
            ((handlerSlot.secondaryAction.sa_flags & SA_NODEFER)
             ...
             /* Only AIX and zos respects that SA_RESETHAND behaves like
                SA_NODEFER by POSIX spec. */
         ...
         * SIG_DFL and the SA_SIGINFO flag will be cleared on entry to the
         * signal handler. Does not work for SIGILL or SIGTRAP, silently. Also
         * behaves like SA_NODEFER according to POSIX standard, but AIX is the
        ...
        if (0 == ec) {
          ...
          if (handlerSlot.secondaryAction.sa_flags & SA_RESETHAND) {
            ...
            omrsig_sigaction_internal(
              sig, &handlerSlot.secondaryAction, NULL, false);
          }
           ...
           * FOR NOW: null, DFL, and IGN check the sa_handler when registering
           * it, not here
           */
          if (flags & SA_SIGINFO) {
...
#pragma warning(pop)
                               ...
                               struct sigaction* oldact)
  {
    int rc = 0;
#if defined(J9ZOS390)
    /* zos does not seem to allow dlopen with NULL and no dll name. Use pragma
     * map to SIGACT instead.*/
    ...
    if (NULL == signalOS) {
      ...
      sighandler_t handler = SIG_DFL;
      /* signal() on WIN only takes SIG_IGN/SIG_DFL/function pointer as the
       * second parameter (NULL is mapped to SIG_DFL). If only querying the
       * existing sighandler_t, use SIG_DFL to get the existing sighandler_t and
       * then set it back.
       */
    ...
    if (!validSignalNum(signum, (NULL == act) || (!handlerIsFunction(act)))) {
      ...
      if (NULL != act) {
        ...
        *savedAction = *act;
         ...
         * possible to reinstall the secondary handler before entering the
        ...
        if (handlerIsFunction(&sigData[signum].primaryAction)) {
          /* If secondary has SA_ONSTACK/NOCLDSTOP/NOCLDWAIT flag and primary
           * does not, add those flags. */
