...
{
#if defined(OMR_OS_WINDOWS)
  return (SIGABRT == signum) || (SIGFPE == signum) || (SIGILL == signum) || (SIGINT == signum) || (SIGSEGV == signum)
    || (SIGTERM == signum) || (SIGBREAK == signum);
#elif defined(J9ZOS390)
  return (signum >= 0) && (signum < NSIG)
    && (((signum != SIGKILL) && (signum != SIGSTOP) && (signum != SIGTHCONT) && (signum != SIGTHSTOP)) || nullAction);
...
int omrsig_signalOS_internal(int signum, const struct sigaction* act, struct sigaction* oldact)
{
  int rc = 0;
#if defined(J9ZOS390)
...
#endif /* defined(POSIX_SIGNAL) */
          if (((sighandler_t)SIG_DFL == oact.sa_handler) || ((sighandler_t)SIG_IGN == oact.sa_handler)) {
    ...
    if (!(__SSET_IGINVALID & options)) {
      for (unsigned int i = 0; i < newct; i += 1) {
        for (unsigned int j = 0; j < NSIG; j += 1) {
          ...
          if (1 == sigismember(&newsets[i].__sa_signals, j) && !validSignalNum(j, !handlerIsFunction(&act))) {
            errno = EINVAL;
            goto failed;
          }
        }
