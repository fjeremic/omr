...
static int omrsig_sigaction_internal(int signum, const struct sigaction *act,
                                     struct sigaction *oldact, bool primary);
static bool validSignalNum(int signum, bool nullAction);
static bool handlerIsFunction(const struct sigaction *act);

#if defined(OMR_OS_WINDOWS)

typedef void(__cdecl *(__cdecl *SIGNAL)(
    _In_ int _SigNum, _In_opt_ void(__cdecl *_Func)(int)))(int);
...
static bool checkMaskEquality(int signum, const sigset_t *mask,
                              const sigset_t *otherMask);
...
static bool handlerIsFunction(const struct sigaction *act) {
  bool functionHandler = (SIG_DFL != act->sa_handler) &&
                         (SIG_IGN != act->sa_handler) &&
                         (NULL != act->sa_handler);
...
static bool validSignalNum(int signum, bool nullAction) {
#if defined(OMR_OS_WINDOWS)
  return (SIGABRT == signum) || (SIGFPE == signum) || (SIGILL == signum) ||
         (SIGINT == signum) || (SIGSEGV == signum) || (SIGTERM == signum) ||
         (SIGBREAK == signum);
#elif defined(J9ZOS390)
  return (signum >= 0) && (signum < NSIG) &&
         (((signum != SIGKILL) && (signum != SIGSTOP) &&
           (signum != SIGTHCONT) && (signum != SIGTHSTOP)) ||
          nullAction);
#else  /* defined(J9ZOS390) */
  return (signum >= 0) && (signum < NSIG) &&
         (((signum != SIGKILL) && (signum != SIGSTOP)) || nullAction);
#endif /* defined(J9ZOS390) */
}

/**
 * Manually process the flags for the registered secondary handler to call it as
 * if it had been evoked by the OS in response to a signal, without re-raising
 * it.
 */
int omrsig_handler(int sig, void *siginfo, void *uc) {
  ...
  if (validSignalNum(sig, false)) {
    ...
    if (handlerIsFunction(&handlerSlot.secondaryAction)) {
      ...
      /* SA_NODEFER - If set, sig will not be added to the process' signal mask
       * on entry to the signal handler unless it is included in sa_mask.
       * Otherwise, sig will always be added to the process' signal mask on
       * entry to the signal handler.
       */
      if ((0 == ec) && ((handlerSlot.secondaryAction.sa_flags & SA_NODEFER)
#if (defined(AIXPPC) || defined(J9ZOS390))
                        /* Only AIX and zos respects that SA_RESETHAND behaves
                           like SA_NODEFER by POSIX spec. */
      ...
      /* SA_RESETHAND - If set, the disposition of the signal will be reset to
       * SIG_DFL and the SA_SIGINFO flag will be cleared on entry to the signal
       * handler. Does not work for SIGILL or SIGTRAP, silently. Also behaves
       * like SA_NODEFER according to POSIX standard, but AIX is the supported
       * OS found to do this.
       */
      if (0 == ec) {
        ...
        if (handlerSlot.secondaryAction.sa_flags & SA_RESETHAND) {
          ...
          omrsig_sigaction_internal(sig, &handlerSlot.secondaryAction, NULL,
                                    false);
        }

        /* SA_SIGINFO - The signal handler takes three arguments, not one. In
         * this case, sa_sigaction should be used instead of sa_handler. NOTE
         * FOR NOW: null, DFL, and IGN check the sa_handler when registering it,
         * not here
         */
        if (flags & SA_SIGINFO) {
...
void(__cdecl *__cdecl signal(_In_ int signum,
                             _In_opt_ void(__cdecl *handler)(int)))(int)
#else  /* defined(OMR_OS_WINDOWS) */
...
int omrsig_signalOS_internal(int signum, const struct sigaction *act,
                             struct sigaction *oldact) {
  int rc = 0;
#if defined(J9ZOS390)
  /* zos does not seem to allow dlopen with NULL and no dll name. Use pragma map
   * to SIGACT instead.*/
...
#else  /* defined(POSIX_SIGNAL) */
  if (NULL == signalOS) {
    signalOS = (SIGNAL)GetProcAddress(GetModuleHandle(TEXT(MSVC_RUNTIME_DLL)),
                                      "signal");
  }
  if (NULL == signalOS) {
    rc = -1;
    ...
    sighandler_t handler = SIG_DFL;
    /* signal() on WIN only takes SIG_IGN/SIG_DFL/function pointer as the second
     * parameter (NULL is mapped to SIG_DFL). If only querying the existing
     * sighandler_t, use SIG_DFL to get the existing sighandler_t and then set
     * it back.
     */
  ...
  if (!validSignalNum(signum, (NULL == act) || (!handlerIsFunction(act)))) {
    ...
    if (NULL != act) {
      ...
      *savedAction = *act;
      ...
      /* Primary handler cannot support the SA_RESETHAND flag as it is not
       * possible to reinstall the secondary handler before entering the primary
       * handler function.
       */
#if defined(POSIX_SIGNAL)
      if (primary) {
      ...
      /* Register primary with OS if it exists, otherwise register the
       * secondary. */
      struct sigaction newRegisteringHandler;
      if (handlerIsFunction(&sigData[signum].primaryAction)) {
        /* If secondary has SA_ONSTACK/NOCLDSTOP/NOCLDWAIT flag and primary does
         * not, add those flags. */
        newRegisteringHandler = sigData[signum].primaryAction;
#if defined(POSIX_SIGNAL)
        newRegisteringHandler.sa_flags |=
            sigData[signum].secondaryAction.sa_flags &
...
#if defined(J9ZOS390)
      /* On zos, when an alt signal stack is set, the second call to
       * pthread_sigmask() or sigprocmask() within a signal handler will cause
       * the program to sig kill itself.
       */
...
int __sigactionset(size_t newct, const __sigactionset_t newsets[],
  ...
  if ((newct > 64) && (NULL != newsets)) {
    ...
    /* First, check for attempts to set an invalid signal. If error, change no
     * handlers. */
