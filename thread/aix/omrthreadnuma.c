...
#if !defined(_AIX61) || defined(J9OS_I5_V6R1)
static int (*PTR_ra_attach)(rstype_t, rsid_t, rstype_t, rsid_t_MODIFIED,
                            uint_t);
#else
 ...
 * @param nodeNumber The J9 node number to which we should bind the thread (0
 * means it can run on all nodes)
...
#if !defined(_AIX61) || defined(J9OS_I5_V6R1)
  PTR_ra_attach = (int (*)(rstype_t, rsid_t, rstype_t, rsid_t_MODIFIED,
                           uint_t))dlsym(RTLD_DEFAULT, "ra_attach");
#else
  PTR_ra_attach = (int (*)(rstype_t, rsid_t, rstype_t, rsid_t, uint_t))dlsym(
...
void omrthread_numa_set_enabled(BOOLEAN enabled) { isNumaAvailable = enabled; }

/**
 * Return the highest NUMA node ID available to the process.
 * The first node is always identified as 1, as 0 is used to indicate no
 * affinity.
 *
 * This function should be called to test the availability of NUMA
 * <em>before</em> calling any of the other NUMA functions.
 *
...
uintptr_t omrthread_numa_get_max_node(void) {
  ...
  if (isNumaAvailable && (0 != __ENHANCED_AFFINITY())) {
    ...
    rs_free(rsAll);
    /* SRADS are 0-indexed and contiguous so the highest node number is always 1
     * less than the total count
     * - we want to return the 1-indexed internal node values so don't subtract
     * 1
     */
    result = numSRADs;
  }
...
intptr_t omrthread_numa_set_node_affinity_nolock(omrthread_t thread,
                                                 const uintptr_t *nodeList,
                                                 uintptr_t numaCount,
                                                 uint32_t flags) {
  intptr_t result = 0;
#if defined(OMR_PORT_NUMA_SUPPORT)
  if (isNumaAvailable && (NULL != thread)) {
     ...
     */
    uintptr_t threadIsStarted =
        thread->flags & (J9THREAD_FLAG_STARTED | J9THREAD_FLAG_ATTACHED);
    uintptr_t numaNode = 0;

    if (1 == numaCount) {
      numaNode = nodeList[0];
      /* specifying that we intend to be bound to 0 doesn't make sense:  an
       * unbound thread is bound to an empty list of nodes */
...
/**
 * Set the affinity for the specified thread so that it runs only (or
 * preferentially) on processors associated with the specified NUMA nodes
 * contained in the numaNodes array (of nodeCount length). This may be called
 * before a thread is started or once it is active.
 *
 * Note that in the case that this function is called on a thread that is not
 * yet started, setting the affinity will be deferred until the thread is
 * started. This function will still return 0, indicating success, however there
 * is no way to tell if the affinity was successfully set once the thread is
 * actually started.
 *
 * @param[in] thread The thread to be modified. Can be any arbitrary
 * omrthread_t.
 * @param[in] numaNodes The array of node indexes with which to associate the
 * given thread, where 1 is the first node.
 * @param[in] nodeCount The number of nodes in the numaNodes array. (0 indicates
 * no affinity)
 *
 * @return 0 on success, -1 on failure. Note that this function will return 0
 * even if NUMA is not available. Use omrthread_numa_get_max_node() to test for
 * the availability of NUMA.
 */
intptr_t omrthread_numa_set_node_affinity(omrthread_t thread,
  ...
  if (isNumaAvailable && (NULL != thread)) {
    if (NULL != thread) {
      ...
      result = omrthread_numa_set_node_affinity_nolock(thread, numaNodes,
                                                       nodeCount, 0);
 ...
 *
 * @param[in] thread - the thread to be queried. Can be any arbitrary
 * omrthread_t
 * @param[out] numaNodes The array of node indexes with which the given thread
 * is associated, where 1 is the first node.
 * @param[in/out] nodeCount The number of nodes in the numaNodes array, on
 * input, and the number of nodes with which this thread is associated, on
 * output.  The minimum of these two values will be the number of entries
 * populated in the numaNodes array (other entries will be untouched).
 *
 * @return 0 on success, non-zero if affinity cannot be determined. Note that
 * this function will return 0 even if NUMA is not available. Use
 * omrthread_numa_get_max_node() to test for the availability of NUMA.
 */
intptr_t omrthread_numa_get_node_affinity(omrthread_t thread,
  ...
  if (isNumaAvailable && (NULL != thread)) {
    ...
    /* handle the case where we aren't on any node - which implies the special
     * node 0 (the global node) */
    if (0 == nodesSoFar) {
...
static intptr_t bindThreadToNode(omrthread_t thread, uintptr_t nodeNumber) {
  ...
  if ((NULL != PTR_ra_attach) && (NULL != PTR_rs_get_homesrad)) {
    ...
    if (nodeNumber > 0) {
      ...
      /* Bind to any SRAD. Since there's no way to explicitly do that on AIX,
       * just (nonstrictly) bind to current thread's SRADID. */
      targetSRADResourceID.at_sradid = PTR_rs_get_homesrad();
    }
    /* Currently we attach non-strictly (no R_STRICT_SRAD).  Don't forget to use
     * a non-strict call to PTR_ra_attach for nodeNumber 0 if we do ever start
     * attaching strictly
     */
    result = PTR_ra_attach(R_THREAD, thisThreadResourceID, R_SRADID,
                           targetSRADResourceID, 0);
  }
  return result;
}
