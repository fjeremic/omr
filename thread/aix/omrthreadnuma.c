  ...
  omrthread_t thread, const uintptr_t* nodeList, uintptr_t numaCount, uint32_t flags)
{
  ...
  if (isNumaAvailable && (NULL != thread)) {
    ...
    if (1 == numaCount) {
      numaNode = nodeList[0];
      /* specifying that we intend to be bound to 0 doesn't make sense:  an unbound thread is bound to an empty list of
       * nodes */
...
static intptr_t bindThreadToNode(omrthread_t thread, uintptr_t nodeNumber)
{
  ...
  if ((NULL != PTR_ra_attach) && (NULL != PTR_rs_get_homesrad)) {
    ...
    if (nodeNumber > 0) {
      ...
      /* don't forget to subtract one from the numaNode to shift it into the 0-indexed numbering scheme used by the
       * system */
      sradid_t desiredSRADID = nodeNumber - 1;
      /* attach srad to current thread */
      targetSRADResourceID.at_sradid = desiredSRADID;
    } else {
      /* Bind to any SRAD. Since there's no way to explicitly do that on AIX, just (nonstrictly) bind to current
       * thread's SRADID. */
