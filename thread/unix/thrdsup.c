 ...
 */
clockid_t timeoutClock = CLOCK_REALTIME; /**< the clock used to derive absolute
                                            time from relative wait time */
pthread_condattr_t *defaultCondAttr =
    NULL; /**< attribute passed to pthread_cond_init(). NULL means the system
             default. */
...
static intptr_t initCondAttr(void);
    ...
    defined(J9OS_I5) /* i5/OS always calls initialize_priority_map */
extern intptr_t initialize_priority_map(void);
#endif

int linux_pthread_cond_timedwait(pthread_cond_t *cond, pthread_mutex_t *mutex,
...
#if defined(LINUX) || !defined(J9_PRIORITY_MAP) || defined(J9OS_I5) ||         \
    defined(OSX)
  if (initialize_priority_map()) {
    goto thread_init_error;
  }
#endif /* defined(LINUX) || !defined(J9_PRIORITY_MAP) || defined(J9OS_I5) ||   \
          defined(OSX) */
...
#endif
  /* we ignore the return value of prctl, since naming is not supported on some
...
static intptr_t initCondAttr(void) {
  ...
  if (0 == rc) {
    ...
    /* Confirm that CLOCK_MONOTONIC is supported. We don't care about the
     * returned time. */
    if (0 == clock_gettime(CLOCK_MONOTONIC, &tp)) {
      ...
      rc = errno;
      if (0 == rc) {
        /* set rc to indicate an error, in case clock_gettime() failed to follow
         * its spec */
        rc = -1;
      }
    }
...
void ztpf_init_proc() {
  ...
  /*
   *  Set ECB attributes, ensure that these attributes are set in child ECBs
   * too.
   */
...
#if defined(J9ZOS390)
/**
 * We have to tell the z/OS LE that we want a specific kind of yielding
 * threshold, not the default kind. But, if someone has said they explicitly
 * want a specific kind, we won't override their wishes.
 *
 * See VM Idea 1090 for more information
 */
static intptr_t zos_init_yielding(void) {
  /* Suspend conversion of strings to ascii, and ensure we use the OS's setenv
...
intptr_t j9OSMutex_allocAndInit(J9OSMutex *mutex) {
  omrthread_library_t lib = GLOBAL_DATA(default_library);
  intptr_t rc = 1;
  *mutex = (J9OSMutex)omrthread_allocate_memory(lib, sizeof(**mutex),
                                                OMRMEM_CATEGORY_OSMUTEXES);
 ...
 * @return 0 on success
 */
intptr_t j9OSMutex_enter(J9OSMutex mutex) { return pthread_mutex_lock(mutex); }
 ...
 * @return 0 on success
 */
intptr_t j9OSMutex_exit(J9OSMutex mutex) { return pthread_mutex_unlock(mutex); }
 ...
 * @return 0 on success
 */
intptr_t j9OSCond_notify(J9OSCond cond) { return pthread_cond_signal(cond); }
...
intptr_t j9OSCond_allocAndInit(J9OSCond *cond) {
  omrthread_library_t lib = GLOBAL_DATA(default_library);
  intptr_t rc = 1;
  *cond = (J9OSCond)omrthread_allocate_memory(lib, sizeof(**cond),
                                              OMRMEM_CATEGORY_OSCONDVARS);
#if J9THREAD_USE_MONOTONIC_COND_CLOCK
