...
int
linux_pthread_cond_timedwait(pthread_cond_t* cond,
...
j9OSMutex_allocAndInit(J9OSMutex* mutex)
{
  omrthread_library_t lib = GLOBAL_DATA(default_library);
  intptr_t rc = 1;
  *mutex = (J9OSMutex)omrthread_allocate_memory(
    lib, sizeof(**mutex), OMRMEM_CATEGORY_OSMUTEXES);
...
j9OSCond_allocAndInit(J9OSCond* cond)
{
  omrthread_library_t lib = GLOBAL_DATA(default_library);
  intptr_t rc = 1;
  *cond = (J9OSCond)omrthread_allocate_memory(
    lib, sizeof(**cond), OMRMEM_CATEGORY_OSCONDVARS);
#if J9THREAD_USE_MONOTONIC_COND_CLOCK
