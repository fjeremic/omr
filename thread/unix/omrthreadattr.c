...
#define J9THREAD_ATTR_IS_VALID(attr) \
  ((attr) && (*(attr)) && ((*(attr))->size == sizeof(unixthread_attr)))
#define J9THREAD_VALUE_OUT_OF_RANGE(val, lo, hi) \
  (((val) < (lo)) || ((val) > (hi)))
...
static intptr_t setSchedpolicy(pthread_attr_t *pattr,
...
intptr_t omrthread_attr_init(omrthread_attr_t *attr) {
  ...
  newAttr = omrthread_allocate_memory(lib, sizeof(unixthread_attr),
                                      OMRMEM_CATEGORY_MM);
  if (!newAttr) {
...
#if defined(RS6000)
  /* this is for AIXPPC */
  /* Solaris, by default, seems to use a fixed priority scheduling mechanism for
   * its multiplexed threads which causes a running thread to never give up the
   * CPU once it has started executing. By setting the PTHREAD_SCOPE_SYSTEM
   * flag, we are telling the system to use the kernel's time-slicing scheduling
   * algorithm to choose how this thread will be scheduled. Effectively the
   * kernel, not the threads library will decide when this thread runs by
   * scheduling it among all the other threads that are running on the system.
   */
  rc = DEBUG_SYSCALL(
      pthread_attr_setscope(&newAttr->pattr, PTHREAD_SCOPE_SYSTEM));
  if (rc != 0) {
    goto destroy_attr;
  }
  ...
  if (failedToSetAttr(
          omrthread_attr_set_name((omrthread_attr_t *)&newAttr, NULL))) {
    goto destroy_attr;
  }

  if (failedToSetAttr(omrthread_attr_set_schedpolicy(
          (omrthread_attr_t *)&newAttr, J9THREAD_SCHEDPOLICY_INHERIT))) {
    goto destroy_attr;
  }

  /* set priority after policy in case RTJ wants to override the policy */
  if (failedToSetAttr(omrthread_attr_set_priority((omrthread_attr_t *)&newAttr,
                                                  J9THREAD_PRIORITY_NORMAL))) {
    goto destroy_attr;
  }

  if (failedToSetAttr(omrthread_attr_set_stacksize((omrthread_attr_t *)&newAttr,
                                                   STACK_DEFAULT_SIZE))) {
    goto destroy_attr;
  }
  ...
  if (lib->flags & J9THREAD_LIB_FLAG_NO_SCHEDULING) {
    rc = DEBUG_SYSCALL(
        pthread_attr_setinheritsched(&newAttr->pattr, PTHREAD_INHERIT_SCHED));
    if (rc != 0) {
      goto destroy_attr;
    }
  }

  if (failedToSetAttr(omrthread_attr_set_detachstate(
          (omrthread_attr_t *)&newAttr, J9THREAD_CREATE_DETACHED))) {
    goto destroy_attr;
  }

  if (failedToSetAttr(omrthread_attr_set_category(
          (omrthread_attr_t *)&newAttr, J9THREAD_CATEGORY_SYSTEM_THREAD))) {
    goto destroy_attr;
  }
 ...
 *
 * @note HACK: For RTJ, omrthread_attr_set_priority() sets the scheduling
 * policy. It doesn't set omrthread_attr_t.schedpolicy, but the schedpolicy
 * value is ignored.
 *
...
intptr_t omrthread_attr_set_priority(omrthread_attr_t *attr,
  ...
  if (J9THREAD_VALUE_OUT_OF_RANGE(priority, J9THREAD_PRIORITY_MIN,
...
static intptr_t setSchedpolicy(pthread_attr_t *pattr,
  ...
  if (J9THREAD_SCHEDPOLICY_INHERIT == policy) {
     ...
     *
     * The docs say that the schedpolicy should be ignored if
     * PTHREAD_INHERIT_SCHED is enabled, but it appears that leaving one of the
     * privileged schedpolicies set in the pthread_attr_t still causes thread
     * creation to fail. This bug has been observed on AIX 5.3, Linux 2.4, and
     * Linux 2.6.
     *
...
static intptr_t setPriority(pthread_attr_t *pattr, omrthread_prio_t priority) {
  ...
  if (omrthread_lib_use_realtime_scheduling()) {
    ...
    rc = DEBUG_SYSCALL(pthread_attr_setschedpolicy(
        pattr, omrthread_get_scheduling_policy(priority)));
    if (rc != 0) {
      return J9THREAD_ERR_INVALID_VALUE;
    }
  }
   ...
   * since we may have previously called pthread_attr_setinheritsched with
   * PTHREAD_INHERIT_SCHED in setSchedpolicy, however there is no harm in
   * setting the priority here and checking the value is valid.
   */
