...
omrthread_attr_init(omrthread_attr_t* attr)
{
  ...
  newAttr =
    omrthread_allocate_memory(lib, sizeof(unixthread_attr), OMRMEM_CATEGORY_MM);
  if (!newAttr) {
   ...
   */
  rc =
    DEBUG_SYSCALL(pthread_attr_setscope(&newAttr->pattr, PTHREAD_SCOPE_SYSTEM));
  if (rc != 0) {
    goto destroy_attr;
  }
 ...
 */
intptr_t
omrthread_attr_set_schedpolicy(omrthread_attr_t* attr,
...
omrthread_attr_set_priority(omrthread_attr_t* attr, omrthread_prio_t priority)
{
  ...
  if (J9THREAD_VALUE_OUT_OF_RANGE(
        priority, J9THREAD_PRIORITY_MIN, J9THREAD_PRIORITY_MAX)) {
    return J9THREAD_ERR_INVALID_VALUE;
  }
...
intptr_t
omrthread_attr_set_detachstate(omrthread_attr_t* attr,
...
setSchedpolicy(pthread_attr_t* pattr, omrthread_schedpolicy_t policy)
{
  ...
  if (J9THREAD_SCHEDPOLICY_INHERIT == policy) {
    ...
    pret =
      DEBUG_SYSCALL(pthread_attr_setinheritsched(pattr, PTHREAD_INHERIT_SCHED));
    if (pret != 0) {
      return J9THREAD_ERR_INVALID_VALUE;
    }
