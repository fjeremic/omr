 ...
 */
static void cpuset_logical_and(cpu_set_t* destination, const cpu_set_t* source)
{
  uintptr_t i = 0;
...
static intptr_t initializeNumaNodeData(omrthread_library_t threadLibrary, uintptr_t numNodes)
{
  uintptr_t result = 0;

  numaNodeData
    = omrthread_allocate_memory(threadLibrary, sizeof(numaNodeData[0]) * (numNodes + 1), OMRMEM_CATEGORY_THREADS);
              ...
              if (-1 != cpumapFile) {
                ...
                if ((0 != bytesRead) && (bytesRead < bufferSize - 1)) { /* the buffer was too small */
                  ...
                  do {
                    ...
                    uint32_t cpuIndex
                      = 4 * charCount; /* each hex digit represents 4 CPUs.  Use the count from the last iteration */
...
void omrthread_numa_init(omrthread_library_t threadLibrary)
{
  ...
  /* find our current affinity mask since we will need it when removing any affinity binding from threads, later (since
   * we still want to honour restrictions we inherited from the environment) */
