...
/* A table of data about each NUMA node. Node 0 is the synthetic node which
 ...
 */
static uintptr_t cpuset_subset_or_equal(cpu_set_t *subset,
                                        cpu_set_t *superset) {
  uintptr_t i = 0;
  uintptr_t result = 1;
...
/**
 * Combines the two given bitsets, destination and source, storing their logical
 * OR result into destination Note that this can be replaced with "CPU_OR" once
 * we update to glibc 2.7.
 ...
 */
static void cpuset_logical_and(cpu_set_t *destination,
...
static uintptr_t countNumaNodes(void) {
  uintptr_t maxNode = 0;
...
static intptr_t initializeNumaNodeData(omrthread_library_t threadLibrary,
                                       uintptr_t numNodes) {
  uintptr_t result = 0;

  numaNodeData = omrthread_allocate_memory(
      threadLibrary, sizeof(numaNodeData[0]) * (numNodes + 1),
  ...
  if (NULL == numaNodeData) {
    ...
    if (NULL == nodes) {
      ...
      if (NULL != node) {
        ...
        char pathBuffer[sizeof(NODE_PATH) + sizeof(CPUMAP) +
                        16]; /* extra space for "node<n>" & terminating null */
        strcpy(pathBuffer, NODE_PATH);
        char *nodeRoot = pathBuffer + sizeof(NODE_PATH) -
                         1; /* shortcut to the end of the string */
        do {
          if (1 == sscanf(node->d_name, "node%lu", &nodeIndex)) {
            if (nodeIndex < numNodes) {
              ...
              if (-1 != cpumapFile) {
                ...
                if (bytesRead ==
                    bufferSize) { /* buffer possibly not big enough */
                  bufferSize = 4096;
                  ...
                  size_t bytesRead =
                      read(cpumapFile, mapBuffer, bufferSize - 1);
                  mapBuffer[bytesRead] = '\0';
                }
                if ((0 != bytesRead) &&
                    (bytesRead <
                     bufferSize - 1)) { /* the buffer was too small */
                  /* scan from the end of the line (lowest numbered CPU) toward
                   * the front */
                  ...
                  do {
                    ...
                    uint32_t cpuIndex =
                        4 * charCount; /* each hex digit represents 4 CPUs.  Use
                                          the count from the last iteration */
                    while ((cursor > mapBuffer) && isxdigit(*cursor)) {
                      charCount += 1;
                      cursor -= 1;
                    }
                    uintmax_t cpuMap = strtoumax(
                        (cursor == mapBuffer) ? cursor : cursor + 1, NULL, 16);
                    /* cursor points to a non-hex char if not at the front of
                     * the buffer */
                    while ((0 != cpuMap)) {
                      if (0 != (cpuMap & 1)) {
              ...
              if (!cpumapOkay) {                /* read the directory entries */
                strcpy(nodeRoot, node->d_name); /* strip off "/cpulist".
                                                   Updates pathBuffer  */
...
void omrthread_numa_init(omrthread_library_t threadLibrary) {
  ...
  /* find our current affinity mask since we will need it when removing any
   * affinity binding from threads, later (since we still want to honour
   * restrictions we inherited from the environment) */
...
#endif
  {
    /* we failed to check our affinity so we shouldn't try to use the affinity
     * APIs */
    isNumaAvailable = FALSE;
  }
...
intptr_t omrthread_numa_set_node_affinity_nolock(omrthread_t thread,
  ...
  if (isNumaAvailable && (NULL != thread)) {
    ...
    if (nodeCount > 0) {
      for (listIndex = 0; (0 == result) && (listIndex < nodeCount);
           listIndex++) {
      ...
      /* remove all affinity bindings from the thread so replace it with the
       * affinity with which we started */
...
intptr_t omrthread_numa_get_node_affinity(omrthread_t thread,
  ...
  if (isNumaAvailable && (NULL != thread)) {
    ...
    if (0 != threadIsStarted) {
      ...
      /* Get the affinity for the current thread. Note that we can't simply
       * lookup the thread->numaAffinity field, since it's possible for the
       * affinity to be changed by an external program (see:
...
#endif /* defined(LINUXPPC) */
      {
        ...
        /* Look through all the numa nodes and try to see if any matches this
         * affinity mask */
        ...
        /* handle the case where we aren't on any node - which implies the
         * special node 0 (the global node) */
        if (0 == nodesSoFar) {
...
uintptr_t omrthread_numa_get_current_node() {
  unsigned node = 0;
#if defined(OMR_PORT_NUMA_SUPPORT)
  /* On some older kernels the syscall appears to be SYS_get_cpu rather than
   * SYS_getcpu */
