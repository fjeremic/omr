...
static void initialize_thread_policies(int *policy_regular_thread,
                                       int *policy_realtime_thread);
static void initializePolicy(int range_start, int range_end, int policy,
                             int map[]);
static void initializeRange(int start_index, int end_index, int range_min,
                            int range_max, int values[]);
...
/* represents the actual range of regular priorities - we may not use up the
 * entire permissible range */
...
Regular ordering:
minRegularPrio <= lowerBoundRegular <= lowerBoundRegularMapped <=
higherBoundRegularMapped <= higherBoundRegular <= maxRegularPrio
...
void printMap(void) {
  ...
  for (k = 0; k <= J9THREAD_PRIORITY_MAX; k++) {
    ...
    printf("j9 thread prio mapping: %d -> %d %s\n", k,
           omrthread_get_mapped_priority(k),
           (pol == SCHED_OTHER)
               ? "SCHED_OTHER"
               : ((pol == SCHED_RR) ? "SCHED_RR" : "SCHED_FIFO"));
  }
...
static intptr_t initialize_realtime_priority_map(void) {
  ...
  /* we will try to base the regular priorities on the current thread's
  ...
  if (range < 0) {
    /* this line should be unreachable: the POSIX.4 standard chose larger
     * numbers to represent stronger priorities */
    return -1;
  }
  ...
  /* shrink the real-time range as much as we can to fit into the 11-89 range if
   * possible */
  if (extra > 0) {
  ...
  initializeRange(J9THREAD_PRIORITY_USER_MIN, J9THREAD_PRIORITY_USER_MAX,
                  lowerBoundRegularMapped, higherBoundRegularMapped,
                  priority_map);
  initializePolicy(J9THREAD_PRIORITY_USER_MIN, J9THREAD_PRIORITY_USER_MAX,
                   policy_regular_thread, priority_map);

  /* assign the lowest priority, which is outside the range of java thread
   * priorities */

  lowestPrio = lowerBoundRegularMapped;
  initializeRange(J9THREAD_PRIORITY_MIN, J9THREAD_PRIORITY_MIN, lowestPrio,
                  lowestPrio, priority_map);
  initializePolicy(J9THREAD_PRIORITY_MIN, J9THREAD_PRIORITY_MIN,
                   policy_regular_thread, priority_map);

  /* now we define priorities 11 and above which differs for hard and soft
   * real-time */

  if (SCHED_FIFO == policy_regular_thread) {
    fprintf(stderr, "JVM cannot be invoked by a thread with scheduling policy "
                    "SCHED_FIFO.\n");
    return -1;
  }
  ...
  initializeRange(J9THREAD_PRIORITY_MAX, J9THREAD_PRIORITY_MAX, highestPrio,
                  highestPrio, priority_map);
  initializePolicy(J9THREAD_PRIORITY_MAX, J9THREAD_PRIORITY_MAX,
                   policy_regular_thread, priority_map);

  /*
   * Note that it is not just when using fixed policies and prios that the
   * current thread needs adjusting. It is also when we are basing the map on
   * the current thread's prio, but we have adjusted the current thread's prio
   * because it must fall within the allowable range.
   */
...
intptr_t set_pthread_priority(pthread_t handle,
                              omrthread_prio_t j9ThreadPriority) {
  struct sched_param sched_param;
  sched_param.sched_priority = omrthread_get_mapped_priority(j9ThreadPriority);
  return pthread_setschedparam(
      handle, omrthread_get_scheduling_policy(j9ThreadPriority), &sched_param);
}
...
#if !defined(OMRZTPF)
  /* on some platforms (i.e. Solaris) we get out of range values (e.g. 0) for
   * threads with no explicitly set priority */
...
static void initializePolicy(int range_start, int range_end, int policy,
  ...
  for (i = range_start; i <= range_end; i++) {
    map[i] =
        omrthread_get_mapped_priority(i) + PRIORITY_MAP_ADJUSTED_POLICY(policy);
  }
}
...
intptr_t set_priority_spread(void) {
  ...
  for (i = J9THREAD_PRIORITY_NORMAL - dups - 1, j = prio;
       i >= J9THREAD_PRIORITY_USER_MIN; i--) {
  ...
  for (i = J9THREAD_PRIORITY_NORMAL + dups + 1, j = prio;
       i <= J9THREAD_PRIORITY_USER_MAX; i++) {
    priority_map[i] = ++j + PRIORITY_MAP_ADJUSTED_POLICY(policy_regular_thread);
  }
  ...
  priority_map[J9THREAD_PRIORITY_MIN] =
      lowestPrio + PRIORITY_MAP_ADJUSTED_POLICY(policy_regular_thread);

  /* assign the highest priority, also outside the range of java thread
   * priorities */
  ...
  priority_map[J9THREAD_PRIORITY_MAX] =
      highestPrio + PRIORITY_MAP_ADJUSTED_POLICY(policy_regular_thread);
#if defined(DEBUG)
  printMap();
#endif
  return 0;
