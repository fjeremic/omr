...
printMap(void)
{
  ...
  fprintf(stderr,
          "min bound rt %d, max bound rt %d\n",
          lowerBoundRealtime,
          higherBoundRealtime);
  fprintf(stderr,
          "min  bound reg %d, max bound reg %d\n",
          lowerBoundRegular,
          higherBoundRegular);
  fprintf(stderr,
          "min  bound reg mapped %d, max bound reg mapped %d\n",
...
initialize_realtime_priority_map(void)
{
  ...
  initializeRange(J9THREAD_PRIORITY_USER_MIN,
                  J9THREAD_PRIORITY_USER_MAX,
                  lowerBoundRegularMapped,
                  higherBoundRegularMapped,
                  priority_map);
  initializePolicy(J9THREAD_PRIORITY_USER_MIN,
                   J9THREAD_PRIORITY_USER_MAX,
                   policy_regular_thread,
                   priority_map);
  ...
  initializeRange(J9THREAD_PRIORITY_MIN,
                  J9THREAD_PRIORITY_MIN,
                  lowestPrio,
                  lowestPrio,
                  priority_map);
  initializePolicy(J9THREAD_PRIORITY_MIN,
                   J9THREAD_PRIORITY_MIN,
                   policy_regular_thread,
                   priority_map);
  ...
  if (SCHED_FIFO == policy_regular_thread) {
    fprintf(
      stderr,
      "JVM cannot be invoked by a thread with scheduling policy SCHED_FIFO.\n");
    return -1;
  }
  ...
  initializeRange(J9THREAD_PRIORITY_MAX,
                  J9THREAD_PRIORITY_MAX,
                  highestPrio,
                  highestPrio,
                  priority_map);
  initializePolicy(J9THREAD_PRIORITY_MAX,
                   J9THREAD_PRIORITY_MAX,
                   policy_regular_thread,
                   priority_map);
