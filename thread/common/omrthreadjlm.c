 ...
 * @return 0 on success, non-zero on failure
 */
static intptr_t jlm_base_init(omrthread_library_t lib)
{
  omrthread_t thread;
...
static intptr_t jlm_init_pools(omrthread_library_t lib)
{
  ...
  if (NULL == lib->monitor_tracing_pool) {
    lib->monitor_tracing_pool = pool_new(sizeof(J9ThreadMonitorTracing), 0, 0, 0, OMR_GET_CALLSITE(),
      OMRMEM_CATEGORY_THREADS, omrthread_mallocWrapper, omrthread_freeWrapper, lib);
    if (NULL == lib->monitor_tracing_pool) {
      return -1;
    }
  }
  ...
  if (NULL == lib->thread_tracing_pool) {
    lib->thread_tracing_pool = pool_new(sizeof(J9ThreadTracing), 0, 0, 0, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_THREADS,
