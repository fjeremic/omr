 ...
 *
 * @note The handle returned will be > 0, so it is safe to test the handle
 * against 0 to see if it's been allocated yet.
 *
 * @param[out] handle pointer to a key to be initialized with a key value
 * @return 0 on success or negative value if a key could not be allocated (i.e.
 * all TLS has been allocated)
 *
 ...
 *
 * @param[out] handle pointer to a key to be initialized with a key value
 * @param[in] finalizer a finalizer function which will be invoked when a thread
 * is detached or terminates if the thread's TLS entry for this key is non-NULL
 * @return 0 on success or negative value if a key could not be allocated (i.e.
 * all TLS has been allocated)
 *
 * @see omrthread_tls_free, omrthread_tls_set
...
void omrthread_tls_finalize(omrthread_t thread) {
  ...
  for (index = 0; index < J9THREAD_MAX_TLS_KEYS; index++) {
    if (thread->tls[index] != NULL) {
      ...
      /* read the value and finalizer together under mutex to be sure that they
       * belong together */
      OMROSMUTEX_ENTER(lib->tls_mutex);
