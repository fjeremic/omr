...
#define ENABLE_RAW_MONITOR_SPIN(thread, monitor)                                                       \
  do {                                                                                                 \
    if (NULL == (monitor)->customSpinOptions) {                                                        \
      ...
      Trc_THR_EnableRawMonitorSpin_CustomSpinOption((monitor)->name, (monitor), (monitor)->spinCount1, \
        (monitor)->spinCount2, (monitor)->spinCount3, (monitor)->customSpinOptions->customAdaptSpin);  \
    }                                                                                                  \
...
#define ADAPT_DISABLE_SPIN_CHECK(thread, monitor)                                                               \
  do {                                                                                                          \
    if (IS_ADAPTIVE_SPIN_REQUIRED(monitor)) {                                                                   \
      if ((IS_ADAPT_HOLDTIME_ENABLED((thread), (monitor)) && ((monitor)->tracing->holdtime_count > 0)           \
            && (JLM_AVERAGE_HOLDTIME(monitor) > (thread)->library->adaptSpinHoldtime))                          \
        || (IS_ADAPT_SLOW_PERCENT_ENABLED((thread), (monitor)) && ((monitor)->tracing->enter_count > 100)       \
...
#define SHOULD_ENABLE_SAMPLING(thread, monitor, isSlowEnter)                            \
  ((isSlowEnter) && SHOULD_SAMPLE_MONITOR_TYPE(monitor) && (NULL != (monitor)->tracing) \
    && (0 == (monitor)->tracing->enter_count) && (0 != ((monitor)->flags & J9THREAD_MONITOR_STOP_SAMPLING)))
#else /* OMR_THR_ADAPTIVE_SPIN */
