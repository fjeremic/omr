...
#define DEBUG (0)
  ...
  intptr_t omrthread_mcs_lock(omrthread_t self,
                              omrthread_monitor_t monitor,
                              omrthread_mcs_node_t mcsNode,
                              BOOLEAN retry);

  intptr_t omrthread_mcs_trylock(omrthread_t self,
                                 omrthread_monitor_t monitor,
                                 omrthread_mcs_node_t mcsNode);

  omrthread_t omrthread_mcs_unlock(omrthread_t self,
                                   omrthread_monitor_t monitor);
...
#define ADAPT_MONITOR_TRACE(thread, monitor, adaptTracepoint)                  \
  adaptTracepoint(                                                             \
    (IS_OBJECT_MONITOR(monitor) ? "object" : "system"),                        \
    (monitor),                                                                 \
...
#define UPDATE_JLM_MON_EXIT(self, monitor)                                     \
  do {                                                                         \
    ...
    CLEAR_MONITOR_FLAGS(                                                       \
      (monitor),                                                               \
      (J9THREAD_MONITOR_IGNORE_ENTER | J9THREAD_MONITOR_SLOW_ENTER));          \
          ...
          if (holdTime > 0) {                                                  \
            if ((0 == (self)->library->clock_skew) ||                          \
              ...
              (monitor)->tracing->holdtime_avg =                               \
                (monitor)->tracing->holdtime_sum / ((uint64_t)holdTimeCount);  \
