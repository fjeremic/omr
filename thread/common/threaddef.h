...
#define IS_JLM_SAMPLE_ENABLED(thread, monitor) ((NULL == (monitor)->customSpinOptions) ? ((thread)->library->flags & J9THREAD_LIB_FLAG_JLM_INFO_SAMPLING_ENABLED) : (CUSTOM_ADAPTIVE_SPIN_TRUE == (monitor)->customSpinOptions->customAdaptSpin))
#define IS_JLM_HOLDTIME_SAMPLING_ENABLED(thread, monitor) ((NULL == (monitor)->customSpinOptions) ? ((thread)->library->flags & J9THREAD_LIB_FLAG_JLM_HOLDTIME_SAMPLING_ENABLED) : (CUSTOM_ADAPTIVE_SPIN_TRUE == (monitor)->customSpinOptions->customAdaptSpin))
#define IS_ADAPT_HOLDTIME_ENABLED(thread, monitor) ((NULL == (monitor)->customSpinOptions) ? ((thread)->library->flags & J9THREAD_LIB_FLAG_JLM_HOLDTIME_SAMPLING_ENABLED) : (CUSTOM_ADAPTIVE_SPIN_TRUE == (monitor)->customSpinOptions->customAdaptSpin))
#define IS_ADAPT_SLOW_ENABLED(thread, monitor) ((NULL == (monitor)->customSpinOptions) ? ((thread)->library->flags & J9THREAD_LIB_FLAG_JLM_SLOW_SAMPLING_ENABLED) : (CUSTOM_ADAPTIVE_SPIN_TRUE == (monitor)->customSpinOptions->customAdaptSpin))
...
#define SHOULD_DISABLE_ADAPT_SAMPLING(thread, monitor) \
    (IS_ADAPT_HOLDTIME_ENABLED((thread), (monitor)) && ((monitor)->tracing->holdtime_count > 0) && (JLM_NON_RECURSIVE_ENTER_COUNT(monitor) >= ADAPT_SAMPLE_STOP_MIN_COUNT((thread), (monitor))) && (JLM_AVERAGE_HOLDTIME(monitor) < ADAPT_SAMPLE_STOP_MAX_HOLDTIME((thread), (monitor))))
...
#define TAKE_JLM_SAMPLE(thread, monitor) \
    (((thread)->library->flags & J9THREAD_LIB_FLAG_JLM_ENABLED) || (IS_ADAPT_SAMPLING_ENABLED((thread), (monitor)) && (0 == (monitor)->sampleCounter)))
...
#define SHOULD_ENABLE_SAMPLING(thread, monitor, isSlowEnter) \
    ((isSlowEnter) && SHOULD_SAMPLE_MONITOR_TYPE(monitor) && (NULL != (monitor)->tracing) && (0 == (monitor)->tracing->enter_count) && (0 != ((monitor)->flags & J9THREAD_MONITOR_STOP_SAMPLING)))
#else /* OMR_THR_ADAPTIVE_SPIN */
