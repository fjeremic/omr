...
intptr_t omrthread_spinlock_acquire(omrthread_t self,
                                    omrthread_monitor_t monitor);
intptr_t omrthread_spinlock_acquire_no_spin(omrthread_t self,
                                            omrthread_monitor_t monitor);
uintptr_t omrthread_spinlock_swapState(omrthread_monitor_t monitor,
                                       uintptr_t newState);

#if defined(OMR_THR_MCS_LOCKS)
intptr_t omrthread_mcs_lock(omrthread_t self, omrthread_monitor_t monitor,
                            omrthread_mcs_node_t mcsNode, BOOLEAN retry);

intptr_t omrthread_mcs_trylock(omrthread_t self, omrthread_monitor_t monitor,
                               omrthread_mcs_node_t mcsNode);
...
#define MACRO_SELF()     \
  ((omrthread_t)TLS_GET( \
      ((omrthread_library_t)GLOBAL_DATA(default_library))->self_ptr))
...
#define GLOBAL_TRY_LOCK(self, caller) \
  OMROSMUTEX_TRY_ENTER((self)->library->monitor_mutex)
...
#define IS_OBJECT_MONITOR(monitor) \
  (J9THREAD_MONITOR_OBJECT == ((monitor)->flags & J9THREAD_MONITOR_OBJECT))

#define IS_JLM_ENABLED(thread) \
  ((thread)->library->flags & J9THREAD_LIB_FLAG_JLM_INIT_DATA_STRUCTURES)
...
#define IS_JLM_HST_ENABLED(thread) \
  ((thread)->library->flags & J9THREAD_LIB_FLAG_JLMHST_ENABLED)
...
#define IS_JLM_SAMPLE_ENABLED(thread, monitor)         \
  ((NULL == (monitor)->customSpinOptions)              \
       ? ((thread)->library->flags &                   \
          J9THREAD_LIB_FLAG_JLM_INFO_SAMPLING_ENABLED) \
       : (CUSTOM_ADAPTIVE_SPIN_TRUE ==                 \
          (monitor)->customSpinOptions->customAdaptSpin))
#define IS_JLM_HOLDTIME_SAMPLING_ENABLED(thread, monitor)  \
  ((NULL == (monitor)->customSpinOptions)                  \
       ? ((thread)->library->flags &                       \
          J9THREAD_LIB_FLAG_JLM_HOLDTIME_SAMPLING_ENABLED) \
       : (CUSTOM_ADAPTIVE_SPIN_TRUE ==                     \
          (monitor)->customSpinOptions->customAdaptSpin))
...
#define IS_ADAPT_SLOW_ENABLED(thread, monitor)         \
  ((NULL == (monitor)->customSpinOptions)              \
       ? ((thread)->library->flags &                   \
          J9THREAD_LIB_FLAG_JLM_SLOW_SAMPLING_ENABLED) \
       : (CUSTOM_ADAPTIVE_SPIN_TRUE ==                 \
          (monitor)->customSpinOptions->customAdaptSpin))
...
#define IS_ADAPT_SLOW_PERCENT_ENABLED(thread, monitor) \
  (IS_ADAPT_SLOW_ENABLED((thread), (monitor)) &&       \
   (0 != (thread)->library->adaptSpinSlowPercent))

#define JLM_NON_RECURSIVE_ENTER_COUNT(monitor) \
  ((monitor)->tracing->enter_count - (monitor)->tracing->recursive_count)
#define JLM_AVERAGE_HOLDTIME(monitor) ((monitor)->tracing->holdtime_avg)
#define JLM_SLOW_PERCENT(monitor)           \
  (((monitor)->tracing->slow_count * 100) / \
   JLM_NON_RECURSIVE_ENTER_COUNT(monitor))

#define ADAPT_MONITOR_TRACE(thread, monitor, adaptTracepoint)                 \
  adaptTracepoint(                                                            \
      (IS_OBJECT_MONITOR(monitor) ? "object" : "system"), (monitor),          \
      (uint64_t)(monitor)->tracing->holdtime_sum,                             \
      (monitor)->tracing->holdtime_count,                                     \
      (uint64_t)((monitor)->tracing->holdtime_count > 0                       \
                     ? JLM_AVERAGE_HOLDTIME(monitor)                          \
                     : 0),                                                    \
...
#define ENABLE_RAW_MONITOR_SPIN(thread, monitor)                            \
  do {                                                                      \
    if (NULL == (monitor)->customSpinOptions) {                             \
      ...
      (monitor)->spinCount1 =                                               \
          (monitor)->customSpinOptions->customThreeTierSpinCount1;          \
      (monitor)->spinCount2 =                                               \
          (monitor)->customSpinOptions->customThreeTierSpinCount2;          \
      (monitor)->spinCount3 =                                               \
          (monitor)->customSpinOptions->customThreeTierSpinCount3;          \
      Trc_THR_EnableRawMonitorSpin_CustomSpinOption(                        \
          (monitor)->name, (monitor), (monitor)->spinCount1,                \
          (monitor)->spinCount2, (monitor)->spinCount3,                     \
          (monitor)->customSpinOptions->customAdaptSpin);                   \
    }                                                                       \
  ...
  do {                                                                     \
    if (IS_ADAPTIVE_SPIN_REQUIRED(monitor)) {                              \
      if ((IS_ADAPT_HOLDTIME_ENABLED((thread), (monitor)) &&               \
           ...
           (JLM_AVERAGE_HOLDTIME(monitor) >                                \
            (thread)->library->adaptSpinHoldtime)) ||                      \
          (IS_ADAPT_SLOW_PERCENT_ENABLED((thread), (monitor)) &&           \
           ((monitor)->tracing->enter_count > 100) &&                      \
           (JLM_SLOW_PERCENT(monitor) >=                                   \
            (thread)->library->adaptSpinSlowPercent))) {                   \
...
#define ADAPT_SAMPLE_STOP_MIN_COUNT(thread, monitor) \
  ((thread)->library->adaptSpinSampleStopCount)

#define ADAPT_SAMPLE_STOP_MAX_HOLDTIME(thread, monitor) \
  (JLM_NON_RECURSIVE_ENTER_COUNT(monitor) *             \
   (thread)->library->adaptSpinSampleCountStopRatio)

#define SHOULD_DISABLE_ADAPT_SAMPLING(thread, monitor)   \
  (IS_ADAPT_HOLDTIME_ENABLED((thread), (monitor)) &&     \
   ((monitor)->tracing->holdtime_count > 0) &&           \
   (JLM_NON_RECURSIVE_ENTER_COUNT(monitor) >=            \
    ADAPT_SAMPLE_STOP_MIN_COUNT((thread), (monitor))) && \
   (JLM_AVERAGE_HOLDTIME(monitor) <                      \
    ADAPT_SAMPLE_STOP_MAX_HOLDTIME((thread), (monitor))))

#define JLM_INIT_SAMPLE_INTERVAL(lib, monitor) \
  ((monitor)->sampleCounter = (lib)->adaptSpinSampleThreshold)
...
#define IS_ADAPT_SAMPLING_ENABLED(thread, monitor) \
  (IS_JLM_SAMPLE_ENABLED((thread), (monitor)) &&   \
...
#define TAKE_JLM_SAMPLE(thread, monitor)                         \
  (((thread)->library->flags & J9THREAD_LIB_FLAG_JLM_ENABLED) || \
   (IS_ADAPT_SAMPLING_ENABLED((thread), (monitor)) &&            \
    (0 == (monitor)->sampleCounter)))
...
#else /* OMR_THR_THREE_TIER_LOCKING */
/* We don't sample for non-object monitors as there is no spinning for these
 * monitors when three tier spinning is disabled. */
...
#define SHOULD_ENABLE_SAMPLING(thread, monitor, isSlowEnter)                 \
  ((isSlowEnter) && SHOULD_SAMPLE_MONITOR_TYPE(monitor) &&                   \
   (NULL != (monitor)->tracing) && (0 == (monitor)->tracing->enter_count) && \
   (0 != ((monitor)->flags & J9THREAD_MONITOR_STOP_SAMPLING)))
#else /* OMR_THR_ADAPTIVE_SPIN */
...
#define UPDATE_JLM_MON_EXIT(self, monitor)                           \
  do {                                                               \
    ...
    CLEAR_MONITOR_FLAGS((monitor), (J9THREAD_MONITOR_IGNORE_ENTER |  \
                                    J9THREAD_MONITOR_SLOW_ENTER));   \
...
#define UPDATE_JLM_MON_EXIT_HOLD_TIMES(self, monitor)                         \
  do {                                                                        \
    if ((NULL != (monitor)->tracing) &&                                       \
        (0 != (monitor)->tracing->enter_time)) {                              \
      ASSERT((self)->tracing);                                                \
      /* If enter count is 0, this monitor was held when jlm was initialized, \
       * so ignore stats this time */                                         \
      if ((monitor)->tracing->enter_count > 0) {                              \
        if ((self)->tracing->pause_count ==                                   \
            (monitor)->tracing->enter_pause_count) {                          \
          omrtime_delta_t holdTime =                                          \
              GET_HIRES_CLOCK() - (monitor)->tracing->enter_time;             \
          if (holdTime > 0) {                                                 \
            if ((0 == (self)->library->clock_skew) ||                         \
                ((omrtime_t)holdTime > (self)->library->clock_skew)) {        \
              uintptr_t holdTimeCount =                                       \
                  (monitor)->tracing->holdtime_count + 1;                     \
