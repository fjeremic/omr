 ...
 *available. See traces for the actual error code.
...
intptr_t omrthread_get_cpu_time_ex(omrthread_t thread, int64_t* cpuTime)
{
...
#if defined(J9ZOS390)
  {
    ...
    if (data_offset > ((struct pgthb*)output_buffer)->lenused || data_offset > output_size - sizeof(struct pgthj)) {
      Trc_THR_ThreadGetCpuTime_BpxgthBuffer(
        thread, ret_val, data_offset, ((struct pgthb*)output_buffer)->lenused, output_size);
      return J9THREAD_ERR;
    }
  ...
  if (processTime != NULL) {
    ...
    if (GetProcessTimes(GetCurrentProcess(), &creationTime, &exitTime, &systemTime, &userTime)) {
      processTime->_userTime
        = GET_PROCESS_TIMES_IN_NANO * ((int64_t)userTime.dwLowDateTime | ((int64_t)userTime.dwHighDateTime << 32));
      processTime->_systemTime
        = GET_PROCESS_TIMES_IN_NANO * ((int64_t)systemTime.dwLowDateTime | ((int64_t)systemTime.dwHighDateTime << 32));
      return 0;
    } else {
