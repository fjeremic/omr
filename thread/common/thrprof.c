...
omrthread_get_process_times(omrthread_process_time_t* processTime)
{
    if (processTime != NULL) {
        ...
        if (GetProcessTimes(GetCurrentProcess(), &creationTime, &exitTime, &systemTime, &userTime)) {
            processTime->_userTime = GET_PROCESS_TIMES_IN_NANO * ((int64_t)userTime.dwLowDateTime | ((int64_t)userTime.dwHighDateTime << 32));
            processTime->_systemTime = GET_PROCESS_TIMES_IN_NANO * ((int64_t)systemTime.dwLowDateTime | ((int64_t)systemTime.dwHighDateTime << 32));
            return 0;
        } else {
        ...
        if (0 == getrusage(RUSAGE_SELF, &rUsage)) {
            processTime->_userTime = (SEC_TO_NANO_CONVERSION_CONSTANT * (int64_t)rUsage.ru_utime.tv_sec) + (MICRO_TO_NANO_CONVERSION_CONSTANT * (int64_t)rUsage.ru_utime.tv_usec);
            processTime->_systemTime = (SEC_TO_NANO_CONVERSION_CONSTANT * (int64_t)rUsage.ru_stime.tv_sec) + (MICRO_TO_NANO_CONVERSION_CONSTANT * (int64_t)rUsage.ru_stime.tv_usec);
            return 0;
        } else {
            /* Error in getrusage */
