...
/*
 *	trent: the following must come before the standard includes because
 *thrdsup.h includes windows.h in Win32.
 */
#include "thrdsup.h"
 ...
 * @retval -1 if not supported on this platform or some other platform specific
 * error occurred. See traces for the actual error code.
 * @see omrthread_get_self_cpu_time, omrthread_get_user_time,
 * omrthread_get_cpu_time_ex
 */
int64_t omrthread_get_cpu_time(omrthread_t thread) {
  int64_t cpuTime = 0;
 ...
 * If arbitrary thread times are not supported, but current thread times are
 * supported, this function will still return J9THREAD_ERR even if the current
 *thread is passed in as an argument.
 * @param[in] thread
 * @param[out] cpuTime	Amount of CPU time used in nanoseconds of thread on
 *success
 * @return success or error code
 * @retval J9THREAD_SUCCESS success
 * @retval J9THREAD_ERR_NO_SUCH_THREAD if the thread handle is invalid, or
 *J9THREAD_ERR if not supported on this platform or some other platform specific
 *error occurred.
 * @retval J9THREAD_ERR_OS_ERRNO_SET Bit flag optionally or'd into the return
 *code. Indicates that an os_errno is available. See traces for the actual error
 *code.
 * @see omrthread_get_self_cpu_time, omrthread_get_user_time,
 *omrthread_get_cpu_time
 */
intptr_t omrthread_get_cpu_time_ex(omrthread_t thread, int64_t* cpuTime) {
...
#if defined(OMR_OS_WINDOWS) && !defined(BREW)
  {
    ...
    /* WARNING! Not supported on Win95!  Need to test to ensure this fails
     * gracefully */

    if (GetThreadTimes(thread->handle, &creationTime, &exitTime, &kernelTime,
                       &userTime)) {
      totalTime = ((int64_t)kernelTime.dwLowDateTime |
                   ((int64_t)kernelTime.dwHighDateTime << 32)) +
                  ((int64_t)userTime.dwLowDateTime |
                   ((int64_t)userTime.dwHighDateTime << 32));
      ...
      *cpuTime = totalTime * 100;
      ...
      /* For any other (internal) errors other than a dead thread, return
       * J9THREAD_ERR. */
    }
...
#ifdef AIXPPC
  {
    ...
    result = pthread_getrusage_np(thread->handle, &usageInfo,
                                  PTHRDSINFO_RUSAGE_COLLECT);
    if (0 == result) {
      int64_t totalTime =
          ((int64_t)usageInfo.ru_stime.tv_sec + usageInfo.ru_utime.tv_sec) *
          1000 * 1000; /* microseconds */
      totalTime +=
          (int64_t)usageInfo.ru_stime.tv_usec + usageInfo.ru_utime.tv_usec;
      *cpuTime = totalTime * 1000; /* convert to nanoseconds */
      return J9THREAD_SUCCESS;
    } else {
      /* Handle error conditions. */
      Trc_THR_omrthread_get_cpu_time_ex_pthread_getrusage_np_failed(
          (int32_t)result, thread);
      thread->os_errno = (omrthread_os_errno_t)result;
      ret = J9THREAD_ERR_OS_ERRNO_SET;
      if (ESRCH == result) {
        /* Such a thread was not found. */
    ...
    if (0 == result) {
      ...
      if (clock_gettime(clock_id, &time) == 0) {
        *cpuTime = ((int64_t)time.tv_sec * 1000 * 1000 * 1000) + time.tv_nsec;
        ...
        Trc_THR_omrthread_get_cpu_time_ex_clock_gettime_failed(
            (int32_t)clock_id, (intptr_t)last_errno, thread);
        thread->os_errno = (omrthread_os_errno_t)last_errno;
        ret = J9THREAD_ERR_OS_ERRNO_SET;
      }
    } else {
      Trc_THR_omrthread_get_cpu_time_ex_pthread_getcpuclockid_failed(
          (int32_t)result, thread);
      thread->os_errno = (omrthread_os_errno_t)result;
      ret = J9THREAD_ERR_OS_ERRNO_SET;
      /* Handle error conditions. */
...
#if defined(J9ZOS390)
  {
...
#else
    BPX1GTH(&input_size, &cursor, &output_size, &output_buffer, &ret_val,
    ...
    if (PGTH_OK != ((struct pgthb*)output_buffer)->limitc) {
      Trc_THR_ThreadGetCpuTime_BpxgthData(
          thread, ret_val, ((struct pgthb*)output_buffer)->limitc);
      return J9THREAD_ERR;
    }
    ...
    if (data_offset > ((struct pgthb*)output_buffer)->lenused ||
      ...
      Trc_THR_ThreadGetCpuTime_BpxgthBuffer(
          thread, ret_val, data_offset, ((struct pgthb*)output_buffer)->lenused,
          output_size);
      return J9THREAD_ERR;
    }
...
#if defined(OSX)
  {
    ...
    *cpuTime =
        ((int64_t)tbi->user_time.seconds + tbi->system_time.seconds) * 1000 *
            1000 * 1000 +
        ((int64_t)tbi->user_time.microseconds + tbi->system_time.microseconds) *
            1000;
 ...
 *
 * @param[in] self The current thread. Must be non-NULL. Pass this in to avoid
 * the cost of pthread_getspecific().
 * @return time in nanoseconds
 * @retval -1 not supported on this platform
 * @see omrthread_get_cpu_time, omrthread_get_self_user_time
 */
int64_t omrthread_get_self_cpu_time(omrthread_t self) {
...
#if defined(J9ZOS390)
  {
    ...
    /* _CPUTIME returns time in TOD format (see z/Architecture Principles of
     * Operation) */
    _CPUTIME(&time);
 ...
 * @see omrthread_get_cpu_time, omrthread_get_self_user_time
 */
int64_t omrthread_get_user_time(omrthread_t thread) {
#if defined(OMR_OS_WINDOWS) && !defined(BREW)
  ...
  if (GetThreadTimes(thread->handle, &creationTime, &exitTime, &kernelTime,
                     &userTime)) {
    totalTime = ((int64_t)userTime.dwLowDateTime |
                 ((int64_t)userTime.dwHighDateTime << 32));
  ...
  memset(&usageInfo, 0, sizeof(usageInfo));
  if (0 == pthread_getrusage_np(thread->handle, &usageInfo,
                                PTHRDSINFO_RUSAGE_COLLECT)) {
    int64_t userTime =
        (int64_t)usageInfo.ru_utime.tv_sec * 1000 * 1000; /* microseconds */
 ...
 */
int64_t omrthread_get_self_user_time(omrthread_t self) {
 ...
 * @return OS handle
 */
uintptr_t omrthread_get_handle(omrthread_t thread) {
#if defined(J9ZOS390)
  /* Hack!! - If we do the simple cast (in the #else case) we get the following
          compiler error in z/OS:
                  "ERROR CBC3117 ./thrprof.c:79    Operand must be a scalar
     type." In order to work around the compiler error, we have to reach inside
 ...
 *
 */
uintptr_t omrthread_get_stack_usage(omrthread_t thread) {
#if defined(LINUX) || defined(J9ZOS390) || defined(AIXPPC) || defined(OSX)
  return 0;
...
void paint_stack(omrthread_t thread) {
  ...
  thread->stacksize =
      ((uintptr_t)stack - (uintptr_t)thread->tos + sysInfo.dwPageSize) &
      ~((uintptr_t)sysInfo.dwPageSize - 1);
#endif /* defined(OMR_OS_WINDOWS) */
}
 ...
 *
 */
uintptr_t omrthread_get_stack_size(omrthread_t thread) {
 ...
 * @param[in] priority pointer to location where priority will be stored
 * (non-NULL)
 * @return 0 on success or negative value on failure
 *
 */
intptr_t omrthread_get_os_priority(omrthread_t thread,
                                   intptr_t* policy,
                                   intptr_t* priority) {
 ...
 * @see omrthread_get_self_cpu_time, omrthread_get_user_time
 */
int64_t omrthread_get_process_cpu_time(void) {
#if defined(OMR_OS_WINDOWS) && !defined(BREW)
  FILETIME creationTime, exitTime, kernelTime, userTime;
  int64_t totalTime;
...
/**
 * Calculates the total amount of process i.e system (kernel) and user time in
 * nanoseconds.
 * @param[out] processTime is the address of the structure that the user and
 * system time gets stored in.
 * @return 0 : successful completion; -1 : Unsupported / not implemented on this
 * platform; -2 : Error occured.
 */
intptr_t omrthread_get_process_times(omrthread_process_time_t* processTime) {
  if (processTime != NULL) {
#if defined(OMR_OS_WINDOWS)
    ...
    /* rvalue's are in 100 nanosecond units. Convert to nano's by multiplying
     * 100 to them WARNING: GetProcessTimes does not exist on pre-Win 98
     */
    if (GetProcessTimes(GetCurrentProcess(), &creationTime, &exitTime,
                        &systemTime, &userTime)) {
      ...
      processTime->_systemTime = GET_PROCESS_TIMES_IN_NANO *
                                 ((int64_t)systemTime.dwLowDateTime |
    ...
    /* if getrusage() returns successfully, store the time values in
     * processTime.*/
    if (0 == getrusage(RUSAGE_SELF, &rUsage)) {
      processTime->_userTime =
          (SEC_TO_NANO_CONVERSION_CONSTANT * (int64_t)rUsage.ru_utime.tv_sec) +
          (MICRO_TO_NANO_CONVERSION_CONSTANT *
           (int64_t)rUsage.ru_utime.tv_usec);
      processTime->_systemTime =
          (SEC_TO_NANO_CONVERSION_CONSTANT * (int64_t)rUsage.ru_stime.tv_sec) +
          (MICRO_TO_NANO_CONVERSION_CONSTANT *
           (int64_t)rUsage.ru_stime.tv_usec);
      return 0;
    } else {
      /* Error in getrusage */
 ...
 * This code is a copy of omrtime_nano_time for Linux and omrtime_hires_clock on
 * Windows from the port library as we cannot call the port functions from the
 * thread library.
 *
...
#define J9TIME_NANOSECONDS_PER_SECOND J9CONST_U64(1000000000)
  ...
  if (0 == clock_gettime(CLOCK_MONOTONIC, &ts)) {
    hiresTime = ((uint64_t)ts.tv_sec * J9TIME_NANOSECONDS_PER_SECOND) +
                (uint64_t)ts.tv_nsec;
  }

  return hiresTime;
#elif defined(OMR_OS_WINDOWS) /* defined(LINUX) && (defined(J9HAMMER) || \
                                 defined(J9X86)) */
  LARGE_INTEGER i;
...
#define J9TIME_NANOSECONDS_PER_SECOND J9CONST_U64(1000000000)
  ...
  if (KERN_SUCCESS == clock_get_time(lib->clockService, &mt)) {
    hiresTime = ((uint64_t)mt.tv_sec * J9TIME_NANOSECONDS_PER_SECOND) +
                (uint64_t)mt.tv_nsec;
  }
...
/**
 * Calculate the cpu usage information for the thread categories of System,
 * application and monitor. Further categorize system JVM threads into GC, JIT
 * and others. Also add the usage details of threads that have already exited
 ...
 *         and -J9THREAD_ERR_USAGE_RETRIEVAL_UNSUPPORTED if
 * -XX:-EnableCPUMonitor has been set.
 */
intptr_t omrthread_get_jvm_cpu_usage_info(J9ThreadsCpuUsage* cpuUsage) {
  omrthread_library_t lib = GLOBAL_DATA(default_library);
  ...
  /* Walk the list of omrthread's and get the cpu time for each and categorize
   * them */
  walkThread = pool_startDo(lib->thread_pool, &state);
  for (; NULL != walkThread; walkThread = pool_nextDo(&state)) {
    /* If a thread has just been created, possible that the mutex is not yet
     * initialized. So we check if J9THREAD_FLAG_CPU_SAMPLING_ENABLED is set
     * without holding the lock first. If not set, we ignore the thread */
    if (0 == (walkThread->flags & J9THREAD_FLAG_CPU_SAMPLING_ENABLED)) {
      continue;
    }
    THREAD_LOCK(walkThread, CALLER_GET_JVM_CPU_USAGE_INFO);
    /* Check the flag again, this time with the THREAD_LOCK held, to ensure that
     * it is still marked ready for cpu accounting */
    if (0 == (walkThread->flags & J9THREAD_FLAG_CPU_SAMPLING_ENABLED)) {
    ...
    /* Obtaining CPU time fails with error code J9THREAD_ERR_NO_SUCH_THREAD,
     * implying the thread isn't available to the OS; skip accounting for such
     * threads, as storeExitCpuUsage() must have already done this for it.
     * Return error for other (internal) errors.
     */
    if (J9THREAD_SUCCESS != result) {
    ...
    if (OMR_ARE_ALL_BITS_SET(walkThread->effective_category,
                             J9THREAD_CATEGORY_RESOURCE_MONITOR_THREAD)) {
      resourceMonitorCpuTime += threadCpuTime;
    } else if (OMR_ARE_ALL_BITS_SET(walkThread->effective_category,
      ...
      /* If the thread has been set to a user defined category, count it in the
       * right category */
      if ((walkThread->effective_category &
           J9THREAD_USER_DEFINED_THREAD_CATEGORY_MASK) > 0) {
        int userCat = (walkThread->effective_category -
  ...
  /* Add the values that we have obtained above to the CPU usage of the threads
   * that have previously exited.
   */
  cpuUsage->timestamp = (preTimestamp + postTimestamp) / 2;
  cpuUsage->applicationCpuTime =
      cumulativeUsage->applicationCpuTime + applicationCpuTime;
  cpuUsage->resourceMonitorCpuTime =
      cumulativeUsage->resourceMonitorCpuTime + resourceMonitorCpuTime;
  cpuUsage->systemJvmCpuTime =
      cumulativeUsage->systemJvmCpuTime + systemJvmCpuTime;
  ...
  for (i = 0; i < J9THREAD_MAX_USER_DEFINED_THREAD_CATEGORIES; i++) {
    /* Temp workaround: i386 generates xmm instructions to optimize the array
     * copy and ends up crashing for some unknown reason. The if check
     * introduces variability in the loop causing gcc not to use xmm.
     */
    if ((cumulativeUsage->applicationUserCpuTime[i] > 0) ||
        (userCpuTime[i] > 0)) {
      cpuUsage->applicationUserCpuTime[i] =
          cumulativeUsage->applicationUserCpuTime[i] + userCpuTime[i];
    }
  }

err_exit:
  lib->threadWalkMutexesHeld &=
      ~(uintptr_t)THREAD_WALK_RESOURCE_USAGE_MUTEX_HELD;
  ...
  if (ret < 0) {
    ...
    Trc_THR_omrthread_get_jvm_cpu_usage_info_get_cpu_time_ex_failed(walkThread,
                                                                    result);
    Trc_THR_omrthread_get_jvm_cpu_usage_timestamp_failed(preTimestamp,
                                                         postTimestamp);
  }
  return ret;
}
