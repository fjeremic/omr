...
omrthread_get_cpu_time_ex(omrthread_t thread, int64_t* cpuTime)
{
...
#ifdef AIXPPC
  {
    ...
    result = pthread_getrusage_np(
      thread->handle, &usageInfo, PTHRDSINFO_RUSAGE_COLLECT);
    if (0 == result) {
      int64_t totalTime =
...
#if defined(J9ZOS390)
  {
    ...
    if (data_offset > ((struct pgthb*)output_buffer)->lenused ||
      ...
      Trc_THR_ThreadGetCpuTime_BpxgthBuffer(
        ...
        ret_val,
        data_offset,
        ((struct pgthb*)output_buffer)->lenused,
        output_size);
      return J9THREAD_ERR;
    }
 ...
 */
intptr_t
omrthread_get_os_priority(omrthread_t thread,
                          intptr_t* policy,
                          intptr_t* priority)
{
  ...
  if (processTime != NULL) {
    ...
    if (GetProcessTimes(GetCurrentProcess(),
      ...
      processTime->_userTime =
        GET_PROCESS_TIMES_IN_NANO * ((int64_t)userTime.dwLowDateTime |
                                     ((int64_t)userTime.dwHighDateTime << 32));
