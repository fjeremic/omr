...
#define J9THREAD_ATTR_IS_VALID(attr)                                           \
  ((attr) && (*(attr)) && ((*(attr))->size == sizeof(omrthread_attr)))
...
intptr_t omrthread_attr_init(omrthread_attr_t *attr) {
  ...
  newAttr = omrthread_allocate_memory(lib, sizeof(omrthread_attr),
                                      OMRMEM_CATEGORY_MM);
  if (!newAttr) {
  ...
  if (failedToSetAttr(omrthread_attr_set_schedpolicy(
          &newAttr, J9THREAD_SCHEDPOLICY_INHERIT))) {
    goto destroy_attr;
  }

  if (failedToSetAttr(
          omrthread_attr_set_priority(&newAttr, J9THREAD_PRIORITY_NORMAL))) {
    goto destroy_attr;
  }

  if (failedToSetAttr(
          omrthread_attr_set_stacksize(&newAttr, STACK_DEFAULT_SIZE))) {
    goto destroy_attr;
  }

  if (failedToSetAttr(
          omrthread_attr_set_detachstate(&newAttr, J9THREAD_CREATE_DETACHED))) {
    goto destroy_attr;
  }

  if (failedToSetAttr(omrthread_attr_set_category(
          &newAttr, J9THREAD_CATEGORY_SYSTEM_THREAD))) {
    goto destroy_attr;
  }
