...
/**
 * Spin on a monitor's spinlockState field until we can atomically swap out a
 * value of SPINLOCK_UNOWNED for the value SPINLOCK_OWNED.
 *
...
intptr_t omrthread_spinlock_acquire(omrthread_t self,
  ...
  for (; spinCount3 > 0; spinCount3--) {
    for (spinCount2 = spinCount2Init; spinCount2 > 0; spinCount2--) {
      ...
      if (oldState == VM_AtomicSupport::lockCompareExchange(target, oldState,
                                                            newState, true)) {
        result = 0;
      ...
      if (OMR_ARE_ALL_BITS_SET(monitor->flags,
                               J9THREAD_MONITOR_DISABLE_SPINNING)) {
        goto update_jlm;
      }
      VM_AtomicSupport::yieldCPU();
      /* begin tight loop */
      for (uintptr_t spinCount1 = spinCount1Init; spinCount1 > 0;
           spinCount1--) {
...
intptr_t omrthread_spinlock_acquire_no_spin(omrthread_t self,
  ...
  uintptr_t newState = J9THREAD_MONITOR_SPINLOCK_OWNED;
  if (oldState ==
      VM_AtomicSupport::lockCompareExchange(target, oldState, newState, true)) {
    result = 0;
    VM_AtomicSupport::readBarrier();
  }
  return result;
...
uintptr_t omrthread_spinlock_swapState(omrthread_monitor_t monitor,
                                       uintptr_t newState) {
  volatile uintptr_t *target = (volatile uintptr_t *)&monitor->spinlockState;
  /* If we are writing in UNOWNED, we are exiting the critical section,
   * therefore have to finish up any writes.
   */
 ...
 * @param[in] retry specifies if a MCS node is reused in which case the MCS lock
 * queue and MCS node characteristics should not be updated. Only spinning
 * should be performed against mcsNode->blocked.
 *
 * @return 0 on success, -1 on failure
 */
intptr_t omrthread_mcs_lock(omrthread_t self, omrthread_monitor_t monitor,
  ...
  if (!retry) {
    ...
    /* Install the mcsNode at the tail of the MCS lock queue
     * (monitor->queueTail). */
  ...
  if ((NULL != predecessor) || retry) {
    /* If a predecessor MCS node exists, then the current thread blocks (waits)
     * until it receives a notification from the thread that owns the
     * predecessor MCS node.
     */
    if (!retry) {
    ...
    /* Three-tier busy-wait loop checks if the mcsNode->blocked value is reset
     * by the thread that owns the predecessor node. A similar three-tier
     * busy-wait loop is also used in omrthread_spinlock_acquire, where the loop
     * has a compare-and-swap operation.
     * TODO: Optimize the MCS lock's three-tier busy-wait loop to account for
     * the absent compare-and-swap operation; this corresponds to an increase in
     * spin parameters.
     */
    for (uintptr_t spinCount3 = monitor->spinCount3; spinCount3 > 0;
         spinCount3--) {
      for (uintptr_t spinCount2 = monitor->spinCount2; spinCount2 > 0;
           spinCount2--) {
        ...
        VM_AtomicSupport::yieldCPU();
        /* Begin tight loop. */
        for (uintptr_t spinCount1 = monitor->spinCount1; spinCount1 > 0;
             spinCount1--) {
          VM_AtomicSupport::nop();
        } /* End tight loop. */
      }
  ...
  lockAcquired:
    /* monitor->spinlockState is maintained for compatibility with the existing
     * omrthread API. */
...
intptr_t omrthread_mcs_trylock(omrthread_t self, omrthread_monitor_t monitor,
  ...
  /* If monitor->queueTail is NULL (no-one is waiting to acquire the lock), then
   * it is swapped with the mcsNode, and the lock is acquired. */
  if (oldState == VM_AtomicSupport::lockCompareExchange(
                      (volatile uintptr_t *)&monitor->queueTail,
                      (uintptr_t)oldState, (uintptr_t)mcsNode)) {
    /* monitor->spinlockState is maintained for compatibility with the existing
     * omrthread API. */
    monitor->spinlockState = J9THREAD_MONITOR_SPINLOCK_OWNED;
...
omrthread_t omrthread_mcs_unlock(omrthread_t self,
  ...
  if (NULL == mcsNode->queueNext) {
    /* If no successor exists, then the mcsNode is at the tail of the MCS lock
     * queue. Release the lock by replacing the mcsNode at the tail of the queue
     * with NULL.
     */
    uintptr_t newState = 0;
    if ((uintptr_t)mcsNode == VM_AtomicSupport::lockCompareExchange(
                                  (volatile uintptr_t *)&monitor->queueTail,
                                  (uintptr_t)mcsNode, newState)) {
      monitor->spinlockState = J9THREAD_MONITOR_SPINLOCK_UNOWNED;
      goto lockReleased;
    }

    /* Another thread is recording a successor in mcsNode->queueNext. Wait for
     * the thread to record the successor.
     */
...
lockReleased:
  ...
  /* Return the MCS node to the thread's MCS node pool since it is no longer
   * used. */
