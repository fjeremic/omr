...
#include "ut_j9thr.h"
...
#if defined(OMR_THR_SPIN_WAKE_CONTROL)
    if (spinning &&
        (OMRTHREAD_IGNORE_SPIN_THREAD_BOUND != lib->maxSpinThreads)) {
   ...
   * @param[in] retry specifies if a MCS node is reused in which case the MCS
   * lock queue and MCS node characteristics should not be updated. Only
   * spinning should be performed against mcsNode->blocked.
   *
   * @return 0 on success, -1 on failure
   */
                              ...
                              BOOLEAN retry)
  {
    ...
    if ((NULL != predecessor) || retry) {
      /* If a predecessor MCS node exists, then the current thread blocks
       * (waits) until it receives a notification from the thread that owns the
       * predecessor MCS node.
       */
      if (!retry) {
        /* Enqueue the mcsNode next to the predecessor node in the MCS lock
         * queue. */
       ...
       * busy-wait loop is also used in omrthread_spinlock_acquire, where the
       * loop has a compare-and-swap operation.
       * TODO: Optimize the MCS lock's three-tier busy-wait loop to account for
       * the absent compare-and-swap operation; this corresponds to an increase
       * in spin parameters.
       */
    ...
    lockAcquired:
      /* monitor->spinlockState is maintained for compatibility with the
       * existing omrthread API. */
                                 ...
                                 omrthread_mcs_node_t mcsNode)
  {
    ...
    /* If monitor->queueTail is NULL (no-one is waiting to acquire the lock),
     * then it is swapped with the mcsNode, and the lock is acquired. */
    if (oldState == VM_AtomicSupport::lockCompareExchange(
                                   ...
                                   omrthread_monitor_t monitor)
  {
    ...
    if (NULL == mcsNode->queueNext) {
       ...
       * queue. Release the lock by replacing the mcsNode at the tail of the
       * queue with NULL.
       */
