...
/* Template category data to be copied into the thread library structure in
 * omrthread_mem_init */
#if defined(OMR_THR_FORK_SUPPORT)
  const uint32_t threadCategoryChildren[] = {
    OMRMEM_CATEGORY_THREADS_RUNTIME_STACK,
    OMRMEM_CATEGORY_THREADS_NATIVE_STACK,
    OMRMEM_CATEGORY_OSMUTEXES,
    OMRMEM_CATEGORY_OSCONDVARS
  };
  const OMRMemCategory threadCategoryTemplate = {
    "Threads", OMRMEM_CATEGORY_THREADS, 0, 0, 4, threadCategoryChildren
  };
  const OMRMemCategory mutexCategoryTemplate = {
    "OS Mutexes", OMRMEM_CATEGORY_OSMUTEXES, 0, 0, 0, NULL
  };
  const OMRMemCategory condvarCategoryTemplate = {
    "OS Condvars", OMRMEM_CATEGORY_OSCONDVARS, 0, 0, 0, NULL
  };
  ...
  const OMRMemCategory threadCategoryTemplate = {
    "Threads", OMRMEM_CATEGORY_THREADS, 0, 0, 2, threadCategoryChildren
  };
  ...
  {
    new (&threadLibrary->threadLibraryCategory)
      OMRMemCategory(threadCategoryTemplate);
    new (&threadLibrary->nativeStackCategory)
      OMRMemCategory(nativeStackCategoryTemplate);
#if defined(OMR_THR_FORK_SUPPORT)
    new (&threadLibrary->mutexCategory) OMRMemCategory(mutexCategoryTemplate);
    new (&threadLibrary->condvarCategory)
      OMRMemCategory(condvarCategoryTemplate);
#endif /* defined(OMR_THR_FORK_SUPPORT) */
  }
   ...
   */
  void* omrthread_allocate_memory(omrthread_library_t threadLibrary,
                                  uintptr_t size,
  ...
  void omrthread_free_memory(omrthread_library_t threadLibrary, void* ptr)
  {
    ...
    header = (J9ThreadMemoryHeader*)mallocPtr;
    decrement_memory_counter(&threadLibrary->threadLibraryCategory,
                             header->blockSize);
   ...
   *
   */
  void* omrthread_mallocWrapper(void* user,
                                uint32_t size,
                                const char* callSite,
                                uint32_t memoryCategory,
                                uint32_t type,
                                uint32_t* doInit)
  {
    return omrthread_allocate_memory(
      (omrthread_library_t)user, size, memoryCategory);
  }
