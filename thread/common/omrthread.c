...
static intptr_t threadCreate(omrthread_t* handle,
                             const omrthread_attr_t* attr,
                             uintptr_t suspend,
...
static omrthread_monitor_t monitor_allocate(omrthread_t self,
                                            intptr_t policy,
                                            intptr_t policyData);
static intptr_t monitor_alloc_and_init(omrthread_monitor_t* handle,
                                       ...
                                       intptr_t policyData,
                                       const char* name);
static intptr_t monitor_init(omrthread_monitor_t monitor,
                             uintptr_t flags,
                             omrthread_library_t lib,
                             const char* name);
static void monitor_free(omrthread_library_t lib, omrthread_monitor_t monitor);
static void monitor_free_nolock(omrthread_library_t lib,
                                omrthread_t thread,
...
static intptr_t monitor_wait(omrthread_monitor_t monitor,
                             int64_t millis,
                             intptr_t nanos,
...
static BOOLEAN fillInEmptyMemCategorySlot(
    const OMRMemCategorySet* const categorySet,
    OMRMemCategory* const category,
    const uint32_t startIndex,
    uint32_t* const filledSlotIndex);
 ...
 */
intptr_t omrthread_create(omrthread_t* handle,
                          uintptr_t stacksize,
                          uintptr_t priority,
                          uintptr_t suspend,
                          omrthread_entrypoint_t entrypoint,
 ...
 */
intptr_t omrthread_create_ex(omrthread_t* handle,
                             const omrthread_attr_t* attr,
                             uintptr_t suspend,
                             omrthread_entrypoint_t entrypoint,
                             void* entryarg) {
 ...
 *
 */
omrthread_t omrthread_self(void) {
  return MACRO_SELF();
}
...
static intptr_t monitor_init(omrthread_monitor_t monitor,
  ...
  if (NULL != name) {
    if (OMR_ARE_ANY_BITS_SET(monitor->flags, J9THREAD_MONITOR_NAME_COPY)) {
       ...
       */
      monitor->name = (char*)omrthread_allocate_memory(lib, length + 1,
 ...
 * @return -1 on error and 0 on success
 */
intptr_t omrthread_set_category(omrthread_t thread,
                                uintptr_t category,
                                uintptr_t type) {
  intptr_t rc = 0;
  omrthread_library_t lib = GLOBAL_DATA(default_library);
