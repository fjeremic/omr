...
intptr_t omrthread_attach_ex(omrthread_t* handle, omrthread_attr_t* attr)
{
  ...
  DuplicateHandle(
    GetCurrentProcess(), GetCurrentThread(), GetCurrentProcess(), &thread->handle, 0, TRUE, DUPLICATE_SAME_ACCESS);
#else
...
static WRAPPER_TYPE thread_wrapper(WRAPPER_ARG arg)
{
...
#if defined(OMR_PORT_NUMA_SUPPORT)
  {
    ...
    if ((affinityCount != 0) || OMR_ARE_NO_BITS_SET(lib->flags, J9THREAD_LIB_FLAG_NO_DEFAULT_AFFINITY)) {
      /* If the affinityCount is zero, this call will have the effect of clearing any affinity that the thread may have
       * inherited and it assuming the initial affinity that the process had (if reverting to that affinity isn't
       * possible on the platform, the call is supposed to do nothing).
       */
...
static void interrupt_blocked_thread(omrthread_t self, omrthread_t threadToInterrupt)
{
  ...
  if (MONITOR_TRY_LOCK(monitor) == 0) {
    ...
    if (threadToInterrupt->monitor == monitor) {
      if ((threadToInterrupt->flags & (J9THREAD_FLAG_BLOCKED | J9THREAD_FLAG_ABORTABLE | J9THREAD_FLAG_ABORTED))
...
static intptr_t interrupt_waiting_thread(omrthread_t self, omrthread_t threadToInterrupt)
{
...
#endif /* ALWAYS_SPAWN_THREAD_TO_INTERRUPT */
  {
     ...
     * cause deadlock
     */
    threadCreate(&threadToInterrupt->interrupter, &lib->systemThreadAttr, 0, interruptServer, (void*)threadToInterrupt,
      GLOBAL_IS_LOCKED);
  }
  return retVal;
}
  ...
  omrthread_t self, omrthread_monitor_t monitor, int64_t millis, intptr_t nanos, uintptr_t interruptible)
{
  ...
  if (monitor_enter_three_tier(
        self, monitor, (BOOLEAN)((interruptible & J9THREAD_FLAG_ABORTABLE) ? SET_ABORTABLE : DONT_SET_ABORTABLE))
    == J9THREAD_INTERRUPTED_MONITOR_ENTER) {
...
static omrthread_monitor_pool_t allocate_monitor_pool(omrthread_library_t lib)
{
  ...
  omrthread_monitor_pool_t pool
    = (omrthread_monitor_pool_t)omrthread_allocate_memory(lib, sizeof(J9ThreadMonitorPool), OMRMEM_CATEGORY_THREADS);
  if (pool == NULL) {
    return NULL;
  }
...
static omrthread_mcs_nodes_t allocate_mcs_nodes(omrthread_library_t lib)
{
  omrthread_mcs_nodes_t mcsNodes
    = (omrthread_mcs_nodes_t)omrthread_allocate_memory(lib, sizeof(OMRThreadMCSNodes), OMRMEM_CATEGORY_THREADS);
  if (NULL == mcsNodes) {
    return NULL;
  }
