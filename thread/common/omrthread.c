        ...
        if (0 == strcmp(J9THREAD_LIB_CONTROL_GET_MEM_CATEGORIES, key)) {
            ...
            if (fillInEmptyMemCategorySlot(categorySet, &lib->threadLibraryCategory, 0, &filledSlotIndex)) {
                if (fillInEmptyMemCategorySlot(
                        categorySet, &lib->nativeStackCategory, filledSlotIndex + 1, &filledSlotIndex)) {
#if defined(OMR_THR_FORK_SUPPORT)
                    if (fillInEmptyMemCategorySlot(
                            categorySet, &lib->mutexCategory, filledSlotIndex + 1, &filledSlotIndex)) {
                        if (fillInEmptyMemCategorySlot(
                                categorySet, &lib->condvarCategory, filledSlotIndex + 1, &filledSlotIndex)) {
                            rc = 0;
                        }
                    }
#else /* defined(OMR_THR_FORK_SUPPORT) */
...
static WRAPPER_TYPE thread_wrapper(WRAPPER_ARG arg)
{
...
#if defined(OMR_PORT_NUMA_SUPPORT)
    {
        ...
        if ((affinityCount != 0) || OMR_ARE_NO_BITS_SET(lib->flags, J9THREAD_LIB_FLAG_NO_DEFAULT_AFFINITY)) {
            /* If the affinityCount is zero, this call will have the effect of clearing any affinity that the thread may
             * have inherited and it assuming the initial affinity that the process had (if reverting to that affinity
             * isn't possible on the platform, the call is supposed to do nothing).
             */
