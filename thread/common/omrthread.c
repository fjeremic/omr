...
static intptr_t
threadCreate(omrthread_t* handle,
...
static intptr_t
monitor_alloc_and_init(omrthread_monitor_t* handle,
                       uintptr_t flags,
                       intptr_t policy,
                       intptr_t policyData,
                       const char* name);
static intptr_t
monitor_init(omrthread_monitor_t monitor,
             uintptr_t flags,
...
static void
monitor_free_nolock(omrthread_library_t lib,
                    omrthread_t thread,
...
static intptr_t
monitor_wait(omrthread_monitor_t monitor,
             int64_t millis,
             intptr_t nanos,
...
#if defined(OMR_THR_THREE_TIER_LOCKING)
static intptr_t
monitor_enter_three_tier(omrthread_t self,
                         omrthread_monitor_t monitor,
                         BOOLEAN isAbortable);
static intptr_t
monitor_wait_three_tier(omrthread_t self,
                        omrthread_monitor_t monitor,
                        int64_t millis,
                        intptr_t nanos,
                        uintptr_t interruptible);
static intptr_t
monitor_notify_three_tier(omrthread_t self,
                          omrthread_monitor_t monitor,
                          int notifyall);
#endif /* OMR_THR_THREE_TIER_LOCKING */
static intptr_t
monitor_wait_original(omrthread_t self,
                      omrthread_monitor_t monitor,
                      int64_t millis,
                      intptr_t nanos,
                      uintptr_t interruptible);
static intptr_t
monitor_notify_original(omrthread_t self,
                        omrthread_monitor_t monitor,
                        int notifyall);
...
static BOOLEAN
fillInEmptyMemCategorySlot(const OMRMemCategorySet* const categorySet,
...
#error "CALLER_LAST_INDEX must be <= MAX_CALLER_INDEX"
  ...
  lib->thread_pool = pool_new(sizeof(J9Thread),
                              0,
                              0,
                              0,
                              OMR_GET_CALLSITE(),
  ...
  lib->global_pool = pool_new(sizeof(J9ThreadGlobal),
                              0,
                              0,
                              0,
                              OMR_GET_CALLSITE(),
...
#if defined(OSX)
  if (KERN_SUCCESS != host_get_clock_service(
                        mach_host_self(), SYSTEM_CLOCK, &lib->clockService)) {
    goto init_cleanup11;
  }
#endif /* defined(OSX) */
...
omrthread_lib_control(const char* key, uintptr_t value)
{
  ...
  if (0 != value) {
    ...
    if (0 == strcmp(J9THREAD_LIB_CONTROL_GET_MEM_CATEGORIES, key)) {
       ...
       */
      if (fillInEmptyMemCategorySlot(
            categorySet, &lib->threadLibraryCategory, 0, &filledSlotIndex)) {
        if (fillInEmptyMemCategorySlot(categorySet,
                                       &lib->nativeStackCategory,
                                       filledSlotIndex + 1,
                                       &filledSlotIndex)) {
#if defined(OMR_THR_FORK_SUPPORT)
          if (fillInEmptyMemCategorySlot(categorySet,
...
omrthread_attach_ex(omrthread_t* handle, omrthread_attr_t* attr)
{
  ...
  if (!attr) {
    omrthread_set_category(
      thread, J9THREAD_CATEGORY_SYSTEM_THREAD, J9THREAD_TYPE_SET_ATTACH);
  } else {
 ...
 */
intptr_t
omrthread_create(omrthread_t* handle,
                 uintptr_t stacksize,
                 uintptr_t priority,
                 uintptr_t suspend,
                 omrthread_entrypoint_t entrypoint,
 ...
 */
intptr_t
omrthread_create_ex(omrthread_t* handle,
                    const omrthread_attr_t* attr,
                    uintptr_t suspend,
                    omrthread_entrypoint_t entrypoint,
                    void* entryarg)
{
  return threadCreate(
    handle, attr, suspend, entrypoint, entryarg, GLOBAL_NOT_LOCKED);
}
...
#if defined(OMR_PORT_NUMA_SUPPORT)
  {
    ...
    if ((affinityCount != 0) ||
      ...
      omrthread_numa_set_node_affinity_nolock(
        thread, affinity, affinityCount, 0);
    }
  }
#endif /* OMR_PORT_NUMA_SUPPORT */
  THREAD_UNLOCK(thread);
             ...
             int globalIsLocked)
{
  ...
  retVal = osthread_create(
    self, &(thread->handle), tempAttr, thread_wrapper, (WRAPPER_ARG)thread);
...
omrthread_nanosleep_to(int64_t wakeTime)
{
  ...
  while (clock_nanosleep(
           NANOSLEEP_CLOCK, NANOSLEEP_ABS_FLAGS, &sleeptime, NULL) == EINTR)
    ;
  return 0;
#else
  return J9THREAD_INVALID_ARGUMENT;
...
interrupt_waiting_thread(omrthread_t self, omrthread_t threadToInterrupt)
{
...
#endif /* ALWAYS_SPAWN_THREAD_TO_INTERRUPT */
  {
     ...
     * cause deadlock
     */
    threadCreate(&threadToInterrupt->interrupter,
                 &lib->systemThreadAttr,
                 0,
                 interruptServer,
                 (void*)threadToInterrupt,
                 GLOBAL_IS_LOCKED);
  }
  return retVal;
}
...
omrthread_park(int64_t millis, intptr_t nanos)
{
  ...
  if (self->flags & J9THREAD_FLAG_UNPARKED) {
    ...
    if (millis || nanos) {
      ...
      OMROSCOND_WAIT_IF_TIMEDOUT(
        self->condition, self->mutex, boundedMillis, nanos)
      {
 ...
 */
intptr_t
omrthread_monitor_init_with_name(omrthread_monitor_t* handle,
                                 uintptr_t flags,
                                 const char* name)
{
  intptr_t rc = 0;

  rc = monitor_alloc_and_init(
    handle, flags, J9THREAD_LOCKING_DEFAULT, J9THREAD_LOCKING_NO_DATA, name);
  return rc;
}
             ...
             const char* name)
{
  ...
  if (!OMR_ARE_ALL_BITS_SET(flags, J9THREAD_MONITOR_OBJECT) ||
      OMR_ARE_ALL_BITS_SET(
        lib->flags, J9THREAD_LIB_FLAG_SECONDARY_SPIN_OBJECT_MONITORS_ENABLED)) {
 ...
 */
intptr_t
omrthread_monitor_enter_using_threadId(omrthread_monitor_t monitor,
 ...
 */
intptr_t
omrthread_monitor_try_enter_using_threadId(omrthread_monitor_t monitor,
                                           omrthread_t threadId)
{
  ...
  if (0 == lockAcquired) {
    ...
    UPDATE_JLM_MON_ENTER(
      threadId, monitor, !IS_RECURSIVE_ENTER, !IS_SLOW_ENTER);
 ...
 */
intptr_t
omrthread_monitor_exit_using_threadId(omrthread_monitor_t monitor,
                                      omrthread_t threadId)
{
  ASSERT(threadId == MACRO_SELF());
  ASSERT(monitor);
 ...
 */
intptr_t
omrthread_monitor_wait_abortable(omrthread_monitor_t monitor,
                                 int64_t millis,
                                 intptr_t nanos)
{
 ...
 */
intptr_t
omrthread_monitor_wait_interruptable(omrthread_monitor_t monitor,
                                     int64_t millis,
                                     intptr_t nanos)
{
 ...
 */
intptr_t
omrthread_monitor_wait_timed(omrthread_monitor_t monitor,
                             int64_t millis,
                             intptr_t nanos)
{
                        ...
                        uintptr_t interruptible)
{
  ...
  if (millis || nanos) {
    ...
    OMROSCOND_WAIT_IF_TIMEDOUT(
      self->condition, monitor->mutex, boundedMillis, nanos)
    {
      THREAD_LOCK(self, CALLER_MONITOR_WAIT2);
      intrFlags = self->flags & intrMask;
      interrupted = J9THR_WAIT_INTERRUPTED(intrFlags);
...
allocate_mcs_nodes(omrthread_library_t lib)
{
  ...
  mcsNodes->pool = pool_new(sizeof(OMRThreadMCSNode),
                            OMRTHREAD_MIN_MCS_NODES,
                            OMRTHREAD_MCS_NODE_ALIGNMENT,
                            0,
                            OMR_GET_CALLSITE(),
                            OMRMEM_CATEGORY_THREADS,
                            omrthread_mallocWrapper,
 ...
 */
void
omrthread_add_node_number_to_affinity_cache(omrthread_t thread,
...
fixupThreadAccounting(omrthread_t thread, uintptr_t type)
{
  ...
  if (OMR_ARE_ALL_BITS_SET(thread->effective_category,
    ...
    if ((thread->effective_category &
      ...
      int userCat =
        (thread->effective_category - J9THREAD_USER_DEFINED_THREAD_CATEGORY_1) &
