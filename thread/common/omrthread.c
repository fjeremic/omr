...
static intptr_t threadCreate(omrthread_t *handle, const omrthread_attr_t *attr,
                             uintptr_t suspend,
...
static omrthread_monitor_t monitor_allocate(omrthread_t self, intptr_t policy,
                                            intptr_t policyData);
static intptr_t monitor_alloc_and_init(omrthread_monitor_t *handle,
                                       uintptr_t flags, intptr_t policy,
                                       intptr_t policyData, const char *name);
static intptr_t monitor_init(omrthread_monitor_t monitor, uintptr_t flags,
                             omrthread_library_t lib, const char *name);
static void monitor_free(omrthread_library_t lib, omrthread_monitor_t monitor);
static void monitor_free_nolock(omrthread_library_t lib, omrthread_t thread,
...
static intptr_t monitor_wait(omrthread_monitor_t monitor, int64_t millis,
                             intptr_t nanos, uintptr_t interruptible);
static intptr_t monitor_notify_one_or_all(omrthread_monitor_t monitor,
                                          int notifyall);
#if defined(OMR_THR_THREE_TIER_LOCKING)
static intptr_t monitor_enter_three_tier(omrthread_t self,
                                         omrthread_monitor_t monitor,
                                         BOOLEAN isAbortable);
static intptr_t monitor_wait_three_tier(omrthread_t self,
                                        omrthread_monitor_t monitor,
                                        int64_t millis, intptr_t nanos,
                                        uintptr_t interruptible);
static intptr_t monitor_notify_three_tier(omrthread_t self,
                                          omrthread_monitor_t monitor,
                                          int notifyall);
#endif /* OMR_THR_THREE_TIER_LOCKING */
static intptr_t monitor_wait_original(omrthread_t self,
                                      omrthread_monitor_t monitor,
                                      int64_t millis, intptr_t nanos,
                                      uintptr_t interruptible);
static intptr_t monitor_notify_original(omrthread_t self,
                                        omrthread_monitor_t monitor,
                                        int notifyall);

#if defined(OMR_THR_THREE_TIER_LOCKING)
static intptr_t init_spinCounts(omrthread_library_t lib);
static void unblock_spinlock_threads(omrthread_t self,
                                     omrthread_monitor_t monitor);
#endif /* OMR_THR_THREE_TIER_LOCKING */
...
static void threadInterruptWake(omrthread_t thread,
                                omrthread_monitor_t monitor);
static void threadNotify(omrthread_t threadToNotify);
static int32_t J9THREAD_PROC interruptServer(void *entryArg);
static intptr_t interrupt_waiting_thread(omrthread_t self,
                                         omrthread_t threadToInterrupt);

#if defined(OMR_THR_THREE_TIER_LOCKING)
static void interrupt_blocked_thread(omrthread_t self,
                                     omrthread_t threadToInterrupt);
#endif /* OMR_THR_THREE_TIER_LOCKING */
...
static uintptr_t monitor_on_notify_all_wait_list(omrthread_t self,
                                                 omrthread_monitor_t monitor);
static void monitor_notify_all_migration(omrthread_monitor_t monitor);

static intptr_t failedToSetAttr(intptr_t rc);
#if defined(OSX)
static intptr_t copyAttr(omrthread_attr_t *attrTo,
                         const omrthread_attr_t *attrFrom);
#endif /* defined(OSX) */

static intptr_t fixupThreadAccounting(omrthread_t thread, uintptr_t type);
static void storeExitCpuUsage(omrthread_t thread);

static BOOLEAN fillInEmptyMemCategorySlot(
    const OMRMemCategorySet *const categorySet, OMRMemCategory *const category,
...
static void free_mcs_nodes(omrthread_library_t lib,
...
#define BOUNDED_I64_TO_IDATA(longValue) \
  ((longValue) > 0x7FFFFFFFLL ? (intptr_t)0x7FFFFFFF : (intptr_t)(longValue))
#else /* AIXPPC */
#define BOUNDED_I64_TO_IDATA(longValue) ((intptr_t)longValue)
#endif /* AIXPPC */
#else  /* OMR_ENV_DATA64 */
#define BOUNDED_I64_TO_IDATA(longValue) \
  ((longValue) > 0x7FFFFFFF ? 0x7FFFFFFF : (intptr_t)(longValue))
 ...
 */
#define J9THREAD_FLAGM_BLOCKED_ABORTABLE \
  (J9THREAD_FLAG_BLOCKED | J9THREAD_FLAG_ABORTABLE)
#define J9THREAD_FLAGM_SLEEPING_TIMED \
  (J9THREAD_FLAG_SLEEPING | J9THREAD_FLAG_TIMER_SET)
#define J9THREAD_FLAGM_SLEEPING_TIMED_INTERRUPTIBLE \
  (J9THREAD_FLAGM_SLEEPING_TIMED | J9THREAD_FLAG_INTERRUPTABLE)
#define J9THREAD_FLAGM_PARKED_INTERRUPTIBLE \
  (J9THREAD_FLAG_PARKED | J9THREAD_FLAG_INTERRUPTABLE)

#define J9THR_WAIT_INTERRUPTED(flags) (((flags)&J9THREAD_FLAG_INTERRUPTED) != 0)
#define J9THR_WAIT_PRI_INTERRUPTED(flags)                                      \
  (((flags) & (J9THREAD_FLAG_PRIORITY_INTERRUPTED | J9THREAD_FLAG_ABORTED)) != \
   0)
...
#define NOTIFY_WRAPPER(thread)                                             \
  do {                                                                     \
    if (OMR_ARE_ALL_BITS_SET((thread)->library->flags,                     \
                             J9THREAD_LIB_FLAG_NOTIFY_POLICY_BROADCAST)) { \
 ...
 * restored and the behaviour of any remaining key destructors that attempt to
 * use the thread library is undefined.
 *
 * @param[in] current_omr_thread the previous value of the TLS slot - the
 * current omrthread_t
 */

#define OMR_MAX_KEY_DELETION_ATTEMPTS 10

static void self_key_destructor(void *current_omr_thread) {
  ...
  if (attempts < OMR_MAX_KEY_DELETION_ATTEMPTS) {
    ...
    TLS_SET(((omrthread_library_t)GLOBAL_DATA(default_library))->self_ptr,
            current_omr_thread);
  }
}
 ...
 *
 * If any OS threads were created before calling this function, they must be
 * attached using omrthread_attach before accessing any J9 thread library
 * functions.
 *
...
#error "CALLER_LAST_INDEX must be <= MAX_CALLER_INDEX"
  ...
  lib->thread_pool = pool_new(sizeof(J9Thread), 0, 0, 0, OMR_GET_CALLSITE(),
                              OMRMEM_CATEGORY_THREADS, omrthread_mallocWrapper,
                              omrthread_freeWrapper, lib);
  if (lib->thread_pool == NULL) {
    goto init_cleanup8;
  }

  lib->global_pool =
      pool_new(sizeof(J9ThreadGlobal), 0, 0, 0, OMR_GET_CALLSITE(),
               OMRMEM_CATEGORY_THREADS, omrthread_mallocWrapper,
               omrthread_freeWrapper, lib);
...
/* init_cleanup12:		mach_port_deallocate(mach_task_self(),
 * lib->clockService); */
init_cleanup11:
...
/**
 * Look for an empty NULL slot in a J9MemoryCategorySet, and add a new category
 * into it.
 *
 ...
 * @param[out] filledSlotIndex If found, the empty slot where the new category
 * is added. Must be non-NULL.
...
intptr_t omrthread_lib_control(const char *key, uintptr_t value) {
  intptr_t rc = -1;
  ...
  if (0 != value) {
    ...
    if (0 == strcmp(J9THREAD_LIB_CONTROL_GET_MEM_CATEGORIES, key)) {
       ...
       */
      if (fillInEmptyMemCategorySlot(categorySet, &lib->threadLibraryCategory,
                                     0, &filledSlotIndex)) {
...
#if defined(LINUX) || defined(OSX)
  omrthread_library_t lib = GLOBAL_DATA(default_library);
  return OMR_ARE_ALL_BITS_SET(lib->flags,
                              J9THREAD_LIB_FLAG_REALTIME_SCHEDULING_ENABLED);
...
static intptr_t init_spinCounts(omrthread_library_t lib) {
  ...
  if (init_threadParam("defaultMonitorSpinCount1",
                       &lib->defaultMonitorSpinCount1)) {
    return -1;
  }
  if (init_threadParam("defaultMonitorSpinCount2",
                       &lib->defaultMonitorSpinCount2)) {
    return -1;
  }
  if (init_threadParam("defaultMonitorSpinCount3",
                       &lib->defaultMonitorSpinCount3)) {
    return -1;
  }
...
static intptr_t init_spinParameters(omrthread_library_t lib) {
  ...
  if (init_threadParam("adaptSpinSampleThreshold",
                       &lib->adaptSpinSampleThreshold)) {
    return -1;
  }

  lib->adaptSpinSampleStopCount = 0;
  if (init_threadParam("adaptSpinSampleStopCount",
                       &lib->adaptSpinSampleStopCount)) {
    return -1;
  }

  lib->adaptSpinSampleCountStopRatio = 0;
  if (init_threadParam("adaptSpinSampleCountStopRatio",
                       &lib->adaptSpinSampleCountStopRatio)) {
    return -1;
  }
#endif
...
static void omrthread_shutdown(void) {
  omrthread_library_t lib = GLOBAL_DATA(default_library);
  ASSERT(lib);
  /* Acquire the global lock here to wait until all outstanding omrthread_exit
   * calls have completed. omrthread_exit holds the global lock until the thread
   * is ready to exit to the OS.
   */
  GLOBAL_LOCK_SIMPLE(lib);
  GLOBAL_UNLOCK_SIMPLE(lib);
#if defined(OMR_PORT_NUMA_SUPPORT)
 ...
 *
 * @note This must not be called recursively by a thread that already owns the
 * global lock.
 ...
 */
intptr_t omrthread_lib_try_lock(omrthread_t self) {
#ifdef THREAD_ASSERTS
  intptr_t retVal;
 ...
 *
 * @note This must be called only by the thread that currently has the global
 * lock locked.
 * @param[in] self omrthread_t for the current thread
 * @return none
 *
...
/**
 * To reset the omrthread's, destroy their mutexes and conds. For the fork
 * survivor thread, fix the tid and handle and recreate the mutex and cond.
 * Otherwise, free J9Thread.tracing, flush the destroyed monitor lists back into
 * the pool, and tls_finalize() the tls.
 *
 * Note that the thread mutex and cond are reallocated and initialized without
 * freeing the old ones, since if it allocates the same piece of memory, it
 * fails with mutex already initialized.
 *
 * @pre There is a pre-condition that the monitors have been reset first with
 * postForkResetMonitors in order for omrthread_tls_finalize to use mutexes. A
 * user-provided tls_finalize callback may fail post-fork if it uses
 * mutexes/cond vars the thread library does not know about. In case it uses
 * omrthread_monitors, they must be reset first.
 */
static void postForkResetThreads(omrthread_t self) {
  ...
  threadIterator =
      (omrthread_t)pool_startDo(lib->thread_pool, &threadPoolState);
...
#error "The implementation of postForkResetThreads() is not complete in WIN32."
      ...
      /* Flush the thread local list of destroyed monitors of the thread that no
       * longer exists to the global list. */
...
/**
 * Reset the omrthread_library's mutexes, the global lock owner, and the dead
 * threads info.
 */
static void postForkResetLib(omrthread_t self) {
  omrthread_library_t lib = self->library;
...
/**
 * For each monitor in the pool, destroy and reinit the mutex if it is
 * allocated, clear the waiting, blocking, and notifyAll thread list (with an
 * assert if its the fork survivor), set the owner to null if it is not the fork
 * survivor, and reset the spinLockState. If its not owned by the fork survivor,
 * reset the owner, pinCount, and sampleCounter as well.
 *
 * Note that the mutex is reallocated and initialized without freeing the old
 * ones, since if it allocates the same piece of memory, it fails with mutex
 * already initialized.
 */
static void postForkResetMonitors(omrthread_t self) {
  omrthread_library_t lib = self->library;
...
static void postForkResetRWMutexes(omrthread_t self) {
  ...
  rwmutexIterator =
      (omrthread_rwmutex_t)pool_startDo(lib->rwmutexPool, &rwmutexPoolState);
 ...
 *
 * Returns the flags for the threading library associated with the current
 * thread.
 *
 ...
 */
uintptr_t omrthread_lib_get_flags(void) {
  omrthread_t self = MACRO_SELF();
 ...
 *
 * @param[in] flags flags to be set (bit vector: 1 means set the flag, 0 means
 * ignore)
 * @return old flags values
 * @see omrthread_lib_clear_flags, omrthread_lib_get_flags
 *
 */
uintptr_t omrthread_lib_set_flags(uintptr_t flags) {
  omrthread_t self;
  uintptr_t oldFlags;
  self = MACRO_SELF();
 ...
 * @param[in] flags flags to be cleared (bit vector: 1 means clear the flag, 0
 * means ignore)
 * @return old flags values
 */
uintptr_t omrthread_lib_clear_flags(uintptr_t flags) {
  omrthread_t self;
  uintptr_t oldFlags;
  self = MACRO_SELF();
 ...
 *
 * Return a pointer to the data associated with a named global with the
 * specified name.<br> A new named global is created if a named global with the
 * specified name can't be found.
 *
 ...
 *
 */
uintptr_t *omrthread_global(char *name) {
 ...
 */
omrthread_monitor_t omrthread_global_monitor(void) {
  omrthread_library_t lib = GLOBAL_DATA(default_library);
 ...
 *
 * @param[out] handle pointer to a omrthread_t to be set (will be ignored if
 * null)
 * @return  0 on success or negative value on failure
 *
 * @see omrthread_detach
 */
intptr_t omrthread_attach(omrthread_t *handle) {
  omrthread_t thread;
 ...
 */
intptr_t omrthread_attach_ex(omrthread_t *handle, omrthread_attr_t *attr) {
  ...
  DuplicateHandle(GetCurrentProcess(), GetCurrentThread(), GetCurrentProcess(),
                  &thread->handle, 0, TRUE, DUPLICATE_SAME_ACCESS);
#else
  ...
  /* Set the thread category to what has been passed in. If no attr has been
   * passed in, then set to J9THREAD_CATEGORY_SYSTEM_THREAD. Since a number of
   * OMR APIs call omrthread_attach, having J9THREAD_CATEGORY_SYSTEM_THREAD as
   * the default, means that these APIs can call with a NULL attribute and it
   * will automatically get associated to the right category. Ignore errors, the
   * cpu monitor should not cause thread attach failure.
   */
  if (!attr) {
    omrthread_set_category(thread, J9THREAD_CATEGORY_SYSTEM_THREAD,
                           J9THREAD_TYPE_SET_ATTACH);
  } else {
...
static void storeExitCpuUsage(omrthread_t thread) {
  ...
  if (OMR_ARE_ALL_BITS_SET(lib->flags, J9THREAD_LIB_FLAG_ENABLE_CPU_MONITOR)) {
    ...
    if (threadCpuTime > 0) {
      ...
      if (OMR_ARE_ALL_BITS_SET(thread->effective_category,
                               J9THREAD_CATEGORY_RESOURCE_MONITOR_THREAD)) {
        cumulativeInfo->resourceMonitorCpuTime += threadCpuTime;
      } else if (OMR_ARE_ALL_BITS_SET(thread->effective_category,
 ...
 *
 * If the thread is an attached thread, then detach should only be called by the
 * thread itself. Internal resources associated with the thread are freed.
 *
 ...
 *
 * @param[out] handle a pointer to a omrthread_t which will point to the thread
 *(if successfully created)
 * @param[in] stacksize the size of the new thread's stack (bytes)<br>
 *			0 indicates use default size
 * @param[in] priority priorities range from J9THREAD_PRIORITY_MIN to
 *J9THREAD_PRIORITY_MAX (inclusive)
...
intptr_t omrthread_create(omrthread_t *handle, uintptr_t stacksize,
  ...
  if (failedToSetAttr(
          omrthread_attr_set_schedpolicy(&attr, J9THREAD_SCHEDPOLICY_OTHER))) {
    retVal = J9THREAD_ERR_INVALID_CREATE_ATTR;
    goto destroy_attr;
  }

  /* HACK: the priority must be set after the policy because RTJ might override
   * the schedpolicy */
 ...
 *
 * @param[out] handle Location where the new omrthread_t should be returned, if
 * successfully created. May be NULL.
 * @param[in] attr Creation attributes. If attr is NULL, we will supply a
 * default attr.
 * @param[in] suspend Set to non-zero to create the thread in a suspended state.
 * @param[in] entrypoint Pointer to the function which the thread will run.
 * @param[in] entryarg A value to pass to the entrypoint function.
 ...
 * @retval J9THREAD_ERR_OS_ERRNO_SET Bit flag optionally or'd into the return
 * code. Indicates that an os_errno is available.
 *
 * @see omrthread_create, omrthread_exit, omrthread_resume
 */
intptr_t omrthread_create_ex(omrthread_t *handle, const omrthread_attr_t *attr,
                             uintptr_t suspend,
                             omrthread_entrypoint_t entrypoint,
                             void *entryarg) {
  return threadCreate(handle, attr, suspend, entrypoint, entryarg,
                      GLOBAL_NOT_LOCKED);
}
...
static WRAPPER_TYPE thread_wrapper(WRAPPER_ARG arg) {
...
#if defined(OMR_PORT_NUMA_SUPPORT)
  {
    ...
    /* if J9THREAD_LIB_FLAG_NO_DEFAULT_AFFINITY is set we only want to modify
     * the affinity if the caller provided us with a non-zero affinity
     */
    if ((affinityCount != 0) ||
        OMR_ARE_NO_BITS_SET(lib->flags,
                            J9THREAD_LIB_FLAG_NO_DEFAULT_AFFINITY)) {
      /* If the affinityCount is zero, this call will have the effect of
       * clearing any affinity that the thread may have inherited and it
       * assuming the initial affinity that the process had (if reverting to
       * that affinity isn't possible on the platform, the call is supposed to
       * do nothing).
       */
      omrthread_numa_set_node_affinity_nolock(thread, affinity, affinityCount,
                                              0);
    }
  }
#endif /* OMR_PORT_NUMA_SUPPORT */
  ...
  /* Workaround for NPTL bug on Linux. See omrthread_exit() */
  {
    ...
    if (0 == setjmp(jumpBuffer)) {
      ...
      /* omrthread_exit was called - global mutex was locked before longjmp to
       * here */
...
void omrthread_cancel(omrthread_t thread) {
  ...
  if (0 == (thread->flags & J9THREAD_FLAG_STARTED)) {
    ...
    /* This is unsafe because thread may have exited and deleted itself. It
 ...
 *
 * If monitor is not NULL, the monitor will be exited before the thread
 * terminates. This is useful if the thread wishes to signal its termination to
 * a watcher, since it exits the monitor and terminates the thread without ever
 * returning control to the thread's routine, which might be running in a DLL
 * which is about to be closed.
 *
 * @param[in] monitor monitor to be exited before exiting (ignored if NULL)
 * @return none
 */
void OMRNORETURN omrthread_exit(omrthread_monitor_t monitor) {
  omrthread_t self = MACRO_SELF();

  /* Hold the global lock before signalling the passed-in monitor until the
   * point where this thread exits to the OS. This (along with code in
   * omrthread_shutdowm) prevents the thread library from being shut down while
   * any thread is running omrthread_exit.
   */
 ...
 * @param[in] suspend Non-zero if the thread should suspend before entering
 * entrypoint, zero to allow the thread to run freely.
 * @param[in] entrypoint
 * @param[in] entryarg
 * @param[in] globalIsLocked Indicates whether the threading library global
...
static intptr_t threadCreate(omrthread_t *handle, const omrthread_attr_t *attr,
  ...
  if (NULL != attr) {
#if defined(OSX)
    /* OSX threads ignore policy inheritance. Create a new attr and do
     * inheritance manually as needed. */
...
#if defined(OSX)
    if (J9THREAD_SUCCESS !=
        omrthread_attr_set_priority(&tempAttr, self->priority)) {
  ...
  /* Ignore errors, the thead cpu monitor should not cause thread creation
   * failure */
  ...
  retVal = osthread_create(self, &(thread->handle), tempAttr, thread_wrapper,
                           (WRAPPER_ARG)thread);
...
static intptr_t copyAttr(omrthread_attr_t *attrTo,
  ...
  /* For each attribute used by omr, set it in attrTo if the initialized default
   * differs from attrFrom. */
  if ((*attrTo)->category != (*attrFrom)->category) {
    if (failedToSetAttr(
            omrthread_attr_set_category(attrTo, (*attrFrom)->category))) {
      rc = J9THREAD_ERR_INVALID_ATTR;
      goto destroyAttr;
    }
  }
  if ((*attrTo)->stacksize != (*attrFrom)->stacksize) {
    if (failedToSetAttr(
            omrthread_attr_set_stacksize(attrTo, (*attrFrom)->stacksize))) {
      rc = J9THREAD_ERR_INVALID_ATTR;
      goto destroyAttr;
    }
  }
  if ((*attrTo)->schedpolicy != (*attrFrom)->schedpolicy) {
    if (failedToSetAttr(
            omrthread_attr_set_schedpolicy(attrTo, (*attrFrom)->schedpolicy))) {
      rc = J9THREAD_ERR_INVALID_ATTR;
      goto destroyAttr;
    }
  }
  if ((*attrTo)->priority != (*attrFrom)->priority) {
    if (failedToSetAttr(
            omrthread_attr_set_priority(attrTo, (*attrFrom)->priority))) {
      rc = J9THREAD_ERR_INVALID_ATTR;
      goto destroyAttr;
    }
  }
  if ((*attrTo)->detachstate != (*attrFrom)->detachstate) {
    if (failedToSetAttr(
            omrthread_attr_set_detachstate(attrTo, (*attrFrom)->detachstate))) {
      rc = J9THREAD_ERR_INVALID_ATTR;
      goto destroyAttr;
    }
  }
 ...
 *
 * This function should initialize anything that is needed whenever a omrthread
 ...
 * @param[in] globalAlreadyLocked indicates whether the thread library global
 * mutex is already locked
 * @return 0 on success or negative value on failure.
 *
 * @see threadCreate
...
static intptr_t threadDestroy(omrthread_t thread, int globalAlreadyLocked) {
...
#if defined(OMR_OS_WINDOWS)
  /* Acquire the global monitor mutex (if needed) while performing handle
   * closing and freeing. */
   ...
   * For created threads, need to close the handle returned from
   * _beginthreadex().
   */
  ...
  /* On z/OS, we cannot call GLOBAL_LOCK_SIMPLE(lib) before calling
   * threadFree(), since this leads to seg faults in the JVM. See PR 82332: [zOS
   * S390] 80 VM_Extended.TestJvmCpuMonitorMXBeanLocal : crash
   */
 ...
 *
 * @param[in] globalAlreadyLocked indicates whether the thread library global
 * mutex is already locked Also implies that omrthread_tls_finalize was called
 * before the global mutex was locked
 * @return none
 */
static void OMRNORETURN threadInternalExit(int globalAlreadyLocked) {
  omrthread_t self = MACRO_SELF();
 ...
 *
 */
omrthread_t omrthread_self(void) { return MACRO_SELF(); }
 ...
 * @see omrthread_sleep
 */
intptr_t omrthread_sleep_interruptable(int64_t millis, intptr_t nanos) {
  omrthread_t self = MACRO_SELF();
  ...
  OMROSCOND_WAIT_IF_TIMEDOUT(self->condition, self->mutex, boundedMillis,
                             nanos) {
    break;
  }
  else {
    if (self->flags & J9THREAD_FLAG_INTERRUPTED) {
      self->flags &= ~(J9THREAD_FLAG_INTERRUPTED |
                       J9THREAD_FLAGM_SLEEPING_TIMED_INTERRUPTIBLE);
      THREAD_UNLOCK(self);
      return J9THREAD_INTERRUPTED;
    }
    if (self->flags & J9THREAD_FLAG_PRIORITY_INTERRUPTED) {
 ...
 */
intptr_t omrthread_sleep(int64_t millis) {
  omrthread_t self = MACRO_SELF();
#ifdef OMR_ENV_DATA64
  intptr_t boundedMillis = millis;
#else
  intptr_t boundedMillis =
      (millis > 0x7FFFFFFF ? 0x7FFFFFFF : (intptr_t)millis);
#endif
 ...
 * @return  0 on success<br> J9THREAD_INVALID_ARGUMENT if nanos < 0 or nanosleep
 * not supported
 *
 * @see omrthread_sleep
 */
intptr_t omrthread_nanosleep(int64_t nanos) {
  ...
  while (clock_nanosleep(NANOSLEEP_CLOCK, NANOSLEEP_FLAGS, &sleeptime, NULL) ==
         EINTR)
    ;
  return 0;
#else
 ...
 *
 */
intptr_t omrthread_nanosleep_supported(void) {
#if USE_CLOCK_NANOSLEEP
  return 1;
#else
  return 0;
 ...
 * Should only be called if omrthread_nanosleep_supported returns true
 *
 * @param[in] wakeTime absolute wake up time in nanoseconds since the epoch (Jan
 * 1, 1970)
 * @return  0 on success<br>
 ...
 * @see omrthread_sleep
 */
intptr_t omrthread_nanosleep_to(int64_t wakeTime) {
#if USE_CLOCK_NANOSLEEP
  omrthread_t self = MACRO_SELF();
  struct timespec sleeptime;
  ASSERT(self);
  ...
  while (clock_nanosleep(NANOSLEEP_CLOCK, NANOSLEEP_ABS_FLAGS, &sleeptime,
                         NULL) == EINTR)
    ;
  return 0;
#else
 ...
 */
intptr_t omrthread_resume(omrthread_t thread) {
  ASSERT(thread);
 ...
 *
 */
intptr_t omrthread_set_name(omrthread_t thread, const char *name) {
  omrthread_t self;
 ...
 *
 */
intptr_t omrthread_set_priority(omrthread_t thread, uintptr_t priority) {
  ASSERT(thread);
...
/**
 * spreads the native priorities so that they are not all mapped to the same
 * value. Returns 0 on success.  If the spread is not possible returns non-zero.
 */
intptr_t omrthread_set_priority_spread(void) {
  intptr_t rc = -1;
 ...
 */
void omrthread_abort(omrthread_t thread) {
 ...
 * Interrupt a thread.
 *
 * If the thread is currently blocked (i.e. waiting on a monitor_wait or
 * sleeping) resume the thread and cause it to return from the blocking function
 * with J9THREAD_INTERRUPTED.
 *
 ...
 */
uintptr_t omrthread_interrupted(omrthread_t thread) {
  ASSERT(thread);
  return (thread->flags & J9THREAD_FLAG_INTERRUPTED) != 0;
}

/**
 * Clear the interrupted flag of the current thread and return its previous
 * value.
 *
 * @return  previous value of interrupted flag: non-zero if the thread had been
 * interrupted.
 */
uintptr_t omrthread_clear_interrupted(void) {
  uintptr_t oldFlags;
  omrthread_t self = MACRO_SELF();
  ASSERT(self);
 ...
 *
 * If the thread is currently blocked (i.e. waiting on a monitor_wait or
 * sleeping) resume the thread and return from the blocking function with
 ...
 *
 * @param[in] thread thread to be queried
 * @return 0 if not priority interrupted, non-zero if priority interrupted flag
 * set
 */
uintptr_t omrthread_priority_interrupted(omrthread_t thread) {
  return (thread->flags & J9THREAD_FLAG_PRIORITY_INTERRUPTED) != 0;
}

/**
 * Clear the priority interrupted flag of the current thread and return its
 * previous value.
 *
 * @return  previous value of priority interrupted flag: nonzero if the thread
 * had been priority interrupted.
 */
uintptr_t omrthread_clear_priority_interrupted(void) {
  uintptr_t oldFlags;
  omrthread_t self = MACRO_SELF();
  ASSERT(self);
 ...
 * @param[in] interruptFlag indicates type of interrupt
 * (normal/priority-interrupt/abort)
 * @return none
 */
static void threadInterrupt(omrthread_t thread, uintptr_t interruptFlag) {
  uintptr_t currFlags;
...
static intptr_t interrupt_waiting_thread(omrthread_t self,
  ...
  ASSERT(threadToInterrupt);
  ASSERT(self != threadToInterrupt);
  ASSERT(threadToInterrupt->flags &
  ...
  /*
   * Holding THREAD_LOCK here prevents threadToInterrupt from exiting
   * monitor_wait. This keeps the monitor pinned while we're using it.
   *
  ...
  if (omrthread_monitor_try_enter_using_threadId(monitor, self) == 0) {
    ...
    /*
     * COND_NOTIFY only if the thread is still waiting and waiting on the same
     * monitor.
     *
     * If the thread is not waiting, we don't need to do anything. The thread
     * has already awakened and observed that it is interrupted by checking its
     * flags.
     *
...
#endif /* ALWAYS_SPAWN_THREAD_TO_INTERRUPT */
  {
     ...
     * cause deadlock
     */
    threadCreate(&threadToInterrupt->interrupter, &lib->systemThreadAttr, 0,
                 interruptServer, (void *)threadToInterrupt, GLOBAL_IS_LOCKED);
  }
  return retVal;
}
...
static int32_t J9THREAD_PROC interruptServer(void *entryArg) {
  ...
  /* This assertion is bogus. If the thread was notified after being
   * interrupted, it could still have an interrupter but would not be WAITING
  ...
  THREAD_LOCK(threadToInterrupt, CALLER_INTERRUPT_SERVER);
  if ((threadToInterrupt->interrupter == self) &&
      (threadToInterrupt->flags & J9THREAD_FLAG_WAITING)) {
    threadInterruptWake(threadToInterrupt, monitor);
  }
  threadToInterrupt->interrupter = NULL;
  ASSERT(threadToInterrupt->flags &
         (J9THREAD_FLAG_INTERRUPTED | J9THREAD_FLAG_ABORTED));
  THREAD_UNLOCK(threadToInterrupt);
 ...
 * @param[in] setNotifiedFlag indicates whether to set the notified thread's
 * notified flag.
 * @return none
 */
static void threadNotify(omrthread_t threadToNotify) {
 ...
 *
 * Stop the current thread from executing until it is unparked, interrupted, or
 ...
 * J9THREAD_PRIORITY_INTERRUPTED if the thread was priority interrupted or
 * aborted while parked<br> J9THREAD_TIMED_OUT if the timeout expired<br>
 *
 * @see omrthread_unpark
 */
intptr_t omrthread_park(int64_t millis, intptr_t nanos) {
  intptr_t rc = 0;
  omrthread_t self = MACRO_SELF();
  ASSERT(self);
  ...
  if (self->flags & J9THREAD_FLAG_UNPARKED) {
    ...
    rc = J9THREAD_INTERRUPTED;
             ...
             (J9THREAD_FLAG_PRIORITY_INTERRUPTED | J9THREAD_FLAG_ABORTED)) {
  ...
  self->flags &=
      ~(J9THREAD_FLAGM_PARKED_INTERRUPTIBLE | J9THREAD_FLAG_TIMER_SET);
 ...
 */
uintptr_t omrthread_current_stack_free(void) {
#if defined(OMR_OS_WINDOWS)
  ...
  stackFree = ((uintptr_t)&memInfo - (uintptr_t)memInfo.AllocationBase) &
              ~sizeof(uintptr_t);

  /* By observation, Win32 reserves 3 pages at the low end of the stack for
   * guard pages, so omit them */
 ...
 *
 * @param[out] handle pointer to a omrthread_monitor_t to be set to point to the
 * new monitor
 * @param[in] flags initial flag values for the monitor
 * @param[in] name pointer to a C string with a description of how the monitor
 * will be used (may be NULL)<br> If non-NULL, the C string must be valid for
 * the entire life of the monitor
 *
 * @return  0 on success or negative value on failure
 *
 ...
 *
 */
intptr_t omrthread_monitor_init_with_name(omrthread_monitor_t *handle,
                                          uintptr_t flags, const char *name) {
  intptr_t rc = 0;

  rc = monitor_alloc_and_init(handle, flags, J9THREAD_LOCKING_DEFAULT,
                              J9THREAD_LOCKING_NO_DATA, name);
  return rc;
}
...
intptr_t omrthread_monitor_destroy(omrthread_monitor_t monitor) {
  ...
  if (monitor->owner || monitor_maximum_wait_number(monitor)) {
    /* This monitor is in use! It was probably abandoned when a thread was
     * cancelled. There's actually a very small timing hole here -- if the
     * thread had just locked the mutex and not yet set the owner field when it
     * was cancelled, we have no way of knowing that the mutex may be in an
     * invalid state. The same thing can happen if the thread has just cleared
     * the field and is about to unlock the mutex. Hopefully the OS takes care
     * of this for us, but it might not.
     */
    GLOBAL_UNLOCK(self);
...
/**
 * Destroy a monitor.  Destroying a monitor frees the OS mutex and links the
 * monitor onto a thread local list.
 * omrthread_monitor_flush_destroyed_monitor_list() MUST be called to flush the
 * thread local list to the global list.  The JLM data for this monitor is not
 * destroyed.
 *
 * @note A monitor must NOT be destroyed if threads are waiting on  it, or if it
 * is currently owned.
 * @note This API can only be called by the GC for object monitor destruction
 * while the GC holds exclusive VM access
 ...
 * @return  0 on success or non-0 on failure (the monitor is in use)
 */
intptr_t omrthread_monitor_destroy_nolock(omrthread_t self,
                                          omrthread_monitor_t monitor) {
  ASSERT(self);
  ASSERT(monitor);
...
/**
 * Flush the thread local list of destroyed monitors to the global list under
 * the GLOBAL_LOCK. Monitors can only be added to the thread local list via
 * omrthread_monitor_destroy_nolock
 *
 * @note This API can only be called by the GC at the end of object monitor
 * clearing while the GC holds exclusive VM access
 *
...
static void monitor_free(omrthread_library_t lib, omrthread_monitor_t monitor) {
  ...
  if (OMR_ARE_NO_BITS_SET(monitor->flags,
                          J9THREAD_MONITOR_MUTEX_UNINITIALIZED)) {
...
static void monitor_free_nolock(omrthread_library_t lib, omrthread_t thread,
  ...
  /* CMVC 144063 J9VM is "leaking" handles */
  if (OMR_ARE_ANY_BITS_SET(lib->flags,
                           J9THREAD_LIB_FLAG_DESTROY_MUTEX_ON_MONITOR_FREE)) {
    if (OMR_ARE_NO_BITS_SET(monitor->flags,
                            J9THREAD_MONITOR_MUTEX_UNINITIALIZED)) {
      OMROSMUTEX_DESTROY(monitor->mutex);
      monitor->flags = J9THREAD_MONITOR_MUTEX_UNINITIALIZED;
...
static intptr_t monitor_init(omrthread_monitor_t monitor, uintptr_t flags,
  ...
  /* check if we should spin on system monitors that are backing a Object
   * monitor the default is now that we do not spin.
   */
  if (!OMR_ARE_ALL_BITS_SET(flags, J9THREAD_MONITOR_OBJECT) ||
      OMR_ARE_ALL_BITS_SET(
          lib->flags,
          J9THREAD_LIB_FLAG_SECONDARY_SPIN_OBJECT_MONITORS_ENABLED)) {
  ...
  if (NULL != name) {
    if (OMR_ARE_ANY_BITS_SET(monitor->flags, J9THREAD_MONITOR_NAME_COPY)) {
       ...
       */
      monitor->name = (char *)omrthread_allocate_memory(
          lib, length + 1, OMRMEM_CATEGORY_THREADS);
 ...
 * @see omrthread_monitor_exit, omrthread_monitor_exit_using_threadId
 */
intptr_t omrthread_monitor_enter_using_threadId(omrthread_monitor_t monitor,
                                                omrthread_t threadId) {
  ASSERT(threadId != 0);
 ...
 * @see omrthread_abort
 */
intptr_t omrthread_monitor_enter_abortable_using_threadId(
    omrthread_monitor_t monitor, omrthread_t threadId) {
  ASSERT(threadId != 0);
  ASSERT(threadId == MACRO_SELF());
  ASSERT(monitor);
  ASSERT(FREE_TAG != monitor->count);
...
static intptr_t monitor_enter_three_tier(omrthread_t self,
  ...
  while (1) {
    ...
    if (J9THREAD_MONITOR_SPINLOCK_UNOWNED ==
        omrthread_spinlock_swapState(monitor,
                                     J9THREAD_MONITOR_SPINLOCK_EXCEEDED)) {
      MONITOR_UNLOCK(monitor);
      monitor->owner = self;
      monitor->count = 1;
      ASSERT(monitor->spinlockState != J9THREAD_MONITOR_SPINLOCK_UNOWNED);
 ...
 *
 */
intptr_t omrthread_monitor_try_enter(omrthread_monitor_t monitor) {
 ...
 *
 */
intptr_t omrthread_monitor_try_enter_using_threadId(omrthread_monitor_t monitor,
                                                    omrthread_t threadId) {
...
#if defined(OMR_THR_THREE_TIER_LOCKING)
  if (J9THREAD_MONITOR_TRY_ENTER_SPIN ==
      (monitor->flags & J9THREAD_MONITOR_TRY_ENTER_SPIN)) {
  ...
  if (0 == lockAcquired) {
    ...
    UPDATE_JLM_MON_ENTER(threadId, monitor, !IS_RECURSIVE_ENTER,
                         !IS_SLOW_ENTER);
 ...
 * @return 0 on success, <br>J9THREAD_ILLEGAL_MONITOR_STATE if the current
 * thread does not own the monitor
 *
 * @see omrthread_monitor_exit_using_threadId, omrthread_monitor_enter,
 ...
 *
 * @see omrthread_monitor_exit, omrthread_monitor_enter,
 * omrthread_monitor_enter_using_threadId
 */
intptr_t omrthread_monitor_exit_using_threadId(omrthread_monitor_t monitor,
                                               omrthread_t threadId) {
  ASSERT(threadId == MACRO_SELF());
  ASSERT(monitor);
  ASSERT(FREE_TAG != monitor->count);
...
static intptr_t monitor_exit(omrthread_t self, omrthread_monitor_t monitor) {
  ...
  if (monitor->count == 0) {
    ...
    if (J9THREAD_MONITOR_SPINLOCK_EXCEEDED ==
        omrthread_spinlock_swapState(monitor,
                                     J9THREAD_MONITOR_SPINLOCK_UNOWNED)) {
      MONITOR_LOCK(monitor, CALLER_MONITOR_EXIT1);
      unblock_spinlock_threads(self, monitor);
      MONITOR_UNLOCK(monitor);
    }
 ...
 *
 * Release the monitor, wait for a signal (notification), then re-acquire the
 * monitor. The wait may not be interrupted.
 *
 * @param[in] monitor a monitor to be waited on
 *
 * @return status code
 * @retval 0 The monitor has been waited on, notified, and reobtained.
 * @retval J9THREAD_ILLEGAL_MONITOR_STATE The current thread does not own the
 * monitor.
 *
 * @see omrthread_monitor_wait_interruptable, omrthread_monitor_wait_timed,
 ...
 * @return status code
 * @retval 0 the monitor has been waited on, notified, and reobtained
 * @retval J9THREAD_INVALID_ARGUMENT millis or nanos is out of range (millis or
 * nanos < 0, or nanos >= 1E6)
 * @retval J9THREAD_ILLEGAL_MONITOR_STATE the current thread does not own the
 * monitor
 * @retval J9THREAD_PRIORITY_INTERRUPTED the thread was aborted and still holds
 * the monitor
 * @retval J9THREAD_INTERRUPTED_MONITOR_ENTER the current thread was aborted and
 * no longer holds the monitor
 * @retval J9THREAD_TIMED_OUT the timeout expired
 *
 * @see omrthread_monitor_wait, omrthread_monitor_wait_timed,
 * omrthread_monitor_wait_interruptable
 * @see omrthread_monitor_enter
 * @see omrthread_abort
 */
intptr_t omrthread_monitor_wait_abortable(omrthread_monitor_t monitor,
                                          int64_t millis, intptr_t nanos) {
  return monitor_wait(monitor, millis, nanos, J9THREAD_FLAG_ABORTABLE);
}

/**
 * Wait on a monitor until notified, interrupted (priority or normal), aborted,
 * or timed out.
 *
 * A timeout of 0 (0ms, 0ns) indicates wait indefinitely.
 *
 ...
 * @retval J9THREAD_ILLEGAL_MONITOR_STATE The current thread does not own the
 * monitor.
 * @retval J9THREAD_INTERRUPTED The thread was interrupted while waiting. It has
 * reobtained the monitor.
 * @retval J9THREAD_PRIORITY_INTERRUPTED The thread was priority interrupted or
 * aborted and has reobtained the monitor.
 * @retval J9THREAD_INTERRUPTED_MONITOR_ENTER The current thread was aborted
 * while reobtaining the monitor. It no longer holds the monitor.
 * @retval J9THREAD_TIMED_OUT The timeout expired.
 *
 * @see omrthread_monitor_wait, omrthread_monitor_wait_timed,
 * omrthread_monitor_wait_abortable
 ...
 */
intptr_t omrthread_monitor_wait_interruptable(omrthread_monitor_t monitor,
                                              int64_t millis, intptr_t nanos) {
  return monitor_wait(monitor, millis, nanos,
                      J9THREAD_FLAG_ABORTABLE | J9THREAD_FLAG_INTERRUPTABLE);
}
 ...
 * @retval J9THREAD_TIMED_OUT The timeout expired.
 *
 * @see omrthread_monitor_wait, omrthread_monitor_wait_interruptable,
 * omrthread_monitor_wait_abortable
 * @see omrthread_monitor_enter
 */
intptr_t omrthread_monitor_wait_timed(omrthread_monitor_t monitor,
                                      int64_t millis, intptr_t nanos) {
 ...
 * monitor.
 *
 * In this function, we 'unwind' any recursive hold (monitor->count) the thread
 * has on the monitor and release the OS monitor. When the monitor is
 * re-acquired, the recursive count is restored to its original value.
 *
 * A timeout of 0 (0ms, 0ns) indicates wait indefinitely.
 *
 * If 'interruptible' is non-zero, the wait may be interrupted by one of the
 * interrupt functions. (i.e. omrthread_interrupt, omrthread_priority_interrupt,
 * omrthread_abort)
 *
 * @param[in] monitor monitor to be waited on
 * @param[in] millis >=0
 * @param[in] nanos >=0
 * @param[in] interruptible bitflag that may have one or more of these bits
 * set:<br> J9THREAD_FLAG_INTERRUPTABLE, J9THREAD_FLAG_ABORTABLE
 *
 * @return status code
 * @retval 0 The monitor has been waited on, notified, and reobtained.
 * @retval J9THREAD_TIMED_OUT The timeout expired.
 ...
 * @retval J9THREAD_PRIORITY_INTERRUPTED The thread was priority interrupted or
 * aborted. The thread still owns the monitor.
 * @retval J9THREAD_INTERRUPTED_MONITOR_ENTER The current thread was aborted.
 * the thread has released the monitor.
 *
 * @see omrthread_monitor_wait, omrthread_monitor_wait_interruptable,
...
static intptr_t monitor_wait_original(omrthread_t self,
  ...
  if (millis || nanos) {
    ...
    OMROSCOND_WAIT_IF_TIMEDOUT(MONITOR_WAIT_CONDITION(self, monitor),
                               monitor->mutex, boundedMillis, nanos) {
      ASSERT_MONITOR_UNOWNED_IF_NOT_3TIER(monitor);
  ...
  ASSERT(!priorityinterrupted || (interruptible & (J9THREAD_FLAG_INTERRUPTABLE |
                                                   J9THREAD_FLAG_ABORTABLE)));

  self->flags &= ~(J9THREAD_FLAG_WAITING | J9THREAD_FLAG_TIMER_SET |
                   J9THREAD_FLAG_INTERRUPTABLE | J9THREAD_FLAG_NOTIFIED
#ifndef OMR_THR_THREE_TIER_LOCKING
  ...
  /*
   * The interrupt remains pending if the thread was priority-interrupted or
   * notified.
   */
   ...
   * Don't clear J9THREAD_FLAG_ABORTABLE. We don't want a hole here where the
   * thread is not abortable until it enters monitor_enter_three_tier().
   */
  ...
  if (monitor_enter_three_tier(
          self, monitor,
          (BOOLEAN)((interruptible & J9THREAD_FLAG_ABORTABLE)
                        ? SET_ABORTABLE
                        : DONT_SET_ABORTABLE)) ==
      J9THREAD_INTERRUPTED_MONITOR_ENTER) {
...
static intptr_t monitor_wait_three_tier(omrthread_t self,
  ...
  if (millis || nanos) {
    ...
    OMROSCOND_WAIT_IF_TIMEDOUT(self->condition, monitor->mutex, boundedMillis,
                               nanos) {
      THREAD_LOCK(self, CALLER_MONITOR_WAIT2);
      intrFlags = self->flags & intrMask;
      interrupted = J9THR_WAIT_INTERRUPTED(intrFlags);
 ...
 */
uintptr_t omrthread_monitor_num_waiting(omrthread_monitor_t monitor) {
 ...
 * it is currently blocked while executing omrthread_monitor_wait on the
 * monitor.
 *
 * If no threads are waiting, no action is taken.
 *
 * @param[in] monitor a monitor to be signaled
 * @return  0 once the monitor has been
 * signaled<br>J9THREAD_ILLEGAL_MONITOR_STATE if the current thread does not own
 * the monitor
 *
 * @see omrthread_monitor_notify_all, omrthread_monitor_enter,
 * omrthread_monitor_wait
 */
intptr_t omrthread_monitor_notify(omrthread_monitor_t monitor) {
 ...
 * the monitor
 *
 * @see omrthread_monitor_notify, omrthread_monitor_enter,
 * omrthread_monitor_wait
 */
...
static intptr_t monitor_notify_three_tier(omrthread_t self,
  ...
  if (queue) {
    ...
    state = omrthread_spinlock_swapState(monitor,
                                         J9THREAD_MONITOR_SPINLOCK_EXCEEDED);
...
static omrthread_monitor_pool_t allocate_monitor_pool(omrthread_library_t lib) {
  ...
  omrthread_monitor_pool_t pool =
      (omrthread_monitor_pool_t)omrthread_allocate_memory(
...
void omrthread_monitor_dump_trace(omrthread_monitor_t monitor) {
  fprintf(stderr,
          ...
          monitor->tracing.monitor_name
              ? monitor->tracing.monitor_name
              : ((monitor->flags & J9THREAD_MONITOR_OBJECT) ? "(object)"
                                                            : "(null)"),
          monitor->tracing.enter_count, monitor->tracing.hold_count,
          monitor->tracing.exit_count, monitor->tracing.holdtime_sum, 0);
  fflush(stderr);
}
 ...
 *
 * @return pointer to the new OMRThreadMCSNodes structure on success, or NULL on
 * failure.
 */
static omrthread_mcs_nodes_t allocate_mcs_nodes(omrthread_library_t lib) {
  omrthread_mcs_nodes_t mcsNodes =
      (omrthread_mcs_nodes_t)omrthread_allocate_memory(
  ...
  mcsNodes->pool = pool_new(sizeof(OMRThreadMCSNode), OMRTHREAD_MIN_MCS_NODES,
                            OMRTHREAD_MCS_NODE_ALIGNMENT, 0, OMR_GET_CALLSITE(),
                            OMRMEM_CATEGORY_THREADS, omrthread_mallocWrapper,
                            omrthread_freeWrapper, lib);
...
void omrthread_dump_trace(omrthread_t thread) {
  ...
  fprintf(stderr,
          ...
          thread, thread->tracing.createtime, GetCycles(),
          thread->tracing.holdtime);
  for (i = 0;
       i < sizeof(thread->tracing.count) / sizeof(thread->tracing.count[0]);
       i++) {
    fprintf(stderr, "<thr_lock: id=%p location=%2d count=%u time=%I64d>\n",
            thread, i + 1, thread->tracing.count[i],
 ...
 */
uintptr_t omrthread_monitor_owned_by_self(omrthread_monitor_t monitor) {
  omrthread_t self = MACRO_SELF();
 ...
 * @param nodeNumber[in] The node number to add (expressed as a 1-indexed
 * J9-level NUMA node number)
 */
void omrthread_add_node_number_to_affinity_cache(omrthread_t thread,
                                                 uintptr_t nodeNumber) {
...
BOOLEAN
omrthread_does_affinity_cache_contain_node(omrthread_t thread,
                                           uintptr_t nodeNumber) {
...
/**
 * Calculate the unaccounted CPU usage quantum and charge it to the appropriate
 * category.
 *
 * @param thread[in] The thread whose unaccounted usage quantum needs to
 * charged.
...
static intptr_t fixupThreadAccounting(omrthread_t thread, uintptr_t type) {
  ...
  if (J9THREAD_TYPE_SET_CREATE == type) {
    /* During thread creation, account for all time from the start into the
     * right category */
  ...
  if (J9THREAD_SUCCESS != result) {
    ...
    if (J9THREAD_ERR_NO_SUCH_THREAD == result) {
      ...
      Trc_THR_fixupThreadAccounting_omrthread_get_cpu_time_ex_error(result,
                                                                    thread);
      return -1;
    }
  }
  ...
  /* Capture the current cpu usage of the thread since the last event (such as
   * attach, category switch */
  if (OMR_ARE_ALL_BITS_SET(thread->effective_category,
                           J9THREAD_CATEGORY_RESOURCE_MONITOR_THREAD)) {
    ...
    if ((thread->effective_category &
         J9THREAD_USER_DEFINED_THREAD_CATEGORY_MASK) > 0) {
      int userCat = (thread->effective_category -
                     J9THREAD_USER_DEFINED_THREAD_CATEGORY_1) &
 ...
 * Most of the error and validity checking for application threads will happen
 * at the JNI layer.
 ...
 * @return -1 on error and 0 on success
 */
intptr_t omrthread_set_category(omrthread_t thread, uintptr_t category,
                                uintptr_t type) {
  intptr_t rc = 0;
  omrthread_library_t lib = GLOBAL_DATA(default_library);
  ...
  /* If the category to be set is the same as the current category, nothing to
   * be done */
  if (category == thread->category && category == thread->effective_category) {
    goto unlock_exit;
  }

  /* If -XX:-EnableCPUMonitor has been set, no cpu usage accounting will be
   * done. Check if we are in the middle of a GC cycle, if so, no accounting
   * updates will be done */
  if (OMR_ARE_ALL_BITS_SET(lib->flags, J9THREAD_LIB_FLAG_ENABLE_CPU_MONITOR) &&
      ((J9THREAD_TYPE_SET_GC == type) ||
       ((J9THREAD_TYPE_SET_GC != type) &&
        (thread->category == thread->effective_category)))) {
    rc = fixupThreadAccounting(thread, type);
    /* Change the category of the thread regardless of fixupThreadAccounting
     * returning an error */
  }

  /* If an application thread is doing GC work, only change effective_category
   * to reflect that */
 ...
 * @return category
 */
uintptr_t omrthread_get_category(omrthread_t thread) {
  ASSERT(thread);
  /* If we ensure all read writes are completed at this time, we dont need to
   * take the GLOBAL_LOCK_SIMPLE */
  issueReadWriteBarrier();
  return thread->category;
}

/* Return 0 on success, non-zero on failure */
intptr_t omrthread_init_library(void) {
  omrthread_library_t lib = GLOBAL_DATA(default_library);
  if (0 == lib->initStatus) {
    init_thread_library();
  }
...
omrthread_t j9thread_self(void) { return omrthread_self(); }
