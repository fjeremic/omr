 ...
 * in-process, and once for out-of-process uses (e.g. debug extensions). The
...
/**
 * Initialize a omrthread_monitor_walk_state_t structure used to walk the
 * monitor pools.
 *
 * @param[in] walkState This is a pointer to a omrthread_monitor_walk_state_t.
 * When this call returns it will be ready to use on the first call to
 ...
 *
 * @param[in] walkState This is a pointer to a omrthread_monitor_walk_state_t.
 * It should be initialized using omrthread_monitor_init_walk before calling
 * omrthread_monitor_walk to start a new walk. (thread lib will be globally
 ...
 * monitors. It can't be used to look for a specific monitor and then quit
 * because the GLOBAL_LOCK be taken when the walk starts and exited when the
 * walk ends. If the walk is not completed the lock will not be released.
 *
