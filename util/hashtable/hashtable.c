 ...
 */
#define NEXT(p)                                                                \
  *((void **)((uint8_t *)p + table->listNodeSize - sizeof(uintptr_t)))
...
#define AVL_TREE_UNTAG(p)                                                      \
  ((J9AVLTree *)(((uintptr_t)(p)) & (~AVL_TREE_TAG_BIT)))

/**
 * Stolen from gc_base/gcutils.h
 */
#define ROUND_TO_SIZEOF_UDATA(number)                                          \
  (((number) + (sizeof(uintptr_t) - 1)) & (~(sizeof(uintptr_t) - 1)))

static uint32_t hashTableNextSize(uint32_t size);
static uintptr_t hashTableGrow(J9HashTable *table);
static J9HashTable *
hashTableNewImpl(OMRPortLibrary *portLibrary, const char *tableName,
                 uint32_t tableSize, uint32_t entrySize,
                 uint32_t entryAlignment, uint32_t flags,
                 uint32_t memoryCategory, uint32_t listToTreeThreshold,
                 J9HashTableHashFn hashFn, J9HashTableEqualFn hashEqualFn,
...
static void **hashTableFindNodeSpaceOpt(J9HashTable *table, void *entry,
                                        void **head);
static void **hashTableFindNodeInList(J9HashTable *table, void *entry,
                                      void **head);
static void *hashTableFindNodeInTree(J9HashTable *table, void *entry,
                                     void **head);
static void *hashTableAddNodeSpaceOpt(J9HashTable *table, void *entry,
                                      void **head);
static void *hashTableAddNodeInList(J9HashTable *table, void *entry,
                                    void **head);
static void *hashTableAddNodeInTree(J9HashTable *table, void *entry,
                                    void **head);
static uint32_t hashTableRemoveNodeSpaceOpt(J9HashTable *table, void *entry,
                                            void **head);
static uint32_t hashTableRemoveNodeInList(J9HashTable *table, void *entry,
                                          void **head);
static uint32_t hashTableRemoveNodeInTree(J9HashTable *table, void *entry,
                                          void **head);
static uintptr_t listToTree(J9HashTable *table, void **head,
                            uintptr_t listLength);
static void rebuildFromPools(J9HashTable *table, uint32_t newSize,
                             void **newNodes);
...
static uintptr_t collisionResilientHashTableGrow(J9HashTable *table,
                                                 uint32_t newSize);

static const uint32_t primesTable[] = {
    17,    37,    73,     149,    293,    587,     811,
    1171,  1693,  2347,   3511,   4691,   9391,    18787,
    37573, 75149, 150299, 300007, 600071, 1200077, 2200103,
};
...
static uintptr_t comparatorToEqualFn(void *leftKey, void *rightKey,
  ...
  /* Note, swapping rightKey and leftKey here to make things consistent.
   * avl_insert() has the search key as the second node argument. The hash table
   * implementation on the other hand has the search key as the first node
   * argument.
   */
...
/**
 * \brief       Create a new hash table that turns list-chains into AVL trees if
 * too many entries hash to the same slot. \ingroup     hash_table
 *
 *
 * @param portLibrary  The port library
 * @param tableName     	A string giving the name of the table, recommended
 * to use OMR_GET_CALLSITE() within caller code, this is not a restriction. If a
 * unique name is used, it must be easily searchable, i.e. there should not be
 * more than a few false hits when searching for the name. If necessary to
 * ensure uniqueness or find-ability, create a list of the unique names or name
 ...
 * @param memoryCategory  memory category for which memory allocated by
 * hashtable should use
 * @param listToTreeThreshold  The threshold after which list chains are
 * transformed into AVL trees
 * @param hashFn  Mandatory hashing function ptr
 * @param printFn  Optional node-print function ptr
 * @param searchComparator  Mandatory comparison function required for tree
 * comparison
 * @param userData  Optional userData ptr to be passed to hashFn and hashEqualFn
 * @return  An initialized hash table
 *
 * In addition to the description provided by hashTableNew, this hashtable it
 * designed to be resilient to hash collisions. Specifically, if there are more
 * than listToTreeThreshold collisions in a single bucket, the backing linked
 * list used for collision resolution will be transformed into an AVL tree.
 *
 * Note that the comparatorFn will be used for both searching and comparisons
 * into AVL trees. Note also that the trees are only transformed back into
 * normal lists during a hash table grow (assuming they are less than or equal
 * to listToTreeThreshold).
 * Note also that lists may not be transformed into trees if there was not
 * enough memory to allocate the nodes.  In other words lists of length greater
 * than listToTreeThreshold may exist.
 * Also note that nodes are automatically aligned to uintptr_t. (Additional
 * alignment cannot be specified)
 *
 * In general, you should expect collisionResilientHashTable to be slower than a
 * regular hashtable and use more memory.
 *
...
J9HashTable *collisionResilientHashTableNew(
  ...
  return hashTableNewImpl(portLibrary, tableName, tableSize, entrySize,
                          sizeof(uintptr_t),
                          flags | J9HASH_TABLE_COLLISION_RESILIENT,
                          memoryCategory, listToTreeThreshold, hashFn, NULL,
 ...
 *to use OMR_GET_CALLSITE() within caller code, this is not a restriction. If a
 *unique name is used, it must be easily searchable, i.e. there should not be
 *more than a few false hits when searching for the name. If necessary to ensure
 *uniqueness or find-ability, create a list of the unique names or name
 *conventions in this file.
 * @param tableSize         Initial number of hash table nodes (if zero, use a
 *suitable default)
 * @param entrySize         Size of the user-data for each node
 * @param entryAlignment    Optional alignment required by entry data (0 for no
 *alignment)
 * @param flags				Flags for extra options
 * @param memoryCategory	memoryCategory for which memory allocated by
 *hashtable should use
 * @param hashFn            Mandatory hashing function ptr
 * @param hashEqualFn       Mandatory hash compare function ptr
 * @param printFn           Optional node-print function ptr
 * @param userData          Optional userData ptr to be passed to hashFn and
 *hashEqualFn
 * @return                  An initialized hash table
 *
 *	Creates a hashtable with specified initial number of nodes. The actual
 *number of nodes is ceiled to the nearest prime number obtained from
 *primesTable[].
 *
 *  When J9HASH_TABLE_ALLOW_SIZE_OPTIMIZATION is defined and the entrySize fits
 *in a uintptr_t, and the tableSize is small, a hashTable will be created
 *without a backing pool. When the hashTable grows beyond a certain size, it
 *will expand to use a backing pool. The following functions are not supported
 *on space optimized hashTables: storing NULL elements hashTableForEachDo()
 *  	hashTableRehash()
 *  	hashTableDoRemove()
 *
 */
J9HashTable *hashTableNew(OMRPortLibrary *portLibrary, const char *tableName,
                          uint32_t tableSize, uint32_t entrySize,
                          uint32_t entryAlignment, uint32_t flags,
                          uint32_t memoryCategory, J9HashTableHashFn hashFn,
                          J9HashTableEqualFn hashEqualFn,
                          J9HashTablePrintFn printFn, void *functionUserData) {
  return hashTableNewImpl(portLibrary, tableName, tableSize, entrySize,
                          entryAlignment, flags, memoryCategory, U_32_MAX,
                          hashFn, hashEqualFn, NULL, printFn, functionUserData);
}
...
hashTableNewImpl(OMRPortLibrary *portLibrary, const char *tableName,
  ...
  hashTable = portLibrary->mem_allocate_memory(portLibrary, sizeof(J9HashTable),
  ...
  /* Set portLibrary and other fields now, so we can use omrmem_free_memory
   * functionality in hashTableFree() if errors occur */
  ...
  if (entryAlignment) {
    hashTable->listNodeSize =
        (((ROUND_TO_SIZEOF_UDATA(entrySize) + sizeof(uintptr_t)) +
          entryAlignment - 1) /
         entryAlignment) *
        entryAlignment;
    hashTable->treeNodeSize =
        (((ROUND_TO_SIZEOF_UDATA(entrySize) + sizeof(J9AVLTreeNode)) +
          entryAlignment - 1) /
         entryAlignment) *
        entryAlignment;
  } else {
  ...
  if (spaceOpt) {
    ...
    if (J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32 ==
      ...
      hashTable->listNodePool = pool_new(
          hashTable->listNodeSize, tableSize, entryAlignment, POOL_NO_ZERO,
          tableName, memoryCategory, POOL_FOR_PORT_PUDDLE32(portLibrary));
    } else
#endif /* OMR_ENV_DATA64 */
    {
      hashTable->listNodePool = pool_new(
          hashTable->listNodeSize, tableSize, entryAlignment, POOL_NO_ZERO,
          tableName, memoryCategory, POOL_FOR_PORT(portLibrary));
    }
  ...
  if (J9HASH_TABLE_COLLISION_RESILIENT ==
      (flags & J9HASH_TABLE_COLLISION_RESILIENT)) {
    /* Additional initialization for capability to turn lists to trees */
    hashTable->treePool =
        pool_new(sizeof(J9AVLTree), 0, sizeof(uintptr_t), 0, tableName,
                 memoryCategory, POOL_FOR_PORT(portLibrary));
    if (NULL == hashTable->treePool) {
      goto error;
    }
    /* Create an AVLTree template we need for comparatorToEqualFn.  Malloc the
     * memory instead of using the pool, as we clear the pool on growing */
    hashTable->avlTreeTemplate = portLibrary->mem_allocate_memory(
        portLibrary, sizeof(J9AVLTree), tableName, memoryCategory);
    ...
    hashTable->avlTreeTemplate->searchComparator = (intptr_t(*)(
        struct J9AVLTree *, uintptr_t, J9AVLTreeNode *))comparatorFn;
    ...
    /* Need to have the avlTreeTemplate here so that the comparatorToEqualFn can
     * access it*/
    ...
    if (J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32 ==
        (flags & J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32)) {
      hashTable->treeNodePool = pool_new(
          hashTable->treeNodeSize, 0, entryAlignment, 0, OMR_GET_CALLSITE(),
          memoryCategory, POOL_FOR_PORT_PUDDLE32(portLibrary));
    } else
#endif /* OMR_ENV_DATA64 */
    {
      hashTable->treeNodePool =
          pool_new(hashTable->treeNodeSize, 0, entryAlignment, 0, tableName,
                   memoryCategory, POOL_FOR_PORT(portLibrary));
    }
  ...
  hashTable->nodes = portLibrary->mem_allocate_memory(
      portLibrary, sizeof(uintptr_t) * hashTable->tableSize, tableName,
      memoryCategory);
 ...
 */
void hashTableFree(J9HashTable *hashTable) {
  if (NULL != hashTable) {
 ...
 * @param table
 * @param entry
 * @return                  NULL if entry is not present in the table; otherwise
 * a pointer to the user-data
 *
 */
void *hashTableFind(J9HashTable *table, void *entry) {
  uintptr_t hash =
      table->hashFn(entry, table->hashFnUserData) % table->tableSize;
...
static void **hashTableFindNodeSpaceOpt(J9HashTable *table, void *entry,
  ...
  void **node = head;
  while ((NULL != *node) &&
         (0 == table->hashEqualFn(node, entry, table->equalFnUserData))) {
    node += 1;
...
static void *hashTableFindNodeInTree(J9HashTable *table, void *entry,
                                     void **head) {
  /* Fake a tree node from an entry by just moving the pointer to where the
   * start of a tree node would be */
 ...
 * @param table
 * @param entry
 * @return                  NULL on failure (to allocate a new node); otherwise
 * the entry pointer
 *
 ...
 */
void *hashTableAdd(J9HashTable *table, void *entry) {
...
static void *hashTableAddNodeInList(J9HashTable *table, void *entry,
  ...
  /* Find entry if it is in the list, otherwise return a slot where we can add a
   * pointer to it */
  while ((NULL != *where) &&
         (0 == table->hashEqualFn(*where, entry, table->equalFnUserData))) {
    where = &NEXT(*where);
    listLength += 1;
  }
  ...
  if (NULL != *where) {
    ...
    /* Reached threshold, need to turn this list into a tree.  Note that failure
     * is okay, we'll just add the node as a list if a tree could not be
     * allocated
     */
    if ((listLength > table->listToTreeThreshold) &&
        (0 == listToTree(table, head, listLength))) {
...
static void *hashTableAddNodeInTree(J9HashTable *table, void *entry,
  ...
  if (NULL != newNode) {
    ...
    if (NULL == insertNode) {
      /* Looking at avl tree code this is currently impossible, but leaving it
       * here to be safe.. */
...
static uintptr_t listToTree(J9HashTable *table, void **head,
  ...
  if ((0 != hashTableCanRehash(table)) && (NULL != tree)) {
    uintptr_t minimumCapacity =
        ((uintptr_t)table->numberOfTreeNodes) + listLength;
    ...
    *tree = *(table->avlTreeTemplate);
    if (0 == pool_ensureCapacity(table->treeNodePool, minimumCapacity)) {
      ...
      while (NULL != nodeToCopy) {
        J9AVLTreeNode *newTreeNode =
            (J9AVLTreeNode *)pool_newElement(table->treeNodePool);
        J9AVLTreeNode *insertNode = NULL;
      ...
      HASHTABLE_ASSERT(((uintptr_t)table->numberOfTreeNodes) ==
                       minimumCapacity);
 ...
 */
uint32_t hashTableRemove(J9HashTable *table, void *entry) {
  uintptr_t hash =
      table->hashFn(entry, table->hashFnUserData) % table->tableSize;
  void **head = &table->nodes[hash];
  uint32_t rc = 1;
...
static uint32_t hashTableRemoveNodeSpaceOpt(J9HashTable *table, void *entry,
  ...
  if (NULL != *node) {
    *node = NULL;
    ...
    while (NULL != *node) {
      uintptr_t hash =
          table->hashFn(node, table->hashFnUserData) % table->tableSize;
 ...
 * node is removed from the table and freed.  Otherwise, the node is left
 * untouched.
 */
void hashTableForEachDo(J9HashTable *table, J9HashTableDoFn doFn,
                        void *opaque) {
 ...
 *
 */
void hashTableRehash(J9HashTable *table) {
  uint32_t i = 0;
  void *chain = NULL;
  void *tail = NULL;
  ...
  if (J9HASH_TABLE_COLLISION_RESILIENT ==
      (table->flags & J9HASH_TABLE_COLLISION_RESILIENT)) {
    /* Not currently supported.
     *
     * Really complicated to implement as it cannot be done in-place as
     * listNodes might become treeNodes (and vice-versa) and it's impossible to
     * predict the required pool capacities.  One might expect this is a just a
     * special case of collisionResilientHashTableGrow, however the problem is
     * much harder as you cannot simply rebuild the original hash table on
     * failure as the entries' hashcodes may have changed compared to the
     * original.
     */
    Assert_hashTable_unreachable();
  }
...
static uintptr_t collisionResilientHashTableGrow(J9HashTable *table,
  ...
  if (0 == pool_ensureCapacity(table->listNodePool, table->numberOfNodes)) {
    void **newNodes = (void **)table->portLibrary->mem_allocate_memory(
        table->portLibrary, newSize * sizeof(uintptr_t), table->tableName,
...
static void rebuildFromPools(J9HashTable *table, uint32_t newSize,
  ...
  /* Sanity checks to make sure the table has had correctly counted the right
   * number of nodes */
  HASHTABLE_ASSERT(nodeCount == table->numberOfNodes);
  HASHTABLE_ASSERT(treeNodeCount == table->numberOfTreeNodes);

  /* Have to check if we should turn chains into trees.  Reset numberOfTreeNodes
   * and clear all trees. ListToTree() will increment numberOfTreeNodes to the
   * correct value as well as allocate new J9AVLTrees.
   */
  ...
  for (i = 0; i < newSize; i++) {
    ...
    if (listLength > table->listToTreeThreshold) {
      /* This can fail, but that should be acceptable, we'll just have a list of
       * length larger than listToTreeThreshold */
 ...
 * @return            NULL if  no more nodes; otherwise a the address of the
 *node
 *
 ...
 */
void *hashTableStartDo(J9HashTable *table, J9HashTableState *handle) {
  void *result = NULL;
  ...
  if (NULL == table->listNodePool) {
    ...
    /* It would be convenient if we could iterate and perform doRemoves on avl
     * trees, but currently that API is not supported, so we just have to
     * iterate the treeNode pool
     */
    if (numberOfListNodes > 0) {
      ...
      /* Had to have found a listNode in the table unless numberOfListNodes was
       * incorrect */
  ...
  hashTable_printf(
      "hashTableStartDo <%s>: handle=%p, table=%p, bucket=%u, node=%p\n",
 ...
 * @return                  NULL if  no more elements; otherwise a pointer to a
 * node
 *
 * Continue an iteration set that will return when code is to be executed.
 */
void *hashTableNextDo(J9HashTableState *handle) {
  ...
  if (NULL == table->listNodePool) {
    ...
    case J9HASH_TABLE_ITERATE_STATE_LIST_NODES:
      if (TRUE == handle->didDeleteCurrentNode) {
        /* if the current node was deleted, don't advance the pointer within the
         * bucket, since it's already correct */
      } else {
  ...
  hashTable_printf(
      "hashTableNextDo <%s>: handle=%p, table=%p, bucket=%u, node=%p\n",
      table->tableName, handle, table, handle->bucketIndex, result);
 ...
 * @return                  0 on success, 1 on failure
 *
 * NOTE: If you're using collisionResilientHashTable you MUST finish the
 * iteration if you remove any elements
 */
uintptr_t hashTableDoRemove(J9HashTableState *handle) {
  J9HashTable *table = handle->table;
  uintptr_t rc = 1;
  HASHTABLE_DEBUG_PORT(table->portLibrary);
  ...
  if (NULL == table->listNodePool) {
    ...
    hashTable_printf("hashTableDoRemove <%s>: handle=%p, table=%p, bucket=%u, "
                     "removedElement=%p\n",
                     table->tableName, handle, table, handle->bucketIndex,
                     currentNode);
    switch (handle->iterateState) {
    case J9HASH_TABLE_ITERATE_STATE_LIST_NODES:
    ...
    case J9HASH_TABLE_ITERATE_STATE_TREE_NODES:
      rc = hashTableRemove(table,
                           AVL_NODE_TO_DATA(handle->pointerToCurrentNode));
 ...
 *
 */
uint32_t hashTableGetCount(J9HashTable *table) { return table->numberOfNodes; }
...
static uintptr_t hashTableGrowListNodes(J9HashTable *table, uint32_t newSize) {
  ...
  void **newNodes = table->portLibrary->mem_allocate_memory(
      table->portLibrary, sizeof(uintptr_t) * newSize, table->tableName,
      table->memoryCategory);
  if (NULL != newNodes) {
    uint32_t i = 0;
    ...
    table->tableSize = newSize;
    table->nodes = newNodes;
    /* Sanity check to make sure that the old hash table had calculated the
...
static uintptr_t hashTableGrowSpaceOpt(J9HashTable *table, uint32_t newSize) {
  void **newNodes = NULL;
  uint32_t numberOfNodes = 0;
  uint32_t i = 0;
  uintptr_t hash = 0;
  ...
  if (newSize > SPACE_OPT_LIMIT) {
    /* start using a pool */
    table->listNodePool =
        pool_new(table->listNodeSize, table->tableSize, table->nodeAlignment,
                 POOL_NO_ZERO, table->tableName, table->memoryCategory,
                 POOL_FOR_PORT(table->portLibrary));
    if (NULL == table->listNodePool) {
      goto error;
    }
    if (0 != pool_ensureCapacity(table->listNodePool, table->numberOfNodes)) {
  ...
  table->tableSize = newSize;
  table->nodes = newNodes;
  /* Sanity check to make sure that the old hash table had calculated the right
   * number of nodes */
  HASHTABLE_ASSERT(numberOfNodes == table->numberOfNodes);
  return 0;
