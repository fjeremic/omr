  ...
  J9HashTableComparatorFn comparatorFn, J9HashTablePrintFn printFn, void* functionUserData)
{
  return hashTableNewImpl(portLibrary, tableName, tableSize, entrySize, sizeof(uintptr_t),
    flags | J9HASH_TABLE_COLLISION_RESILIENT, memoryCategory, listToTreeThreshold, hashFn, NULL, comparatorFn, printFn,
    functionUserData);
}
...
{
  return hashTableNewImpl(portLibrary, tableName, tableSize, entrySize, entryAlignment, flags, memoryCategory, U_32_MAX,
  ...
  J9HashTablePrintFn printFn, void* functionUserData)
{
  ...
  if (spaceOpt) {
    ...
    if (J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32 == (flags & J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32)) {
      hashTable->listNodePool = pool_new(hashTable->listNodeSize, tableSize, entryAlignment, POOL_NO_ZERO, tableName,
  ...
  if (J9HASH_TABLE_COLLISION_RESILIENT == (flags & J9HASH_TABLE_COLLISION_RESILIENT)) {
    ...
    hashTable->treePool
      = pool_new(sizeof(J9AVLTree), 0, sizeof(uintptr_t), 0, tableName, memoryCategory, POOL_FOR_PORT(portLibrary));
    if (NULL == hashTable->treePool) {
      goto error;
    }
    /* Create an AVLTree template we need for comparatorToEqualFn.  Malloc the memory instead of using the pool, as we
     * clear the pool on growing */
...
void hashTableRehash(J9HashTable* table)
{
  ...
  if (J9HASH_TABLE_COLLISION_RESILIENT == (table->flags & J9HASH_TABLE_COLLISION_RESILIENT)) {
     ...
     * special case of collisionResilientHashTableGrow, however the problem is much harder as you cannot simply rebuild
     * the original hash table on failure as the entries' hashcodes may have changed compared to the original.
     */
    Assert_hashTable_unreachable();
  }
...
static void rebuildFromPools(J9HashTable* table, uint32_t newSize, void** newNodes)
{
  ...
  /* Have to check if we should turn chains into trees.  Reset numberOfTreeNodes and clear all trees. ListToTree() will
   * increment numberOfTreeNodes to the correct value as well as allocate new J9AVLTrees.
   */
...
uintptr_t hashTableDoRemove(J9HashTableState* handle)
{
  ...
  if (NULL == table->listNodePool) {
    ...
    hashTable_printf("hashTableDoRemove <%s>: handle=%p, table=%p, bucket=%u, removedElement=%p\n", table->tableName,
      handle, table, handle->bucketIndex, currentNode);
    switch (handle->iterateState) {
    case J9HASH_TABLE_ITERATE_STATE_LIST_NODES:
