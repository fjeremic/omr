...
static J9HashTable* hashTableNewImpl(OMRPortLibrary* portLibrary, const char* tableName, uint32_t tableSize,
    uint32_t entrySize, uint32_t entryAlignment, uint32_t flags, uint32_t memoryCategory, uint32_t listToTreeThreshold,
...
static uintptr_t comparatorToEqualFn(void* leftKey, void* rightKey, void* userData)
{
 ...
 * 							within caller code, this is not a restriction. If a unique name is used,
 * it must be easily searchable, i.e. there should not be more than a few false hits when searching for the name. If
 * necessary to ensure uniqueness or find-ability, create a list of the unique names or name conventions in this file.
 ...
 *
 */
J9HashTable* collisionResilientHashTableNew(OMRPortLibrary* portLibrary, const char* tableName, uint32_t tableSize,
    uint32_t entrySize, uint32_t flags, uint32_t memoryCategory, uint32_t listToTreeThreshold, J9HashTableHashFn hashFn,
    J9HashTableComparatorFn comparatorFn, J9HashTablePrintFn printFn, void* functionUserData)
{
    return hashTableNewImpl(portLibrary, tableName, tableSize, entrySize, sizeof(uintptr_t),
        flags | J9HASH_TABLE_COLLISION_RESILIENT, memoryCategory, listToTreeThreshold, hashFn, NULL, comparatorFn,
        printFn, functionUserData);
}
 ...
 *
 */
J9HashTable* hashTableNew(OMRPortLibrary* portLibrary, const char* tableName, uint32_t tableSize, uint32_t entrySize,
    ...
    J9HashTablePrintFn printFn, void* functionUserData)
{
    ...
    /* Set portLibrary and other fields now, so we can use omrmem_free_memory functionality in hashTableFree() if errors
     * occur */
    ...
    if (entryAlignment) {
        ...
        hashTable->treeNodeSize
            = (((ROUND_TO_SIZEOF_UDATA(entrySize) + sizeof(J9AVLTreeNode)) + entryAlignment - 1) / entryAlignment)
            * entryAlignment;
    } else {
    ...
    if (spaceOpt) {
        ...
        if (J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32 == (flags & J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32)) {
            hashTable->listNodePool = pool_new(hashTable->listNodeSize, tableSize, entryAlignment, POOL_NO_ZERO,
    ...
    if (J9HASH_TABLE_COLLISION_RESILIENT == (flags & J9HASH_TABLE_COLLISION_RESILIENT)) {
        ...
        hashTable->treePool = pool_new(
            sizeof(J9AVLTree), 0, sizeof(uintptr_t), 0, tableName, memoryCategory, POOL_FOR_PORT(portLibrary));
        if (NULL == hashTable->treePool) {
            goto error;
        }
        /* Create an AVLTree template we need for comparatorToEqualFn.  Malloc the memory instead of using the pool, as
         * we clear the pool on growing */
...
#if defined(OMR_ENV_DATA64)
        if (J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32 == (flags & J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32)) {
            hashTable->treeNodePool = pool_new(hashTable->treeNodeSize, 0, entryAlignment, 0, OMR_GET_CALLSITE(),
                memoryCategory, POOL_FOR_PORT_PUDDLE32(portLibrary));
        } else
#endif /* OMR_ENV_DATA64 */
        {
            hashTable->treeNodePool = pool_new(
                hashTable->treeNodeSize, 0, entryAlignment, 0, tableName, memoryCategory, POOL_FOR_PORT(portLibrary));
        }
 ...
 */
void hashTableForEachDo(J9HashTable* table, J9HashTableDoFn doFn, void* opaque)
{
...
void hashTableRehash(J9HashTable* table)
{
    ...
    if (J9HASH_TABLE_COLLISION_RESILIENT == (table->flags & J9HASH_TABLE_COLLISION_RESILIENT)) {
         ...
         *
         * Really complicated to implement as it cannot be done in-place as listNodes might become treeNodes (and
         * vice-versa) and it's impossible to predict the required pool capacities.  One might expect this is a just a
         * special case of collisionResilientHashTableGrow, however the problem is much harder as you cannot simply
         * rebuild the original hash table on failure as the entries' hashcodes may have changed compared to the
         * original.
         */
        Assert_hashTable_unreachable();
    }
...
void* hashTableStartDo(J9HashTable* table, J9HashTableState* handle)
{
    ...
    if (NULL == table->listNodePool) {
        ...
        /* It would be convenient if we could iterate and perform doRemoves on avl trees, but currently that API is not
         * supported, so we just have to iterate the treeNode pool
         */
    ...
    if (NULL == table->listNodePool) {
        ...
        case J9HASH_TABLE_ITERATE_STATE_LIST_NODES:
            if (TRUE == handle->didDeleteCurrentNode) {
                /* if the current node was deleted, don't advance the pointer within the bucket, since it's already
                 * correct */
            } else {
...
static uintptr_t hashTableGrowSpaceOpt(J9HashTable* table, uint32_t newSize)
{
    ...
    if (newSize > SPACE_OPT_LIMIT) {
        /* start using a pool */
        table->listNodePool = pool_new(table->listNodeSize, table->tableSize, table->nodeAlignment, POOL_NO_ZERO,
            table->tableName, table->memoryCategory, POOL_FOR_PORT(table->portLibrary));
        if (NULL == table->listNodePool) {
            goto error;
        }
        if (0 != pool_ensureCapacity(table->listNodePool, table->numberOfNodes)) {
