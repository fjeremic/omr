...
static J9HashTable *hashTableNewImpl(
    OMRPortLibrary *portLibrary, const char *tableName, uint32_t tableSize,
    uint32_t entrySize, uint32_t entryAlignment, uint32_t flags,
    ...
    switch (handle->iterateState) {
      case J9HASH_TABLE_ITERATE_STATE_LIST_NODES:
        if (TRUE == handle->didDeleteCurrentNode) {
          /* if the current node was deleted, don't advance the pointer within
           * the bucket, since it's already correct */
        } else {
...
uintptr_t hashTableDoRemove(J9HashTableState *handle) {
  ...
  if (NULL == table->listNodePool) {
    ...
    hashTable_printf(
        "hashTableDoRemove <%s>: handle=%p, table=%p, bucket=%u, "
        "removedElement=%p\n",
