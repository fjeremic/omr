...
static J9HashTable*
hashTableNewImpl(OMRPortLibrary* portLibrary,
...
static uintptr_t
hashTableGrowSpaceOpt(J9HashTable*, uint32_t newSize);
static uintptr_t
 ...
 */
J9HashTable*
collisionResilientHashTableNew(OMRPortLibrary* portLibrary,
                               const char* tableName,
                               uint32_t tableSize,
                               uint32_t entrySize,
                               uint32_t flags,
                               ...
                               void* functionUserData)
{
  return hashTableNewImpl(portLibrary,
                          tableName,
                          tableSize,
                          entrySize,
                          sizeof(uintptr_t),
 ...
 */
J9HashTable*
hashTableNew(OMRPortLibrary* portLibrary,
             const char* tableName,
             uint32_t tableSize,
             uint32_t entrySize,
             uint32_t entryAlignment,
                 ...
                 void* functionUserData)
{
  ...
  hashTable = portLibrary->mem_allocate_memory(
    portLibrary, sizeof(J9HashTable), tableName, memoryCategory);
  ...
  if (spaceOpt) {
    ...
    if (J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32 ==
      ...
      hashTable->listNodePool = pool_new(hashTable->listNodeSize,
                                         tableSize,
                                         entryAlignment,
                                         POOL_NO_ZERO,
                                         tableName,
                                         memoryCategory,
                                         POOL_FOR_PORT_PUDDLE32(portLibrary));
    } else
#endif /* OMR_ENV_DATA64 */
    {
      hashTable->listNodePool = pool_new(hashTable->listNodeSize,
  ...
  if (J9HASH_TABLE_COLLISION_RESILIENT ==
    ...
    hashTable->treePool = pool_new(sizeof(J9AVLTree),
                                   0,
                                   sizeof(uintptr_t),
                                   0,
                                   tableName,
    ...
    if (J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32 ==
        (flags & J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32)) {
      hashTable->treeNodePool = pool_new(hashTable->treeNodeSize,
                                         0,
                                         entryAlignment,
                                         0,
                                         OMR_GET_CALLSITE(),
  ...
  hashTable->nodes =
    portLibrary->mem_allocate_memory(portLibrary,
                                     sizeof(uintptr_t) * hashTable->tableSize,
                                     tableName,
                                     memoryCategory);
...
hashTableFind(J9HashTable* table, void* entry)
{
  ...
  hashTable_printf(
    "hashTableFind <%s>: table=%p entry=%p\n", table->tableName, table, entry);
...
hashTableAdd(J9HashTable* table, void* entry)
{
  ...
  hashTable_printf(
    "hashTableAdd <%s>: table=%p entry=%p\n", table->tableName, table, entry);
...
hashTableGrowListNodes(J9HashTable* table, uint32_t newSize)
{
  ...
  void** newNodes =
    table->portLibrary->mem_allocate_memory(table->portLibrary,
                                            sizeof(uintptr_t) * newSize,
                                            table->tableName,
                                            table->memoryCategory);
  if (NULL != newNodes) {
    uint32_t i = 0;
...
hashTableGrowSpaceOpt(J9HashTable* table, uint32_t newSize)
{
  ...
  if (newSize > SPACE_OPT_LIMIT) {
    /* start using a pool */
    table->listNodePool = pool_new(table->listNodeSize,
                                   table->tableSize,
                                   table->nodeAlignment,
                                   POOL_NO_ZERO,
                                   table->tableName,
