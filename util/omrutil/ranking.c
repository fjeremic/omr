...
static void updateLowestNotFull(OMRRanking *ranking, void *key,
                                uintptr_t count);
...
/*
 * Implemented using an array which has the nice property of rankings being laid
 * out in order in memory.  This allows access to the kth ranking in constant
 * time.
 *
 * But can also implement using a heap.  Heap has advantage of log(n) bounded
 * updates and probably requires less operations than an array to update a data
 * structure.
 *
 * In practice the difference is probably minor though.
 */

/* Convention we use is the at rankTable[ranking->size-1] is the element with
 * the highest count and entires with decreasing index have decreasing counts
 */

static uintptr_t hashEqualFn(void *leftKey, void *rightKey, void *userData) {
  return ((hashTableEntry *)leftKey)->data ==
         ((hashTableEntry *)rightKey)->data;
}
...
void rankingClear(OMRRanking *ranking) {
  ...
  /* todo: Find less ugly way of doing this other than walking through the whole
   * structure
   */
...
OMRRanking *rankingNew(OMRPortLibrary *portLibrary, uint32_t size) {
  ...
  OMRRanking *newRanking =
      omrmem_allocate_memory(sizeof(OMRRanking), OMRMEM_CATEGORY_MM);
  if (NULL == newRanking) {
    return NULL;
  }
  newRanking->size = size;
  newRanking->curSize = 0;
  newRanking->rankTable =
      omrmem_allocate_memory(size * sizeof(rankTableEntry), OMRMEM_CATEGORY_MM);
  if (NULL == newRanking->rankTable) {
    return NULL;
  }
  newRanking->hashTable = hashTableNew(
      portLibrary, OMR_GET_CALLSITE(), size * HASHTABLE_RATIO,
      sizeof(hashTableEntry), 0, J9HASH_TABLE_ALLOW_SIZE_OPTIMIZATION,
      OMRMEM_CATEGORY_VM, hashFn, hashEqualFn, NULL, NULL);
...
static void bubbleUp(OMRRanking *ranking, uint32_t rank) {
  ...
  while ((rank != ranking->size - 1) && (ranking->rankTable[rank].count >
                                         ranking->rankTable[rank + 1].count)) {
...
uintptr_t rankingIncrementEntry(OMRRanking *ranking, void *key,
  ...
  foundHashTableEntryPtr =
      hashTableFind(ranking->hashTable, &hashTableEntryToFind);
