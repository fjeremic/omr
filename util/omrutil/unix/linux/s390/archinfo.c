 ...
 * support TREX and not end up generating default 9672 code! vendor_id       :
 * IBM/S390 # processors    : 2 bogomips per cpu: 838.86 processor 0: version =
 * FF,  identification = 100003,  machine = 2064 processor 1: version = FF,
 * identification = 200003,  machine = 2064
 *
 * vs.
 *
 * vendor_id       : IBM/S390
    ...
    while (fgets(line, LINE_SIZE, fp) > 0) {
      ...
      if (len > PROC_HEADER_SIZE &&
        ...
        if (len == PROC_LINE_SIZE) {
           ...
           * 9672 */
          sscanf(
            line, "%*s %*d%*c %*s %*c %*s %*s %*c %*s %*s %*c %d", &machine);
        }
      }
    }
    fclose(fp);
