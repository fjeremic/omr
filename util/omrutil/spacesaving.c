 ...
 */
void spaceSavingUpdate(OMRSpaceSaving *spaceSaving, void *data,
                       uintptr_t count) {
  if (rankingIncrementEntry(spaceSaving->ranking, data, count) !=
    ...
    if (spaceSaving->ranking->curSize == spaceSaving->ranking->size) {
      rankingUpdateLowest(spaceSaving->ranking, data,
...
uintptr_t spaceSavingGetKthMostFreqCount(OMRSpaceSaving *spaceSaving,
                                         uintptr_t k) {
