...
extern void
_j9Z10Zero(void* ptr, uintptr_t length);
#endif
...
struct IHAPSA
{
  ...
  unsigned int
    FLCFGIEF : 1; /* General-Instructions-Extension Facility offset X'CC' */
};
#endif
...
OMRZeroMemory(void* ptr, uintptr_t length)
{
  ...
  /* Zeroing by dcbz is effective if requested length is at least twice larger
   * then Data Cache Block size */
  if (length < (2 * cacheLineSize)) {
    memset(ptr, 0, (size_t)length);
    return;
  }

  /* VMDESIGN 1314 - Allow the compile to unroll the loop below by avoiding
   * using the global in the loop */

  localCacheLineSize = cacheLineSize;

  /* Zero any initial portion to first cache line boundary
   * Assumed here that size of first portion (from start to aligned address) is
   * smaller then total requested size This is correct because sizes smaller
   * then 2 * Data Cache Block size are served already
   */
  if ((uintptr_t)addr & (localCacheLineSize - 1)) {
    limit = (char*)(((uintptr_t)addr + localCacheLineSize - 1) &
                    ~(localCacheLineSize - 1));
