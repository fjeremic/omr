...
void*
pool_portLibAlloc(OMRPortLibrary* portLibrary,
                  uint32_t size,
                  const char* callSite,
                  ...
                  uint32_t* doInit)
{
  return portLibrary->mem_allocate_memory(
    portLibrary, size, callSite, memoryCategory);
}
...
void*
pool_portLibAlloc32(OMRPortLibrary* portLibrary,
                    uint32_t size,
                    const char* callSite,
                    uint32_t memoryCategory,
                    uint32_t type,
                    uint32_t* doInit)
{
  ...
  if (POOL_ALLOC_TYPE_PUDDLE == type) {
    address = portLibrary->mem_allocate_memory32(
      portLibrary, size, callSite, memoryCategory);
  } else {
    address = portLibrary->mem_allocate_memory(
      portLibrary, size, callSite, memoryCategory);
  }
