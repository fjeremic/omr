...
extern "C"
{
  ...
  static intptr_t J9HookReserve(struct J9HookInterface** hookInterface,
                                uintptr_t taggedEventNum);
  static uintptr_t J9HookAllocateAgentID(
    struct J9HookInterface** hookInterface);
...
#define HOOK_RECORD(interface, event)                                          \
  (((J9HookRecord**)((uint8_t*)(interface) + (interface)->size))[-1 - (event)])
...
#define HOOK_VALID_ID(id) ((((id) | 1) + 1))
                                     ...
                                     size_t interfaceSize)
  {
    ...
    if (omrthread_monitor_init_with_name(
          &commonInterface->lock, 0, "Hook Interface")) {
      J9HookShutdownInterface(hookInterface);
      return J9HOOK_ERR_NOMEM;
    }

    commonInterface->pool = pool_new(sizeof(J9HookRecord),
                                     0,
                                     0,
                                     0,
                                     OMR_GET_CALLSITE(),
   ...
   *
   * This function should not be called directly. It should be called through
   * the hook interface
   */
  static void J9HookShutdownInterface(struct J9HookInterface** hookInterface)
  {
   ...
   * disabled before the listeners are informed. Any attempts to add listeners
   * to a TAG_ONCE event once it has been reported will fail.
   *
   * This function should not be called directly. It should be called through
   * the hook interface
   *
  ...
  /*
   * Mark this event as disabled. Any future attempts to add a hook for this
   * event will result in an error.
   *
   * This function should not be called directly. It should be called through
   * the hook interface
   *
   * Returns 0 on success, or non-zero if the event is already hooked or
   * reserved
   */
  static intptr_t J9HookDisable(struct J9HookInterface** hookInterface,
                                uintptr_t taggedEventNum)
  ...
  /*
   * Use J9HookReserve to indicate your intention to hook an event in the
   * future. Certain events must be reserved before the providing module
   * initializes completely, or it may make decisions which make it impossible
   * to add the hooks later. Once the point of no-return is reached, the
   * providing module will disable the event if it has not been registered or
   * reserved.
   *
   * This function should not be called directly. It should be called through
   * the hook interface
    ...
    uintptr_t agentID)
  {
    ...
    if (HOOK_FLAGS(commonInterface, eventNum) & J9HOOK_FLAG_DISABLED) {
       ...
       * requested agent. (It is a legitimate position if all records before
       * this position have the same or lower agent IDs (tested previously) and
       * all records after this position have the same or higher IDs)
       */
      if ((emptyRecord != NULL) &&
   ...
   * with lower agent IDs will always receive events before listeners with
   * higher agent IDs. The special J9HOOK_AGENT_FIRST and J9HOOK_AGENT_LAST IDs
   * may be used to register listeners which will be among the first or last to
   * receive an event.
   *
   * This function should not be called directly. It should be called through
   * the hook interface
                                 ...
                                 ...)
  {
    ...
    return J9HookRegisterWithCallSitePrivate(
      hookInterface, taggedEventNum, function, NULL, userData, agentID);
  }
                               ...
                               void* userData)
  {
    ...
    while (record) {
      if (record->function == function) {
        if ((userData == NULL) || (record->userData == userData)) {
          ...
          if (userData != NULL) {
            /* copy data from the event record so that we can report it once
    ...
    if (hooksRemoved != 0) {
      ...
      (*hookInterface)
        ->J9HookDispatch(
          hookInterface, J9HOOK_REGISTRATION_EVENT, &eventStruct);
    }
  }

  /**
   * Determine if the specified event has been disabled. This function does not
   * modify the hook interface in any way. Note that there's no atomic
   * protection of any kind -- just because the event wasn't disabled when you
   * called this function doesn't mean that it won't be by the time you try to
   * register for it.
   *
