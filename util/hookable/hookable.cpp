...
static void J9HookUnregister(struct J9HookInterface **hookInterface,
                             uintptr_t taggedEventNum, J9HookFunction function,
                             void *userData);
static intptr_t J9HookRegister(struct J9HookInterface **hookInterface,
                               uintptr_t taggedEventNum,
                               J9HookFunction function, void *userData, ...);
static intptr_t
J9HookRegisterWithCallSite(struct J9HookInterface **hookInterface,
                           uintptr_t taggedEventNum, J9HookFunction function,
                           const char *callsite, void *userData, ...);
static void J9HookShutdownInterface(struct J9HookInterface **hookInterface);
static intptr_t J9HookDisable(struct J9HookInterface **hookInterface,
                              uintptr_t taggedEventNum);
static intptr_t J9HookIsEnabled(struct J9HookInterface **hookInterface,
                                uintptr_t taggedEventNum);
static void J9HookDispatch(struct J9HookInterface **hookInterface,
                           uintptr_t taggedEventNum, void *eventData);
static intptr_t J9HookReserve(struct J9HookInterface **hookInterface,
                              uintptr_t taggedEventNum);
static uintptr_t J9HookAllocateAgentID(struct J9HookInterface **hookInterface);
static void J9HookDeallocateAgentID(struct J9HookInterface **hookInterface,
                                    uintptr_t agentID);
...
/* flags are stored at the beginning of the interface just after the common
...
#define HOOK_RECORD(interface, event)                                          \
  (((J9HookRecord **)((uint8_t *)(interface) +                                 \
                      (interface)->size))[-1 - (event)])
...
/* each record has an ID which is used to detect inconsistent records without
 * resorting to monitors */
/* even IDs are valid. Odd IDs are invalid */
/* a record is consistent if the same ID is in the record before all the fields
 * are read and after all the fields are read */
/* read and write barriers must be used to ensure that reads and writes occur in
 * the correct order */
...
intptr_t J9HookInitializeInterface(struct J9HookInterface **hookInterface,
  ...
  J9CommonHookInterface *commonInterface =
      (J9CommonHookInterface *)hookInterface;

  memset(commonInterface, 0, interfaceSize);

  commonInterface->hookInterface =
      (J9HookInterface *)GLOBAL_TABLE(hookFunctionTable);

  commonInterface->size = interfaceSize;

  if (omrthread_monitor_init_with_name(&commonInterface->lock, 0,
                                       "Hook Interface")) {
    J9HookShutdownInterface(hookInterface);
    return J9HOOK_ERR_NOMEM;
  }

  commonInterface->pool =
      pool_new(sizeof(J9HookRecord), 0, 0, 0, OMR_GET_CALLSITE(),
               OMRMEM_CATEGORY_VM, POOL_FOR_PORT((OMRPortLibrary *)portLib));
  ...
  commonInterface->threshold4Trace =
      OMRHOOK_DEFAULT_THRESHOLD_IN_MICROSECONDS_WARNING_CALLBACK_ELAPSED_TIME;

  commonInterface->eventSize =
      (interfaceSize - sizeof(J9CommonHookInterface)) /
 ...
 *
 * This function should not be called directly. It should be called through the
 * hook interface
 */
static void J9HookShutdownInterface(struct J9HookInterface **hookInterface) {
  J9CommonHookInterface *commonInterface =
...
/*
 * Inform all registered listeners that the specified event has occurred.
 * Details about the event should be available through eventData.
 *
 * If the J9HOOK_TAG_ONCE bit is set in the taggedEventNum, then the event is
 * disabled before the listeners are informed. Any attempts to add listeners to
 * a TAG_ONCE event once it has been reported will fail.
 *
 * This function should not be called directly. It should be called through the
 * hook interface
 *
...
static void J9HookDispatch(struct J9HookInterface **hookInterface,
  ...
  uintptr_t samplingInterval =
      (taggedEventNum & J9HOOK_TAG_SAMPLING_MASK) >> 16;
  bool sampling = false;
  ...
  if (taggedEventNum & J9HOOK_TAG_ONCE) {
    ...
    HOOK_FLAGS(commonInterface, eventNum) =
        (oldFlags | J9HOOK_FLAG_DISABLED) &
        ~(J9HOOK_FLAG_RESERVED | J9HOOK_FLAG_HOOKED);
  ...
  while (record) {
    ...
    if (HOOK_IS_VALID_ID(id)) {
      ...
      if (record->id == id) {
        ...
        if (NULL != eventDump) {
          count =
              VM_AtomicSupport::add((volatile uintptr_t *)&eventDump->count, 1);
          sampling =
              (1 >= samplingInterval) ||
              ((100 >= samplingInterval) && (0 == (count % samplingInterval)));
        } else {
          sampling = false;
        }
        ...
        if (sampling) {
          uint64_t timeDelta =
              omrtime_hires_delta(startTime, omrtime_usec_clock(),
          ...
          VM_AtomicSupport::add((volatile uintptr_t *)&eventDump->totalTime,
                                (uintptr_t)timeDelta);
          ...
          if (commonInterface->threshold4Trace <= timeDelta) {
            ...
            if (NULL != record->callsite) {
              ...
              /* if the callsite info can not be retrieved, use callback
...
static intptr_t J9HookDisable(struct J9HookInterface **hookInterface,
  ...
  if (HOOK_FLAGS(commonInterface, eventNum) & J9HOOK_FLAG_RESERVED) {
    ...
    if (HOOK_FLAGS(commonInterface, eventNum) &
        (J9HOOK_FLAG_RESERVED | J9HOOK_FLAG_HOOKED)) {
      rc = -1;
    } else {
 ...
 * Certain events must be reserved before the providing module initializes
 * completely, or it may make decisions which make it impossible to add the
 * hooks later. Once the point of no-return is reached, the providing module
 * will disable the event if it has not been registered or reserved.
 *
 * This function should not be called directly. It should be called through the
 * hook interface
 *
...
static intptr_t
J9HookRegisterWithCallSitePrivate(struct J9HookInterface **hookInterface,
                                  uintptr_t taggedEventNum,
                                  J9HookFunction function, const char *callsite,
                                  void *userData, uintptr_t agentID) {
  J9CommonHookInterface *commonInterface =
      (J9CommonHookInterface *)hookInterface;
  ...
  if (HOOK_FLAGS(commonInterface, eventNum) & J9HOOK_FLAG_DISABLED) {
    ...
    J9HookRecord *record = HOOK_RECORD(commonInterface, eventNum);
    /* at the end of the loop, insertionPoint will point to the last record
     * which should be triggered before the one we're adding */
    while (record) {
      if ((taggedEventNum & J9HOOK_TAG_REVERSE_ORDER)
      ...
      } else if ((record->function == function) &&
    ...
    /*
     * Re-use the empty record if it is in a legitimate position for the
     * requested agent. (It is a legitimate position if all records before this
     * position have the same or lower agent IDs (tested previously) and all
     * records after this position have the same or higher IDs)
     */
    if ((emptyRecord != NULL) &&
  ...
  (*hookInterface)
      ->J9HookDispatch(hookInterface, J9HOOK_REGISTRATION_EVENT, &eventStruct);
 ...
 *
 * If a listener with the same function and userData is already registered, no
 * action is taken, and 0 is returned (indicating success). Registrations are
 * NOT counted. Even if a listener is registered twice, it will be removed the
 * first time it is unregistered.
 *
 * If the J9HOOK_TAG_AGENT_ID is set in taggedEventNum, the first var-args
 * argument must be a uintptr_t agent ID. If the bit is not set, the listener
 * will be added to the J9HOOK_AGENTID_DEFAULT agent. The use of an agent ID
 * allows more control over the order in which events are reported. Listeners
 * with lower agent IDs will always receive events before listeners with higher
 * agent IDs. The special J9HOOK_AGENT_FIRST and J9HOOK_AGENT_LAST IDs may be
 * used to register listeners which will be among the first or last to receive
 * an event.
 *
 * This function should not be called directly. It should be called through the
 * hook interface
...
static intptr_t J9HookRegister(struct J9HookInterface **hookInterface,
  ...
  return J9HookRegisterWithCallSitePrivate(hookInterface, taggedEventNum,
                                           function, NULL, userData, agentID);
}
...
J9HookRegisterWithCallSite(struct J9HookInterface **hookInterface,
  ...
  return J9HookRegisterWithCallSitePrivate(
      hookInterface, taggedEventNum, function, callsite, userData, agentID);
}
...
static void J9HookUnregister(struct J9HookInterface **hookInterface,
  ...
  while (record) {
    if (record->function == function) {
      if ((userData == NULL) || (record->userData == userData)) {
        ...
        if (userData != NULL) {
          /* copy data from the event record so that we can report it once we've
           * released the mutex */
  ...
  if (hooksRemoved != 0) {
    ...
    (*hookInterface)
        ->J9HookDispatch(hookInterface, J9HOOK_REGISTRATION_EVENT,
                         &eventStruct);
  }
}

/**
 * Determine if the specified event has been disabled. This function does not
 * modify the hook interface in any way. Note that there's no atomic protection
 * of any kind -- just because the event wasn't disabled when you called this
 * function doesn't mean that it won't be by the time you try to register for
 * it.
 *
 * @return 0 (false) if the event is disabled, non-zero (true) if the event is
 * not disabled.
 */
static intptr_t J9HookIsEnabled(struct J9HookInterface **hookInterface,
                                uintptr_t taggedEventNum) {
