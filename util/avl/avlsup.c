...
static J9AVLTreeNode *deleteNode(J9AVLTree *tree, J9AVLTreeNode **walkPtr,
                                 J9WSRP *walkSRPPtr, J9AVLTreeNode *node,
                                 intptr_t *heightChange);
static J9AVLTreeNode *rotate(J9AVLTree *tree, J9AVLTreeNode *walk,
                             intptr_t direction, intptr_t *heightChange);
static J9AVLTreeNode *doubleRotate(J9AVLTree *tree, J9AVLTreeNode *walk,
                                   intptr_t direction, intptr_t *heightChange);
static void rebalance(J9AVLTree *tree, J9AVLTreeNode **walkPtr,
                      J9WSRP *walkSRPPtr, intptr_t direction,
                      intptr_t *heightChange);
static J9AVLTreeNode *findRightMostLeaf(J9AVLTree *tree, J9WSRP *walkSRPPtr,
                                        intptr_t *heightChange);
static J9AVLTreeNode *findNode(J9AVLTree *tree, J9AVLTreeNode *walk,
                               uintptr_t search);
static J9AVLTreeNode *insertNode(J9AVLTree *tree, J9AVLTreeNode **walkPtr,
                                 J9WSRP *walkSRPPtr, J9AVLTreeNode *node,
                                 intptr_t *heightChange);
  ...
  if (!find) {
    ...
    *heightChange = -1;
    if (tree->genericActionHook) {
      tree->genericActionHook(tree, walk,
                              J9AVLTREE_ACTION_REPLACE_REMOVED_PARENT);
    }
  } else {
 ...
 * Direction specifies the direction to rotate (ie: the direction away from the
 * heavy node)
 *
 * @param[in] tree  The tree
...
static J9AVLTreeNode *rotate(J9AVLTree *tree, J9AVLTreeNode *walk,
  ...
  if (tree->genericActionHook) {
    tree->genericActionHook(tree, (J9AVLTreeNode *)walk,
                            J9AVLTREE_ACTION_SINGLE_ROTATE);
  }
...
static J9AVLTreeNode *doubleRotate(J9AVLTree *tree, J9AVLTreeNode *walk,
  ...
  if (tree->genericActionHook) {
    tree->genericActionHook(tree, (J9AVLTreeNode *)walk,
                            J9AVLTREE_ACTION_DOUBLE_ROTATE);
  }
 ...
 * Use walkPtr if you have a direct pointer to the node or walkSRPPtr if you
 * have a pointer to a WSRP
 *
 * @param[in] tree  The tree
 * @param[in] walkPtr  A pointer to the root of the tree to balance (walkSRPPtr
 * is NULL)
 * @param[in] walkSRPPtr  A pointer to an SRP of the root of the tree to balance
 * (walkPtr is NULL)
 * @param[in|out] heightChange  The height change
 */
static void rebalance(J9AVLTree *tree, J9AVLTreeNode **walkPtr,
  ...
  /* if we shrunk nodes, flip the direction, thus direction points towards heavy
   * direction */
  if (*heightChange < 0) {
    direction = -direction;
  }
  ...
  if (walkBalance == AVL_BALANCED) {
    ...
    /* if the tree was shrinking, we've just unbalanced a node .. no longer
     * shrinking */
    if (*heightChange < 0) {
      *heightChange = 0;
    }
    ...
    if ((direction < 0) && (AVL_GETBALANCE(AVL_NNSRP_GETNODE(
                                walk->leftChild)) == AVL_RIGHTHEAVY)) {
      /* doubleRotate */
      rotateResult =
          (J9AVLTreeNode *)doubleRotate(tree, walk, -direction, heightChange);
    } else if ((direction > 0) && (AVL_GETBALANCE(AVL_NNSRP_GETNODE(
                                       walk->rightChild)) == AVL_LEFTHEAVY)) {
      /* doubleRotate */
      rotateResult =
          (J9AVLTreeNode *)doubleRotate(tree, walk, -direction, heightChange);
    } else {
      /* single rotate */
      rotateResult =
          (J9AVLTreeNode *)rotate(tree, walk, -direction, heightChange);
    }
 ...
 *
 * IMPORTANT: It is the responsibility of the caller to ensure that the nodes
 * are aligned
 *
 * @param[in] tree  The tree
 * @param[in] walkPtr  A pointer to the root of the tree
 * @param[in] walkSRPPtr  Should always be NULL (only used when the function
 * calls itself)
 ...
 */
static J9AVLTreeNode *findNode(J9AVLTree *tree, J9AVLTreeNode *node,
                               uintptr_t search) {
