...
extern "C" {
#endif /* __cplusplus */
/* CMVC 162573 include the header file that includes the prototype for malloc
 * used in functions/macros below*/
#include <stdlib.h>

#pragma map(sysTranslateASM, "SYSXLATE")
extern char *sysTranslateASM(const char *source, int length, char *trtable,
                             char *xlate_buf);
extern char *sysTranslate(const char *source, int length, char *trtable,
                          char *xlate_buf);
...
#define a2e(str, len)                                                          \
  sysTranslate(str, abs(len), a2e_tab,                                         \
               (char *)malloc(abs(len) + 1)) /*ibm@41269*/
#define e2a(str, len)                                                          \
  sysTranslate(str, abs(len), e2a_tab,                                         \
               (char *)malloc(abs(len) + 1)) /*ibm@41269*/

#define a2e_string(str)                                                        \
  sysTranslate(str, strlen(str), a2e_tab,                                      \
               (char *)malloc(strlen(str) + 1)) /*ibm@41269*/
#define e2a_string(str)                                                        \
  sysTranslate(str, strlen(str), e2a_tab,                                      \
               (char *)malloc(strlen(str) + 1)) /*ibm@41269*/
