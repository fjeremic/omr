 ...
 *************************************************************************/
struct hostent*
atoe_gethostbyname_r(char* hostname,
 ...
 *************************************************************************/
struct hostent*
atoe_gethostbyaddr(const void* address,
                   int addrlen,
                   int domain,
 ...
 *************************************************************************/
int
atoe_getaddrinfo(const char* nodename,
                 const char* servname,
 ...
 *************************************************************************/
int
atoe_getnameinfo(const struct sockaddr* sa,
                 socklen_t salen,
                 char* host,
                 socklen_t hostlen,
                 char* serv,
 ...
 *************************************************************************/
pid_t
atoe_spawnpe(const char* filename,
             const int fd_cnt,
             const int* fd_map,
...
pid_t
atoe_spawnp(const char* filename,
            const int fd_cnt,
            const int* fd_map,
            const struct inheritance* inherit,
            char** argv,
            const char** envp)
{
  ...
  for (i = 0; argv[i] != NULL && i < _POSIX_ARG_MAX;
    ...
    if (e_argv[i] == NULL) {                   /*ibm@52465 ...*/
      atoe_fprintf(
        stderr, "atoe_spawnp insufficient memory for command argument %d\n", i);
      validArgs = 0;
      break;
    } /*... ibm@52465*/
  }
  ...
  if (validArgs) { /*ibm@52465*/
    pid = spawnp((const char*)e_filename,
                 fd_cnt,
                 fd_map,
                 inherit, /*ibm.7677*/
                 (const char**)e_argv,
