...
char *sysTranslate(const char *source, int length, char *trtable,
                   char *xlate_buf) {
 ...
 *************************************************************************/
static envvar_t *updateEnvvar(char *name, char *value) {
  envvar_t *prev;
  envvar_t *envvar;
 ...
 *************************************************************************/
char *a2e_func(char *str, int len) { return a2e(str, len); }
 ...
 *************************************************************************/
char *e2a_func(char *str, int len) { return e2a(str, len); }
 ...
 * returns     -
 *************************************************************************/
char *atoe_getenv(const char *name) {
  envvar_t *envvar;
 ...
 * returns     -
 *************************************************************************/
int atoe_putenv(const char *envstr) {
  char *w_envstr;
  int result;
  ...
  /* note that this causes a leak - z/OS needs to have the memory, so we copy
   * here. */
  result = putenv(w_envstr);

  /*
   * Note: We don't add / update the entry in the envvar_t list, as that list is
   * only to keep track of memory we allocated for getenv(), and is not used for
   * lookups.
   */
 ...
 * returns     -
 *************************************************************************/
void atoe_perror(const char *string) {
  char *e;
 ...
 *               0 otherwise
 *************************************************************************/
static int fileTagRequired(const char *filename) {
  struct stat sbuf;
 ...
 * returns     -
 *************************************************************************/
int atoe_open(const char *fname, int options, ...) {
  int fd;
  int mode;
  int tagFile;
  va_list args;
  ...
  /* Convert filename to ebcdic before calling fileTagRequired() - CMVC
   * 191919/199888 */
 ...
 * returns     -
 *************************************************************************/
char *atoe_tempnam(const char *dir, char *pfx) {
  char *tempfn = 0;
  char *a = 0;
 ...
 * returns     -
 *************************************************************************/
int atoe_stat(const char *pathname, struct stat *sbuf) {
  int rc;
  char *e;
 ...
 * returns     -
 *************************************************************************/
int atoe_statvfs(const char *pathname, struct statvfs *buf) {
  int rc;
  char *e;
 ...
 * returns     -
 *************************************************************************/
FILE *atoe_fopen(const char *filename, char *mode) {
  FILE *outfp;
  char *f, *m;
 ...
 * returns     -
 *************************************************************************/
FILE *atoe_freopen(const char *filename, char *mode, FILE *stream) {
  FILE *outfp;
  char *f, *m;
  int fd = -1; /* file descriptor */ /*ibm@57265*/
 ...
 * returns     -
 *************************************************************************/
int atoe_mkdir(const char *pathname, mode_t mode) {
  int rc;
  char *e;
 ...
 * returns     -
 *************************************************************************/
int atoe_remove(const char *pathname) {
  int rc;
  char *e;
...
char *atoe_strerror(int errnum) {
  ...
  for (index = 0; index < ERRNO_CACHE_SIZE, errno_cache[index].errnum != -1;
       index++) {
 ...
 * returns     -
 *************************************************************************/
char *atoe_getcwd(char *buffer, size_t size) {
  char *abuf;
 ...
 * returns     -
 *************************************************************************/
char *atoe_fgets(char *buffer, int num, FILE *file) {
  char *abuf;
 ...
 * returns     -
 *************************************************************************/
int atoe_unlink(const char *pathname) {
  int rc;
  char *e;
 ...
 * returns     -
 *************************************************************************/
int atoe_access(const char *pathname, int how) {
  int rc;
  char *e;
 ...
 * returns     -
 *************************************************************************/
DIR *atoe_opendir(const char *dirname) {
  DIR *dir;
  char *e;
 ...
 * returns     -
 *************************************************************************/
char *atoe_realpath(const char *file_name, char *resolved_name) {
 ...
 * returns     -
 *************************************************************************/
int atoe_rename(const char *oldname, const char *newname) {
  int rc;
  char *o, *n;
 ...
 * returns     -
 *************************************************************************/
struct passwd *atoe_getpwuid(uid_t uid) {
 ...
 * returns     -
 *************************************************************************/
struct passwd *atoe_getpwnam(const char *name) {
 ...
 * returns     -
 *************************************************************************/
struct group *e2a_group(struct group *e_group) {
    ...
    if (a_group != NULL) {
      ...
      if (a_group->gr_mem != NULL) {
        for (e_member = e_group->gr_mem, a_member = a_group->gr_mem;
             *e_member != NULL; e_member++, a_member++) {
 ...
 * returns     -
 *************************************************************************/
struct group *atoe_getgrgid(gid_t gid) {
 ...
 * returns     -
 *************************************************************************/
int atoe_fprintf(FILE *file, const char *ascii_chars, ...) {
  va_list args;
  char buf[BUFLEN];
  char *ebuf;
  int len;
 ...
 * returns     -
 *************************************************************************/
int atoe_printf(const char *ascii_chars, ...) {
  va_list args;
  char buf[BUFLEN];
  char *ebuf;
  int len;
 ...
 * returns     -
 *************************************************************************/
int std_sprintf(const char *buf, char *ascii_chars, ...) {
  int len;
  va_list args;
  va_start(args, ascii_chars);
 ...
 * returns     -
 *************************************************************************/
int atoe_sprintf(char *buf, char *ascii_chars, ...) {
  int len;
  char wrkbuf[BUFLEN];
 ...
 * returns     -
 *************************************************************************/
int atoe_vprintf(const char *ascii_chars, va_list args) {
 ...
 * returns     -
 *************************************************************************/
int atoe_vfprintf(FILE *file, const char *ascii_chars, va_list args) {
  char buf[BUFLEN];
  char *ebuf;
  int len;
 ...
 * returns     -
 *************************************************************************/
int atoe_vsprintf(char *target, const char *ascii_chars, va_list args) {
 ...
 *************************************************************************/
int atoe_sscanf(const char *buffer, const char *format, va_list args) {
 ...
 * returns     -
 *************************************************************************/
size_t atoe_strftime(char *buf, size_t buflen, const char *format,
 ...
 * returns     -
 *************************************************************************/
size_t atoe_fwrite(const void *buffer, size_t size, size_t count,
                   FILE *stream) {
  int numwritten;
  char *e;
 ...
 * returns     -
 *************************************************************************/
size_t atoe_fread(void *buffer, size_t size, size_t count, FILE *stream) {
  int numread;
  char *a;
 ...
 * returns     -
 *************************************************************************/
char *atoe_setlocale(int category, const char *locale) {
  char *eb, *result;
 ...
 * returns     -
 *************************************************************************/
char *atoe_ctime(const time_t *t_in) {
  char *eb;
 ...
 * returns     -
 *************************************************************************/
int atoe_strcasecmp(const char *string1, const char *string2) {
  int return_val;
 ...
 * returns     -
 *************************************************************************/
int atoe_strncasecmp(const char *string1, const char *string2, int n) {
  int return_val;
  char *ebsstr1, *ebsstr2;
 ...
 * returns     -
 *************************************************************************/
double atoe_atof(char *ascii_string) {
  double rc;
  char *ebcdic_string;
 ...
 * returns     -
 *************************************************************************/
int atoe_atoi(char *ascii_string) {
  int rc;
  char *ebcdic_string;
 ...
 * returns     -
 *************************************************************************/
long atoe_atol(char *ascii_string) {
  long rc;
  char *ebcdic_string;
 ...
 * returns     -
 *************************************************************************/
int atoe_gethostname(char *name, int namelen) {
...
struct hostent *atoe_gethostbyname(const char *hostname) {
  char *h;
 ...
 * returns     -
 *************************************************************************/
struct hostent *atoe_gethostbyname_r(char *hostname,
                                     struct hostent *hentptr, /*ibm@38180.1*/
 ...
 * returns     -
 *************************************************************************/
struct hostent *atoe_gethostbyaddr(const void *address, int addrlen, int domain,
                                   struct hostent *hentptr, char *buf,
                                   int bufsize, int *h_error) {
  char *h_n;
  struct hostent *hostptr;

  Log(1, "Entry: atoe_gethostbyaddr:\n");

  if ((hostptr = gethostbyaddr(address, addrlen, domain)) ==
      NULL) {                                           /*ibm.7612*/
    hentptr = NULL;                                     /*ibm.7612*/
  } else if ((strlen(hostptr->h_name) + 1) > bufsize) { /*ibm.7612*/
    *h_error = errno = ERANGE;                          /*ibm.7612*/
 ...
 * returns     -
 *************************************************************************/
unsigned long atoe_inet_addr(char *hostname) {
 ...
 * returns     -
 *************************************************************************/
int atoe_getaddrinfo(const char *nodename, const char *servname,
 ...
 * returns     -
 *************************************************************************/
int atoe_getnameinfo(const struct sockaddr *sa, socklen_t salen, char *host,
...
int iconv_init(void) {
   ...
   */
  {
    ...
    if ((__atoe(a) < 1) && (errno == EINVAL)) {
      ...
      /* This test does not always give the correct result - see below
       * ibm@12464*/
...
#if 0 /* msf - the encoding_names code can not be pulled into here - need to \
         write this yourself if needed */
 ...
 * returns     -
 *************************************************************************/
dllhandle *atoe_dllload(char *dllName) {
  dllhandle *handle;
 ...
 * returns     -
 *************************************************************************/
pid_t atoe_spawnpe(const char *filename, const int fd_cnt, const int *fd_map,
                   const struct inheritance *inherit, char **argv, char **envp,
                   int envlen) {
  char *e_envp[1025];
  int i, j;
  pid_t pid;
...
pid_t atoe_spawnp(const char *filename, const int fd_cnt, const int *fd_map,
  ...
  for (i = 0; argv[i] != NULL && i < _POSIX_ARG_MAX;
       i++) { /*ibm.7677*/                     /*ibm@52465*/
 ...
 * returns     -
 *************************************************************************/
int etoa_uname(struct utsname *a) {
  struct utsname e;
  int rc = 0;
 ...
 * returns     -
 *************************************************************************/
char *etoa_nl_langinfo(int nl_item) {
  char *retVal = NULL;
 ...
 * returns     -
 *************************************************************************/
int etoa___osname(struct utsname *a) {
  struct utsname e;
  int rc = 0;
