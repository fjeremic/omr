...
/* HOLE_FREQUENCY defines how often a hole appears - there is a hole every HOLE_FREQUENCY elements. Must be power of
 * two. */
 ...
 */
static int32_t pool_getElementPuddleSlot(J9Pool* pool, J9PoolPuddle* puddle, void* element)
{
 ...
 *
 */
J9Pool* pool_new(uintptr_t structSizeArg, uintptr_t numberElementsArg, uintptr_t elementAlignmentArg,
  uintptr_t poolFlags, const char* poolCreatorCallsite, uint32_t memoryCategory, omrmemAlloc_fptr_t memAlloc,
  omrmemFree_fptr_t memFree, void* userData)
{
  ...
  do {
    ...
    puddleHeaderAllocSize
      = ROUND_TO(elementAlignment, sizeof(J9PoolPuddle) + puddleBitsSize) + (firstElementAlignment - MALLOC_ALIGNMENT);
    if (poolFlags & POOL_USES_HOLES) {
  ...
  if (NULL != pool) {
    ...
    doInit = 1;
    puddleList = memAlloc(
      userData, sizeof(J9PoolPuddleList), poolCreatorCallsite, memoryCategory, POOL_ALLOC_TYPE_PUDDLE_LIST, &doInit);
 ...
 *
 */
void pool_do(J9Pool* pool, void (*doFunction)(void* anElement, void* userData), void* userData)
{
  void* anElement;
  pool_state aState;
 ...
 *
 */
void* poolPuddle_startDo(J9Pool* pool, J9PoolPuddle* currentPuddle, pool_state* state, uintptr_t followNextPointers)
{
  int32_t slot = 0;
  uintptr_t* currAddr;
 ...
 *
 * @note Make no assumptions about the contents of the pool after invoking this method (it currently does not zero the
 * memory)
 *
