...
#define COMPUTE_FIRST_ELEMENT(align, puddle, bitlength)                        \
  (ROUND_TO((align),                                                           \
            ((uintptr_t)(puddle)) + sizeof(J9PoolPuddle) +                     \
...
pool_getElementPuddleSRP(J9Pool* pool, void* element)
{
  ...
  if (pool->flags & POOL_USES_HOLES) {
     ...
     * masked off. */
    puddleSRP =
      (J9SRP*)((uintptr_t)element & ~(pool->elementSize * HOLE_FREQUENCY - 1));
  } else {
    puddleSRP =
...
poolPuddle_new(J9Pool* pool)
{
  ...
  puddle = pool->memAlloc(pool->userData,
                          (uint32_t)pool->puddleAllocSize,
                          pool->poolCreatorCallsite,
                          pool->memoryCategory,
                          POOL_ALLOC_TYPE_PUDDLE,
                          &doInit);
         ...
         void* userData)
{
  ...
  if (((uintptr_t)structSize != structSizeArg) ||
    ...
    Trc_pool_new_ArgumentTooLargeExit(
      structSizeArg, numberElementsArg, elementAlignmentArg);
    return NULL;
  }
  ...
  pool = memAlloc(userData,
                  sizeof(J9Pool),
                  poolCreatorCallsite,
                  memoryCategory,
                  POOL_ALLOC_TYPE_POOL,
                  &doInit);
  ...
  if (NULL != pool) {
    ...
    doInit = 1;
    puddleList = memAlloc(userData,
                          sizeof(J9PoolPuddleList),
                          poolCreatorCallsite,
                          memoryCategory,
                          POOL_ALLOC_TYPE_PUDDLE_LIST,
                          &doInit);
 ...
 */
void*
poolPuddle_startDo(J9Pool* pool,
                   ...
                   uintptr_t followNextPointers)
{
  ...
  if (0 == currentPuddle->usedElements) { /* this puddle is empty */
    ...
    if ((currentPuddle->nextPuddle != 0) && (followNextPointers != 0)) {
      return poolPuddle_startDo(pool,
                                J9POOLPUDDLE_NEXTPUDDLE(currentPuddle),
                                state,
                                followNextPointers);
    } else {
...
pool_startDo(J9Pool* pool, pool_state* state)
{
  ...
  if (pool) {
    J9PoolPuddleList* puddleList = J9POOL_PUDDLELIST(pool);
    result = poolPuddle_startDo(
      pool, J9POOLPUDDLELIST_NEXTPUDDLE(puddleList), state, TRUE);
  }
...
pool_nextDo(pool_state* state)
{
  ...
  if (state->leftToDo == 0) { /* no more used elements, stop this pool. */
    ...
    if (state->currentPuddle) { /* check the next one. */
      ...
      return poolPuddle_startDo(
        state->thePool, state->currentPuddle, state, TRUE);
    } else {
