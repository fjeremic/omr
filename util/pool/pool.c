...
#define ROUND_TO(granularity, number)                             \
  ((((number) % (granularity))                                    \
        ? ((number) + (granularity) - ((number) % (granularity))) \
        : (number)))
...
#define PUDDLE_SLOT_FREE(puddle, sindex)                  \
  (*(PUDDLE_BITS(puddle) + (((uint32_t)(sindex)) >> 5)) & \
   (1 << (31 - (((uint32_t)(sindex)) & 31))))
...
#define COMPUTE_FIRST_ELEMENT(align, puddle, bitlength)             \
  (ROUND_TO((align), ((uintptr_t)(puddle)) + sizeof(J9PoolPuddle) + \
                         ((bitlength) * sizeof(uint32_t))))

/* HOLE_FREQUENCY defines how often a hole appears - there is a hole every
 * HOLE_FREQUENCY elements. Must be power of two. */
#define HOLE_FREQUENCY 16
#define ELEMENT_IS_HOLE(pool, element)  \
  (((pool)->flags & POOL_USES_HOLES) && \
   ((uintptr_t)(element) % ((pool)->elementSize * HOLE_FREQUENCY) == 0))

/**
 * Get a pointer to the SRP to the puddle, given a puddle element.
 *
 * @param[in] pool    The pool containing the element.
 * @param[in] element The valid element for which the puddle SRP pointer is
 * being queried.
 *
 * @return A pointer to the SRP to the puddle containing the specified element.
 */
static J9SRP *pool_getElementPuddleSRP(J9Pool *pool, void *element) {
  J9SRP *puddleSRP;

  if (pool->flags & POOL_USES_HOLES) {
    /* (pool->elementSize*HOLE_FREQUENCY - 1) evaluates to the bits that must be
     * masked off. */
    puddleSRP = (J9SRP *)((uintptr_t)element &
                          ~(pool->elementSize * HOLE_FREQUENCY - 1));
  } else {
    puddleSRP =
        (J9SRP *)((uintptr_t)element + pool->elementSize - sizeof(J9SRP));
  }
...
/**
 * Get the slot (index) of the specified element in the specified puddle. It is
 * not necessary for the element to be currently allocated (ie: it could be a
 * free slot).
 *
 ...
 *
 * @return The element's index in the puddle, or -1 if the element is not in the
 * puddle.
 */
static int32_t pool_getElementPuddleSlot(J9Pool *pool, J9PoolPuddle *puddle,
                                         void *element) {
  int32_t returnValue = -1;
  uintptr_t firstElementAddress =
      (uintptr_t)J9POOLPUDDLE_FIRSTELEMENTADDRESS(puddle);
 ...
 * @return none
 */
static void poolPuddle_init(J9Pool *pool, J9PoolPuddle *puddle) {
  ...
  NNSRP_SET(puddle->firstElementAddress,
            COMPUTE_FIRST_ELEMENT(firstElementAlignment, puddle, bitlength));
  ...
  if (pool->flags & POOL_USES_HOLES) {
    freeLocation =
        (uintptr_t *)((uintptr_t)J9POOLPUDDLE_FIRSTELEMENTADDRESS(puddle) +
                      pool->elementSize);
  } else {
  ...
  lastLocation =
      (uintptr_t *)((uintptr_t)freeLocation +
                    pool->elementSize * (pool->elementsPerPuddle - 1));
 ...
 *
 * This does not link the puddle into the pool, but simply allocates and
 * initializes it.
 *
 * @param[in] pool  The pool containing the parameters to use for the puddle.
 *
 ...
 */
J9PoolPuddle *poolPuddle_new(J9Pool *pool) {
  J9PoolPuddle *puddle;
  uint32_t doInit = 1;

  Trc_poolPuddle_new_Entry(pool);

  puddle = pool->memAlloc(pool->userData, (uint32_t)pool->puddleAllocSize,
                          pool->poolCreatorCallsite, pool->memoryCategory,
                          POOL_ALLOC_TYPE_PUDDLE, &doInit);
 ...
 * @return none
 */
static void poolPuddle_delete(J9Pool *pool, J9PoolPuddle *puddle) {
 ...
 *
 */
J9Pool *pool_new(uintptr_t structSizeArg, uintptr_t numberElementsArg,
                 uintptr_t elementAlignmentArg, uintptr_t poolFlags,
                 const char *poolCreatorCallsite, uint32_t memoryCategory,
                 omrmemAlloc_fptr_t memAlloc, omrmemFree_fptr_t memFree,
                 void *userData) {
  uint32_t doInit;
  uint64_t tempAllocSize, puddleAllocSize;
  uint32_t finalNumberOfElements, minNumberElements;
  uint32_t roundedStructSize, puddleHeaderAllocSize, puddleBitsSize,
      newPuddleBitsSize;
  J9Pool *pool;
  ...
  Trc_pool_new_Entry(structSize, numberElements, elementAlignment, poolFlags,
                     memAlloc, memFree, userData);

  if (((uintptr_t)structSize != structSizeArg) ||
      ((uintptr_t)numberElements != numberElementsArg) ||
      ((uintptr_t)elementAlignment != elementAlignmentArg)) {
    /* one of the uintptr_t arguments is too large to fit in a uint32_t. Don't
     * even try to allocate this pool. */
    Trc_pool_new_ArgumentTooLargeExit(structSizeArg, numberElementsArg,
                                      elementAlignmentArg);
    return NULL;
  }
  ...
  switch (roundedStructSize) {
    ...
    case 16:
       ...
       *  3. element alignment is divisible by MIN_GRANULARITY (which is a power
       * of 2)
       */
  ...
  if (poolFlags & POOL_USES_HOLES) {
    /* Note: roundedStructSize must be a power of 2, so that
     * firstElementAlignment will be also. */
    firstElementAlignment = roundedStructSize * HOLE_FREQUENCY;
  } else {
    /* Round up the ( structSize + sizeof(puddle SRP) ) to the closest value
     * divisible by elementAlignment. */
    if ((roundedStructSize - structSize) < sizeof(J9SRP)) {
      roundedStructSize =
          ROUND_TO(elementAlignment, structSize + sizeof(J9SRP));
    }
   ...
   * +--------------+-------------+--------------------+
   * +-------------+------------+-------------+------------+-----+ |
   * J9PoolPuddle | puddle bits | alignment overhead |   | 1st element | puddle
   * SRP | 2nd element | puddle SRP | ... |
   * +--------------+-------------+--------------------+
   * +-------------+------------+-------------+------------+-----+
   *
   * Note: With POOL_USES_HOLES, puddleSRPs are instead stored once in every
   * HOLE_FREQUENCY slots.
   */
  newPuddleBitsSize = ((minNumberElements + 31) >> 3);
  do {
    puddleBitsSize = newPuddleBitsSize;
    puddleHeaderAllocSize =
        ROUND_TO(elementAlignment, sizeof(J9PoolPuddle) + puddleBitsSize) +
        (firstElementAlignment - MALLOC_ALIGNMENT);
    if (poolFlags & POOL_USES_HOLES) {
      /* Every sector has HOLE_FREQUENCY (16) slots (one of which is the
       * "hole"). */
      uint32_t sectorSize = roundedStructSize * HOLE_FREQUENCY;
      uint32_t numberOfSectors =
          (minNumberElements + HOLE_FREQUENCY - 2) / (HOLE_FREQUENCY - 1);
      tempAllocSize = sectorSize * numberOfSectors + puddleHeaderAllocSize;
      puddleAllocSize = (poolFlags & POOL_ROUND_TO_PAGE_SIZE)
                            ? ROUND_TO(OS_PAGE_SIZE, tempAllocSize)
                            : tempAllocSize;
      numberOfSectors +=
          (uint32_t)((puddleAllocSize - tempAllocSize) / sectorSize);
      finalNumberOfElements = numberOfSectors * HOLE_FREQUENCY;
    } else {
      tempAllocSize =
          roundedStructSize * minNumberElements + puddleHeaderAllocSize;
      puddleAllocSize = (poolFlags & POOL_ROUND_TO_PAGE_SIZE)
                            ? ROUND_TO(OS_PAGE_SIZE, tempAllocSize)
                            : tempAllocSize;
      finalNumberOfElements = minNumberElements;
      finalNumberOfElements +=
          (uint32_t)((puddleAllocSize - tempAllocSize) / roundedStructSize);
    }
    newPuddleBitsSize = ((finalNumberOfElements + 31) >> 3);
  } while (newPuddleBitsSize != puddleBitsSize);

  /*
   * puddleAllocSize is a uint64_t so that we can detect pool sizes which
   * overflow 32-bits. See CMVC defects 84003 and 90099
   */
  ...
  pool = memAlloc(userData, sizeof(J9Pool), poolCreatorCallsite, memoryCategory,
                  POOL_ALLOC_TYPE_POOL, &doInit);
  ...
  if (NULL != pool) {
    ...
    pool->alignment =
        (uint16_t)elementAlignment; /* we assume no alignment is > 64k */
    ...
    doInit = 1;
    puddleList =
        memAlloc(userData, sizeof(J9PoolPuddleList), poolCreatorCallsite,
                 memoryCategory, POOL_ALLOC_TYPE_PUDDLE_LIST, &doInit);
 ...
 *
 */
void *pool_newElement(J9Pool *pool) {
  int32_t slot;
  void *newElement;
  ...
  if (NULL == puddle) {
    ...
    /* And point to it as the next (and the only, right now) available puddle on
     * the pool. */
 ...
 *
 */
void pool_removeElement(J9Pool *pool, void *anElement) {
  J9SRP *puddleSRP;
  int32_t slot;
  J9PoolPuddle *puddle;
  J9PoolPuddleList *puddleList;
  ...
  if (slot < 0) {
    Trc_pool_removeElement_NotFound(anElement,
                                    J9POOLPUDDLELIST_NEXTPUDDLE(puddleList));
  ...
  if ((puddle->usedElements == 0) && !(pool->flags & POOL_NEVER_FREE_PUDDLES)) {
    ...
    /* It was full before - but not anymore - add it to the top of the available
     * puddles list. */
 ...
 * @param[in] doFunction Pointer to function which will "do" things to the
 *elements of pool
 * @param[in] userData Pointer to data to be passed to "do" function, along with
 *each pool-element
 *
 * @return none
 *
 ...
 *
 */
void pool_do(J9Pool *pool, void (*doFunction)(void *anElement, void *userData),
             void *userData) {
  void *anElement;
  pool_state aState;
 ...
 *
 */
uintptr_t pool_numElements(J9Pool *pool) {
 ...
 *	This is different to pool_startDo because it allows a specific puddle to
 *be specified as the start point and also provides the option to only walk that
 *puddle. Pass in a pointer to an empty pool_state and it will be filled in.
 *
 * @param[in] pool The pool to "do" things to
 ...
 *
 */
void *poolPuddle_startDo(J9Pool *pool, J9PoolPuddle *currentPuddle,
                         pool_state *state, uintptr_t followNextPointers) {
  int32_t slot = 0;
  uintptr_t *currAddr;
  ...
  if (0 == currentPuddle->usedElements) { /* this puddle is empty */
    ...
    if ((currentPuddle->nextPuddle != 0) && (followNextPointers != 0)) {
      return poolPuddle_startDo(pool, J9POOLPUDDLE_NEXTPUDDLE(currentPuddle),
                                state, followNextPointers);
    } else {
  ...
  currAddr =
      (uintptr_t *)((uintptr_t)J9POOLPUDDLE_FIRSTELEMENTADDRESS(currentPuddle) +
                    pool->elementSize * slot);
  ...
  if (state->leftToDo == 0) {
    /* Pre-fetch the next puddle in case this element is deleted and the puddle
     * is freed. */
    if (followNextPointers) {
      state->currentPuddle = J9POOLPUDDLE_NEXTPUDDLE(state->currentPuddle);
      state->lastSlot =
          -1; /* In order to start at slot 0 on the next puddle. */
    } else {
 ...
 *
 */
void *pool_startDo(J9Pool *pool, pool_state *state) {
  void *result = NULL;
  ...
  if (pool) {
    J9PoolPuddleList *puddleList = J9POOL_PUDDLELIST(pool);
    result = poolPuddle_startDo(pool, J9POOLPUDDLELIST_NEXTPUDDLE(puddleList),
                                state, TRUE);
  }
 ...
 *
 */
void *pool_nextDo(pool_state *state) {
  ...
  if (state->leftToDo == 0) { /* no more used elements, stop this pool. */
    /* if leftToDo is 0, then currentPuddle will already have been advanced to
     * point to the next puddle */
    if (state->currentPuddle) { /* check the next one. */
      Trc_pool_nextDo_NextPuddle();
      return poolPuddle_startDo(state->thePool, state->currentPuddle, state,
                                TRUE);
    } else {
  ...
  currAddr = (uintptr_t *)((uintptr_t)J9POOLPUDDLE_FIRSTELEMENTADDRESS(
                               state->currentPuddle) +
  ...
  if (state->leftToDo == 0) {
    if (state->flags & POOLSTATE_FOLLOW_NEXT_POINTERS) {
      /* Pre-fetch the next puddle in case this element is deleted and the
       * puddle is freed. */
      state->currentPuddle = J9POOLPUDDLE_NEXTPUDDLE(state->currentPuddle);
      state->lastSlot =
          -1; /* In order to start at slot 0 on the next puddle. */
 ...
 *
 * @note Make no assumptions about the contents of the pool after invoking this
 * method (it currently does not zero the memory)
 *
 ...
 *
 */
uintptr_t pool_includesElement(J9Pool *pool, void *anElement) {
  J9PoolPuddleList *puddleList;
  J9PoolPuddle *walk;
 ...
 * Functions in pool_cap.c are not getting short-names properly without this
 * fix.  */
void refToPoolCap(J9Pool *aPool) { pool_ensureCapacity(aPool, 0); }
#endif /* J9ZOS390 */
