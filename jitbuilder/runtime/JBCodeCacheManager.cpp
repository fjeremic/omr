...
TR::CodeCacheMemorySegment* JitBuilder::CodeCacheManager::allocateCodeCacheSegment(
  size_t segmentSize, size_t& codeCacheSizeToAllocate, void* preferredStartAddress)
{
...
// TODO: Why is there no OMR_OS_ZOS? Or any other OS for that matter?
  ...
  auto memorySlab = reinterpret_cast<uint8_t*>(
    mmap(NULL, codeCacheSizeToAllocate, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0));
...
#undef NO_MAP_ANONYMOUS
  ...
  TR::CodeCacheMemorySegment* memSegment
    = (TR::CodeCacheMemorySegment*)((size_t)memorySlab + codeCacheSizeToAllocate - sizeof(TR::CodeCacheMemorySegment));
  new (memSegment) TR::CodeCacheMemorySegment(memorySlab, reinterpret_cast<uint8_t*>(memSegment));
  return memSegment;
}

void JitBuilder::CodeCacheManager::freeCodeCacheSegment(TR::CodeCacheMemorySegment* memSegment)
{
#if defined(OMR_OS_WINDOWS)
