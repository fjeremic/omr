...
extern void setupCodeCacheParameters(int32_t *,
...
static void initHelper(void *helper, TR_RuntimeHelper id) {
  ...
  // Helper Address is stored in a function descriptor consisting of [address,
  // TOC, envp] Load out Helper address from this function descriptor.

  // Little-Endian POWER can directly load the helper address, no function
  // descriptor used.
  helper = *(void **)helper;
#endif
...
static void initializeAllHelpers(JitBuilder::JitConfig *jitConfig,
                                 TR_RuntimeHelper *helperIDs,
                                 void **helperAddresses, int32_t numHelpers) {
...
static void initializeCodeCache(TR::CodeCacheManager &codeCacheManager) {
  ...
  // JitBuilder::CodeCacheManager::initialize() because it needs
  // trampolineCodeSize
...
// helperIDs is an array of helper id corresponding to the addresses passed in
// "helpers" helpers is an array of pointers to helpers that compiled code needs
// to reference
//   currently this argument isn't needed by anything so this function can stay
//   internal
// options is any JIT option string passed in to globally influence compilation
bool initializeJitBuilder(TR_RuntimeHelper *helperIDs, void **helperAddresses,
                          int32_t numHelpers, char *options) {
  ...
  try {
    ...
    TR::Compiler = new (rawAllocator)
        TR::CompilerEnv(rawAllocator, TR::PersistentAllocatorKit(rawAllocator));
...
bool internal_initializeJitWithOptions(char *options) {
...
bool internal_initializeJit() {
  return initializeJitBuilder(
      ...
      (char *)"-Xjit:acceptHugeMethods,enableBasicBlockHoisting,"
              "omitFramePointer,useILValidator");
}

int32_t internal_compileMethodBuilder(TR::MethodBuilder *m, void **entry) {
  ...
  *entry = (void *)fd;
  ...
  FunctionDescriptor *fd = new FunctionDescriptor();
  fd->func = *entry;
  // TODO: There should really be a better way to get this. Usually, we would
  // use cg->getTOCBase(), but the code generator has already been destroyed by
  // now...
  fd->toc = toPPCTableOfConstants(
