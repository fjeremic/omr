...
/**
 * @brief A template class for checking whether a particular type is supported
 * by `toIlType<>()`
 * @tparam C/C++ type
 * @return whether `toIlType<>()` can generate a corresponding IlType instance
 * or will fail to compile
 *
 * A type is considered supported iff JitBuilder directly provides, or can
 * derive, a type that corresponds, or that is equivalent, to the specified
 * type.
 *
 * ## Usage
 *
 ...
 * ## Examples
 *
 * - `is_supported<int8_t>::value == true` because JitBuilder directly provides
 * the corresponding type `Int8`
 * - `is_supported<int32_t*>::value == true` because JitBuilder directly
 * provides the corresponding type `pInt32` or `PointerTo(Int32)`
 * - `is_supported<double**>::value == true` because JitBuilder can derive the
 * corresponding type `PointerTo(pFloat)` or `PointerTo(PointerTo(Float))`
 * - `is_supported<uint16_t>::value == true` because JitBuilder provides the
 * equivalent type `Int16`
 * - `is_supported<SomeStruct>::value == false` because JitBuilder cannot derive
 * the type of a struct
 * - `is_supported<SomeEnum>::value == false` because JitBuilder cannot derive a
 * type that is equivalent to the underlying type of the enum
 * - `is_supported<void>::value == true` because JitBuilder directly provides
 * the corresponding `NoType`
 */
template <typename T> struct is_supported {
  static const bool value =
      OMR::IsArithmetic<T>::VALUE // note: IsArithmetic = IsIntegral ||
                                  // IsFloatingPoint
      || OMR::IsVoid<T>::VALUE;
};
template <typename T> struct is_supported<T *> {
 ...
 * ## Design
 *
 * `toIlType<>()` is an overloaded template function. It uses SFINAE and the
 * C++11 type traits library to enable a specific overload (function
 * implementation) that will return an instance of IlType. Type traits are used
 * to define rules that a type must follow to match a given function
 * implementation.
 *
 ...
 *
 * For types that do not belong to a family (e.g. `void`), the size is not
 * needed.
 *
 ...
 * class (metafunction) instead of a template function. However, because
 * instances of `IlType` are stored and returned as pointers, the "return" value
 * of the metafunction cannot be known at compile time. Therefore, a function
 * returning the correct instance must be used instead.
 *
...
// integral
template <typename T>
IlType *toIlType(typename OMR::EnableIf<OMR::IsIntegral<T>::VALUE &&
                                        (sizeof(T) == 1)>::Type * = 0) {
  return Int8;
}
template <typename T>
...
// floating point
template <typename T>
IlType *toIlType(typename OMR::EnableIf<OMR::IsFloatingPoint<T>::VALUE &&
                                        (sizeof(T) == 4)>::Type * = 0) {
  return Float;
}
template <typename T>
...
// void
template <typename T>
IlType *toIlType(typename OMR::EnableIf<OMR::IsVoid<T>::VALUE>::Type * = 0) {
  return NoType;
}
...
template <typename T>
IlType *toIlType(
    typename OMR::EnableIf<
        OMR::IsPointer<T>::VALUE &&
        is_supported<typename OMR::RemovePointer<T>::Type>::value>::Type * =
        0) {
