 ...
 * - `is_supported<int32_t*>::value == true` because JitBuilder directly provides the corresponding type `pInt32` or
 * `PointerTo(Int32)`
 * - `is_supported<double**>::value == true` because JitBuilder can derive the corresponding type `PointerTo(pFloat)` or
 * `PointerTo(PointerTo(Float))`
 ...
 * - `is_supported<SomeEnum>::value == false` because JitBuilder cannot derive a type that is equivalent to the
...
// integral
template <typename T>
IlType* toIlType(typename OMR::EnableIf<OMR::IsIntegral<T>::VALUE && (sizeof(T) == 1)>::Type* = 0)
{
  return Int8;
}
template <typename T>
IlType* toIlType(typename OMR::EnableIf<OMR::IsIntegral<T>::VALUE && (sizeof(T) == 2)>::Type* = 0)
{
  return Int16;
}
template <typename T>
IlType* toIlType(typename OMR::EnableIf<OMR::IsIntegral<T>::VALUE && (sizeof(T) == 4)>::Type* = 0)
{
  return Int32;
}
template <typename T>
IlType* toIlType(typename OMR::EnableIf<OMR::IsIntegral<T>::VALUE && (sizeof(T) == 8)>::Type* = 0)
{
  return Int64;
}
...
// floating point
template <typename T>
IlType* toIlType(typename OMR::EnableIf<OMR::IsFloatingPoint<T>::VALUE && (sizeof(T) == 4)>::Type* = 0)
{
  return Float;
}
template <typename T>
IlType* toIlType(typename OMR::EnableIf<OMR::IsFloatingPoint<T>::VALUE && (sizeof(T) == 8)>::Type* = 0)
{
  return Double;
}
...
// pointer
template <typename T>
IlType* toIlType(typename OMR::EnableIf<OMR::IsPointer<T>::VALUE
