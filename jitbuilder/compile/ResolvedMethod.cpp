...
JitBuilder::ResolvedMethod::ResolvedMethod(TR_OpaqueMethodBlock *method) {
  ...
  TR::ResolvedMethod *resolvedMethod =
      (TR::ResolvedMethod *)_ilInjector->methodSymbol()->getResolvedMethod();
  ...
  strncpy(_signatureChars, resolvedMethod->signatureChars(),
          62); // TODO: introduce concept of robustness
}

JitBuilder::ResolvedMethod::ResolvedMethod(TR::MethodBuilder *m)
    : _fileName(m->getDefiningFile()), _lineNumber(m->getDefiningLine()),
      _name((char *)m->GetMethodName()), // sad cast
      _numParms(m->getNumParameters()), _parmTypes(m->getParameterTypes()),
      _returnType(m->getReturnType()), _entryPoint(0), _signature(0),
      _externalName(0), _ilInjector(static_cast<TR::IlInjector *>(m)) {
  computeSignatureChars();
}

const char *JitBuilder::ResolvedMethod::signature(TR_Memory *trMemory,
                                                  TR_AllocationKind allocKind) {
  if (!_signature) {
    char *s = (char *)trMemory->allocateMemory(
        strlen(_fileName) + 1 + strlen(_lineNumber) + 1 + strlen(_name) + 1,
        allocKind);
...
const char *
JitBuilder::ResolvedMethod::externalName(TR_Memory *trMemory,
                                         TR_AllocationKind allocKind) {
  if (!_externalName) {
    // For C++, need to mangle name
    // char * s = (char *)trMemory->allocateMemory(1 + strlen(_name) + 1,
    // allocKind); sprintf(s, "_Z%d%si", (int32_t)strlen(_name), _name);
...
TR::DataType JitBuilder::ResolvedMethod::parmType(uint32_t slot) {
  TR_ASSERT((slot < _numParms), "Invalid slot provided for Parameter Type");
  return _parmTypes[slot]->getPrimitiveType();
}

void JitBuilder::ResolvedMethod::computeSignatureChars() {
  char *name = NULL;
  uint32_t len = 3;
  ...
  TR_ASSERT(len < 64,
            "signature array may not be large enough"); // TODO: robustness
...
char *JitBuilder::ResolvedMethod::localName(uint32_t slot, uint32_t bcIndex,
                                            int32_t &nameLength,
                                            TR_Memory *trMemory) {
  char *name = NULL;
...
TR::DataType JitBuilder::ResolvedMethod::returnType() {
  return _returnType->getPrimitiveType();
}

char *JitBuilder::ResolvedMethod::getParameterTypeSignature(int32_t parmIndex) {
