...
class TestState : public OMR::JitBuilder::VirtualMachineState {
 public:
  TestState()
      : OMR::JitBuilder::VirtualMachineState(), _stack(NULL), _stackTop(NULL) {}

  TestState(OMR::JitBuilder::VirtualMachineOperandStack *stack,
  ...
  virtual OMR::JitBuilder::VirtualMachineState *MakeCopy() {
    ...
    newState->_stack =
        (OMR::JitBuilder::VirtualMachineOperandStack *)_stack->MakeCopy();
    newState->_stackTop =
        (OMR::JitBuilder::VirtualMachineRegister *)_stackTop->MakeCopy();
    return newState;
  }

  virtual void MergeInto(OMR::JitBuilder::VirtualMachineState *other,
                         OMR::JitBuilder::IlBuilder *b) {
...
static void setupResult12Equals() {
  expectedResult12Top = 11;
  result12Operator = (char *)"==";
}

static void setupResult12NotEquals() {
  expectedResult12Top = 99;
  result12Operator = (char *)"!=";
}

int main(int argc, char *argv[]) {
  if (argc == 2 && strcmp(argv[1], "--verbose") == 0) verbose = true;
  ...
  OperandStackTestMethodFunction *ptrTest =
      (OperandStackTestMethodFunction *)entry2;
  ...
  OperandStackTestUsingStructMethodFunction *threadTest =
      (OperandStackTestUsingStructMethodFunction *)entry4;
...
void OperandStackTestMethod::createStack() {
...
STACKVALUETYPE *OperandStackTestMethod::moveStack() {
  int32_t stackSizeInBytes = _realStackSize * sizeof(STACKVALUETYPE);
...
void OperandStackTestMethod::freeStack() {
...
void verifyResult2(STACKVALUETYPE top) {
...
void verifyResult3(STACKVALUETYPE top) {
  if (verbose) cout << "Commit(); Top()\n";
...
void verifyResult4(STACKVALUETYPE popValue) {
...
void verifyResult5(STACKVALUETYPE popValue) {
  if (verbose) cout << "Pop()    [ no commit]\n";
...
void verifyResult6(STACKVALUETYPE top) {
...
void verifyResult8(STACKVALUETYPE pick) {
  if (verbose)
...
void verifyResult9(STACKVALUETYPE top) {
...
void verifyResult10(STACKVALUETYPE pick) {
...
void verifyResult12(STACKVALUETYPE top) {
  if (verbose)
    cout << "Pop(); Pop(); if (3 " << result12Operator
         << " 3) { Push(11); } else { Push(99); } Commit(); Top();\n";
...
void verifyValuesEqual(STACKVALUETYPE v1, STACKVALUETYPE v2) {
  REPORT2(v1 == v2, "verifyValuesEqual v1", v1, "verifyValuesEqual v2", v2);
}

// take the arguments from the stack and modify them
void modifyTop3Elements(int32_t amountToAdd) {
  if (verbose)
    cout << "Push();Push();Push() - modify elements passed in real stack and "
            "return";
...
bool OperandStackTestMethod::verifyUntouched(int32_t maxTouched) {
  for (int32_t i = maxTouched + 1; i < _realStackSize; i++)
    if (_realStack[i] != 0) return false;
  return true;
}

void OperandStackTestMethod::verifyStack(const char *step, int32_t max,
                                         int32_t num, ...) {
  STACKVALUETYPE *realSP = *verifySP;

  if (verbose)
    cout << "\tResult " << step << ": realSP-_realStack == " << (num - 1)
         << ": ";
  REPORT2((realSP - _realStack) == (num - 1), "_realStackTop-_realStack",
          (realSP - _realStack), "num-1", (num - 1));
...
OperandStackTestMethod::OperandStackTestMethod(
  ...
  DefineFunction("createStack", "0", "0",
                 (void *)&OperandStackTestMethod::createStack, NoType, 0);
  DefineFunction("moveStack", "0", "0",
                 (void *)&OperandStackTestMethod::moveStack, pValueType, 0);
  DefineFunction("freeStack", "0", "0",
                 (void *)&OperandStackTestMethod::freeStack, NoType, 0);
  ...
  DefineFunction("verifyResult8", "0", "0", (void *)&verifyResult8, NoType, 1,
                 _valueType);
  DefineFunction("verifyResult9", "0", "0", (void *)&verifyResult9, NoType, 1,
                 _valueType);
  DefineFunction("verifyResult10", "0", "0", (void *)&verifyResult10, NoType, 1,
                 _valueType);
  DefineFunction("verifyResult11", "0", "0", (void *)&verifyResult11, NoType,
                 0);
  DefineFunction("verifyResult12", "0", "0", (void *)&verifyResult12, NoType, 1,
                 _valueType);
  DefineFunction("verifyValuesEqual", "0", "0", (void *)&verifyValuesEqual,
                 NoType, 2, _valueType, _valueType);
  DefineFunction("modifyTop3Elements", "0", "0", (void *)&modifyTop3Elements,
                 NoType, 1, _valueType);
}
...
bool OperandStackTestMethod::testStack(OMR::JitBuilder::BytecodeBuilder *b,
                                       bool useEqual) {
  ...
  COMMIT(b);
  OMR::JitBuilder::IlValue *newStack = b->Call("moveStack", 0);
  UPDATESTACK(b, newStack);
  b->Call("verifyResult3", 1, TOP(b));

  OMR::JitBuilder::IlValue *val1 = POP(b);
  b->Call("verifyResult4", 1, val1);

  OMR::JitBuilder::IlValue *val2 = POP(b);
  b->Call("verifyResult5", 1, val2);

  OMR::JitBuilder::IlValue *sum = b->Add(val1, val2);
  PUSH(b, sum);
  COMMIT(b);
  ...
  OMR::JitBuilder::IlValue *v1 = POP(b);
  OMR::JitBuilder::IlValue *v2 = POP(b);
  if (useEqual)
  ...
  COMMIT(mergeBB);
  mergeBB->Call("modifyTop3Elements", 1,
                mergeBB->ConstInteger(_valueType, amountToAdd));
  RELOAD(mergeBB);
  OMR::JitBuilder::IlValue *modifiedStackElement = POP(mergeBB);
  OMR::JitBuilder::IlValue *expected =
      mergeBB->ConstInteger(_valueType, 3 + amountToAdd);
...
bool OperandStackTestMethod::buildIL() {
  OMR::JitBuilder::TypeDictionary *dict = typeDictionary();
  OMR::JitBuilder::IlType *pElementType =
      dict->PointerTo(dict->PointerTo(STACKVALUEILTYPE));

  Call("createStack", 0);

  OMR::JitBuilder::IlValue *realStackTopAddress = ConstAddress(&_realStackTop);
  OMR::JitBuilder::VirtualMachineRegister *stackTop =
      new OMR::JitBuilder::VirtualMachineRegister(this, "SP", pElementType,
                                                  sizeof(STACKVALUETYPE),
                                                  realStackTopAddress);
  OMR::JitBuilder::VirtualMachineOperandStack *stack =
      new OMR::JitBuilder::VirtualMachineOperandStack(this, 1, _valueType,
                                                      stackTop);
...
OperandStackTestUsingStructMethod::OperandStackTestUsingStructMethod(
    OMR::JitBuilder::TypeDictionary *d)
    : OperandStackTestMethod(d) {
  d->DefineStruct("Thread");
  d->DefineField("Thread", "sp", d->PointerTo(STACKVALUEILTYPE),
                 offsetof(Thread, sp));
...
bool OperandStackTestUsingStructMethod::buildIL() {
  Call("createStack", 0);

  OMR::JitBuilder::VirtualMachineRegisterInStruct *stackTop =
      new OMR::JitBuilder::VirtualMachineRegisterInStruct(this, "Thread",
                                                          "thread", "sp", "SP");
  OMR::JitBuilder::VirtualMachineOperandStack *stack =
      new OMR::JitBuilder::VirtualMachineOperandStack(this, 1, _valueType,
                                                      stackTop);
