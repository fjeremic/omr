  ...
  : OMR::JitBuilder::MethodBuilder(d)
{
  ...
  DefineFunction((char*)"printInt16", (char*)__FILE__, (char*)PRINTINT16_LINE, (void*)&printInt16, NoType, 1, Int16);
  DefineFunction((char*)"printInt32", (char*)__FILE__, (char*)PRINTINT32_LINE, (void*)&printInt32, NoType, 1, Int32);
  DefineFunction(
    (char*)"printAddress", (char*)__FILE__, (char*)PRINTADDRESS_LINE, (void*)&printAddress, NoType, 1, Address);
}
...
bool LinkedListMethod::buildIL()
{
  ...
  loop->Call("printInt16", 1, loop->LoadIndirect("Element", "key", loop->Load("ptr")));
  loop->Call("printString", 1, loop->ConstInt64((int64_t) ",v "));
  loop->Call("printInt32", 1, loop->LoadIndirect("Element", "val", loop->Load("ptr")));
  loop->Call("printString", 1, loop->ConstInt64((int64_t) "}\n"));
#endif

  loop->IfThen(
    &foundBuilder, loop->EqualTo(loop->LoadIndirect("Element", "key", loop->Load("ptr")), loop->Load("key")));

  foundBuilder->Store("val", foundBuilder->LoadIndirect("Element", "val", foundBuilder->Load("ptr")));
  foundBuilder->Goto(&breakBuilder);

  loop->Store("ptr", loop->LoadIndirect("Element", "next", loop->Load("ptr")));
  loop->Store("ptrNotNull", loop->NotEqualTo(loop->Load("ptr"), loop->NullAddress()));
