...
SwitchMethod::SwitchMethod(OMR::JitBuilder::TypeDictionary *d)
 ...
 */
bool SwitchMethod::RequestFunction(const char *name) {
  if (strncmp(name, "printString", 12) != 0) return false;
...
void SwitchMethod::PrintString(OMR::JitBuilder::IlBuilder *bldr,
                               const char *s) {
  bldr->Call("printString", 1, bldr->ConstInt64((int64_t)(char *)s));
}

bool SwitchMethod::buildIL() {
  OMR::JitBuilder::IlBuilder *defaultBldr = NULL;
  OMR::JitBuilder::IlBuilder *case1Bldr = NULL, *case2Bldr = NULL,
                             *case3Bldr = NULL;
  Switch("selector", &defaultBldr, 3, MakeCase(1, &case1Bldr, false),
         MakeCase(2, &case2Bldr, false), MakeCase(3, &case3Bldr, false));
...
int main(int argc, char *argv[]) {
  ...
  int s;
  s = 0;
  printf("Test varargs switch (%d):", s);
  test(s);  // default
  s = 1;
  printf("Test varargs switch (%d):", s);
  test(s);  // case1
