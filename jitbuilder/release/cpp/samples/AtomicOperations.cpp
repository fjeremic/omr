...
int main(int argc, char* argv[])
{
  ...
  int64_t val;
  val = 0;
  cout << "atomic increment64(" << val;
  cout << ") == " << increment64(&val) << "\n";
  val = 1;
  cout << "atomic increment64(" << val;
  cout << ") == " << increment64(&val) << "\n";
  val = 10;
...
bool AtomicInt32Add::buildIL()
{
  cout << "AtomicInt32Add::buildIL() running!\n";
  AtomicAdd(IndexAt(pInt32, Load("addressOfValue"), ConstInt32(0)), ConstInt32(1));
  Return(LoadAt(pInt32, Load("addressOfValue")));

  return true;
}

AtomicInt64Add::AtomicInt64Add(OMR::JitBuilder::TypeDictionary* d)
  : OMR::JitBuilder::MethodBuilder(d)
{
  DefineLine(LINETOSTR(__LINE__));
  DefineFile(__FILE__);
...
bool AtomicInt64Add::buildIL()
{
  cout << "AtomicInt64Add::buildIL() running!\n";
  AtomicAdd(IndexAt(pInt64, Load("addressOfValue"), ConstInt64(0)), ConstInt64(1));
