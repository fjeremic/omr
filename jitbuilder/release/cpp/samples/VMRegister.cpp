...
int main(int argc, char *argv[]) {
  ...
  cout << "Step 6: invoke compiled vmregisterInStruct function and print "
          "results\n";
  typedef int32_t(VMRegisterInStructMethodFunction)(VMRegisterStruct * param);
  VMRegisterInStructMethodFunction *vmregisterInStruct =
      (VMRegisterInStructMethodFunction *)entry;
...
VMRegisterMethod::VMRegisterMethod(OMR::JitBuilder::TypeDictionary *d)
    : OMR::JitBuilder::MethodBuilder(
          d, (OMR::JitBuilder::VirtualMachineState *)NULL) {
...
bool VMRegisterMethod::buildIL() {
  ...
  OMR::JitBuilder::VirtualMachineRegister *vmreg =
  ...
  loop->Store("result", loop->Add(loop->Load("result"),
                                  loop->ConvertTo(Int32, loop->Load("val"))));
  vmreg->Adjust(loop, 1);
...
VMRegisterInStructMethod::VMRegisterInStructMethod(
  ...
  d->DefineField("VMRegisterStruct", "values", d->PointerTo(Int8),
                 offsetof(VMRegisterStruct, values));
  d->DefineField("VMRegisterStruct", "count", Int32,
                 offsetof(VMRegisterStruct, count));
...
bool VMRegisterInStructMethod::buildIL() {
  ...
  OMR::JitBuilder::VirtualMachineRegisterInStruct *vmreg =
      new OMR::JitBuilder::VirtualMachineRegisterInStruct(
          this, "VMRegisterStruct", "param", "values", "VALUES");
