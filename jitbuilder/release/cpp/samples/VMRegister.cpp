...
VMRegisterMethod::buildIL()
{
  ...
  loop->Store(
    "result",
    loop->Add(loop->Load("result"), loop->ConvertTo(Int32, loop->Load("val"))));
  vmreg->Adjust(loop, 1);
                                   ...
                                   (OMR::JitBuilder::VirtualMachineState*)NULL)
{
  ...
  d->DefineField("VMRegisterStruct",
                 "values",
                 d->PointerTo(Int8),
                 offsetof(VMRegisterStruct, values));
  d->DefineField(
    "VMRegisterStruct", "count", Int32, offsetof(VMRegisterStruct, count));
