...
MandelbrotMethod::buildIL()
{
  ...
  k1Loop->Store(
    "xk",
    k1Loop->Add(k1Loop->Mul(k1Loop->ConstInt32(8), k1Loop->Load("x")),
                k1Loop->Load("k")));

  k1Loop->StoreAt(
    k1Loop->IndexAt(pDouble, k1Loop->Load("cr0"), k1Loop->Load("xk")),
    k1Loop->Sub(
      k1Loop->Div(k1Loop->Mul(k1Loop->ConstDouble(2.0),
                              k1Loop->ConvertTo(Double, k1Loop->Load("xk"))),
  ...
  y2Loop->Store(
    ...
    y2Loop->IndexAt(pInt8,
                    y2Loop->Load("buffer"),
                    y2Loop->Mul(y2Loop->Load("y"), y2Loop->Load("max_x"))));
  ...
  x2Loop->Store(
    ...
    x2Loop->IndexAt(pDouble,
                    x2Loop->Load("cr0"),
                    x2Loop->Mul(x2Loop->ConstInt32(8), x2Loop->Load("x"))));
  ...
  k2Loop->StoreAt(
    ...
    k2Loop->LoadAt(
      pDouble,
      k2Loop->IndexAt(pDouble, k2Loop->Load("cr0_x"), k2Loop->Load("k"))));
  ...
  bit_k_set->Store(
    "cr_k_sq",
    bit_k_set->Mul(bit_k_set->Load("cr_k"), bit_k_set->Load("cr_k")));

  bit_k_set->Store(
    "ci_k_sq",
    bit_k_set->Mul(bit_k_set->Load("ci_k"), bit_k_set->Load("ci_k")));

  bit_k_set->StoreAt(
    bit_k_set->IndexAt(pDouble, bit_k_set->Load("cr"), bit_k_set->Load("k")),
    bit_k_set->Add(
      bit_k_set->Sub(bit_k_set->Load("cr_k_sq"), bit_k_set->Load("ci_k_sq")),
      bit_k_set->LoadAt(pDouble,
                        bit_k_set->IndexAt(pDouble,
  ...
  bit_k_set->IfThen(
    ...
    bit_k_set->GreaterThan(
      bit_k_set->Add(bit_k_set->Load("cr_k_sq"), bit_k_set->Load("ci_k_sq")),
      bit_k_set->Load("limit_sq")));

  clear_bit_k->Store(
    "bits",
    clear_bit_k->Xor(clear_bit_k->Load("bits"), clear_bit_k->Load("bit_k")));

  k3Loop->Store(
    "bit_k",
    k3Loop->UnsignedShiftR(k3Loop->Load("bit_k"), k3Loop->ConstInt32(1)));
