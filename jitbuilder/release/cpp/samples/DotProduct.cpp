...
void DotProduct::PrintString(IlBuilder *bldr, const char *s) {
  bldr->Call("printString", 1, bldr->ConstInt64((int64_t)(char *)s));
}

bool DotProduct::buildIL() {
  PrintString(this, "dotproduct parameters:\n");

  PrintString(this, "   result is ");
  Call("printPointer", 1, Load("result"));
  PrintString(this, "\n");

  PrintString(this, "   vector1 is ");
  Call("printPointer", 1, Load("vector1"));
  PrintString(this, "\n");

  PrintString(this, "   vector2 is ");
  Call("printPointer", 1, Load("vector2"));
  PrintString(this, "\n");

  IlBuilder *loop = NULL;
  ForLoopUp("i", &loop, ConstInt32(0), Load("length"), ConstInt32(1));

  loop->StoreAt(
      loop->IndexAt(pDouble, loop->Load("result"), loop->Load("i")),
      loop->Mul(
          loop->LoadAt(pDouble, loop->IndexAt(pDouble, loop->Load("vector1"),
                                              loop->Load("i"))),
          loop->LoadAt(pDouble, loop->IndexAt(pDouble, loop->Load("vector2"),
                                              loop->Load("i")))));
...
int main(int argc, char *argv[]) {
  ...
  printf("result = [\n");
  for (int32_t i = 0; i < 10; i++) printf("           %lf\n", result[i]);
  printf("         ]\n\n");
