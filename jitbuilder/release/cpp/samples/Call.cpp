...
static int32_t doublesum(int32_t first, int32_t second) {
...
CallMethod::CallMethod(OMR::JitBuilder::TypeDictionary *types)
  ...
  DefineFunction((char *)"doublesum", (char *)__FILE__, (char *)DOUBLESUM_LINE,
                 (void *)&doublesum, Int32, 2, Int32, Int32);
}

bool CallMethod::buildIL() {
  Store("sum", ConstInt32(0));

  IlBuilder *loop = NULL;
  ForLoopUp("i", &loop, ConstInt32(0), Load("n"), ConstInt32(1));

  loop->Store("sum",
              loop->Call("doublesum", 2, loop->Load("sum"), loop->Load("i")));
...
bool ComputedCallMethod::buildIL() {
  Store("sum", ConstInt32(0));

  IlBuilder *loop = NULL;
  ForLoopUp("i", &loop, ConstInt32(0), Load("n"), ConstInt32(1));

  loop->Store("sum", loop->ComputedCall("doublesum", 3,
                                        loop->ConstAddress((void *)&doublesum),
                                        loop->Load("sum"), loop->Load("i")));
...
int main(int argc, char *argv[]) {
  ...
  for (int32_t n = 0; n < 10; n++) printf("call(%2d) = %d\n", n, call(n));
