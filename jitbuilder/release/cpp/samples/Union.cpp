...
SetUnionByteBuilder::SetUnionByteBuilder(OMR::JitBuilder::TypeDictionary* d)
...
bool SetUnionByteBuilder::buildIL() {
  StoreIndirect("TestUnionInt8pChar", "v_uint8", Load("u"), Load("v"));
  Return();
  return true;
}

GetUnionByteBuilder::GetUnionByteBuilder(OMR::JitBuilder::TypeDictionary* d)
    : OMR::JitBuilder::MethodBuilder(d) {
  DefineFile(__FILE__);
  DefineLine(LINETOSTR(__LINE__));
...
bool GetUnionByteBuilder::buildIL() {
  Return(LoadIndirect("TestUnionInt8pChar", "v_uint8", Load("u")));

  return true;
}

SetUnionStrBuilder::SetUnionStrBuilder(OMR::JitBuilder::TypeDictionary* d)
    : OMR::JitBuilder::MethodBuilder(d) {
  DefineFile(__FILE__);
  DefineLine(LINETOSTR(__LINE__));
...
bool SetUnionStrBuilder::buildIL() {
  StoreIndirect("TestUnionInt8pChar", "v_pchar", Load("u"), Load("v"));
  Return();
  return true;
}

GetUnionStrBuilder::GetUnionStrBuilder(OMR::JitBuilder::TypeDictionary* d)
    : OMR::JitBuilder::MethodBuilder(d) {
  DefineFile(__FILE__);
  DefineLine(LINETOSTR(__LINE__));
...
bool GetUnionStrBuilder::buildIL() {
  Return(LoadIndirect("TestUnionInt8pChar", "v_pchar", Load("u")));

  return true;
}

TypePunInt32Int32Builder::TypePunInt32Int32Builder(
...
bool TypePunInt32Int32Builder::buildIL() {
...
TypePunInt16DoubleBuilder::TypePunInt16DoubleBuilder(
    OMR::JitBuilder::TypeDictionary* d)
    : OMR::JitBuilder::MethodBuilder(d) {
  DefineLine(LINETOSTR(__LINE__));
  DefineFile(__FILE__);
...
bool TypePunInt16DoubleBuilder::buildIL() {
...
int main() {
...

  {
    ...
    std::cout << "Step 3: assert that the size of each union is the size of "
                 "its largest member\n";
    assert(sizeof(char*) ==
           typeDictionary.LookupUnion("TestUnionInt8pChar")->getSize());
    assert(sizeof(uint32_t) ==
           typeDictionary.LookupUnion("TestUnionInt32Int32")->getSize());
    assert(sizeof(double) ==
           typeDictionary.LookupUnion("TestUnionInt16Double")->getSize());
  }
...

  {
    ...
    auto typePunInt32Int32 =
        assert_compile<TypePunInt32Int32Function>(&builder);
...

  {
    ...
    auto typePunInt16Dboule =
        assert_compile<TypePunInt16DoubleFunction>(&builder);
