...
GetStructFieldAddressBuilder::GetStructFieldAddressBuilder(
...
GetUnionFieldAddressBuilder::GetUnionFieldAddressBuilder(
    OMR::JitBuilder::TypeDictionary *d)
    : OMR::JitBuilder::MethodBuilder(d) {
  DefineLine(LINETOSTR(__LINE__));
  DefineFile(__FILE__);
...
public:
  StructTypeDictionary() : OMR::JitBuilder::TypeDictionary() {
...
public:
  UnionTypeDictionary() : OMR::JitBuilder::TypeDictionary() {
    DefineUnion("Union");
...
int main(int argc, char *argv[]) {
  ...
  int32_t rc = compileMethodBuilder(&getStructFieldAddressBuilder,
                                    &getStructFieldAddressEntry);
  if (rc != 0) {
    fprintf(stderr, "FAIL: compilation error %d\n", rc);
    exit(-2);
  }

  printf("Step 4: invoke compiled code for getStructFieldAddress and verify "
         "results\n");
  Struct s;
  s.f1 = 1;
  s.f2 = 2;
  auto getStructFieldAddress =
      (GetStructFieldAddressFunction *)getStructFieldAddressEntry;
  ...
  rc = compileMethodBuilder(&getUnionFieldAddressBuilder,
                            &getUnionFieldAddressEntry);
  if (rc != 0) {
    fprintf(stderr, "FAIL: compilation error %d\n", rc);
    exit(-2);
  }

  printf("Step 6: invoke compiled code for getUnionFieldAddress and verify "
         "results\n");
  Union u;
  u.f2 = 2;
  auto getUnionFieldAddress =
      (GetUnionFieldAddressFunction *)getUnionFieldAddressEntry;
