...
#define REPORT_BCI(bci, e_bci)                                          \
  {                                                                     \
    if ((bci) != (e_bci)) {                                             \
      cerr << "Error: bytecode " << (bci)                               \
           << " not found in worklist ( expected " << (e_bci) << ")\n"; \
      return false;                                                     \
    }                                                                   \
  }
...
//#define REPORT_VMSTATE(b,s)     { if ((b)->vmState() != (s)) { cerr << "Error:
//bytecode " << ((b)->bcIndex()) << " does not have correct vmState (" << (s) <<
//")\n"; return false; } }

#define REPORT_RESULT(p, r, e)                                            \
  {                                                                       \
    if ((r) == (e)) {                                                     \
      success++;                                                          \
    } else {                                                              \
      cerr << "Error: path " << (p) << " generated wrong result (" << (r) \
           << " versus " << (e) << ")\n";                                 \
      fails++;                                                            \
    }                                                                     \
  }
...
int32_t computeResult(int32_t p) {
  // BC0
  int32_t r = p;
BC1:
  r++;
...
int main(int argc, char *argv[]) {
  ...
  const int32_t expectation[20] = {9,  9,  9,  9,  9,  11, 11, 17, 17, 17,
  ...
  if (fails == 0) cout << "ALL PASSED\n";

  shutdownJit();
}

WorklistMethod::WorklistMethod(OMR::JitBuilder::TypeDictionary *d)
...
bool WorklistMethod::buildIL() {
  cout << "WorklistMethod::buildIL() running!\n";

  const char *bcName[20] = {"bc0",  "bc1",  "bc2",  "bc3",  "bc4",
                            "bc5",  "bc6",  "bc7",  "bc8",  "bc9",
                            "bc10", "bc11", "bc12", "bc13", "bc14",
                            "bc15", "bc16", "bc17", "bc18", "bc19"};

  OMR::JitBuilder::BytecodeBuilder *builders[20];
  for (int32_t i = 0; i < 20; i++)
    builders[i] = OrphanBytecodeBuilder(i, (char *)bcName[i]);

  OMR::JitBuilder::VirtualMachineState *vmState =
      new OMR::JitBuilder::VirtualMachineState();
  setVMState(vmState);

  Store("result", Load("path"));
  ...
  b = builders[bci];
  REPORT_VMSTATE(b, vmState);
  b->Store("result", b->Add(b->Load("result"), b->ConstInt32(1)));
  b->IfCmpGreaterThan(builders[3], b->Load("result"), b->ConstInt32(10));
  ...
  b = builders[bci];
  REPORT_VMSTATE(b, vmState);
  b->IfCmpGreaterThan(builders[5], b->Load("result"), b->ConstInt32(5));
  ...
  b = builders[bci];
  REPORT_VMSTATE(b, vmState);
  b->IfCmpGreaterThan(builders[7], b->Load("result"), b->ConstInt32(15));
  ...
  b = builders[bci];
  REPORT_VMSTATE(b, vmState);
  b->IfCmpLessThan(builders[11], b->Load("result"), b->ConstInt32(8));
  ...
  b = builders[bci];
  REPORT_VMSTATE(b, vmState);
  b->IfCmpGreaterThan(builders[13], b->Load("result"), b->ConstInt32(12));
