...
#define REPORT_BCI(bci, e_bci)                                                                         \
  {                                                                                                    \
    if ((bci) != (e_bci)) {                                                                            \
      cerr << "Error: bytecode " << (bci) << " not found in worklist ( expected " << (e_bci) << ")\n"; \
      return false;                                                                                    \
    }                                                                                                  \
  }
...
//#define REPORT_VMSTATE(b,s)     { if ((b)->vmState() != (s)) { cerr << "Error: bytecode " << ((b)->bcIndex()) << "
//does not have correct vmState (" << (s) << ")\n"; return false; } }
...
#define REPORT_RESULT(p, r, e)                                                                           \
  {                                                                                                      \
    if ((r) == (e)) {                                                                                    \
      ...
      cerr << "Error: path " << (p) << " generated wrong result (" << (r) << " versus " << (e) << ")\n"; \
      fails++;                                                                                           \
    }                                                                                                    \
  }
...
int main(int argc, char* argv[])
{
  ...
  const int32_t expectation[20] = { 9, 9, 9, 9, 9, 11, 11, 17, 17, 17, 16, 16, 13, 13, 13, 8, 8, 8, 8, 8 };
...
bool WorklistMethod::buildIL()
{
  ...
  const char* bcName[20] = { "bc0", "bc1", "bc2", "bc3", "bc4", "bc5", "bc6", "bc7", "bc8", "bc9", "bc10", "bc11",
