...
IterativeFibonnaciMethod::IterativeFibonnaciMethod(
...
bool IterativeFibonnaciMethod::buildIL() {
  ...
  ForLoopUp("i", &iloop, ConstInt32(1), Load("n"), ConstInt32(1));

  iloop->Store("tempSum",
               iloop->Add(iloop->Load("Sum"), iloop->Load("LastSum")));
  iloop->Store("LastSum", iloop->Load("Sum"));
  iloop->Store("Sum", iloop->Load("tempSum"));
