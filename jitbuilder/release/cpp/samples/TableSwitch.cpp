 ...
 */
bool TableSwitchMethod::RequestFunction(const char *name) {
...
void TableSwitchMethod::PrintString(OMR::JitBuilder::IlBuilder *bldr,
                                    const char *s) {
  bldr->Call("printString", 1, bldr->ConstInt64((int64_t)(char *)s));
}

bool TableSwitchMethod::buildIL() {
  OMR::JitBuilder::IlBuilder *defaultBldr = NULL;
  OMR::JitBuilder::IlBuilder *case1Bldr = NULL, *case2Bldr = NULL,
                             *case3Bldr = NULL;
  TableSwitch("selector", &defaultBldr, true, 3, MakeCase(1, &case1Bldr, false),
...
int main(int argc, char *argv[]) {
  ...
  int s;
  s = 0;
  printf("Test varargs table switch (%d):", s);
  test(s);  // default
  s = 1;
  printf("Test varargs table switch (%d):", s);
  test(s);  // case1
