...
{
  Store("myArray", Call("malloc", 1, Mul(Load("size"), ConstInt32(StructType->getSize()))));

  OMR::JitBuilder::IlBuilder* fillArray = NULL;
  ForLoopUp("i", &fillArray, ConstInt32(0), Load("size"), ConstInt32(1));

  fillArray->Store("element", fillArray->IndexAt(pStructType, fillArray->Load("myArray"), fillArray->Load("i")));

  fillArray->StoreIndirect(
    "Struct", "type", fillArray->Load("element"), fillArray->ConvertTo(Int8, fillArray->Load("i")));

  fillArray->StoreIndirect("Struct", "value", fillArray->Load("element"), fillArray->Load("i"));
  ...
  : OMR::JitBuilder::MethodBuilder(d)
{
  ...
  DefineFunction("printStructElems", __FILE__, PRINTSTRUCTELEMS_LINE, (void*)&printStructElems, NoType, 2, Int8, Int32);

  DefineFunction(
    "printElemOffsets", __FILE__, PRINTELEMOFFSETS_LINE, (void*)&printElemOffsets, NoType, 2, Int32, Int32);
}

bool ReadStructArrayMethod::buildIL()
{
  Call("printElemOffsets", 2, ConstInt32(typeDictionary()->OffsetOf("Struct", "type")),
  ...
  readArray->Store("element", readArray->IndexAt(pStructType, readArray->Load("myArray"), readArray->Load("i")));

  readArray->Call("printStructElems", 2, readArray->LoadIndirect("Struct", "type", readArray->Load("element")),
