...
void MatMult::Store2D(OMR::JitBuilder::IlBuilder* bldr, OMR::JitBuilder::IlValue* base, OMR::JitBuilder::IlValue* first,
  OMR::JitBuilder::IlValue* second, OMR::JitBuilder::IlValue* N, OMR::JitBuilder::IlValue* value)
{
  bldr->StoreAt(bldr->IndexAt(pDouble, base, bldr->Add(bldr->Mul(first, N), second)), value);
}

OMR::JitBuilder::IlValue* MatMult::Load2D(OMR::JitBuilder::IlBuilder* bldr, OMR::JitBuilder::IlValue* base,
  OMR::JitBuilder::IlValue* first, OMR::JitBuilder::IlValue* second, OMR::JitBuilder::IlValue* N)
{
  return bldr->LoadAt(pDouble, bldr->IndexAt(pDouble, base, bldr->Add(bldr->Mul(first, N), second)));
}

bool MatMult::buildIL()
{
  ...
  ForLoopUp("i", &iloop, zero, N, one);
  {
    ...
    iloop->ForLoopUp("j", &jloop, zero, N, one);
    {
      ...
      jloop->ForLoopUp("k", &kloop, zero, N, one);
      {
        ...
        kloop->Store("sum", kloop->Add(kloop->Load("sum"), kloop->Mul(A_ik, B_kj)));
      }
...
void VectorMatMult::VectorStore2D(OMR::JitBuilder::IlBuilder* bldr, OMR::JitBuilder::IlValue* base,
  OMR::JitBuilder::IlValue* first, OMR::JitBuilder::IlValue* second, OMR::JitBuilder::IlValue* N,
  OMR::JitBuilder::IlValue* value)
{
  bldr->VectorStoreAt(bldr->IndexAt(pDouble, base, bldr->Add(bldr->Mul(first, N), second)), value);
}

OMR::JitBuilder::IlValue* VectorMatMult::VectorLoad2D(OMR::JitBuilder::IlBuilder* bldr, OMR::JitBuilder::IlValue* base,
  OMR::JitBuilder::IlValue* first, OMR::JitBuilder::IlValue* second, OMR::JitBuilder::IlValue* N)
{
  return bldr->VectorLoadAt(pDouble, bldr->IndexAt(pDouble, base, bldr->Add(bldr->Mul(first, N), second)));
}

OMR::JitBuilder::IlValue* VectorMatMult::Load2D(OMR::JitBuilder::IlBuilder* bldr, OMR::JitBuilder::IlValue* base,
  OMR::JitBuilder::IlValue* first, OMR::JitBuilder::IlValue* second, OMR::JitBuilder::IlValue* N)
{
  return bldr->LoadAt(pDouble, bldr->IndexAt(pDouble, base, bldr->Add(bldr->Mul(first, N), second)));
}

bool VectorMatMult::buildIL()
{
  // marking all locals as defined allows remaining locals to be temps
  // which enables further optimization opportunities particularly for
  //    floating point types
  ...
  ForLoopUp("i", &iloop, zero, N, one);
  {
    ...
    iloop->ForLoopUp("j", &jloop, zero, N, two);
    {
      ...
      jloop->ForLoopUp("k", &kloop, zero, N, one);
      {
        ...
        kloop->VectorStore("sum", kloop->Add(kloop->VectorLoad("sum"), kloop->Mul(A_ik, B_kj)));
      }
...
void printMatrix(double* M, int32_t N, const char* name)
{
  printf("%s = [\n", name);
