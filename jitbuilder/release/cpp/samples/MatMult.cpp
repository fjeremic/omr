...
void MatMult::Store2D(OMR::JitBuilder::IlBuilder *bldr,
  ...
  bldr->StoreAt(
      bldr->IndexAt(pDouble, base, bldr->Add(bldr->Mul(first, N), second)),
      value);
}

OMR::JitBuilder::IlValue *MatMult::Load2D(OMR::JitBuilder::IlBuilder *bldr,
                                          OMR::JitBuilder::IlValue *base,
                                          OMR::JitBuilder::IlValue *first,
                                          OMR::JitBuilder::IlValue *second,
                                          OMR::JitBuilder::IlValue *N) {
  ...
  ForLoopUp("i", &iloop, zero, N, one);
  {
    ...
    iloop->ForLoopUp("j", &jloop, zero, N, one);
    {
      ...
      jloop->ForLoopUp("k", &kloop, zero, N, one);
      {
        ...
        kloop->Store("sum",
                     kloop->Add(kloop->Load("sum"), kloop->Mul(A_ik, B_kj)));
      }
...
void VectorMatMult::VectorStore2D(OMR::JitBuilder::IlBuilder *bldr,
                                  OMR::JitBuilder::IlValue *base,
                                  OMR::JitBuilder::IlValue *first,
                                  OMR::JitBuilder::IlValue *second,
                                  OMR::JitBuilder::IlValue *N,
...
OMR::JitBuilder::IlValue *VectorMatMult::VectorLoad2D(
    OMR::JitBuilder::IlBuilder *bldr, OMR::JitBuilder::IlValue *base,
    OMR::JitBuilder::IlValue *first, OMR::JitBuilder::IlValue *second,
    OMR::JitBuilder::IlValue *N) {
  return bldr->VectorLoadAt(
      pDouble,
      bldr->IndexAt(pDouble, base, bldr->Add(bldr->Mul(first, N), second)));
}

OMR::JitBuilder::IlValue *VectorMatMult::Load2D(
    OMR::JitBuilder::IlBuilder *bldr, OMR::JitBuilder::IlValue *base,
    OMR::JitBuilder::IlValue *first, OMR::JitBuilder::IlValue *second,
    OMR::JitBuilder::IlValue *N) {
  return bldr->LoadAt(
  ...
  ForLoopUp("i", &iloop, zero, N, one);
  {
    ...
    iloop->ForLoopUp("j", &jloop, zero, N, two);
    {
      ...
      jloop->ForLoopUp("k", &kloop, zero, N, one);
      {
        ...
        kloop->VectorStore("sum", kloop->Add(kloop->VectorLoad("sum"),
                                             kloop->Mul(A_ik, B_kj)));
      }

      VectorStore2D(jloop, C, i, j, N,
                    jloop->Load("sum"));  // C[i,j] is vector over j
    }
  }
...
void printMatrix(double *M, int32_t N, const char *name) {
  ...
  for (int32_t i = 0; i < N; i++) {
    printf("      [ %lf", M[i * N]);
    for (int32_t j = 1; j < N; j++) printf(", %lf", M[i * N + j]);
    printf(" ],\n");
  }
  printf("    ]\n\n");
}
