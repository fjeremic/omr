...
extern "C"
{
  ...
  OMR_VMThread* omr_vmthread_getCurrent(OMR_VM* vm)
  {
    ...
    if (NULL != self) {
      if (vm->_vmThreadKey > 0) {
        currentThread =
          (OMR_VMThread*)omrthread_tls_get(self, vm->_vmThreadKey);
      }
    }
  ...
  omr_error_t omr_vmthread_alloc(OMR_VM* vm, OMR_VMThread** vmthread)
  {
    ...
    *vmthread = (OMR_VMThread*)omrmem_allocate_memory(sizeof(OMR_VMThread),
                                                      OMRMEM_CATEGORY_VM);
    if (NULL == *vmthread) {
  ...
  omr_error_t omr_vmthread_firstAttach(OMR_VM* vm, OMR_VMThread** vmThread)
  {
    ...
    OMR_VMThread* tempThread = (OMR_VMThread*)omrmem_allocate_memory(
      sizeof(OMR_VMThread), OMRMEM_CATEGORY_VM);
    if (NULL != tempThread) {
  ...
  void omr_vmthread_redetach(OMR_VMThread* currentThread)
  {
    Assert_OMRVM_true(0 < currentThread->_attachCount);
    currentThread->_attachCount -= 1;
  }
