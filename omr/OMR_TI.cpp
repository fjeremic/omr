 ...
 */
static omr_error_t getOMRSysInfoSystemCpuTime(
  OMR_VM* omrVM, J9SysinfoCPUTime* systemCpuTime, OMRSysInfoCpuLoadCallStatus* status)
{
...
omr_error_t omrtiGetSystemCpuLoad(OMR_VMThread* vmThread, double* systemCpuLoad)
{
  ...
  if (NULL == vmThread) {
    ...
    if ((SUPPORTED == curSysInfo->systemCpuLoadCallStatus) || (NO_HISTORY == curSysInfo->systemCpuLoadCallStatus)) {
      ...
      if (OMR_ERROR_NONE != ret) {
        ...
        if (OK == rc) {
          /* discard the oldestSystemCpuTime, replace it with interimSystemCpuTime and save newestSystemCpuTime as the
           * new interimSystemCpuTime. */
          ...
          if (OK == rc) {
            ...
            if (MAXIMUM_NEGATIVE_ELAPSED_TIME_COUNT == curSysInfo->systemCpuTimeNegativeElapsedTimeCount) {
              /* if this case found consecutively for MAXIMUM_NEGATIVE_ELAPSED_TIME_COUNT times, discard the
...
omr_error_t omrtiGetProcessCpuLoad(OMR_VMThread* vmThread, double* processCpuLoad)
{
  ...
  if (NULL == vmThread) {
    ...
    OMR_SysInfo* curSysInfo = vmThread->_vm->sysInfo;
    if ((SUPPORTED == curSysInfo->processCpuLoadCallStatus) || (NO_HISTORY == curSysInfo->processCpuLoadCallStatus)) {
      ...
      if (OMR_ERROR_NONE != ret) {
        ...
        if (OK == rc) {
          /* discard the oldestProcessCpuTime and replace it with interimProcessCpuTime, save newestProcessCpuTime as
           * the new interimProcessCpuTime. */
          memcpy(
            &curSysInfo->oldestProcessCpuTime, &curSysInfo->interimProcessCpuTime, sizeof(OMRSysInfoProcessCpuTime));
          memcpy(&curSysInfo->interimProcessCpuTime, &newestProcessCpuTime, sizeof(OMRSysInfoProcessCpuTime));
        } else {
          ...
          if (OK == rc) {
            ...
            /* return OMR_ERROR_RETRY if interval between newestProcessCpuTime and oldestProcessCpuTime is still too
             * small */
            curSysInfo->processCpuTimeNegativeElapsedTimeCount = 0;
            ret = OMR_ERROR_RETRY;
          } else if (ELAPSED_TIME_NEGATIVE == rc) {
...
omr_error_t omrtiGetMemoryCategories(OMR_VMThread* vmThread, int32_t max_categories,
  ...
  size_t* nameBytesRemaining)
{
  ...
  if (NULL == vmThread) {
    ...
    rc = dictionary->getEntries(vmThread, methodArray, methodArrayCount, methodDescriptions, nameBuffer, nameBytes,
 ...
 * locks.
 */
omr_error_t omrtiGetMethodProperties(
