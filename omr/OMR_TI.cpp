...
#define UTSINTERFACE_FROM_OMRVMTHREAD(vmThread)                                \
  ((vmThread)->_vm->_trcEngine ? &(vmThread)->_vm->_trcEngine->omrTraceIntfS   \
                               : NULL)
...
static uint32_t indexFromCategoryCode(uintptr_t categories_mapping_size,
                                      uint32_t cc);
static uintptr_t omrtiGetMemoryCategoriesCallback(
    uint32_t categoryCode, const char *categoryName, uintptr_t liveBytes,
    uintptr_t liveAllocations, BOOLEAN isRoot, uint32_t parentCategoryCode,
    OMRMemCategoryWalkState *state);
static uintptr_t omrtiCountMemoryCategoriesCallback(
    uint32_t categoryCode, const char *categoryName, uintptr_t liveBytes,
    uintptr_t liveAllocations, BOOLEAN isRoot, uint32_t parentCategoryCode,
    OMRMemCategoryWalkState *state);
static uintptr_t omrtiCalculateSlotsForCategoriesMappingCallback(
    uint32_t categoryCode, const char *categoryName, uintptr_t liveBytes,
    uintptr_t liveAllocations, BOOLEAN isRoot, uint32_t parentCategoryCode,
    OMRMemCategoryWalkState *state);
static void
fillInChildAndSiblingCategories(OMR_TI_MemoryCategory *categories_buffer,
                                int32_t written_count);
static void fillInCategoryDeepCounters(OMR_TI_MemoryCategory *category);

static omr_error_t
getOMRSysInfoProcessCpuTime(OMR_VM *omrVM, OMRSysInfoProcessCpuTime *sysInfo,
                            OMRSysInfoCpuLoadCallStatus *status);
static OMRSysInfoCalculateCpuLoadError
calculateProcessCpuLoad(OMRSysInfoProcessCpuTime const *endRecord,
                        OMRSysInfoProcessCpuTime const *startRecord,
                        double *cpuLoad);
static omr_error_t
getOMRSysInfoSystemCpuTime(OMR_VM *omrVM, J9SysinfoCPUTime *sysInfo,
                           OMRSysInfoCpuLoadCallStatus *status);
static OMRSysInfoCalculateCpuLoadError
calculateSystemCpuLoad(J9SysinfoCPUTime const *endRecord,
                       J9SysinfoCPUTime const *startRecord, double *cpuLoad);
static int64_t computeTimeInterval(const int64_t endNS, const int64_t startNS);

omr_error_t omrtiBindCurrentThread(OMR_VM *vm, const char *threadName,
...
omr_error_t omrtiRegisterRecordSubscriber(OMR_VMThread *vmThread,
  ...
  if (NULL == vmThread) {
    rc = OMR_THREAD_NOT_ATTACHED;
  } else if ((NULL == subscriberFunc) || (NULL == subscriptionID) ||
             (NULL == description)) {
    rc = OMR_ERROR_ILLEGAL_ARGUMENT;
  } else {
    /*
     * OMRTODO
     * If the current thread is not attached to the trace engine, we can't
     * guarantee that the serverf pointer is consistent. Another thread could be
     * in the process of deleting the trace engine.
     *
     * This situation could occur if the trace engine were started & shutdown,
     * and the current thread was bound to the OMR_VM after trace shutdown, so
     * it has a valid OMR_VMThread, but is not attached to the trace engine.
     *
     * In practice, this is unlikely to occur because we unload agents before
     * shutting down the trace engine.
     */
    OMR_TraceInterface *serverf = UTSINTERFACE_FROM_OMRVMTHREAD(vmThread);
    if (NULL == serverf) {
      rc = OMR_ERROR_NOT_AVAILABLE;
    } else {
      rc = serverf->RegisterRecordSubscriber(
          OMR_TRACE_THREAD_FROM_VMTHREAD(vmThread), description, subscriberFunc,
...
omr_error_t omrtiDeregisterRecordSubscriber(OMR_VMThread *vmThread,
                                            UtSubscription *subscriptionID) {
  omr_error_t rc = OMR_ERROR_NONE;
...
omr_error_t omrtiGetTraceMetadata(OMR_VMThread *vmThread, void **data,
                                  int32_t *length) {
  omr_error_t rc = OMR_ERROR_NONE;
  OMR_TI_ENTER_FROM_VM_THREAD(vmThread);
 ...
 *
 * Sanity checks to avoid returning negative or unreasonably large intervals if
 * the timer goes backwards.
 *
 * @param[in] endNS end of interval in nanoseconds
 * @param[in] startNS start of interval in nanoseconds
 * @return (end - start) interval in nanoseconds, or a negative value if
 * interval is badly defined
 */
static int64_t computeTimeInterval(const int64_t endNS, const int64_t startNS) {
  uint64_t intervalNS = (uint64_t)endNS - (uint64_t)startNS;

  /*
   * endNS could be less than startNS if the timer wrapped.  However, modular
   * arithmetic would give us a valid positive interval for (endNS - startNS).
   *
   * Excessively large intervals (>292 years approximately or I_64_MAX ns)
   * indicate the timer has gone backwards. In this case, the return value will
   * be negative when interpreted as an int64_t.
   */
  return (int64_t)intervalNS;
}

/**
 * This function fills in a record of J9SysinfoCPUTime or sets
 * systemCpuLoadCallStatus
 * @param[in] omrVM the current OMR VM
 * @param[in,out] sysInfo record of J9SysinfoCPUTime, it cannot be NULL
 * @param[in,out] status whether GetSystemCpuLoad is supported on this platform
 * or user have sufficient rights, it cannot be NULL
 * @return OMR error code
 */
static omr_error_t
getOMRSysInfoSystemCpuTime(OMR_VM *omrVM, J9SysinfoCPUTime *systemCpuTime,
                           OMRSysInfoCpuLoadCallStatus *status) {
...
/**
 * This function returns a record of getOMRSysInfoProcessCpuTime or sets
 * processCpuLoadCallStatus
 * @param[in] omrVM the current OMR VM
 * @param[in,out] vm record of OMRSysInfoProcessCpuTime, it cannot be NULL
 * @param[in,out] status whether GetProcessCpuLoad is supported on this platform
 * or user have sufficient rights, it cannot be NULL
 * @return OMR error code
 */
static omr_error_t
getOMRSysInfoProcessCpuTime(OMR_VM *omrVM,
                            OMRSysInfoProcessCpuTime *processCpuTime,
                            OMRSysInfoCpuLoadCallStatus *status) {
  OMRPORT_ACCESS_FROM_OMRVM(omrVM);
  omr_error_t rc = OMR_ERROR_NONE;
 ...
 * @param[in] endRecord the end point of the period used to calculate the CPU
 * load, it cannot be NULL
 * @param[in] startRecord the start point of the period used to calculate the
 * CPU load, it cannot be NULL
 * @param[in,out] cpuLoad the system CPU load between the observed time period,
 * it cannot be NULL
...
calculateSystemCpuLoad(J9SysinfoCPUTime const *endRecord,
  ...
  int64_t timestampDelta =
      computeTimeInterval(endRecord->timestamp, startRecord->timestamp);
  if (timestampDelta < 0) {
    ...
    double result = (double)cpuTimeDelta /
                    (double)(endRecord->numberOfCpus * timestampDelta);
    /* cpuload is in [0, 1] */
 ...
 * @param[in] startRecord the start point of the period used to calculate the
 * CPU load, it cannot be NULL
 * @param[in,out] cpuLoad the process CPU load between the observed time period,
 * it cannot be NULL
...
calculateProcessCpuLoad(OMRSysInfoProcessCpuTime const *endRecord,
  ...
  OMRSysInfoCalculateCpuLoadError rc = OK;
  int64_t timestampDelta =
      computeTimeInterval(endRecord->timestampNS, startRecord->timestampNS);
  if (timestampDelta < 0) {
    rc = ELAPSED_TIME_NEGATIVE;
  } else if (timestampDelta < MINIMUM_CPU_LOAD_INTERVAL) {
    rc = ELAPSED_TIME_TOO_SMALL;
    ...
    int64_t systemCpuTimeDelta =
        endRecord->systemCpuTimeNS - startRecord->systemCpuTimeNS;
    int64_t userCpuTimeDelta =
        endRecord->userCpuTimeNS - startRecord->userCpuTimeNS;
    int64_t processTimeDelta = systemCpuTimeDelta + userCpuTimeDelta;
    if (processTimeDelta < systemCpuTimeDelta) {
      /* operation processTimeDelta = systemCpuTimeDelta + userCpuTimeDelta
       * overflow, set cpuLoad to 1 in this case */
      *cpuLoad = 1.0;
    } else {
      double result = (double)processTimeDelta /
                      (double)(endRecord->numberOfCpus * timestampDelta);
      /* cpuload is in [0, 1] */
      *cpuLoad = OMR_MIN(result, 1.0);
    }
...
omr_error_t omrtiGetSystemCpuLoad(OMR_VMThread *vmThread,
  ...
  if (NULL == vmThread) {
  ...
  } else if (0 != omrthread_monitor_enter(
                      vmThread->_vm->sysInfo->syncSystemCpuLoad)) {
    ...
    if ((SUPPORTED == curSysInfo->systemCpuLoadCallStatus) ||
      ...
      ret = getOMRSysInfoSystemCpuTime(vmThread->_vm, &newestSystemCpuTime,
                                       &curSysInfo->systemCpuLoadCallStatus);
      if (OMR_ERROR_NONE != ret) {
        ...
        OMRSysInfoCalculateCpuLoadError rc = OK;
        rc = calculateSystemCpuLoad(&newestSystemCpuTime,
                                    &curSysInfo->interimSystemCpuTime,
                                    systemCpuLoad);
        if (OK == rc) {
          /* discard the oldestSystemCpuTime, replace it with
           * interimSystemCpuTime and save newestSystemCpuTime as the new
           * interimSystemCpuTime. */
          memcpy(&curSysInfo->oldestSystemCpuTime,
                 &curSysInfo->interimSystemCpuTime, sizeof(J9SysinfoCPUTime));
          memcpy(&curSysInfo->interimSystemCpuTime, &newestSystemCpuTime,
                 sizeof(J9SysinfoCPUTime));
        } else {
          /* interimSystemCpuTime or newestSystemCpuTime is not valid, or the
           * elapsed time between them is too small. */
          ...
          rc = calculateSystemCpuLoad(&newestSystemCpuTime,
                                      &curSysInfo->oldestSystemCpuTime,
                                      systemCpuLoad);
          if (OK == rc) {
...
omr_error_t omrtiGetProcessCpuLoad(OMR_VMThread *vmThread,
  ...
  if (NULL == vmThread) {
    ...
    ret = OMR_ERROR_NOT_AVAILABLE;
                      ...
                      vmThread->_vm->sysInfo->syncProcessCpuLoad)) {
    ret = OMR_ERROR_INTERNAL;
  } else {
    OMR_SysInfo *curSysInfo = vmThread->_vm->sysInfo;
    if ((SUPPORTED == curSysInfo->processCpuLoadCallStatus) ||
      ...
      ret = getOMRSysInfoProcessCpuTime(vmThread->_vm, &newestProcessCpuTime,
                                        &curSysInfo->processCpuLoadCallStatus);
      if (OMR_ERROR_NONE != ret) {
        ...
        rc = calculateProcessCpuLoad(&newestProcessCpuTime,
                                     &curSysInfo->interimProcessCpuTime,
                                     processCpuLoad);
        if (OK == rc) {
          /* discard the oldestProcessCpuTime and replace it with
           * interimProcessCpuTime, save newestProcessCpuTime as the new
           * interimProcessCpuTime. */
          memcpy(&curSysInfo->oldestProcessCpuTime,
                 &curSysInfo->interimProcessCpuTime,
                 sizeof(OMRSysInfoProcessCpuTime));
          memcpy(&curSysInfo->interimProcessCpuTime, &newestProcessCpuTime,
                 sizeof(OMRSysInfoProcessCpuTime));
        } else {
          /* interimProcessCpuTime or newestProcessCpuTime is not valid, or the
           * elapsed time between them is too small. */
          if (NEGATIVE_INTERVAL == rc) {
          ...
          rc = calculateProcessCpuLoad(&newestProcessCpuTime,
                                       &curSysInfo->oldestProcessCpuTime,
                                       processCpuLoad);
          if (OK == rc) {
            ...
            /* return OMR_ERROR_RETRY if interval between newestProcessCpuTime
             * and oldestProcessCpuTime is still too small */
            curSysInfo->processCpuTimeNegativeElapsedTimeCount = 0;
            ret = OMR_ERROR_RETRY;
          } else if (ELAPSED_TIME_NEGATIVE == rc) {
            if (MAXIMUM_NEGATIVE_ELAPSED_TIME_COUNT ==
              ...
              /* if this case found consecutively for
               * MAXIMUM_NEGATIVE_ELAPSED_TIME_COUNT times, discard the
               * oldestProcessCpuTime and replace it with newestProcessCpuTime.
               */
              memcpy(&curSysInfo->oldestProcessCpuTime, &newestProcessCpuTime,
                     sizeof(OMRSysInfoProcessCpuTime));
              curSysInfo->processCpuTimeNegativeElapsedTimeCount = 0;
...
static uint32_t indexFromCategoryCode(uintptr_t categories_mapping_size,
  ...
  return ((cc) > OMRMEM_LANGUAGE_CATEGORY_LIMIT)
             ? ((((uint32_t)categories_mapping_size) - 1) -
                (OMRMEM_OMR_CATEGORY_INDEX_FROM_CODE(cc)))
             : (cc);
}
...
static uintptr_t omrtiGetMemoryCategoriesCallback(
    ...
    uintptr_t liveAllocations, BOOLEAN isRoot, uint32_t parentCategoryCode,
    OMRMemCategoryWalkState *state) {
  struct omrtiGetMemoryCategoriesState *userData =
      (struct omrtiGetMemoryCategoriesState *)state->userData1;
  uint32_t index =
      indexFromCategoryCode(userData->categories_mapping_size, categoryCode);

  if (userData->written_count < userData->max_categories) {
    OMR_TI_MemoryCategory *omrtiCategory =
        &userData->categories_buffer[userData->written_count];
    ...
    if (isRoot) {
      ...
      uint32_t parentIndex = indexFromCategoryCode(
          userData->categories_mapping_size, parentCategoryCode);

      /* Memory category walk is depth first - we will definitely have walked
       * through and recorded our parent */
...
static uintptr_t omrtiCalculateSlotsForCategoriesMappingCallback(
  ...
  if (categoryCode < OMRMEM_LANGUAGE_CATEGORY_LIMIT) {
    ...
    state->userData1 = (void *)((maxLanguageCategoryIndex > categoryIndex)
                                    ? maxLanguageCategoryIndex
                                    : categoryIndex);

  } else if (categoryCode > OMRMEM_LANGUAGE_CATEGORY_LIMIT) {
    uintptr_t maxOMRCategoryIndex = (uintptr_t)state->userData2;
    uintptr_t categoryIndex =
        (uintptr_t)OMRMEM_OMR_CATEGORY_INDEX_FROM_CODE(categoryCode);

    state->userData2 =
        (void *)((maxOMRCategoryIndex > categoryIndex) ? maxOMRCategoryIndex
                                                       : categoryIndex);
  }
  return J9MEM_CATEGORIES_KEEP_ITERATING;
}
...
fillInChildAndSiblingCategories(OMR_TI_MemoryCategory *categories_buffer,
  ...
  /* Note: for efficiency, we will temporarily retask the liveBytesDeep field as
  ...
  for (i = 0; i < written_count; i++) {
    ...
    if (parent) {
      OMR_TI_MemoryCategory *lastChild =
          (OMR_TI_MemoryCategory *)(uintptr_t)parent->liveBytesDeep;
...
omr_error_t omrtiGetMemoryCategories(OMR_VMThread *vmThread,
                                     int32_t max_categories,
                                     OMR_TI_MemoryCategory *categories_buffer,
  ...
  if (NULL == vmThread) {
    ...
    Trc_OMRTI_omrtiGetMemoryCategories_Entry(
        vmThread, max_categories, categories_buffer, written_count_ptr,
    ...
    /* If we're asked to write to categories_buffer (max_categories > 0), make
     * sure categories_buffer isn't NULL */
    if (max_categories > 0 && NULL == categories_buffer) {
      Trc_OMRTI_omrtiGetMemoryCategories_NullOutput_Exit(vmThread,
                                                         max_categories);
      OMR_TI_RETURN(vmThread, OMR_ERROR_ILLEGAL_ARGUMENT);
    }

    /* If the user has set max_categories and categories_buffer, but hasn't set
     * written_count_ptr, fail. */
    ...
    if (NULL == categories_buffer && NULL == written_count_ptr &&
        NULL == total_categories_ptr) {
    ...
    if (categories_buffer) {
       ...
       * It needs to handle the largest possible indexes for OMR or language
       * categories, not the total number of categories, which may be smaller.
       */
      ...
      userData.categories_mapping =
          (OMR_TI_MemoryCategory **)omrmem_allocate_memory(
              userData.categories_mapping_size *
                  sizeof(OMR_TI_MemoryCategory *),
      ...
      memset(userData.categories_mapping, 0,
             userData.categories_mapping_size *
                 sizeof(OMR_TI_MemoryCategory *));
      memset(categories_buffer, 0,
             max_categories * sizeof(OMR_TI_MemoryCategory));

      /* Walk categories to get shallow values */
      omrmem_walk_categories(&walkState);

      /* Fix-up child and sibling references */
      fillInChildAndSiblingCategories(categories_buffer,
                                      userData.written_count);
...
omr_error_t omrtiGetMethodDescriptions(
  ...
  if (NULL == vmThread) {
    ...
    OMR_MethodDictionary *const dictionary =
        (OMR_MethodDictionary *)vmThread->_vm->_methodDictionary;
    rc = dictionary->getEntries(vmThread, methodArray, methodArrayCount,
                                methodDescriptions, nameBuffer, nameBytes,
...
/*
 * This function doesn't need to be synchronized because it accesses constant
 * data, doesn't modify data, and doesn't use locks.
 */
omr_error_t omrtiGetMethodProperties(OMR_VMThread *vmThread,
  ...
  if (NULL == vmThread) {
    ...
    rc = OMR_ERROR_NOT_AVAILABLE;
  } else if ((NULL == numProperties) || (NULL == propertyNames) ||
             (NULL == sizeofSampledMethodDesc)) {
    rc = OMR_ERROR_ILLEGAL_ARGUMENT;
  } else {
    OMR_MethodDictionary *const dictionary =
        (OMR_MethodDictionary *)vmThread->_vm->_methodDictionary;
    dictionary->getProperties(numProperties, propertyNames,
                              sizeofSampledMethodDesc);
  }
  return rc;
}
