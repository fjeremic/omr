...
static uintptr_t
omrtiGetMemoryCategoriesCallback(uint32_t categoryCode,
...
static uintptr_t
omrtiCountMemoryCategoriesCallback(uint32_t categoryCode,
                                   const char* categoryName,
                                   uintptr_t liveBytes,
                                   uintptr_t liveAllocations,
                                   BOOLEAN isRoot,
                                   ...
                                   OMRMemCategoryWalkState* state);
static uintptr_t
omrtiCalculateSlotsForCategoriesMappingCallback(uint32_t categoryCode,
                                                const char* categoryName,
                                                uintptr_t liveBytes,
                                                uintptr_t liveAllocations,
                                                BOOLEAN isRoot,
...
calculateProcessCpuLoad(OMRSysInfoProcessCpuTime const* endRecord,
                        ...
                        double* cpuLoad);
static omr_error_t
getOMRSysInfoSystemCpuTime(OMR_VM* omrVM,
...
omr_error_t
omrtiBindCurrentThread(OMR_VM* vm,
...
omr_error_t
omrtiRegisterRecordSubscriber(OMR_VMThread* vmThread,
  ...
  if (NULL == vmThread) {
    ...
    if (NULL == serverf) {
      ...
      rc = serverf->RegisterRecordSubscriber(
        OMR_TRACE_THREAD_FROM_VMTHREAD(vmThread),
        description,
        subscriberFunc,
        alarmFunc,
        userData,
...
omr_error_t
omrtiDeregisterRecordSubscriber(OMR_VMThread* vmThread,
                                UtSubscription* subscriptionID)
{
  omr_error_t rc = OMR_ERROR_NONE;
                       ...
                       double* cpuLoad)
{
  ...
  if (timestampDelta < 0) {
    ...
    double result =
      (double)cpuTimeDelta / (double)(endRecord->numberOfCpus * timestampDelta);
    /* cpuload is in [0, 1] */
...
omrtiGetSystemCpuLoad(OMR_VMThread* vmThread, double* systemCpuLoad)
{
  ...
  if (NULL == vmThread) {
    ...
    if ((SUPPORTED == curSysInfo->systemCpuLoadCallStatus) ||
      ...
      ret = getOMRSysInfoSystemCpuTime(vmThread->_vm,
                                       &newestSystemCpuTime,
      ...
      if (OMR_ERROR_NONE != ret) {
        ...
        memcpy(&curSysInfo->interimSystemCpuTime,
               &newestSystemCpuTime,
               sizeof(J9SysinfoCPUTime));
...
omrtiGetProcessCpuLoad(OMR_VMThread* vmThread, double* processCpuLoad)
{
  ...
  if (NULL == vmThread) {
    ...
    if ((SUPPORTED == curSysInfo->processCpuLoadCallStatus) ||
      ...
      ret = getOMRSysInfoProcessCpuTime(vmThread->_vm,
      ...
      if (OMR_ERROR_NONE != ret) {
        ...
        memcpy(&curSysInfo->interimProcessCpuTime,
               &newestProcessCpuTime,
               sizeof(OMRSysInfoProcessCpuTime));
...
omr_error_t
omrtiGetMemoryCategories(OMR_VMThread* vmThread,
                         int32_t max_categories,
                         OMR_TI_MemoryCategory* categories_buffer,
                         ...
                         int32_t* total_categories_ptr)
{
  ...
  if (NULL == vmThread) {
    ...
    Trc_OMRTI_omrtiGetMemoryCategories_Entry(vmThread,
                                             max_categories,
                                             categories_buffer,
                                             written_count_ptr,
    ...
    if (categories_buffer) {
      ...
      memset(userData.categories_mapping,
             0,
             userData.categories_mapping_size * sizeof(OMR_TI_MemoryCategory*));
      memset(
        categories_buffer, 0, max_categories * sizeof(OMR_TI_MemoryCategory));
...
omr_error_t
omrtiGetMethodDescriptions(OMR_VMThread* vmThread,
                           void** methodArray,
                           ...
                           size_t* nameBytesRemaining)
{
  ...
  if (NULL == vmThread) {
    ...
    rc = dictionary->getEntries(vmThread,
                                methodArray,
                                methodArrayCount,
                                methodDescriptions,
                                nameBuffer,
 ...
 */
omr_error_t
omrtiGetMethodProperties(OMR_VMThread* vmThread,
                         ...
                         size_t* sizeofSampledMethodDesc)
{
  ...
  if (NULL == vmThread) {
    ...
    dictionary->getProperties(
      numProperties, propertyNames, sizeofSampledMethodDesc);
  }
  return rc;
}
