...
static OMR_ThreadAPI const theOmrThreadAPI = {omrthread_create,
...
extern "C" OMR_Agent *omr_agent_create(OMR_VM *vm, char const *arg) {
  return OMR_Agent::createAgent(vm, arg);
}

extern "C" void omr_agent_destroy(OMR_Agent *agent) {
  OMR_Agent::destroyAgent(agent);
}

extern "C" omr_error_t omr_agent_openLibrary(OMR_Agent *agent) {
  return agent->openLibrary();
}

extern "C" omr_error_t omr_agent_callOnLoad(OMR_Agent *agent) {
  return agent->callOnLoad();
}

extern "C" omr_error_t omr_agent_callOnUnload(OMR_Agent *agent) {
  return agent->callOnUnload();
}

extern "C" OMR_TI const *omr_agent_getTI(void) { return &OMR_Agent::theOmrTI; }

OMR_Agent::OMR_Agent(OMR_VM *vm, char const *arg)
    : _handle(0), _vm(vm), _onload(NULL), _onunload(NULL), _dllpath(NULL),
      _options(NULL), _state(UNINITIALIZED) {
...
OMR_Agent *OMR_Agent::createAgent(OMR_VM *vm, char const *arg) {
  OMR_Agent *newAgent = NULL;

  if (NULL != arg) {
    OMRPORT_ACCESS_FROM_OMRVM(vm);
    newAgent = static_cast<OMR_Agent *>(
        omrmem_allocate_memory(sizeof(*newAgent), OMRMEM_CATEGORY_VM));
    if (NULL != newAgent) {
...
void OMR_Agent::destroyAgent(OMR_Agent *agent) {
  if (NULL != agent) {
...
omr_error_t OMR_Agent::openLibrary(void) {
  ...
  if (INITIALIZED != _state) {
    ...
    portRc = omrsl_open_shared_library((char *)_dllpath, &_handle,
    ...
    if (OMR_ERROR_NONE == rc) {
      portRc = omrsl_lookup_name(_handle, (char *)"OMRAgent_OnLoad",
                                 (uintptr_t *)&_onload, (char *)"IPPPP");
      if (0 != portRc) {
    ...
    if (OMR_ERROR_NONE == rc) {
      portRc = omrsl_lookup_name(_handle, (char *)"OMRAgent_OnUnload",
                                 (uintptr_t *)&_onunload, (char *)"IPP");
      if (0 != portRc) {
...
omr_error_t OMR_Agent::callOnLoad(void) {
...
omr_error_t OMR_Agent::callOnUnload(void) {
...
omr_error_t OMR_Agent::callOnPreFork(void) {
  return _agentCallbacks.onPreFork();
}

omr_error_t OMR_Agent::callOnPostForkParent(void) {
  return _agentCallbacks.onPostForkParent();
}

omr_error_t OMR_Agent::callOnPostForkChild(void) {
  return _agentCallbacks.onPostForkChild();
}

omr_error_t OMR_Agent::onPreForkDefault(void) { return OMR_ERROR_NONE; }

omr_error_t OMR_Agent::onPostForkParentDefault(void) { return OMR_ERROR_NONE; }

omr_error_t OMR_Agent::onPostForkChildDefault(void) { return OMR_ERROR_NONE; }
