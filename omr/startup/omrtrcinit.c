 ...
 *	 - e.g. TRACEDEBUG=1
 *	 - Support for this env var is enabled by the compile flag
 *OMR_TRACE_ENABLE_DEBUG.
 */

static omr_error_t freeTraceOptions(OMRPortLibrary *portLibrary,
                                    char *trcOpts[]);
static omr_error_t getEnvVar(OMRPortLibrary *portLibrary, const char *envVar,
                             char *buf, size_t bufLen,
                             const char **envVarValue);
static BOOLEAN keyNotNull(const char *key);
static omr_error_t parseTraceOptions(OMRPortLibrary *portLibrary,
                                     char *trcOpts[],
                                     const char *traceOptString);
static void reportOMRCommandLineError(OMRPortLibrary *portLibrary,
                                      const char *detailStr, va_list args);
...
omr_error_t omr_ras_initTraceEngine(OMR_VM *omrVM, const char *traceOptString,
  ...
  omrport_control(OMRPORT_CTLDATA_TRACE_START,
                  (uintptr_t)omrVM->_trcEngine->utIntf);

  /* Load module for thread library tracepoints */
  omrthread_lib_control(J9THREAD_LIB_CONTROL_TRACE_START,
                        (uintptr_t)omrVM->_trcEngine->utIntf);

  /* Load module for hookable library tracepoints */
  omrhook_lib_control(J9HOOK_LIB_CONTROL_TRACE_START,
                      (uintptr_t)omrVM->_trcEngine->utIntf);
...
static omr_error_t parseTraceOptions(OMRPortLibrary *portLibrary,
  ...
  if ((NULL != traceOptString) && ('\0' != *traceOptString)) {
    ...
    copyOfOptString =
        omrmem_allocate_memory(strlen(firstKey) + 1, OMRMEM_CATEGORY_TRACE);
    ...
    /* trcOpts[0] must point to the start of allocated memory so that
     * freeTraceOptions() works correctly */
    key = copyOfOptString;
    /* assert('=' != *key); */
    ...
    while (keyNotNull(key)) {
      if ('=' == *key) {
        ...
        /* ensure we have enough room in the trcOpts array for a key/value pair
...
#if OMR_TRACE_ENABLE_DEBUG
  if (OMR_ERROR_NONE == rc) {
    /* Trace assertions can't be used here because the trace engine has not been
     * started yet. */
    const char *traceDebugEnv = NULL;
    char traceDebugBuf[2] = "";

    if (OMR_ERROR_NONE == getEnvVar(OMRPORTLIB, "TRACEDEBUG", traceDebugBuf,
                                    sizeof(traceDebugBuf), &traceDebugEnv)) {
      if ((NULL != traceDebugEnv) && ('1' == traceDebugEnv[0]) &&
          ('\0' == traceDebugEnv[1])) {
        for (i = 0; i < optIndex; i += 2) {
          omrtty_printf("%d: key = %s\n", i,
                        (NULL == trcOpts[i]) ? NULL : trcOpts[i]);
          ...
          assert((NULL ==
                  trcOpts[i + 1]) /* value must be NULL or non-empty string */
                 || ('\0' != trcOpts[i + 1][0]));
        }

        assert(
            NULL ==
            trcOpts[optIndex]); /* The last element of trcOpts must be NULL */
