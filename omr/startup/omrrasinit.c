 ...
 */
OMRMEM_CATEGORY_3_CHILDREN("RUNTIME", OMR_MEM_CATEGORY_RUNTIME,
                           OMRMEM_CATEGORY_VM, OMRMEM_CATEGORY_JIT,
                           OMRMEM_CATEGORY_UNKNOWN);
#if defined(OMR_OPT_CUDA)
OMRMEM_CATEGORY_6_CHILDREN("VM", OMRMEM_CATEGORY_VM, OMRMEM_CATEGORY_MM,
                           OMRMEM_CATEGORY_THREADS, OMRMEM_CATEGORY_CUDA,
                           OMRMEM_CATEGORY_PORT_LIBRARY, OMRMEM_CATEGORY_TRACE,
                           OMRMEM_CATEGORY_OMRTI);
#else
OMRMEM_CATEGORY_5_CHILDREN("VM", OMRMEM_CATEGORY_VM, OMRMEM_CATEGORY_MM,
                           OMRMEM_CATEGORY_THREADS,
                           OMRMEM_CATEGORY_PORT_LIBRARY, OMRMEM_CATEGORY_TRACE,
                           OMRMEM_CATEGORY_OMRTI);
#endif /* OMR_OPT_CUDA */
OMRMEM_CATEGORY_1_CHILD("Memory Manager (GC)", OMRMEM_CATEGORY_MM,
                        OMRMEM_CATEGORY_MM_RUNTIME_HEAP);
...
OMRMEM_CATEGORY_2_CHILDREN("JIT", OMRMEM_CATEGORY_JIT,
                           OMRMEM_CATEGORY_JIT_CODE_CACHE,
                           OMRMEM_CATEGORY_JIT_DATA_CACHE);
...
omr_error_t omr_ras_initMemCategories(OMRPortLibrary *portLibrary) {
  /*
   * Every category used by the application, except UNKNOWN, must have an entry
   * in the table. The port library will supplement this set with internal port
   * library categories. Space must be reserved for the thread library to insert
   * its four categories.
   *
   * This table should not be a global variable because it is modified by
   * omrthread_lib_control(), and the modifications should not be visible across
  ...
  OMRMemCategorySet memCategorySet = {
      sizeof(memCategories) / sizeof(memCategories[0]), memCategories};
  omr_error_t rc = OMR_ERROR_NONE;
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);

  if (OMR_ERROR_NONE == rc) {
    if (0 != omrthread_lib_control(J9THREAD_LIB_CONTROL_GET_MEM_CATEGORIES,
                                   (uintptr_t)&memCategorySet)) {
      rc = OMR_ERROR_INTERNAL;
    }
  }
  if (OMR_ERROR_NONE == rc) {
    if (0 != omrport_control(OMRPORT_CTLDATA_MEM_CATEGORIES_SET,
                             (uintptr_t)&memCategorySet)) {
      rc = OMR_ERROR_INTERNAL;
    }
  }
...
omr_error_t omr_ras_initTI(OMR_VM *vm) {
  ...
  if (NULL == vm->_omrTIAccessMutex) {
    if (0 != omrthread_monitor_init_with_name(&vm->_omrTIAccessMutex, 0,
                                              "OMRTI access mutex")) {
  ...
  if (OMR_ERROR_NONE == rc) {
    if (NULL == vm->sysInfo) {
      vm->sysInfo = (OMR_SysInfo *)omrmem_allocate_memory(
          sizeof(OMR_SysInfo), OMRMEM_CATEGORY_OMRTI);
      if (NULL != vm->sysInfo) {
        if ((0 !=
             omrthread_monitor_init_with_name(&vm->sysInfo->syncProcessCpuLoad,
                                              0, "syncProcessCpuLoad")) ||
            (0 !=
             omrthread_monitor_init_with_name(&vm->sysInfo->syncSystemCpuLoad,
                                              0, "syncSystemCpuLoad"))) {
          ...
          memset(&vm->sysInfo->oldestProcessCpuTime, 0,
                 sizeof(OMRSysInfoProcessCpuTime));

          vm->sysInfo->systemCpuLoadCallStatus = NO_HISTORY;
          vm->sysInfo->systemCpuTimeNegativeElapsedTimeCount = 0;
          memset(&vm->sysInfo->interimSystemCpuTime, 0,
                 sizeof(J9SysinfoCPUTime));
          memset(&vm->sysInfo->oldestSystemCpuTime, 0,
                 sizeof(J9SysinfoCPUTime));
        }
      } else {
