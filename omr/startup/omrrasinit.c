 ...
 */
OMRMEM_CATEGORY_3_CHILDREN("RUNTIME",
                           OMR_MEM_CATEGORY_RUNTIME,
                           OMRMEM_CATEGORY_VM,
                           OMRMEM_CATEGORY_JIT,
                           OMRMEM_CATEGORY_UNKNOWN);
#if defined(OMR_OPT_CUDA)
OMRMEM_CATEGORY_6_CHILDREN("VM",
                           OMRMEM_CATEGORY_VM,
                           OMRMEM_CATEGORY_MM,
...
OMRMEM_CATEGORY_5_CHILDREN("VM",
                           ...
                           OMRMEM_CATEGORY_OMRTI);
#endif /* OMR_OPT_CUDA */
OMRMEM_CATEGORY_1_CHILD("Memory Manager (GC)",
                        OMRMEM_CATEGORY_MM,
...
OMRMEM_CATEGORY_2_CHILDREN("JIT",
                           OMRMEM_CATEGORY_JIT,
...
omr_error_t
omr_ras_initHealthCenter(OMR_VM* omrVM,
                         OMR_Agent** hc,
...
omr_ras_initTI(OMR_VM* vm)
{
  ...
  if (NULL == vm->_omrTIAccessMutex) {
    if (0 != omrthread_monitor_init_with_name(
               &vm->_omrTIAccessMutex, 0, "OMRTI access mutex")) {
      rc = OMR_ERROR_FAILED_TO_ALLOCATE_MONITOR;
    }
  }
  if (OMR_ERROR_NONE == rc) {
    if (NULL == vm->sysInfo) {
      vm->sysInfo = (OMR_SysInfo*)omrmem_allocate_memory(sizeof(OMR_SysInfo),
                                                         OMRMEM_CATEGORY_OMRTI);
      if (NULL != vm->sysInfo) {
        if ((0 !=
             omrthread_monitor_init_with_name(
               &vm->sysInfo->syncProcessCpuLoad, 0, "syncProcessCpuLoad")) ||
            (0 != omrthread_monitor_init_with_name(
          ...
          memset(&vm->sysInfo->interimProcessCpuTime,
                 0,
                 sizeof(OMRSysInfoProcessCpuTime));
          memset(&vm->sysInfo->oldestProcessCpuTime,
                 0,
                 sizeof(OMRSysInfoProcessCpuTime));
          ...
          memset(
            &vm->sysInfo->interimSystemCpuTime, 0, sizeof(J9SysinfoCPUTime));
          memset(
            &vm->sysInfo->oldestSystemCpuTime, 0, sizeof(J9SysinfoCPUTime));
        }
      } else {
