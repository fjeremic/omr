  ...
  if (NULL == currentThread) {
    ...
    if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
      rc = OMR_Thread_FirstInit(
        omrVM, self, language_vm_thread, threadSlot, threadName);
      if (OMR_ERROR_NONE != rc) {
        omrthread_detach(self);
      }
    } else {
...
omr_error_t
OMR_Thread_FirstInit(OMR_VM* omrVM,
                     omrthread_t self,
                     void* language_vm_thread,
                     OMR_VMThread** threadSlot,
                     const char* threadName)
{
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    if (NULL != threadName) {
      setOMRVMThreadNameWithFlag(
        currentThread, currentThread, (char*)threadName, TRUE);
    }
...
omr_error_t
OMR_Initialize_VM(OMR_VM** omrVMSlot,
                  ...
                  void* languageVMThread)
{
  ...
  *omrVMThreadSlot = vmThread;
...
#if defined(OMR_RAS_TDF_TRACE)
  {
    ...
    rc =
      omr_ras_initMethodDictionary(omrVM,
