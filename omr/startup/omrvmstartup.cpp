...
omr_error_t OMR_Initialize(void *languageVM, OMR_VM **vmSlot) {
  ...
  omrRuntime = (OMR_Runtime *)omrmem_allocate_memory(sizeof(OMR_Runtime),
                                                     OMRMEM_CATEGORY_VM);
  if (NULL == omrRuntime) {
...
omr_error_t OMR_Shutdown(OMR_VM *omrVM) {
  ...
  if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
    ...
    rc = OMR_ERROR_FAILED_TO_ATTACH_NATIVE_THREAD;
    fprintf(stderr, "OMR_Shutdown: Failed to attach to omrthread. Proceeding "
                    "without cleaning up.\n");
  }
  return rc;
}
...
omr_error_t OMR_Thread_Init(OMR_VM *omrVM, void *language_vm_thread,
  ...
  if (NULL == currentThread) {
    omrthread_t self = NULL;
    if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
      rc = OMR_Thread_FirstInit(omrVM, self, language_vm_thread, threadSlot,
                                threadName);
      if (OMR_ERROR_NONE != rc) {
        omrthread_detach(self);
      }
...
omr_error_t OMR_Thread_FirstInit(OMR_VM *omrVM, omrthread_t self,
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    if (NULL != threadName) {
      setOMRVMThreadNameWithFlag(currentThread, currentThread,
                                 (char *)threadName, TRUE);
    }
  ...
  if (NULL == omrVMThread) {
    ...
    if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
      ...
      if (OMR_ERROR_NONE == rc) {
        ...
        /* if (!selfIsCurrent), the runtime must have omrthread_detach()ed when
         * the pthread exited. */
      }
...
omr_error_t OMR_Thread_LastFree(OMR_VMThread *omrVMThread) {
  ...
  if (NULL == omrVMThread) {
    ...
    /* OMRTODO
     * We can't make the call to omr_trc_stopThreadTrace() conditional on
     * whether omrVM->_trcEngine is NULL, because omrVM->_trcEngine might be
     * NULL if the trace engine was already shutdown. However, the current
     * thread must still be detached from the trace engine, to execute final
     * cleanup.
     *
...
omr_error_t OMR_Initialize_VM(OMR_VM **omrVMSlot,
  ...
  *omrVMSlot = omrVM;
...
#if defined(OMR_GC)
  {
    ...
    if (OMR_ERROR_NONE != rc) {
      omrtty_printf("Failed to start up Heap Memory Management and Garbage "
                    "Collector, rc=%d.\n",
                    rc);
      goto failed;
    }
  ...
  *omrVMThreadSlot = vmThread;
...
#if defined(OMR_RAS_TDF_TRACE)
  {
    ...
    rc = omr_ras_initMethodDictionary(
        omrVM, OMR_Glue_GetMethodDictionaryPropertyNum(),
...
omr_error_t OMR_Shutdown_VM(OMR_VM *omrVM, OMR_VMThread *omrVMThread) {
  ...
  if (NULL != omrVMThread) {
    ...
    fprintf(stderr,
            "No OMR VMThread so skipping shutdown phases that required an "
            "attached thread, rc=%d.\n",
            rc);
    fflush(stderr);
  }
  ...
  if (NULL != omrVM) {
    ...
    fprintf(stderr,
            "No OMR VM so skipping shutdown phases that required an "
            "initialized VM, rc=%d.\n",
            rc);
    fflush(stderr);
  }
