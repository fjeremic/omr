...
extern "C" omr_error_t omrtiGetFreePhysicalMemorySize(OMR_VMThread* vmThread, uint64_t* freePhysicalMemorySize)
{
...
extern "C" omr_error_t omrtiGetProcessVirtualMemorySize(OMR_VMThread* vmThread, uint64_t* processMemorySize)
{
  omr_error_t rc = getProcessMemorySizeHelper(vmThread, OMRPORT_VMEM_PROCESS_VIRTUAL, processMemorySize);
  return rc;
}

extern "C" omr_error_t omrtiGetProcessPhysicalMemorySize(OMR_VMThread* vmThread, uint64_t* processMemorySize)
{
  omr_error_t rc = getProcessMemorySizeHelper(vmThread, OMRPORT_VMEM_PROCESS_PHYSICAL, processMemorySize);
  return rc;
}

extern "C" omr_error_t omrtiGetProcessPrivateMemorySize(OMR_VMThread* vmThread, uint64_t* processMemorySize)
{
