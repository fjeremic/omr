...
class OMR_MethodDictionary {
 ...
 private:
  ...
  uint32_t _maxEntries; /* # of entries in the hash table when _maxBytes was
                           achieved */
  OMR_VM *_vm;
  size_t _numProperties;
  const char *const *_propertyNames;
  uint32_t _sizeofEntry; /* size of a method dictionary entry in bytes, not
   ...
   */
 public:
  omr_error_t init(OMR_VM *vm, size_t numProperties,
                   const char *const *propertyNames);
  void cleanup();
  omr_error_t insert(OMR_MethodDictionaryEntry *entry);
  omr_error_t getEntries(OMR_VMThread *vmThread, void **methodArray,
                         size_t methodArrayCount,
                         OMR_SampledMethodDescription *methodDescriptions,
                         char *nameBuffer, size_t nameBytes,
                         size_t *firstRetryMethod, size_t *nameBytesRemaining);
  void getProperties(size_t *numProperties, const char *const **propertyNames,
                     size_t *sizeofSampledMethodDesc) const;
  void print();

 protected:
 private:
  bool entryValueEquals(const OMR_MethodDictionaryEntry *e1,
                        const OMR_MethodDictionaryEntry *e2);
  omr_error_t dupEntryStrings(OMR_MethodDictionaryEntry *dest,
  ...
  void copyEntryNameBytes(OMR_MethodDictionaryEntry *entry,
                          OMR_SampledMethodDescription *desc,
                          char *nameBufferPos) const;

  static uintptr_t entryHash(void *entry, void *userData);
  static uintptr_t entryEquals(void *leftEntry, void *rightEntry,
                               void *userData);

  void formatEntryProperties(const OMR_MethodDictionaryEntry *entry,
                             char **str) const;
  void traceInsertEntrySuccess(const OMR_MethodDictionaryEntry *newEntry) const;
  void traceInsertEntryFailed(omr_error_t rc,
