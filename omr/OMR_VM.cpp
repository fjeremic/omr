...
omr_error_t omr_attach_vm_to_runtime(OMR_VM* vm) {
  ...
  if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
    if ((0 == omrthread_tls_alloc(&vm->_vmThreadKey)) &&
        (0 == omrthread_monitor_init_with_name(&vm->_vmThreadListMutex, 0,
                                               "OMR VM thread list mutex"))) {
...
omr_error_t attachThread(OMR_VM* vm, OMR_VMThread* vmthread) {
  ...
  if (internal || (0 == maximum_thread_count) ||
      (vm->_languageThreadCount < maximum_thread_count)) {
...
void omr_vm_preFork(OMR_VM* omrVM) {
  omrthread_t self = NULL;

  /* We cannot take (and hold) a lock unless the thread is attached. Being
   * unattached is fatal. */
...
void omr_vm_postForkParent(OMR_VM* omrVM) {
...
void omr_vm_postForkChild(OMR_VM* omrVM) {
   ...
   *
   * vmThreads hold references to pthread sync objects that may have been
   * corrupted by forking, such as the mutex protecting the threadName. The
   * memory for those sync objects can't be reused, so they must not be freed.
   * Destroying a vmThread normally frees them. We can destroy vmThreads
   * normally here because we have already run omrthread_lib_postForkChild() to
   * remove the references to these sync objects.
   *
   * If vmThread cleanup is moved prior to omrthread_lib_postForkChild(), we
   * must ensure that pthread sync objects are not freed if they can't be
   * reused.
   */
  OMR_VMThread* currentVMThread = NULL;
  OMR_VMThread* walkThread = J9_LINKED_LIST_START_DO(omrVM->_vmThreadList);
  while (NULL != walkThread) {
    OMR_VMThread* nextThread =
        J9_LINKED_LIST_NEXT_DO(omrVM->_vmThreadList, walkThread);
