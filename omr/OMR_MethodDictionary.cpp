...
omr_error_t OMR_MethodDictionary::init(OMR_VM* vm, size_t numProperties, const char* const* propertyNames)
{
  _lock = NULL;
  _hashTable = NULL;
  _currentBytes = 0;
  ...
  if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
    ...
    _hashTable = hashTableNew(
      OMRPORTLIB, OMR_GET_CALLSITE(), 0, _sizeofEntry, 0, 0, OMRMEM_CATEGORY_OMRTI, entryHash, entryEquals, NULL, NULL);
    if (NULL != _hashTable) {
  ...
  if (NULL != _vm) {
    ...
    if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
      Trc_OMRPROF_methodDictionaryHighWaterMark(
        _maxBytes, _maxEntries, _sizeofEntry, _maxBytes - (_maxEntries * _sizeofEntry));
      if (NULL != _hashTable) {
...
void OMR_MethodDictionary::getProperties(
  size_t* numProperties, const char* const** propertyNames, size_t* sizeofSampledMethodDesc) const
{
...
omr_error_t OMR_MethodDictionary::getEntries(OMR_VMThread* vmThread, void** methodArray, size_t methodArrayCount,
  ...
  size_t* nameBytesRemaining)
{
  ...
  if (0 == omrthread_monitor_enter(_lock)) {
    ...
    for (size_t i = 0; i < methodArrayCount; i++) {
      ...
      currentMthDesc = (OMR_SampledMethodDescription*)((uintptr_t)currentMthDesc + sizeof(OMR_SampledMethodDescription)
...
size_t OMR_MethodDictionary::countEntryNameBytesNeeded(OMR_MethodDictionaryEntry* entry) const
{
...
void OMR_MethodDictionary::copyEntryNameBytes(
  OMR_MethodDictionaryEntry* entry, OMR_SampledMethodDescription* desc, char* nameBufferPos) const
{
  for (size_t i = 0; i < _numProperties; ++i) {
    if (NULL == entry->propertyValues[i]) {
 ...
 */
uintptr_t OMR_MethodDictionary::entryEquals(void* leftEntry, void* rightEntry, void* userData)
{
...
uintptr_t OMR_MethodDictionary::cleanupEntryStrings(void* entry, void* userData)
{
...
omr_error_t OMR_MethodDictionary::dupEntryStrings(OMR_MethodDictionaryEntry* dest, const OMR_MethodDictionaryEntry* src)
{
  omr_error_t rc = OMR_ERROR_NONE;
 ...
 */
bool OMR_MethodDictionary::entryValueEquals(const OMR_MethodDictionaryEntry* e1, const OMR_MethodDictionaryEntry* e2)
{
...
void OMR_MethodDictionary::formatEntryProperties(const OMR_MethodDictionaryEntry* entry, char** str) const
{
  if (_numProperties == 0) {
    *str = NULL;
  } else {
...
void OMR_MethodDictionary::traceInsertEntrySuccess(const OMR_MethodDictionaryEntry* newEntry) const
{
...
void OMR_MethodDictionary::traceInsertEntryFailed(omr_error_t rc, const OMR_MethodDictionaryEntry* newEntry) const
{
...
void OMR_MethodDictionary::traceInsertEntryReplace(const OMR_MethodDictionaryEntry* newEntry) const
{
