...
omr_error_t omr_ras_initMethodDictionary(OMR_VM *vm, size_t numProperties,
  ...
  if (NULL == vm->_methodDictionary) {
    ...
    OMR_MethodDictionary *methodDictionary =
        (OMR_MethodDictionary *)omrmem_allocate_memory(
            sizeof(*methodDictionary), OMRMEM_CATEGORY_OMRTI);
...
omr_error_t OMR_MethodDictionary::init(OMR_VM *vm, size_t numProperties,
                                       const char *const *propertyNames) {
  _lock = NULL;
  _hashTable = NULL;
  _currentBytes = 0;
  ...
  _sizeofEntry = (uint32_t)(sizeof(OMR_MethodDictionaryEntry) +
                            (_numProperties * sizeof(char *)));
  ...
  if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
    OMRPORT_ACCESS_FROM_OMRVM(vm);
    _hashTable =
        hashTableNew(OMRPORTLIB, OMR_GET_CALLSITE(), 0, _sizeofEntry, 0, 0,
                     OMRMEM_CATEGORY_OMRTI, entryHash, entryEquals, NULL, NULL);
    if (NULL != _hashTable) {
      if (0 != omrthread_monitor_init_with_name(&_lock, 0,
                                                "omrVM->_methodDictionary")) {
...
void OMR_MethodDictionary::cleanup() {
  if (NULL != _vm) {
    omrthread_t self = NULL;
    if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
      Trc_OMRPROF_methodDictionaryHighWaterMark(
          _maxBytes, _maxEntries, _sizeofEntry,
          _maxBytes - (_maxEntries * _sizeofEntry));
      if (NULL != _hashTable) {
        hashTableForEachDo(_hashTable,
                           OMR_MethodDictionary::cleanupEntryStrings, this);
...
void OMR_MethodDictionary::getProperties(
    size_t *numProperties, const char *const **propertyNames,
    size_t *sizeofSampledMethodDesc) const {
  *numProperties = _numProperties;
  *propertyNames = _propertyNames;
  *sizeofSampledMethodDesc = sizeof(OMR_SampledMethodDescription) +
                             sizeof(const char *) * _numProperties;
}

omr_error_t OMR_MethodDictionary::insert(OMR_MethodDictionaryEntry *entry) {
  omr_error_t rc = OMR_ERROR_NONE;
  omrthread_t self = NULL;
  if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
...
omr_error_t OMR_MethodDictionary::getEntries(
  ...
  if (0 == omrthread_monitor_enter(_lock)) {
    ...
    /*
     * Collect method descriptions until we encounter a description that can't
     * fit into the available nameBuffer. At this point, flip to RETRY mode:
     * Stop collecting names, and only count the remaining space required.
     */

    for (size_t i = 0; i < methodArrayCount; i++) {
      OMR_MethodDictionaryEntry
          searchEntryHdr; /* NOTE This is only an entry header, and can't hold
                             any propertyValues */
      OMR_MethodDictionaryEntry *entry = NULL;

      searchEntryHdr.key = methodArray[i];
      entry = (OMR_MethodDictionaryEntry *)hashTableFind(_hashTable,
                                                         &searchEntryHdr);
      if (NULL == entry) {
      ...
      currentMthDesc =
          (OMR_SampledMethodDescription *)((uintptr_t)currentMthDesc +
                                           sizeof(
                                               OMR_SampledMethodDescription) +
                                           sizeof(const char *) *
                                               _numProperties);
    }
    if (OMR_ERROR_RETRY == rc) {
...
size_t OMR_MethodDictionary::countEntryNameBytesNeeded(
    OMR_MethodDictionaryEntry *entry) const {
...
void OMR_MethodDictionary::copyEntryNameBytes(
    OMR_MethodDictionaryEntry *entry, OMR_SampledMethodDescription *desc,
 ...
 *
 * Don't access any fields from the flexible array member of
 * OMR_MethodDictionaryEntry. They might not be allocated for the input entry.
 */
uintptr_t OMR_MethodDictionary::entryHash(void *entry, void *userData) {
 ...
 *
 * Don't access any fields from the flexible array member of
 * OMR_MethodDictionaryEntry. They might not be allocated for the input entries.
 *
 * @see OMR_MethodDictionary::entryValueEquals()
 */
uintptr_t OMR_MethodDictionary::entryEquals(void *leftEntry, void *rightEntry,
                                            void *userData) {
...
uintptr_t OMR_MethodDictionary::cleanupEntryStrings(void *entry,
                                                    void *userData) {
...
omr_error_t OMR_MethodDictionary::dupEntryStrings(
    OMR_MethodDictionaryEntry *dest, const OMR_MethodDictionaryEntry *src) {
  omr_error_t rc = OMR_ERROR_NONE;
 ...
 */
bool OMR_MethodDictionary::entryValueEquals(
    const OMR_MethodDictionaryEntry *e1, const OMR_MethodDictionaryEntry *e2) {
...
void OMR_MethodDictionary::formatEntryProperties(
    const OMR_MethodDictionaryEntry *entry, char **str) const {
  if (_numProperties == 0) {
    *str = NULL;
  } else {
    OMRPORT_ACCESS_FROM_OMRVM(_vm);
    size_t buflen =
        _numProperties; /* space between each property + nul-terminator, might
                           include some extra if we have nulls */
    for (size_t i = 0; i < _numProperties; ++i) {
      if (NULL != entry->propertyValues[i]) {
    ...
    if (NULL != buf) {
      bufUsed +=
          omrstr_printf(buf, (uintptr_t)buflen, "%s", entry->propertyValues[0]);
      for (size_t i = 1; i < _numProperties; ++i) {
        if (NULL != entry->propertyValues[i]) {
          bufUsed += omrstr_printf(buf + bufUsed, (uintptr_t)buflen - bufUsed,
                                   " %s", entry->propertyValues[i]);
        }
      }
    }
    *str = buf;
...
void OMR_MethodDictionary::traceInsertEntrySuccess(
    const OMR_MethodDictionaryEntry *newEntry) const {
...
void OMR_MethodDictionary::traceInsertEntryFailed(
    omr_error_t rc, const OMR_MethodDictionaryEntry *newEntry) const {
...
void OMR_MethodDictionary::traceInsertEntryReplace(
    const OMR_MethodDictionaryEntry *newEntry) const {
...
void OMR_MethodDictionary::print() {
  OMRPORT_ACCESS_FROM_OMRVM(_vm);
...
uintptr_t OMR_MethodDictionary::printEntry(void *entry, void *userData) {
  OMR_MethodDictionary *md = (OMR_MethodDictionary *)userData;
  OMR_MethodDictionaryEntry *mdEntry = (OMR_MethodDictionaryEntry *)entry;
  OMRPORT_ACCESS_FROM_OMRVM(md->_vm);
