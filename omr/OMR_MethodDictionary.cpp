...
omr_ras_initMethodDictionary(OMR_VM* vm, size_t numProperties, const char* const* propertyNames)
{
    ...
    if (NULL == vm->_methodDictionary) {
        ...
        OMR_MethodDictionary* methodDictionary = (OMR_MethodDictionary*)omrmem_allocate_memory(sizeof(*methodDictionary), OMRMEM_CATEGORY_OMRTI);
...
void OMR_MethodDictionary::getProperties(size_t* numProperties, const char* const** propertyNames, size_t* sizeofSampledMethodDesc) const
{
...
void OMR_MethodDictionary::copyEntryNameBytes(OMR_MethodDictionaryEntry* entry, OMR_SampledMethodDescription* desc, char* nameBufferPos) const
{
    for (size_t i = 0; i < _numProperties; ++i) {
        if (NULL == entry->propertyValues[i]) {
