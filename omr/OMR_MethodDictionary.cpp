...
omr_error_t
omr_ras_initMethodDictionary(OMR_VM* vm,
...
omr_error_t
OMR_MethodDictionary::init(OMR_VM* vm,
                           size_t numProperties,
                           const char* const* propertyNames)
{
  _lock = NULL;
  ...
  if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
    OMRPORT_ACCESS_FROM_OMRVM(vm);
    _hashTable = hashTableNew(OMRPORTLIB,
                              OMR_GET_CALLSITE(),
                              0,
                              _sizeofEntry,
                              0,
    ...
    if (NULL != _hashTable) {
      if (0 != omrthread_monitor_init_with_name(
                 &_lock, 0, "omrVM->_methodDictionary")) {
...
OMR_MethodDictionary::cleanup()
{
  if (NULL != _vm) {
    omrthread_t self = NULL;
    if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
      Trc_OMRPROF_methodDictionaryHighWaterMark(_maxBytes,
                                                _maxEntries,
                                                _sizeofEntry,
                                                _maxBytes -
                                                  (_maxEntries * _sizeofEntry));
      if (NULL != _hashTable) {
        hashTableForEachDo(
          _hashTable, OMR_MethodDictionary::cleanupEntryStrings, this);
...
void
OMR_MethodDictionary::getProperties(size_t* numProperties,
                                    const char* const** propertyNames,
                                    size_t* sizeofSampledMethodDesc) const
{
  ...
  *sizeofSampledMethodDesc =
    sizeof(OMR_SampledMethodDescription) + sizeof(const char*) * _numProperties;
}
...
omr_error_t
OMR_MethodDictionary::getEntries(
  ...
  if (0 == omrthread_monitor_enter(_lock)) {
    ...
    for (size_t i = 0; i < methodArrayCount; i++) {
      OMR_MethodDictionaryEntry
        searchEntryHdr; /* NOTE This is only an entry header, and can't hold any
                           propertyValues */
      OMR_MethodDictionaryEntry* entry = NULL;

      searchEntryHdr.key = methodArray[i];
      entry =
        (OMR_MethodDictionaryEntry*)hashTableFind(_hashTable, &searchEntryHdr);
      if (NULL == entry) {
...
size_t
OMR_MethodDictionary::countEntryNameBytesNeeded(
...
void
OMR_MethodDictionary::copyEntryNameBytes(OMR_MethodDictionaryEntry* entry,
 ...
 */
uintptr_t
OMR_MethodDictionary::entryEquals(void* leftEntry,
                                  void* rightEntry,
                                  void* userData)
{
 ...
 */
bool
OMR_MethodDictionary::entryValueEquals(const OMR_MethodDictionaryEntry* e1,
...
void
OMR_MethodDictionary::formatEntryProperties(
    ...
    *str = NULL;
    ...
    if (NULL != buf) {
      ...
      for (size_t i = 1; i < _numProperties; ++i) {
        if (NULL != entry->propertyValues[i]) {
          bufUsed += omrstr_printf(buf + bufUsed,
                                   (uintptr_t)buflen - bufUsed,
                                   " %s",
                                   entry->propertyValues[i]);
        }
      }
    }
    *str = buf;
...
void
OMR_MethodDictionary::traceInsertEntrySuccess(
...
void
OMR_MethodDictionary::traceInsertEntryFailed(
  omr_error_t rc,
  const OMR_MethodDictionaryEntry* newEntry) const
{
...
void
OMR_MethodDictionary::traceInsertEntryReplace(
  const OMR_MethodDictionaryEntry* newEntry) const
{
