...
const char* XPATH_GET_ALL_EXPAND_TIME =
    "/verbosegc/heap-resize[@type='expand']";
...
int main(void) {
  ...
  if (0 != rc) {
    fprintf(
        stderr,
        "omrport_init_library(&portLibrary, sizeof(OMRPortLibrary)), rc=%d\n",
        (int)rc);
    return -1;
  }
  ...
  if (rcFile == (uintptr_t)-1) {
    fprintf(stderr,
            "omrfile_findfirst(SRC_DIR, resultBuffer), return code=%d\n",
            (int)rcFile);
    return -1;
  }

  while ((uintptr_t)-1 != rcFile) {
    if (strncmp(resultBuffer, VERBOSE_GC_FILE_PREFIX,
                strlen(VERBOSE_GC_FILE_PREFIX)) == 0) {
...
void analyze(char* fileName, OMRPortLibrary portLibrary) {
  ...
  for (pugi::xpath_node_set::const_iterator it = markTimes.begin();
       it != markTimes.end(); ++it) {
  ...
  for (pugi::xpath_node_set::const_iterator it = sweepTimes.begin();
       it != sweepTimes.end(); ++it) {
    pugi::xpath_node node = *it;
    double value = node.node().attribute("timems").as_double();
  ...
  for (pugi::xpath_node_set::const_iterator it = expandTimes.begin();
       it != expandTimes.end(); ++it) {
    pugi::xpath_node node = *it;
    double value = node.node().attribute("timems").as_double();
  ...
  for (pugi::xpath_node_set::const_iterator it = gcTimes.begin();
       it != gcTimes.end(); ++it) {
    pugi::xpath_node node = *it;
  ...
  if (!gcduration_values.empty()) {
    maxGCDuration =
        *std::max_element(gcduration_values.begin(), gcduration_values.end());
    minGCDuration =
        *std::min_element(gcduration_values.begin(), gcduration_values.end());
    avgGCDuration = getAvg(gcduration_values);
  }

  omrtty_printf(
      "            Mark           Sweep          Expand        GCDuration\n");
  omrtty_printf(
      "-------------------------------------------------------------------\n");
  omrtty_printf("Max     : %f        %f        %f        %f\n", maxMark,
                maxSweep, maxExpand, maxGCDuration);

  omrtty_printf("Min     : %f        %f        %f        %f\n", minMark,
                minSweep, minExpand, minGCDuration);

  omrtty_printf("Average : %f        %f        %f        %f\n\n", avgMark,
                avgSweep, avgExpand, avgGCDuration);
}
