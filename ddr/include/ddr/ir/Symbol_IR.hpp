...
private:
  ...
  DDR_RC readOverridesFile(const char *overridesFile,
                           OverrideInfo *overrideInfo);

public:
  vector<Type *> _types;
  /* Keep a set of names of structures already printed, to avoid printing
   * duplicates in the superset. Currently, only use this approach for AIX,
   * where removeDuplicates() runs too slowly. Using this method on other
   * platforms has not been tested yet. There may still be issues related to
   * differences in the DWARF/intermediate representation structures between
   * platforms which may be revealed by this approach.
   */
  ...
  explicit Symbol_IR(OMRPortLibrary *portLibrary)
      : _portLibrary(portLibrary), _types(), _fullTypeNames(), _typeSet(),
        _typeMap() {}

  explicit Symbol_IR(Symbol_IR *other)
      : _portLibrary(other->_portLibrary), _types(), _fullTypeNames(),
        _typeSet(), _typeMap() {}
...
private:
  template <typename T>
  void mergeTypes(vector<T *> *source, vector<T *> *other,
                  NamespaceUDT *outerNamespace, vector<Type *> *merged);
  void mergeFields(vector<Field *> *source, vector<Field *> *other, Type *type,
                   vector<Type *> *merged);
