...
main(int argc, char* argv[])
{
...
#if defined(DEBUG_PRINT_TYPES)
  const TypePrinter printer(&portLibrary,
                            TypePrinter::FIELDS | TypePrinter::LITERALS |
                              TypePrinter::MACROS);
#endif /* DEBUG_PRINT_TYPES */

  if ((DDR_RC_OK == rc) && !options.debugFiles.empty()) {
    rc = scanner.startScan(
      &portLibrary, &ir, &options.debugFiles, options.blacklistFile);
  ...
  if ((DDR_RC_OK == rc) && !ir._types.empty()) {
    rc = genBlob(&portLibrary,
                 &ir,
                 options.supersetFile,
                 options.blobFile,
...
Options::configure(OMRPortLibrary* portLibrary, int argc, char* argv[])
{
  ...
  if (showHelp) {
    printf(
      ...
      "      Optional file containing list of type names and source file paths "
      "to\n"
      "      ignore. Format is 'file:[filename]' or 'type:[typename]' on each "
      "line.\n"
      "  -e, --show-empty\n"
      "      Print structures, enums, and unions to the superset and blob even "
      "if\n"
      "      they do not contain any fields. The default behaviour is to hide "
      "them.\n"
      "  -sb, --show-blacklisted\n"
