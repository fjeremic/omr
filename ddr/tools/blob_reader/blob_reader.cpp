...
static void swap_bytes(uint8_t *left, uint8_t *right) {
  uint8_t temp = *left;
...
static void swap_u16(uint16_t *value) {
  uint8_t *bytes = (uint8_t *)value;

  swap_bytes(&bytes[0], &bytes[1]);
}

static void swap_u32(uint32_t *value) {
  uint8_t *bytes = (uint8_t *)value;
...
struct BlobConstant {
  uint32_t name;
  /* don't declare value as uint64_t to ensure that no padding is added between
   * it and name */
  uint32_t value[2];
  ...
  void endian_swap() {
    ...
    /* value may not be properly aligned to be treated as uint64_t - swap pairs
     * of bytes */
    uint8_t *bytes = (uint8_t *)value;
...
struct Structure {
  ...
  Structure()
      : name(NULL), nameLength(0), superName(NULL), superNameLength(0), size(0),
        fields(), constants() {}
};

static bool compareStructs(const Structure *first, const Structure *second) {
...
int main(int argc, char *argv[]) {
  ...
  if (sizeof(blobHdrV1) != rb) {
    omrfile_printf(OMRPORT_TTY_ERR,
                   "read blob header returned %zu, expected %zu\n", rb,
                   sizeof(blobHdrV1));
  } else {
    /* Data is written in the blob in the natural byte order of the originating
     * system. All version numbers thus far are small: a large version number is
     * interpreted as a mismatch between the byte order of this system and the
     * originating system.
     */
    ...
    omrfile_printf(
        ...
        " structureCount: %u\n",
        blobHdrV1.coreVersion, blobHdrV1.sizeofBool, blobHdrV1.sizeofUDATA,
        blobHdrV1.bitfieldFormat, blobHdrV1.structDataSize,
        blobHdrV1.stringTableDataSize, blobHdrV1.structureCount);

    /* read the file into memory */
    const size_t blobLength =
        blobHdrV1.structDataSize + blobHdrV1.stringTableDataSize;
    ...
    if (blobLength != rb) {
      omrfile_printf(OMRPORT_TTY_ERR,
                     "read blob data returned %zu, expected %zu\n", rb,
                     blobLength);
      goto done;
    }
    ...
    /* read strings */
    {
      ...
      const uint8_t *const stringDataStart =
          blobBuffer + blobHdrV1.structDataSize;
      const uint8_t *const stringDataEnd =
          stringDataStart + blobHdrV1.stringTableDataSize;
      ...
      for (uint32_t stringNum = 1; currentString < stringDataEnd; ++stringNum) {
        ...
        omrfile_printf(OMRPORT_TTY_OUT, "%5u: %8zx [%zu] %.*s\n", stringNum,
                       ...
                       blobString->length, blobString->length,
                       blobString->data);

        /* NOTE stringTableDataSize includes the space for the lengths */
        currentString +=
            blobString->length + sizeof(blobString->length) + padding;
      }
    }
...
#define BLOBSTRING_AT(offset) ((const BlobString *)(stringDataStart + (offset)))
      ...
      for (uint32_t structCount = 1; structCount <= blobHdrV1.structureCount;
           ++structCount) {
        ...
        if ((uint32_t)-1 == blobStruct->superName) {
          ...
          builtStruct->superNameLength =
              BLOBSTRING_AT(blobStruct->superName)->length;
        }
        builtStruct->size = blobStruct->sizeOf;

        for (uint32_t fieldCount = 1; fieldCount <= blobStruct->fieldCount;
             ++fieldCount) {
        ...
        for (uint32_t constCount = 1; constCount <= blobStruct->constCount;
             ++constCount) {
      ...
      for (size_t i = 0; i < structs.size(); ++i) {
        ...
        omrfile_printf(OMRPORT_TTY_OUT,
                       ...
                       " fieldCount: %zu\n"
                       " constCount: %zu\n",
                       builtStruct->size, builtStruct->fields.size(),
