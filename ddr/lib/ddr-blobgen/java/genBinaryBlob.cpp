...
class BlobBuildVisitor : public TypeVisitor {
 ...
 public:
  ...
  BlobBuildVisitor(JavaBlobGenerator* gen,
                   size_t baseOffset,
                   const string& prefix)
      : _gen(gen),
        _addFieldsOnly(true),
...
static uintptr_t stringTableEquals(void* leftKey,
                                   void* rightKey,
                                   void* userData) {
...
DDR_RC
JavaBlobGenerator::genBinaryBlob(OMRPortLibrary* portLibrary,
                                 Symbol_IR* ir,
                                 const char* blobFile) {
...
JavaBlobGenerator::buildBlobData(OMRPortLibrary* portLibrary, Symbol_IR* ir) {
  ...
  _buildInfo.blobConsts =
      (BlobConstant*)malloc((_buildInfo.constCount + 1) * sizeof(BlobConstant));
...
static bool isNeededBy(UDT* type, const vector<Field*>& fields) {
  ...
  if (type->_blacklisted) {
    ...
    referenced = false;
    for (vector<Field*>::const_iterator it = fields.begin(); it != fields.end();
         ++it) {
...
DDR_RC
JavaBlobGenerator::addBlobField(Field* field,
                                uint32_t* fieldCount,
                                size_t baseOffset,
                                const string& prefix) {
  DDR_RC rc = DDR_RC_OK;
...
DDR_RC
JavaBlobGenerator::addBlobConst(const string& name,
                                long long value,
                                uint32_t* constCount) {
...
DDR_RC
JavaBlobGenerator::addBlobStruct(const string& name,
                                 const string& superName,
                                 uint32_t constCount,
                                 uint32_t fieldCount,
                                 uint32_t size) {
...
DDR_RC
JavaBlobGenerator::addFieldAndConstCount(bool addFieldsOnly,
                                         size_t fieldCount,
                                         size_t constCount) {
  DDR_RC rc = DDR_RC_OK;
