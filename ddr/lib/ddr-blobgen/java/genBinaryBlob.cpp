...
public:
  explicit BlobBuildVisitor(JavaBlobGenerator *gen)
      : _gen(gen), _addFieldsOnly(false), _baseOffset(0), _prefix() {}

  BlobBuildVisitor(JavaBlobGenerator *gen, size_t baseOffset,
                   const string &prefix)
      : _gen(gen), _addFieldsOnly(true), _baseOffset(baseOffset),
        _prefix(prefix) {}
...
void JavaBlobGenerator::BuildBlobInfo::initBlobHeader() {
...
DDR_RC
JavaBlobGenerator::BuildBlobInfo::initializeBitfieldFormat(
...
static uintptr_t stringTableHash(void *key, void *userData) {
...
static uintptr_t stringTableEquals(void *leftKey, void *rightKey,
                                   void *userData) {
...
static string getBlobFullName(Type *type) {
  string blobName = type->getFullName();
  ...
  for (size_t i = 0;;) {
    ...
    blobName.replace(i, 2, "__");
  }

  return blobName;
}

void JavaBlobGenerator::copyStringTable() {
  ...
  StringTableEntry *entry =
      (StringTableEntry *)hashTableStartDo(_buildInfo.stringHash, &state);
  while (NULL != entry) {
...
DDR_RC
JavaBlobGenerator::stringTableOffset(BlobHeader *blobHeader,
                                     J9HashTable *stringTable,
                                     const char *cString, uint32_t *offset) {
  DDR_RC rc = DDR_RC_OK;
  ...
  *offset = INVALID_OFFSET;
  ...
  exemplar.offset = INVALID_OFFSET;

  /* Add will return an existing entry if one matches, NULL on allocation
   * failure, or the new node */
  StringTableEntry *entry =
      (StringTableEntry *)hashTableAdd(stringTable, &exemplar);

  if (NULL == entry) {
    /* the string was new, but memory allocation failed */
    ERRMSG("Unable to allocate memory for string table entry: %s", cString);
    rc = DDR_RC_ERROR;
  } else if (INVALID_OFFSET != entry->offset) {
    /* an existing entry was found */
    *offset = entry->offset;
  } else {
    /* the offset is INVALID_OFFSET which indicates that a new entry was added
     */
    ...
    if (NULL == buffer) {
      ...
      /* Reserve space for the uint16_t size, the string data, and the padding
       * byte, if necessary */
      blobHeader->stringDataSize +=
          (uint32_t)((sizeof(uint16_t) + strlen(cString) + 1) & ~(size_t)1);
      *offset = entry->offset;
    }
  }
...
DDR_RC
JavaBlobGenerator::genBinaryBlob(OMRPortLibrary *portLibrary, Symbol_IR *ir,
                                 const char *blobFile) {
  ...
  if (DDR_RC_OK == rc) {
    /* open blob file */
    fd = omrfile_open(
        blobFile, EsOpenCreate | EsOpenWrite | EsOpenAppend | EsOpenTruncate,
        0644);
    _buildInfo.fd = fd;
    if (-1 == fd) {
  ...
  if (DDR_RC_OK == rc) {
    /* write blob header */
    intptr_t wb =
        omrfile_write(fd, &_buildInfo.header, sizeof(_buildInfo.header));
    ...
    for (uint32_t s_ix = 0; s_ix < _buildInfo.header.structureCount; ++s_ix) {
      ...
      wb = omrfile_write(fd, &_buildInfo.blobFields[f_ix],
                         sizeof(BlobField) *
                             _buildInfo.blobStructs[s_ix].fieldCount);
      amountWritten += wb;
      ...
      wb = omrfile_write(fd, &_buildInfo.blobConsts[c_ix],
                         sizeof(BlobConstant) *
                             _buildInfo.blobStructs[s_ix].constantCount);
      amountWritten += wb;
    ...
    if (amountWritten != (intptr_t)_buildInfo.header.structDataSize) {
      ERRMSG("Wrote %u fields, %u constants, %u structs.\nCounted %u fields, "
             "%u constants, %u structures.\n",
             f_ix, c_ix,
             (uint32_t)(_buildInfo.curBlobStruct - _buildInfo.blobStructs),
             _buildInfo.fieldCount, _buildInfo.constCount,
             _buildInfo.header.structureCount);
      ERRMSG("Expected %u to be written, but %u was written.\n",
             (uint32_t)_buildInfo.header.structDataSize,
             (uint32_t)amountWritten);
    }

    /* write string data */
    wb = omrfile_write(fd, _buildInfo.stringBuffer,
                       _buildInfo.header.stringDataSize);
...
JavaBlobGenerator::countStructsAndStrings(Symbol_IR *ir) {
  ...
  for (vector<Type *>::iterator v = ir->_types.begin(); v != ir->_types.end();
       ++v) {
    Type *type = *v;
...
JavaBlobGenerator::buildBlobData(OMRPortLibrary *portLibrary, Symbol_IR *ir) {
  ...
  /* allocate hashtable */
  _buildInfo.stringHash = hashTableNew(
      portLibrary, OMR_GET_CALLSITE(), 0, sizeof(StringTableEntry), 0, 0,
   ...
   */
  _buildInfo.blobStructs = (BlobStruct *)malloc(
      (_buildInfo.header.structureCount + 1) * sizeof(BlobStruct));
  _buildInfo.curBlobStruct = _buildInfo.blobStructs;
  _buildInfo.blobFields =
      (BlobField *)malloc((_buildInfo.fieldCount + 1) * sizeof(BlobField));
  _buildInfo.curBlobField = _buildInfo.blobFields;
  _buildInfo.blobConsts = (BlobConstant *)malloc((_buildInfo.constCount + 1) *
                                                 sizeof(BlobConstant));
  ...
  if (DDR_RC_OK == rc) {
    _buildInfo.stringBuffer =
        (uint8_t *)malloc(_buildInfo.header.stringDataSize);
...
static bool isNeededBy(UDT *type, const vector<Field *> &fields) {
...
BlobBuildVisitor::visitEnum(EnumUDT *e) const {
  ...
  if (_addFieldsOnly || !e->isAnonymousType()) {
    ...
    if (!_addFieldsOnly) {
      for (vector<EnumMember *>::iterator m = e->_enumMembers.begin();
           m != e->_enumMembers.end(); ++m) {
    ...
    if (DDR_RC_OK == rc) {
      if (_addFieldsOnly) {
        ...
        rc = _gen->addBlobStruct(getBlobFullName(e), "", constCount, 0,
                                 (uint32_t)e->_sizeOf);
      }
    }
  }
...
BlobBuildVisitor::visitUnion(UnionUDT *u) const {
  ...
  if (_addFieldsOnly || !u->isAnonymousType()) {
    ...
    for (vector<Field *>::iterator m = u->_fieldMembers.begin();
         m != u->_fieldMembers.end(); ++m) {
    ...
    if ((DDR_RC_OK == rc) && !_addFieldsOnly) {
      for (vector<EnumMember *>::iterator m = u->_enumMembers.begin();
           m != u->_enumMembers.end(); ++m) {
        EnumMember *literal = *m;
    ...
    if ((DDR_RC_OK == rc) && !_addFieldsOnly) {
      for (vector<Macro>::iterator m = u->_macros.begin();
           m != u->_macros.end(); ++m) {
        long long value = 0;
    ...
    if (DDR_RC_OK == rc) {
      if (_addFieldsOnly) {
        ...
        rc = _gen->addBlobStruct(getBlobFullName(u), "", constCount, fieldCount,
                                 (uint32_t)u->_sizeOf);
      }
    }
  }
  ...
  if ((DDR_RC_OK == rc) && !_addFieldsOnly) {
    ...
    for (vector<UDT *>::const_iterator it = u->_subUDTs.begin();
         it != u->_subUDTs.end(); ++it) {
      UDT *nested = *it;
...
BlobBuildVisitor::visitClass(ClassUDT *cu) const {
  ...
  if (_addFieldsOnly || !cu->isAnonymousType()) {
    ...
    for (vector<Field *>::iterator m = cu->_fieldMembers.begin();
         m != cu->_fieldMembers.end(); ++m) {
      rc = _gen->addBlobField(*m, &fieldCount, _baseOffset, _prefix);
      if (DDR_RC_OK != rc) {
        break;
      }
    ...
    if ((DDR_RC_OK == rc) && !_addFieldsOnly) {
      for (vector<EnumMember *>::iterator m = cu->_enumMembers.begin();
           m != cu->_enumMembers.end(); ++m) {
        EnumMember *literal = *m;
    ...
    if ((DDR_RC_OK == rc) && !_addFieldsOnly) {
      for (vector<Macro>::iterator m = cu->_macros.begin();
           m != cu->_macros.end(); ++m) {
        long long value = 0;
    ...
    if (DDR_RC_OK == rc) {
      if (_addFieldsOnly) {
        ...
        string nameFormatted = getBlobFullName(cu);
        string superName;
        ...
        rc = _gen->addBlobStruct(nameFormatted, superName, constCount,
                                 fieldCount, (uint32_t)cu->_sizeOf);
      }
    }
  }
  ...
  if ((DDR_RC_OK == rc) && !_addFieldsOnly) {
    ...
    for (vector<UDT *>::const_iterator it = cu->_subUDTs.begin();
         it != cu->_subUDTs.end(); ++it) {
      UDT *nested = *it;
...
BlobBuildVisitor::visitNamespace(NamespaceUDT *ns) const {
  ...
  for (vector<Macro>::iterator m = ns->_macros.begin(); m != ns->_macros.end();
       ++m) {
    long long value = 0;
  ...
  if (DDR_RC_OK == rc) {
    if (_addFieldsOnly) {
      ...
      rc = _gen->addBlobStruct(getBlobFullName(ns), "", constCount, 0, 0);
    }
  }
  ...
  if ((DDR_RC_OK == rc) && !_addFieldsOnly) {
    ...
    for (vector<UDT *>::const_iterator it = ns->_subUDTs.begin();
         it != ns->_subUDTs.end(); ++it) {
      UDT *nested = *it;
...
DDR_RC
JavaBlobGenerator::addBlobField(Field *field, uint32_t *fieldCount,
                                size_t baseOffset, const string &prefix) {
  DDR_RC rc = DDR_RC_OK;
  ...
  if (field->_isStatic) {
    ...
    rc = fieldType->acceptVisitor(
        BlobBuildVisitor(this, adjustedOffset, adjustedPrefix));
  } else {
    uint32_t nameOffset = INVALID_OFFSET;
    uint32_t typeOffset = INVALID_OFFSET;
    string typeName;
    ...
    if (DDR_RC_OK == rc) {
      ...
      rc = stringTableOffset(&_buildInfo.header, _buildInfo.stringHash,
                             fieldName.c_str(), &nameOffset);
    }

    if (DDR_RC_OK == rc) {
      rc = stringTableOffset(&_buildInfo.header, _buildInfo.stringHash,
                             typeName.c_str(), &typeOffset);
    }
...
DDR_RC
JavaBlobGenerator::addBlobConst(const string &name, long long value,
                                uint32_t *constCount) {
  uint32_t offset = INVALID_OFFSET;
  DDR_RC rc = stringTableOffset(&_buildInfo.header, _buildInfo.stringHash,
                                name.c_str(), &offset);

  if (DDR_RC_OK == rc) {
    _buildInfo.curBlobConst->nameOffset = offset;

    memset(_buildInfo.curBlobConst->value, 0,
           sizeof(_buildInfo.curBlobConst->value));
    memcpy(_buildInfo.curBlobConst->value, &value, sizeof(value));

    if ((uintptr_t)(_buildInfo.curBlobConst - _buildInfo.blobConsts) <
        (uintptr_t)_buildInfo.constCount) {
...
DDR_RC
JavaBlobGenerator::addBlobStruct(const string &name, const string &superName,
                                 uint32_t constCount, uint32_t fieldCount,
                                 uint32_t size) {
  ...
  uint32_t nameOffset = INVALID_OFFSET;
  uint32_t superOffset = INVALID_OFFSET;
  DDR_RC rc = stringTableOffset(&_buildInfo.header, _buildInfo.stringHash,
                                name.c_str(), &nameOffset);

  if ((DDR_RC_OK == rc) && !superName.empty()) {
    rc = stringTableOffset(&_buildInfo.header, _buildInfo.stringHash,
                           superName.c_str(), &superOffset);
  }
...
public:
  explicit BlobFieldVisitor(string *prefix, string *suffix)
      : _typePrefix(prefix), _typeSuffix(suffix) {}
...
BlobFieldVisitor::visitType(Type *type) const {
  ...
  if (Type::isStandardType(typeName.c_str(), (size_t)typeName.length(),
                           &isSigned, &bitWidth)) {
...
BlobFieldVisitor::visitNamespace(NamespaceUDT *type) const {
  ...
  if (prefix.empty() || ("union" == prefix)) {
    *_typePrefix += type->getFullName();
  } else {
    *_typePrefix += prefix + " " + type->getFullName();
  }
...
BlobFieldVisitor::visitEnum(EnumUDT *type) const {
  *_typePrefix += type->getSymbolKindName() + " " + type->getFullName();
  return DDR_RC_OK;
}

DDR_RC
BlobFieldVisitor::visitTypedef(TypedefUDT *type) const {
  /* If typedef is void*, or otherwise known as a function pointer, return name
   * as void*. */
  Type *opaqueType = type->getOpaqueType();

  if ((NULL != opaqueType) && ("void" == opaqueType->_name) &&
      (0 != type->_modifiers._pointerCount)) {
    *_typePrefix += "void";
  } else {
    ...
    if (prefix.empty() || ("union" == prefix)) {
      ...
      if (Type::isStandardType(fullName.c_str(), (size_t)fullName.length(),
                               &isSigned, &bitWidth)) {
        stringstream newType;
  ...
  for (size_t i = type->getPointerCount(); i > 0; --i) {
    *_typeSuffix = "*" + *_typeSuffix;
  }

  for (size_t i = type->getArrayDimensions(); i > 0; --i) {
    *_typeSuffix = *_typeSuffix + "[]";
  }
...
JavaBlobGenerator::formatFieldType(Field *field, string *fieldType) {
  ...
  *fieldType = "";
  ...
  if (0 != (field->_modifiers._modifierFlags & ~Modifiers::MODIFIER_FLAGS)) {
    ERRMSG("Unhandled field modifer flags: %d",
           field->_modifiers._modifierFlags);
    rc = DDR_RC_ERROR;
  } else {
    typeSuffix = field->_modifiers.getPointerType();

    if ((NULL == type) || type->_blacklisted ||
        (field->_modifiers._pointerCount > 1)) {
      typePrefix = "void";
    } else {
...
DDR_RC
JavaBlobGenerator::addFieldAndConstCount(bool addFieldsOnly, size_t fieldCount,
                                         size_t constCount) {
  DDR_RC rc = DDR_RC_OK;
  size_t structDataSize =
      sizeof(BlobField) * fieldCount + sizeof(BlobConstant) * constCount;
...
BlobEnumerateVisitor::visitNamespace(NamespaceUDT *type) const {
  ...
  if (_addFieldsOnly || !type->isAnonymousType()) {
    ...
    if (!_addFieldsOnly) {
      for (vector<Macro>::iterator it = type->_macros.begin();
           it != type->_macros.end(); ++it) {
...
BlobEnumerateVisitor::visitClass(ClassUDT *type) const {
  ...
  if (_addFieldsOnly || !type->isAnonymousType()) {
    ...
    for (vector<Field *>::iterator v = type->_fieldMembers.begin();
         v != type->_fieldMembers.end(); ++v) {
      Field *field = *v;
      Type *fieldType = field->_fieldType;
      ...
      if (field->_isStatic) {
        ...
        /* Anonymous type members are added to the struct and not counted as
         * fields themselves. */
...
BlobEnumerateVisitor::visitUnion(UnionUDT *type) const {
  ...
  /* When adding the fields from a field with an anonymous type, do not add
   * subUDTs twice. */
  if ((DDR_RC_OK == rc) && !_addFieldsOnly) {
    for (vector<UDT *>::const_iterator it = type->_subUDTs.begin();
         it != type->_subUDTs.end(); ++it) {
