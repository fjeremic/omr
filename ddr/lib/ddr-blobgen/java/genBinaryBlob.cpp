  ...
  uint8_t* bitfieldFormat)
{
  ...
  if ((slot0 == 0xC243F831) && (slot1 == 0x676C952B)) {
    ...
    *bitfieldFormat = 2;
    ...
    ERRMSG(
      "Unable to determine bitfield format from %08X %08X\n", slot0, slot1);
    rc = DDR_RC_ERROR;
  }
                                 ...
                                 const char* blobFile)
{
  ...
  if (DDR_RC_OK == rc) {
    /* open blob file */
    fd =
      omrfile_open(blobFile,
                   EsOpenCreate | EsOpenWrite | EsOpenAppend | EsOpenTruncate,
                   0644);
    _buildInfo.fd = fd;
    if (-1 == fd) {
  ...
  if (DDR_RC_OK == rc) {
    ...
    if (amountWritten != (intptr_t)_buildInfo.header.structDataSize) {
      ERRMSG("Wrote %u fields, %u constants, %u structs.\nCounted %u fields, "
             ...
             _buildInfo.fieldCount,
             _buildInfo.constCount,
    ...
    /* write string data */
    wb = omrfile_write(
      fd, _buildInfo.stringBuffer, _buildInfo.header.stringDataSize);
...
JavaBlobGenerator::buildBlobData(OMRPortLibrary* portLibrary, Symbol_IR* ir)
{
  ...
  /* allocate hashtable */
  _buildInfo.stringHash = hashTableNew(portLibrary,
                                       OMR_GET_CALLSITE(),
                                       0,
    ...
    if (DDR_RC_OK == rc) {
      if (_addFieldsOnly) {
        ...
        rc = _gen->addBlobStruct(
          getBlobFullName(e), "", constCount, 0, (uint32_t)e->_sizeOf);
      }
    }
  }
  ...
  if (_addFieldsOnly || !cu->isAnonymousType()) {
    ...
    if (DDR_RC_OK == rc) {
      if (_addFieldsOnly) {
        ...
        rc = _gen->addBlobStruct(nameFormatted,
                                 superName,
                                 constCount,
                                 fieldCount,
                                 (uint32_t)cu->_sizeOf);
                                ...
                                const string& prefix)
{
  ...
  if (field->_isStatic) {
    ...
    if (DDR_RC_OK == rc) {
      ...
      rc = stringTableOffset(&_buildInfo.header,
                                ...
                                uint32_t* constCount)
{
  ...
  DDR_RC rc = stringTableOffset(
    &_buildInfo.header, _buildInfo.stringHash, name.c_str(), &offset);

  if (DDR_RC_OK == rc) {
    _buildInfo.curBlobConst->nameOffset = offset;

    memset(_buildInfo.curBlobConst->value,
           0,
                                 ...
                                 uint32_t size)
{
  ...
  DDR_RC rc = stringTableOffset(
    &_buildInfo.header, _buildInfo.stringHash, name.c_str(), &nameOffset);
...
BlobFieldVisitor::visitType(Type* type) const
{
  ...
  if (Type::isStandardType(
        typeName.c_str(), (size_t)typeName.length(), &isSigned, &bitWidth)) {
  ...
  if ((NULL != opaqueType) && ("void" == opaqueType->_name) &&
    ...
    *_typePrefix += "void";
    ...
    if (prefix.empty() || ("union" == prefix)) {
      ...
      if (Type::isStandardType(fullName.c_str(),
                               (size_t)fullName.length(),
                               &isSigned,
                               &bitWidth)) {
        stringstream newType;
