...
JavaSupersetGenerator::replaceBaseTypedef(Type* type, string* name)
{
  ...
  if (Type::isStandardType(
        name->c_str() + start, (size_t)length, &isSigned, &bitWidth)) {
    stringstream ss;
...
public:
  SupersetFieldVisitor(JavaSupersetGenerator* gen,
                       string* typeName,
                       string* simpleName,
                       string* prefixBase,
                       string* prefix,
 ...
 */
DDR_RC
JavaSupersetGenerator::getFieldType(Field* field,
                                    ...
                                    string* simpleTypeName)
{
  ...
  if (DDR_RC_OK == rc) {
    ...
    if ((NULL == fieldType) || fieldType->_blacklisted ||
      ...
      rc = fieldType->acceptVisitor(SupersetFieldVisitor(
        this, &typeName, &simpleName, &prefixBase, &prefix, &pointerTypeBase));
    }
  }
...
public:
  ...
  SupersetVisitor(JavaSupersetGenerator* supersetGen,
                  bool addFieldsOnly,
                  const string& prefix)
    : _supersetGen(supersetGen)
...
SupersetVisitor::visitComposite(ClassType* type, Type* superClass) const
{
  ...
  if (_addFieldsOnly || !type->isAnonymousType()) {
    for (vector<Field*>::const_iterator m = type->_fieldMembers.begin();
         (DDR_RC_OK == rc) && (m != type->_fieldMembers.end());
         ++m) {
  ...
  if (!_addFieldsOnly) {
    for (vector<UDT*>::const_iterator it = type->_subUDTs.begin();
         (DDR_RC_OK == rc) && (it != type->_subUDTs.end());
         ++it) {
      UDT* nested = *it;
...
SupersetVisitor::visitNamespace(NamespaceUDT* type) const
{
  ...
  if (!_addFieldsOnly) {
    for (vector<UDT*>::const_iterator v = type->_subUDTs.begin();
         (DDR_RC_OK == rc) && (v != type->_subUDTs.end());
         ++v) {
      UDT* nested = *v;
...
DDR_RC
JavaSupersetGenerator::printSuperset(OMRPortLibrary* portLibrary,
                                     Symbol_IR* ir,
