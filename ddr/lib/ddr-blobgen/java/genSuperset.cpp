...
static string replaceAll(string str, const string& subStr, const string& newStr)
{
  for (size_t i = 0;;) {
    i = str.find(subStr, i);
    if (string::npos == i) {
...
public:
  SupersetFieldVisitor(JavaSupersetGenerator* gen, string* typeName, string* simpleName, string* prefixBase,
    string* prefix, string* pointerTypeBase)
    : _gen(gen)
    , _typeName(typeName)
...
SupersetFieldVisitor::visitTypedef(TypedefUDT* type) const
{
  ...
  if (opaqueType == type) {
    *_typeName = replaceAll(type->getFullName(), "::", "$");
    ...
    opaqueType->acceptVisitor(
      SupersetFieldVisitor(_gen, _typeName, _simpleName, _prefixBase, _prefix, _pointerTypeBase));
  }
  ...
  if (DDR_RC_OK == rc) {
    ...
    if ((NULL == fieldType) || fieldType->_blacklisted || (field->_modifiers._pointerCount > 1)) {
      ...
      rc = fieldType->acceptVisitor(
        SupersetFieldVisitor(this, &typeName, &simpleName, &prefixBase, &prefix, &pointerTypeBase));
    }
  }
    ...
    if ((NULL != fieldType) && fieldType->isAnonymousType()) {
      ...
      if (DDR_RC_OK != rc) {
        ...
        if (DDR_RC_OK == rc) {
          ...
          string lineToPrint = "F|" + replaceAll(nameFormatted, ".", "$") + "|" + nameFormatted + "|"
            + replaceAll(assembledTypeName, "::", "$") + "|" + simpleTypeName + "\n";
...
SupersetVisitor::visitComposite(ClassType* type, Type* superClass) const
{
  ...
  if (!_addFieldsOnly) {
    ...
    for (vector<Macro>::const_iterator m = type->_macros.begin(); (DDR_RC_OK == rc) && (m != type->_macros.end());
         ++m) {
  ...
  if (!_addFieldsOnly) {
    for (vector<UDT*>::const_iterator it = type->_subUDTs.begin(); (DDR_RC_OK == rc) && (it != type->_subUDTs.end());
         ++it) {
      UDT* nested = *it;
      ...
      if (nested->isAnonymousType()) {
        includeSubUDT = false;
        for (vector<Field*>::const_iterator fit = type->_fieldMembers.begin(); fit != type->_fieldMembers.end();
             ++fit) {
...
SupersetVisitor::visitNamespace(NamespaceUDT* type) const
{
  ...
  if (!_addFieldsOnly) {
    for (vector<UDT*>::const_iterator v = type->_subUDTs.begin(); (DDR_RC_OK == rc) && (v != type->_subUDTs.end());
         ++v) {
      UDT* nested = *v;
...
JavaSupersetGenerator::printSuperset(OMRPortLibrary* portLibrary, Symbol_IR* ir, const char* supersetFile)
{
  ...
  for (vector<Type*>::const_iterator v = ir->_types.begin(); (DDR_RC_OK == rc) && (v != ir->_types.end()); ++v) {
    Type* type = *v;
