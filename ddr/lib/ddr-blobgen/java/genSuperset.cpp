...
static string replaceAll(string str, const string &subStr,
                         const string &newStr) {
  for (size_t i = 0;;) {
    i = str.find(subStr, i);
    if (string::npos == i) {
...
JavaSupersetGenerator::JavaSupersetGenerator(bool printEmptyTypes)
    : _file(0), _portLibrary(NULL), _printEmptyTypes(printEmptyTypes),
      _pendingTypeHeading() {}
...
void JavaSupersetGenerator::replaceBaseTypedef(Type *type, string *name) {
   ...
   */
  if (Type::isStandardType(name->c_str() + start, (size_t)length, &isSigned,
                           &bitWidth)) {
    stringstream ss;
...
public:
  SupersetFieldVisitor(JavaSupersetGenerator *gen, string *typeName,
                       string *simpleName, string *prefixBase, string *prefix,
                       string *pointerTypeBase)
      : _gen(gen), _typeName(typeName), _simpleName(simpleName),
        _prefixBase(prefixBase), _prefix(prefix),
...
SupersetFieldVisitor::visitTypedef(TypedefUDT *type) const {
  ...
  if (opaqueType == type) {
    *_typeName = replaceAll(type->getFullName(), "::", "$");
    ...
    opaqueType->acceptVisitor(SupersetFieldVisitor(
        _gen, _typeName, _simpleName, _prefixBase, _prefix, _pointerTypeBase));
  }
 ...
 *     F|PREFIX OUTER_TYPE$assembledBaseTypeName:BIT_FIELD|MODIFIERS PREFIX
 OUTER_TYPE.typeName:BIT_FIELD
 ...
 *   recursively following the chain of typedefs until a non-typedef type is
 found.
 * - MODIFIERS: Field modifiers, such as const or volatile, are only added to
 the second
 *   type name and not the assembled type name.
 * - OUTER_TYPE: Outer type names are prefixed both to assembled type names and
 type names, but with different formatting.
 * - PREFIX: class/struct/enum is only prefixed to the assembled type name of
 types
 *   which are defined with the format "typedef struct NAME {...} NAME". In
 dwarf, for
 ...
 */
DDR_RC
JavaSupersetGenerator::getFieldType(Field *field, string *assembledTypeName,
  ...
  if (DDR_RC_OK == rc) {
    ...
    if ((NULL == fieldType) || fieldType->_blacklisted ||
        (field->_modifiers._pointerCount > 1)) {
      typeName = "void";
      simpleName = "void";
    } else {
      rc = fieldType->acceptVisitor(
          SupersetFieldVisitor(this, &typeName, &simpleName, &prefixBase,
                               &prefix, &pointerTypeBase));
    }
  }
...
public:
  explicit SupersetVisitor(JavaSupersetGenerator *supersetGen)
      : _supersetGen(supersetGen), _addFieldsOnly(false), _prefix() {}

  SupersetVisitor(JavaSupersetGenerator *supersetGen, bool addFieldsOnly,
                  const string &prefix)
      : _supersetGen(supersetGen), _addFieldsOnly(addFieldsOnly),
        _prefix(prefix) {}
 ...
 *     F|PREFIX OUTER_TYPE$assembledBaseTypeName:BIT_FIELD|MODIFIERS PREFIX
 * OUTER_TYPE.typeName:BIT_FIELD See getFieldType() for more details on how
 * these type names are constructed.
 */
DDR_RC
JavaSupersetGenerator::printFieldMember(Field *field, const string &prefix) {
  DDR_RC rc = DDR_RC_OK;

  /* If the type of a field is an anonymous inner type, print its fields
   * prefixed by its field name in place of the field.
   */
  if (!field->_isStatic) {
    Type *fieldType = field->_fieldType;
    ...
    if ((NULL != fieldType) && fieldType->isAnonymousType()) {
      rc = fieldType->acceptVisitor(
          SupersetVisitor(this, true, prefix + field->_name + "."));
    } else {
      ...
      if (DDR_RC_OK != rc) {
        ...
        if (DDR_RC_OK == rc) {
          string nameFormatted =
              prefix + (field->_name == "class" ? "klass" : field->_name);
...
JavaSupersetGenerator::printType(Type *type, Type *superClass) {
  ...
  _pendingTypeHeading = "S|" + nameFormatted + "|" + nameFormatted +
                        "Pointer|" + superFormatted + "\n";
...
JavaSupersetGenerator::print(const string &text) {
  ...
  if (lengthToPrint !=
      (size_t)omrfile_write(_file, text.c_str(), lengthToPrint)) {
    ERRMSG("Write failed");
    rc = DDR_RC_ERROR;
  }
  ...
  if (!_addFieldsOnly) {
    for (vector<UDT *>::const_iterator it = type->_subUDTs.begin();
      ...
      if (nested->isAnonymousType()) {
        includeSubUDT = false;
        for (vector<Field *>::const_iterator fit = type->_fieldMembers.begin();
             fit != type->_fieldMembers.end(); ++fit) {
...
DDR_RC
JavaSupersetGenerator::printSuperset(OMRPortLibrary *portLibrary, Symbol_IR *ir,
  ...
  _file = omrfile_open(
      supersetFile, EsOpenCreate | EsOpenWrite | EsOpenAppend | EsOpenTruncate,
      0644);
