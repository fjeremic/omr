...
MacroTool::getMacros(OMRPortLibrary *portLibrary, const char *filename) {
  ...
  while (macrolist.readLine(line)) {
    if (startsWith(line, FileBeginEyeCatcher,
                   LITERAL_STRLEN(FileBeginEyeCatcher))) {
      ...
      if (includeFileSet.end() == includeFileSet.find(fileName)) {
         ...
         */
        while (macrolist.readLine(line)) {
          if (startsWith(line, FileEndEyeCatcher,
                         LITERAL_STRLEN(FileEndEyeCatcher))) {
            break;
          }
        }
      }
    } else if (startsWith(line, TypeEyeCatcher,
                          LITERAL_STRLEN(TypeEyeCatcher))) {
      const string typeName = line.substr(LITERAL_STRLEN(TypeEyeCatcher));
      MacroInfo info(typeName);
      macroList.push_back(info);
    } else if (startsWith(line, MacroEyeCatcher,
                          LITERAL_STRLEN(MacroEyeCatcher))) {
      ...
      if ((string::npos == pos) || (pos <= LITERAL_STRLEN(MacroEyeCatcher))) {
        ...
        if (string::npos != start) {
          ...
          const string name =
              line.substr(LITERAL_STRLEN(MacroEyeCatcher),
                          pos - LITERAL_STRLEN(MacroEyeCatcher));
...
MacroTool::addMacrosToIR(Symbol_IR *ir) const {
  ...
  for (std::vector<Type *>::const_iterator it = ir->_types.begin();
       it != ir->_types.end(); ++it) {
    Type *type = *it;
  ...
  for (std::vector<MacroInfo>::const_iterator it = macroList.begin();
       it != macroList.end(); ++it) {
