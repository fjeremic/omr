...
MacroTool::getMacros(OMRPortLibrary* portLibrary, const char* filename)
{
  ...
  while (macrolist.readLine(line)) {
    if (startsWith(
          line, FileBeginEyeCatcher, LITERAL_STRLEN(FileBeginEyeCatcher))) {
      ...
      if (includeFileSet.end() == includeFileSet.find(fileName)) {
        ...
        while (macrolist.readLine(line)) {
          if (startsWith(
                line, FileEndEyeCatcher, LITERAL_STRLEN(FileEndEyeCatcher))) {
            break;
          }
        }
      }
    } else if (startsWith(
                 line, TypeEyeCatcher, LITERAL_STRLEN(TypeEyeCatcher))) {
      const string typeName = line.substr(LITERAL_STRLEN(TypeEyeCatcher));
      MacroInfo info(typeName);
      macroList.push_back(info);
    } else if (startsWith(
                 line, MacroEyeCatcher, LITERAL_STRLEN(MacroEyeCatcher))) {
