...
Type::Type(size_t size)
    : _blacklisted(false), _opaque(true), _name(), _sizeOf(size) {}

Type::~Type() {}

bool Type::isAnonymousType() const { return _name.empty(); }

string Type::getFullName() const { return _name; }
...
bool Type::insertUnique(Symbol_IR *ir) {
  /* No-op: since Types aren't printed, there's no need to check if they're
   * duplicates either. */
  return false;
}

NamespaceUDT *Type::getNamespace() { return NULL; }

size_t Type::getPointerCount() { return 0; }

size_t Type::getArrayDimensions() { return 0; }

void Type::addMacro(Macro *macro) {
  /* No-op: macros cannot be associated with base types. */
}

vector<UDT *> *Type::getSubUDTS() { return NULL; }

void Type::renameFieldsAndMacros(const FieldOverride &fieldOverride,
                                 Type *replacementType) {
  /* No-op: base types have no fields. */
}

Type *Type::getBaseType() { return NULL; }
...
bool Type::compareToUDT(const UDT &) const { return false; }

bool Type::compareToNamespace(const NamespaceUDT &) const { return false; }

bool Type::compareToEnum(const EnumUDT &) const { return false; }

bool Type::compareToTypedef(const TypedefUDT &) const { return false; }

bool Type::compareToClasstype(const ClassType &) const { return false; }

bool Type::compareToUnion(const UnionUDT &) const { return false; }

bool Type::compareToClass(const ClassUDT &) const { return false; }
...
bool Type::isStandardType(const char *type, size_t typeLen, bool *isSigned,
                          size_t *bitWidth) {
  ...
  if (num[TK_signed] + num[TK_unsigned] > 1) {
  ...
  } else if ((1 == num[TK_char]) && (0 == num[TK_short]) &&
             (0 == num[TK_int]) && (0 == num[TK_long])) {
    ...
    bits = 8 * sizeof(char);
    goto pass;
  } else if ((0 == num[TK_char]) && (1 == num[TK_short]) &&
             (1 >= num[TK_int]) && (0 == num[TK_long])) {
    bits = 8 * sizeof(short);
    goto pass;
  } else if ((0 == num[TK_char]) && (0 == num[TK_short]) &&
             (1 == num[TK_int]) && (0 == num[TK_long])) {
    bits = 8 * sizeof(int);
    goto pass;
  } else if ((0 == num[TK_char]) && (0 == num[TK_short]) &&
             (1 >= num[TK_int]) && (1 == num[TK_long])) {
    bits = 8 * sizeof(long);
    goto pass;
  } else if ((0 == num[TK_char]) && (0 == num[TK_short]) &&
             (1 >= num[TK_int]) && (2 == num[TK_long])) {
