...
Modifiers::Modifiers()
    : _arrayLengths(), _modifierFlags(NO_MOD), _pointerCount(0),
      _referenceCount(0) {}

Modifiers::~Modifiers() {}

static const char *const MODIFIER_NAMES[] = {"const", "volatile", "unaligned",
                                             "restrict", "shared"};

string Modifiers::getModifierNames() const {
...
string Modifiers::getPointerType() const {
  stringstream s;
...
bool Modifiers::isArray() const { return _arrayLengths.size() > 0; }

size_t Modifiers::getArrayLength(size_t i) const {
...
size_t Modifiers::getArrayDimensions() const { return _arrayLengths.size(); }

size_t Modifiers::getSize(size_t typeSize) const {
  size_t sizeOf = typeSize;
  if (_pointerCount > 0) {
    sizeOf = sizeof(void *);
  }
  for (vector<size_t>::const_iterator it = _arrayLengths.begin();
       it != _arrayLengths.end(); ++it) {
    sizeOf *= *it;
  }
  return sizeOf;
}
...
bool Modifiers::operator==(const Modifiers &type) const {
  ...
  return arrayLengthsEqual && (_modifierFlags == type._modifierFlags) &&
