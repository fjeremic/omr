...
Symbol_IR::OverrideInfo::OverrideInfo() : opaqueTypeNames(), fieldOverrides() {
  static const char *const scalarTypes[] = {
      "char",  "I8",       "I_8",   "U8",        "U_8",   "I16",  "I_16",
      "U16",   "U_16",     "I32",   "I_32",      "U32",   "U_32", "I64",
      "I_64",  "U64",      "U_64",  "I128",      "I_128", "U128", "U_128",
      "IDATA", "intptr_t", "UDATA", "uintptr_t", NULL};
...
Symbol_IR::~Symbol_IR() {
  for (std::vector<Type *>::const_iterator it = _types.begin();
       it != _types.end(); ++it) {
    delete *it;
  }
  _types.clear();
...
Symbol_IR::applyOverridesList(const char *overridesListFile) {
  ...
  if (DDR_RC_OK == rc) {
    /* mark opaque types */
    for (vector<Type *>::const_iterator it = _types.begin(); it != _types.end();
         ++it) {
      Type *type = *it;
      if (overrideInfo.opaqueTypeNames.find(type->_name) !=
          overrideInfo.opaqueTypeNames.end()) {
        type->_opaque = true;
      }
    }
    ...
    for (vector<FieldOverride>::const_iterator it =
             overrideInfo.fieldOverrides.begin();
         it != overrideInfo.fieldOverrides.end(); ++it) {
      const FieldOverride &type = *it;
      /* Check if the structure to override exists. */
      map<string, vector<Type *> >::const_iterator typeEntry =
          typeNames.find(type.structName);
      if (typeNames.end() != typeEntry) {
        Type *replacementType = NULL;
        if (type.isTypeOverride) {
          /* If the type for the override exists in the IR, use it. Otherwise,
           * create it. */
...
DDR_RC
Symbol_IR::readOverridesFile(const char *overridesFile,
                             OverrideInfo *overrideInfo) {
  DDR_RC rc = DDR_RC_OK;
  ...
  if (!file.openRead(overridesFile)) {
    ...
    while (file.readLine(line)) {
      ...
      /* Correctly formatted lines must have exactly 1 dot and 1 equals in that
       * order. */
      size_t dotPosition = line.find('.');
      if ((string::npos == dotPosition) ||
          (string::npos != line.find('.', dotPosition + 1))) {
        /* not exactly one '.' */
        continue;
      }
      size_t equalsPosition = line.find('=', dotPosition);
      if ((string::npos == equalsPosition) ||
          (string::npos != line.find('=', equalsPosition + 1))) {
        /* not exactly one '=' after the '.' */
        continue;
      }
      /* Get the structure name, field name, and override name. Add to list to
       * process. */
...
void Symbol_IR::removeDuplicates() {
...
MergeVisitor::visitNamespace(NamespaceUDT *type) const {
  ...
  /* This type may be derived from only a forward declaration; if so, update the
   * size now. */
  if (0 == type->_sizeOf) {
...
DDR_RC
MergeVisitor::visitUnion(UnionUDT *type) const { return visitComposite(type); }

template <typename T>
void Symbol_IR::mergeTypes(vector<T *> *source, vector<T *> *other,
                           NamespaceUDT *outerNamespace,
                           vector<Type *> *merged) {
  if ((NULL != source) && (NULL != other)) {
    for (typename vector<T *>::iterator it = other->begin();
         it != other->end();) {
...
void Symbol_IR::mergeFields(vector<Field *> *source, vector<Field *> *other,
                            Type *type, vector<Type *> *merged) {
  /* Create a map of all fields in the source list. */
  set<string> fieldNames;
  for (vector<Field *>::const_iterator it = source->begin();
       it != source->end(); ++it) {
...
void Symbol_IR::mergeEnums(vector<EnumMember *> *source,
                           vector<EnumMember *> *other) {
  /* Create a map of all fields in the source list. */
  set<string> enumNames;
  for (vector<EnumMember *>::const_iterator it = source->begin();
       it != source->end(); ++it) {
    enumNames.insert((*it)->_name);
  }
  for (vector<EnumMember *>::iterator it = other->begin();
       it != other->end();) {
...
DDR_RC
TypeReplaceVisitor::visitType(Type *type) const { return DDR_RC_OK; }
...
DDR_RC
TypeReplaceVisitor::visitEnum(EnumUDT *type) const { return DDR_RC_OK; }
...
TypeReplaceVisitor::visitComposite(ClassType *type) const {
  ...
  if (DDR_RC_OK == rc) {
    for (vector<Field *>::const_iterator it = type->_fieldMembers.begin();
         it != type->_fieldMembers.end(); ++it) {
    ...
    if ((NULL != oldType) && (_typeSet.end() == _typeSet.find(oldType))) {
      ...
      if (NULL == replacementType) {
        ERRMSG("Error replacing type '%s' in '%s'",
               oldType->getFullName().c_str(), outer->getFullName().c_str());
        rc = DDR_RC_ERROR;
...
Type *Symbol_IR::findTypeInMap(Type *typeToFind) {
  Type *returnType = NULL;
  if ((NULL != typeToFind) && !typeToFind->isAnonymousType()) {
    const string nameKey = typeToFind->getTypeNameKey();
    unordered_map<string, set<Type *> >::const_iterator map_it =
        _typeMap.find(nameKey);
...
void Symbol_IR::addTypeToMap(Type *type) {
...
Symbol_IR::mergeIR(Symbol_IR *other) {
  ...
  /* Create map of this source IR of type full name to type. Use it to replace
   * all pointers in types acquired from other IRs. The problem of encountering
   * types to replace that are not yet in this IR is solved by doing all of the
   * type replacement after all types are merged.
   */
  ...
  for (vector<Type *>::const_iterator it = mergedTypes.begin();
       it != mergedTypes.end(); ++it) {
