...
Symbol_IR::applyOverridesList(const char* overridesListFile)
{
  ...
  if (DDR_RC_OK == rc) {
    ...
    for (vector<FieldOverride>::const_iterator it =
         ...
         it != overrideInfo.fieldOverrides.end();
         ++it) {
  ...
  if (!file.openRead(overridesFile)) {
    ...
    while (file.readLine(line)) {
      ...
      FieldOverride override = { line.substr(0, dotPosition),
                                 line.substr(dotPosition + 1,
                                             equalsPosition - dotPosition - 1),
...
template<typename T>
void
Symbol_IR::mergeTypes(vector<T*>* source,
                      vector<T*>* other,
...
void
Symbol_IR::mergeFields(vector<Field*>* source,
                       vector<Field*>* other,
                       Type* type,
                       vector<Type*>* merged)
{
