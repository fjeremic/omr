  ...
  , fieldOverrides()
{
  static const char* const scalarTypes[]
    = { "char", "I8", "I_8", "U8", "U_8", "I16", "I_16", "U16", "U_16", "I32", "I_32", "U32", "U_32", "I64", "I_64",
        "U64", "U_64", "I128", "I_128", "U128", "U_128", "IDATA", "intptr_t", "UDATA", "uintptr_t", NULL };
...
template <typename T>
void Symbol_IR::mergeTypes(vector<T*>* source, vector<T*>* other, NamespaceUDT* outerNamespace, vector<Type*>* merged)
{
...
void Symbol_IR::mergeFields(vector<Field*>* source, vector<Field*>* other, Type* type, vector<Type*>* merged)
{
...
void Symbol_IR::mergeEnums(vector<EnumMember*>* source, vector<EnumMember*>* other)
{
  /* Create a map of all fields in the source list. */
  ...
  if (NULL != type) {
    ...
    if ((NULL != oldType) && (_typeSet.end() == _typeSet.find(oldType))) {
      ...
      if (NULL == replacementType) {
        ERRMSG("Error replacing type '%s' in '%s'", oldType->getFullName().c_str(), outer->getFullName().c_str());
        rc = DDR_RC_ERROR;
      } else if (oldType == replacementType) {
        /* don't create a cycle */
        ERRMSG("Cycle found replacing type '%s' in '%s'", oldType->getFullName().c_str(), outer->getFullName().c_str());
        rc = DDR_RC_ERROR;
        *type = NULL;
      } else {
