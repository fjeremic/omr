...
NamespaceUDT::NamespaceUDT(unsigned int lineNumber)
    : UDT(0, lineNumber), _subUDTs(), _macros(), _enumMembers() {}

NamespaceUDT::~NamespaceUDT() {
  for (vector<EnumMember*>::iterator it = _enumMembers.begin();
       it != _enumMembers.end(); ++it) {
    delete *it;
  }
  _enumMembers.clear();

  for (vector<UDT*>::const_iterator it = _subUDTs.begin(); it != _subUDTs.end();
       ++it) {
    delete *it;
  }
  _subUDTs.clear();
...
std::vector<UDT*>* NamespaceUDT::getSubUDTS() {
  return &_subUDTs;
}

void NamespaceUDT::renameFieldsAndMacros(const FieldOverride& fieldOverride,
                                         Type* replacementType) {
  UDT::renameFieldsAndMacros(fieldOverride, replacementType);

  if (!fieldOverride.isTypeOverride) {
    for (vector<Macro>::iterator it = _macros.begin(); it != _macros.end();
         ++it) {
...
bool NamespaceUDT::operator==(const Type& rhs) const {
