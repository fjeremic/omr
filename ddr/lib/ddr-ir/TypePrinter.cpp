...
void TypePrinter::printIndent() const {
  for (int32_t i = _indent; i > 0; --i) {
    _portLibrary->tty_printf(_portLibrary, "  ");
  }
}

void TypePrinter::printFields(const std::vector<Field*>& fields) const {
  if (0 != (_flags & FIELDS)) {
...
void TypePrinter::printLiterals(
    const std::vector<EnumMember*>& literals) const {
  if (0 != (_flags & LITERALS)) {
    for (vector<EnumMember*>::const_iterator it = literals.begin();
         it != literals.end(); ++it) {
      printIndent();
...
void TypePrinter::printMacros(const std::vector<Macro>& macros) const {
  if (0 != (_flags & MACROS)) {
    for (vector<Macro>::const_iterator it = macros.begin(); it != macros.end();
         ++it) {
      printIndent();
...
TypePrinter::visitClass(ClassUDT* type) const {
  ...
  if (NULL != type->_superClass) {
    _portLibrary->tty_printf(_portLibrary, ":  %s",
                             type->_superClass->_name.c_str());
  }
  _portLibrary->tty_printf(_portLibrary, " {\n");
