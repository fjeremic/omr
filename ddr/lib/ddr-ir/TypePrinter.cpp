...
void TypePrinter::printLiterals(const std::vector<EnumMember*>& literals) const
{
...
TypePrinter::visitClass(ClassUDT* type) const
{
  ...
  _portLibrary->tty_printf(
    _portLibrary, "%s '%s' size(%zu)", type->_isClass ? "class" : "struct", type->_name.c_str(), type->_sizeOf);
