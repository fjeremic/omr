...
ClassType::ClassType(size_t size, unsigned int lineNumber)
    : NamespaceUDT(lineNumber), _fieldMembers() {
  _sizeOf = size;
}

ClassType::~ClassType() {
  for (vector<Field*>::iterator it = _fieldMembers.begin();
       it != _fieldMembers.end(); ++it) {
    delete *it;
  }
  _fieldMembers.clear();
}

void ClassType::renameFieldsAndMacros(const FieldOverride& fieldOverride,
...
bool ClassType::operator==(const Type& rhs) const {
...
bool ClassType::compareToClasstype(const ClassType& other) const {
  ...
  for (vector<EnumMember*>::const_iterator it = _enumMembers.begin();
       it != _enumMembers.end() && it2 != other._enumMembers.end() &&
       enumMembersEqual;
       ++it, ++it2) {
    enumMembersEqual =
        ((*it)->_name == (*it2)->_name) && ((*it)->_value == (*it2)->_value);
  }
  ...
  for (vector<Field*>::const_iterator it = _fieldMembers.begin();
       it != _fieldMembers.end() && it3 != other._fieldMembers.end() &&
       fieldMembersEqual;
       ++it, ++it3) {
    fieldMembersEqual =
        ((*it)->_name == (*it3)->_name) &&
