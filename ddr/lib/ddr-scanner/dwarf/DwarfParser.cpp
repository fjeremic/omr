               ...
               Dwarf_Error* error)
{
  ...
  if (0 == strncmp("0x", line, 2)) {
    ...
    if (0 == strncmp(endOfAddress + spaces, "Compile Unit: ", 14)) {
      ...
      ret = parseDwarfDie(endOfAddress + spaces + 4,
                          lastCreatedDie,
                          currentDie,
                          lastIndent,
                          spaces,
    ...
    if ((spaces == *lastIndent + 12) &&
      ...
      ret =
        parseAttribute(line + spaces + 3, lastCreatedDie, refToPopulate, error);
    }
  }
  return ret;
}
