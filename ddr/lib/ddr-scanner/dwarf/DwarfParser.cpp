...
static int parseDwarfInfo(char *line, Dwarf_Die *lastCreatedDie,
                          Dwarf_Die *currentDie, size_t *lastIndent,
...
static int parseAttribute(char *line, Dwarf_Die *lastCreatedDie,
                          unordered_map<Dwarf_Die *, Dwarf_Off> *refToPopulate,
                          Dwarf_Error *error);
static void parseAttrType(char *string, size_t length, Dwarf_Half *type,
                          Dwarf_Half *form);
...
int dwarf_init(int fd, Dwarf_Unsigned access, Dwarf_Handler errhand,
   ...
   * The bundle must first be created by using dsymutil on an object or
   * executable.
   */
  FILE *fp = NULL;
  if (DW_DLV_OK == ret) {
  ...
  if (DW_DLV_OK == ret) {
    ...
    while ((-1 != getline(&buffer, &len, fp)) && (DW_DLV_OK == ret)) {
      if (0 == strncmp(buffer, "error", 5)) {
        ...
        ret = parseDwarfInfo(buffer, &lastCreatedDie, &currentDie, &lastIndent,
                             &refToPopulate, error);
      }
    }
    free(buffer);
  ...
  if (DW_DLV_OK == ret) {
    for (unordered_map<Dwarf_Die *, Dwarf_Off>::iterator it =
             refToPopulate.begin();
...
static void cleanUnknownDies(Dwarf_Die die) {
...
static int parseDwarfInfo(char *line, Dwarf_Die *lastCreatedDie,
   ...
   * "0x[address]: [whitespace...] TAG_*" to represent a new Die
   * org/legal/epl-v10
   * "[whitespae...] AT_*( {[optional address]} ( [value] ) ) for a die
   * attribute. The amount of whitespace varies to indicate parent-child
   * relationships between Die's.
   */
  if (0 == strncmp("0x", line, 2)) {
    ...
    if (0 == strncmp(endOfAddress + spaces, "Compile Unit: ", 14)) {
      ...
      ret = parseDwarfDie(endOfAddress + spaces + 4, lastCreatedDie, currentDie,
                          lastIndent, spaces, error);
    ...
    if ((spaces == *lastIndent + 12) &&
        (0 == strncmp(line + spaces, "AT_", 3))) {
      ret = parseAttribute(line + spaces + 3, lastCreatedDie, refToPopulate,
                           error);
    }
  }
  return ret;
...
static int parseCompileUnit(char *line, size_t *lastIndent,
                            Dwarf_Error *error) {
  /* Compile unit lines are of the format:
   * 0x00000000: Compile Unit: length = 0x[offset]  version = 0x[version]
   * abbr_offset = 0x[offset]  addr_size = 0x[size]  (next CU at 0x[offset])
   */
  int ret = DW_DLV_OK;
  ...
  if (DW_DLV_OK == ret) {
    ...
    if (NULL == valueString) {
      ...
      nextCUheaderOffset =
          (Dwarf_Unsigned)strtoul(valueString + 12, &valueString, 16);
    }
  }
...
static const tuple<const char *, Dwarf_Half, Dwarf_Half> attrStrings[] = {
    ...
    make_tuple("data_member_location", DW_AT_data_member_location,
               DW_FORM_udata),
