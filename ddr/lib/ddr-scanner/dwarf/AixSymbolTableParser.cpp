...
static Dwarf_Half parseDeclarationLine(
    string data, int* typeID, declFormat* format, string* declarationData, string* name);
...
int dwarf_init(
    int fd, Dwarf_Unsigned access, Dwarf_Handler errhand, Dwarf_Ptr errarg, Dwarf_Debug* dbg, Dwarf_Error* error)
{
...
/*
 * Parses the data passed in and populates a DIE as a constant/pointer/volatile/subroutine type (all of those types have
 * the same structure) param[in] data: the data to be parsed, it should be of the form (*|k|V)(TYPE) param[out]
 * currentDie: the DIE to populate with the parsed data.
 */
static int parseNamelessReference(const string data, Dwarf_Die currentDie, Dwarf_Error* error)
{
    int ret = DW_DLV_OK;
 ...
 *
 * @param[in]  data: the string to be parsed. data should be the portion of the declaration line after "debug     0
 * decl" and whitespace.
 ...
 */
static Dwarf_Half parseDeclarationLine(
    const string data, int* typeID, declFormat* format, string* declarationData, string* name)
{
 ...
 * param[in] line: declaration for an entity. It should be the portion of a declaration line after
 * "^\\[.*\\].*m.*debug.*decl\\s+"
 */
static int parseStabstringDeclarationIntoDwarfDie(const string line, Dwarf_Error* error)
{
    int ret = DW_DLV_OK;
    int typeID = 0;
    ...
    /* We are parsing the section of debug info containing type declarations, comprised of lines beginning with "debug
     * 0		decl". Each declaration line is of the format:
     * (DECLARATION_NAME):(c|t|T)(INTEGER_TYPE_ID)=(DECLARATION_TYPE)(FIELD_MEMBERS) where DECLARATION_TYPE is one of:
     * (*(INTEGER)|a|b|c|C|d|D|e|f|g|i|k|m|M|r|s|S|V|w|Z). See parseDeclarationLine() for more information. The format
     * differs for constants, typedefs, structures, etc. First, determine the type of declaration. Then parse that
     * specific format.
     */
...
static int parseSymbolTable(const char* line, Dwarf_Error* error)
{
    ...
    if (0 == _startNewFile) {
         ...
         * Search for the pattern "debug	3	FILE", indicating the start of a new file, to progress to
         * step 2.
         */
        ...
        /* Skip lines of the format "debug		0		(b|e)incl" until finding a line of the format "debug
         * 0		decl". These first lines in the file list includes before the section listing the type
         * declarations.
         */
        ...
        if (string::npos != indexOfDebug) {
            ...
            if (string::npos != indexOfNumber) {
                ...
                /* Check if it is the declaration for the start of another file as the previous file may have contained
                 * no symbols */
         ...
         */
        size_t indexOfDebug = data.find(DECL_FILE[0]);
        /* Verify that the type declaration section has not ended by checking that the line begins with "debug
         * 0		decl". */
        if (string::npos != indexOfDebug) {
            string dataAfterDebug = data.substr(indexOfDebug + DECL_FILE[0].length());
            size_t indexOfNumber = dataAfterDebug.find(DECL_FILE[1]);
 ...
 * (s|u)(INTEGER)(NAME):(TYPE),(BIT_OFFSET),(NUM_BITS);(NAME):(TYPE),(BIT_OFFSET),(NUM_BITS);(...);; param[in] dieName:
 * the name of the structure/union param[out] currentDie: the DIE to populate with the parsed data.
 */
static int parseCstruct(const string data, const string dieName, Dwarf_Die currentDie, Dwarf_Error* error)
{
    int ret = DW_DLV_OK;
            ...
            if (string::npos != indexOfColon) {
                ...
                if ((NULL == type) || (NULL == inheritanceDie)) {
                    ...
                    refsToPopulate.push_back(
                        make_pair<int, Dwarf_Attribute>((int)parentClassID, (Dwarf_Attribute)type));
                        ...
                        if (NULL == newDie) {
                            ...
                            if (2 <= members[i].size()) {
                                if (string::npos != members[i].find_first_of('[')) {
                                    /* The member is a member function. We will delete the DIE as functions are not
                                     * necessary. */
                                    deleteDie(newDie);
                                } else if (string::npos != members[i].find_first_of(']')) {
                                    /* The member is a friend function. We will delete the DIE as functions are not
                                     * necessary. */
                                    deleteDie(newDie);
                                } else if (string::npos != members[i].find_first_of(':')) {
        ...
        if (0 > refsToPopulate[i].first) {
            ...
            if (builtInDies.end() != existingDieEntry) {
                ...
                Dwarf_Die_s::refMap.insert(make_pair<Dwarf_Off, Dwarf_Die>(
                    (Dwarf_Off)existingDieEntry->second.first, (Dwarf_Die)existingDieEntry->second.second));
            }
        } else {
            /* Search for the reference in the DIE map. */
        ...
        if (NULL != parent) {
            ...
            if (createdDies.end() != existingDieEntry) {
                ...
                if (NULL != nestedClass) {
                    ...
                    /* In case the current parent of nestedClass has nestedClass as the child, set it to be the sibling
                     * of nestedClass. */
...
static void removeUselessStubs()
{
    ...
    while (it != createdDies.end()) {
        ...
        if (Dwarf_Die_s::refMap.end() == Dwarf_Die_s::refMap.find(refNumberToCheck)) {
            ...
            if ((DW_TAG_class_type == dieToCheck->_tag) || (DW_TAG_union_type == dieToCheck->_tag)
                || (DW_TAG_structure_type == dieToCheck->_tag)) {
                if ((Dwarf_CU_Context::_currentCU->_die == dieToCheck->_parent) && (NULL == dieToCheck->_child)) {
                    /* If the DIE has a size of zero, has no children, and is not being referred to by any other DIE
                     * then it's useless. */
                    bool zeroSize = true;
