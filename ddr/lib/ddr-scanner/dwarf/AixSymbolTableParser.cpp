...
static int
parseNamelessReference(const string data,
                       Dwarf_Die currentDie,
                       Dwarf_Error* error);
static int
parseStabstringDeclarationIntoDwarfDie(const string data, Dwarf_Error* error);
static int
parseSymbolTable(const char* line, Dwarf_Error* error);
static Dwarf_Half
parseDeclarationLine(string data,
                     int* typeID,
                     declFormat* format,
...
parseBaseType(const string data, Dwarf_Die currentDie, Dwarf_Error* error)
{
  ...
  if (!BUILT_IN_TYPES[index].empty()) {
    ...
    ret = parseTypeDef(
      builtInType, BUILT_IN_TYPES_THAT_ARE_TYPEDEFS[index], currentDie, error);
  }
  return ret;
}
/*
      ...
      *typeID = extractTypeID(tmp[0]);
      ...
      if (string::npos != indexOfFirstEqualsSign) {
        *typeID =
        ...
        string members =
          stripLeading((*declarationData).substr(indexOfFirstEqualsSign), '=');
  ...
  if (string::npos != indexOfFirstParenthesis) {
    ...
    if (string::npos != indexOfFirstNonInteger) {
      ...
      if (string::npos != indexOfColon) {
        ...
        int parentClassID =
          toInt(stripLeading(sizeTypeAndInheritance.substr(indexOfColon), ':'));
            ...
            if (NULL == newDie) {
              ...
              if (2 <= members[i].size()) {
                if (string::npos != members[i].find_first_of('[')) {
                  ...
                  if (string::npos != indexOfFirstColon) {
                    ret = parseFields(
                      members[i].substr(indexOfFirstColon + 1), newDie, error);
                    if (DW_DLV_OK == ret) {
                  ...
                  int ID = toInt(tmp[1]);
                  nestedClassesToPopulate.push_back(
                    make_pair<int, Dwarf_Die>((int)ID, (Dwarf_Die)currentDie));
  ...
  for (unsigned int i = 0; i <= NUM_BUILT_IN_TYPES; ++i) {
    if ((DW_DLV_OK == ret) && (!BUILT_IN_TYPES_THAT_ARE_TYPEDEFS[i].empty())) {
      ...
      if (NULL == newDie) {
        ...
        ret = parseTypeDef(
          data, BUILT_IN_TYPES_THAT_ARE_TYPEDEFS[i], newDie, error);
