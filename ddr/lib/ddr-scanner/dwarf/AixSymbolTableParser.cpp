...
/* This vector contains the typeID of the nested class and parent class for
 * populating nested classes. */
static vector<pair<int, Dwarf_Die> > nestedClassesToPopulate;
/* This vector contains the typeID of the type reference and type attribute to
 * populate. */
...
/* Start numbering DIE refs for refMap at one (zero will be for when no ref is
 * found). */
static Dwarf_Off refNumber = 1;
/* This is used to initialize declLine to a unique value for each unique DIE
...
static int parseArray(const string data, Dwarf_Die currentDie,
                      Dwarf_Error *error);
static int parseBaseType(const string data, Dwarf_Die currentDie,
                         Dwarf_Error *error);
static int parseNamelessReference(const string data, Dwarf_Die currentDie,
                                  Dwarf_Error *error);
static int parseStabstringDeclarationIntoDwarfDie(const string data,
                                                  Dwarf_Error *error);
static int parseSymbolTable(const char *line, Dwarf_Error *error);
static Dwarf_Half parseDeclarationLine(string data, int *typeID,
                                       declFormat *format,
                                       string *declarationData, string *name);
static int parseEnum(const string data, string dieName, Dwarf_Die currentDie,
                     Dwarf_Error *error);
static int parseFields(const string data, Dwarf_Die currentDie,
                       Dwarf_Error *error);
static int parseCPPstruct(const string data, const string dieName,
                          Dwarf_Die currentDie, Dwarf_Error *error);
static int parseCstruct(const string data, const string dieName,
                        Dwarf_Die currentDie, Dwarf_Error *error);
static int parseTypeDef(const string data, const string dieName,
                        Dwarf_Die currentDie, Dwarf_Error *error);
...
static int setDieAttributes(const string dieName, const unsigned int dieSize,
                            Dwarf_Die currentDie, Dwarf_Error *error);
...
int dwarf_init(int fd, Dwarf_Unsigned access, Dwarf_Handler errhand,
  ...
  /* If the symbol table that we parsed contained no new files, create an empty
   * placeholder generic CU and CU DIE. */
...
static void checkBitFields(Dwarf_Error *error) {
  for (size_t i = 0; i < bitFieldsToCheck.size(); ++i) {
    /* Since this is a field, the type attribute is always the attribute after
     * the first of the die passed in. */
      ...
      while (!baseTypeHasBeenFound) {
        switch (tmp->_tag) {
          ...
          case DW_TAG_volatile_type:
            /* Check if the type that this is referring to has been found
             * before. */
            if (DW_DLV_OK ==
                dwarf_attr(tmp, DW_AT_type, &tmpAttribute, error)) {
          ...
          case DW_TAG_pointer_type:
            /* Compare the bit size with the pointer size for the current
             * architecture. */
...
/**
 * Parses the string to file ID. Extracts the integer betwen the square
 * brackets.
 *
 ...
 */
int extractTypeID(const string data, const int index) {
  stringstream ss;
  int ret = 0;
 ...
 * param[in] data: the data to be parsed, should be in the format
 * ar(INDEX_ID);(LOWER_BOUND);(UPPER_BOUND);(ARRAY_TYPE) param[out] currentDie:
 * the DIE to populate with the parsed data.
 */
static int parseArray(const string data, Dwarf_Die currentDie,
                      Dwarf_Error *error) {
  int ret = DW_DLV_OK;
  if (2 <= data.length()) {
    if ("ar" == data.substr(0, 2)) {
      ...
      /* arrayInfo is in the form:
       * ar(INDEX_ID);(LOWER_BOUND);(UPPER_BOUND);(ARRAY_TYPE) arrayInfo[0] : ar
       * + subrange type arrayInfo[1] : lower bound arrayInfo[2] : upper bound
      ...
      if ((NULL == type) || (NULL == newDie) || (NULL == subrangeType) ||
          (NULL == subrangeUpperBound)) {
        ret = DW_DLV_ERROR;
        setError(error, DW_DLE_MAF);
      } else {
        ...
        refsToPopulate.push_back(
            make_pair<int, Dwarf_Attribute>((int)ID, (Dwarf_Attribute)type));
        ...
        refsToPopulate.push_back(make_pair<int, Dwarf_Attribute>(
            (int)-37, (Dwarf_Attribute)subrangeType));
 ...
 * param[in] data: the data to be parsed, it should consist of the following
 * characters only: "1234567890-" param[out] currentDie: the DIE to populate
...
static int parseBaseType(const string data, Dwarf_Die currentDie,
  ...
  if (!BUILT_IN_TYPES[index].empty()) {
    ...
    ret = parseTypeDef(builtInType, BUILT_IN_TYPES_THAT_ARE_TYPEDEFS[index],
                       currentDie, error);
  }
  return ret;
}
/*
 * Parses the data passed in and populates a DIE as a
 * constant/pointer/volatile/subroutine type (all of those types have the same
 * structure) param[in] data: the data to be parsed, it should be of the form
 * (*|k|V)(TYPE) param[out] currentDie: the DIE to populate with the parsed
 * data.
 */
static int parseNamelessReference(const string data, Dwarf_Die currentDie,
                                  Dwarf_Error *error) {
 ...
 *
 * @param[in]  data: the string to be parsed. data should be the portion of the
 * declaration line after "debug     0    decl" and whitespace.
 * @param[out] tag: the type of the declaration of the data string
 * @param[out] typeID: the ID of the entity being declared
 * @param[out] format: the format of the declaration being parsed (used for
 ...
 */
static Dwarf_Half parseDeclarationLine(const string data, int *typeID,
                                       declFormat *format,
                                       string *declarationData, string *name) {
  Dwarf_Half ret = DW_TAG_unknown;

  /* 'data' is of the format:
   * (DECLARATION_NAME):(c|t|T)(INTEGER_TYPE_ID)=(DECLARATION_TYPE_AND_SIZE)(FIELD_MEMBERS)
   * Split the declaration into the name and the information about the
   * declaration.
   */
  size_t indexOfFirstColon = data.find_first_of(':');
  if (string::npos != indexOfFirstColon) {
    /* The data first includes the name of the declaration, then a ":", and then
     * the rest of the data. */
    *declarationData = stripLeading(data.substr(indexOfFirstColon), ':');
    *name = data.substr(0, indexOfFirstColon);

    /* A "T" after the ":" indicates a type and a "t" after the ":" indicates a
     * typedef. The number proceeding it is unique integer type ID for the type.
     * This is followed by "=" and then the type of the declaration
     * (union/struct/etc).
     */
    if ((0 == (*name).size()) && ('t' == (*declarationData).at(0))) {
      ...
      *typeID = extractTypeID(tmp[0]);
      ...
      /* After the "=", the declaration type and field members have two possible
       * formats: C-style and C++-stype. C-style formatting is:
       * (e/s/u)(DECLARED_TYPE_SIZE)(FIELD_MEMBERS) C++-style formatting is:
       * Y(DECLARED_TYPE_SIZE)(c/e/s/u)((FIELD_MEMBERS))
       */
      if (string::npos != indexOfFirstEqualsSign) {
        *typeID =
            extractTypeID((*declarationData).substr(0, indexOfFirstEqualsSign));
        string members = stripLeading(
            (*declarationData).substr(indexOfFirstEqualsSign), '=');
        if (string::npos != (*declarationData).find('(')) {
          /* C++ type class/struct/union declaration */
          size_t indexOfFirstParenthesis = members.find('(');
          string sizeAndType =
              stripLeading(members.substr(0, indexOfFirstParenthesis), 'Y');
          size_t indexOfFirstNonInteger =
              sizeAndType.find_first_not_of("1234567890");
 ...
 * param[in] line: declaration for an entity. It should be the portion of a
 * declaration line after "^\\[.*\\].*m.*debug.*decl\\s+"
 */
static int parseStabstringDeclarationIntoDwarfDie(const string line,
                                                  Dwarf_Error *error) {
  int ret = DW_DLV_OK;
  int typeID = 0;
  ...
  /* We are parsing the section of debug info containing type declarations,
   * comprised of lines beginning with "debug		0		decl".
   * Each declaration line is of the format:
   * (DECLARATION_NAME):(c|t|T)(INTEGER_TYPE_ID)=(DECLARATION_TYPE)(FIELD_MEMBERS)
   * where DECLARATION_TYPE is one of:
   * (*(INTEGER)|a|b|c|C|d|D|e|f|g|i|k|m|M|r|s|S|V|w|Z). See
   * parseDeclarationLine() for more information. The format differs for
   * constants, typedefs, structures, etc. First, determine the type of
   * declaration. Then parse that specific format.
   */

  /* Get the type of the data of the parsed line. */
  Dwarf_Half tag =
      parseDeclarationLine(line, &typeID, &format, &declarationData, &name);
  ...
  if (DW_TAG_unknown != tag) {
    ...
    if (NULL == newDie) {
      ...
      if (NULL == newDie) {
        ...
        createdDies[typeID] = make_pair<Dwarf_Off, Dwarf_Die>(
            (Dwarf_Off)refNumber, (Dwarf_Die)newDie);

        refNumber = refNumber + 1;

        /* If the current CU DIE doesn't have a child then make the first new
         * new DIE the child. */
      ...
      /* Forward declarations of unions can be found as structs. Update them to
       * union. */
      newDie->_tag = tag;
    }
    if (NULL == newDie) {
...
/**
 * Takes in and parses the AIX symbol table line-by-line, creating Dwarf DIEs
 * and CU Contexts. param[in] line: A line read from AIX symbol table
 */
static int parseSymbolTable(const char *line, Dwarf_Error *error) {
  int ret = DW_DLV_OK;
  string data = line;
   ...
   * 1: reached the start of a new file: parse to get info about the file and
   * populate Dwarf_CU_Context::_currentCU 2: inside of a file: skip the section
   * listing the includes until the first type declaration is found 3: parse
   * declarations: parse to get the information to populate dwarf DIEs and check
   * if end of file reached
   */
  if (0 == _startNewFile) {
    /* Ignore the information before the start of the debug information for a
     * file.
     * Search for the pattern "debug	3	FILE", indicating the start of a new
     * file, to progress to step 2.
     */
    size_t indexOfDebug = data.find(START_OF_FILE[0]);
    if (string::npos != indexOfDebug) {
      string dataAfterDebug =
          data.substr(indexOfDebug + START_OF_FILE[0].length());
      size_t indexOfNumber = dataAfterDebug.find(START_OF_FILE[1]);
      if (string::npos != indexOfNumber) {
        string dataAfterNumber =
            dataAfterDebug.substr(indexOfNumber + START_OF_FILE[1].length());
        ...
        if (string::npos != indexOfFile) {
          ...
          if ((indexOfNumber == dataAfterDebug.find_first_not_of(' ')) &&
              (indexOfFile == dataAfterNumber.find_first_not_of(' '))) {
            /* Populate the attribute references and nested classes from the
             * previous file that we parsed. */
            populateAttributeReferences();
            populateNestedClasses();

            /* Remove DIEs which have no children, no size, and are not referred
             * to by any other DIE. */
            removeUselessStubs();
            ...
            nestedClassesToPopulate.clear();
            refsToPopulate.clear();
            /* Change state to expect the start of a section of debug info for a
             * file next. */
            _startNewFile = 1;
          }
        }
    ...
    /* Verify that the section for a new file begins with a line starting with "
     * a0 ". */
    if (string::npos != data.find(FILE_NAME)) {
      ...
      if (filesAdded.end() == filesAdded.find(fileName)) {
        ...
        if ((NULL == newCU) || (NULL == newDie) || (NULL == name)) {
          ...
          name->_udata = 0;
          name->_stringdata =
              strdup(Dwarf_CU_Context::_fileList.back().c_str());
          name->_refdata = 0;
          name->_ref = NULL;
        ...
        /* The file was already parsed through previously, perhaps in another
         * .so file. */
    ...
    /* Skip lines of the format "debug		0		(b|e)incl" until finding a line
     * of the format "debug	0		decl". These first lines in the
     * file list includes before the section listing the type declarations.
     */
    ...
    if (string::npos != indexOfDebug) {
      ...
      if (string::npos != indexOfNumber) {
        string dataAfterNumber =
            dataAfterDebug.substr(indexOfNumber + DECL_FILE[1].length());
        size_t indexOfDecl = dataAfterNumber.find(DECL_FILE[2]);
        if (string::npos != indexOfDecl) {
          /* Verify that there are only whitespaces between the words. */
          if ((indexOfNumber == dataAfterDebug.find_first_not_of(' ')) &&
              (indexOfDecl == dataAfterNumber.find_first_not_of(' '))) {
            /* Parse the data line to populate a DIE. */
            string dataAfterDecl =
                dataAfterNumber.substr(indexOfDecl + DECL_FILE[2].length());
        ...
        /* Check if it is the declaration for the start of another file as the
         * previous file may have contained no symbols */
    ...
    /* Stop parsing whenever the end of the decl section is reached because all
     * of the declarations in a file are guaranteed to be in a continuous block.
     */
    size_t indexOfDebug = data.find(DECL_FILE[0]);
    /* Verify that the type declaration section has not ended by checking that
     * the line begins with "debug		0		decl". */
    if (string::npos != indexOfDebug) {
      string dataAfterDebug = data.substr(indexOfDebug + DECL_FILE[0].length());
      size_t indexOfNumber = dataAfterDebug.find(DECL_FILE[1]);
      if (string::npos != indexOfNumber) {
 ...
 * param[in] data: the data to be parsed, should be in the format
 * e(INTEGER):(NAME):(INTEGER),(NAME):(INTEGER),(...),; param[in] dieName: the
 * name of the enumerator param[out] currentDie: the DIE to populate with the
 * parsed data.
 */
static int parseEnum(const string data, string dieName, Dwarf_Die currentDie,
                     Dwarf_Error *error) {
  ...
  if (string::npos != indexOfFirstNonInteger) {
    ...
    /* Check if the enum was a generated name by AIX, and if so, delete the
     * generated name. */
    if ((0 == strcmp("__", dieName.substr(0, 2).c_str())) &&
        (string::npos == dieName.substr(2).find_first_not_of("1234567890"))) {
    ...
    if (DW_DLV_OK == ret) {
      ...
      if (NULL == newDie) {
        ...
        if (DW_DLV_OK == ret) {
          ...
          for (str_vect::iterator it = enumerators.begin();
               it != enumerators.end(); ++it) {
            tmp = split(*it, ':');
 ...
 * param[in] data: the data to be parsed, should be in the format
 * (NAME):(TYPE),(BIT_OFFSET),(NUM_BITS) param[out] currentDie: the DIE to
 * populate with the parsed data.
 */
static int parseFields(const string data, Dwarf_Die currentDie,
                       Dwarf_Error *error) {
  int ret = DW_DLV_OK;
    ...
    if (fieldAttributes.size() >= 3) {
      ...
      if (0 > ID) {
        if (toInt(fieldAttributes[2]) == BUILT_IN_TYPE_SIZES[ID * -1]) {
          /* If the type is built-in and the sizes match, then bitSize is not
           * needed. */
          bitSizeNeeded = false;
        }
      }
 ...
 * param[in] data: data to be parsed, in the format
 * (s|u)(INTEGER)(NAME):(TYPE),(BIT_OFFSET),(NUM_BITS);(NAME):(TYPE),(BIT_OFFSET),(NUM_BITS);(...);;
...
static int parseCstruct(const string data, const string dieName,
                        Dwarf_Die currentDie, Dwarf_Error *error) {
  int ret = DW_DLV_OK;
  /* Get rid of the first index of data. It only specifies if the declaration is
   * for a struct or union. */
  string unparsedMembers = data.substr(1);
  size_t indexOfFirstNonInteger =
      unparsedMembers.find_first_not_of("1234567890");
  if (string::npos != indexOfFirstNonInteger) {
 ...
 * param[in] data: the data to be parsed, in the format
 * Y(TYPE)(c|u|s){1}(V)?:(TYPE)((MEMBERS);; param[in] dieName: the name of the
 * class/structure/union param[out] currentDie: the DIE to populate with the
 * parsed data.
 */
static int parseCPPstruct(const string data, const string dieName,
                          Dwarf_Die currentDie, Dwarf_Error *error) {
  ...
  if (string::npos != indexOfFirstParenthesis) {
    string sizeTypeAndInheritance =
        stripLeading(data.substr(0, indexOfFirstParenthesis), 'Y');
    size_t indexOfFirstNonInteger =
        sizeTypeAndInheritance.find_first_not_of("1234567890");
    if (string::npos != indexOfFirstNonInteger) {
      string unparsedMembers =
          stripLeading(data.substr(indexOfFirstParenthesis), '(');

      unsigned int size =
          toInt(sizeTypeAndInheritance.substr(0, indexOfFirstNonInteger));

      /* If there is a parent class, create a DIE with tag DW_TAG_inheritance.
       * The numbers after the colon denote the ID of the parent class this
       * class inherits from.
       */
      size_t indexOfColon = sizeTypeAndInheritance.find(':');
      if (string::npos != indexOfColon) {
        /* A parent class exists. */
        int parentClassID = toInt(
            stripLeading(sizeTypeAndInheritance.substr(indexOfColon), ':'));
        ...
        if ((NULL == type) || (NULL == inheritanceDie)) {
          ...
          refsToPopulate.push_back(make_pair<int, Dwarf_Attribute>(
              (int)parentClassID, (Dwarf_Attribute)type));
            ...
            if (NULL == newDie) {
              ...
              if (2 <= members[i].size()) {
                if (string::npos != members[i].find_first_of('[')) {
                  /* The member is a member function. We will delete the DIE as
                   * functions are not necessary. */
                  deleteDie(newDie);
                } else if (string::npos != members[i].find_first_of(']')) {
                  /* The member is a friend function. We will delete the DIE as
                   * functions are not necessary. */
                  deleteDie(newDie);
                } else if (string::npos != members[i].find_first_of(':')) {
                  ...
                  if (string::npos != indexOfFirstColon) {
                    ret = parseFields(members[i].substr(indexOfFirstColon + 1),
                                      newDie, error);
                    if (DW_DLV_OK == ret) {
                  ...
                  int ID = toInt(tmp[1]);
                  nestedClassesToPopulate.push_back(make_pair<int, Dwarf_Die>(
                      (int)ID, (Dwarf_Die)currentDie));
...
static int parseTypeDef(const string data, const string dieName,
  ...
  if ((NULL == type) || (NULL == name) || (NULL == declFile) ||
      (NULL == declLine)) {
    ret = DW_DLV_ERROR;
    setError(error, DW_DLE_MAF);
  } else {
    ...
    /* AIX classifies int64_t and uint64_t as intptr_t (IDATA) and uintptr_t
     * (UDATA) respectively. */
    if ((0 == strcmp("int64_t", dieName.c_str())) ||
        (0 == strcmp("uint64_t", dieName.c_str()))) {
      if (-36 == toInt(data)) {
        ID = -32;
    ...
    /* Add the new type attribute to the list of refs to populate on a second
     * pass. */
    refsToPopulate.push_back(
        make_pair<int, Dwarf_Attribute>((int)ID, (Dwarf_Attribute)type));
    name->_type = DW_AT_name;
    ...
    if (!Dwarf_CU_Context::_fileList.empty()) {
      ...
      /* If there is no declaring file then delete the attributes for the
       * declFile and declLine. */
      name->_nextAttr = NULL;
      delete declFile;
      delete declLine;
    }
...
/**
 * Looks up and populates the reference attributes which were added while
 * populating Dwarf DIEs
 */
static void populateAttributeReferences() {
  ...
  for (unsigned int i = 0; i < refsToPopulate.size(); ++i) {
    ...
    /* If the reference is to a built-in type then search the unordered_map
     * containing built-in types. */
    if (0 > refsToPopulate[i].first) {
      /* Search for the reference in the DIE map. */
      die_map::iterator existingDieEntry =
          builtInDies.find(refsToPopulate[i].first);
      if (builtInDies.end() != existingDieEntry) {
        tmp->_ref = existingDieEntry->second.second;

        /* Set _refdata to the correct value and insert the reference into
         * refMap. */
        tmp->_refdata = existingDieEntry->second.first;
        Dwarf_Die_s::refMap.insert(make_pair<Dwarf_Off, Dwarf_Die>(
            (Dwarf_Off)existingDieEntry->second.first,
            (Dwarf_Die)existingDieEntry->second.second));
      }
    } else {
      /* Search for the reference in the DIE map. */
      die_map::iterator existingDieEntry =
          createdDies.find(refsToPopulate[i].first);
      if (createdDies.end() != existingDieEntry) {
        tmp->_ref = existingDieEntry->second.second;
...
static int populateBuiltInTypeDies(Dwarf_Error *error) {
   ...
   * -16 boolean, .......... 32 bit type < 0 false and 1 true, others
   *unspecified meaning -17 short real, ....... IEEE single precision -18 real,
   *............. IEEE double precision -19 stringptr -20 character, ........ 8
   *bit unsigned character type -21 logical*1, ........ 8 bit type < FORTRAN -22
   *logical*2, ........ 16 bit type < FORTRAN -23 logical*4, ........ 32 bit
   *type for boolean variables < FORTRAN -24 logical, .......... 32 bit type <
   *FORTRAN -25 complex, .......... consisting of two IEEE single-precision
   *floating point values -26 complex, .......... consisting of two IEEE
   *double-precision floating point values -27 integer*1, ........ 8 bit signed
   *integral type -28 integer*2, ........ 16 bit signed integral type -29
   *integer*4, ........ 32 bit signed integral type -30 wchar, ............ wide
   *character 16 bit wide -31 logn long, ........ 64 bit unsigned intergral type
    ...
    if ((DW_DLV_OK == ret) && (!BUILT_IN_TYPES[i].empty())) {
      ...
      if (NULL == newDie) {
        ...
        if ((NULL == name) || (NULL == size)) {
          ...
          /* Insert the newly created DIE into the unordered_map for lookup by
           * offset later on. */
          builtInDies[i * -1] = make_pair<Dwarf_Off, Dwarf_Die>(
              (Dwarf_Off)refNumber, (Dwarf_Die)newDie);
  ...
  /* For redundant types, create typedefs that refer to the base types created
   * above. */
  for (unsigned int i = 0; i <= NUM_BUILT_IN_TYPES; ++i) {
    if ((DW_DLV_OK == ret) && (!BUILT_IN_TYPES_THAT_ARE_TYPEDEFS[i].empty())) {
      ...
      if (NULL == newDie) {
        ...
        ret = parseTypeDef(data, BUILT_IN_TYPES_THAT_ARE_TYPEDEFS[i], newDie,
                           error);
        ...
        if (DW_DLV_OK == ret) {
          ...
          /* Insert the typedef into the unordered_map for lookup by offset
           * later on. */
          builtInDies[i * -1] = make_pair<Dwarf_Off, Dwarf_Die>(
              (Dwarf_Off)refNumber, (Dwarf_Die)newDie);

          /* Increment the refNumber to assign associate a unique number with
           * each DIE. */
          refNumber = refNumber + 1;
        } else {
          delete newDie;
...
/**
 * Finds nested classes, and moves them to their appropriate locations as
 * children of the DIEs that they are a member of
 */
static void populateNestedClasses() {
  ...
  for (unsigned int i = 0; i < nestedClassesToPopulate.size(); ++i) {
    ...
    if (NULL != parent) {
      die_map::iterator existingDieEntry =
          createdDies.find(nestedClassesToPopulate[i].first);
      if (createdDies.end() != existingDieEntry) {
        ...
        if (NULL != nestedClass) {
          ...
          /* In case the current parent of nestedClass has nestedClass as the
           * child, set it to be the sibling of nestedClass. */
...
static void removeUselessStubs() {
  ...
  while (it != createdDies.end()) {
    ...
    if (Dwarf_Die_s::refMap.end() ==
        Dwarf_Die_s::refMap.find(refNumberToCheck)) {
      /* Get rid of empty struct/union/class DIEs. */
      if ((DW_TAG_class_type == dieToCheck->_tag) ||
          (DW_TAG_union_type == dieToCheck->_tag) ||
          (DW_TAG_structure_type == dieToCheck->_tag)) {
        if ((Dwarf_CU_Context::_currentCU->_die == dieToCheck->_parent) &&
            (NULL == dieToCheck->_child)) {
          /* If the DIE has a size of zero, has no children, and is not being
           * referred to by any other DIE then it's useless. */
          bool zeroSize = true;
...
static int setDieAttributes(const string dieName, const unsigned int dieSize,
  ...
  if (NULL != currentDie->_attribute) {
    ...
    if ((NULL == name) || (NULL == size) || (NULL == declFile) ||
        (NULL == declLine)) {
      ret = DW_DLV_ERROR;
      setError(error, DW_DLE_MAF);
    } else {
...
/**
 * Splits the string 'data' on delimeters in argument 'delim' and writes the
 * output to elements.
 *
 * @param[in]  data	  : string that needs to be split
 * @param[in]  delimeters: what the string needs to be seperated on
 * @param[out] elements  : string vector to write out the strings after doing
 * the split to
 */
...
/**
 * Splits the string data on delimeters in argument delimeters and writes the
 * output to elements.
 *
 * @param[in]  data	  : string that needs to be split
 * @param[in]  delimeters: what the string needs to be seperated on
 *
 * @return string vector containing the strings after resulting from the split
 * operation
 */
 ...
 *
 * @return double with decimal point shifted left by length of the argument plus
 * 1
 */
 ...
 */
int toInt(const string value) { return (int)toDouble(value); }
