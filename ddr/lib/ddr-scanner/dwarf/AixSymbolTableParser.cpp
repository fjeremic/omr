...
static int parseStabstringDeclarationIntoDwarfDie(const string line, Dwarf_Error* error)
{
  ...
  /* We are parsing the section of debug info containing type declarations, comprised of lines beginning with "debug 0
   * decl". Each declaration line is of the format:
  ...
  if (0 == _startNewFile) {
    ...
    if (string::npos != indexOfDebug) {
      ...
      if (string::npos != indexOfNumber) {
        ...
        /* Check if it is the declaration for the start of another file as the previous file may have contained no
         * symbols */
     ...
     */
    size_t indexOfDebug = data.find(DECL_FILE[0]);
    /* Verify that the type declaration section has not ended by checking that the line begins with "debug		0
     * decl". */
    if (string::npos != indexOfDebug) {
      string dataAfterDebug = data.substr(indexOfDebug + DECL_FILE[0].length());
      size_t indexOfNumber = dataAfterDebug.find(DECL_FILE[1]);
    ...
    if (NULL != parent) {
      ...
      if (createdDies.end() != existingDieEntry) {
        ...
        if (NULL != nestedClass) {
          ...
          /* In case the current parent of nestedClass has nestedClass as the child, set it to be the sibling of
           * nestedClass. */
    ...
    if (Dwarf_Die_s::refMap.end() == Dwarf_Die_s::refMap.find(refNumberToCheck)) {
      ...
      if ((DW_TAG_class_type == dieToCheck->_tag) || (DW_TAG_union_type == dieToCheck->_tag)
        ...
        if ((Dwarf_CU_Context::_currentCU->_die == dieToCheck->_parent) && (NULL == dieToCheck->_child)) {
          /* If the DIE has a size of zero, has no children, and is not being referred to by any other DIE then it's
           * useless. */
          bool zeroSize = true;
