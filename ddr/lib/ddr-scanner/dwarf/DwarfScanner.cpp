...
DwarfVisitor::visitTypedef(TypedefUDT* newTypedef) const
{
  ...
  if (NULL == newTypedef->_aliasedType) {
    ...
    rc = _scanner->getTypeInfo(_die,
                               &typeDie,
                               &typedefName,
                               &newTypedef->_modifiers,
                               &newTypedef->_sizeOf,
                               NULL);
                            ...
                            const string& fieldName)
{
  ...
  if (DDR_RC_OK ==
      getTypeInfo(
        die, &baseDie, &typeName, &fieldModifiers, &typeSize, &bitField)) {
    /* Get the field UDT. */
    Dwarf_Half tag = 0;
    if (NULL == baseDie) {
    ...
    if (!newField->_isStatic) {
      ...
      if (DW_DLV_ERROR ==
          dwarf_hasattr(
            die, DW_AT_data_member_location, &hasAttrBytes, &error)) {
...
DwarfScanner::traverse_cu_in_debug_section(Symbol_IR* ir)
{
  ...
  while (DDR_RC_OK == rc) {
    ...
    int ret = dwarf_next_cu_header(_debug,
                                   &cuHeaderLength,
                                   &versionStamp,
                                   &abbrevOffset,
                                   &addressSize,
