...
DDR_RC
DwarfScanner::getTypeInfo(
  Dwarf_Die die, Dwarf_Die* dieOut, string* typeName, Modifiers* modifiers, size_t* typeSize, size_t* bitField)
{
  Dwarf_Error err = NULL;
...
DwarfScanner::getTypeTag(Dwarf_Die die, Dwarf_Die* typeDie, Dwarf_Half* tag)
{
  ...
  if (hasAttr) {
    ...
    if (DW_DLV_ERROR == dwarf_whatform(attr, &form, &err)) {
      ...
      dwarf_dealloc(_debug, attr, DW_DLA_ATTR);
      goto getTypeDone;
    } else if ((DW_FORM_ref1 != form) && (DW_FORM_ref2 != form) && (DW_FORM_ref4 != form) && (DW_FORM_ref8 != form)) {
...
DwarfScanner::addDieToIR(Dwarf_Die die, Dwarf_Half tag, NamespaceUDT* outerUDT, Type** type)
{
  ...
  if (DDR_RC_OK == rc) {
    ...
    } else if ((DW_TAG_class_type == tag) || (DW_TAG_structure_type == tag) || (DW_TAG_union_type == tag)
    ...
    do {
      ...
      if (DW_TAG_subprogram == childTag) {
        ...
        if (DDR_RC_OK != addDieToIR(childDie, childTag, newClass, (Type**)&innerUDT)) {
          ...
          /* We only add it to list of subUDTs if innerUDT's _outerNamespace is NULL, because there should only be one
           * outer UDT per inner UDT. */
...
DwarfVisitor::visitClass(ClassUDT* newType) const
{
  ...
  if ((NULL == newType->_superClass) && newType->_fieldMembers.empty() && newType->_enumMembers.empty()
...
DwarfVisitor::visitUnion(UnionUDT* newType) const
{
  DDR_RC rc = DDR_RC_OK;
  /* scan this type if we haven't seen any structure */
  if (newType->_fieldMembers.empty() && newType->_enumMembers.empty() && newType->_subUDTs.empty()) {
    rc = _scanner->scanClassChildren(newType, _die);
  }
  return rc;
}
...
static bool getUdata(Dwarf_Small** value, Dwarf_Unsigned* len, Dwarf_Unsigned* result)
{
...
static bool evaluateOffset(Dwarf_Ptr expr, Dwarf_Unsigned len, Dwarf_Unsigned* offset)
{
  if (0 != len) {
...
DwarfScanner::addClassField(Dwarf_Die die, ClassType* newClass, const string& fieldName)
{
  ...
  if (DDR_RC_OK == getTypeInfo(die, &baseDie, &typeName, &fieldModifiers, &typeSize, &bitField)) {
    ...
    if ((typeName == newClass->_name) && (!newClass->_name.empty())) {
      ...
      if ((DDR_RC_OK != addDieToIR(baseDie, tag, NULL, &newField->_fieldType)) || (NULL == newField->_fieldType)) {
        goto AddUDTFieldDone;
      }
    }
...
DwarfScanner::getSuperUDT(Dwarf_Die die, ClassUDT* udt)
{
  ...
  if (DDR_RC_OK == getTypeTag(die, &superTypeDie, &tag)) {
    ...
    /* Get the super udt. */
    if ((DDR_RC_OK == addDieToIR(superTypeDie, tag, NULL, (Type**)&superUDT)) && (NULL != superUDT)) {
      rc = DDR_RC_OK;
...
DwarfScanner::traverse_cu_in_debug_section(Symbol_IR* ir)
{
  ...
  while (DDR_RC_OK == rc) {
    ...
    int ret = dwarf_next_cu_header(
      _debug, &cuHeaderLength, &versionStamp, &abbrevOffset, &addressSize, &nextCUheader, &error);
    if (DW_DLV_ERROR == ret) {
