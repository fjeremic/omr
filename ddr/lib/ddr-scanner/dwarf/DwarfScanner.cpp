...
public:
  DwarfVisitor(DwarfScanner *scanner, NamespaceUDT *outerUDT, Dwarf_Die die)
      : _scanner(scanner), _outerUDT(outerUDT), _die(die) {}
...
DwarfScanner::getSourcelist(Dwarf_Die die) {
  ...
  if (DW_DLV_ERROR ==
      dwarf_srcfiles(die, &_fileNamesTable, &_fileNameCount, &error)) {
  ...
  for (Dwarf_Signed i = 0; i < _fileNameCount; ++i) {
    ...
    if (0 == strncmp(path, "../", 3)) {
      /* For each file name beginning with "../", reformat the path as an
       * absolute path. */
      fileNamesTableConcat[i] =
          (char *)malloc(strlen(compDir) + strlen(path) + 2);
...
DwarfScanner::blackListedDie(Dwarf_Die die, bool *dieBlackListed) {
  ...
  if (_blacklistedFiles.empty()) {
    ...
    if (DW_DLV_ERROR ==
        dwarf_hasattr(die, DW_AT_decl_file, &hasDeclFile, &err)) {
...
DwarfScanner::getName(Dwarf_Die die, string *name, Dwarf_Off *dieOffset) {
  ...
  if ((DDR_RC_OK == rc) && (NULL != name)) {
    ...
    if (DW_DLV_ERROR == dwarfRC) {
      ...
      /* If the Die has no name attribute, check for specification reference
       * instead. */
      Dwarf_Bool hasAttr = false;
      if (DW_DLV_ERROR ==
          dwarf_hasattr(die, DW_AT_specification, &hasAttr, &err)) {
      ...
      if (hasAttr) {
        /* If it has a specification attribute, check the specification for a
         * name recursively. */
        Dwarf_Attribute attr = NULL;
...
DDR_RC
DwarfScanner::getTypeInfo(Dwarf_Die die, Dwarf_Die *dieOut, string *typeName,
                          Modifiers *modifiers, size_t *typeSize,
                          size_t *bitField) {
  Dwarf_Error err = NULL;
  ...
  if (DDR_RC_OK == getBitField(die, bitField)) {
    ...
    do {
       ...
       */
      if ((DDR_RC_OK != getTypeTag(typeDie, &typeDie, &tag)) ||
          (NULL == typeDie)) {
        rc = DDR_RC_OK;
        break;
      }
      ...
      if (DW_TAG_const_type == tag) {
        ...
        if (DW_DLV_ERROR == ret) {
          ...
          do {
            Dwarf_Bool hasAttr = false;
            if (DW_DLV_ERROR ==
                dwarf_hasattr(child, DW_AT_upper_bound, &hasAttr, &err)) {
            ...
            if (hasAttr) {
              Dwarf_Attribute attr = NULL;
              if (DW_DLV_ERROR ==
                  dwarf_attr(child, DW_AT_upper_bound, &attr, &err)) {
        ...
        /* A "void*" type has the pointer tag, but no base type to derive a
         * name. Pointers of other types will override this name with the base
         * type name.
         */
        rc = DDR_RC_OK;
        if (NULL != typeName) {
        ...
        /* Once a typedef has been found, keep the name and do not replace it
         * with base type's name. */
        if (!foundTypedef) {
          ...
          /* Returned die should be the first type without continuing to the
           * base type through typedefs. */
          /* Be careful not to create a typedef cycle. */
          if ((NULL != dieOut) && (die != typeDie)) {
            *dieOut = typeDie;
          }
        ...
        /* Once a typedef is found, keep the name and do not append more
         * pointer/array notation, but otherwise keep searching the type
         * references for type information.
         */
...
DwarfScanner::getTypeTag(Dwarf_Die die, Dwarf_Die *typeDie, Dwarf_Half *tag) {
  ...
  if (hasAttr) {
    ...
    if (DW_DLV_ERROR == dwarf_whatform(attr, &form, &err)) {
      ...
      dwarf_dealloc(_debug, attr, DW_DLA_ATTR);
      goto getTypeDone;
    } else if ((DW_FORM_ref1 != form) && (DW_FORM_ref2 != form) &&
               (DW_FORM_ref4 != form) && (DW_FORM_ref8 != form)) {
 ...
 * a map of all types if it has not been found before. The type is added to the
 * IR the first time it is discovered. The associated Type object is returned.
 */
DDR_RC
DwarfScanner::addDieToIR(Dwarf_Die die, Dwarf_Half tag, NamespaceUDT *outerUDT,
                         Type **type) {
  DDR_RC rc = DDR_RC_OK;
  Type *newType = NULL;
  ...
  if (DDR_RC_OK == rc) {
    ...
    if (newType->_blacklisted) {
    ...
    } else if ((DW_TAG_class_type == tag) || (DW_TAG_structure_type == tag) ||
               (DW_TAG_union_type == tag) || (DW_TAG_namespace == tag) ||
               (DW_TAG_enumeration_type == tag) || (DW_TAG_typedef == tag)) {
      /*
       ...
       * for types defined within namespaces, do not add it to the main list of
       * types.
       */
      isSubUDT =
          isSubUDT || (string::npos != newType->getFullName().find("::"));
      rc = newType->acceptVisitor(DwarfVisitor(this, outerUDT, die));
      DEBUGPRINTF("Done scanning child info for %s", newType->_name.c_str());
    }

    if (isSubUDT) {
      /* When the type is a stub that was found as a stub before, it might be
       * found as a subUDT now, so we check if it is, and remove it from the
       * main types list if it is.
       */
      if (NULL == newType->getNamespace()) {
        _ir->_types.erase(
            remove(_ir->_types.begin(), _ir->_types.end(), newType),
            _ir->_types.end());
        ...
        if ((nameLen > 9) &&
            (0 == newType->_name.compare(nameLen - 9, 9, "Constants"))) {
          string newName = "";
...
/* Given a Die for a type, return the associated Type object for it. The first
 * time called for each type, this function will create a Type object and add it
 * to the map. On subsequent calls for a given type, the existing Type object in
 * the map will be returned.
 */
DDR_RC
DwarfScanner::getOrCreateNewType(Dwarf_Die die, Dwarf_Half tag, Type **newType,
                                 NamespaceUDT *outerUDT, bool *isNewType) {
  string dieName = "";
  Dwarf_Off dieOffset = 0;
  DDR_RC rc = getName(die, &dieName, &dieOffset);

  if (DDR_RC_OK == rc) {
    unordered_map<Dwarf_Off, Type *>::const_iterator it =
        _typeOffsetMap.find(dieOffset);
 ...
 */
DDR_RC
DwarfScanner::createNewType(Dwarf_Die die, Dwarf_Half tag, const char *dieName,
                            Type **newType) {
  DDR_RC rc = DDR_RC_OK;
  size_t typeSize = 0;
...
DwarfVisitor::visitTypedef(TypedefUDT *newTypedef) const {
  ...
  if (NULL == newTypedef->_aliasedType) {
    ...
    rc = _scanner->getTypeInfo(_die, &typeDie, &typedefName,
                               &newTypedef->_modifiers, &newTypedef->_sizeOf,
                               NULL);
    ...
    if (DDR_RC_OK == rc) {
      ...
      if (NULL != typedefType) {
        ...
        newTypedef->_sizeOf =
            newTypedef->_modifiers.getSize(typedefType->_sizeOf);
        /* If the typedef's type has no associated name, let it assume the
         * typedef's name. */
    ...
    if (DW_DLV_ERROR == dwRc) {
      ...
      do {
        ...
        if (DW_TAG_enumerator == childTag) {
          ...
          if (DDR_RC_OK !=
              _scanner->addEnumMember(childDie, _outerUDT, newUDT)) {
            rc = DDR_RC_ERROR;
            break;
          }
        }
 ...
 */
static bool isVtablePointer(const char *fieldName) {
#if defined(__GNUC__)
  ...
  if (DW_DLV_ERROR == dwRc) {
    ...
    do {
      ...
      if (DW_TAG_subprogram == childTag) {
        ...
        UDT *innerUDT = NULL;
        if (DDR_RC_OK !=
            addDieToIR(childDie, childTag, newClass, (Type **)&innerUDT)) {
          rc = DDR_RC_ERROR;
          break;
        } else if ((NULL != newClass) && (NULL != innerUDT) &&
                   (NULL == innerUDT->getNamespace())) {
          /* We only add it to list of subUDTs if innerUDT's _outerNamespace is
           * NULL, because there should only be one outer UDT per inner UDT. */
          /* Check that the subUDT wasn't already added when the type was found
           * as a stub type */
          if (newClass->_subUDTs.end() == std::find(newClass->_subUDTs.begin(),
                                                    newClass->_subUDTs.end(),
                                                    innerUDT)) {
        ...
        if (!fieldName.empty() && !isVtablePointer(fieldName.c_str())) {
          ...
          if (DDR_RC_OK !=
              addClassField(childDie, (ClassType *)newClass, fieldName)) {
            rc = DDR_RC_ERROR;
            break;
          }
        }
...
DwarfVisitor::visitClass(ClassUDT *newType) const {
  ...
  if ((NULL == newType->_superClass) && newType->_fieldMembers.empty() &&
      newType->_enumMembers.empty() && newType->_subUDTs.empty()) {
...
DwarfVisitor::visitUnion(UnionUDT *newType) const {
  DDR_RC rc = DDR_RC_OK;
  /* scan this type if we haven't seen any structure */
  if (newType->_fieldMembers.empty() && newType->_enumMembers.empty() &&
...
DDR_RC
DwarfScanner::addEnumMember(Dwarf_Die die, NamespaceUDT *outerUDT,
                            EnumUDT *newEnum) {
  Dwarf_Attribute attr = NULL;
  string enumName = "";
  int enumValue = 0;
  ...
  /* literals of a nested anonymous enum are collected in the enclosing
   * namespace */
  ...
  for (vector<EnumMember *>::iterator m = members->begin(); m != members->end();
       ++m) {
    if ((*m)->_name == enumName) {
...
static bool getUdata(Dwarf_Small **value, Dwarf_Unsigned *len,
...
static bool evaluateOffset(Dwarf_Ptr expr, Dwarf_Unsigned len,
...
DDR_RC
DwarfScanner::addClassField(Dwarf_Die die, ClassType *newClass,
   ...
   * get the field type.
   */
  if (DDR_RC_OK == getTypeInfo(die, &baseDie, &typeName, &fieldModifiers,
                               &typeSize, &bitField)) {
    /* Get the field UDT. */
    Dwarf_Half tag = 0;
    if (NULL == baseDie) {
      ERRMSG("Missing field type for field: %s. Possible missing or corrupt "
             "dwarf info.\n",
             fieldName.c_str());
      typeName = "void";
     ...
     */
    {
      ...
      if (DW_DLV_ERROR ==
          dwarf_hasattr(die, DW_AT_external, &hasAttr, &error)) {
      ...
      if (!newField->_isStatic) {
        ...
        if (DW_DLV_ERROR ==
            dwarf_hasattr(die, DW_AT_declaration, &hasAttr, &error)) {
        ...
        if (hasAttr) {
          Dwarf_Attribute attr = NULL;
          if (DW_DLV_ERROR ==
              dwarf_attr(die, DW_AT_declaration, &attr, &error)) {
    ...
    if (!newField->_isStatic) {
      ...
      if (DW_DLV_ERROR == dwarf_hasattr(die, DW_AT_data_member_location,
                                        &hasAttrBytes, &error)) {
      ...
      if (DW_DLV_ERROR ==
          dwarf_hasattr(die, DW_AT_data_bit_offset, &hasAttrBits, &error)) {
      ...
      if (hasAttrBytes || hasAttrBits) {
        ...
        Dwarf_Half attrType =
            hasAttrBytes ? DW_AT_data_member_location : DW_AT_data_bit_offset;
    ...
    if ((typeName == newClass->_name) && (!newClass->_name.empty())) {
      ...
      if ((DDR_RC_OK !=
           addDieToIR(baseDie, tag, NULL, &newField->_fieldType)) ||
...
DwarfScanner::getSuperUDT(Dwarf_Die die, ClassUDT *udt) {
  ...
  if (DDR_RC_OK == getTypeTag(die, &superTypeDie, &tag)) {
    ...
    /* Get the super udt. */
    if ((DDR_RC_OK ==
         addDieToIR(superTypeDie, tag, NULL, (Type **)&superUDT)) &&
        (NULL != superUDT)) {
      rc = DDR_RC_OK;
...
DwarfScanner::traverse_cu_in_debug_section(Symbol_IR *ir) {
  ...
  while (DDR_RC_OK == rc) {
    ...
    int ret = dwarf_next_cu_header(_debug, &cuHeaderLength, &versionStamp,
                                   &abbrevOffset, &addressSize, &nextCUheader,
                                   &error);
    if (DW_DLV_ERROR == ret) {
    ...
    do {
      ...
      if ((tag == DW_TAG_structure_type) || (tag == DW_TAG_class_type) ||
          (tag == DW_TAG_enumeration_type) || (tag == DW_TAG_union_type) ||
          (tag == DW_TAG_namespace) || (tag == DW_TAG_typedef)) {
...
DDR_RC
DwarfScanner::startScan(OMRPortLibrary *portLibrary, Symbol_IR *ir,
  ...
  if (DDR_RC_OK == rc) {
    ...
    for (vector<string>::iterator it = debugFiles->begin();
         it != debugFiles->end(); ++it) {
      Symbol_IR newIR(ir);
...
DDR_RC
DwarfScanner::scanFile(OMRPortLibrary *portLibrary, Symbol_IR *ir,
                       const char *filepath) {
