...
static bool stringMatches(const char *candidate, const char *pattern) {
  for (;;) {
    if ('*' == *pattern) {
...
static bool stringMatchesAny(const string &candidate,
                             const set<string> &patterns) {
  const char *c_candidate = candidate.c_str();

  for (set<string>::const_iterator it = patterns.begin(); it != patterns.end();
       ++it) {
