...
sysThread(void* arg)
{
  ...
  do {
    omrthread_monitor_wait_interruptable(
      thrInfo->synchronization, MILLI_TIMEOUT, NANO_TIMEOUT);
...
TEST(ThreadExtendedTest, DISABLED_TestThreadCpuTime)
{
  ...
  omrthread_monitor_init_with_name(
    &thrInfo->synchronization, 0, "threadCpuTimeInfo monitor");

  ret = omrthread_attr_init(&attr);
  ASSERT_TRUE(ret == 0);
  ret =
    omrthread_attr_set_category(&attr, J9THREAD_CATEGORY_APPLICATION_THREAD);
  ASSERT_TRUE(ret == 0);
  ...
  for (i = 0; i < NUM_APP_THREAD; i++) {
    ret = omrthread_create_ex(
      &(tid[i]), &attr, 0, (omrthread_entrypoint_t)appThread, (void*)&tid[i]);
    ASSERT_TRUE(ret == 0);
  }
