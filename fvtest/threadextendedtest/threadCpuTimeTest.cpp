 ...
 */
static int32_t J9THREAD_PROC runTest(void* arg) {
  uintptr_t i = 0;
  ...
  /* Check that the value returned by omrthread_get_cpu_time has increased after
   * work done */
   ...
   * between the two APIs, esp because of the presence of a 100 threads all CPU
   * bound. Since the entire test runs for about 150s, we are checking the times
   * are within 3 mins of each other
   */
  if (tinfo->self_cpu_time > tinfo->cpu_time) {
    if ((tinfo->self_cpu_time - tinfo->cpu_time) >
        (int64_t)THREEMINS * NANOSEC) {
      tinfo->error_code = ERROR_CPU_TIME_MISMATCH;
      return -1;
    }
  } else {
    if ((tinfo->cpu_time - tinfo->self_cpu_time) >
        (int64_t)THREEMINS * NANOSEC) {
      tinfo->error_code = ERROR_CPU_TIME_MISMATCH;
      return -1;
    }
...
TEST(ThreadExtendedTest, TestOtherThreadCputime) {
   ...
   * and intialize it */
  info = (SupportThreadInfo*)omrmem_allocate_memory(sizeof(SupportThreadInfo),
                                                    OMRMEM_CATEGORY_THREADS);
  ASSERT_TRUE(NULL != info);
  info->counter = 0;
  info->wait = 0;
  info->sync = 0;
  info->thread_info = (ThreadInfo*)omrmem_allocate_memory(
      sizeof(ThreadInfo) * THREAD_NUM, OMRMEM_CATEGORY_THREADS);
  ...
  for (i = 0; i < THREAD_NUM; i++) {
    ...
    tinfo->tcount = i;
    ret = omrthread_create_ex(&(tinfo->j9tid), J9THREAD_ATTR_DEFAULT, 0,
  ...
  for (i = 0; i < THREAD_NUM; i++) {
    ...
    /*
     * Check that the cp time that we got above is similar to the stored cpu
     * time by the thread itself
     */
