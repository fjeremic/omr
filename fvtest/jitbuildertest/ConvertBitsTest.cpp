...
DEFINE_BUILDER(BitConvertInt2Float, Float, PARAM("p", Int32)) {
  Return(ConvertBitsTo(Float, Load("p")));
  return true;
}

TEST_F(BitConversionTest, Int2Float) {
  BitConvertInt2FloatFunction *int2float;
  ASSERT_COMPILE(OMR::JitBuilder::TypeDictionary, BitConvertInt2Float,
                 int2float);
...
DEFINE_BUILDER(BitConvertFloat2Int, Int32, PARAM("p", Float)) {
  Return(ConvertBitsTo(Int32, Load("p")));
  return true;
}

TEST_F(BitConversionTest, Float2Int) {
  BitConvertFloat2IntFunction *float2int;
  ASSERT_COMPILE(OMR::JitBuilder::TypeDictionary, BitConvertFloat2Int,
                 float2int);
...
DEFINE_BUILDER(BitConvertLong2Double, Double, PARAM("p", Int64)) {
  Return(ConvertBitsTo(Double, Load("p")));
  return true;
}

TEST_F(BitConversionTest, Long2Double) {
  BitConvertLong2DoubleFunction *long2double;
  ASSERT_COMPILE(OMR::JitBuilder::TypeDictionary, BitConvertLong2Double,
                 long2double);
...
DEFINE_BUILDER(BitConvertDouble2Long, Int64, PARAM("p", Double)) {
  Return(ConvertBitsTo(Int64, Load("p")));
  return true;
}

TEST_F(BitConversionTest, Double2Long) {
  BitConvertDouble2LongFunction *double2long;
  ASSERT_COMPILE(OMR::JitBuilder::TypeDictionary, BitConvertDouble2Long,
                 double2long);
