 ...
 * from `OMR::JitBuilder::MethodBuilder` and that the constructor and
 * `buildIL()` method are declared. A ';' is required at the end of the macro
 * invocation.
 *
 * Example use:
 *
 ...
 *
 * The argument is the name of the builder class whose constructor is being
 * defined. The body should contain the actual definition of the constructor.
 *
 ...
 *       }
 */
#define DEFINE_BUILDIL(name) inline bool name::buildIL()
 ...
 * class.
 *
 * The second argument is an instance of `OMR::JitBuilder::IlValue *` that
 * represents the return type of the method.
 *
 * The following variadic argument are pairs with a `const char *` component
 * and a `OMR::JitBuilder::IlValue *` component. The first component is the name
 * of parameter and the second is a `OMR::JitBuilder::IlValue` instance
 * representing the parameter's type. The `PARAM` macro should be used when
 * defining these pairs because simple brace initializers cannot be used when
 * invoking a macro.
 *
 ...
 * definition, the services provided by `OMR::JitBuilder::MethodBuilder` are
 * available for use (e.g. `typeDictionary()` can be used when defining types).
 *
 * For convenience, the following methods from `OMR::JitBuilder::TypeDictionary`
 ...
 * Example use:
 *
 *    DEFINE_BUILDER( MyMethod,                                            //
 * name of the builder/method toIlType<int *>(), // return type of the method
 *                    PARAM("arg1", PointerTo(LookupStruct("MyStruct"))),  //
 * first parameter of the method PARAM("arg2", Double) ) // second parameter of
 * the method
 *       {
...
#define DEFINE_BUILDER(name, returnType, ...)                                  \
  ...
  {                                                                            \
    ...
    {                                                                          \
      ...
      std::pair<const char*, OMR::JitBuilder::IlType*> argsArray[] = {         \
        std::pair<const char*, OMR::JitBuilder::IlType*>("", NULL),            \
        __VA_ARGS__                                                            \
      };                                                                       \
      std::vector<std::pair<const char*, OMR::JitBuilder::IlType*>> args(      \
        argsArray,                                                             \
        argsArray +                                                            \
          sizeof(argsArray) /                                                  \
            sizeof(std::pair<const char*, OMR::JitBuilder::IlType*>));         \
    ...
    bool buildIL();                                                            \
    template<typename T>                                                       \
    OMR::JitBuilder::IlType* toIlType()                                        \
    {                                                                          \
      return typeDictionary()->toIlType<T>();                                  \
    }                                                                          \
    OMR::JitBuilder::IlType* LookupStruct(const char* name)                    \
    {                                                                          \
      return typeDictionary()->LookupStruct(name);                             \
    }                                                                          \
    OMR::JitBuilder::IlType* LookupUnion(const char* name)                     \
    {                                                                          \
      return typeDictionary()->LookupUnion(name);                              \
    }                                                                          \
    OMR::JitBuilder::IlType* PointerTo(OMR::JitBuilder::IlType* type)          \
    {                                                                          \
      return typeDictionary()->PointerTo(type);                                \
    }                                                                          \
    OMR::JitBuilder::IlType* PointerTo(const char* structName)                 \
    {                                                                          \
      return typeDictionary()->PointerTo(structName);                          \
    }                                                                          \
    OMR::JitBuilder::IlType* GetFieldType(const char* structName,              \
                                          const char* fieldName)               \
    {                                                                          \
      return typeDictionary()->GetFieldType(structName, fieldName);            \
    }                                                                          \
    OMR::JitBuilder::IlType* UnionFieldType(const char* unionName,             \
                                            const char* fieldName)             \
    {                                                                          \
      return typeDictionary()->UnionFieldType(unionName, fieldName);           \
    }                                                                          \
  };                                                                           \
...
#define PARAM(name, type)                                                      \
  std::pair<const char*, OMR::JitBuilder::IlType*>(name, type)

/**
 * @brief The JitBuilderTest class is a basic test fixture for JitBuilder test
 * cases.
 *
 ...
 *    MyFunctionType myFunction;             // variable to store the entry
 * point
 *
 ...
 *
 * Note: invoking ASSERT_COMPILE is preferred to directly calling
 * `try_compile()`.
 *
