...
DEFINE_BUILDER(SubIfThenElseBothPathBuilder, Int64, PARAM("valueA", Int64),
               PARAM("valueB", Int64)) {
  ...
  thenPath->Return(
      thenPath->Sub(thenPath->Load("valueA"), thenPath->Load("valueB")));
  elsePath->Return(
      elsePath->Sub(elsePath->Load("valueB"), elsePath->Load("valueA")));
  return true;
}

// one path exist and will be taken, still need hardcoded Return
DEFINE_BUILDER(SubIfThenNullBuilder, Int64, PARAM("valueA", Int64),
               PARAM("valueB", Int64)) {
  OMR::JitBuilder::IlBuilder *thenPath = NULL;
...
DEFINE_BUILDER(SubIfNullElseBuilder, Int64, PARAM("valueA", Int64),
               PARAM("valueB", Int64)) {
  OMR::JitBuilder::IlBuilder *elsePath = NULL;
...
TEST_F(IfThenElseTest, TrueBothPath) {
  ...
  ASSERT_COMPILE(OMR::JitBuilder::TypeDictionary, SubIfThenElseBothPathBuilder,
                 typeIfFunction);
...
TEST_F(IfThenElseTest, TrueElsePath) {
  IfLongFunctionType typeIfFunction;
  ASSERT_COMPILE(OMR::JitBuilder::TypeDictionary, SubIfThenNullBuilder,
                 typeIfFunction);
  ASSERT_EQ(typeIfFunction(10, 5), 5);
}
...
TEST_F(IfThenElseTest, FalseBothPath) {
  IfLongFunctionType typeIfFunction;
  ASSERT_COMPILE(OMR::JitBuilder::TypeDictionary, SubIfNullElseBuilder,
                 typeIfFunction);
  ASSERT_EQ(typeIfFunction(10, 5), 5);
}
