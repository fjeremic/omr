...
DEFINE_BUILDER(TestStructFieldNameBuilder, NoType)
{
  EXPECT_EQ(toIlType<uint8_t>(), GetFieldType("MyStruct", "field"))
    << "Expected " << toIlType<uint8_t>()->getName() << " and "
    << GetFieldType("MyStruct", "field")->getName() << " was returned";
  EXPECT_EQ(toIlType<uint16_t>(), GetFieldType("MyStruct", "field11"))
    << "Expected " << toIlType<uint16_t>()->getName() << " and "
    << GetFieldType("MyStruct", "field11")->getName() << " was returned";
  EXPECT_EQ(toIlType<uint32_t>(), GetFieldType("MyStruct", "field1"))
    << "Expected " << toIlType<uint32_t>()->getName() << " and "
    << GetFieldType("MyStruct", "field1")->getName() << " was returned";
  EXPECT_EQ(toIlType<uint64_t>(), GetFieldType("MyStruct", "f"))
    << "Expected " << toIlType<uint64_t>()->getName() << " and "
    << GetFieldType("MyStruct", "f")->getName() << " was returned";
...
DEFINE_BUILDER(TestUnionFieldNameBuilder, NoType)
{
  EXPECT_EQ(toIlType<uint8_t>(), UnionFieldType("MyUnion", "field"))
    << "Expected " << toIlType<uint8_t>()->getName() << " and "
    << UnionFieldType("MyUnion", "field")->getName() << " was returned";
  EXPECT_EQ(toIlType<uint16_t>(), UnionFieldType("MyUnion", "field11"))
    << "Expected " << toIlType<uint16_t>()->getName() << " and "
    << UnionFieldType("MyUnion", "field11")->getName() << " was returned";
  EXPECT_EQ(toIlType<uint32_t>(), UnionFieldType("MyUnion", "field1"))
    << "Expected " << toIlType<uint32_t>()->getName() << " and "
    << UnionFieldType("MyUnion", "field1")->getName() << " was returned";
  EXPECT_EQ(toIlType<uint64_t>(), UnionFieldType("MyUnion", "f"))
    << "Expected " << toIlType<uint64_t>()->getName() << " and "
    << UnionFieldType("MyUnion", "f")->getName() << " was returned";
...
TEST_F(FieldNameTest, StructField)
{
  ...
  ASSERT_COMPILE(
    MyStructTypeDictionary, TestStructFieldNameBuilder, testStructNameAddress);
...
TEST_F(FieldNameTest, UnionField)
{
  ...
  ASSERT_COMPILE(
    MyUnionTypeDictionary, TestUnionFieldNameBuilder, testUnionNameAddress);
