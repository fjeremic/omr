...
#define EXPECT_BCIEQ(expected_index, actual_index)                             \
  do {                                                                         \
    EXPECT_EQ((expected_index), (actual_index))                                \
        << "Unexpected bytecode index (implies bytecode was either missed or " \
           "erroneously added in worklist).";                                  \
  } while (0)
...
int32_t computeExpectedResult(int32_t p) {
  // BC0
  int32_t r = p;
BC1:
  r++;
...
DEFINE_BUILDER(WorklistTestMethod, Int32, PARAM("path", Int32)) {
...
TEST_F(WorklistTest, test) {
  ...
  ASSERT_COMPILE(OMR::JitBuilder::TypeDictionary, WorklistTestMethod,
                 worklistTestMethod);
