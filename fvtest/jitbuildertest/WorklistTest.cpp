...
#define EXPECT_BCIEQ(expected_index, actual_index)                                                           \
  do {                                                                                                       \
    EXPECT_EQ((expected_index), (actual_index))                                                              \
      << "Unexpected bytecode index (implies bytecode was either missed or erroneously added in worklist)."; \
  } while (0)
...
TEST_F(WorklistTest, test)
{
  ...
  for (int32_t p = 0; p < 20; p++) {
    ...
    ASSERT_EQ(computeExpectedResult(p), worklistTestResult) << "Path " << p << " generated incorrect result.";
  }
}
