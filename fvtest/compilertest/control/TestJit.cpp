...
static void initializeAllHelpers(TestCompiler::JitConfig *jitConfig,
...
static void initializeCodeCache(TR::CodeCacheManager &codeCacheManager) {
  ...
  // TestCompiler::CodeCacheManager::initialize() because it needs
  // trampolineCodeSize
...
// An individual test program is intended to load the JIT as a shared library,
// then call:
//     initializeTestJit() providing addresses of required helpers (all others
//     will be NULL) compileMethod() as many times as needed to create compiled
//     code
...
// "helpers" helpers is an array of pointers to helpers that compiled code for
// tests needs to reference options is any JIT option string passed in to
// globally influence compilation
extern "C" bool initializeTestJit(TR_RuntimeHelper *helperIDs,
                                  void **helperAddresses, int32_t numHelpers,
                                  char *options) {
...
extern "C" uint8_t *compileMethod(TR::IlGeneratorMethodDetails &details,
                                  TR_Hotness hotness, int32_t &rc) {
  return compileMethodFromDetails(NULL, details, hotness, rc);
}

extern "C" int32_t compileMethodBuilder(TR::MethodBuilder *m, uint8_t **entry) {
