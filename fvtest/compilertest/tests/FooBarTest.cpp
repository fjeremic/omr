...
void FooBarTest::compileTestMethods() {
  ...
  TR::ResolvedMethod barCompilee(__FILE__, LINETOSTR(__LINE__), "bar",
                                 numberOfArguments, argTypes, Int32, 0,
                                 &barIlInjector);
  ...
  TR::ResolvedMethod fooCompilee(__FILE__, LINETOSTR(__LINE__), "foo",
                                 numberOfArguments, argTypes, Int32, 0,
                                 &fooIlInjector);
...
int32_t FooBarTest::foo(int32_t index) {
  int32_t newIndex = _bar(index);
  if (newIndex < 0 || newIndex >= _dataArraySize) return -1;
  return _dataArray[newIndex];
}

int32_t FooBarTest::bar(int32_t index) {
  if (index < 0 || index >= _dataArraySize) return -1;
...
void FooBarTest::invokeTests() {
  ...
  for (i = 0; i < _dataArraySize; i++) _dataArray[i] = _dataArraySize - i;
  ...
  for (int32_t i = 0; i < _dataArraySize; i++) _dataArray[i] = 1;
  for (i = 0; i < _dataArraySize; i++, testID++) {
...
// "compiler/codegen/FrontEnd.cpp" TR_FrontEnd::methodTrampolineLookup is
// unimplemented and "test/env/FrontEnd.cpp"
// TestCompiler::FrontEnd::methodTrampolineLookup is "methodTrampolineLookup not
// implemented yet". This test also failed intermittent (segfault) on PPCLE,
// temporarily disabled this test on PPCLE, under track of Work Item Please
// remove this #ifdef after those functions are implemented.
#if defined(TR_TARGET_X86) || \
    defined(TR_TARGET_S390) && !defined(TR_TARGET_64BIT) && !defined(J9ZOS390)
