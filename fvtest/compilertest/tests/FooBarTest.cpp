...
void FooBarTest::compileTestMethods()
{
  ...
  TR::ResolvedMethod barCompilee(
    __FILE__, LINETOSTR(__LINE__), "bar", numberOfArguments, argTypes, Int32, 0, &barIlInjector);
  ...
  TR::ResolvedMethod fooCompilee(
    __FILE__, LINETOSTR(__LINE__), "foo", numberOfArguments, argTypes, Int32, 0, &fooIlInjector);
...
// "test/env/FrontEnd.cpp"  TestCompiler::FrontEnd::methodTrampolineLookup is "methodTrampolineLookup not implemented
// yet". This test also failed intermittent (segfault) on PPCLE, temporarily disabled this test on PPCLE, under track of
// Work Item Please remove this #ifdef after those functions are implemented.
