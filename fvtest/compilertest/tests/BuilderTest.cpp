...
int32_t
BuilderTest::basicForLoop(int32_t initial,
                          int32_t final,
                          int32_t bump,
                          bool countsUp)
{
...
int32_t
BuilderTest::forLoopIfThenElse(int32_t initial,
                               int32_t final,
                               int32_t bump,
...
int32_t
BuilderTest::whileDoIfThenElse(int32_t initial,
                               int32_t final,
                               int32_t bump,
                               int32_t compareValue)
{
...
int32_t
BuilderTest::doWhileIfThenElse(int32_t initial,
                               int32_t final,
                               int32_t bump,
                               int32_t compareValue)
{
...
BuilderTest::invokeTests()
{
  for (uint32_t i = 0; i <= 20; i++) {
    OMR_CT_EXPECT_EQ(
      _iterativeFibMethod, iterativeFib(i), _iterativeFibMethod(i));
#if (defined(TR_TARGET_X86) && defined(TR_TARGET_64BIT)) ||                    \
  defined(TR_TARGET_S390)
    OMR_CT_EXPECT_EQ(
      _recursiveFibMethod, recursiveFib(i), _recursiveFibMethod(i));
#endif
  }
}
...
BuilderTest::invokeControlFlowTests()
{
  ...
  for (uint32_t i = 0;
       i < sizeof(basicForLoopParms) / sizeof(basicForLoopParms[0]);
       ++i) {
    OMR_CT_EXPECT_EQ(_basicForLoopUpMethod,
                     basicForLoop(basicForLoopParms[i][0],
                                  ...
                                  true),
                     _basicForLoopUpMethod(basicForLoopParms[i][0],
                                           basicForLoopParms[i][1],
                                           basicForLoopParms[i][2]));
  ...
  for (uint32_t i = 0; i < sizeof(maxIfThenParms) / sizeof(maxIfThenParms[0]);
    ...
    OMR_CT_EXPECT_EQ(
      _maxIfThenMethod,
      maxIfThen(maxIfThenParms[i][0], maxIfThenParms[i][1]),
  ...
  for (uint32_t i = 0;
       i < sizeof(subIfFalseThenParms) / sizeof(subIfFalseThenParms[0]);
       ++i) {
    OMR_CT_EXPECT_EQ(
  ...
  for (uint32_t i = 0; i < sizeof(loopN) / sizeof(loopN[0]); ++i) {
    OMR_CT_EXPECT_EQ(
      _doWhileFibMethod, doWhileFib(loopN[i]), _doWhileFibMethod(loopN[i]));
    OMR_CT_EXPECT_EQ(
      _whileDoFibMethod, whileDoFib(loopN[i]), _whileDoFibMethod(loopN[i]));
    OMR_CT_EXPECT_EQ(_forLoopContinueMethod,
                     forLoopContinue(loopN[i]),
                     _forLoopContinueMethod(loopN[i]));
    OMR_CT_EXPECT_EQ(_forLoopBreakMethod,
                     forLoopBreak(loopN[i]),
    ...
    OMR_CT_EXPECT_EQ(_doWhileWithBreakMethod,
                     doWhileWithBreak(loopN[i]),
                     _doWhileWithBreakMethod(loopN[i]));
    OMR_CT_EXPECT_EQ(_doWhileWithContinueMethod,
                     doWhileWithContinue(loopN[i]),
    ...
    OMR_CT_EXPECT_EQ(_whileDoWithBreakMethod,
                     whileDoWithBreak(loopN[i]),
                     _whileDoWithBreakMethod(loopN[i]));
    OMR_CT_EXPECT_EQ(_whileDoWithContinueMethod,
                     whileDoWithContinue(loopN[i]),
...
BuilderTest::invokeNestedControlFlowLoopTests()
{
  ...
  for (uint32_t i = 0; i < arrSize; ++i) {
    OMR_CT_EXPECT_EQ(_forLoopUpIfThenElseMethod,
                     forLoopIfThenElse(loopIfThenElseArr[i][0],
                                       loopIfThenElseArr[i][1],
                                       loopIfThenElseArr[i][2],
                                       loopIfThenElseArr[i][3]),
                     _forLoopUpIfThenElseMethod(loopIfThenElseArr[i][0],
                                                loopIfThenElseArr[i][1],
                                                loopIfThenElseArr[i][2],
                                                loopIfThenElseArr[i][3]));
  }
  ...
  for (uint32_t i = 0; i < arrSize; ++i) {
    OMR_CT_EXPECT_EQ(_whileDoIfThenElseMethod,
                     whileDoIfThenElse(loopIfThenElseArr[i][0],
                                       loopIfThenElseArr[i][1],
                                       loopIfThenElseArr[i][2],
                                       loopIfThenElseArr[i][3]),
                     _whileDoIfThenElseMethod(loopIfThenElseArr[i][0],
                                              loopIfThenElseArr[i][1],
                                              loopIfThenElseArr[i][2],
                                              loopIfThenElseArr[i][3]));
  }
  ...
  for (uint32_t i = 0; i < arrSize; ++i) {
    OMR_CT_EXPECT_EQ(_doWhileIfThenElseMethod,
                     doWhileIfThenElse(loopIfThenElseArr[i][0],
                                       loopIfThenElseArr[i][1],
                                       loopIfThenElseArr[i][2],
                                       loopIfThenElseArr[i][3]),
                     _doWhileIfThenElseMethod(loopIfThenElseArr[i][0],
                                              loopIfThenElseArr[i][1],
                                              loopIfThenElseArr[i][2],
                                              loopIfThenElseArr[i][3]));
  }
...
DoWhileWithBreakMethod::buildIL()
{
  ...
  doWhileBody->Store(
    "a", doWhileBody->Add(doWhileBody->Load("a"), doWhileBody->ConstInt32(1)));
  ...
  doWhileBody->Store(
    "exitLoop",
    doWhileBody->LessThan(doWhileBody->Load("a"), doWhileBody->Load("N")));
...
DoWhileWithBreakAndContinueMethod::buildIL()
{
  ...
  continueThenPath->Store(
    "a",
    continueThenPath->Sub(continueThenPath->Load("a"),
...
WhileDoWithBreakMethod::buildIL()
{
  ...
  whileDoBody->Store(
    "a", whileDoBody->Add(whileDoBody->Load("a"), whileDoBody->ConstInt32(1)));
  ...
  whileDoBody->Store(
    "exitLoop",
    whileDoBody->LessThan(whileDoBody->Load("a"), whileDoBody->Load("N")));
...
AbsDiffIfThenElseMethod::buildIL()
{
  ...
  thenPath->Store(
    "diff", thenPath->Sub(thenPath->Load("valueA"), thenPath->Load("valueB")));

  elsePath->Store(
    "diff", elsePath->Sub(elsePath->Load("valueB"), elsePath->Load("valueA")));
...
IfThenElseLoopMethod::buildIL()
{
  ...
  thenPath->IfThen(
    &thenReturnN,
    thenPath->LessThan(thenPath->Load("leftV"), thenPath->ConstInt32(2)));
  ...
  thenPath->Store(
    "exitCondition",
    thenPath->LessThan(thenPath->Load("i"), thenPath->Load("leftV")));

  thenPath->WhileDoLoop("exitCondition", &whileDoBody);

  whileDoBody->Store(
    "tempSum",
    whileDoBody->Add(whileDoBody->Load("Sum"), whileDoBody->Load("LastSum")));
  whileDoBody->Store("LastSum", whileDoBody->Load("Sum"));
  whileDoBody->Store("Sum", whileDoBody->Load("tempSum"));
  whileDoBody->Store(
    "i", whileDoBody->Add(whileDoBody->Load("i"), whileDoBody->ConstInt32(1)));
  whileDoBody->Store(
    "exitCondition",
    whileDoBody->LessThan(whileDoBody->Load("i"), whileDoBody->Load("leftV")));
  ...
  elsePath->IfThen(
    &elseReturnN,
    elsePath->LessThan(elsePath->Load("rightV"), elsePath->ConstInt32(2)));

  elseReturnN->Return(elseReturnN->Load("rightV"));

  elsePath->ForLoopUp("I",
                      &forBody,
...
ForLoopUPIfThenElseMethod::buildIL()
{
  ...
  forLoopBody->Store(
    "Sum",
    forLoopBody->Add(forLoopBody->Load("bump"), forLoopBody->Load("Sum")));
...
WhileDoIfThenElseMethod::buildIL()
{
  ...
  whileDoBody->Store(
    "sum",
    whileDoBody->Add(whileDoBody->Load("bump"), whileDoBody->Load("sum")));
...
DoWhileIfThenElseMethod::buildIL()
{
  ...
  doWhileBody->Store(
    "sum",
    doWhileBody->Add(doWhileBody->Load("sum"), doWhileBody->Load("bump")));
  doWhileBody->Store(
    "i", doWhileBody->Add(doWhileBody->Load("i"), doWhileBody->Load("bump")));
  doWhileBody->Store(
    "exitCondition",
    doWhileBody->LessThan(doWhileBody->Load("i"), doWhileBody->Load("final")));
...
ForLoopBreakMethod::buildIL()
{
  ...
  TR::IlBuilder* thenPath = NULL;
  ForLoopWithBreak(
    true, "I", &forBody, &breakBody, ConstInt32(0), Load("n"), ConstInt32(1));
...
ForLoopBreakAndContinueMethod::buildIL()
{
  ...
  breakThenPath->Store(
    "Sum",
    breakThenPath->Add(breakThenPath->Load("Sum"), breakThenPath->Load("I")));
