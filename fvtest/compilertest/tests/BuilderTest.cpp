...
int32_t BuilderTest::basicForLoop(int32_t initial, int32_t final, int32_t bump, bool countsUp)
{
  int32_t sum = 0;
...
int32_t BuilderTest::forLoopIfThenElse(
  int32_t initial, int32_t final, int32_t bump, int32_t compareValue, bool countsUp)
{
  if (countsUp) {
    int32_t sum = 0;
...
int32_t BuilderTest::whileDoIfThenElse(int32_t initial, int32_t final, int32_t bump, int32_t compareValue)
{
  int32_t sum = 0;
  int32_t i = initial;
  while (i < final) {
...
int32_t BuilderTest::doWhileIfThenElse(int32_t initial, int32_t final, int32_t bump, int32_t compareValue)
{
  int32_t sum = 0;
  int32_t i = initial;
  do {
...
{
  int32_t basicForLoopParms[][3] = { -9, 1000, 1, 9, 1000, -1, 0, 10000, 11, 10000, 0, 20 };

  for (uint32_t i = 0; i < sizeof(basicForLoopParms) / sizeof(basicForLoopParms[0]); ++i) {
    OMR_CT_EXPECT_EQ(_basicForLoopUpMethod,
      basicForLoop(basicForLoopParms[i][0], basicForLoopParms[i][1], basicForLoopParms[i][2], true),
    ...
    OMR_CT_EXPECT_EQ(_basicForLoopDownMethod,
      basicForLoop(basicForLoopParms[i][0], basicForLoopParms[i][1], basicForLoopParms[i][2], false),
  ...
  for (uint32_t i = 0; i < sizeof(nestedLoopParms) / sizeof(nestedLoopParms[0]); ++i) {
    OMR_CT_EXPECT_EQ(
      _shootoutNestedLoopMethod, shootoutNestedLoop(nestedLoopParms[i]), _shootoutNestedLoopMethod(nestedLoopParms[i]));
  }

  int32_t absDiffParms[][2] = { (int32_t)TR::getMaxSigned<TR::Int32>(), (int32_t)TR::getMaxSigned<TR::Int32>() - 1,
  ...
  for (uint32_t i = 0; i < sizeof(absDiffParms) / sizeof(absDiffParms[0]); ++i) {
    OMR_CT_EXPECT_EQ(_absDiffIfThenElseMethod, absDiff(absDiffParms[i][0], absDiffParms[i][1]),
      _absDiffIfThenElseMethod(absDiffParms[i][0], absDiffParms[i][1]));
  }

  int32_t maxIfThenParms[][2] = { (int32_t)TR::getMaxSigned<TR::Int32>(), (int32_t)TR::getMinSigned<TR::Int32>(),
    (int32_t)TR::getMinSigned<TR::Int32>(), (int32_t)TR::getMaxSigned<TR::Int32>(),
    (int32_t)TR::getMinSigned<TR::Int32>() + 1, (int32_t)TR::getMinSigned<TR::Int32>(), 9, 9 };

  for (uint32_t i = 0; i < sizeof(maxIfThenParms) / sizeof(maxIfThenParms[0]); ++i) {
    OMR_CT_EXPECT_EQ(_maxIfThenMethod, maxIfThen(maxIfThenParms[i][0], maxIfThenParms[i][1]),
      _maxIfThenMethod(maxIfThenParms[i][0], maxIfThenParms[i][1]));
  }

  const int64_t LONG_NEG = -9;
  const int64_t LONG_POS = 9;
  int64_t subIfFalseThenParms[][2] = { TR::getMaxSigned<TR::Int64>(), LONG_POS, TR::getMinSigned<TR::Int64>(), LONG_NEG,
    LONG_POS, TR::getMaxSigned<TR::Int64>() };

  for (uint32_t i = 0; i < sizeof(subIfFalseThenParms) / sizeof(subIfFalseThenParms[0]); ++i) {
    OMR_CT_EXPECT_EQ(_subIfFalseThenMethod, subIfFalseThen(subIfFalseThenParms[i][0], subIfFalseThenParms[i][1]),
      _subIfFalseThenMethod(subIfFalseThenParms[i][0], subIfFalseThenParms[i][1]));
  }
  ...
  for (uint32_t i = 0; i < sizeof(loopN) / sizeof(loopN[0]); ++i) {
    ...
    OMR_CT_EXPECT_EQ(
      _forLoopBreakAndContinueMethod, forLoopBreakAndContinue(loopN[i]), _forLoopBreakAndContinueMethod(loopN[i]));
    ...
    OMR_CT_EXPECT_EQ(_doWhileWithBreakAndContinueMethod, doWhileWithBreakAndContinue(loopN[i]),
    ...
    OMR_CT_EXPECT_EQ(_whileDoWithContinueMethod, whileDoWithContinue(loopN[i]), _whileDoWithContinueMethod(loopN[i]))
      << i;
    OMR_CT_EXPECT_EQ(_whileDoWithBreakAndContinueMethod, whileDoWithBreakAndContinue(loopN[i]),
...
void BuilderTest::invokeNestedControlFlowLoopTests()
{
  int32_t ifThenElseLoopArr[][2]
    = { 1, 1, 1, 0, 0, 1, 0, 20, 1, 10, 2, 9, 3, 8, 4, 7, 5, 6, 6, 5, 7, 4, 8, 3, 9, 2, 10, 1, 20, 0 };

  int32_t loopIfThenElseArr[][4]
    = { -9, 1000, 1, 500, 9, 1000, 8, 500, 0, 10000, 11, 500, -9, 1000, 1, 500, -20, 1000, 8, 500, 18, 10000, 11, 500 };

  int32_t arrSize = 0;

  arrSize = sizeof(loopIfThenElseArr) / sizeof(loopIfThenElseArr[0]);
  for (uint32_t i = 0; i < arrSize; ++i) {
    OMR_CT_EXPECT_EQ(_forLoopUpIfThenElseMethod,
      forLoopIfThenElse(
        loopIfThenElseArr[i][0], loopIfThenElseArr[i][1], loopIfThenElseArr[i][2], loopIfThenElseArr[i][3]),
  ...
  arrSize = sizeof(loopIfThenElseArr) / sizeof(loopIfThenElseArr[0]);
  for (uint32_t i = 0; i < arrSize; ++i) {
    OMR_CT_EXPECT_EQ(_whileDoIfThenElseMethod,
      whileDoIfThenElse(
        loopIfThenElseArr[i][0], loopIfThenElseArr[i][1], loopIfThenElseArr[i][2], loopIfThenElseArr[i][3]),
  ...
  arrSize = sizeof(loopIfThenElseArr) / sizeof(loopIfThenElseArr[0]);
  for (uint32_t i = 0; i < arrSize; ++i) {
    OMR_CT_EXPECT_EQ(_doWhileIfThenElseMethod,
      doWhileIfThenElse(
        loopIfThenElseArr[i][0], loopIfThenElseArr[i][1], loopIfThenElseArr[i][2], loopIfThenElseArr[i][3]),
  ...
  for (int32_t i = 0; i < arrSize; i++) {
    OMR_CT_EXPECT_EQ(_ifThenElseLoopMethod, ifThenElseLoop(ifThenElseLoopArr[i][0], ifThenElseLoopArr[i][1]),
      _ifThenElseLoopMethod(ifThenElseLoopArr[i][0], ifThenElseLoopArr[i][1]));
  }
}
...
bool RecursiveFibonnaciMethod::buildIL()
{
  ...
  recursiveCase->Store("result",
    recursiveCase->Add(
      recursiveCase->Call("fib_recur", 1, recursiveCase->Sub(recursiveCase->Load("n"), recursiveCase->ConstInt32(1))),
      recursiveCase->Call("fib_recur", 1, recursiveCase->Sub(recursiveCase->Load("n"), recursiveCase->ConstInt32(2)))));
...
bool DoWhileWithBreakMethod::buildIL()
{
  ...
  doWhileBody->Store("a", doWhileBody->Add(doWhileBody->Load("a"), doWhileBody->ConstInt32(1)));

  TR::IlBuilder* thenPath = NULL;
  TR::IlValue* breakCondition = doWhileBody->GreaterThan(doWhileBody->Load("a"), doWhileBody->ConstInt32(10));
  ...
  doWhileBody->Store("a", doWhileBody->Add(doWhileBody->Load("a"), doWhileBody->ConstInt32(1)));
  doWhileBody->Store("exitLoop", doWhileBody->LessThan(doWhileBody->Load("a"), doWhileBody->Load("N")));
...
bool DoWhileWithContinueMethod::buildIL()
{
  ...
  TR::IlBuilder* thenPath = NULL;
  TR::IlValue* continueCondition = doWhileBody->EqualTo(doWhileBody->Load("a"), doWhileBody->ConstInt32(5));

  doWhileBody->IfThen(&thenPath, continueCondition);
  thenPath->Store("a", thenPath->Sub(thenPath->Load("a"), thenPath->ConstInt32(2)));
...
bool DoWhileWithBreakAndContinueMethod::buildIL()
{
  ...
  continueThenPath->Store("a", continueThenPath->Sub(continueThenPath->Load("a"), continueThenPath->ConstInt32(2)));
...
bool WhileDoWithBreakMethod::buildIL()
{
  ...
  whileDoBody->Store("a", whileDoBody->Add(whileDoBody->Load("a"), whileDoBody->ConstInt32(1)));

  TR::IlBuilder* thenPath = NULL;
  TR::IlValue* breakCondition = whileDoBody->GreaterThan(whileDoBody->Load("a"), whileDoBody->ConstInt32(10));
  ...
  whileDoBody->Store("a", whileDoBody->Add(whileDoBody->Load("a"), whileDoBody->ConstInt32(1)));
  whileDoBody->Store("exitLoop", whileDoBody->LessThan(whileDoBody->Load("a"), whileDoBody->Load("N")));
...
bool WhileDoWithContinueMethod::buildIL()
{
  ...
  TR::IlBuilder* thenPath = NULL;
  TR::IlValue* continueCondition = whileDoBody->EqualTo(whileDoBody->Load("a"), whileDoBody->ConstInt32(5));
...
bool ShootoutNestedLoopMethod::buildIL()
{
  ...
  body_a->ForLoopUp("b", &body_b, body_a->ConstInt32(0), body_a->Load("n"), body_a->ConstInt32(1));

  TR::IlBuilder* body_c = NULL;
  body_b->ForLoopUp("c", &body_c, body_b->ConstInt32(0), body_b->Load("n"), body_b->ConstInt32(1));

  TR::IlBuilder* body_d = NULL;
  body_c->ForLoopUp("d", &body_d, body_c->ConstInt32(0), body_c->Load("n"), body_c->ConstInt32(1));

  TR::IlBuilder* body_e = NULL;
  body_d->ForLoopUp("e", &body_e, body_d->ConstInt32(0), body_d->Load("n"), body_d->ConstInt32(1));

  TR::IlBuilder* body_f = NULL;
  body_e->ForLoopUp("f", &body_f, body_e->ConstInt32(0), body_e->Load("n"), body_e->ConstInt32(1));
...
bool AbsDiffIfThenElseMethod::buildIL()
{
  ...
  thenPath->Store("diff", thenPath->Sub(thenPath->Load("valueA"), thenPath->Load("valueB")));

  elsePath->Store("diff", elsePath->Sub(elsePath->Load("valueB"), elsePath->Load("valueA")));
...
bool IfThenElseLoopMethod::buildIL()
{
  ...
  // if
  thenPath->IfThen(&thenReturnN, thenPath->LessThan(thenPath->Load("leftV"), thenPath->ConstInt32(2)));

  thenReturnN->Return(thenReturnN->Load("leftV"));

  thenPath->Store("i", thenPath->ConstInt32(1));

  thenPath->Store("exitCondition", thenPath->LessThan(thenPath->Load("i"), thenPath->Load("leftV")));

  thenPath->WhileDoLoop("exitCondition", &whileDoBody);

  whileDoBody->Store("tempSum", whileDoBody->Add(whileDoBody->Load("Sum"), whileDoBody->Load("LastSum")));
  whileDoBody->Store("LastSum", whileDoBody->Load("Sum"));
  whileDoBody->Store("Sum", whileDoBody->Load("tempSum"));
  whileDoBody->Store("i", whileDoBody->Add(whileDoBody->Load("i"), whileDoBody->ConstInt32(1)));
  whileDoBody->Store("exitCondition", whileDoBody->LessThan(whileDoBody->Load("i"), whileDoBody->Load("leftV")));

  thenPath->Return(thenPath->Load("Sum"));

  // else
  elsePath->IfThen(&elseReturnN, elsePath->LessThan(elsePath->Load("rightV"), elsePath->ConstInt32(2)));

  elseReturnN->Return(elseReturnN->Load("rightV"));

  elsePath->ForLoopUp("I", &forBody, elsePath->ConstInt32(1), elsePath->Load("rightV"), elsePath->ConstInt32(1));

  forBody->Store("tempSum", forBody->Add(forBody->Load("Sum"), forBody->Load("LastSum")));
...
bool ForLoopUPIfThenElseMethod::buildIL()
{
  ...
  TR::IlValue* condition1 = forLoopBody->GreaterThan(forLoopBody->Load("I"), forLoopBody->Load("compareValue"));
  forLoopBody->IfThen(&ifThenBuilder, condition1);

  TR::IlValue* condition2 = ifThenBuilder->GreaterThan(ifThenBuilder->Load("I"), ifThenBuilder->Load("Sum"));
  ...
  forLoopBody->Store("Sum", forLoopBody->Add(forLoopBody->Load("bump"), forLoopBody->Load("Sum")));
...
bool WhileDoIfThenElseMethod::buildIL()
{
  ...
  TR::IlValue* condition1 = whileDoBody->GreaterThan(whileDoBody->Load("i"), whileDoBody->Load("compareValue"));
  ...
  whileDoBody->Store("sum", whileDoBody->Add(whileDoBody->Load("bump"), whileDoBody->Load("sum")));

  whileDoBody->Store("i", whileDoBody->Add(whileDoBody->Load("bump"), whileDoBody->Load("i")));
...
bool DoWhileIfThenElseMethod::buildIL()
{
  ...
  TR::IlValue* conditionGreaterThan
    = doWhileBody->GreaterThan(doWhileBody->Load("i"), doWhileBody->Load("compareValue"));
  doWhileBody->IfThen(&ifGreaterThanBody, conditionGreaterThan);

  TR::IlValue* conditionEqualTo
    = ifGreaterThanBody->EqualTo(ifGreaterThanBody->Load("i"), ifGreaterThanBody->Load("sum"));
  ...
  TR::IlValue* conditionLessThan
    = ifGreaterThanBody->LessThan(ifGreaterThanBody->Load("i"), ifGreaterThanBody->Load("sum"));
  ...
  doWhileBody->Store("sum", doWhileBody->Add(doWhileBody->Load("sum"), doWhileBody->Load("bump")));
  doWhileBody->Store("i", doWhileBody->Add(doWhileBody->Load("i"), doWhileBody->Load("bump")));
  doWhileBody->Store("exitCondition", doWhileBody->LessThan(doWhileBody->Load("i"), doWhileBody->Load("final")));
...
bool ForLoopContinueMethod::buildIL()
{
  ...
  TR::IlBuilder* thenPath = NULL;
  ForLoopWithContinue(true, "I", &forBody, &continueBuilder, ConstInt32(0), Load("n"), ConstInt32(1));

  TR::IlValue* continueCondition = forBody->EqualTo(forBody->Load("I"), forBody->ConstInt32(4));

  forBody->IfThen(&thenPath, continueCondition);

  thenPath->Store("Sum", thenPath->Add(thenPath->Load("Sum"), thenPath->Load("I")));
  thenPath->Goto(&continueBuilder);

  forBody->Store("temp", forBody->Add(forBody->Load("I"), forBody->Load("I")));
  forBody->Store("Sum", forBody->Add(forBody->Load("Sum"), forBody->Load("temp")));
...
bool ForLoopBreakMethod::buildIL()
{
  ...
  TR::IlBuilder* breakBody = NULL;
  TR::IlBuilder* thenPath = NULL;
  ForLoopWithBreak(true, "I", &forBody, &breakBody, ConstInt32(0), Load("n"), ConstInt32(1));

  TR::IlValue* breakCondition = forBody->EqualTo(forBody->Load("I"), forBody->ConstInt32(8));
...
bool ForLoopBreakAndContinueMethod::buildIL()
{
  ...
  TR::IlBuilder* breakThenPath = NULL;
  TR::IlBuilder* continueThenPath = NULL;
  ForLoop(true, "I", &forBody, &breakBuilder, &continueBuilder, ConstInt32(0), Load("n"), ConstInt32(1));

  TR::IlValue* breakCondition = forBody->EqualTo(forBody->Load("I"), forBody->ConstInt32(8));

  forBody->IfThen(&breakThenPath, breakCondition);

  breakThenPath->Store("Sum", breakThenPath->Add(breakThenPath->Load("Sum"), breakThenPath->Load("I")));
  ...
  continueThenPath->Store("Sum", continueThenPath->Add(continueThenPath->Load("Sum"), continueThenPath->Load("I")));
