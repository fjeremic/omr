  ...
  int32_t compileOpCodeMethod(functiontype &resultpointer,
    ...
    if (op.isBooleanCompare() && op.isBranch()) {
      ...
      case 2:
        ...
        break;
      default:
        fprintf(stderr, "didn't select an injector based on argument number %d",
                opCodeArgsNum);
        exit(-1);
      }
    }
  ...
  int32_t compileDirectCallOpCodeMethod(
    ...
    case TR::Address:
      ...
      _addressCompiledMethod =
          (signatureCharL_L_testMethodType *)(compileMethod(functionDetails,
                                                            warm, returnCode));
    ...
    default:
      TR_ASSERT(
          0,
          "compilee dataType should be int32, int64, double, float or address");
    }
    EXPECT_TRUE(COMPILATION_SUCCEEDED == returnCode ||
                COMPILATION_REQUESTED == returnCode)
...
protected:
  ...
  template <typename T> static T imulh(T a, T b) {
    return (int32_t)((((int64_t)a * (int64_t)b) >> 32) & 0x00000000ffffffff);
  }
  template <typename T> static T iumulh(T a, T b) {
  ...
  template <typename T1, typename T2> static T2 convert(T1 a, T2 b) {
    return (T2)a;
  }
  ...
  template <typename T> static int32_t comparel(T a, T b) {
  ...
  template <typename T> static int32_t compareg(T a, T b) {
    return std::isnan(static_cast<long double>(a))
               ? 1
               : std::isnan(static_cast<long double>(b))
                     ? 1
  ...
  template <typename C, typename T> static T ternary(C a, T b, T c) {
    return a ? b : c;
  }
};
