...
public:
        ...
        ...)
    {
        ...
        EXPECT_TRUE(COMPILATION_SUCCEEDED == returnCode || COMPILATION_IL_GEN_FAILURE == returnCode || COMPILATION_REQUESTED == returnCode)
...
protected:
    ...
    template <typename T>
    static T rem(T a, T b) { return a % b; }
    template <typename T>
    static T imulh(T a, T b) { return (int32_t)((((int64_t)a * (int64_t)b) >> 32) & 0x00000000ffffffff); }
    template <typename T>
    static T iumulh(T a, T b) { return (uint32_t)((((uint64_t)a * (uint64_t)b) >> 32) & 0x00000000ffffffff); }

    template <typename T>
    static T txor(T a, T b) { return a ^ b; }
    template <typename T>
    static T tor(T a, T b) { return a | b; }
    template <typename T>
    static T tand(T a, T b) { return a & b; }
    template <typename T>
    static T shl(T a, T b) { return a << b; }
    template <typename T>
    static T shr(T a, T b) { return a >> b; }
    template <typename T>
    ...
    static T2 convert(T1 a, T2 b) { return (T2)a; }
    template <typename T>
    static int32_t compareEQ(T a, T b) { return a == b; }
    template <typename T>
    static int32_t compareNE(T a, T b) { return a != b; }
    template <typename T>
    static int32_t compareLT(T a, T b) { return a < b; }
    template <typename T>
    static int32_t compareGE(T a, T b) { return a >= b; }
    template <typename T>
    static int32_t compareGT(T a, T b) { return a > b; }
    template <typename T>
    static int32_t compareLE(T a, T b) { return a <= b; }
    template <typename T>
    static int32_t comparel(T a, T b) { return std::isnan(static_cast<long double>(a)) ? -1 : std::isnan(static_cast<long double>(b)) ? -1 : a > b ? 1 : a == b ? 0 : -1; }
    template <typename T>
    static int32_t compareg(T a, T b) { return std::isnan(static_cast<long double>(a)) ? 1 : std::isnan(static_cast<long double>(b)) ? 1 : a > b ? 1 : a == b ? 0 : -1; }
    template <typename C, typename T>
    static T ternary(C a, T b, T c) { return a ? b : c; }
};
