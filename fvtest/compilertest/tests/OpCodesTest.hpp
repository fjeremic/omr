...
public:
  ...
  int32_t compileOpCodeMethod(functiontype& resultpointer, int32_t opCodeArgsNum, TR::ILOpCodes opCode,
    char* resolvedMethodName, TR::DataType* argTypes, TR::DataType returnType, int32_t& returnCode, int32_t numArgs = 0,
    ...)
  {
    if ((numArgs % 2) != 0) {
  ...
  int32_t compileDirectCallOpCodeMethod(functiontype& resultpointer, int32_t opCodeArgsNum,
    TR::ILOpCodes opCodeCompilee, TR::ILOpCodes opCode, char* compileeResolvedMethodName, char* testResolvedMethodName,
    TR::DataType* argTypes, TR::DataType returnType, int32_t& returnCode)
  {
    ...
    TR::ResolvedMethod callCompilee(__FILE__, LINETOSTR(__LINE__), testResolvedMethodName, opCodeArgsNum, argIlTypes,
      types.PrimitiveType(returnType), 0, &callIlInjector);
...
protected:
  ...
  {
    return (uint64_t)byteswap(uint32_t(a & 0xffffffff)) << 32 | (uint64_t)byteswap(uint32_t((a >> 32) & 0xffffffff));
  }
  int16_t byteswap(int16_t a)
  {
    return byteswap((uint16_t)a);
  }
  int32_t byteswap(int32_t a)
  {
    return byteswap((uint32_t)a);
  }
  int64_t byteswap(int64_t a)
  {
    return byteswap((uint64_t)a);
  }
