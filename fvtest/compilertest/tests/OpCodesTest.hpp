...
#define OMR_CT_EXPECT_EQ(compilee, a, b) \
  if (compilee != NULL) EXPECT_EQ(a, b)
#define OMR_CT_EXPECT_DOUBLE_EQ(compilee, a, b) \
  if (compilee != NULL) EXPECT_DOUBLE_EQ(a, b)
#define OMR_CT_EXPECT_FLOAT_EQ(compilee, a, b) \
  if (compilee != NULL) EXPECT_FLOAT_EQ(a, b)
...
typedef int32_t(unsignedCompareSignatureCharBB_I_testMethodType)(uint8_t,
                                                                 uint8_t);
typedef int32_t(unsignedCompareSignatureCharSS_I_testMethodType)(uint16_t,
                                                                 uint16_t);
typedef int32_t(unsignedCompareSignatureCharII_I_testMethodType)(uint32_t,
                                                                 uint32_t);
typedef int32_t(unsignedCompareSignatureCharJJ_I_testMethodType)(uint64_t,
                                                                 uint64_t);
...
typedef uintptrj_t(signatureCharILL_L_testMethodType)(int32_t, uintptrj_t,
                                                      uintptrj_t);
...
class OpCodesTest : public TestDriver {
 public:
  ...
  int32_t compileOpCodeMethod(functiontype &resultpointer,
                              int32_t opCodeArgsNum, TR::ILOpCodes opCode,
                              char *resolvedMethodName, TR::DataType *argTypes,
                              TR::DataType returnType, int32_t &returnCode,
                              int32_t numArgs = 0, ...) {
    if ((numArgs % 2) != 0) {
      fprintf(stderr,
              "Error: numArgs must be called with zero or an even args, "
              "numArgs is %d",
              numArgs);
      exit(-1);
    }
    ...
    if (op.isBooleanCompare() && op.isBranch()) {
    ...
    } else if (((op.isLoadVar() || op.isLoadConst()) && !op.isIndirect()) ||
               op.isReturn()) {
      ...
      switch (opCodeArgsNum) {
        ...
        default:
          fprintf(stderr,
                  "didn't select an injector based on argument number %d",
                  opCodeArgsNum);
          exit(-1);
      }
    }
    ...
    TR::ResolvedMethod opCodeCompilee(
        __FILE__, LINETOSTR(__LINE__), resolvedMethodName, opCodeArgsNum,
        argIlTypes, types.PrimitiveType(returnType), 0, opCodeInjector);
    ...
    EXPECT_TRUE(COMPILATION_SUCCEEDED == returnCode ||
        ...
        << "compileOpCodeMethod: Compiling method " << resolvedMethodName
        << " failed unexpectedly";
  ...
  template <typename functiontype>
  int32_t compileDirectCallOpCodeMethod(
      functiontype &resultpointer, int32_t opCodeArgsNum,
      TR::ILOpCodes opCodeCompilee, TR::ILOpCodes opCode,
      char *compileeResolvedMethodName, char *testResolvedMethodName,
      TR::DataType *argTypes, TR::DataType returnType, int32_t &returnCode) {
    TR::TypeDictionary types;
    ...
    TR::ResolvedMethod functionCompilee(
        __FILE__, LINETOSTR(__LINE__), compileeResolvedMethodName,
        opCodeArgsNum, argIlTypes, types.PrimitiveType(returnType), 0,
        &functionIlInjector);
    TR::IlGeneratorMethodDetails functionDetails(&functionCompilee);
    switch (returnType) {
      case TR::Int32:
        _int32Compilee = &functionCompilee;
        _int32CompiledMethod =
            (signatureCharI_I_testMethodType *)(compileMethod(
                functionDetails, warm, returnCode));
        functionCompilee.setEntryPoint((void *)_int32CompiledMethod);
        break;
      case TR::Int64:
        _int64Compilee = &functionCompilee;
        _int64CompiledMethod =
            (signatureCharJ_J_testMethodType *)(compileMethod(
                functionDetails, warm, returnCode));
        functionCompilee.setEntryPoint((void *)_int64CompiledMethod);
        break;
      case TR::Double:
        _doubleCompilee = &functionCompilee;
        _doubleCompiledMethod =
            (signatureCharD_D_testMethodType *)(compileMethod(
                functionDetails, warm, returnCode));
        functionCompilee.setEntryPoint((void *)_doubleCompiledMethod);
        break;
      case TR::Float:
        _floatCompilee = &functionCompilee;
        _floatCompiledMethod =
            (signatureCharF_F_testMethodType *)(compileMethod(
                functionDetails, warm, returnCode));
        functionCompilee.setEntryPoint((void *)_floatCompiledMethod);
        break;
      case TR::Address:
        _addressCompilee = &functionCompilee;
        _addressCompiledMethod =
            (signatureCharL_L_testMethodType *)(compileMethod(
                functionDetails, warm, returnCode));
        functionCompilee.setEntryPoint((void *)_addressCompiledMethod);
        break;
      default:
        TR_ASSERT(0,
                  "compilee dataType should be int32, int64, double, float or "
                  "address");
    }
    EXPECT_TRUE(COMPILATION_SUCCEEDED == returnCode ||
                COMPILATION_REQUESTED == returnCode)
        << "Compiling callee method " << compileeResolvedMethodName
        << " failed unexpectedly";

    CallIlInjector callIlInjector(&types, this, opCode);
    TR::ResolvedMethod callCompilee(
        __FILE__, LINETOSTR(__LINE__), testResolvedMethodName, opCodeArgsNum,
        argIlTypes, types.PrimitiveType(returnType), 0, &callIlInjector);
    TR::IlGeneratorMethodDetails callDetails(&callCompilee);
    uint8_t *startPC = compileMethod(callDetails, warm, returnCode);
    EXPECT_TRUE(COMPILATION_SUCCEEDED == returnCode ||
                COMPILATION_REQUESTED == returnCode)
        << "Compiling test method " << testResolvedMethodName
        << " failed unexpectedly";
    resultpointer = reinterpret_cast<functiontype>(startPC);
    return returnCode;
    ;
  ...
  void addUnsupportedOpCodeTest(int32_t opCodeArgsNum, TR::ILOpCodes opCode,
 ...
 protected:
  ...
  // straight C(++) implementations of testMethodType for initial test
  // validation purposes
  uint16_t byteswap(uint16_t a) {
    return (((a & 0xff) << 8) | ((a & 0xff00) >> 8));
  }
  uint32_t byteswap(uint32_t a) {
    return (uint32_t)byteswap(uint16_t(a & 0xffff)) << 16 |
           (uint32_t)byteswap(uint16_t((a >> 16) & 0xffff));
  }
  uint64_t byteswap(uint64_t a) {
    return (uint64_t)byteswap(uint32_t(a & 0xffffffff)) << 32 |
           (uint64_t)byteswap(uint32_t((a >> 32) & 0xffffffff));
  }
  ...
  template <typename T>
  static T rem(T a, T b) {
    return a % b;
  }
  template <typename T>
  static T imulh(T a, T b) {
    return (int32_t)((((int64_t)a * (int64_t)b) >> 32) & 0x00000000ffffffff);
  }
  template <typename T>
  static T iumulh(T a, T b) {
    return (uint32_t)((((uint64_t)a * (uint64_t)b) >> 32) & 0x00000000ffffffff);
  }

  template <typename T>
  static T txor(T a, T b) {
    return a ^ b;
  }
  template <typename T>
  static T tor(T a, T b) {
    return a | b;
  }
  template <typename T>
  static T tand(T a, T b) {
    return a & b;
  }
  template <typename T>
  static T shl(T a, T b) {
    return a << b;
  }
  template <typename T>
  static T shr(T a, T b) {
    return a >> b;
  }
  template <typename T>
  static T rol(T a, T b) {
    bool isOne = false;
  ...
  static T2 convert(T1 a, T2 b) {
    return (T2)a;
  }
  template <typename T>
  static int32_t compareEQ(T a, T b) {
    return a == b;
  }
  template <typename T>
  static int32_t compareNE(T a, T b) {
    return a != b;
  }
  template <typename T>
  static int32_t compareLT(T a, T b) {
    return a < b;
  }
  template <typename T>
  static int32_t compareGE(T a, T b) {
    return a >= b;
  }
  template <typename T>
  static int32_t compareGT(T a, T b) {
    return a > b;
  }
  template <typename T>
  static int32_t compareLE(T a, T b) {
    return a <= b;
  }
  template <typename T>
  static int32_t comparel(T a, T b) {
    return std::isnan(static_cast<long double>(a))
               ? -1
               : std::isnan(static_cast<long double>(b))
                     ? -1
                     : a > b ? 1 : a == b ? 0 : -1;
  }
  template <typename T>
  static int32_t compareg(T a, T b) {
    return std::isnan(static_cast<long double>(a))
               ? 1
               : std::isnan(static_cast<long double>(b))
                     ? 1
