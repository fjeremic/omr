...
public:
    ...
    int32_t compileOpCodeMethod(functiontype& resultpointer, int32_t opCodeArgsNum, TR::ILOpCodes opCode,
        char* resolvedMethodName, TR::DataType* argTypes, TR::DataType returnType, int32_t& returnCode,
        int32_t numArgs = 0, ...)
    {
        if ((numArgs % 2) != 0) {
        ...
        TR::ResolvedMethod opCodeCompilee(__FILE__, LINETOSTR(__LINE__), resolvedMethodName, opCodeArgsNum, argIlTypes,
            types.PrimitiveType(returnType), 0, opCodeInjector);
        TR::IlGeneratorMethodDetails opCodeDetails(&opCodeCompilee);
        uint8_t* startPC = compileMethod(opCodeDetails, warm, returnCode);
        EXPECT_TRUE(COMPILATION_SUCCEEDED == returnCode || COMPILATION_IL_GEN_FAILURE == returnCode
    ...
    template <typename functiontype>
    int32_t compileDirectCallOpCodeMethod(functiontype& resultpointer, int32_t opCodeArgsNum,
        TR::ILOpCodes opCodeCompilee, TR::ILOpCodes opCode, char* compileeResolvedMethodName,
        char* testResolvedMethodName, TR::DataType* argTypes, TR::DataType returnType, int32_t& returnCode)
    {
        TR::TypeDictionary types;
        ...
        TR::ResolvedMethod functionCompilee(__FILE__, LINETOSTR(__LINE__), compileeResolvedMethodName, opCodeArgsNum,
            argIlTypes, types.PrimitiveType(returnType), 0, &functionIlInjector);
        ...
        case TR::Double:
            ...
            _doubleCompiledMethod
                = (signatureCharD_D_testMethodType*)(compileMethod(functionDetails, warm, returnCode));
        ...
        case TR::Address:
            ...
            _addressCompiledMethod
                = (signatureCharL_L_testMethodType*)(compileMethod(functionDetails, warm, returnCode));
        ...
        TR::ResolvedMethod callCompilee(__FILE__, LINETOSTR(__LINE__), testResolvedMethodName, opCodeArgsNum,
    ...
    void addUnsupportedOpCodeTest(int32_t opCodeArgsNum, TR::ILOpCodes opCode, char* resolvedMethodName,
...
protected:
    ...
    {
        return (uint32_t)byteswap(uint16_t(a & 0xffff)) << 16 | (uint32_t)byteswap(uint16_t((a >> 16) & 0xffff));
    }
    ...
    static int32_t comparel(T a, T b)
    {
        return std::isnan(static_cast<long double>(a))
            ? -1
            : std::isnan(static_cast<long double>(b)) ? -1 : a > b ? 1 : a == b ? 0 : -1;
    }
    template <typename T>
