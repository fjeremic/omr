...
extern "C" uint8_t*
compileMethod(TR::IlGeneratorMethodDetails&, TR_Hotness, int32_t&);
...
TR::DataType OpCodesTest::_argTypesUnaryShort[_numberOfUnaryArgs] = {
  TR::Int16
};
TR::DataType OpCodesTest::_argTypesUnaryInt[_numberOfUnaryArgs] = { TR::Int32 };
TR::DataType OpCodesTest::_argTypesUnaryLong[_numberOfUnaryArgs] = {
  TR::Int64
};
TR::DataType OpCodesTest::_argTypesUnaryFloat[_numberOfUnaryArgs] = {
  TR::Float
};
...
TR::DataType OpCodesTest::_argTypesBinaryByte[_numberOfBinaryArgs] = {
  TR::Int8,
  TR::Int8
};
...
TR::DataType OpCodesTest::_argTypesBinaryInt[_numberOfBinaryArgs] = {
  TR::Int32,
  TR::Int32
};
                                      ...
                                      TR::DataType returnType)
{
  ...
  compileOpCodeMethod(never_succeeds,
                      opCodeArgsNum,
                      opCode,
                      resolvedMethodName,
                      argTypes,
...
OpCodesTest::compileUnaryTestMethods()
{
  ...
  compileOpCodeMethod(
    _i2l, _numberOfUnaryArgs, TR::i2l, "i2l", _argTypesUnaryInt, TR::Int64, rc);
  compileOpCodeMethod(
    _i2b, _numberOfUnaryArgs, TR::i2b, "i2b", _argTypesUnaryInt, TR::Int8, rc);
  compileOpCodeMethod(
    _i2s, _numberOfUnaryArgs, TR::i2s, "i2s", _argTypesUnaryInt, TR::Int16, rc);

  compileOpCodeMethod(_l2i,
                      _numberOfUnaryArgs,
                      TR::l2i,
                      "l2i",
                      _argTypesUnaryLong,
                      ...
                      rc);
  compileOpCodeMethod(
    _l2b, _numberOfUnaryArgs, TR::l2b, "l2b", _argTypesUnaryLong, TR::Int8, rc);
  compileOpCodeMethod(_l2s,
                      _numberOfUnaryArgs,
                      TR::l2s,
                      "l2s",
                      _argTypesUnaryLong,
...
OpCodesTest::compileCompareTestMethods()
{
  ...
  compileOpCodeMethod(_ifIuCmplt,
                      _numberOfBinaryArgs,
                      TR::ifiucmplt,
                      "ifIuCmplt",
                      _argTypesBinaryInt,
                      TR::Int32,
                      rc);
  compileOpCodeMethod(_ifIuCmpge,
                      _numberOfBinaryArgs,
                      TR::ifiucmpge,
                      "ifIuCmpge",
                      _argTypesBinaryInt,
                      TR::Int32,
                      rc);
  compileOpCodeMethod(_ifIuCmpgt,
                      _numberOfBinaryArgs,
                      TR::ifiucmpgt,
                      "ifIuCmpgt",
                      _argTypesBinaryInt,
                      TR::Int32,
                      rc);
  compileOpCodeMethod(_ifIuCmple,
                      _numberOfBinaryArgs,
                      TR::ifiucmple,
                      "ifIuCmple",
                      _argTypesBinaryInt,
                      TR::Int32,
                      rc);
  compileOpCodeMethod(_ifLuCmplt,
                      _numberOfBinaryArgs,
                      TR::iflucmplt,
                      "ifLuCmplt",
                      _argTypesBinaryLong,
                      TR::Int32,
                      rc);
  compileOpCodeMethod(_ifLuCmpge,
                      _numberOfBinaryArgs,
                      TR::iflucmpge,
                      "ifLuCmpge",
                      _argTypesBinaryLong,
                      TR::Int32,
                      rc);
  compileOpCodeMethod(_ifLuCmpgt,
                      _numberOfBinaryArgs,
                      TR::iflucmpgt,
                      "ifLuCmpgt",
                      _argTypesBinaryLong,
                      TR::Int32,
                      rc);
  compileOpCodeMethod(_ifLuCmple,
                      _numberOfBinaryArgs,
                      TR::iflucmple,
                      "ifLuCmple",
                      _argTypesBinaryLong,
...
OpCodesTest::invokeIntegerArithmeticTests()
{
  ...
  int32_t intMulArr[][2] = {
    INT_NEG, INT_MINIMUM, INT_ZERO,    INT_POS,     INT_MINIMUM, INT_NEG,
    INT_POS, INT_ZERO,    INT_MAXIMUM, INT_MINIMUM, INT_MINIMUM, INT_MAXIMUM
  };
  ...
  for (uint32_t i = 0; i < testCaseArrLength; ++i) {
    OMR_CT_EXPECT_EQ(_iAdd,
                     add(intAddArr[i][0], intAddArr[i][1]),
                     _iAdd(intAddArr[i][0], intAddArr[i][1]));

    sprintf(resolvedMethodName, "iAddConst1_Testcase%d", i);
    compileOpCodeMethod(iBinaryCons,
                        _numberOfBinaryArgs,
                        TR::iadd,
                        resolvedMethodName,
                        _argTypesBinaryInt,
  ...
  for (uint32_t i = 0; i < testCaseArrLength; ++i) {
    OMR_CT_EXPECT_EQ(_iSub,
                     sub(intSubArr[i][0], intSubArr[i][1]),
  ...
  for (uint32_t i = 0; i < testCaseArrLength; ++i) {
    OMR_CT_EXPECT_EQ(_iMul,
                     mul(intMulArr[i][0], intMulArr[i][1]),
  ...
  testCaseArrLength = sizeof(intMulArr) / sizeof(intMulArr[0]);
  for (uint32_t i = 0; i < testCaseArrLength; ++i) {
    OMR_CT_EXPECT_EQ(_iMulh,
                     imulh(intMulArr[i][0], intMulArr[i][1]),
  ...
  for (uint32_t i = 0; i < testCaseArrLength; ++i) {
    OMR_CT_EXPECT_EQ(_iDiv,
                     div(intDivArr[i][0], intDivArr[i][1]),
  ...
  for (uint32_t i = 0; i < testCaseArrLength; ++i) {
    OMR_CT_EXPECT_EQ(_iRem,
                     rem(intRemArr[i][0], intRemArr[i][1]),
...
OpCodesTest::invokeMemoryOperationTests()
{
  ...
  for (int32_t i = 0; i < testCaseNum; i++) {
    OMR_CT_EXPECT_EQ(
      _iLoadi, intDataArray[i], _iLoadi((uintptrj_t)(&intDataArray[i])));
  }

  testCaseNum = sizeof(shortDataArray) / sizeof(shortDataArray[0]);
  for (int32_t i = 0; i < testCaseNum; i++) {
    OMR_CT_EXPECT_EQ(
      _sLoadi, shortDataArray[i], _sLoadi((uintptrj_t)(&shortDataArray[i])));
  }

  testCaseNum = sizeof(byteDataArray) / sizeof(byteDataArray[0]);
  for (int32_t i = 0; i < testCaseNum; i++) {
    OMR_CT_EXPECT_EQ(
      _bLoadi, byteDataArray[i], _bLoadi((uintptrj_t)(&byteDataArray[i])));
  }

  testCaseNum = sizeof(longDataArray) / sizeof(longDataArray[0]);
  for (int32_t i = 0; i < testCaseNum; i++) {
    OMR_CT_EXPECT_EQ(
      _lLoadi, longDataArray[i], _lLoadi((uintptrj_t)(&longDataArray[i])));
  }

  testCaseNum = sizeof(doubleDataArray) / sizeof(doubleDataArray[0]);
  for (int32_t i = 0; i < testCaseNum; i++) {
    OMR_CT_EXPECT_EQ(
      _dLoadi, doubleDataArray[i], _dLoadi((uintptrj_t)(&doubleDataArray[i])));
  }

  testCaseNum = sizeof(floatDataArray) / sizeof(floatDataArray[0]);
  for (int32_t i = 0; i < testCaseNum; i++) {
    OMR_CT_EXPECT_EQ(
      _fLoadi, floatDataArray[i], _fLoadi((uintptrj_t)(&floatDataArray[i])));
  }

  testCaseNum = sizeof(addressDataArray) / sizeof(addressDataArray[0]);
  for (int32_t i = 0; i < testCaseNum; i++) {
    OMR_CT_EXPECT_EQ(_aLoadi,
                     addressDataArray[i],
...
OpCodesTest::invokeUnaryTests()
{
  ...
  for (uint32_t i = 0; i < testCaseNum; ++i) {
    ...
    compileOpCodeMethod(iUnaryCons,
                        ...
                        &intDataArray[i]);
    OMR_CT_EXPECT_EQ(
      iUnaryCons, neg(intDataArray[i]), iUnaryCons(INT_PLACEHOLDER_1));
  }
  ...
  for (uint32_t i = 0; i < testCaseNum; ++i) {
    ...
    compileOpCodeMethod(iUnaryCons,
                        ...
                        &intDataArray[i]);
    OMR_CT_EXPECT_EQ(
      iUnaryCons, abs(intDataArray[i]), iUnaryCons(INT_PLACEHOLDER_1));
  }
  ...
  for (int32_t i = 0; i < testCaseNum; i++) {
    ...
    compileOpCodeMethod(iUnaryCons,
                        ...
                        &(intDataArray[i]));
    OMR_CT_EXPECT_EQ(
      iUnaryCons, intDataArray[i], iUnaryCons(INT_PLACEHOLDER_1));
  }
  ...
  for (int32_t i = 0; i < testCaseNum; i++) {
    ...
    compileOpCodeMethod(bUnaryCons,
                        ...
                        &(byteDataArray[i]));
    OMR_CT_EXPECT_EQ(
      bUnaryCons, byteDataArray[i], bUnaryCons(BYTE_PLACEHOLDER_1));
  }
  ...
  for (int32_t i = 0; i < testCaseNum; i++) {
    ...
    compileOpCodeMethod(sUnaryCons,
                        ...
                        &(shortDataArray[i]));
    OMR_CT_EXPECT_EQ(
      sUnaryCons, shortDataArray[i], sUnaryCons(SHORT_PLACEHOLDER_1));
  }
  ...
  for (uint32_t i = 0; i < testCaseNum; ++i) {
    OMR_CT_EXPECT_EQ(
      _i2b, convert(intDataArray[i], BYTE_POS), _i2b(intDataArray[i]));
    OMR_CT_EXPECT_EQ(
      _i2s, convert(intDataArray[i], SHORT_POS), _i2s(intDataArray[i]));
    OMR_CT_EXPECT_EQ(
      _i2l, convert(intDataArray[i], LONG_POS), _i2l(intDataArray[i]));

    sprintf(resolvedMethodName, "i2bConst%d", i + 1);
    compileOpCodeMethod(i2bConst,
                        _numberOfUnaryArgs,
                        TR::i2b,
                        resolvedMethodName,
                        _argTypesUnaryInt,
                        ...
                        1,
                        &intDataArray[i]);
    OMR_CT_EXPECT_EQ(i2bConst,
                     convert(intDataArray[i], BYTE_POS),
                     i2bConst(INT_PLACEHOLDER_1));

    sprintf(resolvedMethodName, "i2sConst%d", i + 1);
    compileOpCodeMethod(i2sConst,
                        _numberOfUnaryArgs,
                        TR::i2s,
                        resolvedMethodName,
                        _argTypesUnaryInt,
                        ...
                        1,
                        &intDataArray[i]);
    OMR_CT_EXPECT_EQ(i2sConst,
                     convert(intDataArray[i], SHORT_POS),
                     i2sConst(INT_PLACEHOLDER_1));

    sprintf(resolvedMethodName, "i2lConst%d", i + 1);
    compileOpCodeMethod(i2lConst,
                        _numberOfUnaryArgs,
                        TR::i2l,
                        resolvedMethodName,
                        _argTypesUnaryInt,
                        ...
                        1,
                        &intDataArray[i]);
    OMR_CT_EXPECT_EQ(i2lConst,
                     convert(intDataArray[i], LONG_POS),
  ...
  for (uint32_t i = 0; i < testCaseNum; ++i) {
    OMR_CT_EXPECT_EQ(
      _l2b, convert(longDataArray[i], BYTE_POS), _l2b(longDataArray[i]));
    OMR_CT_EXPECT_EQ(
      _l2s, convert(longDataArray[i], SHORT_POS), _l2s(longDataArray[i]));
    OMR_CT_EXPECT_EQ(
      _l2i, convert(longDataArray[i], INT_POS), _l2i(longDataArray[i]));

    sprintf(resolvedMethodName, "l2bConst%d", i + 1);
    compileOpCodeMethod(l2bConst,
                        _numberOfUnaryArgs,
                        TR::l2b,
                        resolvedMethodName,
                        _argTypesUnaryLong,
                        ...
                        1,
                        &longDataArray[i]);
    OMR_CT_EXPECT_EQ(l2bConst,
                     convert(longDataArray[i], BYTE_POS),
                     l2bConst(LONG_PLACEHOLDER_1));

    sprintf(resolvedMethodName, "l2sConst%d", i + 1);
    compileOpCodeMethod(l2sConst,
                        _numberOfUnaryArgs,
                        TR::l2s,
                        resolvedMethodName,
                        _argTypesUnaryLong,
                        ...
                        1,
                        &longDataArray[i]);
    OMR_CT_EXPECT_EQ(l2sConst,
                     convert(longDataArray[i], SHORT_POS),
                     l2sConst(LONG_PLACEHOLDER_1));

    sprintf(resolvedMethodName, "l2iConst%d", i + 1);
    compileOpCodeMethod(l2iConst,
                        _numberOfUnaryArgs,
                        TR::l2i,
                        resolvedMethodName,
                        _argTypesUnaryLong,
                        ...
                        1,
                        &longDataArray[i]);
    OMR_CT_EXPECT_EQ(l2iConst,
                     convert(longDataArray[i], INT_POS),
  ...
  for (uint32_t i = 0; i < 3; ++i) {
    OMR_CT_EXPECT_EQ(
      _f2i, convert(floatDataArray[i], INT_POS), _f2i(floatDataArray[i]));

    sprintf(resolvedMethodName, "f2iConst%d", i + 1);
    compileOpCodeMethod(f2iConst,
                        _numberOfUnaryArgs,
                        TR::f2i,
                        resolvedMethodName,
                        _argTypesUnaryFloat,
                        ...
                        1,
                        &floatDataArray[i]);
    OMR_CT_EXPECT_EQ(f2iConst,
                     convert(floatDataArray[i], INT_POS),
  ...
  for (uint32_t i = 0; i < 3; ++i) {
    OMR_CT_EXPECT_EQ(
      _d2i, convert(doubleDataArray[i], INT_POS), _d2i(doubleDataArray[i]));

    sprintf(resolvedMethodName, "d2iConst%d", i + 1);
    compileOpCodeMethod(d2iConst,
                        _numberOfUnaryArgs,
                        TR::d2i,
                        resolvedMethodName,
                        _argTypesUnaryDouble,
                        ...
                        1,
                        &doubleDataArray[i]);
    OMR_CT_EXPECT_EQ(d2iConst,
                     convert(doubleDataArray[i], INT_POS),
...
OpCodesTest::invokeBitwiseTests()
{
  ...
  for (int32_t i = 0; i < testCaseNum; i++) {
    OMR_CT_EXPECT_EQ(_iAnd,
                     tand(intAndArr[i][0], intAndArr[i][1]),
    ...
    compileOpCodeMethod(iBitwiseConst,
                        ...
                        2,
                        &(intAndArr[i][1]));
    OMR_CT_EXPECT_EQ(iBitwiseConst,
                     tand(intAndArr[i][0], intAndArr[i][1]),
  ...
  for (int32_t i = 0; i < testCaseNum; i++) {
    OMR_CT_EXPECT_EQ(_iOr,
                     tor(intOrArr[i][0], intOrArr[i][1]),
  ...
  for (int32_t i = 0; i < testCaseNum; i++) {
    OMR_CT_EXPECT_EQ(_iXor,
                     txor(intXorArr[i][0], intXorArr[i][1]),
...
OpCodesTest::invokeDisabledOpCodesTests()
{
  ...
  // frem
  OMR_CT_EXPECT_FLOAT_EQ(_fRem,
                         remainderf(FLOAT_MINIMUM, FLOAT_MINIMUM),
                         _fRem(FLOAT_MINIMUM, FLOAT_MINIMUM));
  OMR_CT_EXPECT_FLOAT_EQ(_fRem,
                         remainderf(FLOAT_ZERO, FLOAT_MAXIMUM),
                         _fRem(FLOAT_ZERO, FLOAT_MAXIMUM));
  OMR_CT_EXPECT_FLOAT_EQ(
    _fRem, remainderf(FLOAT_POS, FLOAT_NEG), _fRem(FLOAT_POS, FLOAT_NEG));
  OMR_CT_EXPECT_FLOAT_EQ(_fRem,
                         remainderf(FLOAT_MAXIMUM, FLOAT_POS),
                         _fRem(FLOAT_MAXIMUM, FLOAT_POS));

  // drem
  OMR_CT_EXPECT_DOUBLE_EQ(_dRem,
                          remainder(DOUBLE_MINIMUM, DOUBLE_MINIMUM),
                          _dRem(DOUBLE_MINIMUM, DOUBLE_MINIMUM));
  OMR_CT_EXPECT_DOUBLE_EQ(_dRem,
                          remainder(DOUBLE_ZERO, DOUBLE_MAXIMUM),
                          _dRem(DOUBLE_ZERO, DOUBLE_MAXIMUM));
  OMR_CT_EXPECT_DOUBLE_EQ(
    _dRem, remainder(DOUBLE_POS, DOUBLE_NEG), _dRem(DOUBLE_POS, DOUBLE_NEG));
  OMR_CT_EXPECT_DOUBLE_EQ(_dRem,
                          remainder(DOUBLE_MAXIMUM, DOUBLE_POS),
...
OpCodesTest::invokeCompareTests()
{
  ...
  int64_t lCmpneDataArr[][2] = {
    LONG_ZERO, LONG_ZERO, LONG_MAXIMUM, LONG_MINIMUM
  };
  ...
  uint64_t luCmpgtDataArr[][2] = {
    ULONG_MINIMUM, ULONG_MAXIMUM, ULONG_MAXIMUM, ULONG_MINIMUM
  };
  ...
  uint64_t luCmpleDataArr[][2] = {
    ULONG_MAXIMUM, ULONG_POS, ULONG_POS, ULONG_MAXIMUM
  };
  ...
  int32_t ifIcmpltDataArr[][2] = {
    INT_MINIMUM, INT_ZERO, INT_ZERO, INT_MINIMUM
  };
  ...
  uint32_t ifIuCmpgtDataArr[][2] = {
    UINT_MAXIMUM, UINT_MAXIMUM, UINT_MAXIMUM, UINT_POS
  };
  uint32_t ifIuCmpltDataArr[][2] = {
    UINT_MINIMUM, UINT_POS, UINT_POS, UINT_MINIMUM
  };
  uint32_t ifIuCmpgeDataArr[][2] = {
    UINT_POS, UINT_MINIMUM, UINT_MINIMUM, UINT_POS
  };
  uint32_t ifIuCmpleDataArr[][2] = {
    UINT_POS, UINT_MINIMUM, UINT_MINIMUM, UINT_POS
  };
  uint64_t ifLuCmpgtDataArr[][2] = {
    ULONG_MINIMUM, ULONG_MINIMUM, ULONG_MAXIMUM, ULONG_POS
  };
  uint64_t ifLuCmpltDataArr[][2] = {
    ULONG_POS, ULONG_MAXIMUM, ULONG_MAXIMUM, ULONG_POS
  };
  uint64_t ifLuCmpgeDataArr[][2] = {
    ULONG_MAXIMUM, ULONG_POS, ULONG_POS, ULONG_MAXIMUM
  };
  uint64_t ifLuCmpleDataArr[][2] = {
    ULONG_MAXIMUM, ULONG_MAXIMUM, ULONG_POS, ULONG_MINIMUM
  };
...
OpCodesTest::invokeTernaryTests()
{
  ...
  int32_t iternaryChild1Arr[] = { INT_MAXIMUM, INT_MAXIMUM, INT_MINIMUM,
                                  INT_POS,     INT_ZERO,    INT_MINIMUM,
                                  INT_POS,     INT_ZERO,    INT_NEG,
                                  INT_NEG,     INT_NEG,     INT_ZERO,
                                  INT_POS,     INT_NEG,     INT_ZERO,
  ...
  for (int32_t i = 0; i < testCaseNum; i++) {
    ...
    compileOpCodeMethod(iTernaryConst,
                        _numberOfTernaryArgs,
                        TR::iternary,
                        resolvedMethodName,
                        _argTypesTernaryInt,
    ...
    OMR_CT_EXPECT_EQ(iTernaryConst,
                     ternary(iternaryChild1Arr[i], intArr[i][0], intArr[i][1]),
                     iTernaryConst(iternaryChild1Arr[i],
                                   INT_PLACEHOLDER_2,
                                   INT_PLACEHOLDER_3));
...
OpCodesTest::invokeAddressTests()
{
  ...
  for (int32_t i = 0; i < testCaseNum; i++) {
    ...
    compileOpCodeMethod(aUnaryCons,
                        _numberOfUnaryArgs,
                        TR::aconst,
                        resolvedMethodName,
                        _argTypesUnaryAddress,
                        ...
                        &(aUnaryDataArr[i]));
    OMR_CT_EXPECT_EQ(
      aUnaryCons, aUnaryDataArr[i], aUnaryCons(ADDRESS_PLACEHOLDER_1));
  }
  ...
  for (int32_t i = 0; i < testCaseNum; i++) {
    OMR_CT_EXPECT_EQ(
      _a2i, convert(aUnaryDataArr[i], INT_POS), _a2i(aUnaryDataArr[i]));

    sprintf(resolvedMethodName, "a2iConst%d", i + 1);
    compileOpCodeMethod(a2iConst,
                        _numberOfUnaryArgs,
                        TR::a2i,
                        resolvedMethodName,
                        _argTypesUnaryAddress,
                        ...
                        1,
                        &aUnaryDataArr[i]);
    OMR_CT_EXPECT_EQ(a2iConst,
                     convert(aUnaryDataArr[i], INT_POS),
