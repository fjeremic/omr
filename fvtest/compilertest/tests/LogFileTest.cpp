...
void TestCompiler::LogFileTest::compileTests() {
 ...
 */
bool TestCompiler::LogFileTest::fileExists(std::string name) {
  struct stat buf;
 ...
 */
bool TestCompiler::LogFileTest::fileIsNotEmpty(std::string logFile) {
...
std::map<const char *, bool> TestCompiler::LogFileTest::buildKeywordMap(
 ...
 */
void TestCompiler::LogFileTest::createLog(std::string logFile,
                                          const char *logType) {
 ...
 *        with that log type.
 */
void TestCompiler::LogFileTest::runKeywordTests(
 ...
 */
void TestCompiler::LogFileTest::checkLogForKeywords(
    std::map<const char *, bool> keywords, const char *logFile) {
  ...
  while (keywordsFound < keywords.size()) {
    ...
    ASSERT_FALSE(logFileStream.eof())
        << "End of log file reached without finding all keywords";
    ASSERT_TRUE(logFileStream.good())
        << "An error occured during an I/O operation while reading logfile";
  ...
  ASSERT_EQ(keywords.size(), keywordsFound)
      << "Only " << keywordsFound << " of " << keywords.size()
      << " keywords found";
}
 ...
 */
void TestCompiler::LogFileTest::forkAndCompile(std::string logFile,
                                               const char *logType) {
   ...
   */
  ASSERT_EXIT(createLog(logFile, logType), ::testing::ExitedWithCode(0), "")
      << "Error in createLog.";
...
TEST_F(LogFileTest, EmptyTFLogTest) {
  ...
  forkAndCompile(logFile, logType);
  ASSERT_TRUE(fileIsNotEmpty(logFile))
      << "The traceFull log file created is empty";
}
...
TEST_F(LogFileTest, KeywordsLogTest) {
  ...
  */
  const char *keywordsTraceFull[] = {"<jitlog", "<ilgen", "<trees", "</trees>",
                                     "BBStart", "BBEnd",  "<block_"};
  logFileChecks.insert(std::pair<const char *, std::map<const char *, bool>>(
      "traceFull",
      buildKeywordMap(std::vector<const char *>(
          keywordsTraceFull, keywordsTraceFull + sizeof(keywordsTraceFull) /
                                                     sizeof(const char *)))));
  ...
  logFileChecks.insert(std::pair<const char *, std::map<const char *, bool>>(
      ...
      buildKeywordMap(std::vector<const char *>(
          keywordsTraceCG,
          keywordsTraceCG + sizeof(keywordsTraceCG) / sizeof(const char *)))));
