...
std::map<const char*, bool> TestCompiler::LogFileTest::buildKeywordMap(std::vector<const char*> inputs)
{
...
TEST_F(LogFileTest, KeywordsLogTest)
{
  ...
  */
  ...
  logFileChecks.insert(std::pair<const char*, std::map<const char*, bool> >("traceCG",
    buildKeywordMap(
      std::vector<const char*>(keywordsTraceCG, keywordsTraceCG + sizeof(keywordsTraceCG) / sizeof(const char*)))));
