...
void TestCompiler::LimitFileTest::delayUnlink(const char *file) {
 ...
 */
void TestCompiler::LimitFileTest::generateVLog(const char *vlog,
                                               const char *limitFile) {
  std::string args = std::string("-Xjit:verbose,disableSuffixLogs,vlog=");
  args = args + vlog;

  // Add the limit file, if provided.
  if (limitFile) args = args + ",limitFile=" + limitFile;
 ...
 */
void TestCompiler::LimitFileTest::checkVLogForMethod(const char *vlog,
                                                     const char *method,
                                                     const char *level,
                                                     int *foundOnLine) {
 ...
 */
void TestCompiler::LimitFileTest::createVLog(const char *vlog,
                                             const char *limitFile) {
  delayUnlink(vlog);
 ...
 */
void TestCompiler::LimitFileTest::createAndCheckVLog(const char *vlog,
                                                     const char *limitFile,
                                                     int *methodLine) {
...
TEST_F(LimitFileTest, UseLimitFileRangeTest) {
  ...
  std::string limitArg = std::string("(") + limitFile + "," + iNegLineStr +
                         "," + iNegLineStr + ")";
