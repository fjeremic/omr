...
class SimplifierFoldAndInfo : public TestCompiler::MethodInfo {
 public:
  SimplifierFoldAndInfo(TestDriver* test) : _ilInjector(&_types, test) {
    ...
    _args[0] = Int32;
    DefineFunction(__FILE__, LINETOSTR(__LINE__), "simplifierFoldAnd", 1, _args,
                   Int64);
...
class SimplifierFoldAndTest : public OptTestDriver {
 public:
  ...
  void invokeTests() {
    ...
    auto testCompiledMethod =
        getCompiledMethod<SimplifierFoldAndInfo::MethodType>();
