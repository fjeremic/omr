...
bool OpIlInjector::isOpCodeSupported() {
  return comp()->cg()->isILOpCodeSupported(_opCode);
}

TR::Node *OpIlInjector::parm(uint32_t slot) {
  TR_ASSERT(
      slot > 0 && slot <= _numOptArgs,
      "parm: slot %d should be greater than 0 and less than _numOptArgs %d\n",
      slot, _numOptArgs);
