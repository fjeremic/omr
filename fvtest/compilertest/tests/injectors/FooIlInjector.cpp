...
bool FooIlInjector::injectIL() {
  ...
  storeToTemp(newIndexSymRef,
              callFunction(barMethod, Int32, 1, indexParameter()));
  ifjump(TR::ificmplt, loadTemp(newIndexSymRef), iconst(0), 3);

  // Block 3: blocks(1)
  // if (newIndex >= 100) goto Block5;
  ifjump(TR::ificmpge, loadTemp(newIndexSymRef), iconst(test->dataArraySize()),
         3);

  // Block 4: blocks(2)
  // return dataArray[newIndex];
  returnValue(arrayLoad(staticAddress(test->dataArray()),
                        loadTemp(newIndexSymRef), Int32));
