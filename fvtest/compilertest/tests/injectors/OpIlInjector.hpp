...
public:
  OpIlInjector(TR::TypeDictionary *types, TestDriver *test,
               TR::ILOpCodes opCode)
      : TR::IlInjector(types, test), _opCode(opCode), _numOptArgs(0),
  ...
  virtual void iconstParm(uint32_t slot, int32_t value) {
    TR_ASSERT(slot > 0 && slot <= _numOptArgs,
              "iconstParm: slot %d should be greater than 0 and less than "
              "_numOptArgs %d\n",
              slot, _numOptArgs);
  ...
  virtual void lconstParm(uint32_t slot, int64_t value) {
    TR_ASSERT(slot > 0 && slot <= _numOptArgs,
              "lconstParm: slot %d should be greater than 0 and less than "
              "_numOptArgs %d\n",
              slot, _numOptArgs);
  ...
  virtual void bconstParm(uint32_t slot, int8_t value) {
    TR_ASSERT(slot > 0 && slot <= _numOptArgs,
              "bconstParm: slot %d should be greater than 0 and less than "
              "_numOptArgs %d\n",
              slot, _numOptArgs);
  ...
  virtual void sconstParm(uint32_t slot, int16_t value) {
    TR_ASSERT(slot > 0 && slot <= _numOptArgs,
              "sconstParm: slot %d should be greater than 0 and less than "
              "_numOptArgs %d\n",
              slot, _numOptArgs);
  ...
  virtual void aconstParm(uint32_t slot, uintptrj_t value) {
    TR_ASSERT(slot > 0 && slot <= _numOptArgs,
              "aconstParm: slot %d should be greater than 0 and less than "
              "_numOptArgs %d\n",
              slot, _numOptArgs);
  ...
  virtual void dconstParm(uint32_t slot, double value) {
    TR_ASSERT(slot > 0 && slot <= _numOptArgs,
              "dconstParm: slot %d should be greater than 0 and less than "
              "_numOptArgs %d\n",
              slot, _numOptArgs);
  ...
  virtual void fconstParm(uint32_t slot, float value) {
    TR_ASSERT(slot > 0 && slot <= _numOptArgs,
              "fconstParm: slot %d should be greater than 0 and less than "
              "_numOptArgs %d\n",
              slot, _numOptArgs);
