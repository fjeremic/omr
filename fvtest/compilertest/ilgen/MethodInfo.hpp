...
public:
  MethodInfo()
      : _file(NULL), _line(NULL), _name(NULL), _numArgs(0), _argTypes(NULL),
        _returnType(NULL) {}

  /**
   * Define the signature of the method this class is describing.
   */
  void DefineFunction(char *file, char *line, char *name, int32_t numArgs,
                      TR::IlType **argTypes, TR::IlType *returnType) {
    _file = file;
    _line = line;
    _name = name;
    _numArgs = numArgs;
