...
public:
     ...
     */
    void DefineFunction(
        char* file, char* line, char* name, int32_t numArgs, TR::IlType** argTypes, TR::IlType* returnType)
    {
        _file = file;
        _line = line;
        _name = name;
     ...
     */
    void DefineILInjector(TR::IlInjector* ilInjector) { _ilInjector = ilInjector; }
    ...
    {
        TR_ASSERT(_file != NULL && _line != NULL && _name != NULL && (_numArgs == 0 || _argTypes != NULL),
            "Cannot create ResolvedMethod without signature");
        TR_ASSERT(_ilInjector != NULL, "Cannot create ResolvedMethod without IlInjector");
