...
class IlInjector : public OMR::IlInjector {
 public:
  ...
  IlInjector(TR::TypeDictionary *types)
      : OMR::IlInjector(types), _test(NULL), _method(NULL) {}
  ...
  IlInjector(TR::ResolvedMethod *method, TR::TypeDictionary *types,
             TestDriver *test)
      : OMR::IlInjector(types) {
    setMethodAndTest(method, test);
  }
  IlInjector(TR::IlInjector *source) : OMR::IlInjector(source) {
  ...
  TR::Node *callFunction(TR::ResolvedMethod *function, TR::IlType *returnType,
                         int32_t numArgs, TR::Node *firstArg);
...
class IlInjector : public TestCompiler::IlInjector {
 public:
  IlInjector(TR::TypeDictionary *types) : TestCompiler::IlInjector(types) {}

  IlInjector(TR::TypeDictionary *types, TestCompiler::TestDriver *test)
      : TestCompiler::IlInjector(types, test) {}

  IlInjector(TR::ResolvedMethod *method, TR::TypeDictionary *types,
             TestCompiler::TestDriver *test)
      : TestCompiler::IlInjector(method, types, test) {}

  IlInjector(TR::IlInjector *source) : TestCompiler::IlInjector(source) {}
};
