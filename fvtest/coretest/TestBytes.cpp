...
TEST(TestBytes, AlignToZero) {
  // Zero is considered an invalid input, since the calculation always results
  // in zero. This test is validating that behaviour, but users should NOT be
  // aligning to zero. the minimum valid alignment is 1.
...
TEST(TestBytes, AlignMaximumSizeFor16byteAlignment) {
  ...
  EXPECT_EQ(std::numeric_limits<size_t>::max() - 15,
            align(std::numeric_limits<size_t>::max() - 17, 16));
  EXPECT_EQ(std::numeric_limits<size_t>::max(),
            align(std::numeric_limits<size_t>::max(), 1));
}
