...
TEST(TestTypeTraits, RemoveConst)
{
  ...
  EXPECT_TRUE(
    (IsSame<volatile int, RemoveConst<const volatile int>::Type>::VALUE));
}
...
TEST(TestTypeTraits, RemoveVolatile)
{
  ...
  EXPECT_TRUE(
    (IsSame<const int, RemoveVolatile<const volatile int>::Type>::VALUE));
}
...
TEST(TestTypeTraits, RemovePointer)
{
  ...
  EXPECT_TRUE(
    (IsSame<const volatile int,
            RemovePointer<const volatile int* const volatile>::Type>::VALUE));
  EXPECT_TRUE((IsSame<int*&, RemovePointer<int*&>::Type>::VALUE));
}
...
template<typename T>
typename EnableIf<IsSame<T, int>::VALUE, bool>::Type
sillyIsInt(T x)
{
  return true;
}
