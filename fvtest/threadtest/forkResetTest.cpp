...
/**
 * Note: All tests are disabled until issues with hangs related to undefined
 * behavior are solved in omrthread_lib_post_fork_reset.
 *
 * Tests the following:
 * 1. Sibling thread enter monitor before fork and the child process can enter
 * and exit the monitor
 * 2. Sibling thread waiting on monitor before the fork and the child process
 * can enter and exit the monitor
 */
TEST(ThreadForkResetTest, WaitNotify) { waitNotifyStart(1); }
...
static void waitNotifyStart(int forkCount) {
   ...
   * The sibling thread cannot enter the monitor because it is already entered
   * here.
   */
   ...
   * The sibling will do notify once its entered the monitor and then wait until
   * after the fork. The sibling's wait will release the monitor, allowing the
   * parent to continue here.
   */
  EXPECT_FALSE(J9THREAD_TIMED_OUT ==
               omrthread_monitor_wait_timed(testdata.monitor, TIMEOUT, 0))
   ...
   * Then, the wait call will release the monitor to let the sibling thread
   * finish. Once the sibling thread done, it will notify again and exit the
   * monitor to let he parent destroy the monitor.
   */
  ssize_t readBytes =
      read(pipedata[0], (int *)&testdata.rc, sizeof(testdata.rc));
  EXPECT_TRUE(readBytes == sizeof(testdata.rc))
      << "Didn't read back enough from pipe";
  EXPECT_TRUE(0 == testdata.rc) << "Failure occurred in child process.";
  close(pipedata[0]);
  close(pipedata[1]);
  EXPECT_TRUE(0 == omrthread_monitor_notify(testdata.monitor))
      << "Notifying monitor failed after fork: parent thread does not own "
         "monitor.";
  EXPECT_TRUE(0 == omrthread_monitor_exit(testdata.monitor))
      << "Exiting monitor after fork failed: parent thread does not own "
         "monitor.";
...
static void waitNotifyChild(testdata_t *testdata, int pipedata[2]) {
  omrthread_lib_postForkChild();
  ...
  J9_IGNORE_RETURNVAL(
      write(pipedata[1], (int *)&testdata->rc, sizeof(testdata->rc)));
  close(pipedata[0]);
  close(pipedata[1]);
  exit(0);
}
 ...
 *
 * Tests:
 * 1. Monitor owned by sibling thread in parent process can do monitor enter,
 * wait, notify, and exit as if it were unowned. 2.
 */

TEST(ThreadForkResetTest, WaitNotify2) { waitNotify2Start(2); }
 ...
 *
 * Three monitors: one for sibling 1, one for sibling 2, one for the test
 * notification
 ...
 * 			2. Sibling 2 enters and notifies monitor 1 without
 * exiting
...
static void waitNotify2Start(int forkCount) {
  ...
  ASSERT_NO_FATAL_FAILURE(
      createJoinableThread(&t1, waitNotify2aMain, &testdata));
  EXPECT_FALSE(J9THREAD_TIMED_OUT ==
               omrthread_monitor_wait_timed(testdata.monitor1, TIMEOUT, 0))
      << "Timed out waiting for sibling thread 1 to enter monitor 1.";
  EXPECT_TRUE(0 == omrthread_monitor_exit(testdata.monitor1))
      << "Exiting monitor 1 before fork failed: parent thread does not own "
         "monitor.";

  omrthread_monitor_enter(testdata.monitor2);
  ASSERT_NO_FATAL_FAILURE(
      createJoinableThread(&t2, waitNotify2bMain, &testdata));
  EXPECT_FALSE(J9THREAD_TIMED_OUT ==
               omrthread_monitor_wait_timed(testdata.monitor2, TIMEOUT, 0))
  ...
  EXPECT_TRUE(0 == omrthread_monitor_notify(testdata.monitor2))
      << "Notifying monitor 2 failed after fork: parent thread does not own "
         "monitor.";
  EXPECT_TRUE(0 == omrthread_monitor_exit(testdata.monitor2))
      << "Exiting monitor 2 after fork failed: parent thread does not own "
         "monitor.";
...
static void waitNotify2Child(testdata2_t *testdata, int pipedata[2]) {
  omrthread_lib_postForkChild();
  testdata->rc = omrthread_monitor_enter(testdata->monitor1);
  if (0 == testdata->rc) {
    if (J9THREAD_TIMED_OUT !=
        omrthread_monitor_wait_timed(testdata->monitor1, START_DELAY, 0)) {
      testdata->rc = 1;
    }
  }
  if (0 == testdata->rc) {
...
TEST(ThreadForkResetTest, AttachDetach) { attachDetachStart(1); }
...
static void attachDetachStart(int forkCount) {
  ...
  /* After fork, notify the sibling thread and wait for it to unlock the global
   * mutex. */
  ssize_t readBytes =
      read(pipedata[0], (int *)&testdata.rc, sizeof(testdata.rc));
  EXPECT_TRUE(readBytes == sizeof(testdata.rc))
 ...
 * behavior are solved in omrthread_lib_post_fork_reset.
 *
 * The grandchild process test is to test forking in a child process after a
 * fork. The all tests are run again, but with a second call to fork.
 */
