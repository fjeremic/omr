...
initPrioMap(void)
{
  ...
  host_info(
    mach_host_self(), flavor, (host_info_t)&priorityInfoData, &msgTypeNumber);
...
initRealtimePrioMap(void)
{
  ...
  initialize_thread_policies(&policy_regular_thread, &policy_realtime_thread);
  {
    ...
    if (getCurrentThreadParams(&prio, &policy_regular_thread)) {
      ...
      omrTestEnv->log(
        LEVEL_ERROR, "Could not get current thread params, assuming default\n");
      prio = 0;
