...
const char *mapOSPolicy(intptr_t policy) {
  switch (policy) {
    case OS_SCHED_OTHER:
      return "SCHED_OTHER";
    case OS_SCHED_FIFO:
...
void initPrioMap(void) {
  ...
  host_info(mach_host_self(), flavor, (host_info_t)&priorityInfoData,
            &msgTypeNumber);
...
int getRTPolicy(omrthread_prio_t priority) {
...
/*
 * The real soft and hard real-time maps can be dynamically adjusted by command
 * line options. For testing purposes, hardcode the default map here.
 */
void initRealtimePrioMap(void) {
  intptr_t i;
  int prio = 0;
  ...
  /* should be the shift value when -Xthr:realtimePriorityShift appears as a JVM
   * argument */
  ...
  initialize_thread_policies(&policy_regular_thread, &policy_realtime_thread);
  {
     ...
     * so the regular priorities and policies will be based on the current
     * thread's priority and policy
     */
    if (getCurrentThreadParams(&prio, &policy_regular_thread)) {
      /* failed to get the current params */
      omrTestEnv->log(
          LEVEL_ERROR,
          "Could not get current thread params, assuming default\n");
      prio = 0;
  ...
  for (i = 12; i <= VM_PRIO_MAX; i++) {
    VMToOSPrioMap[i] = ((policy_realtime_thread == SCHED_OTHER)
                            ? 0
                            : (i + XthrRealtimeShiftPriosValue));
  }
...
void initPrioMap(void) {
  const osprio_t tmpMap[VM_PRIO_MAX + 1] = {54, 55, 55, 56, 56, 57,
                                            57, 58, 58, 59, 59, 60};
...
void initPrioMap(void) {
  const osprio_t tmpMap[VM_PRIO_MAX + 1] = {40, 41, 43, 45, 47, 49,
                                            51, 53, 55, 57, 59, 60};
