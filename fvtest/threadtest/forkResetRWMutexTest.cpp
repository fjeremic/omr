...
static intptr_t createJoinableThreadNoMacros(omrthread_t* newThread,
...
static void siblingWriteStart(int forkCount) {
  ...
  ASSERT_NO_FATAL_FAILURE(
      createJoinableThread(&t, siblingWriteMain, &testdata));
...
static void siblingWriteChild(testdata_rw_t* testdata, int pipedata[2]) {
  omrthread_lib_postForkChild();
...
static void parentWriteStart(int forkCount) {
  ...
  testdata.rc =
      omrthread_rwmutex_init(&testdata.rwmutex, 0, "OMR_TEST ParentWriteLock");
...
static void parentWriteChild(testdata_rw_t* testdata, int pipedata[2]) {
  omrthread_lib_postForkChild();
  testdata->rc = omrthread_rwmutex_try_enter_write(testdata->rwmutex);
  if (0 == testdata->rc) {
    testdata->rc = omrthread_rwmutex_exit_write(testdata->rwmutex);
...
static void childSiblingWriteChild(testdata_rw_t* testdata, int pipedata[2]) {
  omrthread_t t = NULL;
  omrthread_lib_postForkChild();
  testdata->rc =
      createJoinableThreadNoMacros(&t, childSiblingWriteMain, testdata);
  if (NULL != t) {
    testdata->rc = omrthread_join(t);
    if (testdata->rc & J9THREAD_ERR_OS_ERRNO_SET) {
      omrTestEnv->log(LEVEL_ERROR,
                      "omrthread_join() returned 0x%08x and os_errno=%d\n",
...
static void parentWriteSiblingReadStart(int forkCount) {
  ...
  omrthread_monitor_init(&testdata.monitor, 0);
  omrthread_rwmutex_init(&testdata.rwmutex, 0,
                         "OMR_TEST ParentWriteSiblingReadLock");
  omrthread_monitor_enter(testdata.monitor);
  omrthread_rwmutex_enter_write(testdata.rwmutex);
  ASSERT_NO_FATAL_FAILURE(
      createJoinableThread(&t, parentWriteSiblingReadMain, &testdata));
...
static void parentWriteSiblingReadChild(testdata_rw_t* testdata,
                                        int pipedata[2]) {
  omrthread_lib_postForkChild();
  testdata->rc = omrthread_rwmutex_exit_write(testdata->rwmutex);
  omrthread_sleep(START_DELAY);
  ...
  if (J9THREAD_SUCCESS == rc) {
    ...
    if (J9THREAD_SUCCESS != rc) {
      if (rc & J9THREAD_ERR_OS_ERRNO_SET) {
        omrTestEnv->log(
            LEVEL_ERROR,
            "omrthread_create_ex() returned 0x%08x and os_errno=%d\n", (int)rc,
