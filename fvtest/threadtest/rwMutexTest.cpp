...
runRequest(SupportThreadInfo* info)
{
  ...
  for (i = 0; i < info->numberFunctions; i++) {
    ...
    omrthread_monitor_wait_interruptable(
      info->synchronization, STEP_MILLI_TIMEOUT, STEP_NANO_TIMEOUT);
    omrthread_monitor_exit(info->synchronization);
  }
                        ...
                        uintptr_t numberFunctions)
{
  ...
  omrthread_rwmutex_init(
    (omrthread_rwmutex_t*)&info->handle, 0, "supportThreadInfo rwmutex");
  omrthread_monitor_init_with_name(
    &info->synchronization, 0, "supportThreadAInfo monitor");
  return info;
}
...
triggerNextStepWithStatus(SupportThreadInfo* info, BOOLEAN done)
{
  ...
  omrthread_monitor_notify(info->synchronization);
  omrthread_monitor_wait_interruptable(
    info->synchronization, MILLI_TIMEOUT, NANO_TIMEOUT);
  omrthread_monitor_exit(info->synchronization);
}
...
TEST(RWMutex, AllReadersProceedTest)
{
  ...
  omrthread_monitor_wait_interruptable(
    infoReader1->synchronization, MILLI_TIMEOUT, NANO_TIMEOUT);
  omrthread_monitor_exit(infoReader1->synchronization);
  omrthread_monitor_wait_interruptable(
    infoReader2->synchronization, MILLI_TIMEOUT, NANO_TIMEOUT);
...
TEST(RWMutex, RecursiveReadTest)
{
  ...
  for (i = 3; i > 0; i--) {
    ...
    if (i > 1) {
      omrthread_monitor_wait_interruptable(
        infoWriter1->synchronization, MILLI_TIMEOUT, NANO_TIMEOUT);
...
TEST(RWMutex, RecursiveWriteTest)
{
  ...
  for (i = 3; i > 0; i--) {
    ...
    if (i > 1) {
      omrthread_monitor_wait_interruptable(
        infoReader->synchronization, MILLI_TIMEOUT, NANO_TIMEOUT);
