 ...
 * @param info SupportThreadInfo structure that contains the information for the
 * steps and other parameters that are used
 */
static intptr_t J9THREAD_PROC runRequest(SupportThreadInfo *info) {
  intptr_t result = 0;
  uintptr_t i = 0;
...
/**
 * This method is called to create a SupportThreadInfo for a test.  It will
 * populate the monitor and rwmutex being used and zero out the counter
 *
 * @param functionsToRun an array of functions pointers. Each function will be
 * run one in sequence synchronized using the monitor within the
 * SupporThreadInfo
 ...
 */
SupportThreadInfo *createSupportThreadInfo(
    omrthread_entrypoint_t *functionsToRun, uintptr_t numberFunctions) {
  OMRPORT_ACCESS_FROM_OMRPORT(omrTestEnv->getPortLibrary());
  SupportThreadInfo *info = (SupportThreadInfo *)omrmem_allocate_memory(
  ...
  info->done = FALSE;
  omrthread_rwmutex_init((omrthread_rwmutex_t *)&info->handle, 0,
                         "supportThreadInfo rwmutex");
  omrthread_monitor_init_with_name(&info->synchronization, 0,
                                   "supportThreadAInfo monitor");
  return info;
}
 ...
 */
void triggerNextStepWithStatus(SupportThreadInfo *info, BOOLEAN done) {
  omrthread_monitor_enter(info->synchronization);
  info->done = done;
  omrthread_monitor_notify(info->synchronization);
  omrthread_monitor_wait_interruptable(info->synchronization, MILLI_TIMEOUT,
                                       NANO_TIMEOUT);
  omrthread_monitor_exit(info->synchronization);
}
 ...
 * This method is called to push the concurrent thread to run the next function
 */
void triggerNextStep(SupportThreadInfo *info) {
 ...
 */
void triggerNextStepDone(SupportThreadInfo *info) {
  triggerNextStepWithStatus(info, TRUE);
}

/**
 * This method starts a concurrent thread and runs the functions specified in
 * the SupportThreadInfo passed in.  It only returns once the first function has
 * run
 *
 * @param info SupporThreadInfo structure containing the functions and rwmutex
 * for the tests
 * @returns 0 on success
 */
intptr_t startConcurrentThread(SupportThreadInfo *info) {
 ...
 * @param info the SupporThreadInfo which can be used by the step
 */
static intptr_t J9THREAD_PROC nop(SupportThreadInfo *info) { return 0; }
 ...
 * validates the following
 *
 * readers are excludes while another thread holds the rwmutex for write entered
 * using try_enter once writer exits, reader can enter
 */
...
TEST(RWMutex, SecondReaderExcludesWrite) {
  ...
  /* now release the rwmutex and validate that the second readers still excludes
   * the writer */
  omrthread_monitor_enter(info->synchronization);
...
TEST(RWMutex, AllReadersProceedTest) {
  ...
  omrthread_monitor_wait_interruptable(infoReader1->synchronization,
 ...
 * as a thread exits a rwmutex for read any threads trying to enter remain
 * blocked the same number of exits as enters have been done a thread waiting to
 * enter a rwmutex wakes up and enter when the last exit for a series of
 * recusive enters is called
 */
TEST(RWMutex, RecursiveReadTest) {
  ...
  for (i = 3; i > 0; i--) {
    ...
    if (i > 1) {
      omrthread_monitor_wait_interruptable(infoWriter1->synchronization,
                                           MILLI_TIMEOUT, NANO_TIMEOUT);
      omrthread_monitor_exit(infoWriter1->synchronization);
      /* cannot validate this on RT as although the reader has exited the mutex
       * it will not proceed until the writer has exited
       */
      ASSERT_TRUE((uintptr_t)(i - 1) == infoReader1->readCounter);
      ASSERT_TRUE(0 == infoWriter1->writeCounter);
    } else {
...
TEST(RWMutex, RecursiveWriteTest) {
  ...
  for (i = 3; i > 0; i--) {
    ...
    /* make sure waiter is still waiting*/
    if (i > 1) {
      omrthread_monitor_wait_interruptable(infoReader->synchronization,
                                           MILLI_TIMEOUT, NANO_TIMEOUT);
 ...
 * one of the enters threads waiting to enter for read wake up and enter when
 * the last exit for read occurs when try_enter was used for one of the enters
 */
