...
static void createThread(omrthread_t *newThread, uintptr_t suspend,
...
TEST(JoinTest, createDetachedThread) {
  ...
  /* We can't pass any local data to the child thread because we don't guarantee
   * that it won't go out of scope before the child thread uses it.
   */
  ASSERT_NO_FATAL_FAILURE(createThread(
      &helperThr, FALSE, J9THREAD_CREATE_DETACHED, doNothingHelper, NULL));
}
...
TEST(JoinTest, createJoinableThread) {
  ...
  omrthread_t helperThr = NULL;
  ASSERT_NO_FATAL_FAILURE(createThread(&helperThr, FALSE,
                                       J9THREAD_CREATE_JOINABLE,
                                       joinThreadHelper, (void *)&helperData));
...
TEST(JoinTest, joinExitedThread) {
  ...
  ASSERT_NO_FATAL_FAILURE(createThread(
      &helperThr, FALSE, J9THREAD_CREATE_JOINABLE, doNothingHelper, NULL));
...
TEST(JoinTest, joinLiveThread) {
  ...
  ASSERT_NO_FATAL_FAILURE(createThread(
      &suspendedThr, TRUE, J9THREAD_CREATE_JOINABLE, doNothingHelper, NULL));
  ...
  ASSERT_NO_FATAL_FAILURE(createThread(&joinHelperThr, FALSE,
                                       J9THREAD_CREATE_JOINABLE,
                                       joinThreadHelper, &helperData));
...
TEST(JoinTest, joinCanceledThread) {
  ...
  ASSERT_NO_FATAL_FAILURE(createThread(
      &threadToJoin, TRUE, J9THREAD_CREATE_JOINABLE, doNothingHelper, NULL));
...
static void createThread(omrthread_t *newThread, uintptr_t suspend,
  ...
  ASSERT_EQ(J9THREAD_SUCCESS,
            omrthread_attr_set_detachstate(&attr, detachstate));
  EXPECT_EQ(
      J9THREAD_SUCCESS,
