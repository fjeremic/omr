...
#endif /* defined(J9ZOS390) || defined(LINUX) || defined(AIXPPC) || \
          defined(OSX) */
...
intptr_t omrthread_verboseCall(const char *func, intptr_t retVal) {
  if (retVal != J9THREAD_SUCCESS) {
    ...
    if (retVal == J9THREAD_ERR_UNSUPPORTED_ATTR) {
      if (errnoSet) {
#if defined(J9ZOS390)
        omrTestEnv->log(LEVEL_ERROR,
                        "%s unsupported: retVal %zd (%zx) : errno %zd (%zx) "
                        "%s, errno2 %zd (%zx)\n",
                        func, retVal, retVal, os_errno, os_errno,
                        strerror((int)os_errno), os_errno2, os_errno2);
#else  /* !J9ZOS390 */
        omrTestEnv->log(
            LEVEL_ERROR, "%s unsupported: retVal %zd (%zx) : errno %zd %s\n",
...
#if defined(J9ZOS390)
        omrTestEnv->log(LEVEL_ERROR,
                        "%s failed: retVal %zd (%zx) : errno %zd (%zx) %s, "
                        "errno2 %zd (%zx)\n",
                        func, retVal, retVal, os_errno, os_errno,
                        strerror((int)os_errno), os_errno2, os_errno2);
#else  /* !J9ZOS390 */
        omrTestEnv->log(LEVEL_ERROR,
                        "%s failed: retVal %zd (%zx) : errno %zd %s\n", func,
                        retVal, retVal, os_errno, strerror((int)os_errno));
#endif /* !J9ZOS390 */
      } else {
...
intptr_t pthread_verboseCall(const char *func, intptr_t retVal) {
  if (retVal != 0) {
#ifdef J9ZOS390
  ...
  static void SetUpTestCase() {
    ...
    if (omrTestEnv->realtime) {
      omrthread_lib_control(
          J9THREAD_LIB_CONTROL_USE_REALTIME_SCHEDULING,
          J9THREAD_LIB_CONTROL_USE_REALTIME_SCHEDULING_ENABLED);
    }
...
static uintptr_t canCreateThread(const create_attr_t *expected,
                                 const omrthread_attr_t attr);
static uintptr_t isAttrOk(const create_attr_t *expected,
                          const omrthread_attr_t actual);
static void initDefaultExpected(create_attr_t *expected);
static size_t getOsStacksize(size_t stacksize);
static int getOsPolicy(omrthread_schedpolicy_t policy,
                       omrthread_prio_t priority);
...
static void printMismatchU(const char *property, uintptr_t expected,
                           uintptr_t actual) {
  omrTestEnv->log(LEVEL_ERROR, "wrong %s: expected=%d, actual=%d\n", property,
                  expected, actual);
}

static void printMismatchI(const char *property, intptr_t expected,
                           intptr_t actual) {
  omrTestEnv->log(LEVEL_ERROR, "wrong %s: expected=%d, actual=%d\n", property,
                  expected, actual);
}

static void printMismatchS(const char *property, const char *expected,
                           const char *actual) {
  omrTestEnv->log(LEVEL_ERROR, "wrong %s: expected=%s, actual=%s\n", property,
                  expected, actual);
}

static const char *mapOSInherit(intptr_t policy) {
  switch (policy) {
...
/*
 * The usage of "expected" is not threadsafe. Ensure that expected doesn't go
 * out of scope while the child thread is running, and that the parent waits for
 * the child thread to complete before modifying expected.
 */
...
static int threadmain(void *arg) {
  ...
  if (!omrthread_lib_use_realtime_scheduling() &&
    ...
    if (osPolicy != data->curPolicy) {
      printMismatchS("child thread policy", mapOSPolicy(data->curPolicy),
                     mapOSPolicy(osPolicy));
    ...
    if (osPolicy != expected->osPolicy) {
      printMismatchS("child thread policy", mapOSPolicy(expected->osPolicy),
                     mapOSPolicy(osPolicy));
#if defined(LINUXPPC)
      ...
      if (!((osPolicy == OS_SCHED_OTHER) &&
        ...
        omrTestEnv->log(LEVEL_ERROR,
                        "  LINUXPPC: ignoring wrong schedpolicy\n");
      }
#else
      data->status |= WRONG_OS_SCHEDPOLICY;
#endif
...
static uintptr_t canCreateThread(const create_attr_t *expected,
  ...
  rc = J9THREAD_VERBOSE(omrthread_create_ex(
      &handle, (attr ? &attr : J9THREAD_ATTR_DEFAULT), 1, threadmain, &data));
...
#if defined(LINUX)
    /* bug? Linux can return a success code, but still fail to create the
     * thread. NOTE: It is not guaranteed that tid 0 is invalid in all pthread
     * implementations.
     */
    if (0 == tid) {
...
#if defined(OSX)
    /* OSX TODO: Why do the tests segfault in child thread when accessing &data
     * on OSX without a 1ms sleep? */
    omrthread_sleep(10);
#endif /* defined(OSX) */
...
#else
    /* In this case,
     * threadmain() doesn't access the expected data, so it's ok to continue
...
static uintptr_t isAttrOk(const create_attr_t *expected,
...
#if defined(SPEC_PTHREAD_API)
  {
...
#if (defined(AIXPPC) || defined(LINUX) || defined(OSX))
    /* aix and linux bug - need to set the schedpolicy to OTHER if inheritsched
     * used */
    if (J9THREAD_SCHEDPOLICY_INHERIT == expected->policy) {
      if (osPolicy != OS_SCHED_OTHER) {
        printMismatchS("os schedpolicy", mapOSPolicy(OS_SCHED_OTHER),
                       mapOSPolicy(osPolicy));
    ...
    {
      if (osPolicy != expected->osPolicy) {
        printMismatchS("os schedpolicy", mapOSPolicy(expected->osPolicy),
                       mapOSPolicy(osPolicy));
        status |= WRONG_OS_SCHEDPOLICY;
      }
    }
    ...
    if (osSchedparam.sched_priority != expected->osPriority) {
      printMismatchI("os priority", expected->osPriority,
                     osSchedparam.sched_priority);
    ...
    if (osThreadweight != expected->osThreadweight) {
      printMismatchI("os threadweight", expected->osThreadweight,
                     osThreadweight);
...
TEST_F(ThreadCreateTest, SetAttrStacksize) {
  ...
  rc =
      J9THREAD_VERBOSE(omrthread_attr_set_stacksize(&attr, STACK_DEFAULT_SIZE));
  if (rc != J9THREAD_SUCCESS) {
    status |= EXPECTED_VALID;
  ...
  if (!INVALID_VALUE(
          J9THREAD_VERBOSE(omrthread_attr_set_stacksize(&attr, 0x6000)))) {
  ...
  if (!INVALID_VALUE(
          J9THREAD_VERBOSE(omrthread_attr_set_stacksize(&attr, 0x7000)))) {
  ...
  if (!INVALID_VALUE(
          J9THREAD_VERBOSE(omrthread_attr_set_stacksize(&attr, 0x8000)))) {
...
TEST_F(ThreadCreateTest, SetAttrPolicy) {
   ...
   * thread priority isn't set properly. Our VM->OS mapping converts all
   * priorities to 0 because some versions of Linux support only priority 0 for
   * SCHED_OTHER.
  ...
  if (INVALID_VALUE(J9THREAD_VERBOSE(
          omrthread_attr_set_schedpolicy(&attr, J9THREAD_SCHEDPOLICY_RR)))) {
    status |= EXPECTED_VALID;
  }
  status |= isAttrOk(&expected, attr);
  ...
  if (INVALID_VALUE(J9THREAD_VERBOSE(
          omrthread_attr_set_schedpolicy(&attr, J9THREAD_SCHEDPOLICY_FIFO)))) {
    status |= EXPECTED_VALID;
  }
  status |= isAttrOk(&expected, attr);
#if (defined(LINUX) || defined(AIXPPC))
  ...
  if (INVALID_VALUE(J9THREAD_VERBOSE(omrthread_attr_set_schedpolicy(
          &attr, J9THREAD_SCHEDPOLICY_INHERIT)))) {
    status |= EXPECTED_VALID;
  }
  status |= isAttrOk(&expected, attr);
  END_IF_FAILED(status);
...
TEST_F(ThreadCreateTest, SetAttrBadPolicy) {
  ...
  /* invalid value */
  if (!INVALID_VALUE(J9THREAD_VERBOSE(omrthread_attr_set_schedpolicy(
          &attr, omrthread_schedpolicy_LastEnum)))) {
...
TEST_F(ThreadCreateTest, SetAttrPriority) {
  ...
  /* set the schedpolicy to explicit so that priorities are used by thread
   * creation */
  expected.policy = J9THREAD_SCHEDPOLICY_OTHER;
  expected.osPolicy = getOsPolicy(expected.policy, expected.priority);
  expected.osInheritsched = OS_PTHREAD_EXPLICIT_SCHED;
...
TEST_F(ThreadCreateTest, SetAttrBadPriority) {
  ...
  if (!INVALID_VALUE(
          J9THREAD_VERBOSE(omrthread_attr_set_priority(&attr, -1)))) {
    status |= EXPECTED_INVALID;
  }
  status |= isAttrOk(&expected, attr);
  END_IF_FAILED(status);
  ...
  if (!INVALID_VALUE(
          J9THREAD_VERBOSE(omrthread_attr_set_priority(&attr, 90)))) {
    status |= EXPECTED_INVALID;
  }
  status |= isAttrOk(&expected, attr);
  END_IF_FAILED(status);
  ...
  if (!INVALID_VALUE(
          J9THREAD_VERBOSE(omrthread_attr_set_priority(&attr, 12)))) {
    status |= EXPECTED_INVALID;
  }
  status |= isAttrOk(&expected, attr);
  END_IF_FAILED(status);
...
static int numaSetAffinityThreadMain(void *arg) {
  ...
  if (maxNumaNode > 0) {
    ...
    if (0 != J9THREAD_VERBOSE(omrthread_numa_get_node_affinity(
                 omrthread_self(), &currentAffinity, &nodeCount))) {
      omrTestEnv->log(LEVEL_ERROR, "Failed to get thread affinity\n");
      status |= EXPECTED_VALID;
      goto endthread;
    }

    /* We used to check that a child thread inherits its parent's affinity. This
     * is Linux behaviour, but not AIX or Windows behaviour. We don't rely on,
     * specify or document this behaviour, so don't test it.
     */
#if 0
    ...
    while (numaNode > 0) {
      omrTestEnv->log(LEVEL_ERROR, "Setting thread numa affinity to %zu\n",
                      numaNode);
      result =
          omrthread_numa_set_node_affinity(omrthread_self(), &numaNode, 1, 0);

      if (result == J9THREAD_NUMA_ERR_NO_CPUS_FOR_NODE) {
        omrTestEnv->log(LEVEL_ERROR,
                        "Tried to set thread numa affinity to node %zu, but no "
                        "CPUs associated with node\n",
                        numaNode);
        numaNode--;
        continue;
      } else if (result != 0) {
    ...
    if (numaNode != currentAffinity) {
      omrTestEnv->log(LEVEL_ERROR,
                      "Affinity is incorrectly set. Expected:%zu Actual:%zu\n",
    ...
    currentAffinity = 0;
    if (0 != J9THREAD_VERBOSE(omrthread_numa_set_node_affinity(
                 omrthread_self(), &currentAffinity, 1, 0))) {
...
#define NODE_LIST_SIZE 4
  ...
  ASSERT_EQ((uintptr_t)0, numaMaxNode)
      << "nmaMaxNode not zero with numa disabled";
  omrthread_numa_set_enabled(true);
  numaMaxNode = omrthread_numa_get_max_node();
  ASSERT_EQ(originalMaxNode, numaMaxNode)
      << "Re-enabling max_node doesn't restore original value";
}

/* Since we can't read affinity on Win32, at least make sure that we can call
 * the setAffinity API without crashing. Note that this should be safe on all
 * platforms (either succeeds or fails but we ignore the return code since any
 * answer is correct, so long as we don't crash - without reading the affinity,
 * we don't know of any more interesting tests).
 */
...
#define NODE_LIST_SIZE 4
  ...
  omrthread_numa_set_node_affinity(omrthread_self(), nodeList, NODE_LIST_SIZE,
                                   0);
}
...
TEST_F(ThreadCreateTest, NumaSetAffinity) {
  ...
  nodeCount = 1;
  affinityResultCode = omrthread_numa_get_node_affinity(
      omrthread_self(), &data.expectedAffinity, &nodeCount);
  if (J9THREAD_NUMA_ERR_AFFINITY_NOT_SUPPORTED == affinityResultCode) {
    /* this platform can't meaningfully run this test so just end */
    omrTestEnv->log(
        LEVEL_ERROR,
        "NUMA-level thread affinity not supported on this platform\n");
    goto endtest;
  }
  ...
  if (J9THREAD_SUCCESS !=
      J9THREAD_VERBOSE(omrthread_create_ex(&thread, J9THREAD_ATTR_DEFAULT, 0,
                                           numaSetAffinityThreadMain, &data))) {
...
static int numaSetAffinitySuspendedThreadMain(void *arg) {
  ...
  if (currentAffinity != data->expectedAffinity) {
    omrTestEnv->log(
        LEVEL_ERROR,
        "Affinity different from expected. Expected:%zu Actual:%zu\n",
        data->expectedAffinity, currentAffinity);
    status |= EXPECTED_VALID;
    goto endthread;
  }
...
TEST_F(ThreadCreateTest, NumaSetAffinitySuspended) {
  ...
  if (numaMaxNode > 0) {
    ...
    intptr_t affinityResultCode = omrthread_numa_get_node_affinity(
        omrthread_self(), &data.expectedAffinity, &nodeCount);
    data.monitor = monitor;
    data.status = 0;
    ...
    if (J9THREAD_SUCCESS != J9THREAD_VERBOSE(omrthread_create_ex(
                                &thread, J9THREAD_ATTR_DEFAULT, 1,
    ...
    /* Check that the affinity on the suspended thread is indeed what we set it
     * to */
    if (0 != J9THREAD_VERBOSE(omrthread_numa_get_node_affinity(
                 thread, &expectedAffinityBeforeStart, &nodeCount))) {
      omrTestEnv->log(
          LEVEL_ERROR,
          "Failed to get affinity on the thread while it's still suspended\n");
      status |= EXPECTED_VALID;
      goto endtest;
    }
    ...
    if (expectedAffinityBeforeStart != data.expectedAffinity) {
      omrTestEnv->log(LEVEL_ERROR,
                      "Suspended thread's deferred affinity is not what it "
                      "should be. Expected:%zu Actual:%zu\n",
...
/*
 * Using DISABLED_ to run this test separately, because the thread weight can
 * only be set once. Later changes to omrthread_global("thread_weight") are
 * ignored.
 */
