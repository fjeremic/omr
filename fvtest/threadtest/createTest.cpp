    ...
    if (retVal == J9THREAD_ERR_UNSUPPORTED_ATTR) {
      if (errnoSet) {
        ...
        omrTestEnv->log(LEVEL_ERROR, "%s unsupported: retVal %zd (%zx) : errno %zd (%zx) %s, errno2 %zd (%zx)\n", func,
          retVal, retVal, os_errno, os_errno, strerror((int)os_errno), os_errno2, os_errno2);
#else /* !J9ZOS390 */
        omrTestEnv->log(LEVEL_ERROR, "%s unsupported: retVal %zd (%zx) : errno %zd %s\n", func, retVal, retVal,
          os_errno, strerror((int)os_errno));
#endif /* !J9ZOS390 */
      } else {
  ...
  static void SetUpTestCase()
  {
    ...
    if (omrTestEnv->realtime) {
      omrthread_lib_control(
        J9THREAD_LIB_CONTROL_USE_REALTIME_SCHEDULING, J9THREAD_LIB_CONTROL_USE_REALTIME_SCHEDULING_ENABLED);
    }
...
static void printMismatchU(const char* property, uintptr_t expected, uintptr_t actual)
{
  omrTestEnv->log(LEVEL_ERROR, "wrong %s: expected=%d, actual=%d\n", property, expected, actual);
}

static void printMismatchI(const char* property, intptr_t expected, intptr_t actual)
{
  omrTestEnv->log(LEVEL_ERROR, "wrong %s: expected=%d, actual=%d\n", property, expected, actual);
}

static void printMismatchS(const char* property, const char* expected, const char* actual)
{
  ...
  if (maxNumaNode > 0) {
    ...
    while (numaNode > 0) {
      ...
      if (result == J9THREAD_NUMA_ERR_NO_CPUS_FOR_NODE) {
        omrTestEnv->log(
          LEVEL_ERROR, "Tried to set thread numa affinity to node %zu, but no CPUs associated with node\n", numaNode);
        numaNode--;
        continue;
      } else if (result != 0) {
...
TEST_F(ThreadCreateTest, NumaSetAffinitySuspended)
{
  ...
  if (numaMaxNode > 0) {
    ...
    if (J9THREAD_SUCCESS
      != J9THREAD_VERBOSE(
        omrthread_create_ex(&thread, J9THREAD_ATTR_DEFAULT, 1, numaSetAffinitySuspendedThreadMain, &data))) {
      status |= CREATE_FAILED;
      goto endtest;
    }
    ...
    if (expectedAffinityBeforeStart != data.expectedAffinity) {
      omrTestEnv->log(LEVEL_ERROR,
        "Suspended thread's deferred affinity is not what it should be. Expected:%zu Actual:%zu\n",
