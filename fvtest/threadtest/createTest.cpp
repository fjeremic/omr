  ...
  if (retVal != J9THREAD_SUCCESS) {
    ...
    if (retVal == J9THREAD_ERR_UNSUPPORTED_ATTR) {
...
#else  /* !J9ZOS390 */
        omrTestEnv->log(LEVEL_ERROR,
                        "%s unsupported: retVal %zd (%zx) : errno %zd %s\n",
                        func,
                        retVal,
                        retVal,
                        os_errno,
      ...
      if (errnoSet) {
        ...
        omrTestEnv->log(
          LEVEL_ERROR, "%s failed: retVal %zd (%zx)\n", func, retVal, retVal);
      }
    }
  } else {
    omrTestEnv->log("%s\n", func);
...
threadmain(void* arg)
{
  ...
  if (!omrthread_lib_use_realtime_scheduling() &&
    ...
    if (osPolicy != data->curPolicy) {
      printMismatchS("child thread policy",
                     mapOSPolicy(data->curPolicy),
...
isAttrOk(const create_attr_t* expected, const omrthread_attr_t actual)
{
...
#if defined(SPEC_PTHREAD_API)
  {
    ...
    if (J9THREAD_SCHEDPOLICY_INHERIT == expected->policy) {
      if (osPolicy != OS_SCHED_OTHER) {
        printMismatchS(
          "os schedpolicy", mapOSPolicy(OS_SCHED_OTHER), mapOSPolicy(osPolicy));
    ...
    if (osInheritsched != expected->osInheritsched) {
      printMismatchS("os inheritsched",
                     mapOSInherit(expected->osInheritsched),
    ...
    if (osSchedparam.sched_priority != expected->osPriority) {
      printMismatchI(
        "os priority", expected->osPriority, osSchedparam.sched_priority);
    ...
    if (osThreadweight != expected->osThreadweight) {
      printMismatchI(
        "os threadweight", expected->osThreadweight, osThreadweight);
...
TEST_F(ThreadCreateTest, SetAttrStacksize)
{
  ...
  if (rc != J9THREAD_SUCCESS) {
    status |= EXPECTED_VALID;
    omrTestEnv->log(LEVEL_ERROR,
                    "failed to set STACK_DEFAULT_SIZE %x (%d)\n",
                    STACK_DEFAULT_SIZE,
                    STACK_DEFAULT_SIZE);
  }
  status |= isAttrOk(&expected, attr);
...
TEST_F(ThreadCreateTest, SetAttrPolicy)
{
  ...
  if (INVALID_VALUE(J9THREAD_VERBOSE(
        omrthread_attr_set_schedpolicy(&attr, J9THREAD_SCHEDPOLICY_INHERIT)))) {
    status |= EXPECTED_VALID;
  }
  status |= isAttrOk(&expected, attr);
  END_IF_FAILED(status);
...
numaSetAffinityThreadMain(void* arg)
{
  ...
  if (maxNumaNode > 0) {
    ...
    while (numaNode > 0) {
      omrTestEnv->log(
        LEVEL_ERROR, "Setting thread numa affinity to %zu\n", numaNode);
      result =
      ...
      if (result == J9THREAD_NUMA_ERR_NO_CPUS_FOR_NODE) {
        ...
        omrTestEnv->log(
          LEVEL_ERROR, "Failed to set affinity to %zu\n", numaNode);
        status |= EXPECTED_VALID;
        goto endthread;
      } else {
        break;
...
#define NODE_LIST_SIZE 4
  ...
  omrthread_numa_set_node_affinity(
    omrthread_self(), nodeList, NODE_LIST_SIZE, 0);
}
...
TEST_F(ThreadCreateTest, NumaSetAffinity)
{
  ...
  if (J9THREAD_SUCCESS !=
      J9THREAD_VERBOSE(omrthread_create_ex(
        &thread, J9THREAD_ATTR_DEFAULT, 0, numaSetAffinityThreadMain, &data))) {
...
TEST_F(ThreadCreateTest, NumaSetAffinitySuspended)
{
  ...
  if (numaMaxNode > 0) {
    ...
    if (J9THREAD_SUCCESS !=
        J9THREAD_VERBOSE(omrthread_create_ex(&thread,
                                             J9THREAD_ATTR_DEFAULT,
                                             1,
                                             numaSetAffinitySuspendedThreadMain,
                                             &data))) {
      status |= CREATE_FAILED;
      goto endtest;
    }
