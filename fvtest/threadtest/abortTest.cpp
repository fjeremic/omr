...
static intptr_t createDefaultThread(omrthread_t *t, functionPoint entrypoint,
                                    void *entryarg) {
...
TEST(ThreadAbortTest, Waiting) {
  ...
  EXPECT_TRUE(testdata.rc == J9THREAD_INTERRUPTED_MONITOR_ENTER)
      << "Failed to abort waiting thread!";
}
...
static int waitingMain(void *arg) {
  ...
  /* This test assumes that the thread will be Aborted.  If it is not there is a
   * timing hole which needs to be resolved.
   */
