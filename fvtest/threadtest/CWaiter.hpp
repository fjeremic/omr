...
public:
  CWaiter(CMonitor &monitor, bool interruptable)
      : m_monitor(monitor), m_interruptable(interruptable), m_timeToWait(-1),
        m_doneWaiting(false) {}

  CWaiter(CMonitor &monitor, bool interruptable, int64_t timeToWait)
      : m_monitor(monitor), m_interruptable(interruptable),
        m_timeToWait(timeToWait), m_doneWaiting(false) {}

  bool DoneWaiting() { return m_doneWaiting; }
  intptr_t GetWaitReturnValue() { return m_waitRetVal; }
  const char *GetWaitReturnValueAsString() const {
    const char *pRetVal;
  ...
  static const char *MapReturnValueToString(intptr_t retval) {
...
protected:
  virtual intptr_t Run() {
    ...
    if (m_timeToWait < 0) {
      m_waitRetVal =
          (m_interruptable) ? m_monitor.WaitInterruptable() : m_monitor.Wait();
    } else {
      m_waitRetVal = (m_interruptable)
