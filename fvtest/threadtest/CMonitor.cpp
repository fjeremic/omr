...
char const *CMonitor::GetName(void) const {
  return ((J9ThreadMonitor *)m_monitor)->name;
}

intptr_t CMonitor::Enter(void) { return omrthread_monitor_enter(m_monitor); }

intptr_t CMonitor::EnterUsingThreadId(CThread &self) {
  return omrthread_monitor_enter_using_threadId(m_monitor, self.GetId());
}

intptr_t CMonitor::TryEnter(void) {
  return omrthread_monitor_try_enter(m_monitor);
}

intptr_t CMonitor::TryEnterUsingThreadId(CThread &self) {
  return omrthread_monitor_try_enter_using_threadId(m_monitor, self.GetId());
}

intptr_t CMonitor::Exit(void) { return omrthread_monitor_exit(m_monitor); }

intptr_t CMonitor::ExitUsingThreadId(CThread &self) {
  return omrthread_monitor_exit_using_threadId(m_monitor, self.GetId());
}

intptr_t CMonitor::Wait(void) { return omrthread_monitor_wait(m_monitor); }

intptr_t CMonitor::Wait(int64_t millis) {
  return omrthread_monitor_wait_timed(m_monitor, millis, 0);
}

intptr_t CMonitor::Wait(int64_t millis, intptr_t nanos) {
  return omrthread_monitor_wait_timed(m_monitor, millis, nanos);
}

intptr_t CMonitor::WaitInterruptable(void) {
  return omrthread_monitor_wait_interruptable(m_monitor, 0, 0);
}

intptr_t CMonitor::WaitInterruptable(int64_t millis) {
  return omrthread_monitor_wait_interruptable(m_monitor, millis, 0);
}

intptr_t CMonitor::WaitInterruptable(int64_t millis, intptr_t nanos) {
  return omrthread_monitor_wait_interruptable(m_monitor, millis, nanos);
}

void CMonitor::NotifyAll(void) { omrthread_monitor_notify_all(m_monitor); }

void CMonitor::Notify(void) { omrthread_monitor_notify(m_monitor); }

omrthread_monitor_t CMonitor::GetMonitor(void) const { return m_monitor; }
 ...
 */
unsigned int CMonitor::numBlocking(void) {
...
bool CMonitor::isThreadBlocking(CThread &thread) {
  bool found = false;
...
void CRWMutex::EnterRead(void) { omrthread_rwmutex_enter_read(m_monitor); }

void CRWMutex::EnterWrite(void) { omrthread_rwmutex_enter_write(m_monitor); }
