...
bool TestNThreadsLooping(unsigned int numThreads, unsigned int sleepInterval, unsigned int runTime, bool keepCount)
{
 ...
 */
void SanityTestNThreads(unsigned int numThreads, unsigned int runTime)
{
  omrTestEnv->changeIndent(1);
...
void QuickNDirtyPerformanceTest(unsigned int runTime)
{
  omrTestEnv->changeIndent(1);
...
bool TestWaitNotify(unsigned int runTime)
{
  omrTestEnv->changeIndent(+1);
#if 0
	TestSimpleWaitNotify();
