...
#if defined(J9ZOS390) && !defined(OMR_EBCDIC)
/*  Gtest invokes xlocale, which has function definition for tolower and
 * toupper. This causes compilation issue since the a2e macros (tolower and
 * toupper) automatically replace the function definitions. So we explicitly
 * include <ctype.h> and undefine the macros for gtest, after gtest we then
 * define back the macros.
 */
#include <ctype.h>
#undef toupper
#undef tolower
...
public:
  ...
  OMREventListener(TestEventListener *theEventListener)
      : _eventListener(theEventListener), _showTestCases(true),
        _showTests(true), _showSuccess(true), _showFailures(true) {}
  ...
  static void setDefaultTestListener(bool showTestCases = true,
