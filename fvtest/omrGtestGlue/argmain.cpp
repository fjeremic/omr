...
/*
 * Perform platform-specific conversions on (argc, argv) before invoking the
 * generic test main function.
 *
 * This file needs to be included as source in the test executable's build. It
 * can't be built into a utility library because Windows does not link the wmain
 * function correctly when it is located in a library.
 */
...
int wmain(int argc, wchar_t** argv, wchar_t** envp) {
  ...
  translated_argv = (char**)malloc(
      length + ((argc + 1) * sizeof(char*))); /* + array entries */
  cursor = (char*)&translated_argv[argc + 1];
  for (i = 0; i < argc; i++) {
    int utf8Length;

    translated_argv[i] = cursor;
    if (0 == (utf8Length = WideCharToMultiByte(CP_UTF8, 0, argv[i], -1, cursor,
                                               length, NULL, NULL))) {
      return -1;
    }
    cursor += utf8Length;
  ...
  if (envp) {
    ...
    translated_envp = (char**)malloc(
        length + ((envc + 1) * sizeof(char*))); /* + array entries */
    cursor = (char*)&translated_envp[envc + 1];
    for (i = 0; i < envc; i++) {
      int utf8Length;
      translated_envp[i] = cursor;
      if (0 == (utf8Length = WideCharToMultiByte(CP_UTF8, 0, envp[i], -1,
                                                 cursor, length, NULL, NULL))) {
        return -1;
      }
      cursor += utf8Length;
      *cursor++ = '\0';
