...
static BOOLEAN avlTestInsert(OMRPortLibrary *portlib, J9AVLTree *tree,
                             uintptr_t val);
static intptr_t testInsertionComparator(J9AVLTree *tree,
                                        OMRAVLTestNode *insertNode,
                                        OMRAVLTestNode *walkNode);
static uintptr_t get_node_string(OMRPortLibrary *portlib, J9AVLTree *tree,
                                 OMRAVLTestNode *walk, char *buffer,
                                 uintptr_t bufSize);
static void avlSetup(J9AVLTree *tree);
static uintptr_t get_datum_string(OMRPortLibrary *portlib, OMRAVLTestNode *walk,
                                  char *buffer, uintptr_t bufSize);
static void avl_get_string(OMRPortLibrary *portlib, J9AVLTree *tree,
                           char *buffer, uintptr_t bufSize);
static intptr_t testSearchComparator(J9AVLTree *tree, intptr_t search,
                                     OMRAVLTestNode *walkNode);
static void freeAVLTree(OMRPortLibrary *portlib, J9AVLTreeNode *currentNode);

int32_t buildAndVerifyAVLTree(OMRPortLibrary *portLib, const char *success,
                              const char *testData) {
  J9AVLTree tree;
  char buffer[1024];
...
static void avlSetup(J9AVLTree *tree) {
  ...
  tree->insertionComparator =
      (intptr_t(*)(struct J9AVLTree *, J9AVLTreeNode *,
                   J9AVLTreeNode *))testInsertionComparator;
  tree->searchComparator = (intptr_t(*)(struct J9AVLTree *, uintptr_t,
                                        J9AVLTreeNode *))testSearchComparator;
}
...
static void avl_get_string(OMRPortLibrary *portlib, J9AVLTree *tree,
                           char *buffer, uintptr_t bufSize) {
  uintptr_t len = get_node_string(
      portlib, tree, (OMRAVLTestNode *)tree->rootNode, buffer, bufSize);
  if (len >= bufSize) {
    buffer[bufSize - 1] = 0;
  } else {
    buffer[len] = 0;
...
static uintptr_t get_node_string(OMRPortLibrary *portlib, J9AVLTree *tree,
  ...
  len += get_node_string(portlib, tree,
                         (OMRAVLTestNode *)AVL_SRP_GETNODE(walk->leftChild),
                         &buffer[len], bufSize - len);
  len += get_node_string(portlib, tree,
                         (OMRAVLTestNode *)AVL_SRP_GETNODE(walk->rightChild),
                         &buffer[len], bufSize - len);
