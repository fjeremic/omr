...
static intptr_t
hashComparatorFn(struct J9AVLTree* tree,
...
static BOOLEAN
checkHashtableIntegrity(OMRPortLibrary* portLib,
                        J9HashTable* table,
...
static int32_t
runHashtableTests(OMRPortLibrary* portLib,
                  J9HashTable* table,
                  const uintptr_t* data,
                  uintptr_t dataLength,
                  uintptr_t removeOffset)
{
  ...
  for (i = 0; i < dataLength; i++) {
    ...
    if (checkHashtableIntegrity(
          portLib, table, data, dataLength, removeOffset, i) == FALSE) {
      return -6;
    }
  }
...
buildAndVerifyHashtable(OMRPortLibrary* portLib, HashtableInputData* inputData)
{
  ...
  if (0 != runHashtableTests(
             portLib, table, inputData->data, inputData->dataLength, REVERSE)) {
    result = -2;
    goto fail;
  }
  for (i = 0; i < inputData->dataLength; i++) {
    if (0 != runHashtableTests(
               portLib, table, inputData->data, inputData->dataLength, i)) {
      result = -3;
      goto fail;
    }
  }
