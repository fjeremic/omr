...
#include "omrport.h"
/*
 * Testing the following functions of J9HashTable using the
 * J9HASH_TABLE_ALLOW_SIZE_OPTIMIZATION flag: hashTableAdd() hashTableGetCount()
 * 		hashTableFind()
 * 		hashTableStartDo()
 * 		hashTableNextDo()
 * 		hashTableRemove()
...
static uintptr_t dataOffset(uintptr_t startOffset, uintptr_t dataLength,
                            uintptr_t i);
...
static intptr_t hashComparatorFn(struct J9AVLTree *tree,
                                 struct J9AVLTreeNode *leftNode,
                                 struct J9AVLTreeNode *rightNode) {
...
static BOOLEAN checkHashtableIntegrity(OMRPortLibrary *portLib,
                                       J9HashTable *table,
  ...
  /* walk all the elements, checking for duplicates and ensuring the elements
   * are valid */
...
static int32_t runHashtableTests(OMRPortLibrary *portLib, J9HashTable *table,
                                 const uintptr_t *data, uintptr_t dataLength,
  ...
  if (checkHashtableIntegrity(portLib, table, data, dataLength, 0, -1) ==
      FALSE) {
    return -4;
  }
  ...
  for (i = 0; i < dataLength; i++) {
    ...
    if (checkHashtableIntegrity(portLib, table, data, dataLength, removeOffset,
                                i) == FALSE) {
      return -6;
    }
  }
...
static J9HashTable *allocateHashtable(OMRPortLibrary *portLib,
  ...
  if (TRUE == inputData->collisionResistant) {
    hashtable = collisionResilientHashTableNew(
        portLib, tableName, tableSize, entrySize, flags, OMRMEM_CATEGORY_VM,
        inputData->listToTreeThreshold, hashFn, hashComparatorFn, NULL,
        userData);
  } else {
    hashtable =
        hashTableNew(portLib, tableName, tableSize, entrySize, sizeof(char *),
                     flags | J9HASH_TABLE_ALLOW_SIZE_OPTIMIZATION,
                     OMRMEM_CATEGORY_VM, hashFn, hashEqualFn, NULL, userData);
  }
...
int32_t buildAndVerifyHashtable(OMRPortLibrary *portLib,
  ...
  if (0 != runHashtableTests(portLib, table, inputData->data,
                             inputData->dataLength, REVERSE)) {
    result = -2;
    goto fail;
  }
  for (i = 0; i < inputData->dataLength; i++) {
    if (0 != runHashtableTests(portLib, table, inputData->data,
                               inputData->dataLength, i)) {
      result = -3;
      goto fail;
    }
  }
