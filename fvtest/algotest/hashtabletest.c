...
static intptr_t hashComparatorFn(
  struct J9AVLTree* tree, struct J9AVLTreeNode* leftNode, struct J9AVLTreeNode* rightNode)
{
...
static BOOLEAN checkHashtableIntegrity(OMRPortLibrary* portLib, J9HashTable* table, const uintptr_t* data,
...
static int32_t runHashtableTests(
  OMRPortLibrary* portLib, J9HashTable* table, const uintptr_t* data, uintptr_t dataLength, uintptr_t removeOffset)
{
  uintptr_t i = 0;
  uintptr_t entry = 0;
...
static J9HashTable* allocateHashtable(OMRPortLibrary* portLib, HashtableInputData* inputData)
{
  ...
  if (TRUE == inputData->collisionResistant) {
    hashtable = collisionResilientHashTableNew(portLib, tableName, tableSize, entrySize, flags, OMRMEM_CATEGORY_VM,
      inputData->listToTreeThreshold, hashFn, hashComparatorFn, NULL, userData);
  } else {
    hashtable = hashTableNew(portLib, tableName, tableSize, entrySize, sizeof(char*),
      flags | J9HASH_TABLE_ALLOW_SIZE_OPTIMIZATION, OMRMEM_CATEGORY_VM, hashFn, hashEqualFn, NULL, userData);
  }
