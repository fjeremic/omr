...
static void testRegisterWithAgent(OMRPortLibrary* portLib, uintptr_t* passCount, uintptr_t* failCount,
  J9HookInterface** hookInterface, uintptr_t event, uintptr_t agentID, uintptr_t userData, uintptr_t expectedResult);
  ...
  J9HookInterface** hookInterface, uintptr_t event, uintptr_t expectedResult)
{
  ...
  if (((*hookInterface)->J9HookRegisterWithCallSite(hookInterface, event, hookNormalEvent, OMR_GET_CALLSITE(), NULL)
        == 0)
    == (expectedResult == 0)) {
    (*passCount)++;
  } else {
  ...
  J9HookInterface** hookInterface, uintptr_t event, uintptr_t agentID, uintptr_t userData, uintptr_t expectedResult)
{
  ...
  if (((*hookInterface)
          ->J9HookRegisterWithCallSite(
            hookInterface, event | J9HOOK_TAG_AGENT_ID, hookOrderedEvent, OMR_GET_CALLSITE(), (void*)userData, agentID)
        == 0)
    == (expectedResult == 0)) {
    (*passCount)++;
  } else {
...
static void testUnregister(
  OMRPortLibrary* portLib, uintptr_t* passCount, uintptr_t* failCount, J9HookInterface** hookInterface, uintptr_t event)
{
  ...
  OMRPortLibrary* portLib, uintptr_t* passCount, uintptr_t* failCount, uintptr_t event, uintptr_t expectedResult)
{
  ...
  if (count == expectedResult) {
    ...
    omrtty_printf(
      "Incorrect number of listeners responded for 0x%zx. Got %d, expected %d\n", event, count, expectedResult);
    (*failCount)++;
  }
}
