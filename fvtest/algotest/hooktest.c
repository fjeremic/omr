...
static int32_t
testHookInterface(OMRPortLibrary* portLib,
                  uintptr_t* passCount,
                  uintptr_t* failCount,
                  J9HookInterface** hookInterface);
static void
testEnabled(OMRPortLibrary* portLib,
            uintptr_t* passCount,
            uintptr_t* failCount,
            J9HookInterface** hookInterface,
            uintptr_t event,
...
static void
testDisable(OMRPortLibrary* portLib,
            uintptr_t* passCount,
            uintptr_t* failCount,
            J9HookInterface** hookInterface,
            uintptr_t event,
            uintptr_t expectedResult);
static void
testReserve(OMRPortLibrary* portLib,
            uintptr_t* passCount,
            uintptr_t* failCount,
            J9HookInterface** hookInterface,
            uintptr_t event,
            uintptr_t expectedResult);
static void
testRegister(OMRPortLibrary* portLib,
             uintptr_t* passCount,
             uintptr_t* failCount,
             J9HookInterface** hookInterface,
             uintptr_t event,
             uintptr_t expectedResult);
static void
testRegisterWithAgent(OMRPortLibrary* portLib,
                      uintptr_t* passCount,
                      uintptr_t* failCount,
                      J9HookInterface** hookInterface,
                      uintptr_t event,
                      ...
                      uintptr_t expectedResult);
static void
testUnregister(OMRPortLibrary* portLib,
               uintptr_t* passCount,
               uintptr_t* failCount,
               J9HookInterface** hookInterface,
               uintptr_t event);
static void
testUnregisterWithAgent(OMRPortLibrary* portLib,
                        uintptr_t* passCount,
                        uintptr_t* failCount,
                        J9HookInterface** hookInterface,
                        uintptr_t event,
                        uintptr_t userData);
static void
testDispatch(OMRPortLibrary* portLib,
             uintptr_t* passCount,
             uintptr_t* failCount,
             uintptr_t event,
             uintptr_t expectedResult);
static uintptr_t
testAllocateAgentID(OMRPortLibrary* portLib,
                    uintptr_t* passCount,
                    uintptr_t* failCount,
                    J9HookInterface** hookInterface);
static void
hookNormalEvent(J9HookInterface** hook,
                uintptr_t eventNum,
                void* voidEventData,
                void* userData);
static void
hookOrderedEvent(J9HookInterface** hook,
                 uintptr_t eventNum,
                 void* voidEventData,
                 void* userData);
...
int32_t
verifyHookable(OMRPortLibrary* portLib,
               uintptr_t* passCount,
               uintptr_t* failCount)
{
  int32_t rc = 0;
  ...
  if (J9HookInitializeInterface(
        hookInterface, portLib, sizeof(sampleHookInterface))) {
    (*failCount)++;
    rc = -1;
  } else {
    (*passCount)++;
                  ...
                  J9HookInterface** hookInterface)
{
  ...
  testEnabled(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT1, TRUE);
  testEnabled(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT2, TRUE);
  testEnabled(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT3, TRUE);
  testEnabled(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT4, TRUE);

  /* disable event1 */
  testDisable(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT1, 0);
  testReserve(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT1, -1);
  testEnabled(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT1, FALSE);
  testEnabled(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT2, TRUE);
  testEnabled(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT3, TRUE);
  ...
  testDisable(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT2, -1);
  testEnabled(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT1, FALSE);
  testEnabled(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT2, TRUE);
  ...
  /* register event3 */
  testRegister(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT3, 0);
  testDisable(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT3, -1);
  ...
  testRegister(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT2, 0);
  testRegister(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT2, 0);
  ...
  /* unregister an agent */
  testUnregisterWithAgent(
    portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT3, 2);
                      ...
                      uintptr_t expectedResult)
{
  ...
  if (((*hookInterface)
         ->J9HookRegisterWithCallSite(hookInterface,
                                      event | J9HOOK_TAG_AGENT_ID,
                                      hookOrderedEvent,
                                      OMR_GET_CALLSITE(),
                                      (void*)userData,
                                      agentID) == 0) == (expectedResult == 0)) {
    (*passCount)++;
  } else {
    omrtty_printf(
      "J9HookRegisterWithCallSite for 0x%zx %s. It should have %s.\n",
...
{
  (*hookInterface)
    ->J9HookUnregister(hookInterface, event, hookOrderedEvent, (void*)userData);
}
