...
static int32_t testHookInterface(OMRPortLibrary *portLib, uintptr_t *passCount,
                                 uintptr_t *failCount,
                                 J9HookInterface **hookInterface);
static void testEnabled(OMRPortLibrary *portLib, uintptr_t *passCount,
                        uintptr_t *failCount, J9HookInterface **hookInterface,
                        uintptr_t event, uintptr_t expectedResult);
static void testDisable(OMRPortLibrary *portLib, uintptr_t *passCount,
                        uintptr_t *failCount, J9HookInterface **hookInterface,
                        uintptr_t event, uintptr_t expectedResult);
static void testReserve(OMRPortLibrary *portLib, uintptr_t *passCount,
                        uintptr_t *failCount, J9HookInterface **hookInterface,
                        uintptr_t event, uintptr_t expectedResult);
static void testRegister(OMRPortLibrary *portLib, uintptr_t *passCount,
                         uintptr_t *failCount, J9HookInterface **hookInterface,
                         uintptr_t event, uintptr_t expectedResult);
static void testRegisterWithAgent(OMRPortLibrary *portLib, uintptr_t *passCount,
                                  uintptr_t *failCount,
                                  J9HookInterface **hookInterface,
                                  uintptr_t event, uintptr_t agentID,
                                  uintptr_t userData, uintptr_t expectedResult);
static void testUnregister(OMRPortLibrary *portLib, uintptr_t *passCount,
                           uintptr_t *failCount,
                           J9HookInterface **hookInterface, uintptr_t event);
static void testUnregisterWithAgent(OMRPortLibrary *portLib,
                                    uintptr_t *passCount, uintptr_t *failCount,
                                    J9HookInterface **hookInterface,
                                    uintptr_t event, uintptr_t userData);
static void testDispatch(OMRPortLibrary *portLib, uintptr_t *passCount,
                         uintptr_t *failCount, uintptr_t event,
                         uintptr_t expectedResult);
static uintptr_t testAllocateAgentID(OMRPortLibrary *portLib,
                                     uintptr_t *passCount, uintptr_t *failCount,
                                     J9HookInterface **hookInterface);
static void hookNormalEvent(J9HookInterface **hook, uintptr_t eventNum,
                            void *voidEventData, void *userData);
static void hookOrderedEvent(J9HookInterface **hook, uintptr_t eventNum,
                             void *voidEventData, void *userData);

static SampleHookInterface sampleHookInterface;

int32_t verifyHookable(OMRPortLibrary *portLib, uintptr_t *passCount,
                       uintptr_t *failCount) {
  int32_t rc = 0;
  ...
  if (J9HookInitializeInterface(hookInterface, portLib,
                                sizeof(sampleHookInterface))) {
    (*failCount)++;
    rc = -1;
  } else {
    (*passCount)++;
...
static int32_t testHookInterface(OMRPortLibrary *portLib, uintptr_t *passCount,
  ...
  testEnabled(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT1,
              TRUE);
  testEnabled(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT2,
              TRUE);
  testEnabled(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT3,
              TRUE);
  testEnabled(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT4,
              TRUE);

  /* disable event1 */
  testDisable(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT1, 0);
  testReserve(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT1,
              -1);
  testEnabled(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT1,
              FALSE);
  testEnabled(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT2,
  ...
  testDisable(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT2,
              -1);
  testEnabled(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT1,
              FALSE);
  testEnabled(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT2,
  ...
  /* register event3 */
  testRegister(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT3,
               0);
  testDisable(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT3,
              -1);
  ...
  testRegister(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT2,
               0);
  testRegister(portLib, passCount, failCount, hookInterface, TESTHOOK_EVENT2,
               0);
  ...
  agent2andAHalf =
      testAllocateAgentID(portLib, passCount, failCount, hookInterface);
  agent3 = testAllocateAgentID(portLib, passCount, failCount, hookInterface);

  /* register the agents in order */
  testRegisterWithAgent(portLib, passCount, failCount, hookInterface,
                        TESTHOOK_EVENT4, agent1, 1, 0);
  testRegisterWithAgent(portLib, passCount, failCount, hookInterface,
                        TESTHOOK_EVENT4, agent2, 2, 0);
  testRegisterWithAgent(portLib, passCount, failCount, hookInterface,
                        TESTHOOK_EVENT4, agent3, 3, 0);
  testDispatch(portLib, passCount, failCount, TESTHOOK_EVENT4, 3);

  /* register the agents in reverse order */
  testRegisterWithAgent(portLib, passCount, failCount, hookInterface,
                        TESTHOOK_EVENT3, agent3, 3, 0);
  testRegisterWithAgent(portLib, passCount, failCount, hookInterface,
                        TESTHOOK_EVENT3, agent2, 2, 0);
  testRegisterWithAgent(portLib, passCount, failCount, hookInterface,
                        TESTHOOK_EVENT3, agent1, 1, 0);
  testDispatch(portLib, passCount, failCount, TESTHOOK_EVENT3, 3);

  /* unregister an agent */
  testUnregisterWithAgent(portLib, passCount, failCount, hookInterface,
                          TESTHOOK_EVENT3, 2);
  testDispatch(portLib, passCount, failCount, TESTHOOK_EVENT3, 2);

  /* register it again with another agent in the same list position*/
  testRegisterWithAgent(portLib, passCount, failCount, hookInterface,
                        TESTHOOK_EVENT3, agent2andAHalf, 2, 0);
  testDispatch(portLib, passCount, failCount, TESTHOOK_EVENT3, 3);

  /* register a first chance istener */
  testRegisterWithAgent(portLib, passCount, failCount, hookInterface,
                        TESTHOOK_EVENT3, J9HOOK_AGENTID_FIRST, 0, 0);
  testDispatch(portLib, passCount, failCount, TESTHOOK_EVENT3, 4);

  /* and a last chance listener */
  testRegisterWithAgent(portLib, passCount, failCount, hookInterface,
                        TESTHOOK_EVENT3, J9HOOK_AGENTID_LAST, 4, 0);
  testDispatch(portLib, passCount, failCount, TESTHOOK_EVENT3, 5);

  /* registering a duplicate listener should have no effect, even if it's a
   * different agent */
  testRegisterWithAgent(portLib, passCount, failCount, hookInterface,
                        TESTHOOK_EVENT3, agent2, 3, 0);
  testDispatch(portLib, passCount, failCount, TESTHOOK_EVENT3, 5);
...
static void testEnabled(OMRPortLibrary *portLib, uintptr_t *passCount,
  ...
  if (((*hookInterface)->J9HookIsEnabled(hookInterface, event) == 0) ==
      (expectedResult == 0)) {
    (*passCount)++;
  } else {
    omrtty_printf("Hook 0x%zx is %s. It should be %s.\n", event,
                  (expectedResult ? "disabled" : "enabled"),
...
static void testDisable(OMRPortLibrary *portLib, uintptr_t *passCount,
  ...
  if (((*hookInterface)->J9HookDisable(hookInterface, event) == 0) ==
      (expectedResult == 0)) {
    (*passCount)++;
  } else {
...
static void testReserve(OMRPortLibrary *portLib, uintptr_t *passCount,
  ...
  if (((*hookInterface)->J9HookReserve(hookInterface, event) == 0) ==
      (expectedResult == 0)) {
    (*passCount)++;
  } else {
...
static void testRegister(OMRPortLibrary *portLib, uintptr_t *passCount,
  ...
  if (((*hookInterface)
           ->J9HookRegisterWithCallSite(hookInterface, event, hookNormalEvent,
                                        OMR_GET_CALLSITE(), NULL) == 0) ==
      (expectedResult == 0)) {
    (*passCount)++;
  } else {
    omrtty_printf(
        "J9HookRegisterWithCallSite for 0x%zx %s. It should have %s.\n", event,
        (expectedResult ? "succeeded" : "failed"),
        (expectedResult ? "failed" : "succeeded"));
    (*failCount)++;
  }
...
static void testRegisterWithAgent(OMRPortLibrary *portLib, uintptr_t *passCount,
  ...
  if (((*hookInterface)
           ->J9HookRegisterWithCallSite(
               hookInterface, event | J9HOOK_TAG_AGENT_ID, hookOrderedEvent,
               OMR_GET_CALLSITE(), (void *)userData, agentID) == 0) ==
      (expectedResult == 0)) {
    (*passCount)++;
  } else {
    omrtty_printf(
...
static void testUnregister(OMRPortLibrary *portLib, uintptr_t *passCount,
                           ...
                           J9HookInterface **hookInterface, uintptr_t event) {
  (*hookInterface)
      ->J9HookUnregister(hookInterface, event, hookNormalEvent, NULL);
}
...
static void testUnregisterWithAgent(OMRPortLibrary *portLib,
                                    ...
                                    uintptr_t event, uintptr_t userData) {
  (*hookInterface)
      ->J9HookUnregister(hookInterface, event, hookOrderedEvent,
                         (void *)userData);
}
...
static void testDispatch(OMRPortLibrary *portLib, uintptr_t *passCount,
  ...
  if (count == expectedResult) {
    ...
    omrtty_printf("Incorrect number of listeners responded for 0x%zx. Got %d, "
                  "expected %d\n",
