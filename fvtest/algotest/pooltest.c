...
static int32_t testPoolNewElement(OMRPortLibrary *portLib,
                                  PoolInputData *inputData,
                                  J9Pool *currentPool);
static int32_t testPoolWalkFunctions(OMRPortLibrary *portLib,
                                     PoolInputData *inputData,
                                     J9Pool *currentPool,
                                     uint32_t elementsAllocated);
static int32_t testPoolClear(OMRPortLibrary *portLib, J9Pool *currentPool);
static int32_t testPoolRemoveElement(OMRPortLibrary *portLib,
                                     J9Pool *currentPool);

static void *sharedPuddleListAlloc(OMRPortLibrary *portLib, uint32_t size,
                                   const char *callSite,
                                   uint32_t memoryCategory, uint32_t type,
                                   uint32_t *doInit);
static void sharedPuddleListFree(OMRPortLibrary *portLib, void *address,
                                 uint32_t type);
  ...
  if (type == POOL_ALLOC_TYPE_PUDDLE_LIST) {
    *doInit = 0;
    if (sharedPuddleList == NULL) {
      sharedPuddleList = portLib->mem_allocate_memory(portLib, size, callSite,
                                                      OMRMEM_CATEGORY_VM);
      *doInit = 1;
    }
    if (sharedPuddleList != NULL) {
    ...
    return portLib->mem_allocate_memory(portLib, size, callSite,
                                        OMRMEM_CATEGORY_VM);
  }
}
...
static J9Pool *createNewPool(OMRPortLibrary *portLib, PoolInputData *input) {
  uint32_t expectedNumElems =
      (input->numberElements == 0) ? 1 : input->numberElements;
  uint32_t expectedAlignment = (input->elementAlignment == 0)
                                   ? MIN_GRANULARITY
                                   : input->elementAlignment;

  J9Pool *pool =
      pool_new(input->structSize, input->numberElements,
               input->elementAlignment, input->poolFlags, OMR_GET_CALLSITE(),
               OMRMEM_CATEGORY_VM, POOL_FOR_PORT(portLib));
  ...
  if (pool->puddleAllocSize <
      ((expectedNumElems * ROUND_TO(expectedAlignment, input->structSize)) +
       ROUND_TO(expectedAlignment, sizeof(J9PoolPuddle)))) {
    pool_kill(pool);
    return NULL;
  } else if ((input->poolFlags & POOL_ROUND_TO_PAGE_SIZE) &&
             (pool->puddleAllocSize < OS_PAGE_SIZE)) {
    pool_kill(pool);
    return NULL;
  }
  return pool;
...
static int32_t testPoolNewElement(OMRPortLibrary *portLib,
  ...
  uint32_t expectedNumElems =
      (inputData->numberElements == 0) ? 1 : inputData->numberElements;
  uint32_t expectedAlignment = (inputData->elementAlignment == 0)
                                   ? MIN_GRANULARITY
                                   : inputData->elementAlignment;

  J9PoolPuddleList *puddleList = J9POOL_PUDDLELIST(currentPool);
  J9PoolPuddle *initialPuddle =
      J9POOLPUDDLELIST_NEXTAVAILABLEPUDDLE(puddleList);
  J9PoolPuddle *currentPuddle = initialPuddle;

  /* Call pool_newElement until a new puddle is allocated... and then allocate a
   * couple of new elements into the new puddle */
  while (initialPuddle == currentPuddle) {
    uint32_t j = 0;
    /* Get a single element and check that it's the right size and that each
     * byte can be written to */
    element = pool_newElement(currentPool);
    if (NULL == element) {
      return -1;
    }

    /* Verify element alignment. Aligned elements are expected for use in AVL
     * trees, for example. */
    ...
    if (inputData->poolFlags & POOL_NO_ZERO) {
      memset(element, 0,
             inputData
                 ->structSize); /* Expect a crash if the element is too small */
    }
    ...
    /* currentPuddle will become NULL when the puddle being used becomes FULL if
...
static int32_t testPoolWalkFunctions(OMRPortLibrary *portLib,
  ...
  do {
    ...
    if ((inputData->structSize > 1) &&
        (element[inputData->structSize - 1] != LAST_BYTE_MARKER)) {
      return -2;
    }
...
static int32_t testPoolRemoveElement(OMRPortLibrary *portLib,
  ...
  do {
    ...
    do {
      ...
      /* Remove every other element each time - this deliberately creates
       * fragmentation */
      element = pool_nextDo(&state);
      if (NULL != element) {
        element = pool_nextDo(&state);
...
int32_t testPoolPuddleListSharing(OMRPortLibrary *portLib) {
  ...
  for (index = 0; index < NUM_POOLS_TO_SHARE_PUDDLE_LIST; index++) {
    pool[index] =
        pool_new(sizeof(U_64), 0, 0, 0, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_VM,
                 (omrmemAlloc_fptr_t)sharedPuddleListAlloc,
                 (omrmemFree_fptr_t)sharedPuddleListFree, portLib);
    ...
    /*
     * Since the underlying puddle list is shared, the number of elements in the
     * pool should be the same as the 1 + index of the pool we're on.
     */
