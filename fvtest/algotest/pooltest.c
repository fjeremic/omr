...
static int32_t
testPoolNewElement(OMRPortLibrary* portLib,
                   PoolInputData* inputData,
                   J9Pool* currentPool);
static int32_t
testPoolWalkFunctions(OMRPortLibrary* portLib,
                      PoolInputData* inputData,
                      J9Pool* currentPool,
...
static void*
sharedPuddleListAlloc(OMRPortLibrary* portLib,
                      uint32_t size,
                      const char* callSite,
  ...
  if (type == POOL_ALLOC_TYPE_PUDDLE_LIST) {
    *doInit = 0;
    if (sharedPuddleList == NULL) {
      sharedPuddleList = portLib->mem_allocate_memory(
        portLib, size, callSite, OMRMEM_CATEGORY_VM);
      *doInit = 1;
    }
    if (sharedPuddleList != NULL) {
    ...
    return portLib->mem_allocate_memory(
      portLib, size, callSite, OMRMEM_CATEGORY_VM);
  }
}
...
createNewPool(OMRPortLibrary* portLib, PoolInputData* input)
{
  ...
  uint32_t expectedAlignment =
    (input->elementAlignment == 0) ? MIN_GRANULARITY : input->elementAlignment;

  J9Pool* pool = pool_new(input->structSize,
  ...
  while (initialPuddle == currentPuddle) {
    ...
    if (inputData->poolFlags & POOL_NO_ZERO) {
      memset(
        element,
        0,
        inputData->structSize); /* Expect a crash if the element is too small */
    }
...
testPoolPuddleListSharing(OMRPortLibrary* portLib)
{
  ...
  for (index = 0; index < NUM_POOLS_TO_SHARE_PUDDLE_LIST; index++) {
    pool[index] = pool_new(sizeof(U_64),
                           0,
                           0,
                           0,
                           OMR_GET_CALLSITE(),
