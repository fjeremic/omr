...
class SingleNodeWithName
    : public testing::TestWithParam<TestParamWithExpectedResultString> {};
...
TEST_P(SingleNodeWithName, VerifyNode) {
  ...
  ASSERT_EQ(reinterpret_cast<const ASTNodeArg *>(NULL), trees->getArgs());
  ASSERT_EQ(reinterpret_cast<const ASTNode *>(NULL), trees->getChildren());
  ASSERT_EQ(reinterpret_cast<ASTNode *>(NULL), trees->next);
}

INSTANTIATE_TEST_CASE_P(
    ParserTest, SingleNodeWithName,
    testing::Values(TestParamWithExpectedResultString("(nodeName)", "nodeName"),
                    TestParamWithExpectedResultString("(@commandName)",
                                                      "@commandName")));
...
TEST(ParserTest, NodeWithChildAndSibling) {
  ...
  ASSERT_EQ(reinterpret_cast<const ASTNodeArg *>(NULL), child->getArgs());
  ASSERT_EQ(reinterpret_cast<const ASTNode *>(NULL), child->getChildren());
  ASSERT_EQ(reinterpret_cast<ASTNode *>(NULL), child->next);

  auto sibling = trees->next;
  ASSERT_STREQ("siblingNode", sibling->getName());
  ASSERT_EQ(reinterpret_cast<const ASTNodeArg *>(NULL), sibling->getArgs());
  ASSERT_EQ(reinterpret_cast<const ASTNode *>(NULL), sibling->getChildren());
  ASSERT_EQ(reinterpret_cast<ASTNode *>(NULL), sibling->next);
}

typedef std::pair<const char *, ASTValue::Integer_t>
    TestParamWithExpectedResultInt;

class SingleNodeWithIntArg
    : public testing::TestWithParam<TestParamWithExpectedResultInt> {};
...
INSTANTIATE_TEST_CASE_P(
    ParserTestSingleNodeWithIntArgAsDecValue, SingleNodeWithIntArg,
    testing::Values(
        TestParamWithExpectedResultInt("(nodeName 0)", 0),
        TestParamWithExpectedResultInt("(nodeName 3)", 3),
        TestParamWithExpectedResultInt("(nodeName -3)", -3),
        TestParamWithExpectedResultInt("(nodeName 9223372036854775807)",
                                       9223372036854775807U),
        TestParamWithExpectedResultInt("(nodeName 18446744073709551615)",
                                       18446744073709551615ULL)));

INSTANTIATE_TEST_CASE_P(
    ParserTestSingleNodeWithIntArgAsHexValue, SingleNodeWithIntArg,
    testing::Values(
        TestParamWithExpectedResultInt("(nodeName 0xabc123)", 0xabc123),
        TestParamWithExpectedResultInt("(nodeName -0x249DEF)", -0x249DEF),
        TestParamWithExpectedResultInt("(nodeName 0xFFFFFFFF00000000)",
                                       0xFFFFFFFF00000000)));

typedef std::pair<const char *, ASTValue::FloatingPoint_t>
    TestParamWithExpectedResultFloat;

class SingleNodeWithFloatArg
    : public testing::TestWithParam<TestParamWithExpectedResultFloat> {};
...
INSTANTIATE_TEST_CASE_P(
    ParserTest, SingleNodeWithFloatArg,
    testing::Values(
        TestParamWithExpectedResultFloat("(nodeName 3.00)", 3.00),
        TestParamWithExpectedResultFloat("(nodeName -3.00)", -3.00),
        TestParamWithExpectedResultFloat("(nodeName 9223372036854775807.00)",
...
class SingleNodeWithUnnamedArg
    : public testing::TestWithParam<TestParamWithExpectedResultString> {};
...
INSTANTIATE_TEST_CASE_P(
    ParserTest, SingleNodeWithUnnamedArg,
    testing::Values(TestParamWithExpectedResultString("(nodeName id)", "id"),
                    TestParamWithExpectedResultString("(nodeName @cmd)",
                                                      "@cmd")));
...
TEST(ParserTest, SingleNodeWithNamedListArg) {
  ...
  ASSERT_EQ(reinterpret_cast<ASTValue *>(NULL), value->next);
}
