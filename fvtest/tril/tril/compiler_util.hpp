 ...
 */
static TR::DataTypes
getTRDataTypes(const std::string& name)
{
  if (name == "Int8")
    return TR::Int8;
  else if (name == "Int16")
    ...
    throw std::runtime_error(static_cast<const std::string&>(
      std::string("Unknown type name: ").append(name)));
  }
}
 ...
 */
static std::vector<TR::DataTypes>
parseArgTypes(const ASTNode* node)
{
 ...
 */
static OMR::flags32_t
parseFlags(const ASTNode* node)
{
 ...
 *         recognized.
 */
static TR_LinkageConventions
convertStringToLinkage(const char* linkageName)
{
