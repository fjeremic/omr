...
public:
   ...
   * @param methodNode is the AST node representing the Tril method to be
   * compiled
   */
  explicit MethodCompiler(const ASTNode *methodNode)
      : _method(methodNode), _entry_point(NULL) {}
  ...
  /**
   * @brief Returns the entry point to the compiled body, or null if not yet
   * compiled
   ...
   * compilation error.
   */
  template <typename T> T getEntryPoint() {
