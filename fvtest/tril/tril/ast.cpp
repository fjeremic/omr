...
ASTValue *createIntegerValue(uint64_t val) { return new ASTValue(val); }

ASTValue *createFloatingPointValue(double val) { return new ASTValue(val); }

ASTValue *createStrValue(const char *val) { return new ASTValue(val); }
...
void printTreesToStdErr(const ASTNode *trees) { printTrees(stderr, trees, 1); }
