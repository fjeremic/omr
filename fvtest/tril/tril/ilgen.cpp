...
/**
 * @brief A table to map the string representation of opcode names to
 * corresponding TR::OpCodes value
 */
class OpCodeTable : public TR::ILOpCode
{
public:
  explicit OpCodeTable(TR::ILOpCodes opcode)
    : TR::ILOpCode(opcode)
  {}
  explicit OpCodeTable(const std::string& name)
    : TR::ILOpCode(getOpCodeFromName(name))
  {}
 ...
 */
TR::Node*
Tril::TRLangBuilder::toTRNode(const ASTNode* const tree)
{
  TR::Node* node = NULL;
  ...
  if (strcmp("@id", tree->getName()) == 0) {
    ...
    TraceIL("WARNING: Using @common is deprecated. Please use (@id \"%s\") "
            "instead.\n",
            id);
    fprintf(stderr,
            "WARNING: Using @common is deprecated. Please use (@id \"%s\") "
            "instead.\n",
            id);
    auto iter = _nodeMap.find(id);
    if (iter != _nodeMap.end()) {
  ...
  if (opcode.isLoadConst()) {
    ...
    if (tree->getArgByName("parm") != NULL) {
      ...
      TraceIL("parameter %d\n", arg);
      auto symref = symRefTab()->findOrCreateAutoSymbol(
        _methodSymbol, arg, opcode.getType());
    ...
    // the name of the first argument tells us what kind of symref we're storing
    // to
    if (tree->getArgByName("temp") != NULL) {
      ...
      if (_symRefMap.find(symName) == _symRefMap.end()) {
        _symRefMap[symName] =
          symRefTab()->createTemporary(methodSymbol(), opcode.getDataType());
      }

      auto symref = _symRefMap[symName];
      node =
        TR::Node::createWithSymRef(opcode.getOpCodeValue(), childCount, symref);
    } else {
      // symref kind not recognized
      return NULL;
    }
    ...
    TR::Symbol* sym =
      TR::Symbol::createNamedShadow(compilation->trHeapMemory(),
                                    type,
                                    TR::DataType::getSize(opcode.getType()),
                                    (char*)name);
    TR::SymbolReference* symref =
      new (compilation->trHeapMemory()) TR::SymbolReference(
        compilation->getSymRefTab(),
        sym,
        compilation->getMethodSymbol()->getResolvedMethodIndex(),
        -1);
    ...
    const auto targetName =
      tree->getArgByName("target")->getValue()->getString();
    auto targetId = _blockMap[targetName];
    auto targetEntry = _blocks[targetId]->getEntry();
    TraceIL("  is if with target block %d (%s, entry = %p",
            targetId,
            targetName,
            targetEntry);
    ...
    auto targetId = _blockMap[targetName];
    auto targetEntry = _blocks[targetId]->getEntry();
    TraceIL("  is branch to target block %d (%s, entry = %p",
            targetId,
            targetName,
            targetEntry);
    node = TR::Node::create(opcode.getOpCodeValue(), childCount);
    ...
    /* I don't want to extend the ASTValue type system to include pointers at
     * this moment, so for now, we do the reinterpret_cast to pointer type from
     * long
     */
#if defined(OMR_ENV_DATA64)
    TraceIL("  is call with target 0x%016llX",
            addressArg->getValue()->get<uintptr_t>());
#else
    TraceIL("  is call with target 0x%08lX",
            addressArg->getValue()->get<uintptr_t>());
#endif /* OMR_ENV_DATA64 */
    const auto targetAddress =
      reinterpret_cast<void*>(addressArg->getValue()->get<uintptr_t>());

    /* To generate a call, will create a ResolvedMethodSymbol, but we need to
     * know the signature. The return type is intuitable from the call node, but
     * the arguments must be provided explicitly, hence the args list, that
     * mimics the args list of (method ...)
     */
    ...
    TR::ResolvedMethod* method =
      new (compilation->trHeapMemory()) TR::ResolvedMethod("file",
                                                           "line",
                                                           "name",
                                                           argIlTypes.size(),
                                                           &argIlTypes[0],
    ...
    methodSymRef->getSymbol()->castToMethodSymbol()->setLinkage(
      linkageConvention);
    node = TR::Node::createWithSymRef(
      opcode.getOpCodeValue(), childCount, methodSymRef);
  } else {
 ...
 */
bool
Tril::TRLangBuilder::cfgFor(const ASTNode* const tree)
{
...
Tril::TRLangBuilder::injectIL()
{
  ...
  while (block) {
    ...
    if (fallthroughArg != NULL) {
      ...
      auto dest = _currentBlockNumber + 1 == numBlocks()
                    ? cfg()->getEnd()
                    : _blocks[_currentBlockNumber + 1];
