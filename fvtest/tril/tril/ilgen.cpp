...
TR::Node* Tril::TRLangBuilder::toTRNode(const ASTNode* const tree)
{
  ...
  if (opcode.isLoadConst()) {
    ...
    TR::Symbol* sym = TR::Symbol::createNamedShadow(
      compilation->trHeapMemory(), type, TR::DataType::getSize(opcode.getType()), (char*)name);
    TR::SymbolReference* symref = new (compilation->trHeapMemory()) TR::SymbolReference(
      compilation->getSymRefTab(), sym, compilation->getMethodSymbol()->getResolvedMethodIndex(), -1);
    ...
    TR::ResolvedMethod* method = new (compilation->trHeapMemory())
      TR::ResolvedMethod("file", "line", "name", argIlTypes.size(), &argIlTypes[0], returnIlType, targetAddress, 0);
