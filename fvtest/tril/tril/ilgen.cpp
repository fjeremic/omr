...
TR::Node *Tril::TRLangBuilder::toTRNode(const ASTNode *const tree) {
  ...
  if (opcode.isLoadConst()) {
    ...
    if (tree->getArgByName("parm") != NULL) {
      ...
      TraceIL("parameter %d\n", arg);
      auto symref = symRefTab()->findOrCreateAutoSymbol(_methodSymbol, arg,
                                                        opcode.getType());
    ...
    if (tree->getArgByName("temp") != NULL) {
      ...
      auto symref = _symRefMap[symName];
      node = TR::Node::createWithSymRef(opcode.getOpCodeValue(), childCount,
                                        symref);
    } else {
      // symref kind not recognized
      return NULL;
    ...
    TR::Symbol *sym = TR::Symbol::createNamedShadow(
        compilation->trHeapMemory(), type,
        TR::DataType::getSize(opcode.getType()), (char *)name);
    TR::SymbolReference *symref =
        new (compilation->trHeapMemory()) TR::SymbolReference(
            compilation->getSymRefTab(), sym,
            compilation->getMethodSymbol()->getResolvedMethodIndex(), -1);
    ...
    TraceIL("  is if with target block %d (%s, entry = %p", targetId,
    ...
    auto targetId = _blockMap[targetName];
    auto targetEntry = _blocks[targetId]->getEntry();
    TraceIL("  is branch to target block %d (%s, entry = %p", targetId,
            targetName, targetEntry);
    node = TR::Node::create(opcode.getOpCodeValue(), childCount);
    ...
    TR::ResolvedMethod *method = new (compilation->trHeapMemory())
        TR::ResolvedMethod("file", "line", "name", argIlTypes.size(),
                           &argIlTypes[0], returnIlType, targetAddress, 0);
