 ...
 * --------------|-----------------------|--------------------------------------
 * Integer       | integral values       | any type satisfying
 * `OMR::IsIntegral`/`std::is_integral` FloatingPoint | floating point values |
 * any type satisfying `OMR::IsFloatingPoint`/`std::is_floating_point` String |
 * character strings     | `const char*`
 *
 * Names for these types are defined in the ASTType enum.
 *
 * To aid usability, this struct provides the `get<T>()` template member
 * function that will return the stored value as type `T` if `T` is compatible
 * with the type of the stored value.
 *
...
struct ASTValue {
 public:
  ...
  // constructors
  explicit ASTValue(Integer_t v) : _type(Integer), next(NULL) {
    _value.integer = v;
  }
  explicit ASTValue(FloatingPoint_t v) : _type(FloatingPoint), next(NULL) {
  ...
  /**
   * @brief Return the contained value as its underlying integer type, if that
   * is its type
   */
  ...
  /**
   * @brief Return the contained value as its underlying floating point type, if
   * that is its type
   */
  ...
  /**
   * @brief Return the contained value as its underlying string type, if that is
   * its type
   */
  ...
  /**
   * @brief Checks whether the type of the contained value and the specified
   * type are compatible
  ...
  template <typename T>
  typename OMR::EnableIf<OMR::IsIntegral<T>::VALUE, bool>::Type
  isCompatibleWith() const {
    return Integer == _type;
  }
  template <typename T>
  typename OMR::EnableIf<OMR::IsFloatingPoint<T>::VALUE, bool>::Type
  isCompatibleWith() const {
    return FloatingPoint == _type;
  }
  template <typename T>
  typename OMR::EnableIf<OMR::IsSame<String_t, T>::VALUE, bool>::Type
  isCompatibleWith() const {
    return String == _type;
  }
...
struct ASTNode {
 ...
 public:
  ...
  const ASTNodeArg* getArgByName(const char* name) const {
    ...
    while (arg) {
      if (arg->getName() != NULL &&
          strcmp(name, arg->getName()) == 0) {  // arg need not have a name
        return arg;
      }
      arg = arg->next;
    }
  ...
  /**
   * @brief Finds and returns a positional (nameless) argument by its position
   * (index)
