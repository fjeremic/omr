...
int32_t Tril::SimpleCompiler::compile() { return compileWithVerifier(NULL); }
...
int32_t Tril::SimpleCompiler::compileWithVerifier(TR::IlVerifier *verifier) {
  ...
  TR::ResolvedMethod resolvedMethod(
      "file", "line", "name", argIlTypes.size(), &argIlTypes[0],
  ...
  if (rc == 0) {
    ...
    // use cg->getTOCBase(), but the code generator has already been destroyed
    // by now...
