  ...
  Token::Type stateToType(State s) const
  {
    switch (s) {
      ...
      case RPAREN:
        return Token::RPAREN;
      case LPAREN_SQ:
        return Token::LPAREN_SQ;
      case RPAREN_SQ:
        return Token::RPAREN_SQ;
      case EQUALS:
        return Token::EQUALS;
      case COMMA:
        return Token::COMMA;
      ...
      default:
        throw LexerFailure("Cannot convert state to Type");
    }
  }
...
public:
  ...
  ASTstates()
  {
    ...
    transitionFunction = std::vector<std::vector<State>>(
      LARGEST_STATE + 1, std::vector<State>(128));
  ...
  /* Tokenize the given input following the transition rules until no non-error
   * transitions available If in an accepting state, then emit current token,
   * and go back to the start state Else if in the intermediate state (either
   * accepting or failed state), keep reading input Else throw LexerExecption
   * (in failed state)
   *
    ...
    for (std::string::const_iterator inputPosn = input.begin();
      ...
      if (inputPosn == input.end() || failed(state)) {
        if (accept(oldState)) {
          ...
          throw LexerFailure(
            "ERROR: Simplified maximal munch failed on input: " + munchedInput);
        }
      }
    }
    return result;
...
bool
isCommentOrWhitespace(const Token token)
{
...
scan(const std::string& input)
{
  ...
  tokens.erase(
    std::remove_if(tokens.begin(), tokens.end(), isCommentOrWhitespace),
    tokens.end());
  return tokens;
}
...
operator<<(std::ostream& out, const Token& tok)
{
  switch (tok.getType()) {
    ...
    case Token::RPAREN:
      out << "RPAREN";
      break;
    case Token::LPAREN_SQ:
      out << "LPAREN_SQ";
      break;
    case Token::RPAREN_SQ:
      out << "RPAREN_SQ";
      break;
    case Token::EQUALS:
      out << "EQUALS";
    ...
    case Token::HEXINT:
      ...
      break;
    default:
      throw LexerFailure("ERROR: Failed to determine input: " + tok.getValue());
  }
  out << " " << tok.getValue();
  return out;
}
...
/* Grammer rules: (note: "." is being used to designate epsilon/null
 * productions) nodeList: nodeList node | . node: '(' IDENTIFIER argList
 * nodeList ')' argList: argList arg | . arg: value | IDENTIFIER '=' value |
 * IDENTIFIER '=' '[' valueList ']' valueList: value | valueList ',' value
 ...
 */
bool
isValue(TokenIter tokenIt)
{
  return (tokenIt->getType() == Token::DOUBLE_ ||
          tokenIt->getType() == Token::INT ||
          tokenIt->getType() == Token::STRING_ ||
          tokenIt->getType() == Token::HEXINT ||
...
buildNodeValue(Token token)
{
  ...
  if (tokenType == Token::STRING_ || tokenType == Token::ID) {
    ...
    throw ParserFailure("Cannot build ASTValue from non-value token: " +
                        tokenValue);
  }
}

/* @brief Parse and build a list of node values
 * When the function returns, the token iterator points to the first ']' token
 ...
 */
ASTValue*
parseValueList(TokenIter& tokenIt)
{
 ...
 * When the function returns, the token iterator points to the first '(' or ')'
 * token of the parsed expression. Exceptions can be thrown when the parsed node
 * argument grammer is NOT one of the following:
 *   1. ID EQUALS
 ...
 */
ASTNodeArg*
parseArgList(TokenIter& tokenIt)
{
  ASTNodeArg* currentArg = NULL;
  ASTNodeArg* argList = NULL;

  while (tokenIt->getType() != Token::LPAREN &&
         tokenIt->getType() != Token::RPAREN) {
    if (tokenIt->getType() == Token::ID &&
        tokenIt.peek().getType() == Token::EQUALS) {
      ...
      if (tokenIt->getType() == Token::LPAREN_SQ) {
        ...
        currentArg =
          createNodeArg(node_args_name, parseValueList(tokenIt), NULL);
      } else {
        currentArg =
          createNodeArg(node_args_name, buildNodeValue(*tokenIt), NULL);
      }
    } else if (isValue(tokenIt)) {
      currentArg = createNodeArg("", buildNodeValue(*tokenIt), NULL);
    } else {
      throw ParserFailure("Expecting value token but got " +
                          tokenIt->getValue());
    }
    if (argList == NULL) {
      argList = currentArg;
 ...
 * Exceptions can be thrown when the parsed string is incorrect in the following
 * situation:
...
buildAST(TokenIter& tokenIt)
{
  ...
  while (!tokenIt.isEnd() && tokenIt->getType() == Token::LPAREN) {
    ...
    if (tokenIt.isEnd()) {
      ...
      throw ParserFailure("The next token of LPAREN should be IDENTIFIER, "
    ...
    if (tokenIt.isEnd() || tokenIt->getType() != Token::RPAREN) {
      throw ParserFailure("Missing RPAREN, while current value is " +
                          tokenIt->getValue());
    }
    ...
    if (siblingsList == NULL) {
      ...
      appendSiblingNode(siblingsList,
                        createNode(node_name, argList, childrenList, NULL));
    }
  }
  return siblingsList;
}
