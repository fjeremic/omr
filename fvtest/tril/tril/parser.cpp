...
LexerFailure::LexerFailure(std::string message) : message(message) {}

const std::string& LexerFailure::what() const {
  return message;
}
...
ParserFailure::ParserFailure(std::string message) : message(message) {}

const std::string& ParserFailure::what() const {
  return message;
}
...
class ASTstates {
 ...
 public:
  ...
  void stateTransition(State oldState,
...
ASTNode* buildAST(TokenIter& tokenIt) {
  ...
  while (!tokenIt.isEnd() && tokenIt->getType() == Token::LPAREN) {
    ...
    if (tokenIt.isEnd()) {
      ...
      throw ParserFailure(
          "The next token of LPAREN should be IDENTIFIER, while current value "
          "is " +
          tokenIt->getValue());
    }
