...
bool isValue(TokenIter tokenIt)
{
  return (tokenIt->getType() == Token::DOUBLE_ || tokenIt->getType() == Token::INT
    || tokenIt->getType() == Token::STRING_ || tokenIt->getType() == Token::HEXINT || tokenIt->getType() == Token::ZERO
    || tokenIt->getType() == Token::ID);
}
...
ASTNode* buildAST(TokenIter& tokenIt)
{
  ...
  while (!tokenIt.isEnd() && tokenIt->getType() == Token::LPAREN) {
    ...
    if (tokenIt.isEnd()) {
      ...
      throw ParserFailure(
        "The next token of LPAREN should be IDENTIFIER, while current value is " + tokenIt->getValue());
    }
