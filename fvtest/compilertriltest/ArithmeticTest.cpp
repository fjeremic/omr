...
template <typename T> T sub(T l, T r) { return l - r; }

template <typename T> T mul(T l, T r) { return l * r; }

template <typename T> T div(T l, T r) { return l / r; }

template <typename T> T udiv(T l, T r) { return l / r; }

template <typename T> T rem(T l, T r) { return l % r; }

template <typename T> T urem(T l, T r) { return l % r; }
...
TEST_P(UInt32Arithmetic, UsingConst) {
  ...
  SKIP_IF((param.opcode == "iudiv" || param.opcode == "iurem") &&
              (OMRPORT_ARCH_PPC64 == arch || OMRPORT_ARCH_PPC64LE == arch),
...
TEST_P(Int16Arithmetic, UsingConst) {
  ...
  SKIP_IF(OMRPORT_ARCH_X86 == arch || OMRPORT_ARCH_HAMMER == arch,
          MissingImplementation)
...
INSTANTIATE_TEST_CASE_P(
    ArithmeticTest, Int32Arithmetic,
    ::testing::Combine(
        ::testing::ValuesIn(TRTest::const_value_pairs<int32_t, int32_t>()),
        ::testing::Values(
            std::make_tuple<const char *, int32_t (*)(int32_t, int32_t)>("iadd",
                                                                         add),
            std::make_tuple<const char *, int32_t (*)(int32_t, int32_t)>("isub",
                                                                         sub),
            std::make_tuple<const char *, int32_t (*)(int32_t, int32_t)>("imul",
                                                                         mul),
            std::make_tuple<const char *, int32_t (*)(int32_t, int32_t)>(
                "imulh", imulh))));

INSTANTIATE_TEST_CASE_P(
    ArithmeticTest, Int64Arithmetic,
    ::testing::Combine(
        ::testing::ValuesIn(TRTest::const_value_pairs<int64_t, int64_t>()),
        ::testing::Values(
            std::make_tuple<const char *, int64_t (*)(int64_t, int64_t)>("ladd",
                                                                         add),
            std::make_tuple<const char *, int64_t (*)(int64_t, int64_t)>("lsub",
                                                                         sub),
            std::make_tuple<const char *, int64_t (*)(int64_t, int64_t)>(
                "lmul", mul))));

INSTANTIATE_TEST_CASE_P(
    ArithmeticTest, Int16Arithmetic,
    ::testing::Combine(
        ::testing::ValuesIn(TRTest::const_value_pairs<int16_t, int16_t>()),
        ::testing::Values(
            std::make_tuple<const char *, int16_t (*)(int16_t, int16_t)>(
                "sadd", add<int16_t>),
            std::make_tuple<const char *, int16_t (*)(int16_t, int16_t)>(
                "ssub", sub<int16_t>),
            std::make_tuple<const char *, int16_t (*)(int16_t, int16_t)>(
...
INSTANTIATE_TEST_CASE_P(
    ArithmeticTest, Int8Arithmetic,
    ::testing::Combine(
        ::testing::ValuesIn(TRTest::const_value_pairs<int8_t, int8_t>()),
        ::testing::Values(
            std::make_tuple<const char *, int8_t (*)(int8_t, int8_t)>(
                "badd", add<int8_t>),
            std::make_tuple<const char *, int8_t (*)(int8_t, int8_t)>(
                "bsub", sub<int8_t>),
            std::make_tuple<const char *, int8_t (*)(int8_t, int8_t)>(
 ...
 */
template <typename T> bool div_filter(std::tuple<T, T> a) {
...
INSTANTIATE_TEST_CASE_P(
    DivArithmeticTest, Int32Arithmetic,
    ::testing::Combine(
        ::testing::ValuesIn(
            TRTest::filter(TRTest::const_value_pairs<int32_t, int32_t>(),
                           div_filter<int32_t>)),
        ::testing::Values(
            std::make_tuple<const char *, int32_t (*)(int32_t, int32_t)>("idiv",
                                                                         div),
            std::make_tuple<const char *, int32_t (*)(int32_t, int32_t)>(
                "irem", rem))));

INSTANTIATE_TEST_CASE_P(
    DivArithmeticTest, Int64Arithmetic,
    ::testing::Combine(
        ::testing::ValuesIn(
            TRTest::filter(TRTest::const_value_pairs<int64_t, int64_t>(),
                           div_filter<int64_t>)),
        ::testing::Values(
            std::make_tuple<const char *, int64_t (*)(int64_t, int64_t)>("ldiv",
                                                                         div),
            std::make_tuple<const char *, int64_t (*)(int64_t, int64_t)>(
                "lrem", rem))));
 ...
 */
template <typename T> bool udiv_filter(std::tuple<T, T> a) {
  return std::get<1>(a) == 0;
}

INSTANTIATE_TEST_CASE_P(
    DivArithmeticTest, UInt32Arithmetic,
    ::testing::Combine(
        ::testing::ValuesIn(
            TRTest::filter(TRTest::const_value_pairs<uint32_t, uint32_t>(),
                           udiv_filter<uint32_t>)),
        ::testing::Values(
            std::make_tuple<const char *, uint32_t (*)(uint32_t, uint32_t)>(
                "iudiv", udiv),
            std::make_tuple<const char *, uint32_t (*)(uint32_t, uint32_t)>(
                "iurem", urem))));

INSTANTIATE_TEST_CASE_P(
    DivArithmeticTest, UInt64Arithmetic,
    ::testing::Combine(
        ::testing::ValuesIn(
            TRTest::filter(TRTest::const_value_pairs<uint64_t, uint64_t>(),
                           udiv_filter<uint64_t>)),
        ::testing::Values(
            std::make_tuple<const char *, uint64_t (*)(uint64_t, uint64_t)>(
                "ludiv", udiv))));
...
INSTANTIATE_TEST_CASE_P(
    ArithmeticTest, FloatArithmetic,
    ::testing::Combine(
        ::testing::ValuesIn(TRTest::filter(
            TRTest::const_value_pairs<float, float>(), smallFp_filter<float>)),
        ::testing::Values(
            std::make_tuple<const char *, float (*)(float, float)>(
                "fadd", static_cast<float (*)(float, float)>(add)),
            std::make_tuple<const char *, float (*)(float, float)>(
                "fsub", static_cast<float (*)(float, float)>(sub)),
            std::make_tuple<const char *, float (*)(float, float)>(
                "fmul", static_cast<float (*)(float, float)>(mul)),
            std::make_tuple<const char *, float (*)(float, float)>(
                "fdiv", static_cast<float (*)(float, float)>(div)))));
...
INSTANTIATE_TEST_CASE_P(
    ArithmeticTest, DoubleArithmetic,
    ::testing::Combine(
        ::testing::ValuesIn(
            TRTest::filter(TRTest::const_value_pairs<double, double>(),
                           smallFp_filter<double>)),
        ::testing::Values(
            std::make_tuple<const char *, double (*)(double, double)>("dadd",
                                                                      add),
            std::make_tuple<const char *, double (*)(double, double)>("dsub",
                                                                      sub),
            std::make_tuple<const char *, double (*)(double, double)>("dmul",
                                                                      mul),
            std::make_tuple<const char *, double (*)(double, double)>("ddiv",
                                                                      div))));

template <typename T> bool smallFp_unary_filter(T a) {
  // workaround: avoid failure caused by snprintf("%f")
...
TEST_P(FloatUnaryArithmetic, UsingConst) {
  ...
  std::snprintf(inputTrees, sizeof(inputTrees),
                ...
                "      (%s"
                "        (fconst %f)))))",
                param.opcode.c_str(), param.value);
  auto trees = parseString(inputTrees);
...
INSTANTIATE_TEST_CASE_P(
    ArithmeticTest, FloatUnaryArithmetic,
    ::testing::Combine(
        ::testing::ValuesIn(TRTest::filter(TRTest::const_values<float>(),
                                           smallFp_unary_filter<float>)),
        ::testing::Values(
            std::make_tuple<const char *, float (*)(float)>(
                "fabs", static_cast<float (*)(float)>(std::abs)),
...
INSTANTIATE_TEST_CASE_P(
    ArithmeticTest, DoubleUnaryArithmetic,
    ::testing::Combine(
        ::testing::ValuesIn(TRTest::filter(TRTest::const_values<double>(),
                                           smallFp_unary_filter<double>)),
        ::testing::Values(
            std::make_tuple<const char *, double (*)(double)>(
                "dabs", static_cast<double (*)(double)>(std::abs)),
