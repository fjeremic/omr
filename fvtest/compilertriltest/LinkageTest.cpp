...
TYPED_TEST(LinkageTest, InvalidLinkageTest)
{
  ...
  SKIP_IF(OMRPORT_ARCH_HAMMER != arch, MissingImplementation)
    << "Test is skipped on non-x86-64 platforms because calls are not currently supported on other platforms (see "
       "issue #1645)";

  char inputTrees[200] = { 0 };
  const auto format_string = "(method return=Int32  args=[Int32] (block (ireturn (icall address=0x%jX args=[Int32] "
                             "linkage=noexist  (iload parm=0)) )  ))";
  std::snprintf(inputTrees, 200, format_string,
    reinterpret_cast<uintmax_t>(static_cast<TypeParam (*)(TypeParam)>(passThrough<TypeParam>)));
...
TYPED_TEST(LinkageTest, SystemLinkageParameterPassingSingleArg)
{
  ...
  char inputTrees[200] = { 0 };
  const auto format_string = "(method return=%s args=[%s] (block (%sreturn (%scall address=0x%jX args=[%s] "
                             "linkage=system (%sload parm=0)) )  ))";
...
TYPED_TEST(LinkageTest, SystemLinkageParameterPassingFourArg)
{
  ...
  const auto format_string
    = "(method return=%s args=[%s,%s,%s,%s] (block (%sreturn (%scall address=0x%jX args=[%s,%s,%s,%s] linkage=system"
      " (%sload parm=0)"
      " (%sload parm=1)"
      " (%sload parm=2)"
      " (%sload parm=3)"
...
{
  char inputTrees[400] = { 0 };
  const auto format_string = "(method return=%s args=[%s,%s,%s,%s] (block (%sreturn (%sload parm=3))))";
...
/**
 * In accordance with the Microsoft x64 calling convention, only four parameters can be passed in registers (both
 * integer and floating point). Additional arguments are pushed onto the stack (right to left). It is an interesting
...
TYPED_TEST(LinkageTest, SystemLinkageParameterPassingFiveArg)
{
  ...
  char inputTrees[400] = { 0 };
  const auto format_string = "(method return=%s args=[%s,%s,%s,%s,%s] (block (%sreturn (%scall address=0x%jX "
                             "args=[%s,%s,%s,%s,%s] linkage=system"
                             " (%sload parm=0)"
                             " (%sload parm=1)"
                             " (%sload parm=2)"
                             " (%sload parm=3)"
  ...
  std::snprintf(inputTrees, 400, format_string, TypeToString<TypeParam>::type, // Return
    ...
    TypeToString<TypeParam>::prefix, // return
    TypeToString<TypeParam>::prefix, // call
    reinterpret_cast<uintmax_t>(static_cast<TypeParam (*)(TypeParam, TypeParam, TypeParam, TypeParam, TypeParam)>(
 ...
 * the stack right before calling the function (regardless of the actual number of parameters used), and to pop the
 * stack after the call. In the case, when the callee actively uses the stack, the callee can relate on this "shadow
 * space" and save arguments or other nonvolatile registers there. The test demonstrates what can be happen when the
 * callee is an actively stack user.
 */
TYPED_TEST(LinkageTest, SystemLinkageParameterPassingFiveArgToStackUser)
{
  ...
  EXPECT_EQ(static_cast<TypeParam>(2053), stackUser<TypeParam>(1, 1, 1, 1, static_cast<TypeParam>(1024)))
    << "Input Trees: " << inputTrees;
  EXPECT_EQ(static_cast<TypeParam>(3), stackUser<TypeParam>(1, 1, 1, 1, static_cast<TypeParam>(-1)))
    << "Input Trees: " << inputTrees;
  EXPECT_EQ(static_cast<TypeParam>(0x1e1e23), stackUser<TypeParam>(1, 1, 1, 1, static_cast<TypeParam>(0xf0f0f)))
    << "Input Trees: " << inputTrees;

  // Check the linkage
  EXPECT_EQ(static_cast<TypeParam>(2053), entry_point(1, 1, 1, 1, static_cast<TypeParam>(1024)))
    << "Input Trees: " << inputTrees;
  EXPECT_EQ(static_cast<TypeParam>(3), entry_point(1, 1, 1, 1, static_cast<TypeParam>(-1)))
    << "Input Trees: " << inputTrees;
  EXPECT_EQ(static_cast<TypeParam>(0x1e1e23), entry_point(1, 1, 1, 1, static_cast<TypeParam>(0xf0f0f)))
    << "Input Trees: " << inputTrees;
}
...
double fifthArgFromMixedTypes(double a, int32_t b, double c, int32_t d, double e)
{
  return e;
}
