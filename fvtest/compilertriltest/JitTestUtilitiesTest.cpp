...
TEST(TRTestCombineVectorTest, CombineEmptyVectorsOfSameType)
{
  ...
  ASSERT_TRUE(v.empty()) << "Combining two empty vectors should always result in another empty vector.";
}
...
TEST(TRTestCombineVectorTest, CombineEmptyAndNonEmptyVectorsOfSameType)
{
  ...
  ::testing::StaticAssertTypeEq<vector<tuple<int, int> >, decltype(v)>();
  ASSERT_TRUE(v.empty()) << "Combining any vector with an empty vector should always result in another empty vector.";
}
...
TEST(TRTestCombineBraceInitTest, CombineEmptyListsOfSameType)
{
  ...
  auto v = TRTest::combine(v1, v2);
  ::testing::StaticAssertTypeEq<vector<tuple<int, int> >, decltype(v)>();
  ASSERT_TRUE(v.empty()) << "Combining two empty lists should always result in an empty vector.";
}
...
TEST(TRTestCombineBraceInitTest, CombineEmptyAndNonEmptyListsOfSameType)
{
  ...
  auto v = TRTest::combine(v1, v2);
  ::testing::StaticAssertTypeEq<vector<tuple<int, int> >, decltype(v)>();
  ASSERT_TRUE(v.empty()) << "Combining any list with an empty list should always result in another empty vector.";
}
...
{
  ASSERT_STRNE("unknown", omrsysinfo_get_CPU_architecture()) << "The host architecture should be known";
}
