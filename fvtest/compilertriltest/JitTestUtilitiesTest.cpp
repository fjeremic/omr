...
TEST(TRTestCombineVectorTest, CombineEmptyVectorsOfSameType) {
  ...
  ASSERT_TRUE(v.empty()) << "Combining two empty vectors should always result "
                            "in another empty vector.";
}
...
TEST(TRTestCombineVectorTest, CombineEmptyAndNonEmptyVectorsOfSameType) {
  ...
  auto v =
      TRTest::combine(vector<int>(), vector<int>(test_array, test_array + 3));
  ::testing::StaticAssertTypeEq<vector<tuple<int, int>>, decltype(v)>();
  ASSERT_TRUE(v.empty()) << "Combining any vector with an empty vector should "
                            "always result in another empty vector.";
}
...
TEST(TRTestCombineVectorTest, CombineEmptyAndNonEmptyVectorsOfDifferentTypes) {
  ...
  auto v =
      TRTest::combine(vector<long>(), vector<char>(test_array, test_array + 3));
  ::testing::StaticAssertTypeEq<vector<tuple<long, char>>, decltype(v)>();
  ASSERT_TRUE(v.empty()) << "Combining any vector with an empty vector should "
                            "always result in another empty vector.";
}
...
TEST(TRTestCombineVectorTest, CombineNonEmptyAndEmptyVectorsOfSameType) {
  ...
  auto v =
      TRTest::combine(vector<int>(test_array, test_array + 3), vector<int>());
  ::testing::StaticAssertTypeEq<vector<tuple<int, int>>, decltype(v)>();
  ASSERT_TRUE(v.empty()) << "Combining any vector with an empty vector should "
                            "always result in another empty vector.";
}
...
TEST(TRTestCombineVectorTest, CombineNonEmptyAndEmptyVectorsOfDifferentTypes) {
  ...
  auto v =
      TRTest::combine(vector<long>(test_array, test_array + 3), vector<char>());
  ::testing::StaticAssertTypeEq<vector<tuple<long, char>>, decltype(v)>();
  ASSERT_TRUE(v.empty()) << "Combining any vector with an empty vector should "
                            "always result in another empty vector.";
}
...
TEST(TRTestCombineVectorTest, CombineNonEmptyVectorsOfSameType) {
  ...
  ASSERT_EQ(v1.size() * v2.size(), v.size())
      << "Size of combined vectors should be the product of the sizes of the "
...
TEST(TRTestCombineBraceInitTest, CombineEmptyAndNonEmptyListsOfSameType) {
  ...
  auto v = TRTest::combine(v1, v2);
  ::testing::StaticAssertTypeEq<vector<tuple<int, int>>, decltype(v)>();
  ASSERT_TRUE(v.empty()) << "Combining any list with an empty list should "
                            "always result in another empty vector.";
}
...
TEST(TRTestCombineBraceInitTest, CombineNonEmptyListsOfSameType) {
  ...
  auto v = TRTest::combine(v1, v2);
  ::testing::StaticAssertTypeEq<vector<tuple<int, int>>, decltype(v)>();
  ASSERT_EQ(3 * 2, v.size()) << "Size of combined lists should be the product "
                                "of the sizes of the two individual lists.";
}
...
bool returnFalse(char c) { return false; }
...
TEST(TRTestFilter, FilterNothingFromEmptyVector) {
  ...
  ASSERT_TRUE(v_out.empty())
      << "Filtering an empty vector should result in another empty vector.";
}
...
TEST(TRTestFilter, FilterNothingFromVector) {
  ...
  ASSERT_EQ(v_in, v_out)
      << "Filtering nothing should just return the vector unchanged.";
}
...
TEST(TRTestFilter, FilterEverythingFromVector) {
  ...
  auto v_out = TRTest::filter(v_in, returnTrue);  // should filter everything
  ASSERT_TRUE(v_out.empty())
      << "Filtering everything from vector should result in an empty vector.";
}

bool isChar_c(char l) { return l == 'c'; }
...
TEST(TRTestFilter, FilterVectorWithNoOccurrences) {
  ...
  ASSERT_EQ(v_in, v_out)
      << "Filtering a vector that doesn't contain elements matching the "
         "predicate should just return the same vector.";
...
TEST(SkipTest, SkipIfTrue) {
  SKIP_IF(true, UnsupportedFeature)
      << "Test is intentionally skipped to verify that skipping works";
...
TEST(SkipTest, SkipIfFalse) {
  EXPECT_FATAL_FAILURE(
      {
        SKIP_IF(false, KnownBug)
            << "Test should not have been skipped! SKIP_IF must have a bug.";
