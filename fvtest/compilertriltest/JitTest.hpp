 ...
 *
 * This class makes it possible to make calls to the port library from within
 * test cases. Specifically, it makes it possible to use the port library macros
 * without having to write extra code in the test case body.
 *
 * The static methods `initPortLib()` and `shutdownPortLib()` must be called
 * externally (ideally from the global environment setup and teardown) to
 * initialize and shutdown the port library, respectively.
 */
...
public:
  ...
  public:
    FailedThreadLibraryInit()
        : std::runtime_error("Failed to initialize the thread library.") {}
  };
  ...
  public:
    FailedCurrentThreadAttachment()
        : std::runtime_error(
              "Failed to attach current thread to thread library.") {}
  };
  ...
  public:
    FailedPortLibraryInit()
        : std::runtime_error("Failed to initialize the port library.") {}
  };
  ...
  static void initPortLib() {
    if (0 != omrthread_init_library()) {
      throw FailedThreadLibraryInit();
    }
    if (0 != omrthread_attach_ex(&current_thread, J9THREAD_ATTR_DEFAULT)) {
      throw FailedCurrentThreadAttachment();
    }
    if (0 != omrport_init_library(&PortLib, sizeof(OMRPortLibrary))) {
      throw FailedPortLibraryInit();
    }
  }
...
protected:
  ...
  OMRPortLibrary *privateOmrPortLibrary; // pointer to object to be used by port
                                         // library macro calls in tests

private:
  static omrthread_t current_thread; // handle for current thread; needed to
...
public:
  JitTest() {
    auto initSuccess = initializeJitWithOptions(
        (char *)"-Xjit:acceptHugeMethods,enableBasicBlockHoisting,"
...
public:
  JitOptTest() : JitTest(), _optimizations(), _strategy(NULL) {}
 ...
 * @return the Cartesian product of the input containers as a std::vector of
 * std::tuple
 *
 * Example:
 *
...
std::vector<std::tuple<typename L::value_type, typename R::value_type>>
combine(L l, R r) {
  auto v =
      std::vector<std::tuple<typename L::value_type, typename R::value_type>>();
 ...
 */
template <typename C, typename Predicate> C filter(C range, Predicate pred) {
...
template <typename T> const T negative_one_value() {
...
/**
 * @brief Convenience function returning possible test inputs of the specified
 * type
 */
template <typename T> std::vector<T> const_values() {
  ...
  return std::vector<T>(inputArray,
                        inputArray + sizeof(inputArray) / sizeof(T));
}
 ...
 * type
 */
template <> inline std::vector<int64_t> const_values<int64_t>() {
  int64_t inputArray[] = {
      zero_value<int64_t>(),
      one_value<int64_t>(),
  ...
  return std::vector<int64_t>(inputArray, inputArray + sizeof(inputArray) /
                                                           sizeof(int64_t));
}
 ...
 * type
 */
template <> inline std::vector<uint64_t> const_values<uint64_t>() {
  uint64_t inputArray[] = {
      zero_value<uint64_t>(),
      one_value<uint64_t>(),
  ...
  return std::vector<uint64_t>(inputArray, inputArray + sizeof(inputArray) /
                                                            sizeof(uint64_t));
}
 ...
 * type
 */
template <> inline std::vector<int32_t> const_values<int32_t>() {
  int32_t inputArray[] = {
      zero_value<int32_t>(),
      one_value<int32_t>(),
  ...
  return std::vector<int32_t>(inputArray, inputArray + sizeof(inputArray) /
                                                           sizeof(int32_t));
}
 ...
 * type
 */
template <> inline std::vector<uint32_t> const_values<uint32_t>() {
  uint32_t inputArray[] = {
      zero_value<uint32_t>(),
      one_value<uint32_t>(),
  ...
  return std::vector<uint32_t>(inputArray, inputArray + sizeof(inputArray) /
                                                            sizeof(uint32_t));
}
 ...
 * type
 */
template <> inline std::vector<float> const_values<float>() {
  float inputArray[] = {
      zero_value<float>(),
      one_value<float>(),
  ...
  return std::vector<float>(inputArray,
                            inputArray + sizeof(inputArray) / sizeof(float));
}
 ...
 * type
 */
template <> inline std::vector<double> const_values<double>() {
  double inputArray[] = {
      zero_value<double>(),
      one_value<double>(),
  ...
  return std::vector<double>(inputArray,
                             inputArray + sizeof(inputArray) / sizeof(double));
}

/**
 * @brief Convenience function returning pairs of possible test inputs of the
 * specified types
 */
template <typename L, typename R>
 ...
 * and are mostly useful for logging and reporting purposes. Additional
 * explanations should be specified in the skip message.
 */
enum SkipReason {
...
static_assert(
    SkipReason::NumSkipReasons_ == sizeof(skipReasonStrings) / sizeof(char *),
...
class SkipHelper {
public:
  explicit SkipHelper(SkipReason reason) : reason_(reason) {}
   ...
   *   - the skip message
   * - records the skip reason as a property of the current test in Google Test
   * generated logs
  ...
  void operator=(const ::testing::Message &message) const {
    ...
    ::testing::Test::RecordProperty(
        "skipped", skipReasonStrings[static_cast<int>(reason_)]);
 ...
 *
 * This macro allows a test to be conditionally skipped without failing the
 * test. Multiple invocations can be specified per test. While the macro can can
 * be used anywhere within the scope of a test, it is best to only use at at the
 * beginning, before the main body of a test.
 *
 * To skip a test, a condition aswell as a "reason" for skipping must be
 * specified. The condition may be any boolean expression. The "reason" must be
 * a value from the `SkipReason` enum (see its documentation for further
 * details). Optionally, a more detailed message can also be specified using the
 * `<<` stream operator; as is done with `ASSERT*()` macros. When the test suite
 * is executed, skipped tests will print the (short) reason for skipping, the
 * test name, and the skip message to stdout.
 *
...
#define SKIP_IF(condition, reason)                                             \
  switch (0)                                                                   \
  case 0:                                                                      \
  default:              /* guard against ambiguous else */                     \
