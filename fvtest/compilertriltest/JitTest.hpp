...
class TestWithPortLib : public ::testing::Test {
 ...
 protected:
  ...
  OMRPortLibrary* privateOmrPortLibrary;  // pointer to object to be used by
                                          // port library macro calls in tests
...
class JitTest : public TestWithPortLib {
 public:
  JitTest() {
    ...
    if (!initSuccess) throw std::runtime_error("Failed to initialize jit");
  }
...
class JitOptTest : public JitTest {
 public:
  ...
  void addOptimizations(const OptimizationStrategy* opts) {
    ...
    while (end->_num != OMR::endOpts && end->_num != OMR::endGroup) ++end;
...
std::vector<std::tuple<typename L::value_type, typename R::value_type>> combine(
    L l, R r) {
  auto v =
...
inline std::vector<int64_t> const_values<int64_t>() {
  ...
  return std::vector<int64_t>(
      inputArray, inputArray + sizeof(inputArray) / sizeof(int64_t));
}
...
inline std::vector<uint64_t> const_values<uint64_t>() {
  ...
  return std::vector<uint64_t>(
      inputArray, inputArray + sizeof(inputArray) / sizeof(uint64_t));
}
...
inline std::vector<int32_t> const_values<int32_t>() {
  ...
  return std::vector<int32_t>(
      inputArray, inputArray + sizeof(inputArray) / sizeof(int32_t));
}
...
inline std::vector<uint32_t> const_values<uint32_t>() {
  ...
  return std::vector<uint32_t>(
      inputArray, inputArray + sizeof(inputArray) / sizeof(uint32_t));
}
