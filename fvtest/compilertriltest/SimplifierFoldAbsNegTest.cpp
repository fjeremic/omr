...
std::vector<int32_t> dataForType<int32_t>() {
  ...
  return std::vector<int32_t>(values,
                              values + sizeof(values) / sizeof(int32_t));
}
...
std::vector<int64_t> dataForType<int64_t>() {
  ...
  return std::vector<int64_t>(values,
                              values + sizeof(values) / sizeof(int64_t));
}
...
class SimplifierFoldAbsNegTestIlVerifierBase : public TR::IlVerifier {
 public:
  int32_t verify(TR::ResolvedMethodSymbol* sym) {
    for (TR::PreorderNodeIterator iter(sym->getFirstTreeTop(), sym->comp());
         iter.currentTree(); ++iter) {
...
TYPED_TEST(SimplifierFoldAbsNegTest, FoldAbsAbs) {
  ...
  ASSERT_EQ(0, compiler.compileWithVerifier(&verifier))
