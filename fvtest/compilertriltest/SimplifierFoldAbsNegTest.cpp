...
char* prefixForType();
template <>
char* prefixForType<int32_t>()
{
  return "i";
}
template <>
char* prefixForType<int64_t>()
{
  return "l";
}
template <>
char* prefixForType<float>()
{
  return "f";
}
template <>
char* prefixForType<double>()
{
  return "d";
}
...
char* nameForType();
template <>
char* nameForType<int32_t>()
{
  return "Int32";
}
template <>
char* nameForType<int64_t>()
{
  return "Int64";
}
template <>
char* nameForType<float>()
{
  return "Float";
}
template <>
char* nameForType<double>()
{
  return "Double";
}

template <typename T>
std::vector<T> dataForType();

template <>
std::vector<int32_t> dataForType<int32_t>()
{
  static int32_t values[]
    = { 0, 1, 2, -1, -2, 99999, -99999, std::numeric_limits<int32_t>::max(), std::numeric_limits<int32_t>::min() };

  return std::vector<int32_t>(values, values + sizeof(values) / sizeof(int32_t));
}

template <>
std::vector<int64_t> dataForType<int64_t>()
{
  static int64_t values[]
    = { 0, 1, 2, -1, -2, 99999, -99999, std::numeric_limits<int64_t>::max(), std::numeric_limits<int64_t>::min() };

  return std::vector<int64_t>(values, values + sizeof(values) / sizeof(int64_t));
}

template <>
std::vector<float> dataForType<float>()
{
  static float values[]
    = { 0, 1, 2, -1, -2, 3.14F, -3.14F, std::numeric_limits<float>::min(), std::numeric_limits<float>::min() };

  return std::vector<float>(values, values + sizeof(values) / sizeof(float));
}

template <>
std::vector<double> dataForType<double>()
{
  static double values[]
    = { 0, 1, 2, -1, -2, 3.14F, -3.14F, std::numeric_limits<double>::min(), std::numeric_limits<double>::min() };
...
TYPED_TEST(SimplifierFoldAbsNegTest, FoldAbsAbs)
{
  ...
  std::snprintf(inputTrees, sizeof(inputTrees),
    ...
    "   (%sabs                      "
    "    (%sabs (%sload parm=0))))))",
    nameForType<TypeParam>(), nameForType<TypeParam>(), prefixForType<TypeParam>(), prefixForType<TypeParam>(),
    prefixForType<TypeParam>(), prefixForType<TypeParam>());
