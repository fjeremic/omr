...
// C++11 upgrade (Issue #1916).
template <typename Ret, typename T> struct UnaryOpParamStruct {
  T value;
  std::string opcode;
  Ret (*oracle)(T);
};
...
UnaryOpParamStruct<Ret, T>
to_struct(std::tuple<T, std::tuple<std::string, Ret (*)(T)>> param) {
 ...
 */
template <typename Ret, typename Left, typename Right>
BinaryOpParamStruct<Ret, Left, Right>
to_struct(std::tuple<std::tuple<Left, Right>,
                     std::tuple<std::string, Ret (*)(Left, Right)>>
              param) {
...
class OpCodeTest
    : public JitTest,
      public ::testing::WithParamInterface<std::tuple<
          std::tuple<Args...>, std::tuple<std::string, Ret (*)(Args...)>>> {};
