...
INSTANTIATE_TEST_CASE_P(
  ...
  ::testing::Combine(
    ...
    ::testing::Values(
      ...
      std::make_tuple<const char*, int32_t (*)(int32_t, int32_t)>("imin",
                                                                  imin))));
...
INSTANTIATE_TEST_CASE_P(
  ...
  ::testing::Combine(
    ...
    ::testing::Values(
      ...
      std::make_tuple<const char*, int64_t (*)(int64_t, int64_t)>("lmin",
                                                                  lmin))));
...
TEST_P(FloatMaxMin, UsingConst)
{
  ...
  SKIP_IF(OMRPORT_ARCH_X86 == arch || OMRPORT_ARCH_S390 == arch ||
          ...
          KnownBug)
    << "The " << arch
    << " code generator currently doesn't support fmax/fmin (see issue #4276)";
  auto param = TRTest::to_struct(GetParam());
...
TEST_P(DoubleMaxMin, UsingConst)
{
  ...
  SKIP_IF(OMRPORT_ARCH_X86 == arch || OMRPORT_ARCH_S390 == arch ||
          ...
          KnownBug)
    << "The " << arch
    << " code generator currently doesn't support dmax/dmin (see issue #4276)";
  auto param = TRTest::to_struct(GetParam());
