...
TEST_F(VectorTest, VDoubleAdd) {
  ...
  // have vector support. Determining whether a specific platform has the
  // support at runtime is currently not possible in tril. So the test is being
  // disabled altogether on Z for now.
  ...
  auto entry_point =
      compiler.getEntryPoint<void (*)(double[], double[], double[])>();
  ...
  EXPECT_DOUBLE_EQ(inputA[0] + inputB[0],
                   output[0]); // Epsilon = 4ULP -- is this necessary?
  EXPECT_DOUBLE_EQ(inputA[1] + inputB[1],
                   output[1]); // Epsilon = 4ULP -- is this necessary?
#endif
}
