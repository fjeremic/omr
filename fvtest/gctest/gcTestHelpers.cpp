...
GCTestEnvironment::GCTestSetUp()
{
  ...
  ASSERT_EQ(OMR_ERROR_NONE, rc)
    << "GCTestSetUp(): OMR_Initialize failed, rc=" << rc;
...
GCTestEnvironment::GCTestTearDown()
{
  ...
  ASSERT_EQ((uintptr_t)0, exampleVM._vmExclusiveAccessCount)
    << "GCTestTearDown(): _vmExclusiveAccessCount not 0, "
       "_vmExclusiveAccessCount="
...
printMemUsed(const char* where, OMRPortLibrary* portLib)
{
  ...
  if (-1 == fileDescriptor) {
    gcTestEnv->log(
      LEVEL_ERROR, "%s: failed to open /proc/self/statm.\n", where);
    return;
  }
  ...
  if (NULL == omrfile_read_text(fileDescriptor, lineStr, sizeof(lineStr))) {
    gcTestEnv->log(
      LEVEL_ERROR, "%s: failed to read from /proc/self/statm.\n", where);
    return;
  }

  unsigned long size, resident, share, text, lib, data, dt;
  int numOfTokens = sscanf(lineStr,
  ...
  if (7 != numOfTokens) {
    gcTestEnv->log(LEVEL_ERROR,
                   "%s: failed to query memory info from /proc/self/statm.\n",
                   where);
    return;
  }
  ...
  /* result in pages */
  gcTestEnv->log(
    LEVEL_VERBOSE, "%s: phys: %ld; virt: %ld\n", where, resident, size);
