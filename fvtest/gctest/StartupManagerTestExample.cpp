...
bool MM_StartupManagerTestExample::parseLanguageOptions(MM_GCExtensionsBase* extensions)
{
  bool result = true;
        ...
        if (0 == strcmp(attr.name(), "memoryMax")) {
          ...
          if (0 == j9_cmdla_stricmp(attr.value(), "gencon")) {
            ...
            gcTestEnv->log(LEVEL_ERROR,
              "WARNING: GCPolicy=gencon ignored, requires OMR_GC_MODRON_SCAVENGER (see configure_common.mk)\n");
#endif /* defined(OMR_GC_MODRON_SCAVENGER) */
          } else if (0 != j9_cmdla_stricmp(attr.value(), "optavgpause")) {
            gcTestEnv->log(
              LEVEL_ERROR, "Failed: Unrecognized GC policy (expected gencon or optavgpause): %s\n", attr.value());
            result = false;
          }
...
#else
          gcTestEnv->log(LEVEL_ERROR,
            "WARNING: concurrentMark=true ignored, requires OMR_GC_MODRON_CONCURRENT_MARK (see configure_common.mk)\n");
