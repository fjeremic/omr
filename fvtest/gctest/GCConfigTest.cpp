...
const char* gcTests[] = { "fvtest/gctest/configuration/sample_GC_config.xml",
  "fvtest/gctest/configuration/test_system_gc.xml", "fvtest/gctest/configuration/global_GC_config.xml"
  ...
  ,
  "fvtest/gctest/configuration/scavenger_GC_config.xml", "fvtest/gctest/configuration/scavenger_GC_backout_config.xml"
#endif
#if defined(OMR_GC_MODRON_SCAVENGER) && defined(OMR_GC_MODRON_CONCURRENT_MARK)
  ,
  "fvtest/gctest/configuration/gencon_GC_config.xml", "fvtest/gctest/configuration/gencon_GC_backout_config.xml"
#endif
};
...
void GCConfigTest::SetUp()
{
  ...
  omrstr_printf(verboseFile, MAX_NAME_LENGTH, "%s_%d_%lld.xml", verboseFileNamePrefix, omrsysinfo_get_pid(),
  ...
  gcTestEnv->log(
    LEVEL_VERBOSE, "Verbose GC log name: %s; numOfFiles: %d; numOfCycles: %d.\n", verboseFile, numOfFiles, numOfCycles);
...
int32_t GCConfigTest::parseAttribute(AttributeElem** root, const char* attrStr)
{
  OMRPORT_ACCESS_FROM_OMRPORT(gcTestEnv->portLib);
  int32_t rt = 0;
  const char DELIM = ',';
  ...
  if (0 == strcmp(typeStr, "root")) {
    ...
    if ((0 == strcmp(parentTypeStr, "root")) || (0 == strcmp(parentTypeStr, "normal"))) {
      ...
      gcTestEnv->log(LEVEL_ERROR,
        "%s:%d Invalid XML input: normal object %s has to be child of root or normal object!\n", __FILE__, __LINE__,
        namePrefixStr);
    }
    ...
    if (0 == strcmp(parentStr, "allocation")) {
      ...
      gcTestEnv->log(LEVEL_ERROR,
        "%s:%d Invalid XML input: garbage object %s has to be child of root, normal or allocation node!\n", __FILE__,
        __LINE__, namePrefixStr);
    }
  } else {
...
ObjectEntry* GCConfigTest::allocateHelper(const char* objName, uintptr_t size)
{
  ...
  MM_ObjectAllocationModel* noGc = new (objectAllocationModelSpace) MM_ObjectAllocationModel(
    env, size, MM_ObjectAllocationModel::selectObjectAllocationFlags(false, false, false, true));
  ...
  if (NULL == objEntry.objPtr) {
    ...
    MM_ObjectAllocationModel* withGc = new (objectAllocationModelSpace) MM_ObjectAllocationModel(
      env, size, MM_ObjectAllocationModel::selectObjectAllocationFlags(false, false, false, false));
  ...
  if (NULL != objEntry.objPtr) {
    ...
    if (consumedSize == adjustedSize) {
      gcTestEnv->log(
        LEVEL_VERBOSE, "Allocate object name: %s(%p[0x%llx])\n", objEntry.name, objEntry.objPtr, consumedSize);
    } else {
      gcTestEnv->log(LEVEL_ERROR,
        "Consumed size for allocated object name: %s(%p[0x%llx]) != adjusted request size [0x%llx].\n", objEntry.name,
        objEntry.objPtr, consumedSize, adjustedSize);
    }
...
ObjectEntry* GCConfigTest::createObject(
  const char* namePrefix, OMRGCObjectType objType, int32_t depth, int32_t nthInRow, uintptr_t size)
{
  OMRPORT_ACCESS_FROM_OMRPORT(gcTestEnv->portLib);
  ...
  if (NULL != objEntry) {
    ...
    if (NULL != objEntry) {
      /* Keep count of the new allocated non-garbage object size for garbage insertion. If the object exists in
       * objectTable, its size is ignored. */
...
int32_t GCConfigTest::createFixedSizeTree(ObjectEntry** rootEntryIndirectPtr, const char* namePrefixStr,
  OMRGCObjectType objType, uintptr_t totalSize, uintptr_t objSize, int32_t breadth)
{
  OMRPORT_ACCESS_FROM_OMRPORT(gcTestEnv->portLib);
  ...
  *rootEntryIndirectPtr = createObject(namePrefixStr, objType, 0, 0, objSize);
  ...
  if (NULL == hashTableAdd(exampleVM->rootTable, &rootEntry)) {
    gcTestEnv->log(
      LEVEL_ERROR, "%s:%d Failed to add new root entry %s to root table!\n", __FILE__, __LINE__, rootEntry.name);
    goto done;
  }
  ...
  while (bytesLeft > 0) {
    ...
    for (int32_t j = 0; j < numOfParents; j++) {
      ...
      for (int32_t k = 0; k < breadth; k++) {
        ...
        if (NULL == parentEntry) {
          gcTestEnv->log(
            LEVEL_ERROR, "%s:%d Could not find object %s in hash table.\n", __FILE__, __LINE__, parentName);
          goto done;
        }
...
int32_t GCConfigTest::processObjNode(pugi::xml_node node, const char* namePrefixStr, OMRGCObjectType objType,
...
int32_t GCConfigTest::attachChildEntry(ObjectEntry* parentEntry, ObjectEntry* childEntry)
{
  int32_t rc = 0;
  ...
  if ((uint32_t)parentEntry->numOfRef < slotCount) {
    ...
    gcTestEnv->log(LEVEL_VERBOSE, "\tadd child %s(%p[0x%llx]) to parent %s(%p[0x%llx]) slot %p[%llx].\n",
      childEntry->name, childEntry->objPtr, childEntry->objPtr->header.raw(), parentEntry->name, parentEntry->objPtr,
      parentEntry->objPtr->header.raw(), childSlot, (uintptr_t)*childSlot);
    parentEntry->numOfRef += 1;
  } else {
    gcTestEnv->log(LEVEL_ERROR,
      "%s:%d Invalid XML input: numOfFields %d defined for %s(%p[0x%llx]) is not enough to hold child reference for "
      "%s(%p[0x%llx]).\n",
      __FILE__, __LINE__, parentEntry->numOfRef, parentEntry->name, parentEntry->objPtr,
      parentEntry->objPtr->header.raw(), childEntry->name, childEntry->objPtr, childEntry->objPtr->header.raw());
    rc = 1;
  }
  return rc;
}
...
int32_t GCConfigTest::removeObjectFromParentSlot(const char* name, ObjectEntry* parentEntry)
{
  MM_GCExtensionsBase* extensions = (MM_GCExtensionsBase*)exampleVM->_omrVM->_gcOmrVMExtensions;
  uintptr_t size = extensions->objectModel.getConsumedSizeInBytesWithHeader(parentEntry->objPtr);
  ...
  while (currentSlot < endSlot) {
    ...
    if (objEntry->objPtr == slotObject.readReferenceFromSlot()) {
      gcTestEnv->log(LEVEL_VERBOSE, "Remove object %s(%p[0x%llx]) from parent %s(%p[0x%llx]) slot %p.\n", name,
        objEntry->objPtr, objEntry->objPtr->header.raw(), parentEntry->name, parentEntry->objPtr,
...
int32_t GCConfigTest::parseGarbagePolicy(pugi::xml_node node)
{
  ...
  if (!node.empty()) {
    ...
    if ((0 != strcmp(gp.frequency, "perObject")) && (0 != strcmp(gp.frequency, "perRootStruct"))) {
      rt = 1;
      gcTestEnv->log(LEVEL_ERROR,
        "%s:%d Invalid XML input: the valid value of attribute \"frequency\" is \"perObject\" or \"perRootStruct\".\n",
        __FILE__, __LINE__);
      goto done;
    }
    ...
    if ((0 != strcmp(gp.structure, "node")) && (0 != strcmp(gp.structure, "tree"))) {
      rt = 1;
      gcTestEnv->log(LEVEL_ERROR,
        "%s:%d Invalid XML input: the valid value of attribute \"structure\" is \"node\" or \"tree\".\n", __FILE__,
        __LINE__);
      goto done;
    }
  }
  ...
  for (; node; node = node.next_sibling()) {
    if (0 == strcmp(node.name(), "systemCollect")) {
      ...
      if (OMR_ERROR_NONE != rt) {
        gcTestEnv->log(
          LEVEL_ERROR, "%s:%d Failed to perform OMR_GC_SystemCollect with error code %d.\n", __FILE__, __LINE__, rt);
        goto done;
      }
      OMRGCTEST_CHECK_RT(rt);
...
INSTANTIATE_TEST_CASE_P(gcFunctionalTest, GCConfigTest, ::testing::ValuesIn(gcTests));
