...
const char* perfTests[] = {
  "perftest/gctest/configuration/21645_core.20150126.202455.11862202.0001.xml",
...
GCConfigTest::SetUp()
{
  ...
  rc = OMR_Thread_Init(
    exampleVM->_omrVM, NULL, &exampleVM->_omrVMThread, "OMRTestThread");
  ...
  omrstr_printf(verboseFile,
                MAX_NAME_LENGTH,
                "%s_%d_%lld.xml",
                verboseFileNamePrefix,
                omrsysinfo_get_pid(),
                omrtime_current_time_millis());
  verboseManager = MM_VerboseManager::newInstance(env, exampleVM->_omrVM);
  verboseManager->configureVerboseGC(
    exampleVM->_omrVM, verboseFile, numOfFiles, numOfCycles);
  ...
  exampleVM->rootTable = hashTableNew(exampleVM->_omrVM->_runtime->_portLibrary,
                                      ...
                                      OMRMEM_CATEGORY_MM,
                                      rootTableHashFn,
                                      rootTableHashEqualFn,
                                      NULL,
                                      NULL);

  /* Initialize object table */
  exampleVM->objectTable =
    hashTableNew(exampleVM->_omrVM->_runtime->_portLibrary,
                 OMR_GET_CALLSITE(),
                 0,
                 sizeof(ObjectEntry),
                 0,
  ...
  if ((NULL != verboseFile) && (false == gcTestEnv->keepLog)) {
    if (0 == numOfFiles) {
      ...
      for (int32_t seq = 1; seq <= (int32_t)numOfFiles; seq++) {
        ...
        omrstr_printf(
          verboseFileSeq, MAX_NAME_LENGTH, "%s.%03zu", verboseFile, seq);
        J9FileStat buf;
        int32_t fileStatRC = -1;
...
GCConfigTest::parseAttribute(AttributeElem** root, const char* attrStr)
{
  ...
  if (NULL == copyOfAttrStr) {
    ...
    gcTestEnv->log(LEVEL_ERROR,
                   "%s:%d Failed to allocate native memory.\n",
                   __FILE__,
                   __LINE__);
    goto done;
  }
...
GCConfigTest::allocateHelper(const char* objName, uintptr_t size)
{
  ...
  if (NULL != objEntry.objPtr) {
    ...
    if (consumedSize == adjustedSize) {
      gcTestEnv->log(LEVEL_VERBOSE,
                     "Allocate object name: %s(%p[0x%llx])\n",
                     objEntry.name,
                     objEntry.objPtr,
                     consumedSize);
    } else {
...
ObjectEntry*
GCConfigTest::createObject(const char* namePrefix,
                           ...
                           uintptr_t size)
{
  ...
  omrstr_printf(
    objName, MAX_NAME_LENGTH, "%s_%d_%d", namePrefix, depth, nthInRow);

  objEntry = find(objName);
  if (NULL != objEntry) {
    gcTestEnv->log(
      LEVEL_VERBOSE, "Found object %s in object table.\n", objEntry->name);
...
int32_t
GCConfigTest::createFixedSizeTree(ObjectEntry** rootEntryIndirectPtr,
...
int32_t
GCConfigTest::processObjNode(pugi::xml_node node,
                             const char* namePrefixStr,
                             OMRGCObjectType objType,
                             ...
                             int32_t depth)
{
  ...
  if ((ROOT == objType) || (GARBAGE_ROOT == objType)) {
    ...
    omrstr_printf(parentName,
                  MAX_NAME_LENGTH,
                  "%s_%d_%d",
                  node.parent().attribute(xs.namePrefix).value(),
                  0,
                  0);
    for (int32_t i = 0; i < breadthElem->value; i++) {
      uintptr_t sizeCalculated =
  ...
  for (int32_t i = 1; i < depth; i++) {
    ...
    for (int32_t j = 0; j < numOfParents; j++) {
      char parentName[MAX_NAME_LENGTH];
      omrstr_printf(
        parentName, sizeof(parentName), "%s_%d_%d", namePrefixStr, (i - 1), j);
...
GCConfigTest::insertGarbage()
{
  ...
  omrstr_printf(fullNamePrefix,
                sizeof(fullNamePrefix),
                "%s_%d",
                gp.namePrefix,
                gp.garbageSeq++);
  int32_t breadth = 2;
  ...
  rt = createFixedSizeTree(&rootObjectEntry,
                           fullNamePrefix,
                           GARBAGE_ROOT,
                           totalSize,
                           objSize,
...
int32_t
GCConfigTest::attachChildEntry(ObjectEntry* parentEntry,
                               ObjectEntry* childEntry)
{
  int32_t rc = 0;
  ...
  if ((uint32_t)parentEntry->numOfRef < slotCount) {
    ...
    standardWriteBarrierStore(exampleVM->_omrVMThread,
                              parentEntry->objPtr,
                              childSlot,
                              childEntry->objPtr);
    gcTestEnv->log(
      ...
      childEntry->name,
      childEntry->objPtr,
      childEntry->objPtr->header.raw(),
      parentEntry->name,
      parentEntry->objPtr,
      parentEntry->objPtr->header.raw(),
      childSlot,
      (uintptr_t)*childSlot);
...
int32_t
GCConfigTest::removeObjectFromParentSlot(const char* name,
...
GCConfigTest::allocationWalker(pugi::xml_node node)
{
   ...
   * tree */
  rt = processObjNode(
    node, namePrefixStr, objType, numOfFieldsElem, breadthElem, depth);
  OMRGCTEST_CHECK_RT(rt);
...
GCConfigTest::verifyVerboseGC(pugi::xpath_node_set verboseGCs)
{
  ...
  do {
    ...
    if (0 == numOfFiles) {
      ...
      omrstr_printf(
        currentVerboseFile, MAX_NAME_LENGTH, "%s.%03zu", verboseFile, seq++);
  ...
  for (; node; node = node.next_sibling()) {
    if (0 == strcmp(node.name(), "systemCollect")) {
      ...
      if (OMR_ERROR_NONE != rt) {
        gcTestEnv->log(
          LEVEL_ERROR,
          "%s:%d Failed to perform OMR_GC_SystemCollect with error code %d.\n",
          __FILE__,
          __LINE__,
          rt);
        goto done;
...
TEST_P(GCConfigTest, test)
{
  ...
  for (; configChild; configChild = configChild.next_sibling()) {
    if (0 == strcmp(configChild.name(), "allocation")) {
      ...
      omrstr_printf(verboseNodeSet,
                    MAX_NAME_LENGTH,
                    "verboseGC[not(@spec) or @spec = '%s']",
                    STRINGFY(SPEC));
