...
const char *gcTests[] = {
    "fvtest/gctest/configuration/sample_GC_config.xml",
...
const char *perfTests[] = {
    "perftest/gctest/configuration/"
    "21645_core.20150126.202455.11862202.0001.xml",
    "perftest/gctest/configuration/24404_core.20140723.091737.5812.0002.xml"};
void GCConfigTest::SetUp() {
  ...
  omr_error_t rc =
      OMR_GC_IntializeHeapAndCollector(exampleVM->_omrVM, &startupManager);
  ASSERT_EQ(OMR_ERROR_NONE, rc)
      << "Setup(): OMR_GC_IntializeHeapAndCollector failed, rc=" << rc;

  /* Attach calling thread to the VM */
  rc = OMR_Thread_Init(exampleVM->_omrVM, NULL, &exampleVM->_omrVMThread,
                       "OMRTestThread");
  ...
  ASSERT_EQ(OMR_ERROR_NONE, rc)
      << "Setup(): OMR_GC_InitializeDispatcherThreads failed, rc=" << rc;
  ...
  if (!result) {
    FAIL() << "Failed to load test configuration file (" << GetParam()
           << ") with error description: " << result.description() << ".";
  }

  /* parse verbose information and initialize verbose manager */
  pugi::xml_node optionNode = doc.select_node("/gc-config/option").node();
  const char *verboseFileNamePrefix =
      optionNode.attribute("verboseLog").value();
  numOfFiles = (uintptr_t)optionNode.attribute("numOfFiles").as_int();
  uintptr_t numOfCycles =
      (uintptr_t)optionNode.attribute("numOfCycles").as_int();
  if (0 == strcmp(verboseFileNamePrefix, "")) {
    verboseFileNamePrefix = "VerboseGCOutput";
  }
  verboseFile =
      (char *)omrmem_allocate_memory(MAX_NAME_LENGTH, OMRMEM_CATEGORY_MM);
  if (NULL == verboseFile) {
    FAIL() << "Failed to allocate native memory.";
  }
  omrstr_printf(verboseFile, MAX_NAME_LENGTH, "%s_%d_%lld.xml",
                verboseFileNamePrefix, omrsysinfo_get_pid(),
                omrtime_current_time_millis());
  verboseManager = MM_VerboseManager::newInstance(env, exampleVM->_omrVM);
  verboseManager->configureVerboseGC(exampleVM->_omrVM, verboseFile, numOfFiles,
                                     numOfCycles);
  gcTestEnv->log("Verbose File: %s\n", verboseFile);
  gcTestEnv->log(LEVEL_VERBOSE,
                 "Verbose GC log name: %s; numOfFiles: %d; numOfCycles: %d.\n",
  ...
  exampleVM->rootTable = hashTableNew(exampleVM->_omrVM->_runtime->_portLibrary,
                                      OMR_GET_CALLSITE(), 0, sizeof(RootEntry),
                                      0, 0, OMRMEM_CATEGORY_MM, rootTableHashFn,
                                      rootTableHashEqualFn, NULL, NULL);

  /* Initialize object table */
  exampleVM->objectTable = hashTableNew(
      exampleVM->_omrVM->_runtime->_portLibrary, OMR_GET_CALLSITE(), 0,
      sizeof(ObjectEntry), 0, 0, OMRMEM_CATEGORY_MM, objectTableHashFn,
      objectTableHashEqualFn, NULL, NULL);
}

void GCConfigTest::TearDown() {
  OMRPORT_ACCESS_FROM_OMRPORT(gcTestEnv->portLib);
  ...
  if ((NULL != verboseFile) && (false == gcTestEnv->keepLog)) {
    if (0 == numOfFiles) {
      ...
      for (int32_t seq = 1; seq <= (int32_t)numOfFiles; seq++) {
        ...
        omrstr_printf(verboseFileSeq, MAX_NAME_LENGTH, "%s.%03zu", verboseFile,
                      seq);
        J9FileStat buf;
        int32_t fileStatRC = -1;
  ...
  if (NULL != exampleVM->_omrVMThread) {
    ...
    ASSERT_EQ(OMR_ERROR_NONE, rc)
        << "TearDown(): OMR_GC_ShutdownDispatcherThreads failed, rc=" << rc;
  }

  /* Detach from VM */
  omr_error_t rc = OMR_Thread_Free(exampleVM->_omrVMThread);
  ASSERT_EQ(OMR_ERROR_NONE, rc)
      << "TearDown(): OMR_Thread_Free failed, rc=" << rc;
...
void GCConfigTest::freeAttributeList(AttributeElem *root) {
  OMRPORT_ACCESS_FROM_OMRPORT(gcTestEnv->portLib);
...
int32_t GCConfigTest::parseAttribute(AttributeElem **root,
                                     const char *attrStr) {
  OMRPORT_ACCESS_FROM_OMRPORT(gcTestEnv->portLib);
  int32_t rt = 0;
  const char DELIM = ',';
  char *copyOfAttrStr =
      (char *)omrmem_allocate_memory(strlen(attrStr) + 1, OMRMEM_CATEGORY_MM);
  char *attrStart = NULL;
  char *attrEnd = NULL;
  ...
  while (NULL != attrEnd) {
    ...
    AttributeElem *attrNode = (AttributeElem *)omrmem_allocate_memory(
        sizeof(AttributeElem), OMRMEM_CATEGORY_MM);
    if (NULL == attrNode) {
      rt = 1;
      gcTestEnv->log(LEVEL_ERROR, "%s:%d Failed to allocate native memory.\n",
                     __FILE__, __LINE__);
...
OMRGCObjectType GCConfigTest::parseObjectType(pugi::xml_node node) {
  ...
  if (0 == strcmp(typeStr, "root")) {
    if (0 == strcmp(parentStr, "allocation")) {
      ...
      gcTestEnv->log(LEVEL_ERROR,
                     "%s:%d Invalid XML input: root object %s can not be child "
                     "of other object!\n",
                     __FILE__, __LINE__, namePrefixStr);
    }
  } else if (0 == strcmp(typeStr, "normal")) {
    if ((0 == strcmp(parentTypeStr, "root")) ||
        (0 == strcmp(parentTypeStr, "normal"))) {
      objType = NORMAL;
    } else {
      gcTestEnv->log(LEVEL_ERROR,
                     "%s:%d Invalid XML input: normal object %s has to be "
                     "child of root or normal object!\n",
                     __FILE__, __LINE__, namePrefixStr);
    }
  } else if (0 == strcmp(typeStr, "garbage")) {
    if (0 == strcmp(parentStr, "allocation")) {
      objType = GARBAGE_ROOT;
    } else if ((0 == strcmp(parentTypeStr, "normal")) ||
               (0 == strcmp(parentTypeStr, "root"))) {
      objType = GARBAGE_TOP;
    } else if (0 == strcmp(parentTypeStr, "garbage")) {
      objType = GARBAGE_CHILD;
    } else {
      gcTestEnv->log(LEVEL_ERROR,
                     "%s:%d Invalid XML input: garbage object %s has to be "
                     "child of root, normal or allocation node!\n",
                     __FILE__, __LINE__, namePrefixStr);
    }
  } else {
    gcTestEnv->log(LEVEL_ERROR,
                   "%s:%d Invalid XML input: object %s type should be root, "
                   "normal or garbage .\n",
                   __FILE__, __LINE__, namePrefixStr);
  }
...
ObjectEntry *GCConfigTest::allocateHelper(const char *objName, uintptr_t size) {
  MM_EnvironmentBase *env =
      MM_EnvironmentBase::getEnvironment(exampleVM->_omrVMThread);
  ...
  MM_ObjectAllocationModel *noGc =
      new (objectAllocationModelSpace) MM_ObjectAllocationModel(
          env, size,
          MM_ObjectAllocationModel::selectObjectAllocationFlags(false, false,
                                                                false, true));
  ...
  if (NULL == objEntry.objPtr) {
    gcTestEnv->log("No free memory to allocate %s of size 0x%llx, GC start.\n",
                   objName, size);
    MM_ObjectAllocationModel *withGc =
        new (objectAllocationModelSpace) MM_ObjectAllocationModel(
            env, size,
            MM_ObjectAllocationModel::selectObjectAllocationFlags(
  ...
  if (NULL != objEntry.objPtr) {
    uintptr_t consumedSize =
        env->getExtensions()->objectModel.getConsumedSizeInBytesWithHeader(
            objEntry.objPtr);
    uintptr_t adjustedSize =
        env->getExtensions()->objectModel.adjustSizeInBytes(size);
    if (consumedSize == adjustedSize) {
      ...
      gcTestEnv->log(LEVEL_ERROR,
                     "Consumed size for allocated object name: %s(%p[0x%llx]) "
                     "!= adjusted request size [0x%llx].\n",
                     objEntry.name, objEntry.objPtr, consumedSize,
                     adjustedSize);
    }
    ...
    gcTestEnv->log(LEVEL_ERROR,
                   "%s:%d No free memory after a GC. Failed to allocate object "
                   "%s of size 0x%llx.\n",
...
ObjectEntry *GCConfigTest::createObject(const char *namePrefix,
                                        OMRGCObjectType objType, int32_t depth,
                                        int32_t nthInRow, uintptr_t size) {
  OMRPORT_ACCESS_FROM_OMRPORT(gcTestEnv->portLib);
  ...
  omrstr_printf(objName, MAX_NAME_LENGTH, "%s_%d_%d", namePrefix, depth,
                nthInRow);
  ...
  if (NULL != objEntry) {
    ...
    if (NULL != objEntry) {
      /* Keep count of the new allocated non-garbage object size for garbage
       * insertion. If the object exists in objectTable, its size is ignored. */
      if ((ROOT == objType) || (NORMAL == objType)) {
        gp.accumulatedSize +=
            env->getExtensions()->objectModel.getSizeInBytesWithHeader(
                objEntry->objPtr);
      }
    } else {
      omrmem_free_memory(objName);
...
int32_t GCConfigTest::createFixedSizeTree(ObjectEntry **rootEntryIndirectPtr,
                                          const char *namePrefixStr,
                                          OMRGCObjectType objType,
                                          uintptr_t totalSize,
                                          uintptr_t objSize, int32_t breadth) {
  OMRPORT_ACCESS_FROM_OMRPORT(gcTestEnv->portLib);
  ...
  *rootEntryIndirectPtr = createObject(namePrefixStr, objType, 0, 0, objSize);
  ...
  while (bytesLeft > 0) {
    ...
    for (int32_t j = 0; j < numOfParents; j++) {
      ...
      omrstr_printf(parentName, sizeof(parentName), "%s_%d_%d", namePrefixStr,
                    (depth - 1), j);
      ObjectEntry *parentEntry = find(parentName);
      if (NULL == parentEntry) {
        gcTestEnv->log(LEVEL_ERROR,
                       "%s:%d Could not find object %s in hash table.\n",
      ...
      for (int32_t k = 0; k < breadth; k++) {
        ...
        ObjectEntry *childEntry =
            createObject(namePrefixStr, objType, depth, nthInRow, objSize);
        if (NULL == childEntry) {
          goto done;
        }
  ...
  *rootEntryIndirectPtr = find(rootEntry.name);
  if (NULL == *rootEntryIndirectPtr) {
    gcTestEnv->log(
        LEVEL_ERROR,
        "%s:%d Could not find root of fixed-size tree %s in object table.\n",
        __FILE__, __LINE__, rootEntry.name);
    goto done;
  }
  rt = 0;
...
int32_t GCConfigTest::processObjNode(
    pugi::xml_node node, const char *namePrefixStr, OMRGCObjectType objType,
  ...
  /* Create objects and store them in the root table or the slot object based on
   * objType. */
  if ((ROOT == objType) || (GARBAGE_ROOT == objType)) {
    for (int32_t i = 0; i < breadthElem->value; i++) {
      uintptr_t sizeCalculated =
          numOfFieldsElem->value * sizeof(fomrobject_t) + sizeof(uintptr_t);
      ObjectEntry *objectEntry =
          createObject(namePrefixStr, objType, 0, i, sizeCalculated);
      if (NULL == objectEntry) {
        goto done;
      }
      numOfFieldsElem = numOfFieldsElem->linkNext;

      /* Add object to root hash table. If it's the root of the garbage tree,
       * temporarily keep it as root for allocating
       * the rest of the tree. Remove it from the root set after the entire
       * garbage tree is allocated.*/
      RootEntry rEntry;
      rEntry.name = objectEntry->name;
      rEntry.rootPtr = objectEntry->objPtr;
      if (NULL == hashTableAdd(exampleVM->rootTable, &rEntry)) {
        gcTestEnv->log(LEVEL_ERROR,
                       "%s:%d Failed to add new root entry to root table!\n",
                       __FILE__, __LINE__);
        goto done;
      }
  ...
  } else if ((NORMAL == objType) || (GARBAGE_TOP == objType) ||
             (GARBAGE_CHILD == objType)) {
    char parentName[MAX_NAME_LENGTH];
    omrstr_printf(parentName, MAX_NAME_LENGTH, "%s_%d_%d",
                  node.parent().attribute(xs.namePrefix).value(), 0, 0);
    for (int32_t i = 0; i < breadthElem->value; i++) {
      uintptr_t sizeCalculated =
          numOfFieldsElem->value * sizeof(fomrobject_t) + sizeof(uintptr_t);
      ObjectEntry *childEntry =
  ...
  /* Create the rest of the tree, if any, defined with depth. First, we set the
   * child object with correct objType. */
  if (ROOT == objType) {
    objType = NORMAL;
  } else if ((GARBAGE_TOP == objType) || (GARBAGE_ROOT == objType)) {
    objType = GARBAGE_CHILD;
  ...
  for (int32_t i = 1; i < depth; i++) {
    ...
    for (int32_t j = 0; j < numOfParents; j++) {
      ...
      for (int32_t k = 0; k < breadthElem->value; k++) {
        uintptr_t sizeCalculated =
            sizeof(omrobjectptr_t) +
            numOfFieldsElem->value * sizeof(fomrobject_t);
        ObjectEntry *childEntry =
            createObject(namePrefixStr, objType, i, nthInRow, sizeCalculated);
        if (NULL == childEntry) {
          goto done;
        }
        ObjectEntry *parentEntry = find(parentName);
...
int32_t GCConfigTest::insertGarbage() {
  OMRPORT_ACCESS_FROM_OMRVM(exampleVM->_omrVM);
  int32_t rt = 0;

  char fullNamePrefix[MAX_NAME_LENGTH];
  omrstr_printf(fullNamePrefix, sizeof(fullNamePrefix), "%s_%d", gp.namePrefix,
                gp.garbageSeq++);
  int32_t breadth = 2;
  ...
  gcTestEnv->log(LEVEL_VERBOSE,
                 "Inserting garbage %s of 0x%llx (%f%% of previous object/tree "
                 "size 0x%llx) ...\n",
                 fullNamePrefix, totalSize, gp.percentage, gp.accumulatedSize);

  ObjectEntry *rootObjectEntry = NULL;
  rt = createFixedSizeTree(&rootObjectEntry, fullNamePrefix, GARBAGE_ROOT,
                           totalSize, objSize, breadth);
...
int32_t GCConfigTest::attachChildEntry(ObjectEntry *parentEntry,
                                       ObjectEntry *childEntry) {
  int32_t rc = 0;
  MM_GCExtensionsBase *extensions =
      (MM_GCExtensionsBase *)exampleVM->_omrVM->_gcOmrVMExtensions;
  uintptr_t size = extensions->objectModel.getConsumedSizeInBytesWithHeader(
      parentEntry->objPtr);
  fomrobject_t *firstSlot = (fomrobject_t *)parentEntry->objPtr + 1;
  fomrobject_t *endSlot =
      (fomrobject_t *)((uint8_t *)parentEntry->objPtr + size);
  ...
  if ((uint32_t)parentEntry->numOfRef < slotCount) {
    ...
    standardWriteBarrierStore(exampleVM->_omrVMThread, parentEntry->objPtr,
                              childSlot, childEntry->objPtr);
    gcTestEnv->log(
        LEVEL_VERBOSE,
        "\tadd child %s(%p[0x%llx]) to parent %s(%p[0x%llx]) slot %p[%llx].\n",
        childEntry->name, childEntry->objPtr, childEntry->objPtr->header.raw(),
        parentEntry->name, parentEntry->objPtr,
        parentEntry->objPtr->header.raw(), childSlot, (uintptr_t)*childSlot);
    parentEntry->numOfRef += 1;
  } else {
    gcTestEnv->log(
        LEVEL_ERROR,
        "%s:%d Invalid XML input: numOfFields %d defined for %s(%p[0x%llx]) is "
        "not enough to hold child reference for %s(%p[0x%llx]).\n",
        __FILE__, __LINE__, parentEntry->numOfRef, parentEntry->name,
        parentEntry->objPtr, parentEntry->objPtr->header.raw(),
        childEntry->name, childEntry->objPtr, childEntry->objPtr->header.raw());
    rc = 1;
  }
  return rc;
...
int32_t GCConfigTest::removeObjectFromRootTable(const char *name) {
  int32_t rt = 1;
  RootEntry searchEntry;
  searchEntry.name = name;
  RootEntry *rootEntry =
      (RootEntry *)hashTableFind(exampleVM->rootTable, &searchEntry);
  if (NULL == rootEntry) {
    gcTestEnv->log(LEVEL_ERROR,
                   "%s:%d Failed to find root object %s in root table.\n",
                   __FILE__, __LINE__, name);
    goto done;
  }
  if (0 != hashTableRemove(exampleVM->rootTable, rootEntry)) {
    gcTestEnv->log(LEVEL_ERROR,
                   "%s:%d Failed to remove root object %s from root table!\n",
                   __FILE__, __LINE__, name);
    goto done;
  }
  gcTestEnv->log(LEVEL_VERBOSE,
                 "Remove object %s(%p[0x%llx]) from root table.\n", name,
                 rootEntry->rootPtr, rootEntry->rootPtr->header.raw());
...
int32_t GCConfigTest::removeObjectFromObjectTable(const char *name) {
  int32_t rt = 1;
  ObjectEntry *foundEntry = find(name);
  if (NULL == foundEntry) {
    gcTestEnv->log(LEVEL_ERROR,
                   "%s:%d Failed to find object %s in object table.\n",
                   __FILE__, __LINE__, name);
    goto done;
  }
  if (0 != hashTableRemove(exampleVM->objectTable, foundEntry)) {
    gcTestEnv->log(LEVEL_ERROR,
                   "%s:%d Failed to remove object %s from object table!\n",
                   __FILE__, __LINE__, name);
    goto done;
  }
  gcTestEnv->log(LEVEL_VERBOSE,
                 "Remove object %s(%p[0x%llx]) from object table.\n", name,
                 foundEntry->objPtr, foundEntry->objPtr->header.raw());
...
int32_t GCConfigTest::removeObjectFromParentSlot(const char *name,
  ...
  while (currentSlot < endSlot) {
    ...
    if (objEntry->objPtr == slotObject.readReferenceFromSlot()) {
      gcTestEnv->log(
          LEVEL_VERBOSE,
          "Remove object %s(%p[0x%llx]) from parent %s(%p[0x%llx]) slot %p.\n",
          name, objEntry->objPtr, objEntry->objPtr->header.raw(),
          parentEntry->name, parentEntry->objPtr,
          parentEntry->objPtr->header.raw(), slotObject.readAddressFromSlot());
  ...
  if (0 != rt) {
    gcTestEnv->log(LEVEL_ERROR,
                   "%s:%d Failed to remove object %s from its parent's slot.\n",
                   __FILE__, __LINE__, name);
    goto done;
  }
...
int32_t GCConfigTest::allocationWalker(pugi::xml_node node) {
  OMRPORT_ACCESS_FROM_OMRVM(exampleVM->_omrVM);
  int32_t rt = 0;
  ...
  if ((0 == strcmp(namePrefixStr, "")) || (0 == strcmp(typeStr, ""))) {
    rt = 1;
    gcTestEnv->log(LEVEL_ERROR,
                   "%s:%d Invalid XML input: please specify namePrefix and "
                   "type for object %s.\n",
                   __FILE__, __LINE__, namePrefixStr);
    goto done;
  }
  ...
  /* process current xml node, perform allocation for single object or object
   * tree */
  rt = processObjNode(node, namePrefixStr, objType, numOfFieldsElem,
                      breadthElem, depth);
  OMRGCTEST_CHECK_RT(rt);
  ...
  /* After the entire garbage tree is allocated, remove garbage root object from
   * the root set or remove garbage top object from the slot of its parent
   * object. */
...
int32_t GCConfigTest::verifyVerboseGC(pugi::xpath_node_set verboseGCs) {
  OMRPORT_ACCESS_FROM_OMRPORT(gcTestEnv->portLib);
  int32_t rt = 0;
  uintptr_t seq = 1;
  size_t numOfNode = verboseGCs.size();
  bool *isFound = (bool *)omrmem_allocate_memory(sizeof(int32_t) * numOfNode,
                                                 OMRMEM_CATEGORY_MM);
  if (NULL == isFound) {
    rt = 1;
    gcTestEnv->log(LEVEL_ERROR, "%s:%d Failed to allocate native memory.\n",
  ...
  do {
    ...
    if (0 == numOfFiles) {
      ...
      omrstr_printf(currentVerboseFile, MAX_NAME_LENGTH, "%s.%03zu",
                    verboseFile, seq++);
    ...
    int32_t i = 0;
    for (pugi::xpath_node_set::const_iterator it = verboseGCs.begin();
         it != verboseGCs.end(); ++it) {
      ...
      if (!resultQuery) {
        rt = 1;
        gcTestEnv->log(LEVEL_ERROR,
                       "%s:%d Invalid xquery string \"%s\" specified in "
                       "configuration file.\n",
      ...
      if (!nodeQuery) {
        rt = 1;
        gcTestEnv->log(LEVEL_ERROR,
                       "%s:%d Invalid xpathNodes string \"%s\" specified in "
                       "configuration file.\n",
      ...
      if (!xpathNodes.empty()) {
        ...
        gcTestEnv->log("Verifying xquery \"%s\" on xpath \"%s\":\n", xqueryStr,
                       xpathNodesStr);
        for (pugi::xpath_node_set::const_iterator it = xpathNodes.begin();
             it != xpathNodes.end(); ++it) {
  ...
  for (size_t i = 0; i < numOfNode; i++) {
    if (!isFound[i]) {
      ...
      gcTestEnv->log(
          LEVEL_ERROR,
          "*FAILED* Could not find xpath node \"%s\" in verbose output.\n",
...
int32_t GCConfigTest::parseGarbagePolicy(pugi::xml_node node) {
  int32_t rt = 0;
  gp.namePrefix = NULL;
  gp.percentage = 0.0f;
  gp.frequency = "none";
  ...
  if (!node.empty()) {
    ...
    if ((0 != strcmp(gp.frequency, "perObject")) &&
        (0 != strcmp(gp.frequency, "perRootStruct"))) {
      rt = 1;
      gcTestEnv->log(LEVEL_ERROR,
                     "%s:%d Invalid XML input: the valid value of attribute "
                     "\"frequency\" is \"perObject\" or \"perRootStruct\".\n",
                     __FILE__, __LINE__);
      goto done;
    }
    ...
    if ((0 != strcmp(gp.structure, "node")) &&
        (0 != strcmp(gp.structure, "tree"))) {
      rt = 1;
      gcTestEnv->log(LEVEL_ERROR,
                     "%s:%d Invalid XML input: the valid value of attribute "
                     "\"structure\" is \"node\" or \"tree\".\n",
                     __FILE__, __LINE__);
      goto done;
    }
  }
...
int32_t GCConfigTest::triggerOperation(pugi::xml_node node) {
  int32_t rt = 0;
  for (; node; node = node.next_sibling()) {
    if (0 == strcmp(node.name(), "systemCollect")) {
      ...
      if (OMR_ERROR_NONE != rt) {
        gcTestEnv->log(LEVEL_ERROR,
                       "%s:%d Failed to perform OMR_GC_SystemCollect with "
                       "error code %d.\n",
...
int32_t GCConfigTest::iniXMLStr(const char *configStyle) {
  int32_t rt = 0;
...
TEST_P(GCConfigTest, test) {
  ...
  ASSERT_EQ(0, iniXMLStr(configStyle))
      << "Invalid XML input: unrecognized gc-config style \"" << configStyle
      << "\".";
  ...
  for (; configChild; configChild = configChild.next_sibling()) {
    if (0 == strcmp(configChild.name(), "allocation")) {
      ...
      for (pugi::xpath_node_set::const_iterator it = objects.begin();
           it != objects.end(); ++it) {
      ...
      omrstr_printf(verboseNodeSet, MAX_NAME_LENGTH,
                    "verboseGC[not(@spec) or @spec = '%s']", STRINGFY(SPEC));
      pugi::xpath_node_set verboseGCs =
          configChild.select_nodes(verboseNodeSet);
      ...
      FAIL() << "Invalid XML input: unrecognized XML node \""
             << configChild.name() << "\" in configuration file.";
    }
  }
}
...
INSTANTIATE_TEST_CASE_P(perfTest, GCConfigTest, ::testing::ValuesIn(perfTests));
