...
static unsigned int flagOptions[] = { SA_SIGINFO,
                                      SA_RESETHAND,
                                      SA_NODEFER,
                                      SA_ONSTACK };
...
extern int
sigactionOS(int, const struct sigaction*, struct sigaction*);
#else
static SIGACTION sigactionOS = NULL;
#endif /* defined(J9ZOS390) */
#endif /* defined(OMR_OS_WINDOWS) */
static sighandler_t handlerOptions[] = { SIG_DFL,
                                         SIG_IGN,
                                         NULL,
                                         handlerPrimary,
                                         handlerSecondary };
...
static omr_error_t
setupExistingHandlerConditions(bool existingPrimary,
                               bool existingSecondary,
                               bool onStack,
                               int signum);
static bool
checkPreviousHandler(int signum,
                     TestAction testFunc,
                     sighandler_t expectedPreviousHandler,
                     sighandler_t previousHandler);
static omr_error_t
performTestAndCheck(int expectedHandlerCalls,
                    bool onStackCondition,
                    bool testingSecondaryHandler,
                    int signum);
#if defined(OMR_OS_WINDOWS)
static omr_error_t
runTest(bool existingPrimary,
        bool existingSecondary,
        sighandler_t action,
        int signum,
        TestAction testFunc);
...
runTest(bool existingPrimary,
        ...
        TestAction testFunc);
static omr_error_t
runSigactionTest(int* expectedHandlerCalls,
                 ...
                 sighandler_t* previousHandler);
static omr_error_t
runPrimarySigactionTest(int* expectedHandlerCalls,
                        bool existingPrimary,
                        bool existingSecondary,
                        int signum,
                        struct sigaction* act,
...
#endif /* defined(OMR_OS_WINDOWS) */
static omr_error_t
runSignalTest(int* expectedHandlerCalls,
              bool existingPrimary,
              bool existingSecondary,
              int signum,
              sighandler_t action,
              ...
              sighandler_t* previousHandler);
static omr_error_t
runHandlerTest(int* expectedHandlerCalls,
               bool existingPrimary,
               bool existingSecondary,
               int signum,
               sighandler_t* expectedPreviousHandler,
               sighandler_t* previousHandler);
static omr_error_t
runPrimarySignalTest(int* expectedHandlerCalls,
                     bool existingPrimary,
                     bool existingSecondary,
                     int signum,
                     sighandler_t action,
...
TEST(OmrSigTest, omrsigReentrancyTest)
{
  ...
  for (int i = 0; i < 2; i += 1) {
    ...
    ASSERT_NO_FATAL_FAILURE(createThread(
      &thread, FALSE, J9THREAD_CREATE_JOINABLE, signalRaisingThread, &signum));
    ...
    EXPECT_TRUE(handlerCalls > 0)
      << "Expected handler to be called for primary test, got " << handlerCalls
      << " calls.";
    handlerCalls = 0;
...
test(TestAction testFunc)
{
  ...
  for (int i = 0; (i < NUM_TEST_CONDITIONS) && (OMR_ERROR_NONE == rc); i += 1) {
    ...
    if (OMR_ERROR_NONE != runTest(existingPrimary,
                                  existingSecondary,
#if defined(OMR_OS_WINDOWS)
...
#endif /* defined(OMR_OS_WINDOWS) */
{
  ...
  if (signum != signumOptions[1]) {
    rc = setupExistingHandlerConditions(
      existingPrimary, existingSecondary, onStack, signum);
  }
  ...
  if (OMR_ERROR_NONE == rc) {
    switch (testFunc) {
#if !defined(OMR_OS_WINDOWS)
      case test_sigaction:
        rc = runSigactionTest(&expectedHandlerCalls,
                              existingPrimary,
                              existingSecondary,
                              signum,
                              act,
                              &expectedPreviousHandler,
                              &previousHandler);
        break;
#endif /* !defined(OMR_OS_WINDOWS) */
      case test_signal:
        rc = runSignalTest(&expectedHandlerCalls,
                           existingPrimary,
                           existingSecondary,
                           signum,
                           action,
      ...
      case test_omrsig_handler:
        rc = runHandlerTest(&expectedHandlerCalls,
                            existingPrimary,
                            existingSecondary,
                            signum,
                            &expectedPreviousHandler,
      ...
      case test_omrsig_primary_signal:
        rc = runPrimarySignalTest(&expectedHandlerCalls,
                                  existingPrimary,
                                  existingSecondary,
                                  signum,
                                  action,
      ...
      case test_omrsig_primary_sigaction:
        rc = runPrimarySigactionTest(&expectedHandlerCalls,
                                     existingPrimary,
                                     existingSecondary,
                                     signum,
                                     act,
  ...
  if (OMR_ERROR_NONE == rc) {
    if (!checkPreviousHandler(
          signum, testFunc, expectedPreviousHandler, previousHandler)) {
      rc = OMR_ERROR_INTERNAL;
