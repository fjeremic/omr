...
static unsigned int flagOptions[] = {SA_SIGINFO, SA_RESETHAND, SA_NODEFER,
                                     SA_ONSTACK};
typedef int (*SIGACTION)(int signum, const struct sigaction *act,
...
#endif /* defined(J9ZOS390) */
#endif /* defined(OMR_OS_WINDOWS) */
static sighandler_t handlerOptions[] = {SIG_DFL, SIG_IGN, NULL, handlerPrimary,
                                        handlerSecondary};

static omr_error_t test(TestAction testFunc);
static omr_error_t setupExistingHandlerConditions(bool existingPrimary,
                                                  bool existingSecondary,
                                                  bool onStack, int signum);
static bool checkPreviousHandler(int signum, TestAction testFunc,
                                 sighandler_t expectedPreviousHandler,
                                 sighandler_t previousHandler);
static omr_error_t performTestAndCheck(int expectedHandlerCalls,
                                       bool onStackCondition,
                                       bool testingSecondaryHandler,
                                       int signum);
#if defined(OMR_OS_WINDOWS)
static omr_error_t runTest(bool existingPrimary, bool existingSecondary,
                           sighandler_t action, int signum,
                           TestAction testFunc);
#else  /* defined(OMR_OS_WINDOWS) */
...
static omr_error_t runTest(bool existingPrimary, bool existingSecondary,
                           struct sigaction *action, bool onStack, int signum,
                           TestAction testFunc);
static omr_error_t runSigactionTest(int *expectedHandlerCalls,
                                    bool existingPrimary,
                                    bool existingSecondary, int signum,
                                    struct sigaction *act,
                                    sighandler_t *expectedPreviousHandler,
                                    sighandler_t *previousHandler);
static omr_error_t runPrimarySigactionTest(
    int *expectedHandlerCalls, bool existingPrimary, bool existingSecondary,
    int signum, struct sigaction *act, sighandler_t *expectedPreviousHandler,
    sighandler_t *previousHandler);
#endif /* defined(OMR_OS_WINDOWS) */
static omr_error_t runSignalTest(int *expectedHandlerCalls,
                                 bool existingPrimary, bool existingSecondary,
                                 int signum, sighandler_t action,
                                 sighandler_t *expectedPreviousHandler,
                                 sighandler_t *previousHandler);
static omr_error_t runHandlerTest(int *expectedHandlerCalls,
                                  bool existingPrimary, bool existingSecondary,
                                  int signum,
                                  sighandler_t *expectedPreviousHandler,
                                  sighandler_t *previousHandler);
static omr_error_t runPrimarySignalTest(int *expectedHandlerCalls,
                                        bool existingPrimary,
                                        bool existingSecondary, int signum,
                                        sighandler_t action,
                                        sighandler_t *expectedPreviousHandler,
...
TEST(OmrSigTest, sigactionTest) {
  EXPECT_TRUE(OMR_ERROR_NONE == test(test_sigaction))
...
TEST(OmrSigTest, omrsig_handlerTest) {
  EXPECT_TRUE(OMR_ERROR_NONE == test(test_omrsig_handler))
      << "omrsig_handler() test failed.";
}

TEST(OmrSigTest, omrsig_primary_signalTest) {
  EXPECT_TRUE(OMR_ERROR_NONE == test(test_omrsig_primary_signal))
      << "omrsig_primary_signal() test failed.";
}

#if !defined(OMR_OS_WINDOWS)
TEST(OmrSigTest, omrsig_primary_sigactionTest) {
  EXPECT_TRUE(OMR_ERROR_NONE == test(test_omrsig_primary_sigaction))
      << "omrsig_primary_sigaction() test failed.";
}

TEST(OmrSigTest, omrsigReentrancyTest) {
  /* Install handler while another thread raises signals, both to itself and the
   * main thread. */
  ...
  for (int i = 0; i < 2; i += 1) {
    ...
    act.sa_handler = handlerPrimary;
    ASSERT_NO_FATAL_FAILURE(createThread(&thread, FALSE,
                                         J9THREAD_CREATE_JOINABLE,
                                         signalRaisingThread, &signum));
    ...
    EXPECT_TRUE(handlerCalls > 0)
        << "Expected handler to be called for primary test, got "
        << handlerCalls << " calls.";
    handlerCalls = 0;
    ...
    /* Check handler calls. */
    EXPECT_TRUE(handlerCalls > 0)
        << "Expected handler to be called for secondary test, got "
        << handlerCalls << " calls.";
  }
}
...
static omr_error_t test(TestAction testFunc) {
  ...
  for (int i = 0; (i < NUM_TEST_CONDITIONS) && (OMR_ERROR_NONE == rc); i += 1) {
     ...
     *     -    -||  	Registering SIG_DFL, SIG_IGN, NULL, or a handler
     * function.
     *     -  ||-    	Signal num to test
    ...
    struct sigaction act = {{0}};
    act.sa_flags = 0;
    if ((test_sigaction == testFunc) ||
        (test_omrsig_primary_sigaction == testFunc)) {
      /* Compose flags. */
      for (int j = 0; j < 4; ++j) {
    ...
    if (OMR_ERROR_NONE != runTest(existingPrimary, existingSecondary,
#if defined(OMR_OS_WINDOWS)
      ...
      printf("Test %d:%d failed at %s:%d. Conditions: %sexisting primary, "
             "%sexisting secondary, flags = %" PRId64
             ", signum is %d, handler is 0x%p, %salt signal stack.\n",
             testFunc, i, __FILE__, __LINE__, existingPrimary ? "" : "no ",
             existingSecondary ? "" : "no ", flags, signum,
             handlerOptions[handlerIndex], onStack ? "" : "no ");
    }
  }
  return rc;
}
...
#if defined(OMR_OS_WINDOWS)
static omr_error_t runTest(bool existingPrimary, bool existingSecondary,
                           sighandler_t act, int signum, TestAction testFunc)
#else  /* defined(OMR_OS_WINDOWS) */
static omr_error_t runTest(bool existingPrimary, bool existingSecondary,
                           struct sigaction *act, bool onStack, int signum,
                           TestAction testFunc)
#endif /* defined(OMR_OS_WINDOWS) */
{
  omr_error_t rc = OMR_ERROR_NONE;
  ...
  if (signum != signumOptions[1]) {
    rc = setupExistingHandlerConditions(existingPrimary, existingSecondary,
                                        onStack, signum);
  }

  /* Calculate the expected number of signal handler calls and call the test
   * function. */
  if (OMR_ERROR_NONE == rc) {
    switch (testFunc) {
#if !defined(OMR_OS_WINDOWS)
    case test_sigaction:
      rc = runSigactionTest(&expectedHandlerCalls, existingPrimary,
                            existingSecondary, signum, act,
                            &expectedPreviousHandler, &previousHandler);
      break;
#endif /* !defined(OMR_OS_WINDOWS) */
    case test_signal:
      rc = runSignalTest(&expectedHandlerCalls, existingPrimary,
                         existingSecondary, signum, action,
                         &expectedPreviousHandler, &previousHandler);
      break;
    case test_omrsig_handler:
      rc = runHandlerTest(&expectedHandlerCalls, existingPrimary,
                          existingSecondary, signum, &expectedPreviousHandler,
                          &previousHandler);
      break;
    case test_omrsig_primary_signal:
      rc = runPrimarySignalTest(&expectedHandlerCalls, existingPrimary,
                                existingSecondary, signum, action,
                                &expectedPreviousHandler, &previousHandler);
      break;
#if !defined(OMR_OS_WINDOWS)
    case test_omrsig_primary_sigaction:
      rc = runPrimarySigactionTest(&expectedHandlerCalls, existingPrimary,
                                   existingSecondary, signum, act,
                                   &expectedPreviousHandler, &previousHandler);
      break;
#endif /* !defined(OMR_OS_WINDOWS) */
  ...
  if (OMR_ERROR_NONE == rc) {
    if (!checkPreviousHandler(signum, testFunc, expectedPreviousHandler,
                              previousHandler)) {
      rc = OMR_ERROR_INTERNAL;
      printf("Incorrect returned previous handler. Found 0x%p, expected 0x%p "
             "at %s:%d.\n",
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    rc = performTestAndCheck(expectedHandlerCalls,
                             ...
                             false, false,
#else  /* defined(OMR_OS_WINDOWS) */
                             onStack && (SA_ONSTACK & act->sa_flags) &&
                                 handlerIsFunction(action) &&
...
/* Set existing primary handler, secondary handler, and/or on stack condition
...
static omr_error_t setupExistingHandlerConditions(bool existingPrimary,
  ...
  if (SIG_ERR == omrsig_primary_signal(signum, SIG_DFL)) {
    rc = OMR_ERROR_INTERNAL;
    printf("Setting primary handler to SIG_DFL before setting test conditions "
           "failed at %s:%d.\n",
           __FILE__, __LINE__);
  }
  if (OMR_ERROR_NONE == rc) {
    if (SIG_ERR == signal(signum, SIG_DFL)) {
      rc = OMR_ERROR_INTERNAL;
      printf("Setting secondary handler to SIG_DFL before setting test "
             "conditions failed at %s:%d.\n",
             __FILE__, __LINE__);
    }
  }
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    if (OMR_ERROR_NONE != rc) {
      printf(
          "Installing existing secondary handler condition failed at %s:%d.\n",
          __FILE__, __LINE__);
    }
  }
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    if (0 != sigaltstack(&stack, NULL)) {
      rc = OMR_ERROR_INTERNAL;
      printf("sigaltstack() failed with error code %d in setting test "
             "conditions at %s:%d.\n",
...
static omr_error_t runPrimarySigactionTest(
  ...
  *expectedHandlerCalls = 2;
  ...
  if ((0 == omrsig_primary_sigaction(signum, act, &oldact)) ==
      (signum == signumOptions[1])) {
    rc = OMR_ERROR_INTERNAL;
    printf("omrsig_primary_sigaction() unexpectedly returned %d, expected %d "
           "at %s:%d.\n",
           (signum == signumOptions[1]) ? 0 : -1,
           (signum == signumOptions[1]) ? -1 : 0, __FILE__, __LINE__);
  }
...
static omr_error_t runHandlerTest(int *expectedHandlerCalls,
  ...
  *expectedHandlerCalls = 0;
#if defined(OMR_OS_WINDOWS)
  /* On Windows, primary is reset after first signal, but jump from handler
   * prevents the reset of the secondary when it is called here.
   */
  if (existingPrimary) {
    *expectedHandlerCalls = 1;
  }
  ...
  if (0 == setjmp(env) && (signum != signumOptions[1])) {
    ...
    if ((OMRSIG_RC_SIGNAL_HANDLED != ret) ==
      ...
      rc = OMR_ERROR_INTERNAL;
      printf(
          "omrsig_handler() unexpectedly returned %s, expected %s at %s:%d.\n",
          (OMRSIG_RC_SIGNAL_HANDLED == ret)
...
static omr_error_t performTestAndCheck(int expectedHandlerCalls,
  ...
  if (signum != signumOptions[1]) {
    ...
    if (OMR_ERROR_INTERNAL == rc) {
      ...
      /* Check that the installed handlers have the correct alternate stack
       * flags. */
      bool currentlyOnstack = (actprimary.sa_flags & SA_ONSTACK) &&
                              handlerIsFunction(&actprimary) &&
                              (SS_DISABLE != stack.ss_flags);
      bool secondaryOnstack =
          ((actsecondary.sa_flags & SA_ONSTACK) &&
           handlerIsFunction(&actsecondary)) == testingSecondaryHandler;
      if ((onStackCondition != currentlyOnstack) &&
          handlerIsFunction(&actprimary) &&
          (actprimary.sa_handler != actsecondary.sa_handler)) {
        rc = OMR_ERROR_INTERNAL;
        printf("Signal alternate stack not correctly set: primary handler "
               "%scurrently on stack while %sin on stack condition at %s:%d.\n",
               currentlyOnstack ? "" : "not ", onStackCondition ? "" : "not ",
               __FILE__, __LINE__);
      } else if (onStackCondition && !secondaryOnstack) {
        rc = OMR_ERROR_INTERNAL;
        printf("Signal alternate stack not correctly set: secondary handler "
               "not on alternate stack while in on stack condition at %s:%d.\n",
               __FILE__, __LINE__);
      }
    }
  }
  ...
  for (int i = 0; i < 2; i += 1) {
    if ((65 > signum) &&
        ((expectedHandlerCalls - handlerCalls > 0) || (SIGABRT != signum))) {
      if (SIGABRT == signum) {
        int newHandlerCalls = sigsetjmp(env, 1);
        if (0 == newHandlerCalls) {
          raise(signum);
...
static void checkSignalMask(int signum, bool expected, bool checkingPrimary) {
  ...
  if (sigismember(&currentMask, signum) != expected) {
    handlerCalls -= 1;
    printf("Incorrect signal mask in %s signal handler. Expected signal to "
           "%sbe masked at %s:%d.\n",
           checkingPrimary ? "primary" : "secondary", expected ? "" : "not ",
           __FILE__, __LINE__);
  }
}
#endif /* !defined(OMR_OS_WINDOWS) */
