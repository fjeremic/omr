...
static omr_error_t test(TestAction testFunc) {
  ...
  for (int i = 0; (i < NUM_TEST_CONDITIONS) && (OMR_ERROR_NONE == rc); i += 1) {
    ...
    if (OMR_ERROR_NONE != runTest(existingPrimary, existingSecondary,
      ...
      printf(
          "Test %d:%d failed at %s:%d. Conditions: %sexisting primary, "
...
#endif /* defined(OMR_OS_WINDOWS) */
{
  ...
  if (OMR_ERROR_NONE == rc) {
    switch (testFunc) {
      ...
      case test_omrsig_primary_sigaction:
        rc = runPrimarySigactionTest(
            &expectedHandlerCalls, existingPrimary, existingSecondary, signum,
  ...
  if (OMR_ERROR_NONE == rc) {
    if (!checkPreviousHandler(signum, testFunc, expectedPreviousHandler,
      ...
      rc = OMR_ERROR_INTERNAL;
      printf(
          "Incorrect returned previous handler. Found 0x%p, expected 0x%p at "
          "%s:%d.\n",
...
static omr_error_t setupExistingHandlerConditions(bool existingPrimary,
  ...
  if (SIG_ERR == omrsig_primary_signal(signum, SIG_DFL)) {
    rc = OMR_ERROR_INTERNAL;
    printf(
        "Setting primary handler to SIG_DFL before setting test conditions "
        "failed at %s:%d.\n",
        __FILE__, __LINE__);
  }
  if (OMR_ERROR_NONE == rc) {
    if (SIG_ERR == signal(signum, SIG_DFL)) {
      rc = OMR_ERROR_INTERNAL;
      printf(
          "Setting secondary handler to SIG_DFL before setting test conditions "
          "failed at %s:%d.\n",
          __FILE__, __LINE__);
    }
  }
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    if (0 != sigaltstack(&stack, NULL)) {
      rc = OMR_ERROR_INTERNAL;
      printf(
          "sigaltstack() failed with error code %d in setting test conditions "
          "at %s:%d.\n",
...
static omr_error_t runPrimarySigactionTest(
  ...
  *expectedHandlerCalls = 2;
  ...
  if ((0 == omrsig_primary_sigaction(signum, act, &oldact)) ==
    ...
    rc = OMR_ERROR_INTERNAL;
    printf(
        "omrsig_primary_sigaction() unexpectedly returned %d, expected %d at "
        "%s:%d.\n",
  ...
  if (signum != signumOptions[1]) {
    ...
    if (OMR_ERROR_INTERNAL == rc) {
      ...
      if ((onStackCondition != currentlyOnstack) &&
        ...
        rc = OMR_ERROR_INTERNAL;
        printf(
            "Signal alternate stack not correctly set: primary handler "
        ...
        rc = OMR_ERROR_INTERNAL;
        printf(
            "Signal alternate stack not correctly set: secondary handler not "
            "on alternate stack while in on stack condition at %s:%d.\n",
            __FILE__, __LINE__);
      }
    }
  }
...
static void checkSignalMask(int signum, bool expected, bool checkingPrimary) {
  ...
  if (sigismember(&currentMask, signum) != expected) {
    handlerCalls -= 1;
    printf(
        "Incorrect signal mask in %s signal handler. Expected signal to %sbe "
        "masked at %s:%d.\n",
