...
omr_error_t omrTestPrintError(
  const char* funcCall, const omr_error_t rc, OMRPortLibrary* portLibrary, const char* callFile, intptr_t callLine)
{
  ...
  OMRPortLibrary* portLibrary, const char* callFile, intptr_t callLine)
{
  if (expectedRC != rc) {
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
    omrtty_printf("%s:%d %s failed, got rc=%d (%s), expected rc=%d (%s)\n", callFile, callLine, funcCall, rc,
      omrErrorToString(rc), expectedRC, omrErrorToString(expectedRC));
  }
  return rc;
}
