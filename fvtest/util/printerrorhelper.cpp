...
omr_error_t omrTestPrintUnexpectedIntRC(const char *funcCall, const intptr_t rc,
  ...
  if (expectedRC != rc) {
    ...
    omrtty_printf("%s:%d %s failed, got rc=%d, expected rc=%d\n", callFile,
                  callLine, funcCall, rc, expectedRC);
...
omr_error_t omrTestPrintUnexpectedRC(const char *funcCall, const omr_error_t rc,
  ...
  if (expectedRC != rc) {
    ...
    omrtty_printf("%s:%d %s failed, got rc=%d (%s), expected rc=%d (%s)\n",
                  ...
                  expectedRC, omrErrorToString(expectedRC));
  }
  return rc;
}
 ...
 */
void outputComment(struct OMRPortLibrary *portLibrary, const char *format,
                   ...) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  va_list args;
