...
extern "C" omr_error_t omrTestVMInit(OMRTestVM *const testVM,
  ...
  if (OMR_ERROR_NONE !=
      OMRTEST_PRINT_ERROR(omr_initialize_runtime(&testVM->omrRuntime))) {
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    if (OMR_ERROR_NONE !=
        OMRTEST_PRINT_ERROR(omr_attach_vm_to_runtime(&testVM->omrVM))) {
      rc = OMR_ERROR_INTERNAL;
    }
  }
  ...
  if (OMR_ERROR_NONE == rc) {
    if (OMR_ERROR_NONE !=
        OMRTEST_PRINT_ERROR(omr_ras_initMemCategories(OMRPORTLIB))) {
      rc = OMR_ERROR_INTERNAL;
    }
  }
  if (OMR_ERROR_NONE == rc) {
...
extern "C" omr_error_t omrTestVMFini(OMRTestVM *const testVM) {
  omr_error_t rc = OMR_ERROR_NONE;
  OMRPORT_ACCESS_FROM_OMRPORT(testVM->portLibrary);

  if (OMR_ERROR_NONE == rc) {
    if (OMR_ERROR_NONE !=
        OMRTEST_PRINT_ERROR(omr_ras_cleanupTI(&testVM->omrVM))) {
      rc = OMR_ERROR_INTERNAL;
    }
  }
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    if (0 != omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
      ...
      while ((testVM->omrVM._languageThreadCount +
              testVM->omrVM._internalThreadCount) > 0) {
  ...
  if (OMR_ERROR_NONE == rc) {
     ...
     * During normal VM operation, it is not permitted to change the memory
     * category definitions after they have been initialized.
     */
  ...
  if (OMR_ERROR_NONE == rc) {
    if (OMR_ERROR_NONE !=
        OMRTEST_PRINT_ERROR(omr_detach_vm_from_runtime(&testVM->omrVM))) {
      rc = OMR_ERROR_INTERNAL;
    }
  }
  ...
  if (OMR_ERROR_NONE == rc) {
    if (OMR_ERROR_NONE !=
        OMRTEST_PRINT_ERROR(omr_destroy_runtime(&testVM->omrRuntime))) {
      rc = OMR_ERROR_INTERNAL;
    }
  }
  return rc;
