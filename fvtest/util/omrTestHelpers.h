 ...
 * ================
 */
#define OMRTEST_PRINT_ERROR(x) \
  omrTestPrintError(#x, (x), OMRPORTLIB, __FILE__, __LINE__)
omr_error_t omrTestPrintError(const char *funcCall, const omr_error_t rc,
                              OMRPortLibrary *portLibrary, const char *callFile,
                              intptr_t callLine);

#define OMRTEST_PRINT_UNEXPECTED_INT_RC(x, exp) \
  omrTestPrintUnexpectedIntRC(#x, (x), (exp), OMRPORTLIB, __FILE__, __LINE__)
omr_error_t omrTestPrintUnexpectedIntRC(const char *funcCall, const intptr_t rc,
                                        const intptr_t expectedRC,
                                        OMRPortLibrary *portLibrary,
                                        const char *callFile,
                                        intptr_t callLine);

#define OMRTEST_PRINT_UNEXPECTED_RC(x, exp) \
  omrTestPrintUnexpectedRC(#x, (x), (exp), OMRPORTLIB, __FILE__, __LINE__)
omr_error_t omrTestPrintUnexpectedRC(const char *funcCall, const omr_error_t rc,
                                     const omr_error_t expectedRC,
                                     OMRPortLibrary *portLibrary,
