...
static void startChildThread(OMRTestVM *testVM, omrthread_t *childThread,
                             omrthread_entrypoint_t entryProc,
                             ChildThreadData **childData);
static omr_error_t waitForChildThread(OMRTestVM *testVM,
                                      omrthread_t childThread,
                                      ChildThreadData *childData);
...
static omr_error_t
countTracepointsIter(void *userData, const char *tpMod,
                     const uint32_t tpModLength, const uint32_t tpId,
                     const UtTraceRecord *record, uint32_t firstParameterOffset,
...
TEST(TraceLifecycleTest, startupShutdownSanity) {
  ...
  OMRTEST_ASSERT_ERROR_NONE(omr_ras_initTraceEngine(
      &testVM.omrVM, "buffers=1k:maximal=all:print=omr_test", "."));

  /* Attach the thread to the trace engine */
  OMRTEST_ASSERT_ERROR_NONE(
      OMR_Thread_Init(&testVM.omrVM, NULL, &vmthread, "stub"));
...
TEST(TraceLifecycleTest, moduleLoadBeforeThreadAttach) {
  ...
  OMRPORT_ACCESS_FROM_OMRPORT(rasTestEnv->getPortLibrary());
  char *datDir =
      getTraceDatDir(rasTestEnv->_argc, (const char **)rasTestEnv->_argv);

  OMRTEST_ASSERT_ERROR_NONE(omrTestVMInit(&testVM, OMRPORTLIB));
  /* use small buffers to exercise buffer wrapping */
  OMRTEST_ASSERT_ERROR_NONE(omr_ras_initTraceEngine(
      &testVM.omrVM, "buffers=1k:maximal=all:print=omr_test", datDir));

  /* Attempt to load a module before attaching the thread. This should fail
   * silently without crashing */
  UT_OMR_TEST_MODULE_LOADED(testVM.omrVM._trcEngine->utIntf);

  /* Attach the thread to the trace engine */
  OMRTEST_ASSERT_ERROR_NONE(OMR_Thread_Init(&testVM.omrVM, NULL, &vmthread,
                                            "moduleLoadBeforeThreadAttach"));

  OMRTEST_ASSERT_ERROR_NONE(ti->RegisterRecordSubscriber(
      vmthread, "moduleLoadBeforeThreadAttach", countTracepoints, NULL,
      (void *)&tpCounts, &subscriptionID));

  /* Attempt to log a tracepoint. Nothing should happen. */
  Trc_OMR_Test_UnloggedTracepoint(
      vmthread,
      "The trace module was loaded before the current thread was attached.");

#if ENABLE_BUGS
  /* This can't be done because there is no check for whether the module was
   * actually loaded. */
  ...
  /*
   * Must not deregister before omr_ras_cleanupTraceEngine() because cleanup
   * flushes the trace buffers. This succeeds for a weird reason:
   * omr_ras_cleanupTraceEngine() actually deregistered all subscribers when it
   * detached the current thread. This Deregister call does nothing.
   */
  OMRTEST_ASSERT_ERROR_NONE(
      ti->DeregisterRecordSubscriber(vmthread, subscriptionID));
...
TEST(TraceLifecycleTest, moduleUnloadAfterThreadDetach) {
  ...
  ASSERT_NO_FATAL_FAILURE(startChildThread(
      &testVM, &childThread, moduleUnloadAfterThreadDetachHelper, &childData));
  ASSERT_EQ(1, omrthread_resume(childThread));
  OMRTEST_ASSERT_ERROR_NONE(
      waitForChildThread(&testVM, childThread, childData));
...
#if ENABLE_BUGS
  /* Attempt to load a module. Note this requires bogus caching of UtInterface*,
   * and using it after it was freed. */

  /* moduleLoaded() crashes on attempt to use omrTraceGlobal */
  UT_OMR_TEST_MODULE_LOADED(utIntf);
  Trc_OMR_Test_UnloggedTracepoint(
      vmthread, "The module was loaded after trace shutdown.");
...
TEST(TraceLifecycleTest, traceAndModuleUnloadAfterTraceShutdown) {
  ...
  ASSERT_EQ(0, omrthread_monitor_init_with_name(&tpCountsMT.lock, 0,
                                                "&tpCountsMT.lock"));
  ...
  OMRTEST_ASSERT_ERROR_NONE(omr_ras_initTraceEngine(
      &testVM.omrVM, "buffers=1k:maximal=all:print=omr_test", datDir));
  OMRTEST_ASSERT_ERROR_NONE(
      OMR_Thread_Init(&testVM.omrVM, NULL, &vmthread, "traceTest"));

  /* start counting tracepoints */
  OMRTEST_ASSERT_ERROR_NONE(ti->RegisterRecordSubscriber(
      vmthread, "traceAndModuleUnloadAfterTraceShutdown", countTracepointsMT,
      NULL, (void *)&tpCountsMT, &subscriptionID));
  ...
  OMRTEST_ASSERT_ERROR_NONE(
      waitForChildThread(&testVM, shutdownTrcThr, shutdownTrcData));

  /* Attempt to log a tracepoint. It will not be logged. */
  Trc_OMR_Test_UnloggedTracepoint(
      vmthread, "Tracepoint initiated after trace engine shutdown is started.");
  ...
  omr_trc_stopThreadTrace(vmthread); /* flush the thread's trace buffer and
                                        delete all subscribers */
  OMRTEST_ASSERT_ERROR_NONE(ti->DeregisterRecordSubscriber(
      vmthread, subscriptionID)); /* do nothing */
...
TEST(TraceLifecycleTest, registerSubscriberAfterShutdown) {
  ...
  /* Attach the thread to the trace engine */
  OMRTEST_ASSERT_ERROR_NONE(OMR_Thread_Init(&testVM.omrVM, NULL, &vmthread,
                                            "registerSubscriberAfterShutdown"));
...
TEST(TraceLifecycleTest, deregisterSubscriberAfterShutdown) {
  ...
  /* Attempt to deregister using external agent API. This succeeds because this
   * thread is still attached to the trace engine. */
  OMRTEST_ASSERT_ERROR_NONE(
      ti->DeregisterRecordSubscriber(vmthread, subscriptionID));
...
TEST(TraceLifecycleTest, rebindAndDeregisterSubscriberAfterShutdown) {
  ...
  /* Attach the thread to the trace engine */
  OMRTEST_ASSERT_ERROR_NONE(
      OMR_Thread_Init(&testVM.omrVM, NULL, &vmthread,
                      "rebindAndDeregisterSubscriberAfterShutdown"));

  /* Register the subscriber */
  OMRTEST_ASSERT_ERROR_NONE(ti->RegisterRecordSubscriber(
      vmthread, "rebindAndDeregisterSubscriberAfterShutdown", subscribeFunc,
  ...
  OMRTEST_ASSERT_ERROR_NONE(
      OMR_Thread_Init(&testVM.omrVM, NULL, &vmthread,
                      "rebindAndDeregisterSubscriberAfterShutdown#2"));
  /* Attempt to deregister using external agent API. This should return
   * OMR_ERROR_NOT_AVAILABLE */
  OMRTEST_ASSERT_ERROR_NONE(
      ti->DeregisterRecordSubscriber(vmthread, subscriptionID));
  OMRTEST_ASSERT_ERROR_NONE(OMR_Thread_Free(vmthread));
...
TEST(TraceLifecycleTest, threadAttachDetachStress) {
  ...
  /* use small buffers to exercise buffer wrapping */
  OMRTEST_ASSERT_ERROR_NONE(
      omr_ras_initTraceEngine(&testVM.omrVM, "buffers=1k:maximal=all", datDir));
  ...
  for (int i = 0; i < attachDetachHelpersCount; i++) {
    ASSERT_NO_FATAL_FAILURE(startChildThread(&testVM, &attachDetachHelpers[i],
                                             attachDetachHelper,
                                             &attachDetachData[i]));
  }
  ASSERT_NO_FATAL_FAILURE(startChildThread(&testVM, &shutdownHelper,
                                           shutdownTraceHelper, &shutdownData));
  ...
  for (int i = 0; i < attachDetachHelpersCount; i++) {
    OMRTEST_ASSERT_ERROR_NONE(waitForChildThread(
        &testVM, attachDetachHelpers[i], attachDetachData[i]));
  }
  OMRTEST_ASSERT_ERROR_NONE(
      waitForChildThread(&testVM, shutdownHelper, shutdownData));
...
static void startChildThread(OMRTestVM *testVM, omrthread_t *childThread,
  ...
  ChildThreadData *newChildData = (ChildThreadData *)omrmem_allocate_memory(
      sizeof(*newChildData), OMRMEM_CATEGORY_VM);
  ...
  ASSERT_NO_FATAL_FAILURE(createThread(
      childThread, TRUE, J9THREAD_CREATE_JOINABLE, entryProc, newChildData));
...
static int J9THREAD_PROC moduleUnloadAfterThreadDetachHelper(void *entryArg) {
  ...
  rc = OMRTEST_PRINT_ERROR(OMR_Thread_Init(
      &testVM->omrVM, NULL, &vmthread, "moduleUnloadAfterThreadDetachHelper"));
...
static int J9THREAD_PROC attachDetachHelper(void *entryArg) {
  ...
  rc = OMRTEST_PRINT_ERROR(
      OMR_Thread_Init(&testVM->omrVM, NULL, &vmthread, "attachDetachHelper"));
  if (OMR_ERROR_NONE != rc) {
  ...
  size_t stringsPerTraceBuffer =
      (1024 + strlen(alphabetUpper) - 1) / strlen(alphabetUpper);
...
static int J9THREAD_PROC shutdownTraceHelper(void *entryArg) {
  ...
  rc = OMRTEST_PRINT_ERROR(
      OMR_Thread_Init(&testVM->omrVM, NULL, &vmthread, "shutdownTraceHelper"));
  if (OMR_ERROR_NONE != rc) {
    childData->childRc = rc;
    return -1;
  }
...
countTracepointsIter(void *userData, const char *tpMod,
  ...
  if ((omr_test_len == tpModLength) &&
      (0 == memcmp("omr_test", tpMod, omr_test_len))) {
...
static omr_error_t countTracepoints(UtSubscription *subscriptionID) {
  ...
  const UtTraceRecord *traceRecord =
      (const UtTraceRecord *)subscriptionID->data;
  if ((omrthread_t)(uintptr_t)traceRecord->threadSyn1 == tpCounts->osThread) {
    processTraceRecord(&tpCounts->wrapBuffer, subscriptionID,
                       countTracepointsIter, (void *)tpCounts);
  }
  return OMR_ERROR_NONE;
}
...
static omr_error_t countTracepointsMT(UtSubscription *subscriptionID) {
  TracePointCountsMT *tpCountsMT =
      (TracePointCountsMT *)subscriptionID->userData;
  const UtTraceRecord *traceRecord =
      (const UtTraceRecord *)subscriptionID->data;
  ...
  for (int i = 0; i < tpCountsMT->numThreads; i += 1) {
    if ((omrthread_t)(uintptr_t)traceRecord->threadSyn1 ==
        tpCountsMT->tpCounts[i].osThread) {
