...
static omr_error_t
countTracepointsIter(void* userData,
                     const char* tpMod,
...
TEST(TraceLifecycleTest, moduleLoadBeforeThreadAttach)
{
  ...
  /* Attach the thread to the trace engine */
  OMRTEST_ASSERT_ERROR_NONE(OMR_Thread_Init(
    &testVM.omrVM, NULL, &vmthread, "moduleLoadBeforeThreadAttach"));

  OMRTEST_ASSERT_ERROR_NONE(
    ti->RegisterRecordSubscriber(vmthread,
...
TEST(TraceLifecycleTest, DISABLE_moduleLoadAfterTraceShutdown)
{
  ...
  /* Attach the thread to the trace engine */
  OMRTEST_ASSERT_ERROR_NONE(OMR_Thread_Init(
    &testVM.omrVM, NULL, &vmthread, "moduleLoadAfterTraceShutdown"));
...
TEST(TraceLifecycleTest, traceAndModuleUnloadAfterTraceShutdown)
{
  ...
  ASSERT_EQ(
    0,
    omrthread_monitor_init_with_name(&tpCountsMT.lock, 0, "&tpCountsMT.lock"));
  ...
  OMRTEST_ASSERT_ERROR_NONE(
    ti->RegisterRecordSubscriber(vmthread,
                                 "traceAndModuleUnloadAfterTraceShutdown",
                                 countTracepointsMT,
                                 NULL,
                                 (void*)&tpCountsMT,
                                 &subscriptionID));
  ...
  omr_trc_stopThreadTrace(
    vmthread); /* flush the thread's trace buffer and delete all subscribers */
  OMRTEST_ASSERT_ERROR_NONE(
    ti->DeregisterRecordSubscriber(vmthread, subscriptionID)); /* do nothing */
...
TEST(TraceLifecycleTest, registerSubscriberAfterShutdown)
{
  ...
  /* Attach the thread to the trace engine */
  OMRTEST_ASSERT_ERROR_NONE(OMR_Thread_Init(
    &testVM.omrVM, NULL, &vmthread, "registerSubscriberAfterShutdown"));
...
TEST(TraceLifecycleTest, threadAttachDetachStress)
{
  ...
  for (int i = 0; i < attachDetachHelpersCount; i++) {
    ASSERT_NO_FATAL_FAILURE(startChildThread(&testVM,
                                             &attachDetachHelpers[i],
                                             attachDetachHelper,
                                             &attachDetachData[i]));
  }
  ASSERT_NO_FATAL_FAILURE(startChildThread(
    &testVM, &shutdownHelper, shutdownTraceHelper, &shutdownData));
  ...
  for (int i = 0; i < attachDetachHelpersCount; i++) {
    OMRTEST_ASSERT_ERROR_NONE(
      waitForChildThread(&testVM, attachDetachHelpers[i], attachDetachData[i]));
  }
  OMRTEST_ASSERT_ERROR_NONE(
...
countTracepoints(UtSubscription* subscriptionID)
{
  ...
  if ((omrthread_t)(uintptr_t)traceRecord->threadSyn1 == tpCounts->osThread) {
    processTraceRecord(&tpCounts->wrapBuffer,
                       subscriptionID,
                       countTracepointsIter,
                       (void*)tpCounts);
  }
  return OMR_ERROR_NONE;
}
