...
static const char *sstrstr(const char *haystack, const char *needle,
                           size_t length);
 ...
 protected:
  virtual void SetUp() {
    OMRTEST_ASSERT_ERROR_NONE(
        omrTestVMInit(&testVM, rasTestEnv->getPortLibrary()));
...
TEST_F(RASMethodDictionaryTest, Test0) {
  OMRTEST_ASSERT_ERROR_NONE(omr_ras_initMethodDictionary(
      &testVM.omrVM, propertyCount, propertyNames));

  OMRPORT_ACCESS_FROM_OMRPORT(testVM.portLibrary);
  OMR_MethodDictionaryEntry *newEntry =
      (OMR_MethodDictionaryEntry *)omrmem_allocate_memory(
          sizeof(OMR_MethodDictionaryEntry) +
              sizeof(const char *) * propertyCount,
  ...
  OMRTEST_ASSERT_ERROR_NONE(
      omr_ras_insertMethodDictionary(&testVM.omrVM, newEntry));
  ...
  size_t getSizeof = 0;
  OMRTEST_ASSERT_ERROR_NONE(ti->GetMethodProperties(
      vmthread, &getPropertyCount, &getPropertyNames, &getSizeof));
  ...
  void *methodArray[numMethods] = {(void *)0, (void *)2, (void *)3, (void *)4,
                                   (void *)1};
  ...
  char nameBuffer[50];
  OMRTEST_ASSERT_ERROR_NONE(ti->GetMethodDescriptions(
      vmthread, methodArray, numMethods, (OMR_SampledMethodDescription *)desc,
      nameBuffer, sizeof(nameBuffer), &firstRetryMethod, &nameBytesRemaining));
...
TEST_F(RASMethodDictionaryTest, TestNegativeCasesForGetMethodProperties) {
  ...
  OMRTEST_ASSERT_ERROR(OMR_ERROR_NOT_AVAILABLE,
                       ti->GetMethodProperties(vmthread, &getPropertyCount,
...
TEST_F(RASMethodDictionaryTest, TestNegativeCasesForGetMethodDescriptions) {
   ...
   * zero nameBytes
   */
  OMRTEST_ASSERT_ERROR(OMR_ERROR_RETRY,
                       ti->GetMethodDescriptions(
                           vmthread, methodArray, numMethods,
  ...
  OMRTEST_ASSERT_ERROR(
      ...
      ti->GetMethodDescriptions(vmthread, methodArray, numMethods,
                                ...
                                sizeof(nameBuffer), &firstRetryMethod,
                                &nameBytesRemaining));

  /* zero methodArrayCount */
  OMRTEST_ASSERT_ERROR_NONE(ti->GetMethodDescriptions(
      vmthread, methodArray, (size_t)0, (OMR_SampledMethodDescription *)desc,
      nameBuffer, sizeof(nameBuffer), &firstRetryMethod, NULL));
  ASSERT_STREQ(origBuffer, nameBuffer);

  /* NULL methodArray */
  OMRTEST_ASSERT_ERROR(
      OMR_ERROR_ILLEGAL_ARGUMENT,
      ti->GetMethodDescriptions(vmthread, NULL, numMethods,
                                (OMR_SampledMethodDescription *)desc,
  ...
  OMRTEST_ASSERT_ERROR(
      OMR_ERROR_ILLEGAL_ARGUMENT,
      ti->GetMethodDescriptions(vmthread, methodArray, numMethods, NULL,
                                nameBuffer, sizeof(nameBuffer),
                                &firstRetryMethod, &nameBytesRemaining));
...
TEST_F(RASMethodDictionaryTest, TestUndersizedNameBytes) {
  ...
  ASSERT_TRUE(sstrstr(nameBuffer, "JKLMNOPQRSTUVWXYZ", sizeof(nameBuffer)) !=
              NULL);
  ...
  OMRTEST_ASSERT_ERROR_NONE(ti->GetMethodDescriptions(
      vmthread, methodArray, numMethods, (OMR_SampledMethodDescription *)desc,
      newNameBuffer, sizeof(newNameBuffer), &newFirstRetryMethod,
      &newNameBytesRemaining));
...
TEST_F(RASMethodDictionaryTest, TestUndersizedMethodArrayCount) {
  ...
  OMRTEST_ASSERT_ERROR_NONE(
      ti->GetMethodDescriptions(vmthread, methodArray, numMethods - 1,
                                (OMR_SampledMethodDescription *)desc,
                                nameBuffer, sizeof(nameBuffer), NULL, NULL));
  ASSERT_STRNE(origBuffer, nameBuffer);
 ...
 * Return a pointer of the first occurrence of needle in the haystack, or NULL
 * if haystack does not contain the needle.
 */
