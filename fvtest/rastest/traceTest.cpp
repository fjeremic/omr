...
static omr_error_t startTestChildThread(OMRTestVM *testVM,
                                        OMR_VMThread *curVMThread,
                                        omrthread_t *childThread,
                                        TestChildThreadData **childData);
static omr_error_t waitForTestChildThread(OMRTestVM *testVM,
                                          omrthread_t childThead,
                                          TestChildThreadData *childData);
...
TEST(RASTraceTest, TraceAgent) {
  ...
  OMRTEST_ASSERT_ERROR(
      omr_ras_initTraceEngine(&testVM.omrVM, "print=all:duck=quack", datDir),
      OMR_ERROR_ILLEGAL_ARGUMENT);

  OMRTEST_ASSERT_ERROR_NONE(
      omr_ras_initTraceEngine(&testVM.omrVM, "maximal=all", datDir));
  ...
  OMRTEST_ASSERT_ERROR_NONE(
      startTestChildThread(&testVM, vmthread, &childThread, &childData));

  OMRTEST_ASSERT_ERROR_NONE(
      waitForTestChildThread(&testVM, childThread, childData));
  ...
  ASSERT_FALSE(NULL == agent)
      << "testAgent: createAgent() sampleSubscriber failed";
  ...
  ASSERT_FALSE(NULL == agent)
      << "testAgent: createAgent() bindthreadagent failed";
...
static omr_error_t startTestChildThread(OMRTestVM *testVM,
  ...
  if (OMR_ERROR_NONE == rc) {
    if (0 != omrthread_monitor_init_with_name(&newChildData->shutdownCond, 0,
                                              "traceTestChildShutdown")) {
...
static int J9THREAD_PROC childThreadMain(void *entryArg) {
  ...
  Trc_OMR_Test_String(vmthread, "This tracepoint should be ignored. Trace is "
                                "not yet enabled for this thread.");

  rc = OMRTEST_PRINT_ERROR(
      OMR_Thread_Init(&testVM->omrVM, NULL, &vmthread, "traceTest-child"));
  if (OMR_ERROR_NONE != rc) {
  ...
  Trc_OMR_Test_String(vmthread, "This tracepoint should be ignored. Trace has "
                                "been disabled for this thread.");
