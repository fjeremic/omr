...
static void
preforkSetup(OMR_VMThread** vmthread,
             struct OMR_Agent** agent,
             OMRTestVM* testVM,
             int pipedata[2]);
static void
postForkChild(OMR_VMThread* vmthread,
              struct OMR_Agent* agent,
              OMRTestVM* testVM,
              int pipedata[2]);
static void
postForkParent(OMR_VMThread* vmthread,
               struct OMR_Agent* agent,
               OMRTestVM* testVM,
               int pipedata[2]);
static int
...
static void
recordChildErrorStatus(OMRTraceSubscriberForkTestResult* result,
                       omr_error_t rc,
                       const char* filename,
                       int lineno);
...
runTest(omrthread_entrypoint_t entrypoint)
{
  ...
  if (NULL != entrypoint) {
    ...
    for (int32_t i = 0; i < NUM_THREADS; i += 1) {
      ...
      ASSERT_NO_FATAL_FAILURE(createThread(&newTestData.threads[i],
                                           FALSE,
              ...
              int pipedata[2])
{
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    Trc_OMR_Test_ManyParms(
      vmthread, "Test SubscriberAgentWithJ9thread!", vmthread, 10);
...
duringForkBufferTest(void* entryArg)
{
  ...
  OMR_VMThread* vmthread = NULL;
  siblingRc = OMRTEST_PRINT_ERROR(
    OMR_Thread_Init(&testVM->omrVM, NULL, &vmthread, "traceTestBuffer-child"));
...
duringForkThreadTest(void* entryArg)
{
  ...
  if (OMR_ERROR_NONE == siblingRc) {
    ...
    for (int32_t i = 0; i < STRESS_ITERATIONS_THREAD; i += 1) {
      ...
      createThread(&newTestData.threads[i],
                   FALSE,
                   J9THREAD_CREATE_JOINABLE,
                   doNothing,
                   &testThreadData[i]);
...
doNothing(void* entryArg)
{
  ...
  OMR_VMThread* vmthread = NULL;
  siblingRc = OMRTEST_PRINT_ERROR(
    OMR_Thread_Init(&testVM->omrVM, NULL, &vmthread, "traceTestThread-child"));
  if (OMR_ERROR_NONE == siblingRc) {
    omrthread_sleep(200);
    siblingRc = OMRTEST_PRINT_ERROR(OMR_Thread_Free(vmthread));
  }
...
setupTestData(TestData* testData, OMRTestVM* testVM, int32_t threadCount)
{
  ...
  rc = OMRTEST_PRINT_UNEXPECTED_INT_RC(
    omrthread_monitor_init_with_name(
      &testData->readyCond, 0, "traceTestShutdown"),
    J9THREAD_SUCCESS);
...
waitForThreadsDone(TestData* testData)
{
  ...
  for (int32_t i = 0; i < testData->threadCount; i += 1) {
    ...
    if (OMR_ERROR_NONE != testData->siblingRc[i]) {
      ...
      omrtty_printf(
        "%s:%d ERROR: Failure occurred in thread %d.\n", __FILE__, __LINE__, i);
    }
  }
