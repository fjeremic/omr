...
static void preforkSetup(OMR_VMThread **vmthread, struct OMR_Agent **agent,
                         OMRTestVM *testVM, int pipedata[2]);
static void postForkChild(OMR_VMThread *vmthread, struct OMR_Agent *agent,
                          OMRTestVM *testVM, int pipedata[2]);
static void postForkParent(OMR_VMThread *vmthread, struct OMR_Agent *agent,
                           OMRTestVM *testVM, int pipedata[2]);
...
static omr_error_t setupTestData(TestData *testData, OMRTestVM *testVM,
                                 int32_t threadCount);
...
static void recordChildErrorStatus(OMRTraceSubscriberForkTestResult *result,
                                   omr_error_t rc, const char *filename,
                                   int lineno);
...
static void runTest(omrthread_entrypoint_t entrypoint) {
  ...
  if (NULL != entrypoint) {
    OMRTEST_ASSERT_ERROR_NONE(
        setupTestData(&newTestData, &testVM, NUM_THREADS));
    ...
    for (int32_t i = 0; i < NUM_THREADS; i += 1) {
      ...
      ASSERT_NO_FATAL_FAILURE(createThread(&newTestData.threads[i], FALSE,
                                           J9THREAD_CREATE_JOINABLE, entrypoint,
                                           &threadData[i]));
    }
...
static void preforkSetup(OMR_VMThread **vmthread, struct OMR_Agent **agent,
  ...
  OMRTEST_ASSERT_ERROR_NONE(
      OMR_Thread_Init(&testVM->omrVM, NULL, vmthread, "subscriberForkTest"));

  datDir = getTraceDatDir(rasTestEnv->_argc, (const char **)rasTestEnv->_argv);
  OMRTEST_ASSERT_ERROR_NONE(
      omr_ras_initTraceEngine(&testVM->omrVM, trcOpts, datDir));
  OMRTEST_ASSERT_ERROR_NONE(
      omr_trc_startThreadTrace(*vmthread, "initialization thread"));

  /* load agent */
  *agent = omr_agent_create(&testVM->omrVM, "subscriberAgent");
  ASSERT_FALSE(NULL == *agent)
      << "testAgent: createAgent() subscriberAgent failed";
 ...
 */
void postForkChild(OMR_VMThread *vmthread, struct OMR_Agent *agent,
                   OMRTestVM *testVM, int pipedata[2]) {
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    Trc_OMR_Test_ManyParms(vmthread, "Test SubscriberAgentWithJ9thread!",
                           vmthread, 10);
...
void postForkParent(OMR_VMThread *vmthread, struct OMR_Agent *agent,
                    OMRTestVM *testVM, int pipedata[2]) {
  /*  Fire some trace points! */
  Trc_OMR_Test_Init();
  Trc_OMR_Test_Ptr(vmthread, vmthread);
  ...
  EXPECT_FALSE(0 == rc)
      << "Timed out waiting for child process; possible deadlock.";

  if (0 != rc) {
    ssize_t readBytes = read(pipedata[0], (int *)&result, sizeof(result));
    EXPECT_TRUE(readBytes == sizeof(result))
        << "Didn't read back enough from pipe";
    EXPECT_TRUE(0 == result.rc)
        << "Failure occurred in child process at " << result.failureFile << ":"
        << result.failureLine;
  }
  close(pipedata[0]);
  close(pipedata[1]);
...
static int duringForkBufferTest(void *entryArg) {
  ...
  OMR_VMThread *vmthread = NULL;
  siblingRc = OMRTEST_PRINT_ERROR(OMR_Thread_Init(
      &testVM->omrVM, NULL, &vmthread, "traceTestBuffer-child"));
...
static int duringForkThreadTest(void *entryArg) {
  ...
  if (OMR_ERROR_NONE == siblingRc) {
    ...
    for (int32_t i = 0; i < STRESS_ITERATIONS_THREAD; i += 1) {
      ...
      createThread(&newTestData.threads[i], FALSE, J9THREAD_CREATE_JOINABLE,
                   doNothing, &testThreadData[i]);
...
static int doNothing(void *entryArg) {
  ...
  OMR_VMThread *vmthread = NULL;
  siblingRc = OMRTEST_PRINT_ERROR(OMR_Thread_Init(
      &testVM->omrVM, NULL, &vmthread, "traceTestThread-child"));
  if (OMR_ERROR_NONE == siblingRc) {
    omrthread_sleep(200);
    siblingRc = OMRTEST_PRINT_ERROR(OMR_Thread_Free(vmthread));
  }
...
static omr_error_t setupTestData(TestData *testData, OMRTestVM *testVM,
  ...
  testData->siblingRc = (omr_error_t *)omrmem_allocate_memory(
      sizeof(omr_error_t) * threadCount, OMRMEM_CATEGORY_VM);
  testData->threads = (omrthread_t *)omrmem_allocate_memory(
      sizeof(omrthread_t) * threadCount, OMRMEM_CATEGORY_VM);
  rc = OMRTEST_PRINT_UNEXPECTED_INT_RC(
      omrthread_monitor_init_with_name(&testData->readyCond, 0,
                                       "traceTestShutdown"),
      J9THREAD_SUCCESS);
...
static omr_error_t waitForThreadsReady(TestData *testData) {
  ...
  while ((0 < testData->waitingThreadCount) && (OMR_ERROR_NONE == rc)) {
    rc = OMRTEST_PRINT_UNEXPECTED_INT_RC(
        omrthread_monitor_wait_timed(testData->readyCond, TIMEOUT * 1000, 0),
        0);
  }
...
static omr_error_t waitForThreadsDone(TestData *testData) {
  ...
  for (int32_t i = 0; i < testData->threadCount; i += 1) {
    rc = OMRTEST_PRINT_UNEXPECTED_INT_RC(omrthread_join(testData->threads[i]),
                                         J9THREAD_SUCCESS);
