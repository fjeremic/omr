...
static omr_error_t
initTestData(OMRAgentTestData* testData, OMR_VM* vm, char const* outfile);
static omr_error_t
...
static omr_error_t
startTestChildThread(OMR_TI const* ti,
                     OMR_VM* testVM,
...
static omr_error_t
waitForTestChildThread(OMR_TI const* ti,
                       OMR_VM* testVM,
                       omrthread_t childThread,
                       TestChildThreadData* childData);
...
OMRAgent_OnUnload(OMR_TI const* ti, OMR_VM* vm)
{
  ...
  if (OMR_ERROR_NONE == rc) {
    rc =
      OMRTEST_PRINT_ERROR(ti->BindCurrentThread(vm, "test thread3", &vmThread));
  }

  if (OMR_ERROR_NONE == rc) {
    if (0 != testData2.completed) {
      omrtty_printf(
        "%s:%d ERROR: Failed to call alarmFunc2()\n", __FILE__, __LINE__);
                     ...
                     TestChildThreadData** childData)
{
  ...
  if (OMR_ERROR_NONE == rc) {
    rc =
      OMRTEST_PRINT_UNEXPECTED_INT_RC(threadAPI->omrthread_attr_set_detachstate(
                                        &attr, J9THREAD_CREATE_JOINABLE),
                                      J9THREAD_SUCCESS);

    if (OMR_ERROR_NONE == rc) {
      rc = OMRTEST_PRINT_UNEXPECTED_INT_RC(
        threadAPI->omrthread_create_ex(
          childThread, &attr, FALSE, childThreadMain, newChildData),
        J9THREAD_SUCCESS);
    }
    if (OMR_ERROR_NONE == rc) {
...
testTraceAPIs(OMR_TI const* ti, OMR_VM* vm, const char* threadName)
{
  ...
  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(
      ti->RegisterRecordSubscriber(testData1.vmThread,
                                   "sample1",
                                   subscribeFunc1,
                                   alarmFunc1,
                                   (void*)&testData1,
                                   &testData1.subscriptionID));
  }

  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(
      ti->RegisterRecordSubscriber(testData2.vmThread,
                                   "sample2",
                                   subscribeFunc2,
                                   alarmFunc2,
                                   (void*)&testData2,
                                   &testData2.subscriptionID));
  }
...
subscribeFunc1(UtSubscription* subscriptionID)
{
  ...
  fwrite(
    subscriptionID->data, subscriptionID->dataLength, 1, testData1.outfile);
...
subscribeFunc2(UtSubscription* subscriptionID)
{
  ...
  fwrite(
    subscriptionID->data, subscriptionID->dataLength, 1, testData2.outfile);
  return OMR_ERROR_NONE;
}
