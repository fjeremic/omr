...
static omr_error_t initTestData(OMRAgentTestData* testData,
                                OMR_VM* vm,
                                char const* outfile);
static omr_error_t verifyTraceMetadata(char* traceMeta, OMR_VM* vm);

static int J9THREAD_PROC childThreadMain(void* entryArg);
static omr_error_t startTestChildThread(OMR_TI const* ti,
                                        OMR_VM* testVM,
                                        OMR_VMThread* curVMThread,
                                        omrthread_t* childThread,
                                        TestChildThreadData** childData);
static omr_error_t testTraceAPIs(OMR_TI const* ti,
                                 OMR_VM* vm,
                                 const char* threadName);
static omr_error_t waitForTestChildThread(OMR_TI const* ti,
                                          OMR_VM* testVM,
                                          omrthread_t childThread,
                                          TestChildThreadData* childData);
...
static omr_error_t initTestData(struct OMRAgentTestData* testData,
                                OMR_VM* vm,
                                char const* outfile) {
  omr_error_t rc = OMR_ERROR_NONE;
  OMRPORT_ACCESS_FROM_OMRVM(vm);
