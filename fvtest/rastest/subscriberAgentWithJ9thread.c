...
static omr_error_t initTestData(OMRAgentTestData *testData, OMR_VM *vm,
                                char const *outfile);
static omr_error_t verifyTraceMetadata(char *traceMeta, OMR_VM *vm);

static int J9THREAD_PROC childThreadMain(void *entryArg);
static omr_error_t startTestChildThread(OMR_TI const *ti, OMR_VM *testVM,
                                        OMR_VMThread *curVMThread,
                                        omrthread_t *childThread,
                                        TestChildThreadData **childData);
static omr_error_t testTraceAPIs(OMR_TI const *ti, OMR_VM *vm,
                                 const char *threadName);
static omr_error_t waitForTestChildThread(OMR_TI const *ti, OMR_VM *testVM,
                                          omrthread_t childThread,
                                          TestChildThreadData *childData);
...
omr_error_t OMRAgent_OnLoad(OMR_TI const *ti, OMR_VM *vm, char const *options,
  ...
  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(
        startTestChildThread(ti, vm, curThread, &childThread, &childData));
  }
  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(
        waitForTestChildThread(ti, vm, childThread, childData));
  }
...
omr_error_t OMRAgent_OnUnload(OMR_TI const *ti, OMR_VM *vm) {
  ...
  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(
        ti->BindCurrentThread(vm, "test thread3", &vmThread));
  }
  ...
  /* OMR_ERROR_ILLEGAL_ARGUMENT is expected if alarmFunc1 is executed, otherwise
   * expect OMR_ERROR_NONE to return */
  if (OMR_ERROR_NONE == rc) {
    omr_error_t testRc =
        ti->DeregisterRecordSubscriber(vmThread, testData1.subscriptionID);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != testRc && OMR_ERROR_NONE != testRc) {
      rc = testRc;
      omrtty_printf("%s:%d ERROR: DeregisterRecordSubscriber() returns "
                    "unexpected rc: %d\n",
  ...
  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(
        ti->DeregisterRecordSubscriber(vmThread, testData2.subscriptionID));
  }
...
static omr_error_t startTestChildThread(OMR_TI const *ti, OMR_VM *testVM,
  ...
  TestChildThreadData *newChildData =
      (TestChildThreadData *)omrmem_allocate_memory(sizeof(*newChildData),
                                                    OMRMEM_CATEGORY_VM);
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    rc = OMRTEST_PRINT_UNEXPECTED_INT_RC(threadAPI->omrthread_attr_init(&attr),
                                         J9THREAD_SUCCESS);
  }
  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_UNEXPECTED_INT_RC(
        threadAPI->omrthread_attr_set_detachstate(&attr,
                                                  J9THREAD_CREATE_JOINABLE),
        J9THREAD_SUCCESS);

    if (OMR_ERROR_NONE == rc) {
      rc = OMRTEST_PRINT_UNEXPECTED_INT_RC(
          threadAPI->omrthread_create_ex(childThread, &attr, FALSE,
    ...
    rc = OMRTEST_PRINT_UNEXPECTED_INT_RC(
        threadAPI->omrthread_attr_destroy(&attr), J9THREAD_SUCCESS);
  }
...
static omr_error_t waitForTestChildThread(OMR_TI const *ti, OMR_VM *testVM,
  ...
  childRc = OMRTEST_PRINT_UNEXPECTED_INT_RC(
      threadAPI->omrthread_join(childThread), J9THREAD_SUCCESS);
...
static int J9THREAD_PROC childThreadMain(void *entryArg) {
  ...
  childData->childRc =
      testTraceAPIs(childData->ti, childData->testVM, "child thread");
  return 0;
}
...
static omr_error_t testTraceAPIs(OMR_TI const *ti, OMR_VM *vm,
  ...
  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(
        ti->BindCurrentThread(vm, "test thread1", &testData1.vmThread));
  }

  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(
        ti->BindCurrentThread(vm, "test thread2", &testData2.vmThread));
  }
  ...
  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(
        ti->GetTraceMetadata(testData1.vmThread, &traceMeta, &traceMetaLength));
    if (OMR_ERROR_NONE == rc) {
  ...
  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(ti->RegisterRecordSubscriber(
        testData1.vmThread, "sample1", subscribeFunc1, alarmFunc1,
        (void *)&testData1, &testData1.subscriptionID));
  }

  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(ti->RegisterRecordSubscriber(
        testData2.vmThread, "sample2", subscribeFunc2, alarmFunc2,
        (void *)&testData2, &testData2.subscriptionID));
  }
...
static omr_error_t subscribeFunc1(UtSubscription *subscriptionID) {
  ...
  fwrite(subscriptionID->data, subscriptionID->dataLength, 1,
         testData1.outfile);
...
static omr_error_t initTestData(struct OMRAgentTestData *testData, OMR_VM *vm,
                                char const *outfile) {
  omr_error_t rc = OMR_ERROR_NONE;
  OMRPORT_ACCESS_FROM_OMRVM(vm);
...
static omr_error_t verifyTraceMetadata(char *traceMeta, OMR_VM *vm) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (eyecatcherASCII[i] != c && eyecatcherEBCDIC[i] != c) {
      rc = OMR_ERROR_INTERNAL;
      omrtty_printf("eyecatcher in trace metadata at index %d doesn't match "
                    "expectation. Found: %c\n",
                    i, c);
      break;
    }
