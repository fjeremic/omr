...
TEST(TraceLogTest, stressTraceBufferManagement) {
   ...
   *
   * maximal=!j9thr: Disable j9thr tracepoints because the trace engine uses
   * monitors, and it is unsafe to log tracepoints from a omrthread function
   * that manipulates monitor state. In particular, j9thr.17 is fired from
   * unblock_spinlock_threads() via
   * omrthread_monitor_exit(omrVM->_vmThreadListMutex) in
   * OMR_Thread_FirstInit().
   */
  OMRTEST_ASSERT_ERROR_NONE(omr_ras_initTraceEngine(
      &testVM.omrVM, "buffers=1k:maximal=all:maximal=!j9thr", NULL));
  OMRTEST_ASSERT_ERROR_NONE(OMR_Thread_Init(&testVM.omrVM, NULL, &vmthread,
                                            "stressBufferManagement"));

  /* load traceagent */
  ASSERT_FALSE(NULL ==
               (agent = omr_agent_create(&testVM.omrVM,
                                         "sampleSubscriber=traceLogTest.trc")));
  ...
  OMRTEST_ASSERT_ERROR_NONE(ti->RegisterRecordSubscriber(
      vmthread, "fail", failOnSecondCall, failOnSecondCallAlarm,
      (void *)&failData, &failedSubscription));

  for (size_t i = 0; i < NUM_CHILD_THREADS; i += 1) {
    ASSERT_NO_FATAL_FAILURE(startChildThread(&testVM, &childThread[i],
                                             childThreadMain, &childData[i]));

    /* Create a subscription for each thread */
    OMRTEST_ASSERT_ERROR_NONE(ti->RegisterRecordSubscriber(
        vmthread, "child", countTracepoints, NULL, (void *)&childData[i],
        &subscriptionID[i]));
  }
  for (size_t i = 0; i < NUM_CHILD_THREADS; i += 1) {
  ...
  for (size_t i = 0; i < NUM_CHILD_THREADS; i += 1) {
    OMRTEST_ASSERT_ERROR_NONE(
        waitForChildThread(&testVM, childThread[i], &childData[i]));
  }
  /* All tracepoints from the child threads should have been published when they
   * terminated */
  ...
  /* Implementation detail: The callCount is guaranteed because subscriber
   * callbacks are invoked under mutex. */
...
static void startChildThread(OMRTestVM *testVM, omrthread_t *childThread,
  ...
  ASSERT_NO_FATAL_FAILURE(createThread(
      childThread, TRUE, J9THREAD_CREATE_JOINABLE, entryProc, childData));
...
static int J9THREAD_PROC childThreadMain(void *entryArg) {
  ...
  const size_t stringsPerTraceBuffer =
      (TRACE_BUFFER_BYTES + dataLength - 1) / dataLength;
  /* Fill up at least 5 trace buffers */
  const size_t repeatCount = stringsPerTraceBuffer * 5;

  Trc_OMR_Test_UnloggedTracepoint(vmthread,
                                  "Trace is not yet enabled for this thread.");

  rc = OMRTEST_PRINT_ERROR(
      OMR_Thread_Init(&testVM->omrVM, NULL, &vmthread, "childThreadMain"));
  ...
  Trc_OMR_Test_UnloggedTracepoint(vmthread,
                                  "Trace has been disabled for this thread.");
  return 0;
}
      ...
      if (1 == tpId) { /* String tracepoint, data includes terminating nul */
        if (1 == childData->traceDataCount) {
          ...
          if ((parameterDataLength != strlen(childData->traceData[0]) + 1) ||
              (0 !=
               memcmp(childData->traceData[0], data, parameterDataLength))) {
            fprintf(
                stderr,
...
static omr_error_t countTracepoints(UtSubscription *subscriptionID) {
  TestChildThreadData *childData =
      (TestChildThreadData *)subscriptionID->userData;
  ...
  if ((omrthread_t)(uintptr_t)traceRecord->threadSyn1 == childData->osThread) {
    processTraceRecord(&childData->wrapBuffer, subscriptionID,
                       countTracepointsIter, subscriptionID->userData);
  }
  return OMR_ERROR_NONE;
}
 ...
 */
static omr_error_t failOnSecondCall(UtSubscription *subscriptionID) {
  FailingSubscriberData *failData =
      (FailingSubscriberData *)subscriptionID->userData;
  omr_error_t rc = OMR_ERROR_NONE;
