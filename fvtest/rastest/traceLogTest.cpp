...
TEST(TraceLogTest, stressTraceBufferManagement)
{
  ...
  OMRTEST_ASSERT_ERROR_NONE(
    OMR_Thread_Init(&testVM.omrVM, NULL, &vmthread, "stressBufferManagement"));

  /* load traceagent */
  ASSERT_FALSE(NULL == (agent = omr_agent_create(
                          &testVM.omrVM, "sampleSubscriber=traceLogTest.trc")));
  ...
  OMRTEST_ASSERT_ERROR_NONE(ti->RegisterRecordSubscriber(vmthread,
                                                         "fail",
                                                         failOnSecondCall,
                                                         failOnSecondCallAlarm,
                                                         (void*)&failData,
                                                         &failedSubscription));

  for (size_t i = 0; i < NUM_CHILD_THREADS; i += 1) {
    ASSERT_NO_FATAL_FAILURE(startChildThread(
      &testVM, &childThread[i], childThreadMain, &childData[i]));
...
countTracepoints(UtSubscription* subscriptionID)
{
  ...
  if ((omrthread_t)(uintptr_t)traceRecord->threadSyn1 == childData->osThread) {
    processTraceRecord(&childData->wrapBuffer,
                       subscriptionID,
