...
omr_error_t OMRAgent_OnUnload(OMR_TI const* ti, OMR_VM* vm) {
  ...
  if (OMR_ERROR_NONE == rc) {
    omr_error_t testRc = ti->DeregisterRecordSubscriber(
        testData1.vmThread, testData1.subscriptionID);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != testRc && OMR_ERROR_NONE != testRc) {
      omrtty_printf(
          "%s:%d ERROR: DeregisterRecordSubscriber() returns expected rc: %d\n",
          __FILE__, __LINE__, rc);
    }
  }

  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(ti->DeregisterRecordSubscriber(
        testData2.vmThread, testData2.subscriptionID));
  }

  /* Call DeregisterRecordSubscriber one more time, OMR_ERROR_ILLEGAL_ARGUMENT
   * is expected */
  if (OMR_ERROR_NONE == rc) {
    omr_error_t testRc = OMRTEST_PRINT_UNEXPECTED_RC(
        ti->DeregisterRecordSubscriber(testData2.vmThread,
...
static omr_error_t verifyTraceMetadata(char* traceMeta) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (eyecatcherASCII[i] != c && eyecatcherEBCDIC[i] != c) {
      rc = OMR_ERROR_INTERNAL;
      fprintf(stderr,
              "eyecatcher in trace metadata at index %d doesn't match "
              "expectation. Found: %c\n",
              i, c);
      break;
    }
