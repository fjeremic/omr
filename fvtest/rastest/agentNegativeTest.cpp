...
INSTANTIATE_TEST_CASE_P(InvalidAgentOpts,
                        RASAgentNegativeTest,
...
TEST_F(RASAgentNegativeTest, AgentReturnErrorC) {
  struct OMR_Agent* agent =
      omr_agent_create(&testVM.omrVM, "invalidAgentReturnError=abc");
  ASSERT_FALSE(NULL == agent) << "testAgent: createAgent() failed";
  OMRTEST_ASSERT_ERROR_NONE(omr_agent_openLibrary(agent));
  OMRTEST_ASSERT_ERROR(OMR_ERROR_OUT_OF_NATIVE_MEMORY,
                       omr_agent_callOnLoad(agent));
  OMRTEST_ASSERT_ERROR(OMR_ERROR_ILLEGAL_ARGUMENT,
...
TEST_F(RASAgentNegativeTest, AgentReturnErrorCPP) {
  OMR_Agent* agent =
      OMR_Agent::createAgent(&testVM.omrVM, "invalidAgentReturnError=abc");
  ASSERT_FALSE(NULL == agent) << "testAgent: createAgent() failed";
 ...
 *  - arg is empty string
 *
 * This test is disabled until it is fixed. We expect it has consistent behavior
 * as arg is NULL
 */
 ...
 *  - call omr_agent_create() and OMR_Agent::createAgent by providing the empty
 * agent file
...
TEST_F(RASAgentNegativeTest, CorruptedAgent) {
  ...
  ASSERT_FALSE(NULL == agentC)
      << "testAgent: createAgent() " << agentName << " failed";
  OMRTEST_ASSERT_ERROR(OMR_ERROR_ILLEGAL_ARGUMENT,
                       omr_agent_openLibrary(agentC));
