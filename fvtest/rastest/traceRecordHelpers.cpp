...
omr_error_t processTraceRecord(
  PerThreadWrapBuffer* wrapBuffer, UtSubscription* subscriptionID, TracePointCallback userCallback, void* userData)
{
  ...
  if (-1 == record->nextEntry) {
    ...
    /*
     * Cache the data in the record after the last complete tracepoint, in case the last tracepoint wrapped to the next
     * buffer.
     *
  ...
  PerThreadWrapBuffer* wrapBuffer, const UtTraceRecord* record, TracePointIterator* iter)
{
  ...
  if ((tpLength > offset) || ((int32_t)(offset + 1 - tpLength) < record->firstEntry)) {
    ...
    parseTracePoint((const UtTraceRecord*)wrapBuffer->lastRecord, (uint32_t)(wrapBuffer->lastRecordPos - tpLength - 1),
      tpLength, iter);
