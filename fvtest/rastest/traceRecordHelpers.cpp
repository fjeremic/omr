...
static TracePointIteratorState nextTracePoint(PerThreadWrapBuffer *wrapBuffer,
                                              const UtTraceRecord *record,
                                              TracePointIterator *iter);
static TracePointIteratorState parseTracePoint(const UtTraceRecord *record,
                                               uint32_t offset,
                                               uint32_t tpLength,
                                               TracePointIterator *iter);
static uint32_t getTraceIdFromBuffer(const UtTraceRecord *record,
                                     uint32_t offset);
...
omr_error_t processTraceRecord(PerThreadWrapBuffer *wrapBuffer,
  ...
  if (recordSize > subscriptionID->dataLength) {
    fprintf(stderr,
            "ERROR: trace buffer (%d bytes) doesn't contain last entry (%d "
            "bytes)\n",
  ...
  if (-1 == record->nextEntry) {
    /*
     * If (record->nextEntry == -1), then this entire buffer is spanned by a
     * tracepoint. Add it to the lastRecord cache.
     * TODO test this
     */

    size_t bytesNeeded = subscriptionID->dataLength - record->firstEntry;
    if ((wrapBuffer->lastRecordPos + bytesNeeded) <
        wrapBuffer->lastRecordBytes) {
      wrapBuffer->lastRecordBytes += bytesNeeded;
      wrapBuffer->lastRecord = (uint8_t *)realloc(wrapBuffer->lastRecord,
                                                  wrapBuffer->lastRecordBytes);
    }
    memcpy(wrapBuffer->lastRecord + wrapBuffer->lastRecordPos,
           (uint8_t *)record + record->firstEntry, bytesNeeded);
    ...
    /*
     * We iterate tracepoints backwards, starting from the last tracepoint (the
     * newest tracepoint) in the buffer. Each tracepoint's length is recorded at
     * the end of its data.
     */

    /* Set up the iterator */
    iter.currentPos = record->nextEntry; /* offset to the length byte of the
                                            last tracepoint in the buffer */
    ...
    /* We might have cached some data from the previous trace record even if
     * there was no wrapped tracepoint. Throw away the cached data.
     */
    wrapBuffer->lastRecordPos = 0;

    /*
     * Cache the data in the record after the last complete tracepoint, in case
     * the last tracepoint wrapped to the next buffer.
     *
     * record->nextEntry points to the last byte of the last complete
     * tracepoint. Wrapped data starts at (record->nextEntry + 1).
     */
    ...
    if (wrappedDataSize > 0) {
      /* bytesNeeded = (sizeof UtTraceRecord + thread name) + (data following
       * the last complete trace record) */
      ...
      memcpy(wrapBuffer->lastRecord + record->firstEntry,
             (uint8_t *)record + wrappedDataStart, wrappedDataSize);
      wrapBuffer->lastRecordPos = bytesNeeded;
...
static TracePointIteratorState nextTracePoint(PerThreadWrapBuffer *wrapBuffer,
  ...
  if (iter->currentPos < record->firstEntry) {
    /* currentPos is at or before the start of the data - not possible to
     * continue, or the normal exit condition.
     */
    return STOP;
  }
  ...
  /* Check whether we are about to process a wrapped tracepoint (a tracepoint
   * that spans multiple buffers, not a circular trace buffer).
   *
   * Subscribers shouldn't be given circular buffers.
   * TODO When transitioning from internal to external trace, could the first
   * trace buffer be circular?
   *
   * 'offset' points to location of 'tpLength' for current tracepoint.
   * 'offset - tpLength' points to previous tracepoint's 'tpLength', if it
   * exists. 'offset + 1 - tpLength' points to start of current tracepoint's
   * data, which must be at least 'record->firstEntry' if the tracepoint fits in
   * this buffer.
   */
  if ((tpLength > offset) ||
      ((int32_t)(offset + 1 - tpLength) < record->firstEntry)) {
    /* This tracepoint is wrapped from the previous buffer.
     * This must also be the first tracepoint in this buffer, so we'll stop
     * processing this buffer after this tracepoint.
     */
    ...
    parseTracePoint((const UtTraceRecord *)wrapBuffer->lastRecord,
                    (uint32_t)(wrapBuffer->lastRecordPos - tpLength - 1),
                    tpLength, iter);
  ...
  /* move currentPos to the end of the next tracepoint, where its length is
   * written */
 ...
 * @param[in] offset	Current tracepoint's data offset from record (actually,
 * the length marker of the prev tp)
...
static TracePointIteratorState parseTracePoint(const UtTraceRecord *record,
  ...
  if (traceId == 0x0010) { /* 0x0010nnnn8 == lost record tracepoint */
    fprintf(stderr, "ERROR: *** lost records found in trace buffer - use "
                    "external formatting tools for details.\n");
    return STOP;
    ...
    longTracePointLength = getU8FromTraceRecord(
        record, offset + TRACEPOINT_RAW_DATA_TP_ID_OFFSET + 2);
    /* the next call to nextTracePoint() will pick this up and use it
     * appropriately */
    ...
    /* Module name must not be longer than (the tracepoint length - start of
     * module name string) */
    /* modNameLength == 0 -- no modname - most likely partially overwritten -
     * it's unformattable as a result! */
    if ((0 != modNameLength) &&
      ...
      if (traceId > 257) {
        ...
        const char *modName =
            &recordAsCharBuf[offset +
                             TRACEPOINT_RAW_DATA_MODULE_NAME_DATA_OFFSET];
        uint32_t parameterOffset = offset +
                                   TRACEPOINT_RAW_DATA_MODULE_NAME_DATA_OFFSET +
                                   modNameLength;
        ...
        if (OMR_ERROR_NONE !=
            iter->userCallback(iter->userData, modName, modNameLength, traceId,
                               record, parameterOffset, parameterDataLength,
                               iter->isBigEndian)) {
...
uint32_t getU32FromTraceRecord(const UtTraceRecord *record, uint32_t offset,
                               int32_t isBigEndian) {
  uint32_t ret = 0;
  uint8_t data[4];
...
uint64_t getU64FromTraceRecord(const UtTraceRecord *record, uint32_t offset,
                               int32_t isBigEndian) {
  uint64_t ret = 0;
  uint8_t data[8];
