...
nextTracePoint(PerThreadWrapBuffer* wrapBuffer,
               const UtTraceRecord* record,
               TracePointIterator* iter);
static TracePointIteratorState
parseTracePoint(const UtTraceRecord* record,
                uint32_t offset,
                uint32_t tpLength,
                TracePointIterator* iter);
static uint32_t
                   ...
                   void* userData)
{
  ...
  if (recordSize > subscriptionID->dataLength) {
    fprintf(
      stderr,
      "ERROR: trace buffer (%d bytes) doesn't contain last entry (%d bytes)\n",
  ...
  if (-1 == record->nextEntry) {
    ...
    memcpy(wrapBuffer->lastRecord + wrapBuffer->lastRecordPos,
           (uint8_t*)record + record->firstEntry,
           bytesNeeded);
    ...
    if (wrappedDataSize > 0) {
      ...
      memcpy(wrapBuffer->lastRecord + record->firstEntry,
             (uint8_t*)record + wrappedDataStart,
             wrappedDataSize);
      wrapBuffer->lastRecordPos = bytesNeeded;
    ...
    if ((0 != modNameLength) &&
      ...
      if (traceId > 257) {
        ...
        uint32_t parameterOffset =
          offset + TRACEPOINT_RAW_DATA_MODULE_NAME_DATA_OFFSET + modNameLength;
        uint32_t parameterDataLength =
          tpLength -
          (TRACEPOINT_RAW_DATA_MODULE_NAME_DATA_OFFSET + modNameLength);
        if (OMR_ERROR_NONE != iter->userCallback(iter->userData,
                                                 modName,
                                                 modNameLength,
                                                 traceId,
                                                 record,
