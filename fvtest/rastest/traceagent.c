...
omr_error_t OMRAgent_OnLoad(OMR_TI const* ti, OMR_VM* vm, char const* options, OMR_AgentCallbacks* agentCallbacks, ...)
{
  ...
  if (OMR_ERROR_NONE == rc) {
    omr_error_t apiRc = OMRTEST_PRINT_ERROR(ti->RegisterRecordSubscriber(
      vmThread, "sample", subscribeFunc, alarmFunc, (void*)"my user data", &testData.subscriptionID));
    if (OMR_ERROR_NONE != apiRc) {
      rc = OMR_ERROR_INTERNAL;
    }
  }
...
static omr_error_t testTraceAgentFromUnattachedThread(OMR_TI const* ti, OMRPortLibrary* portLibrary)
{
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetTraceMetadata(vmThread, &traceMeta, &traceMetaLength), OMR_THREAD_NOT_ATTACHED);
    if (OMR_THREAD_NOT_ATTACHED != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
  }
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(ti->RegisterRecordSubscriber(vmThread, "sample", subscribeFunc, alarmFunc,
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetMethodProperties(vmThread, &methodPropertyCount, &methodPropertyNames, &sizeofSampledMethodDesc),
      OMR_THREAD_NOT_ATTACHED);
    if (OMR_THREAD_NOT_ATTACHED != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(ti->GetMethodDescriptions(vmThread, methodArray, numMethods, desc, nameBuffer,
                                       sizeof(nameBuffer), NULL, &nameBytesRemaining),
      OMR_THREAD_NOT_ATTACHED);
    if (OMR_THREAD_NOT_ATTACHED != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
...
static omr_error_t testTraceAgentGetMemorySize(OMR_VMThread* vmThread, OMR_TI const* ti)
{
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    if (OMR_ERROR_NONE != rc) {
      ...
      omrtty_printf(
        "%s:%d Free physical memory size (in bytes): %llu, rc = %d (%s), the function call is successful !\n", __FILE__,
        __LINE__, memorySize, rc, omrErrorToString(rc));
    }
  }
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    if (OMR_ERROR_NONE != rc) {
      ...
      omrtty_printf(
        "%s:%d Process virtual memory size (in bytes): %llu, rc = %d (%s), the function call is successful !\n",
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    if (OMR_ERROR_NONE != rc) {
      ...
      omrtty_printf(
        "%s:%d Process physical memory size (in bytes): %llu, rc = %d (%s), the function call is successful !\n",
        __FILE__, __LINE__, memorySize, rc, omrErrorToString(rc));
    }
  }
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    if (OMR_ERROR_NONE != rc) {
      ...
      omrtty_printf(
        "%s:%d Process private memory size (in bytes): %llu, rc = %d (%s), the function call is successful !\n",
        __FILE__, __LINE__, memorySize, rc, omrErrorToString(rc));
    }
  }
...
static omr_error_t testNegativeCases(OMR_VMThread* vmThread, OMR_TI const* ti)
{
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->RegisterRecordSubscriber(vmThread, NULL, subscribeFunc, alarmFunc, (void*)"my data", &subscriptionID),
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->RegisterRecordSubscriber(vmThread, "test", NULL, alarmFunc, (void*)"my data", &subscriptionID),
      OMR_ERROR_ILLEGAL_ARGUMENT);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->RegisterRecordSubscriber(vmThread, "test", subscribeFunc, alarmFunc, (void*)"my data", NULL),
      OMR_ERROR_ILLEGAL_ARGUMENT);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetFreePhysicalMemorySize(vmThread, invalidMemorySizePtr), OMR_ERROR_ILLEGAL_ARGUMENT);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
  }
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetProcessPrivateMemorySize(vmThread, invalidMemorySizePtr), OMR_ERROR_ILLEGAL_ARGUMENT);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
  }
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetProcessVirtualMemorySize(vmThread, invalidMemorySizePtr), OMR_ERROR_ILLEGAL_ARGUMENT);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
  }
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetProcessPhysicalMemorySize(vmThread, invalidMemorySizePtr), OMR_ERROR_ILLEGAL_ARGUMENT);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
  }
  ...
  /* Test GetFreePhysicalMemorySize and GetProcessPrivateMemorySize on platforms other than LINUX, AIXPPC and WIN,
   * OMR_ERROR_NOT_AVAILABLE is expected */
#if !defined(LINUX) && !defined(AIXPPC) && !defined(OMR_OS_WINDOWS) && !defined(OSX)
  if (OMR_ERROR_NONE == testRc) {
  ...
  /* Test GetProcessVirtualMemorySize and GetProcessPhysicalMemorySize on platforms other than LINUX and WIN,
   * OMR_ERROR_NOT_AVAILABLE is expected */
