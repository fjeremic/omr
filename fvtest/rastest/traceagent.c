...
static omr_error_t testTraceAgentFromUnattachedThread(
    OMR_TI const *ti, OMRPortLibrary *portLibrary);
...
static omr_error_t testTraceAgentGetMemorySize(OMR_VMThread *vmThread,
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    if (OMR_ERROR_NONE != rc) {
      ...
      omrtty_printf(
          "%s:%d Free physical memory size (in bytes): %llu, rc = %d (%s), the "
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    if (OMR_ERROR_NONE != rc) {
      ...
      omrtty_printf(
          "%s:%d Process virtual memory size (in bytes): %llu, rc = %d (%s), "
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    if (OMR_ERROR_NONE != rc) {
      ...
      omrtty_printf(
          "%s:%d Process physical memory size (in bytes): %llu, rc = %d (%s), "
          "the function call is successful !\n",
          __FILE__, __LINE__, memorySize, rc, omrErrorToString(rc));
    }
  }
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    if (OMR_ERROR_NONE != rc) {
      ...
      omrtty_printf(
          "%s:%d Process private memory size (in bytes): %llu, rc = %d (%s), "
          "the function call is successful !\n",
          __FILE__, __LINE__, memorySize, rc, omrErrorToString(rc));
    }
  }
