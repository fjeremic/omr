...
static omr_error_t testBindUnbind(OMR_TI const *ti, OMR_VM *vm,
...
static omr_error_t testBindUnbind(OMR_TI const *ti, OMR_VM *vm,
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    if (OMR_ERROR_NONE != rc) {
      ...
      printf("%s: recursive BindCurrentThread failed, NULL vmThread2 was "
             "returned\n",
             agentName);
      rc = OMR_ERROR_INTERNAL;
    } else if (vmThread != vmThread2) {
      printf("%s: recursive BindCurrentThread failed, vmThread (%p) != "
             "vmThread2 (%p)\n",
