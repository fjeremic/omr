...
static omr_error_t testBindUnbind(OMR_TI const* ti,
                                  OMR_VM* vm,
...
omr_error_t OMRAgent_OnLoad(OMR_TI const* ti,
  ...
  omr_error_t rc =
      OMRTEST_PRINT_ERROR(testBindUnbind(ti, vm, "Initial thread"));
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    rc = OMRTEST_PRINT_ERROR(
        ti->BindCurrentThread(vm, "Initial thread", &curThread));
    if (OMR_ERROR_NONE == rc) {
      rc = OMRTEST_PRINT_ERROR(
...
static omr_error_t startTestChildThread(OMR_TI const* ti,
  ...
  if (OMR_ERROR_NONE == rc) {
    if (0 != threadAPI->omrthread_monitor_init_with_name(
  ...
  if (OMR_ERROR_NONE == rc) {
    if (0 != threadAPI->omrthread_create_ex(NULL,                  /* handle */
...
static omr_error_t waitForTestChildThread(OMR_TI const* ti,
   ...
   * Sleep for 0.2 sec to avoid unloading the agent library before the child
   * thread has completed. For more detail, please see JAZZ103 74016
   */
...
static omr_error_t testBindUnbind(OMR_TI const* ti,
  ...
  if (OMR_ERROR_NONE == rc) {
    ...
    if (OMR_ERROR_NONE != rc) {
      ...
      printf(
          "%s: recursive BindCurrentThread failed, NULL vmThread2 was "
          "returned\n",
          agentName);
      rc = OMR_ERROR_INTERNAL;
    } else if (vmThread != vmThread2) {
      printf(
          "%s: recursive BindCurrentThread failed, vmThread (%p) != vmThread2 "
          "(%p)\n",
