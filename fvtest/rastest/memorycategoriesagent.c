...
static omr_error_t
testMemoryCategoriesFromAttachedThread(OMR_VMThread* vmThread,
                                       OMR_TI const* ti,
                                       OMRPortLibrary* portLibrary);
static omr_error_t
testMemoryCategoriesFromUnattachedThread(OMR_VMThread* vmThread,
                                         OMR_TI const* ti,
                                         OMRPortLibrary* portLibrary);

static BOOLEAN
validateCategories(OMRPortLibrary* portLibrary,
                   OMR_TI_MemoryCategory* categories_buffer,
                   int32_t written_count);
static void
printMemoryCategories(OMRPortLibrary* portLib,
                      OMR_TI_MemoryCategory* categories_buffer,
                      int32_t written_count);
                                       ...
                                       OMRPortLibrary* portLibrary)
{
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    if (NULL == memoryCategories) {
      ...
      memset(
        memoryCategories, 0, total_categories * sizeof(OMR_TI_MemoryCategory));
    }
  }
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetMemoryCategories(
        vmThread, total_categories, memoryCategories, NULL, &total_categories),
      OMR_ERROR_ILLEGAL_ARGUMENT);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
  ...
  if (OMR_ERROR_NONE == testRc) {
    omrtty_printf(
      "\nNegative path: get categories with an undersized buffer\n");
    rc =
      OMRTEST_PRINT_UNEXPECTED_RC(ti->GetMemoryCategories(vmThread,
                                                          total_categories - 1,
                                                          memoryCategories,
                                                          &written_count,
                                                          &total_categories),
    ...
    if (OMR_ERROR_OUT_OF_NATIVE_MEMORY != rc) {
      ...
      if (-1 == written_count) {
        ...
        testRc = OMR_ERROR_INTERNAL;
                   ...
                   OMRPORTLIB, memoryCategories, written_count)) {
        testRc = OMR_ERROR_INTERNAL;
      }
  ...
  if (OMR_ERROR_NONE == testRc) {
    omrtty_printf("\nPositive path: get and validate categories\n");
    rc = OMRTEST_PRINT_ERROR(ti->GetMemoryCategories(vmThread,
                                                     total_categories,
                                                     memoryCategories,
                                                     &written_count,
                                                     &total_categories));
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetMemoryCategories(vmThread,
                              (total_categories + 1),
                              memoryCategories,
                              NULL,
                              &total_categories),
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetMemoryCategories(
        vmThread, (total_categories + 1), NULL, &written_count, NULL),
      OMR_ERROR_ILLEGAL_ARGUMENT);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
...
static void
printMemoryCategoryLine(OMRPortLibrary* portLib,
                        ...
                        int depth)
{
  ...
  omrtty_printf("%s %ld bytes / %ld allocations",
                category->name,
                category->liveBytesDeep,
...
static void
printMemoryCategory(OMRPortLibrary* portLib,
                    OMR_TI_MemoryCategory* category,
                    int depth)
{
 ...
 */
static BOOLEAN
validateCategoryPointer(OMRPortLibrary* portLibrary,
                   ...
                   int32_t written_count)
{
  ...
  for (i = 0; i < written_count; i++) {
    ...
    if (thisCategory->parent != NULL) {
      ...
      if (!validateCategoryPointer(portLibrary,
                                   thisCategory->parent,
                                   categories_buffer,
                                   written_count)) {
        omrtty_err_printf(
          "%s:%d:Parent link for category %s  failed validation. Index = %d\n",
          __FILE__,
          __LINE__,
          thisCategory->name,
          i);
