...
static omr_error_t testMemoryCategoriesFromAttachedThread(
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    if (OMR_ERROR_OUT_OF_NATIVE_MEMORY != rc) {
      ...
      if (-1 == written_count) {
        omrtty_err_printf(
            "%s:%d:Expected written_count to be assigned, even on negative "
            "path.");
        testRc = OMR_ERROR_INTERNAL;
  ...
  if (OMR_ERROR_NONE == testRc) {
    omrtty_printf(
        "\nPositive path: get and validate categories and total_categories is "
        "null\n");
    rc = OMRTEST_PRINT_ERROR(
        ti->GetMemoryCategories(vmThread, total_categories, memoryCategories,
  ...
  if (OMR_ERROR_NONE == testRc) {
    omrtty_printf(
        "\nPositive path: get and validate categories with oversized buffer "
        "and total_categories is null\n");
    rc = OMRTEST_PRINT_ERROR(
        ti->GetMemoryCategories(vmThread, total_categories, memoryCategories,
                                &written_count, total_categories_null));
    if (OMR_ERROR_NONE != rc) {
...
static BOOLEAN validateCategoryPointer(OMRPortLibrary* portLibrary,
  ...
  if (value < categories_buffer ||
    ...
    omrtty_err_printf(
        "%s:%d:Category pointer out of range. Value = %p, categories_buffer = "
        "%p, top of buffer = %p\n",
...
static BOOLEAN validateCategories(OMRPortLibrary* portLibrary,
  ...
  for (i = 0; i < written_count; i++) {
    ...
    if (thisCategory->liveBytesShallow > thisCategory->liveBytesDeep) {
      omrtty_err_printf(
          "%s:%d:liveBytesShallow is larger than liveBytesDeep for category "
          "%s. Index = %d\n",
          __FILE__, __LINE__, thisCategory->name, i);
      return FALSE;
    }
    ...
    if (thisCategory->liveAllocationsShallow >
      ...
      omrtty_err_printf(
          "%s:%d:liveAllocationsShallow is larger than liveAllocationsDeep for "
          "category %s. Index = %d\n",
          __FILE__, __LINE__, thisCategory->name, i);
      return FALSE;
    }
    ...
    if (thisCategory->parent != NULL) {
      if (!validateCategoryPointer(portLibrary, thisCategory->parent,
                                   categories_buffer, written_count)) {
        omrtty_err_printf(
            "%s:%d:Parent link for category %s  failed validation. Index = "
            "%d\n",
            __FILE__, __LINE__, thisCategory->name, i);
        return FALSE;
      }
    ...
    if (thisCategory->firstChild != NULL) {
      if (!validateCategoryPointer(portLibrary, thisCategory->firstChild,
                                   categories_buffer, written_count)) {
        omrtty_err_printf(
            "%s:%d:firstChild link for category %s  failed validation. Index = "
            "%d\n",
            __FILE__, __LINE__, thisCategory->name, i);
        return FALSE;
      }
    ...
    if (thisCategory->nextSibling != NULL) {
      ...
      if (!validateCategoryPointer(portLibrary, thisCategory->nextSibling,
                                   categories_buffer, written_count)) {
        omrtty_err_printf(
            "%s:%d:nextSibling link for category %s  failed validation. Index "
            "= %d\n",
            __FILE__, __LINE__, thisCategory->name, i);
        return FALSE;
      }
      ...
      if (ptr2 && ptr2->nextSibling == ptr1) {
        omrtty_err_printf(
            "%s:%d:1-2-1 loop found in nextSibling chain for category %s. "
            "Index = %d\n",
            __FILE__, __LINE__, thisCategory->name, i);
        return FALSE;
      } else {
        while (ptr1 && ptr2) {
          ...
          if (ptr1 == ptr2) {
            omrtty_err_printf(
                "%s:%d:loop found in nextSibling chain for category %s. Index "
                "= %d\n",
                __FILE__, __LINE__, thisCategory->name, i);
            return FALSE;
          }
