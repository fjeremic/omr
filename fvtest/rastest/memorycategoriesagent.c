  ...
  OMR_VMThread* vmThread, OMR_TI const* ti, OMRPortLibrary* portLibrary)
{
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(ti->GetMemoryCategories(vmThread, 0, NULL, NULL, NULL), OMR_THREAD_NOT_ATTACHED);
  ...
  OMR_VMThread* vmThread, OMR_TI const* ti, OMRPortLibrary* portLibrary)
{
  ...
  rc = OMRTEST_PRINT_UNEXPECTED_RC(ti->GetMemoryCategories(vmThread, 1, NULL, NULL, NULL), OMR_ERROR_ILLEGAL_ARGUMENT);
  if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
    testRc = OMR_ERROR_INTERNAL;
  }

  /* Negative path: try all output pointers NULL */
  rc = OMRTEST_PRINT_UNEXPECTED_RC(ti->GetMemoryCategories(vmThread, 0, NULL, NULL, NULL), OMR_ERROR_ILLEGAL_ARGUMENT);
  if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
    testRc = OMR_ERROR_INTERNAL;
  }
  ...
  if (OMR_ERROR_NONE == testRc) {
    memoryCategories
      = omrmem_allocate_memory(total_categories * sizeof(OMR_TI_MemoryCategory), OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  if (OMR_ERROR_NONE == testRc) {
    omrtty_printf("\nNegative path: get categories with an undersized buffer\n");
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetMemoryCategories(vmThread, total_categories - 1, memoryCategories, &written_count, &total_categories),
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    rc = OMRTEST_PRINT_ERROR(
      ti->GetMemoryCategories(vmThread, total_categories, memoryCategories, &written_count, &total_categories));
    if (OMR_ERROR_NONE != rc) {
      testRc = OMR_ERROR_INTERNAL;
  ...
  if (OMR_ERROR_NONE == testRc) {
    omrtty_printf("\nPositive path: get and validate categories and total_categories is null\n");
    rc = OMRTEST_PRINT_ERROR(
      ti->GetMemoryCategories(vmThread, total_categories, memoryCategories, &written_count, total_categories_null));
    if (OMR_ERROR_NONE != rc) {
      testRc = OMR_ERROR_INTERNAL;
    } else if (total_categories != written_count) {
      omrtty_err_printf("%s:%d:Expected written_count to be equal to total_cateogries.");
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    memoryCategories
      = omrmem_allocate_memory((total_categories + 1) * sizeof(OMR_TI_MemoryCategory), OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == memoryCategories) {
      testRc = OMR_ERROR_OUT_OF_NATIVE_MEMORY;
    } else {
  ...
  /* Positive path: get and validate categories with oversized max_categories, oversized buffer and total_categories is
   * null*/
  written_count = -1;
  if (OMR_ERROR_NONE == testRc) {
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetMemoryCategories(vmThread, (total_categories + 1), memoryCategories, NULL, &total_categories),
      OMR_ERROR_ILLEGAL_ARGUMENT);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetMemoryCategories(vmThread, total_categories - 2, memoryCategories, &written_count, total_categories_null),
      OMR_ERROR_OUT_OF_NATIVE_MEMORY);
    if (OMR_ERROR_OUT_OF_NATIVE_MEMORY != rc) {
      testRc = OMR_ERROR_INTERNAL;
    } else {
...
static void printMemoryCategoryLine(OMRPortLibrary* portLib, OMR_TI_MemoryCategory* category, int depth)
{
  ...
  omrtty_printf(
    "%s %ld bytes / %ld allocations", category->name, category->liveBytesDeep, category->liveAllocationsDeep);
  if (category->liveBytesShallow != category->liveBytesDeep) {
    omrtty_printf(
      " (Shallow: %ld bytes / %ld allocations)", category->liveBytesShallow, category->liveAllocationsShallow);
  }
  omrtty_printf("\n");
}

static void printMemoryCategory(OMRPortLibrary* portLib, OMR_TI_MemoryCategory* category, int depth)
{
 ...
 */
static BOOLEAN validateCategoryPointer(OMRPortLibrary* portLibrary, OMR_TI_MemoryCategory* value,
  OMR_TI_MemoryCategory* categories_buffer, int32_t written_count)
{
  ...
  if (value < categories_buffer || value > (categories_buffer + written_count)) {
    omrtty_err_printf("%s:%d:Category pointer out of range. Value = %p, categories_buffer = %p, top of buffer = %p\n",
  ...
  OMRPortLibrary* portLibrary, OMR_TI_MemoryCategory* categories_buffer, int32_t written_count)
{
  ...
  for (i = 0; i < written_count; i++) {
    ...
    if (thisCategory->liveBytesShallow > thisCategory->liveBytesDeep) {
      omrtty_err_printf("%s:%d:liveBytesShallow is larger than liveBytesDeep for category %s. Index = %d\n", __FILE__,
        __LINE__, thisCategory->name, i);
      return FALSE;
    }

    if (thisCategory->liveAllocationsShallow > thisCategory->liveAllocationsDeep) {
      omrtty_err_printf("%s:%d:liveAllocationsShallow is larger than liveAllocationsDeep for category %s. Index = %d\n",
        __FILE__, __LINE__, thisCategory->name, i);
      return FALSE;
    }
    ...
    if (thisCategory->parent != NULL) {
      ...
      if (thisCategory->parent == thisCategory) {
        omrtty_err_printf(
          "%s:%d:Category %s is its own parent. Index = %d\n", __FILE__, __LINE__, thisCategory->name, i);
        return FALSE;
      }
    }
