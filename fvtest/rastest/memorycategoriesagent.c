...
static omr_error_t
testMemoryCategoriesFromAttachedThread(OMR_VMThread *vmThread, OMR_TI const *ti,
...
static BOOLEAN validateCategories(OMRPortLibrary *portLibrary,
                                  OMR_TI_MemoryCategory *categories_buffer,
                                  int32_t written_count);
static void printMemoryCategories(OMRPortLibrary *portLib,
                                  OMR_TI_MemoryCategory *categories_buffer,
                                  int32_t written_count);
 ...
 * It doesn't set any of the values the test case expects so should fail most of
 * the time.) (Adjust as needed for other cases.)
 */
/*
 omr_error_t
 badGetMemoryCategories_AlwaysErrorNone(OMR_VMThread *vmThread, int32_t
 max_categories, OMR_TI_MemoryCategory *categories_buffer, int32_t
 *written_count_ptr, int32_t *total_categories_ptr)
 {
...
omr_error_t OMRAgent_OnLoad(OMR_TI const *ti, OMR_VM *vm, char const *options,
  ...
  if (OMR_ERROR_NONE == rc) {
    rc = OMRTEST_PRINT_ERROR(
        testMemoryCategoriesFromAttachedThread(vmThread, ti, OMRPORTLIB));
  }
...
static omr_error_t testMemoryCategoriesFromUnattachedThread(
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
        ti->GetMemoryCategories(vmThread, 0, NULL, NULL, NULL),
...
testMemoryCategoriesFromAttachedThread(OMR_VMThread *vmThread, OMR_TI const *ti,
  ...
  rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetMemoryCategories(vmThread, 1, NULL, NULL, NULL),
  ...
  total_categories = -1;
  rc = OMRTEST_PRINT_ERROR(
      ti->GetMemoryCategories(vmThread, 0, NULL, NULL, &total_categories));
  if (OMR_ERROR_NONE != rc) {
    testRc = OMR_ERROR_INTERNAL;
  } else {
  ...
  if (OMR_ERROR_NONE == testRc) {
    memoryCategories =
        omrmem_allocate_memory(total_categories * sizeof(OMR_TI_MemoryCategory),
                               OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == memoryCategories) {
      testRc = OMR_ERROR_OUT_OF_NATIVE_MEMORY;
    } else {
      memset(memoryCategories, 0,
             total_categories * sizeof(OMR_TI_MemoryCategory));
    }
  }
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
        ti->GetMemoryCategories(vmThread, total_categories, memoryCategories,
                                NULL, &total_categories),
        OMR_ERROR_ILLEGAL_ARGUMENT);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
      testRc = OMR_ERROR_INTERNAL;
    }
  ...
  if (OMR_ERROR_NONE == testRc) {
    omrtty_printf(
        "\nNegative path: get categories with an undersized buffer\n");
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
        ti->GetMemoryCategories(vmThread, total_categories - 1,
                                memoryCategories, &written_count,
                                &total_categories),
    ...
    if (OMR_ERROR_OUT_OF_NATIVE_MEMORY != rc) {
      ...
      if (-1 == written_count) {
        omrtty_err_printf("%s:%d:Expected written_count to be assigned, even "
                          "on negative path.");
        testRc = OMR_ERROR_INTERNAL;
      } else if ((total_categories - 1) != written_count) {
        omrtty_err_printf(
            "%s:%d:Expected written_count to be equal to total_cateogries -1.");
        testRc = OMR_ERROR_INTERNAL;
      } else if (initial_total_categories != total_categories) {
        omrtty_err_printf(
            "%s:%d:Expected total_categories to remain constant.");
        testRc = OMR_ERROR_INTERNAL;
      } else if (!validateCategories(OMRPORTLIB, memoryCategories,
                                     written_count)) {
        testRc = OMR_ERROR_INTERNAL;
      }
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    if (OMR_ERROR_NONE != rc) {
      ...
      omrtty_err_printf(
          "%s:%d:Expected written_count to be equal to total_cateogries.");
      testRc = OMR_ERROR_INTERNAL;
    } else if (initial_total_categories != total_categories) {
  ...
  written_count = -1;
  if (OMR_ERROR_NONE == testRc) {
    omrtty_printf("\nPositive path: get and validate categories and "
                  "total_categories is null\n");
    rc = OMRTEST_PRINT_ERROR(
        ti->GetMemoryCategories(vmThread, total_categories, memoryCategories,
                                &written_count, total_categories_null));
    if (OMR_ERROR_NONE != rc) {
      testRc = OMR_ERROR_INTERNAL;
    } else if (total_categories != written_count) {
      omrtty_err_printf(
  ...
  if (OMR_ERROR_NONE == testRc) {
    ...
    memoryCategories = omrmem_allocate_memory((total_categories + 1) *
    ...
    if (NULL == memoryCategories) {
      ...
      memset(memoryCategories, 0,
             (total_categories + 1) * sizeof(OMR_TI_MemoryCategory));
    }
  }

  /* Positive path: get and validate categories with oversized max_categories
   * and oversized buffer */
  written_count = -1;
  if (OMR_ERROR_NONE == testRc) {
    omrtty_printf("\nPositive path: get and validate categories with oversized "
                  "max_categories and oversized buffer\n");
    rc = OMRTEST_PRINT_ERROR(ti->GetMemoryCategories(
        vmThread, (total_categories + 1), memoryCategories, &written_count,
        &total_categories));
    if (OMR_ERROR_NONE != rc) {
      testRc = OMR_ERROR_INTERNAL;
    } else if (total_categories != written_count) {
  ...
  /* Positive path: get and validate categories with oversized buffer and
   * total_categories is null */
  written_count = -1;
  if (OMR_ERROR_NONE == testRc) {
    omrtty_printf("\nPositive path: get and validate categories with oversized "
  ...
  /* Positive path: get and validate categories with oversized max_categories,
   * oversized buffer and total_categories is null*/
  written_count = -1;
  if (OMR_ERROR_NONE == testRc) {
    omrtty_printf(
        "\nPositive path: get and validate categories with oversized "
        "max_categories, oversized buffer and total_categories is null\n");
    rc = OMRTEST_PRINT_ERROR(ti->GetMemoryCategories(
        vmThread, (total_categories + 1), memoryCategories, &written_count,
        total_categories_null));
    if (OMR_ERROR_NONE != rc) {
      testRc = OMR_ERROR_INTERNAL;
    } else if (total_categories != written_count) {
      omrtty_err_printf(
  ...
  /* Negative path: get categories with oversized max_categories, oversized
   * buffer and a NULL written_count_ptr */
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
        ti->GetMemoryCategories(vmThread, (total_categories + 1),
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
        ti->GetMemoryCategories(vmThread, (total_categories + 1), NULL,
                                &written_count, NULL),
        OMR_ERROR_ILLEGAL_ARGUMENT);
    if (OMR_ERROR_ILLEGAL_ARGUMENT != rc) {
      testRc = OMR_ERROR_INTERNAL;
  ...
  /* Negative path: get categories with undersized max_categories and oversized
   * buffer */
  written_count = -1;
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
        ti->GetMemoryCategories(vmThread, total_categories - 2,
                                memoryCategories, &written_count,
                                total_categories_null),
        OMR_ERROR_OUT_OF_NATIVE_MEMORY);
    if (OMR_ERROR_OUT_OF_NATIVE_MEMORY != rc) {
      ...
      if (-1 == written_count) {
        ...
        omrtty_err_printf(
            "%s:%d:Expected written_count to be equal to total_cateogries -2.");
        testRc = OMR_ERROR_INTERNAL;
      } else if (!validateCategories(OMRPORTLIB, memoryCategories,
                                     written_count)) {
        testRc = OMR_ERROR_INTERNAL;
...
static void printMemoryCategoryLine(OMRPortLibrary *portLib,
                                    OMR_TI_MemoryCategory *category,
                                    int depth) {
  ...
  omrtty_printf("%s %ld bytes / %ld allocations", category->name,
                category->liveBytesDeep, category->liveAllocationsDeep);
  if (category->liveBytesShallow != category->liveBytesDeep) {
    omrtty_printf(" (Shallow: %ld bytes / %ld allocations)",
                  category->liveBytesShallow, category->liveAllocationsShallow);
  }
  omrtty_printf("\n");
}

static void printMemoryCategory(OMRPortLibrary *portLib,
                                OMR_TI_MemoryCategory *category, int depth) {
...
static void printMemoryCategories(OMRPortLibrary *portLib,
  ...
  for (categories_index = 0; categories_index < written_count;
       categories_index++) {
 ...
 */
static BOOLEAN validateCategoryPointer(OMRPortLibrary *portLibrary,
                                       OMR_TI_MemoryCategory *value,
                                       OMR_TI_MemoryCategory *categories_buffer,
                                       int32_t written_count) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);

  if (value < categories_buffer ||
      value > (categories_buffer + written_count)) {
    omrtty_err_printf("%s:%d:Category pointer out of range. Value = %p, "
                      "categories_buffer = %p, top of buffer = %p\n",
                      __FILE__, __LINE__, value, categories_buffer,
                      categories_buffer + written_count);
    return FALSE;
  }

  if (((char *)value - (char *)categories_buffer) %
      sizeof(OMR_TI_MemoryCategory)) {
    omrtty_err_printf(
        "%s:%d:Misaligned category pointer = %p. Categories_buffer=%p, "
        "sizeof(OMR_TI_MemoryCategory)=%u, remainder=%d\n",
        __FILE__, __LINE__, value, categories_buffer,
        (unsigned int)sizeof(OMR_TI_MemoryCategory),
...
static BOOLEAN validateCategories(OMRPortLibrary *portLibrary,
  ...
  for (i = 0; i < written_count; i++) {
    ...
    if (thisCategory->liveBytesShallow < 0) {
      omrtty_err_printf(
          "%s:%d:liveBytesShallow negative for category %s. Index = %d\n",
    ...
    if (thisCategory->liveAllocationsShallow < 0) {
      omrtty_err_printf(
          "%s:%d:liveAllocationsShallow negative for category %s. Index = %d\n",
          __FILE__, __LINE__, thisCategory->name, i);
      return FALSE;
    }
    ...
    if (thisCategory->liveBytesShallow > thisCategory->liveBytesDeep) {
      omrtty_err_printf("%s:%d:liveBytesShallow is larger than liveBytesDeep "
                        "for category %s. Index = %d\n",
                        __FILE__, __LINE__, thisCategory->name, i);
      return FALSE;
    }

    if (thisCategory->liveAllocationsShallow >
        thisCategory->liveAllocationsDeep) {
      omrtty_err_printf("%s:%d:liveAllocationsShallow is larger than "
                        "liveAllocationsDeep for category %s. Index = %d\n",
                        __FILE__, __LINE__, thisCategory->name, i);
      return FALSE;
    }
    ...
    if (thisCategory->parent != NULL) {
      ...
      if (!validateCategoryPointer(portLibrary, thisCategory->parent,
                                   categories_buffer, written_count)) {
        omrtty_err_printf("%s:%d:Parent link for category %s  failed "
                          "validation. Index = %d\n",
                          __FILE__, __LINE__, thisCategory->name, i);
        return FALSE;
      }
    ...
    if (thisCategory->firstChild != NULL) {
      if (!validateCategoryPointer(portLibrary, thisCategory->firstChild,
                                   categories_buffer, written_count)) {
        omrtty_err_printf("%s:%d:firstChild link for category %s  failed "
                          "validation. Index = %d\n",
                          __FILE__, __LINE__, thisCategory->name, i);
        return FALSE;
      }
      ...
      if (thisCategory->firstChild == thisCategory) {
        omrtty_err_printf(
            "%s:%d:Category %s is its own firstChild. Index = %d\n", __FILE__,
    ...
    if (thisCategory->nextSibling != NULL) {
      ...
      if (!validateCategoryPointer(portLibrary, thisCategory->nextSibling,
                                   categories_buffer, written_count)) {
        omrtty_err_printf("%s:%d:nextSibling link for category %s  failed "
                          "validation. Index = %d\n",
                          __FILE__, __LINE__, thisCategory->name, i);
        return FALSE;
      }
      ...
      if (thisCategory->nextSibling == thisCategory) {
        omrtty_err_printf(
            "%s:%d:Category %s is its own nextSibling. Index = %d\n", __FILE__,
            __LINE__, thisCategory->name, i);
        return FALSE;
      }
      ...
      if (ptr2 && ptr2->nextSibling == ptr1) {
        omrtty_err_printf("%s:%d:1-2-1 loop found in nextSibling chain for "
                          "category %s. Index = %d\n",
                          __FILE__, __LINE__, thisCategory->name, i);
        return FALSE;
      } else {
        while (ptr1 && ptr2) {
          /* ptr1 increments by 1 step, ptr2 increments by 2. If they are ever
           * equal, we have a loop */
          /* See Van Der Linden, "Deep C Secrets" Appendix for details */
          ptr1 = ptr1->nextSibling;
          ptr2 =
              ptr2->nextSibling != NULL ? ptr2->nextSibling->nextSibling : NULL;

          if (ptr1 == ptr2) {
            omrtty_err_printf("%s:%d:loop found in nextSibling chain for "
                              "category %s. Index = %d\n",
                              __FILE__, __LINE__, thisCategory->name, i);
            return FALSE;
          }
