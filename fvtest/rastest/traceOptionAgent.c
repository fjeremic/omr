...
omr_error_t OMRAgent_OnLoad(OMR_TI const *ti, OMR_VM *vm, char const *options,
  ...
  /* Test invalid OMR trace option (but valid for JVM),
   * OMR_ERROR_ILLEGAL_ARGUMENT is expected */
  if (OMR_ERROR_NONE == rc) {
  ...
  if (OMR_ERROR_NONE == rc) {
    rc = ti->RegisterRecordSubscriber(vmThread, "sample", subscribeFunc,
                                      alarmFunc, (void *)&testData,
...
static void recordErrorStatus(omr_error_t rc, const char *filename,
                              int lineno) {
  if (OMR_ERROR_NONE != rc) {
    testData.rc = rc;
