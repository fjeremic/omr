  ...
  for (j = 0; j < (sizeof(heapSize) / sizeof(heapSize[0])); j++) {
    ...
    do {
      ...
      if (subAllocPtr) {
        ...
        /*The size returned may have been rounded-up to the nearest U64, and may have an extra 2 slots added in some
         * circumstances*/
    ...
    if (0 == operation) {
      ...
      if (0 == reallocSerialNumber % outputInterval) {
        portTestEnv->log(LEVEL_VERBOSE,
          ...
          allocSize, subAllocMem, freeSerialNumber, pool_numElements(allocPool));
      }
      walkHeap(OMRPORTLIB, heapBase, testName);
    } else {
...
TEST(PortHeapTest, heap_realloc_test0)
{
  ...
  if (subAllocPtr != subAllocPtrSaved) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "Unexpected (subAllocPtr (%p) != subAllocPtrSaved (%p))\n", subAllocPtr, subAllocPtrSaved);
    goto exit;
  }
  for (i = 0; i < 128; i++) {
    if (subAllocPtr[i] != i) {
...
TEST(PortHeapTest, heap_grow_test)
{
   ...
   *have value 128 - sizeof(omrheap)/sizeof(uint64_t) - 2(head and tail of the slot) = 124 (on 64bit) or 125 (on 32bit)
   *
   *
   * |--------|-----|--------------------------------------------------------------------------------------------------------
   * |heapSize|first|    |    |																						 |	  |
   * |		|Free |	124|    | ....................................all heap is free...................................|
   *124| |		|Block|	   |    |																						 |    |
   * |--------|-----|-------------------------------------------------------------------------------------------------------
   * 0        1     2	   3	4  																						 127  128
   *     128    1|2
   ...
   *  c. The head of newly added free block of slots should have the value 8. Since 10 slots are added, 2 slots are used
   * for head and tail.
   */
  ...
  if (138 != heapBase->heapSize) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "heap_allocate corrupted heapBase->heapSize. Expected :  138, Found : %zu",
      heapBase->heapSize);
    goto exit;
  }
...
TEST(PortHeapTest, heap_test_pmr_28277_999_760)
{
   ...
   * |
   * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|---------------------------------------------------------------------------------------|---|
   * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17    																					99
   * 100
   *
   */
   ...
   * |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |												     						|
   * |
   * |		|Block  | Slot	|  Size   |-2|Alloc0|-2|-2|Alloc1|-2|-2|Alloc2|-2|-2|Alloc |-2|78|	FREE			FREE				FREE			FREE
   * FREE	|78	| |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |  | |   | |	100	|  20
   * |  20   |   0     |	 |		|  |  |		 |  |  |	  |  |  |      |  |  |																			|   |
   * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|--------------------------------------------------------------------------|---|
   * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17	  19 20 21																			99
   * 100
   *
   */
   ...
   * |heapSize|first  | last  | largest |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |                  	|  |  |
   * |   | |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |                 		|  |  |
   * |   | |		|Block  | Slot	|  Size   |-2|Alloc0|-2|-2|Alloc1|-2|-2|Alloc2|-2|-2|Alloc3|-2|-2|Alloc4_1 |-2|68|
   * FREE				FREE			|68	| |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |
   * |8Slots            	|  |  |												|   | |	100	|  30   |  30   |   0     |	 |		|  |  |		 | |  |
   * |  |  |      |  |  |                  	|  |  |												|   |
   * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|----------------------|--|--|---------------------------------------------|---|
   * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17	  19 20 21	               		29 30 31
   * 99	100
   *
   */
   ...
   * |								|   | |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  | |  |  | |  |
   * |						     	|   | |		|Block  | Slot	|  Size
   * |-2|Alloc0|-2|-2|Alloc1|-2|-2|Alloc2|-2|-2|Alloc3|-2|-2|Alloc4_1          	|-2|-2|Alloc5|-2|64|				FREE			|64
   ...
   * |      |  |  |					|   | |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |
   * |  |  |      |  |  |      |  |  |			     	|   | |		|Block  | Slot	|  Size
   * |-2|Alloc0|-2|-2|Alloc1|-2|-2|Alloc2|-2|-2|Alloc3|-2|-2|Alloc4_1          	|-2|-2|Alloc5|-2|-2|Alloc6|-2|60|	FREE
   * |60	| |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |  |8Slots
   * |  |  |2Slots|  |  |2Slots|  |  |		     		|   | |	100	|  38   |  38   |   0     |	 |		|  |  |		 |  |  | |  |
   * |      |  |  |                  	|  |  |      |  |  |      |  |  | 					|   |
   * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|----------------------|--|--|------|--|--|------|--|--|-------------------|---|
   * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17	  19 20 21	               		29 30 31
   * 33 34 35     37 38 39					99	100
   *
   */
   ...
   * |heapSize|first  | last  | largest |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |        |  |  |        |  |  |
   * |  |  |      |  |  |					|   | |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  | | |
   * |  |        |  |  |      |  |  |      |  |  |			     	|   | |		|Block  | Slot	|  Size   |-2|Alloc	|-2| 2|FREE
   * | 2|-2|Alloc |-2|-2|Alloc |-2|-3|Alloc4_1|-3|3 |FREE    |3 |-2|Alloc |-2|-2|Alloc |-2|60|	FREE			|60	| |		| |
   * |  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |  | 3Slots |  |  |3Slots  |  |  |2Slots|  |
   * |2Slots|  |  |		     		|   | |	100	|   8   |   25  |   2     |	 |		|  |  |		 |  |  |	  |  |  |      |  | | |
   * |  |        |  |  |      |  |  |      |  |  |					|   |
   * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|--------|--|--|--------|--|--|------|--|--|------|--|--|--
   * ---------------|---| 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17	  19 20 21 24 25 26
   * 29 30 31     33 34 35   	37 38 39 				99	100
   *
   */
   ...
   * |  |  |      |  |  |					|   | |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  | | |
   * |  |        |  |  |      |  |  |      |  |  |			     	|   | |		|Block  | Slot	|  Size   |-2|Alloc	|-2| 2|FREE
   * | 2|-2|Alloc |-2|-2|Alloc |-2|-3|Alloc4_1|-3|-3|Alloc4_2|-3|-2|Alloc |-2|-2|Alloc |-2|60|	FREE			|60	| |		| |
   * |  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |  | 3Slots |  |  |3Slots  |  |  |2Slots|  |
   * |2Slots|  |  |		     		|   | |	100	|   8   |   25  |   2     |	 |		|  |  |		 |  |  |	  |  |  |      |  | | |
   * |  |        |  |  |      |  |  |      |  |  |					|   |
   * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|--------|--|--|--------|--|--|------|--|--|------|--|--|--
   ...
   * |  |  |      |  |  |					|   | |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  | | |
   * |  |        |  |  |      |  |  |      |  |  |			     	|   | |		|Block  | Slot	|  Size   |-2|Alloc	|-2| 2|FREE
   * | 2|-2|Alloc |-2|-2|Alloc |-2| 3|FREE	  | 3|-3|Alloc4_2|-3|-2|Alloc |-2|-2|Alloc |-2|60|	FREE			|60	| |		| |
   * |  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |  | 3Slots |  |  |3Slots  |  |  |2Slots|  |
   * |2Slots|  |  |		     		|   | |	100	|   8   |   25  |   3     |	 |		|  |  |		 |  |  |	  |  |  |      |  | | |
   * |  |        |  |  |      |  |  |      |  |  |					|   |
   * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|--------|--|--|--------|--|--|------|--|--|------|--|--|--
  ...
  struct OMRPortLibrary* portLibrary, void* subAllocMem, uintptr_t allocSize, J9Heap* heapBase, const char* testName)
{
  ...
  if (blockSizeStart >= 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
      ...
      blockSizeStart, blockSizeEnd);
    return;
  }
  blockSizeEnd = blockStart[-blockSizeStart];
  if (blockSizeStart != blockSizeEnd) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "Top and bottom padding of the Sub-allocated memory do not match: 0x%p\n", subAllocMem);
    return;
  }
  if (0 == allocSize) {
    if (!(1 <= -blockSizeStart && -blockSizeStart <= 3)) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
        "The allocated block seems to be overly large (3 or more free slots): 0x%p, request size: %zu, block size: "
        "%lld\n",
