...
static void verifySubAllocMem(
    struct OMRPortLibrary* portLibrary, void* subAllocMem, uintptr_t allocSize, J9Heap* heapBase, const char* testName);
 ...
 * It then enters a loop that calls omrheap_create on each of the first 16 byte-aligned locations with a heap size of
 * 1MB. After each omrheap_create, we perform a series of heap_alloc and heap_free from size 0 to the maximum possible.
...
TEST(PortHeapTest, heap_test1)
{
    ...
    for (j = 0; j < (sizeof(heapSize) / sizeof(heapSize[0])); j++) {
        ...
        /*we leave the first <heapStartOffset> bytes alone so that we can check if they are corrupted by heap
         * operations*/
        ...
        if (((NULL == heapBase) && heapSize[j] > MINIMUM_HEAP_SIZE)
            ...
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "Failed to create heap at %p with size %zu\n", allocPtrAdjusted, heapSize[j]);
            goto exit;
        }
        if (NULL == heapBase) {
            break;
        ...
        do {
            ...
            if (subAllocPtr) {
                ...
                /*The size returned may have been rounded-up to the nearest U64, and may have an extra 2 slots added in
                 * some circumstances*/
 ...
 * We go into a loop of heap_allocate and heap_free, the size of alloc is based on the random value (i.e. [0,
 * allocSizeBoundary]). We store the returned address, request size, etc in a sub-allocated memory identifier, which is
 * stored in a J9Pool. When we do a heap_free, we randomly pick an element from the pool and free the memory based on
 * the values stored in the identifier. We keep a separate counter for number of heap_allocate and heap_free calls, and
 * we terminate the loop when heap_free count reaches a certain value e.g. 500,000.
 *
 * As an additional test for memory leak. We determine the largest amount of memory we can allocate before we enter the
 * loop of heap_alloc/heap_free. After the loop is terminated, we free any remaining memory chunks in the pool and again
 * allocate the largest possible size. If this allocation is no longer successful, it means we have leaked memory on the
 * heap during our loop execution.
 *
...
int omrheap_test2(struct OMRPortLibrary* portLibrary, int randomSeed)
{
    ...
    allocPool = pool_new(
        sizeof(AllocListElement), 0, 0, 0, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY, POOL_FOR_PORT(OMRPORTLIB));
    if (NULL == allocPool) {
...
TEST(PortHeapTest, heap_grow_test)
{
     ...
     * So when omrheap created initially, firstFreeBlock always has the value 2(on 64bit) or 1 (on 32bit) and heapSize
     *should be 128 slots in this test heap. And firstFreeBlock should point to the first slot after header which should
     *have value 128 - sizeof(omrheap)/sizeof(uint64_t) - 2(head and tail of the slot) = 124 (on 64bit) or 125 (on
     *32bit)
     *
     *
     * |--------|-----|--------------------------------------------------------------------------------------------------------
     * |heapSize|first|    |    | | | |		|Free |	124|    | ....................................all heap is
     *free...................................| 124| |		|Block|	   |    |
     *|    |
     * |--------|-----|-------------------------------------------------------------------------------------------------------
     * 0        1     2	   3	4 127 128 128    1|2 (32|64bit)
     *
     */
    ...
    /* 2. Allocate the rest of the heap and make it full. So (initialNumberOfSlots-12(used above)-2(head and tail))*8
     ...
     *  c. The head of newly added free block of slots should have the value 8. Since 10 slots are added, 2 slots are
    ...
    if (138 != heapBase->heapSize) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "heap_allocate corrupted heapBase->heapSize. Expected :  138, Found : %zu", heapBase->heapSize);
        goto exit;
    }
 ...
 * JAZZ103 71328
 *
 * In order to create the scenario in this PMR, heap's different segments need to be allocated and freed at a certain
 * order.
 *
 * This is the scenario where lastAllocSlot is not updated correctly and it points to somewhere in the middle of a
 * segment and has the value of 0.
 *
 *
 */
TEST(PortHeapTest, heap_test_pmr_28277_999_760)
{
     ...
     * |		 |Free    | Alloc	|  Alloc   |  | |
     |
     * |		 |Block   | Slot	|  Size    |94|			FREE			FREE			FREE					FREE
     |94	|
     * |		 |        |     	|  Visited |  | |
     |
     * |	100	 |  4     |  4   	|   94     |  |
     ...
     *
     * With the fix, firstFree and lastAllocSlot values should point to the right segments of heap. This test verifies
     * they do.
     *
     */
     ...
     * |----------------------------------|-------------------------------------------------------------------------------------------------------------------------------------
     * |heapSize|first  | last  | largest |	 |		|	|	|
     * | | |		|Free   | Alloc |  Alloc  |	 |		|	|	| |   | |		|Block  | Slot	|  Size |-2|Alloc0|-2
     * |90	|   FREE			FREE			FREE					FREE			FREE			FREE
     * |90 | |		|       |	   	|  Visited|	 |2Slots|	|	|
     * |   | |	100	|  8    |  8    |   0     |	 |		|	|	|
     * |   |
     * |--------|-------|-------|---------|--|------|---|---|---------------------------------------------------------------------------------------------------------------|---|
     ...
     * |----------------------------------|-------------------------------------------------------------------------------------------------------------------------------------
     * |heapSize|first  | last  | largest |	 |		|  |  |		 |  |  |
     * | | |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  | |   | |		|Block  | Slot	|  Size
     * |-2|Alloc0|-2|-2|Alloc1|-2|86|   FREE			FREE			FREE				FREE			FREE
     * FREE |86 | |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  | |   | |	100	|  12   | 12   |   0 |
     * |		|  |  |		 |  |  | |   |
     * |--------|-------|-------|---------|--|------|--|--|------|--|--|----------------------------------------------------------------------------------------------------|---|
     ...
     * |----------------------------------|-------------------------------------------------------------------------------------------------------------------------------------
     * |heapSize|first  | last  | largest |	 |		|  |  |		 |  |  |	  |  |  |
     * | | |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  | |   | |		|Block  | Slot
     * |  Size   |-2|Alloc0|-2|-2|Alloc1|-2|-2|Alloc2|-2|82|	FREE			FREE				FREE			FREE
     * FREE |82 | |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  | |   | |	100 |  16   | 16
     * | 0     |	 |		|  |  |		 |  |  |	  |  |  | |   |
     * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|---------------------------------------------------------------------------------------|---|
     ...
     * |heapSize|first  | last  | largest |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |
     * | | |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  | |  |  |
     * | | |		|Block  | Slot	|  Size   |-2|Alloc0|-2|-2|Alloc1|-2|-2|Alloc2|-2|-2|Alloc |-2|78| FREE FREE
     * FREE			FREE		FREE	|78	| |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |
     * |2Slots|  |  |											     							| | |	100	|  20   | 20
     * |   0     |	 |		|  |  |		 |  |  |	  |  | |      | |  |
     * |
     * |
     ...
     * |heapSize|first  | last  | largest |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |                  	|
     * | | |   | |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  | |  |  | | | |
     * |Block  | Slot	|  Size   |-2|Alloc0|-2|-2|Alloc1|-2|-2|Alloc2|-2|-2|Alloc3|-2|-2|Alloc4_1
     * |-2|68|			FREE				FREE			|68	|
     * |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |  |8Slots            	|
     * | | |   | |	100	|  30   |  30   |   0     |	 |		|  |  |		 |  |  |	  |  |  |      |  |  | |  | | |   |
     * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|----------------------|--|--|---------------------------------------------|---|
     * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17	  19 20 21	               		29
     * 30 31 99	100
     *
     */
     ...
     * |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |                  	|  |  |      |
     * | | |  |  |			     	|   | |		|Block  | Slot	|  Size
     * |-2|Alloc0|-2|-2|Alloc1|-2|-2|Alloc2|-2|-2|Alloc3|-2|-2|Alloc4_1          	|-2|-2|Alloc5|-2|-2|Alloc6|-2|60|
     * FREE |60	| |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots| |  |8Slots |  |
     * |2Slots|  |  |2Slots|  |  |		     		|   | |	100	|  38   |  38   |   0     |	 |		|  |  |		 |  |  |
     * |  |  |      |  |  |                  	|  |  |      | |  |      |  |  | 					|   |
     * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|----------------------|--|--|------|--|--|------|--|--|-------------------|---|
     * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17	  19 20 21	               		29
     * 30 31 33 34 35     37 38 39					99	100
     *
     */
     ...
     * | | |  |      |  |  |					|   |
     * |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |              	    |  |  |      |
     * | | |  |  |			    	|   | |		|Block  | Slot	|  Size   |-2|Alloc0|-2| 2|FREE  |
     * 2|-2|Alloc2|-2|-2|Alloc3|-2|-2|Alloc4_1      	    |-2|-2|Alloc5|-2|-2|Alloc6|-2|60|		FREE		|60
     * | |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |
     * |8Slots        	    |  |  |2Slots|  |  |2Slots|  |  |		     		|   |
     * |	100	|   8   |   8   |   0     |	 |		|  |  |		 |  |  |	  |  |  |      |  |  |              	    |  |
     * | | |  |      |  |  |					|   |
     * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|----------------------|--|--|------|--|--|------|--|--|-------------------|---|
     * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17	  19 20 21	               		29
     ...
     * |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |                  	|  |  |      |
     * | | |  |  |			     	|   | |		|Block  | Slot	|  Size   |-2|Alloc0|-2| 2|FREE  |
     * 2|-2|Alloc2|-2|-2|Alloc3|-2|8 |FREE              	|8 |-2|Alloc |-2|-2|Alloc |-2|60|	FREE			|60
     * | |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |
     * |8slots            	|  |  |2Slots|  |  |2Slots|  |  |		     		|   |
     * |	100	|   8   |   8   |   0     |	 |		|  |  |		 |  |  |	  |  |  |      |  |  |                  	|  |
     * | | |  |      |  |  |					|   |
     ...
     * | | |  |  |      |  |  |					|   |
     * |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |        |  |  |        |  |  |
     * | | |      |  |  |			     	|   | |		|Block  | Slot	|  Size   |-2|Alloc	|-2| 2|FREE  | 2|-2|Alloc
     * |-2|-2|Alloc |-2|-3|Alloc4_1|-3|3 |FREE    |3 |-2|Alloc |-2|-2|Alloc |-2|60|	FREE			|60 | |		| | |
     * Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  | | 3Slots |  |  |3Slots  |  |  |2Slots|  |  |2Slots|
     * |  |		     		|   |
     * |	100	|   8   |   25  |   2     |	 |		|  |  |		 |  |  |	  |  |  |      |  |  |        |  |  |        |
     * | | |  |  |      |  |  |					|   |
     * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|--------|--|--|--------|--|--|------|--|--|------|--|--|--
     * ---------------|---| 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17	  19 20
     * 21	     24 25 26        29 30 31     33 34 35   	37 38 39 				99	100
     *
     */
     ...
     * |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |        |  |  |        |  |  |
     * | | |      |  |  |			     	|   | |		|Block  | Slot	|  Size   |-2|Alloc	|-2| 2|FREE  | 2|-2|Alloc
     * |-2|-2|Alloc |-2|-3|Alloc4_1|-3|-3|Alloc4_2|-3|-2|Alloc |-2|-2|Alloc |-2|60|	FREE			|60 | |		| | |
     * Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  | | 3Slots |  |  |3Slots  |  |  |2Slots|  |  |2Slots|
     * |  |		     		|   |
     * |	100	|   8   |   25  |   2     |	 |		|  |  |		 |  |  |	  |  |  |      |  |  |        |  |  |        |
     * | | |  |  |      |  |  |					|   |
     ...
     * |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |        |  |  |        |  |  |
     * | | |      |  |  |			     	|   | |		|Block  | Slot	|  Size   |-2|Alloc	|-2| 2|FREE  | 2|-2|Alloc
     * |-2|-2|Alloc |-2| 3|FREE	  | 3|-3|Alloc4_2|-3|-2|Alloc |-2|-2|Alloc |-2|60|	FREE			|60 | |		| | |
     * Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  | | 3Slots |  |  |3Slots  |  |  |2Slots|  |  |2Slots|
     * |  |		     		|   |
     * |	100	|   8   |   25  |   3     |	 |		|  |  |		 |  |  |	  |  |  |      |  |  |        |  |  |        |
     * | | |  |  |      |  |  |					|   |
     ...
     *
     *
     * PROBLEM: As you can see above, lastAllocSlot is not pointing to the beginning of alloc/free segment. It is in the
     * middle of the free segment.
     *
     * If this segment is allocated again, and value is written in it, then lastAllocSlot will have random values. See
     * below calls.
     *
     */
     ...
     * |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |                  	|  |  |      |
     * | | |  |  |			     	|   | |		|Block  | Slot	|  Size   |-2|Alloc0|-2| 2|FREE  |
     * 2|-2|Alloc2|-2|-2|Alloc3|-2|-8|Alloc4_1            	|-8|-2|Alloc |-2|-2|Alloc |-2|60|	FREE			|60
     * | |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |
     * |8slots            	|  |  |2Slots|  |  |2Slots|  |  |		     		|   |
     * |	100	|   8	|   25  |   8     |	 |		|  |  |		 |  |  |	  |  |  |      |  |  |                  	|  |  |
     * | | |      |  |  |					|   |
...
/**
 * Helper function that iterates all used elements in the pool and call heap_free to free each memory blocks, whose
 * information is stored in a pool element.
 */
static void freeRemainingElementsInPool(struct OMRPortLibrary* portLibrary, J9Heap* heapBase, J9Pool* allocPool)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
 ...
 * It tries to call heap_allocate with the size used to create the heap, and gradually decreases the size until an
 * heap_allocate succeeds. It returns the size of the largest possible allocation.
 */
static uintptr_t allocLargestChunkPossible(struct OMRPortLibrary* portLibrary, J9Heap* heapBase, uintptr_t heapSize)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
    ...
    struct OMRPortLibrary* portLibrary, void* subAllocMem, uintptr_t allocSize, J9Heap* heapBase, const char* testName)
{
    ...
    if ((subAllocMem < (void*)&basePtr[SIZE_OF_J9HEAP_HEADER / sizeof(uint64_t)])
        ...
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "Sub-allocated memory is not within the valid range of heap: 0x%p\n", subAllocMem);
        return;
    }
    ...
    if (0 == allocSize) {
        if (!(1 <= -blockSizeStart && -blockSizeStart <= 3)) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "The allocated block seems to be overly large (3 or more free slots): 0x%p, request size: %zu, block "
                "size: %lld\n",
...
/**
 * helper function that iterates the used elements in a Pool and produces some verbose output of the information stored
 * in AllocListElement.
 */
static void iteratePool(struct OMRPortLibrary* portLibrary, J9Pool* allocPool)
{
