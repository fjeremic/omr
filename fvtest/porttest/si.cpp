...
TEST(PortSysinfoTest, sysinfo_testProcessorInfo)
{
    ...
    if ((currInfo.totalProcessorCount > 0) && (n_onln > 0) && (currInfo.totalProcessorCount >= n_onln) && (prevInfo.timestamp > 0) && (prevInfo.timestamp < currInfo.timestamp)) {
    ...
    for (cntr = 1; cntr < currInfo.totalProcessorCount + 1; cntr++) {
        ...
        if (OMRPORT_PROCINFO_PROC_ONLINE == currInfo.procInfoArray[cntr].online) {
            if ((OMRPORT_PROCINFO_NOT_AVAILABLE != currInfo.procInfoArray[cntr].userTime) && (OMRPORT_PROCINFO_NOT_AVAILABLE != currInfo.procInfoArray[cntr].systemTime) && (OMRPORT_PROCINFO_NOT_AVAILABLE != currInfo.procInfoArray[cntr].idleTime) &&
#if defined(OMR_OS_WINDOWS) || defined(OSX)
