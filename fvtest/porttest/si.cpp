...
validate_executable_name(const char *expected, const char *found) {
  ...
  if (strncmp(expected_base_path + (expected_length - J9FILE_EXTENSION_LENGTH),
              J9FILE_EXTENSION, J9FILE_EXTENSION_LENGTH) != 0) {
...
TEST(PortSysinfoTest, sysinfo_test0) {
  ...
  if (-1 == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "  Executable name test failed: (rc = %d).\n", rc);
    goto done;
  }
...
#define J9SYSINFO_TEST3_OSNAME_LENGTH 1024
  ...
  if (osName == NULL) {
    ...
    char msg[256];
    omrstr_printf(msg, sizeof(msg),
                  "omrsysinfo_get_OS_type returned : \"%s\"\n", osName);
    portTestEnv->log(msg);
  }
...
#undef SI_DEBUG
  ...
  if (rc < 0) {
    if (OMRPORT_ERROR_NOT_SUPPORTED_ON_THIS_PLATFORM == rc) {
      ...
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "\tCrash passing in NULL buffer while running "
                         "single-threaded. This is a failure because no-one "
                         "else should have been able to modify it\n");
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
...
TEST(PortSysinfoTest, sysinfo_test_sysinfo_set_limit_FILE_DESCRIPTORS) {
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_set_limit FAILED rc=%d\n", rc);
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
...
TEST(PortSysinfoTest, sysinfo_test_sysinfo_set_limit_CORE_FLAGS) {
  ...
  if (geteuid()) {
    ...
    if (OMRPORT_ERROR_FILE_NOPERMISSION == lastErrorNumber) {
      portTestEnv->log(LEVEL_ERROR, "omrsysinfo_set_limit CORE_FLAGS failed as "
                                    "expected because we aren't root.\n");
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
...
TEST(PortSysinfoTest, sysinfo_test_sysinfo_get_limit_FILE_DESCRIPTORS) {
  ...
  if (curLimit > maxLimit) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_get_limit(nofiles): current limit exceeds "
                       "the hard limit.\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
 ...
 */
static int32_t onlineProcessorCount(const struct J9ProcessorInfos *procInfo) {
  int32_t cntr = 0;
  int32_t n_onln = 0;
...
static uintptr_t cpuBurner(OMRPortLibrary *portLibrary, const char *myText) {
  /* burn up CPU */
  uintptr_t counter = 0;
  ...
  for (counter = 0;
    ...
    if (NULL != strstr(result, "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
...
TEST(PortSysinfoTest, sysinfo_test_get_tmp3) {
  ...
  const unsigned char utf8_file[] = {0x63, 0x3A, 0x5C, 0xD0, 0xB6, 0xD0, 0xB0,
                                     0xD0, 0xB1, 0xD0, 0xB0, 0x5C, 0x74, 0x65,
                                     0x73, 0x74, 0x2E, 0x74, 0x78, 0x74, 0x00};
  const wchar_t unicode[] = {0x0063, 0x003A, 0x005C, 0x0436, 0x0430,
                             0x0431, 0x0430, 0x005C, 0x00};

  reportTestEntry(OMRPORTLIB, testName);

  origEnv = (wchar_t *)omrmem_allocate_memory(EsMaxPath,
                                              OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error to update environment variable rc: %d\n", rc);
  }
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error failed to get temp directory rc: %d\n", rc);
  } else {
  ...
  tmpFile = omrfile_open((const char *)utf8_file, EsOpenWrite | EsOpenCreateNew,
                         0666);
  if (-1 == tmpFile) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error failed to create temp file rc: %d\n", rc);
  }

  rc = omrfile_write(tmpFile, data, strlen(data));
  if (0 > rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error failed to write to temp file rc: %d\n", rc);
  }

  rc = omrfile_close(tmpFile);
  if (-1 == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error failed to close temp file rc: %d\n", rc);
  }

  rc = omrfile_unlink((const char *)utf8_file);
  if (-1 == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error failed to delete temp file rc: %d\n", rc);
  }
...
TEST(PortSysinfoTest, sysinfo_test_get_tmp4) {
  ...
  if (rc <= 0) {
    ...
    if (NULL == buffer) {
      ...
      if (0 != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "omrsysinfo_get_tmp failed with rc: %zd\n", rc);
      } else {
  ...
  if (rc <= 0) {
    ...
    if (NULL == buffer) {
      ...
      if (0 != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "omrsysinfo_get_tmp failed with rc: %d\n", rc);
      } else {
        if (!strcmp(modifiedTmpDir, buffer)) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
...
TEST(PortSysinfoTest, sysinfo_test_get_cwd3) {
  ...
  const unsigned char utf8[] = {0x63, 0x3A, 0x5C, 0xE6, 0x88, 0x91, 0xE7, 0x9A,
                                0x84, 0xE7, 0x88, 0xB6, 0xE4, 0xBA, 0xB2, 0xE6,
                                0x98, 0xAF, 0xE6, 0x94, 0xB6, 0xE6, 0xA3, 0x92,
                                0xE5, 0xAD, 0x90, 0xE7, 0x9A, 0x84, 0x5C, 0x00};
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error failed to create directory rc: %d\n", rc);
  }
  rc = _wchdir(unicode);
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
...
TEST(PortSysinfoTest, sysinfo_test_os_kernel_info) {
  ...
  if (FALSE == rc) {
    outputErrorMessage(
        ...
        kernelInfo.kernelVersion, kernelInfo.majorRevision,
