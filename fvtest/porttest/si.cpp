...
/**
 * @brief Function takes an expected name for an executable and also the name
 * that was actually found using the Port API.  It then tries to match these, in
 * a OS specific manner.  For example, on Unices (and z/OS), executables names
 * could be ./exec or full-path to the executable; on windows we do not require
 * ./ as long as the executable is in the current directory, and so on.
 *
...
validate_executable_name(const char* expected, const char* found)
{
  ...
  /* Extract the executable name from the expected path and path found.  Compare
   * these. */
  expected_base_path = strrchr(expected, J9DIRECTORY_SEPARATOR_CHARACTER);
  /* Move past the directory separator, if found. */
  expected_base_path =
    (NULL != expected_base_path) ? (expected_base_path + 1) : expected;
  ...
  /* On Windows, disregard comparing the extension, should this be dropped on
   * the command line (API always returns executable name including the
   * extension (.exe).
   */
#if defined(OMR_OS_WINDOWS)
  /* Check whether argv0 ends with the extension ".exe".  If not, we need to
   * reduce the number of characters to compare (against the executable name
   * found by API).
   */
...
TEST(PortSysinfoTest, sysinfo_test0)
{
  ...
  if (-1 == rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "  Executable name test failed: (rc = %d).\n", rc);
    goto done;
  }
...
#define J9SYSINFO_TEST1_USERNAME_LENGTH 1024
  ...
  if (rc == -1) {
    ...
    portTestEnv->log(
      LEVEL_ERROR,
      "If this is a supported platform, consider this as a failure\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  } else {
    char msg[256] = "";
  ...
  if (length > rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "Error if username buffer is too short: rc= %d, Expecting %d\n",
      rc,
      1);
  }
...
#define J9SYSINFO_HOSTNAME_LENGTH 1024
  ...
  /* Don't check for buffers that are too small, since the call to gethostname()
   * will silently truncate the name and return 0 on some platforms, e.g. MacOS.
   */
  reportTestExit(OMRPORTLIB, testName);
}
...
#define J9SYSINFO_TEST2_GROUPNAME_LENGTH 1024
  ...
  if (rc == -1) {
    ...
    char msg[256] = "";
    omrstr_printf(
      msg, sizeof(msg), "Group name returned = \"%s\"\n", groupname);
    portTestEnv->log(msg);
  }
  ...
  if (length > rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "Error if groupname buffer is too short: rc= %d, Expecting %d\n",
      rc,
      1);
  }
...
#define J9SYSINFO_TEST3_OSNAME_LENGTH 1024
  ...
  if (osName == NULL) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrsysinfo_get_OS_type returned NULL - expected OS name.\n",
      0,
      1);
    reportTestExit(OMRPORTLIB, testName);
    return;
    ...
    char msg[256];
    omrstr_printf(
      msg, sizeof(msg), "omrsysinfo_get_OS_type returned : \"%s\"\n", osName);
    portTestEnv->log(msg);
  }
  ...
  if (osVersion == NULL) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrsysinfo_get_OS_version returned NULL - expected OS name.\n",
      0,
      1);
    reportTestExit(OMRPORTLIB, testName);
    return;
    ...
    omrstr_printf(msg,
                  sizeof(msg),
                  "omrsysinfo_get_OS_version returned : \"%s\"\n",
                  osVersion);
    portTestEnv->log(msg);
  }
  ...
  if (NULL == strstr(osName, "Windows")) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrsysinfo_get_OS_version does not contain \"Windows\".\n",
      0,
      1);
    reportTestExit(OMRPORTLIB, testName);
    return;
    ...
    /*
     * This means we are running a new, unrecognized version of Windows.  We
     * need to update  omrsysinfo_get_OS_type to recognize the specific version.
     */
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrsysinfo_get_OS_version returned the default Windows version.\n",
      0,
      1);
    reportTestExit(OMRPORTLIB, testName);
    return;
  ...
  if (0 != rc) {
    ...
    if (OMRPORT_ERROR_NOT_SUPPORTED_ON_THIS_PLATFORM == rc) {
      portTestEnv->log(LEVEL_ERROR,
                       "This platform does not support the limit iterator\n");
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "\tomrsysinfo_limit_iterator_init returned: %i\n",
                         rc);
    }
    goto done;
  }
 ...
 * 3. Buffer that is big enough to contain an env var, but not big enough for
...
#undef SI_DEBUG
  ...
  /* Test 1: NULL buffer - Pass in NULL for buffer, make sure we get back a
   * positive integer describing the size, or a crash */
  buffer = NULL;
  ...
  if (rc < 0) {
    if (OMRPORT_ERROR_NOT_SUPPORTED_ON_THIS_PLATFORM == rc) {
      portTestEnv->log(LEVEL_ERROR,
                       "This platform does not support the env iterator\n");
    } else if (OMRPORT_ERROR_SYSINFO_ENV_INIT_CRASHED_COPYING_BUFFER == rc) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "\tCrash passing in NULL buffer while running single-threaded. This is "
        "a failure because no-one else should have been able to modify it\n");
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "\tomrsysinfo_env_iterator_init returned: %i\n",
                         rc);
    }
  ...
  if (NULL == buffer) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "OutOfMemory allocating buffer for test - where's all the memory?");
    goto done;
  }
  ...
  if (rc != 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "\tomrsysinfo_env_iterator_init returned: %i\n", rc);
    goto done;
  }
  ...
  /* Test3: Buffer that is big enough to contain an env var, but not big enough
   * for the entire environment */
  omrmem_free_memory(buffer);

  bufferSizeBytes = (uint32_t)envSize - 100;
  buffer =
    omrmem_allocate_memory(bufferSizeBytes, OMRMEM_CATEGORY_PORT_LIBRARY);
...
#if defined(SI_DEBUG)
    portTestEnv->log(
      "Should have a buffer of size %x bytes, using one of size %x instead, "
      "which will result in a truncated environment\n",
      envSize,
      bufferSizeBytes);
#endif
  }
...
TEST(PortSysinfoTest, sysinfo_test_sysinfo_set_limit_ADDRESS_SPACE)
{
  ...
  if (rc == -1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_set_limit returns -1.\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
  ...
  if (as1 == currentLimit) {
    portTestEnv->log(
      "omrsysinfo_set_limit set ADDRESS_SPACE soft max successful\n");
  } else {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrsysinfo_set_limit set ADDRESS_SPACE soft max FAILED\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  }

  /* save original hard limit */
  rc = omrsysinfo_get_limit(OMRPORT_RESOURCE_ADDRESS_SPACE | OMRPORT_LIMIT_HARD,
                            &originalMaxLimit);

  /* lowering the hard limit is irreversible unless privileged */
  if (geteuid()) {
    /* we should be able to set the hard limit to the current value as an
       unprivileged user When the hard limit is set to unlimited (-1) a regular
       user can change it to any value, but not back to unlimited. */
    rc = omrsysinfo_set_limit(
      OMRPORT_RESOURCE_ADDRESS_SPACE | OMRPORT_LIMIT_HARD, originalMaxLimit);
    if (rc == -1) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrsysinfo_set_limit returns -1\n");
    }

    rc = omrsysinfo_get_limit(
      OMRPORT_RESOURCE_ADDRESS_SPACE | OMRPORT_LIMIT_HARD, &currentLimit);
    if (originalMaxLimit == currentLimit) {
      portTestEnv->log(
        "omrsysinfo_set_limit set ADDRESS_SPACE hard max successful\n");
    } else {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrsysinfo_set_limit set ADDRESS_SPACE hard max FAILED\n");
      reportTestExit(OMRPORTLIB, testName);
      return;
    }
    ...
    rc = omrsysinfo_set_limit(
      OMRPORT_RESOURCE_ADDRESS_SPACE | OMRPORT_LIMIT_HARD, as1 + 1);
    if (rc == -1) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrsysinfo_set_limit returns -1.\n");
      reportTestExit(OMRPORTLIB, testName);
    ...
    if (rc == -1) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "restoring the original hard limit failed "
                         "omrsysinfo_set_limit returns -1.\n");
      reportTestExit(OMRPORTLIB, testName);
      return;
    }
  ...
  /* restore original soft limit
     The soft limit is always <= the hard limit. If the hard limit is lowered to
     below the soft limit and then raised again the soft limit isn't
     automatically raised. */
  rc = omrsysinfo_set_limit(OMRPORT_RESOURCE_ADDRESS_SPACE, originalCurLimit);
  if (rc == -1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "restoring the original soft limit failed "
                       "omrsysinfo_set_limit returns -1.\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
...
TEST(PortSysinfoTest, sysinfo_test_sysinfo_set_limit_CORE_FILE)
{
  ...
  if (coreFileSize == currentLimit) {
    portTestEnv->log(
      "omrsysinfo_set_limit set CORE_FILE soft max successful\n");
  } else {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_set_limit set CORE_FILE soft max FAILED\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  }

  /* save original hard limit */
  rc = omrsysinfo_get_limit(OMRPORT_RESOURCE_CORE_FILE | OMRPORT_LIMIT_HARD,
                            &originalMaxLimit);
  ...
  if (geteuid()) {
    /* we should be able to set the hard limit to the current value as an
       unprivileged user When the hard limit is set to unlimited (-1) a regular
       user can change it to any value, but not back to unlimited. */
    rc = omrsysinfo_set_limit(OMRPORT_RESOURCE_CORE_FILE | OMRPORT_LIMIT_HARD,
                              originalMaxLimit);
    if (rc == -1) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrsysinfo_set_limit returns -1\n");
    ...
    if (originalMaxLimit == currentLimit) {
      portTestEnv->log(
        "omrsysinfo_set_limit set CORE_FILE hard max successful\n");
    } else {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrsysinfo_set_limit set CORE_FILE hard max FAILED\n");
      reportTestExit(OMRPORTLIB, testName);
      return;
    }
...
TEST(PortSysinfoTest, sysinfo_test_sysinfo_set_limit_FILE_DESCRIPTORS)
{
  ...
  /* save original soft limit */
  rc =
    omrsysinfo_get_limit(OMRPORT_RESOURCE_FILE_DESCRIPTORS, &originalSoftLimit);
  if (OMRPORT_LIMIT_UNKNOWN == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_get_limit FAILED: OMRPORT_LIMIT_UNKNOWN\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
  ...
  if (0 != rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrsysinfo_set_limit FAILED rc=%d\n", rc);
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
  ...
  if (descriptorLimit == currentLimit) {
    portTestEnv->log(
      "omrsysinfo_set_limit set FILE_DESCRIPTORS soft limit successful\n");
  } else {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrsysinfo_set_limit set FILE_DESCRIPTORS soft limit FAILED "
      "originalSoftLimit=%lld Expected=%lld actual==%lld\n",
      originalSoftLimit,
      descriptorLimit,
      currentLimit);
    reportTestExit(OMRPORTLIB, testName);
  ...
  /* save original hard limit */
  rc = omrsysinfo_get_limit(
    OMRPORT_RESOURCE_FILE_DESCRIPTORS | OMRPORT_LIMIT_HARD, &originalHardLimit);
  if (OMRPORT_LIMIT_UNKNOWN == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_get_limit FAILED: OMRPORT_LIMIT_UNKNOWN\n");
    reportTestExit(OMRPORTLIB, testName);
  ...
  rc =
    omrsysinfo_set_limit(OMRPORT_RESOURCE_FILE_DESCRIPTORS, originalHardLimit);
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_set_limit soft = hard FAILED rc=%d\n",
                       rc);
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
  ...
  /* get new soft limit */
  rc = omrsysinfo_get_limit(OMRPORT_RESOURCE_FILE_DESCRIPTORS,
                            &softSetToHardLimit);
  if (OMRPORT_LIMIT_UNKNOWN == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_get_limit FAILED: OMRPORT_LIMIT_UNKNOWN\n");
  ...
  portTestEnv->log(
    LEVEL_ERROR, "soft set to hard limit=%llu\n", softSetToHardLimit);

  /* set soft limit to old value */
  rc =
    omrsysinfo_set_limit(OMRPORT_RESOURCE_FILE_DESCRIPTORS, originalSoftLimit);
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_set_limit reset soft FAILED rc=%d\n",
                       rc);
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
  ...
  rc = omrsysinfo_get_limit(
    OMRPORT_RESOURCE_FILE_DESCRIPTORS | OMRPORT_LIMIT_HARD, &currentLimit);
  if (currentLimit != originalHardLimit) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_get_limit FAILED: hard limit changed\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
  ...
  if (0 != geteuid()) { /* normal user */
    /* setting the hard limit from unlimited to a finite value has unpredictable
     * results: the actual value may be much smaller than requested. In that
     * case, just try setting it to its current value (softSetToHardLimit) or a
     * value slightly lower. Ensure that we don't try to set the hard limit to a
     * value less than the current soft limit (i.e. originalSoftLimit).
     */
    uint64_t newHardLimit = ((OMRPORT_LIMIT_UNLIMITED == rc) ||
                             (originalSoftLimit == softSetToHardLimit))
                              ? softSetToHardLimit
                              : softSetToHardLimit - 1;

    rc = omrsysinfo_set_limit(
      OMRPORT_RESOURCE_FILE_DESCRIPTORS | OMRPORT_LIMIT_HARD, newHardLimit);
    if (0 != rc) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
    ...
    if (newHardLimit == currentLimit) {
      portTestEnv->log(
        "omrsysinfo_set_limit set FILE_DESCRIPTORS hard limit successful\n");
    } else {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrsysinfo_set_limit set FILE_DESCRIPTORS hard limit FAILED "
        "originalHardLimit=%lld Expected=%lld actual==%lld\n",
        originalHardLimit,
        newHardLimit,
        currentLimit);
      reportTestExit(OMRPORTLIB, testName);
    ...
    finalSoftLimit =
      (originalSoftLimit > newHardLimit) ? newHardLimit : originalSoftLimit;
    ...
    if (newHardLimit == currentLimit) {
      ...
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrsysinfo_set_limit set FILE_DESCRIPTORS hard max "
                         "FAILED. Expected=%lld actual=%lld\n",
                         (descriptorLimit + 1),
                         currentLimit);
      reportTestExit(OMRPORTLIB, testName);
      return;
    ...
    /* restore original hard limit */
    rc = omrsysinfo_set_limit(OMRPORT_RESOURCE_FILE_DESCRIPTORS |
                                OMRPORT_LIMIT_HARD,
                              originalHardLimit);
    if (0 != rc) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "restoring the original hard limit FAILED rc=%d\n",
                         rc);
      reportTestExit(OMRPORTLIB, testName);
      return;
    }
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "restoring the original soft limit FAILED rc=%d\n",
                       rc);
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
...
TEST(PortSysinfoTest, sysinfo_test_sysinfo_set_limit_CORE_FLAGS)
{
  ...
  if (geteuid()) {
    ...
    if (-1 == rc) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrsysinfo_set_limit get AIX full core value failed\n");
      reportTestExit(OMRPORTLIB, testName);
      return;
    }
    ...
    if (OMRPORT_ERROR_FILE_NOPERMISSION == lastErrorNumber) {
      portTestEnv->log(LEVEL_ERROR,
                       "omrsysinfo_set_limit CORE_FLAGS failed as expected "
                       "because we aren't root.\n");
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrsysinfo_set_limit CORE_FLAGS did not fail with "
                         "the proper error.\n");
      reportTestExit(OMRPORTLIB, testName);
      return;
    }
    ...
    if ((rc == OMRPORT_LIMIT_LIMITED) && (0 == currentLimit)) {
      portTestEnv->log(
        "omrsysinfo_set_limit set AIX full core value to 0 successful\n");
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrsysinfo_set_limit set CORE_FLAGS FAILED\n");
      reportTestExit(OMRPORTLIB, testName);
      return;
    }
    ...
    if ((OMRPORT_LIMIT_UNLIMITED == rc) && (U_64_MAX == currentLimit)) {
      portTestEnv->log(
        "omrsysinfo_set_limit set AIX full core value to 1 successful\n");
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrsysinfo_set_limit set CORE_FLAGS FAILED\n");
      reportTestExit(OMRPORTLIB, testName);
    ...
    if (rc == -1) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "restoring the original AIX full core value failed "
                         "omrsysinfo_set_limit returns -1.\n");
      reportTestExit(OMRPORTLIB, testName);
      return;
    }
...
/**
 * Test omrsysinfo_test_sysinfo_get_limit for resource
 * OMRPORT_RESOURCE_FILE_DESCRIPTORS. API available on all Unix platforms.
 */
TEST(PortSysinfoTest, sysinfo_test_sysinfo_get_limit_FILE_DESCRIPTORS)
{
  ...
  if (OMRPORT_LIMIT_UNLIMITED ==
      rc) { /* Not an error, just that it is not configured. */
    /* If the API reported this limit as set to "unlimited", the resource limit
     * must be set to implementation-defined limit, RLIM_INFINITY.
     */
    if (RLIM_INFINITY == curLimit) {
      portTestEnv->log("omrsysinfo_get_limit(nofiles): soft "
                       "limit=RLIM_INFINITY (unlimited).\n");
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrsysinfo_get_limit(nofiles): soft limit "
                         "(unlimited), bad maximum reported %lld.\n",
                         ((int64_t)curLimit));
      reportTestExit(OMRPORTLIB, testName);
      return;
    }
  ...
  rc = omrsysinfo_get_limit(
    OMRPORT_RESOURCE_FILE_DESCRIPTORS | OMRPORT_LIMIT_HARD, &maxLimit);
  if (OMRPORT_LIMIT_UNLIMITED == rc) {
    /* Not an error, just that it is not configured.  Ok to compare!. */
    if (RLIM_INFINITY == maxLimit) {
      portTestEnv->log("omrsysinfo_get_limit(nofiles): hard limit = "
                       "RLIM_INFINITY (unlimited).\n");
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrsysinfo_get_limit(nofiles): hard limit "
                         "(unlimited), bad maximum reported %lld.\n",
                         ((int64_t)curLimit));
      reportTestExit(OMRPORTLIB, testName);
      return;
  ...
  /* Ensure that the resource's current (soft) limit does not exceed the hard
   * limit. */
...
/* Since the processor and memory usage port library APIs are not available on
 * zOS (neither 31-bit not 64-bit) yet, so we exclude these tests from running
 * on zOS. When the zOS implementation is available, we must remove these guards
 * so that they are built and executed on the z as well. Remove this comment
 * too.
 */
#if !defined(J9ZOS390)
/**
 * Test for omrsysinfo_get_memory_info() port library API. Check that we are
 * indeed able to retrieve memory statistics on all supported platforms and
 * also, perform some minimum sanity checks.
 */
TEST(PortSysinfoTest, sysinfo_testMemoryInfo)
{
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_get_memory_info() failed.\n");
  } else {
    /* If any of these parameters are set to OMRPORT_MEMINFO_NOT_AVAILABLE on
     * platforms where they are supposed to be defined, flag an error to fail
     * pltest.
     */
    if ((OMRPORT_MEMINFO_NOT_AVAILABLE == memInfo.totalPhysical) ||
        ...
        /* We do not check totalVirtual since it may be set to some value or -1,
         * depending on whether there is a limit set for this or not on the box.
         */
        || (OMRPORT_MEMINFO_NOT_AVAILABLE != memInfo.availVirtual)
#endif /* defined(OMR_OS_WINDOWS) || defined(OSX) */
#if defined(AIXPPC) || defined(OMR_OS_WINDOWS) || defined(OSX)
        /* Size of the file buffer area is not available on Windows, AIX and
         * OSX. Therefore, it must be set to OMRPORT_MEMINFO_NOT_AVAILABLE.
         */
        || (OMRPORT_MEMINFO_NOT_AVAILABLE != memInfo.buffered)
#else  /* defined(AIXPPC) || defined(OMR_OS_WINDOWS) || defined(OSX) */
        /* On platforms where buffer area is defined,
         * OMRPORT_MEMINFO_NOT_AVAILABLE is surely a failure!
         */
        || (OMRPORT_MEMINFO_NOT_AVAILABLE == memInfo.buffered)
      ...
      /* Fail pltest if one of these memory usage parameters were found
       * inconsistent. */
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Invalid memory usage statistics retrieved.\n");
      reportTestExit(OMRPORTLIB, testName);
      return;
    }
    ...
    if ((memInfo.totalPhysical > 0) &&
...
#if defined(OMR_OS_WINDOWS)
        /* Again, it does not make sense to do checks and comparisons on Virtual
         * Memory on places other than Windows.
         */
...
#else  /* defined(OMR_OS_WINDOWS) || defined(OSX) */
      /* This may or may not be available depending on whether a limit is set.
       * Print out if this is available or else, call this parameter
       * "undefined".
       */
      ...
      /* Leave Available Virtual memory parameter as it is on non-Windows
       * Platforms. */
...
/**
 * @internal
 * Internal function: Counts up the number of processors that are online as per
 * the records delivered by the port library routine
 * omrsysinfo_get_processor_info().
 *
 * @param[in] procInfo Pointer to J9ProcessorInfos filled in with processor info
 * records.
 *
...
cpuBurner(OMRPortLibrary* portLibrary, const char* myText)
{
  ...
  for (counter = 0;
       (strlen(buffer) + strlen(myText) + 1) < CPU_BURNER_BUFF_SIZE;
       ++counter) {
...
TEST(PortSysinfoTest, sysinfo_testProcessorInfo)
{
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_get_processor_info() failed.\n");
   ...
   * This allows other processes and the operating system to use the CPU and
   * drive up the user and kernel utilization.  Use the result of the call to
   * cpuBurner to ensure it isn't optimized out. Used for validating that the
   * total processor usage time is approximately in the range of the time
   * elapsed. Note that this assumption sees deviations of upto 50% at times
   * when the system is lightly loaded but under much system load, the relation
   * indeed becomes accurate: (Busy-time(t2) - Busy-time(t1)) + (Idle-time(t2) -
   * Idle-time(t1)) ~ T2 - T1.
   */
  ...
  if ((currInfo.totalProcessorCount > 0) && (n_onln > 0) &&
      (currInfo.totalProcessorCount >= n_onln) && (prevInfo.timestamp > 0) &&
      (prevInfo.timestamp < currInfo.timestamp)) {

    /* Print out some vital statistics of processor usage - use the current
     * snapshot. */
  ...
  deltaTotalIdleTime =
    currInfo.procInfoArray[0].idleTime - prevInfo.procInfoArray[0].idleTime;
  deltaTotalBusyTime =
    currInfo.procInfoArray[0].busyTime - prevInfo.procInfoArray[0].busyTime;
  ...
  /* Start iterating from 1 since 0^th entry represents Totals - already
   * accounted for above. */
  for (cntr = 1; cntr < currInfo.totalProcessorCount + 1; cntr++) {

    /* Sanity check. Ensure that we successfully retrieved processor usage data
     * in various modes, or else, signal an error. Add platform-specific
     * exceptions (undefined parameter).
     */
    if (OMRPORT_PROCINFO_PROC_ONLINE == currInfo.procInfoArray[cntr].online) {
      if ((OMRPORT_PROCINFO_NOT_AVAILABLE !=
        ...
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "Invalid processor usage statistics retrieved.\n");
        goto _cleanup;
      }
    }
  } /* end for(;;) */

  /* Check whether the processor times have increased since the last iteration.
   * This ensures a monotonically increasing nature of processor times. Note: We
   * cannot do this for each individual processor since there may be
   * architectures such as AIXPPC where times don't change for certain
   * processors that otherwise seem online (actually are in sleep mode); not
   * even the Idle ticks.
   */
  if (0 < (deltaTotalBusyTime + deltaTotalIdleTime)) {
    ...
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "Unexpected change in processor time deltas\ndeltaTotalBusyTime=%lld "
      "deltaTotalIdleTime=%lld\n",
      deltaTotalBusyTime,
      deltaTotalIdleTime);
  }
...
/**
 * Test omrsysinfo_get_number_CPUs_by_type() port library API for the flag
 * OMRPORT_CPU_ONLINE. We obtain the online processor count using other
 * (indirect) method - calling the other port library API
...
TEST(PortSysinfoTest, sysinfo_testOnlineProcessorCount2)
{
  ...
  /* Call omrsysinfo_get_processor_info() to retrieve a set of processor records
   * from which we may then ascertain the number of processors online. This will
   * help us cross-check against the API currently under test.
   */
  ...
  if (0 != rc) {
    ...
    /* Call the port library API omrsysinfo_get_number_online_CPUs() to check
     * that the online processor count received is valid (that is, it does not
     * fail) and that this indeed matches the online processor count as per the
     * processor usage retrieval API.
     */
    intptr_t n_cpus_online =
      omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE);
    if (-1 == n_cpus_online) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrsysinfo_get_number_online_CPUs() failed.\n");
      goto _cleanup;
    }
    ...
    if ((n_cpus_online > 0) &&
      ...
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Invalid online processor count found.\n");
    }
  }
...
/**
 * Test omrsysinfo_get_number_CPUs_by_type() port library API for the flag
 * OMRPORT_CPU_PHYSICAL. Validate the number of available (configured) logical
 * CPUs by cross-checking with what is obtained from invoking the other port
 * library API omrsysinfo_get_processor_info().
 */
TEST(PortSysinfoTest, sysinfo_testTotalProcessorCount)
{
  ...
  /* Call omrsysinfo_get_processor_info() to retrieve a set of processor records
   * from which we may then ascertain the total number of processors configured.
   * We then cross-check this against what the API currently under test returns.
   */
  rc = omrsysinfo_get_processor_info(&procInfo);
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
    ...
    /* Ensure first that the API doesn't fail. If not, check that we obtained
     * the correct total processor count by checking against what
     * omrsysinfo_get_processor_info() returned.
     */
    intptr_t n_cpus_total =
      omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_PHYSICAL);
    if (-1 == n_cpus_total) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrsysinfo_get_number_total_CPUs() failed.\n");
      goto _cleanup;
    }
    ...
    if ((procInfo.totalProcessorCount > 0) &&
      ...
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Invalid processor count retrieved.\n");
    }
  }
...
TEST(PortSysinfoTest, sysinfo_test_get_CPU_utilization)
{
  ...
  /*
   * Call omrsysinfo_get_CPU_utilization() to retrieve the current CPU
   * utilization. Sanity check the results.
   */
  portLibraryStatus = omrsysinfo_get_CPU_utilization(&OldUtil);
  if (0 != portLibraryStatus) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrsysinfo_test_get_CPU_utilization() non-zero return code.\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  } else {
    portTestEnv->log(
      "Old utilization timestamp=%llu cpuTime=%lld numberOfCpus=%d.\n",
      OldUtil.timestamp,
      OldUtil.cpuTime,
      OldUtil.numberOfCpus);
    if ((OldUtil.cpuTime < 0) || (OldUtil.numberOfCpus <= 0)) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrsysinfo_test_get_CPU_utilization() invalid results.\n");
    }
  }
  /* Sleep for 3 seconds before re-sampling processor usage stats.
   * This allows other processes and the operating system to use the CPU and
   * drive up the user and kernel utilization. The call to cpuBurner probably
   * won't be optimized out, but use the result to make absolutely sure that it
   * isn't.
   */
  omrthread_sleep(3000 + cpuBurner(OMRPORTLIB, "a"));
  ...
  if (0 != portLibraryStatus) {
    ...
    portTestEnv->log(
      "New utilization timestamp=%llu cpuTime=%lld numberOfCpus=%d.\n",
      NewUtil.timestamp,
      NewUtil.cpuTime,
      NewUtil.numberOfCpus);
    portTestEnv->log("timestamp delta=%llu cpuTime delta=%lld\n",
                     NewUtil.timestamp - OldUtil.timestamp,
                     NewUtil.cpuTime - OldUtil.cpuTime);
    if ((NewUtil.cpuTime < OldUtil.cpuTime) ||
        (NewUtil.numberOfCpus != OldUtil.numberOfCpus)) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrsysinfo_test_get_CPU_utilization() invalid results.\n");
    }
...
TEST(PortSysinfoTest, sysinfo_test_get_tmp1)
{
  ...
  if (rc <= 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "unexpected return code rc: %d\n", rc);
  } else {
    char* buffer =
      (char*)omrmem_allocate_memory(rc, OMRMEM_CATEGORY_PORT_LIBRARY);
...
TEST(PortSysinfoTest, sysinfo_test_get_tmp2)
{
  ...
  buffer = (char*)omrmem_allocate_memory(smallBufferSize,
...
/*
 * Test omrsysinfo_get_tmp when TMPDIR is non-ascii (Unicode on windows, UTF-8
 * on other platforms) Expected result: Successfully create non-ascii directory,
...
TEST(PortSysinfoTest, sysinfo_test_get_tmp3)
{
  ...
  const unsigned char utf8[] = { 0x63, 0x3A, 0x5C, 0xD0, 0xB6, 0xD0, 0xB0,
                                 0xD0, 0xB1, 0xD0, 0xB0, 0x5C, 0x00 };
  const unsigned char utf8_file[] = {
    0x63, 0x3A, 0x5C, 0xD0, 0xB6, 0xD0, 0xB0, 0xD0, 0xB1, 0xD0, 0xB0,
    0x5C, 0x74, 0x65, 0x73, 0x74, 0x2E, 0x74, 0x78, 0x74, 0x00
  };
  const wchar_t unicode[] = { 0x0063, 0x003A, 0x005C, 0x0436, 0x0430,
                              0x0431, 0x0430, 0x005C, 0x00 };

  reportTestEntry(OMRPORTLIB, testName);

  origEnv =
    (wchar_t*)omrmem_allocate_memory(EsMaxPath, OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  if (NULL != origEnvRef) {
    origEnv = (char*)omrmem_allocate_memory(strlen(origEnvRef) + 1,
                                            OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL != origEnv) {
  ...
  if (0 != rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "error to update environment variable rc: %d\n", rc);
  }

  buffer =
    (char*)omrmem_allocate_memory(EsMaxPath, OMRMEM_CATEGORY_PORT_LIBRARY);
  rc = omrsysinfo_get_tmp(buffer, EsMaxPath, FALSE);
  if (0 != rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "error failed to get temp directory rc: %d\n", rc);
  } else {
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "invalid directory rc: %d buffer %s, utf8 = %s\n",
                       rc,
                       buffer,
                       utf8);
  }
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error failed to create temp directory rc: %d\n",
                       rc);
  }

  tmpFile =
    omrfile_open((const char*)utf8_file, EsOpenWrite | EsOpenCreateNew, 0666);
  if (-1 == tmpFile) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "error failed to create temp file rc: %d\n", rc);
  }

  rc = omrfile_write(tmpFile, data, strlen(data));
  if (0 > rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "error failed to write to temp file rc: %d\n", rc);
  }

  rc = omrfile_close(tmpFile);
  if (-1 == rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "error failed to close temp file rc: %d\n", rc);
  }

  rc = omrfile_unlink((const char*)utf8_file);
  if (-1 == rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "error failed to delete temp file rc: %d\n", rc);
  }

  rc = omrfile_unlinkdir((const char*)utf8);
  if (-1 == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error failed to delete temp directory rc: %d\n",
                       rc);
  }
...
TEST(PortSysinfoTest, sysinfo_test_get_tmp4)
{
  ...
  if (NULL != oldTmpDir) {
    oldTmpDirValue = (char*)omrmem_allocate_memory(
      strlen(oldTmpDir) + 1, OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  if (0 != rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "error in updating environment variable TMPDIR, rc: %zd\n",
      rc);
  }
  ...
  if (rc <= 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "unexpected return code rc: %zd\n", rc);
  } else {
    char* buffer =
      (char*)omrmem_allocate_memory(rc, OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == buffer) {
      ...
      if (0 != rc) {
        outputErrorMessage(
          PORTTEST_ERROR_ARGS, "omrsysinfo_get_tmp failed with rc: %zd\n", rc);
      } else {
        if (strcmp(modifiedTmpDir, buffer)) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "expected omrsysinfo_get_tmp to return same value "
                             "as TMPDIR. TMPDIR: %s, returned: %s\n",
                             modifiedTmpDir,
                             buffer);
        }
      }
  ...
  if (rc <= 0) {
    ...
    if (NULL == buffer) {
      ...
      if (0 != rc) {
        outputErrorMessage(
          PORTTEST_ERROR_ARGS, "omrsysinfo_get_tmp failed with rc: %d\n", rc);
      } else {
        if (!strcmp(modifiedTmpDir, buffer)) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "expected omrsysinfo_get_tmp to ignore TMPDIR. "
                             "TMPDIR: %s, returned: %s\n",
                             modifiedTmpDir,
                             buffer);
        }
...
/*
 * Test omrsysinfo_get_cwd when the buffer size == 0, then allocate required
 * ammount of bites and try again. Expected result size of buffer required
 */
 ...
 * Expected result: Successfully create not ascii directory, change current
 * current directory, verify that omrsysinfo_get_cwd returns valid value.
 */
TEST(PortSysinfoTest, sysinfo_test_get_cwd3)
{
  ...
  const wchar_t unicode[] = { 0x0063, 0x003A, 0x005C, 0x6211, 0x7684,
                              0x7236, 0x4EB2, 0x662F, 0x6536, 0x68D2,
                              0x5B50, 0x7684, 0x005C, 0x00 };
  const unsigned char utf8[] = {
    0x63, 0x3A, 0x5C, 0xE6, 0x88, 0x91, 0xE7, 0x9A, 0x84, 0xE7, 0x88,
    0xB6, 0xE4, 0xBA, 0xB2, 0xE6, 0x98, 0xAF, 0xE6, 0x94, 0xB6, 0xE6,
    0xA3, 0x92, 0xE5, 0xAD, 0x90, 0xE7, 0x9A, 0x84, 0x5C, 0x00
  };
  ...
  if (0 != rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "error failed to create directory rc: %d\n", rc);
  }
  rc = _wchdir(unicode);
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error failed to change current directory rc: %d\n",
                       rc);
  }
#else
#if defined(OSX)
  /* On OSX, /tmp is a symbolic link to /private/tmp. For the cwd to match after
   * chdir, use /private/tmp. */
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error failed to get current working directory rc: %d\n",
                       rc);
  } else {
  ...
  if (-1 == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error: failed to change to directory %s, errno: %d\n",
                       (const char*)orig_cwd,
                       errno);
  }
#endif /* defined(OMR_OS_WINDOWS) */
  ...
  if (-1 == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "error failed to delete directory %s rc: %d\n",
                       (const char*)utf8,
                       rc);
  }
...
TEST(PortSysinfoTest, sysinfo_test_get_groups)
{
  ...
  if (-1 != rc) {
    ...
    for (i = 0; i < rc; i++) {
      int error = 0;
      /* Set errno to 0 before calling getgrgid() to correctly handle NULL
       * return values */
      errno = 0;
      ...
      if (NULL == grent) {
        if (0 == error) {
          portTestEnv->changeIndent(1);
          portTestEnv->log("this group id is not found in group database (not "
                           "an error as per getgrgid documentation)\n");
          portTestEnv->changeIndent(-1);
        } else {
          outputErrorMessage(
            PORTTEST_ERROR_ARGS,
...
TEST(PortSysinfoTest, sysinfo_test_get_open_file_count)
{
  ...
  if (ret < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_get_open_file_count() failed.\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
  portTestEnv->log(
    "omrsysinfo_get_open_file_count(): Files opened by this process=%lld\n",
    openCount);

  /* Now, get the current (soft) limit on the resource "nofiles".  We check the
   * current number of files opened, against this.
   */
  rc = omrsysinfo_get_limit(OMRPORT_RESOURCE_FILE_DESCRIPTORS, &curLimit);
  if (OMRPORT_LIMIT_UNLIMITED == rc) {
    ...
    /* Check that the limits received are sane, before comparing against files
     * opened. */
    if ((((int64_t)curLimit) > 0) && (((int64_t)curLimit) <= INT64_MAX)) {
      portTestEnv->log("omrsysinfo_get_limit(nofiles): soft limit=%lld.\n",
                       ((int64_t)curLimit));
  ...
  if (((int64_t)openCount) > ((int64_t)curLimit)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_get_open_file_count() failed: reports more "
                       "files opened than allowed!\n");
    reportTestExit(OMRPORTLIB, testName);
    return;
  }
  reportTestExit(OMRPORTLIB, testName);
...
TEST(PortSysinfoTest, sysinfo_test_get_os_description)
{
  ...
  for (int i = 0; i < OMRPORT_SYSINFO_OS_FEATURES_SIZE * 32; i++) {
    ...
    portTestEnv->log(
      LEVEL_VERBOSE,
      "omrsysinfo_test_get_os_description() feature %d: value=%d, rc=%zi\n",
      i,
      feature,
      rc);
  }
...
TEST(PortSysinfoTest, sysinfo_test_os_kernel_info)
{
  ...
  if (FALSE == rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrsysinfo_os_kernel_info failed on Linux: kernelVersion=%zu, "
      "majorRevision=%zu, minorRevision=%zu\n",
    ...
    if (0 == kernelInfo.kernelVersion) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrsysinfo_os_kernel_info failed on Linux - kernel "
                         "version can't be 0 (unsupported)\n");
      goto exit;
    }
  }
#else  /* defined(LINUX) */
  if (TRUE == rc) {
    /* Throw an error if omrsysinfo_os_kernel_info passes on an unsupported
     * platform */
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrsysinfo_os_kernel_info passed on an unsupported platform\n");
    goto exit;
  }
#endif /* defined(LINUX) */
...
TEST(PortSysinfoTest, sysinfo_cgroup_get_memlimit)
{
  ...
  if (OMRPORT_ERROR_SYSINFO_CGROUP_UNSUPPORTED_PLATFORM != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_cgroup_get_memlimit returned %d, expected "
                       "%d on platform that does not support cgroups\n",
                       rc,
                       OMRPORT_ERROR_SYSINFO_CGROUP_UNSUPPORTED_PLATFORM);
  }
#endif

  /* Compare sysinfo_get_physical_memory and sysinfo_cgroup_get_memlimit after
  ...
  if (OMR_ARE_ALL_BITS_SET(enabledSubsystems, OMR_CGROUP_SUBSYSTEM_MEMORY)) {
    ...
    if ((0 != rc) && (OMRPORT_ERROR_SYSINFO_CGROUP_MEMLIMIT_NOT_SET != rc)) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrsysinfo_cgroup_get_memlimit failed with error code %d\n",
        rc);
    } else if (0 == rc) {
      ...
      if (cgroupMemLimit != physicalMemLimit) {
        outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "Expected omrsysinfo_cgroup_get_memlimit and "
          "omrsysinfo_get_physical_memory to return same value, but "
          "omrsysinfo_cgroup_get_memlimit returned %ld and "
