...
TEST(PortSlTest, sl_test1)
{
  ...
  rc =
    omrsl_open_shared_library(sharedLibName, &handle, OMRPORT_SLOPEN_DECORATE);
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Unable to open %s, \n",
                       sharedLibName,
...
TEST(PortSlTest, sl_testOpenPathLengths)
{
  ...
  if (OMRPORT_SL_UNSUPPORTED != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       ...
                       "shared library, \n%s,\n",
                       rc,
                       (uintptr_t)OMRPORT_SL_UNSUPPORTED,
                       sharedLibName,
                       omrerror_last_error_message());
    goto exit;
  }
  ...
  if ((OMRPORT_SL_INVALID != rc) && (OMRPORT_SL_NOT_FOUND != rc)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       ...
                       rc,
                       (uintptr_t)OMRPORT_SL_INVALID,
                       (uintptr_t)OMRPORT_SL_NOT_FOUND,
                       sharedLibName,
                       omrerror_last_error_message());
    goto exit;
  }
 ...
 */
static BOOLEAN
isValidLoadErrorMessage(const char* dlerrorOutput,
