 ...
 *
 * Exercise the API for port library shared library system operations.  These
 * functions can be found in the file @ref omrsl.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
TEST(PortSlTest, sl_verify_function_slots) {
  ...
  if (NULL == OMRPORTLIB->sl_close_shared_library) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->sl_close_shared_library is NULL\n");
  }

  if (NULL == OMRPORTLIB->sl_lookup_name) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->sl_lookup_name is NULL\n");
  }

  if (NULL == OMRPORTLIB->sl_open_shared_library) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->sl_open_shared_library is NULL\n");
  }

  if (NULL == OMRPORTLIB->sl_shutdown) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->sl_shutdown is NULL\n");
  }

  if (NULL == OMRPORTLIB->sl_startup) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->sl_startup is NULL\n");
  }
...
TEST(PortSlTest, sl_test1) {
  ...
  rc = omrsl_open_shared_library(sharedLibName, &handle,
                                 OMRPORT_SLOPEN_DECORATE);
  if (0 != rc) {
 ...
 * ii) OMRPORT_SL_INVALID or OMRPORT_SL_NOT_FOUND is received for an incorrect
 * path less than or equal to EsMaxPath chars
 */
TEST(PortSlTest, sl_testOpenPathLengths) {
   ...
   * AIX - "lib.so", "lib.a" and ".srvpgm" are added to the input path
   * consecutively
   */
  ...
  /* Loop to create a random file path that contains more than EsMaxPath + 1
   * chars */
  ...
  /* Reducing path length to EsMaxPath - strlen(extension) chars that includes
   * the NUL terminator */
  sharedLibName[EsMaxPath - extensionLength] = '\0';
  /* Avoid a path ending with a separator */
  sharedLibName[EsMaxPath - extensionLength - 1] = 'a';

  /* Check if OMRPORT_SL_UNSUPPORTED is received for a path of EsMaxPath -
   * strlen(extension) chars that includes the NUL terminator */
  rc = omrsl_open_shared_library(sharedLibName, &handle,
                                 OMRPORT_SLOPEN_DECORATE);
  if (OMRPORT_SL_UNSUPPORTED != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Received error code %zu instead of error code %zu for "
                       "shared library, \n%s,\n",
                       rc, (uintptr_t)OMRPORT_SL_UNSUPPORTED, sharedLibName,
                       omrerror_last_error_message());
    goto exit;
  }

  /* Reducing path length to EsMaxPath - strlen(extension) - 1 chars that
   * includes the NUL terminator */
  sharedLibName[EsMaxPath - extensionLength - 1] = '\0';
  /* Avoid a path ending with a separator */
  sharedLibName[EsMaxPath - extensionLength - 2] = 'a';

  /* Check if OMRPORT_SL_INVALID or OMRPORT_SL_NOT_FOUND is received for an
   * incorrect path of EsMaxPath - strlen(extension) - 1 chars that includes the
   * NUL terminator */
  rc = omrsl_open_shared_library(sharedLibName, &handle,
                                 OMRPORT_SLOPEN_DECORATE);
  if ((OMRPORT_SL_INVALID != rc) && (OMRPORT_SL_NOT_FOUND != rc)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Received error code %zu instead of error code %zu or "
                       "%zu for shared library, \n%s,\n",
                       rc, (uintptr_t)OMRPORT_SL_INVALID,
                       (uintptr_t)OMRPORT_SL_NOT_FOUND, sharedLibName,
                       omrerror_last_error_message());
    goto exit;
  }
...
/**
 * CMVC 197740
 * The exact message varies depending on the OS version. look for any of several
 * possible strings.
 ...
 */
static BOOLEAN isValidLoadErrorMessage(const char* dlerrorOutput,
                                       const char* possibleMessageStrings[]) {
...
/**
 * Loading dll with missing dependency on AIX, expecting a descriptive OS error
 * message
 */
TEST(PortSlTest, sl_AixDLLMissingDependency) {
  ...
  const char* osErrMsg;
  const char* possibleMessageStrings[] = {
      "0509-150", "Dependent module dummy.exp could not be loaded", NULL};
  ...
  if (!isValidLoadErrorMessage(osErrMsg, possibleMessageStrings)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       " Cannot find valid error code, should have failed with "
                       "dependency error\n",
                       sharedLibName, omrerror_last_error_message());
  }
exit:
...
/**
 * Loading dll of wrong platform on AIX, expecting a descriptive OS error
 * message
 */
TEST(PortSlTest, sl_AixDLLWrongPlatform) {
  ...
  const char* osErrMsg;
  const char* possibleMessageStrings[] = {
      "0509-103", "System error: Exec format error", "0509-026",
