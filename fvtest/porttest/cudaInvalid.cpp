...
TEST_F(CudaDeviceTest, invalidDevices)
{
  ...
  int32_t const expectedError =
    (0 == deviceCount) ? J9CUDA_ERROR_NO_DEVICE : J9CUDA_ERROR_INVALID_DEVICE;
...
#define DO_BAD_CALL(func, args)                                                \
  do {                                                                         \
    ...
    DO_BAD_CALL(funcGetAttribute,
                (deviceId, function, functionAttribute, &i32Val));
    ...
    DO_BAD_CALL(launchKernel,
                (deviceId, function, 1, 1, 1, 1, 1, 1, 0, NULL, NULL));
    DO_BAD_CALL(linkerAddData,
                (deviceId, linker, type, address, ptrVal, name, NULL));
