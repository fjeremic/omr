 ...
 *
 * @note port library error handling operations are not optional in the port
 * library table.
 *
...
TEST(PortErrorTest, error_test0) {
  ...
  if (NULL == OMRPORTLIB->error_startup) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->error_startup is NULL\n");
  }

  /* Not tested, implementation dependent.  No known functionality */
  if (NULL == OMRPORTLIB->error_shutdown) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->error_shutdown is NULL\n");
  }

  /* omrerror_test1 */
  if (NULL == OMRPORTLIB->error_last_error_number) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->error_last_error_number is NULL\n");
  }

  /* omrerror_test2 */
  if (NULL == OMRPORTLIB->error_last_error_message) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->error_last_error_message is NULL\n");
  }

  /* omrerror_test1 */
  if (NULL == OMRPORTLIB->error_set_last_error) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->error_set_last_error is NULL\n");
  }
  ...
  if (NULL == OMRPORTLIB->error_set_last_error_with_message) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "portLibrary->error_set_last_error_with_message is NULL\n");
  }

  if (NULL == OMRPORTLIB->error_set_last_error_with_message_format) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "portLibrary->error_set_last_error_with_message_format is NULL\n");
  }
 ...
 * function @ref omrerror.c::omrerror_set_last_error "omrerror_set_last_error()"
 * are recorded in the per thread buffers without an error message. Verify the
 * @ref omrerror_last_error_number "omrerror_last_error_number()" returns the
 * correct portable error code.
 */
TEST(PortErrorTest, error_test1) {
  ...
  if (0 != errorCode) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrerror_last_error_number() returned %d expected %d\n",
                       errorCode, 0);
  }
  ...
  if (200 != errorCode) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrerror_set_last_error() returned %d expected %d\n",
                       errorCode, 200);
  }
  errorCode = omrerror_last_error_number();
  if (200 != errorCode) {
 ...
 * Error codes are stored in per thread buffers so errors reported by one thread
 * do not effect those reported by a second.  Errors stored via the helper
 * function @ref omrerror.c::omrerror_set_last_error_with_message
 * "omrerror_set_last_error_with_message()" are recorded in the per thread
 * buffers without an error message. Verify the @ref omrerror_last_error_message
 * "omrerror_last_error_message()" returns the correct message.
 */
TEST(PortErrorTest, error_test2) {
  ...
  if (0 != strlen(message)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrerror_last_error_message() returned message of "
                       "length %d expected %d\n",
                       strlen(message), 0);
  }
 ...
 * Error codes are stored in per thread buffers so errors reported by one thread
 * do not effect those reported by a second.  Errors stored via the helper
 * function @ref omrerror.c::omrerror_set_last_error_with_message_format
 * "omrerror_set_last_error_with_message_format()" are recorded in the per
 * thread buffers without an error message. Verify the @ref
 * omrerror_last_error_message "omrerror_last_error_message()" returns the
 * correct message.
 */
TEST(PortErrorTest, error_test3) {
  ...
  const char* message;
  const char* formatMessage1 =
      "This is a test message with format specifiers %s %d";
  const char* formatMessage2 =
      "This is also a test message with format specifiers %s %d";
  ...
  errorCode = omrerror_set_last_error_with_message_format(200, formatMessage1,
                                                          arg1, arg2);
  message = omrerror_last_error_message();
  if (200 != errorCode) {
    outputErrorMessage(
  ...
  errorCode = omrerror_set_last_error_with_message_format(100, formatMessage2,
                                                          arg1, arg2);
  message = omrerror_last_error_message();
  if (100 != errorCode) {
    outputErrorMessage(
