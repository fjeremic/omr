...
extern int omrfile_runTests(
    struct OMRPortLibrary *portLibrary, char *argv0, char *omrfile_child,
    BOOLEAN asynch); /** @see omrfileTest.c::omrfile_runTests */
extern int omrsig_runTests(
    struct OMRPortLibrary *portLibrary, char *exeName,
    char *argument); /** @see omrsignalTest.c::omrsig_runTests */
extern int omrmmap_runTests(struct OMRPortLibrary *portLibrary, char *argv0,
                            char *omrmmap_child);
...
extern "C" int omr_main_entry(int argc, char **argv, char **envp) {
  ...
  if (isChild) {
    ...
    if (startsWith(testName, "omrfile")) {
      return omrfile_runTests(portTestEnv->getPortLibrary(), argv[0], testName,
                              FALSE);
