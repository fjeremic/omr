...
#define J9MMAP_TEST3_BUFFER_SIZE 100
  ...
  if (!(mmapCapabilities & OMRPORT_MMAP_CAPABILITY_COPYONWRITE)) {
    portTestEnv->log(
      LEVEL_ERROR, "OMRPORT_MMAP_CAPABILITY_COPYONWRITE not supported on this platform, bypassing test\n");
    goto exit;
  }
...
#define J9MMAP_TEST4_BUFFER_SIZE 100
  ...
  if (!(mmapCapabilities & OMRPORT_MMAP_CAPABILITY_WRITE) || !(mmapCapabilities & OMRPORT_MMAP_CAPABILITY_MSYNC)) {
    portTestEnv->log(
      LEVEL_ERROR, "OMRPORT_MMAP_CAPABILITY_WRITE or _MSYNC not supported on this platform, bypassing test\n");
    goto exit;
  }
...
TEST_F(PortMmapTest, mmap_test6)
{
  ...
  if (mmapCapabilities & OMRPORT_MMAP_CAPABILITY_PROTECT) {
    if (omrmmap_get_region_granularity(mapAddr) == 0) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "Mmap claims to be capable of PROTECT, however, region granularity is 0\n");
      goto exit;
    }
...
TEST_F(PortMmapTest, mmap_test9)
{
  ...
  if (!omrsig_can_protect(signalHandlerFlags)) {
    portTestEnv->log(LEVEL_ERROR,
      "Signal handling framework not available, can't test readonly functionality without crashing, bypassing test");
    goto exit;
  } else {
    ...
    protectResult
      = omrsig_protect(protectedWriter, &writerInfo, simpleHandlerFunction, &handlerInfo, signalHandlerFlags, &result);
...
#define J9MMAP_TEST1_BUFFER_SIZE 100
  ...
  /* Category tests aren't entirely straightforward - because the mmap implementation may have done other allocations as
   * part of the map.*/
...
int32_t omrmmap_runTests(struct OMRPortLibrary* portLibrary, char* argv0, char* omrmmap_child)
{
