...
#define J9MMAP_TEST1_BUFFER_SIZE 100
  ...
  mmapHandle = (J9MmapHandle*)omrmmap_map_file(fd,
                                               0,
                                               (uintptr_t)fileLength,
                                               NULL,
                                               OMRPORT_MMAP_FLAG_READ,
...
TEST_F(PortMmapTest, mmap_test9)
{
  ...
  if (!omrsig_can_protect(signalHandlerFlags)) {
    ...
    protectResult = omrsig_protect(protectedWriter,
                                   &writerInfo,
...
TEST_F(PortMmapTest, mmap_test10)
{
  ...
  /* set the region to read only */
  rc = omrmmap_protect(
    mapAddr, (uintptr_t)fileLength / 2, OMRPORT_PAGE_PROTECT_READ);
  if (rc != 0) {
    if (rc == OMRPORT_PAGE_PROTECT_NOT_SUPPORTED) {
      portTestEnv->log(LEVEL_ERROR,
                       "OMRPORT_MMAP_CAPABILITY_PROTECT not supported on this "
...
TEST_F(PortMmapTest, mmap_test11)
{
  ...
  /* set the region back to be writeable */
  (void)omrmmap_protect(mapAddr + pageSize,
                        pageSize,
                        OMRPORT_PAGE_PROTECT_READ | OMRPORT_PAGE_PROTECT_WRITE);
...
TEST_F(PortMmapTest, mmap_testDontNeed)
{
  ...
  for (cursor = 0; cursor < TEST_BUFF_LEN; ++cursor) {
    if (3 * cursor != testData[cursor]) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "Test buffer corrupted at index %d\n", cursor);
      break;
    }
  }
...
int32_t
omrmmap_runTests(struct OMRPortLibrary* portLibrary,
                 char* argv0,
                 char* omrmmap_child)
{
