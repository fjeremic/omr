 ...
 *
 * @note port library mmap system operations are not optional in the port
 * library table.
 * @note The default processing if memory mapping is not supported by the OS is
 * to allocate memory and read in the file.  This is the equivalent of
 * copy-on-write mmapping
 *
 */
#include <stdlib.h>
#include <string.h>
...
static uintptr_t protectedWriter(OMRPortLibrary *portLibrary, void *arg) {
  ...
  if (info->shouldCrash) {
    /* we should have crashed in the above call, so if we got here we didn't
     * have protection */
...
TEST_F(PortMmapTest, mmap_test0) {
  ...
  if (NULL == OMRPORTLIB->mmap_startup) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mmap_startup is NULL\n");
  }

  /* Not tested, implementation dependent.  No known functionality */
  if (NULL == OMRPORTLIB->mmap_shutdown) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mmap_shutdown is NULL\n");
  }

  if (NULL == OMRPORTLIB->mmap_map_file) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mmap_map_file is NULL\n");
  }

  if (NULL == OMRPORTLIB->mmap_unmap_file) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mmap_unmap_file is NULL\n");
  }

  if (NULL == OMRPORTLIB->mmap_msync) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mmap_msync is NULL\n");
  }

  if (NULL == OMRPORTLIB->mmap_protect) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mmap_protect is NULL\n");
  }

  if (NULL == OMRPORTLIB->mmap_get_region_granularity) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mmap_get_region_granularity is NULL\n");
  }
...
#define J9MMAP_TEST1_BUFFER_SIZE 100
  ...
  if (-1 == fileLength) {
    ...
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Getting length of file %s failed: lastErrorNumber=%d, "
                       "lastErrorMessage=%s\n",
                       filename, lastErrorNumber, lastErrorMessage);
    goto exit;
  }
  ...
  if (-1 == fd) {
    ...
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Open of file %s for mapping failed: "
                       "lastErrorNumber=%d, lastErrorMessage=%s\n",
                       filename, lastErrorNumber, lastErrorMessage);
    goto exit;
  }

  mmapHandle = (J9MmapHandle *)omrmmap_map_file(fd, 0, (uintptr_t)fileLength,
                                                NULL, OMRPORT_MMAP_FLAG_READ,
                                                OMRMEM_CATEGORY_PORT_LIBRARY);
  if ((NULL == mmapHandle) || (NULL == mmapHandle->pointer)) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Mmap_map_file of file %s failed: lastErrorNumber=%d, "
                       "lastErrorMessage=%s\n",
                       filename, lastErrorNumber, lastErrorMessage);
    goto exit;
  }
  ...
  if (-1 == rc) {
    ...
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Close of file %s after mapping failed: "
                       "lastErrorNumber=%d, lastErrorMessage=%s\n",
                       filename, lastErrorNumber, lastErrorMessage);
  }
  ...
  if (strcmp(mapAddr + 30, readableText) != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Strings do not match in mapped area\n");
  }
...
#define J9MMAP_TEST2_BUFFER_SIZE 100
  ...
  if (!(mmapCapabilities & OMRPORT_MMAP_CAPABILITY_WRITE)) {
    portTestEnv->log(LEVEL_ERROR, "OMRPORT_MMAP_CAPABILITY_WRITE not supported "
                                  "on this platform, bypassing test\n");
    goto exit;
  }
  ...
  if (-1 == fd) {
    ...
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Open of file %s for read after mapping failed: "
                       "lastErrorNumber=%d, lastErrorMessage=%s\n",
                       filename, lastErrorNumber, lastErrorMessage);
    goto exit;
  }
  ...
  if (-1 == rc) {
    ...
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Read of file %s after update failed: "
                       "lastErrorNumber=%d, lastErrorMessage=%s\n",
                       filename, lastErrorNumber, lastErrorMessage);
    goto exit;
  }
  ...
  if (-1 == rc) {
    ...
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Close of file %s after read failed: "
                       "lastErrorNumber=%d, lastErrorMessage=%s\n",
                       filename, lastErrorNumber, lastErrorMessage);
  }
  ...
  if (strcmp(updatedBuffer + 30, modifiedText) != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Modified string does not match in updated buffer\n");
  }
...
#define J9MMAP_TEST3_BUFFER_SIZE 100
  ...
  if (!(mmapCapabilities & OMRPORT_MMAP_CAPABILITY_COPYONWRITE)) {
    portTestEnv->log(LEVEL_ERROR,
                     "OMRPORT_MMAP_CAPABILITY_COPYONWRITE not supported on "
                     "this platform, bypassing test\n");
    goto exit;
  }
  ...
  mmapHandle = (J9MmapHandle *)omrmmap_map_file(
      fd, 0, (uintptr_t)fileLength, NULL, OMRPORT_MMAP_FLAG_COPYONWRITE,
      OMRMEM_CATEGORY_PORT_LIBRARY);
  if ((NULL == mmapHandle) || (NULL == mmapHandle->pointer)) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
  ...
  if (strcmp(updatedBuffer + 30, readableText) != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Original string does not match in updated buffer\n");
  }
...
#define J9MMAP_TEST4_BUFFER_SIZE 100
  ...
  if (!(mmapCapabilities & OMRPORT_MMAP_CAPABILITY_WRITE) ||
    ...
    portTestEnv->log(LEVEL_ERROR,
                     "OMRPORT_MMAP_CAPABILITY_WRITE or _MSYNC not supported on "
                     "this platform, bypassing test\n");
    goto exit;
  }
  ...
  if (-1 == rc) {
    ...
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "Msync failed: lastErrorNumber=%d, lastErrorMessage=%s\n",
...
TEST_F(PortMmapTest, mmap_test5) {
  ...
  /* parent */
  pid1 =
      launchChildProcess(OMRPORTLIB, testName, argv0, "-child_omrmmap_test5_1");
  if (NULL == pid1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
  ...
  pid2 =
      launchChildProcess(OMRPORTLIB, testName, argv0, "-child_omrmmap_test5_2");
  if (NULL == pid2) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
...
int omrmmap_test5_child_1(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
#define J9MMAP_TEST5_BUFFER_SIZE 100
  ...
  mmapHandle = (J9MmapHandle *)omrmmap_map_file(
      fd, 0, (uintptr_t)fileLength, mappingName, OMRPORT_MMAP_FLAG_WRITE,
      OMRMEM_CATEGORY_PORT_LIBRARY);
  if ((NULL == mmapHandle) || (NULL == mmapHandle->pointer)) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
  ...
  omrfile_create_status_file(OMRPORTLIB, "omrmmap_test5_child_1_mapped_file",
                             testName);

  while (!omrfile_status_file_exists(
      OMRPORTLIB, "omrmmap_test5_child_2_changed_file", testName)) {
    SleepFor(1);
  }

  portTestEnv->log(
      "Child 1: Child 2 has changed file, readable text = \"%s\"\n",
      mapAddr + 30);

  if (strcmp(mapAddr + 30, modifiedText) != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Modified string does not match in mapped buffer\n");
  }

  omrmmap_unmap_file(mmapHandle);

  portTestEnv->changeIndent(-1);
  omrfile_create_status_file(OMRPORTLIB, "omrmmap_test5_child_1_checked_file",
                             testName);
...
int omrmmap_test5_child_2(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
#define J9MMAP_TEST5_BUFFER_SIZE 100
  ...
  while (!omrfile_status_file_exists(
      OMRPORTLIB, "omrmmap_test5_child_1_mapped_file", testName)) {
    SleepFor(1);
  }
  ...
  if (strcmp(mapAddr + 30, readableText) != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Readable string does not match in mapped buffer\n");
  }
  ...
  omrfile_create_status_file(OMRPORTLIB, "omrmmap_test5_child_2_changed_file",
                             testName);

  while (!omrfile_status_file_exists(
      OMRPORTLIB, "omrmmap_test5_child_1_checked_file", testName)) {
    SleepFor(1);
  }
...
/**
 * Verify that if mmap has the capability to protect, that we get non-zero from
 * omrmmap_get_region_granularity
 *
 * Verify that if @ref omrmmap.c::omrmmap_protect "omrmmap_protect()" returns
 * OMRPORT_MMAP_CAPABILITY_PROTECT, that @ref
 * omrmmap::omrmmap_get_region_granularity "omrmmap_get_region_granularity()"
 * does not return 0
 */
TEST_F(PortMmapTest, mmap_test6) {
  ...
  if (mmapCapabilities & OMRPORT_MMAP_CAPABILITY_PROTECT) {
    if (omrmmap_get_region_granularity(mapAddr) == 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Mmap claims to be capable of PROTECT, however, "
                         "region granularity is 0\n");
      goto exit;
    }
    rc =
        omrmmap_protect(mapAddr, (uintptr_t)fileLength,
                        OMRPORT_PAGE_PROTECT_READ | OMRPORT_PAGE_PROTECT_WRITE);
    if ((rc & OMRPORT_PAGE_PROTECT_NOT_SUPPORTED) != 0) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "Mmap is claims to be capable of PROTECT, however, omrmmap_protect() "
          "returns OMRPORT_PAGE_PROTECT_NOT_SUPPORTED\n");
      goto exit;
    }
    ...
    if ((rc & OMRPORT_PAGE_PROTECT_NOT_SUPPORTED) == 0) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "Mmap is not capable of PROTECT, however, omrmmap_protect() does not "
...
#define J9MMAP_TEST1_BUFFER_SIZE 100
  ...
  if ((mmapCapabilities & OMRPORT_MMAP_CAPABILITY_PROTECT) == 0) {
    portTestEnv->log(LEVEL_ERROR,
                     "OMRPORT_MMAP_CAPABILITY_PROTECT not supported on this "
                     "platform, bypassing test\n");
    goto exit;
  }

  if ((mmapCapabilities & OMRPORT_MMAP_CAPABILITY_READ) == 0) {
    portTestEnv->log(LEVEL_ERROR, "OMRPORT_MMAP_CAPABILITY_READ not supported "
                                  "on this platform, bypassing test\n");
    goto exit;
  }
...
TEST_F(PortMmapTest, mmap_test8) {
  ...
  (void)omrmmap_protect(mapAddr, (uintptr_t)fileLength,
                        OMRPORT_PAGE_PROTECT_READ | OMRPORT_PAGE_PROTECT_WRITE);
 ...
 *
 * Verify @ref omrmmap.c::omrmmap_protect "omrmmap_protect()" with
 * OMRPORT_PAGE_PROTECT_READ causes us to crash when writing to the protected
 * region, and that we are once again able to write when we set it back to
 * OMRPORT_PAGE_PROTECT_READ | OMRPORT_PAGE_PROTECT_READ
 */
TEST_F(PortMmapTest, mmap_test9) {
  ...
  int32_t mmapCapabilities = omrmmap_capabilities();
  J9MmapHandle *mmapHandle = NULL;
  uint32_t signalHandlerFlags = OMRPORT_SIG_FLAG_MAY_RETURN |
                                OMRPORT_SIG_FLAG_SIGSEGV |
                                OMRPORT_SIG_FLAG_SIGBUS;
  ...
  mmapHandle = (J9MmapHandle *)omrmmap_map_file(
      fd, 0, (uintptr_t)fileLength, NULL,
      OMRPORT_MMAP_FLAG_WRITE | OMRPORT_MMAP_FLAG_SHARED,
      OMRMEM_CATEGORY_PORT_LIBRARY);
  if ((NULL == mmapHandle) || (NULL == mmapHandle->pointer)) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
  ...
  rc = omrmmap_protect(mapAddr, (uintptr_t)fileLength,
                       OMRPORT_PAGE_PROTECT_READ);
  if (rc != 0) {
  ...
  if (!omrsig_can_protect(signalHandlerFlags)) {
    portTestEnv->log(LEVEL_ERROR,
                     "Signal handling framework not available, can't test "
                     "readonly functionality without crashing, bypassing test");
    goto exit;
  } else {
    ...
    protectResult =
        omrsig_protect(protectedWriter, &writerInfo, simpleHandlerFunction,
                       &handlerInfo, signalHandlerFlags, &result);

    if (protectResult == OMRPORT_SIG_EXCEPTION_OCCURRED) {
      /* test passed */
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Able to write to read only region\n");
    }
  }
...
/**
 * Verify the page size is correct - create a file over two pages, protect the
 * first and then write to the second page (should be ok) and the last byte of
 * the first page (should fail).
 */
TEST_F(PortMmapTest, mmap_test10) {
  ...
  /* Initialize buffer to be written - 128 chars with a string right in the
   * middle */
  for (i = 0; i < 128; i++) {
    buffer[i] = (char)i;
  }
  ...
  if ((uint64_t)fileLength != pageSize * 2) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "File is wrong length, should be %d but is %d\n",
  ...
  /* set the region to read only */
  rc = omrmmap_protect(mapAddr, (uintptr_t)fileLength / 2,
                       OMRPORT_PAGE_PROTECT_READ);
  if (rc != 0) {
    if (rc == OMRPORT_PAGE_PROTECT_NOT_SUPPORTED) {
      portTestEnv->log(LEVEL_ERROR,
  ...
  if (!omrsig_can_protect(signalHandlerFlags)) {
    ...
    if (protectResult == OMRPORT_SIG_EXCEPTION_OCCURRED) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Crashed writing to writable page\n");
    } else {
      /* test passed */
    }
  }
  ...
  /* set the region back to be writeable */
  (void)omrmmap_protect(mapAddr, (uintptr_t)fileLength / 2,
                        OMRPORT_PAGE_PROTECT_READ | OMRPORT_PAGE_PROTECT_WRITE);
...
TEST_F(PortMmapTest, mmap_test11) {
  ...
  /* set the region back to be writeable */
  (void)omrmmap_protect(mapAddr + pageSize, pageSize,
                        OMRPORT_PAGE_PROTECT_READ | OMRPORT_PAGE_PROTECT_WRITE);
...
#define J9MMAP_TEST1_BUFFER_SIZE 100
  ...
  /* Category tests aren't entirely straightforward - because the mmap
   * implementation may have done other allocations as part of the map.*/
  if (finalBlocks <= initialBlocks) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Block count did not increase as expected. "
                       "initialBlocks=%zu, finalBlocks=%zu\n",
                       initialBlocks, finalBlocks);
  }

  if (finalBytes < (initialBytes + fileLength)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Byte count did not increase as expected. "
                       "initialBytes=%zu, finalBytes=%zu, fileLength=%lld\n",
  ...
  if (finalBlocks != initialBlocks) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Block count did not decrease as expected. "
                       "initialBlocks=%zu, finalBlocks=%zu\n",
                       initialBlocks, finalBlocks);
  }
  ...
  if (finalBytes != initialBytes) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Byte count did not decrease as expected. "
                       "initialBytes=%zu, finalBytes=%zu\n",
...
#define J9MMAP_TEST13_BUFFER_SIZE 100
  ...
  mmapHandle = (J9MmapHandle *)omrmmap_map_file(
      fd, 0, 0, NULL, OMRPORT_MMAP_FLAG_READ, OMRMEM_CATEGORY_PORT_LIBRARY);
  if ((NULL == mmapHandle) || (NULL == mmapHandle->pointer)) {
    lastErrorMessage = (char *)omrerror_last_error_message();
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS,
...
TEST_F(PortMmapTest, mmap_testDontNeed) {
  ...
  reportTestEntry(OMRPORTLIB, testName);
  testData = (uint32_t *)omrmem_allocate_memory(
      TEST_BUFF_LEN * sizeof(uint32_t), OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == testData) {
  ...
  for (cursor = 0; cursor < TEST_BUFF_LEN; ++cursor) {
    if (3 * cursor != testData[cursor]) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Test buffer corrupted at index %d\n", cursor);
      break;
    }
  }
...
int32_t omrmmap_runTests(struct OMRPortLibrary *portLibrary, char *argv0,
                         char *omrmmap_child) {
