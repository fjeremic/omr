...
/**
 * Verify that a call to omrtty_printf/omrtty_vprintf after a call to
 * omrtty_daemonize does not crash.
 *
 * Call omrtty_daemonize, then call omrtty_printf/omrtty_vprintf. If we crash or
 * print anything, then the test has failed.
 */
#if defined(OMR_OS_WINDOWS)
TEST(PortTtyTest, tty_daemonize_test)
{
  ...
  /* Redirect output to a test handle and restart TTY to get the port functions
   * to use it. */
  HANDLE stdoutWrite;
  HANDLE stdoutRead;
  ...
  if (NULL == strstr(readBuffer, "post-daemonize")) {
    outputComment(OMRPORTLIB,
                  "Test passed: 'post-daemonize' not found in test output.\n");
  } else {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "Test failed: post-daemonize unexpectedly found in output.\n");
  }
