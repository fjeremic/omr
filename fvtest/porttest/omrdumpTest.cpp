 ...
 * functions can be found in the file @ref omrosdump.c
 *
 *
 */
#include <stdio.h>
...
static uintptr_t simpleHandlerFunction(struct OMRPortLibrary *portLibrary,
                                       ...
                                       void *handler_arg);
static void removeDump(OMRPortLibrary *portLib, const char *filename,
                       const char *testName);
 ...
 *
 * Ensure the port library is properly setup to run core file generation
 * operations.
 */
...
/**
 * Call omrdump_create() without passing in core file name. This does not
 * actually test that a core file was actually created.
 */
TEST(PortDumpTest, dump_test_create_dump_with_NO_name) {
  ...
  if (rc == 0) {
    ...
    portTestEnv->log(
        "omrdump_create claims to have written a core file to: %s\n",
        coreFileName);

#if defined(AIXPPC)
    /* We defer to fork abort on AIX machines that don't have "Enable full CORE
     * dump" enabled in smit, in which case omrdump_create will not return the
     * filename. So, only check for a specific filename if we are getting full
     * core dumps */
    ...
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrdump_create returned: %u, with filename: %s", rc,
                       coreFileName);
  }
  reportTestExit(OMRPORTLIB, testName);
}
...
TEST(PortDumpTest, dump_test_create_dump_with_name) {
  ...
  strncat(coreFileName, "/",
          EsMaxPath); /* make sure the directory ends with a slash */
  ...
  if (rc == 0) {
    ...
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrdump_create returned: %u, with filename: %s\n", rc,
                       coreFileName);
  }
  reportTestExit(OMRPORTLIB, testName);
}
...
TEST(PortDumpTest, dump_test_create_dump_from_signal_handler) {
  ...
  if (!omrsig_can_protect(sig_protectFlags)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->sig_protect does not offer adequate "
                       "protection for test\n");
  } else {
    handlerInfo.coreFileName = coreFileName;
    handlerInfo.testName = testName;
    protectResult = omrsig_protect(raiseSEGV, NULL, simpleHandlerFunction,
                                   &handlerInfo, sig_protectFlags, &result);
