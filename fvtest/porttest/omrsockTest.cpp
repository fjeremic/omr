 ...
 */
int32_t start_server(struct OMRPortLibrary *portLibrary, const char *addrStr,
                     const char *port, int32_t family,
                     omrsock_socket_t *serverSocket,
                     omrsock_sockaddr_t serverAddr) {
 ...
 * @return 0 on success, return an error otherwise.
 */
int32_t connect_client_to_server(struct OMRPortLibrary *portLibrary,
                                 const char *addrStr, const char *port,
                                 int32_t family,
                                 omrsock_socket_t *sessionClientSocket,
                                 omrsock_sockaddr_t sessionClientAddr) {
  return OMRPORT_ERROR_NOTEXIST;
}
 ...
 *
 * This will iterate through all OMRSock functions in @ref omrsock.c and simply
 * check that the location of the function is not NULL.
 *
 * @note Errors such as an OMRSock function call with a NULL address will be
 * reported.
 */
TEST(PortSockTest, library_function_pointers_not_null) { /* Unimplemented. */ }

/**
 * Test the omrsock per thread buffer.
 *
 * In this test, per thread buffer related functions set up a buffer to store
 * information such as the hints structures created in @ref
 * omrsock_getaddrinfo_create_hints.
 */
TEST(PortSockTest, per_thread_buffer_functionality) { /* Unimplemented. */ }
 ...
 *
 * @note Errors such as failed function calls, and/or returning the wrong family
 * or socket type from @ref omrsock_getaddrinfo compared to the ones passed into
 * hints, will be reported.
 */
TEST(PortSockTest, getaddrinfo_creation_and_extraction) { /* Unimplemented. */ }

/**
 * Test functions to set up a connection by using two sockets, which talk to
 * each other.
 *
 ...
 *
 * @note Errors such as failed function calls, failure to create server and/or
 * client, wrong message sent/received, will be reported.
 */
TEST(PortSockTest, two_socket_communication) { /* Unimplemented. */ }
