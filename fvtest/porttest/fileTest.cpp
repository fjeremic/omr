...
protected:
  virtual void SetUp() {
    ...
    /* generate machine specific file name to prevent conflict between multiple
     * tests running on shared drive */
    ...
    omrstr_printf(
        fileName, fileNameLen, "TestFileFor%s%s",
...
TEST_F(PortFileTest, WriteRead) {
  ...
  OMRTEST_ASSERT_TRUE((-1 != fileDescriptor),
                      "omrfile_open \"" << fileName << "\" failed\n");

  fileWrite(OMRPORTLIB, fileDescriptor, "Try a number - %d", 1);

  fileWrite(OMRPORTLIB, fileDescriptor, "Try a string - %s", "abc");
  fileWrite(OMRPORTLIB, fileDescriptor, "Try a mixture - %d %s %d", 1, "abc",
            2);
  ...
  OMRTEST_ASSERT_TRUE((0 == rc), "omrfile_close() returned " << rc
                                                             << " expected "
                                                             << "0\n");
  ...
  OMRTEST_ASSERT_TRUE((!strcmp(readBufPtr, expectedResult)),
                      "omrfile_read_text() returned \""
                          << readBufPtr << "\" expected \"" << expectedResult
                          << "\"\n");
}
...
TEST_F(PortFileTest, WriteAppendRead) {
  ...
  fileDescriptor =
      omrfile_open(fileName, EsOpenWrite | EsOpenCreate | EsOpenTruncate, 0666);
  OMRTEST_ASSERT_TRUE((-1 != fileDescriptor),
                      "omrfile_open \"" << fileName << "\" failed\n");
  ...
  OMRTEST_ASSERT_TRUE((rc == expectedReturnValue),
                      "omrfile_write() returned "
                          << rc << " expected " << expectedReturnValue << "\n");
  ...
  OMRTEST_ASSERT_TRUE((rc == expectedReturnValue), "omrfile_write() returned "
                                                       << rc << ", expected "
                                                       << expectedReturnValue
                                                       << "\n");
  ...
  OMRTEST_ASSERT_TRUE((rc == expectedReturnValue), "omrfile_read() returned "
                                                       << rc << ", expected "
                                                       << expectedReturnValue
                                                       << "\n");

  OMRTEST_ASSERT_TRUE((!strcmp(expectedOutput, outputLine)),
                      "Read back data \"" << outputLine
                                          << "\" not matching expected data \""
                                          << expectedOutput << "\"\n");
