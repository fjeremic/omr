 ...
 */
bool CudaTest::fillVerify(const void *buffer, uintptr_t size, const void *fill,
                          uintptr_t fillSize) {
 ...
 */
bool CudaTest::patternVerify(const void *buffer, uintptr_t size,
                             uint32_t seed) {
  const uint8_t *bufferBytes = (const uint8_t *)buffer;
  uint64_t value = seed << 16;
  uint64_t i = 0;
...
static const J9CudaCacheConfig cacheConfigs[] = {
    J9CUDA_CACHE_CONFIG_PREFER_EQUAL, J9CUDA_CACHE_CONFIG_PREFER_L1,
...
static const J9CudaSharedMemConfig sharedMemConfigs[] = {
...
const CudaTestArray<const J9CudaSharedMemConfig>
    CudaDeviceTest::allSharedMemConfigs = {LENGTH_OF(sharedMemConfigs),
                                           sharedMemConfigs};
