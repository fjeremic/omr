...
static int omrvmem_testReserveMemoryEx_impl(
  struct OMRPortLibrary*, const char*, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN);
 ...
 */
static void verifyMemory(
  struct OMRPortLibrary* portLibrary, const char* testName, char* memPtr, uintptr_t byteAmount, const char* allocName)
{
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
...
#endif /* J9ZOS390 */
    memPtr = (char*)omrvmem_reserve_memory(0, pageSizes[i], &vmemID,
      OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE | OMRPORT_VMEM_MEMORY_MODE_COMMIT, pageSizes[i],
    ...
    if (memPtr == NULL) {
      ...
      portTestEnv->log("reserved and committed 0x%zx bytes with page size 0x%zx at address 0x%zx\n", pageSizes[i],
        vmemID.pageSize, memPtr);

      getPortLibraryMemoryCategoryData(OMRPORTLIB, &finalBlocks, &finalBytes);

      if (finalBlocks <= initialBlocks) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
          "vmem reserve didn't increment category block as expected. Final blocks=%zu, initial blocks=%zu, page "
          "size=%zu.\n",
      ...
      if (finalBytes < (initialBytes + pageSizes[i])) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
          "vmem reserve didn't increment category bytes as expected. Initial bytes=%zu, final bytes=%zu, page "
          "size=%zu.\n",
...

    {
      ...
      if (finalBlocks != initialBlocks) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
          "vmem free didn't decrement category block as expected. Final blocks=%zu, initial blocks=%zu, page "
          "size=%zu.\n",
          finalBlocks, initialBlocks, pageSizes[i]);
      }
      ...
      if (finalBytes != initialBytes) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
          "vmem free didn't decrement category bytes as expected. Initial bytes=%zu, final bytes=%zu, page size=%zu.\n",
...
TEST(PortVmemTest, vmem_test_free_memory)
{
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (pageSizes[i] < sizeof(struct J9PortVmemIdentifier)) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
        "omrvmem_free_memory pageSize is smaller than sizeof(J9PortVmemIdentifier). pageSize: 0x%zx "
        "sizeof(J9PortVmemIdentifier): 0x%zx\n",
        pageSizes[i], sizeof(struct J9PortVmemIdentifier));
      goto exit;
    }

    J9PortVmemIdentifier* vmemID2 = (J9PortVmemIdentifier*)omrvmem_reserve_memory(0, pageSizes[i], &vmemID,
      OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE | OMRPORT_VMEM_MEMORY_MODE_COMMIT, pageSizes[i],
      OMRMEM_CATEGORY_PORT_LIBRARY);
 ...
 * @param[in] allocName Calling function name to display in errors
 */
static void verifyContiguousMem(struct OMRPortLibrary* portLibrary, const char* testName, size_t pagesize,
  size_t arrayletSize, void* contiguous, void* addresses[], char* vals, const char* allocName)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  /* reserve and commit memory for heap size */
  {
...
#endif /* J9ZOS390 */
    memPtr = (char*)omrvmem_reserve_memory(0, HEAP_SIZE, &vmemID,
      OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE | OMRPORT_VMEM_MEMORY_MODE_COMMIT
        | OMRPORT_VMEM_MEMORY_MODE_SHARE_FILE_OPEN,
    ...
    /* Memory verified successfully */
    {
      ...
      void* contiguous = omrvmem_get_contiguous_region_memory(arrayletLeaveAddrs, ARRAYLET_COUNT, arrayletLeafSize,
        ...
        OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE | OMRPORT_VMEM_MEMORY_MODE_COMMIT, pageSize,
        category);
      if (contiguous == NULL) {
        ...
        verifyContiguousMem(
          OMRPORTLIB, testName, pageSize, arrayletLeafSize, contiguous, arrayletLeaveAddrs, vals, allocName);
...
static void setDisclaimVirtualMemory(struct OMRPortLibrary* portLibrary, BOOLEAN disclaim)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  if (disclaim) {
 ...
 */
int omrvmem_bench_write_and_decommit_memory(struct OMRPortLibrary* portLibrary, uintptr_t pageSize,
  uintptr_t byteAmount, BOOLEAN disclaim, uintptr_t numIterations)
{
  ...
  for (i = 0; numIterations != i; i++) {
    ...
    if (NULL == memPtr) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
        "omrvmem_commit_memory returned NULL when trying to commit 0x%zx bytes backed by 0x%zx-byte pages\n",
        byteAmount, pageSize);
      goto exit;
    }
  ...
  portTestEnv->log(
    "%s pageSize 0x%zx, byteAmount 0x%zx, millis for %u iterations: [write_to_all/decommit] test: %lli\n",
...
int omrvmem_bench_force_overcommit_then_decommit(
  struct OMRPortLibrary* portLibrary, uintptr_t physicalMemorySize, uintptr_t pageSize, BOOLEAN disclaim)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  deltaMillis = (omrtime_nano_time() - startTimeNanos) / (1000 * 1000);
  portTestEnv->log("%s pageSize 0x%zx, byteAmount 0x%zx, millis: [force page, decommit pagedout region, write "
                   "decommitted region] test: %lli\n",
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int omrvmem_bench_reserve_write_decommit_and_free_memory(struct OMRPortLibrary* portLibrary, uintptr_t pageSize,
  uintptr_t byteAmount, BOOLEAN disclaim, uintptr_t numIterations)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  for (i = 0; numIterations != i; i++) {
    ...
    if (memPtr == NULL) {
      ...
      lastErrorNumber = omrerror_last_error_number();
      outputErrorMessage(PORTTEST_ERROR_ARGS,
        "In iteration %d, unable to reserve and commit 0x%zx bytes with page size 0x%zx.\n"
        "\tlastErrorNumber=%d, lastErrorMessage=%s\n",
  ...
  portTestEnv->log(
    "%s pageSize 0x%zx, byteAmount 0x%zx, millis for %u iterations: [reserve/write_to_all/decommit/free] test: %lli\n",
    disclaim ? "disclaim" : "nodisclaim", pageSize, byteAmount, numIterations, deltaMillis);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int omrvmem_exhaust_virtual_memory(
  struct OMRPortLibrary* portLibrary, uintptr_t pageSize, uintptr_t byteAmount, BOOLEAN disclaim)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
#define NUM_VMEM_IDS 1000
  ...
  portTestEnv->log(
    "%s pageSize 0x%zx, byteAmount 0x%zx, millis for %u iterations: [reserve/write_to_all/decommit] test: %lli\n",
...
TEST(PortVmemTest, vmem_testReserveMemoryEx)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    ...
    portTestEnv->log(
      LEVEL_ERROR, "***Did not find enough memory available on system, not running test omrvmem_testReserveMemoryEx\n");
  }
}
...
TEST(PortVmemTest, vmem_testReserveMemoryExTopDown)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB, "omrvmem_testReserveMemoryExTopDown", TRUE, FALSE, FALSE, FALSE);
  } else {
    portTestEnv->log(LEVEL_ERROR,
      "***Did not find enough memory available on system, not running test omrvmem_testReserveMemoryExTopDown\n");
  }
}
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictAdress)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, TRUE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB, "omrvmem_testReserveMemoryExStrictAddress", FALSE, TRUE, FALSE, FALSE);
  } else {
    portTestEnv->log(LEVEL_ERROR,
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictPageSize)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB, "omrvmem_testReserveMemoryExStrictPageSize", FALSE, FALSE, TRUE, FALSE);
  } else {
    portTestEnv->log(LEVEL_ERROR,
...
TEST(PortVmemTest, vmem_testReserveMemoryEx_use2To32)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB, "omrvmem_testReserveMemoryEx_use2To32", FALSE, FALSE, FALSE, TRUE);
  } else {
    portTestEnv->log(
      LEVEL_ERROR, "***Did not find enough memory available on system, not running test omrvmem_testReserveMemoryEx\n");
  }
...
TEST(PortVmemTest, vmem_testReserveMemoryExTopDown_use2To32)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB, "omrvmem_testReserveMemoryExTopDown_use2To32", TRUE, FALSE, FALSE, TRUE);
  } else {
    portTestEnv->log(LEVEL_ERROR,
      "***Did not find enough memory available on system, not running test omrvmem_testReserveMemoryExTopDown\n");
  }
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictAddress_use2To32)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, TRUE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB, "omrvmem_testReserveMemoryExStrictAddress_use2To32", FALSE, TRUE, FALSE, TRUE);
  } else {
    portTestEnv->log(LEVEL_ERROR,
      "***Did not find enough memory available on system, not running test omrvmem_testReserveMemoryExStrictAddress\n");
  }
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictPageSize_use2To32)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB, "omrvmem_testReserveMemoryExStrictPageSize_use2To32", FALSE, FALSE, TRUE, TRUE);
  } else {
    portTestEnv->log(LEVEL_ERROR,
      "***Did not find enough memory available on system, not running test "
      "omrvmem_testReserveMemoryExStrictPageSize\n");
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int omrvmem_testReserveMemoryEx_StandardAndQuickMode(struct OMRPortLibrary* portLibrary, const char* testName,
  BOOLEAN topDown, BOOLEAN strictAddress, BOOLEAN strictPageSize, BOOLEAN use2To32G)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  int rc
    = omrvmem_testReserveMemoryEx_impl(OMRPORTLIB, testName, topDown, FALSE, strictAddress, strictPageSize, use2To32G);
#if defined(LINUX) || defined(OSX)
  if (TEST_PASS == rc) {
    rc |= omrvmem_testReserveMemoryEx_impl(
      OMRPORTLIB, testName, topDown, TRUE, strictAddress, strictPageSize, use2To32G);
  }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int omrvmem_testReserveMemoryEx_impl(struct OMRPortLibrary* portLibrary, const char* testName, BOOLEAN topDown,
  BOOLEAN quickSearch, BOOLEAN strictAddress, BOOLEAN strictPageSize, BOOLEAN use2To32G)
{
  portTestEnv->changeIndent(1);
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    for (j = 0; j < NUM_SEGMENTS; j++) {
      ...
      if (memPtr[j] == NULL) {
        ...
        && (((void*)memPtr[j] < params[j].startAddress) || ((void*)memPtr[j] > params[j].endAddress))) {
        /* if returned pointer is outside of range then fail */
        outputErrorMessage(PORTTEST_ERROR_ARGS,
          "Strict address flag set and returned pointer [0x%zx] is outside of range.\n"
          "\tlastErrorNumber=%d, lastErrorMessage=%s\n",
        ...
        outputErrorMessage(PORTTEST_ERROR_ARGS,
          "Strict page size flag set and returned memory has a page size of [0x%zx] "
        ...
        outputErrorMessage(PORTTEST_ERROR_ARGS,
          "\tuse2To32G flag is set and returned pointer [0x%zx] is outside of range.\n", memPtr[j]);
      ...
      if (NULL != memPtr[j]) {
        ...
        if (vmemID[j].pageSize != omrvmem_get_page_size(&(vmemID[j]))) {
          outputErrorMessage(PORTTEST_ERROR_ARGS, "vmemID[j].pageSize (0x%zx)!= omrvmem_get_page_size (0x%zx) .\n",
        ...
        if (vmemID[j].pageFlags != omrvmem_get_page_flags(&(vmemID[j]))) {
          outputErrorMessage(PORTTEST_ERROR_ARGS, "vmemID[j].pageFlags (0x%zx)!= omrvmem_get_page_flags (0x%zx) .\n",
        ...
        if (finalBytes < (initialBytes + pageSizes[i])) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
            "vmem reserve didn't increment category bytes as expected. Final bytes=%zu, initial bytes=%zu, page "
            "size=%zu.\n",
            finalBytes, initialBytes, pageSizes[i]);
        }
      }
    ...
    for (j = 0; j < NUM_SEGMENTS; j++) {
      if (NULL != memPtr[j]) {
        ...
        if (finalBlocks >= initialBlocks) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
            "vmem free didn't decrement category blocks as expected. Final blocks=%zu, initial blocks=%zu, page "
            "size=%zu.\n",
            finalBlocks, initialBlocks, pageSizes[i]);
        }
        ...
        if (finalBytes > (initialBytes - pageSizes[i])) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
            "vmem free didn't decrement category bytes as expected. Final bytes=%zu, initial bytes=%zu, page "
            "size=%zu.\n",
            finalBytes, initialBytes, pageSizes[i]);
        }
      }
 ...
 *
 */
static BOOLEAN memoryIsAvailable(struct OMRPortLibrary* portLibrary, BOOLEAN strictAddress)
{
  portTestEnv->changeIndent(1);
#define NUM_SEGMENTS 3
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    for (j = 0; j < NUM_SEGMENTS; j++) {
      ...
      if (memPtr[j] == NULL) {
        portTestEnv->log(
          LEVEL_ERROR, "**Could not find 0x%zx bytes available with page size 0x%zx\n", pageSizes[i], pageSizes[i]);
    ...
    for (j = 0; j < NUM_SEGMENTS; j++) {
      ...
      if (NULL != memPtr[j]) {
        ...
        if (rc != 0) {
          isMemoryAvailable = FALSE;
          portTestEnv->log("**omrvmem_free_memory returned %i when trying to free 0x%zx bytes at 0x%zx\n", rc,
            pageSizes[i], memPtr[j]);
        }
      }
    }
  }
...
TEST(PortVmemTest, vmem_testReserveMemoryEx_zOSLargePageBelowBar)
{
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (NULL == memPtr) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
        "unable to reserve and commit 0x%zx bytes with page size 0x%zx, page type 0x%zx.\n", params.byteAmount,
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictAddress_zOSLargePageBelowBar)
{
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (TWO_GB <= pageSizes[i]) {
       ...
       * If the page size is >= 2G, then attempt to allocate memory with OMRPORT_VMEM_STRICT_ADDRESS flag is expected to
       * fail.
       */
      continue;
    }
    ...
    if (NULL == memPtr) {
      ...
      if (FALSE == isPageSizeSupportedBelowBar(pageSizes[i], pageFlags[i])) {
        if (FOUR_KB != vmemID.pageSize) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
            "\tdidn't expect to allocate memory below 2GB using page size 0x%zx and page flags 0x%x with strict "
            "address\n",
...
/**
 * Test request for pages below the 2G bar with OMRPORT_VMEM_STRICT_ADDRESS and OMRPORT_VMEM_ZOS_USE2TO32G_ARE set on
 * z/OS
 */
TEST(PortVmemTest, vmem_testReserveMemoryExStrictAddress_use2To32_zOSLargePageBelowBar)
{
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    /* OMRPORT_VMEM_ZOS_USE2TO32G_AREA allocates memory in 2-32G but the requested address is below 2G with
     * STRICT_ADDRESS set The two conditions are incompatible and should return NULL.
     */
    if (memPtr != NULL) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
        "Did not expect to allocate memory using page size 0x%zx, page type 0x%zx in < 2GB range with "
      ...
      portTestEnv->log(" Expected to fail to allocate large pages in < 2GB range with OMRPORT_VMEM_ZOS_USE2TO32G_AREA "
                       "and OMRPORT_VMEM_STRICT_ADDRESS set\n");
    }
  }
  portTestEnv->changeIndent(-1);
  reportTestExit(OMRPORTLIB, testName);
...
TEST(PortVmemTest, vmem_testReserveLargePagesAboveBar)
{
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    for (pageableStrict = 0; pageableStrict < 4;
         ++pageableStrict) { /* try with both pageable and non-pageable, strict and non-strict */
      ...
      if (NULL != memPtr) {
        ...
        if (expectError) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
            "no error from omrvmem_reserve_memory_ex returned 0x%zx for pageSize=0x%zx pageFlags=0x%zx\n", rc,
        ...
        if (pageable) {
          if (0 != rc) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
              "omrvmem_decommit_memory failed with code 0x%zx when trying to decommit 0x%zx bytes backed by 0x%zx-byte "
              "pages\n",
          ...
          if ((0 == rc) && strict) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
              "omrvmem_decommit_memory no error when trying to decommit fixed pages size=0x%zx flags=0x%zx\n",
              params.pageSize, params.pageFlags);
          }
        }
        omrvmem_free_memory(memPtr, params.byteAmount, &vmemID);
        ...
        portTestEnv->log(LEVEL_ERROR,
          "Failed to allocate memory using page size 0x%zx, pageFlags 0x%zx params.options  0x%zx\n", pageSizes[i],
          params.pageFlags, params.options);
      }
    }
  }
  portTestEnv->changeIndent(-1);
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (NULL == memPtr) {
      ...
      if (NULL != commitResult) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
          "omrvmem_commit_memory did not return an error when attempting to commit more memory"
          "than was reserved. \n");
      } else {
        ...
        if (NULL == commitResult) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
            "omrvmem_commit_memory returned an error while attempting to commit reserved memory: %s\n",
          ...
          if (0 == rc) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
              "omrvmem_decommit_memory did not return an error when attempting to decommit "
            ...
            if (rc != 0) {
              outputErrorMessage(PORTTEST_ERROR_ARGS,
                "omrvmem_decommit_memory returned an error when attempting to decommit "
      ...
      if (rc != 0) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
          "omrvmem_free_memory returned %d when trying to free 0x%zx bytes backed by 0x%zx-byte pages\n", rc,
          params.byteAmount, pageSizes[i]);
      }
    }
  }
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (NULL == memPtr) {
      ...
      *((unsigned int*)memPtr) = (unsigned int)0x4e800020; /* blr instruction (equivalent to RET on x86)*/
      ...
      if (FALSE == isPageSizeSupportedBelowBar(pageSizes[i], pageFlags[i])) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
          "Did not expect to allocate executable pages with large page size 0x%zx, page type 0x%zx\n", pageSizes[i],
          pageFlags[i]);
      } else {
#endif /* J9ZOS39064 */
...
TEST(PortVmemTest, vmem_test_numa)
{
  ...
  if ((detailReturnCode != 0) || (0 == totalNumaNodeCount)) {
    ...
    J9MemoryNodeDetail* nodeArray
      = (J9MemoryNodeDetail*)omrmem_allocate_memory(nodeArraySize, OMRMEM_CATEGORY_PORT_LIBRARY);
    intptr_t rc = 0;

    if (nodeArray == NULL) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
        "omrvmem_free_memory returned NULL when trying to allocate J9MemoryNodeDetail array for NUMA test (FAIL)\n");
      goto exit;
    }
    ...
    if ((detailReturnCode != rc) || (totalNumaNodeCount != originalNodeCount)) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
        "omrvmem_numa_get_node_details returned different results on second call (return %zd versus %zd, count %zu "
        "versus %zu)\n",
    ...
    if (0 == nodesWithMemoryCount) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "Found zero nodes with memory even after NUMA was reported as supported (FAIL)\n");
      goto exit;
    }
    /* reserve and commit memory for each page size */
    for (i = 0; pageSizes[i] != 0; i++) {
...
#endif /* J9ZOS390 */
      memPtr = (char*)omrvmem_reserve_memory(NULL, reserveSizeInBytes, &vmemID,
        OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE, pageSize, OMRMEM_CATEGORY_PORT_LIBRARY);
      ...
      if (memPtr != NULL) {
        ...
        for (nodeIndex = 0; nodeIndex < totalNumaNodeCount; nodeIndex++) {
          if (policyForMemoryNode == nodeArray[nodeIndex].memoryPolicy) {
            ...
            if (rc != 0) {
              outputErrorMessage(PORTTEST_ERROR_ARGS,
                "omrvmem_numa_set_affinity returned 0x%zx when trying to set affinity for 0x%zx bytes at 0x%p to node "
                "%zu\n",
...
#define PRINT_FIND_VALID_PAGE_SIZE_INPUT(mode, pageSize, pageFlags) \
  portTestEnv->log(                                                 \
    "Input > mode: %zu, requestedPageSize: 0x%zx, requestedPageFlags: 0x%zx\n", mode, pageSize, pageFlags)

void verifyFindValidPageSizeOutput(struct OMRPortLibrary* portLibrary, const char* testName, uintptr_t pageSizeExpected,
  uintptr_t pageFlagsExpected, uintptr_t isSizeSupportedExpected, uintptr_t pageSizeReturned,
...
static int omrvmem_testFindValidPageSize_impl(struct OMRPortLibrary* portLibrary, const char* testName)
{
#define MODE_NOT_USED 0
  ...
  verifyFindValidPageSizeOutput(OMRPORTLIB, testName, expectedPageSize, expectedPageFlags, expectedIsSizeSupported,
...
#define MODE_NOT_USED 0
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
#if defined(OMR_ENV_DATA64)
    if ((8 * TWO_GB == pageSizes[i]) && (0 != (OMRPORT_VMEM_PAGE_FLAG_NOT_USED & pageFlags[i]))) {
      sixteenGBPageSize = TRUE;
    }
#endif /* defined(OMR_ENV_DATA64) */
    if ((16 * ONE_MB == pageSizes[i]) && (0 != (OMRPORT_VMEM_PAGE_FLAG_NOT_USED & pageFlags[i]))) {
      sixteenMBPageSize = TRUE;
    }

    if ((16 * FOUR_KB == pageSizes[i]) && (0 != (OMRPORT_VMEM_PAGE_FLAG_NOT_USED & pageFlags[i]))) {
  ...
  /* First get the page size of the data segment. This is the page size used by the JIT code cache if 16M Code Pages are
   * not being used */
#if defined(AIXPPC)
#define SAMPLE_BLOCK_SIZE 4
  ...
  if (NULL == address) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "Failed to allocate block of memory to determine page size of data segment");
    goto _exit;
  }
...
static int omrvmem_testFindValidPageSize_impl(struct OMRPortLibrary* portLibrary, const char* testName)
{
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
#if defined(OMR_ENV_DATA64)
    if ((TWO_GB == pageSizes[i]) && (0 != (OMRPORT_VMEM_PAGE_FLAG_FIXED & pageFlags[i]))) {
      twoGBFixed = TRUE;
    }

    if ((ONE_MB == pageSizes[i]) && (0 != (OMRPORT_VMEM_PAGE_FLAG_FIXED & pageFlags[i]))) {
      oneMBFixed = TRUE;
    }
#endif /* defined(OMR_ENV_DATA64) */
    if ((ONE_MB == pageSizes[i]) && (0 != (OMRPORT_VMEM_PAGE_FLAG_PAGEABLE & pageFlags[i]))) {
      oneMBPageable = TRUE;
    }
  }
...
uintptr_t getSizeFromString(struct OMRPortLibrary* portLibrary, char* sizeString)
{
 ...
 */
int32_t omrvmem_disclaimPerfTests(struct OMRPortLibrary* portLibrary, char* disclaimPerfTestArg, BOOLEAN shouldDisclaim)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  int rc = 0;
   ...
   * disclaimPerfTestArg is the string after the ':' from
   * -disclaimPerfTest:<pageSize>,<byteAmount>,<totalMemory>[,<numIterations>] while shouldDisclaim is set from the +/-
   * prefix, numIterations is optional (default is 50 iterations) Example options: -disclaimPerfTest:4K,256M,4G
  ...
  if (TEST_PASS == rc) {
    portTestEnv->log("\npageSize: 0x%zx, byteAmount 0x%zx, memTotal 0x%zx, numIterations %i, disclaim %i\n", pageSize,
      byteAmount, memTotal, numIterations, shouldDisclaim);
 ...
 */
J9PortVmemParams* getParams(struct OMRPortLibrary* portLibrary, J9PortVmemParams* vmemParams)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
TEST(PortVmemTest, vmem_testOverlappingSegments)
{
  ...
  vmemParams
    = (J9PortVmemParams*)omrmem_allocate_memory(sizeof(J9PortVmemParams) * CYCLES, OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  for (i = 0; i < CYCLES; i++) {
    ...
    portTestEnv->log("%.16d  %.16X  %.16X  %.16X  %d\n", i, (uintptr_t)vmemID[i].address,
      (uintptr_t)vmemID[i].address + vmemID[i].size, vmemID[i].size, keepCycles[i]);
    ...
    for (j = 0; j < i; j++) {
      if (keepCycles[j] >= i) { /* If not already freed */
        if (checkForOverlap(&vmemID[i], &vmemID[j])) {
          ...
          portTestEnv->log("%.16d  %.16X  %.16X  %.16X  %d\n", j, (uintptr_t)vmemID[j].address,
            (uintptr_t)vmemID[j].address + vmemID[j].size, vmemID[j].size, keepCycles[j]);
          keepCycles[i]
            = i - 1; /* Pretend this segment is already freed to avoid double freeing overlapping segments */
          result = -1;
          goto exit;
        }
      }
