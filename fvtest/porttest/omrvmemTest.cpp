...
{
    if ((FOUR_KB == pageSize) || ((ONE_MB == pageSize) && (OMRPORT_VMEM_PAGE_FLAG_FIXED == (OMRPORT_VMEM_PAGE_FLAG_FIXED & pageFlags)))) {
        return FALSE;
    } else {
        return TRUE;
    }
...
{
    if ((FOUR_KB == pageSize) || ((ONE_MB == pageSize) && (OMRPORT_VMEM_PAGE_FLAG_PAGEABLE == (OMRPORT_VMEM_PAGE_FLAG_PAGEABLE & pageFlags)))) {
        return TRUE;
    } else {
        return FALSE;
    }
 ...
 */
int omrvmem_bench_write_and_decommit_memory(struct OMRPortLibrary* portLibrary, uintptr_t pageSize, uintptr_t byteAmount, BOOLEAN disclaim, uintptr_t numIterations)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int omrvmem_bench_force_overcommit_then_decommit(struct OMRPortLibrary* portLibrary, uintptr_t physicalMemorySize, uintptr_t pageSize, BOOLEAN disclaim)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int omrvmem_bench_reserve_write_decommit_and_free_memory(struct OMRPortLibrary* portLibrary, uintptr_t pageSize, uintptr_t byteAmount, BOOLEAN disclaim, uintptr_t numIterations)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int omrvmem_exhaust_virtual_memory(struct OMRPortLibrary* portLibrary, uintptr_t pageSize, uintptr_t byteAmount, BOOLEAN disclaim)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
#define NUM_SEGMENTS 3
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        for (j = 0; j < NUM_SEGMENTS; j++) {
            ...
            } else if (use2To32G && strictAddress && (((uint64_t)memPtr[j] < TWO_GB) || ((uint64_t)memPtr[j] >= SIXTY_FOUR_GB))) {
...
TEST(PortVmemTest, vmem_test_reserveExecutableMemory)
{
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        omrvmem_vmem_params_init(&params);
        params.byteAmount = pageSizes[i];
        params.mode |= OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE | OMRPORT_VMEM_MEMORY_MODE_EXECUTE | OMRPORT_VMEM_MEMORY_MODE_COMMIT;
        params.options |= OMRPORT_VMEM_STRICT_PAGE_SIZE;
        params.pageSize = pageSizes[i];
        params.pageFlags = pageFlags[i];
