...
static int
omrvmem_testReserveMemoryEx_impl(struct OMRPortLibrary*,
                                 const char*,
                                 BOOLEAN,
                                 BOOLEAN,
                                 BOOLEAN,
                                 ...
                                 BOOLEAN);
static int
omrvmem_testReserveMemoryEx_StandardAndQuickMode(struct OMRPortLibrary*,
                                                 const char*,
                                                 BOOLEAN,
                                                 BOOLEAN,
                                                 BOOLEAN,
                                                 BOOLEAN);
static BOOLEAN
memoryIsAvailable(struct OMRPortLibrary*, BOOLEAN);
 ...
 */
static void
verifyMemory(struct OMRPortLibrary* portLibrary,
             const char* testName,
             char* memPtr,
             ...
             const char* allocName)
{
  ...
  if (testCharA != memPtr[0]) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "%s failed write test at 0\n", allocName);
  }
  ...
  if (0 != memcmp(stackMemory, memPtr, testSize)) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "%s failed to retain value at start\n", allocName);
  }
  ...
  if (0 != memcmp(stackMemory, memPtr + byteAmount - testSize, testSize)) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "%s failed to retain value at end\n", allocName);
  }
}
...
TEST(PortVmemTest, vmem_test1)
{
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
...
#endif /* J9ZOS390 */
    memPtr = (char*)omrvmem_reserve_memory(0,
                                           pageSizes[i],
                                           &vmemID,
    ...
    omrstr_printf(
      allocName, allocNameSize, "omrvmem_reserve_memory(%d)", pageSizes[i]);
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (pageSizes[i] < sizeof(struct J9PortVmemIdentifier)) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         ...
                         pageSizes[i],
                         sizeof(struct J9PortVmemIdentifier));
      goto exit;
    }
 ...
 */
static void
verifyContiguousMem(struct OMRPortLibrary* portLibrary,
                    const char* testName,
                    size_t pagesize,
                    size_t arrayletSize,
                    void* contiguous,
  ...
  for (i = 0; i < ARRAYLET_COUNT; i++) {
    ...
    for (; j < arrayletSize; j++) {
      if (address[j] == vals[i] && address[j] == arrayletData[j]) {
        ...
        outputErrorMessage(
          PORTTEST_ERROR_ARGS, "%s failed address verification.\n", allocName);
        return;
      }
    }
  }
...
TEST(PortVmemTest, vmem_test_double_mapping)
{
  ...
  /* reserve and commit memory for heap size */
  {
    ...
    /* can we read and write to the memory? */
    omrstr_printf(
      allocName, allocNameSize, "omrvmem_reserve_memory(%d)", HEAP_SIZE);
    ...
    /* Memory verified successfully */
    {
      ...
      for (i = 0; i < ARRAYLET_COUNT; i++) {
        memset(
          arrayletLeaveAddrs[i], vals[i % ARRAYLET_COUNT], arrayletLeafSize);
      }
      ...
      void* contiguous = omrvmem_get_contiguous_region_memory(
        ...
        ARRAYLET_COUNT,
        arrayletLeafSize,
        (ARRAYLET_COUNT * arrayletLeafSize),
        &vmemID,
        &newIdentifier,
        OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE |
          OMRPORT_VMEM_MEMORY_MODE_COMMIT,
      ...
      if (contiguous == NULL) {
        ...
        outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "unable to reserve and double map 0x%zx bytes with page size 0x%zx.\n"
          "\tlastErrorNumber=%d, lastErrorMessage=%s\n",
          HEAP_SIZE,
          pageSize,
          lastErrorNumber,
        ...
        verifyContiguousMem(OMRPORTLIB,
                            testName,
                            pageSize,
                            arrayletLeafSize,
                            contiguous,
 ...
 */
int
omrvmem_bench_write_and_decommit_memory(struct OMRPortLibrary* portLibrary,
                                        uintptr_t pageSize,
                                        uintptr_t byteAmount,
                                        BOOLEAN disclaim,
 ...
 */
int
omrvmem_bench_force_overcommit_then_decommit(struct OMRPortLibrary* portLibrary,
 ...
 */
int
omrvmem_bench_reserve_write_decommit_and_free_memory(
 ...
 */
int
omrvmem_exhaust_virtual_memory(struct OMRPortLibrary* portLibrary,
                               uintptr_t pageSize,
                               uintptr_t byteAmount,
                               BOOLEAN disclaim)
{
...
TEST(PortVmemTest, vmem_testReserveMemoryExTopDown)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB,
      "omrvmem_testReserveMemoryExTopDown",
      TRUE,
      FALSE,
      FALSE,
      FALSE);
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictAdress)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, TRUE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB,
      "omrvmem_testReserveMemoryExStrictAddress",
      FALSE,
      TRUE,
      FALSE,
      FALSE);
...
TEST(PortVmemTest, vmem_testReserveMemoryEx_use2To32)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB,
      "omrvmem_testReserveMemoryEx_use2To32",
      FALSE,
      FALSE,
      FALSE,
      TRUE);
...
TEST(PortVmemTest, vmem_testReserveMemoryExTopDown_use2To32)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB,
      "omrvmem_testReserveMemoryExTopDown_use2To32",
      TRUE,
      FALSE,
      FALSE,
      TRUE);
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictAddress_use2To32)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, TRUE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB,
      "omrvmem_testReserveMemoryExStrictAddress_use2To32",
      FALSE,
      TRUE,
      FALSE,
      TRUE);
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictPageSize_use2To32)
{
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
      OMRPORTLIB,
      "omrvmem_testReserveMemoryExStrictPageSize_use2To32",
      FALSE,
      FALSE,
      TRUE,
      TRUE);
...
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  int rc = omrvmem_testReserveMemoryEx_impl(OMRPORTLIB,
                                            testName,
                                            topDown,
                                            FALSE,
                                            strictAddress,
  ...
  if (TEST_PASS == rc) {
    rc |= omrvmem_testReserveMemoryEx_impl(OMRPORTLIB,
                                           testName,
                                           topDown,
                                           TRUE,
                                           strictAddress,
...
#define NUM_SEGMENTS 3
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    for (j = 0; j < NUM_SEGMENTS; j++) {
      ...
      /* Sample baseline category data */
      getPortLibraryMemoryCategoryData(
        OMRPORTLIB, &initialBlocks, &initialBytes);
      ...
      portTestEnv->log(
        ...
        "use2To32G: %s\n",
        params[j].pageSize,
        params[j].startAddress,
        params[j].endAddress,
        (0 != (params[j].options & OMRPORT_VMEM_ALLOC_DIR_TOP_DOWN)) ? "true"
                                                                     : "false",
        ...
        (0 != (params[j].options & OMRPORT_VMEM_STRICT_PAGE_SIZE)) ? "true"
                                                                   : "false",
        (0 != (params[j].options & OMRPORT_VMEM_ZOS_USE2TO32G_AREA)) ? "true"
                                                                     : "false");
      ...
      if (NULL != memPtr[j]) {
        ...
        if (vmemID[j].pageFlags != omrvmem_get_page_flags(&(vmemID[j]))) {
          outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "vmemID[j].pageFlags (0x%zx)!= omrvmem_get_page_flags (0x%zx) .\n",
            vmemID[j].pageFlags,
...
TEST(PortVmemTest, vmem_testReserveMemoryEx_zOSLargePageBelowBar)
{
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    /* can we read and write to the memory? */
    omrstr_printf(allocName,
                  allocNameSize,
                  "omrvmem_reserve_memory(%d)",
                  params.byteAmount);
    verifyMemory(
      OMRPORTLIB, testName, (char*)memPtr, params.byteAmount, allocName);
      ...
      if (NULL != memPtr) {
        ...
        if (pageable) {
          ...
          if ((0 == rc) && strict) {
            outputErrorMessage(
              PORTTEST_ERROR_ARGS,
              "omrvmem_decommit_memory no error when trying to decommit fixed "
              "pages size=0x%zx flags=0x%zx\n",
              params.pageSize,
              params.pageFlags);
          }
        }
...
#endif /* J9ZOS390 */
      memPtr = (char*)omrvmem_reserve_memory(NULL,
                                             reserveSizeInBytes,
                                             &vmemID,
                                             OMRPORT_VMEM_MEMORY_MODE_READ |
                                               OMRPORT_VMEM_MEMORY_MODE_WRITE,
      ...
      if (memPtr != NULL) {
        ...
        for (nodeIndex = 0; nodeIndex < totalNumaNodeCount; nodeIndex++) {
          if (policyForMemoryNode == nodeArray[nodeIndex].memoryPolicy) {
            ...
            rc = omrvmem_numa_set_affinity(
              nodeNumber, numaBlock, pageSize, &vmemID);
            if (rc != 0) {
              outputErrorMessage(
                PORTTEST_ERROR_ARGS,
        ...
        /* can we read and write to the memory? */
        omrstr_printf(
          allocName, allocNameSize, "omrvmem_reserve_memory(%d)", pageSize);
        verifyMemory(
          OMRPORTLIB, testName, memPtr, reserveSizeInBytes, allocName);
...
#define MODE_NOT_USED 0
  ...
  omrvmem_default_large_page_size_ex(
    0, &defaultLargePageSize, &defaultLargePageFlags);
  ...
  PRINT_FIND_VALID_PAGE_SIZE_INPUT(
    MODE_NOT_USED, requestedPageSize, requestedPageFlags);

  omrvmem_find_valid_page_size(
    MODE_NOT_USED, &requestedPageSize, &requestedPageFlags, &isSizeSupported);
  ...
  verifyFindValidPageSizeOutput(OMRPORTLIB,
                                testName,
                                expectedPageSize,
                                expectedPageFlags,
...
#define OMRPORT_VMEM_PAGESIZE_COUNT                                            \
  ...
  memcpy(old_vmem_pageSize,
         pageSizes,
         OMRPORT_VMEM_PAGESIZE_COUNT * sizeof(uintptr_t));
  memcpy(old_vmem_pageFlags,
         pageFlags,
         OMRPORT_VMEM_PAGESIZE_COUNT * sizeof(uintptr_t));
...
#define SAMPLE_BLOCK_SIZE 4
  ...
  omrvmem_find_valid_page_size(
    mode, &requestedPageSize, &requestedPageFlags, &isSizeSupported);
  ...
  rc = setenv("TR_ppcCodeCacheConsolidationEnabled",
              "true",
              1 /*overwrite any existing value */);
  if (-1 == rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "Failed to set environment variable TR_ppcCodeCacheConsolidationEnabled");
    goto _exit;
  }
  mode = OMRPORT_VMEM_MEMORY_MODE_EXECUTE;
  requestedPageSize = 16 * ONE_MB;
                                   ...
                                   const char* testName)
{
  ...
  if (0 == defaultLargePageSize) {
    ...
    requestedPageFlags = OMRPORT_VMEM_PAGE_FLAG_NOT_USED;
    PRINT_FIND_VALID_PAGE_SIZE_INPUT(
      mode, requestedPageSize, requestedPageFlags);
 ...
 */
int32_t
omrvmem_disclaimPerfTests(struct OMRPortLibrary* portLibrary,
                          ...
                          BOOLEAN shouldDisclaim)
{
  ...
  if (TEST_PASS == rc) {
    ...
    rc |= omrvmem_bench_reserve_write_decommit_and_free_memory(
      OMRPORTLIB, pageSize, byteAmount, shouldDisclaim, numIterations);
#if defined(OMR_ENV_DATA64)
    rc |= omrvmem_exhaust_virtual_memory(
      OMRPORTLIB, pageSize, byteAmount, shouldDisclaim);
#endif
...
TEST(PortVmemTest, vmem_testOverlappingSegments)
{
  ...
  for (i = 0; i < CYCLES; i++) {
    ...
    if (NULL == memPtr) {
      portTestEnv->log(
        LEVEL_ERROR, "Failed to get memory. Error: %s.\n", strerror(errno));
      portTestEnv->log(
        LEVEL_ERROR,
    ...
    for (j = 0; j < i; j++) {
      if (keepCycles[j] == i) {
        int32_t rc = omrvmem_free_memory(
          vmemID[j].address, vmemParams[j].byteAmount, &vmemID[j]);
        if (0 != rc) {
...
TEST(PortVmemTest, vmem_testGetProcessMemorySize)
{
  ...
  size = 0;
  result =
    omrvmem_get_process_memory_size(OMRPORT_VMEM_PROCESS_EnsureWideEnum, &size);
