 ...
 *
 * Exercise the API for port library virtual memory management operations. These
 * functions can be found in the file @ref omrvmem.c
 *
 * @note port library virtual memory management operations are not optional in
 * the port library table.
 *
 */
#include <errno.h>
#include <stdlib.h>
...
static int omrvmem_testReserveMemoryEx_impl(struct OMRPortLibrary *,
                                            const char *, BOOLEAN, BOOLEAN,
                                            BOOLEAN, BOOLEAN, BOOLEAN);
static int omrvmem_testReserveMemoryEx_StandardAndQuickMode(
    struct OMRPortLibrary *, const char *, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN);
 ...
 */
static void verifyMemory(struct OMRPortLibrary *portLibrary,
                         const char *testName, char *memPtr,
                         uintptr_t byteAmount, const char *allocName) {
  ...
  if (0 != memcmp(stackMemory, memPtr, testSize)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s failed to retain value at start\n", allocName);
  }
  ...
  if (0 != memcmp(stackMemory, memPtr + byteAmount - testSize, testSize)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s failed to retain value at end\n", allocName);
  }
}
...
TEST(PortVmemTest, vmem_verify_functiontable_slots) {
  ...
  if (NULL == OMRPORTLIB->vmem_startup) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->vmem_startup is NULL\n");
  }

  /*  Not tested, implementation dependent.  No known functionality */
  if (NULL == OMRPORTLIB->vmem_shutdown) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->vmem_shutdown is NULL\n");
  }

  /* omrmem_test2 */
  if (NULL == OMRPORTLIB->vmem_reserve_memory) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->vmem_reserve_memory is NULL\n");
  }

  /* omrmem_test1 */
  if (NULL == OMRPORTLIB->vmem_commit_memory) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->vmem_commit_memory is NULL\n");
  }

  /* omrmem_test4 */
  if (NULL == OMRPORTLIB->vmem_decommit_memory) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->vmem_decommit_memory, is NULL\n");
  }

  if (NULL == OMRPORTLIB->vmem_free_memory) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->vmem_free_memory, is NULL\n");
  }

  if (NULL == OMRPORTLIB->vmem_supported_page_sizes) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->vmem_supported_page_sizes, is NULL\n");
  }
 ...
 * Verify port library memory management.
 *
 * Make sure there is at least one page size returned by
 * omrvmem_get_supported_page_sizes
 */
TEST(PortVmemTest, vmem_test_verify_there_are_page_sizes) {
  ...
  if (pageSizes[0] == 0) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "There aren't any supported page sizes on this platform \n");
    goto exit;
  }
...
#if defined(J9ZOS390)
/**
 * Returns TRUE if the given page size and flags corresponds to new page sizes
 * added in z/OS. z/OS traditionally supports 4K and 1M fixed pages. 1M pageable
 * and 2G fixed, added recently, are considered new page sizes.
 */
BOOLEAN
isNewPageSize(uintptr_t pageSize, uintptr_t pageFlags) {
  if ((FOUR_KB == pageSize) ||
      ((ONE_MB == pageSize) && (OMRPORT_VMEM_PAGE_FLAG_FIXED ==
                                (OMRPORT_VMEM_PAGE_FLAG_FIXED & pageFlags)))) {
    return FALSE;
  } else {
    return TRUE;
  }
...
/**
 * Returns TRUE if the given page size and flags can be used to allocate memory
 * below 2G bar, FALSE otherwise. On z/OS, 1M fixed and 2G fixed pages can be
 * used only to allocate above 2G bar. Only 4K and 1M pageable pages can be used
 * to allocate memory below the bar.
 */
isPageSizeSupportedBelowBar(uintptr_t pageSize, uintptr_t pageFlags) {
  if ((FOUR_KB == pageSize) ||
      ((ONE_MB == pageSize) &&
       (OMRPORT_VMEM_PAGE_FLAG_PAGEABLE ==
        (OMRPORT_VMEM_PAGE_FLAG_PAGEABLE & pageFlags)))) {
    return TRUE;
  } else {
    return FALSE;
  }
...
/**
 * Get all the page sizes and make sure we can allocate a memory chunk for each
 * page size.
 *
...
TEST(PortVmemTest, vmem_test1) {
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    /* reserve and commit */
#if defined(J9ZOS390)
    /* On z/OS skip this test for newly added large pages as obsolete
     * omrvmem_reserve_memory() does not support them */
    ...
    if (memPtr == NULL) {
      ...
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "unable to reserve and commit 0x%zx bytes with page size 0x%zx.\n"
      ...
      if (OMRPORT_ERROR_VMEM_INSUFFICENT_RESOURCES == lastErrorNumber) {
        portTestEnv->log(
            LEVEL_ERROR,
            "Portable error OMRPORT_ERROR_VMEM_INSUFFICENT_RESOURCES...\n");
        portTestEnv->changeIndent(1);
        portTestEnv->log(
            LEVEL_ERROR,
            "REBOOT THE MACHINE to free up resources AND TRY THE TEST AGAIN\n");
        portTestEnv->changeIndent(-1);
      }
      goto exit;
    } else {
      ...
      portTestEnv->log("reserved and committed 0x%zx bytes with page size "
                       "0x%zx at address 0x%zx\n",
      ...
      if (finalBlocks <= initialBlocks) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "vmem reserve didn't increment category block as expected. Final "
            "blocks=%zu, initial blocks=%zu, page size=%zu.\n",
      ...
      if (finalBytes < (initialBytes + pageSizes[i])) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "vmem reserve didn't increment category bytes as expected. Initial "
            "bytes=%zu, final bytes=%zu, page size=%zu.\n",
            finalBytes, initialBytes, pageSizes[i]);
      }
    }

    /* can we read and write to the memory? */
    omrstr_printf(allocName, allocNameSize, "omrvmem_reserve_memory(%d)",
                  pageSizes[i]);
    ...
    if (rc != 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrvmem_free_memory returned %i when trying to free "
                         "0x%zx bytes at 0x%zx\n",
...

    {
      ...
      if (finalBlocks != initialBlocks) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "vmem free didn't decrement category block as expected. Final "
            "blocks=%zu, initial blocks=%zu, page size=%zu.\n",
            finalBlocks, initialBlocks, pageSizes[i]);
      }
      ...
      if (finalBytes != initialBytes) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "vmem free didn't decrement category bytes as expected. Initial "
            "bytes=%zu, final bytes=%zu, page size=%zu.\n",
...
/**
 * Get all the page sizes and make sure we can allocate a memory chunk for each
 * page size. In OpenJ9, memory for J9JNIRedirectionBlock is allocated using
 * j9vmem_reserve_memory(). After allocating the memory they copy the
 * J9PortVmemIdentifier into the reserved memory. When j9vmem_free_memory() is
 * called, we free the memory and then attempt to read flags from
 * J9PortVmemIdentifier, resulting in a use-after-free bug.
 *
 * Checks that each allocation manipulates the memory categories appropriately.
 */
...
TEST(PortVmemTest, vmem_test_free_memory) {
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (pageSizes[i] < sizeof(struct J9PortVmemIdentifier)) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrvmem_free_memory pageSize is smaller than "
                         "sizeof(J9PortVmemIdentifier). pageSize: 0x%zx "
                         "sizeof(J9PortVmemIdentifier): 0x%zx\n",
                         pageSizes[i], sizeof(struct J9PortVmemIdentifier));
      goto exit;
    }

    J9PortVmemIdentifier *vmemID2 =
        (J9PortVmemIdentifier *)omrvmem_reserve_memory(
            0, pageSizes[i], &vmemID,
            OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE |
                OMRPORT_VMEM_MEMORY_MODE_COMMIT,
            pageSizes[i], OMRMEM_CATEGORY_PORT_LIBRARY);
    ...
    if (vmemID2 == NULL) {
      lastErrorMessage = (char *)omrerror_last_error_message();
      lastErrorNumber = omrerror_last_error_number();
      /* Succeed instead of error then continue since the platform is not
       * supported */
...
/**
 * Get all the page sizes and make sure we can allocate a memory chunk for each
 * page size. This particular test is testing the flag
 * OMRPORT_VMEM_MEMORY_MODE_SHARE_FILE_OPEN where it forces
 * default_pageSize_reserve_memory to mmap using a shared file handle.
 *
 * Checks that each allocation manipulates the memory categories appropriately.
 */
TEST(PortVmemTest, vmem_test_shared_file_handle) {
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    memPtr = (char *)omrvmem_reserve_memory(
        ...
        OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE |
            OMRPORT_VMEM_MEMORY_MODE_COMMIT |
            OMRPORT_VMEM_MEMORY_MODE_SHARE_FILE_OPEN,
        pageSizes[i], OMRMEM_CATEGORY_PORT_LIBRARY);
 ...
 * @param[in] allocName Calling function name to display in errors
 */
static void verifyContiguousMem(struct OMRPortLibrary *portLibrary,
                                const char *testName, size_t pagesize,
                                size_t arrayletSize, void *contiguous,
                                void *addresses[], char *vals,
  ...
  /* Verify if arraylet leaves and contiguous block of memory contain expected
   * data */
  for (i = 0; i < ARRAYLET_COUNT; i++) {
    ...
    for (; j < arrayletSize; j++) {
      if (address[j] == vals[i] && address[j] == arrayletData[j]) {
      } /* Good */
      else {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "%s failed address verification.\n", allocName);
        return;
      }
    }
  }
  ...
  for (i = 0; i < ARRAYLET_COUNT; i++) {
    ...
    /* write a pattern to the first page of each arraylet to verify proper
     * mappings */
    memset(arrayletData, '*', firstBytes);
    char *arrayletData2 = arrayletData + jump;
    memset(arrayletData2, '*', secondBytes);

    /* Write to the first byte of each of the other pages in the arraylet to
     * ensure all pages are touched */
  ...
  for (i = 0; i < ARRAYLET_COUNT; i++) {
    ...
    size_t j = 0;
    for (; j < firstBytes; j++) {
      if (arrayletData[j] == '*' && arrayletData[j] == address[j]) {
      }      /* Good */
      else { /* Verification failed. Fail double map. */
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "%s failed double mapping test at 0\n", allocName);
        return;
      }
    }
    char *arrayletData2 = arrayletData + jump;
    ...
    for (j = 0; j < secondBytes; j++) {
      if (arrayletData2[j] == '*' && arrayletData2[j] == address2[j]) {
      }      /* Good */
      else { /* Verification failed. Fail double map. */
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "%s failed double mapping test at 1\n", allocName);
        return;
      }
    }
    /* Verify first byte of each of the other pages in the arraylet to ensure
     * all pages were modified in the heap */
    for (j = 1; j < (arrayletSize / pagesize); j++) {
      char *pageData = arrayletData + (j * pagesize);
      char *addressData = address + (j * pagesize);
      if (*pageData == '*' && *pageData == *addressData) {
      }      /* Good */
      else { /* Verification failed. Fail double map. */
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "%s failed double mapping test at 2\n", allocName);
        return;
      }
    }
  }
...
TEST(PortVmemTest, vmem_test_double_mapping) {
  ...
  if (arrayletLeafSize / pageSize == 0 && (arrayletLeafSize < pageSize)) {
    /* Leaf size limit is 2 MB due to heap size and number of leaves. If
     * pageSize is too big for test skip */
  ...
  /* reserve and commit memory for heap size */
  {
    ...
    /* Memory verified successfully */
    {
      ...
      /* Must be multiple of pagesize: sysconf(_SC_PAGE_SIZE) 	Simulates the
       * order of arraylet leaves in an array */
      ...
      arrayLetOffsets[5] =
          (long)((HEAP_SIZE / 2) + (HEAP_SIZE / 4) + (HEAP_SIZE / 8)); /* 8 */
      ...
      for (i = 0; i < ARRAYLET_COUNT; i++) {
        memset(arrayletLeaveAddrs[i], vals[i % ARRAYLET_COUNT],
               arrayletLeafSize);
      }
      /* Arraylet initialization complete */

      OMRMemCategory *category =
          omrmem_get_category(OMRMEM_CATEGORY_PORT_LIBRARY);

      /* Now create contiguous block of memory and then double map arraylet
       * leaves. */
      void *contiguous = omrvmem_get_contiguous_region_memory(
          arrayletLeaveAddrs, ARRAYLET_COUNT, arrayletLeafSize,
          (ARRAYLET_COUNT * arrayletLeafSize), &vmemID, &newIdentifier,
          OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE |
              OMRPORT_VMEM_MEMORY_MODE_COMMIT,
          pageSize, category);
      if (contiguous == NULL) {
        ...
        if (OMRPORT_ERROR_VMEM_NOT_SUPPORTED == lastErrorNumber) {
          portTestEnv->log(LEVEL_ERROR,
                           "Double mapping not supported. Skipping test...\n");
          goto exit;
        }
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "unable to reserve and double map 0x%zx bytes with "
                           "page size 0x%zx.\n"
                           "\tlastErrorNumber=%d, lastErrorMessage=%s\n",
                           HEAP_SIZE, pageSize, lastErrorNumber,
                           lastErrorMessage);
        ...
        if (OMRPORT_ERROR_VMEM_INSUFFICENT_RESOURCES == lastErrorNumber) {
          portTestEnv->log(
              ...
              "Portable error OMRPORT_ERROR_VMEM_INSUFFICENT_RESOURCES...\n");
          portTestEnv->changeIndent(1);
          portTestEnv->log(LEVEL_ERROR, "REBOOT THE MACHINE to free up "
                                        "resources AND TRY THE TEST AGAIN\n");
          portTestEnv->changeIndent(-1);
        }
        goto exit;
      } else {
        ...
        verifyContiguousMem(OMRPORTLIB, testName, pageSize, arrayletLeafSize,
                            contiguous, arrayletLeaveAddrs, vals, allocName);

        /* Free contiguous block of memory. */
        uintptr_t byteAmount = ARRAYLET_COUNT * arrayletLeafSize;
        int32_t rc_contiguous =
            omrvmem_free_memory(contiguous, byteAmount, &newIdentifier);
        if (rc_contiguous != 0) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "omrvmem_free_memory returned %i when trying to "
                             "free 0x%zx bytes at 0x%zx\n",
...
static void setDisclaimVirtualMemory(struct OMRPortLibrary *portLibrary,
                                     BOOLEAN disclaim) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  if (disclaim) {
 ...
 * @param[in] byteAmount This value is the amount of memory in bytes to use for
 * each commit/decommit within the loop
 * @param[in] numIterations The number of times to iterate through the loop
 *
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int omrvmem_bench_write_and_decommit_memory(struct OMRPortLibrary *portLibrary,
                                            uintptr_t pageSize,
                                            uintptr_t byteAmount,
                                            BOOLEAN disclaim,
                                            uintptr_t numIterations) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  if (memPtr == NULL) {
    ...
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "unable to reserve 0x%zx bytes with page size 0x%zx.\n"
                       "\tlastErrorNumber=%d, lastErrorMessage=%s\n",
  ...
  for (i = 0; numIterations != i; i++) {
    ...
    if (NULL == memPtr) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrvmem_commit_memory returned NULL when trying to "
                         "commit 0x%zx bytes backed by 0x%zx-byte pages\n",
                         byteAmount, pageSize);
      goto exit;
    }
    ...
    if (rc != 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrvmem_decommit_memory returned 0x%zx when trying "
                         "to decommit 0x%zx bytes backed by 0x%zx-byte pages\n",
  ...
  portTestEnv->log("%s pageSize 0x%zx, byteAmount 0x%zx, millis for %u "
                   "iterations: [write_to_all/decommit] test: %lli\n",
                   disclaim ? "disclaim" : "nodisclaim", pageSize, byteAmount,
                   numIterations, deltaMillis);
 ...
 *
 * @param[in] portLibrary The port library under test
 * @param[in] physicalMemorySize The size of physical memory on the machine
 * (e.g. as read from /proc/meminfo)
 * @param[in] pageSize The page size to use for this test, e.g. 4096
 * @param[in] disclaim When true, the OS is told it may disclaim the memory
 *
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int omrvmem_bench_force_overcommit_then_decommit(
    struct OMRPortLibrary *portLibrary, uintptr_t physicalMemorySize,
    uintptr_t pageSize, BOOLEAN disclaim) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  if (byteAmount < D512M) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "byteAmount 0x%zx bytes must be greater than 0x%zx\n",
                       byteAmount, D512M);
    goto exit;
  }
  ...
  omrvmem_vmem_params_init(&params);
  params.byteAmount = byteAmount;
  params.mode |= OMRPORT_VMEM_MEMORY_MODE_READ |
  ...
  /* in order to write to region being decommitted, the system would have to go
   * to disk, as the previous memset would have forced the region to have been
   * paged out
   */
  rc = omrvmem_decommit_memory(memPtr, D512M, &vmemID);
  if (rc != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrvmem_decommit_memory returned 0x%zx when trying to "
                       "decommit 0x%zx bytes backed by 0x%zx-byte pages\n",
                       rc, byteAmount, pageSize);
    goto exit;
  }
  ...
  deltaMillis = (omrtime_nano_time() - startTimeNanos) / (1000 * 1000);
  portTestEnv->log(
      "%s pageSize 0x%zx, byteAmount 0x%zx, millis: [force page, decommit "
      "pagedout region, write decommitted region] test: %lli\n",
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int omrvmem_bench_reserve_write_decommit_and_free_memory(
    struct OMRPortLibrary *portLibrary, uintptr_t pageSize,
    uintptr_t byteAmount, BOOLEAN disclaim, uintptr_t numIterations) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  for (i = 0; numIterations != i; i++) {
    ...
    if (memPtr == NULL) {
      ...
      lastErrorNumber = omrerror_last_error_number();
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "In iteration %d, unable to reserve and commit 0x%zx "
                         "bytes with page size 0x%zx.\n"
                         "\tlastErrorNumber=%d, lastErrorMessage=%s\n",
                         i, byteAmount, pageSize, lastErrorNumber,
                         lastErrorMessage);
      if (OMRPORT_ERROR_VMEM_INSUFFICENT_RESOURCES == lastErrorNumber) {
        portTestEnv->log(
            LEVEL_ERROR,
  ...
  portTestEnv->log(
      "%s pageSize 0x%zx, byteAmount 0x%zx, millis for %u iterations: "
      "[reserve/write_to_all/decommit/free] test: %lli\n",
      disclaim ? "disclaim" : "nodisclaim", pageSize, byteAmount, numIterations,
      deltaMillis);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int omrvmem_exhaust_virtual_memory(struct OMRPortLibrary *portLibrary,
                                   uintptr_t pageSize, uintptr_t byteAmount,
                                   BOOLEAN disclaim) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
#define NUM_VMEM_IDS 1000
  ...
  for (i = 0; i < NUM_VMEM_IDS; i++) {
    ...
    params[i].mode |= OMRPORT_VMEM_MEMORY_MODE_READ |
                      OMRPORT_VMEM_MEMORY_MODE_WRITE |
                      OMRPORT_VMEM_MEMORY_MODE_COMMIT;
    ...
    memPtr = memPointers[i] =
        (char *)omrvmem_reserve_memory_ex(&vmemID[i], &params[i]);
  ...
  portTestEnv->log("%s pageSize 0x%zx, byteAmount 0x%zx, millis for %u "
                   "iterations: [reserve/write_to_all/decommit] test: %lli\n",
                   disclaim ? "disclaim" : "nodisclaim", pageSize, byteAmount,
                   NUM_VMEM_IDS, deltaMillis);
 ...
 * page size
 *
 * After reserving and committing the memory, decommit the memory before freeing
 * it.
 */
TEST(PortVmemTest, vmem_decommit_memory_test) {
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (memPtr == NULL) {
      ...
      portTestEnv->log(
          "reserved and committed 0x%zx bytes with page size 0x%zx\n",
...
TEST(PortVmemTest, vmem_testReserveMemoryEx) {
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
        OMRPORTLIB, "omrvmem_testReserveMemoryEx", FALSE, FALSE, FALSE, FALSE);
  } else {
    portTestEnv->log(LEVEL_ERROR,
                     "***Did not find enough memory available on system, not "
                     "running test omrvmem_testReserveMemoryEx\n");
  }
}
...
TEST(PortVmemTest, vmem_testReserveMemoryExTopDown) {
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
        OMRPORTLIB, "omrvmem_testReserveMemoryExTopDown", TRUE, FALSE, FALSE,
        FALSE);
  } else {
    portTestEnv->log(LEVEL_ERROR,
                     "***Did not find enough memory available on system, not "
                     "running test omrvmem_testReserveMemoryExTopDown\n");
  }
}
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictAdress) {
  ...
  if (memoryIsAvailable(OMRPORTLIB, TRUE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
        OMRPORTLIB, "omrvmem_testReserveMemoryExStrictAddress", FALSE, TRUE,
        FALSE, FALSE);
  } else {
    portTestEnv->log(LEVEL_ERROR,
                     "***Did not find enough memory available on system, not "
                     "running test omrvmem_testReserveMemoryExStrictAddress\n");
  }
}
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictPageSize) {
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
        OMRPORTLIB, "omrvmem_testReserveMemoryExStrictPageSize", FALSE, FALSE,
        TRUE, FALSE);
  } else {
    portTestEnv->log(
        LEVEL_ERROR,
        "***Did not find enough memory available on system, not running test "
        "omrvmem_testReserveMemoryExStrictPageSize\n");
  }
}
...
TEST(PortVmemTest, vmem_testReserveMemoryEx_use2To32) {
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
        OMRPORTLIB, "omrvmem_testReserveMemoryEx_use2To32", FALSE, FALSE, FALSE,
        TRUE);
  } else {
    portTestEnv->log(LEVEL_ERROR,
                     "***Did not find enough memory available on system, not "
...
TEST(PortVmemTest, vmem_testReserveMemoryExTopDown_use2To32) {
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
        OMRPORTLIB, "omrvmem_testReserveMemoryExTopDown_use2To32", TRUE, FALSE,
        FALSE, TRUE);
  } else {
    portTestEnv->log(LEVEL_ERROR,
                     "***Did not find enough memory available on system, not "
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictAddress_use2To32) {
  ...
  if (memoryIsAvailable(OMRPORTLIB, TRUE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
        OMRPORTLIB, "omrvmem_testReserveMemoryExStrictAddress_use2To32", FALSE,
        TRUE, FALSE, TRUE);
  } else {
    portTestEnv->log(LEVEL_ERROR,
                     "***Did not find enough memory available on system, not "
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictPageSize_use2To32) {
  ...
  if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
    omrvmem_testReserveMemoryEx_StandardAndQuickMode(
        OMRPORTLIB, "omrvmem_testReserveMemoryExStrictPageSize_use2To32", FALSE,
        FALSE, TRUE, TRUE);
  } else {
    portTestEnv->log(
        LEVEL_ERROR,
 ...
 * @param[in] strictAddress Bit indicating whether or not to set the
 * OMRPORT_VMEM_STRICT_ADDRESS flag
 * @param[in] strictPageSize Bit indicating whether or not to set the
 * OMRPORT_VMEM_STRICT_PAGE_SIZE flag
 * @param[in] use2To32G Bit indicating whether or not to allocate memory in 2 GB
 * to 32 GB range
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int omrvmem_testReserveMemoryEx_StandardAndQuickMode(
    struct OMRPortLibrary *portLibrary, const char *testName, BOOLEAN topDown,
    BOOLEAN strictAddress, BOOLEAN strictPageSize, BOOLEAN use2To32G) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  int rc = omrvmem_testReserveMemoryEx_impl(OMRPORTLIB, testName, topDown,
                                            FALSE, strictAddress,
                                            strictPageSize, use2To32G);
#if defined(LINUX) || defined(OSX)
  if (TEST_PASS == rc) {
    rc |= omrvmem_testReserveMemoryEx_impl(OMRPORTLIB, testName, topDown, TRUE,
                                           strictAddress, strictPageSize,
                                           use2To32G);
  }
 ...
 * @param[in] testName The name of the test
 * @param[in] topDown Bit indicating whether or not to allocate topDown
 * @param[in] quickSearch Bit indicating whether or not to use quick scan:
 ...
 */
static int
omrvmem_testReserveMemoryEx_impl(struct OMRPortLibrary *portLibrary,
                                 const char *testName, BOOLEAN topDown,
                                 BOOLEAN quickSearch, BOOLEAN strictAddress,
                                 BOOLEAN strictPageSize, BOOLEAN use2To32G) {
  portTestEnv->changeIndent(1);
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
#define NUM_SEGMENTS 3
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    for (j = 0; j < NUM_SEGMENTS; j++) {
      ...
      /* Sample baseline category data */
      getPortLibraryMemoryCategoryData(OMRPORTLIB, &initialBlocks,
                                       &initialBytes);
      ...
      params[j].mode |= OMRPORT_VMEM_MEMORY_MODE_READ |
                        OMRPORT_VMEM_MEMORY_MODE_WRITE |
                        OMRPORT_VMEM_MEMORY_MODE_COMMIT;
      ...
      portTestEnv->log(
          "Page Size: 0x%zx Range: [0x%zx,0x%zx] "
          "topDown: %s quickSearch: %s strictAddress: %s strictPageSize: %s "
          "use2To32G: %s\n",
          params[j].pageSize, params[j].startAddress, params[j].endAddress,
          (0 != (params[j].options & OMRPORT_VMEM_ALLOC_DIR_TOP_DOWN))
              ? "true"
              : "false",
          (0 != (params[j].options & OMRPORT_VMEM_ALLOC_QUICK)) ? "true"
                                                                : "false",
          (0 != (params[j].options & OMRPORT_VMEM_STRICT_ADDRESS)) ? "true"
                                                                   : "false",
          (0 != (params[j].options & OMRPORT_VMEM_STRICT_PAGE_SIZE)) ? "true"
                                                                     : "false",
          (0 != (params[j].options & OMRPORT_VMEM_ZOS_USE2TO32G_AREA))
              ? "true"
              : "false");
      ...
      if (memPtr[j] == NULL) {
        if (strictPageSize) {
          portTestEnv->log(LEVEL_ERROR,
                           "! Unable to reserve and commit 0x%zx bytes with "
                           "page size 0x%zx.\n",
      ...
      } else if (strictAddress &&
                 (((void *)memPtr[j] < params[j].startAddress) ||
                  ((void *)memPtr[j] > params[j].endAddress))) {
        /* if returned pointer is outside of range then fail */
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "Strict address flag set and returned pointer "
                           "[0x%zx] is outside of range.\n"
                           "\tlastErrorNumber=%d, lastErrorMessage=%s\n",
                           memPtr[j], lastErrorNumber, lastErrorMessage);
      } else if (strictPageSize && (vmemID[j].pageSize != params[j].pageSize)) {
        /* fail if strict page size flag and returned memory does not have the
         * requested page size */
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "Strict page size flag set and returned memory has "
                           "a page size of [0x%zx] "
                           "while a page size of [0x%zx] was requested.\n"
                           "\tlastErrorNumber=%d, lastErrorMessage=%s\n",
                           vmemID[j].pageSize, params[j].pageSize,
                           lastErrorNumber, lastErrorMessage);
#if defined(OMR_ENV_DATA64)
      } else if (use2To32G && strictAddress &&
                 (((uint64_t)memPtr[j] < TWO_GB) ||
                  ((uint64_t)memPtr[j] >= SIXTY_FOUR_GB))) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "\tuse2To32G flag is set and returned pointer "
                           "[0x%zx] is outside of range.\n",
                           memPtr[j]);
#endif /* OMR_ENV_DATA64 */
      } else {
        portTestEnv->log("reserved and committed 0x%zx bytes with page size "
                         "0x%zx and page flag 0x%zx at address 0x%zx\n",
                         pageSizes[i], vmemID[j].pageSize, vmemID[j].pageFlags,
                         memPtr[j]);
      }
      ...
      if (NULL != memPtr[j]) {
        ...
        if (vmemID[j].pageSize != omrvmem_get_page_size(&(vmemID[j]))) {
          outputErrorMessage(
              PORTTEST_ERROR_ARGS,
              "vmemID[j].pageSize (0x%zx)!= omrvmem_get_page_size (0x%zx) .\n",
        ...
        if (vmemID[j].pageFlags != omrvmem_get_page_flags(&(vmemID[j]))) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "vmemID[j].pageFlags (0x%zx)!= "
                             "omrvmem_get_page_flags (0x%zx) .\n",
                             vmemID[j].pageFlags,
                             omrvmem_get_page_flags(vmemID));
        }
        ...
        if (finalBytes < (initialBytes + pageSizes[i])) {
          outputErrorMessage(
              PORTTEST_ERROR_ARGS,
              "vmem reserve didn't increment category bytes as expected. Final "
              "bytes=%zu, initial bytes=%zu, page size=%zu.\n",
              finalBytes, initialBytes, pageSizes[i]);
        }
      }
    }
    ...
    for (j = 0; j < NUM_SEGMENTS; j++) {
      if (NULL != memPtr[j]) {
        ...
        if (finalBlocks >= initialBlocks) {
          outputErrorMessage(
              PORTTEST_ERROR_ARGS,
              "vmem free didn't decrement category blocks as expected. Final "
              "blocks=%zu, initial blocks=%zu, page size=%zu.\n",
              finalBlocks, initialBlocks, pageSizes[i]);
        }
        ...
        if (finalBytes > (initialBytes - pageSizes[i])) {
          outputErrorMessage(
              PORTTEST_ERROR_ARGS,
              "vmem free didn't decrement category bytes as expected. Final "
              "bytes=%zu, initial bytes=%zu, page size=%zu.\n",
              finalBytes, initialBytes, pageSizes[i]);
        }
      }
 ...
 *
 * @param[in] portLibrary The port library under test
 * @param[in] strictAddress TRUE if OMRPORT_VMEM_STRICT_ADDRESS flag is to be
 * used
 *
 * @return TRUE if memory is available, FALSE otherwise
 *
 */
static BOOLEAN memoryIsAvailable(struct OMRPortLibrary *portLibrary,
                                 BOOLEAN strictAddress) {
  portTestEnv->changeIndent(1);
#define NUM_SEGMENTS 3
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    for (j = 0; j < NUM_SEGMENTS; j++) {
      ...
      if (memPtr[j] == NULL) {
        portTestEnv->log(
            LEVEL_ERROR,
            "**Could not find 0x%zx bytes available with page size 0x%zx\n",
            pageSizes[i], pageSizes[i]);
    ...
    for (j = 0; j < NUM_SEGMENTS; j++) {
      ...
      if (NULL != memPtr[j]) {
        ...
        if (rc != 0) {
          isMemoryAvailable = FALSE;
          portTestEnv->log("**omrvmem_free_memory returned %i when trying to "
                           "free 0x%zx bytes at 0x%zx\n",
                           rc, pageSizes[i], memPtr[j]);
        }
      }
...
TEST(PortVmemTest, vmem_testReserveMemoryEx_zOSLargePageBelowBar) {
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    portTestEnv->log(
        ...
        (0 != (params.options & OMRPORT_VMEM_ALLOC_DIR_TOP_DOWN)) ? "true"
                                                                  : "false",
        (0 != (params.options & OMRPORT_VMEM_STRICT_ADDRESS)) ? "true"
                                                              : "false",
        (0 != (params.options & OMRPORT_VMEM_STRICT_PAGE_SIZE)) ? "true"
                                                                : "false",
        (0 != (params.options & OMRPORT_VMEM_ZOS_USE2TO32G_AREA)) ? "true"
                                                                  : "false");
    ...
    if (NULL == memPtr) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "unable to reserve and commit 0x%zx bytes with page "
                         "size 0x%zx, page type 0x%zx.\n",
                         params.byteAmount, pageSizes[i], pageFlags[i]);
      goto exit;

    } else {
      portTestEnv->log("reserved and committed 0x%zx bytes with page size "
                       "0x%zx and page flag 0x%x at address 0x%zx\n",
                       params.byteAmount, vmemID.pageSize, vmemID.pageFlags,
                       memPtr);
    }
    ...
    omrstr_printf(allocName, allocNameSize, "omrvmem_reserve_memory(%d)",
                  params.byteAmount);
    verifyMemory(OMRPORTLIB, testName, (char *)memPtr, params.byteAmount,
                 allocName);
    ...
    if (rc != 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrvmem_free_memory returned 0x%zx when trying to "
                         "free 0x%zx bytes\n",
...
/**
 * Test request for pages below the 2G bar with OMRPORT_VMEM_STRICT_ADDRESS set
 * on z/OS
 */
TEST(PortVmemTest, vmem_testReserveMemoryExStrictAddress_zOSLargePageBelowBar) {
  ...
  uintptr_t *pageFlags = NULL;
  const char *testName =
      "omrvmem_testReserveMemoryExStrictAddress_zOSLargePageBelowBar";
  int32_t i = 0;
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (TWO_GB <= pageSizes[i]) {
       ...
       * If the page size is >= 2G, then attempt to allocate memory with
       * OMRPORT_VMEM_STRICT_ADDRESS flag is expected to fail.
       */
      continue;
    }
    ...
    if (NULL == memPtr) {
      ...
      if ((uint64_t)memPtr >= TWO_GIG_BAR) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "\treturned memory: %p not less than 2G\n", memPtr);
        /* Don't exit here, need to free the allocate memory */
      }
      /* When using page size that can allocate memory only above 2G bar,
       * omrvmem_reserve_memory_ex should fail to reserve memory below 2G using
       * strict address but should fall back to 4K page size.
       */
      if (FALSE == isPageSizeSupportedBelowBar(pageSizes[i], pageFlags[i])) {
        if (FOUR_KB != vmemID.pageSize) {
          outputErrorMessage(
              PORTTEST_ERROR_ARGS,
              "\tdidn't expect to allocate memory below 2GB using page size "
              "0x%zx and page flags 0x%x with strict address\n",
...
#if defined(OMR_ENV_DATA64)
/**
 * Test request for pages below the 2G bar with OMRPORT_VMEM_ZOS_USE2TO32G_AREA
 * set on z/OS. In this case, since STRICT_ADDRESS flag is not set, USE2TO32G
 * flag is given priority over the requested address range. Therefore, memory is
 * expected to be allocated between 2G-32G.
 */
TEST(PortVmemTest, vmem_testReserveMemoryEx_use2To32_zOSLargePageBelowBar) {
  ...
  uintptr_t *pageFlags = NULL;
  const char *testName =
      "omrvmem_testReserveMemoryEx_use2To32_zOSLargePageBelowBar";
  int32_t i = 0;
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (NULL == memPtr) {
      ...
      if (((uint64_t)memPtr < TWO_GB) || ((uint64_t)memPtr >= SIXTY_FOUR_GB)) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "\treturned memory: %p is not in 2G-32G range\n",
                           memPtr);
        goto exit;
      }
    }
...
/**
 * Test request for pages below the 2G bar with OMRPORT_VMEM_STRICT_ADDRESS and
 * OMRPORT_VMEM_ZOS_USE2TO32G_ARE set on z/OS
 */
TEST(PortVmemTest,
     vmem_testReserveMemoryExStrictAddress_use2To32_zOSLargePageBelowBar) {
  OMRPORT_ACCESS_FROM_OMRPORT(portTestEnv->getPortLibrary());
  uintptr_t *pageSizes = NULL;
  uintptr_t *pageFlags = NULL;
  const char *testName =
      "omrvmem_testReserveMemoryExStrictAddress_use2To32_zOSLargePageBelowBar";
  int32_t i = 0;
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    params.pageFlags = pageFlags[i];
    params.options =
        OMRPORT_VMEM_STRICT_ADDRESS | OMRPORT_VMEM_ZOS_USE2TO32G_AREA;
    ...
    /* OMRPORT_VMEM_ZOS_USE2TO32G_AREA allocates memory in 2-32G but the
     * requested address is below 2G with STRICT_ADDRESS set The two conditions
     * are incompatible and should return NULL.
     */
    if (memPtr != NULL) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "Did not expect to allocate memory using page size 0x%zx, page type "
          "0x%zx in < 2GB range with OMRPORT_VMEM_ZOS_USE2TO32G_AREA and "
          "OMRPORT_VMEM_STRICT_ADDRESS .\n",
          pageSizes[i], pageFlags[i]);

      omrvmem_free_memory(memPtr, params.byteAmount, &vmemID);

    } else {
      portTestEnv->log(" Expected to fail to allocate large pages in < 2GB "
                       "range with OMRPORT_VMEM_ZOS_USE2TO32G_AREA and "
                       "OMRPORT_VMEM_STRICT_ADDRESS set\n");
    }
  }
  portTestEnv->changeIndent(-1);
  reportTestExit(OMRPORTLIB, testName);
...
/**
 * Test request for executable large pages above the 2G bar with various page
 * sizes. Exercises reserve_memory_with_moservices(), except for 4k pages.
 * Because there are no guarantees about the OS version or available memory,
...
TEST(PortVmemTest, vmem_testReserveLargePagesAboveBar) {
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    for (pageableStrict = 0; pageableStrict < 4;
         ++pageableStrict) { /* try with both pageable and non-pageable, strict
                                and non-strict */
      ...
      if (pageable) {
        if (TWO_GB == params.pageSize) {
          expectError = true;
        } else if (!OMR_ARE_ANY_BITS_SET(pageFlags[i],
                                         OMRPORT_VMEM_PAGE_FLAG_PAGEABLE)) {
        ...
        /* allocating 4K fixed pages is technically an error, but not reported
         * in existing code. */
      ...
      if (NULL != memPtr) {
        ...
        if (expectError) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "no error from omrvmem_reserve_memory_ex returned "
                             "0x%zx for pageSize=0x%zx pageFlags=0x%zx\n",
        ...
        if (pageable) {
          if (0 != rc) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS,
                "omrvmem_decommit_memory failed with code 0x%zx when trying to "
                "decommit 0x%zx bytes backed by 0x%zx-byte pages\n",
          ...
          if ((0 == rc) && strict) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS,
                "omrvmem_decommit_memory no error when trying to decommit "
                "fixed pages size=0x%zx flags=0x%zx\n",
        ...
        portTestEnv->log(LEVEL_ERROR,
                         "Failed to allocate memory using page size 0x%zx, "
                         "pageFlags 0x%zx params.options  0x%zx\n",
...
TEST(PortVmemTest, vmem_test_commitOutsideOfReservedRange) {
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (NULL == memPtr) {
      ...
      commitResult =
          omrvmem_commit_memory(memPtr, params.byteAmount + 1, &vmemID);

      if (NULL != commitResult) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "omrvmem_commit_memory did not return an error when "
                           "attempting to commit more memory"
                           "than was reserved. \n");
      } else {
        commitResult =
            omrvmem_commit_memory(memPtr, params.byteAmount, &vmemID);

        if (NULL == commitResult) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "omrvmem_commit_memory returned an error while "
                             "attempting to commit reserved memory: %s\n",
          ...
          if (0 == rc) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                               "omrvmem_decommit_memory did not return an "
                               "error when attempting to decommit "
            ...
            if (rc != 0) {
              outputErrorMessage(PORTTEST_ERROR_ARGS,
                                 "omrvmem_decommit_memory returned an error "
                                 "when attempting to decommit "
      ...
      if (rc != 0) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "omrvmem_free_memory returned %d when trying to "
                           "free 0x%zx bytes backed by 0x%zx-byte pages\n",
                           rc, params.byteAmount, pageSizes[i]);
      }
    }
  }
...
TEST(PortVmemTest, vmem_test_reserveExecutableMemory) {
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (NULL == memPtr) {
      portTestEnv->log(LEVEL_ERROR,
                       "! Could not find 0x%zx bytes available with page size "
                       "0x%zx and specified mode\n",
                       params.byteAmount, pageSizes[i]);
    } else {
      intptr_t rc;
      void (*function)();
      ...
      *((unsigned int *)memPtr) =
          (unsigned int)0x4e800020; /* blr instruction (equivalent to RET on
                                       x86)*/
      __lwsync();
      __isync();
      ...
      portTestEnv->log(
          "About to call dynamically created function, pageSize=0x%zx...\n",
          params.pageSize);
      function();
      portTestEnv->log("Dynamically created function returned successfully\n");

#elif defined(LINUX) || defined(OMR_OS_WINDOWS) || defined(J9ZOS390) ||        \
    defined(OSX)
      size_t length;

#if defined(J9ZOS39064)
      /* On 64-bit z/OS, only 4K and pageable 1M large pages that allocate
       * memory below 2G bar can be used as executable pages. Other page sizes
       * do not support executable pages.
       */
      if (FALSE == isPageSizeSupportedBelowBar(pageSizes[i], pageFlags[i])) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "Did not expect to allocate executable pages with "
                           "large page size 0x%zx, page type 0x%zx\n",
                           pageSizes[i], pageFlags[i]);
      } else {
#endif /* J9ZOS39064 */
        ...
        /* Occasionally, subtracing the addresses gives a weird result. Cap
         * length to avoid corruption. */
        ...
        function();
        portTestEnv->log(
            "Dynamically created function returned successfully\n");
#if defined(J9ZOS39064)
      }
#endif /* J9ZOS39064 */
 ...
 *
 * This test cannot test that the NUMA affinity is correctly set, just that the
 * API succeeds.
 */
TEST(PortVmemTest, vmem_test_numa) {
  ...
  if ((detailReturnCode != 0) || (0 == totalNumaNodeCount)) {
    ...
    J9MemoryNodeDetail *nodeArray =
        (J9MemoryNodeDetail *)omrmem_allocate_memory(
            nodeArraySize, OMRMEM_CATEGORY_PORT_LIBRARY);
    intptr_t rc = 0;

    if (nodeArray == NULL) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "omrvmem_free_memory returned NULL when trying to allocate "
          "J9MemoryNodeDetail array for NUMA test (FAIL)\n");
      goto exit;
    }
    ...
    if ((detailReturnCode != rc) || (totalNumaNodeCount != originalNodeCount)) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "omrvmem_numa_get_node_details returned different results on second "
          "call (return %zd versus %zd, count %zu versus %zu)\n",
    ...
    /* count the number of nodes with physical memory */
    {
      ...
      uintptr_t allowed = 0;
      for (activeNodeIndex = 0; activeNodeIndex < totalNumaNodeCount;
           activeNodeIndex++) {
    ...
    if (0 == nodesWithMemoryCount) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Found zero nodes with memory even after NUMA was "
                         "reported as supported (FAIL)\n");
      goto exit;
    }
    /* reserve and commit memory for each page size */
    for (i = 0; pageSizes[i] != 0; i++) {
...
#endif /* J9ZOS390 */
      memPtr = (char *)omrvmem_reserve_memory(
          NULL, reserveSizeInBytes, &vmemID,
          OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE,
          pageSize, OMRMEM_CATEGORY_PORT_LIBRARY);

      portTestEnv->log(
          "reserved 0x%zx bytes with page size 0x%zx at address 0x%zx\n",
      ...
      if (memPtr != NULL) {
        ...
        for (nodeIndex = 0; nodeIndex < totalNumaNodeCount; nodeIndex++) {
          if (policyForMemoryNode == nodeArray[nodeIndex].memoryPolicy) {
            ...
            rc = omrvmem_numa_set_affinity(nodeNumber, numaBlock, pageSize,
                                           &vmemID);
            if (rc != 0) {
              outputErrorMessage(
                  PORTTEST_ERROR_ARGS,
                  "omrvmem_numa_set_affinity returned 0x%zx when trying to set "
                  "affinity for 0x%zx bytes at 0x%p to node %zu\n",
        ...
        commitResult =
            omrvmem_commit_memory(memPtr, numaBlock - memPtr, &vmemID);
        if (commitResult != memPtr) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "omrvmem_commit_memory returned 0x%p when trying "
                             "to commit 0x%zx bytes at 0x%p\n",
                             commitResult, numaBlock - memPtr, memPtr);
          goto exit;
        }

        /* ideally we'd test that the memory actually has some NUMA
         * characteristics, but that's difficult to prove */

        /* can we read and write to the memory? */
        omrstr_printf(allocName, allocNameSize, "omrvmem_reserve_memory(%d)",
                      pageSize);
        verifyMemory(OMRPORTLIB, testName, memPtr, reserveSizeInBytes,
                     allocName);
        ...
        if (rc != 0) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "omrvmem_free_memory returned 0x%zx when trying "
                             "to free 0x%zx bytes backed by 0x%zx-byte pages\n",
...
#define PRINT_FIND_VALID_PAGE_SIZE_INPUT(mode, pageSize, pageFlags)            \
  portTestEnv->log("Input > mode: %zu, requestedPageSize: 0x%zx, "             \
                   "requestedPageFlags: 0x%zx\n",                              \
                   mode, pageSize, pageFlags)

void verifyFindValidPageSizeOutput(
    struct OMRPortLibrary *portLibrary, const char *testName,
    uintptr_t pageSizeExpected, uintptr_t pageFlagsExpected,
    uintptr_t isSizeSupportedExpected, uintptr_t pageSizeReturned,
    uintptr_t pageFlagsReturned, uintptr_t isSizeSupportedReturned) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  uintptr_t result = FALSE;

  portTestEnv->log("Expected > expectedPageSize: 0x%zx, expectedPageFlags: "
                   "0x%zx, isSizeSupported: 0x%zx\n\n",
                   pageSizeExpected, pageFlagsExpected,
                   isSizeSupportedExpected);

  if ((pageSizeExpected == pageSizeReturned) &&
      (pageFlagsExpected == pageFlagsReturned)) {
    if (isSizeSupportedExpected != isSizeSupportedReturned) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "isSizeSupported should be: 0x%zx but found: 0x%zx\n",
    ...
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Expected omrvmem_find_valid_page_size() to return "
                       "pageSize: 0x%zx, pageFlags: 0x%zx, but found pageSize: "
                       "0x%zx, pageFlags: 0x%zx\n",
                       pageSizeExpected, pageFlagsExpected, pageSizeReturned,
                       pageFlagsReturned);
  }
...
#if (defined(LINUX) && !defined(LINUXPPC)) || defined(OMR_OS_WINDOWS) ||       \
    defined(OSX)

static int
omrvmem_testFindValidPageSize_impl(struct OMRPortLibrary *portLibrary,
                                   const char *testName) {
#define MODE_NOT_USED 0
   ...
   * Moreover, since large page size supported by Linux and Windows is not
   * fixed, we assume it to be 4M for testing purpose.
   */
  omrvmem_default_large_page_size_ex(0, &defaultLargePageSize,
                                     &defaultLargePageFlags);
  if (0 != defaultLargePageSize) {
    portTestEnv->log(
        "defaultLargePageSize: 0x%zx, defaultLargePageFlags: 0x%zx\n",
        defaultLargePageSize, defaultLargePageFlags);
  }

  /* Note that -Xlp<size>, -Xlp:objectheap and -Xlp:codecache are treated same,
   * and callers always pass OMRPORT_VMEM_PAGE_FLAG_NOT_USED as the
   * requestedPageFlags to omrvmem_find_valid_page_size(). Only difference is in
   * case of -Xlp<size>, omrvmem_find_valid_page_size() is not called if default
   * large page size is not present.
   */
  ...
  PRINT_FIND_VALID_PAGE_SIZE_INPUT(MODE_NOT_USED, requestedPageSize,
                                   requestedPageFlags);

  omrvmem_find_valid_page_size(MODE_NOT_USED, &requestedPageSize,
                               &requestedPageFlags, &isSizeSupported);
  ...
  verifyFindValidPageSizeOutput(OMRPORTLIB, testName, expectedPageSize,
                                expectedPageFlags, expectedIsSizeSupported,
                                requestedPageSize, requestedPageFlags,
                                isSizeSupported);
...
TEST(PortVmemTest, vmem_testFindValidPageSize) {
  ...
  portTestEnv->changeIndent(1);
#if defined(LINUX) || defined(OSX)
#define OMRPORT_VMEM_PAGESIZE_COUNT                                            \
  5 /* This should be same as defined in port/unix_include/j9portpg.h */
#elif defined(OMR_OS_WINDOWS)
#define OMRPORT_VMEM_PAGESIZE_COUNT                                            \
  3    /* This should be same as defined in port/win32_include/j9portpg.h */
#endif /* defined(LINUX) || defined(OSX) */
  ...
  const char *testName =
      "omrvmem_testFindValidPageSize(no default large page size)";
  const char *testName2 =
      "omrvmem_testFindValidPageSize(with 4M default large page size)";
  int rc = 0;
  ...
  /* Create copy of existing PPG_vmem_pageSize and PPG_vmem_pageFlags as they
   * would be modified in this test */
  memcpy(old_vmem_pageSize, pageSizes,
         OMRPORT_VMEM_PAGESIZE_COUNT * sizeof(uintptr_t));
  memcpy(old_vmem_pageFlags, pageFlags,
         OMRPORT_VMEM_PAGESIZE_COUNT * sizeof(uintptr_t));

  /* Create new PPG_vmem_pageSize and PPG_vmem_pageFlags without any large page
   * size */
  pageSizes[0] = FOUR_KB;
  ...
  memcpy(pageSizes, old_vmem_pageSize,
         OMRPORT_VMEM_PAGESIZE_COUNT * sizeof(uintptr_t));
  memcpy(pageFlags, old_vmem_pageFlags,
         OMRPORT_VMEM_PAGESIZE_COUNT * sizeof(uintptr_t));
...
#define MODE_NOT_USED 0
   ...
   * callers always pass OMRPORT_VMEM_PAGE_FLAG_NOT_USED as the
   * requestedPageFlags to omrvmem_find_valid_page_size(). Only difference is in
   * case of -Xlp<size> omrvmem_find_valid_page_size() is not called if default
   * large page size is not present.
   */
  ...
  /* First get the page size of the data segment. This is the page size used by
   * the JIT code cache if 16M Code Pages are not being used */
#if defined(AIXPPC)
#define SAMPLE_BLOCK_SIZE 4
  /* Allocate a memory block using omrmem_allocate_memory, and use the address
   * to get the page size of data segment */
  address =
      omrmem_allocate_memory(SAMPLE_BLOCK_SIZE, OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == address) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Failed to allocate block of memory to determine page "
  ...
  if (-1 == rc) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "Failed to get page size of data segment using vmgetinfo()");
    goto _exit;
  } else {
  ...
  omrvmem_find_valid_page_size(mode, &requestedPageSize, &requestedPageFlags,
                               &isSizeSupported);
...
#endif /* defined(OMR_ENV_DATA64) */
  {
    expectedPageSize = dataSegmentPageSize;
    expectedPageFlags = OMRPORT_VMEM_PAGE_FLAG_NOT_USED;
    expectedIsSizeSupported =
        (16 * ONE_MB == dataSegmentPageSize) ? TRUE : FALSE;
  }
...
#if !defined(OMR_ENV_DATA64)
  portTestEnv->log("\nCase %d: -Xlp:codecache:pagesize=16M with "
                   "TR_ppcCodeCacheConsolidationEnabled set\n",
                   caseIndex++);
  /* No port library API to set the environment variable. Use setenv() */
  rc = setenv("TR_ppcCodeCacheConsolidationEnabled", "true",
              1 /*overwrite any existing value */);
  if (-1 == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
  ...
  expectedPageFlags = OMRPORT_VMEM_PAGE_FLAG_NOT_USED;
  expectedIsSizeSupported =
      (16 * FOUR_KB == dataSegmentPageSize) ? TRUE : FALSE;
...
#if !defined(OMR_ENV_DATA64)
  portTestEnv->log("\nCase %d: -Xlp:codecache:pagesize=64K with "
                   "TR_ppcCodeCacheConsolidationEnabled set\n",
                   caseIndex++);
  /* No port library API to set the environment variable. Use setenv() */
  rc = setenv("TR_ppcCodeCacheConsolidationEnabled", "true",
...
#if !defined(OMR_ENV_DATA64)
  portTestEnv->log("\nCase %d: -Xlp:codecache:pagesize=4K with "
                   "TR_ppcCodeCacheConsolidationEnabled set\n",
                   caseIndex++);
  /* No port library API to set the environment variable. Use setenv() */
  rc = setenv("TR_ppcCodeCacheConsolidationEnabled", "true",
...
#if !defined(OMR_ENV_DATA64)
  portTestEnv->log(LEVEL_ERROR,
                   "\nCase %d: -Xlp:codecache:pagesize=<not in 16G, 16M, 64K, "
                   "4K> with TR_ppcCodeCacheConsolidationEnabled set\n",
                   caseIndex++);
  /* No port library API to set the environment variable. Use setenv() */
  rc = setenv("TR_ppcCodeCacheConsolidationEnabled", "true",
              1 /*overwrite any existing value */);
...
int omrvmem_testFindValidPageSize(struct OMRPortLibrary *portLibrary) {
#define OMRPORT_VMEM_PAGESIZE_COUNT                                            \
  5 /* This should be same as defined in port/unix_include/j9portpg.h */
  ...
  rc |= omrvmem_testFindValidPageSize_impl(
      OMRPORTLIB, "omrvmem_testFindValidPageSize(no default large page size)");

  /* Create new PPG_vmem_pageSize and PPG_vmem_pageFlags only 64K large page
   * size  */
  pageSizes[0] = FOUR_KB;
  pageFlags[0] = OMRPORT_VMEM_PAGE_FLAG_NOT_USED;
  ...
  rc |= omrvmem_testFindValidPageSize_impl(
      OMRPORTLIB, "omrvmem_testFindValidPageSize(64K large page size)");

  /* Create new PPG_vmem_pageSize and PPG_vmem_pageFlags with 64K and 16M large
   * page size */
  pageSizes[0] = FOUR_KB;
  pageFlags[0] = OMRPORT_VMEM_PAGE_FLAG_NOT_USED;
  ...
  rc |= omrvmem_testFindValidPageSize_impl(
      OMRPORTLIB, "omrvmem_testFindValidPageSize(64K and 16M large page size)");

#if defined(OMR_ENV_DATA64)
  /* Create new PPG_vmem_pageSize and PPG_vmem_pageFlags with all possible large
   * pages (16G, 16M, 64K)
   */
  pageSizes[0] = FOUR_KB;
  pageFlags[0] = OMRPORT_VMEM_PAGE_FLAG_NOT_USED;
  ...
  rc |= omrvmem_testFindValidPageSize_impl(
      OMRPORTLIB, "omrvmem_testFindValidPageSize(all possible page sizes)");
#endif /* defined(OMR_ENV_DATA64) */
...
omrvmem_testFindValidPageSize_impl(struct OMRPortLibrary *portLibrary,
  ...
  portTestEnv->log("\nCase %d: -Xlp2G\n", caseIndex++);
  if (0 == defaultLargePageSize) {
    portTestEnv->log("Skip this test as the configuration does not support "
                     "default large page size\n");
  } else {
    mode = 0;
    requestedPageSize = TWO_GB;
    requestedPageFlags = OMRPORT_VMEM_PAGE_FLAG_NOT_USED;
    PRINT_FIND_VALID_PAGE_SIZE_INPUT(mode, requestedPageSize,
                                     requestedPageFlags);
...
int omrvmem_testFindValidPageSize(struct OMRPortLibrary *portLibrary) {
#define OMRPORT_VMEM_PAGESIZE_COUNT                                            \
  5 /* This should be same as defined in port/zos390/j9portpg.h */
  ...
  /* Create new PPG_vmem_pageSize and PPG_vmem_pageFlags with only Pageable 1MB
   * large pages  */
  pageSizes[0] = FOUR_KB;
  pageFlags[0] = OMRPORT_VMEM_PAGE_FLAG_PAGEABLE;
  ...
  /* Create new PPG_vmem_pageSize and PPG_vmem_pageFlags with only default large
   * page size (Non-pageable 1MB pages) */
  pageSizes[0] = FOUR_KB;
  pageFlags[0] = OMRPORT_VMEM_PAGE_FLAG_PAGEABLE;
  ...
  testName =
      "omrvmem_testFindValidPageSize(both pageable and nonpageable 1M pages)";

  /* Create new PPG_vmem_pageSize and PPG_vmem_pageFlags with both pageable and
   * non-pageable 1M large pages */
  pageSizes[0] = FOUR_KB;
  pageFlags[0] = OMRPORT_VMEM_PAGE_FLAG_PAGEABLE;
  ...
  /* Create new PPG_vmem_pageSize and PPG_vmem_pageFlags with all possible large
   * pages (non-pageable 2G, non-pageable 1M and pageable 1M)
   */
  pageSizes[0] = FOUR_KB;
  pageFlags[0] = OMRPORT_VMEM_PAGE_FLAG_PAGEABLE;
...
uintptr_t getSizeFromString(struct OMRPortLibrary *portLibrary,
                            char *sizeString) {
...
/**
 * Run with varied arguments based on values from /proc/meminfo, this function
 * tests effects of disclaiming memory on performance and prints out performance
 * results
 *
 * @ref omrvmem.c
 *
 * @param[in] portLibrary The port library under test
 * @param[in] disclaimPerfTestArg This arg holds the page size to disclaim and
 * the byte amount
 * @param[in] shouldDisclaim  If true, disclaim virtual memory, otherwise do not
 * disclaim.
 *
 * @return 0 on success, -1 on failure
 */
int32_t omrvmem_disclaimPerfTests(struct OMRPortLibrary *portLibrary,
                                  char *disclaimPerfTestArg,
                                  BOOLEAN shouldDisclaim) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  int rc = 0;
  ...
  /* parse the disclaimPerfTestArg to get the pageSize, byteAmount and disclaim
   * values disclaimPerfTestArg is the string after the ':' from
   * -disclaimPerfTest:<pageSize>,<byteAmount>,<totalMemory>[,<numIterations>]
   * while shouldDisclaim is set from the +/- prefix, numIterations is optional
   * (default is 50 iterations) Example options: -disclaimPerfTest:4K,256M,4G
  ...
  /* verify sanity of port library for these tests.  If this fails do not
   * continue */
  /* OMROTOD: This function was changed into a test, fix it so it can be called
   *here. rc = omrvmem_verify_functiontable_slots(portLibrary);
   */

  if (TEST_PASS == rc) {
    portTestEnv->log("\npageSize: 0x%zx, byteAmount 0x%zx, memTotal 0x%zx, "
                     "numIterations %i, disclaim %i\n",
                     pageSize, byteAmount, memTotal, numIterations,
                     shouldDisclaim);
    rc |= omrvmem_bench_write_and_decommit_memory(
        OMRPORTLIB, pageSize, byteAmount, shouldDisclaim, numIterations);
    rc |= omrvmem_bench_reserve_write_decommit_and_free_memory(
        OMRPORTLIB, pageSize, byteAmount, shouldDisclaim, numIterations);
#if defined(OMR_ENV_DATA64)
    rc |= omrvmem_exhaust_virtual_memory(OMRPORTLIB, pageSize, byteAmount,
                                         shouldDisclaim);
#endif
    rc |= omrvmem_bench_force_overcommit_then_decommit(
        OMRPORTLIB, memTotal, pageSize, shouldDisclaim);
...
/**
 * Helper for omrvmem_testOverlappingSegments. Initializes the J9PortVmemParams
 * struct.
 */
J9PortVmemParams *getParams(struct OMRPortLibrary *portLibrary,
  ...
  vmemParams->mode |= OMRPORT_VMEM_MEMORY_MODE_READ |
                      OMRPORT_VMEM_MEMORY_MODE_WRITE |
...
/**
 * Helper for omrvmem_testOverlappingSegments. Checks whether two memory
 * segments overlap.
 */
BOOLEAN
checkForOverlap(J9PortVmemIdentifier *a, J9PortVmemIdentifier *b) {
  BOOLEAN A_before_B =
      ((uintptr_t)a->address + a->size) <= (uintptr_t)b->address;
  BOOLEAN B_before_A =
      ((uintptr_t)b->address + b->size) <= (uintptr_t)a->address;
...
TEST(PortVmemTest, vmem_testOverlappingSegments) {
  ...
  vmemParams = (J9PortVmemParams *)omrmem_allocate_memory(
      sizeof(J9PortVmemParams) * CYCLES, OMRMEM_CATEGORY_PORT_LIBRARY);
  vmemID = (J9PortVmemIdentifier *)omrmem_allocate_memory(
      sizeof(J9PortVmemIdentifier) * CYCLES, OMRMEM_CATEGORY_PORT_LIBRARY);
  keepCycles = (int *)omrmem_allocate_memory(sizeof(int) * CYCLES,
  ...
  portTestEnv->log("Segment           Start             End               Size "
                   "             Keep cycle\n");
  ...
  for (i = 0; i < CYCLES; i++) {
    ...
    if (NULL == memPtr) {
      ...
      portTestEnv->log(
          LEVEL_ERROR,
          "Ignoring memory allocation failure(%d of %d loops finished).\n", i,
          CYCLES);
      goto exit;
    }
    /* Determine how long to keep the segment */
    keepCycles[i] =
        (rand() % (((int)((double)CYCLES / FREE_PROB)) - i)) + i + 1;
    portTestEnv->log("%.16d  %.16X  %.16X  %.16X  %d\n", i,
                     (uintptr_t)vmemID[i].address,
                     (uintptr_t)vmemID[i].address + vmemID[i].size,
                     vmemID[i].size, keepCycles[i]);
    ...
    for (j = 0; j < i; j++) {
      if (keepCycles[j] >= i) { /* If not already freed */
        if (checkForOverlap(&vmemID[i], &vmemID[j])) {
          ...
          portTestEnv->log("%.16d  %.16X  %.16X  %.16X  %d\n", j,
                           (uintptr_t)vmemID[j].address,
                           (uintptr_t)vmemID[j].address + vmemID[j].size,
                           vmemID[j].size, keepCycles[j]);
          keepCycles[i] = i - 1; /* Pretend this segment is already freed to
                                    avoid double freeing overlapping segments */
          result = -1;
          goto exit;
        }
      }
    ...
    for (j = 0; j < i; j++) {
      if (keepCycles[j] == i) {
        int32_t rc = omrvmem_free_memory(vmemID[j].address,
                                         vmemParams[j].byteAmount, &vmemID[j]);
        if (0 != rc) {
...
TEST(PortVmemTest, vmem_testGetProcessMemorySize) {
  ...
  result =
      omrvmem_get_process_memory_size(OMRPORT_VMEM_PROCESS_PHYSICAL, &size);
...
#endif /* defined(J9ZOS390) */
  size = 0;
  result = omrvmem_get_process_memory_size(OMRPORT_VMEM_PROCESS_EnsureWideEnum,
                                           &size);
...
TEST(PortVmemTest, omrvmem_get_available_physical_memory) {
  ...
  int32_t status =
      omrvmem_get_available_physical_memory(&freePhysicalMemorySize);
  /* omrvmem_get_available_physical_memory is not supported on zos */
#if !defined(J9ZOS390)
  ASSERT_EQ(0, status)
      << "Non-zero status from omrvmem_get_available_physical_memory";
