...
TEST(PortVmemTest, vmem_test1) {
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (memPtr == NULL) {
      ...
      portTestEnv->log(
          "reserved and committed 0x%zx bytes with page size 0x%zx at address "
          "0x%zx\n",
    ...
    /* Memory verified successfully */
    {
      ...
      if (contiguous == NULL) {
        ...
        if (OMRPORT_ERROR_VMEM_INSUFFICENT_RESOURCES == lastErrorNumber) {
          ...
          portTestEnv->changeIndent(1);
          portTestEnv->log(LEVEL_ERROR,
                           "REBOOT THE MACHINE to free up resources AND TRY "
                           "THE TEST AGAIN\n");
          portTestEnv->changeIndent(-1);
        }
        goto exit;
...
int omrvmem_bench_write_and_decommit_memory(struct OMRPortLibrary *portLibrary,
  ...
  portTestEnv->log(
      ...
      "[write_to_all/decommit] test: %lli\n",
...
#define NUM_VMEM_IDS 1000
  ...
  portTestEnv->log(
      "%s pageSize 0x%zx, byteAmount 0x%zx, millis for %u iterations: "
      "[reserve/write_to_all/decommit] test: %lli\n",
      disclaim ? "disclaim" : "nodisclaim", pageSize, byteAmount, NUM_VMEM_IDS,
      deltaMillis);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int omrvmem_testReserveMemoryEx_impl(
    struct OMRPortLibrary *portLibrary, const char *testName, BOOLEAN topDown,
    BOOLEAN quickSearch, BOOLEAN strictAddress, BOOLEAN strictPageSize,
    BOOLEAN use2To32G) {
  portTestEnv->changeIndent(1);
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    for (j = 0; j < NUM_SEGMENTS; j++) {
      ...
      if (memPtr[j] == NULL) {
        ...
        portTestEnv->log(
            "reserved and committed 0x%zx bytes with page size 0x%zx and page "
            "flag 0x%zx at address 0x%zx\n",
    ...
    for (j = 0; j < NUM_SEGMENTS; j++) {
      ...
      if (NULL != memPtr[j]) {
        ...
        if (rc != 0) {
          isMemoryAvailable = FALSE;
          portTestEnv->log(
              "**omrvmem_free_memory returned %i when trying to free 0x%zx "
              "bytes at 0x%zx\n",
              rc, pageSizes[i], memPtr[j]);
        }
      }
...
TEST(PortVmemTest,
  ...
  for (i = 0; pageSizes[i] != 0; i++) {
    ...
    if (memPtr != NULL) {
      ...
      portTestEnv->log(
          " Expected to fail to allocate large pages in < 2GB range with "
          "OMRPORT_VMEM_ZOS_USE2TO32G_AREA and OMRPORT_VMEM_STRICT_ADDRESS "
          "set\n");
    }
  }
  portTestEnv->changeIndent(-1);
...
#define PRINT_FIND_VALID_PAGE_SIZE_INPUT(mode, pageSize, pageFlags)       \
  portTestEnv->log(                                                       \
      "Input > mode: %zu, requestedPageSize: 0x%zx, requestedPageFlags: " \
      "0x%zx\n",                                                          \
...
void verifyFindValidPageSizeOutput(
  ...
  portTestEnv->log(
      "Expected > expectedPageSize: 0x%zx, expectedPageFlags: 0x%zx, "
      "isSizeSupported: 0x%zx\n\n",
...
static int omrvmem_testFindValidPageSize_impl(
    struct OMRPortLibrary *portLibrary, const char *testName) {
...
static int omrvmem_testFindValidPageSize_impl(
  ...
  if (0 == defaultLargePageSize) {
    portTestEnv->log(
        "Skip this test as the configuration does not support default large "
        "page size\n");
  } else {
    mode = 0;
    requestedPageSize = TWO_GB;
...
int32_t omrvmem_disclaimPerfTests(struct OMRPortLibrary *portLibrary,
  ...
  if (TEST_PASS == rc) {
    portTestEnv->log(
        "\npageSize: 0x%zx, byteAmount 0x%zx, memTotal 0x%zx, numIterations "
        "%i, disclaim %i\n",
