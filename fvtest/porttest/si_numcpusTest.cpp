...
TEST(PortSysinfoTest, sysinfo_numcpus_test0) {
  ...
  if (0 == numberPhysicalCPUs) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "\tomrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_"
                       "PHYSICAL) returned invalid value 0.\n");
  }
...
TEST(PortSysinfoTest, sysinfo_numcpus_test1) {
    ...
    if (startsWith(portTestEnv->_argv[i], BOUNDCPUS)) {
      ...
      if (-1 == boundCPUs) {
        portTestEnv->log(LEVEL_ERROR,
                         "Invalid (non-numeric) format for %s value (%s).\n",
  ...
  if (0 == boundCPUs) {
    ...
    if (numberBoundCPUs < numberPhysicalCPUs) {
      portTestEnv->log(
          LEVEL_ERROR,
          "The test was configured to run without CPU restrictions (e.g. "
          "taskset), but CPU restrictions were detected.\n");
      portTestEnv->log(
          LEVEL_ERROR,
          "The test will continue with CPU restrictions configuration.\n\n");
...
TEST(PortSysinfoTest, sysinfo_numcpus_test2) {
  ...
  if (0 == numberOnlineCPUs) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "\tomrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_"
                       "ONLINE) returned invalid value 0.\n");
  }
...
TEST(PortSysinfoTest, sysinfo_numcpus_test3) {
  ...
  portTestEnv->log("\nGet number of online and physical CPUs. Validate online "
                   "<= physical.\n");

  if (numberOnlineCPUs > numberPhysicalCPUs) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "\tomrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE) returned "
        "value greater than "
        "omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_PHYSICAL).\n");
  }
...
/**
 * Call Port Library function omrsysinfo_get_number_CPUs_by_type with invalid
 * type. Validate that return value is 0.
 */
TEST(PortSysinfoTest, sysinfo_numcpus_test4) {
  ...
  portTestEnv->log("\n");
  portTestEnv->log(
      "Call omrsysinfo_get_number_CPUs_by_type with invalid type.\n");
  portTestEnv->changeIndent(1);
  portTestEnv->log("Expected: 0\n");
  portTestEnv->log("Result:   %d\n", result);
  portTestEnv->changeIndent(-1);
  ...
  if (result > 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "\tomrsysinfo_get_number_CPUs_by_type returned non-zero "
                       "value when given invalid type.\n");
  }
...
#define J9SYSINFO_NUMCPUS_RUNTIME_LOOPS 1000
  ...
  portTestEnv->log("Get number of %s CPUs %d times: %d ns\n", testType,
                   J9SYSINFO_NUMCPUS_RUNTIME_LOOPS, difference);
...
TEST(PortSysinfoTest, sysinfo_numcpus_runTime) {
  ...
  EXPECT_TRUE(TEST_PASS ==
              omrsysinfo_numcpus_runTime(OMRPORTLIB, OMRPORT_CPU_PHYSICAL))
      << "Test failed.";
  EXPECT_TRUE(TEST_PASS ==
              omrsysinfo_numcpus_runTime(OMRPORTLIB, OMRPORT_CPU_ONLINE))
      << "Test failed.";
  EXPECT_TRUE(TEST_PASS ==
              omrsysinfo_numcpus_runTime(OMRPORTLIB, OMRPORT_CPU_BOUND))
      << "Test failed.";
}
 ...
 */
static uint32_t onlineProcessorCount(const struct J9ProcessorInfos *procInfo) {
  int32_t cntr = 0;
  uint32_t n_onln = 0;
...
/**
 * Test for omrsysinfo_get_number_online_CPUs() port library API. We obtain the
 * online processor count using other (indirect) method - calling the other port
 * library API omrsysinfo_get_processor_info() and cross-check against this.
 */
TEST(PortSysinfoTest, sysinfo_testOnlineProcessorCount1) {
  ...
  if (0 != rc) {
     ...
     */
    uintptr_t n_cpus_online =
        omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE);
    if (0 == n_cpus_online) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE) failed.\n");
      goto _cleanup;
    }
...
/**
 * Test for omrsysinfo_get_number_total_CPUs() port library API. Validate the
 * number of available (configured) logical CPUs by cross-checking with what is
 * obtained from invoking the other port library API
 * omrsysinfo_get_processor_info().
 */
TEST(PortSysinfoTest, sysinfo_testtotalProcessorCount) {
  ...
  if (0 != rc) {
     ...
     */
    uintptr_t n_cpus_physical =
        omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_PHYSICAL);
    if (0 == n_cpus_physical) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_PHYSICAL) failed.\n");
      goto _cleanup;
    }
