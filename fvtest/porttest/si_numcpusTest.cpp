...
TEST(PortSysinfoTest, sysinfo_numcpus_test0)
{
  ...
  if (0 == numberPhysicalCPUs) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "\tomrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_PHYSICAL) returned invalid value 0.\n");
  }
  ...
  if (0 == boundCPUs) {
    ...
    if (numberBoundCPUs < numberPhysicalCPUs) {
      portTestEnv->log(LEVEL_ERROR,
        "The test was configured to run without CPU restrictions (e.g. taskset), but CPU restrictions were "
        "detected.\n");
...
TEST(PortSysinfoTest, sysinfo_numcpus_test2)
{
  ...
  if (0 == numberOnlineCPUs) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "\tomrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE) returned invalid value 0.\n");
  }
...
TEST(PortSysinfoTest, sysinfo_numcpus_test3)
{
  ...
  if (numberOnlineCPUs > numberPhysicalCPUs) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
      "\tomrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE) returned value greater than "
...
TEST(PortSysinfoTest, sysinfo_numcpus_test4)
{
  ...
  if (result > 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "\tomrsysinfo_get_number_CPUs_by_type returned non-zero value when given invalid type.\n");
  }
...
#define J9SYSINFO_NUMCPUS_RUNTIME_LOOPS 1000
  ...
  portTestEnv->log("\n");
  portTestEnv->log("Get number of %s CPUs %d times: %d ns\n", testType, J9SYSINFO_NUMCPUS_RUNTIME_LOOPS, difference);
...
TEST(PortSysinfoTest, sysinfo_testtotalProcessorCount)
{
  ...
  if (0 != rc) {
    ...
    if ((procInfo.totalProcessorCount > 0) && ((uintptr_t)procInfo.totalProcessorCount == n_cpus_physical)) {
