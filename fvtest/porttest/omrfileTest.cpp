...
TEST_F(PortFileTest2, file_test3)
{
  ...
  if (strlen(readBufPtr) != strlen(expectedResult)) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrfile_read_text() returned %d expected %d\n", strlen(readBufPtr), strlen(expectedResult));
    goto exit;
  }
...
TEST_F(PortFileTest2, file_test8)
{
  ...
  if (0 != prevFilePtr) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "File pointer location is wrong after opening file. Expected = 0, Got = %lld", prevFilePtr);
  }
  ...
  if (strcmp(expectedOutput, outputLine)) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "Read back data \"%s\" not matching expected data \"%s\"\n", outputLine, expectedOutput);
    goto exit;
  }
...
TEST_F(PortFileTest2, file_test9)
{
  ...
  for (i = 0; i < numberIterations; i++) {
    ...
    if (1 != rc) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "%s() returned %d expected 1, iteration %d\n", FILE_WRITE_FUNCTION_NAME, rc, i + 1);
      goto exit;
    }
    rc = FILE_CLOSE_FUNCTION(OMRPORTLIB, fd);
    if (0 != rc) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "%s() returned %d expected %d, iteration %d\n", FILE_CLOSE_FUNCTION_NAME, rc, 0, i + 1);
      goto exit;
    }
...
TEST_F(PortFileTest2, file_test18)
{
  ...
  lockRc
    = FILE_LOCK_BYTES_FUNCTION(OMRPORTLIB, fd, OMRPORT_FILE_READ_LOCK | OMRPORT_FILE_NOWAIT_FOR_LOCK, offset, length);
...
TEST_F(PortFileTest2, file_test19)
{
  ...
  lockRc
    = FILE_LOCK_BYTES_FUNCTION(OMRPORTLIB, fd, OMRPORT_FILE_WRITE_LOCK | OMRPORT_FILE_NOWAIT_FOR_LOCK, offset, length);
...
int omrfile_test22_child_2(OMRPortLibrary* portLibrary)
{
  ...
  lockRc
    = FILE_LOCK_BYTES_FUNCTION(OMRPORTLIB, fd, OMRPORT_FILE_WRITE_LOCK | OMRPORT_FILE_WAIT_FOR_LOCK, offset, length);
...
TEST_F(PortFileTest2, file_test30)
{
  ...
  if (-1 == invalidFileFd) {
    ...
    if (lastErrorNumber == OMRPORT_ERROR_FILE_EXIST) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
        "omrerror_last_error_number(\"%s\") returned %i (OMRPORT_ERROR_FILE_EXIST), anything but would be fine %i\n",
...
TEST_F(PortFileTest2, file_test37)
{
  ...
  if (rc >= 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
      "Negative test failure: omrfile_fstat(-1, ..) on invalid file descriptor is expected to fail but it returned "
      "success with rc=%d (ignore the following LastErrorNumber and LastErrorMessage)\n",
      rc);
  }
...
TEST_F(PortFileTest2, file_test39)
{
  ...
  if (-1 != fd) {
    ...
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "%s() failed to create file %s with mode=%d\n", FILE_OPEN_FUNCTION_NAME, filename, mode);
  }
  ...
  if (-1 != fd) {
    ...
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "5s() failed to create file %s with mode=%d\n", FILE_OPEN_FUNCTION_NAME, filename, mode);
  }
