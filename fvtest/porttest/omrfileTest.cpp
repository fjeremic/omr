 ...
 */
static void fileWrite(struct OMRPortLibrary* portLibrary, intptr_t fd, const char* format, ...)
{
...
intptr_t omrfile_open_helper(struct OMRPortLibrary* portLibrary, const char* path, int32_t flags, int32_t mode)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
int32_t omrfile_lock_bytes_helper(
    struct OMRPortLibrary* portLibrary, intptr_t fd, int32_t lockFlags, uint64_t offset, uint64_t length)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
    return isAsync ? omrfile_blockingasync_lock_bytes(fd, lockFlags, offset, length)
                   : omrfile_lock_bytes(fd, lockFlags, offset, length);
}

int32_t omrfile_unlock_bytes_helper(struct OMRPortLibrary* portLibrary, intptr_t fd, uint64_t offset, uint64_t length)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
    return isAsync ? omrfile_blockingasync_unlock_bytes(fd, offset, length) : omrfile_unlock_bytes(fd, offset, length);
}

intptr_t omrfile_read_helper(struct OMRPortLibrary* portLibrary, intptr_t fd, void* buf, intptr_t nbytes)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
    return isAsync ? omrfile_blockingasync_read(fd, buf, nbytes) : omrfile_read(fd, buf, nbytes);
}

intptr_t omrfile_write_helper(struct OMRPortLibrary* portLibrary, intptr_t fd, void* buf, intptr_t nbytes)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
    return isAsync ? omrfile_blockingasync_write(fd, buf, nbytes) : omrfile_write(fd, buf, nbytes);
}

int32_t omrfile_set_length_helper(struct OMRPortLibrary* portLibrary, intptr_t fd, I_64 newLength)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
TEST_F(PortFileTest2, file_test3)
{
    ...
    if (strcmp(readBufPtr, expectedResult)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "omrfile_read_text() returned \"%s\" expected \"%s\"\n", readBufPtr, expectedResult);
        goto exit;
    }
    ...
    struct OMRPortLibrary* portLibrary, const char* testName, const char* pathName, int32_t flags, int32_t mode)
{
    ...
    if (-1 == fd) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "%s() returned %d expected valid file handle\n", FILE_OPEN_FUNCTION_NAME, -1);
    }
...
TEST_F(PortFileTest2, file_test4)
{
    ...
    if (0 == omrsysinfo_get_executable_name("java.properties", &knownFile)) {
        ...
        if (rc >= 0) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "omrfile_attr(%s) returned %d expected %d\n", localFilename, rc, -1);
        }
        ...
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "omrsysinfo_get_executable_name(\"java.properties\", &knownFile) failed\n");
    }
...
TEST_F(PortFileTest2, file_test5)
{
    ...
    if (-1 != fd) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "%s() returned valid file handle expected -1\n", FILE_OPEN_FUNCTION_NAME);
    }
...
TEST_F(PortFileTest2, file_test8)
{
    ...
    if (0 != prevFilePtr) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "File pointer location is wrong after opening file. Expected = 0, Got = %lld", prevFilePtr);
    }
    ...
    if (prevFilePtr + expectedReturnValue != filePtr) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "File pointer location is wrong after writing into file. Expected = %lld, Got = %lld",
    ...
    if (5 != prevFilePtr) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "File pointer location is wrong after opening the file. Expected = 5, Got = %lld", prevFilePtr);
    }
#else
    /* Other platforms set the file pointer to the start of file when it is opened in append mode and update the file
     * pointer to the end only when it is needed in the next call. */
    if (0 != prevFilePtr) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "File pointer location is wrong after opening the file. Expected = 0, Got = %lld", prevFilePtr);
    }
#endif /* defined(OMR_OS_WINDOWS) */
    ...
    if (10 != filePtr) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "File pointer location is wrong after writing into file. Expected = 10, Got = %lld", filePtr);
    }
    ...
    if (prevFilePtr + expectedReturnValue != filePtr) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "File pointer location is wrong after reading from file. Expected = %lld, Got = %lld",
            prevFilePtr + expectedReturnValue, filePtr);
    }
    ...
    for (i = 0; i < numberIterations; i++) {
        ...
        if (length > 2) {
            ...
            if (length != (savedLength - 1)) {
                outputErrorMessage(
                    PORTTEST_ERROR_ARGS, "omrfile_length() returned %lld expected %lld\n", length, savedLength - 1);
                goto exit;
            }
        ...
        if (-1 == lastmod) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "omrfile_lastmod() returned %d expected %d, iteration %d\n", lastmod, -1, i + 1);
            goto exit;
        }
...
TEST_F(PortFileTest2, file_test10)
{
    ...
    if (resultingOffset != expectedOffset) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "omrfile_seek() returned %lld expected %lld\n", resultingOffset, expectedOffset);
        goto exit;
    }
...
TEST_F(PortFileTest2, file_test13)
{
    ...
    if (0 != rc) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "omrfile_move() from %s to %s returned %d, expected %d\n", dir1, dir2, rc, 0);
        goto exit;
    }
...
TEST_F(PortFileTest2, file_test14)
{
     ...
     *
            ...
            if (-1 == handle) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrfile_findfirst() returned -1 expected a valid
     handle\n"); goto exit;
            }
...
TEST_F(PortFileTest2, file_test15)
{
    ...
    if (-1 == fd) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "%s() returned -1 expected valid file handle\n", FILE_OPEN_FUNCTION_NAME);
        goto exit;
    }
    ...
    for (i = 0; i < numberIterations; i++) {
        ...
        if (0 != rc) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "omrfile_sync() returned %d expected %d, iteration %d\n", rc, 0, i + 1);
            goto exit;
        }
 ...
 *
 */
intptr_t omrfile_create_file(OMRPortLibrary* portLibrary, const char* filename, int32_t openMode, const char* testName)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
    ...
    if (0 == openMode) {
        ...
        if (-1 == returnFd) {
            ...
            lastErrorNumber = omrerror_last_error_number();
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "Open of file %s in mode %d failed: lastErrorNumber=%d, lastErrorMessage=%s\n", filename, openMode,
...
TEST_F(PortFileTest2, file_test18)
{
    ...
    lockRc = FILE_LOCK_BYTES_FUNCTION(
        OMRPORTLIB, fd, OMRPORT_FILE_READ_LOCK | OMRPORT_FILE_NOWAIT_FOR_LOCK, offset, length);

    if (0 != lockRc) {
        lastErrorMessage = (char*)omrerror_last_error_message();
        lastErrorNumber = omrerror_last_error_number();
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "Locking of bytes %lld for %lld failed: lastErrorNumber=%d, lastErrorMessage=%s\n", offset, length,
            lastErrorNumber, lastErrorMessage);
        goto exit;
    }
    ...
    if (0 != unlockRc) {
        ...
        lastErrorNumber = omrerror_last_error_number();
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "Unlocking of bytes %lld for %lld failed: lastErrorNumber=%d, lastErrorMessage=%s\n", offset, length,
            lastErrorNumber, lastErrorMessage);
        goto exit;
    }
...
TEST_F(PortFileTest2, file_test19)
{
    ...
    lockRc = FILE_LOCK_BYTES_FUNCTION(
        OMRPORTLIB, fd, OMRPORT_FILE_WRITE_LOCK | OMRPORT_FILE_NOWAIT_FOR_LOCK, offset, length);
...
int omrfile_test20_child_2(OMRPortLibrary* portLibrary)
{
    ...
    if (-1 != lockRc) {
        ...
        lastErrorNumber = omrerror_last_error_number();
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "Locking of bytes %lld for %lld incorrectly succeeded: lastErrorNumber=%d, lastErrorMessage=%s\n", offset,
            length, lastErrorNumber, lastErrorMessage);
        goto exit;
    }
...
int omrfile_test22_child_2(OMRPortLibrary* portLibrary)
{
    ...
    lockRc = FILE_LOCK_BYTES_FUNCTION(
        OMRPORTLIB, fd, OMRPORT_FILE_WRITE_LOCK | OMRPORT_FILE_WAIT_FOR_LOCK, offset, length);
...
int omrfile_test23_child_2(OMRPortLibrary* portLibrary)
{
    ...
    lockRc
        = FILE_LOCK_BYTES_FUNCTION(OMRPORTLIB, fd, OMRPORT_FILE_READ_LOCK | OMRPORT_FILE_WAIT_FOR_LOCK, offset, length);
...
int omrfile_test25_child_1(OMRPortLibrary* portLibrary)
{
    ...
    if (0 != unlockRc) {
        ...
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "Unlocking of bytes %lld for %lld failed: lastErrorNumber=%d, lastErrorMessage=%s\n", (offset + 2),
            (length - 2), lastErrorNumber, lastErrorMessage);
        goto exit;
    }
    ...
    lockRc = FILE_LOCK_BYTES_FUNCTION(
        OMRPORTLIB, fd, OMRPORT_FILE_WRITE_LOCK | OMRPORT_FILE_WAIT_FOR_LOCK, (offset + 2), length);
...
TEST_F(PortFileTest2, file_test26)
{
    ...
    if (0 == omrsysinfo_get_executable_name("java.properties", &knownFile)) {
        ...
        if (rc != 0) {
            ...
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "omrfile_stat(%s, ..) set buf.isDir = %d expected 1\n", ".", buf.isDir);
        }
        ...
        if (rc != 0) {
            ...
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "omrfile_stat(%s, ..) set buf.isFile = %d expected 1\n", ".", buf.isFile);
        }
    } else {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "omrsysinfo_get_executable_name(\"java.properties\", &knownFile) failed\n");
...
#define nModes 22
     ...
     */
    int32_t testModes[nModes] = { 0, 1, 2, 4, 6, 7, 010, 020, 040, 070, 0100, 0200, 0400, 0700, 0666, 0777, 0755, 0644,
    ...
    if (-1 == fd1) {
        ...
        for (m = 0; m < nModes; ++m) {
            ...
            if (0 == (rc & ownerWritable)) {
                if (-1 != fd2) {
                    outputErrorMessage(
                        PORTTEST_ERROR_ARGS, "opened read-only file for writing fd=%d expected -1\n", rc);
                    break;
                }
            } else {
                if (-1 == fd2) {
            ...
            if (0 == (rc & ownerReadable)) {
                if (-1 != fd2) {
                    outputErrorMessage(
                        PORTTEST_ERROR_ARGS, "opened write-only file for reading fd=%d expected -1\n", rc);
                    break;
                }
            } else {
                if (-1 == fd2) {
    ...
    if (-1 != rc) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "omrfile_chmod() on non-existent file returned %d expected %d\n", rc, -1);
    }
...
TEST_F(PortFileTest2, file_test28)
{
    ...
    int32_t rc;
#define nModes 22
    int32_t testModes[nModes] = { 0, 1, 2, 4, 6, 7, 01010, 02020, 04040, 070, 0100, 0200, 0400, 0700, 0666, 0777, 0755,
        0644, 01644, 02755, 03600, 07777 };
    ...
    if (-1 == rc) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "%s() failed for %s: could not create\n", FILE_OPEN_FUNCTION_NAME, localDirectoryName);
    } else {
        for (m = 0; m < nModes; ++m) {
#if defined(AIXPPC) || defined(J9ZOS390)
            if (0 != (testModes[m] & J9S_ISGID)) {
...
TEST_F(PortFileTest2, file_test30)
{
    ...
    /* try to create the file again, this should fail. The only reason we do this is to have omrfile set the last error
     * code */
    existingFileFd
        = FILE_OPEN_FUNCTION(OMRPORTLIB, validFilename, EsOpenCreateNew | EsOpenWrite | EsOpenTruncate, 0666);
    if (-1 != existingFileFd) {
    ...
    invalidFileFd
        = FILE_OPEN_FUNCTION(OMRPORTLIB, illegalFileName, EsOpenCreateNew | EsOpenWrite | EsOpenTruncate, 0666);
    if (-1 == invalidFileFd) {
        int32_t lastErrorNumber = omrerror_last_error_number();

        if (lastErrorNumber == OMRPORT_ERROR_FILE_EXIST) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "omrerror_last_error_number(\"%s\") returned %i (OMRPORT_ERROR_FILE_EXIST), anything but would be fine "
                "%i\n",
        ...
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "%s(\"%s\") did not return failure.\n", FILE_OPEN_FUNCTION_NAME, illegalFileName);
    }

    /* try opening a "../" file, expect failure (read open) */
    invalidFileFd
        = FILE_OPEN_FUNCTION(OMRPORTLIB, illegalFileName, EsOpenCreateNew | EsOpenRead | EsOpenTruncate, 0666);
    if (-1 == invalidFileFd) {
        int32_t lastErrorNumber = omrerror_last_error_number();
...
TEST_F(PortFileTest2, file_test31)
{
    ...
    if (myGid != fileGid) {
        portTestEnv->log(
            "unexpected group ID.  Possibly setgid bit set. Comparing to group of the current directory\n");
        ...
        if (fileGid != stat.ownerGid) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "omrfile_stat.ownerGid is %d expected %d\n", fileGid, stat.ownerGid);
        }
    }
exit:
...
TEST_F(PortFileTest2, file_test33)
{
    ...
    omrfile_write_text(
        1, (const char*)three_byte_utf8, sizeof(char) * 4); /* Unicode string representation should be printed */
    ...
    omrfile_write_text(
        1, (const char*)many_byte_utf8, sizeof(char) * 512); /* Unicode string representation should be printed */
...
#define FILENAME_LENGTH \
    MIN_LENGTH * 2 /* double the size to accomodate a file size that is just under twice MIN_LENGTH */
    ...
    for (i = 0; i < 2; i++) {
        ...
        while (strlen(filePathName) < MIN_LENGTH) {
            ...
            omrstr_printf(
                filePathName + strlen(filePathName), FILENAME_LENGTH - strlen(filePathName), "%s", longDirName);
...
#define J9FILETEST_FILESIZE 1000 * 512
    ...
    if (buf.isRemote) {
        portTestEnv->log(
            LEVEL_WARN, "WARNING: Test omrfile_test34 cannot be run on a remote file system.\nSkipping test.\n");
 ...
 */
int omrfile_test34_multiple_tries(struct OMRPortLibrary* portLibrary, int nbOfTries)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
    int i, rc;
...
TEST_F(PortFileTest2, file_test35)
{
    ...
    if (0 != rc) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "%s() returned %lld expected %d\n", FILE_SET_LENGTH_FUNCTION_NAME, rc, 0);
        goto exit;
    }
    ...
    if (newfLength != fileLength) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "%s() returned %lld expected %lld\n", FILE_FLENGTH_FUNCTION_NAME,
            newfLength, fileLength);
        goto exit;
    }
    if (newfLength != newLength) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "%s() returned %lld expected %lld\n", FILE_FLENGTH_FUNCTION_NAME,
            newfLength, newLength);
        goto exit;
    }
...
TEST_F(PortFileTest2, file_test36)
{
    ...
    if (omrfileFD != nativeFD) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "omrfileFD != nativeFD, they should be the same. omrfileFD: %x, nativeFD: %x\n", omrfileFD, nativeFD);
    }
#else /* defined(J9ZOS390) */
    if (omrfileFD == nativeFD) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "omrfileFD == nativeFD, they should be different. omrfileFD: %x, nativeFD: %x\n", omrfileFD, nativeFD);
    }
    ...
    if (omrfileFD == OMRPORT_INVALID_FD) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9fileFD == J9PORT_INVALID_FD, they should be the same. j9fileFD: %x\n", omrfileFD);
    }
    if (omrfileFD != OMRPORT_TTY_OUT) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9fileFD != J9PORT_TTY_OUT, they should be the same. j9fileFD: %x\n", omrfileFD);
    }
...
TEST_F(PortFileTest2, file_test37)
{
    ...
    if (rc >= 0) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "Negative test failure: omrfile_fstat(-1, ..) on invalid file descriptor is expected to fail but it "
            "returned success with rc=%d (ignore the following LastErrorNumber and LastErrorMessage)\n",
            rc);
    }
    ...
    if (0 == omrsysinfo_get_executable_name(NULL, &knownFile)) {
        ...
        if (-1 != fd) {
            ...
            if (0 != rc) {
                outputErrorMessage(
                    PORTTEST_ERROR_ARGS, "omrfile_fstat(...) failed for fd=%d(filename=%s)\n", fd, knownFile);
            ...
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "%s() failed to open file %s\n", FILE_OPEN_FUNCTION_NAME, knownFile);
        }
    } else {
    ...
    if (-1 != fd) {
        ...
        if (0 != rc) {
            ...
            if (myGid != fileGid) {
                ...
                if (0 != rc) {
                    outputErrorMessage(
                        PORTTEST_ERROR_ARGS, "omrfile_stat(...) failed for current directory with rc=%d\n", rc);
                } else {
                    if (fileGid != stat.ownerGid) {
                        outputErrorMessage(PORTTEST_ERROR_ARGS,
                            "omrfile_fstat(...) for current directory returned ownerGid as %d, expected %d\n", fileGid,
                            stat.ownerGid);
                    }
                }
            }
...
TEST_F(PortFileTest2, file_test39)
{
    ...
    if (-1 != fd) {
        ...
        if (0 != rc) {
            ...
            /*umask vary so we can't test 'isGroupWriteable', 'isGroupReadable', 'isOtherWriteable', or
             * 'isOtherReadable'*/
        }
...
TEST_F(PortFileTest2, file_test40)
{
    ...
    if (prevFilePtr != filePtr) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "%s() Changed the location of file pointer. Expected : %lld. Got : %lld \n", FILE_SET_LENGTH_FUNCTION_NAME,
            prevFilePtr, filePtr);
 ...
 */
int32_t omrfile_runTests(struct OMRPortLibrary* portLibrary, char* argv0, char* omrfile_child, BOOLEAN async)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
    isAsync = async;
