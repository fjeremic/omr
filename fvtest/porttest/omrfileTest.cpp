...
TEST_F(PortFileTest2, file_test31) {
  ...
  if (myGid != fileGid) {
    portTestEnv->log(
        "unexpected group ID.  Possibly setgid bit set. Comparing to group of "
...
#define J9FILETEST_FILESIZE 1000 * 512
  ...
  if (buf.isRemote) {
    portTestEnv->log(LEVEL_WARN,
                     "WARNING: Test omrfile_test34 cannot be run on a remote "
                     "file system.\nSkipping test.\n");
...
TEST_F(PortFileTest2, file_test38) {
  ...
  if (-1 != fd) {
    ...
    if (0 != rc) {
      ...
      if (myGid != fileGid) {
        portTestEnv->log(
            "Unexpected group ID. Possibly setgid bit set. Comparing to group "
