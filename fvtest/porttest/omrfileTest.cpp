 ...
 *
 * @note port library file system operations are not optional in the port
 * library table.
 * @note Some file system operations are only available if the standard
 * configuration of the port library is used.
 *
 */
#include <stdio.h>
#include <stdlib.h>
...
#define FILE_OPEN_FUNCTION_NAME                                                \
  (isAsync ? "omrfile_blockingasync_open" : "omrfile_open")
#define FILE_CLOSE_FUNCTION_NAME                                               \
  (isAsync ? "omrfile_blockingasync_close" : "omrfile_close")
#define FILE_LOCK_BYTES_FUNCTION_NAME                                          \
  (isAsync ? "omrfile_blockingasync_lock_bytes" : "omrfile_lock_bytes")
#define FILE_UNLOCK_BYTES_FUNCTION_NAME                                        \
  (isAsync ? "omrfile_blockingasync_unlock_bytes" : "omrfile_unlock_bytes")
#define FILE_READ_FUNCTION_NAME                                                \
  (isAsync ? "omrfile_blockingasync_read" : "omrfile_read")
#define FILE_WRITE_FUNCTION_NAME                                               \
  (isAsync ? "omrfile_blockingasync_write" : "omrfile_write")
#define FILE_SET_LENGTH_FUNCTION_NAME                                          \
  (isAsync ? "omrfile_blockingasync_set_length" : "omrfile_set_length")
#define FILE_FLENGTH_FUNCTION_NAME                                             \
  (isAsync ? "omrfile_blockingasync_flength" : "omrfile_flength")
 ...
 */
static void fileWrite(struct OMRPortLibrary *portLibrary, intptr_t fd,
                      const char *format, ...) {
...
intptr_t omrfile_open_helper(struct OMRPortLibrary *portLibrary,
                             const char *path, int32_t flags, int32_t mode) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  return isAsync ? omrfile_blockingasync_open(path, flags, mode)
                 : omrfile_open(path, flags, mode);
}

int32_t omrfile_close_helper(struct OMRPortLibrary *portLibrary, intptr_t fd) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
int32_t omrfile_lock_bytes_helper(struct OMRPortLibrary *portLibrary,
  ...
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  return isAsync
             ? omrfile_blockingasync_lock_bytes(fd, lockFlags, offset, length)
             : omrfile_lock_bytes(fd, lockFlags, offset, length);
}
...
int32_t omrfile_unlock_bytes_helper(struct OMRPortLibrary *portLibrary,
                                    ...
                                    uint64_t length) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  return isAsync ? omrfile_blockingasync_unlock_bytes(fd, offset, length)
                 : omrfile_unlock_bytes(fd, offset, length);
}

intptr_t omrfile_read_helper(struct OMRPortLibrary *portLibrary, intptr_t fd,
                             void *buf, intptr_t nbytes) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  return isAsync ? omrfile_blockingasync_read(fd, buf, nbytes)
                 : omrfile_read(fd, buf, nbytes);
}

intptr_t omrfile_write_helper(struct OMRPortLibrary *portLibrary, intptr_t fd,
                              void *buf, intptr_t nbytes) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  return isAsync ? omrfile_blockingasync_write(fd, buf, nbytes)
                 : omrfile_write(fd, buf, nbytes);
}

int32_t omrfile_set_length_helper(struct OMRPortLibrary *portLibrary,
                                  intptr_t fd, I_64 newLength) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  return isAsync ? omrfile_blockingasync_set_length(fd, newLength)
                 : omrfile_set_length(fd, newLength);
}

I_64 omrfile_flength_helper(struct OMRPortLibrary *portLibrary, intptr_t fd) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
TEST_F(PortFileTest2, file_test0) {
  ...
  if (FALSE == isAsync) {
    ...
    if (NULL == OMRPORTLIB->file_startup) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "portLibrary->file_startup is NULL\n");
    }

    /* Not tested, implementation dependent.  No known functionality */
    if (NULL == OMRPORTLIB->file_shutdown) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "portLibrary->file_shutdown is NULL\n");
    }
    ...
    if (NULL == OMRPORTLIB->file_blockingasync_write) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "portLibrary->file_blockingasync_write is NULL\n");
    }

    /* omrfile_test7, omrfile_test8 */
    if (NULL == OMRPORTLIB->file_blockingasync_read) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "portLibrary->file_blockingasync_read is NULL\n");
    }

    /* omrfile_test5, omrfile_test6 */
    if (NULL == OMRPORTLIB->file_blockingasync_open) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "portLibrary->file_blockingasync_open is NULL\n");
    }

    /* omrfile_test5, omrfile_test6 */
    if (NULL == OMRPORTLIB->file_blockingasync_close) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "portLibrary->file_blockingasync_close is NULL\n");
    }

    if (NULL == OMRPORTLIB->file_blockingasync_lock_bytes) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "portLibrary->file_blockingasync_lock_bytes is NULL\n");
    }

    if (NULL == OMRPORTLIB->file_blockingasync_unlock_bytes) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "portLibrary->file_blockingasync_unlock_bytes is NULL\n");
    }
    ...
    if (NULL == OMRPORTLIB->file_blockingasync_set_length) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "portLibrary->file_blockingasync_set_length is NULL\n");
    }

    if (NULL == OMRPORTLIB->file_blockingasync_flength) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "portLibrary->file_blockingasync_flength is NULL\n");
    }
    ...
    if (NULL == OMRPORTLIB->file_blockingasync_startup) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "portLibrary->file_blockingasync_startup is NULL\n");
    }

    /* Not tested, implementation dependent.  No known functionality */
    if (NULL == OMRPORTLIB->file_blockingasync_shutdown) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "portLibrary->file_blockingasync_shutdown is NULL\n");
    }
  }
  ...
  if (NULL == OMRPORTLIB->file_write_text) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_write_text is NULL\n");
  }

  if (NULL == OMRPORTLIB->file_get_text_encoding) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_get_text_encoding is NULL\n");
  }

  /* omrfile_test3 */
  if (NULL == OMRPORTLIB->file_vprintf) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_vprintf is NULL\n");
  }

  /* omrfile_test17 */
  if (NULL == OMRPORTLIB->file_printf) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_printf is NULL\n");
  }
  ...
  if (NULL == OMRPORTLIB->file_unlink) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_unlink is NULL\n");
  }
  ...
  if (NULL == OMRPORTLIB->file_chmod) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_chmod is NULL\n");
  }

  if (NULL == OMRPORTLIB->file_chown) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_chown is NULL\n");
  }

  /* omrfile_test9 */
  if (NULL == OMRPORTLIB->file_lastmod) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_lastmod is NULL\n");
  }

  /* omrfile_test9 */
  if (NULL == OMRPORTLIB->file_length) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_length is NULL\n");
  }
  ...
  if (NULL == OMRPORTLIB->file_unlinkdir) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_unlinkdir is NULL\n");
  }

  /* omrfile_test14 */
  if (NULL == OMRPORTLIB->file_findfirst) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_findfirst is NULL\n");
  }

  /* omrfile_test14 */
  if (NULL == OMRPORTLIB->file_findnext) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_findnext is NULL\n");
  }

  /* omrfile_test14 */
  if (NULL == OMRPORTLIB->file_findclose) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_findclose is NULL\n");
  }

  /* omrfile_test16 */
  if (NULL == OMRPORTLIB->file_error_message) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->file_error_message is NULL\n");
  }

  if (NULL == OMRPORTLIB->file_convert_native_fd_to_omrfile_fd) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "portLibrary->file_convert_native_fd_to_omrfile_fd is NULL\n");
  }
...
TEST_F(PortFileTest2, file_test3) {
  ...
  char expectedResult[] =
      "Try a number - 1Try a string - abcTry a mixture - 1 abc 2";
  char readBuf[255];
  char *readBufPtr;
  intptr_t fd1;
  ...
  fd1 = FILE_OPEN_FUNCTION(OMRPORTLIB, file1name, EsOpenCreate | EsOpenWrite,
                           0666);
  ...
  /* having hopefully shoved this stuff onto file we need to read it back and
  ...
  if (NULL == readBufPtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_read_text() returned NULL\n");
    goto exit;
  }

  if (strlen(readBufPtr) != strlen(expectedResult)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_read_text() returned %d expected %d\n",
  ...
  if (strcmp(readBufPtr, expectedResult)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_read_text() returned \"%s\" expected \"%s\"\n",
...
/**
 */
void openFlagTest(struct OMRPortLibrary *portLibrary, const char *testName,
                  const char *pathName, int32_t flags, int32_t mode) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  intptr_t fd;
  J9FileStat buf;
  int32_t rc;
  ...
  if (rc >= 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() returned %d, expected %d\n", rc, -1);
  }
  ...
  if (-1 == fd) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() returned %d expected valid file handle\n",
  ...
  if (rc != 0) {
    ...
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() set buf.isFile = %d, expected %d\n",
                       buf.isFile, 1);
  }
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_unlink() returned %d expected %d\n", rc, -1);
  }
}
...
TEST_F(PortFileTest2, file_test4) {
  ...
  if (0 == omrsysinfo_get_executable_name("java.properties", &knownFile)) {
    ...
    if (rc >= 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrfile_attr(NULL) returned %d expected %d\n", rc,
                         -1);
    }
    ...
    if (rc >= 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrfile_attr(%s) returned %d expected %d\n",
                         localFilename, rc, -1);
    }
    ...
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrsysinfo_get_executable_name(\"java.properties\", "
                       "&knownFile) failed\n");
  }
...
TEST_F(PortFileTest2, file_test5) {
  ...
  if (rc >= 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat(\"%s\") returned %d expected %d\n",
                       dirName, rc, -1);
  }
  ...
  /* try opening a "new" file, expect success. Requires one of read/write
   * attributes too */
  fd = FILE_OPEN_FUNCTION(OMRPORTLIB, fileName,
                          EsOpenCreate | EsOpenRead | EsOpenWrite, 0666);
  if (-1 == fd) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s(\"%s\") returned %d expected valid file handle\n",
  ...
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_mkdir(\"%s\") returned %d expected %d\n",
                       dirName, rc, -1);
  }
  ...
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_unlinkdir(\"%s\") returned %d expected %d\n",
                       dirName, rc, -1);
  }
  ...
  if (-1 != fd) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() returned valid file handle expected -1\n",
                       FILE_OPEN_FUNCTION_NAME);
  }
 ...
 * Verify port file system.
 *
 * Open files in the various @ref omrfile.c::PortFileOperations "modes"
 * supported by the port library file operations.  The two functions
...
TEST_F(PortFileTest2, file_test6) {
  ...
  fd1 = FILE_OPEN_FUNCTION(OMRPORTLIB, path1,
                           EsOpenCreateNew | EsOpenRead | EsOpenWrite, 0666);
  if (-1 == fd1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() returned %d expected file handle\n",
                       FILE_OPEN_FUNCTION_NAME, -1);
    goto exit;
  }
  /* Verify the filesystem sees it */
  ...
  if (EsIsFile != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_attr() returned %d expected %d\n", rc,
                       EsIsFile);
  }
  ...
  /*
   * Testing EsOpenCreateNew Flag - if the file already exist (for which path1
   * is) and we specified EsOpenCreateNew, it should failed and return error
   * number OMRPORT_ERROR_FILE_EXIST
   */
  fd1 = FILE_OPEN_FUNCTION(OMRPORTLIB, path1, EsOpenCreateNew | EsOpenWrite,
                           0666);
  if (-1 != fd1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() returned valid file handle expected -1\n",
  ...
  /* TODO once all platforms are ready
          if(rc != OMRPORT_ERROR_FILE_EXIST) {
                          outputErrorMessage(PORTTEST_ERROR_ARGS,
     "omrfile_open() returned %d expected %d\n", rc, OMRPORT_ERROR_FILE_EXIST);
          }
  */
  ...
  /* TODO
   * Different behaviour on windows/UNIX.  Which is right?  Unix states must be
   one of read/write/execute
   *
          fd2 = omrfile_open(path2, EsOpenCreate, 0444);
          omrfile_close(fd2);
          if (-1 != fd2) {
                  outputErrorMessage(PORTTEST_ERROR_ARGS, "omrfile_open()
   returned valid file handle expected -1\n");
          }
   */
  ...
  /*
          openFlagTest(OMRPORTLIB, testName, path3, EsOpenTruncate |
     EsOpenCreate);
  */

  /* flags = EsOpenAppend, needs read/write and create */
  openFlagTest(OMRPORTLIB, testName, path3,
               EsOpenAppend | EsOpenCreate | EsOpenWrite, 0666);

  /* flags = EsOpenText */
  openFlagTest(OMRPORTLIB, testName, path3,
               EsOpenText | EsOpenCreate | EsOpenWrite, 0666);
...
TEST_F(PortFileTest2, file_test7) {
  ...
  fd = FILE_OPEN_FUNCTION(OMRPORTLIB, fileName, EsOpenWrite | EsOpenCreate,
                          0666);
  if (-1 == fd) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() returned %d expected valid file handle\n",
 ...
 * append operations.
 *
 * This test creates a file, writes ABCDE to it then closes it - reopens it for
 * append and writes FGHIJ to it, fileseeks to start of file and reads 10 chars
 * and hopefully this matches ABCDEFGHIJ ....
 */
TEST_F(PortFileTest2, file_test8) {
  ...
  fd = FILE_OPEN_FUNCTION(OMRPORTLIB, fileName,
                          EsOpenWrite | EsOpenCreate | EsOpenTruncate, 0666);
  if (-1 == fd) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() returned %d expected valid file handle\n",
                       FILE_OPEN_FUNCTION_NAME, -1);
  ...
  if (0 != prevFilePtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "File pointer location is wrong after opening file. "
                       "Expected = 0, Got = %lld",
                       prevFilePtr);
  }
  ...
  if (prevFilePtr + expectedReturnValue != filePtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "File pointer location is wrong after writing into "
                       "file. Expected = %lld, Got = %lld",
                       prevFilePtr + expectedReturnValue, filePtr);
  }
  ...
  fd = FILE_OPEN_FUNCTION(OMRPORTLIB, fileName, EsOpenWrite | EsOpenAppend,
                          0666);

  prevFilePtr = omrfile_seek(fd, 0, EsSeekCur);
#if defined(OMR_OS_WINDOWS)
  /* Windows sets the file pointer to the end of file as soon as it is opened in
   * append mode. */
  if (5 != prevFilePtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "File pointer location is wrong after opening the file. "
                       "Expected = 5, Got = %lld",
                       prevFilePtr);
  }
#else
  /* Other platforms set the file pointer to the start of file when it is opened
   * in append mode and update the file pointer to the end only when it is
   * needed in the next call. */
  if (0 != prevFilePtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "File pointer location is wrong after opening the file. "
                       "Expected = 0, Got = %lld",
  ...
  if (prevFilePtr + expectedReturnValue != filePtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "File pointer location is wrong after reading from "
                       "file. Expected = %lld, Got = %lld",
                       prevFilePtr + expectedReturnValue, filePtr);
  }
  ...
  if (strcmp(expectedOutput, outputLine)) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "Read back data \"%s\" not matching expected data \"%s\"\n", outputLine,
        expectedOutput);
    goto exit;
  }
...
TEST_F(PortFileTest2, file_test9) {
  ...
  for (i = 0; i < numberIterations; i++) {
    ...
    if (i != 0) {
      if (-1 == lastmod) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "omrfile_lastmod() returned -1, iteration %d\n",
                           i + 1);
        goto exit;
      }
    } else {
    ...
    fd = FILE_OPEN_FUNCTION(OMRPORTLIB, fileName,
                            EsOpenCreate | EsOpenWrite | EsOpenAppend, 0666);
    if (-1 == fd) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "%s() returned -1 expected valid file handle iteration %d\n",
    ...
    if (length != i) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrfile_length() returned %lld expected %d\n", length,
                         i);
      goto exit;
    }
    ...
    if (length > 2) {
      ...
      if (length != (savedLength - 1)) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "omrfile_length() returned %lld expected %lld\n",
    ...
    if (1 != rc) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "%s() returned %d expected 1, iteration %d\n",
    ...
    if (0 != rc) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "%s() returned %d expected %d, iteration %d\n",
    ...
    if (-1 == lastmod) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "omrfile_lastmod() returned %d expected %d, iteration %d\n", lastmod,
          -1, i + 1);
      goto exit;
    }
  ...
  if (numberIterations == (timesEqual + timesLess)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_lastmod() failed time did not advance\n");
  }
...
TEST_F(PortFileTest2, file_test10) {
  ...
  char inputLine1[] = "01234567890123456789012345678901234567890123456789012345"
  ...
  if (0 != fileLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_length() returned %lld expected 0\n",
                       fileLength);
    goto exit;
  }
  ...
  if (resultingOffset != expectedOffset) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_seek() returned %lld expected %lld\n",
  ...
  /* ok now lets write something - should overwrite total file length should
   * remain 81 */
   ...
   */
  rc = FILE_WRITE_FUNCTION(OMRPORTLIB, fd, (char *)"X",
                           1); /* Note this should just do 1 character X*/
  if (1 != rc) {
  ...
  if (-1 != resultingOffset) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_seek() returned %lld expected -1\n",
                       resultingOffset);
    goto exit;
  }
  ...
  if (40 != resultingOffset) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_seek() returned %lld expected 40\n",
                       resultingOffset);
    goto exit;
  }
...
TEST_F(PortFileTest2, file_test11) {
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_mkdir() returned %d expected %d\n", rc, 0);
  }
  ...
  if (rc != 0) {
    ...
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() set buf.isDir = %d, expected %d\n",
                       buf.isDir, 1);
  }
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_unlinkdir() returned %d expected 0\n", rc);
    goto exit;
  }

  /* OK basic test worked
   * Try putting a file in the directory (and using DIR_SEPARATOR_STR)
   * first remove the directory in case it is there, don't check for
   * failure/success
   */
  ...
  if (0 == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_unlinkdir() returned %d expected %d\n", rc, -1);
    goto exit;
  }
...
TEST_F(PortFileTest2, file_test12) {
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_mkdir() returned %d expected 0\n", rc);
    goto exit;
  }

  /* create some files within directory */
  fd1 = FILE_OPEN_FUNCTION(OMRPORTLIB, pathName1, EsOpenCreate | EsOpenWrite,
                           0666);
  if (-1 == fd1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "%s() failed\n",
                       FILE_OPEN_FUNCTION_NAME);
  ...
  fd2 = FILE_OPEN_FUNCTION(OMRPORTLIB, pathName2, EsOpenCreate | EsOpenWrite,
                           0666);
  if (-1 == fd2) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "%s() failed\n",
                       FILE_OPEN_FUNCTION_NAME);
  ...
  if (-1 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_unlink() returned %d expected -1\n", rc);
    goto exit;
  }
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_unlink() returned %d expected 0\n", rc);
    goto exit;
  }
  ...
  if (0 == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_unlinkdir() returned %d expected -1\n", rc);
    goto exit;
  }
...
TEST_F(PortFileTest2, file_test13) {
  ...
  if (0 != rc) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrfile_move() from %s to %s returned %d, expected %d\n", dir1, dir2,
        rc, 0);
    goto exit;
  }
...
TEST_F(PortFileTest2, file_test14) {
  ...
  fd1 = FILE_OPEN_FUNCTION(OMRPORTLIB, pathName1, EsOpenWrite | EsOpenCreate,
                           0666);
  if (-1 == fd1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() returned %d expected valid file handle\n",
  ...
  /* try opening a "new" file - if file exists then should still work */
  fd2 = FILE_OPEN_FUNCTION(OMRPORTLIB, pathName2, EsOpenWrite | EsOpenCreate,
                           0666);
  if (-1 == fd2) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() returned %d expected valid file handle\n",
  ...
  rc =
      FILE_WRITE_FUNCTION(OMRPORTLIB, fd1, (char *)"test", expectedReturnValue);
  if (rc != expectedReturnValue) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "%s() returned %d expected %d\n",
                       FILE_WRITE_FUNCTION_NAME, rc, expectedReturnValue);
    goto exit;
  ...
  rc =
      FILE_WRITE_FUNCTION(OMRPORTLIB, fd2, (char *)"test", expectedReturnValue);
  if (rc != expectedReturnValue) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "%s() returned %d expected %d\n",
                       FILE_WRITE_FUNCTION_NAME, rc, expectedReturnValue);
    goto exit;
  ...
  if ((uintptr_t)-1 != handle) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrfile_findfirst() returned valid handle expected -1\n");
    goto exit;
  }
   ...
   *
          ...
          if (-1 == handle) {
                  outputErrorMessage(PORTTEST_ERROR_ARGS, "omrfile_findfirst()
   returned -1 expected a valid handle\n"); goto exit;
          }
          ...
          if (fileCount != expectedFileCount)	{
                  outputErrorMessage(PORTTEST_ERROR_ARGS, "file count
   incorrrect, found %d expected %d\n", fileCount, expectedFileCount); goto
   exit;
          }

          if (dirCount != expectedDirCount) {
                  outputErrorMessage(PORTTEST_ERROR_ARGS, "directory count
   incorrect, found %d expected %d\n", dirCount, expectedDirCount); goto exit;
          }

          if (count != expectedCount) {
                  outputErrorMessage(PORTTEST_ERROR_ARGS, "count incorrect,
   found %d expected %d\n", count, expectedCount); goto exit;
          }
...
TEST_F(PortFileTest2, file_test15) {
  ...
  if (-1 == fd) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() returned -1 expected valid file handle\n",
                       FILE_OPEN_FUNCTION_NAME);
    goto exit;
  }
  ...
  for (i = 0; i < numberIterations; i++) {
    ...
    if (0 != rc) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "omrfile_sync() returned %d expected %d, iteration %d\n", rc, 0,
          i + 1);
      goto exit;
    }
 ...
 *
 */
intptr_t omrfile_create_file(OMRPortLibrary *portLibrary, const char *filename,
                             int32_t openMode, const char *testName) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  createFd =
      omrfile_open(filename, EsOpenCreateNew | EsOpenRead | EsOpenWrite, 0660);
  if (-1 == createFd) {
  ...
  if (0 == openMode) {
    ...
    if (-1 == returnFd) {
      ...
      lastErrorNumber = omrerror_last_error_number();
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Open of file %s in mode %d failed: "
 ...
 *
 */
intptr_t omrfile_create_status_file(OMRPortLibrary *portLibrary,
                                    const char *filename,
                                    const char *testName) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
 ...
 */
intptr_t omrfile_status_file_exists(OMRPortLibrary *portLibrary,
                                    const char *filename,
                                    const char *testName) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  int32_t rc;
...
TEST_F(PortFileTest2, file_test18) {
  ...
  if (-1 == fd) {
    /* No need to print error messages, omrfile_create_file will already have
     * done so */
    goto exit;
  }

  lockRc = FILE_LOCK_BYTES_FUNCTION(
      OMRPORTLIB, fd, OMRPORT_FILE_READ_LOCK | OMRPORT_FILE_NOWAIT_FOR_LOCK,
      offset, length);
  ...
  if (0 != lockRc) {
    ...
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Locking of bytes %lld for %lld failed: "
                       "lastErrorNumber=%d, lastErrorMessage=%s\n",
  ...
  if (0 != unlockRc) {
    ...
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Unlocking of bytes %lld for %lld failed: "
                       "lastErrorNumber=%d, lastErrorMessage=%s\n",
                       offset, length, lastErrorNumber, lastErrorMessage);
    goto exit;
  }
...
TEST_F(PortFileTest2, file_test19) {
  ...
  lockRc = FILE_LOCK_BYTES_FUNCTION(
      OMRPORTLIB, fd, OMRPORT_FILE_WRITE_LOCK | OMRPORT_FILE_NOWAIT_FOR_LOCK,
      offset, length);
...
TEST_F(PortFileTest2, file_test20) {
  ...
  pid1 = launchChildProcess(OMRPORTLIB, testName, argv0,
                            "-child_omrfile_test20_1");
  if (NULL == pid1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "launchChildProcess() failed for pid1!\n");
    goto exit;
  }
  pid2 = launchChildProcess(OMRPORTLIB, testName, argv0,
                            "-child_omrfile_test20_2");
  if (NULL == pid2) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "launchChildProcess() failed for pid2!\n");
    goto exit;
  }
  ...
  if (termstat1 != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Child process 1 exited with status %i: Expected 0.\n",
                       termstat1);
  }
  if (termstat2 != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Child process 2 exited with status %i: Expected 0.\n",
                       termstat2);
  }
...
int omrfile_test20_child_1(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
int omrfile_test20_child_2(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  if (-1 != lockRc) {
    ...
    lastErrorNumber = omrerror_last_error_number();
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Locking of bytes %lld for %lld incorrectly succeeded: "
                       "lastErrorNumber=%d, lastErrorMessage=%s\n",
                       offset, length, lastErrorNumber, lastErrorMessage);
    goto exit;
  }
...
int omrfile_test21_child_1(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
int omrfile_test21_child_2(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
int omrfile_test22_child_1(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
int omrfile_test22_child_2(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  lockRc = FILE_LOCK_BYTES_FUNCTION(
      OMRPORTLIB, fd, OMRPORT_FILE_WRITE_LOCK | OMRPORT_FILE_WAIT_FOR_LOCK,
      offset, length);
...
int omrfile_test23_child_1(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
int omrfile_test23_child_2(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  lockRc = FILE_LOCK_BYTES_FUNCTION(
      OMRPORTLIB, fd, OMRPORT_FILE_READ_LOCK | OMRPORT_FILE_WAIT_FOR_LOCK,
      offset, length);
...
int omrfile_test24_child_1(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
int omrfile_test24_child_2(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
int omrfile_test25_child_1(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  if (0 != unlockRc) {
    ...
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Unlocking of bytes %lld for %lld failed: "
                       "lastErrorNumber=%d, lastErrorMessage=%s\n",
                       (offset + 2), (length - 2), lastErrorNumber,
                       lastErrorMessage);
    goto exit;
  }
...
int omrfile_test25_child_2(OMRPortLibrary *portLibrary) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
TEST_F(PortFileTest2, file_test26) {
  ...
  if (0 == omrsysinfo_get_executable_name("java.properties", &knownFile)) {
    ...
    if (rc >= 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrfile_stat(NULL, ..) returned %d expected %d\n", rc,
                         -1);
    }
    ...
    if (rc != 0) {
      ...
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrfile_stat(%s, ..) set buf.isDir = %d expected 1\n",
                         ".", buf.isDir);
    }
    ...
    if (rc != 0) {
      ...
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "omrfile_stat(%s, ..) set buf.isFile = %d expected 1\n", ".",
          buf.isFile);
    }
  } else {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
...
TEST_F(PortFileTest2, file_test27) {
  ...
  int32_t rc;
#define nModes 22
  /* do not attempt to set sticky bit (01000) on files on AIX: will fail if file
   * is on a locally-mounted filesystem On OSX, when chmod() returns EPERM, it
   * is called again with bits S_ISGID and S_ISUID ignored and returns success.
   */
  int32_t testModes[nModes] = {
      0,    1,    2,    4,    6,    7,    010,  020,   040,   070,   0100,
      0200, 0400, 0700, 0666, 0777, 0755, 0644, 02644, 04755, 06600, 04777};

  reportTestEntry(OMRPORTLIB, testName);

  omrfile_chmod(localFilename, 0777);
  omrfile_unlink(localFilename);
  fd1 = FILE_OPEN_FUNCTION(OMRPORTLIB, localFilename,
                           EsOpenCreate | EsOpenWrite, 0666);
  if (-1 == fd1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "%s() failed\n",
                       FILE_OPEN_FUNCTION_NAME);
  } else {
    ...
    for (m = 0; m < nModes; ++m) {
      ...
      if (expectedMode != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "omrfile_chmod() returned %d expected %d\n", rc,
                           expectedMode);
        break;
      }
      ...
      if (0 == (rc & ownerWritable)) {
        if (-1 != fd2) {
          outputErrorMessage(
              PORTTEST_ERROR_ARGS,
              "opened read-only file for writing fd=%d expected -1\n", rc);
          break;
        }
      } else {
        if (-1 == fd2) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "opened owner-writable file for writing failed");
          break;
        }
      }
      FILE_CLOSE_FUNCTION(OMRPORTLIB, fd2);
      ...
      if (0 == (rc & ownerReadable)) {
        if (-1 != fd2) {
          outputErrorMessage(
              PORTTEST_ERROR_ARGS,
              "opened write-only file for reading fd=%d expected -1\n", rc);
          break;
        }
      } else {
        if (-1 == fd2) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "opened owner-readable file for reading failed");
          break;
        }
      }
      FILE_CLOSE_FUNCTION(OMRPORTLIB, fd2);
  ...
  if (-1 != rc) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrfile_chmod() on non-existent file returned %d expected %d\n", rc,
        -1);
  }
...
#define nModes 22
  int32_t testModes[nModes] = {
      0,    1,    2,    4,    6,    7,    01010, 02020, 04040, 070,   0100,
      0200, 0400, 0700, 0666, 0777, 0755, 0644,  01644, 02755, 03600, 07777};
  ...
  if (-1 == rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() failed for %s: could not create\n",
...
TEST_F(PortFileTest2, file_test29) {
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_chown() change group returned %d expected %d\n",
                       rc, 0);
  }
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_chown()set uid returned %d expected %d\n", rc,
                       0);
  }
  ...
  if (0 == rc) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrfile_chown() to root succeeded, should have failed\n");
  }
  omrfile_unlink(testFile);
#endif /* !defined(OMR_OS_WINDOWS) */
...
/**
 *
 * Verify @ref omrfile.c::omrfile_open returns OMRPORT_ERROR_FILE_ISDIR if we
 * are trying to create a directory.
 */
TEST_F(PortFileTest2, file_test30) {
  ...
  /* 1. First we want to trigger an error so that we know omrerror_last_error()
   * has been set Do this by trying to create the same file twice
  ...
  validFileFd =
      FILE_OPEN_FUNCTION(OMRPORTLIB, validFilename,
                         EsOpenCreateNew | EsOpenWrite | EsOpenTruncate, 0666);
  if (-1 == validFileFd) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "%s failed to create file %s: error_last_error_number %i\n",
        FILE_OPEN_FUNCTION_NAME, validFilename, omrerror_last_error_number());
    goto exit;
  }

  /* try to create the file again, this should fail. The only reason we do this
   * is to have omrfile set the last error code */
  existingFileFd =
      FILE_OPEN_FUNCTION(OMRPORTLIB, validFilename,
                         EsOpenCreateNew | EsOpenWrite | EsOpenTruncate, 0666);
  if (-1 != existingFileFd) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s should have failed to create existing file: %s\n",
  ...
  invalidFileFd =
      FILE_OPEN_FUNCTION(OMRPORTLIB, illegalFileName,
                         EsOpenCreateNew | EsOpenWrite | EsOpenTruncate, 0666);
  if (-1 == invalidFileFd) {
    ...
    if (lastErrorNumber == OMRPORT_ERROR_FILE_EXIST) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "omrerror_last_error_number(\"%s\") returned %i "
          "(OMRPORT_ERROR_FILE_EXIST), anything but would be fine %i\n",
          illegalFileName, lastErrorNumber);
    }

  } else {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s(\"%s\") did not return failure.\n",
  ...
  invalidFileFd =
      FILE_OPEN_FUNCTION(OMRPORTLIB, illegalFileName,
                         EsOpenCreateNew | EsOpenRead | EsOpenTruncate, 0666);
  if (-1 == invalidFileFd) {
    int32_t lastErrorNumber = omrerror_last_error_number();
...
TEST_F(PortFileTest2, file_test31) {
  ...
  fileFd =
      FILE_OPEN_FUNCTION(OMRPORTLIB, filename,
                         EsOpenCreateNew | EsOpenWrite | EsOpenTruncate, 0666);
  if (-1 == fileFd) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "%s failed to create file %s: error_last_error_number %i\n",
        FILE_OPEN_FUNCTION_NAME, filename, omrerror_last_error_number());
    goto exit;
  }
  ...
  if (myUid != stat.ownerUid) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat.ownerUid is %d expected %d\n",
                       stat.ownerUid, myUid);
  }

  if (myGid != fileGid) {
    portTestEnv->log("unexpected group ID.  Possibly setgid bit set. Comparing "
                     "to group of the current directory\n");
    ...
    if (fileGid != stat.ownerGid) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrfile_stat.ownerGid is %d expected %d\n", fileGid,
                         stat.ownerGid);
    }
  }
exit:
...
TEST_F(PortFileTest2, file_test32) {
  OMRPORT_ACCESS_FROM_OMRPORT(portTestEnv->getPortLibrary());
  const char *testName = APPEND_ASYNC(
      omrfile_test32_test_file_permission_bits_using_omrfile_stat());
  ...
  omrfile_unlink(localFilename);
  fd = FILE_OPEN_FUNCTION(OMRPORTLIB, localFilename, EsOpenCreate | EsOpenWrite,
                          0666);
  if (fd == -1) {
  ...
  if (stat.perm.isUserWriteable != 1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isUserWriteable != 1'\n");
  }
  if (stat.perm.isUserReadable != 1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isUserReadable != 1'\n");
  }
#if defined(OMR_OS_WINDOWS)
  if (stat.perm.isGroupWriteable != 1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isGroupWriteable != 1'\n");
  }
  if (stat.perm.isGroupReadable != 1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isGroupReadable != 1'\n");
  }
  if (stat.perm.isOtherWriteable != 1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isOtherWriteable != 1'\n");
  }
  if (stat.perm.isOtherReadable != 1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isOtherReadable != 1'\n");
  }
#else
  /*umask vary so we can't test 'isGroupWriteable', 'isGroupReadable',
   * 'isOtherWriteable', or 'isOtherReadable'*/
#endif /* defined(OMR_OS_WINDOWS) */
  ...
  omrfile_unlink(localFilename);
  fd = FILE_OPEN_FUNCTION(OMRPORTLIB, localFilename, EsOpenCreate | EsOpenRead,
                          0444);
  ...
  if (stat.perm.isUserWriteable != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isUserWriteable != 0'\n");
  }
  if (stat.perm.isUserReadable != 1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isUserReadable != 1'\n");
  ...
  if (stat.perm.isGroupWriteable != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isGroupWriteable != 0'\n");
  }
  if (stat.perm.isGroupReadable != 1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isGroupReadable != 1'\n");
  ...
  if (stat.perm.isOtherWriteable != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isOtherWriteable != 0'\n");
  }
  if (stat.perm.isOtherReadable != 1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isOtherReadable != 1'\n");
  ...
  if (stat.perm.isUserReadable != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_stat() error 'stat.isUserReadable != 0'\n");
  }
#endif /* defined(OMR_OS_WINDOWS) */
...
TEST_F(PortFileTest2, file_test33) {
  ...
  omrfile_write_text(
      1, (const char *)three_byte_utf8,
      sizeof(char) * 4); /* Unicode string representation should be printed */
  omrfile_write_text(1, (const char *)four_byte_utf8,
                     sizeof(char) * 5); /* '????' should be printed */
  omrfile_write_text(1, (const char *)invalid_byte_utf8,
                     sizeof(char) * 3); /* '??' should be printed */
  omrfile_write_text(
      1, (const char *)many_byte_utf8,
      sizeof(char) * 512); /* Unicode string representation should be printed */
 ...
 *
 * For both relative and absolute paths, build up a directory hierarchy that is
 * greater than the system defined MAX_PATH
 *
...
#define FILENAME_LENGTH                                                        \
  MIN_LENGTH * 2 /* double the size to accomodate a file size that is just     \
                    under twice MIN_LENGTH */
  ...
  for (i = 0; i < 2; i++) {
     ...
     * comprised of directories, each of which are less than 256 characters in
     * length*/
    while (strlen(filePathName) < MIN_LENGTH) {

      omrstr_printf(filePathName + strlen(filePathName),
                    FILENAME_LENGTH - strlen(filePathName), "%s", longDirName);
    ...
    omrstr_printf(filePathName + strlen(filePathName),
                  FILENAME_LENGTH - strlen(filePathName), "\\%s", testName);

    portTestEnv->log("\ttesting filename: %s\n", filePathName);

    /* can we open and write to the file? */
    fd = FILE_OPEN_FUNCTION(OMRPORTLIB, filePathName,
                            EsOpenCreate | EsOpenWrite, 0666);
    if (-1 == fd) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "%s() failed\n",
                         FILE_OPEN_FUNCTION_NAME);
  ...
  exit:
    ...
    /* Now we need to unwind and delete all the directories we created one by
     * one
     * 	- we simply delete the base directory because it is not empty.
     * Do this by stripping actual name of the filename, then each occurrence of
     * longDirName from the full path.
     *
     * You know you're done when the filePathName doesn't contain the occurrence
     * of longDirName
     */
 ...
 *
 * This test check the amount of free disk space, create a file and check the
 * amount of free disk space again. It is necessary for the amount of free disk
 * space to changes after the file is created. It is necessary for the total
 * amount of disk space to be constant after the file is created.
 *
 * @return TEST_PASS on success, TEST_FAIL on error
 */
#if !defined(OMR_OS_WINDOWS)
int omrfile_test34(struct OMRPortLibrary *portLibrary) {
#define J9FILETEST_FILESIZE 1000 * 512
  ...
  if (0 != rc) {
    portTestEnv->log(
        LEVEL_ERROR,
        "first omrfile_stat_filesystem() returned %d, expected %d\n", rc, 0);
    returnCode = TEST_FAIL;
    goto exit;
  }
  rc = omrfile_stat(fileName, 0, &buf);
  if (0 != rc) {
    portTestEnv->log(LEVEL_ERROR,
                     "first omrfile_stat() returned %d, expected %d\n", rc, 0);
    returnCode = TEST_FAIL;
    goto exit;
  }

  if (buf.isRemote) {
    portTestEnv->log(LEVEL_WARN, "WARNING: Test omrfile_test34 cannot be run "
                                 "on a remote file system.\nSkipping test.\n");
    returnCode = TEST_PASS;
    goto exit;
  }

  stringToWrite = (char *)omrmem_allocate_memory(J9FILETEST_FILESIZE,
  ...
  if (0 != rc) {
    portTestEnv->log(
        LEVEL_ERROR,
        "second omrfile_stat_filesystem() returned %d, expected %d\n", rc, 0);
    returnCode = TEST_FAIL;
    goto exit;
  }

  freeSpaceDelta =
      (uintptr_t)(buf_before.freeSizeBytes - buf_after.freeSizeBytes);

  /* Checks if the free disk space changed after the file has been created. */
  if (0 == freeSpaceDelta) {
    portTestEnv->log(
        "The amount of free disk space does not change after the creation of a file.\n\
  ...
  if (buf_before.totalSizeBytes != buf_after.totalSizeBytes) {
    portTestEnv->log(
        "Total disk space changed between runs, it was %llu changed to %llu.\n\
 ...
 *
 * @return TEST_PASS as soon as the test passes, TEST_FAIL if the test always
 * fails
 */
int omrfile_test34_multiple_tries(struct OMRPortLibrary *portLibrary,
                                  int nbOfTries) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  int i, rc;
  const char *testName =
      APPEND_ASYNC(omrfile_test34_multiple_tries
                   : _test_omrfile_stat_filesystem_disk_space);

  reportTestEntry(OMRPORTLIB, testName);
  /* The test omrfile_test34 may fail if other processes writes to the hard
   * drive, trying the test nbOfTries times */
  ...
  outputErrorMessage(PORTTEST_ERROR_ARGS,
                     "omrfile_test34_multiple_tries failed %d time(s)\n",
                     nbOfTries);
exit:
  return reportTestExit(OMRPORTLIB, testName);
}
...
TEST_F(PortFileTest2, file_test35) {
  ...
  fd = FILE_OPEN_FUNCTION(OMRPORTLIB, fileName, EsOpenCreate | EsOpenWrite,
                          0666);
  if (-1 == fd) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() returned -1 expected valid file handle",
  ...
  if (seekOffset != offset) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "file_seek() returned %lld expected %lld\n", offset,
                       seekOffset);
    goto exit;
  }
 ...
 * Test omrfile_test36
 *
 * Simple test of omrfile_convert_native_fd_to_omrfile_fd (it's a simple
 * function!)
 */
TEST_F(PortFileTest2, file_test36) {
  ...
  if (omrfileFD != nativeFD) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfileFD != nativeFD, they should be the same. "
                       "omrfileFD: %x, nativeFD: %x\n",
                       omrfileFD, nativeFD);
  }
#else /* defined(J9ZOS390) */
  if (omrfileFD == nativeFD) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfileFD == nativeFD, they should be different. "
                       "omrfileFD: %x, nativeFD: %x\n",
                       omrfileFD, nativeFD);
  }
  ...
  if (nativeFD < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "nativeFD < 0 , should be non-negative. nativeFD: %x\n",
                       nativeFD);
  }
  ...
  if (omrfileFD == OMRPORT_INVALID_FD) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "j9fileFD == J9PORT_INVALID_FD, they should be the "
                       "same. j9fileFD: %x\n",
                       omrfileFD);
  }
  if (omrfileFD != OMRPORT_TTY_OUT) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "j9fileFD != J9PORT_TTY_OUT, they should be the same. j9fileFD: %x\n",
        omrfileFD);
  }
...
TEST_F(PortFileTest2, file_test37) {
  ...
  if (rc >= 0) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "Negative test failure: omrfile_fstat(-1, ..) on invalid file "
        "descriptor is expected to fail but it returned success with rc=%d "
        "(ignore the following LastErrorNumber and LastErrorMessage)\n",
        rc);
  }
  ...
  if (0 == omrsysinfo_get_executable_name(NULL, &knownFile)) {
    ...
    if (-1 != fd) {
      ...
      if (0 != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "omrfile_fstat(...) failed for fd=%d(filename=%s)\n",
                           fd, knownFile);
      } else if (1 != buf.isFile) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "omrfile_fstat(...) for %s returned buf.isFile = %d, expected 1\n",
    ...
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrsysinfo_get_executable_name(NULL, &knownFile) failed\n");
  }
  ...
  if (-1 != fd) {
    ...
    if (0 != rc) {
      ...
      if (myUid != stat.ownerUid) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "omrfile_fstat(...) for %s returned ownerUid as %d, expected %d\n",
            filename, stat.ownerUid, myUid);
      }

      if (myGid != fileGid) {
        portTestEnv->log("Unexpected group ID. Possibly setgid bit set. "
                         "Comparing to group of the current directory\n");
        rc = omrfile_stat(".", 0, &stat);
        if (0 != rc) {
          outputErrorMessage(
              PORTTEST_ERROR_ARGS,
              "omrfile_stat(...) failed for current directory with rc=%d\n",
              rc);
        } else {
          if (fileGid != stat.ownerGid) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                               "omrfile_fstat(...) for current directory "
                               "returned ownerGid as %d, expected %d\n",
                               fileGid, stat.ownerGid);
          }
        }
      }
    ...
    if (0 != rc) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrfile_unlink() failed to delete file %s\n",
                         filename);
    }
  } else {
...
TEST_F(PortFileTest2, file_test39) {
  OMRPORT_ACCESS_FROM_OMRPORT(portTestEnv->getPortLibrary());
  const char *testName =
      APPEND_ASYNC(omrfile_test39
                   : test file permission bits using omrfile_fstat());
  ...
  if (-1 != fd) {
    ...
    if (0 != rc) {
      ...
      if (stat.perm.isUserWriteable != 1) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "omrfile_fstat() error 'stat.isUserWriteable != 1'\n");
      }
      if (stat.perm.isUserReadable != 1) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "omrfile_fstat() error 'stat.isUserReadable != 1'\n");
      }
      /*umask vary so we can't test 'isGroupWriteable', 'isGroupReadable',
       * 'isOtherWriteable', or 'isOtherReadable'*/
    }
    ...
    if (0 != rc) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrfile_unlink() failed to delete the file %s\n",
                         filename);
    }
  } else {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() failed to create file %s with mode=%d\n",
  ...
  mode = 0444;
  fd =
      FILE_OPEN_FUNCTION(OMRPORTLIB, filename, EsOpenCreate | EsOpenRead, mode);
  if (-1 != fd) {
    rc = omrfile_fstat(fd, &stat);
    if (0 != rc) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
      ...
      if (stat.perm.isUserWriteable != 0) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "omrfile_fstat() error 'stat.isUserWriteable != 0'\n");
      }
      if (stat.perm.isUserReadable != 1) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
  ...
  if (-1 != fd) {
    ...
    if (0 != rc) {
      ...
      if (stat.perm.isUserReadable != 0) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "omrfile_fstat() error 'stat.isUserReadable != 0'\n");
      }
      /*umask vary so we can't test 'isGroupWriteable', 'isGroupReadable',
       * 'isOtherWriteable', or 'isOtherReadable'*/
    }
  ...
  if (-1 != fd) {
    ...
    if (0 != rc) {
      ...
      if (stat.perm.isGroupWriteable != 0) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "omrfile_fstat() error 'stat.isGroupWriteable != 0'\n");
      }
      if (stat.perm.isGroupReadable != 0) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "omrfile_fstat() error 'stat.isGroupReadable != 0'\n");
      }
      if (stat.perm.isOtherWriteable != 0) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "omrfile_fstat() error 'stat.isOtherWriteable != 0'\n");
      }
      if (stat.perm.isOtherReadable != 0) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "omrfile_fstat() error 'stat.isOtherReadable != 0'\n");
      }
    }
    ...
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "5s() failed to create file %s with mode=%d\n",
                       FILE_OPEN_FUNCTION_NAME, filename, mode);
  }
...
TEST_F(PortFileTest2, file_test40) {
  ...
  if (prevFilePtr != filePtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "%s() Changed the location of file pointer. Expected : "
                       "%lld. Got : %lld \n",
                       FILE_SET_LENGTH_FUNCTION_NAME, prevFilePtr, filePtr);
    goto exit; /* Although we dont need this, lets keep it to make it look like
                  other failure cases */
  }
...
TEST_F(PortFileTest2, lastmod_failedToFindFile) {
  ...
  if (rc != -1) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrfile_lastmod() did not return -1 on an invalid file.");
  }
  reportTestExit(OMRPORTLIB, testName);
}
 ...
 */
int32_t omrfile_runTests(struct OMRPortLibrary *portLibrary, char *argv0,
                         char *omrfile_child, BOOLEAN async) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  isAsync = async;
