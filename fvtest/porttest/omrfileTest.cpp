...
/**
 */
void openFlagTest(struct OMRPortLibrary* portLibrary, const char* testName, const char* pathName, int32_t flags, int32_t mode)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
    intptr_t fd;
    J9FileStat buf;
...
TEST_F(PortFileTest2, file_test14)
{
    ...
    /* dont forget that the tdirTest14a inside tdirTest14b also counts for the search */
    /*int32_t fileCount=0;
...
TEST_F(PortFileTest2, file_test33)
{
    ...
    const char* testName = APPEND_ASYNC(omrfile_test33
                                        : test UTF - 8 encoding);
...
int omrfile_test34_multiple_tries(struct OMRPortLibrary* portLibrary, int nbOfTries)
{
    OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
    int i, rc;
    const char* testName = APPEND_ASYNC(omrfile_test34_multiple_tries
...
{
    OMRPORT_ACCESS_FROM_OMRPORT(portTestEnv->getPortLibrary());
    const char* testName = APPEND_ASYNC(omrfile_test39
