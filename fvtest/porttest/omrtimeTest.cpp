 ...
 * @arg @ref omrtime.c::omrtime_current_time_nanos
...
TEST(PortTimeTest, time_test1) {
  ...
  /* get a thread so that we can use native time delays (the original solution
   * would spin and fail on fast machines) */
  omrthread_t self;
  ...
  if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
    ...
    if (timeCheck == timeStart) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrtime_current_time_millis did not change after a "
                         "half-second forced delay\n");
    }
    if (mtimeStart == mtimeCheck) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrtime_msec_clock did not change after a "
                         "half-second forced delay\n");
    }
    if (utimeStart == utimeCheck) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrtime_usec_clock did not change after a "
                         "half-second forced delay\n");
    }
    if (ntimeStart == ntimeCheck) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrtime_nano_time did not change after a half-second "
                         "forced delay\n");
    }
    ...
    if (success) {
      if (nClocktimeStart == nClocktimeCheck) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "omrtime_current_time_nanos did not change after a "
                           "half-second forced delay\n");
      }
    } else {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "omrtime_current_time_nanos failed to return a valid time\n");
    }
    ...
    if (0 != omrtime_hires_frequency()) {
      ...
      if (hiresTimeCheck == hiresTimeStart) {
        ...
        if (hiresTimeCheck == hiresTimeStart) {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "omrtime_hires_clock has not advanced after a "
                             "half-second forced delay\n");
        }
      } else {
        /* the timer advanced so don't even try to run the test */
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "omrtime_hires_frequency has advanced\n");
      }
    } else {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "omrtime_hires_frequency is 0, timer will not advance\n");
    }
  } else {
    /* failure initializing thread library: we won't be able to test reliably */
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrthread_attach failed in omrtime_test1 so testing is "
                       "not possible\n");
  }
...
TEST(PortTimeTest, time_test2) {
  ...
  /* start < stop */
  simulatedValue =
      omrtime_hires_delta(0, expectedValue, omrtime_hires_frequency());
  if (simulatedValue != expectedValue) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrtime_hires_delta returned %llu expected %llu\n",
   ...
   * ...*/
  hiresTimeStart =
      ((uint64_t)-1) - ((timeInterval - 2) * omrtime_hires_frequency());
  hiresTimeStop = 2 * omrtime_hires_frequency();
  simulatedValue = omrtime_hires_delta(hiresTimeStart, hiresTimeStop,
                                       omrtime_hires_frequency());
 ...
 * Verify port library timer operations.
 *
 * For various intervals verify the various timers for consistency.  There is no
 * point in testing against clock()
 *
 * Functions verified by this test:
 * @arg @ref omrtime.c::omrtime_msec_clock "omrtime_msec_clock()"
 * @arg @ref omrtime.c::omrtime_usec_clock "omrtime_usec_clock()"
 * @arg @ref omrtime.c::omrtime_current_time_nanos
 * "omrtime_current_time_nanos()"
 * @arg @ref omrtime.c::omrtime_current_time_millis
...
TEST(PortTimeTest, time_test3) {
  ...
  if (1 == omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE)) {
    /* one CPU means we have no chance of falling into the "difference CPUs have
     * different times" trap.  Let the test begin */
    /*let's test the others vs current_time_millis*/
    portTestEnv->log("%10s %10s %10s %10s %12s %12s\n", "millires", "millis",
                     "msec", "usec", "hires msec ", "hires usec");
    for (i = 0; i < J9TIME_REPEAT_TEST; i++) {
      ...
      utimeStop = omrtime_usec_clock(); /*higher-precision CLK should get
                                           presidence NaH!*/

      hiresDeltaAsMillis = (uint32_t)omrtime_hires_delta(
          hiresTimeStart, hiresTimeStop, OMRPORT_TIME_DELTA_IN_MILLISECONDS);
      hiresDeltaAsMicros = (uint32_t)omrtime_hires_delta(
          hiresTimeStart, hiresTimeStop, OMRPORT_TIME_DELTA_IN_MICROSECONDS);
      ...
      portTestEnv->log(
          "%10d %10d %10d %10d %12d %12d %12d\n", millires, (int32_t)timeDelta,
          (int32_t)mtimeDelta, (int32_t)utimeDelta, (int32_t)ntimeDelta,
          (int32_t)hiresDeltaAsMillis, (int32_t)hiresDeltaAsMicros);

      hiresDeltaAsMillis = hiresDeltaAsMillis > mtimeDelta
                               ? hiresDeltaAsMillis - mtimeDelta
                               : mtimeDelta - hiresDeltaAsMillis;
      if (hiresDeltaAsMillis > (0.1 * mtimeDelta)) {
        portTestEnv->log("\n");
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "omrtime_hires_clock() drift greater than 10%%\n");
        failed = 1;
      }

      ntimeDeltaAsMillis = ntimeDeltaAsMillis > mtimeDelta
                               ? ntimeDeltaAsMillis - mtimeDelta
                               : mtimeDelta - ntimeDeltaAsMillis;
      if (ntimeDeltaAsMillis > (0.1 * mtimeDelta)) {
        portTestEnv->log("\n");
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "omrtime_current_time_nanos() drift greater than 10%%\n");
        failed = 1;
      }
      if (failed) {
        break;
    ...
    /* test is invalid since this is a multi-way machine:  if we get the time on
     * one CPU, then get rescheduled on a different one where we ask for the
     * time, there is no reason why the time values need to be monotonically
     * increasing */
    portTestEnv->log(
        LEVEL_ERROR,
        "Test is invalid since the host machine reports more than one CPU "
        "(time may differ across CPUs - makes test results useless - re-enable "
        "if we develop thread affinity support)\n");
  }
  portTestEnv->log("\n");
 ...
 */
static BOOLEAN compareMillis(uint64_t value1, uint64_t value2,
                             uint64_t epsilon) {
  return (value2 >= value1) && (value2 - value1 < epsilon);
}

static uint64_t portGetNanos(struct OMRPortLibrary *portLibrary) {
 ...
 * - the test mostly does not actually test anything, because most of the time
 * it detects: "WARNING: Skipping check due to possible NTP daemon
 * interference."
 * 		- this only adds to the confusion
 * - the test is known to not work on x86 because of the omrtime_hires_clock
 * implementation
 */
TEST(DISABLED_PortTimeTest, time_test4) {
  ...
  for (i = 0; i < NUMBER_OF_ITERATIONS; i++) {
    ...
    if (0 != omrthread_sleep(SLEEP_TIME)) {
      portTestEnv->log(LEVEL_WARN, "WARNING: Skipping check due to "
                                   "omrthread_sleep() returning non-zero.\n");
      continue;
    }
    ...
    delta = (milliDeltaOuter > nanoDeltaInMillis
                 ? milliDeltaOuter - nanoDeltaInMillis
                 : nanoDeltaInMillis - milliDeltaOuter);
    if (delta > EPSILON) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "abs(milliDeltaOuter - nanoDeltaInMillis) > EPSILON (%llu)", delta);
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrtime_current_time_millis() is not consistent with "
                         "omrtime_hires_clock()!");
      break;
    }

    delta = (nanoDeltaInMillis > milliDeltaInner
                 ? nanoDeltaInMillis - milliDeltaInner
                 : milliDeltaInner - nanoDeltaInMillis);
    if (delta > EPSILON) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "abs(nanoDeltaInMillis - milliDeltaInner) > EPSILON (%llu)", delta);
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrtime_current_time_millis() is not consistent with "
                         "omrtime_hires_clock()!");
      break;
...
#if defined(OMR_OS_WINDOWS)
  /**
   * On Windows, if QueryPerformanceCounter is used on a multiprocessor
   * computer, time might be different accross CPUs. Therefore skip this test
   * and only re-enable if we develop thread affinity support.
   */
  {
    LARGE_INTEGER i;
    if (QueryPerformanceCounter(&i)) {
      portTestEnv->log(LEVEL_WARN,
                       "WARNING: Test is invalid since the host machine uses "
                       "QueryPerformanceCounter() (time may differ across CPUs "
                       "- makes test results useless - re-enable if we develop "
  ...
  if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
    ...
    if (0 == omrthread_monitor_init(&tds.monitor, 0)) {
      ...
      omrtimeTestDirectionNumThreads =
          ...
          threadToCPUFactor;
      threads = (omrthread_t *)omrmem_allocate_memory(
          omrtimeTestDirectionNumThreads * sizeof(omrthread_t),
      ...
      if (NULL != threads) {
        if (0 == omrthread_monitor_enter(tds.monitor)) {
          for (i = 0; i < omrtimeTestDirectionNumThreads; i++) {
            intptr_t rc =
                omrthread_create(&threads[i], 128 * 1024, J9THREAD_PRIORITY_MAX,
                                 0, &nanoTimeDirectionTest, &tds);
            if (0 != rc) {
              outputErrorMessage(PORTTEST_ERROR_ARGS,
                                 "Failed to create thread, rc=%zd, i=%zu", rc,
                                 i);
            }
          }
          ...
          while ((0 == waitRetVal) &&
                 (tds.finishedCount < omrtimeTestDirectionNumThreads)) {
            waitRetVal = omrthread_monitor_wait_timed(
                tds.monitor, J9TIME_TEST_DIRECTION_TIMEOUT_MILLIS, 0);
          }
          ...
          if (0 != waitRetVal) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS,
                "omrthread_monitor_wait_timed() failed, waitRetVal=%zd",
                waitRetVal);
          }

          if (tds.failed) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                               "omrtime_hires_clock() did not go forward in at "
                               "least one of the launched threads");
          }

          omrthread_monitor_exit(tds.monitor);
        } else {
          outputErrorMessage(PORTTEST_ERROR_ARGS,
                             "Failed to enter tds.monitor");
        }

        omrmem_free_memory(threads);
      } else {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "Failed to allocate memory for threads array");
      }
      omrthread_monitor_destroy(tds.monitor);
    } else {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Failed to initialize tds.monitor");
    }

    omrthread_detach(self);
  } else {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Failed to attach to thread library");
  }

  reportTestExit(OMRPORTLIB, testName);
}

static int J9THREAD_PROC nanoTimeDirectionTest(void *arg) {
  J9TimeTestDirectionStruct *tds = (J9TimeTestDirectionStruct *)arg;
  uintptr_t i = 0;
  const uintptr_t numLoops = 500;
  const I_64 sleepMillis =
      20; /* 20*500 -> total thread execution time should be ~ 10 seconds */
  ...
  for (i = 0; i < numLoops; i++) {
    ...
    if (0 != omrthread_sleep(sleepMillis)) {
      portTestEnv->log(LEVEL_ERROR,
                       "\tomrthread_sleep() did not return zero.\n");
    ...
    if (finish <= start) {
      portTestEnv->log(LEVEL_ERROR,
                       "\tTime did not go forward after omrthread_sleep, "
                       "start=%llu, finish=%llu\n",
                       start, finish);
      omrthread_monitor_enter(tds->monitor);
      tds->failed = TRUE;
      omrthread_monitor_exit(tds->monitor);
 ...
 */
static double omrtime_test_compute_error_pct(double exp, double actual) {
  double error = 0.0;
 ...
 *
 * omrtime_hires_delta() converts the time interval into the requested
 * resolution by effectively multiplying the interval by (requiredRes /
 * omrtime_hires_frequency()).
 *
 * We use a test interval equal to (ticksPerSec = omrtime_hires_frequency()), so
 * if there was no loss of precision, we should have: delta = ticksPerSec *
 * (requiredRes / ticksPerSec) = requiredRes
 *
 * This test fails if the difference between the computed and expected delta is
 * > 1%.
 *
 * The worst roundoff error cases for omrtime_hires_delta() are:
 * 1. (ticksPerSec > requiredRes) and (ticksPerSec / requiredRes) truncates down
 * a lot.
...
TEST(PortTimeTest, time_test_hires_delta_rounding) {
  ...
  /*
   * requiredRes is picked so that the ratio (ticksPerSec / requiredRes) is 1.9,
   * which has a big fractional part that would be lost if naive integer math
   * were used.
   *
   * If we did lose the fractional part, then we would get:
   * delta = ticksPerSec / (ticksPerSec / requiredRes) = ticksPerSec / 1 =
   * ticksPerSec error = abs(ticksPerSec - requiredRes) / requiredRes = 0.9 ...
  ...
  if (0 == delta) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "    Case 1. omrtime_hires_delta returned 0");
  }
  ...
  if (0 == delta) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "    Case 2. omrtime_hires_delta returned 0");
  }
  ...
  if (0 == delta) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "    Case 3. omrtime_hires_delta returned 0");
  }
