...
nanoTimeDirectionTest(void* portLibrary);
...
TEST(PortTimeTest, time_test1)
{
  ...
  if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
    ...
    if (mtimeStart == mtimeCheck) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrtime_msec_clock did not change after a half-second forced delay\n");
    }
    if (utimeStart == utimeCheck) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrtime_usec_clock did not change after a half-second forced delay\n");
    }
    if (ntimeStart == ntimeCheck) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrtime_nano_time did not change after a half-second forced delay\n");
    }
    ...
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrthread_attach failed in omrtime_test1 so testing is not possible\n");
  }
...
TEST(PortTimeTest, time_test2)
{
  ...
  simulatedValue = omrtime_hires_delta(
    hiresTimeStart, hiresTimeStop, omrtime_hires_frequency());
  ...
  if (1 == omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE)) {
    ...
    for (i = 0; i < J9TIME_REPEAT_TEST; i++) {
      ...
      portTestEnv->log("%10d %10d %10d %10d %12d %12d %12d\n",
                       ...
                       (int32_t)utimeDelta,
                       (int32_t)ntimeDelta,
                       (int32_t)hiresDeltaAsMillis,
                       (int32_t)hiresDeltaAsMicros);
    ...
    portTestEnv->log(
      LEVEL_ERROR,
      "Test is invalid since the host machine reports more than one CPU (time "
      "may differ across CPUs - makes test results useless - re-enable if we "
      "develop thread affinity support)\n");
  }
  portTestEnv->log("\n");
...
TEST(DISABLED_PortTimeTest, time_test4)
{
  ...
  for (i = 0; i < NUMBER_OF_ITERATIONS; i++) {
    ...
    if (delta > EPSILON) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "abs(milliDeltaOuter - nanoDeltaInMillis) > EPSILON (%llu)",
        delta);
      outputErrorMessage(PORTTEST_ERROR_ARGS,
    ...
    if (delta > EPSILON) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "abs(nanoDeltaInMillis - milliDeltaInner) > EPSILON (%llu)",
        delta);
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrtime_current_time_millis() is not consistent with "
                         "omrtime_hires_clock()!");
   ...
   */
  {
    ...
    if (QueryPerformanceCounter(&i)) {
      portTestEnv->log(
        ...
        "QueryPerformanceCounter() (time may differ across CPUs - makes test "
        "results useless - re-enable if we develop thread affinity support)\n");
      reportTestExit(OMRPORTLIB, testName);
    }
  }
#endif /* defined(OMR_OS_WINDOWS) */
      ...
      if (NULL != threads) {
        if (0 == omrthread_monitor_enter(tds.monitor)) {
          for (i = 0; i < omrtimeTestDirectionNumThreads; i++) {
            intptr_t rc = omrthread_create(&threads[i],
                                           128 * 1024,
                                           J9THREAD_PRIORITY_MAX,
                                           0,
                                           &nanoTimeDirectionTest,
                                           &tds);
            if (0 != rc) {
              outputErrorMessage(PORTTEST_ERROR_ARGS,
                                 "Failed to create thread, rc=%zd, i=%zu",
                                 rc,
                                 i);
            }
          }
...
static int J9THREAD_PROC
nanoTimeDirectionTest(void* arg)
{
