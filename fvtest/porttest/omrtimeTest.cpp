...
TEST(PortTimeTest, time_test1)
{
  ...
  if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
    ...
    if (timeCheck == timeStart) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "omrtime_current_time_millis did not change after a half-second forced delay\n");
    }
    ...
    if (success) {
      if (nClocktimeStart == nClocktimeCheck) {
        outputErrorMessage(
          PORTTEST_ERROR_ARGS, "omrtime_current_time_nanos did not change after a half-second forced delay\n");
      }
    } else {
    ...
    if (0 != omrtime_hires_frequency()) {
      ...
      if (hiresTimeCheck == hiresTimeStart) {
        ...
        if (hiresTimeCheck == hiresTimeStart) {
          outputErrorMessage(
            PORTTEST_ERROR_ARGS, "omrtime_hires_clock has not advanced after a half-second forced delay\n");
        }
      } else {
...
TEST(PortTimeTest, time_test2)
{
  ...
  if (simulatedValue != expectedValue) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrtime_hires_delta returned %llu expected %llu\n", simulatedValue, expectedValue);
  }
...
TEST(PortTimeTest, time_test3)
{
  ...
  if (1 == omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE)) {
    /* one CPU means we have no chance of falling into the "difference CPUs have different times" trap.  Let the test
     * begin */
    ...
    for (i = 0; i < J9TIME_REPEAT_TEST; i++) {
      ...
      hiresDeltaAsMillis
        = (uint32_t)omrtime_hires_delta(hiresTimeStart, hiresTimeStop, OMRPORT_TIME_DELTA_IN_MILLISECONDS);
      hiresDeltaAsMicros
        = (uint32_t)omrtime_hires_delta(hiresTimeStart, hiresTimeStop, OMRPORT_TIME_DELTA_IN_MICROSECONDS);
      ...
      portTestEnv->log("%10d %10d %10d %10d %12d %12d %12d\n", millires, (int32_t)timeDelta, (int32_t)mtimeDelta,
        (int32_t)utimeDelta, (int32_t)ntimeDelta, (int32_t)hiresDeltaAsMillis, (int32_t)hiresDeltaAsMicros);

      hiresDeltaAsMillis
        = hiresDeltaAsMillis > mtimeDelta ? hiresDeltaAsMillis - mtimeDelta : mtimeDelta - hiresDeltaAsMillis;
      ...
      ntimeDeltaAsMillis
        = ntimeDeltaAsMillis > mtimeDelta ? ntimeDeltaAsMillis - mtimeDelta : mtimeDelta - ntimeDeltaAsMillis;
    ...
    /* test is invalid since this is a multi-way machine:  if we get the time on one CPU, then get rescheduled on a
     * different one where we ask for the time, there is no reason why the time values need to be monotonically
     * increasing */
    portTestEnv->log(LEVEL_ERROR,
      "Test is invalid since the host machine reports more than one CPU (time may differ across CPUs - makes test "
 ...
 */
static BOOLEAN compareMillis(uint64_t value1, uint64_t value2, uint64_t epsilon)
{
...
TEST(DISABLED_PortTimeTest, time_test4)
{
  ...
  for (i = 0; i < NUMBER_OF_ITERATIONS; i++) {
    ...
    if (!compareMillis(milliStartOuter, milliStartInner, EPSILON)
      || !compareMillis(SLEEP_TIME, milliDeltaInner, EPSILON) || !compareMillis(milliEndInner, milliEndOuter, EPSILON)
      || (milliDeltaOuter < milliDeltaInner)) {
      portTestEnv->log(LEVEL_WARN, "WARNING: Skipping check due to possible NTP daemon interference.\n");
      continue;
    }

    delta = (milliDeltaOuter > nanoDeltaInMillis ? milliDeltaOuter - nanoDeltaInMillis
    ...
    if (delta > EPSILON) {
      ...
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "omrtime_current_time_millis() is not consistent with omrtime_hires_clock()!");
      break;
    }

    delta = (nanoDeltaInMillis > milliDeltaInner ? nanoDeltaInMillis - milliDeltaInner
   ...
   */
  {
    ...
    if (QueryPerformanceCounter(&i)) {
      portTestEnv->log(LEVEL_WARN,
        "WARNING: Test is invalid since the host machine uses QueryPerformanceCounter() (time may differ across CPUs - "
        "makes test results useless - re-enable if we develop thread affinity support)\n");
      reportTestExit(OMRPORTLIB, testName);
    }
  }
#endif /* defined(OMR_OS_WINDOWS) */
  ...
  if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
    ...
    if (0 == omrthread_monitor_init(&tds.monitor, 0)) {
      ...
      threads = (omrthread_t*)omrmem_allocate_memory(
        omrtimeTestDirectionNumThreads * sizeof(omrthread_t), OMRMEM_CATEGORY_PORT_LIBRARY);

      if (NULL != threads) {
        if (0 == omrthread_monitor_enter(tds.monitor)) {
          for (i = 0; i < omrtimeTestDirectionNumThreads; i++) {
            intptr_t rc
              = omrthread_create(&threads[i], 128 * 1024, J9THREAD_PRIORITY_MAX, 0, &nanoTimeDirectionTest, &tds);
            if (0 != rc) {
          ...
          if (0 != waitRetVal) {
            outputErrorMessage(
              PORTTEST_ERROR_ARGS, "omrthread_monitor_wait_timed() failed, waitRetVal=%zd", waitRetVal);
          }

          if (tds.failed) {
            outputErrorMessage(
              PORTTEST_ERROR_ARGS, "omrtime_hires_clock() did not go forward in at least one of the launched threads");
          }
...
static int J9THREAD_PROC nanoTimeDirectionTest(void* arg)
{
  ...
  for (i = 0; i < numLoops; i++) {
    ...
    if (finish <= start) {
      portTestEnv->log(
        LEVEL_ERROR, "\tTime did not go forward after omrthread_sleep, start=%llu, finish=%llu\n", start, finish);
      omrthread_monitor_enter(tds->monitor);
      tds->failed = TRUE;
      omrthread_monitor_exit(tds->monitor);
      break;
