      ...
      switch (infoKind) {
        ...
        case OMRPORT_SIG_VALUE_FLOAT_64:
          ...
          portTestEnv->log("info %u:%u: %s=%016.16llx (f: %f, d: %e)\n",
                           ...
                           (float)LOW_U32_FROM_DBL_PTR(value),
                           *(double *)value);
          break;
...
static uintptr_t currentSigNumHandlerFunction(
    struct OMRPortLibrary *portLibrary, uint32_t gpType, void *gpInfo,
    void *handler_arg) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  SigProtectHandlerInfo *info = (SigProtectHandlerInfo *)handler_arg;
...
TEST(PortSigTest, sig_test0) {
  ...
  if (!omrsig_can_protect(OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION)) {
     ...
     */
    portTestEnv->log(
        "OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION is not supported if the "
        "default settings for TRAP(ON,SPIE) have been changed. The should both "
        "be set to 1\n");
#endif
  }
