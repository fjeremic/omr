    ...
    for (category = 0; category < OMRPORT_SIG_NUM_CATEGORIES; category++) {
        ...
        for (index = 0; index < infoCount; index++) {
            ...
            case OMRPORT_SIG_VALUE_UNDEFINED:
                outputErrorMessage(
                    PORTTEST_ERROR_ARGS, "Undefined value in info (%u:%u, name=%s)\n", category, index, name);
                break;
            default:
    ...
    struct OMRPortLibrary* portLibrary, uint32_t gpType, void* gpInfo, void* handler_arg)
{
    ...
    if (currentSigNum < 0) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "currentSigNumHandlerFunction -- omrsig_get_current_signal call failed\n");
...
TEST(PortSigTest, sig_test4)
{
    ...
    if (!omrsig_can_protect(flags)) {
        ...
        protectResult = omrsig_protect(
            nestedTestCatchNothing, (void*)testName, simpleHandlerFunction, &handlerInfo, flags, &result);
...
TEST(PortSigTest, sig_test5)
{
    ...
    if (!omrsig_can_protect(flags)) {
        ...
        handlerInfo.testName = testName;
        protectResult = omrsig_protect(
            nestedTestCatchAndIgnore, (void*)testName, simpleHandlerFunction, &handlerInfo, flags, &result);
...
TEST(PortSigTest, sig_test6)
{
    ...
    if (!omrsig_can_protect(flags)) {
        ...
        handlerInfo.testName = testName;
        protectResult = omrsig_protect(
            nestedTestCatchAndCrash, (void*)testName, simpleHandlerFunction, &handlerInfo, flags, &result);
