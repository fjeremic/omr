...
static uintptr_t
asyncTestHandler(struct OMRPortLibrary* portLibrary,
                 uint32_t gpType,
                 void* gpInfo,
                 void* userData);
static void
 ...
 */
int32_t
omrsig_runTests(struct OMRPortLibrary* portLibrary,
                char* exeName,
                char* argument)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
 ...
 */
static void
validatePlatformRegister(struct OMRPortLibrary* portLibrary,
                         void* gpInfo,
                         uint32_t category,
                         int32_t index,
...
void
validateGPInfo(struct OMRPortLibrary* portLibrary,
               uint32_t gpType,
               ...
               const char* testName)
{
  ...
  infoKind = omrsig_info(
    gpInfo, OMRPORT_SIG_SIGNAL, OMRPORT_SIG_SIGNAL_TYPE, &name, &value);
  ...
  infoKind = omrsig_info(
    gpInfo, OMRPORT_SIG_SIGNAL, OMRPORT_SIG_SIGNAL_HANDLER, &name, &value);
  if (infoKind != OMRPORT_SIG_VALUE_ADDRESS) {
    ...
    outputErrorMessage(
      ...
      handler,
      *(omrsig_handler_fn*)value);
  }

  validatePlatformRegister(OMRPORTLIB,
                           gpInfo,
                           OMRPORT_SIG_MODULE,
                           OMRPORT_SIG_MODULE_NAME,
                           OMRPORT_SIG_VALUE_STRING,
                           TRUE,
                           NULL,
                           testName);
  ...
  validatePlatformRegister(OMRPORTLIB,
                           gpInfo,
                           OMRPORT_SIG_SIGNAL,
                           OMRPORT_SIG_SIGNAL_ERROR_VALUE,
  ...
  validatePlatformRegister(OMRPORTLIB,
                           gpInfo,
                           OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_POWERPC_MQ,
                           OMRPORT_SIG_VALUE_ADDRESS,
                           TRUE,
                           OMRPORT_ARCH_PPC,
                           testName);
...
static uintptr_t
simpleHandlerFunction(struct OMRPortLibrary* portLibrary,
                      uint32_t gpType,
                      void* gpInfo,
                      void* handler_arg)
{
...
static uintptr_t
crashingHandlerFunction(struct OMRPortLibrary* portLibrary,
                        uint32_t gpType,
                        void* gpInfo,
                        void* handler_arg)
{
...
nestedTestCatchSEGV(OMRPortLibrary* portLibrary, void* arg)
{
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      protectResult = omrsig_protect(raiseSEGV,
                                     (void*)testName,
...
TEST(PortSigTest, sig_test1)
{
  ...
  if (!omrsig_can_protect(0)) {
    ...
    handlerInfo.returnValue = OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH;
    handlerInfo.testName = testName;
    protectResult = omrsig_protect(sampleFunction,
                                   NULL,
                                   simpleHandlerFunction,
                                   &handlerInfo,
...
TEST(PortSigTest, sig_test3)
{
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.testName = testName;
    protectResult = omrsig_protect(nestedTestCatchSEGV,
                                   (void*)testName,
                                   simpleHandlerFunction,
                                   &handlerInfo,
                                   flags,
...
TEST(PortSigTest, sig_test4)
{
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.returnValue = OMRPORT_SIG_EXCEPTION_RETURN;
    handlerInfo.testName = testName;
    protectResult = omrsig_protect(nestedTestCatchNothing,
                                   (void*)testName,
                                   simpleHandlerFunction,
                                   &handlerInfo,
                                   flags,
...
TEST(PortSigTest, sig_test5)
{
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.returnValue = OMRPORT_SIG_EXCEPTION_RETURN;
    handlerInfo.testName = testName;
    protectResult = omrsig_protect(nestedTestCatchAndIgnore,
                                   (void*)testName,
                                   simpleHandlerFunction,
                                   &handlerInfo,
                                   flags,
...
TEST(PortSigTest, sig_test6)
{
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.returnValue = OMRPORT_SIG_EXCEPTION_RETURN;
    handlerInfo.testName = testName;
    protectResult = omrsig_protect(nestedTestCatchAndCrash,
                                   (void*)testName,
                                   simpleHandlerFunction,
                                   &handlerInfo,
                                   flags,
...
callSigProtectWithContinueSearch(OMRPortLibrary* portLibrary, void* arg)
{
  ...
  handlerInfo.returnValue = OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH;
  handlerInfo.testName = testName;
  protectResult = omrsig_protect(nestedTestCatchAndContinueSearch,
                                 (void*)testName,
                                 simpleHandlerFunction,
                                 &handlerInfo,
                                 flags,
...
TEST(PortSigTest, sig_test8)
{
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.returnValue = OMRPORT_SIG_EXCEPTION_RETURN;
    handlerInfo.testName = testName;
    protectResult = omrsig_protect(callSigProtectWithContinueSearch,
                                   (void*)testName,
                                   simpleHandlerFunction,
                                   &handlerInfo,
                                   flags,
...
TEST(PortSigTest, sig_test_async_unix_handler)
{
  ...
  monitorRC = omrthread_monitor_init_with_name(
    &asyncMonitor, 0, "omrsignalTest_async_monitor");
...
#if defined(AIXPPC)
  setAsyncRC = omrsig_set_async_signal_handler(
    asyncTestHandler, &handlerInfo, OMRPORT_SIG_FLAG_SIGRECONFIG);
  ...
  for (index = 0; index < sizeof(testSignalMap) / sizeof(testSignalMap[0]);
    ...
    omrstr_printf(options,
                  SIG_TEST_SIZE_EXENAME,
                  "-child_omrsig_injectSignal_%i_%i",
                  pid,
                  signum);
    ...
    childProcess =
      launchChildProcess(OMRPORTLIB, testName, portTestEnv->_argv[0], options);
    ...
    omrstr_printf(options,
                  SIG_TEST_SIZE_EXENAME,
                  "-child_omrsig_injectSignal_%i_%i_unhandled",
                  pid,
                  signum);
    portTestEnv->log("\t\tlaunching child process with arg %s\n", options);
