 ...
 *
 */
static uintptr_t asyncTestHandler(
  struct OMRPortLibrary* portLibrary, uint32_t gpType, void* handlerInfo, void* userData)
{
...
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  /* this is run by the child process. To see the tty_printf output on the console, set OMRPORT_PROCESS_INHERIT_STDOUT |
   * OMRPORT_PROCESS_INHERIT_STDIN in the options passed into j9process_create() in launchChildProcess (in
 ...
 */
int32_t omrsig_runTests(struct OMRPortLibrary* portLibrary, char* exeName, char* argument)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
 ...
 */
static void validatePlatformRegister(struct OMRPortLibrary* portLibrary, void* gpInfo, uint32_t category, int32_t index,
  uint32_t expectedKind, uintptr_t optional, const char* platform, const char* testName)
{
  ...
  if ((platform == NULL) || (strcmp(platform, omrsysinfo_get_CPU_architecture()) == 0)) {
    if (infoKind == OMRPORT_SIG_VALUE_UNDEFINED) {
      ...
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "Platform register %d type is not %u. It is %u\n", index, expectedKind, infoKind);
    }
  } else {
    if (infoKind != OMRPORT_SIG_VALUE_UNDEFINED) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "OMRPORT_SIG_GPR_X86_EDI type is not OMRPORT_SIG_VALUE_UNDEFINED. It is %u\n", infoKind);
    }
  }
}
  ...
  struct OMRPortLibrary* portLibrary, uint32_t gpType, omrsig_handler_fn handler, void* gpInfo, const char* testName)
{
  ...
  if (infoKind != OMRPORT_SIG_VALUE_32) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "OMRPORT_SIG_SIGNAL_TYPE type is not OMRPORT_SIG_VALUE_32. It is %u\n", infoKind);
  } else if (*(uint32_t*)value != gpType) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "OMRPORT_SIG_SIGNAL_TYPE is incorrect. Expecting %u, got %u\n", gpType, *(uint32_t*)value);
  }
  ...
  if (infoKind != OMRPORT_SIG_VALUE_ADDRESS) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "OMRPORT_SIG_SIGNAL_HANDLER type is not OMRPORT_SIG_VALUE_ADDRESS. It is %u\n", infoKind);
  ...
  validatePlatformRegister(
    OMRPORTLIB, gpInfo, OMRPORT_SIG_MODULE, OMRPORT_SIG_MODULE_NAME, OMRPORT_SIG_VALUE_STRING, TRUE, NULL, testName);

  validatePlatformRegister(
    OMRPORTLIB, gpInfo, OMRPORT_SIG_SIGNAL, OMRPORT_SIG_SIGNAL_ERROR_VALUE, OMRPORT_SIG_VALUE_32, TRUE, NULL, testName);
  ...
  validatePlatformRegister(
    OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL, OMRPORT_SIG_CONTROL_PC, OMRPORT_SIG_VALUE_ADDRESS, FALSE, NULL, testName);
  validatePlatformRegister(
    OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL, OMRPORT_SIG_CONTROL_SP, OMRPORT_SIG_VALUE_ADDRESS, TRUE, NULL, testName);
  validatePlatformRegister(
    OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL, OMRPORT_SIG_CONTROL_BP, OMRPORT_SIG_VALUE_ADDRESS, TRUE, NULL, testName);
...
static uintptr_t crashingHandlerFunction(
  struct OMRPortLibrary* portLibrary, uint32_t gpType, void* gpInfo, void* handler_arg)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
static uintptr_t nestedTestCatchSEGV(OMRPortLibrary* portLibrary, void* arg)
{
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      protectResult = omrsig_protect(raiseSEGV, (void*)testName, simpleHandlerFunction, &handlerInfo, flags, &result);

      if (protectResult != OMRPORT_SIG_EXCEPTION_OCCURRED) {
        outputErrorMessage(
          PORTTEST_ERROR_ARGS, "portLibrary->sig_protect -- expected OMRPORT_SIG_EXCEPTION_OCCURRED return value\n");
      }
...
static uintptr_t nestedTestCatchNothing(OMRPortLibrary* portLibrary, void* arg)
{
  uint32_t flags = 0;
  const char* testName = (const char*)arg;
  uintptr_t result;
...
static uintptr_t nestedTestCatchAndIgnore(OMRPortLibrary* portLibrary, void* arg)
{
...
static uintptr_t nestedTestCatchAndContinueSearch(OMRPortLibrary* portLibrary, void* arg)
{
...
static uintptr_t nestedTestCatchAndCrash(OMRPortLibrary* portLibrary, void* arg)
{
  uint32_t flags = OMRPORT_SIG_FLAG_SIGSEGV;
  const char* testName = (const char*)arg;
  uintptr_t result;
...
TEST(PortSigTest, sig_test0)
{
  ...
  if (!omrsig_can_protect(OMRPORT_SIG_FLAG_MAY_RETURN)) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "portLibrary->can_protect() must always support 0 | OMRPORT_SIG_FLAG_MAY_RETURN\n");
  }
...
#if defined(J9ZOS390)
    /* z/OS does not always support this, however, pltest should never be run under a configuration that does not
     * support it. See implementation of omrsig_can_proctect() and omrsig_startup()
     */
    portTestEnv->log("OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION is not supported if the default settings for "
                     "TRAP(ON,SPIE) have been changed. The should both be set to 1\n");
#endif
  }
...
TEST(PortSigTest, sig_test1)
{
  ...
  if (!omrsig_can_protect(0)) {
    ...
    if (result != 8096) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "portLibrary->sig_protect -- protected function did not execute correctly\n");
    }
  }
 ...
 * nothing with handler HB. Function B raises a sigsegv. HB won't get invoked because it was not protecting against
 *anything so HA will be invoked, and return control to A (and then omrsig_test4).
 *
 * 		->SEGV--->--
 * 		|			|
 * 		|	 		|
...
TEST(PortSigTest, sig_test4)
{
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.testName = testName;
    protectResult
      = omrsig_protect(nestedTestCatchNothing, (void*)testName, simpleHandlerFunction, &handlerInfo, flags, &result);
...
/*
 * Test protecting Function A against a SEGV with handler HA. A in turn protects Function B
 * against a SEGV with handler HB. Function B raises a sigsegv. HB gets invoked, returns
 * OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH so that HA gets invoked, which then returns control to A (and then
 * omrsig_test5).
 *
 * 		->SEGV
 * 		|	 |
...
TEST(PortSigTest, sig_test5)
{
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.testName = testName;
    protectResult
      = omrsig_protect(nestedTestCatchAndIgnore, (void*)testName, simpleHandlerFunction, &handlerInfo, flags, &result);
...
TEST(PortSigTest, sig_test6)
{
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.testName = testName;
    protectResult
      = omrsig_protect(nestedTestCatchAndCrash, (void*)testName, simpleHandlerFunction, &handlerInfo, flags, &result);
 ...
 */
static uintptr_t callSigProtectWithContinueSearch(OMRPortLibrary* portLibrary, void* arg)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  SigProtectHandlerInfo handlerInfo;
  uint32_t protectResult;
  ...
  protectResult = omrsig_protect(
    nestedTestCatchAndContinueSearch, (void*)testName, simpleHandlerFunction, &handlerInfo, flags, &result);
  if (protectResult != OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "portLibrary->sig_protect -- expected OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH return value\n");
  }
  return result;
}
...
TEST(PortSigTest, sig_test8)
{
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.testName = testName;
    protectResult = omrsig_protect(
      callSigProtectWithContinueSearch, (void*)testName, simpleHandlerFunction, &handlerInfo, flags, &result);
...
#if defined(J9SIGNAL_TEST_RUN_ASYNC_UNIX_TESTS)
/*
 * Tests the async signals to verify that our handler is called both when the signal is raised and injected by another
 * process
 *
 * To test that the handler was called, omrsig_test_async_unix_handler sets controlFlag to 0, then expects the handler
 * to have set it to 1
 *
 * The child process is another instance of pltest with the argument -child_omrsig_injectSignal_<PID>_<signal to
 * inject>. In the child process:
 ...
 *
 * NOTE : Assumes 0.5 seconds is long enough for a child process to be kicked off, inject a signal and have our handler
 * kick in
 * */
TEST(PortSigTest, sig_test_async_unix_handler)
{
  ...
  setAsyncRC = omrsig_set_async_signal_handler(asyncTestHandler, &handlerInfo,
    OMRPORT_SIG_FLAG_SIGQUIT | OMRPORT_SIG_FLAG_SIGABRT | OMRPORT_SIG_FLAG_SIGTERM | OMRPORT_SIG_FLAG_SIGINT);
