...
#if defined(AIXPPC)
    ,
    {OMRPORT_SIG_FLAG_SIGRECONFIG, "OMRPORT_SIG_FLAG_SIGRECONFIG", SIGRECONFIG,
     "SIGRECONFIG"}
#endif
...
static uintptr_t asyncTestHandler(struct OMRPortLibrary *portLibrary,
                                  uint32_t gpType, void *gpInfo,
                                  void *userData);
static void injectUnixSignal(struct OMRPortLibrary *portLibrary, int pid,
                             int unixSignal);
 ...
 */
static uintptr_t asyncTestHandler(struct OMRPortLibrary *portLibrary,
                                  uint32_t gpType, void *handlerInfo,
                                  void *userData) {
  ...
  if (info->expectedType != gpType) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "asyncTestHandler -- incorrect type. Expecting 0x%x, got 0x%x\n",
...
static void injectUnixSignal(struct OMRPortLibrary *portLibrary, int pid,
   ...
   * in the options passed into j9process_create() in launchChildProcess (in
 ...
 * @param[in  arg			either NULL or of the form
 * omrsig_injectSignal_<pid>_<signal>
 *
 * @return 0 on success, -1 on failure
 */
int32_t omrsig_runTests(struct OMRPortLibrary *portLibrary, char *exeName,
                        char *argument) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
 ...
 * If platform is not NULL and is not the current platform, the register MUST be
 * undefined. Otherwise the register must be expectedKind, or it may be
 * undefined if optional is TRUE.
 */
static void validatePlatformRegister(struct OMRPortLibrary *portLibrary,
                                     void *gpInfo, uint32_t category,
                                     int32_t index, uint32_t expectedKind,
                                     uintptr_t optional, const char *platform,
                                     const char *testName) {
  uint32_t infoKind;
  void *value;
  const char *name;
  ...
  if ((platform == NULL) ||
      (strcmp(platform, omrsysinfo_get_CPU_architecture()) == 0)) {
    if (infoKind == OMRPORT_SIG_VALUE_UNDEFINED) {
      if (optional == FALSE) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "Required platform register %d is undefined\n",
                           index);
      }
    } else if (infoKind != expectedKind) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Platform register %d type is not %u. It is %u\n",
                         index, expectedKind, infoKind);
    }
  } else {
    if (infoKind != OMRPORT_SIG_VALUE_UNDEFINED) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "OMRPORT_SIG_GPR_X86_EDI type is not "
                         "OMRPORT_SIG_VALUE_UNDEFINED. It is %u\n",
                         infoKind);
    }
  }
}

void validateGPInfo(struct OMRPortLibrary *portLibrary, uint32_t gpType,
                    omrsig_handler_fn handler, void *gpInfo,
                    const char *testName) {
  uint32_t category;
  void *value;
  const char *name;
  ...
  for (category = 0; category < OMRPORT_SIG_NUM_CATEGORIES; category++) {
    ...
    for (index = 0; index < infoCount; index++) {
      ...
      case OMRPORT_SIG_VALUE_FLOAT_64:
        /* make sure when casting to a float that we get least significant
         * 32-bits. */
        portTestEnv->log("info %u:%u: %s=%016.16llx (f: %f, d: %e)\n", category,
                         index, name, *(uint64_t *)value,
                         (float)LOW_U32_FROM_DBL_PTR(value), *(double *)value);
        break;
      ...
      case OMRPORT_SIG_VALUE_UNDEFINED:
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "Undefined value in info (%u:%u, name=%s)\n",
                           category, index, name);
        break;
      default:
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "Unrecognized type in info (%u:%u, name=%s, type=%u)\n", category,
            index, name, infoKind);
        break;
      }
    }
    ...
    if (infoKind != OMRPORT_SIG_VALUE_UNDEFINED) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "Unexpected data after end of info (%u:%u, name=%s, type=%u)\n",
          category, index, name, infoKind);
    }
  }

  /* test the known names */
  infoKind = omrsig_info(gpInfo, OMRPORT_SIG_SIGNAL, OMRPORT_SIG_SIGNAL_TYPE,
                         &name, &value);
  if (infoKind != OMRPORT_SIG_VALUE_32) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "OMRPORT_SIG_SIGNAL_TYPE type is not OMRPORT_SIG_VALUE_32. It is %u\n",
        infoKind);
  } else if (*(uint32_t *)value != gpType) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "OMRPORT_SIG_SIGNAL_TYPE is incorrect. Expecting %u, got %u\n", gpType,
        *(uint32_t *)value);
  }

  infoKind = omrsig_info(gpInfo, OMRPORT_SIG_SIGNAL, OMRPORT_SIG_SIGNAL_HANDLER,
                         &name, &value);
  if (infoKind != OMRPORT_SIG_VALUE_ADDRESS) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "OMRPORT_SIG_SIGNAL_HANDLER type is not "
                       "OMRPORT_SIG_VALUE_ADDRESS. It is %u\n",
                       infoKind);
  } else if (*(omrsig_handler_fn *)value != handler) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "OMRPORT_SIG_SIGNAL_HANDLER is incorrect. Expecting %p, got %p\n",
        handler, *(omrsig_handler_fn *)value);
  }

  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_MODULE,
                           OMRPORT_SIG_MODULE_NAME, OMRPORT_SIG_VALUE_STRING,
                           TRUE, NULL, testName);

  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_SIGNAL,
                           OMRPORT_SIG_SIGNAL_ERROR_VALUE, OMRPORT_SIG_VALUE_32,
                           TRUE, NULL, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_SIGNAL,
                           OMRPORT_SIG_SIGNAL_ADDRESS,
  ...
  if (0 != (OMRPORT_SIG_OPTIONS_ZOS_USE_CEEHDLR & portTestOptionsGlobal)) {
    ...
    validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_SIGNAL,
                             OMRPORT_SIG_SIGNAL_PLATFORM_SIGNAL_TYPE,
  ...
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_SIGNAL,
                           OMRPORT_SIG_SIGNAL_INACCESSIBLE_ADDRESS,
                           OMRPORT_SIG_VALUE_ADDRESS, TRUE, NULL, testName);

  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_PC, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, NULL, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_SP, OMRPORT_SIG_VALUE_ADDRESS,
                           TRUE, NULL, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_BP, OMRPORT_SIG_VALUE_ADDRESS,
                           TRUE, NULL, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_S390_BEA,
                           OMRPORT_SIG_VALUE_ADDRESS, TRUE, NULL, testName);

  /*
   * x86 registers
   */
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_X86_EDI, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_X86, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_X86_ESI, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_X86, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_X86_EAX, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_X86, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_X86_EBX, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_X86, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_X86_ECX, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_X86, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_X86_EDX, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_X86, testName);
   ...
   */
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_RDI, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_RSI, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_RAX, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_RBX, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_RCX, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_RDX, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_R8, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_R9, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_R10, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_R11, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_R12, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_R13, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_R14, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_GPR,
                           OMRPORT_SIG_GPR_AMD64_R15, OMRPORT_SIG_VALUE_ADDRESS,
                           FALSE, OMRPORT_ARCH_HAMMER, testName);
  ...
  /* these registers are valid on both PPC32 and PPC64, so use whichever we're
   * running on if we are on a PPC */
  arch = OMRPORT_ARCH_PPC;
  if (0 == strcmp(OMRPORT_ARCH_PPC64, omrsysinfo_get_CPU_architecture())) {
    arch = OMRPORT_ARCH_PPC64;
  } else if (0 ==
             strcmp(OMRPORT_ARCH_PPC64LE, omrsysinfo_get_CPU_architecture())) {
    arch = OMRPORT_ARCH_PPC64LE;
  }

  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_POWERPC_LR,
                           OMRPORT_SIG_VALUE_ADDRESS, FALSE, arch, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_POWERPC_MSR,
                           OMRPORT_SIG_VALUE_ADDRESS, TRUE, arch, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_POWERPC_CTR,
                           OMRPORT_SIG_VALUE_ADDRESS, TRUE, arch, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_POWERPC_CR,
                           OMRPORT_SIG_VALUE_ADDRESS, TRUE, arch, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_POWERPC_FPSCR,
                           OMRPORT_SIG_VALUE_ADDRESS, TRUE, arch, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_POWERPC_XER,
                           OMRPORT_SIG_VALUE_ADDRESS, TRUE, arch, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_POWERPC_DAR,
                           OMRPORT_SIG_VALUE_ADDRESS, TRUE, arch, testName);
  validatePlatformRegister(OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL,
                           OMRPORT_SIG_CONTROL_POWERPC_DSIR,
                           OMRPORT_SIG_VALUE_ADDRESS, TRUE, arch, testName);

  /* this one is PPC32 only */
  validatePlatformRegister(
      OMRPORTLIB, gpInfo, OMRPORT_SIG_CONTROL, OMRPORT_SIG_CONTROL_POWERPC_MQ,
      OMRPORT_SIG_VALUE_ADDRESS, TRUE, OMRPORT_ARCH_PPC, testName);
}

static uintptr_t sampleFunction(OMRPortLibrary *portLibrary, void *arg) {
  return 8096;
}

static uintptr_t simpleHandlerFunction(struct OMRPortLibrary *portLibrary,
                                       uint32_t gpType, void *gpInfo,
                                       void *handler_arg) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  ...
  if (info->expectedType != gpType) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "simpleHandlerFunction -- incorrect type. Expecting 0x%x, got 0x%x\n",
        info->expectedType, gpType);
  }
...
static uintptr_t
currentSigNumHandlerFunction(struct OMRPortLibrary *portLibrary,
                             uint32_t gpType, void *gpInfo, void *handler_arg) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  SigProtectHandlerInfo *info = (SigProtectHandlerInfo *)handler_arg;
  ...
  if (currentSigNum < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "currentSigNumHandlerFunction -- "
                       "omrsig_get_current_signal call failed\n");
    return OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH;
  }
  if (gpType != (uint32_t)currentSigNum) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "currentSigNumHandlerFunction -- unexpected value for "
                       "current signal. Expecting 0x%x, got 0x%x\n",
...
static uintptr_t crashingHandlerFunction(struct OMRPortLibrary *portLibrary,
                                         uint32_t gpType, void *gpInfo,
                                         void *handler_arg) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
static uintptr_t nestedTestCatchSEGV(OMRPortLibrary *portLibrary, void *arg) {
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      protectResult =
          omrsig_protect(raiseSEGV, (void *)testName, simpleHandlerFunction,
                         &handlerInfo, flags, &result);

      if (protectResult != OMRPORT_SIG_EXCEPTION_OCCURRED) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "portLibrary->sig_protect -- expected "
                           "OMRPORT_SIG_EXCEPTION_OCCURRED return value\n");
      }

      if (result != 0) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "portLibrary->sig_protect -- expected 0 in *result\n");
      }
    }
  }
...
static uintptr_t nestedTestCatchNothing(OMRPortLibrary *portLibrary,
                                        void *arg) {
  uint32_t flags = 0;
  const char *testName = (const char *)arg;
  uintptr_t result;
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.returnValue = OMRPORT_SIG_EXCEPTION_RETURN;
    handlerInfo.testName = testName;
    (void)omrsig_protect(raiseSEGV, (void *)testName, simpleHandlerFunction,
                         &handlerInfo, flags, &result);
...
static uintptr_t nestedTestCatchAndIgnore(OMRPortLibrary *portLibrary,
                                          void *arg) {
...
static uintptr_t nestedTestCatchAndContinueSearch(OMRPortLibrary *portLibrary,
                                                  void *arg) {
...
static uintptr_t nestedTestCatchAndCrash(OMRPortLibrary *portLibrary,
                                         void *arg) {
  uint32_t flags = OMRPORT_SIG_FLAG_SIGSEGV;
  const char *testName = (const char *)arg;
  uintptr_t result;
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    (void)omrsig_protect(raiseSEGV, (void *)testName, crashingHandlerFunction,
                         (void *)testName, flags, &result);
...
TEST(PortSigTest, sig_test0) {
  ...
  if (!omrsig_can_protect(0)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->can_protect() must always support 0\n");
  }
  ...
  if (!omrsig_can_protect(OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       ...
                       "OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION\n");
#if defined(J9ZOS390)
    /* z/OS does not always support this, however, pltest should never be run
     * under a configuration that does not support it. See implementation of
     * omrsig_can_proctect() and omrsig_startup()
     */
    portTestEnv->log("OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION is not supported "
                     "if the default settings for TRAP(ON,SPIE) have been "
                     "changed. The should both be set to 1\n");
#endif
  }
...
TEST(PortSigTest, sig_test1) {
  ...
  if (!omrsig_can_protect(0)) {
    ...
    handlerInfo.returnValue = OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH;
    handlerInfo.testName = testName;
    protectResult = omrsig_protect(sampleFunction, NULL, simpleHandlerFunction,
                                   &handlerInfo, 0, /* protect from nothing */
                                   &result);

    if (protectResult != 0) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "portLibrary->sig_protect -- expected 0 return value\n");
    }

    if (result != 8096) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "portLibrary->sig_protect -- protected function did "
...
/*
 * Test protecting Function A against a SEGV with handler HA. A in turn protects
 *Function B against a SEGV with handler HB. Function B raises a sigsegv. HB
 *should handle it, returning OMRPORT_SIG_EXCEPTION_RETURN, control ends up
 *after the call to omrsig_protect in B. B then returns normally to A and then
 *omrsigTest3. A checks: omrsig_protect returns OMRPORT_SIG_EXCEPTION_OCCURRED
...
TEST(PortSigTest, sig_test3) {
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.testName = testName;
    protectResult =
        omrsig_protect(nestedTestCatchSEGV, (void *)testName,
                       simpleHandlerFunction, &handlerInfo, flags, &result);
    ...
    if (result != 8096) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "portLibrary->sig_protect -- expected 8096 in *result\n");
    }
  }
...
/*
 * Test protecting Function A against a SEGV with handler HA. A in turn protects
 *Function B against nothing with handler HB. Function B raises a sigsegv. HB
 *won't get invoked because it was not protecting against anything so HA will be
 *invoked, and return control to A (and then omrsig_test4).
 *
 * 		->SEGV--->--
 * 		|			|
 * 		|	 		|
...
TEST(PortSigTest, sig_test4) {
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.testName = testName;
    protectResult =
        omrsig_protect(nestedTestCatchNothing, (void *)testName,
                       simpleHandlerFunction, &handlerInfo, flags, &result);
 ...
 * Test protecting Function A against a SEGV with handler HA. A in turn protects
 * Function B against a SEGV with handler HB. Function B raises a sigsegv. HB
 * gets invoked, returns OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH so that HA gets
 * invoked, which then returns control to A (and then omrsig_test5).
 *
 * 		->SEGV
 * 		|	 |
 * 		|	 |
...
TEST(PortSigTest, sig_test5) {
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.testName = testName;
    protectResult =
        omrsig_protect(nestedTestCatchAndIgnore, (void *)testName,
                       simpleHandlerFunction, &handlerInfo, flags, &result);
...
TEST(PortSigTest, sig_test6) {
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.testName = testName;
    protectResult =
        omrsig_protect(nestedTestCatchAndCrash, (void *)testName,
                       simpleHandlerFunction, &handlerInfo, flags, &result);
...
TEST(PortSigTest, sig_test7) {
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    if (0 != currentSigNum) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "portLibrary->sig_protect -- expected 0 for current "
                         "signal, received 0x%X\n",
                         currentSigNum);
    }

    protectResult = omrsig_protect(raiseSEGV, (void *)testName,
 ...
 */
static uintptr_t callSigProtectWithContinueSearch(OMRPortLibrary *portLibrary,
                                                  void *arg) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  SigProtectHandlerInfo handlerInfo;
  uint32_t protectResult;
  ...
  handlerInfo.testName = testName;
  protectResult =
      omrsig_protect(nestedTestCatchAndContinueSearch, (void *)testName,
                     simpleHandlerFunction, &handlerInfo, flags, &result);
  if (protectResult != OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->sig_protect -- expected "
                       "OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH return value\n");
  }
  return result;
}
...
TEST(PortSigTest, sig_test8) {
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    handlerInfo.testName = testName;
    protectResult =
        omrsig_protect(callSigProtectWithContinueSearch, (void *)testName,
                       simpleHandlerFunction, &handlerInfo, flags, &result);
    ...
    if (protectResult != OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "portLibrary->sig_protect -- expected "
          "OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH in protectResult\n");
    }
#else
    if (protectResult != OMRPORT_SIG_EXCEPTION_OCCURRED) {
      outputErrorMessage(
 ...
 * -child_omrsig_injectSignal_<PID>_<signal to inject>. In the child process:
 * 	- main.c peels off -child_
 * 	- omrsig_run_tests looks for omrsig_injectSignal, and if found, reads
 * the PID and signal number and injects it
 *
 * NOTE : Assumes 0.5 seconds is long enough for a child process to be kicked
 * off, inject a signal and have our handler kick in
 * */
TEST(PortSigTest, sig_test_async_unix_handler) {
  ...
  monitorRC = omrthread_monitor_init_with_name(&asyncMonitor, 0,
                                               "omrsignalTest_async_monitor");

  if (0 != monitorRC) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrthread_monitor_init_with_name failed with %i\n",
                       monitorRC);
    FAIL();
  }
  ...
  setAsyncRC = omrsig_set_async_signal_handler(
      asyncTestHandler, &handlerInfo,
  ...
  if (setAsyncRC == (uint32_t)OMRPORT_SIG_ERROR) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrsig_set_async_signal_handler returned: OMRPORT_SIG_ERROR\n");
    goto exit;
  }

#if defined(AIXPPC)
  setAsyncRC = omrsig_set_async_signal_handler(asyncTestHandler, &handlerInfo,
                                               OMRPORT_SIG_FLAG_SIGRECONFIG);
  ...
  for (index = 0; index < sizeof(testSignalMap) / sizeof(testSignalMap[0]);
       index++) {
    ...
    if (0 != (OMRPORT_SIG_OPTIONS_ZOS_USE_CEEHDLR & portTestOptionsGlobal)) {
#if defined(J9ZOS390)
      /* This prevents LE from sending the corresponding LE condition to our
       * thread */
      sighold(signum);
      raise(signum);
      sigrelse(signum);
    ...
    /* we don't want to hang - if we haven't been notified in 20 seconds it
     * doesn't work */
    ...
    /* build the pid and signal number in the form
     * "-child_omrsig_injectSignal_<PID>_<signal>" */
    omrstr_printf(options, SIG_TEST_SIZE_EXENAME,
                  "-child_omrsig_injectSignal_%i_%i", pid, signum);
    portTestEnv->log("\t\tlaunching child process with arg %s\n", options);

    omrthread_monitor_enter(asyncMonitor);

    childProcess = launchChildProcess(OMRPORTLIB, testName,
                                      portTestEnv->_argv[0], options);

    if (NULL == childProcess) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Cannot launch test process! Can not perform test!");
      omrthread_monitor_exit(asyncMonitor);
      goto exit;
    }
    ...
    if (1 != handlerInfo.controlFlag) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "%s handler was NOT invoked when %s was injected by "
                         "another process\n",
                         testSignalMap[index].portLibSignalString,
                         testSignalMap[index].unixSignalString);
    }
...
#if defined(J9ZOS390)
    /*
     * Verify that async signals can't be received by the asyncSignalReporter
     * thread. pltest runs with 2 threads only, this thread and the
     * asyncSignalReporter thread. We will mask the signal on this thread,
     * inject it via kill, and verify that it does not get received.
     */
    portTestEnv->log("\t\tmasking signal %i on this thread\n", signum);
    sighold(signum);

    /* build the pid and signal number in the form
     * "-child_omrsig_injectSignal_<PID>_<signal>_unhandled" */
    omrstr_printf(options, SIG_TEST_SIZE_EXENAME,
                  "-child_omrsig_injectSignal_%i_%i_unhandled", pid, signum);
    portTestEnv->log("\t\tlaunching child process with arg %s\n", options);
    ...
    if (0 != handlerInfo.controlFlag) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "%s handler was unexpectedly invoked when %s was "
