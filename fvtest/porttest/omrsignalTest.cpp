...
static uintptr_t asyncTestHandler(struct OMRPortLibrary* portLibrary,
                                  ...
                                  void* gpInfo,
                                  void* userData);
static void injectUnixSignal(struct OMRPortLibrary* portLibrary,
                             int pid,
                             int unixSignal);
 ...
 */
int32_t omrsig_runTests(struct OMRPortLibrary* portLibrary,
                        char* exeName,
                        char* argument) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
void validateGPInfo(struct OMRPortLibrary* portLibrary,
                    uint32_t gpType,
...
TEST(PortSigTest, sig_test7) {
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    protectResult =
        omrsig_protect(raiseSEGV, (void*)testName, currentSigNumHandlerFunction,
                       &handlerInfo, flags, &result);
