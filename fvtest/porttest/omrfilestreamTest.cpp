 ...
 */
static intptr_t filestream_write_all(struct OMRPortLibrary* portLibrary, const char* testName,
  OMRFileStream* fileStream, const char* output, intptr_t nbytes)
{
  intptr_t rc = 0;
 ...
 */
static intptr_t file_write_all(
 ...
 */
static intptr_t file_read_all(
  struct OMRPortLibrary* portLibrary, const char* testName, intptr_t file, char* buf, intptr_t nbytes)
{
  intptr_t rc = 0;
...
TEST(PortFileStreamTest, omrfilestream_test_sync)
{
  ...
  if (readCount != expectedReturnValue) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrfilestream_write() returned %d expected %d\n", readCount, expectedReturnValue);
    goto closeFile;
  }
...
TEST(PortFileStreamTest, omrfilestream_test_fdopen_and_fileno)
{
  ...
  if (resultingOffset != 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrfiles_seek() returned %lld expected %lld\n", resultingOffset, (int64_t)0);
    goto test1CloseFile;
  }
...
TEST(PortFileStreamTest, omrfilestream_test_buffering)
{
  ...
  if (characterCount > rc) {
    /* read from the file when it should be in the buffer.  Not sure if the c standard guarantees it will respect your
     * buffering wishes */
    outputErrorMessage(PORTTEST_ERROR_ARGS, "file_read_all() returned %d expected %d\n", rc, characterCount);
    goto test1CloseFile;
  }
...
TEST(PortFileStreamTest, omrfilestream_test_omrfile_text_comparison)
{
  ...
  if (rc != rc2) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
      "omrfile_read on filestream returned %d bytes, while omrfile_read on file returned %d bytes", rc, rc2);
    goto closeFiles;
  } else if (strncmp(fileInput, streamInput, rc)) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "Stream data \"%s\" not matching file data \"%s\"\n", streamInput, fileInput);
  }
...
TEST(PortFileStreamTest, omrfilestream_test_invalid_filestream_access)
{
  ...
  validStream = omrfilestream_fdopen(
    omrfile_open(fileName3, EsOpenCreate | EsOpenRead | EsOpenWrite, 0666), EsOpenCreate | EsOpenRead | EsOpenWrite);
  if (NULL == validStream) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrfilestream_open() returned NULL expected valid file handle\n");
    goto unlinkFiles;
  }
...
writeTextInvalidFDEnd:
  ...
  if (rc != -1) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrfilestream_fileno() returned %d, expected invalid file descriptor -1\n", rc);
  }
