 ...
 * writing.
 */
static intptr_t filestream_write_all(struct OMRPortLibrary *portLibrary,
                                     const char *testName,
  ...
  while (bytesRemaining > 0) {
    rc = omrfilestream_write(fileStream, output + (nbytes - bytesRemaining),
                             bytesRemaining);
    if (rc < 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrfilestream_write() returned %d", rc);
      nbytes = rc;
      break;
    }
    bytesRemaining -= rc;
 ...
 * writing.
 */
static intptr_t file_write_all(struct OMRPortLibrary *portLibrary,
                               const char *testName, intptr_t file,
                               const char *output, intptr_t nbytes) {
  intptr_t rc = 0;
  intptr_t bytesRemaining = nbytes;
  ...
  while (bytesRemaining > 0) {
    ...
    if (rc < 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS, "omrfile_write() returned %d",
                         rc);
      nbytes = rc;
      break;
    }
 ...
 */
static intptr_t file_read_all(struct OMRPortLibrary *portLibrary,
                              const char *testName, intptr_t file, char *buf,
                              intptr_t nbytes) {
  intptr_t rc = 0;
  ...
  if (currentOffset < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrfile_seek() returned %d",
                       currentOffset);
    return -1;
  }
 ...
 */
static void openFlagTest(struct OMRPortLibrary *portLibrary,
                         const char *testName, const char *pathName,
...
TEST(PortFileStreamTest, omrfilestream_test_open_modes) {
  ...
  openFlagTest(portTestEnv->getPortLibrary(), testName, path3,
               EsOpenCreate | EsOpenWrite, 0666);
  /* flags = EsOpenWrite */
  openFlagTest(portTestEnv->getPortLibrary(), testName, path3,
               EsOpenWrite | EsOpenCreate, 0666);
...
TEST(PortFileStreamTest, omrfilestream_test_basic_write_then_read) {
  ...
  rc = filestream_write_all(OMRPORTLIB, testName, filestream, outputLine,
                            sizeof(outputLine));
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
...
cleanupFD:
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_close() returned %d expected %d\n", rc, 0);
  }
...
TEST(PortFileStreamTest, omrfilestream_test_write_append) {
  ...
  filestream_write_all(OMRPORTLIB, testName, filestream, outputLine1,
  ...
  /* append to the file */
  filestream_write_all(OMRPORTLIB, testName, filestream, outputLine2,
  ...
  rc = file_read_all(OMRPORTLIB, testName, file, inputBuffer,
                     sizeof(inputBuffer));
...
closeFD:
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_close() returned %d expected 0\n", rc);
  }
...
#define FILENAME_LENGTH                                                        \
  ...
  for (i = 0; i < 2; i++) {
    ...
    rc = file_read_all(OMRPORTLIB, testName, fd, inputBuffer,
                       sizeof(inputBuffer));
  ...
  cleanupFD:
    ...
    if (rc != 0) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "omrfile_close() returned %d, expected %d", rc, 0);
    }
...
TEST(PortFileStreamTest, omrfilestream_test_sync) {
  ...
  readCount = file_read_all(OMRPORTLIB, testName, readFile, inputLine,
                            expectedReturnValue);
  if (readCount != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "file_read_all() returned %d, expected %d\n", readCount,
   ...
   */
  totalReadCount = file_read_all(OMRPORTLIB, testName, readFile, inputLine,
                                 expectedReturnValue);
  if (totalReadCount < 0) {
  ...
  readCount = file_read_all(OMRPORTLIB, testName, readFile,
                            inputLine + totalReadCount, expectedReturnValue);
...
TEST(PortFileStreamTest, omrfilestream_test_fdopen_and_fileno) {
  ...
  rc = file_write_all(OMRPORTLIB, testName, fileDescriptor, output,
                      characterCount);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
...
test3:
  ...
  byteCount = file_read_all(OMRPORTLIB, testName, fileDescriptor, input,
                            characterCount);
...
TEST(PortFileStreamTest, omrfilestream_test_buffering) {
  ...
  rc = omrfilestream_setbuffer(writeFile, buffer,
                               OMRPORT_FILESTREAM_FULL_BUFFERING, 256);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
  ...
  rc = filestream_write_all(OMRPORTLIB, testName, writeFile, output,
                            characterCount);
  ...
  if (0 > rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_sync() failed: returned %d\n", rc);
    goto test1CloseFile;
  }
...
test2:
  ...
  rc = omrfilestream_setbuffer(writeFile, NULL,
                               OMRPORT_FILESTREAM_FULL_BUFFERING, 256);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_setbuffer failed to set the buffer\n");
  ...
  if (0 > rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_synced() failed: returned %d\n", rc);
    goto test2CloseFile;
  }
...
test3:
  ...
  rc = omrfilestream_setbuffer(writeFile, NULL,
                               OMRPORT_FILESTREAM_LINE_BUFFERING, 256);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_setbuffer failed to set the buffer\n");
    goto test3CloseFile;
  ...
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "file_read_all() returned %d success\n", rc);
    goto test3CloseFile;
  }
  ...
  rc = file_read_all(OMRPORTLIB, testName, readFile, input + readCount,
                     characterCount);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "file_read_all() returned %d success\n", rc);
...
test4:
  ...
  rc = omrfilestream_setbuffer(writeFile, NULL, OMRPORT_FILESTREAM_NO_BUFFERING,
                               256);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_setbuffer failed to set the buffer\n");
...
test6:
  ...
  rc = omrfilestream_setbuffer(writeFile, NULL,
                               OMRPORT_FILESTREAM_FULL_BUFFERING, 1);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_setbuffer failed to set the buffer\n");
    goto test6CloseFile;
...
TEST(PortFileStreamTest, omrfilestream_test_text) {
  ...
  rc = omrfilestream_write_text(filestream, mutf8String, mutf8StringSize,
                                J9STR_CODE_MUTF8);
  if (rc < 0) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrfilestream_write_text returned negative error code: %d", rc);
  ...
  if (rc != (expectedmutf8Size)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_read returned %d bytes, expected: %d ", rc,
                       expectedmutf8Size);
    goto mutf8TestCloseFiles;
...
wideTest:
  ...
  rc = omrfilestream_write_text(filestream, "\xe2\x82", sizeof("\xe2\x82") - 1,
                                J9STR_CODE_WIDE);
  if (rc >= 0) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
...
TEST(PortFileStreamTest, omrfilestream_test_omrfile_text_comparison) {
  ...
  rc = omrfilestream_write_text(fileStream, output, outputCount,
                                J9STR_CODE_PLATFORM_RAW);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
  ...
  if (rc != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_write_text() failed: returned %d\n", rc);
  }
  ...
  rc = file_read_all(OMRPORTLIB, testName, fileStreamFile, streamInput,
                     sizeof(streamInput));
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "file_read_all() returned %d expected %d\n", rc, 0);
...
TEST(PortFileStreamTest, omrfilestream_test_invalid_filestream_access) {
  ...
  if (NULL != noAccessStream) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_open() returned non-null, expected "
                       "invalid file handle\n");
  ...
  rc = omrfilestream_setbuffer(invalidStream, NULL,
...
writeInvalidFDEnd:
  ...
  rc = omrfilestream_write_text(validStream, NULL, sizeof(output),
                                J9STR_CODE_MUTF8);
  if (rc >= 0) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
  ...
  omrfilestream_sync(validStream);
  rc = omrfilestream_write_text(roStream, output, sizeof(output),
                                J9STR_CODE_MUTF8);
  if (rc >= 0) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
  ...
  rc = omrfilestream_write_text(invalidStream, output, sizeof(output),
                                J9STR_CODE_MUTF8);
  if (rc >= 0) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
...
writeTextInvalidFDEnd:
   ...
   * setbuffer
   */
  rc = omrfilestream_setbuffer(NULL, NULL, OMRPORT_FILESTREAM_FULL_BUFFERING,
                               256);
  if (rc >= 0) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
  ...
  rc = omrfilestream_setbuffer(validStream, NULL,
                               ~OMRPORT_FILESTREAM_FULL_BUFFERING, 256);
  if (rc >= 0) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
