...
/**
 * Write all data in the buffer. Will write in a loop until all data is sent. If
 * an error occurs, it will stop writing, output an error message, and stop
 * writing.
 */
static intptr_t
filestream_write_all(struct OMRPortLibrary* portLibrary,
                     const char* testName,
                     ...
                     intptr_t nbytes)
{
  ...
  while (bytesRemaining > 0) {
    rc = omrfilestream_write(
      fileStream, output + (nbytes - bytesRemaining), bytesRemaining);
    if (rc < 0) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "omrfilestream_write() returned %d", rc);
      nbytes = rc;
      break;
    }
    bytesRemaining -= rc;
 ...
 */
static intptr_t
file_write_all(struct OMRPortLibrary* portLibrary,
               const char* testName,
               intptr_t file,
               const char* output,
               intptr_t nbytes)
{
  ...
  while (bytesRemaining > 0) {
    rc =
      omrfile_write(file, output + (nbytes - bytesRemaining), bytesRemaining);
    if (rc < 0) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "omrfile_write() returned %d", rc);
      nbytes = rc;
      break;
    }
    bytesRemaining -= rc;
...
/**
 * Read until the supplied buffer is full, or end of file is reached.  Returns
 * the number of bytes written to the buffer on success (0 bytes is success), or
 * negative error code on failure.
 */
static intptr_t
file_read_all(struct OMRPortLibrary* portLibrary,
              const char* testName,
              intptr_t file,
              char* buf,
              intptr_t nbytes)
{
  ...
  /* omrfile_read will return -1 if the end of the file is reached.  We want to
  ...
  if (currentOffset < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrfile_seek() returned %d", currentOffset);
    return -1;
  }
  fileEndOffset = omrfile_seek(file, 0, EsSeekEnd);
  if (fileEndOffset < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrfile_seek() returned %d", fileEndOffset);
    return -1;
  }
...
/**
 * Verify port library properly setup to run filestream tests. Check to make
 * sure all the functions are in the function table.
 *
...
TEST(PortFileStreamTest, omrfilestream_test_function_table)
{
  ...
  if (NULL == OMRPORTLIB->filestream_startup) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->filestream_startup is NULL\n");
  }
  if (NULL == OMRPORTLIB->filestream_shutdown) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->filestream_shutdown is NULL\n");
  }
  if (NULL == OMRPORTLIB->filestream_open) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->filestream_open is NULL\n");
  }
  if (NULL == OMRPORTLIB->filestream_close) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->filestream_close is NULL\n");
  }
  if (NULL == OMRPORTLIB->filestream_write) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->filestream_write is NULL\n");
  }
  if (NULL == OMRPORTLIB->filestream_vprintf) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->filestream_vprintf is NULL\n");
  }
  if (NULL == OMRPORTLIB->filestream_printf) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->filestream_printf is NULL\n");
  }
  if (NULL == OMRPORTLIB->filestream_sync) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->filestream_sync is NULL\n");
  }
  if (NULL == OMRPORTLIB->filestream_setbuffer) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->filestream_setbuffer is NULL\n");
  }
  if (NULL == OMRPORTLIB->filestream_fdopen) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->filestream_fdopen is NULL\n");
  }
  if (NULL == OMRPORTLIB->filestream_fileno) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->filestream_fileno is NULL\n");
  }
...
/**
 * @internal
 * Verify that file open flags and modes are working correctly.  This will
 * create the file, open and close the file twice, and verify that the mode and
 * flags are set.
 *
 * @param[in] portLibrary The port library under test.
             ...
             int32_t mode)
{
  ...
  if (NULL == file) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned NULL expected valid file handle\n");
    goto exit;
  }
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_close() returned %d expected %d\n",
                       rc,
                       0);
    goto exit;
  }
  ...
  if (NULL == file) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned %d expected valid file handle\n",
      -1);
    goto exit;
  }
...
/**
 * Check that opening a file correctly uses modes. Verify that file open flags
 * and modes are working correctly.  This will create the file, open and close
 * the file twice, and verify that the mode and flags are set.
 *
 * @param[in] portLibrary The port library under test.
 *
 * @return TEST_PASS on success, TEST_FAIL on failure.
...
TEST(PortFileStreamTest, omrfilestream_test_open_modes)
{
  ...
  /* Make sure none of the files exist prior to starting the test, ignoring
  ...
  openFlagTest(portTestEnv->getPortLibrary(),
               testName,
               path3,
               EsOpenCreate | EsOpenWrite,
               0666);
  /* flags = EsOpenWrite */
  openFlagTest(portTestEnv->getPortLibrary(),
               testName,
               path3,
               EsOpenWrite | EsOpenCreate,
               0666);
  ...
  openFlagTest(portTestEnv->getPortLibrary(),
               testName,
               path3,
               EsOpenText | EsOpenCreate | EsOpenWrite,
               0666);
  ...
  if (NULL != file2) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned valid file handle, expected failure\n");
  ...
  file2 =
    omrfilestream_open(path2, EsOpenCreate | EsOpenWrite | EsOpenRead, 0444);
  if (NULL != file2) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned valid file handle, expected failure\n");
  ...
  /*
   * Check that opening a non-existent file will fail when not told to create
   * the file
   */
  ...
  if (NULL == file1) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned %d expected file handle\n",
      -1);
    goto exit;
  }
  ...
  if (NULL != file2) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned valid file handle expected -1\n");
  }
 ...
 *
 * Verify basic @ref j9file::omrfilestream_write "omrfilestream_write()"
 * operations. Test by writing to the file, reopening it and reading back the
 * contents.
 *
 * @param[in] portLibrary The port library under test
 *
...
TEST(PortFileStreamTest, omrfilestream_test_basic_write_then_read)
{
  ...
  rc = filestream_write_all(
    OMRPORTLIB, testName, filestream, outputLine, sizeof(outputLine));
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "filestream_write_all() returned %d expected %d\n",
                       rc,
                       sizeof(outputLine));
  }
  ...
  /* Reopen the same file, read back what we wrote (verifying the contents), and
   * close the file */
  file = omrfile_open(fileName, EsOpenRead, 0444);
  if (file < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_open() returned error, expected valid file handle\n",
      -1);
    goto unlinkFile;
  }
  ...
  if (rc != sizeof(outputLine)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "file_read_all() returned %d, expected %d\n");
    goto cleanupFD;
  }
...
cleanupFD:
  ...
  if (0 != rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrfile_close() returned %d expected %d\n", rc, 0);
  }
...
/**
 * Verify write appending to a file. Verify @ref
 * omrfilestream::omrfilestream_write "omrfilestream_write()" append operations.
 *
...
TEST(PortFileStreamTest, omrfilestream_test_write_append)
{
  ...
  filestream = omrfilestream_open(
    fileName, EsOpenWrite | EsOpenCreate | EsOpenTruncate, 0666);
  if (NULL == filestream) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned %d expected valid file handle\n",
  ...
  filestream_write_all(
    OMRPORTLIB, testName, filestream, outputLine1, sizeof(outputLine1) - 1);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "filestream_write_all() returned %d expected %d\n",
                       rc,
  ...
  /* append to the file */
  filestream_write_all(
    OMRPORTLIB, testName, filestream, outputLine2, sizeof(outputLine2) - 1);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "filestream_write_all() returned %d expected %d\n",
                       rc,
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_close() returned %d expected 0\n",
                       rc);
  }
  ...
  if (file < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_open() returned %d expected valid file handle\n",
      -1);
    goto unlinkFile;
  }
  ...
  rc =
    file_read_all(OMRPORTLIB, testName, file, inputBuffer, sizeof(inputBuffer));
...
closeFD:
  ...
  if (0 != rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrfile_close() returned %d expected 0\n", rc);
  }
...
TEST(PortFileStreamTest, omrfilestream_test_file_owner_and_group_attributes)
{
  ...
  const char* filename =
    "omrfilestream_test_file_owner_and_group_attributes.tst";
  ...
  file = omrfilestream_open(
    filename, EsOpenCreateNew | EsOpenWrite | EsOpenTruncate, 0666);
  if (NULL == file) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_open failed to create file %s: "
                       "error_last_error_number %i\n",
                       filename,
...
TEST(PortFileStreamTest, omrfilestream_test_open_permission_flags)
{
  ...
  if (NULL == file) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_open() file open failed\n");
    goto readOnlyTest;
  }
  rc = omrfilestream_close(file);
  if (0 != rc) {
...
#if defined(OMR_OS_WINDOWS)
  /*The mode param is ignored by omrfilestream_open, so omrfile_chmod() is
   * called*/
  ...
  if (stat.perm.isUserWriteable != 1) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_stat() error 'stat.perm.isUserWriteable != 1'\n");
  }
  if (stat.perm.isUserReadable != 1) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_stat() error 'stat.perm.isUserReadable != 1'\n");
  }
#if defined(OMR_OS_WINDOWS)
  if (stat.perm.isGroupWriteable != 1) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_stat() error 'stat.perm.isGroupWriteable != 1'\n");
  }
  if (stat.perm.isGroupReadable != 1) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_stat() error 'stat.perm.isGroupReadable != 1'\n");
  }
  if (stat.perm.isOtherWriteable != 1) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_stat() error 'stat.perm.isOtherWriteable != 1'\n");
  }
  if (stat.perm.isOtherReadable != 1) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_stat() error 'stat.perm.isOtherReadable != 1'\n");
  }
   ...
   * omrfilestream_open() will not open a filestream for reading.  This test
   * makes sure that file permissions won't be destroyed when using
   * omrfilestream_fdopen().
   */
readOnlyTest:
  ...
  if (stat.perm.isUserWriteable != 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_stat() error 'stat.perm.isUserWriteable != 0'\n");
  }
  if (stat.perm.isUserReadable != 1) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
  ...
  if (stat.perm.isGroupWriteable != 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_stat() error 'stat.perm.isGroupWriteable != 0'\n");
  }
  if (stat.perm.isGroupReadable != 1) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
  ...
  if (stat.perm.isOtherWriteable != 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_stat() error 'stat.perm.isOtherWriteable != 0'\n");
  }
  if (stat.perm.isOtherReadable != 1) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
...
writeOnlyTest:
  ...
  if (stat.perm.isUserReadable != 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_stat() error 'stat.perm.isUserReadable != 0'\n");
  }
#endif /* defined(OMR_OS_WINDOWS) */
...
#else  /* defined(OMR_OS_WINDOWS) */
  /*umask vary so we can't test 'isGroupWritable', 'isGroupReadable',
   * 'isOtherWritable', or 'isOtherReadable'*/
#endif /* defined(OMR_OS_WINDOWS) */
...
#if defined(OMR_OS_WINDOWS)
/**
 * Test a very long file name. For both relative and absolute paths, build up a
 * directory hierarchy that is greater than the system defined MAX_PATH.
 *
 * @param[in] portLibrary The port library under test.
 *
 * @return TEST_PASS on success, TEST_FAIL on error
 *
 * @note that this test is only run on windows, as it is the only OS
 * (encountered so far) that supports file names longer than 256 chars.
 */
...
#define FILENAME_LENGTH                                                        \
  MIN_LENGTH * 2 /* double the size to accommodate a file size that is just    \
  ...
  for (i = 0; i < 2; i++) {
    ...
    /* Reopen the same file, read back what we wrote (verifying the contents),
     * and close the file */
    ...
    rc =
      file_read_all(OMRPORTLIB, testName, fd, inputBuffer, sizeof(inputBuffer));
  ...
  cleanupFD:
    ...
    if (rc != 0) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "omrfile_close() returned %d, expected %d", rc, 0);
    }
...
/**
 * Verify flush.  Check to make sure that a flush will write all data to the
 * file.
 *
 * @param[in] portLibrary The port library under test
 * @return TEST_PASS on success, TEST_FAIL on error
...
TEST(PortFileStreamTest, omrfilestream_test_sync)
{
  ...
  writeFile = omrfilestream_open(
    fileName, EsOpenCreate | EsOpenWrite | EsOpenAppend, 0666);
  if (NULL == writeFile) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned -1 expected valid file handle\n");
    goto unlinkFile;
  }
  ...
  readCount = file_read_all(
    OMRPORTLIB, testName, readFile, inputLine, expectedReturnValue);
  if (readCount != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "file_read_all() returned %d, expected %d\n",
                       readCount,
  ...
  if (readCount != expectedReturnValue) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_write() returned %d expected %d\n",
                       readCount,
                       expectedReturnValue);
    goto closeFile;
  }
   ...
   */
  totalReadCount = file_read_all(
    OMRPORTLIB, testName, readFile, inputLine, expectedReturnValue);
  if (totalReadCount < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "file_read_all() returned %d \n", totalReadCount);
    goto closeFile;
  }
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_sync() returned %d expected %d\n",
                       rc,
                       expectedReturnValue);
    goto closeFile;
  }
  ...
  readCount = file_read_all(OMRPORTLIB,
                            testName,
                            readFile,
                            inputLine + totalReadCount,
                            expectedReturnValue);
...
TEST(PortFileStreamTest, omrfilestream_test_fdopen_and_fileno)
{
  ...
  /*
   * Open a file, open a filestream on the file, get the file descriptor from
   * the filestream and make sure that it stays the same
   */

  /* open a file descriptor */
  fileDescriptor =
    omrfile_open(filename, EsOpenCreate | EsOpenWrite | EsOpenTruncate, 0666);
  if (fileDescriptor < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_open failed to create file descriptor\n");
    goto test2;
  }
  ...
  if (NULL == file) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_fdopen failed to create file %s: "
                       "error_last_error_number %i\n",
                       filename,
                       omrerror_last_error_number());
  ...
  /*
   * Get a stream from a filedescriptor, and write to the descriptor, then write
   * to the stream,
   */
  ...
  rc = file_write_all(
    OMRPORTLIB, testName, fileDescriptor, output, characterCount);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "file_write_all() returned %d expected %d\n",
                       rc,
                       characterCount);
  }
  ...
  /* make sure the file descriptor position is correct after the written text in
   * the file */
  ...
  /* go back to the begining of the file (the filestream and file share a
   * position) */
  resultingOffset = omrfile_seek(fileDescriptor, 0, EsSeekSet);
  if (resultingOffset != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfiles_seek() returned %lld expected %lld\n",
                       resultingOffset,
                       (int64_t)0);
    goto test1CloseFile;
  }

  /* write to the file stream. This should overwrite everything written to the
   * file descriptor */
...
test1CloseFile:
  ...
  /*
   * Get a stream from a filedescriptor, write to the stream, write to the
   * filedescriptor.
   */
test2:
  /* open a file descriptor */
...
test2CloseFile:
  ...
  /*
   * Get a filestream from the file descriptor, write to the filestream, read
   * from the file descriptor
   */
test3:
  /* open a file descriptor */
  fileDescriptor =
    omrfile_open(filename, EsOpenCreate | EsOpenRead | EsOpenWrite, 0666);
  if (fileDescriptor < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_open failed to create file descriptor\n");
    goto exit;
  ...
  file = omrfilestream_fdopen(fileDescriptor,
                              EsOpenCreate | EsOpenRead | EsOpenWrite);
  if (NULL == file) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_fd open failed to open fd for: %s: "
                       "error_last_error_number %i\n",
                       filename,
                       omrerror_last_error_number());
    omrfile_close(fileDescriptor);
  ...
  byteCount =
    file_read_all(OMRPORTLIB, testName, fileDescriptor, input, characterCount);
...
/**
 * Test stream buffering. This test will write to the stream using different
 * buffering methods. With a different stream to the same file, it will ensure
 * that the proper file buffering works correctly.
 *
 * @param[in] portLibrary The port library under test
 *
 * @return TEST_PASS on success, TEST_FAIL on error
...
TEST(PortFileStreamTest, omrfilestream_test_buffering)
{
  ...
  /* Make the buffer large enough to avoid all file writes using full buffering.
   * Reading from the stream should return nothing. After flushing writes to the
   * file, read the text.
   */

  /* open a filestream for writing */
  writeFile = omrfilestream_open(
    filename, EsOpenCreate | EsOpenWrite | EsOpenTruncate, 0666);
  if (NULL == writeFile) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned NULL expected valid file handle\n");
  ...
  if (NULL == buffer) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrmem_allocate_memory failed to allocate buffer of size 256\n");
    omrfilestream_close(writeFile);
  ...
  rc = omrfilestream_setbuffer(
    writeFile, buffer, OMRPORT_FILESTREAM_FULL_BUFFERING, 256);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_setbuffer failed to set the buffer\n");
    goto test1CloseFile;
  }

  /* write to the file, it should all be in the buffer */
  rc = filestream_write_all(
    OMRPORTLIB, testName, writeFile, output, characterCount);
  ...
  if (0 > rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrfilestream_sync() failed: returned %d\n", rc);
    goto test1CloseFile;
  }
  ...
  if (characterCount > rc) {
    /* read from the file when it should be in the buffer.  Not sure if the c
     * standard guarantees it will respect your buffering wishes */
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "file_read_all() returned %d expected %d\n",
                       rc,
                       characterCount);
...
test2:
  ...
  rc = omrfilestream_setbuffer(
    writeFile, NULL, OMRPORT_FILESTREAM_FULL_BUFFERING, 256);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_setbuffer failed to set the buffer\n");
    goto test2CloseFile;
  ...
  if (0 > rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrfilestream_synced() failed: returned %d\n", rc);
    goto test2CloseFile;
  }
...
test2CloseFile:
  ...
  /*
   * Attempt to line buffer output, checking flushed output. After flushing the
   * entire buffer, make sure the strings are the same.  Note that there is no
   * guarantee that line buffering will actually line buffer.  The only
   * guarantee is that all text has been written to the file after a
   * omrfilestream_sync.
   */
test3:
  /* open a filestream for writing */
  ...
  rc = omrfilestream_setbuffer(
    writeFile, NULL, OMRPORT_FILESTREAM_LINE_BUFFERING, 256);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_setbuffer failed to set the buffer\n");
    goto test3CloseFile;
  ...
  readCount =
    file_read_all(OMRPORTLIB, testName, readFile, input, characterCount);
  if (rc < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "file_read_all() returned %d success\n", rc);
    goto test3CloseFile;
  }
  ...
  rc = file_read_all(
    OMRPORTLIB, testName, readFile, input + readCount, characterCount);
  if (rc < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "file_read_all() returned %d success\n", rc);
    goto test3CloseFile;
...
test3CloseFile:
  ...
  /*
   * Attempt to use no buffering.  Ensure that flushing wont write more
...
test4:
  ...
  rc = omrfilestream_setbuffer(
    writeFile, NULL, OMRPORT_FILESTREAM_NO_BUFFERING, 256);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_setbuffer failed to set the buffer\n");
    goto test4CloseFile;
...
test4CloseFile:
  ...
  /* Test full buffering with omrfilestream_setbuffer.  Make the buffer large
   * enough to avoid all file writes using full buffering. Reading from the
   * stream should return nothing. Flush the file write, and then read the text.
   */
test5:
  writeFile = omrfilestream_open(
    filename, EsOpenCreate | EsOpenWrite | EsOpenTruncate, 0666);
...
test5CloseFile:
  ...
  /* Test buffering with a small buffer.  On Windows, if the buffer size is less
   * than 2, it should set the minimum buffer size to 2.
   */
test6:
  writeFile = omrfilestream_open(
    filename, EsOpenCreate | EsOpenWrite | EsOpenTruncate, 0666);
  ...
  rc = omrfilestream_setbuffer(
    writeFile, NULL, OMRPORT_FILESTREAM_FULL_BUFFERING, 1);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_setbuffer failed to set the buffer\n");
    goto test6CloseFile;
...
/**
 * Verify that omrfilestream_write_text is working.  Try writing using different
 * variables, with a mix of utf-8 characters with different byte-lengths.
 *
 * @param[in] portLibrary The port library under test
 *
 * @return TEST_PASS on success, TEST_FAIL on error
...
TEST(PortFileStreamTest, omrfilestream_test_text)
{
  ...
  /* utf-16 */
#if defined(CRLFNEWLINES)
  const U_16 wideData[] = { 0x24, 0x0d,   0x0a, 0xa2, 0x0d,
                            0x0a, 0x20ac, 0x0d, 0x0a };
#else  /* defined (CRLFNEWLINES) */
  ...
  if (fileDescriptor < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_open() returned NULL expected valid file handle\n");
    omrfilestream_close(filestream);
    goto wideTest;
  }

  /* correct mutf-8 - basic string */
  rc = omrfilestream_write_text(
    filestream, mutf8String, mutf8StringSize, J9STR_CODE_MUTF8);
  if (rc < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_write_text returned negative error code: %d",
      rc);
  ...
  rc =
    file_read_all(OMRPORTLIB, testName, fileDescriptor, input, sizeof(input));
  if (rc < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "file_read_all() returned %d expected %d\n", rc, 0);
    goto mutf8TestCloseFiles;
  ...
  if (rc != (expectedmutf8Size)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_read returned %d bytes, expected: %d ",
                       rc,
                       expectedmutf8Size);
    goto mutf8TestCloseFiles;
  } else if (strcmp(input, expectedmutf8)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "File data \"%s\" not matching expecting \"%s\"\n",
                       input,
                       expectedmutf8);
    goto mutf8TestCloseFiles;
  }
  ...
  if (rc != 2) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_write_text() returned %d, expected error code\n",
      rc);
    goto mutf8TestCloseFiles;
  }
  rc = omrfilestream_sync(filestream);
  ...
  if (memcmp(input, "\xc0\x80", 2)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "File data \"%s\" not matching expected \"%s\"\n",
                       input,
                       "\xc0\x80");
    goto mutf8TestCloseFiles;
  }
...
wideTest:
  ...
  rc = omrfilestream_write_text(
    filestream, mutf8String, mutf8StringSize, J9STR_CODE_WIDE);
  if (rc < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_write_text returned negative error code: %d",
  ...
  rc = omrfilestream_write_text(
    filestream, "\xe2\x82", sizeof("\xe2\x82") - 1, J9STR_CODE_WIDE);
  if (rc >= 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_write_text() returned %d, expected error code\n",
...
platformTest:
  ...
  /* this write can fail.  Some platform encodings do not support mutf-8, they
   * may only be ascii compatable */
  rc = omrfilestream_write_text(
    filestream, mutf8String, mutf8StringSize, J9STR_CODE_PLATFORM_RAW);
  if (rc < 0) {
    /* do nothing */
  }
  rc = omrfilestream_sync(filestream);
  ...
  /* bad mutf-8 - malformed utf */
  rc = omrfilestream_write_text(
    filestream, "\xe2\x82", sizeof("\xe2\x82") - 1, J9STR_CODE_PLATFORM_RAW);
  if (rc >= 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_write_text() returned %d, expected error code\n",
...
/**
 * Verify that text transliteration is working by comparing the results of
 * omrfile_write_text() and omrfilestream_write_text() to each other.
 *
 * The expectation is that omrfile_write_text will have the same output as
 * omrfilestream_write_text() when the toCode is J9STR_CODE_PLATFORM_RAW.  The
 * test then reads in the the files as binary and compares what is in both
 * files.
 *
 * @param[in] portLibrary The port library under test
 *
...
TEST(PortFileStreamTest, omrfilestream_test_omrfile_text_comparison)
{
  ...
  const char* streamPath =
    "omrfilestream_test_omrfile_text_comparison_stream_text.tst";
  const char* filePath =
    "omrfilestream_test_omrfile_text_comparison_file_text.tst";
  ...
  /* Open both files */
  fileStream = omrfilestream_open(
    streamPath, EsOpenCreate | EsOpenWrite | EsOpenTruncate, 0666);
  if (NULL == fileStream) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned NULL expected valid file handle\n");
  ...
  file =
    omrfile_open(filePath, EsOpenCreate | EsOpenWrite | EsOpenTruncate, 0666);
  if (-1 == file) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned NULL expected valid file handle\n");
  ...
  rc = omrfilestream_write_text(
    fileStream, output, outputCount, J9STR_CODE_PLATFORM_RAW);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_write_text() failed: returned %d\n",
                       rc);
  }
  ...
  if (rc != 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrfile_write_text() failed: returned %d\n", rc);
  }
  ...
  rc = file_read_all(
    OMRPORTLIB, testName, fileStreamFile, streamInput, sizeof(streamInput));
  if (rc < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "file_read_all() returned %d expected %d\n", rc, 0);
  }
  ...
  if (rc != rc2) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfile_read on filestream returned %d bytes, while "
                       "omrfile_read on file returned %d bytes",
                       rc,
                       rc2);
    goto closeFiles;
  } else if (strncmp(fileInput, streamInput, rc)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Stream data \"%s\" not matching file data \"%s\"\n",
                       streamInput,
                       fileInput);
  }
...
/**
 * Verify that printf accepts the proper parameters and writes to the platform
 * encoding. There is no way to read back the characters and verify that they
 * are correct, as not all encoding conversions are reversible.
 *
 * @param[in] portLibrary The port library under test
 *
 * @return TEST_PASS on success, TEST_FAIL on error
...
TEST(PortFileStreamTest, omrfilestream_test_printf)
{
  ...
  /* Fill long string with printable characters, make sure that there is no
   * embedded '%'character */
...
/**
 * Verify the behaviors of writing and reading from invalid filestream, or
 * filestream with a restrictive open mode.
 *
 * @param[in] portLibrary The port library under test
 *
 * @return TEST_PASS on success, TEST_FAIL on error
...
TEST(PortFileStreamTest, omrfilestream_test_invalid_filestream_access)
{
   ...
   */
  noAccessStream =
    omrfilestream_open(fileName1, EsOpenWrite | EsOpenTruncate, 0666);
  if (NULL != noAccessStream) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned non-null, expected invalid file handle\n");
  ...
  /* Create a stream with no read or write permission.  Create a file with mode
   * 0000.  Must reopen the file to get the "Permission Denied" error */
  noAccessStream = omrfilestream_open(
    fileName1, EsOpenWrite | EsOpenCreate | EsOpenTruncate, 0000);
  if (NULL == noAccessStream) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned NULL expected valid file handle\n");
  ...
  if (rc < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_close() returned negative error code %d\n",
      rc);
    goto unlinkFiles;
  }
#if !defined(OMR_OS_WINDOWS)
  /* This test has different behavior on windows, and it will allow you to open
   * files even if you do not have the correct permissions.
   */
  noAccessStream = omrfilestream_open(
    fileName1, EsOpenWrite | EsOpenRead | EsOpenTruncate, 0666);
  if (NULL != noAccessStream) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned valid file handle, expected NULL\n");
  }
#endif /* !defined(OMR_OS_WINDOWS) */
  ...
  if (rc >= 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_close() returned %d, expected error code\n",
      rc);
  }
  ...
  if (-1 == fileDescriptor) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfile_open() returned -1, expected valid file descriptor");
  }
  invalidStream =
    omrfilestream_fdopen(fileDescriptor, EsOpenCreate | EsOpenWrite);
  if (NULL == invalidStream) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_fdopen() returned NULL expected valid file handle\n");
  }
  rc = omrfile_close(fileDescriptor);
  if (-1 == rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_close() returned failed, expected success\n");
  }
  rc = omrfilestream_close(invalidStream);
  if (rc == 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_close() returned success, expected failure\n");
  }

  /*
   * Get a some valid filestreams this time (for future testing)
   */
  validStream = omrfilestream_fdopen(
    omrfile_open(fileName3, EsOpenCreate | EsOpenRead | EsOpenWrite, 0666),
    EsOpenCreate | EsOpenRead | EsOpenWrite);
  if (NULL == validStream) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
  ...
  roStream = omrfilestream_fdopen(
    omrfile_open(fileName3, EsOpenCreate | EsOpenRead, 0666),
    EsOpenCreate | EsOpenRead);
  if (NULL == roStream) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_open() returned NULL expected valid file handle\n");
  ...
  if (rc >= 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_write() returned %d, expected error code\n",
      rc);
  }
  ...
  rc = omrfilestream_setbuffer(
    invalidStream, NULL, OMRPORT_FILESTREAM_NO_BUFFERING, 0);
  if (rc < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_setbuffer() failed, expected success\n");
  }
  rc = omrfile_close(fileDescriptor);
  if (-1 == rc) {
    outputErrorMessage(
  ...
  if (rc >= 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_write() returned success, expected failure\n");
  }
writeInvalidFDEnd:
  ...
  if (rc == 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_sync() returned %d, expected error code\n",
      rc);
  }
  rc = omrfilestream_sync(roStream);
  if (rc != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_sync() returned %d, expected 0\n",
                       rc);
  }
  /* Flush a filestream with an invalid file descriptor (must have something to
   * flush) */
  fileDescriptor = omrfile_open(fileName5, EsOpenCreate | EsOpenWrite, 0666);
  if (-1 == fileDescriptor) {
    outputErrorMessage(
  ...
  if (rc == 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_sync() returned success, expected failure\n");
  }
  ...
  rc = omrfilestream_write_text(
    validStream, NULL, sizeof(output), J9STR_CODE_MUTF8);
  if (rc >= 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_write_text() returned %d, expected error code\n",
  ...
  if (rc != 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_write_text() returned %d, expected 0\n",
                       rc);
  }
  omrfilestream_sync(validStream);
  rc = omrfilestream_write_text(
    roStream, output, sizeof(output), J9STR_CODE_MUTF8);
  if (rc >= 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_write_text() returned %d, expected error code\n",
  ...
  rc = omrfilestream_write_text(
    invalidStream, output, sizeof(output), J9STR_CODE_MUTF8);
  if (rc >= 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_write_text() returned success, expected failure\n");
  }
writeTextInvalidFDEnd:
   ...
   */
  rc =
    omrfilestream_setbuffer(NULL, NULL, OMRPORT_FILESTREAM_FULL_BUFFERING, 256);
  if (rc >= 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_setbuffer() returned %d, expected error code\n",
      rc);
  }
  rc = omrfilestream_setbuffer(
    validStream, NULL, ~OMRPORT_FILESTREAM_FULL_BUFFERING, 256);
  if (rc >= 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_setbuffer() returned %d, expected error code\n",
  ...
  if (NULL != omrfilestream_fdopen(0, ~(EsOpenWrite | EsOpenRead))) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrfilestream_fdopen() did not return expected error code\n",
      rc);
  }
  ...
  if (rc != -1) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrfilestream_fileno() returned %d, expected invalid "
                       "file descriptor -1\n",
                       rc);
  }
...
/**
 * Test to make sure that the standard IO streams are working.  The specific
 * streams are stdin, stdout, and stderr.  Make sure that they will not error
 * when opened and written to.
 *
 * @param[in] portLibrary The port library under test
 *
 * @return TEST_PASS on success, TEST_FAIL on error
...
TEST(PortFileStreamTest, omrfilestream_test_std_streams)
{
  ...
  /* Cannot free these filestreams (causing a memory leak for the test suite),
   * as that will close the underlying file descriptors.  In the future the
   * smart thing to do would be to DUP the file descriptor before opening it as
   * a stream.
   */
