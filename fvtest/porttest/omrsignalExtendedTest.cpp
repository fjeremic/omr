...
static int sigMaskThread(void *arg) {
  ...
  portTestEnv->log(
      "%s\t:pthread_sigmask test has been done. send result back to main "
      "thread.\n",
      testName);
...
TEST(PortSignalExtendedTests, sig_ext_test1) {
  ...
  if ((NULL != mainThread) && (NULL != maskThread) && (NULL != unmaskThread)) {
    ...
    portTestEnv->log(
        "%s\t:operation pthread_sigmask has been done. checking "
    ...
    if (!sigismember(&maskThread_mask, SIGBUS) ||
      ...
      portTestEnv->log(
          "%s\t:pthread_sigmask operation in each child thread did not "
    ...
    portTestEnv->log(
        "%s\t:configure unmask thread to prepare for unmasked signal SIGSEGV "
        "test.\n",
        testName);
    ...
    if (!waitForEvent(testName, &unmaskThreadInfo, FINISHED, NULL, 0)) {
      ...
      portTestEnv->log(
          "%s\t:unmasked thread finished. checking unmask thread's signal "
          "status...\n",
          testName);
    ...
    portTestEnv->log(
        "%s\t:mask thread is ready to receive signal. sending "
        "pthread_kill...\n",
        testName);
    ...
    if (!waitForEvent(testName, &maskThreadInfo, FINISHED, NULL, 0)) {
      ...
      portTestEnv->log(
          "%s\t:masked thread finished. checking mask thread's signal "
          "status...\n",
          testName);
