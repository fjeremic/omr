...
static void
setSigMaskTestInfo(SigMaskTestInfo* info,
                   struct OMRPortLibrary* portLibrary,
                   const char* testName,
                   omrthread_monitor_t monitor,
                   omrsig_protected_fn fn,
                   SignalEvent event);
static omrthread_t
create_thread(struct OMRPortLibrary* portLibrary,
              omrthread_entrypoint_t entpoint,
              void* entarg);
static BOOLEAN
waitForEvent(const char* testName,
...
static uintptr_t
sigHandlerFunction(struct OMRPortLibrary* portLibrary,
                   uint32_t gpType,
                   void* gpInfo,
                   void* handler_arg);
static uintptr_t
              ...
              void* entarg)
{
  ...
  if (J9THREAD_SUCCESS != ret) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "failed to create omrthread, returned:%d\n", ret);
  }
                   ...
                   void* handler_arg)
{
  ...
  portTestEnv->log(
    "%s\t:sigHandlerFunction invoked (type = 0x%x)\n", info->testName, gpType);
...
sigMaskThread(void* arg)
{
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    protectResult = omrsig_protect(
      info->fn, (void*)info, sigHandlerFunction, (void*)info, flags, &result);
...
TEST(PortSignalExtendedTests, sig_ext_test1)
{
  ...
  monitorRC = omrthread_monitor_init_with_name(
    &maskMonitor, 0, "omrsig_ext_sigmask_monitor");
  if (0 != monitorRC) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
  ...
  setSigMaskTestInfo(&maskThreadInfo,
                     OMRPORTLIB,
                     "Masked Thread",
                     maskMonitor,
  ...
  monitorRC = omrthread_monitor_init_with_name(
    &unmaskMonitor, 0, "omrsig_ext_sigunmask_monitor");
  if (0 != monitorRC) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrthread_monitor_init_with_name failed with %i\n",
                       monitorRC);
  ...
  setSigMaskTestInfo(&unmaskThreadInfo,
                     OMRPORTLIB,
                     "Unmasked Thread",
                     unmaskMonitor,
                     unmaskProtectedFunction,
                     INVALID);

  mainThread = omrthread_self();
  maskThread = create_thread(
    OMRPORTLIB, (omrthread_entrypoint_t)sigMaskThread, &maskThreadInfo);
  ...
  if ((NULL != mainThread) && (NULL != maskThread) && (NULL != unmaskThread)) {
    ...
    if (!waitForEvent(
          testName, &maskThreadInfo, READY, &osMaskThread, sizeof(pthread_t))) {
      goto exit;
    }
    if (!waitForEvent(testName,
                      &unmaskThreadInfo,
                      READY,
                      &osUnmaskThread,
                      sizeof(pthread_t))) {
    ...
    portTestEnv->log(
      "%s\t:mask thread is ready to receive signal. sending pthread_kill...\n",
      testName);
