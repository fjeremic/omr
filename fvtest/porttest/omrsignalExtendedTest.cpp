...
static void setSigMaskTestInfo(SigMaskTestInfo *info,
                               struct OMRPortLibrary *portLibrary,
                               const char *testName,
                               omrthread_monitor_t monitor,
                               omrsig_protected_fn fn, SignalEvent event);
static omrthread_t create_thread(struct OMRPortLibrary *portLibrary,
                                 omrthread_entrypoint_t entpoint, void *entarg);
static BOOLEAN waitForEvent(const char *testName, SigMaskTestInfo *info,
                            SignalEvent event, void *result, size_t size);
static void sendEvent(SigMaskTestInfo *info, SignalEvent event, void *value,
                      size_t size);
static uintptr_t sigHandlerFunction(struct OMRPortLibrary *portLibrary,
                                    uint32_t gpType, void *gpInfo,
                                    void *handler_arg);
static uintptr_t maskProtectedFunction(struct OMRPortLibrary *portLibrary,
                                       void *arg);
static uintptr_t unmaskProtectedFunction(struct OMRPortLibrary *portLibrary,
                                         void *arg);
 ...
 * @param monitor The monitor object that is used to synchronize with main
 * thread
 ...
 * @return pointer to a J9Thread on success; otherwise, a NULL pointer and log
 * error message
 */
static omrthread_t create_thread(struct OMRPortLibrary *portLibrary,
                                 omrthread_entrypoint_t entpoint,
                                 void *entarg) {
  ...
  intptr_t ret =
      omrthread_create_ex(&handle, J9THREAD_ATTR_DEFAULT, 0, entpoint, entarg);
  if (J9THREAD_SUCCESS != ret) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "failed to create omrthread, returned:%d\n", ret);
  }
 ...
 * @return TRUE upon success; FALSE if expected event did not occur before
 * timeout.
 */
static BOOLEAN waitForEvent(const char *testName, SigMaskTestInfo *info,
                            SignalEvent event, void *result, size_t size) {
 ...
 * telling it to finish the execution
 */
static uintptr_t maskProtectedFunction(struct OMRPortLibrary *portLibrary,
                                       void *arg) {
...
/**
 * This is child thread's entry point. It uses waitForEvent() and sendEvent() to
 * synchronize its operations with the main thread.
 * @param arg The pointer to passed in parameter which is a type of
 * SigMaskTestInfo.
...
static int sigMaskThread(void *arg) {
  ...
  portTestEnv->log("%s\t:pthread_sigmask test has been done. send result back "
                   "to main thread.\n",
                   testName);
  ...
  portTestEnv->log(
      "%s\t:starting signal handling test in protected function...\n",
      testName);
  ...
  if (!omrsig_can_protect(flags)) {
    ...
    protectResult = omrsig_protect(info->fn, (void *)info, sigHandlerFunction,
                                   (void *)info, flags, &result);
 ...
 *  - create 2 threads
 *  - one thread calls pthread_sigmask() and verifies that the specified mask is
 * set
 *  - other thread verifies that the specified mask is not set
 *  - send some signals to each thread, e.g. by pthread_kill and verify that it
 * is blocked or not.
 *
 *  Note: under heavy load test environment, the signal may not be delivered in
 * timely fashion
 *
 * mask
 ...
 *
 * main
 * thread:-w(READY)---------s(SIGMASK)-w(READY)------------c----s(PTHREADKILL)----w(FINISHED)---------->>
 * unmask
 * thread:---s(READY)-w(SIGMASK)-------m--s(READY)-w(PTHREADKILL)----------j----i----s(FINISHED)-------------->>
 *
 * where:
...
TEST(PortSignalExtendedTests, sig_ext_test1) {
  ...
  /*
   * OSX will redirect the SIGILL from the masked thread to other threads unless
   * they are also masked. This thread's mask will be inherited. Mask SIGILL for
   * all threads for mask testing.
   */
  ...
  monitorRC = omrthread_monitor_init_with_name(&maskMonitor, 0,
                                               "omrsig_ext_sigmask_monitor");
  if (0 != monitorRC) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
  ...
  setSigMaskTestInfo(&maskThreadInfo, OMRPORTLIB, "Masked Thread", maskMonitor,
                     maskProtectedFunction, INVALID);

  monitorRC = omrthread_monitor_init_with_name(&unmaskMonitor, 0,
                                               "omrsig_ext_sigunmask_monitor");
  if (0 != monitorRC) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrthread_monitor_init_with_name failed with %i\n",
                       monitorRC);
  ...
  setSigMaskTestInfo(&unmaskThreadInfo, OMRPORTLIB, "Unmasked Thread",
                     unmaskMonitor, unmaskProtectedFunction, INVALID);

  mainThread = omrthread_self();
  maskThread = create_thread(OMRPORTLIB, (omrthread_entrypoint_t)sigMaskThread,
                             &maskThreadInfo);
  unmaskThread = create_thread(
      OMRPORTLIB, (omrthread_entrypoint_t)sigMaskThread, &unmaskThreadInfo);
  ...
  if ((NULL != mainThread) && (NULL != maskThread) && (NULL != unmaskThread)) {
    ...
    if (!waitForEvent(testName, &maskThreadInfo, READY, &osMaskThread,
                      sizeof(pthread_t))) {
      goto exit;
    }
    if (!waitForEvent(testName, &unmaskThreadInfo, READY, &osUnmaskThread,
                      sizeof(pthread_t))) {
      goto exit;
    }
    ...
    flags = 0;
    portTestEnv->log(
        "%s\t:configure unmask thread to not mask any tested signal.\n",
        testName);
    ...
    if (!waitForEvent(testName, &maskThreadInfo, READY, &maskThread_mask,
                      sizeof(maskThread_mask))) {
      goto exit;
    }
    if (!waitForEvent(testName, &unmaskThreadInfo, READY, &unmaskThread_mask,
                      sizeof(unmaskThread_mask))) {
      goto exit;
    }
    portTestEnv->log("%s\t:operation pthread_sigmask has been done. checking "
                     "pthread_sigmask result...\n",
                     testName);
     ...
     * 3. child threads' pthread_sigmask operation shall not interfere each
     * other
     */
    ...
    /* check whether main thread signal mask was affected by child thread
     * pthread_sigmask operation */
    ...
    /* UNIX opengroup example says that newly created thread shall inherit the
     * mask */
    if (!sigismember(&maskThread_mask, SIGILL) ||
        !sigismember(&unmaskThread_mask, SIGILL)) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "created threads did not inherit "
                         "mask.(maskThreadInfo=0x%X, unmaskThreadInfo=0x%X)\n",
                         *((uint32_t *)&maskThread_mask),
                         *((uint32_t *)&unmaskThread_mask));
      goto exit;
    } else {
      portTestEnv->log(
          "%s\t:child thread inherited main thread's signal mask.\n", testName);
    }

    /* Check whether two child threads' pthread_sigmask operation can interfere
     * each other. */
    if (!sigismember(&maskThread_mask, SIGBUS) ||
        sigismember(&unmaskThread_mask, SIGBUS)) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "pthread_sigmask did not work.(maskThreadInfo=0x%X, "
                         "unmaskThreadInfo=0x%X)\n",
                         *((uint32_t *)&maskThread_mask),
                         *((uint32_t *)&unmaskThread_mask));
      goto exit;
    } else {
      portTestEnv->log("%s\t:pthread_sigmask operation in each child thread "
     ...
     * unmask thread will install SIGSEGV handler and launch a protected
     * function unmaskProtectedFunction(). unmaskProtectedFunction() shall be
     * terminated by SIGSEGV generated in its body, and the unmask thread will
     * send report to this main thread.
     */
    flags = OMRPORT_SIG_FLAG_MAY_RETURN |
            OMRPORT_SIG_FLAG_SIGSEGV; /* SIGSEGV shall be received */
    portTestEnv->log("%s\t:configure unmask thread to prepare for unmasked "
                     "signal SIGSEGV test.\n",
                     testName);
    ...
    if (!waitForEvent(testName, &unmaskThreadInfo, FINISHED, NULL, 0)) {
      ...
      portTestEnv->log("%s\t:unmasked thread finished. checking unmask "
                       "thread's signal status...\n",
                       testName);
      ...
      if (unmaskThreadInfo.bulletinBoard.status == SIGNALED) {
        ...
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "unmasked thread did not received signal as "
                           "expected.(SignalStatus=%d)\n",
     ...
     * mask thread will install SIGILL handler and launch a protected function
     * maskProtectedFunction().
     */
    portTestEnv->log("%s\t:testing pthread_kill...\n", testName);
    flags = OMRPORT_SIG_FLAG_MAY_RETURN |
            OMRPORT_SIG_FLAG_SIGILL; /* SIGILL shall not be received */
    sendEvent(&maskThreadInfo, PTHREADKILL, &flags, sizeof(flags));
    portTestEnv->log(
        "%s\t:configure mask thread to prepare for pthread_kill test.\n",
        testName);

    if (!waitForEvent(testName, &maskThreadInfo, READY, NULL, 0)) {
      goto exit;
    }
    portTestEnv->log("%s\t:mask thread is ready to receive signal. sending "
                     "pthread_kill...\n",
                     testName);
    ...
    /*
     * Expected behavior:
     * 1. child thread with signal mask on SIGILL shall not receive this signal
     * and therefore NOTSIGNALED.
     */
    ...
    if (!waitForEvent(testName, &maskThreadInfo, FINISHED, NULL, 0)) {
      ...
      portTestEnv->log("%s\t:masked thread finished. checking mask thread's "
                       "signal status...\n",
                       testName);
      if (maskThreadInfo.bulletinBoard.status == NOTSIGNALED) {
        portTestEnv->log(
            "%s\t:masked thread did not receive signal as expected.\n",
            testName);
      } else {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
