...
static void streamCallback(J9CudaStream stream, int32_t error,
                           uintptr_t userData) {
...
TEST_F(CudaDeviceTest, streams) {
  ...
  for (uint32_t deviceId = 0; deviceId < deviceCount; ++deviceId) {
    ...
    rc = omrcuda_streamCreateWithPriority(
        deviceId, -1, J9CUDA_STREAM_FLAG_NON_BLOCKING, &stream2);
    ...
    rc = omrcuda_streamAddCallback(deviceId, stream2, streamCallback,
