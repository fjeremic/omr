 ...
 * can be found in the file @ref omrport.c  The majority of these functions are
 * exported by the port library and are not in the actual port library table,
 * however there are a few.
 *
 * @note port library maintenace operations are not optional in the port library
 * table.
 */
#include <stdlib.h>
#include <string.h>
 ...
 */
static void verifyCreateTableContents(struct OMRPortLibrary *portLibrary,
                                      const char *testName, uintptr_t size) {
  ...
  if (NULL != OMRPORTLIB->portGlobals) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrport_create_library() portGlobals pointer is not NULL\n");
  }
  ...
  if (NULL != OMRPORTLIB->self_handle) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrport_create_library() self_handle pointer is not NULL\n");
  }

  /* Verify the pointers look correct, must skip version and portGlobals portion
   * information */
  ...
  if (NULL == OMRPORTLIB->tty_printf) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_create_library() tty_printf pointer is NULL\n");
  }
}
 ...
 */
static int32_t fake_mem_startup(struct OMRPortLibrary *portLibrary,
 ...
 * @return negative error code
 */
static int32_t fake_time_startup(struct OMRPortLibrary *portLibrary) {
  return failMemStartup;
}
 ...
 *
 * Ensure the port library is properly setup to run port library maintenance
 * tests.
 *
...
TEST(PortTest, port_test0) {
  ...
  if (NULL == OMRPORTLIB->port_shutdown_library) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->port_shutdown_library is NULL\n");
  }

  /* omrport_test9 */
  if (NULL == OMRPORTLIB->port_isFunctionOverridden) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->port_isFunctionOverridden is NULL\n");
  }
 ...
 * Verify port library management.
 *
 * Applications wishing to self allocate the port library are required to
 * provide a size for the port library to create.  The size of any port library
 * version can be obtained via the exported function
...
TEST(PortTest, port_test1) {
  ...
  if (actualSize != expectedSize) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_getSize() returned %zu expected %zu\n",
 ...
 * in the table need to be initialized.  The ability to initialize the table
 * with default values prior to starting the port library allows applications to
 * override functionality.  For example an application may with to control all
 * memory management, so overriding the appropriate functions is required prior
 * to any memory being allocated.  Creation of the port library table is done
 * via the function
...
TEST(PortTest, port_test2) {
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_create_library() returned %d expected %d\n",
                       rc);
  }
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_create_library() returned %d expected 0\n", rc);
  }
  ...
  if (eyeCatcher != bigBuffer[0]) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_create_library() trampled memory expected "
                       "\"%c\" found \"%c\"\n",
...
TEST(PortTest, port_test3) {
  ...
  /* create a library and ensure the port globals is non NULL, what else can we
   * do? */
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_startup_library() returned %d expected 0\n",
                       rc);
  }
  ...
  if (NULL == fakePtr->portGlobals) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_startup_library() portGlobals not allocated\n");
  }

  if (NULL != fakePtr->self_handle) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrport_startup_library() self_handle pointer not NULL\n");
  }

  /* Clean this port library up */
  portLibraryToTest.port_shutdown_library(fakePtr);

  /* Do it again, but before starting override memory (first startup function)
  ...
  if (failMemStartup != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_startup_library() returned %d expected %d\n",
                       rc, failMemStartup);
  }
 ...
 * This table must not be declared on a stack that is torn down prior to the
 * application running to completion and tearing down the port library.  It can
 * either be created as a global variable to the program, or if a running port
 * library is present it can be allocated on behalf of the application.  Self
 * allocating port libraries use the exported function
...
TEST(PortTest, port_test5) {
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_allocate_library() returned %d expected 0\n",
                       rc);
    goto exit;
  }
  ...
  if (NULL == fakePortLibrary) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_allocate_library() returned NULL pointer\n");
    goto exit;
  }
  ...
  if (NULL != fakePortLibrary->portGlobals) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrport_allocate_library() portGlobals pointer is not NULL\n");
  }
  ...
  if (NULL == fakePortLibrary->self_handle) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrport_allocate_library() self_handle pointer is NULL\n");
  }
  ...
  /* Try again, but force failure during startup by over riding one of the
   * startup functions */
  fakePortLibrary = NULL;
  rc = omrport_allocate_library(&fakePortLibrary);
  if (0 != rc) {
  ...
  /* No way to tell if it freed the memory properly.  The pointer we have to the
   * port library is not updated as part of omrport_startup_library(), so we are
   * now pointing at dead memory.
   *
...
TEST(PortTest, port_test6) {
  ...
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_init_library() returned %d expected 0\n", rc);
  }
  /* Shut it down */
  portLibraryToTest.port_shutdown_library(fakePtr);
  omrthread_detach(attachedThread);
  ...
  if (NULL != fakePtr->portGlobals) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "port_shutdown_library() portGlobal pointer is not NULL\n");
  }
...
TEST(PortTest, port_test7) {
  ...
  rc = fakePtr->port_isFunctionOverridden(
      fakePtr, offsetof(OMRPortLibrary, mem_startup));
  if (1 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_isFunctionOverridden returned %d expected 1\n",
                       rc);
  }

  rc = fakePtr->port_isFunctionOverridden(
      fakePtr, offsetof(OMRPortLibrary, mem_shutdown));
  if (1 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_isFunctionOverridden returned %d expected 1\n",
                       rc);
  ...
  rc = fakePtr->port_isFunctionOverridden(
      fakePtr, offsetof(OMRPortLibrary, time_hires_clock));
  if (1 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_isFunctionOverridden returned %d expected 1\n",
                       rc);
  ...
  rc = fakePtr->port_isFunctionOverridden(
      fakePtr, offsetof(OMRPortLibrary, time_startup));
  if (1 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_isFunctionOverridden returned %d expected 1\n",
                       rc);
  ...
  rc = fakePtr->port_isFunctionOverridden(
      fakePtr, offsetof(OMRPortLibrary, time_hires_frequency));
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_isFunctionOverridden returned %d expected 0\n",
                       rc);
  }

  rc = fakePtr->port_isFunctionOverridden(fakePtr,
                                          offsetof(OMRPortLibrary, tty_printf));
  if (0 != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrport_isFunctionOverridden returned %d expected 0\n",
                       rc);
