 ...
 */
static void verifyCreateTableContents(struct OMRPortLibrary* portLibrary, const char* testName, uintptr_t size)
{
 ...
 */
static int32_t fake_mem_startup(struct OMRPortLibrary* portLibrary, uintptr_t portGlobalSize)
{
...
TEST(PortTest, port_test2)
{
  ...
  if (OMRPORT_ERROR_INIT_OMR_WRONG_SIZE != rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrport_create_library() returned %d expected %d\n", rc, OMRPORT_ERROR_INIT_OMR_WRONG_SIZE);
  }
