...
#define COMPLETE_ALL_SIZES \
  (COMPLETE_LARGE_REGION | COMPLETE_NEAR_REGION | COMPLETE_SUBALLOC_BLOCK)

static void freeMemPointers(struct OMRPortLibrary *portLibrary, void **memPtrs,
                            uintptr_t length);
static void shuffleArray(struct OMRPortLibrary *portLibrary, uintptr_t *sizes,
                         uintptr_t length);
 ...
 * @typdef
 */
typedef void *(*J9MEM_ALLOCATE_MEMORY_FUNC)(struct OMRPortLibrary *portLibrary,
...
static void verifyMemory(struct OMRPortLibrary *portLibrary,
  ...
  /*
     ...
     "omrmem_allocate_memory(%d) pointer mis aligned\n", byteAmount);
          }
  */
...
TEST(PortMemTest, mem_test0) {
  ...
  if (NULL == OMRPORTLIB->mem_startup) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mem_startup is NULL\n");
  }

  /*  Not tested, implementation dependent.  No known functionality */
  if (NULL == OMRPORTLIB->mem_shutdown) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mem_shutdown is NULL\n");
  }

  /* omrmem_test2 */
  if (NULL == OMRPORTLIB->mem_allocate_memory) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mem_allocate_memory is NULL\n");
  }

  /* omrmem_test1 */
  if (NULL == OMRPORTLIB->mem_free_memory) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mem_free_memory is NULL\n");
  }

  /* omrmem_test4 */
  if (NULL == OMRPORTLIB->mem_reallocate_memory) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mem_reallocate_memory, is NULL\n");
  }

  if (NULL == OMRPORTLIB->mem_allocate_memory32) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mem_allocate_memory32, is NULL\n");
  }

  if (NULL == OMRPORTLIB->mem_free_memory32) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mem_free_memory32, is NULL\n");
  }

  if (NULL == OMRPORTLIB->mem_walk_categories) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary->mem_walk_categories, is NULL\n");
  }
...
TEST(PortMemTest, mem_test1) {
  ...
  /* First verify that a NULL pointer can be freed, as specified by API docs.
   * This ensures that one can free any pointer returned from allocation
   * routines without checking for NULL first.
   */
  memPtr = NULL;
  omrmem_free_memory(memPtr);
  if (NULL != memPtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrmem_free_memory(NULL) altered pointer\n");
  ...
  if (NULL == memPtr) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrmem_allocate_memory(32) returned NULL\n");
    goto exit;
  }
  omrmem_free_memory(memPtr);
...
TEST(PortMemTest, mem_test2) {
  ...
  byteAmount = 0;
  omrstr_printf(allocName, allocNameSize, "omrmem_allocate_memory(%d)",
                byteAmount);
  memPtr =
      (char *)omrmem_allocate_memory(byteAmount, OMRMEM_CATEGORY_PORT_LIBRARY);
...
/* removed test 3 due to DESIGN 355 (no longer separate allocate function for
 * callsite) */
...
TEST(PortMemTest, mem_test4) {
  ...
  /* Not visible outside the port library, so can't verify.  Should this be
   * visible outside the portlibrary?
   */
#if 0
	void *memPtr = 0;
 ...
 * @ref omrmem.c::omrmem_free_memory "omrmem_free_memory()", but it may be on
 * some platforms.
 */
 ...
 * Verify port library memory management.
 *
 * Self allocating port libraries call a function not available via the port
 * library table.  Need to verify that these functions have been correctly
 * implemented.  These functios are called by the port library management
 * routines in @ref j9port.c "j9port.c". The API for these functions clearly
 * state they must be implemented.
 *
 * Verify @ref omrmem.c::omrmem_allocate_portLibrary
 * "omrmem_allocate_portLibrary()" allocates memory correctly.
 *
 * @note This function does not need to be implemented in terms of
 * @ref omrmem.c::omrmem_allocate_memory "omrmem_allocate_memory()", but it may
...
static void shuffleArray(struct OMRPortLibrary *portLibrary, uintptr_t *array,
                         uintptr_t length) {
  uintptr_t i = 0;
 ...
 * Verify port library memory management.
 *
 * Ensure the function @ref omrmem.c::omrmem_allocate_memory32
 * "omrmem_allocate_memory()" returns a memory location that is below the 4 gig
 * boundary, and as well that the entire region is below the 4 gig boundary.
 *
 * We allocate a variety of sizes comparable to the heap size used in
 * mem_allocate_memory32 implementation (see HEAP_SIZE_BYTES in
 * omrmem32helpers.c):
 * 1. size requests that are much smaller than the heap size are satisfied with
 * omrheap suballocation.
 * 2. size requests that are slightly smaller than the heap size will not use
 * omrheap suallocation (because they cannot be satisfied by suballocating a
 * omrheap due to heap management overhead), but rather given their own vmem
 * allocated chunk.
 * 3. size requests that are greater or equal to the heap size also will not use
 * omrheap suallocation, but rather given their own vmem allocated chunk.
 *
 * @note This function does not need to be implemented in terms of
 * @ref omrmem.c::omrmem_free_memory "omrmem_free_memory()", but it may be on
 * some platforms.
...
TEST(PortMemTest, mem_test7_allocate32) {
  ...
  uintptr_t allocBlockSizesLength =
      sizeof(allocBlockSizes) / sizeof(allocBlockSizes[0]);
  void *allocBlockReturnPtrs[sizeof(allocBlockSizes) /
                             sizeof(allocBlockSizes[0])];
  ...
  for (; allocBlockCursor < allocBlockSizesLength; allocBlockCursor++) {
    ...
    omrstr_printf(allocName, allocNameSize, "\nomrmem_allocate_memory32(%d)",
                  byteAmount);
    pointer =
        omrmem_allocate_memory32(byteAmount, OMRMEM_CATEGORY_PORT_LIBRARY);
    ...
    if (((uintptr_t)pointer + byteAmount) > MEM32_LIMIT) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "Entire memory block allocated by %s is not below 32-bit limit\n",
          allocName);
    }
#endif
...
#if defined(OMR_ENV_DATA64)
  /* now we should have at least one entirely free J9Heap, try suballocate a
   * large portion of the it. This should not incur any vmem allocation.
   */
  ...
  verifyMemory(OMRPORTLIB, testName, (char *)pointer, finalAllocSize,
               allocName);
...
static uint32_t childrenOfDummyCategoryOne[] = {
    DUMMY_CATEGORY_TWO, DUMMY_CATEGORY_THREE, OMRMEM_CATEGORY_PORT_LIBRARY,
    OMRMEM_CATEGORY_UNKNOWN};

static OMRMemCategory dummyCategoryOne = {
    "Dummy One", DUMMY_CATEGORY_ONE, 0, 0, 4, childrenOfDummyCategoryOne};

static OMRMemCategory dummyCategoryTwo = {
    "Dummy Two", DUMMY_CATEGORY_TWO, 0, 0, 0, NULL};

static OMRMemCategory dummyCategoryThree = {
    "Dummy Three", DUMMY_CATEGORY_THREE, 0, 0, 0, NULL};

static OMRMemCategory *categoryList[3] = {&dummyCategoryOne, &dummyCategoryTwo,
                                          &dummyCategoryThree};
...
static uintptr_t categoryWalkFunction(uint32_t categoryCode,
                                      const char *categoryName,
                                      uintptr_t liveBytes,
                                      uintptr_t liveAllocations, BOOLEAN isRoot,
  ...
  struct CategoriesState *state =
      (struct CategoriesState *)walkState->userData2;
  ...
  switch (categoryCode) {
    case DUMMY_CATEGORY_ONE:
      if (!isRoot) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "DUMMY_CATEGORY_ONE should be a root, and isn't\n");
      ...
      if (categoryName != dummyCategoryOne.name) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "DUMMY_CATEGORY_ONE has the wrong name: %p=%s instead of %p=%s\n",
            categoryName, categoryName, dummyCategoryOne.name,
            dummyCategoryOne.name);
        state->otherError = TRUE;
        return J9MEM_CATEGORIES_STOP_ITERATING;
      }
    ...
    case DUMMY_CATEGORY_TWO:
      if (isRoot) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "DUMMY_CATEGORY_TWO shouldn't be a root, and is\n");
        state->otherError = TRUE;
        return J9MEM_CATEGORIES_STOP_ITERATING;
      }
      ...
      if (DUMMY_CATEGORY_ONE != parentCategoryCode) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "DUMMY_CATEGORY_TWO's parent should be "
                           "DUMMY_CATEGORY_ONE, not %u\n",
                           parentCategoryCode);
        state->otherError = TRUE;
        return J9MEM_CATEGORIES_STOP_ITERATING;
      }
      ...
      if (categoryName != dummyCategoryTwo.name) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "DUMMY_CATEGORY_TWO has the wrong name: %p=%s instead of %p=%s\n",
            categoryName, categoryName, dummyCategoryTwo.name,
            dummyCategoryTwo.name);
        state->otherError = TRUE;
        return J9MEM_CATEGORIES_STOP_ITERATING;
      }
    ...
    case DUMMY_CATEGORY_THREE:
      if (isRoot) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "DUMMY_CATEGORY_THREE shouldn't be a root, and is\n");
        state->otherError = TRUE;
        return J9MEM_CATEGORIES_STOP_ITERATING;
      }
      ...
      if (DUMMY_CATEGORY_ONE != parentCategoryCode) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
                           "DUMMY_CATEGORY_THREE's parent should be "
                           "DUMMY_CATEGORY_ONE, not %u\n",
                           parentCategoryCode);
        state->otherError = TRUE;
        return J9MEM_CATEGORIES_STOP_ITERATING;
      ...
      if (categoryName != dummyCategoryThree.name) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS,
            "DUMMY_CATEGORY_THREE has the wrong name: %p=%s instead of %p=%s\n",
            categoryName, categoryName, dummyCategoryThree.name,
            dummyCategoryThree.name);
        state->otherError = TRUE;
        return J9MEM_CATEGORIES_STOP_ITERATING;
      }
    ...
    case OMRMEM_CATEGORY_PORT_LIBRARY:
      /* Root status changes depending on whether we're using the default
       * category set or not. */
...
/**
 * Utility method that calls omrmem_walk_categories and populates the
 * CategoriesState structure
 */
static void getCategoriesState(struct OMRPortLibrary *portLibrary,
                               struct CategoriesState *state) {
  OMRMemCategoryWalkState walkState;
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
/**
 * Verifies that memory categories work with the omrmem_memory_allocate,
 * reallocate and allocate32 APIs
 *
 * We test:
 *
 ...
 *
 * The tests for vmem, mmap and shmem test those APIs also manipulate the
 * counters properly
 */
TEST(PortMemTest, mem_test8_categories) {
  ...
  /* Check that walking the categories state only walks port library & unknown
   * categories */
  getCategoriesState(OMRPORTLIB, &categoriesState);

  if (categoriesState.dummyCategoryOneWalked) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "dummyCategoryOne unexpectedly walked\n");
    goto end;
  }
  if (categoriesState.dummyCategoryTwoWalked) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "dummyCategoryTwo unexpectedly walked\n");
    goto end;
  }
  if (categoriesState.dummyCategoryThreeWalked) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "dummyCategoryThree unexpectedly walked\n");
    goto end;
  }
  if (!categoriesState.portLibraryWalked) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "portLibrary unexpectedly not walked\n");
    goto end;
  }
  if (!categoriesState.unknownWalked) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "unknown unexpectedly not walked\n");
    goto end;
  }
#if defined(OMR_ENV_DATA64)
  if (!categoriesState.unused32bitSlabWalked) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "unused32bitSlab unexpectedly not walked\n");
    goto end;
  }
#endif
  if (categoriesState.otherError) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "Some other error hit while walking categories (see "
                       "messages above).\n");
    goto end;
  }

  /* Try allocating under the port library category and check the block and byte
   * counters are incremented */
  {
    ...
    if (finalBlocks != expectedBlocks) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Unexpected number of blocks after allocate. Expected "
                         "%zd, got %zd.\n",
    ...
    if (finalBlocks != initialBlocks) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "Unexpected number of blocks after free. Expected %zd, got %zd.\n",
    ...
    if (finalBytes != initialBytes) {
      outputErrorMessage(
          PORTTEST_ERROR_ARGS,
          "Unexpected number of bytes after free. Expected %zd, got %zd.\n",
          initialBytes, finalBytes);
      goto end;
    }
  }

  /* Try allocating with a user category code - having not registered any
   * categories. Check it maps to unknown */
  {
    uintptr_t initialBlocks = 0;
    uintptr_t initialBytes = 0;
    uintptr_t finalBlocks = 0;
  ...
  omrport_control(OMRPORT_CTLDATA_MEM_CATEGORIES_SET,
                  (uintptr_t)&dummyCategorySet);

  /* Try allocating to one of our user category codes - check the arithmetic is
   * done correctly */
  {
    uintptr_t initialBlocks = 0;
    uintptr_t initialBytes = 0;
  ...
  /* On 64 bit, check that allocate32 manipulates the "unused 32 bit slab"
   * category */
...
#if defined(OMR_ENV_DATA64)
  {
    ...
    if (finalUnused32BitSlabBlocks != initialUnused32BitSlabBlocks) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Number of unused 32bit slab blocks should have "
                         "stayed the same. Was %zu, now %zu\n",
                         initialUnused32BitSlabBlocks,
                         finalUnused32BitSlabBlocks);
      goto end;
    }

    if (finalUnused32BitSlabBytes > minimumExpectedUnused32BitSlabBytes) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Unexpected value for unused 32 bit slab bytes. "
                         "Expected %zu, was %zu. Initial value was %zu.\n",
                         minimumExpectedUnused32BitSlabBytes,
                         finalUnused32BitSlabBytes,
                         initialUnused32BitSlabBytes);
      goto end;
    }
    ...
    if (finalUnused32BitSlabBlocks != initialUnused32BitSlabBlocks) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Unexpected number of unused 32 bit slab blocks after "
                         "free. Expected %zu, now %zu\n",
                         initialUnused32BitSlabBlocks,
                         finalUnused32BitSlabBlocks);
      goto end;
    }
    if (finalUnused32BitSlabBytes != initialUnused32BitSlabBytes) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Unexpected number of unused 32 bit slab bytes after "
                         "free. Expected %zu, now %zu\n",
                         initialUnused32BitSlabBytes,
                         finalUnused32BitSlabBytes);
      goto end;
  ...
  /* Try allocating to an unknown category code - check it maps to unknown
   * properly */
  {
    uintptr_t initialBlocks = 0;
    uintptr_t initialBytes = 0;
    ...
    if (!categoriesState.unknownWalked) {
      outputErrorMessage(PORTTEST_ERROR_ARGS,
                         "Category walk didn't cover unknown category.\n");
      omrmem_free_memory(ptr);
      goto end;
    }
    if (categoriesState.otherError) {
 ...
 */
static uintptr_t categoryWalkFunction2(
...
/*
 * Tests that the memory category walk can be stopped by returning
 * J9MEM_CATEGORIES_STOP_ITERATING
 */
TEST(PortMemTest, mem_test9_category_walk) {
  ...
  if (categoriesWalked != 1) {
    outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "Unexpected number of categories walk. Expected 1, got %u\n",
        categoriesWalked);
  }
