...
static void
freeMemPointers(struct OMRPortLibrary* portLibrary,
                void** memPtrs,
                uintptr_t length);
static void
shuffleArray(struct OMRPortLibrary* portLibrary,
             uintptr_t* sizes,
             uintptr_t length);
             ...
             const char* allocName)
{
  ...
  */
  ...
  if (testCharA != memPtr[0]) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "%s failed write test 0\n", allocName);
  }
  ...
  if (0 != memcmp(stackMemory, memPtr, testSize)) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "%s failed to retain value\n", allocName);
  }
  ...
  if (strlen(memPtr) != byteAmount - 1) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "%s failed write test\n", allocName);
  }
}
...
TEST(PortMemTest, mem_test2)
{
  ...
  byteAmount = 0;
  omrstr_printf(
    allocName, allocNameSize, "omrmem_allocate_memory(%d)", byteAmount);
  memPtr =
...
TEST(PortMemTest, mem_test7_allocate32)
{
  ...
  void*
    allocBlockReturnPtrs[sizeof(allocBlockSizes) / sizeof(allocBlockSizes[0])];
...
static uint32_t childrenOfDummyCategoryOne[] = { DUMMY_CATEGORY_TWO,
                                                 DUMMY_CATEGORY_THREE,
...
static OMRMemCategory dummyCategoryTwo = { "Dummy Two", DUMMY_CATEGORY_TWO,
                                           0,           0,
                                           0,           NULL };
...
static OMRMemCategory* categoryList[3] = { &dummyCategoryOne,
                                           &dummyCategoryTwo,
                                           &dummyCategoryThree };
...
static uintptr_t
categoryWalkFunction(uint32_t categoryCode,
                     ...
                     OMRMemCategoryWalkState* walkState)
{
  ...
  switch (categoryCode) {
    ...
    default:
      outputErrorMessage(
        PORTTEST_ERROR_ARGS, "Unexpected categoryCode: %u\n", categoryCode);
      state->otherError = TRUE;
      return J9MEM_CATEGORIES_STOP_ITERATING;
  }
 ...
 */
static void
getCategoriesState(struct OMRPortLibrary* portLibrary,
 ...
 */
static uintptr_t
categoryWalkFunction2(uint32_t categoryCode,
                      const char* categoryName,
                      uintptr_t liveBytes,
                      uintptr_t liveAllocations,
                      BOOLEAN isRoot,
