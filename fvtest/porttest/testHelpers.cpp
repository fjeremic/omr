 ...
 * an error message is output via @ref outputMessage it is incremented.  Upon
 * exit of a testcase @ref reportTestExit will indicate if the test passed or
 * failed based on this variable.
 */
 ...
 * As a result entire areas of functionality may not be present.  Sockets and
 * file systems are examples of these areas.  In addition even if an area is
 * present not all functionality for that area may be available.  Again sockets
 * and file systems are examples.
 *
 * @param[in] portLibrary The port library
 * @param[in] operation The operation that is not supported
 */
void operationNotSupported(struct OMRPortLibrary *portLibrary,
...
/**
 * Iterate through the global @ref testFailures printing out each failed test
 * case
 *
 * The memory for each string in testFailers is freed after writing the string
 * to the console.
 *
...
void dumpTestFailuresToConsole(struct OMRPortLibrary *portLibrary) {
  ...
  portTestEnv->log(LEVEL_ERROR, "----------------------------------------------"
                                "---------------------------\n\n");
  portTestEnv->log(LEVEL_ERROR,
                   "FAILURES DETECTED. Number of failed tests: %u\n\n",
                   numTestFailures);
 ...
 * @param[in]	source string to be copied into dest
 */
static void allocateMemoryForAndCopyInto(struct OMRPortLibrary *portLibrary,
  ...
  *dest = (char *)omrmem_allocate_memory(strLenPlusTerminator,
 ...
 *
 * Allocate the memory required to log the failures. This memory is freed in
 * @ref dumpTestFailuresToConsole
 *
 * @param[in] portLibrary
...
static void logTestFailure(struct OMRPortLibrary *portLibrary,
                           const char *fileName, int32_t lineNumber,
                           const char *testName, int32_t portErrorNumber,
                           const char *portErrorMessage,
                           const char *testErrorMessage) {
  if (MAX_NUM_TEST_FAILURES <= numTestFailures) {
    return;
  }

  allocateMemoryForAndCopyInto(
      portLibrary, &testFailures[numTestFailures].testName, testName);
  allocateMemoryForAndCopyInto(portLibrary,
                               &testFailures[numTestFailures].errorMessage,
                               testErrorMessage);
  allocateMemoryForAndCopyInto(
      portLibrary, &testFailures[numTestFailures].fileName, fileName);
  allocateMemoryForAndCopyInto(portLibrary,
                               &testFailures[numTestFailures].portErrorMessage,
                               portErrorMessage);
...
/**
 * Write a formatted string to the console, lookup the last port error message
 * and port error number, then store it.
 *
 ...
 */
void outputErrorMessage(struct OMRPortLibrary *portLibrary,
                        const char *fileName, int32_t lineNumber,
   ...
   * Get the size needed to hold the last error message (don't use str_printf to
   * avoid polluting error message */
  sizePortErrorBuf = strlen(lastErrorMessage) + 1 /* for the null terminator */;

  portErrorBuf = (char *)omrmem_allocate_memory(sizePortErrorBuf,
                                                OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  if (NULL != portErrorBuf) {
    ...
    portTestEnv->log(LEVEL_ERROR,
                     "\n\n******* omrmem_allocate_memory failed to allocate %i "
                     "bytes, exiting.\n\n",
                     sizePortErrorBuf);
  ...
  portTestEnv->log(LEVEL_ERROR, "%s line %4zi: %s ", fileName, lineNumber,
                   testName);
  portTestEnv->log(LEVEL_ERROR, "%s\n", buf);
  portTestEnv->log(LEVEL_ERROR, "\t\tLastErrorNumber: %i\n", lastErrorNumber);
  portTestEnv->log(LEVEL_ERROR, "\t\tLastErrorMessage: %s\n\n", portErrorBuf);

  logTestFailure(OMRPORTLIB, fileName, lineNumber, testName, lastErrorNumber,
                 portErrorBuf, buf);
 ...
 *
 * This takes the porttest filename and line number information so that the
 * caller is properly identified as failing.
 *
 * @param[in] portLibrary The port library
 ...
 */
uintptr_t verifyFileExists(struct OMRPortLibrary *portLibrary,
                           const char *pltestFileName, int32_t lineNumber,
  ...
  if (0 == fileStatRC) {
    ...
    /* error in file_stat */
    outputErrorMessage(
        OMRPORTLIB, pltestFileName, lineNumber, testName,
...
void deleteControlDirectory(struct OMRPortLibrary *portLibrary, char *baseDir) {
  ...
  if (buf.isDir != 1) {
    ...
    while ((uintptr_t)-1 != rc) {
      ...
      omrstr_printf(nextEntry, EsMaxPath, "%s/%s", mybaseFilePath,
                    resultBuffer);
 ...
 * @param[out]         blocks         Pointer to memory to store the live blocks
 * for the port library category
 * @param[out]         bytes          Pointer to memory to store the live bytes
 * for the port library category
 */
void getPortLibraryMemoryCategoryData(struct OMRPortLibrary *portLibrary,
                                      uintptr_t *blocks, uintptr_t *bytes) {
  OMRMemCategoryWalkState walkState;
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
...
uintptr_t raiseSEGV(OMRPortLibrary *portLibrary, void *arg) {
   ...
   * - z/OS doesn't provide a value for the psw1 (PC) register when you use
   * raise()
   */
  char *ptr = (char *)(-1);
  ptr -= 4;
