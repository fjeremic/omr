 ...
 */
void
operationNotSupported(struct OMRPortLibrary* portLibrary,
...
dumpTestFailuresToConsole(struct OMRPortLibrary* portLibrary)
{
  ...
  for (i = 0; i < numTestFailures; i++) {
    ...
    portTestEnv->log(LEVEL_ERROR,
                     "\t%s line %4zi: %s\n",
                     testFailures[i].fileName,
                     testFailures[i].lineNumber,
 ...
 */
static void
allocateMemoryForAndCopyInto(struct OMRPortLibrary* portLibrary,
                             char** dest,
                             const char* source)
{
...
static void
logTestFailure(struct OMRPortLibrary* portLibrary,
               const char* fileName,
               int32_t lineNumber,
               const char* testName,
               int32_t portErrorNumber,
               ...
               const char* testErrorMessage)
{
  ...
  allocateMemoryForAndCopyInto(
    portLibrary, &testFailures[numTestFailures].errorMessage, testErrorMessage);
  allocateMemoryForAndCopyInto(
 ...
 */
void
outputErrorMessage(struct OMRPortLibrary* portLibrary,
                   const char* fileName,
                   int32_t lineNumber,
                   const char* testName,
                   const char* format,
 ...
 */
uintptr_t
verifyFileExists(struct OMRPortLibrary* portLibrary,
...
deleteControlDirectory(struct OMRPortLibrary* portLibrary, char* baseDir)
{
  ...
  if (buf.isDir != 1) {
    ...
    while ((uintptr_t)-1 != rc) {
      ...
      omrstr_printf(
        nextEntry, EsMaxPath, "%s/%s", mybaseFilePath, resultBuffer);
 ...
 */
void
getPortLibraryMemoryCategoryData(struct OMRPortLibrary* portLibrary,
                                 uintptr_t* blocks,
                                 uintptr_t* bytes)
{
  OMRMemCategoryWalkState walkState;
