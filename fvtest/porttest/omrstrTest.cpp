 ...
 */
static void
validate_omrstr_vprintf_with_NULL(struct OMRPortLibrary *portLibrary,
                                  const char *testName, uint32_t bufLength,
                                  uint32_t expectedLength, const char *format,
                                  va_list args) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  uintptr_t rc;
...
TEST(PortStrTest, str_test3) {
  ...
  portTestEnv->log("\t This test could fail if you abut the international "
                   "dateline (westside of dateline)\n");
  timeMillis = 0;
...
#define TEST_OVERFLOW_LEN 8
  ...
  portTestEnv->log("\n\tThe results of omrstr_test5 are not evaluated as time "
                   "conversions are specific to a time zone.\n");
  ...
  tokens = omrstr_create_tokens(timeMillis);
  (void)omrstr_subst_tokens(buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens);
  portTestEnv->log("\tExpecting local time (in Ottawa, Eastern Standard Time): "
                   "2006/02/14 16:30:06 \n",
                   timeMillis);
  portTestEnv->log(
      "\t                                       ... converted to: %s\n", buf);
  ...
  tokens = omrstr_create_tokens(timeMillis);
  (void)omrstr_subst_tokens(buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens);
  portTestEnv->log("\tExpecting local time (in Ottawa, Eastern Daylight Time): "
                   "2006/06/14 17:30:06 \n",
                   timeMillis);
  portTestEnv->log(
      "\t                                       ... converted to: %s\n", buf);
...
static int printToBuffer(struct OMRPortLibrary *portLibrary, BOOLEAN precision,
  ...
  if (stringLength >= 0) {
    if (precision) {
      ...
      portTestEnv->log("\n\tFinish Testing without precision specifier: String "
                       "in buffer is: \"%s\", length = %d\n",
                       buffer, stringLength);
    }
