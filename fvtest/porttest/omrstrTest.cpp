 ...
 */
typedef uintptr_t (*J9STR_VPRINTF_FUNC)(
 ...
 */
static uintptr_t fake_omrstr_vprintf(
  struct OMRPortLibrary* portLibrary, char* buf, uintptr_t bufLen, const char* format, va_list args)
{
  return J9STR_PRIVATE_RETURN_VALUE;
}
 ...
 */
static void validate_omrstr_vprintf(struct OMRPortLibrary* portLibrary, const char* testName, char* buffer,
  uintptr_t bufLength, const char* expectedResult, const char* format, va_list args)
{
  ...
  if (strlen(buffer) != rc) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
      "omrstr_vprintf(\"%s\") with \"%s\" returned buffer with length %d, expected %d\n", format, expectedResult,
      strlen(buffer), rc);
  }
 ...
 */
static void validate_omrstr_vprintf_with_NULL(struct OMRPortLibrary* portLibrary, const char* testName,
  uint32_t bufLength, uint32_t expectedLength, const char* format, va_list args)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  uintptr_t rc;
  ...
  if (expectedLength != rc) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrstr_vprintf(\"%s\") returned %d expected %d\n", format, rc, expectedLength);
  }
}
 ...
 */
static void test_omrstr_vprintf(
  struct OMRPortLibrary* portLibrary, const char* testName, const char* format, const char* expectedResult, ...)
{
  ...
  if (strlen(expectedResult) > 1) {
    ...
    validate_omrstr_vprintf(
      OMRPORTLIB, testName, actualResult, strlen(expectedResult), truncatedExpectedResult, format, args);
    ...
    if (strlen(expectedResult) > 2) {
      ...
      validate_omrstr_vprintf(
        OMRPORTLIB, testName, actualResult, strlen(expectedResult) - 1, truncatedExpectedResult, format, args);
    }
  }
  ...
  validate_omrstr_vprintf_with_NULL(
    OMRPORTLIB, testName, (uint32_t)-1, (uint32_t)strlen(expectedResult) + 1, format, args);

  /* NULL, truncated buffer, use strlen(expectedResult) as length of buffer thus reducing the size by 1 */
  validate_omrstr_vprintf_with_NULL(
    OMRPORTLIB, testName, (uint32_t)strlen(expectedResult), (uint32_t)strlen(expectedResult) + 1, format, args);
  va_end(args);
}
 ...
 */
static void test_omrstr_vprintfNulChar(struct OMRPortLibrary* portLibrary, const char* testName, ...)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  uintptr_t rc;
 ...
 */
static void test_omrstr_ftime(struct OMRPortLibrary* portLibrary, const char* testName, char* buf, uintptr_t bufLen,
  const char* format, I_64 timeMillis, const char* expectedBuf)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
 ...
 */
static void test_omrstr_tokens(struct OMRPortLibrary* portLibrary, const char* testName, char* buf, uintptr_t bufLen,
  const char* format, struct J9StringTokens* tokens, const char* expectedBuf, uintptr_t expectedRet)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  uintptr_t rc;
...
TEST(PortStrTest, str_test3)
{
  ...
  test_omrstr_ftime(OMRPORTLIB, testName, buf, J9STR_BUFFER_SIZE, "%Y %m %b %d %S %pid %uid %job %home %last %seq",
    timeMillis, expected);
  ...
  test_omrstr_ftime(
    OMRPORTLIB, testName, buf, J9STR_BUFFER_SIZE, "%y,(%Y) %m,(%b) %d XX:%M:%S %", timeMillis, expected);
...
#define TEST_OVERFLOW_LEN 8
  ...
  if (NULL == tokens) {
    ...
    /* Test 1: No tokens */
    test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "Teststring", tokens, "Teststring", 10);
    /* Test 2: Single token */
    test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "%Y", tokens, "2004", 4);
    /* Test 3: End with a token */
    test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "Teststring %Y", tokens, "Teststring 2004", 15);
    /* Test 4: Start with a token */
    test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "%y Teststring", tokens, "04 Teststring", 13);
    /* Test 5: Many tokens */
    test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "%Y/%m/%d %S seconds %longtkn", tokens,
      "2004/02/29 00 seconds Long Token Value", 38);
    /* Test 6: Tokens and strings combined */
    test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "Test1 %Y Test2-%m%d%y-%S %longtkn, %longtkn", tokens,
      "Test1 2004 Test2-022904-00 Long Token Value, Long Token Value", 61);
    /* Test 7: %% and end with % */
    test_omrstr_tokens(
      OMRPORTLIB, testName, buf, TEST_BUF_LEN, "99%% is the same as 99%", tokens, "99% is the same as 99%", 22);
    /* Test 8: Invalid tokens */
    test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "%zzz = %%zzz", tokens, "%zzz = %zzz", 11);
    /* Test 9: Excessive % stuff */
    test_omrstr_tokens(
      OMRPORTLIB, testName, buf, TEST_BUF_LEN, "%Y%%%m%%%d %zzz% %%%%%%", tokens, "2004%02%29 %zzz% %%%", 20);
    ...
    test_omrstr_tokens(OMRPORTLIB, testName, bufOverflow, TEST_OVERFLOW_LEN, "Teststring", tokens, "Teststr", 11);
    /* Test 13: Test for overflow with an actual short buffer (token) */
    test_omrstr_tokens(OMRPORTLIB, testName, bufOverflow, TEST_OVERFLOW_LEN, "Test %Y", tokens, "Test 20", 10);
    /* Test 14: Test for token precedence based on length */
    test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "%yyy is not %yy is not %y", tokens,
      "yup yup yup is not 04y is not 04", 32);
    /* Test 15: Test the read only mode (should return the required buf len) */
    test_omrstr_tokens(
      OMRPORTLIB, testName, NULL, 0, "%yyy is not %yy is not %y", tokens, NULL, 33); /* 33 because must include \0 */
    /* Test 16: Test an empty token */
    test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "x%emptyx == xx", tokens, "xx == xx", 8);
...
#define TEST_OVERFLOW_LEN 8
  ...
  tokens = omrstr_create_tokens(timeMillis);
  test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens, "1970/01/01 00:00:00", 19);
  reportTestExit(OMRPORTLIB, testName);
}

#define TEST_BUF_LEN 1024
static const char utf8String[] = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q',
  'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z' };
static const uint32_t utf8StringLength = sizeof(utf8String);
static const U_16 utf16Data[] = { 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e,
...
#endif
static const char platformString[] = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p',
...
static void dumpStringBytes(const char* message, const char* buffer, uint32_t length)
{
  uint32_t index = 0;
...
static BOOLEAN compareBytes(const char* expected, const char* actual, uint32_t length)
{
  uint32_t index = 0;
  for (index = 0; index < length; ++index) {
    if (actual[index] != expected[index]) {
      fprintf(stderr, "Error at position %d \nexpected %0x actual %0x\n", index, (unsigned char)expected[index],
...
TEST(PortStrTest, str_convPlatTo8)
{
  ...
  convertedStringLength = omrstr_convert(
    J9STR_CODE_PLATFORM_RAW, J9STR_CODE_MUTF8, platformString, originalStringLength, outBuff, sizeof(outBuff));
  ...
  /* test string length */
  convertedStringLength
    = omrstr_convert(J9STR_CODE_PLATFORM_RAW, J9STR_CODE_MUTF8, platformString, originalStringLength, NULL, 0);
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
      convertedStringLength);
  }
...
TEST(PortStrTest, str_convLongString)
{
  ...
  /* test string length */
  mutf8StringLength
    = omrstr_convert(J9STR_CODE_PLATFORM_RAW, J9STR_CODE_MUTF8, longPlatformString, originalStringLength, NULL, 0);
  if (mutf8StringLength < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "Length calculation for modified UTF8 failed, error code %d\n", mutf8StringLength);
  }
  /* do the actual conversion */
  mutf8StringLength = omrstr_convert(
    J9STR_CODE_PLATFORM_RAW, J9STR_CODE_MUTF8, longPlatformString, originalStringLength, mutf8Buff, sizeof(mutf8Buff));
  if (mutf8StringLength < 0) {
  ...
  returnStringLength = omrstr_convert(J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, mutf8Buff, mutf8StringLength, NULL, 0);
  if (returnStringLength < 0) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "Length calculation for platform failed, error code %d\n", mutf8StringLength);
  }

  /* convert back and verify that it matches the original */
  returnStringLength = omrstr_convert(
    J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, mutf8Buff, mutf8StringLength, returnBuff, sizeof(returnBuff));
  if (returnStringLength < 0) {
  ...
  if (returnStringLength != originalStringLength) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", originalStringLength, returnStringLength);
  }
  reportTestExit(OMRPORTLIB, testName);
}
...
TEST(PortStrTest, str_convU8ToPlat)
{
  ...
  memset(outBuff, 0, sizeof(outBuff));
  convertedStringLength = omrstr_convert(
    J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, utf8String, originalStringLength, outBuff, sizeof(outBuff));
  if (convertedStringLength != expectedStringLength) {
  ...
  /* test string length */
  convertedStringLength
    = omrstr_convert(J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, utf8String, originalStringLength, NULL, 0);
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", originalStringLength,
      convertedStringLength);
  }
...
TEST(PortStrTest, str_convU8ToWide)
{
  ...
  memset(outBuff, 0, sizeof(outBuff));
  convertedStringLength
    = omrstr_convert(J9STR_CODE_MUTF8, J9STR_CODE_WIDE, utf8String, originalStringLength, outBuff, sizeof(outBuff));
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
      convertedStringLength);
  }
  ...
  /* test string length */
  convertedStringLength = omrstr_convert(J9STR_CODE_MUTF8, J9STR_CODE_WIDE, utf8String, originalStringLength, NULL, 0);
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", originalStringLength,
      convertedStringLength);
  }
...
TEST(PortStrTest, str_convWideToU8)
{
  ...
  memset(outBuff, 0, sizeof(outBuff));
  convertedStringLength
    = omrstr_convert(J9STR_CODE_WIDE, J9STR_CODE_MUTF8, utf16String, utf16StringLength, outBuff, sizeof(outBuff));
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
      convertedStringLength);
  }
  ...
  /* test string length */
  convertedStringLength = omrstr_convert(J9STR_CODE_WIDE, J9STR_CODE_MUTF8, utf16String, utf16StringLength, NULL, 0);
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
      convertedStringLength);
  }
...
TEST(PortStrTest, str_convWideToU8_Null)
{
  ...
  if (0 != outBuff[convertedStringLength]) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Converted string  not null terminated: expected 0, got %0x",
...
TEST(PortStrTest, str_convU8ToWide_Null)
{
  ...
  convertedStringLength
    = omrstr_convert(J9STR_CODE_MUTF8, J9STR_CODE_WIDE, inputString, inputStringLength, outBuff, sizeof(outBuff));
  convertedStringTerminator = (U_16*)(outBuff + convertedStringLength);
  if (0 != *convertedStringTerminator) {

    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "Converted string  not null terminated: expected 0, got %0x", *convertedStringTerminator);
  }
...
TEST(PortStrTest, str_convU8ToPlat_Null)
{
  ...
  (void)memset(outBuff, '^', sizeof(outBuff)); /* initialize to non-zero */
  convertedStringLength = omrstr_convert(
    J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, inputString, inputStringLength, outBuff, sizeof(outBuff));
...
TEST(PortStrTest, str_convUtf8ToMUtf8)
{
  ...
  const char mutf8Data[] = { '\xc0', '\x80', /* embedded null */
    '\x41', '\x42', '\x43', '\x44', /* ASCII */
    '\xc0', '\x80', /* embedded null */
    '\xD0', '\xB0', '\xD0', '\xB1', '\xD0', '\xB2', /* 2-byte UTF-8 */
    '\xE4', '\xBA', '\x8C', '\xE4', '\xBA', '\x8D', '\xE4', '\xBA', '\x8E', /* 3-byte UTF-8 */
    '\xed', '\xaf', '\xbf', '\xed', '\xbf', '\xbf', /* Maximum code point */
    '\xed', '\xa0', '\x80', '\xed', '\xbc', '\x82', '\xed', '\xa0', '\x80', '\xed', '\xbc', '\x83', '\xed', '\xa0',
    '\x80', '\xed', '\xbc', '\x84', '\x44', '\x45', '\x46', '\xef', '\xbf', '\xbd', '\xc0', '\x80' };
  ...
  convertedStringLength
    = omrstr_convert(J9STR_CODE_UTF8, J9STR_CODE_MUTF8, utf8Data, utf8DataLength, outBuff, sizeof(outBuff));
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
      convertedStringLength);
  }
  ...
  /* test string length */
  convertedStringLength = omrstr_convert(J9STR_CODE_UTF8, J9STR_CODE_MUTF8, utf8Data, utf8DataLength, NULL, 0);
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
      convertedStringLength);
  }
...
TEST(PortStrTest, str_convRandomUtf8ToMUtf8)
{
  ...
  const char utf8Data[] = { '\x41', '\xf2', '\x86', '\x84', '\xaf', '\xf0', '\xbc', '\x8f', '\x9b', '\xee', '\xb5',
    '\xa0', '\xf4', '\x86', '\xbf', '\xae', '\xf0', '\xbe', '\xa7', '\xac', '\xf4', '\x86', '\x81', '\x84', '\xf0',
    '\xb3', '\x93', '\x99', '\xf2', '\xb6', '\x91', '\xae', '\xf2', '\x96', '\xba', '\xa9', '\xf0', '\xbc', '\xb6',
    '\x92', '\xf1', '\x8c', '\x9c', '\xa4', '\xe4', '\x99', '\xad', '\xf2', '\xb3', '\xa4', '\x95', '\xf0', '\xa6',
    '\x85', '\xab', '\xf1', '\xaa', '\xbb', '\xa4', '\xf3', '\x8c', '\x9e', '\xb8', '\xf2', '\x98', '\x9e', '\xb8',
    '\xf3', '\xb8', '\x8c', '\xa5', '\xf3', '\x84', '\x8b', '\x8a', '\xf3', '\x94', '\x95', '\xb3', '\xf0', '\x9b',
    '\xa3', '\xa6', '\xf1', '\xbb', '\x9b', '\xa7', '\xf1', '\x88', '\x9a', '\x81', '\xf4', '\x83', '\xa8', '\x83',
    '\xf2', '\xb4', '\x88', '\xb7', '\xf1', '\xa0', '\xa2', '\xb6', '\xf3', '\x97', '\x88', '\xb7', '\xf2', '\x88',
    '\x83', '\xb8', '\xf0', '\xa9', '\x9b', '\x9e', '\xf0', '\xa7', '\x92', '\xab', '\x5a' };
  const char mutf8Data[] = { '\x41', '\xed', '\xa7', '\x98', '\xed', '\xb4', '\xaf', '\xed', '\xa2', '\xb0', '\xed',
    '\xbf', '\x9b', '\xee', '\xb5', '\xa0', '\xed', '\xaf', '\x9b', '\xed', '\xbf', '\xae', '\xed', '\xa2', '\xba',
    '\xed', '\xb7', '\xac', '\xed', '\xaf', '\x98', '\xed', '\xb1', '\x84', '\xed', '\xa2', '\x8d', '\xed', '\xb3',
    '\x99', '\xed', '\xaa', '\x99', '\xed', '\xb1', '\xae', '\xed', '\xa8', '\x9b', '\xed', '\xba', '\xa9', '\xed',
    '\xa2', '\xb3', '\xed', '\xb6', '\x92', '\xed', '\xa3', '\xb1', '\xed', '\xbc', '\xa4', '\xe4', '\x99', '\xad',
    '\xed', '\xaa', '\x8e', '\xed', '\xb4', '\x95', '\xed', '\xa1', '\x98', '\xed', '\xb5', '\xab', '\xed', '\xa5',
    '\xab', '\xed', '\xbb', '\xa4', '\xed', '\xab', '\xb1', '\xed', '\xbe', '\xb8', '\xed', '\xa8', '\xa1', '\xed',
    '\xbe', '\xb8', '\xed', '\xae', '\xa0', '\xed', '\xbc', '\xa5', '\xed', '\xab', '\x90', '\xed', '\xbb', '\x8a',
    '\xed', '\xac', '\x91', '\xed', '\xb5', '\xb3', '\xed', '\xa0', '\xae', '\xed', '\xb3', '\xa6', '\xed', '\xa6',
    '\xad', '\xed', '\xbb', '\xa7', '\xed', '\xa3', '\xa1', '\xed', '\xba', '\x81', '\xed', '\xaf', '\x8e', '\xed',
    '\xb8', '\x83', '\xed', '\xaa', '\x90', '\xed', '\xb8', '\xb7', '\xed', '\xa5', '\x82', '\xed', '\xb2', '\xb6',
    '\xed', '\xac', '\x9c', '\xed', '\xb8', '\xb7', '\xed', '\xa7', '\xa0', '\xed', '\xb3', '\xb8', '\xed', '\xa1',
    '\xa5', '\xed', '\xbb', '\x9e', '\xed', '\xa1', '\x9d', '\xed', '\xb2', '\xab', '\x5a' };
  int32_t utf8DataLength = sizeof(utf8Data); /* skip the terminating null */
  int32_t expectedStringLength = sizeof(mutf8Data);
  int32_t convertedStringLength = 0;
...
TEST(PortStrTest, str_convRoundTrip)
{
  ...
  convertedStringLength = omrstr_convert(
    J9STR_CODE_WIDE, J9STR_CODE_MUTF8, (char*)unicodeData, 2 * numCodePoints, (char*)mutf8Buffer, bufferSize);
  ...
  convertedStringLength = omrstr_convert(
    J9STR_CODE_MUTF8, J9STR_CODE_WIDE, (char*)mutf8Buffer, convertedStringLength, (char*)unicodeResult, bufferSize);
  if (convertedStringLength < 0) {
...
static const char mutf8DataForLatin1[]
  = { '\xc0', '\x80', '\x1', '\x2', '\x3', '\x4', '\x5', '\x6', '\x7', '\x8', '\x9', '\xa', '\xb', '\xc', '\xd', '\xe',
      '\xf', '\x10', '\x11', '\x12', '\x13', '\x14', '\x15', '\x16', '\x17', '\x18', '\x19', '\x1a', '\x1b', '\x1c',
      '\x1d', '\x1e', '\x1f', '\x20', '\x21', '\x22', '\x23', '\x24', '\x25', '\x26', '\x27', '\x28', '\x29', '\x2a',
      '\x2b', '\x2c', '\x2d', '\x2e', '\x2f', '\x30', '\x31', '\x32', '\x33', '\x34', '\x35', '\x36', '\x37', '\x38',
      '\x39', '\x3a', '\x3b', '\x3c', '\x3d', '\x3e', '\x3f', '\x40', '\x41', '\x42', '\x43', '\x44', '\x45', '\x46',
      '\x47', '\x48', '\x49', '\x4a', '\x4b', '\x4c', '\x4d', '\x4e', '\x4f', '\x50', '\x51', '\x52', '\x53', '\x54',
      '\x55', '\x56', '\x57', '\x58', '\x59', '\x5a', '\x5b', '\x5c', '\x5d', '\x5e', '\x5f', '\x60', '\x61', '\x62',
      '\x63', '\x64', '\x65', '\x66', '\x67', '\x68', '\x69', '\x6a', '\x6b', '\x6c', '\x6d', '\x6e', '\x6f', '\x70',
      '\x71', '\x72', '\x73', '\x74', '\x75', '\x76', '\x77', '\x78', '\x79', '\x7a', '\x7b', '\x7c', '\x7d', '\x7e',
      '\x7f', '\xc2', '\x80', '\xc2', '\x81', '\xc2', '\x82', '\xc2', '\x83', '\xc2', '\x84', '\xc2', '\x85', '\xc2',
      '\x86', '\xc2', '\x87', '\xc2', '\x88', '\xc2', '\x89', '\xc2', '\x8a', '\xc2', '\x8b', '\xc2', '\x8c', '\xc2',
      '\x8d', '\xc2', '\x8e', '\xc2', '\x8f', '\xc2', '\x90', '\xc2', '\x91', '\xc2', '\x92', '\xc2', '\x93', '\xc2',
      '\x94', '\xc2', '\x95', '\xc2', '\x96', '\xc2', '\x97', '\xc2', '\x98', '\xc2', '\x99', '\xc2', '\x9a', '\xc2',
      '\x9b', '\xc2', '\x9c', '\xc2', '\x9d', '\xc2', '\x9e', '\xc2', '\x9f', '\xc2', '\xa0', '\xc2', '\xa1', '\xc2',
      '\xa2', '\xc2', '\xa3', '\xc2', '\xa4', '\xc2', '\xa5', '\xc2', '\xa6', '\xc2', '\xa7', '\xc2', '\xa8', '\xc2',
      '\xa9', '\xc2', '\xaa', '\xc2', '\xab', '\xc2', '\xac', '\xc2', '\xad', '\xc2', '\xae', '\xc2', '\xaf', '\xc2',
      '\xb0', '\xc2', '\xb1', '\xc2', '\xb2', '\xc2', '\xb3', '\xc2', '\xb4', '\xc2', '\xb5', '\xc2', '\xb6', '\xc2',
      '\xb7', '\xc2', '\xb8', '\xc2', '\xb9', '\xc2', '\xba', '\xc2', '\xbb', '\xc2', '\xbc', '\xc2', '\xbd', '\xc2',
      '\xbe', '\xc2', '\xbf', '\xc3', '\x80', '\xc3', '\x81', '\xc3', '\x82', '\xc3', '\x83', '\xc3', '\x84', '\xc3',
      '\x85', '\xc3', '\x86', '\xc3', '\x87', '\xc3', '\x88', '\xc3', '\x89', '\xc3', '\x8a', '\xc3', '\x8b', '\xc3',
      '\x8c', '\xc3', '\x8d', '\xc3', '\x8e', '\xc3', '\x8f', '\xc3', '\x90', '\xc3', '\x91', '\xc3', '\x92', '\xc3',
      '\x93', '\xc3', '\x94', '\xc3', '\x95', '\xc3', '\x96', '\xc3', '\x97', '\xc3', '\x98', '\xc3', '\x99', '\xc3',
      '\x9a', '\xc3', '\x9b', '\xc3', '\x9c', '\xc3', '\x9d', '\xc3', '\x9e', '\xc3', '\x9f', '\xc3', '\xa0', '\xc3',
      '\xa1', '\xc3', '\xa2', '\xc3', '\xa3', '\xc3', '\xa4', '\xc3', '\xa5', '\xc3', '\xa6', '\xc3', '\xa7', '\xc3',
      '\xa8', '\xc3', '\xa9', '\xc3', '\xaa', '\xc3', '\xab', '\xc3', '\xac', '\xc3', '\xad', '\xc3', '\xae', '\xc3',
      '\xaf', '\xc3', '\xb0', '\xc3', '\xb1', '\xc3', '\xb2', '\xc3', '\xb3', '\xc3', '\xb4', '\xc3', '\xb5', '\xc3',
      '\xb6', '\xc3', '\xb7', '\xc3', '\xb8', '\xc3', '\xb9', '\xc3', '\xba', '\xc3', '\xbb', '\xc3', '\xbc', '\xc3',
      '\xbd', '\xc3', '\xbe', '\xc3', '\xbf', '\xc0', '\x80', '\x1', '\x2', '\x3', '\x4', '\x5', '\x6', '\x7' };
...
TEST(PortStrTest, str_Latin1ToMutf8)
{
  ...
  convertedStringLength
    = omrstr_convert(J9STR_CODE_LATIN1, J9STR_CODE_MUTF8, inBuff, originalStringLength, outBuff, sizeof(outBuff));
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
      convertedStringLength);
  }
  ...
  /* test string length */
  convertedStringLength = omrstr_convert(J9STR_CODE_LATIN1, J9STR_CODE_MUTF8, inBuff, originalStringLength, NULL, 0);
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
      convertedStringLength);
  }
...
TEST(PortStrTest, str_Mutf8ToLatin1)
{
  ...
  convertedStringLength = omrstr_convert(
    J9STR_CODE_MUTF8, J9STR_CODE_LATIN1, mutf8DataForLatin1, mutf8DataLength, outBuff, sizeof(outBuff));
  ...
  /* test string length */
  convertedStringLength
    = omrstr_convert(J9STR_CODE_MUTF8, J9STR_CODE_LATIN1, mutf8DataForLatin1, mutf8DataLength, NULL, 0);
  if (convertedStringLength != expectedLatin1ToFromMutf8Length) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n",
      expectedLatin1ToFromMutf8Length, convertedStringLength);
  }
...
{
  OMRPORT_ACCESS_FROM_OMRPORT(portTestEnv->getPortLibrary());
  char winacpData[] = { '\x1', '\x2', '\x3', '\x4', '\x5', '\x6', '\x7', '\x8', '\x81', '\x91', '\xa1', '\xb1', '\xc1',
    '\xd1', '\xe1', '\xf1' };
#if defined(OMR_OS_WINDOWS)
  char expectedMutf8[] = { '\x1', '\x2', '\x3', '\x4', '\x5', '\x6', '\x7', '\x8', '\xc2', '\x81', '\xe2', '\x80',
    '\x98', '\xc2', '\xa1', '\xc2', '\xb1', '\xc3', '\x81', '\xc3', '\x91', '\xc3', '\xa1', '\xc3', '\xb1' };
#endif /* defined(OMR_OS_WINDOWS) */
  ...
  memset(outBuff, 0, sizeof(outBuff));
  convertedStringLength = omrstr_convert(
    J9STR_CODE_WINDEFAULTACP, J9STR_CODE_MUTF8, winacpData, originalStringLength, outBuff, sizeof(outBuff));
#if defined(OMR_OS_WINDOWS)
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
      convertedStringLength);
 ...
 *         TEST_FAIL on error
 */
static int printToBuffer(struct OMRPortLibrary* portLibrary, BOOLEAN precision, char* buffer, size_t bufferLength,
  const char* expectedOutput, const char* format, ...)
{
  ...
  if (stringLength >= 0) {
    ...
    if ((stringLength == strlen(expectedOutput)) && (0 == memcmp(buffer, expectedOutput, stringLength))) {
...
TEST(PortStrTest, str_test_atoe_vsnprintf)
{
  ...
  portTestEnv->log("\n\tTesting case 7\n");
  rc
    |= printToBuffer(OMRPORTLIB, TRUE, buffer0, bufferLength0, expectedOutput1, "%*.*s", 4, 4, nonNullTerminatedString);

  /* the length of inputString is equal to precision but less than min_width */
  portTestEnv->log("\n\tTesting case 8\n");
  rc |= printToBuffer(
    OMRPORTLIB, TRUE, buffer0, bufferLength0, expectedOutput3, "%*.*s", 10, 4, nonNullTerminatedString);
