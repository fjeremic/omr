 ...
 */
static uintptr_t
fake_omrstr_vprintf(struct OMRPortLibrary* portLibrary,
                    char* buf,
                    uintptr_t bufLen,
                    const char* format,
                    va_list args)
 ...
 */
static void
validate_omrstr_vprintf(struct OMRPortLibrary* portLibrary,
                        const char* testName,
                        char* buffer,
                        uintptr_t bufLength,
 ...
 */
static void
test_omrstr_vprintf(struct OMRPortLibrary* portLibrary,
                    const char* testName,
                    const char* format,
                    const char* expectedResult,
                    ...)
{
  ...
  validate_omrstr_vprintf(OMRPORTLIB,
                          testName,
                          actualResult,
                          sizeof(actualResult),
                          expectedResult,
  ...
  validate_omrstr_vprintf_with_NULL(OMRPORTLIB,
                                    testName,
                                    0,
  ...
  validate_omrstr_vprintf_with_NULL(OMRPORTLIB,
                                    testName,
                                    (uint32_t)strlen(expectedResult),
                                    (uint32_t)strlen(expectedResult) + 1,
                                    format,
                                    args);
  va_end(args);
 ...
 */
static void
test_omrstr_vprintfNulChar(struct OMRPortLibrary* portLibrary,
                           const char* testName,
                           ...)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
 ...
 */
static void
test_omrstr_ftime(struct OMRPortLibrary* portLibrary,
                  const char* testName,
                  char* buf,
                  uintptr_t bufLen,
                  const char* format,
                  ...
                  const char* expectedBuf)
{
  ...
  if (0 != strncmp(buf, expectedBuf, bufLen)) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "Expected \"%s\", Got \"%s\"\n", expectedBuf, buf);
  }
}
 ...
 */
static void
test_omrstr_tokens(struct OMRPortLibrary* portLibrary,
                   const char* testName,
                   char* buf,
                   uintptr_t bufLen,
                   const char* format,
                   ...
                   uintptr_t expectedRet)
{
  ...
  if (rc != expectedRet) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "Expected rc = %d, Got rc = %d\n", expectedRet, rc);
  }
...
TEST(PortStrTest, str_test2)
{
  ...
  test_omrstr_vprintf(
    OMRPORTLIB, testName, "%p", "BAADF00DDEADBEEF", (void*)0xBAADF00DDEADBEEF);
#else  /* defined(OMR_ENV_DATA64) */
  test_omrstr_vprintf(
    OMRPORTLIB, testName, "%p", "DEADBEEF", (void*)0xDEADBEEF);
  ...
  unicodeString[7] = '\0';
  test_omrstr_vprintf(OMRPORTLIB,
                      testName,
                      "%ls",
  ...
  test_omrstr_vprintf(OMRPORTLIB,
                      ...
                      unicodeString);
  test_omrstr_vprintf(
    OMRPORTLIB, testName, "%-8.2ls", "\xc2\x80\xe0\xa0\x80   ", unicodeString);
...
TEST(PortStrTest, str_test3)
{
  ...
  timeMillis = J9CONST64(1078056000000);
  strncpy(expected,
          "2004 02 Feb 29 00 %pid %uid %job %home %last %seq",
          J9STR_BUFFER_SIZE);
  test_omrstr_ftime(OMRPORTLIB,
                    testName,
                    buf,
...
#define TEST_OVERFLOW_LEN 8
  ...
  if (NULL == tokens) {
    ...
    /* Test 2: Single token */
    test_omrstr_tokens(
      OMRPORTLIB, testName, buf, TEST_BUF_LEN, "%Y", tokens, "2004", 4);
    ...
    test_omrstr_tokens(OMRPORTLIB,
                       ...
                       buf,
                       TEST_BUF_LEN,
                       "%Y/%m/%d %S seconds %longtkn",
                       tokens,
                       "2004/02/29 00 seconds Long Token Value",
                       38);
    ...
    test_omrstr_tokens(
      ...
      buf,
      TEST_BUF_LEN,
      "Test1 %Y Test2-%m%d%y-%S %longtkn, %longtkn",
      tokens,
    ...
    test_omrstr_tokens(
      OMRPORTLIB, testName, buf, 7, "Teststring", tokens, "Testst", 11);
    ...
    test_omrstr_tokens(OMRPORTLIB,
                       ...
                       buf,
                       TEST_BUF_LEN,
                       "%yyy is not %yy is not %y",
                       tokens,
                       "yup yup yup is not 04y is not 04",
                       32);
     ...
     * that the user may be relying on) */
    {
      ...
      strncpy(
        fullFormatString, timePortionOfFormatString, sizeOfFullFormatString);
      for (i = 0; i < (sizeof(default_tokens) / sizeof(char*)); i++) {
        strncat(fullFormatString,
                default_tokens[i],
      ...
      if (0 == strstr(buf, expected)) {
        outputErrorMessage(
          PORTTEST_ERROR_ARGS, "[%s] was not found in [%s]\n", expected, buf);
      }
      for (i = 0; i < (sizeof(default_tokens) / sizeof(char*)); i++) {
        if (NULL != strstr(buf, default_tokens[i])) {
          outputErrorMessage(
            PORTTEST_ERROR_ARGS, "%s was not substituted\n", default_tokens[i]);
        }
      }
...
#endif
static const char platformString[] = { 'a', 'b', 'c', 'd', 'e', 'f', 'g',
                                       'h', 'i', 'j', 'k', 'l', 'm', 'n',
                                       'o', 'p', 'q', 'r', 's', 't', 'u',
                                       'v', 'w', 'x', 'y', 'z', '\0' };
...
TEST(PortStrTest, str_convPlatTo8)
{
  ...
  convertedStringLength = omrstr_convert(J9STR_CODE_PLATFORM_RAW,
                                         J9STR_CODE_MUTF8,
                                         platformString,
...
TEST(PortStrTest, str_convLongString)
{
  ...
  /* test string length */
  mutf8StringLength = omrstr_convert(J9STR_CODE_PLATFORM_RAW,
                                     J9STR_CODE_MUTF8,
                                     longPlatformString,
                                     originalStringLength,
                                     NULL,
  ...
  returnStringLength = omrstr_convert(J9STR_CODE_MUTF8,
...
TEST(PortStrTest, str_convU8ToPlat)
{
  ...
  reportTestEntry(OMRPORTLIB, testName);
  memset(outBuff, 0, sizeof(outBuff));
  convertedStringLength = omrstr_convert(J9STR_CODE_MUTF8,
                                         J9STR_CODE_PLATFORM_RAW,
                                         utf8String,
                                         originalStringLength,
                                         outBuff,
...
TEST(PortStrTest, str_convWideToU8)
{
  ...
  reportTestEntry(OMRPORTLIB, testName);
  memset(outBuff, 0, sizeof(outBuff));
  convertedStringLength = omrstr_convert(J9STR_CODE_WIDE,
                                         J9STR_CODE_MUTF8,
                                         utf16String,
                                         utf16StringLength,
                                         outBuff,
  ...
  /* test string length */
  convertedStringLength = omrstr_convert(
    J9STR_CODE_WIDE, J9STR_CODE_MUTF8, utf16String, utf16StringLength, NULL, 0);
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "buffer length wrong.  Expected %d actual %d\n",
                       expectedStringLength,
...
TEST(PortStrTest, str_convUtf8ToMUtf8)
{
  ...
  convertedStringLength = omrstr_convert(J9STR_CODE_UTF8,
                                         J9STR_CODE_MUTF8,
                                         utf8Data,
                                         utf8DataLength,
                                         outBuff,
  ...
  /* test string length */
  convertedStringLength = omrstr_convert(
    J9STR_CODE_UTF8, J9STR_CODE_MUTF8, utf8Data, utf8DataLength, NULL, 0);
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "buffer length wrong.  Expected %d actual %d\n",
                       expectedStringLength,
...
TEST(PortStrTest, str_convRoundTrip)
{
  ...
  uint8_t* mutf8Buffer =
    (uint8_t*)omrmem_allocate_memory(bufferSize, OMRMEM_CATEGORY_PORT_LIBRARY);
  uint8_t* unicodeResult =
    (uint8_t*)omrmem_allocate_memory(bufferSize, OMRMEM_CATEGORY_PORT_LIBRARY);
  uint32_t i;
  ...
  if (convertedStringLength < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "unicode to MUTF8 test failed: %d",
                       convertedStringLength);
  }
  ...
  if (convertedStringLength < 0) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "MUTF8 to  unicode test failed: %d",
                       convertedStringLength);
  }
  portTestEnv->log("mutf string length = %d\n", convertedStringLength);
  if (!compareBytes(
        (char*)unicodeData, (char*)unicodeResult, 2 * numCodePoints)) {
    outputErrorMessage(PORTTEST_ERROR_ARGS, "Converted string wrong.");
  }
  reportTestExit(OMRPORTLIB, testName);
}
...
TEST(PortStrTest, str_Latin1ToMutf8)
{
  ...
  convertedStringLength = omrstr_convert(J9STR_CODE_LATIN1,
                                         J9STR_CODE_MUTF8,
                                         inBuff,
                                         originalStringLength,
                                         outBuff,
  ...
  /* test string length */
  convertedStringLength = omrstr_convert(
    J9STR_CODE_LATIN1, J9STR_CODE_MUTF8, inBuff, originalStringLength, NULL, 0);
  if (convertedStringLength != expectedStringLength) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "buffer length wrong.  Expected %d actual %d\n",
                       expectedStringLength,
...
TEST(PortStrTest, str_Mutf8ToLatin1)
{
  ...
  if (convertedStringLength != expectedLatin1ToFromMutf8Length) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "buffer length wrong.  Expected %d actual %d\n",
                       expectedLatin1ToFromMutf8Length,
                       convertedStringLength);
  }
...
TEST(PortStrTest, str_WinacpToMutf8)
{
  ...
  char winacpData[] = { '\x1',  '\x2',  '\x3',  '\x4',  '\x5',  '\x6',
                        ...
                        '\xc1', '\xd1', '\xe1', '\xf1' };
#if defined(OMR_OS_WINDOWS)
  char expectedMutf8[] = { '\x1',  '\x2',  '\x3',  '\x4',  '\x5',
                           '\x6',  '\x7',  '\x8',  '\xc2', '\x81',
                           '\xe2', '\x80', '\x98', '\xc2', '\xa1',
                           '\xc2', '\xb1', '\xc3', '\x81', '\xc3',
                           '\x91', '\xc3', '\xa1', '\xc3', '\xb1' };
...
#endif /* defined(OMR_OS_WINDOWS) */
  memset(outBuff, 0, sizeof(outBuff));
  convertedStringLength = omrstr_convert(J9STR_CODE_WINDEFAULTACP,
                                         J9STR_CODE_MUTF8,
                                         winacpData,
                                         originalStringLength,
                                         outBuff,
 ...
 */
static int
printToBuffer(struct OMRPortLibrary* portLibrary,
              BOOLEAN precision,
              char* buffer,
              size_t bufferLength,
