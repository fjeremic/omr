 ...
 */
void
CudaDeviceTest::testFunction(OMRPortLibrary* portLibrary,
                             uint32_t deviceId,
                             J9CudaFunction function)
{
  ...
  for (size_t index = 0; index < LENGTH_OF(functionAttributes); ++index) {
    int32_t value = 0;
    int32_t rc = omrcuda_funcGetAttribute(
      deviceId, function, functionAttributes[index], &value);
  ...
  for (size_t index = 0; index < allCacheConfigs.length; ++index) {
    int32_t rc = omrcuda_funcSetCacheConfig(
      deviceId, function, allCacheConfigs.data[index]);
  ...
  /* occupancy */
  {
    ...
    int32_t rc =
      omrcuda_funcMaxActiveBlocksPerMultiprocessor(deviceId,
                                                   function,
                                                   blockSize,
...

  {
    ...
    int32_t rc = omrcuda_funcMaxPotentialBlockSize(deviceId,
                                                   function,
                                                   NULL,
                                                   fixedDynamicSharedMemorySize,
                                                   blockSizeLimit,
                                                   flags,
                                                   &minGridSize,
                                                   &maxBlockSize);
    ...
    rc = omrcuda_funcMaxPotentialBlockSize(deviceId,
                                           function,
                                           variableDynamicSharedMemorySize,
                                           userData,
                                           blockSizeLimit,
                                           flags,
                                           &minGridSize,
...
TEST_F(CudaDeviceTest, module)
{
  ...
  for (uint32_t deviceId = 0; deviceId < deviceCount; ++deviceId) {
    ...
    rc =
      omrcuda_moduleGetSurfaceRef(deviceId, module, "surfaceHandle", &address);

    ASSERT_EQ(0, rc) << "omrcuda_moduleGetSurfaceRef failed";
    ASSERT_NE(0U, address) << "null surface address";

    rc =
      omrcuda_moduleGetTextureRef(deviceId, module, "textureHandle", &address);
