...
static uintptr_t variableDynamicSharedMemorySize(uint32_t blockSize,
                                                 uintptr_t userData) {
 ...
 */
void CudaDeviceTest::testFunction(OMRPortLibrary *portLibrary,
                                  uint32_t deviceId, J9CudaFunction function) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);

  for (size_t index = 0; index < LENGTH_OF(functionAttributes); ++index) {
    int32_t value = 0;
    int32_t rc = omrcuda_funcGetAttribute(deviceId, function,
                                          functionAttributes[index], &value);

    ASSERT_EQ(0, rc) << "omrcuda_funcGetAttribute(" << functionAttributes[index]
                     << ") failed";
  }

  for (size_t index = 0; index < allCacheConfigs.length; ++index) {
    int32_t rc = omrcuda_funcSetCacheConfig(deviceId, function,
                                            allCacheConfigs.data[index]);

    ASSERT_EQ(0, rc) << "omrcuda_funcSetCacheConfig("
                     << allCacheConfigs.data[index] << ") failed";
  }

  for (size_t index = 0; index < allSharedMemConfigs.length; ++index) {
    int32_t rc = omrcuda_funcSetSharedMemConfig(
        deviceId, function, allSharedMemConfigs.data[index]);

    ASSERT_EQ(0, rc) << "omrcuda_funcSetSharedMemConfig("
                     << allSharedMemConfigs.data[index] << ") failed";
  }
  ...
  /* occupancy */
  {
    ...
    if (J9CUDA_ERROR_NOT_SUPPORTED != rc) {
      ASSERT_EQ(0, rc)
          << "omrcuda_funcMaxActiveBlocksPerMultiprocessor() failed";
    }
  }

  {
    ...
    int32_t rc = omrcuda_funcMaxPotentialBlockSize(
        deviceId, function, NULL, fixedDynamicSharedMemorySize, blockSizeLimit,
        flags, &minGridSize, &maxBlockSize);

    if (J9CUDA_ERROR_NOT_SUPPORTED != rc) {
      ASSERT_EQ(0, rc) << "omrcuda_funcMaxPotentialBlockSize(fixed) failed";
    }

    rc = omrcuda_funcMaxPotentialBlockSize(
        deviceId, function, variableDynamicSharedMemorySize, userData,
