...
static const Attribute deviceAttributes[] = {
    ...
    ATTRIBUTE_ENTRY(MAXIMUM_TEXTURE1D_LINEAR_WIDTH, 128 * 1024 * 1024,
                    128 * 1024 * 1024),
...
TEST_F(CudaDeviceTest, attribute) {
  ...
  for (uint32_t deviceId = 0; deviceId < deviceCount; ++deviceId) {
    for (const Attribute *entry = deviceAttributes; NULL != entry->name;
         ++entry) {
      int32_t value = 0;
      int32_t rc =
          omrcuda_deviceGetAttribute(deviceId, entry->attribute, &value);

      ASSERT_EQ(0, rc) << "omrcuda_deviceGetAttribute(" << entry->attribute
                       << ") failed";
...
TEST_F(CudaDeviceTest, cacheConfig) {
  ...
  for (uint32_t deviceId = 0; deviceId < deviceCount; ++deviceId) {
    ...
    for (size_t index = 0; index < allCacheConfigs.length; ++index) {
      ...
      ASSERT_EQ(0, rc) << "omrcuda_deviceSetCacheConfig("
...
#define LIMIT_ENTRY(name, minComputeCapability, minValue, maxValue)    \
  {                                                                    \
#name, J9CUDA_DEVICE_LIMIT_##name, minComputeCapability, minValue, \
        maxValue                                                       \
  }
...
TEST_F(CudaDeviceTest, limit) {
  ...
  for (uint32_t deviceId = 0; deviceId < deviceCount; ++deviceId) {
    ...
    rc = omrcuda_deviceGetAttribute(deviceId,
    ...
    for (const Limit *entry = deviceLimits; NULL != entry->name; ++entry) {
      ...
      if (computeCapability < entry->minComputeCapability) {
        ASSERT_EQ(J9CUDA_ERROR_UNSUPPORTED_LIMIT, rc)
            << "omrcuda_deviceGetLimit should have failed";
      } else {
...
TEST_F(CudaDeviceTest, name) {
  ...
  for (uint32_t deviceId = 0; deviceId < deviceCount; ++deviceId) {
    char deviceName[200];
    int32_t rc =
        omrcuda_deviceGetName(deviceId, sizeof(deviceName), deviceName);
...
TEST_F(CudaDeviceTest, sharedMemoryConfig) {
  ...
  for (uint32_t deviceId = 0; deviceId < deviceCount; ++deviceId) {
    J9CudaSharedMemConfig sharedMemConfig =
    ...
    for (size_t index = 0; index < allSharedMemConfigs.length; ++index) {
      rc = omrcuda_deviceSetSharedMemConfig(deviceId,
                                            allSharedMemConfigs.data[index]);

      ASSERT_EQ(0, rc) << "omrcuda_deviceSetSharedMemConfig("
...
TEST_F(CudaDeviceTest, streamPriorityRange) {
  ...
  for (uint32_t deviceId = 0; deviceId < deviceCount; ++deviceId) {
    ...
    int32_t rc = omrcuda_deviceGetStreamPriorityRange(deviceId, &leastPriority,
                                                      &greatestPriority);
