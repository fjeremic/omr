...
TEST(PortIntrospectTest, introspect_test_set_signal_offset)
{
  ...
  if (0 == status) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrintrospect_set_suspend_signal_offset with negative offset succeeded unexpectedly");
  }
  ...
  if (0 == status) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS, "omrintrospect_set_suspend_signal_offset out of range offset succeeded unexpectedly");
  }
