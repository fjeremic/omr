...
TEST(PortIntrospectTest, introspect_test_set_signal_offset)
{
  ...
  for (int32_t i = 0; i <= SIGRTMAX - SIGRTMIN; ++i) {
    ...
    if (0 != status) {
      outputErrorMessage(
        PORTTEST_ERROR_ARGS,
        "omrintrospect_set_suspend_signal_offset failed unexpectedly");
    }
  }
  ...
  if (0 == status) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrintrospect_set_suspend_signal_offset with negative "
                       "offset succeeded unexpectedly");
  }
  ...
  if (0 == status) {
    outputErrorMessage(PORTTEST_ERROR_ARGS,
                       "omrintrospect_set_suspend_signal_offset out of range "
                       "offset succeeded unexpectedly");
  }
#else  /* defined(OMR_CONFIGURABLE_SUSPEND_SIGNAL) */
  portTestEnv->log("verify that omrintrospect_set_suspend_signal_offset "
                   "returns failure on unsupported platforms\n");
  ...
  if (0 == status) {
    outputErrorMessage(
      PORTTEST_ERROR_ARGS,
      "omrintrospect_set_suspend_signal_offset succeeded unexpectedly");
  }
