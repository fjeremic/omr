...
TEST_F(CudaDeviceTest, memory)
{
  ...
  for (uint32_t deviceId = 0; deviceId < deviceCount; ++deviceId) {
    ...
    rc =
      omrcuda_memcpyHostToDevice(deviceId, deviceBuf1, hostBuf1, BufferBytes);

    ASSERT_EQ(0, rc) << "omrcuda_memcpyHostToDevice failed";

    rc = omrcuda_memcpyDeviceToDevice(
      deviceId, deviceBuf2, deviceBuf1, BufferBytes);

    ASSERT_EQ(0, rc) << "omrcuda_memcpyDeviceToDevice failed";

    rc =
      omrcuda_memcpyDeviceToHost(deviceId, hostBuf2, deviceBuf2, BufferBytes);
    ...
    rc =
      omrcuda_memcpyDeviceToHost(deviceId, hostBuf1, deviceBuf1, BufferBytes);
