  ...
  if (0 != retVal) {
    ...
    if (NULL == errMsg) {
      ...
      portTestEnv->log(LEVEL_ERROR,
        ...
        command[0], command[1], portableErrno, errMsg);
    }
...
intptr_t waitForTestProcess(OMRPortLibrary* portLibrary, OMRProcessHandle processHandle)
{
  ...
  if (0 != j9process_waitfor(OMRPORTLIB, processHandle)) {
    ...
    if (NULL == errMsg) {
      portTestEnv->log(
        LEVEL_ERROR, "waitForTestProcess: j9process_waitfor() failed\n\tportableErrno = %d\n", portableErrno);
    } else {
      portTestEnv->log(LEVEL_ERROR,
  ...
  if (0 != j9process_close(OMRPORTLIB, &processHandle, 0)) {
    ...
    if (NULL == errMsg) {
      portTestEnv->log(
        LEVEL_ERROR, "waitForTestProcess: j9process_close() failed\n\tportableErrno = %d\n", portableErrno);
    } else {
      portTestEnv->log(LEVEL_ERROR,
 ...
 */
static intptr_t getUnicodeCmdline(
  ...
  uint32_t options, OMRProcessHandle* processHandle)
{
  ...
  processHandleStruct
    = (OMRProcessHandleStruct*)omrmem_allocate_memory(sizeof(OMRProcessHandleStruct), OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  if (0 == CreatePipe(&(pipes.inR), &(pipes.inW), &sAttrib, 512)) {
    ...
    if (0
      == DuplicateHandle(
        GetCurrentProcess(), pipes.inW, GetCurrentProcess(), &(pipes.inDup), 0, FALSE, DUPLICATE_SAME_ACCESS)) {
      closeAndDestroyPipes(&pipes);
      return OMRPROCESS_ERROR;
    }
  ...
  if (OMRPROCESS_DEBUG == options) {
    ...
    if (0 == CreatePipe(&(pipes.outR), &(pipes.outW), &sAttrib, 512)) {
      ...
      if (0
        == DuplicateHandle(
          GetCurrentProcess(), pipes.outR, GetCurrentProcess(), &(pipes.outDup), 0, FALSE, DUPLICATE_SAME_ACCESS)) {
        closeAndDestroyPipes(&pipes);
        return OMRPROCESS_ERROR;
      }
    ...
    if (0 == CreatePipe(&(pipes.errR), &(pipes.errW), &sAttrib, 512)) {
      ...
      if (0
        == DuplicateHandle(
          GetCurrentProcess(), pipes.errR, GetCurrentProcess(), &(pipes.errDup), 0, FALSE, DUPLICATE_SAME_ACCESS)) {
        closeAndDestroyPipes(&pipes);
        return OMRPROCESS_ERROR;
      }
      if (NULL != pipes.errR) {
      ...
      if (0 == rc) {
        ...
        if (0
          == CreateProcessW(/* CreateProcessW returns 0 upon failure */
            ...
            unicodeEnv, unicodeDir, &sinfo, &pinfo) /* Pointer to PROCESS_INFORMATION structure; */
        ) {
          rc = OMRPROCESS_ERROR;
        } else {
