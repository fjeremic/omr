...
intptr_t openLaunchSemaphore(OMRPortLibrary* portLibrary,
                             const char* name,
                             uintptr_t nProcess) {
  intptr_t semid =
...
intptr_t SetLaunchSemaphore(OMRPortLibrary* portLibrary,
                            intptr_t semaphore,
                            uintptr_t nProcess) {
  union semun sem_union;
...
intptr_t ReleaseLaunchSemaphore(OMRPortLibrary* portLibrary,
                                intptr_t semaphore,
                                uintptr_t nProcess) {
...
static intptr_t translateModifiedUtf8ToPlatform(OMRPortLibrary* portLibrary,
  ...
  *outBuffer = NULL;
  ...
  result =
      (char*)omrmem_allocate_memory(bufferLength, OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == result) {
...
wchar_t* convertFromUTF8(OMRPortLibrary* portLibrary,
                         const char* string,
...
intptr_t j9process_create(OMRPortLibrary* portLibrary,
                          const char* command[],
                          uintptr_t commandLength,
                          const char* dir,
                          uint32_t options,
...
#if defined(OSX)
    newCommand[i] = (char*)omrmem_allocate_memory(strlen(command[i]) + 1,
                                                  OMRMEM_CATEGORY_PORT_LIBRARY);
