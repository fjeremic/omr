...
j9process_create(OMRPortLibrary* portLibrary, const char* command[], uintptr_t commandLength, const char* dir, uint32_t options, OMRProcessHandle* processHandle)
{
    ...
    if (0 == CreatePipe(&(pipes.inR), &(pipes.inW), &sAttrib, 512)) {
        ...
        if (0 == DuplicateHandle(GetCurrentProcess(), pipes.inW, GetCurrentProcess(), &(pipes.inDup), 0, FALSE, DUPLICATE_SAME_ACCESS)) {
            closeAndDestroyPipes(&pipes);
            return OMRPROCESS_ERROR;
        }
    ...
    if (OMRPROCESS_DEBUG == options) {
        ...
        if (0 == CreatePipe(&(pipes.outR), &(pipes.outW), &sAttrib, 512)) {
            ...
            if (0 == DuplicateHandle(GetCurrentProcess(), pipes.outR, GetCurrentProcess(), &(pipes.outDup), 0, FALSE, DUPLICATE_SAME_ACCESS)) {
                closeAndDestroyPipes(&pipes);
                return OMRPROCESS_ERROR;
            }
        ...
        if (0 == CreatePipe(&(pipes.errR), &(pipes.errW), &sAttrib, 512)) {
            ...
            if (0 == DuplicateHandle(GetCurrentProcess(), pipes.errR, GetCurrentProcess(), &(pipes.errDup), 0, FALSE, DUPLICATE_SAME_ACCESS)) {
                closeAndDestroyPipes(&pipes);
                return OMRPROCESS_ERROR;
            }
            if (NULL != pipes.errR) {
        ...
        if (0 == rc) {
            ...
            if (0 == rc) {
                ...
                if (0 == CreateProcessW(/* CreateProcessW returns 0 upon failure */
                             NULL, unicodeCmdline, NULL, NULL, TRUE, creationFlags, /* add DEBUG_ONLY_THIS_PROCESS for smoother debugging */
