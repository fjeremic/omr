...
#if !defined(OSX)
static intptr_t
translateModifiedUtf8ToPlatform(OMRPortLibrary* portLibrary,
                                const char* inBuffer,
...
intptr_t
openLaunchSemaphore(OMRPortLibrary* portLibrary,
                    const char* name,
                    uintptr_t nProcess)
{
  intptr_t semid =
...
intptr_t
SetLaunchSemaphore(OMRPortLibrary* portLibrary,
                   intptr_t semaphore,
                   uintptr_t nProcess)
{
  union semun sem_union;
...
intptr_t
ReleaseLaunchSemaphore(OMRPortLibrary* portLibrary,
                       intptr_t semaphore,
                       uintptr_t nProcess)
{
...
OMRProcessHandle
launchChildProcess(OMRPortLibrary* portLibrary,
                   const char* testname,
                   const char* argv0,
                   const char* options)
{
  ...
  retVal = j9process_create(OMRPORTLIB,
                            (const char**)command,
                            commandLength,
                            ".",
                            exeoptions,
                                ...
                                char** outBuffer)
{
  ...
  *outBuffer = NULL;
  bufferLength = omrstr_convert(
    J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, inBuffer, inBufferSize, NULL, 0);
  ...
  resultLength = omrstr_convert(J9STR_CODE_MUTF8,
...
intptr_t
j9process_close(OMRPortLibrary* portLibrary,
...
intptr_t
j9process_get_exitCode(OMRPortLibrary* portLibrary,
                       OMRProcessHandle processHandle)
{
  OMRProcessHandleStruct* processHandleStruct =
    (OMRProcessHandleStruct*)processHandle;
...
wchar_t*
convertFromUTF8(OMRPortLibrary* portLibrary,
                const char* string,
                ...
                uintptr_t unicodeBufferSize)
{
  ...
  if (0 == MultiByteToWideChar(OS_ENCODING_CODE_PAGE,
 ...
 */
static intptr_t
getUnicodeCmdline(struct OMRPortLibrary* portLibrary,
                  const char* command[],
                  ...
                  wchar_t** unicodeCmdline)
{
   ...
   */
  needToBeQuoted =
    (char*)omrmem_allocate_memory(commandLength, OMRMEM_CATEGORY_PORT_LIBRARY);
    ...
    if (NULL == commandAsString) {
      ...
      *unicodeCmdline = (wchar_t*)omrmem_allocate_memory(
      ...
      if (NULL == *unicodeCmdline) {
        ...
        convertFromUTF8(
          OMRPORTLIB, commandAsString, *unicodeCmdline, length + 1);
...
intptr_t
j9process_create(OMRPortLibrary* portLibrary,
                 const char* command[],
                 uintptr_t commandLength,
                 const char* dir,
                 uint32_t options,
