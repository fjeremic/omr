...
#if !defined(OSX)
static intptr_t translateModifiedUtf8ToPlatform(OMRPortLibrary *portLibrary,
                                                const char *inBuffer,
...
intptr_t openLaunchSemaphore(OMRPortLibrary *portLibrary, const char *name,
                             uintptr_t nProcess) {
  intptr_t semid =
      semget(PROCESS_HELPER_SEM_KEY, 1, 0660 | IPC_CREAT | IPC_EXCL);
...
intptr_t SetLaunchSemaphore(OMRPortLibrary *portLibrary, intptr_t semaphore,
                            uintptr_t nProcess) {
  union semun sem_union;
...
intptr_t ReleaseLaunchSemaphore(OMRPortLibrary *portLibrary, intptr_t semaphore,
                                uintptr_t nProcess) {
...
intptr_t WaitForLaunchSemaphore(OMRPortLibrary *portLibrary,
                                intptr_t semaphore) {
  struct sembuf buffer;
...
OMRProcessHandle launchChildProcess(OMRPortLibrary *portLibrary,
                                    const char *testname, const char *argv0,
                                    const char *options) {
  ...
  if (retVal != 0) {
    ...
    if (NULL == errMsg) {
      portTestEnv->log(LEVEL_ERROR,
                       "%s: launchChildProcess: omrsysinfo_get_executable_name "
                       "failed!\n\tportableErrno = %d\n",
  ...
  retVal = j9process_create(OMRPORTLIB, (const char **)command, commandLength,
                            ".", exeoptions, &processHandle);
  ...
  if (0 != retVal) {
    ...
    if (NULL == errMsg) {
      portTestEnv->log(LEVEL_ERROR,
                       "%s: launchChildProcess: Failed to start process '%s "
                       "%s'\n\tportableErrno = %d\n",
...
intptr_t waitForTestProcess(OMRPortLibrary *portLibrary,
                            OMRProcessHandle processHandle) {
  intptr_t retval = -1;
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);

  if (NULL == processHandle) {
    portTestEnv->log(LEVEL_ERROR,
                     "waitForTestProcess: processHandle == NULL\n");
    goto done;
  }
  ...
  if (0 != j9process_waitfor(OMRPORTLIB, processHandle)) {
    ...
    if (NULL == errMsg) {
      portTestEnv->log(LEVEL_ERROR,
                       "waitForTestProcess: j9process_waitfor() "
                       "failed\n\tportableErrno = %d\n",
                       portableErrno);
    } else {
      portTestEnv->log(LEVEL_ERROR,
  ...
  if (0 != j9process_close(OMRPORTLIB, &processHandle, 0)) {
    ...
    if (NULL == errMsg) {
      portTestEnv->log(LEVEL_ERROR,
                       "waitForTestProcess: j9process_close() "
                       "failed\n\tportableErrno = %d\n",
                       portableErrno);
    } else {
      portTestEnv->log(LEVEL_ERROR,
...
static intptr_t translateModifiedUtf8ToPlatform(OMRPortLibrary *portLibrary,
  ...
  *outBuffer = NULL;
  ...
  result = (char *)omrmem_allocate_memory(bufferLength,
...
intptr_t j9process_close(OMRPortLibrary *portLibrary,
                         OMRProcessHandle *processHandle, uint32_t options) {
  OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
  int32_t rc = 0;
  OMRProcessHandleStruct *processHandleStruct =
      (OMRProcessHandleStruct *)*processHandle;
...
intptr_t j9process_waitfor(OMRPortLibrary *portLibrary,
                           OMRProcessHandle processHandle) {
  OMRProcessHandleStruct *processHandleStruct =
      (OMRProcessHandleStruct *)processHandle;
...
#if defined(OMR_OS_WINDOWS)
  if (WAIT_OBJECT_0 ==
      WaitForSingleObject((HANDLE)processHandleStruct->procHandle, INFINITE)) {
    DWORD procstat = 0;

    if (0 != GetExitCodeProcess((HANDLE)processHandleStruct->procHandle,
                                (LPDWORD)&procstat)) {
  ...
  pid_t retVal =
      waitpid((pid_t)processHandleStruct->procHandle, &statusLocation, 0);
...
intptr_t j9process_get_exitCode(OMRPortLibrary *portLibrary,
                                OMRProcessHandle processHandle) {
  OMRProcessHandleStruct *processHandleStruct =
      (OMRProcessHandleStruct *)processHandle;
 ...
 */
static void closeAndDestroyPipes(OMRProcessWin32Pipes *pipes) {
...
wchar_t *convertFromUTF8(OMRPortLibrary *portLibrary, const char *string,
                         wchar_t *unicodeBuffer, uintptr_t unicodeBufferSize) {
  ...
  if (length < unicodeBufferSize) {
    ...
    unicodeString = (wchar_t *)omrmem_allocate_memory(
        (length + 1) * 2, OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == unicodeString) {
      return NULL;
    }
  }
  if (0 == MultiByteToWideChar(OS_ENCODING_CODE_PAGE, OS_ENCODING_MB_FLAGS,
                               string, -1, unicodeString, (int)length + 1)) {
 ...
 */
static intptr_t getUnicodeCmdline(struct OMRPortLibrary *portLibrary,
                                  const char *command[],
                                  uintptr_t commandLength,
                                  wchar_t **unicodeCmdline) {
   ...
   */
  needToBeQuoted = (char *)omrmem_allocate_memory(commandLength,
                                                  OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == needToBeQuoted) {
    ...
    length = commandLength; /*add 1 <blank> between each token + a reserved
                               place for the last NULL*/
      ...
      if (commandILength > 0) {
        ...
        if (commandStart[0] != '"') {
          for (j = 0; j < (intptr_t)commandILength; j += 1) {
            if (commandStart[j] == ' ') {
              needToBeQuoted[i] =
                  '\1';    /* a random value, different from zero though*/
    ...
    ptr = commandAsString =
        (char *)omrmem_allocate_memory(length, OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == commandAsString) {
      ...
      *unicodeCmdline = (wchar_t *)omrmem_allocate_memory(
          (length + 1) * 2, OMRMEM_CATEGORY_PORT_LIBRARY);
      if (NULL == *unicodeCmdline) {
        rc = OMRPROCESS_ERROR;
      } else {
        convertFromUTF8(OMRPORTLIB, commandAsString, *unicodeCmdline,
                        length + 1);
...
intptr_t j9process_create(OMRPortLibrary *portLibrary, const char *command[],
                          uintptr_t commandLength, const char *dir,
  ...
  DWORD dwMode =
      PIPE_NOWAIT; /* it's only used if OMRPORT_PROCESS_NONBLOCKING_IO is set */
  ...
  if (0 == CreatePipe(&(pipes.inR), &(pipes.inW), &sAttrib, 512)) {
    ...
    if (0 == DuplicateHandle(GetCurrentProcess(), pipes.inW,
                             GetCurrentProcess(), &(pipes.inDup), 0, FALSE,
  ...
  if (OMRPROCESS_DEBUG == options) {
    ...
    if (0 == CreatePipe(&(pipes.outR), &(pipes.outW), &sAttrib, 512)) {
      ...
      if (0 == DuplicateHandle(GetCurrentProcess(), pipes.outR,
                               GetCurrentProcess(), &(pipes.outDup), 0, FALSE,
                               DUPLICATE_SAME_ACCESS)) {
        closeAndDestroyPipes(&pipes);
        return OMRPROCESS_ERROR;
      }
    ...
    if (0 == CreatePipe(&(pipes.errR), &(pipes.errW), &sAttrib, 512)) {
      ...
      if (0 == DuplicateHandle(GetCurrentProcess(), pipes.errR,
                               GetCurrentProcess(), &(pipes.errDup), 0, FALSE,
                               DUPLICATE_SAME_ACCESS)) {
        closeAndDestroyPipes(&pipes);
        return OMRPROCESS_ERROR;
      }
  ...
  /* Create a NULL terminated array to contain the command for call to
   * execv[p|e]() We could do this in the child, but if mem_allocate_memory
   * fails, it's easier to diagnose failure in parent. Remember to free the
   * memory
   */
  newCommandSize = (commandLength + 1) * sizeof(uintptr_t);
  newCommand = (char **)omrmem_allocate_memory(newCommandSize,
                                               OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == newCommand) {
    return OMRPROCESS_ERROR;
  }
...
#if defined(OSX)
    newCommand[i] = (char *)omrmem_allocate_memory(
        strlen(command[i]) + 1, OMRMEM_CATEGORY_PORT_LIBRARY);
    omrstr_printf(newCommand[i], strlen(command[i]) + 1, command[i]);
#else  /* defined(OSX) */
    intptr_t translateStatus = translateModifiedUtf8ToPlatform(
        OMRPORTLIB, command[i], strlen(command[i]), &(newCommand[i]));
  ...
  if (grdpid == 0) {
    ...
    /* If we get here, tell the parent that the execv failed! Send the error
     * number. */
    if (-1 != rc) {
    ...
    /* If the exec failed, we must exit or there will be two VM processes
     * running. */
    exit(rc);
  } else {
    ...
    /* [PR CMVC 143339]
     * Instead of using timeout to determine if child process has been created
     * successfully, a single block read/write pipe call is used, if process
     * creation failed, errorNumber with sizeof(errno) will be returned,
     * otherwise, read will fail due to pipe closure.
     */
    if (-1 != rc) {
