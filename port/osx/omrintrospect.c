...
resumeAllPreempted(PlatformWalkData* data)
{
  if (data->cleanupRequired) {
    ...
    if ((0 == (SA_SIGINFO & data->oldHandler.sa_flags)) &&
        (SIG_DFL == data->oldHandler.sa_handler)) {
      /* If there wasn't an old signal handler then set this to ignore. There
       * shouldn't be any signals, but better safe than sorry.
       */
...
setupNativeThread(J9ThreadWalkState* state, thread_context* sigContext)
{
  ...
  if (0 == rc) {
    ...
    if (NULL == sigContext) {
      /* Generate the context by installing a signal handler and raising a
       * signal. None was provided. */
      ...
      /* Copy the context. We're using the provided context instead of
       * generating it. */
  ...
  if (0 == rc) {
    ...
    if ((NULL != state->current_thread->callstack) &&
        (NULL == state->current_thread->callstack->symbol)) {
                                          ...
                                          void* signal_info)
{
  ...
  data = (PlatformWalkData*)portLibrary->heap_allocate(
    portLibrary, heap, sizeof(PlatformWalkData));
...
/* This function is identical to j9introspect_threads_startDo_with_signal but
