...
void *reserveMemory(struct OMRPortLibrary *portLibrary, void *address,
...
int32_t omrvmem_startup(struct OMRPortLibrary *portLibrary) {
  ...
  /* The OSX man pages for mmap state that mach/vm_statistics.h specifies the
   * superpage size, but in practice it appears to always be 2MB.
   */
...
void *omrvmem_commit_memory(struct OMRPortLibrary *portLibrary, void *address,
  ...
  if (rangeIsValid(identifier, address, byteAmount)) {
    ...
    } else if (PPG_vmem_pageSize[2] == identifier->pageSize ||
               PPG_vmem_pageFlags[1] == identifier->pageFlags) {
      rc = address;
    }
  } else {
    ...
    if (rangeIsValid(identifier, address, byteAmount)) {
      ...
      if (0 < byteAmount) {
        if (OMRPORT_VMEM_RESERVE_USED_MMAP == identifier->allocator) {
          ...
          /* OSX cannot use shmat/shmget to allocate large parges. It uses mmap
           * to allocate superpages. */
          result = -1;
        }
...
void *omrvmem_reserve_memory_ex(struct OMRPortLibrary *portLibrary,
  ...
  if (0 != (OMRPORT_VMEM_PAGE_FLAG_SUPERPAGE_ANY & params->pageFlags)) {
    memoryPointer = getMemoryInRange(
        ...
        params->startAddress, params->endAddress, 0, params->options,
        params->pageSize, params->pageFlags, params->mode);
    ...
    if ((0 == minimumGranule) || (0 == (alignmentInBytes % minimumGranule))) {
      memoryPointer = getMemoryInRange(
          ...
          params->options, params->pageSize, params->pageFlags, params->mode);
    }
    ...
    if ((0 == largePageMinimumGranule) ||
      ...
      memoryPointer = getMemoryInRange(
          portLibrary, identifier, category, params->byteAmount,
          params->startAddress, params->endAddress, largePageAlignmentInBytes,
          params->options, params->pageSize, params->pageFlags, params->mode);
    }
      ...
      if (0 == (OMRPORT_VMEM_STRICT_PAGE_SIZE & params->options)) {
        ...
        if ((0 == minimumGranule) ||
          ...
          memoryPointer = getMemoryInRange(
              portLibrary, identifier, category, params->byteAmount,
              params->startAddress, params->endAddress, alignmentInBytes,
              params->options, PPG_vmem_pageSize[0],
              OMRPORT_VMEM_PAGE_FLAG_NOT_USED, params->mode);
        }
      } else {
        update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, 0, NULL);
      }
...
void *reserveMemory(struct OMRPortLibrary *portLibrary, void *address,
  ...
  if (MAP_FAILED == result) {
    ...
    if (0 != (OMRPORT_VMEM_MEMORY_MODE_COMMIT & mode)) {
      if (NULL ==
          omrvmem_commit_memory(portLibrary, result, byteAmount, identifier)) {
...
intptr_t omrvmem_find_valid_page_size(struct OMRPortLibrary *portLibrary,
  ...
  /* If flag OMRPORT_VMEM_PAGE_FLAG_SUPERPAGE_ANY is specified, page size is
   * irrelevant. */
...
static void *getMemoryInRange(struct OMRPortLibrary *portLibrary,
  ...
  if (NULL == memoryPointer) {
    ...
    while ((startAddress <= currentAddress) && (endAddress >= currentAddress)) {
      memoryPointer =
          reserveMemory(portLibrary, currentAddress, byteAmount, identifier,
                        mode, pageSize, pageFlags, category);
  ...
  if (0 == (OMRPORT_VMEM_STRICT_ADDRESS & vmemOptions) &&
      (NULL == memoryPointer)) {
  allocAnywhere:
    memoryPointer = reserveMemory(portLibrary, NULL, byteAmount, identifier,
                                  mode, pageSize, pageFlags, category);
  }
...
int32_t omrvmem_get_process_memory_size(struct OMRPortLibrary *portLibrary,
  ...
  rc = task_info(mach_task_self(), TASK_BASIC_INFO, (task_info_t)&taskInfoData,
                 &msgTypeNumberOut);
  ...
  if (KERN_SUCCESS == rc) {
    ...
    switch (queryType) {
      ...
      case OMRPORT_VMEM_PROCESS_PRIVATE:
         ...
         * top is open source and has an implementation that estimates it by
         * walking the virtual address space of the task and looking at the
         * resident page and reference counts of mapped objects, but its broken
