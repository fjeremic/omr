...
/*
 * We don't need anything in this structure, because assembler code has the
 * required fields hard coded This structure has to be large enough to include
 * the fields used by the assembler routines Creating the structure the same
 * size as sizeof(vdso_data) when using asm/vdso_datapage.h
 */
struct vdso_data
{
 ...
 */
/*  technically, this should return int64_t since both timeval.tv_sec and
                           ...
                           uintptr_t* success)
{
  ...
  *success = 0;
  if (0 == clock_gettime(CLOCK_REALTIME, &ts)) {
    nsec = ((uint64_t)ts.tv_sec * OMRTIME_NANOSECONDS_PER_SECOND) +
           (uint64_t)ts.tv_nsec;
    *success = 1;
  }
  return nsec;
...
omrtime_nano_time(struct OMRPortLibrary* portLibrary)
{
  ...
  if (0 == clock_gettime(J9TIME_NANO_CLOCK, &ts)) {
    hiresTime = ((int64_t)ts.tv_sec * OMRTIME_NANOSECONDS_PER_SECOND) +
                (int64_t)ts.tv_nsec;
  }
 ...
 * as requested by the required resolution
 *
 * @param[in] portLibrary The port library.
 ...
 * second.  For example: \arg 1 to report elapsed time in seconds \arg 1,000 to
 * report elapsed time in milliseconds \arg 1,000,000 to report elapsed time in
 * microseconds
 *
                    ...
                    uint64_t requiredResolution)
{
  ...
  if (OMRTIME_HIRES_CLOCK_FREQUENCY == requiredResolution) {
    ...
    ticks = (uint64_t)((double)ticks * ((double)requiredResolution /
                                        (double)OMRTIME_HIRES_CLOCK_FREQUENCY));
  } else {
    ticks = (uint64_t)((double)ticks / ((double)OMRTIME_HIRES_CLOCK_FREQUENCY /
                                        (double)requiredResolution));
  }
  return ticks;
}
/**
 ...
 * that were created by @ref omrtime_startup should be destroyed here.
 *
 ...
 * that are required for the time operations may be created here.  All resources
 * created here should be destroyed in @ref omrtime_shutdown.
 *
 * @param[in] portLibrary The port library.
 *
 * @return 0 on success, negative error code on failure.  Error code values
 * returned are \arg OMRPORT_ERROR_STARTUP_TIME
 *
...
omrtime_startup(struct OMRPortLibrary* portLibrary)
{
  ...
  if (-1 != procfd) {
    systemcfgP_millis =
      mmap(0, sizeof(struct vdso_data), PROT_READ, MAP_SHARED, procfd, 0);
    close(procfd);
  ...
  /* cmvc 89134 - on some linux kernel versions the struct exists but the time
   * is not filled in, null the pointer to cause the slower fallback code to be
   * used */
  ...
  /*
   * The Linux Kernels used in SLES11 and RHEL6 (both 2.6.32) have a bug that
   * breaks our implementation of __getNanos(). The issue is that the tb_to_xs
   * field in the vdso struct can change at runtime, which causes __getNanos()
   * to sometimes report inaccurate time - including time that can go backwards.
   * SLES10 and RHEL5 do not have this issue.
   *
   * We work around this by falling back to the gettimeofday() implementation,
   * which was measured to be around 3x slower compared to __getNanos(), on
   * newer Kernel versions. Only use __getNanos() if we're on a 2.6 Kernel
   * <= 2.6.18.
   *
