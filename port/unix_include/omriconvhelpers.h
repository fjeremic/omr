...
/* To add a new use of iconv, add a descriptor name to the J9IconvName enum.
 *Names before UNCACHED_ICONV_DESCRIPTOR are cached and all others are uncached.
 *Usage pattern:
 *
 *	iconv_t converter = iconv_get(portLibrary, MY_ICONV_DESCRIPTOR, "UTF-8",
 *nl_langinfo(CODESET)); if (J9VM_INVALID_ICONV_DESCRIPTOR == converter) {
 *		handle error
 *	}
 *	...
