...
const char* errorMessage(struct OMRPortLibrary* portLibrary, int32_t errorCode)
{
...
#if defined(AIXPPC)
  /* On AIX, strerror() returns an error string based on current locale encoding. When printing out the error message in
   * file_write_using_iconv(), The NLS message (UTF-8) + the error string is converted from UTF-8 to locale encoding. So
   * non-UTF-8 error string needs to be converted to UTF-8 here.
   */
  if (0 != strcmp(nl_langinfo(CODESET), "UTF-8")) {
    ...
    if (0 < portLibrary->str_convert(
          portLibrary, J9STR_CODE_PLATFORM_RAW, J9STR_CODE_UTF8, errString, strlen(errString), stackBuf, bufLen)) {
      errString = stackBuf;
    }
  }
  ...
  portLibrary->str_printf(
    portLibrary, ptBuffers->errorMessageBuffer, ptBuffers->errorMessageBufferSize, "%s", errString);
