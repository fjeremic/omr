...
#if defined(J9OS_I5)
void get_thread_Info(struct PlatformWalkData* data, void* context_arg, unsigned long tid)
{
 ...
 */
static int setup_native_thread(J9ThreadWalkState* state, thread_context* sigContext, int heapAllocate)
{
  struct PlatformWalkData* data = (struct PlatformWalkData*)state->platform_data;
  ...
  if (heapAllocate || sigContext) {
    ...
    state->current_thread
      = (J9PlatformThread*)state->portLibrary->heap_allocate(state->portLibrary, state->heap, sizeof(J9PlatformThread));
  ...
  if (state->current_thread->callstack == NULL) {
    /* don't pass sigContext in here as we should have fixed up the thread already. It confuses heap/not heap
     * allocations if we pass it here.
     */
    ...
    if (state->current_thread->callstack == NULL && data->uninitializedThreads) {
      ...
      /* we have to copy the message into heap allocated memory because we can't differentiate constant strings and
       * alloc'd strings when freeing thread data.
       */
...
J9PlatformThread* omrintrospect_threads_nextDo(J9ThreadWalkState* state)
{
  ...
  if ((result = pthread_getthrds_np(&data->thr, PTHRDSINFO_QUERY_ALL, &data->pinfo, sizeof(data->pinfo), regbuf, &val))
    != 0) {
    RECORD_ERROR(state, COLLECTION_FAILURE, result);
    goto cleanup;
  }
