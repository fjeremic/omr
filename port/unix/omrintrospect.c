...
barrier_init_r(barrier_r* barrier, int value)
{
  ...
  } while (compareAndSwapUDATA(
             (uintptr_t*)&barrier->in_count, old_value, value) != old_value);
  do {
...
barrier_enter_r(barrier_r* barrier, uintptr_t deadline)
{
  ...
  } while (compareAndSwapUDATA((uintptr_t*)&barrier->out_count,
                               old_value,
                               old_value + 1) != old_value);
...
sem_timedwait_r(sem_t_r* sem, uintptr_t seconds)
{
  ...
  while (1) {
    ...
    while (old_value > 0) {
      if (compareAndSwapUDATA((uintptr_t*)&sem->sem_value,
                              old_value,
                              old_value - 1) == old_value) {
...
sem_trywait_r(sem_t_r* sem)
{
  ...
  old_value = compareAndSwapUDATA((uintptr_t*)&sem->sem_value, -1, -1);
  while (old_value > 0) {
    int value = compareAndSwapUDATA(
      (uintptr_t*)&sem->sem_value, old_value, old_value - 1);
    if (value == old_value) {
      /* successfully acquired lock */
      return 0;
    }
...
sem_post_r(sem_t_r* sem)
{
  ...
  do {
    old_value = sem->sem_value;
  } while (compareAndSwapUDATA((uintptr_t*)&sem->sem_value,
                               old_value,
                               old_value + 1) != old_value);
...
#if defined(J9OS_I5)
void
get_thread_Info(struct PlatformWalkData* data,
                void* context_arg,
                unsigned long tid)
{
...
upcall_handler(int signal, siginfo_t* siginfo, void* context_arg)
{
  ...
  if (ret != 0) {
    ...
    if (data->thread == NULL) {
      ...
      data->thread->dsa = __dsa_prev(getdsa(),
                                     __EDCWCCWI_LOGICAL,
                                     __EDCWCCWI_DOWN,
                                     NULL,
                                     data->thread->caa,
...
count_threads(struct PlatformWalkData* data)
{
  ...
  while (getthrds64(
           getpid(), &thread, sizeof(struct thrdentry64), &cursor, 1) == 1) {
...
suspend_all_preemptive(struct PlatformWalkData* data)
{
  ...
  do {
    int i = 0;
    do {
      ret = pthread_getthrds_np(
        &thr, PTHRDSINFO_QUERY_TID, &pinfo, sizeof(pinfo), regbuf, &val);
      if (ret != 0) {
        if (ret == ESRCH) {
...
freeThread(J9ThreadWalkState* state, J9PlatformThread* thread)
{
  ...
  while (frame) {
    ...
    if (tmp->symbol) {
      state->portLibrary->heap_free(
        state->portLibrary, state->heap, tmp->symbol);
      tmp->symbol = NULL;
    }
  ...
  if (!data->platformAllocatedContext && thread->context) {
    state->portLibrary->heap_free(
      state->portLibrary, state->heap, thread->context);
  }
...
resume_all_preempted(struct PlatformWalkData* data)
{
  ...
  /* clean up the heap */
  data->state->portLibrary->heap_free(
    data->state->portLibrary, data->state->heap, data);
 ...
 */
static int
setup_native_thread(J9ThreadWalkState* state,
                    ...
                    int heapAllocate)
{
  ...
  if (heapAllocate || sigContext) {
    ...
    state->current_thread->context =
      (thread_context*)state->portLibrary->heap_allocate(
        state->portLibrary, state->heap, size);
    ...
    if (sigContext) {
      ...
      memcpy(
        state->current_thread->context, (void*)data->thread->context, size);
    }
  } else {
 ...
 */
J9PlatformThread*
omrintrospect_threads_startDo(struct OMRPortLibrary* portLibrary,
                              J9Heap* heap,
                              J9ThreadWalkState* state)
{
  return omrintrospect_threads_startDo_with_signal(
    portLibrary, heap, state, NULL);
}
...
omrintrospect_threads_nextDo(J9ThreadWalkState* state)
{
  ...
  while (data->thr == pthread_self() || data->thr == 0) {
    if ((result = pthread_getthrds_np(&data->thr,
