...
/*
 * This is the signal that we use to suspend the various threads. This signal
 * must be not be collapsable and must pass on values specified by sigqueue.
 */
...
/*
 * Internal semaphore retry timeouts. Some implementations of poll seem
 * unreliable so we add a timeout to poll calls whether or not we have a
 * deadline so we have a sample based scheduler visible spinlock.
 */
#if defined(LINUXPPC)
/* poll timeout in milliseconds */
#define POLL_RETRY_INTERVAL 100
#elif defined(AIXPPC)
/* AIX close call blocks until all other calls using the file descriptor return
 * to user space so we need to spin reasonably quickly or we'll not resume until
 * the timeout in error cases.
 */
...
/*
 * This structure contains platform specific information necessary to iterate
 * over the threads
 */
struct PlatformWalkData {
  ...
  /* volatile error flag for upcall handlers, shared across threads, non-zero
   * means error */
  volatile unsigned char error;
  /* is the state currently consistent. False inside startDo/nextDo, must be
   * true otherwise */
  ...
  /* barrier to prevent any threads exiting the introspect logic until all are
   * ready */
...
/*
 * This function constructs a barrier for use within signal handler contexts.
 * It's a combination of spinlock techniques modified with blocking
 * read/write/poll scheduler visible calls to avoid thread contention.
 *
 ...
 */
static int barrier_init_r(barrier_r *barrier, int value) {
  uintptr_t old_value;
  ...
  } while (compareAndSwapUDATA((uintptr_t *)&barrier->initial_value, old_value,
                               value) != old_value);
  do {
    old_value = barrier->in_count;
  } while (compareAndSwapUDATA((uintptr_t *)&barrier->in_count, old_value,
                               value) != old_value);
  do {
    old_value = barrier->released;
  } while (compareAndSwapUDATA((uintptr_t *)&barrier->released, old_value, 0) !=
           old_value);
...
/*
 * This function is a utility function used by the blocking barrier functions to
 * wait in a polite, scheduler visible fashion until either prompted or the
 * deadline has expired.
 *
 * @param barrier the barrier we're operating on
 * @param deadline the system time in seconds after which we should return
 * regardless of prompting
 *
 * @return 0 if cleanly prompted, -1 if timed out, other values are various
 * errors
 */
static int barrier_block_until_poked(barrier_r *barrier, uintptr_t deadline) {
  int ret = 0;
  struct pollfd fds[1];
  ...
  if (deadline == 0) {
    ...
    if (clock_gettime(CLOCK_REALTIME, &spec) == -1) {
      ...
      /* ensure that the poll timeout is set to the lesser of the retry interval
       * or the deadline */
      if (interval < 0) {
  ...
  if ((ret == -1 && errno != EINTR) ||
      fds[0].revents & (POLLHUP | POLLERR | POLLNVAL)) {
    return -2;
  }

  if (deadline != 0) {
    /* check if we've timed out. Do this last as the other possibilities for
     * return are of more interest */
    if (clock_gettime(CLOCK_REALTIME, &spec) == -1) {
      return -3;
    }
...
/*
 * This function is called by the conductor of the barrier. It will block until
 * all entrants have entered the barrier via barrier_enter_r. If a maximum
 * amount of time to block for is specified then this will time out once that
 * deadline has expired.
 *
 * @param barrier the barrier to release
 * @param seconds the maximum number of second for which to block. Indefinitely
 * if 0 is specified.
 *
 * @return 0 on clean release, other values are various errors
 */
static int barrier_release_r(barrier_r *barrier, uintptr_t seconds) {
  char byte = 0;
  int result = 0;
  ...
  if ((compareAndSwapUDATA((uintptr_t *)&barrier->released, 0, 1)) != 0) {
    /* barrier->released should have been 0, write something into the pipe so
     * that poll doesn't block, converts this to a busy wait
     */
    barrier->released = 1;
...
/*
 * This function is called by entrants to the barrier. It will block until all
 * expected entrants have called this function and barrier_release_r has been
 * called, or the deadline has expired.
 *
 ...
 */
static int barrier_enter_r(barrier_r *barrier, uintptr_t deadline) {
  ...
  if (old_value == 1 &&
      (compareAndSwapUDATA((uintptr_t *)&barrier->released, 0, 0))) {
  ...
  /* if we're entering a barrier with a negative count then count us out but we
  ...
  } while (compareAndSwapUDATA((uintptr_t *)&barrier->out_count, old_value,
                               old_value + 1) != old_value);
...
/*
 * This function updates the expected number of clients the barrier is waiting
 * for. If all clients have already entered the barrier the the update will
 * fail.
 *
 ...
 */
static int barrier_update_r(barrier_r *barrier, int new_value) {
  ...
  if (old_value == 0 && barrier->initial_value != 0) {
    ...
    /* don't need to notify anyone if updated_value is <= 0 as release will do
     * it when called */
 ...
 * @return 0 on success, -1 on failure
 */
static int barrier_destroy_r(barrier_r *barrier, int block) {
  int rc = 0;
  char byte = 1;
...
/*
 * Constructs a semaphore for use within signal handlers. The semaphore uses a
 * combination of spinlock techniques to acquire the lock and blocking
 * read/write/poll calls to prevent massive contention between threads.
 *
 ...
 * @return 0 on success, -1 otherwise
 */
static int sem_init_r(sem_t_r *sem, int value) {
...
/*
 * This function attempts to acquire the semaphore. It will block for, at most,
 * the number of seconds specified.
 *
 * @param sem the semaphore to acquire
 * @param seconds the maximum number of seconds the block for. Indefinitely if 0
 * is specified.
 *
 * @return 0 if semaphore is acquire, < 0 on error, -1 means timeout occured
 */
static int sem_timedwait_r(sem_t_r *sem, uintptr_t seconds) {
  ...
  while (1) {
    ...
    while (old_value > 0) {
      if (compareAndSwapUDATA((uintptr_t *)&sem->sem_value, old_value,
                              old_value - 1) == old_value) {
    ...
    if (ret > 0) {
      /* the pipe is configured as non-blocking, waiting is done on the poll
       * call above*/
...
/*
 * This function attempts to acquire the semaphore. It will not block if the
 * semaphore isn't available.
 * @param sem the semaphore to acquire
 *
 * @return 0 if the semaphore was acquired, -1 otherwise.
 */
static int sem_trywait_r(sem_t_r *sem) {
  uintptr_t old_value = 0;

  /* try to get the lock */
  old_value = compareAndSwapUDATA((uintptr_t *)&sem->sem_value, -1, -1);
  while (old_value > 0) {
    int value = compareAndSwapUDATA((uintptr_t *)&sem->sem_value, old_value,
                                    old_value - 1);
    if (value == old_value) {
      /* successfully acquired lock */
      return 0;
...
/*
 * This function wakes a client or controller thread waiting on the semaphore if
 * any. Otherwise it increments the semaphore count and will allow a single
 * subsequent call to semget to succeed.
 *
 * @param sem the semaphore to post
 *
 * @return 0 on success, -1 on error
 */
static int sem_post_r(sem_t_r *sem) {
  uintptr_t old_value;
  char byte = 1;
  ...
  do {
    old_value = sem->sem_value;
  } while (compareAndSwapUDATA((uintptr_t *)&sem->sem_value, old_value,
                               old_value + 1) != old_value);
...
/*
 * This function destroys a semaphore created with sem_init_r. It first reduces
 * the semaphores capacity to 0 so that no more clients can acquire the lock. If
 * something holds the lock then we undo the block and return an error,
 * otherwise we close the file descriptors which should wake up any clients
 * waiting on the lock.
 *
 ...
 * @return 0 on success, -1 on error
 */
static int sem_destroy_r(sem_t_r *sem) {
  uintptr_t old_value;
  ...
  do {
    old_value = sem->sem_value;
                               ...
                               old_value - sem->initial_value) != old_value);
 ...
 */
static int timedOut(uintptr_t deadline) {
  struct timespec spec;
  if (clock_gettime(CLOCK_REALTIME, &spec) == -1) {
    return 0;
  }
 ...
 */
static int timeout(uintptr_t deadline) {
  int secs = 0;
...
#if defined(J9OS_I5)
void get_thread_Info(struct PlatformWalkData *data, void *context_arg,
                     unsigned long tid) {
  ...
  data->thread = state->portLibrary->heap_allocate(
...
/*
 * Signal handler used to provide preemptive up-call into native threads. We
 * first check that the signal we're handling is one that's expected to give a
 * degree of confidence that the value we're passed is a legitimate platform
 * data pointer. After that the procedure is simple:
 * 1. wait until posted
 * 2. publish thread structure with the threads data to platform data.
 * 3. generate backtraces (must be done here for linux if using 'backtrace'
 * call, other platforms could do this from another thread).
 * 4. post the coordinating thread
 * 5. wait for release
 *
 * The wait in step one is a cancellation point. If we wake from that semaphore
 * and an error is noted then we skip stack production and wait for release.
 * There's no timeout on the barrier enter as that logic should be dealt with by
 * the coordinating thread.
 *
 * @param signal the number of the signal we receive. Should be SIGRTMIN or
 * equivilent
 * @param siginfo contains data about the signal including the platform data
 * pointer provided when queued
 * @param context_arg the context of the thread when the signal was dispatched
 */
static void upcall_handler(int signal, siginfo_t *siginfo, void *context_arg) {
  thread_context *context = (thread_context *)context_arg;
  J9ThreadWalkState *state;
  ...
  if (sigaction(SUSPEND_SIG, NULL, &handler) == -1 ||
      handler.sa_sigaction != upcall_handler) {
    return;
  }
#endif
  ...
  /* ignore the signal if we are the controller thread, or if an error/timeout
  ...
  if (ret != 0) {
    /* error or timeout in sem_timedwait_r(), set shared error flag and don't do
     * the backtrace */
    data->error = ret;
  } else if (data->error) {
    /* error set by another thread while we were in sem_timedwait_r(), don't do
     * the backtrace */
  } else {
    /* construct the thread to pass back */
    data->thread = state->portLibrary->heap_allocate(
    ...
    if (data->thread == NULL) {
      ...
      data->thread->dsa =
          __dsa_prev(getdsa(), __EDCWCCWI_LOGICAL, __EDCWCCWI_DOWN, NULL,
      ...
      state->portLibrary->introspect_backtrace_symbols(
          state->portLibrary, data->thread, state->heap);
#endif /* LINUX */
    }
  }

  if (data->error) {
    /* error or timeout, exit signal handler without waiting for the controller
     * thread */
    return;
  }
...
/* These functions count the number of threads in the process. This information
 * is necessary so we can queue the correct number of signals to suspend all the
 * threads (we might be able to simply call sigqueue until we get a queue full
 * error, but then we can't queue additional signals to prompt thread
 * scheduling. Also it may not be capped on all systems). Linux kernels >= 2.6
 * we use a count of the entries in /proc/self/task (omitting . and ..) Linux
 * kernels == 2.4 we check the status file for /proc/.* entries and count those
 * where the Tgid matches our pid AIX and z/OS supply the getthrds64 and
 * getthent system calls respectively
 *
 * @param data - the platform specific data, ignored on everything but AIX
 * @return number of threads in the process
 */
#if defined(LINUX)
static int count_threads(struct PlatformWalkData *data) {
  int thread_count = 0;
  ...
  if (tids == NULL) {
    ...
    /* threads are found in hidden folders in proc, i.e. /proc/.<pid>. We can
     * tell if they belong to us by checking the thread group id from the 3rd
     * line of /proc/.<pid>/status
     */
    while ((file = readdir(proc)) != NULL) {
      /* we need a directory who's name starts with a '.' - we filter out '.'
       * and '..' */
...
static int count_threads(struct PlatformWalkData *data) {
  ...
  while (getthrds64(getpid(), &thread, sizeof(struct thrdentry64), &cursor,
                    1) == 1) {
    /* we don't want to count threads that are being disposed of or are pooled
     * for reuse. Experimentation shows that threads can receive signals before
     * they have a stack or complete construction so those are included.
     */
...
static int count_threads(struct PlatformWalkData *data) {
  ...
  getthent(&input_size, &cursor, &output_size, (void **)&output_buffer,
           &ret_val, &ret_code, &reason_code);
  if (ret_val == -1) {
...
/* This function installs a signal handler then generates signals until all
 * threads in the process bar the calling thread are suspended. The signals have
 * to be real-time signals (ie. >= SIGRTMIN) or it's not possible to pass a data
 * via sigqueue. Also, non real-time signals of the same value can be collapsed
 * and we require any threads scheduled during this suspension to immediately
 * receive a signal, so we need at least one signal per unsuspended thread on
 * the queue.
 *
 * @param data platform specific control data used during suspend/resume
 * @return the number of threads suspended
 */
static int suspend_all_preemptive(struct PlatformWalkData *data) {
#if !defined(J9OS_I5)
  ...
  if (data->oldHandler.sa_sigaction == upcall_handler) {
    /* Handler's already installed so already iterating threads. We mustn't
     * uninstall the handler while cleaning as the thread that installed the
     * initial handler will do so
     */
  ...
  do {
    ...
    for (i = data->threadCount; i < thread_count - 1; i++) {
      ...
      if (0 != ret) {
        if (errno == EAGAIN) {
          ...
          while ((0 != ret) && (EAGAIN == errno) &&
                 !timedOut(data->state->deadline1)) {
            omrthread_yield();
      ...
      /* Allow the signals time to be dispatched so we don't overflow the signal
       * queue. 10 is an arbitrary number to see if we can avoid the added
       * complexity of coordination between the signal handler and this suspend
       * logic. Testing shows 48 threads suspended on AIX5.3 at the point where
       * we get EAGAIN.
       */
      if ((i % 10) == 0) {
        omrthread_yield();
    ...
    if (data->threadCount == thread_count) {
      ...
      /* threads exited in between us counting and generating signals, so
       * swallow the surplus */
      sigset_t set;
      int sig;
  ...
  do {
    int i = 0;
    do {
      ret = pthread_getthrds_np(&thr, PTHRDSINFO_QUERY_TID, &pinfo,
    ...
    if (data->threadCount == thread_count) {
      ...
      /* threads exited in between us counting and suspend threads, so swallow
       * the surplus */
      data->threadCount = thread_count;
      break;
    } else {
      /* threads created in between us counting and suspend threads, re-counting
       * and suspend again */
    }
...
static void freeThread(J9ThreadWalkState *state, J9PlatformThread *thread) {
  ...
  struct PlatformWalkData *data =
      (struct PlatformWalkData *)state->platform_data;
  ...
  while (frame) {
    ...
    if (tmp->symbol) {
      state->portLibrary->heap_free(state->portLibrary, state->heap,
                                    tmp->symbol);
      tmp->symbol = NULL;
    }
...
/*
 * This function resumes all threads suspended by suspend_all_preemptive. The
 * general behaviour is to swallow any spurious signals from the queue so they
 * are not delivered after we remove our custom handler, the custom handler is
 * removed, then we wake any threads blocking on the relase barrier. On AIX we
 * set the signal handler to SIG_IGN before attempting to swallow signals off
 * the queue because it seems to be possible for signals to be allocated to
 * threads without yet being delivered, preventing that from working. Setting
 * SIG_IGN seems to remove not yet delivered signals.
 *
 * @param data - platform specific control data used during suspend/resume
 */
static void resume_all_preempted(struct PlatformWalkData *data) {
#if !defined(J9OS_I5)
  sigset_t set;
   ...
   * sucessfully install the signal handlers.
   */
  ...
  if (data->cleanupRequired) {
    /* On AIX it seems that a signal can be unavailable to sigpending/sigwait
     * but not yet have been delivered to a thread. As a result we can't be sure
     * that SIG_SUSPEND won't be delivered after we uninstall the handler so we
     * set it to ignore instead of default
     */
  ...
  if (data->threadsOutstanding > 0) {
    /* inhibit collection of contexts from any of the outstanding threads we
     * release */
    data->error = 1;
  }

  /* allow any outstanding threads to skip the handler. This will cause semwait
  ...
  if (data->cleanupRequired) {
    ...
    struct OMRPortLibrary *portLibrary = data->state->portLibrary;
#endif /* defined(OMR_CONFIGURABLE_SUSPEND_SIGNAL) */
    /* clear out the signal queue so that any undispatched suspend signals are
     * not left lying around */
    while (sigpending(&set) == 0 && sigismember(&set, SUSPEND_SIG)) {
      /* there is a suspend signal pending so swallow it. It is worth noting
       * that sigwait isn't in the initial set of posix signal safe functions,
       * however the fact that it bypasses the installed signal handler is too
       * useful it ignore, so we risk it. It could be replaced with sigsuspend
       * and some handler juggling if need be but the added complexity isn't
       * worth it until it's shown to be necessary
       */
      sigemptyset(&set);
      sigaddset(&set, SUSPEND_SIG);
#if defined(J9ZOS390)
      sigwait(&set);
#else
      /* the pending signal may have been dispatched to another thread since we
       * made the sigpending() call above, so use a non-blocking sigtimedwait()
       * call to clear it if it's still there, don't actually wait
       */
      time_out.tv_sec = 0;
      time_out.tv_nsec = 0;
    ...
    if ((data->oldHandler.sa_flags & SA_SIGINFO) == 0 &&
        data->oldHandler.sa_handler == SIG_DFL) {
      /* if there wasn't an old signal handler the set this to ignore. There
       * shouldn't be any suspend signals left but better safe than sorry
       */
    ...
    /* make sure they've all exited. 1 means we block until all threads that
     * have entered the barrier leave */
  ...
  /* clean up the heap */
  data->state->portLibrary->heap_free(data->state->portLibrary,
                                      data->state->heap, data);
  ...
  if (data->threadsOutstanding > 0) {
    /* inhibt collection of contexts from any of the outstanding threads we
     * release */
    data->error = 1;
  }

  while (pthread_getthrds_np(&thr, PTHRDSINFO_QUERY_TID, &pinfo, sizeof(pinfo),
                             regbuf, &val) == 0 &&
         thr != 0) {
...
/*
 * Sets up the native thread structures including the backtrace. If a context is
 * specified it is used instead of grabbing the context for the thread pointed
 * to by state->current_thread.
 *
 ...
 */
static int setup_native_thread(J9ThreadWalkState *state,
                               thread_context *sigContext, int heapAllocate) {
  struct PlatformWalkData *data =
      (struct PlatformWalkData *)state->platform_data;
  ...
  if (heapAllocate || sigContext) {
    ...
    state->current_thread =
        (J9PlatformThread *)state->portLibrary->heap_allocate(
            state->portLibrary, state->heap, sizeof(J9PlatformThread));
    ...
    state->current_thread->context =
        (thread_context *)state->portLibrary->heap_allocate(state->portLibrary,
                                                            state->heap, size);
    ...
    if (sigContext) {
      ...
      memcpy(state->current_thread->context,
             ((OMRUnixSignalInfo *)sigContext)->platformSignalInfo.context,
             size);
      ...
      memcpy(state->current_thread->context, (void *)data->thread->context,
             size);
    }
  } else {
  ...
  if (state->current_thread->callstack == NULL) {
    /* don't pass sigContext in here as we should have fixed up the thread
     * already. It confuses heap/not heap allocations if we pass it here.
     */
    ...
    if (state->current_thread->callstack == NULL &&
        data->uninitializedThreads) {
      /* if we encountered threads under construction while counting then this
       * could be legitimate */
      char *message = "no stack frames available, the thread may not have "
                      "finished initialization";
      char *symbol = (char *)state->portLibrary->heap_allocate(
          state->portLibrary, state->heap, strlen(message) + 1);
      J9PlatformStackFrame *frame =
          (J9PlatformStackFrame *)state->portLibrary->heap_allocate(
              state->portLibrary, state->heap, sizeof(J9PlatformStackFrame));
      /* we have to copy the message into heap allocated memory because we can't
       * differentiate constant strings and alloc'd strings when freeing thread
       * data.
       */
  ...
  if (state->current_thread->callstack &&
      state->current_thread->callstack->symbol == NULL) {
 ...
 * number of signals: On those platforms, the controller must not use
 * sem_timedwait_r otherwise the process may hang waiting for threads which were
 * assumed to have received the SUSPEND_SIG signal but did not.
 */
static uintptr_t sigqueue_is_reliable(void) {
#if defined(LINUX)
   ...
   */
  if (0 == uname(&sysinfo)) {
    sscanf(sysinfo.release, "%" SCNuPTR ".%" SCNuPTR, &release_major,
           &release_minor);
  }

  /* sigqueue() is sufficiently reliable on newer Linux kernels (version 3.11
   * and later). */
...
/*
 * Creates an iterator for the native threads present within the process, either
 *all native threads or a subset depending on platform (see platform specific
 *documentation for detail). This function will suspend all of the native
 *threads that will be presented through the iterator and they will remain
 *suspended until this function or nextDo return NULL. The context for the
 * calling thread is always presented first.
 *
 * @param heap used to contain the thread context, the stack frame
 *representations and symbol strings. No memory is allocated outside of the heap
 *provided. Must not be NULL.
 * @param state semi-opaque structure used as a cursor for iteration. Must not
 *be NULL User visible fields are: deadline - the system time in seconds at
 *which to abort introspection and resume error - numeric error description, 0
 *on success error_detail - detail for the specific error error_string - string
 *description of error
 * @param signal_info a signal context to use. This will be used in place of the
 *live context for the calling thread.
 *
 * @return NULL if there is a problem suspending threads, gathering thread
 *contexts or if the heap is too small to contain even the context without stack
 *trace. Errors are reported in the error, and error_detail fields of the state
 *structure. A brief textual description is in error_string.
 */
J9PlatformThread *omrintrospect_threads_startDo_with_signal(
  ...
  /* ensure that we can tell which file handles need closing if part of the
  ...
  if ((sem_init_r(&data->client_sem, 0)) != 0 ||
      (sem_init_r(&data->controller_sem, 0)) != 0) {
...
#ifdef AIXPPC
  /* On AIX, initialize semaphore pipes to be sync-on-write (O_DSYNC flag).
   * Previously we used the fdatasync() call after writing to the pipe, but on
   * AIX it is not legal to call fdatasync() inside a signal handler.
   */
  ...
  /* pass back the current thread as we can be sure we won't hit a problem
   * getting it */
...
/* This function is identical to omrintrospect_threads_startDo_with_signal but
 * omits the signal argument and instead uses a live context for the calling
 * thread.
 */
J9PlatformThread *
...
/* This function is called repeatedly to get subsequent threads in the
 * iteration. The only way to resume suspended threads is to continue calling
 * this function until it returns NULL.
 *
 * @param state state structure initialized by a call to one of the startDo
 * functions.
 *
 * @return NULL if there is an error or if no more threads are available. Sets
 * the error fields as listed detailed for
...
J9PlatformThread *omrintrospect_threads_nextDo(J9ThreadWalkState *state) {
...
#if !defined(J9OS_I5)
  /* it's possible someone has played with the signal mask, most likely
   * siglongjmp if there was a segfault or similar, so ensure it matches our
   * needs
   */
  sigemptyset(&mask);
  sigaddset(&mask, SUSPEND_SIG);
  if (sigprocmask(SIG_BLOCK, &mask, &old_mask) != 0 &&
      !sigismember(&old_mask, SUSPEND_SIG)) {
    /* can't risk it if we can't filter the suspend signal from this thread */
    RECORD_ERROR(state, SIGNAL_SETUP_ERROR, errno);
    return NULL;
  }
  ...
  if (sigqueue_is_reliable()) {
    result =
        sem_timedwait_r(&data->controller_sem, timeout(data->state->deadline1));
  } else {
    for (;;) {
  ...
  while (data->thr == pthread_self() || data->thr == 0) {
    if ((result =
             pthread_getthrds_np(&data->thr, PTHRDSINFO_QUERY_TID, &data->pinfo,
                                 sizeof(data->pinfo), regbuf, &val)) != 0) {
  ...
  if ((result =
           pthread_getthrds_np(&data->thr, PTHRDSINFO_QUERY_ALL, &data->pinfo,
                               sizeof(data->pinfo), regbuf, &val)) != 0) {
    RECORD_ERROR(state, COLLECTION_FAILURE, result);
    goto cleanup;
  }
...
int32_t omrintrospect_startup(struct OMRPortLibrary *portLibrary) {
#if defined(OMR_CONFIGURABLE_SUSPEND_SIGNAL)
