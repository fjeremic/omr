...
#if defined(J9OS_I5)
void get_thread_Info(struct PlatformWalkData* data,
                     void* context_arg,
                     unsigned long tid) {
...
static int count_threads(struct PlatformWalkData* data) {
  ...
  getthent(&input_size, &cursor, &output_size, (void**)&output_buffer, &ret_val,
           &ret_code, &reason_code);
  if (ret_val == -1) {
