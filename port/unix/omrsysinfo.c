...
/* a2e overrides nl_langinfo to return ASCII strings. We need the native EBCDIC
 * string */
...
static uintptr_t copyEnvToBuffer(struct OMRPortLibrary *portLibrary,
                                 void *args);
static uintptr_t copyEnvToBufferSignalHandler(struct OMRPortLibrary *portLib,
                                              uint32_t gpType, void *gpInfo,
                                              void *unUsed);

static void setPortableError(OMRPortLibrary *portLibrary, const char *funcName,
                             int32_t portlibErrno, int systemErrno);
...
#define MAX_DEFAULT_VALUE_CHECK                                                \
  (LLONG_MAX -                                                                 \
   (1024 * 1024 * 1024)) /* subtracting the MAX page size (1GB) from LLONG_MAX \
                            to check against a value */
...
struct {
  const char *name;
  uint64_t flag;
} supportedSubsystems[] = {{"cpu", OMR_CGROUP_SUBSYSTEM_CPU},
...
static struct OMRCgroupSubsystemMetricMap omrCgroupMemoryMetricMap[] = {
    {"memory.limit_in_bytes",
     &(OMRCgroupMetricInfoElement){"Memory Limit", NULL, "bytes", TRUE},
     SINGLE_CGROUP_METRIC},
    {"memory.memsw.limit_in_bytes",
     &(OMRCgroupMetricInfoElement){"Memory + Swap Limit", NULL, "bytes", TRUE},
     SINGLE_CGROUP_METRIC},
    {"memory.usage_in_bytes",
     &(OMRCgroupMetricInfoElement){"Memory Usage", NULL, "bytes", FALSE},
     SINGLE_CGROUP_METRIC},
    {"memory.memsw.usage_in_bytes",
     &(OMRCgroupMetricInfoElement){"Memory + Swap Usage", NULL, "bytes", FALSE},
     SINGLE_CGROUP_METRIC},
    {"memory.max_usage_in_bytes",
     &(OMRCgroupMetricInfoElement){"Memory Max Usage", NULL, "bytes", FALSE},
     SINGLE_CGROUP_METRIC},
    {"memory.memsw.max_usage_in_bytes",
     &(OMRCgroupMetricInfoElement){"Memory + Swap Max Usage", NULL, "bytes",
                                   FALSE},
     SINGLE_CGROUP_METRIC},
    {"memory.failcnt",
     &(OMRCgroupMetricInfoElement){"Memory limit exceeded count", NULL, NULL,
                                   FALSE},
     SINGLE_CGROUP_METRIC},
    {"memory.memsw.failcnt",
     &(OMRCgroupMetricInfoElement){"Memory + Swap limit exceeded count", NULL,
                                   NULL, FALSE},
     SINGLE_CGROUP_METRIC},
    {"memory.oom_control", &oomControlMetricElementList[0],
     sizeof(oomControlMetricElementList) /
         sizeof(oomControlMetricElementList[0])}};

static struct OMRCgroupSubsystemMetricMap omrCgroupCpuMetricMap[] = {
    {"cpu.cfs_period_us",
     &(OMRCgroupMetricInfoElement){"CPU Period", NULL, "microseconds", FALSE},
     SINGLE_CGROUP_METRIC},
    {"cpu.cfs_quota_us",
     &(OMRCgroupMetricInfoElement){"CPU Quota", NULL, "microseconds", TRUE},
     SINGLE_CGROUP_METRIC},
    {"cpu.shares",
     &(OMRCgroupMetricInfoElement){"CPU Shares", NULL, NULL, FALSE},
     SINGLE_CGROUP_METRIC},
    {"cpu.stat", &cpuStatMetricElementList[0],
     sizeof(cpuStatMetricElementList) / sizeof(cpuStatMetricElementList[0])}};

static struct OMRCgroupSubsystemMetricMap omrCgroupCpusetMetricMap[] = {
    {"cpuset.cpu_exclusive",
     &(OMRCgroupMetricInfoElement){"CPU exclusive", NULL, NULL, FALSE},
     SINGLE_CGROUP_METRIC},
    {"cpuset.mem_exclusive",
     &(OMRCgroupMetricInfoElement){"Mem exclusive", NULL, NULL, FALSE},
     SINGLE_CGROUP_METRIC},
    {"cpuset.cpus", &(OMRCgroupMetricInfoElement){"CPUs", NULL, NULL, FALSE},
     SINGLE_CGROUP_METRIC},
    {"cpuset.mems", &(OMRCgroupMetricInfoElement){"Mems", NULL, NULL, FALSE},
     SINGLE_CGROUP_METRIC}};
...
static uint32_t getLimitSharedMemory(struct OMRPortLibrary *portLibrary,
                                     uint64_t *limit);
#if defined(LINUX) || defined(AIXPPC) || defined(J9ZOS390)
static intptr_t readSymbolicLink(struct OMRPortLibrary *portLibrary,
                                 char *linkFilename, char **result);
#endif /* defined(LINUX) || defined(AIXPPC) || defined(J9ZOS390) */
#if defined(AIXPPC) || defined(J9ZOS390)
static BOOLEAN isSymbolicLink(struct OMRPortLibrary *portLibrary,
                              char *filename);
static intptr_t searchSystemPath(struct OMRPortLibrary *portLibrary,
                                 char *filename, char **result);
#endif /* defined(AIXPPC) || defined(J9ZOS390) */

#if defined(J9ZOS390)
static void setOSFeature(struct OMROSDesc *desc, uint32_t feature);
static intptr_t getZOSDescription(struct OMRPortLibrary *portLibrary,
...
#elif defined(OMRZTPF) /* !defined(RS6000) && !defined(J9ZOS390) && \
                          !defined(OSX)  && !defined(OMRZTPF) */
...
static void freeCgroupEntries(struct OMRPortLibrary *portLibrary,
                              OMRCgroupEntry *cgEntryList);
static char *getCgroupNameForSubsystem(struct OMRPortLibrary *portLibrary,
                                       OMRCgroupEntry *cgEntryList,
                                       const char *subsystem);
static int32_t addCgroupEntry(struct OMRPortLibrary *portLibrary,
                              OMRCgroupEntry **cgEntryList, int32_t hierId,
                              const char *subsystem, const char *cgroupName,
                              uint64_t flag);
static int32_t readCgroupFile(struct OMRPortLibrary *portLibrary, int pid,
                              BOOLEAN inContainer,
                              OMRCgroupEntry **cgroupEntryList,
                              uint64_t *availableSubsystems);
static OMRCgroupSubsystem getCgroupSubsystemFromFlag(uint64_t subsystemFlag);
static int32_t getAbsolutePathOfCgroupSubsystemFile(
    struct OMRPortLibrary *portLibrary, uint64_t subsystemFlag,
    const char *fileName, char *fullPath, intptr_t *bufferLength);
static int32_t getHandleOfCgroupSubsystemFile(
    struct OMRPortLibrary *portLibrary, uint64_t subsystemFlag,
    const char *fileName, FILE **subsystemFile);
static int32_t readCgroupMetricFromFile(struct OMRPortLibrary *portLibrary,
                                        uint64_t subsystemFlag,
                                        const char *fileName,
                                        const char *metricKeyInFile,
                                        char **fileContent, char *value);
static int32_t readCgroupSubsystemFile(struct OMRPortLibrary *portLibrary,
                                       uint64_t subsystemFlag,
                                       const char *fileName,
                                       int32_t numItemsToRead,
                                       const char *format, ...);
static int32_t isRunningInContainer(struct OMRPortLibrary *portLibrary,
                                    BOOLEAN *inContainer);
static int32_t getCgroupMemoryLimit(struct OMRPortLibrary *portLibrary,
                                    uint64_t *limit);
#endif /* defined(LINUX) */

#if defined(LINUX)
static int32_t retrieveLinuxMemoryStatsFromProcFS(
    struct OMRPortLibrary *portLibrary, struct J9MemoryInfo *memInfo);
static int32_t retrieveLinuxCgroupMemoryStats(
    struct OMRPortLibrary *portLibrary,
    struct OMRCgroupMemoryInfo *cgroupMemInfo);
static int32_t retrieveLinuxMemoryStats(struct OMRPortLibrary *portLibrary,
                                        struct J9MemoryInfo *memInfo);
#elif defined(OSX)
static int32_t retrieveOSXMemoryStats(struct OMRPortLibrary *portLibrary,
                                      struct J9MemoryInfo *memInfo);
#elif defined(AIXPPC)
static int32_t retrieveAIXMemoryStats(struct OMRPortLibrary *portLibrary,
                                      struct J9MemoryInfo *memInfo);
#endif
...
void omrsysinfo_set_number_user_specified_CPUs(
...
intptr_t omrsysinfo_process_exists(struct OMRPortLibrary *portLibrary,
                                   uintptr_t pid) {
  intptr_t rc = 0;
  int result = 0;
   ...
   * By using signal == 0, kill just checks for the existence and accessibility
   * of the process.
   */

  if (0 == result) {
    rc = 1; /* The process with pid exists and this process has sufficient
               permissions to send it a signal. */
  } else if (-1 == result) { /* note that kill() returns only 0 or -1 */
    if (errno == ESRCH) {
      /* The kill() failed because was there are no processes or process groups
       * corresponding to pid. */
      rc = 0;
    } else if (errno == EPERM) {
      /* The target process exists but this process does not have permission to
       * send it a signal. */
      rc = 1;
    } else {
      rc = -1;
...
const char *omrsysinfo_get_CPU_architecture(
    struct OMRPortLibrary *portLibrary) {
...
intptr_t omrsysinfo_get_env(struct OMRPortLibrary *portLibrary,
...
const char *omrsysinfo_get_OS_type(struct OMRPortLibrary *portLibrary) {
#if defined(J9OS_I5)
...
const char *omrsysinfo_get_OS_version(struct OMRPortLibrary *portLibrary) {
  if (NULL == PPG_si_osVersion) {
    ...
    rc = __osname(&sysinfo);
#elif defined(OSX)
    /* uname() on MacOS returns the version of the Darwin kernel, not the
     * operating system product name. For compatibility with the reference
     * implementation, we need the operating system product name. On newer
     * versions of MacOS, the OS name is provided by sysctlbyname(). On older
     * versions of MacOS, the OS name is provided by a property list file. The
     * relevant portion is: <key>ProductVersion</key> <string>10.13.6</string>
     *
     */
    size_t resultSize = 0;
    ...
    if (rc < 0) { /* older version of MacOS.  Fall back to parsing the plist. */
      intptr_t plistFile =
          portLibrary->file_open(portLibrary, PLIST_FILE, EsOpenRead, 0444);
      if (plistFile >= 0) {
        ...
        if (len >= sizeof(myBuffer)) { /* leave space for a terminating null */
          allocatedFileBuffer = portLibrary->mem_allocate_memory(
              portLibrary, len + 1, OMR_GET_CALLSITE(),
        ...
        if (NULL != buffer) {
          ...
          while (bytesRemaining > 0) {
            intptr_t result = portLibrary->file_read(portLibrary, plistFile,
                                                     readPtr, bytesRemaining);
            if (result > 0) {
          ...
          if (0 == rc) {
            ...
            if (NULL != readPtr) {
              readPtr = strstr(readPtr + sizeof(PRODUCT_VERSION_ELEMENT) - 1,
                               STRING_OPEN_TAG);
            }
            if (NULL != readPtr) {
    ...
    if (rc >= 0) {
      ...
      len =
          strlen(sysinfo.version) + strlen(sysinfo.release) + RELEASE_OVERHEAD;
      buffer = portLibrary->mem_allocate_memory(
          portLibrary, len, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
      if (NULL != buffer) {
        portLibrary->str_printf(portLibrary, buffer, len, FORMAT_STRING,
                                sysinfo.version, sysinfo.release);
      }
#elif defined(OSX)
      len = resultSize + 1;
      buffer = portLibrary->mem_allocate_memory(
      ...
      if (NULL != buffer) {
        if (NULL != versionString) {
          ...
          rc =
              sysctlbyname(KERN_OSPRODUCTVERSION, buffer, &resultSize, NULL, 0);
        }
        buffer[resultSize] = '\0';
      }
...
uintptr_t omrsysinfo_get_pid(struct OMRPortLibrary *portLibrary) {
  return getpid();
}

uintptr_t omrsysinfo_get_ppid(struct OMRPortLibrary *portLibrary) {
  return getppid();
}

uintptr_t omrsysinfo_get_euid(struct OMRPortLibrary *portLibrary) {
  uintptr_t uid = geteuid();
  Trc_PRT_sysinfo_get_egid(uid);
  return uid;
}

uintptr_t omrsysinfo_get_egid(struct OMRPortLibrary *portLibrary) {
  uintptr_t gid = getegid();
  Trc_PRT_sysinfo_get_egid(gid);
  return gid;
}

intptr_t omrsysinfo_get_groups(struct OMRPortLibrary *portLibrary,
  ...
  *gidList = NULL;
  ...
  if (-1 == size) {
    /* Above call getgroups(0, ...) is not expected to fail. In case it does,
     * return appropriate error code */
    myerror = errno;
    Trc_PRT_sysinfo_get_groups_Error_GetGroupsSize(myerror);
    setPortableError(portLibrary, getgroupsErrorMsgPrefix,
                     OMRPORT_ERROR_SYSINFO_GETGROUPSSIZE_ERROR, myerror);
  } else {
    list = portLibrary->mem_allocate_memory(
        portLibrary, size * sizeof(uint32_t), OMR_GET_CALLSITE(), categoryCode);
    if (NULL == list) {
      ...
      if (-1 == size) {
        ...
        setPortableError(portLibrary, getgroupsErrorMsgPrefix,
                         OMRPORT_ERROR_SYSINFO_GETGROUPS_ERROR, myerror);
      } else {
...
/**
 * @internal Helper routine that determines the full path of the current
 * executable that launched the JVM instance.
 */
static intptr_t find_executable_name(struct OMRPortLibrary *portLibrary,
                                     char **result) {
#if defined(LINUX)
  intptr_t retval = readSymbolicLink(portLibrary, "/proc/self/exe", result);
  if (NULL != *result) {
    /* The code in this block is a work around for a linux bug. If
     * /proc/self/exe is read after a call to fstat on a NFSv4 share, then the
     * string ' (deleted)' will be appended to the file name.
     *
     * The work around to this problem for now is to prune off the ' (deleted)'
     * string.
     *
     * Until this fix numerous 'pltest.exe' failures were occurring on 'vmfarm'.
     * I expect over time once kernels are patch this code can be removed.
     */
    ...
    if (exeNameLen > deletedStrLen) {
      if (strncmp(&((*result)[exeNameLen - deletedStrLen]), deletedStr,
                  deletedStrLen) == 0) {
  ...
  *result = portLibrary->mem_allocate_memory(
      portLibrary, size, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  buf.ps_pathptr = portLibrary->mem_allocate_memory(
      portLibrary, buf.ps_pathlen = PS_PATHBLEN, OMR_GET_CALLSITE(),
      OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  /* Return val of w_getpsent == -1 indicates error, == 0 indicates no more
   * processes */
  if (token <= 0) {
    retval = -1;
    goto cleanup;
  }
  currentPath = (portLibrary->mem_allocate_memory)(
      portLibrary, strlen(e2aName) + 1, OMR_GET_CALLSITE(),
      OMRMEM_CATEGORY_PORT_LIBRARY);
  if (currentPath) {
...
#if defined(AIXPPC)
  /* On AIX, "/proc" provides a way of determining argument vector (not just
   * using argv[0] from within main()).
   */
  ...
  portLibrary->str_printf(portLibrary, buffer, PATH_MAX, "/proc/%ld/psinfo",
                          getpid());
  ...
  /* Read the process info; loop in order to ensure the complete structure is
   * read in. */
  do {
    readBytes = portLibrary->file_read(
        portLibrary, fd, ((char *)&psinfoData) + readOffset, remainingBytes);
    /* Advance readOffset by bytes read and adjust this out from remainingBytes
     * (to be read). */
    remainingBytes -= readBytes;
    readOffset += readBytes;

    /* Check for error and EOF conditions.  Should this arise before reading
     * ample bytes, file read has supposedly failed.
     */
    if (-1 == readBytes) {
      ...
      /* EOF reached but haven't read required number bytes (file simply too
       * short)!  No error number is expected to be set here, so use
       * OMRPORT_ERROR_FILE_IO to indicate I/O error.
       */
      Trc_PRT_find_executable_name_invalidRead(OMRPORT_ERROR_FILE_IO);
      portLibrary->error_set_last_error_with_message(
          portLibrary, OMRPORT_ERROR_FILE_IO,
          "Input/output error: /proc file truncated");
      retval = -1;
      portLibrary->file_close(portLibrary, fd);
      goto cleanup;
    }
  } while (0 != remainingBytes); /* Read until there are no bytes remaining to
                                    fill the structure. */
  ...
  currentPath = (portLibrary->mem_allocate_memory)(
      portLibrary, strLen, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL != currentPath) {
...
gotPathName:
  ...
  if (p) {
    *p++ = '\0';
    currentName = (portLibrary->mem_allocate_memory)(
        portLibrary, strlen(p) + 1, OMR_GET_CALLSITE(),
        OMRMEM_CATEGORY_PORT_LIBRARY);
    if (!currentName) {
      retval = -1;
      goto cleanup;
  ...
  *result = (portLibrary->mem_allocate_memory)(
      portLibrary, strlen(currentPath) + strlen(currentName) + 2,
 ...
 *
 * @note Caller should /not/ de-allocate memory in the result buffer, as string
 * containing the executable name is system-owned (managed internally by the
 * port library).
 */
intptr_t omrsysinfo_get_executable_name(struct OMRPortLibrary *portLibrary,
 ...
 *
 * @note The buffer to hold this string (including its terminating NUL) is
 * allocated with portLibrary->mem_allocate_memory.  The caller should free this
 * memory with portLibrary->mem_free_memory when it is no longer needed.
 */
static intptr_t cwdname(struct OMRPortLibrary *portLibrary, char **result) {
  char *cwd;
  int allocSize = 256;
...
doAlloc:
  cwd = (portLibrary->mem_allocate_memory)(
      portLibrary, allocSize, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
  if (!cwd) {
    return -1;
  }
  if (!getcwd(cwd, allocSize - 1)) {
    int32_t error =
        errno; /* Save the error code before re-trying with bigger buffer. */
...
#if defined(AIXPPC) || defined(J9ZOS390)
/**
 * @internal  Examines the named file to determine if it is a symbolic link.  On
 * platforms which don't have symbolic links (or where we can't tell) or if an
 * unexpected error occurs, just answer FALSE.
 */
static BOOLEAN isSymbolicLink(struct OMRPortLibrary *portLibrary,
                              char *filename) {
  struct stat statbuf;
...
/**
 * @internal  Attempts to read the contents of a symbolic link.  (The contents
 * are the relative pathname of the thing linked to).  A buffer large enough to
 * hold the result (and the terminating NUL) is allocated with
 * portLibrary->mem_allocate_memory.  The caller should free this buffer with
...
#if defined(AIXPPC) || defined(J9ZOS390)
/**
 * @internal  Searches through the system PATH for the named file.  If found, it
 * returns the path entry which matched the file.  A buffer large enough to hold
 * the proper path entry (without a trailing slash, but with the terminating
 * NUL) is allocated with portLibrary->mem_allocate_memory. The caller should
 * free this buffer with portLibrary->mem_free_memory when it is no longer
 * needed.  On success, returns 0.  On error (including if the file is not
 * found), -1 is returned.
 */
static intptr_t searchSystemPath(struct OMRPortLibrary *portLibrary,
                                 char *filename, char **result) {
  ...
  /* This should be sufficient for a single entry in the PATH var, though the
   * var itself */
  /* could be considerably longer.. */
  char temp[PATH_MAX + 1];

  /* We use getenv() instead of the portLibrary function because getenv()
   * doesn't require */
    ...
    if ((sdir = opendir(temp))) {
      while ((dirEntry = readdir(sdir))) {
        ...
        if (!strcmp(dirEntry->d_name, filename)) {
          ...
          /* found! */
          *result = (portLibrary->mem_allocate_memory)(
              portLibrary, strlen(temp) + 1, OMR_GET_CALLSITE(),
              OMRMEM_CATEGORY_PORT_LIBRARY);
          if (!result) {
            return -1;
          }
...
uintptr_t omrsysinfo_get_number_CPUs_by_type(struct OMRPortLibrary *portLibrary,
                                             uintptr_t type) {
  ...
  switch (type) {
    case OMRPORT_CPU_PHYSICAL:
      ...
      if (0 == toReturn) {
        Trc_PRT_sysinfo_get_number_CPUs_by_type_failedPhysical("errno: ",
                                                               errno);
      }
#elif defined(J9ZOS390)
      /* Temporarily using ONLINE for PHYSICAL on z/OS */
      toReturn = portLibrary->sysinfo_get_number_CPUs_by_type(
          portLibrary, OMRPORT_CPU_ONLINE);

      if (0 == toReturn) {
        Trc_PRT_sysinfo_get_number_CPUs_by_type_failedPhysical("(no errno) ",
                                                               0);
      }
    ...
    case OMRPORT_CPU_ONLINE:
      ...
      toReturn = portLibrary->sysinfo_get_number_CPUs_by_type(
          portLibrary, OMRPORT_CPU_PHYSICAL);
    ...
    case OMRPORT_CPU_BOUND: {
      ...
      /* If system calls failed to retrieve info, do not check cgroup at all,
       * and give an error */
      if (0 == toReturn) {
        Trc_PRT_sysinfo_get_number_CPUs_by_type_failedBound("errno: ", errno);
      } else if (portLibrary->sysinfo_cgroup_are_subsystems_enabled(
                     portLibrary, OMR_CGROUP_SUBSYSTEM_CPU)) {
        int32_t rc = 0;
        int64_t cpuQuota = 0;
        uint64_t cpuPeriod = 0;
        int32_t numItemsToRead =
            1; /* cpu.cfs_quota_us and cpu.cfs_period_us files each contain only
                  one integer value */

        /* If either file read fails, ignore cgroup cpu quota limits and
         * continue */
        rc = readCgroupSubsystemFile(portLibrary, OMR_CGROUP_SUBSYSTEM_CPU,
                                     "cpu.cfs_quota_us", numItemsToRead, "%ld",
                                     &cpuQuota);
        if (0 == rc) {
          rc = readCgroupSubsystemFile(portLibrary, OMR_CGROUP_SUBSYSTEM_CPU,
                                       "cpu.cfs_period_us", numItemsToRead,
                                       "%lu", &cpuPeriod);
          if (0 == rc) {
            int32_t numCpusQuota =
                (int32_t)(((double)cpuQuota / cpuPeriod) + 0.5);

            if ((cpuQuota > 0) && (numCpusQuota < toReturn)) {
              toReturn = numCpusQuota;
              /* If the CPU quota rounds down to 0, then just return 1 as the
               * closest usable value */
              if (0 == toReturn) {
                toReturn = 1;
              }
            }
       ...
       * supported. Thus, this just returns number of CPUs.
       */
      toReturn = sysconf(_SC_NPROCESSORS_CONF);
      if (0 == toReturn) {
        Trc_PRT_sysinfo_get_number_CPUs_by_type_failedBound("errno: ", errno);
    ...
    case OMRPORT_CPU_TARGET: {
      ...
      if (0 < specified) {
        ...
        toReturn = _system_configuration.ncpus;
        if (0 == toReturn) {
          Trc_PRT_sysinfo_get_number_CPUs_by_type_failedOnline("(no errno) ",
                                                               0);
        }
#elif defined(OMRZTPF)
        toReturn = get_Dispatch_IstreamCount();
...
/**
 * Function collects memory usage statistics by reading /proc/meminfo on Linux
 * platforms.
 *
 * @param[in] portLibrary The port library.
 * @param[in] memInfo A pointer to the J9MemoryInfo struct which we populate
 * with memory usage.
 *
...
static int32_t retrieveLinuxMemoryStatsFromProcFS(
  ...
  while (0 == feof(memStatFs)) {
    ...
    /* Get the various fields that we are interested in and extract the
     * corresponding value. */
    if (0 == strncmp(tmpPtr, MEMTOTAL_PREFIX, MEMTOTAL_PREFIX_SZ)) {
      ...
      if ((LONG_MIN == memInfo->totalPhysical) ||
          (LONG_MAX == memInfo->totalPhysical)) {
      ...
      if ((LONG_MIN == memInfo->availPhysical) ||
          (LONG_MAX == memInfo->availPhysical)) {
      ...
      if ((LONG_MIN == memInfo->totalSwap) ||
          (LONG_MAX == memInfo->totalSwap)) {
      ...
      if ((LONG_MIN == memInfo->availSwap) ||
          (LONG_MAX == memInfo->availSwap)) {
...
#if !defined(OMRZTPF)
/**
 * Function collects memory usage statistics from the memory subsystem of the
 * process's cgroup.
 *
 * @param[in] portLibrary The port library.
 * @param[in] cgroupMemInfo A pointer to the OMRCgroupMemoryInfo struct which
 * will be populated with memory usage.
 *
 * @return 0 on success and negative error code on failure.
 */
static int32_t retrieveLinuxCgroupMemoryStats(
  ...
  rc =
      readCgroupSubsystemFile(portLibrary, OMR_CGROUP_SUBSYSTEM_MEMORY,
                              CGROUP_MEMORY_LIMIT_IN_BYTES_FILE, numItemsToRead,
                              "%lu", &cgroupMemInfo->memoryLimit);
  if (0 != rc) {
    goto _exit;
  }
  rc =
      readCgroupSubsystemFile(portLibrary, OMR_CGROUP_SUBSYSTEM_MEMORY,
                              CGROUP_MEMORY_USAGE_IN_BYTES_FILE, numItemsToRead,
                              "%lu", &cgroupMemInfo->memoryUsage);
  if (0 != rc) {
    goto _exit;
  }
  rc = readCgroupSubsystemFile(portLibrary, OMR_CGROUP_SUBSYSTEM_MEMORY,
                               CGROUP_MEMORY_SWAP_LIMIT_IN_BYTES_FILE,
                               numItemsToRead, "%lu",
                               &cgroupMemInfo->memoryAndSwapLimit);
  if (0 != rc) {
    if (OMRPORT_ERROR_FILE_NOENT == rc) {
      /* It is possible file memory.memsw.limit_in_bytes is not present if
       * swap space is not configured. In such cases, set memoryAndSwapLimit to
       * same as memoryLimit.
       */
  ...
  rc = readCgroupSubsystemFile(portLibrary, OMR_CGROUP_SUBSYSTEM_MEMORY,
                               CGROUP_MEMORY_SWAP_USAGE_IN_BYTES_FILE,
                               numItemsToRead, "%lu",
                               &cgroupMemInfo->memoryAndSwapUsage);
  if (0 != rc) {
    if (OMRPORT_ERROR_FILE_NOENT == rc) {
      /* It is possible file memory.memsw.usage_in_bytes is not present if
       * swap space is not configured. In such cases, set memoryAndSwapUsage to
       * memoryUsage.
       */
  ...
  rc = getHandleOfCgroupSubsystemFile(portLibrary, OMR_CGROUP_SUBSYSTEM_MEMORY,
                                      CGROUP_MEMORY_STAT_FILE, &memStatFs);
  if (0 != rc) {
    goto _exit;
  }
  ...
  while (0 == feof(memStatFs)) {
    ...
    /* Extract "cache" value */
    if (0 == strncmp(tmpPtr, CGROUP_MEMORY_STAT_CACHE,
                     CGROUP_MEMORY_STAT_CACHE_SZ)) {
      tmpPtr += CGROUP_MEMORY_STAT_CACHE_SZ;
      rc = sscanf(tmpPtr, "%" SCNu64, &cgroupMemInfo->cached);
      if (1 != rc) {
        Trc_PRT_retrieveLinuxCgroupMemoryStats_invalidValue(
            CGROUP_MEMORY_STAT_CACHE, CGROUP_MEMORY_STAT_FILE);
        rc = portLibrary->error_set_last_error_with_message_format(
            portLibrary,
            OMRPORT_ERROR_SYSINFO_CGROUP_SUBSYSTEM_FILE_INVALID_VALUE,
            "invalid value for field %s in file %s", CGROUP_MEMORY_STAT_CACHE,
...
/**
 * Function collects memory usage statistics on Linux platforms and returns the
 * same. This function takes into account cgroup limits if available.
 *
 * @param[in] portLibrary The port library.
 * @param[in] memInfo A pointer to the J9MemoryInfo struct which we populate
 * with memory usage.
...
static int32_t retrieveLinuxMemoryStats(struct OMRPortLibrary *portLibrary,
  ...
  rc = portLibrary->sysinfo_get_limit(
      portLibrary, OMRPORT_RESOURCE_ADDRESS_SPACE | OMRPORT_LIMIT_HARD,
      &maxVirtual);
   ...
   */
  if (!portLibrary->sysinfo_cgroup_are_subsystems_enabled(
  ...
  if (0 != rc) {
    ...
    /* If we failed to retrieve memory stats for cgroup, just use values for the
     * host; don't consider it as failure */
    rc = 0;
    goto _exit;
  }
  ...
  if (cgroupMemInfo.memoryUsage <= cgroupMemInfo.memoryLimit) {
    memInfo->availPhysical =
        cgroupMemInfo.memoryLimit - cgroupMemInfo.memoryUsage;
...
static int32_t retrieveOSXMemoryStats(struct OMRPortLibrary *portLibrary,
  ...
  if (0 == ret) {
    ...
    if (KERN_SUCCESS == task_info(mach_task_self(), TASK_BASIC_INFO,
                                  (task_info_t)&taskInfo, &t_info_count)) {
  ...
  /* The amount of RAM used for cache memory and file buffers is not available
   * on OSX. */
...
static int32_t retrieveAIXMemoryStats(struct OMRPortLibrary *portLibrary,
  ...
  rc = perfstat_memory_total(NULL, &aixMemoryInfo,
                             sizeof(perfstat_memory_total_t), 1);
  if (1 != rc) {
    Trc_PRT_retrieveAIXMemoryStats_perfstatFailed(errno);
    Trc_PRT_retrieveAIXMemoryStats_Exit(
        OMRPORT_ERROR_SYSINFO_ERROR_READING_MEMORY_INFO);
  ...
  /* AIX does not define buffers, so: memInfo->buffered =
...
int32_t omrsysinfo_get_memory_info(struct OMRPortLibrary *portLibrary,
                                   struct J9MemoryInfo *memInfo, ...) {
  int32_t rc = -1;
  ...
  if (NULL == memInfo) {
    Trc_PRT_sysinfo_get_memory_info_Exit(
        OMRPORT_ERROR_SYSINFO_NULL_OBJECT_RECEIVED);
  ...
  memInfo->timestamp =
      (portLibrary->time_nano_time(portLibrary) / NANOSECS_PER_USEC);

  Trc_PRT_sysinfo_get_memory_info_Exit(rc);
  return rc;
}

uint64_t omrsysinfo_get_addressable_physical_memory(
    struct OMRPortLibrary *portLibrary) {
  ...
  if (OMRPORT_LIMIT_LIMITED ==
      portLibrary->sysinfo_get_limit(
          portLibrary, OMRPORT_RESOURCE_ADDRESS_SPACE, &memoryLimit)) {
    /* there is a limit on the memory we can use so take the minimum of this
     * usable amount and the physical memory */
...
uint64_t omrsysinfo_get_physical_memory(struct OMRPortLibrary *portLibrary) {
  uint64_t result = 0;
  ...
  /* on systems with 43K headers. However, this is not an issue as we only
   * support AIX 5.2 and above only */
...
static uint64_t getPhysicalMemory(void) {
  struct ebmaxc_parmlist newValues[] = {{MAX64HEAP, MAXVALUE}, {0, 0}};

  tpf_ebmaxc(newValues);
  uint64_t physMemory =
      (uint64_t)(*(uint16_t *)(cinfc_fast(CINFC_CMMMMES) + 8));
...
int32_t omrsysinfo_startup(struct OMRPortLibrary *portLibrary) {
  PPG_isRunningInContainer = FALSE;
  /* Obtain and cache executable name; if this fails, executable name remains
   * NULL, but shouldn't cause failure to startup port library.  Failure will be
   * noticed only when the omrsysinfo_get_executable_name() actually gets
   * invoked.
   */
   ...
   * as done by some fvtests (eg fvtest/porttest/j9portTest.cpp) that create
   * fake portlibrary to test its management and lifecycle, we need to ensure
   * globals like cgroupEntryListMonitor are initialized and destroyed only
   * once.
   */
  if (0 == attachedPortLibraries) {
    if (omrthread_monitor_init_with_name(&cgroupEntryListMonitor, 0,
                                         "cgroup entry list monitor")) {
      return -1;
    }
  }
  attachedPortLibraries += 1;
...
intptr_t omrsysinfo_get_username(struct OMRPortLibrary *portLibrary,
...
#define USERID_BUF_LEN 16
  ...
  /* _IBM_ATOE_H_ is used instead of J9ZOS390 is to ensure that this code
   * becomes dead code path if we ever decide to retire atoe library. This macro
   * is defined in atoe.h, which is only included by macro J9ZOS390. Hence,
   * there are no danger of undefined variable. j9 atoe library used malloc.
   * Need to free any structure obtained from getpwnam and getpwuid.
   */
...
intptr_t omrsysinfo_get_groupname(struct OMRPortLibrary *portLibrary,
                                  char *buffer, uintptr_t length) {
  char *remoteCopy = NULL;
...
intptr_t omrsysinfo_get_hostname(struct OMRPortLibrary *portLibrary,
...
uint32_t omrsysinfo_get_limit(struct OMRPortLibrary *portLibrary,
                              uint32_t resourceID, uint64_t *limit) {
  uint32_t hardLimitRequested =
      OMRPORT_LIMIT_HARD == (resourceID & OMRPORT_LIMIT_HARD);
...
#if defined(AIXPPC) || (defined(LINUX) && !defined(OMRZTPF)) || \
    defined(OSX) || defined(J9ZOS390)
...
#else  /* defined(AIXPPC) || (defined(LINUX) && !defined(OMRZTPF)) || \
          defined(OSX) || defined(J9ZOS390) */
      /* unsupported on other platforms (just in case). */
      *limit = OMRPORT_LIMIT_UNKNOWN_VALUE;
      rc = OMRPORT_LIMIT_UNKNOWN;
#endif /* defined(AIXPPC) || (defined(LINUX) && !defined(OMRZTPF)) || \
          defined(OSX) || defined(J9ZOS390) */
    } break;
    default:
...
uint32_t omrsysinfo_set_limit(struct OMRPortLibrary *portLibrary,
                              uint32_t resourceID, uint64_t limit) {
  uint32_t hardLimitRequested =
      OMRPORT_LIMIT_HARD == (resourceID & OMRPORT_LIMIT_HARD);
  uint32_t resourceRequested = resourceID & ~OMRPORT_LIMIT_HARD;
        ...
        if (hardLimitRequested) {
          ...
          if ((OMRPORT_RESOURCE_FILE_DESCRIPTORS == resourceRequested) &&
            ...
            if (-1 == rc) {
              portLibrary->error_set_last_error(portLibrary, errno,
                                                findError(errno));
              Trc_PRT_sysinfo_setrlimit_error(resource, limit,
                                              findError(errno));
            } else {
              limit = maxFiles;
            }
...
static uint32_t getLimitSharedMemory(struct OMRPortLibrary *portLibrary,
  ...
  fd = portLibrary->file_open(portLibrary, "/proc/sys/kernel/shmmax",
                              EsOpenRead, 0);
  if (-1 == fd) {
    portableError = portLibrary->error_last_error_number(portLibrary);
    Trc_PRT_sysinfo_getLimitSharedMemory_invalidFileHandle(portableError);
    portLibrary->error_set_last_error_with_message(
        portLibrary, portableError,
        "getLimitSharedMemory invalid return from file open");
    goto errorReturn;
  }

  bytesRead =
      portLibrary->file_read(portLibrary, fd, (char *)buf, sizeof(buf) - 1);
  portableError = portLibrary->error_last_error_number(portLibrary);
...
errorReturn:
  Trc_PRT_sysinfo_getLimitSharedMemory_ErrorExit(OMRPORT_LIMIT_UNKNOWN,
                                                 OMRPORT_LIMIT_UNKNOWN_VALUE);
  *limit = OMRPORT_LIMIT_UNKNOWN_VALUE;
...
intptr_t omrsysinfo_get_load_average(
    struct OMRPortLibrary *portLibrary,
  ...
  int returnValue =
      perfstat_cpu_total(NULL, &perfstats, sizeof(perfstat_cpu_total_t), 1);
  if ((returnValue != EINVAL) && (returnValue != EFAULT) &&
      (returnValue != ENOMEM)) {
    loadAverageData->oneMinuteAverage =
        ((double)perfstats.loadavg[0]) / (1 << SBITS);
    loadAverageData->fiveMinuteAverage =
        ((double)perfstats.loadavg[1]) / (1 << SBITS);
    loadAverageData->fifteenMinuteAverage =
        ((double)perfstats.loadavg[2]) / (1 << SBITS);
    return 0;
  }
  return -1;
#else
...
intptr_t omrsysinfo_get_CPU_utilization(struct OMRPortLibrary *portLibrary,
                                        struct J9SysinfoCPUTime *cpuTime) {
  intptr_t status = OMRPORT_ERROR_SYSINFO_OPFAILED;
#if (defined(LINUX) && !defined(OMRZTPF)) || defined(AIXPPC) || defined(OSX)
  /* omrtime_nano_time() gives monotonically increasing times as against
   * omrtime_hires_clock() that returns times that can (and does) decrease. Use
   * this to compute timestamps.
   */
   ...
   * where user, nice, and system are 64-bit numbers in units of USER_HZ
   * (typically 10 ms).
   *
   * Each number will have up to 21 decimal digits.  Adding spaces and the "cpu
   * " prefix brings the total to ~70.  Allocate 128 bytes to give lots of
   * margin.
   */
  char buf[128];
  ...
  intptr_t fd =
      portLibrary->file_open(portLibrary, "/proc/stat", EsOpenRead, 0);
  if (-1 == fd) {
  ...
  if (bytesRead <= 0) {
    ...
    if (0 == sscanf(buf, "cpu  %" SCNd64 " %" SCNd64 " %" SCNd64, &userTime,
                    &niceTime, &systemTime)) {
      return OMRPORT_ERROR_SYSINFO_GET_STATS_FAILED;
    }
    cpuTime->cpuTime = (userTime + niceTime + systemTime) * NS_PER_CLK;
    cpuTime->numberOfCpus = portLibrary->sysinfo_get_number_CPUs_by_type(
        portLibrary, OMRPORT_CPU_ONLINE);
    status = 0;
  }
#elif defined(OSX)
  ...
  Trc_PRT_sysinfo_get_CPU_utilization_perfstat(stats.user, stats.sys,
                                               stats.ncpus);
  cpuTime->numberOfCpus = stats.ncpus; /* get the actual number of CPUs against
                                          which the time is reported */
  cpuTime->cpuTime = (stats.user + stats.sys) * NS_PER_CPU_TICK;
  status = 0;
#endif
  postTimestamp = portLibrary->time_nano_time(portLibrary);

  if ((0 == preTimestamp) || (0 == postTimestamp) ||
      (postTimestamp < preTimestamp)) {
    Trc_PRT_sysinfo_get_CPU_utilization_timeFailed();
    return OMRPORT_ERROR_SYSINFO_INVALID_TIME;
  }
  /* Use the average of the timestamps before and after reading processor times
   * to reduce bias. */
  cpuTime->timestamp = (preTimestamp + postTimestamp) / 2;
  return status;
#else /* (defined(LINUX) && !defined(OMRZTPF)) || defined(AIXPPC) || \
         defined(OSX) */
  /* Support on z/OS being temporarily removed to avoid wrong CPU stats being
   * passed. */
...
BOOLEAN
omrsysinfo_limit_iterator_hasNext(struct OMRPortLibrary *portLibrary,
                                  J9SysinfoLimitIteratorState *state) {
...
int32_t omrsysinfo_limit_iterator_next(
  ...
  if (0 != getrlimitRC) {
    /* getrlimit failed, but the user may want to try the next value so bump the
     * counter*/
...
/* returns the number of bytes written to the output buffer, including the null
 * terminator, or -1 if the buffer wasn't big enough */
static intptr_t convertWithIConv(struct OMRPortLibrary *portLibrary,
                                 iconv_t *converter, char *inputBuffer,
                                 char *outputBuffer, uintptr_t bufLen) {
  ...
  *outbuf = '\0';
   ...
   * To find how much we wrote, subtract outbytesleft, then add 1 for the null
   * terminator */
...
/* returns the number of bytes written to the output buffer, including the null
 * terminator, or -1 if the buffer wasn't big enough */
static intptr_t convertWithMBTOWC(struct OMRPortLibrary *portLibrary,
...
/**
 * Initializes the supplied buffer such that it can be used by the @ref
 * omrsysinfo_env_iteraror_next() and @ref omrsysinfo_env_iterator_hasNext()
 * APIs.
 *
 * If the caller supplies a non-NULL buffer that is not big enough to store all
 * the name=value entries of the environment, copyEnvToBuffer will store as many
 * entries as possible, until the buffer is used up.
 *
 * If the buffer wasn't even big enough to contain a single entry, set its
 * entire contents to '\0'
 *
 * This call must be protected because memory in char **environ may become
 * inaccessible while copying, resulting in a crash.
 *
 * @param[in] args pointer to a CopyEnvToBufferArgs structure
 *
 * @return
 * 		 - 0 if the buffer was big enough to store the entire
 * environment.
 * 		 - the required size of the buffer, if it was not big enough to
 * store the entire environment.
 *
...
static uintptr_t copyEnvToBuffer(struct OMRPortLibrary *portLibrary,
   ...
   *  - we are converting to UTF-8, where 3 bytes is the maximum it can take to
   * encode anything
   *  - therefore, to keep things simple, just multiple the results of strlen by
   * 3
   */
#define J9_MAX_UTF8_SIZE_BYTES 3
  ...
  for (i = 0; NULL != environ[i]; i++) {
    storageRequiredForEnvironment =
        ...
        (intptr_t)(
            strlen(environ[i]) *
            J9_MAX_UTF8_SIZE_BYTES) /* for the string itself (name=value ) */
...
#if defined(J9VM_USE_ICONV)
  /* iconv_get is not an a2e function, so we need to pass it honest-to-goodness
   * EBCDIC strings */
#ifdef J9ZOS390
#pragma convlit(suspend)
#endif

  converter = iconv_get(portLibrary, J9SYSINFO_ICONV_DESCRIPTOR, "UTF-8",
                        nl_langinfo(CODESET));
...
#pragma convlit(resume)
  ...
  /* Copy each entry from environment to the user supplied buffer until we run
   * out of space */
  cursor = buffer;
  prevItem = NULL;
  ...
  for (i = 0; NULL != environ[i]; i++) {
    ...
    bytesWrittenThisEntry =
        convertWithMBTOWC(portLibrary, environ[i], currentItem->nameAndValue,
    ...
    if (J9VM_INVALID_ICONV_DESCRIPTOR == converter) {
       ...
       */
      strncpy(currentItem->nameAndValue, environ[i],
              spaceLeft - sizeof(EnvListItem));
      bytesWrittenThisEntry = strlen(environ[i]) + 1;
    } else {
      bytesWrittenThisEntry = convertWithIConv(
          portLibrary, &converter, environ[i], currentItem->nameAndValue,
          spaceLeft - sizeof(EnvListItem));
    }
...
int32_t omrsysinfo_env_iterator_init(struct OMRPortLibrary *portLibrary,
  ...
  copyEnvToBufferArgs.numElements =
      0; /* this value is returned by copyEnvToBuffer */

  sigProtectRc = portLibrary->sig_protect(
      portLibrary, copyEnvToBuffer, &copyEnvToBufferArgs,
      copyEnvToBufferSignalHandler, NULL,
      OMRPORT_SIG_FLAG_SIGALLSYNC | OMRPORT_SIG_FLAG_MAY_RETURN,
      &copyEnvToBufferRC);

  if (0 == sigProtectRc) {
    /* copyEnvToToBuffer completed without a signal occurring so we can trust
     * the return value contained in it's args */
    state->buffer = buffer;
    state->bufferSizeBytes = bufferSizeBytes;
    rc = (intptr_t)copyEnvToBufferRC;
  } else if (OMRPORT_SIG_EXCEPTION_OCCURRED == sigProtectRc) {
    /*  copyEnvToToBuffer triggered a signal, and copyEnvToBufferSignalHandler
     * returned OMRPORT_SIG_EXCEPTION_RETURN */
  ...
  if (0 == copyEnvToBufferArgs.numElements) {
    /* This is used by omrsysinfo_env_iterator_hasNext to indicate that there
     * are no elements */
    state->current = NULL;
  } else {
...
BOOLEAN
omrsysinfo_env_iterator_hasNext(struct OMRPortLibrary *portLibrary,
                                J9SysinfoEnvIteratorState *state) {
...
int32_t omrsysinfo_env_iterator_next(struct OMRPortLibrary *portLibrary,
                                     J9SysinfoEnvIteratorState *state,
...
/**
 * Function collects processor usage statistics on Linux platforms and returns
 * the same.
 *
 * @param[in] portLibrary The port library.
 * @param[in] procInfo A pointer to J9ProcessorInfos struct that we populate
 * with processor usage.
 *
 * @return 0 on success and -1 on failure.
 */
static int32_t retrieveLinuxProcessorStats(struct OMRPortLibrary *portLibrary,
                                           struct J9ProcessorInfos *procInfo) {
  int32_t rc = -1;
  int32_t cpuCntr = 0;

  /* Dummy variables for reading processor stats on Linux. Not worth saving, but
   * can be thrown away once used to compute and save into other fields.
   */
  uint64_t nice = 0;
  uint64_t irq = 0;
  uint64_t softirq = 0;
  ...
  if (NULL == procStatFs) {
    ...
    Trc_PRT_retrieveLinuxProcessorStats_Exit(
        OMRPORT_ERROR_SYSINFO_ERROR_READING_PROCESSOR_INFO);
    return OMRPORT_ERROR_SYSINFO_ERROR_READING_PROCESSOR_INFO;
  }

  Trc_PRT_retrieveLinuxProcessorStats_openedProcFs();

  /* getline() expects this line buffer to be malloc()ed so that it can re-size
   * this according to its requirements. As such, this cannot be allocated from
   * port-library heap allocators.
   */
  ...
  if (NULL == linePtr) {
    ...
    Trc_PRT_retrieveLinuxProcessorStats_Exit(
        OMRPORT_ERROR_SYSINFO_MEMORY_ALLOC_FAILED);
  ...
  while ((-1 != getline(&linePtr, &lineSz, procStatFs)) &&
    ...
    /* Insufficient memory allocated since cpu count increased. Cleanup and
     * return error. */
    if (J9_UNEXPECTED(cpuCntr > procInfo->totalProcessorCount)) {
      free(linePtr);
      fclose(procStatFs);
      Trc_PRT_retrieveLinuxProcessorStats_unexpectedCpuCount(
          procInfo->totalProcessorCount, cpuCntr);
      Trc_PRT_retrieveLinuxProcessorStats_Exit(
          OMRPORT_ERROR_SYSINFO_UNEXPECTED_PROCESSOR_COUNT);
      return OMRPORT_ERROR_SYSINFO_UNEXPECTED_PROCESSOR_COUNT;
    }

    /* Either processor count has decreased or remained the same. In either
     * case, parse the data. */
    if (0 == cpuCntr) {
      rc = sscanf(linePtr,
                  "cpu  %" SCNu64 " %" SCNu64 " %" SCNu64 " %" SCNu64
                  " %" SCNu64 " %" SCNu64 " %" SCNu64 " %" SCNu64 " %" SCNu64,
                  &procInfo->procInfoArray[cpuCntr].userTime, &nice,
                  &procInfo->procInfoArray[cpuCntr].systemTime,
                  &procInfo->procInfoArray[cpuCntr].idleTime,
                  &procInfo->procInfoArray[cpuCntr].waitTime, &irq, &softirq,
                  &steal, &guest);
    } else {
      rc = sscanf(linePtr,
                  "cpu%" SCNd32 " %" SCNu64 " %" SCNu64 " %" SCNu64 " %" SCNu64
                  " %" SCNu64 " %" SCNu64 " %" SCNu64 " %" SCNu64 " %" SCNu64,
      ...
      /* If there are disabled SMT's, there may be processors that are online,
       * identified by integers not in range 1 ... (N-1), [where N is the number
       * of CPUs], i.e., exceed sysconf(_SC_NPROCESSORS_CONF).
       */
      procInfo->procInfoArray[cpuCntr].proc_id = cpuid;
    } /* end if */

    /* Check whether for some reason operating system ended up writing a corrupt
     * /proc/stat file or did the platform change the file format (unlikely, but
     * even so, must check against inconsistencies in the expected /proc/stat
     * format).
     */
     ...
     * irq, softirq, steal, and guest times to arrive at the busy times, in
     * addition to the user, system, and wait times.
     */
    procInfo->procInfoArray[cpuCntr].busyTime =
        procInfo->procInfoArray[cpuCntr].userTime +
        procInfo->procInfoArray[cpuCntr].systemTime + nice +
        procInfo->procInfoArray[cpuCntr].waitTime + irq + softirq + steal +
        guest;
    ...
    /* Mark this cpu online using our counter rather than cpu identifier
     * retrieved since that may exceed the cpu count.
     */
 ...
 * @return 0 on success and -1 on failure.
 */
static int32_t retrieveOSXProcessorStats(struct OMRPortLibrary *portLibrary,
                                         struct J9ProcessorInfos *procInfo) {
  ...
  rc = host_processor_info(host, PROCESSOR_CPU_LOAD_INFO, &processorCount,
                           (processor_info_array_t *)&cpuLoadInfo,
                           &msgTypeNumber);
  if (KERN_SUCCESS == rc) {
    rc = host_processor_info(host, PROCESSOR_BASIC_INFO, &processorCount,
                             (processor_info_array_t *)&basicInfo,
                             &msgTypeNumber);
  }
  if (KERN_SUCCESS == rc) {
    for (i = 0; i < processorCount; i += 1) {
      procInfo->procInfoArray[i + 1].online =
          basicInfo[i].running ? OMRPORT_PROCINFO_PROC_ONLINE
                               : OMRPORT_PROCINFO_PROC_OFFLINE;
      procInfo->procInfoArray[i + 1].userTime =
          cpuLoadInfo[i].cpu_ticks[CPU_STATE_USER];
      procInfo->procInfoArray[i + 1].systemTime =
          cpuLoadInfo[i].cpu_ticks[CPU_STATE_SYSTEM];
      procInfo->procInfoArray[i + 1].idleTime =
      ...
      procInfo->procInfoArray[0].userTime +=
          procInfo->procInfoArray[i + 1].userTime;
      procInfo->procInfoArray[0].systemTime +=
          procInfo->procInfoArray[i + 1].systemTime;
      procInfo->procInfoArray[0].idleTime +=
          procInfo->procInfoArray[i + 1].idleTime;
 ...
 * @return 0 on success and -1 on failure.
 */
static int32_t retrieveAIXProcessorStats(struct OMRPortLibrary *portLibrary,
                                         struct J9ProcessorInfos *procInfo) {
#if !defined(J9OS_I5)
  int32_t rc = -1;
  int32_t cpuCntr = 0;
  ...
  /* First, get total (system wide) processor usage statistics using
   * perfstat_cpu_total(). */
  rc = perfstat_cpu_total(NULL, &total_cpu_usage, sizeof(perfstat_cpu_total_t),
                          1);
  if (1 != rc) {
    Trc_PRT_retrieveAIXProcessorStats_perfstatFailed(errno);
    Trc_PRT_retrieveAIXProcessorStats_Exit(
        OMRPORT_ERROR_SYSINFO_ERROR_READING_PROCESSOR_INFO);
    return OMRPORT_ERROR_SYSINFO_ERROR_READING_PROCESSOR_INFO;
  }
  ...
  /* perfstat_cpu() returns (highest_CPUID+1) that was online since last boot,
   * but counts-in stale CPU records too. At present, we can't tell exactly
   * which CPUs are online at this very moment. Note: AIX perfstat
   * documentations incorrectly states this as highest_CPUID, but it actually is
   * highest_CPUID + 1. This is likely to corrected in subsequent release notes.
   */
  onlineProcessorCount = perfstat_cpu(NULL, NULL, sizeof(perfstat_cpu_t), 0);
  if (0 >= onlineProcessorCount) {
    /* Either perfstat_cpu() failed (returned -1) or returned 0 CPUs. 'errno'
     * was set by the system. */
    Trc_PRT_retrieveAIXProcessorStats_perfstatFailed(errno);
    Trc_PRT_retrieveAIXProcessorStats_Exit(
        OMRPORT_ERROR_SYSINFO_ERROR_READING_PROCESSOR_INFO);
    return OMRPORT_ERROR_SYSINFO_ERROR_READING_PROCESSOR_INFO;
  } else if (onlineProcessorCount > procInfo->totalProcessorCount) {
    /* perfstat_cpu() succeeded, but by now number of processors configured has
     * increased! */
    Trc_PRT_retrieveAIXProcessorStats_unexpectedCpuCount(
        procInfo->totalProcessorCount, onlineProcessorCount);
    Trc_PRT_retrieveAIXProcessorStats_Exit(
        OMRPORT_ERROR_SYSINFO_UNEXPECTED_PROCESSOR_COUNT);
    return OMRPORT_ERROR_SYSINFO_UNEXPECTED_PROCESSOR_COUNT;
  }

  /* Allocate as many entries perfstat_cpu_t as there are processors on the
   * machine. */
  statp = (perfstat_cpu_t *)portLibrary->mem_allocate_memory(
      portLibrary, onlineProcessorCount * sizeof(perfstat_cpu_t),
      OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == statp) {
  ...
  rc = perfstat_cpu(&firstcpu, statp, sizeof(perfstat_cpu_t),
                    onlineProcessorCount);
  if (0 >= rc) {
    /* Either perfstat_cpu() failed (with -1) or returned 0 CPUs. 'errno' was
     * set by the system. */
    Trc_PRT_retrieveAIXProcessorStats_perfstatFailed(errno);
    ...
    /* After the last call to perfstat_cpu(3), the name must be "", unless more
     * CPUs were added. */
    portLibrary->mem_free_memory(portLibrary, statp);
    Trc_PRT_retrieveAIXProcessorStats_unexpectedCpuCount(
        procInfo->totalProcessorCount, rc);
    Trc_PRT_retrieveAIXProcessorStats_Exit(
        OMRPORT_ERROR_SYSINFO_UNEXPECTED_PROCESSOR_COUNT);
    return OMRPORT_ERROR_SYSINFO_UNEXPECTED_PROCESSOR_COUNT;
  }
  ...
  for (cpuCntr = 0; cpuCntr <= onlineProcessorCount; cpuCntr++) {
    ...
    if (0 != cpuCntr) {
      ...
      /* Convert ticks values found in JIFFIES to microseconds. */
      procInfo->procInfoArray[cpuid + 1].userTime =
          statp[cpuCntr - 1].user * TICKS_TO_USEC;
      procInfo->procInfoArray[cpuid + 1].systemTime =
          statp[cpuCntr - 1].sys * TICKS_TO_USEC;
      procInfo->procInfoArray[cpuid + 1].idleTime =
          statp[cpuCntr - 1].idle * TICKS_TO_USEC;
      procInfo->procInfoArray[cpuid + 1].waitTime =
          statp[cpuCntr - 1].wait * TICKS_TO_USEC;
      procInfo->procInfoArray[cpuid + 1].busyTime =
          procInfo->procInfoArray[cpuid + 1].userTime +
          procInfo->procInfoArray[cpuid + 1].systemTime +
          procInfo->procInfoArray[cpuid + 1].waitTime;
    } /* end if else */
...
int32_t omrsysinfo_get_processor_info(struct OMRPortLibrary *portLibrary,
                                      struct J9ProcessorInfos *procInfo) {
  int32_t rc = -1;
  int32_t cpuCntr = 0;
  int32_t retries = 0;
  ...
  if (NULL == procInfo) {
    Trc_PRT_sysinfo_get_processor_info_Exit(
        OMRPORT_ERROR_SYSINFO_NULL_OBJECT_RECEIVED);
    return OMRPORT_ERROR_SYSINFO_NULL_OBJECT_RECEIVED;
  }
  ...
  /* Loop, (since we may allocate a processor array that falls short when we
   * actually try reading processor usage statistics) until the time we hit the
   * situation where the allocated space is sufficient for the number of holding
   * information on the available processors.
   */
  do {
    ...
    procInfo->totalProcessorCount =
        portLibrary->sysinfo_get_number_CPUs_by_type(portLibrary,
                                                     OMRPORT_CPU_PHYSICAL);
    Assert_PRT_true(0 < procInfo->totalProcessorCount);

    /* Allocate memory for the 'totalProcessorCount + 1' count of processors.
     * But first check whether we are here to re-allocate, since we fell short
     * with the allocated space or this is the first allocation.
     */
    if (NULL != procInfo->procInfoArray) {
      portLibrary->mem_free_memory(portLibrary, procInfo->procInfoArray);
    }
    procInfoArraySize =
        (procInfo->totalProcessorCount + 1) * sizeof(J9ProcessorInfo);
    procInfo->procInfoArray = portLibrary->mem_allocate_memory(
        portLibrary, procInfoArraySize, OMR_GET_CALLSITE(),
        OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == procInfo->procInfoArray) {
    ...
    Trc_PRT_sysinfo_get_processor_info_allocatedCPUs(
        procInfo->totalProcessorCount);
  ...
  if (0 != rc) {
    /* The helper failed fetching stats, free up memory allocated for the CPU
     * array. */
    portLibrary->mem_free_memory(portLibrary, procInfo->procInfoArray);
    procInfo->procInfoArray = NULL;
    /* Maximum retries exceeded. CPU count seems unstable (increasing
     * everytime). */
    if (SYSINFO_MAX_RETRY_COUNT == retries) {
      Trc_PRT_sysinfo_get_processor_info_cpuCountUnstable();
      Trc_PRT_sysinfo_get_processor_info_Exit(
          OMRPORT_ERROR_SYSINFO_PROCESSOR_COUNT_UNSTABLE);
...
void omrsysinfo_destroy_processor_info(struct OMRPortLibrary *portLibrary,
...
intptr_t omrsysinfo_get_cwd(struct OMRPortLibrary *portLibrary, char *buf,
                            uintptr_t bufLen) {
  intptr_t rc = -1;
...
intptr_t omrsysinfo_get_tmp(struct OMRPortLibrary *portLibrary, char *buf,
...
static void setPortableError(OMRPortLibrary *portLibrary, const char *funcName,
  ...
  errmsglen = portLibrary->str_printf(portLibrary, NULL, 0, "%s%s", funcName,
  ...
  portLibrary->str_printf(portLibrary, errmsgbuff, errmsglen, "%s%s", funcName,
                          strerror(systemErrno));

  /*Set the error message*/
  portLibrary->error_set_last_error_with_message(portLibrary, portableErrno,
                                                 errmsgbuff);
...
int32_t omrsysinfo_get_open_file_count(struct OMRPortLibrary *portLibrary,
                                       uint64_t *count) {
  int32_t ret = -1;
  uint64_t fdCount = 0;
  ...
  if (NULL == count) {
     ...
     */
    portLibrary->str_printf(portLibrary, buffer, sizeof(buffer), procDirectory,
                            getpid());
    dir = opendir(buffer);
    if (NULL == dir) {
    ...
    if (bufferSize <= 0) {
      ...
      struct proc_fdinfo *procFDInfo =
          (struct proc_fdinfo *)portLibrary->mem_allocate_memory(
              portLibrary, bufferSize, OMR_GET_CALLSITE(),
              OMRMEM_CATEGORY_PORT_LIBRARY);
      if (NULL == procFDInfo) {
        Trc_PRT_sysinfo_get_open_file_count_memAllocFailed();
        portLibrary->error_set_last_error_with_message(
            portLibrary, OMRPORT_ERROR_SYSINFO_MEMORY_ALLOC_FAILED,
            "memory allocation for proc_fdinfo failed");
      } else {
        /* Second call with a buffer just allocated, retrieve those available
         * proc_fdinfo instances */
        int32_t retTmp =
...
static void setOSFeature(struct OMROSDesc *desc, uint32_t feature) {
  if ((NULL != desc) && (feature < (OMRPORT_SYSINFO_OS_FEATURES_SIZE * 32))) {
    ...
    desc->features[featureIndex] =
        (desc->features[featureIndex] | (1 << (featureShift)));
  }
}
...
intptr_t omrsysinfo_get_os_description(struct OMRPortLibrary *portLibrary,
                                       struct OMROSDesc *desc) {
  intptr_t rc = -1;
...
BOOLEAN
omrsysinfo_os_has_feature(struct OMRPortLibrary *portLibrary,
                          struct OMROSDesc *desc, uint32_t feature) {
  BOOLEAN rc = FALSE;
...
BOOLEAN
omrsysinfo_os_kernel_info(struct OMRPortLibrary *portLibrary,
                          struct OMROSKernelInfo *kernelInfo) {
  BOOLEAN success = FALSE;

#if defined(LINUX)
  struct utsname name = {{0}};
  if (0 == uname(&name)) {
    if (3 == sscanf(name.release, "%u.%u.%u", &kernelInfo->kernelVersion,
                    &kernelInfo->majorRevision, &kernelInfo->minorRevision)) {
      success = TRUE;
    }
  }
#endif /* defined(LINUX) */
...
static BOOLEAN isCgroupV1Available(struct OMRPortLibrary *portLibrary) {
  ...
  /* If tmpfs is mounted on /sys/fs/cgroup, then it indicates cgroup v1 system
   * is available */
  rc = statfs(OMR_CGROUP_V1_MOUNT_POINT, &buf);
  if (0 != rc) {
    int32_t osErrCode = errno;
    Trc_PRT_isCgroupV1Available_statfs_failed(OMR_CGROUP_V1_MOUNT_POINT,
                                              osErrCode);
    portLibrary->error_set_last_error(
        portLibrary, osErrCode,
        OMRPORT_ERROR_SYSINFO_SYS_FS_CGROUP_STATFS_FAILED);
    result = FALSE;
  } else if (TMPFS_MAGIC != buf.f_type) {
    Trc_PRT_isCgroupV1Available_tmpfs_not_mounted(OMR_CGROUP_V1_MOUNT_POINT);
    portLibrary->error_set_last_error_with_message_format(
        portLibrary, OMRPORT_ERROR_SYSINFO_SYS_FS_CGROUP_TMPFS_NOT_MOUNTED,
        "tmpfs is not mounted on " OMR_CGROUP_V1_MOUNT_POINT);
    result = FALSE;
  }
 ...
 *
 * @param[in] portLibrary pointer to OMRPortLibrary
 * @param[in] cgEntryList pointer to OMRCgroupEntry* which points to circular
 * linked list
 * @param[in] subsystem The subsystem/resource controller for which cgroup is
 * required
 *
...
/**
 * Allocates a new OMRCgroupEntry structure and adds it to the circular linked
 * list pointed by *cgEntryList. On success, *cgEntryList points to the new
 * OMRCgroupEntry structure.
 *
 * @param[in] portLibrary pointer to OMRPortLibrary
 * @param[in] cgEntryList pointer to OMRCgroupEntry* which points to circular
 * linked list
...
static int32_t addCgroupEntry(struct OMRPortLibrary *portLibrary,
  ...
  int32_t rc = 0;
  int32_t cgEntrySize =
      sizeof(OMRCgroupEntry) + strlen(subsystem) + 1 + strlen(cgroupName) + 1;
  OMRCgroupEntry *cgEntry = portLibrary->mem_allocate_memory(
      portLibrary, cgEntrySize, OMR_GET_CALLSITE(),
      OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  if (NULL == cgEntry) {
    ...
    rc = portLibrary->error_set_last_error_with_message(
        portLibrary, OMRPORT_ERROR_SYSINFO_MEMORY_ALLOC_FAILED,
        "memory allocation for cgroup entry failed");
    goto _end;
  }
...
/**
 * Reads /proc/<pid>/cgroup to get information about the cgroups to which the
 * given process belongs.
 *
 * @param[in] portLibrary pointer to OMRPortLibrary
 * @param[in] pid process id
 * @param[in] inContainer if set to TRUE then ignore cgroup in
 * /proc/<pid>/cgroup and use ROOT_CGROUP instead
 * @param[out] cgroupEntryList pointer to OMRCgroupEntry *. On successful
 * return, *cgroupEntry points to a circular linked list. Each element of the
 * list is populated based on the contents of /proc/<pid>/cgroup file.
 * @param[out] availableSubsystems on successful return, contains bitwise-OR of
 * flags of type OMR_CGROUP_SUBSYSTEMS_* indicating the subsystems available for
 * use
 *
...
static int32_t readCgroupFile(struct OMRPortLibrary *portLibrary, int pid,
  ...
  requiredSize = portLibrary->str_printf(portLibrary, NULL, (uint32_t)-1,
                                         "/proc/%d/cgroup", pid);
  Assert_PRT_true(requiredSize <= PATH_MAX);
  portLibrary->str_printf(portLibrary, cgroupFilePath, sizeof(cgroupFilePath),
                          "/proc/%d/cgroup", pid);

  /* Even if 'inContainer' is TRUE, we need to parse the cgroup file to get the
   * list of subsystems */
  ...
  while (0 == feof(cgroupFile)) {
    ...
    /* This array should be large enough to read names of all subsystems. 1024
     * should be enough based on current supported subsystems. */
    ...
    if (0 != ferror(cgroupFile)) {
      ...
      rc = portLibrary->error_set_last_error_with_message_format(
          ...
          "fgets failed to read %s file stream with errno=%d", cgroupFilePath,
          osErrCode);
      goto _end;
    } else if (NULL == buffer) {
      break;
    }
    rc = sscanf(buffer, PROC_PID_CGROUP_ENTRY_FORMAT, &hierId, subsystems,
                cgroup);
    ...
    if (EOF == rc) {
      ...
      rc =
          sscanf(buffer, PROC_PID_CGROUP_SYSTEMD_ENTRY_FORMAT, &hierId, cgroup);

      if (2 != rc) {
        Trc_PRT_readCgroupFile_unexpected_format(cgroupFilePath);
        rc = portLibrary->error_set_last_error_with_message_format(
            portLibrary, OMRPORT_ERROR_SYSINFO_PROCESS_CGROUP_FILE_READ_FAILED,
            "unexpected format of %s", cgroupFilePath);
        goto _end;
      }
      subsystems[0] = '\0';
    } else if (3 != rc) {
    ...
    do {
      ...
      for (i = 0;
           i < sizeof(supportedSubsystems) / sizeof(supportedSubsystems[0]);
           i++) {
        if (OMR_ARE_NO_BITS_SET(available, supportedSubsystems[i].flag) &&
          ...
          rc = addCgroupEntry(portLibrary, &cgEntryList, hierId, cursor,
                              cgroupToUse, supportedSubsystems[i].flag);
          if (0 != rc) {
            goto _end;
          }
 ...
 *
 * @param[in] portLibrary pointer to OMRPortLibrary
 * @param[in] subsystemFlag flag of type OMR_CGROUP_SUBSYSTEMS_* representing
 * the cgroup subsystem
 * @param[in] fileName name of the file under cgroup subsystem
 * @param[in/out] fullPath Absolute Path for the specified file in the cgroup
 * subsystem
...
static int32_t getAbsolutePathOfCgroupSubsystemFile(
  ...
  uint64_t availableSubsystem =
      portLibrary->sysinfo_cgroup_are_subsystems_available(portLibrary,
                                                           subsystemFlag);

  if (availableSubsystem != subsystemFlag) {
    Trc_PRT_readCgroupSubsystemFile_subsystem_not_available(subsystemFlag);
    rc = portLibrary->error_set_last_error_with_message_format(
        portLibrary, OMRPORT_ERROR_SYSINFO_CGROUP_SUBSYSTEM_UNAVAILABLE,
        "cgroup subsystem %s is not available", subsystemNames[subsystem]);
    goto _end;
  }

  cgroup = getCgroupNameForSubsystem(portLibrary, PPG_cgroupEntryList,
  ...
  if (NULL == cgroup) {
    ...
    rc = portLibrary->error_set_last_error_with_message_format(
        portLibrary, OMRPORT_ERROR_SYSINFO_CGROUP_NAME_NOT_AVAILABLE,
        "cgroup name for subsystem %s is not available",
        subsystemNames[MEMORY]);
    Trc_PRT_Assert_ShouldNeverHappen();
    goto _end;
  }

  /* absolute path of the file to be read is:
   * /sys/fs/cgroup/subsystemNames[subsystem]/cgroup/filenName */
  fullPathLen = portLibrary->str_printf(
      portLibrary, NULL, (uint32_t)-1, "%s/%s/%s/%s", OMR_CGROUP_V1_MOUNT_POINT,
      subsystemNames[subsystem], cgroup, fileName);
  if (fullPathLen > *bufferLength) {
    *bufferLength = fullPathLen;
    rc = portLibrary->error_set_last_error_with_message_format(
        portLibrary, OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL,
        "buffer size should be %d bytes", fullPathLen);
    goto _end;
  }

  portLibrary->str_printf(portLibrary, fullPath, fullPathLen, "%s/%s/%s/%s",
                          OMR_CGROUP_V1_MOUNT_POINT, subsystemNames[subsystem],
                          cgroup, fileName);
 ...
 * the cgroup subsystem
 * @param[in] fileName name of the file under cgroup subsystem
 * @param[in/out] subsystemFile pointer to FILE * which stores the handle for
 * the specified subsystem file
 *
 * @return 0 on success, negative error code on any error
 */
static int32_t getHandleOfCgroupSubsystemFile(
  ...
  rc = getAbsolutePathOfCgroupSubsystemFile(portLibrary, subsystemFlag,
                                            fileName, fullPath, &bufferLength);
  if (OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL == rc) {
    fullPath = portLibrary->mem_allocate_memory(portLibrary, bufferLength,
                                                OMR_GET_CALLSITE(),
                                                OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == fullPath) {
      ...
      rc = portLibrary->error_set_last_error_with_message(
          portLibrary, OMRPORT_ERROR_SYSINFO_MEMORY_ALLOC_FAILED,
          "memory allocation for filename buffer failed");
      goto _end;
    }
    allocateMemory = TRUE;
    rc = getAbsolutePathOfCgroupSubsystemFile(
        portLibrary, subsystemFlag, fileName, fullPath, &bufferLength);
  }
  if (0 == rc) {
    *subsystemFile = fopen(fullPath, "r");
    if (NULL == *subsystemFile) {
      ...
      if (ENOENT == osErrCode) {
        rc = portLibrary->error_set_last_error(portLibrary, osErrCode,
                                               OMRPORT_ERROR_FILE_NOENT);
      } else {
        rc = portLibrary->error_set_last_error(
            portLibrary, osErrCode,
            OMRPORT_ERROR_SYSINFO_CGROUP_SUBSYSTEM_FILE_FOPEN_FAILED);
      }
      goto _end;
    }
  }
...
/**
 * Reads the Cgroup metric (if has multiple searched for metric key and reads
 * it) and returns the value to the param `value`
 *
 * @param[in] portLibrary pointer to OMRPortLibrary
 * @param[in] subsystemFlag flag of type OMR_CGROUP_SUBSYSTEMS_* representing
 * the cgroup subsystem
 * @param[in] fileName name of the file under cgroup subsystem
 * @param[in] metricKeyInFile name of the cgroup metric which is in a group of
 * metrics in a cgroup subsystem file
 * @param[in/out] value pointer to char * which stores the value for the
 * specified cgroup subsystem metric
 * @param[in/out] fileContent double pointer to allocate memory and write file
 * content in case of a multiline file with more than 1 metric available in it
 *
 * @return 0 on success, negative error code on any error
 */
static int32_t readCgroupMetricFromFile(struct OMRPortLibrary *portLibrary,
  ...
  int32_t rc = 0;
  FILE *file = NULL;
  rc = getHandleOfCgroupSubsystemFile(portLibrary, subsystemFlag, fileName,
                                      &file);
  if (0 != rc) {
    goto _end;
  }
  if (NULL ==
      metricKeyInFile) { /* If `metricKeyInFile` is NULL then it states the file
                            has only one Cgroup metric to be read */
    if (NULL == fgets(value, MAX_LINE_LENGTH, file)) {
      rc = OMRPORT_ERROR_SYSINFO_CGROUP_SUBSYSTEM_METRIC_NOT_AVAILABLE;
    }
    goto _end;
  } else { /* Else we read the file to a buffer and pick the metrics from it
              later */
    *fileContent = portLibrary->mem_allocate_memory(
        portLibrary, CGROUP_METRIC_FILE_CONTENT_MAX_LIMIT, OMR_GET_CALLSITE(),
        OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL != *fileContent) {
      size_t returnvalue =
          fread(*fileContent, 1, CGROUP_METRIC_FILE_CONTENT_MAX_LIMIT, file);
      if (0 == returnvalue) {
...
/**
 * Read a file under a subsystem in cgroup hierarchy based on the format
 * specified
 *
 * @param[in] portLibrary pointer to OMRPortLibrary
 * @param[in] subsystemFlag flag of type OMR_CGROUP_SUBSYSTEMS_* representing
 ...
 *
 * @return 0 on successfully reading 'numItemsToRead' items from the file,
 * negative error code on any error
 */
static int32_t readCgroupSubsystemFile(struct OMRPortLibrary *portLibrary,
                                       uint64_t subsystemFlag,
                                       const char *fileName,
  ...
  if (numItemsToRead != rc) {
    ...
    rc = portLibrary->error_set_last_error_with_message_format(
        portLibrary, OMRPORT_ERROR_SYSINFO_PROCESS_CGROUP_FILE_READ_FAILED,
        "unexpected format of file %s", fileName);
    goto _end;
  } else {
    rc = 0;
  }
 ...
 *
 * @param[in] portLibrary pointer to OMRPortLibrary
 * @param[out] inContainer pointer to BOOLEAN which on successful return
 * indicates if the process is running in container or not.  On error it
 * indicates FALSE.
 *
...
static int32_t isRunningInContainer(struct OMRPortLibrary *portLibrary,
  ...
  *inContainer = FALSE;
  ...
  if (isCgroupV1Available(portLibrary)) {
    /* Read PID 1's cgroup file /proc/1/cgroup and check cgroup name for each
     * subsystem. If cgroup name for each subsystem points to the root cgroup
     * "/", then the process is not running in a container. For any other cgroup
     * name, assume we are in a container.
     */
    cgroupFile = fopen(OMR_PROC_PID_ONE_CGROUP_FILE, "r");

    if (NULL == cgroupFile) {
      int32_t osErrCode = errno;
      Trc_PRT_isRunningInContainer_fopen_failed(OMR_PROC_PID_ONE_CGROUP_FILE,
                                                osErrCode);
      rc = portLibrary->error_set_last_error(
          portLibrary, osErrCode,
          OMRPORT_ERROR_SYSINFO_PROCESS_CGROUP_FILE_FOPEN_FAILED);
    ...
    while (0 == feof(cgroupFile)) {
      ...
      if (0 != ferror(cgroupFile)) {
        int32_t osErrCode = errno;
        Trc_PRT_isRunningInContainer_fgets_failed(OMR_PROC_PID_ONE_CGROUP_FILE,
                                                  osErrCode);
        rc = portLibrary->error_set_last_error_with_message_format(
            portLibrary, OMRPORT_ERROR_SYSINFO_PROCESS_CGROUP_FILE_READ_FAILED,
            "fgets failed to read %s file stream with errno=%d",
            OMR_PROC_PID_ONE_CGROUP_FILE, osErrCode);
        goto _end;
      } else if (NULL == buffer) {
        break;
      }
      ...
      if (EOF == rc) {
        ...
        rc = sscanf(buffer, PROC_PID_CGROUP_SYSTEMD_ENTRY_FORMAT, &hierId,
                    cgroup);

        if (2 != rc) {
          Trc_PRT_isRunningInContainer_unexpected_format(
              OMR_PROC_PID_ONE_CGROUP_FILE);
          rc = portLibrary->error_set_last_error_with_message_format(
              portLibrary,
              OMRPORT_ERROR_SYSINFO_PROCESS_CGROUP_FILE_READ_FAILED,
              "unexpected format of %s file", OMR_PROC_PID_ONE_CGROUP_FILE);
          goto _end;
        }
      } else if (3 != rc) {
        Trc_PRT_isRunningInContainer_unexpected_format(
      ...
      if ((0 != strcmp(ROOT_CGROUP, cgroup)) &&
          (0 != strcmp(SYSTEMD_INIT_CGROUP, cgroup))) {
        *inContainer = TRUE;
        break;
      }
    }
...
static int32_t getCgroupMemoryLimit(struct OMRPortLibrary *portLibrary,
  ...
  int32_t numItemsToRead =
      1; /* memory.limit_in_bytes file contains only one integer value */
  int32_t rc = 0;

  Trc_PRT_sysinfo_cgroup_get_memlimit_Entry();

  rc = readCgroupSubsystemFile(portLibrary, OMR_CGROUP_SUBSYSTEM_MEMORY,
                               "memory.limit_in_bytes", numItemsToRead,
                               "%" SCNu64, &cgroupMemLimit);
  if (0 != rc) {
    Trc_PRT_sysinfo_cgroup_get_memlimit_memory_limit_read_failed(
        "memory.limit_in_bytes", rc);
    goto _end;
  }

  physicalMemLimit = getPhysicalMemory(portLibrary);
  /* If the cgroup is not imposing any memory limit then the value in
   * memory.limit_in_bytes is close to max value of 64-bit integer, and is more
   * than the physical memory in the system.
   */
  if (cgroupMemLimit > physicalMemLimit) {
    Trc_PRT_sysinfo_cgroup_get_memlimit_unlimited();
    rc = portLibrary->error_set_last_error_with_message(
        portLibrary, OMRPORT_ERROR_SYSINFO_CGROUP_MEMLIMIT_NOT_SET,
...
omrsysinfo_cgroup_is_system_available(struct OMRPortLibrary *portLibrary) {
  ...
  if (NULL == PPG_cgroupEntryList) {
    if (isCgroupV1Available(portLibrary)) {
      ...
      omrthread_monitor_enter(cgroupEntryListMonitor);
      if (NULL == PPG_cgroupEntryList) {
        rc = readCgroupFile(portLibrary, getpid(), inContainer,
                            &PPG_cgroupEntryList,
                            &PPG_cgroupSubsystemsAvailable);
      }
      omrthread_monitor_exit(cgroupEntryListMonitor);
      if (0 != rc) {
        goto _end;
...
_end:
  if (0 == rc) {
    Trc_PRT_sysinfo_cgroup_is_system_available_subsystems(
        PPG_cgroupSubsystemsAvailable);
    result = TRUE;
  } else {
...
uint64_t omrsysinfo_cgroup_get_available_subsystems(
    struct OMRPortLibrary *portLibrary) {
#if defined(LINUX) && !defined(OMRZTPF)
  ...
  Trc_PRT_sysinfo_cgroup_get_available_subsystems_Exit(
      PPG_cgroupSubsystemsAvailable);
...
uint64_t omrsysinfo_cgroup_are_subsystems_available(
...
uint64_t omrsysinfo_cgroup_enable_subsystems(struct OMRPortLibrary *portLibrary,
...
uint64_t omrsysinfo_cgroup_are_subsystems_enabled(
    struct OMRPortLibrary *portLibrary, uint64_t subsystemsFlags) {
#if defined(LINUX) && !defined(OMRZTPF)
...
int32_t omrsysinfo_cgroup_get_memlimit(struct OMRPortLibrary *portLibrary,
                                       uint64_t *limit) {
  int32_t rc = OMRPORT_ERROR_SYSINFO_CGROUP_UNSUPPORTED_PLATFORM;
 ...
 */
struct OMRCgroupEntry *omrsysinfo_get_cgroup_subsystem_list(
    struct OMRPortLibrary *portLibrary) {
#if defined(LINUX) && !defined(OMRZTPF)
...
/*
 * Gets if the Runtime is running in a Container by assigning the BOOLEAN value
 * to inContainer param Returns TRUE if running inside a container and FALSE if
...
int32_t omrsysinfo_cgroup_subsystem_iterator_init(
  ...
  switch (subsystem) {
    case OMR_CGROUP_SUBSYSTEM_MEMORY:
      state->numElements = sizeof(omrCgroupMemoryMetricMap) /
                           sizeof(omrCgroupMemoryMetricMap[0]);
      break;
    case OMR_CGROUP_SUBSYSTEM_CPU:
      state->numElements =
          sizeof(omrCgroupCpuMetricMap) / sizeof(omrCgroupCpuMetricMap[0]);
      break;
    case OMR_CGROUP_SUBSYSTEM_CPUSET:
      state->numElements = sizeof(omrCgroupCpusetMetricMap) /
                           sizeof(omrCgroupCpusetMetricMap[0]);
      break;
    default:
      goto _end;
  }
...
int32_t omrsysinfo_cgroup_subsystem_iterator_metricKey(
    struct OMRPortLibrary *portLibrary,
  ...
  if (NULL != metricKey) {
    ...
    if (state->fileMetricCounter <
        subsystemMetricMap[state->count].metricElementsCount) {
      *metricKey = (subsystemMetricMap[state->count].metricInfoElementList +
                    state->fileMetricCounter)
                       ->metricTag;
      rc = 0;
    } else {
      rc = OMRPORT_ERROR_SYSINFO_CGROUP_SUBSYSTEM_METRIC_NOT_AVAILABLE;
...
int32_t omrsysinfo_cgroup_subsystem_iterator_next(
    struct OMRPortLibrary *portLibrary,
    struct OMRCgroupMetricIteratorState *state,
  ...
  if (NULL == state->fileContent) {
    rc = readCgroupMetricFromFile(portLibrary, state->subsystemid,
                                  subsystemMetricMapElement->metricFileName,
                                  currentElement->metricKeyInFile,
                                  &(state->fileContent), metricElement->value);

    /* In error condition or single metric condition we increment the counter to
  ...
  if (state->fileMetricCounter <
      subsystemMetricMapElement->metricElementsCount) {
    ...
    while (current < end) {
      if (0 ==
          strncmp(current, currentElement->metricKeyInFile, metricKeyLen)) {
    ...
    /* if we complete parse the fileContent and couldn't find the metric return
     * error code */
    if (!isMetricFound) {
      rc = OMRPORT_ERROR_SYSINFO_CGROUP_SUBSYSTEM_METRIC_NOT_AVAILABLE;
    }
...
void omrsysinfo_cgroup_subsystem_iterator_destroy(
    struct OMRPortLibrary *portLibrary,
    struct OMRCgroupMetricIteratorState *state) {
  if (NULL != state->fileContent) {
    portLibrary->mem_free_memory(portLibrary, state->fileContent);
 ...
 */
uintptr_t get_IPL_IstreamCount(void) {
  struct dctist *ist;
 ...
 */
uintptr_t get_Dispatch_IstreamCount(void) {
  struct dctist *ist;
  ist = (struct dctist *)cinfc_fast(CINFC_CMMIST);
