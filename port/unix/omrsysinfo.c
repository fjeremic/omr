...
static uintptr_t
copyEnvToBufferSignalHandler(struct OMRPortLibrary* portLib,
                             uint32_t gpType,
                             void* gpInfo,
                             void* unUsed);

static void
setPortableError(OMRPortLibrary* portLibrary,
                 const char* funcName,
...
static struct OMRCgroupSubsystemMetricMap omrCgroupMemoryMetricMap[] = {
  ...
  { "memory.memsw.usage_in_bytes",
    &(OMRCgroupMetricInfoElement){ "Memory + Swap Usage",
                                   NULL,
                                   "bytes",
                                   FALSE },
    SINGLE_CGROUP_METRIC },
  ...
  { "memory.memsw.max_usage_in_bytes",
    &(OMRCgroupMetricInfoElement){ "Memory + Swap Max Usage",
                                   NULL,
                                   "bytes",
                                   FALSE },
    SINGLE_CGROUP_METRIC },
  { "memory.failcnt",
    &(OMRCgroupMetricInfoElement){ "Memory limit exceeded count",
                                   NULL,
                                   NULL,
                                   FALSE },
    SINGLE_CGROUP_METRIC },
  { "memory.memsw.failcnt",
    &(OMRCgroupMetricInfoElement){ "Memory + Swap limit exceeded count",
                                   NULL,
                                   NULL,
                                   FALSE },
    SINGLE_CGROUP_METRIC },
  { "memory.oom_control",
    &oomControlMetricElementList[0],
...
static struct OMRCgroupSubsystemMetricMap omrCgroupCpusetMetricMap[] = {
  ...
  { "cpuset.mem_exclusive",
    ...
    SINGLE_CGROUP_METRIC },
  { "cpuset.cpus",
    &(OMRCgroupMetricInfoElement){ "CPUs", NULL, NULL, FALSE },
    SINGLE_CGROUP_METRIC },
  { "cpuset.mems",
    &(OMRCgroupMetricInfoElement){ "Mems", NULL, NULL, FALSE },
    SINGLE_CGROUP_METRIC }
};
...
static intptr_t
readSymbolicLink(struct OMRPortLibrary* portLibrary,
                 char* linkFilename,
                 char** result);
...
static intptr_t
searchSystemPath(struct OMRPortLibrary* portLibrary,
                 char* filename,
                 char** result);
...
#if defined(OMRZTPF)
uintptr_t
get_IPL_IstreamCount();
uintptr_t
get_Dispatch_IstreamCount();
#endif /* defined(OMRZTPF) */
...
static void
freeCgroupEntries(struct OMRPortLibrary* portLibrary,
                  OMRCgroupEntry* cgEntryList);
static char*
getCgroupNameForSubsystem(struct OMRPortLibrary* portLibrary,
                          OMRCgroupEntry* cgEntryList,
                          const char* subsystem);
static int32_t
addCgroupEntry(struct OMRPortLibrary* portLibrary,
               OMRCgroupEntry** cgEntryList,
               int32_t hierId,
               const char* subsystem,
               ...
               uint64_t flag);
static int32_t
readCgroupFile(struct OMRPortLibrary* portLibrary,
               int pid,
               BOOLEAN inContainer,
...
static int32_t
getAbsolutePathOfCgroupSubsystemFile(struct OMRPortLibrary* portLibrary,
...
getHandleOfCgroupSubsystemFile(struct OMRPortLibrary* portLibrary,
                               ...
                               FILE** subsystemFile);
static int32_t
readCgroupMetricFromFile(struct OMRPortLibrary* portLibrary,
                         uint64_t subsystemFlag,
                         const char* fileName,
                         ...
                         char* value);
static int32_t
readCgroupSubsystemFile(struct OMRPortLibrary* portLibrary,
                        uint64_t subsystemFlag,
                        const char* fileName,
...
static int32_t
retrieveLinuxMemoryStats(struct OMRPortLibrary* portLibrary,
                         struct J9MemoryInfo* memInfo);
#elif defined(OSX)
static int32_t
retrieveOSXMemoryStats(struct OMRPortLibrary* portLibrary,
                       struct J9MemoryInfo* memInfo);
#elif defined(AIXPPC)
static int32_t
retrieveAIXMemoryStats(struct OMRPortLibrary* portLibrary,
                       struct J9MemoryInfo* memInfo);
#endif
...
void
omrsysinfo_set_number_user_specified_CPUs(struct OMRPortLibrary* portLibrary,
                                          uintptr_t number)
{
...
intptr_t
omrsysinfo_get_env(struct OMRPortLibrary* portLibrary,
                   const char* envVar,
                   char* infoString,
                   uintptr_t bufSize)
{
      ...
      if (plistFile >= 0) {
        ...
        if (NULL != buffer) {
          ...
          while (bytesRemaining > 0) {
            intptr_t result = portLibrary->file_read(
              portLibrary, plistFile, readPtr, bytesRemaining);
            if (result > 0) {
...
intptr_t
omrsysinfo_get_groups(struct OMRPortLibrary* portLibrary,
                      uint32_t** gidList,
                      uint32_t categoryCode)
{
  ...
  *gidList = NULL;
  ...
  if (-1 == size) {
    ...
    setPortableError(portLibrary,
                     getgroupsErrorMsgPrefix,
                     OMRPORT_ERROR_SYSINFO_GETGROUPSSIZE_ERROR,
                     myerror);
  } else {
    ...
    if (NULL == list) {
      ...
      if (-1 == size) {
        ...
        setPortableError(portLibrary,
                         getgroupsErrorMsgPrefix,
                         OMRPORT_ERROR_SYSINFO_GETGROUPS_ERROR,
                         myerror);
      } else {
...
find_executable_name(struct OMRPortLibrary* portLibrary, char** result)
{
  ...
  if (NULL != *result) {
    ...
    if (exeNameLen > deletedStrLen) {
      if (strncmp(&((*result)[exeNameLen - deletedStrLen]),
                  deletedStr,
                  deletedStrLen) == 0) {
  ...
  *result = portLibrary->mem_allocate_memory(
  ...
  portLibrary->str_printf(
    portLibrary, buffer, PATH_MAX, "/proc/%ld/psinfo", getpid());
  ...
  if (-1 == fd) {
    ...
    portLibrary->error_set_last_error_with_message(
      portLibrary, portableError, "Failed to open /proc fs");
    retval = -1;
    goto cleanup;
  }
  ...
  do {
    ...
    if (-1 == readBytes) {
      ...
      portLibrary->error_set_last_error_with_message(
        portLibrary, portableError, "Input/output error");
      retval = -1;
...
gotPathName:
  ...
  *result = (portLibrary->mem_allocate_memory)(portLibrary,
                                               strlen(currentPath) +
 ...
 */
intptr_t
omrsysinfo_get_executable_name(struct OMRPortLibrary* portLibrary,
                               const char* argv0,
                               char** result)
{
                 ...
                 char** result)
{
  ...
  portLibrary->tty_printf(
    portLibrary, "readSymbolicLink: \"%s\"\n%i\n", linkFilename, size);
#endif
  if (size <= 0) {
    return -1;
  }
...
uintptr_t
omrsysinfo_get_number_CPUs_by_type(struct OMRPortLibrary* portLibrary,
                                   uintptr_t type)
{
  ...
  switch (type) {
    ...
    case OMRPORT_CPU_BOUND: {
      ...
      if (0 == toReturn) {
         ...
         * continue */
        rc = readCgroupSubsystemFile(portLibrary,
                               ...
                               struct OMRCgroupMemoryInfo* cgroupMemInfo)
{
  ...
  rc = readCgroupSubsystemFile(portLibrary,
                               ...
                               numItemsToRead,
                               "%lu",
                               &cgroupMemInfo->memoryLimit);
  if (0 != rc) {
    goto _exit;
  }
  rc = readCgroupSubsystemFile(portLibrary,
                               OMR_CGROUP_SUBSYSTEM_MEMORY,
                               CGROUP_MEMORY_USAGE_IN_BYTES_FILE,
                               numItemsToRead,
                               "%lu",
                               &cgroupMemInfo->memoryUsage);
  if (0 != rc) {
    goto _exit;
  }
  rc = readCgroupSubsystemFile(portLibrary,
  ...
  rc = getHandleOfCgroupSubsystemFile(portLibrary,
                                      OMR_CGROUP_SUBSYSTEM_MEMORY,
  ...
  while (0 == feof(memStatFs)) {
    ...
    /* Extract "cache" value */
    if (0 == strncmp(
               tmpPtr, CGROUP_MEMORY_STAT_CACHE, CGROUP_MEMORY_STAT_CACHE_SZ)) {
                         ...
                         struct J9MemoryInfo* memInfo)
{
  ...
  rc = portLibrary->sysinfo_get_limit(portLibrary,
                       ...
                       struct J9MemoryInfo* memInfo)
{
  ...
  if (0 == ret) {
    ...
    if (KERN_SUCCESS == host_statistics(mach_host_self(),
                                        HOST_VM_INFO,
                       ...
                       struct J9MemoryInfo* memInfo)
{
  ...
  rc = perfstat_memory_total(
    NULL, &aixMemoryInfo, sizeof(perfstat_memory_total_t), 1);
  if (1 != rc) {
...
int32_t
omrsysinfo_get_memory_info(struct OMRPortLibrary* portLibrary,
                           struct J9MemoryInfo* memInfo,
                           ...)
{
  int32_t rc = -1;
...
omrsysinfo_startup(struct OMRPortLibrary* portLibrary)
{
  ...
  if (0 == attachedPortLibraries) {
    if (omrthread_monitor_init_with_name(
          &cgroupEntryListMonitor, 0, "cgroup entry list monitor")) {
      return -1;
    }
  }
  attachedPortLibraries += 1;
...
intptr_t
omrsysinfo_get_username(struct OMRPortLibrary* portLibrary,
                        char* buffer,
                        uintptr_t length)
{
...
intptr_t
omrsysinfo_get_groupname(struct OMRPortLibrary* portLibrary,
                         char* buffer,
                         uintptr_t length)
{
  char* remoteCopy = NULL;
...
intptr_t
omrsysinfo_get_hostname(struct OMRPortLibrary* portLibrary,
                        char* buffer,
                        size_t length)
{
...
uint32_t
omrsysinfo_get_limit(struct OMRPortLibrary* portLibrary,
                     uint32_t resourceID,
                     uint64_t* limit)
{
...
uint32_t
omrsysinfo_set_limit(struct OMRPortLibrary* portLibrary,
                     uint32_t resourceID,
                     uint64_t limit)
{
  uint32_t hardLimitRequested =
      ...
      case OMRPORT_RESOURCE_CORE_FILE: {
        ...
        if (-1 == rc) {
          portLibrary->error_set_last_error(
            portLibrary, errno, findError(errno));
          Trc_PRT_sysinfo_getrlimit_error(resource, findError(errno));
          break;
        }
        ...
        if (hardLimitRequested) {
          ...
          if ((OMRPORT_RESOURCE_FILE_DESCRIPTORS == resourceRequested) &&
            ...
            if (-1 == rc) {
              portLibrary->error_set_last_error(
                portLibrary, errno, findError(errno));
              Trc_PRT_sysinfo_setrlimit_error(
                resource, limit, findError(errno));
            } else {
              limit = maxFiles;
            }
          }
...
getLimitSharedMemory(struct OMRPortLibrary* portLibrary, uint64_t* limit)
{
  ...
  fd = portLibrary->file_open(
    portLibrary, "/proc/sys/kernel/shmmax", EsOpenRead, 0);
  if (-1 == fd) {
    portableError = portLibrary->error_last_error_number(portLibrary);
...
intptr_t
omrsysinfo_get_CPU_utilization(struct OMRPortLibrary* portLibrary,
                               struct J9SysinfoCPUTime* cpuTime)
{
  ...
  rc = host_processor_info(mach_host_self(),
  ...
  Trc_PRT_sysinfo_get_CPU_utilization_perfstat(
    stats.user, stats.sys, stats.ncpus);
...
int32_t
omrsysinfo_limit_iterator_init(struct OMRPortLibrary* portLibrary,
...
static intptr_t
convertWithIConv(struct OMRPortLibrary* portLibrary,
                 iconv_t* converter,
                 char* inputBuffer,
                 char* outputBuffer,
                 uintptr_t bufLen)
 ...
 * terminator, or -1 if the buffer wasn't big enough */
static intptr_t
convertWithMBTOWC(struct OMRPortLibrary* portLibrary,
                  char* inputBuffer,
                  char* outputBuffer,
                  uintptr_t bufLen)
{
...
#pragma convlit(suspend)
  ...
  converter = iconv_get(
    portLibrary, J9SYSINFO_ICONV_DESCRIPTOR, "UTF-8", nl_langinfo(CODESET));
...
#pragma convlit(resume)
  ...
  for (i = 0; NULL != environ[i]; i++) {
    ...
    bytesWrittenThisEntry = convertWithMBTOWC(portLibrary,
                                              environ[i],
                                              currentItem->nameAndValue,
    ...
    if (J9VM_INVALID_ICONV_DESCRIPTOR == converter) {
       ...
       */
      strncpy(
        currentItem->nameAndValue, environ[i], spaceLeft - sizeof(EnvListItem));
      bytesWrittenThisEntry = strlen(environ[i]) + 1;
    } else {
      bytesWrittenThisEntry = convertWithIConv(portLibrary,
                                               &converter,
                                               environ[i],
                                               currentItem->nameAndValue,
                                               spaceLeft - sizeof(EnvListItem));
...
int32_t
omrsysinfo_env_iterator_init(struct OMRPortLibrary* portLibrary,
                             ...
                             uintptr_t bufferSizeBytes)
{
  ...
  sigProtectRc = portLibrary->sig_protect(portLibrary,
                                          copyEnvToBuffer,
                                          &copyEnvToBufferArgs,
                                          copyEnvToBufferSignalHandler,
                                          NULL,
...
int32_t
omrsysinfo_env_iterator_next(struct OMRPortLibrary* portLibrary,
                             J9SysinfoEnvIteratorState* state,
 ...
 */
static int32_t
retrieveLinuxProcessorStats(struct OMRPortLibrary* portLibrary,
 ...
 */
static int32_t
retrieveOSXProcessorStats(struct OMRPortLibrary* portLibrary,
                          struct J9ProcessorInfos* procInfo)
{
  ...
  if (KERN_SUCCESS == rc) {
    for (i = 0; i < processorCount; i += 1) {
      procInfo->procInfoArray[i + 1].online = basicInfo[i].running
                                                ? OMRPORT_PROCINFO_PROC_ONLINE
 ...
 */
static int32_t
retrieveAIXProcessorStats(struct OMRPortLibrary* portLibrary,
                          struct J9ProcessorInfos* procInfo)
{
#if !defined(J9OS_I5)
  int32_t rc = -1;
   ...
   * perfstat_cpu_total(). */
  rc =
    perfstat_cpu_total(NULL, &total_cpu_usage, sizeof(perfstat_cpu_total_t), 1);
  if (1 != rc) {
  ...
  rc = perfstat_cpu(
    &firstcpu, statp, sizeof(perfstat_cpu_t), onlineProcessorCount);
  if (0 >= rc) {
...
int32_t
omrsysinfo_get_processor_info(struct OMRPortLibrary* portLibrary,
                              struct J9ProcessorInfos* procInfo)
{
  int32_t rc = -1;
  int32_t cpuCntr = 0;
...
void
omrsysinfo_destroy_processor_info(struct OMRPortLibrary* portLibrary,
...
intptr_t
omrsysinfo_get_cwd(struct OMRPortLibrary* portLibrary,
                   char* buf,
                   uintptr_t bufLen)
{
  intptr_t rc = -1;
...
intptr_t
omrsysinfo_get_tmp(struct OMRPortLibrary* portLibrary,
                   char* buf,
                   uintptr_t bufLen,
                 ...
                 int systemErrno)
{
  ...
  errmsglen = portLibrary->str_printf(
    portLibrary, NULL, 0, "%s%s", funcName, strerror(systemErrno));
  if (errmsglen <= 0) {
  ...
  portLibrary->error_set_last_error_with_message(
    portLibrary, portableErrno, errmsgbuff);
...
int32_t
omrsysinfo_get_open_file_count(struct OMRPortLibrary* portLibrary,
                               uint64_t* count)
{
  int32_t ret = -1;
  uint64_t fdCount = 0;
  ...
  if (NULL == count) {
     ...
     */
    portLibrary->str_printf(
      portLibrary, buffer, sizeof(buffer), procDirectory, getpid());
    dir = opendir(buffer);
    if (NULL == dir) {
...
intptr_t
omrsysinfo_get_os_description(struct OMRPortLibrary* portLibrary,
                          ...
                          struct OMROSKernelInfo* kernelInfo)
{
  ...
  if (0 == uname(&name)) {
    if (3 == sscanf(name.release,
                    "%u.%u.%u",
                    &kernelInfo->kernelVersion,
                    &kernelInfo->majorRevision,
                    &kernelInfo->minorRevision)) {
      success = TRUE;
    }
  }
#endif /* defined(LINUX) */
...
isCgroupV1Available(struct OMRPortLibrary* portLibrary)
{
  ...
  if (0 != rc) {
    ...
    portLibrary->error_set_last_error_with_message_format(
      portLibrary,
      OMRPORT_ERROR_SYSINFO_SYS_FS_CGROUP_TMPFS_NOT_MOUNTED,
               ...
               uint64_t flag)
{
  ...
  OMRCgroupEntry* cgEntry = portLibrary->mem_allocate_memory(
    portLibrary, cgEntrySize, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
               ...
               uint64_t* availableSubsystems)
{
  ...
  requiredSize = portLibrary->str_printf(
    portLibrary, NULL, (uint32_t)-1, "/proc/%d/cgroup", pid);
  ...
  while (0 == feof(cgroupFile)) {
    ...
    rc =
      sscanf(buffer, PROC_PID_CGROUP_ENTRY_FORMAT, &hierId, subsystems, cgroup);
    ...
    do {
      ...
      for (i = 0;
        ...
        if (OMR_ARE_NO_BITS_SET(available, supportedSubsystems[i].flag) &&
          ...
          rc = addCgroupEntry(portLibrary,
                              &cgEntryList,
                              hierId,
                              cursor,
                              cgroupToUse,
                              supportedSubsystems[i].flag);
          if (0 != rc) {
            goto _end;
          }
                                     ...
                                     intptr_t* bufferLength)
{
  ...
  if (availableSubsystem != subsystemFlag) {
    ...
    rc = portLibrary->error_set_last_error_with_message_format(
      portLibrary,
      OMRPORT_ERROR_SYSINFO_CGROUP_SUBSYSTEM_UNAVAILABLE,
      "cgroup subsystem %s is not available",
      subsystemNames[subsystem]);
    goto _end;
  }

  cgroup = getCgroupNameForSubsystem(
    portLibrary, PPG_cgroupEntryList, subsystemNames[subsystem]);
  if (NULL == cgroup) {
    ...
    rc = portLibrary->error_set_last_error_with_message_format(
      portLibrary,
      OMRPORT_ERROR_SYSINFO_CGROUP_NAME_NOT_AVAILABLE,
  ...
  fullPathLen = portLibrary->str_printf(portLibrary,
                                        NULL,
                                        (uint32_t)-1,
                                        "%s/%s/%s/%s",
                                        OMR_CGROUP_V1_MOUNT_POINT,
                                        subsystemNames[subsystem],
                                        cgroup,
                                        fileName);
  if (fullPathLen > *bufferLength) {
    *bufferLength = fullPathLen;
    rc = portLibrary->error_set_last_error_with_message_format(
      portLibrary,
      OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL,
      "buffer size should be %d bytes",
      fullPathLen);
    goto _end;
  }
                               ...
                               FILE** subsystemFile)
{
  ...
  if (OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL == rc) {
    fullPath = portLibrary->mem_allocate_memory(portLibrary,
                                                bufferLength,
                                                OMR_GET_CALLSITE(),
                                                OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == fullPath) {
  ...
  if (0 == rc) {
    *subsystemFile = fopen(fullPath, "r");
    if (NULL == *subsystemFile) {
      ...
      if (ENOENT == osErrCode) {
        rc = portLibrary->error_set_last_error(
          portLibrary, osErrCode, OMRPORT_ERROR_FILE_NOENT);
      } else {
        rc = portLibrary->error_set_last_error(
          portLibrary,
          osErrCode,
                         ...
                         char* value)
{
  ...
  FILE* file = NULL;
  rc =
    getHandleOfCgroupSubsystemFile(portLibrary, subsystemFlag, fileName, &file);
  if (0 != rc) {
    goto _end;
  }
  if (NULL ==
    ...
    *fileContent =
      portLibrary->mem_allocate_memory(portLibrary,
                                       CGROUP_METRIC_FILE_CONTENT_MAX_LIMIT,
                                       OMR_GET_CALLSITE(),
                                       OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL != *fileContent) {
      size_t returnvalue =
  ...
  *inContainer = FALSE;
  ...
  if (isCgroupV1Available(portLibrary)) {
    ...
    while (0 == feof(cgroupFile)) {
      ...
      rc = sscanf(
        buffer, PROC_PID_CGROUP_ENTRY_FORMAT, &hierId, subsystems, cgroup);
...
getCgroupMemoryLimit(struct OMRPortLibrary* portLibrary, uint64_t* limit)
{
  ...
  if (cgroupMemLimit > physicalMemLimit) {
    ...
    rc = portLibrary->error_set_last_error_with_message(
      portLibrary,
      OMRPORT_ERROR_SYSINFO_CGROUP_MEMLIMIT_NOT_SET,
...
uint64_t
omrsysinfo_cgroup_enable_subsystems(struct OMRPortLibrary* portLibrary,
...
int32_t
omrsysinfo_cgroup_get_memlimit(struct OMRPortLibrary* portLibrary,
                               uint64_t* limit)
{
  int32_t rc = OMRPORT_ERROR_SYSINFO_CGROUP_UNSUPPORTED_PLATFORM;
  ...
  struct OMRCgroupMetricElement* metricElement)
{
  ...
  if (NULL == state->fileContent) {
    rc = readCgroupMetricFromFile(portLibrary,
                                  state->subsystemid,
