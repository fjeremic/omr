    ...
    while (0 == feof(memStatFs)) {
        ...
        if (0 == strncmp(tmpPtr, MEMTOTAL_PREFIX, MEMTOTAL_PREFIX_SZ)) {
            ...
            if ((LONG_MIN == memInfo->totalPhysical) || (LONG_MAX == memInfo->totalPhysical)) {
                ...
                rc = (ERANGE == errno) ? OMRPORT_ERROR_SYSINFO_PARAM_HAS_INVALID_RANGE : OMRPORT_ERROR_SYSINFO_ERROR_READING_MEMORY_INFO;
                goto _cleanup;
            } /* end outer-if */
...
retrieveLinuxProcessorStats(struct OMRPortLibrary* portLibrary, struct J9ProcessorInfos* procInfo)
{
    ...
    while ((-1 != getline(&linePtr, &lineSz, procStatFs)) && (0 == strncmp(linePtr, PROCSTATPREFIX, sizeof(PROCSTATPREFIX) - 1))) {
        int32_t cpuid = -1;
		 ...
		 */
        procInfo->procInfoArray[cpuCntr].busyTime = procInfo->procInfoArray[cpuCntr].userTime + procInfo->procInfoArray[cpuCntr].systemTime + nice + procInfo->procInfoArray[cpuCntr].waitTime + irq + softirq + steal + guest;
...
retrieveOSXProcessorStats(struct OMRPortLibrary* portLibrary, struct J9ProcessorInfos* procInfo)
{
    ...
    if (KERN_SUCCESS == rc) {
        for (i = 0; i < processorCount; i += 1) {
            ...
            procInfo->procInfoArray[i + 1].busyTime = cpuLoadInfo[i].cpu_ticks[CPU_STATE_USER] + cpuLoadInfo[i].cpu_ticks[CPU_STATE_SYSTEM] + cpuLoadInfo[i].cpu_ticks[CPU_STATE_NICE];
...
retrieveAIXProcessorStats(struct OMRPortLibrary* portLibrary, struct J9ProcessorInfos* procInfo)
{
    ...
    procInfo->procInfoArray[0].busyTime = procInfo->procInfoArray[0].userTime + procInfo->procInfoArray[0].systemTime + procInfo->procInfoArray[0].waitTime;
    ...
    for (cpuCntr = 0; cpuCntr <= onlineProcessorCount; cpuCntr++) {
        ...
        if (0 != cpuCntr) {
            ...
            procInfo->procInfoArray[cpuid + 1].busyTime = procInfo->procInfoArray[cpuid + 1].userTime + procInfo->procInfoArray[cpuid + 1].systemTime + procInfo->procInfoArray[cpuid + 1].waitTime;
        } /* end if else */
...
void omrsysinfo_cgroup_subsystem_iterator_destroy(struct OMRPortLibrary* portLibrary, struct OMRCgroupMetricIteratorState* state)
{
    if (NULL != state->fileContent) {
        portLibrary->mem_free_memory(portLibrary, state->fileContent);
        state->fileContent = NULL;
