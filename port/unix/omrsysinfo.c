...
static int32_t
getHandleOfCgroupSubsystemFile(struct OMRPortLibrary *portLibrary,
                               uint64_t subsystemFlag, const char *fileName,
                               FILE **subsystemFile);
...
#if defined(LINUX)
static int32_t
retrieveLinuxMemoryStatsFromProcFS(struct OMRPortLibrary *portLibrary,
                                   struct J9MemoryInfo *memInfo);
static int32_t
retrieveLinuxCgroupMemoryStats(struct OMRPortLibrary *portLibrary,
...
uintptr_t omrsysinfo_get_number_CPUs_by_type(struct OMRPortLibrary *portLibrary,
  ...
  case OMRPORT_CPU_PHYSICAL:
    ...
    toReturn = portLibrary->sysinfo_get_number_CPUs_by_type(portLibrary,
                                                            OMRPORT_CPU_ONLINE);
  ...
  case OMRPORT_CPU_BOUND: {
    ...
    /* If system calls failed to retrieve info, do not check cgroup at all, and
     * give an error */
    if (0 == toReturn) {
      Trc_PRT_sysinfo_get_number_CPUs_by_type_failedBound("errno: ", errno);
      ...
      if (0 == rc) {
        rc = readCgroupSubsystemFile(portLibrary, OMR_CGROUP_SUBSYSTEM_CPU,
                                     "cpu.cfs_period_us", numItemsToRead, "%lu",
                                     &cpuPeriod);
        if (0 == rc) {
     ...
     * Thus, this just returns number of CPUs.
     */
    toReturn = sysconf(_SC_NPROCESSORS_CONF);
    if (0 == toReturn) {
      Trc_PRT_sysinfo_get_number_CPUs_by_type_failedBound("errno: ", errno);
...
intptr_t
omrsysinfo_get_load_average(struct OMRPortLibrary *portLibrary,
...
int32_t
omrsysinfo_limit_iterator_next(struct OMRPortLibrary *portLibrary,
                               J9SysinfoLimitIteratorState *state,
...
uint64_t
omrsysinfo_cgroup_are_subsystems_available(struct OMRPortLibrary *portLibrary,
...
uint64_t
omrsysinfo_cgroup_are_subsystems_enabled(struct OMRPortLibrary *portLibrary,
                                         uint64_t subsystemsFlags) {
#if defined(LINUX) && !defined(OMRZTPF)
...
int32_t omrsysinfo_cgroup_subsystem_iterator_init(
  ...
  case OMR_CGROUP_SUBSYSTEM_MEMORY:
    state->numElements =
        sizeof(omrCgroupMemoryMetricMap) / sizeof(omrCgroupMemoryMetricMap[0]);
    break;
  case OMR_CGROUP_SUBSYSTEM_CPU:
    state->numElements =
  ...
  case OMR_CGROUP_SUBSYSTEM_CPUSET:
    state->numElements =
        sizeof(omrCgroupCpusetMetricMap) / sizeof(omrCgroupCpusetMetricMap[0]);
    break;
  default:
    goto _end;
  }
