...
/* Used to add space for the null terminator. Adding a wchar in case the
 * platform locale uses wchar
 */
#define NULL_TERM_SZ sizeof(wchar_t)

static uintptr_t get_conv_txt_num_bytes(struct OMRPortLibrary *portLibrary,
                                        const char *message);
static void syslog_write(struct OMRPortLibrary *portLibrary, const char *buf,
                         int priority);
...
static uintptr_t get_conv_txt_num_bytes(struct OMRPortLibrary *portLibrary,
  ...
  const int32_t convRes = portLibrary->str_convert(
      portLibrary, J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, message,
      strlen(message), NULL, 0U);
...
static void syslog_write(struct OMRPortLibrary *portLibrary, const char *buf,
  ...
  if (NULL_TERM_SZ < lclMsgLen) {
    lclMsg = portLibrary->mem_allocate_memory(portLibrary, lclMsgLen,
                                              OMR_GET_CALLSITE(),
    ...
    if (NULL != lclMsg) {
       ...
       */
      const int32_t convRes = portLibrary->str_convert(
          portLibrary, J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, buf,
          strlen(buf), lclMsg, lclMsgLen);
