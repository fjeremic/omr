...
static int
growBuffer(struct OMRPortLibrary* portLibrary,
           char* stackBuf,
           char** bufStart,
           char** cursor,
           size_t* bytesLeft,
           uintptr_t* bufLen);
static intptr_t
file_write_using_iconv(struct OMRPortLibrary* portLibrary,
                       intptr_t fd,
                       const char* buf,
                       intptr_t nbytes);
#endif
...
static intptr_t
file_write_using_wctomb(struct OMRPortLibrary* portLibrary,
                        intptr_t fd,
                        const char* buf,
                        intptr_t nbytes);
#endif
                        ...
                        intptr_t nbytes)
{
  ...
  if (0 != newLength) {
    if (newLength > sizeof(stackBuf)) {
      newBuf = portLibrary->mem_allocate_memory(portLibrary,
                                                newLength,
                                                OMR_GET_CALLSITE(),
...
#ifndef OMRZTPF
    converter = iconv_get(
      portLibrary, J9FILETEXT_ICONV_DESCRIPTOR, nl_langinfo(CODESET), "UTF-8");
#else
  converter =
    iconv_get(portLibrary, J9FILETEXT_ICONV_DESCRIPTOR, "IBM1047", "ISO8859-1");
#endif
...
#define J9FILETEXT_ESCAPE_STR_SIZE 6 /* max size of unicode format */
      ...
      if (utf8Length == 0) {
        ...
        escapedLength = portLibrary->str_printf(portLibrary,
                                                escapedStr,
                                                J9FILETEXT_ESCAPE_STR_SIZE,
                                                unicodeFormat,
                                                (uintptr_t)unicodeC);
      }
