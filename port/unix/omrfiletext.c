...
static int growBuffer(struct OMRPortLibrary *portLibrary, char *stackBuf,
                      char **bufStart, char **cursor, size_t *bytesLeft,
                      uintptr_t *bufLen);
static intptr_t file_write_using_iconv(struct OMRPortLibrary *portLibrary,
...
static void translateUTF8String(const uint8_t *in, uint8_t *out,
                                intptr_t nbytes);
static intptr_t file_write_using_wctomb(struct OMRPortLibrary *portLibrary,
                                        intptr_t fd, const char *buf,
                                        intptr_t nbytes);
#endif

intptr_t omrfile_write_text(struct OMRPortLibrary *portLibrary, intptr_t fd,
...
#pragma convlit(resume)
  ...
  /* z/OS and z/TPF always needs to translate to EBCDIC, unless we are already
   * using EBCDIC as the native encoding, */
 ...
 * Copies over the specified contents from the original buffer to the new one,
 ...
 * including cursor will be copied to the new buffer. cursor is updated to have
 * the same offset in the new buffer.
...
#ifdef J9ZOS390
  /* LIR 1280 (z/OS only) - every failed call to iconv_open() is recorded on the
   * operator console, so don't retry */
  if (FALSE == PPG_file_text_iconv_open_failed) {
    /* iconv_get is not an a2e function, so we need to pass it
     * honest-to-goodness EBCDIC strings */
#pragma convlit(suspend)
#endif

#ifndef OMRZTPF
    converter = iconv_get(portLibrary, J9FILETEXT_ICONV_DESCRIPTOR,
                          nl_langinfo(CODESET), "UTF-8");
#else
  converter = iconv_get(portLibrary, J9FILETEXT_ICONV_DESCRIPTOR, "IBM1047",
                        "ISO8859-1");
#endif
...
#define J9FILETEXT_ESCAPE_STR_SIZE 6 /* max size of unicode format */
      ...
      size_t utf8Length =
          decodeUTF8CharN((const uint8_t *)inbuf, &unicodeC, inbytesleft);
      ...
      if (utf8Length == 0) {
        ...
        escapedLength = portLibrary->str_printf(
            portLibrary, escapedStr, J9FILETEXT_ESCAPE_STR_SIZE, unicodeFormat,
            (uintptr_t)unicodeC);
      }
      ...
      if ((size_t)-1 == iconv(converter, &escapedStrStart, &escapedLength,
                              &outbuf, &outbytesleft)) {
        /* not handling EILSEQ here because:
         *  1. we can't do much if iconv() fails to convert ascii.
         *  2. inbuf and inbytesleft have been explicitly updated so the while
         * loop will get terminated after converting the rest of the characters.
         */

        tmp_errno = errno;

        /* if the remaining outbuf is too small, then grow it before storing
         * Unicode string representation */
        if (tmp_errno == E2BIG) {
          if (growBuffer(portLibrary, stackBuf, &bufStart, &outbuf,
                         &outbytesleft, &outBufLen) < 0) {
            /* failed to grow buffer, just output what we've got so far */
            break;
          }
        }
  ...
  /* CMVC 152575 - the converted string is not necessarily the same length in
   * bytes as the original string */
  bytesToWrite = outbuf - bufStart;
  result =
      portLibrary->file_write(portLibrary, fd, (void *)bufStart, bytesToWrite);
...
char *omrfile_read_text(struct OMRPortLibrary *portLibrary, intptr_t fd,
  ...
  while ((!foundEOL) && (nbytes > 0)) {
    ...
    for (i = 0; i < count; i++) {
      ...
      *cursor = c;
      ...
      if (eol == c) { /* EOL */
        /*function will return on EOL, move the file pointer to the EOL, prepare
         * for next read*/
...
int32_t omrfile_get_text_encoding(struct OMRPortLibrary *portLibrary,
  ...
  /* In case of very detailed text from OS truncate the string at first
   * whitespace. */
  c_ptr = codepage;
  while (*c_ptr++ != '\0') {
    if (*c_ptr == ' ') {
