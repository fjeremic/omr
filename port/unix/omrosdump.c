...
#if defined(AIXPPC)
/* **********
These definitions were copied from /usr/include/sys/proc.h in AIX 6.1, because
this functionality does not exist on AIX 5.3. The code that uses these
definitions does a runtime lookup to see if the proc_setattr() function exists.
********* */
#ifndef PA_IGNORE
/*
...
uintptr_t omrdump_create(struct OMRPortLibrary *portLibrary, char *filename,
  ...
  if ((sys_parmRC == 0) && (myvar.v.v_fullcore.value == 1)) {
    ...
    portLibrary->tty_err_printf(
        portLibrary,
        "Note: \"Enable full CORE dump\" in smit is set to FALSE and as a "
        "result there will be limited threading information in core file.\n");
  }
#endif /* aix_ppc */
  ...
  if (0 == pid) {
    ...
    /*
     * CMVC 95748: don't use abort() after fork() on Linux as this seems to
     * upset certain levels of glibc
     */
...
#define J9_DUMP_SIGNAL SIGABRT
  ...
  /* We are now in the parent process. First check that the fork() worked OK
   * (CMVC 130439) */
  if (pid < 0) {
    portLibrary->str_printf(
        portLibrary, filename, EsMaxPath,
        "insufficient system resources to generate dump, errno=%d \"%s\"",
  ...
  if (filename && filename[0] != '\0') {
    ...
    /* check if the generated core filename ends in .Z (which indicates core
     * compression was on) */
    if (strstr(corepath, ".Z") ==
        (corepath + strlen(corepath) - strlen(".Z"))) {
    ...
    if (rename(corepath, filename)) {
      portLibrary->str_printf(portLibrary, filename, EsMaxPath,
                              "cannot find core file: \"%s\". check \"ulimit "
                              "-Hc\" is set high enough",
                              strerror(errno));
      return 1;
    }
...
#error "This platform doesn't have an implementation of omrdump_create"
  ...
  if (getenv("I5_JVM_SUPPLIED_DUMP") == 0) {
    ...
    /* Generate the core file. On I5 gencore will always generate a full core
     * dump */
...
int32_t omrdump_startup(struct OMRPortLibrary *portLibrary) {
  ...
  if (0 == portLibrary->sl_open_shared_library(portLibrary, NULL, &handle, 0)) {
    ...
    if (0 == portLibrary->sl_lookup_name(portLibrary, handle, "proc_setattr",
                                         (uintptr_t *)&setattr, "IPLp")) {
      procattr_t attr;
      int rc = 0;

      memset(&attr, PA_IGNORE, sizeof(procattr_t));
      /* enable the flags that are required for including non-anonymous mmap
       * regions and shared memory in core file */
      ...
      if (0 != rc) {
        if (ENOSYS == errno) {
          /* CMVC 176613: we can get here if the machine was updated from
           * 6100-04 to 6100-05 without a subsequent reboot, in which case,
           * behave the same as if we're running on an AIX version which does
           * not support proc_setattr().
           */
        } else {
          int32_t error =
              errno; /* Save errno for past closing the shared library. */
      ...
      /* do not return error as we may be running on an AIX version which does
       * not support proc_setattr() */
    }
    portLibrary->sl_close_shared_library(portLibrary, handle);
  }
