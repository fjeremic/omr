 ...
 */
uintptr_t omrdump_create(struct OMRPortLibrary* portLibrary, char* filename, char* dumpType, void* userData)
{
#if !defined(J9OS_I5)
    char* lastSep = NULL;
    intptr_t pid = 0;
    ...
    if ((sys_parmRC == 0) && (myvar.v.v_fullcore.value == 1)) {
        ...
        portLibrary->tty_err_printf(portLibrary,
            "Note: \"Enable full CORE dump\" in smit is set to FALSE and as a result there will be limited threading "
            "information in core file.\n");
    }
#endif /* aix_ppc */
...
#define J9_DUMP_SIGNAL SIGABRT
    ...
    if (filename && filename[0] != '\0') {
        ...
        if (rename(corepath, filename)) {
            portLibrary->str_printf(portLibrary, filename, EsMaxPath,
                "cannot find core file: \"%s\". check \"ulimit -Hc\" is set high enough", strerror(errno));
            return 1;
        }
...
int32_t omrdump_startup(struct OMRPortLibrary* portLibrary)
{
    ...
    if (0 == portLibrary->sl_open_shared_library(portLibrary, NULL, &handle, 0)) {
        ...
        if (0 == portLibrary->sl_lookup_name(portLibrary, handle, "proc_setattr", (uintptr_t*)&setattr, "IPLp")) {
            ...
            /* enable the flags that are required for including non-anonymous mmap regions and shared memory in core
             * file */
            ...
            if (0 != rc) {
                if (ENOSYS == errno) {
                    /* CMVC 176613: we can get here if the machine was updated from 6100-04 to 6100-05 without a
                     * subsequent reboot, in which case, behave the same as if we're running on an AIX version which
                     * does not support proc_setattr().
                     */
                } else {
