...
int32_t omrdump_startup(struct OMRPortLibrary* portLibrary)
{
  ...
  if (0 == portLibrary->sl_open_shared_library(portLibrary, NULL, &handle, 0)) {
    ...
    if (0 == portLibrary->sl_lookup_name(portLibrary, handle, "proc_setattr", (uintptr_t*)&setattr, "IPLp")) {
      ...
      if (0 != rc) {
        if (ENOSYS == errno) {
          /* CMVC 176613: we can get here if the machine was updated from 6100-04 to 6100-05 without a subsequent
           * reboot, in which case, behave the same as if we're running on an AIX version which does not support
           * proc_setattr().
           */
        } else {
