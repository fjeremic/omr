...
static int32_t findError(int32_t errorCode);
    ...
    (defined(AIXPPC) && !defined(J9OS_I5))
static void updateJ9FileStat(struct OMRPortLibrary *portLibrary,
                             J9FileStat *j9statBuf, struct stat *statBuf,
                             PlatformStatfs *statfsBuf);
#else  /* (defined(LINUX) && !defined(OMRZTPF)) || defined(OSX) || \
          (defined(AIXPPC) && !defined(J9OS_I5)) */
static void updateJ9FileStat(struct OMRPortLibrary *portLibrary,
                             J9FileStat *j9statBuf, struct stat *statBuf);
#endif /* (defined(LINUX) && !defined(OMRZTPF)) || defined(OSX) || \
          (defined(AIXPPC) && !defined(J9OS_I5)) */
 ...
 * @param[in] statBuf Pointer to struct stat that contains system specific
 * information about the file
 * @param[in] statfsBuf Pointer to PlatformStatfs that contains system specific
 ...
 */
int32_t omrfile_close(struct OMRPortLibrary *portLibrary, intptr_t inFD) {
  int32_t rc = 0;
  int fd = (int)inFD;
  ...
  if (0 == stat(path, &buffer)) {
    if (S_ISDIR(buffer.st_mode)) {
      ...
      errMsg = portLibrary->nls_lookup_message(
          portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE,
          J9NLS_PORT_FILE_OPEN_FILE_IS_DIR, NULL);
      portLibrary->error_set_last_error_with_message(
          portLibrary, OMRPORT_ERROR_FILE_ISDIR, errMsg);
      return -1;
    }
  }

#if defined(J9ZOS390) && !defined(OMR_EBCDIC)
  /* On zOS use the non-tagging version of atoe_open for VM generated files that
   * are always platform encoded (EBCDIC), e.g. javacore dumps. See CMVC 199888
   */
 ...
 * Write to a file.
 *
 * Writes up to nbytes from the provided buffer  to the file referenced by the
 * file descriptor.
 *
 * @param[in] portLibrary The port library
 ...
 *
 * @return Number of bytes written on success, portable error return code (which
 * is negative) on failure.
 */
intptr_t omrfile_write(struct OMRPortLibrary *portLibrary, intptr_t inFD,
...
#error FD_BIAS must be 0
#endif
  {
    ...
    do {
      /* write will just do the right thing for OMRPORT_TTY_OUT and
       * OMRPORT_TTY_ERR */
 ...
 */
intptr_t omrfile_read(struct OMRPortLibrary *portLibrary, intptr_t inFD,
...
/**
 * Repositions the offset of the file descriptor to a given offset as per
 * directive whence.
 *
 * @param[in] portLibrary The port library
 * @param[in] fd The file descriptor.
 ...
 */
int64_t omrfile_seek(struct OMRPortLibrary *portLibrary, intptr_t inFD,
  ...
  if ((whence < EsSeekSet) || (whence > EsSeekEnd)) {
    int32_t errorCode = portLibrary->error_set_last_error(
        portLibrary, -1, OMRPORT_ERROR_FILE_INVAL);
  ...
  if (fd < FD_BIAS) {
    ...
    int32_t errorCode = portLibrary->error_set_last_error(
        portLibrary, -1, OMRPORT_ERROR_FILE_BADF);
    Trc_PRT_file_seek_Exit(errorCode);
    return -1;
  }
#endif
...
int32_t omrfile_chmod(struct OMRPortLibrary *portLibrary, const char *path,
  ...
  struct stat buffer;
  const int32_t MODEBITS = 07777; /* bits from  S_ISUID down: lstat can return
                                     mode with the high-order bits set. */
...
int64_t omrfile_flength(struct OMRPortLibrary *portLibrary, intptr_t inFD) {
  struct stat st;
  int fd = (int)inFD;
 ...
 * @internal @todo return negative portable return code on failure.
 */
int32_t omrfile_sync(struct OMRPortLibrary *portLibrary, intptr_t inFD) {
  int fd = (int)inFD;
  int32_t result = 0;
 ...
 * error returned is not thread safe, it may not be related to the operation
 * that failed for this thread.
 *
 * @param[in] portLibrary The port library
 *
 ...
 * the corresponding string.  It is here as a helper function for JCL.  Once
 * omrerror is integrated into the port library this function should probably
 * disappear.
 */
 ...
 * that were created by @ref omrfile_startup should be destroyed here.
 *
 * @param[in] portLibrary The port library
 *
 ...
 * that are required for the file operations may be created here.  All resources
 * created here should be destroyed in @ref omrfile_shutdown.
 *
 * @param[in] portLibrary The port library
 *
 * @return 0 on success, negative error code on failure.  Error code values
 * returned are \arg OMRPORT_ERROR_STARTUP_FILE
 *
 ...
 */
int32_t omrfile_set_length(struct OMRPortLibrary *portLibrary, intptr_t inFD,
                           int64_t newLength) {
  int fd = (int)inFD;
  int32_t rc;
...
/**
 * This function will acquire a lock of the requested type on the given file,
 * starting at offset bytes from the start of the file and continuing for length
 * bytes
 *
 * @param [in]   portLibrary            The port library
 * @param [in]   fd                              The file descriptor/handle of
 * the file to be locked
 * @param [in]   lockFlags              Flags indicating the type of lock
 * required and whether the call should block
...
int32_t omrfile_lock_bytes(struct OMRPortLibrary *portLibrary, intptr_t fd,
  ...
  if (!(lockFlags & OMRPORT_FILE_READ_LOCK) &&
      !(lockFlags & OMRPORT_FILE_WRITE_LOCK)) {
    Trc_PRT_file_lock_bytes_unix_failed_noReadWrite();
    errMsg = portLibrary->nls_lookup_message(
        portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE,
        J9NLS_PORT_FILE_LOCK_INVALID_FLAG, NULL);
    portLibrary->str_printf(portLibrary, errBuf, sizeof(errBuf), errMsg,
                            lockFlags);
    portLibrary->error_set_last_error_with_message(
  ...
  if (!(lockFlags & OMRPORT_FILE_WAIT_FOR_LOCK) &&
      !(lockFlags & OMRPORT_FILE_NOWAIT_FOR_LOCK)) {
  ...
  Trc_PRT_file_lock_bytes_unix_callingFcntl(
      fcntlCmd, fcntlFlock.l_type, fcntlFlock.l_whence, fcntlFlock.l_start,
      fcntlFlock.l_len);
  ...
  if (rc == -1) {
    ...
    if (errno == EDEADLK) {
      portLibrary->error_set_last_error(portLibrary, errno,
                                        OMRPORT_ERROR_FILE_LOCK_EDEADLK);
    } else {
      portLibrary->error_set_last_error(portLibrary, errno,
                                        OMRPORT_ERROR_FILE_LOCK_BADLOCK);
    }
    Trc_PRT_file_lock_bytes_unix_exiting();
    return -1;
  }
...
/**
 * This function will release the lock on the given file, starting at offset
 * bytes from the start of the file and continuing for length bytes
 *
 * @param [in]   portLibrary            The port library
 * @param [in]   fd                              The file descriptor/handle of
 * the file to be locked
...
int32_t omrfile_unlock_bytes(struct OMRPortLibrary *portLibrary, intptr_t fd,
  ...
  if (rc == -1) {
    ...
    portLibrary->error_set_last_error(portLibrary, errno,
                                      OMRPORT_ERROR_FILE_UNLOCK_BADUNLOCK);
...
int32_t omrfile_fstat(struct OMRPortLibrary *portLibrary, intptr_t fd,
  ...
  PlatformStatfs statfsbuf;
#endif /* (!defined(OMRZTPF) && defined(LINUX)) || defined(AIXPPC) || \
          defined(OSX) */
  int32_t rc = 0;
  int localfd = (int)fd;
  ...
  if (0 != fstat(localfd - FD_BIAS, &statbuf)) {
    ...
    setPortableError(portLibrary, fileFStatErrorMsgPrefix,
                     OMRPORT_ERROR_FILE_FSTAT_ERROR, myerror);
    rc = -1;
    goto _end;
  }
  ...
  if (0 != fstatfs(localfd - FD_BIAS, &statfsbuf)) {
    ...
    setPortableError(portLibrary, fileFStatFSErrorMsgPrefix,
                     OMRPORT_ERROR_FILE_FSTATFS_ERROR, myerror);
    rc = -1;
    goto _end;
  }
  ...
  if (0 != fstatvfs(localfd - FD_BIAS, &statfsbuf)) {
    ...
    setPortableError(portLibrary, fileFStatVFSErrorMsgPrefix,
                     OMRPORT_ERROR_FILE_FSTATVFS_ERROR, myerror);
    rc = -1;
    goto _end;
  }
  updateJ9FileStat(portLibrary, buf, &statbuf, &statfsbuf);
...
int32_t omrfile_stat_filesystem(struct OMRPortLibrary *portLibrary,
  ...
  buf->totalSizeBytes =
      (uint64_t)statvfsbuf.f_bsize * (uint64_t)statvfsbuf.f_blocks;
  if (0 == omrsysinfo_get_euid(portLibrary)) {
    /* superuser */
    buf->freeSizeBytes =
        (uint64_t)statvfsbuf.f_bsize * (uint64_t)statvfsbuf.f_bfree;
  } else {
    /* non-superuser */
    buf->freeSizeBytes =
        (uint64_t)statvfsbuf.f_bsize * (uint64_t)statvfsbuf.f_bavail;
  }
