  ...
  if (0 == stat(path, &buffer)) {
    if (S_ISDIR(buffer.st_mode)) {
      ...
      errMsg = portLibrary->nls_lookup_message(
        portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_FILE_OPEN_FILE_IS_DIR, NULL);
 ...
 */
intptr_t omrfile_write(struct OMRPortLibrary* portLibrary, intptr_t inFD, const void* buf, intptr_t nbytes)
{
  int fd = (int)inFD;
  intptr_t rc;
 ...
 */
intptr_t omrfile_read(struct OMRPortLibrary* portLibrary, intptr_t inFD, void* buf, intptr_t nbytes)
{
  int fd = (int)inFD;
  intptr_t result;
 ...
 */
int64_t omrfile_seek(struct OMRPortLibrary* portLibrary, intptr_t inFD, int64_t offset, int32_t whence)
{
  int fd = (int)inFD;
 ...
 */
int32_t omrfile_set_length(struct OMRPortLibrary* portLibrary, intptr_t inFD, int64_t newLength)
{
  int fd = (int)inFD;
  int32_t rc;
  ...
  struct OMRPortLibrary* portLibrary, intptr_t fd, int32_t lockFlags, uint64_t offset, uint64_t length)
{
  ...
  if (!(lockFlags & OMRPORT_FILE_READ_LOCK) && !(lockFlags & OMRPORT_FILE_WRITE_LOCK)) {
    ...
    errMsg = portLibrary->nls_lookup_message(
      portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_FILE_LOCK_INVALID_FLAG, NULL);
