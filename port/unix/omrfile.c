...
static void
updateJ9FileStat(struct OMRPortLibrary* portLibrary,
                   ...
                   uint64_t length)
{
  ...
  Trc_PRT_file_lock_bytes_unix_callingFcntl(fcntlCmd,
                                            fcntlFlock.l_type,
                                            fcntlFlock.l_whence,
                                            fcntlFlock.l_start,
                                            fcntlFlock.l_len);
  ...
  if (rc == -1) {
    ...
    if (errno == EDEADLK) {
      portLibrary->error_set_last_error(
        portLibrary, errno, OMRPORT_ERROR_FILE_LOCK_EDEADLK);
    } else {
      portLibrary->error_set_last_error(
        portLibrary, errno, OMRPORT_ERROR_FILE_LOCK_BADLOCK);
    }
    Trc_PRT_file_lock_bytes_unix_exiting();
    return -1;
  }
                     ...
                     uint64_t length)
{
  ...
  fcntlFlock.l_start = offset;
  fcntlFlock.l_len = length;
  Trc_PRT_file_unlock_bytes_unix_callingFcntl(fcntlCmd,
                                              fcntlFlock.l_type,
                                              fcntlFlock.l_whence,
                                              fcntlFlock.l_start,
                                              fcntlFlock.l_len);
  ...
  if (rc == -1) {
    ...
    portLibrary->error_set_last_error(
      portLibrary, errno, OMRPORT_ERROR_FILE_UNLOCK_BADUNLOCK);
              ...
              struct J9FileStat* buf)
{
  ...
  if (0 != fstat(localfd - FD_BIAS, &statbuf)) {
    ...
    setPortableError(portLibrary,
                     fileFStatErrorMsgPrefix,
                     OMRPORT_ERROR_FILE_FSTAT_ERROR,
                     myerror);
    rc = -1;
    goto _end;
  }
  ...
  if (0 != fstatfs(localfd - FD_BIAS, &statfsbuf)) {
    ...
    setPortableError(portLibrary,
                     fileFStatFSErrorMsgPrefix,
                     OMRPORT_ERROR_FILE_FSTATFS_ERROR,
                     myerror);
    rc = -1;
    goto _end;
  }
  ...
  if (0 != fstatvfs(localfd - FD_BIAS, &statfsbuf)) {
    ...
    setPortableError(portLibrary,
                     fileFStatVFSErrorMsgPrefix,
                     OMRPORT_ERROR_FILE_FSTATVFS_ERROR,
                     myerror);
    rc = -1;
    goto _end;
  }
