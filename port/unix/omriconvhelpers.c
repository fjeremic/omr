 ...
 */
static BOOLEAN
openIconvDescriptor(iconv_t* globalConverter,
...
iconv_global_init(struct OMRPortLibrary* portLibrary)
{
  ...
  success = openIconvDescriptor(globalConverter,
                                globalConverterMutex,
                                OMRPORT_UTF8_TO_EBCDIC_ICONV_DESCRIPTOR,
                                ebcdic,
                                utf8,
                                &initializedConvertors) &&
            openIconvDescriptor(globalConverter,
                                globalConverterMutex,
  ...
  if (success) {
    ...
    for (iconvIndex = OMRPORT_FIRST_ICONV_DESCRIPTOR;
         iconvIndex < UNCACHED_ICONV_DESCRIPTOR;
         ++iconvIndex) {
 ...
 */
iconv_t
iconv_get(struct OMRPortLibrary* portLibrary,
...
void
iconv_free(struct OMRPortLibrary* portLibrary,
           J9IconvName converterName,
           iconv_t converter)
{
#if defined(J9ZOS390)
