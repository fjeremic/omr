 ...
 */
static BOOLEAN openIconvDescriptor(iconv_t *globalConverter,
                                   MUTEX *globalConverterMutex, uint32_t index,
                                   const char *toCode, const char *fromCode,
                                   uint32_t *initializedConvertors) {
 ...
 * The cached converters are serialized via their respective mutex. The mutex is
 * freed in iconv_free() Populate the converter cache within portLibGlobal
 * struct and their respective mutexes.
 *
...
int32_t iconv_global_init(struct OMRPortLibrary *portLibrary) {
  ...
  BOOLEAN success = TRUE;
  /* NOTE: using original nl_langinfo implementation that returns an EBCDIC
   * string by defining J9_USE_ORIG_EBCDIC_LANGINFO which is checked in
   * a2e/headers/langinfo.h
   */
  char *langinfo = nl_langinfo(CODESET);

  Assert_PRT_true(UNCACHED_ICONV_DESCRIPTOR <=
                  32); /* ensure we have the right number of bits */
  success = openIconvDescriptor(globalConverter, globalConverterMutex,
                                OMRPORT_UTF8_TO_EBCDIC_ICONV_DESCRIPTOR, ebcdic,
                                utf8, &initializedConvertors) &&
            openIconvDescriptor(globalConverter, globalConverterMutex,
                                OMRPORT_EBCDIC_TO_UTF8_ICONV_DESCRIPTOR, utf8,
                                ebcdic, &initializedConvertors) &&
            openIconvDescriptor(globalConverter, globalConverterMutex,
                                OMRPORT_UTF8_TO_LANG_ICONV_DESCRIPTOR, langinfo,
                                utf8, &initializedConvertors) &&
            openIconvDescriptor(globalConverter, globalConverterMutex,
                                OMRPORT_LANG_TO_UTF8_ICONV_DESCRIPTOR, utf8,
                                langinfo, &initializedConvertors) &&
            openIconvDescriptor(globalConverter, globalConverterMutex,
                                OMRPORT_UTF16_TO_LANG_ICONV_DESCRIPTOR,
                                langinfo, utf16, &initializedConvertors) &&
            openIconvDescriptor(globalConverter, globalConverterMutex,
                                OMRPORT_LANG_TO_UTF16_ICONV_DESCRIPTOR, utf16,
                                langinfo, &initializedConvertors);
  ...
  if (success) {
    ...
    for (iconvIndex = OMRPORT_FIRST_ICONV_DESCRIPTOR;
         iconvIndex < UNCACHED_ICONV_DESCRIPTOR; ++iconvIndex) {
 ...
 * @internal
 *
 * Allocates a iconv descriptor that converts character sequences from the
 *fromCode character encoding to the toCode character encoding.  The iconv
 *descriptor may be stored in a per-thread cache.  Caching is determined by
 *available memory and the value of the converterName argument:
 *		cache(converterName) = converterName < UNCACHED_ICONV_DESCRIPTOR
 *
 * @param[in] converterName A J9IconvName indicating the intended use of the
 *iconv descriptor.
 * @param[in] toCode		The target character encoding.
 * @param[in] fromCode		The source character encoding.
 *
 * @return J9VM_INVALID_ICONV_DESCRIPTOR on failure, otherwise a valid iconv
 *descriptor.
 */
iconv_t iconv_get(struct OMRPortLibrary *portLibrary, J9IconvName converterName,
                  const char *toCode, const char *fromCode) {
  /* ptBuffer is NULL if the iconv descriptor should not be cached or the
   * per-thread buffer could not be allocated. We attempt to allocate an
   * uncached iconv descriptor in those cases, and use ptBuffer == NULL as a
   * check for caching.
   */
  ...
  if (PPG_global_converter_enabled) {
    ...
    /* this part is only pertinent if -Xipt argument is not specified. It is a
     * hack for CICS the logic is based on the toCode and fromCode. If it
     * correspond to one of the four converters opened, then per-thread cache is
     * not consulted.
     */
    ...
    if (strcmp(fromCode, utf8) == 0) {
      ...
      if (strcmp(toCode, utf8) ==
          0) { /* requesting current codeset -> UTF8 converter */
        index = OMRPORT_LANG_TO_UTF8_ICONV_DESCRIPTOR;
      } else if (strcmp(toCode, utf16) ==
                 0) { /* requesting current codeset -> UTF16 converter */
...
void iconv_free(struct OMRPortLibrary *portLibrary, J9IconvName converterName,
                iconv_t converter) {
#if defined(J9ZOS390)
  if (PPG_global_converter_enabled) {
    iconv_t *globalConverter = PPG_global_converter;
    ...
    int32_t index = UNCACHED_ICONV_DESCRIPTOR;
    /* this part is only pertinent if -Xipt argument is not specified. It is a
     * hack for CICS the logic is based on comparing the converter to be freed
     * to the existing converter, if a match is found within the cached
     * converters, only the corresponding mutex is exited
     */
  ...
  if (converterName < UNCACHED_ICONV_DESCRIPTOR) {
    PortlibPTBuffers_t ptBuffer =
        (PortlibPTBuffers_t)omrport_tls_get(portLibrary);
