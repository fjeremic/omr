...
struct
{
  ...
  int unixPermission;
} permissionsMap[] = { { OMRPORT_PAGE_PROTECT_NONE, PROT_NONE },
 ...
 */
intptr_t
protect_memory(struct OMRPortLibrary* portLibrary,
               void* address,
               uintptr_t length,
               uintptr_t flags)
{
  ...
  for (index = 0; index < sizeof(permissionsMap) / sizeof(permissionsMap[0]);
       index++) {
  ...
  if (rc != 0) {
    portLibrary->error_set_last_error(
      portLibrary, errno, OMRPORT_PAGE_PROTECT_FAILED);
