...
void *omrmem_allocate_memory_basic(struct OMRPortLibrary *portLibrary,
  ...
  Assert_AddressAbove4GBBar((NULL == retval) ||
                            (0x100000000 <= ((uintptr_t)retval)));
  return retval;
...
void omrmem_free_memory_basic(struct OMRPortLibrary *portLibrary,
                              void *memoryPointer) {
  free(memoryPointer);
}

void omrmem_advise_and_free_memory_basic(struct OMRPortLibrary *portLibrary,
                                         void *memoryPointer,
  ...
  /*  retrieve size of the page backing portLibrary->portGlobals which is
   * allocated using omrmem_allocate_memory() in port/unix/omrmem.c
   */
  ...
  Trc_PRT_mem_advise_and_free_memory_basic_params(pageSize, memoryPointer,
                                                  memorySize);
  ...
  if (pageSize > 0 && memorySize >= pageSize) {
    ...
    if (memPtrSizePageRounded >= pageSize) {
      ...
      Trc_PRT_mem_advise_and_free_memory_basic_oscall(memPtrPageRounded,
                                                      memPtrSizePageRounded);

#if (defined(LINUX) && !defined(OMRZTPF)) || defined(OSX)
      if (-1 == madvise((void *)memPtrPageRounded, memPtrSizePageRounded,
                        MADV_DONTNEED)) {
        Trc_PRT_mem_advise_and_free_memory_basic_madvise_failed(
            (void *)memPtrPageRounded, memPtrSizePageRounded, errno);
      }
#elif defined(AIXPPC)
      if (-1 == disclaim64((void *)memPtrPageRounded, memPtrSizePageRounded,
                           DISCLAIM_ZEROMEM)) {
...
#if defined(OMR_ENV_DATA64)
      if (omrdiscard_data((void *)memPtrPageRounded,
                          memPtrSizePageRounded >> ZOS_REAL_FRAME_SIZE_SHIFT) !=
          0) {
...
#else
      if (Pgser_Release((void *)memPtrPageRounded, memPtrSizePageRounded) !=
          0) {
...
void *omrmem_reallocate_memory_basic(struct OMRPortLibrary *portLibrary,
                                     void *memoryPointer,
                                     uintptr_t byteAmount) {
#if (defined(S390) || defined(J9ZOS390)) && !defined(OMR_ENV_DATA64)
...
void omrmem_startup_basic(struct OMRPortLibrary *portLibrary,
  ...
  portLibrary->portGlobals =
      omrmem_allocate_memory_basic(portLibrary, portGlobalSize);
  if (NULL == portLibrary->portGlobals) {
    return;
  }
  memset(portLibrary->portGlobals, 0, portGlobalSize);
#if defined(LINUX)
  portLibrary->portGlobals->procSelfMap =
      omrmem_allocate_memory_basic(portLibrary, J9OSDUMP_SIZE);
#endif /* defined(LINUX) */

#if defined(J9ZOS390) && defined(OMR_GC_COMPRESSED_POINTERS)
  /* 'disableEnsureCap32' is set using
   * omrport_control(OMRPORT_CTLDATA_NOSUBALLOC32BITMEM,...), or by setting the
   * J9_SUBALLOC_FOR_32 environment variable before starting the JVM.
   *
   * By default the sub allocator is enabled, and will not use __malloc31 on
   * z/OS.
   */
  portLibrary->portGlobals->disableEnsureCap32 = OMRPORT_ENABLE_ENSURE_CAP32;
  if (0 <= portLibrary->sysinfo_get_env(portLibrary, "J9_SUBALLOC_FOR_32",
                                        (void *)tmpbuff, 256)) {
    if ((0 == strncmp("FALSE", tmpbuff, 6)) ||
        (0 == strncmp("0", tmpbuff, 2))) {
      portLibrary->portGlobals->disableEnsureCap32 =
          OMRPORT_DISABLE_ENSURE_CAP32;
    }
  }
#endif
}

void *omrmem_allocate_portLibrary_basic(uintptr_t byteAmount) {
  return (void *)malloc(byteAmount);
}

void omrmem_deallocate_portLibrary_basic(void *memoryPointer) {
  free(memoryPointer);
}
