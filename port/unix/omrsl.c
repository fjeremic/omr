  ...
  if (!openExec && decorate) {
    ...
    if (NULL != p) {
      ...
      pathLength = portLibrary->str_printf(
        portLibrary, mangledName, (MAX_STRING_LENGTH + 1), "lib%s" PLATFORM_DLL_EXTENSION, name);
    }
    ...
    if (0 != rc) {
      ...
      if (NULL != dirSep) {
        ...
        if (MAX_STRING_LENGTH < (pathLength + strlen(openName) + 1)) {
          const char* error = portLibrary->nls_lookup_message(portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE,
...
static void getDLError(struct OMRPortLibrary* portLibrary, char* errBuf, uintptr_t bufLen)
{
  ...
  if ((NULL == error) || ('\0' == error[0])) {
    ...
    error = portLibrary->nls_lookup_message(
      portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_SL_UNKOWN_ERROR, "Unknown error");
