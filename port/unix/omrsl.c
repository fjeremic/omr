  ...
  if (0 != descriptor) {
    ...
    if (0 != result) {
      ...
      portLibrary->tty_printf(
        portLibrary,
        "dlclose() failed: return code: %d message: \"%s\" \n",
        result,
        errBuf);
    }
  }
  ...
  if ((NULL == handle) && !openExec) {
    ...
    if (0 != rc) {
      ...
      if (NULL != dirSep) {
        ...
        /* proceed only if buffer size can fit the new concatenated path (+1 for
         * NUL) */
        if (MAX_STRING_LENGTH < (pathLength + strlen(openName) + 1)) {
          const char* error = portLibrary->nls_lookup_message(
            portLibrary,
            J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE,
            J9NLS_PORT_SL_BUFFER_EXCEEDED_ERROR,
            "Insufficient buffer memory while attempting to load a shared "
            "library");
          strncpy(errBuf, error, MAX_ERR_BUF_LENGTH);
          errBuf[MAX_ERR_BUF_LENGTH - 1] =
            '\0'; /* If NLS message size exceeds buffer length */
