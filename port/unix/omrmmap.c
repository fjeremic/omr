                 ...
                 uint32_t categoryCode)
{
  ...
  if (0 == size) {
    ...
    if (-1 == fstat(file - FD_BIAS, &buf)) {
      ...
      portLibrary->error_set_last_error(
        portLibrary, errno, OMRPORT_ERROR_MMAP_MAP_FILE_STATFAILED);
      return NULL;
    }
    size = buf.st_size;
  }
  ...
  if (!(returnVal =
          (J9MmapHandle*)portLibrary->mem_allocate_memory(portLibrary,
                                                          sizeof(J9MmapHandle),
                                                          OMR_GET_CALLSITE(),
                                                          categoryCode))) {
              ...
              uint32_t flags)
{
  ...
  if ((flags & OMRPORT_MMAP_SYNC_WAIT) && (flags & OMRPORT_MMAP_SYNC_ASYNC)) {
    portLibrary->error_set_last_error(
      portLibrary, -1, OMRPORT_ERROR_MMAP_MSYNC_INVALIDFLAGS);
  ...
  if (rc == -1) {
    ...
    portLibrary->error_set_last_error(
      portLibrary, errno, OMRPORT_ERROR_MMAP_MSYNC_FAILED);
    return -1;
  }
  ...
  if (pageSize > 0 && length >= pageSize) {
    ...
    if (roundedLength >= pageSize) {
      ...
      if (-1 == madvise((void*)roundedStart, roundedLength, MADV_DONTNEED)) {
        Trc_PRT_mmap_dont_need_madvise_failed(
          (void*)roundedStart, roundedLength, errno);
      }
#elif defined(AIXPPC)
      ...
      if (-1 ==
        ...
        Trc_PRT_mmap_dont_need_disclaim64_failed(
          (void*)roundedStart, roundedLength, errno);
      }
