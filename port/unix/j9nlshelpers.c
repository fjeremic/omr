...
static char nls_tolower(char toConvert) {
  char lower;

#if !defined(J9ZOS390)
  lower = j9_ascii_tolower(toConvert);
#else
  /* check to see if it is an uppercase character, and if so, return the
   * corresponding lowercase char */
...
void nls_determine_locale(struct OMRPortLibrary *portLibrary) {
  ...
  /* Set locale, returns NULL in case locale data cannot be initialized. This
   * may indicate that the locale is not installed OR not selected & generated
   * (see /etc/locale.gen) */
  setlocale(LC_ALL, "");

#if defined(LINUX) || defined(OSX)
  lang = setlocale(LC_CTYPE, NULL);
  /* set lang for later usage, we cannot use the return of setlocale(LC_ALL, "")
   * as its not the correct interface to retrieve it (lang/region) */

  /*[PR 104520] Use LANG environment variable when locale cannot be obtained */
  if (!lang || !strcmp(lang, "C") || !strcmp(lang, "POSIX")) {
    result =
        omrsysinfo_get_env(portLibrary, "LANG", langProp, sizeof(langProp));
...
#if defined(J9ZOS390)
  if (NULL != lang) {
    /* CMVC 107274:  z/OS sometimes returns the HFS path to a "locale object" so
     * carve it up to make it look like the corresponding locale name */
...
#endif /* defined(LINUX) || defined(OSX) */
  if ((NULL != lang) && strcmp(lang, "POSIX") && strcmp(lang, "C") &&
      strcmp(lang, "UTF-8")) {
