  ...
  omrsig_handler_fn handler, void* handler_arg, uint32_t flags, uintptr_t* result)
{
  ...
  if (OMR_ARE_ANY_BITS_SET(flags, OMRPORT_SIG_FLAG_MAY_RETURN)) {
     ...
     * omrsig_protect. In this case, the current signal in tls will be pointing at a stale stack frame and signal: CMVC
     * 126838
     */
...
#endif /* defined(S390) && defined(LINUX) */
{
   ...
   * returns such that the transaction is executed in the failed state, causing the non-transactional failure path to be
   * taken (referred to as the "abort handler" in
...
#define MSR_TS_MASK 0x6U
  ...
  if (NULL != thisThread) {
    ...
    while (NULL != thisRecord) {
      if (OMR_ARE_ALL_BITS_SET(thisRecord->flags, portLibTypeFPEFilter)) {
         ...
         * OMRPORT_SIG_EXCEPTION_RETURN In this case we will remove it from the top after executing the siglongjmp */
  ...
  if ((OMRPORT_SIG_EXCEPTION_COOPERATIVE_SHUTDOWN == result)
     ...
     * Recovery Services (RRS) and terminate the enclave, or if we need to explicitly trigger an abend to force RRS and
     * termination.
     *
     ...
     *
     * 		Hardware signals - return from the signal handler. This is preferable as the signal we're handling is reported
     * in LE diagnostics as the cause of the crash. However, returning doesn't work for software signals.
     *
     * 		Software signals -  explicitly trigger an abend. Whereas this forces LE RRS in all cases,
     * 								LE diagnostics will report the JVM abend as the cause of the crash, which somewhat clouds the
     * issue.
     */
...
int32_t omrsig_set_options(struct OMRPortLibrary* portLibrary, uint32_t options)
{
  ...
  if (OMR_ARE_ANY_BITS_SET(
        options, OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS | OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_ASYNCHRONOUS)) {
