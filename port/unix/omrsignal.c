               ...
               uintptr_t* result)
{
  ...
  if (0 != flagsWithoutMasterHandlers) {
    ...
    rc = registerMasterHandlers(
      portLibrary, flags, OMRPORT_SIG_FLAG_SIGALLSYNC, NULL);
                                ...
                                uint32_t flags)
{
  ...
  if (OMR_ARE_ANY_BITS_SET(signalOptionsGlobal,
    ...
    if (OMR_ARE_ALL_BITS_SET(flags, OMRPORT_SIG_FLAG_SIGXFSZ) &&
      ...
      rc = registerMasterHandlers(portLibrary,
    ...
    rc = registerMasterHandlers(
      portLibrary, flags, OMRPORT_SIG_FLAG_SIGALLASYNC, NULL);
  }
  omrthread_monitor_exit(registerHandlerMonitor);
                           ...
                           void** oldOSHandler)
{
  ...
  if ((0 == portlibSignalFlag) ||
    ...
    omrthread_monitor_enter(registerHandlerMonitor);
    rc = registerSignalHandlerWithOS(portLibrary,
                                     portlibSignalFlag,
                                     (unix_sigaction)newOSHandler,
                                     oldOSHandler);
    omrthread_monitor_exit(registerHandlerMonitor);
  }
  ...
  if (NULL != oldOSHandler) {
    ...
    Trc_PRT_signal_omrsig_register_os_handler_exiting(
      rc, portlibSignalFlag, newOSHandler, NULL);
  }
  ...
  if ((OMRPORT_SIG_EXCEPTION_COOPERATIVE_SHUTDOWN == result) ||
    ...
    if (TRUE == triggerAbend) {
      sigrelse(SIGABND); /* CMVC 191934: need to unblock sigabnd before issuing
                            the abend call */
#if defined(OMR_ENV_DATA64)
      __cabend(PORT_ABEND_CODE,
               PORT_ABEND_REASON_CODE,
...
initializeSignalTools(OMRPortLibrary* portLibrary)
{
  ...
  portLibrary->str_printf(
    portLibrary, semNames[0], 128, "/omr/wakeUpASyncReporter-%d", getpid());
