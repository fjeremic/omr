 ...
 *
 * syncSignalsWithHandlers represents if handlers are registered with
 * synchronous signals, and asyncSignalsWithHandlers represents if handlers are
...
#define SIGSEM_INIT(_sem, _name) \
  ((_sem) = sem_open((_name), O_CREAT | O_EXCL, S_IRWXU, 0))
 ...
 * being present for the masterASyncSignalHandler, so use pthread
 * synchronization. Note: the use of pthread functions is allowed on z/OS
 */
...
static struct {
  ...
  int unixSignalNo;
} signalMap[] = {{OMRPORT_SIG_FLAG_SIGSEGV, SIGSEGV},
...
static int32_t registerSignalHandlerWithOS(OMRPortLibrary *portLibrary,
...
static void masterASynchSignalHandler(int signal, siginfo_t *sigInfo,
                                      void *contextInfo, uintptr_t nullArg);
#else  /* defined(S390) && defined(LINUX) */
static void masterSynchSignalHandler(int signal, siginfo_t *sigInfo,
                                     void *contextInfo);
static void masterASynchSignalHandler(int signal, siginfo_t *sigInfo,
...
static BOOLEAN checkForAmbiguousSignalFlags(uint32_t flags,
...
int32_t omrsig_protect(struct OMRPortLibrary *portLibrary,
  ...
  uint32_t flagsWithoutMasterHandlers =
      (flagsSignalsOnly & ~syncSignalsWithMasterHandlers) &
  ...
  if (0 != flagsWithoutMasterHandlers) {
    uint32_t rc = 0;
    /* Acquire the registerHandlerMonitor and install the handler via
...
int32_t omrsig_set_async_signal_handler(struct OMRPortLibrary *portLibrary,
  ...
  if (OMR_ARE_ANY_BITS_SET(signalOptionsGlobal,
    ...
    /* -Xrs was set, we can't protect against any signals, do not install any
     * handlers except SIGXFSZ */
    if (OMR_ARE_ALL_BITS_SET(flags, OMRPORT_SIG_FLAG_SIGXFSZ) &&
      ...
      rc = registerMasterHandlers(portLibrary, OMRPORT_SIG_FLAG_SIGXFSZ,
                                  OMRPORT_SIG_FLAG_SIGALLASYNC, NULL);
    } else {
...
int32_t omrsig_set_single_async_signal_handler(
  ...
  Trc_PRT_signal_omrsig_set_single_async_signal_handler_entered(
      handler, handler_arg, portlibSignalFlag);

  if (0 != portlibSignalFlag) {
    /* For non-zero portlibSignalFlag, check if only one signal bit is set.
     * Otherwise, fail. */
    ...
    if (checkForAmbiguousSignalFlags(
            portlibSignalFlag, "omrsig_set_single_async_signal_handler")) {
      return OMRPORT_SIG_ERROR;
    }
  }
  ...
  if (OMR_ARE_ANY_BITS_SET(signalOptionsGlobal,
    ...
    if (OMR_ARE_ALL_BITS_SET(portlibSignalFlag, OMRPORT_SIG_FLAG_SIGXFSZ) &&
        OMR_ARE_ANY_BITS_SET(signalOptionsGlobal,
                             OMRPORT_SIG_OPTIONS_SIGXFSZ)) {
      rc = registerMasterHandlers(portLibrary, OMRPORT_SIG_FLAG_SIGXFSZ,
                                  OMRPORT_SIG_FLAG_SIGALLASYNC, oldOSHandler);
    } else {
    ...
    rc = registerMasterHandlers(portLibrary, portlibSignalFlag,
                                OMRPORT_SIG_FLAG_SIGALLASYNC, oldOSHandler);
  }
  ...
  while (NULL != cursor) {
    if (cursor->portLib == portLibrary) {
      if ((cursor->handler == handler) &&
        ...
        /* Unset the portlibSignalFlag for other handlers. One signal must be
         * associated to only one handler. */
        cursor->flags &= ~portlibSignalFlag;
      }
    }
    previousLink = &cursor->next;
...
int32_t omrsig_register_os_handler(struct OMRPortLibrary *portLibrary,
  ...
  Trc_PRT_signal_omrsig_register_os_handler_entered(portlibSignalFlag,
                                                    newOSHandler);

  if ((0 == portlibSignalFlag) ||
      !OMR_IS_ONLY_ONE_BIT_SET(portlibSignalFlag &
                               ~OMRPORT_SIG_FLAG_CONTROL_BITS_MASK)) {
    /* If portlibSignalFlag is 0 or if portlibSignalFlag has multiple signal
     * bits set, then fail. */
    Trc_PRT_signal_omrsig_register_os_handler_invalid_portlibSignalFlag(
        portlibSignalFlag);
    rc = OMRPORT_SIG_ERROR;
  } else if (checkForAmbiguousSignalFlags(portlibSignalFlag,
                                          "omrsig_register_os_handler")) {
    return OMRPORT_SIG_ERROR;
  } else {
    omrthread_monitor_enter(registerHandlerMonitor);
    rc =
        registerSignalHandlerWithOS(portLibrary, portlibSignalFlag,
                                    (unix_sigaction)newOSHandler, oldOSHandler);
    omrthread_monitor_exit(registerHandlerMonitor);
  }

  if (NULL != oldOSHandler) {
    Trc_PRT_signal_omrsig_register_os_handler_exiting(
        rc, portlibSignalFlag, newOSHandler, *oldOSHandler);
  } else {
    Trc_PRT_signal_omrsig_register_os_handler_exiting(rc, portlibSignalFlag,
                                                      newOSHandler, NULL);
  }
...
int32_t omrsig_is_signal_ignored(struct OMRPortLibrary *portLibrary,
  ...
  *isSignalIgnored = FALSE;
  ...
  if (0 != portlibSignalFlag) {
    ...
    if (checkForAmbiguousSignalFlags(portlibSignalFlag,
 ...
 * call to either of omrsig_protect or omrsig_set_async_signal_handler
 */
int32_t omrsig_startup(struct OMRPortLibrary *portLibrary) {
  ...
  if (result == 0) {
    /* we have successfully started up the signal portion, install the full
     * shutdown routine */
...
#if defined(OMR_PORT_ASYNC_HANDLER)
/**
 * Given a port library signal flag and Unix signal value, execute the
 * associated handlers stored within asyncHandlerList (list of
 * OMRUnixAsyncHandlerRecord).
 *
...
static void runHandlers(uint32_t asyncSignalFlag, int unixSignal) {
  ...
  if (OMR_ARE_NO_BITS_SET(signalOptionsGlobal,
    ...
    /* mapPortLibSignalToOSSignal returns OMRPORT_SIG_ERROR (-1) on unknown
     * mapping */
...
static int J9THREAD_PROC asynchSignalReporter(void *userData) {
   ...
   * from being caught by this thread. We can't allow
   * masterASynchSignalHandler() to recursively lock wakeUpASyncReporterMutex
   * because it could cause this thread to miss signals on
   * wakeUpASyncReporterCond.
   *
   * It's ok if this thread caught a signal just before this, because it has
   * not locked wakeUpASyncReporterMutex yet.
   *
   * This limitation is unique to the z/OS implementation of async signal
   * handling.
   */
  {
    sigset_t asyncSigs;
 ...
 * It will call all of the user's handlers that were registered with the vm
 * using omrsig_protect, upon receiving a signal they listen for.
 *
 */
#if defined(S390) && defined(LINUX)
static void masterSynchSignalHandler(int signal, siginfo_t *sigInfo,
...
#if defined(LINUXPPC)
  /*
   * PR 56956: ensure the right register context is used if a signal occurs in a
   * transaction. If there is a signal during a transaction, two contexts are
   * provided: one for the start of the transaction and one for the context
   * within the transaction.
   *
   * On Power 8 Linux, if a signal occurs during a transaction,
   * the transaction aborts and the corresponding signal handler is invoked with
   * the context from the start of the transaction. The masterSynchSignalHandler
   * determines if the signal occurred within a transaction, and if so, returns
   * such that the transaction is executed in the failed state, causing the
   * non-transactional failure path to be taken (referred to as the "abort
   * handler" in
   * https://www.kernel.org/doc/Documentation/powerpc/transactional_memory.txt).
   * Note: this means signals that occur during the transaction, but not in the
   * failure path, are lost. Please see Jazz design 63039: Handle signals in
   * PowerPC transactional memory operations for further discussion.
   */

  /*
   * This contains two contexts: the context at the start of the transaction,
   * plus a link to the context within the transaction.  We use the former,
   * other than to determine the transaction state.
   */
...
#define MSR_TS_MASK 0x6U
  ...
  if ((NULL != platformContext->uc_link) &&
      OMR_ARE_ANY_BITS_SET(platformContext->uc_link->uc_mcontext.regs->msr,
                           MSR_TS_MASK))
#endif /* defined(LINUXPPC64) */
  {
    /*
     * resume the transaction in the failed state, so it executes the failure
     * path.
     */
    return;
  }
  ...
  if (NULL != thisThread) {
     ...
     * bit-mask. The received signal can be a variant of SIGFPE: DIV_BY_ZERO,
     * INT_DIV_BY_ZERO or INT_OVERFLOW. This will handle all the SIGFPE variants
     * if thisRecord->flags has OMRPORT_SIG_FLAG_SIGFPE set.
     */
    ...
    /* record this signal in tls so that omrsig_handler can be called if any of
...
#if defined(J9ZOS390)
          fillInJumpInfo(thisRecord->portLibrary, contextInfo,
                         &(thisRecord->farJumpInfo));
  ...
  if ((OMRPORT_SIG_EXCEPTION_COOPERATIVE_SHUTDOWN == result) ||
      (OMR_ARE_ALL_BITS_SET(signalOptionsGlobal,
                            OMRPORT_SIG_OPTIONS_COOPERATIVE_SHUTDOWN))) {
    /* Determine if returning from the signal handler will result in Language
     * Environment (LE) initiating Resource Recovery Services (RRS) and
     * terminate the enclave, or if we need to explicitly trigger an abend to
     * force RRS and termination.
     *
     * RRS is critical to backing out database multi-phase commit transactions
     * that were interrupted before completion
     *
     * In general, the rules are:
     *
     * 		Hardware signals - return from the signal handler. This is preferable
     * as the signal we're handling is reported in LE diagnostics as the cause
     * of the crash. However, returning doesn't work for software signals.
     *
     * 		Software signals -  explicitly trigger an abend. Whereas this forces LE
     * RRS in all cases, LE diagnostics will report the JVM abend as the cause
     * of the crash, which somewhat clouds the issue.
     */

    /* Default to explicitly triggering the abend, as this is guaranteed to
     * trigger RRS */
...
#endif /* defined(OMR_ENV_DATA64) */
    {
      ...
      /* check the abend and program check flags, if they are set, this is a
       * "real" hardware signal */
      if ((1 == conditionInfoBlock->cib_abf) ||
          (1 == conditionInfoBlock->cib_pcf)) {
    ...
    if (TRUE == triggerAbend) {
      sigrelse(SIGABND); /* CMVC 191934: need to unblock sigabnd before issuing
                            the abend call */
#if defined(OMR_ENV_DATA64)
    ...
    /* we now know that returning from the signal handler will result in LE
     * initiating RRS and terminate the enclave */
    return;
  }
  ...
  /* The only way to get here is if (1) this thread was not attached to the
   * thread library or (2) the thread hadn't registered any signal handlers with
   ...
   * to with this signal default action is to abort */
#if defined(J9ZOS390)
  if (SIGABND != signal) {
 ...
 *
 * During first registration, the old action for the signal handler is stored in
 * oldActions. The original OS handler must be restored before the portlibrary
 * is shut down. For subsequent registrations, old action is not stored in
 * oldActions in order to avoid overwriting the original OS handler. Instead, a
 * local sigaction variable is used to store old action for subsequent
 * registrations. oldOSHandler is updated to point to oldAction.sa_sigaction
...
static int32_t registerSignalHandlerWithOS(OMRPortLibrary *portLibrary,
   ...
   * members of struct sigaction. Using the former allows us to access more than
   * just the signal number.
   */
   ...
   * the sa_mask field, as done on z/OS.
   */
  newAction.sa_flags |= SA_NODEFER;

#if defined(J9ZOS390)
  /* z/OS doesn't have POSIX semaphores. As a precaution, re-entering the
   * masterASyncHandler must be avoided. Therefore, all the asynchronous signals
   * are masked for the masterASyncHandler. The signal(s) are queued and
   * delivered to the masterASyncHandler once the handler returns. No signals
   * are lost.
   */
  if (OMR_ARE_ALL_BITS_SET(OMRPORT_SIG_FLAG_SIGALLASYNC, portLibrarySignalNo)) {
...
#if defined(AIXPPC)
  /* Do the following while installing a handler for an asynchronous signal
   * block SIGTRAP. */
  if (OMR_ARE_ALL_BITS_SET(OMRPORT_SIG_FLAG_SIGALLASYNC, portLibrarySignalNo)) {
   ...
   * "uintptr_t breakingEventAddr" has been introduced in
   * masterSynchSignalHandler() to obtain BEA on zLinux but not for other
   * platforms. As a result, the total number of parameters in
   * masterSynchSignalHandler() on zLinux is 4 while the number is 3 on other
   * platforms, because there is no need to change the signature of
   * masterSynchSignalHandler in there. Since the code is shared on all
   * platforms, the change here is used to split them up to avoid any compiling
   * error.
   */
#if defined(S390) && defined(LINUX)
  ...
  /* After setting up the sigaction struct, register the handler with the OS.
   * When registering the handler for the first time, the old OS handler is
   * stored in the oldActions[unixSignalNo].action. This way the original OS
   * handler can be restored during shutdown. For subsequent registrations, the
   * oldActions[unixSignalNo].action is not updated since the original OS
   * handler would be overwritten. Instead, a local sigaction variable is used
  ...
  if (0 == oldActions[unixSignalNo].restore) {
    ...
    if (OMRSIG_SIGACTION(unixSignalNo, &newAction,
      ...
      Trc_PRT_signal_registerSignalHandlerWithOS_registeredHandler1(
          portLibrarySignalNo, unixSignalNo, handler,
          oldActions[unixSignalNo].action.sa_sigaction);
  ...
  if ((handler == masterSynchSignalHandler) ||
 ...
 *
 * @return The corresponding Unix signal number or OMRPORT_SIG_ERROR (-1) if the
 * portLibSignal could not be mapped
 */
static int mapPortLibSignalToOSSignal(uint32_t portLibSignal) {
  uint32_t index = 0;
 ...
 *
 * @param ss Signal set to be modified. Signals already in the set are not
 * removed.
...
static intptr_t addAsyncSignalsToSet(sigset_t *ss) {
  ...
  for (i = 0; i < sizeof(signalMap) / sizeof(signalMap[0]); i++) {
    if (OMR_ARE_ALL_BITS_SET(OMRPORT_SIG_FLAG_SIGALLASYNC,
                             signalMap[i].portLibSignalNo)) {
 ...
 * registered with a signal, then a master handler isn't registered again for
 *that signal.
 *
 * Calls to this function must be synchronized using registerHandlerMonitor.
 *
 * @param[in] flags the flags that we want signals for
 * @param[in] allowedSubsetOfFlags must be one of OMRPORT_SIG_FLAG_SIGALLSYNC,
 *or OMRPORT_SIG_FLAG_SIGALLASYNC
...
static int32_t registerMasterHandlers(OMRPortLibrary *portLibrary,
  ...
  uint32_t signalFlags =
      (flags & allowedSubsetOfFlags) & ~OMRPORT_SIG_FLAG_CONTROL_BITS_MASK;
  ...
  if (signalType == allowedSignalType) {
    while (0 != signalFlags) {
      ...
      if (!OMR_ARE_ALL_BITS_SET(signalsWithMasterHandlersLocal,
                                portSignalFlagWithType)) {
        /* Register a master handler for this (portSignalFlagWithType's) signal.
         */
        if (0 != registerSignalHandlerWithOS(portLibrary,
                                             portSignalFlagWithType, handler,
                                             oldOSHandler)) {
          return OMRPORT_SIG_ERROR;
        }
      } else {
        /* If the master handler is already registered, then the oldOSHandler
         * must represent the master handler.
         */
        if (NULL != oldOSHandler) {
...
static int32_t initializeSignalTools(OMRPortLibrary *portLibrary) {
  ...
  /* use this to record the last signal that occurred such that we can call
  ...
  if (omrthread_monitor_init_with_name(
          &registerHandlerMonitor, 0,
          "portLibrary_omrsig_registerHandler_monitor")) {
    return -1;
  }
...
#if !defined(J9ZOS390)
#if defined(OSX)
  /* OSX only has named semaphores. They are not shared across processes, so
   * unlink immediately. */
  portLibrary->str_printf(portLibrary, semNames[0], 128,
                          "/omr/wakeUpASyncReporter-%d", getpid());
...
int32_t omrsig_set_options(struct OMRPortLibrary *portLibrary,
  ...
  if (OMR_ARE_ANY_BITS_SET(
          options, OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS |
                       OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_ASYNCHRONOUS)) {
  ...
  if (OMR_ARE_ANY_BITS_SET(options, OMRPORT_SIG_OPTIONS_ZOS_USE_CEEHDLR)) {
    /* Received notification to use LE condition handling. Switch over to LE
     * condition handling, unless POSIX handlers are installed for any
     * synchronous signals.
     */
    ...
    if (OMR_ARE_NO_BITS_SET(syncSignalsWithHandlers,
                            OMRPORT_SIG_FLAG_SIGALLSYNC &
                                ~OMRPORT_SIG_FLAG_CONTROL_BITS_MASK)) {
      /* No synchronous handlers are installed; so, it is fine to switch to LE
       * condition handling. */
 ...
 */
static void unsetBitMaskSignalsWithHandlers(uint32_t flags) {
  /* When unsetting the signal flags from the bit-mask, the control bits are not
   * unset because the control bits are shared between all the port library
   * signal flags. Also, this simplifies the checks to see if a signal flag is
   * set in the bit-mask.
   */
  if (OMR_ARE_ALL_BITS_SET(flags, OMRPORT_SIG_FLAG_IS_SYNC)) {
...
/**
 * Unset the port library signal flags in either syncSignalsWithMasterHandlers
 * or asyncSignalsWithMasterHandlers depending on whether the input is a set of
 * synchronous or asynchronous signals.
 *
 * @param[in] flags the port library signal flags
 *
...
static void unsetBitMaskSignalsWithMasterHandlers(uint32_t flags) {
   ...
   */
  if (OMR_ARE_ALL_BITS_SET(flags, OMRPORT_SIG_FLAG_IS_SYNC)) {
    syncSignalsWithMasterHandlers &=
        ~(flags & ~OMRPORT_SIG_FLAG_CONTROL_BITS_MASK);
  } else {
    asyncSignalsWithMasterHandlers &=
        ~(flags & ~OMRPORT_SIG_FLAG_CONTROL_BITS_MASK);
  }
}

/**
 * Check if a set of port library flags is ambiguous. It can either have
 * synchronous or asynchronous signals. It cannot have both types of signals. If
 * both OMRPORT_SIG_FLAG_IS_ASYNC and OMRPORT_SIG_FLAG_IS_SYNC are set, then the
 * set of signal flags is considered ambiguous. If none of the two signal
 * identifier flags are set, then the set of signal flags is also considered
 * ambiguous. flags=0 is valid in some cases, and it indicates cleanup/removal
 * of a handler.
 *
 * @param[in] flags the set of port library signal flags
 * @param[in] functionName the name of the function invoking
 * checkForAmbiguousSignalFlags
 *
...
static BOOLEAN checkForAmbiguousSignalFlags(uint32_t flags,
  ...
  if ((0 != flags) &&
    ...
    rc = TRUE;
    /* The tracepoint below is an exit and exception tracepoint. The calling
     * function should exit with an error code if an ambiguous signal is found,
     * instead of flowing through and executing another exit tracepoint.
     */
