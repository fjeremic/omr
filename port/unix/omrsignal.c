...
static struct {
    ...
    int unixSignalNo;
} signalMap[] = { { OMRPORT_SIG_FLAG_SIGSEGV, SIGSEGV }, { OMRPORT_SIG_FLAG_SIGBUS, SIGBUS },
    { OMRPORT_SIG_FLAG_SIGILL, SIGILL }, { OMRPORT_SIG_FLAG_SIGFPE, SIGFPE }, { OMRPORT_SIG_FLAG_SIGTRAP, SIGTRAP },
    { OMRPORT_SIG_FLAG_SIGQUIT, SIGQUIT }, { OMRPORT_SIG_FLAG_SIGABRT, SIGABRT }, { OMRPORT_SIG_FLAG_SIGTERM, SIGTERM },
    { OMRPORT_SIG_FLAG_SIGXFSZ, SIGXFSZ }, { OMRPORT_SIG_FLAG_SIGINT, SIGINT }, { OMRPORT_SIG_FLAG_SIGHUP, SIGHUP },
    { OMRPORT_SIG_FLAG_SIGCONT, SIGCONT }, { OMRPORT_SIG_FLAG_SIGWINCH, SIGWINCH },
    { OMRPORT_SIG_FLAG_SIGPIPE, SIGPIPE }, { OMRPORT_SIG_FLAG_SIGALRM, SIGALRM }, { OMRPORT_SIG_FLAG_SIGCHLD, SIGCHLD },
    { OMRPORT_SIG_FLAG_SIGTSTP, SIGTSTP }, { OMRPORT_SIG_FLAG_SIGUSR1, SIGUSR1 }, { OMRPORT_SIG_FLAG_SIGUSR2, SIGUSR2 },
    { OMRPORT_SIG_FLAG_SIGURG, SIGURG }, { OMRPORT_SIG_FLAG_SIGXCPU, SIGXCPU },
    ...
    omrsig_handler_fn handler, void* handler_arg, uint32_t flags, uintptr_t* result)
{
    ...
    uint32_t flagsWithoutMasterHandlers
        = (flagsSignalsOnly & ~syncSignalsWithMasterHandlers) & ~OMRPORT_SIG_FLAG_CONTROL_BITS_MASK;
    ...
    if (OMR_ARE_ANY_BITS_SET(flags, OMRPORT_SIG_FLAG_MAY_RETURN)) {
         ...
         * because any signals that may have occurred within the scope of this layer of protection would have been
         * handled by that point.
         *
         * The only scenario where this is of real concern, is if more than one signal was handled per call to
         * omrsig_protect. In this case, the current signal in tls will be pointing at a stale stack frame and signal:
         * CMVC 126838
         */
...
#endif /* defined(S390) && defined(LINUX) */
{
     ...
     * returns such that the transaction is executed in the failed state, causing the non-transactional failure path to
     * be taken (referred to as the "abort handler" in
...
#define MSR_TS_MASK 0x6U
    ...
    if (NULL != thisThread) {
        ...
        while (NULL != thisRecord) {
            if (OMR_ARE_ALL_BITS_SET(thisRecord->flags, portLibTypeFPEFilter)) {
                ...
                result
                    = thisRecord->handler(thisRecord->portLibrary, portLibType, &signalInfo, thisRecord->handler_arg);

                /* The only case in which we don't want the previous handler back on top is if it just returned
                 * OMRPORT_SIG_EXCEPTION_RETURN In this case we will remove it from the top after executing the
                 * siglongjmp */
    ...
    if ((OMRPORT_SIG_EXCEPTION_COOPERATIVE_SHUTDOWN == result)
        ...
        /* Determine if returning from the signal handler will result in Language Environment (LE) initiating Resource
         * Recovery Services (RRS) and terminate the enclave, or if we need to explicitly trigger an abend to force RRS
         * and termination.
         *
         ...
         *
         * 		Hardware signals - return from the signal handler. This is preferable as the signal we're handling
         * is reported in LE diagnostics as the cause of the crash. However, returning doesn't work for software
         * signals.
         *
         * 		Software signals -  explicitly trigger an abend. Whereas this forces LE RRS in all cases,
         * 								LE diagnostics will report the JVM abend as the cause of the crash,
         * which somewhat clouds the issue.
         */
...
#endif /* defined(OMR_ENV_DATA64) */
        {
            ...
            if ((1 == conditionInfoBlock->cib_abf) || (1 == conditionInfoBlock->cib_pcf)) {
                if ((0 != sigInfo->si_code) && (_SEGV_SOFTLIMIT != sigInfo->si_code)
                    && ((SIGSEGV == signal) || (SIGILL == signal) || (SIGFPE == signal) || (SIGABND == signal))) {
...
#if defined(OMR_ENV_DATA64)
            __cabend(
                PORT_ABEND_CODE, PORT_ABEND_REASON_CODE, PORT_ABEND_CLEANUP_CODE /* normal termination processing */);
#else /* defined(OMR_ENV_DATA64) */
            {
    ...
    /* The only way to get here is if (1) this thread was not attached to the thread library or (2) the thread hadn't
     * registered any signal handlers with the port library that could handle the signal
     */
    ...
    if (0 == oldActions[unixSignalNo].restore) {
        ...
        if (OMRSIG_SIGACTION(unixSignalNo, &newAction, &oldActions[unixSignalNo].action)) {
            ...
            Trc_PRT_signal_registerSignalHandlerWithOS_registeredHandler1(
                portLibrarySignalNo, unixSignalNo, handler, oldActions[unixSignalNo].action.sa_sigaction);
...
int32_t omrsig_set_options(struct OMRPortLibrary* portLibrary, uint32_t options)
{
    ...
    if (OMR_ARE_ANY_BITS_SET(options, OMRPORT_SIG_OPTIONS_ZOS_USE_CEEHDLR)) {
        ...
        if (OMR_ARE_NO_BITS_SET(
                syncSignalsWithHandlers, OMRPORT_SIG_FLAG_SIGALLSYNC & ~OMRPORT_SIG_FLAG_CONTROL_BITS_MASK)) {
