...
static uintptr_t
getDirectoryOfLibrary(struct OMRPortLibrary* portLibrary,
                      char* buf,
                      uintptr_t bufLen);
...
isFileinLibPath(struct OMRPortLibrary* portLibrary, char* fileName)
{
  ...
  libPath = portLibrary->mem_allocate_memory(portLibrary,
                                             envLibPathLen,
                                             OMR_GET_CALLSITE(),
  ...
  while (NULL != path) {
    ...
    if (totalPathLen >= EsMaxPath) {
      tmpPath = portLibrary->mem_allocate_memory(portLibrary,
                                                 totalPathLen,
                                                 OMR_GET_CALLSITE(),
                                                 OMRMEM_CATEGORY_PORT_LIBRARY);
      if (NULL == tmpPath) {
    ...
    portLibrary->str_printf(
      portLibrary, pathBuf, totalPathLen, "%s/%s", path, fileName);
                          ...
                          uintptr_t flags)
{
  ...
  if (!openExec && decorate) {
    if (NULL != fileName) {
      ...
      pathLength = portLibrary->str_printf(portLibrary,
                                           mangledName,
                                           (EsMaxPath + 1),
                                           "lib%s" PLATFORM_DLL_EXTENSION,
                                           name);
    }
  ...
  if ((NULL == handle) && !openExec) {
    ...
    if ((suffixLength < sizeof(portLibDir)) &&
        (0 != getDirectoryOfLibrary(
                portLibrary, portLibDir, sizeof(portLibDir) - suffixLength))) {
      strcat(portLibDir, "/");
            ...
            if (NULL == handle) {
              ...
              if (NULL != fileName) {
                ...
                pathLength = portLibrary->str_printf(portLibrary,
                                                     ...
                                                     (EsMaxPath + 1),
                                                     "%.*s%s.srvpgm",
                                                     (uintptr_t)fileName + 1 -
                                                       (uintptr_t)name,
                                                     name,
                                                     fileName + 1);
              } else {
                pathLength = portLibrary->str_printf(
                  portLibrary, mangledName, (EsMaxPath + 1), "%s.srvpgm", name);
              }
              if (pathLength >= EsMaxPath) {
                result = OMRPORT_SL_UNSUPPORTED;
                goto exit;
    ...
    if (loadquery(L_GETMESSAGES, buffer, sizeof(buffer)) != -1) {
      ...
      if ((L_ERROR_NOLIB == atoi(buffer[0])) &&
        ...
        error = portLibrary->nls_lookup_message(
          ...
          NULL);
        portLibrary->str_printf(
          portLibrary, errBuf, bufLen, error, &buffer[1][3]);
        return;
      }
    }
 ...
 */
static uintptr_t
getDirectoryOfLibrary(struct OMRPortLibrary* portLib,
                      char* buf,
                      uintptr_t bufLen)
{
  ...
  linfoSize = 1024;
  linfo = portLib->mem_allocate_memory(
    portLib, linfoSize, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == linfo) {
    return 0;
  }
  for (;;) {
    ...
    linfop = portLib->mem_reallocate_memory(portLib,
                                            linfo,
                                            linfoSize,
                                            OMR_GET_CALLSITE(),
                                            OMRMEM_CATEGORY_PORT_LIBRARY);
                 ...
                 uintptr_t bufLen)
{
  ...
  converter = iconv_get(
    portLibrary, J9SL_ICONV_DESCRIPTOR, "UTF-8", nl_langinfo(CODESET));
