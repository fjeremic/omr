...
static uintptr_t isFileinLibPath(struct OMRPortLibrary* portLibrary, char* fileName)
{
  ...
  while (NULL != path) {
    ...
    if (totalPathLen >= EsMaxPath) {
      tmpPath
        = portLibrary->mem_allocate_memory(portLibrary, totalPathLen, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
      if (NULL == tmpPath) {
  ...
  struct OMRPortLibrary* portLibrary, char* name, uintptr_t* descriptor, uintptr_t flags)
{
  ...
  if (!openExec && decorate) {
    if (NULL != fileName) {
      ...
      pathLength
        = portLibrary->str_printf(portLibrary, mangledName, (EsMaxPath + 1), "lib%s" PLATFORM_DLL_EXTENSION, name);
    }
        ...
        if (decorate) {
          ...
          if (NULL == handle) {
            ...
            if (NULL == handle) {
              ...
              if (NULL != fileName) {
                /* the names specifies a path */
                pathLength = portLibrary->str_printf(portLibrary, mangledName, (EsMaxPath + 1), "%.*s%s.srvpgm",
                  (uintptr_t)fileName + 1 - (uintptr_t)name, name, fileName + 1);
              } else {
              ...
              if (NULL == handle) {
                ...
                if ((NULL != actualFileName) && (NULL != strchr(fileName, ')'))) {
                   ...
                   * eg archive(member).
                   * This translates into a file of name 'archive', to which dlopen will look to load 'member' since
                   * RTLD_MEMBER is specified.
                   */
                  ...
                  if (NULL == handle) {
                    ...
                    pathLength = portLibrary->str_printf(portLibrary, mangledName, (EsMaxPath + 1), "%.*s%.*s",
    ...
    if (loadquery(L_GETMESSAGES, buffer, sizeof(buffer)) != -1) {
      ...
      if ((L_ERROR_NOLIB == atoi(buffer[0])) && (L_ERROR_DEPENDENT == atoi(buffer[1]))) {
        error = portLibrary->nls_lookup_message(
          portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_SL_ERROR_LOADING_DEPENDANT_MODULE, NULL);
