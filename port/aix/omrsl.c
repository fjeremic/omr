 ...
 */
static int32_t VMINLINE isFile(struct OMRPortLibrary* portLibrary, const char* path)
{
    struct stat buffer;
 ...
 */
static uintptr_t isFileinLibPath(struct OMRPortLibrary* portLibrary, char* fileName)
{
    /*
    ...
    libPath = portLibrary->mem_allocate_memory(
        portLibrary, envLibPathLen, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
    ...
    while (NULL != path) {
        ...
        if (totalPathLen >= EsMaxPath) {
            tmpPath = portLibrary->mem_allocate_memory(
                portLibrary, totalPathLen, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
            if (NULL == tmpPath) {
...
/* This macro will seek out whether specified path is indeed a file in current folder or in any folders specified by
 * LIBPATH */
    ...
    if (!openExec && decorate) {
        if (NULL != fileName) {
            ...
            pathLength = portLibrary->str_printf(portLibrary, mangledName, (EsMaxPath + 1),
                "%.*slib%s" PLATFORM_DLL_EXTENSION, (uintptr_t)fileName + 1 - (uintptr_t)name, name, fileName + 1);
        } else {
            pathLength = portLibrary->str_printf(
                portLibrary, mangledName, (EsMaxPath + 1), "lib%s" PLATFORM_DLL_EXTENSION, name);
        }
        ...
        if (NULL == handle) {
            ...
            if (NULL == handle) {
                ...
                if (decorate) {
                    if (NULL != fileName) {
                        /* the names specifies a path */
                        pathLength = portLibrary->str_printf(portLibrary, mangledName, (EsMaxPath + 1), "%.*slib%s.a",
                    ...
                    if (NULL == handle) {
                        ...
                        if (NULL == handle) {
                            ...
                            if (NULL != fileName) {
                                /* the names specifies a path */
                                pathLength = portLibrary->str_printf(portLibrary, mangledName, (EsMaxPath + 1),
                                    "%.*s%s.srvpgm", (uintptr_t)fileName + 1 - (uintptr_t)name, name, fileName + 1);
                            } else {
                                pathLength = portLibrary->str_printf(
                            ...
                            if (NULL == handle) {
                                ...
                                if ((NULL != actualFileName) && (NULL != strchr(fileName, ')'))) {
                                     ...
                                     * In case where library name contains '(' and ')', we are specifying an archive
                                     * member. eg archive(member). This translates into a file of name 'archive', to
                                     * which dlopen will look to load 'member' since RTLD_MEMBER is specified.
                                     */
                                    ...
                                    if (NULL == handle) {
                                        ...
                                        pathLength = portLibrary->str_printf(portLibrary, mangledName, (EsMaxPath + 1),
                                            "%.*s%.*s", (uintptr_t)fileName + 1 - (uintptr_t)name, name,
...
static void getDLError(struct OMRPortLibrary* portLibrary, char* errBuf, uintptr_t bufLen)
{
    ...
    if (ENOENT == dlopen_errno) {
        ...
        error = portLibrary->nls_lookup_message(
            portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_SL_SYMBOL_RESOLUTION_FAILURE, NULL);
 ...
 */
static uintptr_t getDirectoryOfLibrary(struct OMRPortLibrary* portLib, char* buf, uintptr_t bufLen)
{
    ...
    for (;;) {
        ...
        linfop = portLib->mem_reallocate_memory(
            portLib, linfo, linfoSize, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
        if (NULL == linfop) {
