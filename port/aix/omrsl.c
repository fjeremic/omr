...
static void convertWithMBTOWC(struct OMRPortLibrary *portLibrary, char *error,
                              char *errBuf, uintptr_t bufLen);
#endif /* J9VM_USE_MBTOWC (autogen) */

#if (defined(J9VM_USE_ICONV)) /* priv. proto (autogen) */

static void convertWithIConv(struct OMRPortLibrary *portLibrary,
                             const char *error, char *errBuf, uintptr_t bufLen);
#endif /* J9VM_USE_ICONV (autogen) */

static void getDLError(struct OMRPortLibrary *portLibrary, char *errBuf,
                       uintptr_t bufLen);
static uintptr_t getDirectoryOfLibrary(struct OMRPortLibrary *portLibrary,
                                       char *buf, uintptr_t bufLen);

/**
 * Returns if the path specified is a file or a folder.
 * This function has the same logic as omrfile_attr(). However, this function
 * does not overwrite the portlib err buffer as omrfile_attr() does.
 *
 ...
 */
static int32_t VMINLINE isFile(struct OMRPortLibrary *portLibrary,
                               const char *path) {
  struct stat buffer;
 ...
 */
static uintptr_t isFileinLibPath(struct OMRPortLibrary *portLibrary,
                                 char *fileName) {
  /*
   * Because redirector adds onto LIBPATH, fetching
   * LIBPATH via getenv is more accurate than fetching LIBPATH from loadquery
   * using L_GETLIBATH flag.
   */
  ...
  libPath = portLibrary->mem_allocate_memory(portLibrary, envLibPathLen,
                                             OMR_GET_CALLSITE(),
  ...
  while (NULL != path) {
    ...
    if (totalPathLen >= EsMaxPath) {
      tmpPath = portLibrary->mem_allocate_memory(portLibrary, totalPathLen,
                                                 OMR_GET_CALLSITE(),
                                                 OMRMEM_CATEGORY_PORT_LIBRARY);
      if (NULL == tmpPath) {
        /* move on to next path in LIBPATH if we can't allocate enough memory to
    ...
    portLibrary->str_printf(portLibrary, pathBuf, totalPathLen, "%s/%s", path,
                            fileName);
...
/* This macro will seek out whether specified path is indeed a file in current
 * folder or in any folders specified by LIBPATH */
#define IS_FILE_VISIBLE(portlib, path)        \
  ((EsIsFile == isFile(portLibrary, path)) || \
   (1 == isFileinLibPath(portLibrary, path)))
...
uintptr_t omrsl_open_shared_library(struct OMRPortLibrary *portLibrary,
  ...
  uintptr_t result;
  int lazyOrNow =
      OMR_ARE_ALL_BITS_SET(flags, OMRPORT_SLOPEN_LAZY) ? RTLD_LAZY : RTLD_NOW;
  BOOLEAN decorate = OMR_ARE_ALL_BITS_SET(flags, OMRPORT_SLOPEN_DECORATE);
  uintptr_t lastErrno = 0;
  BOOLEAN openExec =
      OMR_ARE_ALL_BITS_SET(flags, OMRPORT_SLOPEN_OPEN_EXECUTABLE);
  ...
  if (!openExec && decorate) {
    if (NULL != fileName) {
      ...
      pathLength = portLibrary->str_printf(
          portLibrary, mangledName, (EsMaxPath + 1),
          "%.*slib%s" PLATFORM_DLL_EXTENSION,
          (uintptr_t)fileName + 1 - (uintptr_t)name, name, fileName + 1);
    } else {
      pathLength =
          portLibrary->str_printf(portLibrary, mangledName, (EsMaxPath + 1),
  ...
  /* CMVC 137341:
   * dlopen() searches for libraries using the LIBPATH envvar as it was when the
   * process was launched.  This causes multiple issues such as:
   *  - finding 32 bit binaries for libomrsig.so instead of the 64 bit binary
   * needed and vice versa
   *  - finding compressed reference binaries instead of non-compressed ref
   * binaries
   *
   * calling loadAndInit(libname, 0 -> no flags, NULL -> use the currently
   * defined LIBPATH) allows us to load the library with the current libpath
   * instead of the one at process creation time. We can then call dlopen() as
   * per normal and the just loaded library will be found.
   */
  ...
  if ((NULL == handle) && !openExec) {
    ...
    if ((suffixLength < sizeof(portLibDir)) &&
        (0 != getDirectoryOfLibrary(portLibrary, portLibDir,
                                    sizeof(portLibDir) - suffixLength))) {
      strcat(portLibDir, "/");
      ...
      if (NULL == handle) {
        lastErrno = errno;
        /* update the error string and return code only if a file was actually
         * found */
        if ((ENOENT != lastErrno) &&
            (IS_FILE_VISIBLE(portLibrary, portLibDir))) {
          getDLError(portLibrary, errBuf, sizeof(errBuf));
          result = portLibrary->error_set_last_error_with_message(
              portLibrary, OMRPORT_SL_INVALID, errBuf);
        }
      ...
      if (NULL == handle) {
        ...
        if (decorate) {
          if (NULL != fileName) {
            /* the names specifies a path */
            pathLength = portLibrary->str_printf(
                portLibrary, mangledName, (EsMaxPath + 1), "%.*slib%s.a",
                (uintptr_t)fileName + 1 - (uintptr_t)name, name, fileName + 1);
          } else {
            pathLength = portLibrary->str_printf(
                portLibrary, mangledName, (EsMaxPath + 1), "lib%s.a", name);
          }
          if (pathLength >= EsMaxPath) {
            result = OMRPORT_SL_UNSUPPORTED;
            goto exit;
          ...
          if (NULL == handle) {
            ...
            lastErrno = errno;
            /* update the error string and return code only if a file was
             * actually found */
            if ((ENOENT != lastErrno) &&
                (IS_FILE_VISIBLE(portLibrary, mangledName))) {
              getDLError(portLibrary, errBuf, sizeof(errBuf));
              result = portLibrary->error_set_last_error_with_message(
                  portLibrary, OMRPORT_SL_INVALID, errBuf);
            }
            ...
            if (NULL == handle) {
              ...
              if (NULL != fileName) {
                ...
                pathLength =
                    portLibrary->str_printf(portLibrary, mangledName,
                                            (EsMaxPath + 1), "%s.srvpgm", name);
              }
              if (pathLength >= EsMaxPath) {
                result = OMRPORT_SL_UNSUPPORTED;
                goto exit;
              ...
              if (NULL == handle) {
                ...
                if ((NULL != actualFileName) &&
                  ...
                  /*
                   * reference :
                   * http://publib.boulder.ibm.com/infocenter/pseries/v5r3/index.jsp?topic=/com.ibm.aix.basetechref/doc/basetrf1/load.htm
                   * In case where library name contains '(' and ')', we are
                   * specifying an archive member. eg archive(member). This
                   * translates into a file of name 'archive', to which dlopen
                   * will look to load 'member' since RTLD_MEMBER is specified.
                   */
...
static void getDLError(struct OMRPortLibrary *portLibrary, char *errBuf,
  ...
  /* dlerror can be misleading on AIX.  If the file we tried to dlopen exists
   * maybe loadquery will generate a better message.
   */
  if (ENOENT == dlopen_errno) {
    char *buffer[1024];
    if (loadquery(L_GETMESSAGES, buffer, sizeof(buffer)) != -1) {
      /* This checks for an error loading a dependant module which dlopen seems
       * to report incorrectly */
      if ((L_ERROR_NOLIB == atoi(buffer[0])) &&
          (L_ERROR_DEPENDENT == atoi(buffer[1]))) {
        ...
        portLibrary->str_printf(portLibrary, errBuf, bufLen, error,
                                &buffer[1][3]);
        return;
      }
    }
  } else if ((ENOEXEC == dlopen_errno) &&
             (((NULL == error) || ('\0' == error[0])))) {
 ...
 *
 * @param[out] buf Pointer to memory which is filled in with path to port
 * library
 *
 * @return 0 on failure, any other value on failure
 */
static uintptr_t getDirectoryOfLibrary(struct OMRPortLibrary *portLib,
                                       char *buf, uintptr_t bufLen) {
  ...
  linfoSize = 1024;
  linfo = portLib->mem_allocate_memory(portLib, linfoSize, OMR_GET_CALLSITE(),
                                       OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == linfo) {
    return 0;
  }
  for (;;) {
    ...
    linfop = portLib->mem_reallocate_memory(portLib, linfo, linfoSize,
                                            OMR_GET_CALLSITE(),
                                            OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == linfop) {
...
static void convertWithIConv(struct OMRPortLibrary *portLibrary,
  ...
  converter = iconv_get(portLibrary, J9SL_ICONV_DESCRIPTOR, "UTF-8",
                        nl_langinfo(CODESET));
 ...
 * that were created by @ref omrsl_startup should be destroyed here.
 *
 * @param[in] portLibrary The port library.
 *
 ...
 * that are required for the shared library operations may be created here.  All
 * resources created here should be destroyed in @ref omrsl_shutdown.
 *
 * @param[in] portLibrary The port library.
 *
 * @return 0 on success, negative error code on failure.  Error code values
 * returned are \arg OMRPORT_ERROR_STARTUP_SL
 *
