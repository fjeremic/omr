...
static void* getMemoryInRangeForLargePages(struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier,
  key_t addressKey, OMRMemCategory* category, uintptr_t byteAmount, void* startAddress, void* endAddress,
...
static void* getMemoryInRangeForDefaultPages(struct OMRPortLibrary* portLibrary,
  struct J9PortVmemIdentifier* identifier, OMRMemCategory* category, uintptr_t byteAmount, void* startAddress,
  void* endAddress, uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode);
static void* getMemoryInRangeForDefaultPagesUsingMmap(struct OMRPortLibrary* portLibrary,
  struct J9PortVmemIdentifier* identifier, OMRMemCategory* category, uintptr_t byteAmount, void* startAddress,
  void* endAddress, uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode);
...
static void* attemptToReserveInLargePages(struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier,
  OMRMemCategory* category, uintptr_t byteAmount, void* startAddress, void* endAddress, uintptr_t pageSize,
  uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode);
static void* attemptToReserveInDefaultPages(struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier,
  OMRMemCategory* category, uintptr_t byteAmount, void* startAddress, void* endAddress, uintptr_t pageSize,
  uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode);
static void* reserveLargePages(struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier,
  OMRMemCategory* category, uintptr_t byteAmount, void* startAddress, void* endAddress, uintptr_t pageSize,
  uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode);
static BOOLEAN detectAndRecordPageSize(struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier,
  key_t addressKey, void* memoryPointer, OMRMemCategory* category, uintptr_t pageSize, uintptr_t byteAmount,
...
void* default_pageSize_reserve_memory(struct OMRPortLibrary* portLibrary, void* address, uintptr_t byteAmount,
  struct J9PortVmemIdentifier* identifier, uintptr_t mode, uintptr_t pageSize, OMRMemCategory* category);
...
void update_vmemIdentifier(J9PortVmemIdentifier* identifier, void* address, void* handle, uintptr_t byteAmount,
  uintptr_t mode, uintptr_t pageSize, uintptr_t pageFlags, uintptr_t allocator, OMRMemCategory* category);
...
int32_t omrvmem_startup(struct OMRPortLibrary* portLibrary)
{
  ...
  if (num_psizes == -1) {
    ...
    /* assert(num_psizes < OMRPORT_VMEM_PAGESIZE_COUNT); We don't want to do anything dangerous if this happens as it
     * may, some day. We will use the earliest values in the set since they are the ones we should understand (if the OS
     * adds new sizes at the beginning of the list there isn't much that we can do
     */
  ...
  struct OMRPortLibrary* portLibrary, void* address, uintptr_t byteAmount, struct J9PortVmemIdentifier* identifier)
{
  ...
  if (rangeIsValid(identifier, address, byteAmount)) {
    if (0 == (identifier->mode & OMRPORT_VMEM_MEMORY_MODE_EXECUTE)) {
       ...
       * which may not return page aligned address and any attempt to align it to page boundary would waste one page of
       * memory.
       */
...
int32_t omrvmem_free_memory(
  struct OMRPortLibrary* portLibrary, void* userAddress, uintptr_t byteAmount, struct J9PortVmemIdentifier* identifier)
{
  int32_t result = 0;
...
{
  /* Under AIX 5.2, a 32-bit process can request up to 3.25 GB with each shmget call (essentially all of the available
   * memory), and a 64-bit process can request at least 1 TB.  We will consider 1 TB to be an acceptable restriction,
   * and not resort to the additional complexity of multiple shmget calls.
   */
  int pageSizeCheckIndex = 0;
  void* memoryPointer = NULL;
  /* this use of "localParams" is only in place until we implement native reserve alignment (this is just the
   * implementation we use in callers) */
  ...
  if (0 == params->pageSize) {
    ...
    /* Make sure that the alignment is a multiple of both requested alignment and page size (enforces that arguments are
     * powers of two and, thus, their max is their lowest common multiple) */
    if ((0 == minimumGranule) || (0 == (alignment % minimumGranule))) {
      memoryPointer = getMemoryInRangeForDefaultPages(portLibrary, identifier, category, params->byteAmount,
        params->startAddress, params->endAddress, alignment, params->options, params->mode);
    }
  } else {
    ...
    /* If we were not called with the default page size, we must ensure that we were called with one of our understood
     *sizes. AIX offers a total of 4 page sizes: 4 kB (default) 64 kB (5.3E or later on POWER5+) 16 MB (legacy large
     *page) 16 GB (very large page in 5.3E or later on POWER5+)
     */
    /* make sure that this is valid page size for this configuration
     * check that PREFERRED_DEFAULT_PAGE_SIZE is in list as well - it will be used if original requested allocation is
     * not succeed
     */
    ...
    if (requestedPageSizeIsSupported) {
      ...
      memoryPointer = attemptToReserveInLargePages(portLibrary, identifier, category, params->byteAmount,
        params->startAddress, params->endAddress, params->pageSize, params->alignmentInBytes,
      ...
      if ((NULL == memoryPointer) && (0 == (OMRPORT_VMEM_STRICT_PAGE_SIZE & params->options))) {
        if (preferedDefaultPageSizeIsSupported && !preferedDefaultPageSizeIsRequested) {
           ...
           */
          memoryPointer = attemptToReserveInLargePages(portLibrary, identifier, category, params->byteAmount,
            params->startAddress, params->endAddress, PREFERRED_DEFAULT_PAGE_SIZE, params->alignmentInBytes,
            params->options | OMRPORT_VMEM_STRICT_PAGE_SIZE, params->mode);
        }
        ...
        if (NULL == memoryPointer) {
          ...
          memoryPointer
            = attemptToReserveInLargePages(portLibrary, identifier, category, params->byteAmount, params->startAddress,
              params->endAddress, PPG_vmem_pageSize[0], params->alignmentInBytes, params->options, params->mode);
        }
        ...
        if (NULL == memoryPointer) {
          ...
          fflush(stdout);
#endif
          memoryPointer = attemptToReserveInDefaultPages(portLibrary, identifier, category, params->byteAmount,
            params->startAddress, params->endAddress, PPG_vmem_pageSize[0], params->alignmentInBytes, params->options,
            params->mode);
        }
      }
    }
    ...
    if (NULL == memoryPointer) {
      /* we failed to find the page size before hitting the null terminator so use the unsupported page size trace point
       * and fail */
      update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, 0, NULL);
      if (!requestedPageSizeIsSupported) {
  ...
  uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
  ...
  if ((0 == minimumGranule) || (0 == (alignment % minimumGranule))) {
    memoryPointer = reserveLargePages(
      portLibrary, identifier, category, byteAmount, startAddress, endAddress, pageSize, alignment, vmemOptions, mode);
  }
  ...
  uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
  ...
  if ((0 == minimumGranule) || (0 == (alignment % minimumGranule))) {
    memoryPointer = getMemoryInRangeForDefaultPages(
      portLibrary, identifier, category, byteAmount, startAddress, endAddress, alignment, vmemOptions, mode);
  }
  ...
  uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
  ...
  if (-1 == addressKey) {
    ...
    memoryPointer = getMemoryInRangeForLargePages(portLibrary, identifier, addressKey, category, byteAmount,
      startAddress, endAddress, alignmentInBytes, vmemOptions);
    ...
    if (0 == shmctl(addressKey, IPC_RMID, NULL)) {
      if (NULL != memoryPointer) {
        ...
        if (FALSE
          == detectAndRecordPageSize(
            portLibrary, identifier, addressKey, memoryPointer, category, pageSize, byteAmount, vmemOptions, mode)) {
  ...
  uintptr_t vmemOptions, uintptr_t mode)
{
  ...
  update_vmemIdentifier(identifier, memoryPointer, (void*)(intptr_t)addressKey, byteAmount, mode,
    (uintptr_t)page_info.pagesize, OMRPORT_VMEM_PAGE_FLAG_NOT_USED, OMRPORT_VMEM_RESERVE_USED_SHM, category);
...
#else
  /* Update identifier and commit memory if required, else return reserved memory */
  update_vmemIdentifier(identifier, memoryPointer, (void*)addressKey, byteAmount, mode, pageSize,
  ...
  struct J9PortVmemIdentifier* identifier, uintptr_t mode, uintptr_t pageSize, OMRMemCategory* category)
{
  ...
  if (0 != (OMRPORT_VMEM_MEMORY_MODE_EXECUTE & mode)) {
    ...
    update_vmemIdentifier(identifier, result, result, byteAmount, mode, pageSize, OMRPORT_VMEM_PAGE_FLAG_NOT_USED,
      OMRPORT_VMEM_RESERVE_USED_J9MEM_ALLOCATE_MEMORY, category);
    Trc_PRT_vmem_default_reserve_exit(result, address, byteAmount);
    return result;
  }
  ...
  uintptr_t* pageFlags, BOOLEAN* isSizeSupported)
{
  ...
  if (0 != validPageSize) {
    ...
    if ((OMRPORT_VMEM_MEMORY_MODE_EXECUTE != (OMRPORT_VMEM_MEMORY_MODE_EXECUTE & mode)) || (SIXTEEN_M == validPageSize))
#else
  ...
  uintptr_t alignmentInBytes, uintptr_t vmemOptions)
{
  ...
  while ((startAddress <= currentAddress) && (endAddress >= currentAddress)) {
    ...
    memoryPointer
      = allocateMemoryForLargePages(portLibrary, identifier, currentAddress, addressKey, category, byteAmount);
  ...
  void* endAddress, uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
#if !defined(OMR_ENV_DATA64)
  /**
   * Using shmat to allocate vmem on AIX32 reduces the number of segments which can be used for thread stacks.  Thus,
   * explicitly use the mmap allocator for default-sized pages (large pages still must use shmat). The only reasons to
   * use shmat, other than unification of the code paths, are to support large pages or NUMA.  NUMA is only supported on
  ...
  return getMemoryInRangeForDefaultPagesUsingMmap(
    portLibrary, identifier, category, byteAmount, startAddress, endAddress, alignmentInBytes, vmemOptions, mode);
#else /* !defined(OMR_ENV_DATA64) */
  /*
   * Balanced GC requires that omrvmem_numa_set_affinity() can be used on memory returned by omrvmem_reserve_memory().
   * This is not supported for memory returned by default_pageSize_reserve_memory(), as it uses the mmap() API for
   * historical reasons.
   *
   ...
   *
   * Note that there is a limitation in older versions of AIX (pre-6.1) where shmem may be unable to allocate as much
   * memory as mmap (up to one segment is lost - 256M).  However, the NUMA API is not supported prior to AIX 6.1 so
   * these requirements never appear together.  Thus, if NUMA is supported on the system, use shmem.  Otherwise, use
   * mmap (as we will not be trying to use the NUMA API). See CMVC 178946 for more detail regarding this limitation
   *
   * Also note, do not use shmem for OMRPORT_VMEM_MEMORY_MODE_EXECUTE memory (JIT codecache) as it causes the following
   * problems:
   ...
   * 			- i.e. SLB(segment lookaside buffer), TLB(table lookaside buffer), and ERAT(effective address to real address
   * table). In particular, each 256MB requires an SLB entry.
   *
   */
  if (__ENHANCED_AFFINITY()
    && (OMR_ARE_NO_BITS_SET(mode, OMRPORT_VMEM_MEMORY_MODE_EXECUTE | OMRPORT_VMEM_NO_AFFINITY))) {
    /* If we have __ENHANCED_AFFINITY() and we're not looking for executable memory */
    return reserveLargePages(portLibrary, identifier, category, byteAmount, startAddress, endAddress,
      PPG_vmem_pageSize[0], alignmentInBytes, vmemOptions, mode);
  } else {
    return getMemoryInRangeForDefaultPagesUsingMmap(
      portLibrary, identifier, category, byteAmount, startAddress, endAddress, alignmentInBytes, vmemOptions, mode);
  }
  ...
  void* endAddress, uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
  ...
  while ((startAddress <= currentAddress) && (endAddress >= currentAddress)) {
    ...
    memoryPointer = default_pageSize_reserve_memory(
      portLibrary, currentAddress, byteAmount, identifier, mode, PPG_vmem_pageSize[0], category);
  ...
  allocAnywhere:
    memoryPointer = default_pageSize_reserve_memory(
      portLibrary, NULL, byteAmount, identifier, mode, PPG_vmem_pageSize[0], category);
  }
  ...
  uintptr_t byteAmount, struct J9PortVmemIdentifier* identifier)
{
...
#if defined(OMR_ENV_DATA64)
  /**
   * NUMA only supported on AIX64 since Balanced does not yet exist on AIX32 and using shmem for vmem, which NUMA
   * requires, reduces the number of thread stacks available.
   *
   * See CMVC 178983 for more detail regarding this limitation.
   */
  if (OMR_ARE_NO_BITS_SET(identifier->mode, OMRPORT_VMEM_NO_AFFINITY)) {
    ...
    int (*PTR_ra_attach)(rstype_t, rsid_t, rstype_t, rsid_t_MODIFIED, uint_t)
      = (int (*)(rstype_t, rsid_t, rstype_t, rsid_t_MODIFIED, uint_t))dlsym(RTLD_DEFAULT, "ra_attach");
    rsid_t_MODIFIED targetSRADResourceID;
#else
    int (*PTR_ra_attach)(rstype_t, rsid_t, rstype_t, rsid_t, uint_t)
      = (int (*)(rstype_t, rsid_t, rstype_t, rsid_t, uint_t))dlsym(RTLD_DEFAULT, "ra_attach");
