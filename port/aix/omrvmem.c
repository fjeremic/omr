...
allocateMemoryForLargePages(struct OMRPortLibrary *portLibrary,
                            ...
                            void *currentAddress, key_t addressKey,
...
#if defined(OMRVMEM_DEBUG)
  printf("\t\t omrvmem_reserve_memory_ex shmget(byteAmount 0x%zX, shmgetFlags "
...
static void *getMemoryInRangeForDefaultPages(
  ...
  if (__ENHANCED_AFFINITY() &&
      (OMR_ARE_NO_BITS_SET(mode, OMRPORT_VMEM_MEMORY_MODE_EXECUTE |
    ...
    if (1 == getprocs64(&pe, sizeof(pe), NULL, 0, &pid, 1)) {
      ...
      if (size > 0) {
        *memorySize = size;
        ...
        Trc_PRT_vmem_get_process_memory_failed("negative size from getprocs64",
                                               0);
