...
static void*
getNextPossibleAddress(uintptr_t alignment,
                       void* currentAddress,
                       intptr_t direction);
static void*
...
getMemoryInRangeForDefaultPages(struct OMRPortLibrary* portLibrary,
                                ...
                                uintptr_t mode);
static void*
getMemoryInRangeForDefaultPagesUsingMmap(
...
reserveLargePages(struct OMRPortLibrary* portLibrary,
                  ...
                  uintptr_t mode);
static BOOLEAN
detectAndRecordPageSize(struct OMRPortLibrary* portLibrary,
                        struct J9PortVmemIdentifier* identifier,
                        key_t addressKey,
                        void* memoryPointer,
                        OMRMemCategory* category,
...
#endif
extern sradid_t
rs_get_homesrad(void);
extern int
rs_info(void* out, int command, long arg1, long arg2);
#ifndef _AIX61
    ...
    if (requestedPageSizeIsSupported) {
      ...
      if ((NULL == memoryPointer) &&
        ...
        if (NULL == memoryPointer) {
          ...
          memoryPointer = attemptToReserveInLargePages(portLibrary,
                                                       identifier,
                                                       category,
                                                       params->byteAmount,
                                                       params->startAddress,
...
#endif
          memoryPointer =
            attemptToReserveInDefaultPages(portLibrary,
                                           identifier,
                                           category,
                                           params->byteAmount,
                                           params->startAddress,
  ...
  if (-1 == addressKey) {
    ...
    if (0 == shmctl(addressKey, IPC_RMID, NULL)) {
      if (NULL != memoryPointer) {
        ...
        if (FALSE == detectAndRecordPageSize(portLibrary,
                                             identifier,
                                             addressKey,
                                             memoryPointer,
                                             category,
                        ...
                        uintptr_t mode)
{
  ...
  update_vmemIdentifier(identifier,
                        memoryPointer,
                        (void*)(intptr_t)addressKey,
                        byteAmount,
                        mode,
                        (uintptr_t)page_info.pagesize,
                                ...
                                uintptr_t mode)
{
   ...
   */
  return getMemoryInRangeForDefaultPagesUsingMmap(portLibrary,
                                                  identifier,
                                                  category,
                                                  byteAmount,
                                                  startAddress,
  ...
  if (__ENHANCED_AFFINITY() &&
     ...
     * memory */
    return reserveLargePages(portLibrary,
                             identifier,
                             category,
                             byteAmount,
                             startAddress,
...
#if defined(OMRVMEM_DEBUG)
  printf(
    "\t\t shmat(currentAddress = 0x%zx) returned 0x%zx\n", currentAddress, ptr);
#endif
  return ptr;
}
                          ...
                          struct J9PortVmemIdentifier* identifier)
{
  ...
  if (OMR_ARE_NO_BITS_SET(identifier->mode, OMRPORT_VMEM_NO_AFFINITY)) {
    ...
    result = PTR_ra_attach(R_SUBRANGE,
                           thisSubrangeResourceID,
                           R_SRADID,
                           targetSRADResourceID,
                           R_MIGRATE_ASYNC);
  }
#endif /* defined(OMR_ENV_DATA64) */
