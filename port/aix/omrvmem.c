...
/* LIR 1394: Search for 64K page support and set it as default if it exists for
 * AIX PPC. */
#define PREFERRED_DEFAULT_PAGE_SIZE SIXTY_FOUR_K

static void *getNextPossibleAddress(uintptr_t alignment, void *currentAddress,
                                    intptr_t direction);
...
static void *getMemoryInRangeForDefaultPages(
    ...
    uintptr_t mode);
static void *getMemoryInRangeForDefaultPagesUsingMmap(
    struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
    OMRMemCategory *category, uintptr_t byteAmount, void *startAddress,
    void *endAddress, uintptr_t alignmentInBytes, uintptr_t vmemOptions,
    uintptr_t mode);
...
static void *attemptToReserveInLargePages(
    struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
    OMRMemCategory *category, uintptr_t byteAmount, void *startAddress,
    void *endAddress, uintptr_t pageSize, uintptr_t alignmentInBytes,
    uintptr_t vmemOptions, uintptr_t mode);
static void *attemptToReserveInDefaultPages(
    struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
    OMRMemCategory *category, uintptr_t byteAmount, void *startAddress,
    void *endAddress, uintptr_t pageSize, uintptr_t alignmentInBytes,
    uintptr_t vmemOptions, uintptr_t mode);
static void *reserveLargePages(struct OMRPortLibrary *portLibrary,
                               ...
                               uintptr_t vmemOptions, uintptr_t mode);
static BOOLEAN detectAndRecordPageSize(struct OMRPortLibrary *portLibrary,
                                       struct J9PortVmemIdentifier *identifier,
                                       key_t addressKey, void *memoryPointer,
...
/* This ifdef is specifically provided to allow compilation on AIX 5.3 of
 * features which we can only use on 6.1 */
...
#ifndef _AIX61
/* create rsid_t_MODIFIED since rsid_t is already defined in 5.3 but is missing
 * the at_sradid field */
typedef union {
...
int32_t omrvmem_startup(struct OMRPortLibrary *portLibrary) {
  ...
  if (num_psizes == -1) {
    ...
    /* assert(num_psizes < OMRPORT_VMEM_PAGESIZE_COUNT); We don't want to do
     * anything dangerous if this happens as it may, some day. We will use the
     * earliest values in the set since they are the ones we should understand
     * (if the OS adds new sizes at the beginning of the list there isn't much
     * that we can do
     */
    ...
    for (counter = 0; counter < num_psizes; counter++) {
      if (psizes[counter] > physicalMemory) {
        /* Don't advertise pagesizes that exceed the physical memory on the
         * machine. */
        Trc_PRT_vmem_omrvmem_startup_pagessize_exceeds_physical_memory(
            psizes[counter], physicalMemory);
        break;
      }
...
void *omrvmem_commit_memory(struct OMRPortLibrary *portLibrary, void *address,
  ...
  if (rangeIsValid(identifier, address, byteAmount)) {
    if (0 == (identifier->mode & OMRPORT_VMEM_MEMORY_MODE_EXECUTE)) {
      /* Assert that address and byteAmount are page aligned. These restrictions
       * are not required for executable pages because when using default page
       * size they are allocated using malloc() which may not return page
       * aligned address and any attempt to align it to page boundary would
       * waste one page of memory.
       */
    ...
    if (PPG_vmem_pageSize[0] == identifier->pageSize) {
      if (0 ==
          mprotect(address, byteAmount,
                   get_default_pageSize_protectionBits(identifier->mode))) {
        ptr = address;
      } else {
        ptr = NULL;
      }
  ...
  /*
   * CMVC 70354 change
   * DANGER: memory is already zeroed when it gets faulted in so no memset is
   * required. However, if the memory cannot actually be faulted in when needed,
   * the VM will crash during execution in a much less obvious way.  Adding a VM
   * flag to force the init is a RAS TODO.
   */
}
...
intptr_t omrvmem_decommit_memory(struct OMRPortLibrary *portLibrary,
  ...
  if (portLibrary->portGlobals->vmemAdviseOSonFree == 1) {
    if (rangeIsValid(identifier, address, byteAmount)) {
      ...
      if (byteAmount > 0) {
        if ((OMRPORT_VMEM_RESERVE_USED_J9MEM_ALLOCATE_MEMORY ==
             identifier->allocator) ||
            (OMRPORT_VMEM_RESERVE_USED_SHM == identifier->allocator)) {
          if ((FOUR_K == identifier->pageSize) ||
              (SIXTY_FOUR_K == identifier->pageSize)) {
            unsigned long flags = DISCLAIM_ZEROMEM;

            Trc_PRT_vmem_decommit_memory_calling_disclaim(flags);
            result = (intptr_t)disclaim64((void *)address, (size_t)byteAmount,
                                          flags);
          } else {
            /* noop: both 16M and 16G pages are pinned in memory so don't
             * disclaim them */
            result = 0;
          }
...
void *omrvmem_reserve_memory_ex(struct OMRPortLibrary *portLibrary,
                                struct J9PortVmemIdentifier *identifier,
                                struct J9PortVmemParams *params) {
  /* Under AIX 5.2, a 32-bit process can request up to 3.25 GB with each shmget
   * call (essentially all of the available memory), and a 64-bit process can
   * request at least 1 TB.  We will consider 1 TB to be an acceptable
   * restriction, and not resort to the additional complexity of multiple shmget
   * calls.
   */
  int pageSizeCheckIndex = 0;
  void *memoryPointer = NULL;
  /* this use of "localParams" is only in place until we implement native
   * reserve alignment (this is just the implementation we use in callers) */
  ...
  printf(
      ...
      "endAddress: %p, pageSize: 0x%zX, %s, %s, %s\n ",
  ...
  if (0 == params->pageSize) {
    ...
    if ((0 == minimumGranule) || (0 == (alignment % minimumGranule))) {
      memoryPointer = getMemoryInRangeForDefaultPages(
          ...
          params->startAddress, params->endAddress, alignment, params->options,
          params->mode);
    }
  } else {
    BOOLEAN requestedPageSizeIsSupported = FALSE;
    BOOLEAN preferedDefaultPageSizeIsSupported = FALSE;
    BOOLEAN preferedDefaultPageSizeIsRequested =
        (PREFERRED_DEFAULT_PAGE_SIZE == params->pageSize);

    /* If we were not called with the default page size, we must ensure that we
     *were called with one of our understood sizes. AIX offers a total of 4 page
     *sizes: 4 kB (default) 64 kB (5.3E or later on POWER5+) 16 MB (legacy large
     *page) 16 GB (very large page in 5.3E or later on POWER5+)
     */
    /* make sure that this is valid page size for this configuration
     * check that PREFERRED_DEFAULT_PAGE_SIZE is in list as well - it will be
     * used if original requested allocation is not succeed
     */
    while (0 != PPG_vmem_pageSize[pageSizeCheckIndex]) {
      ...
      if (PREFERRED_DEFAULT_PAGE_SIZE ==
          PPG_vmem_pageSize[pageSizeCheckIndex]) {
    ...
    if (requestedPageSizeIsSupported) {
      ...
      memoryPointer = attemptToReserveInLargePages(
          portLibrary, identifier, category, params->byteAmount,
          params->startAddress, params->endAddress, params->pageSize,
          params->alignmentInBytes,
          params->options | OMRPORT_VMEM_STRICT_PAGE_SIZE, params->mode);

      if ((NULL == memoryPointer) &&
          (0 == (OMRPORT_VMEM_STRICT_PAGE_SIZE & params->options))) {
        if (preferedDefaultPageSizeIsSupported &&
            !preferedDefaultPageSizeIsRequested) {
          /*
           * If requested page is not supported or memory can not be allocated
           * with such page size repeat an allocation with
           * PREFERRED_DEFAULT_PAGE_SIZE
           */
          memoryPointer = attemptToReserveInLargePages(
              portLibrary, identifier, category, params->byteAmount,
              params->startAddress, params->endAddress,
              PREFERRED_DEFAULT_PAGE_SIZE, params->alignmentInBytes,
              params->options | OMRPORT_VMEM_STRICT_PAGE_SIZE, params->mode);
        }
        ...
        if (NULL == memoryPointer) {
          ...
          memoryPointer = attemptToReserveInLargePages(
              portLibrary, identifier, category, params->byteAmount,
              params->startAddress, params->endAddress, PPG_vmem_pageSize[0],
              params->alignmentInBytes, params->options, params->mode);
        }

        if (NULL == memoryPointer) {
          /*
           * Try again with default size pages using another allocation methods
           * Do it only if known page size allocation was requested - for
          ...
          fflush(stdout);
#endif
          memoryPointer = attemptToReserveInDefaultPages(
              portLibrary, identifier, category, params->byteAmount,
              params->startAddress, params->endAddress, PPG_vmem_pageSize[0],
              params->alignmentInBytes, params->options, params->mode);
        }
    ...
    if (NULL == memoryPointer) {
      /* we failed to find the page size before hitting the null terminator so
       * use the unsupported page size trace point and fail */
      update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, 0, NULL);
      if (!requestedPageSizeIsSupported) {
...
static void *attemptToReserveInLargePages(
  ...
  /* Make sure that the alignment is a multiple of both requested alignment and
   * page size (enforces that arguments are powers of two and, thus, their max
   * is their lowest common multiple) */
  if ((0 == minimumGranule) || (0 == (alignment % minimumGranule))) {
...
static void *attemptToReserveInDefaultPages(
  ...
  if ((0 == minimumGranule) || (0 == (alignment % minimumGranule))) {
    memoryPointer = getMemoryInRangeForDefaultPages(
        portLibrary, identifier, category, byteAmount, startAddress, endAddress,
        alignment, vmemOptions, mode);
  }
...
static void *reserveLargePages(struct OMRPortLibrary *portLibrary,
...
#if defined(OMRVMEM_DEBUG)
  printf(
      "\t\t omrvmem_reserve_memory_ex shmget(byteAmount 0x%zX, shmgetFlags = "
      "0x%x) returning addressKey: 0x%zx\n",
  ...
  if (-1 == addressKey) {
    ...
    if (-1 == shmctl(addressKey, SHM_PAGESIZE, (struct shmid_ds *)&mutate)) {
      /* Failed to set page size, this will happen if the env var EXTSHM=ON is
       * set It is safe to continue with the memory request.
       * detectAndRecordPageSize() (below) will determine if the page size we
       * get is acceptable */
#if defined(OMRVMEM_DEBUG)
    ...
    memoryPointer = getMemoryInRangeForLargePages(
        ...
        endAddress, alignmentInBytes, vmemOptions);
    ...
    if (0 == shmctl(addressKey, IPC_RMID, NULL)) {
      if (NULL != memoryPointer) {
        ...
        if (FALSE == detectAndRecordPageSize(
                         portLibrary, identifier, addressKey, memoryPointer,
                         category, pageSize, byteAmount, vmemOptions, mode)) {
...
static BOOLEAN detectAndRecordPageSize(struct OMRPortLibrary *portLibrary,
...
#if !(defined(J9OS_I5) && defined(J9OS_I5_V5R4))
  /* Ensure we got back the correct page size if OMRPORT_VMEM_STRICT_PAGE_SIZE
   * is set */
  page_info.addr = (uint64_t)memoryPointer;
  vmgetinfoResult =
      vmgetinfo(&page_info, VM_PAGE_INFO, sizeof(struct vm_page_info));
   ...
   * memory */
  update_vmemIdentifier(identifier, memoryPointer, (void *)(intptr_t)addressKey,
                        byteAmount, mode, (uintptr_t)page_info.pagesize,
                        OMRPORT_VMEM_PAGE_FLAG_NOT_USED,
                        OMRPORT_VMEM_RESERVE_USED_SHM, category);
  ...
  if (0 != vmgetinfoResult) {
    ...
    if (!vmgetinfoFailedFirstTime) {
      portLibrary->tty_printf(portLibrary,
                              "Warning: large pages not available\n");
    } else {
      portLibrary->tty_printf(
          portLibrary,
          "Warning: not enough large pages to satisfy entire allocation\n");
    }
#endif
  ...
  /* Update identifier and commit memory if required, else return reserved
   * memory */
  update_vmemIdentifier(
      identifier, memoryPointer, (void *)addressKey, byteAmount, mode, pageSize,
 ...
 * @param[in] mode Bitmap indicating how memory is to be reserved.  Expected
 * values combination of:
 * @param[in] category Memory category
 *
 * @return pointer to the reserved memory on success, NULL on failure.
 *
 * @internal @warning Do not call error handling code @ref omrerror upon error
 * as the error handling code uses per thread buffers to store the last error.
 * If memory can not be allocated the result would be an infinite loop.
 */
void *default_pageSize_reserve_memory(struct OMRPortLibrary *portLibrary,
                                      void *address, uintptr_t byteAmount,
                                      struct J9PortVmemIdentifier *identifier,
  ...
  if (0 != (OMRPORT_VMEM_MEMORY_MODE_EXECUTE & mode)) {
    ...
    result = portLibrary->mem_allocate_memory(
        portLibrary, byteAmount, OMR_GET_CALLSITE(), category->categoryCode);
#if defined(OMRVMEM_DEBUG)
    ...
    /* mem_allocate_memory will have called omrmem_category_increment_counters()
    ...
    update_vmemIdentifier(identifier, result, result, byteAmount, mode,
                          ...
                          OMRPORT_VMEM_RESERVE_USED_J9MEM_ALLOCATE_MEMORY,
                          category);
    Trc_PRT_vmem_default_reserve_exit(result, address, byteAmount);
    return result;
  }
 ...
 *
 * @param[in] mode Bitmap indicating how memory is to be reserved.  Expected
 * values combination of: \arg OMRPORT_VMEM_MEMORY_MODE_READ memory is readable
 ...
 *
 * @return the protection flags required by the functions used to reserve memory
 ...
 * To get data segment page size, retrieve size of the page backing
 * portLibrary->portGlobals which is allocated using omrmem_allocate_memory() in
 * port/unix/omrmem.c
 *
 * @param[in] portLibrary The port library.
 *
...
static uintptr_t getDataSegmentPageSize(struct OMRPortLibrary *portLibrary) {
  ...
  if (0 == rc) {
    ...
    /* vmgetinfo() can fail with VM_PAGE_INFO only if vm_page_info.addr is
     * invalid, which should never happen here as portLibrary->portGlobals is
     * valid until VM shutdown.
     */
...
void omrvmem_default_large_page_size_ex(struct OMRPortLibrary *portLibrary,
  ...
  if (NULL != pageSize) {
    if (OMRPORT_VMEM_MEMORY_MODE_EXECUTE !=
      ...
      /* AIX 5.3E (or later) on POWER5+ supports 4 kiB, 64 kiB, 16 MiB, 16 GiB
       * page sizes (in that order). To be compatible with how the -Xlp option
       * has historically worked, 16 MiB is the default page size. If, for some
       * reason, that page size can't be found, we search for 64KB page size. If
       * that is also not found, we will return the page size at [1] since
       * failing is probably not a reasonable answer as it would break our
       * support for that legacy option (and that situation should never occur).
       */
      ...
      while (0 != PPG_vmem_pageSize[index]) {
        ...
        if (SIXTY_FOUR_K == PPG_vmem_pageSize[index]) {
          ...
          /* We don't break here because preferred page size is 16M. So keep
           * looking for 16M. */
        }
        index++;
      }
      *pageSize = preferredPageSize;
       ...
       * This is the same page size as used by JIT code cache when allocated
       * using omrmem_allocate_memory().
       */
...
intptr_t omrvmem_find_valid_page_size(struct OMRPortLibrary *portLibrary,
  ...
  if (0 != validPageSize) {
    /* For executable pages search through the list of supported page sizes only
     * if
    ...
    if ((OMRPORT_VMEM_MEMORY_MODE_EXECUTE !=
         (OMRPORT_VMEM_MEMORY_MODE_EXECUTE & mode)) ||
        ((TRUE == codeCacheConsolidationEnabled) &&
         (SIXTEEN_M == validPageSize)))
 ...
 * be allocated as documented on aix publib article General Programming
 * Concepts: Writing and Debugging Programs Understanding Memory Mapping. URL =>
 ...
 *
 * @param[in] alignment The required alignment of the address to return (always
 * a multiple of the page size being used)
 * @param[in] currentAddress	The last address at which we attempted to
 * allocate memory
 * @param[in] direction	The direction to follow when scanning through the
 * address range 1 = bottom-up -1 = top-down
 */
static void *getNextPossibleAddress(uintptr_t alignment, void *currentAddress,
                                    intptr_t direction) {
  ...
  if (direction == 1) {
    ...
    if ((uintptr_t)currentAddress < ranges[0][0]) {
      ...
      for (i = 0; i < (numRanges - 1); i++) {
        if (((uintptr_t)currentAddress > ranges[i][1]) &&
            ((uintptr_t)currentAddress < ranges[i + 1][0])) {
  ...
  if (currentAddress != NULL) {
    currentAddress =
        (void *)((uintptr_t)currentAddress & ~(alignment - 1)); /* align */
...
static void *getMemoryInRangeForLargePages(
  ...
  if (0 != (vmemOptions & OMRPORT_VMEM_ALLOC_DIR_TOP_DOWN)) {
    ...
    currentAddress =
        (void *)((uintptr_t)endAddress & ~(alignmentInBytes - 1)); /* align */
  } else if (0 != (vmemOptions & OMRPORT_VMEM_ALLOC_DIR_BOTTOM_UP)) {
    if (startAddress == NULL) {
      currentAddress =
          getNextPossibleAddress(alignmentInBytes, NULL, direction);
    }
  ...
  while ((startAddress <= currentAddress) && (endAddress >= currentAddress)) {
    ...
    memoryPointer =
        allocateMemoryForLargePages(portLibrary, identifier, currentAddress,
                                    addressKey, category, byteAmount);
    ...
    currentAddress =
        getNextPossibleAddress(alignmentInBytes, currentAddress, direction);
  ...
  allocAnywhere:
    memoryPointer = allocateMemoryForLargePages(
        portLibrary, identifier, NULL, addressKey, category, byteAmount);
  }
...
static void *getMemoryInRangeForDefaultPages(
...
#if !defined(OMR_ENV_DATA64)
  /**
   * Using shmat to allocate vmem on AIX32 reduces the number of segments which
   * can be used for thread stacks.  Thus, explicitly use the mmap allocator for
   * default-sized pages (large pages still must use shmat). The only reasons to
   * use shmat, other than unification of the code paths, are to support large
   * pages or NUMA.  NUMA is only supported on the Balanced policy which does
   * not yet run on 32-bit and large page allocations come through a different
   * path.
   *
   * See CMVC 178983 for more detail regarding this limitation.
   */
  return getMemoryInRangeForDefaultPagesUsingMmap(
      portLibrary, identifier, category, byteAmount, startAddress, endAddress,
      alignmentInBytes, vmemOptions, mode);
#else  /* !defined(OMR_ENV_DATA64) */
  /*
   * Balanced GC requires that omrvmem_numa_set_affinity() can be used on memory
   * returned by omrvmem_reserve_memory(). This is not supported for memory
   * returned by default_pageSize_reserve_memory(), as it uses the mmap() API
   * for historical reasons.
   *
   * To support omrvmem_numa_set_affinity() under balanced, we redefine
   * getMemoryInRangeForDefaultPages() to simply call reserveLargePages(), which
   * uses the shmat()/shmget() APIs which do not have the same problem.
   *
   * Note that there is a limitation in older versions of AIX (pre-6.1) where
   * shmem may be unable to allocate as much memory as mmap (up to one segment
   * is lost - 256M).  However, the NUMA API is not supported prior to AIX 6.1
   * so these requirements never appear together.  Thus, if NUMA is supported on
   * the system, use shmem.  Otherwise, use mmap (as we will not be trying to
   * use the NUMA API). See CMVC 178946 for more detail regarding this
   * limitation
   *
   ...
   *
   */
  if (__ENHANCED_AFFINITY() &&
      (OMR_ARE_NO_BITS_SET(
          mode, OMRPORT_VMEM_MEMORY_MODE_EXECUTE | OMRPORT_VMEM_NO_AFFINITY))) {
    /* If we have __ENHANCED_AFFINITY() and we're not looking for executable
     * memory */
    return reserveLargePages(portLibrary, identifier, category, byteAmount,
                             startAddress, endAddress, PPG_vmem_pageSize[0],
                             alignmentInBytes, vmemOptions, mode);
  } else {
    return getMemoryInRangeForDefaultPagesUsingMmap(
        portLibrary, identifier, category, byteAmount, startAddress, endAddress,
...
static void *getMemoryInRangeForDefaultPagesUsingMmap(
  ...
  while ((startAddress <= currentAddress) && (endAddress >= currentAddress)) {
    ...
    /* stop if returned pointer is within range */
    if ((NULL != memoryPointer) && (startAddress <= memoryPointer) &&
        (endAddress >= memoryPointer)) {
      break;
    }
...
intptr_t omrvmem_numa_set_affinity(struct OMRPortLibrary *portLibrary,
...
#if defined(OMR_ENV_DATA64)
  /**
   * NUMA only supported on AIX64 since Balanced does not yet exist on AIX32 and
   * using shmem for vmem, which NUMA requires, reduces the number of thread
   * stacks available.
   *
   * See CMVC 178983 for more detail regarding this limitation.
   */
  if (OMR_ARE_NO_BITS_SET(identifier->mode, OMRPORT_VMEM_NO_AFFINITY)) {
#ifndef _AIX61
    int (*PTR_ra_attach)(rstype_t, rsid_t, rstype_t, rsid_t_MODIFIED, uint_t) =
        (int (*)(rstype_t, rsid_t, rstype_t, rsid_t_MODIFIED, uint_t))dlsym(
            RTLD_DEFAULT, "ra_attach");
    rsid_t_MODIFIED targetSRADResourceID;
#else
    int (*PTR_ra_attach)(rstype_t, rsid_t, rstype_t, rsid_t, uint_t) =
        (int (*)(rstype_t, rsid_t, rstype_t, rsid_t, uint_t))dlsym(RTLD_DEFAULT,
                                                                   "ra_attach");
    rsid_t targetSRADResourceID;
#endif
    rsid_t thisSubrangeResourceID;
    /* don't forget to subtract one from the numaNode to shift it into the
     * 0-indexed numbering scheme used by the system */
    ...
    /* subrange policy is ignored in favour of the ra_attach policy so set it to
     * 0 */
    ...
    /* attach the specified subrange to the SRAD (allow the memory to be
     * asynchronously migrated to the specified SRAD) */
    result = PTR_ra_attach(R_SUBRANGE, thisSubrangeResourceID, R_SRADID,
                           targetSRADResourceID, R_MIGRATE_ASYNC);
  }
#endif /* defined(OMR_ENV_DATA64) */
...
intptr_t omrvmem_numa_get_node_details(struct OMRPortLibrary *portLibrary,
  ...
  PTR_rs_info = (int (*)(void *out, int command, long arg1, long arg2))dlsym(
      RTLD_DEFAULT, "rs_info");
  if ((0 != __ENHANCED_AFFINITY()) && (NULL != PTR_rs_get_homesrad) &&
      (NULL != PTR_rs_info)) {
    ...
    for (sradid = 0; sradid < num_srads; sradid++) {
      ...
      int infoRetCode =
          PTR_rs_info(&load_avg, RS_SRADID_LOADAVG, sradid, sizeof(load_avg));
      int vmRetCode = 0;
      ...
      if ((-1 != infoRetCode) && (-1 != vmRetCode)) {
        if (nextNodeIndex < *nodeCount) {
          ...
          numaNodes[nextNodeIndex].computationalResourcesAvailable =
              load_avg.cpu_count;
        }
        nextNodeIndex += 1;
      }
...
int32_t omrvmem_get_available_physical_memory(
  ...
  for (pageIndex = 0; 0 != pageSizes[pageIndex]; ++pageIndex) {
    ...
    if (0 == vmgetinfo(&pageSize, VMINFO_PSIZE, sizeof(pageSize))) {
      ...
      Trc_PRT_vmem_get_available_physical_memory_failed("vmgetinfo",
                                                        vmgetinfoError);
...
int32_t omrvmem_get_process_memory_size(struct OMRPortLibrary *portLibrary,
  ...
  switch (queryType) {
      ...
      if (1 == getprocs64(&pe, sizeof(pe), NULL, 0, &pid, 1)) {
        ...
        if (size > 0) {
          *memorySize = size;
          ...
          Trc_PRT_vmem_get_process_memory_failed(
              "negative size from getprocs64", 0);
    ...
    case OMRPORT_VMEM_PROCESS_VIRTUAL:
      /* There is no API on AIX to get shared memory usage for the process. If
       * such an API existed, we could return getProcessPrivateMemorySize() +
       * sharedSize here.
       *
       * Note: Iterating through /proc/<pid>/map and looking at the pages that
       * are not marked MA_SHARED does not account for shared code pages when in
       * fact command-line AIX utilities (such as svmon) do show that pages are
       * shared.
       */
