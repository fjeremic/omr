...
{
  /* 32bit AIX needs 130 slots available for the base VM as of Java6sr7. This allows a little overhead for 3rd party jni
   * libraries */
  ...
  for (i = 0, frame = threadInfo->callstack; frame != NULL; frame = frame->parent_frame, i++) {
    ...
    if (symbol_name[0] != '\0') {
      cursor += omrstr_printf(
        portLibrary, cursor, sizeof(output_buf) - (cursor - output_buf), "%.*s", symbol_length, symbol_name);
      if (symbol_offset >= 0) {
        cursor
