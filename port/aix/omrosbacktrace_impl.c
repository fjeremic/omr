...
char *tbtable_name(struct tbtable *table, short *length) {
...
void *tbtable_symbol_start(struct tbtable *table) {
...
uintptr_t omrintrospect_backtrace_symbols_raw(
    ...
    J9Heap *heap) {
  /* 32bit AIX needs 130 slots available for the base VM as of Java6sr7. This
   * allows a little overhead for 3rd party jni libraries */
  ...
  for (i = 0, frame = threadInfo->callstack; frame != NULL;
    ...
    if (loadqueryResult != -1) {
      ...
      while (cursor != next && cursor != NULL) {
        ...
        if (module_offset >= 0 &&
            module_offset < (uintptr_t)cursor->ldinfo_textsize) {
          ldInfo = cursor;
    ...
    if (ldInfo != NULL) {
      void *moduleEnd =
          (void *)(ldInfo->ldinfo_textorg + ldInfo->ldinfo_textsize -
                   sizeof(struct AIXFunctionEpilogue));
      /* align our search for the word aligned null word that terminates the
       * procedure */
      struct tbtable *epilogue =
          (struct tbtable *)(iar & ((~(uintptr_t)0) << 5));
