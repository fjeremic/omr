...
/**
 * Look in the specified directory for the core file that was generated by the
 * OS and return the full path to it.
 *
 * @param[in] 	portLibrary pointer to the port library
 * @param[in] 	corepath the directory where the core file is expected to be
 * found
...
void appendCoreName(OMRPortLibrary* portLibrary, char* corepath, intptr_t pid) {
  ...
  while ((entry = readdir(folder))) {
     ...
     * unless we're on AIX, in which case if core compression is enabled, they
     * can be of the form:
 ...
 * @param[in] filename the requested filename. Provide NULL string if none
 * specified
...
intptr_t genSystemCoreUsingGencore(struct OMRPortLibrary* portLibrary,
  ...
  if (*filename != '/') {
    /* filename is not an absolute path, get the cwd to prepend the filename
     * with */
    char cwd[EsMaxPath];
    ...
    if (NULL != getcwd(cwd, EsMaxPath)) {
      ...
      /*  append cwd with the filename, then copy the resulting absolute path
       * back into filename */
  ...
  portLibrary->tty_printf(
      portLibrary,
      "\tomrdump_create: attempting to generate corefile, filename: %s\n",
      coreDumpInfo.name);
  fflush(stdout);
#endif
...
#if defined(DUMP_DBG)
  portLibrary->tty_printf(
      portLibrary, "\tomrdump_create: gencore/coredump returned: %i\n", rc);
  if (rc == -1) {
  ...
  if (rc == 0) {
    ...
    if (0 == strncmp(value, "on", strlen("on"))) {
      /* the core file will have been compressed and the name will have been
       * appended with ".Z" */
 ...
 *
 * @param[out] coreFilePath caller allocated buffer to put preferred destination
 * for core file.
 *
 ...
 */
void findOSPreferredCoreDir(struct OMRPortLibrary* portLibrary,
                            char* coreFilePath) {
  int i, bufferLen;
  ...
  /* path via the chcore command takes precedence, if it was not set, check the
   * path set via the syscorepath command*/
  if (buffer[0] == '\0') {
 ...
 * @param[in] envVar user-allocated buffer containing environment variable to
 query.
