...
#define J9TPF_SLAVE_OUTSTANDING                                                \
  0x02                                /* The pthread spawned to			*/
...
#define J9TPF_FLAGS_ERR_MASK                                                   \
  (J9TPF_OUT_OF_BUFFERSPACE | J9TPF_JDUMPBUFFER_LOCKED |                       \
   J9TPF_NO_JAVA_DUMPBUFFER | J9TPF_FILE_SYSTEM_ERROR)
...
void*
ztpfBuildCoreFile(
  void* argv); /* This decl is set up to take a
 ...
 */
extern void
ztpfDeriveSiginfo(siginfo_t* build) __attribute__((nonnull));
extern void
translateInterruptContexts(args* argv) __attribute__((nonnull));
extern uint8_t*
allocateContextStorage(void);
extern void
...
#define DBTIVADDR                                                              \
  (curr->ijavsad) /** The starting EVA of the data region.                     \
                   */
#define DBTIOFFS                                                               \
  (curr->ijavsbuf) /** The starting address in our local copy buffer.	*/
#define DBTISIZ                                                                \
  (curr->ijavsiz) /** The size of the memory region                            \
                   */
#define DBTIGAP                                                                \
  (curr->ijavgap) /** The gap field                                            \
                   */
/*
 ...
 */
#define DBTITYPE                                                               \
  (curr->ijavdesc + 35)           /** A byte of bitfields, as follows:	*/
 ...
 *	nothing to z/TPF; we don't turn over control to the system interpreter
 *(we don't even HAVE one) like *ix systems do. So instead, we're going to
 *	fulfill the requirement for an NT_AUXV segment with a fake one of 304
 *zeros.
 */
...
typedef struct shortsiginfo_t
{                    /* This is the leading struct in ELF		*/
...
typedef struct
{                     /* Process state information block			*/
