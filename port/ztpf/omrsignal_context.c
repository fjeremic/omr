...
static void ztpfDeriveSigcontext(struct sigcontext *sigcPtr,
                                 ucontext_t *uctPtr);
...
void ztpfDeriveSiginfo(siginfo_t *build) {
  uint16_t pic;
  uint16_t ilc;
  uint8_t dxc;
  ...
  pic = s390FetchPIC(
      lowcore); /* Translate the PIC into si_signo & si_code	*/
  ...
  switch (build->si_signo) {
    ...
    case SIGSEGV:
      if (build->si_code == SEGV_MAPERR && pEri) {
        ...
        build->si_addr =
            (void *)*address; /* Move the failed address.		*/
...
static void ztpfDeriveSigcontext(struct sigcontext *sigcPtr,
  ...
  memcpy(&(sigcPtr->sregs->regs.gprs), &(uctPtr->uc_mcontext.gregs),
         sizeof(uctPtr->uc_mcontext.gregs));
  memcpy(&(sigcPtr->sregs->regs.acrs), &(uctPtr->uc_mcontext.aregs),
         sizeof(uctPtr->uc_mcontext.aregs));

  memcpy(&(sigcPtr->sregs->fpregs), &(uctPtr->uc_mcontext.fpregs),
         sizeof(fpregset_t));
...
static void ztpfDeriveUcontext(ucontext_t *uscPtr) {
  ...
  memcpy(uscPtr->uc_mcontext.fpregs.fprs, pDib->dfreg,
         sizeof(pDib->dfreg));          /*FPRs*/
  uscPtr->uc_flags = ecbp2()->ce2thnum; /* Put the TID in ucontext->uc_flags  */
  sigprocmask(SIG_SETMASK, NULL,
              &(uscPtr->uc_sigmask)); /* Our mask is the old mask  */
  return;
}
 ...
 */
void translateInterruptContexts(args *argv) {
 ...
 */
U_32 infoForSignal(struct OMRPortLibrary *portLibrary,
                   struct OMRUnixSignalInfo *info, int32_t index,
 ...
 */
U_32 infoForGPR(struct OMRPortLibrary *portLibrary,
                struct OMRUnixSignalInfo *info, int32_t index,
                const char **name, void **value) {
 ...
 *
 */
U_32 infoForControl(struct OMRPortLibrary *portLibrary,
                    struct OMRUnixSignalInfo *info, int32_t index,
                    const char **name, void **value) {
  uint8_t *eip;
  mcontext_t *mcontext =
...
U_32 infoForModule(struct OMRPortLibrary *portLibrary,
                   struct OMRUnixSignalInfo *info, int32_t index,
                   const char **name, void **value) {
