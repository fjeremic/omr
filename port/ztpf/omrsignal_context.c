...
/*
 *	What follows is a set of signal number & signal code values to which a
 *s390 PIC should translate. For multiply-assignable high-level PICs, leave the
 *	si_code field zero; we can try to clear it up after PIC translation.
 *This table was pretty well matched up with how CPSE treats PICs, apart from
 ...
 */
static const sigvpair_t pic2pair[] = {
  IMPOSSIBLE,               /*	0: Not assigned               */
 ...
 *
 * Prior to some testing, we discovered that allocating UNIX-formatted
 *signal-related context blocks on the stack wasn't such a wise idea once the
 *original faulting thread had gone back to the thread pool and its stack got
 *tromped all over. This fact was completely unknown until late in development.
 *
 * The solution was to grab some storage that wouldn't be subject to such
 *problems. The memory into which the .bss segment is assigned is a (set of)
 *master page(s) of hex zeros, and once written upon, is copied-on-write and
 *become the property of the PROCESS (don't want to raise any confusion here)
 *which did the writing. Therefore, this storage appears zero to each new
 *process which calls this module. NO SMP- RELATED CONCURRENCY CONCERNS EXIST;
 *one of the attractive features which forced us into this storage class. The
 *biggest users, by far, of this space are threads which have called the system
 *dump agent, whether they've taken an actual fault or not. We labor under the
 *presumption that the lifetime of these context blocks is well under 16
 *seconds, and TOD-stamp test the time of each block's allocation before reusing
 *it.
 *
 * This E/P must be exported! File module.xml is therefore modified
 *conditionally to export this label, and it will appear in omrprt.exp when it
 *is created for z/TPF, when the buildtools are built. z/TPF is the only known
 *system which needs this kind of allocation. It is used exclusively for the
 *translation of z/TPF-ish data into UNIX-ish context & signal blocks.
 *
 *
 * \returns		A uint8_t pointer sufficiently large to house all three
 *required UNIX- style context & signal information blocks. IT IS UP TO THE
 *CALLER TO PARTITION THIS BLOCK.
 */
...
reenter:
  ...
  rtnv->sigcontext.sregs =
    &(rtnv->_sregs); /* point to actual _sigregs struct */
...
ztpfDeriveSiginfo(siginfo_t* build)
{
  ...
  if (pDbf &&
      pDbf
        ->ijavcnt) { /* We have the full Error Recording Info, use it.	*/
  ...
  pic =
    s390FetchPIC(lowcore); /* Translate the PIC into si_signo & si_code	*/
  ...
  switch (build->si_signo) {
    ...
    case SIGSEGV:
      if (build->si_code == SEGV_MAPERR && pEri) {
        ...
        address = lowcore +
                  (unsigned long int)0x1b8; /*	at offset 0x1B8 into the ERI*/
    ...
    case SIGILL:
      build->si_addr =
        (char*)pDib->dispw[1]; /*	Failing PSW $IP			*/
      break;
    default:
      break;
  }
  build->si_pid =
    procPtr->iproc_pid; /* All of them need PID and			 */
  build->si_uid =
    procPtr->iproc_uid; /*	UID values, with TID & errno	 */
  build->si_ztpf_tid =
    ecbp2()->ce2thnum; /* values, too.					 */
...
ztpfDeriveSigcontext(struct sigcontext* sigcPtr, ucontext_t* uctPtr)
{
  ...
  memcpy(&(sigcPtr->sregs->regs.gprs),
         &(uctPtr->uc_mcontext.gregs),
         sizeof(uctPtr->uc_mcontext.gregs));
  memcpy(&(sigcPtr->sregs->regs.acrs),
         &(uctPtr->uc_mcontext.aregs),
         sizeof(uctPtr->uc_mcontext.aregs));

  memcpy(&(sigcPtr->sregs->fpregs),
         &(uctPtr->uc_mcontext.fpregs),
         sizeof(fpregset_t));
...
ztpfDeriveUcontext(ucontext_t* uscPtr)
{
  ...
  memcpy(uscPtr->uc_mcontext.gregs, pDib->direg, sizeof(pDib->direg)); /*GPRs*/
  memcpy(
    uscPtr->uc_mcontext.fpregs.fprs, pDib->dfreg, sizeof(pDib->dfreg)); /*FPRs*/
  uscPtr->uc_flags = ecbp2()->ce2thnum; /* Put the TID in ucontext->uc_flags  */
  sigprocmask(
    SIG_SETMASK, NULL, &(uscPtr->uc_sigmask)); /* Our mask is the old mask  */
  return;
}
 ...
 */
void
fillInUnixSignalInfo(struct OMRPortLibrary* portLibrary,
                     void* contextInfo,
                     struct OMRUnixSignalInfo* signalInfo)
{
  signalInfo->platformSignalInfo.context =
    (ucontext_t*)contextInfo; /* module info is filled on demand */
}
 ...
 * \param[in]	info			A pointer to the OMRUnixSignalInfo from
 *which the information is taken \param[in]	index			The
 *signal value assigned to the 'signal type'
 * \param[out]	name			The name of the signal type class,
 *double pointer to type char
 * \param[out]	value			The real (raw) signal number
 *
 * \returns		Unsigned 32-bit integer describing what the \ref value
 *return is, an address, or an address. Also can return an error indicator.
 */
U_32
    ...
    case 6:
      ...
      if (info->sigInfo->si_code > 0) {
        if ((info->sigInfo->si_signo == SIGSEGV)) {
          ...
          *value =
            &info->sigInfo->si_addr; /* This will be wrong for z/TPF until I
                                        figure out where to get the failing
                                        address.		*/
          return OMRPORT_SIG_VALUE_ADDRESS;
        }
      }
 ...
 *
 * This routine is passed an index value, and returns the name of the register
 *at that index location as the <TT>name</TT>. The <TT>value</TT> parameter
 *appears to receive the register * content from context information.
 *
 * \param[in]	portLibrary		Pointer to the OMRPortLibrary in use
 * \param[in]	info			Pointer to applicable OMRUnixSignalInfo
 *structure.
 * \param[in]	index			Number of the floating point register
 *sought. \param[out]	name			Double pointer to type char,
 *name of the register
 * \param[out]	value			Double pointer to type void, likely to
 *be last known content of the register, so it'd be a pointer to a pointer of
 *type <TT>double</TT>.
 */
uint32_t
infoForFPR(struct OMRPortLibrary* portLibrary,
           ...
           void** value)
{
  const char* n_fpr[NUM_REGS] = { "fpr0",  "fpr1",  "fpr2",  "fpr3",
                                  "fpr4",  "fpr5",  "fpr6",  "fpr7",
  ...
  *name = "";
  if ((index >= 0) && (index < NUM_REGS)) {
    *name = n_fpr[index];
    *value =
      &(info->platformSignalInfo.context->uc_mcontext.fpregs.fprs[index]);
 ...
 * \param[in]	portLibrary		Pointer to the OMRPortLibrary in use
 * \param[in]	info			Pointer to applicable OMRUnixSignalInfo
 *structure. \param[in]	index			Number of the general register
 *sought. \param[out]	name			Double pointer to type char,
 *name of the register
 * \param[out]	value			Double pointer to type void, last known
 *value stored in the register.
 *
 * \note	Original z/TPF POC had this routine guarded out and returning a
 *value of OMRPORT_SIG_VALUE_UNDEFINED, so it appeared that the signal was not
 *supported in z/TPF, no matter what it was.
 * \note	This routine was unguarded as part of making the CP emulate
 *kernel-space, UNIX- style implementations of reactions to program interrupts.
 * \note	There were guard macros here based on #ifndef OMR_ENV_DATA64 which
 *I've simply removed. If the guard value were true, there were 16 more
 *definition slots for the high parts of the general registers. All gone; we
 *don't support 32-bit hardware (even if we do grudgingly support AMODE=31).
 */
U_32
infoForGPR(struct OMRPortLibrary* portLibrary,
           ...
           void** value)
{
  const char* n_gpr[NUM_REGS] = { "gpr0",  "gpr1",  "gpr2",  "gpr3",
                                  "gpr4",  "gpr5",  "gpr6",  "gpr7",
 ...
 *
 * \param[in]	portLibrary		Pointer to the portability library in
 *use \param[in]	info			Pointer to OMRUnixSignalInfo
 *block \param[in]	index			The 'type class' of the signal
 *associated with this dump \param[out]	name			Dbl pointer to a
 *buffer large enough to hold an object name
 * \param[out]	value			Dbl pointer to a buffer large enough to
 *hold an object value
 *
 * \note	We have to trust that the pointers to <tt>name</tt> and
 *<tt>value</tt> are properly sized and permissioned.
 *
 */
U_32
