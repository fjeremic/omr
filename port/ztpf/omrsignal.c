 ...
 * signal deliveries</b>, and that's what this segment is about: <i>the context switch that would have taken place if
 * z/TPF supported the delivery of reliable signals to the signal handler</i>.
 *
...
static uint32_t ztpf_enableSignalHandlers(OMRPortLibrary* portLibrary)
{
  ...
  rc = createThreadWithCategory(&ztpf_signal_enabler_thd, 512 * 256, J9THREAD_PRIORITY_NORMAL, 0,
...
struct {
  ...
  int unixSignalNo;
} signalMap[] = { { OMRPORT_SIG_FLAG_SIGSEGV, SIGSEGV }, { OMRPORT_SIG_FLAG_SIGILL, SIGILL },
  { OMRPORT_SIG_FLAG_SIGFPE, SIGFPE }, { OMRPORT_SIG_FLAG_SIGQUIT, SIGIQUIT }, { OMRPORT_SIG_FLAG_SIGABRT, SIGABRT },
...
#endif
static uint32_t registerSignalHandlerWithOS(
  OMRPortLibrary* portLibrary, uint32_t portLibrarySignalNo, unix_sigaction handler);
...
void masterSynchSignalHandler(int signal, siginfo_t* sigInfo, void* contextInfo, UDATA breakingEventAddr)
{
  ...
  if (NULL != thisThread) {
    ...
    /* record this signal in tls so that jsig_handler can be called if any of the handlers decide we should be shutting
     * down */
 ...
 *
 * The old action for the signal handler is stored in oldActions. These must be restored before the portlibrary is shut
 * down.
 *
...
static int mapPortLibSignalToUnix(uint32_t portLibSignal)
{
  ...
  /* mask out subtypes */
  portLibSignal &= OMRPORT_SIG_FLAG_SIGALLSYNC | OMRPORT_SIG_FLAG_SIGQUIT | OMRPORT_SIG_FLAG_SIGABRT
    | OMRPORT_SIG_FLAG_SIGTERM | OMRPORT_SIG_FLAG_SIGXFSZ;
  for (index = 0; index < sizeof(signalMap) / sizeof(signalMap[0]); index++) {
...
static int32_t setReporterPriority(OMRPortLibrary* portLibrary, UDATA priority)
{
 ...
 */
int32_t omrsig_set_reporter_priority(struct OMRPortLibrary* portLibrary, UDATA priority)
{
