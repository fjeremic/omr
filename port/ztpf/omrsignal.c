 ...
 *
 * \details z/TPF does not have <i>reliable signals</i>, also known as
 * <i>kernel-space</i> signals in certain circles. Instead, all signals
 * processing takes place in user space. Normally, the act of delivering a
 * signal (via <tt>kill()</tt>, <tt>pthread_kill()</tt>, <tt>raise()</tt>, et
 * al) takes place in user-space with no supervisory intervention; and once it's
 * delivered, the process (or thread) must <i>declare itself ready to accept
 * signals</i> via either a <tt>tpf_process_signals()</tt> or
 * <tt>tpf_thread_process_signals()</tt> call. That's going to sound weird to a
 * modern UNIX programmer, but that's the way it is, and it actually
 * <i>works</i> for certain kinds of programs. Unfortunately, those programs
 * which expect to be rerouted to a signal handler without warning (or
 * consciously <i>"being ready" to process a signal</i>) aren't among that set
 * of code. In other words, there are no such things as 'synchronous signals' in
 * z/TPF: all units of work (UOW) must declare themselves ready and willing to
 * process any signals which may have been delivered to them since the last such
 * declaration through the issuance of a <tt>tpf_process_signals()</tt> call.
 *
 * UNIX, on which most of this segment was originally founded, _does_ have
 * reliable signals whereas z/TPF has none. Reliable signals is a way of saying
 * that when a signal is delivered and is NOT masked out, control is immediately
 * transferred to the signal handler registered for that particular signal -- it
 * spends no time in a 'signal pending' state. The way UNIX scheduling works, a
 * <i>"context switch"</i> is performed when one scheduleable unit of work
 * (hereafter, UOW) has the CPU taken from it and assigned to the next
 * scheduleable UOW. To put it another way, the scheduler will change the now
 * active UOW's <i>context</i> from "active, running" to "dormant, awaiting
 * rescheduling" (or whichever event it's awaiting, which could be an I/O
 * completion, for example).
 *
 * The dirty work of taking a CPU core away from a UOW is accomplished by
 * stopping the current CPU, taking a copy of the machine state into what's
 * called the <b><i>"machine state context"</i></b> (<tt>struct
 * uc_mcontext</tt>) consisting of the PSW, registers, etc, then setting its
 * <b>"link state"</b> context (<tt>struct uc_link</tt>) to the next executable
 * instruction the outgoing UOW would have executed, putting this UOW at the end
 * of the queue; then selecting the next runnable UOW, restoring <i>its</i>
 * <tt>uc_link</tt> data members to the proper places as dictated by hardware,
 * then causing execution to resume at the place <tt>uc_link</tt> says it should
 * until the next timeslice expires. <i>Ad infinitum</i>.
 *
 * One of the descriptive statistics of how hard a UNIX-like machine is working
 * is the <i>"context switch per second"</i> rate. A large *NIX server executes
 * approximately twice as many context switches as it does interrupts over any
 * small slice of time; some run with over 10,000 (and more) context switches
 * per second.
 *
 * z/TPF schedules very differently. For a timesliced process, which all JVM
 * threads belong to, interrupts happen many, many times a second, and at each
 * interrupt, the scheduler decides if the UOW has had its timeslice expire (or
 * for more traditional TPF processes, which perform the equivalent of a
 * <tt>yield()</tt> instruction for SCHED_FIFO scheduling, which is the original
 * scheduling paradigm of the z/TPF system. If so, the machine state is stored
 * in pages 2 and 3 of the Entry Control Block, and its scheduleable UOW address
 * (the <i>Entry Control Block</i> or <b>ECB</b>) is queued on one of at least
 * five "CPU lists", depending on what its next expected state is. This data
 * placement is the z/TPF equivalent of a UNIX context switch. But <b>the
 * current machine state has nothing to do with signal deliveries</b>, and
 * that's what this segment is about: <i>the context switch that would have
 * taken place if z/TPF supported the delivery of reliable signals to the signal
 * handler</i>.
 *
 * What we <em>do</em> have is the full machine state <b>when any dump is
 * taken.</b> Whether the dump is the result of what we've been calling a
 * "non-preemptible" event (i.e. a <i>program check</i>, meaning that the
 * process is going to exit shortly) or a "hooked" event (meaning that the Java
 * user has called for a dump on some otherwise normal event taking place, such
 * as a JVM start or stop, or perhaps a 'soft' Java exception having been
 * thrown, where function <TT>omrdump_create()</TT> has to call for a dump from
 * the z/TPF Control Program), we still have the <b>extremely recent</b> machine
 * state captured for us by the z/TPF SERRC processor, CCCPSE. Therefore, we
 * <i>can</i> create a 'fake' ucontext structure <b>as long as the data the
 * structure is to contain comes from a recent dump</b>.
 *
 * In this segment, we labor under the presumption that the required dump has
 * indeed been anticipated by the "kernel's" (we call the supervisor the "z/TPF
 * Control Program") program check interrupt routine. If such a dump has been
 * anticipated and the faulting UOW belongs to a process which is known to have
 * been bound to a JVM, CCCPSE (the Control Program's program check handler) has
 * created what's called a "Dump Interchange Record", out of which, in turn, is
 * anchored a "Java Dump Buffer", or JDB.
 *
 * CCCPSE determines that eligible program checks fall into one of two
 * categories. The first category is called a "preemptible error", one which the
 * Java language has specified as a divide-by-zero or null-pointer exception
 * (DBZ or NPE, respectively). In these situations, the Java interpreter routine
 * receiving the signal is supposed to know what to do with it, namely issue the
 * 'soft' Java ArithmeticException("Divide-by-zero") or NullPointerException,
 * depending upon the exact type of interrupt taken. The TPF-ish data is
 * translated into its corresponding UNIX-information, such as "context blocks"
 * required for each of these two situations. In these cases, no JDB is anchored
 * in the DIB, as no system dump agent activity will take place.
 *
 * The second category is called a "non-preemptible error", one which CCCPSE
 * cannot classify as preemptible and is associated with a Java program under
 * interpretation or a byte interpreter malfunction. These kinds of program
 * checks indicate something seriously wrong, such as a programming or data
 * error, and infers that the JVM may not be in a condition to continue. In this
 * kind of case, a JDB is anchored off the DIB in anticipation of the system
 * dump agent being required to write an ELF core dump file.
 *
 * The JDB contains in-memory copies of all the native stack frames, an Error
 * Information Record, and has made in-memory copies of every heap (malloc()'d
 * heap nodes) in use by the faulting address space. We write our own ELF core
 * dump file from this information since the z/TPF kernel does not have this
 * task as one of its duties like a UNIX kernel would.
 *
 * Regardless of the kind of error CCCPSE has determined we have, we always have
 * a DIB, which contains enough information to create the required context
 * blocks associated with a synchronous signal. If the error is not preemptible,
 * we have also spawned a thread to write the ELF core dump file, in
 * anticipation that it will be required by the system dump agent's execution,
 * to follow.
 */
...
static uint32_t ztpf_enableSignalHandlers(OMRPortLibrary *portLibrary) {
  int rc = J9THREAD_SUCCESS;
  omrthread_t ztpf_signal_enabler_thd;

  rc = createThreadWithCategory(
      &ztpf_signal_enabler_thd, 512 * 256, J9THREAD_PRIORITY_NORMAL, 0,
      &ztpf_fireSignalHandlers, portLibrary, J9THREAD_CATEGORY_SYSTEM_THREAD);
...
/*
 * z/TPF does not have kernel threads. As such, signals are delivered reliably
 * to a process' or thread's pending signal queue; but the signal handler -- if
 * any -- for any signal does not receive control until a function called
 * tpf_process_signals() is called. The usleep() function does this on its way
 * into -- and back from -- an EVNWC service whose timeout value we pass to it
 * in microseconds; we've #defined it as WAIT_QUANTUM above. Net-net, all we
 * have to do is usleep, and we get any pending signals processed. This is a
 * much lower-overhead operation than a pthread_cond_timedwait() operation or
 * similar; even much lower than a tight DEFRC loop.
 *
...
static int J9THREAD_PROC ztpf_fireSignalHandlers(void *portlib) {
  OMRPortLibrary *portLibrary = (OMRPortLibrary *)portlib;

  omrthread_monitor_init_with_name(&ztpfSigShutdownMonitor, 0,
                                   "ztpf_thread_enabler_shutdown_monitor");
  ...
  while (1) { /* Never break this loop		*/
    if (0 == shutdown_ztpf_sigpush) {
      ...
      omrthread_exit(
          ztpfSigShutdownMonitor); /* This thread is gone. */
    }
  }
...
/* Records the (port library defined) signals for which we have registered a
 * master handler.
...
struct {
  ...
  int unixSignalNo;
    ...
    {OMRPORT_SIG_FLAG_SIGSEGV, SIGSEGV}, {OMRPORT_SIG_FLAG_SIGILL, SIGILL},
    {OMRPORT_SIG_FLAG_SIGFPE, SIGFPE},   {OMRPORT_SIG_FLAG_SIGQUIT, SIGIQUIT},
    {OMRPORT_SIG_FLAG_SIGABRT, SIGABRT}, {OMRPORT_SIG_FLAG_SIGTERM, SIGTERM}};

static omrthread_t asynchSignalReporterThread = NULL;
static int32_t registerMasterHandlers(OMRPortLibrary *portLibrary,
                                      uint32_t flags,
                                      uint32_t allowedSubsetOfFlags);
...
#endif
static uint32_t registerSignalHandlerWithOS(OMRPortLibrary *portLibrary,
                                            uint32_t portLibrarySignalNo,
...
static uint32_t countInfoInCategory(struct OMRPortLibrary *portLibrary,
...
void masterSynchSignalHandler(int signal, siginfo_t *sigInfo, void *contextInfo,
                              UDATA breakingEventAddr);
static void masterASynchSignalHandler(int signal, siginfo_t *sigInfo,
                                      void *contextInfo, UDATA nullArg);

int32_t omrsig_can_protect(struct OMRPortLibrary *portLibrary, uint32_t flags) {
  ...
  if (OMR_ARE_NO_BITS_SET(signalOptionsGlobal,
  ...
  if (OMR_ARE_ALL_BITS_SET(supportedFlags, flags)) {
    Trc_PRT_signal_omrsig_can_protect_exiting_is_able_to_protect(
        supportedFlags);
    return 1;
  }

  Trc_PRT_signal_omrsig_can_protect_exiting_is_not_able_to_protect(
      supportedFlags);
  return 0;
}

uint32_t omrsig_info(struct OMRPortLibrary *portLibrary, void *info,
...
uint32_t omrsig_info_count(struct OMRPortLibrary *portLibrary, void *info,
                           uint32_t category) {
 ...
 */
int32_t omrsig_protect(struct OMRPortLibrary *portLibrary,
                       omrsig_protected_fn fn, void *fn_arg,
                       omrsig_handler_fn handler, void *handler_arg,
                       uint32_t flags, UDATA *result) {
  ...
  Trc_PRT_signal_omrsig_protect_entered(fn, fn_arg, handler, handler_arg,
                                        flags);

  if (OMR_ARE_ANY_BITS_SET(signalOptionsGlobal,
                           OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS)) {
    /* -Xrs was set, we can't protect against any signals, do not install the
     * master handler */
  ...
  /*
   * Check to see if we have already installed a masterHandler for these
   * signal(s). We will need to aquire the masterHandlerMonitor if
   * flagsWithoutMasterHandlers is not 0 in order to install the handlers (and
   * to check if another thread had "just" taken care of it).
   *
   * If we do have masterHandlers installed already then
   * OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS has not been set.
   */
  ...
  if (0 != flagsWithoutMasterHandlers) {
    /*
     * We have not installed handlers for all the signals, so acquire the
     * masterHandlerMonitor, check again, then install handlers as required: all
     * done in registerMasterHandlers
     */
    omrthread_monitor_enter(masterHandlerMonitor);
    rc =
        registerMasterHandlers(portLibrary, flags, OMRPORT_SIG_FLAG_SIGALLSYNC);
  ...
  if (OMR_ARE_ANY_BITS_SET(flags, OMRPORT_SIG_FLAG_MAY_RETURN)) {
    ...
    /*
     * Record the current signal. We need to store this value back into tls if
     * we jump back into this function because any signals that may have
     * occurred within the scope of this layer of protection would have been
     * handled by that point.
     *
     * The only scenario where this is of real concern, is if more than one
     * signal was handled per call to omrsig_protect. In this case, the current
     * signal in tls will be pointing at a stale stack frame and signal: CMVC
     * 126838
     */
    currentSignal = omrthread_tls_get(thisThread, tlsKeyCurrentSignal);
    /*
     * setjmp/longjmp does not clear the mask setup by the OS when it delivers
     * the signal. Use sigsetjmp/siglongjmp(buf, 1) instead
     */
  ...
  if (0 != omrthread_tls_set(thisThread, tlsKey, &thisRecord)) {
    Trc_PRT_signal_omrsignal_sig_protect_Exit_ERROR_accessing_tls(
        fn, fn_arg, handler, handler_arg, flags);
    return OMRPORT_SIG_ERROR;
  }
...
int32_t omrsig_set_async_signal_handler(struct OMRPortLibrary *portLibrary,
                                        omrsig_handler_fn handler,
  ...
  Trc_PRT_signal_omrsig_set_async_signal_handler_entered(handler, handler_arg,
                                                         flags);
  ...
  if (OMR_ARE_ANY_BITS_SET(signalOptionsGlobal,
    ...
    if (OMR_ARE_ANY_BITS_SET(flags, OMRPORT_SIG_FLAG_SIGXFSZ) &&
        OMR_ARE_ANY_BITS_SET(signalOptionsGlobal,
                             OMRPORT_SIG_OPTIONS_SIGXFSZ)) {
      rc = registerMasterHandlers(portLibrary, OMRPORT_SIG_FLAG_SIGXFSZ,
    ...
    rc = registerMasterHandlers(portLibrary, flags,
                                OMRPORT_SIG_FLAG_SIGALLASYNC);
  }
  omrthread_monitor_exit(masterHandlerMonitor);
  ...
  while (NULL != cursor) {
    if ((cursor->portLib == portLibrary) && (cursor->handler == handler) &&
      ...
      if (0 == flags) { /* Remove the listener, but not masterHandlers, which
                           get removed at omrsignal shutdown */
  ...
  if (NULL == cursor) { /* cursor will only be NULL if we failed to find it in
                           the list */
    if (0 != flags) {
      OMRUnixAsyncHandlerRecord *record = portLibrary->mem_allocate_memory(
          portLibrary, sizeof(*record), OMR_GET_CALLSITE(),
  ...
  Trc_PRT_signal_omrsig_set_async_signal_handler_exiting(handler, handler_arg,
                                                         flags);
  return rc;
}

int32_t omrsig_set_single_async_signal_handler(
    struct OMRPortLibrary *portLibrary, omrsig_handler_fn handler,
    void *handler_arg, uint32_t portlibSignalFlag, void **oldOSHandler) {
  return OMRPORT_SIG_ERROR;
}

uint32_t omrsig_map_os_signal_to_portlib_signal(
    struct OMRPortLibrary *portLibrary, uint32_t osSignalValue) {
  return 0;
}

int32_t omrsig_map_portlib_signal_to_os_signal(
    struct OMRPortLibrary *portLibrary, uint32_t portlibSignalFlag) {
  return OMRPORT_SIG_ERROR;
}

int32_t omrsig_register_os_handler(struct OMRPortLibrary *portLibrary,
...
int32_t omrsig_is_signal_ignored(struct OMRPortLibrary *portLibrary,
                                 uint32_t portlibSignalFlag,
 ...
 *
 * Note: none of the master handlers are registered with the OS until the first
 *call to either omrsig_protect or omrsig_set_async_signal_handler.
 */
int32_t omrsig_startup(struct OMRPortLibrary *portLibrary) {
  int32_t result = 0;
  ...
  if (J9THREAD_SUCCESS != ztpf_enableSignalHandlers(portLibrary)) {
    portLibrary->tty_err_printf(
        portLibrary, "JVMZTPF01E Cannot enable tpf_process_signals()");
  }
  /*
...
static uint32_t countInfoInCategory(struct OMRPortLibrary *portLibrary,
  ...
  while (portLibrary->sig_info(portLibrary, info, category, count, &name,
                               &value) != OMRPORT_SIG_VALUE_UNDEFINED) {
    count++;
  }
  return count;
}
 ...
 */
static int32_t J9THREAD_PROC asynchSignalReporter(void *userData) {
  OMRUnixAsyncHandlerRecord *cursor;
  ...
  for (;;) {
    ...
    /*
     * report the signal recorded in signalType to all registered listeners (for
     * this signal) incrementing the asyncThreadCount will prevent the list from
     * being modified while we use it
     */
    omrthread_monitor_enter(asyncMonitor);
    asyncThreadCount++;
    omrthread_monitor_exit(asyncMonitor);
    ...
    if (OMR_ARE_NO_BITS_SET(signalOptionsGlobal,
      ...
      int unixSignal = mapPortLibSignalToUnix(
          asyncSignalFlag); /* mapPortLibSignalToUnix returns */
...
void masterSynchSignalHandler(int signal, siginfo_t *sigInfo, void *contextInfo,
  ...
  if (NULL != thisThread) {
    ...
    /* Trc_PRT_signal_masterSynchSignalHandler_entered(signal, sigInfo,
     * contextInfo); */
    ...
    /* record this signal in tls so that jsig_handler can be called if any of
     * the handlers decide we should be shutting down */
    ...
    while (NULL != thisRecord) {
      if (OMR_ARE_ANY_BITS_SET(thisRecord->flags, portLibType)) {
        ...
        /*
         * the equivalent of these memsets were here before, but were they
         * needed?
         */
        ...
        /*
         * remove the handler we are about to invoke, now, in case the handler
         * crashes
         */
        omrthread_tls_set(thisThread, tlsKey, thisRecord->previous);
#if 0
...
#endif
        result = thisRecord->handler(thisRecord->portLibrary, portLibType,
                                     &signalInfo, thisRecord->handler_arg);
#if 0
  ...
  /* The only ways to get here are: (1) if this thread was not attached to the
   * thread library or (2) the thread hasn't registered any signal handlers with
   * the port library that could handle the signal
   */
  if (OMR_ARE_NO_BITS_SET(signalOptionsGlobal,
                          OMRPORT_SIG_OPTIONS_OMRSIG_NO_CHAIN)) {
#if 0
    ...
    if ((OMRSIG_RC_DEFAULT_ACTION_REQUIRED == rc) &&
  ...
  /*
   * If we got this far there weren't any handlers on the stack that knew what
   * to with this signal, so the default action is to abort
   */
#if 0
 ...
 *
 * One semaphore is used to notify the asynchronous signal reporter that it is
 * time to act. Each expected aynch signal type has an associated semaphore
 * which is used to count the number of "pending" signals.
 *
 */
static void masterASynchSignalHandler(int signal, siginfo_t *sigInfo,
                                      void *contextInfo, UDATA nullArg) {
...
/**
 * Register the signal handler with the OS, generally used to register the
 * master signal handlers Not to be confused with omrsig_protect, which
 * registers the user's handler with the port library.
 *
 * Calls to this function must be synchronized using "masterHandlerMonitor".
 *
 * The use of this function forces the flags SA_RESTART | SA_SIGINFO |
 * SA_NODEFER to be set for the new signal action
 *
 * The old action for the signal handler is stored in oldActions. These must be
 * restored before the portlibrary is shut down.
 *
...
static uint32_t registerSignalHandlerWithOS(OMRPortLibrary *portLibrary,
  ...
  /*
   * Setting to SA_SIGINFO will result in "void (*sa_sigaction) (int, siginfo_t
   * *, void *)" to be used, and not "__sighandler_t sa_handler". Both are
   * members of struct sigaction, using the former allows us to access much more
   * information than just the signal number
   */
  newAction.sa_flags |= SA_SIGINFO;

  /*
   * SA_NODEFER prevents the current signal from being masked by default in the
   * handler. However, it can still be masked if one explicitly requests so in
   * sa_mask field, as is done on z/OS
   */
  newAction.sa_flags |= SA_NODEFER;

  /*
   * The master exception handler:
   *		The (void *) casting only applies to zLinux because a new parameter
   *"UDATA breakingEventAddr" has been introduced in masterSynchSignalHandler()
   *to obtain BEA on zLinux but not for other platforms. As a result, the total
   *number of parameters in masterSynchSignalHandler() on zLinux & z/TPF is 4
   *while the number is 3 on other platforms, because there is no need to change
   *the signature of masterSynchSignalHandler in there. Since the code is shared
   *on all platforms, the change here is used to split them up to avoid any
   *compiling error.
   */
  newAction.sa_sigaction = (void *)handler;

  /* now that we've set up the sigaction struct the way we want it, register the
   * handler with the OS */
  if (OMRSIG_SIGACTION(unixSignalNo, &newAction,
                       &oldActions[unixSignalNo].action)) {
    Trc_PRT_signal_registerSignalHandlerWithOS_failed_to_registerHandler(
        portLibrarySignalNo, unixSignalNo, handler);
    return -1;
  } else {
    Trc_PRT_signal_registerSignalHandlerWithOS_registeredHandler(
        portLibrarySignalNo, unixSignalNo, handler);
    oldActions[unixSignalNo].restore = 1;
  }

  /* CMVC 96193 signalsWithMasterHandlers is checked without acquiring the
   * masterHandlerMonitor */
  issueWriteBarrier();
  /*
 ...
 *	siginfo_t passed by reference is already built. If so, do not rebuild
 *it.
 *
 *	Some signals have subtypes which are detailed in the siginfo_t
 *structure. That's fine for other platforms, but not too likely on z/TPF; and
 *even if so, probably doesn't distinguish between floating point, integer, and
 *	packed decimal datatype errors (for example). So go ahead and do it
 *	ourselves -- that way, we're sure.
 *
 *	The only signal value we do this for is SIGFPE; all the others just
 *return what they're passed.
 *
 *	\param[in]	signalNo	The signal number as passed back to the
 *signal handler. \param[in]	sigInfo		The <tt>siginfo_t</tt> structure
 *as passed back to the upper-layer signal handler.
 *
 *	\note	Make sure to test the sigInfo pointer, treat it as non-extant if
 *zero. Go ahead and write on it if it's there. If we have one, we'll store the
 *signal value and the translated PIC into it.
 */
uint32_t mapUnixSignalToPortLib(uint32_t signalNo, siginfo_t *sigInfo) {
  uint32_t index;
  ...
  for (index = 0; index < sizeof(signalMap) / sizeof(signalMap[0]); index++) {
    if (signalMap[index].unixSignalNo == signalNo) {
      ...
      /*	Otherwise, do translation from the JDB if there's one of those
       * as long		*/
      /*	as we have a DIB pointer belonging to this thread from which to
       * build it.	*/
      /*	If these conditions aren't met, then just return it to the
       * caller verbatim. */

      if ((0 != sigInfo->si_signo || 0 != sigInfo->si_code) && dbfPtr) {
        ztpfDeriveSiginfo(
            sigInfo);                 /* Build a new siginfo_t				*/
 ...
 * signal number.
 *
 * Note that FPE signal codes (subtypes) all map to the same signal number and
 * are not included
 *
 * \param[in] portLibSignal The internal port library signal number
 * \return The corresponding Unix signal number or -1 if the portLibSignal could
 * not be mapped
 */
static int mapPortLibSignalToUnix(uint32_t portLibSignal) {
  ...
  /* mask out subtypes */
  portLibSignal &= OMRPORT_SIG_FLAG_SIGALLSYNC | OMRPORT_SIG_FLAG_SIGQUIT |
                   OMRPORT_SIG_FLAG_SIGABRT | OMRPORT_SIG_FLAG_SIGTERM |
                   OMRPORT_SIG_FLAG_SIGXFSZ;
  for (index = 0; index < sizeof(signalMap) / sizeof(signalMap[0]); index++) {
...
/**
 * \fn Registers the master handler for the signals in flags that don't have
 *one.
 *
 * Calls to this function must be synchronized using masterHandlerMonitor.
 *
 * @param[in] portLibrary			Address of this JVM's
 *OMRPortLibrary
 * @param[in] flags					The flags that we want signals
 *for
 * @param[in] allowedSubsetOfFlags	Must be either
 ...
 *handler for a signal that is not included in the allowedSubsetOfFlags
 */
static int32_t registerMasterHandlers(OMRPortLibrary *portLibrary,
                                      uint32_t flags,
                                      uint32_t allowedSubsetOfFlags) {
  ...
  if (0 != flagsWithoutHandlers) {
     ...
     */
    for (portSignalType = OMRPORT_SIG_SMALLEST_SIGNAL_FLAG;
         ((portSignalType < allowedSubsetOfFlags) && (portSignalType != 0));
      ...
      if (OMR_ARE_ANY_BITS_SET(flagsWithoutHandlers, portSignalType)) {
        ...
        if (0 !=
            registerSignalHandlerWithOS(portLibrary, portSignalType, handler)) {
          return OMRPORT_SIG_ERROR;
        }
      }
    }
...
static int32_t initializeSignalTools(OMRPortLibrary *portLibrary) {
   ...
   * omrsig_handler in omrexit_shutdown_and_exit */
  if (omrthread_tls_alloc(&tlsKeyCurrentSignal)) {
    return -1;
  }

  if (omrthread_monitor_init_with_name(
          &masterHandlerMonitor, 0,
          "portLibrary_omrsig_masterHandler_monitor")) {
    return -1;
  }

  if (omrthread_monitor_init_with_name(
          &asyncReporterShutdownMonitor, 0,
          "portLibrary_omrsig_asynch_reporter_shutdown_monitor")) {
    return -1;
  }

  if (omrthread_monitor_init_with_name(&asyncMonitor, 0,
                                       "portLibrary_omrsig_async_monitor")) {
    return -1;
  }
  ...
  /* The asynchronous signal reporter keeps track of the number of pending
   * singals with these... */
...
#if defined(OMR_PORT_ASYNC_HANDLER)
  if (J9THREAD_SUCCESS !=
      createThreadWithCategory(&asynchSignalReporterThread, 256 * 1024,
                               J9THREAD_PRIORITY_MAX, 0, &asynchSignalReporter,
                               NULL, J9THREAD_CATEGORY_SYSTEM_THREAD)) {
    return -1;
  }
#endif
  return 0;
...
static int32_t setReporterPriority(OMRPortLibrary *portLibrary,
                                   UDATA priority) {
 ...
 */
int32_t omrsig_set_reporter_priority(struct OMRPortLibrary *portLibrary,
                                     UDATA priority) {
  int32_t result = 0;
...
int32_t omrsig_set_options(struct OMRPortLibrary *portLibrary,
                           uint32_t options) {
  Trc_PRT_signal_omrsig_set_options(options);

  if ((OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS |
       OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_ASYNCHRONOUS) &
      options) {
...
uint32_t omrsig_get_options(struct OMRPortLibrary *portLibrary) {
  return signalOptionsGlobal;
}

intptr_t omrsig_get_current_signal(struct OMRPortLibrary *portLibrary) {
  OMRCurrentSignal *currentSignal =
      omrthread_tls_get(omrthread_self(), tlsKeyCurrentSignal);
...
static void sig_full_shutdown(struct OMRPortLibrary *portLibrary) {
  ...
  globalMonitor = omrthread_global_monitor();
  omrthread_monitor_enter(globalMonitor);
  if (--attachedPortLibraries ==
      0) { /* register the old actions we overwrote with our own */
    for (index = 0; index < MAX_UNIX_SIGNAL_TYPES; index++) {
      if (oldActions[index].restore) {
        OMRSIG_SIGACTION(index, &oldActions[index].action, NULL);
        /* record that we no longer have a handler installed with the OS for
         * this signal */
        Trc_PRT_signal_sig_full_shutdown_deregistered_handler_with_OS(
            portLibrary, index);
...
/*	@internal
 *
 * omrexit_shutdown_and_exit needs to call this to ensure the signal is chained
 * to omrsig (the application handler) in the case when the shutdown is due to a
 * fatal signal.
 *
 * @return
 */
void omrsig_chain_at_shutdown_and_exit(struct OMRPortLibrary *portLibrary) {
  OMRCurrentSignal *currentSignal =
      omrthread_tls_get(omrthread_self(), tlsKeyCurrentSignal);
  ...
  if (currentSignal != NULL) {
    /* we are shutting down due to a signal, forward it to the application
     * handlers */
    if (!(signalOptionsGlobal & OMRPORT_SIG_OPTIONS_OMRSIG_NO_CHAIN)) {
      ...
      omrsig_handler(currentSignal->signal, currentSignal->sigInfo,
