...
static uint32_t
ztpf_enableSignalHandlers(OMRPortLibrary*);
static int J9THREAD_PROC
ztpf_fireSignalHandlers(void*);
...
ztpf_enableSignalHandlers(OMRPortLibrary* portLibrary)
{
  ...
  rc = createThreadWithCategory(&ztpf_signal_enabler_thd,
                                512 * 256,
                                J9THREAD_PRIORITY_NORMAL,
                                0,
                                &ztpf_fireSignalHandlers,
                                portLibrary,
...
ztpf_fireSignalHandlers(void* portlib)
{
  ...
  omrthread_monitor_init_with_name(
    &ztpfSigShutdownMonitor, 0, "ztpf_thread_enabler_shutdown_monitor");
...
static int32_t
registerMasterHandlers(OMRPortLibrary* portLibrary,
                       uint32_t flags,
...
#endif
static uint32_t
registerSignalHandlerWithOS(OMRPortLibrary* portLibrary,
...
static uint32_t
countInfoInCategory(struct OMRPortLibrary* portLibrary,
                    void* info,
                    uint32_t category);
static void
...
void
masterSynchSignalHandler(int signal,
                         siginfo_t* sigInfo,
                         void* contextInfo,
                         UDATA breakingEventAddr);
static void
masterASynchSignalHandler(int signal,
                          siginfo_t* sigInfo,
                          void* contextInfo,
                          UDATA nullArg);
...
uint32_t
omrsig_info(struct OMRPortLibrary* portLibrary,
            void* info,
            uint32_t category,
            int32_t index,
            const char** name,
...
uint32_t
omrsig_info_count(struct OMRPortLibrary* portLibrary,
                  void* info,
                  uint32_t category)
{
 ...
 */
int32_t
omrsig_protect(struct OMRPortLibrary* portLibrary,
               ...
               UDATA* result)
{
  ...
  Trc_PRT_signal_omrsig_protect_entered(
                                ...
                                uint32_t flags)
{
  ...
  Trc_PRT_signal_omrsig_set_async_signal_handler_entered(
  ...
  if (OMR_ARE_ANY_BITS_SET(signalOptionsGlobal,
    ...
    if (OMR_ARE_ANY_BITS_SET(flags, OMRPORT_SIG_FLAG_SIGXFSZ) &&
      ...
      rc = registerMasterHandlers(
        portLibrary, OMRPORT_SIG_FLAG_SIGXFSZ, OMRPORT_SIG_FLAG_SIGALLASYNC);
    } else {
    ...
    rc =
      registerMasterHandlers(portLibrary, flags, OMRPORT_SIG_FLAG_SIGALLASYNC);
  }
  omrthread_monitor_exit(masterHandlerMonitor);
  ...
  if (NULL == cursor) { /* cursor will only be NULL if we failed to find it in
                           the list */
    if (0 != flags) {
      OMRUnixAsyncHandlerRecord* record =
  ...
  Trc_PRT_signal_omrsig_set_async_signal_handler_exiting(
    handler, handler_arg, flags);
  return rc;
}
                    ...
                    uint32_t category)
{
  ...
  while (
    portLibrary->sig_info(portLibrary, info, category, count, &name, &value) !=
  ...
  for (;;) {
    ...
    while (NULL != cursor) {
      if (OMR_ARE_ANY_BITS_SET(cursor->flags, asyncSignalFlag)) {
        ...
        cursor->handler(
          cursor->portLib, asyncSignalFlag, NULL, cursor->handler_arg);
      }
      cursor = cursor->next;
    }
    omrthread_monitor_enter(asyncMonitor);
...
#endif
        result = thisRecord->handler(thisRecord->portLibrary,
                                     portLibType,
                                     &signalInfo,
                                     thisRecord->handler_arg);
#if 0
...
initializeSignalTools(OMRPortLibrary* portLibrary)
{
  ...
  if (omrthread_monitor_init_with_name(
        &masterHandlerMonitor, 0, "portLibrary_omrsig_masterHandler_monitor")) {
    return -1;
  }
  ...
  if (omrthread_monitor_init_with_name(
        &asyncMonitor, 0, "portLibrary_omrsig_async_monitor")) {
    return -1;
  }
...
#if defined(OMR_PORT_ASYNC_HANDLER)
  if (J9THREAD_SUCCESS !=
      createThreadWithCategory(&asynchSignalReporterThread,
                               256 * 1024,
...
omrsig_chain_at_shutdown_and_exit(struct OMRPortLibrary* portLibrary)
{
  ...
  if (currentSignal != NULL) {
    ...
    if (!(signalOptionsGlobal & OMRPORT_SIG_OPTIONS_OMRSIG_NO_CHAIN)) {
      ...
      omrsig_handler(currentSignal->signal,
                     currentSignal->sigInfo,
