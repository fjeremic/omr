...
uintptr_t backtrace_sigprotect(struct OMRPortLibrary *portLibrary,
                               ...
                               void **address_array, int capacity);
...
/*
 * NULL handler. We only care about preventing the signal from propagating up
 * the call stack, no need to do anything in the handler.
 */
static uintptr_t handler(struct OMRPortLibrary *portLibrary, uintptr_t gpType,
...
/*
 * Provides signal protection for the libc backtrace call. If the stack walk
 * causes a segfault then we check the output array to see if we got any frames
 * as we may be able to provide a partial backtrace. We also record that the
 * stack walk was incomplete so that information is available after the fuction
 * returns.
 */
uintptr_t backtrace_sigprotect(struct OMRPortLibrary *portLibrary,
                               J9PlatformThread *threadInfo,
  ...
  if (omrthread_self()) {
    if (portLibrary->sig_protect(
            portLibrary, protectedBacktrace, &args, (omrsig_handler_fn)handler,
            NULL, OMRPORT_SIG_FLAG_SIGALLSYNC | OMRPORT_SIG_FLAG_MAY_RETURN,
            &ret) != 0) {
...
uintptr_t omrintrospect_backtrace_thread_raw(struct OMRPortLibrary *portLibrary,
  ...
  /*
   * z/TPF Stack grows down.  The top-of-stack has a dummy frame which should be
   * ignored. Therefore reducing stack frame count by one.
   */
  ret--;
 ...
 * j9introspect_backtrace_thread and looks up the symbols for the frames. The
  ...
  for (i = 0, frame = threadInfo->callstack; frame != NULL;
    ...
    if (symbol_length > 0) {
      ...
      cursor += omrstr_printf(portLibrary, cursor,
                              ...
                              "%.*s", symbol_length, asciiSymbolBuffer);
      cursor += omrstr_printf(portLibrary, cursor,
                              sizeof(output_buf) - (cursor - output_buf),
                              "+0x%x ", symbol_offset);
    }
    ...
    if (module_name[0] != '\0') {
      ...
      cursor += omrstr_printf(portLibrary, cursor,
                              sizeof(output_buf) - (cursor - output_buf),
                              " [%s+0x%x]", asciiModuleBuffer, module_offset);
    }
    *(cursor++) = ')';
    *cursor = 0;
