 ...
 */
static uint16_t buildELFPheaderBlk(Elf64_Phdr* buffer, DBFHDR* dbfHdr, uint64_t phnum, uint64_t* numBytes)
{
  ...
  *numBytes = netOffset - (sizeof(Elf64_Ehdr) + ((phnum + 1 + numJavaPgms) * sizeof(Elf64_Phdr)) + NOTE_TABLE_SIZE);
  int i = 0;
  for (i = 0; i < numJavaPgms; i++) {
    struct pat* pgmpat = progc(javaPgmsToDump[i], PROGC_PBI);
    if (pgmpat != NULL) {
...
static uintptr_t buildELFNoteArea(Elf64_Nhdr* buffer, DIB* dibPtr)
{
  ...
  sprintf((char*)wDest, "SE-%4.4d OPR-%c%6.6X PGM-%4.4s %5.5c %8.8s", eriPtr->eriseq, eriPtr->eridpfx, eriPtr->eridnum,
    eriPtr->eridate, eriPtr->eritime);
  UNKEY();
  return tableSize; /* We're done.                */
}
