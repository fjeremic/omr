...
extern void
translateInterruptContexts(args* argv) __attribute__((nonnull));
extern void
masterSynchSignalHandler(int nbr,
                         siginfo_t* siginfo,
                         void* ucontext,
                         uintptr_t bear);
...
static uint16_t
buildELFPheaderBlk(Elf64_Phdr* buffer,
                   DBFHDR* dbfHdr,
                   uintptr_t phnum,
                   uint64_t* numBytes);
static uintptr_t
...
static void
errMsg(args* argv, uint8_t* message, ...) __attribute__((nonnull(1, 2)));
static uint64_t
...
adjustDBFTable(DBFHDR* dbfHdr)
{
  ...
  while (curr < limit) {
    memset(
      DBTITYPE, 0, sizeof(char) + (sizeof(DBTIGAP))); /* Clear DBTI tail */
...
#define MOUNT_TFS 4 /* TPF Filesystem equate from imount.h */
  ...
  if (-1 == octetsWritten) {
    errMsg(
      arg, (uint8_t*)"Error writing dump file %s: %s", ofn, strerror(errno));
  ...
  for (i = 0; i < numJavaPgms; i++) {
    ...
    if (pgmpat != NULL) {
      ...
      if (pgmbase != NULL) {
        ...
        if (-1 == octetsWritten) {
          errMsg(arg,
                 (uint8_t*)"Error writing dump file %s: %s",
                 ofn,
                 strerror(errno));
          dumpFlags(arg) |= J9TPF_FILE_SYSTEM_ERROR;
          KEY0();
  ...
  if (-1 == rc) {
    errMsg(arg,
           (uint8_t*)"I/O error attempting to close %s:%s\n",
           ofn,
           strerror(errno));
    KEY0();
    dbfptr->ijavcnt = 0L; /* Unlock the JDB so CCCPSE can reuse it*/
...
buildELFNoteArea(Elf64_Nhdr* buffer, DIB* dibPtr)
{
  ...
  sprintf((char*)wDest,
          "SE-%4.4d OPR-%c%6.6X PGM-%4.4s %5.5c %8.8s",
          eriPtr->eriseq,
          eriPtr->eridpfx,
          eriPtr->eridnum,
          eriPtr->eridate,
...
ztpf_preemptible_helper(void)
{
  ...
  masterSynchSignalHandler(
    s->siginfo.si_signo, &(s->siginfo), &(s->ucontext), s->pDIB->dbrevn);
  /*
...
ztpf_nonpreemptible_helper(void)
{
  ...
  do {
    ...
    pthread_sigmask(
      SIG_BLOCK, NULL, &oldmask); /* Test the signal mask again.    */
