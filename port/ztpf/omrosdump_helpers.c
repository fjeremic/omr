...
char javaPgmsToDump[][5] = {
    /* JVM 2.9 programs */
    "DJHK", "DJNC", "DJVB", "DJCK", "DJHS", "DJPR", "DJVM", "DJDD", "DJHT",
    "DJRD", "DJZL", "DJDP", "DJHV", "DJSG", "D9VM", "DJHY", "DJSH", "DJ7B",
    "DJGC", "DJIT", "DJTH", "DJAR", "DJGK", "DJMT", "DJTR", "DJFF", "DJXT",
    "DJOR",

    /* JCL programs */
    "DJVA", "DJRM", "DJNM", "DJAT", "DJAW", "DJAH", "DJDB", "DJDC", "DJDS",
    "DJFM", "DJHP", "DJIN", "DJAV", "DJCD", "DJJW", "DJWP", "DJLI", "DJPG",
    "DJSD", "DJSN", "DJKC", "DJLC", "DJMN", "DJML", "DJNT", "DJIO", "DJPT",
    "DJTK", "DJUN", "DJVR", "DJWR", "DJZP"};
...
static uint16_t buildELFPheaderBlk(Elf64_Phdr *buffer, DBFHDR *dbfHdr,
                                   uintptr_t phnum, uint64_t *numBytes);
static uintptr_t buildELFNoteArea(Elf64_Nhdr *buffer, DIB *dibPtr);
static void errMsg(args *argv, uint8_t *message, ...)
 ...
 * \retval nz    These items are not adjacent, the distance in bytes between
 * them.
 */
static void calcGap(DBFITEM *curr, DBFITEM *next, uint64_t *gap) {
...
/**
 * \internal Make sure the DBF table, which points to all the memory regions
 * collected by CCCPSE, is as compact as possible. The goal is to cut the number
 * of ELF Phdrs down as much as possible by merging adjoining memory regions. In
 * order to do this, it needs to mark each DBFITEM, which is in protected
 * memory, SPK=0. This function is therefore in SPK=0 for its duration, and
 * restores SPK1 prior to return.
 *
 ...
 */
static uintptr_t adjustDBFTable(DBFHDR *dbfHdr) {
  DBFITEM *dbfTbl =
      (DBFITEM *)dbfHdr + 1; /* DBTI[0] starts right after dbfHdr    */
  ...
  while (curr < limit) {
    memset(DBTITYPE, 0,
           sizeof(char) + (sizeof(DBTIGAP))); /* Clear DBTI tail            */
    ...
    if (0 == wGap) {
      ...
      /*
       *    This pair does not qualify to be combined. Is it last in set, or is
       * its predecessor not part of this one?
       */
 ...
 *
 * \param[in]  finalname    Buffer containing full path of the final filename.
 * (const) \param[out] pathname        Ptr to user-acquired buffer of length
 * PATH_MAX for the path.
 *
 ...
 * \param[in]        arg->OSFilename        Pointer to scratch storage for a
 * path + file name string. It will be presumed to be at least PATH_MAX+1 in
 * size. \param[in]        arg->wkspcSize        32-bit quantity representing
 * the size in bytes of the wkSpace field, following. \param[in] arg->wkSpace
 * 64-bit pointer to scratch workspace for use by this function. It is
 * recommended to make it equal to PATH_MAX, since file & path names will be
 * built in this space. \param[in][out]    arg->flags            Indicators as
 * to what is present and what is not. \param[in]        arg->sii Pointer to
 * scratch storage to be used forever more as a siginfo_t block \param[in]
 * arg->uct            Pointer to scratch storage to be used forever more as a
 * ucontext_t block \param[in]        arg->sct            Pointer to scratch
 * storage to be used forever more as a sigcontext block \param[in]
 * arg->portLibrary    Pointer to an initialized OMRPortLibrary block. If there
 * isn't one at call time, leave this value NULL and set flag
 *                                        J9ZTPF_NO_PORT_LIBRARY
 * \param[in]        arg->dibPtr            Address of the DIB attached to the
 * faulting UOW at post-interrupt time.
 *
 ...
 */
void *ztpfBuildCoreFile(void *argv_block) {
#define MOUNT_TFS 4 /* TPF Filesystem equate from imount.h */
          ...
          dumpSiginfo(arg)->si_pid);
  {
    ...
    if (fsystype == MOUNT_TFS) {
      errMsg(arg,
             (uint8_t *)"Cannot use the tfs for java dumps: path used='%s'\n",
             (uint8_t *)pathName);
      returnCode(arg) = -1;
      KEY0();
      dbfptr->ijavcnt = 0L; /* Unlock the JDB so CCCPSE can reuse it*/
  ...
  wPtr += ((phCount + 1 + numJavaPgms) *
           sizeof(Elf64_Phdr));    /*    of the Elf64_Phdr section        */
  ...
  for (i = 0; i < numJavaPgms; i++) {
    ...
    if (pgmpat != NULL) {
      ...
      if (pgmbase != NULL) {
        int offset =
            pgmbase->_iftch_txt_off + pgmbase->_iftch_txt_size + 0x1000;
   ...
   */
  rc = chmod((const char *)workSpace(arg),
   ...
   * avoid a mem leak.
   */
  strcpy((char *)dumpFilename(arg),
         ofn);        /* Store it in the args block so the */
...
/**
 * \internal Build the ELF64 header. This is really simple, just overlay the
 * input buffer region with a set of constants. We will worry about the number
 * of Program Headers later.
 *
 * \param[in]  buffer    Address of writeable memory large enough to hold the
 * ELF64_Ehdr structure. \param[out] buffer    As above, filled in for the
 * sizeof(Elf64_Ehdr).
 *
 * \return
 ...
 *    We'll have to calculate the offset value and keep a running total,
 * starting from p_offset for the PT_NOTE segment, then incrementing it by each
 *    p_filesz value.
 *
...
static uint16_t buildELFPheaderBlk(Elf64_Phdr *buffer, DBFHDR *dbfHdr,
  ...
  int numJavaPgms = sizeof(javaPgmsToDump) / 5;
  uint64_t netOffset =
      sizeof(Elf64_Ehdr) + ((phnum + 1 + numJavaPgms) * sizeof(Elf64_Phdr));
  ...
  while (dbfiCount > 0) {
    ...
    phdrPtr->p_vaddr =
        (uintptr_t)DBTIVADDR; /*  ... the virtual memory address        */
    phdrPtr->p_filesz = phdrPtr->p_memsz =
        DBTISIZ;          /*  ... and memory sizes.        */
  ...
  *numBytes = netOffset - (sizeof(Elf64_Ehdr) +
 ...
 *
 * \note The final sub-section, tagged <i><tt>NT_ZTPF_SERRC</tt></i>, is
 * invented for this routine's purposes: its role is to express the SE number
 * associated with the already-written traditional SERRC dump as requested
 * during design. Its existence may require addition of a method to the Java
 * class which processes ELF NOTEs in <b><tt>jdmpview</tt></b> - there is no ELF
 * NOTE tag otherwise suited for this purpose. Otherwise, this information stays
 * buried.
 *
...
static uintptr_t buildELFNoteArea(Elf64_Nhdr *buffer, DIB *dibPtr) {
   ...
   *    past the DBFHDR, which just so happens to be the same length as a
   * DBFITEM itself.
   */
   ...
   *     as to make the commonly-used typecasting approach cumbersome and
   * difficult to understand. Often working pointer fields which have a length
   * of 1 for address arithmetic uses are the easiest or best way to do this yet
   * still have legible code. We use all three of the following data objects in
   * that vein.
   */
   ...
   * precision, we'd be amplifying the error. And since we have to divide by
   * 1000 anyway, we might as well divide by 4098 (would shifting right 12 bits,
   * for 4096, be close enough? No. That would cause us to lose a full
   * millisecond in precision error!) to be accurate. So that's what we're doing
   * below, since we have to eat the cycles in the division operation anyway --
   * might as well use the right divisor. Never mind rounding up with the
   * remainder; we're already generating a larger result than reality because we
   * can't go fractional. Does it really matter? Nah. But if you're gonna do it,
   * you might as well do it right ....
   */
  prstat->stime.tv_usec =
      *((unsigned long long *)(tpf_get_ecb_field(CE1ISTIM_FIELD))) / 4098;
  ...
  sprintf((char *)wDest, "SE-%4.4d OPR-%c%6.6X PGM-%4.4s %5.5c %8.8s",
          eriPtr->eriseq, eriPtr->eridpfx, eriPtr->eridnum, eriPtr->eridate,
          eriPtr->eritime);
  UNKEY();
  return tableSize; /* We're done.                */
}

void ztpf_preemptible_helper(void) {
...
void ztpf_nonpreemptible_helper(void) {
  safeStorage *s = allocateDurableStorage();
  uint8_t holdkey;
  sigset_t newmask;
  sigset_t oldmask;
  ...
  sigaddset(&newmask,
            s->siginfo.si_signo); /* Build a constant signal mask word        */

  do {
    PROC_LOCK(&(s->pPROC->iproc_JVMLock), holdkey);
    if (s->pPROC->iproc_tdibptr) { /* Are we blocked because we're already
                                      working    */
      PROC_UNLOCK(&(s->pPROC->iproc_JVMLock),
                  holdkey); /* another signal? If so,unlock    */
      usleep(10000);        /* and sleep for 10 ms.            */
    }
    pthread_sigmask(SIG_BLOCK, NULL,
                    &oldmask); /* Test the signal mask again.    */
   ...
   */
  s->pPROC->iproc_tdibptr =
      (uint64_t)s->pDIB; /*    Stash the faulting DIB pointer            */
  PROC_UNLOCK(&(s->pPROC->iproc_JVMLock), holdkey);
  /*
   * Call the master structured signal handler for synchronous signals.
   * It's not possible that an async signal can get here. The signal handler
   * will either hand off execution to the JIT, or will run the dump agents. Do
   * not expect a return; else the normal SIG_DFL (bye, bye) rule applies.
   */
  masterSynchSignalHandler(s->siginfo.si_signo, &(s->siginfo), &(s->ucontext),
                           s->pDIB->dbrevn);
  /*
