               ...
               void* userData)
{
  ...
  if (!(pDBF)) { /* Is there a JDB?              */
    serrc_op_ext(
      SERRC_RETURN, 0x4006, NULL, '\xc9', NULL); /* No, go create one.   */
  ...
  if (!JVMfn) { /* Uh oh. There was an error. What to say?      */
    ...
    portLibrary->tty_err_printf(
      portLibrary, "unable to write ELF core dump: %s\n", errMsg);
    ...
    if ((s->argv.rc < 0) && (errno == ENOENT)) {
      ...
      if (s->argv.rc < 0) {
        portLibrary->tty_err_printf(
          portLibrary, "Failed to rename: %s\n", oldPath);
      }
