  ...
  if (!JVMfn) { /* Uh oh. There was an error. What to say?      */
    if (s->argv.flags & J9TPF_ERRMSG_IN_WKSPC) {
      ...
      case J9TPF_OUT_OF_BUFFERSPACE:
        ...
        break;
      default:
        sprintf((const char*)workspace, "unantipated err flag value (0x%X). errno=%x.", s->argv.flags, s->argv.rc);
    ...
    if ((s->argv.rc < 0) && (errno == ENOENT)) {
      ...
      if (targetDirectory == NULL) {
        portLibrary->tty_err_printf(portLibrary, "Problem determining directory for %s\n", JVMfn);
      }
