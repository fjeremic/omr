...
static uintptr_t get_hugepages_info(struct OMRPortLibrary *portLibrary,
  ...
  while (line_ptr && *line_ptr) {
    tokens_assigned =
        sscanf(line_ptr, "%127s %zu %*s", token_name, &token_value);
...
void *default_pageSize_reserve_memory(struct OMRPortLibrary *portLibrary,
...
#endif
  {
...

    {
      ...
      uintptr_t allocSize =
          (uintptr_t)GET_4K_ALIGNED_ALLOCATION_SIZE(byteAmount);
...
void *default_pageSize_reserve_memory_32bit(
...
static void *getMemoryInRangeForDefaultPages(
  ...
  if (OMR_ARE_ANY_BITS_SET(vmemOptions, OMRPORT_VMEM_ALLOC_QUICK)) {
    ...
    if (1 == direction) {
      smartAddress = findAvailableMemoryBlockNoMalloc(
          portLibrary, currentAddress, endAddress, byteAmount, FALSE);
    } else {
      smartAddress = findAvailableMemoryBlockNoMalloc(
          portLibrary, startAddress, currentAddress, byteAmount, TRUE);
    }

    if (NULL == smartAddress) {
      /* None of the available regions are suitable. There's no point in
       * performing the linear search below: it would (slowly) arrive at the
       * same conclusion.
       */
      return NULL;
    }
  ...
  if (NULL == memoryPointer) {
    ...
    if (0 != (vmemOptions & OMRPORT_VMEM_ZTPF_USE_31BIT_MALLOC)) {
      memoryPointer = default_pageSize_reserve_memory_32bit(
...
intptr_t omrvmem_numa_get_node_details(struct OMRPortLibrary *portLibrary,
  ...
  if (PPG_numa_platform_supports_numa) {
    ...
    if (NULL == nodes) {
          ...
          if (nodeIndex < PPG_numa_max_node_bits) {
            ...
            if (NULL != oneNode) {
              ...
              if (populatedNodeCount < arraySize) {
                ...
                while ((0 == readdir_r(oneNode, &cpuStorage, &fileEntry)) &&
                       (NULL != fileEntry)) {
      ...
      /* save back how many entries we saw (the caller knows that the number of
       * entries we populated is the minimum of how many we were given and how
       * many we found) */
