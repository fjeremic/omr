 ...
 * It's a combination o spinlock techniques modified with blocking
...
/*
 * This function destroys a barrier created by barrier_init_r. It first wakes up
 * any waiters then waits for them to exit the barrier before returning if
 * requested.
 *
 * @param barrier the barrier to destroy
 * @int block if non-zero the barrier will block until waiters have exited
 */
static void barrier_destroy_r(barrier_r *barrier, int block) {
  int current = 0;
  int in = 0;
  char byte = 1;
 ...
 * scheduling. Also it may not be capped on all systems).
 *
...
/*
 * This functions resumes all threads suspended by suspend_all_preemptive. The
...
static void resume_all_preempted(struct PlatformWalkData *data) {
  ...
  if (data->cleanupRequired) {
    ...
    clean_release = barrier_release_r(&data->release_barrier,
                                      timeout(data->state->deadline2));
...
int __attribute__((optimize(0))) J9ZTPF_getcontext(void *region) {
   ...
   *      This block needs to point at the *caller's* NSI, so R14's contents
   * need to replace bytes 8-15 of the PSW, we'll do that too.
   */
  asm volatile(
      /*
       *              This section writes the primary ucontext_t block fields
       * the user called for, using all 512 bytes of storage we were passed.
       *              First we set uc_flags & uc_link to zero, as well as the
       * entire region occupied by uc_stack.
       */
      "xc  0(40,%0),0(%0)\n\t" /* Zeroize uc_flags & uc_link */
      /*
       *              Next portion is the _sigregs (field: uc_mcontext) block
       *              It starts at offset 0x0028, where the PSW goes. But we
       * need to save the caller's registers first thing, so do that first.
       */
      "stmg %%r0,%%r10,56(%0)\n\t"    /* store grandes 0-10 at  +0x0038 for 0x58
                                         bytes */
      "mvc 144(48,%0),544(%%r11)\n\t" /*              grandes 11-15 at +0x0090
                                         for 0x28 bytes */
      ...
      "xc  252(4,%0),252(%0)\n\t" /* Zeroize 4 bytes' alignment padding (+0x0FC
                                     for 4)     */
  ...
  sigprocmask(SIG_BLOCK, NULL,
              &((ucontext_t *)region)->uc_sigmask); /* Get sig masks        */
  /*
 ...
 * assumed to have received the SUSPEND_SIG signal but did not. z/TPF doesn't
 * have kernel-space signals and therefore there are _no_ reliable signals in
 * the system at all: always return 0.
 */
static uintptr_t sigqueue_is_reliable(void) { return 0; }
...
J9PlatformThread *omrintrospect_threads_startDo_with_signal(
  ...
  /*
   *	The following preprocessor guard will stop all attempts at attempting to
   *walk the native thread stacks, thus avoiding some pretty sticky code which
   *may or may not work on z/TPF. We'll leave this gate open for now, with a
   *pending decision to close it or not.
   */
