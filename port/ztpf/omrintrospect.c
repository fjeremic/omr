...
int __attribute__((optimize(0))) J9ZTPF_getcontext(void* region)
{
  ...
  asm volatile(
     ...
     *              user called for, using all 512 bytes of storage we were
     * passed. First we set uc_flags & uc_link to zero, as well as the entire
     *              region occupied by uc_stack.
     */
    "xc  0(40,%0),0(%0)\n\t" /* Zeroize uc_flags & uc_link */
    /*
     *              Next portion is the _sigregs (field: uc_mcontext) block
     *              It starts at offset 0x0028, where the PSW goes. But we need
     * to save the caller's registers first thing, so do that first.
     */
    "stmg %%r0,%%r10,56(%0)\n\t"    /* store grandes 0-10 at  +0x0038 for 0x58
                                       bytes */
    "mvc 144(48,%0),544(%%r11)\n\t" /*              grandes 11-15 at +0x0090 for
                                       0x28 bytes */
  ...
  sigprocmask(
    SIG_BLOCK, NULL, &((ucontext_t*)region)->uc_sigmask); /* Get sig masks */
  /*
