 ...
 */
static const inline uint16_t
s390FetchILC(uint8_t* b)
{
  uint16_t* q = (uint16_t*)(b + 0x8c);
  return (*q & 0x06);
}
/**
 ...
 */
static const inline uint16_t
s390FetchPIC(uint8_t* b)
{
  uint16_t* q = (uint16_t*)(b + 0x8e);
  return (*q & 0x13f);
}
...
infoForFPR(struct OMRPortLibrary* portLibrary,
           ...
           int32_t index,
           const char** name,
           void** value) __attribute__((nonnull(1, 2, 4, 5)));
