 ...
 */
static const inline uint16_t s390FetchILC(uint8_t* b)
{
 ...
 * action context.
 */
static const inline uint16_t s390FetchPIC(uint8_t* b)
{
 ...
 * of characters meant to represent offset zero in low memory.
 */
static const inline uint8_t s390FetchDXC(uint8_t* b)
{
  return *(b + 0x93);
}
...
uint32_t infoForFPR(struct OMRPortLibrary* portLibrary, struct OMRUnixSignalInfo* info, int32_t index,
  const char** name, void** value) __attribute__((nonnull(1, 2, 4, 5)));
