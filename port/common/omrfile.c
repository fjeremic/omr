 ...
 *                      If the mode argument requests write access for all of owner, group, and world, the READONLY
 * attribute is cleared. Otherwise READONLY is set.
 *
 ...
 * 					if the Windows READONLY bit is set and 0666 (owner, group, and world readable and writable)if READONLY is not
 * set.
 *
 * @note If a multi-threaded application passes in a relative path the caller must synchronize on the current working
 * directory, otherwise the behaviour is undefined
