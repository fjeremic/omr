 ...
 *
 * If an OS system call fails the error code reported by the OS needs to be
 * saved for future reference. The OS error code as well as the corresponding
 * portable error code are stored in the per thread buffers @ref omrportptb.h
 * via a call the @ref omrerror_set_last_error or @ref
 * omrerror_set_last_error_with_message. These functions are meant for private
 * use by the port library, to set the error related to a system call failure.
 * They are in the port library table as people overriding port library
 * functions may need to set the error message accordingly.
 *
 * The majority of applications are not interested in the human readable error
 * message corresponding to the error.  As a result the error message is not
 * stored at time of the reported error, but can be looked up at a later time.
 */
...
static const char *swapMessageBuffer(PortlibPTBuffers_t ptBuffers,
                                     const char *message) {
 ...
 * that are required for the error handling operations may be created here.  All
 * resources created here should be destroyed in @ref omrerror_shutdown.
 *
 * @param[in] portLibrary The port library
 *
 * @return 0 on success, negative error code on failure.  Error code values
 * returned are \arg OMRPORT_ERROR_STARTUP_ERROR
 *
 * @note Most implementations will simply return success.
 */
int32_t omrerror_startup(struct OMRPortLibrary *portLibrary) { return 0; }
/**
 * PortLibrary shutdown.
 *
 * This function is called during shutdown of the portLibrary.  Any resources
 * that were created by @ref omrerror_startup should be destroyed here.
 *
 * @param[in] portLibrary The port library
 *
 ...
 */
const char *omrerror_last_error_message(struct OMRPortLibrary *portLibrary) {
 ...
 */
int32_t omrerror_last_error_number(struct OMRPortLibrary *portLibrary) {
  PortlibPTBuffers_t ptBuffers;
 ...
 * @brief Error Handling
 *
 * Save the platform specific error code and the portable error code for future
 * reference.  Once stored an application may obtain the error message
 * describing the last stored error by calling omrerror_last_error_message.
 * Likewise the last portable error code can be obtained by calling
 * omrerror_last_error_number.
 *
 * @param[in] portLibrary The port library
 * @param[in] platformCode The error code reported by the OS
 * @param[in] portableCode The corresponding portable error code as determined
 * by the caller
 *
 ...
 *
 * @note If per thread buffers @see omrportptb.h are not available the error
 * code is not stored. This event would only occur if the per thread buffers
 * could not be allocated, which is highly unlikely.  In this case an
 * application will receive a generic message/errorCode when querying for the
 * last stored values.
 */
int32_t omrerror_set_last_error(struct OMRPortLibrary *portLibrary,
 ...
 * @brief Error Handling
 *
 * Save the portable error code and corresponding error message for future
 * reference.  Once stored an application may obtain the error message
 * describing the last stored error by calling omrerror_last_error_message.
 * Likewise the last portable error code can be obtained by calling
 * omrerror_last_error_number.
 ...
 * last stored values.
 */
int32_t omrerror_set_last_error_with_message(struct OMRPortLibrary *portLibrary,
  ...
  /* Store the message, allocate a bigger buffer if required.  Keep the old
   * buffer around just in case memory can not be allocated
   */
  requiredSize = (uint32_t)strlen(errorMessage) + 1;
  requiredSize = requiredSize < J9ERROR_DEFAULT_BUFFER_SIZE
                     ? J9ERROR_DEFAULT_BUFFER_SIZE
                     : requiredSize;
  if (requiredSize > ptBuffers->errorMessageBufferSize) {
    char *newBuffer = portLibrary->mem_allocate_memory(
        portLibrary, requiredSize, OMR_GET_CALLSITE(),
  ...
  if (ptBuffers->errorMessageBufferSize > 0) {
    portLibrary->str_printf(portLibrary, ptBuffers->errorMessageBuffer,
                            ptBuffers->errorMessageBufferSize, "%s",
                            errorMessage);
 ...
 * @note If per thread buffers @see omrportptb.h are not available the error
 * code is not stored. This event would only occur if the per thread buffers
 * could not be allocated, which is highly unlikely. In this case an application
 * will receive a generic message/errorCode when querying for the last stored
 * values.
 */
int32_t omrerror_set_last_error_with_message_format(
  ...
  requiredSize = (requiredSize < J9ERROR_DEFAULT_BUFFER_SIZE)
                     ...
                     : requiredSize;
  if ((requiredSize > ptBuffers->errorMessageBufferSize) &&
      (J9ERROR_MAXIMUM_BUFFER_SIZE > requiredSize)) {
    char *newBuffer = portLibrary->mem_allocate_memory(
        portLibrary, requiredSize, OMR_GET_CALLSITE(),
        OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL != newBuffer) {
  ...
  /* Save the message -- if we failed allocate an appropriate size buffer above
   * it may be truncated into preexisting buffer*/
  if ((NULL != ptBuffers->errorMessageBuffer) &&
      (ptBuffers->errorMessageBufferSize > 0)) {
    uintptr_t sizeWritten = 0;
    sizeWritten = portLibrary->str_vprintf(
        portLibrary, ptBuffers->errorMessageBuffer,
        ptBuffers->errorMessageBufferSize, format, args);
