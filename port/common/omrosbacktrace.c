 ...
 */
static uintptr_t handler(struct OMRPortLibrary* portLibrary, uint32_t gpType, void* gpInfo, void* userData)
{
  return OMRPORT_SIG_EXCEPTION_RETURN;
}

/* Wrapper to unpack structure into arguments */
static uintptr_t protectedIntrospectBacktraceThread(struct OMRPortLibrary* port, void* arg)
{
 ...
 */
uintptr_t omrintrospect_backtrace_thread(
  struct OMRPortLibrary* portLibrary, J9PlatformThread* threadInfo, J9Heap* heap, void* signalInfo)
{
  uintptr_t ret;
  ...
  if (omrthread_self()) {
    if (portLibrary->sig_protect(portLibrary, protectedIntrospectBacktraceThread, &args, handler, NULL,
...
/* Wrapper to unpack structure into arguments */
static uintptr_t protectedIntrospectBacktraceSymbols(struct OMRPortLibrary* port, void* arg)
{
  struct threadData* args = (struct threadData*)arg;
 ...
 */
uintptr_t omrintrospect_backtrace_symbols(
  struct OMRPortLibrary* portLibrary, J9PlatformThread* threadInfo, J9Heap* heap)
{
  uintptr_t ret;
  struct threadData args;
  ...
  if (omrthread_self()) {
    if (portLibrary->sig_protect(portLibrary, protectedIntrospectBacktraceSymbols, &args, handler, NULL,
          OMRPORT_SIG_FLAG_SIGALLSYNC | OMRPORT_SIG_FLAG_MAY_RETURN, &ret)
      == 0) {
      return ret;
    } else {
