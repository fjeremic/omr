 ...
 * of chars. It is converted to the appropriate platform encoding.
 *
 * @param[in] portLibrary The port library
 ...
 */
int32_t omrfile_write_text(struct OMRPortLibrary *portLibrary, intptr_t fd,
                           const char *buf, uintptr_t nbytes) {
  intptr_t result =
 ...
 * of the character set to. In practice, this string will be generally less than
 * 64 chars but this cannot be guaranteed.
 * @param[in] nbytes the size of charsetName. A terminating null is appended, so
 * on success at most nbytes-1 will be written.
 *
 * @return 0 on success, negative on failure, positive return code when
 * character set is available but charsetName was too short to store. The
 * required space is given by the return code.
 */
