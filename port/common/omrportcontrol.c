                ...
                uintptr_t value)
{
  ...
  if (!strcmp("SIG_INTERNAL_HANDLER", key)) {
    ...
    extern int structuredExceptionHandler(struct OMRPortLibrary * portLibrary,
                                          omrsig_handler_fn handler,
                                          void* handler_arg,
                                          uint32_t flags,
                                          EXCEPTION_POINTERS* exceptionInfo);
    *(int (**)(struct OMRPortLibrary*,
               omrsig_handler_fn,
               void*,
               uint32_t,
  ...
  if (!strcmp("SIG_INTERNAL_HANDLER", key)) {
    if (portLibrary->sig_protect == omrsig_protect_ceehdlr) {
      ...
      *(void (**)(
        _FEEDBACK * fc, _INT4 * token, _INT4 * leResult, _FEEDBACK * newfc))
        value = omrsig_le_condition_handler;
      return 0;
    } else {
      return 1;
    }
  ...
  if (!strcmp(OMRPORT_CTLDATA_MEM_CATEGORIES_SET, key)) {
    ...
    if (0 == value) {
      ...
      portControl->language_memory_categories.categories =
        portLibrary->mem_allocate_memory(OMRPORTLIB,
                                         (languageCategoryCount) *
      ...
      memset(portControl->language_memory_categories.categories,
             0,
             (languageCategoryCount) * sizeof(OMRMemCategory*));
      memset(portControl->omr_memory_categories.categories,
             0,
