...
int32_t omrport_control(struct OMRPortLibrary *portLibrary, const char *key,
                        uintptr_t value) {
  ...
  if (!strcmp(OMRPORT_CTLDATA_ALLOCATE32_COMMIT_SIZE, key)) {
    if (0 != value) {
      ...
      if (0 == PPG_mem_mem32_subAllocHeapMem32.suballocator_commitSize) {
        /* Round up the commit size to the page size and set it to global
         * variable */
        uintptr_t pageSize =
            portLibrary->vmem_supported_page_sizes(portLibrary)[0];
  ...
  if (!strcmp("SIG_INTERNAL_HANDLER", key)) {
    ...
    extern int structuredExceptionHandler(
    ...
    *(int (**)(struct OMRPortLibrary *, omrsig_handler_fn, void *, uint32_t,
  ...
  if (!strcmp("SIG_INTERNAL_HANDLER", key)) {
    if (portLibrary->sig_protect == omrsig_protect_ceehdlr) {
       ...
       * the port library overrides sig_protect itself (with
       * omrsig_protect_ceehdlr) when the option
       * OMRPORT_SIG_OPTIONS_ZOS_USE_CEEHDLR is passed into omrsig_set_options
       */
      extern void omrsig_le_condition_handler(
          _FEEDBACK * fc, _INT4 * token, _INT4 * leResult, _FEEDBACK * newfc);

      *(void (**)(_FEEDBACK * fc, _INT4 * token, _INT4 * leResult,
                  _FEEDBACK * newfc)) value = omrsig_le_condition_handler;
      return 0;
    } else {
      return 1;
    }
  ...
  if (!strcmp(OMRPORT_CTLDATA_MEM_CATEGORIES_SET, key)) {
    ...
    /* Allow categories to be reset to NULL (for testing purposes) - but not
    ...
    if (0 == value) {
      ...
      /* The port library knows the port library will include it's own
       * allocations so we will need an array at least large enough for that. */
      uint32_t omrCategoryCount =
          OMRMEM_OMR_CATEGORY_INDEX_FROM_CODE(OMRMEM_CATEGORY_PORT_LIBRARY);
...
#endif
      /* Now sort the categories into two sets < OMRMEM_LANGUAGE_CATEGORY_LIMIT
       * and > OMRMEM_LANGUAGE_CATEGORY_LIMIT */
      ...
      for (i = 0; i < categories->numberOfCategories; i++) {
        ...
        if (categoryCode < OMRMEM_LANGUAGE_CATEGORY_LIMIT) {
          languageCategoryCount = (categoryCode > languageCategoryCount)
                                      ? categoryCode
                                      : languageCategoryCount;
        } else if (categoryCode > OMRMEM_LANGUAGE_CATEGORY_LIMIT) {
          uint32_t categoryIndex =
              OMRMEM_OMR_CATEGORY_INDEX_FROM_CODE(categoryCode);
          omrCategoryCount = (categoryIndex > omrCategoryCount)
                                 ? categoryIndex
                                 : omrCategoryCount;
        }
      }
      ...
      portControl->language_memory_categories.categories =
          portLibrary->mem_allocate_memory(
              OMRPORTLIB, (languageCategoryCount) * sizeof(OMRMemCategory *),
      ...
      portControl->omr_memory_categories.categories =
          portLibrary->mem_allocate_memory(
              OMRPORTLIB, (omrCategoryCount) * sizeof(OMRMemCategory *),
              OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
      if (NULL == portControl->omr_memory_categories.categories) {
        portLibrary->mem_free_memory(
            OMRPORTLIB, portControl->language_memory_categories.categories);
      ...
      memset(portControl->omr_memory_categories.categories, 0,
             (omrCategoryCount) * sizeof(OMRMemCategory *));
      /* Copy the categories. */
      for (i = 0; i < categories->numberOfCategories; i++) {
        uint32_t categoryCode = categories->categories[i]->categoryCode;
        if (categoryCode < OMRMEM_LANGUAGE_CATEGORY_LIMIT) {
        ...
        if (categoryCode > OMRMEM_LANGUAGE_CATEGORY_LIMIT) {
          uint32_t categoryIndex =
              OMRMEM_OMR_CATEGORY_INDEX_FROM_CODE(categoryCode);
          portControl->omr_memory_categories.categories[categoryIndex] =
              categories->categories[i];
        }
      }
      /* These two won't have been passed in by the caller of port control. */
      portControl->omr_memory_categories
          .categories[OMRMEM_OMR_CATEGORY_INDEX_FROM_CODE(
      ...
      Trc_Assert_PRT_mem_categories_already_set(
          NULL != portControl->language_memory_categories.categories);
      return 1;
    }
  }
