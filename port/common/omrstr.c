...
static void
readValues(struct OMRPortLibrary* portLibrary,
           J9FormatData* result,
           va_list args);
static int
parseFormatString(struct OMRPortLibrary* portLibrary, J9FormatData* result);
static uintptr_t
writeDoubleToBuffer(char* buf,
                    uintptr_t bufLen,
                    uint64_t width,
                    uint64_t precision,
                    double value,
...
static uintptr_t
writeFormattedString(struct OMRPortLibrary* portLibrary,
                     J9FormatData* data,
                     char* result,
                     uintptr_t length);
static uintptr_t
writeSpec(J9FormatData* data,
...
writeIntToBuffer(char* buf,
                 ...
                 const char* digits);
static uintptr_t
writeUnicodeStringToBuffer(char* buf,
                           uintptr_t bufLen,
                           uint64_t width,
                           uint64_t precision,
                           ...
                           uint8_t tag);
static void
convertUTCMillisToLocalJ9Time(int64_t millisUTC, struct J9TimeInfo* tm);
static void
setJ9TimeToEpoch(struct J9TimeInfo* tm);
static uint32_t
omrstr_subst_time(struct OMRPortLibrary* portLibrary,
                  char* buf,
                  uint32_t bufLen,
                  const char* format,
                  int64_t timeMillis);
static intptr_t
omrstr_set_token_from_buf(struct OMRPortLibrary* portLibrary,
                          struct J9StringTokens* tokens,
                          const char* key,
                          char* tokenBuf,
                          uint32_t tokenLen);
static int32_t
convertPlatformToMutf8(struct OMRPortLibrary* portLibrary,
                       uint32_t codePage,
...
static int32_t
convertMutf8ToPlatform(struct OMRPortLibrary* portLibrary,
                       uint32_t codePage,
                       const uint8_t* inBuffer,
                       uintptr_t inBufferSize,
                       uint8_t* outBuffer,
                       uintptr_t outBufferSize);
static int32_t
convertWideToMutf8(const uint8_t** inBuffer,
                   uintptr_t* inBufferSize,
                   uint8_t* outBuffer,
                   uintptr_t outBufferSize);
static int32_t
convertUtf8ToMutf8(struct OMRPortLibrary* portLibrary,
                   const uint8_t** inBuffer,
                   uintptr_t* inBufferSize,
                   uint8_t* outBuffer,
                   uintptr_t outBufferSize);
static int32_t
convertPlatformToUtf8(struct OMRPortLibrary* portLibrary,
                      const uint8_t* inBuffer,
                      uintptr_t inBufferSize,
                      uint8_t* outBuffer,
                      uintptr_t outBufferSize);
static int32_t
convertMutf8ToWide(const uint8_t** inBuffer,
                   uintptr_t* inBufferSize,
                   uint8_t* outBuffer,
                   uintptr_t outBufferSize);
static int32_t
...
convertWideToPlatform(struct OMRPortLibrary* portLibrary,
                      ...
                      uintptr_t outBufferSize);
static int32_t
convertLatin1ToMutf8(struct OMRPortLibrary* portLibrary,
                     const uint8_t** inBuffer,
                     uintptr_t* inBufferSize,
                     uint8_t* outBuffer,
                     uintptr_t outBufferSize);
static int32_t
convertMutf8ToLatin1(struct OMRPortLibrary* portLibrary,
                     const uint8_t** inBuffer,
                     uintptr_t* inBufferSize,
                     uint8_t* outBuffer,
                     uintptr_t outBufferSize);
 ...
 */
uintptr_t
omrstr_printf(struct OMRPortLibrary* portLibrary,
              char* buf,
              uintptr_t bufLen,
              const char* format,
              ...)
...
#endif
int32_t
omrstr_convert(struct OMRPortLibrary* portLibrary,
               int32_t fromCode,
               int32_t toCode,
               uint8_t* inBuffer,
               uintptr_t inBufferSize,
               ...
               uintptr_t outBufferSize)
{
  ...
  switch (fromCode) {
    case J9STR_CODE_PLATFORM_RAW: {
      switch (toCode) {
        case J9STR_CODE_MUTF8:
          result = convertPlatformToMutf8(portLibrary,
                                          OS_ENCODING_CODE_PAGE,
                                          inBuffer,
                                          inBufferSize,
                                          outBuffer,
                                          outBufferSize);
        ...
        case J9STR_CODE_UTF8:
          result = convertPlatformToUtf8(portLibrary,
                                         (const uint8_t*)inBuffer,
                                         inBufferSize,
                                         outBuffer,
                                         outBufferSize);
    ...
    case J9STR_CODE_MUTF8: {
      switch (toCode) {
        case J9STR_CODE_PLATFORM_RAW:
          result = convertMutf8ToPlatform(portLibrary,
                                          OS_ENCODING_CODE_PAGE,
                                          inBuffer,
                                          inBufferSize,
                                          outBuffer,
        ...
        case J9STR_CODE_LATIN1: {
          ...
          result = convertMutf8ToLatin1(portLibrary,
                                        &mutf8Cursor,
                                        &mutf8Remaining,
                                        outBuffer,
                                        outBufferSize);
        ...
        case J9STR_CODE_WIDE: {
          const uint8_t* mutf8Cursor = inBuffer;
          uintptr_t mutf8Remaining = inBufferSize;
          result = convertMutf8ToWide(
            &mutf8Cursor, &mutf8Remaining, outBuffer, outBufferSize);
          /*
    ...
    case J9STR_CODE_UTF8: {
      switch (toCode) {
        case J9STR_CODE_MUTF8: {
          ...
          result = convertUtf8ToMutf8(
            portLibrary, &utf8Cursor, &utf8Remaining, outBuffer, outBufferSize);
          /*
    ...
    case J9STR_CODE_WIDE: {
      switch (toCode) {
        ...
        case J9STR_CODE_MUTF8: {
          ...
          result = convertWideToMutf8(
            &wideCursor, &wideRemaining, outBuffer, outBufferSize);
          /*
           * inBuffer parameters are updated to reflect  data untranslated due
           * to insufficient space in the output buffer. In this case, all input
           * characters should have been consumed.
    ...
    case J9STR_CODE_LATIN1: {
      switch (toCode) {
        case J9STR_CODE_MUTF8: {
          ...
          result = convertLatin1ToMutf8(portLibrary,
                                        &latin1Cursor,
                                        &latin1Remaining,
                                        outBuffer,
                                        outBufferSize);
 ...
 */
uintptr_t
omrstr_vprintf(struct OMRPortLibrary* portLibrary,
               char* buf,
               uintptr_t bufLen,
               const char* format,
               va_list args)
          ...
          uintptr_t length)
{
  ...
  switch (*spec->type) {
    case 'i':
    case 'd':
      index = writeIntToBuffer(
        result, length, width, precision, value->u64, spec->tag, 1, digits_dec);
      break;
    case 'u':
      index = writeIntToBuffer(
        result, length, width, precision, value->u64, spec->tag, 0, digits_dec);
      break;
    case 'x':
      index = writeIntToBuffer(result,
                               length,
                               width,
                               precision,
                               value->u64,
    ...
    case 'c':
      if (spec->tag & J9FSPEC_L) {
        ...
        index = writeStringToBuffer(
          result, length, width, precision, asUTF8, spec->tag);
      } else {
        index =
    ...
    case 's':
      if (value->ptr) {
        if (spec->tag & J9FSPEC_L) {
          index = writeUnicodeStringToBuffer(result,
                                             length,
                                             width,
                                             precision,
          ...
          index = writeStringToBuffer(result,
                                      length,
                                      width,
                                      precision,
        ...
        index = writeStringToBuffer(
          result, length, width, precision, "<NULL>", spec->tag);
      }
      break;
    ...
    case 'g':
    case 'G':
      index = writeDoubleToBuffer(
        result, length, width, precision, value->dbl, *spec->type, spec->tag);
      break;
  }
      ...
      case '%':
        ...
        switch (*format) {
          ...
          default:
            if (result) {
              index += writeSpec(
                data, &data->spec[specIndex], result + index, length - index);
            } else {
                    ...
                    uint8_t tag)
{
  ...
  *formatCursor++ = '%';
  ...
  if (width != J9F_NO_VALUE) {
    formatCursor += writeIntToBuffer(formatCursor,
                                     lastFormat - formatCursor,
                                     J9F_NO_VALUE,
                                     J9F_NO_VALUE,
                                     width,
                                     'u',
...
convertUTCMillisToLocalJ9Time(int64_t millisUTC, struct J9TimeInfo* omrtimeInfo)
{
...

  {
    ...
    if (SystemTimeToTzSpecificLocalTime(
          NULL, &systemTimeUTC, &systemTimeLocal)) {
 ...
 */
static intptr_t
populateWithDefaultTokens(struct OMRPortLibrary* portLibrary,
                          struct J9StringTokens* tokens,
                          int64_t timeMillis)
{
...
#define ASID_BUF_LEN 16
  ...
  if (0 == portLibrary->sysinfo_get_username(
             portLibrary, username, USERNAME_BUF_LEN)) {
                       ...
                       int64_t timeMillis)
{
  ...
  omrstr_subst_time(
    portLibrary, buf, TICK_BUF_SIZE + 20, "%Y%y%m%d%H%M%S%b%tick", timeMillis);
  ...
  if (omrstr_set_token_from_buf(portLibrary, tokens, "Y", buf, 4) ||
      ...
      omrstr_set_token_from_buf(
        portLibrary, tokens, "tick", buf + 19, (uint32_t)strlen(buf + 19))) {
    /* If any of the above fail, we're out of memory */
    return -1;
  }
  return 0;
...
omrstr_create_tokens(struct OMRPortLibrary* portLibrary, int64_t timeMillis)
{
  ...
  tokens = hashTableNew(portLibrary,
                        OMR_GET_CALLSITE(),
                        J9TOKEN_TABLE_INIT_SIZE,
                        sizeof(J9TokenEntry),
                        J9TOKEN_TABLE_ALIGNMENT,
                        0,
 ...
 */
intptr_t
omrstr_set_token(struct OMRPortLibrary* portLibrary,
                 struct J9StringTokens* tokens,
                 const char* key,
                 const char* format,
                 ...)
{
  ...
  valueLen =
    portLibrary->str_vprintf(portLibrary, tokenBuf, tokenBufLen, format, args);
  va_end(args);

  return omrstr_set_token_from_buf(
    portLibrary, tokens, key, tokenBuf, (uint32_t)valueLen);
}
/**
 * Add the specified key to the list of tokens.
 ...
 */
void
omrstr_free_tokens(struct OMRPortLibrary* portLibrary,
                   struct J9StringTokens* tokens)
{
  if (NULL == tokens) {
    return;
 ...
 */
uint32_t
omrstr_ftime(struct OMRPortLibrary* portLibrary,
             char* buf,
             uint32_t bufLen,
             const char* format,
    ...
    if (c == '%') {
      ...
      switch (key) {
        case 'Y':
          ...
          if (count <= bufLen) {
            writeIntToBuffer(
              write, 4, 4, J9F_NO_VALUE, tm.year, J9FFLAG_ZERO, 0, digits_dec);
            write += 4;
          }
          read += 2;
          break;
        ...
        case 'm':
          ...
          if (count <= bufLen) {
            writeIntToBuffer(
              write, 2, 2, J9F_NO_VALUE, tm.month, J9FFLAG_ZERO, 0, digits_dec);
            write += 2;
          }
          read += 2;
          break;
        case 'd':
          ...
          if (count <= bufLen) {
            writeIntToBuffer(
              write, 2, 2, J9F_NO_VALUE, tm.day, J9FFLAG_ZERO, 0, digits_dec);
            write += 2;
          }
          read += 2;
          break;
        case 'H':
          ...
          if (count <= bufLen) {
            writeIntToBuffer(
              write, 2, 2, J9F_NO_VALUE, tm.hour, J9FFLAG_ZERO, 0, digits_dec);
            write += 2;
          }
          read += 2;
          break;
        ...
        case 't':
          if (strncmp(read + 2, "ick", 3) == 0) {
            ...
            tickSize = (uint32_t)writeIntToBuffer(ticks,
                                                  TICK_BUF_SIZE,
                                                  J9F_NO_VALUE,
                                                  J9F_NO_VALUE,
                                                  curTick,
                       ...
                       uintptr_t outBufferSize)
{
  ...
  requiredBufferSize =
    WIDE_CHAR_SIZE * MultiByteToWideChar(codePage,
                                         OS_ENCODING_MB_FLAGS,
                                         (LPCSTR)inBuffer,
                                         (int)inBufferSize,
                                         NULL,
                                         0); /* get required buffer size */
  if (requiredBufferSize > CONVERSION_BUFFER_SIZE) {
    wideBuffer = portLibrary->mem_allocate_memory(portLibrary,
                                                  requiredBufferSize,
                                                  OMR_GET_CALLSITE(),
                                                  OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == wideBuffer) {
  ...
  encodingState = iconv_get(portLibrary,
  ...
  while ((platformRemaining > 0) &&
    ...
    int32_t wideBufferPartialSize = convertPlatformToWide(portLibrary,
                                                          encodingState,
                                                          codePage,
                                                          &platformCursor,
                                                          &platformRemaining,
...
#if defined(J9STR_USE_ICONV)
      iconv_free(
        portLibrary, OMRPORT_LANG_TO_UTF16_ICONV_DESCRIPTOR, encodingState);
#endif
    ...
    mutf8PartialSize = convertWideToMutf8(
      &tempWideBuffer, &tempWideBufferSize, mutf8Cursor, mutf8Limit);
                       ...
                       uintptr_t outBufferSize)
{
  ...
  while (mutf8Remaining > 0) { /* translated section by section as dictated by
    ...
    int32_t result = convertMutf8ToWide(
      &mutf8Cursor, &mutf8Remaining, wideBuffer, wideBufferSize);
    if (result < 0) { /* conversion error */
      return result;
    }
    wideBufferCount = (uintptr_t)result;

    platformPartCount = convertWideToPlatform(portLibrary,
                                              encodingState,
                                              codePage,
                                              &wideBufferCursor,
                                              &wideBufferCount,
...
#if defined(J9STR_USE_ICONV)
      iconv_free(
        portLibrary, OMRPORT_UTF16_TO_LANG_ICONV_DESCRIPTOR, encodingState);
#endif
                      ...
                      uintptr_t outBufferSize)
{
  ...
  charconvState_t converter = iconv_get(portLibrary,
  ...
  while ((outbytesleft > 0) && (inbytesleft > 0)) {
    if ((size_t)-1 ==
      ...
      if (errno == E2BIG) {
        iconv_free(
          portLibrary, OMRPORT_LANG_TO_UTF8_ICONV_DESCRIPTOR, converter);
        return OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL;
      }
                     ...
                     uintptr_t outBufferSize)
{
  ...
  while (
    ...
    ((mutf8Remaining > 0) ||
     (0 ==
      outBufferSize)) /* outBufferSize == 0 indicates we just want the length */
    && (resultSize >= 0)) {
    ...
    mutf8Result = convertWideToMutf8((const uint8_t**)&wideBuffer,
                                     &wideRemaining,
                                     mutf8Cursor,
                                     mutf8Remaining);
    latinBytesConsumed =
      (wideLength - wideRemaining) / 2; /* 1 Latin byte per 2 wide characters */
                     ...
                     uintptr_t outBufferSize)
{
  ...
  while (
    ...
    mutf8BytesConsumed =
      (wideLength - wideRemaining) / 2; /* 1 mutf8 byte per 2 wide characters */
 ...
 */
static BOOLEAN
checkAndCopyUtfBytes(const uint8_t* utf8Bytes,
                     uintptr_t numBytes,
                     uint8_t* mutf8Bytes)
{
  uintptr_t cursor;
                      ...
                      uintptr_t outBufferSize)
{
...
#if defined(OMR_OS_WINDOWS)
  int32_t mbChars = (int32_t)MultiByteToWideChar(codePage,
                                                 OS_ENCODING_MB_FLAGS,
                                                 (LPCSTR)*inBuffer,
                                                 (int)*inBufferSize,
                                                 (LPWSTR)outBuffer,
  ...
  if ((outBufferSize > 0) && (0 == mbChars)) {
    resultSize =
      OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL; /* should not happen: caller should
                                                have allocated a sufficiently
                                                large buffer */
  } else {
                      ...
                      uintptr_t outBufferSize)
{
  ...
  if (0 == outBufferSize) { /* get the required buffer size */
    ...
    while (wideRemaining > 0) {
      ...
      oldOsbSize = osbSize;
      iconvResult = iconv(encodingState,
                          (char**)inBuffer,
                          (size_t*)&wideRemaining,
                          &osbCursor,
                          (size_t*)&osbSize); /* input and output pointers and
                                                 limits are updated */
    ...
    /* initialize convertor */
    uintptr_t iconvResult = iconv(
      encodingState, NULL, 0, (char**)&platformCursor, (size_t*)&platformLimit);
    iconvResult = iconv(encodingState,
                        (char**)inBuffer,
                        (size_t*)inBufferSize,
                        (char**)&platformCursor,
  ...
  resultSize = WideCharToMultiByte(codePage,
                                   OS_ENCODING_MB_FLAGS,
                                   wideCursor,
                                   (int)wideRemaining,
                                   (LPSTR)outBuffer,
