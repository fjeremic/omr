...
static void readValues(struct OMRPortLibrary *portLibrary, J9FormatData *result,
                       va_list args);
static int parseFormatString(struct OMRPortLibrary *portLibrary,
                             J9FormatData *result);
static uintptr_t writeDoubleToBuffer(char *buf, uintptr_t bufLen,
                                     uint64_t width, uint64_t precision,
                                     double value, uint8_t type, uint8_t tag);
...
static uintptr_t writeFormattedString(struct OMRPortLibrary *portLibrary,
                                      J9FormatData *data, char *result,
                                      uintptr_t length);
static uintptr_t writeSpec(J9FormatData *data, J9FormatSpecifier *spec,
                           char *result, uintptr_t length);
static const char *parseIndex(const char *format, uint8_t *result);
static uintptr_t writeStringToBuffer(char *buf, uintptr_t bufLen,
                                     uint64_t width, uint64_t precision,
                                     const char *value, uint8_t tag);
static const char *parsePrecision(const char *format, J9FormatData *result);
static uintptr_t writeIntToBuffer(char *buf, uintptr_t bufLen, uint64_t width,
                                  uint64_t precision, uint64_t value,
                                  uint8_t tag, int isSigned,
                                  const char *digits);
static uintptr_t writeUnicodeStringToBuffer(char *buf, uintptr_t bufLen,
                                            uint64_t width, uint64_t precision,
                                            const uint16_t *value, uint8_t tag);
static void convertUTCMillisToLocalJ9Time(int64_t millisUTC,
                                          struct J9TimeInfo *tm);
static void setJ9TimeToEpoch(struct J9TimeInfo *tm);
static uint32_t omrstr_subst_time(struct OMRPortLibrary *portLibrary, char *buf,
                                  uint32_t bufLen, const char *format,
                                  int64_t timeMillis);
static intptr_t omrstr_set_token_from_buf(struct OMRPortLibrary *portLibrary,
                                          struct J9StringTokens *tokens,
                                          const char *key, char *tokenBuf,
                                          uint32_t tokenLen);
static int32_t
convertPlatformToMutf8(struct OMRPortLibrary *portLibrary, uint32_t codePage,
                       const uint8_t *inBuffer, uintptr_t inBufferSize,
                       uint8_t *outBuffer, uintptr_t outBufferSize);
static int32_t
convertMutf8ToPlatform(struct OMRPortLibrary *portLibrary, uint32_t codePage,
                       const uint8_t *inBuffer, uintptr_t inBufferSize,
                       uint8_t *outBuffer, uintptr_t outBufferSize);
static int32_t convertWideToMutf8(const uint8_t **inBuffer,
                                  uintptr_t *inBufferSize, uint8_t *outBuffer,
                                  uintptr_t outBufferSize);
static int32_t convertUtf8ToMutf8(struct OMRPortLibrary *portLibrary,
                                  const uint8_t **inBuffer,
                                  uintptr_t *inBufferSize, uint8_t *outBuffer,
                                  uintptr_t outBufferSize);
static int32_t convertPlatformToUtf8(struct OMRPortLibrary *portLibrary,
                                     const uint8_t *inBuffer,
                                     uintptr_t inBufferSize, uint8_t *outBuffer,
                                     uintptr_t outBufferSize);
static int32_t convertMutf8ToWide(const uint8_t **inBuffer,
                                  uintptr_t *inBufferSize, uint8_t *outBuffer,
                                  uintptr_t outBufferSize);
static int32_t
convertPlatformToWide(struct OMRPortLibrary *portLibrary,
                      charconvState_t encodingState, uint32_t codePage,
                      const uint8_t **inBuffer, uintptr_t *inBufferSize,
                      uint8_t *outBuffer, uintptr_t outBufferSize);
static int32_t
convertWideToPlatform(struct OMRPortLibrary *portLibrary,
                      charconvState_t encodingState, uint32_t codePage,
                      const uint8_t **inBuffer, uintptr_t *inBufferSize,
                      uint8_t *outBuffer, uintptr_t outBufferSize);
static int32_t convertLatin1ToMutf8(struct OMRPortLibrary *portLibrary,
                                    const uint8_t **inBuffer,
                                    uintptr_t *inBufferSize, uint8_t *outBuffer,
                                    uintptr_t outBufferSize);
static int32_t convertMutf8ToLatin1(struct OMRPortLibrary *portLibrary,
                                    const uint8_t **inBuffer,
                                    uintptr_t *inBufferSize, uint8_t *outBuffer,
                                    uintptr_t outBufferSize);

#if defined(OMR_OS_WINDOWS)
static void convertJ9TimeToSYSTEMTIME(J9TimeInfo *j9TimeInfo,
                                      SYSTEMTIME *systemTime);
static void convertTimeMillisToJ9Time(int64_t timeMillis, J9TimeInfo *tm);
static void convertSYSTEMTIMEToJ9Time(SYSTEMTIME *systemTime,
                                      J9TimeInfo *j9TimeInfo);
static BOOLEAN firstDateComesBeforeSecondDate(J9TimeInfo *firstDate,
                                              J9TimeInfo *secondDate);
#endif /* defined(OMR_OS_WINDOWS) */
 ...
 *
 * @note When buf is NULL, the size of the buffer required to print to the
 * string, including the NUL terminator is returned.
 *
 * @internal @note Supported, portable format specifiers are described in the
 * document entitled "PortLibrary printf" in the "Inside J9" Lotus Notes
 * database.
 */
uintptr_t omrstr_printf(struct OMRPortLibrary *portLibrary, char *buf,
...
/**
 * This function converts strings between either the environment's platform
 *encoding or Unicode (wide character), and modified UTF-8. Modified UTF-8 is
 *described in the JVMTI specification. Conversion between arbitrary character
 *encodings is not supported.
 *
 * @param[in] portLibrary   The port library
 * @param[in] fromCode      Input string encoding.  Only the following encodings
 *are allowed: J9STR_CODE_MUTF8 (Modified UTF-8) J9STR_CODE_WIDE (UTF-16)
 *                              J9STR_CODE_LATIN1
 *                              J9STR_CODE_PLATFORM_RAW (encoding used by the
 *operating system) J9STR_CODE_PLATFORM_OMR_INTERNAL (encoding used by certain
 *operating system calls) J9STR_CODE_WINTHREADACP, J9STR_CODE_WINDEFAULTACP
 *(thread and default ANSI code page, Windows only)
 * @param[in] toCode        Output string encoding.  Only the encodings listed
 *above are allowed.
 * @param[in] inBuffer      Input text to be converted.  May contain embedded
 *null characters.
 * @param[in] inBufferSize  input string size in bytes, not including the
 *terminating null.
 * @param[in] outBuffer     user-allocated output buffer that stores converted
 *characters, ignored if outBufferSize is 0.
 * @param[in] outBufferSize output buffer size in bytes (zero to request the
 *required output buffer size)
 *
 * @return:
 * - positive value on success.
 *  	- the number of bytes required to hold the converted text if
 *outBufferSize is 0.
 ...
 *failed.
 * 		- OMRPORT_ERROR_STRING_ILLEGAL_STRING if the input string is
 *malformed
 *  	- OMRPORT_ERROR_STRING_UNSUPPORTED_ENCODING if the input or output
 *encoding is not supported
 *  	- OMRPORT_ERROR_STRING_MEM_ALLOCATE_FAILED if the port library could not
 *allocate a working buffer The following translations are supported: ANSI code
 *page to modified UTF-8 (Windows only) platform raw to [wide, modified UTF-8,
 *UTF-8] modified UTF-8 to [platform raw, wide, ISO Latin-1 (8859-1)] wide to
 *[modified UTF-8, platform raw] [ISO Latin-1 (8859-1), UTF-8, Windows ANSI
 *default and current code pages] to modified UTF-8
 * @note J9STR_CODE_PLATFORM_OMR_INTERNAL is an alias for other encodings
 *depending on the platform.
 * @note J9STR_CODE_PLATFORM is deprecated. Use J9STR_CODE_PLATFORM_OMR_INTERNAL
 *for results of system calls such as getenv (see stdlib.h) and
 * @note J9STR_CODE_PLATFORM_RAW where the port library does not do implicit
 *translation.
 */
...
#endif
int32_t omrstr_convert(struct OMRPortLibrary *portLibrary, int32_t fromCode,
                       int32_t toCode, uint8_t *inBuffer,
                       uintptr_t inBufferSize, uint8_t *outBuffer,
                       uintptr_t outBufferSize) {
  ...
  case J9STR_CODE_PLATFORM_RAW: {
    ...
    case J9STR_CODE_MUTF8:
      result =
          convertPlatformToMutf8(portLibrary, OS_ENCODING_CODE_PAGE, inBuffer,
    ...
    case J9STR_CODE_UTF8:
      result = convertPlatformToUtf8(portLibrary, (const uint8_t *)inBuffer,
                                     inBufferSize, outBuffer, outBufferSize);
      break;
    default:
      result = OMRPORT_ERROR_STRING_UNSUPPORTED_ENCODING;
  ...
  case J9STR_CODE_WINDEFAULTACP: {
    switch (toCode) {
    case J9STR_CODE_MUTF8:
      result = convertPlatformToMutf8(
          portLibrary,
          (fromCode == J9STR_CODE_WINTHREADACP) ? CP_THREAD_ACP : CP_ACP,
  ...
  case J9STR_CODE_MUTF8: {
    ...
    case J9STR_CODE_PLATFORM_RAW:
      result =
          convertMutf8ToPlatform(portLibrary, OS_ENCODING_CODE_PAGE, inBuffer,
                                 inBufferSize, outBuffer, outBufferSize);
      break;
    case J9STR_CODE_LATIN1: {
      const uint8_t *mutf8Cursor = inBuffer;
      uintptr_t mutf8Remaining = inBufferSize;
      result = convertMutf8ToLatin1(portLibrary, &mutf8Cursor, &mutf8Remaining,
                                    outBuffer, outBufferSize);
      /*
       * convertMutf8ToLatin1 is resumable so does not return error if buffer
       * too small. In this case we should have consumed all the data
       */
      if (mutf8Remaining > 0) {
...
#if defined(OMR_OS_WINDOWS)
    case J9STR_CODE_WINTHREADACP:
      result = convertMutf8ToPlatform(portLibrary, CP_THREAD_ACP, inBuffer,
                                      inBufferSize, outBuffer, outBufferSize);
      break;
    case J9STR_CODE_WINDEFAULTACP:
      result = convertMutf8ToPlatform(portLibrary, CP_ACP, inBuffer,
                                      inBufferSize, outBuffer, outBufferSize);
      break;
#endif /* defined(OMR_OS_WINDOWS) */
    case J9STR_CODE_WIDE: {
      const uint8_t *mutf8Cursor = inBuffer;
      uintptr_t mutf8Remaining = inBufferSize;
      result = convertMutf8ToWide(&mutf8Cursor, &mutf8Remaining, outBuffer,
                                  outBufferSize);
      /*
       * inBuffer parameters are updated to reflect  data untranslated due to
       * insufficient space in the output buffer. In this case, all input
       * characters should have been consumed.
       */
      if (mutf8Remaining > 0) {
        result = OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL;
      }
  ...
  case J9STR_CODE_UTF8: {
    ...
    case J9STR_CODE_MUTF8: {
      ...
      result = convertUtf8ToMutf8(portLibrary, &utf8Cursor, &utf8Remaining,
                                  outBuffer, outBufferSize);
      /*
       * convertUtf8ToMutf8 is resumable so does not return error if buffer too
       * small. In this case we should have consumed all the data
       */
      if (utf8Remaining > 0) {
        result = OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL;
      }
  ...
  case J9STR_CODE_WIDE: {
    ...
    case J9STR_CODE_MUTF8: {
      ...
      result = convertWideToMutf8(&wideCursor, &wideRemaining, outBuffer,
                                  outBufferSize);
      /*
       * inBuffer parameters are updated to reflect  data untranslated due to
       * insufficient space in the output buffer. In this case, all input
  ...
  case J9STR_CODE_LATIN1: {
    ...
    case J9STR_CODE_MUTF8: {
      ...
      result = convertLatin1ToMutf8(portLibrary, &latin1Cursor,
                                    &latin1Remaining, outBuffer, outBufferSize);
      /*
       * convertLatin1ToMutf8 is resumable so does not return error if buffer
       * too small. In this case we should have consumed all the data
       */
 ...
 * database.
 */
uintptr_t omrstr_vprintf(struct OMRPortLibrary *portLibrary, char *buf,
...
static const char *parsePrecision(const char *format, J9FormatData *result) {
  ...
  if (*format == '*') {
    format =
        parseIndex(format + 1, &result->spec[result->specCount].precisionIndex);
...
static void readValues(struct OMRPortLibrary *portLibrary, J9FormatData *result,
  ...
  for (index = 0; index < result->valueCount; index++) {
    ...
    case J9FTYPE_IMMEDIATE:
      /* shouldn't be encountered -- these should all be at the end of the value
       * array */
      break;
    }
  }
...
static uintptr_t writeSpec(J9FormatData *data, J9FormatSpecifier *spec,
  ...
  case 'i':
  case 'd':
    index = writeIntToBuffer(result, length, width, precision, value->u64,
  ...
  case 'p':
    index = writeIntToBuffer(result, length, sizeof(uintptr_t) * 2,
                             sizeof(uintptr_t) * 2, (uintptr_t)value->ptr, 0, 0,
                             digits_hex_upper);
    break;
  ...
  case 'c':
    if (spec->tag & J9FSPEC_L) {
      ...
      uint32_t numberWritten =
          encodeUTF8Char((uintptr_t)value->u64, (uint8_t *)asUTF8);

      /* what if width/precision is less than size of asUTF8? [truncate?] */
      asUTF8[numberWritten] = '\0';
      index = writeStringToBuffer(result, length, width, precision, asUTF8,
                                  spec->tag);
    } else {
      index =
          writeStringToBuffer(result, length, width, precision, " ", spec->tag);
      if (index <= length) {
        if (result) {
      ...
      if (spec->tag & J9FSPEC_L) {
        index =
            writeUnicodeStringToBuffer(result, length, width, precision,
                                       (const uint16_t *)value->ptr, spec->tag);
      } else {
        index = writeStringToBuffer(result, length, width, precision,
                                    (const char *)value->ptr, spec->tag);
      }
    } else {
      index = writeStringToBuffer(result, length, width, precision, "<NULL>",
                                  spec->tag);
    }
    break;
  ...
  case 'g':
  case 'G':
    index = writeDoubleToBuffer(result, length, width, precision, value->dbl,
                                *spec->type, spec->tag);
    break;
  }
 ...
 * @internal
 *
 * Writes the string representation of the specified value into the provided
 * buffer using the specified precision.
 *
 * @param[in] buf       The buffer into which results should be written.
 * @param[in] bufLen    The length of the buffer in bytes, never writes more
 * than this number of bytes.
 * @param[in] precision The maximum number of digits to display, J9F_NO_VALUE
 * for as many digits as required.
 * @param[in] value     The value to print.
 * @param[in] tag       One of the J9FFLAG_* constants that controls the output
 * format.
 * @param[in] isSigned  Non-zero if the value is signed.
 * @param[in] digits    A string containing hex/decimal digits (starting at
 * zero).
 *
...
static uintptr_t writeIntToBuffer(char *buf, uintptr_t bufLen, uint64_t width,
  ...
  /* Account for "-" Must be after setting actualPrecision, before calculation
   * of rightSpace */
  if (signChar) {
    length++;
  }
  ...
  if (width != J9F_NO_VALUE) {
    uint32_t actualWidth =
        (uint32_t)width; /* shorten user-specified width to uint32_t */
 ...
 * @internal
 *
 * To determine size of buffer required for format string pass in a NULL buffer
 * and the maximum size willing to create.  For example for no restrictions
 * result=NULL, length=(uint32_t)(-1), to restrict the buffer to 2k,
 * result=NULL, length=2048
 *
...
static uintptr_t writeFormattedString(struct OMRPortLibrary *portLibrary,
  ...
  while (*format && index < length - 1) {
    ...
    case '%':
      ...
      default:
        if (result) {
          index += writeSpec(data, &data->spec[specIndex], result + index,
                             length - index);
        } else {
...
static uintptr_t writeDoubleToBuffer(char *buf, uintptr_t bufLen,
  ...
  *formatCursor++ = '%';
  ...
  if (width != J9F_NO_VALUE) {
    formatCursor +=
        writeIntToBuffer(formatCursor, lastFormat - formatCursor, J9F_NO_VALUE,
                         J9F_NO_VALUE, width, 'u', 0, digits_dec);
  }
  if (precision != J9F_NO_VALUE) {
    *formatCursor++ = '.';
    formatCursor +=
        writeIntToBuffer(formatCursor, lastFormat - formatCursor, J9F_NO_VALUE,
                         J9F_NO_VALUE, precision, 'u', 0, digits_dec);
  }
 ...
 * that were created by @ref omrstr_startup should be destroyed here.
 *
 * @param[in] portLibrary The port library.
 *
 * @note Most implementations will be empty.
 */
void omrstr_shutdown(struct OMRPortLibrary *portLibrary) {}
/**
 * PortLibrary startup.
 *
 * This function is called during startup of the portLibrary.  Any resources
 * that are required for the string operations may be created here.  All
 * resources created here should be destroyed in @ref omrstr_shutdown.
 *
 * @param[in] portLibrary The port library.
 *
 * @return 0 on success, negative error code on failure.  Error code values
 * returned are \arg OMRPORT_ERROR_STARTUP_STR
 *
 * @note Most implementations will simply return success.
 */
int32_t omrstr_startup(struct OMRPortLibrary *portLibrary) { return 0; }
static uintptr_t writeUnicodeStringToBuffer(char *buf, uintptr_t bufLen,
                                            uint64_t width, uint64_t precision,
 ...
 * @internal
 *
 * Converts the UTC time passed in as milli seconds since Epoch to local time as
 * the J9-specific representation of time.
 *
 * @param[in] millisUTC the UTC time passed in as milli seconds since Epoch.
 * Negative values are not allowed.
 * @param[in,out] the local time as the J9-specific representation of time. Will
 * not pass back any values that occurred before Epoch.
 *
...
static void convertUTCMillisToLocalJ9Time(int64_t millisUTC,
...

  {
    ...
    if (SystemTimeToTzSpecificLocalTime(NULL, &systemTimeUTC,
                                        &systemTimeLocal)) {
  ...
  /* Note: timeZoneInformation.Bias is the bias to "local time" and does not
  ...
  /* first get the transition dates, and convert them to J9TimeInfos so that we
   * can compare apples and apples */
  convertSYSTEMTIMEToJ9Time(&timeZoneInformation.DaylightDate,
                            &daylightDateAsJ9TimeInfo);
  convertSYSTEMTIMEToJ9Time(&timeZoneInformation.StandardDate,
                            &standardDateAsJ9TimeInfo);

  /* Now see if we're in Daylight Saving Time */

  /* Are we in the northern or southern hemisphere? In the northern hemisphere
   * we move to Daylight time in the spring, and back to Standard in the fall */
  daylightBeforeStandard = firstDateComesBeforeSecondDate(
      &daylightDateAsJ9TimeInfo, &standardDateAsJ9TimeInfo);

  if (daylightBeforeStandard) {
    /* we're in the northern hemisphere */
    if ((TRUE == firstDateComesBeforeSecondDate(&daylightDateAsJ9TimeInfo,
                                                omrtimeInfo)) &&
        (TRUE == firstDateComesBeforeSecondDate(omrtimeInfo,
                                                &standardDateAsJ9TimeInfo))) {
      standardTime = FALSE;
    } else {
      standardTime = TRUE;
    ...
    if ((TRUE == firstDateComesBeforeSecondDate(&standardDateAsJ9TimeInfo,
                                                omrtimeInfo)) &&
        (TRUE == firstDateComesBeforeSecondDate(omrtimeInfo,
                                                &daylightDateAsJ9TimeInfo))) {
      standardTime = TRUE;
 ...
 * @internal
 *
 * Ingnoring the year, returns TRUE if the first date/time of year comes before
 * the second date/time of year, false otherwise.
 *
 * param[in] firstDate
 * param[in] secondDate
 *
 * @return TRUE if the first date/time of year comes before the second date/time
 * of year, false otherwise.
 *
 ...
 * @internal
 *
 * Convert the J9-Specific representation of time to the Windows-specific
 * representation.
 *
 ...
 * @internal
 *
 * Convert the Windows-specific representation of time into the J9-Specific
 * representation.
 *
...
#define J9SFT_NUM_SECS_IN_LEAP_FEB (29 * J9SFT_NUM_SECS_IN_DAY)
  ...
  int32_t normalSecondsInMonth[12] = {
      J9SFT_NUM_SECS_IN_JAN, J9SFT_NUM_SECS_IN_FEB, J9SFT_NUM_SECS_IN_MAR,
  ...
  int32_t leapYearSecondsInMonth[12] = {
      J9SFT_NUM_SECS_IN_JAN, J9SFT_NUM_SECS_IN_LEAP_FEB, J9SFT_NUM_SECS_IN_MAR,
      J9SFT_NUM_SECS_IN_APR, J9SFT_NUM_SECS_IN_MAY,      J9SFT_NUM_SECS_IN_JUN,
      J9SFT_NUM_SECS_IN_JUL, J9SFT_NUM_SECS_IN_AUG,      J9SFT_NUM_SECS_IN_SEP,
      J9SFT_NUM_SECS_IN_OCT, J9SFT_NUM_SECS_IN_NOV,      J9SFT_NUM_SECS_IN_DEC};
 ...
 */
static uintptr_t tokenHashFn(void *entry, void *userData) {
  J9TokenEntry *token = (J9TokenEntry *)entry;

  /* Because the token keys will mostly be single characters, using the first
   * char as the hash value is pretty reasonable */
 ...
 *
 * @note that the keys correspond to the tokens that show up in the format
 * string
 */
static uintptr_t tokenHashEqualFn(void *lhsEntry, void *rhsEntry,
                                  void *userData) {
...
/**
 * @internal
 * Helper function for populating the hash table with default, platform
 * independant tokens
 *
 * @param[in] portLibrary The port library in use
 * @param[in] tokens The token cookie that should be populated
 * @param[in] timeMillis The time in milliseconds that should be used as a basis
 * for the time tokens
 *
 * @return 0 on success, -1 on failure
 */
static intptr_t populateWithDefaultTokens(struct OMRPortLibrary *portLibrary,
                                          struct J9StringTokens *tokens,
                                          int64_t timeMillis) {
...
#define ASID_BUF_LEN 16
  ...
  if (0 == portLibrary->sysinfo_get_username(portLibrary, username,
                                             USERNAME_BUF_LEN)) {
 ...
 * memory used by the token is freed up (and not just the hash table itself).
 *
 * @param[in] entry The entry to free
 * @param[in] portLibrary This is passed along as "user data" and is used for
 * calling mem_free_memory
 */
static uintptr_t tokenDoFreeFn(void *entry, void *portLibrary) {
  J9TokenEntry *token = (J9TokenEntry *)entry;

  ((struct OMRPortLibrary *)portLibrary)
      ->mem_free_memory(portLibrary, token->key);
  token->key = NULL;
  token->keyLen = 0;
  ((struct OMRPortLibrary *)portLibrary)
      ->mem_free_memory(portLibrary, token->value);
  token->value = NULL;
  token->valueLen = 0;
...
/**
 * @internal
 * Helper function for token expansion; reads key and determines if a token is
 * present
 *
 ...
 *
 * @return When a token is found, its entry with the value at expansion is
 * returned. When no token is found, NULL is returned.
 */
static J9TokenEntry *consumeToken(J9StringTokens *tokens, const char *key) {
  char tokenKey[J9TOKEN_MAX_KEY_LEN];
  J9TokenEntry lookupEntry;
  J9TokenEntry *entry;
  char *write = tokenKey;

  /* For consistency, we enforce starting on a %, this simplifies the calling
   * code */
  if (*key != '%') {
    return NULL;
  }
  ...
  /* Let's start by reading in the largest amount we can so longer token names
   * can have precedence */
...
/**
 * Update the time tokens stored as local time, from the timeMillis parameter
 *that was passed in as UTC. The cookie must be freed with
 *omrstr_tokens_destroy() when it is no longer needed.
 *
 * The time tokens always include:
 *
 * 	  %Y     year    1900..????		(local time, based on millis
 *parameter) %y     year of century  00..99 %m     month     01..12 %b
 *abbreviated month name in english %d     day       01..31 %H     hour 00..23
 *	  %M     minute    00..59
 *	  %S     second    00..59
 *
 *	  %tick  high res timer
 ...
 */
intptr_t omrstr_set_time_tokens(struct OMRPortLibrary *portLibrary,
                                struct J9StringTokens *tokens,
                                int64_t timeMillis) {
  char buf[TICK_BUF_SIZE + 20]; /* TICK_BUF_SIZE + 4 + 2 + 2 + 2 + 2 + 2 + 2 + 3
                                   + an extra byte */

  omrstr_subst_time(portLibrary, buf, TICK_BUF_SIZE + 20,
                    "%Y%y%m%d%H%M%S%b%tick", timeMillis);
  ...
  if (omrstr_set_token_from_buf(portLibrary, tokens, "Y", buf, 4) ||
      ...
      omrstr_set_token_from_buf(portLibrary, tokens, "tick", buf + 19,
                                (uint32_t)strlen(buf + 19))) {
    /* If any of the above fail, we're out of memory */
    return -1;
  }
  return 0;
 ...
 * Time tokens will be initialized to the local time based on the UTC time that
 is passed in.
 *
 * The cookie must be freed with omrstr_tokens_destroy() when it is no longer
 needed.
 *
 ...
 *
 * @return the opaque J9StringTokens structure, or NULL if out of memory. The
 date/time will never be less than Epoch, which is Jan 1st, 1970, 00:00:00 AM.
 */
struct J9StringTokens *omrstr_create_tokens(struct OMRPortLibrary *portLibrary,
                                            int64_t timeMillis) {
  ...
  tokens = hashTableNew(
      portLibrary, OMR_GET_CALLSITE(), J9TOKEN_TABLE_INIT_SIZE,
      sizeof(J9TokenEntry), J9TOKEN_TABLE_ALIGNMENT, 0,
      OMRMEM_CATEGORY_PORT_LIBRARY, tokenHashFn, tokenHashEqualFn, NULL, NULL);

  if (NULL == tokens) {
    goto fail;
  }

  if (0 != populateWithDefaultTokens(
               portLibrary, (struct J9StringTokens *)tokens, timeMillis)) {
    goto fail;
  }
  ...
  percentEntry.key = (char *)portLibrary->mem_allocate_memory(
      portLibrary, 2, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
  percentEntry.value = (char *)portLibrary->mem_allocate_memory(
      portLibrary, 2, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
 ...
 *
 * The token value is determined by expanding format and the var args using
 * omrstr_vprintf. The resulting value is used in a call to
 * omrstr_set_token_from_buf
 *
 ...
 * @parm[in]     key      the key to be modified (e.g. "H" to modify the hour
 * key)
 * @parm[in]     format   a printf format string which describes how to create
 * the value for the token
 * @parm[in]     ...      arguments which match the format string
 *
 * @return 0 on success, -1 on failure. Failure can occur if the key is too
 * large, of size 0 or if we're out of memory.
 *
 */
intptr_t omrstr_set_token(struct OMRPortLibrary *portLibrary,
  ...
  valueLen = portLibrary->str_vprintf(portLibrary, tokenBuf, tokenBufLen,
                                      format, args);
  va_end(args);

  return omrstr_set_token_from_buf(portLibrary, tokens, key, tokenBuf,
                                   (uint32_t)valueLen);
}
/**
 * Add the specified key to the list of tokens.
 * If the key is already in the list, the new value supercedes the existing
 * value.
 *
 * Both key and value will be copied, so it is safe to free or overwrite them
 * once this API returns.
 ...
 * buffer of 511 bytes + 1 null terminator (512 in total) is allocated for
 * holding data. This is useful is RAS where memory must be pre-allocated,
 * allowing strings up to 511 chars + null terminator to be subsequently written
 * into these keys.
 *
 */
static intptr_t omrstr_set_token_from_buf(struct OMRPortLibrary *portLibrary,
...
#define TOKEN_BUF_LEN 511
  ...
  if (!existingEntry) {
    if (NULL == (entry.key = (char *)portLibrary->mem_allocate_memory(
                     portLibrary, entry.keyLen + 1, OMR_GET_CALLSITE(),
    ...
    if (NULL == (entry.value = (char *)portLibrary->mem_allocate_memory(
                     portLibrary, entry.memLen + 1, OMR_GET_CALLSITE(),
                     OMRMEM_CATEGORY_PORT_LIBRARY))) {
...
/**
 * Writes a new string into buf by substituting tokens into format, writing the
 * result into buf.
 *
 * format is a (typically user provided) string which may contain arbitrary
 * tokens to be substituted. A token is any character sequence starting with the
 * % character.
 *
 * When a % character is encountered, this API will compare the following
 * characters with the token provided. The longest key which matches the
 * characters in format will be selected.
 *
 * If buf is not long enough to hold the result, the required length will be
 * returned. buf will always be NUL terminated if it is longer than 0 bytes.
 *
 * @parm[in]  portLib  the port library
 ...
 * @return The number of characters printed not including the NUL terminator.
 *
 * @note When buf is NULL or bufLen is too short, the size of the buffer
 * required to print to the string, including the NUL terminator is returned.
 */
uintptr_t omrstr_subst_tokens(struct OMRPortLibrary *portLibrary, char *buf,
                              uintptr_t bufLen, const char *format,
                              struct J9StringTokens *tokens) {
  J9TokenEntry *entry;
  uintptr_t cnt = 0;
  ...
  /* Second case: we're actually writing to buf. */
    ...
    while ((cnt < bufLen) && *read) {
      if (*read == '%' && (entry = consumeToken(tokens, read))) {
        ...
        uintptr_t lengthToCopy =
            (entry->valueLen < maxExpandLen) ? entry->valueLen : maxExpandLen;
    ...
    /* Second sub-case: the buffer was too small, let's go back and NULL
     * terminate it then set cnt to the minimum buf size required for a
     * successful subst call */
    else {
      buf[bufLen - 1] = '\0';

      /* Calling subst_tokens with a NULL buffer returns the min buf size for
       * success */
 ...
 */
void omrstr_free_tokens(struct OMRPortLibrary *portLibrary,
                        struct J9StringTokens *tokens) {
  if (NULL == tokens) {
    return;
  }
...
/**
 * Returns the UTC time that was passed in as a formatted string in local time.
 * Formatted according to the 'format' parameter.
 *
 * @param[in] portLibrary  The port library.
 * @param[in,out] buf A pointer to a character buffer where the resulting time
 * string will be stored.
 * @param[in] bufLen The length of the 'buf' character buffer.
 * @param[in] format The format string, ordinary characters placed in the format
 * string are copied. to buf without conversion.  Conversion specifiers are
 * introduced by a '%' character, and are replaced in buf as follows:. <ul>
 ...
 * </ul>
 * @param[in] timeMillis The time value to format.  The value is expressed in
 * milliseconds since January 1st 1970.  Note that the implementation will not
 * do any timezone correction and assumes that this value is in terms of the
 * required timezone.
 *
 * @return The number of characters placed in the array buf, not including NULL
 * terminator.
 *
 * If buf is too small, will return the minimum buf size required.
 */
uint32_t omrstr_ftime(struct OMRPortLibrary *portLibrary, char *buf,
                      uint32_t bufLen, const char *format,
                      int64_t timeMillisUTC) {
  if (buf && bufLen > 0) {
...
static uint32_t omrstr_subst_time(struct OMRPortLibrary *portLibrary, char *buf,
  ...
  uint32_t tickSize = 0;
  J9TimeInfo tm;
  static const char abbMonthName[12][4] = {"Jan", "Feb", "Mar", "Apr",
                                           "May", "Jun", "Jul", "Aug",
                                           "Sep", "Oct", "Nov", "Dec"};
  ...
  while (0 != *read) {
    ...
    if (c == '%') {
      ...
      case 'Y':
        count += 4;
        if (count <= bufLen) {
          writeIntToBuffer(write, 4, 4, J9F_NO_VALUE, tm.year, J9FFLAG_ZERO, 0,
                           digits_dec);
          write += 4;
        }
        read += 2;
      ...
      case 'y':
        count += 2;
        if (count <= bufLen) {
          writeIntToBuffer(write, 2, 2, J9F_NO_VALUE, tm.year % 100,
                           J9FFLAG_ZERO, 0, digits_dec);
          write += 2;
        }
        read += 2;
        break;
      case 'm':
        count += 2;
        if (count <= bufLen) {
          writeIntToBuffer(write, 2, 2, J9F_NO_VALUE, tm.month, J9FFLAG_ZERO, 0,
                           digits_dec);
          write += 2;
        }
        read += 2;
      ...
      case 'd':
        count += 2;
        if (count <= bufLen) {
          writeIntToBuffer(write, 2, 2, J9F_NO_VALUE, tm.day, J9FFLAG_ZERO, 0,
                           digits_dec);
          write += 2;
        }
        read += 2;
      ...
      case 'H':
        count += 2;
        if (count <= bufLen) {
          writeIntToBuffer(write, 2, 2, J9F_NO_VALUE, tm.hour, J9FFLAG_ZERO, 0,
                           digits_dec);
          write += 2;
        }
        read += 2;
      ...
      case 'M':
        count += 2;
        if (count <= bufLen) {
          writeIntToBuffer(write, 2, 2, J9F_NO_VALUE, tm.minute, J9FFLAG_ZERO,
                           0, digits_dec);
          write += 2;
        }
        read += 2;
      ...
      case 'S':
        count += 2;
        if (count <= bufLen) {
          writeIntToBuffer(write, 2, 2, J9F_NO_VALUE, tm.second, J9FFLAG_ZERO,
                           0, digits_dec);
          write += 2;
        }
        read += 2;
      ...
      case 'b':
        count += 3;
        if (count <= bufLen) {
          writeStringToBuffer(write, 3, J9F_NO_VALUE, J9F_NO_VALUE,
                              abbMonthName[tm.month - 1], 0);
          write += 3;
        }
        read += 2;
        break;
      ...
      case 't':
        if (strncmp(read + 2, "ick", 3) == 0) {
          ...
          tickSize = (uint32_t)writeIntToBuffer(
              ticks, TICK_BUF_SIZE, J9F_NO_VALUE, J9F_NO_VALUE, curTick,
...
/*
 * Two step process: convert platform encoding to wide characters, then convert
 * wide characters to modified UTF-8. iconv() is resumable if the output buffer
 * is full, but Windows MultiByteToWideChar() is not.
 *  @param[in] codePage specify the "platform" code page: CP_THREAD_ACP, CP_ACP
 * (Windows ANSI code pages) or OS_ENCODING_CODE_PAGE. Ignored on non-Windows
 * systems
 * @param[in] inBuffer  input string  to be converted
 * @param[in] inBufferSize input string size in bytes.
 * @param[in] outBuffer user-allocated output buffer that stores converted
 * characters, ignored if inBufferSize is 0.
 * @param[in] outBufferSize output buffer size in bytes (zero to request the
 * required output buffer size)
 * @return number of bytes generated, or required size of output buffer if
 * outBufferSize is 0. Negative error code on failure.
 */
static int32_t
convertPlatformToMutf8(struct OMRPortLibrary *portLibrary, uint32_t codePage,
                       const uint8_t *inBuffer, uintptr_t inBufferSize,
  ...
  encodingState = NULL;
  /* MultiByteToWideChar is not resumable, so we need a buffer large enough to
   * hold the entire intermediate result */
  requiredBufferSize =
      WIDE_CHAR_SIZE * MultiByteToWideChar(codePage, OS_ENCODING_MB_FLAGS,
                                           (LPCSTR)inBuffer, (int)inBufferSize,
                                           NULL,
                                           0); /* get required buffer size */
  if (requiredBufferSize > CONVERSION_BUFFER_SIZE) {
    wideBuffer = portLibrary->mem_allocate_memory(
        portLibrary, requiredBufferSize, OMR_GET_CALLSITE(),
        OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == wideBuffer) {
  ...
  encodingState = iconv_get(portLibrary, OMRPORT_LANG_TO_UTF16_ICONV_DESCRIPTOR,
                            utf16, nl_langinfo(CODESET));
...
#endif
  /* do the conversion. outBufferSize==0 indicates that only the buffer size is
   * required. */
  while ((platformRemaining > 0) &&
         ((0 == outBufferSize) || (mutf8Limit > 0))) {
    int32_t wideBufferPartialSize = convertPlatformToWide(
        portLibrary, encodingState, codePage, &platformCursor,
...
#endif /* defined(OMR_OS_WINDOWS) */
#if defined(J9STR_USE_ICONV)
      iconv_free(portLibrary, OMRPORT_LANG_TO_UTF16_ICONV_DESCRIPTOR,
                 encodingState);
#endif
    ...
    mutf8PartialSize = convertWideToMutf8(&tempWideBuffer, &tempWideBufferSize,
                                          mutf8Cursor, mutf8Limit);
...
/**
 * Two step process: convert modified UTF-8 to wide characters, then wide
 * characters to platform encoding. iconv() both is resumable if the output
 * buffer is full, but Windows WideCharToMultiByte() is not.
...
convertMutf8ToPlatform(struct OMRPortLibrary *portLibrary, uint32_t codePage,
...
#if defined(J9STR_USE_ICONV)
  encodingState = iconv_get(portLibrary, OMRPORT_UTF16_TO_LANG_ICONV_DESCRIPTOR,
                            nl_langinfo(CODESET), utf16);
  if (J9VM_INVALID_ICONV_DESCRIPTOR == encodingState) {
    /* wideBuffer is always on stack - no need to free it */
    return OMRPORT_ERROR_STRING_ICONV_OPEN_FAILED;
  }
  ...
  encodingState = NULL;
#endif                         /* defined(OMR_OS_WINDOWS) */
  while (mutf8Remaining > 0) { /* translated section by section as dictated by
                                  the size of wideBuffer */
    ...
    int32_t result = convertMutf8ToWide(&mutf8Cursor, &mutf8Remaining,
                                        wideBuffer, wideBufferSize);
    if (result < 0) { /* conversion error */
      return result;
    }
    wideBufferCount = (uintptr_t)result;

    platformPartCount = convertWideToPlatform(
        portLibrary, encodingState, codePage, &wideBufferCursor,
        &wideBufferCount, outCursor, outLimit);
    /*
     * convertWideToPlatform may return OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL.
     * convertWideToPlatform sets up to MAX_STRING_TERMINATOR_LENGTH bytes bytes
     * after the converted characters to 0, depending on the length of the
     * buffer
     */
    if (platformPartCount <
        0) { /* conversion error or output buffer too small */
#if defined(J9STR_USE_ICONV)
      iconv_free(portLibrary, OMRPORT_UTF16_TO_LANG_ICONV_DESCRIPTOR,
                 encodingState);
#endif
...
static int32_t convertPlatformToUtf8(struct OMRPortLibrary *portLibrary,
  ...
  int32_t resultSize = 0;
#if defined(J9STR_USE_ICONV)
  /* J9STR_USE_ICONV is defined on LINUX, AIXPPC and J9ZOS390 at the beginning
   * of the file */
  ...
  charconvState_t converter =
      iconv_get(portLibrary, OMRPORT_LANG_TO_UTF8_ICONV_DESCRIPTOR, utf8,
                nl_langinfo(CODESET));
  ...
  while ((outbytesleft > 0) && (inbytesleft > 0)) {
    if ((size_t)-1 ==
      ...
      if (errno == E2BIG) {
        iconv_free(portLibrary, OMRPORT_LANG_TO_UTF8_ICONV_DESCRIPTOR,
                   converter);
        return OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL;
      }
 ...
 * If the buffer is large enough, the byte after the last output character is
 * set to 0. Resumable, so it updates its inputs
 * @param[inout]  inBuffer input string  to be converted.   Updated to first
 * untranslated character.
 * @param[inout]  inBufferSize input string size in bytes.  Updated to number of
 * untranslated characters. Must be even.
 * @param[in] outBuffer user-allocated output buffer that stores converted
 * characters, ignored if outBufferSize is 0.
 * @param[in] outBufferSize output buffer size in bytes (zero to request the
 * output buffer size).
...
static int32_t convertWideToMutf8(const uint8_t **inBuffer,
                                  uintptr_t *inBufferSize, uint8_t *outBuffer,
                                  uintptr_t outBufferSize) {
  uintptr_t wideRemaining =
      *inBufferSize; /* number of untranslated characters in inBuffer */
  const uint8_t *wideCursor = *inBuffer;
  int32_t resultSize = 0;
  ...
  if (0 == outBufferSize) { /* we just want the length */
    while (wideRemaining > 0) {
      ...
      encodeResult = encodeUTF8CharN(
          wideChar, NULL, 3); /* convert one wide character, get the length,
                                 ignore the converted result */
      if (0 == encodeResult) {
...
static int32_t convertLatin1ToMutf8(struct OMRPortLibrary *portLibrary,
  ...
  uintptr_t latinRemaining =
      *inBufferSize; /* number of untranslated characters in inBuffer */
  uint8_t *mutf8Cursor = outBuffer; /* Mutable copy */
  uintptr_t mutf8Remaining =
      outBufferSize; /* number of unused bytes in outBuffer */
  int32_t resultSize = 0;

  while ((latinRemaining > 0) &&
         ((mutf8Remaining > 0) ||
          (0 == outBufferSize)) /* outBufferSize == 0 indicates we just want the
                                   length */
         && (resultSize >= 0)) {
    /* still have input data, output space, and no errors */
    /* convert in CONVERSION_BUFFER_SIZE-size segments */
    uintptr_t wideLimit = (latinRemaining > CONVERSION_BUFFER_SIZE)
                              ? CONVERSION_BUFFER_SIZE
                              : latinRemaining;
    uintptr_t latinBytesConsumed = 0;
    uint16_t
        localWideBuffer[CONVERSION_BUFFER_SIZE]; /* handle short strings without
                                                    allocating memory */
    ...
    /* copy the characters to an array of Unicode characters and transliterate
     * from Unicode to modified UTF-8 */
    for (cursor = 0; cursor < wideLimit; ++cursor) {
      wideBuffer[cursor] = latinString[cursor] & 0x00ff;
    }

    mutf8Result =
        convertWideToMutf8((const uint8_t **)&wideBuffer, &wideRemaining,
                           mutf8Cursor, mutf8Remaining);
    latinBytesConsumed = (wideLength - wideRemaining) /
                         2; /* 1 Latin byte per 2 wide characters */
    latinRemaining -= latinBytesConsumed;
    latinString +=
        latinBytesConsumed; /* Now points to first unconsumed character */
...
static int32_t convertMutf8ToLatin1(struct OMRPortLibrary *portLibrary,
  ...
  while ((mutf8Remaining > 0) &&
    ...
    /* still have input data, output space, and no errors */
    /* convert in CONVERSION_BUFFER_SIZE-size segments */
    uintptr_t wideLimit = (mutf8Remaining > CONVERSION_BUFFER_SIZE)
                              ? CONVERSION_BUFFER_SIZE
                              : mutf8Remaining;
    uint16_t
        localWideBuffer[CONVERSION_BUFFER_SIZE]; /* handle short strings without
    ...
    int32_t unicodeResultBytes = convertMutf8ToWide(
        &mutf8String, &mutf8Remaining, (uint8_t *)wideBuffer, wideRemaining);
    mutf8BytesConsumed = (wideLength - wideRemaining) /
                         2; /* 1 mutf8 byte per 2 wide characters */
    mutf8Remaining -= mutf8BytesConsumed;
    mutf8String +=
        mutf8BytesConsumed; /* Now points to first unconsumed character */
    if (unicodeResultBytes < 0) {
      /* conversion error */
      resultSize = unicodeResultBytes;
    } else {
      int32_t latin1CharactersConverted =
          (unicodeResultBytes /
           2); /* Latin1 characters take up only 1 of 2 wide bytes */
...
/**
 * Test if the second and later bytes of a UTF-8 character are well-formed, i.e.
 * 2 MSbs are 10 If they are and mutf8Bytes is not null, copy the bytes.
 ...
 */
static BOOLEAN checkAndCopyUtfBytes(const uint8_t *utf8Bytes,
                                    uintptr_t numBytes, uint8_t *mutf8Bytes) {
  uintptr_t cursor;
...
/*
 * Convert Unicode UTF8 (including supplementary characters) encoding to
 * modified UTF-8. May terminate early if the output buffer is too small.
 * Resumable, so it updates its inputs.
 * If the buffer is large enough, the byte after the last output character is
 * set to 0.
 * @param[inout]  inBuffer input string  to be converted.   Updated to first
 ...
 * output buffer size).
 * @return number of bytes generated, or required size of output buffer if
 * outBufferSize is 0, not including any final zero byte.
 * @note invalid characters are replaced with the Unicode code point U+FFFD
...
static int32_t convertUtf8ToMutf8(struct OMRPortLibrary *portLibrary,
  ...
  while ((utf8BufferSize > 0) && (lengthOnly || (mutf8BufferSize > 0))) {
    ...
    if ((0 == utf8Buffer[0]) &&
        (lengthOnly ||
         (mutf8BufferSize > 1))) { /* null - convert to double-byte form */
      if (!lengthOnly) {
        mutf8Buffer[0] = 0xc0;
        mutf8Buffer[1] = 0x80;
      }
      ...
      produced = 3;
               ...
               (lengthOnly ||
                (mutf8BufferSize > 5))) { /* Quadruple byte UTF-8 */
      ...
      if (!lengthOnly) {
        mutf8Buffer[0] = 0xED;
        mutf8Buffer[1] =
            0xA0 + (((unicodeValue >> 16) - 1) & 0x0f); /* (bits 20-16) - 1 */
        mutf8Buffer[2] =
            0x80 | ((unicodeValue & 0x0FC00) >> 10); /* bits 15-10 */
        mutf8Buffer[3] = 0xED;
 ...
 * @param[in] outBufferSize output buffer size in bytes (zero to request the
 * output buffer size).
 * @return number of bytes generated, or required size in bytes of output buffer
 * if outBufferSize is 0. Negative error code on failure.
 * @note callers are responsible for detecting buffer overflow
 */
static int32_t convertMutf8ToWide(const uint8_t **inBuffer,
                                  uintptr_t *inBufferSize, uint8_t *outBuffer,
                                  uintptr_t outBufferSize) {
  uintptr_t mutf8Remaining =
      *inBufferSize; /* number of untranslated bytes in inBuffer */
  const uint8_t *mutf8Cursor = *inBuffer;
  int32_t resultSize = 0;
  if (0 == outBufferSize) { /* we just want the length */
    uint16_t temp;
    while (mutf8Remaining > 0) {
      uint32_t bytesConsumed =
          decodeUTF8CharN(mutf8Cursor, &temp, mutf8Remaining);
      if (0 == bytesConsumed) {
        return OMRPORT_ERROR_STRING_ILLEGAL_STRING;
      }
    ...
    while ((wideBufferLimit > 0) && (mutf8Remaining > 0)) {
      uint32_t bytesConsumed =
          decodeUTF8CharN(mutf8Cursor, wideBufferCursor, mutf8Remaining);
      if (0 == bytesConsumed) {
        return OMRPORT_ERROR_STRING_ILLEGAL_STRING;
      }
      mutf8Remaining -= bytesConsumed;
...
/*
 * transliterate from platform encoding to wide (UTF-16).  The input buffer
 * pointer and size are updated (except on Windows) to allow resumption if the
 * translation failed due to output buffer too small.  The output buffer may not
 * be null. The output buffer may contain additional data such as a byte order
 * mark (0xff, 0xfe or vice versa) Resumable, so it updates its inputs
 * @param [in] portLibrary port library
 * @param [in] encodingState iconv_t on on Linux, z/OS and AIX, void * on
 * Windows (unused)
 * @param[in] codePage specify the "platform" code page: CI_ACP (Windows ANSI
 * code page) or OS_ENCODING_CODE_PAGE.  Ignored on non-Windows systems
 * @param [inout] inBuffer point to start of buffer containing platform-encoded
 * string.  Updated to point to start of first untranslated character
 * @param [inout] inBufferSize size in characters of the platform-encoded
 * string.  Updated to indicate number of remaining untranslated characters.
...
convertPlatformToWide(struct OMRPortLibrary *portLibrary,
...
#if defined(OMR_OS_WINDOWS)
  int32_t mbChars = (int32_t)MultiByteToWideChar(
      codePage, OS_ENCODING_MB_FLAGS, (LPCSTR)*inBuffer, (int)*inBufferSize,
      (LPWSTR)outBuffer, (int)outBufferSize);
  if ((outBufferSize > 0) && (0 == mbChars)) {
    resultSize =
        OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL; /* should not happen: caller
                                                  should have allocated a
  ...
  if ((size_t)-1 ==
      iconv(encodingState, (char **)inBuffer, (size_t *)inBufferSize,
            (char **)&wideBufferCursor,
    ...
    if (EILSEQ == errno) {
      ...
      *inBufferSize -= 1;
      wideBufferLimit -= 2;
      resultSize =
          (outBufferSize - wideBufferLimit); /* number of bytes written */
    } else if (E2BIG == errno) {
      resultSize =
          (outBufferSize - wideBufferLimit); /* number of bytes written */
    } else {
...
/**
 * Transliterate from wide (UTF-16) to platform encoding. The input and output
 * buffer pointers and sizes are updated (except on Windows) to allow resumption
 * if the translation failed due to output buffer too small. Resumable, so it
 * updates its inputs. The output buffer may contain additional data such as a
 * byte order mark (0xff, 0xfe or vice versa). If the buffer is large enough,
 * the byte after the last output character is set to 0.
 * @param [in] portLibrary port library
 * @param [in] encodingState iconv_t on Linux, z/OS and AIX, void * on Windows
 * (unused)
 * @param [in] codePage specify the "platform" code page: CP_THREAD_ACP, CP_ACP
 * (Windows ANSI code pages) or OS_ENCODING_CODE_PAGE. Ignored on non-Windows
 * systems
 * @param [inout] inBuffer input buffer. Updated to point to start of first
 * untranslated character
 * @param [inout] inBufferSize number of wide characters in wideBuffer. Updated
 * to number of untranslated characters
 * @param [in] outBuffer point to start of buffer to receive platform-encoded
 * string
 * @param [in] outBufferSize size in characters of the platformBuffer. Set to 0
 * to get the length of bytes required to hold the output
...
convertWideToPlatform(struct OMRPortLibrary *portLibrary,
  ...
  if (0 == outBufferSize) { /* get the required buffer size */
    ...
    while (wideRemaining > 0) {
      ...
      oldOsbSize = osbSize;
      iconvResult =
          iconv(encodingState, (char **)inBuffer, (size_t *)&wideRemaining,
                &osbCursor, (size_t *)&osbSize); /* input and output pointers
                                                    and limits are updated */
    ...
    /* initialize convertor */
    uintptr_t iconvResult =
        iconv(encodingState, NULL, 0, (char **)&platformCursor,
              (size_t *)&platformLimit);
    iconvResult = iconv(encodingState, (char **)inBuffer,
                        (size_t *)inBufferSize, (char **)&platformCursor,
                        (size_t *)&platformLimit); /* input and output pointers
                                                      and limits are updated */
  ...
  resultSize = WideCharToMultiByte(codePage, OS_ENCODING_MB_FLAGS, wideCursor,
                                   (int)wideRemaining, (LPSTR)outBuffer,
                                   (int)outBufferSize, NULL, NULL);
  if (0 == resultSize) {
...
#endif
  if ((resultSize >= 0) && (outBufferSize > 0) &&
      (((uintptr_t)resultSize) < outBufferSize)) {
    ...
    memset(outBuffer + resultSize, 0,
           terminatorLength); /* null-terminate if possible */
  }
