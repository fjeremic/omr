...
static void readValues(struct OMRPortLibrary* portLibrary,
                       J9FormatData* result,
                       va_list args);
static int parseFormatString(struct OMRPortLibrary* portLibrary,
                             J9FormatData* result);
static uintptr_t writeDoubleToBuffer(char* buf,
                                     uintptr_t bufLen,
                                     uint64_t width,
                                     uint64_t precision,
                                     double value,
...
static uintptr_t writeFormattedString(struct OMRPortLibrary* portLibrary,
                                      ...
                                      char* result,
                                      uintptr_t length);
static uintptr_t writeSpec(J9FormatData* data,
                           J9FormatSpecifier* spec,
                           char* result,
                           uintptr_t length);
static const char* parseIndex(const char* format, uint8_t* result);
static uintptr_t writeStringToBuffer(char* buf,
                                     uintptr_t bufLen,
                                     uint64_t width,
                                     uint64_t precision,
                                     const char* value,
...
static uintptr_t writeIntToBuffer(char* buf,
                                  uintptr_t bufLen,
                                  uint64_t width,
                                  uint64_t precision,
                                  uint64_t value,
                                  ...
                                  int isSigned,
                                  const char* digits);
static uintptr_t writeUnicodeStringToBuffer(char* buf,
                                            uintptr_t bufLen,
                                            uint64_t width,
                                            uint64_t precision,
...
static uint32_t omrstr_subst_time(struct OMRPortLibrary* portLibrary,
                                  char* buf,
                                  uint32_t bufLen,
                                  const char* format,
                                  int64_t timeMillis);
...
static int32_t convertMutf8ToWide(const uint8_t** inBuffer,
                                  ...
                                  uint8_t* outBuffer,
                                  uintptr_t outBufferSize);
static int32_t convertPlatformToWide(struct OMRPortLibrary* portLibrary,
                                     ...
                                     uint8_t* outBuffer,
                                     uintptr_t outBufferSize);
static int32_t convertWideToPlatform(struct OMRPortLibrary* portLibrary,
                                     charconvState_t encodingState,
                                     uint32_t codePage,
                                     const uint8_t** inBuffer,
                                     uintptr_t* inBufferSize,
 ...
 * database.
 */
uintptr_t omrstr_printf(struct OMRPortLibrary* portLibrary,
                        char* buf,
                        uintptr_t bufLen,
                        const char* format,
                        ...) {
...
#endif
int32_t omrstr_convert(struct OMRPortLibrary* portLibrary,
                       int32_t fromCode,
                       int32_t toCode,
                       uint8_t* inBuffer,
                       uintptr_t inBufferSize,
 ...
 * database.
 */
uintptr_t omrstr_vprintf(struct OMRPortLibrary* portLibrary,
                         char* buf,
                         uintptr_t bufLen,
                         const char* format,
                         va_list args) {
 ...
 */
int32_t omrstr_startup(struct OMRPortLibrary* portLibrary) {
  return 0;
}
static uintptr_t writeUnicodeStringToBuffer(char* buf,
                                            uintptr_t bufLen,
 ...
 * string
 */
static uintptr_t tokenHashEqualFn(void* lhsEntry,
                                  void* rhsEntry,
                                  void* userData) {
 ...
 */
uintptr_t omrstr_subst_tokens(struct OMRPortLibrary* portLibrary,
                              char* buf,
                              uintptr_t bufLen,
                              const char* format,
                              struct J9StringTokens* tokens) {
 ...
 */
uint32_t omrstr_ftime(struct OMRPortLibrary* portLibrary,
                      char* buf,
                      uint32_t bufLen,
                      const char* format,
...
static int32_t convertWideToPlatform(struct OMRPortLibrary* portLibrary,
  ...
  if (0 == outBufferSize) { /* get the required buffer size */
    ...
    while (wideRemaining > 0) {
      ...
      iconvResult =
          iconv(encodingState, (char**)inBuffer, (size_t*)&wideRemaining,
                &osbCursor, (size_t*)&osbSize); /* input and output pointers and
                                                   limits are updated */
    ...
    iconvResult = iconv(encodingState, (char**)inBuffer, (size_t*)inBufferSize,
                        (char**)&platformCursor,
