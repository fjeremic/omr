...
static intptr_t omrstr_set_token_from_buf(struct OMRPortLibrary *portLibrary,
                                          ...
                                          uint32_t tokenLen);
static int32_t convertPlatformToMutf8(struct OMRPortLibrary *portLibrary,
                                      uint32_t codePage,
                                      const uint8_t *inBuffer,
                                      uintptr_t inBufferSize,
                                      uint8_t *outBuffer,
                                      uintptr_t outBufferSize);
static int32_t convertMutf8ToPlatform(struct OMRPortLibrary *portLibrary,
                                      uint32_t codePage,
                                      const uint8_t *inBuffer,
                                      uintptr_t inBufferSize,
                                      uint8_t *outBuffer,
...
static int32_t convertMutf8ToWide(const uint8_t **inBuffer,
                                  uintptr_t *inBufferSize, uint8_t *outBuffer,
                                  uintptr_t outBufferSize);
static int32_t convertPlatformToWide(
    struct OMRPortLibrary *portLibrary, charconvState_t encodingState,
    uint32_t codePage, const uint8_t **inBuffer, uintptr_t *inBufferSize,
    uint8_t *outBuffer, uintptr_t outBufferSize);
static int32_t convertWideToPlatform(
    struct OMRPortLibrary *portLibrary, charconvState_t encodingState,
    uint32_t codePage, const uint8_t **inBuffer, uintptr_t *inBufferSize,
    uint8_t *outBuffer, uintptr_t outBufferSize);
...
int32_t omrstr_convert(struct OMRPortLibrary *portLibrary, int32_t fromCode,
  ...
  switch (fromCode) {
    case J9STR_CODE_PLATFORM_RAW: {
      switch (toCode) {
        case J9STR_CODE_MUTF8:
          result = convertPlatformToMutf8(portLibrary, OS_ENCODING_CODE_PAGE,
        ...
        case J9STR_CODE_UTF8:
          result =
              convertPlatformToUtf8(portLibrary, (const uint8_t *)inBuffer,
    ...
    case J9STR_CODE_MUTF8: {
      switch (toCode) {
        case J9STR_CODE_PLATFORM_RAW:
          result = convertMutf8ToPlatform(portLibrary, OS_ENCODING_CODE_PAGE,
                                          inBuffer, inBufferSize, outBuffer,
                                          outBufferSize);
          break;
        case J9STR_CODE_LATIN1: {
          ...
          result =
              convertMutf8ToLatin1(portLibrary, &mutf8Cursor, &mutf8Remaining,
                                   outBuffer, outBufferSize);
          /*
           * convertMutf8ToLatin1 is resumable so does not return error if
           * buffer too small. In this case we should have consumed all the data
           */
          if (mutf8Remaining > 0) {
        ...
        case J9STR_CODE_WINTHREADACP:
          result =
              convertMutf8ToPlatform(portLibrary, CP_THREAD_ACP, inBuffer,
                                     inBufferSize, outBuffer, outBufferSize);
          break;
        case J9STR_CODE_WINDEFAULTACP:
          result =
              convertMutf8ToPlatform(portLibrary, CP_ACP, inBuffer,
                                     inBufferSize, outBuffer, outBufferSize);
          break;
#endif /* defined(OMR_OS_WINDOWS) */
        case J9STR_CODE_WIDE: {
          ...
          result = convertMutf8ToWide(&mutf8Cursor, &mutf8Remaining, outBuffer,
                                      outBufferSize);
          /*
           * inBuffer parameters are updated to reflect  data untranslated due
           * to insufficient space in the output buffer. In this case, all input
      ...
      switch (toCode) {
        case J9STR_CODE_MUTF8: {
          ...
          result = convertUtf8ToMutf8(portLibrary, &utf8Cursor, &utf8Remaining,
                                      outBuffer, outBufferSize);
          /*
           * convertUtf8ToMutf8 is resumable so does not return error if buffer
    ...
    case J9STR_CODE_LATIN1: {
      switch (toCode) {
        case J9STR_CODE_MUTF8: {
          ...
          result =
              convertLatin1ToMutf8(portLibrary, &latin1Cursor, &latin1Remaining,
                                   outBuffer, outBufferSize);
          /*
           * convertLatin1ToMutf8 is resumable so does not return error if
           * buffer too small. In this case we should have consumed all the data
           */
  ...
  for (index = 0; index < result->valueCount; index++) {
    switch (result->valueType[index]) {
      ...
      case J9FTYPE_IMMEDIATE:
        /* shouldn't be encountered -- these should all be at the end of the
         * value array */
        break;
    }
  }
  ...
  switch (*spec->type) {
    ...
    case 'p':
      index = writeIntToBuffer(result, length, sizeof(uintptr_t) * 2,
                               sizeof(uintptr_t) * 2, (uintptr_t)value->ptr, 0,
                               0, digits_hex_upper);
      break;
    ...
    case 's':
      if (value->ptr) {
        if (spec->tag & J9FSPEC_L) {
          index = writeUnicodeStringToBuffer(result, length, width, precision,
    ...
    if (c == '%') {
      ...
      switch (key) {
        case 'Y':
          count += 4;
          if (count <= bufLen) {
            writeIntToBuffer(write, 4, 4, J9F_NO_VALUE, tm.year, J9FFLAG_ZERO,
                             0, digits_dec);
            write += 4;
          }
          read += 2;
        ...
        case 'm':
          count += 2;
          if (count <= bufLen) {
            writeIntToBuffer(write, 2, 2, J9F_NO_VALUE, tm.month, J9FFLAG_ZERO,
                             0, digits_dec);
            write += 2;
          }
          read += 2;
        ...
        case 'H':
          count += 2;
          if (count <= bufLen) {
            writeIntToBuffer(write, 2, 2, J9F_NO_VALUE, tm.hour, J9FFLAG_ZERO,
                             0, digits_dec);
            write += 2;
          }
          read += 2;
