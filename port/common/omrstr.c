  ...
  uintptr_t inBufferSize, uint8_t* outBuffer, uintptr_t outBufferSize)
{
  ...
  case J9STR_CODE_PLATFORM_RAW: {
    ...
    case J9STR_CODE_MUTF8:
      result
        = convertPlatformToMutf8(portLibrary, OS_ENCODING_CODE_PAGE, inBuffer, inBufferSize, outBuffer, outBufferSize);
      break;
  ...
  case J9STR_CODE_MUTF8: {
    ...
    case J9STR_CODE_PLATFORM_RAW:
      result
        = convertMutf8ToPlatform(portLibrary, OS_ENCODING_CODE_PAGE, inBuffer, inBufferSize, outBuffer, outBufferSize);
      break;
...
static uintptr_t writeSpec(J9FormatData* data, J9FormatSpecifier* spec, char* result, uintptr_t length)
{
  ...
  case 'p':
    index = writeIntToBuffer(
      result, length, sizeof(uintptr_t) * 2, sizeof(uintptr_t) * 2, (uintptr_t)value->ptr, 0, 0, digits_hex_upper);
    break;
...
#define J9SFT_NUM_SECS_IN_LEAP_FEB (29 * J9SFT_NUM_SECS_IN_DAY)
  ...
  int32_t normalSecondsInMonth[12] = { J9SFT_NUM_SECS_IN_JAN, J9SFT_NUM_SECS_IN_FEB, J9SFT_NUM_SECS_IN_MAR,
    J9SFT_NUM_SECS_IN_APR, J9SFT_NUM_SECS_IN_MAY, J9SFT_NUM_SECS_IN_JUN, J9SFT_NUM_SECS_IN_JUL, J9SFT_NUM_SECS_IN_AUG,
 ...
 *
 */
static intptr_t omrstr_set_token_from_buf(
  struct OMRPortLibrary* portLibrary, struct J9StringTokens* tokens, const char* key, char* tokenBuf, uint32_t tokenLen)
{
  J9TokenEntry entry;
  ...
  uintptr_t inBufferSize, uint8_t* outBuffer, uintptr_t outBufferSize)
{
  ...
  requiredBufferSize = WIDE_CHAR_SIZE
    * MultiByteToWideChar(
      codePage, OS_ENCODING_MB_FLAGS, (LPCSTR)inBuffer, (int)inBufferSize, NULL, 0); /* get required buffer size */
  ...
  uint32_t codePage, const uint8_t** inBuffer, uintptr_t* inBufferSize, uint8_t* outBuffer, uintptr_t outBufferSize)
{
  ...
  resultSize = WideCharToMultiByte(
    codePage, OS_ENCODING_MB_FLAGS, wideCursor, (int)wideRemaining, (LPSTR)outBuffer, (int)outBufferSize, NULL, NULL);
  if (0 == resultSize) {
