 ...
 *
 * @param[in] 	tagAddress 	the in-process or out-of-proces address of the
...
uint32_t checkTagSumCheck(J9MemTag* tagAddress, uint32_t eyeCatcher) {
  ...
  /* Could be unrolled into chained xors with a OMR_ENV_DATA64 conditional on
   * the extra 2 uint32_ts */
  for (i = 0; i < (sizeof(J9MemTag) / sizeof(uint32_t)); i++) {
    sum ^= *slots++;
  }
#ifdef OMR_ENV_DATA64
  sum ^= ((uint32_t)(((uintptr_t)tagAddress) >> 32)) ^
         ((uint32_t)(((uintptr_t)tagAddress) & U_32_MAX));
#else
 ...
 * corrupted
 *
 * @param[in]	headerTagAddress	the address of the header memory tag for
 * the memory block.
 *
...
/* Given the address of the headerEyecatcher for the memory block, return the
 * memory pointer that was returned by omrmem_allocate_memory() when the block
 * was allocated.
 */
...
/**
 * Given the address of the headerEyecatcher for the memory block, return the
 * address of the footer padding.
 *
 * Note that there not be any padding, in which case this returns the same as
 * @ref omrmem_get_footer_tag(), the address of the footer tag.
 *
 */
...
/**
 * Given the address of the headerEyecatcher for the memory block, return the
 * address of the corresponding footer tag.
 */
J9MemTag* omrmem_get_footer_tag(J9MemTag* headerEyeCatcherAddress) {
  ...
  return (J9MemTag*)((uint8_t*)headerEyeCatcherAddress +
                     ROUNDED_FOOTER_OFFSET(headerTagStoragePointer->allocSize));
}

/**
 * Given the address returned by @ref omrmem_allocate_memory(), return address
 * of the header tag for the memory block
 *
 */
