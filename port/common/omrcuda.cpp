...
#define J9CUDA_ALLOCATE_MEMORY(byteCount)                                      \
  OMRPORTLIB->mem_allocate_memory(OMRPORTLIB, byteCount, OMR_GET_CALLSITE(),   \
                                  OMRMEM_CATEGORY_CUDA)
 ...
 */
int32_t initializeTable(OMRPortLibrary *portLibrary, uintptr_t dllHandle,
                        uint32_t libraryVersion,
                        const J9CudaEntryDescriptor *entries,
                        uintptr_t entryCount, uintptr_t *table) {
  ...
  for (uintptr_t index = 0; index < entryCount; ++index) {
    ...
    if (0 != portLibrary->sl_lookup_name(portLibrary, dllHandle,
                                         (char *)entry->name, &function,
                                         entry->signature)) {
 ...
 */
int32_t getDeviceData(OMRPortLibrary *portLibrary, uint32_t deviceId,
  ...
  if (globals->deviceCount <= deviceId) {
    ...
    if (J9CUDA_NO_ERROR !=
        omrcuda_deviceGetMemInfo(portLibrary, deviceId,
 ...
 */
VMINLINE uint32_t normalizeVersion(uint32_t apiVersion) {
 ...
 * @return J9CUDA_NO_ERROR on success or a non-zero value on failure
 */
int32_t getSummaryData(OMRPortLibrary *portLibrary,
                       J9CudaSummaryDescriptor *summaryData) {
...
struct J9CudaFunctionTable {
  /* The signatures of the following must match the driver API declared in
   * cuda.h. */
  CUresult(CUDAAPI *DeviceGet)(CUdevice *device, int ordinal);
  CUresult(CUDAAPI *DeviceGetName)(char *nameOut, int nameSize,
                                   CUdevice device);
  CUresult(CUDAAPI *FuncGetAttribute)(int *value,
                                      CUfunction_attribute attribute,
                                      CUfunction function);
  CUresult(CUDAAPI *FuncSetCacheConfig)(CUfunction function,
                                        CUfunc_cache config);
  CUresult(CUDAAPI *FuncSetSharedMemConfig)(CUfunction function,
                                            CUsharedconfig config);
  CUresult(CUDAAPI *GetDriverErrorString)(CUresult error, const char **nameOut);
  CUresult(CUDAAPI *LaunchKernel)(CUfunction function, unsigned int gridDimX,
  ...
  CUresult(CUDAAPI *LinkAddData)(CUlinkState state, CUjitInputType type,
                                 void *data, size_t size, const char *name,
                                 unsigned int numOptions, CUjit_option *options,
                                 void **optionValues);
  CUresult(CUDAAPI *LinkComplete)(CUlinkState state, void **cubinOut,
                                  size_t *sizeOut);
  CUresult(CUDAAPI *LinkCreate)(unsigned int numOptions, CUjit_option *options,
                                void **optionValues, CUlinkState *stateOut);
  CUresult(CUDAAPI *LinkDestroy)(CUlinkState state);
  CUresult(CUDAAPI *MemsetD8Async)(CUdeviceptr dstDevice, unsigned char value,
                                   size_t count, CUstream stream);
  CUresult(CUDAAPI *MemsetD16Async)(CUdeviceptr dstDevice, unsigned short value,
                                    size_t count, CUstream stream);
  CUresult(CUDAAPI *MemsetD32Async)(CUdeviceptr dstDevice, unsigned int value,
                                    size_t count, CUstream stream);
  CUresult(CUDAAPI *ModuleGetFunction)(CUfunction *functionOut, CUmodule module,
                                       const char *name);
  CUresult(CUDAAPI *ModuleGetGlobal)(CUdeviceptr *addressOut, size_t *sizeOut,
                                     CUmodule module, const char *name);
  CUresult(CUDAAPI *ModuleGetSurfRef)(CUsurfref *surfRefOut, CUmodule module,
                                      const char *name);
  CUresult(CUDAAPI *ModuleGetTexRef)(CUtexref *texRefOut, CUmodule module,
                                     const char *name);
  CUresult(CUDAAPI *ModuleLoadDataEx)(CUmodule *module, const void *image,
                                      unsigned int numOptions,
                                      CUjit_option *options,
                                      void **optionValues);
  CUresult(CUDAAPI *ModuleUnload)(CUmodule module);
  CUresult(CUDAAPI *OccupancyMaxActiveBlocksPerMultiprocessorWithFlags)(
      int *numBlocks, CUfunction function, int blockSize,
      size_t dynamicSMemSize, unsigned int flags);

  /* The signatures of the following must match the runtime API declared in
   * cuda_runtime_api.h. */
  cudaError_t(CUDARTAPI *DeviceCanAccessPeer)(int *canAccessPeerOut, int device,
                                              int peerDevice);
  cudaError_t(CUDARTAPI *DeviceDisablePeerAccess)(int peerDevice);
  cudaError_t(CUDARTAPI *DeviceEnablePeerAccess)(int peerDevice,
                                                 unsigned int flags);
  cudaError_t(CUDARTAPI *DeviceGetAttribute)(int *value, cudaDeviceAttr attr,
                                             int device);
  ...
  cudaError_t(CUDARTAPI *DeviceGetStreamPriorityRange)(int *leastPriority,
  ...
  cudaError_t(CUDARTAPI *EventCreateWithFlags)(cudaEvent_t *event,
                                               unsigned int flags);
  cudaError_t(CUDARTAPI *EventDestroy)(cudaEvent_t event);
  cudaError_t(CUDARTAPI *EventElapsedTime)(float *elapsedMillis,
  ...
  cudaError_t(CUDARTAPI *HostAlloc)(void **pHost, size_t size,
                                    unsigned int flags);
  cudaError_t(CUDARTAPI *Malloc)(void **deviceAddressOut, size_t size);
  cudaError_t(CUDARTAPI *Memcpy)(void *dst, const void *src, size_t count,
                                 cudaMemcpyKind kind);
  cudaError_t(CUDARTAPI *MemcpyAsync)(void *dst, const void *src, size_t count,
                                      cudaMemcpyKind kind, cudaStream_t stream);
  cudaError_t(CUDARTAPI *Memcpy2D)(void *dst, size_t dpitch, const void *src,
                                   size_t spitch, size_t width, size_t height,
                                   cudaMemcpyKind kind);
  cudaError_t(CUDARTAPI *Memcpy2DAsync)(void *dst, size_t dpitch,
                                        const void *src, size_t spitch,
                                        size_t width, size_t height,
                                        cudaMemcpyKind kind,
                                        cudaStream_t stream);
  cudaError_t(CUDARTAPI *MemcpyPeer)(void *dst, int dstDevice, const void *src,
                                     int srcDevice, size_t count);
  cudaError_t(CUDARTAPI *MemcpyPeerAsync)(void *dst, int dstDevice,
                                          const void *src, int srcDevice,
                                          size_t count, cudaStream_t stream);
  cudaError_t(CUDARTAPI *MemGetInfo)(size_t *free, size_t *total);
  cudaError_t(CUDARTAPI *SetDevice)(int deviceId);
  cudaError_t(CUDARTAPI *StreamAddCallback)(cudaStream_t stream,
                                            cudaStreamCallback_t callback,
                                            void *userData, unsigned int flags);
  cudaError_t(CUDARTAPI *StreamCreate)(cudaStream_t *stream);
  cudaError_t(CUDARTAPI *StreamCreateWithPriority)(cudaStream_t *stream,
                                                   unsigned int flags,
                                                   int priority);
  cudaError_t(CUDARTAPI *StreamDestroy)(cudaStream_t stream);
  cudaError_t(CUDARTAPI *StreamGetFlags)(cudaStream_t stream,
                                         unsigned int *flags);
  ...
  cudaError_t(CUDARTAPI *StreamWaitEvent)(cudaStream_t stream,
                                          cudaEvent_t event,
                                          unsigned int flags);
};

/**
 * The function table in J9CudaGlobalData must be the same size as
 * J9CudaFunctionTable.
 */
J9CUDA_STATIC_ASSERT(functionTable_size,
                     sizeof(J9CudaFunctionTable) ==
                         sizeof(((J9CudaGlobalData *)0)->functionTable));
 ...
 */
cudaError_t getDeviceName(J9CudaFunctionTable *functions, uint32_t deviceId,
 ...
 */
cudaError_t initDeviceData(J9CudaFunctionTable *functions, uint32_t deviceId,
                           J9CudaDeviceDescriptor *deviceData) {
...

  {
    ...
    result = functions->DeviceGetAttribute(&pciDomainId, cudaDevAttrPciDomainId,
                                           (int)deviceId);
...

  {
    ...
    result = functions->DeviceGetAttribute(&pciBusId, cudaDevAttrPciBusId,
                                           (int)deviceId);
...

  {
    ...
    result = functions->DeviceGetAttribute(&pciDeviceId, cudaDevAttrPciDeviceId,
                                           (int)deviceId);
...

  {
    ...
    result = functions->DeviceGetAttribute(
        &computeMajor, cudaDevAttrComputeCapabilityMajor, (int)deviceId);
...

  {
    ...
    result = functions->DeviceGetAttribute(
        &computeMinor, cudaDevAttrComputeCapabilityMinor, (int)deviceId);
...

  {
    ...
    result = functions->DeviceGetAttribute(&computeMode, cudaDevAttrComputeMode,
                                           (int)deviceId);
  ...
  {
    result = getDeviceName(functions, deviceId,
                           (uint32_t)sizeof(deviceData->deviceName),
 ...
 * @return cudaSuccess on success or a non-zero value on failure
 */
cudaError_t initConfigData(OMRPortLibrary *portLibrary) {
  J9CudaGlobalData *globals = &portLibrary->portGlobals->cudaGlobals;
   ...
   */
  size_t configSize =
      sizeof(J9CudaConfig) + (sizeof(J9CudaDeviceDescriptor) * deviceCount);
  ...
  if (0 != deviceCount) {
    J9CudaDeviceDescriptor *details = (J9CudaDeviceDescriptor *)&config[1];
    J9CudaFunctionTable *functions =
        (J9CudaFunctionTable *)globals->functionTable;

    if (1 == deviceCount) {
      /* With only one device, there's no need to save and restore the active
       * device. */
...
#define J9CUDA_DRIVER_ENTRY_v1(version, name, signature)                       \
  {                                                                            \
    "cu" #name, signature, (uint32_t)offsetof(J9CudaFunctionTable, name),      \
        version                                                                \
  }
#define J9CUDA_DRIVER_ENTRY_v2(version, name, signature)                       \
  {                                                                            \
    "cu" #name "_v2", signature,                                               \
        (uint32_t)offsetof(J9CudaFunctionTable, name), version                 \
  }
...
const J9CudaEntryDescriptor driverDescriptors[] = {
    ...
    /* GetDriverErrorString is so named to avoid clashing with the runtime entry
       GetErrorString. */
...
#define J9CUDA_RUNTIME_ENTRY(version, name, signature)                         \
  {                                                                            \
    "cuda" #name, signature, (uint32_t)offsetof(J9CudaFunctionTable, name),    \
        version                                                                \
  }
 ...
 */
uint32_t openDriver(OMRPortLibrary *portLibrary) {
#if defined(LINUX)
  ...
  if (0 != portLibrary->sl_open_shared_library(
               portLibrary, (char *)driverLibrary, &globals->driverHandle,
               OMRPORT_SLOPEN_LAZY)) {
  ...
  /* initialize the driver */
  {
    ...
    if (0 != portLibrary->sl_lookup_name(portLibrary, globals->driverHandle,
                                         (char *)"cuInit",
  ...
  if (0 != initializeTable(portLibrary, globals->driverHandle,
                           (uint32_t)version, driverDescriptors,
 ...
 */
uint32_t openRuntimeAndGetVersion(OMRPortLibrary *portLibrary,
                                  uint32_t bestVersion, const char *library) {
  ...
  if (0 != portLibrary->sl_open_shared_library(
               portLibrary, (char *)library, &dllHandle, OMRPORT_SLOPEN_LAZY)) {
  ...
  /* query the runtime version */
  {
    ...
    if (0 != portLibrary->sl_lookup_name(
                 portLibrary, dllHandle, (char *)"cudaRuntimeGetVersion",
  ...
  if (0 == initializeTable(portLibrary, dllHandle, (uint32_t)version,
                           runtimeDescriptors, LENGTH_OF(runtimeDescriptors),
    ...
    for (uintptr_t index = 0; index < LENGTH_OF(runtimeDescriptors); ++index) {
      ...
      *(uintptr_t *)(globalTable + offset) =
          *(uintptr_t *)(localTable + offset);
    }
  } else {
  fail:
    if (0 != dllHandle) {
...
#elif defined(OSX)
#define OMRCUDA_LIBRARY_NAME(major, minor)                                     \
  ("libcudart." #major "." #minor ".dylib")
 ...
 */
uint32_t openRuntime(OMRPortLibrary *portLibrary) {
  ...
  for (uint32_t index = 0; index < LENGTH_OF(runtimeLibraries); ++index) {
    ...
    uint32_t version =
        openRuntimeAndGetVersion(portLibrary, bestVersion, library->name);
 ...
 * @param[in] portLibrary the port library pointer
 */
void closeLibraries(OMRPortLibrary *portLibrary) {
  J9CudaGlobalData *globals = &portLibrary->portGlobals->cudaGlobals;
 ...
 * @param[in] portLibrary the port library pointer
 */
void attemptInitialization(OMRPortLibrary *portLibrary) {
  J9CudaGlobalData *globals = &portLibrary->portGlobals->cudaGlobals;
 ...
 */
void resetDevices(J9CudaGlobalData *globals) {
  if ((J9CUDA_STATE_INITIALIZED == globals->state) &&
      (0 != globals->deviceCount)) {
    int current = 0;
    J9CudaFunctionTable *functions =
        (J9CudaFunctionTable *)globals->functionTable;
    cudaError_t result = cudaSuccess;
 ...
 */
VMINLINE J9CudaFunctionTable *getFunctions(OMRPortLibrary *portLibrary) {
 ...
 */
VMINLINE uint32_t getDeviceCount(OMRPortLibrary *portLibrary) {
 ...
 * @param[in] portLibrary the port library pointer
 * @param[in] deviceId the device identifier
 * @return cudaSuccess on success; cudaErrorNoDevice or cudaErrorInvalidDevice
 * on failure
 */
VMINLINE cudaError_t validateDeviceId(OMRPortLibrary *portLibrary,
                                      uint32_t deviceId) {
  uint32_t deviceCount = getDeviceCount(portLibrary);

  return (0 == deviceCount)
             ? cudaErrorNoDevice
             : (deviceId < deviceCount) ? cudaSuccess : cudaErrorInvalidDevice;
}
...
private:
   ...
   */
  static void finalizer(void *handle) {
   ...
   */
  VMINLINE uint32_t *getStateWord(uint32_t deviceId) {
   ...
   * @return the ThreadState pointer for the calling thread
   */
  static ThreadState *getStateForCurrentThread(OMRPortLibrary *portLibrary,
    ...
    J9CudaGlobalData *globals = &portLibrary->portGlobals->cudaGlobals;
    ThreadState *state =
        (ThreadState *)omrthread_tls_get(self, globals->tlsKey);
    ...
    if (NULL == state) {
      ...
      state = (ThreadState *)J9CUDA_ALLOCATE_MEMORY(sizeof(ThreadState) +
                                                    initWordsSize);
   ...
   * @param[in] deviceId the device identifier
   */
  VMINLINE void clear(uint32_t deviceId) {
   ...
   * @param[in] deviceId the device identifier
   */
  VMINLINE void set(uint32_t deviceId) {
   ...
   */
  VMINLINE uint32_t test(uint32_t deviceId) {
...
public:
   ...
   * @return 0 on success, any other value on failure
   */
  static int32_t startup(J9CudaGlobalData *globals) {
   ...
   * @param[in] globals the CUDA global data pointer
   */
  static void shutdown(J9CudaGlobalData *globals) {
   ...
   * @return cudaSuccess on success, any other value on failure
   */
  static cudaError_t initDeviceForCurrentThread(OMRPortLibrary *portLibrary,
                                                J9CudaFunctionTable *functions,
                                                uint32_t deviceId) {
   ...
   * @return cudaSuccess on success, any other value on failure
   */
  static cudaError_t markDeviceForCurrentThread(OMRPortLibrary *portLibrary,
                                                uint32_t deviceId) {
   ...
   * @return cudaSuccess on success, any other value on failure
   */
  static cudaError_t unmarkDeviceForCurrentThread(OMRPortLibrary *portLibrary,
                                                  uint32_t deviceId) {
...
struct InitializerNotNeeded {
   ...
   * @return cudaSuccess on success, any other value on failure
   */
  VMINLINE cudaError_t prepare(OMRPortLibrary *, J9CudaFunctionTable *,
                               uint32_t) const {
   ...
   * @return cudaSuccess on success, any other value on failure
   */
  VMINLINE cudaError_t onSuccess(OMRPortLibrary *, uint32_t) const {
    /* Initialization not needed. */
    return cudaSuccess;
  }
};
...
struct InitializedAfter : public InitializerNotNeeded {
   ...
   * @return cudaSuccess on success, any other value on failure
   */
  VMINLINE cudaError_t onSuccess(OMRPortLibrary *portLibrary,
...
struct InitializedBefore : public InitializerNotNeeded {
   ...
   * @return cudaSuccess on success, any other value on failure
   */
  VMINLINE cudaError_t prepare(OMRPortLibrary *portLibrary,
                               J9CudaFunctionTable *functions,
                               uint32_t deviceId) const {
    return ThreadState::initDeviceForCurrentThread(portLibrary, functions,
                                                   deviceId);
  }
};
...
VMINLINE cudaError_t withDevice(OMRPortLibrary *portLibrary, uint32_t deviceId,
                                Operation &operation) {
 ...
 */
extern "C" int32_t omrcuda_startup(OMRPortLibrary *portLibrary) {
  J9CudaGlobalData *globals = &portLibrary->portGlobals->cudaGlobals;
 ...
 * @param[in] portLibrary the port library pointer
 */
extern "C" void omrcuda_shutdown(OMRPortLibrary *portLibrary) {
...
struct Allocate : public InitializedAfter {
  ...
  explicit VMINLINE Allocate(uintptr_t byteCount)
      : byteCount(byteCount), deviceAddress(NULL) {}

  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) {
 ...
 */
extern "C" int32_t omrcuda_deviceAlloc(OMRPortLibrary *portLibrary,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceCanAccessPeer(OMRPortLibrary *portLibrary,
                                               uint32_t deviceId,
  ...
  if ((cudaSuccess == result) && (NULL != functions->DeviceCanAccessPeer)) {
    ...
    result = functions->DeviceCanAccessPeer(&access, (int)deviceId,
                                            (int)peerDeviceId);
...
struct DisablePeerAccess : public InitializerNotNeeded {
  ...
  explicit VMINLINE DisablePeerAccess(uint32_t peerDeviceId)
      : peerDeviceId(peerDeviceId) {}

  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) const {
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceDisablePeerAccess(OMRPortLibrary *portLibrary,
                                                   uint32_t deviceId,
                                                   uint32_t peerDeviceId) {
...
struct EnablePeerAccess : public InitializerNotNeeded {
  ...
  explicit VMINLINE EnablePeerAccess(uint32_t peerDeviceId)
      : peerDeviceId(peerDeviceId) {}
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceEnablePeerAccess(OMRPortLibrary *portLibrary,
                                                  uint32_t deviceId,
                                                  uint32_t peerDeviceId) {
...
struct Free : public InitializedAfter {
  ...
  explicit VMINLINE Free(void *deviceAddress) : deviceAddress(deviceAddress) {}
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceFree(OMRPortLibrary *portLibrary,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceGetAttribute(OMRPortLibrary *portLibrary,
                                              uint32_t deviceId,
  ...
  if ((cudaSuccess == result) && (NULL != functions->DeviceGetAttribute)) {
    ...
    result =
        functions->DeviceGetAttribute(&value, deviceAttribute, (int)deviceId);
    ...
    if (J9CUDA_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY == attribute) {
      ...
      result = functions->DeviceGetAttribute(
          &minor, cudaDevAttrComputeCapabilityMinor, (int)deviceId);
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceGetCacheConfig(OMRPortLibrary *portLibrary,
                                                uint32_t deviceId,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceGetCount(OMRPortLibrary *portLibrary,
                                          uint32_t *countOut) {
...
struct DeviceGetLimit : public InitializedAfter {
  ...
  explicit VMINLINE DeviceGetLimit(J9CudaDeviceLimit limit)
      : limit(limit), value(0) {}
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceGetLimit(OMRPortLibrary *portLibrary,
                                          uint32_t deviceId,
 ...
 * @param[in] portLibrary the port library pointer
 * @param[in] deviceId the device identifier
 * @param[out] freeOut where the number of bytes of free device memory should be
 * stored
 * @param[out] totalOut where the total number of bytes of device memory should
 * be stored
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceGetMemInfo(OMRPortLibrary *portLibrary,
                                            uint32_t deviceId,
                                            uintptr_t *freeOut,
                                            uintptr_t *totalOut) {
  ...
  if (cudaSuccess == result) {
    Trc_PRT_cuda_deviceGetMemInfo_result(operation.freeBytes,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceGetName(OMRPortLibrary *portLibrary,
 ...
 */
extern "C" int32_t
omrcuda_deviceGetSharedMemConfig(OMRPortLibrary *portLibrary, uint32_t deviceId,
...
struct DeviceGetPriorities : public InitializedAfter {
  ...
  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) {
    return functions->DeviceGetStreamPriorityRange(&leastPriority,
                                                   &greatestPriority);
  }
};
...
/**
 * Query the limits on stream priorities of a device. Note that stream
 * priorities follow a convention where lower numbers imply greater priorities,
 * thus upon successful return, (*greatestPriorityOut <= *leastPriorityOut).
 *
 * @param[in] portLibrary the port library pointer
 * @param[in] deviceId the device identifier
 * @param[out] leastPriorityOut where the least possible priority should be
 * stored
 * @param[out] greatestPriorityOut where the greatest possible priority should
 * be stored
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceGetStreamPriorityRange(
    OMRPortLibrary *portLibrary, uint32_t deviceId, int32_t *leastPriorityOut,
  ...
  if (cudaSuccess == result) {
    Trc_PRT_cuda_deviceGetStreamPriorityRange_result(
        operation.leastPriority, operation.greatestPriority);
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceReset(OMRPortLibrary *portLibrary,
                                       uint32_t deviceId) {
...
struct DeviceSetCacheConfig : public InitializedAfter {
  ...
  explicit VMINLINE DeviceSetCacheConfig(J9CudaCacheConfig config)
      : config(config) {}
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceSetCacheConfig(OMRPortLibrary *portLibrary,
                                                uint32_t deviceId,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceSetLimit(OMRPortLibrary *portLibrary,
                                          uint32_t deviceId,
                                          J9CudaDeviceLimit limit,
                                          uintptr_t value) {
...
struct DeviceSetSharedMemConfig : public InitializedAfter {
  ...
  explicit VMINLINE DeviceSetSharedMemConfig(J9CudaSharedMemConfig config)
      : config(config) {}
 ...
 */
extern "C" int32_t
omrcuda_deviceSetSharedMemConfig(OMRPortLibrary *portLibrary, uint32_t deviceId,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_deviceSynchronize(OMRPortLibrary *portLibrary,
                                             uint32_t deviceId) {
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_driverGetVersion(OMRPortLibrary *portLibrary,
                                            uint32_t *versionOut) {
...
struct EventCreate : public InitializedAfter {
  ...
  explicit VMINLINE EventCreate(uint32_t flags) : event(NULL), flags(flags) {}
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_eventCreate(OMRPortLibrary *portLibrary,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_eventDestroy(OMRPortLibrary *portLibrary,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_eventElapsedTime(OMRPortLibrary *portLibrary,
                                            J9CudaEvent startEvent,
                                            J9CudaEvent endEvent,
  ...
  if (NULL != functions->EventElapsedTime) {
    result = functions->EventElapsedTime(
        elapsedMillisOut, (cudaEvent_t)startEvent, (cudaEvent_t)endEvent);
 ...
 * @param[in] portLibrary the port library pointer
 * @param[in] event the event handle
 * @return 0 if the event has been recorded; else J9CUDA_ERROR_NOT_READY or
 * another error
 */
extern "C" int32_t omrcuda_eventQuery(OMRPortLibrary *portLibrary,
                                      J9CudaEvent event) {
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_eventRecord(OMRPortLibrary *portLibrary,
                                       uint32_t deviceId, J9CudaEvent event,
                                       J9CudaStream stream) {
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_eventSynchronize(OMRPortLibrary *portLibrary,
                                            J9CudaEvent event) {
...
struct FunctionGetAttribute : public InitializedBefore {
  ...
  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) {
    ...
    return translate(
        functions->FuncGetAttribute(&value, functionAttribute, function));
  }
};
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_funcGetAttribute(OMRPortLibrary *portLibrary,
                                            uint32_t deviceId,
 ...
 * @param[in] dynamicSharedMemorySize the intended dynamic shared memory usage
 * in bytes
 * @param[in] flags the query flags (see J9CudaOccupancyFlags)
 * @return numBlocks the maximum number of active thread blocks per streaming
 * multiprocessor
 */
struct FunctionMaxActiveBlocks : public InitializedBefore {
  ...
  VMINLINE
  FunctionMaxActiveBlocks(CUfunction function, uint32_t blockSize,
                          uint32_t dynamicSharedMemorySize, uint32_t flags)
      : blockSize(blockSize), dynamicSharedMemorySize(dynamicSharedMemorySize),
        flags(flags), function(function), numBlocks(0) {}

  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) {
    return translate(
        functions->OccupancyMaxActiveBlocksPerMultiprocessorWithFlags(
            &numBlocks, function, (int)blockSize,
            (size_t)dynamicSharedMemorySize, (int)flags));
  }
};
...
/**
 * Query the maximum number of active thread blocks per streaming multiprocessor
 * for a kernel function. The set of flags is defined by the enum
 * J9CudaOccupancyFlags, declared in omrcuda.h.
 *
 * @param[in] portLibrary the port library pointer
 * @param[in] deviceId the device identifier
 * @param[in] function the function handle
 ...
 * @param[out] valueOut where the maximum number of active thread blocks per
 * streaming multiprocessor should be stored
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_funcMaxActiveBlocksPerMultiprocessor(
    OMRPortLibrary *portLibrary, uint32_t deviceId, J9CudaFunction function,
  ...
  Trc_PRT_cuda_funcMaxActiveBlocksPerMultiprocessor_entry(
      deviceId, function, blockSize, dynamicSharedMemorySize, flags);
  ...
  if (cudaSuccess == result) {
    if (NULL == functions->OccupancyMaxActiveBlocksPerMultiprocessorWithFlags) {
      ...
      FunctionMaxActiveBlocks operation((CUfunction)function, blockSize,
                                        dynamicSharedMemorySize, flags);

      result = withDevice(portLibrary, deviceId, operation);

      if (cudaSuccess == result) {
        Trc_PRT_cuda_funcMaxActiveBlocksPerMultiprocessor_result(
            (uint32_t)operation.numBlocks);
...
/**
 * The closure required to query reasonable block and grid sizes to achieve
 * maximal occupancy of a kernel function in conjunction with use of the
 * template function 'withDevice'.
 *
 * @param[in] function the function pointer
 * @param[in] dynamicSharedMemoryFunction a function which gives the dynamic
 * shared memory required for a particular block size
 * @param[in] userData the user data to be provided in calls to
 * dynamicSharedMemoryFunction
 * @param[in] blockSizeLimit the maximum block size to consider;
 * 0 is equivalent to specifying the maximum block size permitted by the device
 * / function
 * @param[in] flags the query flags
 * @param[in] maxThreadsPerMultiProcessor the maximum number of threads per
 * multiprocessor
 ...
 * @return bestBlockCountPerMultiProcessor the minimum number of blocks per
 * multiprocessor that achieves maximal occupancy
 */
struct FunctionPotentialBlockSize : public InitializedBefore {
  ...
  FunctionPotentialBlockSize(
      ...
      : bestBlockCountPerMultiProcessor(0), bestBlockSize(0),
        blockSizeLimit(blockSizeLimit), callback(dynamicSharedMemoryFunction),
        flags(flags), function(function),
        maxThreadsPerMultiProcessor(maxThreadsPerMultiProcessor),
        userData(userData), warpSize(warpSize) {}
  ...
  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) {
    ...
    for (uint32_t blockSize = blockSizeLimit; blockSize != 0;) {
      ...
      result = functions->OccupancyMaxActiveBlocksPerMultiprocessorWithFlags(
          &blocksPerMultiProcessor, function, (int)blockSize,
          (size_t)dynamicSharedMemorySize, (int)flags);
...
/**
 * Query reasonable block and grid sizes to achieve maximal occupancy of a
 * kernel function.
 *
 * @param[in] portLibrary the port library pointer
 * @param[in] deviceId the device identifier
 * @param[in] function the function handle
 * @param[in] dynamicSharedMemoryFunction a function which gives the dynamic
 * shared memory required for a particular block size; NULL may be used if the
 * requirements don't depend upon the block size, in which case userData will be
 * used to define the dynamic shared for all block sizes
 * @param[in] userData the user data to be provided in calls to
 * dynamicSharedMemoryFunction, or the fixed dynamic shared memory requirement
 * @param[in] blockSizeLimit the maximum block size that should be considered;
 * 0 is equivalent to specifying the maximum block size permitted by the device
 * / function
 * @param[in] flags the query flags
 * @param[out] minGridSizeOut where the minimum computed grid size should be
 * stored
 * @param[out] maxBlockSizeOut where the maximum computed block size should be
 * stored
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_funcMaxPotentialBlockSize(
    OMRPortLibrary *portLibrary, uint32_t deviceId, J9CudaFunction function,
    J9CudaBlockToDynamicSharedMemorySize dynamicSharedMemoryFunction,
    uintptr_t userData, uint32_t blockSizeLimit, uint32_t flags,
    uint32_t *minGridSizeOut, uint32_t *maxBlockSizeOut) {
  Trc_PRT_cuda_funcMaxPotentialBlockSize_entry(deviceId, function,
                                               dynamicSharedMemoryFunction,
                                               userData, blockSizeLimit, flags);
...
#define GET_DEVICE_ATTRIBUTE(attribute)                                        \
  do {                                                                         \
    int value = 0;                                                             \
    result = functions->DeviceGetAttribute(&value, (cudaDevAttr##attribute),   \
                                           (int)deviceId);                     \
    if (cudaSuccess != result) {                                               \
      goto done;                                                               \
    }                                                                          \
...
#undef GET_DEVICE_ATTRIBUTE
    ...
    CUresult error = functions->FuncGetAttribute(
        &value, CU_FUNC_ATTRIBUTE_MAX_THREADS_PER_BLOCK, (CUfunction)function);
    ...
    FunctionPotentialBlockSize operation(
        (CUfunction)function, dynamicSharedMemoryFunction, userData,
    ...
    if (cudaSuccess == result) {
      *minGridSizeOut = (uint32_t)operation.bestBlockCountPerMultiProcessor *
                        deviceMultiProcessorCount;
      *maxBlockSizeOut = (uint32_t)operation.bestBlockSize;

      Trc_PRT_cuda_funcMaxPotentialBlockSize_result(*minGridSizeOut,
                                                    *maxBlockSizeOut);
    }
  }
...
struct FunctionSetCacheConfig : public InitializedBefore {
  ...
  FunctionSetCacheConfig(CUfunction function, J9CudaCacheConfig config)
      : config(config), function(function) {}
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_funcSetCacheConfig(OMRPortLibrary *portLibrary,
                                              uint32_t deviceId,
                                              J9CudaFunction function,
                                              J9CudaCacheConfig config) {
 ...
 */
extern "C" int32_t
omrcuda_funcSetSharedMemConfig(OMRPortLibrary *portLibrary, uint32_t deviceId,
                               J9CudaFunction function,
                               J9CudaSharedMemConfig config) {
 ...
 */
extern "C" const char *omrcuda_getErrorString(OMRPortLibrary *portLibrary,
                                              int32_t error) {
  ...
  if (error >= 0) {
    ...
    if ((NULL == functions->GetDriverErrorString) ||
        (CUDA_SUCCESS !=
         functions->GetDriverErrorString((CUresult)-error, &result))) {
      switch (-error) {
...
/**
 * Allocate host storage to be used for transfers between host and device
 * memory. The value of the flags parameter must be produced by ORing together
 * one or more of the literals defined by the enum J9CudaHostAllocFlags,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_hostAlloc(OMRPortLibrary *portLibrary,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_hostFree(OMRPortLibrary *portLibrary,
                                    void *hostAddress) {
...
struct Launch : public InitializedBefore {
  ...
  VMINLINE
  Launch(CUfunction function, const dim3 &gridDim, const dim3 &blockDim,
         uint32_t sharedMem, CUstream stream, void **args)
      : function(function), gridDim(gridDim), blockDim(blockDim),
        sharedMem(sharedMem), stream(stream), args(args) {}

  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) const {
    return translate(functions->LaunchKernel(
 ...
 * @param[in] sharedMemBytes the number of bytes of dynamic shared memory
 * required
 * @param[in] stream the stream handle, or NULL for the default stream
 * @param[in] kernelParms the kernel arguments
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_launchKernel(
    OMRPortLibrary *portLibrary, uint32_t deviceId, J9CudaFunction function,
    ...
    J9CudaStream stream, void **kernelParms) {
  Trc_PRT_cuda_launchKernel_entry(deviceId, function, gridDimX, gridDimY,
  ...
  const Launch operation((CUfunction)function, gridDim, blockDim,
                         sharedMemBytes, (CUstream)stream, kernelParms);
...
public:
  ...
  VMINLINE uint32_t getCount() const { return count; }

  VMINLINE CUjit_option *getKeys() { return keys; }

  VMINLINE void **getValues() { return values; }
};
 ...
 */
cudaError_t JitOptions::set(J9CudaJitOptions *j9options) {
...
struct LinkerAdd : public InitializedBefore {
  ...
  VMINLINE
  LinkerAdd(J9CudaLinker linker, J9CudaJitInputType type, void *image,
            size_t imageSize, const char *name, J9CudaJitOptions *options)
      : image(image), imageSize(imageSize), linker(linker), name(name),
  ...
  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) const {
    ...
    if (cudaSuccess == result) {
      ...
      result = translate(functions->LinkAddData(
          linker->state, inputType, image, imageSize, name,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_linkerAddData(OMRPortLibrary *portLibrary,
  ...
  Trc_PRT_cuda_linkerAddData_entry(deviceId, linker, type, data, size, name,
                                   options);
...
struct LinkerComplete : public InitializedBefore {
  ...
  explicit VMINLINE LinkerComplete(J9CudaLinker linker)
 ...
 * @param[out] sizeOut where the size (in bytes) of the module image should be
 * stored
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_linkerComplete(OMRPortLibrary *portLibrary,
                                          uint32_t deviceId,
                                          J9CudaLinker linker, void **cubinOut,
                                          uintptr_t *sizeOut) {
...
struct LinkerCreate : public InitializedBefore {
  ...
  explicit VMINLINE LinkerCreate(J9CudaLinkerState *linker) : linker(linker) {}

  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) const {
    return translate(functions->LinkCreate(
        linker->createOptions.getCount(), linker->createOptions.getKeys(),
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_linkerCreate(OMRPortLibrary *portLibrary,
                                        uint32_t deviceId,
                                        J9CudaJitOptions *options,
  ...
  J9CudaLinker linker =
      (J9CudaLinker)J9CUDA_ALLOCATE_MEMORY(sizeof(J9CudaLinkerState));
...
struct LinkerDestroy : public InitializedBefore {
  ...
  explicit VMINLINE LinkerDestroy(J9CudaLinker linker) : linker(linker) {}
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_linkerDestroy(OMRPortLibrary *portLibrary,
                                         uint32_t deviceId,
                                         J9CudaLinker linker) {
...
struct Copy2D : public InitializedAfter {
  ...
  VMINLINE
  Copy2D(void *targetAddress, size_t targetPitch, const void *sourceAddress,
         size_t sourcePitch, size_t width, size_t height,
         cudaMemcpyKind direction)
      : direction(direction), height(height), sourceAddress(sourceAddress),
        sourcePitch(sourcePitch), stream(NULL), sync(true),
        targetAddress(targetAddress), targetPitch(targetPitch), width(width) {}

  VMINLINE
  Copy2D(void *targetAddress, size_t targetPitch, const void *sourceAddress,
         size_t sourcePitch, size_t width, size_t height,
         cudaMemcpyKind direction, J9CudaStream stream)
      : direction(direction), height(height), sourceAddress(sourceAddress),
        sourcePitch(sourcePitch), stream(stream), sync(false),
        targetAddress(targetAddress), targetPitch(targetPitch), width(width) {}

  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) const {
    if (sync) {
      return functions->Memcpy2D(targetAddress, targetPitch, sourceAddress,
                                 sourcePitch, width, height, direction);
    } else {
      return functions->Memcpy2DAsync(targetAddress, targetPitch, sourceAddress,
                                      sourcePitch, width, height, direction,
                                      (cudaStream_t)stream);
    }
  }
 ...
 */
extern "C" int32_t
omrcuda_memcpy2DDeviceToDevice(OMRPortLibrary *portLibrary, uint32_t deviceId,
                               void *targetAddress, size_t targetPitch,
                               const void *sourceAddress, size_t sourcePitch,
                               uintptr_t width, uintptr_t height) {
  Trc_PRT_cuda_memcpy2D_entry(deviceId, targetAddress, targetPitch,
                              sourceAddress, sourcePitch, width, height,
                              cudaMemcpyDeviceToDevice);

  const Copy2D operation(targetAddress, targetPitch, sourceAddress, sourcePitch,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpy2DDeviceToDeviceAsync(
    OMRPortLibrary *portLibrary, uint32_t deviceId, void *targetAddress,
    size_t targetPitch, const void *sourceAddress, size_t sourcePitch,
    uintptr_t width, uintptr_t height, J9CudaStream stream) {
  Trc_PRT_cuda_memcpy2DAsync_entry(deviceId, targetAddress, targetPitch,
                                   sourceAddress, sourcePitch, width, height,
 ...
 */
extern "C" int32_t
omrcuda_memcpy2DDeviceToHost(OMRPortLibrary *portLibrary, uint32_t deviceId,
                             void *targetAddress, size_t targetPitch,
                             const void *sourceAddress, size_t sourcePitch,
                             uintptr_t width, uintptr_t height) {
  Trc_PRT_cuda_memcpy2D_entry(deviceId, targetAddress, targetPitch,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpy2DDeviceToHostAsync(
    OMRPortLibrary *portLibrary, uint32_t deviceId, void *targetAddress,
    size_t targetPitch, const void *sourceAddress, size_t sourcePitch,
    uintptr_t width, uintptr_t height, J9CudaStream stream) {
  Trc_PRT_cuda_memcpy2DAsync_entry(deviceId, targetAddress, targetPitch,
 ...
 */
extern "C" int32_t
omrcuda_memcpy2DHostToDevice(OMRPortLibrary *portLibrary, uint32_t deviceId,
                             void *targetAddress, size_t targetPitch,
                             const void *sourceAddress, size_t sourcePitch,
                             uintptr_t width, uintptr_t height) {
  Trc_PRT_cuda_memcpy2D_entry(deviceId, targetAddress, targetPitch,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpy2DHostToDeviceAsync(
    OMRPortLibrary *portLibrary, uint32_t deviceId, void *targetAddress,
    size_t targetPitch, const void *sourceAddress, size_t sourcePitch,
    uintptr_t width, uintptr_t height, J9CudaStream stream) {
  Trc_PRT_cuda_memcpy2DAsync_entry(deviceId, targetAddress, targetPitch,
...
struct Copy : public InitializedAfter {
  ...
  VMINLINE
  Copy(void *targetAddress, void const *sourceAddress, uintptr_t byteCount,
       cudaMemcpyKind direction)
      : byteCount(byteCount), direction(direction),
        sourceAddress(sourceAddress), stream(NULL), sync(true),
  ...
  Copy(void *targetAddress, void const *sourceAddress, uintptr_t byteCount,
       cudaMemcpyKind direction, J9CudaStream stream)
      : byteCount(byteCount), direction(direction),
        sourceAddress(sourceAddress), stream(stream), sync(false),
        targetAddress(targetAddress) {}

  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) const {
    if (sync) {
      return functions->Memcpy(targetAddress, sourceAddress, byteCount,
                               direction);
    } else {
      return functions->MemcpyAsync(targetAddress, sourceAddress, byteCount,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpyDeviceToDevice(OMRPortLibrary *portLibrary,
                                                uint32_t deviceId,
                                                void *targetAddress,
                                                const void *sourceAddress,
                                                uintptr_t byteCount) {
  Trc_PRT_cuda_memcpy_entry(deviceId, targetAddress, sourceAddress, byteCount,
                            cudaMemcpyDeviceToDevice);

  const Copy operation(targetAddress, sourceAddress, byteCount,
                       cudaMemcpyDeviceToDevice);
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpyDeviceToDeviceAsync(
    OMRPortLibrary *portLibrary, uint32_t deviceId, void *targetAddress,
    const void *sourceAddress, uintptr_t byteCount, J9CudaStream stream) {
  Trc_PRT_cuda_memcpyAsync_entry(deviceId, targetAddress, sourceAddress,
                                 byteCount, cudaMemcpyDeviceToDevice, stream);
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpyDeviceToHost(OMRPortLibrary *portLibrary,
                                              uint32_t deviceId,
                                              void *targetAddress,
                                              const void *sourceAddress,
                                              uintptr_t byteCount) {
 ...
 */
extern "C" int32_t
omrcuda_memcpyDeviceToHostAsync(OMRPortLibrary *portLibrary, uint32_t deviceId,
                                void *targetAddress, const void *sourceAddress,
                                uintptr_t byteCount, J9CudaStream stream) {
  Trc_PRT_cuda_memcpyAsync_entry(deviceId, targetAddress, sourceAddress,
                                 byteCount, cudaMemcpyDeviceToHost, stream);
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpyHostToDevice(OMRPortLibrary *portLibrary,
                                              uint32_t deviceId,
                                              void *targetAddress,
                                              const void *sourceAddress,
                                              uintptr_t byteCount) {
 ...
 */
extern "C" int32_t
omrcuda_memcpyHostToDeviceAsync(OMRPortLibrary *portLibrary, uint32_t deviceId,
                                void *targetAddress, const void *sourceAddress,
                                uintptr_t byteCount, J9CudaStream stream) {
  Trc_PRT_cuda_memcpyAsync_entry(deviceId, targetAddress, sourceAddress,
                                 byteCount, cudaMemcpyHostToDevice, stream);
 ...
 */
extern "C" int32_t
omrcuda_memcpyPeer(OMRPortLibrary *portLibrary, uint32_t targetDeviceId,
                   void *targetAddress, uint32_t sourceDeviceId,
                   const void *sourceAddress, uintptr_t byteCount) {
  Trc_PRT_cuda_memcpyPeer_entry(targetDeviceId, targetAddress, sourceDeviceId,
  ...
  if ((cudaSuccess == result) && (NULL != functions->MemcpyPeer)) {
    result = functions->MemcpyPeer(targetAddress, targetDeviceId, sourceAddress,
                                   sourceDeviceId, byteCount);
  }
 ...
 */
extern "C" int32_t
omrcuda_memcpyPeerAsync(OMRPortLibrary *portLibrary, uint32_t targetDeviceId,
                        void *targetAddress, uint32_t sourceDeviceId,
                        const void *sourceAddress, uintptr_t byteCount,
                        J9CudaStream stream) {
  Trc_PRT_cuda_memcpyPeerAsync_entry(targetDeviceId, targetAddress,
  ...
  if ((cudaSuccess == result) && (NULL != functions->MemcpyPeerAsync)) {
    result = functions->MemcpyPeerAsync(targetAddress, targetDeviceId,
                                        sourceAddress, sourceDeviceId,
                                        byteCount, (cudaStream_t)stream);
  }
...
struct Fill8 : InitializedBefore {
  ...
  VMINLINE
  Fill8(void *deviceAddress, uint8_t value, uintptr_t count,
        J9CudaStream stream)
      : count(count), deviceAddress(deviceAddress), stream(stream),
        value(value) {}

  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) const {
    return translate(functions->MemsetD8Async((CUdeviceptr)deviceAddress, value,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memset8Async(OMRPortLibrary *portLibrary,
                                        uint32_t deviceId, void *deviceAddress,
...
struct Fill16 : public InitializedBefore {
  ...
  VMINLINE
  Fill16(void *deviceAddress, uint16_t value, uintptr_t count,
         J9CudaStream stream)
      : count(count), deviceAddress(deviceAddress), stream(stream),
        value(value) {}

  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) const {
    return translate(functions->MemsetD16Async((CUdeviceptr)deviceAddress,
                                               value, count, (CUstream)stream));
  }
};
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memset16Async(OMRPortLibrary *portLibrary,
                                         uint32_t deviceId, void *deviceAddress,
...
struct Fill32 : public InitializedBefore {
  ...
  VMINLINE
  Fill32(void *deviceAddress, uint32_t value, uintptr_t count,
         J9CudaStream stream)
      : count(count), deviceAddress(deviceAddress), stream(stream),
        value(value) {}

  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) const {
    return translate(functions->MemsetD32Async((CUdeviceptr)deviceAddress,
                                               value, count, (CUstream)stream));
  }
};
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memset32Async(OMRPortLibrary *portLibrary,
                                         uint32_t deviceId, void *deviceAddress,
...
struct GetFunction : public InitializedBefore {
  ...
  GetFunction(CUmodule module, const char *name)
      : function(NULL), module(module), name(name) {}
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_moduleGetFunction(OMRPortLibrary *portLibrary,
                                             uint32_t deviceId,
                                             J9CudaModule module,
                                             const char *name,
...
struct GetGlobal : public InitializedBefore {
  ...
  GetGlobal(CUmodule module, const char *name)
      : module(module), name(name), size(0), symbol(0) {}
 ...
 * @param[out] addressOut where the device address of the symbol should be
 * stored
 * @param[out] sizeOut where the size in bytes of the symbol should be stored
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t
omrcuda_moduleGetGlobal(OMRPortLibrary *portLibrary, uint32_t deviceId,
                        J9CudaModule module, const char *name,
  ...
  if (cudaSuccess == result) {
    Trc_PRT_cuda_moduleGetGlobal_result((void *)operation.symbol,
                                        operation.size);
...
struct GetSurface : public InitializedBefore {
  ...
  GetSurface(CUmodule module, const char *name)
      : module(module), name(name), surface(NULL) {}
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_moduleGetSurfaceRef(OMRPortLibrary *portLibrary,
                                               uint32_t deviceId,
                                               J9CudaModule module,
                                               const char *name,
...
struct GetTexture : public InitializedBefore {
  ...
  GetTexture(CUmodule module, const char *name)
      : module(module), name(name), texture(NULL) {}
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_moduleGetTextureRef(OMRPortLibrary *portLibrary,
                                               uint32_t deviceId,
                                               J9CudaModule module,
                                               const char *name,
                                               uintptr_t *texRefOut) {
...
struct Load : public InitializedBefore {
  ...
  Load(const void *image, J9CudaJitOptions *options)
      : image(image), module(NULL), options(options) {}
  ...
  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) {
    ...
    if (cudaSuccess == result) {
      result = translate(functions->ModuleLoadDataEx(
          &module, image, loadOptions.getCount(), loadOptions.getKeys(),
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_moduleLoad(OMRPortLibrary *portLibrary,
...
struct Unload : public InitializedBefore {
  ...
  explicit VMINLINE Unload(CUmodule module) : module(module) {}
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_moduleUnload(OMRPortLibrary *portLibrary,
                                        uint32_t deviceId,
                                        J9CudaModule module) {
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_runtimeGetVersion(OMRPortLibrary *portLibrary,
                                             uint32_t *versionOut) {
...
struct StreamCallback {
  ...
  static void CUDART_CB handler(cudaStream_t stream, cudaError_t error,
                                void *data) {
...
struct StreamAddCallback : public InitializedAfter {
  ...
  StreamAddCallback(cudaStream_t stream, StreamCallback *callback)
      : callback(callback), stream(stream) {}

  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) const {
    return functions->StreamAddCallback((cudaStream_t)stream,
                                        &StreamCallback::handler, callback, 0);
  }
};
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_streamAddCallback(
    OMRPortLibrary *portLibrary, uint32_t deviceId, J9CudaStream stream,
    J9CudaStreamCallback clientFunction, uintptr_t clientData) {
  Trc_PRT_cuda_streamAddCallback_entry(deviceId, stream, (void *)clientFunction,
                                       clientData);
  ...
  StreamCallback *callback =
      (StreamCallback *)J9CUDA_ALLOCATE_MEMORY(sizeof(StreamCallback));
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_streamCreate(OMRPortLibrary *portLibrary,
                                        uint32_t deviceId,
...
struct StreamCreateEx : public InitializedAfter {
  ...
  StreamCreateEx(uint32_t flags, int32_t priority)
      : flags(flags), priority(priority), stream(NULL) {}
  ...
  VMINLINE cudaError_t execute(J9CudaFunctionTable *functions) {
    ...
    if (OMR_ARE_NO_BITS_SET(flags, ~allFlags)) {
      ...
      error =
          functions->StreamCreateWithPriority(&stream, streamFlags, priority);
    }
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_streamCreateWithPriority(OMRPortLibrary *portLibrary,
                                                    uint32_t deviceId,
                                                    int32_t priority,
                                                    uint32_t flags,
                                                    J9CudaStream *streamOut) {
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_streamDestroy(OMRPortLibrary *portLibrary,
                                         uint32_t deviceId,
                                         J9CudaStream stream) {
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_streamGetFlags(OMRPortLibrary *portLibrary,
                                          uint32_t deviceId,
                                          J9CudaStream stream,
                                          uint32_t *flagsOut) {
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_streamGetPriority(OMRPortLibrary *portLibrary,
                                             uint32_t deviceId,
                                             J9CudaStream stream,
                                             int32_t *priorityOut) {
 ...
 * @param[in] deviceId the device identifier
 * @param[in] stream the stream handle
 * @return 0 if the stream is complete; else J9CUDA_ERROR_NOT_READY or another
 * error
 */
extern "C" int32_t omrcuda_streamQuery(OMRPortLibrary *portLibrary,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_streamSynchronize(OMRPortLibrary *portLibrary,
                                             uint32_t deviceId,
                                             J9CudaStream stream) {
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_streamWaitEvent(OMRPortLibrary *portLibrary,
                                           uint32_t deviceId,
                                           J9CudaStream stream,
                                           J9CudaEvent event) {
  ...
  if ((cudaSuccess == result) && (NULL != functions->StreamWaitEvent)) {
    result =
        functions->StreamWaitEvent((cudaStream_t)stream, (cudaEvent_t)event, 0);
