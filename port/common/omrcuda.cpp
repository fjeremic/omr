 ...
 */
int32_t initializeTable(OMRPortLibrary* portLibrary,
                        uintptr_t dllHandle,
 ...
 */
int32_t getDeviceData(OMRPortLibrary* portLibrary,
                      uint32_t deviceId,
...
struct J9CudaFunctionTable {
  ...
  CUresult(CUDAAPI* DeviceGetName)(char* nameOut,
                                   int nameSize,
                                   CUdevice device);
  ...
  CUresult(CUDAAPI* LaunchKernel)(CUfunction function,
                                  ...
                                  void** kernelParms,
                                  void** extra);
  CUresult(CUDAAPI* LinkAddData)(CUlinkState state,
                                 CUjitInputType type,
                                 void* data,
                                 size_t size,
                                 const char* name,
                                 ...
                                 void** optionValues);
  CUresult(CUDAAPI* LinkComplete)(CUlinkState state,
                                  void** cubinOut,
                                  size_t* sizeOut);
  CUresult(CUDAAPI* LinkCreate)(unsigned int numOptions,
                                CUjit_option* options,
                                void** optionValues,
                                CUlinkState* stateOut);
  CUresult(CUDAAPI* LinkDestroy)(CUlinkState state);
  CUresult(CUDAAPI* MemsetD8Async)(CUdeviceptr dstDevice,
                                   unsigned char value,
                                   size_t count,
                                   CUstream stream);
  CUresult(CUDAAPI* MemsetD16Async)(CUdeviceptr dstDevice,
                                    unsigned short value,
                                    size_t count,
                                    CUstream stream);
  CUresult(CUDAAPI* MemsetD32Async)(CUdeviceptr dstDevice,
                                    unsigned int value,
                                    size_t count,
                                    CUstream stream);
  CUresult(CUDAAPI* ModuleGetFunction)(CUfunction* functionOut,
                                       CUmodule module,
                                       const char* name);
  CUresult(CUDAAPI* ModuleGetGlobal)(CUdeviceptr* addressOut,
                                     size_t* sizeOut,
                                     CUmodule module,
                                     const char* name);
  CUresult(CUDAAPI* ModuleGetSurfRef)(CUsurfref* surfRefOut,
                                      CUmodule module,
                                      const char* name);
  CUresult(CUDAAPI* ModuleGetTexRef)(CUtexref* texRefOut,
                                     CUmodule module,
                                     const char* name);
  CUresult(CUDAAPI* ModuleLoadDataEx)(CUmodule* module,
                                      const void* image,
                                      unsigned int numOptions,
                                      CUjit_option* options,
                                      void** optionValues);
  ...
  cudaError_t(CUDARTAPI* HostAlloc)(void** pHost,
                                    size_t size,
                                    unsigned int flags);
  cudaError_t(CUDARTAPI* Malloc)(void** deviceAddressOut, size_t size);
  cudaError_t(CUDARTAPI* Memcpy)(void* dst,
                                 const void* src,
                                 size_t count,
                                 cudaMemcpyKind kind);
  cudaError_t(CUDARTAPI* MemcpyAsync)(void* dst,
                                      const void* src,
                                      size_t count,
                                      cudaMemcpyKind kind,
                                      cudaStream_t stream);
  cudaError_t(CUDARTAPI* Memcpy2D)(void* dst,
                                   size_t dpitch,
                                   const void* src,
                                   size_t spitch,
                                   size_t width,
                                   size_t height,
                                   cudaMemcpyKind kind);
  cudaError_t(CUDARTAPI* Memcpy2DAsync)(void* dst,
                                        size_t dpitch,
                                        const void* src,
                                        size_t spitch,
                                        size_t width,
                                        ...
                                        cudaMemcpyKind kind,
                                        cudaStream_t stream);
  cudaError_t(CUDARTAPI* MemcpyPeer)(void* dst,
                                     int dstDevice,
                                     const void* src,
                                     int srcDevice,
                                     size_t count);
  cudaError_t(CUDARTAPI* MemcpyPeerAsync)(void* dst,
                                          int dstDevice,
                                          const void* src,
                                          int srcDevice,
                                          size_t count,
 ...
 */
cudaError_t getDeviceName(J9CudaFunctionTable* functions,
                          uint32_t deviceId,
                          uint32_t nameSize,
                          char* nameOut) {
 ...
 */
cudaError_t initDeviceData(J9CudaFunctionTable* functions,
                           uint32_t deviceId,
                           J9CudaDeviceDescriptor* deviceData) {
...
struct InitializerNotNeeded {
   ...
   * @return cudaSuccess on success, any other value on failure
   */
  VMINLINE cudaError_t prepare(OMRPortLibrary*,
                               J9CudaFunctionTable*,
                               uint32_t) const {
...
VMINLINE cudaError_t withDevice(OMRPortLibrary* portLibrary,
                                uint32_t deviceId,
                                Operation& operation) {
...
struct FunctionMaxActiveBlocks : public InitializedBefore {
  ...
  VMINLINE
  FunctionMaxActiveBlocks(CUfunction function,
                          uint32_t blockSize,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_launchKernel(OMRPortLibrary* portLibrary,
                                        uint32_t deviceId,
                                        J9CudaFunction function,
                                        uint32_t gridDimX,
                                        uint32_t gridDimY,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpy2DDeviceToDevice(OMRPortLibrary* portLibrary,
                                                  uint32_t deviceId,
                                                  void* targetAddress,
                                                  size_t targetPitch,
                                                  const void* sourceAddress,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpy2DDeviceToHost(OMRPortLibrary* portLibrary,
                                                uint32_t deviceId,
                                                void* targetAddress,
                                                size_t targetPitch,
                                                const void* sourceAddress,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpy2DHostToDevice(OMRPortLibrary* portLibrary,
                                                uint32_t deviceId,
                                                void* targetAddress,
                                                size_t targetPitch,
                                                const void* sourceAddress,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpyDeviceToHostAsync(OMRPortLibrary* portLibrary,
                                                   uint32_t deviceId,
                                                   void* targetAddress,
                                                   const void* sourceAddress,
                                                   uintptr_t byteCount,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpyHostToDeviceAsync(OMRPortLibrary* portLibrary,
                                                   uint32_t deviceId,
                                                   void* targetAddress,
                                                   const void* sourceAddress,
                                                   uintptr_t byteCount,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpyPeer(OMRPortLibrary* portLibrary,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_memcpyPeerAsync(OMRPortLibrary* portLibrary,
                                           uint32_t targetDeviceId,
                                           void* targetAddress,
                                           uint32_t sourceDeviceId,
                                           const void* sourceAddress,
 ...
 * @return J9CUDA_NO_ERROR on success, any other value on failure
 */
extern "C" int32_t omrcuda_moduleGetGlobal(OMRPortLibrary* portLibrary,
                                           uint32_t deviceId,
                                           J9CudaModule module,
                                           const char* name,
...
struct StreamCallback {
  ...
  static void CUDART_CB handler(cudaStream_t stream,
                                cudaError_t error,
                                void* data) {
