...
static void*
wrapBlockAndSetTags(struct OMRPortLibrary* portLibrary,
                    void* memoryPointer,
                    uintptr_t byteAmount,
                    const char* callSite,
                    const uint32_t category);
static void*
unwrapBlockAndCheckTags(struct OMRPortLibrary* portLibrary,
                        void* memoryPointer);
                       ...
                       uint32_t category)
{
  ...
  if (NULL == pointer) {
    ...
    pointer =
      wrapBlockAndSetTags(portLibrary, pointer, byteAmount, callSite, category);
  }
                         ...
                         uint32_t category)
{
  ...
  Trc_PRT_mem_omrmem_reallocate_memory_Entry(
    memoryPointer, byteAmount, callSite, category);

  if (memoryPointer == NULL) {
    pointer =
      omrmem_allocate_memory(portLibrary,
                             byteAmount,
                             NULL == callSite ? OMR_GET_CALLSITE() : callSite,
                             category);
    ...
    if (NULL != pointer) {
      pointer = wrapBlockAndSetTags(
        portLibrary, pointer, byteAmount, callSite, category);
    }
    if (NULL == pointer) {
      Trc_PRT_mem_omrmem_reallocate_memory_failed_2(
        callSite, memoryPointer, allocationByteAmount);
    }
  }
