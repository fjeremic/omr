  ...
  const char* callSite, uint32_t category)
{
  ...
  if (memoryPointer == NULL) {
    pointer
      = omrmem_allocate_memory(portLibrary, byteAmount, NULL == callSite ? OMR_GET_CALLSITE() : callSite, category);
 ...
 *
 * @return
 * 	OMRPORT_ENSURE_CAPACITY_SUCCESS: if the sub-allocator has allocated byteAmount in low 4G and initialized a heap of
 * that size, OMRPORT_ENSURE_CAPACITY_NOT_REQUIRED: on systems where allocations below the 4 GB bar are not required,
