 ...
 * - allocate32 has no special case for size 0, nor does it have a tracepoint
...
static void *wrapBlockAndSetTags(struct OMRPortLibrary *portLibrary,
                                 void *memoryPointer, uintptr_t byteAmount,
                                 const char *callSite, const uint32_t category);
static void *unwrapBlockAndCheckTags(struct OMRPortLibrary *portLibrary,
                                     void *memoryPointer);

/* Typedefs for basic allocators */
typedef void *(*allocate_memory_func_t)(struct OMRPortLibrary *portLibrary,
                                        uintptr_t byteAmount);
typedef void (*free_memory_func_t)(struct OMRPortLibrary *portLibrary,
                                   void *memoryPointer);
typedef void (*advise_and_free_memory_func_t)(
    struct OMRPortLibrary *portLibrary, void *memoryPointer,
    uintptr_t memorySize);
typedef void *(*reallocate_memory_func_t)(struct OMRPortLibrary *portLibrary,
                                          void *memoryPointer,
                                          uintptr_t byteAmount);
...
BOOLEAN
isLocatedInIgnoredRegion(struct OMRPortLibrary *portLibrary,
                         void *memoryPointer);
...
static void *wrapBlockAndSetTags(struct OMRPortLibrary *portLibrary,
  ...
  footerTag = (J9MemTag *)((uint8_t *)memoryPointer +
                           ROUNDED_FOOTER_OFFSET(byteAmount));
  ...
  omrmem_categories_increment_counters(category,
                                       ROUNDED_BYTE_AMOUNT(byteAmount));
...
static void *unwrapBlockAndCheckTags(struct OMRPortLibrary *portLibrary,
  ...
  if ((checkTagSumCheck(headerTag, J9MEMTAG_EYECATCHER_ALLOC_HEADER) == 0) &&
    ...
    omrmem_categories_decrement_counters(
        headerTag->category, ROUNDED_BYTE_AMOUNT(headerTag->allocSize));

    /* Optimized freed header sumCheck setting */
    headerTag->eyeCatcher = J9MEMTAG_EYECATCHER_FREED_HEADER;
    headerTag->sumCheck = headerTag->sumCheck ^
                          J9MEMTAG_EYECATCHER_ALLOC_HEADER ^
                          J9MEMTAG_EYECATCHER_FREED_HEADER;
    footerTag->eyeCatcher = J9MEMTAG_EYECATCHER_FREED_FOOTER;
    footerTag->sumCheck = footerTag->sumCheck ^
                          J9MEMTAG_EYECATCHER_ALLOC_FOOTER ^
                          J9MEMTAG_EYECATCHER_FREED_FOOTER;
  } else {
 ...
 */
void *omrmem_allocate_memory(struct OMRPortLibrary *portLibrary,
  ...
  /* note that this monitor is protecting a larger area than strictly required
   * but this will make the trace points sane */
  ...
  if (NULL == pointer) {
    ...
    pointer = wrapBlockAndSetTags(portLibrary, pointer, byteAmount, callSite,
                                  category);
  }
 ...
 */
void omrmem_free_memory(struct OMRPortLibrary *portLibrary,
                        void *memoryPointer) {
 ...
 * @note memoryPointer may be NULL. In that case, no action is taken.
 */
void omrmem_advise_and_free_memory(struct OMRPortLibrary *portLibrary,
                                   void *memoryPointer) {
  uintptr_t memorySize = 0;
  advise_and_free_memory_func_t adviseAndFreeFunction =
      omrmem_advise_and_free_memory_basic;
  ...
  if (memoryPointer != NULL) {
     ...
     * No error is raised here b/c one will be raised in
     * 'unwrapBlockAndCheckTags' below.
     */
    if ((checkTagSumCheck(headerTag, J9MEMTAG_EYECATCHER_ALLOC_HEADER) == 0) &&
        (checkPadding(headerTag) == 0)) {
 ...
 * @param[in] callSite Allocation callsite, or NULL if the callsite from the
 * original allocation should be inherited
 * @param[in] category Memory allocation category code
 *
 * @return pointer to memory on success, NULL on error or if byteAmount
 * is 0 and memoryPointer is non-NULL
 *
 * @note if memoryPointer is NULL, this function will behave like
 * mem_allocate_memory for the specified byteAmount
 *
 ...
 * returns NULL
 *
 * @note If this function is unable to allocate the requested memory, the
 * original memory is not freed and may still be used.
 *
 * @internal @warning Do not call error handling code @ref omrerror upon error
 * as the error handling code uses per thread buffers to store the last error.
 * If memory can not be allocated the result would be an infinite loop.
 */
void *omrmem_reallocate_memory(struct OMRPortLibrary *portLibrary,
                               void *memoryPointer, uintptr_t byteAmount,
  ...
  Trc_PRT_mem_omrmem_reallocate_memory_Entry(memoryPointer, byteAmount,
                                             callSite, category);

  if (memoryPointer == NULL) {
    pointer = omrmem_allocate_memory(
        portLibrary, byteAmount,
        NULL == callSite ? OMR_GET_CALLSITE() : callSite, category);
    ...
    pointer =
        reallocateFunction(portLibrary, memoryPointer, allocationByteAmount);
    if (NULL != pointer) {
      pointer = wrapBlockAndSetTags(portLibrary, pointer, byteAmount, callSite,
                                    category);
    }
    if (NULL == pointer) {
      Trc_PRT_mem_omrmem_reallocate_memory_failed_2(callSite, memoryPointer,
                                                    allocationByteAmount);
    }
  }
 ...
 * that were created by @ref omrmem_startup should be destroyed here.
 *
 * @param[in] portLibrary The port library
 *
 ...
 *that are required for the memory operations may be created here.  All
 *resources created here should be destroyed in @ref omrmem_shutdown.
 *
 * @param[in] portLibrary The port library
 ...
 *returned are \arg OMRPORT_ERROR_STARTUP_MEM
 *.
 ...
 */
int32_t omrmem_startup(struct OMRPortLibrary *portLibrary,
...
/**
 * Allocate memory such that the entire allocated block resides in the low 4Gb
 ...
 * success. It will always return a non-NULL pointer to allocated memory.
 *
 * @note This function relies on the implementation of omrvmem and as such
 * memory requests will be rounded up to the default page size.
 *
 * @note A return of NULL may indicate only that memory was not available below
 * the 32-bit bar.
 *
 * @internal @warning Do not call error handling code @ref omrerror upon error
 * as the error handling code uses per thread buffers to store the last error.
 * If memory can not be allocated the result would be an infinite loop.
 */
void *omrmem_allocate_memory32(struct OMRPortLibrary *portLibrary,
                               uintptr_t byteAmount, const char *callSite,
                               uint32_t category) {
  void *pointer = NULL;
#if defined(OMR_ENV_DATA64)
  ...
  if (NULL == pointer) {
    Trc_PRT_mem_omrmem_allocate_memory32_returned_null(callSite,
                                                       allocationByteAmount);
  } else {
    pointer = wrapBlockAndSetTags(portLibrary, pointer, byteAmount, callSite,
                                  category);
 ...
 * @note memoryPointer may be NULL. In that case, no action is taken.
 */
void omrmem_free_memory32(struct OMRPortLibrary *portLibrary,
                          void *memoryPointer) {
...
/**
 * Ensure that byteAmount is available for the sub-allocator in low 4G memory
 * and initialize a sub-allocator heap of that size
 *
 * @param[in] portLibrary The port library
 * @param[in] byteAmount The number of bytes as ensured capacity.
 *
 * @return
 * 	OMRPORT_ENSURE_CAPACITY_SUCCESS: if the sub-allocator has allocated
 * byteAmount in low 4G and initialized a heap of that size,
 * 	OMRPORT_ENSURE_CAPACITY_NOT_REQUIRED: on systems where allocations below
 * the 4 GB bar are not required, OMRPORT_ENSURE_CAPACITY_FAILED: an error
 * occurred.
 */
uintptr_t omrmem_ensure_capacity32(struct OMRPortLibrary *portLibrary,
                                   uintptr_t byteAmount) {
