 ...
 *
 * This file contains the functions supported by the port library for creating,
 * accessing and destroying per thread buffers.@see omrportptb.h for details on
 * the per thread buffer structure.
 *
 * Only the function @omrport_tls_free is available via the port library
 * function table.  The rest of the functions are helpers for the port library
 * only.
 */
...
void *omrport_tls_get(struct OMRPortLibrary *portLibrary) {
  ...
  ptBuffers =
      omrthread_tls_get(omrthread_self(), portLibrary->portGlobals->tls_key);
  if (NULL == ptBuffers) {
    MUTEX_ENTER(portLibrary->portGlobals->tls_mutex);

    ptBuffers = portLibrary->mem_allocate_memory(
        portLibrary, sizeof(PortlibPTBuffers_struct), OMR_GET_CALLSITE(),
        OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL != ptBuffers) {
      if (0 == omrthread_tls_set(omrthread_self(),
        ...
        if (portLibrary->portGlobals->buffer_list) {
          ((PortlibPTBuffers_t)portLibrary->portGlobals->buffer_list)
              ->previous = ptBuffers;
        }
...
void omrport_tls_free(struct OMRPortLibrary *portLibrary) {
  ...
  ptBuffers =
      omrthread_tls_get(omrthread_self(), portLibrary->portGlobals->tls_key);
  if (ptBuffers) {
    omrthread_tls_set(omrthread_self(), portLibrary->portGlobals->tls_key,
                      NULL);
 ...
 * that are required for the portl library thread local storage operations may
 * be created here.  All resources created here should be destroyed in @ref
 * omrport_tls_shutdown.
 *
 * @param[in] portLibrary The port library
 *
 * @return 0 on success, negative error code on failure.  Error code values
 * returned are \arg OMRPORT_ERROR_STARTUP_TLS \arg
 * OMRPORT_ERROR_STARTUP_TLS_ALLOC \arg OMRPORT_ERROR_STARTUP_TLS_MUTEX
 */
 ...
 * @brief Per Thread Buffer Support
 *
 * Get the per thread buffer for a thread. If the buffer has not been allocated
 * do not allocate a new one, the function @ref omrport_tls_get is used for that
 * purpose. This function is not exported outside the port library as most
 * applications will want a per thread buffer created to store their data.  This
