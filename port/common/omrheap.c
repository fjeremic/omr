 ...
 */
void* omrheap_allocate(struct OMRPortLibrary* portLibrary, struct J9Heap* heap, uintptr_t byteAmount)
{
 ...
 * @note omrheap_allocate and omrheap_free are not thread safe. It is up to the user to serialize access if necessary.
 */
void omrheap_free(struct OMRPortLibrary* portLibrary, struct J9Heap* heap, void* address)
{
  ...
  /*determine if the block freed is the first block by checking its slot number. If so there is no previous block to
   * check.*/
 ...
 */
void* omrheap_reallocate(struct OMRPortLibrary* portLibrary, struct J9Heap* heap, void* address, uintptr_t byteAmount)
{
  int64_t* baseSlot = (int64_t*)heap;
 ...
 */
uintptr_t omrheap_query_size(struct OMRPortLibrary* portLibrary, struct J9Heap* heap, void* address)
{
