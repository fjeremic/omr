...
#define ALIGNMENT_ROUND_DOWN(value) \
  (((uintptr_t)value) & (~(sizeof(uint64_t) - 1)))
#define ALIGNMENT_ROUND_UP(value) \
  ((((uintptr_t)value) + (sizeof(uint64_t) - 1)) & (~(sizeof(uint64_t) - 1)))

#define GET_SLOT_NUMBER_FROM(heapBase, heapSlot) \
  ((((uintptr_t)heapSlot) - ((uintptr_t)heapBase)) / sizeof(uint64_t))

/* Amount of metadata used for heap management in bytes. A velid heap size
 * should be at least larger than this. We account for the header size plus 2
 * padding slots for the initial block in the heap.
 */
#define HEAP_MANAGEMENT_OVERHEAD (sizeof(J9Heap) + 2 * sizeof(uint64_t))

/**
 * Initialize a contiguous region of memory at heapBase as a heap. The size of
 * the heap is bounded by heapSize.
 *
 * @param[in] portLibrary The port library
 * @param[in] heapBase Base address of memory region.
 * @param[in] heapSize The size of the memory region to be used as a heap in
 * bytes.
 * @param[in] heapFlags Flags that can affect the heap. Currently, this is not
 * used and is there for future use, for example, to zero out memory. User must
 * pass in zero.
 *
 * @return pointer to an opaque struct representing the heap on success, NULL on
 * failure.
 *
 * @note this function can also be used to re-initialize a heap -- useful when
 * we want to erase its contents and start from scratch.
 *
 * @note in case heapBase isn't 8-aligned, it will be rounded up to the nearest
 * 8-aligned value and the heap will be created at the 8-aligned value. The same
 * goes for heapSize, it will be rounded down if not 8 aligned.
 *
 * @note the algorithm used in this suballocator is based on the first-fit
 * method in KNUTH, D. E. The Art of Computer Programming. Vol. 1: Fundamental
 * Algorithms. (2nd edition). Addison-Wesley, Reading, Mass., 1973, Sect. 2.5.
 *
 * @note due to the overhead of heap management, the actual available space
 * consumed by the user is less than the size of the heap.
 */
struct J9Heap *omrheap_create(struct OMRPortLibrary *portLibrary,
                              void *heapBase, uintptr_t heapSize,
                              uint32_t heapFlags) {
  ...
  /* check if we have enough space taking into account space wasted in rounding
   * up heapBase */
 ...
 *
 * @note omrheap_allocate and omrheap_free are not thread safe. It is up to the
 * user to serialize access if necessary.
 */
void *omrheap_allocate(struct OMRPortLibrary *portLibrary, struct J9Heap *heap,
  ...
  if (0 == byteAmount) {
    ...
    /* round up byteAmount to nearest 8-aligned value and calculate num of slots
     * required */
  ...
  while (blockPaddingCursor < lastSlot) {
    ...
    if (chunkSize < 0) {
      ...
      if ((uintptr_t)chunkSize >= adjustedRequestSize) {
        /* found the chunk, now calculate the residual size after the
         * allocation*/
  ...
  /* need 2 slots for padding, hence we only make a new free block if we have 3
   * or more slots left */
  if (residualSize >= 3) {
    /* write the negated value of newSize to the top and bottom padding slot of
     * the allocated block */
    ...
    if (isFirstFree) {
      heap->firstFreeBlock = (uintptr_t)GET_SLOT_NUMBER_FROM(
          heap, &blockPaddingCursor[newSize + 2]);
      heap->lastAllocSlot = heap->firstFreeBlock;
      heap->largestAllocSizeVisited = 0;
    } else {
      heap->lastAllocSlot = (uintptr_t)GET_SLOT_NUMBER_FROM(
          heap, &blockPaddingCursor[newSize + 2]);
...
/**
 * Free a chunk of memory that was allocated from the heap. address is a memory
 * location returned from calling heap_allocate on the heap
 *
 * @param[in] portLibrary The port library
 ...
 * @return nothing.
 *
 * @note passing in a NULL address is OK. omrheap_free simply returns in this
 * case.
 * @note omrheap_allocate and omrheap_free are not thread safe. It is up to the
 * user to serialize access if necessary.
 */
void omrheap_free(struct OMRPortLibrary *portLibrary, struct J9Heap *heap,
                  void *address) {
  int64_t *thisBlockTopPadding, *previousBlockBottomPadding,
      *nextBlockTopPadding;
  ...
  /*determine if the block freed is the first block by checking its slot number.
   * If so there is no previous block to check.*/
  ...
  /*determine if the block freed is the last block by checking its slot number.
   * If so there is no next block to check.*/
  if (GET_SLOT_NUMBER_FROM(heap, &thisBlockTopPadding[thisBlockSize + 1]) !=
      (totalNumSlots - 1)) {
 ...
 * @note omrheap_reallocate() is not thread safe. It is up to the user to
 * serialize access if necessary.
 */
void *omrheap_reallocate(struct OMRPortLibrary *portLibrary,
  ...
  if (0 == byteAmount) {
    ...
    /* Round up byteAmount to nearest 8-aligned value and calculate number of
     * slots required. */
    adjustedRequestSize =
        (int64_t)(ALIGNMENT_ROUND_UP(byteAmount)) / sizeof(uint64_t);
    /* In case of arithmetic overflow, return NULL. */
    if (byteAmount > (adjustedRequestSize * sizeof(uint64_t))) {
  ...
  isLastBlock =
      GET_SLOT_NUMBER_FROM(heap, &thisBlockTopPadding[thisBlockSize + 1]) ==
      (heap->heapSize - 1);
  if (FALSE == isLastBlock) {
    ...
    if (0 != heap->firstFreeBlock) {
      ...
      nextBlockIsFirstFreeBlock =
          (nextBlockTopPadding == firstFreeBlockTopPadding);
    }
  }
  ...
  if (growAmount > 0) {
     ...
     */
    if (isLastBlock || (nextBlockSize < 0) ||
        (nextBlockSize + 2 < growAmount)) {
    ...
    if ((FALSE == isLastBlock) && (nextBlockSize > 0)) {
      ...
      if ((0 == heap->firstFreeBlock) ||
          (nextBlockSlot < heap->firstFreeBlock)) {
 ...
 * @param[in] portLibrary The port library
 * @param[in] heap Opaque heap pointer.
 * @param[in] address Memory location returned from omrheap_allocate or
 * omrheap_reallocate
 *
 * @return size of the allocation in bytes
 *
 * @note omrheap_query_size() is not thread safe. It is up to the user to
 * serialize access if necessary.
 */
uintptr_t omrheap_query_size(struct OMRPortLibrary *portLibrary,
                             struct J9Heap *heap, void *address) {
...
/**
 * Increase the bounds of the heap by Suballocate byteAmount bytes from the
 * initialized heap. Grow is assumed to happen at the end of the memory range
 * originally used for the heap, i.e. ideal for situations where the heap is
 * backed by uncommitted virtual memory.
 *
 * @param[in] portLibrary The port library
 * @param[in] heap Opaque heap pointer.
 * @param[in] byteAmount number of bytes to be suballocated from the heap.
 ...
 *
 * @note omrheap_grow is not thread safe. It is up to the user to serialize
 * access if necessary.
 */
BOOLEAN
omrheap_grow(struct OMRPortLibrary *portLibrary, struct J9Heap *heap,
  ...
  /*
   * Merge the new free slots with the free slots (if there is any) at the end
   * of the current heap. Initialize the header and tail of the newly added
   * slots. Head/Tail should be set to the number of free slots between them
   */
  baseSlot = (int64_t *)heap;
  /* Check whether the tail of the heap is free or not. */
  temp = baseSlot[heapSize - 1];
  /* If the value at the tail node is negative, then it is occupied, if not, it
   * is free */
  if (0 > temp) {
   ...
   * otherwise previous heap was completely full and firstFreeBlock should point
   * to the newly added slots */
  if (0 == firstFreeBlock) {
