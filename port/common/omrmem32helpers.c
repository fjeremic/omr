...
static void* allocateVmemRegion32(struct OMRPortLibrary* portLibrary, uintptr_t byteAmount, J9HeapWrapper** heapWrapper,
  const char* callSite, uint32_t memoryCategory, uintptr_t vmemMode, uintptr_t vmemAllocOptions);
...
static void* allocateLargeRegion(
  struct OMRPortLibrary* portLibrary, uintptr_t regionSize, const char* callSite, uintptr_t vmemAllocOptions);
static void* iterateHeapsAndSubAllocate(struct OMRPortLibrary* portLibrary, uintptr_t byteAmount);
static J9HeapWrapper* findMatchingHeap(
  struct OMRPortLibrary* portLibrary, void* memoryPointer, J9HeapWrapper*** heapWrapperLocation);
static void* allocateRegion(struct OMRPortLibrary* portLibrary, uintptr_t regionSize, uintptr_t byteAmount,
  const char* callSite, uintptr_t vmemAllocOptions);
static void* reserveAndCommitRegion(
  struct OMRPortLibrary* portLibrary, uintptr_t reserveSize, const char* callSite, uintptr_t vmemAllocOptions);
 ...
 * For testing purposes, this value is mirrored in port library test. If we tune this value, we should also adjust it in
 * omrmemTest.cpp
 */
  ...
  if (NULL != portLibrary->portGlobals) {
    ...
    while (NULL != heapWrapperCursor) {
      ...
      if (!currentHeapWrapper->heap) {
        /* jmem double-accounting prevention: Increment the memory counter so it can be decremented again inside
         * vmem_free_memory */
  ...
  struct OMRPortLibrary* portLibrary, uintptr_t regionSize, const char* callSite, uintptr_t vmemAllocOptions)
{
  ...
  allocPtr = allocateVmemRegion32(portLibrary, roundedRegionSize, &heapWrapper, callSite, OMRMEM_CATEGORY_PORT_LIBRARY,
...
static void* iterateHeapsAndSubAllocate(struct OMRPortLibrary* portLibrary, uintptr_t byteAmount)
{
    ...
    if (NULL != heap) {
      ...
      if (NULL != subAllocPtr) {
        ...
        omrmem_categories_decrement_bytes(
          omrmem_get_category(portLibrary, OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS), byteAmount);
  ...
  if ((NULL != PPG_mem_mem32_subAllocHeapMem32.subCommitHeapWrapper)
    ...
    uintptr_t remainingSize = PPG_mem_mem32_subAllocHeapMem32.suballocator_initialSize
     ...
     */
    commitPtr = (void*)(((uintptr_t)PPG_mem_mem32_subAllocHeapMem32.subCommitHeapWrapper->vmemID->address)
      + PPG_mem_mem32_subAllocHeapMem32.subCommitCommittedMemorySize);
    commitPtr = omrvmem_commit_memory(
      portLibrary, commitPtr, commitSize, PPG_mem_mem32_subAllocHeapMem32.subCommitHeapWrapper->vmemID);

    if (NULL == commitPtr) {
      PPG_mem_mem32_subAllocHeapMem32.canSubCommitHeapGrow = FALSE;
      Trc_PRT_mem_iterateHeapsAndSubAllocate_commitMemoryFailure(
        commitPtr, commitSize, PPG_mem_mem32_subAllocHeapMem32.subCommitHeapWrapper->vmemID);
      return NULL;
    }
 ...
 * If the overhead of omrheap precludes using suballocation, omrheap will not be used and the memory will be used
 * directly instead.
 */
static void* allocateRegion(struct OMRPortLibrary* portLibrary, uintptr_t regionSize, uintptr_t byteAmount,
  const char* callSite, uintptr_t vmemAllocOptions)
{
   ...
   */
  alloc32Ptr = allocateVmemRegion32(portLibrary, roundedRegionSize, &heapWrapper, callSite,
    OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS, VMEM_MODE_COMMIT, vmemAllocOptions);
  if (NULL == alloc32Ptr) {
  ...
  struct OMRPortLibrary* portLibrary, uintptr_t reserveSize, const char* callSite, uintptr_t vmemAllocOptions)
{
  ...
  reserve32Ptr = allocateVmemRegion32(portLibrary, roundedInitialSize, &heapWrapper, callSite,
    OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS, VMEM_MODE_WITHOUT_COMMIT, vmemAllocOptions);
  ...
  const char* callSite, uint32_t memoryCategory, uintptr_t vmemMode, uintptr_t vmemAllocOptions)
{
  ...
  vmemID = portLibrary->mem_allocate_memory(
    portLibrary, sizeof(J9PortVmemIdentifier), OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == vmemID) {
  ...
  wrapper = portLibrary->mem_allocate_memory(
    portLibrary, sizeof(J9HeapWrapper), OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == wrapper) {
      ...
      if (NULL == omrheap) {
        ...
        *heapWrapperUpdate = foundHeapWrapper->nextHeapWrapper;
        ...
        omrmem_categories_increment_bytes(
          omrmem_get_category(portLibrary, OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS), allocationSize);
