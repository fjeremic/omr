...
static void*
allocateVmemRegion32(struct OMRPortLibrary* portLibrary,
...
static void
prependHeapWrapper(struct OMRPortLibrary* portLibrary,
                   J9HeapWrapper* heapWrapper);
static void
updatePPGHeapSizeInfo(struct OMRPortLibrary* portLibrary,
                      uintptr_t totalSizeDelta,
                      BOOLEAN isIncrement);
static void*
allocateLargeRegion(struct OMRPortLibrary* portLibrary,
                    uintptr_t regionSize,
                    const char* callSite,
                    uintptr_t vmemAllocOptions);
static void*
iterateHeapsAndSubAllocate(struct OMRPortLibrary* portLibrary,
                           uintptr_t byteAmount);
static J9HeapWrapper*
findMatchingHeap(struct OMRPortLibrary* portLibrary,
                 void* memoryPointer,
                 J9HeapWrapper*** heapWrapperLocation);
static void*
allocateRegion(struct OMRPortLibrary* portLibrary,
               uintptr_t regionSize,
               uintptr_t byteAmount,
               const char* callSite,
               uintptr_t vmemAllocOptions);
static void*
reserveAndCommitRegion(struct OMRPortLibrary* portLibrary,
...
shutdown_memory32(struct OMRPortLibrary* portLibrary)
{
  ...
  if (NULL != portLibrary->portGlobals) {
    ...
    while (NULL != heapWrapperCursor) {
      ...
      portLibrary->vmem_free_memory(
        portLibrary, vmemID->address, vmemID->size, vmemID);
      /* free the vmemID */
                    ...
                    uintptr_t vmemAllocOptions)
{
  ...
  allocPtr = allocateVmemRegion32(portLibrary,
                                  roundedRegionSize,
                                  &heapWrapper,
                                  callSite,
                           ...
                           uintptr_t byteAmount)
{
  ...
  while (NULL != heapWrapperCursor) {
    ...
    if (NULL != heap) {
      ...
      if (NULL != subAllocPtr) {
        Trc_PRT_mem_allocate_memory32_suballoc_block(
          subAllocPtr, heap, byteAmount);
  ...
  if ((NULL != PPG_mem_mem32_subAllocHeapMem32.subCommitHeapWrapper) &&
     ...
     */
    commitPtr =
      (void*)(((uintptr_t)PPG_mem_mem32_subAllocHeapMem32.subCommitHeapWrapper
                 ->vmemID->address) +
              PPG_mem_mem32_subAllocHeapMem32.subCommitCommittedMemorySize);
               ...
               uintptr_t vmemAllocOptions)
{
   ...
   */
  alloc32Ptr =
    allocateVmemRegion32(portLibrary,
                         roundedRegionSize,
                         &heapWrapper,
                         callSite,
                         OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS,
  ...
  if (NULL != subAllocPtr) {
    ...
    omrmem_categories_decrement_bytes(
      omrmem_get_category(
        portLibrary, OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS),
      byteAmount);
                       ...
                       uintptr_t vmemAllocOptions)
{
  ...
  reserve32Ptr = omrvmem_commit_memory(
    portLibrary, reserve32Ptr, commitSize, heapWrapper->vmemID);
 ...
 */
void*
allocate_memory32(struct OMRPortLibrary* portLibrary,
                  uintptr_t byteAmount,
                  const char* callSite)
{
  void* returnPtr = NULL;
  ...
  if (TRUE == useMalloc) {
    ...
    if (NULL == returnPtr) {
      if (byteAmount >= HEAP_SIZE_BYTES) {
        ...
        returnPtr =
          allocateRegion(portLibrary, HEAP_SIZE_BYTES, byteAmount, callSite, 0);
      }
    }
                     ...
                     uintptr_t vmemAllocOptions)
{
  ...
  vmemID = portLibrary->mem_allocate_memory(portLibrary,
                                            sizeof(J9PortVmemIdentifier),
                                            OMR_GET_CALLSITE(),
                                            OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == vmemID) {
  ...
  wrapper = portLibrary->mem_allocate_memory(portLibrary,
                                             sizeof(J9HeapWrapper),
                                             OMR_GET_CALLSITE(),
                                             OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == wrapper) {
...
free_memory32(struct OMRPortLibrary* portLibrary, void* memoryPointer)
{
  ...
  if (OMRPORT_DISABLE_ENSURE_CAP32 ==
    ...
    if ((foundHeapWrapper = findMatchingHeap(
           portLibrary, memoryPointer, &heapWrapperUpdate)) != NULL) {
