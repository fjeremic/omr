...
static void *allocateVmemRegion32(struct OMRPortLibrary *portLibrary,
                                  uintptr_t byteAmount,
                                  J9HeapWrapper **heapWrapper,
                                  const char *callSite, uint32_t memoryCategory,
                                  uintptr_t vmemMode,
                                  uintptr_t vmemAllocOptions);
static void prependHeapWrapper(struct OMRPortLibrary *portLibrary,
                               J9HeapWrapper *heapWrapper);
static void updatePPGHeapSizeInfo(struct OMRPortLibrary *portLibrary,
                                  uintptr_t totalSizeDelta,
                                  BOOLEAN isIncrement);
static void *allocateLargeRegion(struct OMRPortLibrary *portLibrary,
                                 uintptr_t regionSize, const char *callSite,
                                 uintptr_t vmemAllocOptions);
static void *iterateHeapsAndSubAllocate(struct OMRPortLibrary *portLibrary,
                                        uintptr_t byteAmount);
static J9HeapWrapper *findMatchingHeap(struct OMRPortLibrary *portLibrary,
                                       void *memoryPointer,
                                       J9HeapWrapper ***heapWrapperLocation);
static void *allocateRegion(struct OMRPortLibrary *portLibrary,
                            uintptr_t regionSize, uintptr_t byteAmount,
                            const char *callSite, uintptr_t vmemAllocOptions);
static void *reserveAndCommitRegion(struct OMRPortLibrary *portLibrary,
                                    uintptr_t reserveSize, const char *callSite,
                                    uintptr_t vmemAllocOptions);

#define VMEM_MODE_COMMIT                                                       \
  OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE |             \
      OMRPORT_VMEM_MEMORY_MODE_COMMIT
#define VMEM_MODE_WITHOUT_COMMIT                                               \
  OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE

struct {
  uintptr_t base;
  uintptr_t limit;
} regions[] = {{0x0, 0xFFFFFFFF}};
 ...
 * We use a 8MB heap to give us more room in case an application loads a larger
 * amount of classes than usual. For testing purposes, this value is mirrored in
 * port library test. If we tune this value, we should also adjust it in
 * omrmemTest.cpp
 */
#if defined(AIXPPC) && defined(OMR_GC_COMPRESSED_POINTERS)
/* virtual memory is allocated in 256M segments on AIX, so grab the whole
 * segment */
 ...
 *
 * Note: Any resources created here need to be cleaned up in
 * shutdown_memory32_using_vmem
 */
int32_t startup_memory32(struct OMRPortLibrary *portLibrary) {
  ...
  if (0 !=
      omrthread_monitor_init(&(PPG_mem_mem32_subAllocHeapMem32.monitor), 0)) {
    return (int32_t)OMRPORT_ERROR_STARTUP_MEM;
  }
  return 0;
}

/* Cleans up any resources created by startup_memory32 and during
  ...
  if (NULL != portLibrary->portGlobals) {
    ...
    while (NULL != heapWrapperCursor) {
      ...
      if (!currentHeapWrapper->heap) {
        /* jmem double-accounting prevention: Increment the memory counter so it
         * can be decremented again inside vmem_free_memory */
        omrmem_categories_increment_counters(vmemID->category, vmemID->size);
      }

      /* free the vmem for each heap */
      portLibrary->vmem_free_memory(portLibrary, vmemID->address, vmemID->size,
                                    vmemID);
      /* free the vmemID */
...
/* prepend the heapWrapper to the list in
 * OMRPortPlatformGlobals.J9SubAllocateHeap32 */
static void prependHeapWrapper(struct OMRPortLibrary *portLibrary,
                               J9HeapWrapper *heapWrapper) {
  heapWrapper->nextHeapWrapper =
      PPG_mem_mem32_subAllocHeapMem32.firstHeapWrapper;
  PPG_mem_mem32_subAllocHeapMem32.firstHeapWrapper = heapWrapper;
}

/* Adjust the totalSize in OMRPortPlatformGlobals.J9SubAllocateHeap32 based on
 * boolean value isIncrement. Increment the size info if true and decrement if
 * isIncrement is false.
 */
static void updatePPGHeapSizeInfo(struct OMRPortLibrary *portLibrary,
                                  uintptr_t totalSizeDelta,
                                  BOOLEAN isIncrement) {
...
static void *allocateLargeRegion(struct OMRPortLibrary *portLibrary,
  ...
  allocPtr = allocateVmemRegion32(portLibrary, roundedRegionSize, &heapWrapper,
                                  callSite, OMRMEM_CATEGORY_PORT_LIBRARY,
                                  VMEM_MODE_COMMIT, vmemAllocOptions);

  if (NULL != allocPtr) {
    prependHeapWrapper(portLibrary, heapWrapper);
    updatePPGHeapSizeInfo(portLibrary, roundedRegionSize, TRUE);
    Trc_PRT_mem_allocate_memory32_alloc_large_region(allocPtr,
                                                     roundedRegionSize);

    /* The memory category for the user of this memory will be incremented in
     * omrmemtag.c. To prevent double-accounting, we must reset the port library
     * counter.
     */
    omrmem_categories_decrement_counters(heapWrapper->vmemID->category,
                                         roundedRegionSize);
  } else {
    Trc_PRT_mem_allocate_memory32_alloc_large_region_failed_2(
        callSite, roundedRegionSize);
  }
  return allocPtr;
}

/* attemp suballocation on each omrheap found in the chain untill there is a
 * success, otherwise return NULL */
static void *iterateHeapsAndSubAllocate(struct OMRPortLibrary *portLibrary,
                                        uintptr_t byteAmount) {
  J9HeapWrapper *heapWrapperCursor = NULL;

  /* traverse the linked list of J9HeapWrapper starting at
   * J9SubAllocateHeap32.firstHeapWrapper */
  heapWrapperCursor = PPG_mem_mem32_subAllocHeapMem32.firstHeapWrapper;
  while (NULL != heapWrapperCursor) {
    J9Heap *heap = heapWrapperCursor->heap;

    /* heap field is non-null, we have a valid J9Heap to attempt our
     * suballocation */
    if (NULL != heap) {
      void *subAllocPtr =
          portLibrary->heap_allocate(portLibrary, heap, byteAmount);
      if (NULL != subAllocPtr) {
        Trc_PRT_mem_allocate_memory32_suballoc_block(subAllocPtr, heap,
                                                     byteAmount);

        /* omrmem category double-accounting prevention: subtract the amount
         * allocated from the unused category */
        omrmem_categories_decrement_bytes(
            omrmem_get_category(
                portLibrary,
  ...
  /**
   * If we are here, then there is not enough space in the existing heapWrappers
   * to satisfy the requested byteAmount. Then check the subCommitHeapWrapper
   * whether it can grow or not. If it can grow, meaning not all of the
   * initially reserved memory is committed, then commit more memory from the
   * reserved memory, grow the heap and try to allocate byteAmount. If the
   * memory is allocated successfully after growing the heap, then return the
   * memory address of allocated memory, otherwise, return NULL.
   *
   */
  if ((NULL != PPG_mem_mem32_subAllocHeapMem32.subCommitHeapWrapper) &&
    ...
    uintptr_t commitSize =
        PPG_mem_mem32_subAllocHeapMem32.suballocator_commitSize;
    uintptr_t remainingSize =
        PPG_mem_mem32_subAllocHeapMem32.suballocator_initialSize -
        PPG_mem_mem32_subAllocHeapMem32.subCommitCommittedMemorySize;

    heap = PPG_mem_mem32_subAllocHeapMem32.subCommitHeapWrapper->heap;

    /* Make sure commitSize is not bigger than remaining size in the reserved
     * memory*/
    if (commitSize > remainingSize) {
      commitSize = remainingSize;
    }
    /* commit the memory from the end of the last committed chunk,
     * not the end address of the heap since it is probably different cause of
     * rounding down in heap creation.
     */
    commitPtr =
        (void *)(((uintptr_t)PPG_mem_mem32_subAllocHeapMem32
                      .subCommitHeapWrapper->vmemID->address) +
                 PPG_mem_mem32_subAllocHeapMem32.subCommitCommittedMemorySize);
    commitPtr = omrvmem_commit_memory(
        portLibrary, commitPtr, commitSize,
        PPG_mem_mem32_subAllocHeapMem32.subCommitHeapWrapper->vmemID);

    if (NULL == commitPtr) {
      PPG_mem_mem32_subAllocHeapMem32.canSubCommitHeapGrow = FALSE;
      Trc_PRT_mem_iterateHeapsAndSubAllocate_commitMemoryFailure(
          commitPtr, commitSize,
          PPG_mem_mem32_subAllocHeapMem32.subCommitHeapWrapper->vmemID);
      return NULL;
    }

    /* If memory is committed successfully, then update the value of
     * committedSize */
    PPG_mem_mem32_subAllocHeapMem32.subCommitCommittedMemorySize += commitSize;

    if (PPG_mem_mem32_subAllocHeapMem32.subCommitCommittedMemorySize ==
        PPG_mem_mem32_subAllocHeapMem32.suballocator_initialSize) {
      PPG_mem_mem32_subAllocHeapMem32.canSubCommitHeapGrow = FALSE;
    }
    ...
    if (omrheap_grow(portLibrary, heap, commitSize)) {
      void *subAllocPtr =
          portLibrary->heap_allocate(portLibrary, heap, byteAmount);
      Trc_PRT_mem_iterateHeapsAndSubAllocate_heap_grow_successful(heap,
                                                                  commitSize);
 ...
 * suballocat byteAmount within that memory. If the overhead of omrheap
 * precludes using suballocation, omrheap will not be used and the memory will
...
static void *allocateRegion(struct OMRPortLibrary *portLibrary,
   ...
   * allocate and create a new roundedRegionSize bytes J9Heap and attempt the
   * request again
   */
  alloc32Ptr = allocateVmemRegion32(
      portLibrary, roundedRegionSize, &heapWrapper, callSite,
      OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS, VMEM_MODE_COMMIT,
      vmemAllocOptions);
  if (NULL == alloc32Ptr) {
    Trc_PRT_mem_allocate_memory32_alloc_normal_region_failed_2(
        callSite, roundedRegionSize);
    return NULL;
  }
  ...
  omrheap =
      portLibrary->heap_create(portLibrary, alloc32Ptr, roundedRegionSize, 0);
  ...
  if (NULL != subAllocPtr) {
    ...
    /* omrmem_category double-accounting prevention: subtract allocated amount
     * from unused category */
    omrmem_categories_decrement_bytes(
        omrmem_get_category(
            portLibrary,
            OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS),
    ...
    Trc_PRT_mem_allocate_memory32_alloc_normal_region_suballoc(
        alloc32Ptr, omrheap, returnPtr, byteAmount);
  } else {
    /* if the byteAmount requested is so close to roundedRegionSize that
     * suballocate byteAmount in the omrheap cannot be satisfied due to omrheap
     * management overhead and roudning effect, we simply return the vmem
     * allocated pointer and not treat it as a J9Heap.
     */
    /* no need to set heapWrapper->heap = NULL since it is already done in
     * allocateVmemRegion32 */
    returnPtr = alloc32Ptr;

    /* Need to fix-up the category so the block is associated with port library,
     * not the allocate32 slab */
    omrmem_categories_decrement_counters(heapWrapper->vmemID->category,
                                         heapWrapper->vmemID->size);
    heapWrapper->vmemID->category =
        omrmem_get_category(portLibrary, OMRMEM_CATEGORY_PORT_LIBRARY);
    /* Don't increment the category for port library - this category will be
     * incremented for the user in omrmemtag.c */

    Trc_PRT_mem_allocate_memory32_alloc_normal_region_consumed(returnPtr,
                                                               byteAmount);
  }
...
static void *reserveAndCommitRegion(struct OMRPortLibrary *portLibrary,
  ...
  reserve32Ptr = allocateVmemRegion32(
      portLibrary, roundedInitialSize, &heapWrapper, callSite,
      OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS,
      VMEM_MODE_WITHOUT_COMMIT, vmemAllocOptions);
  ...
  reserve32Ptr = omrvmem_commit_memory(portLibrary, reserve32Ptr, commitSize,
                                       heapWrapper->vmemID);
  /* If committing part/all of the reserved memory has failed, then do not
   * continue and return NULL */
  if (NULL == reserve32Ptr) {
    PPG_mem_mem32_subAllocHeapMem32.canSubCommitHeapGrow = FALSE;
    Trc_PRT_mem_reserveAndCommitRegion_commitMemoryFailure(
        reserve32Ptr, commitSize, heapWrapper->vmemID);
    return NULL;
  }

  /* PPG_mem_mem32_subAllocHeapMem32.subCommitCommittedMemorySize should be zero
   * here, since this function is only called from ensure_capacity32 which is
  ...
  /* If there is no space left to grow heap, then set the global
   * 'canSubCommitHeapGrow' to FALSE */
 ...
 *
 * Uses vmem as underlying storage and j9Heap API for the suballocation on the
 * underlying storage.
 */
...
#if defined(J9ZOS390) && defined(OMR_GC_COMPRESSED_POINTERS)
  if (OMRPORT_DISABLE_ENSURE_CAP32 ==
      portLibrary->portGlobals->disableEnsureCap32) {
    useMalloc31 = TRUE;
  }
  if (TRUE == useMalloc31) {
  ...
  if (TRUE == useMalloc) {
    ...
    if (NULL == returnPtr) {
      if (byteAmount >= HEAP_SIZE_BYTES) {
        ...
        returnPtr = allocateRegion(portLibrary, HEAP_SIZE_BYTES, byteAmount,
                                   callSite, 0);
      }
    }
...
uintptr_t ensure_capacity32(struct OMRPortLibrary *portLibrary,
  ...
  /* For 64 bit os, use flag OMRPORT_VMEM_ALLOC_QUICK as it is in the startup
   * period. */
  ...
  while (NULL != heapWrapperCursor) {
    if (heapWrapperCursor->heapSize >= byteAmount && heapWrapperCursor->heap) {
      ...
      Trc_PRT_mem_ensure_capacity32_already_initialized(
          heapWrapperCursor->heap, heapWrapperCursor->heapSize);
      break;
    }
    heapWrapperCursor = heapWrapperCursor->nextHeapWrapper;
  }
  ...
  if (0 == returnValue) {
    if (0 == PPG_mem_mem32_subAllocHeapMem32.subCommitCommittedMemorySize) {
      ...
      returnValue = (uintptr_t)reserveAndCommitRegion(
          portLibrary, byteAmount, OMR_GET_CALLSITE(), vmemAllocOptions);
    } else {
      returnValue = (uintptr_t)allocateRegion(
          portLibrary, byteAmount, 0, OMR_GET_CALLSITE(), vmemAllocOptions);
    }
  }
...
/**
 * Allocates a chunk of memory with vmem in the low 32 bit using a brute force
 * search. If the underlying vmem allocation is successful, allocate a
 * J9HeapWrapper and initialize its fields.
 */
static void *allocateVmemRegion32(struct OMRPortLibrary *portLibrary,
                                  uintptr_t byteAmount,
                                  J9HeapWrapper **heapWrapper,
  ...
  /* If the port library does not support
   * OMR_PORT_CAPABILITY_CAN_RESERVE_SPECIFIC_ADDRESS capability on this
   * platform, allocateVmemRegion32 should return NULL, as omrvmem won't be able
   * to satisfy the request for a specific address (it might get lucky, but we
  ...
  vmemID = portLibrary->mem_allocate_memory(
      portLibrary, sizeof(J9PortVmemIdentifier), OMR_GET_CALLSITE(),
  ...
  wrapper = portLibrary->mem_allocate_memory(portLibrary, sizeof(J9HeapWrapper),
                                             OMR_GET_CALLSITE(),
                                             OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == wrapper) {
  ...
  for (i = 0; i < sizeof(regions) / sizeof(regions[0]); i++) {
    ...
    lowAddress = (regions[i].base != 0) ? (uint8_t *)regions[i].base
                                        : (uint8_t *)pageSize;
...
/* Go through the list of heaps to find a J9HeapWrapper that contains
 * memoryPointer */
static J9HeapWrapper *findMatchingHeap(struct OMRPortLibrary *portLibrary,
                                       void *memoryPointer,
                                       J9HeapWrapper ***heapWrapperLocation) {
  ...
  /* iterate through the J9HeapWrapper list and look for the entry associated
   * with memoryPointer */
  ...
  *heapWrapperLocation = &(PPG_mem_mem32_subAllocHeapMem32.firstHeapWrapper);
  ...
  while (NULL != currentHeapWrapper) {
    ...
    /* we found the belonging heap if memoryPointer falls in the range of vmem
     * allocated for the heap */
    if ((((uint8_t *)memoryPointer) >= regionStart) &&
        (((uint8_t *)memoryPointer) < regionEnd)) {
...
/* Frees memory that was allocated below the 4GB boundary using
 * mem_allocate_memory32 */
void free_memory32(struct OMRPortLibrary *portLibrary, void *memoryPointer) {
  ...
  if (OMRPORT_DISABLE_ENSURE_CAP32 ==
    ...
    if ((foundHeapWrapper = findMatchingHeap(portLibrary, memoryPointer,
                                             &heapWrapperUpdate)) != NULL) {
      ...
      if (NULL == omrheap) {
        ...
        /* a null heap field in J9HeapWrapper means this is not a suballocating
         * J9Heap, so we call vmem_free_memory to free the underlying vmem.
         */
        portLibrary->vmem_free_memory(portLibrary, vmemID->address,
                                      vmemID->size, vmemID);
        ...
        *heapWrapperUpdate = foundHeapWrapper->nextHeapWrapper;
        ...
        /* omrmem_category double-accounting prevention: add the size of the
         * block to the "unused" category */
        allocationSize =
            portLibrary->heap_query_size(portLibrary, omrheap, memoryPointer);
        omrmem_categories_increment_bytes(
            omrmem_get_category(
                portLibrary,
                OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS),
            allocationSize);
