                   ...
                   int32_t mode)
{
  ...
  if (NULL == fileStream) {
    ...
    Trc_PRT_filestream_open_fdopenFailed(
      path, flags, mode, findError(savedErrno));
    portLibrary->error_set_last_error(
      portLibrary, savedErrno, findError(savedErrno));
  }
                    ...
                    intptr_t nbytes)
{
  ...
  if ((nbytes < 0) || (NULL == buf) || (NULL == fileStream)) {
    ...
    if (0 != ferror(fileStream)) {
      rc =
        portLibrary->error_set_last_error(portLibrary, errno, findError(errno));
      Trc_PRT_filestream_write_failedToWrite(
        fileStream, buf, nbytes, (int32_t)rc);
    }
  }
 ...
 */
int32_t
omrfilestream_setbuffer(OMRPortLibrary* portLibrary,
                        OMRFileStream* fileStream,
                        char* buffer,
                        int32_t mode,
                        uintptr_t size)
