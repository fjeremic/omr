 ...
 *
 * This file implements file I/O using streams.  It is similar in API to
 * omrfile, and functionally similar to C standard IO.
 */
...
/**
 * @internal
 * Take a portable buffering mode, and turn it into a platform-specific
 * buffering mode.
 *
 * @param[in] mode Portable buffering mode. See @ref omrfilestream_setbuffer for
 * more mode information.
 * @return Platform specific buffering mode
 *
 * @note This will not check to make sure that the mode is valid
 */
static int translateBufferingMode(int32_t mode) {
...
/**
 * @internal
 * Turns portable open flags into system-specific open flags. These flags will
 * be suitable for the function fdopen. fdopen does not accept 'b', and does not
 * truncate a file when using 'w' or 'w+'.
 *
 * Most flags are ignored by this function: EsOpenCreate, EsOpenCreateAlways,
 * EsOpenCreateNew, EsOpenTrunc, EsOpenText, EsOpenSync, EsOpenForInherit.
 *
 * @param[in] flags The open flags for the functions.  At least one of the
 * reading or writing open flags should be used: EsOpenWrite, EsOpenRead.
 *
 * @return String flags for the filestream. NULL if the flags do not open the
 * file for reading or writing.
 */
static const char *EsTranslateOpenFlags(int32_t flags) {
  /* Ignore all other flags: they are taken care of earlier */
  flags &= (EsOpenWrite | EsOpenRead | EsOpenAppend);

#if defined(J9ZOS390)
  /* On Z/OS, stdio functions are shadowed to take ASCII strings, and turn them
   * into EBCDIC. Since fdopen is a POSIX function, it needs EBCDIC strings.
   */
...
/**
 * @internal
 * Determines the proper portable error code to return given a native error
 * code.
 *
 ...
 * that are required for the filestream operations may be created here.  All
 * resources created here will be destroyed in
 ...
 */
int32_t omrfilestream_startup(struct OMRPortLibrary *portLibrary) { return 0; }
 ...
 * @param[in] portLibrary The port library.
 */
void omrfilestream_shutdown(struct OMRPortLibrary *portLibrary) { return; }

/**
 * Opens a buffered filestream.  All filestreams opened this way start fully
 * buffered. Opening a filestream with EsOpenRead is not supported, and will
 ...
 *
 * @note accesses the "current working directory" property of the process if the
 * path is relative.
 */
OMRFileStream *omrfilestream_open(struct OMRPortLibrary *portLibrary,
  ...
  fileDescriptor =
      portLibrary->file_convert_omrfile_fd_to_native_fd(portLibrary, file);
  ...
  if (NULL == fileStream) {
    ...
    Trc_PRT_filestream_open_fdopenFailed(path, flags, mode,
                                         findError(savedErrno));
    portLibrary->error_set_last_error(portLibrary, savedErrno,
                                      findError(savedErrno));
  }
 ...
 */
int32_t omrfilestream_close(struct OMRPortLibrary *portLibrary,
 ...
 * @return 0 on success, negative portable error code on failure.
 */
int32_t omrfilestream_sync(struct OMRPortLibrary *portLibrary,
                           OMRFileStream *fileStream) {
  int32_t rc = 0;
...
/**
 * Write to a file. Writes up to nbytes from the provided buffer to the file
 * referenced by the filestream. It is not an error if not all bytes requested
 ...
 */
intptr_t omrfilestream_write(struct OMRPortLibrary *portLibrary,
  ...
  if ((nbytes < 0) || (NULL == buf) || (NULL == fileStream)) {
    ...
    /* nbytes may be truncated during this cast, resulting in smaller writes
     * than intended */
    rc = (intptr_t)fwrite(buf, 1, (size_t)nbytes, fileStream);
    if (0 != ferror(fileStream)) {
      rc = portLibrary->error_set_last_error(portLibrary, errno,
                                             findError(errno));
      Trc_PRT_filestream_write_failedToWrite(fileStream, buf, nbytes,
                                             (int32_t)rc);
    }
  }
 ...
 * @param[in] buffer User allocated buffer.  It must be size bytes long.  If
 * buffer is NULL, a buffer is automatically allocated, and deallocated when the
 * stream is closed.  If a buffer is passed in, it is the caller's
 ...
 *   \arg OMRPORT_FILESTREAM_LINE_BUFFERING - On output, buffer until a newline
 * is written. \arg OMRPORT_FILESTREAM_NO_BUFFERING - Do not buffer file writes
 * If mode is OMRPORT_FILESTREAM_NO_BUFFERING, the buffer and size parameters
 * are ignored. OMRPORT_FILESTREAM_LINE_BUFFERING has limited support, and is
 * not guaranteed that it will flush the buffer at the newline.  It should be
 * treated similarly to full buffering.
 * @param[in] size The size of buffer. If buffer is NULL, this may set the size
 * of the buffer automatically allocated.  It must be greater than 0, unless
 * mode is OMRPORT_FILESTREAM_NO_BUFFERING.
 *
 * @return 0 on success, negative portable error code on failure.
 *
 * @note It is not guaranteed that the entire buffer will be used.
 */
int32_t omrfilestream_setbuffer(OMRPortLibrary *portLibrary,
 ...
 */
OMRFileStream *omrfilestream_fdopen(OMRPortLibrary *portLibrary, intptr_t fd,
                                    int32_t flags) {
  ...
  /* Get the native file descriptor */
  fileDescriptor =
      portLibrary->file_convert_omrfile_fd_to_native_fd(portLibrary, fd);
 ...
 * @param[in] portLibrary The port library.
 * @param[in] fileStream The filestream;
 * @return The portable file descriptor. Will return OMRPORT_INVALID_FD on
 * error. Calling this function on an invalid filestream will result in
 * undefined behaviour.
 */
intptr_t omrfilestream_fileno(OMRPortLibrary *portLibrary,
                              OMRFileStream *fileStream) {
  ...
  if (NULL == fileStream) {
    ...
    if (-1 == fileDescriptor) {
      int32_t rc = portLibrary->error_set_last_error(portLibrary, errno,
                                                     findError(errno));
      Trc_PRT_filestream_fileno_failed(fileStream, rc);
    } else {
      omrfile = portLibrary->file_convert_native_fd_to_omrfile_fd(
          portLibrary, fileDescriptor);
    }
  }
