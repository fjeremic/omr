...
void omrmem_free_memory_basic(struct OMRPortLibrary* portLibrary, void* memoryPointer)
{
  free(memoryPointer);
}
...
void omrmem_startup_basic(struct OMRPortLibrary* portLibrary, uintptr_t portGlobalSize)
{
  portLibrary->portGlobals
    = portLibrary->mem_allocate_memory(portLibrary, portGlobalSize, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
