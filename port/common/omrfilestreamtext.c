...
static intptr_t transliterate_write_text(
  struct OMRPortLibrary* portLibrary, OMRFileStream* fileStream, const char* buf, intptr_t nbytes, int32_t toEncoding);
 ...
 * "PortLibrary printf" in the "Inside J9" Lotus Notes database.
 */
void omrfilestream_printf(struct OMRPortLibrary* portLibrary, OMRFileStream* fileStream, const char* format, ...)
{
  va_list args;
 ...
 */
intptr_t omrfilestream_write_text(
  struct OMRPortLibrary* portLibrary, OMRFileStream* fileStream, const char* buf, intptr_t nbytes, int32_t toEncoding)
{
  intptr_t rc = 0;
  ...
  struct OMRPortLibrary* portLibrary, OMRFileStream* fileStream, const char* buf, intptr_t nbytes, int32_t toEncoding)
{
  ...
  if (rc < 0) {
    ...
    expandedBuffer
      = portLibrary->mem_allocate_memory(portLibrary, rc, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
