...
static intptr_t
write_all(struct OMRPortLibrary* portLibrary,
          OMRFileStream* fileStream,
          const char* buf,
          intptr_t nbytes);

static intptr_t
transliterate_write_text(struct OMRPortLibrary* portLibrary,
                         OMRFileStream* fileStream,
                         const char* buf,
                         intptr_t nbytes,
                         int32_t toEncoding);
                      ...
                      va_list args)
{
  ...
  if (sizeof(localBuffer) >= bufferSize) {
    ...
    Trc_PRT_filestream_vprintf_stackBufferNotBigEnough(
      fileStream, format, bufferSize);
    writeBuffer =
  ...
  if (NULL != writeBuffer) {
    ...
    portLibrary->filestream_write_text(portLibrary,
                                       fileStream,
                                       writeBuffer,
                                       stringSize,
                         ...
                         int32_t toEncoding)
{
  ...
  if ((nbytes < 0) || (NULL == buf) || (NULL == fileStream)) {
    Trc_PRT_filestream_write_text_invalidArgs(
      fileStream, buf, nbytes, toEncoding);
  ...
  if (fromEncoding == toEncoding) {
    ...
    rc = transliterate_write_text(
      portLibrary, fileStream, buf, nbytes, toEncoding);
  }
                         ...
                         int32_t toEncoding)
{
  ...
  rc = portLibrary->str_convert(
    portLibrary, fromEncoding, toEncoding, buf, nbytes, NULL, 0);
  if (rc < 0) {
  ...
  rc = portLibrary->str_convert(portLibrary,
                                fromEncoding,
                                toEncoding,
                                buf,
                                nbytes,
  ...
  if (rc < 0) {
    ...
    if (rc < 0) {
      Trc_PRT_filestream_write_text_failedToWrite(
        fileStream, expandedBuffer, convertedSize, rc);
    }
  }
