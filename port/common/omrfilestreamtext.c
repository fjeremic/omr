 ...
 *
 * This file implements stream based file IO with the ability to convert to
...
static intptr_t write_all(struct OMRPortLibrary *portLibrary,
                          OMRFileStream *fileStream, const char *buf,
                          intptr_t nbytes);

static intptr_t transliterate_write_text(struct OMRPortLibrary *portLibrary,
 ...
 *
 * Writes formatted output to the filestream. All text will be transliterated
 * from modifed UTF-8 to the platform encoding.
 *
 ...
 * database.
 */
void omrfilestream_vprintf(struct OMRPortLibrary *portLibrary,
  ...
  if (sizeof(localBuffer) >= bufferSize) {
    ...
    Trc_PRT_filestream_vprintf_stackBufferNotBigEnough(fileStream, format,
                                                       bufferSize);
  ...
  if (NULL != writeBuffer) {
    stringSize = portLibrary->str_vprintf(portLibrary, writeBuffer, bufferSize,
                                          format, args);
    portLibrary->filestream_write_text(portLibrary, fileStream, writeBuffer,
                                       stringSize, J9STR_CODE_PLATFORM_RAW);
    ...
    portLibrary->filestream_write_text(portLibrary, fileStream, localBuffer,
                                       stringSize, J9STR_CODE_PLATFORM_RAW);
  }
 ...
 * database.
 */
void omrfilestream_printf(struct OMRPortLibrary *portLibrary,
                          OMRFileStream *fileStream, const char *format, ...) {
  va_list args;
...
/**
 * Write text to a filestream. This function will take a MUTF-8 encoded string,
 * and convert it into the specified encoding. This makes it suitable for
 * printing human readable text to the console or a file.
 *
 ...
 * @param[in] toEncoding The desired output encoding.The desired output
 * encoding. May be any of the output encodings accepted by omrstr_convert().
 *
 * @return Number of bytes written from buf, negative portable error code on
 * failure. This may not be the number of bytes actually written to the file,
 * which is locale specific.
 *
 * @note If an error occurred, the file position is unknown.
 */
intptr_t omrfilestream_write_text(struct OMRPortLibrary *portLibrary,
                                  OMRFileStream *fileStream, const char *buf,
  ...
  if ((nbytes < 0) || (NULL == buf) || (NULL == fileStream)) {
    Trc_PRT_filestream_write_text_invalidArgs(fileStream, buf, nbytes,
                                              toEncoding);
...
#if defined(CRLFNEWLINES)
  /* We have to change the line endings.  This must be done entirely before
   * transliteration. */
  ...
  if (fromEncoding == toEncoding) {
    /* omrstr_convert() does not support MUTF-8 -> MUTF-8 conversion, so do not
     * use it. This has the side effect of not validating the string */
    rc = write_all(portLibrary, fileStream, buf, nbytes);
  } else {
    rc = transliterate_write_text(portLibrary, fileStream, buf, nbytes,
                                  toEncoding);
  }
  ...
  /* if rc is not an error code, then we assume we have written everything from
   * the buffer.  At this point, we want to return the number of characters
   * consumed from the input buffer, not the bytes output to the file.
   */
  if (rc > 0) {
    rc = nbytes;
  }
...
static intptr_t transliterate_write_text(struct OMRPortLibrary *portLibrary,
   ...
   */
  rc = portLibrary->str_convert(portLibrary, fromEncoding, toEncoding, buf,
                                nbytes, NULL, 0);
  if (rc < 0) {
  ...
  rc = portLibrary->str_convert(portLibrary, fromEncoding, toEncoding, buf,
                                nbytes, expandedBuffer, convertedSize);
  if (rc < 0) {
    Trc_PRT_filestream_write_text_failedStringTransliteration(
        fileStream, buf, nbytes, toEncoding, (int32_t)rc);
    rc = OMRPORT_ERROR_FILE_OPFAILED;
  } else {
    /* Write the converted bytes to the file, we will return whatever
     * error/bytecount */
    convertedSize = rc;
    rc = write_all(portLibrary, fileStream, expandedBuffer, convertedSize);
    if (rc < 0) {
      Trc_PRT_filestream_write_text_failedToWrite(fileStream, expandedBuffer,
                                                  convertedSize, rc);
    }
  }
...
/**
 * @internal
 * Write all text to a filestream.  This will write in a loop to ensure that all
 * text is written.
 */
static intptr_t write_all(struct OMRPortLibrary *portLibrary,
                          OMRFileStream *fileStream, const char *buf,
  ...
  while (remaining > 0) {
    rc =
        portLibrary->filestream_write(portLibrary, fileStream, from, remaining);
    if (rc < 0) {
      result = rc;
      break;
    }
