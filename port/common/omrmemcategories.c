...
extern uintptr_t
compareAndSwapUDATA(uintptr_t* location,
                    uintptr_t oldValue,
                    uintptr_t newValue);
...
omrmem_categories_increment_counters(OMRMemCategory* category, uintptr_t size)
{
  ...
  do {
    oldValue = category->liveAllocations;
             ...
             &category->liveAllocations, oldValue, oldValue + 1) != oldValue);
...
omrmem_categories_increment_bytes(OMRMemCategory* category, uintptr_t size)
{
  ...
  do {
    oldValue = category->liveBytes;
             ...
             &category->liveBytes, oldValue, oldValue + size) != oldValue);
}
                                  ...
                                  OMRMemCategory* parent)
{
  ...
  for (i = 0; i < parent->numberOfChildren; i++) {
    ...
    result = state->walkFunction(child->categoryCode,
                                 child->name,
                                 child->liveBytes,
                                 child->liveAllocations,
                                 FALSE,
                                 parent->categoryCode,
                                 state);
                              ...
                              OMRMemCategory* walkPoint)
{
  ...
  result = state->walkFunction(walkPoint->categoryCode,
                               walkPoint->name,
                               walkPoint->liveBytes,
                               walkPoint->liveAllocations,
                               TRUE,
                               0,
                               state);
                       ...
                       OMRMemCategoryWalkState* state)
{
  ...
  if (portLibrary->portGlobals->control.language_memory_categories.categories !=
      NULL) {
    _recursive_category_walk_root(portLibrary,
                                  state,
                                  portLibrary->portGlobals->control
                                    .language_memory_categories.categories[0]);
  } else {
    uintptr_t result = _recursive_category_walk_root(
      portLibrary, state, &portLibrary->portGlobals->portLibraryMemoryCategory);
    if (result != J9MEM_CATEGORIES_KEEP_ITERATING) {
      return;
    }
...
omrmem_startup_categories(struct OMRPortLibrary* portLibrary)
{
  memcpy(&portLibrary->portGlobals->unknownMemoryCategory,
         CATEGORY_TABLE_ENTRY(OMRMEM_CATEGORY_UNKNOWN),
