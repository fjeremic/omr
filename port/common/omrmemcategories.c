...
extern uintptr_t compareAndSwapUDATA(uintptr_t *location, uintptr_t oldValue,
                                     uintptr_t newValue);
#endif /* _J9VMATOMICFUNCTIONS_ */

/* Templates for categories that are copied into malloc'd memory in
 * omrmem_startup_categories */
OMRMEM_CATEGORY_NO_CHILDREN("Unknown", OMRMEM_CATEGORY_UNKNOWN);

#if defined(OMR_ENV_DATA64)
/* On 64 bit we define an additional special category to cover unused regions of
 * allocate32 slabs. */
OMRMEM_CATEGORY_NO_CHILDREN(
    "Unused <32bit allocation regions",
    OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS);
OMRMEM_CATEGORY_1_CHILD("Port Library", OMRMEM_CATEGORY_PORT_LIBRARY,
                        OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS);
#else
 ...
 */
void omrmem_categories_increment_counters(OMRMemCategory *category,
                                          uintptr_t size) {
  uintptr_t oldValue;

  Trc_Assert_PTR_mem_categories_increment_counters_NULL_category(NULL !=
                                                                 category);

  /* Increment block count */
  do {
    oldValue = category->liveAllocations;
  } while (compareAndSwapUDATA(&category->liveAllocations, oldValue,
                               oldValue + 1) != oldValue);
 ...
 */
void omrmem_categories_increment_bytes(OMRMemCategory *category,
                                       uintptr_t size) {
  uintptr_t oldValue;
  ...
  } while (compareAndSwapUDATA(&category->liveBytes, oldValue,
                               oldValue + size) != oldValue);
}
 ...
 */
void omrmem_categories_decrement_counters(OMRMemCategory *category,
                                          uintptr_t size) {
  uintptr_t oldValue;

  Trc_Assert_PTR_mem_categories_decrement_counters_NULL_category(NULL !=
                                                                 category);
 ...
 * Used in omrmem32helpers to manage <32 bit slab regions
 */
void omrmem_categories_decrement_bytes(OMRMemCategory *category,
                                       uintptr_t size) {
  uintptr_t oldValue;
...
/**
 * Returns a reference to the OMRMemCategory structure represented by
 * categoryCode.
 *
 ...
 */
OMRMemCategory *omrmem_get_category(struct OMRPortLibrary *portLibrary,
  ...
  if (categoryCode < OMRMEM_LANGUAGE_CATEGORY_LIMIT) {
    if (categoryCode <
            portControl->language_memory_categories.numberOfCategories &&
        NULL !=
            portControl->language_memory_categories.categories[categoryCode]) {
  ...
  /**
   * Nothing past here should be called after port control has intialised the
   * categories, unless someone uses a bad category code. PORT_LIBRARY and
   * ALLOCATE32 are inserted in the category tables but the port library may use
   * them before port control is called. The hot path should be above here.
   */
...
static uintptr_t
_recursive_category_walk_children(struct OMRPortLibrary *portLibrary,
                                  OMRMemCategoryWalkState *state,
                                  OMRMemCategory *parent) {
  uint32_t i;
  uintptr_t result;
  ...
  for (i = 0; i < parent->numberOfChildren; i++) {
    ...
    result = state->walkFunction(child->categoryCode, child->name,
                                 child->liveBytes, child->liveAllocations,
                                 FALSE, parent->categoryCode, state);
...
static uintptr_t
_recursive_category_walk_root(struct OMRPortLibrary *portLibrary,
                              OMRMemCategoryWalkState *state,
                              OMRMemCategory *walkPoint) {
  uintptr_t result;

  result = state->walkFunction(walkPoint->categoryCode, walkPoint->name,
                               walkPoint->liveBytes, walkPoint->liveAllocations,
                               TRUE, 0, state);
 ...
 *
 */
void omrmem_walk_categories(struct OMRPortLibrary *portLibrary,
                            OMRMemCategoryWalkState *state) {
  /* User supplied categories are expected to include PORT_LIBRARY and UNKNOWN
   * as part of their tree */
  if (portLibrary->portGlobals->control.language_memory_categories.categories !=
      NULL) {
    _recursive_category_walk_root(
        portLibrary, state,
        portLibrary->portGlobals->control.language_memory_categories
            .categories[0]);
  } else {
    uintptr_t result = _recursive_category_walk_root(
        portLibrary, state,
    ...
    result = _recursive_category_walk_root(
        portLibrary, state, &portLibrary->portGlobals->unknownMemoryCategory);
    if (result != J9MEM_CATEGORIES_KEEP_ITERATING) {
      return;
    }
 ...
 *
 * @note Sets up categories for "Unknown" and "Port Library". Others will be
 * supplied by the application.
 *
 */
int32_t omrmem_startup_categories(struct OMRPortLibrary *portLibrary) {
  memcpy(&portLibrary->portGlobals->unknownMemoryCategory,
         CATEGORY_TABLE_ENTRY(OMRMEM_CATEGORY_UNKNOWN), sizeof(OMRMemCategory));
  memcpy(&portLibrary->portGlobals->portLibraryMemoryCategory,
         CATEGORY_TABLE_ENTRY(OMRMEM_CATEGORY_PORT_LIBRARY),
         sizeof(OMRMemCategory));
#if defined(OMR_ENV_DATA64)
  memcpy(&portLibrary->portGlobals->unusedAllocate32HeapRegionsMemoryCategory,
         CATEGORY_TABLE_ENTRY(
             OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS),
         sizeof(OMRMemCategory));
#endif
  portLibrary->portGlobals->control.language_memory_categories
      .numberOfCategories = 0;
  portLibrary->portGlobals->control.language_memory_categories.categories =
      NULL;
  portLibrary->portGlobals->control.omr_memory_categories.numberOfCategories =
      0;
...
void omrmem_shutdown_categories(struct OMRPortLibrary *portLibrary) {
  ...
  if (NULL !=
      portLibrary->portGlobals->control.language_memory_categories.categories) {
    portLibrary->mem_free_memory(OMRPORTLIB,
                                 portLibrary->portGlobals->control
                                     .language_memory_categories.categories);
    portLibrary->portGlobals->control.language_memory_categories.categories =
        NULL;
    portLibrary->portGlobals->control.language_memory_categories
        .numberOfCategories = 0;
  ...
  if (NULL !=
      portLibrary->portGlobals->control.omr_memory_categories.categories) {
