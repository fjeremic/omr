 ...
 */
int32_t omrport_init_library(struct OMRPortLibrary *portLibrary,
                             uintptr_t size) {
 ...
 *
 * Shutdown the port library, de-allocate resources required by the components
 * of the portlibrary. Any resources that were created by @ref
 * omrport_startup_library should be destroyed here.
 *
 * @param[in] portLibrary The portlibrary.
 *
 * @return 0 on success, negative return code on failure
 */
int32_t omrport_shutdown_library(struct OMRPortLibrary *portLibrary) {
  ...
  /* Shutdown the socket library before the tty, so it can write to the tty if
   * required */
...
int32_t omrport_create_library(struct OMRPortLibrary *portLibrary,
                               uintptr_t size) {
 ...
 * PortLibrary startup.
 *
 * Start the port library, allocate resources required by the components of the
 * portlibrary. All resources created here should be destroyed in @ref
 * omrport_shutdown_library.
 *
 * @param[in] portLibrary The portlibrary.
 *
 * @return 0 on success, negative error code on failure.  Error code values
 * returned are \arg OMRPORT_ERROR_STARTUP_THREAD \arg OMRPORT_ERROR_STARTUP_MEM
 ...
 *
 * @note The port library memory is deallocated if it was created by @ref
 * omrport_allocate_library
 */
int32_t omrport_startup_library(struct OMRPortLibrary *portLibrary) {
  int32_t rc = 0;
...
cleanup:
   ...
   *  safe if the corresponding startup was not called.  No worse than the
   * existing code
   */
 ...
 *
 * @note The portlibrary version must be compatible with the portlibrary that
 * which we are compiled against
 */
uintptr_t omrport_getSize(void) { return sizeof(OMRPortLibrary); }

/**
 * Query the port library.
 *
 * Given a pointer to the port library and an offset into the table determine if
 * the function at that offset has been overridden from the default value
 * expected by J9.
 *
 * @param[in] portLibrary The port library.
 ...
 *
 * omrport_isFunctionOverridden(portLibrary, offsetof(OMRPortLibrary,
 * mem_allocate_memory));
 */
int32_t omrport_isFunctionOverridden(struct OMRPortLibrary *portLibrary,
                                     uintptr_t offset) {
  ...
  return *((uintptr_t *)&(((uint8_t *)portLibrary)[offset])) !=
         *((uintptr_t *)&(((uint8_t *)&MasterPortLibraryTable)[offset]));
}
/**
 * Allocate a port library.
 *
 * Given a pointer to the port library, allocate and initialize the structure.
 * The startup function is not called (@ref omrport_startup_library) allowing
 * the application to override any functions they desire.  In the event @ref
 * omrport_startup_library fails when called by the application the port library
 ...
 * @note @ref omrport_shutdown_library will deallocate this memory as part of
 * regular shutdown
 */
int32_t omrport_allocate_library(struct OMRPortLibrary **portLibrary) {
