...
char* read_from_catalog(struct OMRPortLibrary* portLibrary,
                        intptr_t fd,
                        char* buf,
                        intptr_t bufsize);
...
static void writeSyslog(struct OMRPortLibrary* portLibrary,
                        uintptr_t flags,
                        const char* format,
                        va_list args);
 ...
 * @deprecated NLS API is deprecated.
 */
void j9nls_set_locale(struct OMRPortLibrary* portLibrary,
                      const char* lang,
                      const char* region,
                      const char* variant) {
  J9NLSDataCache* nls;
 ...
 * database.
 */
void j9nls_printf(struct OMRPortLibrary* portLibrary,
                  uintptr_t flags,
                  uint32_t module_name,
                  uint32_t message_num,
                  ...) {
 ...
 * database.
 */
void j9nls_vprintf(struct OMRPortLibrary* portLibrary,
                   uintptr_t flags,
                   uint32_t module_name,
                   uint32_t message_num,
                   va_list args) {
 ...
 * @deprecated NLS API is deprecated.
 */
void j9nls_set_catalog(struct OMRPortLibrary* portLibrary,
                       const char** paths,
                       const int nPaths,
                       const char* baseName,
...
#define MSG_IGNORE 5
  ...
  if (nls->catalog) {
    fd =
        portLibrary->file_open(portLibrary, (char*)nls->catalog, EsOpenRead, 0);
  }
  if (fd == -1) {
