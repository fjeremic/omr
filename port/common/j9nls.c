...
static char *build_catalog_name(struct OMRPortLibrary *portLibrary,
                                int32_t usePath, int32_t useDepth);
static void free_catalog(struct OMRPortLibrary *portLibrary);
char *read_from_catalog(struct OMRPortLibrary *portLibrary, intptr_t fd,
                        char *buf, intptr_t bufsize);
static void open_catalog(struct OMRPortLibrary *portLibrary);

static const char *nlsh_lookup(struct OMRPortLibrary *portLibrary,
                               uint32_t module_name, uint32_t message_num);
static J9NLSHashEntry *nls_allocateHashEntry(struct OMRPortLibrary *portLibrary,
                                             uint32_t module_name,
                                             uint32_t message_num,
                                             const char *message,
                                             uint32_t sizeOfMessage);
static const char *parse_catalog(struct OMRPortLibrary *portLibrary,
                                 uintptr_t flags, uint32_t module_name,
                                 uint32_t message_num,
                                 const char *default_string);
static void nlsh_insert(struct OMRPortLibrary *portLibrary,
                        J9NLSHashEntry *entry);
static void writeSyslog(struct OMRPortLibrary *portLibrary, uintptr_t flags,
 ...
 * @param[in] lang - the language of the locale (e.g., "en"), 2 characters or
 * less
 * @param[in] region - the region of the locale (e.g., "US"), 2 characters or
 * less
 * @param[in] variant - the variant of the locale (e.g., "boont"), 31 characters
 * or less
 *
 * @deprecated NLS API is deprecated.
 */
void j9nls_set_locale(struct OMRPortLibrary *portLibrary, const char *lang,
 ...
 * @deprecated NLS API is deprecated.
 */
const char *j9nls_get_language(struct OMRPortLibrary *portLibrary) {
  J9NLSDataCache *nls;
#if defined(NLS_DEBUG_TRACE)
 ...
 * @deprecated NLS API is deprecated.
 */
const char *j9nls_get_region(struct OMRPortLibrary *portLibrary) {
  J9NLSDataCache *nls;
#if defined(NLS_DEBUG_TRACE)
 ...
 * @deprecated NLS API is deprecated.
 */
const char *j9nls_get_variant(struct OMRPortLibrary *portLibrary) {
  J9NLSDataCache *nls;
#if defined(NLS_DEBUG_TRACE)
 ...
 *
 * @param[in] portLibrary The port library
 * @param[in] flags - to indicate what type of message (e.g., ERROR) and whether
...
static void writeSyslog(struct OMRPortLibrary *portLibrary, uintptr_t flags,
  ...
  /* What is size of buffer required ? str_vprintf(..,NULL,..) result includes
   * the null terminator */
 ...
 * database.
 */
void j9nls_printf(struct OMRPortLibrary *portLibrary, uintptr_t flags,
                  uint32_t module_name, uint32_t message_num, ...) {
  va_list args;
#if defined(NLS_DEBUG_TRACE)
 ...
 * database.
 */
void j9nls_vprintf(struct OMRPortLibrary *portLibrary, uintptr_t flags,
  ...
  message = portLibrary->nls_lookup_message(portLibrary, flags, module_name,
                                            message_num, NULL);
  ...
  /* If tracepoint enabled, mirror NLS message to tracepoint omrport.657,
   * tracepoint group omrport{nlsmessage] */
  if (TrcEnabled_Trc_PRT_j9nls_vprintf) {
     ...
     * Uses simple buffer for message text, limit of 1024 characters. NLS
     * messages are typically less than 80 characters.
     */
    COPY_VA_LIST(argsForTrace, args);
    if (sizeof(buffer) == portLibrary->str_vprintf(portLibrary, buffer,
                                                   sizeof(buffer), message,
                                                   argsForTrace)) {
      /* str_vprintf will fill buffer with no null terminator if message is
       * longer than buffer length -- add null terminator */
...
/**
 * Return the NLS string for the module name and message number.  If no string
 * is found, or a failure occurs, return the default_string.
 *
 * @param[in] portLibrary The port library
 * @param[in] flags - to indicate what type of message (e.g., ERROR) and whether
 * a newline is required
 * @param[in] module_name - the module identifier of the NLS message
 * @param[in] message_num - the NLS message number within the module
 * @param[in] default_string - a default message, in case no NLS message is
 * found
 *
 * @return NLS String
 *
 * @deprecated NLS API is deprecated.
 */
const char *j9nls_lookup_message(struct OMRPortLibrary *portLibrary,
                                 uintptr_t flags, uint32_t module_name,
                                 uint32_t message_num,
                                 const char *default_string) {
  const char *message;
  ...
  if (!message) {
    message = parse_catalog(portLibrary, flags, module_name, message_num,
                            default_string);
    if (!message) {
 ...
 *
 * @param[in] portLibrary The port library
 * @param[in] paths - an array of directory paths where the NLS catalog may be
 * found
 * @param[in] nPaths - the number of entries in the @ref paths array
 * @param[in] baseName - the lead name of the catalog file name (i.e., the
 * "java" in java_en_US.properties)
 * @param[in] extension - the extension of the catalog file name (i.e., the
 * "properties in java_en_US.properties)
 *
 * @deprecated NLS API is deprecated.
 */
void j9nls_set_catalog(struct OMRPortLibrary *portLibrary, const char **paths,
  ...
  for (i = 0; i < nPaths; i++) {
    nls->baseCatalogPaths[i] = portLibrary->mem_allocate_memory(
        portLibrary, strlen(paths[i]) + 1, OMR_GET_CALLSITE(),
        OMRMEM_CATEGORY_PORT_LIBRARY);
    if (nls->baseCatalogPaths[i]) {
  ...
  nls->baseCatalogName = portLibrary->mem_allocate_memory(
      portLibrary, strlen(baseName) + 1, OMR_GET_CALLSITE(),
      OMRMEM_CATEGORY_PORT_LIBRARY);
  if (nls->baseCatalogName) {
  ...
  nls->baseCatalogExtension = portLibrary->mem_allocate_memory(
      portLibrary, strlen(extension) + 1, OMR_GET_CALLSITE(),
      OMRMEM_CATEGORY_PORT_LIBRARY);
  if (nls->baseCatalogExtension) {
...
static char *build_catalog_name(struct OMRPortLibrary *portLibrary,
  ...
  if (!nls->nPaths) {
    portLibrary->nls_set_catalog(portLibrary, (const char **)&defaultCatalog, 1,
                                 "java", "properties");
    if (!nls->baseCatalogName) {
      goto _done;
    }
  ...
  catalog = portLibrary->mem_allocate_memory(
...
static void open_catalog(struct OMRPortLibrary *portLibrary) {
   ...
   * The base name will typically be "<path>/java.properties". Append as many
   *locale descriptors to the file name as possible to find a file. e.g.
...
#define MSG_IGNORE 5
  ...
  /* calculate a size which is larger than we could possibly need by putting
   * together all of the prefixes and suffixes */
  char prefix[sizeof(J9NLS_ERROR_PREFIX
                     "" J9NLS_INFO_PREFIX "" J9NLS_WARNING_PREFIX
                     "" J9NLS_COMMON_PREFIX "" J9NLS_EXEMPLAR
                     "" J9NLS_ERROR_SUFFIX "" J9NLS_INFO_SUFFIX
                     "" J9NLS_WARNING_SUFFIX " \n")];
  char *searchKey;
  ...
  if (0 == (flags & J9NLS_DO_NOT_PRINT_MESSAGE_TAG)) {
    if (flags & J9NLS_ERROR) {
      format = "" J9NLS_ERROR_PREFIX "" J9NLS_COMMON_PREFIX
               "%s%03u" J9NLS_ERROR_SUFFIX " %s";
    } else if (flags & J9NLS_WARNING) {
      format = "" J9NLS_WARNING_PREFIX "" J9NLS_COMMON_PREFIX
               "%s%03u" J9NLS_WARNING_SUFFIX " %s";
    } else if (flags & J9NLS_INFO) {
      format = "" J9NLS_INFO_PREFIX "" J9NLS_COMMON_PREFIX
               "%s%03u" J9NLS_INFO_SUFFIX " %s";
    }
  }
  ...
  if (nls->catalog) {
    fd = portLibrary->file_open(portLibrary, (char *)nls->catalog, EsOpenRead,
                                0);
  }
  if (fd == -1) {
    ...
    entry = nls_allocateHashEntry(portLibrary, module_name, message_num, tmpStr,
                                  (uint32_t)strlen(tmpStr));
    if (!entry) {
      return default_string;
    }
  ...
  while (read_from_catalog(portLibrary, fd, (char *)dataBuf, BUF_SIZE) !=
    ...
    while (charPointer < endPointer) {
      ...
      if (offset + 2 >= bufSize) {
        bufSize <<= 1;
        if (!(newBuf = portLibrary->mem_allocate_memory(
                  portLibrary, bufSize, OMR_GET_CALLSITE(),
                  OMRMEM_CATEGORY_PORT_LIBRARY))) {
          goto finished;
        }
      ...
      if (mode == MSG_SLASH) {
          ...
          if (firstChar) {
            ...
            while (1) {
              if (charPointer >= endPointer) {
                if (read_from_catalog(portLibrary, fd, (char *)dataBuf,
                                      BUF_SIZE) != NULL) {
                  charPointer = dataBuf;
        ...
        makeStrings:
          if (keyLength >= 0) {
            ...
            if (strncmp(searchKey, buf, sizeof(J9NLS_EXEMPLAR) - 1) == 0) {
              ...
              if (flags & J9NLS_DO_NOT_PRINT_MESSAGE_TAG) {
                ...
                entry = nls_allocateHashEntry(
                    portLibrary, module_name, message_num, prefix,
          ...
          finished:
            ...
            if (!entry) {
              ...
              entry =
                  nls_allocateHashEntry(portLibrary, module_name, message_num,
                                        tmpStr, (uint32_t)strlen(tmpStr));
              if (!entry) {
                portLibrary->file_close(portLibrary, fd);
                return default_string;
              }
        ...
        if ((nextChar >= 0x1c && nextChar <= ' ') ||
          ...
          if (offset == 0 || offset == (uint32_t)keyLength ||
              mode == MSG_IGNORE) {
            continue;
          }
          if (keyLength == -1) { /* if parsing the key */
 ...
 * that are required for the NLS library operations may be created here.  All
 * resources created here should be destroyed in @ref j9nls_shutdown.
 *
 * @param[in] portLibrary The port library
 *
 * @return 0 on success, negative error code on failure.  Error code values
 * returned are \arg OMRPORT_ERROR_STARTUP_NLS
 *
 * @deprecated NLS API is deprecated.
 */
int32_t j9nls_startup(struct OMRPortLibrary *portLibrary) {
  J9NLSDataCache *nls;
  ...
  if (0 !=
      omrthread_monitor_init_with_name(&nls->monitor, 0, "NLS hash table")) {
    return (int32_t)OMRPORT_ERROR_STARTUP_NLS;
  }
...
/**
 * Free dynamically cached data (allocated and cached since @ref j9nls_startup
 * was called).  This should be called whenever the port library memory
 * allocation routines are changed, to ensure that allocation and deallocation
 * routines are correctly paired ( @ref omrmem_allocate_memory and @ref
 * omrmem_free_memory ).
 *
 * @param[in] portLibrary The port library
 *
 * @deprecated NLS API is deprecated.
 */
void j9nls_free_cached_data(struct OMRPortLibrary *portLibrary) {
  J9NLSHashEntry *entry;
  uint32_t i;
  J9NLSDataCache *nls;
 ...
 * @ref j9nls_startup should be destroyed here.  Any dynamically cached data
 * should also be freed here.  This function must be called only once.
 *
 * @param[in] portLibrary The port library
 *
 * @deprecated NLS API is deprecated.
 */
void j9nls_shutdown(struct OMRPortLibrary *portLibrary) {
  uint32_t i;
  J9NLSDataCache *nls;
...
static J9NLSHashEntry *nls_allocateHashEntry(struct OMRPortLibrary *portLibrary,
                                             ...
                                             const char *message,
                                             uint32_t sizeOfMessage) {
  J9NLSHashEntry *entry = portLibrary->mem_allocate_memory(
      portLibrary,
      sizeof(J9NLSHashEntry) + sizeOfMessage + 1 - sizeof(entry->message),
...
#pragma convlit(suspend)
  converter =
      iconv_get(portLibrary, J9NLS_ICONV_DESCRIPTOR, "UTF-8", "IBM-1047");
