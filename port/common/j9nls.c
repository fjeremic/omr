...
static char*
build_catalog_name(struct OMRPortLibrary* portLibrary,
                   int32_t usePath,
                   int32_t useDepth);
static void
free_catalog(struct OMRPortLibrary* portLibrary);
char*
read_from_catalog(struct OMRPortLibrary* portLibrary,
                  intptr_t fd,
                  char* buf,
                  intptr_t bufsize);
static void
...
static const char*
nlsh_lookup(struct OMRPortLibrary* portLibrary,
            uint32_t module_name,
            uint32_t message_num);
static J9NLSHashEntry*
nls_allocateHashEntry(struct OMRPortLibrary* portLibrary,
                      uint32_t module_name,
                      uint32_t message_num,
                      const char* message,
                      uint32_t sizeOfMessage);
static const char*
parse_catalog(struct OMRPortLibrary* portLibrary,
              uintptr_t flags,
              uint32_t module_name,
              uint32_t message_num,
...
static void
writeSyslog(struct OMRPortLibrary* portLibrary,
            uintptr_t flags,
            const char* format,
            va_list args);
static void
              ...
              va_list args)
{
  ...
  message = portLibrary->nls_lookup_message(
    portLibrary, flags, module_name, message_num, NULL);

#if defined(NLS_DEBUG)
  portLibrary->tty_printf(
    portLibrary, "NLS - vprintf - message: %s\n", message);
#endif
  ...
  if (TrcEnabled_Trc_PRT_j9nls_vprintf) {
    ...
    if (sizeof(buffer) ==
        portLibrary->str_vprintf(
          portLibrary, buffer, sizeof(buffer), message, argsForTrace)) {
                     ...
                     const char* default_string)
{
  ...
  if (!message) {
    message = parse_catalog(
      portLibrary, flags, module_name, message_num, default_string);
    if (!message) {
                   ...
                   int32_t useDepth)
{
  ...
  if (!nls->nPaths) {
    portLibrary->nls_set_catalog(
      portLibrary, (const char**)&defaultCatalog, 1, "java", "properties");
    if (!nls->baseCatalogName) {
      goto _done;
    }
    if (nls->language[0] == 0 && nls->region[0] == 0 && nls->variant[0] == 0) {
...
#if defined(NLS_DEBUG)
      portLibrary->tty_printf(
        portLibrary, "NLS - attempting to open: %s\n", catalog);
#endif
...
#if defined(NLS_DEBUG)
  portLibrary->tty_printf(
    portLibrary, "NLS - succesfully opened %s\n", catalog);
#endif
...
#define MSG_IGNORE 5
  ...
  char
    prefix[sizeof(J9NLS_ERROR_PREFIX
                  "" J9NLS_INFO_PREFIX "" J9NLS_WARNING_PREFIX
                  "" J9NLS_COMMON_PREFIX "" J9NLS_EXEMPLAR "" J9NLS_ERROR_SUFFIX
                  "" J9NLS_INFO_SUFFIX "" J9NLS_WARNING_SUFFIX " \n")];
  char* searchKey;
...
#if defined(NLS_DEBUG)
  portLibrary->tty_printf(
    portLibrary, "NLS - parse_catalog - searchKey: %s\n", searchKey);
#endif
  ...
  if (fd == -1) {
    ...
    entry = nls_allocateHashEntry(
      portLibrary, module_name, message_num, tmpStr, (uint32_t)strlen(tmpStr));
    if (!entry) {
      return default_string;
    }
  ...
  if (!(buf = portLibrary->mem_allocate_memory(portLibrary,
                                               bufSize,
                                               OMR_GET_CALLSITE(),
                                               OMRMEM_CATEGORY_PORT_LIBRARY))) {
    goto finished;
              ...
              while (1) {
                if (charPointer >= endPointer) {
                  if (read_from_catalog(
                        portLibrary, fd, (char*)dataBuf, BUF_SIZE) != NULL) {
                    charPointer = dataBuf;
            ...
            if (keyLength >= 0) {
              ...
              if (strncmp(searchKey, buf, sizeof(J9NLS_EXEMPLAR) - 1) == 0) {
                ...
                if (flags & J9NLS_DO_NOT_PRINT_MESSAGE_TAG) {
                  entry = nls_allocateHashEntry(portLibrary,
                                                module_name,
                                                message_num,
                                                buf + keyLength,
                                                offset - keyLength + 1);
                  ...
                  if (entry) {
                    ...
                    strncat(
                      entry->message, buf + keyLength, offset - keyLength);
                    if (newline) {
                      strcat(entry->message, "\n");
                    }
                  }
