 ...
 * @deprecated NLS API is deprecated.
 */
void j9nls_set_locale(struct OMRPortLibrary* portLibrary, const char* lang, const char* region, const char* variant)
{
  J9NLSDataCache* nls;
 ...
 */
void j9nls_printf(struct OMRPortLibrary* portLibrary, uintptr_t flags, uint32_t module_name, uint32_t message_num, ...)
{
  va_list args;
#if defined(NLS_DEBUG_TRACE)
  ...
  struct OMRPortLibrary* portLibrary, uintptr_t flags, uint32_t module_name, uint32_t message_num, va_list args)
{
  ...
  if (TrcEnabled_Trc_PRT_j9nls_vprintf) {
    ...
    if (sizeof(buffer) == portLibrary->str_vprintf(portLibrary, buffer, sizeof(buffer), message, argsForTrace)) {
      /* str_vprintf will fill buffer with no null terminator if message is longer than buffer length -- add null
       * terminator */
 ...
 * @deprecated NLS API is deprecated.
 */
const char* j9nls_lookup_message(struct OMRPortLibrary* portLibrary, uintptr_t flags, uint32_t module_name,
  uint32_t message_num, const char* default_string)
{
  const char* message;
  J9NLSDataCache* nls;
 ...
 * @deprecated NLS API is deprecated.
 */
void j9nls_set_catalog(
  struct OMRPortLibrary* portLibrary, const char** paths, const int nPaths, const char* baseName, const char* extension)
{
  int i;
  char* p;
  J9NLSDataCache* nls;
...
static void open_catalog(struct OMRPortLibrary* portLibrary)
{
   ...
   * The base name will typically be "<path>/java.properties". Append as many locale descriptors to the file name as
   *possible to find a file. e.g. java_en_US_Texas.properties java_en_US.properties java_en.properties java.properties
   */
...
#define MSG_IGNORE 5
  ...
  if (!(buf
        = portLibrary->mem_allocate_memory(portLibrary, bufSize, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY))) {
    goto finished;
  }
        ...
        makeStrings:
          if (keyLength >= 0) {
            ...
            portLibrary->tty_printf(
              portLibrary, "NLS - parse_catalog - keyLength: %d -- buf: %20.20s\n", keyLength, buf);
#endif
            if (strncmp(searchKey, buf, sizeof(J9NLS_EXEMPLAR) - 1) == 0) {
              ...
              if (flags & J9NLS_DO_NOT_PRINT_MESSAGE_TAG) {
                entry = nls_allocateHashEntry(
                  portLibrary, module_name, message_num, buf + keyLength, offset - keyLength + 1);
                if (entry) {
                ...
                entry = nls_allocateHashEntry(
                  portLibrary, module_name, message_num, prefix, offset - keyLength + (uint32_t)strlen(prefix));
                if (entry) {
...
{
  J9NLSHashEntry* entry
    = portLibrary->mem_allocate_memory(portLibrary, sizeof(J9NLSHashEntry) + sizeOfMessage + 1 - sizeof(entry->message),
...
#if defined(NLS_DEBUG)
  portLibrary->tty_printf(
    portLibrary, "NLS - nls_allocateHashEntry - message: %s - sizeOfMessage: %d\n", entry->message, sizeOfMessage);
#endif
  return entry;
}
static void convertModuleName(uint32_t module_name, uint8_t* module_str)
