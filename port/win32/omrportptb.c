...
/* These functions don't belong here, but they don't belong anywhere else
 * either. */
/**
 * @internal
 ...
 */
wchar_t *port_convertFromUTF8(OMRPortLibrary *portLibrary, const char *string,
  ...
  if (length < unicodeBufferSize) {
    ...
    unicodeString = portLibrary->mem_allocate_memory(
        portLibrary, (length + 1) * 2, OMR_GET_CALLSITE(),
  ...
  if (0 == MultiByteToWideChar(OS_ENCODING_CODE_PAGE, OS_ENCODING_MB_FLAGS,
    ...
    portLibrary->error_set_last_error(portLibrary, GetLastError(),
 ...
 */
int32_t port_convertToUTF8(OMRPortLibrary *portLibrary,
                           const wchar_t *unicodeString, char *utf8Buffer,
                           uintptr_t size) {
  if (0 == WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS,
                               unicodeString, -1, utf8Buffer, (int)size, NULL,
                               NULL)) {
    portLibrary->error_set_last_error(portLibrary, GetLastError(),
                                      OMRPORT_ERROR_OPFAILED); /* continue */
    return -1;
  }
  return 0;
}
