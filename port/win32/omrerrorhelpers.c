...
const char* errorMessage(struct OMRPortLibrary* portLibrary, int32_t errorCode)
{
  ...
  if (rc == 0) {
    const char* format;
    format = portLibrary->nls_lookup_message(portLibrary, J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
      J9NLS_PORT_ERROR_OPERATION_FAILED, "Operation Failed: %d (%s failed: %d)");
    portLibrary->str_printf(
      portLibrary, message, ptBuffers->errorMessageBufferSize, format, errorCode, "FormatMessage", GetLastError());
