...
const char* errorMessage(struct OMRPortLibrary* portLibrary,
  ...
  rc = FormatMessageW(
      ...
      errorCode, 0, ubuffer, J9ERROR_DEFAULT_BUFFER_SIZE, NULL);
  if (rc == 0) {
    const char* format;
    format = portLibrary->nls_lookup_message(
        portLibrary,
  ...
  out = portLibrary->str_printf(portLibrary, message,
                                ptBuffers->errorMessageBufferSize, "(%d) ",
                                errorCode);
  for (i = 0; i < rc; i++) {
  ...
  /* There may be extra spaces at the end of the message, due to stripping of
   * the LF it is replaced by a space.  Multi-line OS messages are thus one long
   * continuous line for us
   */
