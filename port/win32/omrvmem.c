...
/* VMDESIGN 1778 : always allocate using the MEM_TOP_DOWN flag to avoid
 * competition for the low memory area */
...
/**
 * The structure stored in the bindingTree to describe NUMA-bound memory
 * extents.
 */
typedef struct BindingNode {
  J9AVLTreeNode avlTree; /**< This struct needs to be embedded so that the
                            struct can act like a proper AVL node */
  ...
  uintptr_t j9NodeNumber; /**< The node number (in our internal node numbering
                             scheme (1-indexed)) */
} BindingNode;

static void *getMemoryInRange(struct OMRPortLibrary *portLibrary,
                              struct J9PortVmemIdentifier *identifier,
                              DWORD allocationFlags, DWORD protection,
                              OMRMemCategory *category, uintptr_t byteAmount,
                              void *startAddress, void *endAddress,
                              uintptr_t alignmentInBytes, uintptr_t vmemOptions,
                              uintptr_t mode);
...
void update_vmemIdentifier(J9PortVmemIdentifier *identifier, void *address,
                           void *handle, uintptr_t byteAmount, uintptr_t mode,
                           uintptr_t pageSize, uintptr_t pageFlags,
                           OMRMemCategory *category);
static void *commitAndTouch(struct OMRPortLibrary *portLibrary,
                            J9PortVmemIdentifier *identifier, void *address,
                            uintptr_t byteAmount);
static void touchOnNumaNode(struct OMRPortLibrary *portLibrary,
                            uintptr_t pageSizeInBytes, void *address,
                            uintptr_t byteAmount, uintptr_t j9NumaNode);
static intptr_t rangeInsertionComparator(J9AVLTree *tree,
                                         BindingNode *insertNode,
                                         BindingNode *walkNode);
static intptr_t rangeSearchComparator(J9AVLTree *tree, uintptr_t value,
                                      BindingNode *searchNode);
static int32_t getProcessPrivateMemorySize(struct OMRPortLibrary *portLibrary,
 ...
 */
void *omrvmem_commit_memory(struct OMRPortLibrary *portLibrary, void *address,
                            uintptr_t byteAmount,
                            struct J9PortVmemIdentifier *identifier) {
  void *ptr = NULL;
 ...
 *
 * Decommits physical storage of the size specified starting at the address
 * specified.
 *
 * @param[in] portLibrary The port library.
 ...
 */
intptr_t omrvmem_decommit_memory(struct OMRPortLibrary *portLibrary,
                                 void *address, uintptr_t byteAmount,
                                 struct J9PortVmemIdentifier *identifier) {
  intptr_t ret = -1;
  ...
  if (portLibrary->portGlobals->vmemAdviseOSonFree == 1) {
    if (rangeIsValid(identifier, address, byteAmount)) {
      ...
      if (byteAmount > 0) {
        if (PPG_vmem_pageSize[0] == identifier->pageSize) {
          ret = (intptr_t)VirtualFree((LPVOID)address, (size_t)byteAmount,
                                      MEM_DECOMMIT);
          if (ret != 0) {
            ...
            ret = -1;
            Trc_PRT_vmem_omrvmem_decommit_memory_failure(GetLastError(),
                                                         address, byteAmount);
          }
        } else {
          ret = 0;
 ...
 *
 * Frees physical storage of the size specified starting at the address
 * specified.
 *
 * @param[in] portLibrary The port library.
 ...
 * @return 0 on success, non zero on failure.
 */
int32_t omrvmem_free_memory(struct OMRPortLibrary *portLibrary, void *address,
                            uintptr_t byteAmount,
                            struct J9PortVmemIdentifier *identifier) {
  int32_t ret = 0;
  ...
  if (ret != 0) {
    /* ensure that we also delete any NUMA binding extents which describe the
     * memory under this area */
    /* (note that the sub-allocator can cause us to shut-down the binding pool
     * prior to freeing the last reserved area so ignore NUMA clean-up if the
     * pool is already gone) */
    if (NULL != portLibrary->portGlobals->platformGlobals.bindingPool) {
      void *nextBaseToLookup = address;
      J9AVLTree *boundExtents =
          &portLibrary->portGlobals->platformGlobals.bindingTree;

      /* There is one case where the tree can have precisely zero entries -
       * failure in allocating the first node in the range. Otherwise, the
       * sequence of nodes should be complete and contiguous.
       */
      omrthread_monitor_enter(PPG_bindingAccessMonitor);
      if (NULL != (BindingNode *)avl_search(boundExtents,
                                            (uintptr_t)nextBaseToLookup)) {
        J9Pool *extentPool =
            portLibrary->portGlobals->platformGlobals.bindingPool;
        void *topOfExtent = (void *)((uintptr_t)address + byteAmount);

        while (nextBaseToLookup < topOfExtent) {
          BindingNode *node = (BindingNode *)avl_search(
              boundExtents, (uintptr_t)nextBaseToLookup);
          Assert_PRT_true(NULL != node);
          nextBaseToLookup = (void *)((uintptr_t)nextBaseToLookup +
                                      node->extentOfRangeInBytes);
...
int32_t omrvmem_vmem_params_init(struct OMRPortLibrary *portLibrary,
...
void *omrvmem_reserve_memory(struct OMRPortLibrary *portLibrary, void *address,
                             uintptr_t byteAmount,
                             struct J9PortVmemIdentifier *identifier,
...
void *omrvmem_reserve_memory_ex(struct OMRPortLibrary *portLibrary,
                                struct J9PortVmemIdentifier *identifier,
                                struct J9PortVmemParams *params) {
  ...
  printf(
      "\n\tomrvmem_reserve_memory_ex byteAmount: %p, startAddress: %p, "
      "endAddress: %p, pageSize: %p\n"
      " \t\t%s\n"
      " \t\t%s\n"
      " \t\t%s\n"
      " \t\t%s\n ",
      ...
      (OMRPORT_VMEM_STRICT_ADDRESS & params->options)
          ...
          : "\t",
      (OMRPORT_VMEM_ALLOC_DIR_BOTTOM_UP & params->options)
          ? "OMRPORT_VMEM_ALLOC_DIR_BOTTOM_UP"
          : "\t",
      (OMRPORT_VMEM_ALLOC_DIR_TOP_DOWN & params->options)
          ? "OMRPORT_VMEM_ALLOC_DIR_TOP_DOWN"
          : "\t",
      (OMRPORT_VMEM_ZOS_USE2TO32G_AREA & params->options)
          ? "OMRPORT_VMEM_ZOS_USE2TO32G_AREA"
          : "\t");
#endif

  Trc_PRT_vmem_omrvmem_reserve_memory_Entry(params->startAddress,
                                            params->byteAmount);
  ...
  if (0 == params->pageSize) {
    ...
    uintptr_t alignmentInBytes =
        OMR_MAX(params->pageSize, params->alignmentInBytes);
    uintptr_t minimumGranule =
        OMR_MIN(params->pageSize, params->alignmentInBytes);
    ...
    if (0 != (OMRPORT_VMEM_MEMORY_MODE_COMMIT & params->mode)) {
       ...
       * On Win32 the protection bits appear to be ignored for uncommitted
       * memory.
       */
      protection = PAGE_NOACCESS;
    }

    /* Make sure that the alignment is a multiple of both requested alignment
     * and page size (enforces that arguments are powers of two and, thus, their
     * max is their lowest common multiple) */
    if ((0 == minimumGranule) || (0 == (alignmentInBytes % minimumGranule))) {
      memoryPointer = getMemoryInRange(
          portLibrary, identifier, allocationFlags, protection, category,
          params->byteAmount, params->startAddress, params->endAddress,
    ...
    if (NULL == memoryPointer) {
      ...
      update_vmemIdentifier(identifier, (void *)memoryPointer,
                            (void *)memoryPointer, params->byteAmount,
                            params->mode, PPG_vmem_pageSize[0],
                            OMRPORT_VMEM_PAGE_FLAG_NOT_USED, category);
    }
    Trc_PRT_vmem_omrvmem_reserve_memory_Exit2(memoryPointer);
  } else if (PPG_vmem_pageSize[1] == params->pageSize) {
    uintptr_t largePageAlignmentInBytes =
        OMR_MAX(params->pageSize, params->alignmentInBytes);
    uintptr_t largePageMinimumGranule =
        OMR_MIN(params->pageSize, params->alignmentInBytes);
    ...
    allocationFlags |=
        MEM_LARGE_PAGES | MEM_COMMIT | MEM_RESERVE | J9_EXTRA_TYPE_FLAGS;
    ...
    if ((0 == largePageMinimumGranule) ||
      ...
      memoryPointer = getMemoryInRange(
          portLibrary, identifier, allocationFlags, protection, category,
          totalAllocateSize, params->startAddress, params->endAddress,
          largePageAlignmentInBytes, params->options, params->mode);
    }
    if (NULL == memoryPointer) {
      /* if OMRPORT_VMEM_STRICT_PAGE_SIZE is not set try again with default page
       * size */
      if (0 == (OMRPORT_VMEM_STRICT_PAGE_SIZE & params->options)) {
        /* Handle default page size */
        uintptr_t pageSize = PPG_vmem_pageSize[0];
        uintptr_t alignmentInBytes =
            OMR_MAX(pageSize, params->alignmentInBytes);
        ...
        if (0 != (OMRPORT_VMEM_MEMORY_MODE_COMMIT & params->mode)) {
          ...
          /*
           * If we don't reserve with PAGE_NOACCESS, CE won't give us large
           * blocks. On Win32 the protection bits appear to be ignored for
           * uncommitted memory.
           */
          defaultProtection = PAGE_NOACCESS;
        }

        /* Make sure that the alignment is a multiple of both requested
         * alignment and page size (enforces that arguments are powers of two
         * and, thus, their max is their lowest common multiple) */
        if ((0 == minimumGranule) ||
            (0 == (alignmentInBytes % minimumGranule))) {
          memoryPointer =
              getMemoryInRange(portLibrary, identifier, defaultAllocationFlags,
                               defaultProtection, category, params->byteAmount,
        ...
        portLibrary->error_set_last_error(portLibrary, lastError,
                                          findError(lastError));
        update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, NULL);
      }
    } else {
      update_vmemIdentifier(identifier, (void *)memoryPointer,
                            (void *)memoryPointer, totalAllocateSize,
                            params->mode, largePageSize,
                            OMRPORT_VMEM_PAGE_FLAG_NOT_USED, category);
    }
  ...
  if (NULL != memoryPointer) {
    /* we don't want to have to search the space for a node descriptor when
     * committing something which might only include part of a bound range so
     * create the "unbound range" entry so that we can immediately find the
     * boundaries of the extent
     */
    BindingNode *baseNode = NULL;
    omrthread_monitor_enter(PPG_bindingAccessMonitor);
    baseNode =
        pool_newElement(portLibrary->portGlobals->platformGlobals.bindingPool);
    if (NULL != baseNode) {
      baseNode->baseAddress = (void *)memoryPointer;
      baseNode->extentOfRangeInBytes = params->byteAmount;
      baseNode->j9NodeNumber = 0;
      avl_insert(&portLibrary->portGlobals->platformGlobals.bindingTree,
                 (J9AVLTreeNode *)baseNode);
      omrthread_monitor_exit(PPG_bindingAccessMonitor);
    } else {
      omrthread_monitor_exit(PPG_bindingAccessMonitor);
      omrvmem_free_memory(portLibrary, memoryPointer, params->byteAmount,
                          identifier);
      memoryPointer = NULL;
    }
  }
...
uintptr_t omrvmem_get_page_size(struct OMRPortLibrary *portLibrary,
                                struct J9PortVmemIdentifier *identifier) {
  return identifier->pageSize;
}

uintptr_t omrvmem_get_page_flags(struct OMRPortLibrary *portLibrary,
                                 struct J9PortVmemIdentifier *identifier) {
 ...
 * that were created by @ref omrvmem_startup should be destroyed here. This
 * function replaces the stub version during omrvmem_startup. This is done so
 * that omrvmem resources are not destroyed without first being initialized.
 *
 * @param[in] portLibrary The port library.
 */
void omrvmem_shutdownImpl(struct OMRPortLibrary *portLibrary) {
  omrthread_monitor_enter(PPG_bindingAccessMonitor);
  if (NULL != portLibrary->portGlobals->platformGlobals.bindingPool) {
 ...
 * that are required for the virtual memory operations may be created here.  All
 * resources created here should be destroyed in @ref omrvmem_shutdown.
 *
 * @param[in] portLibrary The port library.
 *
 * @return 0 on success, negative error code on failure.  Error code values
 * returned are \arg OMRPORT_ERROR_STARTUP_VMEM
 *
 * @note Most implementations will simply return success.
 */
int32_t omrvmem_startup(struct OMRPortLibrary *portLibrary) {
  ...
  /*
   * Initialize the bindingAccessMonitor. Required to ensure thread safety when
   * calls are made to omrvmem operations. Specifically, to provide
   * synchronization when modifying the bindingTree and accessing the
   * bindingPool.
   */
  if (omrthread_monitor_init_with_name(&(PPG_bindingAccessMonitor), 0,
                                       "omrvmem_binding_access_monitor")) {
  ...
  if (0 == portLibrary->sl_open_shared_library(portLibrary, "Kernel32", &handle,
                                               TRUE)) {
    if (0 == portLibrary->sl_lookup_name(
                 portLibrary, handle, "GetLargePageMinimum",
                 (uintptr_t *)&GetLargePageMinimumFunc, "PV")) {
  ...
  if (SetLockPagesPrivilege(GetCurrentProcess(), TRUE) != TRUE) {
    /* local policy forbids raising the priveledge - user must check
     * documentation */
  ...
  if (NULL != kernel32Module) {
    portLibrary->portGlobals->platformGlobals.GetThreadIdealProcessorExProc =
        (GetThreadIdealProcessorExType)GetProcAddress(
            kernel32Module, "GetThreadIdealProcessorEx");
    portLibrary->portGlobals->platformGlobals.SetThreadIdealProcessorExProc =
        (SetThreadIdealProcessorExType)GetProcAddress(
            kernel32Module, "SetThreadIdealProcessorEx");
    portLibrary->portGlobals->platformGlobals.GetNumaNodeProcessorMaskExProc =
        (GetNumaNodeProcessorMaskExType)GetProcAddress(
            kernel32Module, "GetNumaNodeProcessorMaskEx");
  }

  memset(boundExtents, 0x0, sizeof(J9AVLTree));
  boundExtents->insertionComparator = (intptr_t(*)(
      J9AVLTree *, J9AVLTreeNode *, J9AVLTreeNode *))rangeInsertionComparator;
  boundExtents->searchComparator = (intptr_t(*)(
      J9AVLTree *, uintptr_t, J9AVLTreeNode *))rangeSearchComparator;
  boundExtents->portLibrary = portLibrary;
  portLibrary->portGlobals->platformGlobals.bindingPool =
      pool_new(sizeof(BindingNode), 5, 0, 0, OMR_GET_CALLSITE(),
               OMRMEM_CATEGORY_PORT_LIBRARY, POOL_FOR_PORT(portLibrary));
  if (NULL == portLibrary->portGlobals->platformGlobals.bindingPool) {
  ...
  portLibrary->portGlobals->platformGlobals.managementCounterPathStatus =
      MANAGEMENT_COUNTER_PATH_UNINITIALIZED;
  if (0 == returnValue) {
 ...
 * @param[in] portLibrary The port library.
 *
 * @return A 0 terminated array of supported page sizes.  The first entry is the
 * default page size, other entries are the large page sizes supported.
 */
uintptr_t *omrvmem_supported_page_sizes(struct OMRPortLibrary *portLibrary) {
  return PPG_vmem_pageSize;
}

uintptr_t *omrvmem_supported_page_flags(struct OMRPortLibrary *portLibrary) {
...
BOOL SetLockPagesPrivilege(HANDLE hProcess, BOOL bEnable) {
  ...
  if (LookupPrivilegeValue(NULL, SE_LOCK_MEMORY_NAME,
                           &(Info.Privilege[0].Luid)) != TRUE) {
    return FALSE;
  }

  /* Adjust the privilege. */
  if (AdjustTokenPrivileges(Token, FALSE, (PTOKEN_PRIVILEGES)&Info, 0, NULL,
                            NULL) != TRUE) {
    return FALSE;
  } else {
...
void omrvmem_default_large_page_size_ex(struct OMRPortLibrary *portLibrary,
                                        uintptr_t mode, uintptr_t *pageSize,
                                        uintptr_t *pageFlags) {
   ...
   * so the 1 element will be zero or the default large size where only 2 are
   * supported (platforms with more complicated logic will use their own special
   * process to determine the best response
   */
  if (NULL != pageSize) {
...
intptr_t omrvmem_find_valid_page_size(struct OMRPortLibrary *portLibrary,
                                      uintptr_t mode, uintptr_t *pageSize,
                                      uintptr_t *pageFlags,
  ...
  if (0 != validPageSize) {
    ...
    uintptr_t *supportedPageSizes =
        portLibrary->vmem_supported_page_sizes(portLibrary);
    uintptr_t *supportedPageFlags =
        portLibrary->vmem_supported_page_flags(portLibrary);
...
static void *getMemoryInRange(struct OMRPortLibrary *portLibrary,
  ...
  if (mode & OMRPORT_VMEM_MEMORY_MODE_SHARE_FILE_OPEN) {
    portLibrary->error_set_last_error(portLibrary, errno,
                                      OMRPORT_ERROR_VMEM_NOT_SUPPORTED);
  ...
  if (0 != (vmemOptions & OMRPORT_VMEM_ALLOC_DIR_TOP_DOWN)) {
    ...
    if (startAddress == NULL) {
      currentAddress =
          (void *)((uintptr_t)currentAddress + direction * alignmentInBytes);
    }
#if defined(OMRVMEM_DEBUG)
    ...
    if (OMRPORT_VMEM_MAX_ADDRESS == endAddress) {
      /* if caller specified the entire address range and does not care about
       * the direction save time by letting OS choose where to allocate the
       * memory
       */
      goto allocAnywhere;
    } else {
  ...
  /*
   * Don't bother scanning the entire address space if there simply isn't enough
   * memory to satisfy the request. Large allocations with large pages can be
   * slow on certain Windows versions, see CMVC 194435. Test using the caller's
   * setting for MEM_LARGE_PAGES, because the allocation may be feasible with
   * small pages but not with large pages.
   */
  memoryPointer =
      VirtualAlloc(NULL, (SIZE_T)byteAmount, allocationFlags, protection);
  if (NULL == memoryPointer) {
    /* there isn't enough memory */
    return NULL;
  } else {
    /* there is memory, free it then continue to request it in the range and
     * direction we're after */
  ...
  while ((startAddress <= currentAddress) && (endAddress >= currentAddress)) {
    ...
    if ((count % 0x10) == 0) {
      printf(
          "\t\t getMemoryInRange calling VirtualAlloc: address: %p, size: %p, "
          "allocationFlags: %x, protection: %x\n",
    ...
    memoryPointer =
        VirtualAlloc((LPVOID)currentAddress, (SIZE_T)byteAmount,
                     allocationFlags & ~MEM_LARGE_PAGES, protection);
    ...
    if (NULL != memoryPointer) {
      if (MEM_LARGE_PAGES == (allocationFlags & MEM_LARGE_PAGES)) {
        ...
        memoryPointer = VirtualAlloc((LPVOID)currentAddress, (SIZE_T)byteAmount,
                                     allocationFlags, protection);
        if (NULL == memoryPointer) {
          Trc_PRT_vmem_omrvmem_reserve_memory_ex_UnableToReallocateWithLargePages(
              byteAmount, smallPagesMemoryPointer);
        }
      }
    }
    ...
    if (NULL != memoryPointer) {
      if ((startAddress <= memoryPointer) && (endAddress >= memoryPointer)) {
        ...
        VirtualFree((LPVOID)memoryPointer, (size_t)0, MEM_RELEASE);
        memoryPointer =
            NULL; /* This is necessary, cause below OMRPORT_VMEM_STRICT_ADDRESS
                     if statement might try to free the same memoryPointer*/
      }
    }
...
intptr_t omrvmem_numa_set_affinity(struct OMRPortLibrary *portLibrary,
  ...
  if (OMR_ARE_ANY_BITS_SET(identifier->mode, OMRPORT_VMEM_NO_AFFINITY)) {
    ...
    if (before == match) {
      ...
      uintptr_t newBeforeSize =
          (uintptr_t)address - (uintptr_t)before->baseAddress;
    ...
    if (0 == returnValue) {
      ...
      if (end == after) {
        ...
        uintptr_t newEndSize =
            (uintptr_t)topAddress - (uintptr_t)end->baseAddress;
    ...
    if (0 == returnValue) {
      /* now, walk all the extents in the range [address, address+byteAmount)
       * and delete them */
      while (nextSearchAddress < topAddress) {
        BindingNode *toDelete = (BindingNode *)avl_search(
            boundExtents, (uintptr_t)nextSearchAddress);
        nextSearchAddress = (void *)((uintptr_t)nextSearchAddress +
                                     toDelete->extentOfRangeInBytes);
    ...
    if (0 == returnValue) {
      /* coalesce matches */
      if ((NULL != before) &&
          (before->j9NodeNumber == insertedNode->j9NodeNumber)) {
        uintptr_t sizeToGrowTo =
            before->extentOfRangeInBytes + insertedNode->extentOfRangeInBytes;
        avl_delete(boundExtents, (J9AVLTreeNode *)before);
      ...
      if ((NULL != after) &&
          (after->j9NodeNumber == insertedNode->j9NodeNumber)) {
        uintptr_t sizeToGrowTo =
            after->extentOfRangeInBytes + insertedNode->extentOfRangeInBytes;
...
intptr_t omrvmem_numa_get_node_details(struct OMRPortLibrary *portLibrary,
                                       J9MemoryNodeDetail *numaNodes,
                                       uintptr_t *nodeCount) {
  uintptr_t arraySize = *nodeCount;
  uintptr_t totalNodeCount = 0;
  ULONG platformMaxNodeNumber = 0;
  typedef BOOL(__stdcall * getNumaNodeProcessorMaskExProcType)(USHORT,
                                                               PGROUP_AFFINITY);
  typedef BOOL(__stdcall * getNumaAvailableMemoryNodeExProcType)(USHORT,
                                                                 PULONGLONG);
  HMODULE kernel32Module = GetModuleHandle("Kernel32.dll");
  ...
  /* Check the parameters such that if numaNodes is NULL that
   * nodeCount/arraySize is 0, as per the specification. */
  Assert_PRT_true((NULL == numaNodes) == (0 == arraySize));

  if (NULL != kernel32Module) {
    getNumaNodeProcessorMaskExProc =
        (getNumaNodeProcessorMaskExProcType)GetProcAddress(
            kernel32Module, "GetNumaNodeProcessorMaskEx");
    getNumaAvailableMemoryNodeExProc =
        (getNumaAvailableMemoryNodeExProcType)GetProcAddress(
            kernel32Module, "GetNumaAvailableMemoryNodeEx");

    /* Note that we only want to describe NUMA capabilities if we see more than
     * 1 node.  At some point we might want to change our definition of "NUMA
     * enabled" so that all systems have NUMA properties but some have only 1
     * node.  This way, the special case is removed and the GC can decide how to
     * handle that environment.
     */
    if ((NULL != getNumaNodeProcessorMaskExProc) &&
        (NULL != getNumaAvailableMemoryNodeExProc) &&
        GetNumaHighestNodeNumber(&platformMaxNodeNumber) &&
        (platformMaxNodeNumber > 1)) {
      USHORT i = 0;
      ...
      for (i = 0; i <= platformMaxNodeNumber; i++) {
        ...
        if (getNumaAvailableMemoryNodeExProc(i, &bytesAvailable) &&
            getNumaNodeProcessorMaskExProc(i, &processorMask)) {
          /* this node appears to exist so populate its data in the node detail
           * array */
          uintptr_t processorCount = 0;
          KAFFINITY groupAffinity = processorMask.Mask;

          if (totalNodeCount < arraySize) {
            /* Count the bits (TODO:  see if there is a helper to do this more
             * efficiently) */
            ...
            numaNodes[totalNodeCount].memoryPolicy =
                (0 != bytesAvailable) ? J9NUMA_ALLOWED : J9NUMA_DENIED;
            numaNodes[totalNodeCount].computationalResourcesAvailable =
                processorCount;
          }
          totalNodeCount += 1;
        }
...
/**
 * @internal
 * Commits byteAmount bytes starting at address and touches the pages within the
 * range which possess a NUMA binding.
 * @param portLibrary[in] The Port Library instance
 * @param identifier[in] The identifier for this particular virtual memory
 * fragment
 * @param address[in] The base address of the region of memory to be committed
 * @param byteAmount[in] The length, in bytes, of the virtual memory fragment
 * starting at address
 * @return The base address of the committed region or NULL if the operation
 * failed
 */
static void *commitAndTouch(struct OMRPortLibrary *portLibrary,
                            J9PortVmemIdentifier *identifier, void *address,
  ...
  while ((current != 0) && (current < top)) {
    /* touchOnNumaNode() is going to write to the pages we commit so it's
     * important to only commit pages which haven't yet been committed. If we
     * were to recommit a page touchOnNumaNode() would overwrite its first word,
     * causing data corruption (see CMVC 175149 for example).
     */
    MEMORY_BASIC_INFORMATION memInfo;
    SIZE_T queryResult =
        VirtualQuery((LPVOID)current, &memInfo, sizeof(memInfo));
    ...
    if ((0 != queryResult) && (0 == (memInfo.State & MEM_COMMIT))) {
      ...
      uintptr_t commitTop =
          OMR_MIN((uintptr_t)memInfo.BaseAddress + memInfo.RegionSize, top);
      LPVOID result = VirtualAlloc((LPVOID)commitBase, commitTop - commitBase,
                                   MEM_COMMIT | J9_EXTRA_TYPE_FLAGS,
                                   getProtectionBits(identifier->mode));

      if (NULL != result) {
        /* the commit succeeded so we just need to touch the memory with a
         * thread on the correct node */
        J9AVLTree *boundExtents =
            &portLibrary->portGlobals->platformGlobals.bindingTree;
        ...
        while ((node != NULL) && ((uintptr_t)thisAddress < commitTop)) {
          void *nextAddress =
              (void *)OMR_MIN(commitTop, (uintptr_t)node->baseAddress +
                                             node->extentOfRangeInBytes);
          uintptr_t bytesToTouch =
              (uintptr_t)nextAddress - (uintptr_t)thisAddress;
          uintptr_t j9NodeNumber = node->j9NodeNumber;

          if (0 != j9NodeNumber) {
            touchOnNumaNode(portLibrary, pageSizeInBytes, thisAddress,
                            bytesToTouch, j9NodeNumber);
          }
          /* we didn't need to touch the memory or the touch was successful so
           * continue */
          thisAddress = nextAddress;
          node =
              (BindingNode *)avl_search(boundExtents, (uintptr_t)thisAddress);
        }
        omrthread_monitor_exit(PPG_bindingAccessMonitor);
      }
    }
  ...
  /* It's possible that some of the intermediate operations failed. Commit the
   * whole range again and use the result of that the determine overall success.
   */
  finalResult = VirtualAlloc((LPVOID)address, (SIZE_T)byteAmount,
                             MEM_COMMIT | J9_EXTRA_TYPE_FLAGS,
                             getProtectionBits(identifier->mode));
  return finalResult;
}
...
/**
 * @internal
 * Touches every page of memory for byteAmount bytes, starting at address, in
 * such a way that the pages should be allocated on the given j9NumaNode.
 * @param portLibrary[in] The Port Library instance
 * @param pageSizeInBytes[in] The size of each page in the memory extent
 * @param address[in] The base address of the region of memory to be committed
 * @param byteAmount[in] The length, in bytes, of the virtual memory fragment
 * starting at address
 * @param j9NumaNode[in] The node number to which the pages must be bound
 * (1-indexed node number)
 */
static void touchOnNumaNode(struct OMRPortLibrary *portLibrary,
                            uintptr_t pageSizeInBytes, void *address,
                            uintptr_t byteAmount, uintptr_t j9NumaNode) {
  GetThreadIdealProcessorExType GetThreadIdealProcessorExProc =
      portLibrary->portGlobals->platformGlobals.GetThreadIdealProcessorExProc;
  SetThreadIdealProcessorExType SetThreadIdealProcessorExProc =
      portLibrary->portGlobals->platformGlobals.SetThreadIdealProcessorExProc;
  GetNumaNodeProcessorMaskExType GetNumaNodeProcessorMaskExProc =
      portLibrary->portGlobals->platformGlobals.GetNumaNodeProcessorMaskExProc;

  if ((NULL != GetThreadIdealProcessorExProc) &&
      (NULL != SetThreadIdealProcessorExProc) &&
      (NULL != GetNumaNodeProcessorMaskExProc)) {
    ...
    if (GetThreadIdealProcessorExProc(thisThread, &originalProc)) {
      ...
      memset(&processorMask, 0x0, sizeof(processorMask));
      /* get the processors for the target node (remember the native layer is
       * 0-indexed and we are 1-indexed) */
      if (GetNumaNodeProcessorMaskExProc((USHORT)(j9NumaNode - 1),
                                         &processorMask)) {
        ...
        /* find a processor in the node to which we can bind ourselves for the
         * touch */
        while ((!didBind) && (0 != groupAffinity)) {
          if (0x1 == (0x1 & groupAffinity)) {
            ...
            if (SetThreadIdealProcessorExProc(thisThread, &newProc, NULL)) {
              ...
              /* we failed to bind which typically means that the core is parked
               * so we will try another */
              Trc_PRT_vmem_touchOnNumaNode_failedToBindToComputeResource(
                  (uintptr_t)index, j9NumaNode, (uintptr_t)GetLastError());
            }
          }
          groupAffinity >>= 1;
          index += 1;
        ...
        if (didBind) {
          ...
          /* now that we have bound this thread to the node where we want to
           * commit this, touch all the pages to force them to be allocated here
           */
          for (loop = address; loop < top;
               loop = (void *)((uintptr_t)loop + pageSizeInBytes)) {
            *((uintptr_t *)loop) = 0x0;
          }
          ...
          if (SetThreadIdealProcessorExProc(thisThread, &originalProc, NULL)) {
            ...
            /* we failed to set our affinity back to what it just was - this
             * failure should not ever be possible */
            Trc_PRT_vmem_touchOnNumaNode_failedToReturnToComputeResource(
                (uintptr_t)originalProc.Number, (uintptr_t)newProc.Number,
                (uintptr_t)GetLastError());
          }
        } else {
          /* we weren't able to change to the given node so it is possible that
           * all the cores on the node are parked so we will have to fall back
           * to the system's default page allocation strategy */
          Trc_PRT_vmem_touchOnNumaNode_failedToBindToNodeForTouch(j9NumaNode);
        }
      } else {
        /* we weren't able to look up what node this is which implies that we
         * were asking for an invalid node - this indicates a usage error
         * somewhere in the caller */
        Assert_PRT_true(FALSE);
      }
    } else {
      /* we failed to read the thread's affinity which implies something is
       * seriously wrong (the thread probably doesn't have any access rights so
       * we can't be running any of this code) */
      Assert_PRT_true(FALSE);
    }
  }
}
...
/**
 * @internal
 * Used by the AVL tree "bindingTree" to place the given binding node in the
 * tree.
 ...
 * @return The location where insertNode should be stored relative to walkNode
 * (1 for after, -1 for before, 0 for match)
 */
static intptr_t rangeInsertionComparator(J9AVLTree *tree,
                                         BindingNode *insertNode,
                                         BindingNode *walkNode) {
...
/**
 * @internal
 * Used by the AVL tree "bindingTree" to find a given value's binding node in
 * the tree.
 * @param tree[in] The AVL tree
 * @param value[in] The key we are searching for
 * @param searchNode[in] The current node we are comparing to the given value
 * @return The location where we would find the appropriate node relative to
 * searchNode (1 for after, -1 for before, 0 for match)
 */
static intptr_t rangeSearchComparator(J9AVLTree *tree, uintptr_t value,
                                      BindingNode *searchNode) {
  uintptr_t numericalBaseAddress = (uintptr_t)searchNode->baseAddress;
  uintptr_t numericalTopAddress =
      numericalBaseAddress + searchNode->extentOfRangeInBytes;
...
int32_t omrvmem_get_available_physical_memory(
  ...
  if (0 != GlobalMemoryStatusEx(&statex)) {
    *freePhysicalMemorySize = statex.ullAvailPhys;
    ...
    Trc_PRT_vmem_get_available_physical_memory_failed("GlobalMemoryStatusEx",
                                                      lastError);
    return OMRPORT_ERROR_VMEM_OPFAILED;
  }
  return 0;
...
int32_t omrvmem_get_process_memory_size(struct OMRPortLibrary *portLibrary,
                                        J9VMemMemoryQuery queryType,
                                        uint64_t *memorySize) {
  ...
  switch (queryType) {
    case OMRPORT_VMEM_PROCESS_PHYSICAL: {
      ...
      if (0 != GetProcessMemoryInfo(GetCurrentProcess(), &info, sizeof(info))) {
        ...
        Trc_PRT_vmem_get_available_physical_memory_failed(
            "GetProcessMemoryInfo failed", 0);
...
static int32_t getProcessPrivateMemorySize(struct OMRPortLibrary *portLibrary,
  ...
  if (MANAGEMENT_COUNTER_PATH_UNINITIALIZED == isCounterPathInitialized) {
    /* pathElementSize is used to pre-allocate a buffer for PdhParseCounterPath.
     * This is done for performance reason as normal API invocation needs two
     * passes, first to acquire the space needed, then second pass to actually
     * fetch the data. Per MSDN documentation, pathElement contains pointers to
     * each individual string path elements of the structure. The actual strings
     * are appended to the end of structure. Hence, the total memory required
     * cannot exceed the size of the structure plus the size of maximum counter
     * path allowed.
     */
    DWORD pathElementsSize =
        PDH_MAX_COUNTER_PATH + sizeof(PDH_COUNTER_PATH_ELEMENTS);
    TCHAR *str = NULL;
    DWORD pathListSize = 0;
    DWORD pid = GetCurrentProcessId();

    /* First pass to obtain the requirement memory buffer size. Note, buffer
     * cannot be pre-allocated as there is no upper bound to how much memory
     * will be needed.*/
    if (PDH_MORE_DATA != PdhExpandWildCardPath(NULL, "\\Process(*)\\ID Process",
                                               pathList, &pathListSize, 0)) {
      goto CLEAN_UP;
    }

    pathList = portLibrary->mem_allocate_memory(
        portLibrary, pathListSize + 1, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_VM);
    ...
    if (ERROR_SUCCESS != PdhExpandWildCardPath(NULL, "\\Process(*)\\ID Process",
                                               pathList, &pathListSize, 0)) {
      goto CLEAN_UP;
    }

    pathElements =
        (PDH_COUNTER_PATH_ELEMENTS *)portLibrary->mem_allocate_memory(
            portLibrary, pathElementsSize, OMR_GET_CALLSITE(),
            OMRMEM_CATEGORY_VM);
    ...
    while ('\0' != *str) {
      ...
      if (ERROR_SUCCESS !=
          PdhGetFormattedCounterValue(counter, PDH_FMT_LONG, &ret, &value)) {
        PdhRemoveCounter(counter);
        isCounterAdded = FALSE;
        str += strlen(str) + 1;
        continue;
      ...
      if (value.longValue == pid) {
        ...
        /* PdhParseCounterPath resets the pathElementSize value, needs to set it
         * to actual size of buffer to avoid error */
        pathElementsSize =
            PDH_MAX_COUNTER_PATH + sizeof(PDH_COUNTER_PATH_ELEMENTS);

        /*
         * Typical process is to invoke PdhParseCounterPath twice, first time to
         * obtain the necessary buffer size. However, to avoid overhead of
         * invoking the function twice, we pass a pre-allocated max size buffer.
         */
        if (ERROR_SUCCESS !=
            PdhParseCounterPath(str, pathElements, &pathElementsSize, 0)) {
          goto CLEAN_UP;
        }
        ...
        /*
         * Store the correct counter path into counterPath, and toggle path
         * initialized bit to avoid necessary processing in subsequent
         * invocation
         */
        MUTEX_ENTER(PPG_managementDataLock);
        if (MANAGEMENT_COUNTER_PATH_UNINITIALIZED ==
            PPG_managementCounterPathStatus) {
          DWORD counterPathSize = PDH_MAX_COUNTER_PATH;

          if (ERROR_SUCCESS != PdhMakeCounterPath(&tmp,
                                                  PPG_managementCounterPath,
                                                  &counterPathSize, 0)) {
            MUTEX_EXIT(PPG_managementDataLock);
            goto CLEAN_UP;
          }
  ...
  if (ERROR_SUCCESS !=
      PdhAddCounter(query, (LPCSTR)PPG_managementCounterPath, 0, &counter)) {
    MUTEX_EXIT(PPG_managementDataLock);
    goto CLEAN_UP;
  }
  isCounterAdded = TRUE;
  ...
  if ((status != PDH_INVALID_HANDLE) && (status != PDH_NO_DATA)) {
    if (ERROR_SUCCESS !=
        PdhGetFormattedCounterValue(counter, PDH_FMT_LARGE, &ret, &value)) {
      goto CLEAN_UP;
    }
...
OLD_IMPL:
  if (!pdhSuccess) {
    ...
    /* PDH family failed. Cache this fact for all subsequent invocation to
     * directly pick the old impl */
    if (MANAGEMENT_COUNTER_USE_OLD_ALGORITHM !=
        PPG_managementCounterPathStatus) {
      MUTEX_ENTER(PPG_managementDataLock);
...
void *omrvmem_get_contiguous_region_memory(
