...
static void* getMemoryInRange(struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier,
    DWORD allocationFlags, DWORD protection, OMRMemCategory* category, uintptr_t byteAmount, void* startAddress,
    void* endAddress, uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode);
...
void update_vmemIdentifier(J9PortVmemIdentifier* identifier, void* address, void* handle, uintptr_t byteAmount,
    uintptr_t mode, uintptr_t pageSize, uintptr_t pageFlags, OMRMemCategory* category);
static void* commitAndTouch(
    struct OMRPortLibrary* portLibrary, J9PortVmemIdentifier* identifier, void* address, uintptr_t byteAmount);
static void touchOnNumaNode(struct OMRPortLibrary* portLibrary, uintptr_t pageSizeInBytes, void* address,
...
static int32_t getProcessPrivateMemorySize(
    ...
    struct OMRPortLibrary* portLibrary, void* address, uintptr_t byteAmount, struct J9PortVmemIdentifier* identifier)
{
    ...
    if (ret != 0) {
        ...
        /* (note that the sub-allocator can cause us to shut-down the binding pool prior to freeing the last reserved
         * area so ignore NUMA clean-up if the pool is already gone) */
        if (NULL != portLibrary->portGlobals->platformGlobals.bindingPool) {
            ...
            /* There is one case where the tree can have precisely zero entries - failure in allocating the first node
             * in the range. Otherwise, the sequence of nodes should be complete and contiguous.
             */
    ...
    struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier, struct J9PortVmemParams* params)
{
    ...
    if (0 == params->pageSize) {
        ...
        /* Make sure that the alignment is a multiple of both requested alignment and page size (enforces that arguments
         * are powers of two and, thus, their max is their lowest common multiple) */
        if ((0 == minimumGranule) || (0 == (alignmentInBytes % minimumGranule))) {
            memoryPointer
                = getMemoryInRange(portLibrary, identifier, allocationFlags, protection, category, params->byteAmount,
                    params->startAddress, params->endAddress, alignmentInBytes, params->options, params->mode);
        }
        if (NULL == memoryPointer) {
            Trc_PRT_vmem_omrvmem_reserve_memory_failure();
            update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, NULL);
        } else {
            update_vmemIdentifier(identifier, (void*)memoryPointer, (void*)memoryPointer, params->byteAmount,
                params->mode, PPG_vmem_pageSize[0], OMRPORT_VMEM_PAGE_FLAG_NOT_USED, category);
        }
        ...
        if ((0 == largePageMinimumGranule) || (0 == (largePageAlignmentInBytes % largePageMinimumGranule))) {
            memoryPointer
                = getMemoryInRange(portLibrary, identifier, allocationFlags, protection, category, totalAllocateSize,
                    params->startAddress, params->endAddress, largePageAlignmentInBytes, params->options, params->mode);
        }
        if (NULL == memoryPointer) {
            ...
            if (0 == (OMRPORT_VMEM_STRICT_PAGE_SIZE & params->options)) {
                ...
                /* Make sure that the alignment is a multiple of both requested alignment and page size (enforces that
                 * arguments are powers of two and, thus, their max is their lowest common multiple) */
                if ((0 == minimumGranule) || (0 == (alignmentInBytes % minimumGranule))) {
                    memoryPointer = getMemoryInRange(portLibrary, identifier, defaultAllocationFlags, defaultProtection,
                        category, params->byteAmount, params->startAddress, params->endAddress, alignmentInBytes,
            ...
            update_vmemIdentifier(identifier, (void*)memoryPointer, (void*)memoryPointer, totalAllocateSize,
                params->mode, largePageSize, OMRPORT_VMEM_PAGE_FLAG_NOT_USED, category);
        }
        SetLockPagesPrivilege(GetCurrentProcess(), FALSE);
    }

    if (NULL != memoryPointer) {
        /* we don't want to have to search the space for a node descriptor when committing something which might only
         * include part of a bound range so create the "unbound range" entry so that we can immediately find the
 ...
 * should be destroyed here. This function replaces the stub version during omrvmem_startup. This is done so that
 * omrvmem resources are not destroyed without first being initialized.
 *
 * @param[in] portLibrary The port library.
 */
...
int32_t omrvmem_startup(struct OMRPortLibrary* portLibrary)
{
    ...
    if (0 == portLibrary->sl_open_shared_library(portLibrary, "Kernel32", &handle, TRUE)) {
        if (0
            == portLibrary->sl_lookup_name(
                portLibrary, handle, "GetLargePageMinimum", (uintptr_t*)&GetLargePageMinimumFunc, "PV")) {
    ...
    if (NULL != kernel32Module) {
        portLibrary->portGlobals->platformGlobals.GetThreadIdealProcessorExProc
            = (GetThreadIdealProcessorExType)GetProcAddress(kernel32Module, "GetThreadIdealProcessorEx");
        portLibrary->portGlobals->platformGlobals.SetThreadIdealProcessorExProc
            = (SetThreadIdealProcessorExType)GetProcAddress(kernel32Module, "SetThreadIdealProcessorEx");
        portLibrary->portGlobals->platformGlobals.GetNumaNodeProcessorMaskExProc
            = (GetNumaNodeProcessorMaskExType)GetProcAddress(kernel32Module, "GetNumaNodeProcessorMaskEx");
    }

    memset(boundExtents, 0x0, sizeof(J9AVLTree));
    boundExtents->insertionComparator
        = (intptr_t(*)(J9AVLTree*, J9AVLTreeNode*, J9AVLTreeNode*))rangeInsertionComparator;
    ...
    portLibrary->portGlobals->platformGlobals.bindingPool = pool_new(
        sizeof(BindingNode), 5, 0, 0, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY, POOL_FOR_PORT(portLibrary));
    if (NULL == portLibrary->portGlobals->platformGlobals.bindingPool) {
    ...
    uintptr_t* pageFlags, BOOLEAN* isSizeSupported)
{
    ...
    if (0 != validPageSize) {
        ...
        for (pageIndex = 0; 0 != supportedPageSizes[pageIndex]; pageIndex++) {
            if ((supportedPageSizes[pageIndex] == validPageSize) && (supportedPageFlags[pageIndex] == validPageFlags)) {
                goto _end;
            }
        }
    }
    ...
    void* endAddress, uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
    ...
    while ((startAddress <= currentAddress) && (endAddress >= currentAddress)) {
...
#if defined(OMRVMEM_DEBUG)
        if ((count % 0x10) == 0) {
            printf("\t\t getMemoryInRange calling VirtualAlloc: address: %p, size: %p, allocationFlags: %x, "
                   "protection: %x\n",
        ...
        memoryPointer
            = VirtualAlloc((LPVOID)currentAddress, (SIZE_T)byteAmount, allocationFlags & ~MEM_LARGE_PAGES, protection);
        ...
        if (NULL != memoryPointer) {
            if ((startAddress <= memoryPointer) && (endAddress >= memoryPointer)) {
                ...
                VirtualFree((LPVOID)memoryPointer, (size_t)0, MEM_RELEASE);
                memoryPointer = NULL; /* This is necessary, cause below OMRPORT_VMEM_STRICT_ADDRESS if statement might
                                         try to free the same memoryPointer*/
            }
        }
    ...
    struct OMRPortLibrary* portLibrary, J9MemoryNodeDetail* numaNodes, uintptr_t* nodeCount)
{
    ...
    if (NULL != kernel32Module) {
        getNumaNodeProcessorMaskExProc
            = (getNumaNodeProcessorMaskExProcType)GetProcAddress(kernel32Module, "GetNumaNodeProcessorMaskEx");
        getNumaAvailableMemoryNodeExProc
            = (getNumaAvailableMemoryNodeExProcType)GetProcAddress(kernel32Module, "GetNumaAvailableMemoryNodeEx");

        /* Note that we only want to describe NUMA capabilities if we see more than 1 node.  At some point we might want
         * to change our definition of "NUMA enabled" so that all systems have NUMA properties but some have only 1
         * node.  This way, the special case is removed and the GC can decide how to handle that environment.
         */
        if ((NULL != getNumaNodeProcessorMaskExProc) && (NULL != getNumaAvailableMemoryNodeExProc)
            && GetNumaHighestNodeNumber(&platformMaxNodeNumber) && (platformMaxNodeNumber > 1)) {
            USHORT i = 0;
    ...
    struct OMRPortLibrary* portLibrary, J9PortVmemIdentifier* identifier, void* address, uintptr_t byteAmount)
{
    ...
    while ((current != 0) && (current < top)) {
        ...
        if ((0 != queryResult) && (0 == (memInfo.State & MEM_COMMIT))) {
            ...
            LPVOID result = VirtualAlloc((LPVOID)commitBase, commitTop - commitBase, MEM_COMMIT | J9_EXTRA_TYPE_FLAGS,
            ...
            if (NULL != result) {
                ...
                while ((node != NULL) && ((uintptr_t)thisAddress < commitTop)) {
                    void* nextAddress
                        = (void*)OMR_MIN(commitTop, (uintptr_t)node->baseAddress + node->extentOfRangeInBytes);
    ...
    /* It's possible that some of the intermediate operations failed. Commit the whole range again and use the result of
     * that the determine overall success. */
    finalResult = VirtualAlloc(
        (LPVOID)address, (SIZE_T)byteAmount, MEM_COMMIT | J9_EXTRA_TYPE_FLAGS, getProtectionBits(identifier->mode));
    return finalResult;
}

/**
 * @internal
 * Touches every page of memory for byteAmount bytes, starting at address, in such a way that the pages should be
 * allocated on the given j9NumaNode.
 * @param portLibrary[in] The Port Library instance
    ...
    uintptr_t byteAmount, uintptr_t j9NumaNode)
{
    ...
    if ((NULL != GetThreadIdealProcessorExProc) && (NULL != SetThreadIdealProcessorExProc)
        ...
        if (GetThreadIdealProcessorExProc(thisThread, &originalProc)) {
            ...
            if (GetNumaNodeProcessorMaskExProc((USHORT)(j9NumaNode - 1), &processorMask)) {
                ...
                if (didBind) {
                    ...
                    /* now that we have bound this thread to the node where we want to commit this, touch all the pages
                     * to force them to be allocated here */
                    ...
                    if (SetThreadIdealProcessorExProc(thisThread, &originalProc, NULL)) {
                        ...
                        /* we failed to set our affinity back to what it just was - this failure should not ever be
                         * possible */
                        Trc_PRT_vmem_touchOnNumaNode_failedToReturnToComputeResource(
                            (uintptr_t)originalProc.Number, (uintptr_t)newProc.Number, (uintptr_t)GetLastError());
                    }
                } else {
                    /* we weren't able to change to the given node so it is possible that all the cores on the node are
                     * parked so we will have to fall back to the system's default page allocation strategy */
                    Trc_PRT_vmem_touchOnNumaNode_failedToBindToNodeForTouch(j9NumaNode);
                }
            } else {
                /* we weren't able to look up what node this is which implies that we were asking for an invalid node -
                 * this indicates a usage error somewhere in the caller */
                Assert_PRT_true(FALSE);
            }
        } else {
            /* we failed to read the thread's affinity which implies something is seriously wrong (the thread probably
             * doesn't have any access rights so we can't be running any of this code) */
            Assert_PRT_true(FALSE);
        }
    }
}
 ...
 * @return The location where we would find the appropriate node relative to searchNode (1 for after, -1 for before, 0
 * for match)
 */
static intptr_t rangeSearchComparator(J9AVLTree* tree, uintptr_t value, BindingNode* searchNode)
{
    ...
    struct OMRPortLibrary* portLibrary, J9VMemMemoryQuery queryType, uint64_t* memorySize)
{
    ...
    if (MANAGEMENT_COUNTER_PATH_UNINITIALIZED == isCounterPathInitialized) {
        ...
        pathList
            = portLibrary->mem_allocate_memory(portLibrary, pathListSize + 1, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_VM);
        ...
        while ('\0' != *str) {
            ...
            if (value.longValue == pid) {
                ...
                /* PdhParseCounterPath resets the pathElementSize value, needs to set it to actual size of buffer to
                 * avoid error */
                pathElementsSize = PDH_MAX_COUNTER_PATH + sizeof(PDH_COUNTER_PATH_ELEMENTS);

                /*
                 * Typical process is to invoke PdhParseCounterPath twice, first time to obtain the necessary buffer
                 * size. However, to avoid overhead of invoking the function twice, we pass a pre-allocated max size
                 * buffer.
                 */
...
void* omrvmem_get_contiguous_region_memory(struct OMRPortLibrary* portLibrary, void* addresses[],
