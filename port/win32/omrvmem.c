...
void update_vmemIdentifier(J9PortVmemIdentifier* identifier,
                           void* address,
                           void* handle,
                           uintptr_t byteAmount,
                           uintptr_t mode,
...
static void touchOnNumaNode(struct OMRPortLibrary* portLibrary,
                            uintptr_t pageSizeInBytes,
                            void* address,
                            uintptr_t byteAmount,
                            uintptr_t j9NumaNode);
static intptr_t rangeInsertionComparator(J9AVLTree* tree,
                                         BindingNode* insertNode,
                                         BindingNode* walkNode);
static intptr_t rangeSearchComparator(J9AVLTree* tree,
                                      uintptr_t value,
 ...
 */
void* omrvmem_commit_memory(struct OMRPortLibrary* portLibrary,
                            void* address,
                            uintptr_t byteAmount,
                            struct J9PortVmemIdentifier* identifier) {
  void* ptr = NULL;
 ...
 * @return 0 on success, non zero on failure.
 */
int32_t omrvmem_free_memory(struct OMRPortLibrary* portLibrary,
                            void* address,
                            uintptr_t byteAmount,
                            struct J9PortVmemIdentifier* identifier) {
  int32_t ret = 0;
  ...
  if (ret != 0) {
    ...
    if (NULL != portLibrary->portGlobals->platformGlobals.bindingPool) {
      ...
      if (NULL !=
          (BindingNode*)avl_search(boundExtents, (uintptr_t)nextBaseToLookup)) {
        J9Pool* extentPool =
        ...
        while (nextBaseToLookup < topOfExtent) {
          ...
          nextBaseToLookup =
              (void*)((uintptr_t)nextBaseToLookup + node->extentOfRangeInBytes);
...
void* omrvmem_reserve_memory(struct OMRPortLibrary* portLibrary,
                             void* address,
                             uintptr_t byteAmount,
                             struct J9PortVmemIdentifier* identifier,
                             uintptr_t mode,
...
int32_t omrvmem_startup(struct OMRPortLibrary* portLibrary) {
  ...
  boundExtents->searchComparator =
      (intptr_t(*)(J9AVLTree*, uintptr_t, J9AVLTreeNode*))rangeSearchComparator;
...
static int32_t getProcessPrivateMemorySize(struct OMRPortLibrary* portLibrary,
  ...
  if (MANAGEMENT_COUNTER_PATH_UNINITIALIZED == isCounterPathInitialized) {
    ...
    pathElements = (PDH_COUNTER_PATH_ELEMENTS*)portLibrary->mem_allocate_memory(
        portLibrary, pathElementsSize, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_VM);
