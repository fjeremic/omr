  ...
  struct OMRPortLibrary* portLibrary, void* address, uintptr_t byteAmount, struct J9PortVmemIdentifier* identifier)
{
  ...
  if (ret != 0) {
    ...
    /* (note that the sub-allocator can cause us to shut-down the binding pool prior to freeing the last reserved area
     * so ignore NUMA clean-up if the pool is already gone) */
    if (NULL != portLibrary->portGlobals->platformGlobals.bindingPool) {
      ...
      /* There is one case where the tree can have precisely zero entries - failure in allocating the first node in the
       * range. Otherwise, the sequence of nodes should be complete and contiguous.
       */
  ...
  struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier, struct J9PortVmemParams* params)
{
  ...
  if (0 == params->pageSize) {
    ...
    if ((0 == minimumGranule) || (0 == (alignmentInBytes % minimumGranule))) {
      memoryPointer = getMemoryInRange(portLibrary, identifier, allocationFlags, protection, category,
        params->byteAmount, params->startAddress, params->endAddress, alignmentInBytes, params->options, params->mode);
    }
    if (NULL == memoryPointer) {
      Trc_PRT_vmem_omrvmem_reserve_memory_failure();
      update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, NULL);
    } else {
      update_vmemIdentifier(identifier, (void*)memoryPointer, (void*)memoryPointer, params->byteAmount, params->mode,
        PPG_vmem_pageSize[0], OMRPORT_VMEM_PAGE_FLAG_NOT_USED, category);
    }
    ...
    if (NULL == memoryPointer) {
      ...
      if (0 == (OMRPORT_VMEM_STRICT_PAGE_SIZE & params->options)) {
        ...
        if ((0 == minimumGranule) || (0 == (alignmentInBytes % minimumGranule))) {
          memoryPointer = getMemoryInRange(portLibrary, identifier, defaultAllocationFlags, defaultProtection, category,
            params->byteAmount, params->startAddress, params->endAddress, alignmentInBytes, params->options,
            params->mode);
        }
        if (NULL == memoryPointer) {
          Trc_PRT_vmem_omrvmem_reserve_memory_failure();
      ...
      update_vmemIdentifier(identifier, (void*)memoryPointer, (void*)memoryPointer, totalAllocateSize, params->mode,
        largePageSize, OMRPORT_VMEM_PAGE_FLAG_NOT_USED, category);
    }
  ...
  if (NULL != memoryPointer) {
     ...
     * include part of a bound range so create the "unbound range" entry so that we can immediately find the boundaries
     * of the extent
     */
  ...
  while ((startAddress <= currentAddress) && (endAddress >= currentAddress)) {
    ...
    if (NULL != memoryPointer) {
      if ((startAddress <= memoryPointer) && (endAddress >= memoryPointer)) {
        ...
        VirtualFree((LPVOID)memoryPointer, (size_t)0, MEM_RELEASE);
        memoryPointer = NULL; /* This is necessary, cause below OMRPORT_VMEM_STRICT_ADDRESS if statement might try to
                                 free the same memoryPointer*/
      }
    }
  ...
  struct OMRPortLibrary* portLibrary, J9MemoryNodeDetail* numaNodes, uintptr_t* nodeCount)
{
  ...
  if (NULL != kernel32Module) {
    ...
    /* Note that we only want to describe NUMA capabilities if we see more than 1 node.  At some point we might want to
     * change our definition of "NUMA enabled" so that all systems have NUMA properties but some have only 1 node.  This
     * way, the special case is removed and the GC can decide how to handle that environment.
     */
    ...
    if (GetThreadIdealProcessorExProc(thisThread, &originalProc)) {
      ...
      if (GetNumaNodeProcessorMaskExProc((USHORT)(j9NumaNode - 1), &processorMask)) {
        ...
        if (didBind) {
          ...
          /* now that we have bound this thread to the node where we want to commit this, touch all the pages to force
           * them to be allocated here */
          ...
          /* we weren't able to change to the given node so it is possible that all the cores on the node are parked so
           * we will have to fall back to the system's default page allocation strategy */
          Trc_PRT_vmem_touchOnNumaNode_failedToBindToNodeForTouch(j9NumaNode);
        }
      } else {
        /* we weren't able to look up what node this is which implies that we were asking for an invalid node - this
         * indicates a usage error somewhere in the caller */
        Assert_PRT_true(FALSE);
      }
    } else {
      /* we failed to read the thread's affinity which implies something is seriously wrong (the thread probably doesn't
       * have any access rights so we can't be running any of this code) */
      Assert_PRT_true(FALSE);
    }
  }
}
