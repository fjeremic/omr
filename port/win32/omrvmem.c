...
static void*
getMemoryInRange(struct OMRPortLibrary* portLibrary,
...
update_vmemIdentifier(J9PortVmemIdentifier* identifier,
                      ...
                      OMRMemCategory* category);
static void*
commitAndTouch(struct OMRPortLibrary* portLibrary,
               J9PortVmemIdentifier* identifier,
               void* address,
               uintptr_t byteAmount);
static void
touchOnNumaNode(struct OMRPortLibrary* portLibrary,
                uintptr_t pageSizeInBytes,
                void* address,
                uintptr_t byteAmount,
                uintptr_t j9NumaNode);
static intptr_t
rangeInsertionComparator(J9AVLTree* tree,
                         BindingNode* insertNode,
                         BindingNode* walkNode);
static intptr_t
rangeSearchComparator(J9AVLTree* tree,
                      uintptr_t value,
                      BindingNode* searchNode);
static int32_t
getProcessPrivateMemorySize(struct OMRPortLibrary* portLibrary,
                        ...
                        struct J9PortVmemIdentifier* identifier)
{
  ...
  if (portLibrary->portGlobals->vmemAdviseOSonFree == 1) {
    if (rangeIsValid(identifier, address, byteAmount)) {
      ...
      if (byteAmount > 0) {
        if (PPG_vmem_pageSize[0] == identifier->pageSize) {
          ret = (intptr_t)VirtualFree(
            (LPVOID)address, (size_t)byteAmount, MEM_DECOMMIT);
          if (ret != 0) {
            ...
            Trc_PRT_vmem_omrvmem_decommit_memory_failure(
              GetLastError(), address, byteAmount);
          }
        } else {
          ret = 0;
        }
                          ...
                          struct J9PortVmemParams* params)
{
  ...
  if (0 == params->pageSize) {
    ...
    if ((0 == minimumGranule) || (0 == (alignmentInBytes % minimumGranule))) {
      memoryPointer = getMemoryInRange(portLibrary,
                                       identifier,
                                       allocationFlags,
                                       protection,
                                       category,
    ...
    if (NULL == memoryPointer) {
      ...
      if (0 == (OMRPORT_VMEM_STRICT_PAGE_SIZE & params->options)) {
        ...
        portLibrary->error_set_last_error(
          portLibrary, lastError, findError(lastError));
        update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, NULL);
      }
    } else {
      update_vmemIdentifier(identifier,
  ...
  if (NULL != memoryPointer) {
    ...
    if (NULL != baseNode) {
      ...
      omrthread_monitor_exit(PPG_bindingAccessMonitor);
      omrvmem_free_memory(
        portLibrary, memoryPointer, params->byteAmount, identifier);
      memoryPointer = NULL;
    }
  }
...
omrvmem_startup(struct OMRPortLibrary* portLibrary)
{
  ...
  if (omrthread_monitor_init_with_name(
        &(PPG_bindingAccessMonitor), 0, "omrvmem_binding_access_monitor")) {
  ...
  if (0 == portLibrary->sl_open_shared_library(
             portLibrary, "Kernel32", &handle, TRUE)) {
    if (0 == portLibrary->sl_lookup_name(portLibrary,
                                         handle,
                                         "GetLargePageMinimum",
                                         (uintptr_t*)&GetLargePageMinimumFunc,
                                         "PV")) {
...
SetLockPagesPrivilege(HANDLE hProcess, BOOL bEnable)
{
  ...
  if (AdjustTokenPrivileges(
        Token, FALSE, (PTOKEN_PRIVILEGES)&Info, 0, NULL, NULL) != TRUE) {
    return FALSE;
  } else {
                 ...
                 uintptr_t mode)
{
  ...
  while ((startAddress <= currentAddress) && (endAddress >= currentAddress)) {
    ...
    memoryPointer = VirtualAlloc((LPVOID)currentAddress,
                                 (SIZE_T)byteAmount,
    ...
    if (NULL != memoryPointer) {
      if ((startAddress <= memoryPointer) && (endAddress >= memoryPointer)) {
        ...
        VirtualFree((LPVOID)memoryPointer, (size_t)0, MEM_RELEASE);
        memoryPointer =
          NULL; /* This is necessary, cause below OMRPORT_VMEM_STRICT_ADDRESS if
                   statement might try to free the same memoryPointer*/
      }
    }
  ...
  if (OMR_ARE_ANY_BITS_SET(identifier->mode, OMRPORT_VMEM_NO_AFFINITY)) {
    ...
    if (0 == returnValue) {
      ...
      while (nextSearchAddress < topAddress) {
        BindingNode* toDelete =
          (BindingNode*)avl_search(boundExtents, (uintptr_t)nextSearchAddress);
               ...
               uintptr_t byteAmount)
{
  ...
  while ((current != 0) && (current < top)) {
    ...
    if ((0 != queryResult) && (0 == (memInfo.State & MEM_COMMIT))) {
      ...
      LPVOID result = VirtualAlloc((LPVOID)commitBase,
                                   commitTop - commitBase,
      ...
      if (NULL != result) {
        ...
        while ((node != NULL) && ((uintptr_t)thisAddress < commitTop)) {
          void* nextAddress = (void*)OMR_MIN(commitTop,
                                             (uintptr_t)node->baseAddress +
   ...
   */
  finalResult = VirtualAlloc((LPVOID)address,
                             (SIZE_T)byteAmount,
                             MEM_COMMIT | J9_EXTRA_TYPE_FLAGS,
                             getProtectionBits(identifier->mode));
  return finalResult;
        ...
        if (didBind) {
          ...
          if (SetThreadIdealProcessorExProc(thisThread, &originalProc, NULL)) {
            ...
            Trc_PRT_vmem_touchOnNumaNode_failedToReturnToComputeResource(
              (uintptr_t)originalProc.Number,
              (uintptr_t)newProc.Number,
  ...
  if (MANAGEMENT_COUNTER_PATH_UNINITIALIZED == isCounterPathInitialized) {
    ...
    if (PDH_MORE_DATA !=
        PdhExpandWildCardPath(
          NULL, "\\Process(*)\\ID Process", pathList, &pathListSize, 0)) {
      goto CLEAN_UP;
    }
    ...
    while ('\0' != *str) {
      ...
      if (value.longValue == pid) {
        ...
        if (MANAGEMENT_COUNTER_PATH_UNINITIALIZED ==
          ...
          if (ERROR_SUCCESS !=
              PdhMakeCounterPath(
                &tmp, PPG_managementCounterPath, &counterPathSize, 0)) {
            MUTEX_EXIT(PPG_managementDataLock);
            goto CLEAN_UP;
          }
