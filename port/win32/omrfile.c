 ...
 * 					If the return value is not the same as @ref unicodeBuffer, port_file_get_unicode_path had to
 * allocate memory to hold the unicode path - the caller is responsible for freeing this memory by calling @ref
 * omrmem_free_memory on the returned value.
 */
    ...
    OMRPortLibrary* portLibrary, const char* utf8Path, wchar_t* unicodeBuffer, uintptr_t unicodeBufferSize)
{
    ...
    if (pathLen * sizeof(wchar_t) <= MAX_PATH) {
        ...
        if (('\\' == utf8Path[0]) && ('\\' == utf8Path[1])) {
            ...
            /*
             * We do not have to account for a null terminator as we are replacing one of the initial "\" in the file
             * path eg. \\server\share -> \\?\UNC\server\share
             * */
...
int32_t omrfile_sync(struct OMRPortLibrary* portLibrary, intptr_t fd)
{
     ...
     * all buffered data from the pipe. 2) The function fails if hFile is a handle to the console output. That is
     * because the console output is not buffered. The function returns FALSE, and GetLastError returns
     ...
     *
     * Manual testing showed that even if we called System.out.print('X') and System.err.print('Y') a parent process
     * would still get receive 'X' and 'Y' (with and without flushing) after child process in terminated.
     *
