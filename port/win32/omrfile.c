 ...
 */
wchar_t*
port_file_get_unicode_path(OMRPortLibrary* portLibrary,
                           const char* utf8Path,
                           ...
                           uintptr_t unicodeBufferSize)
{
  ...
  unicodePath = port_convertFromUTF8(
    portLibrary, utf8Path, unicodeBuffer, unicodeBufferSize);
  ...
  if (pathLen * sizeof(wchar_t) <= MAX_PATH) {
    ...
    fullPathNameLengthInWcharts =
      GetFullPathNameW(unicodePath,
                       0 /* zero means give us back the size we need */,
                       NULL,
                       NULL);
...
omrfile_attr(struct OMRPortLibrary* portLibrary, const char* path)
{
  ...
  unicodePath = port_file_get_unicode_path(
    portLibrary, path, unicodeBuffer, UNICODE_BUFFER_SIZE);
  if (NULL == unicodePath) {
    return -1;
  }
  ...
  if (result == INVALID_FILE_ATTRIBUTES) {
    ...
    result = GetLastError();
    setError =
      portLibrary->error_set_last_error(portLibrary, result, findError(result));
...
omrfile_close(struct OMRPortLibrary* portLibrary, intptr_t fd)
{
  ...
  if ((intptr_t)INVALID_HANDLE_VALUE == fd) {
    ...
    portLibrary->error_set_last_error(
      portLibrary, 0, OMRPORT_ERROR_FILE_INVALID_PARAMETER);
                  ...
                  char* resultbuf)
{
  ...
  /* Convert the filename from UTF8 to Unicode */
  unicodePath = port_file_get_unicode_path(
    portLibrary, newPath, unicodeBuffer, UNICODE_BUFFER_SIZE);
  if (NULL == unicodePath) {
             ...
             int32_t mode)
{
  ...
  if (flags & EsOpenForInherit) {
    ...
    aHandle = CreateFileW(unicodePath,
                          accessMode,
                          shareMode,
                          &sAttrib,
                          createMode,
  ...
  if ((FILE_TYPE_DISK == GetFileType(aHandle)) &&
    ...
    if (0 == CloseHandle(aHandle)) {
      error = GetLastError();
      portLibrary->error_set_last_error(
        portLibrary, error, findError(error)); /* continue */
    }
             ...
             int32_t whence)
{
  ...
  liOffset.LowPart = SetFilePointer(
    (HANDLE)fd, liOffset.LowPart, &liOffset.HighPart, moveMethod);
                ...
                va_list args)
{
  ...
  if (NULL != writeBuffer) {
    stringSize = portLibrary->str_vprintf(
      portLibrary, writeBuffer, bufferSize, format, args);
    ...
    portLibrary->nls_printf(
      portLibrary, J9NLS_ERROR, J9NLS_PORT_FILE_MEMORY_ALLOCATE_FAILURE);
    stringSize = portLibrary->str_vprintf(
      portLibrary, localBuffer, sizeof(localBuffer), format, args);
              ...
              intptr_t nbytes)
{
  ...
  while (nbytes > 0) {
    ...
    if (FALSE ==
        WriteFile(
          handle, (char*)buf + offset, (DWORD)toWrite, &nCharsWritten, NULL)) {
      ...
      errorCode = portLibrary->error_set_last_error(
        portLibrary, errorCode, findError(errorCode));
                   ...
                   int64_t newLength)
{
  ...
  if ((-1 != currentFilePtr) &&
    ...
    if (0 == SetEndOfFile((HANDLE)fd)) {
      ...
      lastError = portLibrary->error_set_last_error(
...
omrfile_lock_bytes(struct OMRPortLibrary* portLibrary,
                   ...
                   uint64_t length)
{
  return omrfile_lock_bytes_helper(
    portLibrary, fd, lockFlags, offset, length, FALSE);
}
             ...
             struct J9FileStat* buf)
{
  ...
  if (result == 0xFFFFFFFF) {
    ...
    return portLibrary->error_set_last_error(
      portLibrary, result, findError(result));
  }
