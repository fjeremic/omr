 ...
 * OMRPORT_TTY_IN, OMRPORT_TTY_OUT and OMRPORT_TTY_ERR are handled as a special
 * case because they don't match UNIX standard stream file descriptors.
 * @param[in] fd returned by port library functions
 * @return windows file handle.
 */
static HANDLE toHandle(OMRPortLibrary *portLibrary, intptr_t fd) {
  switch (fd) {
    case OMRPORT_TTY_IN:
 ...
 * @return windows file handle.
 */
static intptr_t fromHandle(OMRPortLibrary *portLibrary, HANDLE handle) {
...
/*
 * Uses the standard port_convertFromUTF8 and if the resulting unicode path is
 * longer than the Windows defined MAX_PATH, the path is converted to an
 * absolute path and prepended with \\?\ or \\?\UNC if it is a networked path
 * (eg. \\server\share\..)
 *
 * @note	Relative paths are not thread-safe as the current directory is a
 * process global that may change.
 *
 ...
 * representation of @ref utf8Path
 ...
 * unicodeBuffer, port_file_get_unicode_path had to allocate memory to hold the
 * 					unicode path - the caller is responsible for freeing
 * this memory by calling @ref omrmem_free_memory on the returned value.
 */
wchar_t *port_file_get_unicode_path(OMRPortLibrary *portLibrary,
                                    const char *utf8Path,
  ...
  unicodePath = port_convertFromUTF8(portLibrary, utf8Path, unicodeBuffer,
                                     unicodeBufferSize);
  ...
  /* If the file name is longer than (the system-defined) MAX_PATH characters we
   * need to prepend it with \\?\  */
  if (pathLen * sizeof(wchar_t) <= MAX_PATH) {
    ...
    if (('\\' == utf8Path[0]) && ('\\' == utf8Path[1])) {
      ...
      /*
       * We do not have to account for a null terminator as we are replacing one
       * of the initial "\" in the file path eg. \\server\share ->
       * \\?\UNC\server\share
       * */
    ...
    /*
     * We need to prepend the path with "\\?\" or "\\?\UNC", but this format is
     * only supported if the path is fully qualified.
     *
     * The only way to get the fully qualified path is using GetFullPathNameW.
     * The MSDN docs for GetFullPathNameW state that it should not be used in
     * multi-threaded applications, but the docs are fairly clear that this is
     * because the conversion from a relative path to a fully qualified path
     * depends on the current working directory (CWD), which is a process global
     * that may change.
     *
     * However, we can't simply limit the use of GetFullPathNameW to
     * non-absolute paths, because all occurrences of "." and "..", need to be
     * removed. This includes the occurrences that don't simply prepend the
     * path, in which case the CWD is not relevant and where we assume the call
     * to GetFullPathNameW is thread-safe.
     *
     */

    /* find out how big a buffer we need */
    fullPathNameLengthInWcharts = GetFullPathNameW(
        unicodePath, 0 /* zero means give us back the size we need */, NULL,
        NULL);
    ...
    fullPathNameBufferSizeBytes =
        fullPathNameLengthInWcharts * sizeof(wchar_t) + sizeOfPathPrefix;

    fullPathNameBuffer = portLibrary->mem_allocate_memory(
        portLibrary, fullPathNameBufferSizeBytes, OMR_GET_CALLSITE(),
    ...
    getFullPathNameRc = GetFullPathNameW(
        unicodePath, fullPathNameLengthInWcharts, fullPathNameBuffer, NULL);
    ...
    memmove((void *)(fullPathNameBuffer + startOfPathInBuffer),
            fullPathNameBuffer, fullPathNameLengthInWcharts * sizeof(wchar_t));
...
int32_t omrfile_attr(struct OMRPortLibrary *portLibrary, const char *path) {
  DWORD result;
  ...
  unicodePath = port_file_get_unicode_path(portLibrary, path, unicodeBuffer,
                                           UNICODE_BUFFER_SIZE);
  if (NULL == unicodePath) {
    return -1;
  }
  ...
  if (result == INVALID_FILE_ATTRIBUTES) {
    int32_t setError;
    result = GetLastError();
    setError = portLibrary->error_set_last_error(portLibrary, result,
                                                 findError(result));
...
int32_t omrfile_chmod(struct OMRPortLibrary *portLibrary, const char *path,
                      int32_t mode) {
  int32_t attrs;
  int32_t result;
  wchar_t unicodeBuffer[UNICODE_BUFFER_SIZE], *unicodePath;
...
intptr_t omrfile_chown(struct OMRPortLibrary *portLibrary, const char *path,
                       uintptr_t owner, uintptr_t group) {
  return 0;
}

int32_t omrfile_close(struct OMRPortLibrary *portLibrary, intptr_t fd) {
  HANDLE handle = toHandle(portLibrary, fd);

  Trc_PRT_file_close_Entry(fd);

  if ((intptr_t)INVALID_HANDLE_VALUE == fd) {
    /* RTC 100203 Windows 10 does not report an error when closing an invalid
     * handle */
    portLibrary->error_set_last_error(portLibrary, 0,
                                      OMRPORT_ERROR_FILE_INVALID_PARAMETER);
...
const char *omrfile_error_message(struct OMRPortLibrary *portLibrary) {
  return portLibrary->error_last_error_message(portLibrary);
}

void omrfile_findclose(struct OMRPortLibrary *portLibrary,
...
uintptr_t omrfile_findfirst(struct OMRPortLibrary *portLibrary,
  ...
  /* Convert the filename from UTF8 to Unicode */
  unicodePath = port_file_get_unicode_path(portLibrary, newPath, unicodeBuffer,
                                           UNICODE_BUFFER_SIZE);
  if (NULL == unicodePath) {
...
int32_t omrfile_findnext(struct OMRPortLibrary *portLibrary,
...
int64_t omrfile_lastmod(struct OMRPortLibrary *portLibrary, const char *path) {
  int64_t result = -1;
  wchar_t unicodeBuffer[UNICODE_BUFFER_SIZE], *unicodePath;
  ...
  if (NULL != unicodePath) {
    ...
    if (0 ==
        GetFileAttributesExW(unicodePath, GetFileExInfoStandard, &myStat)) {
      int32_t error = GetLastError();
      portLibrary->error_set_last_error(portLibrary, error, findError(error));
    } else {
      /*
       * Search MSDN for 'Converting a time_t Value to a File Time' for
       * following implementation.
       */
      result = ((int64_t)myStat.ftLastWriteTime.dwHighDateTime << 32) |
               (int64_t)myStat.ftLastWriteTime.dwLowDateTime;
...
int64_t omrfile_length(struct OMRPortLibrary *portLibrary, const char *path) {
  int64_t result = -1;
  wchar_t unicodeBuffer[UNICODE_BUFFER_SIZE], *unicodePath;
  ...
  if (NULL != unicodePath) {
    ...
    if (0 ==
        GetFileAttributesExW(unicodePath, GetFileExInfoStandard, &myStat)) {
      int32_t error = GetLastError();
      result = portLibrary->error_set_last_error(portLibrary, error,
                                                 findError(error));
    } else {
...
int64_t omrfile_flength(struct OMRPortLibrary *portLibrary, intptr_t fd) {
  int32_t result = 0;
...
int32_t omrfile_mkdir(struct OMRPortLibrary *portLibrary, const char *path) {
  wchar_t unicodeBuffer[UNICODE_BUFFER_SIZE], *unicodePath;
  int32_t result = 0;
...
int32_t omrfile_move(struct OMRPortLibrary *portLibrary, const char *pathExist,
                     const char *pathNew) {
  ...
  unicodePathExist = port_file_get_unicode_path(
      portLibrary, pathExist, unicodeBufferExist, UNICODE_BUFFER_SIZE);
  if (NULL == unicodePathExist) {
    return -1;
  }
  unicodePathNew = port_file_get_unicode_path(
      portLibrary, pathNew, unicodeBufferNew, UNICODE_BUFFER_SIZE);
...
intptr_t omrfile_open(struct OMRPortLibrary *portLibrary, const char *path,
  ...
  /* this flag allows files to be deleted/renamed while they are still open,
   * which more in line with unix semantics */
  ...
  if (flags & EsOpenForInherit) {
    ...
    aHandle = CreateFileW(unicodePath, accessMode, shareMode, &sAttrib,
                          createMode, flagsAndAttributes, NULL);
  } else {
    aHandle = CreateFileW(unicodePath, accessMode, shareMode, NULL, createMode,
  ...
  if ((FILE_TYPE_DISK == GetFileType(aHandle)) &&
      ((flags & EsOpenTruncate) == EsOpenTruncate)) {
    if (0 == CloseHandle(aHandle)) {
      error = GetLastError();
      portLibrary->error_set_last_error(portLibrary, error,
                                        findError(error)); /* continue */
    }

    aHandle = CreateFileW(unicodePath, accessMode, shareMode, NULL,
                          TRUNCATE_EXISTING, flagsAndAttributes, NULL);
    if (INVALID_HANDLE_VALUE == aHandle) {
      error = GetLastError();
      portLibrary->error_set_last_error(portLibrary, error, findError(error));
      if (unicodeBuffer != unicodePath) {
...
intptr_t omrfile_read(struct OMRPortLibrary *portLibrary, intptr_t fd,
  ...
  /* ReadFile limits the buffer to DWORD size and returns the number of bytes
   * read, so the (DWORD)nbytes cast is functionally correct, though less than
   * ideal. If the caller wants to read more than 2^32 bytes they will have to
   * make more than one call, but the logic of the calling code should already
   * account for this.
   */
  ...
  if (0 == bytesRead) {
    errorCode = portLibrary->error_set_last_error(
        portLibrary, -1, OMRPORT_ERROR_FILE_READ_NO_BYTES_READ);
    Trc_PRT_file_read_Exit(errorCode);
    return -1;
  }
...
int64_t omrfile_seek(struct OMRPortLibrary *portLibrary, intptr_t fd,
  ...
  liOffset.LowPart = SetFilePointer((HANDLE)fd, liOffset.LowPart,
                                    &liOffset.HighPart, moveMethod);
...
void omrfile_shutdown(struct OMRPortLibrary *portLibrary) {}

int32_t omrfile_startup(struct OMRPortLibrary *portLibrary) { return 0; }
...
int32_t omrfile_sync(struct OMRPortLibrary *portLibrary, intptr_t fd) {
  ...
  /*
   * According to MSDN:
   * 1) If hFile is a handle to the server end of a named pipe, the function
   * does not return until the client has read all buffered data from the pipe.
   * 2) The function fails if hFile is a handle to the console output. That is
   * because the console output is not buffered. The function returns FALSE, and
   * GetLastError returns ERROR_INVALID_HANDLE.
   *
   * This behaviour caused JAZZ 44899 defect: Test_SimpleTenantLimitXmt would
   * freeze while waiting for process to terminate while child was waiting for
   * parent to read from STDIN and STDOUT streams of a child.
   *
   * Manual testing showed that even if we called System.out.print('X') and
   * System.err.print('Y') a parent process would still get receive 'X' and 'Y'
   * (with and without flushing) after child process in terminated.
   *
   * This check does not modify original behaviour of the port library because
   * we never actually flushed STDIN and STDOUT handles but some arbitrary
   * handles 1 & 2.
   */
  if (((HANDLE)OMRPORT_TTY_OUT == (HANDLE)fd) ||
      ((HANDLE)OMRPORT_TTY_ERR == (HANDLE)fd)) {
...
int32_t omrfile_unlink(struct OMRPortLibrary *portLibrary, const char *path) {
  wchar_t unicodeBuffer[UNICODE_BUFFER_SIZE], *unicodePath;
  BOOL result;
  ...
  /*PR 93036 - should be able to delete read-only files, so we set the file
   * attribute back to normal*/
...
int32_t omrfile_unlinkdir(struct OMRPortLibrary *portLibrary,
                          const char *path) {
  wchar_t unicodeBuffer[UNICODE_BUFFER_SIZE], *unicodePath;
  BOOL result;
  ...
  /*PR 93036 - should be able to delete read-only dirs, so we set the file
   * attribute back to normal*/
  if (0 == SetFileAttributesW(unicodePath, FILE_ATTRIBUTE_NORMAL)) {
    int32_t error = GetLastError();
    portLibrary->error_set_last_error(portLibrary, error,
...
void omrfile_vprintf(struct OMRPortLibrary *portLibrary, intptr_t fd,
  ...
  /* str_vprintf(.., NULL, ..) result is size of buffer required including the
   * null terminator */
  bufferSize = portLibrary->str_vprintf(portLibrary, NULL, 0, format, args);

  /* use local buffer if possible, allocate a buffer from system memory if local
  ...
  if (NULL != writeBuffer) {
    stringSize = portLibrary->str_vprintf(portLibrary, writeBuffer, bufferSize,
                                          format, args);
    ...
    stringSize = portLibrary->str_vprintf(portLibrary, localBuffer,
                                          sizeof(localBuffer), format, args);
    portLibrary->file_write_text(portLibrary, fd, localBuffer, stringSize);
  }
}

intptr_t omrfile_write(struct OMRPortLibrary *portLibrary, intptr_t fd,
  ...
  while (nbytes > 0) {
    ...
    /* The (DWORD)nbytes downcasts can make toWrite smaller than nbytes,
     * however, the while loop ensures that nbytes of data is still written */
    if (FALSE == WriteFile(handle, (char *)buf + offset, (DWORD)toWrite,
                           &nCharsWritten, NULL)) {
      ...
      errorCode = portLibrary->error_set_last_error(portLibrary, errorCode,
                                                    findError(errorCode));
...
void omrfile_printf(struct OMRPortLibrary *portLibrary, intptr_t fd,
                    const char *format, ...) {
  va_list args;
...
int32_t omrfile_set_length(struct OMRPortLibrary *portLibrary, intptr_t fd,
                           int64_t newLength) {
  int32_t lastError = 0;
  ...
  if ((-1 != currentFilePtr) &&
      (-1 != omrfile_seek(portLibrary, fd, newLength, FILE_BEGIN))) {
    if (0 == SetEndOfFile((HANDLE)fd)) {
      lastError = GetLastError();
      lastError = portLibrary->error_set_last_error(portLibrary, lastError,
                                                    findError(lastError));
...
int32_t omrfile_lock_bytes(struct OMRPortLibrary *portLibrary, intptr_t fd,
                           int32_t lockFlags, uint64_t offset,
                           uint64_t length) {
  return omrfile_lock_bytes_helper(portLibrary, fd, lockFlags, offset, length,
                                   FALSE);
}

int32_t omrfile_unlock_bytes(struct OMRPortLibrary *portLibrary, intptr_t fd,
                             uint64_t offset, uint64_t length) {
  return omrfile_unlock_bytes_helper(portLibrary, fd, offset, length, FALSE);
}

int32_t omrfile_fstat(struct OMRPortLibrary *portLibrary, intptr_t fd,
                      struct J9FileStat *buf) {
  /* TODO: Use GetFileInformationByHandleEx() to get file stats */
  return -1;
}

int32_t omrfile_stat(struct OMRPortLibrary *portLibrary, const char *path,
...

  {
    ...
    result =
        GetFullPathNameW(unicodePath, UNICODE_BUFFER_SIZE, driveBuffer, NULL);
...
int32_t omrfile_stat_filesystem(struct OMRPortLibrary *portLibrary,
  ...
  result =
      GetDiskFreeSpaceExW(driveBuffer, (PULARGE_INTEGER)&buf->freeSizeBytes,
...
intptr_t omrfile_convert_native_fd_to_omrfile_fd(
    struct OMRPortLibrary *portLibrary, intptr_t nativeFD) {
  return fromHandle(portLibrary, (HANDLE)nativeFD);
}

intptr_t omrfile_convert_omrfile_fd_to_native_fd(
