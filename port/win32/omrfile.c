 ...
 * the path is converted to an absolute path and prepended with \\?\ or \\?\UNC if it is a networked path (eg.
 * \\server\share\..)
 *
 ...
 * 					If the return value is not the same as @ref unicodeBuffer, port_file_get_unicode_path had to allocate memory to
 * hold the unicode path - the caller is responsible for freeing this memory by calling @ref omrmem_free_memory on the
 * returned value.
 */
  ...
  OMRPortLibrary* portLibrary, const char* utf8Path, wchar_t* unicodeBuffer, uintptr_t unicodeBufferSize)
{
  ...
  if (pathLen * sizeof(wchar_t) <= MAX_PATH) {
    ...
    /*
     * We need to prepend the path with "\\?\" or "\\?\UNC", but this format is only supported if the path is fully
     * qualified.
     *
     * The only way to get the fully qualified path is using GetFullPathNameW. The MSDN docs for GetFullPathNameW state
     * that it should not be used in multi-threaded applications, but the docs are fairly clear that this is because the
     * conversion from a relative path to a fully qualified path depends on the current working directory (CWD), which
     * is a process global that may change.
     *
     * However, we can't simply limit the use of GetFullPathNameW to non-absolute paths, because all occurrences of "."
     * and "..", need to be removed. This includes the occurrences that don't simply prepend the path, in which case the
     * CWD is not relevant and where we assume the call to GetFullPathNameW is thread-safe.
     *
     */
    ...
    fullPathNameBuffer = portLibrary->mem_allocate_memory(
      portLibrary, fullPathNameBufferSizeBytes, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
...
int32_t omrfile_chmod(struct OMRPortLibrary* portLibrary, const char* path, int32_t mode)
{
  int32_t attrs;
  int32_t result;
  wchar_t unicodeBuffer[UNICODE_BUFFER_SIZE], *unicodePath;
...
intptr_t omrfile_chown(struct OMRPortLibrary* portLibrary, const char* path, uintptr_t owner, uintptr_t group)
{
  return 0;
}
...
void omrfile_findclose(struct OMRPortLibrary* portLibrary, uintptr_t findhandle)
{
  int32_t error = 0;
...
uintptr_t omrfile_findfirst(struct OMRPortLibrary* portLibrary, const char* path, char* resultbuf)
{
...
int32_t omrfile_findnext(struct OMRPortLibrary* portLibrary, uintptr_t findhandle, char* resultbuf)
{
  WIN32_FIND_DATAW findFileData;
...
int32_t omrfile_move(struct OMRPortLibrary* portLibrary, const char* pathExist, const char* pathNew)
{
...
intptr_t omrfile_open(struct OMRPortLibrary* portLibrary, const char* path, int32_t flags, int32_t mode)
{
...
intptr_t omrfile_read(struct OMRPortLibrary* portLibrary, intptr_t fd, void* buf, intptr_t nbytes)
{
  DWORD bytesRead;
  HANDLE handle;
  int32_t errorCode = 0;
...
int64_t omrfile_seek(struct OMRPortLibrary* portLibrary, intptr_t fd, int64_t offset, int32_t whence)
{
  DWORD moveMethod;
...
int32_t omrfile_sync(struct OMRPortLibrary* portLibrary, intptr_t fd)
{
  ...
  /*
   * According to MSDN:
   * 1) If hFile is a handle to the server end of a named pipe, the function does not return until the client has read
   * all buffered data from the pipe. 2) The function fails if hFile is a handle to the console output. That is because
   * the console output is not buffered. The function returns FALSE, and GetLastError returns ERROR_INVALID_HANDLE.
   *
   * This behaviour caused JAZZ 44899 defect: Test_SimpleTenantLimitXmt would freeze while waiting for process to
   * terminate while child was waiting for parent to read from STDIN and STDOUT streams of a child.
   *
   * Manual testing showed that even if we called System.out.print('X') and System.err.print('Y') a parent process would
   * still get receive 'X' and 'Y' (with and without flushing) after child process in terminated.
   *
   * This check does not modify original behaviour of the port library because we never actually flushed STDIN and
   * STDOUT handles but some arbitrary handles 1 & 2.
   */
...
int32_t omrfile_unlinkdir(struct OMRPortLibrary* portLibrary, const char* path)
{
  wchar_t unicodeBuffer[UNICODE_BUFFER_SIZE], *unicodePath;
  BOOL result;
...
void omrfile_vprintf(struct OMRPortLibrary* portLibrary, intptr_t fd, const char* format, va_list args)
{
  ...
  if (sizeof(localBuffer) >= bufferSize) {
    ...
    writeBuffer
      = portLibrary->mem_allocate_memory(portLibrary, bufferSize, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
  }
...
void omrfile_printf(struct OMRPortLibrary* portLibrary, intptr_t fd, const char* format, ...)
{
  va_list args;
...
int32_t omrfile_set_length(struct OMRPortLibrary* portLibrary, intptr_t fd, int64_t newLength)
{
  int32_t lastError = 0;
...
int32_t omrfile_unlock_bytes(struct OMRPortLibrary* portLibrary, intptr_t fd, uint64_t offset, uint64_t length)
{
...
int32_t omrfile_stat_filesystem(
  struct OMRPortLibrary* portLibrary, const char* path, uint32_t flags, struct J9FileStatFilesystem* buf)
{
  ...
  result = GetDiskFreeSpaceExW(
    driveBuffer, (PULARGE_INTEGER)&buf->freeSizeBytes, (PULARGE_INTEGER)&buf->totalSizeBytes, NULL);
  if (0 == result) {
    result = GetLastError();
    return portLibrary->error_set_last_error(portLibrary, result, findError(result));
  }
...
intptr_t omrfile_convert_native_fd_to_omrfile_fd(struct OMRPortLibrary* portLibrary, intptr_t nativeFD)
{
  return fromHandle(portLibrary, (HANDLE)nativeFD);
}

intptr_t omrfile_convert_omrfile_fd_to_native_fd(struct OMRPortLibrary* portLibrary, intptr_t omrfileFD)
{
