...
static void fillInWin32SignalInfo(struct OMRPortLibrary *portLibrary,
                                  omrsig_handler_fn handler,
...
int structuredExceptionHandler(struct OMRPortLibrary *portLibrary,
...
static uint32_t infoForControl(struct OMRPortLibrary *portLibrary,
                               struct OMRWin32SignalInfo *info, int32_t index,
                               const char **name, void **value);
static intptr_t setCurrentSignal(struct OMRPortLibrary *portLibrary,
                                 intptr_t signal);
...
static OMRWin32AsyncHandlerRecord *createAsyncHandlerRecord(
  ...
  if (0 == (signalOptions & OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS)) {
    __try {
      *result = fn(portLibrary, fn_arg);
      ...
      /* this code is only reachable if the handler returned
       * OMRPORT_SIG_EXCEPTION_RETURN */
      *result = 0;
...
int32_t omrsig_set_async_signal_handler(struct OMRPortLibrary *portLibrary,
  ...
  if ((NULL == cursor) && (0 != flags)) {
    OMRWin32AsyncHandlerRecord *record =
...
int32_t omrsig_set_single_async_signal_handler(
  ...
  if (!foundHandler && (0 != portlibSignalFlag)) {
    OMRWin32AsyncHandlerRecord *record = createAsyncHandlerRecord(
...
static void fillInWin32SignalInfo(struct OMRPortLibrary *portLibrary,
  ...
  signalInfo->type = mapWin32ExceptionToPortlibType(
      exceptionInfo->ExceptionRecord->ExceptionCode);
...
int32_t omrsig_set_options(struct OMRPortLibrary *portLibrary,
                           uint32_t options) {
  uint32_t supportedFlags = OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS |
...
static OMRWin32AsyncHandlerRecord *createAsyncHandlerRecord(
    ...
    void *handler_arg, uint32_t flags) {
  OMRWin32AsyncHandlerRecord *record =
      (OMRWin32AsyncHandlerRecord *)portLibrary->mem_allocate_memory(
