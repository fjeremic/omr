...
static HANDLE
openFileInCWD(struct OMRPortLibrary* portLibrary,
              char* fileNameBuff,
              uint32_t fileNameBuffSize);
static HANDLE
openFileFromEnvVar(struct OMRPortLibrary* portLibrary,
                   char* envVarName,
                   char* fileNameBuff,
                   uint32_t fileNameBuffSize);
static HANDLE
openFileFromReg(const char* keyName,
                const char* valName,
                char* fileNameBuff,
                uint32_t fileNameBuffSize);
static void
               ...
               void* userData)
{
  ...
  if (0 != args.errorCode) {
    portLibrary->error_set_last_error(
      portLibrary, args.errorCode, convertError(args.errorCode));
    portLibrary->str_printf(portLibrary,
                            filename,
                            EsMaxPath,
...
#define MAX_MINIDUMP_ATTEMPTS 10
  ...
  for (i = 0; i < MAX_MINIDUMP_ATTEMPTS; i++) {
     ...
     */
    retVal =
      args->dmp_function(GetCurrentProcess(),
                         GetCurrentProcessId(),
                         hFile,
                   ...
                   uint32_t fileNameBuffSize)
{
  ...
  retCode = portLibrary->sysinfo_get_env(
    portLibrary, envVarName, fileNameBuff, fileNameBuffSize);
  if (retCode != 0) {
    return INVALID_HANDLE_VALUE;
  }

  retCode =
    portLibrary->sysinfo_get_env(portLibrary, J9USE_UNIQUE_DUMP_NAMES, NULL, 0);
  if (-1 != retCode) {
    pidStore[0] = '-';
                ...
                uint32_t fileNameBuffSize)
{
  ...
  lRet =
    RegQueryValueEx(hKey, valName, NULL, NULL, (LPBYTE)fileNameBuff, &buffUsed);
  if (lRet != ERROR_SUCCESS) {
    return INVALID_HANDLE_VALUE;
  }
