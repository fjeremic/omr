...
uintptr_t omrdump_create(struct OMRPortLibrary* portLibrary, char* filename, char* dumpType, void* userData)
{
  ...
  if (hFile == (HANDLE)-1) {
    portLibrary->str_printf(
      portLibrary, filename, EsMaxPath, "Dump failed - could not create dump file:\n"); /*make sure useful*/
    return 1;
  }
  ...
  if (dump_fn == NULL) {
    portLibrary->str_printf(
      portLibrary, filename, EsMaxPath, "Dump failed - could not link %s in %s\n", MINIDUMPWRITEDUMP, DBGHELP_DLL);
    return 1;
  }
  ...
  if ((portLibrary->file_close(portLibrary, (int32_t)(intptr_t)hFile)) == -1) {
    portLibrary->str_printf(
      portLibrary, filename, EsMaxPath, "Dump failed - could not close dump file.\n"); /*make sure useful*/
    return 1;
  }
  ...
  struct OMRPortLibrary* portLibrary, char* envVarName, char* fileNameBuff, uint32_t fileNameBuffSize)
{
  ...
  hFile = CreateFileA(fileNameBuff, GENERIC_READ | GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
