...
static HANDLE openFileInCWD(struct OMRPortLibrary *portLibrary,
                            char *fileNameBuff, uint32_t fileNameBuffSize);
static HANDLE openFileFromEnvVar(struct OMRPortLibrary *portLibrary,
                                 char *envVarName, char *fileNameBuff,
                                 uint32_t fileNameBuffSize);
...
uintptr_t omrdump_create(struct OMRPortLibrary *portLibrary, char *filename,
  ...
  if (*filename == '\0') {
    ...
    hFile =
        openFileFromReg(DUMP_FNAME_KEY, DUMP_FNAME_VALUE, filename, EsMaxPath);
    if (hFile == INVALID_HANDLE_VALUE) {
      hFile =
          openFileFromEnvVar(portLibrary, "USERPROFILE", filename, EsMaxPath);
      if (hFile == INVALID_HANDLE_VALUE) {
  ...
  if (dllHandle == NULL) {
    portLibrary->str_printf(portLibrary, filename, EsMaxPath,
                            "Dump failed - could not load library %s\n",
                            DBGHELP_DLL);
    return 1;
  }
  ...
  /* call createCrashDumpFile on a different thread so we can walk the stack
   * back to our code */
  ...
  /* Check for errors passed back from the dump thread via the WriteDumpFileArgs
   * args structure */
  if (0 != args.errorCode) {
    portLibrary->error_set_last_error(portLibrary, args.errorCode,
                                      convertError(args.errorCode));
    portLibrary->str_printf(portLibrary, filename, EsMaxPath,
                            "Dump failed, error code %s",
...
/* will remove private from name once duplicate functionality removed from
 * GPHandler */
static HINSTANCE loadDumpLib(void) {
...
/**
 * Calls the Windows MiniDumpWriteDump API to create a core dump. This function
 * is run on its own thread.
 *
 * @param[in] args  pointer to a WriteDumpFileArgs structure containing the
 * MiniDumpWriteDump function pointer, dump file handle, dump parameters and
 * return code
 * @return  none
 */
static void writeDumpFile(PWriteDumpFileArgs args) {
#define MAX_MINIDUMP_ATTEMPTS 10
  ...
  for (i = 0; i < MAX_MINIDUMP_ATTEMPTS; i++) {
    ...
    /* Dump flags match the defaults used when a dump is created via Windows
     * Task Manager. The dump flags for a dump can be viewed in windbg using the
     * ".dumpdebug" command. (The command is not documented in the help but will
     * display the dump flags and streams included in the dump.)
     */
    ...
    if (TRUE == retVal) {
      ...
      args->errorCode =
          GetLastError(); /* retrieve Windows error code (HRESULT) */
...
static HANDLE openFileFromEnvVar(struct OMRPortLibrary *portLibrary,
  ...
  retCode = portLibrary->sysinfo_get_env(portLibrary, envVarName, fileNameBuff,
                                         fileNameBuffSize);
  if (retCode != 0) {
    return INVALID_HANDLE_VALUE;
  }

  retCode = portLibrary->sysinfo_get_env(portLibrary, J9USE_UNIQUE_DUMP_NAMES,
                                         NULL, 0);
  if (-1 != retCode) {
    pidStore[0] = '-';
  ...
  hFile = CreateFileA(fileNameBuff, GENERIC_READ | GENERIC_WRITE, 0, NULL,
...
static HANDLE openFileFromReg(const char *keyName, const char *valName,
  ...
  lRet = RegQueryValueEx(hKey, valName, NULL, NULL, (LPBYTE)fileNameBuff,
                         &buffUsed);
  if (lRet != ERROR_SUCCESS) {
    return INVALID_HANDLE_VALUE;
  }
...
int32_t omrdump_startup(struct OMRPortLibrary *portLibrary) {
  ...
  PPG_dbgHlpLibraryFunctions =
      (Dbg_Entrypoints *)portLibrary->mem_allocate_memory(
          portLibrary, sizeof(Dbg_Entrypoints), OMR_GET_CALLSITE(),
