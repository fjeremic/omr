...
int32_t omrfile_blockingasync_close(struct OMRPortLibrary *portLibrary,
                                    intptr_t fd) {
  return omrfile_close(portLibrary, fd);
}

intptr_t omrfile_blockingasync_open(struct OMRPortLibrary *portLibrary,
...
int32_t omrfile_blockingasync_lock_bytes(struct OMRPortLibrary *portLibrary,
...
int32_t omrfile_blockingasync_unlock_bytes(struct OMRPortLibrary *portLibrary,
                                           intptr_t fd, uint64_t offset,
                                           uint64_t length) {
  return omrfile_unlock_bytes_helper(portLibrary, fd, offset, length, TRUE);
}

intptr_t omrfile_blockingasync_read(struct OMRPortLibrary *portLibrary,
  ...
  overlapped.Offset =
      SetFilePointer(fileHandle, 0, (PLONG)&highOffset, FILE_CURRENT);
...
error:
  errorCode = portLibrary->error_set_last_error(portLibrary, GetLastError(),
                                                findError(GetLastError()));
  Trc_PRT_file_blockingasync_read_Exit(errorCode);
  return -1;
}

intptr_t omrfile_blockingasync_write(struct OMRPortLibrary *portLibrary,
  ...
  overlapped.OffsetHigh = highOffset;
  filePtr =
      (DWORDLONG)overlapped.Offset | ((DWORDLONG)overlapped.OffsetHigh << 32);
  ...
  while (nbytes > 0) {
    ...
    result = WriteFile(fileHandle, (char *)buf + offset, (DWORD)toWrite,
...
int32_t omrfile_blockingasync_set_length(struct OMRPortLibrary *portLibrary,
                                         intptr_t fd, int64_t newLength) {
  return omrfile_set_length(portLibrary, fd, newLength);
}

int64_t omrfile_blockingasync_flength(struct OMRPortLibrary *portLibrary,
                                      intptr_t fd) {
  return omrfile_flength(portLibrary, fd);
}

static void J9THREAD_PROC tls_blockingasync_finalizer(void *entry) {
  HANDLE overlappedHandle =
      omrthread_tls_get(omrthread_self(), tlsKeyOverlappedHandle);
  if (NULL != overlappedHandle) {
    CloseHandle(overlappedHandle);
  }
}

int32_t omrfile_blockingasync_startup(struct OMRPortLibrary *portLibrary) {
  int32_t lastError = 0;

  Trc_PRT_file_blockingasync_startup_Entry();
  if (omrthread_tls_alloc_with_finalizer(&tlsKeyOverlappedHandle,
                                         tls_blockingasync_finalizer)) {
    lastError = portLibrary->error_set_last_error(
        portLibrary, -1, OMRPORT_ERROR_FILE_FAILED_TO_ALLOCATE_TLS);
