                   ...
                   intptr_t nbytes)
{
  ...
  /* if there are any non-ASCII chars, convert to Unicode and then to the local
   * code page */
  if (highchars) {
    ...
    if (newLen > sizeof(wStackBuf)) {
      wBuf = portLibrary->mem_allocate_memory(
        portLibrary, newLen, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
    }
    if (wBuf) {
      ...
      if (in) {
        ...
        intptr_t mbLen = WideCharToMultiByte(
          codePage, 0, wBuf, (int)wLen, NULL, 0, NULL, NULL);
        if (mbLen > 0) {
          if (mbLen > sizeof(stackBuf)) {
            ...
            /* if we couldn't allocate the buffer, just output the data the way
             * it was */
          }
          if (newBuf) {
            WideCharToMultiByte(
              codePage, 0, wBuf, (int)wLen, newBuf, (int)mbLen, NULL, NULL);
            buf = newBuf;
            nbytes = mbLen;
          }
        }
