  ...
  if (NULL == PPG_dbgHlpLibraryFunctions->hDbgHelpLib) {
    ...
    if (globalDbgHelpVersionInfo.MajorVersion >= 4) {
      ...
      PPG_dbgHlpLibraryFunctions->SymInitializeW =
        (SYMINITIALIZEW)GetProcAddress(PPG_dbgHlpLibraryFunctions->hDbgHelpLib,
                                       "SymInitializeW");
  ...
  if (0 == load_dbg_functions(portLibrary) &&
    ...
    for (frameNumber = 0; rc != FALSE; frameNumber++) {
      ...
      if (heap == NULL) {
        ...
        *nextFrame = portLibrary->heap_allocate(
          portLibrary, heap, sizeof(J9PlatformStackFrame));
      }
  ...
  if (0 == load_dbg_functions(portLibrary) &&
    ...
    for (i = 0, frame = threadInfo->callstack; frame != NULL;
      ...
      rc = PPG_dbgHlpLibraryFunctions->SymFromAddr(
        process,
        (uint64_t)frame->instruction_pointer,
        &symbol_offset,
        symbol_info);
      if (rc == FALSE) {
        symbol_name = "";
      ...
      if (symbol_name[0] != '\0') {
        cursor += omrstr_printf(portLibrary,
                                cursor,
