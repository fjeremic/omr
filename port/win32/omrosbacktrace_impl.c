...
static wchar_t *buildPDBPath(struct OMRPortLibrary *portLibrary,
                             HANDLE process) {
  ...
  if (EnumProcessModules(process, hModules, sizeof(hModules), &cbNeeded) ==
    ...
    for (i = 0; i < nModules; i++) {
      modulePathLength =
          GetModuleFileNameW(hModules[i], modulePath, (EsMaxPath + 1));
      if ((0 != modulePathLength) && (modulePathLength < (EsMaxPath + 1))) {
        ...
        *cursor = '\0';
        ...
        if (NULL == PPG_pdbData.searchPath) {
          /* the constructed pdb path is empty, so just cat the first entry onto
           * the null string */
          ...
          /* the constructed pdb path has entries, so search it to prevent
           * adding the same entry again */
...
/* This function loads the table of debug library function pointers, if not
 * already loaded. Once loaded, we keep the DLL and function table for the life
 * of the JVM. PPG_dbgHlpLibraryFunctions->hDbgHelpLib indicates whether we have
 * already loaded the library and function pointers.
 *
 ...
 */
int32_t load_dbg_functions(struct OMRPortLibrary *portLibrary) {
  ...
  if (NULL == PPG_dbgHlpLibraryFunctions->hDbgHelpLib) {
    ...
    if (NULL == PPG_dbgHlpLibraryFunctions->hDbgHelpLib) {
      PPG_dbgHlpLibraryFunctions->hDbgHelpLib =
          (HINSTANCE)omrgetdbghelp_loadDLL();
    }
    ...
    /* found and loaded the debug help library, so now locate the entry points
     * we need */
    PPG_dbgHlpLibraryFunctions->ImagehlpApiVersion =
        (IMAGEHLPAPIVERSION)GetProcAddress(
            PPG_dbgHlpLibraryFunctions->hDbgHelpLib, "ImagehlpApiVersion");

    /* check which debug help library was loaded by this process */
    globalDbgHelpVersionInfo =
        *(PPG_dbgHlpLibraryFunctions->ImagehlpApiVersion());

    /* ensure that we can load the minimum required version of the dbghelp API
     */
    if (globalDbgHelpVersionInfo.MajorVersion >= 4) {
      PPG_dbgHlpLibraryFunctions->SymGetOptions = (SYMGETOPTIONS)GetProcAddress(
          PPG_dbgHlpLibraryFunctions->hDbgHelpLib, "SymGetOptions");
      PPG_dbgHlpLibraryFunctions->SymSetOptions = (SYMSETOPTIONS)GetProcAddress(
          PPG_dbgHlpLibraryFunctions->hDbgHelpLib, "SymSetOptions");
      PPG_dbgHlpLibraryFunctions->SymInitializeW =
          (SYMINITIALIZEW)GetProcAddress(
              PPG_dbgHlpLibraryFunctions->hDbgHelpLib, "SymInitializeW");
      PPG_dbgHlpLibraryFunctions->SymCleanup = (SYMCLEANUP)GetProcAddress(
          PPG_dbgHlpLibraryFunctions->hDbgHelpLib, "SymCleanup");
      PPG_dbgHlpLibraryFunctions->SymGetModuleBase64 =
          (SYMGETMODULEBASE64)GetProcAddress(
              PPG_dbgHlpLibraryFunctions->hDbgHelpLib, "SymGetModuleBase64");
      PPG_dbgHlpLibraryFunctions->SymFunctionTableAccess64 =
          (SYMFUNCTIONTABLEACCESS64)GetProcAddress(
              PPG_dbgHlpLibraryFunctions->hDbgHelpLib,
              "SymFunctionTableAccess64");
      PPG_dbgHlpLibraryFunctions->StackWalk64 = (STACKWALK64)GetProcAddress(
          PPG_dbgHlpLibraryFunctions->hDbgHelpLib, "StackWalk64");
      PPG_dbgHlpLibraryFunctions->SymFromAddr = (SYMFROMADDR)GetProcAddress(
          PPG_dbgHlpLibraryFunctions->hDbgHelpLib, "SymFromAddr");
      PPG_dbgHlpLibraryFunctions->SymGetLineFromAddr64 =
          (SYMGETLINEFROMADDR64)GetProcAddress(
              PPG_dbgHlpLibraryFunctions->hDbgHelpLib, "SymGetLineFromAddr64");
      PPG_dbgHlpLibraryFunctions->SymGetModuleInfo64 =
          (SYMGETMODULEINFO64)GetProcAddress(
              PPG_dbgHlpLibraryFunctions->hDbgHelpLib, "SymGetModuleInfo64");
    } else {
...
/* This function sets symbol options and initializes the debug PDB path and
 * symbol handler, if not already initialized. We reset the symbol options and
 * free loaded symbols on completion of the native stack walk for javacores.
 * PPG_pdbData.numberOfModules indicates whether we have currently initialized
 * symbols.
 *
 * Note - this is called single threaded, e.g. under PPG_osBacktrace_Mutex
 *
 ...
 * @return - 0 on success, -1 on failure
 */
int32_t load_dbg_symbols(struct OMRPortLibrary *portLibrary) {
  HANDLE process;
  ...
  if (0 == PPG_pdbData.numberOfModules) {
    ...
    PPG_dbgHlpLibraryFunctions->SymSetOptions(
        SYMOPT_DEFERRED_LOADS | SYMOPT_LOAD_LINES | SYMOPT_CASE_INSENSITIVE |
    ...
    if (FALSE == PPG_dbgHlpLibraryFunctions->SymInitializeW(
                     process, PPG_pdbData.searchPath, TRUE)) {
...
/* This function frees loaded debug symbols and resets debug symbol options.
 * PPG_pdbData.numberOfModules set to 0 indicates that we have unloaded the
 * debug symbols and reset the options.
 *
 * Note - this function is called single threaded, e.g. under
 * PPG_osBacktrace_Mutex
 *
 * @param portLibrary - a pointer to an initialized port library
 * @returns - none
 */
void free_dbg_symbols(struct OMRPortLibrary *portLibrary) {
  if (NULL != PPG_dbgHlpLibraryFunctions) {
    if (NULL != PPG_dbgHlpLibraryFunctions->hDbgHelpLib) {
...
/* This function constructs a backtrace from a CPU context. Generally there are
 * only one or two values in the context that are actually used to construct the
 * stack but these vary by platform so arn't detailed here. If no heap is
 * specified then this function will use malloc to allocate the memory necessary
 * for the stack frame structures which must be freed by the caller.
 *
 * @param portLbirary a pointer to an initialized port library
 * @param threadInfo the thread structure we want  to attach the backtrace to.
 * Must not be NULL.
 * @param heap a heap from which to allocate any necessary memory. If NULL
 * malloc is used instead.
 * @param signalInfo a platform signal context. If not null the context held in
 * threadInfo is replaced with the signal context before the backtrace is
 * generated.
 *
 * @return the number of frames in the backtrace.
 */
uintptr_t omrintrospect_backtrace_thread_raw(struct OMRPortLibrary *portLibrary,
  ...
  /* if it's a stack overflow then we need to supress generation of a backtrace
   * or we'll overflow the stack buffer */
  ...
  thread = OpenThread(THREAD_GET_CONTEXT | THREAD_QUERY_INFORMATION, FALSE,
                      (DWORD)threadInfo->thread_id);
  if (thread == NULL) {
  ...
  if (0 == load_dbg_functions(portLibrary) &&
      0 == load_dbg_symbols(portLibrary)) {
    ...
    for (frameNumber = 0; rc != FALSE; frameNumber++) {
      ...
      if (heap == NULL) {
        *nextFrame = portLibrary->mem_allocate_memory(
            portLibrary, sizeof(J9PlatformStackFrame), OMR_GET_CALLSITE(),
      ...
      /*
       * this truncates from DWORD64 to uintptr_t on Win32 systems. However
       * win32 is exclusively little endian so this works as expected
       */
...
/* This function takes a thread structure already populated with a backtrace by
 * omrintrospect_backtrace_thread and looks up the symbols for the frames. The
 * format of the string generated is: symbol_name (statement_id
 * instruction_pointer [module+offset]) If it isn't possible to determine any of
 * the items in the string then they are omitted. If no heap is specified then
 * this function will use malloc to allocate the memory necessary for the
 * symbols which must be freed by the caller.
 *
 * @param portLbirary a pointer to an initialized port library
 ...
 */
uintptr_t omrintrospect_backtrace_symbols_raw(
  ...
  /* CAUTION: symbol_info is mapped to the start of buffer, this is because the
   * Name field in */
  /* the SYMBOL_INFO structure is a 1 char array positioned at the end of the
   * structure */
  /* The APIs that use this structure rely on there being extra memory available
   * at the end */
  /* of the structure */
  BYTE buffer[256];
  ...
  if (0 == load_dbg_functions(portLibrary) &&
    ...
    for (i = 0, frame = threadInfo->callstack; frame != NULL;
      ...
      rc = PPG_dbgHlpLibraryFunctions->SymGetModuleInfo64(
          process, frame->instruction_pointer, &module_info);
      if (rc == FALSE) {
        module_name = "";
      } else {
      ...
      rc = PPG_dbgHlpLibraryFunctions->SymFromAddr(
          process, (uint64_t)frame->instruction_pointer, &symbol_offset,
          symbol_info);
      if (rc == FALSE) {
        symbol_name = "";
      } else {
      ...
      rc = PPG_dbgHlpLibraryFunctions->SymGetLineFromAddr64(
          process, (DWORD64)frame->instruction_pointer, &disp, &line_info);
      if (rc == FALSE) {
        file_name = "";
      } else {
      ...
      if (symbol_name[0] != '\0') {
        cursor += omrstr_printf(portLibrary, cursor,
                                sizeof(output_buf) - (cursor - output_buf),
                                "%s", symbol_name);
        cursor += omrstr_printf(portLibrary, cursor,
                                sizeof(output_buf) - (cursor - output_buf),
                                "+0x%x ", symbol_offset);
      ...
      *(cursor++) = '(';
      if (file_name[0] != '\0') {
        cursor += omrstr_printf(portLibrary, cursor,
                                sizeof(output_buf) - (cursor - output_buf),
                                "%s:%i, ", file_name, line_info.LineNumber);
      }
      cursor += omrstr_printf(portLibrary, cursor,
                              sizeof(output_buf) - (cursor - output_buf),
      ...
      *cursor = 0;
      ...
      if (heap == NULL) {
        frame->symbol = portLibrary->mem_allocate_memory(
            portLibrary, length, OMR_GET_CALLSITE(),
            OMRMEM_CATEGORY_PORT_LIBRARY);
      } else {
