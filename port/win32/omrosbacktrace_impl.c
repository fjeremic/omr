...
static wchar_t* buildPDBPath(struct OMRPortLibrary* portLibrary, HANDLE process)
{
  ...
  if (NULL == PPG_dbgHlpLibraryFunctions->hDbgHelpLib) {
    ...
    if (globalDbgHelpVersionInfo.MajorVersion >= 4) {
      ...
      PPG_dbgHlpLibraryFunctions->SymFunctionTableAccess64
        = (SYMFUNCTIONTABLEACCESS64)GetProcAddress(PPG_dbgHlpLibraryFunctions->hDbgHelpLib, "SymFunctionTableAccess64");
...
int32_t load_dbg_symbols(struct OMRPortLibrary* portLibrary)
{
  ...
  if (0 == PPG_pdbData.numberOfModules) {
    ...
    PPG_dbgHlpLibraryFunctions->SymSetOptions(
      SYMOPT_DEFERRED_LOADS | SYMOPT_LOAD_LINES | SYMOPT_CASE_INSENSITIVE | SYMOPT_LOAD_ANYTHING);
 ...
 */
uintptr_t omrintrospect_backtrace_thread_raw(
  ...
  if (0 == load_dbg_functions(portLibrary) && 0 == load_dbg_symbols(portLibrary)) {
    ...
    for (frameNumber = 0; rc != FALSE; frameNumber++) {
      rc = PPG_dbgHlpLibraryFunctions->StackWalk64(
        ...
        NULL, PPG_dbgHlpLibraryFunctions->SymFunctionTableAccess64, PPG_dbgHlpLibraryFunctions->SymGetModuleBase64,
        NULL);
      ...
      if (heap == NULL) {
        *nextFrame = portLibrary->mem_allocate_memory(
          portLibrary, sizeof(J9PlatformStackFrame), OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
      } else {
  ...
  if (0 == load_dbg_functions(portLibrary) && 0 == load_dbg_symbols(portLibrary)) {
    ...
    for (i = 0, frame = threadInfo->callstack; frame != NULL; frame = frame->parent_frame, i++) {
      ...
      rc = PPG_dbgHlpLibraryFunctions->SymFromAddr(
        process, (uint64_t)frame->instruction_pointer, &symbol_offset, symbol_info);
      if (rc == FALSE) {
        symbol_name = "";
      } else {
      ...
      if (symbol_name[0] != '\0') {
        ...
        cursor
          += omrstr_printf(portLibrary, cursor, sizeof(output_buf) - (cursor - output_buf), "+0x%x ", symbol_offset);
      }
      *(cursor++) = '(';
      if (file_name[0] != '\0') {
        cursor += omrstr_printf(
          portLibrary, cursor, sizeof(output_buf) - (cursor - output_buf), "%s:%i, ", file_name, line_info.LineNumber);
      }
      cursor += omrstr_printf(
        portLibrary, cursor, sizeof(output_buf) - (cursor - output_buf), "0x%p", frame->instruction_pointer);
      if (module_name[0] != '\0') {
        cursor += omrstr_printf(
          portLibrary, cursor, sizeof(output_buf) - (cursor - output_buf), " [%s+0x%x]", module_name, module_offset);
      }
      *(cursor++) = ')';
      *cursor = 0;
