 ...
 */
static uintptr_t omrsyslog_write_utf8(struct OMRPortLibrary *portLibrary,
                                      uintptr_t flags, const char *message) {
  if (NULL != portLibrary->portGlobals && NULL != PPG_syslog_handle) {
    switch (flags) {
    case J9NLS_ERROR:
      return ReportEvent(PPG_syslog_handle, EVENTLOG_ERROR_TYPE, 0, MSG_ERROR,
                         NULL, 1, 0, (LPCTSTR *)&message, NULL);
    case J9NLS_WARNING:
      return ReportEvent(PPG_syslog_handle, EVENTLOG_WARNING_TYPE, 0,
                         MSG_WARNING, NULL, 1, 0, (LPCTSTR *)&message, NULL);
    case J9NLS_INFO:
    default:
      return ReportEvent(PPG_syslog_handle, EVENTLOG_INFORMATION_TYPE, 0,
                         MSG_INFO, NULL, 1, 0, (LPCTSTR *)&message, NULL);
    }
  }
  return FALSE;
}
 ...
 * @return Boolean: true on success, false on failure
 */
static uintptr_t omrsyslog_write_unicode(struct OMRPortLibrary *portLibrary,
                                         uintptr_t flags,
                                         const wchar_t *message) {
  if (NULL != portLibrary->portGlobals && NULL != PPG_syslog_handle) {
    switch (flags) {
    case J9NLS_ERROR:
      return ReportEventW(PPG_syslog_handle, EVENTLOG_ERROR_TYPE, 0, MSG_ERROR,
                          NULL, 1, 0, (LPCWSTR *)&message, NULL);
    case J9NLS_WARNING:
      return ReportEventW(PPG_syslog_handle, EVENTLOG_WARNING_TYPE, 0,
                          MSG_WARNING, NULL, 1, 0, (LPCWSTR *)&message, NULL);
    case J9NLS_INFO:
    default:
      return ReportEventW(PPG_syslog_handle, EVENTLOG_INFORMATION_TYPE, 0,
                          MSG_INFO, NULL, 1, 0, (LPCWSTR *)&message, NULL);
    }
  }
  return FALSE;
}
 ...
 * @return Boolean: true on success, false on failure
 */
uintptr_t omrsyslog_write(struct OMRPortLibrary *portLibrary, uintptr_t flags,
                          const char *message) {
  ...
  wchar_t *const unicodeMsg = portLibrary->mem_allocate_memory(
      portLibrary, unicodeMsgLen, OMR_GET_CALLSITE(),
      OMRMEM_CATEGORY_PORT_LIBRARY);

  if (NULL != unicodeMsg) {
    /* If we got here, we are guaranteed to have enough space for the converted
     * string and the null termination char. Under these circumstances,
     * str_convert routine guarantees that the resulting string will be null
     * terminated.
     */
    const int32_t convRes = portLibrary->str_convert(
        portLibrary, J9STR_CODE_MUTF8, J9STR_CODE_WIDE, message, messageLen,
        (char *)unicodeMsg, unicodeMsgLen);
...
uintptr_t syslogOpen(struct OMRPortLibrary *portLibrary, uintptr_t flags) {
  ...
  /* note that a registry entry for that event source name needs to be found for
   * the messages to get logged correctly */
  /* registry key:
   * HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\EventLog\Application\IBM
   * Java */
