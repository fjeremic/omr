...
omrsyslog_write(struct OMRPortLibrary* portLibrary, uintptr_t flags, const char* message)
{
    ...
    wchar_t* const unicodeMsg = portLibrary->mem_allocate_memory(portLibrary, unicodeMsgLen, OMR_GET_CALLSITE(),
    ...
    if (NULL != unicodeMsg) {
		 ...
		 */
        const int32_t convRes = portLibrary->str_convert(portLibrary, J9STR_CODE_MUTF8, J9STR_CODE_WIDE, message,
