 ...
 */
static uintptr_t
omrsyslog_write_utf8(struct OMRPortLibrary* portLibrary,
                     uintptr_t flags,
                     const char* message)
{
  if (NULL != portLibrary->portGlobals && NULL != PPG_syslog_handle) {
    switch (flags) {
      case J9NLS_ERROR:
        return ReportEvent(PPG_syslog_handle,
                           EVENTLOG_ERROR_TYPE,
                           0,
                           MSG_ERROR,
                           NULL,
 ...
 */
static uintptr_t
omrsyslog_write_unicode(struct OMRPortLibrary* portLibrary,
                        uintptr_t flags,
                        const wchar_t* message)
{
  if (NULL != portLibrary->portGlobals && NULL != PPG_syslog_handle) {
    switch (flags) {
      case J9NLS_ERROR:
        return ReportEventW(PPG_syslog_handle,
                            EVENTLOG_ERROR_TYPE,
                            0,
                            MSG_ERROR,
                            NULL,
                ...
                const char* message)
{
  ...
  if (NULL != unicodeMsg) {
     ...
     * terminated.
     */
    const int32_t convRes = portLibrary->str_convert(portLibrary,
                                                     J9STR_CODE_MUTF8,
                                                     J9STR_CODE_WIDE,
                                                     message,
                                                     messageLen,
