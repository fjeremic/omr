...
#include <winsock2.h>
    ...
    (_WIN32_WINNT_MAXVER >= _WIN32_WINNT_WINBLUE)
 ...
 *
 * @return A null-terminated string describing the CPU architecture of the
 * hardware, NULL on error.
 *
 * @note portLibrary is responsible for allocation/deallocation of returned
 * buffer.
 * @note See http://www.tolstoy.com/samizdat/sysprops.html for good values to
 * return.
 */
...
intptr_t omrsysinfo_get_env(struct OMRPortLibrary *portLibrary,
   ...
   * If the name is larger than the buffer, a new buffer is dynamically
   * allocated.
   */
  envVarWideCharName =
      port_convertFromUTF8(portLibrary, envVar, envVarNameConversionBuffer,
                           ENVVAR_NAME_BUFFER_LENGTH);
  if (NULL == envVarWideCharName) {
    return -1;
  }
  rc = GetEnvironmentVariableW(
      envVarWideCharName, envVarWideCharValue,
      ENVVAR_VALUE_BUFFER_LENGTH); /* Try with the stack buffer first */
  if ((ENVVAR_VALUE_BUFFER_LENGTH <= rc) && (0 != rc)) {
    /* if the value fit, rc, which does not include the null, is at most
     * ENVVAR_VALUE_BUFFER_LENGTH-1. */
    DWORD envVarValueLength = rc;
    envVarWideCharValue = (wchar_t *)portLibrary->mem_allocate_memory(
        portLibrary, envVarValueLength * sizeof(wchar_t), OMR_GET_CALLSITE(),
        OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL != envVarWideCharValue) {
      rc = GetEnvironmentVariableW(envVarWideCharName, envVarWideCharValue,
                                   envVarValueLength);
    } else {
  ...
  if (0 == rc) {
    ...
    rc = WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS,
                             envVarWideCharValue, -1, NULL, 0, NULL, NULL);
    if (0 == rc) { /* error, probably bogus Unicode */
      result = -1;
    } else if (rc > bufSize) {
      /* Caller-supplied buffer is too small. Return the actual number of bytes
       * required. */
      result = rc;
    } else {
      /* Do the conversion for real. */
      rc = WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS,
                               envVarWideCharValue, -1, infoString,
                               (int)bufSize, NULL, NULL);
      Assert_PRT_true(0 != rc); /* Bogus Unicode or buffer too small should be
                                   caught by previous tests */
      result = 0;
    }
  }
...
#endif
    PPG_si_osType =
        defaultTypeName; /* by default, use the "unrecognized version" string */
...
#endif /* defined(_WIN32_WINNT_WIN10) && (_WIN32_WINNT_MAXVER >= \
          _WIN32_WINNT_WIN10) */
...
#else  /* defined(_WIN32_WINNT_WINBLUE) && (_WIN32_WINNT_MAXVER >= \
      ...
      switch (versionInfo.dwMajorVersion) {
        ...
        case 10: {
          switch (versionInfo.wProductType) {
            ...
            default: {
              /* Starting with major version 10, use the registry to get the
               * version */
              PPG_si_osType = defaultTypeName;
              isServerMajorVersion10 = TRUE;
            } break;
...
#endif /* defined(_WIN32_WINNT_WINBLUE) && (_WIN32_WINNT_MAXVER >= \
          _WIN32_WINNT_WINBLUE) */
    ...
    if (defaultTypeName == PPG_si_osType) {
      ...
      if (ERROR_SUCCESS ==
        ...
        if (isServerMajorVersion10 &&
            (ERROR_SUCCESS == RegQueryValueExA(hKey, RELEASE_ID_KEY, NULL, NULL,
                                               NULL, &valueSize))) {
          char ridBuffer[5]; /* the ReleaseId string format is YYMM */
          if (sizeof(ridBuffer) >= valueSize) {
            if (ERROR_SUCCESS == RegQueryValueExA(hKey, RELEASE_ID_KEY, NULL,
                                                  NULL, (LPBYTE)ridBuffer,
                                                  &valueSize)) {
              if (0 == strcmp(ridBuffer, "1607")) {
                ...
                size_t bufferSize =
                    sizeof(WINDOWS_SERVER_PREFIX) + valueSize + 1;
                char *productNameBuffer = portLibrary->mem_allocate_memory(
                    portLibrary, valueSize, OMR_GET_CALLSITE(),
                    OMRMEM_CATEGORY_PORT_LIBRARY);
                if (NULL != productNameBuffer) {
                  /* Print the version string using the format used by Microsoft
                   */
                  portLibrary->str_printf(portLibrary, productNameBuffer,
                                          bufferSize, "%s%s",
        ...
        if (defaultTypeName == PPG_si_osType) {
          ...
          if (ERROR_SUCCESS == RegQueryValueExA(hKey, PRODUCT_NAME_KEY, NULL,
                                                NULL, NULL, &valueSize)) {
            char *productNameBuffer = portLibrary->mem_allocate_memory(
                portLibrary, valueSize, OMR_GET_CALLSITE(),
                OMRMEM_CATEGORY_PORT_LIBRARY);
...
#else
      Trc_PRT_sysinfo_unrecognized_Windows_version(versionInfo.wProductType,
...
const char *omrsysinfo_get_OS_version(struct OMRPortLibrary *portLibrary) {
  /* OS Version format 	: <CurrentVersion> build <CurrentBuildNumber>
   * <CSDVersion>
  ...
  if (NULL == PPG_si_osVersion) {
    ...
    position = portLibrary->str_printf(
        portLibrary, buffer, len, "%d.%d build %d", versionInfo.dwMajorVersion,
        versionInfo.dwMinorVersion, versionInfo.dwBuildNumber & 0x0000FFFF);

    if ((NULL != versionInfo.szCSDVersion) &&
        ('\0' != versionInfo.szCSDVersion[0])) {
      buffer[position++] = ' ';
      WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS,
                          versionInfo.szCSDVersion, -1, &buffer[position],
                          (int)(len - position - 1), NULL, NULL);
    }
...
intptr_t omrsysinfo_process_exists(struct OMRPortLibrary *portLibrary,
  ...
  /* OpenProcess returns NULL if the process does not exist or if the call fails
   * for other reasons */
...
static intptr_t find_executable_name(struct OMRPortLibrary *portLibrary,
  ...
  utf8Result = portLibrary->mem_allocate_memory(portLibrary, (length + 1) * 3,
                                                OMR_GET_CALLSITE(),
                                                OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == utf8Result) {
    return -1;
...
uintptr_t omrsysinfo_get_number_CPUs_by_type(struct OMRPortLibrary *portLibrary,
  ...
  switch (type) {
    ...
    case OMRPORT_CPU_BOUND: {
      ...
      GetProcessAffinityMask(currentProcess, (PDWORD_PTR)&processAffinity,
      ...
      if (0 == toReturn) {
        Trc_PRT_sysinfo_get_number_CPUs_by_type_failedBound("errno: ",
                                                            GetLastError());
      }
...
int32_t omrsysinfo_get_memory_info(struct OMRPortLibrary *portLibrary,
  ...
  status = PdhAddCounter(statsHandle, MEMORY_COMMIT_LIMIT_COUNTER_PATH,
                         (DWORD_PTR)NULL, &memoryCommitLimitCounter);
  if (ERROR_SUCCESS != status) {
  ...
  status = PdhAddCounter(statsHandle, MEMORY_COMMITTED_BYTES_COUNTER_PATH,
                         (DWORD_PTR)NULL, &memoryCommittedBytesCounter);
  if (ERROR_SUCCESS != status) {
    Trc_PRT_sysinfo_get_memory_info_failedAddingCounter("Committed Bytes",
                                                        status);
    Trc_PRT_sysinfo_get_memory_info_Exit(
        OMRPORT_ERROR_SYSINFO_ERROR_READING_MEMORY_INFO);
    PdhCloseQuery(statsHandle);
  ...
  status = PdhAddCounter(statsHandle, MEMORY_CACHE_BYTES_COUNTER_PATH,
                         (DWORD_PTR)NULL, &memoryCacheBytesCounter);
  if (ERROR_SUCCESS != status) {
  ...
  /* Collect the current raw data value for all counters in the usage stats
   * query. */
  ...
  status = PdhGetRawCounterValue(memoryCommitLimitCounter, (LPDWORD)NULL,
                                 &counterValue);
  if ((ERROR_SUCCESS == status) &&
  ...
  status = PdhGetRawCounterValue(memoryCommittedBytesCounter, (LPDWORD)NULL,
                                 &counterValue);
  if ((ERROR_SUCCESS == status) &&
      ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) ||
       (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
  ...
  status = PdhGetRawCounterValue(memoryCacheBytesCounter, (LPDWORD)NULL,
                                 &counterValue);
  if ((ERROR_SUCCESS == status) &&
      ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) ||
       (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
  ...
  /* Note that Windows does not have 'buffered memory' and hence,
   * memInfo->buffered remains -1. */
 ...
 * @param[in] portLibrary The port library.
 *
 * @return 0 if the information was unavailable, otherwise total physical memory
 * in bytes.
 */
 ...
 * that were created by @ref omrsysinfo_startup should be destroyed here.
 *
 * @param[in] portLibrary The port library.
 *
 ...
 * that are required for the system information operations may be created here.
 * All resources created here should be destroyed in @ref omrsysinfo_shutdown.
 *
 * @param[in] portLibrary The port library.
 *
 * @return 0 on success, negative error code on failure.  Error code values
 * returned are \arg OMRPORT_ERROR_STARTUP_SYSINFO
 *
...
/**
 * Query the operating system for the name of the user associate with the
 * current thread
 *
 * Obtain the value of the name of the user associated with the current thread,
 * and then write it out into the buffer supplied by the user
 *
 * @param[in] portLibrary The port Library
...
intptr_t omrsysinfo_get_username(struct OMRPortLibrary *portLibrary,
  ...
  unicodeBuffer = portLibrary->mem_allocate_memory(
      portLibrary, nameLength * 2, OMR_GET_CALLSITE(),
      OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  stored into the buffer, including the terminating null character. If the
  buffer is not large enough, call fails and the required size is stored in
  nameLength.
  */
  ...
  if (0 == rc) {
    ...
    /* convert */
    rc = WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS,
                             unicodeBuffer, -1, NULL, 0, NULL, NULL);
    if (0 == rc) {
      result = -1;
    } else {
      if (rc > length) {
        ...
        WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS,
                            unicodeBuffer, -1, buffer, (int)length, NULL, NULL);
        result = 0;
      }
    }
  }
...
/**
 * Query the operating system for the name of the group associate with the
 * current thread
 *
 * Obtain the value of the name of the group associated with the current thread,
 * and then write it out into the buffer supplied by the user
 *
 * @param[in] portLibrary The port Library
...
/**
 * Provides the system load average for the past 1, 5 and 15 minutes, if
 * available. The load average is defined as the number of runnable (including
 * running) processes averaged over a period of time, however, it may also
 * include processes in uninterruptable sleep states (Linux does this).
 *
 * @param[in] portLibrary The port library.
 * @param[out] loadAverage must be non-Null. Contains the load average data for
 * the past 1, 5 and 15 minutes. A load average of -1 indicates the load average
 * for that specific period was not available.
 *
...
intptr_t omrsysinfo_get_CPU_utilization(struct OMRPortLibrary *portLibrary,
  ...
  idleTime = (lpIdleTime.dwLowDateTime +
              (((uint64_t)lpIdleTime.dwHighDateTime) << 32)) *
             100; /* FILETIME resolution is 100 ns */
  kernelActiveTime = (lpKernelTime.dwLowDateTime +
                      (((uint64_t)lpKernelTime.dwHighDateTime) << 32)) *
                         100 -
                     idleTime; /* kernel time includes idle time */
  Trc_PRT_sysinfo_get_CPU_utilization_GST_result("kernel active",
                                                 kernelActiveTime);
  userTime = (lpUserTime.dwLowDateTime +
              (((uint64_t)lpUserTime.dwHighDateTime) << 32)) *
             100;
  ...
  cpuTime->numberOfCpus = (int32_t)portLibrary->sysinfo_get_number_CPUs_by_type(
...
static int32_t copyEnvToBuffer(struct OMRPortLibrary *portLibrary, void *args) {
  ...
  while ((0 == envVars[0] && 0 == envVars[1]) == FALSE) {
    if (firstEnvVar) {
      ...
      envVars++; /* skip over single terminating null after each individual
                    envVar */
    }
    storageRequired +=
        WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS,
                            envVars, -1, (LPSTR)cursor, 0, NULL, NULL);
  ...
  while ((0 == envVars[0] && 0 == envVars[1]) == FALSE) {
    ...
    if (spaceLeft >= (uintptr_t)spaceForThisEntry) {
      WideCharToMultiByte(OS_ENCODING_CODE_PAGE, 0, envVars, -1, (LPSTR)cursor,
                          spaceForThisEntry, NULL, NULL);
...
/* As of now we are dealing with only the above four counters. Update this when
 * more counters are added.
 */
 ...
 * @param [in] procInfo         - a handle to the processor usage statistics
 * object.
 ...
 */
static int32_t AddCounter(OMRPortLibrary *portLibrary, PDH_HQUERY statsHandle,
                          struct J9ProcessorInfos *procInfo,
                          const char *cntrName, PDH_HCOUNTER **cntrPtrRef) {
  ...
  for (cpuCntr = 0; cpuCntr < procInfo->totalProcessorCount + 1; cpuCntr++) {
    if (0 == cpuCntr) {
      portLibrary->str_printf(portLibrary, counterPathBuf,
                              sizeof(counterPathBuf), "\\%s(%s)\\%s",
                              PROCESSOR_OBJECT_NAME, "_Total", cntrName);
    } else {
      portLibrary->str_printf(
          portLibrary, counterPathBuf, sizeof(counterPathBuf), "\\%s(%d)\\%s",
          PROCESSOR_OBJECT_NAME, procInfo->procInfoArray[cpuCntr].proc_id,
          cntrName);
    }
    status = PdhAddCounter(statsHandle, counterPathBuf, (DWORD_PTR)NULL,
                           &(cntrPtr[cpuCntr]));
    if (ERROR_SUCCESS != status) {
      return -1;
    }
...
int32_t omrsysinfo_get_processor_info(struct OMRPortLibrary *portLibrary,
  ...
  /* Windows specific counter handles; number of handles varies depending on
  ...
  /* Keep looping until we get the size of processor set right, i.e., what if it
   * changes between our allocating space and actually obtaining the processor
   * data? We shall need to rehash things here.
   */
  do {
    ...
    procInfo->totalProcessorCount =
        (int32_t)portLibrary->sysinfo_get_number_CPUs_by_type(
    ...
    rootCntrPtrSize =
        (SIZEOF_PDH_HCOUNTER * (procInfo->totalProcessorCount + 1) *
         PROCESSOR_COUNTER_CATEGORIES);
    /* Allocate memory for all processor related counters at once. */
    rootCntrPtr = (PDH_HCOUNTER *)portLibrary->mem_allocate_memory(
        portLibrary, rootCntrPtrSize, OMR_GET_CALLSITE(),
        OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == rootCntrPtr) {
    ...
    privilegedTimeCounter =
        (userTimeCounter + (procInfo->totalProcessorCount + 1));
    idleTimeCounter =
        (privilegedTimeCounter + (procInfo->totalProcessorCount + 1));

    /* Add the % User Time counter. */
    rc = AddCounter(portLibrary, statsHandle, procInfo,
                    PROCESSOR_USER_TIME_COUNTER_NAME, &userTimeCounter);
    if (0 != rc) {
     ...
     */
    rc = AddCounter(portLibrary, statsHandle, procInfo,
                    PROCESSOR_PRIVILEGED_TIME_COUNTER_NAME,
                    &privilegedTimeCounter);
    if (0 != rc) {
      rc = OMRPORT_ERROR_SYSINFO_ERROR_READING_PROCESSOR_INFO;
    ...
    rc = AddCounter(portLibrary, statsHandle, procInfo,
                    PROCESSOR_IDLE_TIME_COUNTER_NAME, &idleTimeCounter);
    if (0 != rc) {
      rc = OMRPORT_ERROR_SYSINFO_ERROR_READING_PROCESSOR_INFO;
    ...
    /* Check whether the number of processors have changed since we last
     * obtained this count and allocated enough memory for the same.
     */
    if (((int32_t)portLibrary->sysinfo_get_number_CPUs_by_type(
            portLibrary, OMRPORT_CPU_PHYSICAL)) ==
        procInfo->totalProcessorCount) {
      /* Nailed it! The processor count we obtained seems to hold and thus, we
       * have sufficient space to forge ahead with collecting data. Collect
       * current raw data value for all counters in the usage stats query.
       */
      status = PdhCollectQueryData(statsHandle);
      if (ERROR_SUCCESS != status) {
        rc = OMRPORT_ERROR_SYSINFO_ERROR_READING_PROCESSOR_INFO;
    ...
    /* Seems the processor count changed between getting the size and actually
     * collecting data. Must free up the memory since this is insufficient for
     * us to continue with, and start all over again.
     */
    portLibrary->mem_free_memory(portLibrary, rootCntrPtr);
    rootCntrPtr = NULL;
    portLibrary->mem_free_memory(portLibrary, procInfo->procInfoArray);
  ...
  /* On Windows all processors that are configured are supposed to be online as
   * well. Windows' counters are in 100s' of nanoseconds. So convert this to
   * microseconds.
   */
  for (cpuCntr = 0; cpuCntr < procInfo->totalProcessorCount + 1; cpuCntr++) {
    status = PdhGetRawCounterValue(userTimeCounter[cpuCntr], (LPDWORD)NULL,
                                   &counterValue);
    if ((ERROR_SUCCESS == status) &&
        ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) ||
         (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
      procInfo->procInfoArray[cpuCntr].userTime =
          counterValue.FirstValue / NS100_TO_USEC;
    }

    status = PdhGetRawCounterValue(privilegedTimeCounter[cpuCntr],
                                   (LPDWORD)NULL, &counterValue);
    if ((ERROR_SUCCESS == status) &&
        ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) ||
         (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
      procInfo->procInfoArray[cpuCntr].systemTime =
    ...
    /* On Windows we compute busy times based on system and user times, though
     * we need add the Processor Times counter for retrieving the processor
     * timestamp, obtained below.
     */
    procInfo->procInfoArray[cpuCntr].busyTime =
        procInfo->procInfoArray[cpuCntr].systemTime +
        procInfo->procInfoArray[cpuCntr].userTime;

    status = PdhGetRawCounterValue(idleTimeCounter[cpuCntr], (LPDWORD)NULL,
                                   &counterValue);
    if ((ERROR_SUCCESS == status) &&
        ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) ||
         (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
      ...
      procInfo->timestamp =
          (((((uint64_t)counterValue.TimeStamp.dwHighDateTime) << 32)) |
  ...
  /* Windows '_Total' for each mode is actually a misnomer - since they are not
   * the aggregates, but averages over all processors. So we need to multiply
   * with processor counts as port library clients are interested in aggregates
   * and also, to be consistent with other platforms.
   */
...
intptr_t omrsysinfo_get_cwd(struct OMRPortLibrary *portLibrary, char *buf,
  ...
  wchar_t *unicodeBuffer =
      NULL; /* unicode version of CWD, size: pathLengthInWChars*sizeof(wchar_t)
               plus 1 for terminator and 1 for terminating slash if required */
  ...
  /* GetCurrentDirectoryW returns the numbers of TCHARS needed to store the
   * string. The returned string not necessarily ends with a backslash.
   *     - seeing as we always operate on systems that support, TCHAR is in
   * factt wchar_t
  ...
  /* we need a buffer to store the unicode string, as we'll use the supplied buf
   * for the UTF8 conversion */
  unicodeBuffer = portLibrary->mem_allocate_memory(
      portLibrary, unicodeBufferSizeBytes, OMR_GET_CALLSITE(),
      OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  pathLengthInWChars =
      GetCurrentDirectoryW(pathLengthInWChars + 1, unicodeBuffer);
...
intptr_t omrsysinfo_get_tmp(struct OMRPortLibrary *portLibrary, char *buf,
  ...
  /* GetTempPathW returns the numbers of TCHARS (not including the terminating
   * null character) needed to store string. The returned string ends with a
   * backslash
   *     - seeing as we always operate on systems that support, TCHAR is in fact
   * wchar_t
   *     - see "Working with Strings"  on MSDN for more information
  ...
  mutf8Size = portLibrary->str_convert(
      portLibrary, J9STR_CODE_WIDE, J9STR_CODE_MUTF8,
      (const char *)unicodeBuffer,
      (uintptr_t)pathLengthInWChars * sizeof(wchar_t), NULL, 0);
  if (mutf8Size >= 0) {
    ...
    if (bufLen < (uintptr_t)mutf8Size) {
      ...
      mutf8Size = portLibrary->str_convert(
          portLibrary, J9STR_CODE_WIDE, J9STR_CODE_MUTF8,
          (const char *)unicodeBuffer,
          (uintptr_t)pathLengthInWChars * sizeof(wchar_t), buf, bufLen);
      if (mutf8Size < 0) {
