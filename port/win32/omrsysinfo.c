                   ...
                   uintptr_t bufSize)
{
   ...
   * allocated.
   */
  envVarWideCharName = port_convertFromUTF8(
    portLibrary, envVar, envVarNameConversionBuffer, ENVVAR_NAME_BUFFER_LENGTH);
  ...
  if ((ENVVAR_VALUE_BUFFER_LENGTH <= rc) && (0 != rc)) {
    ...
    envVarWideCharValue = (wchar_t*)portLibrary->mem_allocate_memory(
      portLibrary,
      envVarValueLength * sizeof(wchar_t),
      OMR_GET_CALLSITE(),
      OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL != envVarWideCharValue) {
      rc = GetEnvironmentVariableW(
        envVarWideCharName, envVarWideCharValue, envVarValueLength);
    } else {
  ...
  if (0 == rc) {
    ...
    rc = WideCharToMultiByte(OS_ENCODING_CODE_PAGE,
                             OS_ENCODING_WC_FLAGS,
                             envVarWideCharValue,
                             -1,
                             NULL,
                             0,
...
omrsysinfo_get_OS_version(struct OMRPortLibrary* portLibrary)
{
  ...
  (_WIN32_WINNT_MAXVER >= _WIN32_WINNT_WINBLUE)
    ...
    if (NULL != versionInfo.szCSDVersion) {
      len += WideCharToMultiByte(OS_ENCODING_CODE_PAGE,
                                 OS_ENCODING_WC_FLAGS,
    ...
    position = portLibrary->str_printf(portLibrary,
                                       buffer,
                                       len,
                                       "%d.%d build %d",
                                       versionInfo.dwMajorVersion,
    ...
    if ((NULL != versionInfo.szCSDVersion) &&
      ...
      buffer[position++] = ' ';
      WideCharToMultiByte(OS_ENCODING_CODE_PAGE,
                          OS_ENCODING_WC_FLAGS,
                          versionInfo.szCSDVersion,
                          -1,
                          &buffer[position],
...
find_executable_name(struct OMRPortLibrary* portLibrary, char** result)
{
  ...
  utf8Result = portLibrary->mem_allocate_memory(portLibrary,
                                                (length + 1) * 3,
                                                OMR_GET_CALLSITE(),
                                                OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == utf8Result) {
                                   ...
                                   uintptr_t type)
{
  ...
  switch (type) {
    ...
    case OMRPORT_CPU_BOUND: {
      ...
      GetProcessAffinityMask(currentProcess,
                           ...
                           ...)
{
  ...
  PDH_RAW_COUNTER counterValue = {
    PDH_CSTATUS_INVALID_DATA, { 0, 0 }, 0, 0, 0
  };
  ...
  status = PdhGetRawCounterValue(
    memoryCommitLimitCounter, (LPDWORD)NULL, &counterValue);
  if ((ERROR_SUCCESS == status) &&
  ...
  status = PdhGetRawCounterValue(
    memoryCommittedBytesCounter, (LPDWORD)NULL, &counterValue);
  if ((ERROR_SUCCESS == status) &&
      ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) ||
       (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
  ...
  status = PdhGetRawCounterValue(
    memoryCacheBytesCounter, (LPDWORD)NULL, &counterValue);
  if ((ERROR_SUCCESS == status) &&
      ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) ||
       (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
                               ...
                               struct J9SysinfoCPUTime* cpuTime)
{
  ...
  idleTime =
    (lpIdleTime.dwLowDateTime + (((uint64_t)lpIdleTime.dwHighDateTime) << 32)) *
  ...
  Trc_PRT_sysinfo_get_CPU_utilization_GST_result("kernel active",
                                                 kernelActiveTime);
  userTime =
    (lpUserTime.dwLowDateTime + (((uint64_t)lpUserTime.dwHighDateTime) << 32)) *
    100;
...
copyEnvToBuffer(struct OMRPortLibrary* portLibrary, void* args)
{
  ...
  while ((0 == envVars[0] && 0 == envVars[1]) == FALSE) {
    ...
    storageRequired += WideCharToMultiByte(OS_ENCODING_CODE_PAGE,
                                           OS_ENCODING_WC_FLAGS,
                                           envVars,
                                           -1,
                                           (LPSTR)cursor,
  ...
  while ((0 == envVars[0] && 0 == envVars[1]) == FALSE) {
    ...
    spaceForThisEntry = WideCharToMultiByte(OS_ENCODING_CODE_PAGE,
                                            OS_ENCODING_WC_FLAGS,
                                            envVars,
                                            -1,
                                            (LPSTR)cursor,
           ...
           PDH_HCOUNTER** cntrPtrRef)
{
  ...
  for (cpuCntr = 0; cpuCntr < procInfo->totalProcessorCount + 1; cpuCntr++) {
    ...
    status = PdhAddCounter(
      statsHandle, counterPathBuf, (DWORD_PTR)NULL, &(cntrPtr[cpuCntr]));
    if (ERROR_SUCCESS != status) {
      return -1;
    }
  }
                              ...
                              struct J9ProcessorInfos* procInfo)
{
  ...
  do {
    ...
    rc = AddCounter(portLibrary,
                    statsHandle,
                    procInfo,
                    PROCESSOR_USER_TIME_COUNTER_NAME,
                    &userTimeCounter);
    if (0 != rc) {
    ...
    rc = AddCounter(portLibrary,
                    statsHandle,
                    procInfo,
                    PROCESSOR_IDLE_TIME_COUNTER_NAME,
                    &idleTimeCounter);
    if (0 != rc) {
      rc = OMRPORT_ERROR_SYSINFO_ERROR_READING_PROCESSOR_INFO;
  ...
  for (cpuCntr = 0; cpuCntr < procInfo->totalProcessorCount + 1; cpuCntr++) {
    status = PdhGetRawCounterValue(
      userTimeCounter[cpuCntr], (LPDWORD)NULL, &counterValue);
    if ((ERROR_SUCCESS == status) &&
        ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) ||
         (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
      procInfo->procInfoArray[cpuCntr].userTime =
    ...
    status = PdhGetRawCounterValue(
      privilegedTimeCounter[cpuCntr], (LPDWORD)NULL, &counterValue);
    if ((ERROR_SUCCESS == status) &&
        ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) ||
         (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
      procInfo->procInfoArray[cpuCntr].systemTime =
    ...
    status = PdhGetRawCounterValue(
      idleTimeCounter[cpuCntr], (LPDWORD)NULL, &counterValue);
    if ((ERROR_SUCCESS == status) &&
        ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) ||
         (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
      procInfo->procInfoArray[cpuCntr].idleTime =
                   ...
                   BOOLEAN ignoreEnvVariable)
{
  ...
  mutf8Size =
    portLibrary->str_convert(portLibrary,
                             ...
                             (uintptr_t)pathLengthInWChars * sizeof(wchar_t),
                             NULL,
                             0);
  if (mutf8Size >= 0) {
    ...
    if (bufLen < (uintptr_t)mutf8Size) {
      ...
      mutf8Size = portLibrary->str_convert(portLibrary,
                                           J9STR_CODE_WIDE,
                                           J9STR_CODE_MUTF8,
                                           (const char*)unicodeBuffer,
                                           (uintptr_t)pathLengthInWChars *
                                             sizeof(wchar_t),
                                           buf,
                                           bufLen);
      if (mutf8Size < 0) {
