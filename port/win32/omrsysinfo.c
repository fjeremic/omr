...
omrsysinfo_get_memory_info(struct OMRPortLibrary* portLibrary, struct J9MemoryInfo* memInfo, ...)
{
    ...
    if ((ERROR_SUCCESS == status) && ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) || (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
...
omrsysinfo_get_processor_info(struct OMRPortLibrary* portLibrary, struct J9ProcessorInfos* procInfo)
{
    ...
    for (cpuCntr = 0; cpuCntr < procInfo->totalProcessorCount + 1; cpuCntr++) {
		 ...
		 */
        procInfo->procInfoArray[cpuCntr].busyTime = procInfo->procInfoArray[cpuCntr].systemTime + procInfo->procInfoArray[cpuCntr].userTime;
        ...
        if ((ERROR_SUCCESS == status) && ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) || (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
            ...
            procInfo->timestamp = (((((uint64_t)counterValue.TimeStamp.dwHighDateTime) << 32)) | (((uint64_t)counterValue.TimeStamp.dwLowDateTime))) / NS100_TO_USEC;
        }
