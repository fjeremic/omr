...
intptr_t omrsysinfo_get_env(struct OMRPortLibrary* portLibrary, const char* envVar, char* infoString, uintptr_t bufSize)
{
  ...
  rc = GetEnvironmentVariableW(
    envVarWideCharName, envVarWideCharValue, ENVVAR_VALUE_BUFFER_LENGTH); /* Try with the stack buffer first */
  if ((ENVVAR_VALUE_BUFFER_LENGTH <= rc) && (0 != rc)) {
    ...
    envVarWideCharValue = (wchar_t*)portLibrary->mem_allocate_memory(
      portLibrary, envVarValueLength * sizeof(wchar_t), OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  if (0 == rc) {
    ...
    if (0 == rc) { /* error, probably bogus Unicode */
      ...
      rc = WideCharToMultiByte(
        OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, envVarWideCharValue, -1, infoString, (int)bufSize, NULL, NULL);
            ...
            if (ERROR_SUCCESS == RegQueryValueExA(hKey, RELEASE_ID_KEY, NULL, NULL, (LPBYTE)ridBuffer, &valueSize)) {
              if (0 == strcmp(ridBuffer, "1607")) {
                ...
                char* productNameBuffer = portLibrary->mem_allocate_memory(
                  portLibrary, valueSize, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
                if (NULL != productNameBuffer) {
                  /* Print the version string using the format used by Microsoft */
                  portLibrary->str_printf(
                    portLibrary, productNameBuffer, bufferSize, "%s%s", WINDOWS_SERVER_PREFIX, ridBuffer);
        ...
        if (defaultTypeName == PPG_si_osType) {
          ...
          if (ERROR_SUCCESS == RegQueryValueExA(hKey, PRODUCT_NAME_KEY, NULL, NULL, NULL, &valueSize)) {
            ...
            if (NULL != productNameBuffer) {
              ...
              if (ERROR_SUCCESS
                == RegQueryValueExA(hKey, PRODUCT_NAME_KEY, NULL, NULL, (LPBYTE)productNameBuffer, &valueSize)) {
  ...
  if (NULL == PPG_si_osVersion) {
    ...
    if (NULL != versionInfo.szCSDVersion) {
      len += WideCharToMultiByte(
        OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, versionInfo.szCSDVersion, -1, NULL, 0, NULL, NULL);
    }
    ...
    position = portLibrary->str_printf(portLibrary, buffer, len, "%d.%d build %d", versionInfo.dwMajorVersion,
    ...
    if ((NULL != versionInfo.szCSDVersion) && ('\0' != versionInfo.szCSDVersion[0])) {
      buffer[position++] = ' ';
      WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, versionInfo.szCSDVersion, -1, &buffer[position],
...
static intptr_t find_executable_name(struct OMRPortLibrary* portLibrary, char** result)
{
  ...
  utf8Result
    = portLibrary->mem_allocate_memory(portLibrary, (length + 1) * 3, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
  if (NULL == utf8Result) {
    return -1;
  }
...
int32_t omrsysinfo_get_memory_info(struct OMRPortLibrary* portLibrary, struct J9MemoryInfo* memInfo, ...)
{
  ...
  status = PdhAddCounter(statsHandle, MEMORY_COMMIT_LIMIT_COUNTER_PATH, (DWORD_PTR)NULL, &memoryCommitLimitCounter);
  if (ERROR_SUCCESS != status) {
  ...
  status
    = PdhAddCounter(statsHandle, MEMORY_COMMITTED_BYTES_COUNTER_PATH, (DWORD_PTR)NULL, &memoryCommittedBytesCounter);
  if (ERROR_SUCCESS != status) {
  ...
  if ((ERROR_SUCCESS == status)
    && ((PDH_CSTATUS_VALID_DATA == counterValue.CStatus) || (PDH_CSTATUS_NEW_DATA == counterValue.CStatus))) {
...
intptr_t omrsysinfo_get_username(struct OMRPortLibrary* portLibrary, char* buffer, uintptr_t length)
{
  ...
  unicodeBuffer
    = portLibrary->mem_allocate_memory(portLibrary, nameLength * 2, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
  ...
  if (0 == rc) {
    ...
    if (0 == rc) {
      ...
      if (rc > length) {
        ...
        WideCharToMultiByte(
          OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, unicodeBuffer, -1, buffer, (int)length, NULL, NULL);
        result = 0;
      }
    }
  }
...
intptr_t omrsysinfo_get_CPU_utilization(struct OMRPortLibrary* portLibrary, struct J9SysinfoCPUTime* cpuTime)
{
  ...
  idleTime = (lpIdleTime.dwLowDateTime + (((uint64_t)lpIdleTime.dwHighDateTime) << 32))
...
static int32_t copyEnvToBuffer(struct OMRPortLibrary* portLibrary, void* args)
{
  ...
  while ((0 == envVars[0] && 0 == envVars[1]) == FALSE) {
    ...
    storageRequired
      += WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, envVars, -1, (LPSTR)cursor, 0, NULL, NULL);
 ...
 */
static int32_t AddCounter(OMRPortLibrary* portLibrary, PDH_HQUERY statsHandle, struct J9ProcessorInfos* procInfo,
  const char* cntrName, PDH_HCOUNTER** cntrPtrRef)
{
  ...
  for (cpuCntr = 0; cpuCntr < procInfo->totalProcessorCount + 1; cpuCntr++) {
    if (0 == cpuCntr) {
      portLibrary->str_printf(
        portLibrary, counterPathBuf, sizeof(counterPathBuf), "\\%s(%s)\\%s", PROCESSOR_OBJECT_NAME, "_Total", cntrName);
    } else {
...
int32_t omrsysinfo_get_processor_info(struct OMRPortLibrary* portLibrary, struct J9ProcessorInfos* procInfo)
{
  ...
  do {
    ...
    if (NULL == rootCntrPtr) {
      Trc_PRT_sysinfo_get_processor_info_categoryAllocFailed(PROCESSOR_COUNTER_CATEGORIES, PROCESSOR_OBJECT_NAME);
  ...
  for (cpuCntr = 0; cpuCntr < procInfo->totalProcessorCount + 1; cpuCntr++) {
    ...
    procInfo->procInfoArray[cpuCntr].busyTime
      = procInfo->procInfoArray[cpuCntr].systemTime + procInfo->procInfoArray[cpuCntr].userTime;
...
intptr_t omrsysinfo_get_cwd(struct OMRPortLibrary* portLibrary, char* buf, uintptr_t bufLen)
{
  ...
  /* GetCurrentDirectoryW returns the numbers of TCHARS needed to store the string. The returned string not necessarily
...
intptr_t omrsysinfo_get_tmp(struct OMRPortLibrary* portLibrary, char* buf, uintptr_t bufLen, BOOLEAN ignoreEnvVariable)
{
  ...
  /* GetTempPathW returns the numbers of TCHARS (not including the terminating null character) needed to store string.
  ...
  mutf8Size = portLibrary->str_convert(portLibrary, J9STR_CODE_WIDE, J9STR_CODE_MUTF8, (const char*)unicodeBuffer,
