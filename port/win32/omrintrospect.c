...
struct PlatformWalkData {
  ...
  HANDLE snapshot;
  /* if set this will filter the specified thread out of the system iterator,
   * for use when signal context is specified */
  DWORD filterThread;
...
/*
 * Conceptually this function resumes all the threads we suspend with
 * suspend_all_preemtive. In actuality it decrements the suspend count of every
 * thread in the process by 1 even if, somehow, we've not incremented it in
 * suspend_all_preemptive (externally spawned threads by debuggers for example).
 */
int resume_all_preempted(struct PlatformWalkData *data) {
  int resumed = 0;
  ...
  do {
    ...
    if (data->thread32.th32OwnerProcessID != current_pid ||
...
/*
 * This function takes a system snapshot, walks all the threads for this process
 * and suspends all of our threads barring the one executing the function. It
 * then takes a new snapshot to determine if new threads were created during the
 * suspension. This is repeated until the thread count matches between the two
 * snapshots. Use GetLastError() to get error details if any.
 */
int suspend_all_preemptive(struct PlatformWalkData *data) {
  int a_count = 0;
  int b_count = 0;
  char passA = 1;
  DWORD result;
...
error_return:
  /* we don't close the snapshot so that it's still available to resume any
   * threads we suspended */
  SetLastError(result);
 ...
 */
void freeThread(J9ThreadWalkState *state, J9PlatformThread *thread) {
...
/*
 * Cleans up all state that we've created as part of the walk, including freeing
 * up the last returned thread, the debug library symbols, etc.
 */
void cleanup(J9ThreadWalkState *state) {
  BOOL resumedOK = TRUE;
    ...
    if (data) {
      if (data->snapshot && data->snapshot != INVALID_HANDLE_VALUE) {
        ...
        if (!resumedOK || GetLastError() != ERROR_INVALID_HANDLE) {
          /* it seems this can raise an exception if the handle has become
           * invalid */
 ...
 */
int setup_native_thread(J9ThreadWalkState *state, CONTEXT *sigContext) {
  CONTEXT tmpContext;
  int result = 0;
  ...
  state->current_thread->context = (PCONTEXT)state->portLibrary->heap_allocate(
      state->portLibrary, state->heap, sizeof(CONTEXT));
  ...
  state->current_thread->thread_id =
      ((struct PlatformWalkData *)state->platform_data)->thread32.th32ThreadID;
  ...
  if (sigContext) {
    ...
    HANDLE thread_handle =
        OpenThread(THREAD_GET_CONTEXT | THREAD_QUERY_INFORMATION, FALSE,
                   (DWORD)state->current_thread->thread_id);
    if (thread_handle == NULL) {
      return -4;
    }
...
J9PlatformThread *omrintrospect_threads_startDo_with_signal(
  ...
  /* likely can't operate in stack overflow conditions so inhibit construction
   * of the backtrace */
  if (sigFaultInfo != NULL) {
    if (sigFaultInfo->ExceptionRecord != NULL &&
        sigFaultInfo->ExceptionRecord->ExceptionCode ==
...
J9PlatformThread *omrintrospect_threads_nextDo(J9ThreadWalkState *state) {
  ...
  while (data->thread32.th32OwnerProcessID != processId ||
         data->thread32.th32ThreadID == data->filterThread) {
