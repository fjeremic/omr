...
static int EsTranslateOpenFlagsToSystemFlags(int32_t flags) {
...
static const char *EsTranslateOpenFlagsToStandardFlags(int32_t flags) {
  /* Ignore all other flags; they are taken care of earlier */
  flags &= (EsOpenWrite | EsOpenRead | EsOpenAppend);
...
/**
 * @internal
 * Determines the proper portable error code to return given a standard C
 * library error code.
 *
 ...
 */
static int32_t findErrorFromErrno(int32_t errorCode) {
  switch (errorCode) {
    case EACCES:
      /* FALLTHROUGH */
    case EPERM:
...
OMRFileStream *omrfilestream_open(struct OMRPortLibrary *portLibrary,
  ...
  if (NULL == path) {
    ...
    portLibrary->error_set_last_error(portLibrary, ERROR_INVALID_PARAMETER,
                                      findError(ERROR_INVALID_PARAMETER));
  ...
  /* Attempt to portably honor the open flags and mode by opening the file
   * descriptor */
  ...
  handle = (HANDLE)portLibrary->file_convert_omrfile_fd_to_native_fd(
      portLibrary, file);
  if (INVALID_HANDLE_VALUE == handle) {
    int32_t error = portLibrary->error_set_last_error(
        portLibrary, ERROR_INVALID_HANDLE, findError(ERROR_INVALID_HANDLE));
    Trc_PRT_filestream_open_failedToOpen(path, flags, mode, error);
    Trc_PRT_filestream_open_Exit(NULL);
    return NULL;
  }
  ...
  if (-1 == fileDescriptor) {
    ...
    error = portLibrary->error_set_last_error(portLibrary, savedErrno,
                                              findErrorFromErrno(savedErrno));
    Trc_PRT_filestream_open_failedToOpen(path, flags, mode, error);
    Trc_PRT_filestream_open_Exit(NULL);
    return NULL;
  }
...
int32_t omrfilestream_setbuffer(OMRPortLibrary *portLibrary,
  ...
  /* On windows, size must be greater than 2.  This is different than the C
   * standard. In this case, ignore the user's buffer and allocate a new one. */
  if (size < 2) {
    size = 2;
    buffer = NULL;
  }
...
OMRFileStream *omrfilestream_fdopen(OMRPortLibrary *portLibrary, intptr_t fd,
  ...
  switch (fd) {
    ...
    default:
      ...
      if (-1 == fileDescriptor) {
        ...
        int savedErrno = errno;
        int32_t error = 0;
        error = portLibrary->error_set_last_error(
            portLibrary, savedErrno, findErrorFromErrno(savedErrno));
  ...
  fileStream = _fdopen(fileDescriptor, standardFlags);
  if (NULL == fileStream) {
    /* _close will close the C file descriptor and the underlying HANDLE, there
     * is no API for closing the fileDescriptor without closing the HANDLE.
     * This may leak. */
    int32_t error = portLibrary->error_set_last_error(
        portLibrary, errno, findErrorFromErrno(errno));
    Trc_PRT_filestream_fdopen_failed(fd, flags, error);
  }
