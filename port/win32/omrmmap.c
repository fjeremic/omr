 ...
 */
J9MmapHandle* omrmmap_map_file(struct OMRPortLibrary* portLibrary, intptr_t file, uint64_t offset, uintptr_t size,
    const char* mappingName, uint32_t flags, uint32_t categoryCode)
{
    ...
    if ((HANDLE)file == INVALID_HANDLE_VALUE) {
        ...
        errMsg = portLibrary->nls_lookup_message(
            portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_MMAP_INVALID_FILE_HANDLE, NULL);
    ...
    if (1 != rwCount) {
        ...
        errMsg = portLibrary->nls_lookup_message(
            portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_MMAP_INVALID_MEMORY_PROTECTION, NULL);
    ...
    if (mappingName != NULL) {
        ...
        if (lpName == NULL) {
            errMsg = portLibrary->nls_lookup_message(
                portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_FILE_MEMORY_ALLOCATE_FAILURE, NULL);
    ...
    if (mapping == NULL) {
        ...
        portLibrary->error_set_last_error(
            portLibrary, lastError, OMRPORT_ERROR_MMAP_MAP_FILE_CREATEMAPPINGOBJECTFAILED);
        return NULL;
    }
...
{
    return (OMRPORT_MMAP_CAPABILITY_COPYONWRITE | OMRPORT_MMAP_CAPABILITY_READ | OMRPORT_MMAP_CAPABILITY_WRITE
        | OMRPORT_MMAP_CAPABILITY_PROTECT | OMRPORT_MMAP_CAPABILITY_MSYNC);
}

intptr_t omrmmap_protect(struct OMRPortLibrary* portLibrary, void* address, uintptr_t length, uintptr_t flags)
{
    return protect_memory(portLibrary, address, length, flags);
}

uintptr_t omrmmap_get_region_granularity(struct OMRPortLibrary* portLibrary, void* address)
{
