                 ...
                 uint32_t categoryCode)
{
  ...
  if (mappingName != NULL) {
    ...
    lpName = portLibrary->mem_allocate_memory(
      portLibrary, lpNameSize, OMR_GET_CALLSITE(), categoryCode);
    if (lpName == NULL) {
  ...
  mapping = (HANDLE)CreateFileMapping(
    (HANDLE)file, NULL, flProtect, 0, 0, (LPCTSTR)lpName);
  if (lpName != NULL) {
    portLibrary->mem_free_memory(portLibrary, lpName);
    lpName = NULL;
  }
  ...
  Trc_PRT_mmap_map_file_win32_callingMapViewOfFile(
    mapping, dwFileOffsetHigh, dwFileOffsetLow);
  pointer = MapViewOfFile(
    mapping, dwDesiredAccess, dwFileOffsetHigh, dwFileOffsetLow, size);
  CloseHandle(mapping);
  if (pointer == NULL) {
    portLibrary->mem_free_memory(portLibrary, returnVal);
    lastError = GetLastError();
              ...
              uint32_t flags)
{
  ...
  if (!success) {
    ...
    portLibrary->error_set_last_error(
      portLibrary, lastError, OMRPORT_ERROR_MMAP_MSYNC_FAILED);
    return -1;
  }
...
{
  size_t pageSize =
    portLibrary->mmap_get_region_granularity(portLibrary, (void*)startAddress);
