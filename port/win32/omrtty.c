...
intptr_t omrtty_available(struct OMRPortLibrary* portLibrary)
{
  ...
  if (PeekNamedPipe(PPG_tty_consoleInputHd, NULL, 0, NULL, &result, NULL)) {
    ...
    if (ERROR_INVALID_HANDLE == GetLastError()) {
      /* Note that this could be done on the stack if we believe that it might overflow (40 k) so dynamic allocation is
       * safer for now */
...
#define EVENTS_TO_CAPTURE 2000
      ...
      if (NULL == PPG_tty_consoleEventBuffer) {
        PPG_tty_consoleEventBuffer = portLibrary->mem_allocate_memory(
          portLibrary, EVENTS_TO_CAPTURE * sizeof(INPUT_RECORD), OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
      ...
      if (PeekConsoleInput(
        ...
        for (x = 0; x < (int)available; x++) {
          if ((KEY_EVENT == inputRecordPointer[x].EventType) && (inputRecordPointer[x].Event.KeyEvent.bKeyDown)
            && ('\r' == inputRecordPointer[x].Event.KeyEvent.uChar.AsciiChar)) {
            bytesCanBeRead = TRUE; /* report true if we find a return char since we know that it is safe to read at
                                      least that one byte */
            break;
          }
        }
