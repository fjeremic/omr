 ...
 * that are required for the TTY library operations may be created here.  All
 * resources created here should be destroyed in @ref omrtty_shutdown.
 *
 * @param[in] portLibrary The port library.
 *
 * @return 0 on success, negative error code on failure.  Error code values
 * returned are \arg OMRPORT_ERROR_STARTUP_TTY \arg
 * OMRPORT_ERROR_STARTUP_TTY_HANDLE \arg OMRPORT_ERROR_STARTUP_TTY_CONSOLE
 *
...
#ifdef TTY_LOG_FILE
  fd = portLibrary->file_open(portLibrary, TTY_LOG_FILE,
                              EsOpenCreate | EsOpenWrite, 0);
#define MSG "========== BEGIN ==========\n\n"
...
#undef MSG
  ...
  if (omrthread_monitor_init_with_name(&PPG_tty_consoleBufferMonitor, 0,
                                       "Windows native console event lock")) {
 ...
 * that were created by @ref omrtty_startup should be destroyed here.
 *
 * @param[in] portLibrary The port library.
 *
 ...
 */
intptr_t omrtty_get_chars(struct OMRPortLibrary *portLibrary, char *s,
                          uintptr_t length) {
  DWORD nCharsRead;
  DWORD result;

  result =
      ReadFile(PPG_tty_consoleInputHd, s, (DWORD)length, &nCharsRead, NULL);
...
intptr_t omrtty_available(struct OMRPortLibrary *portLibrary) {
  ...
  if (PeekNamedPipe(PPG_tty_consoleInputHd, NULL, 0, NULL, &result, NULL)) {
    ...
    if (ERROR_INVALID_HANDLE == GetLastError()) {
      /* Note that this could be done on the stack if we believe that it might
       * overflow (40 k) so dynamic allocation is safer for now */
      /* The number of events being 2000 is consistent with other JDKs.
       * (If you put more than 999 chars into the buffer before hitting enter,
       * you lock up standard input.)
       */
#define EVENTS_TO_CAPTURE 2000
      ...
      if (NULL == PPG_tty_consoleEventBuffer) {
        PPG_tty_consoleEventBuffer = portLibrary->mem_allocate_memory(
            portLibrary, EVENTS_TO_CAPTURE * sizeof(INPUT_RECORD),
            OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
        /* if we failed to make the buffer, just give up and say we found
         * nothing */
        if (NULL == PPG_tty_consoleEventBuffer) {
      ...
      if (PeekConsoleInput(PPG_tty_consoleInputHd,
                           (INPUT_RECORD *)PPG_tty_consoleEventBuffer,
                           EVENTS_TO_CAPTURE, &available)) {
        int x = 0;
        INPUT_RECORD *inputRecordPointer = (INPUT_RECORD *)
            PPG_tty_consoleEventBuffer; /* provided to avoid some casting */
        ...
        for (x = 0; x < (int)available; x++) {
          if ((KEY_EVENT == inputRecordPointer[x].EventType) &&
            ...
            bytesCanBeRead =
                TRUE; /* report true if we find a return char since we know that
                         it is safe to read at least that one byte */
            break;
          }
        }
        omrthread_monitor_exit(PPG_tty_consoleBufferMonitor);
 ...
 * database.
 */
void omrtty_vprintf(struct OMRPortLibrary *portLibrary, const char *format,
                    va_list args) {
 ...
 * database.
 */
void omrtty_err_vprintf(struct OMRPortLibrary *portLibrary, const char *format,
                        va_list args) {
  portLibrary->file_vprintf(portLibrary, OMRPORT_TTY_ERR, format, args);
}
 ...
 * handles used by the port library
 *
 * @param[in] portLibrary The port library.
 *
 */
