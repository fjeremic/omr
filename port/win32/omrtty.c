...
omrtty_startup(struct OMRPortLibrary* portLibrary)
{
  ...
  fd = portLibrary->file_open(
    portLibrary, TTY_LOG_FILE, EsOpenCreate | EsOpenWrite, 0);
#define MSG "========== BEGIN ==========\n\n"
...
#undef MSG
  ...
  if (omrthread_monitor_init_with_name(&PPG_tty_consoleBufferMonitor,
                                       0,
...
#define EVENTS_TO_CAPTURE 2000
      ...
      if (NULL == PPG_tty_consoleEventBuffer) {
        PPG_tty_consoleEventBuffer = portLibrary->mem_allocate_memory(
          portLibrary,
          EVENTS_TO_CAPTURE * sizeof(INPUT_RECORD),
          OMR_GET_CALLSITE(),
