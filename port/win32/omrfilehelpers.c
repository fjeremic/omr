...
int32_t omrfile_lock_bytes_helper(struct OMRPortLibrary *portLibrary,
  ...
  Trc_PRT_file_lock_bytes_win32_callingLockFileEx(
      dwFlags, overlapped.Offset, overlapped.OffsetHigh,
   ...
   *
   */
  success = LockFileEx(hFile, dwFlags, 0, nNumberOfBytesToLockLow,
...
int32_t omrfile_unlock_bytes_helper(struct OMRPortLibrary *portLibrary,
  ...
  Trc_PRT_file_unlock_bytes_win32_callingUnlockFileEx(
      overlapped.Offset, overlapped.OffsetHigh, nNumberOfBytesToLockLow,
   ...
   *
   */
  success = UnlockFileEx(hFile, 0, nNumberOfBytesToLockLow,
                         nNumberOfBytesToLockHigh, &overlapped);
  if ((TRUE == async) && (FALSE == success) &&
      (ERROR_IO_PENDING == GetLastError())) {
    success = GetOverlappedResult(hFile, &overlapped, &bytes, TRUE);
