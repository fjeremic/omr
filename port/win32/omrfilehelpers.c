...
omrfile_get_overlapped_handle_helper(struct OMRPortLibrary* portLibrary)
{
  ...
  HANDLE overlappedHandle =
    omrthread_tls_get(thisThread, tlsKeyOverlappedHandle);
                          ...
                          BOOLEAN async)
{
  ...
  Trc_PRT_file_lock_bytes_win32_callingLockFileEx(dwFlags,
                                                  overlapped.Offset,
  ...
  /*
   * Regarding to asychronous files, this function returns right away without
   * waiting its execution to finish. If the returned result is FALSE and error
   * is ERROR_IO_PENDING, use GetOverlappedResult to wait the execution
   * finishes.
   *
   */
  success = LockFileEx(hFile,
                       dwFlags,
                       0,
                       nNumberOfBytesToLockLow,
                       nNumberOfBytesToLockHigh,
                       &overlapped);
  if ((TRUE == async) && (FALSE == success) &&
...
error:
  lastError = portLibrary->error_set_last_error(
    portLibrary, GetLastError(), OMRPORT_ERROR_FILE_LOCK_BADLOCK);
  Trc_PRT_file_lock_bytes_win32_exiting_with_error(lastError);
  return -1;
}
                            ...
                            BOOLEAN async)
{
  ...
  Trc_PRT_file_unlock_bytes_win32_callingUnlockFileEx(overlapped.Offset,
                                                      overlapped.OffsetHigh,
                                                      nNumberOfBytesToLockLow,
                                                      nNumberOfBytesToLockHigh);
   ...
   *
   */
  success = UnlockFileEx(
    hFile, 0, nNumberOfBytesToLockLow, nNumberOfBytesToLockHigh, &overlapped);
  if ((TRUE == async) && (FALSE == success) &&
      (ERROR_IO_PENDING == GetLastError())) {
    success = GetOverlappedResult(hFile, &overlapped, &bytes, TRUE);
  }
...
error:
  lastError = portLibrary->error_set_last_error(
    portLibrary, GetLastError(), OMRPORT_ERROR_FILE_UNLOCK_BADUNLOCK);
