...
int64_t omrtime_nano_time(struct OMRPortLibrary *portLibrary) {
  ...
  if (PPG_time_hiresClockFrequency == OMRTIME_NANOSECONDS_PER_SECOND) {
    ...
    nanos = (int64_t)(ticks * (OMRTIME_NANOSECONDS_PER_SECOND /
                               PPG_time_hiresClockFrequency));
  } else {
    nanos = (int64_t)(ticks / (PPG_time_hiresClockFrequency /
                               OMRTIME_NANOSECONDS_PER_SECOND));
  }
 ...
 */
void omrtime_shutdown(struct OMRPortLibrary *portLibrary) { shutdown_timer(); }
/**
 * PortLibrary startup.
 *
