...
omrtime_msec_clock(struct OMRPortLibrary* portLibrary)
{
  ...
  /* Try to use the performance counter, but revert to GetTickCount() if a
   * problem occurs */
                           ...
                           uintptr_t* success)
{
  ...
  /* subtract 116444736000000000 = number of 100ns from jan 1, 1601 to jan 1,
   * 1970 */
...
omrtime_nano_time(struct OMRPortLibrary* portLibrary)
{
  ...
  if (PPG_time_hiresClockFrequency == OMRTIME_NANOSECONDS_PER_SECOND) {
    ...
    nanos = (int64_t)(
      ticks * (OMRTIME_NANOSECONDS_PER_SECOND / PPG_time_hiresClockFrequency));
  } else {
    nanos = (int64_t)(
      ticks / (PPG_time_hiresClockFrequency / OMRTIME_NANOSECONDS_PER_SECOND));
  }
                    ...
                    uint64_t requiredResolution)
{
  ...
  if (frequency == requiredResolution) {
    ...
    ticks = (uint64_t)((double)ticks *
                       ((double)requiredResolution / (double)frequency));
  } else {
    ticks = (uint64_t)((double)ticks /
                       ((double)frequency / (double)requiredResolution));
  }
  return ticks;
}
