...
static uintptr_t EsSharedLibraryLookupName(uintptr_t descriptor, char *name,
                                           uintptr_t *func);
...
uintptr_t omrsl_open_shared_library(struct OMRPortLibrary *portLibrary,
  ...
  if (!openExec) {
    if (decorate) {
      pathLength = portLibrary->str_printf(portLibrary, mangledName,
                                           (EsMaxPath + 1), "%s.dll", name);
    ...
    if (NULL == dllHandle) {
      BOOLEAN useNlsMessage = OMR_ARE_ALL_BITS_SET(
          flags, OMRPORT_SLOPEN_NO_LOOKUP_MSG_FOR_NOT_FOUND);
      BOOLEAN platformErrorFailed = FALSE;

      error = GetLastError();
      /* Check whether a system error message is requested or not, since
       * retrieving system based error messages can be costly.  If not, use the
       * NLS message instead.  Also, if system message retrieval failed, use NLS
       * messages anyway.
       */
      if (!useNlsMessage) {
        ...
        rc = FormatMessageW(
            msgFlags, NULL, error, /* The error code from above */
            MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), /* Default language */
            (LPWSTR)&errorString, 0, NULL);

        if ((0 != rc) && (NULL != errorString)) {
          port_convertToUTF8(portLibrary, errorString, errBuf,
                             sizeof(errBuf) - 1);
      ...
      if (useNlsMessage || platformErrorFailed) {
        errorMessage = portLibrary->nls_lookup_message(
  ...
  unicodeName = port_convertFromUTF8(portLibrary, openName, unicodeBuffer,
                                     UNICODE_BUFFER_SIZE);
  if (NULL == unicodeName) {
  ...
  /* try again in the port lib dir */
  {
    ...
    if (OMR_ERROR_NONE ==
        detectVMDirectory(portLibDir, EsMaxPath - 2, &cursor)) {
      if (NULL != cursor) {
        /* detectVMDirectory ensures (wcslen(portLibDir) < (EsMaxPath - 2)) so
         * length > 0 */
        size_t length = (EsMaxPath - 2) - wcslen(portLibDir);
        _snwprintf(cursor, length, L"\\%s", unicodeName);
        portLibDir[EsMaxPath - 1] = L'\0';
        dllHandle =
            LoadLibraryExW(portLibDir, NULL, LOAD_WITH_ALTERED_SEARCH_PATH);
        if (dllHandle >= (HINSTANCE)HINSTANCE_ERROR) {
          *descriptor = (uintptr_t)dllHandle;
          SetErrorMode(prevMode);
          if (unicodeBuffer != unicodeName) {
  ...
  /*
   * CVMC 201408:
   * When VM is launched from a network share drive, ERROR_BAD_PATHNAME is
   * generated by loadLibrary() on Windows if an invalid quoted path is detected
   * in the library path list (-Djava.library.path=). In such case, VM fails to
   * process the remaining paths and exits immediately. To fix up the issue,
   * ERROR_BAD_PATHNAME should be covered in the error code list below so as to
   * proceed with the searching.
   */
  notFound = ((ERROR_MOD_NOT_FOUND == error) ||
              (ERROR_DLL_NOT_FOUND == error) || (ERROR_BAD_PATHNAME == error));
  if (notFound) {
    /* try to report a better error message.  Check if the library can be found
     * at all. */
    dllHandle = LoadLibraryExW(unicodeName, NULL,
    ...
    /* When the flag OMRPORT_SLOPEN_NO_LOOKUP_MSG_FOR_NOT_FOUND is set and the
     * error is ERROR_MOD_NOT_FOUND,
     * do not call FormatMessageW() since it cost 700K of memory when called the
     * first time. */
    if ((ERROR_MOD_NOT_FOUND == error) &&
        OMR_ARE_ALL_BITS_SET(flags,
                             OMRPORT_SLOPEN_NO_LOOKUP_MSG_FOR_NOT_FOUND)) {
  ...
  if (0 != sizeof(errBuf)) {
    ...
    if (NULL == unicodeName) {
      ...
      portLibrary->str_printf(portLibrary, errBuf, sizeof(errBuf), errorMessage,
                              error);
      errBuf[sizeof(errBuf) - 1] = '\0';
      SetErrorMode(prevMode);
      Trc_PRT_sl_open_shared_library_Exit3(nameSize * 2 + 2,
                                           notFound ? OMRPORT_SL_NOT_FOUND
                                                    : OMRPORT_SL_INVALID);
      return portLibrary->error_set_last_error_with_message(
          portLibrary, notFound ? OMRPORT_SL_NOT_FOUND : OMRPORT_SL_INVALID,
          errBuf);
    }
  ...
  SetErrorMode(prevMode);
  Trc_PRT_sl_open_shared_library_Exit2(notFound ? OMRPORT_SL_NOT_FOUND
                                                : OMRPORT_SL_INVALID);
  return portLibrary->error_set_last_error_with_message(
      portLibrary, notFound ? OMRPORT_SL_NOT_FOUND : OMRPORT_SL_INVALID,
      errBuf);
...
uintptr_t omrsl_lookup_name(struct OMRPortLibrary *portLibrary,
  ...
  if (0 != result) {
    ...
    /* + 7 comes from: 256 parameters * 8 bytes each = 2048 bytes -> 4 digits +
     * @ symbol + leading '_' + NULL */
    ...
    portLibrary->str_printf(portLibrary, mangledName, length, "_%s@%d", name,
                            (len + pad - 1) * 4);
...
int32_t findError(int32_t errorCode) {
  /* As of now, we don't distinguish between most of the Windows DLL-related
   * error codes, but translate most of these to an OMRPORT_SL_INVALID.
   */
  switch (errorCode) {
