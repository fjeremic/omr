                          ...
                          uintptr_t flags)
{
  ...
  if (!openExec) {
    if (decorate) {
      pathLength = portLibrary->str_printf(
        portLibrary, mangledName, (EsMaxPath + 1), "%s.dll", name);
    ...
    if (NULL == dllHandle) {
      BOOLEAN useNlsMessage =
        OMR_ARE_ALL_BITS_SET(flags, OMRPORT_SLOPEN_NO_LOOKUP_MSG_FOR_NOT_FOUND);
      ...
      if (!useNlsMessage) {
        ...
        if ((0 != rc) && (NULL != errorString)) {
          port_convertToUTF8(
            portLibrary, errorString, errBuf, sizeof(errBuf) - 1);
      ...
      if (useNlsMessage || platformErrorFailed) {
        ...
        portLibrary->str_printf(
          portLibrary, errBuf, sizeof(errBuf), errorMessage, error);
        errBuf[sizeof(errBuf) - 1] = '\0';
      }
      ...
      return portLibrary->error_set_last_error(
        portLibrary, error, findError(error));
    }
  ...
  unicodeName = port_convertFromUTF8(
    portLibrary, openName, unicodeBuffer, UNICODE_BUFFER_SIZE);
  if (NULL == unicodeName) {
  ...
  if (notFound) {
     ...
     * at all. */
    dllHandle = LoadLibraryExW(unicodeName,
                               NULL,
  ...
  if (0 != sizeof(errBuf)) {
    ...
    if (NULL == unicodeName) {
      ...
      Trc_PRT_sl_open_shared_library_Exit3(
        nameSize * 2 + 2, notFound ? OMRPORT_SL_NOT_FOUND : OMRPORT_SL_INVALID);
      return portLibrary->error_set_last_error_with_message(
        portLibrary,
                  ...
                  const char* argSignature)
{
  ...
  if (0 != result) {
    ...
    portLibrary->str_printf(
      portLibrary, mangledName, length, "_%s@%d", name, (len + pad - 1) * 4);
