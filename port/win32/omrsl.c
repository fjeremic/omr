  ...
  struct OMRPortLibrary* portLibrary, char* name, uintptr_t* descriptor, uintptr_t flags)
{
  ...
  if (!openExec) {
    ...
    if (NULL == dllHandle) {
      ...
      if (!useNlsMessage) {
        ...
        DWORD msgFlags = FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ARGUMENT_ARRAY
      ...
      if (useNlsMessage || platformErrorFailed) {
        errorMessage = portLibrary->nls_lookup_message(
          portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_SL_EXECUTABLE_OPEN_ERROR, NULL);
  ...
  if (notFound) {
    ...
    if ((HINSTANCE)0 != dllHandle) {
      if (0 != sizeof(errBuf)) {
        errorMessage = portLibrary->nls_lookup_message(
          portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_SL_UNABLE_TO_RESOLVE_REFERENCES, NULL);
  ...
  if (0 != sizeof(errBuf)) {
    ...
    if (NULL == unicodeName) {
      errorMessage = portLibrary->nls_lookup_message(
        portLibrary, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_SL_INTERNAL_ERROR, NULL);
      portLibrary->str_printf(portLibrary, errBuf, sizeof(errBuf), errorMessage, error);
      errBuf[sizeof(errBuf) - 1] = '\0';
      SetErrorMode(prevMode);
