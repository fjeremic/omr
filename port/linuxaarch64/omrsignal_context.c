  ...
  *name = "";
  ...
  switch (index) {
    ...
    case 6:
      ...
      // case 7:
      // 	*name = "fault_address";
      // 	*value = &((struct sigcontext
      // *)&info->platformSignalInfo.context->uc_mcontext)->fault_address;
      // 	return OMRPORT_SIG_VALUE_ADDRESS;
...
/**
 * Walk the 4k reserve space to get the floating point registers since the
 layout differ
 * per-chip
 * @param	magic_to_find	input	magic number #define in the linux kernel
 (sigcontext.h)\
 ...
 * @see linux/arch/arm64/include/uapi/asm/sigcontext.h for the defines and
 information to why we need
 * to walk the space
           ...
           void** value)
{
  *name = "";
  ...
  /**
   * per documentation these are 128 bits registers, can be used as 64, 32 and
   * 16 bit floats (16 bit floats CANNOT be used for arithmetic)
   */

  if ((0 <= index) && (NFPRS > index)) {
    struct sigcontext* const context =
      (struct sigcontext*)&info->platformSignalInfo.context->uc_mcontext;
    struct fpsimd_context* const fp_regs =
      walkReserveSpace(FPSIMD_MAGIC, (uint8_t*)context->__reserved);

    const char* n_fpr[NFPRS] = { "V0",  "V1",  "V2",  "V3",  "V4",  "V5",
                                 "V6",  "V7",  "V8",  "V9",  "V10", "V11",
           ...
           void** value)
{
  *name = "";
  ...
  if ((0 <= index) && (NGPRS > index)) {
    struct sigcontext* const context =
      (struct sigcontext*)&info->platformSignalInfo.context->uc_mcontext;
    const char* n_gpr[NGPRS] = { "R0",  "R1",  "R2",  "R3",  "R4",  "R5",
                                 "R6",  "R7",  "R8",  "R9",  "R10", "R11",
