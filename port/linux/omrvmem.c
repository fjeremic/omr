...
static intptr_t initializeNumaGlobals(struct OMRPortLibrary* portLibrary)
{
    ...
    /* look-up the process's default NUMA policy as applied to the set of nodes (the policy is usually "0" and applies
     * to no nodes) */
    ...
    if (0 == mempolicyReturnCode) {
        ...
        /* Check if any bits are set in PPG_numa_mempolicy_node_mask. If no nodes are specified in either the system
         * default or process default policies then the mask will be all 0's which (if used later) would indicate no
         * nodes are available for virtual memory allocation. */
        for (i = 0; ((0 == anySet) && (i < arrayLen)); i++) {
            anySet |= PPG_numa_mempolicy_node_mask.mask[i];
        }
        if (0 == anySet) {
            /* If an empty set is returned, e.g. if numactl is not used, ask again with MPOL_F_MEMS_ALLOWED to get the
             * correct node mask. */
            ...
            /* MPOL_F_MEMS_ALLOWED is unavailable on older systems. If it fails, we use PPG_numa_available_node_mask
             * instead. */
            if (0 != mempolicyReturnCode) {
 ...
 * 											 The returned memory address should be within the range
 * defined by the @param start and the @param end.
 * @param uintptr_t 	byteAmount		[in] The block size required.
 * @param BOOLEAN 		reverse			[in] Returns the first available memory block when this param equals
 * FALSE, returns the last available memory block when this param equals TRUE
    ...
    if ((fd = omrfile_open(portLibrary, VMEM_PROC_MAPS_FNAME, EsOpenRead, 0)) != -1) {
        ...
        while (TRUE) { /* read-file-loop */
            ...
            do { /* proc-chars-loop */
                 ...
                 * 2. the size of the 'lineBuf' is fixed, we skip the extra chars in order to avoid
                 * out-of-bound-writting
                 */
    ...
    struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier, struct J9PortVmemParams* params)
{
    ...
    if (0 == params->pageSize) {
        ...
        if ((0 == largePageMinimumGranule) || (0 == (largePageAlignmentInBytes % largePageMinimumGranule))) {
            memoryPointer
                = reserveLargePages(portLibrary, identifier, category, params->byteAmount, params->startAddress,
        ...
        if (NULL == memoryPointer) {
            ...
            if (OMR_ARE_NO_BITS_SET(params->options, OMRPORT_VMEM_STRICT_PAGE_SIZE)) {
                ...
                if ((0 == minimumGranule) || (0 == (alignmentInBytes % minimumGranule))) {
                    memoryPointer
                        = getMemoryInRangeForDefaultPages(portLibrary, identifier, category, params->byteAmount,
                            params->startAddress, params->endAddress, alignmentInBytes, params->options, params->mode);
                }
            } else {
                update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, 0, NULL, -1);
    ...
    if ((fd == -1 && (!useBackingSharedFile && !useBackingFile))
        ...
        if (MAP_FAILED == result) {
            ...
            update_vmemIdentifier(identifier, result, result, byteAmount, mode, pageSize,
                OMRPORT_VMEM_PAGE_FLAG_NOT_USED, OMRPORT_VMEM_RESERVE_USED_MMAP, category, fd);
...
void port_numa_interleave_memory(struct OMRPortLibrary* portLibrary, void* start, uintptr_t size)
{
    ...
    if (1 == PPG_numa_platform_interleave_memory) {
        ...
        if (0 != do_mbind(start, size, MPOL_INTERLEAVE, PPG_numa_mempolicy_node_mask.mask, maxnode, 0)) {
            if (0
                != do_mbind(
                    start, size, MPOL_INTERLEAVE, PPG_numa_mempolicy_node_mask.mask, PPG_numa_max_node_bits, 0)) {
    ...
    uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t pageSize, uintptr_t mode)
{
    ...
    allocAnywhere:
        memoryPointer = allocateMemoryForLargePages(
            portLibrary, identifier, NULL, addressKey, category, byteAmount, pageSize, mode);
    }
        ...
        if ((numaNode > 0) && (numaNode <= PPG_numa_max_node_bits) && (numaNode <= maxnode)) {
            ...
            if (bit == (PPG_numa_available_node_mask.mask[wordIndex] & bit)) {
                ...
                if (0 != do_mbind(address, byteAmount, MPOL_PREFERRED, nodeMask.mask, PPG_numa_max_node_bits, 0)) {
                     ...
                     * port_numa_interleave_memory. This assert is useful to determine any problems calling mbind and
                     * improve the performance by succeeding mbind call.
                     *
                     */
                    Assert_PRT_true(FALSE);
                } else {
    ...
    struct OMRPortLibrary* portLibrary, J9MemoryNodeDetail* numaNodes, uintptr_t* nodeCount)
{
    ...
    if (PPG_numa_platform_supports_numa) {
        DIR* nodes = opendir("/sys/devices/system/node/");
        if (NULL == nodes) {
            /* couldn't find the directory - this would be inconsistent since PPG_numa_platform_supports_numa requires
             * seeing this directory, in the current implementation */
            result = OMRPORT_ERROR_VMEM_OPFAILED;
        } else {
            ...
            /* by default, set the SET and CLEAR states to the same value since "default" NUMA mode has all nodes
             * cleared */
            ...
            */
            ...
            while (NULL != (node = readdir(nodes))) {
                ...
                if (1 == sscanf(node->d_name, "node%lu", &nodeIndex)) {
                    if (nodeIndex < PPG_numa_max_node_bits) {
                        ...
                        if (NULL != oneNode) {
                            /* as long as we haven't yet filled the user-provided buffer, populate the entry with the
                             * node details */
                            if (populatedNodeCount < arraySize) {
                                ...
                                while (NULL != (fileEntry = readdir(oneNode))) {
                                    ...
                                    if ((1 == sscanf(fileName, "cpu%lu", &cpuIndex))
                                        ...
                                        /* the CPU exists and is in our process-wide affinity so we know that we can use
                                         * it */
                                        allowedCPUCount += 1;
                                    } else if (0 == strcmp(fileName, "meminfo")) {
                                        /* parse out the memory quantity of this node since a 0-byte node will need to
                                         * be reported as "J9NUMA_DENIED" to ensure that the mbind won't fail with
                                         * EINVAL */
                                        ...
                                        if (NULL != memInfoStream) {
                                            if (1
                                                != fscanf(memInfoStream, " Node %*u MemTotal: %" SCNu64 " kB",
            ...
            if (node == NULL && errno != 0) {
                ...
                /* save back how many entries we saw (the caller knows that the number of entries we populated is the
                 * minimum of how many we were given and how many we found) */
