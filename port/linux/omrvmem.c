...
#define VMEM_TRANSPARENT_HUGEPAGE_FNAME \
...
static BOOLEAN addressRange_Intersect(AddressRange *a, AddressRange *b,
                                      AddressRange *result);
static BOOLEAN addressRange_IsValid(AddressRange *range);
static uintptr_t addressRange_Width(AddressRange *range);
static ADDRESS findAvailableMemoryBlockNoMalloc(
    struct OMRPortLibrary *portLibrary, ADDRESS start, ADDRESS end,
    uintptr_t byteAmount, BOOLEAN reverse, BOOLEAN strictRange);

/*
 * This structure captures the state of an iterator of addresses between minimum
 * and maximum (inclusive). Each address returned will be a multiple of
...
typedef struct AddressIterator {
  ...
  uintptr_t alignment;
  /* addresses are returned in increasing order if this is positive, decreasing
   * order otherwise */
  intptr_t direction;
...
static void addressIterator_init(AddressIterator *iterator, ADDRESS minimum,
                                 ADDRESS maximum, uintptr_t alignment,
                                 intptr_t direction);
static BOOLEAN addressIterator_next(AddressIterator *iterator,
                                    ADDRESS *address);

static void *getMemoryInRangeForLargePages(
...
static void *getMemoryInRangeForDefaultPages(
    struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
    ...
    uintptr_t mode);
static void *allocateMemoryForLargePages(
    struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
...
static void *reserveLargePages(struct OMRPortLibrary *portLibrary,
                               ...
                               uintptr_t pageSize, uintptr_t alignmentInBytes,
                               uintptr_t vmemOptions, uintptr_t mode);
static uintptr_t adviseHugepage(struct OMRPortLibrary *portLibrary,
                                void *address, uintptr_t byteAmount);

static void *default_pageSize_reserve_memory(
...
#endif /* OMR_PORT_NUMA_SUPPORT */
static void update_vmemIdentifier(J9PortVmemIdentifier *identifier,
                                  void *address, void *handle,
                                  uintptr_t byteAmount, uintptr_t mode,
                                  uintptr_t pageSize, uintptr_t pageFlags,
                                  uintptr_t allocator, OMRMemCategory *category,
                                  int fd);
static uintptr_t get_hugepages_info(struct OMRPortLibrary *portLibrary,
                                    vmem_hugepage_info_t *page_info);
static uintptr_t get_transparent_hugepage_info(
    struct OMRPortLibrary *portLibrary);
static int get_protectionBits(uintptr_t mode);

#if defined(OMR_PORT_NUMA_SUPPORT)
/*
 * glibc doesn't include mbind yet. It is in libnuma, but we can't rely on that
 * being available on all systems. We define our own helper to make the syscall
 * so that we can use it without relying on external libraries.
 */
static long do_mbind(void *start, unsigned long len, int policy,
...
/**
 * glibc doesn't include get_mempolicy. It is in libnuma, but we can't rely on
 * that being available on all systems. We define our own helper to make the
 * syscall so that we can use it without relying on external libraries.
 *
 ...
 *
 * @param int           *policy  [out] returns the policy, or next interleave
 * node
 * @param unsigned long *nmask   [out] the node bit mask associated with the
 * policy
 * @param unsigned long  maxnode [in]  length of nmask array
 * @param unsigned long  addr    [in]  virtual memory address of requested
 * policy info
 ...
 */
static long do_get_mempolicy(int *policy, unsigned long *nmask,
...
/*
 * Read the /sys/devices/system/node/ directory, if it exists, to find out the
 * indices of all NUMA nodes on this system. Store them into a bitmask in the
 * port platform globals.
 */
static intptr_t initializeNumaGlobals(struct OMRPortLibrary *portLibrary) {
  uintptr_t result = 0;
  ...
  /* populate the processAffinity with the process's incoming affinity - this
   * will allow us to honour options like "numactl --cpubind=" */
  CPU_ZERO(&PPG_process_affinity);
  schedReturnCode =
      sched_getaffinity(0, sizeof(PPG_process_affinity), &PPG_process_affinity);

  /* look-up the process's default NUMA policy as applied to the set of nodes
   * (the policy is usually "0" and applies to no nodes) */
  PPG_numa_policy_mode = -1;
  memset(&PPG_numa_mempolicy_node_mask, 0, sizeof(J9PortNodeMask));
  mempolicyReturnCode =
      do_get_mempolicy(&PPG_numa_policy_mode, PPG_numa_mempolicy_node_mask.mask,
                       maxNodes, (unsigned long)NULL, 0);
  if ((0 != mempolicyReturnCode) && (EPERM == errno)) {
    PPG_numaSyscallNotAllowed = TRUE;
  }
  if (0 == mempolicyReturnCode) {
    long anySet = 0;
    int arrayLen = sizeof(PPG_numa_mempolicy_node_mask.mask) /
                   sizeof(PPG_numa_mempolicy_node_mask.mask[0]);
    int i = 0;

    /* Check if any bits are set in PPG_numa_mempolicy_node_mask. If no nodes
     * are specified in either the system default or process default policies
     * then the mask will be all 0's which (if used later) would indicate no
     * nodes are available for virtual memory allocation. */
    for (i = 0; ((0 == anySet) && (i < arrayLen)); i++) {
      anySet |= PPG_numa_mempolicy_node_mask.mask[i];
    }
    if (0 == anySet) {
      /* If an empty set is returned, e.g. if numactl is not used, ask again
       * with MPOL_F_MEMS_ALLOWED to get the correct node mask. */
      mempolicyReturnCode =
          do_get_mempolicy((int *)NULL, PPG_numa_mempolicy_node_mask.mask,
                           maxNodes, (unsigned long)NULL, MPOL_F_MEMS_ALLOWED);

      /* MPOL_F_MEMS_ALLOWED is unavailable on older systems. If it fails, we
       * use PPG_numa_available_node_mask instead. */
      if (0 != mempolicyReturnCode) {
        Trc_PRT_vmem_omrvmem_initializeNumaGlobals_get_allowed_mems_failure(
            errno);
   ...
   * We proceed to get node count even if `getmempolicy` fails due to security
   * restrictions.
   */
  if ((0 == schedReturnCode) &&
      ((0 == mempolicyReturnCode) || (PPG_numaSyscallNotAllowed))) {
        ...
        if (1 == sscanf(node->d_name, "node%lu", &nodeIndex)) {
          if (nodeIndex < maxNodes) {
            unsigned long wordIndex =
                nodeIndex / sizeof(PPG_numa_available_node_mask.mask[0]);
            unsigned long bit =
                1 << (nodeIndex % sizeof(PPG_numa_available_node_mask.mask[0]));
            PPG_numa_available_node_mask.mask[wordIndex] |= bit;
            /**
             * PPG_numa_max_node_bits represents the maximum number of nodes.
             * Node indexes start from 0, therefore PPG_numa_max_node_bits is
             * max node index number plus 1. For instance, if there are
             * following nodes:node0, node5, node8, then PPG_numa_max_node_bits
             * is equal to 8 + 1 = 9
             */
  ...
  if (nodeCount < 2) {
    ...
    /* Failed to get memory policy allowed nodes, use all available nodes
     * instead. */
 ...
 * @param void*  		end		[in]	The end address of the
 * range
 */
static void addressRange_Init(AddressRange *range, ADDRESS start, ADDRESS end) {
  range->start = start;
 ...
 * In order to avoid file context corruption, this method implemented without
 * memory operation such as malloc/free
 *
 ...
 * see also @param start. The returned memory address should be within the range
 * defined by the @param start and the @param end.
 * @param uintptr_t 	byteAmount		[in] The block size required.
 * @param BOOLEAN 		reverse			[in] Returns the first
 * available memory block when this param equals FALSE, returns the last
 * available memory block when this param equals TRUE
 * @param BOOLEAN       strictRange     [in] Must the returned address within
 * the range defined by @param start and @param end?
 *
...
static ADDRESS findAvailableMemoryBlockNoMalloc(
  ...
  if ((fd = omrfile_open(portLibrary, VMEM_PROC_MAPS_FNAME, EsOpenRead, 0)) !=
     ...
     * For example: "1234567890120000-1234567890123000" (NOTE: there is a '\0'
     * in the end)
     */
    char lineBuf[100];
    ...
    while (TRUE) { /* read-file-loop */
      ...
      do { /* proc-chars-loop */
         ...
         * 1. we only need the memory range data, it always at the beginning of
         * the lines.
         * 2. the size of the 'lineBuf' is fixed, we skip the extra chars in
         * order to avoid out-of-bound-writting
         */
        ...
        if (lineEnd) { /* proc-line-data */
          ...
          if (gotEOF) {
            ...
            addressRange_Init(&currentMmapRange, (ADDRESS)(uintptr_t)(-1),
                              (ADDRESS)(uintptr_t)(-1));
          } else {
            char *next = NULL;
            uintptr_t start = 0;
          ...
          if (currentMmapRange.start == lastMmapRange.end) {
             ...
             */
            haveIntersect = addressRange_Intersect(&allowedRange, &freeRange,
                                                   &intersectAvailable);
            if (haveIntersect &&
                addressRange_Width(&intersectAvailable) >= byteAmount) {
#if defined(OMRVMEM_DEBUG)
              printf("intersection %p-%p\n", intersectAvailable.start,
                     intersectAvailable.end);
              fflush(stdout);
#endif
              ...
              if (FALSE == reverse) {
                break;
                       ...
                       (addressRange_Width(&freeRange) >= byteAmount)) {
...
int32_t omrvmem_startup(struct OMRPortLibrary *portLibrary) {
  ...
  /* clear page info data, this has the effect of starting off in a standard
   * state */
  ...
  /* set value to advise OS about vmem to consider for Transparent HugePage
   * (Only for Linux) */
  portLibrary->portGlobals->vmemEnableMadvise =
      get_transparent_hugepage_info(portLibrary);
...
void *omrvmem_commit_memory(struct OMRPortLibrary *portLibrary, void *address,
  ...
  if (rangeIsValid(identifier, address, byteAmount)) {
    ...
    if (PPG_vmem_pageSize[0] == identifier->pageSize ||
      ...
      if (0 ==
          mprotect(address, byteAmount, get_protectionBits(identifier->mode))) {
#if defined(OMRVMEM_DEBUG)
    ...
    if (rangeIsValid(identifier, address, byteAmount)) {
      ...
      if (byteAmount > 0) {
        if (identifier->allocator == OMRPORT_VMEM_RESERVE_USED_MMAP) {
          result = (intptr_t)madvise((void *)address, (size_t)byteAmount,
                                     MADV_DONTNEED);
        } else {
          /* need to determine what to use in the case of shmat/shmget, till
           * then return success */
          result = 0;
        }

        if (0 != result) {
          Trc_PRT_vmem_omrvmem_decommit_memory_failure(errno, address,
                                                       byteAmount);
        }
      } else {
      ...
      Trc_PRT_vmem_omrvmem_decommit_memory_invalidRange(
          identifier->address, identifier->size, address, byteAmount);
      portLibrary->error_set_last_error(portLibrary, result,
                                        OMRPORT_ERROR_VMEM_INVALID_PARAMS);
    }
  } else {
    if (!rangeIsValid(identifier, address, byteAmount)) {
...
int32_t omrvmem_free_memory(struct OMRPortLibrary *portLibrary, void *address,
  ...
  /* CMVC 180372 - Identifier must be cleared before memory is freed, see
   * comment above */
...
void *omrvmem_reserve_memory_ex(struct OMRPortLibrary *portLibrary,
  ...
  Trc_PRT_vmem_omrvmem_reserve_memory_Entry_replacement(
      params->startAddress, params->byteAmount, params->pageSize);
  ...
  if (0 == params->pageSize) {
    ...
    if ((0 == minimumGranule) || (0 == (alignmentInBytes % minimumGranule))) {
      memoryPointer = getMemoryInRangeForDefaultPages(
          portLibrary, identifier, category, params->byteAmount,
    ...
    if ((0 == largePageMinimumGranule) ||
      ...
      memoryPointer = reserveLargePages(
          portLibrary, identifier, category, params->byteAmount,
    ...
    if (NULL == memoryPointer) {
      ...
      if (OMR_ARE_NO_BITS_SET(params->options, OMRPORT_VMEM_STRICT_PAGE_SIZE)) {
        ...
        uintptr_t alignmentInBytes =
            OMR_MAX(defaultPageSize, params->alignmentInBytes);
        uintptr_t minimumGranule =
            OMR_MIN(defaultPageSize, params->alignmentInBytes);
...
#if defined(OMRVMEM_DEBUG)
  printf(
      "\tomrvmem_reserve_memory_ex(start=%p,end=%p,size=0x%zx,page=0x%zx,"
      "options=0x%zx) returning %p\n",
      params->startAddress, params->endAddress, params->byteAmount,
      params->pageSize, (size_t)params->options, memoryPointer);
  fflush(stdout);
#endif
...
static void *reserveLargePages(struct OMRPortLibrary *portLibrary,
                               ...
                               uintptr_t pageSize, uintptr_t alignmentInBytes,
                               uintptr_t vmemOptions, uintptr_t mode) {
  /* The address should usually be passed in as NULL in order to let the kernel
   * find and assign a large enough window of virtual memory
   *
   * Requesting HUGETLB pages via shmget has the effect of "reserving" them.
   * They have to be attached to become allocated for use The execute flags are
   * ignored by shmget
   */
  key_t addressKey;
  ...
  if (-1 == addressKey) {
    ...
    memoryPointer = getMemoryInRangeForLargePages(
        portLibrary, identifier, addressKey, category, byteAmount, startAddress,
    ...
    if (NULL != memoryPointer) {
      ...
      if (0 != (OMRPORT_VMEM_MEMORY_MODE_COMMIT & mode)) {
        if (NULL == omrvmem_commit_memory(portLibrary, memoryPointer,
 ...
 *
 * Notify kernel that the virtual memory region specified by address and
 * byteAmount should be labelled with MADV_HUGEPAGE, where the khugepage process
 * could promote to THP when possible.
 *
 ...
 *
 * @return 0 on success, OMRPORT_ERROR_VMEM_OPFAILED if an error occurred, or
...
static uintptr_t adviseHugepage(struct OMRPortLibrary *portLibrary,
  ...
  if (portLibrary->portGlobals->vmemEnableMadvise) {
    ...
    start =
        start + ((start % PPG_vmem_pageSize[0])
                     ? (PPG_vmem_pageSize[0] - (start % PPG_vmem_pageSize[0]))
                     : 0);
...
static uintptr_t get_transparent_hugepage_info(
  ...
  fd =
      omrfile_open(portLibrary, VMEM_TRANSPARENT_HUGEPAGE_FNAME, EsOpenRead, 0);
  if (fd < 0) {
    return 0;
  }

  bytes_read =
      omrfile_read(portLibrary, fd, read_buf, VMEM_MEMINFO_SIZE_MAX - 1);
  ...
  if (!strncmp(read_buf, VMEM_TRANSPARENT_HUGEPAGE_MADVISE,
               VMEM_TRANSPARENT_HUGEPAGE_MADVISE_LENGTH)) {
    return 1;
  }
...
static uintptr_t get_hugepages_info(struct OMRPortLibrary *portLibrary,
  ...
  /** Why this is data is not available via a well defined system call remains a
   * mystery. Meanwhile we have to parse /proc/meminfo to figure out how many
   * pages are available/free as well as their size
   */
  line_ptr = read_buf;
  while (line_ptr && *line_ptr) {
    ...
    int tokens_assigned =
        sscanf(line_ptr, "%127s %" SCNuPTR " %*s", token_name, &token_value);

#ifdef LPDEBUG
    portLibrary->tty_printf(
        portLibrary, VMEM_PROC_MEMINFO_FNAME " => %s [%" PRIuPTR "] %d\n",
    ...
    if (2 == tokens_assigned) {
      if (!strcmp(token_name, "HugePages_Total:")) {
        ...
        page_info->page_size =
            token_value * 1024; /* value is in kB, convert to bytes */
      }
    }
...
static void *default_pageSize_reserve_memory(
  ...
  if (useBackingSharedFile) {
    ...
    sprintf(filename, "omrvmem_temp_%zu_%09d_%zu",
            (size_t)omrtime_current_time_millis(portLibrary), getpid(),
            (size_t)pthread_self());
  ...
  if ((fd == -1 && (!useBackingSharedFile && !useBackingFile)) ||
    ...
    if (MAP_FAILED == result) {
      if (useBackingSharedFile && fd != -1) close(fd);
      result = NULL;
    } else {
      ...
      update_vmemIdentifier(identifier, result, result, byteAmount, mode,
                            ...
                            OMRPORT_VMEM_RESERVE_USED_MMAP, category, fd);
      omrmem_categories_increment_counters(category, byteAmount);
      if (0 != (OMRPORT_VMEM_MEMORY_MODE_COMMIT & mode)) {
        if (NULL == omrvmem_commit_memory(portLibrary, result, byteAmount,
                                          identifier)) {
          /* If the commit fails free the memory */
 ...
 *
 *  @param OMRPortLibrary       *portLibrary                            [in] The
 * portLibrary object
 *  @param void*                addressesOffeset[] [in] Addresses to be double
 * mapped
 *  @param uintptr_t            byteAmount [in] Total size to allocate for
 * contiguous block of memory
 *  @param struct J9PortVmemIdentifier *oldIdentifier   [in]  old Identifier
 * containing file descriptor
 *  @param struct J9PortVmemIdentifier *newIdentifier   [out] new Identifier for
 * new block of memory. The structure to be updated
...
void *omrvmem_get_contiguous_region_memory(
  ...
  contiguousMap = mmap(NULL, byteAmount, protectionFlags, flags, fd, 0);

  if (contiguousMap == MAP_FAILED) {
    contiguousMap = NULL;
    portLibrary->error_set_last_error_with_message(
        portLibrary, OMRPORT_ERROR_VMEM_OPFAILED,
        "Failed to map contiguous block of memory");
  } else {
    shouldUnmapAddr = TRUE;
    successfulContiguousMap = TRUE;
    /* Update identifier and commit memory if required, else return reserved
     * memory */
    update_vmemIdentifier(newIdentifier, contiguousMap, contiguousMap,
                          byteAmount, mode, pageSize,
                          OMRPORT_VMEM_PAGE_FLAG_NOT_USED,
                          OMRPORT_VMEM_RESERVE_USED_MMAP, category, -1);
    omrmem_categories_increment_counters(category, byteAmount);
    if (0 != (OMRPORT_VMEM_MEMORY_MODE_COMMIT & mode)) {
      if (NULL == omrvmem_commit_memory(portLibrary, contiguousMap, byteAmount,
                                        newIdentifier)) {
        /* If the commit fails free the memory  */
#if defined(OMRVMEM_DEBUG)
        printf(
            "omrvmem_commit_memory failed at "
            "contiguous_region_reserve_memory.\n");
        fflush(stdout);
#endif
  ...
  if (contiguousMap != NULL) {
    ...
    printf(
        "About to double map arraylets. File handle got from old identifier: "
        "%d\n",
        fd);
    fflush(stdout);
#endif
    ...
    for (i = 0; i < addressesCount; i++) {
      ...
      uintptr_t addressOffset =
          (uintptr_t)(addresses[i] - (uintptr_t)oldIdentifier->address);
      void *address = mmap(nextAddress, addressSize, protectionFlags, flags, fd,
                           addressOffset);

      if (address == MAP_FAILED) {
        successfulContiguousMap = FALSE;
        portLibrary->error_set_last_error_with_message(
            portLibrary, OMRPORT_ERROR_VMEM_OPFAILED, "Failed to double map.");
#if defined(OMRVMEM_DEBUG)
        printf("***************************** errno: %d\n", errno);
        ...
        portLibrary->error_set_last_error_with_message(
            portLibrary, OMRPORT_ERROR_VMEM_OPFAILED,
            "Double Map failed to provide the correct address");
#if defined(OMRVMEM_DEBUG)
        printf(
            "Map failed to provide the correct address. nextAddress %p != %p\n",
            nextAddress, address);
        fflush(stdout);
#endif
        break;
...
#if defined(OMR_PORT_NUMA_SUPPORT)
void port_numa_interleave_memory(struct OMRPortLibrary *portLibrary,
                                 void *start, uintptr_t size) {
  Trc_PRT_vmem_port_numa_interleave_memory_enter();

  if (1 == PPG_numa_platform_interleave_memory) {
    /* There seem to be variations in different kernel levels regarding how they
     * treat the maxnode argument. Depending on the system we're running on, it
     * may fail if we specify a mask that is greater than the number of nodes.
     *
     * To support both configs, we first try to pass in the count of all bits in
     * the mask, regardless of how many we're using (the unused ones are zero
     * and will simply be ignored). If that fails, we try again, this time
     * passing in the actual number of nodes.
     */
    unsigned long maxnode = sizeof(J9PortNodeMask) * 8;

    if (0 != do_mbind(start, size, MPOL_INTERLEAVE,
                      PPG_numa_mempolicy_node_mask.mask, maxnode, 0)) {
      if (0 != do_mbind(start, size, MPOL_INTERLEAVE,
                        PPG_numa_mempolicy_node_mask.mask,
                        PPG_numa_max_node_bits, 0)) {
        /* record the error in the trace buffer and assert */
        Trc_PRT_vmem_port_numa_interleave_memory_mbind_failed(errno,
                                                              strerror(errno));

        /* There are cases where mbind() can fail that we are currently not
         * handling. Instead of asserting, and thereby bringing down the VM in
         * such cases, simply record the above tracepoint and continue
         * execution, since this isn't a fatal error.
         *
         * In one instance on RHEL6, it was observed that the
         * /sys/devices/system/node/ filesystem had entries for node0 and node4,
         * but setting node0 in the mask would cause the mbind() call to fail.
         * On this sytem, there was a file
         * /sys/devices/system/node/has_normal_memory which only listed node 4.
         * It might make sense to parse that file, if it exists, and to only set
         * the bits for nodes that are listed in that file. Alternatively we
         * could test each node using mbind() with only the node's bit set, to
         * determine if we should set that bit in
         * PPG_numa_available_node_mask.mask.
         */
#if 0
...
void omrvmem_default_large_page_size_ex(struct OMRPortLibrary *portLibrary,
                                        ...
                                        uintptr_t *pageFlags) {
#if defined(LINUXPPC)
  if (OMRPORT_VMEM_MEMORY_MODE_EXECUTE !=
...
#endif /* defined(LINUXPPC) */
  {
     ...
     * supported (platforms with more complicated logic will use their own
     * special process to determine the best response
     */
    if (NULL != pageSize) {
...
intptr_t omrvmem_find_valid_page_size(struct OMRPortLibrary *portLibrary,
  ...
  if (0 != validPageSize) {
#if defined(LINUXPPC)
    /* On Linux PPC, for executable pages, search through the list of supported
     * page sizes only if
     ...
     * TR_ppcCodeCacheConsolidationEnabled is a manually set env variable used
     * to indicate that Code Cache Consolidation is enabled in the JIT.
     */
    BOOLEAN codeCacheConsolidationEnabled = FALSE;

    if (OMRPORT_VMEM_MEMORY_MODE_EXECUTE ==
        (OMRPORT_VMEM_MEMORY_MODE_EXECUTE & mode)) {
      if (portLibrary->sysinfo_get_env(portLibrary,
...
static void *getMemoryInRangeForDefaultPages(
  ...
  if (OMR_ARE_ANY_BITS_SET(vmemOptions, OMRPORT_VMEM_ALLOC_DIR_TOP_DOWN)) {
    direction = -1;
  } else if (OMR_ARE_ANY_BITS_SET(vmemOptions,
                                  OMRPORT_VMEM_ALLOC_DIR_BOTTOM_UP)) {
    direction = 1;
  } else if ((NULL == startAddress) &&
             (OMRPORT_VMEM_MAX_ADDRESS == endAddress)) {
    /* if caller specified the entire address range and does not care about the
     * direction save time by letting OS choose where to allocate the memory
     */
    goto allocAnywhere;
  }
  ...
  if (OMR_ARE_ANY_BITS_SET(vmemOptions, OMRPORT_VMEM_ALLOC_QUICK)) {
    void *smartAddress = findAvailableMemoryBlockNoMalloc(
        portLibrary, startAddress, endAddress, byteAmount,
        (1 == direction) ? FALSE : TRUE,
        OMR_ARE_ANY_BITS_SET(vmemOptions, OMRPORT_VMEM_STRICT_ADDRESS) ? TRUE
                                                                       : FALSE);

    if (NULL != smartAddress) {
      /* smartAddress is not NULL: try to get memory there */
      memoryPointer = default_pageSize_reserve_memory(
          portLibrary, smartAddress, byteAmount, identifier, mode,
      ...
      if (NULL != memoryPointer) {
        ...
        } else if ((startAddress <= memoryPointer) &&
                   (memoryPointer <= endAddress)) {
          /* the address is in the requested range */
        } else {
          /* the address is outside of the range, free it */
          if (0 != omrvmem_free_memory(portLibrary, memoryPointer, byteAmount,
                                       identifier)) {
  ...
  if (NULL == memoryPointer) {
    ...
    if (OMR_ARE_ANY_BITS_SET(vmemOptions, OMRPORT_VMEM_ADDRESS_HINT)) {
      return default_pageSize_reserve_memory(portLibrary, startAddress,
                                             byteAmount, identifier, mode,
                                             PPG_vmem_pageSize[0], category);
    }
    /* try all addresses within range */
    addressIterator_init(&iterator, startAddress, endAddress, alignmentInBytes,
                         direction);
    while (addressIterator_next(&iterator, &currentAddress)) {
      memoryPointer = default_pageSize_reserve_memory(
          portLibrary, currentAddress, byteAmount, identifier, mode,
          PPG_vmem_pageSize[0], category);
  ...
  /* if strict flag is not set and we did not get any memory, attempt to get
   * memory at any address */
  if (OMR_ARE_NO_BITS_SET(vmemOptions, OMRPORT_VMEM_STRICT_ADDRESS) &&
      (NULL == memoryPointer)) {
  allocAnywhere:
    memoryPointer = default_pageSize_reserve_memory(
        portLibrary, NULL, byteAmount, identifier, mode, PPG_vmem_pageSize[0],
        category);
  }
...
static void *getMemoryInRangeForLargePages(
  ...
  while (addressIterator_next(&iterator, &currentAddress)) {
    ...
    memoryPointer = allocateMemoryForLargePages(
        portLibrary, identifier, currentAddress, addressKey, category,
  ...
  allocAnywhere:
    memoryPointer =
        allocateMemoryForLargePages(portLibrary, identifier, NULL, addressKey,
                                    category, byteAmount, pageSize, mode);
  }

  if (MAP_FAILED == memoryPointer) {
    Trc_PRT_vmem_omrvmem_reserve_memory_shmat_failed2(byteAmount, startAddress,
                                                      endAddress);
    memoryPointer = NULL;
  } else if (isStrictAndOutOfRange(memoryPointer, startAddress, endAddress,
                                   vmemOptions)) {
...
/**
 * Attempts to allocate memory at currentAddress by delegating to the
 * appropriate function depending on whether or not the default page size is
 * being used. Returns pointer to newly allocated memory upon success and if
 * default page size is being used NULL upon failure else -1 upon failure.
 */
static void *allocateMemoryForLargePages(
    struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
    void *currentAddress, key_t addressKey, OMRMemCategory *category,
  ...
  if (MAP_FAILED != memoryPointer) {
    update_vmemIdentifier(identifier, memoryPointer,
                          (void *)(uintptr_t)addressKey, byteAmount, mode,
                          pageSize, OMRPORT_VMEM_PAGE_FLAG_NOT_USED,
                          OMRPORT_VMEM_RESERVE_USED_SHM, category, -1);
    omrmem_categories_increment_counters(category, byteAmount);
  }
...
intptr_t omrvmem_numa_set_affinity(struct OMRPortLibrary *portLibrary,
  ...
  if ((1 == PPG_numa_platform_supports_numa) &&
      (OMR_ARE_NO_BITS_SET(identifier->mode, OMRPORT_VMEM_NO_AFFINITY))) {
    unsigned long maxnode = sizeof(J9PortNodeMask) * 8;

    if ((numaNode > 0) && (numaNode <= PPG_numa_max_node_bits) &&
        (numaNode <= maxnode)) {
      ...
      /* Check if this node actually exists. (The PPG_numa_available_node_mask
       * has all valid nodes set.) */
      if (bit == (PPG_numa_available_node_mask.mask[wordIndex] & bit)) {
         ...
         *
         */
        if (0 != do_mbind(address, byteAmount, MPOL_PREFERRED, nodeMask.mask,
                          PPG_numa_max_node_bits, 0)) {
          /* record the error in the trace buffer and assert */
           ...
           * This might not be a fatal error and we could ignore it and continue
           * running just like in port_numa_interleave_memory. This assert is
           * useful to determine any problems calling mbind and improve the
           * performance by succeeding mbind call.
           *
           */
          Assert_PRT_true(FALSE);
        } else {
...
intptr_t omrvmem_numa_get_node_details(struct OMRPortLibrary *portLibrary,
  ...
  if (PPG_numa_platform_supports_numa) {
    DIR *nodes = opendir("/sys/devices/system/node/");
    if (NULL == nodes) {
      /* couldn't find the directory - this would be inconsistent since
       * PPG_numa_platform_supports_numa requires seeing this directory, in the
      ...
      /* by default, set the SET and CLEAR states to the same value since
       * "default" NUMA mode has all nodes cleared */
      ...
      /* walk through the /sys/devices/system/node/ directory to find each
       * individual node */
      ...
      */
      ...
      while (NULL != (node = readdir(nodes))) {
        ...
        if (1 == sscanf(node->d_name, "node%lu", &nodeIndex)) {
          if (nodeIndex < PPG_numa_max_node_bits) {
            DIR *oneNode = NULL;
            char nodeName[sizeof("/sys/devices/system/node/") + NAME_MAX + 1] =
            ...
            if (NULL != oneNode) {
              /* as long as we haven't yet filled the user-provided buffer,
               * populate the entry with the node details */
              if (populatedNodeCount < arraySize) {
                J9MemoryState memoryState = nodeClearState;

                /* find each CPU and check to see if it is part of our global
                 * mask */
                ...
                while (NULL != (fileEntry = readdir(oneNode))) {
                  ...
                  if ((1 == sscanf(fileName, "cpu%lu", &cpuIndex)) &&
                    ...
                    /* the CPU exists and is in our process-wide affinity so we
                     * know that we can use it */
                    allowedCPUCount += 1;
                  } else if (0 == strcmp(fileName, "meminfo")) {
                    /* parse out the memory quantity of this node since a 0-byte
                     * node will need to be reported as "J9NUMA_DENIED" to
                     * ensure that the mbind won't fail with EINVAL */
                    ...
                    if (NULL != memInfoStream) {
                      if (1 != fscanf(memInfoStream,
                                      " Node %*u MemTotal: %" SCNu64 " kB",
                ...
                /* check to see if this node was described in the default policy
                 * for the process */
                if (0 != (PPG_numa_mempolicy_node_mask.mask[nodeIndex / 8] &
                          (1 << nodeIndex % 8))) {
                  memoryState = nodeSetState;
                }
                /* if we didn't find any memory on this node, don't let us use
                 * it */
                ...
                numaNodes[populatedNodeCount].computationalResourcesAvailable =
                    allowedCPUCount;
              }
      ...
      if (node == NULL && errno != 0) {
        ...
        /* save back how many entries we saw (the caller knows that the number
         * of entries we populated is the minimum of how many we were given and
         * how many we found) */
...
int32_t omrvmem_get_available_physical_memory(
  ...
  if (availablePages < 0) {
    intptr_t sysconfError = (intptr_t)errno;
    Trc_PRT_vmem_get_available_physical_memory_failed("availablePages",
                                                      sysconfError);
    return OMRPORT_ERROR_VMEM_OPFAILED;
  }
...
int32_t omrvmem_get_process_memory_size(struct OMRPortLibrary *portLibrary,
  ...
  if (pageSize <= 0) {
    ...
    if (NULL != statmStream) {
      ...
      int parsedFields = fscanf(statmStream, "%lu %lu %lu", &programSize,
                                &residentSize, &sharedSize);
      if (3 == parsedFields) {
        result = 0;
        switch (queryType) {
