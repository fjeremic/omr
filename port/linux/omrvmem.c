...
static void
addressIterator_init(AddressIterator* iterator,
                     ADDRESS minimum,
                     ADDRESS maximum,
                     uintptr_t alignment,
                     intptr_t direction);
...
allocateMemoryForLargePages(struct OMRPortLibrary* portLibrary,
                            ...
                            uintptr_t mode);
static BOOLEAN
isStrictAndOutOfRange(void* memoryPointer,
                      void* startAddress,
                      void* endAddress,
                      uintptr_t vmemOptions);
static BOOLEAN
...
reserveLargePages(struct OMRPortLibrary* portLibrary,
                  ...
                  uintptr_t mode);
static uintptr_t
adviseHugepage(struct OMRPortLibrary* portLibrary,
               void* address,
               uintptr_t byteAmount);
...
#if defined(OMR_PORT_NUMA_SUPPORT)
static void
port_numa_interleave_memory(struct OMRPortLibrary* portLibrary,
                            void* start,
                            uintptr_t size);
#endif /* OMR_PORT_NUMA_SUPPORT */
static void
...
initializeNumaGlobals(struct OMRPortLibrary* portLibrary)
{
  ...
  mempolicyReturnCode = do_get_mempolicy(&PPG_numa_policy_mode,
  ...
  if (0 == mempolicyReturnCode) {
    ...
    if (0 == anySet) {
      ...
      mempolicyReturnCode = do_get_mempolicy((int*)NULL,
                                             PPG_numa_mempolicy_node_mask.mask,
                                             maxNodes,
                                             (unsigned long)NULL,
                                             MPOL_F_MEMS_ALLOWED);
  ...
  if (nodeCount < 2) {
     ...
     * instead. */
    memcpy(PPG_numa_mempolicy_node_mask.mask,
      ...
      do { /* proc-chars-loop */
        ...
        if (lineEnd) { /* proc-line-data */
          ...
          if (currentMmapRange.start == lastMmapRange.end) {
            ...
            addressRange_Init(
              &freeRange, lastMmapRange.end, currentMmapRange.start);
             ...
             */
            haveIntersect = addressRange_Intersect(
              &allowedRange, &freeRange, &intersectAvailable);
            if (haveIntersect &&
...
#endif
              memcpy(
                &lastAvailableRange, &intersectAvailable, sizeof(AddressRange));
              matchFound = TRUE;
    ...
    if (PPG_vmem_pageSize[0] == identifier->pageSize ||
      ...
      if (0 ==
        ...
        portLibrary->error_set_last_error(
          portLibrary, errno, OMRPORT_ERROR_VMEM_OPFAILED);
      }
    ...
    if (rangeIsValid(identifier, address, byteAmount)) {
      ...
      if (byteAmount > 0) {
        if (identifier->allocator == OMRPORT_VMEM_RESERVE_USED_MMAP) {
          result = (intptr_t)madvise(
            (void*)address, (size_t)byteAmount, MADV_DONTNEED);
        } else {
                  ...
                  uintptr_t mode)
{
  ...
  if (-1 == addressKey) {
    ...
    memoryPointer = getMemoryInRangeForLargePages(portLibrary,
                                                  identifier,
                                                  addressKey,
                                                  category,
                                                  byteAmount,
    ...
    if (NULL != memoryPointer) {
      ...
      if (0 != (OMRPORT_VMEM_MEMORY_MODE_COMMIT & mode)) {
        if (NULL == omrvmem_commit_memory(
                      portLibrary, memoryPointer, byteAmount, identifier)) {
               ...
               uintptr_t byteAmount)
{
  ...
  if (portLibrary->portGlobals->vmemEnableMadvise) {
    ...
    start = start + ((start % PPG_vmem_pageSize[0])
...
get_transparent_hugepage_info(struct OMRPortLibrary* portLibrary)
{
  ...
  if (!strncmp(read_buf,
               VMEM_TRANSPARENT_HUGEPAGE_MADVISE,
                   ...
                   vmem_hugepage_info_t* page_info)
{
  ...
  while (line_ptr && *line_ptr) {
    ...
    portLibrary->tty_printf(portLibrary,
                            VMEM_PROC_MEMINFO_FNAME " => %s [%" PRIuPTR
                                                    "] %d\n",
                            token_name,
                            token_value,
                            tokens_assigned);
                                ...
                                OMRMemCategory* category)
{
  ...
  if (useBackingSharedFile) {
    ...
    sprintf(filename,
            "omrvmem_temp_%zu_%09d_%zu",
            (size_t)omrtime_current_time_millis(portLibrary),
            getpid(),
            (size_t)pthread_self());
    ...
    fd = portLibrary->file_open(
      portLibrary, "/dev/zero", EsOpenRead | EsOpenWrite, 0);
  }
  ...
  if ((fd == -1 && (!useBackingSharedFile && !useBackingFile)) ||
    ...
    if (MAP_FAILED == result) {
      ...
      update_vmemIdentifier(identifier,
                            ...
                            OMRPORT_VMEM_RESERVE_USED_MMAP,
                            category,
                            fd);
      omrmem_categories_increment_counters(category, byteAmount);
      if (0 != (OMRPORT_VMEM_MEMORY_MODE_COMMIT & mode)) {
        if (NULL == omrvmem_commit_memory(
                      portLibrary, result, byteAmount, identifier)) {
          /* If the commit fails free the memory */
                                     ...
                                     OMRMemCategory* category)
{
  ...
  if (contiguousMap == MAP_FAILED) {
    ...
    /* Update identifier and commit memory if required, else return reserved
     * memory */
    update_vmemIdentifier(newIdentifier,
                          contiguousMap,
                          contiguousMap,
                          byteAmount,
                          mode,
    ...
    if (0 != (OMRPORT_VMEM_MEMORY_MODE_COMMIT & mode)) {
      if (NULL == omrvmem_commit_memory(
                    portLibrary, contiguousMap, byteAmount, newIdentifier)) {
        /* If the commit fails free the memory  */
#if defined(OMRVMEM_DEBUG)
  ...
  if (contiguousMap != NULL) {
    ...
    for (i = 0; i < addressesCount; i++) {
      ...
      void* address = mmap(
        nextAddress, addressSize, protectionFlags, flags, fd, addressOffset);
                             ...
                             BOOLEAN* isSizeSupported)
{
  ...
  if (0 != validPageSize) {
    ...
    if (OMRPORT_VMEM_MEMORY_MODE_EXECUTE ==
        (OMRPORT_VMEM_MEMORY_MODE_EXECUTE & mode)) {
      if (portLibrary->sysinfo_get_env(
            portLibrary, "TR_ppcCodeCacheConsolidationEnabled", NULL, 0) !=
          -1) {
                                ...
                                uintptr_t mode)
{
  ...
  if (NULL == memoryPointer) {
    ...
    if (OMR_ARE_ANY_BITS_SET(vmemOptions, OMRPORT_VMEM_ADDRESS_HINT)) {
      return default_pageSize_reserve_memory(portLibrary,
                                             startAddress,
                                             byteAmount,
                                             identifier,
                                             mode,
    ...
    addressIterator_init(
      &iterator, startAddress, endAddress, alignmentInBytes, direction);
  ...
  if (NULL == memoryPointer) {
    memoryPointer = NULL;
  } else if (isStrictAndOutOfRange(
               memoryPointer, startAddress, endAddress, vmemOptions)) {
                              ...
                              uintptr_t mode)
{
  ...
  while (addressIterator_next(&iterator, &currentAddress)) {
    ...
    memoryPointer = allocateMemoryForLargePages(portLibrary,
                                                identifier,
                                                currentAddress,
                                                addressKey,
                                                category,
  ...
  if (MAP_FAILED == memoryPointer) {
    Trc_PRT_vmem_omrvmem_reserve_memory_shmat_failed2(
      byteAmount, startAddress, endAddress);
    memoryPointer = NULL;
  } else if (isStrictAndOutOfRange(
               memoryPointer, startAddress, endAddress, vmemOptions)) {
                            ...
                            uintptr_t mode)
{
  ...
  if (MAP_FAILED != memoryPointer) {
    update_vmemIdentifier(identifier,
                          memoryPointer,
                          (void*)(uintptr_t)addressKey,
                          byteAmount,
                          mode,
                          pageSize,
                          OMRPORT_VMEM_PAGE_FLAG_NOT_USED,
                          OMRPORT_VMEM_RESERVE_USED_SHM,
                          category,
                          -1);
    omrmem_categories_increment_counters(category, byteAmount);
  }
                                ...
                                uint64_t* memorySize)
{
  ...
  if (pageSize <= 0) {
    ...
    if (NULL != statmStream) {
      ...
      int parsedFields = fscanf(
        statmStream, "%lu %lu %lu", &programSize, &residentSize, &sharedSize);
      if (3 == parsedFields) {
        result = 0;
        switch (queryType) {
