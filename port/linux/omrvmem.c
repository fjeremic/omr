...
static ADDRESS findAvailableMemoryBlockNoMalloc(struct OMRPortLibrary* portLibrary, ADDRESS start, ADDRESS end,
...
static void addressIterator_init(
  AddressIterator* iterator, ADDRESS minimum, ADDRESS maximum, uintptr_t alignment, intptr_t direction);
...
static void* getMemoryInRangeForLargePages(struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier,
  ...
  uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t pageSize, uintptr_t mode);
...
static void* allocateMemoryForLargePages(struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier,
  void* currentAddress, key_t addressKey, OMRMemCategory* category, uintptr_t byteAmount, uintptr_t pageSize,
  uintptr_t mode);
...
static void* default_pageSize_reserve_memory(struct OMRPortLibrary* portLibrary, void* address, uintptr_t byteAmount,
...
#endif /* OMR_PORT_NUMA_SUPPORT */
static void update_vmemIdentifier(J9PortVmemIdentifier* identifier, void* address, void* handle, uintptr_t byteAmount,
  uintptr_t mode, uintptr_t pageSize, uintptr_t pageFlags, uintptr_t allocator, OMRMemCategory* category, int fd);
 ...
 */
static long do_mbind(
  void* start, unsigned long len, int policy, const unsigned long* nodemask, unsigned long maxnode, unsigned flags)
{
...
static intptr_t initializeNumaGlobals(struct OMRPortLibrary* portLibrary)
{
  ...
  /* populate the processAffinity with the process's incoming affinity - this will allow us to honour options like
   * "numactl --cpubind=" */
  CPU_ZERO(&PPG_process_affinity);
  schedReturnCode = sched_getaffinity(0, sizeof(PPG_process_affinity), &PPG_process_affinity);

  /* look-up the process's default NUMA policy as applied to the set of nodes (the policy is usually "0" and applies to
   * no nodes) */
  ...
  if (0 == mempolicyReturnCode) {
    ...
    /* Check if any bits are set in PPG_numa_mempolicy_node_mask. If no nodes are specified in either the system default
     * or process default policies then the mask will be all 0's which (if used later) would indicate no nodes are
     * available for virtual memory allocation. */
    for (i = 0; ((0 == anySet) && (i < arrayLen)); i++) {
      anySet |= PPG_numa_mempolicy_node_mask.mask[i];
    }
    if (0 == anySet) {
      /* If an empty set is returned, e.g. if numactl is not used, ask again with MPOL_F_MEMS_ALLOWED to get the correct
       * node mask. */
      mempolicyReturnCode = do_get_mempolicy(
        (int*)NULL, PPG_numa_mempolicy_node_mask.mask, maxNodes, (unsigned long)NULL, MPOL_F_MEMS_ALLOWED);
  ...
  if (nodeCount < 2) {
    ...
    memcpy(
      PPG_numa_mempolicy_node_mask.mask, PPG_numa_available_node_mask.mask, sizeof(PPG_numa_available_node_mask.mask));
  }
 ...
 * 											 The returned memory address should be within the range defined by the @param start and the @param
 * end.
 * @param uintptr_t 	byteAmount		[in] The block size required.
 * @param BOOLEAN 		reverse			[in] Returns the first available memory block when this param equals FALSE, returns the
 * last available memory block when this param equals TRUE
 * @param BOOLEAN       strictRange     [in] Must the returned address within the range defined by @param start and
 * @param end?
 *
      ...
      do { /* proc-chars-loop */
        ...
        if (lineEnd) { /* proc-line-data */
          ...
          if (gotEOF) {
            ...
            if ((currentMmapRange.start >= currentMmapRange.end) || (currentMmapRange.start < lastMmapRange.end)) {
              dataCorrupt = TRUE;
              break;
            }
          }
  ...
  struct OMRPortLibrary* portLibrary, void* address, uintptr_t byteAmount, struct J9PortVmemIdentifier* identifier)
{
  ...
  if (rangeIsValid(identifier, address, byteAmount)) {
    ...
    /* Default page size */
    if (PPG_vmem_pageSize[0] == identifier->pageSize || 0 != (identifier->mode & OMRPORT_VMEM_MEMORY_MODE_EXECUTE)) {
  ...
  struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier, struct J9PortVmemParams* params)
{
  ...
  if (0 == params->pageSize) {
    ...
    if ((0 == largePageMinimumGranule) || (0 == (largePageAlignmentInBytes % largePageMinimumGranule))) {
      memoryPointer = reserveLargePages(portLibrary, identifier, category, params->byteAmount, params->startAddress,
        params->endAddress, params->pageSize, largePageAlignmentInBytes, params->options, params->mode);
    }
  ...
  printf("\tomrvmem_reserve_memory_ex(start=%p,end=%p,size=0x%zx,page=0x%zx,options=0x%zx) returning %p\n",
    params->startAddress, params->endAddress, params->byteAmount, params->pageSize, (size_t)params->options,
    memoryPointer);
  fflush(stdout);
#endif
  ...
  uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
   ...
   *
   * Requesting HUGETLB pages via shmget has the effect of "reserving" them. They have to be attached to become
   * allocated for use The execute flags are ignored by shmget
   */
  key_t addressKey;
  ...
  if (-1 == addressKey) {
    ...
    memoryPointer = getMemoryInRangeForLargePages(portLibrary, identifier, addressKey, category, byteAmount,
      startAddress, endAddress, alignmentInBytes, vmemOptions, pageSize, mode);
  ...
  struct J9PortVmemIdentifier* identifier, uintptr_t mode, uintptr_t pageSize, OMRMemCategory* category)
{
  ...
  if (useBackingSharedFile) {
    ...
    sprintf(filename, "omrvmem_temp_%zu_%09d_%zu", (size_t)omrtime_current_time_millis(portLibrary), getpid(),
      (size_t)pthread_self());
 ...
 *  @param struct J9PortVmemIdentifier *newIdentifier   [out] new Identifier for new block of memory. The structure to
 * be updated
  ...
  struct J9PortVmemIdentifier* newIdentifier, uintptr_t mode, uintptr_t pageSize, OMRMemCategory* category)
{
  ...
  if (contiguousMap == MAP_FAILED) {
    ...
    portLibrary->error_set_last_error_with_message(
      portLibrary, OMRPORT_ERROR_VMEM_OPFAILED, "Failed to map contiguous block of memory");
  } else {
    shouldUnmapAddr = TRUE;
    successfulContiguousMap = TRUE;
    /* Update identifier and commit memory if required, else return reserved memory */
    update_vmemIdentifier(newIdentifier, contiguousMap, contiguousMap, byteAmount, mode, pageSize,
      OMRPORT_VMEM_PAGE_FLAG_NOT_USED, OMRPORT_VMEM_RESERVE_USED_MMAP, category, -1);
    omrmem_categories_increment_counters(category, byteAmount);
    if (0 != (OMRPORT_VMEM_MEMORY_MODE_COMMIT & mode)) {
  ...
  if (contiguousMap != NULL) {
    ...
    for (i = 0; i < addressesCount; i++) {
      ...
      void* address = mmap(nextAddress, addressSize, protectionFlags, flags, fd, addressOffset);
      ...
      if (address == MAP_FAILED) {
        ...
        successfulContiguousMap = FALSE;
        portLibrary->error_set_last_error_with_message(
          portLibrary, OMRPORT_ERROR_VMEM_OPFAILED, "Double Map failed to provide the correct address");
#if defined(OMRVMEM_DEBUG)
...
#if defined(OMR_PORT_NUMA_SUPPORT)
void port_numa_interleave_memory(struct OMRPortLibrary* portLibrary, void* start, uintptr_t size)
{
  ...
  if (1 == PPG_numa_platform_interleave_memory) {
     ...
     * Depending on the system we're running on, it may fail if we specify a mask that is greater than the number of
     * nodes.
     *
  ...
  void* endAddress, uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
  ...
  if (OMR_ARE_ANY_BITS_SET(vmemOptions, OMRPORT_VMEM_ALLOC_QUICK)) {
    void* smartAddress = findAvailableMemoryBlockNoMalloc(portLibrary, startAddress, endAddress, byteAmount,
      (1 == direction) ? FALSE : TRUE, OMR_ARE_ANY_BITS_SET(vmemOptions, OMRPORT_VMEM_STRICT_ADDRESS) ? TRUE : FALSE);

    if (NULL != smartAddress) {
      /* smartAddress is not NULL: try to get memory there */
      memoryPointer = default_pageSize_reserve_memory(
        portLibrary, smartAddress, byteAmount, identifier, mode, PPG_vmem_pageSize[0], category);
      if (NULL != memoryPointer) {
  ...
  if (NULL == memoryPointer) {
    ...
    if (OMR_ARE_ANY_BITS_SET(vmemOptions, OMRPORT_VMEM_ADDRESS_HINT)) {
      return default_pageSize_reserve_memory(
        portLibrary, startAddress, byteAmount, identifier, mode, PPG_vmem_pageSize[0], category);
    }
  ...
  uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t pageSize, uintptr_t mode)
{
  ...
  while (addressIterator_next(&iterator, &currentAddress)) {
    ...
    memoryPointer = allocateMemoryForLargePages(
      portLibrary, identifier, currentAddress, addressKey, category, byteAmount, pageSize, mode);
  ...
  allocAnywhere:
    memoryPointer
      = allocateMemoryForLargePages(portLibrary, identifier, NULL, addressKey, category, byteAmount, pageSize, mode);
  }
  ...
  uintptr_t mode)
{
  ...
  if (MAP_FAILED != memoryPointer) {
    update_vmemIdentifier(identifier, memoryPointer, (void*)(uintptr_t)addressKey, byteAmount, mode, pageSize,
      OMRPORT_VMEM_PAGE_FLAG_NOT_USED, OMRPORT_VMEM_RESERVE_USED_SHM, category, -1);
    omrmem_categories_increment_counters(category, byteAmount);
  }
    ...
    if ((numaNode > 0) && (numaNode <= PPG_numa_max_node_bits) && (numaNode <= maxnode)) {
      ...
      if (bit == (PPG_numa_available_node_mask.mask[wordIndex] & bit)) {
        ...
        if (0 != do_mbind(address, byteAmount, MPOL_PREFERRED, nodeMask.mask, PPG_numa_max_node_bits, 0)) {
           ...
           * This might not be a fatal error and we could ignore it and continue running just like in
           * port_numa_interleave_memory. This assert is useful to determine any problems calling mbind and improve the
  ...
  struct OMRPortLibrary* portLibrary, J9MemoryNodeDetail* numaNodes, uintptr_t* nodeCount)
{
  ...
  if (PPG_numa_platform_supports_numa) {
    DIR* nodes = opendir("/sys/devices/system/node/");
    if (NULL == nodes) {
      /* couldn't find the directory - this would be inconsistent since PPG_numa_platform_supports_numa requires seeing
       * this directory, in the current implementation */
      result = OMRPORT_ERROR_VMEM_OPFAILED;
    } else {
      ...
      */
              ...
              if (populatedNodeCount < arraySize) {
                ...
                while (NULL != (fileEntry = readdir(oneNode))) {
                  ...
                  if ((1 == sscanf(fileName, "cpu%lu", &cpuIndex)) && (CPU_ISSET(cpuIndex, &PPG_process_affinity))) {
                    ...
                    /* parse out the memory quantity of this node since a 0-byte node will need to be reported as
                     * "J9NUMA_DENIED" to ensure that the mbind won't fail with EINVAL */
      ...
      if (node == NULL && errno != 0) {
        ...
        /* save back how many entries we saw (the caller knows that the number of entries we populated is the minimum of
         * how many we were given and how many we found) */
