...
static uintptr_t
getSharedAndPrivateDataSegments(struct OMRPortLibrary* portLibrary,
                                intptr_t fd,
                                uintptr_t addrSize,
                                uint8_t** ppStart,
                                uint8_t** ppEnd);
...
static uintptr_t
readElfHeader(struct OMRPortLibrary* portLibrary,
              intptr_t fd,
              Elf64_Ehdr* pElfheader);
static uintptr_t
findProgramHeader(struct OMRPortLibrary* portLibrary,
                  intptr_t fd,
                  uintptr_t addrSize,
                  Elf64_Off phoff,
                  Elf64_Half phnum,
                  ...
                  uint8_t* end);
static void
copyDataIntoELFCore(struct OMRPortLibrary* portLibrary,
                    intptr_t fd_core,
                    uintptr_t addrSize,
                    Elf64_Off phoff);
static intptr_t
...
insertSharedAndPrivateDataSegments(struct OMRPortLibrary* portLibrary,
                                   char* corePath);
static intptr_t
deriveCoreFileName(struct OMRPortLibrary* portLibrary,
                   char* corePatterFormat,
                   BOOLEAN coreUsesPID,
                   char* baseDir,
                   char* derivedCoreFileName,
                   uintptr_t derivedCoreFileNameLen,
                   pid_t pid,
                   int signalNumber);
...
static void
printProgramHeader(struct OMRPortLibrary* portLibrary,
                   uintptr_t addrSize,
                   void* pelfPgmHdr,
                   uintptr_t count);
static void
printProgramHeaders(struct OMRPortLibrary* portLibrary,
                    intptr_t fd,
                    uintptr_t addrSize,
                    Elf64_Off phoff,
                    Elf64_Half phnum);
...
uintptr_t
renameDump(struct OMRPortLibrary* portLibrary,
           char* filename,
           pid_t pid,
           int signalNumber)
{
  ...
  procRC = getContentsFromProcFileSystem(
    portLibrary, corePatternFileName, corePatternFormat, PATH_MAX);
  if (0 == procRC) {
  ...
  procRC = getContentsFromProcFileSystem(
    portLibrary, coreUsesPIDFileName, coreUsesPIDBuffer, PATH_MAX);
  if (0 == procRC) {
   ...
   * and core_uses_pid */
  createCoreFileNameRC = deriveCoreFileName(portLibrary,
                                            corePatternFormat,
                                            coreUsesPID,
                                            tempPath,
      ...
      case '%':
        ...
        switch (*inCursor) {
          ...
          case 'd': {
            ...
            if (0 > dumpableFlag) {
              charsPrinted = portLibrary->str_printf(
                portLibrary, outCursor, bytesLeft, "%d", dumpableFlag);
            } else {
              portLibrary->str_printf(portLibrary,
                                      derivedCoreFileName,
                                      PATH_MAX,
                                      "%s",
                                      "Failed to obtain dumpable flag");
              return -1;
            }
          } break;
          case 'P':
            charsPrinted = portLibrary->str_printf(
              portLibrary, outCursor, bytesLeft, "%s", "*");
            ++numWildcards;
            break;
          case 'E': {
            char* pathSep = NULL;
            ...
            charsPrinted = portLibrary->str_printf(
              portLibrary, outCursor, bytesLeft, "%s", scratchSpace);
          } break;
          case 'I':
            charsPrinted = portLibrary->str_printf(
              portLibrary, outCursor, bytesLeft, "%s", "*");
          ...
          case 'p':
            ...
            charsPrinted = portLibrary->str_printf(
              portLibrary, outCursor, bytesLeft, "%i", pid);
            break;
          case 'u':
            charsPrinted = portLibrary->str_printf(
              portLibrary,
          ...
          case 's':
            charsPrinted = portLibrary->str_printf(
              portLibrary, outCursor, bytesLeft, "%i", signalNumber);
            break;
          case 'h':
...
#define PR_GET_NAME 16
            ...
            if (prctl(PR_GET_NAME, scratchSpace) >= 0) {
               ...
               * scratchSpace contains the thread name, which is null terminated
               * if shorter than 16 bytes.
               */
              scratchSpace[16] = '\0'; /* ensure the name is terminated by a
                                          null if it is exactly 16 bytes long */
              charsPrinted = portLibrary->str_printf(
                portLibrary, outCursor, bytesLeft, "%s", scratchSpace);
            } else
#endif
            {
              ...
              int readlinkRc =
                readlink("/proc/self/exe", scratchSpace, PATH_MAX);
              ...
              charsPrinted = portLibrary->str_printf(
                portLibrary, outCursor, bytesLeft, "%s", startExeName);
            }
            break;
          case 't':
            charsPrinted = portLibrary->str_printf(
  ...
  *outCursor = '\0';
  ...
  if (!corePatternSpecifiesPercentP && coreUsesPID) {
    charsPrinted = portLibrary->str_printf(portLibrary,
                                           outCursor,
                                           derivedCoreFileNameLen -
                                             (outCursor - derivedCoreFileName),
                                           ".%i",
                                           pid);
    /*outCursor = outCursor + charsPrinted;*/
  }
                   ...
                   uintptr_t count)
{
  ...
  if (32 == addrSize) {
    ...
    portLibrary->tty_printf(
      portLibrary, "===32 bit=== ====pgmhdr# %d====\n", count);
    portLibrary->tty_printf(
      portLibrary, "p_offset %d\n", p32elfPgmHdr->p_offset);
    portLibrary->tty_printf(
      portLibrary, "p_vaddr %lx\n", p32elfPgmHdr->p_vaddr);
    portLibrary->tty_printf(
      portLibrary, "p_paddr %lx\n", p32elfPgmHdr->p_paddr);
    portLibrary->tty_printf(
      portLibrary, "p_filesz %d\n", p32elfPgmHdr->p_filesz);
    portLibrary->tty_printf(
      portLibrary, "p_memsz %ld\n", p32elfPgmHdr->p_memsz);
    portLibrary->tty_printf(
      portLibrary, "p_align %ld\n", p32elfPgmHdr->p_align);
  } else if (64 == addrSize) {
    p64elfPgmHdr = pelfPgmHdr;
    portLibrary->tty_printf(
      portLibrary, "===64 bit=== ====pgmhdr# %d====\n", count);
    portLibrary->tty_printf(
      portLibrary, "p_offset %d\n", p64elfPgmHdr->p_offset);
    portLibrary->tty_printf(
      portLibrary, "p_vaddr %llx\n", p64elfPgmHdr->p_vaddr);
    portLibrary->tty_printf(
      portLibrary, "p_paddr %llx\n", p64elfPgmHdr->p_paddr);
    portLibrary->tty_printf(
      portLibrary, "p_filesz %d\n", p64elfPgmHdr->p_filesz);
    portLibrary->tty_printf(
      portLibrary, "p_memsz %lld\n", p64elfPgmHdr->p_memsz);
    portLibrary->tty_printf(
      portLibrary, "p_align %lld\n", p64elfPgmHdr->p_align);
  }
}
                    ...
                    Elf64_Half phnum)
{
  ...
  if (32 == addrSize) {
    for (count = 0; count < phnum; count++) {
      bytesread = portLibrary->file_read(
        portLibrary, fd, p32elfPgmHdr, sizeof(Elf32_Phdr));
      printProgramHeader(portLibrary, 32, (Elf64_Phdr*)p32elfPgmHdr, count);
    }
  } else if (64 == addrSize) {
    for (count = 0; count < phnum; count++) {
      bytesread = portLibrary->file_read(
        portLibrary, fd, p64elfPgmHdr, sizeof(Elf64_Phdr));
    ...
    while (bytesRead > -1) {
      ...
      if (bytesRead > 0) {
        ...
        if (ch == '\n') {
          ...
          if (32 == addrSize) {
            portLibrary->tty_printf(
              portLibrary, "%lx %lx %s %s\n", start, end, flags, path);
          } else if (64 == addrSize) {
            portLibrary->tty_printf(
              portLibrary, "%llx %llx %s %s\n", start, end, flags, path);
          }
        } else {
          outcursor++;
        }
        ...
        if ('\n' == ch) {
          ...
          if ('s' == attr_flag || 'p' == attr_flag) {
            ...
            if (32 == addrSize) {
              portLibrary->tty_printf(
                portLibrary,
                "found Shared/Private mem: %s 0x%lx->0x%lx\n",
                path,
                start,
                end);
            } else if (64 == addrSize) {
              portLibrary->tty_printf(
                portLibrary,
                "found Shared/Private mem: %s 0x%llx->0x%llx\n",
                path,
                start,
                end);
            }
  ...
  if (count == EI_NIDENT) {
    ...
    if (memcmp(pElfHeader->e_ident, magic, sizeof(magic)) == 0) {
      ...
      if (1 == pElfHeader->e_ident[EI_CLASS]) {
        ...
        count = portLibrary->file_read(portLibrary,
                                       fd,
                                       &p32elfHeader->e_type,
                    ...
                    Elf64_Off phoff)
{
  ...
  if (32 == addrSize) {
    ...
    portLibrary->file_read(
      portLibrary, fd_core, p32elfPgmHdr, sizeof(Elf32_Phdr));
    ...
    if (0 == p32elfPgmHdr->p_filesz) {
      ...
      if (dataOffset != -1) {
        dataSize =
          portLibrary->file_write(portLibrary,
                                  fd_core,
    ...
    portLibrary->file_seek(portLibrary, fd_core, phoff, EsSeekSet);
    portLibrary->file_read(
      portLibrary, fd_core, p64elfPgmHdr, sizeof(Elf64_Phdr));
                                   ...
                                   char* corePath)
{
  ...
  if (fd_maps != -1) {
    ...
    fd_core = portLibrary->file_open(
      portLibrary, corePath, EsOpenWrite | EsOpenRead, 0);
    if (fd_core != -1) {
      ...
      while (getSharedAndPrivateDataSegments(
        portLibrary, fd_maps, addrSize, &start, &end)) {
        /* find a program header that represents this data */
        phoff_update = findProgramHeader(
          portLibrary, fd_core, addrSize, phoff, phnum, start, end);
        if (0 == phoff_update) {
