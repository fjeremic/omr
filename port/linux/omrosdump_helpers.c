...
static void setChecksumMarkAllPagesWritableHeader(
    MarkAllPagesWritableHeader* header);
static char linuxMemoryMapsFile[] = "/proc/self/maps";
static uintptr_t getSharedAndPrivateDataSegments(
...
static uintptr_t readElfHeader(struct OMRPortLibrary* portLibrary,
                               intptr_t fd,
...
static void copyDataIntoELFCore(struct OMRPortLibrary* portLibrary,
                                ...
                                uintptr_t addrSize,
                                Elf64_Off phoff);
static intptr_t getContentsFromProcFileSystem(
    struct OMRPortLibrary* portLibrary,
    const char* fileName,
    char* buffer,
    size_t buflen);
static void insertSharedAndPrivateDataSegments(
    struct OMRPortLibrary* portLibrary,
    char* corePath);
...
static void printProgramHeader(struct OMRPortLibrary* portLibrary,
                               ...
                               void* pelfPgmHdr,
                               uintptr_t count);
static void printProgramHeaders(struct OMRPortLibrary* portLibrary,
                                intptr_t fd,
                                uintptr_t addrSize,
                                Elf64_Off phoff,
                                Elf64_Half phnum);
      ...
      case '%':
        ...
        switch (*inCursor) {
          ...
          case 'c': {
            ...
            if (-1 == rc) {
              portLibrary->str_printf(portLibrary, derivedCoreFileName,
                                      PATH_MAX, "%s",
          ...
          case 'u':
            charsPrinted = portLibrary->str_printf(
                portLibrary, outCursor, bytesLeft, "%u",
...
static void copyDataIntoELFCore(struct OMRPortLibrary* portLibrary,
  ...
  if (32 == addrSize) {
    ...
    if (0 == p32elfPgmHdr->p_filesz) {
      ...
      if (dataOffset != -1) {
        dataSize = portLibrary->file_write(
            portLibrary, fd_core, (const void*)(uintptr_t)p32elfPgmHdr->p_vaddr,
            p32elfPgmHdr->p_memsz);
        if (dataSize > 0) {
    ...
    if (0 == p64elfPgmHdr->p_filesz) {
      ...
      if (dataOffset != -1) {
        dataSize = portLibrary->file_write(
            portLibrary, fd_core, (const void*)(uintptr_t)p64elfPgmHdr->p_vaddr,
            p64elfPgmHdr->p_memsz);
        if (dataSize > 0) {
