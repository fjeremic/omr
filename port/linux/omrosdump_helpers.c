...
static uintptr_t getSharedAndPrivateDataSegments(
    struct OMRPortLibrary* portLibrary, intptr_t fd, uintptr_t addrSize, uint8_t** ppStart, uint8_t** ppEnd);

static uintptr_t readElfHeader(struct OMRPortLibrary* portLibrary, intptr_t fd, Elf64_Ehdr* pElfheader);
static uintptr_t findProgramHeader(struct OMRPortLibrary* portLibrary, intptr_t fd, uintptr_t addrSize, Elf64_Off phoff,
    Elf64_Half phnum, uint8_t* start, uint8_t* end);
static void copyDataIntoELFCore(
    struct OMRPortLibrary* portLibrary, intptr_t fd_core, uintptr_t addrSize, Elf64_Off phoff);
static intptr_t getContentsFromProcFileSystem(
    struct OMRPortLibrary* portLibrary, const char* fileName, char* buffer, size_t buflen);
static void insertSharedAndPrivateDataSegments(struct OMRPortLibrary* portLibrary, char* corePath);
static intptr_t deriveCoreFileName(struct OMRPortLibrary* portLibrary, char* corePatterFormat, BOOLEAN coreUsesPID,
    char* baseDir, char* derivedCoreFileName, uintptr_t derivedCoreFileNameLen, pid_t pid, int signalNumber);

#ifdef DUMP_DBG
static void printMemoryMap(struct OMRPortLibrary* portLibrary, uintptr_t addrSize);
static void printProgramHeader(
    struct OMRPortLibrary* portLibrary, uintptr_t addrSize, void* pelfPgmHdr, uintptr_t count);
static void printProgramHeaders(
    struct OMRPortLibrary* portLibrary, intptr_t fd, uintptr_t addrSize, Elf64_Off phoff, Elf64_Half phnum);
#endif
...
uintptr_t renameDump(struct OMRPortLibrary* portLibrary, char* filename, pid_t pid, int signalNumber)
{
    ...
    if (0 == procRC) {
        ...
        if ('|' == corePatternFormat[0]) {
             ...
             */
            portLibrary->nls_printf(portLibrary, J9NLS_WARNING | J9NLS_STDERR, J9NLS_PORT_LINUXDUMP_PIPE_CORE,
                corePatternFileName, corePatternFormat, pid);
            strncpy(corePatternFormat, "core", PATH_MAX);
        }
    } else if (1 == procRC) {
        /* corePatternFileName does not exist (it should be there from kernel 2.5 onwards), use the default core file
         * name */
        strncpy(corePatternFormat, "core", PATH_MAX);
    } else {
        /* there was an error opening corePatternFileName */
        portLibrary->str_printf(portLibrary, filename, EsMaxPath,
            "Error opening \"%s\". Look for core file generated by child process with pid = %i", corePatternFileName,
            pid);
        return 1;
    }
    ...
    if (0 == procRC) {
        ...
        portLibrary->str_printf(portLibrary, filename, EsMaxPath,
            "Error opening \"%s\". Look for core file generated by child process with pid = %i", coreUsesPIDFileName,
            pid);
        return 1;
    }
    ...
    if (NULL != lastSep) {
        /* filename was an absolute path: strip it down to the directory name, make sure to preserve the trailing front
         * slash */
    ...
    createCoreFileNameRC = deriveCoreFileName(
        portLibrary, corePatternFormat, coreUsesPID, tempPath, derivedAbsoluteCorePath, PATH_MAX, pid, signalNumber);
    ...
    if (0 != waitCoreRC) {
        ...
        portLibrary->str_printf(portLibrary, filename, EsMaxPath,
            "The core file created by child process with pid = %i was not found. Expected to find core file with name "
            "\"%s\"",
    ...
    if (stat(derivedAbsoluteCorePath, &attrBuf) == 0) {
        if (!S_ISREG(attrBuf.st_mode)) {
            portLibrary->nls_printf(
                portLibrary, J9NLS_ERROR | J9NLS_STDERR, J9NLS_PORT_DUMP_PATH_EXISTS, derivedAbsoluteCorePath);
            return 1;
        }
    } else {
        portLibrary->str_printf(portLibrary, filename, EsMaxPath,
    ...
    if (filename[0] != '\0') {
        ...
        if ((0 != renameRC) && (errno == EXDEV)) { /* failed with 'cross device rename error' */
            ...
            portLibrary->tty_printf(portLibrary,
                "Warning: unable to move dump to \"%s\" across file systems (check kernel core_pattern). Using "
                "alternate file location \"%s\"\n",
                filename, tempPath);
 ...
 *
 * If corePatternString does not specify an absolute path and and baseDir is not empty, use the absolute path in baseDir
 * as the base directory for the core file.
 *
 * 		%%  A single % character
 ...
 *
 * 	Note: We cannot determine the exact time of the dump, as set by the OS. If we find a %t token, and %p is also
 * specified, we use a wildcard to obtain a filename match. The child process PID provides reasonable confidence that we
 * will find the correct dump. Otherwise return failure. Note: From the man pages: "A single % at the end of the
 * template is dropped from the core filename, as is the combination of a % followed by any character other than those
 * listed above."
 *
 * If we cannot reasonably infer what the fileName should be, return -1
 *
 * @param[in] corePatternFormat the null-terminated string, with no line delimiters, representing the format of the core
 * file name.
 * 									- if there was no core_pattern file, corePatternFormat should be empty
 * string.
 * @param[in] coreUsesPID		TRUE indicates that core_uses_pid indicated that the PID should be appended to the
 * filename
 * @param[in] baseDir			the null-terminated string indicating the absolute path (with trailing dir
 * separator) of the base directory to use if corePatternFormat does not specify an absolute path.
 * 									- should be empty string if no baseDirectory was
 * specified.
 *
        ...
        case '%':
            ...
            case 'c': {
                ...
                if (-1 == rc) {
                    portLibrary->str_printf(
                        portLibrary, derivedCoreFileName, PATH_MAX, "%s", "Failed to obtain core limit");
                    return -1;
                }
            ...
            case 'd': {
                ...
                if (0 > dumpableFlag) {
                    ...
                    portLibrary->str_printf(
                        portLibrary, derivedCoreFileName, PATH_MAX, "%s", "Failed to obtain dumpable flag");
                    return -1;
                }
            } break;
            case 'P':
            ...
            case 'E': {
                ...
                if (-1 == readlinkRc) {
                    portLibrary->str_printf(portLibrary, derivedCoreFileName, PATH_MAX,
                        "readlink() on \"/proc/self/exe\" failed: %s", scratchSpace, strerror(errno));
                    return -1;
                }
            ...
            case 'u':
                charsPrinted = portLibrary->str_printf(
                    portLibrary, outCursor, bytesLeft, "%u", portLibrary->sysinfo_get_euid(portLibrary));
                break;
            case 'g':
                charsPrinted = portLibrary->str_printf(
                    portLibrary, outCursor, bytesLeft, "%u", portLibrary->sysinfo_get_egid(portLibrary));
                break;
            case 's':
            ...
            case 'h':
                if (-1 != gethostname(scratchSpace, PATH_MAX)) {
                    ...
                    portLibrary->str_printf(
                        portLibrary, derivedCoreFileName, PATH_MAX, "%s", "Failed to obtain hostname");
                    return -1;
                }
            case 'e':
#if defined(LINUX)
...
#define PR_GET_NAME 16
                ...
                if (prctl(PR_GET_NAME, scratchSpace) >= 0) {
                     ...
                     */
                    scratchSpace[16]
                        = '\0'; /* ensure the name is terminated by a null if it is exactly 16 bytes long */
                    charsPrinted = portLibrary->str_printf(portLibrary, outCursor, bytesLeft, "%s", scratchSpace);
                } else
#endif
                {
    ...
    *outCursor = '\0';
    ...
    if ((0 < numWildcards) && !corePatternSpecifiesPercentP) {
        ...
        portLibrary->str_printf(portLibrary, derivedCoreFileName, PATH_MAX, "%s",
    ...
    if (!corePatternSpecifiesPercentP && coreUsesPID) {
        charsPrinted = portLibrary->str_printf(
            portLibrary, outCursor, derivedCoreFileNameLen - (outCursor - derivedCoreFileName), ".%i", pid);
        /*outCursor = outCursor + charsPrinted;*/
    }
 ...
 *PATH_MAX. the filename portion of the path may contain a single '*' wildcard.
 *
        ...
        while (bytesRead > -1) {
            ...
            if (bytesRead > 0) {
                ...
                if (ch == '\n') {
                    ...
                    /* use strtoull for correctness on 64-bit platforms and gratuitous extra precision on 32-bit
                     * platforms */
    ...
    struct OMRPortLibrary* portLibrary, intptr_t fd_core, uintptr_t addrSize, Elf64_Off phoff)
{
    ...
    if (32 == addrSize) {
        ...
        if (0 == p32elfPgmHdr->p_filesz) {
            ...
            if (dataOffset != -1) {
                dataSize = portLibrary->file_write(
                    portLibrary, fd_core, (const void*)(uintptr_t)p32elfPgmHdr->p_vaddr, p32elfPgmHdr->p_memsz);
                if (dataSize > 0) {
        ...
        if (0 == p64elfPgmHdr->p_filesz) {
            ...
            if (dataOffset != -1) {
                dataSize = portLibrary->file_write(
                    portLibrary, fd_core, (const void*)(uintptr_t)p64elfPgmHdr->p_vaddr, p64elfPgmHdr->p_memsz);
                if (dataSize > 0) {
 ...
 * @return void.
 */
void insertSharedAndPrivateDataSegments(struct OMRPortLibrary* portLibrary, char* corePath)
{
    intptr_t fd_maps = 0;
    intptr_t fd_core = 0;
    uintptr_t addrSize;
        ...
        if (fd_core != -1) {
            ...
            while (getSharedAndPrivateDataSegments(portLibrary, fd_maps, addrSize, &start, &end)) {
                ...
                if (0 == phoff_update) {
                    ...
                    if (32 == addrSize) {
                        portLibrary->tty_printf(
                            portLibrary, "could not find ELF pgmhdr to match SC mem: 0x%lx->0x%lx\n", start, end);
                    } else if (64 == addrSize) {
                        portLibrary->tty_printf(
                            portLibrary, "could not find ELF pgmhdr to match SC mem: 0x%llx->0x%llx\n", start, end);
                    }
#endif
                } else {
