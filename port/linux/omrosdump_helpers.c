...
uintptr_t renameDump(struct OMRPortLibrary* portLibrary, char* filename, pid_t pid, int signalNumber)
{
  ...
  if (0 == procRC) {
    ...
    portLibrary->str_printf(portLibrary, filename, EsMaxPath,
      "Error opening \"%s\". Look for core file generated by child process with pid = %i", corePatternFileName, pid);
    return 1;
  }
  ...
  if (0 == procRC) {
    ...
    portLibrary->str_printf(portLibrary, filename, EsMaxPath,
      "Error opening \"%s\". Look for core file generated by child process with pid = %i", coreUsesPIDFileName, pid);
    return 1;
  }
  ...
  if (stat(derivedAbsoluteCorePath, &attrBuf) == 0) {
    ...
    portLibrary->str_printf(portLibrary, filename, EsMaxPath, "Unable to read file status for core file path \"%s\"",
  ...
  if (filename[0] != '\0') {
    ...
    if ((0 != renameRC) && (errno == EXDEV)) { /* failed with 'cross device rename error' */
      ...
      portLibrary->tty_printf(portLibrary,
        "Warning: unable to move dump to \"%s\" across file systems (check kernel core_pattern). Using alternate file "
        "location \"%s\"\n",
        filename, tempPath);
 ...
 * @param[in] baseDir			the null-terminated string indicating the absolute path (with trailing dir separator) of the
 * base directory to use if corePatternFormat does not specify an absolute path.
  ...
  *outCursor = '\0';
  ...
  if (1 < numWildcards) {
    portLibrary->str_printf(
      portLibrary, derivedCoreFileName, PATH_MAX, "%s", "only one instance of \"%t\",\"%P\", or\"%I\"  is supported.");
    return -1;
  }
