...
typedef struct OMRPortPlatformGlobals {
  ...
  void* tty_consoleEventBuffer; /* windows.h cannot be included here and it has a complex definition so use void * in
  ...
  GetThreadIdealProcessorExType GetThreadIdealProcessorExProc; /**< Cached pointer to GetThreadIdealProcessorEx function
                                                                  (or NULL if not supported on host OS level) */
  SetThreadIdealProcessorExType SetThreadIdealProcessorExProc; /**< Cached pointer to SetThreadIdealProcessorEx function
                                                                  (or NULL if not supported on host OS level) */
  GetNumaNodeProcessorMaskExType
    GetNumaNodeProcessorMaskExProc; /**< Cached pointer to GetNumaNodeProcessorMaskEx function (or NULL if not supported
                                       on host OS level) */
  J9AVLTree bindingTree; /**< The tree of all tracked NUMA memory extents for all reserved memory extents - Should
                            ideally be moved into VMIdentifier and tracked on a per-extent basis */
