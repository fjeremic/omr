...
struct pgtha {
  ...
  unsigned char flag1; /* ONLY THREAD and PTAG checked if ACCESSPID==CURRENT &
                          ACCESSTHID==NEXT */
...
*/
struct pgthh {
  char id[2]; /* rd = root directory (fist), cd = current directory (second), fd
                 = file directory, vd = vnode directory */
  char type;
  char open;
  int inode;
  int devno;
...
#ifndef J9ZOS39064
void getthent(int *input_length, struct pgtha **input, int *output_length,
              void **output_buffer, int *return_value, int *return_code,
              int *reason_code);

void pthread_quiesce(int *request_type, char **user_data, int *return_value,
                     int *return_code, int *reason_code);

#endif

/* TCB structure:
 * http://publib.boulder.ibm.com/infocenter/zos/v1r9/topic/com.ibm.zos.r9.iead500/iea2d580218.htm
 */
#ifndef J9ZOS39064
struct tcb {
...
/* quiesce types from
 * http://publib.boulder.ibm.com/infocenter/zos/v1r9/topic/com.ibm.zos.r9.bpxb100/ycons.htm?resultof=%22%51%55%49%45%53%43%45%5f%46%52%45%45%5a%45%22%20
 */
