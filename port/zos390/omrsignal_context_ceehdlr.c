 ...
 */
void fillInLEJumpInfo(struct OMRPortLibrary *portLibrary, omr_31bit_mch *regs,
                      void *jumpInfo) {
  int i = 0;
...
/* infoForFPR_ceehdlr() follows the same convention as infoForFPR() in
 * port/zos390/omrsignal_context.c */
uint32_t infoForFPR_ceehdlr(struct OMRPortLibrary *portLibrary,
                            OMRLEConditionInfo *info, int32_t index,
...
/* infoForGPR_ceehdlr() follows the same convention as infoForGPR() in
 * port/zos390/omrsignal_context.c */
...
/* infoForVR_ceehdlr() follows the same convention as infoForGPR() in
 * port/zos390/omrsignal_context.c */
...
/* infoForControl_ceehdlr() follows the same convention as infoForControl() in
 * port/zos390/omrsignal_context.c Some case statements for an immediate value
 * are preceded by a fall-through case statement on a constant. This is done so
 * that it can be used both as an iterator (see documentation for omrsig_info()
 * in port/common/omrsignal.c and so that values for a specific
 * register/attribute can requested.
 */
...
/*
 * infoForModule_ceehdlr()  follows the same convention as infoForModule() in
 * port/zos390/omrsignal_context.c Some case statements for an immediate value
 * are preceded by a fall-through case statement on a constant. This is done so
 * that it can be used both as an iterator (see documentation for omrsig_info()
 * in port/common/omrsignal.c and so that values for a specific
 ...
 * triggered the condition corresponding to @ref info
 *
 */
uint32_t infoForModule_ceehdlr(struct OMRPortLibrary *portLibrary,
  ...
  *name = "";
   ...
   * We don't know what the stack format was for the routine that triggered the
   * condition, but the DSA (stack pointer) is stored in cib_sv1 for both the
   * XPLINK and OSLINK formats */
  void *dsaptr = (void *)info->cib->cib_sv1;
  _INT4 dsa_format =
      -1; /* we don't know if the interrupted routine was OSLINK/XPLINK */
  void *caaptr = (void *)
  ...
  /* call CEETBCK */
  ceetbck(&dsaptr, &dsa_format, &caaptr, &member_id, info->program_unit_name,
          &program_unit_name_length, &info->program_unit_address,
          &call_instruction_address, info->entry_name, &entry_name_length,
          &info->entry_address, &callers_call_instruction_address,
          &callers_dsaptr, &callers_dsa_format, statement_id,
  ...
  case 0:
    *name = "Program_Unit_Name";
    if (program_unit_name_length > 0) {
      ...
      *value =
          (void *)e2a_func(info->program_unit_name, program_unit_name_length);
...
/* infoForSignal_ceehdlr()  follows the same convention as infoForSignal() in
 * port/zos390/omrsignal_context.c Some case statements for an immediate value
 * are preceded by a fall-through case statement on a constant. This is done so
 * that it can be used both as an iterator (see documentation for omrsig_info()
 * in port/common/omrsignal.c and so that values for a specific
 * register/attribute can requested.
 */
uint32_t infoForSignal_ceehdlr(struct OMRPortLibrary *portLibrary,
                               OMRLEConditionInfo *info, int32_t index,
                               const char **name, void **value) {
  omr_31bit_mch *mchRegs = (omr_31bit_mch *)info->cib->cib_machine;
  *name = "";
