...
#define J9CFGOP_QUERY_CLIENT_CACHE_SIZE                                        \
  231 /* Configuration sub-command: query client cache size */
...
#define J9STATOP_USER_CACHE                                                    \
  242 /* Statistics sub-command: user data cache information */
#define J9STATOP_META_CACHE                                                    \
  248 /* Statistics sub-command: metadata cache information */
...
#define J9SA_RESET                                                             \
  0x80 /* Flags for statistics API structure - x80 means reset statistics */
...
extern void
BPX4PCT(char*, int, int, char*, int*, int*, int*);
#else /* AMODE 31-bit */
#pragma linkage(BPX1PCT, OS)
extern void
BPX1PCT(char*, int, int, char*, int*, int*, int*);
#endif
...
extern "C"
{
  ...
  typedef struct J9ZFSSyscallParamList
  {
    ...
    int32_t params[7]; /* Specific to type of operation, provides access to the
                          params */
  ...
  typedef struct J9ZFSStatApi
  {
    ...
    int32_t sa_len; /**< length of the buffer that follows this structure for
                       returning data */
    int32_t sa_ver; /**< version number (currently always 1) */
    char sa_flags;  /**< flags field (must be x00 or x80, x80 means reset
                       statistics) */
  ...
  typedef struct j9vm_stats
  {
    ...
    uint32_t
      vm_reclaim_steal; /* Number of times steal from others function invoked */
  ...
  {
    j9vm_stats
      stuc[2]; /**< Various statistics for both LOCAL and REMOTE systems*/
  ...
  typedef struct J9ZFSConfigOption
  {
    ...
    char co_string[J9CO_SLEN +
                   1]; /**< String value for option must be 0 terminated */
   ...
   * @param[out] userCacheUsedPtr Pointer to the returned used user cache area
   * size (in bytes).
   ...
   * @param[out] bufferCacheSizePtr Pointer to the returned metadata cache
