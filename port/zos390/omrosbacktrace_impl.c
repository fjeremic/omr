...
int getEntryName(uint8_t *entry_point, char **name) {
  ...
  routine_entry =
      (XPLINK_Routine_entry *)(entry_point - sizeof(XPLINK_Routine_entry));

  /* get the address of the XLINK PPA1 */
  ppa =
      (PPA1_Version3 *)((uint8_t *)routine_entry + routine_entry->ppa1_offset);
  ...
  if (ppa->version == 0x02) {
    if (ppa->flags4 & NAME_LENGTH_AND_NAME) {
      ...
      nameLenPtr = (uint16_t *)((uint8_t *)ppa + sizeof(PPA1_Version3) +
                                optional_area_length);
      nameLen = *nameLenPtr;
...
uintptr_t omrintrospect_backtrace_thread_raw(struct OMRPortLibrary *portLibrary,
  ...
  /* if we've been passed a port library wrapped signal, then extract info from
   * there */
  if (sigInfo != NULL) {
    threadInfo->context = sigInfo->platformSignalInfo.context;

    /* get the faulting address so we can discard frames that are part of the
     * signal handling */
    infoForControl(portLibrary, sigInfo, 0, &regName,
                   (void **)&faultingAddress);
  }

  /* check that CAA and DSA are available. We can't take these values from the
   * signal as we have no way of knowing what the DSA format was for the code
  ...
  if (threadInfo->dsa) {
    ...
    /* have to skip this first frame as we don't want to return a DSA entry for
     * this function as it'll be invalid once it returns
     */
    dsaptr = __dsa_prev(dsaptr, __EDCWCCWI_LOGICAL, (int)dsa_format, NULL, NULL,
                        (int *)&callers_dsa_format, NULL, NULL);
  ...
  for (frameNumber = 0; callers_dsaptr != NULL; frameNumber++) {
    ...
    /* CAA field CEECAA_STACKDIRECTION holds stack direction... is this for the
     * current frame or initial thread? */
    ceetbck(&dsaptr,                           /* in */
            &dsa_format,                       /* inout */
            &caaptr,                           /* in */
            &member_id,                        /* out */
...
#else
    errno = 0;
    callers_dsaptr = __dsa_prev(dsaptr, __EDCWCCWI_PHYSICAL, dsa_format, NULL,
                                NULL, &callers_dsa_format, NULL, NULL);
...
uintptr_t omrintrospect_backtrace_symbols_raw(
  ...
  for (i = 0, frame = threadInfo->callstack; frame != NULL;
    ...
    entry_address =
        __ep_find((void *)frame->base_pointer, frame->register2, NULL);
#endif
    ...
    if (frame->register2 == __EDCWCCWI_DOWN) {
      ...
      /* some upstack names seem to translate badly, e.g. C?????????0x0000ED28
       * [+0x8f8]) We just omit the symbol instead
       */
      entry_name[0] = 0;
    }
    ...
    /* the statement ID's only seem to be coherent for MVS modules, in USS code
     * it looks like random data */

    /* if we have symbol information then present offset into symbol instead of
     * offset into module */
    if (entry_name_length > 0) {
    ...
    if (program_unit_address != 0) {
      module_offset =
          frame->instruction_pointer - (uintptr_t)program_unit_address;
    }
