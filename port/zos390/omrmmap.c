...
void omrmmap_dont_need(struct OMRPortLibrary *portLibrary,
  ...
  uintptr_t pageSize = portLibrary->mmap_get_region_granularity(
...
#if defined(OMR_ENV_DATA64)
      if (omrdiscard_data((void *)roundedStart,
                          roundedLength >> ZOS_REAL_FRAME_SIZE_SHIFT) != 0) {
        Trc_PRT_mmap_dont_need_j9discard_data_failed((void *)roundedStart,
                                                     roundedLength);
      }
#else
      if (Pgser_Release((void *)roundedStart, roundedLength) != 0) {
        Trc_PRT_mmap_dont_need_Pgser_Release_failed((void *)roundedStart,
                                                    roundedLength);
      }
