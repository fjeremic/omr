...
static void
convertToUpper(struct OMRPortLibrary* portLibrary,
               char* toConvert,
               uintptr_t len);
static void
appendCoreName(struct OMRPortLibrary* portLibrary,
               char* corepath,
               intptr_t pathBufferLimit,
               intptr_t pid);
static uintptr_t
tdump_wrapper(struct OMRPortLibrary* portLibrary,
              char* filename,
              char* dsnName);
static intptr_t
tdump(struct OMRPortLibrary* portLibrary,
      char* asciiLabel,
      char* ebcdicLabel,
      uint32_t* returnCode,
      uint32_t* reasonCode);
static intptr_t
ceedump(struct OMRPortLibrary* portLibrary,
        char* asciiLabel,
        char* ebcdicLabel);
               ...
               intptr_t pid)
{
  ...
  pidLen = portLibrary->str_printf(
    portLibrary, pidFilter, sizeof(pidFilter), ".%d", pid);
...
#define J9_MAX_JOBNAME 16
    ...
    result = portLibrary->sysinfo_get_username(
      portLibrary, userNameStack, USERNAME_STACK_BUFFER_SIZE);
    if (-1 == result) {
      /* we tried to get the username, but failed, use ANON instead */
      portLibrary->str_printf(
        portLibrary, userNameStack, USERNAME_STACK_BUFFER_SIZE, "%s", "ANON");
    }
    ...
    if (jobname) {
      omrget_jobname(portLibrary, jobname, J9_MAX_JOBNAME);
                           ...
                           on dataset names */
    portLibrary->str_printf(
      portLibrary, filename, EsMaxPath, "%s.JVM.%s.", userNameStack, jobname);
#else /* 31 bits */
    ...
    strftime(filename + fileNamePrefixLength,
             EsMaxPath - fileNamePrefixLength,
             "D%y%m%d.T%H"
             "%M"
             "%S",
             localtime(&now));
...
#undef USERNAME_STACK_BUFFER_SIZE
  ...
  if (filenameSpecified) {
    ...
    if (strstr(filename, "&DS") == NULL) {
      ...
      portLibrary->nls_printf(portLibrary,
  ...
  if (0 == err) {
    ...
    if (0x8 == returnCode && 0x22 == reasonCode) {
      ...
      portLibrary->nls_printf(
        portLibrary, J9NLS_ERROR | J9NLS_STDERR, J9NLS_PORT_IEATDUMP_DISK_FULL);
    }
  } else {
      ...
      uint32_t* reasonCode)
{
  ...
  if (ioParms31->retcode) {
    ...
    omrtty_err_printf(
      portLibrary, "IEATDUMP success for DSN='%s'\n", asciiLabel);
  }
...
ceedump(struct OMRPortLibrary* portLibrary, char* asciiLabel, char* ebcdicLabel)
{
  ...
  if (rc) {
    omrtty_err_printf(
      portLibrary, "CEEDUMP failure for FILE='%s' RC=0x%08X\n", corepath, rc);
  } else {
    if (asciiLabel != NULL) {
