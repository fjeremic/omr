...
static intptr_t tdump(
  struct OMRPortLibrary* portLibrary, char* asciiLabel, char* ebcdicLabel, uint32_t* returnCode, uint32_t* reasonCode);
...
uintptr_t tdump_wrapper(struct OMRPortLibrary* portLibrary, char* filename, char* dsnName)
{
  uintptr_t retVal = 0;
  ...
  struct OMRPortLibrary* portLibrary, char* asciiLabel, char* ebcdicLabel, uint32_t* returnCode, uint32_t* reasonCode)
{
  ...
  if (ioParms31->retcode) {
    ...
    omrstr_printf(portLibrary, errmsg, sizeof errmsg, "JVMDMP025I IEATDUMP failed RC=0x%08X RSN=0x%08X DSN=%s",
