...
static void convertToUpper(struct OMRPortLibrary *portLibrary, char *toConvert,
                           uintptr_t len);
static void appendCoreName(struct OMRPortLibrary *portLibrary, char *corepath,
                           intptr_t pathBufferLimit, intptr_t pid);
static uintptr_t tdump_wrapper(struct OMRPortLibrary *portLibrary,
                               char *filename, char *dsnName);
static intptr_t tdump(struct OMRPortLibrary *portLibrary, char *asciiLabel,
                      char *ebcdicLabel, uint32_t *returnCode,
                      uint32_t *reasonCode);
static intptr_t ceedump(struct OMRPortLibrary *portLibrary, char *asciiLabel,
                        char *ebcdicLabel);
 ...
 * @param[in] filename Buffer for filename optionally containing the filename
 * where dump is to be output.
 ...
 * @note filename buffer length is platform dependent, assumed to be
 * EsMaxPath/MAX_PATH
 *
...
/**
 * @internal
 * Converts any lower case characters in the the passed in string to uppercase
 * characters
 *
 * @param[in,out] toConvert the string to convert
 * @param[in] len the number of characters in the string, not including any null
 * terminator
 */
static void convertToUpper(struct OMRPortLibrary *portLibrary, char *toConvert,
                           uintptr_t len) {
  ...
  /* check each character to see if it is lowercase character, and if so, slam
   * in the corresponding uppercase char */
  for (i = 0; i < len; i++) {
...
static void appendCoreName(struct OMRPortLibrary *portLibrary, char *corepath,
  ...
  pidLen = portLibrary->str_printf(portLibrary, pidFilter, sizeof(pidFilter),
                                   ".%d", pid);
...
uintptr_t tdump_wrapper(struct OMRPortLibrary *portLibrary, char *filename,
                        char *dsnName) {
  uintptr_t retVal = 0;
...
#define J9_MAX_JOBNAME 16
    ...
    result = portLibrary->sysinfo_get_username(portLibrary, userNameStack,
                                               USERNAME_STACK_BUFFER_SIZE);
    if (-1 == result) {
      /* we tried to get the username, but failed, use ANON instead */
      portLibrary->str_printf(portLibrary, userNameStack,
                              USERNAME_STACK_BUFFER_SIZE, "%s", "ANON");
    }
...
#if defined(J9ZOS39064) /* Remove 'TDUMP' to alleviate the 44 chars constraint \
                           on dataset names */
    portLibrary->str_printf(portLibrary, filename, EsMaxPath, "%s.JVM.%s.",
                            userNameStack, jobname);
#else /* 31 bits */
    portLibrary->str_printf(portLibrary, filename, EsMaxPath,
                            "%s.JVM.TDUMP.%s.", userNameStack, jobname);
#endif
...
#undef USERNAME_STACK_BUFFER_SIZE
   ...
   * zOS machines without these PTF will fail to generate TDUMP due to invalid
   * dataset name (unable to interpret &DS token).
   */
  if (filenameSpecified) {
    ...
    if (strstr(filename, "&DS") == NULL) {
      ...
      portLibrary->nls_printf(portLibrary, J9NLS_INFO | J9NLS_STDERR,
                              J9NLS_PORT_ZOS_64_APPENDING_XDS);
    }
    ...
    if (0x8 == returnCode && 0x22 == reasonCode) {
      ...
      if (filenameSpecified) {
        portLibrary->nls_printf(portLibrary, J9NLS_WARNING | J9NLS_STDERR,
                                J9NLS_PORT_IEATDUMP_NAME_TOO_LONG);
        filename[0] = '\0';
#if !defined(OMR_EBCDIC)
        /* Convert filename into EBCDIC... */
        dsnName = a2e_func(filename, strlen(filename) + 1);
 ...
 *
 * @param[in] portLibrary the port library
 * @param[in,out] asciiLabel the filename to use in ascii. Lowercase letters are
 * _not_ allowed
 * @param[in,out] ebcdicLabel the filename to use in ebcdic. Lowercase letters
 * are _not_ allowed
 ...
 *  *
 * @return 0 if IEATDUMP was actually called, non-zero if there was an error
 * that prevented us getting that far
 *
 * If we return 0, it's the callers responsibility to check returnCode to find
 * out if the TDUMP was actually successful
 *
...
static intptr_t tdump(struct OMRPortLibrary *portLibrary, char *asciiLabel,
  ...
  if (ioParms31 == NULL) {
    omrtty_err_printf(portLibrary,
                      "__malloc31 failed to allocate buffer for ioParms31\n");
    return 1;
  }
  ...
  if (dsnPattern31 == NULL) {
    omrtty_err_printf(
        portLibrary, "__malloc31 failed to allocate buffer for dsnPattern31\n");
    free(ioParms31);
    return 2;
  }
  ...
  /* Note: the actual IEATDUMP options are specified on the assembler macro call
   * in omrgenerate_ieat_dumps.s. The message below needs to be kept consistent
   * with the options set on the macro call.
   */
  omrtty_err_printf(portLibrary,
  ...
  if (ioParms31->retcode) {
    ...
    omrtty_err_printf(portLibrary,
                      "IEATDUMP failure for DSN='%s' RC=0x%08X RSN=0x%08X\n",
                      asciiLabel, ioParms31->retcode, ioParms31->rsncode);

    memset(errmsg, 0, sizeof errmsg);

    omrstr_printf(portLibrary, errmsg, sizeof errmsg,
                  "JVMDMP025I IEATDUMP failed RC=0x%08X RSN=0x%08X DSN=%s",
                  ioParms31->retcode, ioParms31->rsncode, asciiLabel);
...
static intptr_t ceedump(struct OMRPortLibrary *portLibrary, char *asciiLabel,
  ...
  /* replace call to __cdump() with call to csnap() as this does not lock up.
   * see CMVC 134311 */
   ...
   *  	If the data set is not defined, or is not usable for any reason, cdump()
   * returns a failure code of 1. This occurs even if the call to CEE3DMP is
   * successful. For more information see "Debugging C/C++ Routines" in "z/OS
