...
uintptr_t omrsl_open_shared_library(struct OMRPortLibrary *portLibrary,
    ...
    if (p) {
      ...
      pathLength =
          portLibrary->str_printf(portLibrary, mangledName, (EsMaxPath + 1),
                                  "%.*slib%s.so", dirNameLength, name, p + 1);
    } else {
      pathLength = portLibrary->str_printf(portLibrary, mangledName,
                                           (EsMaxPath + 1), "lib%s.so", name);
    }
  ...
  /* dllload() can open a handle to the executable with its name itself (unlike
   * NULL on other platforms) provided the executable is itself built as a DLL
   * (in XPLINK mode), which anyway is required for enabling symbol resolution.
   */
  handle = dllload(openName);

  if ((NULL == handle) && !openExec && decorate && (0 != dirNameLength)) {
    /* z/OS doesn't support dladdr so we can't search the dir of the portLib for
     * the dll as is the case on linux and macos. Instead, attempt to load the
     * DLL with just its name and not the full path. dllload will search the
     * filesystem for the library */
