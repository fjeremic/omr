  ...
  struct J9PortVmemIdentifier* identifier, uintptr_t mode, OMRMemCategory* category)
{
  ...
  if (NULL != ptr) {
    ...
    update_vmemIdentifier(identifier, (void*)ptr, (void*)ptr, allocSize, mode, FOUR_K, OMRPORT_VMEM_PAGE_FLAG_PAGEABLE,
      allocator, category);
...
#define MAX_2TO64G_MEMORY_IN_MB ((UDATA)62 * 1024) /* unit is MB i.e. 62*1024 MB = 62 GB */
  ...
  LP_DEBUG_PRINTF5("\t reservePagesAboveBar byteAmount=0x%zx, pageSize=x%zx, pageFlags=0x%zx, useStrictPageSize=0x%x, "
  ...
  if ((FOUR_K == pageSize) && (ZOS64_VMEM_ABOVE_BAR_GENERAL != userExtendedPrivateAreaMemoryType)) {
    ...
    if (0 != PPG_vmem_pageSize[index]) {
      ...
      LP_DEBUG_PRINTF4("\t reservePagesAboveBar invalid params: byteAmount = 0x%x, pageSize = 0x%x, pageFlags = 0x%x, "
                       "userExtendedPrivateAreaMemoryType = 0x%x\n",
  ...
  struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier, struct J9PortVmemParams* callerParams)
{
   ...
   * block.  Note that we still want to create a copy of the structure (because this function modifies the struct) if we
   * implement native alignment and remove this helper.
   */
  ...
  struct J9PortVmemParams* params, OMRMemCategory* category)
{
  ...
  if (!use2To32GArea) {
    ...
    if (((uintptr_t)params->endAddress <= FOUR_GIG_LIMIT) || (TRUE == useExecutablePages)) {
      ...
      baseAddress
        = default_pageSize_reserve_low_memory(portLibrary, params->byteAmount, identifier, params->mode, category);
  ...
  struct OMRPortLibrary* portLibrary, uintptr_t mode, uintptr_t* pageSize, uintptr_t* pageFlags)
{
  ...
  while (0 != PPG_vmem_pageSize[index]) {
    if (0 != (OMRPORT_VMEM_MEMORY_MODE_EXECUTE & mode)) {
      if ((ONE_M == PPG_vmem_pageSize[index]) && (0 != (OMRPORT_VMEM_PAGE_FLAG_PAGEABLE & PPG_vmem_pageFlags[index]))) {
        break;
      }
#if defined(OMR_ENV_DATA64)
    } else {
      if ((ONE_M == PPG_vmem_pageSize[index]) && (0 != (OMRPORT_VMEM_PAGE_FLAG_FIXED & PPG_vmem_pageFlags[index]))) {
        break;
      }
#endif
    }
