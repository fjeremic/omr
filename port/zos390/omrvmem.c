...
/* we use the 4G limit as the point where we switch from the malloc31 allocator to malloc.  We use 4G-1 here so that we
 * don't overflow on 32-bit */
/* (note that we also use this define in the legacy Trc_PRT_vmem_default_reserve* tracepoints so we can determine which
 * one of the default page size allocators we were using (0 is for low mem and this constant is for the general
 * allocator)) */
 ...
 */
static void get_ipt_ttoken(struct OMRPortLibrary* portLibrary, char* buf, uint32_t buf_len)
{
    if (NULL != buf) {
...
static void* reserve_memory_with_moservices(struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier,
...
static void* reservePages(struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier,
    struct J9PortVmemParams* params, OMRMemCategory* category);
static void* reserve4KPages(struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier,
    struct J9PortVmemParams* params, OMRMemCategory* category);
    ...
    struct OMRPortLibrary* portLibrary, void* address, uintptr_t byteAmount, struct J9PortVmemIdentifier* identifier)
{
    ...
    /* Default page Size */
    case OMRPORT_VMEM_RESERVE_USED_J9MEM_ALLOCATE_MEMORY: {
        /* calculating the base address is done in omrvmem_reserve_memory, so the free address here must reflect the
         * allocation address there */
...
static void* default_pageSize_reserve_low_memory(struct OMRPortLibrary* portLibrary, uintptr_t byteAmount,
    struct J9PortVmemIdentifier* identifier, uintptr_t mode, OMRMemCategory* category)
{
    void* ptr = NULL;
    ...
    if (OMR_ARE_ANY_BITS_SET(OMRPORT_VMEM_MEMORY_MODE_VIRTUAL, mode)) {
        /* No need to make byteAmount pagesize (=4K) aligned, as the caller omrvmem_reserve_memory_ex() already ensures
         * that */
    ...
    if (NULL != ptr) {
        ...
        update_vmemIdentifier(identifier, (void*)ptr, (void*)ptr, allocSize, mode, FOUR_K,
 ...
 */
static void* reserve1MPageableBelowBar(struct OMRPortLibrary* portLibrary, J9PortVmemIdentifier* identifier,
    uintptr_t mode, uintptr_t byteAmount, OMRMemCategory* category)
{
    void* ptr = NULL;
    ...
    LP_DEBUG_PRINTF1(
        "\t reserve1MPageableBelowBar calling omrallocate_1M_pageable_pages_below_bar(0x%x bytes)\n", byteAmount);
    ...
    if (NULL != ptr) {
        ...
        update_vmemIdentifier(
            identifier, ptr, ptr, byteAmount, mode, ONE_M, OMRPORT_VMEM_PAGE_FLAG_PAGEABLE, allocator, category);
    } else {
 ...
 * For large page sizes (> 4KB), it attempts to use page sizes in following order (provided
 *OMRPORT_VMEM_STRICT_PAGE_SIZE is not set) to satisfy the allocation request, starting with the requested page size :
 * 	- 2GB fixed pages
 *	- 1MB fixed pages
 *	- 1MB pageable pages
 *
 ...
 * @return NULL on failure, otherwise pointer to the memory that was allocated.
 */
static void* reservePagesAboveBar(struct OMRPortLibrary* portLibrary, J9PortVmemIdentifier* identifier, uintptr_t mode,
    uintptr_t byteAmount, uintptr_t pageSize, uintptr_t pageFlags, uintptr_t options, OMRMemCategory* category)
{
    /* The maximum amount of memory that can be allocated with userExtendedPrivateAreaMemoryType detected for 2To32G and
     * 2To64 */
...
#define MAX_2TO64G_MEMORY_IN_MB ((UDATA)62 * 1024) /* unit is MB i.e. 62*1024 MB = 62 GB */
    ...
    Trc_PRT_vmem_reservePagesAboveBar_Entry(
        byteAmount, pageSize, pageFlags, options, userExtendedPrivateAreaMemoryType);

    LP_DEBUG_PRINTF5("\t reservePagesAboveBar byteAmount=0x%zx, pageSize=x%zx, pageFlags=0x%zx, "
    ...
    if ((FOUR_K == pageSize) && (ZOS64_VMEM_ABOVE_BAR_GENERAL != userExtendedPrivateAreaMemoryType)) {
        ...
        LP_DEBUG_PRINTF1(
            "\t reservePagesAboveBar calling omrallocate_4K_pages_in_userExtendedPrivate_area(0x%zx)\n", numSegments);
        ...
        LP_DEBUG_PRINTF2(
            "\t omrallocate_4K_pages_in_userExtendedPrivateArea(0x%zx) returned 0x%zx\n", numSegments, ptr);
        ...
        do {
            ...
            if ((pageSize == PPG_vmem_pageSize[index]) && (pageFlags == PPG_vmem_pageFlags[index])) {
                break;
            }
        ...
        if (0 != PPG_vmem_pageSize[index]) {
            ...
            if ((NULL == ptr) && (index > 0) && (ONE_M == PPG_vmem_pageSize[index])
                ...
                LP_DEBUG_PRINTF2(
                    "\t reservePagesAboveBar calling omrallocate_1M_pageable_pages_above_bar(0x%zx, 0x%x)\n",
                    numSegments, userExtendedPrivateAreaMemoryType);
            ...
            LP_DEBUG_PRINTF4("\t reservePagesAboveBar invalid params: byteAmount = 0x%x, pageSize = 0x%x, pageFlags = "
                             "0x%x, userExtendedPrivateAreaMemoryType = 0x%x\n",
...
_end:
    if (NULL != ptr) {
        omrmem_categories_increment_counters(category, byteAmount);
        update_vmemIdentifier(
            identifier, ptr, ptr, byteAmount, mode, actualPageSize, actualPageFlags, allocator, category);
    } else {
        update_vmemIdentifier(identifier, 0, 0, 0, 0, 0, 0, 0, NULL);
    }
...
static void* default_pageSize_reserve_memory(struct OMRPortLibrary* portLibrary, uintptr_t byteAmount,
    struct J9PortVmemIdentifier* identifier, uintptr_t mode, OMRMemCategory* category)
{
    void* ptr = NULL;
    uintptr_t allocSize = 0;
    ...
    if (OMR_ARE_ANY_BITS_SET(OMRPORT_VMEM_MEMORY_MODE_VIRTUAL, mode)) {
...
#else /* if 0 */
        {
            ...
            if (0 != rc) {
                /* As per z/OS C/C++ Run-Time Library Reference, __moservices() returns EINVAL or EMVSERR in case of
                 * failure. EINVAL indicates invalid argument. More detailed information is returned by __errno2().
    ...
    struct J9PortVmemParams* params, OMRMemCategory* category)
{
    ...
    if (0 != rc) {
        ...
        omrmem_categories_increment_counters(category, allocSize);
        /* Update identifier and commit memory if required, else return reserved memory */
        update_vmemIdentifier(identifier, (void*)ptr, (void*)ptr, allocSize, params->mode, params->pageSize,
            OMRPORT_VMEM_PAGE_FLAG_PAGEABLE, OMRPORT_VMEM_RESERVE_USED_MOSERVICES, category);
...
void* omrvmem_reserve_memory_ex(
    struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier, struct J9PortVmemParams* callerParams)
{
    /*
     ...
     *  Routines using STORAGE allocate memory below 2GB bar. Such routines can be used on both 31-bit and 64-bit z/OS
     * system.s
     */

    /* create a local copy of the parameters since we may change them and we don't want to change the caller's arguments
     */
    /* The "includeAlignmentInAllocation" helper creates a copy of the struct but also, until native alignment is
     * implemented, adjusts the parameters to ensure that the requested alignment can be honoured within the allocated
     * block.  Note that we still want to create a copy of the structure (because this function modifies the struct) if
     * we implement native alignment and remove this helper.
     */
    ...
    if (use2To32GArea && (ZOS64_VMEM_ABOVE_BAR_GENERAL == PPG_userExtendedPrivateAreaMemoryType)) {
         ...
         *
         * Having done the check here, we only need to check for the request for OMRPORT_VMEM_ZOS_USE2TO32G_AREA later
         * on knowing that if they had asked for it and made it that far that the support is there
         */
...
#if defined(OMR_ENV_DATA64)
        if (((UDATA)params.startAddress >= TWO_G) && isRmode64Supported() && !use2To32GArea
    ...
    struct J9PortVmemParams* params, OMRMemCategory* category)
{
    ...
    BOOLEAN useExecutablePages
        = (OMRPORT_VMEM_MEMORY_MODE_EXECUTE == (OMRPORT_VMEM_MEMORY_MODE_EXECUTE & params->mode));
    ...
    if (FALSE == isLargePageSizeSupported(portLibrary, params->pageSize, params->pageFlags)) {
        ...
        if (use2To32GArea || (useStrictAddress && ((UDATA)params->startAddress + params->byteAmount >= TWO_G))) {
        ...
        if ((ONE_M != params->pageSize) || (0 == (OMRPORT_VMEM_PAGE_FLAG_PAGEABLE & params->pageFlags))) {
            update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, 0, NULL);
            Trc_PRT_vmem_omrvmem_reserve_memory_unsupported_page_size_executable_largepages(
                params->pageSize, params->pageFlags);
        } else {
            baseAddress
                = reserve1MPageableBelowBar(portLibrary, identifier, params->mode, params->byteAmount, category);
        }
#if defined(OMR_ENV_DATA64)
    } else if ((params->startAddress >= 0) && ((uintptr_t)params->endAddress <= FOUR_GIG_LIMIT)) {
        ...
        if (TRUE == use2To32GArea) {
            baseAddress = reservePagesAboveBar(portLibrary, identifier, params->mode, params->byteAmount,
                params->pageSize, params->pageFlags, params->options, category);
            if ((NULL != baseAddress) && (TRUE == isStrictAndOutOfRange(params, baseAddress))) {
        ...
        baseAddress = reservePagesAboveBar(portLibrary, identifier, params->mode, params->byteAmount, params->pageSize,
            params->pageFlags, params->options, category);
#else /* OMR_ENV_DATA64 */
    } else {
    ...
    if (NULL == baseAddress) {
        if (TRUE == useStrictPageSize) {
            ...
            LP_DEBUG_PRINTF1(
                "\t reservePages returned: %i, failed to allocate memory using large pages\n", baseAddress);
    ...
    struct J9PortVmemParams* params, OMRMemCategory* category)
{
    ...
    if (!use2To32GArea) {
        ...
        if (((uintptr_t)params->endAddress <= FOUR_GIG_LIMIT) || (TRUE == useExecutablePages)) {
            /* Allocate executable memory below the 2G bit bar as ZOS does not properly support code loaded above this
             * mark */
            /* Or if the desired address is in low memory, use the low 2G in the hopes that that's close to what's
             * wanted */
            ...
            if ((NULL == baseAddress) && (FALSE == useStrictAddress) && (FALSE == useExecutablePages)) {
            ...
            baseAddress
                = default_pageSize_reserve_memory(portLibrary, params->byteAmount, identifier, params->mode, category);
        }
#if defined(OMR_ENV_DATA64)
    } else {
        /* use the 2to32GArea */
        baseAddress = reservePagesAboveBar(portLibrary, identifier, params->mode, params->byteAmount, FOUR_K,
...
/**
 * On 31-bit system, default page size when mode is OMRPORT_VMEM_MEMORY_MODE_EXECUTE is 1M pageable pages (provided it
 * is supported), else there is no default page size. On 64-bit system, default page size when mode is
 * OMRPORT_VMEM_MEMORY_MODE_EXECUTE is 1M pageable pages (provided it is supported), else it is 1M fixed pages.
 */
    ...
    uintptr_t* pageFlags, BOOLEAN* isSizeSupported)
{
    ...
    if (FALSE == useExecutablePages) {
        ...
        if ((0 != defaultLargePageSize) && (defaultLargePageFlags == validPageFlags)) {
 ...
 */
static BOOLEAN isStrictAndOutOfRange(struct J9PortVmemParams* params, void* memoryPointer)
{
