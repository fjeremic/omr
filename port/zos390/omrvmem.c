 ...
 */
static void get_ipt_ttoken(struct OMRPortLibrary *portLibrary, char *buf,
                           uint32_t buf_len) {
  if (NULL != buf) {
...
void *omrallocate_1M_fixed_pages(int numMBSegments,
...
void *omrallocate_4K_pages_in_userExtendedPrivateArea(
    int numMBSegments, int userExtendedPrivateAreaMemoryType, const char *ttkn);

/* omrvmem_support_above_bar.s */
#pragma linkage(omrallocate_1M_pageable_pages_above_bar, OS_NOSTACK)
void *omrallocate_1M_pageable_pages_above_bar(
    int numMBSegments, int userExtendedPrivateAreaMemoryType, const char *ttkn);

/* omrvmem_support_above_bar.s */
#pragma linkage(omrallocate_2G_pages, OS_NOSTACK)
void *omrallocate_2G_pages(int num2GBUnits,
                           int userExtendedPrivateAreaMemoryType,
                           const char *ttkn);
...
static void *reserve_memory_with_moservices(
    struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
    struct J9PortVmemParams *params, OMRMemCategory *category);
...
static void *reservePages(struct OMRPortLibrary *portLibrary,
                          struct J9PortVmemIdentifier *identifier,
                          struct J9PortVmemParams *params,
                          OMRMemCategory *category);
static void *reserve4KPages(struct OMRPortLibrary *portLibrary,
                            struct J9PortVmemIdentifier *identifier,
                            struct J9PortVmemParams *params,
                            OMRMemCategory *category);
static struct J9PortVmemParams includeAlignmentInAllocation(
    struct J9PortVmemParams *incomingParams);
static void *adjustForRequestedAlignment(struct J9PortVmemParams *params,
                                         void *memoryPointer);
static void getPageInfo(struct OMRPortLibrary *portLibrary);
static BOOLEAN isLargePageSizeSupported(struct OMRPortLibrary *portLibrary,
                                        uintptr_t pageSize,
                                        uintptr_t pageFlags);
  ...
  if (1 == portLibrary->portGlobals->vmemAdviseOSonFree) {
    if (rangeIsValid(identifier, address, byteAmount)) {
      ...
      if (byteAmount > 0) {
        ...
        if (0 != result) {
          Trc_PRT_vmem_omrvmem_decommit_memory_failure(result, address,
                                                       byteAmount);
        }
      } else {
...
int32_t omrvmem_free_memory(struct OMRPortLibrary *portLibrary, void *address,
   ...
   */
  printf(
      "omrvmem_free_memory freeing %p, pageSize 0x%x pageFlags %u using "
      "allocate %zu\n",
...
static void *default_pageSize_reserve_low_memory(
  ...
  if (NULL != ptr) {
     ...
     * memory */
    update_vmemIdentifier(identifier, (void *)ptr, (void *)ptr, allocSize, mode,
                          FOUR_K, OMRPORT_VMEM_PAGE_FLAG_PAGEABLE, allocator,
                          category);
    if (0 != (OMRPORT_VMEM_MEMORY_MODE_COMMIT & mode)) {
      ptr = omrvmem_commit_memory(portLibrary, (void *)ptr, byteAmount,
                                  identifier);
    }
  } else {
 ...
 */
static void *reserve1MPageableBelowBar(struct OMRPortLibrary *portLibrary,
                                       J9PortVmemIdentifier *identifier,
                                       uintptr_t mode, uintptr_t byteAmount,
                                       OMRMemCategory *category) {
  void *ptr = NULL;
  ...
  LP_DEBUG_PRINTF2(
      ...
      byteAmount);
  LP_DEBUG_PRINTF1(
      "\t reserve1MPageableBelowBar calling "
  ...
  if (NULL != ptr) {
    ...
    update_vmemIdentifier(identifier, ptr, ptr, byteAmount, mode, ONE_M,
                          OMRPORT_VMEM_PAGE_FLAG_PAGEABLE, allocator, category);
  } else {
 ...
 * @return NULL on failure, otherwise pointer to the memory that was allocated.
 */
static void *reservePagesAboveBar(struct OMRPortLibrary *portLibrary,
                                  J9PortVmemIdentifier *identifier,
                                  uintptr_t mode, uintptr_t byteAmount,
                                  uintptr_t pageSize, uintptr_t pageFlags,
                                  uintptr_t options, OMRMemCategory *category) {
...
#define MAX_2TO64G_MEMORY_IN_MB \
  ...
  Trc_PRT_vmem_reservePagesAboveBar_Entry(byteAmount, pageSize, pageFlags,
                                          options,
                                          userExtendedPrivateAreaMemoryType);

  LP_DEBUG_PRINTF5(
      "\t reservePagesAboveBar byteAmount=0x%zx, pageSize=x%zx, "
      "pageFlags=0x%zx, useStrictPageSize=0x%x, "
      "userExtendedPrivateAreaMemoryType=0x%x\n",
      byteAmount, pageSize, pageFlags, useStrictPageSize,
      userExtendedPrivateAreaMemoryType);
  ...
  if ((ZOS64_VMEM_ABOVE_BAR_GENERAL != userExtendedPrivateAreaMemoryType) &&
    ...
    if (TWO_G == pageSize) {
      LP_DEBUG_PRINTF2(
          "\t *** ERROR reservePagesAboveBar max 1GB units is 0x%zx, "
          "requesting (0x%zx)\n",
          (userExtendedPrivateAreaMemoryMax / 1024), numUnits);
    } else {
      LP_DEBUG_PRINTF2(
          "\t *** ERROR reservePagesAboveBar max 1MB segments is 0x%zx, "
          "requesting (0x%zx)\n",
  ...
  if ((FOUR_K == pageSize) &&
    ...
    if (0 != PPG_vmem_pageSize[index]) {
      if ((index > 0) && (TWO_G == PPG_vmem_pageSize[index]) &&
        ...
        LP_DEBUG_PRINTF2(
            "\t reservePagesAboveBar calling omrallocate_2G_pages(0x%zx, "
            "0x%x)\n",
            numUnits, userExtendedPrivateAreaMemoryType);
        Trc_PRT_vmem_reservePagesAboveBar_allocate_large_2G_pages(
            numUnits, userExtendedPrivateAreaMemoryType);
        ptr = omrallocate_2G_pages(numUnits, userExtendedPrivateAreaMemoryType,
                                   ttkn);

        LP_DEBUG_PRINTF3(
            "\t omrallocate_2G_pages(0x%zx, 0x%x) returned 0x%zx\n", numUnits,
            userExtendedPrivateAreaMemoryType, ptr);
        if (NULL == ptr) {
      ...
      if ((NULL == ptr) && (index > 0) && (ONE_M == PPG_vmem_pageSize[index]) &&
        ...
        LP_DEBUG_PRINTF2(
            "\t reservePagesAboveBar calling omrallocate_1M_fixed_pages(0x%zx, "
            "0x%x)\n",
      ...
      if ((NULL == ptr) && (index > 0) && (ONE_M == PPG_vmem_pageSize[index]) &&
        ...
        LP_DEBUG_PRINTF3(
            "\t omrallocate_1M_pageable_pages_above_bar(0x%zx, 0x%x) returned "
            "0x%zx\n",
            numSegments, userExtendedPrivateAreaMemoryType, ptr);
        if (NULL == ptr) {
          if (TRUE == useStrictPageSize) {
      ...
      LP_DEBUG_PRINTF4(
          ...
          "= 0x%x, pageFlags = 0x%x, userExtendedPrivateAreaMemoryType = "
          "0x%x\n",
...
_end:
  if (NULL != ptr) {
    omrmem_categories_increment_counters(category, byteAmount);
    update_vmemIdentifier(identifier, ptr, ptr, byteAmount, mode,
                          actualPageSize, actualPageFlags, allocator, category);
  } else {
    update_vmemIdentifier(identifier, 0, 0, 0, 0, 0, 0, 0, NULL);
  }
...
static void *reserve_memory_with_moservices(
  ...
  if (0 != rc) {
    ...
    update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0,
                          OMRPORT_VMEM_RESERVE_USED_MOSERVICES, NULL);
  } else {
    omrmem_categories_increment_counters(category, allocSize);
    /* Update identifier and commit memory if required, else return reserved
     * memory */
    update_vmemIdentifier(identifier, (void *)ptr, (void *)ptr, allocSize,
                          params->mode, params->pageSize,
                          OMRPORT_VMEM_PAGE_FLAG_PAGEABLE,
                          OMRPORT_VMEM_RESERVE_USED_MOSERVICES, category);
    if (OMR_ARE_ANY_BITS_SET(params->mode, OMRPORT_VMEM_MEMORY_MODE_COMMIT)) {
      ptr = omrvmem_commit_memory(portLibrary, (void *)ptr, allocSize,
                                  identifier);
    }
  }
...
void *omrvmem_reserve_memory_ex(struct OMRPortLibrary *portLibrary,
  ...
  LP_DEBUG_PRINTF4(
      "\n omrvmem_reserve_memory_ex: address=0x%zx, byteAmount=0x%zx, "
      "pageSize=0x%zx, pageFlags=0x%zx\n",
  ...
  if (use2To32GArea &&
    ...
    LP_DEBUG_PRINTF(
        "\n omrvmem_reserve_memory_ex: OMRPORT_VMEM_ZOS_USE2TO32G_AREA "
  ...
  if (0) {
    ...
    if (((UDATA)params.startAddress >= TWO_G) && isRmode64Supported() &&
      ...
      baseAddress = reserve_memory_with_moservices(portLibrary, identifier,
                                                   &params, category);
    } else
#endif /* OMR_ENV_DATA64 */
    ...
    if ((NULL != baseAddress) && isStrictAndOutOfRange(&params, baseAddress)) {
       ...
       * fail */
      omrvmem_free_memory(portLibrary, baseAddress, params.byteAmount,
                          identifier);
...
static void *reservePages(struct OMRPortLibrary *portLibrary,
  ...
  if (FALSE == isLargePageSizeSupported(portLibrary, params->pageSize,
                                        params->pageFlags)) {
    update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, 0, NULL);
    ...
    if (TRUE == use2To32GArea) {
      baseAddress = reservePagesAboveBar(
          portLibrary, identifier, params->mode, params->byteAmount,
          params->pageSize, params->pageFlags, params->options, category);
      if ((NULL != baseAddress) &&
      ...
      if ((ONE_M == params->pageSize) &&
          (0 != (OMRPORT_VMEM_PAGE_FLAG_PAGEABLE & params->pageFlags))) {
        baseAddress =
            reserve1MPageableBelowBar(portLibrary, identifier, params->mode,
                                      params->byteAmount, category);
      }
  ...
  if (NULL == baseAddress) {
    if (TRUE == useStrictPageSize) {
      ...
      LP_DEBUG_PRINTF1(
          "\t reservePages returned: %i, failed to allocate memory using large "
          "pages\n",
          baseAddress);
  ...
  if (!use2To32GArea) {
    ...
    if (((uintptr_t)params->endAddress <= FOUR_GIG_LIMIT) ||
      ...
      if ((NULL == baseAddress) && (FALSE == useStrictAddress) &&
        ...
        baseAddress =
            default_pageSize_reserve_memory(portLibrary, params->byteAmount,
                                            identifier, params->mode, category);
      }
    } else {
    ...
    /* use the 2to32GArea */
    baseAddress = reservePagesAboveBar(
        portLibrary, identifier, params->mode, params->byteAmount, FOUR_K,
        OMRPORT_VMEM_PAGE_FLAG_PAGEABLE, params->options, category);
  }
#endif /* OMR_ENV_DATA64 */
...
intptr_t omrvmem_find_valid_page_size(struct OMRPortLibrary *portLibrary,
   ...
   * pageable pages */
  pageSizeFound = isLargePageSizeSupported(portLibrary, ONE_M,
                                           OMRPORT_VMEM_PAGE_FLAG_PAGEABLE);
  if (TRUE == pageSizeFound) {
    validPageSize = ONE_M;
 ...
 */
static BOOLEAN isStrictAndOutOfRange(struct J9PortVmemParams *params,
                                     void *memoryPointer) {
