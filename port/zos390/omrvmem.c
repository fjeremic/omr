...
default_pageSize_reserve_low_memory(struct OMRPortLibrary *portLibrary,
                                    uintptr_t byteAmount,
                                    struct J9PortVmemIdentifier *identifier,
                                    uintptr_t mode, OMRMemCategory *category) {
  void *ptr = NULL;
...
#define MAX_2TO64G_MEMORY_IN_MB                                                \
  ...
  LP_DEBUG_PRINTF5("\t reservePagesAboveBar byteAmount=0x%zx, pageSize=x%zx, "
    ...
    if (0 != PPG_vmem_pageSize[index]) {
      if ((index > 0) && (TWO_G == PPG_vmem_pageSize[index]) &&
        ...
        LP_DEBUG_PRINTF2("\t reservePagesAboveBar calling "
                         "omrallocate_2G_pages(0x%zx, 0x%x)\n",
      ...
      LP_DEBUG_PRINTF4("\t reservePagesAboveBar invalid params: byteAmount = "
                       "0x%x, pageSize = 0x%x, pageFlags = 0x%x, "
                       "userExtendedPrivateAreaMemoryType = 0x%x\n",
                       byteAmount, pageSize, pageFlags,
                       userExtendedPrivateAreaMemoryType);
    }
  }
