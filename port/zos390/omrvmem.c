...
#define MAX_2TO64G_MEMORY_IN_MB ((UDATA)62 * 1024) /* unit is MB i.e. 62*1024 MB = 62 GB */
    ...
    if ((FOUR_K == pageSize) && (ZOS64_VMEM_ABOVE_BAR_GENERAL != userExtendedPrivateAreaMemoryType)) {
        ...
        if (0 != PPG_vmem_pageSize[index]) {
            if ((index > 0) && (TWO_G == PPG_vmem_pageSize[index]) && (0 != (OMRPORT_VMEM_PAGE_FLAG_FIXED & PPG_vmem_pageFlags[index]))) {
                const char* const ttkn = PPG_ipt_ttoken;
            ...
            if ((NULL == ptr) && (index > 0) && (ONE_M == PPG_vmem_pageSize[index]) && (0 != (OMRPORT_VMEM_PAGE_FLAG_FIXED & PPG_vmem_pageFlags[index]))) {
                const char* const ttkn = PPG_ipt_ttoken;
            ...
            if ((NULL == ptr) && (index > 0) && (ONE_M == PPG_vmem_pageSize[index]) && (0 != (OMRPORT_VMEM_PAGE_FLAG_PAGEABLE & PPG_vmem_pageFlags[index]))) {
                const char* const ttkn = PPG_ipt_ttoken;
...
void* omrvmem_reserve_memory_ex(struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier, struct J9PortVmemParams* callerParams)
{
    /*
...
{
    if ((0 != (OMRPORT_VMEM_STRICT_ADDRESS & params->options)) && ((memoryPointer > params->endAddress) || (memoryPointer < params->startAddress))) {
        return TRUE;
    } else {
        return FALSE;
    }
