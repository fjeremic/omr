...
/* Returns TRUE if the configuration supports resuming execution from a signal
 * handler, FALSE otherwise
 */
BOOLEAN
checkIfResumableTrapsSupported(struct OMRPortLibrary *portLibrary) {
  /* Note that omrceeocb.h was introduced solely to provide support for this
   * function */
  ...
  trap_opts = (struct ceeocb_trap_sub_options
                   *)((char *)ceeocb() +
...
void fillInUnixSignalInfo(struct OMRPortLibrary *portLibrary, void *contextInfo,
  ...
  /* __mcontext_t_ is defined in port/zos390/edcwccwi.h and appears to overlay
   * the ucontext_t structure */
  signalInfo->platformSignalInfo.context = (__mcontext_t_ *)contextInfo;
  /* module info is filled on demand */
}

void fillInJumpInfo(struct OMRPortLibrary *portLibrary, void *contextInfo,
                    void *jumpInfo) {
  int i = 0;
...
uint32_t infoForSignal(struct OMRPortLibrary *portLibrary,
                       OMRUnixSignalInfo *info, int32_t index,
...
uint32_t infoForFPR(struct OMRPortLibrary *portLibrary, OMRUnixSignalInfo *info,
...
uint32_t infoForGPR(struct OMRPortLibrary *portLibrary, OMRUnixSignalInfo *info,
                    int32_t index, const char **name, void **value) {
...
uint32_t infoForVR(struct OMRPortLibrary *portLibrary, OMRUnixSignalInfo *info,
                   int32_t index, const char **name, void **value) {
  ...
  switch (index) {
    ...
    case 2:
      *name = "psw1";
      if (info->platformSignalInfo.context->__mc_psw_flag) {
        *value =
            &(info->platformSignalInfo.context->__mc_psw[sizeof(uintptr_t)]);
        return OMRPORT_SIG_VALUE_ADDRESS;
      } else {
        return OMRPORT_SIG_VALUE_UNDEFINED;
      }
    ...
    case 4:
#if defined(OMR_ENV_DATA64)
      /* 64-bit: request the condition information block to access the machine
       * context for BEA */
      conditionInfoBlock = __le_cib_get();

      if (NULL != conditionInfoBlock) {
        mchRegs = (__mch_t *)conditionInfoBlock->cib_machine;
        info->platformSignalInfo.breakingEventAddr =
            *(uintptr_t *)(&mchRegs->__mch_bea);
#else
      /* 31-bit: request the condition information block to access the machine
       * context for BEA */
     ...
     * hide it from the iterator by only casing it with
     * OMRPORT_SIG_CONTROL_S390_GPR7 It is available to the iterator in the GPR
     * category anyways.
     */
...
/*
 * NOTE: because we use _gtca() to get the CAA (common anchor area, all the
 * thread info dangles off the caa), this routine must run on the thread that
 * triggered the signal corresponding to @ref info
 *
 */
uint32_t infoForModule(struct OMRPortLibrary *portLibrary,
  ...
  *name = "";
  ...
  void *caaptr = (void *)
      _gtca(); /* see note about _gtca() use in function documentation */
  _INT4 dsa_format =
      -1; /* we don't know if the interrupted routine was OSLINK or XPLINK */
  void *dsaptr;

  /* We don't know if the failing routine was XPLINK or OSLINK so we don't know
   * if we should look at r4 (XPLINK) or r13 (OSLINK) for the dsa (stack
   * pointer). Fortunately, in both XPLINK/OSLINK cases, the dsa is stored in
   * __mc_int_dsa, but only if __mc_int_dsa_flag is set */
  if (0 == info->platformSignalInfo.context->__mc_int_dsa_flag) {
    *name = "";
    return OMRPORT_SIG_VALUE_UNDEFINED;
  }
  ...
  /* call CEETBCK */
  ceetbck(&dsaptr, &dsa_format, &caaptr, &member_id,
          info->platformSignalInfo.program_unit_name, &program_unit_name_length,
          &info->platformSignalInfo.program_unit_address,
          &call_instruction_address, info->platformSignalInfo.entry_name,
          &entry_name_length, &info->platformSignalInfo.entry_address,
          &callers_call_instruction_address, &callers_dsaptr,
          &callers_dsa_format, statement_id, &statement_id_length, &cibptr,
          &main_program, &fc);
    ...
    case 0:
      *name = "Program_Unit_Name";
      if (program_unit_name_length > 0) {
        ...
        info->platformSignalInfo.program_unit_name[program_unit_name_length] =
            0;
        *value = (void *)e2a_func(info->platformSignalInfo.program_unit_name,
    ...
    case 2:
      *name = "Entry_Name";
      if (entry_name_length > 0) {
        ...
        *value = (void *)e2a_func(info->platformSignalInfo.entry_name,
                                  entry_name_length);
        return OMRPORT_SIG_VALUE_STRING;
      }
      return OMRPORT_SIG_VALUE_UNDEFINED;
