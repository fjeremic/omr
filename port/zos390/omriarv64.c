 ...
 *
 * omrvmem_support_above_bar.s indicates where to put the contents of
 * omriarv64.s. Search for: Insert contents of omriarv64.s below
 *
 * *******
 ...
 * @params[in] userExtendedPrivateAreaMemoryType capability of OS: 0 - general,
 * 1 - 2G-32G range, 2 - 2G-64G range
 *
...
void *omrallocate_1M_fixed_pages(int *numMBSegments,
  ...
  switch (useMemoryType) {
    case ZOS64_VMEM_ABOVE_BAR_GENERAL:
      __asm(
          " IARV64 "
          "REQUEST=GETSTOR,COND=YES,SADMP=NO,CONTROL=UNAUTH,PAGEFRAMESIZE=1MEG,"
          "SEGMENTS=(%2),ORIGIN=(%1),TTOKEN=(%4),RETCODE=%0,MF=(E,(%3))" ::"m"(
              iarv64_rc),
          "r"(&origin), "r"(&segments), "r"(&wgetstor), "r"(ttkn));
      break;
    case ZOS64_VMEM_2_TO_32G:
      __asm(
          " IARV64 REQUEST=GETSTOR,COND=YES,SADMP=NO,USE2GTO32G=YES,"
    ...
    case ZOS64_VMEM_2_TO_64G:
      __asm(
          " IARV64 REQUEST=GETSTOR,COND=YES,SADMP=NO,USE2GTO64G=YES,"
          "CONTROL=UNAUTH,PAGEFRAMESIZE=1MEG,"
          "SEGMENTS=(%2),ORIGIN=(%1),TTOKEN=(%4),RETCODE=%0,MF=(E,(%3))" ::"m"(
              iarv64_rc),
          "r"(&origin), "r"(&segments), "r"(&wgetstor), "r"(ttkn));
...
void *omrallocate_1M_pageable_pages_above_bar(
    int *numMBSegments, int *userExtendedPrivateAreaMemoryType,
    const char *ttkn) {
  long segments;
  long origin;
  long useMemoryType = *userExtendedPrivateAreaMemoryType;
  ...
  switch (useMemoryType) {
    case ZOS64_VMEM_ABOVE_BAR_GENERAL:
      __asm(
          " IARV64 REQUEST=GETSTOR,COND=YES,SADMP=NO,CONTROL=UNAUTH,"
          "PAGEFRAMESIZE=PAGEABLE1MEG,TYPE=PAGEABLE,SEGMENTS=(%2),"
          "ORIGIN=(%1),TTOKEN=(%4),RETCODE=%0,MF=(E,(%3))" ::"m"(iarv64_rc),
          "r"(&origin), "r"(&segments), "r"(&wgetstor), "r"(ttkn));
      break;
    case ZOS64_VMEM_2_TO_32G:
      __asm(
          " IARV64 "
          "REQUEST=GETSTOR,COND=YES,SADMP=NO,CONTROL=UNAUTH,USE2GTO32G=YES,"
          "PAGEFRAMESIZE=PAGEABLE1MEG,TYPE=PAGEABLE,SEGMENTS=(%2),"
          "ORIGIN=(%1),TTOKEN=(%4),RETCODE=%0,MF=(E,(%3))" ::"m"(iarv64_rc),
          "r"(&origin), "r"(&segments), "r"(&wgetstor), "r"(ttkn));
      break;
    case ZOS64_VMEM_2_TO_64G:
      __asm(
          " IARV64 "
          "REQUEST=GETSTOR,COND=YES,SADMP=NO,CONTROL=UNAUTH,USE2GTO64G=YES,"
          "PAGEFRAMESIZE=PAGEABLE1MEG,TYPE=PAGEABLE,SEGMENTS=(%2),"
          "ORIGIN=(%1),TTOKEN=(%4),RETCODE=%0,MF=(E,(%3))" ::"m"(iarv64_rc),
          "r"(&origin), "r"(&segments), "r"(&wgetstor), "r"(ttkn));
      break;
  ...
  switch (useMemoryType) {
    case ZOS64_VMEM_ABOVE_BAR_GENERAL:
      __asm(
          ...
          "ORIGIN=(%1),TTOKEN=(%4),RETCODE=%0,MF=(E,(%3))" ::"m"(iarv64_rc),
          "r"(&origin), "r"(&units), "r"(&wgetstor), "r"(ttkn));
      break;
    case ZOS64_VMEM_2_TO_32G:
      __asm(
          " IARV64 "
...
void *omrallocate_4K_pages_above_bar(int *numMBSegments, const char *ttkn) {
  ...
  __asm(
      " IARV64 REQUEST=GETSTOR,COND=YES,SADMP=NO,"
      "CONTROL=UNAUTH,PAGEFRAMESIZE=4K,"
      "SEGMENTS=(%2),ORIGIN=(%1),TTOKEN=(%4),RETCODE=%0,MF=(E,(%3))" ::"m"(
          iarv64_rc),
      "r"(&origin), "r"(&segments), "r"(&wgetstor), "r"(ttkn));
...
int omrfree_memory_above_bar(void *address, const char *ttkn) {
  ...
  __asm(
      " IARV64 "
      "REQUEST=DETACH,COND=YES,MEMOBJSTART=(%2),TTOKEN=(%3),RETCODE=%0,MF=(E,(%"
      "1))" ::"m"(iarv64_rc),
      "r"(&wgetstor), "r"(&xmemobjstart), "r"(ttkn));
  return iarv64_rc;
}
...
int omrdiscard_data(void *address, int *numFrames) {
  ...
  __asm(
      " IARV64 REQUEST=DISCARDDATA,KEEPREAL=NO,"
      "RANGLIST=(%1),RETCODE=%0,MF=(E,(%2))" ::"m"(iarv64_rc),
      "r"(&rangePtr), "r"(&wgetstor));
