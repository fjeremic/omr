...
#pragma convlit(resume)
  ...
  if (OMR_ARE_ANY_BITS_SET(thisRecord->flags, portlibSignalNo) ||
    ...
    case OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH:
      /* thisRecord->handler was not adequately able to address the signal, try
       * the previously registered handler
       * To do this, percolate the signal such as to invoke the previously
       * registered LE condition handler */
    ...
    case OMRPORT_SIG_EXCEPTION_RETURN:
      /* jump back to where this was registered in @ref omrsig_protect_ceehdlr,
       * so that we can return back to it's caller  */
