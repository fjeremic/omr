...
void
omrsig_le_condition_handler(_FEEDBACK*, _INT4*, _INT4*, _FEEDBACK*);
static uint32_t
 ...
 */
void
omrsig_le_condition_handler(_FEEDBACK* fc,
                            _INT4* token,
                            _INT4* leResult,
                            _FEEDBACK* newfc)
{
...
#pragma convlit(resume)
  ...
  if (OMR_ARE_ANY_BITS_SET(thisRecord->flags, portlibSignalNo) ||
    ...
    handlerResult = thisRecord->handler(thisRecord->portLibrary,
                                        portlibSignalNo,
                                        &signalInfo,
    ...
    switch (handlerResult) {
      ...
      case OMRPORT_SIG_EXCEPTION_CONTINUE_EXECUTION: {
        ...
        fillInLEJumpInfo(thisRecord->portLibrary,
                         cib_ptr->cib_machine,
...
int32_t
omrsig_protect_ceehdlr(struct OMRPortLibrary* portLibrary,
...
uint32_t
omrsig_info_ceehdlr(struct OMRPortLibrary* portLibrary,
                    void* info,
                    uint32_t category,
                    int32_t index,
                    const char** name,
...
{
  omrthread_tls_set(
    omrthread_self(), tlsKeyCurrentSignal, (intptr_t*)portLibSigType);
}
