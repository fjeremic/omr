...
/*
 * omrsignal_ceehdlr provides an implementation of signal support that uses LE
 *condition handling it is only to be used on 31-bit z/OS
 **/
...
static void setCurrentSignal(struct OMRPortLibrary *portLibrary,
                             uint32_t portLibSigType);

/*
 *
 * Much of the following omrsig_le_condition_handler parameter documentation is
 *from:
 *
 ...
 *the current condition being processed. Language Environment uses this
 *parameter to tell your condition handler what condition has occurred.
 *	@param[in]	token 	An OMRZOSLEConditionHandlerRecord as specified
 *when this handler was registered by @ref omrsig_protect_ceehdlr
 *	@param[in]	leResult	A 4-byte integer that contains
 *instructions about responses the user-written condition handler wants Language
 *Environment to make when processing the condition. The result_code is passed
 *by reference. Valid responses are shown in Table 38 of the Language
 *Environment Programming Guide For example, use 20 to percolate the condition.
 *	@param[in]	newfc	new_condition (output) A 12-byte condition token
 *that represents either the promoted condition for a promote response
 *(result_code values of 30, 31, and 32) or the requested fix-up actions for a
 *fix-up and resume response (result_code value of 60). When a result_code of
 *60, denoting fix-up and resume, is set by the condition handler, new_condition
 *must be set to a condition token that indicates what fix-up action is
 *requested. Many conditions, including mathematical routines, use the condition
 *tokens in Table 39 to resume with corrective action (either resume with new
 *input value or resume with new output value). For some conditions, there may
 *be other condition tokens that can be provided by the condition handler in
 *new_condition to request specific fix-up actions.
 *
 *
 * The Language environment documentation for all the condition messages are
 *here http://publibz.boulder.ibm.com/epubs/pdf/ceea91a0.pdf, but the POSIX
 *signal mappings are not included for hardware raised signals.
 *
 * The messages for hardware raised POSIX signals are in Table 74, pg. 421 here
 *http://publibz.boulder.ibm.com/epubs/pdf/cbcpg1a0.pdf The messages for
 *software raised POSIX signals are in Table 76, pg. 423 here
 *http://publibz.boulder.ibm.com/epubs/pdf/cbcpg1a0.pdf
 *
 */
void omrsig_le_condition_handler(_FEEDBACK *fc, _INT4 *token, _INT4 *leResult,
                                 _FEEDBACK *newfc) {
  struct OMRZOSLEConditionHandlerRecord *thisRecord =
      (struct OMRZOSLEConditionHandlerRecord *)*token;
...
#pragma convlit(resume)
  ...
  printf("omrsig_le_condition_handler count: %i, %s%i, sev: %i\n", count,
         e2a_func(fc->tok_facid, 3), fc->tok_msgno, fc->tok_sev);
  fflush(NULL);
  count++;
#endif

  if (1 == thisRecord->recursiveCheck) {
    /* This handler has been invoked recursively, percolate, but first reset the
     * current signal. Any handler that was already handling a condition will
     * also percolate, until we reach a handler that was not already handling a
     * condition, at which point there is no notion of a previous signal.
     */
    ...
    if ((3207 <= fc->tok_msgno) && (fc->tok_msgno <= 3234)) {
      ...
      if ((5201 <= fc->tok_msgno) && (fc->tok_msgno <= 5238)) {
        ...
        portLibrary->nls_printf(
            portLibrary, J9NLS_ERROR,
            J9NLS_PORT_ZOS_CONDITION_FOR_SOFTWARE_RAISED_SIGNAL_RECEIVED,
            (NULL == asciiFacilityID) ? "NULL" : asciiFacilityID,
            fc->tok_msgno);
   ...
   *
   * If portlibSignalNo is 0 at this point, we have received a condition that
   * cannot be mapped to a hardware-raised POSIX signal. In this case, only
   * handlers that were registered against OMRPORT_SIG_FLAG_SIGALLSYNC should
   * get called,
   *
   * If portlibSignalNo is set, then call handlers registered explicitly for the
   * specified signal type.
   */
  if (OMR_ARE_ANY_BITS_SET(thisRecord->flags, portlibSignalNo) ||
      ((0 == portlibSignalNo) &&
       OMR_ARE_ANY_BITS_SET(thisRecord->flags, OMRPORT_SIG_FLAG_SIGALLSYNC))) {
    ...
    /* get the condition information block so that we can access the machine
     * context */
    ...
    signalInfo.facilityID =
        e2a_func(fc->tok_facid, 3); /* e2a_func() uses malloc, free below */

    /* Save the previous signal, set the current, restore the previous
     * If thisRecord->handler crashes without registering its own protection
     * (using omrsig_protect), omrsig_le_condition_handler will be invoked again
     * with thisRecord, and the recursive check (at top of function) will reset
     * it to zero.
     */
    prevPortlibSignalNo =
        omrsig_get_current_signal_ceehdlr(thisRecord->portLibrary);
    setCurrentSignal(thisRecord->portLibrary, portlibSignalNo);

    thisRecord->recursiveCheck = 1;
    handlerResult =
        thisRecord->handler(thisRecord->portLibrary, portlibSignalNo,
                            &signalInfo, thisRecord->handler_arg);
    thisRecord->recursiveCheck = 0;

    /* control leaves omrsig_le_condition_handler in all cases following this
     * point, so restore the previous signal */
    ...
    switch (handlerResult) {
      case OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH:
        /* thisRecord->handler was not adequately able to address the signal,
         * try the previously registered handler To do this, percolate the
         * signal such as to invoke the previously registered LE condition
         * handler */
      ...
      case OMRPORT_SIG_EXCEPTION_CONTINUE_EXECUTION: {
        ...
        fillInLEJumpInfo(thisRecord->portLibrary, cib_ptr->cib_machine,
      ...
      case OMRPORT_SIG_EXCEPTION_RETURN:
        /* jump back to where this was registered in @ref
         * omrsig_protect_ceehdlr, so that we can return back to it's caller  */
...
int32_t omrsig_protect_ceehdlr(struct OMRPortLibrary *portLibrary,
  ...
  if (0 ==
      (signalOptionsGlobal & OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS)) {
    ...
    /* omrsig_le_condition_handler will jump here in the case of
...
uint32_t omrsig_info_ceehdlr(struct OMRPortLibrary *portLibrary, void *info,
                             uint32_t category, int32_t index,
...
static void setCurrentSignal(struct OMRPortLibrary *portLibrary,
                             uint32_t portLibSigType) {
  omrthread_tls_set(omrthread_self(), tlsKeyCurrentSignal,
                    (intptr_t *)portLibSigType);
}
