 ...
 * @ingroup Port
 * @brief Contains implementation of methods to retrieve memory and processor
 * info details.
 */
 ...
 */
int32_t
retrieveZOSMemoryStats(struct OMRPortLibrary* portLibrary,
                       struct J9MemoryInfo* memInfo)
{
  uint64_t userCacheUsed = 0;
  uint64_t bufferCacheSize = 0;
  int32_t rc = 0;
  /* CVT address should be obtained in the following way to avoid Zero-Address
   * Detection (ZAD) event (see JTCJAZZ: 60569)
   */
  ...
  memInfo->availSwap =
    ((uint64_t)(asmvtp->asmslots -
                (asmvtp->asmvsc + asmvtp->asmnvsc + asmvtp->asmerrs)) *
     J9BYTES_PER_PAGE);
  ...
  Trc_PRT_retrieveZOSMemoryStats_memUsageStats_v1(memInfo->totalPhysical,
 ...
 * platform. */
int32_t
retrieveZOSProcessorStats(struct OMRPortLibrary* portLibrary,
                          struct J9ProcessorInfos* procInfo)
{
  ...
  /* The only valid processor times that z/OS exposes are the total Busy and
   * Idle times. */
  procInfo->procInfoArray[0].idleTime = cctp->ccvrbswt;
  procInfo->procInfoArray[0].busyTime =
    (cctp->ccvrbstd * cctp->ccvcpuct) - cctp->ccvrbswt;

  Trc_PRT_retrieveZOSProcessorStats_cpuUsageStats_v1(
    procInfo->procInfoArray[0].idleTime, procInfo->procInfoArray[0].busyTime);
