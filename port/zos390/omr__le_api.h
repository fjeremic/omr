...
/* if this bit is set in the mch_flags field of omr_31bit_mch, the values in the
 * hgprs field in omr_31bit_mch are valid */
/*
...
/*
 * If this bit is set in the mch_flags field of the omr_31bit_mch, the values in
 * the VR registers in omr_31bit_mch are valid
 *
...
/*
 * Derived from the assembler version, CEEMCH.COPY, as there is no C-mapping of
 * the 31-bit (machine context)
 *
 * The 64-bit C mapping is in __le_api.h
 *
 * This structure is pointed to by the cib_machine field of struct _CEECIB as
 * defined by leawi.h.
 *
 * struct _CEECIB is accessible from a condition handler via the LE service
 * CEE3CIB
 *
 */
typedef struct omr_31bit_mch {
   ...
   */
  uint32_t gprs[16]; /* gprs 0-15 (low word of 64-bit gprs, see hgprs below for
                        high word) */
   ...
   *                	ORG   &NAME.ZERO1                                 @D3A
   *                	&NAME.INT_SF     DC    AL4(0)     Interrupt stack frame
   * @D5A DC    XL11'00'   (reserved)                       @D5C
   */
