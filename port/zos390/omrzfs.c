 ...
 * @file omrzfs.c
 * @ingroup Port
 * @brief Contains implementation of methods to retrieve ZFS file system cache
 * and buffer details.
 */
...
/**
 * Internal helper method to initialize members of the file system call
 * parameter structure.
 * @param[in] syscallParamListPtr Pointer to the system call parameter
 * structure.
 * @param[in] opcode the file system operation code.
 * @param[in] param2 value for the second parameter.
 * @return none
 */
static void fillSyscallParamList(J9ZFSSyscallParamList *syscallParamListPtr,
                                 int32_t opcode, int32_t param2) {
 ...
 * @param[out] clientCacheSizePtr Pointer to the returned client cache area size
 * (in bytes).
 * @return 0 on success; negative value on failure.
 */
static int32_t getZFSClientCacheSize(uint64_t *clientCacheSizePtr) {
   ...
   * is 0
   */
  fillSyscallParamList(&(configParam.syscallParamList),
                       J9CFGOP_QUERY_CLIENT_CACHE_SIZE, 0);
...
#else                           /* AMODE 31-bit */
  BPX1PCT(fsName, J9ZFSCALL_CONFIG, sizeof(configParam), (char *)&configParam,
          &bpxrv, &bpxrc, &bpxrs);
#endif                          /* defined(_LP64) */
#if !defined(OMR_EBCDIC)
  free(fsName);
...
#if !defined(OMR_EBCDIC)
  ascName = e2a_func(configParam.configOption.co_string,
                     strlen(configParam.configOption.co_string));
#else  /* !defined(OMR_EBCDIC) */
...
int32_t getZFSUserCacheUsed(uint64_t *userCacheUsedPtr) {
  ...
  BPX4PCT(fsName,             /* File system type: ZFS (in) */
          ...
          &bpxrs);            /* Reason_code (out) */
#else                         /* AMODE 31-bit */
  BPX1PCT(fsName, J9ZFSCALL_STATS, sizeof(statParam), (char *)&statParam,
          &bpxrv, &bpxrc, &bpxrs);
#endif                        /* defined(_LP64) */
#if !defined(OMR_EBCDIC)
  free(fsName);
  ...
  /* The above statistics operation returns the total and available user cache
   * in pages. */
  userCacheTotal =
      (statParam.statUserCache.stuc_cache_pages * J9BYTES_PER_PAGE);
...
int32_t getZFSMetaCacheSize(uint64_t *bufferCacheSizePtr) {
  ...
  BPX4PCT(fsName,                 /* File system type: ZFS (in) */
          ...
          &bpxrs);                /* Reason_code (out) */
#else                             /* AMODE 31-bit */
  BPX1PCT(fsName, J9ZFSCALL_STATS, sizeof(metaStatParam),
          (char *)&metaStatParam, &bpxrv, &bpxrc, &bpxrs);
#endif                            /* defined(_LP64) */
#if !defined(OMR_EBCDIC)
  free(fsName);
#endif /* !defined(OMR_EBCDIC) */
  ...
  primaryBufferSize = metaStatParam.apiMetaStats.am_primary.buffsize *
                      J9K_TO_BYTES_CONVERSION_FACTOR;
  /* Metadata Backing Cache */
  backingBufferCount = metaStatParam.apiMetaStats.am_backcache.buffers.low;
  backingBufferSize = metaStatParam.apiMetaStats.am_backcache.buffsize *
                      J9K_TO_BYTES_CONVERSION_FACTOR;

  bufferCacheSize = (primaryBufferCount * primaryBufferSize) +
                    (backingBufferCount * backingBufferSize);
