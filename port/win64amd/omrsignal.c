...
/* UNWIND_CODE and UNWIND_INFO are not in the Windows SDK headers, but they are
 * documented on MSDN */
...
static struct {
  ...
  int osSignalNo;
    ...
    {OMRPORT_SIG_FLAG_SIGFPE, SIGFPE},    {OMRPORT_SIG_FLAG_SIGABRT, SIGABRT},
    {OMRPORT_SIG_FLAG_SIGTERM, SIGTERM},  {OMRPORT_SIG_FLAG_SIGINT, SIGINT},
...
/* Calls to registerSignalHandlerWithOS are synchronized using
 * registerHandlerMonitor */
static omrthread_monitor_t registerHandlerMonitor;

/* wakeUpASyncReporter semaphore coordinates between master async signal handler
 * and async signal reporter thread.
 */
...
static uint32_t infoForGPR(struct OMRPortLibrary *portLibrary,
                           struct OMRWin32SignalInfo *info, int32_t index,
                           const char **name, void **value);
static uint32_t infoForFPR(struct OMRPortLibrary *portLibrary,
                           struct OMRWin32SignalInfo *info, int32_t index,
                           const char **name, void **value);
static uint32_t infoForSignal(struct OMRPortLibrary *portLibrary,
                              struct OMRWin32SignalInfo *info, int32_t index,
                              const char **name, void **value);
static uint32_t infoForModule(struct OMRPortLibrary *portLibrary,
                              struct OMRWin32SignalInfo *info, int32_t index,
                              const char **name, void **value);
static uint32_t infoForOther(struct OMRPortLibrary *portLibrary,
                             struct OMRWin32SignalInfo *info, int32_t index,
                             const char **name, void **value);
...
static uint32_t infoForControl(struct OMRPortLibrary *portLibrary,
                               struct OMRWin32SignalInfo *info, int32_t index,
                               const char **name, void **value);
static void fillInWinAMD64SignalInfo(struct OMRPortLibrary *portLibrary,
                                     omrsig_handler_fn handler,
                                     EXCEPTION_POINTERS *exceptionInfo,
...
static uint32_t addMasterVectoredExceptionHandler(
    struct OMRPortLibrary *portLibrary);
static int32_t initializeSignalTools(OMRPortLibrary *portLibrary);
static int structuredExceptionHandler(struct OMRPortLibrary *portLibrary,
                                      omrsig_handler_fn handler,
                                      void *handler_arg, uint32_t flags,
                                      EXCEPTION_POINTERS *exceptionInfo);
static int32_t runInTryExcept(struct OMRPortLibrary *portLibrary,
...
static OMRWinAMD64AsyncHandlerRecord *createAsyncHandlerRecord(
...
static int32_t runInTryExcept(struct OMRPortLibrary *portLibrary,
  ...
  } __except (structuredExceptionHandler(portLibrary, handler, handler_arg,
                                         flags, GetExceptionInformation())) {
    *result = 0;
...
int32_t omrsig_protect(struct OMRPortLibrary *portLibrary,
  ...
  /* omrsig_protect cannot protect with a handler that may do both
   * OMRPORT_SIG_FLAG_MAY_RETURN and OMRPORT_SIG_FLAG_CONTINUE_EXECUTION */
  Assert_PRT_true(
      (OMRPORT_SIG_FLAG_MAY_RETURN | OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION) !=
      (flags & (OMRPORT_SIG_FLAG_MAY_RETURN |
  ...
  /* Check to see if we have already registered the
   * masterVectoredExceptionHandler. Because we are checking without acquiring
   * the monitor first, we'll have to check again. Note that that we don't
   * uninstall the masterHandlerMonitor, so we only have to account for it going
  ...
  /* We can not use setjmp/longjmp to jump out of a VectoredExceptionHandler
   * (CMVC 175576), so use _try/_except semantics instead */
  if (OMR_ARE_ALL_BITS_SET(flags, OMRPORT_SIG_FLAG_MAY_RETURN)) {
    ...
    /* The VectoredExceptionHandler will get notified before the _except
     * clause/block below, this tells the VectoredExceptionHandler to defer to
     * this _except clause/block */
    thisRecord.deferToTryExcept = TRUE;

    /*
     * Jazz 55704 - functions are statically marked with UNW_FLAG_EHANDLER if
     * they contain a _try/__except block. This confuses
     * tryExceptHandlerExistsOnStack() into seeing omrsig_protect as a
     * _try/__except function, even when it (dynamically) isn't using it.
     */
    exceptionStatus = runInTryExcept(portLibrary, fn, fn_arg, handler,
                                     handler_arg, flags, result);
...
int32_t omrsig_set_async_signal_handler(struct OMRPortLibrary *portLibrary,
  ...
  if ((NULL == cursor) && (0 != flags)) {
    OMRWinAMD64AsyncHandlerRecord *record =
        createAsyncHandlerRecord(portLibrary, handler, handler_arg, flags);
    if (NULL != record) {
...
int32_t omrsig_set_single_async_signal_handler(
  ...
  if (!foundHandler && (0 != portlibSignalFlag)) {
    OMRWinAMD64AsyncHandlerRecord *record = createAsyncHandlerRecord(
        portLibrary, handler, handler_arg, portlibSignalFlag);
    if (NULL != record) {
      /* Add the new record to the end of the list. */
...
int32_t omrsig_register_os_handler(struct OMRPortLibrary *portLibrary,
  ...
  if ((0 == portlibSignalFlag) || !OMR_IS_ONLY_ONE_BIT_SET(portlibSignalFlag)) {
    ...
    rc = registerSignalHandlerWithOS(portLibrary, portlibSignalFlag,
                                     (win_signal)newOSHandler, oldOSHandler);
...
int32_t omrsig_can_protect(struct OMRPortLibrary *portLibrary, uint32_t flags) {
  ...
  if (OMR_ARE_NO_BITS_SET(signalOptions,
...
int32_t omrsig_startup(struct OMRPortLibrary *portLibrary) {
  ...
  sigProtectFunction = RtlLookupFunctionEntry((ULONG64)omrsig_protect,
...
/**
 * Set the priority of the asynchronous signal reporting thread
...
/**
 * Look for a try/except signal handler on the stack.  Do not search past
 * omrsig_protect.
...
static BOOLEAN tryExceptHandlerExistsOnStack(OMRPortLibrary *portLibrary,
...
static uint32_t infoForOther(struct OMRPortLibrary *portLibrary,
  ...
  *name = "";
  ...
  switch (index) {
    /* hide this from users that iterate through all of the omrsig_info() by
     * only exposing it through OMRPORT_SIG_WINDOWS_DEFER_TRY_EXCEPT_HANDLER
     * which is a negative value
     */
    case OMRPORT_SIG_WINDOWS_DEFER_TRY_EXCEPT_HANDLER:
      *name = "Defer_to_Try_Except_Handler";
      info->deferToTryExcept =
          !info->tryExceptHandlerIgnore &&
  ...
  *name = "";
  ...
  switch (index) {
    ...
    case 6:
       ...
       * that was being read or written to and whether it was an attempt to
       * read, write or execute the data at that address.
...
static uint32_t infoForModule(struct OMRPortLibrary *portLibrary,
  ...
  if (info->moduleBaseAddress == NULL) {
    ...
    if (VirtualQuery((LPBYTE)info->ExceptionRecord->ExceptionAddress, &mbi,
      ...
      info->offsetInDLL = (uintptr_t)info->ExceptionRecord->ExceptionAddress -
...
masterVectoredExceptionHandler(EXCEPTION_POINTERS *exceptionInfo) {
  ...
  if ((exceptionInfo->ExceptionRecord->ExceptionCode &
       (ERROR_SEVERITY_ERROR | APPLICATION_ERROR_MASK)) !=
  ...
  portLibType = mapWin32ExceptionToPortlibType(
  ...
  /* walk the stack of registered handlers from top to bottom searching for one
   * which handles this type of exception */
  while (thisRecord) {
    if (thisRecord->flags & portLibType) {
      ...
      fillInWinAMD64SignalInfo(thisRecord->portLibrary, thisRecord->handler,
                               exceptionInfo, &signalInfo);
 ...
 * @param exceptionInfo Operatingsystem supplied information about the signal
 * and context.
...
static int structuredExceptionHandler(struct OMRPortLibrary *portLibrary,
  ...
  type = mapWin32ExceptionToPortlibType(
      exceptionInfo->ExceptionRecord->ExceptionCode);
  if (0 == (type & flags)) {
    return EXCEPTION_CONTINUE_SEARCH;
  }
  ...
  __try {
    result =
        handler(portLibrary, signalInfo.portLibType, &signalInfo, handler_arg);
  } __except (EXCEPTION_EXECUTE_HANDLER) {
    /* if a recursive exception occurs, ignore it and pass control to the next
     * handler */
    omrthread_tls_set(thisThread, tlsKeyCurrentSignal, previousSignal);
    omrthread_tls_set(thisThread, tlsKey, thisRecord->previous);
    return EXCEPTION_CONTINUE_SEARCH;
  ...
  if (OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH == result) {
    /*
     * vmSignalHandler will defer to try/except without running the crash
     * handler if it finds a handler on the stack.  Setting
     * tryExceptHandlerIgnore=TRUE forces it to ignore try/except handlers.
     */
    ...
    while (thisRecord) {
      if (thisRecord->flags & type) {
        ...
        result = thisRecord->handler(thisRecord->portLibrary, type, &signalInfo,
        ...
        if (result == OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH) {
          ...
          /* This usage deprecated: it is not exhaustively tested or guaranteed
           * to work */
          omrthread_tls_set(thisThread, tlsKeyCurrentSignal, previousSignal);
          return EXCEPTION_CONTINUE_EXECUTION;
        } else if (result == OMRPORT_SIG_EXCEPTION_RETURN) {
  ...
  if (result == OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH) {
    return EXCEPTION_CONTINUE_SEARCH;
  } else /* if (result == OMRPORT_SIG_EXCEPTION_RETURN) || (result ==
            OMRPORT_SIG_EXCEPTION_CONTINUE_EXECUTION) */
  {
    /* Note that OMRPORT_SIG_EXCEPTION_CONTINUE_EXECUTION isn't supported in
     * this use of SEH, but there's no way to report the error */
...
static uint32_t addMasterVectoredExceptionHandler(
   ...
   * On AMD64, SEH relies on finding exception records associated with the
   * current point of execution. This only works for code in a module (e.g. a
   * DLL). Since JIT code is actually in allocated memory, SEH is bypassed for
   * exceptions which occur in JIT code
   */
  ...
  /* vectoredExceptionHandlerInstalled is checked in omrsig_protect without
   * checking the masterExceptionMonitor */
  /* issueWriteBarrier here in response to CMVC 96193, although it's likely not
...
static int32_t initializeSignalTools(OMRPortLibrary *portLibrary) {
  if (0 != omrthread_monitor_init_with_name(
               &asyncMonitor, 0, "portLibrary_omrsig_async_monitor")) {
    goto error;
  }

  if (0 != omrthread_monitor_init_with_name(
               &masterExceptionMonitor, 0,
  ...
  if (0 != omrthread_monitor_init_with_name(
               ...
               "portLibrary_omrsig_register_handler_monitor")) {
    goto cleanup2;
  }
...
static void fillInWinAMD64SignalInfo(struct OMRPortLibrary *portLibrary,
  ...
  signalInfo->portLibType = mapWin32ExceptionToPortlibType(
      exceptionInfo->ExceptionRecord->ExceptionCode);
...
/**
 * The port library signal flag is converted to the corresponding OS signal
 * number.
 *
 * @param portLibSignal the port library signal flag
 *
 * @return The corresponding OS signal number or OMRPORT_SIG_ERROR (-1) if the
 * portLibSignal can't be mapped.
 */
static int mapPortLibSignalToOSSignal(uint32_t portLibSignal) {
  uint32_t index = 0;
...
/**
 * Register the signal handler with the OS. Calls to this function must be
 * synchronized using "registerHandlerMonitor". *oldOSHandler points to the old
 * signal handler function. It is only set if oldOSHandler is non-null. During
 * the first registration, the old signal handler is stored in
 * handlerInfo[osSignalNo].originalHandler. The original OS handler must be
 * restored during port library shut down.
 *
 * @param[in] portLibrary the OMR port library
 * @param[in] portLibrarySignalNo the port library signal flag
 * @param[in] handler the OS handler to register
 * @param[out] oldOSHandler points to the old OS handler, if oldOSHandler is
 * non-null
 *
...
static int32_t registerSignalHandlerWithOS(OMRPortLibrary *portLibrary,
  ...
  if (SIG_ERR == localOldOSHandler) {
    Trc_PRT_signal_registerSignalHandlerWithOS_failed_to_registerHandler(
        portLibrarySignalNo, osSignalNo, handler);
    return OMRPORT_SIG_ERROR;
  }

  Trc_PRT_signal_registerSignalHandlerWithOS_registeredHandler1(
      portLibrarySignalNo, osSignalNo, handler, localOldOSHandler);
...
/**
 * Atomically increment signalCounts[osSignalNo] and notify the async signal
 * reporter thread that a signal is received.
 *
...
/**
 * Invoke updateSignalCount(osSignalNo), and re-register
 * masterASynchSignalHandler since the masterASynchSignalHandler is unregistered
 * after invocation.
 *
 * @param[in] osSignalNo the integer value of the signal that is raised
 *
...
static void runHandlers(uint32_t asyncSignalFlag, int osSignal) {
  ...
  /* incrementing the asyncThreadCount will prevent the list from being modified
   * while we use it */
  omrthread_monitor_enter(asyncMonitor);
  asyncThreadCount++;
  omrthread_monitor_exit(asyncMonitor);
 ...
 *			2) Attempting to register a handler for a signal that is not
 *included in the allowedSubsetOfFlags. 3) Failure to register the OS signal
 *handler.
 */
static int32_t registerMasterHandlers(OMRPortLibrary *portLibrary,
                                      uint32_t flags,
  ...
  if (OMR_ARE_ALL_BITS_SET(signalOptions,
                           OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_ASYNCHRONOUS)) {
  ...
  if (0 != flagsSignalsOnly) {
    ...
    for (portSignalType = OMRPORT_SIG_SMALLEST_SIGNAL_FLAG;
      ...
      /* Iterate through all the  signals and register the master handler for
       * the signals specified in flagsSignalsOnly.
       */
      if (OMR_ARE_ALL_BITS_SET(flagsSignalsOnly, portSignalType)) {
        if (0 != registerSignalHandlerWithOS(portLibrary, portSignalType,
                                             handler, oldOSHandler)) {
          rc = OMRPORT_SIG_ERROR;
          break;
        }
      }
...
/**
 * Set the thread priority for asynchronous signal reporting thread
 * (asynchSignalReporterThread).
 *
 * @param[in] portLibrary the OMR port library
 * @param[in] priority the thread priority
...
/**
 * Create a new async handler record. If this is the first handler in
 * asyncHandlerList, then register consoleCtrlHandler (HandlerRoutine) using
 ...
 *
 * @return pointer to the new async handler record on success and NULL on
 * failure.
 */
static OMRWinAMD64AsyncHandlerRecord *createAsyncHandlerRecord(
    struct OMRPortLibrary *portLibrary, omrsig_handler_fn handler,
    void *handler_arg, uint32_t flags) {
  OMRWinAMD64AsyncHandlerRecord *record =
      (OMRWinAMD64AsyncHandlerRecord *)portLibrary->mem_allocate_memory(
