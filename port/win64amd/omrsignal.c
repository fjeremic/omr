...
static uint32_t
infoForGPR(struct OMRPortLibrary* portLibrary,
           struct OMRWin32SignalInfo* info,
           int32_t index,
           const char** name,
           void** value);
static uint32_t
infoForFPR(struct OMRPortLibrary* portLibrary,
           struct OMRWin32SignalInfo* info,
           int32_t index,
           const char** name,
           void** value);
static uint32_t
infoForSignal(struct OMRPortLibrary* portLibrary,
              struct OMRWin32SignalInfo* info,
              int32_t index,
              const char** name,
              void** value);
static uint32_t
infoForModule(struct OMRPortLibrary* portLibrary,
              struct OMRWin32SignalInfo* info,
              int32_t index,
              const char** name,
              void** value);
static uint32_t
infoForOther(struct OMRPortLibrary* portLibrary,
             struct OMRWin32SignalInfo* info,
             int32_t index,
             const char** name,
             void** value);
...
static uint32_t
infoForControl(struct OMRPortLibrary* portLibrary,
               struct OMRWin32SignalInfo* info,
               int32_t index,
               const char** name,
               void** value);
static void
fillInWinAMD64SignalInfo(struct OMRPortLibrary* portLibrary,
                         omrsig_handler_fn handler,
                         EXCEPTION_POINTERS* exceptionInfo,
...
static int
structuredExceptionHandler(struct OMRPortLibrary* portLibrary,
                           omrsig_handler_fn handler,
                           void* handler_arg,
                           uint32_t flags,
                           EXCEPTION_POINTERS* exceptionInfo);
               ...
               uintptr_t* result)
{
  __try {
    *result = fn(portLibrary, fn_arg);
    ...
    portLibrary, handler, handler_arg, flags, GetExceptionInformation())) {
    *result = 0;
               ...
               uintptr_t* result)
{
  ...
  if (OMR_ARE_ALL_BITS_SET(flags, OMRPORT_SIG_FLAG_MAY_RETURN)) {
     ...
     */
    exceptionStatus = runInTryExcept(
      portLibrary, fn, fn_arg, handler, handler_arg, flags, result);
                                       ...
                                       void** oldOSHandler)
{
  ...
  rc = registerMasterHandlers(
    portLibrary, portlibSignalFlag, OMRPORT_SIG_FLAG_SIGALLASYNC, oldOSHandler);
  if (0 != rc) {
                           ...
                           void** oldOSHandler)
{
  ...
  if ((0 == portlibSignalFlag) || !OMR_IS_ONLY_ONE_BIT_SET(portlibSignalFlag)) {
    ...
    rc = registerSignalHandlerWithOS(
      portLibrary, portlibSignalFlag, (win_signal)newOSHandler, oldOSHandler);
...
omrsig_startup(struct OMRPortLibrary* portLibrary)
{
  ...
  sigProtectFunction = RtlLookupFunctionEntry(
    (ULONG64)omrsig_protect, &imageBase, &unwindHistoryTable);
...
static BOOLEAN
tryExceptHandlerExistsOnStack(OMRPortLibrary* portLibrary,
...
#define UNW_FLAG_CHAININFO 0x04
  ...
  do {
    ...
    if (!runtimeFunction) {
      ...
      RtlVirtualUnwind(
        UNW_FLAG_NHANDLER, /* suppresses the invocation of any unwind or
                              exception handlers registered by the function).*/
        imageBase,
        context->Rip,
        runtimeFunction,
        context,
              ...
              void** value)
{
  if (info->moduleBaseAddress == NULL) {
    ...
    if (VirtualQuery((LPBYTE)info->ExceptionRecord->ExceptionAddress,
                     &mbi,
...
masterVectoredExceptionHandler(EXCEPTION_POINTERS* exceptionInfo)
{
  ...
  while (thisRecord) {
    if (thisRecord->flags & portLibType) {
      ...
      fillInWinAMD64SignalInfo(thisRecord->portLibrary,
                               thisRecord->handler,
                               exceptionInfo,
                               &signalInfo);
    ...
    while (thisRecord) {
      if (thisRecord->flags & type) {
        ...
        result = thisRecord->handler(
          thisRecord->portLibrary, type, &signalInfo, thisRecord->handler_arg);
    ...
    for (portSignalType = OMRPORT_SIG_SMALLEST_SIGNAL_FLAG;
      ...
      if (OMR_ARE_ALL_BITS_SET(flagsSignalsOnly, portSignalType)) {
        if (0 != registerSignalHandlerWithOS(
                   portLibrary, portSignalType, handler, oldOSHandler)) {
          rc = OMRPORT_SIG_ERROR;
          break;
        }
      }
