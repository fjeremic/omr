...
createAsyncHandlerRecord(struct OMRPortLibrary *portLibrary,
                         omrsig_handler_fn handler, void *handler_arg,
                         uint32_t flags);
...
static uint32_t infoForOther(struct OMRPortLibrary *portLibrary,
  ...
  *name = "";
  ...
  /* hide this from users that iterate through all of the omrsig_info() by only
   * exposing it through OMRPORT_SIG_WINDOWS_DEFER_TRY_EXCEPT_HANDLER which is a
   * negative value
   */
...
static uint32_t infoForSignal(struct OMRPortLibrary *portLibrary,
  ...
  *name = "";
  ...
  case 6:
     ...
     * that was being read or written to and whether it was an attempt to read,
     * write or execute the data at that address.
