...
#define UNW_FLAG_CHAININFO 0x04
  ...
  do {
    ...
    if (!runtimeFunction) {
      ...
      RtlVirtualUnwind(UNW_FLAG_NHANDLER, /* suppresses the invocation of any unwind or exception handlers registered by
                                             the function).*/
...
static LONG WINAPI masterVectoredExceptionHandler(EXCEPTION_POINTERS* exceptionInfo)
{
  ...
  /* record this signal in tls so that omrsig_handler can be called if any of the handlers decide we should be shutting
   * down */
...
static int32_t initializeSignalTools(OMRPortLibrary* portLibrary)
{
  ...
  if (0
    != omrthread_monitor_init_with_name(&masterExceptionMonitor, 0, "portLibrary_omrsig_master_exception_monitor")) {
    goto cleanup1;
  }

  if (0
    != omrthread_monitor_init_with_name(&registerHandlerMonitor, 0, "portLibrary_omrsig_register_handler_monitor")) {
    goto cleanup2;
  }
