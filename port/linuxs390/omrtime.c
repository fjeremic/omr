...
#define OMRTIME_CLOCK_DELTA_ADJUSTMENT_INTERVAL_USEC \
  J9CONST_I64(60 * 1000 * 1000)
 ...
 */
static int64_t muldiv64(const int64_t num, const int64_t numerator,
...
uintptr_t omrtime_msec_clock(struct OMRPortLibrary *portLibrary) {
  ...
  if ((usec - PPG_last_clock_delta_update >
       OMRTIME_CLOCK_DELTA_ADJUSTMENT_INTERVAL_USEC) ||
...
int64_t omrtime_current_time_millis(struct OMRPortLibrary *portLibrary) {
   ...
   * function.
   *
   * The s390 provides an instruction to retrieve a hardware clock,
   * unfortunately that particular clock does not necessarily match the user
   * adjusted OS clock. This leads to a discrepency between the time the user
   * has set and wants to see and what we provide by retrieving the hw clock
   * that the user is not allowed to change.  This code will apply a time delta
   * to the retrieved hardware clock effectively synchronizing it. Some
   * precision is lost due to the problems inherent in calculating the delta
   * itself.
   *
...
int64_t omrtime_nano_time(struct OMRPortLibrary *portLibrary) {
  ...
  // calculate nanosec = subnanosec * NUMERATOR / DENOMINATOR via integer
  // arithmetics
  const int64_t nanosec =
      muldiv64(subnanosec, OMRPORT_TIME_HIRES_NANOTIME_NUMERATOR,
               OMRPORT_TIME_HIRES_NANOTIME_DENOMINATOR);
...
uint64_t omrtime_hires_delta(struct OMRPortLibrary *portLibrary,
  ...
  if (OMRTIME_HIRES_CLOCK_FREQUENCY == requiredResolution) {
    ...
    /*equivalent to ticks / (OMRTIME_HIRES_CLOCK_FREQUENCY /
     * requiredResolution)*/
    ticks = muldiv64(ticks, requiredResolution, OMRTIME_HIRES_CLOCK_FREQUENCY);
  }
  return ticks;
...
/**
 * \brief Calculate the time delta between the zLinux hardware clock and OS
 *clock
 *
 * @param[in] portLibrary The port library.
 *
 *	Calculate the time difference between the STCK retrieved hw clock and
 *the OS gettimeofday() retrieved clock. The time delta is applied to all
 *	subsequent msec, usec and current_time_milli calls.
 */
static void omrtime_calculate_hw_time_delta(
  ...
  /* Calculate the millisecond time delta between the software and hardware
   * clocks */
