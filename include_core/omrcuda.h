...
extern "C"
{
  ...
  /**
   * A function pointer for use with
   * omrcuda_funcMaxActiveBlocksPerMultiprocessor. Given a potential block size
   * the function should return the number of bytes of dynamic shared memory
   * required to launch a related kernel function with that block size.
   */
  typedef uintptr_t (*J9CudaBlockToDynamicSharedMemorySize)(uint32_t blockSize,
                                                            uintptr_t userData);
  ...
  /**
   * Possible values returned for device attribute
   * J9CUDA_DEVICE_ATTRIBUTE_COMPUTE_MODE.
   *
   * Note: literal values must not be changed without affecting public API of
   * CUDA4J (see CudaDevice.COMPUTE_MODE_*).
   */
  typedef enum J9CudaComputeMode
  {
    ...
    /**
     * Compute exclusive process mode (at most one context used by a single
     * process can be present on this device at a time).
     */
    J9CUDA_COMPUTE_MODE_PROCESS_EXCLUSIVE,

    /** Compute prohibited mode (no contexts can be created on this device at
       this time). */
  ...
  typedef enum J9CudaDeviceAttribute
  {
    ...
    /** Memory available on device for __constant__ variables in a kernel in
       bytes. */
  ...
  /**
   * Codes for device limits for use with omrcuda_deviceGetLimit &
   * omrcuda_deviceSetLimit.
   *
   * Note: literal values must not be changed without making the corresponding
  ...
  typedef enum J9CudaDeviceLimit
  {
    ...
    /** maximum grid depth at which a thread can issue the device runtime call
       omrcuda_deviceSynchronize() to wait on child grid launches to complete */
    J9CUDA_DEVICE_LIMIT_DEV_RUNTIME_SYNC_DEPTH,

    /** size in bytes of the heap used by the ::malloc() and ::free() device
       system calls */
   ...
   *
   * Note: The literal values here match those defined by CUDA (in
   * driver_types.h) to avoid requiring translations across the API boundary
   * defined here, and must not be changed without affecting public API of
   * CUDA4J (see CudaError).
   */
  typedef enum J9CudaError
  {
     ...
     * only occur if timeouts are enabled
     * (J9CUDA_DEVICE_ATTRIBUTE_KERNEL_EXEC_TIMEOUT). The device cannot be used
     * until omrcuda_deviceReset() is called. All existing device memory
     ...
     * user has attempted to pass too many arguments to the device kernel, or
     * the kernel launch specifies too many threads for the kernel's register
     * count.
     */
     ...
     * indicated differently than J9CUDA_NO_ERROR (which indicates completion).
     * Calls that may return this value include omrcuda_eventQuery() and
     * omrcuda_streamQuery().
     */
    ...
    /**
     * This indicates that a call tried to access an thread-exclusive device
     ...
     * Kernels launched via the device runtime only support textures created
     * with the Texture Object APIs.
     */
     ...
     * Kernels launched via the device runtime only support surfaces created
     * with the Surface Object APIs.
     */
     ...
     * limit J9CUDA_DEVICE_LIMIT_DEV_RUNTIME_SYNC_DEPTH. To be able to
     * synchronize on launched grids at a greater depth successfully, the
     * maximum nested depth at which omrcuda_deviceSynchronize() will be called
     * must be specified with the J9CUDA_DEVICE_LIMIT_DEV_RUNTIME_SYNC_DEPTH
     * limit to the omrcuda_deviceSetLimit() API before the host-side launch of
     * a kernel using the device runtime. Keep in mind that additional levels of
     * sync depth require the runtime to reserve large amounts of device memory
     ...
     * J9CUDA_DEVICE_LIMIT_DEV_RUNTIME_PENDING_LAUNCH_COUNT. For this launch to
     * proceed successfully, omrcuda_deviceSetLimit() must be called to set the
     * J9CUDA_DEVICE_LIMIT_DEV_RUNTIME_PENDING_LAUNCH_COUNT to be higher than
     * the upper bound of outstanding launches that can be issued to the device
   ...
   *
   * Note: literal values must not be changed without affecting public API of
   * CUDA4J (see CudaFunction.ATTRIBUTE_*).
   */
  ...
  /**
   * Values that may be ORed together to produce valid flags for
   * omrcuda_hostAlloc.
   */
  ...
  /**
   * Available fall-back strategies when an exactly matching object is not
   * available.
   *
   * Note: literal values must not be changed without making the corresponding
  ...
  typedef struct J9CudaJitOptions
  {
    ...
    /**
     * Specifies whether the compiler or linker should generate debug
     * information.
     */
    ...
    /**
     * Specifies whether compiler or linker should generate verbose log
     * messages.
     */
    J9CudaJitFlag verboseLogging;

    /**
     * The maximum number of registers that the compiler may permit a thread to
     * use. If set to zero, no limit will be imposed.
     */
    ...
    /**
     * Limits resource usage of the compiler so that thread blocks of the
     * specified size (or larger) may be actually be launched. If set to zero,
     * no limit will be imposed. If set to a non-zero value, this field will be
     * updated to reflect the number of threads the compiler actually targeted.
     */
  ...
  /**
   * Flags that can be ORed together for use with
   * omrcuda_streamCreateWithPriority or returned by omrcuda_streamGetFlags.
   *
   * Note: literal values must not be changed without affecting public API of
   * CUDA4J (see CudaStream.FLAG_*).
   */
   ...
   */
  typedef void (*J9CudaStreamCallback)(J9CudaStream stream,
                                       int32_t error,
                                       uintptr_t userData);
  ...
  {
    int32_t (*getSummaryData)(struct OMRPortLibrary* portLibrary,
                              J9CudaSummaryDescriptor* summaryData);
    int32_t (*getDeviceData)(struct OMRPortLibrary* portLibrary,
                             uint32_t deviceId,
