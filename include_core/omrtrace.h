...
#define OMR_TRACE_THREAD_FROM_VMTHREAD(thr)                                    \
  ((thr) ? (thr)->_trace.omrTraceThread : NULL)
...
#define UT_NO_FATAL_ASSERT_KEYWORD "NOFATALASSERT"
  ...
  typedef struct OMR_TraceThread
  {
    ...
    unsigned char
      currentOutputMask;     /* Bitmask containing the options
  ...
  {
    omr_error_t (*RegisterRecordSubscriber)(struct OMR_TraceThread* thr,
                                            const char* description,
                                            utsSubscriberCallback func,
                                            utsSubscriberAlarmCallback alarm,
                                            void* userData,
                                            UtSubscription** record);
    omr_error_t (*DeregisterRecordSubscriber)(struct OMR_TraceThread* thr,
   ...
   * A callback may be NULL. We will NULL-check each function pointer before
   * invoking it.
   * =============================================================================
   */
  typedef void (*OMR_TraceCommandLineErrorFunc)(OMRPortLibrary* portLibrary,
   ...
   * omr_trc_startup can only be called once during the life of the process.
   * Restarting trace is not supported.
   *
   ...
   * must be provided in name / value pairs. If a name has no value, a NULL
   * array entry must be provided. e.g. { "print", "none", NULL } is valid e.g.
   ...
   * omr_trc_startup cannot be called to restart trace after omr_trc_shutdown
   * has been called.
   *
   ...
   *
   * @param[in,out] currentThread The current OMR VM thread.
   *                currentThread, currentThread->_language_vmthread, and
   * currentThread->_os_thread are used to register the thread. The trace
   * engine's thread identifier will be saved into a field of currentThread.
  ...
  /**
   * This function should be called in the parent process prior to any fork()
   * operation in any environment where OMR expects the runtime to call fork().
   * Trace initialization and termination concurrent to fork is not supported;
   * these operations must be done at vm start up and shutdown.
   */
  void omr_trc_preForkHandler(void);

  /**
   * This function should be called in the parent process after any fork()
   * operation in any environment where OMR expects the runtime to call fork().
   */
  void omr_trc_postForkParentHandler(void);

  /**
   * This function should be called in the child process after any fork()
   * operation in any environment where OMR expects the runtime to call fork().
   */
