...
/*
A thread priority calue can be one of three possible types: a java thread priority, a omrthread thread priority or a
native operating system priority. A single integer can be used to store both the priority value and type, and these
macros allow for this storage and conversion to the desired java or omrthread priority value.
*/
...
#define getJavaPriorityFromIndicator(vm, value)                                                            \
  ((PRIORITY_INDICATOR_TYPE(value) == PRIORITY_INDICATOR_JAVA_PRIORITY)                                    \
      ...
      : ((PRIORITY_INDICATOR_TYPE(value) == PRIORITY_INDICATOR_NATIVE_PRIORITY)                            \
          ? (vm)->j9Thread2JavaPriorityMap[omrthread_map_native_priority(PRIORITY_INDICATOR_VALUE(value))] \
...
#define J9THREAD_LIB_FLAG_JLM_ENABLED_ALL \
  (J9THREAD_LIB_FLAG_JLM_ENABLED | J9THREAD_LIB_FLAG_JLM_TIME_STAMPS_ENABLED | J9THREAD_LIB_FLAG_JLMHST_ENABLED)
...
#define J9THREAD_LIB_FLAG_JLM_INFO_SAMPLING_ENABLED \
  (J9THREAD_LIB_FLAG_JLM_HOLDTIME_SAMPLING_ENABLED | J9THREAD_LIB_FLAG_JLM_SLOW_SAMPLING_ENABLED)
#define J9THREAD_LIB_FLAG_JLM_INIT_DATA_STRUCTURES                             \
  (J9THREAD_LIB_FLAG_JLM_ENABLED | J9THREAD_LIB_FLAG_JLM_INFO_SAMPLING_ENABLED \
