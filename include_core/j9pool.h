...
typedef void *(*omrmemAlloc_fptr_t)(void *, uint32_t, const char *, uint32_t,
                                    uint32_t, uint32_t *);
...
typedef struct J9Pool {
  ...
  J9WSRP puddleList;
  void *(*memAlloc)(void *userData, uint32_t byteAmount, const char *callsite,
...
#define J9POOLPUDDLE_FIRSTFREESLOT(parm) \
  SRP_GET((parm)->firstFreeSlot, uintptr_t *)
#define J9POOLPUDDLE_FIRSTELEMENTADDRESS(parm) \
  NNSRP_GET((parm)->firstElementAddress, void *)
#define J9POOLPUDDLE_PREVPUDDLE(parm) \
  WSRP_GET((parm)->prevPuddle, J9PoolPuddle *)
#define J9POOLPUDDLE_NEXTPUDDLE(parm) \
  WSRP_GET((parm)->nextPuddle, J9PoolPuddle *)
#define J9POOLPUDDLE_NEXTAVAILABLEPUDDLE(parm) \
  WSRP_GET((parm)->nextAvailablePuddle, J9PoolPuddle *)
#define J9POOLPUDDLE_PREVAVAILABLEPUDDLE(parm) \
  WSRP_GET((parm)->prevAvailablePuddle, J9PoolPuddle *)
#define J9POOL_PUDDLELIST(pool) \
  NNWSRP_GET((pool)->puddleList, J9PoolPuddleList *)
#define J9POOLPUDDLELIST_NEXTPUDDLE(parm) \
  NNWSRP_GET((parm)->nextPuddle, J9PoolPuddle *)
#define J9POOLPUDDLELIST_NEXTAVAILABLEPUDDLE(parm) \
  WSRP_GET((parm)->nextAvailablePuddle, J9PoolPuddle *)
