...
/* Platform specific VM "types":
   ...
   pointer storage.
        intptr_t			signed data, can be used as an integer
   or pointer storage. U_64 / I_64		unsigned/signed 64 bits. U_32 /
   I_32		unsigned/signed 32 bits. U_16 / I_16
   unsigned/signed 16 bits. U_8 / I_8		unsigned/signed 8 bits (bytes --
...
#elif defined(LINUX) || defined(RS6000) || defined(J9ZOS390) || \
    defined(OSX) || defined(OMRZTPF) /* MVS */
...
CLOCK_PRIMS:					We want the timer/clock prims to
be used
...
SYS_FLOAT:	For the MPW C compiler on MACintosh. Most of the math functions
                                                there return extended type which
has 80 or 96 bits depending on 68881 option. On All other platforms it is double
...
EXE_EXTENSION_CHAR: the executable has a delimiter that we want to stop at as
part of argv[0].
...
/* LINUXPPC64LE has a new ABI that uses direct functions, not function
 * descriptors */
...
#define TOC_STORE_TOC(dest, wrappedPointer) \
  (dest = (((uintptr_t *)(wrappedPointer))[1]))
...
#if (defined(LINUXPPC) && !defined(LINUXPPC64)) || defined(S390) || \
    defined(J9HAMMER)
...
/*
 * xlC11 C++ compiler reportedly supports attributes before function names, but
 * we've only tested xlC12. The C compiler doesn't support it.
 */
...
/* XLC doesn't support __attribute before a function name and doesn't support
 * inline in the stdc89 language level */
#ifndef VMINLINE_ALWAYS
 ...
 */
#define TOC_UNWRAP_ENV(wrappedPointer) \
  (wrappedPointer ? ((J9FunctionDescriptor_T *)(wrappedPointer))->ada : NULL)
#define TOC_UNWRAP_ADDRESS(wrappedPointer)                                     \
  (wrappedPointer                                                              \
       ? ((J9FunctionDescriptor_T *)(uintptr_t)(wrappedPointer))->rawFnAddress \
       : NULL)
...
/* on some systems (e.g. LinuxPPC) va_list is an array type.  This is probably
 * in violation of the ANSI C spec, but it's not entirely clear.  Because of
 * this, we end up with an undesired extra level of indirection if we take the
 * address of a va_list argument.
 *
...
#define PTR_LONG_STORE(dstPtr, aLongPtr) \
  ((*U32P(dstPtr) = *U32P(aLongPtr)),    \
   (*(U32P(dstPtr) + 1) = *(U32P(aLongPtr) + 1)))
#define PTR_LONG_VALUE(dstPtr, aLongPtr) \
  ((*U32P(aLongPtr) = *U32P(dstPtr)),    \
   (*(U32P(aLongPtr) + 1) = *(U32P(dstPtr) + 1)))
#else
...
#ifndef J9_SORT
#define J9_SORT(base, nmemb, size, compare) \
