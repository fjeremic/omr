...
#define J9_EVENT_IS_HOOKED(interface, event)                               \
  (((uint8_t *)(&(interface).common + 1))[(event)&J9HOOK_EVENT_NUM_MASK] & \
   J9HOOK_FLAG_HOOKED)
#define J9_EVENT_IS_RESERVED(interface, event)                             \
  (((uint8_t *)(&(interface).common + 1))[(event)&J9HOOK_EVENT_NUM_MASK] & \
   J9HOOK_FLAG_RESERVED)
...
typedef void (*J9HookFunction)(struct J9HookInterface **hookInterface,
                               uintptr_t eventNum, void *eventData,
                               void *userData); /* Forward struct declaration */
typedef struct J9HookInterface {
  void (*J9HookDispatch)(struct J9HookInterface **hookInterface,
                         uintptr_t eventNum, void *eventData);
  intptr_t (*J9HookDisable)(struct J9HookInterface **hookInterface,
                            uintptr_t eventNum);
  intptr_t (*J9HookReserve)(struct J9HookInterface **hookInterface,
                            uintptr_t eventNum);
  intptr_t (*J9HookRegister)(struct J9HookInterface **hookInterface,
                             uintptr_t eventNum, J9HookFunction function,
                             void *userData, ...);
  intptr_t (*J9HookRegisterWithCallSite)(struct J9HookInterface **hookInterface,
                                         uintptr_t eventNum,
                                         J9HookFunction function,
                                         const char *callsite, void *userData,
                                         ...);
  void (*J9HookUnregister)(struct J9HookInterface **hookInterface,
                           uintptr_t eventNum, J9HookFunction function,
                           void *userData);
  void (*J9HookShutdownInterface)(struct J9HookInterface **hookInterface);
  intptr_t (*J9HookIsEnabled)(struct J9HookInterface **hookInterface,
                              uintptr_t eventNum);
  uintptr_t (*J9HookAllocateAgentID)(struct J9HookInterface **hookInterface);
  void (*J9HookDeallocateAgentID)(struct J9HookInterface **hookInterface,
                                  uintptr_t agentID);
} J9HookInterface;
...
#define OMRHOOK_DEFAULT_THRESHOLD_IN_MICROSECONDS_WARNING_CALLBACK_ELAPSED_TIME \
  (100 * 1000)

/* array of OMREventInfo4Dump is added in individual hookInterface by Hook
   generation tool to avoid rumtime native memory allocation(malloc), use this
   macro to access &infos4Dump[event] locate infos4Dump from end of interface to
   avoid alignment calculating
*/

#define J9HOOK_DUMPINFO(interface, event)                             \
  (&((OMREventInfo4Dump *)&(                                          \
      ((J9HookRecord **)((uint8_t *)(interface) + (interface)->size)) \
          [0 - (interface)->eventSize]))[event - (interface)->eventSize])
 ...
 * Invoked every time a listener is succesfully registered or unregistered for
 * an event (including this one). No event is reported if the listener was not
 * added since it was already registered.
 */
