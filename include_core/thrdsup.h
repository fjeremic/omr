...
/** This header is included from outside the thread library directory and does
   not have access to vpath information encoded in the makefile.  Do a
   compile-time test of a J9THREAD_LIB constant defined in j9cfg.h.
 ...
 * @param[out] handle Location where this function should return the new
 * thread's handle. Must be non-NULL.
 * @param[in] attr Creation attributes. Must be non-NULL. This function will not
 * modify attr.
 ...
 * @retval J9THREAD_ERR_OS_ERRNO_SET bit flag indicating that an os_errno is
 * available.
 */
intptr_t osthread_create(struct J9Thread *self, OSTHREAD *handle,
                         const omrthread_attr_t attr, WRAPPER_FUNC entrypoint,
