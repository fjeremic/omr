...
#define J9_LINEAR_LINKED_LIST_ADD(linkNext, linkPrevious, root, element)       \
  do {                                                                         \
    if ((root) == NULL) {                                                      \
      ...
      J9_LINEAR_LINKED_LIST_ADD_BEFORE(                                        \
        linkNext, linkPrevious, root, root, element);                          \
    }                                                                          \
  } while (0)

#define J9_LINEAR_LINKED_LIST_ADD_BEFORE(                                      \
  linkNext, linkPrevious, root, before, element)                               \
  do {                                                                         \
    (element)->linkNext = (before);                                            \
    if ((before) == (root)) {                                                  \
      (element)->linkPrevious = NULL;                                          \
    } else {                                                                   \
      ((element)->linkPrevious = (before)->linkPrevious)->linkNext =           \
        (element);                                                             \
    }                                                                          \
    (before)->linkPrevious = (element);                                        \
    if ((before) == (root)) {                                                  \
...
#define J9_LINEAR_LINKED_LIST_ADD_AFTER(                                       \
  linkNext, linkPrevious, root, after, element)                                \
  do {                                                                         \
    (element)->linkPrevious = (after);                                         \
