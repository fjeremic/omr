...
extern "C"
{
  ...
  /**
   * @brief Store unsigned 32 bit value at memory location as an atomic
   * operation. Compare the unsigned 32 bit value at the provided memory
   * location.  If it is equal to oldValue then update this memory location with
   * newValue else retain the oldValue.
   *
   * @param[in] location The memory location to be updated
   * @param[in] oldValue The expected value at memory address
   * @param[in] newValue The new value to be stored at memory address
   ...
   * the next following instructions. It is used to ensure that no loads
   * following entry into a critical section can access data (because of
   * aggressive out-of-order and speculative execution in the processor) before
   ...
   * operation. Adds the addend to the value stored at memory location pointed
   * to by address.
   *
  ...
  /**
   * @brief Subtracts a number from the value at a specific memory location as
   * an atomic operation. Subtracts the value from the value stored at memory
