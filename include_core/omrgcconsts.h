...
typedef enum MM_GCWriteBarrierType {
  ...
  gc_modron_wrtbar_cardmark_and_oldcheck =
      OMR_GC_WRITE_BARRIER_TYPE_CARDMARK_AND_OLDCHECK,
  gc_modron_wrtbar_satb = OMR_GC_WRITE_BARRIER_TYPE_SATB,
  gc_modron_wrtbar_satb_and_oldcheck =
      OMR_GC_WRITE_BARRIER_TYPE_SATB_AND_OLDCHECK,
...
typedef enum MM_MarkingSchemeScanReason {
  SCAN_REASON_PACKET =
      1, /**< Indicates the object being scanned came from a work packet */
  SCAN_REASON_DIRTY_CARD =
      2, /**< Indicates the object being scanned was found in a dirty card */
  SCAN_REASON_REMEMBERED_SET_SCAN =
      3, /**< Indicates the object being scanned was in a remembered set */
  SCAN_REASON_OVERFLOWED_OBJECT =
      4, /**< Indicates the object being scanned was in an overflowed region */
...
/* Core allocation flags defined for OMR are <
 * OMR_GC_ALLOCATE_OBJECT_LANGUAGE_DEFINED_BASE */
...
/* Languages may define additional allocation flags >=
 * OMR_GC_ALLOCATE_OBJECT_LANGUAGE_DEFINED_BASE */
...
/* Object-model related constants moved from builder (J9VMConstantValue.st /
 * j9generated.h) These replace the following constants from builder
 * (J9VMConstantValue.st / j9consts.h): #define J9_OBJECT_HEADER_AGE_MAX 0xE
 * #define J9_OBJECT_HEADER_AGE_MIN 0x1
 * Note that some related defines remain, in both j9generated.h and j9consts.h,
 * e.g.: J9_OBJECT_HEADER_AGE_DEFAULT		/ OBJECT_HEADER_AGE_DEFAULT
...
/* When changing the heap alignment below you need to check that the new
 * alignment is compatible with the requirements of compaction and concurrent
 * mark.
 */
...
#define OMRPORT_ACCESS_FROM_ENVIRONMENT(env) \
 ...
 * Do not use the custom spinlocks on AIX as yield calls create performance
 * issues.
 */
...
/*
 * if CURRENT_UNIT_ONLY_TAG bit is clear, the work unit is from index till the
 * end of the array (normal case) if CURRENT_UNIT_ONLY_TAG bit is set, the work
 * unit is from the index with size only of arraySplitSize (exceptional case)
 */
 ...
 * - for 32 bit platforms, no reference array of size more than 2^30 can be
 * created, since each ref slot is 4 bytes (unless we use slot compression,
 * which we do not for now)
 */
...
/**
 * Concurrent status symbols. Client language may define additional status
 * values > CONCURRENT_ROOT_TRACING and < CONCURRENT_TRACE_ONLY corresponding to
 * parallelizable units of tracing work.
 ...
 * Currently the "extended" set above those found in omrgcconsts.h - on the VM
 * side.  Those constants should be moved back over to this side and everything
 * unified.  Until then, continue the list here.
 * @{
 */
...
/* Define the low memory heap ceiling (max heap address when
 * -Xgc:forceLowMemHeap is specified) */
#if defined(OMR_ENV_DATA64)
...
#define LOW_MEMORY_HEAP_CEILING \
  (NON_SCALING_LOW_MEMORY_HEAP_CEILING << LOW_MEMORY_HEAP_CEILING_SHIFT)
#define DEFAULT_LOW_MEMORY_HEAP_CEILING \
  (NON_SCALING_LOW_MEMORY_HEAP_CEILING << DEFAULT_LOW_MEMORY_HEAP_CEILING_SHIFT)
#else /* defined(OMR_ENV_DATA64) */
#define LOW_MEMORY_HEAP_CEILING ((uintptr_t)0)
#define NON_SCALING_LOW_MEMORY_HEAP_CEILING ((uintptr_t)0)
#endif /* defined(OMR_ENV_DATA64) */

/* Because SLES zLinux/31 never allocates mmap()ed memory below the 1GB line
 * unless you ask it to, we always request that the heap is allocated low in the
 * address range. This leaves the space above 2GB free for other mmap()
 * allocations (e.g. pthread stacks).*/
...
#if defined(OMR_GC_REALTIME)
#define METRONOME_DEFAULT_HRT_PERIOD_MICRO \
  1000 /* This gives vanilla linux a chance to use the HRT */
