 ...
 * Hash table flags
 */
#define J9HASH_TABLE_DO_NOT_GROW \
  0x00000001 /*!< Do not grow & rehash the table while set */
#define J9HASH_TABLE_COLLISION_RESILIENT                                  \
  0x00000002 /*!< Use hash table using avl trees for collision resolution \
                instead of lists */
#define J9HASH_TABLE_ALLOCATE_ELEMENTS_USING_MALLOC32 \
  0x00000004 /*!< Allocate table elements using the malloc32 function */
#define J9HASH_TABLE_ALLOW_SIZE_OPTIMIZATION                          \
  0x00000008 /*!< Allow space optimized hashTable, some functions not \
                supported */
#define J9HASH_TABLE_DO_NOT_REHASH \
  0x00000010 /*!< Do not rehash the table while set */

/*
 * This used to include a cast to uintptr_t, but ddrgen doesn't
 * handle casts; that cast has been moved to hashtable.c.
 */
#define J9HASH_TABLE_AVL_TREE_TAG_BIT                                         \
  0x00000001 /*!< Bit to indicate that hastable slot contains a pointer to an \
                AVL tree */
...
#define AVL_DATA_TO_NODE(p) \
  (((J9AVLTreeNode *)((uint8_t *)(p) - sizeof(J9AVLTreeNode))))

/**
 * Hash table flag macros
 */
#define hashTableCanGrow(table) \
  (((table)->flags & J9HASH_TABLE_DO_NOT_GROW) ? 0 : 1)
#define hashTableCanRehash(table) \
  (((table)->flags & J9HASH_TABLE_DO_NOT_REHASH) ? 0 : 1)
...
typedef uintptr_t (*J9HashTableHashFn)(
    void *entry, void *userData); /* Forward struct declaration */
typedef uintptr_t (*J9HashTableEqualFn)(
    void *leftEntry, void *rightEntry,
    void *userData); /* Forward struct declaration */
typedef intptr_t (*J9HashTableComparatorFn)(
    struct J9AVLTree *tree, struct J9AVLTreeNode *leftNode,
    struct J9AVLTreeNode *rightNode); /* Forward struct declaration */
typedef void (*J9HashTablePrintFn)(
    OMRPortLibrary *portLibrary, void *entry,
    void *userData); /* Forward struct declaration */
typedef uintptr_t (*J9HashTableDoFn)(
    void *entry, void *userData); /* Forward struct declaration */
