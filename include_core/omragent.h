...
typedef struct OMR_TI {
     ...
     * 								return OMR_ERROR_NOT_AVAILABLE if not supported or insufficient user privilege or vm->sysInfo
     * is not initialized. return OMR_ERROR_RETRY on the first call or if the interval between two calls is too small.
     * return OMR_ERROR_INTERNAL if the calculated CPU load is invalid. return OMR_ERROR_ILLEGAL_ARGUMENT if
     * systemCpuLoad is NULL. return OMR_THREAD_NOT_ATTACHED if vmThread is NULL.
     */
     ...
     * is not initialized. return OMR_ERROR_RETRY on the first call or if the interval between two calls is too small.
     * return OMR_ERROR_INTERNAL if the calculated CPU load is invalid. return OMR_ERROR_ILLEGAL_ARGUMENT if
     * processCpuLoad is NULL. return OMR_THREAD_NOT_ATTACHED if vmThread is NULL.
     */
