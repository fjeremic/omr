...
typedef struct OMR_TI {
   ...
   * @param[in] subscriberFunc Trace subscriber callback function. May not be NULL. This function will be passed records
   * containing trace data as they are processed.
   * @param[in] alarmFunc Alarm callback function that's executed if the subscriber shuts down, returns an error or
   * misbehaves. May be NULL.
   ...
   * 								return OMR_ERROR_NOT_AVAILABLE if not supported or insufficient user privilege or vm->sysInfo is not
   * initialized. return OMR_ERROR_RETRY on the first call or if the interval between two calls is too small. return
   * OMR_ERROR_INTERNAL if the calculated CPU load is invalid. return OMR_ERROR_ILLEGAL_ARGUMENT if systemCpuLoad is
   * NULL. return OMR_THREAD_NOT_ATTACHED if vmThread is NULL.
   */
   ...
   * 								return OMR_ERROR_NOT_AVAILABLE if not supported or insufficient user privilege or vm->sysInfo is not
   * initialized. return OMR_ERROR_RETRY on the first call or if the interval between two calls is too small. return
   * OMR_ERROR_INTERNAL if the calculated CPU load is invalid. return OMR_ERROR_ILLEGAL_ARGUMENT if processCpuLoad is
   * NULL. return OMR_THREAD_NOT_ATTACHED if vmThread is NULL.
   */
   ...
   * @param[out] categories_buffer Block of memory to write result into. 0th entry is the first root category. All other
   * nodes can be walked from the root.
   ...
   * OMR_ERROR_OUT_OF_MEMORY - Memory category data was truncated because category_buffer/max_categories was not large
   * enough OMR_ERROR_INTERNAL - GetMemoryCategories was unable to allocate memory for it's own use.
   ...
   *
   * @return An OMR error code.
   * @retval OMR_ERROR_NONE All available method descriptions were successfully retrieved. Some descriptions may have
   * been unavailable.
   * @retval OMR_THREAD_NOT_ATTACHED vmThread is NULL.
   ...
   */
  omr_error_t (*GetMethodProperties)(
    OMR_VMThread* vmThread, size_t* numProperties, const char* const** propertyNames, size_t* sizeofSampledMethodDesc);
} OMR_TI;
