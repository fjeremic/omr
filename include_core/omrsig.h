 ...
 *secondary are registered, as if it were invoked directly by the OS in response
 *to a caught signal. Secondary handlers support only the SA_RESETHAND,
 *SA_SIGINFO, and SA_NODEFER flags. Secondary handlers will inherit the masked
 *signals of the current primary handler.
 *
 ...
 *signal handling. DEFAULT_ACTION_REQUIRED - If caller needs to call default
 *action (SIG_DFL) for this signal.
 */
