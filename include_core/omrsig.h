...
#define OMRSIG_RC_DEFAULT_ACTION_REQUIRED 1
   ...
   *secondary are registered, as if it were invoked directly by the OS in
   *response to a caught signal. Secondary handlers support only the
   *SA_RESETHAND, SA_SIGINFO, and SA_NODEFER flags. Secondary handlers will
   *inherit the masked signals of the current primary handler.
   *
   * @param[in] sig The signal number
   * @param[in] siginfo siginfo_t* passed into the primary handler by the
   *system.
   * @param[in] uc ucontext_t* passed into the primary handler by the system.
   * @return an int error code
   *		SIGNAL_HANDLED - If caller should not continue to do any more signal
   *handling. DEFAULT_ACTION_REQUIRED - If caller needs to call default action
...
/**
 * Register a primary signal handler function, emulating the behavior,
 *parameters, and return of sigaction(). Primary handlers only support the
 *SA_SIGINFO and SA_NODEFER flags.
 *
 * @param[in] signum Signal number
 ...
 */
int
omrsig_primary_sigaction(int signum,
