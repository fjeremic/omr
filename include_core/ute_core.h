...
/* Trace suspend/resume and disable/enable options. Suspend/resume is the
 * external user facility, disable/enable is the internal mechanism that trace,
 * dump and memcheck components use. Both mechanisms provide global (all
 * threads) and current thread options.
 */
 ...
 * =============================================================================
 */
typedef void (*UtListenerWrapper)(void *userData, OMR_VMThread *env,
                                  void **thrLocal, const char *modName,
                                  uint32_t traceId, const char *format,
                                  va_list varargs);

struct UtSubscription;

typedef omr_error_t (*utsSubscriberCallback)(
    struct UtSubscription *subscription);
 ...
 * =============================================================================
 */
/* If you modify this structure, please update dbgext_utthreaddata in dbgTrc.c
 * to cope with the modifications.
 */
...
struct UtServerInterface {
  void (*TraceVNoThread)(UtModuleInfo *modInfo, uint32_t traceId,
                         const char *spec, va_list varArgs);
  omr_error_t (*TraceSnap)(UtThreadData **thr, char *label, char **response);
  omr_error_t (*TraceSnapWithPriority)(UtThreadData **thr, char *label,
                                       int32_t snapPriority, char **response,
                                       int32_t sync);
  omr_error_t (*AddComponent)(UtModuleInfo *modInfo, const char **fmt);
  omr_error_t (*GetComponents)(UtThreadData **thr, char ***list,
                               int32_t *number);
  omr_error_t (*GetComponent)(char *name, unsigned char **bitmap,
                              int32_t *first, int32_t *last);
  int32_t (*Suspend)(UtThreadData **thr, int32_t type);
  int32_t (*Resume)(UtThreadData **thr, int32_t type);

  struct UtTracePointIterator *(*GetTracePointIteratorForBuffer)(
      UtThreadData **thr, const char *bufferName);
  char *(*FormatNextTracePoint)(struct UtTracePointIterator *iter, char *buffer,
                                uint32_t buffLen);
  omr_error_t (*FreeTracePointIterator)(UtThreadData **thr,
                                        struct UtTracePointIterator *iter);
  omr_error_t (*RegisterRecordSubscriber)(
      UtThreadData **thr, const char *description, utsSubscriberCallback func,
      utsSubscriberAlarmCallback alarm, void *userData,
      struct UtTraceBuffer *start, struct UtTraceBuffer *stop,
      UtSubscription **record, int32_t attach);
  omr_error_t (*DeregisterRecordSubscriber)(UtThreadData **thr,
                                            UtSubscription *subscriptionID);
  omr_error_t (*FlushTraceData)(UtThreadData **thr,
                                struct UtTraceBuffer **first,
                                struct UtTraceBuffer **last, int32_t pause);
  omr_error_t (*GetTraceMetadata)(void **data, int32_t *length);
  void (*DisableTrace)(int32_t type);
  void (*EnableTrace)(int32_t type);
  omr_error_t (*RegisterTracePointSubscriber)(UtThreadData **thr,
                                              char *description,
                                              utsSubscriberCallback callback,
                                              utsSubscriberAlarmCallback alarm,
                                              void *userData,
                                              UtSubscription **subscription);
  omr_error_t (*DeregisterTracePointSubscriber)(UtThreadData **thr,
                                                UtSubscription *subscription);
  omr_error_t (*TraceRegister)(UtThreadData **thr, UtListenerWrapper func,
                               void *userData);
  omr_error_t (*TraceDeregister)(UtThreadData **thr, UtListenerWrapper func,
                                 void *userData);
