...
#define EsOpenCreateNew                                                                                            \
    0x40 /* Use this flag with EsOpenCreate, if this flag is specified then trying to create an existing file will \
            fail */
#define EsOpenSync 0x80
#define EsOpenForMapping                                                                                            \
    0x100 /* Required for WinCE for file memory mapping, ignored on other platforms. WINCE is not supported so this \
...
#define J9NLS_ERROR_MESSAGE(id, message) \
    ("" J9NLS_ERROR_PREFIX "" J9NLS_COMMON_PREFIX "" id##__PREFIX "" J9NLS_ERROR_SUFFIX " " message)
#define J9NLS_INFO_MESSAGE(id, message) \
    ("" J9NLS_INFO_PREFIX "" J9NLS_COMMON_PREFIX "" id##__PREFIX "" J9NLS_INFO_SUFFIX " " message)
#define J9NLS_WARNING_MESSAGE(id, message) \
    ("" J9NLS_WARNING_PREFIX "" J9NLS_COMMON_PREFIX "" id##__PREFIX "" J9NLS_WARNING_SUFFIX " " message)
/** @} */
...
typedef struct J9PortVmemParams {
     ...
     *			- used to allocate memory in 4K pages using system macros instead of malloc() or __malloc31()
     *routines
     ...
     * 			- if this flag is set and the 2to32G support is not there omrvmem_reserve_memory_ex will return
     *failure \arg OMRPORT_VMEM_ALLOC_QUICK
     ...
     *		- this option is based on the observation that mmap would take the given address as a hint about where to
     *place the mapping
     *		- this option does not apply to large page allocations as the allocation is done with shmat instead of
     *mmap
     */
    uintptr_t options;
...
/* z/OS 31-bit uses signed pointer comparison so this UDATA_MAX maximum address becomes -1 which is less than the
 * minimum address of 0 so use IDATA_MAX instead */
...
/* omrfile_chown takes unsigned arguments for user/group IDs, but uses -1 to indicate that group/user id are not to be
 * changed */
...
typedef struct J9SysinfoEnvIteratorState {
    ...
    void* buffer; /* caller-allocated buffer. This can be freed by the caller once they have finished using the
                     iterator. */
...
typedef struct J9SysinfoCPUTime {
    ...
    int64_t cpuTime; /* cumulative CPU utilization (sum of system and user time in nanoseconds) of all CPUs on the
                        system. */
...
typedef struct OMRMemCategoryWalkState {
     ...
     */
    uintptr_t (*walkFunction)(uint32_t categoryCode, const char* categoryName, uintptr_t liveBytes,
        uintptr_t liveAllocations, BOOLEAN isRoot, uint32_t parentCategoryCode, struct OMRMemCategoryWalkState* state);
...
typedef struct J9MemoryNodeDetail {
    ...
    J9MemoryState memoryPolicy; /**< Whether the memory on this node is preferred, allowed, or denied use by this
                                   process under the current NUMA policy */
    uintptr_t computationalResourcesAvailable; /**< The number of computational resources on this node which are
                                                  currently available for use under the current NUMA policy */
} J9MemoryNodeDetail;
...
#define OMRPORT_SLOPEN_DECORATE \
    1 /* Note this value must remain 1, in order for legacy callers using TRUE and FALSE to control decoration */
#if !defined(OMRZTPF)
...
#if defined(J9ZOS390)
/*
 * OMR on z/OS translates the output of certain system calls such as getenv to ASCII using functions in atoe.c; see
 * stdlib.h for a list. Use J9STR_CODE_PLATFORM_OMR_INTERNAL to when processing the output of these calls.  Otherwise,
 * use J9STR_CODE_PLATFORM_RAW.
 */
...
typedef uintptr_t (*omrsig_handler_fn)(
    struct OMRPortLibrary* portLib, uint32_t gpType, void* gpInfo, void* handler_arg);
...
typedef struct OMRPortLibrary {
    ...
    int32_t (*error_set_last_error_with_message)(
        struct OMRPortLibrary* portLibrary, int32_t portableCode, const char* errorMessage);
    ...
    intptr_t (*sysinfo_get_env)(
        struct OMRPortLibrary* portLibrary, const char* envVar, char* infoString, uintptr_t bufSize);
    ...
    intptr_t (*sysinfo_get_load_average)(
        struct OMRPortLibrary* portLibrary, struct J9PortSysInfoLoadData* loadAverageData);
    ...
    int32_t (*sysinfo_limit_iterator_next)(struct OMRPortLibrary* portLibrary, J9SysinfoLimitIteratorState* state,
        J9SysinfoUserLimitElement* limitElement);
    /** see @ref omrsysinfo.c::omrsysinfo_env_iterator_init "omrsysinfo_env_iterator_init"*/
    int32_t (*sysinfo_env_iterator_init)(struct OMRPortLibrary* portLibrary, struct J9SysinfoEnvIteratorState* state,
    ...
    int32_t (*sysinfo_env_iterator_next)(
        struct OMRPortLibrary* portLibrary, struct J9SysinfoEnvIteratorState* state, J9SysinfoEnvElement* envElement);
    ...
    intptr_t (*file_blockingasync_open)(
        struct OMRPortLibrary* portLibrary, const char* path, int32_t flags, int32_t mode);
    ...
    intptr_t (*file_blockingasync_write)(
        struct OMRPortLibrary* portLibrary, intptr_t fd, const void* buf, intptr_t nbytes);
    ...
    intptr_t (*filestream_write)(
        struct OMRPortLibrary* portLibrary, OMRFileStream* fileStream, const void* buf, intptr_t nbytes);
    /** see @ref omrfilestream::omrfilestream_write_text "filestream_write_text"*/
    intptr_t (*filestream_write_text)(struct OMRPortLibrary* portLibrary, OMRFileStream* fileStream, const char* buf,
        intptr_t nbytes, int32_t toCode);
    /** see @ref omrfilestream::omrfilestream_vprintf "filestream_vprintf"*/
    void (*filestream_vprintf)(
        struct OMRPortLibrary* portLibrary, OMRFileStream* fileStream, const char* format, va_list args);
    ...
    int32_t (*filestream_setbuffer)(
        struct OMRPortLibrary* portLibrary, OMRFileStream* fileStream, char* buffer, int32_t mode, uintptr_t size);
    ...
    uintptr_t (*sl_open_shared_library)(
    ...
    uintptr_t (*sl_lookup_name)(struct OMRPortLibrary* portLibrary, uintptr_t descriptor, char* name, uintptr_t* func,
    ...
    void* (*heap_reallocate)(
        struct OMRPortLibrary* portLibrary, struct J9Heap* heap, void* address, uintptr_t byteAmount);
    ...
    void* (*mem_allocate_memory)(
        struct OMRPortLibrary* portLibrary, uintptr_t byteAmount, const char* callSite, uint32_t category);
    ...
    void* (*mem_reallocate_memory)(struct OMRPortLibrary* portLibrary, void* memoryPointer, uintptr_t byteAmount,
    ...
    void* (*vmem_commit_memory)(struct OMRPortLibrary* portLibrary, void* address, uintptr_t byteAmount,
        struct J9PortVmemIdentifier* identifier);
    /** see @ref omrvmem.c::omrvmem_decommit_memory "omrvmem_decommit_memory"*/
    intptr_t (*vmem_decommit_memory)(struct OMRPortLibrary* portLibrary, void* address, uintptr_t byteAmount,
        struct J9PortVmemIdentifier* identifier);
    /** see @ref omrvmem.c::omrvmem_free_memory "omrvmem_free_memory"*/
    int32_t (*vmem_free_memory)(struct OMRPortLibrary* portLibrary, void* userAddress, uintptr_t byteAmount,
        struct J9PortVmemIdentifier* identifier);
    ...
    void* (*vmem_reserve_memory)(struct OMRPortLibrary* portLibrary, void* address, uintptr_t byteAmount,
    ...
    void* (*vmem_get_contiguous_region_memory)(struct OMRPortLibrary* portLibrary, void* addresses[],
        uintptr_t addressesCount, uintptr_t addressSize, uintptr_t byteAmount,
        struct J9PortVmemIdentifier* oldIdentifier, struct J9PortVmemIdentifier* newIdentifier, uintptr_t mode,
    ...
    intptr_t (*vmem_find_valid_page_size)(struct OMRPortLibrary* portLibrary, uintptr_t mode, uintptr_t* pageSize,
        uintptr_t* pageFlags, BOOLEAN* isSizeSupported);
    /** see @ref omrvmem.c::omrvmem_numa_set_affinity "omrvmem_numa_set_affinity"*/
    intptr_t (*vmem_numa_set_affinity)(struct OMRPortLibrary* portLibrary, uintptr_t numaNode, void* address,
    ...
    intptr_t (*vmem_numa_get_node_details)(
        struct OMRPortLibrary* portLibrary, struct J9MemoryNodeDetail* numaNodes, uintptr_t* nodeCount);
    ...
    int32_t (*vmem_get_process_memory_size)(
    ...
    uintptr_t (*str_vprintf)(
        struct OMRPortLibrary* portLibrary, char* buf, uintptr_t bufLen, const char* format, va_list args);
    ...
    intptr_t (*str_set_token)(
        struct OMRPortLibrary* portLibrary, struct J9StringTokens* tokens, const char* key, const char* format, ...);
    /** see @ref omrstr.c::omrstr_subst_tokens "omrstr_subst_tokens"*/
    uintptr_t (*str_subst_tokens)(struct OMRPortLibrary* portLibrary, char* buf, uintptr_t bufLen, const char* format,
    ...
    intptr_t (*str_set_time_tokens)(
        struct OMRPortLibrary* portLibrary, struct J9StringTokens* tokens, int64_t timeMillis);
    /** see @ref omrstr.c::omrstr_convert "omrstr_convert"*/
    int32_t (*str_convert)(struct OMRPortLibrary* portLibrary, int32_t fromCode, int32_t toCode, const char* inBuffer,
        uintptr_t inBufferSize, char* outBuffer, uintptr_t outBufferSize);
    ...
    void (*nls_set_catalog)(struct OMRPortLibrary* portLibrary, const char** paths, const int nPaths,
        const char* baseName, const char* extension);
    /** see @ref j9nls.c::j9nls_set_locale "j9nls_set_locale" Deprecated*/
    void (*nls_set_locale)(
        struct OMRPortLibrary* portLibrary, const char* lang, const char* region, const char* variant);
    ...
    void (*nls_printf)(
        struct OMRPortLibrary* portLibrary, uintptr_t flags, uint32_t module_name, uint32_t message_num, ...);
    ...
    const char* (*nls_lookup_message)(struct OMRPortLibrary* portLibrary, uintptr_t flags, uint32_t module_name,
    ...
    int32_t (*sig_protect)(struct OMRPortLibrary* portLibrary, omrsig_protected_fn fn, void* fn_arg,
    ...
    int32_t (*sig_set_async_signal_handler)(
    ...
    int32_t (*sig_set_single_async_signal_handler)(struct OMRPortLibrary* portLibrary, omrsig_handler_fn handler,
    ...
    int32_t (*sig_register_os_handler)(
    ...
    int32_t (*sig_is_signal_ignored)(
        struct OMRPortLibrary* portLibrary, uint32_t portlibSignalFlag, BOOLEAN* isSignalIgnored);
    /** see @ref omrsignal.c::omrsig_info "omrsig_info"*/
    uint32_t (*sig_info)(struct OMRPortLibrary* portLibrary, void* info, uint32_t category, int32_t index,
    ...
    int32_t (*file_blockingasync_unlock_bytes)(
    ...
    int32_t (*file_blockingasync_lock_bytes)(
        struct OMRPortLibrary* portLibrary, intptr_t fd, int32_t lockFlags, uint64_t offset, uint64_t length);
    /** see @ref omrstr.c::omrstr_ftime "omrstr_ftime"*/
    uintptr_t (*str_ftime)(
        struct OMRPortLibrary* portLibrary, char* buf, uintptr_t bufLen, const char* format, int64_t timeMillis);
    ...
    struct J9MmapHandle* (*mmap_map_file)(struct OMRPortLibrary* portLibrary, intptr_t file, uint64_t offset,
        uintptr_t size, const char* mappingName, uint32_t flags, uint32_t category);
    ...
    intptr_t (*sysinfo_get_tmp)(
        struct OMRPortLibrary* portLibrary, char* buf, uintptr_t bufLen, BOOLEAN ignoreEnvVariable);
    ...
    int32_t (*sysinfo_cgroup_subsystem_iterator_next)(struct OMRPortLibrary* portLibrary,
    ...
    uintptr_t (*introspect_backtrace_thread)(
        struct OMRPortLibrary* portLibrary, J9PlatformThread* thread, J9Heap* heap, void* signalInfo);
    /** see @ref omrintrospect.c::omrintrospect_backtrace_symbols "omrintrospect_backtrace_symbols"*/
    uintptr_t (*introspect_backtrace_symbols)(
        struct OMRPortLibrary* portLibrary, J9PlatformThread* thread, J9Heap* heap);
    ...
    int32_t (*sock_getaddrinfo)(struct OMRPortLibrary* portLibrary, char* node, char* service, omrsock_addrinfo_t hints,
    ...
    int32_t (*sock_getaddrinfo_family)(
        struct OMRPortLibrary* portLibrary, omrsock_addrinfo_t* handle, int32_t* family, int32_t index);
    /** see @ref omrsock.c::omrsock_getaddrinfo_socktype "omrsock_getaddrinfo_socktype"*/
    int32_t (*sock_getaddrinfo_socktype)(
        struct OMRPortLibrary* portLibrary, omrsock_addrinfo_t handle, int32_t* socktype, int32_t index);
    /** see @ref omrsock.c::omrsock_getaddrinfo_protocol "omrsock_getaddrinfo_protocol"*/
    int32_t (*sock_getaddrinfo_protocol)(
        struct OMRPortLibrary* portLibrary, omrsock_addrinfo_t handle, int32_t* protocol, int32_t index);
    ...
    int32_t (*sock_socket)(
        struct OMRPortLibrary* portLibrary, omrsock_socket_t sock, int32_t family, int32_t socktype, int32_t protocol);
    ...
    int32_t (*sock_accept)(struct OMRPortLibrary* portLibrary, omrsock_socket_t serverSock,
        omrsock_sockaddr_t addrHandle, omrsock_socket_t* sockHandle);
    /** see @ref omrsock.c::omrsock_send "omrsock_send"*/
    int32_t (*sock_send)(
        struct OMRPortLibrary* portLibrary, omrsock_socket_t sock, uint8_t* buf, int32_t nbyte, int32_t flags);
    /** see @ref omrsock.c::omrsock_sendto "omrsock_sendto"*/
    int32_t (*sock_sendto)(struct OMRPortLibrary* portLibrary, omrsock_socket_t sock, uint8_t* buf, int32_t nbyte,
    ...
    int32_t (*sock_recvfrom)(struct OMRPortLibrary* portLibrary, omrsock_socket_t sock, uint8_t* buf, int32_t nbyte,
        int32_t flags, omrsock_sockaddr_t addrHandle);
    ...
    int32_t (*cuda_deviceAlloc)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, uintptr_t size, void** deviceAddressOut);
    /** see @ref omrcuda.cpp::omrcuda_deviceCanAccessPeer "omrcuda_deviceCanAccessPeer" */
    int32_t (*cuda_deviceCanAccessPeer)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, uint32_t peerDeviceId, BOOLEAN* canAccessPeerOut);
    /** see @ref omrcuda.cpp::omrcuda_deviceDisablePeerAccess "omrcuda_deviceDisablePeerAccess" */
    int32_t (*cuda_deviceDisablePeerAccess)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, uint32_t peerDeviceId);
    /** see @ref omrcuda.cpp::omrcuda_deviceEnablePeerAccess "omrcuda_deviceEnablePeerAccess" */
    int32_t (*cuda_deviceEnablePeerAccess)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, uint32_t peerDeviceId);
    ...
    int32_t (*cuda_deviceGetAttribute)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaDeviceAttribute attribute, int32_t* valueOut);
    /** see @ref omrcuda.cpp::omrcuda_deviceGetCacheConfig "omrcuda_deviceGetCacheConfig" */
    int32_t (*cuda_deviceGetCacheConfig)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaCacheConfig* configOut);
    ...
    int32_t (*cuda_deviceGetLimit)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaDeviceLimit limit, uintptr_t* valueOut);
    /** see @ref omrcuda.cpp::omrcuda_deviceGetMemInfo "omrcuda_deviceGetMemInfo" */
    int32_t (*cuda_deviceGetMemInfo)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, uintptr_t* freeOut, uintptr_t* totalOut);
    /** see @ref omrcuda.cpp::omrcuda_deviceGetName "omrcuda_deviceGetName" */
    int32_t (*cuda_deviceGetName)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, uint32_t nameSize, char* nameOut);
    /** see @ref omrcuda.cpp::omrcuda_deviceGetSharedMemConfig "omrcuda_deviceGetSharedMemConfig" */
    int32_t (*cuda_deviceGetSharedMemConfig)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaSharedMemConfig* configOut);
    ...
    int32_t (*cuda_deviceGetStreamPriorityRange)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, int32_t* leastPriorityOut, int32_t* greatestPriorityOut);
    ...
    int32_t (*cuda_deviceSetCacheConfig)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaCacheConfig config);
    /** see @ref omrcuda.cpp::omrcuda_deviceSetLimit "omrcuda_deviceSetLimit" */
    int32_t (*cuda_deviceSetLimit)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaDeviceLimit limit, uintptr_t value);
    /** see @ref omrcuda.cpp::omrcuda_deviceSetSharedMemConfig "omrcuda_deviceSetSharedMemConfig" */
    int32_t (*cuda_deviceSetSharedMemConfig)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaSharedMemConfig config);
    ...
    int32_t (*cuda_eventCreate)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, uint32_t flags, J9CudaEvent* eventOut);
    ...
    int32_t (*cuda_eventElapsedTime)(
        struct OMRPortLibrary* portLibrary, J9CudaEvent start, J9CudaEvent end, float* elapsedMillisOut);
    ...
    int32_t (*cuda_eventRecord)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaEvent event, J9CudaStream stream);
    ...
    int32_t (*cuda_funcGetAttribute)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaFunction function,
    ...
    int32_t (*cuda_funcMaxActiveBlocksPerMultiprocessor)(struct OMRPortLibrary* portLibrary, uint32_t deviceId,
        J9CudaFunction function, uint32_t blockSize, uint32_t dynamicSharedMemorySize, uint32_t flags,
        uint32_t* valueOut);
    /** see @ref omrcuda.cpp::omrcuda_funcMaxPotentialBlockSize "omrcuda_funcMaxPotentialBlockSize" */
    int32_t (*cuda_funcMaxPotentialBlockSize)(struct OMRPortLibrary* portLibrary, uint32_t deviceId,
        J9CudaFunction function, J9CudaBlockToDynamicSharedMemorySize dynamicSharedMemoryFunction, uintptr_t userData,
        uint32_t blockSizeLimit, uint32_t flags, uint32_t* minGridSizeOut, uint32_t* maxBlockSizeOut);
    /** see @ref omrcuda.cpp::omrcuda_funcSetCacheConfig "omrcuda_funcSetCacheConfig" */
    int32_t (*cuda_funcSetCacheConfig)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaFunction function, J9CudaCacheConfig config);
    /** see @ref omrcuda.cpp::omrcuda_funcSetSharedMemConfig "omrcuda_funcSetSharedMemConfig" */
    int32_t (*cuda_funcSetSharedMemConfig)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaFunction function, J9CudaSharedMemConfig config);
    ...
    int32_t (*cuda_hostAlloc)(
        struct OMRPortLibrary* portLibrary, uintptr_t size, uint32_t flags, void** hostAddressOut);
    ...
    int32_t (*cuda_launchKernel)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaFunction function,
        uint32_t gridDimX, uint32_t gridDimY, uint32_t gridDimZ, uint32_t blockDimX, uint32_t blockDimY,
        uint32_t blockDimZ, uint32_t sharedMemBytes, J9CudaStream stream, void** kernelParms);
    /** see @ref omrcuda.cpp::omrcuda_linkerAddData "omrcuda_linkerAddData" */
    int32_t (*cuda_linkerAddData)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaLinker linker,
    ...
    int32_t (*cuda_linkerComplete)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaLinker linker,
        void** cubinOut, uintptr_t* sizeOut);
    /** see @ref omrcuda.cpp::omrcuda_linkerCreate "omrcuda_linkerCreate" */
    int32_t (*cuda_linkerCreate)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaJitOptions* options, J9CudaLinker* linkerOut);
    ...
    int32_t (*cuda_memcpy2DDeviceToDevice)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* targetAddress,
        size_t targetPitch, const void* sourceAddress, size_t sourcePitch, uintptr_t width, uintptr_t height);
    /** see @ref omrcuda.cpp::omrcuda_memcpy2DDeviceToDeviceAsync "omrcuda_memcpy2DDeviceToDeviceAsync" */
    int32_t (*cuda_memcpy2DDeviceToDeviceAsync)(struct OMRPortLibrary* portLibrary, uint32_t deviceId,
        void* targetAddress, size_t targetPitch, const void* sourceAddress, size_t sourcePitch, uintptr_t width,
        uintptr_t height, J9CudaStream stream);
    /** see @ref omrcuda.cpp::omrcuda_memcpy2DDeviceToHost "omrcuda_memcpy2DDeviceToHost" */
    int32_t (*cuda_memcpy2DDeviceToHost)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* targetAddress,
        size_t targetPitch, const void* sourceAddress, size_t sourcePitch, uintptr_t width, uintptr_t height);
    /** see @ref omrcuda.cpp::omrcuda_memcpy2DDeviceToHostAsync "omrcuda_memcpy2DDeviceToHostAsync" */
    int32_t (*cuda_memcpy2DDeviceToHostAsync)(struct OMRPortLibrary* portLibrary, uint32_t deviceId,
        void* targetAddress, size_t targetPitch, const void* sourceAddress, size_t sourcePitch, uintptr_t width,
        uintptr_t height, J9CudaStream stream);
    /** see @ref omrcuda.cpp::omrcuda_memcpy2DHostToDevice "omrcuda_memcpy2DHostToDevice" */
    int32_t (*cuda_memcpy2DHostToDevice)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* targetAddress,
        size_t targetPitch, const void* sourceAddress, size_t sourcePitch, uintptr_t width, uintptr_t height);
    /** see @ref omrcuda.cpp::omrcuda_memcpy2DHostToDeviceAsync "omrcuda_memcpy2DHostToDeviceAsync" */
    int32_t (*cuda_memcpy2DHostToDeviceAsync)(struct OMRPortLibrary* portLibrary, uint32_t deviceId,
        void* targetAddress, size_t targetPitch, const void* sourceAddress, size_t sourcePitch, uintptr_t width,
        uintptr_t height, J9CudaStream stream);
    /** see @ref omrcuda.cpp::omrcuda_memcpyDeviceToDevice "omrcuda_memcpyDeviceToDevice" */
    int32_t (*cuda_memcpyDeviceToDevice)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* targetAddress,
    ...
    int32_t (*cuda_memcpyDeviceToDeviceAsync)(struct OMRPortLibrary* portLibrary, uint32_t deviceId,
        void* targetAddress, const void* sourceAddress, uintptr_t byteCount, J9CudaStream stream);
    /** see @ref omrcuda.cpp::omrcuda_memcpyDeviceToHost "omrcuda_memcpyDeviceToHost" */
    int32_t (*cuda_memcpyDeviceToHost)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* targetAddress,
        const void* sourceAddress, uintptr_t byteCount);
    /** see @ref omrcuda.cpp::omrcuda_memcpyDeviceToHostAsync "omrcuda_memcpyDeviceToHostAsync" */
    int32_t (*cuda_memcpyDeviceToHostAsync)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* targetAddress,
    ...
    int32_t (*cuda_memcpyHostToDevice)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* targetAddress,
        const void* sourceAddress, uintptr_t byteCount);
    /** see @ref omrcuda.cpp::omrcuda_memcpyHostToDeviceAsync "omrcuda_memcpyHostToDeviceAsync" */
    int32_t (*cuda_memcpyHostToDeviceAsync)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* targetAddress,
        const void* sourceAddress, uintptr_t byteCount, J9CudaStream stream);
    /** see @ref omrcuda.cpp::omrcuda_memcpyPeer "omrcuda_memcpyPeer" */
    int32_t (*cuda_memcpyPeer)(struct OMRPortLibrary* portLibrary, uint32_t targetDeviceId, void* targetAddress,
    ...
    int32_t (*cuda_memcpyPeerAsync)(struct OMRPortLibrary* portLibrary, uint32_t targetDeviceId, void* targetAddress,
    ...
    int32_t (*cuda_memset8Async)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* deviceAddress,
        uint8_t value, uintptr_t count, J9CudaStream stream);
    /** see @ref omrcuda.cpp::omrcuda_memset16Async "omrcuda_memset16Async" */
    int32_t (*cuda_memset16Async)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* deviceAddress,
        uint16_t value, uintptr_t count, J9CudaStream stream);
    /** see @ref omrcuda.cpp::omrcuda_memset32Async "omrcuda_memset32Async" */
    int32_t (*cuda_memset32Async)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* deviceAddress,
        uint32_t value, uintptr_t count, J9CudaStream stream);
    /** see @ref omrcuda.cpp::omrcuda_moduleGetFunction "omrcuda_moduleGetFunction" */
    int32_t (*cuda_moduleGetFunction)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaModule module,
        const char* name, J9CudaFunction* functionOut);
    /** see @ref omrcuda.cpp::omrcuda_moduleGetGlobal "omrcuda_moduleGetGlobal" */
    int32_t (*cuda_moduleGetGlobal)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaModule module,
        const char* name, uintptr_t* addressOut, uintptr_t* sizeOut);
    /** see @ref omrcuda.cpp::omrcuda_moduleGetSurfaceRef "omrcuda_moduleGetSurfaceRef" */
    int32_t (*cuda_moduleGetSurfaceRef)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaModule module,
        const char* name, uintptr_t* surfRefOut);
    /** see @ref omrcuda.cpp::omrcuda_moduleGetTextureRef "omrcuda_moduleGetTextureRef" */
    int32_t (*cuda_moduleGetTextureRef)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaModule module,
        const char* name, uintptr_t* texRefOut);
    /** see @ref omrcuda.cpp::omrcuda_moduleLoad "omrcuda_moduleLoad" */
    int32_t (*cuda_moduleLoad)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, const void* image,
        J9CudaJitOptions* options, J9CudaModule* moduleOut);
    ...
    int32_t (*cuda_streamAddCallback)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaStream stream,
        J9CudaStreamCallback callback, uintptr_t userData);
    ...
    int32_t (*cuda_streamCreateWithPriority)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, int32_t priority,
        uint32_t flags, J9CudaStream* streamOut);
    ...
    int32_t (*cuda_streamGetFlags)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaStream stream, uint32_t* flagsOut);
    /** see @ref omrcuda.cpp::omrcuda_streamGetPriority "omrcuda_streamGetPriority" */
    int32_t (*cuda_streamGetPriority)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaStream stream, int32_t* priorityOut);
    ...
    int32_t (*cuda_streamWaitEvent)(
        struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaStream stream, J9CudaEvent event);
#endif /* OMR_OPT_CUDA */
} OMRPortLibrary;
...
#define omrport_isFunctionOverridden(param1) \
    privateOmrPortLibrary->port_isFunctionOverridden(privateOmrPortLibrary, (param1))
...
#define omrerror_set_last_error(param1, param2) \
    privateOmrPortLibrary->error_set_last_error(privateOmrPortLibrary, (param1), (param2))
#define omrerror_set_last_error_with_message(param1, param2) \
    privateOmrPortLibrary->error_set_last_error_with_message(privateOmrPortLibrary, (param1), (param2))
#define omrerror_set_last_error_with_message_format(...) \
    privateOmrPortLibrary->error_set_last_error_with_message_format(privateOmrPortLibrary, __VA_ARGS__)
...
#define omrtime_current_time_nanos(param1) \
    privateOmrPortLibrary->time_current_time_nanos(privateOmrPortLibrary, (param1))
...
#define omrtime_hires_delta(param1, param2, param3) \
    privateOmrPortLibrary->time_hires_delta(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrsysinfo_get_groups(param1, param2) \
    privateOmrPortLibrary->sysinfo_get_groups(privateOmrPortLibrary, (param1), (param2))
...
#define omrsysinfo_get_memory_info(param1) \
    privateOmrPortLibrary->sysinfo_get_memory_info(privateOmrPortLibrary, (param1))
#define omrsysinfo_get_memory_info_with_flags(param1, param2) \
    privateOmrPortLibrary->sysinfo_get_memory_info(privateOmrPortLibrary, (param1), (param2))
#define omrsysinfo_get_processor_info(param1) \
    privateOmrPortLibrary->sysinfo_get_processor_info(privateOmrPortLibrary, (param1))
#define omrsysinfo_destroy_processor_info(param1) \
    privateOmrPortLibrary->sysinfo_destroy_processor_info(privateOmrPortLibrary, (param1))
#define omrsysinfo_get_addressable_physical_memory() \
    privateOmrPortLibrary->sysinfo_get_addressable_physical_memory(privateOmrPortLibrary)
...
#define omrsysinfo_get_env(param1, param2, param3) \
    privateOmrPortLibrary->sysinfo_get_env(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrsysinfo_get_executable_name(param1, param2) \
    privateOmrPortLibrary->sysinfo_get_executable_name(privateOmrPortLibrary, (param1), (param2))
#define omrsysinfo_get_username(param1, param2) \
    privateOmrPortLibrary->sysinfo_get_username(privateOmrPortLibrary, (param1), (param2))
#define omrsysinfo_get_groupname(param1, param2) \
    privateOmrPortLibrary->sysinfo_get_groupname(privateOmrPortLibrary, (param1), (param2))
#define omrsysinfo_get_hostname(param1, param2) \
    privateOmrPortLibrary->sysinfo_get_hostname(privateOmrPortLibrary, (param1), (param2))
#define omrsysinfo_get_load_average(param1) \
    privateOmrPortLibrary->sysinfo_get_load_average(privateOmrPortLibrary, (param1))
#define omrsysinfo_get_CPU_utilization(param1) \
    privateOmrPortLibrary->sysinfo_get_CPU_utilization(privateOmrPortLibrary, (param1))
#define omrsysinfo_limit_iterator_init(param1) \
    privateOmrPortLibrary->sysinfo_limit_iterator_init(privateOmrPortLibrary, (param1))
#define omrsysinfo_limit_iterator_hasNext(param1) \
    privateOmrPortLibrary->sysinfo_limit_iterator_hasNext(privateOmrPortLibrary, (param1))
#define omrsysinfo_limit_iterator_next(param1, param2) \
    privateOmrPortLibrary->sysinfo_limit_iterator_next(privateOmrPortLibrary, (param1), (param2))
#define omrsysinfo_env_iterator_init(param1, param2, param3) \
    privateOmrPortLibrary->sysinfo_env_iterator_init(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrsysinfo_env_iterator_hasNext(param1) \
    privateOmrPortLibrary->sysinfo_env_iterator_hasNext(privateOmrPortLibrary, (param1))
#define omrsysinfo_env_iterator_next(param1, param2) \
    privateOmrPortLibrary->sysinfo_env_iterator_next(privateOmrPortLibrary, (param1), (param2))
#define omrsysinfo_set_number_user_specified_CPUs(param1) \
    privateOmrPortLibrary->sysinfo_set_number_user_specified_CPUs(privateOmrPortLibrary, (param1))
...
#define omrfile_write(param1, param2, param3) \
    privateOmrPortLibrary->file_write(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrfile_write_text(param1, param2, param3) \
    privateOmrPortLibrary->file_write_text(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrfile_get_text_encoding(param1, param2) \
    privateOmrPortLibrary->file_get_text_encoding(privateOmrPortLibrary, (param1), (param2))
#define omrfile_vprintf(param1, param2, param3) \
    privateOmrPortLibrary->file_vprintf(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrfile_open(param1, param2, param3) \
    privateOmrPortLibrary->file_open(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrfile_seek(param1, param2, param3) \
    privateOmrPortLibrary->file_seek(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrfile_read(param1, param2, param3) \
    privateOmrPortLibrary->file_read(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrfile_chown(param1, param2, param3) \
    privateOmrPortLibrary->file_chown(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrfile_set_length(param1, param2) \
    privateOmrPortLibrary->file_set_length(privateOmrPortLibrary, (param1), (param2))
...
#define omrfile_stat(param1, param2, param3) \
    privateOmrPortLibrary->file_stat(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrfile_stat_filesystem(param1, param2, param3) \
    privateOmrPortLibrary->file_stat_filesystem(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrfile_blockingasync_open(param1, param2, param3) \
    privateOmrPortLibrary->file_blockingasync_open(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrfile_blockingasync_close(param1) \
    privateOmrPortLibrary->file_blockingasync_close(privateOmrPortLibrary, (param1))
#define omrfile_blockingasync_read(param1, param2, param3) \
    privateOmrPortLibrary->file_blockingasync_read(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrfile_blockingasync_write(param1, param2, param3) \
    privateOmrPortLibrary->file_blockingasync_write(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrfile_blockingasync_unlock_bytes(param1, param2, param3) \
    privateOmrPortLibrary->file_blockingasync_unlock_bytes(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrfile_blockingasync_lock_bytes(param1, param2, param3, param4) \
    privateOmrPortLibrary->file_blockingasync_lock_bytes(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
#define omrfile_blockingasync_set_length(param1, param2) \
    privateOmrPortLibrary->file_blockingasync_set_length(privateOmrPortLibrary, (param1), (param2))
#define omrfile_blockingasync_flength(param1) \
    privateOmrPortLibrary->file_blockingasync_flength(privateOmrPortLibrary, (param1))
...
#define omrfilestream_open(param1, param2, param3) \
    privateOmrPortLibrary->filestream_open(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrfilestream_write(param1, param2, param3) \
    privateOmrPortLibrary->filestream_write(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrfilestream_write_text(param1, param2, param3, param4) \
    privateOmrPortLibrary->filestream_write_text(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
#define omrfilestream_vprintf(param1, param2, param3) \
    privateOmrPortLibrary->filestream_vprintf(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrfilestream_setbuffer(param1, param2, param3, param4) \
    privateOmrPortLibrary->filestream_setbuffer(privateOmrPortLibrary, param1, param2, param3, param4)
#define omrfilestream_fdopen(param1, param2) \
    privateOmrPortLibrary->filestream_fdopen(privateOmrPortLibrary, param1, param2)
...
#define omrsl_close_shared_library(param1) \
    privateOmrPortLibrary->sl_close_shared_library(privateOmrPortLibrary, (param1))
#define omrsl_open_shared_library(param1, param2, param3) \
    privateOmrPortLibrary->sl_open_shared_library(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrsl_lookup_name(param1, param2, param3, param4) \
    privateOmrPortLibrary->sl_lookup_name(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
...
#define omrtty_err_vprintf(param1, param2) \
    privateOmrPortLibrary->tty_err_vprintf(privateOmrPortLibrary, (param1), (param2))
...
#define omrheap_create(param1, param2, param3) \
    privateOmrPortLibrary->heap_create(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrheap_reallocate(param1, param2, param3) \
    privateOmrPortLibrary->heap_reallocate(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrmem_allocate_memory(param1, category) \
    privateOmrPortLibrary->mem_allocate_memory(privateOmrPortLibrary, (param1), OMR_GET_CALLSITE(), (category))
...
#define omrmem_advise_and_free_memory(param1) \
    privateOmrPortLibrary->mem_advise_and_free_memory(privateOmrPortLibrary, (param1))
...
#define omrmem_allocate_memory32(param1, category) \
    privateOmrPortLibrary->mem_allocate_memory32(privateOmrPortLibrary, (param1), OMR_GET_CALLSITE(), (category))
...
#define omrcpu_flush_icache(param1, param2) \
    privateOmrPortLibrary->cpu_flush_icache(privateOmrPortLibrary, (param1), (param2))
#define omrcpu_get_cache_line_size(param1) \
    privateOmrPortLibrary->cpu_get_cache_line_size(privateOmrPortLibrary, (param1))
...
#define omrvmem_commit_memory(param1, param2, param3) \
    privateOmrPortLibrary->vmem_commit_memory(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrvmem_decommit_memory(param1, param2, param3) \
    privateOmrPortLibrary->vmem_decommit_memory(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrvmem_free_memory(param1, param2, param3) \
    privateOmrPortLibrary->vmem_free_memory(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrvmem_reserve_memory(param1, param2, param3, param4, param5, param6) \
    privateOmrPortLibrary->vmem_reserve_memory(                                \
        privateOmrPortLibrary, (param1), (param2), (param3), (param4), (param5), (param6))
#define omrvmem_reserve_memory_ex(param1, param2) \
    privateOmrPortLibrary->vmem_reserve_memory_ex(privateOmrPortLibrary, (param1), (param2))
#define omrvmem_get_contiguous_region_memory(param1, param2, param3, param4, param5, param6, param7, param8, param9) \
    privateOmrPortLibrary->vmem_get_contiguous_region_memory(privateOmrPortLibrary, (param1), (param2), (param3),    \
...
#define omrvmem_default_large_page_size_ex(param1, param2, param3) \
    privateOmrPortLibrary->vmem_default_large_page_size_ex(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrvmem_find_valid_page_size(param1, param2, param3, param4) \
    privateOmrPortLibrary->vmem_find_valid_page_size(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
#define omrvmem_numa_set_affinity(param1, param2, param3, param4) \
    privateOmrPortLibrary->vmem_numa_set_affinity(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
#define omrvmem_numa_get_node_details(param1, param2) \
    privateOmrPortLibrary->vmem_numa_get_node_details(privateOmrPortLibrary, (param1), (param2))
#define omrvmem_get_available_physical_memory(param1) \
    privateOmrPortLibrary->vmem_get_available_physical_memory(privateOmrPortLibrary, (param1))
#define omrvmem_get_process_memory_size(param1, param2) \
    privateOmrPortLibrary->vmem_get_process_memory_size(privateOmrPortLibrary, (param1), (param2))
...
#define omrstr_vprintf(param1, param2, param3, param4) \
    privateOmrPortLibrary->str_vprintf(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
...
#define omrstr_subst_tokens(param1, param2, param3, param4) \
    privateOmrPortLibrary->str_subst_tokens(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
...
#define omrstr_set_time_tokens(param1, param2) \
    privateOmrPortLibrary->str_set_time_tokens(privateOmrPortLibrary, (param1), (param2))
#define omrstr_convert(param1, param2, param3, param4, param5, param6) \
    privateOmrPortLibrary->str_convert(                                \
        privateOmrPortLibrary, (param1), (param2), (param3), (param4), (param5), (param6))
...
#define omrdump_create(param1, param2, param3) \
    privateOmrPortLibrary->dump_create(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrnls_set_catalog(param1, param2, param3, param4) \
    privateOmrPortLibrary->nls_set_catalog(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
#define omrnls_set_locale(param1, param2, param3) \
    privateOmrPortLibrary->nls_set_locale(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrnls_vprintf(param1, param2, param3) \
    privateOmrPortLibrary->nls_vprintf(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrnls_lookup_message(param1, param2, param3, param4) \
    privateOmrPortLibrary->nls_lookup_message(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
...
#define omrsig_protect(param1, param2, param3, param4, param5, param6) \
    privateOmrPortLibrary->sig_protect(                                \
        privateOmrPortLibrary, (param1), (param2), (param3), (param4), (param5), (param6))
...
#define omrsig_set_async_signal_handler(param1, param2, param3) \
    privateOmrPortLibrary->sig_set_async_signal_handler(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrsig_map_os_signal_to_portlib_signal(param1) \
    privateOmrPortLibrary->sig_map_os_signal_to_portlib_signal(privateOmrPortLibrary, (param1))
#define omrsig_map_portlib_signal_to_os_signal(param1) \
    privateOmrPortLibrary->sig_map_portlib_signal_to_os_signal(privateOmrPortLibrary, (param1))
#define omrsig_register_os_handler(param1, param2, param3) \
    privateOmrPortLibrary->sig_register_os_handler(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrsig_is_master_signal_handler(param1) \
    privateOmrPortLibrary->sig_is_master_signal_handler(privateOmrPortLibrary, (param1))
#define omrsig_is_signal_ignored(param1, param2) \
    privateOmrPortLibrary->sig_is_signal_ignored(privateOmrPortLibrary, (param1), (param2))
#define omrsig_info(param1, param2, param3, param4, param5) \
    privateOmrPortLibrary->sig_info(privateOmrPortLibrary, (param1), (param2), (param3), (param4), (param5))
#define omrsig_info_count(param1, param2) \
    privateOmrPortLibrary->sig_info_count(privateOmrPortLibrary, (param1), (param2))
...
#define omrsig_set_reporter_priority(param1) \
    privateOmrPortLibrary->sig_set_reporter_priority(privateOmrPortLibrary, (param1))
#define omrfile_read_text(param1, param2, param3) \
    privateOmrPortLibrary->file_read_text(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrfile_findfirst(param1, param2) \
    privateOmrPortLibrary->file_findfirst(privateOmrPortLibrary, (param1), (param2))
...
#define omrfile_unlock_bytes(param1, param2, param3) \
    privateOmrPortLibrary->file_unlock_bytes(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrfile_lock_bytes(param1, param2, param3, param4) \
    privateOmrPortLibrary->file_lock_bytes(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
#define omrfile_convert_native_fd_to_omrfile_fd(param1) \
    privateOmrPortLibrary->file_convert_native_fd_to_omrfile_fd(privateOmrPortLibrary, (param1))
#define omrfile_convert_omrfile_fd_to_native_fd(param1) \
    privateOmrPortLibrary->file_convert_omrfile_fd_to_native_fd(privateOmrPortLibrary, param1)
#define omrstr_ftime(param1, param2, param3, param4) \
    privateOmrPortLibrary->str_ftime(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
...
#define omrmmap_map_file(param1, param2, param3, param4, param5, param6) \
    privateOmrPortLibrary->mmap_map_file(                                \
        privateOmrPortLibrary, (param1), (param2), (param3), (param4), (param5), (param6))
...
#define omrmmap_msync(param1, param2, param3) \
    privateOmrPortLibrary->mmap_msync(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrmmap_protect(param1, param2, param3) \
    privateOmrPortLibrary->mmap_protect(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrmmap_get_region_granularity(param1) \
    privateOmrPortLibrary->mmap_get_region_granularity(privateOmrPortLibrary, (param1))
...
#define omrsysinfo_get_limit(param1, param2) \
    privateOmrPortLibrary->sysinfo_get_limit(privateOmrPortLibrary, (param1), (param2))
#define omrsysinfo_set_limit(param1, param2) \
    privateOmrPortLibrary->sysinfo_set_limit(privateOmrPortLibrary, (param1), (param2))
#define omrsysinfo_get_number_CPUs_by_type(param1) \
    privateOmrPortLibrary->sysinfo_get_number_CPUs_by_type(privateOmrPortLibrary, (param1))
#define omrsysinfo_get_cwd(param1, param2) \
    privateOmrPortLibrary->sysinfo_get_cwd(privateOmrPortLibrary, (param1), (param2))
#define omrsysinfo_get_tmp(param1, param2, param3) \
    privateOmrPortLibrary->sysinfo_get_tmp(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrsysinfo_get_open_file_count(param1) \
    privateOmrPortLibrary->sysinfo_get_open_file_count(privateOmrPortLibrary, (param1))
#define omrsysinfo_get_os_description(param1) \
    privateOmrPortLibrary->sysinfo_get_os_description(privateOmrPortLibrary, (param1))
#define omrsysinfo_os_has_feature(param1, param2) \
    privateOmrPortLibrary->sysinfo_os_has_feature(privateOmrPortLibrary, (param1), (param2))
...
#define omrsysinfo_cgroup_is_system_available() \
    privateOmrPortLibrary->sysinfo_cgroup_is_system_available(privateOmrPortLibrary)
#define omrsysinfo_cgroup_get_available_subsystems() \
    privateOmrPortLibrary->sysinfo_cgroup_get_available_subsystems(privateOmrPortLibrary)
#define omrsysinfo_cgroup_are_subsystems_available(param1) \
    privateOmrPortLibrary->sysinfo_cgroup_are_subsystems_available(privateOmrPortLibrary, param1)
#define omrsysinfo_cgroup_get_enabled_subsystems() \
    privateOmrPortLibrary->sysinfo_cgroup_get_enabled_subsystems(privateOmrPortLibrary)
#define omrsysinfo_cgroup_enable_subsystems(param1) \
    privateOmrPortLibrary->sysinfo_cgroup_enable_subsystems(privateOmrPortLibrary, param1)
#define omrsysinfo_cgroup_are_subsystems_enabled(param1) \
    privateOmrPortLibrary->sysinfo_cgroup_are_subsystems_enabled(privateOmrPortLibrary, param1)
#define omrsysinfo_cgroup_get_memlimit(param1) \
    privateOmrPortLibrary->sysinfo_cgroup_get_memlimit(privateOmrPortLibrary, param1)
...
#define omrsysinfo_get_cgroup_subsystem_list() \
    privateOmrPortLibrary->sysinfo_get_cgroup_subsystem_list(privateOmrPortLibrary)
...
#define omrsysinfo_cgroup_subsystem_iterator_init(param1, param2) \
    privateOmrPortLibrary->sysinfo_cgroup_subsystem_iterator_init(privateOmrPortLibrary, param1, param2)
#define omrsysinfo_cgroup_subsystem_iterator_hasNext(param1) \
    privateOmrPortLibrary->sysinfo_cgroup_subsystem_iterator_hasNext(privateOmrPortLibrary, param1)
#define omrsysinfo_cgroup_subsystem_iterator_metricKey(param1, param2) \
    privateOmrPortLibrary->sysinfo_cgroup_subsystem_iterator_metricKey(privateOmrPortLibrary, param1, param2)
#define omrsysinfo_cgroup_subsystem_iterator_next(param1, param2) \
    privateOmrPortLibrary->sysinfo_cgroup_subsystem_iterator_next(privateOmrPortLibrary, param1, param2)
#define omrsysinfo_cgroup_subsystem_iterator_destroy(param1) \
    privateOmrPortLibrary->sysinfo_cgroup_subsystem_iterator_destroy(privateOmrPortLibrary, param1)
...
#define omrintrospect_set_suspend_signal_offset(param1) \
    privateOmrPortLibrary->introspect_set_suspend_signal_offset(privateOmrPortLibrary, param1)
#define omrintrospect_threads_startDo(param1, param2) \
    privateOmrPortLibrary->introspect_threads_startDo(privateOmrPortLibrary, (param1), (param2))
#define omrintrospect_threads_startDo_with_signal(param1, param2, param3) \
    privateOmrPortLibrary->introspect_threads_startDo_with_signal(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrintrospect_backtrace_thread(param1, param2, param3) \
    privateOmrPortLibrary->introspect_backtrace_thread(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrintrospect_backtrace_symbols(param1, param2) \
    privateOmrPortLibrary->introspect_backtrace_symbols(privateOmrPortLibrary, (param1), (param2))
...
#define omrsock_getaddrinfo(param1, param2, param3, param4) \
    privateOmrPortLibrary->sock_getaddrinfo(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
#define omrsock_getaddrinfo_length(param1, param2) \
    privateOmrPortLibrary->sock_getaddrinfo_length(privateOmrPortLibrary, (param1), (param2))
#define omrsock_getaddrinfo_family(param1, param2, param3) \
    privateOmrPortLibrary->sock_getaddrinfo_family(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrsock_getaddrinfo_socktype(param1, param2, param3) \
    privateOmrPortLibrary->sock_getaddrinfo_socktype(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrsock_getaddrinfo_protocol(param1, param2, param3) \
    privateOmrPortLibrary->sock_getaddrinfo_protocol(privateOmrPortLibrary, (param1), (param2), (param3))
...
#define omrsock_socket(param1, param2, param3, param4) \
    privateOmrPortLibrary->sock_socket(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
...
#define omrsock_accept(param1, param2, param3) \
    privateOmrPortLibrary->sock_accept(privateOmrPortLibrary, (param1), (param2), (param3))
#define omrsock_send(param1, param2, param3, param4) \
    privateOmrPortLibrary->sock_send(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
#define omrsock_sendto(param1, param2, param3, param4, param5) \
    privateOmrPortLibrary->sock_sendto(privateOmrPortLibrary, (param1), (param2), (param3), (param4), (param5))
#define omrsock_recv(param1, param2, param3, param4) \
    privateOmrPortLibrary->sock_recv(privateOmrPortLibrary, (param1), (param2), (param3), (param4))
...
#define omrcuda_shutdown() privateOmrPortLibrary->cuda_shutdown(privateOmrPortLibrary)
...
#define omrcuda_deviceGetCount(countOut) privateOmrPortLibrary->cuda_deviceGetCount(privateOmrPortLibrary, (countOut))
...
#define omrcuda_deviceGetStreamPriorityRange(deviceId, leastPriorityOut, greatestPriorityOut) \
    privateOmrPortLibrary->cuda_deviceGetStreamPriorityRange(                                 \
        privateOmrPortLibrary, (deviceId), (leastPriorityOut), (greatestPriorityOut))
#define omrcuda_deviceReset(deviceId) privateOmrPortLibrary->cuda_deviceReset(privateOmrPortLibrary, (deviceId))
...
#define omrcuda_eventQuery(event) privateOmrPortLibrary->cuda_eventQuery(privateOmrPortLibrary, (event))
...
#define omrcuda_eventSynchronize(event) privateOmrPortLibrary->cuda_eventSynchronize(privateOmrPortLibrary, (event))
...
#define omrcuda_funcMaxActiveBlocksPerMultiprocessor(                        \
    ...
    privateOmrPortLibrary->cuda_funcMaxActiveBlocksPerMultiprocessor(        \
        privateOmrPortLibrary, (deviceId), (function), (blockSize), (dynamicSharedMemorySize), (flags), (valueOut))
#define omrcuda_funcMaxPotentialBlockSize(                                                                             \
    deviceId, function, dynamicSharedMemoryFunction, userData, blockSizeLimit, flags, minGridSizeOut, maxBlockSizeOut) \
    privateOmrPortLibrary->cuda_funcMaxPotentialBlockSize(privateOmrPortLibrary, (deviceId), (function),               \
        (dynamicSharedMemoryFunction), (userData), (blockSizeLimit), (flags), (minGridSizeOut), (maxBlockSizeOut))
...
#define omrcuda_getErrorString(error) privateOmrPortLibrary->cuda_getErrorString(privateOmrPortLibrary, (error))
...
#define omrcuda_hostFree(hostAddress) privateOmrPortLibrary->cuda_hostFree(privateOmrPortLibrary, (hostAddress))
#define omrcuda_launchKernel(deviceId, function, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ,     \
    sharedMemBytes, stream, kernelParms)                                                                            \
    privateOmrPortLibrary->cuda_launchKernel(privateOmrPortLibrary, (deviceId), (function), (gridDimX), (gridDimY), \
        (gridDimZ), (blockDimX), (blockDimY), (blockDimZ), (sharedMemBytes), (stream), (kernelParms))
#define omrcuda_linkerAddData(deviceId, linker, type, data, size, name, options) \
    privateOmrPortLibrary->cuda_linkerAddData(                                   \
        privateOmrPortLibrary, (deviceId), (linker), (type), (data), (size), (name), options)
...
#define omrcuda_memcpy2DDeviceToDevice(                                                                    \
    deviceId, targetAddress, targetPitch, sourceAddress, sourcePitch, width, height)                       \
    privateOmrPortLibrary->cuda_memcpy2DDeviceToDevice(privateOmrPortLibrary, (deviceId), (targetAddress), \
        (targetPitch), (sourceAddress), (sourcePitch), (width), (height))
#define omrcuda_memcpy2DDeviceToDeviceAsync(                                                                    \
    deviceId, targetAddress, targetPitch, sourceAddress, sourcePitch, width, height, stream)                    \
    privateOmrPortLibrary->cuda_memcpy2DDeviceToDeviceAsync(privateOmrPortLibrary, (deviceId), (targetAddress), \
        (targetPitch), (sourceAddress), (sourcePitch), (width), (height), (stream))
#define omrcuda_memcpy2DDeviceToHost(deviceId, targetAddress, targetPitch, sourceAddress, sourcePitch, width, height) \
    privateOmrPortLibrary->cuda_memcpy2DDeviceToHost(privateOmrPortLibrary, (deviceId), (targetAddress),              \
        (targetPitch), (sourceAddress), (sourcePitch), (width), (height))
#define omrcuda_memcpy2DDeviceToHostAsync(                                                                    \
    deviceId, targetAddress, targetPitch, sourceAddress, sourcePitch, width, height, stream)                  \
    privateOmrPortLibrary->cuda_memcpy2DDeviceToHostAsync(privateOmrPortLibrary, (deviceId), (targetAddress), \
        (targetPitch), (sourceAddress), (sourcePitch), (width), (height), (stream))
#define omrcuda_memcpy2DHostToDevice(deviceId, targetAddress, targetPitch, sourceAddress, sourcePitch, width, height) \
    privateOmrPortLibrary->cuda_memcpy2DHostToDevice(privateOmrPortLibrary, (deviceId), (targetAddress),              \
        (targetPitch), (sourceAddress), (sourcePitch), (width), (height))
#define omrcuda_memcpy2DHostToDeviceAsync(                                                                    \
    deviceId, targetAddress, targetPitch, sourceAddress, sourcePitch, width, height, stream)                  \
    privateOmrPortLibrary->cuda_memcpy2DHostToDeviceAsync(privateOmrPortLibrary, (deviceId), (targetAddress), \
        (targetPitch), (sourceAddress), (sourcePitch), (width), (height), (stream))
#define omrcuda_memcpyDeviceToDevice(deviceId, targetAddress, sourceAddress, byteCount) \
    privateOmrPortLibrary->cuda_memcpyDeviceToDevice(                                   \
        privateOmrPortLibrary, (deviceId), (targetAddress), (sourceAddress), (byteCount))
#define omrcuda_memcpyDeviceToDeviceAsync(deviceId, targetAddress, sourceAddress, byteCount, stream) \
    privateOmrPortLibrary->cuda_memcpyDeviceToDeviceAsync(                                           \
        privateOmrPortLibrary, (deviceId), (targetAddress), (sourceAddress), (byteCount), (stream))
#define omrcuda_memcpyDeviceToHost(deviceId, targetAddress, sourceAddress, byteCount) \
    privateOmrPortLibrary->cuda_memcpyDeviceToHost(                                   \
        privateOmrPortLibrary, (deviceId), (targetAddress), (sourceAddress), (byteCount))
...
#define omrcuda_memcpyHostToDevice(deviceId, targetAddress, sourceAddress, byteCount) \
    privateOmrPortLibrary->cuda_memcpyHostToDevice(                                   \
        privateOmrPortLibrary, (deviceId), (targetAddress), (sourceAddress), (byteCount))
...
#define omrcuda_memcpyPeer(targetDeviceId, targetAddress, sourceDeviceId, sourceAddress, byteCount) \
    privateOmrPortLibrary->cuda_memcpyPeer(                                                         \
        privateOmrPortLibrary, (targetDeviceId), (targetAddress), (sourceDeviceId), (sourceAddress), (byteCount))
#define omrcuda_memcpyPeerAsync(targetDeviceId, targetAddress, sourceDeviceId, sourceAddress, byteCount, stream) \
    privateOmrPortLibrary->cuda_memcpyPeerAsync(privateOmrPortLibrary, (targetDeviceId), (targetAddress),        \
...
#define omrcuda_memset8Async(deviceId, deviceAddress, value, count, stream) \
    privateOmrPortLibrary->cuda_memset8Async(                               \
        privateOmrPortLibrary, (deviceId), (deviceAddress), (value), (count), (stream))
...
#define omrcuda_memset16Async(deviceId, deviceAddress, value, count, stream) \
    privateOmrPortLibrary->cuda_memset16Async(                               \
        privateOmrPortLibrary, (deviceId), (deviceAddress), (value), (count), (stream))
...
#define omrcuda_memset32Async(deviceId, deviceAddress, value, count, stream) \
    privateOmrPortLibrary->cuda_memset32Async(                               \
        privateOmrPortLibrary, (deviceId), (deviceAddress), (value), (count), (stream))
...
#define omrcuda_moduleGetGlobal(deviceId, module, name, addressOut, sizeOut) \
    privateOmrPortLibrary->cuda_moduleGetGlobal(                             \
        privateOmrPortLibrary, (deviceId), (module), (name), (addressOut), (sizeOut))
