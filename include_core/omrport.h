...
#define EsOpenCreateNew                                                                                               \
  0x40 /* Use this flag with EsOpenCreate, if this flag is specified then trying to create an existing file will fail \
        */
#define EsOpenSync 0x80
#define EsOpenForMapping                                                                                               \
  0x100 /* Required for WinCE for file memory mapping, ignored on other platforms. WINCE is not supported so this flag \
           is obsolete now. */
...
typedef struct J9PortVmemParams {
   ...
   *		- this option is based on the observation that mmap would take the given address as a hint about where to place
   *the mapping
...
typedef struct J9SysinfoEnvIteratorState {
  ...
  void*
    buffer; /* caller-allocated buffer. This can be freed by the caller once they have finished using the iterator. */
...
typedef struct J9SysinfoCPUTime {
  ...
  int64_t
    cpuTime; /* cumulative CPU utilization (sum of system and user time in nanoseconds) of all CPUs on the system. */
...
typedef struct J9MemoryNodeDetail {
  ...
  J9MemoryState memoryPolicy; /**< Whether the memory on this node is preferred, allowed, or denied use by this process
                                 under the current NUMA policy */
  uintptr_t computationalResourcesAvailable; /**< The number of computational resources on this node which are currently
                                                available for use under the current NUMA policy */
} J9MemoryNodeDetail;
...
typedef struct OMRPortLibrary {
  ...
  intptr_t (*filestream_write_text)(
    struct OMRPortLibrary* portLibrary, OMRFileStream* fileStream, const char* buf, intptr_t nbytes, int32_t toCode);
  ...
  void* (*vmem_get_contiguous_region_memory)(struct OMRPortLibrary* portLibrary, void* addresses[],
    uintptr_t addressesCount, uintptr_t addressSize, uintptr_t byteAmount, struct J9PortVmemIdentifier* oldIdentifier,
  ...
  uintptr_t (*str_subst_tokens)(
    struct OMRPortLibrary* portLibrary, char* buf, uintptr_t bufLen, const char* format, struct J9StringTokens* tokens);
  ...
  int32_t (*sock_getaddrinfo)(
    struct OMRPortLibrary* portLibrary, char* node, char* service, omrsock_addrinfo_t hints, omrsock_addrinfo_t result);
  ...
  int32_t (*sock_accept)(struct OMRPortLibrary* portLibrary, omrsock_socket_t serverSock, omrsock_sockaddr_t addrHandle,
  ...
  int32_t (*cuda_funcMaxActiveBlocksPerMultiprocessor)(struct OMRPortLibrary* portLibrary, uint32_t deviceId,
    J9CudaFunction function, uint32_t blockSize, uint32_t dynamicSharedMemorySize, uint32_t flags, uint32_t* valueOut);
  ...
  int32_t (*cuda_launchKernel)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaFunction function,
    uint32_t gridDimX, uint32_t gridDimY, uint32_t gridDimZ, uint32_t blockDimX, uint32_t blockDimY, uint32_t blockDimZ,
  ...
  int32_t (*cuda_linkerComplete)(
    struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaLinker linker, void** cubinOut, uintptr_t* sizeOut);
  ...
  int32_t (*cuda_memcpy2DDeviceToHostAsync)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* targetAddress,
    size_t targetPitch, const void* sourceAddress, size_t sourcePitch, uintptr_t width, uintptr_t height,
    J9CudaStream stream);
  ...
  int32_t (*cuda_memcpy2DHostToDeviceAsync)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* targetAddress,
    size_t targetPitch, const void* sourceAddress, size_t sourcePitch, uintptr_t width, uintptr_t height,
    J9CudaStream stream);
  ...
  int32_t (*cuda_memcpyDeviceToDeviceAsync)(struct OMRPortLibrary* portLibrary, uint32_t deviceId, void* targetAddress,
  ...
  int32_t (*cuda_moduleGetTextureRef)(
    struct OMRPortLibrary* portLibrary, uint32_t deviceId, J9CudaModule module, const char* name, uintptr_t* texRefOut);
  ...
  int32_t (*cuda_streamCreateWithPriority)(
    struct OMRPortLibrary* portLibrary, uint32_t deviceId, int32_t priority, uint32_t flags, J9CudaStream* streamOut);
...
#define omrvmem_get_contiguous_region_memory(param1, param2, param3, param4, param5, param6, param7, param8, param9) \
  privateOmrPortLibrary->vmem_get_contiguous_region_memory(                                                          \
    privateOmrPortLibrary, (param1), (param2), (param3), (param4), (param5), (param6), (param7), (param8), (param9))
...
#define omrstr_convert(param1, param2, param3, param4, param5, param6) \
  privateOmrPortLibrary->str_convert(privateOmrPortLibrary, (param1), (param2), (param3), (param4), (param5), (param6))
...
#define omrsig_protect(param1, param2, param3, param4, param5, param6) \
  privateOmrPortLibrary->sig_protect(privateOmrPortLibrary, (param1), (param2), (param3), (param4), (param5), (param6))
...
#define omrcuda_memcpy2DDeviceToHost(deviceId, targetAddress, targetPitch, sourceAddress, sourcePitch, width, height) \
  privateOmrPortLibrary->cuda_memcpy2DDeviceToHost(privateOmrPortLibrary, (deviceId), (targetAddress), (targetPitch), \
...
#define omrcuda_memcpy2DHostToDevice(deviceId, targetAddress, targetPitch, sourceAddress, sourcePitch, width, height) \
  privateOmrPortLibrary->cuda_memcpy2DHostToDevice(privateOmrPortLibrary, (deviceId), (targetAddress), (targetPitch), \
    (sourceAddress), (sourcePitch), (width), (height))
