...
typedef struct J9AVLTree {
  intptr_t (*insertionComparator)(struct J9AVLTree *tree,
                                  struct J9AVLTreeNode *insertNode,
                                  struct J9AVLTreeNode *walkNode);
  intptr_t (*searchComparator)(struct J9AVLTree *tree, uintptr_t searchValue,
                               struct J9AVLTreeNode *node);
  void (*genericActionHook)(struct J9AVLTree *tree, struct J9AVLTreeNode *node,
                            uintptr_t action);
  uintptr_t (*performNodeAction)(struct J9AVLTree *tree,
                                 struct J9AVLTreeNode *node, uintptr_t action,
                                 void *userData);
  uintptr_t flags;
...
/* NOTE: JIT HashTable (and walk state) likely need to be pushed into the JIT
 * side of jvm/jit compilation */
...
#define J9JITHASHTABLE_LEFTCHILD(base)                                         \
  AVL_SRP_GETNODE((base)->parentAVLTreeNode.leftChild)
#define J9JITHASHTABLE_RIGHTCHILD(base)                                        \
  AVL_SRP_GETNODE((base)->parentAVLTreeNode.rightChild)
...
#define J9MEMAVLTREENODE_LEFTCHILD(base)                                       \
  AVL_SRP_GETNODE((base)->parentAVLTreeNode.leftChild)
#define J9MEMAVLTREENODE_RIGHTCHILD(base)                                      \
  AVL_SRP_GETNODE((base)->parentAVLTreeNode.rightChild)
...
/* MEMORY_TYPE_VIRTUAL is expected to be used along with other types like
 * MEMORY_TYPE_JIT_SCRATCH_SPACE or MEMORY_TYPE_JIT_PERSISTENT to allocate
 * virtual memory instead of malloc'ed memory.
 */
#define MEMORY_TYPE_VIRTUAL 0x400
/* MEMORY_TYPE_FIXED_RAM_CLASS is virtually allocated, setting
 * MEMORY_TYPE_VIRTUAL is not required. */
...
/* MEMORY_TYPE_CODE is used for virtually allocated JIT code segments, setting
 * MEMORY_TYPE_VIRTUAL is not required. */
...
#define J9MEMORYSEGMENT_LEFTCHILD(base)                                        \
  AVL_SRP_GETNODE((base)->parentAVLTreeNode.leftChild)
#define J9MEMORYSEGMENT_RIGHTCHILD(base)                                       \
  AVL_SRP_GETNODE((base)->parentAVLTreeNode.rightChild)
