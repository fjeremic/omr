 ...
 * @return uint32_t
 */
uint32_t decodeUTF8CharN(const uint8_t* input, uint16_t* result, uintptr_t bytesRemaining);
 ...
 * @return uint32_t
 */
uint32_t encodeUTF8CharN(uintptr_t unicode, uint8_t* result, uint32_t bytesRemaining);
...
uintptr_t escapeXMLString(
  OMRPortLibrary* portLibrary, char* outBuf, uintptr_t outBufLen, const char* string, uintptr_t stringLen);
...
/**
 * @brief Load the version of dbghelp.dll that shipped with the JRE. If we can't find the shipped version, try to find
 * it somewhere else.
...
#define POOL_FOR_PORT_PUDDLE32(portLib) \
  (omrmemAlloc_fptr_t) pool_portLibAlloc32, (omrmemFree_fptr_t)pool_portLibFree32, (portLib)
#else /* defined(OMR_ENV_DATA64) */
#define POOL_FOR_PORT_PUDDLE32(portLib) NULL, NULL, NULL
#endif /* defined(OMR_ENV_DATA64) */

void* pool_portLibAlloc(OMRPortLibrary* portLibrary, uint32_t size, const char* callSite, uint32_t memoryCategory,
...
void* pool_portLibAlloc32(OMRPortLibrary* portLibrary, uint32_t size, const char* callSite, uint32_t memoryCategory,
  uint32_t type, uint32_t* doInit);
 ...
 */
void setOMRVMThreadNameWithFlag(
  struct OMR_VMThread* currentThread, struct OMR_VMThread* vmThread, char* name, uint8_t nameIsStatic);
 ...
 */
intptr_t createThreadWithCategory(omrthread_t* handle, uintptr_t stacksize, uintptr_t priority, uintptr_t suspend,
