...
#define getVMThreadRawState dbgGetVMThreadRawState
   ...
   * @retval J9THREAD_ERR|J9THREAD_ERR_OS_ERRNO_SET The O/S join function
   * returned an error code, and an os_errno is also available.
   */
  ...
  /**
   * @brief initializes jlm for capturing data needed by the adaptive spin
   * options
   * @param thread
...
#define J9THREAD_LIB_CONTROL_USE_REALTIME_SCHEDULING_DISABLED ((uintptr_t)0)
   ...
   *
   * Since WIN32 has no fork, this code is currently not included. For WIN32,
   * some of the reset code is incomplete: in postForkResetThreads, the
   ...
   * reset. This function is to be called directly after a fork, before any
   * other omrthread functions, and requires an attached thread.
   */
  void omrthread_lib_postForkParent(void);

  /**
   * Perform pre-fork locking of mutexes that must be held over a fork and
   * reset. This function is to be called directly after a fork, before any
   * other omrthread functions, and requires an attached thread.
   */
   ...
   * @return intptr_t
   */
  intptr_t omrthread_monitor_try_enter_using_threadId(
    omrthread_monitor_t monitor,
    omrthread_t threadId);
   ...
   */
  intptr_t omrthread_attr_set_category(omrthread_attr_t* attr,
                                       uint32_t category);
