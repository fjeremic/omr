 ...
 * @return uintptr_t
 */
uintptr_t omrthread_get_flags(omrthread_t thread, omrthread_monitor_t *blocker);
 ...
 * @return intptr_t
 */
intptr_t omrthread_rwmutex_init(omrthread_rwmutex_t *handle, uintptr_t flags,
                                const char *name);
 ...
 * - The target thread must have been created with
 * detachstate=J9THREAD_CREATE_JOINABLE.
 *
 ...
 * @retval J9THREAD_ERR|J9THREAD_ERR_OS_ERRNO_SET The O/S join function returned
 * an error code, and an os_errno is also available.
 */
 ...
 *
 * If a NULL attr is passed, the thread will be classified as a
 * J9THREAD_CATEGORY_SYSTEM_THREAD thread. If a non NULL attr is passed, only
 * the thread category will be used, all other attributes will be ignored.
 *
 ...
 * @return J9ThreadMonitorTracing*
 */
J9ThreadMonitorTracing *omrthread_jlm_get_gc_lock_tracing(void);
#endif /* OMR_THR_JLM */
 ...
 * @return intptr_t
 */
intptr_t omrthread_jlm_init(uintptr_t flags);
#endif /* OMR_THR_JLM */
 ...
 * @param thread
 * @param adaptiveFlags flags indicating the adaptive modes that have been
 * enabled
 * @return intptr_t
 */
intptr_t jlm_adaptive_spin_init(void);
#endif
...
#define J9THREAD_LIB_CONTROL_USE_REALTIME_SCHEDULING_ENABLED \
  ((uintptr_t)J9THREAD_LIB_FLAG_REALTIME_SCHEDULING_ENABLED)
...
/**
 * Post-fork reset function used to clean up and reset old omrthread and
 * omrthread_monitor structures associated with non-existent threads in the
 * child process. This function is to be called directly before fork and
 * requires an attached thread.
 *
 * Since WIN32 has no fork, this code is currently not included. For WIN32, some
 * of the reset code is incomplete: in postForkResetThreads, the
 * omrthread_t->handle must be reset differently.
 *
 * @note This function must only be called in the child process immediately
 * following a fork().
 */
void omrthread_lib_postForkChild(void);

/**
 * Perform post-fork unlocking of mutexes that must be held over a fork and
 * reset. This function is to be called directly after a fork, before any other
 * omrthread functions, and requires an attached thread.
 */
void omrthread_lib_postForkParent(void);

/**
 * Perform pre-fork locking of mutexes that must be held over a fork and reset.
 * This function is to be called directly after a fork, before any other
 * omrthread functions, and requires an attached thread.
 */
 ...
 * @return intptr_t
 */
intptr_t omrthread_tls_alloc_with_finalizer(
    omrthread_tls_key_t *handle, omrthread_tls_finalizer_t finalizer);
 ...
 * @return intptr_t
 */
intptr_t omrthread_tls_set(omrthread_t thread, omrthread_tls_key_t key,
                           void *value);
 ...
 * @return intptr_t
 */
intptr_t omrthread_attr_set_schedpolicy(omrthread_attr_t *attr,
 ...
 * @return intptr_t
 */
intptr_t omrthread_attr_set_priority(omrthread_attr_t *attr,
 ...
 */
intptr_t omrthread_attr_set_detachstate(omrthread_attr_t *attr,
 ...
 * @return intptr_t
 */
intptr_t omrthread_attr_set_stacksize(omrthread_attr_t *attr,
                                      uintptr_t stacksize);
 ...
 * @return const char *
 */
const char *omrthread_get_errordesc(intptr_t err);

/**
 * @brief
 * @return omrthread_os_errno_t
 */
omrthread_os_errno_t omrthread_get_os_errno(void);
 ...
 * @return omrthread_os_errno_t
 */
omrthread_os_errno_t omrthread_get_os_errno2(void);
#endif /* J9ZOS390 */
...
/* in case affinity is not supported (or at least can't be queried) on the given
 * platform */
...
intptr_t omrthread_numa_set_node_affinity(omrthread_t thread,
                                          const uintptr_t *numaNodes,
                                          uintptr_t nodeCount, uint32_t flags);
 ...
 * @return intptr_t
 */
intptr_t omrthread_numa_get_node_affinity(omrthread_t thread,
                                          uintptr_t *numaNodes,
                                          uintptr_t *nodeCount);
 ...
 * @return uintptr_t
 */
uintptr_t omrthread_numa_get_current_node();
