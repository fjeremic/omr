...
#define OMRVM_REFERENCE_SHIFT(omrVM) OMR_LOG_POINTER_SIZE
  ...
  typedef struct OMR_VMThread
  {
    ...
    uint8_t* threadName;
    BOOLEAN threadNameIsStatic; /**< threadName is managed externally; Don't
                                   free it. */
    omrthread_monitor_t
...
#define OMRVMTHREAD_REFERENCE_SHIFT(omrVMThread) OMR_LOG_POINTER_SIZE
   ...
   * Not currently used by JVM.
   *
   * @param[in,out] vmThread The thread to detach. If the thread is already
   * dead, it won't be the current thread. It will be freed.
   * @return an OMR error code
   */
   ...
   * Re-attaches must be paired with an equal number of
   * omr_vmthread_redetach()es.
   *
   * @param[in,out] currentThread The current OMR_VMThread.
   * @param[in] threadName A new name for the thread.
   */
  void omr_vmthread_reattach(OMR_VMThread* currentThread,
   ...
   * Re-detaches must be paired with an equal number of
   * omr_vmthread_reattach()es.
   *
   * @param[in,out] omrVMThread An OMR_VMThread. It must have been re-attached
   * at least once.
   */
   ...
   * reset the OMR_VM, including cleaning up the _vmThreadList of threads that
   * do not exist after a fork.
   *
   * @param[in] vm The OMR vm.
   */
   ...
   * _vmThreadListMutex to ensure that the _vmThreadList is in a consistent
   * state during a fork.
   *
   * @param[in] vm The OMR vm.
   */
   ...
   * @return an OMR error code
   */
  omr_error_t OMR_Glue_BindCurrentThread(OMR_VM* omrVM,
                                         const char* threadName,
